// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n107_, new_n108_, new_n109_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x17), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x02), .O(new_n49_));
  inv1   g08(.a(x03), .O(new_n50_));
  inv1   g09(.a(x05), .O(new_n51_));
  inv1   g10(.a(x06), .O(new_n52_));
  inv1   g11(.a(x07), .O(new_n53_));
  xnor2a g12(.a(x09), .b(x08), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(x04), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n57_));
  nor2   g16(.a(x17), .b(new_n42_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(new_n57_), .c(new_n44_), .O(z01));
  nor2   g18(.a(x03), .b(x02), .O(new_n60_));
  nor2   g19(.a(x05), .b(x04), .O(new_n61_));
  nor2   g20(.a(x07), .b(x06), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n48_), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(x00), .O(new_n64_));
  nor2   g23(.a(x02), .b(x01), .O(new_n65_));
  nor2   g24(.a(x04), .b(x03), .O(new_n66_));
  nor2   g25(.a(x06), .b(x05), .O(new_n67_));
  inv1   g26(.a(x09), .O(new_n68_));
  nand3  g27(.a(new_n68_), .b(x08), .c(new_n53_), .O(new_n69_));
  inv1   g28(.a(new_n69_), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x19), .c(new_n43_), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z02));
  inv1   g33(.a(x20), .O(new_n75_));
  nor2   g34(.a(new_n45_), .b(new_n75_), .O(z03));
  inv1   g35(.a(x21), .O(new_n77_));
  nand3  g36(.a(new_n46_), .b(new_n77_), .c(new_n75_), .O(z04));
  inv1   g37(.a(x10), .O(new_n79_));
  nand3  g38(.a(new_n65_), .b(new_n61_), .c(new_n50_), .O(new_n80_));
  inv1   g39(.a(x08), .O(new_n81_));
  nand4  g40(.a(new_n62_), .b(x19), .c(new_n43_), .d(new_n81_), .O(new_n82_));
  oai22  g41(.a(new_n82_), .b(new_n80_), .c(new_n45_), .d(new_n79_), .O(z05));
  oai21  g42(.a(new_n68_), .b(new_n81_), .c(x17), .O(new_n84_));
  nand4  g43(.a(new_n84_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n85_));
  nor2   g44(.a(new_n85_), .b(x04), .O(new_n86_));
  nand4  g45(.a(new_n86_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n87_));
  nor3   g46(.a(new_n63_), .b(x17), .c(new_n42_), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n44_), .O(z06));
  nand3  g48(.a(new_n46_), .b(x24), .c(x18), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z07));
  inv1   g50(.a(x11), .O(new_n92_));
  nand2  g51(.a(new_n46_), .b(new_n92_), .O(z08));
  nand3  g52(.a(new_n46_), .b(x24), .c(x11), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z09));
  inv1   g54(.a(x24), .O(new_n96_));
  nand4  g55(.a(new_n46_), .b(new_n96_), .c(x22), .d(x14), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z10));
  nand4  g57(.a(new_n96_), .b(x22), .c(new_n44_), .d(x17), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z11));
  nand3  g59(.a(new_n96_), .b(x23), .c(x14), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n46_), .O(z12));
  nand4  g61(.a(new_n96_), .b(x23), .c(new_n44_), .d(x17), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z13));
  nand2  g63(.a(new_n96_), .b(x16), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n46_), .O(z14));
  inv1   g65(.a(x12), .O(new_n107_));
  inv1   g66(.a(x13), .O(new_n108_));
  nor2   g67(.a(x15), .b(x14), .O(new_n109_));
  nand4  g68(.a(new_n109_), .b(new_n46_), .c(new_n108_), .d(new_n107_), .O(z15));
endmodule


