// Benchmark "FAU" written by ABC on Wed Aug 19 17:35:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n107_, new_n109_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nand2  g02(.a(x21), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x01), .O(new_n46_));
  inv1   g05(.a(x02), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x05), .O(new_n49_));
  inv1   g08(.a(x06), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  xnor2a g10(.a(x09), .b(x08), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x04), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x00), .O(new_n56_));
  nand3  g15(.a(new_n56_), .b(new_n44_), .c(x19), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(z01));
  nand3  g17(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n59_));
  inv1   g18(.a(x04), .O(new_n60_));
  nor2   g19(.a(x07), .b(x06), .O(new_n61_));
  nand3  g20(.a(new_n61_), .b(new_n49_), .c(new_n60_), .O(new_n62_));
  oai21  g21(.a(new_n62_), .b(new_n59_), .c(x00), .O(new_n63_));
  nand4  g22(.a(new_n60_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(new_n65_));
  inv1   g24(.a(x08), .O(new_n66_));
  nand2  g25(.a(new_n50_), .b(new_n49_), .O(new_n67_));
  nor4   g26(.a(new_n67_), .b(x09), .c(new_n66_), .d(x07), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x19), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n44_), .O(z02));
  inv1   g31(.a(x20), .O(new_n73_));
  nand2  g32(.a(new_n44_), .b(new_n73_), .O(z03));
  nand2  g33(.a(x21), .b(x15), .O(new_n75_));
  oai21  g34(.a(x21), .b(new_n73_), .c(new_n75_), .O(z04));
  inv1   g35(.a(new_n44_), .O(new_n77_));
  nand3  g36(.a(x19), .b(new_n66_), .c(new_n51_), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n65_), .c(x10), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n77_), .O(z05));
  inv1   g40(.a(x09), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x08), .O(new_n83_));
  nand4  g42(.a(new_n83_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n84_));
  nor2   g43(.a(new_n84_), .b(x04), .O(new_n85_));
  nand4  g44(.a(new_n85_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(x00), .c(new_n77_), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n70_), .c(x19), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n44_), .O(z06));
  inv1   g48(.a(x18), .O(new_n90_));
  inv1   g49(.a(x24), .O(new_n91_));
  nor3   g50(.a(new_n77_), .b(new_n91_), .c(new_n90_), .O(z07));
  inv1   g51(.a(x11), .O(new_n93_));
  nor2   g52(.a(new_n77_), .b(new_n93_), .O(z08));
  oai21  g53(.a(new_n91_), .b(new_n93_), .c(new_n44_), .O(z09));
  nand4  g54(.a(new_n44_), .b(new_n91_), .c(x22), .d(x14), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z10));
  nand3  g56(.a(new_n91_), .b(x22), .c(x17), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n44_), .O(z11));
  inv1   g58(.a(x23), .O(new_n100_));
  nand2  g59(.a(new_n44_), .b(new_n91_), .O(new_n101_));
  nor2   g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x14), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z12));
  nand2  g63(.a(new_n102_), .b(x17), .O(new_n105_));
  inv1   g64(.a(new_n105_), .O(z13));
  inv1   g65(.a(x16), .O(new_n107_));
  nor2   g66(.a(new_n101_), .b(new_n107_), .O(z14));
  nor3   g67(.a(x14), .b(x13), .c(x12), .O(new_n109_));
  oai21  g68(.a(new_n109_), .b(x21), .c(new_n43_), .O(z15));
endmodule


