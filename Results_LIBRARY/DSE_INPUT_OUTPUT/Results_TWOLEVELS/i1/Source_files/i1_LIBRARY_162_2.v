// Benchmark "FAU" written by ABC on Wed Aug 19 17:35:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n69_, new_n70_, new_n72_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n83_, new_n84_, new_n86_, new_n88_,
    new_n91_, new_n92_;
  nor2   g00(.a(x02), .b(x01), .O(new_n42_));
  nor2   g01(.a(x04), .b(x03), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  inv1   g03(.a(x07), .O(new_n45_));
  nor2   g04(.a(x06), .b(x05), .O(new_n46_));
  nand4  g05(.a(new_n46_), .b(x09), .c(x08), .d(new_n45_), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(new_n44_), .c(x19), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(x00), .O(new_n49_));
  inv1   g08(.a(x08), .O(new_n50_));
  inv1   g09(.a(x09), .O(new_n51_));
  nand4  g10(.a(new_n46_), .b(new_n51_), .c(new_n50_), .d(new_n45_), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(new_n44_), .c(x00), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x19), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n49_), .O(z01));
  nand3  g14(.a(new_n46_), .b(x19), .c(new_n45_), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n44_), .c(x00), .O(new_n57_));
  nor3   g16(.a(x05), .b(x04), .c(x03), .O(new_n58_));
  nor2   g17(.a(x07), .b(x06), .O(new_n59_));
  nand3  g18(.a(x19), .b(new_n51_), .c(x08), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(new_n59_), .c(new_n58_), .d(new_n42_), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n57_), .O(z02));
  inv1   g22(.a(x20), .O(new_n64_));
  inv1   g23(.a(x19), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(x00), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n64_), .O(z03));
  or2    g26(.a(z03), .b(x21), .O(z04));
  nand2  g27(.a(new_n66_), .b(x10), .O(new_n69_));
  nand4  g28(.a(new_n46_), .b(x19), .c(new_n50_), .d(new_n45_), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(new_n44_), .c(new_n69_), .O(z05));
  inv1   g30(.a(x00), .O(new_n72_));
  nand2  g31(.a(new_n65_), .b(new_n72_), .O(z06));
  nand2  g32(.a(x24), .b(x18), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n66_), .O(z07));
  inv1   g34(.a(x11), .O(new_n76_));
  inv1   g35(.a(new_n66_), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n76_), .O(z08));
  inv1   g37(.a(x24), .O(new_n79_));
  nor3   g38(.a(new_n77_), .b(new_n79_), .c(new_n76_), .O(z09));
  nand3  g39(.a(new_n79_), .b(x22), .c(x14), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n66_), .O(z10));
  nor2   g41(.a(new_n77_), .b(x24), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(x22), .c(x17), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z11));
  nand3  g44(.a(new_n83_), .b(x23), .c(x14), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z12));
  nand3  g46(.a(new_n79_), .b(x23), .c(x17), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n66_), .O(z13));
  and2   g48(.a(new_n83_), .b(x16), .O(z14));
  nor2   g49(.a(x13), .b(x12), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n77_), .O(z15));
  buf    g52(.a(x00), .O(z00));
endmodule


