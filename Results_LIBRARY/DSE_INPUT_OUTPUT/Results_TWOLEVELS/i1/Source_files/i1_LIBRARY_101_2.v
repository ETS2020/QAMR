// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x07), .O(new_n43_));
  nor2   g02(.a(x08), .b(new_n43_), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x19), .O(new_n46_));
  inv1   g05(.a(new_n44_), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  inv1   g07(.a(x01), .O(new_n49_));
  inv1   g08(.a(x02), .O(new_n50_));
  inv1   g09(.a(x03), .O(new_n51_));
  inv1   g10(.a(x05), .O(new_n52_));
  inv1   g11(.a(x06), .O(new_n53_));
  xnor2a g12(.a(x09), .b(x08), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n43_), .c(new_n53_), .d(new_n52_), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(x04), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n48_), .c(new_n46_), .O(z01));
  inv1   g17(.a(x08), .O(new_n59_));
  aoi21  g18(.a(x19), .b(x00), .c(new_n59_), .O(new_n60_));
  nand3  g19(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n61_));
  inv1   g20(.a(x04), .O(new_n62_));
  nand3  g21(.a(new_n53_), .b(new_n52_), .c(new_n62_), .O(new_n63_));
  oai21  g22(.a(new_n63_), .b(new_n61_), .c(x00), .O(new_n64_));
  nand4  g23(.a(new_n62_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n65_));
  inv1   g24(.a(x09), .O(new_n66_));
  nor2   g25(.a(x06), .b(x05), .O(new_n67_));
  nand3  g26(.a(new_n67_), .b(new_n66_), .c(x08), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n65_), .c(new_n64_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(x19), .c(new_n43_), .O(new_n70_));
  oai21  g29(.a(new_n60_), .b(new_n43_), .c(new_n70_), .O(z02));
  inv1   g30(.a(x20), .O(new_n72_));
  nor2   g31(.a(new_n44_), .b(new_n72_), .O(z03));
  inv1   g32(.a(x21), .O(new_n74_));
  nand3  g33(.a(new_n47_), .b(new_n74_), .c(new_n72_), .O(z04));
  inv1   g34(.a(x10), .O(new_n76_));
  nand3  g35(.a(new_n67_), .b(x19), .c(new_n59_), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(new_n65_), .c(new_n76_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n43_), .O(new_n79_));
  oai21  g38(.a(new_n76_), .b(new_n59_), .c(new_n79_), .O(z05));
  nor3   g39(.a(x06), .b(x05), .c(x04), .O(new_n81_));
  nand4  g40(.a(new_n81_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  nand3  g42(.a(x08), .b(x07), .c(x00), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(new_n85_));
  aoi21  g44(.a(new_n83_), .b(new_n43_), .c(new_n85_), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n48_), .c(new_n46_), .O(z06));
  nand2  g46(.a(x24), .b(x18), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n47_), .O(z07));
  inv1   g48(.a(x11), .O(new_n90_));
  nor2   g49(.a(new_n44_), .b(new_n90_), .O(z08));
  inv1   g50(.a(x24), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n90_), .c(new_n47_), .O(z09));
  nor2   g52(.a(new_n44_), .b(x24), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(x22), .c(x14), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z10));
  nand3  g55(.a(new_n92_), .b(x22), .c(x17), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n47_), .O(z11));
  nand3  g57(.a(new_n94_), .b(x23), .c(x14), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z12));
  nand3  g59(.a(new_n92_), .b(x23), .c(x17), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n47_), .O(z13));
  and2   g61(.a(new_n94_), .b(x16), .O(z14));
  nor2   g62(.a(x15), .b(x14), .O(new_n104_));
  nor3   g63(.a(new_n44_), .b(x13), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(z15));
endmodule


