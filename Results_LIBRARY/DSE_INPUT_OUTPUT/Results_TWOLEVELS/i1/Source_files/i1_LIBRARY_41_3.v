// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n73_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n85_, new_n87_,
    new_n89_, new_n90_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  inv1   g02(.a(x01), .O(new_n44_));
  inv1   g03(.a(x02), .O(new_n45_));
  inv1   g04(.a(x03), .O(new_n46_));
  inv1   g05(.a(x04), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nor2   g08(.a(x06), .b(x05), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  inv1   g10(.a(x09), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x08), .c(new_n51_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(new_n50_), .c(new_n49_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(new_n42_), .c(new_n43_), .O(z02));
  inv1   g15(.a(x20), .O(new_n57_));
  nor2   g16(.a(new_n43_), .b(new_n42_), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n57_), .O(z03));
  nor2   g19(.a(x21), .b(x20), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n58_), .O(z04));
  inv1   g21(.a(x10), .O(new_n63_));
  nor2   g22(.a(x02), .b(x01), .O(new_n64_));
  nor2   g23(.a(x04), .b(x03), .O(new_n65_));
  nor2   g24(.a(x08), .b(x07), .O(new_n66_));
  nand4  g25(.a(new_n66_), .b(new_n50_), .c(new_n65_), .d(new_n64_), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n42_), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(x19), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n63_), .O(z05));
  nand2  g29(.a(x24), .b(x18), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n59_), .O(z07));
  inv1   g31(.a(x11), .O(new_n73_));
  nand2  g32(.a(new_n59_), .b(new_n73_), .O(z08));
  nand3  g33(.a(new_n59_), .b(x24), .c(x11), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z09));
  inv1   g35(.a(x24), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x22), .c(x14), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n59_), .O(z10));
  nand4  g38(.a(new_n59_), .b(new_n77_), .c(x22), .d(x17), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z11));
  inv1   g40(.a(x14), .O(new_n82_));
  nand2  g41(.a(new_n77_), .b(x23), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n82_), .c(new_n59_), .O(z12));
  inv1   g43(.a(x17), .O(new_n85_));
  oai21  g44(.a(new_n83_), .b(new_n85_), .c(new_n59_), .O(z13));
  nand3  g45(.a(new_n59_), .b(new_n77_), .c(x16), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z14));
  nor2   g47(.a(x13), .b(x12), .O(new_n89_));
  nor2   g48(.a(x15), .b(x14), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n58_), .O(z15));
  buf    g50(.a(x00), .O(z00));
  buf    g51(.a(x19), .O(z01));
  buf    g52(.a(x19), .O(z06));
endmodule


