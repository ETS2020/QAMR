// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n89_, new_n90_, new_n91_, new_n93_, new_n95_,
    new_n97_, new_n99_, new_n101_, new_n102_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x19), .b(new_n42_), .O(z00));
  inv1   g02(.a(x19), .O(new_n44_));
  inv1   g03(.a(x01), .O(new_n45_));
  inv1   g04(.a(x02), .O(new_n46_));
  inv1   g05(.a(x03), .O(new_n47_));
  inv1   g06(.a(x05), .O(new_n48_));
  inv1   g07(.a(x06), .O(new_n49_));
  inv1   g08(.a(x07), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x04), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x00), .c(new_n44_), .O(z01));
  nor2   g14(.a(x03), .b(x02), .O(new_n56_));
  nor2   g15(.a(x05), .b(x04), .O(new_n57_));
  nor2   g16(.a(x07), .b(x06), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n45_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x00), .O(new_n60_));
  inv1   g19(.a(x04), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(new_n63_));
  nor2   g22(.a(x06), .b(x05), .O(new_n64_));
  inv1   g23(.a(x08), .O(new_n65_));
  nor2   g24(.a(x09), .b(new_n65_), .O(new_n66_));
  nand4  g25(.a(new_n66_), .b(new_n64_), .c(new_n63_), .d(new_n50_), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(new_n60_), .c(new_n44_), .O(z02));
  inv1   g27(.a(x20), .O(new_n69_));
  nor2   g28(.a(x19), .b(x00), .O(new_n70_));
  nor2   g29(.a(new_n70_), .b(new_n69_), .O(z03));
  inv1   g30(.a(x21), .O(new_n72_));
  inv1   g31(.a(new_n70_), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(new_n72_), .c(new_n69_), .O(z04));
  inv1   g33(.a(x10), .O(new_n75_));
  nand4  g34(.a(new_n64_), .b(x19), .c(new_n65_), .d(new_n50_), .O(new_n76_));
  oai22  g35(.a(new_n76_), .b(new_n62_), .c(new_n70_), .d(new_n75_), .O(z05));
  nor3   g36(.a(x07), .b(x06), .c(x05), .O(new_n78_));
  nand4  g37(.a(new_n78_), .b(new_n61_), .c(new_n47_), .d(new_n46_), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(x01), .c(new_n60_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x19), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x00), .O(z06));
  inv1   g41(.a(x18), .O(new_n83_));
  nand2  g42(.a(new_n73_), .b(x24), .O(new_n84_));
  nor2   g43(.a(new_n84_), .b(new_n83_), .O(z07));
  inv1   g44(.a(x11), .O(new_n86_));
  nand2  g45(.a(new_n73_), .b(new_n86_), .O(z08));
  nor2   g46(.a(new_n84_), .b(new_n86_), .O(z09));
  inv1   g47(.a(x14), .O(new_n89_));
  inv1   g48(.a(x24), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x22), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n89_), .c(new_n73_), .O(z10));
  inv1   g51(.a(x17), .O(new_n93_));
  oai21  g52(.a(new_n91_), .b(new_n93_), .c(new_n73_), .O(z11));
  nand4  g53(.a(new_n73_), .b(new_n90_), .c(x23), .d(x14), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z12));
  nand3  g55(.a(new_n90_), .b(x23), .c(x17), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n73_), .O(z13));
  nand2  g57(.a(new_n90_), .b(x16), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n73_), .O(z14));
  nor2   g59(.a(x13), .b(x12), .O(new_n101_));
  nor2   g60(.a(x15), .b(x14), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n70_), .O(z15));
endmodule


