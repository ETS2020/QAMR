// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  nor2   g00(.a(x12), .b(x07), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  nand2  g02(.a(x03), .b(x02), .O(new_n22_));
  nand3  g03(.a(x13), .b(x09), .c(x04), .O(new_n23_));
  oai21  g04(.a(new_n23_), .b(new_n22_), .c(x05), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  inv1   g06(.a(x02), .O(new_n26_));
  inv1   g07(.a(x04), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n26_), .c(x08), .O(new_n28_));
  inv1   g09(.a(x08), .O(new_n29_));
  nand3  g10(.a(new_n29_), .b(x04), .c(x02), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x03), .O(new_n32_));
  nor2   g13(.a(x03), .b(x00), .O(new_n33_));
  nor2   g14(.a(new_n33_), .b(new_n20_), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(z0));
  inv1   g16(.a(x03), .O(new_n36_));
  nand2  g17(.a(new_n30_), .b(x10), .O(new_n37_));
  nand4  g18(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  oai21  g20(.a(x03), .b(x01), .c(x05), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n39_), .c(new_n21_), .O(new_n41_));
  nor3   g22(.a(new_n20_), .b(x10), .c(x08), .O(new_n42_));
  nand4  g23(.a(new_n42_), .b(x04), .c(x03), .d(x02), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n41_), .O(z1));
  nor2   g25(.a(x11), .b(x10), .O(new_n45_));
  nand4  g26(.a(new_n45_), .b(new_n29_), .c(x04), .d(x02), .O(new_n46_));
  inv1   g27(.a(x10), .O(new_n47_));
  nand4  g28(.a(new_n47_), .b(new_n29_), .c(x04), .d(x02), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x11), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x03), .O(new_n51_));
  nor2   g32(.a(x06), .b(x03), .O(new_n52_));
  nor2   g33(.a(new_n52_), .b(new_n20_), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n51_), .c(new_n25_), .O(z2));
  xnor2a g35(.a(x12), .b(x11), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n47_), .c(new_n29_), .d(x04), .O(new_n56_));
  nand2  g37(.a(new_n48_), .b(x12), .O(new_n57_));
  oai21  g38(.a(new_n56_), .b(new_n26_), .c(new_n57_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x03), .O(new_n59_));
  inv1   g40(.a(x07), .O(new_n60_));
  nand2  g41(.a(x12), .b(x03), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g43(.a(new_n62_), .b(new_n59_), .c(new_n25_), .O(z3));
  nand3  g44(.a(new_n21_), .b(x13), .c(x09), .O(new_n64_));
  nor2   g45(.a(new_n64_), .b(new_n27_), .O(z4));
endmodule


