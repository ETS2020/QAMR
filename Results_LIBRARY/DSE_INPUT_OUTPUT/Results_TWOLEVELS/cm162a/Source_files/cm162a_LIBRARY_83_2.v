// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(new_n21_), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nor2   g05(.a(x11), .b(x10), .O(new_n25_));
  nand3  g06(.a(x13), .b(x11), .c(x09), .O(new_n26_));
  oai21  g07(.a(new_n25_), .b(x08), .c(new_n26_), .O(new_n27_));
  nand3  g08(.a(new_n27_), .b(x04), .c(x02), .O(new_n28_));
  oai21  g09(.a(new_n22_), .b(x02), .c(new_n28_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g11(.a(x00), .O(new_n31_));
  nand2  g12(.a(new_n21_), .b(new_n31_), .O(new_n32_));
  inv1   g13(.a(x05), .O(new_n33_));
  nand2  g14(.a(x11), .b(new_n33_), .O(new_n34_));
  inv1   g15(.a(x11), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(x04), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n34_), .c(new_n32_), .O(new_n37_));
  inv1   g18(.a(new_n37_), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n30_), .c(new_n24_), .O(z0));
  oai21  g20(.a(x03), .b(x01), .c(x05), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nand2  g22(.a(x13), .b(x09), .O(new_n42_));
  oai21  g23(.a(x10), .b(x08), .c(new_n42_), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(x04), .c(x02), .O(new_n44_));
  inv1   g25(.a(x02), .O(new_n45_));
  oai21  g26(.a(x08), .b(new_n45_), .c(x10), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n44_), .c(new_n35_), .O(new_n47_));
  inv1   g28(.a(x10), .O(new_n48_));
  nor2   g29(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n47_), .c(x03), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n41_), .O(z1));
  oai21  g32(.a(new_n48_), .b(x08), .c(new_n26_), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(x04), .c(x02), .O(new_n53_));
  nand3  g34(.a(new_n22_), .b(x04), .c(x02), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x11), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x03), .O(new_n57_));
  inv1   g38(.a(x06), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n21_), .O(new_n59_));
  nand4  g40(.a(new_n59_), .b(new_n57_), .c(new_n36_), .d(x05), .O(z2));
  nor3   g41(.a(new_n26_), .b(new_n20_), .c(new_n45_), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(x12), .c(x03), .O(new_n62_));
  inv1   g43(.a(x07), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n21_), .O(new_n64_));
  nand4  g45(.a(new_n64_), .b(new_n62_), .c(new_n36_), .d(x05), .O(z3));
  nand4  g46(.a(x13), .b(x11), .c(x09), .d(x04), .O(new_n66_));
  inv1   g47(.a(new_n66_), .O(z4));
endmodule


