// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x11), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x10), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(new_n23_), .O(new_n25_));
  and2   g06(.a(x13), .b(x09), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n24_), .c(new_n22_), .O(new_n28_));
  oai21  g09(.a(x03), .b(x00), .c(x05), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(new_n21_), .O(new_n30_));
  aoi21  g11(.a(new_n20_), .b(x10), .c(x08), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n30_), .O(z0));
  nor2   g14(.a(x03), .b(x01), .O(new_n34_));
  nand2  g15(.a(x03), .b(x02), .O(new_n35_));
  nand2  g16(.a(new_n26_), .b(x04), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n35_), .c(x05), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n34_), .c(new_n21_), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  inv1   g20(.a(x08), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(x04), .c(x02), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x11), .O(new_n42_));
  nand3  g23(.a(new_n25_), .b(new_n39_), .c(new_n40_), .O(new_n43_));
  oai21  g24(.a(new_n42_), .b(new_n39_), .c(new_n43_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x03), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n38_), .O(z1));
  inv1   g27(.a(x05), .O(new_n47_));
  nand2  g28(.a(new_n21_), .b(new_n47_), .O(new_n48_));
  inv1   g29(.a(x06), .O(new_n49_));
  nor2   g30(.a(new_n20_), .b(new_n39_), .O(new_n50_));
  nor2   g31(.a(x10), .b(x03), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  xor2a  g33(.a(x11), .b(x10), .O(new_n53_));
  nand3  g34(.a(x13), .b(new_n39_), .c(x09), .O(new_n54_));
  oai21  g35(.a(new_n53_), .b(x08), .c(new_n54_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(x04), .c(x02), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n42_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x03), .O(new_n58_));
  nand3  g39(.a(new_n58_), .b(new_n52_), .c(new_n48_), .O(z2));
  oai22  g40(.a(new_n36_), .b(new_n35_), .c(x07), .d(x03), .O(new_n60_));
  oai21  g41(.a(new_n60_), .b(new_n47_), .c(new_n21_), .O(new_n61_));
  nand2  g42(.a(new_n41_), .b(x12), .O(new_n62_));
  inv1   g43(.a(x12), .O(new_n63_));
  nand4  g44(.a(new_n25_), .b(new_n63_), .c(new_n20_), .d(new_n40_), .O(new_n64_));
  aoi21  g45(.a(new_n64_), .b(new_n62_), .c(x10), .O(new_n65_));
  nor2   g46(.a(new_n63_), .b(new_n20_), .O(new_n66_));
  oai21  g47(.a(new_n66_), .b(new_n65_), .c(x03), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(new_n61_), .O(z3));
  nand4  g49(.a(new_n21_), .b(x13), .c(x09), .d(x04), .O(new_n69_));
  inv1   g50(.a(new_n69_), .O(z4));
endmodule


