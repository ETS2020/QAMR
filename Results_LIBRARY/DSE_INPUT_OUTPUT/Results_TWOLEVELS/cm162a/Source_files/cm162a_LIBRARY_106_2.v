// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_;
  inv1   g00(.a(x10), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x09), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nand3  g03(.a(new_n22_), .b(x08), .c(x03), .O(new_n23_));
  inv1   g04(.a(x00), .O(new_n24_));
  inv1   g05(.a(x03), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(new_n23_), .c(x05), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n21_), .O(new_n28_));
  inv1   g09(.a(x08), .O(new_n29_));
  inv1   g10(.a(x09), .O(new_n30_));
  oai21  g11(.a(x10), .b(new_n30_), .c(new_n29_), .O(new_n31_));
  nand3  g12(.a(x13), .b(x10), .c(x09), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand4  g14(.a(new_n33_), .b(x04), .c(x03), .d(x02), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n28_), .O(z0));
  inv1   g16(.a(x04), .O(new_n36_));
  nand3  g17(.a(new_n20_), .b(new_n30_), .c(new_n29_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n32_), .c(new_n36_), .O(new_n38_));
  nor2   g19(.a(x08), .b(new_n36_), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(x02), .c(new_n20_), .O(new_n40_));
  aoi21  g21(.a(new_n38_), .b(x02), .c(new_n40_), .O(new_n41_));
  inv1   g22(.a(x01), .O(new_n42_));
  nand2  g23(.a(new_n25_), .b(new_n42_), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n21_), .c(x05), .O(new_n44_));
  inv1   g25(.a(new_n44_), .O(new_n45_));
  oai21  g26(.a(new_n41_), .b(new_n25_), .c(new_n45_), .O(z1));
  inv1   g27(.a(x11), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n20_), .c(new_n29_), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n32_), .c(new_n36_), .O(new_n49_));
  inv1   g30(.a(new_n22_), .O(new_n50_));
  nor2   g31(.a(x10), .b(x08), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(new_n52_));
  aoi21  g33(.a(new_n49_), .b(x02), .c(new_n52_), .O(new_n53_));
  inv1   g34(.a(x06), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n25_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n21_), .c(x05), .O(new_n56_));
  inv1   g37(.a(new_n56_), .O(new_n57_));
  oai21  g38(.a(new_n53_), .b(new_n25_), .c(new_n57_), .O(z2));
  inv1   g39(.a(x12), .O(new_n59_));
  nand4  g40(.a(new_n59_), .b(new_n47_), .c(new_n20_), .d(new_n29_), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(new_n32_), .c(new_n36_), .O(new_n61_));
  nor3   g42(.a(x11), .b(x10), .c(x08), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(new_n50_), .c(new_n59_), .O(new_n63_));
  aoi21  g44(.a(new_n61_), .b(x02), .c(new_n63_), .O(new_n64_));
  inv1   g45(.a(x07), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n25_), .O(new_n66_));
  nand3  g47(.a(new_n66_), .b(new_n21_), .c(x05), .O(new_n67_));
  inv1   g48(.a(new_n67_), .O(new_n68_));
  oai21  g49(.a(new_n64_), .b(new_n25_), .c(new_n68_), .O(z3));
  nand4  g50(.a(x13), .b(x10), .c(x09), .d(x04), .O(new_n70_));
  inv1   g51(.a(new_n70_), .O(z4));
endmodule


