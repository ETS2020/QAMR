// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_;
  nand2  g00(.a(x11), .b(x06), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  inv1   g04(.a(new_n22_), .O(new_n24_));
  nand2  g05(.a(x13), .b(x09), .O(new_n25_));
  inv1   g06(.a(new_n25_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n23_), .c(new_n21_), .O(new_n28_));
  oai21  g09(.a(x03), .b(x00), .c(x05), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(new_n20_), .O(new_n30_));
  inv1   g11(.a(x06), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  oai21  g13(.a(x11), .b(new_n32_), .c(new_n31_), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(x11), .c(x08), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(x04), .c(x03), .d(x02), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n30_), .O(z0));
  inv1   g17(.a(new_n20_), .O(new_n37_));
  oai21  g18(.a(x10), .b(x08), .c(new_n25_), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(x04), .c(x02), .O(new_n39_));
  inv1   g20(.a(x08), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(x04), .c(x02), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x10), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x03), .O(new_n44_));
  inv1   g25(.a(x01), .O(new_n45_));
  inv1   g26(.a(x05), .O(new_n46_));
  aoi21  g27(.a(new_n21_), .b(new_n45_), .c(new_n46_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n44_), .c(new_n37_), .O(z1));
  nand2  g29(.a(x03), .b(x02), .O(new_n49_));
  nand2  g30(.a(new_n26_), .b(x04), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n49_), .c(x05), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n20_), .O(new_n52_));
  nand3  g33(.a(x11), .b(x10), .c(new_n31_), .O(new_n53_));
  inv1   g34(.a(x11), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n32_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n53_), .c(x08), .O(new_n56_));
  nand4  g37(.a(new_n56_), .b(x04), .c(x03), .d(x02), .O(new_n57_));
  nand3  g38(.a(new_n24_), .b(new_n40_), .c(new_n31_), .O(new_n58_));
  aoi22  g39(.a(new_n58_), .b(x11), .c(new_n31_), .d(new_n21_), .O(new_n59_));
  nand3  g40(.a(new_n59_), .b(new_n57_), .c(new_n52_), .O(z2));
  inv1   g41(.a(x12), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(new_n21_), .c(new_n31_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(x11), .O(new_n63_));
  nand2  g44(.a(x12), .b(x10), .O(new_n64_));
  nor2   g45(.a(x12), .b(x10), .O(new_n65_));
  nand4  g46(.a(new_n65_), .b(new_n40_), .c(x04), .d(x02), .O(new_n66_));
  aoi21  g47(.a(new_n66_), .b(new_n64_), .c(x11), .O(new_n67_));
  and2   g48(.a(new_n41_), .b(x12), .O(new_n68_));
  oai21  g49(.a(new_n68_), .b(new_n67_), .c(x03), .O(new_n69_));
  inv1   g50(.a(x07), .O(new_n70_));
  nand2  g51(.a(new_n70_), .b(new_n21_), .O(new_n71_));
  nand4  g52(.a(new_n71_), .b(new_n69_), .c(new_n63_), .d(new_n52_), .O(z3));
  nand4  g53(.a(new_n20_), .b(x13), .c(x09), .d(x04), .O(new_n73_));
  inv1   g54(.a(new_n73_), .O(z4));
endmodule


