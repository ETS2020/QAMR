// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  and2   g02(.a(x13), .b(x09), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  aoi21  g04(.a(new_n23_), .b(x08), .c(new_n21_), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n25_), .O(new_n26_));
  aoi21  g07(.a(new_n24_), .b(x02), .c(new_n26_), .O(new_n27_));
  inv1   g08(.a(x05), .O(new_n28_));
  nor2   g09(.a(x11), .b(x04), .O(new_n29_));
  nor2   g10(.a(x03), .b(x00), .O(new_n30_));
  nor3   g11(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(new_n31_));
  oai21  g12(.a(new_n27_), .b(new_n20_), .c(new_n31_), .O(z0));
  oai21  g13(.a(x03), .b(x01), .c(x05), .O(new_n33_));
  oai21  g14(.a(x11), .b(x04), .c(new_n33_), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n22_), .c(x02), .O(new_n36_));
  inv1   g17(.a(x02), .O(new_n37_));
  oai21  g18(.a(x08), .b(new_n37_), .c(x10), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n36_), .c(new_n21_), .O(new_n39_));
  nand2  g20(.a(x11), .b(x10), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(x04), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n39_), .c(x03), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n34_), .O(z1));
  nor2   g24(.a(x11), .b(x10), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nand2  g26(.a(new_n22_), .b(x04), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n45_), .c(new_n37_), .O(new_n47_));
  nand2  g28(.a(new_n35_), .b(x02), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x11), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(x04), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n47_), .c(x03), .O(new_n51_));
  nor2   g32(.a(x06), .b(x03), .O(new_n52_));
  nor3   g33(.a(new_n52_), .b(new_n29_), .c(new_n28_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(z2));
  nor2   g35(.a(x12), .b(x11), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n35_), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n46_), .c(new_n37_), .O(new_n57_));
  inv1   g38(.a(x12), .O(new_n58_));
  nor2   g39(.a(x08), .b(new_n37_), .O(new_n59_));
  aoi21  g40(.a(new_n44_), .b(new_n59_), .c(new_n58_), .O(new_n60_));
  oai21  g41(.a(new_n60_), .b(new_n57_), .c(x03), .O(new_n61_));
  nor2   g42(.a(x07), .b(x03), .O(new_n62_));
  nor3   g43(.a(new_n62_), .b(new_n29_), .c(new_n28_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(z3));
  nand3  g45(.a(x13), .b(x09), .c(x04), .O(new_n65_));
  inv1   g46(.a(new_n65_), .O(z4));
endmodule


