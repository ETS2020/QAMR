// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  nand3  g02(.a(x13), .b(x09), .c(x05), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(x08), .c(new_n21_), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n24_), .O(new_n25_));
  aoi21  g06(.a(new_n23_), .b(x02), .c(new_n25_), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x05), .O(new_n28_));
  aoi21  g09(.a(new_n20_), .b(new_n27_), .c(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n26_), .b(new_n20_), .c(new_n29_), .O(z0));
  nand2  g11(.a(x13), .b(x09), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n24_), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand2  g16(.a(new_n24_), .b(x02), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x10), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n35_), .c(new_n28_), .O(new_n38_));
  nor2   g19(.a(new_n32_), .b(x04), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(x03), .O(new_n40_));
  inv1   g21(.a(x01), .O(new_n41_));
  nor2   g22(.a(new_n28_), .b(x03), .O(new_n42_));
  nor2   g23(.a(x05), .b(x04), .O(new_n43_));
  aoi21  g24(.a(new_n42_), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n40_), .O(z1));
  inv1   g26(.a(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n32_), .c(new_n24_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  nand3  g29(.a(new_n48_), .b(x04), .c(x02), .O(new_n49_));
  inv1   g30(.a(x02), .O(new_n50_));
  oai21  g31(.a(new_n33_), .b(new_n50_), .c(x11), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n49_), .c(new_n28_), .O(new_n52_));
  nor2   g33(.a(new_n46_), .b(x04), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n52_), .c(x03), .O(new_n54_));
  nor2   g35(.a(x06), .b(new_n28_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n20_), .c(new_n43_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n54_), .O(z2));
  inv1   g38(.a(x12), .O(new_n58_));
  nand4  g39(.a(new_n58_), .b(new_n46_), .c(new_n32_), .d(new_n24_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n31_), .O(new_n60_));
  nand3  g41(.a(new_n60_), .b(x04), .c(x02), .O(new_n61_));
  nand2  g42(.a(new_n46_), .b(new_n32_), .O(new_n62_));
  oai21  g43(.a(new_n62_), .b(new_n36_), .c(x12), .O(new_n63_));
  aoi21  g44(.a(new_n63_), .b(new_n61_), .c(new_n28_), .O(new_n64_));
  nor2   g45(.a(new_n58_), .b(x04), .O(new_n65_));
  oai21  g46(.a(new_n65_), .b(new_n64_), .c(x03), .O(new_n66_));
  nor2   g47(.a(x07), .b(new_n28_), .O(new_n67_));
  aoi21  g48(.a(new_n67_), .b(new_n20_), .c(new_n43_), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n66_), .O(z3));
  nand4  g50(.a(x13), .b(x09), .c(x05), .d(x04), .O(new_n70_));
  inv1   g51(.a(new_n70_), .O(z4));
endmodule


