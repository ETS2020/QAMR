// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_;
  nor2   g00(.a(x12), .b(x07), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  nand3  g03(.a(new_n22_), .b(x04), .c(x02), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(new_n27_), .c(new_n20_), .O(z0));
  nand2  g13(.a(x03), .b(x02), .O(new_n33_));
  nand3  g14(.a(x13), .b(x09), .c(x04), .O(new_n34_));
  oai21  g15(.a(new_n34_), .b(new_n33_), .c(x05), .O(new_n35_));
  oai21  g16(.a(x12), .b(x07), .c(new_n35_), .O(new_n36_));
  inv1   g17(.a(x08), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x10), .O(new_n39_));
  inv1   g20(.a(new_n24_), .O(new_n40_));
  nor2   g21(.a(x10), .b(x08), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x03), .O(new_n44_));
  nor2   g25(.a(x03), .b(x01), .O(new_n45_));
  nor2   g26(.a(new_n45_), .b(new_n20_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n44_), .c(new_n36_), .O(z1));
  inv1   g28(.a(x11), .O(new_n48_));
  aoi21  g29(.a(new_n41_), .b(new_n40_), .c(new_n48_), .O(new_n49_));
  inv1   g30(.a(x10), .O(new_n50_));
  nand3  g31(.a(new_n48_), .b(new_n50_), .c(new_n37_), .O(new_n51_));
  nor2   g32(.a(new_n51_), .b(new_n24_), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n49_), .c(x03), .O(new_n53_));
  nor2   g34(.a(x06), .b(x03), .O(new_n54_));
  nor2   g35(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n53_), .c(new_n36_), .O(z2));
  inv1   g37(.a(x07), .O(new_n57_));
  inv1   g38(.a(x12), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n29_), .c(new_n57_), .O(new_n59_));
  nor3   g40(.a(x11), .b(x10), .c(x08), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(new_n40_), .c(new_n58_), .O(new_n61_));
  nand3  g42(.a(new_n58_), .b(new_n48_), .c(new_n50_), .O(new_n62_));
  nor2   g43(.a(new_n62_), .b(new_n38_), .O(new_n63_));
  oai21  g44(.a(new_n63_), .b(new_n61_), .c(x03), .O(new_n64_));
  nand3  g45(.a(new_n64_), .b(new_n59_), .c(new_n36_), .O(z3));
  inv1   g46(.a(x04), .O(new_n66_));
  inv1   g47(.a(x09), .O(new_n67_));
  inv1   g48(.a(x13), .O(new_n68_));
  nor4   g49(.a(new_n20_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(z4));
endmodule


