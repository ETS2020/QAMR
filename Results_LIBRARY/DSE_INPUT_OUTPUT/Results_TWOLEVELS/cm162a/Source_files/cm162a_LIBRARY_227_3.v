// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand3  g03(.a(new_n20_), .b(x04), .c(x02), .O(new_n23_));
  oai21  g04(.a(new_n22_), .b(new_n20_), .c(new_n23_), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x03), .O(new_n25_));
  nor2   g06(.a(x03), .b(x00), .O(new_n26_));
  inv1   g07(.a(x09), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x05), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n25_), .O(z0));
  inv1   g11(.a(x03), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand3  g13(.a(new_n22_), .b(new_n32_), .c(new_n20_), .O(new_n33_));
  inv1   g14(.a(new_n33_), .O(new_n34_));
  aoi21  g15(.a(new_n23_), .b(x10), .c(new_n34_), .O(new_n35_));
  nor2   g16(.a(x03), .b(x01), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n28_), .O(new_n37_));
  oai21  g18(.a(new_n35_), .b(new_n31_), .c(new_n37_), .O(z1));
  nor3   g19(.a(x11), .b(x10), .c(x08), .O(new_n39_));
  aoi22  g20(.a(new_n39_), .b(new_n22_), .c(new_n33_), .d(x11), .O(new_n40_));
  inv1   g21(.a(x05), .O(new_n41_));
  nor2   g22(.a(x06), .b(x03), .O(new_n42_));
  nor3   g23(.a(new_n42_), .b(x09), .c(new_n41_), .O(new_n43_));
  oai21  g24(.a(new_n40_), .b(new_n31_), .c(new_n43_), .O(z2));
  nand2  g25(.a(x12), .b(x11), .O(new_n45_));
  inv1   g26(.a(x11), .O(new_n46_));
  inv1   g27(.a(x12), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n46_), .c(new_n32_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n21_), .c(new_n45_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n20_), .O(new_n50_));
  nand2  g31(.a(new_n33_), .b(x12), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n50_), .c(new_n31_), .O(new_n52_));
  nor2   g33(.a(x07), .b(x03), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n52_), .c(new_n27_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x05), .O(z3));
  nand4  g36(.a(x13), .b(x09), .c(new_n41_), .d(x04), .O(new_n56_));
  inv1   g37(.a(new_n56_), .O(z4));
endmodule


