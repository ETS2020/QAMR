// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_;
  nand4  g00(.a(x13), .b(x09), .c(x04), .d(x03), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x05), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x02), .O(new_n22_));
  inv1   g03(.a(x03), .O(new_n23_));
  nand2  g04(.a(x05), .b(x00), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g06(.a(x04), .O(new_n26_));
  nand2  g07(.a(x08), .b(new_n26_), .O(new_n27_));
  inv1   g08(.a(x08), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x04), .O(new_n29_));
  nand3  g10(.a(new_n29_), .b(new_n27_), .c(x02), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x03), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(new_n25_), .c(new_n22_), .O(z0));
  nand2  g13(.a(x05), .b(x01), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n23_), .O(new_n34_));
  nand2  g15(.a(new_n29_), .b(x10), .O(new_n35_));
  inv1   g16(.a(x02), .O(new_n36_));
  nor2   g17(.a(x10), .b(x08), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(x04), .c(new_n36_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n35_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n34_), .c(new_n22_), .O(z1));
  nand2  g22(.a(x06), .b(x05), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n23_), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  aoi21  g25(.a(new_n37_), .b(x04), .c(new_n44_), .O(new_n45_));
  inv1   g26(.a(x10), .O(new_n46_));
  nand2  g27(.a(new_n44_), .b(new_n46_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n29_), .c(x02), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n45_), .c(x03), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n43_), .c(new_n22_), .O(z2));
  nand2  g31(.a(x07), .b(x05), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n23_), .O(new_n52_));
  inv1   g33(.a(x05), .O(new_n53_));
  nor2   g34(.a(x12), .b(x11), .O(new_n54_));
  aoi22  g35(.a(new_n54_), .b(new_n37_), .c(x13), .d(x09), .O(new_n55_));
  nand4  g36(.a(new_n44_), .b(new_n46_), .c(new_n28_), .d(x04), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x12), .O(new_n57_));
  oai21  g38(.a(new_n55_), .b(new_n26_), .c(new_n57_), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(x03), .c(new_n53_), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n36_), .c(new_n52_), .O(z3));
  nand2  g41(.a(x03), .b(new_n36_), .O(new_n61_));
  nand4  g42(.a(new_n61_), .b(x13), .c(x09), .d(x04), .O(new_n62_));
  inv1   g43(.a(new_n62_), .O(z4));
endmodule


