// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  nand2  g00(.a(x12), .b(x11), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  nand4  g04(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n24_));
  aoi21  g05(.a(new_n24_), .b(new_n23_), .c(new_n21_), .O(new_n25_));
  oai21  g06(.a(x03), .b(x00), .c(x05), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n25_), .c(new_n20_), .O(new_n27_));
  inv1   g08(.a(x10), .O(new_n28_));
  inv1   g09(.a(x11), .O(new_n29_));
  oai21  g10(.a(x12), .b(new_n28_), .c(new_n29_), .O(new_n30_));
  aoi21  g11(.a(new_n30_), .b(x12), .c(x08), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n27_), .O(z0));
  inv1   g14(.a(x05), .O(new_n34_));
  nand2  g15(.a(x03), .b(x02), .O(new_n35_));
  nand3  g16(.a(x13), .b(x09), .c(x04), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  inv1   g19(.a(x08), .O(new_n39_));
  nand3  g20(.a(new_n39_), .b(x04), .c(x02), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x10), .O(new_n41_));
  nand4  g22(.a(new_n28_), .b(new_n39_), .c(x04), .d(x02), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x03), .O(new_n44_));
  inv1   g25(.a(x01), .O(new_n45_));
  nand2  g26(.a(new_n21_), .b(new_n45_), .O(new_n46_));
  nand4  g27(.a(new_n46_), .b(new_n44_), .c(new_n38_), .d(new_n20_), .O(z1));
  oai22  g28(.a(new_n36_), .b(new_n35_), .c(x06), .d(x03), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n34_), .c(new_n20_), .O(new_n49_));
  inv1   g30(.a(x12), .O(new_n50_));
  nand3  g31(.a(new_n42_), .b(new_n50_), .c(x11), .O(new_n51_));
  nor2   g32(.a(x11), .b(x10), .O(new_n52_));
  nand4  g33(.a(new_n52_), .b(new_n39_), .c(x04), .d(x02), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x03), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n49_), .O(z2));
  oai22  g37(.a(new_n36_), .b(new_n35_), .c(x07), .d(x03), .O(new_n57_));
  oai21  g38(.a(new_n57_), .b(new_n34_), .c(new_n20_), .O(new_n58_));
  xnor2a g39(.a(x12), .b(x10), .O(new_n59_));
  nand4  g40(.a(new_n59_), .b(new_n39_), .c(x04), .d(x02), .O(new_n60_));
  nand2  g41(.a(new_n40_), .b(x12), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g43(.a(new_n62_), .b(new_n29_), .c(x03), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n58_), .O(z3));
  nand2  g45(.a(new_n36_), .b(new_n20_), .O(z4));
endmodule


