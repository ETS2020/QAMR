// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(new_n21_), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nand2  g05(.a(x13), .b(x09), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x08), .O(new_n26_));
  nand4  g07(.a(new_n26_), .b(x12), .c(x04), .d(x02), .O(new_n27_));
  oai21  g08(.a(new_n22_), .b(x02), .c(new_n27_), .O(new_n28_));
  inv1   g09(.a(x00), .O(new_n29_));
  nand2  g10(.a(new_n21_), .b(new_n29_), .O(new_n30_));
  inv1   g11(.a(x05), .O(new_n31_));
  nand2  g12(.a(x12), .b(new_n31_), .O(new_n32_));
  inv1   g13(.a(x12), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x04), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(new_n32_), .c(new_n30_), .O(new_n35_));
  aoi21  g16(.a(new_n28_), .b(x03), .c(new_n35_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n24_), .O(z0));
  oai21  g18(.a(x03), .b(x01), .c(x05), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  oai21  g20(.a(x10), .b(x08), .c(new_n25_), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(x04), .c(x02), .O(new_n41_));
  inv1   g22(.a(x02), .O(new_n42_));
  oai21  g23(.a(x08), .b(new_n42_), .c(x10), .O(new_n43_));
  aoi21  g24(.a(new_n43_), .b(new_n41_), .c(new_n33_), .O(new_n44_));
  inv1   g25(.a(x10), .O(new_n45_));
  nor2   g26(.a(new_n45_), .b(x04), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(new_n44_), .c(x03), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n39_), .O(z1));
  inv1   g29(.a(x11), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n45_), .c(new_n22_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  nand4  g32(.a(new_n51_), .b(x12), .c(x04), .d(x02), .O(new_n52_));
  nand4  g33(.a(new_n45_), .b(new_n22_), .c(x04), .d(x02), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x03), .O(new_n56_));
  inv1   g37(.a(x06), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n21_), .O(new_n58_));
  nand4  g39(.a(new_n58_), .b(new_n56_), .c(new_n34_), .d(x05), .O(z2));
  oai21  g40(.a(x07), .b(x03), .c(x05), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n34_), .O(new_n61_));
  nand2  g42(.a(new_n49_), .b(new_n45_), .O(new_n62_));
  aoi22  g43(.a(new_n62_), .b(new_n22_), .c(x13), .d(x09), .O(new_n63_));
  nand4  g44(.a(new_n63_), .b(new_n22_), .c(x04), .d(x02), .O(new_n64_));
  nand3  g45(.a(new_n64_), .b(x12), .c(x03), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n61_), .O(z3));
  aoi21  g47(.a(new_n25_), .b(x12), .c(new_n20_), .O(z4));
endmodule


