// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_;
  inv1   g00(.a(x05), .O(new_n20_));
  inv1   g01(.a(x12), .O(new_n21_));
  nor2   g02(.a(new_n21_), .b(x07), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  inv1   g04(.a(x02), .O(new_n24_));
  inv1   g05(.a(x03), .O(new_n25_));
  inv1   g06(.a(x04), .O(new_n26_));
  nand2  g07(.a(x13), .b(x09), .O(new_n27_));
  nor4   g08(.a(new_n27_), .b(new_n26_), .c(new_n25_), .d(new_n24_), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n20_), .c(new_n23_), .O(new_n29_));
  inv1   g10(.a(x08), .O(new_n30_));
  nor2   g11(.a(new_n26_), .b(new_n24_), .O(new_n31_));
  nand3  g12(.a(new_n30_), .b(x04), .c(x02), .O(new_n32_));
  oai21  g13(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x03), .O(new_n34_));
  nor2   g15(.a(x03), .b(x00), .O(new_n35_));
  nor2   g16(.a(new_n35_), .b(new_n22_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n34_), .c(new_n29_), .O(z0));
  nand2  g18(.a(new_n32_), .b(x10), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  nand4  g20(.a(new_n39_), .b(new_n30_), .c(x04), .d(x02), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x03), .O(new_n42_));
  nor2   g23(.a(x03), .b(x01), .O(new_n43_));
  nor2   g24(.a(new_n43_), .b(new_n22_), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n42_), .c(new_n29_), .O(z1));
  inv1   g26(.a(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n39_), .c(new_n30_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n27_), .O(new_n48_));
  nand3  g29(.a(new_n48_), .b(x04), .c(x02), .O(new_n49_));
  nand2  g30(.a(new_n40_), .b(x11), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x03), .O(new_n52_));
  inv1   g33(.a(x06), .O(new_n53_));
  aoi21  g34(.a(new_n53_), .b(new_n25_), .c(new_n20_), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n52_), .c(new_n22_), .O(z2));
  nand3  g36(.a(x04), .b(x03), .c(x02), .O(new_n56_));
  oai22  g37(.a(new_n56_), .b(new_n47_), .c(x07), .d(x03), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n21_), .O(new_n58_));
  nand4  g39(.a(new_n31_), .b(new_n46_), .c(new_n39_), .d(new_n30_), .O(new_n59_));
  nand4  g40(.a(new_n59_), .b(x12), .c(x07), .d(x03), .O(new_n60_));
  nand3  g41(.a(new_n60_), .b(new_n58_), .c(new_n29_), .O(z3));
  nand4  g42(.a(new_n23_), .b(x13), .c(x09), .d(x04), .O(new_n62_));
  inv1   g43(.a(new_n62_), .O(z4));
endmodule


