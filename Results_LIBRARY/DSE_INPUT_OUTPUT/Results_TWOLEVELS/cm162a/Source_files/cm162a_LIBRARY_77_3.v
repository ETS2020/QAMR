// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_;
  inv1   g00(.a(x02), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(new_n21_), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  inv1   g05(.a(x12), .O(new_n25_));
  nand3  g06(.a(x13), .b(new_n25_), .c(x09), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x08), .O(new_n27_));
  nand3  g08(.a(new_n27_), .b(x04), .c(x03), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  inv1   g10(.a(x00), .O(new_n30_));
  nand2  g11(.a(new_n21_), .b(new_n30_), .O(new_n31_));
  inv1   g12(.a(x05), .O(new_n32_));
  nand2  g13(.a(new_n25_), .b(new_n32_), .O(new_n33_));
  inv1   g14(.a(x04), .O(new_n34_));
  nand3  g15(.a(x08), .b(new_n34_), .c(x03), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(new_n33_), .c(new_n31_), .O(new_n36_));
  aoi21  g17(.a(new_n29_), .b(x02), .c(new_n36_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n24_), .O(z0));
  nand2  g19(.a(x12), .b(x02), .O(new_n39_));
  oai21  g20(.a(x03), .b(x01), .c(x05), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g22(.a(x13), .b(x09), .O(new_n42_));
  oai21  g23(.a(x10), .b(x08), .c(new_n42_), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(x04), .c(x02), .O(new_n44_));
  oai21  g25(.a(x08), .b(new_n34_), .c(x10), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n44_), .c(x12), .O(new_n46_));
  inv1   g27(.a(x10), .O(new_n47_));
  nor2   g28(.a(new_n47_), .b(x02), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n46_), .c(x03), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n41_), .O(z1));
  inv1   g31(.a(x11), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n21_), .c(x05), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n20_), .O(new_n53_));
  nand3  g34(.a(new_n51_), .b(new_n47_), .c(new_n22_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n42_), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n25_), .c(x04), .d(x02), .O(new_n56_));
  nand3  g37(.a(new_n47_), .b(new_n22_), .c(x04), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x11), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(x03), .O(new_n60_));
  inv1   g41(.a(x06), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n21_), .O(new_n62_));
  nand3  g43(.a(new_n62_), .b(new_n39_), .c(new_n33_), .O(new_n63_));
  inv1   g44(.a(new_n63_), .O(new_n64_));
  nand3  g45(.a(new_n64_), .b(new_n60_), .c(new_n53_), .O(z2));
  nand4  g46(.a(new_n55_), .b(x04), .c(x03), .d(x02), .O(new_n66_));
  inv1   g47(.a(x07), .O(new_n67_));
  aoi21  g48(.a(new_n67_), .b(new_n21_), .c(new_n32_), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g50(.a(new_n69_), .b(new_n25_), .O(new_n70_));
  aoi21  g51(.a(x07), .b(new_n21_), .c(new_n25_), .O(new_n71_));
  oai21  g52(.a(new_n71_), .b(new_n32_), .c(new_n20_), .O(new_n72_));
  nand2  g53(.a(new_n72_), .b(new_n70_), .O(z3));
  nand4  g54(.a(new_n39_), .b(x13), .c(x09), .d(x04), .O(new_n74_));
  inv1   g55(.a(new_n74_), .O(z4));
endmodule


