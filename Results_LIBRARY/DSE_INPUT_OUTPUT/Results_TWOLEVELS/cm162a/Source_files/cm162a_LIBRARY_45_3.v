// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_;
  nand2  g00(.a(x11), .b(x06), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n23_), .c(new_n21_), .O(new_n27_));
  oai21  g08(.a(x03), .b(x00), .c(x05), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(new_n20_), .O(new_n29_));
  inv1   g10(.a(x06), .O(new_n30_));
  inv1   g11(.a(x10), .O(new_n31_));
  oai21  g12(.a(x11), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  aoi21  g13(.a(new_n32_), .b(x11), .c(x08), .O(new_n33_));
  nand4  g14(.a(new_n33_), .b(x04), .c(x03), .d(x02), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n29_), .O(z0));
  oai21  g16(.a(x10), .b(x08), .c(new_n24_), .O(new_n36_));
  nand4  g17(.a(new_n36_), .b(x04), .c(x03), .d(x02), .O(new_n37_));
  and2   g18(.a(new_n37_), .b(x05), .O(new_n38_));
  inv1   g19(.a(x08), .O(new_n39_));
  nand3  g20(.a(new_n39_), .b(x04), .c(x02), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(x10), .c(x03), .O(new_n41_));
  inv1   g22(.a(x01), .O(new_n42_));
  nand2  g23(.a(new_n21_), .b(new_n42_), .O(new_n43_));
  nand4  g24(.a(new_n43_), .b(new_n41_), .c(new_n38_), .d(new_n20_), .O(z1));
  nand2  g25(.a(x03), .b(x02), .O(new_n45_));
  nand2  g26(.a(new_n25_), .b(x04), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(new_n45_), .c(x05), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  nand3  g29(.a(x11), .b(x10), .c(new_n30_), .O(new_n49_));
  inv1   g30(.a(x11), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n31_), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n49_), .c(x08), .O(new_n52_));
  nand4  g33(.a(new_n52_), .b(x04), .c(x03), .d(x02), .O(new_n53_));
  inv1   g34(.a(new_n40_), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n50_), .c(x03), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n53_), .c(new_n48_), .O(z2));
  inv1   g38(.a(x12), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n21_), .c(new_n30_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(x11), .O(new_n60_));
  nand2  g41(.a(x12), .b(x10), .O(new_n61_));
  nor2   g42(.a(x12), .b(x10), .O(new_n62_));
  nand4  g43(.a(new_n62_), .b(new_n39_), .c(x04), .d(x02), .O(new_n63_));
  aoi21  g44(.a(new_n63_), .b(new_n61_), .c(x11), .O(new_n64_));
  nor2   g45(.a(new_n54_), .b(new_n58_), .O(new_n65_));
  oai21  g46(.a(new_n65_), .b(new_n64_), .c(x03), .O(new_n66_));
  inv1   g47(.a(x07), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(new_n21_), .O(new_n68_));
  nand4  g49(.a(new_n68_), .b(new_n66_), .c(new_n60_), .d(new_n48_), .O(z3));
  nand2  g50(.a(new_n46_), .b(new_n20_), .O(z4));
endmodule


