// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x07), .O(new_n20_));
  inv1   g01(.a(x12), .O(new_n21_));
  nor2   g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(x03), .b(x02), .O(new_n24_));
  nand3  g05(.a(x13), .b(x09), .c(x04), .O(new_n25_));
  oai21  g06(.a(new_n25_), .b(new_n24_), .c(x05), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n23_), .O(new_n27_));
  inv1   g08(.a(x08), .O(new_n28_));
  nand2  g09(.a(x04), .b(x02), .O(new_n29_));
  inv1   g10(.a(new_n29_), .O(new_n30_));
  nand3  g11(.a(new_n28_), .b(x04), .c(x02), .O(new_n31_));
  oai21  g12(.a(new_n30_), .b(new_n28_), .c(new_n31_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(x03), .O(new_n33_));
  nor2   g14(.a(x03), .b(x00), .O(new_n34_));
  nor2   g15(.a(new_n34_), .b(new_n22_), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(new_n33_), .c(new_n27_), .O(z0));
  nand2  g17(.a(x13), .b(x09), .O(new_n37_));
  oai21  g18(.a(x10), .b(x08), .c(new_n37_), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(x04), .c(x02), .O(new_n39_));
  nand2  g20(.a(new_n31_), .b(x10), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x03), .O(new_n42_));
  inv1   g23(.a(x01), .O(new_n43_));
  inv1   g24(.a(x03), .O(new_n44_));
  inv1   g25(.a(x05), .O(new_n45_));
  aoi21  g26(.a(new_n44_), .b(new_n43_), .c(new_n45_), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n42_), .c(new_n22_), .O(z1));
  inv1   g28(.a(x11), .O(new_n48_));
  nor2   g29(.a(x10), .b(x08), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n30_), .c(new_n48_), .O(new_n50_));
  inv1   g31(.a(x10), .O(new_n51_));
  nand3  g32(.a(new_n48_), .b(new_n51_), .c(new_n28_), .O(new_n52_));
  nor2   g33(.a(new_n52_), .b(new_n29_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n50_), .c(x03), .O(new_n54_));
  nor2   g35(.a(x06), .b(x03), .O(new_n55_));
  nor2   g36(.a(new_n55_), .b(new_n22_), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n54_), .c(new_n27_), .O(z2));
  nand3  g38(.a(new_n20_), .b(x04), .c(x02), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n52_), .c(x12), .O(new_n59_));
  nand2  g40(.a(new_n20_), .b(new_n44_), .O(new_n60_));
  nand3  g41(.a(x04), .b(x03), .c(x02), .O(new_n61_));
  inv1   g42(.a(new_n61_), .O(new_n62_));
  nand4  g43(.a(new_n62_), .b(new_n49_), .c(new_n21_), .d(new_n48_), .O(new_n63_));
  nand4  g44(.a(new_n63_), .b(new_n60_), .c(new_n59_), .d(new_n27_), .O(z3));
  nand2  g45(.a(new_n25_), .b(new_n23_), .O(z4));
endmodule


