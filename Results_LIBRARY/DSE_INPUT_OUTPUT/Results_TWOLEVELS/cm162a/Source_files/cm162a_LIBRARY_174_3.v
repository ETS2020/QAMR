// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  inv1   g02(.a(x10), .O(new_n22_));
  nand3  g03(.a(x13), .b(new_n22_), .c(x09), .O(new_n23_));
  aoi21  g04(.a(new_n23_), .b(x08), .c(new_n21_), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  aoi22  g06(.a(new_n25_), .b(x08), .c(new_n24_), .d(x02), .O(new_n26_));
  nor2   g07(.a(x03), .b(x00), .O(new_n27_));
  nand2  g08(.a(new_n22_), .b(x05), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  oai21  g10(.a(new_n26_), .b(new_n20_), .c(new_n29_), .O(z0));
  nand3  g11(.a(new_n24_), .b(x03), .c(x02), .O(new_n31_));
  nor2   g12(.a(x03), .b(x01), .O(new_n32_));
  nor2   g13(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n31_), .O(z1));
  inv1   g15(.a(x08), .O(new_n35_));
  inv1   g16(.a(x11), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n23_), .c(new_n21_), .O(new_n38_));
  nor2   g19(.a(x08), .b(new_n21_), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(x02), .c(new_n36_), .O(new_n40_));
  aoi21  g21(.a(new_n38_), .b(x02), .c(new_n40_), .O(new_n41_));
  inv1   g22(.a(x05), .O(new_n42_));
  nor2   g23(.a(x06), .b(x03), .O(new_n43_));
  nor3   g24(.a(new_n43_), .b(x10), .c(new_n42_), .O(new_n44_));
  oai21  g25(.a(new_n41_), .b(new_n20_), .c(new_n44_), .O(z2));
  inv1   g26(.a(x12), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n36_), .c(new_n35_), .O(new_n47_));
  nand2  g28(.a(x13), .b(x09), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(x04), .c(x02), .O(new_n50_));
  oai21  g31(.a(new_n37_), .b(new_n25_), .c(x12), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n50_), .c(new_n20_), .O(new_n52_));
  nor2   g33(.a(x07), .b(x03), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n52_), .c(new_n22_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x05), .O(z3));
  oai22  g36(.a(new_n48_), .b(new_n21_), .c(new_n22_), .d(new_n42_), .O(z4));
endmodule


