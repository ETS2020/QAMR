// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_;
  inv1   g00(.a(x02), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(new_n21_), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nand3  g05(.a(x13), .b(x09), .c(x05), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x03), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x05), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x02), .O(new_n29_));
  nor2   g10(.a(x03), .b(x00), .O(new_n30_));
  nor2   g11(.a(new_n22_), .b(x04), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(x03), .c(new_n30_), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(new_n29_), .c(new_n24_), .O(z0));
  inv1   g14(.a(x05), .O(new_n34_));
  nand2  g15(.a(x13), .b(x09), .O(new_n35_));
  oai21  g16(.a(x10), .b(x08), .c(new_n35_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g18(.a(new_n22_), .b(x04), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x10), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(new_n37_), .c(new_n34_), .O(new_n40_));
  inv1   g21(.a(x10), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(x02), .O(new_n42_));
  oai21  g23(.a(new_n42_), .b(new_n40_), .c(x03), .O(new_n43_));
  inv1   g24(.a(x01), .O(new_n44_));
  nor2   g25(.a(new_n34_), .b(x03), .O(new_n45_));
  aoi22  g26(.a(new_n45_), .b(new_n44_), .c(new_n34_), .d(new_n20_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n43_), .O(z1));
  inv1   g28(.a(x11), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n21_), .c(x05), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n20_), .O(new_n50_));
  nand2  g31(.a(new_n48_), .b(new_n41_), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(x08), .c(new_n25_), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(x04), .c(x02), .O(new_n53_));
  nor2   g34(.a(x10), .b(x08), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x04), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x11), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x03), .O(new_n58_));
  nor2   g39(.a(x05), .b(new_n20_), .O(new_n59_));
  nor2   g40(.a(x06), .b(x03), .O(new_n60_));
  nor2   g41(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g42(.a(new_n61_), .b(new_n58_), .c(new_n50_), .O(z2));
  inv1   g43(.a(x12), .O(new_n63_));
  oai21  g44(.a(new_n63_), .b(new_n21_), .c(x05), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n20_), .O(new_n65_));
  nand3  g46(.a(new_n54_), .b(new_n63_), .c(new_n48_), .O(new_n66_));
  nand2  g47(.a(new_n66_), .b(new_n25_), .O(new_n67_));
  nand3  g48(.a(new_n67_), .b(x04), .c(x02), .O(new_n68_));
  oai21  g49(.a(new_n51_), .b(new_n38_), .c(x12), .O(new_n69_));
  nand2  g50(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g51(.a(new_n70_), .b(x03), .O(new_n71_));
  nor2   g52(.a(x07), .b(x03), .O(new_n72_));
  nor2   g53(.a(new_n72_), .b(new_n59_), .O(new_n73_));
  nand3  g54(.a(new_n73_), .b(new_n71_), .c(new_n65_), .O(z3));
  inv1   g55(.a(x04), .O(new_n75_));
  oai22  g56(.a(new_n35_), .b(new_n75_), .c(x05), .d(new_n20_), .O(z4));
endmodule


