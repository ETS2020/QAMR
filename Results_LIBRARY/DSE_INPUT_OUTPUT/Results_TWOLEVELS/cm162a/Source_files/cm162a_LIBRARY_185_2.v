// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x11), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  oai21  g03(.a(x03), .b(x00), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g05(.a(x04), .O(new_n25_));
  nand3  g06(.a(x13), .b(x11), .c(x09), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(x08), .c(new_n25_), .O(new_n27_));
  nand2  g08(.a(x04), .b(x02), .O(new_n28_));
  nand3  g09(.a(new_n28_), .b(x11), .c(x08), .O(new_n29_));
  inv1   g10(.a(new_n29_), .O(new_n30_));
  aoi21  g11(.a(new_n27_), .b(x02), .c(new_n30_), .O(new_n31_));
  oai21  g12(.a(new_n31_), .b(new_n20_), .c(new_n24_), .O(z0));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  oai21  g14(.a(x10), .b(x08), .c(new_n33_), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  inv1   g16(.a(x08), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x10), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n35_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  inv1   g21(.a(x01), .O(new_n41_));
  nand2  g22(.a(new_n20_), .b(new_n41_), .O(new_n42_));
  nand3  g23(.a(new_n42_), .b(new_n22_), .c(x05), .O(new_n43_));
  inv1   g24(.a(new_n43_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n40_), .O(z1));
  oai21  g26(.a(x11), .b(x10), .c(new_n33_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  inv1   g28(.a(x10), .O(new_n48_));
  nand3  g29(.a(new_n48_), .b(x04), .c(x02), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(x11), .c(x08), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x03), .O(new_n52_));
  inv1   g33(.a(x06), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n20_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n22_), .c(x05), .O(new_n55_));
  inv1   g36(.a(new_n55_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n52_), .O(z2));
  inv1   g38(.a(x12), .O(new_n58_));
  nand3  g39(.a(new_n58_), .b(new_n21_), .c(new_n48_), .O(new_n59_));
  aoi21  g40(.a(new_n59_), .b(new_n33_), .c(new_n25_), .O(new_n60_));
  inv1   g41(.a(new_n28_), .O(new_n61_));
  nor2   g42(.a(x11), .b(x10), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(new_n61_), .c(new_n58_), .O(new_n63_));
  aoi21  g44(.a(new_n60_), .b(x02), .c(new_n63_), .O(new_n64_));
  inv1   g45(.a(x07), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n20_), .O(new_n66_));
  nand3  g47(.a(new_n66_), .b(new_n22_), .c(x05), .O(new_n67_));
  inv1   g48(.a(new_n67_), .O(new_n68_));
  oai21  g49(.a(new_n64_), .b(new_n20_), .c(new_n68_), .O(z3));
  oai21  g50(.a(new_n33_), .b(new_n25_), .c(new_n22_), .O(z4));
endmodule


