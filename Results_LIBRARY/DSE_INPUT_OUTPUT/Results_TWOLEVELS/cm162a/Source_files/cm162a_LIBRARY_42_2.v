// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_;
  inv1   g00(.a(x11), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x10), .O(new_n21_));
  nand2  g02(.a(x03), .b(x02), .O(new_n22_));
  nand3  g03(.a(x13), .b(x09), .c(x04), .O(new_n23_));
  oai21  g04(.a(new_n23_), .b(new_n22_), .c(x05), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand2  g07(.a(new_n20_), .b(x10), .O(new_n27_));
  nand4  g08(.a(new_n27_), .b(new_n26_), .c(x04), .d(x02), .O(new_n28_));
  and2   g09(.a(x04), .b(x02), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n26_), .c(new_n28_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g12(.a(x00), .O(new_n32_));
  inv1   g13(.a(x03), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(new_n31_), .c(new_n25_), .d(new_n21_), .O(z0));
  nor2   g16(.a(x03), .b(x01), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n24_), .c(new_n21_), .O(new_n37_));
  inv1   g18(.a(x10), .O(new_n38_));
  nand3  g19(.a(new_n26_), .b(x04), .c(x02), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x11), .O(new_n40_));
  nand3  g21(.a(new_n29_), .b(new_n38_), .c(new_n26_), .O(new_n41_));
  oai21  g22(.a(new_n40_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(x03), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n37_), .O(z1));
  inv1   g25(.a(x05), .O(new_n45_));
  nand2  g26(.a(new_n21_), .b(new_n45_), .O(new_n46_));
  inv1   g27(.a(x06), .O(new_n47_));
  nor2   g28(.a(new_n20_), .b(new_n38_), .O(new_n48_));
  nor2   g29(.a(x10), .b(x03), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  xor2a  g31(.a(x11), .b(x10), .O(new_n51_));
  nand3  g32(.a(x13), .b(new_n38_), .c(x09), .O(new_n52_));
  oai21  g33(.a(new_n51_), .b(x08), .c(new_n52_), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(x04), .c(x02), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n40_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x03), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n50_), .c(new_n46_), .O(z2));
  inv1   g38(.a(x12), .O(new_n58_));
  nand3  g39(.a(new_n58_), .b(new_n26_), .c(x04), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n22_), .c(new_n38_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n20_), .O(new_n61_));
  nand3  g42(.a(new_n29_), .b(new_n20_), .c(new_n26_), .O(new_n62_));
  nand3  g43(.a(new_n62_), .b(x12), .c(x03), .O(new_n63_));
  inv1   g44(.a(x07), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n33_), .O(new_n65_));
  nand4  g46(.a(new_n65_), .b(new_n63_), .c(new_n61_), .d(new_n25_), .O(z3));
  nand2  g47(.a(new_n23_), .b(new_n21_), .O(z4));
endmodule


