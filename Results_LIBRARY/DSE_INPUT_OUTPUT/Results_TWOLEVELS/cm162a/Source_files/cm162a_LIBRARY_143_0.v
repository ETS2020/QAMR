// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_;
  nor2   g00(.a(x13), .b(x09), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n24_), .c(new_n22_), .O(new_n27_));
  oai21  g08(.a(x03), .b(x00), .c(x05), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(new_n21_), .O(new_n29_));
  nand2  g10(.a(x03), .b(x02), .O(new_n30_));
  nand3  g11(.a(x13), .b(x09), .c(x04), .O(new_n31_));
  oai21  g12(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(z0));
  inv1   g13(.a(x04), .O(new_n33_));
  nand2  g14(.a(x13), .b(x09), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n25_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n34_), .c(new_n33_), .O(new_n37_));
  aoi22  g18(.a(new_n37_), .b(x02), .c(new_n26_), .d(x10), .O(new_n38_));
  inv1   g19(.a(x05), .O(new_n39_));
  nor2   g20(.a(x03), .b(x01), .O(new_n40_));
  nor3   g21(.a(new_n40_), .b(new_n20_), .c(new_n39_), .O(new_n41_));
  oai21  g22(.a(new_n38_), .b(new_n22_), .c(new_n41_), .O(z1));
  inv1   g23(.a(x11), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n35_), .c(new_n25_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n34_), .c(new_n33_), .O(new_n45_));
  inv1   g26(.a(new_n23_), .O(new_n46_));
  nor2   g27(.a(x10), .b(x08), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n46_), .c(new_n43_), .O(new_n48_));
  aoi21  g29(.a(new_n45_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  nor3   g31(.a(new_n50_), .b(new_n20_), .c(new_n39_), .O(new_n51_));
  oai21  g32(.a(new_n49_), .b(new_n22_), .c(new_n51_), .O(z2));
  inv1   g33(.a(x12), .O(new_n53_));
  nand4  g34(.a(new_n53_), .b(new_n43_), .c(new_n35_), .d(new_n25_), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n34_), .c(new_n33_), .O(new_n55_));
  nor3   g36(.a(x11), .b(x10), .c(x08), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n46_), .c(new_n53_), .O(new_n57_));
  aoi21  g38(.a(new_n55_), .b(x02), .c(new_n57_), .O(new_n58_));
  nor2   g39(.a(x07), .b(x03), .O(new_n59_));
  nor3   g40(.a(new_n59_), .b(new_n20_), .c(new_n39_), .O(new_n60_));
  oai21  g41(.a(new_n58_), .b(new_n22_), .c(new_n60_), .O(z3));
  nand2  g42(.a(new_n31_), .b(new_n21_), .O(z4));
endmodule


