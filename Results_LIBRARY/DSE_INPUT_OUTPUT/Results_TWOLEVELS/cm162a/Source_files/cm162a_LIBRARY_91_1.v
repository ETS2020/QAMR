// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_;
  inv1   g00(.a(x13), .O(new_n20_));
  nor2   g01(.a(new_n20_), .b(x11), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  and2   g03(.a(x04), .b(x02), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  oai21  g05(.a(x03), .b(x00), .c(x05), .O(new_n25_));
  aoi21  g06(.a(new_n24_), .b(x03), .c(new_n25_), .O(new_n26_));
  nand3  g07(.a(x13), .b(x11), .c(x09), .O(new_n27_));
  oai21  g08(.a(new_n21_), .b(x08), .c(new_n27_), .O(new_n28_));
  nand4  g09(.a(new_n28_), .b(x04), .c(x03), .d(x02), .O(new_n29_));
  oai21  g10(.a(new_n26_), .b(new_n21_), .c(new_n29_), .O(z0));
  inv1   g11(.a(new_n21_), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  inv1   g13(.a(x11), .O(new_n33_));
  nand2  g14(.a(x13), .b(new_n33_), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(new_n32_), .c(new_n22_), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n27_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(x04), .c(x02), .O(new_n37_));
  nand3  g18(.a(new_n22_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x10), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x03), .O(new_n41_));
  inv1   g22(.a(x01), .O(new_n42_));
  inv1   g23(.a(x03), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand4  g25(.a(new_n44_), .b(new_n41_), .c(new_n31_), .d(x05), .O(z1));
  oai21  g26(.a(x06), .b(x03), .c(x05), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n31_), .O(new_n47_));
  inv1   g28(.a(x02), .O(new_n48_));
  inv1   g29(.a(new_n27_), .O(new_n49_));
  nand2  g30(.a(x11), .b(x10), .O(new_n50_));
  nand3  g31(.a(new_n20_), .b(new_n33_), .c(new_n32_), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n50_), .c(x08), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g34(.a(new_n38_), .b(x11), .O(new_n54_));
  oai21  g35(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x03), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n47_), .O(z2));
  inv1   g38(.a(x12), .O(new_n58_));
  nand4  g39(.a(new_n58_), .b(new_n33_), .c(new_n32_), .d(new_n22_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n27_), .O(new_n60_));
  nand3  g41(.a(new_n60_), .b(x04), .c(x02), .O(new_n61_));
  nand2  g42(.a(new_n20_), .b(x10), .O(new_n62_));
  nand4  g43(.a(new_n62_), .b(new_n23_), .c(new_n33_), .d(new_n22_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(x12), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(x03), .O(new_n66_));
  inv1   g47(.a(x07), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(new_n43_), .O(new_n68_));
  nand4  g49(.a(new_n68_), .b(new_n66_), .c(new_n31_), .d(x05), .O(z3));
  nand2  g50(.a(x09), .b(x04), .O(new_n70_));
  aoi21  g51(.a(new_n70_), .b(x11), .c(new_n20_), .O(z4));
endmodule


