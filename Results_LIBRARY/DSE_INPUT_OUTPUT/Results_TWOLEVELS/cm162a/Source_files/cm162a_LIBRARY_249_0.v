// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x10), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  oai21  g03(.a(x03), .b(x00), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g05(.a(x02), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(x08), .c(new_n25_), .O(new_n27_));
  inv1   g08(.a(x08), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(x02), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n27_), .c(x04), .O(new_n30_));
  nand3  g11(.a(x10), .b(x08), .c(new_n20_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(x03), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n24_), .O(z0));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  inv1   g16(.a(new_n35_), .O(new_n36_));
  nand3  g17(.a(x13), .b(x09), .c(x04), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n36_), .c(new_n25_), .O(new_n38_));
  nor2   g19(.a(x08), .b(new_n25_), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n21_), .c(x04), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n38_), .c(x03), .O(new_n41_));
  inv1   g22(.a(x01), .O(new_n42_));
  inv1   g23(.a(x03), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand4  g25(.a(new_n44_), .b(new_n41_), .c(new_n22_), .d(x05), .O(z1));
  nor2   g26(.a(x11), .b(x10), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n28_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n37_), .c(new_n25_), .O(new_n48_));
  inv1   g29(.a(x11), .O(new_n49_));
  aoi21  g30(.a(new_n35_), .b(x02), .c(new_n49_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n48_), .c(x03), .O(new_n51_));
  inv1   g32(.a(x06), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n43_), .O(new_n53_));
  nand4  g34(.a(new_n53_), .b(new_n51_), .c(new_n22_), .d(x05), .O(z2));
  inv1   g35(.a(x12), .O(new_n55_));
  nand3  g36(.a(new_n35_), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n37_), .c(new_n25_), .O(new_n57_));
  aoi21  g38(.a(new_n46_), .b(new_n39_), .c(new_n55_), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n57_), .c(x03), .O(new_n59_));
  inv1   g40(.a(x07), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n43_), .O(new_n61_));
  nand4  g42(.a(new_n61_), .b(new_n59_), .c(new_n22_), .d(x05), .O(z3));
  nand2  g43(.a(new_n37_), .b(new_n22_), .O(z4));
endmodule


