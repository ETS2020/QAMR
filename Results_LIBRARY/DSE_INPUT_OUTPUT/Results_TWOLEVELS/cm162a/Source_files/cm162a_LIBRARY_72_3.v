// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x08), .O(new_n20_));
  inv1   g01(.a(x06), .O(new_n21_));
  nand3  g02(.a(x13), .b(x09), .c(new_n21_), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  inv1   g06(.a(new_n25_), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n20_), .c(new_n24_), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x03), .O(new_n28_));
  nor2   g09(.a(x03), .b(x00), .O(new_n29_));
  nand2  g10(.a(new_n21_), .b(x05), .O(new_n30_));
  nor2   g11(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n28_), .O(z0));
  inv1   g13(.a(x03), .O(new_n33_));
  inv1   g14(.a(x04), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n20_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n22_), .c(new_n34_), .O(new_n37_));
  nor2   g18(.a(x08), .b(new_n34_), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(x02), .c(new_n35_), .O(new_n39_));
  aoi21  g20(.a(new_n37_), .b(x02), .c(new_n39_), .O(new_n40_));
  nor2   g21(.a(x03), .b(x01), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(new_n30_), .O(new_n42_));
  oai21  g23(.a(new_n40_), .b(new_n33_), .c(new_n42_), .O(z1));
  nand2  g24(.a(x13), .b(x09), .O(new_n44_));
  inv1   g25(.a(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n35_), .c(new_n20_), .O(new_n46_));
  oai21  g27(.a(new_n44_), .b(new_n33_), .c(new_n46_), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(x04), .c(x02), .O(new_n48_));
  oai21  g29(.a(new_n36_), .b(new_n25_), .c(x11), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n48_), .c(x03), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n21_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x05), .O(z2));
  inv1   g33(.a(x12), .O(new_n53_));
  nand4  g34(.a(new_n53_), .b(new_n45_), .c(new_n35_), .d(new_n20_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n44_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(x04), .c(x02), .O(new_n56_));
  oai21  g37(.a(new_n46_), .b(new_n25_), .c(x12), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n56_), .c(new_n33_), .O(new_n58_));
  nor2   g39(.a(x07), .b(x03), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n58_), .c(new_n21_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(x05), .O(z3));
  nand2  g42(.a(x06), .b(x05), .O(new_n62_));
  nand4  g43(.a(new_n62_), .b(x13), .c(x09), .d(x04), .O(new_n63_));
  inv1   g44(.a(new_n63_), .O(z4));
endmodule


