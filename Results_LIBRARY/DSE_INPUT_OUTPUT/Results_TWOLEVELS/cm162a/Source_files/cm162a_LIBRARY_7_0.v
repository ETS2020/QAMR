// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x12), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  oai21  g03(.a(x03), .b(x00), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g05(.a(x02), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(x08), .c(new_n25_), .O(new_n27_));
  inv1   g08(.a(x08), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(x02), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n27_), .c(x04), .O(new_n30_));
  nand3  g11(.a(x12), .b(x08), .c(new_n20_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(x03), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n24_), .O(z0));
  oai21  g15(.a(x10), .b(x08), .c(new_n26_), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  nand3  g17(.a(new_n28_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x10), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  inv1   g21(.a(x01), .O(new_n41_));
  inv1   g22(.a(x03), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand4  g24(.a(new_n43_), .b(new_n40_), .c(new_n22_), .d(x05), .O(z1));
  nor2   g25(.a(x11), .b(x10), .O(new_n45_));
  aoi22  g26(.a(new_n45_), .b(new_n28_), .c(x13), .d(x09), .O(new_n46_));
  nor2   g27(.a(x10), .b(x08), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x11), .O(new_n49_));
  oai21  g30(.a(new_n46_), .b(new_n25_), .c(new_n49_), .O(new_n50_));
  inv1   g31(.a(x06), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n42_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x05), .O(new_n53_));
  aoi21  g34(.a(new_n50_), .b(x03), .c(new_n53_), .O(new_n54_));
  nand3  g35(.a(x11), .b(new_n20_), .c(x03), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n52_), .c(x05), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x12), .O(new_n57_));
  oai21  g38(.a(new_n54_), .b(new_n20_), .c(new_n57_), .O(z2));
  nor2   g39(.a(x12), .b(x11), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n47_), .O(new_n60_));
  nand3  g41(.a(x13), .b(x09), .c(x04), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(new_n60_), .c(new_n25_), .O(new_n62_));
  nand3  g43(.a(new_n45_), .b(new_n28_), .c(x02), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(x12), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(x04), .O(new_n65_));
  oai21  g46(.a(new_n65_), .b(new_n62_), .c(x03), .O(new_n66_));
  inv1   g47(.a(x07), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(new_n42_), .O(new_n68_));
  nand4  g49(.a(new_n68_), .b(new_n66_), .c(new_n22_), .d(x05), .O(z3));
  nand2  g50(.a(new_n61_), .b(new_n22_), .O(z4));
endmodule


