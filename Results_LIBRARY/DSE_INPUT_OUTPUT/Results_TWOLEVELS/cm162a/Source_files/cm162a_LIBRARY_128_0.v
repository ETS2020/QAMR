// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x07), .O(new_n20_));
  inv1   g01(.a(x11), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g03(.a(x03), .b(x02), .O(new_n23_));
  nand3  g04(.a(x13), .b(x09), .c(x04), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n23_), .c(x05), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  inv1   g07(.a(x08), .O(new_n27_));
  and2   g08(.a(x04), .b(x02), .O(new_n28_));
  nand3  g09(.a(new_n27_), .b(x04), .c(x02), .O(new_n29_));
  oai21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g12(.a(x00), .O(new_n32_));
  inv1   g13(.a(x03), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(new_n31_), .c(new_n26_), .d(new_n22_), .O(z0));
  nand2  g16(.a(new_n29_), .b(x10), .O(new_n36_));
  nand3  g17(.a(new_n28_), .b(x13), .c(x09), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n36_), .c(new_n33_), .O(new_n38_));
  oai21  g19(.a(x03), .b(x01), .c(x05), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(new_n22_), .O(new_n40_));
  nor2   g21(.a(x11), .b(x07), .O(new_n41_));
  nor3   g22(.a(new_n41_), .b(x10), .c(x08), .O(new_n42_));
  nand4  g23(.a(new_n42_), .b(x04), .c(x03), .d(x02), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n40_), .O(z1));
  nand3  g25(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g26(.a(x10), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n27_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n45_), .c(x07), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n21_), .O(new_n49_));
  nand3  g30(.a(new_n28_), .b(new_n46_), .c(new_n27_), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(x11), .c(x03), .O(new_n51_));
  inv1   g32(.a(x06), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n33_), .O(new_n53_));
  nand4  g34(.a(new_n53_), .b(new_n51_), .c(new_n49_), .d(new_n26_), .O(z2));
  oai21  g35(.a(new_n21_), .b(new_n33_), .c(new_n20_), .O(new_n55_));
  inv1   g36(.a(x12), .O(new_n56_));
  nor3   g37(.a(x11), .b(x10), .c(x08), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n28_), .c(new_n56_), .O(new_n58_));
  nand3  g39(.a(new_n56_), .b(new_n21_), .c(new_n46_), .O(new_n59_));
  nor2   g40(.a(new_n59_), .b(new_n29_), .O(new_n60_));
  oai21  g41(.a(new_n60_), .b(new_n58_), .c(x03), .O(new_n61_));
  nand3  g42(.a(new_n61_), .b(new_n55_), .c(new_n26_), .O(z3));
  nand2  g43(.a(new_n24_), .b(new_n22_), .O(z4));
endmodule


