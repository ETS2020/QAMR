// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  nor2   g00(.a(x11), .b(x08), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  oai21  g02(.a(x03), .b(x00), .c(x05), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  nand4  g05(.a(x11), .b(new_n24_), .c(x04), .d(x02), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x08), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x03), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n23_), .O(z0));
  inv1   g12(.a(x03), .O(new_n32_));
  inv1   g13(.a(x04), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand3  g15(.a(x11), .b(new_n34_), .c(new_n24_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n26_), .c(new_n33_), .O(new_n36_));
  nand3  g17(.a(new_n24_), .b(x04), .c(x02), .O(new_n37_));
  and2   g18(.a(new_n37_), .b(x10), .O(new_n38_));
  aoi21  g19(.a(new_n36_), .b(x02), .c(new_n38_), .O(new_n39_));
  inv1   g20(.a(x05), .O(new_n40_));
  nor2   g21(.a(x03), .b(x01), .O(new_n41_));
  nor3   g22(.a(new_n41_), .b(new_n20_), .c(new_n40_), .O(new_n42_));
  oai21  g23(.a(new_n39_), .b(new_n32_), .c(new_n42_), .O(z1));
  nand2  g24(.a(x11), .b(x10), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(x08), .c(new_n26_), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(x04), .c(x02), .O(new_n46_));
  nand2  g27(.a(new_n37_), .b(x11), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x03), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  nor3   g31(.a(new_n50_), .b(new_n20_), .c(new_n40_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(z2));
  nand4  g33(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n53_));
  inv1   g34(.a(new_n53_), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(x12), .c(x03), .O(new_n55_));
  nor2   g36(.a(x07), .b(x03), .O(new_n56_));
  nor3   g37(.a(new_n56_), .b(new_n20_), .c(new_n40_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(z3));
  nand3  g39(.a(new_n21_), .b(x13), .c(x09), .O(new_n59_));
  nor2   g40(.a(new_n59_), .b(new_n33_), .O(z4));
endmodule


