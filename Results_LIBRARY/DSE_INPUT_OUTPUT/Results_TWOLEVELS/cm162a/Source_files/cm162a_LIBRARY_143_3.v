// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  and2   g00(.a(x13), .b(x09), .O(z4));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand3  g04(.a(new_n21_), .b(x04), .c(x02), .O(new_n24_));
  oai21  g05(.a(new_n23_), .b(new_n21_), .c(new_n24_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x03), .O(new_n26_));
  inv1   g07(.a(x05), .O(new_n27_));
  nor2   g08(.a(x03), .b(x00), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  aoi21  g10(.a(new_n29_), .b(new_n26_), .c(z4), .O(z0));
  or2    g11(.a(z4), .b(x05), .O(new_n31_));
  nand2  g12(.a(new_n24_), .b(x10), .O(new_n32_));
  nor2   g13(.a(x10), .b(x08), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n23_), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(x03), .O(new_n36_));
  nor2   g17(.a(x03), .b(x01), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(z4), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n36_), .c(new_n31_), .O(z1));
  inv1   g20(.a(x11), .O(new_n40_));
  aoi21  g21(.a(new_n33_), .b(new_n23_), .c(new_n40_), .O(new_n41_));
  inv1   g22(.a(x10), .O(new_n42_));
  nand3  g23(.a(new_n40_), .b(new_n42_), .c(new_n21_), .O(new_n43_));
  nor2   g24(.a(new_n43_), .b(new_n22_), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(new_n41_), .c(x03), .O(new_n45_));
  nor2   g26(.a(x06), .b(x03), .O(new_n46_));
  nor2   g27(.a(new_n46_), .b(z4), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n45_), .c(new_n31_), .O(z2));
  inv1   g29(.a(x12), .O(new_n49_));
  nor3   g30(.a(x11), .b(x10), .c(x08), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n23_), .c(new_n49_), .O(new_n51_));
  nand3  g32(.a(new_n49_), .b(new_n40_), .c(new_n42_), .O(new_n52_));
  nor2   g33(.a(new_n52_), .b(new_n24_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n51_), .c(x03), .O(new_n54_));
  nor2   g35(.a(x07), .b(x03), .O(new_n55_));
  nor2   g36(.a(new_n55_), .b(z4), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n54_), .c(new_n31_), .O(z3));
endmodule


