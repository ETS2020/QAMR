// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x13), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  nand3  g03(.a(new_n22_), .b(x04), .c(x02), .O(new_n23_));
  inv1   g04(.a(x02), .O(new_n24_));
  inv1   g05(.a(x04), .O(new_n25_));
  oai21  g06(.a(new_n25_), .b(new_n24_), .c(x08), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n23_), .c(new_n20_), .O(new_n27_));
  nor2   g08(.a(x03), .b(x00), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(x09), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x05), .O(z0));
  inv1   g11(.a(x09), .O(new_n31_));
  nor2   g12(.a(x10), .b(x08), .O(new_n32_));
  oai21  g13(.a(new_n32_), .b(x13), .c(x04), .O(new_n33_));
  inv1   g14(.a(x08), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(x10), .O(new_n36_));
  oai21  g17(.a(new_n33_), .b(new_n24_), .c(new_n36_), .O(new_n37_));
  nor2   g18(.a(x03), .b(x01), .O(new_n38_));
  aoi21  g19(.a(new_n37_), .b(x03), .c(new_n38_), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n31_), .c(x05), .O(z1));
  inv1   g21(.a(x10), .O(new_n41_));
  inv1   g22(.a(x11), .O(new_n42_));
  nand3  g23(.a(new_n42_), .b(new_n41_), .c(new_n34_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(x04), .c(x02), .O(new_n45_));
  nand3  g26(.a(new_n32_), .b(x04), .c(x02), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x11), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n45_), .c(new_n20_), .O(new_n48_));
  nor2   g29(.a(x06), .b(x03), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n48_), .c(x09), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x05), .O(z2));
  nor2   g32(.a(x12), .b(x11), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n32_), .O(new_n53_));
  nand2  g34(.a(x13), .b(x09), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(x04), .c(x02), .O(new_n56_));
  aoi21  g37(.a(new_n42_), .b(new_n41_), .c(new_n31_), .O(new_n57_));
  oai21  g38(.a(new_n57_), .b(new_n35_), .c(x12), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(x03), .O(new_n60_));
  inv1   g41(.a(x05), .O(new_n61_));
  nor2   g42(.a(x07), .b(x03), .O(new_n62_));
  nor3   g43(.a(new_n62_), .b(new_n31_), .c(new_n61_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n60_), .O(z3));
  oai22  g45(.a(new_n54_), .b(new_n25_), .c(x09), .d(new_n61_), .O(z4));
endmodule


