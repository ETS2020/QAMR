// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x07), .O(new_n20_));
  nor2   g01(.a(x12), .b(new_n20_), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(x03), .b(x02), .O(new_n23_));
  nand3  g04(.a(x13), .b(x09), .c(x04), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n23_), .c(x05), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  inv1   g07(.a(x08), .O(new_n27_));
  nand2  g08(.a(x04), .b(x02), .O(new_n28_));
  inv1   g09(.a(new_n28_), .O(new_n29_));
  nand3  g10(.a(new_n27_), .b(x04), .c(x02), .O(new_n30_));
  oai21  g11(.a(new_n29_), .b(new_n27_), .c(new_n30_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x03), .O(new_n32_));
  nor2   g13(.a(x03), .b(x00), .O(new_n33_));
  nor2   g14(.a(new_n33_), .b(new_n21_), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(new_n32_), .c(new_n26_), .O(z0));
  nand2  g16(.a(x13), .b(x09), .O(new_n36_));
  oai21  g17(.a(x10), .b(x08), .c(new_n36_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g19(.a(new_n30_), .b(x10), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x03), .O(new_n41_));
  inv1   g22(.a(x05), .O(new_n42_));
  nor2   g23(.a(x03), .b(x01), .O(new_n43_));
  nor2   g24(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n41_), .c(new_n21_), .O(z1));
  inv1   g26(.a(x11), .O(new_n46_));
  nor2   g27(.a(x10), .b(x08), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n29_), .c(new_n46_), .O(new_n48_));
  inv1   g29(.a(x10), .O(new_n49_));
  nand3  g30(.a(new_n46_), .b(new_n49_), .c(new_n27_), .O(new_n50_));
  nor2   g31(.a(new_n50_), .b(new_n28_), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n48_), .c(x03), .O(new_n52_));
  nor2   g33(.a(x06), .b(x03), .O(new_n53_));
  nor2   g34(.a(new_n53_), .b(new_n21_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n52_), .c(new_n26_), .O(z2));
  inv1   g36(.a(x12), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n46_), .c(new_n49_), .O(new_n57_));
  oai21  g38(.a(new_n57_), .b(new_n30_), .c(x03), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n20_), .O(new_n59_));
  nor2   g40(.a(new_n51_), .b(new_n56_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(x03), .O(new_n61_));
  nand3  g42(.a(new_n61_), .b(new_n59_), .c(new_n26_), .O(z3));
  nand2  g43(.a(new_n24_), .b(new_n22_), .O(z4));
endmodule


