// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_;
  nor2   g00(.a(x13), .b(x08), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  oai21  g02(.a(x03), .b(x00), .c(x05), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  inv1   g05(.a(x09), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x08), .O(new_n26_));
  nand4  g07(.a(new_n26_), .b(x13), .c(x04), .d(x02), .O(new_n27_));
  and2   g08(.a(x04), .b(x02), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n24_), .c(new_n27_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x03), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n23_), .O(z0));
  nand2  g12(.a(x13), .b(x09), .O(new_n32_));
  oai21  g13(.a(x10), .b(x08), .c(new_n32_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  nand3  g15(.a(new_n24_), .b(x04), .c(x02), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(x10), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x03), .O(new_n38_));
  inv1   g19(.a(x05), .O(new_n39_));
  nor2   g20(.a(x03), .b(x01), .O(new_n40_));
  nor3   g21(.a(new_n40_), .b(new_n20_), .c(new_n39_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n38_), .O(z1));
  inv1   g23(.a(x03), .O(new_n43_));
  inv1   g24(.a(x04), .O(new_n44_));
  inv1   g25(.a(x10), .O(new_n45_));
  inv1   g26(.a(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n45_), .c(new_n24_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n32_), .c(new_n44_), .O(new_n48_));
  aoi21  g29(.a(x13), .b(x10), .c(x08), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n28_), .c(new_n46_), .O(new_n50_));
  aoi21  g31(.a(new_n48_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g32(.a(x06), .b(x03), .O(new_n52_));
  nor3   g33(.a(new_n52_), .b(new_n20_), .c(new_n39_), .O(new_n53_));
  oai21  g34(.a(new_n51_), .b(new_n43_), .c(new_n53_), .O(z2));
  inv1   g35(.a(x12), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n46_), .c(new_n45_), .d(new_n24_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n25_), .O(new_n57_));
  nand3  g38(.a(new_n57_), .b(x04), .c(x02), .O(new_n58_));
  nand3  g39(.a(new_n28_), .b(new_n46_), .c(new_n45_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(x12), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(new_n58_), .c(new_n43_), .O(new_n61_));
  inv1   g42(.a(x07), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(new_n43_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(x05), .O(new_n64_));
  oai21  g45(.a(new_n64_), .b(new_n61_), .c(x13), .O(new_n65_));
  nand2  g46(.a(x12), .b(x03), .O(new_n66_));
  nand3  g47(.a(new_n66_), .b(new_n63_), .c(x05), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(x08), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n65_), .O(z3));
  oai21  g50(.a(new_n32_), .b(new_n44_), .c(new_n21_), .O(z4));
endmodule


