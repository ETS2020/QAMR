// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_;
  nand2  g00(.a(x13), .b(x08), .O(new_n20_));
  oai21  g01(.a(x03), .b(x00), .c(x05), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(x13), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(new_n23_), .c(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x03), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n22_), .O(z0));
  inv1   g11(.a(x10), .O(new_n31_));
  inv1   g12(.a(new_n27_), .O(new_n32_));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x10), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(new_n26_), .c(x04), .d(x02), .O(new_n35_));
  oai21  g16(.a(new_n32_), .b(new_n31_), .c(new_n35_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x03), .O(new_n37_));
  inv1   g18(.a(x01), .O(new_n38_));
  inv1   g19(.a(x03), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g21(.a(new_n40_), .b(new_n37_), .c(new_n20_), .d(x05), .O(z1));
  oai21  g22(.a(x11), .b(x10), .c(new_n33_), .O(new_n42_));
  nand3  g23(.a(new_n42_), .b(x04), .c(x02), .O(new_n43_));
  nand2  g24(.a(x11), .b(x10), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n43_), .c(x08), .O(new_n45_));
  inv1   g26(.a(x11), .O(new_n46_));
  nor2   g27(.a(new_n32_), .b(new_n46_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n45_), .c(x03), .O(new_n48_));
  inv1   g29(.a(x06), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n39_), .O(new_n50_));
  nand4  g31(.a(new_n50_), .b(new_n48_), .c(new_n20_), .d(x05), .O(z2));
  inv1   g32(.a(x12), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(new_n46_), .c(new_n31_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(x04), .c(x02), .O(new_n55_));
  nand2  g36(.a(new_n46_), .b(new_n31_), .O(new_n56_));
  oai21  g37(.a(new_n56_), .b(new_n24_), .c(x12), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n55_), .c(new_n39_), .O(new_n58_));
  inv1   g39(.a(x07), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n39_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(x05), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(new_n58_), .c(new_n26_), .O(new_n62_));
  nand3  g43(.a(x12), .b(x08), .c(x03), .O(new_n63_));
  nand3  g44(.a(new_n63_), .b(new_n60_), .c(x05), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n23_), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n62_), .O(z3));
  nand2  g47(.a(x09), .b(x04), .O(new_n67_));
  aoi21  g48(.a(new_n67_), .b(new_n26_), .c(new_n23_), .O(z4));
endmodule


