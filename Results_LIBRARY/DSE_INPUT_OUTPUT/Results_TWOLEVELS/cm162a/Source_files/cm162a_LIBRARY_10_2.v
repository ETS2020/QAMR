// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_;
  inv1   g00(.a(x13), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x04), .O(new_n21_));
  oai21  g02(.a(x03), .b(x00), .c(x05), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g04(.a(x09), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  inv1   g07(.a(x02), .O(new_n27_));
  nand2  g08(.a(x08), .b(new_n27_), .O(new_n28_));
  aoi21  g09(.a(new_n28_), .b(new_n26_), .c(new_n20_), .O(new_n29_));
  inv1   g10(.a(x08), .O(new_n30_));
  nor2   g11(.a(new_n30_), .b(x04), .O(new_n31_));
  oai21  g12(.a(new_n31_), .b(new_n29_), .c(x03), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n23_), .O(z0));
  inv1   g14(.a(x03), .O(new_n34_));
  oai21  g15(.a(x03), .b(x01), .c(x05), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n21_), .O(new_n36_));
  nor2   g17(.a(x10), .b(x08), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(x09), .c(x04), .O(new_n38_));
  oai21  g19(.a(x08), .b(new_n27_), .c(x10), .O(new_n39_));
  oai21  g20(.a(new_n38_), .b(new_n27_), .c(new_n39_), .O(new_n40_));
  inv1   g21(.a(x10), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(x04), .O(new_n42_));
  aoi21  g23(.a(new_n40_), .b(x13), .c(new_n42_), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(new_n34_), .c(new_n36_), .O(z1));
  inv1   g25(.a(x04), .O(new_n45_));
  inv1   g26(.a(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n41_), .c(new_n30_), .O(new_n47_));
  nand2  g28(.a(x13), .b(x09), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  nor2   g30(.a(new_n45_), .b(new_n27_), .O(new_n50_));
  aoi21  g31(.a(x13), .b(x10), .c(x08), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n50_), .c(new_n46_), .O(new_n52_));
  aoi21  g33(.a(new_n49_), .b(x02), .c(new_n52_), .O(new_n53_));
  inv1   g34(.a(x06), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n34_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n21_), .c(x05), .O(new_n56_));
  inv1   g37(.a(new_n56_), .O(new_n57_));
  oai21  g38(.a(new_n53_), .b(new_n34_), .c(new_n57_), .O(z2));
  nor2   g39(.a(x12), .b(x11), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n37_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n48_), .O(new_n61_));
  nand3  g42(.a(new_n61_), .b(x04), .c(x02), .O(new_n62_));
  oai21  g43(.a(x11), .b(x10), .c(x13), .O(new_n63_));
  nand4  g44(.a(new_n63_), .b(new_n30_), .c(x04), .d(x02), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(x12), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nand2  g47(.a(new_n66_), .b(x03), .O(new_n67_));
  inv1   g48(.a(x07), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n34_), .O(new_n69_));
  nand3  g50(.a(new_n69_), .b(new_n21_), .c(x05), .O(new_n70_));
  inv1   g51(.a(new_n70_), .O(new_n71_));
  nand2  g52(.a(new_n71_), .b(new_n67_), .O(z3));
  nand3  g53(.a(x13), .b(x09), .c(x04), .O(new_n73_));
  inv1   g54(.a(new_n73_), .O(z4));
endmodule


