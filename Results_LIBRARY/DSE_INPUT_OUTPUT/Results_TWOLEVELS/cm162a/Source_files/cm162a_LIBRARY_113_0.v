// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x10), .O(new_n20_));
  inv1   g01(.a(x12), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(x03), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  and2   g06(.a(x04), .b(x02), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x13), .c(x09), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n25_), .c(new_n23_), .O(new_n28_));
  oai21  g09(.a(x03), .b(x00), .c(x05), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(new_n22_), .O(new_n30_));
  aoi21  g11(.a(new_n21_), .b(new_n20_), .c(x08), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n30_), .O(z0));
  inv1   g14(.a(x05), .O(new_n34_));
  nand2  g15(.a(x03), .b(x02), .O(new_n35_));
  nand3  g16(.a(x13), .b(x09), .c(x04), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  inv1   g19(.a(x02), .O(new_n39_));
  inv1   g20(.a(x08), .O(new_n40_));
  nand4  g21(.a(x12), .b(new_n20_), .c(new_n40_), .d(x04), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand3  g23(.a(new_n40_), .b(x04), .c(x02), .O(new_n43_));
  and2   g24(.a(new_n43_), .b(x10), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(new_n42_), .c(x03), .O(new_n45_));
  inv1   g26(.a(x01), .O(new_n46_));
  nand2  g27(.a(new_n23_), .b(new_n46_), .O(new_n47_));
  nand4  g28(.a(new_n47_), .b(new_n45_), .c(new_n38_), .d(new_n22_), .O(z1));
  oai22  g29(.a(new_n36_), .b(new_n35_), .c(x06), .d(x03), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n34_), .c(new_n22_), .O(new_n50_));
  inv1   g31(.a(x11), .O(new_n51_));
  nand2  g32(.a(new_n43_), .b(x12), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n20_), .c(new_n51_), .O(new_n53_));
  nand3  g34(.a(x12), .b(new_n51_), .c(new_n20_), .O(new_n54_));
  nor2   g35(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  oai21  g36(.a(new_n55_), .b(new_n53_), .c(x03), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n50_), .O(z2));
  oai22  g38(.a(new_n36_), .b(new_n35_), .c(x07), .d(x03), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n34_), .c(new_n22_), .O(new_n59_));
  nand4  g40(.a(new_n26_), .b(new_n51_), .c(new_n20_), .d(new_n40_), .O(new_n60_));
  nand3  g41(.a(new_n60_), .b(x12), .c(x03), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n59_), .O(z3));
  nand2  g43(.a(new_n36_), .b(new_n22_), .O(z4));
endmodule


