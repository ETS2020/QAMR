// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_;
  inv1   g00(.a(x05), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x08), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n24_), .c(new_n21_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n20_), .c(x10), .O(new_n28_));
  nand2  g09(.a(x05), .b(x00), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n21_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n28_), .O(z0));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(x05), .b(x01), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n21_), .O(new_n34_));
  inv1   g15(.a(x08), .O(new_n35_));
  inv1   g16(.a(new_n25_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n22_), .c(new_n35_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(x03), .c(new_n20_), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n32_), .c(new_n34_), .O(z1));
  inv1   g20(.a(x11), .O(new_n40_));
  nand3  g21(.a(new_n36_), .b(x13), .c(x09), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n40_), .c(new_n21_), .O(new_n42_));
  oai21  g23(.a(new_n42_), .b(new_n20_), .c(x10), .O(new_n43_));
  nand2  g24(.a(x06), .b(x05), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n21_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n43_), .O(z2));
  inv1   g27(.a(x12), .O(new_n47_));
  aoi21  g28(.a(new_n41_), .b(new_n47_), .c(new_n21_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n20_), .c(x10), .O(new_n49_));
  nand2  g30(.a(x07), .b(x05), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n21_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(z3));
  inv1   g33(.a(x04), .O(new_n53_));
  oai22  g34(.a(new_n22_), .b(new_n53_), .c(x10), .d(new_n21_), .O(z4));
endmodule


