// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(x08), .c(new_n21_), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  and2   g05(.a(x04), .b(x02), .O(new_n25_));
  nor2   g06(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  aoi21  g07(.a(new_n23_), .b(x02), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n20_), .c(x05), .O(z0));
  oai22  g09(.a(new_n22_), .b(new_n20_), .c(x10), .d(x08), .O(new_n29_));
  nand3  g10(.a(new_n29_), .b(x04), .c(x02), .O(new_n30_));
  nand3  g11(.a(new_n24_), .b(x04), .c(x02), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x10), .O(new_n32_));
  nand4  g13(.a(new_n32_), .b(new_n30_), .c(x05), .d(x03), .O(z1));
  inv1   g14(.a(x10), .O(new_n34_));
  inv1   g15(.a(x11), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(new_n34_), .c(new_n24_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n22_), .c(new_n21_), .O(new_n37_));
  nor2   g18(.a(x10), .b(x08), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n25_), .c(new_n35_), .O(new_n39_));
  aoi21  g20(.a(new_n37_), .b(x02), .c(new_n39_), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n20_), .c(x05), .O(z2));
  inv1   g22(.a(x12), .O(new_n42_));
  nand4  g23(.a(new_n42_), .b(new_n35_), .c(new_n34_), .d(new_n24_), .O(new_n43_));
  aoi21  g24(.a(new_n43_), .b(new_n22_), .c(new_n21_), .O(new_n44_));
  nor3   g25(.a(x11), .b(x10), .c(x08), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n25_), .c(new_n42_), .O(new_n46_));
  aoi21  g27(.a(new_n44_), .b(x02), .c(new_n46_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n20_), .c(x05), .O(z3));
  inv1   g29(.a(x05), .O(new_n49_));
  oai22  g30(.a(new_n22_), .b(new_n21_), .c(new_n49_), .d(x03), .O(z4));
endmodule


