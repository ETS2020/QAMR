// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_;
  nand2  g00(.a(x04), .b(x02), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x08), .O(new_n21_));
  nor2   g02(.a(x12), .b(x10), .O(new_n22_));
  nand3  g03(.a(x13), .b(x12), .c(x09), .O(new_n23_));
  oai21  g04(.a(new_n22_), .b(x08), .c(new_n23_), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(x04), .c(x02), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n21_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand4  g11(.a(new_n30_), .b(new_n27_), .c(x12), .d(x05), .O(z0));
  inv1   g12(.a(x08), .O(new_n32_));
  inv1   g13(.a(x10), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand2  g15(.a(x13), .b(x09), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(x04), .c(x02), .O(new_n37_));
  nand3  g18(.a(new_n32_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x10), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(new_n37_), .c(new_n29_), .O(new_n40_));
  nor2   g21(.a(x03), .b(x01), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n40_), .c(x12), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(x05), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n33_), .c(new_n32_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n35_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  oai21  g28(.a(new_n34_), .b(new_n20_), .c(x11), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n47_), .c(new_n29_), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n49_), .c(x12), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x05), .O(z2));
  inv1   g33(.a(x12), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x05), .O(new_n54_));
  inv1   g35(.a(new_n23_), .O(new_n55_));
  oai21  g36(.a(new_n53_), .b(new_n44_), .c(new_n33_), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n32_), .c(new_n55_), .O(new_n57_));
  nand4  g38(.a(new_n57_), .b(new_n32_), .c(x04), .d(x02), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x03), .O(new_n59_));
  inv1   g40(.a(x07), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n29_), .O(new_n61_));
  nand4  g42(.a(new_n61_), .b(new_n59_), .c(new_n54_), .d(x05), .O(z3));
  inv1   g43(.a(x04), .O(new_n63_));
  oai21  g44(.a(new_n35_), .b(new_n63_), .c(new_n54_), .O(z4));
endmodule


