// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_;
  nand2  g00(.a(x12), .b(x07), .O(new_n20_));
  nand2  g01(.a(x03), .b(x02), .O(new_n21_));
  nand3  g02(.a(x13), .b(x09), .c(x04), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(new_n21_), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  and2   g06(.a(x04), .b(x02), .O(new_n26_));
  nand3  g07(.a(new_n25_), .b(x04), .c(x02), .O(new_n27_));
  oai21  g08(.a(new_n26_), .b(new_n25_), .c(new_n27_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g10(.a(new_n20_), .O(new_n30_));
  nor2   g11(.a(x03), .b(x00), .O(new_n31_));
  nor2   g12(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(new_n29_), .c(new_n24_), .O(z0));
  inv1   g14(.a(x02), .O(new_n34_));
  nand2  g15(.a(new_n27_), .b(x10), .O(new_n35_));
  nand3  g16(.a(new_n26_), .b(x13), .c(x09), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  oai21  g18(.a(x03), .b(x01), .c(x05), .O(new_n38_));
  aoi21  g19(.a(new_n37_), .b(x03), .c(new_n38_), .O(new_n39_));
  inv1   g20(.a(x07), .O(new_n40_));
  inv1   g21(.a(x11), .O(new_n41_));
  oai21  g22(.a(x12), .b(new_n41_), .c(new_n40_), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(x12), .c(x10), .O(new_n43_));
  nand4  g24(.a(new_n43_), .b(new_n25_), .c(x04), .d(x03), .O(new_n44_));
  oai22  g25(.a(new_n44_), .b(new_n34_), .c(new_n39_), .d(new_n30_), .O(z1));
  nand2  g26(.a(x13), .b(x09), .O(new_n46_));
  inv1   g27(.a(x10), .O(new_n47_));
  nand3  g28(.a(new_n41_), .b(new_n47_), .c(new_n25_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g30(.a(new_n49_), .b(x04), .c(x03), .d(x02), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x05), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n20_), .O(new_n52_));
  nor2   g33(.a(x10), .b(x08), .O(new_n53_));
  aoi21  g34(.a(new_n53_), .b(new_n26_), .c(new_n41_), .O(new_n54_));
  oai21  g35(.a(x06), .b(x03), .c(new_n20_), .O(new_n55_));
  aoi21  g36(.a(new_n54_), .b(x03), .c(new_n55_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n52_), .O(z2));
  nand3  g38(.a(x12), .b(x11), .c(new_n40_), .O(new_n58_));
  inv1   g39(.a(x12), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n41_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand4  g42(.a(new_n61_), .b(new_n47_), .c(new_n25_), .d(x04), .O(new_n62_));
  inv1   g43(.a(new_n62_), .O(new_n63_));
  nand3  g44(.a(new_n63_), .b(x03), .c(x02), .O(new_n64_));
  inv1   g45(.a(x03), .O(new_n65_));
  aoi21  g46(.a(new_n53_), .b(new_n26_), .c(new_n59_), .O(new_n66_));
  oai21  g47(.a(new_n66_), .b(new_n65_), .c(new_n40_), .O(new_n67_));
  nand3  g48(.a(new_n67_), .b(new_n64_), .c(new_n24_), .O(z3));
  nand2  g49(.a(new_n22_), .b(new_n20_), .O(z4));
endmodule


