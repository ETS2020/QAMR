// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  nand3  g02(.a(x13), .b(x09), .c(x06), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(x08), .c(new_n21_), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  aoi22  g05(.a(new_n24_), .b(x08), .c(new_n23_), .d(x02), .O(new_n25_));
  inv1   g06(.a(x06), .O(new_n26_));
  nand2  g07(.a(x09), .b(new_n26_), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  nand2  g09(.a(new_n20_), .b(new_n28_), .O(new_n29_));
  nand3  g10(.a(new_n29_), .b(new_n27_), .c(x05), .O(new_n30_));
  inv1   g11(.a(new_n30_), .O(new_n31_));
  oai21  g12(.a(new_n25_), .b(new_n20_), .c(new_n31_), .O(z0));
  inv1   g13(.a(x08), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(x10), .O(new_n35_));
  inv1   g16(.a(x10), .O(new_n36_));
  nand4  g17(.a(new_n36_), .b(new_n33_), .c(x04), .d(x02), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n35_), .c(new_n20_), .O(new_n38_));
  oai21  g19(.a(x03), .b(x01), .c(x05), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(new_n27_), .O(new_n40_));
  inv1   g21(.a(new_n22_), .O(new_n41_));
  nand4  g22(.a(new_n41_), .b(x04), .c(x03), .d(x02), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n40_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n36_), .c(new_n33_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n22_), .c(new_n21_), .O(new_n46_));
  aoi22  g27(.a(new_n46_), .b(x02), .c(new_n37_), .d(x11), .O(new_n47_));
  inv1   g28(.a(x05), .O(new_n48_));
  inv1   g29(.a(x09), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(x03), .O(new_n50_));
  aoi22  g31(.a(new_n50_), .b(new_n26_), .c(new_n27_), .d(new_n48_), .O(new_n51_));
  oai21  g32(.a(new_n47_), .b(new_n20_), .c(new_n51_), .O(z2));
  oai21  g33(.a(new_n45_), .b(new_n24_), .c(x12), .O(new_n53_));
  nor2   g34(.a(x08), .b(new_n21_), .O(new_n54_));
  nor3   g35(.a(x12), .b(x11), .c(x10), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n54_), .c(x02), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n53_), .c(new_n20_), .O(new_n57_));
  oai21  g38(.a(x07), .b(x03), .c(x05), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n57_), .c(new_n27_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n42_), .O(z3));
  nand4  g41(.a(x13), .b(x09), .c(x06), .d(x04), .O(new_n61_));
  inv1   g42(.a(new_n61_), .O(z4));
endmodule


