// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  inv1   g00(.a(x10), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  inv1   g05(.a(x02), .O(new_n25_));
  inv1   g06(.a(x04), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n25_), .c(x08), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n24_), .c(new_n21_), .O(new_n28_));
  nor2   g09(.a(x03), .b(x00), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(new_n20_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x05), .O(z0));
  nand4  g12(.a(new_n23_), .b(new_n20_), .c(x04), .d(x03), .O(new_n32_));
  nor2   g13(.a(x03), .b(x01), .O(new_n33_));
  nand2  g14(.a(new_n20_), .b(x05), .O(new_n34_));
  nor2   g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g16(.a(new_n32_), .b(new_n25_), .c(new_n35_), .O(z1));
  oai21  g17(.a(x11), .b(x08), .c(new_n22_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(x04), .c(x02), .O(new_n38_));
  inv1   g19(.a(x08), .O(new_n39_));
  nand3  g20(.a(new_n39_), .b(x04), .c(x02), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x11), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n38_), .c(new_n21_), .O(new_n42_));
  nor2   g23(.a(x06), .b(x03), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(new_n42_), .c(new_n20_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x05), .O(z2));
  nor2   g26(.a(x12), .b(x11), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n39_), .O(new_n47_));
  nand3  g28(.a(x13), .b(new_n20_), .c(x09), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(x04), .c(x02), .O(new_n50_));
  nand2  g31(.a(x11), .b(new_n20_), .O(new_n51_));
  nand4  g32(.a(new_n51_), .b(new_n39_), .c(x04), .d(x02), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x12), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x03), .O(new_n55_));
  inv1   g36(.a(x07), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n21_), .O(new_n57_));
  nand4  g38(.a(new_n57_), .b(new_n55_), .c(new_n20_), .d(x05), .O(z3));
  nand2  g39(.a(x10), .b(x05), .O(new_n59_));
  oai21  g40(.a(new_n22_), .b(new_n26_), .c(new_n59_), .O(z4));
endmodule


