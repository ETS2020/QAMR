// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x10), .O(new_n20_));
  nand2  g01(.a(x12), .b(new_n20_), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  nand4  g05(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n25_));
  aoi21  g06(.a(new_n25_), .b(new_n24_), .c(new_n22_), .O(new_n26_));
  oai21  g07(.a(x03), .b(x00), .c(x05), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n26_), .c(new_n21_), .O(new_n28_));
  aoi21  g09(.a(x12), .b(new_n20_), .c(x08), .O(new_n29_));
  nand4  g10(.a(new_n29_), .b(x04), .c(x03), .d(x02), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n28_), .O(z0));
  nand3  g12(.a(x13), .b(x09), .c(x04), .O(new_n32_));
  inv1   g13(.a(new_n32_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(x03), .c(x02), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(x05), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n21_), .O(new_n36_));
  inv1   g17(.a(x12), .O(new_n37_));
  nand2  g18(.a(x03), .b(x02), .O(new_n38_));
  inv1   g19(.a(x08), .O(new_n39_));
  nand3  g20(.a(new_n37_), .b(new_n39_), .c(x04), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n20_), .O(new_n42_));
  nand3  g23(.a(new_n39_), .b(x04), .c(x02), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(x10), .c(x03), .O(new_n44_));
  inv1   g25(.a(x01), .O(new_n45_));
  nand2  g26(.a(new_n22_), .b(new_n45_), .O(new_n46_));
  nand4  g27(.a(new_n46_), .b(new_n44_), .c(new_n42_), .d(new_n36_), .O(z1));
  inv1   g28(.a(x05), .O(new_n48_));
  oai21  g29(.a(x06), .b(x03), .c(new_n34_), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n48_), .c(new_n21_), .O(new_n50_));
  inv1   g31(.a(x11), .O(new_n51_));
  nand2  g32(.a(new_n43_), .b(new_n37_), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n20_), .c(new_n51_), .O(new_n53_));
  nand3  g34(.a(new_n37_), .b(new_n51_), .c(new_n20_), .O(new_n54_));
  nor2   g35(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  oai21  g36(.a(new_n55_), .b(new_n53_), .c(x03), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n50_), .O(z2));
  nand2  g38(.a(x13), .b(x09), .O(new_n58_));
  nand3  g39(.a(new_n51_), .b(new_n20_), .c(new_n39_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand4  g41(.a(new_n60_), .b(x04), .c(x03), .d(x02), .O(new_n61_));
  inv1   g42(.a(x07), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(new_n22_), .c(new_n48_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n37_), .O(new_n65_));
  aoi21  g46(.a(x07), .b(new_n22_), .c(new_n37_), .O(new_n66_));
  oai21  g47(.a(new_n66_), .b(new_n48_), .c(x10), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(new_n65_), .O(z3));
  nand2  g49(.a(new_n32_), .b(new_n21_), .O(z4));
endmodule


