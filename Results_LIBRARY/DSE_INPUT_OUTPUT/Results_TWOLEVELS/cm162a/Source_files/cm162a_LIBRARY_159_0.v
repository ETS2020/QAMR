// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n65_;
  nor2   g00(.a(x12), .b(x11), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  inv1   g03(.a(x10), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  oai21  g05(.a(new_n23_), .b(x08), .c(new_n24_), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  nand2  g07(.a(x04), .b(x02), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x08), .O(new_n28_));
  aoi21  g09(.a(new_n28_), .b(new_n26_), .c(new_n22_), .O(new_n29_));
  oai21  g10(.a(x03), .b(x00), .c(x05), .O(new_n30_));
  oai21  g11(.a(new_n30_), .b(new_n29_), .c(new_n21_), .O(new_n31_));
  nor2   g12(.a(x12), .b(x11), .O(new_n32_));
  nor3   g13(.a(new_n32_), .b(x10), .c(x08), .O(new_n33_));
  nand4  g14(.a(new_n33_), .b(x04), .c(x03), .d(x02), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n31_), .O(z0));
  inv1   g16(.a(x08), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x10), .O(new_n38_));
  inv1   g19(.a(new_n24_), .O(new_n39_));
  inv1   g20(.a(new_n27_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n38_), .c(new_n22_), .O(new_n42_));
  oai21  g23(.a(x03), .b(x01), .c(x05), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(new_n42_), .c(new_n21_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n34_), .O(z1));
  inv1   g26(.a(x05), .O(new_n46_));
  nand2  g27(.a(x03), .b(x02), .O(new_n47_));
  nand2  g28(.a(new_n39_), .b(x04), .O(new_n48_));
  nor2   g29(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g30(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand2  g31(.a(x11), .b(x10), .O(new_n51_));
  inv1   g32(.a(x11), .O(new_n52_));
  nand3  g33(.a(x12), .b(new_n52_), .c(new_n23_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n36_), .c(x04), .d(x02), .O(new_n55_));
  nand2  g36(.a(new_n37_), .b(x11), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x03), .O(new_n58_));
  inv1   g39(.a(x06), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n22_), .O(new_n60_));
  nand4  g41(.a(new_n60_), .b(new_n58_), .c(new_n50_), .d(new_n21_), .O(z2));
  oai22  g42(.a(new_n48_), .b(new_n47_), .c(x07), .d(x03), .O(new_n62_));
  oai21  g43(.a(new_n62_), .b(new_n46_), .c(new_n21_), .O(new_n63_));
  nand4  g44(.a(new_n40_), .b(new_n52_), .c(new_n23_), .d(new_n36_), .O(new_n64_));
  nand3  g45(.a(new_n64_), .b(x12), .c(x03), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n63_), .O(z3));
  nand2  g47(.a(new_n48_), .b(new_n21_), .O(z4));
endmodule


