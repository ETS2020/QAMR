// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x13), .O(new_n20_));
  nor2   g01(.a(new_n20_), .b(x12), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x08), .c(x03), .O(new_n24_));
  inv1   g05(.a(x00), .O(new_n25_));
  inv1   g06(.a(x03), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand3  g08(.a(new_n27_), .b(new_n24_), .c(x05), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n22_), .O(new_n29_));
  nand3  g10(.a(x13), .b(x12), .c(x09), .O(new_n30_));
  oai21  g11(.a(new_n21_), .b(x08), .c(new_n30_), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n29_), .O(z0));
  inv1   g14(.a(x08), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(x10), .c(x03), .O(new_n36_));
  inv1   g17(.a(new_n36_), .O(new_n37_));
  oai21  g18(.a(x03), .b(x01), .c(x05), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n37_), .c(new_n22_), .O(new_n39_));
  inv1   g20(.a(x10), .O(new_n40_));
  nand3  g21(.a(new_n22_), .b(new_n40_), .c(new_n34_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n30_), .O(new_n42_));
  nand4  g23(.a(new_n42_), .b(x04), .c(x03), .d(x02), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n39_), .O(z1));
  inv1   g25(.a(x05), .O(new_n45_));
  nand2  g26(.a(new_n22_), .b(new_n45_), .O(new_n46_));
  inv1   g27(.a(x11), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n40_), .c(new_n34_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(x04), .c(x02), .O(new_n50_));
  nand2  g31(.a(new_n40_), .b(new_n34_), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n23_), .c(x11), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x03), .O(new_n54_));
  inv1   g35(.a(x06), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n26_), .c(new_n21_), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n54_), .c(new_n46_), .O(z2));
  xnor2a g38(.a(x12), .b(x11), .O(new_n58_));
  and2   g39(.a(x12), .b(x10), .O(new_n59_));
  aoi21  g40(.a(new_n58_), .b(new_n40_), .c(new_n59_), .O(new_n60_));
  oai21  g41(.a(new_n60_), .b(x08), .c(new_n30_), .O(new_n61_));
  nand3  g42(.a(new_n61_), .b(x04), .c(x02), .O(new_n62_));
  nand2  g43(.a(new_n35_), .b(x12), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(x03), .O(new_n65_));
  nor2   g46(.a(x07), .b(x03), .O(new_n66_));
  nor2   g47(.a(new_n66_), .b(new_n21_), .O(new_n67_));
  nand3  g48(.a(new_n67_), .b(new_n65_), .c(new_n46_), .O(z3));
  nand4  g49(.a(x13), .b(x12), .c(x09), .d(x04), .O(new_n69_));
  inv1   g50(.a(new_n69_), .O(z4));
endmodule


