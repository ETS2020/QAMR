// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x11), .b(x08), .O(new_n21_));
  oai21  g02(.a(x03), .b(x00), .c(x05), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g04(.a(x04), .O(new_n24_));
  inv1   g05(.a(x11), .O(new_n25_));
  nand3  g06(.a(x13), .b(new_n25_), .c(x09), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(x08), .c(new_n24_), .O(new_n27_));
  and2   g08(.a(x04), .b(x02), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(x11), .O(new_n29_));
  aoi22  g10(.a(new_n29_), .b(x08), .c(new_n27_), .d(x02), .O(new_n30_));
  oai21  g11(.a(new_n30_), .b(new_n20_), .c(new_n23_), .O(z0));
  nand2  g12(.a(x13), .b(x09), .O(new_n32_));
  oai21  g13(.a(x10), .b(x08), .c(new_n32_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  inv1   g15(.a(x08), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x10), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x03), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  nand2  g21(.a(new_n20_), .b(new_n40_), .O(new_n41_));
  nand4  g22(.a(new_n41_), .b(new_n39_), .c(new_n21_), .d(x05), .O(z1));
  inv1   g23(.a(x10), .O(new_n43_));
  nand3  g24(.a(new_n25_), .b(new_n43_), .c(new_n35_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n32_), .c(new_n24_), .O(new_n45_));
  nor2   g26(.a(x10), .b(new_n24_), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(x02), .c(new_n25_), .O(new_n47_));
  aoi21  g28(.a(new_n45_), .b(x02), .c(new_n47_), .O(new_n48_));
  inv1   g29(.a(x06), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n20_), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(new_n21_), .c(x05), .O(new_n51_));
  inv1   g32(.a(new_n51_), .O(new_n52_));
  oai21  g33(.a(new_n48_), .b(new_n20_), .c(new_n52_), .O(z2));
  inv1   g34(.a(x12), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n25_), .c(new_n43_), .d(new_n35_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n32_), .c(new_n24_), .O(new_n56_));
  nor3   g37(.a(x11), .b(x10), .c(x08), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n28_), .c(new_n54_), .O(new_n58_));
  aoi21  g39(.a(new_n56_), .b(x02), .c(new_n58_), .O(new_n59_));
  inv1   g40(.a(x07), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n20_), .O(new_n61_));
  nand3  g42(.a(new_n61_), .b(new_n21_), .c(x05), .O(new_n62_));
  inv1   g43(.a(new_n62_), .O(new_n63_));
  oai21  g44(.a(new_n59_), .b(new_n20_), .c(new_n63_), .O(z3));
  oai21  g45(.a(new_n32_), .b(new_n24_), .c(new_n21_), .O(z4));
endmodule


