// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  nand3  g02(.a(x13), .b(x09), .c(x06), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(x08), .c(new_n21_), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  and2   g05(.a(x04), .b(x02), .O(new_n25_));
  nor2   g06(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  aoi21  g07(.a(new_n23_), .b(x02), .c(new_n26_), .O(new_n27_));
  inv1   g08(.a(x06), .O(new_n28_));
  nand2  g09(.a(x08), .b(new_n28_), .O(new_n29_));
  inv1   g10(.a(x00), .O(new_n30_));
  nand2  g11(.a(new_n20_), .b(new_n30_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(new_n29_), .c(x05), .O(new_n32_));
  inv1   g13(.a(new_n32_), .O(new_n33_));
  oai21  g14(.a(new_n27_), .b(new_n20_), .c(new_n33_), .O(z0));
  nand2  g15(.a(x03), .b(x02), .O(new_n35_));
  nand3  g16(.a(x13), .b(x09), .c(x04), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n35_), .c(x05), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n29_), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n20_), .c(x06), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x08), .O(new_n41_));
  nand3  g22(.a(new_n25_), .b(new_n39_), .c(new_n24_), .O(new_n42_));
  oai21  g23(.a(new_n25_), .b(new_n39_), .c(new_n42_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x03), .O(new_n44_));
  inv1   g25(.a(x01), .O(new_n45_));
  nand2  g26(.a(new_n20_), .b(new_n45_), .O(new_n46_));
  nand4  g27(.a(new_n46_), .b(new_n44_), .c(new_n41_), .d(new_n38_), .O(z1));
  nand3  g28(.a(new_n39_), .b(x04), .c(x02), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x11), .O(new_n49_));
  inv1   g30(.a(x11), .O(new_n50_));
  nand4  g31(.a(new_n50_), .b(new_n39_), .c(x04), .d(x02), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n49_), .c(x08), .O(new_n52_));
  nand3  g33(.a(x11), .b(x08), .c(x06), .O(new_n53_));
  inv1   g34(.a(new_n53_), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n52_), .c(x03), .O(new_n55_));
  nand3  g36(.a(new_n24_), .b(new_n28_), .c(new_n20_), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n55_), .c(new_n38_), .O(z2));
  inv1   g38(.a(x05), .O(new_n58_));
  oai22  g39(.a(new_n36_), .b(new_n35_), .c(x07), .d(x03), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n58_), .c(new_n29_), .O(new_n60_));
  nand2  g41(.a(new_n51_), .b(x12), .O(new_n61_));
  nor2   g42(.a(x12), .b(x11), .O(new_n62_));
  nand3  g43(.a(new_n62_), .b(new_n25_), .c(new_n39_), .O(new_n63_));
  aoi21  g44(.a(new_n63_), .b(new_n61_), .c(x08), .O(new_n64_));
  nand3  g45(.a(x12), .b(x08), .c(x06), .O(new_n65_));
  inv1   g46(.a(new_n65_), .O(new_n66_));
  oai21  g47(.a(new_n66_), .b(new_n64_), .c(x03), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(new_n60_), .O(z3));
  nand4  g49(.a(new_n29_), .b(x13), .c(x09), .d(x04), .O(new_n69_));
  inv1   g50(.a(new_n69_), .O(z4));
endmodule


