// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_;
  nand2  g00(.a(x13), .b(x08), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x03), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand4  g10(.a(new_n29_), .b(new_n26_), .c(new_n20_), .d(x05), .O(z0));
  oai21  g11(.a(x03), .b(x01), .c(x05), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n20_), .O(new_n32_));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x10), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand2  g16(.a(new_n21_), .b(x10), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n35_), .c(x08), .O(new_n37_));
  inv1   g18(.a(x13), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(x10), .c(x08), .O(new_n39_));
  inv1   g20(.a(new_n39_), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n37_), .c(x03), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n32_), .O(z1));
  oai21  g23(.a(x11), .b(x10), .c(new_n33_), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(x04), .c(x02), .O(new_n44_));
  inv1   g25(.a(x10), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(x04), .c(x02), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x11), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n44_), .c(new_n28_), .O(new_n48_));
  inv1   g29(.a(x06), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n28_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x05), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n48_), .c(new_n23_), .O(new_n52_));
  nand3  g33(.a(x11), .b(x08), .c(x03), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n50_), .c(x05), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n38_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n52_), .O(z2));
  inv1   g37(.a(x11), .O(new_n57_));
  inv1   g38(.a(x12), .O(new_n58_));
  nand3  g39(.a(new_n58_), .b(new_n57_), .c(new_n45_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n33_), .O(new_n60_));
  nand4  g41(.a(new_n60_), .b(new_n23_), .c(x04), .d(x02), .O(new_n61_));
  nand3  g42(.a(new_n57_), .b(new_n45_), .c(new_n23_), .O(new_n62_));
  oai21  g43(.a(new_n62_), .b(new_n21_), .c(x12), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(x03), .O(new_n65_));
  inv1   g46(.a(x07), .O(new_n66_));
  nand2  g47(.a(new_n66_), .b(new_n28_), .O(new_n67_));
  nand4  g48(.a(new_n67_), .b(new_n65_), .c(new_n20_), .d(x05), .O(z3));
  nand4  g49(.a(x13), .b(x09), .c(new_n23_), .d(x04), .O(new_n69_));
  inv1   g50(.a(new_n69_), .O(z4));
endmodule


