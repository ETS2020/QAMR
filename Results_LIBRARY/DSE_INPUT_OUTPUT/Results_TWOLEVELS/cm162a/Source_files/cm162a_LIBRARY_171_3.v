// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x09), .O(new_n20_));
  inv1   g01(.a(x11), .O(new_n21_));
  nor2   g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(x08), .c(x03), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  inv1   g07(.a(x03), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g09(.a(new_n28_), .b(new_n25_), .c(x05), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  inv1   g11(.a(x08), .O(new_n31_));
  aoi21  g12(.a(new_n21_), .b(x10), .c(x09), .O(new_n32_));
  oai21  g13(.a(new_n32_), .b(new_n21_), .c(new_n31_), .O(new_n33_));
  nand3  g14(.a(x13), .b(new_n21_), .c(x09), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g16(.a(new_n35_), .b(x04), .c(x03), .d(x02), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n30_), .O(z0));
  nand3  g18(.a(new_n31_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x10), .O(new_n39_));
  inv1   g20(.a(x10), .O(new_n40_));
  nand4  g21(.a(new_n40_), .b(new_n31_), .c(x04), .d(x02), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n39_), .c(new_n27_), .O(new_n42_));
  oai21  g23(.a(x03), .b(x01), .c(x05), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(new_n42_), .c(new_n23_), .O(new_n44_));
  inv1   g25(.a(new_n34_), .O(new_n45_));
  nand4  g26(.a(new_n45_), .b(x04), .c(x03), .d(x02), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n44_), .O(z1));
  inv1   g28(.a(x05), .O(new_n48_));
  nand2  g29(.a(new_n23_), .b(new_n48_), .O(new_n49_));
  inv1   g30(.a(x02), .O(new_n50_));
  nand3  g31(.a(x11), .b(x10), .c(new_n20_), .O(new_n51_));
  nand2  g32(.a(new_n21_), .b(new_n40_), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n51_), .c(x08), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n45_), .c(x04), .O(new_n54_));
  nand2  g35(.a(new_n38_), .b(x11), .O(new_n55_));
  oai21  g36(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x03), .O(new_n57_));
  nor2   g38(.a(x06), .b(x03), .O(new_n58_));
  nor2   g39(.a(new_n58_), .b(new_n22_), .O(new_n59_));
  nand3  g40(.a(new_n59_), .b(new_n57_), .c(new_n49_), .O(z2));
  inv1   g41(.a(x12), .O(new_n61_));
  nand3  g42(.a(new_n61_), .b(new_n40_), .c(new_n31_), .O(new_n62_));
  nand2  g43(.a(x13), .b(x09), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g45(.a(new_n64_), .b(x04), .c(x02), .O(new_n65_));
  nand2  g46(.a(new_n41_), .b(x12), .O(new_n66_));
  aoi21  g47(.a(new_n66_), .b(new_n65_), .c(new_n27_), .O(new_n67_));
  inv1   g48(.a(x07), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n27_), .O(new_n69_));
  nand2  g50(.a(new_n69_), .b(x05), .O(new_n70_));
  oai21  g51(.a(new_n70_), .b(new_n67_), .c(new_n21_), .O(new_n71_));
  nand3  g52(.a(x12), .b(x11), .c(x03), .O(new_n72_));
  nand3  g53(.a(new_n72_), .b(new_n69_), .c(x05), .O(new_n73_));
  nand2  g54(.a(new_n73_), .b(new_n20_), .O(new_n74_));
  nand2  g55(.a(new_n74_), .b(new_n71_), .O(z3));
  nand2  g56(.a(x13), .b(x04), .O(new_n76_));
  aoi21  g57(.a(new_n76_), .b(new_n21_), .c(new_n20_), .O(z4));
endmodule


