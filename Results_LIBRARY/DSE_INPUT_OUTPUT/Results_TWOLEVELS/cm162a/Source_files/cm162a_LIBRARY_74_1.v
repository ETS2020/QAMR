// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_;
  aoi21  g00(.a(x05), .b(x00), .c(x03), .O(new_n20_));
  inv1   g01(.a(x02), .O(new_n21_));
  inv1   g02(.a(x04), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  aoi21  g04(.a(new_n23_), .b(x08), .c(new_n22_), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nor2   g06(.a(new_n25_), .b(x04), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n24_), .c(x03), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(x05), .c(new_n21_), .O(new_n28_));
  or2    g09(.a(new_n28_), .b(new_n20_), .O(z0));
  nand2  g10(.a(x04), .b(x03), .O(new_n30_));
  oai21  g11(.a(new_n30_), .b(new_n23_), .c(x05), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x02), .O(new_n32_));
  inv1   g13(.a(x03), .O(new_n33_));
  nand2  g14(.a(x05), .b(x01), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g16(.a(new_n25_), .b(x04), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x10), .O(new_n37_));
  nor2   g18(.a(x10), .b(x08), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(x04), .c(new_n21_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x03), .O(new_n41_));
  nand3  g22(.a(new_n41_), .b(new_n35_), .c(new_n32_), .O(z1));
  nand2  g23(.a(x06), .b(x05), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  nand2  g25(.a(x11), .b(x10), .O(new_n45_));
  nand3  g26(.a(x13), .b(x09), .c(x04), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n45_), .c(new_n21_), .O(new_n47_));
  nand2  g28(.a(new_n36_), .b(x11), .O(new_n48_));
  inv1   g29(.a(x10), .O(new_n49_));
  inv1   g30(.a(x11), .O(new_n50_));
  nand4  g31(.a(new_n50_), .b(new_n49_), .c(new_n25_), .d(x04), .O(new_n51_));
  nand3  g32(.a(new_n51_), .b(new_n48_), .c(x02), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n47_), .c(x03), .O(new_n53_));
  inv1   g34(.a(x05), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x02), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n53_), .c(new_n44_), .O(z2));
  nand2  g37(.a(x07), .b(x05), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n33_), .O(new_n58_));
  nor2   g39(.a(x12), .b(x11), .O(new_n59_));
  aoi22  g40(.a(new_n59_), .b(new_n38_), .c(x13), .d(x09), .O(new_n60_));
  nand2  g41(.a(new_n51_), .b(x12), .O(new_n61_));
  oai21  g42(.a(new_n60_), .b(new_n22_), .c(new_n61_), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(x03), .c(new_n54_), .O(new_n63_));
  oai21  g44(.a(new_n63_), .b(new_n21_), .c(new_n58_), .O(z3));
  nand2  g45(.a(x03), .b(new_n21_), .O(new_n65_));
  nand4  g46(.a(new_n65_), .b(x13), .c(x09), .d(x04), .O(new_n66_));
  inv1   g47(.a(new_n66_), .O(z4));
endmodule


