// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  oai21  g02(.a(new_n21_), .b(x06), .c(x08), .O(new_n22_));
  nand3  g03(.a(new_n22_), .b(x04), .c(x02), .O(new_n23_));
  and2   g04(.a(x04), .b(x02), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n20_), .c(new_n23_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x03), .O(new_n26_));
  nand2  g07(.a(x08), .b(x06), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(new_n27_), .c(x05), .O(new_n31_));
  inv1   g12(.a(new_n31_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n26_), .O(z0));
  nand2  g14(.a(x03), .b(x02), .O(new_n34_));
  nand3  g15(.a(x13), .b(x09), .c(x04), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(x05), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  inv1   g18(.a(x06), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n29_), .c(new_n38_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x08), .O(new_n41_));
  nand3  g22(.a(new_n24_), .b(new_n39_), .c(new_n20_), .O(new_n42_));
  oai21  g23(.a(new_n24_), .b(new_n39_), .c(new_n42_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x03), .O(new_n44_));
  inv1   g25(.a(x01), .O(new_n45_));
  nand2  g26(.a(new_n29_), .b(new_n45_), .O(new_n46_));
  nand4  g27(.a(new_n46_), .b(new_n44_), .c(new_n41_), .d(new_n37_), .O(z1));
  inv1   g28(.a(x11), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n20_), .c(x03), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n38_), .O(new_n50_));
  nand3  g31(.a(new_n39_), .b(x04), .c(x02), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x11), .O(new_n52_));
  nand4  g33(.a(new_n48_), .b(new_n39_), .c(x04), .d(x02), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n20_), .c(x03), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n50_), .c(new_n37_), .O(z2));
  inv1   g37(.a(x05), .O(new_n57_));
  oai22  g38(.a(new_n35_), .b(new_n34_), .c(x07), .d(x03), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n57_), .c(new_n27_), .O(new_n59_));
  nand2  g40(.a(new_n53_), .b(x12), .O(new_n60_));
  inv1   g41(.a(x12), .O(new_n61_));
  nand4  g42(.a(new_n24_), .b(new_n61_), .c(new_n48_), .d(new_n39_), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(new_n60_), .c(x08), .O(new_n63_));
  nor3   g44(.a(new_n61_), .b(new_n20_), .c(x06), .O(new_n64_));
  oai21  g45(.a(new_n64_), .b(new_n63_), .c(x03), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n59_), .O(z3));
  nand4  g47(.a(new_n27_), .b(x13), .c(x09), .d(x04), .O(new_n67_));
  inv1   g48(.a(new_n67_), .O(z4));
endmodule


