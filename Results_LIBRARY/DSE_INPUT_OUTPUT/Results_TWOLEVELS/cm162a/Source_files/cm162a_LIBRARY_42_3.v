// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_;
  nand2  g00(.a(x11), .b(x10), .O(new_n20_));
  nand3  g01(.a(x13), .b(x09), .c(x04), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand3  g03(.a(new_n22_), .b(x03), .c(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x05), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  and2   g07(.a(x04), .b(x02), .O(new_n27_));
  nand3  g08(.a(new_n26_), .b(x04), .c(x02), .O(new_n28_));
  oai21  g09(.a(new_n27_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g11(.a(x00), .O(new_n31_));
  inv1   g12(.a(x03), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand4  g14(.a(new_n33_), .b(new_n30_), .c(new_n25_), .d(new_n20_), .O(z0));
  nor2   g15(.a(x03), .b(x01), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n24_), .c(new_n20_), .O(new_n36_));
  inv1   g17(.a(x11), .O(new_n37_));
  nand3  g18(.a(new_n28_), .b(new_n37_), .c(x10), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  nand3  g20(.a(new_n27_), .b(new_n39_), .c(new_n26_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x03), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n36_), .O(z1));
  inv1   g24(.a(x05), .O(new_n44_));
  oai21  g25(.a(x06), .b(x03), .c(new_n23_), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n44_), .c(new_n20_), .O(new_n46_));
  inv1   g27(.a(new_n28_), .O(new_n47_));
  nand3  g28(.a(new_n27_), .b(new_n37_), .c(new_n26_), .O(new_n48_));
  oai21  g29(.a(new_n47_), .b(new_n37_), .c(new_n48_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n39_), .c(x03), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n46_), .O(z2));
  inv1   g32(.a(x12), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n32_), .c(new_n37_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x10), .O(new_n54_));
  nand2  g35(.a(x12), .b(x11), .O(new_n55_));
  nor2   g36(.a(x12), .b(x11), .O(new_n56_));
  nand4  g37(.a(new_n56_), .b(new_n26_), .c(x04), .d(x02), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n55_), .c(x10), .O(new_n58_));
  nor2   g39(.a(new_n47_), .b(new_n52_), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n58_), .c(x03), .O(new_n60_));
  inv1   g41(.a(x07), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n32_), .O(new_n62_));
  nand4  g43(.a(new_n62_), .b(new_n60_), .c(new_n54_), .d(new_n25_), .O(z3));
  nand2  g44(.a(new_n21_), .b(new_n20_), .O(z4));
endmodule


