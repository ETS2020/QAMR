// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x10), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x04), .O(new_n21_));
  oai21  g02(.a(x03), .b(x00), .c(x05), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  inv1   g07(.a(x02), .O(new_n27_));
  nand2  g08(.a(x08), .b(new_n27_), .O(new_n28_));
  aoi21  g09(.a(new_n28_), .b(new_n26_), .c(new_n20_), .O(new_n29_));
  inv1   g10(.a(x08), .O(new_n30_));
  nor2   g11(.a(new_n30_), .b(x04), .O(new_n31_));
  oai21  g12(.a(new_n31_), .b(new_n29_), .c(x03), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n23_), .O(z0));
  nand2  g14(.a(x03), .b(x02), .O(new_n34_));
  nand3  g15(.a(x13), .b(x10), .c(x09), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(x10), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x04), .O(new_n37_));
  inv1   g18(.a(x04), .O(new_n38_));
  inv1   g19(.a(x03), .O(new_n39_));
  oai21  g20(.a(new_n20_), .b(new_n39_), .c(x05), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  inv1   g22(.a(x05), .O(new_n42_));
  aoi21  g23(.a(new_n30_), .b(x02), .c(new_n39_), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(new_n42_), .c(x10), .O(new_n44_));
  inv1   g25(.a(x01), .O(new_n45_));
  nand2  g26(.a(new_n39_), .b(new_n45_), .O(new_n46_));
  nand4  g27(.a(new_n46_), .b(new_n44_), .c(new_n41_), .d(new_n37_), .O(z1));
  nor3   g28(.a(new_n35_), .b(new_n38_), .c(new_n27_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(x11), .c(x03), .O(new_n49_));
  inv1   g30(.a(x06), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n39_), .O(new_n51_));
  nand4  g32(.a(new_n51_), .b(new_n49_), .c(new_n21_), .d(x05), .O(z2));
  oai21  g33(.a(new_n48_), .b(x12), .c(x03), .O(new_n53_));
  inv1   g34(.a(x07), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n39_), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n53_), .c(new_n21_), .d(x05), .O(z3));
  aoi21  g37(.a(new_n24_), .b(x10), .c(new_n38_), .O(z4));
endmodule


