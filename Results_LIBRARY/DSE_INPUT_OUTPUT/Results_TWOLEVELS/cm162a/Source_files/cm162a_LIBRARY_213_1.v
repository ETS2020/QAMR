// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_;
  inv1   g00(.a(x12), .O(new_n20_));
  nor2   g01(.a(new_n20_), .b(x07), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(x03), .b(x02), .O(new_n23_));
  nand3  g04(.a(x13), .b(x09), .c(x04), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n23_), .c(x05), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  inv1   g07(.a(x08), .O(new_n27_));
  and2   g08(.a(x04), .b(x02), .O(new_n28_));
  nand3  g09(.a(new_n27_), .b(x04), .c(x02), .O(new_n29_));
  oai21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x03), .O(new_n31_));
  nor2   g12(.a(x03), .b(x00), .O(new_n32_));
  nor2   g13(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(new_n31_), .c(new_n26_), .O(z0));
  inv1   g15(.a(x02), .O(new_n35_));
  nand2  g16(.a(new_n29_), .b(x10), .O(new_n36_));
  nand3  g17(.a(new_n28_), .b(x13), .c(x09), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  oai21  g19(.a(x03), .b(x01), .c(x05), .O(new_n39_));
  aoi21  g20(.a(new_n38_), .b(x03), .c(new_n39_), .O(new_n40_));
  inv1   g21(.a(x11), .O(new_n41_));
  oai21  g22(.a(x12), .b(new_n41_), .c(x07), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(x12), .c(x10), .O(new_n43_));
  nand4  g24(.a(new_n43_), .b(new_n27_), .c(x04), .d(x03), .O(new_n44_));
  oai22  g25(.a(new_n44_), .b(new_n35_), .c(new_n40_), .d(new_n21_), .O(z1));
  nand2  g26(.a(x13), .b(x09), .O(new_n46_));
  inv1   g27(.a(x10), .O(new_n47_));
  nand3  g28(.a(new_n41_), .b(new_n47_), .c(new_n27_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g30(.a(new_n49_), .b(x04), .c(x03), .d(x02), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x05), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  nand3  g33(.a(new_n28_), .b(new_n47_), .c(new_n27_), .O(new_n53_));
  and2   g34(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g35(.a(x06), .b(x03), .c(new_n22_), .O(new_n55_));
  aoi21  g36(.a(new_n54_), .b(x03), .c(new_n55_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n52_), .O(z2));
  nand3  g38(.a(x12), .b(x11), .c(x07), .O(new_n58_));
  oai21  g39(.a(x12), .b(x11), .c(new_n58_), .O(new_n59_));
  nand4  g40(.a(new_n59_), .b(new_n47_), .c(new_n27_), .d(x04), .O(new_n60_));
  nand3  g41(.a(new_n53_), .b(x12), .c(x07), .O(new_n61_));
  oai21  g42(.a(new_n60_), .b(new_n35_), .c(new_n61_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(x03), .O(new_n63_));
  inv1   g44(.a(x03), .O(new_n64_));
  inv1   g45(.a(x07), .O(new_n65_));
  nand3  g46(.a(new_n20_), .b(new_n65_), .c(new_n64_), .O(new_n66_));
  nand3  g47(.a(new_n66_), .b(new_n63_), .c(new_n26_), .O(z3));
  nand2  g48(.a(new_n24_), .b(new_n22_), .O(z4));
endmodule


