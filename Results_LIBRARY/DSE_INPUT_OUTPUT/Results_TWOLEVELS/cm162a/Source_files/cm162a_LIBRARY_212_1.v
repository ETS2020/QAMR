// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x12), .O(new_n20_));
  nor2   g01(.a(new_n20_), .b(x07), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(x03), .b(x02), .O(new_n23_));
  nand3  g04(.a(x13), .b(x09), .c(x04), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n23_), .c(x05), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  inv1   g07(.a(x08), .O(new_n27_));
  and2   g08(.a(x04), .b(x02), .O(new_n28_));
  nand3  g09(.a(new_n27_), .b(x04), .c(x02), .O(new_n29_));
  oai21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x03), .O(new_n31_));
  nor2   g12(.a(x03), .b(x00), .O(new_n32_));
  nor2   g13(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(new_n31_), .c(new_n26_), .O(z0));
  nand2  g15(.a(x13), .b(x09), .O(new_n35_));
  oai21  g16(.a(x10), .b(x08), .c(new_n35_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g18(.a(new_n29_), .b(x10), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  inv1   g21(.a(x01), .O(new_n41_));
  inv1   g22(.a(x03), .O(new_n42_));
  inv1   g23(.a(x05), .O(new_n43_));
  aoi21  g24(.a(new_n42_), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n40_), .c(new_n21_), .O(z1));
  inv1   g26(.a(x11), .O(new_n46_));
  nor2   g27(.a(x10), .b(x08), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n28_), .c(new_n46_), .O(new_n48_));
  nor3   g29(.a(x11), .b(x10), .c(x08), .O(new_n49_));
  and2   g30(.a(new_n49_), .b(new_n28_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n48_), .c(x03), .O(new_n51_));
  nor2   g32(.a(x06), .b(x03), .O(new_n52_));
  nor2   g33(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n51_), .c(new_n26_), .O(z2));
  inv1   g35(.a(x07), .O(new_n55_));
  oai21  g36(.a(x12), .b(new_n42_), .c(new_n55_), .O(new_n56_));
  aoi21  g37(.a(new_n49_), .b(new_n28_), .c(new_n20_), .O(new_n57_));
  nand2  g38(.a(new_n20_), .b(new_n46_), .O(new_n58_));
  nor3   g39(.a(new_n58_), .b(new_n29_), .c(x10), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n57_), .c(x03), .O(new_n60_));
  nand3  g41(.a(new_n60_), .b(new_n56_), .c(new_n26_), .O(z3));
  nand2  g42(.a(new_n24_), .b(new_n22_), .O(z4));
endmodule


