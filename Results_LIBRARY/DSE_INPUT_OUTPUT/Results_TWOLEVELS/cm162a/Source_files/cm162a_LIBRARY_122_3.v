// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  inv1   g02(.a(x11), .O(new_n22_));
  nand3  g03(.a(x13), .b(new_n22_), .c(x09), .O(new_n23_));
  aoi21  g04(.a(new_n23_), .b(x08), .c(new_n21_), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  and2   g06(.a(x04), .b(x02), .O(new_n26_));
  nor2   g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  aoi21  g08(.a(new_n24_), .b(x02), .c(new_n27_), .O(new_n28_));
  nor2   g09(.a(x03), .b(x00), .O(new_n29_));
  nand2  g10(.a(new_n22_), .b(x05), .O(new_n30_));
  nor2   g11(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  oai21  g12(.a(new_n28_), .b(new_n20_), .c(new_n31_), .O(z0));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  oai21  g14(.a(x10), .b(x08), .c(new_n33_), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(new_n22_), .c(x04), .d(x02), .O(new_n35_));
  nand3  g16(.a(new_n25_), .b(x04), .c(x02), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x10), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x03), .O(new_n39_));
  nor2   g20(.a(x03), .b(x01), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(new_n30_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n39_), .O(z1));
  nand4  g23(.a(new_n34_), .b(x04), .c(x03), .d(x02), .O(new_n43_));
  inv1   g24(.a(x06), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n20_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n22_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(x05), .O(z2));
  inv1   g29(.a(x10), .O(new_n49_));
  inv1   g30(.a(x12), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(new_n49_), .c(new_n25_), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n23_), .c(new_n21_), .O(new_n52_));
  nor2   g33(.a(x10), .b(x08), .O(new_n53_));
  aoi21  g34(.a(new_n53_), .b(new_n26_), .c(new_n50_), .O(new_n54_));
  aoi21  g35(.a(new_n52_), .b(x02), .c(new_n54_), .O(new_n55_));
  inv1   g36(.a(x05), .O(new_n56_));
  nor2   g37(.a(x07), .b(x03), .O(new_n57_));
  nor3   g38(.a(new_n57_), .b(x11), .c(new_n56_), .O(new_n58_));
  oai21  g39(.a(new_n55_), .b(new_n20_), .c(new_n58_), .O(z3));
  oai22  g40(.a(new_n33_), .b(new_n21_), .c(new_n22_), .d(new_n56_), .O(z4));
endmodule


