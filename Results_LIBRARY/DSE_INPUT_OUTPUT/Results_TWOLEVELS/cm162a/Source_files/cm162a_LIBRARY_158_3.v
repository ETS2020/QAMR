// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x05), .b(x00), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(x04), .O(new_n23_));
  nand2  g04(.a(x08), .b(x03), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x05), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n22_), .O(z0));
  nand2  g08(.a(x05), .b(x01), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n20_), .O(new_n29_));
  nand2  g10(.a(x10), .b(x03), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x05), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n23_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n29_), .O(z1));
  nand2  g14(.a(x06), .b(x05), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n20_), .O(new_n35_));
  nand2  g16(.a(x11), .b(x03), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x05), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n23_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n35_), .O(z2));
  oai21  g20(.a(x12), .b(x04), .c(x03), .O(new_n40_));
  nand2  g21(.a(x07), .b(x05), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n20_), .O(new_n42_));
  inv1   g23(.a(x05), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n23_), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n42_), .c(new_n40_), .O(z3));
  nand4  g26(.a(x13), .b(x09), .c(x04), .d(new_n20_), .O(new_n46_));
  inv1   g27(.a(new_n46_), .O(z4));
endmodule


