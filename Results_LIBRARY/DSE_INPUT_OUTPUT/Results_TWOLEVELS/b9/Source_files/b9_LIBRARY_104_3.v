// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x03), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x03), .c(new_n70_), .O(z00));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  inv1   g026(.a(x29), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  inv1   g028(.a(x27), .O(new_n91_));
  oai21  g029(.a(x08), .b(new_n71_), .c(x40), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  inv1   g031(.a(new_n83_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x04), .O(new_n95_));
  oai21  g033(.a(x28), .b(new_n91_), .c(x04), .O(new_n96_));
  nand2  g034(.a(new_n64_), .b(x39), .O(new_n97_));
  aoi21  g035(.a(new_n96_), .b(new_n74_), .c(new_n97_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n95_), .c(new_n93_), .d(new_n90_), .O(z02));
  nand2  g037(.a(new_n66_), .b(x27), .O(new_n100_));
  nand2  g038(.a(new_n65_), .b(new_n91_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n100_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g040(.a(new_n65_), .b(new_n91_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n100_), .c(new_n79_), .O(z04));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n64_), .c(new_n65_), .O(z05));
  inv1   g044(.a(z05), .O(z06));
  inv1   g045(.a(x03), .O(new_n108_));
  inv1   g046(.a(x31), .O(new_n109_));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n72_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand4  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .d(new_n109_), .O(new_n114_));
  inv1   g052(.a(x14), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n112_), .O(new_n117_));
  aoi21  g055(.a(new_n117_), .b(new_n115_), .c(x40), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n114_), .c(new_n108_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n108_), .c(new_n74_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n64_), .O(z09));
  nor2   g061(.a(new_n83_), .b(x36), .O(new_n124_));
  inv1   g062(.a(x07), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x05), .O(new_n126_));
  oai21  g064(.a(x39), .b(new_n125_), .c(new_n126_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n108_), .O(new_n128_));
  nand2  g066(.a(new_n74_), .b(x07), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n128_), .c(new_n124_), .O(new_n130_));
  nand3  g068(.a(new_n64_), .b(x37), .c(x06), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  aoi21  g070(.a(new_n130_), .b(new_n71_), .c(new_n132_), .O(new_n133_));
  nand2  g071(.a(new_n81_), .b(new_n80_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n120_), .O(new_n136_));
  nand4  g074(.a(new_n136_), .b(x05), .c(new_n71_), .d(new_n108_), .O(new_n137_));
  oai21  g075(.a(new_n133_), .b(new_n91_), .c(new_n137_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  inv1   g077(.a(x28), .O(new_n140_));
  nand3  g078(.a(x35), .b(new_n140_), .c(new_n71_), .O(new_n141_));
  aoi22  g079(.a(new_n141_), .b(new_n108_), .c(new_n64_), .d(new_n91_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n108_), .c(new_n74_), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n83_), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(new_n147_));
  oai22  g085(.a(new_n147_), .b(new_n144_), .c(new_n142_), .d(x08), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n80_), .c(new_n139_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n64_), .O(z11));
  nor3   g088(.a(x37), .b(x36), .c(x35), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n91_), .c(new_n73_), .O(new_n152_));
  nand2  g090(.a(x40), .b(x39), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(x04), .c(new_n68_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n152_), .c(new_n64_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n74_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n71_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n67_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n159_), .c(new_n91_), .O(new_n163_));
  nand4  g101(.a(new_n134_), .b(x39), .c(new_n156_), .d(new_n71_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n108_), .c(new_n74_), .O(new_n165_));
  or2    g103(.a(new_n165_), .b(new_n163_), .O(z13));
  nand3  g104(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n167_));
  nand4  g105(.a(new_n157_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  nand4  g107(.a(x40), .b(x39), .c(new_n156_), .d(new_n71_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n169_), .c(new_n167_), .O(new_n171_));
  nand3  g109(.a(new_n65_), .b(new_n82_), .c(x28), .O(new_n172_));
  oai21  g110(.a(x27), .b(x03), .c(new_n172_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n81_), .c(new_n80_), .O(new_n174_));
  aoi21  g112(.a(new_n170_), .b(new_n68_), .c(new_n79_), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(new_n171_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n64_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(x22), .c(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n64_), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(x23), .c(x22), .d(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n64_), .O(z17));
  oai22  g122(.a(new_n153_), .b(new_n89_), .c(new_n94_), .d(x27), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(x08), .O(new_n186_));
  nor2   g124(.a(new_n91_), .b(x04), .O(new_n187_));
  aoi21  g125(.a(new_n187_), .b(new_n83_), .c(x30), .O(new_n188_));
  nand4  g126(.a(new_n188_), .b(new_n186_), .c(new_n64_), .d(new_n139_), .O(z18));
  inv1   g127(.a(new_n149_), .O(z19));
  aoi21  g128(.a(x28), .b(new_n91_), .c(new_n82_), .O(new_n191_));
  aoi21  g129(.a(x35), .b(x27), .c(x03), .O(new_n192_));
  nor2   g130(.a(x40), .b(x27), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n192_), .c(new_n88_), .O(new_n194_));
  oai21  g132(.a(new_n191_), .b(new_n144_), .c(new_n194_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n80_), .c(new_n139_), .O(new_n196_));
  inv1   g134(.a(new_n196_), .O(z20));
endmodule


