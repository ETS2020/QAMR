// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n108_,
    new_n109_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x39), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n63_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n66_), .d(new_n73_), .O(new_n77_));
  oai21  g015(.a(new_n72_), .b(new_n65_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n65_), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n82_), .b(new_n73_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x40), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(x35), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x04), .O(new_n92_));
  inv1   g030(.a(x39), .O(new_n93_));
  nand2  g031(.a(new_n70_), .b(new_n69_), .O(new_n94_));
  inv1   g032(.a(x18), .O(new_n95_));
  inv1   g033(.a(x19), .O(new_n96_));
  nand4  g034(.a(new_n71_), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n97_));
  nand4  g035(.a(x27), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n97_), .c(new_n94_), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n93_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n92_), .c(new_n90_), .d(new_n64_), .O(z02));
  nand2  g040(.a(new_n70_), .b(x27), .O(new_n103_));
  aoi21  g041(.a(new_n69_), .b(new_n68_), .c(new_n65_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n103_), .c(x21), .O(z03));
  inv1   g043(.a(x21), .O(new_n106_));
  nand3  g044(.a(new_n104_), .b(new_n103_), .c(new_n106_), .O(z04));
  nor2   g045(.a(new_n82_), .b(new_n68_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n64_), .c(new_n69_), .O(z05));
  oai21  g048(.a(new_n108_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g049(.a(x33), .O(new_n112_));
  nand2  g050(.a(x17), .b(new_n66_), .O(new_n113_));
  inv1   g051(.a(x00), .O(new_n114_));
  oai21  g052(.a(x25), .b(new_n114_), .c(x38), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(x25), .O(new_n117_));
  nand3  g055(.a(x38), .b(new_n117_), .c(new_n114_), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  oai22  g057(.a(new_n119_), .b(x14), .c(new_n116_), .d(x31), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n64_), .c(x03), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z07));
  nor2   g060(.a(new_n63_), .b(new_n93_), .O(z08));
  inv1   g061(.a(x11), .O(new_n124_));
  nand4  g062(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n124_), .O(z09));
  inv1   g064(.a(x07), .O(new_n127_));
  nand2  g065(.a(x39), .b(x05), .O(new_n128_));
  oai21  g066(.a(x39), .b(new_n127_), .c(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n84_), .c(x27), .O(new_n130_));
  nand2  g068(.a(new_n80_), .b(new_n79_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x39), .c(x05), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n73_), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n64_), .O(z10));
  inv1   g074(.a(new_n83_), .O(new_n137_));
  inv1   g075(.a(x29), .O(new_n138_));
  aoi21  g076(.a(x40), .b(new_n138_), .c(new_n93_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n137_), .c(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n64_), .b(new_n68_), .O(new_n142_));
  nand3  g080(.a(x35), .b(new_n82_), .c(new_n73_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x40), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n142_), .c(x08), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n141_), .c(new_n79_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(x09), .O(z11));
  nor3   g085(.a(x37), .b(x36), .c(x35), .O(new_n148_));
  nor2   g086(.a(new_n63_), .b(x27), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n74_), .O(new_n150_));
  aoi21  g088(.a(x40), .b(x04), .c(new_n93_), .O(new_n151_));
  or2    g089(.a(new_n151_), .b(new_n71_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n150_), .c(new_n64_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x28), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n73_), .O(new_n157_));
  nand4  g095(.a(new_n94_), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n157_), .c(new_n63_), .O(new_n159_));
  nand4  g097(.a(new_n94_), .b(new_n93_), .c(x20), .d(new_n96_), .O(new_n160_));
  nor2   g098(.a(new_n160_), .b(x18), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n159_), .c(x27), .O(new_n162_));
  aoi21  g100(.a(new_n80_), .b(new_n79_), .c(new_n63_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x39), .c(new_n154_), .d(new_n73_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n162_), .O(z13));
  nand2  g103(.a(new_n99_), .b(new_n94_), .O(new_n166_));
  aoi21  g104(.a(new_n154_), .b(new_n73_), .c(new_n63_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n93_), .c(new_n166_), .O(new_n168_));
  nand3  g106(.a(x20), .b(new_n96_), .c(new_n95_), .O(new_n169_));
  nor2   g107(.a(x37), .b(x35), .O(new_n170_));
  aoi21  g108(.a(new_n169_), .b(new_n155_), .c(new_n170_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n82_), .c(x27), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(x40), .c(new_n80_), .d(new_n79_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n168_), .O(z14));
  nand4  g112(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n64_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand4  g115(.a(new_n64_), .b(new_n177_), .c(x22), .d(x01), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(x23), .c(x22), .d(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n64_), .O(z17));
  oai22  g120(.a(new_n83_), .b(x27), .c(new_n93_), .d(new_n138_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(x08), .O(new_n184_));
  nor2   g122(.a(new_n68_), .b(x04), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n137_), .c(x09), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n184_), .c(new_n64_), .d(new_n79_), .O(z18));
  inv1   g125(.a(x09), .O(new_n188_));
  aoi21  g126(.a(new_n63_), .b(x39), .c(x08), .O(new_n189_));
  aoi21  g127(.a(x39), .b(x29), .c(new_n82_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n189_), .c(new_n68_), .O(new_n191_));
  inv1   g129(.a(x35), .O(new_n192_));
  aoi21  g130(.a(x29), .b(x08), .c(new_n63_), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n93_), .c(new_n192_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n191_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n79_), .c(new_n188_), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n64_), .O(z20));
  nor2   g135(.a(new_n146_), .b(x09), .O(z19));
endmodule


