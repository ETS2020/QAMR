// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_;
  inv1   g000(.a(x14), .O(new_n63_));
  nor2   g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n79_), .c(x04), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  aoi21  g027(.a(new_n84_), .b(x04), .c(new_n89_), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  nand3  g029(.a(new_n67_), .b(new_n91_), .c(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(x40), .d(x39), .O(z02));
  nand2  g031(.a(new_n69_), .b(x27), .O(new_n94_));
  nand3  g032(.a(new_n79_), .b(new_n68_), .c(new_n67_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n79_), .d(x21), .O(z03));
  nor2   g034(.a(new_n64_), .b(x21), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(new_n94_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n64_), .O(z05));
  oai22  g038(.a(x40), .b(new_n63_), .c(new_n83_), .d(new_n67_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(x37), .O(z06));
  inv1   g040(.a(x03), .O(new_n103_));
  inv1   g041(.a(x31), .O(new_n104_));
  inv1   g042(.a(x33), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  aoi22  g045(.a(new_n107_), .b(x38), .c(x17), .d(new_n65_), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(x40), .c(new_n105_), .d(new_n104_), .O(new_n109_));
  nand2  g047(.a(x38), .b(new_n106_), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(x00), .c(new_n63_), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n109_), .c(new_n103_), .O(z07));
  nand2  g050(.a(x40), .b(x39), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n79_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n79_), .O(z09));
  nand2  g054(.a(new_n113_), .b(x07), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n85_), .c(x27), .O(new_n120_));
  nand2  g058(.a(new_n81_), .b(new_n80_), .O(new_n121_));
  nand4  g059(.a(new_n121_), .b(x40), .c(x39), .d(x05), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n72_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n124_), .c(new_n79_), .O(z10));
  inv1   g064(.a(x09), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  nor2   g066(.a(new_n67_), .b(new_n72_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n84_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(x40), .b(x04), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(x35), .c(new_n83_), .d(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n91_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n80_), .c(new_n127_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n79_), .O(z11));
  inv1   g074(.a(x35), .O(new_n137_));
  nand3  g075(.a(new_n68_), .b(new_n82_), .c(new_n137_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x27), .c(x10), .O(new_n139_));
  aoi21  g077(.a(x39), .b(new_n72_), .c(new_n70_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n139_), .c(x40), .O(new_n141_));
  nand2  g079(.a(new_n69_), .b(new_n68_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x27), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n74_), .c(new_n63_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n141_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x28), .c(new_n74_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x39), .c(new_n146_), .d(new_n72_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand4  g089(.a(new_n142_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nand3  g092(.a(new_n121_), .b(x40), .c(x39), .O(new_n155_));
  nor2   g093(.a(new_n155_), .b(x13), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n72_), .c(new_n64_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n154_), .O(z13));
  nand3  g096(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n159_));
  nand4  g097(.a(new_n147_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n160_));
  nand3  g098(.a(x39), .b(new_n146_), .c(new_n72_), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n160_), .c(new_n74_), .O(new_n163_));
  nor2   g101(.a(x40), .b(x14), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n163_), .c(new_n159_), .O(new_n165_));
  nand3  g103(.a(new_n68_), .b(new_n137_), .c(x28), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x27), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n81_), .c(new_n80_), .O(new_n168_));
  nand2  g106(.a(new_n161_), .b(new_n143_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x40), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n165_), .c(new_n144_), .O(z14));
  inv1   g110(.a(x12), .O(new_n173_));
  nand4  g111(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n174_));
  nor2   g112(.a(new_n174_), .b(new_n173_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand4  g114(.a(new_n79_), .b(new_n176_), .c(x22), .d(x01), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(z16));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(x23), .c(x22), .d(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n79_), .O(z17));
  oai21  g119(.a(new_n84_), .b(x27), .c(new_n128_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x08), .O(new_n183_));
  inv1   g121(.a(new_n84_), .O(new_n184_));
  nor2   g122(.a(new_n67_), .b(x04), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n184_), .c(x09), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n183_), .c(new_n79_), .d(new_n80_), .O(z18));
  inv1   g125(.a(new_n128_), .O(new_n188_));
  oai21  g126(.a(new_n137_), .b(new_n67_), .c(new_n91_), .O(new_n189_));
  aoi21  g127(.a(x28), .b(new_n67_), .c(new_n137_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n188_), .c(new_n189_), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(new_n80_), .c(new_n127_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n79_), .O(z20));
  nand2  g131(.a(new_n135_), .b(new_n79_), .O(z19));
endmodule


