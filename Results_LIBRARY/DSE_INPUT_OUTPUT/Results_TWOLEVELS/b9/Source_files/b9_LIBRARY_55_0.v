// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n195_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n203_, new_n204_, new_n205_,
    new_n206_, new_n207_, new_n208_, new_n211_, new_n212_, new_n213_,
    new_n214_, new_n215_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n71_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g026(.a(x28), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  inv1   g028(.a(x27), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g030(.a(x40), .b(new_n81_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(new_n89_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x04), .O(new_n95_));
  inv1   g033(.a(x39), .O(new_n96_));
  nor2   g034(.a(new_n74_), .b(new_n96_), .O(z08));
  nand3  g035(.a(z08), .b(new_n95_), .c(new_n88_), .O(z02));
  nand2  g036(.a(new_n67_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x21), .O(new_n100_));
  aoi21  g038(.a(new_n66_), .b(new_n91_), .c(new_n100_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n99_), .c(new_n63_), .O(z03));
  nand2  g040(.a(new_n66_), .b(new_n91_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n99_), .c(new_n64_), .d(new_n100_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n63_), .O(z05));
  oai21  g044(.a(x40), .b(x39), .c(new_n91_), .O(new_n107_));
  oai21  g045(.a(x40), .b(x39), .c(new_n89_), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(x37), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n65_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n64_), .c(x03), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z07));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n64_), .O(z09));
  aoi21  g060(.a(x35), .b(new_n89_), .c(x36), .O(new_n123_));
  xor2a  g061(.a(x40), .b(x39), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x07), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x05), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(new_n127_));
  inv1   g065(.a(x06), .O(new_n128_));
  nor3   g066(.a(new_n63_), .b(new_n66_), .c(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n127_), .b(new_n72_), .c(new_n129_), .O(new_n130_));
  inv1   g068(.a(x05), .O(new_n131_));
  nand2  g069(.a(new_n80_), .b(new_n79_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x40), .c(x39), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n72_), .O(new_n135_));
  oai21  g073(.a(new_n130_), .b(new_n91_), .c(new_n135_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  oai21  g075(.a(new_n81_), .b(x28), .c(x39), .O(new_n138_));
  nand2  g076(.a(x40), .b(x04), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n107_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n90_), .O(new_n141_));
  nand2  g079(.a(x40), .b(new_n96_), .O(new_n142_));
  nand2  g080(.a(x40), .b(x29), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x39), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand2  g083(.a(x27), .b(x04), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n82_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n141_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n79_), .c(new_n137_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n64_), .O(z11));
  nand2  g089(.a(new_n66_), .b(new_n81_), .O(new_n152_));
  aoi22  g090(.a(new_n152_), .b(x27), .c(x39), .d(new_n72_), .O(new_n153_));
  nor3   g091(.a(x37), .b(x36), .c(x35), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n91_), .c(new_n73_), .O(new_n155_));
  nand3  g093(.a(new_n96_), .b(new_n66_), .c(new_n89_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n153_), .c(x40), .O(new_n158_));
  oai21  g096(.a(new_n74_), .b(x04), .c(new_n89_), .O(new_n159_));
  nand2  g097(.a(new_n74_), .b(new_n81_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n159_), .c(x37), .O(new_n161_));
  nor2   g099(.a(x40), .b(x27), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n161_), .c(x39), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n158_), .O(z12));
  inv1   g102(.a(x13), .O(new_n165_));
  nand2  g103(.a(x36), .b(x35), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(x28), .c(new_n74_), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(x39), .c(new_n165_), .d(new_n72_), .O(new_n168_));
  inv1   g106(.a(x18), .O(new_n169_));
  inv1   g107(.a(x19), .O(new_n170_));
  nand4  g108(.a(new_n68_), .b(x20), .c(new_n170_), .d(new_n169_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x27), .O(new_n173_));
  nor2   g111(.a(new_n133_), .b(x13), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n72_), .c(new_n63_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n173_), .O(z13));
  nand3  g114(.a(x20), .b(new_n170_), .c(new_n169_), .O(new_n177_));
  nand4  g115(.a(new_n166_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n178_));
  nor2   g116(.a(x13), .b(x04), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n178_), .c(z08), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n177_), .O(new_n181_));
  oai22  g119(.a(new_n152_), .b(new_n89_), .c(new_n74_), .d(x27), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n80_), .c(new_n79_), .O(new_n183_));
  nand2  g121(.a(x39), .b(new_n89_), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n93_), .c(new_n72_), .O(new_n185_));
  inv1   g123(.a(new_n67_), .O(new_n186_));
  nor2   g124(.a(new_n124_), .b(x13), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(new_n186_), .O(new_n188_));
  oai21  g126(.a(new_n188_), .b(new_n185_), .c(new_n66_), .O(new_n189_));
  aoi21  g127(.a(x40), .b(x27), .c(x39), .O(new_n190_));
  nand2  g128(.a(new_n74_), .b(x39), .O(new_n191_));
  oai21  g129(.a(new_n179_), .b(new_n74_), .c(new_n191_), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n91_), .c(new_n190_), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(new_n189_), .c(new_n183_), .d(new_n181_), .O(z14));
  nand4  g132(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n64_), .O(z15));
  inv1   g134(.a(x23), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(x22), .c(x01), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n64_), .O(z16));
  inv1   g137(.a(x24), .O(new_n200_));
  nand4  g138(.a(new_n200_), .b(x23), .c(x22), .d(x01), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n64_), .O(z17));
  inv1   g140(.a(z08), .O(new_n203_));
  nand2  g141(.a(x27), .b(new_n72_), .O(new_n204_));
  nand2  g142(.a(new_n91_), .b(x08), .O(new_n205_));
  aoi21  g143(.a(new_n205_), .b(new_n204_), .c(new_n81_), .O(new_n206_));
  nand2  g144(.a(new_n79_), .b(new_n137_), .O(new_n207_));
  aoi21  g145(.a(new_n206_), .b(new_n89_), .c(new_n207_), .O(new_n208_));
  oai22  g146(.a(new_n208_), .b(new_n63_), .c(new_n203_), .d(new_n87_), .O(z18));
  inv1   g147(.a(new_n150_), .O(z19));
  nand2  g148(.a(x28), .b(new_n91_), .O(new_n211_));
  aoi22  g149(.a(new_n211_), .b(x35), .c(new_n144_), .d(new_n142_), .O(new_n212_));
  nand2  g150(.a(x39), .b(new_n81_), .O(new_n213_));
  aoi21  g151(.a(new_n213_), .b(new_n107_), .c(x08), .O(new_n214_));
  oai21  g152(.a(new_n214_), .b(new_n212_), .c(new_n79_), .O(new_n215_));
  nor2   g153(.a(new_n215_), .b(x09), .O(z20));
endmodule


