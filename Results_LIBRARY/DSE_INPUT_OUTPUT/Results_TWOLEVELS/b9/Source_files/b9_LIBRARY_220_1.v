// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x16), .O(new_n77_));
  inv1   g015(.a(x40), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x32), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n76_), .O(z00));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x27), .c(x30), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x40), .c(new_n82_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g029(.a(x35), .b(new_n82_), .c(new_n83_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(new_n82_), .O(new_n93_));
  inv1   g031(.a(x39), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x04), .c(new_n94_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n93_), .c(new_n91_), .d(x40), .O(z02));
  nand2  g035(.a(x35), .b(x28), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x21), .O(new_n100_));
  inv1   g038(.a(x37), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n64_), .c(new_n100_), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n79_), .O(z03));
  inv1   g041(.a(new_n79_), .O(new_n104_));
  nand2  g042(.a(new_n101_), .b(new_n64_), .O(new_n105_));
  nand4  g043(.a(new_n105_), .b(new_n99_), .c(new_n104_), .d(new_n100_), .O(z04));
  aoi21  g044(.a(x28), .b(x27), .c(x37), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(new_n79_), .O(z05));
  aoi21  g046(.a(new_n104_), .b(new_n64_), .c(new_n83_), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(x37), .c(new_n104_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n63_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x14), .O(new_n116_));
  inv1   g054(.a(x25), .O(new_n117_));
  nand3  g055(.a(x38), .b(new_n117_), .c(new_n113_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  oai21  g057(.a(new_n115_), .b(x31), .c(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n104_), .c(x03), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z07));
  aoi21  g060(.a(new_n94_), .b(x32), .c(new_n78_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n104_), .O(z09));
  nand4  g063(.a(new_n85_), .b(new_n72_), .c(x27), .d(x07), .O(new_n126_));
  nand2  g064(.a(new_n73_), .b(x05), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n65_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n104_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  inv1   g070(.a(x09), .O(new_n133_));
  inv1   g071(.a(x30), .O(new_n134_));
  nand4  g072(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(x04), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x32), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x40), .O(new_n137_));
  nand3  g075(.a(x40), .b(x39), .c(x29), .O(new_n138_));
  nor2   g076(.a(new_n64_), .b(new_n65_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n84_), .c(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n84_), .b(new_n64_), .c(new_n132_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n134_), .c(new_n133_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n137_), .O(z11));
  nand3  g082(.a(new_n101_), .b(new_n67_), .c(new_n66_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x27), .c(x10), .O(new_n146_));
  nor2   g084(.a(new_n78_), .b(new_n65_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n94_), .c(new_n64_), .O(new_n148_));
  nand2  g086(.a(x39), .b(new_n65_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n98_), .c(new_n101_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n146_), .c(x32), .O(new_n152_));
  oai21  g090(.a(new_n70_), .b(new_n64_), .c(new_n78_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(z12));
  inv1   g092(.a(x19), .O(new_n155_));
  nor2   g093(.a(new_n79_), .b(new_n70_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x27), .c(x20), .d(new_n155_), .O(new_n157_));
  nor2   g095(.a(x13), .b(x04), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n73_), .c(x32), .O(new_n159_));
  oai21  g097(.a(new_n157_), .b(x18), .c(new_n159_), .O(z13));
  inv1   g098(.a(new_n70_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  nand3  g100(.a(x20), .b(new_n155_), .c(new_n162_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x40), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n161_), .c(new_n158_), .O(new_n165_));
  nand4  g103(.a(x27), .b(x20), .c(new_n155_), .d(new_n162_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n70_), .c(new_n94_), .O(new_n167_));
  oai21  g105(.a(new_n147_), .b(x13), .c(new_n64_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n165_), .c(x32), .O(new_n170_));
  oai21  g108(.a(new_n166_), .b(new_n70_), .c(new_n78_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(z14));
  inv1   g110(.a(x12), .O(new_n173_));
  nand4  g111(.a(new_n104_), .b(x34), .c(x27), .d(x26), .O(new_n174_));
  nor2   g112(.a(new_n174_), .b(new_n173_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(x22), .c(x01), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n104_), .O(z16));
  inv1   g116(.a(x01), .O(new_n179_));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n104_), .b(new_n180_), .c(x23), .d(x22), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n179_), .O(z17));
  oai21  g120(.a(new_n84_), .b(x27), .c(new_n138_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(x08), .O(new_n184_));
  inv1   g122(.a(new_n84_), .O(new_n185_));
  nor2   g123(.a(new_n64_), .b(x04), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(x09), .O(new_n187_));
  nand4  g125(.a(new_n187_), .b(new_n184_), .c(new_n104_), .d(new_n134_), .O(z18));
  inv1   g126(.a(new_n138_), .O(new_n189_));
  oai21  g127(.a(new_n66_), .b(new_n64_), .c(new_n132_), .O(new_n190_));
  aoi21  g128(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n191_));
  oai21  g129(.a(new_n191_), .b(new_n189_), .c(new_n190_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n134_), .c(new_n133_), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n104_), .O(z20));
  nand2  g132(.a(new_n143_), .b(new_n137_), .O(z19));
endmodule


