// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n196_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n204_, new_n205_,
    new_n206_, new_n207_, new_n210_, new_n211_, new_n212_, new_n213_,
    new_n214_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n71_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n64_), .c(x04), .O(z01));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x04), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x39), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n86_), .c(x40), .O(new_n92_));
  nand2  g030(.a(new_n74_), .b(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(z02));
  nand2  g032(.a(new_n67_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x27), .O(new_n96_));
  aoi21  g034(.a(new_n66_), .b(new_n96_), .c(new_n63_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n95_), .c(new_n99_), .O(z04));
  nor2   g038(.a(new_n87_), .b(new_n96_), .O(new_n101_));
  inv1   g039(.a(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n64_), .c(new_n66_), .O(z05));
  oai21  g041(.a(new_n101_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n65_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n64_), .c(x03), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z07));
  inv1   g053(.a(x39), .O(new_n116_));
  nor2   g054(.a(new_n74_), .b(new_n116_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  aoi21  g060(.a(x35), .b(new_n87_), .c(x36), .O(new_n123_));
  xor2a  g061(.a(x40), .b(x39), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x07), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x05), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(new_n127_));
  inv1   g065(.a(x06), .O(new_n128_));
  nor3   g066(.a(new_n63_), .b(new_n66_), .c(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n127_), .b(new_n72_), .c(new_n129_), .O(new_n130_));
  inv1   g068(.a(x05), .O(new_n131_));
  nand2  g069(.a(new_n80_), .b(new_n79_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x40), .c(x39), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n72_), .O(new_n135_));
  oai21  g073(.a(new_n130_), .b(new_n96_), .c(new_n135_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  inv1   g075(.a(x08), .O(new_n138_));
  oai21  g076(.a(x40), .b(x39), .c(new_n96_), .O(new_n139_));
  oai21  g077(.a(new_n81_), .b(x28), .c(x39), .O(new_n140_));
  nand2  g078(.a(x40), .b(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n139_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n138_), .O(new_n143_));
  nand2  g081(.a(x40), .b(new_n116_), .O(new_n144_));
  inv1   g082(.a(x29), .O(new_n145_));
  oai21  g083(.a(new_n74_), .b(new_n145_), .c(x39), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g085(.a(x27), .b(x04), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n82_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n143_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n79_), .c(new_n137_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n64_), .O(z11));
  nand2  g091(.a(new_n66_), .b(new_n81_), .O(new_n154_));
  aoi22  g092(.a(new_n154_), .b(x27), .c(x39), .d(new_n72_), .O(new_n155_));
  nor3   g093(.a(x37), .b(x36), .c(x35), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n96_), .c(new_n73_), .O(new_n157_));
  nand3  g095(.a(new_n116_), .b(new_n66_), .c(new_n87_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n155_), .c(x40), .O(new_n160_));
  oai21  g098(.a(new_n74_), .b(x04), .c(new_n87_), .O(new_n161_));
  nand2  g099(.a(new_n74_), .b(new_n81_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n161_), .c(x37), .O(new_n163_));
  nor2   g101(.a(x40), .b(x27), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n163_), .c(x39), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n160_), .O(z12));
  inv1   g104(.a(x13), .O(new_n167_));
  nand2  g105(.a(x36), .b(x35), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(x28), .c(new_n74_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x39), .c(new_n167_), .d(new_n72_), .O(new_n170_));
  inv1   g108(.a(x18), .O(new_n171_));
  inv1   g109(.a(x19), .O(new_n172_));
  nand4  g110(.a(new_n68_), .b(x20), .c(new_n172_), .d(new_n171_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n170_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nor2   g113(.a(new_n133_), .b(x13), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n72_), .c(new_n63_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n175_), .O(z13));
  nand3  g116(.a(x20), .b(new_n172_), .c(new_n171_), .O(new_n179_));
  nand4  g117(.a(new_n168_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n117_), .c(new_n167_), .d(new_n72_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n179_), .O(new_n182_));
  oai21  g120(.a(new_n154_), .b(new_n87_), .c(x27), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n80_), .c(new_n79_), .O(new_n184_));
  nand2  g122(.a(x39), .b(new_n87_), .O(new_n185_));
  nand2  g123(.a(x40), .b(new_n81_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(new_n72_), .O(new_n187_));
  inv1   g125(.a(new_n67_), .O(new_n188_));
  nor2   g126(.a(new_n124_), .b(x13), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n188_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n187_), .c(new_n66_), .O(new_n191_));
  aoi21  g129(.a(x40), .b(x27), .c(x39), .O(new_n192_));
  nand3  g130(.a(new_n141_), .b(new_n93_), .c(new_n167_), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(new_n96_), .c(new_n192_), .O(new_n194_));
  nand4  g132(.a(new_n194_), .b(new_n191_), .c(new_n184_), .d(new_n182_), .O(z14));
  nand4  g133(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n64_), .O(z15));
  inv1   g135(.a(x23), .O(new_n198_));
  nand4  g136(.a(new_n64_), .b(new_n198_), .c(x22), .d(x01), .O(new_n199_));
  inv1   g137(.a(new_n199_), .O(z16));
  inv1   g138(.a(x24), .O(new_n201_));
  nand4  g139(.a(new_n201_), .b(x23), .c(x22), .d(x01), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n64_), .O(z17));
  oai22  g141(.a(new_n118_), .b(new_n145_), .c(new_n88_), .d(x27), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(x08), .O(new_n205_));
  nor2   g143(.a(new_n96_), .b(x04), .O(new_n206_));
  aoi21  g144(.a(new_n206_), .b(new_n82_), .c(x09), .O(new_n207_));
  nand4  g145(.a(new_n207_), .b(new_n205_), .c(new_n64_), .d(new_n79_), .O(z18));
  inv1   g146(.a(new_n152_), .O(z19));
  aoi21  g147(.a(x28), .b(new_n96_), .c(new_n81_), .O(new_n210_));
  aoi21  g148(.a(new_n146_), .b(new_n144_), .c(new_n210_), .O(new_n211_));
  nand2  g149(.a(x39), .b(new_n81_), .O(new_n212_));
  aoi21  g150(.a(new_n212_), .b(new_n139_), .c(x08), .O(new_n213_));
  oai21  g151(.a(new_n213_), .b(new_n211_), .c(new_n79_), .O(new_n214_));
  nor2   g152(.a(new_n214_), .b(x09), .O(z20));
endmodule


