// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n189_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x12), .O(new_n64_));
  nand2  g002(.a(x40), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x15), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n63_), .c(new_n65_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(x12), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n71_), .O(z00));
  inv1   g016(.a(new_n65_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g029(.a(x35), .b(new_n64_), .c(new_n83_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(new_n64_), .O(new_n93_));
  inv1   g031(.a(x39), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x04), .c(new_n94_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n93_), .c(new_n91_), .d(x40), .O(z02));
  nand2  g035(.a(new_n67_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(x27), .O(new_n100_));
  aoi21  g038(.a(new_n66_), .b(new_n100_), .c(new_n99_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n79_), .O(z03));
  nand2  g040(.a(new_n66_), .b(new_n100_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n98_), .c(new_n65_), .d(new_n99_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n79_), .O(z05));
  aoi21  g044(.a(new_n65_), .b(new_n100_), .c(new_n83_), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(x37), .c(new_n65_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n72_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n65_), .O(z07));
  aoi21  g057(.a(new_n94_), .b(x12), .c(new_n74_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n65_), .O(z09));
  inv1   g060(.a(x04), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  nand4  g062(.a(x40), .b(x39), .c(x12), .d(x05), .O(new_n125_));
  oai21  g063(.a(z08), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n85_), .c(new_n123_), .O(new_n127_));
  nand3  g065(.a(new_n65_), .b(x37), .c(x06), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n81_), .b(new_n80_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x40), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n94_), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(x12), .c(x05), .d(new_n123_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n130_), .O(z10));
  nand2  g073(.a(new_n65_), .b(new_n100_), .O(new_n136_));
  nand2  g074(.a(x40), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n83_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x12), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n136_), .c(x08), .O(new_n140_));
  inv1   g078(.a(new_n84_), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x12), .c(new_n74_), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n141_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n140_), .c(new_n80_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(x09), .c(new_n65_), .O(z11));
  inv1   g085(.a(x35), .O(new_n148_));
  nand3  g086(.a(new_n66_), .b(new_n82_), .c(new_n148_), .O(new_n149_));
  nor2   g087(.a(new_n74_), .b(x27), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x12), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n73_), .O(new_n153_));
  oai21  g091(.a(x27), .b(new_n123_), .c(x12), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x40), .O(new_n155_));
  oai21  g093(.a(new_n94_), .b(x04), .c(x12), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x40), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n67_), .c(new_n66_), .O(new_n158_));
  or2    g096(.a(z08), .b(x27), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n158_), .c(new_n155_), .d(new_n153_), .O(z12));
  inv1   g098(.a(x13), .O(new_n161_));
  nand2  g099(.a(x36), .b(x35), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(x28), .c(new_n74_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x39), .c(new_n161_), .d(new_n123_), .O(new_n164_));
  inv1   g102(.a(x18), .O(new_n165_));
  inv1   g103(.a(x19), .O(new_n166_));
  nand4  g104(.a(new_n68_), .b(x20), .c(new_n166_), .d(new_n165_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n164_), .c(new_n100_), .O(new_n168_));
  nand4  g106(.a(new_n131_), .b(x39), .c(new_n161_), .d(new_n123_), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(x12), .c(new_n74_), .O(new_n170_));
  or2    g108(.a(new_n170_), .b(new_n168_), .O(z13));
  nand3  g109(.a(x20), .b(new_n166_), .c(new_n165_), .O(new_n172_));
  nand4  g110(.a(new_n162_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n173_));
  oai21  g111(.a(x13), .b(x04), .c(x40), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n173_), .c(x39), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x12), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x40), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n172_), .O(new_n178_));
  nand3  g116(.a(new_n66_), .b(new_n148_), .c(x28), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x27), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n81_), .c(new_n80_), .O(new_n181_));
  inv1   g119(.a(new_n68_), .O(new_n182_));
  oai22  g120(.a(new_n150_), .b(new_n182_), .c(x13), .d(x04), .O(new_n183_));
  nand2  g121(.a(new_n69_), .b(new_n94_), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n183_), .c(new_n181_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(x12), .O(new_n186_));
  nand2  g124(.a(new_n69_), .b(new_n74_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n186_), .c(new_n178_), .O(z14));
  nand4  g126(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n189_));
  inv1   g127(.a(new_n189_), .O(z15));
  inv1   g128(.a(x23), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(x22), .c(x01), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n65_), .O(z16));
  inv1   g131(.a(x24), .O(new_n194_));
  nand4  g132(.a(new_n194_), .b(x23), .c(x22), .d(x01), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n65_), .O(z17));
  nand3  g134(.a(x40), .b(x39), .c(x29), .O(new_n197_));
  oai21  g135(.a(new_n84_), .b(x27), .c(new_n197_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(x08), .O(new_n199_));
  nor2   g137(.a(new_n79_), .b(x09), .O(new_n200_));
  nor2   g138(.a(new_n100_), .b(x04), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n141_), .c(x30), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n200_), .c(new_n199_), .O(z18));
  nor2   g141(.a(new_n146_), .b(x09), .O(z19));
  aoi21  g142(.a(x28), .b(new_n100_), .c(new_n148_), .O(new_n205_));
  nor2   g143(.a(new_n205_), .b(new_n143_), .O(new_n206_));
  oai21  g144(.a(new_n148_), .b(new_n100_), .c(x12), .O(new_n207_));
  nand2  g145(.a(new_n74_), .b(new_n100_), .O(new_n208_));
  aoi21  g146(.a(new_n208_), .b(new_n207_), .c(x08), .O(new_n209_));
  oai21  g147(.a(new_n209_), .b(new_n206_), .c(new_n80_), .O(new_n210_));
  nor2   g148(.a(new_n210_), .b(x09), .O(z20));
endmodule


