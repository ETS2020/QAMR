// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  oai21  g004(.a(x36), .b(x35), .c(x39), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(x04), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(new_n68_), .c(x27), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  nand3  g011(.a(x39), .b(x10), .c(new_n73_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(x40), .c(new_n63_), .O(new_n76_));
  oai21  g014(.a(new_n66_), .b(x16), .c(new_n76_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x35), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(x28), .c(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n66_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  oai21  g028(.a(new_n64_), .b(x35), .c(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n89_), .c(x04), .O(new_n92_));
  inv1   g030(.a(x39), .O(new_n93_));
  nor2   g031(.a(new_n64_), .b(new_n93_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(new_n88_), .O(z02));
  nand2  g033(.a(new_n70_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x27), .O(new_n97_));
  aoi21  g035(.a(new_n69_), .b(new_n97_), .c(new_n66_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n96_), .c(new_n100_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n66_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n63_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n65_), .O(z07));
  inv1   g053(.a(new_n94_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n65_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(x40), .b(new_n93_), .O(new_n122_));
  nand2  g060(.a(new_n64_), .b(x15), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n121_), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x05), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n124_), .c(new_n82_), .O(new_n127_));
  nand3  g065(.a(new_n65_), .b(x37), .c(x06), .O(new_n128_));
  oai21  g066(.a(new_n127_), .b(x04), .c(new_n128_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x27), .O(new_n130_));
  oai21  g068(.a(x32), .b(x30), .c(x40), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n93_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x05), .c(new_n73_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(z10));
  nand3  g072(.a(x35), .b(new_n90_), .c(new_n73_), .O(new_n135_));
  aoi22  g073(.a(new_n135_), .b(x40), .c(new_n65_), .d(new_n97_), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(x08), .O(new_n137_));
  nor2   g075(.a(new_n80_), .b(x28), .O(new_n138_));
  nand2  g076(.a(x39), .b(x29), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x40), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  aoi22  g079(.a(new_n141_), .b(new_n138_), .c(new_n140_), .d(new_n123_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n137_), .c(new_n78_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(x09), .c(new_n65_), .O(z11));
  nand3  g082(.a(new_n69_), .b(new_n81_), .c(new_n80_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x27), .c(x10), .O(new_n146_));
  aoi22  g084(.a(new_n71_), .b(x27), .c(x39), .d(new_n73_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n146_), .c(x40), .O(new_n148_));
  nand2  g086(.a(new_n71_), .b(x27), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n64_), .c(x15), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n148_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x28), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n73_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  nand4  g095(.a(new_n71_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n155_), .c(new_n64_), .O(new_n159_));
  nor2   g097(.a(new_n158_), .b(new_n63_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(x27), .O(new_n161_));
  nand3  g099(.a(new_n132_), .b(new_n152_), .c(new_n73_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n161_), .O(z13));
  oai21  g101(.a(new_n93_), .b(x04), .c(x40), .O(new_n164_));
  aoi21  g102(.a(new_n64_), .b(x15), .c(x13), .O(new_n165_));
  aoi22  g103(.a(new_n165_), .b(new_n164_), .c(x35), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(new_n69_), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n170_));
  nand4  g108(.a(new_n153_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n171_));
  nor2   g109(.a(x13), .b(x04), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n171_), .c(new_n94_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n170_), .O(new_n174_));
  aoi21  g112(.a(x27), .b(x15), .c(x40), .O(new_n175_));
  aoi21  g113(.a(new_n79_), .b(new_n78_), .c(new_n93_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n172_), .c(new_n64_), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n97_), .c(new_n175_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n174_), .c(new_n169_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nor2   g118(.a(new_n119_), .b(new_n180_), .O(z15));
  inv1   g119(.a(x23), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(x22), .c(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n65_), .O(z16));
  inv1   g122(.a(x01), .O(new_n185_));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n65_), .b(new_n186_), .c(x23), .d(x22), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(new_n185_), .O(z17));
  nand2  g126(.a(x27), .b(new_n73_), .O(new_n189_));
  nand2  g127(.a(new_n97_), .b(x08), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n189_), .c(new_n80_), .O(new_n191_));
  inv1   g129(.a(x09), .O(new_n192_));
  nand2  g130(.a(new_n78_), .b(new_n192_), .O(new_n193_));
  aoi21  g131(.a(new_n191_), .b(new_n90_), .c(new_n193_), .O(new_n194_));
  oai22  g132(.a(new_n194_), .b(new_n66_), .c(new_n116_), .d(new_n87_), .O(z18));
  nor2   g133(.a(new_n143_), .b(x09), .O(z19));
  nor2   g134(.a(x40), .b(x35), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n89_), .c(x15), .O(new_n198_));
  aoi21  g136(.a(x35), .b(x27), .c(x08), .O(new_n199_));
  aoi21  g137(.a(x39), .b(x29), .c(x35), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n199_), .c(x40), .O(new_n201_));
  nand2  g139(.a(new_n94_), .b(x29), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(x28), .c(new_n97_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n201_), .c(new_n198_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n78_), .c(new_n192_), .O(new_n205_));
  nand2  g143(.a(new_n205_), .b(new_n65_), .O(z20));
endmodule


