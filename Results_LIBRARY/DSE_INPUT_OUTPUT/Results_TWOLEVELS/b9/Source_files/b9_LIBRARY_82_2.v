// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n185_, new_n186_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n201_, new_n202_, new_n203_, new_n204_, new_n205_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x13), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n63_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n71_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  aoi21  g025(.a(new_n82_), .b(x04), .c(new_n87_), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  inv1   g027(.a(x27), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n88_), .c(x40), .d(x39), .O(z02));
  nand2  g030(.a(new_n67_), .b(x27), .O(new_n93_));
  nand3  g031(.a(new_n64_), .b(new_n66_), .c(new_n90_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n93_), .c(new_n64_), .d(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand4  g034(.a(new_n94_), .b(new_n93_), .c(new_n64_), .d(new_n96_), .O(z04));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n64_), .c(new_n66_), .O(z05));
  inv1   g037(.a(x13), .O(new_n100_));
  oai22  g038(.a(x40), .b(new_n100_), .c(new_n81_), .d(new_n90_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(x37), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n65_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n64_), .O(z07));
  nand2  g051(.a(x40), .b(x39), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n64_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  inv1   g056(.a(x39), .O(new_n119_));
  nor2   g057(.a(x40), .b(x13), .O(new_n120_));
  aoi21  g058(.a(x40), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(x05), .O(new_n122_));
  oai21  g060(.a(new_n121_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n83_), .c(new_n72_), .O(new_n124_));
  nand3  g062(.a(new_n64_), .b(x37), .c(x06), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n79_), .b(new_n78_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x40), .c(x39), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n72_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n127_), .O(z10));
  nand2  g071(.a(new_n64_), .b(new_n90_), .O(new_n134_));
  oai21  g072(.a(new_n82_), .b(x04), .c(x40), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n134_), .c(x08), .O(new_n136_));
  inv1   g074(.a(new_n82_), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x40), .c(new_n120_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n137_), .c(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n136_), .c(new_n78_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(x09), .O(z11));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  nor2   g082(.a(new_n63_), .b(x27), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n73_), .O(new_n146_));
  oai21  g084(.a(new_n119_), .b(x04), .c(new_n67_), .O(new_n147_));
  oai21  g085(.a(x28), .b(x13), .c(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n63_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n66_), .O(new_n151_));
  aoi21  g089(.a(x27), .b(new_n100_), .c(x40), .O(new_n152_));
  nand2  g090(.a(x40), .b(x04), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x39), .c(x27), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n151_), .c(new_n146_), .O(z12));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n63_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n100_), .d(new_n72_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n68_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nor2   g102(.a(new_n130_), .b(x13), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n72_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n164_), .c(new_n64_), .O(z13));
  nand3  g105(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n168_));
  nand4  g106(.a(new_n157_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n169_));
  nor2   g107(.a(new_n63_), .b(x04), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n169_), .c(x39), .d(new_n100_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  inv1   g110(.a(x35), .O(new_n173_));
  nand3  g111(.a(new_n66_), .b(new_n173_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n79_), .c(new_n78_), .O(new_n176_));
  nand3  g114(.a(x39), .b(new_n100_), .c(new_n72_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n67_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n149_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n66_), .O(new_n180_));
  aoi21  g118(.a(x40), .b(x27), .c(new_n100_), .O(new_n181_));
  nand2  g119(.a(new_n170_), .b(x39), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n90_), .c(new_n181_), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(new_n180_), .c(new_n176_), .d(new_n172_), .O(z14));
  inv1   g122(.a(x12), .O(new_n185_));
  nand4  g123(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n185_), .O(z15));
  inv1   g125(.a(x23), .O(new_n188_));
  nand3  g126(.a(new_n188_), .b(x22), .c(x01), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n64_), .O(z16));
  inv1   g128(.a(x01), .O(new_n191_));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n64_), .b(new_n192_), .c(x23), .d(x22), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(new_n191_), .O(z17));
  inv1   g132(.a(x29), .O(new_n195_));
  oai22  g133(.a(new_n114_), .b(new_n195_), .c(new_n82_), .d(x27), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(x08), .O(new_n197_));
  nor2   g135(.a(new_n90_), .b(x04), .O(new_n198_));
  aoi21  g136(.a(new_n198_), .b(new_n137_), .c(x09), .O(new_n199_));
  nand4  g137(.a(new_n199_), .b(new_n197_), .c(new_n64_), .d(new_n78_), .O(z18));
  aoi21  g138(.a(x28), .b(new_n90_), .c(new_n173_), .O(new_n201_));
  nor2   g139(.a(new_n201_), .b(new_n139_), .O(new_n202_));
  nand2  g140(.a(x40), .b(new_n173_), .O(new_n203_));
  aoi21  g141(.a(new_n203_), .b(new_n134_), .c(x08), .O(new_n204_));
  oai21  g142(.a(new_n204_), .b(new_n202_), .c(new_n78_), .O(new_n205_));
  nor2   g143(.a(new_n205_), .b(x09), .O(z20));
  nor2   g144(.a(new_n142_), .b(x09), .O(z19));
endmodule


