// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n109_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x37), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x40), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x35), .c(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n63_), .c(new_n67_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  nand2  g009(.a(x10), .b(new_n71_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x37), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n70_), .c(new_n66_), .O(new_n77_));
  oai21  g015(.a(new_n65_), .b(x16), .c(new_n77_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n83_), .c(x04), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x37), .O(new_n94_));
  inv1   g032(.a(x09), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x04), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n79_), .c(x27), .O(new_n97_));
  oai21  g035(.a(new_n97_), .b(new_n96_), .c(new_n68_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n94_), .O(z02));
  nand2  g037(.a(x35), .b(x28), .O(new_n100_));
  nor2   g038(.a(new_n63_), .b(new_n67_), .O(new_n101_));
  nor2   g039(.a(x40), .b(x37), .O(new_n102_));
  oai21  g040(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  oai21  g041(.a(x40), .b(new_n67_), .c(new_n63_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n103_), .c(x21), .O(z03));
  nand2  g043(.a(new_n64_), .b(x21), .O(new_n106_));
  nand2  g044(.a(new_n102_), .b(new_n67_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(new_n103_), .O(z04));
  nand3  g046(.a(new_n68_), .b(x28), .c(x27), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n63_), .O(z05));
  nor2   g048(.a(new_n82_), .b(new_n67_), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n68_), .c(x37), .O(z06));
  inv1   g050(.a(x33), .O(new_n113_));
  nand2  g051(.a(x17), .b(new_n66_), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  oai21  g053(.a(x25), .b(new_n115_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n115_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  oai22  g058(.a(new_n120_), .b(x14), .c(new_n117_), .d(x31), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x03), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n64_), .O(z07));
  nand3  g061(.a(x40), .b(x39), .c(x37), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(z08));
  inv1   g063(.a(x11), .O(new_n126_));
  nand4  g064(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n126_), .O(z09));
  inv1   g066(.a(x07), .O(new_n129_));
  inv1   g067(.a(x39), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(x37), .c(new_n68_), .O(new_n131_));
  nand4  g069(.a(x40), .b(x39), .c(x37), .d(x05), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(new_n129_), .c(new_n132_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n84_), .c(x27), .O(new_n134_));
  aoi21  g072(.a(new_n80_), .b(new_n79_), .c(new_n68_), .O(new_n135_));
  nand4  g073(.a(new_n135_), .b(x39), .c(x37), .d(x05), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n71_), .O(new_n138_));
  nand2  g076(.a(new_n101_), .b(x06), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(z10));
  nand2  g078(.a(new_n64_), .b(new_n67_), .O(new_n141_));
  nand3  g079(.a(x35), .b(new_n82_), .c(new_n71_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x37), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n141_), .c(x08), .O(new_n144_));
  inv1   g082(.a(new_n83_), .O(new_n145_));
  nand2  g083(.a(x39), .b(x29), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x37), .c(new_n68_), .O(new_n147_));
  nand2  g085(.a(x27), .b(x04), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n145_), .c(new_n147_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n144_), .c(new_n79_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(x09), .c(new_n64_), .O(z11));
  aoi21  g089(.a(new_n100_), .b(new_n63_), .c(new_n67_), .O(new_n152_));
  nand3  g090(.a(x39), .b(x10), .c(new_n71_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(x37), .c(new_n67_), .O(new_n154_));
  oai21  g092(.a(new_n152_), .b(x40), .c(new_n154_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n68_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n71_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand2  g099(.a(new_n100_), .b(new_n63_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x27), .O(new_n165_));
  oai21  g103(.a(x32), .b(x30), .c(x39), .O(new_n166_));
  nor3   g104(.a(new_n166_), .b(x13), .c(x04), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n63_), .c(x40), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(z13));
  nand3  g107(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n170_));
  nand4  g108(.a(new_n157_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n74_), .c(new_n156_), .d(new_n71_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n170_), .O(new_n173_));
  oai21  g111(.a(new_n166_), .b(x04), .c(x37), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(x40), .c(new_n156_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n67_), .O(new_n176_));
  oai21  g114(.a(new_n100_), .b(x40), .c(new_n63_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n176_), .c(new_n173_), .O(z14));
  inv1   g116(.a(x12), .O(new_n179_));
  nor2   g117(.a(new_n127_), .b(new_n179_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n64_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n64_), .O(z17));
  inv1   g124(.a(x35), .O(new_n187_));
  nand2  g125(.a(x27), .b(new_n71_), .O(new_n188_));
  nand2  g126(.a(new_n67_), .b(x08), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n188_), .c(new_n187_), .O(new_n190_));
  nand2  g128(.a(new_n79_), .b(new_n95_), .O(new_n191_));
  aoi21  g129(.a(new_n190_), .b(new_n82_), .c(new_n191_), .O(new_n192_));
  oai22  g130(.a(new_n192_), .b(new_n65_), .c(new_n89_), .d(new_n75_), .O(z18));
  nor2   g131(.a(new_n150_), .b(x09), .O(z19));
  aoi21  g132(.a(x28), .b(new_n67_), .c(new_n187_), .O(new_n195_));
  nor2   g133(.a(new_n195_), .b(new_n147_), .O(new_n196_));
  nand2  g134(.a(x37), .b(new_n187_), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n141_), .c(x08), .O(new_n198_));
  oai21  g136(.a(new_n198_), .b(new_n196_), .c(new_n79_), .O(new_n199_));
  nor2   g137(.a(new_n199_), .b(x09), .O(z20));
endmodule


