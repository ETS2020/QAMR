// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x39), .O(new_n67_));
  nor3   g005(.a(new_n67_), .b(new_n66_), .c(x04), .O(new_n68_));
  aoi21  g006(.a(new_n65_), .b(x27), .c(new_n68_), .O(new_n69_));
  inv1   g007(.a(x40), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x39), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x30), .O(new_n74_));
  inv1   g012(.a(x32), .O(new_n75_));
  inv1   g013(.a(x35), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  oai21  g015(.a(new_n76_), .b(x28), .c(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x27), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n75_), .c(new_n74_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n71_), .c(x04), .O(z01));
  inv1   g019(.a(x02), .O(new_n82_));
  nand2  g020(.a(x29), .b(x08), .O(new_n83_));
  nor2   g021(.a(new_n76_), .b(x28), .O(new_n84_));
  oai21  g022(.a(x27), .b(x08), .c(new_n84_), .O(new_n85_));
  aoi22  g023(.a(new_n85_), .b(x04), .c(new_n83_), .d(new_n82_), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n70_), .c(x39), .O(z02));
  inv1   g025(.a(new_n71_), .O(new_n88_));
  nand2  g026(.a(x35), .b(x28), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  inv1   g028(.a(x21), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n90_), .c(new_n88_), .O(z03));
  nand2  g033(.a(new_n93_), .b(new_n92_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n90_), .c(new_n71_), .d(new_n91_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n88_), .O(z05));
  inv1   g037(.a(x28), .O(new_n100_));
  aoi21  g038(.a(new_n71_), .b(new_n92_), .c(new_n100_), .O(new_n101_));
  oai21  g039(.a(new_n101_), .b(x37), .c(new_n71_), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  inv1   g041(.a(x15), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n104_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(new_n111_));
  oai22  g049(.a(new_n111_), .b(x14), .c(new_n108_), .d(x31), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n71_), .c(x03), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z07));
  nand2  g052(.a(x40), .b(x39), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n71_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  inv1   g057(.a(x04), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  oai22  g060(.a(new_n115_), .b(new_n121_), .c(x39), .d(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n78_), .c(new_n120_), .O(new_n124_));
  nand3  g062(.a(new_n71_), .b(x37), .c(x06), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  nor2   g065(.a(x32), .b(x30), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n70_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(x39), .c(x05), .d(new_n120_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  oai21  g070(.a(new_n70_), .b(x29), .c(x39), .O(new_n133_));
  nand2  g071(.a(x27), .b(x04), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x35), .c(new_n100_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand3  g074(.a(x35), .b(new_n100_), .c(new_n120_), .O(new_n137_));
  aoi22  g075(.a(new_n137_), .b(x40), .c(new_n71_), .d(new_n92_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(x08), .c(new_n136_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n74_), .c(new_n132_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z11));
  nor3   g079(.a(x37), .b(x36), .c(x35), .O(new_n142_));
  nor2   g080(.a(new_n70_), .b(x27), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n142_), .c(new_n66_), .O(new_n144_));
  oai21  g082(.a(new_n70_), .b(new_n120_), .c(x39), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n92_), .O(new_n146_));
  nand2  g084(.a(x39), .b(new_n120_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n89_), .c(new_n93_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(new_n146_), .c(new_n144_), .d(new_n71_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x28), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x39), .c(new_n150_), .d(new_n120_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  inv1   g093(.a(new_n64_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x20), .c(new_n155_), .d(new_n154_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nor3   g097(.a(new_n128_), .b(x13), .c(x04), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n70_), .c(x39), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n159_), .O(z13));
  inv1   g100(.a(new_n143_), .O(new_n163_));
  nor2   g101(.a(x19), .b(x18), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n163_), .c(new_n156_), .d(x20), .O(new_n165_));
  oai21  g103(.a(x13), .b(x04), .c(new_n165_), .O(new_n166_));
  oai21  g104(.a(new_n128_), .b(new_n67_), .c(new_n92_), .O(new_n167_));
  nand4  g105(.a(new_n151_), .b(new_n75_), .c(new_n74_), .d(x28), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x39), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n155_), .c(new_n154_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  aoi21  g109(.a(new_n128_), .b(x28), .c(new_n67_), .O(new_n172_));
  nand2  g110(.a(new_n67_), .b(new_n100_), .O(new_n173_));
  oai21  g111(.a(new_n172_), .b(x35), .c(new_n173_), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n93_), .c(new_n88_), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(new_n171_), .c(new_n167_), .d(new_n166_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n71_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand4  g117(.a(new_n71_), .b(new_n179_), .c(x22), .d(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(x23), .c(x22), .d(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n71_), .O(z17));
  nand2  g122(.a(x27), .b(new_n120_), .O(new_n185_));
  nand2  g123(.a(new_n92_), .b(x08), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(new_n76_), .O(new_n187_));
  nand2  g125(.a(new_n74_), .b(new_n132_), .O(new_n188_));
  aoi21  g126(.a(new_n187_), .b(new_n100_), .c(new_n188_), .O(new_n189_));
  oai22  g127(.a(new_n189_), .b(new_n88_), .c(new_n115_), .d(new_n83_), .O(z18));
  nand2  g128(.a(new_n140_), .b(new_n71_), .O(z19));
  oai21  g129(.a(new_n100_), .b(x27), .c(x35), .O(new_n192_));
  and2   g130(.a(new_n192_), .b(new_n133_), .O(new_n193_));
  nand2  g131(.a(new_n71_), .b(new_n92_), .O(new_n194_));
  nand2  g132(.a(x40), .b(new_n76_), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n194_), .c(x08), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n193_), .c(new_n74_), .O(new_n197_));
  nor2   g135(.a(new_n197_), .b(x09), .O(z20));
endmodule


