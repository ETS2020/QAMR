// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x07), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x07), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n70_), .O(z00));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  inv1   g020(.a(x36), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(x28), .c(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n81_), .c(new_n80_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  nand2  g029(.a(x35), .b(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(x07), .O(new_n93_));
  inv1   g031(.a(x39), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x04), .c(new_n94_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n93_), .c(new_n90_), .d(x40), .O(z02));
  nand2  g035(.a(new_n66_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(x27), .O(new_n100_));
  aoi21  g038(.a(new_n65_), .b(new_n100_), .c(new_n99_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n79_), .O(z03));
  aoi21  g040(.a(new_n65_), .b(new_n100_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n98_), .c(new_n79_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n79_), .O(z05));
  aoi21  g044(.a(x40), .b(x07), .c(x27), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n91_), .c(new_n65_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n64_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n72_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n64_), .O(z07));
  aoi21  g058(.a(new_n94_), .b(new_n71_), .c(new_n74_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n64_), .O(z09));
  inv1   g061(.a(x04), .O(new_n124_));
  nand4  g062(.a(new_n84_), .b(x40), .c(x39), .d(x05), .O(new_n125_));
  nand3  g063(.a(new_n74_), .b(x35), .c(new_n91_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n83_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x07), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n125_), .c(new_n100_), .O(new_n129_));
  inv1   g067(.a(x05), .O(new_n130_));
  nand2  g068(.a(new_n81_), .b(new_n80_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x40), .O(new_n132_));
  nor3   g070(.a(new_n132_), .b(new_n94_), .c(new_n130_), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n129_), .c(new_n124_), .O(new_n134_));
  nor2   g072(.a(new_n65_), .b(new_n100_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x06), .c(new_n79_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand3  g076(.a(x35), .b(new_n91_), .c(new_n124_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n71_), .c(new_n107_), .O(new_n140_));
  aoi21  g078(.a(x39), .b(x29), .c(x07), .O(new_n141_));
  nor2   g079(.a(new_n100_), .b(new_n124_), .O(new_n142_));
  oai22  g080(.a(new_n142_), .b(new_n92_), .c(new_n141_), .d(new_n74_), .O(new_n143_));
  oai21  g081(.a(new_n140_), .b(x08), .c(new_n143_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n80_), .c(new_n138_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n64_), .O(z11));
  nand3  g084(.a(new_n65_), .b(new_n83_), .c(new_n82_), .O(new_n147_));
  nand3  g085(.a(x40), .b(new_n100_), .c(new_n71_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n73_), .O(new_n150_));
  nor2   g088(.a(new_n74_), .b(new_n94_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n124_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n68_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n150_), .c(new_n64_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x28), .c(new_n74_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n124_), .O(new_n158_));
  inv1   g096(.a(x18), .O(new_n159_));
  inv1   g097(.a(x19), .O(new_n160_));
  nand4  g098(.a(new_n67_), .b(x20), .c(new_n160_), .d(new_n159_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n158_), .c(x07), .O(new_n162_));
  nand4  g100(.a(new_n67_), .b(new_n74_), .c(x20), .d(new_n160_), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(x18), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n162_), .c(x27), .O(new_n165_));
  nor2   g103(.a(new_n132_), .b(new_n94_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n155_), .c(new_n71_), .d(new_n124_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n165_), .O(z13));
  nand2  g106(.a(x40), .b(x13), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n131_), .c(x07), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n152_), .c(new_n100_), .O(new_n171_));
  nand3  g109(.a(x20), .b(new_n160_), .c(new_n159_), .O(new_n172_));
  nand4  g110(.a(new_n156_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n173_));
  nor2   g111(.a(x13), .b(x04), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n173_), .c(new_n151_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n172_), .O(new_n176_));
  aoi22  g114(.a(new_n174_), .b(new_n151_), .c(x35), .d(x28), .O(new_n177_));
  nand4  g115(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n177_), .c(new_n65_), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n176_), .c(new_n171_), .d(new_n64_), .O(z14));
  inv1   g119(.a(x12), .O(new_n182_));
  nand4  g120(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n182_), .O(z15));
  inv1   g122(.a(x23), .O(new_n185_));
  nand4  g123(.a(new_n64_), .b(new_n185_), .c(x22), .d(x01), .O(new_n186_));
  inv1   g124(.a(new_n186_), .O(z16));
  inv1   g125(.a(x01), .O(new_n188_));
  inv1   g126(.a(x24), .O(new_n189_));
  nand4  g127(.a(new_n64_), .b(new_n189_), .c(x23), .d(x22), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n188_), .O(z17));
  nand2  g129(.a(x40), .b(x07), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(x27), .c(new_n124_), .O(new_n193_));
  nand2  g131(.a(new_n107_), .b(x08), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n193_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(x35), .c(new_n91_), .O(new_n196_));
  oai21  g134(.a(x30), .b(x09), .c(new_n64_), .O(new_n197_));
  nand4  g135(.a(new_n151_), .b(x29), .c(x08), .d(new_n71_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n197_), .c(new_n196_), .O(z18));
  inv1   g137(.a(new_n145_), .O(z19));
  nand2  g138(.a(x39), .b(x29), .O(new_n201_));
  nor2   g139(.a(new_n91_), .b(x27), .O(new_n202_));
  nor2   g140(.a(x35), .b(x07), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n202_), .c(new_n201_), .O(new_n204_));
  inv1   g142(.a(x08), .O(new_n205_));
  aoi21  g143(.a(x35), .b(x27), .c(x07), .O(new_n206_));
  nor2   g144(.a(x40), .b(x27), .O(new_n207_));
  oai21  g145(.a(new_n207_), .b(new_n206_), .c(new_n205_), .O(new_n208_));
  oai21  g146(.a(new_n202_), .b(new_n82_), .c(new_n74_), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(new_n208_), .c(new_n204_), .O(new_n210_));
  nand3  g148(.a(new_n210_), .b(new_n80_), .c(new_n138_), .O(new_n211_));
  nand2  g149(.a(new_n211_), .b(new_n64_), .O(z20));
endmodule


