// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n191_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n205_,
    new_n206_, new_n207_, new_n208_, new_n209_, new_n210_, new_n211_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n71_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x39), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n87_), .c(x40), .O(new_n91_));
  nand2  g029(.a(new_n74_), .b(x39), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(z02));
  nand2  g031(.a(new_n67_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x27), .O(new_n95_));
  nand3  g033(.a(new_n64_), .b(new_n66_), .c(new_n95_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n94_), .c(new_n64_), .d(x21), .O(z03));
  nor2   g035(.a(new_n63_), .b(x21), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n96_), .c(new_n94_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n64_), .c(new_n66_), .O(z05));
  oai21  g039(.a(x40), .b(x39), .c(new_n95_), .O(new_n102_));
  oai21  g040(.a(x40), .b(x39), .c(new_n81_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(x37), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n65_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n64_), .c(x03), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z07));
  nand2  g053(.a(x40), .b(x39), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  aoi21  g058(.a(x35), .b(new_n81_), .c(x36), .O(new_n121_));
  xor2a  g059(.a(x40), .b(x39), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x07), .O(new_n123_));
  nand2  g061(.a(z08), .b(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n121_), .O(new_n125_));
  inv1   g063(.a(x06), .O(new_n126_));
  nor3   g064(.a(new_n63_), .b(new_n66_), .c(new_n126_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n72_), .c(new_n127_), .O(new_n128_));
  inv1   g066(.a(x39), .O(new_n129_));
  oai21  g067(.a(x32), .b(x30), .c(x40), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x05), .c(new_n72_), .O(new_n132_));
  oai21  g070(.a(new_n128_), .b(new_n95_), .c(new_n132_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  inv1   g072(.a(x08), .O(new_n135_));
  nand2  g073(.a(x40), .b(x04), .O(new_n136_));
  inv1   g074(.a(x35), .O(new_n137_));
  aoi21  g075(.a(x39), .b(new_n137_), .c(x28), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n136_), .c(new_n102_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand2  g078(.a(x40), .b(x29), .O(new_n141_));
  nand2  g079(.a(x27), .b(x04), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n138_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand2  g082(.a(x40), .b(new_n137_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n142_), .c(new_n81_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n129_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n144_), .c(new_n140_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n79_), .c(new_n134_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n64_), .O(z11));
  nor3   g088(.a(x37), .b(x36), .c(x35), .O(new_n151_));
  nor2   g089(.a(new_n74_), .b(x27), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n151_), .c(new_n73_), .O(new_n153_));
  nand2  g091(.a(x39), .b(new_n81_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n145_), .c(new_n72_), .O(new_n155_));
  and2   g093(.a(new_n122_), .b(new_n67_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n155_), .c(new_n66_), .O(new_n157_));
  aoi21  g095(.a(x40), .b(x27), .c(x39), .O(new_n158_));
  nand2  g096(.a(new_n136_), .b(new_n92_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n95_), .c(new_n158_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n157_), .c(new_n153_), .O(z12));
  inv1   g099(.a(x13), .O(new_n162_));
  nand2  g100(.a(x36), .b(x35), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x28), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x40), .c(new_n162_), .d(new_n72_), .O(new_n165_));
  inv1   g103(.a(x18), .O(new_n166_));
  inv1   g104(.a(x19), .O(new_n167_));
  nand4  g105(.a(new_n68_), .b(x20), .c(new_n167_), .d(new_n166_), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n165_), .c(new_n129_), .O(new_n169_));
  nand4  g107(.a(new_n68_), .b(x40), .c(x20), .d(new_n167_), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(x18), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n169_), .c(x27), .O(new_n172_));
  nand3  g110(.a(new_n131_), .b(new_n162_), .c(new_n72_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n172_), .O(z13));
  inv1   g112(.a(new_n92_), .O(new_n175_));
  nand3  g113(.a(x20), .b(new_n167_), .c(new_n166_), .O(new_n176_));
  nand4  g114(.a(new_n163_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n177_));
  nand3  g115(.a(x39), .b(new_n162_), .c(new_n72_), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n177_), .c(new_n74_), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n175_), .c(new_n176_), .O(new_n181_));
  nand4  g119(.a(x40), .b(new_n66_), .c(new_n137_), .d(x28), .O(new_n182_));
  oai21  g120(.a(new_n129_), .b(x27), .c(new_n182_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n80_), .c(new_n79_), .O(new_n184_));
  oai21  g122(.a(new_n129_), .b(x13), .c(x40), .O(new_n185_));
  aoi22  g123(.a(new_n185_), .b(new_n92_), .c(x35), .d(x28), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n155_), .c(new_n66_), .O(new_n187_));
  oai21  g125(.a(new_n179_), .b(new_n74_), .c(new_n92_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n95_), .O(new_n189_));
  nand4  g127(.a(new_n189_), .b(new_n187_), .c(new_n184_), .d(new_n181_), .O(z14));
  inv1   g128(.a(x12), .O(new_n191_));
  nor2   g129(.a(new_n119_), .b(new_n191_), .O(z15));
  inv1   g130(.a(x23), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(x22), .c(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n64_), .O(z16));
  inv1   g133(.a(x24), .O(new_n196_));
  nand4  g134(.a(new_n196_), .b(x23), .c(x22), .d(x01), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n64_), .O(z17));
  inv1   g136(.a(x29), .O(new_n199_));
  oai22  g137(.a(new_n116_), .b(new_n199_), .c(new_n82_), .d(x27), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(x08), .O(new_n201_));
  nor2   g139(.a(new_n95_), .b(x04), .O(new_n202_));
  aoi21  g140(.a(new_n202_), .b(new_n83_), .c(x09), .O(new_n203_));
  nand4  g141(.a(new_n203_), .b(new_n201_), .c(new_n64_), .d(new_n79_), .O(z18));
  nand2  g142(.a(x40), .b(new_n129_), .O(new_n205_));
  nand2  g143(.a(x28), .b(new_n95_), .O(new_n206_));
  nand2  g144(.a(new_n141_), .b(x39), .O(new_n207_));
  aoi22  g145(.a(new_n207_), .b(new_n205_), .c(new_n206_), .d(x35), .O(new_n208_));
  nand2  g146(.a(x39), .b(new_n137_), .O(new_n209_));
  aoi21  g147(.a(new_n209_), .b(new_n102_), .c(x08), .O(new_n210_));
  oai21  g148(.a(new_n210_), .b(new_n208_), .c(new_n79_), .O(new_n211_));
  nor2   g149(.a(new_n211_), .b(x09), .O(z20));
  nand2  g150(.a(new_n149_), .b(new_n64_), .O(z19));
endmodule


