// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n187_, new_n188_, new_n190_, new_n191_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x03), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x27), .c(new_n66_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n64_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n66_), .d(new_n73_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n72_), .O(z00));
  inv1   g016(.a(new_n65_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  aoi22  g028(.a(new_n90_), .b(new_n89_), .c(new_n84_), .d(x04), .O(new_n91_));
  inv1   g029(.a(x08), .O(new_n92_));
  inv1   g030(.a(x27), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(x04), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n91_), .c(x40), .d(x39), .O(z02));
  nand2  g033(.a(new_n68_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n67_), .b(new_n93_), .c(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n96_), .c(new_n79_), .O(z03));
  aoi21  g037(.a(new_n67_), .b(new_n93_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n96_), .c(new_n79_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n79_), .O(z05));
  oai22  g041(.a(x40), .b(x03), .c(new_n83_), .d(new_n93_), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(x37), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n65_), .O(z07));
  inv1   g054(.a(x39), .O(new_n117_));
  nor2   g055(.a(new_n64_), .b(new_n117_), .O(z08));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(x40), .b(new_n117_), .O(new_n123_));
  nand2  g061(.a(new_n64_), .b(x03), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x05), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n125_), .c(new_n85_), .O(new_n128_));
  nand3  g066(.a(new_n65_), .b(x37), .c(x06), .O(new_n129_));
  oai21  g067(.a(new_n128_), .b(x04), .c(new_n129_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x27), .O(new_n131_));
  oai21  g069(.a(x32), .b(x30), .c(x40), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n117_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x05), .c(new_n73_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(z10));
  nand3  g073(.a(x35), .b(new_n83_), .c(new_n73_), .O(new_n136_));
  aoi22  g074(.a(new_n136_), .b(x40), .c(new_n65_), .d(new_n93_), .O(new_n137_));
  nor2   g075(.a(new_n137_), .b(x08), .O(new_n138_));
  inv1   g076(.a(new_n84_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand2  g078(.a(x39), .b(x29), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x40), .O(new_n142_));
  aoi22  g080(.a(new_n142_), .b(new_n124_), .c(new_n140_), .d(new_n139_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n138_), .c(new_n80_), .O(new_n144_));
  nor2   g082(.a(new_n144_), .b(x09), .O(z11));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  nand2  g084(.a(x40), .b(new_n93_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n146_), .c(new_n74_), .O(new_n149_));
  aoi22  g087(.a(x39), .b(new_n73_), .c(x35), .d(x28), .O(new_n150_));
  nand2  g088(.a(new_n83_), .b(x03), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x35), .c(x40), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(new_n67_), .O(new_n153_));
  aoi21  g091(.a(x27), .b(x03), .c(x40), .O(new_n154_));
  oai21  g092(.a(new_n64_), .b(new_n73_), .c(x39), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n93_), .c(new_n154_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n153_), .c(new_n149_), .O(z12));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n69_), .b(new_n65_), .c(x20), .d(new_n158_), .O(new_n159_));
  inv1   g097(.a(x13), .O(new_n160_));
  nand2  g098(.a(x36), .b(x35), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x28), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x40), .c(x39), .d(new_n160_), .O(new_n163_));
  oai22  g101(.a(new_n163_), .b(x04), .c(new_n159_), .d(x18), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x27), .O(new_n165_));
  nand3  g103(.a(new_n133_), .b(new_n160_), .c(new_n73_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n165_), .O(z13));
  nand2  g105(.a(x20), .b(new_n158_), .O(new_n168_));
  nand4  g106(.a(new_n161_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(z08), .c(new_n160_), .d(new_n73_), .O(new_n170_));
  oai21  g108(.a(new_n168_), .b(x18), .c(new_n170_), .O(new_n171_));
  nor2   g109(.a(x37), .b(x35), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n147_), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n81_), .c(new_n80_), .O(new_n175_));
  nand2  g113(.a(new_n147_), .b(new_n69_), .O(new_n176_));
  oai21  g114(.a(x13), .b(x04), .c(new_n176_), .O(new_n177_));
  inv1   g115(.a(z08), .O(new_n178_));
  oai21  g116(.a(new_n172_), .b(new_n93_), .c(new_n178_), .O(new_n179_));
  nand2  g117(.a(new_n124_), .b(x39), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n67_), .c(new_n83_), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n179_), .c(new_n65_), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(new_n177_), .c(new_n175_), .d(new_n171_), .O(z14));
  inv1   g122(.a(x12), .O(new_n185_));
  nor2   g123(.a(new_n120_), .b(new_n185_), .O(z15));
  inv1   g124(.a(x23), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(x22), .c(x01), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n65_), .O(z16));
  inv1   g127(.a(x24), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(x23), .c(x22), .d(x01), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n65_), .O(z17));
  inv1   g130(.a(x35), .O(new_n193_));
  nand2  g131(.a(x27), .b(new_n73_), .O(new_n194_));
  nand2  g132(.a(new_n93_), .b(x08), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n194_), .c(new_n193_), .O(new_n196_));
  inv1   g134(.a(x09), .O(new_n197_));
  nand2  g135(.a(new_n80_), .b(new_n197_), .O(new_n198_));
  aoi21  g136(.a(new_n196_), .b(new_n83_), .c(new_n198_), .O(new_n199_));
  oai22  g137(.a(new_n199_), .b(new_n79_), .c(new_n178_), .d(new_n90_), .O(z18));
  oai21  g138(.a(new_n144_), .b(x09), .c(new_n65_), .O(z19));
  oai22  g139(.a(x40), .b(x35), .c(x27), .d(x08), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(x03), .O(new_n203_));
  aoi21  g141(.a(x35), .b(x27), .c(x08), .O(new_n204_));
  aoi21  g142(.a(x39), .b(x29), .c(x35), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n204_), .c(x40), .O(new_n206_));
  nand2  g144(.a(z08), .b(x29), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(x28), .c(new_n93_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n206_), .c(new_n203_), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(new_n80_), .c(new_n197_), .O(new_n210_));
  nand2  g148(.a(new_n210_), .b(new_n65_), .O(z20));
endmodule


