// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n181_, new_n183_, new_n184_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_;
  inv1   g000(.a(x14), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n67_), .c(new_n68_), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n75_), .c(new_n64_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n67_), .d(new_n74_), .O(new_n78_));
  oai21  g016(.a(new_n73_), .b(new_n66_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n65_), .c(x04), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  aoi21  g027(.a(new_n84_), .b(x04), .c(new_n89_), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  nand3  g029(.a(new_n69_), .b(new_n91_), .c(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(x40), .d(x39), .O(z02));
  nand2  g031(.a(new_n71_), .b(x27), .O(new_n94_));
  nand3  g032(.a(new_n65_), .b(new_n70_), .c(new_n69_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n65_), .d(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand4  g035(.a(new_n95_), .b(new_n94_), .c(new_n65_), .d(new_n97_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n65_), .c(new_n70_), .O(z05));
  oai22  g038(.a(x40), .b(x14), .c(new_n83_), .d(new_n69_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(x37), .O(z06));
  inv1   g040(.a(x03), .O(new_n103_));
  inv1   g041(.a(x31), .O(new_n104_));
  inv1   g042(.a(x33), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  aoi22  g045(.a(new_n107_), .b(x38), .c(x17), .d(new_n67_), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(new_n105_), .c(new_n104_), .d(x14), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n106_), .c(new_n110_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(x40), .c(new_n63_), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(new_n109_), .c(new_n103_), .O(z07));
  nand2  g051(.a(x40), .b(x39), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z08));
  inv1   g053(.a(x11), .O(new_n116_));
  nand4  g054(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  nand2  g056(.a(new_n114_), .b(x07), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n85_), .c(x27), .O(new_n122_));
  nand2  g060(.a(new_n81_), .b(new_n80_), .O(new_n123_));
  nand4  g061(.a(new_n123_), .b(x40), .c(x39), .d(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n74_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n65_), .O(z10));
  nand2  g066(.a(new_n65_), .b(new_n69_), .O(new_n129_));
  nand3  g067(.a(x35), .b(new_n83_), .c(new_n74_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x40), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n129_), .c(x08), .O(new_n132_));
  inv1   g070(.a(x35), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(x28), .O(new_n134_));
  nand2  g072(.a(x39), .b(x29), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x40), .O(new_n136_));
  nand2  g074(.a(new_n64_), .b(x14), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  aoi22  g076(.a(new_n138_), .b(new_n134_), .c(new_n137_), .d(new_n136_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n132_), .c(new_n80_), .O(new_n140_));
  nor2   g078(.a(new_n140_), .b(x09), .O(z11));
  nand3  g079(.a(new_n70_), .b(new_n82_), .c(new_n133_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  aoi21  g081(.a(x39), .b(new_n74_), .c(new_n72_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x40), .O(new_n145_));
  nor2   g083(.a(new_n72_), .b(x40), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x14), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x28), .c(new_n64_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n149_), .d(new_n74_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand2  g092(.a(new_n71_), .b(new_n70_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n152_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand3  g096(.a(new_n123_), .b(x40), .c(x39), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(x13), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n74_), .c(new_n66_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(z13));
  inv1   g100(.a(x39), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x04), .c(x40), .O(new_n164_));
  aoi21  g102(.a(new_n64_), .b(x14), .c(x13), .O(new_n165_));
  aoi22  g103(.a(new_n165_), .b(new_n164_), .c(x35), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n133_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(new_n70_), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n170_));
  nand4  g108(.a(new_n150_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  nand4  g110(.a(x40), .b(x39), .c(new_n149_), .d(new_n74_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n170_), .O(new_n174_));
  aoi21  g112(.a(x27), .b(x14), .c(x40), .O(new_n175_));
  nand3  g113(.a(x39), .b(new_n149_), .c(new_n74_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x40), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n123_), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n69_), .c(new_n175_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n174_), .c(new_n169_), .O(z14));
  inv1   g118(.a(x12), .O(new_n181_));
  nor2   g119(.a(new_n117_), .b(new_n181_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(x22), .c(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n65_), .O(z16));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(x23), .c(x22), .d(x01), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n65_), .O(z17));
  nand2  g126(.a(z08), .b(x29), .O(new_n189_));
  oai21  g127(.a(new_n84_), .b(x27), .c(new_n189_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(x08), .O(new_n191_));
  nor2   g129(.a(new_n69_), .b(x04), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n134_), .c(x09), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(new_n191_), .c(new_n65_), .d(new_n80_), .O(z18));
  inv1   g132(.a(x09), .O(new_n195_));
  oai22  g133(.a(x40), .b(x35), .c(x27), .d(x08), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(x14), .O(new_n197_));
  aoi21  g135(.a(x35), .b(x27), .c(x08), .O(new_n198_));
  aoi21  g136(.a(x39), .b(x29), .c(x35), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n198_), .c(x40), .O(new_n200_));
  nand3  g138(.a(new_n189_), .b(x28), .c(new_n69_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n200_), .c(new_n197_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n80_), .c(new_n195_), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n65_), .O(z20));
  nor2   g142(.a(new_n140_), .b(x09), .O(z19));
endmodule


