// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  nor2   g002(.a(x40), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  inv1   g005(.a(x15), .O(new_n68_));
  oai21  g006(.a(x36), .b(x35), .c(x39), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x04), .O(new_n70_));
  inv1   g008(.a(x37), .O(new_n71_));
  nand2  g009(.a(x35), .b(x28), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(new_n70_), .c(x27), .O(new_n74_));
  inv1   g012(.a(x04), .O(new_n75_));
  nand3  g013(.a(x39), .b(x10), .c(new_n75_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x40), .c(new_n68_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n67_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n66_), .c(x04), .O(z01));
  inv1   g026(.a(x08), .O(new_n89_));
  inv1   g027(.a(x29), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g029(.a(x39), .O(new_n92_));
  aoi21  g030(.a(new_n84_), .b(x04), .c(new_n92_), .O(new_n93_));
  oai21  g031(.a(new_n91_), .b(x02), .c(new_n93_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(new_n95_));
  inv1   g033(.a(x40), .O(new_n96_));
  aoi21  g034(.a(new_n89_), .b(x04), .c(new_n96_), .O(new_n97_));
  oai21  g035(.a(new_n97_), .b(x27), .c(new_n95_), .O(z02));
  nor2   g036(.a(x40), .b(new_n64_), .O(new_n99_));
  nand3  g037(.a(new_n72_), .b(x40), .c(x27), .O(new_n100_));
  inv1   g038(.a(new_n100_), .O(new_n101_));
  aoi21  g039(.a(new_n71_), .b(new_n64_), .c(new_n101_), .O(new_n102_));
  oai21  g040(.a(new_n99_), .b(x21), .c(new_n102_), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  oai21  g042(.a(new_n99_), .b(new_n104_), .c(new_n102_), .O(z04));
  nand2  g043(.a(x40), .b(new_n83_), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(x27), .c(x37), .O(z06));
  inv1   g045(.a(z06), .O(z05));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n68_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n66_), .O(z07));
  nand2  g057(.a(x40), .b(x39), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  nand3  g059(.a(x34), .b(x26), .c(x11), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(x40), .c(new_n64_), .O(z09));
  inv1   g061(.a(x07), .O(new_n124_));
  nand2  g062(.a(x39), .b(x05), .O(new_n125_));
  oai21  g063(.a(x39), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n85_), .c(x27), .O(new_n127_));
  nand2  g065(.a(new_n81_), .b(new_n80_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x40), .c(x39), .d(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n75_), .O(new_n131_));
  aoi21  g069(.a(x37), .b(x06), .c(new_n96_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n64_), .c(new_n131_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(x39), .b(x29), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x35), .c(new_n83_), .O(new_n137_));
  nand3  g075(.a(x35), .b(new_n83_), .c(new_n75_), .O(new_n138_));
  aoi22  g076(.a(new_n138_), .b(new_n89_), .c(new_n137_), .d(new_n135_), .O(new_n139_));
  aoi21  g077(.a(x35), .b(new_n83_), .c(x40), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n89_), .c(new_n64_), .O(new_n141_));
  oai21  g079(.a(new_n139_), .b(new_n96_), .c(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n80_), .c(new_n134_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n66_), .O(z11));
  inv1   g082(.a(x35), .O(new_n145_));
  nand3  g083(.a(new_n71_), .b(new_n82_), .c(new_n145_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x27), .c(x10), .O(new_n147_));
  nand2  g085(.a(x39), .b(new_n75_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n72_), .c(new_n71_), .O(new_n149_));
  oai21  g087(.a(x27), .b(new_n75_), .c(new_n149_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n147_), .c(x40), .O(new_n151_));
  oai21  g089(.a(z08), .b(x27), .c(new_n151_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x28), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n75_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n73_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  aoi21  g099(.a(new_n81_), .b(new_n80_), .c(new_n92_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n153_), .c(new_n75_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n161_), .c(new_n96_), .O(z13));
  nand2  g102(.a(new_n148_), .b(x40), .O(new_n165_));
  aoi22  g103(.a(new_n165_), .b(new_n153_), .c(x35), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n145_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(new_n71_), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n170_));
  nand4  g108(.a(new_n154_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n171_));
  oai21  g109(.a(x13), .b(x04), .c(x40), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n171_), .c(x39), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n170_), .O(new_n174_));
  nand3  g112(.a(new_n172_), .b(new_n162_), .c(x40), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n64_), .c(new_n65_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n174_), .c(new_n169_), .O(z14));
  nand4  g115(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n96_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(x22), .c(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n66_), .O(z16));
  inv1   g120(.a(x01), .O(new_n183_));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n66_), .b(new_n184_), .c(x23), .d(x22), .O(new_n185_));
  nor2   g123(.a(new_n185_), .b(new_n183_), .O(z17));
  oai22  g124(.a(new_n120_), .b(new_n90_), .c(new_n84_), .d(x27), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(x08), .O(new_n188_));
  nand2  g126(.a(new_n138_), .b(x40), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x27), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n188_), .c(new_n80_), .d(new_n134_), .O(z18));
  inv1   g129(.a(new_n143_), .O(z19));
  oai21  g130(.a(new_n96_), .b(x35), .c(x27), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n89_), .O(new_n194_));
  oai22  g132(.a(new_n96_), .b(x35), .c(new_n83_), .d(x27), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n135_), .O(new_n196_));
  nand2  g134(.a(new_n140_), .b(new_n64_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n196_), .c(new_n194_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n80_), .c(new_n134_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n66_), .O(z20));
endmodule


