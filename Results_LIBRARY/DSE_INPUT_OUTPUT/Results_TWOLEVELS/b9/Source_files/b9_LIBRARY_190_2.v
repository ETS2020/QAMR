// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x28), .O(new_n70_));
  nor2   g008(.a(new_n66_), .b(new_n70_), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x37), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n69_), .c(new_n64_), .O(new_n73_));
  and2   g011(.a(x40), .b(x39), .O(z08));
  nand3  g012(.a(z08), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n73_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x40), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x37), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nor2   g020(.a(new_n66_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n79_), .c(x04), .O(z01));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  inv1   g025(.a(new_n87_), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(x02), .O(new_n89_));
  nand2  g027(.a(x35), .b(new_n70_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(x04), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x39), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n89_), .c(x40), .O(new_n94_));
  nor2   g032(.a(x40), .b(x37), .O(new_n95_));
  inv1   g033(.a(new_n95_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z02));
  inv1   g035(.a(new_n71_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x37), .O(new_n100_));
  nor2   g038(.a(x40), .b(new_n100_), .O(new_n101_));
  aoi21  g039(.a(new_n100_), .b(new_n64_), .c(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n99_), .c(x21), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  nand3  g042(.a(new_n102_), .b(new_n99_), .c(new_n104_), .O(z04));
  aoi21  g043(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g044(.a(z06), .O(z05));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n63_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n79_), .O(z07));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  nor2   g059(.a(new_n83_), .b(x36), .O(new_n122_));
  nor2   g060(.a(new_n78_), .b(x39), .O(new_n123_));
  oai21  g061(.a(new_n123_), .b(new_n95_), .c(x07), .O(new_n124_));
  nand2  g062(.a(z08), .b(x05), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n122_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  nand2  g065(.a(new_n82_), .b(new_n81_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(x40), .c(x39), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  aoi21  g068(.a(new_n126_), .b(x27), .c(new_n130_), .O(new_n131_));
  nand4  g069(.a(x40), .b(x37), .c(x27), .d(x06), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(x04), .c(new_n132_), .O(z10));
  nand2  g071(.a(new_n79_), .b(new_n64_), .O(new_n134_));
  oai21  g072(.a(new_n90_), .b(x04), .c(x40), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n134_), .c(x08), .O(new_n136_));
  nand2  g074(.a(x39), .b(x29), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x40), .O(new_n138_));
  nand2  g076(.a(x27), .b(x04), .O(new_n139_));
  aoi22  g077(.a(new_n139_), .b(new_n83_), .c(new_n138_), .d(new_n96_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n136_), .c(new_n81_), .O(new_n141_));
  nor2   g079(.a(new_n141_), .b(x09), .O(z11));
  inv1   g080(.a(x10), .O(new_n143_));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  nor2   g082(.a(new_n78_), .b(x27), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n143_), .O(new_n146_));
  aoi21  g084(.a(x39), .b(new_n65_), .c(new_n71_), .O(new_n147_));
  nand3  g085(.a(new_n78_), .b(x35), .c(new_n70_), .O(new_n148_));
  nor3   g086(.a(new_n148_), .b(new_n64_), .c(x04), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n147_), .c(new_n100_), .O(new_n150_));
  nand3  g088(.a(x40), .b(x39), .c(new_n65_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n64_), .O(new_n152_));
  oai21  g090(.a(x37), .b(new_n66_), .c(new_n78_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n152_), .c(new_n150_), .d(new_n146_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x28), .c(new_n78_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n65_), .O(new_n158_));
  inv1   g096(.a(x18), .O(new_n159_));
  inv1   g097(.a(x19), .O(new_n160_));
  nand2  g098(.a(new_n98_), .b(new_n100_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x20), .c(new_n160_), .d(new_n159_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n158_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nor2   g102(.a(new_n129_), .b(x13), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n65_), .c(new_n101_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n164_), .O(z13));
  nand3  g105(.a(x20), .b(new_n160_), .c(new_n159_), .O(new_n168_));
  nand4  g106(.a(new_n156_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n169_));
  nand2  g107(.a(new_n155_), .b(new_n65_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x40), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n169_), .c(z08), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  nand3  g111(.a(new_n100_), .b(new_n66_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n82_), .c(new_n81_), .O(new_n176_));
  oai21  g114(.a(new_n145_), .b(new_n72_), .c(new_n170_), .O(new_n177_));
  or2    g115(.a(z08), .b(x27), .O(new_n178_));
  nand4  g116(.a(new_n78_), .b(x35), .c(x27), .d(new_n65_), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(x39), .c(x28), .O(new_n180_));
  nor2   g118(.a(x39), .b(x35), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n180_), .c(new_n100_), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(new_n178_), .c(new_n177_), .d(new_n153_), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n176_), .c(new_n173_), .O(z14));
  inv1   g123(.a(x12), .O(new_n186_));
  nor2   g124(.a(new_n120_), .b(new_n186_), .O(z15));
  inv1   g125(.a(x23), .O(new_n188_));
  nand4  g126(.a(new_n79_), .b(new_n188_), .c(x22), .d(x01), .O(new_n189_));
  inv1   g127(.a(new_n189_), .O(z16));
  inv1   g128(.a(x01), .O(new_n191_));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n79_), .b(new_n192_), .c(x23), .d(x22), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(new_n191_), .O(z17));
  inv1   g132(.a(x08), .O(new_n195_));
  nand2  g133(.a(new_n78_), .b(x37), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(x27), .c(new_n65_), .O(new_n197_));
  oai21  g135(.a(new_n134_), .b(new_n195_), .c(new_n197_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(x35), .c(new_n70_), .O(new_n199_));
  inv1   g137(.a(x09), .O(new_n200_));
  nand2  g138(.a(new_n81_), .b(new_n200_), .O(new_n201_));
  aoi22  g139(.a(new_n201_), .b(new_n79_), .c(new_n88_), .d(z08), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n199_), .O(z18));
  oai21  g141(.a(new_n141_), .b(x09), .c(new_n79_), .O(z19));
  oai22  g142(.a(new_n78_), .b(x35), .c(new_n70_), .d(x27), .O(new_n205_));
  nand2  g143(.a(new_n205_), .b(new_n137_), .O(new_n206_));
  nor2   g144(.a(new_n78_), .b(x08), .O(new_n207_));
  oai21  g145(.a(new_n207_), .b(new_n95_), .c(new_n66_), .O(new_n208_));
  nor2   g146(.a(new_n101_), .b(x08), .O(new_n209_));
  nor2   g147(.a(x40), .b(new_n70_), .O(new_n210_));
  oai21  g148(.a(new_n210_), .b(new_n209_), .c(new_n64_), .O(new_n211_));
  nand3  g149(.a(new_n211_), .b(new_n208_), .c(new_n206_), .O(new_n212_));
  nand3  g150(.a(new_n212_), .b(new_n81_), .c(new_n200_), .O(new_n213_));
  nand2  g151(.a(new_n213_), .b(new_n79_), .O(z20));
endmodule


