// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x32), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x15), .O(new_n71_));
  inv1   g009(.a(x32), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n70_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n72_), .c(new_n79_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n82_), .b(x04), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n89_), .c(new_n74_), .O(new_n91_));
  inv1   g029(.a(x08), .O(new_n92_));
  inv1   g030(.a(x27), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(x04), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x39), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n91_), .c(new_n72_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x40), .O(z02));
  nand2  g035(.a(new_n66_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n65_), .b(new_n93_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n64_), .d(x21), .O(z03));
  inv1   g038(.a(new_n64_), .O(new_n101_));
  aoi21  g039(.a(new_n65_), .b(new_n93_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n98_), .c(new_n101_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n65_), .c(new_n101_), .O(z05));
  nand3  g043(.a(new_n104_), .b(new_n64_), .c(new_n65_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n71_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n64_), .O(z07));
  nand3  g056(.a(x40), .b(x39), .c(new_n72_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n64_), .O(z09));
  inv1   g060(.a(x04), .O(new_n123_));
  inv1   g061(.a(x05), .O(new_n124_));
  nand2  g062(.a(x40), .b(x39), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x07), .O(new_n126_));
  oai21  g064(.a(new_n125_), .b(new_n124_), .c(new_n126_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n83_), .c(x27), .O(new_n128_));
  nand4  g066(.a(x40), .b(x39), .c(x30), .d(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n123_), .O(new_n131_));
  nand3  g069(.a(x37), .b(x27), .c(x06), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n64_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(x40), .b(x04), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x35), .c(new_n81_), .O(new_n136_));
  aoi22  g074(.a(new_n136_), .b(new_n72_), .c(new_n64_), .d(new_n93_), .O(new_n137_));
  aoi21  g075(.a(x39), .b(x29), .c(x32), .O(new_n138_));
  nor2   g076(.a(new_n93_), .b(new_n123_), .O(new_n139_));
  oai22  g077(.a(new_n139_), .b(new_n82_), .c(new_n138_), .d(new_n74_), .O(new_n140_));
  oai21  g078(.a(new_n137_), .b(x08), .c(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n79_), .c(new_n134_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n64_), .O(z11));
  inv1   g081(.a(x35), .O(new_n144_));
  nand3  g082(.a(new_n65_), .b(new_n80_), .c(new_n144_), .O(new_n145_));
  nand2  g083(.a(x40), .b(new_n93_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n145_), .c(x10), .O(new_n147_));
  nand2  g085(.a(new_n135_), .b(x39), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n93_), .O(new_n149_));
  nand2  g087(.a(x39), .b(new_n123_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n66_), .c(new_n65_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n147_), .c(new_n72_), .O(new_n153_));
  nand2  g091(.a(new_n68_), .b(new_n74_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n74_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n123_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n67_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n159_), .c(x32), .O(new_n163_));
  nand4  g101(.a(new_n67_), .b(new_n74_), .c(x20), .d(new_n161_), .O(new_n164_));
  nor2   g102(.a(new_n164_), .b(x18), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n163_), .c(x27), .O(new_n166_));
  nor2   g104(.a(new_n125_), .b(x32), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(x30), .c(new_n156_), .d(new_n123_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n166_), .O(z13));
  nand3  g107(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n170_));
  nand3  g108(.a(new_n157_), .b(new_n79_), .c(x28), .O(new_n171_));
  oai21  g109(.a(x13), .b(x04), .c(x40), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n171_), .c(x39), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n72_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x40), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n170_), .O(new_n176_));
  nand3  g114(.a(new_n65_), .b(new_n144_), .c(x28), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(x27), .c(x30), .O(new_n178_));
  nand3  g116(.a(x39), .b(new_n156_), .c(new_n123_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n66_), .c(new_n65_), .O(new_n180_));
  oai21  g118(.a(new_n148_), .b(x13), .c(new_n93_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n178_), .c(new_n72_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n176_), .c(new_n154_), .O(z14));
  nand4  g122(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n64_), .O(z15));
  inv1   g124(.a(x23), .O(new_n187_));
  nand4  g125(.a(new_n64_), .b(new_n187_), .c(x22), .d(x01), .O(new_n188_));
  inv1   g126(.a(new_n188_), .O(z16));
  inv1   g127(.a(x01), .O(new_n190_));
  inv1   g128(.a(x24), .O(new_n191_));
  nand4  g129(.a(new_n64_), .b(new_n191_), .c(x23), .d(x22), .O(new_n192_));
  nor2   g130(.a(new_n192_), .b(new_n190_), .O(z17));
  inv1   g131(.a(new_n167_), .O(new_n194_));
  nand2  g132(.a(x27), .b(new_n123_), .O(new_n195_));
  nand2  g133(.a(new_n93_), .b(x08), .O(new_n196_));
  aoi21  g134(.a(new_n196_), .b(new_n195_), .c(new_n144_), .O(new_n197_));
  nand2  g135(.a(new_n79_), .b(new_n134_), .O(new_n198_));
  aoi21  g136(.a(new_n197_), .b(new_n81_), .c(new_n198_), .O(new_n199_));
  oai22  g137(.a(new_n199_), .b(new_n101_), .c(new_n194_), .d(new_n88_), .O(z18));
  inv1   g138(.a(new_n142_), .O(z19));
  nand2  g139(.a(x39), .b(x29), .O(new_n202_));
  nor2   g140(.a(new_n81_), .b(x27), .O(new_n203_));
  nor2   g141(.a(x35), .b(x32), .O(new_n204_));
  oai21  g142(.a(new_n204_), .b(new_n203_), .c(new_n202_), .O(new_n205_));
  nor2   g143(.a(new_n101_), .b(x27), .O(new_n206_));
  oai21  g144(.a(new_n204_), .b(new_n206_), .c(new_n92_), .O(new_n207_));
  oai21  g145(.a(new_n203_), .b(new_n144_), .c(new_n74_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n207_), .c(new_n205_), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(new_n79_), .c(new_n134_), .O(new_n210_));
  nand2  g148(.a(new_n210_), .b(new_n64_), .O(z20));
endmodule


