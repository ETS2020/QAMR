// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n189_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  inv1   g017(.a(x39), .O(new_n80_));
  inv1   g018(.a(x40), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n79_), .c(x16), .O(z00));
  nor2   g021(.a(x40), .b(x39), .O(new_n84_));
  inv1   g022(.a(x30), .O(new_n85_));
  inv1   g023(.a(x32), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  inv1   g026(.a(new_n88_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(x36), .c(x27), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n86_), .c(new_n85_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(new_n84_), .O(z01));
  aoi21  g030(.a(x29), .b(x08), .c(x02), .O(new_n93_));
  nor2   g031(.a(x27), .b(x08), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n88_), .c(x04), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x39), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n93_), .c(x40), .O(new_n97_));
  nand2  g035(.a(new_n81_), .b(x39), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n97_), .O(z02));
  nand2  g037(.a(new_n71_), .b(x27), .O(new_n100_));
  nand2  g038(.a(new_n70_), .b(new_n64_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n100_), .c(new_n82_), .d(x21), .O(z03));
  aoi21  g040(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n100_), .c(new_n84_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n84_), .O(z05));
  nor2   g044(.a(new_n84_), .b(x27), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n87_), .c(new_n70_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n82_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n63_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n82_), .c(x03), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z07));
  nand2  g058(.a(new_n82_), .b(new_n75_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n82_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  aoi21  g062(.a(x35), .b(new_n87_), .c(x36), .O(new_n125_));
  xor2a  g063(.a(x40), .b(x39), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x07), .O(new_n127_));
  nand2  g065(.a(new_n76_), .b(x05), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n125_), .O(new_n129_));
  inv1   g067(.a(x06), .O(new_n130_));
  nor3   g068(.a(new_n84_), .b(new_n70_), .c(new_n130_), .O(new_n131_));
  aoi21  g069(.a(new_n129_), .b(new_n65_), .c(new_n131_), .O(new_n132_));
  oai21  g070(.a(x32), .b(x30), .c(x40), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n80_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x05), .c(new_n65_), .O(new_n135_));
  oai21  g073(.a(new_n132_), .b(new_n64_), .c(new_n135_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  inv1   g075(.a(x08), .O(new_n138_));
  oai21  g076(.a(x28), .b(x04), .c(x40), .O(new_n139_));
  nand2  g077(.a(x39), .b(new_n66_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n107_), .c(new_n138_), .O(new_n142_));
  nand2  g080(.a(x40), .b(x29), .O(new_n143_));
  oai21  g081(.a(new_n64_), .b(new_n65_), .c(new_n140_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  oai22  g083(.a(x39), .b(x35), .c(x29), .d(new_n87_), .O(new_n146_));
  nand2  g084(.a(new_n75_), .b(x28), .O(new_n147_));
  nand3  g085(.a(new_n80_), .b(x27), .c(x04), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  aoi21  g087(.a(new_n146_), .b(x40), .c(new_n149_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n145_), .c(new_n142_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n85_), .c(new_n137_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n82_), .O(z11));
  nand3  g091(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x27), .c(x10), .O(new_n155_));
  aoi22  g093(.a(new_n72_), .b(x27), .c(x39), .d(new_n65_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n155_), .c(x40), .O(new_n157_));
  nand2  g095(.a(new_n72_), .b(x27), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n81_), .c(x39), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n157_), .O(z12));
  inv1   g098(.a(x13), .O(new_n161_));
  nand2  g099(.a(x36), .b(x35), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x28), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x40), .c(new_n161_), .d(new_n65_), .O(new_n164_));
  inv1   g102(.a(x18), .O(new_n165_));
  inv1   g103(.a(x19), .O(new_n166_));
  nand4  g104(.a(new_n72_), .b(x20), .c(new_n166_), .d(new_n165_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n164_), .c(new_n80_), .O(new_n168_));
  nand4  g106(.a(new_n72_), .b(x40), .c(x20), .d(new_n166_), .O(new_n169_));
  nor2   g107(.a(new_n169_), .b(x18), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n168_), .c(x27), .O(new_n171_));
  nand3  g109(.a(new_n134_), .b(new_n161_), .c(new_n65_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(z13));
  nand3  g111(.a(x20), .b(new_n166_), .c(new_n165_), .O(new_n174_));
  nand4  g112(.a(new_n162_), .b(new_n86_), .c(new_n85_), .d(x28), .O(new_n175_));
  nor2   g113(.a(x13), .b(x04), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n175_), .c(new_n76_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n174_), .O(new_n178_));
  nand3  g116(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n179_));
  oai21  g117(.a(new_n80_), .b(x27), .c(new_n179_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n86_), .c(new_n85_), .O(new_n181_));
  oai21  g119(.a(new_n80_), .b(x04), .c(x40), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n98_), .c(new_n161_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n71_), .c(new_n70_), .O(new_n184_));
  aoi21  g122(.a(x40), .b(x27), .c(x39), .O(new_n185_));
  oai21  g123(.a(new_n176_), .b(new_n81_), .c(new_n98_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n64_), .c(new_n185_), .O(new_n187_));
  nand4  g125(.a(new_n187_), .b(new_n184_), .c(new_n181_), .d(new_n178_), .O(z14));
  inv1   g126(.a(x12), .O(new_n189_));
  nor2   g127(.a(new_n123_), .b(new_n189_), .O(z15));
  inv1   g128(.a(x23), .O(new_n191_));
  nand4  g129(.a(new_n82_), .b(new_n191_), .c(x22), .d(x01), .O(new_n192_));
  inv1   g130(.a(new_n192_), .O(z16));
  inv1   g131(.a(x01), .O(new_n194_));
  inv1   g132(.a(x24), .O(new_n195_));
  nand4  g133(.a(new_n82_), .b(new_n195_), .c(x23), .d(x22), .O(new_n196_));
  nor2   g134(.a(new_n196_), .b(new_n194_), .O(z17));
  inv1   g135(.a(x29), .O(new_n198_));
  oai22  g136(.a(new_n88_), .b(x27), .c(new_n75_), .d(new_n198_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(x08), .O(new_n200_));
  nor2   g138(.a(new_n64_), .b(x04), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n89_), .c(x09), .O(new_n202_));
  nand4  g140(.a(new_n202_), .b(new_n200_), .c(new_n82_), .d(new_n85_), .O(z18));
  nand2  g141(.a(x40), .b(new_n80_), .O(new_n204_));
  nand2  g142(.a(x28), .b(new_n64_), .O(new_n205_));
  nand2  g143(.a(new_n143_), .b(x39), .O(new_n206_));
  aoi22  g144(.a(new_n206_), .b(new_n204_), .c(new_n205_), .d(x35), .O(new_n207_));
  nand2  g145(.a(new_n82_), .b(new_n64_), .O(new_n208_));
  aoi21  g146(.a(new_n140_), .b(new_n208_), .c(x08), .O(new_n209_));
  oai21  g147(.a(new_n209_), .b(new_n207_), .c(new_n85_), .O(new_n210_));
  nor2   g148(.a(new_n210_), .b(x09), .O(z20));
  nand2  g149(.a(new_n152_), .b(new_n82_), .O(z19));
endmodule


