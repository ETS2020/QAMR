// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  nand2  g002(.a(x35), .b(x28), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g004(.a(new_n66_), .O(new_n67_));
  oai21  g005(.a(new_n63_), .b(x04), .c(new_n67_), .O(new_n68_));
  inv1   g006(.a(x10), .O(new_n69_));
  inv1   g007(.a(x39), .O(new_n70_));
  nor3   g008(.a(new_n70_), .b(new_n69_), .c(x04), .O(new_n71_));
  aoi21  g009(.a(new_n68_), .b(x27), .c(new_n71_), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  nor2   g011(.a(x40), .b(new_n70_), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  oai21  g013(.a(new_n72_), .b(x15), .c(new_n75_), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n78_), .c(new_n77_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n74_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(x40), .c(new_n86_), .O(new_n88_));
  inv1   g026(.a(x27), .O(new_n89_));
  inv1   g027(.a(x04), .O(new_n90_));
  oai21  g028(.a(x08), .b(new_n90_), .c(x39), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g030(.a(new_n81_), .b(x04), .O(new_n93_));
  oai21  g031(.a(x28), .b(new_n89_), .c(x04), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n70_), .c(new_n74_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n93_), .c(new_n92_), .d(new_n88_), .O(z02));
  nand2  g034(.a(new_n65_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n64_), .b(new_n89_), .c(new_n98_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n97_), .c(new_n74_), .O(z03));
  inv1   g038(.a(new_n74_), .O(new_n101_));
  nand2  g039(.a(new_n64_), .b(new_n89_), .O(new_n102_));
  nand4  g040(.a(new_n102_), .b(new_n97_), .c(new_n101_), .d(new_n98_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n101_), .c(new_n64_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  inv1   g045(.a(x15), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n108_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n101_), .c(x03), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z07));
  inv1   g056(.a(x40), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n70_), .O(z08));
  inv1   g058(.a(x11), .O(new_n121_));
  nand4  g059(.a(new_n101_), .b(x34), .c(x27), .d(x26), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n121_), .O(z09));
  inv1   g061(.a(x07), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x05), .O(new_n125_));
  oai21  g063(.a(x39), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n82_), .c(new_n90_), .O(new_n127_));
  nand3  g065(.a(new_n101_), .b(x37), .c(x06), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x27), .O(new_n130_));
  oai21  g068(.a(x32), .b(x30), .c(x40), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n70_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x05), .c(new_n90_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  nand2  g073(.a(x39), .b(x29), .O(new_n136_));
  oai21  g074(.a(new_n89_), .b(new_n90_), .c(new_n80_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  inv1   g076(.a(x08), .O(new_n139_));
  oai21  g077(.a(x40), .b(new_n70_), .c(new_n89_), .O(new_n140_));
  inv1   g078(.a(x35), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(x04), .c(x40), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n140_), .c(new_n80_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n139_), .O(new_n144_));
  oai21  g082(.a(new_n119_), .b(x29), .c(x39), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n141_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n144_), .c(new_n138_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n77_), .c(new_n135_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n101_), .O(z11));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  nor2   g088(.a(new_n119_), .b(x27), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n69_), .O(new_n152_));
  oai21  g090(.a(new_n119_), .b(new_n90_), .c(x39), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n89_), .O(new_n154_));
  nand2  g092(.a(x39), .b(new_n90_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n65_), .c(new_n64_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(new_n154_), .c(new_n152_), .d(new_n101_), .O(z12));
  inv1   g095(.a(x13), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x28), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x39), .c(new_n158_), .d(new_n90_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  nand4  g101(.a(new_n66_), .b(x20), .c(new_n163_), .d(new_n162_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n161_), .c(new_n119_), .O(new_n165_));
  nand4  g103(.a(new_n66_), .b(new_n70_), .c(x20), .d(new_n163_), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x18), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n165_), .c(x27), .O(new_n168_));
  nand3  g106(.a(new_n132_), .b(new_n158_), .c(new_n90_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n168_), .O(z13));
  nand3  g108(.a(x39), .b(new_n158_), .c(new_n90_), .O(new_n171_));
  nand3  g109(.a(x20), .b(new_n163_), .c(new_n162_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n67_), .c(new_n171_), .O(new_n173_));
  inv1   g111(.a(new_n151_), .O(new_n174_));
  aoi22  g112(.a(new_n172_), .b(new_n159_), .c(new_n64_), .d(new_n141_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n80_), .c(new_n174_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n78_), .c(new_n77_), .O(new_n177_));
  oai21  g115(.a(x13), .b(x04), .c(x40), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(x39), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n89_), .c(new_n74_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n177_), .c(new_n173_), .O(z14));
  inv1   g119(.a(x12), .O(new_n182_));
  nor2   g120(.a(new_n122_), .b(new_n182_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(x22), .c(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n101_), .O(z16));
  inv1   g124(.a(x01), .O(new_n187_));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n101_), .b(new_n188_), .c(x23), .d(x22), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n187_), .O(z17));
  oai21  g128(.a(new_n81_), .b(x27), .c(new_n136_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(x08), .O(new_n192_));
  inv1   g130(.a(new_n81_), .O(new_n193_));
  nor2   g131(.a(new_n89_), .b(x04), .O(new_n194_));
  aoi21  g132(.a(new_n194_), .b(new_n193_), .c(x09), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n192_), .c(new_n101_), .d(new_n77_), .O(z18));
  oai21  g134(.a(new_n80_), .b(x27), .c(x35), .O(new_n197_));
  and2   g135(.a(new_n197_), .b(new_n145_), .O(new_n198_));
  nand2  g136(.a(x40), .b(new_n141_), .O(new_n199_));
  aoi21  g137(.a(new_n199_), .b(new_n140_), .c(x08), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n198_), .c(new_n77_), .O(new_n201_));
  nor2   g139(.a(new_n201_), .b(x09), .O(z20));
  nand2  g140(.a(new_n148_), .b(new_n101_), .O(z19));
endmodule


