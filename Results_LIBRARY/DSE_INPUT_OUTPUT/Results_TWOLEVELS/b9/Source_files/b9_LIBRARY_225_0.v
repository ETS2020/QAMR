// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_;
  inv1   g000(.a(x18), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x15), .c(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x15), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n64_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n73_), .d(new_n72_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n71_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n65_), .c(x04), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n83_), .c(x04), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x39), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n88_), .c(x40), .O(new_n92_));
  nand2  g030(.a(new_n64_), .b(x18), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(z02));
  inv1   g032(.a(x35), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n82_), .c(x27), .O(new_n96_));
  inv1   g034(.a(x37), .O(new_n97_));
  nand3  g035(.a(new_n65_), .b(new_n97_), .c(new_n66_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n96_), .c(new_n65_), .d(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand4  g038(.a(new_n98_), .b(new_n96_), .c(new_n65_), .d(new_n100_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n65_), .c(new_n97_), .O(z05));
  oai22  g041(.a(x40), .b(x18), .c(new_n82_), .d(new_n66_), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(x37), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n73_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n65_), .O(z07));
  nand2  g054(.a(x40), .b(x39), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n65_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n65_), .O(z09));
  inv1   g058(.a(x07), .O(new_n121_));
  inv1   g059(.a(x39), .O(new_n122_));
  nand2  g060(.a(x40), .b(new_n122_), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n93_), .c(new_n121_), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x05), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n124_), .c(new_n84_), .O(new_n127_));
  nand3  g065(.a(new_n65_), .b(x37), .c(x06), .O(new_n128_));
  oai21  g066(.a(new_n127_), .b(x04), .c(new_n128_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x27), .O(new_n130_));
  nor2   g068(.a(x32), .b(x30), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n64_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(x39), .c(x05), .d(new_n72_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(z10));
  nand2  g072(.a(new_n65_), .b(new_n66_), .O(new_n135_));
  oai21  g073(.a(new_n83_), .b(x04), .c(x40), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n135_), .c(x08), .O(new_n137_));
  inv1   g075(.a(new_n83_), .O(new_n138_));
  inv1   g076(.a(x29), .O(new_n139_));
  oai21  g077(.a(new_n122_), .b(new_n139_), .c(x40), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  aoi22  g079(.a(new_n141_), .b(new_n138_), .c(new_n140_), .d(new_n93_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n137_), .c(new_n79_), .O(new_n143_));
  nor2   g081(.a(new_n143_), .b(x09), .O(z11));
  nand3  g082(.a(new_n97_), .b(new_n81_), .c(new_n95_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x27), .c(x10), .O(new_n146_));
  aoi21  g084(.a(x39), .b(new_n72_), .c(new_n68_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n146_), .c(x40), .O(new_n148_));
  nand3  g086(.a(new_n69_), .b(new_n64_), .c(x18), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x28), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n72_), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  inv1   g093(.a(new_n67_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x20), .c(new_n155_), .d(new_n63_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  inv1   g097(.a(new_n131_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x39), .c(new_n151_), .d(new_n72_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n159_), .c(new_n64_), .O(z13));
  oai21  g100(.a(new_n68_), .b(new_n64_), .c(new_n63_), .O(new_n163_));
  oai21  g101(.a(new_n122_), .b(x04), .c(new_n163_), .O(new_n164_));
  nand2  g102(.a(x20), .b(new_n155_), .O(new_n165_));
  nand4  g103(.a(new_n152_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n166_));
  oai21  g104(.a(x13), .b(x04), .c(x40), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n166_), .c(x39), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nor2   g107(.a(new_n64_), .b(new_n151_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n131_), .c(new_n66_), .O(new_n171_));
  oai21  g109(.a(new_n67_), .b(x18), .c(x13), .O(new_n172_));
  nand2  g110(.a(new_n152_), .b(x18), .O(new_n173_));
  oai21  g111(.a(x37), .b(x35), .c(new_n173_), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n172_), .c(new_n171_), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n169_), .c(new_n164_), .d(x40), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n65_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n65_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n65_), .O(z17));
  oai22  g124(.a(new_n117_), .b(new_n139_), .c(new_n83_), .d(x27), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(x08), .O(new_n188_));
  nor2   g126(.a(new_n66_), .b(x04), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n138_), .c(x09), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n188_), .c(new_n65_), .d(new_n79_), .O(z18));
  oai21  g129(.a(new_n143_), .b(x09), .c(new_n65_), .O(z19));
  aoi21  g130(.a(x28), .b(new_n66_), .c(new_n95_), .O(new_n193_));
  aoi21  g131(.a(new_n140_), .b(new_n93_), .c(new_n193_), .O(new_n194_));
  nand2  g132(.a(x40), .b(new_n95_), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n135_), .c(x08), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n194_), .c(new_n79_), .O(new_n197_));
  nor2   g135(.a(new_n197_), .b(x09), .O(z20));
endmodule


