// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  nand2  g016(.a(x40), .b(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x27), .c(x30), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x40), .c(new_n78_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x04), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n78_), .c(x40), .O(new_n88_));
  oai21  g026(.a(x08), .b(new_n65_), .c(x40), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n64_), .O(new_n90_));
  nand2  g028(.a(new_n82_), .b(x04), .O(new_n91_));
  aoi21  g029(.a(new_n73_), .b(x27), .c(new_n72_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n88_), .O(z02));
  inv1   g031(.a(new_n79_), .O(new_n94_));
  nand2  g032(.a(x35), .b(x28), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n64_), .c(new_n97_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n94_), .O(z03));
  nand2  g038(.a(new_n98_), .b(new_n64_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n96_), .c(new_n79_), .d(new_n97_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n79_), .c(new_n98_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n79_), .O(z07));
  nand3  g054(.a(x40), .b(x39), .c(x32), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n79_), .O(z09));
  nand3  g058(.a(new_n79_), .b(x37), .c(x06), .O(new_n121_));
  oai21  g059(.a(x39), .b(new_n78_), .c(x40), .O(new_n122_));
  nand4  g060(.a(new_n122_), .b(new_n83_), .c(x07), .d(new_n65_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x27), .O(new_n125_));
  nand4  g063(.a(new_n74_), .b(x32), .c(x05), .d(new_n65_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(z10));
  inv1   g065(.a(x09), .O(new_n128_));
  inv1   g066(.a(x30), .O(new_n129_));
  aoi21  g067(.a(x40), .b(new_n78_), .c(x27), .O(new_n130_));
  nand2  g068(.a(x40), .b(x04), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x35), .c(new_n81_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(x32), .c(new_n130_), .O(new_n133_));
  nand2  g071(.a(x39), .b(x29), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x32), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x40), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n81_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  oai21  g077(.a(new_n133_), .b(x08), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n129_), .c(new_n128_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n79_), .O(z11));
  nand3  g080(.a(new_n98_), .b(new_n67_), .c(new_n66_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  nand2  g082(.a(new_n131_), .b(x39), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n64_), .O(new_n146_));
  nand2  g084(.a(x39), .b(new_n65_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n95_), .c(new_n98_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n144_), .c(x32), .O(new_n150_));
  nor2   g088(.a(new_n70_), .b(new_n64_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(x40), .c(new_n150_), .O(z12));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand4  g092(.a(new_n151_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n155_));
  inv1   g093(.a(x13), .O(new_n156_));
  nand3  g094(.a(x39), .b(new_n156_), .c(new_n65_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x32), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x40), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n155_), .O(z13));
  oai21  g098(.a(x27), .b(new_n65_), .c(x32), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x40), .O(new_n162_));
  oai22  g100(.a(new_n122_), .b(x13), .c(new_n70_), .d(new_n64_), .O(new_n163_));
  nand2  g101(.a(new_n70_), .b(x32), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  nand3  g103(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n165_), .c(x04), .O(new_n167_));
  inv1   g105(.a(new_n74_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(x13), .c(new_n166_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n167_), .c(new_n163_), .d(new_n162_), .O(z14));
  nand4  g108(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n79_), .O(z15));
  inv1   g110(.a(x23), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(x22), .c(x01), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n79_), .O(z16));
  inv1   g113(.a(x01), .O(new_n176_));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n79_), .b(new_n177_), .c(x23), .d(x22), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n176_), .O(z17));
  nand2  g117(.a(new_n74_), .b(x29), .O(new_n180_));
  oai21  g118(.a(new_n82_), .b(x27), .c(new_n180_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(x08), .O(new_n182_));
  nor2   g120(.a(new_n66_), .b(x28), .O(new_n183_));
  nor2   g121(.a(new_n64_), .b(x04), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(x09), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n182_), .c(new_n79_), .d(new_n129_), .O(z18));
  inv1   g124(.a(new_n141_), .O(z19));
  nor2   g125(.a(new_n81_), .b(x27), .O(new_n188_));
  nor2   g126(.a(x35), .b(new_n78_), .O(new_n189_));
  oai21  g127(.a(new_n189_), .b(new_n188_), .c(new_n134_), .O(new_n190_));
  inv1   g128(.a(x08), .O(new_n191_));
  oai21  g129(.a(new_n189_), .b(new_n130_), .c(new_n191_), .O(new_n192_));
  oai21  g130(.a(new_n188_), .b(new_n66_), .c(new_n73_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n192_), .c(new_n190_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n129_), .c(new_n128_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n79_), .O(z20));
endmodule


