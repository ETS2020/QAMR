// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n198_, new_n199_, new_n200_,
    new_n201_;
  inv1   g000(.a(x14), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n67_), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n66_), .d(x14), .O(new_n78_));
  oai22  g016(.a(new_n78_), .b(x04), .c(new_n73_), .d(new_n65_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n64_), .c(x04), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  inv1   g027(.a(x04), .O(new_n90_));
  inv1   g028(.a(x35), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(x28), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n89_), .c(x14), .O(new_n94_));
  inv1   g032(.a(x08), .O(new_n95_));
  inv1   g033(.a(x27), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n95_), .c(x04), .O(new_n97_));
  inv1   g035(.a(x39), .O(new_n98_));
  nor2   g036(.a(new_n65_), .b(new_n98_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n94_), .d(x40), .O(z02));
  nand2  g038(.a(new_n69_), .b(x27), .O(new_n101_));
  inv1   g039(.a(x21), .O(new_n102_));
  aoi21  g040(.a(new_n68_), .b(new_n96_), .c(new_n102_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n101_), .c(new_n65_), .O(z03));
  nand2  g042(.a(new_n68_), .b(new_n96_), .O(new_n105_));
  nand4  g043(.a(new_n105_), .b(new_n101_), .c(new_n64_), .d(new_n102_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n68_), .c(new_n65_), .O(z05));
  nand3  g046(.a(new_n107_), .b(new_n64_), .c(new_n68_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z06));
  inv1   g048(.a(x03), .O(new_n111_));
  inv1   g049(.a(x31), .O(new_n112_));
  inv1   g050(.a(x33), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x00), .O(new_n115_));
  aoi22  g053(.a(new_n115_), .b(x38), .c(x17), .d(new_n66_), .O(new_n116_));
  nand4  g054(.a(new_n116_), .b(new_n113_), .c(new_n112_), .d(x14), .O(new_n117_));
  inv1   g055(.a(x00), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n114_), .c(new_n118_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n75_), .c(new_n63_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n117_), .c(new_n111_), .O(z07));
  nand3  g059(.a(x40), .b(x39), .c(x14), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z08));
  inv1   g061(.a(x11), .O(new_n124_));
  nand4  g062(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n124_), .O(z09));
  inv1   g064(.a(x05), .O(new_n127_));
  nand2  g065(.a(x40), .b(x39), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x07), .O(new_n129_));
  oai21  g067(.a(new_n128_), .b(new_n127_), .c(new_n129_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n85_), .c(x27), .O(new_n131_));
  oai21  g069(.a(x32), .b(x30), .c(x40), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n98_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x05), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n90_), .O(new_n136_));
  nand3  g074(.a(x37), .b(x27), .c(x06), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n64_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  inv1   g077(.a(new_n128_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x29), .O(new_n141_));
  nor2   g079(.a(new_n96_), .b(new_n90_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n84_), .c(new_n141_), .O(new_n143_));
  oai21  g081(.a(new_n63_), .b(new_n90_), .c(x27), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n84_), .c(new_n95_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n80_), .c(new_n139_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n64_), .O(z11));
  nor2   g086(.a(x27), .b(new_n63_), .O(new_n149_));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n149_), .c(new_n74_), .O(new_n151_));
  aoi21  g089(.a(x39), .b(new_n90_), .c(new_n63_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n75_), .c(new_n71_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n151_), .c(new_n64_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x28), .c(new_n75_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n90_), .O(new_n158_));
  inv1   g096(.a(x18), .O(new_n159_));
  inv1   g097(.a(x19), .O(new_n160_));
  nand4  g098(.a(new_n70_), .b(x20), .c(new_n160_), .d(new_n159_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n158_), .c(new_n63_), .O(new_n162_));
  nand4  g100(.a(new_n70_), .b(new_n75_), .c(x20), .d(new_n160_), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(x18), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n162_), .c(x27), .O(new_n165_));
  nand4  g103(.a(new_n133_), .b(x14), .c(new_n155_), .d(new_n90_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n165_), .O(z13));
  nand3  g105(.a(x20), .b(new_n160_), .c(new_n159_), .O(new_n168_));
  nand4  g106(.a(new_n156_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n169_));
  nand3  g107(.a(x39), .b(new_n155_), .c(new_n90_), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n169_), .c(new_n63_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n75_), .c(new_n168_), .O(new_n173_));
  nand3  g111(.a(new_n68_), .b(new_n91_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n81_), .c(new_n80_), .O(new_n176_));
  oai21  g114(.a(new_n171_), .b(new_n72_), .c(new_n176_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x14), .O(new_n178_));
  nand2  g116(.a(new_n71_), .b(new_n75_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n178_), .c(new_n173_), .O(z14));
  nand4  g118(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n64_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(x22), .c(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n64_), .O(z16));
  inv1   g123(.a(x01), .O(new_n186_));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n64_), .b(new_n187_), .c(x23), .d(x22), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n186_), .O(z17));
  nand2  g127(.a(x27), .b(new_n90_), .O(new_n190_));
  nand2  g128(.a(new_n96_), .b(x08), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n190_), .c(new_n91_), .O(new_n192_));
  nand2  g130(.a(new_n80_), .b(new_n139_), .O(new_n193_));
  aoi21  g131(.a(new_n192_), .b(new_n83_), .c(new_n193_), .O(new_n194_));
  inv1   g132(.a(new_n141_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(x14), .c(x08), .O(new_n196_));
  oai21  g134(.a(new_n194_), .b(new_n65_), .c(new_n196_), .O(z18));
  oai21  g135(.a(new_n91_), .b(new_n96_), .c(new_n95_), .O(new_n198_));
  aoi21  g136(.a(x28), .b(new_n96_), .c(new_n91_), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n195_), .c(new_n198_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n80_), .c(new_n139_), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n64_), .O(z20));
  nand2  g140(.a(new_n147_), .b(new_n64_), .O(z19));
endmodule


