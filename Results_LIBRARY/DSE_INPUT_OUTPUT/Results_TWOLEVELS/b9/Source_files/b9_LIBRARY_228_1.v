// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_;
  oai21  g000(.a(x36), .b(x35), .c(x40), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x40), .O(new_n67_));
  nor3   g005(.a(new_n67_), .b(new_n66_), .c(x04), .O(new_n68_));
  aoi21  g006(.a(new_n65_), .b(x27), .c(new_n68_), .O(new_n69_));
  inv1   g007(.a(x39), .O(new_n70_));
  nand2  g008(.a(x40), .b(new_n70_), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(new_n71_), .O(new_n74_));
  inv1   g012(.a(x30), .O(new_n75_));
  inv1   g013(.a(x32), .O(new_n76_));
  inv1   g014(.a(x35), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  oai21  g016(.a(new_n77_), .b(x28), .c(new_n78_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n74_), .O(z01));
  inv1   g020(.a(x02), .O(new_n83_));
  nand2  g021(.a(x29), .b(x08), .O(new_n84_));
  nor2   g022(.a(new_n77_), .b(x28), .O(new_n85_));
  oai21  g023(.a(x27), .b(x08), .c(new_n85_), .O(new_n86_));
  aoi22  g024(.a(new_n86_), .b(x04), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n70_), .c(x40), .O(z02));
  inv1   g026(.a(x21), .O(new_n89_));
  nand2  g027(.a(x35), .b(x28), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x27), .c(new_n89_), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x40), .b(new_n70_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  oai21  g033(.a(new_n91_), .b(new_n74_), .c(new_n95_), .O(z03));
  nand3  g034(.a(new_n71_), .b(new_n90_), .c(x27), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n71_), .d(new_n89_), .O(z04));
  nand2  g036(.a(x40), .b(x39), .O(new_n99_));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  aoi22  g038(.a(new_n100_), .b(new_n93_), .c(new_n99_), .d(x40), .O(z05));
  nand3  g039(.a(new_n100_), .b(new_n94_), .c(new_n93_), .O(new_n102_));
  inv1   g040(.a(new_n102_), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n71_), .c(x03), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z07));
  inv1   g053(.a(new_n99_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n71_), .O(z09));
  inv1   g056(.a(x04), .O(new_n119_));
  inv1   g057(.a(x05), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  oai22  g059(.a(new_n99_), .b(new_n120_), .c(x40), .d(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n79_), .c(new_n119_), .O(new_n123_));
  nand3  g061(.a(new_n71_), .b(x37), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x27), .O(new_n126_));
  aoi21  g064(.a(new_n76_), .b(new_n75_), .c(new_n67_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(x39), .c(x05), .d(new_n119_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(z10));
  inv1   g067(.a(x29), .O(new_n130_));
  aoi21  g068(.a(x39), .b(new_n130_), .c(new_n67_), .O(new_n131_));
  nand2  g069(.a(x27), .b(x04), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n85_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n71_), .b(new_n92_), .O(new_n134_));
  inv1   g072(.a(x28), .O(new_n135_));
  nand3  g073(.a(x35), .b(new_n135_), .c(new_n119_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x39), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n134_), .c(x08), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n133_), .c(new_n75_), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(x09), .O(z11));
  nor3   g078(.a(x37), .b(x36), .c(x35), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n92_), .c(new_n66_), .O(new_n142_));
  oai21  g080(.a(new_n70_), .b(new_n119_), .c(x40), .O(new_n143_));
  oai21  g081(.a(new_n64_), .b(new_n92_), .c(new_n143_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n142_), .c(new_n71_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x28), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x40), .c(new_n146_), .d(new_n119_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand2  g089(.a(new_n90_), .b(new_n93_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n149_), .c(new_n92_), .O(new_n154_));
  nand2  g092(.a(new_n76_), .b(new_n75_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n146_), .c(new_n119_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x39), .c(new_n67_), .O(new_n157_));
  or2    g095(.a(new_n157_), .b(new_n154_), .O(z13));
  aoi21  g096(.a(new_n146_), .b(new_n119_), .c(new_n70_), .O(new_n159_));
  nand4  g097(.a(x27), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n160_));
  oai22  g098(.a(new_n160_), .b(new_n64_), .c(new_n159_), .d(new_n67_), .O(new_n161_));
  nand3  g099(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n162_));
  nor2   g100(.a(x37), .b(x35), .O(new_n163_));
  aoi21  g101(.a(new_n162_), .b(new_n147_), .c(new_n163_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n135_), .c(x27), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(x39), .c(new_n76_), .d(new_n75_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n161_), .O(z14));
  nand4  g105(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n71_), .O(z15));
  inv1   g107(.a(x23), .O(new_n170_));
  nand4  g108(.a(new_n71_), .b(new_n170_), .c(x22), .d(x01), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z16));
  inv1   g110(.a(x24), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(x23), .c(x22), .d(x01), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n71_), .O(z17));
  nand2  g113(.a(x27), .b(new_n119_), .O(new_n176_));
  nand2  g114(.a(new_n92_), .b(x08), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n176_), .c(new_n77_), .O(new_n178_));
  inv1   g116(.a(x09), .O(new_n179_));
  nand2  g117(.a(new_n75_), .b(new_n179_), .O(new_n180_));
  aoi21  g118(.a(new_n178_), .b(new_n135_), .c(new_n180_), .O(new_n181_));
  oai22  g119(.a(new_n181_), .b(new_n74_), .c(new_n99_), .d(new_n84_), .O(z18));
  aoi21  g120(.a(x40), .b(new_n70_), .c(x08), .O(new_n183_));
  aoi21  g121(.a(x40), .b(x29), .c(new_n135_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n183_), .c(new_n92_), .O(new_n185_));
  nand2  g123(.a(new_n84_), .b(x39), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(x40), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n77_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n185_), .O(new_n189_));
  nand3  g127(.a(new_n189_), .b(new_n75_), .c(new_n179_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n71_), .O(z20));
  nor2   g129(.a(new_n139_), .b(x09), .O(z19));
endmodule


