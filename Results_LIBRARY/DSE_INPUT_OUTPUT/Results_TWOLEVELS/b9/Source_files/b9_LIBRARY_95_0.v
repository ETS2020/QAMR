// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(x15), .c(x16), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n70_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x27), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  aoi21  g020(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  or2    g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(x35), .b(new_n82_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(x04), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n89_), .c(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x40), .O(new_n94_));
  nand2  g032(.a(new_n74_), .b(x39), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(z02));
  nand2  g034(.a(new_n66_), .b(x27), .O(new_n97_));
  aoi21  g035(.a(new_n65_), .b(new_n81_), .c(new_n63_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n97_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n97_), .c(new_n100_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n63_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n72_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n64_), .O(z07));
  nand2  g053(.a(x40), .b(x39), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  xor2a  g058(.a(x40), .b(x39), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x07), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x05), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n83_), .O(new_n124_));
  inv1   g062(.a(x06), .O(new_n125_));
  nor3   g063(.a(new_n63_), .b(new_n65_), .c(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n71_), .c(new_n126_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n80_), .b(new_n79_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x40), .c(x39), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n71_), .O(new_n132_));
  oai21  g070(.a(new_n127_), .b(new_n81_), .c(new_n132_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  nor2   g073(.a(new_n81_), .b(new_n71_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n90_), .c(new_n135_), .O(new_n137_));
  inv1   g075(.a(x08), .O(new_n138_));
  nand2  g076(.a(x27), .b(new_n71_), .O(new_n139_));
  nand2  g077(.a(x40), .b(x28), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x35), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n139_), .c(new_n138_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n137_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n79_), .c(new_n134_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n64_), .O(z11));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  nor2   g084(.a(new_n74_), .b(x27), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n146_), .c(new_n73_), .O(new_n148_));
  inv1   g086(.a(new_n95_), .O(new_n149_));
  aoi21  g087(.a(x39), .b(new_n71_), .c(new_n74_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n149_), .c(new_n68_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n148_), .c(new_n64_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n74_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n71_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n67_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nor2   g099(.a(new_n130_), .b(x13), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n71_), .c(new_n63_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n161_), .O(z13));
  nand3  g102(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n165_));
  nand4  g103(.a(new_n154_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n166_));
  nand3  g104(.a(x39), .b(new_n153_), .c(new_n71_), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n166_), .c(new_n74_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n149_), .c(new_n165_), .O(new_n170_));
  nand2  g108(.a(x39), .b(new_n81_), .O(new_n171_));
  inv1   g109(.a(x35), .O(new_n172_));
  nand4  g110(.a(x40), .b(new_n65_), .c(new_n172_), .d(x28), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n171_), .c(x32), .O(new_n174_));
  oai21  g112(.a(new_n168_), .b(new_n74_), .c(new_n95_), .O(new_n175_));
  aoi22  g113(.a(new_n175_), .b(new_n68_), .c(new_n174_), .d(new_n79_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n170_), .O(z14));
  inv1   g115(.a(x12), .O(new_n178_));
  nor2   g116(.a(new_n119_), .b(new_n178_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand4  g118(.a(new_n64_), .b(new_n180_), .c(x22), .d(x01), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z16));
  inv1   g120(.a(x01), .O(new_n183_));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n64_), .b(new_n184_), .c(x23), .d(x22), .O(new_n185_));
  nor2   g123(.a(new_n185_), .b(new_n183_), .O(z17));
  nand2  g124(.a(new_n81_), .b(x08), .O(new_n187_));
  aoi21  g125(.a(new_n187_), .b(new_n139_), .c(new_n172_), .O(new_n188_));
  nand2  g126(.a(new_n79_), .b(new_n134_), .O(new_n189_));
  aoi21  g127(.a(new_n188_), .b(new_n82_), .c(new_n189_), .O(new_n190_));
  oai22  g128(.a(new_n190_), .b(new_n63_), .c(new_n116_), .d(new_n88_), .O(z18));
  inv1   g129(.a(new_n135_), .O(new_n192_));
  oai21  g130(.a(new_n172_), .b(new_n81_), .c(new_n138_), .O(new_n193_));
  aoi21  g131(.a(x28), .b(new_n81_), .c(new_n172_), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n192_), .c(new_n193_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n79_), .c(new_n134_), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n64_), .O(z20));
  nand2  g135(.a(new_n144_), .b(new_n64_), .O(z19));
endmodule


