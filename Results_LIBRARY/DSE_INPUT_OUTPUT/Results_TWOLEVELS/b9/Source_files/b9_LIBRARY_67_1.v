// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x38), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(x38), .d(new_n66_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n72_), .d(new_n65_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n65_), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n83_), .c(x04), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x39), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n88_), .c(x38), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x40), .O(z02));
  nand2  g031(.a(new_n70_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n69_), .b(new_n68_), .c(new_n95_), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n94_), .c(new_n65_), .O(z03));
  aoi21  g035(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n94_), .c(new_n65_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n64_), .c(new_n69_), .O(z05));
  aoi21  g039(.a(new_n64_), .b(new_n68_), .c(new_n82_), .O(new_n102_));
  oai21  g040(.a(new_n102_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n66_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(new_n111_));
  oai22  g049(.a(new_n111_), .b(x14), .c(new_n108_), .d(x31), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n64_), .O(z07));
  inv1   g052(.a(x39), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(x38), .c(new_n74_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  inv1   g057(.a(x04), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand4  g059(.a(x40), .b(x39), .c(x38), .d(x05), .O(new_n122_));
  oai21  g060(.a(z08), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n84_), .c(new_n120_), .O(new_n124_));
  nand3  g062(.a(new_n64_), .b(x37), .c(x06), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  nand2  g065(.a(new_n80_), .b(new_n79_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x40), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n115_), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(x38), .c(x05), .d(new_n120_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n127_), .O(z10));
  nand2  g070(.a(new_n64_), .b(new_n68_), .O(new_n133_));
  oai21  g071(.a(new_n83_), .b(x04), .c(x38), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n133_), .c(x08), .O(new_n135_));
  inv1   g073(.a(new_n83_), .O(new_n136_));
  nand2  g074(.a(x39), .b(x29), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(x38), .c(new_n74_), .O(new_n138_));
  nand2  g076(.a(x27), .b(x04), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n136_), .c(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n135_), .c(new_n79_), .O(new_n141_));
  nor2   g079(.a(new_n141_), .b(x09), .O(z11));
  inv1   g080(.a(x35), .O(new_n143_));
  nand3  g081(.a(new_n69_), .b(new_n81_), .c(new_n143_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  aoi21  g083(.a(x39), .b(new_n120_), .c(new_n71_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(x38), .O(new_n147_));
  oai21  g085(.a(new_n71_), .b(x40), .c(new_n147_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x28), .c(new_n74_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n149_), .d(new_n120_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand2  g092(.a(new_n70_), .b(new_n69_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n152_), .c(new_n68_), .O(new_n157_));
  nand4  g095(.a(new_n128_), .b(x39), .c(new_n149_), .d(new_n120_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x38), .c(new_n74_), .O(new_n159_));
  or2    g097(.a(new_n159_), .b(new_n157_), .O(z13));
  nand3  g098(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n161_));
  nor2   g099(.a(new_n74_), .b(new_n115_), .O(new_n162_));
  nand4  g100(.a(new_n150_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(new_n162_), .c(new_n149_), .d(new_n120_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand3  g103(.a(new_n69_), .b(new_n143_), .c(x28), .O(new_n166_));
  oai21  g104(.a(new_n63_), .b(x27), .c(new_n166_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n80_), .c(new_n79_), .O(new_n168_));
  oai21  g106(.a(new_n115_), .b(x04), .c(x38), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(x40), .c(new_n149_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n70_), .c(new_n69_), .O(new_n171_));
  nand3  g109(.a(x39), .b(new_n149_), .c(new_n120_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x38), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x40), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n68_), .c(new_n65_), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(new_n171_), .c(new_n168_), .d(new_n165_), .O(z14));
  inv1   g114(.a(x12), .O(new_n177_));
  nor2   g115(.a(new_n118_), .b(new_n177_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand4  g117(.a(new_n64_), .b(new_n179_), .c(x22), .d(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(x23), .c(x22), .d(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n64_), .O(z17));
  nand2  g122(.a(new_n162_), .b(x29), .O(new_n185_));
  oai21  g123(.a(new_n83_), .b(x27), .c(new_n185_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(x08), .O(new_n187_));
  nor2   g125(.a(new_n68_), .b(x04), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n136_), .c(x09), .O(new_n189_));
  nand4  g127(.a(new_n189_), .b(new_n187_), .c(new_n64_), .d(new_n79_), .O(z18));
  oai22  g128(.a(new_n63_), .b(x35), .c(new_n82_), .d(x27), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n137_), .O(new_n192_));
  oai21  g130(.a(new_n63_), .b(x08), .c(x40), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n143_), .O(new_n194_));
  aoi21  g132(.a(x40), .b(new_n63_), .c(x08), .O(new_n195_));
  nor2   g133(.a(x40), .b(new_n82_), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n195_), .c(new_n68_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n194_), .c(new_n192_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n79_), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(x09), .c(new_n64_), .O(z20));
  nor2   g138(.a(new_n141_), .b(x09), .O(z19));
endmodule


