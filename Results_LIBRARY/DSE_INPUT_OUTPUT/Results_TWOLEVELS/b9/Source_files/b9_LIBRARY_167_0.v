// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n107_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_;
  inv1   g000(.a(x38), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n67_), .c(new_n68_), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n75_), .c(new_n64_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n67_), .d(new_n74_), .O(new_n78_));
  oai21  g016(.a(new_n73_), .b(new_n66_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n65_), .c(x04), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  inv1   g028(.a(x29), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  inv1   g031(.a(x35), .O(new_n94_));
  nand2  g032(.a(x40), .b(new_n94_), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n83_), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n93_), .c(x04), .O(new_n97_));
  nand2  g035(.a(x40), .b(x39), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(z08));
  nand3  g037(.a(z08), .b(new_n97_), .c(new_n92_), .O(z02));
  nand2  g038(.a(new_n71_), .b(x27), .O(new_n101_));
  inv1   g039(.a(x21), .O(new_n102_));
  aoi21  g040(.a(new_n70_), .b(new_n69_), .c(new_n102_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n101_), .c(new_n66_), .O(z03));
  aoi21  g042(.a(new_n70_), .b(new_n69_), .c(x21), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n101_), .c(new_n66_), .O(z04));
  aoi21  g044(.a(x28), .b(x27), .c(x37), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(new_n66_), .O(z05));
  aoi21  g046(.a(new_n65_), .b(new_n69_), .c(new_n83_), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(x37), .c(new_n65_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n67_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n65_), .O(z07));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n65_), .O(z09));
  inv1   g061(.a(x07), .O(new_n124_));
  inv1   g062(.a(x39), .O(new_n125_));
  nand2  g063(.a(x40), .b(new_n125_), .O(new_n126_));
  nand2  g064(.a(new_n64_), .b(x38), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n124_), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x05), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n128_), .c(new_n85_), .O(new_n131_));
  nand3  g069(.a(new_n65_), .b(x37), .c(x06), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(x04), .c(new_n132_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x27), .O(new_n134_));
  oai21  g072(.a(x32), .b(x30), .c(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n125_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x05), .c(new_n74_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  nand2  g077(.a(new_n65_), .b(new_n69_), .O(new_n140_));
  nand4  g078(.a(new_n140_), .b(new_n95_), .c(new_n83_), .d(new_n74_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n90_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n95_), .c(new_n83_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand2  g084(.a(x38), .b(new_n94_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n144_), .c(new_n83_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n64_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n146_), .c(new_n142_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n80_), .c(new_n139_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n65_), .O(z11));
  nand3  g090(.a(new_n70_), .b(new_n82_), .c(new_n94_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x27), .c(x10), .O(new_n154_));
  aoi21  g092(.a(x39), .b(new_n74_), .c(new_n72_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n154_), .c(x40), .O(new_n156_));
  nor2   g094(.a(new_n72_), .b(x40), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x38), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n156_), .O(z12));
  inv1   g097(.a(x13), .O(new_n160_));
  nand2  g098(.a(x36), .b(x35), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x28), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x39), .c(new_n160_), .d(new_n74_), .O(new_n163_));
  inv1   g101(.a(x18), .O(new_n164_));
  inv1   g102(.a(x19), .O(new_n165_));
  nand2  g103(.a(new_n71_), .b(new_n70_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x20), .c(new_n165_), .d(new_n164_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n163_), .c(new_n64_), .O(new_n168_));
  nand4  g106(.a(new_n166_), .b(x38), .c(x20), .d(new_n165_), .O(new_n169_));
  nor2   g107(.a(new_n169_), .b(x18), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n168_), .c(x27), .O(new_n171_));
  nand3  g109(.a(new_n136_), .b(new_n160_), .c(new_n74_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(z13));
  inv1   g111(.a(new_n127_), .O(new_n174_));
  nand3  g112(.a(x20), .b(new_n165_), .c(new_n164_), .O(new_n175_));
  nand4  g113(.a(new_n161_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n176_));
  nand3  g114(.a(x39), .b(new_n160_), .c(new_n74_), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n176_), .c(new_n64_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n174_), .c(new_n175_), .O(new_n180_));
  nand3  g118(.a(new_n70_), .b(new_n94_), .c(x28), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(x27), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n81_), .c(new_n80_), .O(new_n183_));
  oai21  g121(.a(new_n178_), .b(new_n72_), .c(new_n183_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x40), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n180_), .c(new_n158_), .O(z14));
  inv1   g124(.a(x12), .O(new_n187_));
  nand4  g125(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n187_), .O(z15));
  inv1   g127(.a(x23), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x22), .c(x01), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n65_), .O(z16));
  inv1   g130(.a(x24), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x23), .c(x22), .d(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n65_), .O(z17));
  oai22  g133(.a(new_n98_), .b(new_n91_), .c(new_n84_), .d(x27), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(x08), .O(new_n197_));
  inv1   g135(.a(new_n84_), .O(new_n198_));
  nor2   g136(.a(new_n69_), .b(x04), .O(new_n199_));
  aoi21  g137(.a(new_n199_), .b(new_n198_), .c(x09), .O(new_n200_));
  nand4  g138(.a(new_n200_), .b(new_n197_), .c(new_n65_), .d(new_n80_), .O(z18));
  nand2  g139(.a(x28), .b(new_n69_), .O(new_n202_));
  nand2  g140(.a(new_n143_), .b(x40), .O(new_n203_));
  aoi22  g141(.a(new_n203_), .b(new_n127_), .c(new_n202_), .d(x35), .O(new_n204_));
  aoi21  g142(.a(new_n140_), .b(new_n95_), .c(x08), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n204_), .c(new_n80_), .O(new_n206_));
  nor2   g144(.a(new_n206_), .b(x09), .O(z20));
  nand2  g145(.a(new_n151_), .b(new_n65_), .O(z19));
endmodule


