// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n196_, new_n197_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x38), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x15), .O(new_n71_));
  inv1   g009(.a(x38), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n70_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(x35), .O(new_n90_));
  nand2  g028(.a(new_n72_), .b(new_n90_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n81_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(x38), .O(new_n93_));
  inv1   g031(.a(x39), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x04), .c(new_n94_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n93_), .c(new_n89_), .d(x40), .O(z02));
  inv1   g035(.a(new_n64_), .O(new_n98_));
  nand2  g036(.a(new_n66_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x21), .O(new_n100_));
  inv1   g038(.a(x27), .O(new_n101_));
  aoi21  g039(.a(new_n65_), .b(new_n101_), .c(new_n100_), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n98_), .O(z03));
  aoi21  g041(.a(new_n65_), .b(new_n101_), .c(x21), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n99_), .c(new_n98_), .O(z04));
  aoi21  g043(.a(x28), .b(x27), .c(x37), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(new_n98_), .O(z05));
  aoi21  g045(.a(x40), .b(x38), .c(x27), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n81_), .c(new_n65_), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n64_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n71_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n64_), .O(z07));
  nand3  g059(.a(x40), .b(x39), .c(new_n72_), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n64_), .O(z09));
  inv1   g063(.a(x04), .O(new_n126_));
  inv1   g064(.a(x36), .O(new_n127_));
  oai21  g065(.a(x39), .b(x38), .c(x40), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x07), .O(new_n129_));
  nand4  g067(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n130_));
  aoi22  g068(.a(new_n130_), .b(new_n129_), .c(new_n82_), .d(new_n127_), .O(new_n131_));
  nor2   g069(.a(new_n98_), .b(new_n65_), .O(new_n132_));
  aoi22  g070(.a(new_n132_), .b(x06), .c(new_n131_), .d(new_n126_), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor3   g072(.a(new_n134_), .b(new_n94_), .c(x38), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x05), .c(new_n126_), .O(new_n136_));
  oai21  g074(.a(new_n133_), .b(new_n101_), .c(new_n136_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  inv1   g076(.a(x08), .O(new_n139_));
  nand2  g077(.a(x40), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n91_), .c(new_n81_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n108_), .c(new_n139_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  nor2   g081(.a(new_n101_), .b(new_n126_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n92_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n144_), .b(new_n82_), .c(new_n74_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(new_n142_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n79_), .c(new_n138_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n64_), .O(z11));
  nand3  g087(.a(new_n65_), .b(new_n127_), .c(new_n90_), .O(new_n150_));
  nand2  g088(.a(x40), .b(new_n101_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n150_), .c(x10), .O(new_n152_));
  nand2  g090(.a(new_n140_), .b(x39), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n101_), .O(new_n154_));
  nand2  g092(.a(x39), .b(new_n126_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n66_), .c(new_n65_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n152_), .c(new_n72_), .O(new_n158_));
  nand2  g096(.a(new_n68_), .b(new_n74_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z12));
  inv1   g098(.a(x13), .O(new_n161_));
  nand2  g099(.a(x36), .b(x35), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(x28), .c(new_n74_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x39), .c(new_n161_), .d(new_n126_), .O(new_n164_));
  inv1   g102(.a(x18), .O(new_n165_));
  inv1   g103(.a(x19), .O(new_n166_));
  nand4  g104(.a(new_n67_), .b(x20), .c(new_n166_), .d(new_n165_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n164_), .c(x38), .O(new_n168_));
  nand4  g106(.a(new_n67_), .b(new_n74_), .c(x20), .d(new_n166_), .O(new_n169_));
  nor2   g107(.a(new_n169_), .b(x18), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n168_), .c(x27), .O(new_n171_));
  nand3  g109(.a(new_n135_), .b(new_n161_), .c(new_n126_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(z13));
  nand3  g111(.a(x20), .b(new_n166_), .c(new_n165_), .O(new_n174_));
  nand4  g112(.a(new_n162_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n175_));
  oai21  g113(.a(x13), .b(x04), .c(x40), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n175_), .c(x39), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n72_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(x40), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n174_), .O(new_n180_));
  nand3  g118(.a(new_n65_), .b(new_n90_), .c(x28), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(x27), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n80_), .c(new_n79_), .O(new_n183_));
  nand2  g121(.a(new_n151_), .b(new_n67_), .O(new_n184_));
  oai21  g122(.a(x13), .b(x04), .c(new_n184_), .O(new_n185_));
  nand2  g123(.a(new_n68_), .b(new_n94_), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n185_), .c(new_n183_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n72_), .O(new_n188_));
  nand3  g126(.a(new_n188_), .b(new_n180_), .c(new_n159_), .O(z14));
  inv1   g127(.a(x12), .O(new_n190_));
  nand4  g128(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n191_));
  nor2   g129(.a(new_n191_), .b(new_n190_), .O(z15));
  inv1   g130(.a(x23), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(x22), .c(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n64_), .O(z16));
  inv1   g133(.a(x24), .O(new_n196_));
  nand4  g134(.a(new_n196_), .b(x23), .c(x22), .d(x01), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n64_), .O(z17));
  nand3  g136(.a(x40), .b(x39), .c(x29), .O(new_n199_));
  oai21  g137(.a(new_n82_), .b(x27), .c(new_n199_), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(x08), .O(new_n201_));
  nor2   g139(.a(new_n101_), .b(x04), .O(new_n202_));
  aoi21  g140(.a(new_n202_), .b(new_n83_), .c(x09), .O(new_n203_));
  nand4  g141(.a(new_n203_), .b(new_n201_), .c(new_n64_), .d(new_n79_), .O(z18));
  aoi21  g142(.a(x28), .b(new_n101_), .c(new_n90_), .O(new_n205_));
  aoi21  g143(.a(new_n143_), .b(new_n72_), .c(new_n74_), .O(new_n206_));
  inv1   g144(.a(new_n91_), .O(new_n207_));
  oai21  g145(.a(new_n108_), .b(new_n207_), .c(new_n139_), .O(new_n208_));
  oai21  g146(.a(new_n206_), .b(new_n205_), .c(new_n208_), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(new_n79_), .c(new_n138_), .O(new_n210_));
  inv1   g148(.a(new_n210_), .O(z20));
  nand2  g149(.a(new_n148_), .b(new_n64_), .O(z19));
endmodule


