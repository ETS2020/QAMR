// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  nand2  g015(.a(new_n73_), .b(x37), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nor2   g019(.a(new_n66_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  aoi22  g027(.a(new_n89_), .b(new_n86_), .c(new_n88_), .d(x04), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  nand3  g029(.a(new_n64_), .b(new_n91_), .c(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(x40), .d(x39), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(x40), .c(x27), .O(new_n95_));
  oai21  g033(.a(x37), .b(new_n66_), .c(new_n73_), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(z06));
  nor2   g036(.a(z06), .b(new_n97_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n96_), .c(new_n95_), .O(z03));
  oai21  g038(.a(x37), .b(new_n64_), .c(new_n73_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x21), .O(new_n102_));
  inv1   g040(.a(x37), .O(new_n103_));
  oai21  g041(.a(x40), .b(x35), .c(x28), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(new_n64_), .c(new_n103_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n102_), .c(new_n95_), .O(z04));
  nand3  g044(.a(new_n103_), .b(x28), .c(x27), .O(new_n107_));
  oai21  g045(.a(new_n73_), .b(new_n103_), .c(new_n107_), .O(z05));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n78_), .c(x03), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z07));
  oai21  g057(.a(new_n73_), .b(new_n72_), .c(new_n78_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n78_), .O(z09));
  nor2   g060(.a(new_n82_), .b(x36), .O(new_n123_));
  nor2   g061(.a(new_n73_), .b(x39), .O(new_n124_));
  nor2   g062(.a(x40), .b(x37), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n124_), .c(x07), .O(new_n126_));
  nand2  g064(.a(new_n74_), .b(x05), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n123_), .O(new_n128_));
  oai21  g066(.a(x32), .b(x30), .c(x40), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n72_), .O(new_n130_));
  and2   g068(.a(new_n130_), .b(x05), .O(new_n131_));
  aoi21  g069(.a(new_n128_), .b(x27), .c(new_n131_), .O(new_n132_));
  nand4  g070(.a(x40), .b(x37), .c(x27), .d(x06), .O(new_n133_));
  oai21  g071(.a(new_n132_), .b(x04), .c(new_n133_), .O(z10));
  nand2  g072(.a(new_n78_), .b(new_n64_), .O(new_n135_));
  oai21  g073(.a(new_n88_), .b(x04), .c(x40), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n135_), .c(x08), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x40), .c(new_n125_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n82_), .c(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n137_), .c(new_n80_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(x09), .O(z11));
  inv1   g081(.a(x10), .O(new_n144_));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  nand2  g083(.a(x40), .b(new_n64_), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n145_), .c(new_n144_), .O(new_n148_));
  or2    g086(.a(new_n147_), .b(new_n70_), .O(new_n149_));
  oai21  g087(.a(new_n72_), .b(x04), .c(new_n149_), .O(new_n150_));
  nand3  g088(.a(new_n82_), .b(x27), .c(new_n65_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(x35), .c(x27), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n73_), .c(new_n103_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n150_), .c(new_n148_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x28), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n65_), .O(new_n158_));
  inv1   g096(.a(x18), .O(new_n159_));
  inv1   g097(.a(x19), .O(new_n160_));
  nand4  g098(.a(x37), .b(x20), .c(new_n160_), .d(new_n159_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n158_), .c(new_n73_), .O(new_n162_));
  nand3  g100(.a(x20), .b(new_n160_), .c(new_n159_), .O(new_n163_));
  nor4   g101(.a(new_n163_), .b(x37), .c(new_n66_), .d(new_n87_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n162_), .c(x27), .O(new_n165_));
  nand3  g103(.a(new_n130_), .b(new_n155_), .c(new_n65_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n165_), .O(z13));
  nand4  g105(.a(new_n156_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  nand4  g107(.a(x40), .b(x39), .c(new_n155_), .d(new_n65_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n169_), .c(new_n163_), .O(new_n171_));
  nand3  g109(.a(new_n103_), .b(new_n66_), .c(x28), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x27), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n81_), .c(new_n80_), .O(new_n174_));
  nand3  g112(.a(x39), .b(new_n155_), .c(new_n65_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n149_), .O(new_n176_));
  oai21  g114(.a(new_n152_), .b(x37), .c(new_n73_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n176_), .c(new_n174_), .d(new_n171_), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n78_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n78_), .O(z16));
  inv1   g121(.a(x01), .O(new_n184_));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n78_), .b(new_n185_), .c(x23), .d(x22), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z17));
  nand2  g125(.a(new_n73_), .b(x37), .O(new_n188_));
  nand3  g126(.a(new_n188_), .b(x27), .c(new_n65_), .O(new_n189_));
  oai21  g127(.a(new_n135_), .b(new_n91_), .c(new_n189_), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x35), .c(new_n87_), .O(new_n191_));
  inv1   g129(.a(new_n89_), .O(new_n192_));
  inv1   g130(.a(x09), .O(new_n193_));
  nand2  g131(.a(new_n80_), .b(new_n193_), .O(new_n194_));
  aoi22  g132(.a(new_n194_), .b(new_n78_), .c(new_n192_), .d(new_n74_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n191_), .O(z18));
  oai22  g134(.a(new_n73_), .b(x35), .c(new_n87_), .d(x27), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n138_), .O(new_n198_));
  nor2   g136(.a(new_n73_), .b(x08), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n125_), .c(new_n66_), .O(new_n200_));
  aoi21  g138(.a(new_n73_), .b(x37), .c(x08), .O(new_n201_));
  nor2   g139(.a(x40), .b(new_n87_), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n201_), .c(new_n64_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n200_), .c(new_n198_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n80_), .c(new_n193_), .O(new_n205_));
  nand2  g143(.a(new_n205_), .b(new_n78_), .O(z20));
  nor2   g144(.a(new_n142_), .b(x09), .O(z19));
endmodule


