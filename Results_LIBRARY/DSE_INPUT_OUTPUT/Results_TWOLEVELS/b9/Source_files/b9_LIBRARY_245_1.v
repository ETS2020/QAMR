// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_;
  inv1   g000(.a(x03), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n66_), .d(new_n73_), .O(new_n78_));
  oai22  g016(.a(new_n78_), .b(new_n63_), .c(new_n72_), .d(new_n65_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  oai21  g029(.a(x35), .b(new_n63_), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n90_), .c(x04), .O(new_n93_));
  nand3  g031(.a(new_n69_), .b(x28), .c(x04), .O(new_n94_));
  nand2  g032(.a(new_n64_), .b(x39), .O(new_n95_));
  aoi21  g033(.a(new_n94_), .b(new_n75_), .c(new_n95_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n93_), .c(new_n89_), .O(z02));
  nand2  g035(.a(new_n70_), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n64_), .b(new_n69_), .c(new_n68_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n64_), .d(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand4  g039(.a(new_n99_), .b(new_n98_), .c(new_n64_), .d(new_n101_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n69_), .c(new_n65_), .O(z05));
  nand3  g042(.a(new_n103_), .b(new_n64_), .c(new_n69_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(z07));
  inv1   g055(.a(x39), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(x03), .c(new_n75_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  nor2   g060(.a(new_n83_), .b(x36), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x05), .O(new_n125_));
  oai21  g063(.a(x39), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x03), .O(new_n127_));
  nand2  g065(.a(new_n75_), .b(x07), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n123_), .O(new_n129_));
  inv1   g067(.a(x06), .O(new_n130_));
  nor3   g068(.a(new_n65_), .b(new_n69_), .c(new_n130_), .O(new_n131_));
  aoi21  g069(.a(new_n129_), .b(new_n73_), .c(new_n131_), .O(new_n132_));
  nand2  g070(.a(new_n81_), .b(new_n80_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x40), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n118_), .O(new_n135_));
  nand4  g073(.a(new_n135_), .b(x05), .c(new_n73_), .d(x03), .O(new_n136_));
  oai21  g074(.a(new_n132_), .b(new_n68_), .c(new_n136_), .O(z10));
  nand2  g075(.a(new_n64_), .b(new_n68_), .O(new_n138_));
  nand2  g076(.a(x35), .b(new_n91_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(x04), .c(x03), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n138_), .c(x08), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x03), .c(new_n75_), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n83_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n141_), .c(new_n80_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(x09), .O(z11));
  inv1   g085(.a(x36), .O(new_n148_));
  nand3  g086(.a(new_n69_), .b(new_n148_), .c(new_n82_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x27), .c(x10), .O(new_n150_));
  aoi21  g088(.a(x39), .b(new_n73_), .c(new_n71_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(x03), .O(new_n152_));
  inv1   g090(.a(new_n71_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n75_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n75_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n73_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand2  g099(.a(new_n70_), .b(new_n69_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n159_), .c(new_n68_), .O(new_n164_));
  nand4  g102(.a(new_n133_), .b(x39), .c(new_n156_), .d(new_n73_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x03), .c(new_n75_), .O(new_n166_));
  or2    g104(.a(new_n166_), .b(new_n164_), .O(z13));
  nand3  g105(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n168_));
  nand4  g106(.a(new_n157_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n169_));
  nand3  g107(.a(x39), .b(new_n156_), .c(new_n73_), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n169_), .c(new_n63_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n75_), .c(new_n168_), .O(new_n173_));
  nand3  g111(.a(new_n69_), .b(new_n82_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n81_), .c(new_n80_), .O(new_n176_));
  oai21  g114(.a(new_n171_), .b(new_n71_), .c(new_n176_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x03), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n173_), .c(new_n154_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nor2   g118(.a(new_n121_), .b(new_n180_), .O(z15));
  inv1   g119(.a(x23), .O(new_n182_));
  nand4  g120(.a(new_n64_), .b(new_n182_), .c(x22), .d(x01), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(z16));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(x23), .c(x22), .d(x01), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n64_), .O(z17));
  nand3  g125(.a(x40), .b(x39), .c(x29), .O(new_n188_));
  oai21  g126(.a(new_n139_), .b(x27), .c(new_n188_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x08), .O(new_n190_));
  nor2   g128(.a(new_n65_), .b(x09), .O(new_n191_));
  nor2   g129(.a(new_n68_), .b(x04), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n83_), .c(x30), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n191_), .c(new_n190_), .O(z18));
  aoi21  g132(.a(x28), .b(new_n68_), .c(new_n82_), .O(new_n195_));
  nor2   g133(.a(new_n195_), .b(new_n143_), .O(new_n196_));
  oai21  g134(.a(new_n82_), .b(new_n68_), .c(x03), .O(new_n197_));
  nand2  g135(.a(new_n75_), .b(new_n68_), .O(new_n198_));
  aoi21  g136(.a(new_n198_), .b(new_n197_), .c(x08), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n196_), .c(new_n80_), .O(new_n200_));
  nor2   g138(.a(new_n200_), .b(x09), .O(z20));
  nor2   g139(.a(new_n146_), .b(x09), .O(z19));
endmodule


