// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n110_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n191_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n204_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  and2   g012(.a(x40), .b(x39), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(x10), .c(new_n65_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n74_), .c(new_n63_), .O(new_n78_));
  nand2  g016(.a(x40), .b(x32), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n78_), .c(x16), .O(z00));
  inv1   g018(.a(new_n79_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  aoi21  g022(.a(x35), .b(new_n84_), .c(x36), .O(new_n85_));
  or2    g023(.a(new_n85_), .b(new_n64_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n83_), .c(new_n82_), .O(new_n87_));
  oai21  g025(.a(new_n81_), .b(x04), .c(new_n87_), .O(z01));
  inv1   g026(.a(x40), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g030(.a(x28), .b(x04), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n92_), .c(new_n89_), .O(new_n94_));
  inv1   g032(.a(x08), .O(new_n95_));
  aoi21  g033(.a(new_n64_), .b(new_n95_), .c(new_n66_), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n65_), .c(x39), .O(new_n97_));
  oai21  g035(.a(new_n97_), .b(new_n94_), .c(new_n83_), .O(new_n98_));
  inv1   g036(.a(x09), .O(new_n99_));
  aoi21  g037(.a(x28), .b(new_n99_), .c(x30), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(x28), .c(x27), .d(new_n99_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n89_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n98_), .O(z02));
  nand2  g041(.a(new_n71_), .b(x27), .O(new_n104_));
  inv1   g042(.a(x21), .O(new_n105_));
  aoi21  g043(.a(new_n70_), .b(new_n64_), .c(new_n105_), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n104_), .c(new_n81_), .O(z03));
  nand2  g045(.a(new_n70_), .b(new_n64_), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(new_n104_), .c(new_n79_), .d(new_n105_), .O(z04));
  nand2  g047(.a(x28), .b(x27), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n79_), .c(new_n70_), .O(z05));
  aoi21  g049(.a(x40), .b(x32), .c(x27), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n84_), .c(new_n70_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n79_), .O(z06));
  inv1   g052(.a(x33), .O(new_n115_));
  nand2  g053(.a(x17), .b(new_n63_), .O(new_n116_));
  inv1   g054(.a(x00), .O(new_n117_));
  oai21  g055(.a(x25), .b(new_n117_), .c(x38), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(new_n119_));
  inv1   g057(.a(x25), .O(new_n120_));
  nand3  g058(.a(x38), .b(new_n120_), .c(new_n117_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  oai22  g060(.a(new_n122_), .b(x14), .c(new_n119_), .d(x31), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n79_), .c(x03), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(z07));
  oai21  g063(.a(x39), .b(x32), .c(x40), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(z08));
  nand4  g065(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n79_), .O(z09));
  nand2  g067(.a(new_n126_), .b(x07), .O(new_n130_));
  nand4  g068(.a(x40), .b(x39), .c(new_n83_), .d(x05), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n130_), .c(new_n85_), .O(new_n132_));
  nand3  g070(.a(new_n79_), .b(x37), .c(x06), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(new_n134_));
  aoi21  g072(.a(new_n132_), .b(new_n65_), .c(new_n134_), .O(new_n135_));
  nand3  g073(.a(x30), .b(x05), .c(new_n65_), .O(new_n136_));
  nand2  g074(.a(new_n75_), .b(new_n83_), .O(new_n137_));
  or2    g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  oai21  g076(.a(new_n135_), .b(new_n64_), .c(new_n138_), .O(z10));
  nand2  g077(.a(x40), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x35), .c(new_n84_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n83_), .c(new_n112_), .O(new_n142_));
  aoi21  g080(.a(x39), .b(x29), .c(x32), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(x35), .c(new_n84_), .O(new_n145_));
  oai21  g083(.a(new_n143_), .b(new_n89_), .c(new_n145_), .O(new_n146_));
  oai21  g084(.a(new_n142_), .b(x08), .c(new_n146_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n82_), .c(new_n99_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(z11));
  nand3  g087(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n150_));
  nand2  g088(.a(x40), .b(new_n64_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n150_), .c(x10), .O(new_n152_));
  nand2  g090(.a(new_n140_), .b(x39), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n64_), .O(new_n154_));
  nand2  g092(.a(x39), .b(new_n65_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n71_), .c(new_n70_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n152_), .c(new_n83_), .O(new_n158_));
  oai21  g096(.a(new_n73_), .b(new_n64_), .c(new_n89_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z12));
  inv1   g098(.a(x13), .O(new_n161_));
  nand2  g099(.a(x36), .b(x35), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(x28), .c(new_n89_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x39), .c(new_n161_), .d(new_n65_), .O(new_n164_));
  inv1   g102(.a(x18), .O(new_n165_));
  inv1   g103(.a(x19), .O(new_n166_));
  nand4  g104(.a(new_n72_), .b(x20), .c(new_n166_), .d(new_n165_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x27), .O(new_n169_));
  nor2   g107(.a(x13), .b(x04), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(x39), .c(x30), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n83_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x40), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n169_), .O(z13));
  nand3  g112(.a(x20), .b(new_n166_), .c(new_n165_), .O(new_n175_));
  nand3  g113(.a(new_n162_), .b(new_n82_), .c(x28), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n170_), .c(new_n75_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n175_), .O(new_n178_));
  nand3  g116(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n179_));
  oai21  g117(.a(x32), .b(x27), .c(new_n179_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n82_), .O(new_n181_));
  nand2  g119(.a(new_n155_), .b(new_n83_), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(x40), .c(new_n161_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n71_), .c(new_n70_), .O(new_n184_));
  oai21  g122(.a(x13), .b(x04), .c(x40), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(x39), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n83_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(x40), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n64_), .c(new_n81_), .O(new_n189_));
  nand4  g127(.a(new_n189_), .b(new_n184_), .c(new_n181_), .d(new_n178_), .O(z14));
  nand4  g128(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n79_), .O(z15));
  inv1   g130(.a(x23), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(x22), .c(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n79_), .O(z16));
  inv1   g133(.a(x01), .O(new_n196_));
  inv1   g134(.a(x24), .O(new_n197_));
  nand4  g135(.a(new_n79_), .b(new_n197_), .c(x23), .d(x22), .O(new_n198_));
  nor2   g136(.a(new_n198_), .b(new_n196_), .O(z17));
  nand2  g137(.a(x27), .b(new_n65_), .O(new_n200_));
  nand2  g138(.a(new_n64_), .b(x08), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n200_), .c(new_n66_), .O(new_n202_));
  nand2  g140(.a(new_n82_), .b(new_n99_), .O(new_n203_));
  aoi21  g141(.a(new_n202_), .b(new_n84_), .c(new_n203_), .O(new_n204_));
  oai22  g142(.a(new_n204_), .b(new_n81_), .c(new_n137_), .d(new_n91_), .O(z18));
  nor2   g143(.a(new_n143_), .b(new_n89_), .O(new_n206_));
  aoi21  g144(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n207_));
  nor2   g145(.a(x35), .b(x32), .O(new_n208_));
  oai21  g146(.a(new_n208_), .b(new_n112_), .c(new_n95_), .O(new_n209_));
  oai21  g147(.a(new_n207_), .b(new_n206_), .c(new_n209_), .O(new_n210_));
  nand3  g148(.a(new_n210_), .b(new_n82_), .c(new_n99_), .O(new_n211_));
  inv1   g149(.a(new_n211_), .O(z20));
  inv1   g150(.a(new_n148_), .O(z19));
endmodule


