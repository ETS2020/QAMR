// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x10), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(new_n66_), .c(x10), .d(new_n73_), .O(new_n76_));
  oai21  g014(.a(new_n72_), .b(new_n65_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n65_), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  inv1   g025(.a(new_n82_), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n73_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n87_), .c(x10), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  nand3  g029(.a(new_n68_), .b(new_n91_), .c(x04), .O(new_n92_));
  inv1   g030(.a(x39), .O(new_n93_));
  nor2   g031(.a(new_n65_), .b(new_n93_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n90_), .d(x40), .O(z02));
  nand2  g033(.a(new_n70_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n69_), .b(new_n68_), .c(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n96_), .c(new_n65_), .O(z03));
  aoi21  g037(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n96_), .c(new_n65_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n69_), .c(new_n65_), .O(z05));
  nand3  g041(.a(new_n102_), .b(new_n64_), .c(new_n69_), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n64_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  nand3  g054(.a(x40), .b(x39), .c(x10), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z08));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x07), .O(new_n122_));
  inv1   g060(.a(x40), .O(new_n123_));
  aoi21  g061(.a(new_n93_), .b(x10), .c(new_n123_), .O(new_n124_));
  nand4  g062(.a(x40), .b(x39), .c(x10), .d(x05), .O(new_n125_));
  oai21  g063(.a(new_n124_), .b(new_n122_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n83_), .c(new_n73_), .O(new_n127_));
  nand3  g065(.a(new_n64_), .b(x37), .c(x06), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n79_), .b(new_n78_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x40), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n93_), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(x10), .c(x05), .d(new_n73_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n130_), .O(z10));
  nand2  g073(.a(new_n64_), .b(new_n68_), .O(new_n136_));
  oai21  g074(.a(new_n82_), .b(x04), .c(x10), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n136_), .c(x08), .O(new_n138_));
  nand2  g076(.a(x39), .b(x29), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x10), .c(new_n123_), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n88_), .c(new_n140_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n138_), .c(new_n78_), .O(new_n143_));
  nor2   g081(.a(new_n143_), .b(x09), .O(z11));
  nand2  g082(.a(x39), .b(new_n73_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x10), .c(new_n123_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n71_), .c(new_n64_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x28), .c(new_n123_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x39), .c(new_n148_), .d(new_n73_), .O(new_n151_));
  inv1   g089(.a(x18), .O(new_n152_));
  inv1   g090(.a(x19), .O(new_n153_));
  nand2  g091(.a(new_n70_), .b(new_n69_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x20), .c(new_n153_), .d(new_n152_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n151_), .c(new_n68_), .O(new_n156_));
  nand4  g094(.a(new_n131_), .b(x39), .c(new_n148_), .d(new_n73_), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x10), .c(new_n123_), .O(new_n158_));
  or2    g096(.a(new_n158_), .b(new_n156_), .O(z13));
  nand3  g097(.a(x20), .b(new_n153_), .c(new_n152_), .O(new_n160_));
  nand4  g098(.a(new_n149_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n161_));
  nand3  g099(.a(x39), .b(new_n148_), .c(new_n73_), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n161_), .c(new_n63_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n123_), .c(new_n160_), .O(new_n165_));
  inv1   g103(.a(x35), .O(new_n166_));
  nand3  g104(.a(new_n69_), .b(new_n166_), .c(x28), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x27), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n79_), .c(new_n78_), .O(new_n169_));
  oai21  g107(.a(new_n163_), .b(new_n71_), .c(new_n169_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x10), .O(new_n171_));
  or2    g109(.a(new_n71_), .b(x40), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n171_), .c(new_n165_), .O(z14));
  inv1   g111(.a(x12), .O(new_n174_));
  nor2   g112(.a(new_n120_), .b(new_n174_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(x22), .c(x01), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n64_), .O(z16));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(x23), .c(x22), .d(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n64_), .O(z17));
  inv1   g119(.a(x29), .O(new_n182_));
  oai22  g120(.a(new_n82_), .b(x27), .c(new_n74_), .d(new_n182_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(x08), .O(new_n184_));
  nor2   g122(.a(new_n65_), .b(x09), .O(new_n185_));
  nor2   g123(.a(new_n68_), .b(x04), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n88_), .c(x30), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n185_), .c(new_n184_), .O(z18));
  aoi21  g126(.a(x28), .b(new_n68_), .c(new_n166_), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n140_), .O(new_n190_));
  oai21  g128(.a(new_n166_), .b(new_n68_), .c(x10), .O(new_n191_));
  nand2  g129(.a(new_n123_), .b(new_n68_), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n191_), .c(x08), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n190_), .c(new_n78_), .O(new_n194_));
  nor2   g132(.a(new_n194_), .b(x09), .O(z20));
  nor2   g133(.a(new_n143_), .b(x09), .O(z19));
endmodule


