// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n184_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand4  g012(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n74_), .c(new_n63_), .O(new_n77_));
  nand2  g015(.a(x40), .b(x05), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nor2   g019(.a(new_n66_), .b(x28), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n67_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n81_), .c(new_n80_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  oai21  g029(.a(x35), .b(x05), .c(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(x05), .O(new_n93_));
  inv1   g031(.a(x39), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x04), .c(new_n94_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n93_), .c(new_n90_), .d(x40), .O(z02));
  inv1   g035(.a(new_n78_), .O(new_n98_));
  nand2  g036(.a(new_n71_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x21), .O(new_n100_));
  aoi21  g038(.a(new_n70_), .b(new_n64_), .c(new_n100_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(z03));
  nand2  g040(.a(new_n70_), .b(new_n64_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n99_), .c(new_n78_), .d(new_n100_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n98_), .O(z05));
  aoi21  g044(.a(new_n78_), .b(new_n64_), .c(new_n91_), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(x37), .c(new_n78_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n78_), .c(x03), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z07));
  inv1   g057(.a(x05), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(new_n120_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z08));
  nand4  g060(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n78_), .O(z09));
  nand3  g062(.a(new_n78_), .b(x37), .c(x06), .O(new_n125_));
  oai21  g063(.a(x39), .b(x05), .c(x40), .O(new_n126_));
  nand4  g064(.a(new_n126_), .b(new_n84_), .c(x07), .d(new_n65_), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n125_), .c(new_n64_), .O(z10));
  inv1   g066(.a(x09), .O(new_n129_));
  nand2  g067(.a(x40), .b(x04), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(x35), .c(new_n91_), .O(new_n131_));
  aoi22  g069(.a(new_n131_), .b(new_n120_), .c(new_n78_), .d(new_n64_), .O(new_n132_));
  inv1   g070(.a(x40), .O(new_n133_));
  nand2  g071(.a(x39), .b(x29), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n120_), .c(new_n133_), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x35), .c(new_n91_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(new_n138_));
  oai22  g076(.a(new_n138_), .b(new_n135_), .c(new_n132_), .d(x08), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n80_), .c(new_n129_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z11));
  nand3  g079(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n142_));
  nand2  g080(.a(x40), .b(new_n64_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n142_), .c(x10), .O(new_n144_));
  nand2  g082(.a(new_n130_), .b(x39), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n64_), .O(new_n146_));
  nand2  g084(.a(x39), .b(new_n65_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n71_), .c(new_n70_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n144_), .c(new_n120_), .O(new_n150_));
  oai21  g088(.a(new_n73_), .b(new_n64_), .c(new_n133_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n133_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n65_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n72_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n156_), .c(x05), .O(new_n160_));
  nand4  g098(.a(new_n72_), .b(new_n133_), .c(x20), .d(new_n158_), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(x18), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n160_), .c(x27), .O(new_n163_));
  oai21  g101(.a(x32), .b(x30), .c(x40), .O(new_n164_));
  nor2   g102(.a(new_n164_), .b(new_n94_), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(new_n153_), .c(new_n120_), .d(new_n65_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n163_), .O(z13));
  nand3  g105(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n168_));
  nand4  g106(.a(new_n154_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n169_));
  oai21  g107(.a(x13), .b(x04), .c(x40), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n169_), .c(x39), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n120_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x40), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n168_), .O(new_n174_));
  nand3  g112(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x27), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n81_), .c(new_n80_), .O(new_n177_));
  nand2  g115(.a(new_n143_), .b(new_n72_), .O(new_n178_));
  oai21  g116(.a(x13), .b(x04), .c(new_n178_), .O(new_n179_));
  oai21  g117(.a(new_n73_), .b(new_n64_), .c(new_n94_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n179_), .c(new_n177_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n120_), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n174_), .c(new_n151_), .O(z14));
  nand4  g121(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n78_), .O(z15));
  inv1   g123(.a(x23), .O(new_n186_));
  nand4  g124(.a(new_n78_), .b(new_n186_), .c(x22), .d(x01), .O(new_n187_));
  inv1   g125(.a(new_n187_), .O(z16));
  inv1   g126(.a(x24), .O(new_n189_));
  nand4  g127(.a(new_n189_), .b(x23), .c(x22), .d(x01), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n78_), .O(z17));
  nand3  g129(.a(x40), .b(x39), .c(x29), .O(new_n192_));
  oai21  g130(.a(new_n83_), .b(x27), .c(new_n192_), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(x08), .O(new_n194_));
  nor2   g132(.a(new_n64_), .b(x04), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n82_), .c(x30), .O(new_n196_));
  nand4  g134(.a(new_n196_), .b(new_n194_), .c(new_n78_), .d(new_n129_), .O(z18));
  nand2  g135(.a(new_n140_), .b(new_n78_), .O(z19));
  aoi21  g136(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n199_));
  aoi21  g137(.a(x35), .b(x27), .c(x05), .O(new_n200_));
  nor2   g138(.a(x40), .b(x27), .O(new_n201_));
  nor2   g139(.a(new_n201_), .b(new_n200_), .O(new_n202_));
  oai22  g140(.a(new_n202_), .b(x08), .c(new_n199_), .d(new_n135_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n80_), .c(new_n129_), .O(new_n204_));
  inv1   g142(.a(new_n204_), .O(z20));
endmodule


