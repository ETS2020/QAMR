// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_;
  nand2  g000(.a(x40), .b(x15), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g008(.a(new_n70_), .b(x40), .c(x39), .d(new_n67_), .O(new_n71_));
  aoi21  g009(.a(x35), .b(x28), .c(x37), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n71_), .c(new_n66_), .O(new_n73_));
  inv1   g011(.a(x39), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n67_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n73_), .c(new_n65_), .O(new_n79_));
  oai21  g017(.a(new_n64_), .b(x16), .c(new_n79_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  aoi21  g021(.a(x35), .b(new_n83_), .c(x36), .O(new_n84_));
  or2    g022(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n82_), .c(new_n81_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(x35), .b(x15), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(x28), .O(new_n92_));
  oai21  g030(.a(x27), .b(x08), .c(new_n92_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x04), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(x28), .c(x04), .O(new_n96_));
  nand2  g034(.a(new_n63_), .b(x39), .O(new_n97_));
  aoi21  g035(.a(new_n96_), .b(new_n75_), .c(new_n97_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n94_), .c(new_n90_), .O(z02));
  nand2  g037(.a(x35), .b(x28), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x27), .O(new_n101_));
  aoi21  g039(.a(new_n95_), .b(new_n66_), .c(new_n64_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n101_), .c(x21), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  nand3  g042(.a(new_n102_), .b(new_n101_), .c(new_n104_), .O(z04));
  nand2  g043(.a(x28), .b(x27), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n95_), .c(new_n64_), .O(z05));
  inv1   g045(.a(z05), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n65_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n63_), .O(z07));
  oai21  g057(.a(x39), .b(x15), .c(x40), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  nand2  g062(.a(new_n120_), .b(x07), .O(new_n125_));
  nand3  g063(.a(new_n76_), .b(new_n65_), .c(x05), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n84_), .O(new_n127_));
  nand3  g065(.a(new_n63_), .b(x37), .c(x06), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n127_), .b(new_n67_), .c(new_n129_), .O(new_n130_));
  oai21  g068(.a(x32), .b(x30), .c(x40), .O(new_n131_));
  nor3   g069(.a(new_n131_), .b(new_n74_), .c(x15), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x05), .c(new_n67_), .O(new_n133_));
  oai21  g071(.a(new_n130_), .b(new_n66_), .c(new_n133_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  nand3  g073(.a(x35), .b(new_n83_), .c(new_n67_), .O(new_n136_));
  aoi22  g074(.a(new_n136_), .b(new_n65_), .c(new_n63_), .d(new_n66_), .O(new_n137_));
  nand2  g075(.a(x35), .b(new_n83_), .O(new_n138_));
  aoi21  g076(.a(x39), .b(x29), .c(x15), .O(new_n139_));
  nor2   g077(.a(new_n66_), .b(new_n67_), .O(new_n140_));
  oai22  g078(.a(new_n140_), .b(new_n138_), .c(new_n139_), .d(new_n75_), .O(new_n141_));
  oai21  g079(.a(new_n137_), .b(x08), .c(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n81_), .c(new_n135_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n63_), .O(z11));
  nor2   g082(.a(new_n72_), .b(new_n66_), .O(new_n145_));
  nand3  g083(.a(new_n95_), .b(new_n69_), .c(new_n68_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x27), .c(x10), .O(new_n147_));
  aoi21  g085(.a(x39), .b(new_n67_), .c(new_n145_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n147_), .c(new_n65_), .O(new_n149_));
  oai21  g087(.a(new_n145_), .b(x40), .c(new_n149_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n75_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n67_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  inv1   g094(.a(new_n72_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n154_), .c(x15), .O(new_n159_));
  nand4  g097(.a(new_n157_), .b(new_n75_), .c(x20), .d(new_n156_), .O(new_n160_));
  nor2   g098(.a(new_n160_), .b(x18), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n159_), .c(x27), .O(new_n162_));
  nand3  g100(.a(new_n132_), .b(new_n151_), .c(new_n67_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n162_), .O(z13));
  nand3  g102(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n165_));
  nand4  g103(.a(new_n152_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n76_), .c(new_n151_), .d(new_n67_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n165_), .O(new_n168_));
  nand3  g106(.a(new_n95_), .b(new_n68_), .c(x28), .O(new_n169_));
  oai21  g107(.a(x27), .b(x15), .c(new_n169_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n82_), .c(new_n81_), .O(new_n171_));
  oai21  g109(.a(new_n74_), .b(x04), .c(new_n65_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(x40), .c(new_n151_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n100_), .c(new_n95_), .O(new_n174_));
  nand3  g112(.a(x39), .b(new_n151_), .c(new_n67_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n65_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x40), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n66_), .c(new_n64_), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n174_), .c(new_n171_), .d(new_n168_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nor2   g118(.a(new_n123_), .b(new_n180_), .O(z15));
  inv1   g119(.a(x23), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(x22), .c(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n63_), .O(z16));
  inv1   g122(.a(x01), .O(new_n185_));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n63_), .b(new_n186_), .c(x23), .d(x22), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(new_n185_), .O(z17));
  nand2  g126(.a(x27), .b(new_n67_), .O(new_n189_));
  nand2  g127(.a(new_n66_), .b(x08), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n189_), .c(new_n68_), .O(new_n191_));
  nand2  g129(.a(new_n81_), .b(new_n135_), .O(new_n192_));
  aoi21  g130(.a(new_n191_), .b(new_n83_), .c(new_n192_), .O(new_n193_));
  nand4  g131(.a(new_n76_), .b(x29), .c(new_n65_), .d(x08), .O(new_n194_));
  oai21  g132(.a(new_n193_), .b(new_n64_), .c(new_n194_), .O(z18));
  inv1   g133(.a(new_n143_), .O(z19));
  nand2  g134(.a(x39), .b(x29), .O(new_n197_));
  nor2   g135(.a(new_n83_), .b(x27), .O(new_n198_));
  oai21  g136(.a(new_n198_), .b(new_n91_), .c(new_n197_), .O(new_n199_));
  inv1   g137(.a(x08), .O(new_n200_));
  aoi21  g138(.a(x35), .b(x27), .c(x15), .O(new_n201_));
  nor2   g139(.a(x40), .b(x27), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n201_), .c(new_n200_), .O(new_n203_));
  oai21  g141(.a(new_n198_), .b(new_n68_), .c(new_n75_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n203_), .c(new_n199_), .O(new_n205_));
  nand3  g143(.a(new_n205_), .b(new_n81_), .c(new_n135_), .O(new_n206_));
  nand2  g144(.a(new_n206_), .b(new_n63_), .O(z20));
endmodule


