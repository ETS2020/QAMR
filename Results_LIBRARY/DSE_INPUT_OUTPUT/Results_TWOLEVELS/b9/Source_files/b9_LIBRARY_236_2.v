// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_;
  inv1   g000(.a(x39), .O(new_n63_));
  nor2   g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n66_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x16), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n65_), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  oai21  g014(.a(x36), .b(x35), .c(x27), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n76_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(x39), .c(new_n66_), .d(new_n74_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n73_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x36), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n82_), .c(new_n81_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n65_), .c(x04), .O(z01));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(x35), .b(new_n74_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(x40), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(x28), .c(x04), .O(new_n94_));
  inv1   g032(.a(x09), .O(new_n95_));
  aoi21  g033(.a(x28), .b(new_n95_), .c(x30), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(x28), .c(x27), .d(new_n95_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n63_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n94_), .c(new_n92_), .d(new_n65_), .O(z02));
  nand2  g037(.a(new_n69_), .b(x27), .O(new_n100_));
  nand2  g038(.a(new_n68_), .b(new_n67_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n100_), .c(new_n65_), .d(x21), .O(z03));
  aoi21  g040(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n100_), .c(new_n64_), .O(z04));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n65_), .c(new_n68_), .O(z05));
  inv1   g044(.a(z05), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n66_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n65_), .O(z07));
  nor2   g056(.a(new_n76_), .b(new_n63_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  inv1   g060(.a(x05), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  nand2  g062(.a(x39), .b(x05), .O(new_n125_));
  oai21  g063(.a(x39), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n86_), .c(x27), .O(new_n127_));
  oai21  g065(.a(x32), .b(x30), .c(x39), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n123_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n74_), .O(new_n130_));
  nor2   g068(.a(new_n68_), .b(new_n67_), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(x06), .c(new_n64_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(z10));
  inv1   g071(.a(x29), .O(new_n134_));
  nor2   g072(.a(new_n67_), .b(new_n74_), .O(new_n135_));
  oai22  g073(.a(new_n135_), .b(new_n85_), .c(new_n63_), .d(new_n134_), .O(new_n136_));
  inv1   g074(.a(x08), .O(new_n137_));
  nand2  g075(.a(x27), .b(new_n74_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n85_), .c(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n81_), .c(new_n95_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n65_), .O(z11));
  inv1   g080(.a(x35), .O(new_n143_));
  nand3  g081(.a(new_n68_), .b(new_n83_), .c(new_n143_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  nor2   g083(.a(new_n70_), .b(new_n74_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(x40), .O(new_n147_));
  oai21  g085(.a(new_n70_), .b(x39), .c(new_n147_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n149_), .d(new_n74_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand2  g092(.a(new_n69_), .b(new_n68_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n152_), .c(new_n76_), .O(new_n157_));
  nand4  g095(.a(new_n155_), .b(new_n63_), .c(x20), .d(new_n154_), .O(new_n158_));
  nor2   g096(.a(new_n158_), .b(x18), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n157_), .c(x27), .O(new_n160_));
  aoi21  g098(.a(new_n82_), .b(new_n81_), .c(new_n76_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n149_), .d(new_n74_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n160_), .O(z13));
  nand2  g101(.a(x40), .b(x04), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(x39), .c(new_n149_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n69_), .O(new_n166_));
  nand4  g104(.a(new_n143_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n68_), .O(new_n169_));
  nand2  g107(.a(x40), .b(new_n67_), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n171_));
  oai21  g109(.a(x13), .b(x04), .c(new_n171_), .O(new_n172_));
  nand2  g110(.a(new_n128_), .b(new_n67_), .O(new_n173_));
  nand4  g111(.a(new_n150_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x39), .O(new_n175_));
  nand3  g113(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n175_), .c(new_n64_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n173_), .c(new_n172_), .d(new_n169_), .O(z14));
  inv1   g116(.a(x12), .O(new_n179_));
  nor2   g117(.a(new_n121_), .b(new_n179_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n65_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n65_), .O(z17));
  nand2  g124(.a(new_n67_), .b(x08), .O(new_n187_));
  aoi21  g125(.a(new_n187_), .b(new_n138_), .c(new_n143_), .O(new_n188_));
  nand2  g126(.a(new_n81_), .b(new_n95_), .O(new_n189_));
  aoi21  g127(.a(new_n188_), .b(new_n84_), .c(new_n189_), .O(new_n190_));
  nand3  g128(.a(z08), .b(x29), .c(x08), .O(new_n191_));
  oai21  g129(.a(new_n190_), .b(new_n64_), .c(new_n191_), .O(z18));
  nor2   g130(.a(new_n63_), .b(new_n134_), .O(new_n193_));
  oai21  g131(.a(new_n143_), .b(new_n67_), .c(new_n137_), .O(new_n194_));
  aoi21  g132(.a(x28), .b(new_n67_), .c(new_n143_), .O(new_n195_));
  oai21  g133(.a(new_n195_), .b(new_n193_), .c(new_n194_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n81_), .c(new_n95_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n65_), .O(z20));
  nand2  g136(.a(new_n141_), .b(new_n65_), .O(z19));
endmodule


