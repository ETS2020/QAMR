// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x39), .O(new_n67_));
  nor3   g005(.a(new_n67_), .b(new_n66_), .c(x04), .O(new_n68_));
  aoi21  g006(.a(new_n65_), .b(x27), .c(new_n68_), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  nor2   g008(.a(x40), .b(new_n67_), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x30), .O(new_n74_));
  inv1   g012(.a(x32), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(new_n74_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n71_), .O(z01));
  inv1   g020(.a(x40), .O(new_n83_));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  inv1   g023(.a(x08), .O(new_n86_));
  inv1   g024(.a(x27), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(x35), .c(new_n77_), .O(new_n89_));
  aoi22  g027(.a(new_n89_), .b(x04), .c(new_n85_), .d(new_n84_), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n83_), .c(x39), .O(z02));
  inv1   g029(.a(x21), .O(new_n92_));
  inv1   g030(.a(x35), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x27), .c(new_n92_), .O(new_n94_));
  nand2  g032(.a(new_n83_), .b(x39), .O(new_n95_));
  nor2   g033(.a(x28), .b(new_n87_), .O(new_n96_));
  nor2   g034(.a(x37), .b(x27), .O(new_n97_));
  oai21  g035(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  oai21  g036(.a(new_n94_), .b(new_n71_), .c(new_n98_), .O(z03));
  inv1   g037(.a(new_n71_), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n93_), .c(x27), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n98_), .c(new_n100_), .d(new_n92_), .O(z04));
  inv1   g040(.a(x37), .O(new_n103_));
  nand2  g041(.a(x40), .b(x39), .O(new_n104_));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  aoi22  g043(.a(new_n105_), .b(new_n103_), .c(new_n104_), .d(x39), .O(z05));
  aoi21  g044(.a(new_n95_), .b(new_n87_), .c(new_n77_), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(x37), .c(new_n100_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  inv1   g047(.a(x15), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n110_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n100_), .O(z07));
  nand2  g058(.a(new_n103_), .b(x27), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(x27), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(x40), .c(new_n67_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n100_), .O(z09));
  inv1   g063(.a(x04), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  inv1   g065(.a(x07), .O(new_n128_));
  oai22  g066(.a(new_n104_), .b(new_n127_), .c(x39), .d(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n79_), .c(new_n126_), .O(new_n130_));
  nand3  g068(.a(new_n100_), .b(x37), .c(x06), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x27), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n67_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x05), .c(new_n126_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  inv1   g076(.a(x29), .O(new_n139_));
  nor2   g077(.a(new_n87_), .b(new_n126_), .O(new_n140_));
  oai22  g078(.a(new_n140_), .b(new_n78_), .c(new_n67_), .d(new_n139_), .O(new_n141_));
  nand2  g079(.a(x27), .b(new_n126_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n78_), .c(new_n86_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n74_), .c(new_n138_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n100_), .O(z11));
  nor3   g084(.a(x37), .b(x36), .c(x35), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n87_), .c(new_n66_), .O(new_n148_));
  oai22  g086(.a(new_n64_), .b(new_n87_), .c(new_n67_), .d(x04), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n100_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x28), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n126_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand2  g094(.a(x35), .b(x28), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n103_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n154_), .c(new_n83_), .O(new_n160_));
  nand4  g098(.a(new_n158_), .b(new_n67_), .c(x20), .d(new_n156_), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(x18), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n160_), .c(x27), .O(new_n163_));
  nand3  g101(.a(new_n135_), .b(new_n151_), .c(new_n126_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(z13));
  nand3  g103(.a(x39), .b(new_n151_), .c(new_n126_), .O(new_n166_));
  nand4  g104(.a(x27), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n64_), .c(new_n166_), .O(new_n168_));
  nand3  g106(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n169_));
  nor2   g107(.a(x37), .b(x35), .O(new_n170_));
  aoi21  g108(.a(new_n169_), .b(new_n152_), .c(new_n170_), .O(new_n171_));
  nand2  g109(.a(x40), .b(new_n87_), .O(new_n172_));
  oai21  g110(.a(new_n171_), .b(new_n77_), .c(new_n172_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n75_), .c(new_n74_), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n168_), .c(new_n100_), .O(z14));
  nand4  g113(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n100_), .O(z15));
  inv1   g115(.a(x23), .O(new_n178_));
  nand4  g116(.a(new_n100_), .b(new_n178_), .c(x22), .d(x01), .O(new_n179_));
  inv1   g117(.a(new_n179_), .O(z16));
  inv1   g118(.a(x01), .O(new_n181_));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n100_), .b(new_n182_), .c(x23), .d(x22), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n181_), .O(z17));
  nand2  g122(.a(new_n87_), .b(x08), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n142_), .c(new_n93_), .O(new_n186_));
  nand2  g124(.a(new_n74_), .b(new_n138_), .O(new_n187_));
  aoi21  g125(.a(new_n186_), .b(new_n77_), .c(new_n187_), .O(new_n188_));
  oai22  g126(.a(new_n188_), .b(new_n71_), .c(new_n104_), .d(new_n85_), .O(z18));
  nor2   g127(.a(new_n67_), .b(new_n139_), .O(new_n190_));
  oai21  g128(.a(new_n93_), .b(new_n87_), .c(new_n86_), .O(new_n191_));
  aoi21  g129(.a(x28), .b(new_n87_), .c(new_n93_), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n190_), .c(new_n191_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n74_), .c(new_n138_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n100_), .O(z20));
  nand2  g133(.a(new_n145_), .b(new_n100_), .O(z19));
endmodule


