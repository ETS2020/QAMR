// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x21), .O(new_n78_));
  nand2  g016(.a(x40), .b(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(new_n79_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n83_), .c(new_n82_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n81_), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n85_), .c(x04), .O(new_n94_));
  aoi21  g032(.a(new_n72_), .b(x21), .c(new_n81_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n92_), .d(x40), .O(z02));
  nand2  g034(.a(x35), .b(x28), .O(new_n97_));
  inv1   g035(.a(new_n97_), .O(new_n98_));
  inv1   g036(.a(x37), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n64_), .c(new_n78_), .O(new_n100_));
  oai21  g038(.a(new_n98_), .b(new_n64_), .c(new_n100_), .O(z03));
  nor2   g039(.a(new_n98_), .b(x40), .O(new_n102_));
  nor2   g040(.a(new_n84_), .b(new_n78_), .O(new_n103_));
  oai21  g041(.a(new_n103_), .b(new_n102_), .c(x27), .O(new_n104_));
  nand3  g042(.a(new_n73_), .b(x39), .c(new_n99_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x21), .O(new_n106_));
  nand3  g044(.a(new_n73_), .b(new_n99_), .c(new_n64_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(z04));
  nand2  g046(.a(x28), .b(x27), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n99_), .c(new_n81_), .O(z05));
  nand3  g048(.a(new_n109_), .b(new_n79_), .c(new_n99_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z06));
  inv1   g050(.a(x33), .O(new_n113_));
  nand2  g051(.a(x17), .b(new_n63_), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  oai21  g053(.a(x25), .b(new_n115_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n115_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  oai22  g058(.a(new_n120_), .b(x14), .c(new_n117_), .d(x31), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x03), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n79_), .O(z07));
  nand3  g061(.a(x40), .b(x39), .c(x21), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(z08));
  nand4  g063(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n79_), .O(z09));
  inv1   g065(.a(x07), .O(new_n128_));
  aoi21  g066(.a(new_n72_), .b(x21), .c(new_n73_), .O(new_n129_));
  nand4  g067(.a(x40), .b(x39), .c(x21), .d(x05), .O(new_n130_));
  oai21  g068(.a(new_n129_), .b(new_n128_), .c(new_n130_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n86_), .c(new_n65_), .O(new_n132_));
  nand3  g070(.a(new_n79_), .b(x37), .c(x06), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x27), .O(new_n135_));
  oai21  g073(.a(x32), .b(x30), .c(x40), .O(new_n136_));
  nor3   g074(.a(new_n136_), .b(new_n72_), .c(new_n78_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x05), .c(new_n65_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n135_), .O(z10));
  nand2  g077(.a(new_n79_), .b(new_n64_), .O(new_n140_));
  oai21  g078(.a(new_n85_), .b(x04), .c(x21), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n140_), .c(x08), .O(new_n142_));
  inv1   g080(.a(new_n85_), .O(new_n143_));
  nand2  g081(.a(x39), .b(x29), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x21), .c(new_n73_), .O(new_n145_));
  nand2  g083(.a(x27), .b(x04), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n143_), .c(new_n145_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n142_), .c(new_n82_), .O(new_n148_));
  nor2   g086(.a(new_n148_), .b(x09), .O(z11));
  nand3  g087(.a(new_n99_), .b(new_n67_), .c(new_n66_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x27), .c(x10), .O(new_n151_));
  nor2   g089(.a(new_n70_), .b(new_n64_), .O(new_n152_));
  aoi21  g090(.a(x39), .b(new_n65_), .c(new_n152_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n151_), .c(x21), .O(new_n154_));
  oai21  g092(.a(new_n70_), .b(new_n64_), .c(new_n73_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x28), .c(new_n73_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  inv1   g100(.a(new_n70_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n160_), .c(new_n78_), .O(new_n165_));
  nand4  g103(.a(new_n163_), .b(new_n73_), .c(x20), .d(new_n162_), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x18), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n165_), .c(x27), .O(new_n168_));
  nand3  g106(.a(new_n137_), .b(new_n157_), .c(new_n65_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n168_), .O(z13));
  nand3  g108(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n171_));
  nand4  g109(.a(new_n158_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n172_));
  nand3  g110(.a(x39), .b(new_n157_), .c(new_n65_), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n172_), .c(new_n78_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n73_), .c(new_n171_), .O(new_n176_));
  nand3  g114(.a(new_n99_), .b(new_n66_), .c(x28), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x27), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n83_), .c(new_n82_), .O(new_n179_));
  oai21  g117(.a(new_n174_), .b(new_n152_), .c(new_n179_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x21), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n176_), .c(new_n155_), .O(z14));
  nand4  g120(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n79_), .O(z15));
  inv1   g122(.a(x23), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(x22), .c(x01), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n79_), .O(z16));
  inv1   g125(.a(x01), .O(new_n188_));
  inv1   g126(.a(x24), .O(new_n189_));
  nand4  g127(.a(new_n79_), .b(new_n189_), .c(x23), .d(x22), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n188_), .O(z17));
  nand2  g129(.a(x27), .b(new_n65_), .O(new_n192_));
  nand2  g130(.a(new_n64_), .b(x08), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(new_n192_), .c(new_n66_), .O(new_n194_));
  inv1   g132(.a(x09), .O(new_n195_));
  nand2  g133(.a(new_n82_), .b(new_n195_), .O(new_n196_));
  aoi21  g134(.a(new_n194_), .b(new_n84_), .c(new_n196_), .O(new_n197_));
  nand4  g135(.a(new_n74_), .b(x29), .c(x21), .d(x08), .O(new_n198_));
  oai21  g136(.a(new_n197_), .b(new_n81_), .c(new_n198_), .O(z18));
  nor2   g137(.a(new_n84_), .b(x27), .O(new_n200_));
  nor2   g138(.a(x35), .b(new_n78_), .O(new_n201_));
  oai21  g139(.a(new_n201_), .b(new_n200_), .c(new_n144_), .O(new_n202_));
  inv1   g140(.a(x08), .O(new_n203_));
  aoi21  g141(.a(x35), .b(x27), .c(new_n78_), .O(new_n204_));
  nor2   g142(.a(x40), .b(x27), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n204_), .c(new_n203_), .O(new_n206_));
  oai21  g144(.a(new_n200_), .b(new_n66_), .c(new_n73_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n206_), .c(new_n202_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n82_), .c(new_n195_), .O(new_n209_));
  nand2  g147(.a(new_n209_), .b(new_n79_), .O(z20));
  nor2   g148(.a(new_n148_), .b(x09), .O(z19));
endmodule


