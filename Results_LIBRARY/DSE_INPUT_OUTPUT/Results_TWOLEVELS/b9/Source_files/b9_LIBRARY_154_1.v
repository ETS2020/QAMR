// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n198_, new_n199_, new_n200_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x08), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n63_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x39), .c(new_n65_), .d(x08), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  inv1   g016(.a(x35), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(x28), .c(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n78_), .c(new_n77_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  oai21  g024(.a(x35), .b(new_n85_), .c(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(new_n88_));
  nand3  g026(.a(x28), .b(x27), .c(x04), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n63_), .O(new_n90_));
  inv1   g028(.a(x02), .O(new_n91_));
  inv1   g029(.a(x29), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(x08), .c(new_n91_), .O(new_n93_));
  inv1   g031(.a(x39), .O(new_n94_));
  nor2   g032(.a(new_n64_), .b(new_n94_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n93_), .c(new_n90_), .d(new_n88_), .O(z02));
  inv1   g034(.a(new_n64_), .O(new_n97_));
  nand2  g035(.a(new_n69_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n68_), .b(new_n67_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n64_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n97_), .c(new_n68_), .O(z05));
  aoi21  g042(.a(new_n97_), .b(new_n67_), .c(new_n86_), .O(new_n105_));
  oai21  g043(.a(new_n105_), .b(x37), .c(new_n97_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n65_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n97_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  nand3  g055(.a(x40), .b(x39), .c(x08), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n97_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  inv1   g060(.a(x04), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  aoi21  g062(.a(new_n94_), .b(x08), .c(new_n63_), .O(new_n125_));
  nand4  g063(.a(x40), .b(x39), .c(x08), .d(x05), .O(new_n126_));
  oai21  g064(.a(new_n125_), .b(new_n124_), .c(new_n126_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n81_), .c(new_n123_), .O(new_n128_));
  nand3  g066(.a(new_n97_), .b(x37), .c(x06), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n78_), .b(new_n77_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x40), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n94_), .O(new_n134_));
  nand4  g072(.a(new_n134_), .b(x08), .c(x05), .d(new_n123_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n131_), .O(z10));
  nor2   g074(.a(x27), .b(x09), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n77_), .c(x40), .O(new_n138_));
  inv1   g076(.a(x09), .O(new_n139_));
  nand2  g077(.a(x39), .b(x29), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(x08), .c(new_n63_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(new_n142_));
  nand2  g080(.a(x27), .b(x04), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(x35), .c(new_n86_), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(new_n142_), .c(new_n77_), .d(new_n139_), .O(new_n145_));
  oai21  g083(.a(new_n138_), .b(x08), .c(new_n145_), .O(z11));
  nand3  g084(.a(new_n68_), .b(new_n80_), .c(new_n79_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  aoi21  g086(.a(x39), .b(new_n123_), .c(new_n70_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(x08), .O(new_n150_));
  oai21  g088(.a(new_n70_), .b(x40), .c(new_n150_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n63_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n123_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  nand2  g095(.a(new_n69_), .b(new_n68_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n155_), .c(new_n67_), .O(new_n160_));
  nand4  g098(.a(new_n132_), .b(x39), .c(new_n152_), .d(new_n123_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(x08), .c(new_n63_), .O(new_n162_));
  or2    g100(.a(new_n162_), .b(new_n160_), .O(z13));
  nand3  g101(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n164_));
  nor2   g102(.a(new_n63_), .b(new_n94_), .O(new_n165_));
  nand4  g103(.a(new_n153_), .b(new_n78_), .c(new_n77_), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n165_), .c(new_n152_), .d(new_n123_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand2  g106(.a(new_n67_), .b(x08), .O(new_n169_));
  nand3  g107(.a(new_n68_), .b(new_n79_), .c(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n78_), .c(new_n77_), .O(new_n172_));
  oai21  g110(.a(new_n94_), .b(x04), .c(x08), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(x40), .c(new_n152_), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n69_), .c(new_n68_), .O(new_n175_));
  nand3  g113(.a(x39), .b(new_n152_), .c(new_n123_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x08), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x40), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n67_), .c(new_n64_), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(new_n175_), .c(new_n172_), .d(new_n168_), .O(z14));
  inv1   g118(.a(x12), .O(new_n181_));
  nor2   g119(.a(new_n121_), .b(new_n181_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(x22), .c(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n97_), .O(z16));
  inv1   g123(.a(x01), .O(new_n186_));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n97_), .b(new_n187_), .c(x23), .d(x22), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n186_), .O(z17));
  oai21  g127(.a(new_n67_), .b(x04), .c(new_n169_), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x35), .c(new_n86_), .O(new_n191_));
  nand4  g129(.a(new_n191_), .b(new_n142_), .c(new_n77_), .d(new_n139_), .O(z18));
  inv1   g130(.a(new_n144_), .O(new_n193_));
  nand3  g131(.a(new_n63_), .b(new_n67_), .c(new_n85_), .O(new_n194_));
  oai21  g132(.a(new_n193_), .b(new_n141_), .c(new_n194_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n77_), .c(new_n139_), .O(new_n196_));
  inv1   g134(.a(new_n196_), .O(z19));
  aoi21  g135(.a(x28), .b(new_n67_), .c(new_n79_), .O(new_n198_));
  oai21  g136(.a(new_n198_), .b(new_n141_), .c(new_n194_), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(new_n77_), .c(new_n139_), .O(new_n200_));
  inv1   g138(.a(new_n200_), .O(z20));
endmodule


