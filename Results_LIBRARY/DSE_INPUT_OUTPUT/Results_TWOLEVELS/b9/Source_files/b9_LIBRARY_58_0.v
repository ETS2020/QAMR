// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_;
  nor2   g000(.a(x40), .b(x27), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x27), .O(new_n65_));
  inv1   g003(.a(x04), .O(new_n66_));
  inv1   g004(.a(x35), .O(new_n67_));
  inv1   g005(.a(x36), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g007(.a(new_n69_), .b(x40), .c(x39), .d(new_n66_), .O(new_n70_));
  inv1   g008(.a(x37), .O(new_n71_));
  nand2  g009(.a(x35), .b(x28), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n70_), .c(new_n65_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n66_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n75_), .c(new_n64_), .O(new_n80_));
  oai21  g018(.a(new_n63_), .b(x16), .c(new_n80_), .O(z00));
  oai21  g019(.a(x32), .b(x30), .c(x40), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n65_), .O(new_n83_));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nand4  g025(.a(new_n87_), .b(new_n68_), .c(new_n85_), .d(new_n84_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n83_), .c(x04), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  inv1   g029(.a(x29), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  nor2   g031(.a(x27), .b(x08), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n87_), .c(x04), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n93_), .c(new_n77_), .O(z02));
  aoi21  g034(.a(x28), .b(x27), .c(x40), .O(new_n97_));
  nand3  g035(.a(x40), .b(new_n71_), .c(new_n65_), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(new_n99_));
  aoi21  g037(.a(new_n72_), .b(x27), .c(new_n99_), .O(new_n100_));
  oai21  g038(.a(new_n97_), .b(x21), .c(new_n100_), .O(z03));
  inv1   g039(.a(x21), .O(new_n102_));
  oai21  g040(.a(new_n97_), .b(new_n102_), .c(new_n100_), .O(z04));
  inv1   g041(.a(new_n63_), .O(new_n104_));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n104_), .c(new_n71_), .O(z05));
  inv1   g044(.a(x40), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n71_), .c(new_n65_), .O(new_n108_));
  oai21  g046(.a(x37), .b(x28), .c(new_n108_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n64_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n104_), .c(x03), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z07));
  nand2  g058(.a(new_n76_), .b(new_n104_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z09));
  nand2  g061(.a(new_n87_), .b(new_n68_), .O(new_n124_));
  inv1   g062(.a(x05), .O(new_n125_));
  nand2  g063(.a(new_n76_), .b(x07), .O(new_n126_));
  oai21  g064(.a(new_n76_), .b(new_n125_), .c(new_n126_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n124_), .c(x27), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  inv1   g067(.a(x39), .O(new_n130_));
  nor3   g068(.a(new_n82_), .b(new_n130_), .c(new_n125_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n129_), .c(new_n66_), .O(new_n132_));
  nand3  g070(.a(x37), .b(x27), .c(x06), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  nor2   g073(.a(new_n130_), .b(new_n92_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(new_n137_));
  oai21  g075(.a(x28), .b(x04), .c(x27), .O(new_n138_));
  aoi21  g076(.a(x28), .b(new_n65_), .c(new_n67_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n107_), .c(new_n138_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g079(.a(x40), .b(x08), .O(new_n142_));
  nand3  g080(.a(x35), .b(new_n86_), .c(new_n66_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n142_), .c(x27), .O(new_n144_));
  nand3  g082(.a(x40), .b(new_n65_), .c(new_n91_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n141_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n84_), .c(new_n135_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(z11));
  nor2   g088(.a(x37), .b(x36), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n67_), .c(new_n65_), .O(new_n152_));
  nand2  g090(.a(new_n73_), .b(x27), .O(new_n153_));
  oai21  g091(.a(new_n76_), .b(x04), .c(new_n153_), .O(new_n154_));
  oai21  g092(.a(new_n152_), .b(x10), .c(new_n154_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n107_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n66_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n73_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nor3   g102(.a(new_n82_), .b(new_n130_), .c(x13), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n66_), .c(new_n63_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n164_), .O(z13));
  nand3  g105(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n168_));
  nand4  g106(.a(new_n157_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  nand3  g108(.a(new_n77_), .b(new_n156_), .c(new_n66_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n170_), .c(new_n168_), .O(new_n172_));
  nand3  g110(.a(new_n71_), .b(new_n67_), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n85_), .c(new_n84_), .O(new_n175_));
  nand2  g113(.a(new_n171_), .b(new_n153_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n175_), .c(new_n172_), .O(z14));
  nand4  g115(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(x22), .c(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n104_), .O(z16));
  inv1   g120(.a(x01), .O(new_n183_));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n104_), .b(new_n184_), .c(x23), .d(x22), .O(new_n185_));
  nor2   g123(.a(new_n185_), .b(new_n183_), .O(z17));
  oai22  g124(.a(new_n87_), .b(x27), .c(new_n76_), .d(new_n92_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(x08), .O(new_n188_));
  inv1   g126(.a(new_n87_), .O(new_n189_));
  nor2   g127(.a(new_n65_), .b(x04), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n189_), .c(x09), .O(new_n191_));
  nand4  g129(.a(new_n191_), .b(new_n188_), .c(new_n104_), .d(new_n84_), .O(z18));
  nand2  g130(.a(new_n149_), .b(new_n104_), .O(z19));
  nor2   g131(.a(new_n107_), .b(x27), .O(new_n194_));
  nor2   g132(.a(x35), .b(new_n65_), .O(new_n195_));
  oai21  g133(.a(new_n195_), .b(new_n194_), .c(new_n91_), .O(new_n196_));
  nor2   g134(.a(new_n139_), .b(new_n136_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(x40), .O(new_n198_));
  nand3  g136(.a(new_n107_), .b(new_n67_), .c(x27), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(new_n198_), .c(new_n196_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n84_), .c(new_n135_), .O(new_n201_));
  inv1   g139(.a(new_n201_), .O(z20));
endmodule


