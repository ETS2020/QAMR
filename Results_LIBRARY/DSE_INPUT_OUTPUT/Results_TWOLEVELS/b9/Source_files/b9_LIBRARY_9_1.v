// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n184_,
    new_n186_, new_n187_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_;
  inv1   g000(.a(x24), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n67_), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(x24), .d(new_n66_), .O(new_n78_));
  oai22  g016(.a(new_n78_), .b(x04), .c(new_n73_), .d(new_n65_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  inv1   g028(.a(x29), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  nand2  g030(.a(new_n84_), .b(x04), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n92_), .c(new_n75_), .O(new_n94_));
  inv1   g032(.a(x27), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n90_), .c(x04), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x39), .O(new_n97_));
  oai21  g035(.a(new_n97_), .b(new_n94_), .c(x24), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x40), .O(z02));
  nand2  g037(.a(new_n69_), .b(x27), .O(new_n100_));
  inv1   g038(.a(x21), .O(new_n101_));
  aoi21  g039(.a(new_n68_), .b(new_n95_), .c(new_n101_), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n100_), .c(new_n65_), .O(z03));
  nand2  g041(.a(new_n68_), .b(new_n95_), .O(new_n104_));
  nand4  g042(.a(new_n104_), .b(new_n100_), .c(new_n64_), .d(new_n101_), .O(z04));
  nand2  g043(.a(x28), .b(x27), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n64_), .c(new_n68_), .O(z05));
  aoi21  g045(.a(new_n64_), .b(new_n95_), .c(new_n83_), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n66_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n64_), .O(z07));
  nand3  g058(.a(x40), .b(x39), .c(x24), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z08));
  nand4  g060(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n64_), .O(z09));
  inv1   g062(.a(x04), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  nand2  g064(.a(x40), .b(x39), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x07), .O(new_n128_));
  oai21  g066(.a(new_n127_), .b(new_n126_), .c(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n85_), .c(x27), .O(new_n130_));
  nand2  g068(.a(new_n81_), .b(new_n80_), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(x40), .c(x39), .d(x05), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n125_), .O(new_n134_));
  nor2   g072(.a(new_n68_), .b(new_n95_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x06), .c(new_n65_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand3  g076(.a(x35), .b(new_n83_), .c(new_n125_), .O(new_n139_));
  aoi22  g077(.a(new_n139_), .b(x24), .c(new_n64_), .d(new_n95_), .O(new_n140_));
  nand2  g078(.a(x39), .b(x29), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x24), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x40), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(x35), .c(new_n83_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  oai21  g084(.a(new_n140_), .b(x08), .c(new_n146_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n80_), .c(new_n138_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(z11));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  nor2   g088(.a(new_n75_), .b(x27), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n74_), .O(new_n152_));
  oai21  g090(.a(new_n127_), .b(x04), .c(new_n71_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n152_), .c(new_n64_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x28), .c(new_n75_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n125_), .O(new_n158_));
  inv1   g096(.a(x18), .O(new_n159_));
  inv1   g097(.a(x19), .O(new_n160_));
  nand4  g098(.a(new_n70_), .b(x20), .c(new_n160_), .d(new_n159_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nand4  g101(.a(new_n131_), .b(x39), .c(new_n155_), .d(new_n125_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x24), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x40), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n163_), .O(z13));
  oai21  g105(.a(x27), .b(new_n155_), .c(x24), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x40), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n160_), .c(new_n159_), .O(new_n170_));
  inv1   g108(.a(new_n127_), .O(new_n171_));
  nand4  g109(.a(new_n156_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n172_));
  nor2   g110(.a(x13), .b(x04), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n172_), .c(new_n171_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n170_), .O(new_n175_));
  aoi22  g113(.a(new_n173_), .b(new_n171_), .c(x35), .d(x28), .O(new_n176_));
  inv1   g114(.a(x35), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n176_), .c(new_n68_), .O(new_n180_));
  nand3  g118(.a(new_n131_), .b(new_n171_), .c(new_n125_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n95_), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(new_n180_), .c(new_n175_), .d(new_n169_), .O(z14));
  nand4  g121(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n64_), .O(z15));
  inv1   g123(.a(x23), .O(new_n186_));
  nand4  g124(.a(new_n64_), .b(new_n186_), .c(x22), .d(x01), .O(new_n187_));
  inv1   g125(.a(new_n187_), .O(z16));
  nand3  g126(.a(x23), .b(x22), .c(x01), .O(new_n189_));
  nor3   g127(.a(new_n189_), .b(x40), .c(x24), .O(z17));
  oai22  g128(.a(new_n127_), .b(new_n91_), .c(new_n84_), .d(x27), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(x08), .O(new_n192_));
  inv1   g130(.a(new_n84_), .O(new_n193_));
  nor2   g131(.a(new_n95_), .b(x04), .O(new_n194_));
  aoi21  g132(.a(new_n194_), .b(new_n193_), .c(x30), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n192_), .c(new_n64_), .d(new_n138_), .O(z18));
  nand2  g134(.a(new_n148_), .b(new_n64_), .O(z19));
  nand2  g135(.a(x28), .b(new_n95_), .O(new_n198_));
  aoi22  g136(.a(new_n198_), .b(x35), .c(new_n142_), .d(x40), .O(new_n199_));
  oai21  g137(.a(new_n177_), .b(new_n95_), .c(x24), .O(new_n200_));
  nand2  g138(.a(new_n75_), .b(new_n95_), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n200_), .c(x08), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n199_), .c(new_n80_), .O(new_n203_));
  nor2   g141(.a(new_n203_), .b(x09), .O(z20));
endmodule


