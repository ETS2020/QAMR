// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_;
  nand2  g000(.a(x40), .b(x13), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x13), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(x28), .c(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n63_), .c(x04), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  inv1   g025(.a(x04), .O(new_n88_));
  inv1   g026(.a(x28), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(new_n72_), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n87_), .c(x40), .O(new_n91_));
  oai21  g029(.a(x27), .b(x08), .c(x35), .O(new_n92_));
  nand2  g030(.a(x40), .b(x39), .O(new_n93_));
  aoi21  g031(.a(new_n92_), .b(x04), .c(new_n93_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n91_), .O(z02));
  nand2  g033(.a(new_n69_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n68_), .b(new_n67_), .c(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n96_), .c(new_n64_), .O(z03));
  aoi21  g037(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n96_), .c(new_n64_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n63_), .c(new_n68_), .O(z05));
  aoi21  g041(.a(x40), .b(x13), .c(x27), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(new_n89_), .c(new_n68_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n63_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n65_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x14), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n109_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  oai21  g053(.a(new_n111_), .b(x31), .c(new_n115_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n63_), .O(z07));
  oai21  g056(.a(x39), .b(x13), .c(x40), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n63_), .O(z09));
  nand2  g060(.a(new_n93_), .b(x07), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n83_), .c(x27), .O(new_n126_));
  nand2  g064(.a(new_n80_), .b(new_n79_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(x40), .c(x39), .d(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n88_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n63_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  nand2  g071(.a(x40), .b(x04), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x35), .c(new_n89_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n72_), .c(new_n104_), .O(new_n136_));
  aoi21  g074(.a(x39), .b(x29), .c(x13), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(x35), .c(new_n89_), .O(new_n139_));
  oai21  g077(.a(new_n137_), .b(new_n74_), .c(new_n139_), .O(new_n140_));
  oai21  g078(.a(new_n136_), .b(x08), .c(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n79_), .c(new_n133_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n63_), .O(z11));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  nor2   g082(.a(new_n74_), .b(x27), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n73_), .O(new_n146_));
  oai21  g084(.a(x27), .b(new_n88_), .c(new_n72_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x40), .O(new_n148_));
  inv1   g086(.a(x39), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(x04), .c(new_n72_), .O(new_n150_));
  aoi22  g088(.a(new_n150_), .b(x40), .c(x35), .d(x28), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n68_), .O(new_n152_));
  nand2  g090(.a(new_n119_), .b(new_n67_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n152_), .c(new_n148_), .d(new_n146_), .O(z12));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x28), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x40), .c(x39), .d(new_n88_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  nand2  g097(.a(new_n69_), .b(new_n68_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nand3  g101(.a(new_n127_), .b(x39), .c(new_n88_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n72_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x40), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n163_), .O(z13));
  nand3  g105(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n168_));
  nor3   g106(.a(new_n168_), .b(new_n89_), .c(x13), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n151_), .c(new_n68_), .O(new_n170_));
  nand3  g108(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n171_));
  nand4  g109(.a(new_n155_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n172_));
  aoi21  g110(.a(x40), .b(x04), .c(new_n149_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n172_), .c(x13), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n74_), .c(new_n171_), .O(new_n175_));
  aoi21  g113(.a(new_n173_), .b(new_n127_), .c(x13), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n74_), .c(new_n67_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n175_), .c(new_n170_), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n63_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n63_), .O(z16));
  inv1   g121(.a(x01), .O(new_n184_));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n63_), .b(new_n185_), .c(x23), .d(x22), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z17));
  nand2  g125(.a(x27), .b(new_n88_), .O(new_n188_));
  nand2  g126(.a(new_n67_), .b(x08), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n188_), .c(new_n81_), .O(new_n190_));
  nand2  g128(.a(new_n79_), .b(new_n133_), .O(new_n191_));
  aoi21  g129(.a(new_n190_), .b(new_n89_), .c(new_n191_), .O(new_n192_));
  inv1   g130(.a(new_n93_), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x29), .c(new_n72_), .d(x08), .O(new_n194_));
  oai21  g132(.a(new_n192_), .b(new_n64_), .c(new_n194_), .O(z18));
  inv1   g133(.a(new_n142_), .O(z19));
  nand2  g134(.a(x39), .b(x29), .O(new_n197_));
  nor2   g135(.a(new_n89_), .b(x27), .O(new_n198_));
  nor2   g136(.a(x35), .b(x13), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n198_), .c(new_n197_), .O(new_n200_));
  inv1   g138(.a(x08), .O(new_n201_));
  aoi21  g139(.a(x35), .b(x27), .c(x13), .O(new_n202_));
  nor2   g140(.a(x40), .b(x27), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n202_), .c(new_n201_), .O(new_n204_));
  oai21  g142(.a(new_n198_), .b(new_n81_), .c(new_n74_), .O(new_n205_));
  nand3  g143(.a(new_n205_), .b(new_n204_), .c(new_n200_), .O(new_n206_));
  nand3  g144(.a(new_n206_), .b(new_n79_), .c(new_n133_), .O(new_n207_));
  nand2  g145(.a(new_n207_), .b(new_n63_), .O(z20));
endmodule


