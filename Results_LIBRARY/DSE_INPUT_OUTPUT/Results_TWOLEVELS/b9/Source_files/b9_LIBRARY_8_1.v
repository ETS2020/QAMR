// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n174_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_;
  inv1   g000(.a(x39), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n65_), .d(new_n73_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n72_), .O(z00));
  inv1   g017(.a(new_n64_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x36), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n82_), .c(new_n81_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n80_), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g030(.a(x08), .b(new_n73_), .c(x40), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  nand2  g032(.a(new_n85_), .b(x04), .O(new_n95_));
  oai21  g033(.a(x28), .b(new_n66_), .c(x04), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n75_), .c(new_n80_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n94_), .d(new_n92_), .O(z02));
  nand2  g036(.a(new_n68_), .b(x27), .O(new_n99_));
  nand2  g037(.a(new_n67_), .b(new_n66_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g039(.a(new_n67_), .b(new_n66_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n80_), .O(z04));
  aoi21  g041(.a(x28), .b(x27), .c(x37), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(new_n80_), .O(z05));
  aoi21  g043(.a(new_n64_), .b(new_n66_), .c(new_n84_), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n65_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n64_), .c(x03), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z07));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x05), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(x40), .b(x39), .O(new_n124_));
  oai22  g062(.a(new_n124_), .b(new_n122_), .c(x40), .d(new_n123_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n86_), .c(new_n73_), .O(new_n126_));
  nand3  g064(.a(new_n64_), .b(x37), .c(x06), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x27), .O(new_n129_));
  aoi21  g067(.a(new_n82_), .b(new_n81_), .c(new_n75_), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(x39), .c(x05), .d(new_n73_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  oai21  g071(.a(new_n63_), .b(x29), .c(x40), .O(new_n134_));
  nand2  g072(.a(x27), .b(x04), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x35), .c(new_n84_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand3  g075(.a(x35), .b(new_n84_), .c(new_n73_), .O(new_n138_));
  aoi22  g076(.a(new_n138_), .b(x39), .c(new_n64_), .d(new_n66_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(x08), .c(new_n137_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n81_), .c(new_n133_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n64_), .O(z11));
  inv1   g080(.a(x35), .O(new_n143_));
  nand3  g081(.a(new_n67_), .b(new_n83_), .c(new_n143_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  nor2   g083(.a(new_n69_), .b(new_n73_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(x39), .O(new_n147_));
  oai21  g085(.a(new_n69_), .b(x40), .c(new_n147_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x40), .c(new_n149_), .d(new_n73_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand2  g092(.a(new_n68_), .b(new_n67_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n152_), .c(new_n66_), .O(new_n157_));
  nand2  g095(.a(new_n82_), .b(new_n81_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n149_), .c(new_n73_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x39), .c(new_n75_), .O(new_n160_));
  or2    g098(.a(new_n160_), .b(new_n157_), .O(z13));
  aoi21  g099(.a(new_n149_), .b(new_n73_), .c(new_n63_), .O(new_n162_));
  nor2   g100(.a(x19), .b(x18), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(new_n155_), .c(x27), .d(x20), .O(new_n164_));
  oai21  g102(.a(new_n162_), .b(new_n75_), .c(new_n164_), .O(new_n165_));
  nand3  g103(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n166_));
  nor2   g104(.a(x37), .b(x35), .O(new_n167_));
  aoi21  g105(.a(new_n166_), .b(new_n150_), .c(new_n167_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n84_), .c(x27), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x39), .c(new_n82_), .d(new_n81_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n165_), .O(z14));
  nand4  g109(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n64_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand4  g112(.a(new_n64_), .b(new_n174_), .c(x22), .d(x01), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(x23), .c(x22), .d(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n64_), .O(z17));
  nand2  g117(.a(x27), .b(new_n73_), .O(new_n180_));
  nand2  g118(.a(new_n66_), .b(x08), .O(new_n181_));
  aoi21  g119(.a(new_n181_), .b(new_n180_), .c(new_n143_), .O(new_n182_));
  nand2  g120(.a(new_n81_), .b(new_n133_), .O(new_n183_));
  aoi21  g121(.a(new_n182_), .b(new_n84_), .c(new_n183_), .O(new_n184_));
  oai22  g122(.a(new_n184_), .b(new_n80_), .c(new_n124_), .d(new_n91_), .O(z18));
  inv1   g123(.a(new_n141_), .O(z19));
  oai21  g124(.a(new_n84_), .b(x27), .c(x35), .O(new_n187_));
  and2   g125(.a(new_n187_), .b(new_n134_), .O(new_n188_));
  nand2  g126(.a(new_n64_), .b(new_n66_), .O(new_n189_));
  nand2  g127(.a(x39), .b(new_n143_), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n189_), .c(x08), .O(new_n191_));
  oai21  g129(.a(new_n191_), .b(new_n188_), .c(new_n81_), .O(new_n192_));
  nor2   g130(.a(new_n192_), .b(x09), .O(z20));
  buf    g131(.a(x40), .O(z08));
endmodule


