// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n187_, new_n188_, new_n190_, new_n191_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x31), .O(new_n64_));
  nand2  g002(.a(x40), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x15), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n63_), .c(new_n65_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(x31), .d(new_n72_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n71_), .O(z00));
  inv1   g016(.a(new_n65_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n84_), .c(x04), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x39), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x31), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x40), .O(z02));
  nand2  g034(.a(new_n67_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x21), .O(new_n98_));
  inv1   g036(.a(x27), .O(new_n99_));
  aoi21  g037(.a(new_n66_), .b(new_n99_), .c(new_n98_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n97_), .c(new_n79_), .O(z03));
  nand2  g039(.a(new_n66_), .b(new_n99_), .O(new_n102_));
  nand4  g040(.a(new_n102_), .b(new_n97_), .c(new_n65_), .d(new_n98_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n66_), .c(new_n79_), .O(z05));
  nand3  g043(.a(new_n104_), .b(new_n65_), .c(new_n66_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x03), .O(new_n108_));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n72_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand4  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .d(new_n64_), .O(new_n113_));
  inv1   g051(.a(x14), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n111_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n74_), .O(new_n119_));
  nand3  g057(.a(new_n116_), .b(x31), .c(new_n114_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(new_n108_), .O(z07));
  nand3  g059(.a(x40), .b(x39), .c(x31), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z08));
  inv1   g061(.a(x11), .O(new_n124_));
  nand4  g062(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n124_), .O(z09));
  inv1   g064(.a(x04), .O(new_n127_));
  inv1   g065(.a(x07), .O(new_n128_));
  inv1   g066(.a(x39), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(x31), .c(new_n74_), .O(new_n130_));
  nand4  g068(.a(x40), .b(x39), .c(x31), .d(x05), .O(new_n131_));
  oai21  g069(.a(new_n130_), .b(new_n128_), .c(new_n131_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n85_), .c(new_n127_), .O(new_n133_));
  nand3  g071(.a(new_n65_), .b(x37), .c(x06), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x27), .O(new_n136_));
  nor2   g074(.a(x32), .b(x30), .O(new_n137_));
  nor3   g075(.a(new_n137_), .b(new_n74_), .c(new_n129_), .O(new_n138_));
  nand4  g076(.a(new_n138_), .b(x31), .c(x05), .d(new_n127_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(z10));
  nand2  g078(.a(new_n65_), .b(new_n99_), .O(new_n141_));
  nand3  g079(.a(x35), .b(new_n83_), .c(new_n127_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x31), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n141_), .c(x08), .O(new_n144_));
  inv1   g082(.a(new_n84_), .O(new_n145_));
  nand2  g083(.a(x39), .b(x29), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x31), .c(new_n74_), .O(new_n147_));
  nand2  g085(.a(x27), .b(x04), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n145_), .c(new_n147_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n144_), .c(new_n80_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(x09), .c(new_n65_), .O(z11));
  nor2   g089(.a(new_n64_), .b(x27), .O(new_n152_));
  nor3   g090(.a(x37), .b(x36), .c(x35), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n152_), .c(new_n73_), .O(new_n154_));
  nor2   g092(.a(new_n74_), .b(new_n129_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n127_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n69_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n154_), .c(new_n65_), .O(z12));
  inv1   g096(.a(x13), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x28), .c(new_n74_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n159_), .d(new_n127_), .O(new_n162_));
  inv1   g100(.a(x18), .O(new_n163_));
  inv1   g101(.a(x19), .O(new_n164_));
  nand4  g102(.a(new_n68_), .b(x20), .c(new_n164_), .d(new_n163_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x27), .O(new_n167_));
  nor4   g105(.a(new_n137_), .b(new_n129_), .c(x13), .d(x04), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n64_), .c(x40), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n167_), .O(z13));
  nor2   g108(.a(new_n137_), .b(x13), .O(new_n171_));
  nor2   g109(.a(new_n171_), .b(new_n64_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n156_), .c(new_n99_), .O(new_n173_));
  nand3  g111(.a(x20), .b(new_n164_), .c(new_n163_), .O(new_n174_));
  nand4  g112(.a(new_n160_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n175_));
  nor2   g113(.a(x13), .b(x04), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n175_), .c(new_n155_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n174_), .O(new_n178_));
  aoi22  g116(.a(new_n176_), .b(new_n155_), .c(x35), .d(x28), .O(new_n179_));
  inv1   g117(.a(x35), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n179_), .c(new_n66_), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(new_n178_), .c(new_n173_), .d(new_n65_), .O(z14));
  inv1   g122(.a(x12), .O(new_n185_));
  nor2   g123(.a(new_n125_), .b(new_n185_), .O(z15));
  inv1   g124(.a(x23), .O(new_n187_));
  nand4  g125(.a(new_n65_), .b(new_n187_), .c(x22), .d(x01), .O(new_n188_));
  inv1   g126(.a(new_n188_), .O(z16));
  inv1   g127(.a(x24), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(x23), .c(x22), .d(x01), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n65_), .O(z17));
  nand2  g130(.a(x27), .b(new_n127_), .O(new_n193_));
  nand2  g131(.a(new_n99_), .b(x08), .O(new_n194_));
  aoi21  g132(.a(new_n194_), .b(new_n193_), .c(new_n180_), .O(new_n195_));
  inv1   g133(.a(x09), .O(new_n196_));
  nand2  g134(.a(new_n80_), .b(new_n196_), .O(new_n197_));
  aoi21  g135(.a(new_n195_), .b(new_n83_), .c(new_n197_), .O(new_n198_));
  nand2  g136(.a(new_n155_), .b(x31), .O(new_n199_));
  oai22  g137(.a(new_n199_), .b(new_n90_), .c(new_n198_), .d(new_n79_), .O(z18));
  nor2   g138(.a(new_n150_), .b(x09), .O(z19));
  aoi21  g139(.a(x28), .b(new_n99_), .c(new_n180_), .O(new_n202_));
  nor2   g140(.a(new_n202_), .b(new_n147_), .O(new_n203_));
  nand2  g141(.a(new_n180_), .b(x31), .O(new_n204_));
  aoi21  g142(.a(new_n204_), .b(new_n141_), .c(x08), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n203_), .c(new_n80_), .O(new_n206_));
  nor2   g144(.a(new_n206_), .b(x09), .O(z20));
endmodule


