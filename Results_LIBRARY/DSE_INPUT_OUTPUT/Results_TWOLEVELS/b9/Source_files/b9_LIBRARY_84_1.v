// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_;
  inv1   g000(.a(x32), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n66_), .c(new_n67_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(x32), .d(new_n66_), .O(new_n76_));
  oai22  g014(.a(new_n76_), .b(x04), .c(new_n71_), .d(new_n65_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand4  g021(.a(new_n83_), .b(new_n73_), .c(new_n63_), .d(new_n78_), .O(new_n84_));
  oai21  g022(.a(new_n65_), .b(x04), .c(new_n84_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n81_), .c(x04), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n88_), .c(x39), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x32), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x40), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n80_), .c(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  inv1   g034(.a(x37), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n68_), .c(new_n96_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n95_), .c(new_n65_), .O(z03));
  aoi21  g037(.a(new_n97_), .b(new_n68_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n95_), .c(new_n65_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n65_), .O(z05));
  aoi21  g041(.a(x40), .b(new_n63_), .c(x27), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(new_n80_), .c(new_n97_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n64_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(z07));
  oai21  g055(.a(x39), .b(new_n63_), .c(x40), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  nand3  g060(.a(new_n64_), .b(x37), .c(x06), .O(new_n123_));
  inv1   g061(.a(x04), .O(new_n124_));
  nand4  g062(.a(new_n118_), .b(new_n82_), .c(x07), .d(new_n124_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x32), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x05), .c(new_n124_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z10));
  nand2  g069(.a(new_n64_), .b(new_n68_), .O(new_n132_));
  nand3  g070(.a(x35), .b(new_n80_), .c(new_n124_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x32), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n132_), .c(x08), .O(new_n135_));
  inv1   g073(.a(new_n81_), .O(new_n136_));
  nand2  g074(.a(x39), .b(x29), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(x32), .c(new_n73_), .O(new_n138_));
  nand2  g076(.a(x27), .b(x04), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n136_), .c(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n135_), .c(new_n78_), .O(new_n141_));
  nor2   g079(.a(new_n141_), .b(x09), .O(z11));
  nand3  g080(.a(new_n97_), .b(new_n79_), .c(new_n94_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  aoi21  g082(.a(x39), .b(new_n124_), .c(new_n70_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(x32), .O(new_n146_));
  oai21  g084(.a(new_n69_), .b(new_n68_), .c(new_n73_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(z12));
  inv1   g086(.a(x19), .O(new_n149_));
  nor2   g087(.a(new_n69_), .b(new_n65_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x27), .c(x20), .d(new_n149_), .O(new_n151_));
  inv1   g089(.a(x13), .O(new_n152_));
  nand3  g090(.a(new_n129_), .b(new_n152_), .c(new_n124_), .O(new_n153_));
  oai21  g091(.a(new_n151_), .b(x18), .c(new_n153_), .O(z13));
  nand3  g092(.a(x39), .b(new_n152_), .c(new_n124_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x32), .c(new_n73_), .O(new_n156_));
  nand2  g094(.a(x27), .b(x20), .O(new_n157_));
  nor4   g095(.a(new_n157_), .b(new_n69_), .c(x19), .d(x18), .O(new_n158_));
  nor2   g096(.a(new_n158_), .b(new_n156_), .O(z14));
  inv1   g097(.a(x12), .O(new_n160_));
  nor2   g098(.a(new_n121_), .b(new_n160_), .O(z15));
  inv1   g099(.a(x23), .O(new_n162_));
  nand4  g100(.a(new_n64_), .b(new_n162_), .c(x22), .d(x01), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(x23), .c(x22), .d(x01), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n64_), .O(z17));
  nand2  g105(.a(x27), .b(new_n124_), .O(new_n168_));
  nand2  g106(.a(new_n68_), .b(x08), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n168_), .c(new_n94_), .O(new_n170_));
  inv1   g108(.a(x09), .O(new_n171_));
  nand2  g109(.a(new_n78_), .b(new_n171_), .O(new_n172_));
  aoi21  g110(.a(new_n170_), .b(new_n80_), .c(new_n172_), .O(new_n173_));
  oai22  g111(.a(new_n173_), .b(new_n65_), .c(new_n128_), .d(new_n87_), .O(z18));
  nor2   g112(.a(new_n80_), .b(x27), .O(new_n175_));
  nor2   g113(.a(x35), .b(new_n63_), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n175_), .c(new_n137_), .O(new_n177_));
  inv1   g115(.a(x08), .O(new_n178_));
  oai21  g116(.a(new_n176_), .b(new_n104_), .c(new_n178_), .O(new_n179_));
  oai21  g117(.a(new_n175_), .b(new_n94_), .c(new_n73_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n179_), .c(new_n177_), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n78_), .c(new_n171_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n64_), .O(z20));
  nor2   g121(.a(new_n141_), .b(x09), .O(z19));
endmodule


