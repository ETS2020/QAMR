// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n74_), .c(new_n63_), .O(new_n80_));
  inv1   g018(.a(x31), .O(new_n81_));
  nand2  g019(.a(x40), .b(new_n81_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n80_), .c(x16), .O(z00));
  inv1   g021(.a(new_n82_), .O(new_n84_));
  inv1   g022(.a(x30), .O(new_n85_));
  inv1   g023(.a(x32), .O(new_n86_));
  oai21  g024(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n86_), .c(new_n85_), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(x04), .c(new_n84_), .O(z01));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g031(.a(x28), .b(x04), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n93_), .c(new_n76_), .O(new_n95_));
  oai21  g033(.a(x27), .b(x08), .c(x35), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x04), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x39), .O(new_n98_));
  oai21  g036(.a(new_n98_), .b(new_n95_), .c(x31), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x40), .O(z02));
  nand2  g038(.a(new_n71_), .b(x27), .O(new_n101_));
  inv1   g039(.a(x21), .O(new_n102_));
  aoi21  g040(.a(new_n70_), .b(new_n64_), .c(new_n102_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n101_), .c(new_n84_), .O(z03));
  aoi21  g042(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n101_), .c(new_n84_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n82_), .c(new_n70_), .O(z05));
  inv1   g046(.a(z05), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n63_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand4  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .d(new_n81_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai21  g055(.a(new_n117_), .b(x14), .c(new_n114_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n82_), .O(z07));
  aoi21  g058(.a(new_n75_), .b(x31), .c(new_n76_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n82_), .O(z09));
  inv1   g061(.a(x07), .O(new_n124_));
  nand4  g062(.a(x40), .b(x39), .c(x31), .d(x05), .O(new_n125_));
  oai21  g063(.a(z08), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n87_), .c(new_n65_), .O(new_n127_));
  nand3  g065(.a(new_n82_), .b(x37), .c(x06), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x27), .O(new_n130_));
  oai21  g068(.a(x32), .b(x30), .c(x40), .O(new_n131_));
  nor3   g069(.a(new_n131_), .b(new_n75_), .c(new_n81_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x05), .c(new_n65_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(z10));
  nand2  g072(.a(new_n82_), .b(new_n64_), .O(new_n135_));
  inv1   g073(.a(x28), .O(new_n136_));
  nand2  g074(.a(x40), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x31), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n135_), .c(x08), .O(new_n140_));
  nor2   g078(.a(new_n66_), .b(x28), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x31), .c(new_n76_), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n141_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n140_), .c(new_n85_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(x09), .O(z11));
  nand3  g085(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n148_));
  nand2  g086(.a(x40), .b(new_n64_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n148_), .c(x10), .O(new_n150_));
  nand2  g088(.a(new_n137_), .b(x39), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n64_), .O(new_n152_));
  nand2  g090(.a(x39), .b(new_n65_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n71_), .c(new_n70_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n150_), .c(x31), .O(new_n156_));
  oai21  g094(.a(new_n73_), .b(new_n64_), .c(new_n76_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(z12));
  inv1   g096(.a(x13), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x28), .c(new_n76_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n159_), .d(new_n65_), .O(new_n162_));
  inv1   g100(.a(x18), .O(new_n163_));
  inv1   g101(.a(x19), .O(new_n164_));
  nand4  g102(.a(new_n72_), .b(x20), .c(new_n164_), .d(new_n163_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n162_), .c(new_n81_), .O(new_n166_));
  nand4  g104(.a(new_n72_), .b(new_n76_), .c(x20), .d(new_n164_), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(x18), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(x27), .O(new_n169_));
  nand3  g107(.a(new_n132_), .b(new_n159_), .c(new_n65_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(z13));
  nand3  g109(.a(x20), .b(new_n164_), .c(new_n163_), .O(new_n172_));
  nand4  g110(.a(new_n160_), .b(new_n86_), .c(new_n85_), .d(x28), .O(new_n173_));
  oai21  g111(.a(x13), .b(x04), .c(x40), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n173_), .c(x39), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x31), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x40), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n172_), .O(new_n178_));
  nand3  g116(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x27), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n86_), .c(new_n85_), .O(new_n181_));
  nand2  g119(.a(new_n149_), .b(new_n72_), .O(new_n182_));
  oai21  g120(.a(x13), .b(x04), .c(new_n182_), .O(new_n183_));
  oai21  g121(.a(new_n73_), .b(new_n64_), .c(new_n75_), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n183_), .c(new_n181_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(x31), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n178_), .c(new_n157_), .O(z14));
  nand4  g125(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n82_), .O(z15));
  inv1   g127(.a(x23), .O(new_n190_));
  nand4  g128(.a(new_n82_), .b(new_n190_), .c(x22), .d(x01), .O(new_n191_));
  inv1   g129(.a(new_n191_), .O(z16));
  inv1   g130(.a(x24), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x23), .c(x22), .d(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n82_), .O(z17));
  nand2  g133(.a(x27), .b(new_n65_), .O(new_n196_));
  nand2  g134(.a(new_n64_), .b(x08), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n196_), .c(new_n66_), .O(new_n198_));
  inv1   g136(.a(x09), .O(new_n199_));
  nand2  g137(.a(new_n85_), .b(new_n199_), .O(new_n200_));
  aoi21  g138(.a(new_n198_), .b(new_n136_), .c(new_n200_), .O(new_n201_));
  nand2  g139(.a(new_n77_), .b(x31), .O(new_n202_));
  oai22  g140(.a(new_n202_), .b(new_n92_), .c(new_n201_), .d(new_n84_), .O(z18));
  aoi21  g141(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n204_));
  nor2   g142(.a(new_n204_), .b(new_n143_), .O(new_n205_));
  nand2  g143(.a(new_n66_), .b(x31), .O(new_n206_));
  aoi21  g144(.a(new_n206_), .b(new_n135_), .c(x08), .O(new_n207_));
  oai21  g145(.a(new_n207_), .b(new_n205_), .c(new_n85_), .O(new_n208_));
  nor2   g146(.a(new_n208_), .b(x09), .O(z20));
  nor2   g147(.a(new_n146_), .b(x09), .O(z19));
endmodule


