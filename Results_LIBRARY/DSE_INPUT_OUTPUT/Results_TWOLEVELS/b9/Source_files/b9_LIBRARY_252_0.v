// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  inv1   g005(.a(x40), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x39), .c(new_n65_), .O(new_n70_));
  aoi21  g008(.a(x35), .b(x28), .c(x37), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n70_), .c(new_n64_), .O(new_n72_));
  inv1   g010(.a(x39), .O(new_n73_));
  nor2   g011(.a(new_n68_), .b(new_n73_), .O(z08));
  nand3  g012(.a(z08), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n72_), .c(new_n63_), .O(new_n77_));
  nor2   g015(.a(x40), .b(x28), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  oai21  g020(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n79_), .c(x04), .O(z01));
  inv1   g024(.a(x08), .O(new_n87_));
  inv1   g025(.a(x29), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g027(.a(x27), .b(x08), .c(x35), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x04), .c(new_n73_), .O(new_n91_));
  oai21  g029(.a(new_n89_), .b(x02), .c(new_n91_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x40), .O(new_n93_));
  oai21  g031(.a(new_n68_), .b(x04), .c(x28), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(z02));
  oai21  g033(.a(new_n68_), .b(x28), .c(x35), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x27), .O(new_n97_));
  nor2   g035(.a(x37), .b(x27), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n78_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n97_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n97_), .c(new_n101_), .O(z04));
  inv1   g040(.a(x28), .O(new_n103_));
  inv1   g041(.a(x37), .O(new_n104_));
  oai22  g042(.a(new_n98_), .b(new_n103_), .c(new_n68_), .d(new_n104_), .O(z05));
  inv1   g043(.a(new_n98_), .O(new_n106_));
  oai21  g044(.a(new_n68_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n106_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n79_), .O(z07));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(x39), .b(x05), .O(new_n124_));
  oai21  g062(.a(x39), .b(new_n123_), .c(new_n124_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n83_), .c(x40), .O(new_n126_));
  nand4  g064(.a(new_n68_), .b(x36), .c(x28), .d(x07), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(x04), .O(new_n128_));
  inv1   g066(.a(x06), .O(new_n129_));
  nor3   g067(.a(new_n78_), .b(new_n104_), .c(new_n129_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n128_), .c(x27), .O(new_n131_));
  oai21  g069(.a(x32), .b(x30), .c(x40), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n73_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x05), .c(new_n65_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(z10));
  inv1   g073(.a(x09), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand2  g075(.a(x40), .b(new_n66_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n103_), .O(new_n139_));
  oai21  g077(.a(new_n73_), .b(new_n88_), .c(new_n139_), .O(new_n140_));
  nand2  g078(.a(x35), .b(x27), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(x04), .c(x40), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n103_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n87_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n140_), .c(x40), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n81_), .c(new_n136_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n79_), .O(z11));
  nand3  g085(.a(new_n104_), .b(new_n67_), .c(new_n66_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x27), .c(x10), .O(new_n149_));
  nor2   g087(.a(new_n71_), .b(new_n64_), .O(new_n150_));
  aoi21  g088(.a(x39), .b(new_n65_), .c(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(x40), .O(new_n152_));
  nand2  g090(.a(new_n104_), .b(new_n66_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n68_), .c(x28), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x28), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand4  g100(.a(x37), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n160_), .c(new_n68_), .O(new_n164_));
  nand4  g102(.a(new_n153_), .b(x28), .c(x20), .d(new_n162_), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(x18), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n164_), .c(x27), .O(new_n167_));
  nand3  g105(.a(new_n133_), .b(new_n157_), .c(new_n65_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z13));
  nand3  g107(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n170_));
  nand4  g108(.a(new_n158_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  nand4  g110(.a(x40), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n170_), .O(new_n174_));
  oai21  g112(.a(new_n153_), .b(new_n103_), .c(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n82_), .c(new_n81_), .O(new_n176_));
  oai21  g114(.a(new_n73_), .b(x04), .c(x40), .O(new_n177_));
  nand2  g115(.a(new_n68_), .b(x28), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n154_), .O(new_n180_));
  oai21  g118(.a(new_n66_), .b(new_n103_), .c(x13), .O(new_n181_));
  oai21  g119(.a(new_n177_), .b(x28), .c(new_n181_), .O(new_n182_));
  nand3  g120(.a(x40), .b(new_n64_), .c(x13), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n79_), .O(new_n184_));
  aoi21  g122(.a(new_n182_), .b(new_n104_), .c(new_n184_), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n180_), .c(new_n176_), .d(new_n174_), .O(z14));
  nand4  g124(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n79_), .O(z15));
  inv1   g126(.a(x23), .O(new_n189_));
  nand4  g127(.a(new_n79_), .b(new_n189_), .c(x22), .d(x01), .O(new_n190_));
  inv1   g128(.a(new_n190_), .O(z16));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(x23), .c(x22), .d(x01), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n79_), .O(z17));
  nand3  g132(.a(x35), .b(new_n103_), .c(new_n64_), .O(new_n195_));
  nand2  g133(.a(z08), .b(x29), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(x08), .O(new_n198_));
  nand4  g136(.a(new_n198_), .b(new_n143_), .c(new_n81_), .d(new_n136_), .O(z18));
  nand2  g137(.a(x40), .b(new_n87_), .O(new_n200_));
  aoi22  g138(.a(new_n200_), .b(new_n178_), .c(x35), .d(x27), .O(new_n201_));
  nand2  g139(.a(x28), .b(new_n64_), .O(new_n202_));
  aoi22  g140(.a(new_n202_), .b(new_n138_), .c(x39), .d(x29), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n201_), .c(new_n81_), .O(new_n204_));
  nor2   g142(.a(new_n204_), .b(x09), .O(z20));
  nand2  g143(.a(new_n146_), .b(new_n79_), .O(z19));
endmodule


