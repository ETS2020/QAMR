// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x19), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x13), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g009(.a(new_n71_), .b(x40), .c(x39), .d(new_n68_), .O(new_n72_));
  aoi21  g010(.a(x35), .b(x28), .c(x37), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n68_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n66_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  aoi21  g020(.a(new_n64_), .b(x28), .c(new_n69_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(new_n69_), .b(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(new_n75_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n89_), .c(new_n65_), .O(z02));
  nor2   g031(.a(x37), .b(x27), .O(new_n94_));
  aoi21  g032(.a(new_n69_), .b(x27), .c(new_n94_), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x21), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n64_), .O(new_n97_));
  oai21  g035(.a(x35), .b(x04), .c(x19), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(x13), .c(x28), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x27), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n97_), .O(z03));
  inv1   g039(.a(x21), .O(new_n102_));
  nand4  g040(.a(new_n100_), .b(new_n95_), .c(new_n64_), .d(new_n102_), .O(z04));
  inv1   g041(.a(x37), .O(new_n104_));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n65_), .O(z05));
  nand3  g044(.a(new_n105_), .b(new_n64_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n66_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  inv1   g058(.a(x13), .O(new_n121_));
  oai21  g059(.a(x19), .b(new_n121_), .c(x40), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n120_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n64_), .O(z09));
  oai21  g063(.a(new_n69_), .b(x28), .c(new_n70_), .O(new_n126_));
  nand2  g064(.a(new_n75_), .b(x07), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n126_), .c(x27), .O(new_n130_));
  oai21  g068(.a(x32), .b(x30), .c(x40), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n120_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x05), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n68_), .O(new_n135_));
  nand3  g073(.a(x37), .b(x27), .c(x06), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n135_), .c(new_n65_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand2  g076(.a(new_n76_), .b(x29), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n90_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  inv1   g080(.a(x08), .O(new_n143_));
  nand3  g081(.a(new_n90_), .b(x27), .c(new_n68_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(new_n64_), .c(new_n81_), .d(new_n138_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n64_), .O(z11));
  inv1   g086(.a(x10), .O(new_n149_));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n67_), .c(new_n149_), .O(new_n151_));
  oai22  g089(.a(new_n75_), .b(x04), .c(new_n73_), .d(new_n67_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n151_), .c(new_n64_), .O(z12));
  inv1   g091(.a(x40), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n154_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n121_), .d(new_n68_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x20), .O(new_n159_));
  nor2   g097(.a(new_n73_), .b(new_n159_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n63_), .c(new_n158_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nor3   g101(.a(new_n131_), .b(new_n120_), .c(x13), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n68_), .c(new_n65_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n163_), .O(z13));
  nor2   g104(.a(new_n75_), .b(x04), .O(new_n167_));
  nand4  g105(.a(new_n155_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n168_));
  aoi22  g106(.a(new_n168_), .b(new_n167_), .c(x20), .d(new_n158_), .O(new_n169_));
  nand3  g107(.a(new_n104_), .b(new_n69_), .c(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n82_), .c(new_n81_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n152_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n169_), .c(new_n121_), .O(new_n174_));
  oai21  g112(.a(x28), .b(x27), .c(x04), .O(new_n175_));
  aoi21  g113(.a(x39), .b(x13), .c(new_n154_), .O(new_n176_));
  nand4  g114(.a(new_n176_), .b(new_n175_), .c(new_n168_), .d(x39), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x19), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n174_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nand4  g118(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n180_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(x22), .c(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n64_), .O(z16));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(x23), .c(x22), .d(x01), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n64_), .O(z17));
  nand2  g126(.a(new_n90_), .b(new_n67_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n139_), .O(new_n190_));
  nand3  g128(.a(new_n144_), .b(new_n81_), .c(new_n138_), .O(new_n191_));
  aoi21  g129(.a(new_n190_), .b(x08), .c(new_n191_), .O(new_n192_));
  nor2   g130(.a(new_n192_), .b(new_n65_), .O(z18));
  inv1   g131(.a(new_n147_), .O(z19));
  inv1   g132(.a(new_n139_), .O(new_n195_));
  oai21  g133(.a(new_n69_), .b(new_n67_), .c(new_n143_), .O(new_n196_));
  aoi21  g134(.a(x28), .b(new_n67_), .c(new_n69_), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n195_), .c(new_n196_), .O(new_n198_));
  nand4  g136(.a(new_n198_), .b(new_n64_), .c(new_n81_), .d(new_n138_), .O(new_n199_));
  inv1   g137(.a(new_n199_), .O(z20));
endmodule


