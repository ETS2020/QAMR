// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n110_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand4  g012(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n74_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x29), .O(new_n78_));
  inv1   g016(.a(x40), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  aoi21  g023(.a(x35), .b(new_n85_), .c(x36), .O(new_n86_));
  or2    g024(.a(new_n86_), .b(new_n64_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n84_), .c(new_n83_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n81_), .c(x04), .O(z01));
  inv1   g027(.a(x18), .O(new_n90_));
  inv1   g028(.a(x19), .O(new_n91_));
  nand4  g029(.a(new_n72_), .b(x20), .c(new_n91_), .d(new_n90_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x35), .c(new_n64_), .O(new_n93_));
  aoi21  g031(.a(new_n70_), .b(new_n85_), .c(new_n64_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(x20), .c(new_n91_), .d(new_n90_), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n93_), .c(new_n79_), .O(new_n96_));
  nand2  g034(.a(x28), .b(x04), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n78_), .c(new_n79_), .O(new_n98_));
  inv1   g036(.a(x08), .O(new_n99_));
  aoi21  g037(.a(new_n64_), .b(new_n99_), .c(new_n66_), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n65_), .O(new_n101_));
  nand2  g039(.a(x39), .b(x02), .O(new_n102_));
  nor3   g040(.a(new_n102_), .b(new_n101_), .c(new_n98_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n96_), .O(z02));
  nand2  g042(.a(new_n71_), .b(x27), .O(new_n105_));
  nand2  g043(.a(new_n70_), .b(new_n64_), .O(new_n106_));
  nand4  g044(.a(new_n106_), .b(new_n105_), .c(new_n81_), .d(x21), .O(z03));
  aoi21  g045(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n105_), .c(new_n80_), .O(z04));
  nand2  g047(.a(x28), .b(x27), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n81_), .c(new_n70_), .O(z05));
  aoi21  g049(.a(new_n81_), .b(new_n64_), .c(new_n85_), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(x37), .c(new_n81_), .O(z06));
  inv1   g051(.a(x33), .O(new_n114_));
  nand2  g052(.a(x17), .b(new_n63_), .O(new_n115_));
  inv1   g053(.a(x00), .O(new_n116_));
  oai21  g054(.a(x25), .b(new_n116_), .c(x38), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(new_n118_));
  inv1   g056(.a(x25), .O(new_n119_));
  nand3  g057(.a(x38), .b(new_n119_), .c(new_n116_), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  oai22  g059(.a(new_n121_), .b(x14), .c(new_n118_), .d(x31), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x03), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n81_), .O(z07));
  oai21  g062(.a(x39), .b(x29), .c(x40), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(z08));
  nand4  g064(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n81_), .O(z09));
  nand2  g066(.a(new_n125_), .b(x07), .O(new_n129_));
  nand4  g067(.a(x40), .b(x39), .c(new_n78_), .d(x05), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n129_), .c(new_n86_), .O(new_n131_));
  nor2   g069(.a(new_n80_), .b(new_n70_), .O(new_n132_));
  aoi22  g070(.a(new_n132_), .b(x06), .c(new_n131_), .d(new_n65_), .O(new_n133_));
  inv1   g071(.a(x39), .O(new_n134_));
  oai21  g072(.a(x32), .b(x30), .c(x40), .O(new_n135_));
  nor3   g073(.a(new_n135_), .b(new_n134_), .c(x29), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x05), .c(new_n65_), .O(new_n137_));
  oai21  g075(.a(new_n133_), .b(new_n64_), .c(new_n137_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  nand2  g077(.a(new_n64_), .b(new_n99_), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  nand4  g079(.a(new_n141_), .b(new_n140_), .c(x35), .d(new_n85_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(new_n81_), .c(new_n83_), .d(new_n139_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(z11));
  nand3  g082(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n145_));
  nand2  g083(.a(x40), .b(new_n64_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n145_), .c(x10), .O(new_n147_));
  aoi21  g085(.a(x40), .b(x04), .c(new_n134_), .O(new_n148_));
  nand2  g086(.a(x39), .b(new_n65_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n71_), .c(new_n70_), .O(new_n150_));
  oai21  g088(.a(new_n148_), .b(x27), .c(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n147_), .c(new_n78_), .O(new_n152_));
  oai21  g090(.a(new_n73_), .b(new_n64_), .c(new_n79_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x28), .c(new_n79_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n65_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n92_), .c(x29), .O(new_n159_));
  nand4  g097(.a(new_n72_), .b(new_n79_), .c(x20), .d(new_n91_), .O(new_n160_));
  nor2   g098(.a(new_n160_), .b(x18), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n159_), .c(x27), .O(new_n162_));
  nand3  g100(.a(new_n136_), .b(new_n155_), .c(new_n65_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n162_), .O(z13));
  nand3  g102(.a(x20), .b(new_n91_), .c(new_n90_), .O(new_n165_));
  nand4  g103(.a(new_n156_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n166_));
  oai21  g104(.a(x13), .b(x04), .c(x40), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n166_), .c(x39), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n78_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x40), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n165_), .O(new_n171_));
  nand3  g109(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x27), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n84_), .c(new_n83_), .O(new_n174_));
  nand2  g112(.a(new_n146_), .b(new_n72_), .O(new_n175_));
  oai21  g113(.a(x13), .b(x04), .c(new_n175_), .O(new_n176_));
  oai21  g114(.a(new_n73_), .b(new_n64_), .c(new_n134_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n176_), .c(new_n174_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n78_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n171_), .c(new_n153_), .O(z14));
  inv1   g118(.a(x12), .O(new_n181_));
  nand4  g119(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n182_));
  nor2   g120(.a(new_n182_), .b(new_n181_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(x22), .c(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n81_), .O(z16));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n187_), .b(x23), .c(x22), .d(x01), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n81_), .O(z17));
  nand2  g127(.a(x27), .b(new_n65_), .O(new_n190_));
  oai21  g128(.a(x27), .b(new_n99_), .c(new_n190_), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(x35), .c(new_n85_), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(new_n81_), .c(new_n83_), .d(new_n139_), .O(z18));
  oai22  g131(.a(new_n100_), .b(new_n80_), .c(new_n85_), .d(x27), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n83_), .c(new_n139_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n81_), .O(z20));
  inv1   g134(.a(new_n143_), .O(z19));
endmodule


