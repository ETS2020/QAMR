// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n110_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x39), .O(new_n64_));
  inv1   g002(.a(x27), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  inv1   g005(.a(new_n67_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(x15), .c(x16), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n63_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n70_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  inv1   g025(.a(new_n82_), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  nand2  g027(.a(new_n65_), .b(new_n89_), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n88_), .c(new_n71_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n87_), .c(x40), .O(new_n92_));
  inv1   g030(.a(x39), .O(new_n93_));
  inv1   g031(.a(x18), .O(new_n94_));
  inv1   g032(.a(x19), .O(new_n95_));
  nand4  g033(.a(new_n67_), .b(x20), .c(new_n95_), .d(new_n94_), .O(new_n96_));
  nor2   g034(.a(x19), .b(x18), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(x27), .c(x20), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n66_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n93_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n92_), .O(z02));
  nand2  g040(.a(x35), .b(x28), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x27), .O(new_n104_));
  inv1   g042(.a(x37), .O(new_n105_));
  nand3  g043(.a(new_n64_), .b(new_n105_), .c(new_n65_), .O(new_n106_));
  nand4  g044(.a(new_n106_), .b(new_n104_), .c(new_n64_), .d(x21), .O(z03));
  inv1   g045(.a(x21), .O(new_n108_));
  nand4  g046(.a(new_n106_), .b(new_n104_), .c(new_n64_), .d(new_n108_), .O(z04));
  nand2  g047(.a(x28), .b(x27), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n64_), .c(new_n105_), .O(z05));
  inv1   g049(.a(z05), .O(z06));
  inv1   g050(.a(x33), .O(new_n113_));
  nand2  g051(.a(x17), .b(new_n72_), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  oai21  g053(.a(x25), .b(new_n115_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n115_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  oai22  g058(.a(new_n120_), .b(x14), .c(new_n117_), .d(x31), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n64_), .c(x03), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z07));
  nor2   g061(.a(new_n63_), .b(new_n93_), .O(z08));
  inv1   g062(.a(x11), .O(new_n125_));
  nand4  g063(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n125_), .O(z09));
  inv1   g065(.a(x07), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x05), .O(new_n129_));
  oai21  g067(.a(x39), .b(new_n128_), .c(new_n129_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n83_), .c(new_n71_), .O(new_n131_));
  nand3  g069(.a(new_n64_), .b(x37), .c(x06), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x27), .O(new_n134_));
  oai21  g072(.a(x32), .b(x30), .c(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n93_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x05), .c(new_n71_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  nand2  g077(.a(x39), .b(x29), .O(new_n140_));
  oai21  g078(.a(new_n65_), .b(new_n71_), .c(new_n81_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  oai21  g080(.a(x40), .b(new_n93_), .c(new_n65_), .O(new_n143_));
  inv1   g081(.a(x35), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(x04), .c(x40), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n143_), .c(new_n81_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n89_), .O(new_n147_));
  oai21  g085(.a(new_n63_), .b(x29), .c(x39), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n147_), .c(new_n142_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n78_), .c(new_n139_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n64_), .O(z11));
  nor3   g090(.a(x37), .b(x36), .c(x35), .O(new_n153_));
  nor2   g091(.a(new_n63_), .b(x27), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n153_), .c(new_n73_), .O(new_n155_));
  oai21  g093(.a(new_n63_), .b(new_n71_), .c(x39), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n68_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n155_), .c(new_n64_), .O(z12));
  inv1   g096(.a(x13), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x28), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n159_), .d(new_n71_), .O(new_n162_));
  nand2  g100(.a(new_n103_), .b(new_n105_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x20), .c(new_n95_), .d(new_n94_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n162_), .c(new_n63_), .O(new_n165_));
  nand4  g103(.a(new_n163_), .b(new_n93_), .c(x20), .d(new_n95_), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x18), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n165_), .c(x27), .O(new_n168_));
  nand3  g106(.a(new_n136_), .b(new_n159_), .c(new_n71_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n168_), .O(z13));
  aoi21  g108(.a(new_n159_), .b(new_n71_), .c(new_n63_), .O(new_n171_));
  oai22  g109(.a(new_n171_), .b(new_n93_), .c(new_n98_), .d(new_n66_), .O(new_n172_));
  nand3  g110(.a(x20), .b(new_n95_), .c(new_n94_), .O(new_n173_));
  nor2   g111(.a(x37), .b(x35), .O(new_n174_));
  aoi21  g112(.a(new_n173_), .b(new_n160_), .c(new_n174_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n81_), .c(x27), .O(new_n176_));
  nand4  g114(.a(new_n176_), .b(x40), .c(new_n79_), .d(new_n78_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n172_), .O(z14));
  inv1   g116(.a(x12), .O(new_n179_));
  nor2   g117(.a(new_n126_), .b(new_n179_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n64_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n64_), .O(z17));
  oai21  g124(.a(new_n82_), .b(x27), .c(new_n140_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(x08), .O(new_n188_));
  nor2   g126(.a(new_n65_), .b(x04), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n88_), .c(x09), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n188_), .c(new_n64_), .d(new_n78_), .O(z18));
  oai21  g129(.a(new_n81_), .b(x27), .c(x35), .O(new_n192_));
  and2   g130(.a(new_n192_), .b(new_n148_), .O(new_n193_));
  nand2  g131(.a(x40), .b(new_n144_), .O(new_n194_));
  aoi21  g132(.a(new_n194_), .b(new_n143_), .c(x08), .O(new_n195_));
  oai21  g133(.a(new_n195_), .b(new_n193_), .c(new_n78_), .O(new_n196_));
  nor2   g134(.a(new_n196_), .b(x09), .O(z20));
  nand2  g135(.a(new_n151_), .b(new_n64_), .O(z19));
endmodule


