// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  nand2  g004(.a(x40), .b(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x39), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand4  g012(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n74_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x40), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x35), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(x36), .c(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n79_), .c(x04), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n84_), .c(x04), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n89_), .c(x40), .O(new_n93_));
  nor2   g031(.a(x40), .b(x35), .O(new_n94_));
  inv1   g032(.a(new_n94_), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n93_), .O(z02));
  nand2  g034(.a(new_n71_), .b(x27), .O(new_n97_));
  nor2   g035(.a(new_n78_), .b(x27), .O(new_n98_));
  oai21  g036(.a(new_n98_), .b(new_n94_), .c(new_n70_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n79_), .d(x21), .O(z03));
  inv1   g038(.a(new_n79_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(x21), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n99_), .c(new_n97_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n79_), .c(new_n70_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n63_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n79_), .O(z07));
  nand2  g055(.a(x40), .b(x39), .O(new_n118_));
  nand2  g056(.a(new_n79_), .b(new_n118_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n79_), .O(z09));
  nand2  g059(.a(new_n84_), .b(new_n67_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(x39), .c(x05), .O(new_n123_));
  nand2  g061(.a(new_n118_), .b(x36), .O(new_n124_));
  inv1   g062(.a(x39), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(x35), .c(new_n83_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x07), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n123_), .c(new_n64_), .O(new_n129_));
  inv1   g067(.a(x05), .O(new_n130_));
  nor2   g068(.a(x32), .b(x30), .O(new_n131_));
  nor4   g069(.a(new_n131_), .b(new_n78_), .c(new_n125_), .d(new_n130_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n129_), .c(new_n65_), .O(new_n133_));
  nand2  g071(.a(x37), .b(x27), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x06), .c(new_n101_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand2  g076(.a(x39), .b(x29), .O(new_n139_));
  aoi21  g077(.a(x27), .b(x04), .c(x28), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n78_), .c(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  inv1   g080(.a(x08), .O(new_n143_));
  oai21  g081(.a(new_n78_), .b(new_n143_), .c(new_n66_), .O(new_n144_));
  nand3  g082(.a(new_n83_), .b(x27), .c(new_n65_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(x40), .c(new_n143_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n144_), .c(new_n142_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n81_), .c(new_n138_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n79_), .O(z11));
  inv1   g087(.a(x10), .O(new_n150_));
  nor3   g088(.a(x37), .b(x36), .c(x35), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n98_), .c(new_n150_), .O(new_n152_));
  nand2  g090(.a(x40), .b(new_n83_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x35), .c(x37), .O(new_n154_));
  oai22  g092(.a(new_n154_), .b(new_n98_), .c(new_n125_), .d(x04), .O(new_n155_));
  oai21  g093(.a(new_n134_), .b(x35), .c(new_n78_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n155_), .c(new_n152_), .O(z12));
  inv1   g095(.a(x13), .O(new_n158_));
  inv1   g096(.a(x36), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n66_), .c(x28), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x39), .c(new_n158_), .d(new_n65_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  nand4  g101(.a(new_n72_), .b(x20), .c(new_n163_), .d(new_n162_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n161_), .c(new_n64_), .O(new_n165_));
  nor4   g103(.a(new_n131_), .b(new_n125_), .c(x13), .d(x04), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n165_), .c(x40), .O(new_n167_));
  nand3  g105(.a(x20), .b(new_n163_), .c(new_n162_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x37), .c(new_n66_), .d(x27), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n167_), .O(z13));
  nand3  g109(.a(x39), .b(new_n158_), .c(new_n65_), .O(new_n172_));
  nor2   g110(.a(x19), .b(x18), .O(new_n173_));
  and2   g111(.a(x27), .b(x20), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n173_), .c(new_n78_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n154_), .c(new_n172_), .O(new_n176_));
  nand3  g114(.a(x40), .b(new_n82_), .c(new_n81_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n95_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n64_), .O(new_n179_));
  nand2  g117(.a(new_n131_), .b(x28), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x40), .O(new_n181_));
  nand3  g119(.a(new_n173_), .b(x37), .c(x20), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n181_), .c(new_n66_), .O(new_n183_));
  nand4  g121(.a(new_n168_), .b(x40), .c(new_n159_), .d(new_n82_), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n81_), .c(x28), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n183_), .c(new_n179_), .d(new_n176_), .O(z14));
  nand4  g125(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n79_), .O(z15));
  inv1   g127(.a(x23), .O(new_n190_));
  nand4  g128(.a(new_n79_), .b(new_n190_), .c(x22), .d(x01), .O(new_n191_));
  inv1   g129(.a(new_n191_), .O(z16));
  inv1   g130(.a(x24), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x23), .c(x22), .d(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n79_), .O(z17));
  nand2  g133(.a(new_n85_), .b(new_n64_), .O(new_n196_));
  aoi21  g134(.a(new_n196_), .b(new_n139_), .c(new_n143_), .O(new_n197_));
  nand3  g135(.a(new_n85_), .b(x27), .c(new_n65_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n81_), .c(new_n138_), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n197_), .c(x40), .O(new_n200_));
  oai21  g138(.a(x30), .b(x09), .c(new_n66_), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n200_), .O(z18));
  inv1   g140(.a(new_n148_), .O(z19));
  oai21  g141(.a(new_n98_), .b(new_n66_), .c(new_n143_), .O(new_n204_));
  nand2  g142(.a(x40), .b(x28), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(x27), .c(x35), .O(new_n206_));
  nand2  g144(.a(new_n206_), .b(new_n139_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n204_), .c(new_n95_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n81_), .c(new_n138_), .O(new_n209_));
  inv1   g147(.a(new_n209_), .O(z20));
endmodule


