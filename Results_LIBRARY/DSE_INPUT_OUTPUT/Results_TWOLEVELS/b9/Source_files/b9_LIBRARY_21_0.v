// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_;
  nor2   g000(.a(x40), .b(x14), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n64_), .d(new_n71_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(new_n63_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n78_), .c(x04), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  aoi21  g026(.a(new_n83_), .b(x04), .c(new_n88_), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  nand3  g028(.a(new_n66_), .b(new_n90_), .c(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n89_), .c(x40), .d(x39), .O(z02));
  nand2  g030(.a(new_n68_), .b(x27), .O(new_n93_));
  nand3  g031(.a(new_n78_), .b(new_n67_), .c(new_n66_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n93_), .c(new_n78_), .d(x21), .O(z03));
  nor2   g033(.a(new_n63_), .b(x21), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(new_n93_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n63_), .O(z05));
  oai22  g037(.a(x40), .b(x14), .c(new_n82_), .d(new_n66_), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(x37), .O(z06));
  inv1   g039(.a(x03), .O(new_n102_));
  inv1   g040(.a(x31), .O(new_n103_));
  inv1   g041(.a(x33), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  aoi22  g044(.a(new_n106_), .b(x38), .c(x17), .d(new_n64_), .O(new_n107_));
  nand4  g045(.a(new_n107_), .b(new_n104_), .c(new_n103_), .d(x14), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n105_), .c(new_n110_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(x40), .c(new_n109_), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(new_n108_), .c(new_n102_), .O(z07));
  nand2  g051(.a(x40), .b(x39), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n78_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n78_), .O(z09));
  nand2  g055(.a(new_n114_), .b(x07), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n84_), .c(x27), .O(new_n121_));
  nand2  g059(.a(new_n80_), .b(new_n79_), .O(new_n122_));
  nand4  g060(.a(new_n122_), .b(x40), .c(x39), .d(x05), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n71_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n78_), .O(z10));
  inv1   g065(.a(x09), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  nor2   g067(.a(new_n66_), .b(new_n71_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n83_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(x40), .b(x04), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(x35), .c(new_n82_), .d(x27), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n90_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n79_), .c(new_n128_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n78_), .O(z11));
  inv1   g075(.a(x35), .O(new_n138_));
  nand3  g076(.a(new_n67_), .b(new_n81_), .c(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x27), .c(x10), .O(new_n140_));
  aoi21  g078(.a(x39), .b(new_n71_), .c(new_n69_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n140_), .c(x40), .O(new_n142_));
  nand2  g080(.a(new_n68_), .b(new_n67_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x27), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n73_), .c(x14), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x28), .c(new_n73_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x39), .c(new_n147_), .d(new_n71_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand4  g090(.a(new_n143_), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x27), .O(new_n155_));
  nand3  g093(.a(new_n122_), .b(x40), .c(x39), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(x13), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n71_), .c(new_n63_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n155_), .O(z13));
  nand3  g097(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n160_));
  nand4  g098(.a(new_n148_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n161_));
  nand3  g099(.a(x39), .b(new_n147_), .c(new_n71_), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n161_), .c(new_n73_), .O(new_n164_));
  nor2   g102(.a(x40), .b(new_n109_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n164_), .c(new_n160_), .O(new_n166_));
  nand3  g104(.a(new_n67_), .b(new_n138_), .c(x28), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x27), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n80_), .c(new_n79_), .O(new_n169_));
  nand2  g107(.a(new_n162_), .b(new_n144_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x40), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n166_), .c(new_n145_), .O(z14));
  inv1   g111(.a(x12), .O(new_n174_));
  nand4  g112(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n174_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand4  g115(.a(new_n78_), .b(new_n177_), .c(x22), .d(x01), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(x23), .c(x22), .d(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n78_), .O(z17));
  oai21  g120(.a(new_n83_), .b(x27), .c(new_n129_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(x08), .O(new_n184_));
  inv1   g122(.a(new_n83_), .O(new_n185_));
  nor2   g123(.a(new_n66_), .b(x04), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(x09), .O(new_n187_));
  nand4  g125(.a(new_n187_), .b(new_n184_), .c(new_n78_), .d(new_n79_), .O(z18));
  inv1   g126(.a(new_n129_), .O(new_n189_));
  oai21  g127(.a(new_n138_), .b(new_n66_), .c(new_n90_), .O(new_n190_));
  aoi21  g128(.a(x28), .b(new_n66_), .c(new_n138_), .O(new_n191_));
  oai21  g129(.a(new_n191_), .b(new_n189_), .c(new_n190_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n79_), .c(new_n128_), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n78_), .O(z20));
  nand2  g132(.a(new_n136_), .b(new_n78_), .O(z19));
endmodule


