// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  nand2  g015(.a(x40), .b(x23), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nor2   g019(.a(new_n66_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g026(.a(x08), .b(new_n65_), .c(x40), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n64_), .O(new_n90_));
  nor2   g028(.a(x35), .b(x23), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(x28), .c(x04), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(x28), .c(new_n64_), .O(new_n94_));
  nand2  g032(.a(new_n78_), .b(x39), .O(new_n95_));
  aoi21  g033(.a(new_n94_), .b(new_n73_), .c(new_n95_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n92_), .c(new_n90_), .d(new_n88_), .O(z02));
  inv1   g035(.a(x28), .O(new_n98_));
  oai21  g036(.a(new_n66_), .b(new_n98_), .c(x27), .O(new_n99_));
  nand3  g037(.a(new_n78_), .b(new_n93_), .c(new_n64_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n78_), .d(x21), .O(z03));
  inv1   g039(.a(x21), .O(new_n102_));
  nand4  g040(.a(new_n100_), .b(new_n99_), .c(new_n78_), .d(new_n102_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  aoi22  g042(.a(new_n104_), .b(new_n93_), .c(x40), .d(x23), .O(z05));
  nand3  g043(.a(new_n104_), .b(new_n78_), .c(new_n93_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n63_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n78_), .O(z07));
  oai21  g056(.a(x39), .b(x23), .c(x40), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z08));
  inv1   g058(.a(x11), .O(new_n121_));
  nand4  g059(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n121_), .O(z09));
  aoi21  g061(.a(x35), .b(new_n98_), .c(x36), .O(new_n124_));
  nand2  g062(.a(new_n119_), .b(x07), .O(new_n125_));
  inv1   g063(.a(x23), .O(new_n126_));
  nand3  g064(.a(new_n74_), .b(new_n126_), .c(x05), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n125_), .c(new_n124_), .O(new_n128_));
  nand3  g066(.a(new_n78_), .b(x37), .c(x06), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  aoi21  g068(.a(new_n128_), .b(new_n65_), .c(new_n130_), .O(new_n131_));
  oai21  g069(.a(x32), .b(x30), .c(x40), .O(new_n132_));
  nor3   g070(.a(new_n132_), .b(new_n72_), .c(x23), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x05), .c(new_n65_), .O(new_n134_));
  oai21  g072(.a(new_n131_), .b(new_n64_), .c(new_n134_), .O(z10));
  inv1   g073(.a(x09), .O(new_n136_));
  nand3  g074(.a(x35), .b(new_n98_), .c(new_n65_), .O(new_n137_));
  aoi22  g075(.a(new_n137_), .b(new_n126_), .c(new_n78_), .d(new_n64_), .O(new_n138_));
  nand2  g076(.a(x39), .b(x29), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n126_), .c(new_n73_), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n82_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(new_n143_));
  oai22  g081(.a(new_n143_), .b(new_n140_), .c(new_n138_), .d(x08), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n80_), .c(new_n136_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(z11));
  nand3  g084(.a(new_n93_), .b(new_n67_), .c(new_n66_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  nor2   g086(.a(new_n70_), .b(new_n64_), .O(new_n149_));
  aoi21  g087(.a(x39), .b(new_n65_), .c(new_n149_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n148_), .c(new_n126_), .O(new_n151_));
  oai21  g089(.a(new_n70_), .b(new_n64_), .c(new_n73_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n151_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n73_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n65_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  inv1   g097(.a(new_n70_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n157_), .c(x23), .O(new_n162_));
  nand4  g100(.a(new_n160_), .b(new_n73_), .c(x20), .d(new_n159_), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(x18), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n162_), .c(x27), .O(new_n165_));
  nand3  g103(.a(new_n133_), .b(new_n154_), .c(new_n65_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n165_), .O(z13));
  nand3  g105(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n168_));
  nand4  g106(.a(new_n155_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n169_));
  nand3  g107(.a(x39), .b(new_n154_), .c(new_n65_), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n169_), .c(x23), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n73_), .c(new_n168_), .O(new_n173_));
  nand3  g111(.a(new_n93_), .b(new_n66_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n81_), .c(new_n80_), .O(new_n176_));
  oai21  g114(.a(new_n171_), .b(new_n149_), .c(new_n176_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n126_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n173_), .c(new_n152_), .O(z14));
  nand4  g117(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n78_), .O(z15));
  nand2  g119(.a(x22), .b(x01), .O(new_n182_));
  nor2   g120(.a(new_n182_), .b(x23), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(x22), .c(x01), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n73_), .c(new_n126_), .O(z17));
  nand2  g124(.a(new_n82_), .b(new_n64_), .O(new_n187_));
  nand2  g125(.a(new_n74_), .b(x29), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x08), .O(new_n190_));
  nor2   g128(.a(new_n64_), .b(x04), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n82_), .c(x30), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(new_n190_), .c(new_n78_), .d(new_n136_), .O(z18));
  aoi21  g131(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n194_));
  aoi21  g132(.a(x35), .b(x27), .c(x23), .O(new_n195_));
  nor2   g133(.a(x40), .b(x27), .O(new_n196_));
  nor2   g134(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  oai22  g135(.a(new_n197_), .b(x08), .c(new_n194_), .d(new_n140_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n80_), .c(new_n136_), .O(new_n199_));
  inv1   g137(.a(new_n199_), .O(z20));
  inv1   g138(.a(new_n145_), .O(z19));
endmodule


