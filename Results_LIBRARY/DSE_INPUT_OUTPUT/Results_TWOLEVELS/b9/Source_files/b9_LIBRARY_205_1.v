// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_;
  inv1   g000(.a(x39), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n65_), .d(new_n73_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n72_), .O(z00));
  inv1   g017(.a(new_n64_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  inv1   g021(.a(x36), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(x28), .c(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n80_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  aoi21  g029(.a(x28), .b(x04), .c(new_n63_), .O(new_n92_));
  oai21  g030(.a(x27), .b(x08), .c(x35), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x04), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n91_), .d(x40), .O(z02));
  nor2   g033(.a(x37), .b(x27), .O(new_n96_));
  aoi21  g034(.a(new_n68_), .b(x27), .c(new_n96_), .O(new_n97_));
  aoi22  g035(.a(new_n97_), .b(x21), .c(x40), .d(new_n63_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi22  g037(.a(new_n97_), .b(new_n99_), .c(x40), .d(new_n63_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n64_), .c(new_n67_), .O(z05));
  inv1   g040(.a(x28), .O(new_n103_));
  aoi21  g041(.a(new_n64_), .b(new_n66_), .c(new_n103_), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n65_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z07));
  nand2  g054(.a(x40), .b(x39), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n64_), .O(z09));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(x40), .b(x05), .O(new_n122_));
  oai21  g060(.a(x40), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n85_), .c(x27), .O(new_n124_));
  oai21  g062(.a(x32), .b(x30), .c(x40), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x05), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n73_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n64_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  oai21  g070(.a(new_n63_), .b(x29), .c(x40), .O(new_n133_));
  nand2  g071(.a(x27), .b(x04), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x35), .c(new_n103_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand3  g074(.a(x35), .b(new_n103_), .c(new_n73_), .O(new_n137_));
  aoi22  g075(.a(new_n137_), .b(x39), .c(new_n64_), .d(new_n66_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(x08), .c(new_n136_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n81_), .c(new_n132_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z11));
  nand3  g079(.a(new_n67_), .b(new_n84_), .c(new_n83_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  nor2   g081(.a(new_n69_), .b(new_n73_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x39), .O(new_n145_));
  oai21  g083(.a(new_n69_), .b(x40), .c(new_n145_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x40), .c(new_n147_), .d(new_n73_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand2  g090(.a(new_n68_), .b(new_n67_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n150_), .c(new_n63_), .O(new_n155_));
  nand4  g093(.a(new_n153_), .b(new_n75_), .c(x20), .d(new_n152_), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(x18), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n155_), .c(x27), .O(new_n158_));
  nand4  g096(.a(new_n126_), .b(x39), .c(new_n147_), .d(new_n73_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  aoi21  g098(.a(new_n147_), .b(new_n73_), .c(new_n63_), .O(new_n161_));
  nor2   g099(.a(x19), .b(x18), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n153_), .c(x27), .d(x20), .O(new_n163_));
  oai21  g101(.a(new_n161_), .b(new_n75_), .c(new_n163_), .O(new_n164_));
  nand3  g102(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n165_));
  nor2   g103(.a(x37), .b(x35), .O(new_n166_));
  aoi21  g104(.a(new_n165_), .b(new_n148_), .c(new_n166_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n103_), .c(x27), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(x39), .c(new_n82_), .d(new_n81_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n164_), .O(z14));
  inv1   g108(.a(x12), .O(new_n171_));
  nand4  g109(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(new_n171_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand4  g112(.a(new_n64_), .b(new_n174_), .c(x22), .d(x01), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z16));
  inv1   g114(.a(x01), .O(new_n177_));
  inv1   g115(.a(x24), .O(new_n178_));
  nand4  g116(.a(new_n64_), .b(new_n178_), .c(x23), .d(x22), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n177_), .O(z17));
  nand2  g118(.a(x27), .b(new_n73_), .O(new_n181_));
  nand2  g119(.a(new_n66_), .b(x08), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n181_), .c(new_n83_), .O(new_n183_));
  nand2  g121(.a(new_n81_), .b(new_n132_), .O(new_n184_));
  aoi21  g122(.a(new_n183_), .b(new_n103_), .c(new_n184_), .O(new_n185_));
  oai22  g123(.a(new_n185_), .b(new_n80_), .c(new_n117_), .d(new_n90_), .O(z18));
  nand2  g124(.a(new_n140_), .b(new_n64_), .O(z19));
  oai21  g125(.a(new_n103_), .b(x27), .c(x35), .O(new_n188_));
  and2   g126(.a(new_n188_), .b(new_n133_), .O(new_n189_));
  nand2  g127(.a(new_n64_), .b(new_n66_), .O(new_n190_));
  nand2  g128(.a(x39), .b(new_n83_), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n190_), .c(x08), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n189_), .c(new_n81_), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(x09), .O(z20));
endmodule


