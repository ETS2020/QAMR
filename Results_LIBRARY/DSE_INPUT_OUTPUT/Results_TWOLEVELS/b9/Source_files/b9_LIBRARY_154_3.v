// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n195_, new_n196_, new_n197_, new_n198_;
  nand2  g000(.a(x40), .b(x08), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x08), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g024(.a(x04), .O(new_n87_));
  oai21  g025(.a(x27), .b(new_n87_), .c(x02), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n72_), .O(new_n89_));
  nand2  g027(.a(new_n82_), .b(x04), .O(new_n90_));
  nand3  g028(.a(x28), .b(x27), .c(x04), .O(new_n91_));
  nand2  g029(.a(new_n63_), .b(x39), .O(new_n92_));
  aoi21  g030(.a(new_n91_), .b(new_n74_), .c(new_n92_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n90_), .c(new_n89_), .O(z02));
  nand2  g032(.a(new_n69_), .b(x27), .O(new_n95_));
  nand2  g033(.a(new_n68_), .b(new_n67_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n95_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g035(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n95_), .c(new_n64_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n63_), .c(new_n68_), .O(z05));
  aoi21  g039(.a(new_n63_), .b(new_n67_), .c(new_n81_), .O(new_n102_));
  oai21  g040(.a(new_n102_), .b(x37), .c(new_n63_), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n65_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n106_), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  oai21  g050(.a(new_n108_), .b(x31), .c(new_n112_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n63_), .c(x03), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z07));
  nand3  g053(.a(x40), .b(x39), .c(new_n72_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  aoi21  g058(.a(x35), .b(new_n81_), .c(x36), .O(new_n121_));
  oai21  g059(.a(x39), .b(x08), .c(x40), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x07), .O(new_n123_));
  nand4  g061(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n121_), .O(new_n125_));
  nand3  g063(.a(new_n63_), .b(x37), .c(x06), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n87_), .c(new_n127_), .O(new_n128_));
  inv1   g066(.a(x39), .O(new_n129_));
  nand2  g067(.a(new_n80_), .b(new_n79_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x40), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(new_n72_), .c(x05), .d(new_n87_), .O(new_n133_));
  oai21  g071(.a(new_n128_), .b(new_n67_), .c(new_n133_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  oai21  g073(.a(x40), .b(new_n67_), .c(x08), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x04), .O(new_n137_));
  nand2  g075(.a(new_n82_), .b(new_n63_), .O(new_n138_));
  nand2  g076(.a(new_n67_), .b(new_n72_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n137_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n79_), .c(new_n135_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n63_), .O(z11));
  inv1   g080(.a(x35), .O(new_n143_));
  inv1   g081(.a(x36), .O(new_n144_));
  nand3  g082(.a(new_n68_), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x27), .c(x10), .O(new_n146_));
  aoi21  g084(.a(x39), .b(new_n87_), .c(new_n70_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n146_), .c(new_n72_), .O(new_n148_));
  oai21  g086(.a(new_n70_), .b(x40), .c(new_n148_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x28), .c(new_n74_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x39), .c(new_n150_), .d(new_n87_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  nand2  g093(.a(new_n69_), .b(new_n68_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x20), .c(new_n155_), .d(new_n154_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nand4  g097(.a(new_n130_), .b(x39), .c(new_n150_), .d(new_n87_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n72_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x40), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n159_), .O(z13));
  nand3  g101(.a(x20), .b(new_n155_), .c(new_n154_), .O(new_n164_));
  nor2   g102(.a(new_n74_), .b(new_n129_), .O(new_n165_));
  nand4  g103(.a(new_n151_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n165_), .c(new_n150_), .d(new_n87_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand3  g106(.a(new_n68_), .b(new_n143_), .c(x28), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n139_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n80_), .c(new_n79_), .O(new_n171_));
  oai21  g109(.a(new_n129_), .b(x04), .c(new_n72_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(x40), .c(new_n150_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n69_), .c(new_n68_), .O(new_n174_));
  nand3  g112(.a(x39), .b(new_n150_), .c(new_n87_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n72_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x40), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n67_), .c(new_n64_), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n174_), .c(new_n171_), .d(new_n168_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nor2   g118(.a(new_n119_), .b(new_n180_), .O(z15));
  inv1   g119(.a(x23), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(x22), .c(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n63_), .O(z16));
  inv1   g122(.a(x01), .O(new_n185_));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n63_), .b(new_n186_), .c(x23), .d(x22), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(new_n185_), .O(z17));
  oai21  g126(.a(new_n82_), .b(x27), .c(new_n74_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x08), .O(new_n190_));
  nor2   g128(.a(new_n67_), .b(x04), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n83_), .c(x30), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n190_), .c(new_n135_), .O(z18));
  inv1   g131(.a(new_n141_), .O(z19));
  oai21  g132(.a(new_n143_), .b(new_n67_), .c(new_n72_), .O(new_n195_));
  aoi21  g133(.a(x28), .b(new_n67_), .c(new_n143_), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(x40), .c(new_n195_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n79_), .c(new_n135_), .O(new_n198_));
  inv1   g136(.a(new_n198_), .O(z20));
endmodule


