// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand4  g009(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(new_n71_), .c(new_n63_), .O(new_n74_));
  inv1   g012(.a(x16), .O(new_n75_));
  inv1   g013(.a(x13), .O(new_n76_));
  inv1   g014(.a(x22), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n74_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  oai21  g020(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n78_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(x40), .b(x39), .O(new_n90_));
  nor2   g028(.a(new_n66_), .b(x28), .O(new_n91_));
  oai21  g029(.a(x27), .b(x08), .c(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(new_n90_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n89_), .c(new_n78_), .O(z02));
  inv1   g032(.a(new_n78_), .O(new_n95_));
  nand2  g033(.a(x35), .b(x28), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  nand3  g036(.a(new_n95_), .b(new_n98_), .c(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n95_), .d(x21), .O(z03));
  nor2   g038(.a(new_n78_), .b(x21), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n78_), .O(z05));
  aoi22  g042(.a(x28), .b(x27), .c(x22), .d(x13), .O(new_n105_));
  and2   g043(.a(new_n105_), .b(new_n98_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n63_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x14), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n109_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  oai21  g053(.a(new_n111_), .b(x31), .c(new_n115_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n95_), .c(x03), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z07));
  nand3  g056(.a(new_n95_), .b(x40), .c(x39), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n95_), .O(z09));
  nand2  g060(.a(new_n90_), .b(x07), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n83_), .c(x27), .O(new_n126_));
  nand2  g064(.a(new_n82_), .b(new_n81_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(x40), .c(x39), .d(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n65_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n95_), .O(z10));
  inv1   g070(.a(x08), .O(new_n133_));
  oai21  g071(.a(new_n66_), .b(new_n64_), .c(new_n133_), .O(new_n134_));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n66_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n134_), .c(new_n78_), .O(new_n137_));
  inv1   g075(.a(new_n135_), .O(new_n138_));
  nor2   g076(.a(x28), .b(x04), .O(new_n139_));
  aoi21  g077(.a(x27), .b(x04), .c(x28), .O(new_n140_));
  oai22  g078(.a(new_n140_), .b(new_n138_), .c(new_n139_), .d(x08), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n137_), .c(new_n81_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(x09), .c(new_n95_), .O(z11));
  inv1   g081(.a(x10), .O(new_n144_));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n64_), .c(new_n144_), .O(new_n146_));
  oai22  g084(.a(new_n90_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n146_), .c(new_n78_), .O(z12));
  inv1   g086(.a(x40), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x28), .c(new_n149_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n76_), .d(new_n65_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  inv1   g092(.a(new_n70_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n152_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand3  g096(.a(new_n127_), .b(x40), .c(x39), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(x13), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n65_), .c(new_n78_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(z13));
  nand3  g100(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n163_));
  nor2   g101(.a(new_n90_), .b(x04), .O(new_n164_));
  nand4  g102(.a(new_n150_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n164_), .c(x13), .O(new_n166_));
  nor2   g104(.a(x22), .b(new_n76_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n166_), .c(new_n163_), .O(new_n168_));
  nand3  g106(.a(new_n98_), .b(new_n66_), .c(x28), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x27), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n82_), .c(new_n81_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n147_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n76_), .O(new_n173_));
  nand2  g111(.a(new_n155_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n77_), .c(x13), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n173_), .c(new_n168_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n95_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand3  g117(.a(x22), .b(new_n76_), .c(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n179_), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  nor3   g121(.a(new_n180_), .b(x24), .c(new_n179_), .O(z17));
  inv1   g122(.a(x09), .O(new_n185_));
  nand2  g123(.a(new_n91_), .b(new_n64_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n135_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(x08), .O(new_n188_));
  nor2   g126(.a(new_n64_), .b(x04), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n91_), .c(x30), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n188_), .c(new_n95_), .d(new_n185_), .O(z18));
  aoi21  g129(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n138_), .c(new_n134_), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(new_n95_), .c(new_n81_), .d(new_n185_), .O(new_n194_));
  inv1   g132(.a(new_n194_), .O(z20));
  oai21  g133(.a(new_n142_), .b(x09), .c(new_n95_), .O(z19));
endmodule


