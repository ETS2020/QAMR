// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n109_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n74_), .c(new_n63_), .O(new_n80_));
  nand2  g018(.a(x40), .b(x29), .O(new_n81_));
  and2   g019(.a(new_n81_), .b(x16), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(z00));
  inv1   g021(.a(new_n81_), .O(new_n84_));
  inv1   g022(.a(x30), .O(new_n85_));
  inv1   g023(.a(x32), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  aoi21  g025(.a(x35), .b(new_n87_), .c(x36), .O(new_n88_));
  inv1   g026(.a(new_n88_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n86_), .c(new_n85_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(new_n84_), .O(z01));
  inv1   g030(.a(x08), .O(new_n93_));
  nor2   g031(.a(x28), .b(new_n93_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n76_), .c(x35), .O(new_n95_));
  oai21  g033(.a(x08), .b(new_n65_), .c(new_n95_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n64_), .O(new_n97_));
  oai21  g035(.a(new_n66_), .b(x28), .c(x04), .O(new_n98_));
  oai21  g036(.a(x28), .b(new_n64_), .c(x04), .O(new_n99_));
  nand3  g037(.a(new_n81_), .b(x39), .c(x02), .O(new_n100_));
  aoi21  g038(.a(new_n99_), .b(new_n76_), .c(new_n100_), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n98_), .c(new_n97_), .O(z02));
  nand2  g040(.a(new_n71_), .b(x27), .O(new_n103_));
  inv1   g041(.a(x21), .O(new_n104_));
  aoi21  g042(.a(new_n70_), .b(new_n64_), .c(new_n104_), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n103_), .c(new_n84_), .O(z03));
  nand2  g044(.a(new_n70_), .b(new_n64_), .O(new_n107_));
  nand4  g045(.a(new_n107_), .b(new_n103_), .c(new_n81_), .d(new_n104_), .O(z04));
  nand2  g046(.a(x28), .b(x27), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n81_), .c(new_n70_), .O(z05));
  inv1   g048(.a(z05), .O(z06));
  inv1   g049(.a(x33), .O(new_n112_));
  nand2  g050(.a(x17), .b(new_n63_), .O(new_n113_));
  inv1   g051(.a(x00), .O(new_n114_));
  oai21  g052(.a(x25), .b(new_n114_), .c(x38), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(x14), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n114_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  oai21  g058(.a(new_n116_), .b(x31), .c(new_n120_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x03), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n81_), .O(z07));
  inv1   g061(.a(x29), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(z08));
  inv1   g064(.a(x11), .O(new_n127_));
  nand4  g065(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n127_), .O(z09));
  oai21  g067(.a(x39), .b(x29), .c(x40), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x07), .O(new_n131_));
  nand4  g069(.a(x40), .b(x39), .c(new_n124_), .d(x05), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n131_), .c(new_n88_), .O(new_n133_));
  nand3  g071(.a(new_n81_), .b(x37), .c(x06), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n133_), .b(new_n65_), .c(new_n135_), .O(new_n136_));
  nor2   g074(.a(x32), .b(x30), .O(new_n137_));
  nor3   g075(.a(new_n137_), .b(new_n76_), .c(new_n75_), .O(new_n138_));
  nand4  g076(.a(new_n138_), .b(new_n124_), .c(x05), .d(new_n65_), .O(new_n139_));
  oai21  g077(.a(new_n136_), .b(new_n64_), .c(new_n139_), .O(z10));
  inv1   g078(.a(x09), .O(new_n141_));
  aoi21  g079(.a(new_n64_), .b(new_n93_), .c(new_n66_), .O(new_n142_));
  aoi21  g080(.a(x27), .b(x04), .c(x28), .O(new_n143_));
  oai21  g081(.a(new_n142_), .b(new_n84_), .c(new_n143_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n85_), .c(new_n141_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n81_), .O(z11));
  inv1   g084(.a(x10), .O(new_n147_));
  nor2   g085(.a(x29), .b(x27), .O(new_n148_));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n147_), .O(new_n150_));
  nand2  g088(.a(new_n77_), .b(new_n65_), .O(new_n151_));
  oai21  g089(.a(new_n73_), .b(new_n64_), .c(new_n151_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n150_), .c(new_n81_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n76_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n65_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  nand4  g097(.a(new_n72_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n157_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x27), .O(new_n162_));
  nor2   g100(.a(new_n137_), .b(new_n75_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n154_), .c(new_n65_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n124_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x40), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n162_), .O(z13));
  nor2   g105(.a(new_n137_), .b(x13), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(x29), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n151_), .c(new_n64_), .O(new_n170_));
  nand3  g108(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n171_));
  nand4  g109(.a(new_n155_), .b(new_n86_), .c(new_n85_), .d(x28), .O(new_n172_));
  nor2   g110(.a(x13), .b(x04), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n172_), .c(new_n77_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n171_), .O(new_n175_));
  aoi22  g113(.a(new_n173_), .b(new_n77_), .c(x35), .d(x28), .O(new_n176_));
  nand4  g114(.a(new_n66_), .b(new_n86_), .c(new_n85_), .d(x28), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n176_), .c(new_n70_), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(new_n175_), .c(new_n170_), .d(new_n81_), .O(z14));
  nand4  g118(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n81_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand4  g121(.a(new_n81_), .b(new_n183_), .c(x22), .d(x01), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(z16));
  inv1   g123(.a(x01), .O(new_n186_));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n81_), .b(new_n187_), .c(x23), .d(x22), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n186_), .O(z17));
  nand2  g127(.a(x27), .b(new_n65_), .O(new_n190_));
  oai21  g128(.a(x27), .b(new_n93_), .c(new_n190_), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(x35), .c(new_n87_), .O(new_n192_));
  nor2   g130(.a(x30), .b(x09), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(new_n192_), .c(new_n84_), .O(z18));
  oai21  g132(.a(new_n94_), .b(x27), .c(x35), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n81_), .c(new_n85_), .d(new_n141_), .O(new_n196_));
  inv1   g134(.a(new_n196_), .O(z20));
  nand2  g135(.a(new_n145_), .b(new_n81_), .O(z19));
endmodule


