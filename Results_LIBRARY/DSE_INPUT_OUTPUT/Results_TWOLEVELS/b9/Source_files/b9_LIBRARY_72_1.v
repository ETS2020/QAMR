// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x11), .O(new_n64_));
  nand2  g002(.a(x40), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x15), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n63_), .c(new_n65_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(x11), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n71_), .O(z00));
  inv1   g016(.a(new_n65_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  inv1   g030(.a(x04), .O(new_n93_));
  oai21  g031(.a(x08), .b(new_n93_), .c(x40), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g033(.a(new_n84_), .b(x04), .O(new_n96_));
  oai21  g034(.a(x28), .b(new_n92_), .c(x04), .O(new_n97_));
  nand2  g035(.a(new_n65_), .b(x39), .O(new_n98_));
  aoi21  g036(.a(new_n97_), .b(new_n74_), .c(new_n98_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n96_), .c(new_n95_), .d(new_n91_), .O(z02));
  nand2  g038(.a(new_n67_), .b(x27), .O(new_n101_));
  inv1   g039(.a(x21), .O(new_n102_));
  aoi21  g040(.a(new_n66_), .b(new_n92_), .c(new_n102_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n101_), .c(new_n79_), .O(z03));
  nand2  g042(.a(new_n66_), .b(new_n92_), .O(new_n105_));
  nand4  g043(.a(new_n105_), .b(new_n101_), .c(new_n65_), .d(new_n102_), .O(z04));
  aoi21  g044(.a(x28), .b(x27), .c(x37), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(new_n79_), .O(z05));
  aoi21  g046(.a(new_n65_), .b(new_n92_), .c(new_n83_), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(x37), .c(new_n65_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n72_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n65_), .c(x03), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z07));
  nand3  g059(.a(x40), .b(x39), .c(x11), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n65_), .O(z09));
  inv1   g063(.a(x07), .O(new_n126_));
  inv1   g064(.a(x39), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(x11), .c(new_n74_), .O(new_n128_));
  nand4  g066(.a(x40), .b(x39), .c(x11), .d(x05), .O(new_n129_));
  oai21  g067(.a(new_n128_), .b(new_n126_), .c(new_n129_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n85_), .c(new_n93_), .O(new_n131_));
  nand3  g069(.a(new_n65_), .b(x37), .c(x06), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x27), .O(new_n134_));
  nor2   g072(.a(x32), .b(x30), .O(new_n135_));
  nor3   g073(.a(new_n135_), .b(new_n74_), .c(new_n127_), .O(new_n136_));
  nand4  g074(.a(new_n136_), .b(x11), .c(x05), .d(new_n93_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(z10));
  nand2  g076(.a(new_n65_), .b(new_n92_), .O(new_n139_));
  oai21  g077(.a(new_n84_), .b(x04), .c(x11), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n139_), .c(x08), .O(new_n141_));
  inv1   g079(.a(new_n84_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x11), .c(new_n74_), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n142_), .c(new_n144_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n141_), .c(new_n80_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(x09), .O(z11));
  nor2   g086(.a(x27), .b(new_n64_), .O(new_n149_));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n149_), .c(new_n73_), .O(new_n151_));
  nor2   g089(.a(new_n74_), .b(new_n127_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n93_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n69_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n151_), .c(new_n65_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n74_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n93_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n68_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nor4   g102(.a(new_n135_), .b(new_n127_), .c(x13), .d(x04), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n64_), .c(x40), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n164_), .O(z13));
  nor2   g105(.a(new_n135_), .b(x13), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(new_n64_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n153_), .c(new_n92_), .O(new_n170_));
  nand3  g108(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n171_));
  nand4  g109(.a(new_n157_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n172_));
  nor2   g110(.a(x13), .b(x04), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n172_), .c(new_n152_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n171_), .O(new_n175_));
  aoi22  g113(.a(new_n173_), .b(new_n152_), .c(x35), .d(x28), .O(new_n176_));
  inv1   g114(.a(x35), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n176_), .c(new_n66_), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n175_), .c(new_n170_), .d(new_n65_), .O(z14));
  inv1   g119(.a(x12), .O(new_n182_));
  nand4  g120(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n182_), .O(z15));
  inv1   g122(.a(x23), .O(new_n185_));
  nand4  g123(.a(new_n65_), .b(new_n185_), .c(x22), .d(x01), .O(new_n186_));
  inv1   g124(.a(new_n186_), .O(z16));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n188_), .b(x23), .c(x22), .d(x01), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n65_), .O(z17));
  nand2  g128(.a(x27), .b(new_n93_), .O(new_n191_));
  nand2  g129(.a(new_n92_), .b(x08), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n191_), .c(new_n177_), .O(new_n193_));
  inv1   g131(.a(x09), .O(new_n194_));
  nand2  g132(.a(new_n80_), .b(new_n194_), .O(new_n195_));
  aoi21  g133(.a(new_n193_), .b(new_n83_), .c(new_n195_), .O(new_n196_));
  nand4  g134(.a(new_n152_), .b(x29), .c(x11), .d(x08), .O(new_n197_));
  oai21  g135(.a(new_n196_), .b(new_n79_), .c(new_n197_), .O(z18));
  aoi21  g136(.a(x28), .b(new_n92_), .c(new_n177_), .O(new_n199_));
  nor2   g137(.a(new_n199_), .b(new_n144_), .O(new_n200_));
  oai21  g138(.a(new_n177_), .b(new_n92_), .c(x11), .O(new_n201_));
  nand2  g139(.a(new_n74_), .b(new_n92_), .O(new_n202_));
  aoi21  g140(.a(new_n202_), .b(new_n201_), .c(x08), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n200_), .c(new_n80_), .O(new_n204_));
  nor2   g142(.a(new_n204_), .b(x09), .O(z20));
  nor2   g143(.a(new_n147_), .b(x09), .O(z19));
endmodule


