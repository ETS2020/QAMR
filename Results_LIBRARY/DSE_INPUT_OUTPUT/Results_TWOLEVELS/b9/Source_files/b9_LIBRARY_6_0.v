// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n212_;
  nor2   g000(.a(x40), .b(x07), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(x15), .c(x16), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n70_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  aoi22  g027(.a(new_n89_), .b(new_n88_), .c(new_n83_), .d(x04), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n91_), .c(x04), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n90_), .c(x40), .d(x39), .O(z02));
  nand2  g032(.a(new_n66_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n65_), .b(new_n92_), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n95_), .c(new_n63_), .O(z03));
  aoi21  g036(.a(new_n65_), .b(new_n92_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n63_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n63_), .O(z05));
  nor2   g040(.a(new_n63_), .b(x27), .O(new_n103_));
  oai21  g041(.a(new_n103_), .b(new_n82_), .c(new_n65_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n64_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n72_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z07));
  inv1   g054(.a(x39), .O(new_n117_));
  nor2   g055(.a(new_n74_), .b(new_n117_), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n64_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n64_), .O(z09));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(x39), .b(x05), .O(new_n124_));
  oai21  g062(.a(x39), .b(new_n123_), .c(new_n124_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n84_), .O(new_n126_));
  nand3  g064(.a(x35), .b(new_n82_), .c(x07), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n81_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n74_), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n126_), .c(new_n92_), .O(new_n130_));
  oai21  g068(.a(x32), .b(x30), .c(x40), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n117_), .O(new_n132_));
  and2   g070(.a(new_n132_), .b(x05), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n130_), .c(new_n71_), .O(new_n134_));
  nor2   g072(.a(new_n65_), .b(new_n92_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x06), .c(new_n63_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand3  g076(.a(x35), .b(new_n82_), .c(new_n71_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x40), .c(new_n103_), .O(new_n140_));
  nand2  g078(.a(x39), .b(x29), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x40), .O(new_n142_));
  nand2  g080(.a(new_n74_), .b(x07), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(x35), .c(new_n82_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  oai21  g085(.a(new_n140_), .b(x08), .c(new_n147_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n79_), .c(new_n138_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n64_), .O(z11));
  nor3   g088(.a(x37), .b(x36), .c(x35), .O(new_n151_));
  nor2   g089(.a(new_n74_), .b(x27), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n151_), .c(new_n73_), .O(new_n153_));
  nand2  g091(.a(new_n118_), .b(new_n71_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n66_), .c(new_n65_), .O(new_n155_));
  aoi21  g093(.a(x27), .b(x07), .c(x40), .O(new_n156_));
  oai21  g094(.a(new_n74_), .b(new_n71_), .c(x39), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n92_), .c(new_n156_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n155_), .c(new_n153_), .O(z12));
  inv1   g097(.a(x13), .O(new_n160_));
  nand2  g098(.a(x36), .b(x35), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x28), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x39), .c(new_n160_), .d(new_n71_), .O(new_n163_));
  inv1   g101(.a(x18), .O(new_n164_));
  inv1   g102(.a(x19), .O(new_n165_));
  nand4  g103(.a(new_n67_), .b(x20), .c(new_n165_), .d(new_n164_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n163_), .c(new_n74_), .O(new_n167_));
  nor2   g105(.a(new_n166_), .b(new_n123_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n167_), .c(x27), .O(new_n169_));
  nand3  g107(.a(new_n132_), .b(new_n160_), .c(new_n71_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(z13));
  nand3  g109(.a(x20), .b(new_n165_), .c(new_n164_), .O(new_n172_));
  nand4  g110(.a(new_n161_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n118_), .c(new_n160_), .d(new_n71_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n172_), .O(new_n175_));
  inv1   g113(.a(x35), .O(new_n176_));
  nand3  g114(.a(new_n65_), .b(new_n176_), .c(x28), .O(new_n177_));
  oai21  g115(.a(new_n74_), .b(x27), .c(new_n177_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n80_), .c(new_n79_), .O(new_n179_));
  oai21  g117(.a(new_n74_), .b(x27), .c(new_n67_), .O(new_n180_));
  oai21  g118(.a(x13), .b(x04), .c(new_n180_), .O(new_n181_));
  aoi21  g119(.a(new_n119_), .b(new_n68_), .c(new_n63_), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(new_n181_), .c(new_n179_), .d(new_n175_), .O(z14));
  inv1   g121(.a(x12), .O(new_n184_));
  nand4  g122(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n185_));
  nor2   g123(.a(new_n185_), .b(new_n184_), .O(z15));
  inv1   g124(.a(x23), .O(new_n187_));
  nand4  g125(.a(new_n64_), .b(new_n187_), .c(x22), .d(x01), .O(new_n188_));
  inv1   g126(.a(new_n188_), .O(z16));
  inv1   g127(.a(x01), .O(new_n190_));
  inv1   g128(.a(x24), .O(new_n191_));
  nand4  g129(.a(new_n64_), .b(new_n191_), .c(x23), .d(x22), .O(new_n192_));
  nor2   g130(.a(new_n192_), .b(new_n190_), .O(z17));
  nand2  g131(.a(new_n74_), .b(new_n123_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(x27), .c(new_n71_), .O(new_n195_));
  nand2  g133(.a(new_n103_), .b(x08), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(x35), .c(new_n82_), .O(new_n198_));
  inv1   g136(.a(new_n89_), .O(new_n199_));
  nand2  g137(.a(new_n79_), .b(new_n138_), .O(new_n200_));
  aoi22  g138(.a(new_n200_), .b(new_n64_), .c(new_n118_), .d(new_n199_), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n198_), .O(z18));
  inv1   g140(.a(new_n149_), .O(z19));
  oai22  g141(.a(x40), .b(x35), .c(x27), .d(x08), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(x07), .O(new_n205_));
  aoi21  g143(.a(x35), .b(x27), .c(x08), .O(new_n206_));
  aoi21  g144(.a(x39), .b(x29), .c(x35), .O(new_n207_));
  oai21  g145(.a(new_n207_), .b(new_n206_), .c(x40), .O(new_n208_));
  nand2  g146(.a(new_n118_), .b(x29), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(x28), .c(new_n92_), .O(new_n210_));
  nand3  g148(.a(new_n210_), .b(new_n208_), .c(new_n205_), .O(new_n211_));
  nand3  g149(.a(new_n211_), .b(new_n79_), .c(new_n138_), .O(new_n212_));
  nand2  g150(.a(new_n212_), .b(new_n64_), .O(z20));
endmodule


