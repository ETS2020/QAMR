// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n174_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  nand3  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand3  g009(.a(new_n71_), .b(x27), .c(new_n63_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x10), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(new_n72_), .c(x16), .O(z00));
  inv1   g012(.a(x30), .O(new_n75_));
  inv1   g013(.a(x32), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n73_), .c(x04), .O(z01));
  inv1   g020(.a(x10), .O(new_n83_));
  aoi21  g021(.a(x29), .b(x08), .c(x02), .O(new_n84_));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n78_), .c(x04), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x39), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n84_), .c(new_n83_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x40), .O(z02));
  inv1   g027(.a(new_n73_), .O(new_n90_));
  nand2  g028(.a(new_n70_), .b(x27), .O(new_n91_));
  inv1   g029(.a(x21), .O(new_n92_));
  inv1   g030(.a(x27), .O(new_n93_));
  aoi21  g031(.a(new_n64_), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n91_), .c(new_n90_), .O(z03));
  nand2  g033(.a(new_n64_), .b(new_n93_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n91_), .c(new_n73_), .d(new_n92_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n90_), .O(z05));
  aoi21  g037(.a(new_n73_), .b(new_n93_), .c(new_n77_), .O(new_n100_));
  oai21  g038(.a(new_n100_), .b(x37), .c(new_n73_), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n63_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  oai22  g047(.a(new_n109_), .b(x14), .c(new_n106_), .d(x31), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n73_), .c(x03), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z07));
  nand3  g050(.a(x40), .b(x39), .c(new_n83_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand4  g053(.a(new_n73_), .b(x34), .c(x27), .d(x26), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  inv1   g055(.a(x05), .O(new_n118_));
  nand2  g056(.a(x40), .b(x39), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x07), .O(new_n120_));
  oai21  g058(.a(new_n119_), .b(new_n118_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n79_), .c(x27), .O(new_n122_));
  inv1   g060(.a(x39), .O(new_n123_));
  oai21  g061(.a(x32), .b(x30), .c(x40), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x05), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n65_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n73_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  nand3  g069(.a(x35), .b(new_n77_), .c(new_n65_), .O(new_n132_));
  aoi22  g070(.a(new_n132_), .b(new_n83_), .c(new_n73_), .d(new_n93_), .O(new_n133_));
  inv1   g071(.a(x40), .O(new_n134_));
  aoi21  g072(.a(x39), .b(x29), .c(x10), .O(new_n135_));
  nor2   g073(.a(new_n93_), .b(new_n65_), .O(new_n136_));
  oai22  g074(.a(new_n136_), .b(new_n78_), .c(new_n135_), .d(new_n134_), .O(new_n137_));
  oai21  g075(.a(new_n133_), .b(x08), .c(new_n137_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n75_), .c(new_n131_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  inv1   g078(.a(new_n119_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n65_), .O(new_n142_));
  aoi22  g080(.a(new_n142_), .b(new_n70_), .c(new_n67_), .d(new_n66_), .O(new_n143_));
  nor2   g081(.a(new_n90_), .b(new_n93_), .O(new_n144_));
  oai21  g082(.a(new_n143_), .b(x37), .c(new_n144_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x28), .c(new_n134_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x39), .c(new_n146_), .d(new_n65_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand2  g089(.a(new_n70_), .b(new_n64_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n149_), .c(x10), .O(new_n154_));
  nand4  g092(.a(new_n152_), .b(new_n134_), .c(x20), .d(new_n151_), .O(new_n155_));
  nor2   g093(.a(new_n155_), .b(x18), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n154_), .c(x27), .O(new_n157_));
  nand4  g095(.a(new_n125_), .b(new_n146_), .c(new_n83_), .d(new_n65_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(z13));
  nand3  g097(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n160_));
  oai21  g098(.a(x13), .b(x04), .c(new_n83_), .O(new_n161_));
  nand4  g099(.a(new_n147_), .b(new_n76_), .c(new_n75_), .d(x28), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n161_), .c(new_n141_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  nand3  g102(.a(new_n64_), .b(new_n66_), .c(x28), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x27), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n76_), .c(new_n75_), .O(new_n167_));
  nand2  g105(.a(new_n152_), .b(x27), .O(new_n168_));
  nand3  g106(.a(new_n141_), .b(new_n146_), .c(new_n65_), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n168_), .c(new_n90_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n167_), .c(new_n164_), .O(z14));
  inv1   g109(.a(x12), .O(new_n172_));
  nor2   g110(.a(new_n116_), .b(new_n172_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand4  g112(.a(new_n73_), .b(new_n174_), .c(x22), .d(x01), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(x23), .c(x22), .d(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n73_), .O(z17));
  nand2  g117(.a(x27), .b(new_n65_), .O(new_n180_));
  nand2  g118(.a(new_n93_), .b(x08), .O(new_n181_));
  aoi21  g119(.a(new_n181_), .b(new_n180_), .c(new_n66_), .O(new_n182_));
  nand2  g120(.a(new_n75_), .b(new_n131_), .O(new_n183_));
  aoi21  g121(.a(new_n182_), .b(new_n77_), .c(new_n183_), .O(new_n184_));
  nand4  g122(.a(new_n141_), .b(x29), .c(new_n83_), .d(x08), .O(new_n185_));
  oai21  g123(.a(new_n184_), .b(new_n90_), .c(new_n185_), .O(z18));
  nand2  g124(.a(x39), .b(x29), .O(new_n187_));
  nor2   g125(.a(new_n77_), .b(x27), .O(new_n188_));
  nor2   g126(.a(x35), .b(x10), .O(new_n189_));
  oai21  g127(.a(new_n189_), .b(new_n188_), .c(new_n187_), .O(new_n190_));
  inv1   g128(.a(x08), .O(new_n191_));
  aoi21  g129(.a(x35), .b(x27), .c(x10), .O(new_n192_));
  nor2   g130(.a(x40), .b(x27), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n192_), .c(new_n191_), .O(new_n194_));
  oai21  g132(.a(new_n188_), .b(new_n66_), .c(new_n134_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n194_), .c(new_n190_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n75_), .c(new_n131_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n73_), .O(z20));
  inv1   g136(.a(new_n139_), .O(z19));
endmodule


