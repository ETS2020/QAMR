// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  nand2  g015(.a(x40), .b(x05), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g017(.a(new_n78_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  oai21  g020(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n80_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  oai21  g028(.a(x35), .b(x05), .c(new_n90_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(x05), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x04), .c(new_n72_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n89_), .d(x40), .O(z02));
  nand2  g033(.a(x35), .b(x28), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x21), .O(new_n98_));
  inv1   g036(.a(x37), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n64_), .c(new_n98_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n97_), .c(new_n80_), .O(z03));
  nand2  g039(.a(new_n99_), .b(new_n64_), .O(new_n102_));
  nand4  g040(.a(new_n102_), .b(new_n97_), .c(new_n78_), .d(new_n98_), .O(z04));
  aoi21  g041(.a(x28), .b(x27), .c(x37), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(new_n80_), .O(z05));
  aoi21  g043(.a(x40), .b(x05), .c(x27), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(new_n90_), .c(new_n99_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n78_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n78_), .O(z07));
  oai21  g057(.a(x39), .b(x05), .c(x40), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n78_), .O(z09));
  nand3  g061(.a(new_n78_), .b(x37), .c(x06), .O(new_n124_));
  nand4  g062(.a(new_n120_), .b(new_n83_), .c(x07), .d(new_n65_), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n64_), .O(z10));
  inv1   g064(.a(x09), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(x40), .b(x04), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x35), .c(new_n90_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n128_), .c(new_n106_), .O(new_n131_));
  aoi21  g069(.a(x39), .b(x29), .c(x05), .O(new_n132_));
  nand2  g070(.a(x27), .b(x04), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x35), .c(new_n90_), .O(new_n134_));
  oai21  g072(.a(new_n132_), .b(new_n73_), .c(new_n134_), .O(new_n135_));
  oai21  g073(.a(new_n131_), .b(x08), .c(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n81_), .c(new_n127_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  inv1   g076(.a(x10), .O(new_n139_));
  nand3  g077(.a(new_n99_), .b(new_n67_), .c(new_n66_), .O(new_n140_));
  nor2   g078(.a(new_n73_), .b(x27), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n128_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n139_), .O(new_n144_));
  oai21  g082(.a(x27), .b(new_n65_), .c(new_n128_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x40), .O(new_n146_));
  oai21  g084(.a(new_n72_), .b(x04), .c(new_n128_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x40), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n96_), .c(new_n99_), .O(new_n149_));
  nand2  g087(.a(new_n120_), .b(new_n64_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n149_), .c(new_n146_), .d(new_n144_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n73_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n65_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  inv1   g095(.a(new_n70_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand2  g099(.a(new_n82_), .b(new_n81_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x39), .c(new_n152_), .d(new_n65_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n128_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x40), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n161_), .O(z13));
  nand3  g104(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n167_));
  nand4  g105(.a(new_n153_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n168_));
  oai21  g106(.a(x13), .b(x04), .c(x40), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n168_), .c(x39), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n128_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x40), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n167_), .O(new_n173_));
  nand3  g111(.a(new_n99_), .b(new_n66_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n82_), .c(new_n81_), .O(new_n176_));
  oai22  g114(.a(new_n141_), .b(new_n70_), .c(x13), .d(x04), .O(new_n177_));
  oai21  g115(.a(new_n70_), .b(new_n64_), .c(new_n72_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n177_), .c(new_n176_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n128_), .O(new_n180_));
  oai21  g118(.a(new_n70_), .b(new_n64_), .c(new_n73_), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n180_), .c(new_n173_), .O(z14));
  inv1   g120(.a(x12), .O(new_n183_));
  nand4  g121(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n183_), .O(z15));
  inv1   g123(.a(x23), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(x22), .c(x01), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n78_), .O(z16));
  inv1   g126(.a(x01), .O(new_n189_));
  inv1   g127(.a(x24), .O(new_n190_));
  nand4  g128(.a(new_n78_), .b(new_n190_), .c(x23), .d(x22), .O(new_n191_));
  nor2   g129(.a(new_n191_), .b(new_n189_), .O(z17));
  nand2  g130(.a(x27), .b(new_n65_), .O(new_n193_));
  nand2  g131(.a(new_n64_), .b(x08), .O(new_n194_));
  aoi21  g132(.a(new_n194_), .b(new_n193_), .c(new_n66_), .O(new_n195_));
  nand2  g133(.a(new_n81_), .b(new_n127_), .O(new_n196_));
  aoi21  g134(.a(new_n195_), .b(new_n90_), .c(new_n196_), .O(new_n197_));
  nand4  g135(.a(new_n74_), .b(x29), .c(x08), .d(new_n128_), .O(new_n198_));
  oai21  g136(.a(new_n197_), .b(new_n80_), .c(new_n198_), .O(z18));
  nor2   g137(.a(new_n132_), .b(new_n73_), .O(new_n200_));
  aoi21  g138(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n201_));
  nand2  g139(.a(x35), .b(x27), .O(new_n202_));
  aoi22  g140(.a(new_n202_), .b(new_n128_), .c(new_n73_), .d(new_n64_), .O(new_n203_));
  oai22  g141(.a(new_n203_), .b(x08), .c(new_n201_), .d(new_n200_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n81_), .c(new_n127_), .O(new_n205_));
  inv1   g143(.a(new_n205_), .O(z20));
  inv1   g144(.a(new_n137_), .O(z19));
endmodule


