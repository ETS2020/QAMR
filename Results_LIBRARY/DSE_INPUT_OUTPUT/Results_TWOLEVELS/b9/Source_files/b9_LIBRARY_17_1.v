// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  inv1   g017(.a(x25), .O(new_n80_));
  nand2  g018(.a(x40), .b(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n79_), .c(x16), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  oai21  g022(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n81_), .c(x04), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g029(.a(x28), .O(new_n92_));
  oai21  g030(.a(x35), .b(new_n80_), .c(new_n92_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x04), .c(new_n80_), .O(new_n94_));
  inv1   g032(.a(x39), .O(new_n95_));
  nor2   g033(.a(x27), .b(x08), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x04), .c(new_n95_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n94_), .c(new_n91_), .d(x40), .O(z02));
  nand2  g036(.a(new_n71_), .b(x27), .O(new_n99_));
  nand2  g037(.a(new_n70_), .b(new_n64_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n81_), .d(x21), .O(z03));
  inv1   g039(.a(new_n81_), .O(new_n102_));
  aoi21  g040(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n99_), .c(new_n102_), .O(z04));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n70_), .c(new_n102_), .O(z05));
  nand3  g044(.a(new_n105_), .b(new_n81_), .c(new_n70_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z06));
  inv1   g046(.a(x03), .O(new_n109_));
  inv1   g047(.a(x31), .O(new_n110_));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n63_), .O(new_n112_));
  inv1   g050(.a(x40), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n80_), .c(x00), .O(new_n114_));
  oai21  g052(.a(new_n102_), .b(x38), .c(new_n114_), .O(new_n115_));
  nand4  g053(.a(new_n115_), .b(new_n112_), .c(new_n111_), .d(new_n110_), .O(new_n116_));
  inv1   g054(.a(x14), .O(new_n117_));
  inv1   g055(.a(x00), .O(new_n118_));
  aoi21  g056(.a(x38), .b(new_n118_), .c(x40), .O(new_n119_));
  oai21  g057(.a(new_n119_), .b(x25), .c(new_n117_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n116_), .c(new_n109_), .O(z07));
  aoi21  g059(.a(new_n95_), .b(x25), .c(new_n113_), .O(z08));
  nand4  g060(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n81_), .O(z09));
  inv1   g062(.a(x05), .O(new_n125_));
  nand2  g063(.a(new_n75_), .b(x07), .O(new_n126_));
  oai21  g064(.a(new_n75_), .b(new_n125_), .c(new_n126_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n85_), .c(x27), .O(new_n128_));
  nor2   g066(.a(x32), .b(x30), .O(new_n129_));
  nor3   g067(.a(new_n129_), .b(new_n113_), .c(new_n95_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x05), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n65_), .O(new_n133_));
  nand3  g071(.a(x37), .b(x27), .c(x06), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n81_), .O(z10));
  nand2  g073(.a(new_n81_), .b(new_n64_), .O(new_n136_));
  nand2  g074(.a(x40), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n92_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x25), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n136_), .c(x08), .O(new_n140_));
  nor2   g078(.a(new_n66_), .b(x28), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x25), .c(new_n113_), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n141_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n140_), .c(new_n83_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(x09), .c(new_n81_), .O(z11));
  inv1   g085(.a(x10), .O(new_n148_));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  nor2   g087(.a(new_n113_), .b(x27), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n149_), .c(new_n148_), .O(new_n151_));
  oai22  g089(.a(new_n75_), .b(x04), .c(new_n73_), .d(new_n64_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n151_), .c(new_n81_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n113_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n65_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  nand4  g097(.a(new_n72_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n157_), .c(new_n80_), .O(new_n161_));
  nand4  g099(.a(new_n72_), .b(new_n113_), .c(x20), .d(new_n159_), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(x18), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n161_), .c(x27), .O(new_n164_));
  nand4  g102(.a(new_n130_), .b(x25), .c(new_n154_), .d(new_n65_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(z13));
  oai21  g104(.a(x27), .b(new_n154_), .c(x25), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x40), .O(new_n168_));
  nand3  g106(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n169_));
  nand4  g107(.a(new_n155_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n170_));
  nor2   g108(.a(x13), .b(x04), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n170_), .c(new_n76_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n169_), .O(new_n173_));
  inv1   g111(.a(new_n71_), .O(new_n174_));
  aoi21  g112(.a(new_n171_), .b(new_n76_), .c(new_n174_), .O(new_n175_));
  nand4  g113(.a(new_n66_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n175_), .c(new_n70_), .O(new_n178_));
  oai21  g116(.a(x32), .b(x30), .c(new_n65_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n75_), .c(new_n64_), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n178_), .c(new_n173_), .d(new_n168_), .O(z14));
  inv1   g119(.a(x12), .O(new_n182_));
  nand4  g120(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n182_), .O(z15));
  inv1   g122(.a(x23), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(x22), .c(x01), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n81_), .O(z16));
  inv1   g125(.a(x01), .O(new_n188_));
  inv1   g126(.a(x24), .O(new_n189_));
  nand4  g127(.a(new_n81_), .b(new_n189_), .c(x23), .d(x22), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n188_), .O(z17));
  nand2  g129(.a(x27), .b(new_n65_), .O(new_n192_));
  nand2  g130(.a(new_n64_), .b(x08), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(new_n192_), .c(new_n66_), .O(new_n194_));
  inv1   g132(.a(x09), .O(new_n195_));
  nand2  g133(.a(new_n83_), .b(new_n195_), .O(new_n196_));
  aoi21  g134(.a(new_n194_), .b(new_n92_), .c(new_n196_), .O(new_n197_));
  nand4  g135(.a(new_n76_), .b(x29), .c(x25), .d(x08), .O(new_n198_));
  oai21  g136(.a(new_n197_), .b(new_n102_), .c(new_n198_), .O(z18));
  nor2   g137(.a(new_n146_), .b(x09), .O(z19));
  aoi21  g138(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n201_));
  nor2   g139(.a(new_n201_), .b(new_n143_), .O(new_n202_));
  oai21  g140(.a(new_n66_), .b(new_n64_), .c(x25), .O(new_n203_));
  nand2  g141(.a(new_n113_), .b(new_n64_), .O(new_n204_));
  aoi21  g142(.a(new_n204_), .b(new_n203_), .c(x08), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n202_), .c(new_n83_), .O(new_n206_));
  nor2   g144(.a(new_n206_), .b(x09), .O(z20));
endmodule


