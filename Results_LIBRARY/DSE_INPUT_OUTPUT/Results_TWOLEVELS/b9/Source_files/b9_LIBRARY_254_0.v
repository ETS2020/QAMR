// Benchmark "FAU" written by ABC on Tue Aug 18 17:27:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n204_, new_n205_,
    new_n206_;
  nor2   g000(.a(x40), .b(x38), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n64_), .d(new_n71_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n63_), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x39), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n87_), .c(x40), .O(new_n91_));
  nand2  g029(.a(new_n73_), .b(x38), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(z02));
  inv1   g031(.a(new_n63_), .O(new_n94_));
  nand2  g032(.a(new_n68_), .b(x27), .O(new_n95_));
  nand3  g033(.a(new_n94_), .b(new_n67_), .c(new_n66_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(x21), .O(z03));
  nor2   g035(.a(new_n63_), .b(x21), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n94_), .c(new_n67_), .O(z05));
  nand2  g039(.a(new_n94_), .b(new_n66_), .O(new_n102_));
  oai21  g040(.a(x40), .b(x38), .c(new_n81_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(x37), .O(z06));
  inv1   g042(.a(x03), .O(new_n105_));
  inv1   g043(.a(x31), .O(new_n106_));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n64_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(x00), .O(new_n110_));
  inv1   g048(.a(x38), .O(new_n111_));
  nand2  g049(.a(x40), .b(new_n111_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand4  g051(.a(new_n113_), .b(new_n108_), .c(new_n107_), .d(new_n106_), .O(new_n114_));
  inv1   g052(.a(x14), .O(new_n115_));
  oai21  g053(.a(x25), .b(x00), .c(x38), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n112_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n114_), .c(new_n105_), .O(z07));
  nand2  g057(.a(x40), .b(x39), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n94_), .O(z09));
  inv1   g061(.a(x07), .O(new_n124_));
  inv1   g062(.a(x39), .O(new_n125_));
  nand2  g063(.a(x40), .b(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n92_), .c(new_n124_), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x05), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n127_), .c(new_n83_), .O(new_n130_));
  nand3  g068(.a(new_n94_), .b(x37), .c(x06), .O(new_n131_));
  oai21  g069(.a(new_n130_), .b(x04), .c(new_n131_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x27), .O(new_n133_));
  inv1   g071(.a(x05), .O(new_n134_));
  nand2  g072(.a(new_n79_), .b(new_n78_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x40), .c(x39), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n71_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n133_), .O(z10));
  oai21  g077(.a(new_n82_), .b(x04), .c(x40), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n102_), .c(x08), .O(new_n141_));
  inv1   g079(.a(new_n82_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x40), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  aoi22  g083(.a(new_n145_), .b(new_n142_), .c(new_n144_), .d(new_n92_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n141_), .c(new_n78_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(x09), .O(z11));
  inv1   g086(.a(x35), .O(new_n149_));
  nand3  g087(.a(new_n67_), .b(new_n80_), .c(new_n149_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x27), .c(x10), .O(new_n151_));
  aoi21  g089(.a(x39), .b(new_n71_), .c(new_n69_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n151_), .c(x40), .O(new_n153_));
  nor2   g091(.a(new_n69_), .b(x40), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x38), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x28), .c(new_n73_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n71_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand2  g100(.a(new_n68_), .b(new_n67_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n160_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x27), .O(new_n166_));
  nor2   g104(.a(new_n136_), .b(x13), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n71_), .c(new_n63_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n166_), .O(z13));
  inv1   g107(.a(new_n92_), .O(new_n170_));
  nand3  g108(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n171_));
  nand4  g109(.a(new_n158_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n172_));
  nand3  g110(.a(x39), .b(new_n157_), .c(new_n71_), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n172_), .c(new_n73_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n170_), .c(new_n171_), .O(new_n176_));
  nand3  g114(.a(new_n67_), .b(new_n149_), .c(x28), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x27), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n79_), .c(new_n78_), .O(new_n179_));
  oai21  g117(.a(new_n174_), .b(new_n69_), .c(new_n179_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x40), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n176_), .c(new_n155_), .O(z14));
  inv1   g120(.a(x12), .O(new_n183_));
  nand4  g121(.a(new_n94_), .b(x34), .c(x27), .d(x26), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n183_), .O(z15));
  inv1   g123(.a(x23), .O(new_n186_));
  nand4  g124(.a(new_n94_), .b(new_n186_), .c(x22), .d(x01), .O(new_n187_));
  inv1   g125(.a(new_n187_), .O(z16));
  inv1   g126(.a(x24), .O(new_n189_));
  nand4  g127(.a(new_n189_), .b(x23), .c(x22), .d(x01), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n94_), .O(z17));
  inv1   g129(.a(x29), .O(new_n192_));
  oai22  g130(.a(new_n120_), .b(new_n192_), .c(new_n82_), .d(x27), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(x08), .O(new_n194_));
  nor2   g132(.a(new_n66_), .b(x04), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n142_), .c(x09), .O(new_n196_));
  nand4  g134(.a(new_n196_), .b(new_n194_), .c(new_n94_), .d(new_n78_), .O(z18));
  oai22  g135(.a(new_n73_), .b(x35), .c(new_n81_), .d(x27), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n143_), .O(new_n199_));
  nor2   g137(.a(new_n73_), .b(x08), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n170_), .c(new_n149_), .O(new_n201_));
  nor2   g139(.a(new_n63_), .b(x08), .O(new_n202_));
  nor2   g140(.a(x40), .b(new_n81_), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n202_), .c(new_n66_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n201_), .c(new_n199_), .O(new_n205_));
  nand2  g143(.a(new_n205_), .b(new_n78_), .O(new_n206_));
  oai21  g144(.a(new_n206_), .b(x09), .c(new_n94_), .O(z20));
  nor2   g145(.a(new_n147_), .b(x09), .O(z19));
endmodule


