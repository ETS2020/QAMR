// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_;
  nand2  g000(.a(x40), .b(x11), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x11), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(x04), .O(new_n90_));
  oai21  g028(.a(x08), .b(new_n90_), .c(x40), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n67_), .O(new_n92_));
  inv1   g030(.a(x35), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(x28), .c(x04), .O(new_n94_));
  oai21  g032(.a(x28), .b(new_n67_), .c(x04), .O(new_n95_));
  nand2  g033(.a(new_n63_), .b(x39), .O(new_n96_));
  aoi21  g034(.a(new_n95_), .b(new_n74_), .c(new_n96_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n94_), .c(new_n92_), .d(new_n89_), .O(z02));
  nand2  g036(.a(new_n69_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x21), .O(new_n100_));
  aoi21  g038(.a(new_n68_), .b(new_n67_), .c(new_n100_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n99_), .c(new_n64_), .O(z03));
  nand2  g040(.a(new_n68_), .b(new_n67_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n99_), .c(new_n63_), .d(new_n100_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n64_), .O(z05));
  aoi21  g044(.a(x40), .b(x11), .c(x27), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n81_), .c(new_n68_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n63_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n65_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x14), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n112_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  oai21  g056(.a(new_n114_), .b(x31), .c(new_n118_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n63_), .c(x03), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z07));
  nand3  g059(.a(x40), .b(x39), .c(new_n72_), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z08));
  nand3  g061(.a(x34), .b(x27), .c(x26), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n74_), .c(new_n72_), .O(z09));
  oai21  g063(.a(x39), .b(x11), .c(x40), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x07), .O(new_n127_));
  nand4  g065(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n82_), .O(new_n129_));
  nand3  g067(.a(new_n63_), .b(x37), .c(x06), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  aoi21  g069(.a(new_n129_), .b(new_n90_), .c(new_n131_), .O(new_n132_));
  inv1   g070(.a(x39), .O(new_n133_));
  nand2  g071(.a(new_n80_), .b(new_n79_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand4  g074(.a(new_n136_), .b(new_n72_), .c(x05), .d(new_n90_), .O(new_n137_));
  oai21  g075(.a(new_n132_), .b(new_n67_), .c(new_n137_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  nand3  g077(.a(x35), .b(new_n81_), .c(new_n90_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n72_), .c(new_n107_), .O(new_n141_));
  aoi21  g079(.a(x39), .b(x29), .c(x11), .O(new_n142_));
  nand2  g080(.a(x27), .b(x04), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(x35), .c(new_n81_), .O(new_n144_));
  oai21  g082(.a(new_n142_), .b(new_n74_), .c(new_n144_), .O(new_n145_));
  oai21  g083(.a(new_n141_), .b(x08), .c(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n79_), .c(new_n139_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(z11));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n67_), .c(new_n73_), .O(new_n150_));
  inv1   g088(.a(new_n70_), .O(new_n151_));
  nor2   g089(.a(new_n74_), .b(new_n133_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n90_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n150_), .c(new_n63_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n74_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n90_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand2  g099(.a(new_n69_), .b(new_n68_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x27), .O(new_n165_));
  nand4  g103(.a(new_n134_), .b(x39), .c(new_n156_), .d(new_n90_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n72_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x40), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(z13));
  nand3  g107(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n170_));
  nand4  g108(.a(new_n157_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  nand3  g110(.a(new_n152_), .b(new_n156_), .c(new_n90_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n170_), .O(new_n174_));
  nand3  g112(.a(new_n68_), .b(new_n93_), .c(x28), .O(new_n175_));
  oai21  g113(.a(x27), .b(x11), .c(new_n175_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n80_), .c(new_n79_), .O(new_n177_));
  aoi21  g115(.a(new_n173_), .b(new_n151_), .c(new_n64_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n177_), .c(new_n174_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nand4  g118(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n180_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand4  g121(.a(new_n63_), .b(new_n183_), .c(x22), .d(x01), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(z16));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(x23), .c(x22), .d(x01), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n63_), .O(z17));
  nand2  g126(.a(x27), .b(new_n90_), .O(new_n189_));
  nand2  g127(.a(new_n67_), .b(x08), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n189_), .c(new_n93_), .O(new_n191_));
  nand2  g129(.a(new_n79_), .b(new_n139_), .O(new_n192_));
  aoi21  g130(.a(new_n191_), .b(new_n81_), .c(new_n192_), .O(new_n193_));
  nand4  g131(.a(new_n152_), .b(x29), .c(new_n72_), .d(x08), .O(new_n194_));
  oai21  g132(.a(new_n193_), .b(new_n64_), .c(new_n194_), .O(z18));
  nor2   g133(.a(new_n142_), .b(new_n74_), .O(new_n196_));
  aoi21  g134(.a(x28), .b(new_n67_), .c(new_n93_), .O(new_n197_));
  nand2  g135(.a(x35), .b(x27), .O(new_n198_));
  aoi22  g136(.a(new_n198_), .b(new_n72_), .c(new_n74_), .d(new_n67_), .O(new_n199_));
  oai22  g137(.a(new_n199_), .b(x08), .c(new_n197_), .d(new_n196_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n79_), .c(new_n139_), .O(new_n201_));
  inv1   g139(.a(new_n201_), .O(z20));
  inv1   g140(.a(new_n147_), .O(z19));
endmodule


