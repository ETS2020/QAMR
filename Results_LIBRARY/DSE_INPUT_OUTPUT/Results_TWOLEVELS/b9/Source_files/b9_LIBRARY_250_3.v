// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x03), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x03), .c(new_n70_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g023(.a(x03), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g027(.a(new_n81_), .b(x28), .c(x04), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n91_), .c(x04), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n90_), .c(new_n89_), .d(x39), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n86_), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x40), .O(z02));
  inv1   g034(.a(new_n64_), .O(new_n97_));
  nand2  g035(.a(new_n66_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n65_), .b(new_n92_), .c(new_n99_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(z03));
  nand2  g039(.a(new_n65_), .b(new_n92_), .O(new_n102_));
  nand4  g040(.a(new_n102_), .b(new_n98_), .c(new_n64_), .d(new_n99_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n65_), .c(new_n97_), .O(z05));
  nand3  g043(.a(new_n104_), .b(new_n64_), .c(new_n65_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x31), .O(new_n108_));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n72_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand4  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .d(new_n108_), .O(new_n113_));
  inv1   g051(.a(x14), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n111_), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(new_n114_), .c(x40), .O(new_n117_));
  aoi21  g055(.a(new_n117_), .b(new_n113_), .c(new_n86_), .O(z07));
  inv1   g056(.a(x39), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n86_), .c(new_n74_), .O(z08));
  inv1   g058(.a(x11), .O(new_n121_));
  nand4  g059(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n121_), .O(z09));
  nor2   g061(.a(new_n82_), .b(x36), .O(new_n124_));
  inv1   g062(.a(x07), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x05), .O(new_n126_));
  oai21  g064(.a(x39), .b(new_n125_), .c(new_n126_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n86_), .O(new_n128_));
  nand2  g066(.a(new_n74_), .b(x07), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n128_), .c(new_n124_), .O(new_n130_));
  nand3  g068(.a(new_n64_), .b(x37), .c(x06), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  aoi21  g070(.a(new_n130_), .b(new_n71_), .c(new_n132_), .O(new_n133_));
  nand2  g071(.a(new_n80_), .b(new_n79_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n119_), .O(new_n136_));
  nand4  g074(.a(new_n136_), .b(x05), .c(new_n71_), .d(new_n86_), .O(new_n137_));
  oai21  g075(.a(new_n133_), .b(new_n92_), .c(new_n137_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  nand3  g077(.a(x40), .b(x39), .c(x29), .O(new_n140_));
  oai21  g078(.a(new_n92_), .b(new_n71_), .c(new_n82_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  inv1   g080(.a(x28), .O(new_n143_));
  nand2  g081(.a(x39), .b(x04), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(x35), .c(new_n143_), .d(x27), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n91_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n142_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n79_), .c(new_n139_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n64_), .O(z11));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  nor2   g088(.a(new_n119_), .b(x27), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n73_), .O(new_n152_));
  oai21  g090(.a(new_n119_), .b(x04), .c(new_n86_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x40), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n66_), .c(new_n65_), .O(new_n155_));
  aoi21  g093(.a(x39), .b(new_n71_), .c(x03), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n74_), .c(new_n92_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(new_n155_), .c(new_n152_), .d(new_n64_), .O(z12));
  inv1   g096(.a(x13), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x28), .c(new_n74_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n159_), .d(new_n71_), .O(new_n162_));
  inv1   g100(.a(x18), .O(new_n163_));
  inv1   g101(.a(x19), .O(new_n164_));
  nand4  g102(.a(new_n67_), .b(x20), .c(new_n164_), .d(new_n163_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n162_), .c(new_n92_), .O(new_n166_));
  nand4  g104(.a(new_n134_), .b(x39), .c(new_n159_), .d(new_n71_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n86_), .c(new_n74_), .O(new_n168_));
  or2    g106(.a(new_n168_), .b(new_n166_), .O(z13));
  nand3  g107(.a(x20), .b(new_n164_), .c(new_n163_), .O(new_n170_));
  nand4  g108(.a(new_n160_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n171_));
  nor2   g109(.a(x13), .b(x04), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x39), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n171_), .c(x03), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n74_), .c(new_n170_), .O(new_n176_));
  nand3  g114(.a(new_n65_), .b(new_n81_), .c(x28), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x27), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n80_), .c(new_n79_), .O(new_n179_));
  inv1   g117(.a(new_n179_), .O(new_n180_));
  aoi22  g118(.a(new_n172_), .b(x39), .c(new_n67_), .d(x27), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n180_), .c(new_n86_), .O(new_n182_));
  nand2  g120(.a(new_n68_), .b(new_n74_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n182_), .c(new_n176_), .O(z14));
  nand4  g122(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n64_), .O(z15));
  inv1   g124(.a(x23), .O(new_n187_));
  nand4  g125(.a(new_n64_), .b(new_n187_), .c(x22), .d(x01), .O(new_n188_));
  inv1   g126(.a(new_n188_), .O(z16));
  inv1   g127(.a(x01), .O(new_n190_));
  inv1   g128(.a(x24), .O(new_n191_));
  nand4  g129(.a(new_n64_), .b(new_n191_), .c(x23), .d(x22), .O(new_n192_));
  nor2   g130(.a(new_n192_), .b(new_n190_), .O(z17));
  nand2  g131(.a(new_n82_), .b(new_n92_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n140_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(x08), .O(new_n196_));
  nor2   g134(.a(new_n92_), .b(x04), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n82_), .c(x30), .O(new_n198_));
  nand4  g136(.a(new_n198_), .b(new_n196_), .c(new_n64_), .d(new_n139_), .O(z18));
  inv1   g137(.a(new_n140_), .O(new_n200_));
  oai21  g138(.a(new_n81_), .b(new_n92_), .c(new_n91_), .O(new_n201_));
  aoi21  g139(.a(x28), .b(new_n92_), .c(new_n81_), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n200_), .c(new_n201_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n79_), .c(new_n139_), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(new_n64_), .O(z20));
  nand2  g143(.a(new_n148_), .b(new_n64_), .O(z19));
endmodule


