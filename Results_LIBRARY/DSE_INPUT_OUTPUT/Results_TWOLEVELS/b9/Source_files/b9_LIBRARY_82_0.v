// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_;
  inv1   g000(.a(x13), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x27), .c(new_n66_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n64_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n66_), .d(new_n73_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n72_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n65_), .c(x04), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  aoi21  g026(.a(x28), .b(x04), .c(new_n88_), .O(new_n89_));
  oai21  g027(.a(x27), .b(x08), .c(x35), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n89_), .c(x40), .d(x39), .O(z02));
  nand2  g030(.a(new_n68_), .b(x27), .O(new_n93_));
  inv1   g031(.a(x27), .O(new_n94_));
  nand3  g032(.a(new_n65_), .b(new_n67_), .c(new_n94_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n93_), .c(new_n65_), .d(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand4  g035(.a(new_n95_), .b(new_n93_), .c(new_n65_), .d(new_n97_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n65_), .c(new_n67_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n66_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  oai22  g047(.a(new_n109_), .b(x14), .c(new_n106_), .d(x31), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n65_), .O(z07));
  nand2  g050(.a(x40), .b(x39), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n65_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n65_), .O(z09));
  inv1   g054(.a(x07), .O(new_n117_));
  inv1   g055(.a(x39), .O(new_n118_));
  nand2  g056(.a(x40), .b(new_n118_), .O(new_n119_));
  nand2  g057(.a(new_n64_), .b(x13), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(new_n117_), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(x05), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  oai21  g061(.a(new_n123_), .b(new_n121_), .c(new_n84_), .O(new_n124_));
  nand3  g062(.a(new_n65_), .b(x37), .c(x06), .O(new_n125_));
  oai21  g063(.a(new_n124_), .b(x04), .c(new_n125_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  aoi21  g065(.a(new_n80_), .b(new_n79_), .c(new_n64_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x39), .c(x05), .d(new_n73_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(z10));
  nand2  g068(.a(new_n65_), .b(new_n94_), .O(new_n131_));
  oai21  g069(.a(new_n83_), .b(x04), .c(x40), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n131_), .c(x08), .O(new_n133_));
  inv1   g071(.a(new_n83_), .O(new_n134_));
  aoi21  g072(.a(x39), .b(x29), .c(new_n64_), .O(new_n135_));
  aoi21  g073(.a(new_n64_), .b(x13), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n134_), .c(new_n136_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n133_), .c(new_n79_), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(x09), .O(z11));
  nor3   g078(.a(x37), .b(x36), .c(x35), .O(new_n141_));
  nor2   g079(.a(new_n64_), .b(x27), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n141_), .c(new_n74_), .O(new_n143_));
  nand3  g081(.a(x40), .b(x39), .c(new_n73_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n68_), .c(new_n67_), .O(new_n145_));
  oai21  g083(.a(new_n94_), .b(new_n63_), .c(new_n64_), .O(new_n146_));
  aoi21  g084(.a(x40), .b(x04), .c(new_n118_), .O(new_n147_));
  or2    g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(new_n146_), .c(new_n145_), .d(new_n143_), .O(z12));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n63_), .d(new_n73_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand4  g092(.a(new_n69_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  oai21  g095(.a(x32), .b(x30), .c(x39), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(x04), .c(x40), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n63_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n157_), .O(z13));
  inv1   g099(.a(new_n69_), .O(new_n162_));
  nand3  g100(.a(x39), .b(new_n63_), .c(new_n73_), .O(new_n163_));
  nand3  g101(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n162_), .c(new_n163_), .O(new_n165_));
  inv1   g103(.a(x35), .O(new_n166_));
  aoi22  g104(.a(new_n164_), .b(new_n150_), .c(new_n67_), .d(new_n166_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n82_), .c(x27), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n80_), .c(new_n79_), .O(new_n169_));
  nand2  g107(.a(new_n147_), .b(new_n63_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n94_), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n169_), .c(new_n165_), .d(new_n65_), .O(z14));
  inv1   g110(.a(x12), .O(new_n173_));
  nand4  g111(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n174_));
  nor2   g112(.a(new_n174_), .b(new_n173_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(x22), .c(x01), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n65_), .O(z16));
  inv1   g116(.a(x01), .O(new_n179_));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n65_), .b(new_n180_), .c(x23), .d(x22), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n179_), .O(z17));
  inv1   g120(.a(x29), .O(new_n183_));
  oai22  g121(.a(new_n113_), .b(new_n183_), .c(new_n83_), .d(x27), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x08), .O(new_n185_));
  nor2   g123(.a(new_n94_), .b(x04), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n134_), .c(x09), .O(new_n187_));
  nand4  g125(.a(new_n187_), .b(new_n185_), .c(new_n65_), .d(new_n79_), .O(z18));
  aoi21  g126(.a(x28), .b(new_n94_), .c(new_n166_), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n136_), .O(new_n190_));
  nand2  g128(.a(x40), .b(new_n166_), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n131_), .c(x08), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n190_), .c(new_n79_), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(x09), .O(z20));
  nor2   g132(.a(new_n139_), .b(x09), .O(z19));
endmodule


