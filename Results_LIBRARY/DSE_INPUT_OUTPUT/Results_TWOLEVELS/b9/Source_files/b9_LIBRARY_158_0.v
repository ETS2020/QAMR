// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n108_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n197_, new_n199_, new_n200_, new_n202_, new_n203_, new_n205_,
    new_n206_, new_n207_, new_n208_, new_n209_, new_n212_, new_n213_,
    new_n214_, new_n215_, new_n216_, new_n217_, new_n218_, new_n219_,
    new_n220_, new_n221_, new_n222_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n74_), .c(new_n63_), .O(new_n80_));
  nor2   g018(.a(x40), .b(x39), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n80_), .c(x16), .O(z00));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  aoi21  g024(.a(x35), .b(new_n86_), .c(x36), .O(new_n87_));
  or2    g025(.a(new_n87_), .b(new_n64_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n85_), .c(new_n84_), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(x04), .c(new_n81_), .O(z01));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g031(.a(x35), .b(new_n86_), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n94_), .c(x04), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n93_), .c(x39), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x40), .O(new_n98_));
  nor2   g036(.a(x40), .b(new_n75_), .O(new_n99_));
  inv1   g037(.a(new_n99_), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n98_), .O(z02));
  nand2  g039(.a(new_n71_), .b(x27), .O(new_n102_));
  inv1   g040(.a(x21), .O(new_n103_));
  aoi21  g041(.a(new_n70_), .b(new_n64_), .c(new_n103_), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n102_), .c(new_n81_), .O(z03));
  aoi21  g043(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n102_), .c(new_n81_), .O(z04));
  nand2  g045(.a(x28), .b(x27), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n82_), .c(new_n70_), .O(z05));
  oai21  g047(.a(x40), .b(x39), .c(new_n64_), .O(new_n110_));
  oai21  g048(.a(x40), .b(x39), .c(new_n86_), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n110_), .c(x37), .O(z06));
  inv1   g050(.a(x33), .O(new_n113_));
  nand2  g051(.a(x17), .b(new_n63_), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  oai21  g053(.a(x25), .b(new_n115_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n115_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  oai22  g058(.a(new_n120_), .b(x14), .c(new_n117_), .d(x31), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x03), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n82_), .O(z07));
  inv1   g061(.a(new_n77_), .O(new_n124_));
  nand2  g062(.a(new_n82_), .b(new_n124_), .O(z08));
  inv1   g063(.a(x11), .O(new_n126_));
  nand4  g064(.a(new_n82_), .b(x34), .c(x27), .d(x26), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n126_), .O(z09));
  xor2a  g066(.a(x40), .b(x39), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x07), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x05), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n130_), .c(new_n87_), .O(new_n132_));
  inv1   g070(.a(x06), .O(new_n133_));
  nor3   g071(.a(new_n81_), .b(new_n70_), .c(new_n133_), .O(new_n134_));
  aoi21  g072(.a(new_n132_), .b(new_n65_), .c(new_n134_), .O(new_n135_));
  nor2   g073(.a(x32), .b(x30), .O(new_n136_));
  nor3   g074(.a(new_n136_), .b(new_n76_), .c(new_n75_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x05), .c(new_n65_), .O(new_n138_));
  oai21  g076(.a(new_n135_), .b(new_n64_), .c(new_n138_), .O(z10));
  inv1   g077(.a(x09), .O(new_n140_));
  inv1   g078(.a(x08), .O(new_n141_));
  oai21  g079(.a(new_n66_), .b(x04), .c(x40), .O(new_n142_));
  nand2  g080(.a(x39), .b(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n142_), .c(new_n110_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g083(.a(x39), .b(x29), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(x27), .c(x04), .O(new_n147_));
  oai21  g085(.a(new_n66_), .b(x28), .c(new_n75_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x40), .O(new_n150_));
  aoi22  g088(.a(x40), .b(x29), .c(x35), .d(new_n86_), .O(new_n151_));
  nand3  g089(.a(new_n76_), .b(x27), .c(x04), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n151_), .c(x39), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n150_), .c(new_n145_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n84_), .c(new_n140_), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(z11));
  inv1   g095(.a(x10), .O(new_n158_));
  nor3   g096(.a(x37), .b(x36), .c(x35), .O(new_n159_));
  nor2   g097(.a(new_n76_), .b(x27), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n158_), .O(new_n161_));
  oai22  g099(.a(new_n73_), .b(new_n64_), .c(new_n75_), .d(x04), .O(new_n162_));
  nand2  g100(.a(x39), .b(new_n86_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x35), .c(x37), .O(new_n164_));
  nand2  g102(.a(x39), .b(x27), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n164_), .c(new_n76_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n162_), .c(new_n161_), .O(z12));
  inv1   g105(.a(x13), .O(new_n168_));
  nand2  g106(.a(x36), .b(x35), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x28), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(x40), .c(new_n168_), .d(new_n65_), .O(new_n171_));
  inv1   g109(.a(x18), .O(new_n172_));
  inv1   g110(.a(x19), .O(new_n173_));
  nand4  g111(.a(new_n72_), .b(x20), .c(new_n173_), .d(new_n172_), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n171_), .c(new_n75_), .O(new_n175_));
  nand4  g113(.a(new_n72_), .b(x40), .c(x20), .d(new_n173_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(x18), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n175_), .c(x27), .O(new_n178_));
  nand3  g116(.a(new_n137_), .b(new_n168_), .c(new_n65_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n178_), .O(z13));
  oai21  g118(.a(new_n136_), .b(x13), .c(x40), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n77_), .c(new_n65_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n64_), .O(new_n183_));
  nand3  g121(.a(x20), .b(new_n173_), .c(new_n172_), .O(new_n184_));
  nand4  g122(.a(new_n169_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n77_), .c(new_n168_), .d(new_n65_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n184_), .O(new_n187_));
  nand3  g125(.a(x39), .b(new_n168_), .c(new_n65_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n71_), .O(new_n189_));
  nand3  g127(.a(new_n85_), .b(new_n84_), .c(x28), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(x40), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n66_), .O(new_n192_));
  nand2  g130(.a(new_n99_), .b(new_n86_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n192_), .c(new_n189_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n70_), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n187_), .c(new_n183_), .d(new_n82_), .O(z14));
  inv1   g134(.a(x12), .O(new_n197_));
  nor2   g135(.a(new_n127_), .b(new_n197_), .O(z15));
  inv1   g136(.a(x23), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(x22), .c(x01), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(new_n82_), .O(z16));
  inv1   g139(.a(x24), .O(new_n202_));
  nand4  g140(.a(new_n202_), .b(x23), .c(x22), .d(x01), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n82_), .O(z17));
  nand2  g142(.a(x27), .b(new_n65_), .O(new_n205_));
  nand2  g143(.a(new_n64_), .b(x08), .O(new_n206_));
  aoi21  g144(.a(new_n206_), .b(new_n205_), .c(new_n66_), .O(new_n207_));
  nand2  g145(.a(new_n84_), .b(new_n140_), .O(new_n208_));
  aoi21  g146(.a(new_n207_), .b(new_n86_), .c(new_n208_), .O(new_n209_));
  oai22  g147(.a(new_n209_), .b(new_n81_), .c(new_n92_), .d(new_n124_), .O(z18));
  nand2  g148(.a(new_n156_), .b(new_n82_), .O(z19));
  nand2  g149(.a(x40), .b(x29), .O(new_n212_));
  oai22  g150(.a(new_n75_), .b(x35), .c(new_n86_), .d(x27), .O(new_n213_));
  nand2  g151(.a(new_n213_), .b(new_n212_), .O(new_n214_));
  aoi21  g152(.a(x35), .b(x27), .c(x08), .O(new_n215_));
  nor2   g153(.a(x39), .b(x35), .O(new_n216_));
  oai21  g154(.a(new_n216_), .b(new_n215_), .c(x40), .O(new_n217_));
  nor2   g155(.a(new_n75_), .b(x08), .O(new_n218_));
  nor2   g156(.a(x39), .b(new_n86_), .O(new_n219_));
  oai21  g157(.a(new_n219_), .b(new_n218_), .c(new_n64_), .O(new_n220_));
  nand3  g158(.a(new_n220_), .b(new_n217_), .c(new_n214_), .O(new_n221_));
  nand3  g159(.a(new_n221_), .b(new_n84_), .c(new_n140_), .O(new_n222_));
  nand2  g160(.a(new_n222_), .b(new_n82_), .O(z20));
endmodule


