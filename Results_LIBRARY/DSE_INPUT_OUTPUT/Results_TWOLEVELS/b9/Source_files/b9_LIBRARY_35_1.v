// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x32), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n63_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x39), .c(x32), .d(new_n65_), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand4  g021(.a(new_n83_), .b(new_n63_), .c(new_n78_), .d(new_n77_), .O(new_n84_));
  oai21  g022(.a(new_n64_), .b(x04), .c(new_n84_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  inv1   g024(.a(x08), .O(new_n87_));
  inv1   g025(.a(x29), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(new_n89_));
  nand2  g027(.a(new_n81_), .b(x04), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n89_), .c(new_n63_), .O(new_n91_));
  nand3  g029(.a(new_n67_), .b(new_n87_), .c(x04), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x39), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n91_), .c(x32), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(z02));
  nand2  g033(.a(new_n69_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n68_), .b(new_n67_), .c(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n96_), .c(new_n64_), .O(z03));
  nand2  g037(.a(x40), .b(new_n78_), .O(new_n100_));
  nand2  g038(.a(new_n68_), .b(new_n67_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n96_), .c(new_n100_), .d(new_n97_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n64_), .O(z05));
  aoi21  g042(.a(x40), .b(new_n78_), .c(x27), .O(new_n105_));
  oai21  g043(.a(new_n105_), .b(new_n80_), .c(new_n68_), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n100_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n65_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n100_), .c(x03), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z07));
  oai21  g056(.a(x39), .b(new_n78_), .c(x40), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n100_), .O(z09));
  nand3  g060(.a(new_n100_), .b(x37), .c(x06), .O(new_n123_));
  inv1   g061(.a(x04), .O(new_n124_));
  nand4  g062(.a(new_n119_), .b(new_n82_), .c(x07), .d(new_n124_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  nand2  g065(.a(x40), .b(x39), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(x32), .c(x05), .d(new_n124_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nand2  g070(.a(x40), .b(x04), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x35), .c(new_n80_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(x32), .c(new_n105_), .O(new_n135_));
  nand2  g073(.a(x39), .b(x29), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x32), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x40), .O(new_n138_));
  nand2  g076(.a(x27), .b(x04), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(x35), .c(new_n80_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  oai21  g079(.a(new_n135_), .b(x08), .c(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n77_), .c(new_n132_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(z11));
  inv1   g082(.a(x35), .O(new_n145_));
  nand3  g083(.a(new_n68_), .b(new_n79_), .c(new_n145_), .O(new_n146_));
  nand2  g084(.a(x40), .b(new_n67_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n146_), .c(x10), .O(new_n148_));
  nand2  g086(.a(new_n133_), .b(x39), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n67_), .O(new_n150_));
  nand2  g088(.a(x39), .b(new_n124_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n69_), .c(new_n68_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n148_), .c(x32), .O(new_n154_));
  oai21  g092(.a(new_n70_), .b(x40), .c(new_n154_), .O(z12));
  nand2  g093(.a(new_n129_), .b(x32), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  aoi21  g095(.a(new_n69_), .b(new_n68_), .c(new_n64_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x27), .c(x20), .d(new_n157_), .O(new_n159_));
  inv1   g097(.a(x13), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n124_), .O(new_n161_));
  oai22  g099(.a(new_n161_), .b(new_n156_), .c(new_n159_), .d(x18), .O(z13));
  inv1   g100(.a(x18), .O(new_n163_));
  nand3  g101(.a(x40), .b(x32), .c(new_n67_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x20), .c(new_n157_), .d(new_n163_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n161_), .O(new_n166_));
  nand2  g104(.a(new_n151_), .b(x32), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(x40), .c(new_n160_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n69_), .c(new_n68_), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n157_), .c(new_n163_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n128_), .O(new_n171_));
  aoi21  g109(.a(new_n119_), .b(new_n67_), .c(new_n64_), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(new_n171_), .c(new_n169_), .d(new_n166_), .O(z14));
  nand4  g111(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n100_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(x22), .c(x01), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n100_), .O(z16));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(x23), .c(x22), .d(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n100_), .O(z17));
  oai22  g119(.a(new_n128_), .b(new_n88_), .c(new_n81_), .d(x27), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x08), .O(new_n183_));
  inv1   g121(.a(new_n81_), .O(new_n184_));
  nor2   g122(.a(new_n67_), .b(x04), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n184_), .c(x09), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n183_), .c(new_n100_), .d(new_n77_), .O(z18));
  nand2  g125(.a(new_n143_), .b(new_n100_), .O(z19));
  nand2  g126(.a(x28), .b(new_n67_), .O(new_n189_));
  aoi22  g127(.a(new_n189_), .b(x35), .c(new_n137_), .d(x40), .O(new_n190_));
  nand2  g128(.a(new_n100_), .b(new_n67_), .O(new_n191_));
  nand2  g129(.a(new_n145_), .b(x32), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n191_), .c(x08), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n190_), .c(new_n77_), .O(new_n194_));
  nor2   g132(.a(new_n194_), .b(x09), .O(z20));
endmodule


