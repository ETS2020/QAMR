// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_;
  nand2  g000(.a(x40), .b(x13), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x13), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n65_), .d(new_n71_), .O(new_n76_));
  oai22  g014(.a(new_n76_), .b(x04), .c(new_n70_), .d(new_n64_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  and2   g023(.a(x29), .b(x08), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x02), .O(new_n87_));
  inv1   g025(.a(x04), .O(new_n88_));
  oai21  g026(.a(new_n82_), .b(new_n88_), .c(new_n71_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n87_), .c(x40), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  nand3  g029(.a(new_n67_), .b(new_n91_), .c(x04), .O(new_n92_));
  inv1   g030(.a(x39), .O(new_n93_));
  nor2   g031(.a(new_n73_), .b(new_n93_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(new_n90_), .O(z02));
  inv1   g033(.a(x35), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n80_), .c(x27), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  nand3  g036(.a(new_n63_), .b(new_n98_), .c(new_n67_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n63_), .d(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand4  g039(.a(new_n99_), .b(new_n97_), .c(new_n63_), .d(new_n101_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n63_), .c(new_n98_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n65_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x14), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n108_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  oai21  g052(.a(new_n110_), .b(x31), .c(new_n114_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n63_), .O(z07));
  oai21  g055(.a(x39), .b(x13), .c(x40), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n63_), .O(z09));
  aoi21  g059(.a(x35), .b(new_n80_), .c(x36), .O(new_n122_));
  nand2  g060(.a(new_n118_), .b(x07), .O(new_n123_));
  nand4  g061(.a(x40), .b(x39), .c(new_n71_), .d(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(new_n125_));
  nand3  g063(.a(new_n63_), .b(x37), .c(x06), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n88_), .c(new_n127_), .O(new_n128_));
  oai21  g066(.a(x32), .b(x30), .c(x40), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n93_), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(new_n71_), .c(x05), .d(new_n88_), .O(new_n131_));
  oai21  g069(.a(new_n128_), .b(new_n67_), .c(new_n131_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  aoi21  g071(.a(x40), .b(x13), .c(x27), .O(new_n134_));
  nand3  g072(.a(x35), .b(new_n80_), .c(new_n88_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n71_), .c(new_n134_), .O(new_n136_));
  aoi21  g074(.a(x39), .b(x29), .c(x13), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(x35), .c(new_n80_), .O(new_n139_));
  oai21  g077(.a(new_n137_), .b(new_n73_), .c(new_n139_), .O(new_n140_));
  oai21  g078(.a(new_n136_), .b(x08), .c(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n78_), .c(new_n133_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(z11));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  nor2   g082(.a(new_n73_), .b(x27), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n72_), .O(new_n146_));
  nand2  g084(.a(new_n94_), .b(new_n88_), .O(new_n147_));
  oai21  g085(.a(new_n68_), .b(new_n67_), .c(new_n147_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n146_), .c(new_n63_), .O(z12));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x40), .c(x39), .d(new_n88_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  inv1   g092(.a(new_n68_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n152_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  oai21  g096(.a(x32), .b(x30), .c(x39), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(x04), .c(new_n71_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x40), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(z13));
  nand4  g100(.a(x27), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n68_), .c(new_n147_), .O(new_n164_));
  nand3  g102(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n165_));
  nor2   g103(.a(x37), .b(x35), .O(new_n166_));
  aoi21  g104(.a(new_n165_), .b(new_n150_), .c(new_n166_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n80_), .c(x27), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n79_), .c(new_n78_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n164_), .c(new_n63_), .O(z14));
  inv1   g108(.a(x12), .O(new_n171_));
  nand4  g109(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(new_n171_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(x22), .c(x01), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n63_), .O(z16));
  inv1   g114(.a(x01), .O(new_n177_));
  inv1   g115(.a(x24), .O(new_n178_));
  nand4  g116(.a(new_n63_), .b(new_n178_), .c(x23), .d(x22), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n177_), .O(z17));
  nand2  g118(.a(new_n94_), .b(x29), .O(new_n181_));
  oai21  g119(.a(new_n81_), .b(x27), .c(new_n181_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x08), .O(new_n183_));
  nor2   g121(.a(new_n67_), .b(x04), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n82_), .c(x30), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n183_), .c(new_n63_), .d(new_n133_), .O(z18));
  nor2   g124(.a(new_n137_), .b(new_n73_), .O(new_n187_));
  aoi21  g125(.a(x28), .b(new_n67_), .c(new_n96_), .O(new_n188_));
  aoi21  g126(.a(x35), .b(x27), .c(x13), .O(new_n189_));
  nor2   g127(.a(x40), .b(x27), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n189_), .c(new_n91_), .O(new_n191_));
  oai21  g129(.a(new_n188_), .b(new_n187_), .c(new_n191_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n78_), .c(new_n133_), .O(new_n193_));
  inv1   g131(.a(new_n193_), .O(z20));
  inv1   g132(.a(new_n142_), .O(z19));
endmodule


