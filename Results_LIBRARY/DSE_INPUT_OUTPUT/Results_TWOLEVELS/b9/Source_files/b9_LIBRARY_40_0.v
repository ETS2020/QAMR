// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n188_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(z08));
  nand3  g014(.a(z08), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  inv1   g017(.a(x39), .O(new_n80_));
  inv1   g018(.a(x40), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n79_), .c(x16), .O(z00));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n67_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n85_), .c(new_n84_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n82_), .c(x04), .O(z01));
  aoi21  g029(.a(x29), .b(x08), .c(x02), .O(new_n92_));
  aoi21  g030(.a(new_n87_), .b(x04), .c(new_n92_), .O(new_n93_));
  inv1   g031(.a(x08), .O(new_n94_));
  nand3  g032(.a(new_n64_), .b(new_n94_), .c(x04), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n93_), .c(x40), .d(x39), .O(z02));
  nand2  g034(.a(new_n71_), .b(x27), .O(new_n97_));
  nand2  g035(.a(new_n70_), .b(new_n64_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n97_), .c(new_n82_), .d(x21), .O(z03));
  nor2   g037(.a(x40), .b(x39), .O(new_n100_));
  aoi21  g038(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n97_), .c(new_n100_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n82_), .c(new_n70_), .O(z05));
  nand2  g042(.a(new_n82_), .b(new_n64_), .O(new_n105_));
  oai21  g043(.a(x40), .b(x39), .c(new_n86_), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(x37), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n63_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n82_), .c(x03), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z07));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n82_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x05), .O(new_n122_));
  nand2  g060(.a(new_n75_), .b(x07), .O(new_n123_));
  oai21  g061(.a(new_n75_), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n88_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n85_), .b(new_n84_), .O(new_n126_));
  nand4  g064(.a(new_n126_), .b(x40), .c(x39), .d(x05), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n65_), .O(new_n129_));
  nor2   g067(.a(new_n70_), .b(new_n64_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(x06), .c(new_n100_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  nand3  g071(.a(x35), .b(new_n86_), .c(new_n65_), .O(new_n134_));
  aoi22  g072(.a(new_n134_), .b(x39), .c(new_n82_), .d(new_n64_), .O(new_n135_));
  nand2  g073(.a(x40), .b(x29), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x39), .O(new_n137_));
  nand2  g075(.a(x40), .b(new_n80_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x35), .c(new_n86_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  oai21  g080(.a(new_n135_), .b(x08), .c(new_n142_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n84_), .c(new_n133_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(z11));
  inv1   g083(.a(x10), .O(new_n146_));
  nor3   g084(.a(x37), .b(x36), .c(x35), .O(new_n147_));
  nor2   g085(.a(new_n81_), .b(x27), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n147_), .c(new_n146_), .O(new_n149_));
  aoi22  g087(.a(x39), .b(new_n65_), .c(x35), .d(x28), .O(new_n150_));
  nand2  g088(.a(x39), .b(new_n86_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x35), .c(x40), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(new_n70_), .O(new_n153_));
  aoi21  g091(.a(x40), .b(x27), .c(x39), .O(new_n154_));
  nand2  g092(.a(x40), .b(new_n65_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n64_), .c(new_n154_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n153_), .c(new_n149_), .O(z12));
  inv1   g095(.a(x13), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x28), .c(new_n81_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x39), .c(new_n158_), .d(new_n65_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  nand4  g101(.a(new_n72_), .b(x20), .c(new_n163_), .d(new_n162_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x27), .O(new_n166_));
  nand3  g104(.a(new_n126_), .b(x40), .c(x39), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(x13), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n65_), .c(new_n100_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n166_), .O(z13));
  nand3  g108(.a(x20), .b(new_n163_), .c(new_n162_), .O(new_n171_));
  nand4  g109(.a(new_n159_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n172_));
  nand2  g110(.a(new_n158_), .b(new_n65_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x40), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n172_), .c(z08), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n171_), .O(new_n176_));
  nor2   g114(.a(x37), .b(x35), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x28), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(x27), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n85_), .c(new_n84_), .O(new_n180_));
  oai21  g118(.a(new_n148_), .b(new_n73_), .c(new_n173_), .O(new_n181_));
  oai21  g119(.a(new_n177_), .b(new_n64_), .c(new_n75_), .O(new_n182_));
  nand2  g120(.a(x40), .b(x39), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n70_), .c(new_n86_), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n182_), .c(new_n82_), .O(new_n185_));
  inv1   g123(.a(new_n185_), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n181_), .c(new_n180_), .d(new_n176_), .O(z14));
  inv1   g125(.a(x12), .O(new_n188_));
  nor2   g126(.a(new_n120_), .b(new_n188_), .O(z15));
  inv1   g127(.a(x23), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x22), .c(x01), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n82_), .O(z16));
  inv1   g130(.a(x24), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x23), .c(x22), .d(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n82_), .O(z17));
  inv1   g133(.a(x29), .O(new_n196_));
  oai22  g134(.a(new_n87_), .b(x27), .c(new_n75_), .d(new_n196_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(x08), .O(new_n198_));
  nor2   g136(.a(new_n66_), .b(x28), .O(new_n199_));
  nor2   g137(.a(new_n64_), .b(x04), .O(new_n200_));
  aoi21  g138(.a(new_n200_), .b(new_n199_), .c(x09), .O(new_n201_));
  nand4  g139(.a(new_n201_), .b(new_n198_), .c(new_n82_), .d(new_n84_), .O(z18));
  nand2  g140(.a(new_n144_), .b(new_n82_), .O(z19));
  oai22  g141(.a(new_n80_), .b(x35), .c(new_n86_), .d(x27), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(new_n136_), .O(new_n205_));
  oai21  g143(.a(new_n80_), .b(x08), .c(new_n138_), .O(new_n206_));
  nand2  g144(.a(new_n206_), .b(new_n66_), .O(new_n207_));
  nor2   g145(.a(new_n100_), .b(x08), .O(new_n208_));
  nor2   g146(.a(x39), .b(new_n86_), .O(new_n209_));
  oai21  g147(.a(new_n209_), .b(new_n208_), .c(new_n64_), .O(new_n210_));
  nand3  g148(.a(new_n210_), .b(new_n207_), .c(new_n205_), .O(new_n211_));
  nand3  g149(.a(new_n211_), .b(new_n84_), .c(new_n133_), .O(new_n212_));
  nand2  g150(.a(new_n212_), .b(new_n82_), .O(z20));
endmodule


