// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n192_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n201_, new_n202_, new_n203_, new_n204_, new_n205_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(z08));
  nand3  g014(.a(z08), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  nor2   g017(.a(x40), .b(x38), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n79_), .c(x16), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n67_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n84_), .c(new_n83_), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(x04), .c(new_n80_), .O(z01));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor2   g031(.a(x27), .b(x08), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n86_), .c(x04), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n93_), .c(x39), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x40), .O(new_n97_));
  inv1   g035(.a(x40), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x38), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n97_), .O(z02));
  nand2  g038(.a(new_n71_), .b(x27), .O(new_n101_));
  nand2  g039(.a(new_n70_), .b(new_n64_), .O(new_n102_));
  nand4  g040(.a(new_n102_), .b(new_n101_), .c(new_n81_), .d(x21), .O(z03));
  aoi21  g041(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n101_), .c(new_n80_), .O(z04));
  nand2  g043(.a(x28), .b(x27), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n81_), .c(new_n70_), .O(z05));
  oai21  g045(.a(x40), .b(x38), .c(new_n64_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x28), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n70_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n81_), .O(z06));
  inv1   g049(.a(x03), .O(new_n112_));
  inv1   g050(.a(x31), .O(new_n113_));
  inv1   g051(.a(x33), .O(new_n114_));
  nand2  g052(.a(x17), .b(new_n63_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(x00), .O(new_n117_));
  inv1   g055(.a(x38), .O(new_n118_));
  nand2  g056(.a(x40), .b(new_n118_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand4  g058(.a(new_n120_), .b(new_n115_), .c(new_n114_), .d(new_n113_), .O(new_n121_));
  inv1   g059(.a(x14), .O(new_n122_));
  oai21  g060(.a(x25), .b(x00), .c(x38), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n121_), .c(new_n112_), .O(z07));
  nand4  g064(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n81_), .O(z09));
  inv1   g066(.a(x05), .O(new_n129_));
  nand2  g067(.a(new_n75_), .b(x07), .O(new_n130_));
  oai21  g068(.a(new_n75_), .b(new_n129_), .c(new_n130_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n87_), .c(x27), .O(new_n132_));
  inv1   g070(.a(x39), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x05), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n65_), .O(new_n138_));
  nand3  g076(.a(x37), .b(x27), .c(x06), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n81_), .O(z10));
  inv1   g078(.a(x09), .O(new_n141_));
  inv1   g079(.a(x08), .O(new_n142_));
  aoi21  g080(.a(x40), .b(new_n66_), .c(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n108_), .c(new_n65_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand2  g083(.a(x39), .b(x29), .O(new_n146_));
  nand2  g084(.a(x27), .b(x04), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand2  g087(.a(x38), .b(new_n66_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n147_), .c(new_n85_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n98_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n149_), .c(new_n145_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n83_), .c(new_n141_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n81_), .O(z11));
  inv1   g093(.a(x10), .O(new_n156_));
  nor3   g094(.a(x37), .b(x36), .c(x35), .O(new_n157_));
  nor2   g095(.a(new_n98_), .b(x27), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n157_), .c(new_n156_), .O(new_n159_));
  inv1   g097(.a(new_n99_), .O(new_n160_));
  nand2  g098(.a(new_n72_), .b(x27), .O(new_n161_));
  aoi21  g099(.a(x39), .b(new_n65_), .c(new_n98_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n160_), .c(new_n161_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n159_), .c(new_n81_), .O(z12));
  inv1   g102(.a(x13), .O(new_n165_));
  nand2  g103(.a(x36), .b(x35), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x28), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(x39), .c(new_n165_), .d(new_n65_), .O(new_n168_));
  inv1   g106(.a(x18), .O(new_n169_));
  inv1   g107(.a(x19), .O(new_n170_));
  nand4  g108(.a(new_n72_), .b(x20), .c(new_n170_), .d(new_n169_), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n168_), .c(new_n98_), .O(new_n172_));
  nand4  g110(.a(new_n72_), .b(x38), .c(x20), .d(new_n170_), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(x18), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n172_), .c(x27), .O(new_n175_));
  nand3  g113(.a(new_n135_), .b(new_n165_), .c(new_n65_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n175_), .O(z13));
  nand3  g115(.a(x20), .b(new_n170_), .c(new_n169_), .O(new_n178_));
  nand4  g116(.a(new_n166_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n179_));
  nand3  g117(.a(x39), .b(new_n165_), .c(new_n65_), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(new_n181_));
  aoi21  g119(.a(new_n181_), .b(new_n179_), .c(new_n98_), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n160_), .c(new_n178_), .O(new_n183_));
  nand3  g121(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x27), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n84_), .c(new_n83_), .O(new_n186_));
  nand2  g124(.a(new_n180_), .b(new_n161_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n186_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(x40), .O(new_n189_));
  nand3  g127(.a(new_n161_), .b(new_n98_), .c(x38), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(new_n189_), .c(new_n183_), .O(z14));
  nand4  g129(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n81_), .O(z15));
  inv1   g131(.a(x23), .O(new_n194_));
  nand4  g132(.a(new_n81_), .b(new_n194_), .c(x22), .d(x01), .O(new_n195_));
  inv1   g133(.a(new_n195_), .O(z16));
  inv1   g134(.a(x01), .O(new_n197_));
  inv1   g135(.a(x24), .O(new_n198_));
  nand4  g136(.a(new_n81_), .b(new_n198_), .c(x23), .d(x22), .O(new_n199_));
  nor2   g137(.a(new_n199_), .b(new_n197_), .O(z17));
  nand2  g138(.a(x27), .b(new_n65_), .O(new_n201_));
  nand2  g139(.a(new_n64_), .b(x08), .O(new_n202_));
  aoi21  g140(.a(new_n202_), .b(new_n201_), .c(new_n66_), .O(new_n203_));
  nand2  g141(.a(new_n83_), .b(new_n141_), .O(new_n204_));
  aoi21  g142(.a(new_n203_), .b(new_n85_), .c(new_n204_), .O(new_n205_));
  oai22  g143(.a(new_n205_), .b(new_n80_), .c(new_n92_), .d(new_n75_), .O(z18));
  nand2  g144(.a(x28), .b(new_n64_), .O(new_n207_));
  nand2  g145(.a(new_n146_), .b(x40), .O(new_n208_));
  aoi22  g146(.a(new_n208_), .b(new_n99_), .c(new_n207_), .d(x35), .O(new_n209_));
  nand2  g147(.a(x40), .b(new_n66_), .O(new_n210_));
  aoi21  g148(.a(new_n210_), .b(new_n108_), .c(x08), .O(new_n211_));
  oai21  g149(.a(new_n211_), .b(new_n209_), .c(new_n83_), .O(new_n212_));
  nor2   g150(.a(new_n212_), .b(x09), .O(z20));
  nand2  g151(.a(new_n154_), .b(new_n81_), .O(z19));
endmodule


