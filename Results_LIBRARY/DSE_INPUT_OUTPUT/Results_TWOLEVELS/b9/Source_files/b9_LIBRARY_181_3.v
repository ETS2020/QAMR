// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_;
  nand2  g000(.a(x40), .b(x26), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x26), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  or2    g020(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g026(.a(x04), .O(new_n89_));
  oai21  g027(.a(x08), .b(new_n89_), .c(x40), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n67_), .O(new_n91_));
  inv1   g029(.a(x09), .O(new_n92_));
  nand4  g030(.a(new_n74_), .b(new_n79_), .c(x27), .d(new_n92_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(x35), .c(new_n81_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x04), .O(new_n95_));
  nand3  g033(.a(new_n79_), .b(new_n92_), .c(x04), .O(new_n96_));
  nand2  g034(.a(new_n63_), .b(x39), .O(new_n97_));
  aoi21  g035(.a(new_n96_), .b(new_n74_), .c(new_n97_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n95_), .c(new_n91_), .d(new_n88_), .O(z02));
  nand2  g037(.a(new_n69_), .b(x27), .O(new_n100_));
  nand2  g038(.a(new_n68_), .b(new_n67_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n100_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g040(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n100_), .c(new_n64_), .O(z04));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n63_), .c(new_n68_), .O(z05));
  aoi21  g044(.a(new_n63_), .b(new_n67_), .c(new_n81_), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(x37), .c(new_n63_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n65_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n63_), .c(x03), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z07));
  oai21  g057(.a(x39), .b(x26), .c(x40), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(x40), .O(z09));
  nand2  g061(.a(new_n120_), .b(x07), .O(new_n124_));
  inv1   g062(.a(x39), .O(new_n125_));
  nor2   g063(.a(new_n74_), .b(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n72_), .c(x05), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n124_), .c(new_n82_), .O(new_n128_));
  nand3  g066(.a(new_n63_), .b(x37), .c(x06), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  aoi21  g068(.a(new_n128_), .b(new_n89_), .c(new_n130_), .O(new_n131_));
  oai21  g069(.a(x32), .b(x30), .c(x40), .O(new_n132_));
  nor3   g070(.a(new_n132_), .b(new_n125_), .c(x26), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x05), .c(new_n89_), .O(new_n134_));
  oai21  g072(.a(new_n131_), .b(new_n67_), .c(new_n134_), .O(z10));
  nand3  g073(.a(x35), .b(new_n81_), .c(new_n89_), .O(new_n136_));
  aoi22  g074(.a(new_n136_), .b(new_n72_), .c(new_n63_), .d(new_n67_), .O(new_n137_));
  nand2  g075(.a(x35), .b(new_n81_), .O(new_n138_));
  aoi21  g076(.a(x39), .b(x29), .c(x26), .O(new_n139_));
  nor2   g077(.a(new_n67_), .b(new_n89_), .O(new_n140_));
  oai22  g078(.a(new_n140_), .b(new_n138_), .c(new_n139_), .d(new_n74_), .O(new_n141_));
  oai21  g079(.a(new_n137_), .b(x08), .c(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n79_), .c(new_n92_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(z11));
  inv1   g082(.a(x35), .O(new_n145_));
  inv1   g083(.a(x36), .O(new_n146_));
  nand3  g084(.a(new_n68_), .b(new_n146_), .c(new_n145_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  aoi21  g086(.a(x39), .b(new_n89_), .c(new_n70_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n72_), .O(new_n150_));
  or2    g088(.a(new_n70_), .b(x40), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n74_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n89_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand2  g096(.a(new_n69_), .b(new_n68_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n156_), .c(x26), .O(new_n161_));
  nand4  g099(.a(new_n159_), .b(new_n74_), .c(x20), .d(new_n158_), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(x18), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n161_), .c(x27), .O(new_n164_));
  nand3  g102(.a(new_n133_), .b(new_n153_), .c(new_n89_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(z13));
  nand3  g104(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n167_));
  nand4  g105(.a(new_n154_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n168_));
  nand3  g106(.a(x39), .b(new_n153_), .c(new_n89_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n168_), .c(x26), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n74_), .c(new_n167_), .O(new_n172_));
  nand3  g110(.a(new_n68_), .b(new_n145_), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n80_), .c(new_n79_), .O(new_n175_));
  oai21  g113(.a(new_n170_), .b(new_n70_), .c(new_n175_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n72_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n172_), .c(new_n151_), .O(z14));
  nand3  g116(.a(x34), .b(x27), .c(x12), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n74_), .c(new_n72_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n63_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n63_), .O(z17));
  nand2  g124(.a(new_n79_), .b(new_n92_), .O(new_n187_));
  nand2  g125(.a(x27), .b(new_n89_), .O(new_n188_));
  nand2  g126(.a(new_n67_), .b(x08), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n188_), .c(new_n145_), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n81_), .c(new_n187_), .O(new_n191_));
  nand4  g129(.a(new_n126_), .b(x29), .c(new_n72_), .d(x08), .O(new_n192_));
  oai21  g130(.a(new_n191_), .b(new_n64_), .c(new_n192_), .O(z18));
  nand2  g131(.a(x39), .b(x29), .O(new_n194_));
  nor2   g132(.a(new_n81_), .b(x27), .O(new_n195_));
  nor2   g133(.a(x35), .b(x26), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n195_), .c(new_n194_), .O(new_n197_));
  inv1   g135(.a(x08), .O(new_n198_));
  aoi21  g136(.a(x35), .b(x27), .c(x26), .O(new_n199_));
  nor2   g137(.a(x40), .b(x27), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n199_), .c(new_n198_), .O(new_n201_));
  oai21  g139(.a(new_n195_), .b(new_n145_), .c(new_n74_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n201_), .c(new_n197_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n79_), .c(new_n92_), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(new_n63_), .O(z20));
  inv1   g143(.a(new_n143_), .O(z19));
endmodule


