// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x05), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x05), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n70_), .O(z00));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x28), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n81_), .c(new_n80_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g030(.a(x28), .O(new_n93_));
  oai21  g031(.a(x35), .b(x05), .c(new_n93_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x04), .c(x05), .O(new_n95_));
  inv1   g033(.a(x39), .O(new_n96_));
  nor2   g034(.a(x27), .b(x08), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x04), .c(new_n96_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n95_), .c(new_n92_), .d(x40), .O(z02));
  nand2  g037(.a(new_n66_), .b(x27), .O(new_n100_));
  inv1   g038(.a(x21), .O(new_n101_));
  inv1   g039(.a(x27), .O(new_n102_));
  aoi21  g040(.a(new_n65_), .b(new_n102_), .c(new_n101_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n100_), .c(new_n79_), .O(z03));
  nand2  g042(.a(new_n65_), .b(new_n102_), .O(new_n105_));
  nand4  g043(.a(new_n105_), .b(new_n100_), .c(new_n64_), .d(new_n101_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n65_), .c(new_n79_), .O(z05));
  nand3  g046(.a(new_n107_), .b(new_n64_), .c(new_n65_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n72_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n64_), .O(z07));
  oai21  g059(.a(x39), .b(x05), .c(x40), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n64_), .O(z09));
  nand3  g063(.a(new_n64_), .b(x37), .c(x06), .O(new_n126_));
  inv1   g064(.a(x04), .O(new_n127_));
  nand4  g065(.a(new_n122_), .b(new_n86_), .c(x07), .d(new_n127_), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n126_), .c(new_n102_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  inv1   g068(.a(x09), .O(new_n131_));
  nand4  g069(.a(new_n80_), .b(new_n131_), .c(new_n130_), .d(x04), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n71_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x40), .O(new_n134_));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  nor2   g073(.a(new_n102_), .b(new_n127_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n85_), .c(new_n135_), .O(new_n137_));
  oai21  g075(.a(new_n85_), .b(new_n102_), .c(new_n130_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n80_), .c(new_n131_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n134_), .O(z11));
  nand3  g079(.a(new_n65_), .b(new_n82_), .c(new_n83_), .O(new_n142_));
  nor2   g080(.a(new_n74_), .b(x27), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n71_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n73_), .O(new_n146_));
  oai21  g084(.a(x27), .b(new_n127_), .c(new_n71_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x40), .O(new_n148_));
  oai21  g086(.a(new_n96_), .b(x04), .c(new_n71_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x40), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n66_), .c(new_n65_), .O(new_n151_));
  nand2  g089(.a(new_n122_), .b(new_n102_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(new_n151_), .c(new_n148_), .d(new_n146_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n74_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n127_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  nand4  g097(.a(new_n67_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n157_), .c(x05), .O(new_n161_));
  nand4  g099(.a(new_n67_), .b(new_n74_), .c(x20), .d(new_n159_), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(x18), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n161_), .c(x27), .O(new_n164_));
  oai21  g102(.a(x32), .b(x30), .c(x40), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(new_n96_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n154_), .c(new_n71_), .d(new_n127_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n164_), .O(z13));
  nand3  g106(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n169_));
  nand4  g107(.a(new_n155_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n170_));
  oai21  g108(.a(x13), .b(x04), .c(x40), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n170_), .c(x39), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n71_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x40), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n169_), .O(new_n175_));
  nand3  g113(.a(new_n65_), .b(new_n83_), .c(x28), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n81_), .c(new_n80_), .O(new_n178_));
  inv1   g116(.a(new_n67_), .O(new_n179_));
  oai22  g117(.a(new_n143_), .b(new_n179_), .c(x13), .d(x04), .O(new_n180_));
  nand2  g118(.a(new_n68_), .b(new_n96_), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n180_), .c(new_n178_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n71_), .O(new_n183_));
  nand2  g121(.a(new_n68_), .b(new_n74_), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n183_), .c(new_n175_), .O(z14));
  inv1   g123(.a(x12), .O(new_n186_));
  nand4  g124(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(new_n186_), .O(z15));
  inv1   g126(.a(x23), .O(new_n189_));
  nand3  g127(.a(new_n189_), .b(x22), .c(x01), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n64_), .O(z16));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(x23), .c(x22), .d(x01), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n64_), .O(z17));
  oai21  g132(.a(new_n85_), .b(x27), .c(new_n135_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(x08), .O(new_n196_));
  nor2   g134(.a(new_n102_), .b(x04), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n84_), .c(x30), .O(new_n198_));
  nand4  g136(.a(new_n198_), .b(new_n196_), .c(new_n64_), .d(new_n131_), .O(z18));
  inv1   g137(.a(new_n135_), .O(new_n200_));
  oai21  g138(.a(new_n83_), .b(new_n102_), .c(new_n130_), .O(new_n201_));
  aoi21  g139(.a(x28), .b(new_n102_), .c(new_n83_), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n200_), .c(new_n201_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n80_), .c(new_n131_), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(new_n64_), .O(z20));
  nand2  g143(.a(new_n140_), .b(new_n134_), .O(z19));
endmodule


