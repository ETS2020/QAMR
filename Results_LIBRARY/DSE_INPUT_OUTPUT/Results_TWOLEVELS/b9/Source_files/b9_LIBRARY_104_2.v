// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n189_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x03), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n63_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n71_), .O(z00));
  inv1   g015(.a(new_n64_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n78_), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  aoi21  g026(.a(new_n83_), .b(x04), .c(new_n88_), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  inv1   g028(.a(x27), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(new_n90_), .c(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(x40), .d(x39), .O(z02));
  nand2  g031(.a(new_n67_), .b(x27), .O(new_n94_));
  nand2  g032(.a(new_n66_), .b(new_n91_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g034(.a(new_n66_), .b(new_n91_), .c(x21), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n78_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n64_), .c(new_n66_), .O(z05));
  inv1   g038(.a(x03), .O(new_n101_));
  oai22  g039(.a(x40), .b(new_n101_), .c(new_n82_), .d(new_n91_), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(x37), .O(z06));
  inv1   g041(.a(x31), .O(new_n104_));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n65_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .d(new_n104_), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n107_), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(new_n110_), .c(new_n63_), .O(new_n113_));
  aoi21  g051(.a(new_n113_), .b(new_n109_), .c(new_n101_), .O(z07));
  nand2  g052(.a(x40), .b(x39), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n64_), .O(z09));
  inv1   g056(.a(x07), .O(new_n119_));
  inv1   g057(.a(x39), .O(new_n120_));
  nor2   g058(.a(x40), .b(x03), .O(new_n121_));
  aoi21  g059(.a(x40), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x05), .O(new_n123_));
  oai21  g061(.a(new_n122_), .b(new_n119_), .c(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n84_), .c(new_n72_), .O(new_n125_));
  nand3  g063(.a(new_n64_), .b(x37), .c(x06), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x27), .O(new_n128_));
  inv1   g066(.a(x05), .O(new_n129_));
  nand2  g067(.a(new_n80_), .b(new_n79_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(x40), .c(x39), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n72_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n128_), .O(z10));
  nand2  g072(.a(new_n64_), .b(new_n91_), .O(new_n135_));
  oai21  g073(.a(new_n83_), .b(x04), .c(x40), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n135_), .c(x08), .O(new_n137_));
  inv1   g075(.a(new_n83_), .O(new_n138_));
  inv1   g076(.a(new_n121_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  inv1   g078(.a(x29), .O(new_n141_));
  oai21  g079(.a(new_n120_), .b(new_n141_), .c(x40), .O(new_n142_));
  aoi22  g080(.a(new_n142_), .b(new_n139_), .c(new_n140_), .d(new_n138_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n137_), .c(new_n79_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(x09), .c(new_n64_), .O(z11));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  nand2  g084(.a(x40), .b(new_n91_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n146_), .c(new_n73_), .O(new_n149_));
  oai21  g087(.a(new_n120_), .b(x04), .c(new_n67_), .O(new_n150_));
  inv1   g088(.a(x35), .O(new_n151_));
  aoi21  g089(.a(new_n82_), .b(new_n101_), .c(new_n151_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(x40), .c(new_n150_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n66_), .O(new_n154_));
  aoi21  g092(.a(x27), .b(new_n101_), .c(x40), .O(new_n155_));
  nand2  g093(.a(x40), .b(x04), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x39), .c(x27), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n154_), .c(new_n149_), .O(z12));
  inv1   g097(.a(x13), .O(new_n160_));
  nand2  g098(.a(x36), .b(x35), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(x28), .c(new_n63_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x39), .c(new_n160_), .d(new_n72_), .O(new_n163_));
  inv1   g101(.a(x18), .O(new_n164_));
  inv1   g102(.a(x19), .O(new_n165_));
  nand4  g103(.a(new_n68_), .b(x20), .c(new_n165_), .d(new_n164_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n163_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x27), .O(new_n168_));
  nor2   g106(.a(new_n131_), .b(x13), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n72_), .c(new_n78_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n168_), .O(z13));
  nand3  g109(.a(x20), .b(new_n165_), .c(new_n164_), .O(new_n172_));
  nand4  g110(.a(new_n161_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n173_));
  nor2   g111(.a(x13), .b(x04), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(new_n173_), .c(x40), .d(x39), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n172_), .O(new_n176_));
  nor2   g114(.a(x37), .b(x35), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x28), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n147_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n80_), .c(new_n79_), .O(new_n180_));
  inv1   g118(.a(new_n174_), .O(new_n181_));
  nand2  g119(.a(new_n147_), .b(new_n68_), .O(new_n182_));
  oai21  g120(.a(new_n177_), .b(new_n91_), .c(new_n115_), .O(new_n183_));
  nand2  g121(.a(new_n139_), .b(x39), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n66_), .c(new_n82_), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n183_), .c(new_n64_), .O(new_n186_));
  aoi21  g124(.a(new_n182_), .b(new_n181_), .c(new_n186_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n180_), .c(new_n176_), .O(z14));
  nand4  g126(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n64_), .O(z15));
  inv1   g128(.a(x23), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(x22), .c(x01), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n64_), .O(z16));
  inv1   g131(.a(x24), .O(new_n194_));
  nand4  g132(.a(new_n194_), .b(x23), .c(x22), .d(x01), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n64_), .O(z17));
  oai22  g134(.a(new_n115_), .b(new_n141_), .c(new_n83_), .d(x27), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(x08), .O(new_n198_));
  nor2   g136(.a(new_n91_), .b(x04), .O(new_n199_));
  aoi21  g137(.a(new_n199_), .b(new_n138_), .c(x09), .O(new_n200_));
  nand4  g138(.a(new_n200_), .b(new_n198_), .c(new_n64_), .d(new_n79_), .O(z18));
  nor2   g139(.a(new_n144_), .b(x09), .O(z19));
  aoi21  g140(.a(x28), .b(new_n91_), .c(new_n151_), .O(new_n203_));
  aoi21  g141(.a(new_n142_), .b(new_n139_), .c(new_n203_), .O(new_n204_));
  nand2  g142(.a(x40), .b(new_n151_), .O(new_n205_));
  aoi21  g143(.a(new_n205_), .b(new_n135_), .c(x08), .O(new_n206_));
  oai21  g144(.a(new_n206_), .b(new_n204_), .c(new_n79_), .O(new_n207_));
  nor2   g145(.a(new_n207_), .b(x09), .O(z20));
endmodule


