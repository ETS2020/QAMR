// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n110_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n178_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x39), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n66_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x16), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n65_), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n75_), .c(new_n63_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n66_), .d(new_n74_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n73_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n64_), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  inv1   g027(.a(x35), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(x28), .O(new_n91_));
  inv1   g029(.a(x08), .O(new_n92_));
  nand2  g030(.a(new_n67_), .b(new_n92_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n91_), .c(new_n74_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n89_), .c(x39), .O(new_n95_));
  inv1   g033(.a(x18), .O(new_n96_));
  inv1   g034(.a(x19), .O(new_n97_));
  nand2  g035(.a(new_n69_), .b(new_n68_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x35), .c(new_n67_), .O(new_n100_));
  aoi21  g038(.a(new_n68_), .b(new_n83_), .c(new_n67_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n102_));
  oai21  g040(.a(new_n102_), .b(new_n100_), .c(new_n63_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n95_), .O(z02));
  nand2  g042(.a(new_n69_), .b(x27), .O(new_n105_));
  nand2  g043(.a(new_n68_), .b(new_n67_), .O(new_n106_));
  nand4  g044(.a(new_n106_), .b(new_n105_), .c(new_n65_), .d(x21), .O(z03));
  aoi21  g045(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n105_), .c(new_n64_), .O(z04));
  nand2  g047(.a(x28), .b(x27), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n65_), .c(new_n68_), .O(z05));
  inv1   g049(.a(z05), .O(z06));
  inv1   g050(.a(x33), .O(new_n113_));
  nand2  g051(.a(x17), .b(new_n66_), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  oai21  g053(.a(x25), .b(new_n115_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n115_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  oai22  g058(.a(new_n120_), .b(x14), .c(new_n117_), .d(x31), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x03), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n65_), .O(z07));
  inv1   g061(.a(x11), .O(new_n124_));
  nand4  g062(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n124_), .O(z09));
  inv1   g064(.a(x07), .O(new_n127_));
  nand2  g065(.a(x40), .b(x05), .O(new_n128_));
  oai21  g066(.a(x40), .b(new_n127_), .c(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n85_), .c(x27), .O(new_n130_));
  aoi21  g068(.a(new_n81_), .b(new_n80_), .c(new_n63_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x05), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n74_), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n65_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  nand2  g075(.a(x40), .b(x29), .O(new_n138_));
  nor2   g076(.a(new_n67_), .b(new_n74_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n84_), .c(new_n138_), .O(new_n140_));
  nand3  g078(.a(new_n91_), .b(x27), .c(new_n74_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n92_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n80_), .c(new_n137_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n65_), .O(z11));
  nand3  g083(.a(new_n68_), .b(new_n82_), .c(new_n90_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x27), .c(x10), .O(new_n147_));
  nor2   g085(.a(new_n70_), .b(new_n74_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n147_), .c(x39), .O(new_n149_));
  oai21  g087(.a(new_n70_), .b(x40), .c(new_n149_), .O(z12));
  inv1   g088(.a(x39), .O(new_n151_));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x28), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x40), .c(new_n152_), .d(new_n74_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n99_), .c(new_n151_), .O(new_n156_));
  nand4  g094(.a(new_n98_), .b(new_n63_), .c(x20), .d(new_n97_), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(x18), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n156_), .c(x27), .O(new_n159_));
  nand4  g097(.a(new_n131_), .b(x39), .c(new_n152_), .d(new_n74_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(z13));
  nand2  g099(.a(x39), .b(x04), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(x40), .c(new_n152_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n69_), .O(new_n164_));
  nand4  g102(.a(new_n90_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n68_), .O(new_n167_));
  nand3  g105(.a(x20), .b(new_n97_), .c(new_n96_), .O(new_n168_));
  nand4  g106(.a(new_n153_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x40), .c(new_n152_), .d(new_n74_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n168_), .O(new_n171_));
  aoi21  g109(.a(new_n81_), .b(new_n80_), .c(x13), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(new_n74_), .c(new_n151_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n63_), .c(new_n67_), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(new_n171_), .c(new_n167_), .d(new_n65_), .O(z14));
  inv1   g113(.a(x12), .O(new_n176_));
  nor2   g114(.a(new_n125_), .b(new_n176_), .O(z15));
  inv1   g115(.a(x23), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(x22), .c(x01), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n65_), .O(z16));
  inv1   g118(.a(x24), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(x23), .c(x22), .d(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n65_), .O(z17));
  oai21  g121(.a(new_n84_), .b(x27), .c(new_n138_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x08), .O(new_n185_));
  nor2   g123(.a(new_n64_), .b(x30), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n185_), .c(new_n141_), .d(new_n137_), .O(z18));
  inv1   g125(.a(new_n138_), .O(new_n188_));
  oai21  g126(.a(new_n90_), .b(new_n67_), .c(new_n92_), .O(new_n189_));
  aoi21  g127(.a(x28), .b(new_n67_), .c(new_n90_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n188_), .c(new_n189_), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(new_n80_), .c(new_n137_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n65_), .O(z20));
  buf    g131(.a(x40), .O(z08));
  nand2  g132(.a(new_n144_), .b(new_n65_), .O(z19));
endmodule


