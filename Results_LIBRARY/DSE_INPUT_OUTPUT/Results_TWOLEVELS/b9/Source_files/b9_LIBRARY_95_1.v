// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x39), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x15), .c(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x15), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n63_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n73_), .d(new_n72_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n71_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(x28), .c(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g024(.a(x39), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(x35), .c(new_n90_), .O(new_n94_));
  aoi22  g032(.a(new_n94_), .b(x04), .c(new_n89_), .d(new_n88_), .O(new_n95_));
  nand2  g033(.a(new_n90_), .b(x27), .O(new_n96_));
  nand2  g034(.a(new_n66_), .b(x28), .O(new_n97_));
  oai21  g035(.a(new_n97_), .b(new_n96_), .c(new_n63_), .O(new_n98_));
  oai21  g036(.a(new_n95_), .b(new_n87_), .c(new_n98_), .O(z02));
  nand2  g037(.a(new_n67_), .b(x27), .O(new_n100_));
  aoi21  g038(.a(new_n66_), .b(new_n92_), .c(new_n64_), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n100_), .c(x21), .O(z03));
  inv1   g040(.a(x21), .O(new_n103_));
  nand3  g041(.a(new_n101_), .b(new_n100_), .c(new_n103_), .O(z04));
  nor2   g042(.a(new_n90_), .b(new_n92_), .O(new_n105_));
  oai21  g043(.a(new_n105_), .b(x37), .c(new_n65_), .O(z06));
  inv1   g044(.a(z06), .O(z05));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n73_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n65_), .O(z07));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x05), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(x40), .b(x39), .O(new_n124_));
  oai22  g062(.a(new_n124_), .b(new_n122_), .c(x40), .d(new_n123_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n83_), .c(new_n72_), .O(new_n126_));
  nand3  g064(.a(new_n65_), .b(x37), .c(x06), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x27), .O(new_n129_));
  aoi21  g067(.a(new_n80_), .b(new_n79_), .c(new_n63_), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(x39), .c(x05), .d(new_n72_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  inv1   g071(.a(x29), .O(new_n134_));
  oai21  g072(.a(new_n92_), .b(new_n72_), .c(x35), .O(new_n135_));
  oai21  g073(.a(new_n63_), .b(new_n134_), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n89_), .b(x39), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x40), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x28), .O(new_n139_));
  nor2   g077(.a(new_n81_), .b(new_n92_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n72_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n91_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n139_), .c(new_n136_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n79_), .c(new_n133_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n65_), .O(z11));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  nor2   g084(.a(new_n87_), .b(x27), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n146_), .c(new_n74_), .O(new_n148_));
  oai21  g086(.a(new_n87_), .b(new_n72_), .c(x40), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n69_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n148_), .c(new_n65_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x28), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x40), .c(new_n152_), .d(new_n72_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  nand4  g095(.a(new_n68_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n155_), .c(new_n92_), .O(new_n159_));
  nand2  g097(.a(new_n80_), .b(new_n79_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n152_), .c(new_n72_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(x39), .c(new_n63_), .O(new_n162_));
  or2    g100(.a(new_n162_), .b(new_n159_), .O(z13));
  aoi21  g101(.a(new_n152_), .b(new_n72_), .c(new_n87_), .O(new_n164_));
  nor2   g102(.a(x19), .b(x18), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(new_n68_), .c(x27), .d(x20), .O(new_n166_));
  oai21  g104(.a(new_n164_), .b(new_n63_), .c(new_n166_), .O(new_n167_));
  nand3  g105(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n168_));
  nor2   g106(.a(x37), .b(x35), .O(new_n169_));
  aoi21  g107(.a(new_n168_), .b(new_n153_), .c(new_n169_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n90_), .c(x27), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(x39), .c(new_n80_), .d(new_n79_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n167_), .O(z14));
  inv1   g111(.a(x12), .O(new_n174_));
  nor2   g112(.a(new_n120_), .b(new_n174_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand4  g114(.a(new_n65_), .b(new_n176_), .c(x22), .d(x01), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(z16));
  inv1   g116(.a(x01), .O(new_n179_));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n65_), .b(new_n180_), .c(x23), .d(x22), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n179_), .O(z17));
  nand2  g120(.a(x27), .b(new_n72_), .O(new_n183_));
  nand2  g121(.a(new_n92_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n81_), .O(new_n185_));
  nand2  g123(.a(new_n79_), .b(new_n133_), .O(new_n186_));
  aoi21  g124(.a(new_n185_), .b(new_n90_), .c(new_n186_), .O(new_n187_));
  oai22  g125(.a(new_n187_), .b(new_n64_), .c(new_n124_), .d(new_n89_), .O(z18));
  nor2   g126(.a(new_n63_), .b(new_n134_), .O(new_n189_));
  aoi21  g127(.a(x28), .b(new_n92_), .c(new_n81_), .O(new_n190_));
  oai22  g128(.a(new_n190_), .b(new_n189_), .c(new_n140_), .d(x08), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(new_n79_), .c(new_n133_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n65_), .O(z20));
  buf    g131(.a(x40), .O(z08));
  nand2  g132(.a(new_n144_), .b(new_n65_), .O(z19));
endmodule


