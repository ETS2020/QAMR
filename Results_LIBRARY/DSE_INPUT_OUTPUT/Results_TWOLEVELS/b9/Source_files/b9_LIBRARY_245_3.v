// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n189_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x03), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x03), .c(new_n70_), .O(z00));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n79_), .O(z01));
  and2   g024(.a(x29), .b(x08), .O(new_n87_));
  nor2   g025(.a(x35), .b(x03), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(x28), .c(x04), .O(new_n89_));
  oai21  g027(.a(new_n87_), .b(x02), .c(new_n89_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x39), .O(new_n91_));
  nand3  g029(.a(new_n65_), .b(x28), .c(x04), .O(new_n92_));
  inv1   g030(.a(x08), .O(new_n93_));
  inv1   g031(.a(x27), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n93_), .c(x04), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n64_), .c(x39), .O(new_n96_));
  aoi21  g034(.a(new_n92_), .b(new_n74_), .c(new_n96_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n91_), .O(z02));
  nand2  g036(.a(new_n66_), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n64_), .b(new_n65_), .c(new_n94_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n64_), .d(x21), .O(z03));
  inv1   g039(.a(x21), .O(new_n102_));
  nand4  g040(.a(new_n100_), .b(new_n99_), .c(new_n64_), .d(new_n102_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n65_), .c(new_n79_), .O(z05));
  nand3  g043(.a(new_n104_), .b(new_n64_), .c(new_n65_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x03), .O(new_n108_));
  inv1   g046(.a(x31), .O(new_n109_));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n72_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand4  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .d(new_n109_), .O(new_n114_));
  inv1   g052(.a(x14), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n112_), .O(new_n117_));
  aoi21  g055(.a(new_n117_), .b(new_n115_), .c(x40), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n114_), .c(new_n108_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n108_), .c(new_n74_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  nor2   g062(.a(new_n83_), .b(x36), .O(new_n125_));
  inv1   g063(.a(x07), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x05), .O(new_n127_));
  oai21  g065(.a(x39), .b(new_n126_), .c(new_n127_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n108_), .O(new_n129_));
  nand2  g067(.a(new_n74_), .b(x07), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n129_), .c(new_n125_), .O(new_n131_));
  nand3  g069(.a(new_n64_), .b(x37), .c(x06), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(new_n133_));
  aoi21  g071(.a(new_n131_), .b(new_n71_), .c(new_n133_), .O(new_n134_));
  nand2  g072(.a(new_n81_), .b(new_n80_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x40), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(new_n120_), .O(new_n137_));
  nand4  g075(.a(new_n137_), .b(x05), .c(new_n71_), .d(new_n108_), .O(new_n138_));
  oai21  g076(.a(new_n134_), .b(new_n94_), .c(new_n138_), .O(z10));
  inv1   g077(.a(x09), .O(new_n140_));
  inv1   g078(.a(x28), .O(new_n141_));
  nand2  g079(.a(x39), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(x35), .c(new_n141_), .O(new_n143_));
  aoi22  g081(.a(new_n143_), .b(new_n108_), .c(new_n64_), .d(new_n94_), .O(new_n144_));
  nand2  g082(.a(x39), .b(x29), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n108_), .c(new_n74_), .O(new_n146_));
  nand2  g084(.a(x27), .b(x04), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n83_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n146_), .c(new_n144_), .d(x08), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n80_), .c(new_n140_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z11));
  inv1   g090(.a(x36), .O(new_n153_));
  nand3  g091(.a(new_n65_), .b(new_n153_), .c(new_n82_), .O(new_n154_));
  nand2  g092(.a(x39), .b(new_n94_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n154_), .c(x10), .O(new_n156_));
  oai21  g094(.a(new_n120_), .b(x04), .c(new_n94_), .O(new_n157_));
  nand2  g095(.a(x39), .b(new_n71_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n66_), .c(new_n65_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n156_), .c(new_n108_), .O(new_n161_));
  nand2  g099(.a(new_n68_), .b(new_n74_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n161_), .O(z12));
  inv1   g101(.a(x13), .O(new_n164_));
  nand2  g102(.a(x36), .b(x35), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x28), .c(new_n74_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x39), .c(new_n164_), .d(new_n71_), .O(new_n167_));
  inv1   g105(.a(x18), .O(new_n168_));
  inv1   g106(.a(x19), .O(new_n169_));
  nand4  g107(.a(new_n67_), .b(x20), .c(new_n169_), .d(new_n168_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n167_), .c(new_n94_), .O(new_n171_));
  nand4  g109(.a(new_n135_), .b(x39), .c(new_n164_), .d(new_n71_), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(new_n108_), .c(new_n74_), .O(new_n173_));
  or2    g111(.a(new_n173_), .b(new_n171_), .O(z13));
  nand3  g112(.a(x20), .b(new_n169_), .c(new_n168_), .O(new_n175_));
  nand4  g113(.a(new_n165_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n176_));
  nor2   g114(.a(x13), .b(x04), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x39), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n176_), .c(x03), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n74_), .c(new_n175_), .O(new_n181_));
  nand3  g119(.a(new_n65_), .b(new_n82_), .c(x28), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x27), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n81_), .c(new_n80_), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(new_n185_));
  aoi22  g123(.a(new_n177_), .b(x39), .c(new_n67_), .d(x27), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n185_), .c(new_n108_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n181_), .c(new_n162_), .O(z14));
  inv1   g126(.a(x12), .O(new_n189_));
  nor2   g127(.a(new_n123_), .b(new_n189_), .O(z15));
  inv1   g128(.a(x23), .O(new_n191_));
  nand4  g129(.a(new_n64_), .b(new_n191_), .c(x22), .d(x01), .O(new_n192_));
  inv1   g130(.a(new_n192_), .O(z16));
  inv1   g131(.a(x24), .O(new_n194_));
  nand4  g132(.a(new_n194_), .b(x23), .c(x22), .d(x01), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n64_), .O(z17));
  nand2  g134(.a(new_n83_), .b(new_n94_), .O(new_n197_));
  nand3  g135(.a(x40), .b(x39), .c(x29), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n197_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(x08), .O(new_n200_));
  nor2   g138(.a(new_n94_), .b(x04), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n83_), .c(x30), .O(new_n202_));
  nand4  g140(.a(new_n202_), .b(new_n200_), .c(new_n64_), .d(new_n140_), .O(z18));
  aoi21  g141(.a(x28), .b(new_n94_), .c(new_n82_), .O(new_n204_));
  aoi21  g142(.a(x35), .b(x27), .c(x03), .O(new_n205_));
  nor2   g143(.a(x40), .b(x27), .O(new_n206_));
  oai21  g144(.a(new_n206_), .b(new_n205_), .c(new_n93_), .O(new_n207_));
  oai21  g145(.a(new_n204_), .b(new_n146_), .c(new_n207_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n80_), .c(new_n140_), .O(new_n209_));
  inv1   g147(.a(new_n209_), .O(z20));
  inv1   g148(.a(new_n151_), .O(z19));
endmodule


