// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x36), .O(new_n66_));
  nand2  g004(.a(x40), .b(x35), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x39), .c(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(z08));
  nand3  g011(.a(z08), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  nor2   g014(.a(x40), .b(new_n66_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(x16), .O(z00));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n64_), .c(new_n65_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x35), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(x28), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(x36), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n83_), .c(new_n82_), .O(new_n87_));
  oai21  g025(.a(new_n81_), .b(new_n77_), .c(new_n87_), .O(z01));
  and2   g026(.a(x29), .b(x08), .O(new_n89_));
  aoi21  g027(.a(x40), .b(new_n84_), .c(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(new_n72_), .O(new_n92_));
  oai21  g030(.a(new_n89_), .b(x02), .c(new_n92_), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  inv1   g034(.a(x37), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n64_), .c(new_n96_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n95_), .c(new_n77_), .O(z03));
  nand2  g037(.a(new_n97_), .b(new_n64_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n95_), .c(new_n78_), .d(new_n96_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n77_), .O(z05));
  inv1   g041(.a(x28), .O(new_n104_));
  oai22  g042(.a(x40), .b(new_n66_), .c(new_n104_), .d(new_n64_), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x37), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n63_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n78_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n78_), .O(z09));
  oai21  g057(.a(new_n67_), .b(x28), .c(new_n66_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(x39), .c(x05), .O(new_n121_));
  nand3  g059(.a(new_n72_), .b(x35), .c(new_n104_), .O(new_n122_));
  inv1   g060(.a(x39), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x36), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x07), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n121_), .c(new_n64_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  inv1   g066(.a(x40), .O(new_n129_));
  nor2   g067(.a(new_n80_), .b(new_n129_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x39), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n127_), .c(new_n65_), .O(new_n133_));
  nor2   g071(.a(new_n97_), .b(new_n64_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(x06), .c(new_n77_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n133_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  nand2  g075(.a(z08), .b(x29), .O(new_n138_));
  oai21  g076(.a(new_n64_), .b(new_n65_), .c(new_n85_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  inv1   g078(.a(x08), .O(new_n141_));
  nand3  g079(.a(new_n85_), .b(x27), .c(new_n65_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n82_), .c(new_n137_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n78_), .O(z11));
  nor2   g084(.a(x37), .b(x36), .O(new_n147_));
  aoi22  g085(.a(new_n147_), .b(new_n84_), .c(x40), .d(new_n64_), .O(new_n148_));
  nor2   g086(.a(new_n70_), .b(new_n64_), .O(new_n149_));
  nor2   g087(.a(x40), .b(x36), .O(new_n150_));
  nand2  g088(.a(x39), .b(new_n65_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x40), .c(new_n150_), .O(new_n152_));
  oai22  g090(.a(new_n152_), .b(new_n149_), .c(new_n148_), .d(x10), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  oai21  g093(.a(new_n129_), .b(x28), .c(new_n155_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n65_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  inv1   g097(.a(new_n70_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nand4  g101(.a(new_n130_), .b(x39), .c(new_n154_), .d(new_n65_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n163_), .c(new_n78_), .O(z13));
  nor2   g103(.a(new_n149_), .b(new_n129_), .O(new_n166_));
  nand3  g104(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n166_), .c(new_n151_), .O(new_n168_));
  oai21  g106(.a(new_n150_), .b(x13), .c(new_n94_), .O(new_n169_));
  nand4  g107(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n97_), .O(new_n172_));
  nor2   g110(.a(new_n80_), .b(x13), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(new_n129_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n150_), .c(new_n64_), .O(new_n175_));
  nand4  g113(.a(new_n155_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(x40), .c(new_n154_), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n167_), .c(new_n77_), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n175_), .c(new_n172_), .d(new_n168_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nand4  g118(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n180_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(x22), .c(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n78_), .O(z16));
  inv1   g123(.a(x01), .O(new_n186_));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n78_), .b(new_n187_), .c(x23), .d(x22), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n186_), .O(z17));
  nand2  g127(.a(new_n85_), .b(new_n64_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n138_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(x08), .O(new_n192_));
  nor2   g130(.a(new_n77_), .b(x30), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(new_n192_), .c(new_n142_), .d(new_n137_), .O(z18));
  inv1   g132(.a(new_n138_), .O(new_n195_));
  oai21  g133(.a(new_n84_), .b(new_n64_), .c(new_n141_), .O(new_n196_));
  aoi21  g134(.a(x28), .b(new_n64_), .c(new_n84_), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n195_), .c(new_n196_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n82_), .c(new_n137_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n78_), .O(z20));
  nand2  g138(.a(new_n145_), .b(new_n78_), .O(z19));
endmodule


