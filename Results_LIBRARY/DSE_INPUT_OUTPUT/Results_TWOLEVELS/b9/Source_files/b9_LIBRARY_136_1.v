// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_;
  inv1   g000(.a(x31), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(x31), .d(new_n66_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n72_), .d(new_n65_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n64_), .c(x04), .O(z01));
  and2   g025(.a(x29), .b(x08), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(x02), .O(new_n89_));
  inv1   g027(.a(x04), .O(new_n90_));
  inv1   g028(.a(new_n83_), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n89_), .c(x31), .O(new_n93_));
  inv1   g031(.a(x08), .O(new_n94_));
  nand3  g032(.a(new_n68_), .b(new_n94_), .c(x04), .O(new_n95_));
  inv1   g033(.a(x39), .O(new_n96_));
  nor2   g034(.a(new_n65_), .b(new_n96_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n93_), .d(x40), .O(z02));
  nand2  g036(.a(new_n70_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x21), .O(new_n100_));
  aoi21  g038(.a(new_n69_), .b(new_n68_), .c(new_n100_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n99_), .c(new_n65_), .O(z03));
  nand2  g040(.a(new_n69_), .b(new_n68_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n99_), .c(new_n64_), .d(new_n100_), .O(z04));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n69_), .c(new_n65_), .O(z05));
  nand3  g044(.a(new_n105_), .b(new_n64_), .c(new_n69_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n66_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand4  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .d(new_n63_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(x14), .c(new_n113_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(z07));
  nand3  g057(.a(x40), .b(x39), .c(x31), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  inv1   g062(.a(x07), .O(new_n125_));
  aoi21  g063(.a(new_n96_), .b(x31), .c(new_n74_), .O(new_n126_));
  nand4  g064(.a(x40), .b(x39), .c(x31), .d(x05), .O(new_n127_));
  oai21  g065(.a(new_n126_), .b(new_n125_), .c(new_n127_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n84_), .c(new_n90_), .O(new_n129_));
  nand3  g067(.a(new_n64_), .b(x37), .c(x06), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n80_), .b(new_n79_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x40), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n96_), .O(new_n135_));
  nand4  g073(.a(new_n135_), .b(x31), .c(x05), .d(new_n90_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n132_), .O(z10));
  aoi21  g075(.a(x40), .b(new_n63_), .c(x27), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n83_), .b(x04), .c(x31), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n139_), .c(x08), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x31), .c(new_n74_), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n91_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n141_), .c(new_n79_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(x09), .O(z11));
  inv1   g085(.a(x35), .O(new_n148_));
  nand3  g086(.a(new_n69_), .b(new_n81_), .c(new_n148_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x27), .c(x10), .O(new_n150_));
  aoi21  g088(.a(x39), .b(new_n90_), .c(new_n71_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(x31), .O(new_n152_));
  or2    g090(.a(new_n71_), .b(x40), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x28), .c(new_n74_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n90_), .O(new_n158_));
  inv1   g096(.a(x18), .O(new_n159_));
  inv1   g097(.a(x19), .O(new_n160_));
  nand2  g098(.a(new_n70_), .b(new_n69_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x20), .c(new_n160_), .d(new_n159_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n158_), .c(new_n68_), .O(new_n163_));
  nand4  g101(.a(new_n133_), .b(x39), .c(new_n155_), .d(new_n90_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(x31), .c(new_n74_), .O(new_n165_));
  or2    g103(.a(new_n165_), .b(new_n163_), .O(z13));
  nand3  g104(.a(x20), .b(new_n160_), .c(new_n159_), .O(new_n167_));
  nand4  g105(.a(new_n156_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n168_));
  nand3  g106(.a(x39), .b(new_n155_), .c(new_n90_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n168_), .c(new_n63_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n74_), .c(new_n167_), .O(new_n172_));
  nand3  g110(.a(new_n69_), .b(new_n148_), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n80_), .c(new_n79_), .O(new_n175_));
  oai21  g113(.a(new_n170_), .b(new_n71_), .c(new_n175_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x31), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n172_), .c(new_n153_), .O(z14));
  inv1   g116(.a(x12), .O(new_n179_));
  nor2   g117(.a(new_n123_), .b(new_n179_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n64_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x01), .O(new_n184_));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n64_), .b(new_n185_), .c(x23), .d(x22), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z17));
  nand2  g125(.a(x27), .b(new_n90_), .O(new_n188_));
  nand2  g126(.a(new_n68_), .b(x08), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n188_), .c(new_n148_), .O(new_n190_));
  inv1   g128(.a(x09), .O(new_n191_));
  nand2  g129(.a(new_n79_), .b(new_n191_), .O(new_n192_));
  aoi21  g130(.a(new_n190_), .b(new_n82_), .c(new_n192_), .O(new_n193_));
  nand4  g131(.a(new_n88_), .b(x40), .c(x39), .d(x31), .O(new_n194_));
  oai21  g132(.a(new_n193_), .b(new_n65_), .c(new_n194_), .O(z18));
  nor2   g133(.a(new_n82_), .b(x27), .O(new_n196_));
  nor2   g134(.a(x35), .b(new_n63_), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n196_), .c(new_n142_), .O(new_n198_));
  oai21  g136(.a(new_n197_), .b(new_n138_), .c(new_n94_), .O(new_n199_));
  oai21  g137(.a(new_n196_), .b(new_n148_), .c(new_n74_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n199_), .c(new_n198_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n79_), .c(new_n191_), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n64_), .O(z20));
  nor2   g141(.a(new_n146_), .b(x09), .O(z19));
endmodule


