// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n175_, new_n176_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_;
  nand2  g000(.a(x40), .b(x32), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x32), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  aoi21  g018(.a(x35), .b(new_n80_), .c(x36), .O(new_n81_));
  or2    g019(.a(new_n81_), .b(new_n67_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n72_), .c(new_n79_), .O(new_n83_));
  oai21  g021(.a(new_n64_), .b(x04), .c(new_n83_), .O(z01));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n80_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x04), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n87_), .c(x39), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n72_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x40), .O(z02));
  nand2  g031(.a(new_n69_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n68_), .b(new_n67_), .c(new_n95_), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n94_), .c(new_n64_), .O(z03));
  aoi21  g035(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n94_), .c(new_n64_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n64_), .O(z05));
  aoi21  g039(.a(x40), .b(x32), .c(x27), .O(new_n102_));
  oai21  g040(.a(new_n102_), .b(new_n80_), .c(new_n68_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n63_), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n65_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n63_), .O(z07));
  oai21  g053(.a(x39), .b(x32), .c(x40), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  inv1   g058(.a(x04), .O(new_n121_));
  nand2  g059(.a(new_n116_), .b(x07), .O(new_n122_));
  nand4  g060(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n81_), .O(new_n124_));
  nor2   g062(.a(new_n64_), .b(new_n68_), .O(new_n125_));
  aoi22  g063(.a(new_n125_), .b(x06), .c(new_n124_), .d(new_n121_), .O(new_n126_));
  nand2  g064(.a(x40), .b(x39), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(x32), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x30), .c(x05), .d(new_n121_), .O(new_n129_));
  oai21  g067(.a(new_n126_), .b(new_n67_), .c(new_n129_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  nand3  g069(.a(x35), .b(new_n80_), .c(new_n121_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n72_), .c(new_n102_), .O(new_n133_));
  aoi21  g071(.a(x39), .b(x29), .c(x32), .O(new_n134_));
  nor2   g072(.a(new_n67_), .b(new_n121_), .O(new_n135_));
  oai22  g073(.a(new_n135_), .b(new_n88_), .c(new_n134_), .d(new_n74_), .O(new_n136_));
  oai21  g074(.a(new_n133_), .b(x08), .c(new_n136_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n79_), .c(new_n131_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z11));
  inv1   g077(.a(x35), .O(new_n140_));
  inv1   g078(.a(x36), .O(new_n141_));
  nand3  g079(.a(new_n68_), .b(new_n141_), .c(new_n140_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  aoi21  g081(.a(x39), .b(new_n121_), .c(new_n70_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(new_n72_), .O(new_n145_));
  or2    g083(.a(new_n70_), .b(x40), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x28), .c(new_n74_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x39), .c(new_n148_), .d(new_n121_), .O(new_n151_));
  inv1   g089(.a(x18), .O(new_n152_));
  inv1   g090(.a(x19), .O(new_n153_));
  nand2  g091(.a(new_n69_), .b(new_n68_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x20), .c(new_n153_), .d(new_n152_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n151_), .c(x32), .O(new_n156_));
  nand4  g094(.a(new_n154_), .b(new_n74_), .c(x20), .d(new_n153_), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(x18), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n156_), .c(x27), .O(new_n159_));
  nand4  g097(.a(new_n128_), .b(x30), .c(new_n148_), .d(new_n121_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(z13));
  nand3  g099(.a(x20), .b(new_n153_), .c(new_n152_), .O(new_n162_));
  nand3  g100(.a(new_n149_), .b(new_n79_), .c(x28), .O(new_n163_));
  nand3  g101(.a(x39), .b(new_n148_), .c(new_n121_), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n163_), .c(x32), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n74_), .c(new_n162_), .O(new_n167_));
  nand3  g105(.a(new_n68_), .b(new_n140_), .c(x28), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(x27), .c(x30), .O(new_n169_));
  nor2   g107(.a(new_n165_), .b(new_n70_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n169_), .c(new_n72_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n167_), .c(new_n146_), .O(z14));
  inv1   g110(.a(x12), .O(new_n173_));
  nor2   g111(.a(new_n119_), .b(new_n173_), .O(z15));
  inv1   g112(.a(x23), .O(new_n175_));
  nand4  g113(.a(new_n63_), .b(new_n175_), .c(x22), .d(x01), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(z16));
  inv1   g115(.a(x24), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(x23), .c(x22), .d(x01), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n63_), .O(z17));
  inv1   g118(.a(new_n128_), .O(new_n181_));
  nand2  g119(.a(x27), .b(new_n121_), .O(new_n182_));
  nand2  g120(.a(new_n67_), .b(x08), .O(new_n183_));
  aoi21  g121(.a(new_n183_), .b(new_n182_), .c(new_n140_), .O(new_n184_));
  nand2  g122(.a(new_n79_), .b(new_n131_), .O(new_n185_));
  aoi21  g123(.a(new_n184_), .b(new_n80_), .c(new_n185_), .O(new_n186_));
  oai22  g124(.a(new_n186_), .b(new_n64_), .c(new_n181_), .d(new_n86_), .O(z18));
  nand2  g125(.a(x39), .b(x29), .O(new_n188_));
  nor2   g126(.a(new_n80_), .b(x27), .O(new_n189_));
  nor2   g127(.a(x35), .b(x32), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n189_), .c(new_n188_), .O(new_n191_));
  inv1   g129(.a(x08), .O(new_n192_));
  oai21  g130(.a(new_n190_), .b(new_n102_), .c(new_n192_), .O(new_n193_));
  oai21  g131(.a(new_n189_), .b(new_n140_), .c(new_n74_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n193_), .c(new_n191_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n79_), .c(new_n131_), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n63_), .O(z20));
  inv1   g135(.a(new_n138_), .O(z19));
endmodule


