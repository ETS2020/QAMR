// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  nand2  g017(.a(x40), .b(x32), .O(new_n80_));
  and2   g018(.a(new_n80_), .b(x16), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n79_), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  inv1   g024(.a(new_n86_), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(x36), .c(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n84_), .c(new_n83_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n80_), .c(x04), .O(z01));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(x32), .c(x40), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n86_), .c(x04), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(new_n76_), .O(z02));
  inv1   g033(.a(new_n80_), .O(new_n96_));
  nand2  g034(.a(new_n71_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n70_), .b(new_n64_), .c(new_n98_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n97_), .c(new_n96_), .O(z03));
  nand2  g038(.a(new_n70_), .b(new_n64_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n97_), .c(new_n80_), .d(new_n98_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n80_), .c(new_n70_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x14), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n108_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  oai21  g052(.a(new_n110_), .b(x31), .c(new_n114_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n80_), .O(z07));
  nand3  g055(.a(x40), .b(x39), .c(new_n84_), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n80_), .O(z09));
  aoi21  g059(.a(x35), .b(new_n85_), .c(x36), .O(new_n122_));
  oai21  g060(.a(x39), .b(x32), .c(x40), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x07), .O(new_n124_));
  nand4  g062(.a(x40), .b(x39), .c(new_n84_), .d(x05), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n122_), .O(new_n126_));
  nand3  g064(.a(new_n80_), .b(x37), .c(x06), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  aoi21  g066(.a(new_n126_), .b(new_n65_), .c(new_n128_), .O(new_n129_));
  nor2   g067(.a(new_n75_), .b(x32), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(x30), .c(x05), .d(new_n65_), .O(new_n131_));
  oai21  g069(.a(new_n129_), .b(new_n64_), .c(new_n131_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  aoi21  g071(.a(x40), .b(x32), .c(x27), .O(new_n134_));
  nand2  g072(.a(x40), .b(x04), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x35), .c(new_n85_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n84_), .c(new_n134_), .O(new_n137_));
  inv1   g075(.a(x40), .O(new_n138_));
  aoi21  g076(.a(x39), .b(x29), .c(x32), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x35), .c(new_n85_), .O(new_n141_));
  oai21  g079(.a(new_n139_), .b(new_n138_), .c(new_n141_), .O(new_n142_));
  oai21  g080(.a(new_n137_), .b(x08), .c(new_n142_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n83_), .c(new_n133_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n80_), .O(z11));
  nand3  g083(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n146_));
  nand2  g084(.a(x40), .b(new_n64_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n146_), .c(x10), .O(new_n148_));
  nand2  g086(.a(new_n135_), .b(x39), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n64_), .O(new_n150_));
  nand2  g088(.a(x39), .b(new_n65_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n71_), .c(new_n70_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n148_), .c(new_n84_), .O(new_n154_));
  oai21  g092(.a(new_n73_), .b(new_n64_), .c(new_n138_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x28), .c(new_n138_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand4  g100(.a(new_n72_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x27), .O(new_n165_));
  nand4  g103(.a(x39), .b(x30), .c(new_n157_), .d(new_n65_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n84_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x40), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(z13));
  oai21  g107(.a(x27), .b(new_n65_), .c(new_n84_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x40), .O(new_n171_));
  nand3  g109(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n172_));
  nand3  g110(.a(new_n158_), .b(new_n83_), .c(x28), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n76_), .c(new_n157_), .d(new_n65_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n172_), .O(new_n175_));
  inv1   g113(.a(new_n71_), .O(new_n176_));
  nand2  g114(.a(new_n151_), .b(new_n84_), .O(new_n177_));
  nor2   g115(.a(new_n138_), .b(x13), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n177_), .c(new_n176_), .O(new_n179_));
  nand3  g117(.a(new_n66_), .b(new_n83_), .c(x28), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n179_), .c(new_n70_), .O(new_n182_));
  inv1   g120(.a(new_n178_), .O(new_n183_));
  aoi21  g121(.a(x39), .b(x30), .c(x32), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n183_), .c(new_n64_), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n182_), .c(new_n175_), .d(new_n171_), .O(z14));
  nand4  g124(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n80_), .O(z15));
  inv1   g126(.a(x23), .O(new_n189_));
  nand3  g127(.a(new_n189_), .b(x22), .c(x01), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n80_), .O(z16));
  inv1   g129(.a(x01), .O(new_n192_));
  inv1   g130(.a(x24), .O(new_n193_));
  nand4  g131(.a(new_n80_), .b(new_n193_), .c(x23), .d(x22), .O(new_n194_));
  nor2   g132(.a(new_n194_), .b(new_n192_), .O(z17));
  inv1   g133(.a(x29), .O(new_n196_));
  oai22  g134(.a(new_n86_), .b(x27), .c(new_n75_), .d(new_n196_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(x08), .O(new_n198_));
  nor2   g136(.a(new_n64_), .b(x04), .O(new_n199_));
  aoi21  g137(.a(new_n199_), .b(new_n87_), .c(x09), .O(new_n200_));
  nand4  g138(.a(new_n200_), .b(new_n198_), .c(new_n80_), .d(new_n83_), .O(z18));
  inv1   g139(.a(new_n144_), .O(z19));
  nand2  g140(.a(x39), .b(x29), .O(new_n203_));
  nor2   g141(.a(new_n85_), .b(x27), .O(new_n204_));
  nor2   g142(.a(x35), .b(x32), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n204_), .c(new_n203_), .O(new_n206_));
  inv1   g144(.a(x08), .O(new_n207_));
  oai21  g145(.a(new_n205_), .b(new_n134_), .c(new_n207_), .O(new_n208_));
  oai21  g146(.a(new_n204_), .b(new_n66_), .c(new_n138_), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(new_n208_), .c(new_n206_), .O(new_n210_));
  nand3  g148(.a(new_n210_), .b(new_n83_), .c(new_n133_), .O(new_n211_));
  nand2  g149(.a(new_n211_), .b(new_n80_), .O(z20));
endmodule


