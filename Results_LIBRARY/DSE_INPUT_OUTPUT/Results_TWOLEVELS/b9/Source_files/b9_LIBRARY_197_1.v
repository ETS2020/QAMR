// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x16), .O(new_n77_));
  inv1   g015(.a(x40), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x30), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n76_), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand4  g025(.a(new_n87_), .b(new_n78_), .c(new_n83_), .d(new_n82_), .O(new_n88_));
  oai21  g026(.a(new_n79_), .b(x04), .c(new_n88_), .O(z01));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n85_), .c(x04), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x39), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n90_), .c(x30), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(z02));
  oai21  g033(.a(new_n66_), .b(new_n84_), .c(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n64_), .c(new_n97_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n79_), .O(z03));
  aoi21  g038(.a(new_n98_), .b(new_n64_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n96_), .c(new_n79_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n98_), .c(new_n79_), .O(z05));
  inv1   g042(.a(new_n79_), .O(new_n105_));
  nand3  g043(.a(new_n103_), .b(new_n105_), .c(new_n98_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n63_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x14), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n110_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  oai21  g054(.a(new_n112_), .b(x31), .c(new_n116_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n105_), .O(z07));
  nand3  g057(.a(x40), .b(x39), .c(x30), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n105_), .O(z09));
  nand4  g061(.a(new_n86_), .b(new_n72_), .c(x27), .d(x07), .O(new_n124_));
  nand2  g062(.a(new_n73_), .b(x05), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n65_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n105_), .O(z10));
  inv1   g067(.a(x09), .O(new_n130_));
  inv1   g068(.a(new_n85_), .O(new_n131_));
  aoi21  g069(.a(x27), .b(x04), .c(new_n91_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n131_), .c(x40), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n78_), .c(x30), .O(z11));
  nand3  g073(.a(new_n98_), .b(new_n67_), .c(new_n66_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x27), .c(x10), .O(new_n137_));
  nor2   g075(.a(new_n70_), .b(new_n64_), .O(new_n138_));
  aoi21  g076(.a(x39), .b(new_n65_), .c(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n137_), .c(x30), .O(new_n140_));
  oai21  g078(.a(new_n70_), .b(new_n64_), .c(new_n78_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(z12));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand4  g082(.a(new_n138_), .b(x20), .c(new_n144_), .d(new_n143_), .O(new_n145_));
  inv1   g083(.a(x13), .O(new_n146_));
  nand3  g084(.a(x39), .b(new_n146_), .c(new_n65_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x30), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x40), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n145_), .O(z13));
  nand4  g088(.a(x27), .b(x20), .c(new_n144_), .d(new_n143_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n70_), .O(new_n152_));
  aoi21  g090(.a(new_n148_), .b(x40), .c(new_n152_), .O(z14));
  inv1   g091(.a(x12), .O(new_n154_));
  nand4  g092(.a(new_n105_), .b(x34), .c(x27), .d(x26), .O(new_n155_));
  nor2   g093(.a(new_n155_), .b(new_n154_), .O(z15));
  inv1   g094(.a(x23), .O(new_n157_));
  nand4  g095(.a(new_n105_), .b(new_n157_), .c(x22), .d(x01), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z16));
  inv1   g097(.a(x24), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x23), .c(x22), .d(x01), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n105_), .O(z17));
  nand2  g100(.a(x27), .b(new_n65_), .O(new_n163_));
  nand2  g101(.a(new_n64_), .b(x08), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n163_), .c(new_n66_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n84_), .c(x09), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(x40), .c(new_n82_), .O(z18));
  nand3  g105(.a(new_n133_), .b(new_n82_), .c(new_n130_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z19));
  inv1   g107(.a(x08), .O(new_n170_));
  oai21  g108(.a(x28), .b(new_n170_), .c(new_n64_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x35), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(new_n78_), .c(new_n82_), .d(new_n130_), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z20));
endmodule


