// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_;
  inv1   g000(.a(x39), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(x15), .c(x16), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n70_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(x28), .c(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nor2   g026(.a(new_n81_), .b(x28), .O(new_n89_));
  oai21  g027(.a(x27), .b(x08), .c(new_n89_), .O(new_n90_));
  aoi22  g028(.a(new_n90_), .b(x04), .c(new_n88_), .d(new_n87_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n63_), .c(x40), .O(z02));
  nor2   g030(.a(x37), .b(x27), .O(new_n93_));
  aoi21  g031(.a(new_n66_), .b(x27), .c(new_n93_), .O(new_n94_));
  aoi22  g032(.a(new_n94_), .b(x21), .c(x40), .d(new_n63_), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi22  g034(.a(new_n94_), .b(new_n96_), .c(x40), .d(new_n63_), .O(z04));
  inv1   g035(.a(new_n64_), .O(new_n98_));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n98_), .O(z05));
  inv1   g038(.a(x27), .O(new_n101_));
  inv1   g039(.a(x28), .O(new_n102_));
  aoi21  g040(.a(new_n64_), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  oai21  g041(.a(new_n103_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n72_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n64_), .O(z07));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  nand2  g056(.a(x40), .b(x05), .O(new_n119_));
  oai21  g057(.a(x40), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n83_), .c(x27), .O(new_n121_));
  aoi21  g059(.a(new_n80_), .b(new_n79_), .c(new_n74_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x05), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n71_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n64_), .O(z10));
  inv1   g065(.a(x29), .O(new_n128_));
  aoi21  g066(.a(x39), .b(new_n128_), .c(new_n74_), .O(new_n129_));
  nand2  g067(.a(x27), .b(x04), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n89_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n64_), .b(new_n101_), .O(new_n132_));
  nand3  g070(.a(x35), .b(new_n102_), .c(new_n71_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x39), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n132_), .c(x08), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n131_), .c(new_n79_), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(x09), .O(z11));
  nor3   g075(.a(x37), .b(x36), .c(x35), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n101_), .c(new_n73_), .O(new_n139_));
  oai21  g077(.a(new_n63_), .b(new_n71_), .c(x40), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n68_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n139_), .c(new_n64_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x28), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(x40), .c(new_n143_), .d(new_n71_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand4  g086(.a(new_n67_), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n146_), .c(new_n63_), .O(new_n150_));
  nand4  g088(.a(new_n67_), .b(new_n74_), .c(x20), .d(new_n148_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(x18), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(x27), .O(new_n153_));
  nand4  g091(.a(new_n122_), .b(x39), .c(new_n143_), .d(new_n71_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  aoi21  g093(.a(new_n143_), .b(new_n71_), .c(new_n63_), .O(new_n156_));
  nor2   g094(.a(x19), .b(x18), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(new_n67_), .c(x27), .d(x20), .O(new_n158_));
  oai21  g096(.a(new_n156_), .b(new_n74_), .c(new_n158_), .O(new_n159_));
  nand3  g097(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n160_));
  nor2   g098(.a(x37), .b(x35), .O(new_n161_));
  aoi21  g099(.a(new_n160_), .b(new_n144_), .c(new_n161_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n102_), .c(x27), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x39), .c(new_n80_), .d(new_n79_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n159_), .O(z14));
  inv1   g103(.a(x12), .O(new_n166_));
  nand4  g104(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(new_n166_), .O(z15));
  inv1   g106(.a(x23), .O(new_n169_));
  nand4  g107(.a(new_n64_), .b(new_n169_), .c(x22), .d(x01), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z16));
  inv1   g109(.a(x01), .O(new_n172_));
  inv1   g110(.a(x24), .O(new_n173_));
  nand4  g111(.a(new_n64_), .b(new_n173_), .c(x23), .d(x22), .O(new_n174_));
  nor2   g112(.a(new_n174_), .b(new_n172_), .O(z17));
  nand2  g113(.a(x40), .b(x39), .O(new_n176_));
  nand2  g114(.a(x27), .b(new_n71_), .O(new_n177_));
  nand2  g115(.a(new_n101_), .b(x08), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n177_), .c(new_n81_), .O(new_n179_));
  inv1   g117(.a(x09), .O(new_n180_));
  nand2  g118(.a(new_n79_), .b(new_n180_), .O(new_n181_));
  aoi21  g119(.a(new_n179_), .b(new_n102_), .c(new_n181_), .O(new_n182_));
  oai22  g120(.a(new_n182_), .b(new_n98_), .c(new_n176_), .d(new_n88_), .O(z18));
  aoi21  g121(.a(x28), .b(new_n101_), .c(new_n81_), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n129_), .O(new_n185_));
  nand2  g123(.a(x39), .b(new_n81_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n132_), .c(x08), .O(new_n187_));
  oai21  g125(.a(new_n187_), .b(new_n185_), .c(new_n79_), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(x09), .O(z20));
  buf    g127(.a(x40), .O(z08));
  nor2   g128(.a(new_n136_), .b(x09), .O(z19));
endmodule


