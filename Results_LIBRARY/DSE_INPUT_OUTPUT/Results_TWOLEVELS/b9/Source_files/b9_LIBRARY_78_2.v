// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n74_), .c(new_n63_), .O(new_n80_));
  nand2  g018(.a(new_n76_), .b(x31), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n80_), .c(x16), .O(z00));
  inv1   g020(.a(new_n81_), .O(new_n83_));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n67_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n85_), .c(new_n84_), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x04), .c(new_n83_), .O(z01));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n87_), .c(x04), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x39), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x40), .O(new_n98_));
  nor2   g036(.a(x40), .b(x31), .O(new_n99_));
  inv1   g037(.a(new_n99_), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n98_), .O(z02));
  nand2  g039(.a(new_n71_), .b(x27), .O(new_n102_));
  nand2  g040(.a(new_n70_), .b(new_n64_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n102_), .c(new_n81_), .d(x21), .O(z03));
  aoi21  g042(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n102_), .c(new_n83_), .O(z04));
  aoi21  g044(.a(x28), .b(x27), .c(x37), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(new_n83_), .O(z05));
  nand2  g046(.a(new_n81_), .b(new_n64_), .O(new_n109_));
  inv1   g047(.a(x31), .O(new_n110_));
  oai21  g048(.a(x40), .b(new_n110_), .c(new_n86_), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n109_), .c(x37), .O(z06));
  inv1   g050(.a(x03), .O(new_n113_));
  nand2  g051(.a(x17), .b(new_n63_), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  oai21  g053(.a(x25), .b(new_n115_), .c(x38), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  inv1   g055(.a(x14), .O(new_n118_));
  inv1   g056(.a(x25), .O(new_n119_));
  nand3  g057(.a(x38), .b(new_n119_), .c(new_n115_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  oai21  g059(.a(new_n117_), .b(x33), .c(new_n121_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n110_), .O(new_n123_));
  nand3  g061(.a(new_n120_), .b(x40), .c(new_n118_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n113_), .O(z07));
  inv1   g063(.a(new_n77_), .O(new_n126_));
  nand2  g064(.a(new_n81_), .b(new_n126_), .O(z08));
  inv1   g065(.a(x11), .O(new_n128_));
  nand4  g066(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n128_), .O(z09));
  inv1   g068(.a(x07), .O(new_n131_));
  aoi21  g069(.a(x40), .b(new_n75_), .c(new_n99_), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x05), .O(new_n133_));
  oai21  g071(.a(new_n132_), .b(new_n131_), .c(new_n133_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n88_), .c(new_n65_), .O(new_n135_));
  nand3  g073(.a(new_n81_), .b(x37), .c(x06), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x27), .O(new_n138_));
  inv1   g076(.a(x05), .O(new_n139_));
  nand2  g077(.a(new_n85_), .b(new_n84_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x40), .c(x39), .O(new_n141_));
  nor2   g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n65_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n138_), .O(z10));
  oai21  g082(.a(new_n87_), .b(x04), .c(x40), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n109_), .c(x08), .O(new_n146_));
  inv1   g084(.a(new_n87_), .O(new_n147_));
  nand2  g085(.a(x39), .b(x29), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x40), .c(new_n99_), .O(new_n149_));
  nand2  g087(.a(x27), .b(x04), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n147_), .c(new_n149_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n146_), .c(new_n84_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(x09), .O(z11));
  inv1   g091(.a(x10), .O(new_n154_));
  nor3   g092(.a(x37), .b(x36), .c(x35), .O(new_n155_));
  nor2   g093(.a(new_n76_), .b(x27), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n155_), .c(new_n154_), .O(new_n157_));
  nand2  g095(.a(new_n72_), .b(x27), .O(new_n158_));
  aoi21  g096(.a(x39), .b(new_n65_), .c(new_n76_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n99_), .c(new_n158_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n157_), .c(new_n81_), .O(z12));
  inv1   g099(.a(x13), .O(new_n162_));
  nand2  g100(.a(x36), .b(x35), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x28), .c(new_n76_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x39), .c(new_n162_), .d(new_n65_), .O(new_n165_));
  inv1   g103(.a(x18), .O(new_n166_));
  inv1   g104(.a(x19), .O(new_n167_));
  nand4  g105(.a(new_n72_), .b(x20), .c(new_n167_), .d(new_n166_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x27), .O(new_n170_));
  nor2   g108(.a(new_n141_), .b(x13), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n65_), .c(new_n83_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n170_), .O(z13));
  nand3  g111(.a(x20), .b(new_n167_), .c(new_n166_), .O(new_n174_));
  nand4  g112(.a(new_n163_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n175_));
  nand3  g113(.a(x39), .b(new_n162_), .c(new_n65_), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n175_), .c(new_n76_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n99_), .c(new_n174_), .O(new_n179_));
  nand3  g117(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n85_), .c(new_n84_), .O(new_n182_));
  nand2  g120(.a(new_n176_), .b(new_n158_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x40), .O(new_n185_));
  nand3  g123(.a(new_n158_), .b(new_n76_), .c(new_n110_), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n185_), .c(new_n179_), .O(z14));
  nand4  g125(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n81_), .O(z15));
  inv1   g127(.a(x23), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x22), .c(x01), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n81_), .O(z16));
  inv1   g130(.a(x24), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x23), .c(x22), .d(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n81_), .O(z17));
  nand2  g133(.a(x27), .b(new_n65_), .O(new_n196_));
  nand2  g134(.a(new_n64_), .b(x08), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n196_), .c(new_n66_), .O(new_n198_));
  inv1   g136(.a(x09), .O(new_n199_));
  nand2  g137(.a(new_n84_), .b(new_n199_), .O(new_n200_));
  aoi21  g138(.a(new_n198_), .b(new_n86_), .c(new_n200_), .O(new_n201_));
  oai22  g139(.a(new_n201_), .b(new_n83_), .c(new_n93_), .d(new_n126_), .O(z18));
  oai22  g140(.a(new_n76_), .b(x35), .c(new_n86_), .d(x27), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n148_), .O(new_n204_));
  nor2   g142(.a(x40), .b(x35), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n95_), .c(new_n110_), .O(new_n206_));
  inv1   g144(.a(x08), .O(new_n207_));
  nand2  g145(.a(x35), .b(x27), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(x40), .c(new_n207_), .O(new_n209_));
  nand3  g147(.a(new_n76_), .b(x28), .c(new_n64_), .O(new_n210_));
  nand4  g148(.a(new_n210_), .b(new_n209_), .c(new_n206_), .d(new_n204_), .O(new_n211_));
  nand3  g149(.a(new_n211_), .b(new_n84_), .c(new_n199_), .O(new_n212_));
  nand2  g150(.a(new_n212_), .b(new_n81_), .O(z20));
  nor2   g151(.a(new_n152_), .b(x09), .O(z19));
endmodule


