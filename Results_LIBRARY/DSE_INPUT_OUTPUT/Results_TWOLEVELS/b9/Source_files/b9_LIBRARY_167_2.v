// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x38), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n63_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n66_), .d(new_n73_), .O(new_n77_));
  oai21  g015(.a(new_n72_), .b(new_n65_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  inv1   g027(.a(x29), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  inv1   g030(.a(x35), .O(new_n93_));
  nand2  g031(.a(x40), .b(new_n93_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n82_), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n92_), .c(x04), .O(new_n96_));
  nand2  g034(.a(x40), .b(x39), .O(new_n97_));
  inv1   g035(.a(new_n97_), .O(z08));
  nand3  g036(.a(z08), .b(new_n96_), .c(new_n91_), .O(z02));
  nand2  g037(.a(new_n70_), .b(x27), .O(new_n100_));
  inv1   g038(.a(x21), .O(new_n101_));
  aoi21  g039(.a(new_n69_), .b(new_n68_), .c(new_n101_), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n100_), .c(new_n65_), .O(z03));
  aoi21  g041(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n100_), .c(new_n65_), .O(z04));
  aoi21  g043(.a(x28), .b(x27), .c(x37), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(new_n65_), .O(z05));
  aoi21  g045(.a(new_n64_), .b(new_n68_), .c(new_n82_), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n66_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n64_), .O(z07));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n64_), .O(z09));
  inv1   g060(.a(x07), .O(new_n123_));
  inv1   g061(.a(x39), .O(new_n124_));
  nor2   g062(.a(x40), .b(x38), .O(new_n125_));
  aoi21  g063(.a(x40), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand2  g064(.a(z08), .b(x05), .O(new_n127_));
  oai21  g065(.a(new_n126_), .b(new_n123_), .c(new_n127_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n84_), .c(new_n73_), .O(new_n129_));
  nand3  g067(.a(new_n64_), .b(x37), .c(x06), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x27), .O(new_n132_));
  oai21  g070(.a(x32), .b(x30), .c(x40), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n124_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x05), .c(new_n73_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n132_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  nand2  g075(.a(new_n64_), .b(new_n68_), .O(new_n138_));
  nand4  g076(.a(new_n138_), .b(new_n94_), .c(new_n82_), .d(new_n73_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n89_), .O(new_n140_));
  nand2  g078(.a(x39), .b(x29), .O(new_n141_));
  nand2  g079(.a(x27), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n94_), .c(new_n82_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  inv1   g082(.a(x38), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n93_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n142_), .c(new_n82_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n63_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n144_), .c(new_n140_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n79_), .c(new_n137_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n64_), .O(z11));
  nand3  g089(.a(new_n69_), .b(new_n81_), .c(new_n93_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x27), .c(x10), .O(new_n153_));
  aoi21  g091(.a(x39), .b(new_n73_), .c(new_n71_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n153_), .c(x40), .O(new_n155_));
  nor2   g093(.a(new_n71_), .b(x40), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n145_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n155_), .O(z12));
  inv1   g096(.a(x13), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x28), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n159_), .d(new_n73_), .O(new_n162_));
  inv1   g100(.a(x18), .O(new_n163_));
  inv1   g101(.a(x19), .O(new_n164_));
  nand2  g102(.a(new_n70_), .b(new_n69_), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(x20), .c(new_n164_), .d(new_n163_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n162_), .c(new_n63_), .O(new_n167_));
  nand4  g105(.a(new_n165_), .b(new_n145_), .c(x20), .d(new_n164_), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(x18), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n167_), .c(x27), .O(new_n170_));
  nand3  g108(.a(new_n134_), .b(new_n159_), .c(new_n73_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(z13));
  nand3  g110(.a(x20), .b(new_n164_), .c(new_n163_), .O(new_n173_));
  nand4  g111(.a(new_n160_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n174_));
  nand3  g112(.a(x39), .b(new_n159_), .c(new_n73_), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n174_), .c(new_n63_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n125_), .c(new_n173_), .O(new_n178_));
  nand3  g116(.a(new_n69_), .b(new_n93_), .c(x28), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x27), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n80_), .c(new_n79_), .O(new_n181_));
  oai21  g119(.a(new_n176_), .b(new_n71_), .c(new_n181_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x40), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n178_), .c(new_n157_), .O(z14));
  inv1   g122(.a(x12), .O(new_n185_));
  nand4  g123(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n185_), .O(z15));
  inv1   g125(.a(x23), .O(new_n188_));
  nand3  g126(.a(new_n188_), .b(x22), .c(x01), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n64_), .O(z16));
  inv1   g128(.a(x24), .O(new_n191_));
  nand4  g129(.a(new_n191_), .b(x23), .c(x22), .d(x01), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n64_), .O(z17));
  oai22  g131(.a(new_n97_), .b(new_n90_), .c(new_n83_), .d(x27), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(x08), .O(new_n195_));
  inv1   g133(.a(new_n83_), .O(new_n196_));
  nor2   g134(.a(new_n68_), .b(x04), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n196_), .c(x09), .O(new_n198_));
  nand4  g136(.a(new_n198_), .b(new_n195_), .c(new_n64_), .d(new_n79_), .O(z18));
  aoi21  g137(.a(x28), .b(new_n68_), .c(new_n93_), .O(new_n200_));
  aoi21  g138(.a(new_n141_), .b(x40), .c(new_n125_), .O(new_n201_));
  nor2   g139(.a(new_n201_), .b(new_n200_), .O(new_n202_));
  aoi21  g140(.a(new_n138_), .b(new_n94_), .c(x08), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n202_), .c(new_n79_), .O(new_n204_));
  nor2   g142(.a(new_n204_), .b(x09), .O(z20));
  nand2  g143(.a(new_n150_), .b(new_n64_), .O(z19));
endmodule


