// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x15), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g009(.a(new_n71_), .b(x40), .c(x39), .d(new_n68_), .O(new_n72_));
  inv1   g010(.a(x37), .O(new_n73_));
  nand2  g011(.a(x35), .b(x28), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n72_), .c(new_n67_), .O(new_n77_));
  inv1   g015(.a(x39), .O(new_n78_));
  nor2   g016(.a(new_n63_), .b(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(x10), .c(new_n68_), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n77_), .c(new_n66_), .O(new_n82_));
  oai21  g020(.a(new_n65_), .b(x16), .c(new_n82_), .O(z00));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  nor2   g023(.a(new_n69_), .b(x28), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x36), .c(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g030(.a(x27), .b(x08), .c(new_n86_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x04), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(new_n79_), .O(z02));
  nand2  g033(.a(new_n74_), .b(x27), .O(new_n96_));
  aoi21  g034(.a(new_n73_), .b(new_n67_), .c(new_n65_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(z04));
  nand2  g038(.a(new_n64_), .b(x37), .O(new_n101_));
  nor2   g039(.a(x40), .b(x37), .O(new_n102_));
  oai21  g040(.a(new_n102_), .b(x35), .c(new_n66_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n63_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(x28), .c(x27), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n101_), .O(z05));
  inv1   g044(.a(x28), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n67_), .c(new_n73_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n64_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n66_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n64_), .O(z07));
  inv1   g058(.a(new_n79_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n64_), .O(z08));
  inv1   g060(.a(x11), .O(new_n123_));
  nand4  g061(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n123_), .O(z09));
  nor2   g063(.a(new_n86_), .b(x36), .O(new_n126_));
  nor2   g064(.a(new_n63_), .b(x39), .O(new_n127_));
  nor2   g065(.a(x40), .b(x15), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n127_), .c(x07), .O(new_n129_));
  nand2  g067(.a(new_n79_), .b(x05), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n129_), .c(new_n126_), .O(new_n131_));
  inv1   g069(.a(x06), .O(new_n132_));
  nor2   g070(.a(new_n101_), .b(new_n132_), .O(new_n133_));
  aoi21  g071(.a(new_n131_), .b(new_n68_), .c(new_n133_), .O(new_n134_));
  oai21  g072(.a(x32), .b(x30), .c(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n78_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x05), .c(new_n68_), .O(new_n137_));
  oai21  g075(.a(new_n134_), .b(new_n67_), .c(new_n137_), .O(z10));
  nand3  g076(.a(x35), .b(new_n107_), .c(new_n68_), .O(new_n139_));
  aoi22  g077(.a(new_n139_), .b(x40), .c(new_n64_), .d(new_n67_), .O(new_n140_));
  nor2   g078(.a(new_n140_), .b(x08), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x40), .c(new_n128_), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n86_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n141_), .c(new_n84_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(x09), .c(new_n64_), .O(z11));
  nand3  g085(.a(new_n73_), .b(new_n70_), .c(new_n69_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x27), .c(x10), .O(new_n149_));
  aoi22  g087(.a(new_n75_), .b(x27), .c(x39), .d(new_n68_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n149_), .c(x40), .O(new_n151_));
  oai21  g089(.a(x35), .b(new_n67_), .c(x28), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n73_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n63_), .c(new_n66_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n151_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x28), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n68_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand4  g100(.a(new_n75_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n160_), .c(new_n63_), .O(new_n164_));
  nor2   g102(.a(new_n163_), .b(x15), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n164_), .c(x27), .O(new_n166_));
  nand3  g104(.a(new_n136_), .b(new_n157_), .c(new_n68_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(z13));
  aoi21  g106(.a(x39), .b(new_n68_), .c(new_n63_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(x13), .c(new_n74_), .O(new_n170_));
  nand3  g108(.a(new_n152_), .b(new_n63_), .c(new_n66_), .O(new_n171_));
  nand4  g109(.a(new_n69_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n171_), .c(new_n170_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n73_), .O(new_n174_));
  nand3  g112(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n175_));
  nand4  g113(.a(new_n158_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n176_));
  nor2   g114(.a(x13), .b(x04), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n176_), .c(new_n79_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n175_), .O(new_n179_));
  aoi21  g117(.a(x27), .b(new_n66_), .c(x40), .O(new_n180_));
  aoi21  g118(.a(new_n85_), .b(new_n84_), .c(new_n78_), .O(new_n181_));
  aoi21  g119(.a(new_n181_), .b(new_n177_), .c(new_n63_), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n67_), .c(new_n180_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n179_), .c(new_n174_), .O(z14));
  inv1   g122(.a(x12), .O(new_n185_));
  nor2   g123(.a(new_n124_), .b(new_n185_), .O(z15));
  inv1   g124(.a(x23), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(x22), .c(x01), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n64_), .O(z16));
  inv1   g127(.a(x01), .O(new_n190_));
  inv1   g128(.a(x24), .O(new_n191_));
  nand4  g129(.a(new_n64_), .b(new_n191_), .c(x23), .d(x22), .O(new_n192_));
  nor2   g130(.a(new_n192_), .b(new_n190_), .O(z17));
  nand2  g131(.a(x27), .b(new_n68_), .O(new_n194_));
  nand2  g132(.a(new_n67_), .b(x08), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n194_), .c(new_n69_), .O(new_n196_));
  inv1   g134(.a(x09), .O(new_n197_));
  nand2  g135(.a(new_n84_), .b(new_n197_), .O(new_n198_));
  aoi21  g136(.a(new_n196_), .b(new_n107_), .c(new_n198_), .O(new_n199_));
  oai22  g137(.a(new_n199_), .b(new_n65_), .c(new_n91_), .d(new_n121_), .O(z18));
  nor2   g138(.a(new_n146_), .b(x09), .O(z19));
  oai22  g139(.a(x40), .b(x35), .c(x27), .d(x08), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n66_), .O(new_n203_));
  aoi21  g141(.a(x35), .b(x27), .c(x08), .O(new_n204_));
  aoi21  g142(.a(x39), .b(x29), .c(x35), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n204_), .c(x40), .O(new_n206_));
  nand2  g144(.a(new_n79_), .b(x29), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(x28), .c(new_n67_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n206_), .c(new_n203_), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(new_n84_), .c(new_n197_), .O(new_n210_));
  nand2  g148(.a(new_n210_), .b(new_n64_), .O(z20));
endmodule


