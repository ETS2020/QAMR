// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n108_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_;
  inv1   g000(.a(x18), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n66_), .c(new_n67_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(x18), .d(new_n66_), .O(new_n76_));
  oai22  g014(.a(new_n76_), .b(x04), .c(new_n71_), .d(new_n65_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n64_), .c(x04), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  nand2  g027(.a(x40), .b(x39), .O(new_n90_));
  inv1   g028(.a(new_n90_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n87_), .c(x18), .O(new_n93_));
  inv1   g031(.a(x19), .O(new_n94_));
  nand4  g032(.a(new_n70_), .b(x20), .c(new_n94_), .d(new_n63_), .O(new_n95_));
  nand3  g033(.a(x27), .b(x20), .c(new_n94_), .O(new_n96_));
  nor2   g034(.a(new_n96_), .b(new_n69_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n73_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n93_), .O(z02));
  inv1   g038(.a(x35), .O(new_n101_));
  oai21  g039(.a(new_n101_), .b(new_n81_), .c(x27), .O(new_n102_));
  inv1   g040(.a(x37), .O(new_n103_));
  nand3  g041(.a(new_n64_), .b(new_n103_), .c(new_n68_), .O(new_n104_));
  nand4  g042(.a(new_n104_), .b(new_n102_), .c(new_n64_), .d(x21), .O(z03));
  inv1   g043(.a(x21), .O(new_n106_));
  nand4  g044(.a(new_n104_), .b(new_n102_), .c(new_n64_), .d(new_n106_), .O(z04));
  nand2  g045(.a(x28), .b(x27), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n64_), .c(new_n103_), .O(z05));
  inv1   g047(.a(z05), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n66_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n64_), .O(z07));
  inv1   g059(.a(x39), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(x18), .c(new_n73_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n64_), .O(z09));
  inv1   g063(.a(x04), .O(new_n126_));
  inv1   g064(.a(x07), .O(new_n127_));
  nand4  g065(.a(x40), .b(x39), .c(x18), .d(x05), .O(new_n128_));
  oai21  g066(.a(z08), .b(new_n127_), .c(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n83_), .c(new_n126_), .O(new_n130_));
  nand3  g068(.a(new_n64_), .b(x37), .c(x06), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x27), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor3   g072(.a(new_n134_), .b(new_n122_), .c(new_n63_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x05), .c(new_n126_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(z10));
  nand2  g075(.a(new_n64_), .b(new_n68_), .O(new_n138_));
  nand3  g076(.a(x35), .b(new_n81_), .c(new_n126_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x18), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n138_), .c(x08), .O(new_n141_));
  inv1   g079(.a(new_n82_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x18), .c(new_n73_), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n142_), .c(new_n144_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n141_), .c(new_n78_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(x09), .O(z11));
  nand3  g086(.a(new_n103_), .b(new_n80_), .c(new_n101_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x27), .c(x10), .O(new_n150_));
  aoi21  g088(.a(x39), .b(new_n126_), .c(new_n70_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(x18), .O(new_n152_));
  oai21  g090(.a(new_n70_), .b(x40), .c(new_n152_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n73_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(x18), .d(new_n154_), .O(new_n157_));
  nor2   g095(.a(new_n69_), .b(x40), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x20), .c(new_n94_), .d(new_n63_), .O(new_n159_));
  oai21  g097(.a(new_n157_), .b(x04), .c(new_n159_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(new_n135_), .b(new_n154_), .c(new_n126_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n161_), .O(z13));
  nand2  g101(.a(new_n90_), .b(x18), .O(new_n164_));
  nand2  g102(.a(new_n134_), .b(new_n68_), .O(new_n165_));
  nand4  g103(.a(new_n155_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x18), .c(new_n154_), .d(new_n126_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x40), .O(new_n168_));
  nand2  g106(.a(x20), .b(new_n94_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n69_), .c(new_n73_), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n168_), .c(new_n165_), .d(new_n164_), .O(z14));
  nand4  g109(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n64_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(x22), .c(x01), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n64_), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(x23), .c(x22), .d(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n64_), .O(z17));
  inv1   g117(.a(x29), .O(new_n180_));
  oai22  g118(.a(new_n90_), .b(new_n180_), .c(new_n82_), .d(x27), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(x08), .O(new_n182_));
  nor2   g120(.a(new_n65_), .b(x09), .O(new_n183_));
  nor2   g121(.a(new_n68_), .b(x04), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n142_), .c(x30), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n183_), .c(new_n182_), .O(z18));
  oai21  g124(.a(new_n147_), .b(x09), .c(new_n64_), .O(z19));
  aoi21  g125(.a(x28), .b(new_n68_), .c(new_n101_), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n144_), .O(new_n189_));
  oai21  g127(.a(new_n101_), .b(new_n68_), .c(x18), .O(new_n190_));
  nand2  g128(.a(new_n73_), .b(new_n68_), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n190_), .c(x08), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n189_), .c(new_n78_), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(x09), .O(z20));
endmodule


