// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_;
  inv1   g000(.a(x20), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x27), .c(new_n66_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n64_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n66_), .d(new_n73_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n72_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand3  g018(.a(new_n75_), .b(new_n80_), .c(new_n79_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x04), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n65_), .O(new_n83_));
  aoi21  g021(.a(new_n64_), .b(new_n63_), .c(x36), .O(new_n84_));
  nand4  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n83_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  nand2  g029(.a(x35), .b(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n90_), .c(x04), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n89_), .c(x39), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(new_n95_));
  nand2  g033(.a(new_n64_), .b(x20), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n95_), .O(z02));
  nand2  g035(.a(new_n68_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x27), .O(new_n99_));
  inv1   g037(.a(new_n65_), .O(new_n100_));
  aoi21  g038(.a(new_n67_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n98_), .c(x21), .O(z03));
  inv1   g040(.a(x21), .O(new_n103_));
  nand3  g041(.a(new_n101_), .b(new_n98_), .c(new_n103_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n100_), .O(z05));
  inv1   g044(.a(z05), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n66_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n65_), .O(z07));
  inv1   g056(.a(x39), .O(new_n119_));
  nor2   g057(.a(new_n64_), .b(new_n119_), .O(z08));
  inv1   g058(.a(x11), .O(new_n121_));
  nand4  g059(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n121_), .O(z09));
  inv1   g061(.a(x36), .O(new_n124_));
  nand2  g062(.a(new_n92_), .b(new_n124_), .O(new_n125_));
  inv1   g063(.a(x07), .O(new_n126_));
  nand2  g064(.a(x40), .b(new_n119_), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n96_), .c(new_n126_), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x05), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n128_), .c(new_n125_), .O(new_n131_));
  nand3  g069(.a(new_n65_), .b(x37), .c(x06), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(x04), .c(new_n132_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x27), .O(new_n134_));
  oai21  g072(.a(x32), .b(x30), .c(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n119_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x05), .c(new_n73_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(z10));
  nand2  g076(.a(new_n65_), .b(new_n99_), .O(new_n139_));
  oai21  g077(.a(new_n92_), .b(x04), .c(x40), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n139_), .c(x08), .O(new_n141_));
  inv1   g079(.a(new_n92_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x40), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  aoi22  g083(.a(new_n145_), .b(new_n142_), .c(new_n144_), .d(new_n96_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n141_), .c(new_n79_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(x09), .c(new_n65_), .O(z11));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  nand2  g087(.a(x40), .b(new_n99_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(new_n74_), .O(new_n152_));
  nand2  g090(.a(z08), .b(new_n73_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n68_), .c(new_n67_), .O(new_n154_));
  aoi21  g092(.a(x27), .b(x20), .c(x40), .O(new_n155_));
  oai21  g093(.a(new_n64_), .b(new_n73_), .c(x39), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n99_), .c(new_n155_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n154_), .c(new_n152_), .O(z12));
  inv1   g096(.a(x13), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x28), .c(new_n64_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n159_), .d(new_n73_), .O(new_n162_));
  inv1   g100(.a(x18), .O(new_n163_));
  inv1   g101(.a(x19), .O(new_n164_));
  nand4  g102(.a(new_n69_), .b(x20), .c(new_n164_), .d(new_n163_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x27), .O(new_n167_));
  nand3  g105(.a(new_n136_), .b(new_n159_), .c(new_n73_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z13));
  oai21  g107(.a(x19), .b(x18), .c(new_n160_), .O(new_n170_));
  oai21  g108(.a(new_n64_), .b(x36), .c(x35), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n63_), .O(new_n172_));
  inv1   g110(.a(x35), .O(new_n173_));
  nand2  g111(.a(new_n67_), .b(new_n173_), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n172_), .c(new_n170_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x28), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n150_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n80_), .c(new_n79_), .O(new_n178_));
  nand4  g116(.a(x27), .b(x20), .c(new_n164_), .d(new_n163_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x40), .O(new_n180_));
  aoi22  g118(.a(new_n180_), .b(new_n69_), .c(new_n159_), .d(new_n73_), .O(new_n181_));
  inv1   g119(.a(new_n179_), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n69_), .c(z08), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n181_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n178_), .O(z14));
  inv1   g123(.a(x12), .O(new_n186_));
  nor2   g124(.a(new_n122_), .b(new_n186_), .O(z15));
  inv1   g125(.a(x23), .O(new_n188_));
  nand4  g126(.a(new_n65_), .b(new_n188_), .c(x22), .d(x01), .O(new_n189_));
  inv1   g127(.a(new_n189_), .O(z16));
  inv1   g128(.a(x24), .O(new_n191_));
  nand4  g129(.a(new_n191_), .b(x23), .c(x22), .d(x01), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n65_), .O(z17));
  inv1   g131(.a(z08), .O(new_n194_));
  nand2  g132(.a(x27), .b(new_n73_), .O(new_n195_));
  nand2  g133(.a(new_n99_), .b(x08), .O(new_n196_));
  aoi21  g134(.a(new_n196_), .b(new_n195_), .c(new_n173_), .O(new_n197_));
  inv1   g135(.a(x09), .O(new_n198_));
  nand2  g136(.a(new_n79_), .b(new_n198_), .O(new_n199_));
  aoi21  g137(.a(new_n197_), .b(new_n91_), .c(new_n199_), .O(new_n200_));
  oai22  g138(.a(new_n200_), .b(new_n100_), .c(new_n194_), .d(new_n88_), .O(z18));
  nor2   g139(.a(new_n147_), .b(x09), .O(z19));
  nand2  g140(.a(x28), .b(new_n99_), .O(new_n203_));
  aoi22  g141(.a(new_n203_), .b(x35), .c(new_n144_), .d(new_n96_), .O(new_n204_));
  nand2  g142(.a(x40), .b(new_n173_), .O(new_n205_));
  aoi21  g143(.a(new_n205_), .b(new_n139_), .c(x08), .O(new_n206_));
  oai21  g144(.a(new_n206_), .b(new_n204_), .c(new_n79_), .O(new_n207_));
  nor2   g145(.a(new_n207_), .b(x09), .O(z20));
endmodule


