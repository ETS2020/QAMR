// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_;
  nand2  g000(.a(x40), .b(x32), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x32), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n72_), .c(new_n79_), .O(new_n84_));
  oai21  g022(.a(new_n64_), .b(x04), .c(new_n84_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  inv1   g024(.a(x08), .O(new_n87_));
  inv1   g025(.a(x29), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(new_n89_));
  nand2  g027(.a(new_n81_), .b(x04), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n89_), .c(new_n74_), .O(new_n91_));
  nand3  g029(.a(new_n67_), .b(new_n87_), .c(x04), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x39), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n91_), .c(new_n72_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(z02));
  nand2  g033(.a(new_n69_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n68_), .b(new_n67_), .c(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n96_), .c(new_n64_), .O(z03));
  nand2  g037(.a(new_n68_), .b(new_n67_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n96_), .c(new_n63_), .d(new_n97_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n64_), .O(z05));
  aoi21  g041(.a(x40), .b(x32), .c(x27), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(new_n80_), .c(new_n68_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n63_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n65_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n63_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  oai21  g055(.a(x39), .b(x32), .c(x40), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n63_), .O(z09));
  inv1   g059(.a(x04), .O(new_n122_));
  inv1   g060(.a(x36), .O(new_n123_));
  nand2  g061(.a(new_n118_), .b(x07), .O(new_n124_));
  nand2  g062(.a(x40), .b(x39), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n72_), .c(x05), .O(new_n127_));
  aoi22  g065(.a(new_n127_), .b(new_n124_), .c(new_n81_), .d(new_n123_), .O(new_n128_));
  nand3  g066(.a(new_n63_), .b(x37), .c(x06), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  aoi21  g068(.a(new_n128_), .b(new_n122_), .c(new_n130_), .O(new_n131_));
  nor2   g069(.a(new_n125_), .b(x32), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(x30), .c(x05), .d(new_n122_), .O(new_n133_));
  oai21  g071(.a(new_n131_), .b(new_n67_), .c(new_n133_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  nand2  g073(.a(x40), .b(x04), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x35), .c(new_n80_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n72_), .c(new_n104_), .O(new_n138_));
  nand2  g076(.a(x39), .b(x29), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n72_), .c(new_n74_), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(x35), .c(new_n80_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(new_n143_));
  oai22  g081(.a(new_n143_), .b(new_n140_), .c(new_n138_), .d(x08), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n79_), .c(new_n135_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(z11));
  inv1   g084(.a(x35), .O(new_n147_));
  nand3  g085(.a(new_n68_), .b(new_n123_), .c(new_n147_), .O(new_n148_));
  nand2  g086(.a(x40), .b(new_n67_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n148_), .c(x10), .O(new_n150_));
  nand2  g088(.a(new_n136_), .b(x39), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n67_), .O(new_n152_));
  nand2  g090(.a(x39), .b(new_n122_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n69_), .c(new_n68_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n150_), .c(new_n72_), .O(new_n156_));
  oai21  g094(.a(new_n70_), .b(x40), .c(new_n156_), .O(z12));
  inv1   g095(.a(x13), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x28), .c(new_n74_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x39), .c(new_n158_), .d(new_n122_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  nand2  g101(.a(new_n69_), .b(new_n68_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x20), .c(new_n163_), .d(new_n162_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n161_), .c(x32), .O(new_n166_));
  nand4  g104(.a(new_n164_), .b(new_n74_), .c(x20), .d(new_n163_), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(x18), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(x27), .O(new_n169_));
  nand4  g107(.a(new_n132_), .b(x30), .c(new_n158_), .d(new_n122_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(z13));
  nand3  g109(.a(x20), .b(new_n163_), .c(new_n162_), .O(new_n172_));
  nand3  g110(.a(new_n159_), .b(new_n79_), .c(x28), .O(new_n173_));
  nor2   g111(.a(x13), .b(x04), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n173_), .c(new_n126_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n172_), .O(new_n176_));
  nand3  g114(.a(new_n68_), .b(new_n147_), .c(x28), .O(new_n177_));
  oai21  g115(.a(x32), .b(x27), .c(new_n177_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n79_), .O(new_n179_));
  nand2  g117(.a(new_n153_), .b(new_n72_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(x40), .c(new_n158_), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n69_), .c(new_n68_), .O(new_n182_));
  oai21  g120(.a(new_n174_), .b(new_n74_), .c(x39), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n72_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x40), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n67_), .c(new_n64_), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n182_), .c(new_n179_), .d(new_n176_), .O(z14));
  nand4  g125(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n63_), .O(z15));
  inv1   g127(.a(x23), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x22), .c(x01), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n63_), .O(z16));
  inv1   g130(.a(x24), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x23), .c(x22), .d(x01), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n63_), .O(z17));
  oai22  g133(.a(new_n125_), .b(new_n88_), .c(new_n81_), .d(x27), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(x08), .O(new_n197_));
  nor2   g135(.a(new_n67_), .b(x04), .O(new_n198_));
  aoi21  g136(.a(new_n198_), .b(new_n82_), .c(x09), .O(new_n199_));
  nand4  g137(.a(new_n199_), .b(new_n197_), .c(new_n63_), .d(new_n79_), .O(z18));
  nand2  g138(.a(new_n145_), .b(new_n63_), .O(z19));
  aoi21  g139(.a(x28), .b(new_n67_), .c(new_n147_), .O(new_n202_));
  nor2   g140(.a(x35), .b(x32), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n104_), .c(new_n87_), .O(new_n204_));
  oai21  g142(.a(new_n202_), .b(new_n140_), .c(new_n204_), .O(new_n205_));
  nand3  g143(.a(new_n205_), .b(new_n79_), .c(new_n135_), .O(new_n206_));
  inv1   g144(.a(new_n206_), .O(z20));
endmodule


