// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x36), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(x27), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  aoi21  g012(.a(x35), .b(x28), .c(x37), .O(new_n75_));
  nor3   g013(.a(new_n75_), .b(x40), .c(new_n67_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n74_), .c(new_n63_), .O(new_n77_));
  nand2  g015(.a(x40), .b(new_n64_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(x16), .c(new_n77_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x28), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x36), .c(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n82_), .c(new_n81_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n84_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x04), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x36), .O(new_n94_));
  inv1   g032(.a(x40), .O(new_n95_));
  inv1   g033(.a(x28), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x27), .O(new_n97_));
  nand2  g035(.a(new_n70_), .b(x28), .O(new_n98_));
  oai21  g036(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n94_), .O(z02));
  nand2  g038(.a(new_n71_), .b(x27), .O(new_n101_));
  aoi21  g039(.a(new_n70_), .b(new_n67_), .c(new_n79_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n101_), .c(x21), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  nand3  g042(.a(new_n102_), .b(new_n101_), .c(new_n104_), .O(z04));
  nor2   g043(.a(new_n96_), .b(new_n67_), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(x37), .c(new_n78_), .O(z06));
  inv1   g045(.a(z06), .O(z05));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n78_), .c(x03), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z07));
  nand3  g057(.a(x40), .b(x39), .c(x36), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  nand3  g062(.a(new_n78_), .b(x37), .c(x06), .O(new_n125_));
  inv1   g063(.a(x07), .O(new_n126_));
  nand2  g064(.a(x40), .b(x39), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x36), .O(new_n128_));
  nand3  g066(.a(new_n95_), .b(x35), .c(new_n96_), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n128_), .c(new_n126_), .O(new_n130_));
  nand4  g068(.a(x40), .b(x39), .c(x36), .d(x05), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n130_), .c(new_n65_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n125_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x27), .O(new_n135_));
  inv1   g073(.a(x39), .O(new_n136_));
  nand2  g074(.a(new_n82_), .b(new_n81_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x40), .O(new_n138_));
  nor2   g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand4  g077(.a(new_n139_), .b(x36), .c(x05), .d(new_n65_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n135_), .O(z10));
  nand2  g079(.a(new_n78_), .b(new_n67_), .O(new_n142_));
  nand3  g080(.a(x35), .b(new_n96_), .c(new_n65_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x36), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n142_), .c(x08), .O(new_n145_));
  nand2  g083(.a(x39), .b(x29), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x36), .c(new_n95_), .O(new_n147_));
  nand2  g085(.a(x27), .b(x04), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n84_), .c(new_n147_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n145_), .c(new_n81_), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(x09), .O(z11));
  inv1   g089(.a(new_n73_), .O(new_n152_));
  nand2  g090(.a(x39), .b(new_n65_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x36), .c(new_n95_), .O(new_n154_));
  nand3  g092(.a(x36), .b(new_n67_), .c(new_n66_), .O(new_n155_));
  oai21  g093(.a(new_n154_), .b(new_n152_), .c(new_n155_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  aoi21  g095(.a(new_n83_), .b(x28), .c(new_n95_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n72_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n159_), .c(new_n67_), .O(new_n163_));
  nand4  g101(.a(new_n137_), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(x36), .c(new_n95_), .O(new_n165_));
  or2    g103(.a(new_n165_), .b(new_n163_), .O(z13));
  nand3  g104(.a(x39), .b(new_n157_), .c(new_n65_), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(new_n168_));
  nand4  g106(.a(x27), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n72_), .c(new_n168_), .O(new_n171_));
  nand4  g109(.a(x37), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n83_), .c(x28), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(x27), .c(x32), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n81_), .c(new_n171_), .O(new_n175_));
  oai21  g113(.a(new_n169_), .b(new_n75_), .c(new_n95_), .O(new_n176_));
  oai21  g114(.a(new_n175_), .b(new_n64_), .c(new_n176_), .O(z14));
  inv1   g115(.a(x12), .O(new_n178_));
  nor2   g116(.a(new_n123_), .b(new_n178_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand4  g118(.a(new_n78_), .b(new_n180_), .c(x22), .d(x01), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z16));
  inv1   g120(.a(x24), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(x23), .c(x22), .d(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n78_), .O(z17));
  nand2  g123(.a(x27), .b(new_n65_), .O(new_n186_));
  nand2  g124(.a(new_n67_), .b(x08), .O(new_n187_));
  aoi21  g125(.a(new_n187_), .b(new_n186_), .c(new_n83_), .O(new_n188_));
  inv1   g126(.a(x09), .O(new_n189_));
  nand2  g127(.a(new_n81_), .b(new_n189_), .O(new_n190_));
  aoi21  g128(.a(new_n188_), .b(new_n96_), .c(new_n190_), .O(new_n191_));
  nand3  g129(.a(x40), .b(x39), .c(x36), .O(new_n192_));
  oai22  g130(.a(new_n192_), .b(new_n89_), .c(new_n191_), .d(new_n79_), .O(z18));
  oai21  g131(.a(new_n150_), .b(x09), .c(new_n78_), .O(z19));
  aoi21  g132(.a(x28), .b(new_n67_), .c(new_n83_), .O(new_n195_));
  nor2   g133(.a(new_n195_), .b(new_n147_), .O(new_n196_));
  nand2  g134(.a(x36), .b(new_n83_), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n142_), .c(x08), .O(new_n198_));
  oai21  g136(.a(new_n198_), .b(new_n196_), .c(new_n81_), .O(new_n199_));
  nor2   g137(.a(new_n199_), .b(x09), .O(z20));
endmodule


