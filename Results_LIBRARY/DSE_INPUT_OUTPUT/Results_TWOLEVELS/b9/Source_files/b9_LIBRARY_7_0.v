// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x40), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  inv1   g005(.a(x15), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  inv1   g007(.a(x35), .O(new_n70_));
  inv1   g008(.a(x36), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand4  g010(.a(new_n72_), .b(x40), .c(x39), .d(new_n69_), .O(new_n73_));
  aoi21  g011(.a(x35), .b(x28), .c(x37), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n64_), .O(new_n75_));
  nand4  g013(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n75_), .c(new_n68_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n67_), .O(z00));
  oai21  g017(.a(x32), .b(x30), .c(x40), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n64_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand4  g023(.a(new_n85_), .b(new_n71_), .c(new_n83_), .d(new_n82_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(x04), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  aoi21  g026(.a(new_n85_), .b(x04), .c(new_n88_), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  nand3  g028(.a(new_n64_), .b(new_n90_), .c(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n89_), .c(x40), .d(x39), .O(z02));
  inv1   g030(.a(x37), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(x28), .c(x35), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x27), .O(new_n95_));
  nand2  g033(.a(x40), .b(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n64_), .O(new_n97_));
  nand2  g035(.a(new_n93_), .b(new_n84_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n97_), .c(new_n95_), .d(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand4  g038(.a(new_n98_), .b(new_n97_), .c(new_n95_), .d(new_n100_), .O(z04));
  oai21  g039(.a(x37), .b(x28), .c(x27), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n96_), .O(z05));
  nand2  g041(.a(new_n98_), .b(new_n97_), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n68_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n107_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  oai21  g051(.a(new_n109_), .b(x31), .c(new_n113_), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n66_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  nand2  g054(.a(x40), .b(x39), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n66_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n66_), .O(z09));
  nand2  g058(.a(new_n85_), .b(new_n71_), .O(new_n121_));
  nand2  g059(.a(new_n117_), .b(x07), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x05), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n121_), .c(x27), .O(new_n125_));
  inv1   g063(.a(x39), .O(new_n126_));
  nor2   g064(.a(new_n80_), .b(new_n126_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n69_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n90_), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(x30), .c(x40), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n64_), .O(new_n136_));
  nand2  g074(.a(x39), .b(x29), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  nand2  g076(.a(x40), .b(new_n70_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n84_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g079(.a(x40), .b(x08), .O(new_n142_));
  nand2  g080(.a(new_n70_), .b(x27), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n84_), .c(new_n69_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n141_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n82_), .c(new_n133_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n136_), .O(z11));
  nor2   g086(.a(x37), .b(x36), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n70_), .c(new_n64_), .O(new_n150_));
  oai22  g088(.a(new_n117_), .b(x04), .c(new_n74_), .d(new_n64_), .O(new_n151_));
  oai21  g089(.a(new_n150_), .b(x10), .c(new_n151_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n65_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n69_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  inv1   g096(.a(x20), .O(new_n159_));
  nor2   g097(.a(new_n74_), .b(new_n159_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n158_), .c(new_n157_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n156_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nand3  g101(.a(new_n127_), .b(new_n153_), .c(new_n69_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(z13));
  nand3  g103(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n166_));
  nand4  g104(.a(new_n154_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n167_));
  nor2   g105(.a(x13), .b(x04), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n167_), .c(x40), .d(x39), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n166_), .O(new_n170_));
  nand3  g108(.a(new_n93_), .b(new_n70_), .c(x28), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x27), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n83_), .c(new_n82_), .O(new_n173_));
  inv1   g111(.a(new_n168_), .O(new_n174_));
  oai22  g112(.a(new_n174_), .b(new_n117_), .c(new_n74_), .d(new_n64_), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n173_), .c(new_n170_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(x22), .c(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n66_), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(x23), .c(x22), .d(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n66_), .O(z17));
  inv1   g122(.a(x29), .O(new_n185_));
  oai22  g123(.a(new_n85_), .b(x27), .c(new_n117_), .d(new_n185_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(x08), .O(new_n187_));
  inv1   g125(.a(new_n85_), .O(new_n188_));
  nor2   g126(.a(new_n64_), .b(x04), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n188_), .c(x09), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n187_), .c(new_n66_), .d(new_n82_), .O(z18));
  nor2   g129(.a(x35), .b(new_n64_), .O(new_n192_));
  nor2   g130(.a(new_n65_), .b(x27), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n192_), .c(new_n90_), .O(new_n194_));
  oai21  g132(.a(new_n84_), .b(x27), .c(x35), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n137_), .c(x40), .O(new_n196_));
  nand3  g134(.a(new_n65_), .b(new_n70_), .c(x27), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n196_), .c(new_n194_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n82_), .c(new_n133_), .O(new_n199_));
  inv1   g137(.a(new_n199_), .O(z20));
  nand2  g138(.a(new_n147_), .b(new_n136_), .O(z19));
endmodule


