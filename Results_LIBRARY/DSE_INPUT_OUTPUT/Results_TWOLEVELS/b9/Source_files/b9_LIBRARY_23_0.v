// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_;
  nor2   g000(.a(x40), .b(x24), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(x15), .c(x16), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n70_), .O(z00));
  inv1   g016(.a(x27), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(x28), .b(x27), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x35), .c(x36), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n79_), .c(new_n80_), .O(new_n83_));
  nor2   g021(.a(new_n63_), .b(new_n71_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(x30), .c(new_n84_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  nand2  g028(.a(x35), .b(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n89_), .c(x04), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n88_), .c(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x40), .O(new_n94_));
  nand2  g032(.a(new_n74_), .b(x24), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(z02));
  nand2  g034(.a(new_n66_), .b(x27), .O(new_n97_));
  nand2  g035(.a(new_n65_), .b(new_n79_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n97_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g037(.a(new_n65_), .b(new_n79_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n97_), .c(new_n63_), .O(z04));
  aoi21  g039(.a(new_n81_), .b(new_n65_), .c(new_n63_), .O(z05));
  oai21  g040(.a(x40), .b(x24), .c(new_n79_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x28), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n65_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n64_), .O(z06));
  inv1   g044(.a(x31), .O(new_n107_));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n72_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand4  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .d(new_n107_), .O(new_n112_));
  inv1   g050(.a(x14), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n110_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n112_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(z07));
  nand2  g057(.a(x40), .b(x39), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n64_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  inv1   g062(.a(x36), .O(new_n125_));
  nand2  g063(.a(new_n91_), .b(new_n125_), .O(new_n126_));
  nand2  g064(.a(new_n120_), .b(x07), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n126_), .c(x27), .O(new_n130_));
  inv1   g068(.a(x30), .O(new_n131_));
  nand2  g069(.a(new_n80_), .b(new_n131_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(x40), .c(x39), .d(x05), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n71_), .O(new_n135_));
  nor2   g073(.a(new_n65_), .b(new_n79_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x06), .c(new_n63_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n135_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  nor2   g077(.a(x40), .b(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n89_), .c(x24), .O(new_n141_));
  oai21  g079(.a(new_n79_), .b(new_n71_), .c(new_n90_), .O(new_n142_));
  nand3  g080(.a(x40), .b(x39), .c(x29), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  aoi21  g082(.a(x35), .b(x27), .c(x08), .O(new_n145_));
  aoi21  g083(.a(x39), .b(x29), .c(x35), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(x40), .O(new_n147_));
  inv1   g085(.a(x08), .O(new_n148_));
  oai21  g086(.a(x28), .b(x04), .c(new_n148_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(new_n147_), .c(new_n144_), .d(new_n141_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n131_), .c(new_n139_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n64_), .O(z11));
  nor3   g090(.a(x37), .b(x36), .c(x35), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n79_), .c(new_n73_), .O(new_n154_));
  oai21  g092(.a(new_n120_), .b(x04), .c(new_n68_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n154_), .c(new_n64_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x28), .c(new_n74_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n71_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand4  g100(.a(new_n67_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x27), .O(new_n165_));
  nand3  g103(.a(new_n132_), .b(x40), .c(x39), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x13), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n71_), .c(new_n63_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(z13));
  nand3  g107(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n170_));
  nand4  g108(.a(new_n158_), .b(new_n80_), .c(new_n131_), .d(x28), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  nand4  g110(.a(x40), .b(x39), .c(new_n157_), .d(new_n71_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n170_), .O(new_n174_));
  inv1   g112(.a(x35), .O(new_n175_));
  nand3  g113(.a(new_n65_), .b(new_n175_), .c(x28), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n80_), .c(new_n131_), .O(new_n178_));
  aoi21  g116(.a(new_n173_), .b(new_n68_), .c(new_n63_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n178_), .c(new_n174_), .O(z14));
  nand4  g118(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n64_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(x22), .c(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n64_), .O(z16));
  nand3  g123(.a(x23), .b(x22), .c(x01), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(x40), .c(x24), .O(z17));
  nand2  g125(.a(x27), .b(new_n71_), .O(new_n188_));
  nand2  g126(.a(new_n79_), .b(x08), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n188_), .c(new_n175_), .O(new_n190_));
  nand2  g128(.a(new_n131_), .b(new_n139_), .O(new_n191_));
  aoi21  g129(.a(new_n190_), .b(new_n90_), .c(new_n191_), .O(new_n192_));
  oai22  g130(.a(new_n192_), .b(new_n63_), .c(new_n120_), .d(new_n87_), .O(z18));
  nand2  g131(.a(x28), .b(new_n79_), .O(new_n194_));
  nand2  g132(.a(x39), .b(x29), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(x40), .O(new_n196_));
  aoi22  g134(.a(new_n196_), .b(new_n95_), .c(new_n194_), .d(x35), .O(new_n197_));
  nand2  g135(.a(x40), .b(new_n175_), .O(new_n198_));
  aoi21  g136(.a(new_n198_), .b(new_n103_), .c(x08), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n197_), .c(new_n131_), .O(new_n200_));
  nor2   g138(.a(new_n200_), .b(x09), .O(z20));
  nand2  g139(.a(new_n151_), .b(new_n64_), .O(z19));
endmodule


