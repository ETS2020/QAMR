// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_;
  inv1   g000(.a(x07), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n67_), .c(new_n68_), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n75_), .c(new_n64_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n67_), .d(new_n74_), .O(new_n78_));
  oai21  g016(.a(new_n73_), .b(new_n66_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n65_), .c(x04), .O(z01));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  inv1   g027(.a(new_n89_), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(x02), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n84_), .c(x04), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x39), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n91_), .c(x40), .O(new_n95_));
  nand2  g033(.a(new_n64_), .b(x07), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n95_), .O(z02));
  nand2  g035(.a(new_n71_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n70_), .b(new_n69_), .c(new_n99_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n98_), .c(new_n66_), .O(z03));
  aoi21  g039(.a(new_n70_), .b(new_n69_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n98_), .c(new_n66_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n65_), .c(new_n70_), .O(z05));
  oai22  g043(.a(x40), .b(x07), .c(new_n83_), .d(new_n69_), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x37), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n67_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n65_), .O(z07));
  inv1   g056(.a(x39), .O(new_n119_));
  oai21  g057(.a(new_n64_), .b(new_n119_), .c(new_n65_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n65_), .O(z09));
  nand2  g060(.a(x39), .b(x05), .O(new_n123_));
  oai21  g061(.a(x39), .b(new_n63_), .c(new_n123_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n85_), .O(new_n125_));
  nand3  g063(.a(x35), .b(new_n83_), .c(x07), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n82_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n64_), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n125_), .c(new_n69_), .O(new_n129_));
  inv1   g067(.a(x05), .O(new_n130_));
  nor2   g068(.a(x32), .b(x30), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n64_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x39), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(new_n129_), .c(new_n74_), .O(new_n135_));
  nor2   g073(.a(new_n70_), .b(new_n69_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x06), .c(new_n66_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n135_), .O(z10));
  nand2  g076(.a(new_n65_), .b(new_n69_), .O(new_n139_));
  oai21  g077(.a(new_n84_), .b(x04), .c(x40), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n139_), .c(x08), .O(new_n141_));
  inv1   g079(.a(new_n84_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x40), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  aoi22  g083(.a(new_n145_), .b(new_n142_), .c(new_n144_), .d(new_n96_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n141_), .c(new_n80_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(x09), .O(z11));
  inv1   g086(.a(x35), .O(new_n149_));
  nand3  g087(.a(new_n70_), .b(new_n82_), .c(new_n149_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x27), .c(x10), .O(new_n151_));
  aoi21  g089(.a(x39), .b(new_n74_), .c(new_n72_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n151_), .c(x40), .O(new_n153_));
  nor2   g091(.a(new_n72_), .b(x40), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x07), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x28), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n74_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand2  g100(.a(new_n71_), .b(new_n70_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n160_), .c(new_n64_), .O(new_n165_));
  nor2   g103(.a(new_n164_), .b(new_n63_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n165_), .c(x27), .O(new_n167_));
  nand4  g105(.a(new_n132_), .b(x39), .c(new_n157_), .d(new_n74_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z13));
  oai21  g107(.a(new_n119_), .b(x04), .c(x40), .O(new_n170_));
  aoi21  g108(.a(new_n64_), .b(x07), .c(x13), .O(new_n171_));
  aoi22  g109(.a(new_n171_), .b(new_n170_), .c(x35), .d(x28), .O(new_n172_));
  nand4  g110(.a(new_n149_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n172_), .c(new_n70_), .O(new_n175_));
  nand3  g113(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n176_));
  nand4  g114(.a(new_n158_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(new_n178_));
  nand4  g116(.a(x40), .b(x39), .c(new_n157_), .d(new_n74_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n178_), .c(new_n176_), .O(new_n180_));
  aoi21  g118(.a(x27), .b(x07), .c(x40), .O(new_n181_));
  nand3  g119(.a(x39), .b(new_n157_), .c(new_n74_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x40), .O(new_n183_));
  oai21  g121(.a(x32), .b(x30), .c(new_n183_), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n69_), .c(new_n181_), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n180_), .c(new_n175_), .O(z14));
  nand4  g124(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n65_), .O(z15));
  inv1   g126(.a(x23), .O(new_n189_));
  nand3  g127(.a(new_n189_), .b(x22), .c(x01), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n65_), .O(z16));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(x23), .c(x22), .d(x01), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n65_), .O(z17));
  inv1   g132(.a(x08), .O(new_n195_));
  nand2  g133(.a(new_n64_), .b(new_n63_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(x27), .c(new_n74_), .O(new_n197_));
  oai21  g135(.a(new_n139_), .b(new_n195_), .c(new_n197_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(x35), .c(new_n83_), .O(new_n199_));
  nor2   g137(.a(new_n64_), .b(new_n119_), .O(new_n200_));
  inv1   g138(.a(x09), .O(new_n201_));
  nand2  g139(.a(new_n80_), .b(new_n201_), .O(new_n202_));
  aoi22  g140(.a(new_n202_), .b(new_n65_), .c(new_n200_), .d(new_n90_), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n199_), .O(z18));
  nand2  g142(.a(x28), .b(new_n69_), .O(new_n205_));
  aoi22  g143(.a(new_n205_), .b(x35), .c(new_n144_), .d(new_n96_), .O(new_n206_));
  nand2  g144(.a(x40), .b(new_n149_), .O(new_n207_));
  aoi21  g145(.a(new_n207_), .b(new_n139_), .c(x08), .O(new_n208_));
  oai21  g146(.a(new_n208_), .b(new_n206_), .c(new_n80_), .O(new_n209_));
  nor2   g147(.a(new_n209_), .b(x09), .O(z20));
  nor2   g148(.a(new_n147_), .b(x09), .O(z19));
endmodule


