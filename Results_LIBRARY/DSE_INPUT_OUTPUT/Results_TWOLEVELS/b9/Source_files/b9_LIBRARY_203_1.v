// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_;
  oai21  g000(.a(x36), .b(x35), .c(x40), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  nand2  g003(.a(x40), .b(x10), .O(new_n66_));
  nor2   g004(.a(new_n66_), .b(x04), .O(new_n67_));
  aoi21  g005(.a(new_n65_), .b(x27), .c(new_n67_), .O(new_n68_));
  inv1   g006(.a(x39), .O(new_n69_));
  nand2  g007(.a(x40), .b(new_n69_), .O(new_n70_));
  and2   g008(.a(new_n70_), .b(x16), .O(new_n71_));
  oai21  g009(.a(new_n68_), .b(x15), .c(new_n71_), .O(z00));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n70_), .c(x04), .O(z01));
  inv1   g018(.a(x02), .O(new_n81_));
  inv1   g019(.a(x08), .O(new_n82_));
  inv1   g020(.a(x29), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n69_), .O(new_n87_));
  inv1   g025(.a(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n87_), .c(new_n84_), .d(x40), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand3  g031(.a(new_n70_), .b(new_n93_), .c(new_n88_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n70_), .d(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand4  g034(.a(new_n94_), .b(new_n92_), .c(new_n70_), .d(new_n96_), .O(z04));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n70_), .c(new_n93_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x31), .O(new_n101_));
  inv1   g039(.a(x33), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand4  g044(.a(new_n106_), .b(new_n104_), .c(new_n102_), .d(new_n101_), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n105_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n107_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n70_), .c(x03), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z07));
  inv1   g052(.a(x40), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n69_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n70_), .O(z09));
  inv1   g056(.a(x04), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  nand2  g058(.a(x40), .b(x05), .O(new_n121_));
  oai21  g059(.a(x40), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n77_), .c(x27), .O(new_n123_));
  nor2   g061(.a(x32), .b(x30), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(x40), .c(x05), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n119_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n70_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  nand2  g069(.a(x40), .b(x29), .O(new_n132_));
  nand2  g070(.a(x27), .b(x04), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x35), .c(new_n85_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand4  g073(.a(x35), .b(new_n85_), .c(x27), .d(new_n119_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(x08), .c(new_n135_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n73_), .c(new_n131_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n70_), .O(z11));
  nand3  g078(.a(new_n93_), .b(new_n76_), .c(new_n75_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x27), .c(x10), .O(new_n142_));
  nand2  g080(.a(new_n91_), .b(new_n93_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(new_n119_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n142_), .c(x39), .O(new_n145_));
  oai21  g083(.a(new_n64_), .b(new_n88_), .c(new_n115_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x28), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x40), .c(new_n148_), .d(new_n119_), .O(new_n151_));
  inv1   g089(.a(x18), .O(new_n152_));
  inv1   g090(.a(x19), .O(new_n153_));
  nand4  g091(.a(new_n143_), .b(x20), .c(new_n153_), .d(new_n152_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x27), .O(new_n156_));
  nor3   g094(.a(new_n124_), .b(x13), .c(x04), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n69_), .c(x40), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n156_), .O(z13));
  oai21  g097(.a(x27), .b(new_n148_), .c(x39), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x40), .O(new_n161_));
  nand2  g099(.a(x39), .b(x04), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x40), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n124_), .c(new_n88_), .O(new_n164_));
  nand3  g102(.a(new_n162_), .b(x40), .c(new_n148_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n91_), .O(new_n166_));
  nand4  g104(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(x28), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n93_), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n153_), .c(new_n152_), .O(new_n170_));
  nand4  g108(.a(new_n149_), .b(new_n74_), .c(new_n73_), .d(x28), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(x40), .c(new_n148_), .d(new_n119_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n170_), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n169_), .c(new_n164_), .d(new_n161_), .O(z14));
  nand4  g112(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n70_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand4  g115(.a(new_n70_), .b(new_n177_), .c(x22), .d(x01), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z16));
  inv1   g117(.a(x01), .O(new_n180_));
  inv1   g118(.a(x24), .O(new_n181_));
  nand4  g119(.a(new_n70_), .b(new_n181_), .c(x23), .d(x22), .O(new_n182_));
  nor2   g120(.a(new_n182_), .b(new_n180_), .O(z17));
  oai21  g121(.a(new_n86_), .b(x27), .c(new_n132_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x08), .O(new_n185_));
  nor2   g123(.a(new_n137_), .b(x09), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n185_), .c(new_n70_), .d(new_n73_), .O(z18));
  inv1   g125(.a(new_n132_), .O(new_n188_));
  oai21  g126(.a(new_n75_), .b(new_n88_), .c(new_n82_), .O(new_n189_));
  aoi21  g127(.a(x28), .b(new_n88_), .c(new_n75_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n188_), .c(new_n189_), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(new_n73_), .c(new_n131_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n70_), .O(z20));
  nand2  g131(.a(new_n139_), .b(new_n70_), .O(z19));
endmodule


