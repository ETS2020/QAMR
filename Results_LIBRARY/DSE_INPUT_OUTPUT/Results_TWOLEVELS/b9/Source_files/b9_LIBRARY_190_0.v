// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  nand2  g002(.a(x35), .b(x28), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g004(.a(new_n66_), .O(new_n67_));
  oai21  g005(.a(new_n63_), .b(x04), .c(new_n67_), .O(new_n68_));
  inv1   g006(.a(x10), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x04), .O(new_n70_));
  and2   g008(.a(x40), .b(x39), .O(z08));
  aoi22  g009(.a(z08), .b(new_n70_), .c(new_n68_), .d(x27), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  nor2   g011(.a(x40), .b(x37), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  oai21  g013(.a(new_n72_), .b(x15), .c(new_n75_), .O(z00));
  inv1   g014(.a(x40), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n64_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g026(.a(x27), .b(x08), .c(new_n82_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x04), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n88_), .c(x39), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x40), .O(new_n92_));
  nor2   g030(.a(x40), .b(new_n64_), .O(new_n93_));
  inv1   g031(.a(new_n93_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n92_), .O(z02));
  nand2  g033(.a(new_n65_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x27), .O(new_n97_));
  oai21  g035(.a(new_n77_), .b(new_n97_), .c(new_n64_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n96_), .c(new_n100_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(x40), .c(new_n64_), .O(z05));
  nand3  g041(.a(new_n102_), .b(x40), .c(new_n64_), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  inv1   g044(.a(x15), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n107_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n78_), .O(z07));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  nor2   g058(.a(new_n82_), .b(x36), .O(new_n121_));
  nor2   g059(.a(new_n77_), .b(x39), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(new_n93_), .c(x07), .O(new_n123_));
  nand2  g061(.a(z08), .b(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n121_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  nand2  g064(.a(new_n80_), .b(new_n79_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(x40), .c(x39), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  aoi21  g067(.a(new_n125_), .b(x27), .c(new_n129_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  oai21  g069(.a(new_n130_), .b(x04), .c(new_n131_), .O(z10));
  inv1   g070(.a(x04), .O(new_n133_));
  inv1   g071(.a(x28), .O(new_n134_));
  nand3  g072(.a(x35), .b(new_n134_), .c(new_n133_), .O(new_n135_));
  aoi22  g073(.a(new_n135_), .b(x40), .c(new_n78_), .d(new_n97_), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(x08), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x40), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi22  g078(.a(new_n140_), .b(new_n82_), .c(new_n139_), .d(new_n94_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n137_), .c(new_n79_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(x09), .O(z11));
  nor2   g081(.a(new_n77_), .b(x27), .O(new_n144_));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n69_), .O(new_n146_));
  nand2  g084(.a(x39), .b(new_n133_), .O(new_n147_));
  oai21  g085(.a(new_n67_), .b(new_n97_), .c(new_n147_), .O(new_n148_));
  oai21  g086(.a(new_n64_), .b(new_n97_), .c(new_n77_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n146_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n77_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n133_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand4  g094(.a(new_n66_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nor2   g097(.a(new_n128_), .b(x13), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n133_), .c(new_n74_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n159_), .O(z13));
  nand3  g100(.a(x39), .b(new_n151_), .c(new_n133_), .O(new_n163_));
  nand3  g101(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n67_), .c(new_n163_), .O(new_n165_));
  nand4  g103(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x40), .O(new_n167_));
  nand4  g105(.a(x37), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  inv1   g107(.a(x36), .O(new_n170_));
  nand3  g108(.a(new_n164_), .b(new_n170_), .c(x28), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x27), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n80_), .c(new_n79_), .O(new_n173_));
  nand2  g111(.a(x40), .b(new_n151_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n147_), .c(new_n97_), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(new_n173_), .c(new_n169_), .d(new_n165_), .O(z14));
  inv1   g114(.a(x12), .O(new_n177_));
  nor2   g115(.a(new_n119_), .b(new_n177_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand4  g117(.a(new_n78_), .b(new_n179_), .c(x22), .d(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z16));
  inv1   g119(.a(x01), .O(new_n182_));
  inv1   g120(.a(x24), .O(new_n183_));
  nand4  g121(.a(new_n78_), .b(new_n183_), .c(x23), .d(x22), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n182_), .O(z17));
  inv1   g123(.a(z08), .O(new_n186_));
  nand2  g124(.a(x27), .b(new_n133_), .O(new_n187_));
  nand2  g125(.a(new_n97_), .b(x08), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n187_), .c(new_n81_), .O(new_n189_));
  inv1   g127(.a(x09), .O(new_n190_));
  nand2  g128(.a(new_n79_), .b(new_n190_), .O(new_n191_));
  aoi21  g129(.a(new_n189_), .b(new_n134_), .c(new_n191_), .O(new_n192_));
  oai22  g130(.a(new_n192_), .b(new_n74_), .c(new_n87_), .d(new_n186_), .O(z18));
  oai21  g131(.a(new_n142_), .b(x09), .c(new_n78_), .O(z19));
  oai22  g132(.a(new_n77_), .b(x35), .c(new_n134_), .d(x27), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n138_), .O(new_n196_));
  nor2   g134(.a(new_n77_), .b(x08), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n93_), .c(new_n81_), .O(new_n198_));
  nor2   g136(.a(new_n74_), .b(x08), .O(new_n199_));
  nor2   g137(.a(x40), .b(new_n134_), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n199_), .c(new_n97_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n198_), .c(new_n196_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n79_), .c(new_n190_), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n78_), .O(z20));
endmodule


