// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x40), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x39), .c(new_n65_), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  nand3  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand3  g009(.a(new_n71_), .b(x27), .c(new_n63_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x10), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(new_n72_), .c(x16), .O(z00));
  inv1   g012(.a(x30), .O(new_n75_));
  inv1   g013(.a(x32), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(x36), .c(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n73_), .c(x04), .O(z01));
  inv1   g020(.a(x10), .O(new_n83_));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g024(.a(new_n78_), .b(x04), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n86_), .c(new_n66_), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  inv1   g027(.a(x27), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n88_), .c(new_n83_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x40), .O(z02));
  nand2  g032(.a(new_n70_), .b(x27), .O(new_n95_));
  nand3  g033(.a(new_n73_), .b(new_n64_), .c(new_n90_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n95_), .c(new_n73_), .d(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand4  g036(.a(new_n96_), .b(new_n95_), .c(new_n73_), .d(new_n98_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n73_), .c(new_n64_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n63_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n105_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  oai21  g049(.a(new_n107_), .b(x31), .c(new_n111_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n73_), .O(z07));
  oai21  g052(.a(x39), .b(x10), .c(x40), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n73_), .O(z09));
  aoi21  g056(.a(x35), .b(new_n77_), .c(x36), .O(new_n119_));
  nand2  g057(.a(new_n115_), .b(x07), .O(new_n120_));
  nand4  g058(.a(x40), .b(x39), .c(new_n83_), .d(x05), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nand3  g060(.a(new_n73_), .b(x37), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n65_), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(x39), .O(new_n126_));
  nand2  g064(.a(new_n76_), .b(new_n75_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x40), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(new_n83_), .c(x05), .d(new_n65_), .O(new_n130_));
  oai21  g068(.a(new_n125_), .b(new_n90_), .c(new_n130_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  aoi21  g070(.a(x40), .b(x10), .c(x27), .O(new_n133_));
  nand3  g071(.a(x35), .b(new_n77_), .c(new_n65_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n83_), .c(new_n133_), .O(new_n135_));
  aoi21  g073(.a(x39), .b(x29), .c(x10), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n77_), .O(new_n138_));
  oai21  g076(.a(new_n136_), .b(new_n66_), .c(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n135_), .b(x08), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n75_), .c(new_n132_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z11));
  inv1   g080(.a(new_n133_), .O(new_n143_));
  oai21  g081(.a(new_n126_), .b(x04), .c(new_n83_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x40), .O(new_n145_));
  aoi22  g083(.a(new_n145_), .b(new_n70_), .c(new_n67_), .d(new_n83_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(x37), .c(new_n143_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x28), .c(new_n66_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x39), .c(new_n148_), .d(new_n65_), .O(new_n151_));
  inv1   g089(.a(x18), .O(new_n152_));
  inv1   g090(.a(x19), .O(new_n153_));
  nand2  g091(.a(new_n70_), .b(new_n64_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x20), .c(new_n153_), .d(new_n152_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand4  g095(.a(new_n127_), .b(x39), .c(new_n148_), .d(new_n65_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n83_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x40), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n157_), .O(z13));
  nand3  g099(.a(x20), .b(new_n153_), .c(new_n152_), .O(new_n162_));
  nand4  g100(.a(new_n149_), .b(new_n76_), .c(new_n75_), .d(x28), .O(new_n163_));
  nand3  g101(.a(x39), .b(new_n148_), .c(new_n65_), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n163_), .c(x10), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n66_), .c(new_n162_), .O(new_n167_));
  inv1   g105(.a(x35), .O(new_n168_));
  nand3  g106(.a(new_n64_), .b(new_n168_), .c(x28), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x27), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n76_), .c(new_n75_), .O(new_n171_));
  nand2  g109(.a(new_n154_), .b(x27), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n164_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n83_), .O(new_n175_));
  nand2  g113(.a(new_n172_), .b(new_n66_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n175_), .c(new_n167_), .O(z14));
  inv1   g115(.a(x12), .O(new_n178_));
  nand4  g116(.a(new_n73_), .b(x34), .c(x27), .d(x26), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n178_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n73_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n73_), .O(z17));
  nand3  g124(.a(x40), .b(x39), .c(x29), .O(new_n187_));
  oai21  g125(.a(new_n78_), .b(x27), .c(new_n187_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(x08), .O(new_n189_));
  nor2   g127(.a(new_n90_), .b(x04), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n79_), .c(x30), .O(new_n191_));
  nand4  g129(.a(new_n191_), .b(new_n189_), .c(new_n73_), .d(new_n132_), .O(z18));
  nand2  g130(.a(new_n141_), .b(new_n73_), .O(z19));
  nor2   g131(.a(new_n136_), .b(new_n66_), .O(new_n194_));
  aoi21  g132(.a(x28), .b(new_n90_), .c(new_n168_), .O(new_n195_));
  aoi21  g133(.a(x35), .b(x27), .c(x10), .O(new_n196_));
  nor2   g134(.a(x40), .b(x27), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n196_), .c(new_n89_), .O(new_n198_));
  oai21  g136(.a(new_n195_), .b(new_n194_), .c(new_n198_), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(new_n75_), .c(new_n132_), .O(new_n200_));
  inv1   g138(.a(new_n200_), .O(z20));
endmodule


