// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n204_, new_n205_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x05), .O(new_n78_));
  nand2  g016(.a(x40), .b(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(new_n79_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  oai21  g021(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n81_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  oai21  g029(.a(x35), .b(new_n78_), .c(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(new_n78_), .O(new_n93_));
  nor2   g031(.a(x27), .b(x08), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x04), .c(new_n72_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n93_), .c(new_n90_), .d(x40), .O(z02));
  nand2  g034(.a(x35), .b(x28), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(x37), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n64_), .c(new_n99_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n81_), .O(z03));
  nand2  g040(.a(new_n100_), .b(new_n64_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n98_), .c(new_n79_), .d(new_n99_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n81_), .O(z05));
  aoi21  g044(.a(new_n79_), .b(new_n64_), .c(new_n91_), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(x37), .c(new_n79_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n79_), .O(z07));
  oai21  g057(.a(x39), .b(new_n78_), .c(x40), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n79_), .O(z09));
  nand3  g061(.a(new_n84_), .b(x07), .c(new_n65_), .O(new_n124_));
  nand2  g062(.a(x37), .b(x06), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n81_), .O(new_n126_));
  nand4  g064(.a(new_n84_), .b(x40), .c(x39), .d(x05), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(x04), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n126_), .c(x27), .O(new_n129_));
  aoi21  g067(.a(new_n83_), .b(new_n82_), .c(new_n73_), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(x39), .c(x05), .d(new_n65_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z10));
  nand2  g070(.a(new_n79_), .b(new_n64_), .O(new_n133_));
  nand2  g071(.a(x40), .b(x04), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x35), .c(new_n91_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x05), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n133_), .c(x08), .O(new_n137_));
  nor2   g075(.a(new_n66_), .b(x28), .O(new_n138_));
  nand2  g076(.a(x39), .b(x29), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x05), .c(new_n73_), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n138_), .c(new_n140_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n137_), .c(new_n82_), .O(new_n143_));
  nor2   g081(.a(new_n143_), .b(x09), .O(z11));
  inv1   g082(.a(x10), .O(new_n145_));
  nand3  g083(.a(new_n100_), .b(new_n67_), .c(new_n66_), .O(new_n146_));
  nor2   g084(.a(new_n73_), .b(x27), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x05), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  oai21  g088(.a(x27), .b(new_n65_), .c(x05), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x40), .O(new_n152_));
  oai21  g090(.a(new_n72_), .b(x04), .c(x05), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x40), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n97_), .c(new_n100_), .O(new_n155_));
  nand2  g093(.a(new_n120_), .b(new_n64_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(new_n155_), .c(new_n152_), .d(new_n150_), .O(z12));
  inv1   g095(.a(x13), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x28), .c(new_n73_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x39), .c(new_n158_), .d(new_n65_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  inv1   g101(.a(new_n70_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x20), .c(new_n163_), .d(new_n162_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n161_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x27), .O(new_n167_));
  nand2  g105(.a(new_n83_), .b(new_n82_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(x39), .c(new_n158_), .d(new_n65_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x05), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x40), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n167_), .O(z13));
  nand3  g110(.a(x20), .b(new_n163_), .c(new_n162_), .O(new_n173_));
  nand4  g111(.a(new_n159_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n174_));
  oai21  g112(.a(x13), .b(x04), .c(x40), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(x39), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x05), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x40), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n173_), .O(new_n179_));
  nand3  g117(.a(new_n100_), .b(new_n66_), .c(x28), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n83_), .c(new_n82_), .O(new_n182_));
  oai22  g120(.a(new_n147_), .b(new_n70_), .c(x13), .d(x04), .O(new_n183_));
  oai21  g121(.a(new_n70_), .b(new_n64_), .c(new_n72_), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n183_), .c(new_n182_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(x05), .O(new_n186_));
  oai21  g124(.a(new_n70_), .b(new_n64_), .c(new_n73_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n186_), .c(new_n179_), .O(z14));
  inv1   g126(.a(x12), .O(new_n189_));
  nand4  g127(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n189_), .O(z15));
  inv1   g129(.a(x23), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(x22), .c(x01), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n79_), .O(z16));
  inv1   g132(.a(x01), .O(new_n195_));
  inv1   g133(.a(x24), .O(new_n196_));
  nand4  g134(.a(new_n79_), .b(new_n196_), .c(x23), .d(x22), .O(new_n197_));
  nor2   g135(.a(new_n197_), .b(new_n195_), .O(z17));
  nand2  g136(.a(x27), .b(new_n65_), .O(new_n199_));
  nand2  g137(.a(new_n64_), .b(x08), .O(new_n200_));
  aoi21  g138(.a(new_n200_), .b(new_n199_), .c(new_n66_), .O(new_n201_));
  inv1   g139(.a(x09), .O(new_n202_));
  nand2  g140(.a(new_n82_), .b(new_n202_), .O(new_n203_));
  aoi21  g141(.a(new_n201_), .b(new_n91_), .c(new_n203_), .O(new_n204_));
  nand4  g142(.a(new_n74_), .b(x29), .c(x08), .d(x05), .O(new_n205_));
  oai21  g143(.a(new_n204_), .b(new_n81_), .c(new_n205_), .O(z18));
  aoi21  g144(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n207_));
  nor2   g145(.a(new_n207_), .b(new_n140_), .O(new_n208_));
  oai21  g146(.a(new_n66_), .b(new_n64_), .c(x05), .O(new_n209_));
  nand2  g147(.a(new_n73_), .b(new_n64_), .O(new_n210_));
  aoi21  g148(.a(new_n210_), .b(new_n209_), .c(x08), .O(new_n211_));
  oai21  g149(.a(new_n211_), .b(new_n208_), .c(new_n82_), .O(new_n212_));
  nor2   g150(.a(new_n212_), .b(x09), .O(z20));
  nor2   g151(.a(new_n143_), .b(x09), .O(z19));
endmodule


