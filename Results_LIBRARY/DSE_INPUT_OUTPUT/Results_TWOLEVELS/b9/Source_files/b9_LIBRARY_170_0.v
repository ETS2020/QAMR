// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n64_), .d(new_n71_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  aoi21  g018(.a(x35), .b(new_n80_), .c(x36), .O(new_n81_));
  or2    g019(.a(new_n81_), .b(new_n66_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n79_), .c(new_n78_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n80_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x04), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n87_), .c(x40), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  inv1   g030(.a(x39), .O(new_n93_));
  nand2  g031(.a(x40), .b(new_n93_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n92_), .O(z02));
  inv1   g033(.a(new_n63_), .O(new_n96_));
  nand2  g034(.a(new_n68_), .b(x27), .O(new_n97_));
  nand3  g035(.a(new_n96_), .b(new_n67_), .c(new_n66_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(x21), .O(z03));
  nor2   g037(.a(new_n63_), .b(x21), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n96_), .c(new_n67_), .O(z05));
  oai21  g041(.a(x40), .b(x39), .c(new_n66_), .O(new_n104_));
  oai21  g042(.a(x40), .b(x39), .c(new_n80_), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(x37), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n64_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n96_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  nand2  g055(.a(x40), .b(x39), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n96_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n96_), .O(z09));
  xor2a  g059(.a(x40), .b(x39), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x07), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n81_), .O(new_n125_));
  inv1   g063(.a(x06), .O(new_n126_));
  nor3   g064(.a(new_n63_), .b(new_n67_), .c(new_n126_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n71_), .c(new_n127_), .O(new_n128_));
  oai21  g066(.a(x32), .b(x30), .c(x40), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n93_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(x05), .c(new_n71_), .O(new_n131_));
  oai21  g069(.a(new_n128_), .b(new_n66_), .c(new_n131_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  inv1   g071(.a(x08), .O(new_n134_));
  inv1   g072(.a(x35), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(x04), .c(x39), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n104_), .c(new_n80_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand2  g076(.a(x40), .b(x29), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand2  g078(.a(x39), .b(new_n135_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n80_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g081(.a(x40), .b(new_n135_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n140_), .c(new_n80_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n93_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n143_), .c(new_n138_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n78_), .c(new_n133_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n96_), .O(z11));
  inv1   g087(.a(x36), .O(new_n150_));
  nand3  g088(.a(new_n67_), .b(new_n150_), .c(new_n135_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x27), .c(x10), .O(new_n152_));
  aoi21  g090(.a(x40), .b(new_n71_), .c(new_n69_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n152_), .c(x39), .O(new_n154_));
  inv1   g092(.a(new_n69_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(x40), .c(new_n93_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n154_), .O(z12));
  inv1   g095(.a(x13), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x28), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x40), .c(new_n158_), .d(new_n71_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  nand2  g101(.a(new_n68_), .b(new_n67_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x20), .c(new_n163_), .d(new_n162_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n161_), .c(new_n93_), .O(new_n166_));
  nand4  g104(.a(new_n164_), .b(x40), .c(x20), .d(new_n163_), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(x18), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(x27), .O(new_n169_));
  nand3  g107(.a(new_n130_), .b(new_n158_), .c(new_n71_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(z13));
  inv1   g109(.a(new_n94_), .O(new_n172_));
  nand3  g110(.a(x20), .b(new_n163_), .c(new_n162_), .O(new_n173_));
  nand4  g111(.a(new_n159_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n174_));
  nand3  g112(.a(x40), .b(new_n158_), .c(new_n71_), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n174_), .c(new_n93_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n172_), .c(new_n173_), .O(new_n178_));
  nand4  g116(.a(x39), .b(new_n67_), .c(new_n135_), .d(x28), .O(new_n179_));
  nand2  g117(.a(x40), .b(new_n66_), .O(new_n180_));
  aoi21  g118(.a(new_n180_), .b(new_n179_), .c(x32), .O(new_n181_));
  oai21  g119(.a(new_n176_), .b(new_n93_), .c(new_n94_), .O(new_n182_));
  aoi22  g120(.a(new_n182_), .b(new_n155_), .c(new_n181_), .d(new_n78_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n178_), .O(z14));
  inv1   g122(.a(x12), .O(new_n185_));
  nand4  g123(.a(new_n96_), .b(x34), .c(x27), .d(x26), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n185_), .O(z15));
  inv1   g125(.a(x23), .O(new_n188_));
  nand4  g126(.a(new_n96_), .b(new_n188_), .c(x22), .d(x01), .O(new_n189_));
  inv1   g127(.a(new_n189_), .O(z16));
  inv1   g128(.a(x24), .O(new_n191_));
  nand4  g129(.a(new_n191_), .b(x23), .c(x22), .d(x01), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n96_), .O(z17));
  nand2  g131(.a(x27), .b(new_n71_), .O(new_n194_));
  nand2  g132(.a(new_n66_), .b(x08), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n194_), .c(new_n135_), .O(new_n196_));
  nand2  g134(.a(new_n78_), .b(new_n133_), .O(new_n197_));
  aoi21  g135(.a(new_n196_), .b(new_n80_), .c(new_n197_), .O(new_n198_));
  oai22  g136(.a(new_n198_), .b(new_n63_), .c(new_n118_), .d(new_n86_), .O(z18));
  nand2  g137(.a(x28), .b(new_n66_), .O(new_n200_));
  nand2  g138(.a(new_n139_), .b(x39), .O(new_n201_));
  aoi22  g139(.a(new_n201_), .b(new_n94_), .c(new_n200_), .d(x35), .O(new_n202_));
  aoi21  g140(.a(new_n141_), .b(new_n104_), .c(x08), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n202_), .c(new_n78_), .O(new_n204_));
  nor2   g142(.a(new_n204_), .b(x09), .O(z20));
  nand2  g143(.a(new_n148_), .b(new_n96_), .O(z19));
endmodule


