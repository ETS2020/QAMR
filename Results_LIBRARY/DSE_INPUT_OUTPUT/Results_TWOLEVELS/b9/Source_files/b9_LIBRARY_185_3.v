// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  nand2  g015(.a(x40), .b(x21), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g017(.a(new_n78_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  aoi21  g021(.a(x35), .b(new_n83_), .c(x36), .O(new_n84_));
  or2    g022(.a(new_n84_), .b(new_n64_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n82_), .c(new_n81_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n80_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g028(.a(x08), .b(new_n65_), .c(x40), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n64_), .O(new_n92_));
  nand2  g030(.a(x35), .b(new_n83_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x04), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(x28), .c(new_n64_), .O(new_n96_));
  nand2  g034(.a(new_n78_), .b(x39), .O(new_n97_));
  aoi21  g035(.a(new_n96_), .b(new_n73_), .c(new_n97_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n94_), .c(new_n92_), .d(new_n90_), .O(z02));
  nand3  g037(.a(new_n78_), .b(new_n95_), .c(new_n64_), .O(new_n100_));
  inv1   g038(.a(x21), .O(new_n101_));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n95_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g042(.a(x35), .b(x28), .c(new_n64_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(new_n106_));
  nand4  g044(.a(new_n106_), .b(new_n104_), .c(new_n100_), .d(new_n78_), .O(z03));
  aoi21  g045(.a(new_n95_), .b(new_n64_), .c(new_n105_), .O(new_n108_));
  nand2  g046(.a(new_n73_), .b(x21), .O(new_n109_));
  oai21  g047(.a(new_n108_), .b(x21), .c(new_n109_), .O(z04));
  aoi21  g048(.a(new_n102_), .b(new_n95_), .c(new_n80_), .O(z05));
  nand3  g049(.a(new_n102_), .b(new_n78_), .c(new_n95_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z06));
  inv1   g051(.a(x33), .O(new_n114_));
  nand2  g052(.a(x17), .b(new_n63_), .O(new_n115_));
  inv1   g053(.a(x00), .O(new_n116_));
  oai21  g054(.a(x25), .b(new_n116_), .c(x38), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(new_n118_));
  inv1   g056(.a(x25), .O(new_n119_));
  nand3  g057(.a(x38), .b(new_n119_), .c(new_n116_), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  oai22  g059(.a(new_n121_), .b(x14), .c(new_n118_), .d(x31), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x03), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n78_), .O(z07));
  nand3  g062(.a(x40), .b(x39), .c(new_n101_), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(z08));
  nand4  g064(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n78_), .O(z09));
  oai21  g066(.a(x39), .b(x21), .c(x40), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x07), .O(new_n130_));
  nand3  g068(.a(new_n74_), .b(new_n101_), .c(x05), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n130_), .c(new_n84_), .O(new_n132_));
  nand3  g070(.a(new_n78_), .b(x37), .c(x06), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(new_n134_));
  aoi21  g072(.a(new_n132_), .b(new_n65_), .c(new_n134_), .O(new_n135_));
  oai21  g073(.a(x32), .b(x30), .c(x40), .O(new_n136_));
  nor3   g074(.a(new_n136_), .b(new_n72_), .c(x21), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x05), .c(new_n65_), .O(new_n138_));
  oai21  g076(.a(new_n135_), .b(new_n64_), .c(new_n138_), .O(z10));
  inv1   g077(.a(x09), .O(new_n140_));
  nand3  g078(.a(x35), .b(new_n83_), .c(new_n65_), .O(new_n141_));
  aoi22  g079(.a(new_n141_), .b(new_n101_), .c(new_n78_), .d(new_n64_), .O(new_n142_));
  aoi21  g080(.a(x39), .b(x29), .c(x21), .O(new_n143_));
  nor2   g081(.a(new_n64_), .b(new_n65_), .O(new_n144_));
  oai22  g082(.a(new_n144_), .b(new_n93_), .c(new_n143_), .d(new_n73_), .O(new_n145_));
  oai21  g083(.a(new_n142_), .b(x08), .c(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n81_), .c(new_n140_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(z11));
  nand3  g086(.a(new_n95_), .b(new_n67_), .c(new_n66_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x27), .c(x10), .O(new_n150_));
  nor2   g088(.a(new_n70_), .b(new_n64_), .O(new_n151_));
  aoi21  g089(.a(x39), .b(new_n65_), .c(new_n151_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(new_n101_), .O(new_n153_));
  oai21  g091(.a(new_n70_), .b(new_n64_), .c(new_n73_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n73_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n65_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  inv1   g099(.a(new_n70_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n159_), .c(x21), .O(new_n164_));
  nand4  g102(.a(new_n162_), .b(new_n73_), .c(x20), .d(new_n161_), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(x18), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n164_), .c(x27), .O(new_n167_));
  nand3  g105(.a(new_n137_), .b(new_n156_), .c(new_n65_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z13));
  nand3  g107(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n170_));
  nand4  g108(.a(new_n157_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n171_));
  nand3  g109(.a(x39), .b(new_n156_), .c(new_n65_), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n171_), .c(x21), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n73_), .c(new_n170_), .O(new_n175_));
  nand3  g113(.a(new_n95_), .b(new_n66_), .c(x28), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n82_), .c(new_n81_), .O(new_n178_));
  oai21  g116(.a(new_n173_), .b(new_n151_), .c(new_n178_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n101_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n175_), .c(new_n154_), .O(z14));
  nand4  g119(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n78_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(x22), .c(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n78_), .O(z16));
  inv1   g124(.a(x01), .O(new_n187_));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n78_), .b(new_n188_), .c(x23), .d(x22), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n187_), .O(z17));
  nand2  g128(.a(x27), .b(new_n65_), .O(new_n191_));
  nand2  g129(.a(new_n64_), .b(x08), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n191_), .c(new_n66_), .O(new_n193_));
  nand2  g131(.a(new_n81_), .b(new_n140_), .O(new_n194_));
  aoi21  g132(.a(new_n193_), .b(new_n83_), .c(new_n194_), .O(new_n195_));
  nand4  g133(.a(new_n74_), .b(x29), .c(new_n101_), .d(x08), .O(new_n196_));
  oai21  g134(.a(new_n195_), .b(new_n80_), .c(new_n196_), .O(z18));
  nand2  g135(.a(x39), .b(x29), .O(new_n198_));
  nor2   g136(.a(new_n83_), .b(x27), .O(new_n199_));
  nor2   g137(.a(x35), .b(x21), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n199_), .c(new_n198_), .O(new_n201_));
  inv1   g139(.a(x08), .O(new_n202_));
  aoi21  g140(.a(x35), .b(x27), .c(x21), .O(new_n203_));
  nor2   g141(.a(x40), .b(x27), .O(new_n204_));
  oai21  g142(.a(new_n204_), .b(new_n203_), .c(new_n202_), .O(new_n205_));
  oai21  g143(.a(new_n199_), .b(new_n66_), .c(new_n73_), .O(new_n206_));
  nand3  g144(.a(new_n206_), .b(new_n205_), .c(new_n201_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n81_), .c(new_n140_), .O(new_n208_));
  nand2  g146(.a(new_n208_), .b(new_n78_), .O(z20));
  inv1   g147(.a(new_n147_), .O(z19));
endmodule


