// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n107_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n193_, new_n194_, new_n195_,
    new_n196_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(x10), .c(new_n65_), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(new_n71_), .c(new_n63_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x32), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n75_), .c(x16), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n76_), .c(x04), .O(z01));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n81_), .c(x04), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x39), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n86_), .c(new_n79_), .O(new_n90_));
  inv1   g028(.a(x40), .O(new_n91_));
  inv1   g029(.a(x09), .O(new_n92_));
  nand3  g030(.a(new_n78_), .b(new_n92_), .c(x04), .O(new_n93_));
  nand3  g031(.a(x35), .b(new_n80_), .c(new_n65_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(new_n95_));
  nand4  g033(.a(new_n82_), .b(new_n78_), .c(x27), .d(new_n92_), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n95_), .c(new_n91_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n90_), .O(z02));
  oai21  g036(.a(new_n66_), .b(new_n80_), .c(x27), .O(new_n99_));
  inv1   g037(.a(x37), .O(new_n100_));
  inv1   g038(.a(new_n76_), .O(new_n101_));
  aoi21  g039(.a(new_n100_), .b(new_n64_), .c(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n99_), .c(x21), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  nand3  g042(.a(new_n102_), .b(new_n99_), .c(new_n104_), .O(z04));
  nor2   g043(.a(new_n80_), .b(new_n64_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n76_), .c(new_n100_), .O(z05));
  oai21  g046(.a(new_n106_), .b(x37), .c(new_n76_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n63_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n76_), .c(x03), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z07));
  nand3  g058(.a(x40), .b(x39), .c(new_n79_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z08));
  inv1   g060(.a(x11), .O(new_n123_));
  nand4  g061(.a(new_n76_), .b(x34), .c(x27), .d(x26), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n123_), .O(z09));
  oai21  g063(.a(x39), .b(x32), .c(x40), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x07), .O(new_n127_));
  nand3  g065(.a(new_n72_), .b(new_n79_), .c(x05), .O(new_n128_));
  aoi22  g066(.a(new_n128_), .b(new_n127_), .c(new_n81_), .d(new_n67_), .O(new_n129_));
  nand3  g067(.a(new_n76_), .b(x37), .c(x06), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  aoi21  g069(.a(new_n129_), .b(new_n65_), .c(new_n131_), .O(new_n132_));
  and2   g070(.a(new_n72_), .b(new_n79_), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(x30), .c(x05), .d(new_n65_), .O(new_n134_));
  oai21  g072(.a(new_n132_), .b(new_n64_), .c(new_n134_), .O(z10));
  aoi21  g073(.a(x40), .b(x32), .c(x27), .O(new_n136_));
  aoi21  g074(.a(new_n94_), .b(new_n79_), .c(new_n136_), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n79_), .c(new_n91_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x35), .c(new_n80_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(new_n142_));
  oai22  g080(.a(new_n142_), .b(new_n139_), .c(new_n137_), .d(x08), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n78_), .c(new_n92_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(z11));
  nor2   g083(.a(new_n70_), .b(new_n64_), .O(new_n146_));
  nand3  g084(.a(new_n100_), .b(new_n67_), .c(new_n66_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  aoi21  g086(.a(x39), .b(new_n65_), .c(new_n146_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n79_), .O(new_n150_));
  oai21  g088(.a(new_n146_), .b(x40), .c(new_n150_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n91_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n65_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  inv1   g095(.a(new_n70_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n155_), .c(x32), .O(new_n160_));
  nand4  g098(.a(new_n158_), .b(new_n91_), .c(x20), .d(new_n157_), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(x18), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n160_), .c(x27), .O(new_n163_));
  nand4  g101(.a(new_n133_), .b(x30), .c(new_n152_), .d(new_n65_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(z13));
  nand3  g103(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n166_));
  nand3  g104(.a(new_n153_), .b(new_n78_), .c(x28), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(new_n72_), .c(new_n152_), .d(new_n65_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n166_), .O(new_n169_));
  nand3  g107(.a(new_n100_), .b(new_n66_), .c(x28), .O(new_n170_));
  oai21  g108(.a(x32), .b(x27), .c(new_n170_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n78_), .O(new_n172_));
  aoi21  g110(.a(x39), .b(new_n65_), .c(x32), .O(new_n173_));
  nand2  g111(.a(x40), .b(new_n152_), .O(new_n174_));
  oai22  g112(.a(new_n174_), .b(new_n173_), .c(new_n70_), .d(new_n64_), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(new_n172_), .c(new_n169_), .d(new_n76_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n76_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(x22), .c(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n76_), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(x23), .c(x22), .d(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n76_), .O(z17));
  nand2  g122(.a(new_n78_), .b(new_n92_), .O(new_n185_));
  nand2  g123(.a(x27), .b(new_n65_), .O(new_n186_));
  nand2  g124(.a(new_n64_), .b(x08), .O(new_n187_));
  aoi21  g125(.a(new_n187_), .b(new_n186_), .c(new_n66_), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n80_), .c(new_n185_), .O(new_n189_));
  nand3  g127(.a(new_n133_), .b(x29), .c(x08), .O(new_n190_));
  oai21  g128(.a(new_n189_), .b(new_n101_), .c(new_n190_), .O(z18));
  nand2  g129(.a(new_n144_), .b(new_n76_), .O(z19));
  aoi21  g130(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n193_));
  aoi21  g131(.a(new_n66_), .b(new_n79_), .c(new_n136_), .O(new_n194_));
  oai22  g132(.a(new_n194_), .b(x08), .c(new_n193_), .d(new_n139_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n78_), .c(new_n92_), .O(new_n196_));
  inv1   g134(.a(new_n196_), .O(z20));
endmodule


