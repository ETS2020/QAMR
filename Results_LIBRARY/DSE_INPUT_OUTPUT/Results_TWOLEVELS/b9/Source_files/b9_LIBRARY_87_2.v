// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n196_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n204_, new_n205_,
    new_n206_, new_n207_, new_n208_, new_n209_, new_n212_, new_n213_,
    new_n214_, new_n215_, new_n216_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x31), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n63_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n71_), .O(z00));
  inv1   g015(.a(new_n64_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n78_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n83_), .c(x04), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x40), .O(new_n94_));
  nor2   g032(.a(x40), .b(x31), .O(new_n95_));
  inv1   g033(.a(new_n95_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z02));
  nand2  g035(.a(new_n67_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(x27), .O(new_n100_));
  aoi21  g038(.a(new_n66_), .b(new_n100_), .c(new_n99_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n78_), .O(z03));
  nand2  g040(.a(new_n66_), .b(new_n100_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n98_), .c(new_n64_), .d(new_n99_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n78_), .O(z05));
  nand2  g044(.a(new_n64_), .b(new_n100_), .O(new_n107_));
  inv1   g045(.a(x31), .O(new_n108_));
  oai21  g046(.a(x40), .b(new_n108_), .c(new_n82_), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n107_), .c(x37), .O(z06));
  inv1   g048(.a(x03), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n65_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  inv1   g053(.a(x14), .O(new_n116_));
  inv1   g054(.a(x25), .O(new_n117_));
  nand3  g055(.a(x38), .b(new_n117_), .c(new_n113_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  oai21  g057(.a(new_n115_), .b(x33), .c(new_n119_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n108_), .O(new_n121_));
  nand3  g059(.a(new_n118_), .b(x40), .c(new_n116_), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(new_n111_), .O(z07));
  nand2  g061(.a(x40), .b(x39), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(z08));
  inv1   g063(.a(x11), .O(new_n126_));
  nand4  g064(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n126_), .O(z09));
  inv1   g066(.a(x07), .O(new_n129_));
  inv1   g067(.a(x39), .O(new_n130_));
  aoi21  g068(.a(x40), .b(new_n130_), .c(new_n95_), .O(new_n131_));
  nand2  g069(.a(z08), .b(x05), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(new_n129_), .c(new_n132_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n84_), .c(new_n72_), .O(new_n134_));
  nand3  g072(.a(new_n64_), .b(x37), .c(x06), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x27), .O(new_n137_));
  inv1   g075(.a(x05), .O(new_n138_));
  nand2  g076(.a(new_n80_), .b(new_n79_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(x40), .c(x39), .O(new_n140_));
  nor2   g078(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n72_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n137_), .O(z10));
  oai21  g081(.a(new_n83_), .b(x04), .c(x40), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n107_), .c(x08), .O(new_n145_));
  inv1   g083(.a(new_n83_), .O(new_n146_));
  nand2  g084(.a(x39), .b(x29), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x40), .O(new_n148_));
  nand2  g086(.a(x27), .b(x04), .O(new_n149_));
  aoi22  g087(.a(new_n149_), .b(new_n146_), .c(new_n148_), .d(new_n96_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n145_), .c(new_n79_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(x09), .c(new_n64_), .O(z11));
  nor3   g090(.a(x37), .b(x36), .c(x35), .O(new_n153_));
  nand2  g091(.a(x40), .b(new_n100_), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n153_), .c(new_n73_), .O(new_n156_));
  oai21  g094(.a(new_n130_), .b(x04), .c(new_n67_), .O(new_n157_));
  inv1   g095(.a(x35), .O(new_n158_));
  aoi21  g096(.a(new_n108_), .b(new_n82_), .c(new_n158_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(x40), .c(new_n157_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n66_), .O(new_n161_));
  aoi21  g099(.a(new_n108_), .b(x27), .c(x40), .O(new_n162_));
  nand2  g100(.a(x40), .b(x04), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x39), .c(x27), .O(new_n164_));
  nor2   g102(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n161_), .c(new_n156_), .O(z12));
  inv1   g104(.a(x13), .O(new_n167_));
  nand2  g105(.a(x36), .b(x35), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(x28), .c(new_n63_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x39), .c(new_n167_), .d(new_n72_), .O(new_n170_));
  inv1   g108(.a(x18), .O(new_n171_));
  inv1   g109(.a(x19), .O(new_n172_));
  nand4  g110(.a(new_n68_), .b(x20), .c(new_n172_), .d(new_n171_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n170_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nor2   g113(.a(new_n140_), .b(x13), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n72_), .c(new_n78_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n175_), .O(z13));
  nand3  g116(.a(x20), .b(new_n172_), .c(new_n171_), .O(new_n179_));
  nand4  g117(.a(new_n168_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n180_));
  nor2   g118(.a(x13), .b(x04), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(new_n180_), .c(x40), .d(x39), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n179_), .O(new_n183_));
  nor2   g121(.a(x37), .b(x35), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x28), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n154_), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n80_), .c(new_n79_), .O(new_n187_));
  inv1   g125(.a(new_n181_), .O(new_n188_));
  nand2  g126(.a(new_n154_), .b(new_n68_), .O(new_n189_));
  oai21  g127(.a(new_n184_), .b(new_n100_), .c(new_n124_), .O(new_n190_));
  nand2  g128(.a(new_n96_), .b(x39), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(new_n66_), .c(new_n82_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n190_), .c(new_n64_), .O(new_n193_));
  aoi21  g131(.a(new_n189_), .b(new_n188_), .c(new_n193_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n187_), .c(new_n183_), .O(z14));
  inv1   g133(.a(x12), .O(new_n196_));
  nor2   g134(.a(new_n127_), .b(new_n196_), .O(z15));
  inv1   g135(.a(x23), .O(new_n198_));
  nand4  g136(.a(new_n64_), .b(new_n198_), .c(x22), .d(x01), .O(new_n199_));
  inv1   g137(.a(new_n199_), .O(z16));
  inv1   g138(.a(x24), .O(new_n201_));
  nand4  g139(.a(new_n201_), .b(x23), .c(x22), .d(x01), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n64_), .O(z17));
  nand2  g141(.a(x27), .b(new_n72_), .O(new_n204_));
  nand2  g142(.a(new_n100_), .b(x08), .O(new_n205_));
  aoi21  g143(.a(new_n205_), .b(new_n204_), .c(new_n158_), .O(new_n206_));
  inv1   g144(.a(x09), .O(new_n207_));
  nand2  g145(.a(new_n79_), .b(new_n207_), .O(new_n208_));
  aoi21  g146(.a(new_n206_), .b(new_n82_), .c(new_n208_), .O(new_n209_));
  oai22  g147(.a(new_n209_), .b(new_n78_), .c(new_n124_), .d(new_n89_), .O(z18));
  nor2   g148(.a(new_n151_), .b(x09), .O(z19));
  aoi21  g149(.a(x28), .b(new_n100_), .c(new_n158_), .O(new_n212_));
  aoi21  g150(.a(new_n148_), .b(new_n96_), .c(new_n212_), .O(new_n213_));
  nand2  g151(.a(x40), .b(new_n158_), .O(new_n214_));
  aoi21  g152(.a(new_n214_), .b(new_n107_), .c(x08), .O(new_n215_));
  oai21  g153(.a(new_n215_), .b(new_n213_), .c(new_n79_), .O(new_n216_));
  nor2   g154(.a(new_n216_), .b(x09), .O(z20));
endmodule


