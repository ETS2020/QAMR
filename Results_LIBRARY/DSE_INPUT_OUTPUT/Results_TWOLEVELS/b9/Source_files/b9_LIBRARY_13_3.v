// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n109_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  nand2  g017(.a(x40), .b(x31), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n79_), .c(x16), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n83_), .c(new_n82_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n80_), .c(x04), .O(z01));
  inv1   g027(.a(x31), .O(new_n90_));
  inv1   g028(.a(x40), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g032(.a(new_n85_), .b(x04), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n94_), .c(new_n91_), .O(new_n96_));
  inv1   g034(.a(x08), .O(new_n97_));
  nand3  g035(.a(new_n64_), .b(new_n97_), .c(x04), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x39), .O(new_n99_));
  oai21  g037(.a(new_n99_), .b(new_n96_), .c(new_n90_), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x40), .O(z02));
  inv1   g039(.a(new_n80_), .O(new_n102_));
  nand2  g040(.a(new_n71_), .b(x27), .O(new_n103_));
  inv1   g041(.a(x21), .O(new_n104_));
  aoi21  g042(.a(new_n70_), .b(new_n64_), .c(new_n104_), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(z03));
  nand2  g044(.a(new_n70_), .b(new_n64_), .O(new_n107_));
  nand4  g045(.a(new_n107_), .b(new_n103_), .c(new_n80_), .d(new_n104_), .O(z04));
  aoi21  g046(.a(x28), .b(x27), .c(x37), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n102_), .O(z05));
  aoi21  g048(.a(x40), .b(x31), .c(x27), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n84_), .c(new_n70_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n80_), .O(z06));
  inv1   g051(.a(x33), .O(new_n114_));
  nand2  g052(.a(x17), .b(new_n63_), .O(new_n115_));
  inv1   g053(.a(x00), .O(new_n116_));
  oai21  g054(.a(x25), .b(new_n116_), .c(x38), .O(new_n117_));
  nand4  g055(.a(new_n117_), .b(new_n115_), .c(new_n114_), .d(new_n90_), .O(new_n118_));
  inv1   g056(.a(x25), .O(new_n119_));
  nand3  g057(.a(x38), .b(new_n119_), .c(new_n116_), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  oai21  g059(.a(new_n121_), .b(x14), .c(new_n118_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x03), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n80_), .O(z07));
  nand3  g062(.a(x40), .b(x39), .c(new_n90_), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(z08));
  nand4  g064(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n80_), .O(z09));
  inv1   g066(.a(x05), .O(new_n129_));
  nand2  g067(.a(new_n75_), .b(x07), .O(new_n130_));
  oai21  g068(.a(new_n75_), .b(new_n129_), .c(new_n130_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n86_), .c(x27), .O(new_n132_));
  inv1   g070(.a(x39), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x05), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n65_), .O(new_n138_));
  nand3  g076(.a(x37), .b(x27), .c(x06), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n80_), .O(z10));
  inv1   g078(.a(x09), .O(new_n141_));
  nand3  g079(.a(x35), .b(new_n84_), .c(new_n65_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(new_n90_), .c(new_n111_), .O(new_n143_));
  nand2  g081(.a(x39), .b(x29), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n90_), .c(new_n91_), .O(new_n145_));
  nand2  g083(.a(x27), .b(x04), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(x35), .c(new_n84_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n145_), .c(new_n143_), .d(x08), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n82_), .c(new_n141_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(z11));
  inv1   g089(.a(x10), .O(new_n152_));
  nor3   g090(.a(x37), .b(x36), .c(x35), .O(new_n153_));
  nor2   g091(.a(new_n91_), .b(x27), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n153_), .c(new_n152_), .O(new_n155_));
  oai21  g093(.a(x27), .b(new_n65_), .c(new_n90_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x40), .O(new_n157_));
  nand2  g095(.a(new_n76_), .b(new_n65_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n71_), .c(new_n70_), .O(new_n159_));
  nand2  g097(.a(new_n75_), .b(new_n64_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(new_n159_), .c(new_n157_), .d(new_n155_), .O(z12));
  inv1   g099(.a(x13), .O(new_n162_));
  nand2  g100(.a(x36), .b(x35), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x28), .c(new_n91_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x39), .c(new_n162_), .d(new_n65_), .O(new_n165_));
  inv1   g103(.a(x18), .O(new_n166_));
  inv1   g104(.a(x19), .O(new_n167_));
  nand4  g105(.a(new_n72_), .b(x20), .c(new_n167_), .d(new_n166_), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n165_), .c(x31), .O(new_n169_));
  nand4  g107(.a(new_n72_), .b(new_n91_), .c(x20), .d(new_n167_), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(x18), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n169_), .c(x27), .O(new_n172_));
  nand4  g110(.a(new_n135_), .b(new_n90_), .c(new_n162_), .d(new_n65_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n172_), .O(z13));
  nand3  g112(.a(x20), .b(new_n167_), .c(new_n166_), .O(new_n175_));
  nand4  g113(.a(new_n163_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(new_n177_));
  nand3  g115(.a(new_n76_), .b(new_n162_), .c(new_n65_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n177_), .c(new_n175_), .O(new_n179_));
  nand3  g117(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n83_), .c(new_n82_), .O(new_n182_));
  nand3  g120(.a(new_n178_), .b(new_n71_), .c(new_n70_), .O(new_n183_));
  nand4  g121(.a(x40), .b(x39), .c(new_n162_), .d(new_n65_), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n64_), .c(new_n102_), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n183_), .c(new_n182_), .d(new_n179_), .O(z14));
  inv1   g124(.a(x12), .O(new_n187_));
  nand4  g125(.a(new_n80_), .b(x34), .c(x27), .d(x26), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n187_), .O(z15));
  inv1   g127(.a(x23), .O(new_n190_));
  nand4  g128(.a(new_n80_), .b(new_n190_), .c(x22), .d(x01), .O(new_n191_));
  inv1   g129(.a(new_n191_), .O(z16));
  inv1   g130(.a(x01), .O(new_n193_));
  inv1   g131(.a(x24), .O(new_n194_));
  nand4  g132(.a(new_n80_), .b(new_n194_), .c(x23), .d(x22), .O(new_n195_));
  nor2   g133(.a(new_n195_), .b(new_n193_), .O(z17));
  nand2  g134(.a(x27), .b(new_n65_), .O(new_n197_));
  nand2  g135(.a(new_n64_), .b(x08), .O(new_n198_));
  aoi21  g136(.a(new_n198_), .b(new_n197_), .c(new_n66_), .O(new_n199_));
  nand2  g137(.a(new_n82_), .b(new_n141_), .O(new_n200_));
  aoi21  g138(.a(new_n199_), .b(new_n84_), .c(new_n200_), .O(new_n201_));
  nand2  g139(.a(new_n76_), .b(new_n90_), .O(new_n202_));
  oai22  g140(.a(new_n202_), .b(new_n93_), .c(new_n201_), .d(new_n102_), .O(z18));
  aoi21  g141(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n204_));
  nor2   g142(.a(x35), .b(x31), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n111_), .c(new_n97_), .O(new_n206_));
  oai21  g144(.a(new_n204_), .b(new_n145_), .c(new_n206_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n82_), .c(new_n141_), .O(new_n208_));
  inv1   g146(.a(new_n208_), .O(z20));
  inv1   g147(.a(new_n150_), .O(z19));
endmodule


