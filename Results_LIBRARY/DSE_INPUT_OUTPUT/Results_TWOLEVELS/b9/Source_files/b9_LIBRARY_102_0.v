// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n194_, new_n196_,
    new_n197_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n208_, new_n209_, new_n210_, new_n211_,
    new_n212_, new_n213_, new_n214_, new_n215_, new_n216_, new_n217_;
  inv1   g000(.a(x39), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x27), .c(new_n66_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n64_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n66_), .d(new_n73_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n72_), .O(z00));
  nor2   g016(.a(x40), .b(x39), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n79_), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  aoi21  g027(.a(new_n84_), .b(x04), .c(new_n89_), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n91_), .c(x04), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n90_), .c(x40), .d(x39), .O(z02));
  nand2  g032(.a(new_n68_), .b(x27), .O(new_n95_));
  aoi21  g033(.a(new_n67_), .b(new_n92_), .c(new_n79_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n95_), .c(new_n98_), .O(z04));
  nor2   g037(.a(new_n83_), .b(new_n92_), .O(new_n100_));
  inv1   g038(.a(new_n100_), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n65_), .c(new_n67_), .O(z05));
  oai21  g040(.a(new_n100_), .b(x37), .c(new_n65_), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n66_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(new_n111_));
  oai22  g049(.a(new_n111_), .b(x14), .c(new_n108_), .d(x31), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n65_), .c(x03), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z07));
  nand2  g052(.a(x40), .b(x39), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  inv1   g057(.a(x05), .O(new_n120_));
  nand2  g058(.a(new_n115_), .b(x07), .O(new_n121_));
  oai21  g059(.a(new_n115_), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n85_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n81_), .b(new_n80_), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(x40), .c(x39), .d(x05), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n73_), .O(new_n127_));
  nor2   g065(.a(new_n67_), .b(new_n92_), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(x06), .c(new_n79_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(z10));
  nand2  g068(.a(new_n65_), .b(new_n92_), .O(new_n131_));
  nand3  g069(.a(x35), .b(new_n83_), .c(new_n73_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x39), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n131_), .c(x08), .O(new_n134_));
  inv1   g072(.a(x35), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(x28), .O(new_n136_));
  nand2  g074(.a(x40), .b(x29), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x39), .O(new_n138_));
  nand2  g076(.a(x40), .b(new_n63_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi22  g078(.a(new_n140_), .b(new_n136_), .c(new_n139_), .d(new_n138_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n134_), .c(new_n80_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(x09), .O(z11));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  nor2   g082(.a(new_n64_), .b(x27), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n74_), .O(new_n146_));
  nand2  g084(.a(x39), .b(new_n83_), .O(new_n147_));
  nand2  g085(.a(x40), .b(new_n135_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n147_), .c(new_n73_), .O(new_n149_));
  nand2  g087(.a(new_n64_), .b(x39), .O(new_n150_));
  aoi22  g088(.a(new_n150_), .b(new_n139_), .c(x35), .d(x28), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(new_n67_), .O(new_n152_));
  aoi21  g090(.a(x40), .b(x27), .c(x39), .O(new_n153_));
  oai21  g091(.a(new_n64_), .b(new_n73_), .c(new_n150_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n92_), .c(new_n153_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n152_), .c(new_n146_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x28), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x40), .c(new_n157_), .d(new_n73_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand4  g100(.a(new_n69_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n160_), .c(new_n63_), .O(new_n164_));
  nand4  g102(.a(new_n69_), .b(x40), .c(x20), .d(new_n162_), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(x18), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n164_), .c(x27), .O(new_n167_));
  aoi21  g105(.a(new_n81_), .b(new_n80_), .c(new_n64_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(x39), .c(new_n157_), .d(new_n73_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n167_), .O(z13));
  nand3  g108(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n171_));
  nand2  g109(.a(x39), .b(new_n82_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n148_), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n174_));
  nand3  g112(.a(x39), .b(new_n157_), .c(new_n73_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x40), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n174_), .c(new_n150_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n171_), .O(new_n178_));
  oai21  g116(.a(x37), .b(x35), .c(x27), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n175_), .O(new_n180_));
  oai21  g118(.a(new_n63_), .b(x13), .c(new_n83_), .O(new_n181_));
  nand4  g119(.a(new_n135_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n67_), .O(new_n184_));
  nand3  g122(.a(new_n81_), .b(new_n80_), .c(new_n92_), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n184_), .c(new_n180_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(x40), .O(new_n187_));
  oai21  g125(.a(new_n64_), .b(x04), .c(new_n83_), .O(new_n188_));
  nand2  g126(.a(new_n64_), .b(new_n135_), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n188_), .c(x37), .O(new_n190_));
  nor2   g128(.a(x40), .b(x27), .O(new_n191_));
  oai21  g129(.a(new_n191_), .b(new_n190_), .c(x39), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n187_), .c(new_n178_), .O(z14));
  nand4  g131(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n65_), .O(z15));
  inv1   g133(.a(x23), .O(new_n196_));
  nand4  g134(.a(new_n65_), .b(new_n196_), .c(x22), .d(x01), .O(new_n197_));
  inv1   g135(.a(new_n197_), .O(z16));
  inv1   g136(.a(x24), .O(new_n199_));
  nand4  g137(.a(new_n199_), .b(x23), .c(x22), .d(x01), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(new_n65_), .O(z17));
  inv1   g139(.a(x29), .O(new_n202_));
  oai22  g140(.a(new_n115_), .b(new_n202_), .c(new_n84_), .d(x27), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(x08), .O(new_n204_));
  nor2   g142(.a(new_n92_), .b(x04), .O(new_n205_));
  aoi21  g143(.a(new_n205_), .b(new_n136_), .c(x09), .O(new_n206_));
  nand4  g144(.a(new_n206_), .b(new_n204_), .c(new_n65_), .d(new_n80_), .O(z18));
  inv1   g145(.a(x09), .O(new_n208_));
  oai22  g146(.a(new_n63_), .b(x35), .c(new_n83_), .d(x27), .O(new_n209_));
  nand2  g147(.a(new_n209_), .b(new_n137_), .O(new_n210_));
  oai21  g148(.a(new_n63_), .b(x08), .c(new_n139_), .O(new_n211_));
  nand2  g149(.a(new_n211_), .b(new_n135_), .O(new_n212_));
  nor2   g150(.a(new_n79_), .b(x08), .O(new_n213_));
  nor2   g151(.a(x39), .b(new_n83_), .O(new_n214_));
  oai21  g152(.a(new_n214_), .b(new_n213_), .c(new_n92_), .O(new_n215_));
  nand3  g153(.a(new_n215_), .b(new_n212_), .c(new_n210_), .O(new_n216_));
  nand3  g154(.a(new_n216_), .b(new_n80_), .c(new_n208_), .O(new_n217_));
  nand2  g155(.a(new_n217_), .b(new_n65_), .O(z20));
  nor2   g156(.a(new_n142_), .b(x09), .O(z19));
endmodule


