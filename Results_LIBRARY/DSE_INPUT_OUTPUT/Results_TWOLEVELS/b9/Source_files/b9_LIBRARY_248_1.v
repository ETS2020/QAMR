// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n173_, new_n174_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x16), .O(new_n78_));
  nor2   g016(.a(new_n73_), .b(x30), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n77_), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x27), .c(x32), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x40), .c(new_n82_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n84_), .c(x04), .O(new_n93_));
  nor2   g031(.a(new_n79_), .b(new_n72_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n93_), .c(new_n91_), .d(x40), .O(z02));
  oai21  g033(.a(new_n66_), .b(new_n83_), .c(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n64_), .c(new_n97_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n79_), .O(z03));
  aoi21  g038(.a(new_n98_), .b(new_n64_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n96_), .c(new_n79_), .O(z04));
  inv1   g040(.a(new_n79_), .O(new_n103_));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n103_), .c(new_n98_), .O(z05));
  aoi21  g043(.a(new_n103_), .b(new_n64_), .c(new_n83_), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(x37), .c(new_n103_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n63_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x14), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n110_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  oai21  g054(.a(new_n112_), .b(x31), .c(new_n116_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n103_), .c(x03), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z07));
  nand3  g057(.a(x40), .b(x39), .c(x30), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n103_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  nand3  g062(.a(new_n103_), .b(x37), .c(x06), .O(new_n125_));
  oai21  g063(.a(x39), .b(new_n82_), .c(x40), .O(new_n126_));
  nand4  g064(.a(new_n126_), .b(new_n85_), .c(x07), .d(new_n65_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x27), .O(new_n129_));
  nand4  g067(.a(new_n74_), .b(x30), .c(x05), .d(new_n65_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nand2  g070(.a(x27), .b(x04), .O(new_n133_));
  oai21  g071(.a(x27), .b(x08), .c(new_n133_), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(new_n84_), .c(new_n132_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n73_), .c(x30), .O(z11));
  nand3  g074(.a(new_n98_), .b(new_n67_), .c(new_n66_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(x27), .c(x10), .O(new_n138_));
  nor2   g076(.a(new_n70_), .b(new_n64_), .O(new_n139_));
  aoi21  g077(.a(x39), .b(new_n65_), .c(new_n139_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n138_), .c(x30), .O(new_n141_));
  inv1   g079(.a(new_n139_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n73_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(z12));
  nand2  g082(.a(new_n74_), .b(x30), .O(new_n145_));
  inv1   g083(.a(x19), .O(new_n146_));
  nor2   g084(.a(new_n79_), .b(new_n70_), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(x27), .c(x20), .d(new_n146_), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n65_), .O(new_n150_));
  oai22  g088(.a(new_n150_), .b(new_n145_), .c(new_n148_), .d(x18), .O(z13));
  inv1   g089(.a(new_n74_), .O(new_n152_));
  nand2  g090(.a(x20), .b(new_n146_), .O(new_n153_));
  oai22  g091(.a(new_n153_), .b(x18), .c(new_n150_), .d(new_n152_), .O(new_n154_));
  oai21  g092(.a(new_n72_), .b(x04), .c(x30), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(x40), .c(new_n149_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n142_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n154_), .c(new_n103_), .O(z14));
  nand4  g096(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n103_), .O(z15));
  inv1   g098(.a(x23), .O(new_n161_));
  nand4  g099(.a(new_n103_), .b(new_n161_), .c(x22), .d(x01), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z16));
  inv1   g101(.a(x01), .O(new_n164_));
  inv1   g102(.a(x24), .O(new_n165_));
  nand4  g103(.a(new_n103_), .b(new_n165_), .c(x23), .d(x22), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(new_n164_), .O(z17));
  nand2  g105(.a(x27), .b(new_n65_), .O(new_n168_));
  nand2  g106(.a(new_n64_), .b(x08), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n168_), .c(new_n66_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n83_), .c(x09), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(x40), .c(new_n82_), .O(z18));
  aoi21  g110(.a(new_n83_), .b(x08), .c(x27), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n66_), .c(new_n132_), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n73_), .c(x30), .O(z20));
  aoi21  g113(.a(new_n135_), .b(new_n73_), .c(x30), .O(z19));
endmodule


