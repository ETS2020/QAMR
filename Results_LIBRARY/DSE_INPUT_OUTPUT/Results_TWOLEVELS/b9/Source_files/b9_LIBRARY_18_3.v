// Benchmark "FAU" written by ABC on Tue Aug 18 17:25:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n109_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n74_), .c(new_n63_), .O(new_n80_));
  nand2  g018(.a(x40), .b(x05), .O(new_n81_));
  and2   g019(.a(new_n81_), .b(x16), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(z00));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n67_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n85_), .c(new_n84_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n81_), .c(x04), .O(z01));
  inv1   g029(.a(x05), .O(new_n92_));
  inv1   g030(.a(x02), .O(new_n93_));
  nand2  g031(.a(x29), .b(x08), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g033(.a(new_n87_), .b(x04), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n95_), .c(new_n76_), .O(new_n97_));
  inv1   g035(.a(x08), .O(new_n98_));
  nand3  g036(.a(new_n64_), .b(new_n98_), .c(x04), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x39), .O(new_n100_));
  oai21  g038(.a(new_n100_), .b(new_n97_), .c(new_n92_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x40), .O(z02));
  nand2  g040(.a(new_n71_), .b(x27), .O(new_n103_));
  nand2  g041(.a(new_n70_), .b(new_n64_), .O(new_n104_));
  nand4  g042(.a(new_n104_), .b(new_n103_), .c(new_n81_), .d(x21), .O(z03));
  inv1   g043(.a(new_n81_), .O(new_n106_));
  aoi21  g044(.a(new_n70_), .b(new_n64_), .c(x21), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n103_), .c(new_n106_), .O(z04));
  nand2  g046(.a(x28), .b(x27), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n81_), .c(new_n70_), .O(z05));
  aoi21  g048(.a(x40), .b(x05), .c(x27), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n86_), .c(new_n70_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n81_), .O(z06));
  inv1   g051(.a(x33), .O(new_n114_));
  nand2  g052(.a(x17), .b(new_n63_), .O(new_n115_));
  inv1   g053(.a(x00), .O(new_n116_));
  oai21  g054(.a(x25), .b(new_n116_), .c(x38), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(new_n118_));
  inv1   g056(.a(x14), .O(new_n119_));
  inv1   g057(.a(x25), .O(new_n120_));
  nand3  g058(.a(x38), .b(new_n120_), .c(new_n116_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  oai21  g060(.a(new_n118_), .b(x31), .c(new_n122_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x03), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n81_), .O(z07));
  oai21  g063(.a(x39), .b(x05), .c(x40), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(z08));
  nand4  g065(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n81_), .O(z09));
  nand3  g067(.a(new_n81_), .b(x37), .c(x06), .O(new_n130_));
  nand4  g068(.a(new_n126_), .b(new_n88_), .c(x07), .d(new_n65_), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n130_), .c(new_n64_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  nand3  g071(.a(x35), .b(new_n86_), .c(new_n65_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n92_), .c(new_n111_), .O(new_n135_));
  aoi21  g073(.a(x39), .b(x29), .c(x05), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n86_), .O(new_n138_));
  oai21  g076(.a(new_n136_), .b(new_n76_), .c(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n135_), .b(x08), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n84_), .c(new_n133_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z11));
  nand3  g080(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n143_));
  nand2  g081(.a(x40), .b(new_n64_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n143_), .c(x10), .O(new_n145_));
  aoi22  g083(.a(new_n72_), .b(x27), .c(x39), .d(new_n65_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(new_n92_), .O(new_n147_));
  oai21  g085(.a(new_n73_), .b(new_n64_), .c(new_n76_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x28), .c(new_n76_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x39), .c(new_n150_), .d(new_n65_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  nand4  g093(.a(new_n72_), .b(x20), .c(new_n155_), .d(new_n154_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand2  g096(.a(new_n85_), .b(new_n84_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n150_), .d(new_n65_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n92_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x40), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n158_), .O(z13));
  oai21  g101(.a(x27), .b(new_n150_), .c(new_n92_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x40), .O(new_n165_));
  nand3  g103(.a(x20), .b(new_n155_), .c(new_n154_), .O(new_n166_));
  nand4  g104(.a(new_n151_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(new_n77_), .c(new_n150_), .d(new_n65_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n166_), .O(new_n169_));
  oai21  g107(.a(new_n75_), .b(x04), .c(new_n92_), .O(new_n170_));
  nor2   g108(.a(new_n76_), .b(x13), .O(new_n171_));
  aoi22  g109(.a(new_n171_), .b(new_n170_), .c(x35), .d(x28), .O(new_n172_));
  nand4  g110(.a(new_n66_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n172_), .c(new_n70_), .O(new_n175_));
  nand3  g113(.a(new_n170_), .b(new_n159_), .c(x40), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n64_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n175_), .c(new_n169_), .d(new_n165_), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n81_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n81_), .O(z16));
  inv1   g121(.a(x01), .O(new_n184_));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n81_), .b(new_n185_), .c(x23), .d(x22), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z17));
  nand2  g125(.a(new_n77_), .b(x29), .O(new_n188_));
  oai21  g126(.a(new_n87_), .b(x27), .c(new_n188_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x08), .O(new_n190_));
  inv1   g128(.a(new_n87_), .O(new_n191_));
  nor2   g129(.a(new_n64_), .b(x04), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n191_), .c(x30), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(new_n190_), .c(new_n81_), .d(new_n133_), .O(z18));
  nor2   g132(.a(new_n136_), .b(new_n76_), .O(new_n195_));
  aoi21  g133(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n196_));
  aoi21  g134(.a(x35), .b(x27), .c(x05), .O(new_n197_));
  nor2   g135(.a(x40), .b(x27), .O(new_n198_));
  oai21  g136(.a(new_n198_), .b(new_n197_), .c(new_n98_), .O(new_n199_));
  oai21  g137(.a(new_n196_), .b(new_n195_), .c(new_n199_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n84_), .c(new_n133_), .O(new_n201_));
  inv1   g139(.a(new_n201_), .O(z20));
  inv1   g140(.a(new_n141_), .O(z19));
endmodule


