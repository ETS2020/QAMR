// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n110_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(x40), .b(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n79_), .c(x16), .O(z00));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x27), .c(x30), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x40), .c(new_n80_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n84_), .c(x04), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n89_), .c(x32), .O(new_n93_));
  inv1   g031(.a(x40), .O(new_n94_));
  inv1   g032(.a(x18), .O(new_n95_));
  inv1   g033(.a(x19), .O(new_n96_));
  nand4  g034(.a(new_n72_), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x35), .c(new_n64_), .O(new_n98_));
  aoi21  g036(.a(new_n70_), .b(new_n83_), .c(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n100_));
  oai21  g038(.a(new_n100_), .b(new_n98_), .c(new_n94_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n93_), .O(z02));
  inv1   g040(.a(new_n81_), .O(new_n103_));
  nand2  g041(.a(new_n71_), .b(x27), .O(new_n104_));
  inv1   g042(.a(x21), .O(new_n105_));
  aoi21  g043(.a(new_n70_), .b(new_n64_), .c(new_n105_), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(z03));
  nand2  g045(.a(new_n70_), .b(new_n64_), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(new_n104_), .c(new_n81_), .d(new_n105_), .O(z04));
  nand2  g047(.a(x28), .b(x27), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n81_), .c(new_n70_), .O(z05));
  inv1   g049(.a(z05), .O(z06));
  inv1   g050(.a(x33), .O(new_n113_));
  nand2  g051(.a(x17), .b(new_n63_), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  oai21  g053(.a(x25), .b(new_n115_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n115_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  oai22  g058(.a(new_n120_), .b(x14), .c(new_n117_), .d(x31), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n81_), .c(x03), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z07));
  nand3  g061(.a(x40), .b(x39), .c(x32), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(z08));
  inv1   g063(.a(x11), .O(new_n126_));
  nand4  g064(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n126_), .O(z09));
  nand4  g066(.a(new_n85_), .b(new_n75_), .c(x27), .d(x07), .O(new_n129_));
  nand2  g067(.a(new_n76_), .b(x05), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n65_), .O(new_n132_));
  nand3  g070(.a(x37), .b(x27), .c(x06), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n81_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  inv1   g073(.a(x30), .O(new_n136_));
  nand3  g074(.a(x35), .b(new_n83_), .c(new_n65_), .O(new_n137_));
  aoi22  g075(.a(new_n137_), .b(x32), .c(new_n81_), .d(new_n64_), .O(new_n138_));
  nand2  g076(.a(x39), .b(x29), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x32), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x40), .O(new_n141_));
  nand2  g079(.a(x27), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(x35), .c(new_n83_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  oai21  g082(.a(new_n138_), .b(x08), .c(new_n144_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n136_), .c(new_n135_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n81_), .O(z11));
  nand3  g085(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x27), .c(x10), .O(new_n149_));
  aoi22  g087(.a(new_n72_), .b(x27), .c(x39), .d(new_n65_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n149_), .c(x32), .O(new_n151_));
  oai21  g089(.a(new_n73_), .b(new_n64_), .c(new_n94_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n151_), .O(z12));
  nor2   g091(.a(new_n103_), .b(new_n73_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x27), .c(x20), .d(new_n96_), .O(new_n155_));
  inv1   g093(.a(x13), .O(new_n156_));
  nand4  g094(.a(new_n76_), .b(x32), .c(new_n156_), .d(new_n65_), .O(new_n157_));
  oai21  g095(.a(new_n155_), .b(x18), .c(new_n157_), .O(z13));
  nand3  g096(.a(x39), .b(new_n156_), .c(new_n65_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x32), .c(new_n94_), .O(new_n160_));
  nand4  g098(.a(x27), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n72_), .c(new_n160_), .O(z14));
  nand4  g101(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n81_), .O(z15));
  inv1   g103(.a(x23), .O(new_n166_));
  nand4  g104(.a(new_n81_), .b(new_n166_), .c(x22), .d(x01), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z16));
  inv1   g106(.a(x24), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x23), .c(x22), .d(x01), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n81_), .O(z17));
  inv1   g109(.a(x29), .O(new_n172_));
  oai22  g110(.a(new_n84_), .b(x27), .c(new_n75_), .d(new_n172_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x08), .O(new_n174_));
  nor2   g112(.a(new_n66_), .b(x28), .O(new_n175_));
  nor2   g113(.a(new_n64_), .b(x04), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n175_), .c(x09), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n174_), .c(new_n81_), .d(new_n136_), .O(z18));
  inv1   g116(.a(new_n146_), .O(z19));
  nor2   g117(.a(new_n83_), .b(x27), .O(new_n180_));
  nor2   g118(.a(x35), .b(new_n80_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n180_), .c(new_n139_), .O(new_n182_));
  inv1   g120(.a(x08), .O(new_n183_));
  aoi21  g121(.a(x40), .b(new_n80_), .c(x27), .O(new_n184_));
  oai21  g122(.a(new_n181_), .b(new_n184_), .c(new_n183_), .O(new_n185_));
  oai21  g123(.a(new_n180_), .b(new_n66_), .c(new_n94_), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n185_), .c(new_n182_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n136_), .c(new_n135_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n81_), .O(z20));
endmodule


