// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n175_, new_n176_, new_n178_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand4  g009(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(new_n71_), .c(new_n63_), .O(new_n74_));
  inv1   g012(.a(x26), .O(new_n75_));
  nand2  g013(.a(x40), .b(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n74_), .c(x16), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n67_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n76_), .c(x04), .O(z01));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n81_), .c(x04), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x39), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n86_), .c(x26), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x40), .O(z02));
  oai21  g029(.a(new_n66_), .b(new_n80_), .c(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n76_), .d(x21), .O(z03));
  inv1   g033(.a(new_n76_), .O(new_n96_));
  aoi21  g034(.a(new_n93_), .b(new_n64_), .c(x21), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n92_), .c(new_n96_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n96_), .O(z05));
  aoi21  g038(.a(new_n76_), .b(new_n64_), .c(new_n80_), .O(new_n101_));
  oai21  g039(.a(new_n101_), .b(x37), .c(new_n76_), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n63_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n76_), .O(z07));
  inv1   g051(.a(x39), .O(new_n114_));
  inv1   g052(.a(x40), .O(new_n115_));
  aoi21  g053(.a(new_n114_), .b(x26), .c(new_n115_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z09));
  inv1   g056(.a(x07), .O(new_n119_));
  nand4  g057(.a(x40), .b(x39), .c(x26), .d(x05), .O(new_n120_));
  oai21  g058(.a(z08), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n82_), .c(new_n65_), .O(new_n122_));
  nand3  g060(.a(new_n76_), .b(x37), .c(x06), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x27), .O(new_n125_));
  nand2  g063(.a(new_n79_), .b(new_n78_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x40), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n114_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x26), .c(x05), .d(new_n65_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n125_), .O(z10));
  nand2  g068(.a(new_n76_), .b(new_n64_), .O(new_n131_));
  nand3  g069(.a(x35), .b(new_n80_), .c(new_n65_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x26), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n131_), .c(x08), .O(new_n134_));
  inv1   g072(.a(new_n81_), .O(new_n135_));
  nand2  g073(.a(x39), .b(x29), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x26), .c(new_n115_), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n135_), .c(new_n137_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n134_), .c(new_n78_), .O(new_n140_));
  nor2   g078(.a(new_n140_), .b(x09), .O(z11));
  nand3  g079(.a(new_n93_), .b(new_n67_), .c(new_n66_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  nor2   g081(.a(new_n70_), .b(new_n64_), .O(new_n144_));
  aoi21  g082(.a(x39), .b(new_n65_), .c(new_n144_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n143_), .c(x26), .O(new_n146_));
  oai21  g084(.a(new_n70_), .b(new_n64_), .c(new_n115_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x28), .c(new_n115_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n149_), .d(new_n65_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  inv1   g092(.a(new_n70_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n152_), .c(new_n64_), .O(new_n157_));
  nand4  g095(.a(new_n126_), .b(x39), .c(new_n149_), .d(new_n65_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x26), .c(new_n115_), .O(new_n159_));
  or2    g097(.a(new_n159_), .b(new_n157_), .O(z13));
  nand3  g098(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n161_));
  nand4  g099(.a(new_n150_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n162_));
  nand3  g100(.a(x39), .b(new_n149_), .c(new_n65_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n162_), .c(new_n75_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n115_), .c(new_n161_), .O(new_n166_));
  nand3  g104(.a(new_n93_), .b(new_n66_), .c(x28), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x27), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n79_), .c(new_n78_), .O(new_n169_));
  oai21  g107(.a(new_n164_), .b(new_n144_), .c(new_n169_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x26), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n166_), .c(new_n147_), .O(z14));
  nand4  g110(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n76_), .O(z15));
  inv1   g112(.a(x23), .O(new_n175_));
  nand4  g113(.a(new_n76_), .b(new_n175_), .c(x22), .d(x01), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(z16));
  inv1   g115(.a(x01), .O(new_n178_));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n76_), .b(new_n179_), .c(x23), .d(x22), .O(new_n180_));
  nor2   g118(.a(new_n180_), .b(new_n178_), .O(z17));
  nand3  g119(.a(x40), .b(x39), .c(x29), .O(new_n182_));
  oai21  g120(.a(new_n81_), .b(x27), .c(new_n182_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(x08), .O(new_n184_));
  nor2   g122(.a(new_n96_), .b(x09), .O(new_n185_));
  nor2   g123(.a(new_n64_), .b(x04), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n135_), .c(x30), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n185_), .c(new_n184_), .O(z18));
  oai21  g126(.a(new_n140_), .b(x09), .c(new_n76_), .O(z19));
  aoi21  g127(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n137_), .O(new_n191_));
  oai21  g129(.a(new_n66_), .b(new_n64_), .c(x26), .O(new_n192_));
  nand2  g130(.a(new_n115_), .b(new_n64_), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(new_n192_), .c(x08), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n191_), .c(new_n78_), .O(new_n195_));
  nor2   g133(.a(new_n195_), .b(x09), .O(z20));
endmodule


