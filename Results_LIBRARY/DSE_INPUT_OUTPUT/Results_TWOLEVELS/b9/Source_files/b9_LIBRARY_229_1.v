// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n97_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_;
  oai21  g000(.a(x36), .b(x35), .c(x40), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  nand2  g003(.a(x40), .b(x10), .O(new_n66_));
  nor2   g004(.a(new_n66_), .b(x04), .O(new_n67_));
  aoi21  g005(.a(new_n65_), .b(x27), .c(new_n67_), .O(new_n68_));
  inv1   g006(.a(x39), .O(new_n69_));
  nand2  g007(.a(x40), .b(new_n69_), .O(new_n70_));
  and2   g008(.a(new_n70_), .b(x16), .O(new_n71_));
  oai21  g009(.a(new_n68_), .b(x15), .c(new_n71_), .O(z00));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n70_), .c(x04), .O(z01));
  inv1   g018(.a(x02), .O(new_n81_));
  nand2  g019(.a(x29), .b(x08), .O(new_n82_));
  nor2   g020(.a(new_n75_), .b(x28), .O(new_n83_));
  oai21  g021(.a(x27), .b(x08), .c(new_n83_), .O(new_n84_));
  aoi22  g022(.a(new_n84_), .b(x04), .c(new_n82_), .d(new_n81_), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n69_), .c(x40), .O(z02));
  inv1   g024(.a(x27), .O(new_n87_));
  inv1   g025(.a(x37), .O(new_n88_));
  nand2  g026(.a(x40), .b(new_n69_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand3  g029(.a(new_n70_), .b(new_n91_), .c(x27), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n70_), .d(x21), .O(z03));
  inv1   g031(.a(new_n70_), .O(new_n94_));
  aoi21  g032(.a(new_n91_), .b(x27), .c(x21), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n94_), .c(new_n90_), .O(z04));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  aoi22  g035(.a(new_n97_), .b(new_n88_), .c(x40), .d(new_n69_), .O(z05));
  inv1   g036(.a(x40), .O(new_n99_));
  inv1   g037(.a(new_n97_), .O(new_n100_));
  oai22  g038(.a(new_n100_), .b(x37), .c(new_n99_), .d(x39), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n102_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n70_), .O(z07));
  nand2  g051(.a(x40), .b(x39), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n70_), .O(z09));
  inv1   g055(.a(x04), .O(new_n118_));
  inv1   g056(.a(x07), .O(new_n119_));
  nand2  g057(.a(x40), .b(x05), .O(new_n120_));
  oai21  g058(.a(x40), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n77_), .c(x27), .O(new_n122_));
  oai21  g060(.a(x32), .b(x30), .c(x40), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x05), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n118_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n70_), .O(z10));
  inv1   g067(.a(x09), .O(new_n130_));
  oai21  g068(.a(new_n69_), .b(x29), .c(x40), .O(new_n131_));
  inv1   g069(.a(x28), .O(new_n132_));
  nand2  g070(.a(x27), .b(x04), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x35), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand3  g073(.a(x35), .b(new_n132_), .c(new_n118_), .O(new_n136_));
  aoi22  g074(.a(new_n136_), .b(x39), .c(new_n70_), .d(new_n87_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(x08), .c(new_n135_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n73_), .c(new_n130_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  nor2   g078(.a(new_n64_), .b(new_n87_), .O(new_n141_));
  nand3  g079(.a(new_n88_), .b(new_n76_), .c(new_n75_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  nor2   g081(.a(new_n141_), .b(new_n118_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x39), .O(new_n145_));
  oai21  g083(.a(new_n141_), .b(x40), .c(new_n145_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x40), .c(new_n147_), .d(new_n118_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand2  g090(.a(new_n91_), .b(new_n88_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n150_), .c(new_n69_), .O(new_n155_));
  nand4  g093(.a(new_n153_), .b(new_n99_), .c(x20), .d(new_n152_), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(x18), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n155_), .c(x27), .O(new_n158_));
  nand4  g096(.a(new_n124_), .b(x39), .c(new_n147_), .d(new_n118_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  oai21  g098(.a(x27), .b(new_n147_), .c(x39), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x40), .O(new_n162_));
  nor2   g100(.a(x32), .b(x30), .O(new_n163_));
  oai21  g101(.a(new_n69_), .b(new_n118_), .c(x40), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n163_), .c(new_n87_), .O(new_n165_));
  oai21  g103(.a(new_n164_), .b(x13), .c(new_n91_), .O(new_n166_));
  nand4  g104(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(x28), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n88_), .O(new_n169_));
  nand3  g107(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n170_));
  nand4  g108(.a(new_n148_), .b(new_n74_), .c(new_n73_), .d(x28), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(x40), .c(new_n147_), .d(new_n118_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n170_), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n169_), .c(new_n165_), .d(new_n162_), .O(z14));
  nand4  g112(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n70_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand4  g115(.a(new_n70_), .b(new_n177_), .c(x22), .d(x01), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(x23), .c(x22), .d(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n70_), .O(z17));
  nand2  g120(.a(x27), .b(new_n118_), .O(new_n183_));
  nand2  g121(.a(new_n87_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n75_), .O(new_n185_));
  nand2  g123(.a(new_n73_), .b(new_n130_), .O(new_n186_));
  aoi21  g124(.a(new_n185_), .b(new_n132_), .c(new_n186_), .O(new_n187_));
  oai22  g125(.a(new_n187_), .b(new_n94_), .c(new_n114_), .d(new_n82_), .O(z18));
  nand2  g126(.a(new_n139_), .b(new_n70_), .O(z19));
  aoi21  g127(.a(x40), .b(new_n69_), .c(x08), .O(new_n190_));
  aoi21  g128(.a(x40), .b(x29), .c(new_n132_), .O(new_n191_));
  oai21  g129(.a(new_n191_), .b(new_n190_), .c(new_n87_), .O(new_n192_));
  nand2  g130(.a(new_n82_), .b(x39), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(x40), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n75_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n192_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n73_), .c(new_n130_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n70_), .O(z20));
endmodule


