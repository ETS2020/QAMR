// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_;
  inv1   g000(.a(x12), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n64_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n66_), .d(new_n63_), .O(new_n76_));
  oai22  g014(.a(new_n76_), .b(x04), .c(new_n72_), .d(new_n65_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  inv1   g026(.a(x29), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  nor2   g028(.a(x35), .b(x12), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(x28), .O(new_n92_));
  oai21  g030(.a(x27), .b(x08), .c(new_n92_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x04), .O(new_n94_));
  inv1   g032(.a(x39), .O(new_n95_));
  nor2   g033(.a(new_n65_), .b(new_n95_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n94_), .c(new_n90_), .d(x40), .O(z02));
  nand2  g035(.a(new_n70_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n69_), .b(new_n68_), .c(new_n99_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n98_), .c(new_n65_), .O(z03));
  aoi21  g039(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n98_), .c(new_n65_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n69_), .c(new_n65_), .O(z05));
  inv1   g043(.a(new_n65_), .O(new_n106_));
  nand3  g044(.a(new_n104_), .b(new_n106_), .c(new_n69_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n66_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x14), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n111_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  oai21  g055(.a(new_n113_), .b(x31), .c(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n106_), .c(x03), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z07));
  aoi21  g058(.a(new_n95_), .b(new_n63_), .c(new_n64_), .O(z08));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n106_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  inv1   g062(.a(x04), .O(new_n125_));
  nand2  g063(.a(x40), .b(x39), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x07), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n83_), .c(x27), .O(new_n130_));
  nand2  g068(.a(new_n79_), .b(new_n78_), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(x40), .c(x39), .d(x05), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n125_), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n106_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  aoi21  g075(.a(x40), .b(x12), .c(x27), .O(new_n138_));
  nand3  g076(.a(x35), .b(new_n81_), .c(new_n125_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n63_), .c(new_n138_), .O(new_n140_));
  aoi21  g078(.a(x39), .b(x29), .c(x12), .O(new_n141_));
  nand2  g079(.a(x27), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(x35), .c(new_n81_), .O(new_n143_));
  oai21  g081(.a(new_n141_), .b(new_n64_), .c(new_n143_), .O(new_n144_));
  oai21  g082(.a(new_n140_), .b(x08), .c(new_n144_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n78_), .c(new_n137_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n106_), .O(z11));
  nor2   g085(.a(x27), .b(x12), .O(new_n148_));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n73_), .O(new_n150_));
  nor2   g088(.a(new_n126_), .b(x04), .O(new_n151_));
  or2    g089(.a(new_n151_), .b(new_n71_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n150_), .c(new_n106_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n64_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n125_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  nand2  g097(.a(new_n70_), .b(new_n69_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nand4  g101(.a(new_n131_), .b(x39), .c(new_n154_), .d(new_n125_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n63_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x40), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n163_), .O(z13));
  inv1   g105(.a(new_n151_), .O(new_n168_));
  aoi21  g106(.a(new_n131_), .b(new_n154_), .c(x12), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n168_), .c(new_n68_), .O(new_n170_));
  nand3  g108(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n171_));
  inv1   g109(.a(new_n126_), .O(new_n172_));
  nand4  g110(.a(new_n155_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n173_));
  nor2   g111(.a(x13), .b(x04), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n173_), .c(new_n172_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n171_), .O(new_n176_));
  aoi22  g114(.a(new_n174_), .b(new_n172_), .c(x35), .d(x28), .O(new_n177_));
  inv1   g115(.a(x35), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n179_));
  inv1   g117(.a(new_n179_), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n177_), .c(new_n69_), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(new_n176_), .c(new_n170_), .d(new_n106_), .O(z14));
  nand3  g120(.a(x34), .b(x27), .c(x26), .O(new_n183_));
  aoi21  g121(.a(new_n183_), .b(new_n64_), .c(new_n63_), .O(z15));
  inv1   g122(.a(x23), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(x22), .c(x01), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n106_), .O(z16));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n188_), .b(x23), .c(x22), .d(x01), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n106_), .O(z17));
  oai22  g128(.a(new_n126_), .b(new_n89_), .c(new_n82_), .d(x27), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(x08), .O(new_n192_));
  inv1   g130(.a(new_n82_), .O(new_n193_));
  nor2   g131(.a(new_n68_), .b(x04), .O(new_n194_));
  aoi21  g132(.a(new_n194_), .b(new_n193_), .c(x30), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n192_), .c(new_n106_), .d(new_n137_), .O(z18));
  inv1   g134(.a(new_n146_), .O(z19));
  nand2  g135(.a(x39), .b(x29), .O(new_n198_));
  nor2   g136(.a(new_n81_), .b(x27), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n91_), .c(new_n198_), .O(new_n200_));
  aoi21  g138(.a(x35), .b(x27), .c(x12), .O(new_n201_));
  nor2   g139(.a(x40), .b(x27), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n201_), .c(new_n88_), .O(new_n203_));
  oai21  g141(.a(new_n199_), .b(new_n178_), .c(new_n64_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n203_), .c(new_n200_), .O(new_n205_));
  nand3  g143(.a(new_n205_), .b(new_n78_), .c(new_n137_), .O(new_n206_));
  nand2  g144(.a(new_n206_), .b(new_n106_), .O(z20));
endmodule


