// Benchmark "FAU" written by ABC on Tue Aug 18 17:26:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  nand2  g015(.a(x40), .b(new_n77_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(x16), .O(z00));
  inv1   g017(.a(new_n78_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x40), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand4  g024(.a(new_n86_), .b(new_n82_), .c(new_n77_), .d(new_n81_), .O(new_n87_));
  oai21  g025(.a(new_n80_), .b(x04), .c(new_n87_), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n84_), .c(x04), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n89_), .c(x32), .O(new_n93_));
  nand2  g031(.a(new_n77_), .b(new_n81_), .O(new_n94_));
  nand3  g032(.a(x35), .b(new_n83_), .c(x08), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n94_), .c(x27), .O(new_n96_));
  inv1   g034(.a(x09), .O(new_n97_));
  nand4  g035(.a(new_n81_), .b(new_n64_), .c(new_n97_), .d(x04), .O(new_n98_));
  oai21  g036(.a(new_n98_), .b(new_n96_), .c(new_n82_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n93_), .O(z02));
  oai21  g038(.a(new_n66_), .b(new_n83_), .c(x27), .O(new_n101_));
  inv1   g039(.a(x37), .O(new_n102_));
  nand3  g040(.a(new_n78_), .b(new_n102_), .c(new_n64_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n101_), .c(new_n78_), .d(x21), .O(z03));
  inv1   g042(.a(x21), .O(new_n105_));
  nand4  g043(.a(new_n103_), .b(new_n101_), .c(new_n78_), .d(new_n105_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n78_), .c(new_n102_), .O(z05));
  inv1   g046(.a(z05), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n63_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n78_), .O(z07));
  nand3  g058(.a(x40), .b(x39), .c(x32), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z08));
  inv1   g060(.a(x11), .O(new_n123_));
  nand4  g061(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n123_), .O(z09));
  nand4  g063(.a(new_n85_), .b(new_n72_), .c(x27), .d(x07), .O(new_n126_));
  nand2  g064(.a(new_n73_), .b(x05), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n65_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n78_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  oai21  g070(.a(new_n82_), .b(x32), .c(new_n64_), .O(new_n133_));
  oai21  g071(.a(new_n66_), .b(x04), .c(x32), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n83_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nand2  g074(.a(x39), .b(x29), .O(new_n137_));
  nor2   g075(.a(new_n64_), .b(new_n65_), .O(new_n138_));
  nand2  g076(.a(new_n66_), .b(x32), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n83_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n138_), .c(new_n137_), .O(new_n141_));
  oai21  g079(.a(new_n138_), .b(new_n84_), .c(new_n82_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n136_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n81_), .c(new_n97_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n78_), .O(z11));
  inv1   g083(.a(x10), .O(new_n146_));
  nor3   g084(.a(x37), .b(x36), .c(x35), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n64_), .c(new_n146_), .O(new_n148_));
  oai22  g086(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n78_), .O(z12));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nor2   g090(.a(new_n70_), .b(new_n64_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n154_));
  inv1   g092(.a(x13), .O(new_n155_));
  nand3  g093(.a(x39), .b(new_n155_), .c(new_n65_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x32), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x40), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n154_), .O(z13));
  nand2  g097(.a(new_n155_), .b(new_n65_), .O(new_n160_));
  nand4  g098(.a(x27), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n161_));
  oai22  g099(.a(new_n161_), .b(new_n70_), .c(new_n160_), .d(new_n72_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n78_), .O(z14));
  inv1   g101(.a(x12), .O(new_n164_));
  nor2   g102(.a(new_n124_), .b(new_n164_), .O(z15));
  inv1   g103(.a(x23), .O(new_n166_));
  nand4  g104(.a(new_n78_), .b(new_n166_), .c(x22), .d(x01), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z16));
  inv1   g106(.a(x24), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x23), .c(x22), .d(x01), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n78_), .O(z17));
  nand2  g109(.a(x27), .b(new_n65_), .O(new_n172_));
  nand2  g110(.a(new_n64_), .b(x08), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n172_), .c(new_n66_), .O(new_n174_));
  nand2  g112(.a(new_n81_), .b(new_n97_), .O(new_n175_));
  aoi21  g113(.a(new_n174_), .b(new_n83_), .c(new_n175_), .O(new_n176_));
  nand4  g114(.a(new_n73_), .b(x32), .c(x29), .d(x08), .O(new_n177_));
  oai21  g115(.a(new_n176_), .b(new_n80_), .c(new_n177_), .O(z18));
  aoi21  g116(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n179_));
  aoi21  g117(.a(new_n137_), .b(x32), .c(new_n82_), .O(new_n180_));
  nor2   g118(.a(new_n180_), .b(new_n179_), .O(new_n181_));
  aoi21  g119(.a(new_n139_), .b(new_n133_), .c(x08), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n181_), .c(new_n81_), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(x09), .O(z20));
  nand2  g122(.a(new_n144_), .b(new_n78_), .O(z19));
endmodule


