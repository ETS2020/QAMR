// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n111_, new_n114_;
  inv1   g00(.a(x12), .O(new_n35_));
  inv1   g01(.a(x18), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  aoi21  g09(.a(new_n35_), .b(x05), .c(new_n36_), .O(new_n44_));
  aoi21  g10(.a(x15), .b(new_n35_), .c(new_n36_), .O(new_n45_));
  oai22  g11(.a(new_n45_), .b(x05), .c(new_n44_), .d(x04), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  nand2  g13(.a(x18), .b(x12), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n38_), .c(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n37_), .O(new_n52_));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n52_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n49_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n52_), .O(z04));
  inv1   g27(.a(x04), .O(new_n62_));
  inv1   g28(.a(x03), .O(new_n63_));
  inv1   g29(.a(x16), .O(new_n64_));
  nand2  g30(.a(x04), .b(new_n38_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n63_), .c(new_n64_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n62_), .c(new_n52_), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n57_), .b(new_n70_), .c(x08), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n69_), .c(new_n67_), .d(new_n52_), .O(z05));
  nand2  g38(.a(new_n71_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n57_), .c(x08), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n67_), .d(new_n52_), .O(z06));
  inv1   g42(.a(new_n66_), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n70_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n68_), .c(x04), .O(new_n81_));
  aoi21  g47(.a(new_n75_), .b(x11), .c(new_n81_), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n77_), .c(new_n37_), .O(z07));
  inv1   g49(.a(new_n80_), .O(new_n84_));
  nand3  g50(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(x08), .d(x07), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n36_), .c(x12), .O(new_n87_));
  inv1   g53(.a(new_n68_), .O(new_n88_));
  nand4  g54(.a(new_n74_), .b(new_n88_), .c(new_n35_), .d(new_n79_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n87_), .c(new_n67_), .O(z08));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n49_), .b(new_n91_), .c(new_n79_), .d(new_n78_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x09), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(x08), .c(x07), .d(x06), .O(new_n94_));
  oai21  g60(.a(new_n80_), .b(new_n68_), .c(x13), .O(new_n95_));
  oai21  g61(.a(new_n94_), .b(new_n62_), .c(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n35_), .O(new_n97_));
  nand3  g63(.a(new_n36_), .b(x13), .c(x12), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n97_), .c(new_n67_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  and2   g66(.a(new_n49_), .b(new_n70_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(x08), .c(x07), .d(x06), .O(new_n102_));
  oai22  g68(.a(new_n102_), .b(new_n100_), .c(new_n49_), .d(new_n41_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n52_), .O(new_n104_));
  nand4  g70(.a(new_n49_), .b(new_n91_), .c(new_n35_), .d(new_n79_), .O(new_n105_));
  nor3   g71(.a(new_n105_), .b(x10), .c(x09), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(x08), .c(x07), .d(x06), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n104_), .c(new_n62_), .O(z10));
  nor2   g74(.a(new_n37_), .b(new_n38_), .O(z11));
  oai21  g75(.a(new_n64_), .b(x02), .c(new_n63_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(new_n52_), .c(x04), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(z12));
  nand2  g78(.a(new_n52_), .b(new_n62_), .O(z13));
  nand2  g79(.a(x17), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n52_), .O(z14));
endmodule


