// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_, new_n121_;
  inv1   g00(.a(x10), .O(new_n35_));
  nor2   g01(.a(x16), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n36_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n37_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x03), .O(new_n52_));
  nand3  g18(.a(x16), .b(new_n52_), .c(x02), .O(new_n53_));
  oai21  g19(.a(x16), .b(x10), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n48_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n46_), .O(z04));
  nand2  g28(.a(new_n46_), .b(new_n43_), .O(new_n63_));
  nand2  g29(.a(x16), .b(x04), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(x10), .c(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  inv1   g32(.a(new_n48_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(x09), .c(new_n36_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n66_), .d(new_n63_), .O(z05));
  nor2   g37(.a(x03), .b(new_n37_), .O(new_n72_));
  inv1   g38(.a(x09), .O(new_n73_));
  nand3  g39(.a(new_n58_), .b(new_n73_), .c(x08), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n35_), .c(new_n72_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(x16), .c(x04), .O(new_n77_));
  nand4  g43(.a(new_n58_), .b(new_n35_), .c(new_n73_), .d(x08), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n63_), .O(z06));
  nand2  g45(.a(x11), .b(x10), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n72_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x16), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand4  g49(.a(new_n58_), .b(new_n83_), .c(new_n73_), .d(x08), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x04), .O(new_n85_));
  aoi21  g51(.a(new_n74_), .b(x11), .c(new_n85_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(x10), .c(new_n82_), .O(z07));
  nor2   g53(.a(new_n72_), .b(new_n43_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n43_), .c(x16), .O(new_n89_));
  inv1   g55(.a(x16), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(x12), .c(x10), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  inv1   g58(.a(new_n69_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(new_n83_), .d(new_n73_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n35_), .O(new_n96_));
  nand2  g62(.a(new_n84_), .b(x12), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n96_), .c(new_n91_), .d(new_n89_), .O(z08));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n48_), .b(new_n99_), .c(new_n92_), .d(new_n83_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(x09), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(x08), .c(x07), .d(x06), .O(new_n102_));
  nand2  g68(.a(new_n94_), .b(x13), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n43_), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n35_), .O(new_n105_));
  nor2   g71(.a(new_n99_), .b(new_n35_), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n88_), .c(x16), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n105_), .c(new_n63_), .O(z09));
  nand2  g74(.a(new_n54_), .b(x00), .O(new_n109_));
  inv1   g75(.a(new_n100_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n35_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n109_), .c(x09), .O(new_n112_));
  nand4  g78(.a(new_n112_), .b(x08), .c(x07), .d(x06), .O(new_n113_));
  nand2  g79(.a(new_n67_), .b(x14), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n113_), .c(new_n43_), .O(z10));
  nor2   g81(.a(new_n36_), .b(new_n37_), .O(z11));
  nand2  g82(.a(new_n46_), .b(x03), .O(new_n117_));
  nand2  g83(.a(x16), .b(new_n37_), .O(new_n118_));
  aoi21  g84(.a(new_n118_), .b(new_n117_), .c(new_n43_), .O(z12));
  aoi21  g85(.a(new_n90_), .b(x10), .c(new_n43_), .O(z13));
  nand2  g86(.a(x17), .b(x04), .O(new_n121_));
  nand2  g87(.a(new_n121_), .b(new_n46_), .O(z14));
endmodule


