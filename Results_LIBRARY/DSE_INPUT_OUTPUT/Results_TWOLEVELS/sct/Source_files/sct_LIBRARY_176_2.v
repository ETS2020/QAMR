// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_;
  inv1   g00(.a(x17), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x14), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g08(.a(new_n36_), .O(new_n43_));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x03), .O(new_n51_));
  inv1   g17(.a(x16), .O(new_n52_));
  aoi21  g18(.a(new_n51_), .b(x02), .c(new_n52_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n36_), .c(new_n50_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z02));
  xor2a  g22(.a(x07), .b(x06), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n54_), .c(new_n36_), .d(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n60_), .b(x07), .c(x06), .O(new_n62_));
  oai21  g28(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n54_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n36_), .O(z04));
  oai21  g31(.a(x17), .b(new_n38_), .c(new_n40_), .O(new_n66_));
  nand3  g32(.a(x17), .b(x14), .c(x03), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n66_), .c(new_n47_), .O(new_n68_));
  nor2   g34(.a(x14), .b(new_n51_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n68_), .c(x16), .O(new_n70_));
  nand3  g36(.a(x08), .b(x07), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x09), .O(new_n72_));
  inv1   g38(.a(x09), .O(new_n73_));
  nand3  g39(.a(new_n61_), .b(new_n73_), .c(x08), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n72_), .c(new_n36_), .d(x04), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n70_), .O(z05));
  nor2   g43(.a(x10), .b(x09), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n61_), .c(x08), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x04), .O(new_n80_));
  aoi21  g46(.a(new_n74_), .b(x10), .c(new_n80_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n43_), .c(new_n70_), .O(z06));
  inv1   g48(.a(x10), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  inv1   g50(.a(new_n71_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n73_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x04), .O(new_n87_));
  aoi21  g53(.a(new_n79_), .b(x11), .c(new_n87_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n43_), .c(new_n70_), .O(z07));
  and2   g55(.a(new_n86_), .b(x12), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand4  g57(.a(new_n78_), .b(new_n85_), .c(new_n91_), .d(new_n84_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x04), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n90_), .c(new_n36_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n70_), .O(z08));
  nand2  g61(.a(new_n92_), .b(x13), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n91_), .c(new_n84_), .d(new_n83_), .O(new_n98_));
  or2    g64(.a(new_n98_), .b(new_n74_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n96_), .c(x04), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n36_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n70_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  aoi21  g69(.a(new_n98_), .b(new_n103_), .c(new_n53_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n73_), .c(x08), .d(x07), .O(new_n105_));
  nand2  g71(.a(new_n51_), .b(x02), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(x17), .c(x16), .d(x14), .O(new_n107_));
  oai21  g73(.a(new_n105_), .b(new_n50_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n36_), .O(z10));
  nand2  g76(.a(new_n36_), .b(new_n40_), .O(z11));
  aoi21  g77(.a(new_n35_), .b(x14), .c(new_n52_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n40_), .c(x03), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(new_n47_), .c(new_n36_), .O(z12));
  nor2   g80(.a(new_n43_), .b(new_n47_), .O(z13));
  nor2   g81(.a(new_n35_), .b(new_n47_), .O(z14));
endmodule


