// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x08), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand3  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(new_n36_), .b(x01), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(x08), .b(new_n35_), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n48_), .c(new_n46_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n43_), .O(new_n52_));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n48_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n52_), .O(z03));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(x08), .c(x01), .O(new_n57_));
  nand3  g23(.a(new_n36_), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n48_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(x16), .O(new_n62_));
  aoi21  g28(.a(x04), .b(new_n47_), .c(x03), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x07), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x01), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  aoi22  g35(.a(new_n69_), .b(x09), .c(new_n68_), .d(x08), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n65_), .O(z05));
  nand2  g37(.a(new_n67_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(x08), .c(x07), .d(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x01), .O(new_n76_));
  nand2  g42(.a(x10), .b(new_n36_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n65_), .O(z06));
  nor4   g44(.a(new_n56_), .b(x11), .c(x10), .d(x09), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n35_), .c(x08), .O(new_n80_));
  nand2  g46(.a(new_n74_), .b(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n65_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand3  g50(.a(new_n73_), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  oai21  g53(.a(new_n79_), .b(new_n83_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x01), .O(new_n89_));
  nand2  g55(.a(x12), .b(new_n36_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n65_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n48_), .b(new_n92_), .c(new_n83_), .d(new_n84_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(x10), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n66_), .c(x07), .d(x06), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n42_), .c(x01), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x08), .O(new_n97_));
  nand2  g63(.a(new_n87_), .b(x13), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n97_), .c(new_n65_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  inv1   g66(.a(x03), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x02), .c(new_n62_), .O(new_n102_));
  inv1   g68(.a(x10), .O(new_n103_));
  nand4  g69(.a(new_n92_), .b(new_n83_), .c(new_n84_), .d(new_n103_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n100_), .c(new_n102_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n66_), .c(x08), .d(x07), .O(new_n106_));
  oai22  g72(.a(new_n106_), .b(new_n46_), .c(new_n48_), .d(new_n37_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x01), .O(new_n108_));
  nand3  g74(.a(new_n102_), .b(x14), .c(new_n36_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n42_), .O(z10));
  nand2  g76(.a(new_n52_), .b(new_n47_), .O(z11));
  oai21  g77(.a(new_n62_), .b(x02), .c(new_n101_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n52_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  nand2  g80(.a(new_n52_), .b(new_n42_), .O(z13));
  inv1   g81(.a(x17), .O(new_n116_));
  nor3   g82(.a(new_n43_), .b(new_n116_), .c(new_n42_), .O(z14));
endmodule


