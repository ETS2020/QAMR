// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_;
  nor2   g00(.a(x17), .b(x13), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n35_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n36_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n57_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n51_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n36_), .O(z04));
  inv1   g29(.a(x16), .O(new_n64_));
  nand2  g30(.a(x04), .b(new_n40_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n49_), .c(new_n64_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n67_), .c(x04), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n66_), .c(new_n36_), .O(new_n71_));
  aoi21  g37(.a(x13), .b(x04), .c(x17), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(x08), .c(x07), .d(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n71_), .O(z05));
  nand3  g41(.a(new_n59_), .b(new_n67_), .c(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x04), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n66_), .c(new_n36_), .O(new_n79_));
  nor3   g45(.a(new_n72_), .b(x10), .c(x09), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(x08), .c(x07), .d(x06), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n79_), .O(z06));
  oai21  g48(.a(new_n66_), .b(new_n45_), .c(new_n36_), .O(new_n83_));
  nor2   g49(.a(x10), .b(x09), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n59_), .c(x08), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x11), .O(new_n86_));
  nor2   g52(.a(x11), .b(x10), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n69_), .c(new_n67_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n86_), .c(new_n83_), .d(new_n36_), .O(z07));
  nand2  g55(.a(new_n88_), .b(x12), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand4  g58(.a(new_n84_), .b(new_n69_), .c(new_n92_), .d(new_n91_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n90_), .c(new_n83_), .d(new_n36_), .O(z08));
  inv1   g60(.a(x13), .O(new_n95_));
  inv1   g61(.a(x17), .O(new_n96_));
  aoi21  g62(.a(new_n50_), .b(x16), .c(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n95_), .c(new_n92_), .d(new_n91_), .O(new_n98_));
  nand2  g64(.a(x13), .b(x10), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(x10), .c(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n67_), .c(x08), .d(x07), .O(new_n101_));
  nand2  g67(.a(new_n76_), .b(x13), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n48_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x04), .O(new_n104_));
  oai21  g70(.a(x12), .b(x11), .c(x13), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n83_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  aoi21  g73(.a(new_n50_), .b(x16), .c(x09), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(x08), .c(x07), .d(x06), .O(new_n109_));
  oai22  g75(.a(new_n109_), .b(new_n107_), .c(new_n51_), .d(new_n38_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n36_), .O(new_n111_));
  nor2   g77(.a(new_n98_), .b(x10), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n67_), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(new_n114_));
  nand4  g80(.a(new_n114_), .b(x08), .c(x07), .d(x06), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n111_), .c(new_n45_), .O(z10));
  nand2  g82(.a(new_n36_), .b(new_n40_), .O(z11));
  oai21  g83(.a(new_n64_), .b(x02), .c(new_n49_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n36_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  nand4  g86(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n121_));
  inv1   g87(.a(new_n121_), .O(new_n122_));
  oai21  g88(.a(new_n122_), .b(new_n76_), .c(x13), .O(new_n123_));
  aoi21  g89(.a(new_n123_), .b(new_n96_), .c(new_n45_), .O(z13));
  nor2   g90(.a(new_n96_), .b(new_n45_), .O(z14));
endmodule


