// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  nand2  g00(.a(x17), .b(x14), .O(new_n35_));
  inv1   g01(.a(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(z00));
  inv1   g07(.a(new_n35_), .O(new_n42_));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n35_), .c(new_n49_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z02));
  xor2a  g21(.a(x07), .b(x06), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n53_), .c(new_n35_), .d(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n53_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n35_), .O(z04));
  nand2  g30(.a(x17), .b(x14), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n39_), .O(new_n66_));
  inv1   g32(.a(x17), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x14), .c(x03), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n66_), .c(new_n46_), .O(new_n69_));
  nor2   g35(.a(x14), .b(new_n50_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  nand3  g37(.a(x08), .b(x07), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x09), .O(new_n73_));
  inv1   g39(.a(x09), .O(new_n74_));
  nand3  g40(.a(new_n60_), .b(new_n74_), .c(x08), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n35_), .d(x04), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n71_), .O(z05));
  nor2   g44(.a(x10), .b(x09), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n60_), .c(x08), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x04), .O(new_n81_));
  aoi21  g47(.a(new_n75_), .b(x10), .c(new_n81_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n42_), .c(new_n71_), .O(z06));
  inv1   g49(.a(x10), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(new_n72_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n74_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x04), .O(new_n88_));
  aoi21  g54(.a(new_n80_), .b(x11), .c(new_n88_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n42_), .c(new_n71_), .O(z07));
  and2   g56(.a(new_n87_), .b(x12), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand4  g58(.a(new_n79_), .b(new_n86_), .c(new_n92_), .d(new_n85_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n91_), .c(new_n35_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n71_), .O(z08));
  nand2  g62(.a(new_n93_), .b(x13), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n92_), .c(new_n85_), .d(new_n84_), .O(new_n99_));
  or2    g65(.a(new_n99_), .b(new_n75_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n97_), .c(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n35_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n71_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  aoi21  g70(.a(new_n99_), .b(new_n104_), .c(new_n52_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n74_), .c(x08), .d(x07), .O(new_n106_));
  nand2  g72(.a(new_n50_), .b(x02), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n67_), .c(x16), .d(x14), .O(new_n108_));
  oai21  g74(.a(new_n106_), .b(new_n49_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n35_), .O(z10));
  nand2  g77(.a(new_n35_), .b(new_n39_), .O(z11));
  nand3  g78(.a(new_n65_), .b(x16), .c(new_n39_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n50_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n35_), .O(z12));
  aoi21  g82(.a(x17), .b(x14), .c(new_n46_), .O(z13));
  nand3  g83(.a(x17), .b(new_n37_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


