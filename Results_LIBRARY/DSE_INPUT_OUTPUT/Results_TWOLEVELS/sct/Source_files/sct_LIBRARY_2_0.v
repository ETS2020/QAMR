// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n130_;
  inv1   g00(.a(x14), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(x14), .b(x01), .O(new_n38_));
  nand2  g04(.a(new_n36_), .b(new_n38_), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(x01), .c(new_n40_), .O(z00));
  nor2   g07(.a(x14), .b(x06), .O(new_n42_));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n37_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(x14), .c(new_n49_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand3  g18(.a(x14), .b(x07), .c(new_n49_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(new_n42_), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  inv1   g24(.a(x07), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(new_n49_), .O(new_n60_));
  nand3  g26(.a(new_n58_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n50_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n57_), .O(z04));
  nor3   g30(.a(new_n42_), .b(new_n46_), .c(x02), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x03), .c(x16), .O(new_n66_));
  oai21  g32(.a(new_n35_), .b(x09), .c(new_n49_), .O(new_n67_));
  oai21  g33(.a(new_n58_), .b(new_n59_), .c(x09), .O(new_n68_));
  nor2   g34(.a(x09), .b(new_n58_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n60_), .c(new_n46_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(z05));
  oai21  g37(.a(new_n35_), .b(x10), .c(new_n49_), .O(new_n72_));
  nand2  g38(.a(new_n69_), .b(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x08), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n60_), .c(new_n46_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n74_), .c(new_n72_), .d(new_n66_), .O(z06));
  oai21  g45(.a(new_n35_), .b(x11), .c(new_n49_), .O(new_n80_));
  nor2   g46(.a(new_n58_), .b(new_n59_), .O(new_n81_));
  nand2  g47(.a(new_n75_), .b(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x11), .O(new_n83_));
  nand2  g49(.a(new_n81_), .b(x06), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nor3   g51(.a(x11), .b(x10), .c(x09), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n85_), .c(new_n46_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n83_), .c(new_n80_), .d(new_n66_), .O(z07));
  oai21  g54(.a(new_n35_), .b(x12), .c(new_n49_), .O(new_n89_));
  nand2  g55(.a(new_n86_), .b(new_n81_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x12), .O(new_n91_));
  inv1   g57(.a(x11), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand3  g59(.a(new_n75_), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n85_), .c(new_n46_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n91_), .c(new_n89_), .d(new_n66_), .O(z08));
  nand3  g63(.a(new_n57_), .b(x16), .c(new_n37_), .O(new_n98_));
  inv1   g64(.a(x09), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n50_), .b(new_n100_), .c(new_n93_), .d(new_n92_), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(x10), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n99_), .c(x08), .d(x07), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n49_), .c(new_n98_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x04), .O(new_n105_));
  oai21  g71(.a(new_n35_), .b(x13), .c(new_n49_), .O(new_n106_));
  inv1   g72(.a(x10), .O(new_n107_));
  nand3  g73(.a(new_n93_), .b(new_n92_), .c(new_n107_), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n73_), .c(x13), .O(new_n109_));
  aoi21  g75(.a(x16), .b(x03), .c(new_n46_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(new_n109_), .c(new_n106_), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n105_), .O(z09));
  inv1   g79(.a(x00), .O(new_n114_));
  nand4  g80(.a(new_n100_), .b(new_n93_), .c(new_n92_), .d(new_n107_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g82(.a(new_n116_), .b(new_n50_), .c(new_n99_), .d(x08), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(x07), .c(x06), .O(new_n119_));
  inv1   g85(.a(new_n50_), .O(new_n120_));
  nand2  g86(.a(new_n120_), .b(x14), .O(new_n121_));
  aoi21  g87(.a(new_n121_), .b(new_n119_), .c(new_n46_), .O(z10));
  nand2  g88(.a(new_n57_), .b(new_n37_), .O(z11));
  inv1   g89(.a(x03), .O(new_n124_));
  inv1   g90(.a(x16), .O(new_n125_));
  oai21  g91(.a(new_n125_), .b(x02), .c(new_n124_), .O(new_n126_));
  nand3  g92(.a(new_n126_), .b(new_n57_), .c(x04), .O(new_n127_));
  inv1   g93(.a(new_n127_), .O(z12));
  nand2  g94(.a(new_n57_), .b(new_n46_), .O(z13));
  nand2  g95(.a(x17), .b(x04), .O(new_n130_));
  nand2  g96(.a(new_n130_), .b(new_n57_), .O(z14));
endmodule


