// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n127_;
  inv1   g00(.a(x10), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x07), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(new_n36_), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n37_), .c(x16), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nand3  g20(.a(new_n35_), .b(new_n54_), .c(x06), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(x06), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n51_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nor2   g25(.a(new_n54_), .b(new_n49_), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n51_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n50_), .O(z04));
  inv1   g30(.a(x16), .O(new_n65_));
  aoi21  g31(.a(x04), .b(new_n37_), .c(x03), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n65_), .c(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n50_), .O(new_n68_));
  nand2  g34(.a(x08), .b(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(x08), .c(x06), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n70_), .c(new_n54_), .O(new_n73_));
  nor2   g39(.a(x10), .b(new_n71_), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n54_), .c(new_n73_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n68_), .O(z05));
  inv1   g42(.a(new_n69_), .O(new_n77_));
  nor2   g43(.a(x10), .b(x09), .O(new_n78_));
  aoi22  g44(.a(new_n78_), .b(new_n77_), .c(new_n72_), .d(x10), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n54_), .c(new_n68_), .O(z06));
  oai21  g46(.a(x10), .b(new_n59_), .c(x07), .O(new_n81_));
  nand3  g47(.a(new_n71_), .b(x07), .c(x06), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n35_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x11), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  nand3  g52(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n86_), .c(new_n35_), .d(new_n71_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n85_), .c(new_n68_), .O(z07));
  inv1   g56(.a(new_n81_), .O(new_n91_));
  nor2   g57(.a(x11), .b(x09), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n60_), .c(x10), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n91_), .c(x12), .O(new_n94_));
  inv1   g60(.a(x12), .O(new_n95_));
  nand4  g61(.a(new_n88_), .b(new_n78_), .c(new_n95_), .d(new_n86_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n94_), .c(new_n68_), .O(z08));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n51_), .b(new_n98_), .c(new_n95_), .d(new_n86_), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n35_), .c(new_n71_), .d(x08), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(x07), .c(x06), .d(x04), .O(new_n103_));
  aoi21  g69(.a(new_n98_), .b(new_n35_), .c(x07), .O(new_n104_));
  inv1   g70(.a(new_n72_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n95_), .c(new_n86_), .d(new_n35_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(x13), .c(new_n104_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n103_), .c(new_n68_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  inv1   g75(.a(x03), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(x02), .c(new_n65_), .O(new_n111_));
  nand4  g77(.a(new_n98_), .b(new_n95_), .c(new_n86_), .d(new_n35_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n109_), .c(new_n111_), .O(new_n113_));
  nand4  g79(.a(new_n113_), .b(new_n71_), .c(x08), .d(x07), .O(new_n114_));
  oai22  g80(.a(new_n114_), .b(new_n49_), .c(new_n51_), .d(new_n40_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n50_), .O(z10));
  nor2   g83(.a(new_n36_), .b(new_n37_), .O(z11));
  aoi21  g84(.a(x16), .b(new_n37_), .c(x03), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(new_n120_));
  nand3  g86(.a(new_n120_), .b(new_n50_), .c(x04), .O(new_n121_));
  inv1   g87(.a(new_n121_), .O(z12));
  nor2   g88(.a(new_n54_), .b(new_n49_), .O(new_n123_));
  oai21  g89(.a(new_n123_), .b(new_n111_), .c(new_n119_), .O(new_n124_));
  nand2  g90(.a(new_n124_), .b(new_n35_), .O(new_n125_));
  aoi21  g91(.a(new_n125_), .b(new_n54_), .c(new_n46_), .O(z13));
  inv1   g92(.a(x17), .O(new_n127_));
  nor3   g93(.a(new_n36_), .b(new_n127_), .c(new_n46_), .O(z14));
endmodule


