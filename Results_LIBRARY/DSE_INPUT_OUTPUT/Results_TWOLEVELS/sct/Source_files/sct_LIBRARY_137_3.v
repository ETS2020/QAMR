// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n123_, new_n124_, new_n127_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  nand2  g04(.a(x13), .b(x01), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n39_), .O(z02));
  inv1   g16(.a(x07), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(x06), .O(new_n52_));
  nand3  g18(.a(x13), .b(x07), .c(new_n47_), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(new_n52_), .c(x01), .O(new_n54_));
  nand2  g20(.a(x07), .b(new_n47_), .O(new_n55_));
  aoi21  g21(.a(new_n55_), .b(new_n52_), .c(x13), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n54_), .c(new_n48_), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(new_n44_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nor2   g25(.a(new_n51_), .b(new_n47_), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n48_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n39_), .O(z04));
  inv1   g30(.a(x03), .O(new_n65_));
  oai21  g31(.a(new_n44_), .b(x02), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x16), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n39_), .O(new_n69_));
  nor2   g35(.a(new_n59_), .b(new_n51_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x09), .O(new_n72_));
  inv1   g38(.a(x09), .O(new_n73_));
  nand3  g39(.a(new_n60_), .b(new_n73_), .c(x08), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n72_), .c(new_n69_), .d(new_n39_), .O(z05));
  nand2  g41(.a(new_n74_), .b(x10), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n60_), .c(x08), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n76_), .c(new_n69_), .d(new_n39_), .O(z06));
  nand2  g45(.a(new_n78_), .b(x11), .O(new_n80_));
  inv1   g46(.a(x10), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  inv1   g48(.a(new_n71_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n82_), .c(new_n81_), .d(new_n73_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n80_), .c(new_n69_), .d(new_n39_), .O(z07));
  nand2  g51(.a(new_n84_), .b(x12), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n67_), .c(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n39_), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n48_), .b(new_n36_), .c(x04), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x01), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n89_), .c(new_n82_), .d(new_n81_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x09), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(x08), .c(x07), .d(x06), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n88_), .O(z08));
  inv1   g61(.a(x01), .O(new_n96_));
  nand3  g62(.a(x13), .b(new_n47_), .c(new_n96_), .O(new_n97_));
  nand3  g63(.a(new_n73_), .b(x08), .c(x06), .O(new_n98_));
  nand4  g64(.a(new_n36_), .b(new_n89_), .c(new_n82_), .d(new_n81_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n48_), .c(x07), .O(new_n101_));
  nand3  g67(.a(new_n39_), .b(x16), .c(new_n35_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x04), .O(new_n104_));
  inv1   g70(.a(x16), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n65_), .c(x04), .O(new_n106_));
  nor2   g72(.a(x12), .b(x11), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n77_), .c(new_n70_), .d(new_n96_), .O(new_n108_));
  aoi22  g74(.a(new_n108_), .b(x13), .c(new_n106_), .d(new_n39_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n104_), .O(z09));
  inv1   g76(.a(x00), .O(new_n111_));
  aoi21  g77(.a(new_n65_), .b(x02), .c(new_n105_), .O(new_n112_));
  nor2   g78(.a(new_n112_), .b(x09), .O(new_n113_));
  nand4  g79(.a(new_n113_), .b(x08), .c(x07), .d(x06), .O(new_n114_));
  nand2  g80(.a(new_n112_), .b(x14), .O(new_n115_));
  oai21  g81(.a(new_n114_), .b(new_n111_), .c(new_n115_), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n39_), .O(new_n117_));
  nand4  g83(.a(new_n48_), .b(new_n36_), .c(new_n89_), .d(new_n82_), .O(new_n118_));
  nor3   g84(.a(new_n118_), .b(x10), .c(x09), .O(new_n119_));
  nand4  g85(.a(new_n119_), .b(x08), .c(x07), .d(x06), .O(new_n120_));
  aoi21  g86(.a(new_n120_), .b(new_n117_), .c(new_n44_), .O(z10));
  nor2   g87(.a(new_n40_), .b(new_n35_), .O(z11));
  oai21  g88(.a(new_n105_), .b(x02), .c(new_n65_), .O(new_n123_));
  nand3  g89(.a(new_n123_), .b(new_n39_), .c(x04), .O(new_n124_));
  inv1   g90(.a(new_n124_), .O(z12));
  nand2  g91(.a(new_n39_), .b(new_n44_), .O(z13));
  nand3  g92(.a(new_n39_), .b(x17), .c(x04), .O(new_n127_));
  inv1   g93(.a(new_n127_), .O(z14));
endmodule


