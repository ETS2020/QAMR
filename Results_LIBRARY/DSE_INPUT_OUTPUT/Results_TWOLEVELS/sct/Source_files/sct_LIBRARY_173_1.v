// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_;
  inv1   g00(.a(x06), .O(new_n35_));
  nand2  g01(.a(x15), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x15), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(new_n35_), .O(new_n44_));
  aoi21  g10(.a(new_n43_), .b(x05), .c(new_n44_), .O(new_n45_));
  nor2   g11(.a(x18), .b(x15), .O(new_n46_));
  nor2   g12(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai22  g13(.a(new_n47_), .b(x05), .c(new_n45_), .d(x04), .O(z01));
  oai21  g14(.a(x03), .b(new_n40_), .c(x16), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x04), .c(x15), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(x06), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n36_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand3  g21(.a(new_n43_), .b(x08), .c(new_n35_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n35_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n49_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  inv1   g25(.a(x03), .O(new_n60_));
  inv1   g26(.a(x04), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(x02), .c(new_n60_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n36_), .c(x16), .O(new_n63_));
  oai21  g29(.a(new_n43_), .b(x06), .c(new_n61_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nand3  g33(.a(new_n65_), .b(x08), .c(x07), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nand3  g36(.a(new_n43_), .b(x09), .c(new_n35_), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n70_), .c(new_n64_), .d(new_n63_), .O(z05));
  inv1   g38(.a(x10), .O(new_n73_));
  inv1   g39(.a(new_n68_), .O(new_n74_));
  nand3  g40(.a(new_n67_), .b(new_n73_), .c(new_n65_), .O(new_n75_));
  oai21  g41(.a(new_n74_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand3  g43(.a(new_n43_), .b(x10), .c(new_n35_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n64_), .d(new_n63_), .O(z06));
  and2   g45(.a(new_n75_), .b(x11), .O(new_n80_));
  nor4   g46(.a(new_n66_), .b(x11), .c(x10), .d(x09), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n80_), .c(x06), .O(new_n82_));
  nand3  g48(.a(new_n43_), .b(x11), .c(new_n35_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n82_), .c(new_n64_), .d(new_n63_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  nand4  g52(.a(new_n74_), .b(new_n85_), .c(new_n86_), .d(new_n73_), .O(new_n87_));
  oai21  g53(.a(new_n81_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x06), .O(new_n89_));
  nand3  g55(.a(new_n43_), .b(x12), .c(new_n35_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n64_), .d(new_n63_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n49_), .b(new_n92_), .c(new_n85_), .d(new_n86_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(x10), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n65_), .c(x08), .d(x07), .O(new_n95_));
  aoi22  g61(.a(new_n87_), .b(x13), .c(x15), .d(new_n61_), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n61_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x06), .O(new_n98_));
  oai21  g64(.a(new_n92_), .b(x06), .c(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n43_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n98_), .c(new_n63_), .O(z09));
  inv1   g67(.a(x07), .O(new_n102_));
  inv1   g68(.a(x00), .O(new_n103_));
  nand4  g69(.a(new_n92_), .b(new_n85_), .c(new_n86_), .d(new_n73_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n49_), .c(new_n65_), .d(x08), .O(new_n106_));
  oai22  g72(.a(new_n106_), .b(new_n102_), .c(new_n49_), .d(new_n38_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x06), .O(new_n108_));
  inv1   g74(.a(new_n49_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(new_n43_), .c(x14), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n108_), .c(new_n61_), .O(z10));
  nand2  g77(.a(new_n36_), .b(new_n40_), .O(z11));
  aoi21  g78(.a(x16), .b(new_n40_), .c(x03), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n36_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  oai21  g82(.a(new_n109_), .b(x06), .c(new_n113_), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n43_), .O(new_n118_));
  aoi21  g84(.a(new_n118_), .b(new_n35_), .c(new_n61_), .O(z13));
  nand2  g85(.a(x17), .b(x04), .O(new_n120_));
  nand2  g86(.a(new_n120_), .b(new_n36_), .O(z14));
endmodule


