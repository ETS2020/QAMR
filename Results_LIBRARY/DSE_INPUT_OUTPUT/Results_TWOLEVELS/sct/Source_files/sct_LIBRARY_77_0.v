// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n126_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(x02), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  nor2   g04(.a(x02), .b(x01), .O(new_n39_));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n39_), .O(z01));
  inv1   g11(.a(new_n39_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n46_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n46_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  and2   g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n55_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand2  g24(.a(x16), .b(x03), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x02), .O(new_n60_));
  inv1   g26(.a(x16), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x01), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n58_), .c(x04), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n55_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n56_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g37(.a(x03), .b(x02), .O(new_n72_));
  nand3  g38(.a(x04), .b(new_n48_), .c(x01), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x16), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n71_), .O(z05));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n56_), .c(x08), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x04), .O(new_n79_));
  aoi21  g45(.a(new_n69_), .b(x10), .c(new_n79_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n39_), .c(new_n75_), .O(z06));
  nand2  g47(.a(new_n78_), .b(x11), .O(new_n82_));
  inv1   g48(.a(x09), .O(new_n83_));
  nor2   g49(.a(x11), .b(x10), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n66_), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(new_n43_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n82_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n46_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n75_), .O(z07));
  inv1   g56(.a(x12), .O(new_n91_));
  nor2   g57(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  inv1   g58(.a(x11), .O(new_n93_));
  inv1   g59(.a(new_n66_), .O(new_n94_));
  nand4  g60(.a(new_n77_), .b(new_n94_), .c(new_n91_), .d(new_n93_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n92_), .c(new_n46_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n75_), .O(z08));
  oai21  g64(.a(new_n61_), .b(new_n43_), .c(x01), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n48_), .O(new_n100_));
  nand4  g66(.a(new_n68_), .b(x07), .c(x06), .d(x04), .O(new_n101_));
  inv1   g67(.a(x13), .O(new_n102_));
  nand4  g68(.a(new_n84_), .b(new_n61_), .c(new_n102_), .d(new_n91_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n101_), .c(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n46_), .O(new_n105_));
  inv1   g71(.a(x03), .O(new_n106_));
  nand3  g72(.a(new_n94_), .b(x04), .c(new_n106_), .O(new_n107_));
  nand4  g73(.a(new_n77_), .b(new_n102_), .c(new_n91_), .d(new_n93_), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n107_), .c(new_n59_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x02), .O(new_n110_));
  nand2  g76(.a(new_n95_), .b(x13), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n110_), .c(new_n105_), .d(new_n100_), .O(z09));
  inv1   g78(.a(x00), .O(new_n113_));
  inv1   g79(.a(x10), .O(new_n114_));
  nand4  g80(.a(new_n102_), .b(new_n91_), .c(new_n93_), .d(new_n114_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand4  g82(.a(new_n116_), .b(new_n63_), .c(new_n83_), .d(x08), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(x07), .c(x06), .O(new_n119_));
  oai21  g85(.a(x02), .b(new_n35_), .c(new_n72_), .O(new_n120_));
  nand3  g86(.a(new_n120_), .b(x16), .c(x14), .O(new_n121_));
  aoi21  g87(.a(new_n121_), .b(new_n119_), .c(new_n43_), .O(z10));
  nand2  g88(.a(new_n48_), .b(x01), .O(z11));
  oai21  g89(.a(new_n43_), .b(new_n106_), .c(new_n100_), .O(z12));
  nor2   g90(.a(new_n39_), .b(new_n43_), .O(z13));
  inv1   g91(.a(x17), .O(new_n126_));
  nor3   g92(.a(new_n39_), .b(new_n126_), .c(new_n43_), .O(z14));
endmodule


