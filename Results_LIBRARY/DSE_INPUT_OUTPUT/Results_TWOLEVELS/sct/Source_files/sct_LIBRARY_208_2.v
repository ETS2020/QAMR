// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n112_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x15), .O(new_n36_));
  oai21  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  nand2  g09(.a(new_n36_), .b(x01), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(new_n44_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(new_n46_), .O(new_n58_));
  nand3  g24(.a(new_n56_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(new_n48_), .c(new_n44_), .d(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nor2   g28(.a(new_n42_), .b(x02), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x03), .c(x16), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n58_), .b(new_n67_), .c(x08), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n44_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n66_), .c(new_n64_), .d(x04), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n58_), .c(x08), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n44_), .c(x04), .O(new_n73_));
  aoi21  g39(.a(new_n68_), .b(x10), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n64_), .O(z06));
  nand2  g41(.a(new_n72_), .b(x11), .O(new_n76_));
  nor4   g42(.a(new_n65_), .b(x11), .c(x10), .d(x09), .O(new_n77_));
  nor2   g43(.a(new_n77_), .b(new_n47_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n76_), .c(new_n64_), .d(x04), .O(z07));
  inv1   g45(.a(new_n77_), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  inv1   g48(.a(new_n65_), .O(new_n83_));
  nand4  g49(.a(new_n71_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n44_), .c(x04), .O(new_n85_));
  aoi21  g51(.a(new_n80_), .b(x12), .c(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n64_), .O(z08));
  inv1   g53(.a(x10), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand4  g55(.a(new_n48_), .b(new_n44_), .c(new_n89_), .d(new_n82_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(x11), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n88_), .c(new_n67_), .d(x08), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n57_), .O(new_n93_));
  inv1   g59(.a(x16), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x02), .O(new_n95_));
  aoi21  g61(.a(new_n93_), .b(x06), .c(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n84_), .b(x13), .O(new_n97_));
  aoi21  g63(.a(x16), .b(x03), .c(new_n47_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(x04), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand4  g66(.a(new_n89_), .b(new_n82_), .c(new_n81_), .d(new_n88_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n49_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n67_), .c(x08), .d(x07), .O(new_n103_));
  nand2  g69(.a(new_n49_), .b(x14), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n46_), .c(new_n104_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n44_), .c(x04), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(z10));
  nor2   g73(.a(new_n47_), .b(new_n35_), .O(z11));
  oai21  g74(.a(new_n95_), .b(x03), .c(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n44_), .O(z12));
  nand2  g76(.a(new_n44_), .b(new_n42_), .O(z13));
  nand2  g77(.a(x17), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n44_), .O(z14));
endmodule


