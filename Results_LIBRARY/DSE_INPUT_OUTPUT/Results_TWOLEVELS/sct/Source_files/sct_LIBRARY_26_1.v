// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n36_), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n36_), .b(x05), .c(new_n42_), .O(new_n45_));
  oai21  g11(.a(new_n44_), .b(new_n41_), .c(new_n45_), .O(z01));
  inv1   g12(.a(new_n37_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n36_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n47_), .O(z02));
  nand3  g16(.a(x07), .b(new_n36_), .c(x04), .O(new_n51_));
  oai21  g17(.a(x07), .b(new_n36_), .c(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n47_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand3  g21(.a(x08), .b(new_n36_), .c(x04), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n36_), .c(new_n56_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n48_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n47_), .O(z04));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nor2   g28(.a(x09), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x07), .c(x06), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n61_), .c(new_n48_), .d(x04), .O(z05));
  nand2  g31(.a(new_n64_), .b(x10), .O(new_n66_));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n66_), .c(new_n48_), .d(x04), .O(z06));
  nand2  g35(.a(new_n68_), .b(x11), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  inv1   g37(.a(new_n60_), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n70_), .c(new_n48_), .d(x04), .O(z07));
  oai21  g41(.a(x12), .b(new_n42_), .c(new_n36_), .O(new_n76_));
  nand2  g42(.a(x08), .b(x07), .O(new_n77_));
  nand2  g43(.a(new_n73_), .b(new_n71_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n77_), .c(x12), .O(new_n79_));
  nor2   g45(.a(x12), .b(x11), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n67_), .c(new_n72_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n79_), .c(new_n48_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n76_), .O(z08));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n48_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(x10), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n71_), .c(x08), .d(x07), .O(new_n90_));
  inv1   g56(.a(new_n48_), .O(new_n91_));
  inv1   g57(.a(x10), .O(new_n92_));
  nand4  g58(.a(new_n80_), .b(new_n63_), .c(new_n92_), .d(x07), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(x13), .c(new_n91_), .O(new_n94_));
  oai21  g60(.a(new_n90_), .b(new_n36_), .c(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  oai21  g62(.a(x13), .b(new_n42_), .c(new_n36_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nand3  g65(.a(new_n73_), .b(new_n87_), .c(new_n86_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n48_), .c(new_n71_), .d(x08), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(x07), .c(x06), .O(new_n104_));
  nand2  g70(.a(new_n91_), .b(x14), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n42_), .O(z10));
  nor2   g72(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n35_), .c(x03), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(new_n42_), .O(z12));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n47_), .O(z14));
  buf    g77(.a(x04), .O(z13));
endmodule


