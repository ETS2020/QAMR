// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n121_,
    new_n122_, new_n124_;
  inv1   g00(.a(x13), .O(new_n35_));
  nor2   g01(.a(x16), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n36_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n37_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n46_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n46_), .O(z04));
  inv1   g25(.a(x03), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x02), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n43_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n55_), .b(new_n66_), .c(x08), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(x04), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n63_), .c(x16), .O(new_n69_));
  nand2  g35(.a(new_n68_), .b(new_n35_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(z05));
  oai21  g37(.a(new_n61_), .b(new_n43_), .c(x16), .O(new_n72_));
  nand2  g38(.a(new_n67_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n55_), .c(x08), .O(new_n75_));
  aoi21  g41(.a(new_n35_), .b(new_n43_), .c(new_n36_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n75_), .c(new_n73_), .d(new_n72_), .O(z06));
  inv1   g43(.a(x10), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand2  g45(.a(new_n35_), .b(x12), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n66_), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(x08), .c(x07), .d(x06), .O(new_n83_));
  nand2  g49(.a(new_n75_), .b(x11), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n83_), .c(new_n62_), .d(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x16), .O(new_n86_));
  inv1   g52(.a(new_n64_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n79_), .c(new_n78_), .d(new_n66_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n84_), .c(x04), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n35_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n86_), .O(z07));
  nand2  g57(.a(new_n46_), .b(new_n43_), .O(z13));
  inv1   g58(.a(x12), .O(new_n93_));
  inv1   g59(.a(x16), .O(new_n94_));
  oai21  g60(.a(x13), .b(new_n43_), .c(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n93_), .c(new_n79_), .d(new_n78_), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(x09), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x08), .c(x07), .d(x06), .O(new_n98_));
  inv1   g64(.a(new_n48_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x04), .O(new_n100_));
  aoi21  g66(.a(new_n88_), .b(x12), .c(new_n36_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n100_), .c(new_n98_), .d(z13), .O(z08));
  nand4  g68(.a(new_n48_), .b(new_n35_), .c(new_n93_), .d(new_n79_), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(x10), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n66_), .c(x08), .d(x07), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n47_), .c(new_n48_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x04), .O(new_n107_));
  nand4  g73(.a(new_n74_), .b(new_n87_), .c(new_n93_), .d(new_n79_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(x16), .c(x13), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(new_n107_), .c(z13), .O(z09));
  inv1   g76(.a(new_n103_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n78_), .O(new_n112_));
  nand3  g78(.a(x16), .b(new_n60_), .c(x02), .O(new_n113_));
  oai21  g79(.a(x16), .b(x13), .c(new_n113_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(x00), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n112_), .c(x09), .O(new_n116_));
  nand4  g82(.a(new_n116_), .b(x08), .c(x07), .d(x06), .O(new_n117_));
  nand2  g83(.a(new_n99_), .b(x14), .O(new_n118_));
  aoi21  g84(.a(new_n118_), .b(new_n117_), .c(new_n43_), .O(z10));
  nand2  g85(.a(new_n46_), .b(new_n37_), .O(z11));
  nand2  g86(.a(new_n46_), .b(x03), .O(new_n121_));
  nand2  g87(.a(x16), .b(new_n37_), .O(new_n122_));
  aoi21  g88(.a(new_n122_), .b(new_n121_), .c(new_n43_), .O(z12));
  inv1   g89(.a(x17), .O(new_n124_));
  nor3   g90(.a(new_n36_), .b(new_n124_), .c(new_n43_), .O(z14));
endmodule


