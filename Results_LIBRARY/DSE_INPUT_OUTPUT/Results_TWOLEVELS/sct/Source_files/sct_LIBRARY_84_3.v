// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n109_, new_n110_, new_n113_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  nand2  g04(.a(x10), .b(x01), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n39_), .c(new_n47_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n48_), .c(new_n39_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n54_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n54_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n48_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n39_), .O(z04));
  inv1   g26(.a(x03), .O(new_n61_));
  inv1   g27(.a(x16), .O(new_n62_));
  nand2  g28(.a(x04), .b(new_n35_), .O(new_n63_));
  aoi21  g29(.a(new_n63_), .b(new_n61_), .c(new_n62_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand3  g34(.a(new_n56_), .b(new_n65_), .c(x08), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nor4   g36(.a(new_n70_), .b(new_n68_), .c(new_n64_), .d(new_n44_), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(new_n40_), .O(z05));
  oai21  g38(.a(new_n64_), .b(new_n44_), .c(new_n39_), .O(new_n73_));
  inv1   g39(.a(x01), .O(new_n74_));
  nand3  g40(.a(new_n69_), .b(x10), .c(new_n74_), .O(new_n75_));
  nand4  g41(.a(new_n56_), .b(new_n36_), .c(new_n65_), .d(x08), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n73_), .O(z06));
  inv1   g43(.a(x11), .O(new_n78_));
  nor2   g44(.a(new_n70_), .b(new_n78_), .O(new_n79_));
  nor4   g45(.a(new_n55_), .b(x11), .c(x09), .d(new_n54_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n79_), .c(new_n36_), .O(new_n81_));
  nand3  g47(.a(x11), .b(x10), .c(new_n74_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n73_), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nand4  g50(.a(new_n67_), .b(new_n84_), .c(new_n78_), .d(new_n65_), .O(new_n85_));
  oai21  g51(.a(new_n80_), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n36_), .O(new_n87_));
  nand3  g53(.a(x12), .b(x10), .c(new_n74_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n73_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n48_), .b(new_n90_), .c(new_n84_), .d(new_n78_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(x09), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(x08), .c(x07), .d(x06), .O(new_n93_));
  nand2  g59(.a(new_n85_), .b(x13), .O(new_n94_));
  oai21  g60(.a(new_n93_), .b(new_n44_), .c(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n36_), .O(new_n96_));
  nand3  g62(.a(x13), .b(x10), .c(new_n74_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n96_), .c(new_n73_), .O(z09));
  aoi21  g64(.a(new_n61_), .b(x02), .c(new_n62_), .O(new_n99_));
  inv1   g65(.a(x00), .O(new_n100_));
  nand4  g66(.a(new_n90_), .b(new_n84_), .c(new_n78_), .d(new_n36_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n65_), .c(x08), .d(x07), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  aoi21  g70(.a(new_n99_), .b(x14), .c(new_n104_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n44_), .c(new_n39_), .O(z10));
  nand2  g72(.a(new_n39_), .b(new_n35_), .O(z11));
  nand3  g73(.a(new_n39_), .b(x16), .c(new_n35_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n61_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n39_), .O(z12));
  nand2  g77(.a(new_n39_), .b(new_n44_), .O(z13));
  nand3  g78(.a(new_n39_), .b(x17), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z14));
endmodule


