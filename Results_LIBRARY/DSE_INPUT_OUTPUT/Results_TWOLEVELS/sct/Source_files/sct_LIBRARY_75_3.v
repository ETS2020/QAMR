// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  nand2  g00(.a(x17), .b(x09), .O(new_n35_));
  inv1   g01(.a(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  inv1   g10(.a(new_n35_), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n44_), .c(new_n45_), .O(new_n46_));
  oai21  g12(.a(new_n43_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n39_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n35_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n49_), .c(new_n35_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  and2   g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n55_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n49_), .c(new_n35_), .d(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n39_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n35_), .O(new_n64_));
  inv1   g30(.a(x17), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n65_), .c(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n56_), .b(new_n68_), .c(x08), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n64_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  inv1   g37(.a(new_n66_), .O(new_n72_));
  nand3  g38(.a(new_n56_), .b(new_n71_), .c(x08), .O(new_n73_));
  oai21  g39(.a(new_n72_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n68_), .O(new_n75_));
  nand3  g41(.a(new_n65_), .b(x10), .c(x09), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n64_), .O(z06));
  nand2  g43(.a(new_n73_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand4  g45(.a(new_n56_), .b(new_n79_), .c(new_n71_), .d(x08), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n68_), .O(new_n82_));
  nand3  g48(.a(new_n65_), .b(x11), .c(x09), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n64_), .O(z07));
  oai21  g50(.a(x17), .b(x12), .c(x09), .O(new_n85_));
  nor2   g51(.a(x12), .b(x11), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n71_), .c(new_n68_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  aoi22  g54(.a(new_n88_), .b(new_n72_), .c(new_n80_), .d(x12), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n85_), .c(new_n64_), .O(z08));
  inv1   g56(.a(x12), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n49_), .b(new_n92_), .c(new_n91_), .d(new_n79_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n71_), .c(new_n68_), .d(x08), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x07), .c(x06), .d(x04), .O(new_n97_));
  aoi21  g63(.a(new_n65_), .b(new_n92_), .c(new_n68_), .O(new_n98_));
  nand3  g64(.a(new_n86_), .b(new_n72_), .c(new_n71_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(x13), .c(new_n98_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n97_), .c(new_n64_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand4  g68(.a(new_n92_), .b(new_n91_), .c(new_n79_), .d(new_n71_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n49_), .c(x08), .d(x07), .O(new_n105_));
  oai22  g71(.a(new_n105_), .b(new_n48_), .c(new_n49_), .d(new_n37_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n68_), .O(new_n107_));
  inv1   g73(.a(x03), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x02), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n65_), .c(x16), .d(x14), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n107_), .c(new_n44_), .O(z10));
  nor2   g77(.a(new_n45_), .b(new_n39_), .O(z11));
  nand3  g78(.a(new_n35_), .b(x16), .c(new_n39_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n108_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n35_), .O(z12));
  aoi21  g82(.a(x17), .b(x09), .c(new_n44_), .O(z13));
  nand3  g83(.a(x17), .b(new_n68_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


