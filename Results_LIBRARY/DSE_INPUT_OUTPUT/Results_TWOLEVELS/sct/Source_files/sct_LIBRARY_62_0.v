// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(z00));
  oai21  g07(.a(new_n37_), .b(x05), .c(new_n36_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  oai21  g11(.a(x03), .b(new_n39_), .c(x16), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n37_), .c(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z03));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(x08), .b(new_n52_), .O(new_n53_));
  inv1   g19(.a(x08), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(x07), .c(x06), .O(new_n55_));
  aoi21  g21(.a(new_n55_), .b(new_n53_), .c(new_n36_), .O(new_n56_));
  nor2   g22(.a(new_n54_), .b(x06), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n56_), .c(new_n46_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n38_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x08), .c(x07), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x06), .O(new_n63_));
  inv1   g29(.a(x03), .O(new_n64_));
  inv1   g30(.a(x16), .O(new_n65_));
  aoi21  g31(.a(new_n64_), .b(x02), .c(new_n65_), .O(new_n66_));
  nor2   g32(.a(new_n54_), .b(new_n52_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x09), .c(new_n66_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n36_), .c(new_n63_), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n67_), .c(new_n36_), .O(new_n72_));
  nand4  g38(.a(new_n60_), .b(x08), .c(x07), .d(x06), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(x10), .c(new_n66_), .O(new_n74_));
  oai22  g40(.a(new_n74_), .b(new_n36_), .c(new_n72_), .d(new_n37_), .O(z06));
  nor3   g41(.a(x11), .b(x10), .c(x09), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n67_), .c(new_n36_), .O(new_n77_));
  nand4  g43(.a(new_n71_), .b(x08), .c(x07), .d(x06), .O(new_n78_));
  aoi21  g44(.a(new_n78_), .b(x11), .c(new_n66_), .O(new_n79_));
  oai22  g45(.a(new_n79_), .b(new_n36_), .c(new_n77_), .d(new_n37_), .O(z07));
  inv1   g46(.a(x10), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n61_), .c(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand3  g52(.a(new_n76_), .b(new_n67_), .c(x06), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(x12), .c(new_n66_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n36_), .c(new_n86_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n46_), .b(new_n90_), .c(new_n83_), .d(new_n82_), .O(new_n91_));
  nor3   g57(.a(new_n91_), .b(x10), .c(x09), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(x08), .c(x07), .d(x06), .O(new_n93_));
  nand3  g59(.a(new_n71_), .b(new_n83_), .c(new_n82_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n68_), .c(x13), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n93_), .c(new_n46_), .d(x04), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand4  g63(.a(new_n90_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n66_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n60_), .c(x08), .d(x07), .O(new_n100_));
  nand2  g66(.a(new_n66_), .b(x14), .O(new_n101_));
  oai21  g67(.a(new_n100_), .b(new_n37_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n38_), .O(z10));
  nand2  g70(.a(new_n38_), .b(new_n39_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n39_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n36_), .c(new_n38_), .O(z12));
  nand2  g73(.a(x06), .b(new_n36_), .O(z13));
  nand2  g74(.a(x17), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n38_), .O(z14));
endmodule


