// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n117_, new_n118_, new_n121_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(x09), .b(x00), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n36_), .O(z02));
  inv1   g15(.a(x07), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x06), .O(new_n51_));
  nand3  g17(.a(x09), .b(x07), .c(new_n46_), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(new_n51_), .c(x00), .O(new_n53_));
  nand2  g19(.a(x07), .b(new_n46_), .O(new_n54_));
  aoi21  g20(.a(new_n54_), .b(new_n51_), .c(x09), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n53_), .c(new_n47_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n43_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  nor2   g24(.a(new_n50_), .b(new_n46_), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n47_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n36_), .O(z04));
  inv1   g29(.a(x16), .O(new_n64_));
  aoi21  g30(.a(x04), .b(new_n35_), .c(x03), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n64_), .c(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n36_), .O(new_n67_));
  inv1   g33(.a(x00), .O(new_n68_));
  nand4  g34(.a(new_n47_), .b(x07), .c(new_n46_), .d(x04), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(new_n68_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x09), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nand3  g38(.a(new_n59_), .b(new_n72_), .c(x08), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n67_), .O(z05));
  nand3  g40(.a(x08), .b(x07), .c(x06), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x10), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  nand3  g43(.a(new_n59_), .b(new_n77_), .c(x08), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n72_), .O(new_n80_));
  nand3  g46(.a(x10), .b(x09), .c(new_n68_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n67_), .O(z06));
  nand2  g48(.a(new_n78_), .b(x11), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand4  g50(.a(new_n59_), .b(new_n84_), .c(new_n77_), .d(x08), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n72_), .O(new_n87_));
  nand3  g53(.a(x11), .b(x09), .c(new_n68_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n67_), .O(z07));
  oai21  g55(.a(x12), .b(x00), .c(x09), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nor2   g57(.a(x10), .b(x09), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n84_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n75_), .O(new_n94_));
  aoi21  g60(.a(new_n85_), .b(x12), .c(new_n94_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n90_), .c(new_n67_), .O(z08));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n47_), .b(new_n97_), .c(new_n91_), .d(new_n84_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(x10), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x08), .c(x07), .d(x06), .O(new_n100_));
  nand3  g66(.a(new_n91_), .b(new_n84_), .c(new_n77_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n75_), .c(x13), .O(new_n102_));
  oai21  g68(.a(new_n100_), .b(new_n43_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n72_), .O(new_n104_));
  nand3  g70(.a(x13), .b(x09), .c(new_n68_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n67_), .O(z09));
  inv1   g72(.a(new_n47_), .O(new_n107_));
  nand4  g73(.a(new_n92_), .b(new_n97_), .c(new_n91_), .d(new_n84_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n68_), .c(new_n107_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(x08), .c(x07), .d(x06), .O(new_n110_));
  nand2  g76(.a(new_n107_), .b(x14), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n36_), .O(z10));
  nand2  g80(.a(new_n36_), .b(new_n35_), .O(z11));
  nand3  g81(.a(new_n36_), .b(x16), .c(new_n35_), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(x03), .c(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n36_), .O(z12));
  nand2  g85(.a(new_n36_), .b(new_n43_), .O(z13));
  nand3  g86(.a(new_n36_), .b(x17), .c(x04), .O(new_n121_));
  inv1   g87(.a(new_n121_), .O(z14));
endmodule


