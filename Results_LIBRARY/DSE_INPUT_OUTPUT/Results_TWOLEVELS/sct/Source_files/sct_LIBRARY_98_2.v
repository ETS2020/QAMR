// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x08), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x17), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x08), .O(new_n51_));
  oai21  g17(.a(x03), .b(new_n37_), .c(x16), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n51_), .c(new_n49_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  inv1   g20(.a(new_n36_), .O(new_n55_));
  xor2a  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n52_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n55_), .O(z03));
  inv1   g24(.a(x07), .O(new_n59_));
  aoi21  g25(.a(x17), .b(new_n49_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n35_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n35_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n52_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n55_), .O(z04));
  inv1   g30(.a(x16), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x02), .c(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n55_), .O(new_n67_));
  oai21  g33(.a(x08), .b(new_n46_), .c(new_n50_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(x16), .c(x03), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x07), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x17), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x08), .O(new_n73_));
  nand3  g39(.a(x08), .b(x07), .c(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x09), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n69_), .d(new_n67_), .O(z05));
  nor2   g42(.a(new_n59_), .b(new_n49_), .O(new_n77_));
  nor2   g43(.a(x10), .b(x09), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x17), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x08), .O(new_n81_));
  nand3  g47(.a(new_n77_), .b(new_n70_), .c(x08), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x10), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n81_), .c(new_n69_), .d(new_n67_), .O(z06));
  nand2  g50(.a(new_n81_), .b(x11), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  inv1   g52(.a(new_n74_), .O(new_n87_));
  nand4  g53(.a(new_n78_), .b(new_n87_), .c(x17), .d(new_n86_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n85_), .c(new_n69_), .d(new_n67_), .O(z07));
  nor3   g55(.a(x11), .b(x10), .c(x09), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n77_), .c(new_n50_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n35_), .c(x12), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  inv1   g59(.a(new_n82_), .O(new_n94_));
  nor2   g60(.a(x11), .b(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(x17), .d(new_n93_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n92_), .c(new_n69_), .d(new_n67_), .O(z08));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n52_), .b(x17), .c(new_n98_), .d(new_n93_), .O(new_n99_));
  nor3   g65(.a(new_n99_), .b(x11), .c(x10), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n70_), .c(x07), .d(x06), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n46_), .c(x17), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x08), .O(new_n103_));
  nand3  g69(.a(new_n78_), .b(new_n93_), .c(new_n86_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n74_), .c(x13), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n103_), .c(new_n69_), .d(new_n67_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  inv1   g73(.a(new_n52_), .O(new_n108_));
  nand3  g74(.a(new_n95_), .b(new_n98_), .c(new_n93_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n107_), .c(new_n108_), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(new_n70_), .c(x08), .d(x07), .O(new_n111_));
  oai22  g77(.a(new_n111_), .b(new_n49_), .c(new_n52_), .d(new_n40_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x17), .O(new_n113_));
  nand3  g79(.a(new_n108_), .b(x14), .c(new_n35_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n113_), .c(new_n46_), .O(z10));
  nor2   g81(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g82(.a(x03), .O(new_n117_));
  oai21  g83(.a(new_n65_), .b(x02), .c(new_n117_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n55_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  aoi21  g86(.a(new_n59_), .b(x06), .c(new_n35_), .O(new_n121_));
  nor2   g87(.a(new_n121_), .b(new_n108_), .O(new_n122_));
  oai21  g88(.a(new_n52_), .b(x08), .c(new_n50_), .O(new_n123_));
  oai21  g89(.a(new_n123_), .b(new_n122_), .c(x04), .O(new_n124_));
  nand2  g90(.a(new_n124_), .b(new_n55_), .O(z13));
  nor2   g91(.a(new_n50_), .b(new_n46_), .O(z14));
endmodule


