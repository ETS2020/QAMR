// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n110_, new_n111_, new_n113_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  inv1   g02(.a(x16), .O(new_n37_));
  oai21  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(x16), .b(x01), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand3  g13(.a(x16), .b(new_n47_), .c(x02), .O(new_n48_));
  oai21  g14(.a(x16), .b(new_n35_), .c(new_n48_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(new_n43_), .O(new_n55_));
  inv1   g21(.a(x02), .O(new_n56_));
  oai21  g22(.a(x03), .b(new_n56_), .c(x16), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n57_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n55_), .O(z04));
  aoi21  g29(.a(new_n47_), .b(x02), .c(new_n42_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n42_), .c(x16), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nand2  g33(.a(new_n42_), .b(x01), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n59_), .b(new_n69_), .c(x08), .O(new_n70_));
  and2   g36(.a(new_n70_), .b(new_n55_), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n68_), .c(new_n67_), .d(new_n65_), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n59_), .c(x08), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n68_), .c(new_n55_), .O(new_n75_));
  aoi21  g41(.a(new_n70_), .b(x10), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n65_), .O(z06));
  nand2  g43(.a(new_n74_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x10), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(new_n66_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n69_), .O(new_n82_));
  and2   g48(.a(new_n82_), .b(new_n55_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n78_), .c(new_n68_), .d(new_n65_), .O(z07));
  nand2  g50(.a(new_n82_), .b(x12), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand4  g52(.a(new_n73_), .b(new_n81_), .c(new_n86_), .d(new_n80_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(x04), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n64_), .c(x16), .O(new_n89_));
  nand2  g55(.a(new_n88_), .b(x01), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n89_), .O(z08));
  nand2  g57(.a(new_n55_), .b(new_n42_), .O(z13));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n57_), .b(new_n93_), .c(new_n86_), .d(new_n80_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n69_), .c(x08), .d(x07), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n46_), .c(new_n57_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  aoi21  g64(.a(new_n87_), .b(x13), .c(new_n43_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n98_), .c(z13), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  aoi21  g67(.a(new_n47_), .b(x02), .c(new_n37_), .O(new_n102_));
  nand4  g68(.a(new_n93_), .b(new_n86_), .c(new_n80_), .d(new_n79_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n101_), .c(new_n102_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n69_), .c(x08), .d(x07), .O(new_n105_));
  oai22  g71(.a(new_n105_), .b(new_n46_), .c(new_n57_), .d(new_n36_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n55_), .O(z10));
  nand2  g74(.a(new_n55_), .b(new_n56_), .O(z11));
  nand2  g75(.a(new_n55_), .b(x03), .O(new_n110_));
  nand2  g76(.a(x16), .b(new_n56_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n42_), .O(z12));
  inv1   g78(.a(x17), .O(new_n113_));
  nor3   g79(.a(new_n43_), .b(new_n113_), .c(new_n42_), .O(z14));
endmodule


