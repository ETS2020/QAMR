// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n118_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x15), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g06(.a(x02), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x01), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n40_), .c(new_n37_), .O(z00));
  aoi21  g09(.a(x06), .b(x05), .c(x15), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(x06), .c(x15), .O(new_n46_));
  oai22  g12(.a(new_n46_), .b(x05), .c(new_n44_), .d(x04), .O(z01));
  oai21  g13(.a(x03), .b(new_n41_), .c(x16), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x04), .c(new_n36_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(x06), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand3  g20(.a(x15), .b(x08), .c(new_n35_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n35_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n48_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  inv1   g24(.a(x16), .O(new_n59_));
  aoi21  g25(.a(x04), .b(new_n41_), .c(x03), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n59_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n37_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand2  g29(.a(x08), .b(x07), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n63_), .b(x08), .c(x07), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nor3   g33(.a(new_n36_), .b(new_n63_), .c(x06), .O(new_n68_));
  aoi21  g34(.a(new_n67_), .b(x06), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n62_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  inv1   g37(.a(new_n66_), .O(new_n72_));
  nand3  g38(.a(new_n65_), .b(new_n71_), .c(new_n63_), .O(new_n73_));
  oai21  g39(.a(new_n72_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand3  g41(.a(x15), .b(x10), .c(new_n35_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n62_), .O(z06));
  and2   g43(.a(new_n73_), .b(x11), .O(new_n78_));
  nor4   g44(.a(new_n64_), .b(x11), .c(x10), .d(x09), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n78_), .c(x06), .O(new_n80_));
  nand3  g46(.a(x15), .b(x11), .c(new_n35_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n62_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand4  g50(.a(new_n72_), .b(new_n83_), .c(new_n84_), .d(new_n71_), .O(new_n85_));
  oai21  g51(.a(new_n79_), .b(new_n83_), .c(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x06), .O(new_n87_));
  nand3  g53(.a(x15), .b(x12), .c(new_n35_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n62_), .O(z08));
  inv1   g55(.a(x04), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n48_), .b(new_n91_), .c(new_n83_), .d(new_n84_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x10), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n63_), .c(x08), .d(x07), .O(new_n94_));
  nand2  g60(.a(new_n85_), .b(x13), .O(new_n95_));
  oai21  g61(.a(new_n94_), .b(new_n90_), .c(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x06), .O(new_n97_));
  nand3  g63(.a(x15), .b(x13), .c(new_n35_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n97_), .c(new_n62_), .O(z09));
  inv1   g65(.a(x07), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand4  g67(.a(new_n91_), .b(new_n83_), .c(new_n84_), .d(new_n71_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n48_), .c(new_n63_), .d(x08), .O(new_n104_));
  oai22  g70(.a(new_n104_), .b(new_n100_), .c(new_n48_), .d(new_n39_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x06), .O(new_n106_));
  inv1   g72(.a(new_n48_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(x15), .c(x14), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n106_), .c(new_n90_), .O(z10));
  nand2  g75(.a(new_n37_), .b(new_n41_), .O(z11));
  aoi21  g76(.a(x16), .b(new_n41_), .c(x03), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n37_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  oai21  g80(.a(new_n107_), .b(x06), .c(new_n111_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(x15), .O(new_n116_));
  aoi21  g82(.a(new_n116_), .b(new_n35_), .c(new_n90_), .O(z13));
  nand2  g83(.a(x17), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n37_), .O(z14));
endmodule


