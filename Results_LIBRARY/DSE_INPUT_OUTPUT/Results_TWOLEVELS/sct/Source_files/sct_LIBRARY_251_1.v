// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n107_;
  inv1   g00(.a(x01), .O(new_n35_));
  aoi21  g01(.a(x14), .b(new_n35_), .c(x02), .O(z00));
  inv1   g02(.a(x02), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n38_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  oai22  g14(.a(new_n48_), .b(new_n35_), .c(x16), .d(x02), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n45_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n38_), .O(new_n52_));
  inv1   g18(.a(new_n48_), .O(new_n53_));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n52_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n53_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n52_), .O(z04));
  oai22  g28(.a(new_n47_), .b(new_n46_), .c(x04), .d(new_n37_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x01), .O(new_n64_));
  oai21  g30(.a(x16), .b(new_n42_), .c(new_n37_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n58_), .b(new_n68_), .c(x08), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n52_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n64_), .O(z05));
  nand2  g37(.a(new_n69_), .b(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand4  g39(.a(new_n58_), .b(new_n73_), .c(new_n68_), .d(x08), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(new_n52_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n65_), .d(new_n64_), .O(z06));
  nand2  g42(.a(new_n74_), .b(x11), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  inv1   g44(.a(new_n66_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n73_), .d(new_n68_), .O(new_n80_));
  and2   g46(.a(new_n80_), .b(new_n52_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n77_), .c(new_n65_), .d(new_n64_), .O(z07));
  nand2  g48(.a(new_n80_), .b(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n78_), .c(new_n73_), .d(new_n68_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n66_), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(new_n38_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n83_), .c(new_n65_), .d(new_n64_), .O(z08));
  oai21  g54(.a(new_n37_), .b(x01), .c(new_n42_), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n84_), .c(new_n78_), .d(new_n73_), .O(new_n91_));
  oai22  g57(.a(new_n91_), .b(new_n69_), .c(new_n86_), .d(new_n90_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n52_), .O(new_n93_));
  oai22  g59(.a(new_n42_), .b(x02), .c(new_n46_), .d(new_n35_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x16), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n93_), .c(new_n89_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  aoi21  g63(.a(new_n91_), .b(new_n97_), .c(new_n48_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n68_), .c(x08), .d(x07), .O(new_n99_));
  nand2  g65(.a(new_n48_), .b(x14), .O(new_n100_));
  oai21  g66(.a(new_n99_), .b(new_n45_), .c(new_n100_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x04), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n52_), .O(z10));
  aoi21  g69(.a(x16), .b(new_n37_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n42_), .c(new_n52_), .O(z12));
  aoi21  g71(.a(x02), .b(new_n35_), .c(new_n42_), .O(z13));
  inv1   g72(.a(x17), .O(new_n107_));
  nor3   g73(.a(new_n38_), .b(new_n107_), .c(new_n42_), .O(z14));
  buf    g74(.a(x02), .O(z11));
endmodule


