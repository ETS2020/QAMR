// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n121_,
    new_n123_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(new_n36_), .b(x08), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  nand2  g09(.a(x08), .b(new_n35_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(x08), .b(new_n35_), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n48_), .c(new_n46_), .d(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n44_), .O(z02));
  nand2  g17(.a(x07), .b(new_n46_), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand3  g19(.a(x08), .b(new_n53_), .c(x06), .O(new_n54_));
  aoi21  g20(.a(new_n54_), .b(new_n52_), .c(new_n35_), .O(new_n55_));
  nand2  g21(.a(new_n53_), .b(x06), .O(new_n56_));
  aoi21  g22(.a(new_n56_), .b(new_n52_), .c(x08), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n55_), .c(new_n48_), .O(new_n58_));
  nor2   g24(.a(new_n58_), .b(new_n42_), .O(z03));
  nand2  g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x08), .c(x01), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n48_), .c(x04), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(z04));
  inv1   g32(.a(x16), .O(new_n67_));
  aoi21  g33(.a(x04), .b(new_n47_), .c(x03), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n67_), .c(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n44_), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nand2  g37(.a(x07), .b(x06), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(x08), .O(new_n74_));
  oai21  g40(.a(new_n73_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  aoi22  g41(.a(new_n75_), .b(x01), .c(x09), .d(new_n62_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n70_), .O(z05));
  nand3  g43(.a(new_n71_), .b(x07), .c(x06), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x10), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  nand4  g46(.a(new_n73_), .b(new_n80_), .c(new_n71_), .d(x08), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x01), .O(new_n83_));
  nand2  g49(.a(x10), .b(new_n62_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n70_), .O(z06));
  nor4   g51(.a(new_n72_), .b(x11), .c(x10), .d(x09), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n35_), .c(x08), .O(new_n87_));
  nand2  g53(.a(new_n81_), .b(x11), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n70_), .O(z07));
  inv1   g55(.a(x12), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  nand4  g57(.a(new_n90_), .b(new_n91_), .c(new_n80_), .d(new_n71_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(x08), .c(x07), .d(x06), .O(new_n94_));
  oai21  g60(.a(new_n86_), .b(new_n90_), .c(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x01), .O(new_n96_));
  nand2  g62(.a(x12), .b(new_n62_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n96_), .c(new_n70_), .O(z08));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n48_), .b(new_n99_), .c(new_n90_), .d(new_n91_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(x10), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n71_), .c(x07), .d(x06), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n42_), .c(x01), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x08), .O(new_n104_));
  nand2  g70(.a(new_n94_), .b(x13), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n70_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  inv1   g73(.a(x03), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(x02), .c(new_n67_), .O(new_n109_));
  nand4  g75(.a(new_n99_), .b(new_n90_), .c(new_n91_), .d(new_n80_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n107_), .c(new_n109_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n71_), .c(x08), .d(x07), .O(new_n112_));
  oai22  g78(.a(new_n112_), .b(new_n46_), .c(new_n48_), .d(new_n36_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(x01), .O(new_n114_));
  nand3  g80(.a(new_n109_), .b(x14), .c(new_n62_), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n114_), .c(new_n42_), .O(z10));
  nand2  g82(.a(new_n44_), .b(new_n47_), .O(z11));
  oai21  g83(.a(new_n67_), .b(x02), .c(new_n108_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n44_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  oai21  g86(.a(new_n118_), .b(new_n48_), .c(new_n62_), .O(new_n121_));
  aoi21  g87(.a(new_n121_), .b(new_n35_), .c(new_n42_), .O(z13));
  nand2  g88(.a(x17), .b(x04), .O(new_n123_));
  nand2  g89(.a(new_n123_), .b(new_n44_), .O(z14));
endmodule


