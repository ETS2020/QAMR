// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n119_;
  nand2  g00(.a(x17), .b(x11), .O(new_n35_));
  or2    g01(.a(x14), .b(x01), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  inv1   g08(.a(new_n35_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n37_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n35_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n35_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  nand4  g19(.a(new_n35_), .b(new_n53_), .c(x07), .d(x06), .O(new_n54_));
  nand2  g20(.a(x17), .b(x11), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(x08), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n47_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x03), .O(new_n61_));
  oai21  g27(.a(new_n42_), .b(x02), .c(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x16), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n35_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  inv1   g34(.a(new_n56_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(x08), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n35_), .O(z05));
  inv1   g37(.a(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n68_), .c(x08), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x04), .O(new_n76_));
  aoi21  g42(.a(new_n70_), .b(x10), .c(new_n76_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n63_), .c(new_n43_), .O(z06));
  inv1   g44(.a(x17), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  nand4  g46(.a(new_n56_), .b(new_n47_), .c(x08), .d(x04), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n74_), .c(new_n80_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n64_), .c(new_n79_), .O(new_n83_));
  aoi21  g49(.a(new_n62_), .b(x16), .c(new_n76_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(x11), .c(new_n83_), .O(z07));
  oai21  g51(.a(x17), .b(x12), .c(x11), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n80_), .c(new_n72_), .d(new_n68_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(new_n66_), .O(new_n89_));
  aoi21  g55(.a(new_n75_), .b(x12), .c(new_n89_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n86_), .c(new_n65_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n47_), .b(new_n92_), .c(new_n87_), .d(new_n80_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n72_), .c(new_n68_), .d(x08), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x07), .c(x06), .d(x04), .O(new_n97_));
  aoi21  g63(.a(new_n79_), .b(new_n92_), .c(new_n80_), .O(new_n98_));
  inv1   g64(.a(new_n66_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n87_), .c(new_n72_), .d(new_n68_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x13), .c(new_n98_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n97_), .c(new_n65_), .O(z09));
  nor3   g68(.a(new_n93_), .b(x10), .c(x09), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(x08), .c(x07), .d(x06), .O(new_n104_));
  inv1   g70(.a(x00), .O(new_n105_));
  inv1   g71(.a(x16), .O(new_n106_));
  aoi21  g72(.a(new_n61_), .b(x02), .c(new_n106_), .O(new_n107_));
  nor2   g73(.a(new_n107_), .b(x09), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(x08), .c(x07), .d(x06), .O(new_n109_));
  nand2  g75(.a(new_n107_), .b(x14), .O(new_n110_));
  oai21  g76(.a(new_n109_), .b(new_n105_), .c(new_n110_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n35_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n104_), .c(new_n42_), .O(z10));
  nor2   g79(.a(new_n43_), .b(new_n37_), .O(z11));
  oai21  g80(.a(new_n106_), .b(x02), .c(new_n61_), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(new_n35_), .c(x04), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(z12));
  aoi21  g83(.a(x17), .b(x11), .c(new_n42_), .O(z13));
  nand3  g84(.a(x17), .b(new_n80_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z14));
endmodule


