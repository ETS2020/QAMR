// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n94_, new_n97_;
  inv1   g00(.a(x04), .O(new_n35_));
  inv1   g01(.a(x16), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n35_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n43_), .b(x05), .c(new_n44_), .O(z01));
  aoi21  g11(.a(new_n36_), .b(x06), .c(new_n35_), .O(z02));
  xor2a  g12(.a(x07), .b(x06), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n36_), .c(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z03));
  nand2  g15(.a(x07), .b(x06), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x08), .O(new_n51_));
  inv1   g17(.a(x08), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x07), .c(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n51_), .c(new_n36_), .O(new_n54_));
  and2   g20(.a(new_n54_), .b(x04), .O(z04));
  inv1   g21(.a(x09), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  inv1   g24(.a(new_n50_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n56_), .c(x08), .O(new_n60_));
  oai21  g26(.a(new_n58_), .b(new_n56_), .c(new_n60_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n36_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x04), .O(z05));
  nand2  g29(.a(new_n60_), .b(x10), .O(new_n64_));
  nor2   g30(.a(x10), .b(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n59_), .c(x08), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n64_), .c(new_n36_), .d(x04), .O(z06));
  nand2  g33(.a(new_n66_), .b(x11), .O(new_n68_));
  nor2   g34(.a(x11), .b(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n58_), .c(new_n56_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n36_), .d(x04), .O(z07));
  nand2  g37(.a(new_n70_), .b(x12), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  inv1   g39(.a(x12), .O(new_n74_));
  nand3  g40(.a(new_n65_), .b(new_n74_), .c(new_n73_), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n58_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n72_), .c(new_n36_), .d(x04), .O(z08));
  oai21  g44(.a(new_n75_), .b(new_n57_), .c(x13), .O(new_n79_));
  nand4  g45(.a(x08), .b(x07), .c(x06), .d(x04), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  nor3   g47(.a(x13), .b(x12), .c(x11), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n65_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n36_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(z09));
  inv1   g52(.a(x00), .O(new_n87_));
  nor2   g53(.a(x16), .b(x13), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n69_), .c(new_n74_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n87_), .c(x09), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(x08), .c(x07), .d(x06), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n36_), .c(new_n35_), .O(z10));
  oai21  g58(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(z11));
  nand2  g59(.a(x04), .b(x03), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x16), .O(z12));
  nor2   g61(.a(x16), .b(new_n35_), .O(z13));
  inv1   g62(.a(x17), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n36_), .c(new_n35_), .O(z14));
endmodule


