// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_;
  inv1   g00(.a(x12), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x17), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x12), .O(new_n51_));
  oai21  g17(.a(x03), .b(new_n37_), .c(x16), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n51_), .c(new_n49_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  inv1   g20(.a(x07), .O(new_n55_));
  nand3  g21(.a(new_n51_), .b(new_n55_), .c(x06), .O(new_n56_));
  inv1   g22(.a(new_n36_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x07), .c(new_n49_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n52_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z03));
  inv1   g27(.a(x08), .O(new_n62_));
  nand4  g28(.a(new_n51_), .b(new_n62_), .c(x07), .d(x06), .O(new_n63_));
  nor2   g29(.a(new_n55_), .b(new_n49_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n57_), .c(x08), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n52_), .c(x04), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(z04));
  inv1   g35(.a(x03), .O(new_n70_));
  inv1   g36(.a(x16), .O(new_n71_));
  nand2  g37(.a(x04), .b(new_n37_), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n70_), .c(new_n71_), .O(new_n73_));
  nand3  g39(.a(x08), .b(x07), .c(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x09), .O(new_n75_));
  inv1   g41(.a(x09), .O(new_n76_));
  nand3  g42(.a(new_n64_), .b(new_n76_), .c(x08), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(x04), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n73_), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n36_), .O(z05));
  oai21  g46(.a(new_n73_), .b(new_n46_), .c(new_n57_), .O(new_n81_));
  nand2  g47(.a(new_n77_), .b(x10), .O(new_n82_));
  nor2   g48(.a(x10), .b(x09), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n64_), .c(x08), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n82_), .c(new_n81_), .d(new_n57_), .O(z06));
  nand2  g51(.a(new_n84_), .b(x11), .O(new_n86_));
  inv1   g52(.a(new_n74_), .O(new_n87_));
  nor3   g53(.a(x11), .b(x10), .c(x09), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n86_), .c(new_n81_), .d(new_n57_), .O(z07));
  nand3  g56(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n52_), .c(x04), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n88_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(x17), .c(x12), .O(new_n94_));
  inv1   g60(.a(x11), .O(new_n95_));
  nand4  g61(.a(new_n83_), .b(new_n87_), .c(new_n35_), .d(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n94_), .c(new_n81_), .O(z08));
  inv1   g63(.a(x10), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n52_), .b(new_n99_), .c(new_n95_), .d(new_n98_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(x09), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(x08), .c(x07), .d(x06), .O(new_n102_));
  nand2  g68(.a(new_n89_), .b(x13), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n46_), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n35_), .O(new_n105_));
  nand3  g71(.a(x17), .b(x13), .c(x12), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n105_), .c(new_n81_), .O(z09));
  inv1   g73(.a(x00), .O(new_n108_));
  and2   g74(.a(new_n52_), .b(new_n76_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(x08), .c(x07), .d(x06), .O(new_n110_));
  oai22  g76(.a(new_n110_), .b(new_n108_), .c(new_n52_), .d(new_n40_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n57_), .O(new_n112_));
  nand4  g78(.a(new_n52_), .b(new_n99_), .c(new_n35_), .d(new_n95_), .O(new_n113_));
  nor3   g79(.a(new_n113_), .b(x10), .c(x09), .O(new_n114_));
  nand4  g80(.a(new_n114_), .b(x08), .c(x07), .d(x06), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n112_), .c(new_n46_), .O(z10));
  nor2   g82(.a(new_n36_), .b(new_n37_), .O(z11));
  nor2   g83(.a(new_n36_), .b(new_n71_), .O(new_n118_));
  aoi21  g84(.a(new_n118_), .b(new_n37_), .c(x03), .O(new_n119_));
  oai21  g85(.a(new_n119_), .b(new_n46_), .c(new_n57_), .O(z12));
  nor2   g86(.a(new_n36_), .b(new_n46_), .O(z13));
  nor2   g87(.a(new_n50_), .b(new_n46_), .O(z14));
endmodule


