// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x17), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x12), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(new_n36_), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n37_), .c(x16), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n51_), .c(new_n50_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(new_n51_), .c(new_n50_), .d(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x03), .O(new_n63_));
  inv1   g29(.a(x16), .O(new_n64_));
  nand2  g30(.a(x04), .b(new_n37_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n63_), .c(new_n64_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand3  g36(.a(new_n58_), .b(new_n67_), .c(x08), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nor4   g38(.a(new_n72_), .b(new_n70_), .c(new_n66_), .d(new_n46_), .O(new_n73_));
  nor2   g39(.a(new_n73_), .b(new_n36_), .O(z05));
  oai21  g40(.a(new_n66_), .b(new_n46_), .c(new_n50_), .O(new_n75_));
  nand2  g41(.a(new_n71_), .b(x10), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  nand4  g43(.a(new_n58_), .b(new_n77_), .c(new_n67_), .d(x08), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n76_), .c(new_n75_), .d(new_n50_), .O(z06));
  nand2  g45(.a(new_n78_), .b(x11), .O(new_n80_));
  nor3   g46(.a(x11), .b(x10), .c(x09), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n69_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n80_), .c(new_n75_), .d(new_n50_), .O(z07));
  nand3  g49(.a(x08), .b(x07), .c(x06), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n51_), .c(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nor2   g54(.a(x11), .b(x10), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n72_), .c(new_n35_), .d(new_n88_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n87_), .c(new_n75_), .O(z08));
  inv1   g57(.a(x11), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n51_), .b(new_n93_), .c(new_n88_), .d(new_n92_), .O(new_n94_));
  nor3   g60(.a(new_n94_), .b(x10), .c(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  nand2  g62(.a(new_n82_), .b(x13), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n46_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n35_), .O(new_n99_));
  nand2  g65(.a(x13), .b(x12), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n99_), .c(new_n75_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  and2   g68(.a(new_n51_), .b(new_n67_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(x08), .c(x07), .d(x06), .O(new_n104_));
  oai22  g70(.a(new_n104_), .b(new_n102_), .c(new_n51_), .d(new_n40_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n50_), .O(new_n106_));
  nand4  g72(.a(new_n51_), .b(new_n35_), .c(new_n93_), .d(new_n88_), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n92_), .c(new_n77_), .d(new_n67_), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(x08), .c(x07), .d(x06), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n106_), .c(new_n46_), .O(z10));
  nor2   g78(.a(new_n36_), .b(new_n37_), .O(z11));
  nor2   g79(.a(new_n36_), .b(new_n64_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n37_), .c(x03), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n46_), .c(new_n50_), .O(z12));
  aoi21  g82(.a(x17), .b(new_n88_), .c(new_n46_), .O(z13));
  nand3  g83(.a(x17), .b(x12), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


