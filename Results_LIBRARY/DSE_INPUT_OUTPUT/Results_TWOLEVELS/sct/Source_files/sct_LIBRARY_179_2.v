// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n113_;
  inv1   g00(.a(x15), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi22  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .d(x04), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n42_), .O(new_n44_));
  oai21  g10(.a(new_n35_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x15), .c(new_n42_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(x15), .c(new_n42_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  and2   g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n53_), .b(x07), .c(x06), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  aoi21  g23(.a(new_n57_), .b(x15), .c(new_n42_), .O(z04));
  inv1   g24(.a(x16), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x02), .c(x15), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x04), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n54_), .b(new_n64_), .c(x08), .O(new_n65_));
  nand3  g31(.a(x16), .b(x15), .c(x03), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n63_), .c(new_n61_), .d(x04), .O(z05));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n54_), .c(x08), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n66_), .c(x04), .O(new_n71_));
  aoi21  g37(.a(new_n65_), .b(x10), .c(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n61_), .O(z06));
  nand2  g39(.a(new_n70_), .b(x11), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  inv1   g42(.a(new_n62_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n64_), .O(new_n78_));
  and2   g44(.a(new_n78_), .b(new_n66_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n74_), .c(new_n61_), .d(x04), .O(z07));
  inv1   g46(.a(x03), .O(new_n81_));
  oai21  g47(.a(new_n42_), .b(x02), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x16), .O(new_n83_));
  nand2  g49(.a(new_n78_), .b(x12), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  nand4  g51(.a(new_n69_), .b(new_n77_), .c(new_n85_), .d(new_n76_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x15), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n47_), .b(new_n90_), .c(new_n85_), .d(new_n76_), .O(new_n91_));
  nor3   g57(.a(new_n91_), .b(x10), .c(x09), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(x08), .c(x07), .d(x06), .O(new_n93_));
  nand2  g59(.a(x16), .b(new_n36_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(x15), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  nand2  g62(.a(new_n66_), .b(x04), .O(new_n97_));
  aoi21  g63(.a(new_n86_), .b(x13), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n96_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand4  g66(.a(new_n90_), .b(new_n85_), .c(new_n76_), .d(new_n75_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  and2   g68(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n64_), .c(x08), .d(x07), .O(new_n104_));
  oai22  g70(.a(new_n104_), .b(new_n46_), .c(new_n47_), .d(new_n39_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(x15), .c(x04), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(z10));
  oai21  g73(.a(x15), .b(new_n42_), .c(new_n36_), .O(z11));
  nand2  g74(.a(new_n94_), .b(new_n81_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(x15), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z12));
  nor2   g77(.a(new_n35_), .b(new_n42_), .O(z13));
  nand2  g78(.a(z13), .b(x17), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z14));
endmodule


