// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n106_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  oai21  g01(.a(x16), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x01), .O(new_n41_));
  inv1   g07(.a(x16), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n40_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n39_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(x02), .c(new_n41_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(new_n43_), .O(new_n55_));
  oai21  g21(.a(x03), .b(new_n35_), .c(x16), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n56_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n55_), .O(z04));
  aoi21  g28(.a(new_n47_), .b(x02), .c(new_n42_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n40_), .c(new_n41_), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n58_), .b(new_n67_), .c(x08), .O(new_n68_));
  aoi21  g34(.a(new_n42_), .b(new_n40_), .c(new_n43_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n68_), .c(new_n66_), .d(new_n64_), .O(z05));
  nand2  g36(.a(new_n68_), .b(x10), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n58_), .c(x08), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n71_), .c(new_n69_), .d(new_n64_), .O(z06));
  nand2  g40(.a(new_n73_), .b(x11), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  inv1   g43(.a(new_n65_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n67_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n75_), .c(new_n69_), .d(new_n64_), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  nand4  g47(.a(new_n72_), .b(new_n78_), .c(new_n81_), .d(new_n77_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  aoi21  g49(.a(new_n79_), .b(x12), .c(new_n83_), .O(new_n84_));
  oai21  g50(.a(new_n56_), .b(new_n40_), .c(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n41_), .O(new_n86_));
  oai21  g52(.a(new_n84_), .b(x16), .c(new_n86_), .O(z08));
  nand2  g53(.a(new_n55_), .b(new_n40_), .O(z13));
  nand2  g54(.a(new_n63_), .b(new_n41_), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n56_), .b(new_n90_), .c(new_n81_), .d(new_n77_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(x10), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n67_), .c(x08), .d(x07), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n46_), .c(new_n89_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(new_n95_));
  aoi21  g61(.a(new_n82_), .b(x13), .c(new_n43_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n95_), .c(z13), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand4  g64(.a(new_n90_), .b(new_n81_), .c(new_n77_), .d(new_n76_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n63_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n67_), .c(x08), .d(x07), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n46_), .O(new_n102_));
  aoi21  g68(.a(new_n63_), .b(x14), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n40_), .c(new_n55_), .O(z10));
  nand2  g70(.a(new_n55_), .b(new_n35_), .O(z11));
  nand2  g71(.a(new_n42_), .b(x03), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n89_), .c(new_n40_), .O(z12));
  nand3  g73(.a(new_n55_), .b(x17), .c(x04), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(z14));
endmodule


