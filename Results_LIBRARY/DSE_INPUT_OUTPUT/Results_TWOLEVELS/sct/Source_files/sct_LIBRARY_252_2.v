// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_;
  inv1   g00(.a(x06), .O(new_n35_));
  nor2   g01(.a(x11), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  oai21  g14(.a(x03), .b(new_n37_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n35_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand3  g18(.a(x11), .b(new_n52_), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  aoi21  g23(.a(x11), .b(new_n52_), .c(new_n35_), .O(new_n58_));
  nand2  g24(.a(x07), .b(x06), .O(new_n59_));
  nand2  g25(.a(x11), .b(new_n57_), .O(new_n60_));
  oai22  g26(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n49_), .c(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  inv1   g29(.a(new_n36_), .O(new_n64_));
  inv1   g30(.a(x03), .O(new_n65_));
  oai21  g31(.a(new_n46_), .b(x02), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x16), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n64_), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x08), .c(x07), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n46_), .c(x11), .O(new_n72_));
  nand3  g38(.a(x08), .b(x07), .c(x06), .O(new_n73_));
  aoi22  g39(.a(new_n73_), .b(x09), .c(new_n72_), .d(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n69_), .O(z05));
  nand2  g41(.a(x07), .b(x04), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n70_), .c(x08), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n76_), .c(x11), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x06), .O(new_n80_));
  nand2  g46(.a(new_n70_), .b(x08), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n59_), .c(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n69_), .O(z06));
  nand2  g49(.a(x08), .b(x07), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n49_), .c(x06), .O(new_n85_));
  inv1   g51(.a(x16), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(x02), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x09), .O(new_n88_));
  nor2   g54(.a(new_n86_), .b(new_n65_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n35_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n88_), .c(new_n85_), .d(x04), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x11), .O(new_n92_));
  nand3  g58(.a(x10), .b(new_n70_), .c(x08), .O(new_n93_));
  nand3  g59(.a(x16), .b(new_n35_), .c(new_n37_), .O(new_n94_));
  oai21  g60(.a(new_n93_), .b(new_n59_), .c(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  oai21  g62(.a(new_n89_), .b(new_n46_), .c(new_n35_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n96_), .c(new_n92_), .d(new_n64_), .O(z07));
  nor2   g64(.a(x12), .b(new_n46_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n67_), .c(new_n36_), .O(z08));
  nor2   g66(.a(x13), .b(new_n46_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n67_), .c(new_n36_), .O(z09));
  nand4  g68(.a(new_n49_), .b(new_n70_), .c(x08), .d(x07), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n35_), .O(new_n104_));
  nor2   g70(.a(new_n49_), .b(new_n40_), .O(new_n105_));
  aoi21  g71(.a(new_n104_), .b(x00), .c(new_n105_), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n46_), .c(new_n64_), .O(z10));
  nand2  g73(.a(new_n64_), .b(new_n37_), .O(z11));
  oai21  g74(.a(new_n87_), .b(x03), .c(new_n64_), .O(new_n109_));
  or2    g75(.a(new_n109_), .b(new_n46_), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z12));
  nand2  g77(.a(new_n84_), .b(x11), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x06), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n49_), .O(new_n114_));
  inv1   g80(.a(new_n71_), .O(new_n115_));
  aoi22  g81(.a(new_n115_), .b(x06), .c(x11), .d(x09), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n114_), .c(new_n109_), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n64_), .O(z13));
  inv1   g85(.a(x17), .O(new_n120_));
  nor3   g86(.a(new_n36_), .b(new_n120_), .c(new_n46_), .O(z14));
endmodule


