// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n117_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  inv1   g03(.a(x15), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  oai21  g05(.a(new_n39_), .b(x01), .c(new_n36_), .O(z00));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(x15), .c(x01), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nor2   g09(.a(x18), .b(x05), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n43_), .c(new_n44_), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n42_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nor2   g14(.a(new_n38_), .b(x01), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n48_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n48_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n50_), .O(z04));
  inv1   g27(.a(x03), .O(new_n62_));
  oai21  g28(.a(new_n43_), .b(x02), .c(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n50_), .c(x16), .O(new_n64_));
  oai21  g30(.a(new_n38_), .b(x01), .c(new_n43_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n57_), .b(new_n68_), .c(x08), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n50_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n64_), .O(z05));
  nand2  g37(.a(new_n63_), .b(x16), .O(new_n72_));
  nand2  g38(.a(new_n69_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n57_), .c(x08), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n50_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n65_), .O(z06));
  nand2  g44(.a(new_n75_), .b(x11), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  inv1   g47(.a(new_n66_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(new_n68_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n79_), .c(new_n72_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n50_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n65_), .O(z07));
  nand2  g52(.a(new_n83_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand4  g54(.a(new_n74_), .b(new_n82_), .c(new_n88_), .d(new_n81_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n87_), .c(new_n72_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n65_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n48_), .b(new_n93_), .c(new_n88_), .d(new_n81_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n68_), .c(x08), .d(x07), .O(new_n96_));
  nand2  g62(.a(x16), .b(new_n35_), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n47_), .c(new_n97_), .O(new_n98_));
  inv1   g64(.a(x16), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n62_), .O(new_n100_));
  aoi21  g66(.a(new_n98_), .b(x04), .c(new_n100_), .O(new_n101_));
  nand2  g67(.a(new_n89_), .b(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n101_), .c(new_n65_), .d(new_n50_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand4  g70(.a(new_n93_), .b(new_n88_), .c(new_n81_), .d(new_n80_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  and2   g72(.a(new_n106_), .b(new_n48_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n68_), .c(x08), .d(x07), .O(new_n108_));
  oai22  g74(.a(new_n108_), .b(new_n47_), .c(new_n48_), .d(new_n37_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(new_n50_), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z10));
  nand2  g77(.a(new_n50_), .b(new_n35_), .O(z11));
  nor2   g78(.a(new_n49_), .b(new_n99_), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n35_), .c(x03), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(new_n43_), .c(new_n50_), .O(z12));
  nor2   g81(.a(new_n49_), .b(new_n43_), .O(z13));
  nand2  g82(.a(x17), .b(x04), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n50_), .O(z14));
endmodule


