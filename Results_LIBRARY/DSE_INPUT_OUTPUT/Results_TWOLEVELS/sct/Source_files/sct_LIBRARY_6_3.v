// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n116_, new_n119_;
  nand2  g00(.a(x17), .b(x09), .O(new_n35_));
  inv1   g01(.a(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  inv1   g10(.a(new_n35_), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n44_), .c(new_n45_), .O(new_n46_));
  oai21  g12(.a(new_n43_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n39_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n35_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n35_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  and2   g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n55_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n49_), .c(new_n35_), .d(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n39_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n35_), .O(new_n64_));
  inv1   g30(.a(x17), .O(new_n65_));
  nand3  g31(.a(new_n56_), .b(new_n65_), .c(x08), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n55_), .O(new_n67_));
  aoi22  g33(.a(new_n67_), .b(new_n56_), .c(new_n66_), .d(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n64_), .O(z05));
  inv1   g35(.a(x09), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand3  g37(.a(x08), .b(x07), .c(x06), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand3  g39(.a(new_n56_), .b(new_n71_), .c(x08), .O(new_n74_));
  oai21  g40(.a(new_n73_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n70_), .O(new_n76_));
  nand3  g42(.a(new_n65_), .b(x10), .c(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n64_), .O(z06));
  nand2  g44(.a(new_n74_), .b(x11), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  nand4  g46(.a(new_n56_), .b(new_n80_), .c(new_n71_), .d(x08), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n70_), .O(new_n83_));
  nand3  g49(.a(new_n65_), .b(x11), .c(x09), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n64_), .O(z07));
  oai21  g51(.a(x17), .b(x12), .c(x09), .O(new_n86_));
  nor2   g52(.a(x12), .b(x11), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n71_), .c(new_n70_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  aoi22  g55(.a(new_n89_), .b(new_n73_), .c(new_n81_), .d(x12), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n86_), .c(new_n64_), .O(z08));
  inv1   g57(.a(x12), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n49_), .b(new_n93_), .c(new_n92_), .d(new_n80_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n71_), .c(new_n70_), .d(x08), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x07), .c(x06), .d(x04), .O(new_n98_));
  aoi21  g64(.a(new_n65_), .b(new_n93_), .c(new_n70_), .O(new_n99_));
  nand3  g65(.a(new_n87_), .b(new_n73_), .c(new_n71_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x13), .c(new_n99_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n98_), .c(new_n64_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  nand4  g69(.a(new_n93_), .b(new_n92_), .c(new_n80_), .d(new_n71_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n49_), .c(x08), .d(x07), .O(new_n106_));
  oai22  g72(.a(new_n106_), .b(new_n48_), .c(new_n49_), .d(new_n37_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n70_), .O(new_n108_));
  inv1   g74(.a(x03), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x02), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(new_n65_), .c(x16), .d(x14), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n108_), .c(new_n44_), .O(z10));
  nor2   g78(.a(new_n45_), .b(new_n39_), .O(z11));
  nand3  g79(.a(new_n35_), .b(x16), .c(new_n39_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n109_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n35_), .O(z12));
  aoi21  g83(.a(x17), .b(x09), .c(new_n44_), .O(z13));
  nand3  g84(.a(x17), .b(new_n70_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z14));
endmodule


