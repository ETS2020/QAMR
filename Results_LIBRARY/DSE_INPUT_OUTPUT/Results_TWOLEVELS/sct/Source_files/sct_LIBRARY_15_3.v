// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n116_, new_n119_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  or2    g02(.a(x14), .b(x01), .O(new_n37_));
  inv1   g03(.a(x00), .O(new_n38_));
  inv1   g04(.a(x06), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n37_), .b(new_n36_), .c(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n40_), .O(z01));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n39_), .c(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  inv1   g16(.a(x07), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(x06), .c(new_n38_), .O(new_n52_));
  oai21  g18(.a(new_n51_), .b(x06), .c(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n48_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(new_n40_), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  nor2   g23(.a(new_n51_), .b(new_n39_), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n48_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n56_), .O(z04));
  inv1   g28(.a(x16), .O(new_n63_));
  aoi21  g29(.a(x04), .b(new_n35_), .c(x03), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n63_), .c(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n56_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x08), .c(x07), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n38_), .c(new_n39_), .O(new_n69_));
  nor2   g35(.a(new_n57_), .b(new_n51_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(x09), .c(new_n69_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n66_), .O(z05));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(x00), .c(x06), .O(new_n77_));
  nand3  g43(.a(new_n58_), .b(new_n67_), .c(x08), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x10), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(z06));
  inv1   g46(.a(x11), .O(new_n81_));
  nor2   g47(.a(new_n76_), .b(new_n81_), .O(new_n82_));
  inv1   g48(.a(x10), .O(new_n83_));
  nand3  g49(.a(new_n81_), .b(new_n83_), .c(new_n67_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n71_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n82_), .c(new_n38_), .O(new_n86_));
  nand2  g52(.a(x11), .b(new_n39_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n66_), .O(z07));
  inv1   g54(.a(new_n70_), .O(new_n89_));
  oai21  g55(.a(new_n84_), .b(new_n89_), .c(x12), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand3  g57(.a(new_n74_), .b(new_n91_), .c(new_n81_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n71_), .c(new_n90_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n38_), .O(new_n94_));
  nand2  g60(.a(x12), .b(new_n39_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n66_), .O(z08));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n48_), .b(new_n97_), .c(new_n91_), .d(new_n81_), .O(new_n98_));
  nor3   g64(.a(new_n98_), .b(x10), .c(x09), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x08), .c(x07), .d(x06), .O(new_n100_));
  nand3  g66(.a(new_n91_), .b(new_n81_), .c(new_n83_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n68_), .c(x13), .O(new_n102_));
  oai21  g68(.a(new_n100_), .b(new_n45_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n38_), .O(new_n104_));
  nand2  g70(.a(x13), .b(new_n39_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n66_), .O(z09));
  inv1   g72(.a(x03), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x02), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n56_), .c(x16), .d(x14), .O(new_n109_));
  inv1   g75(.a(new_n100_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n38_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n109_), .c(new_n45_), .O(z10));
  nor2   g78(.a(new_n40_), .b(new_n35_), .O(z11));
  nand3  g79(.a(new_n56_), .b(x16), .c(new_n35_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n107_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n56_), .O(z12));
  nand2  g83(.a(new_n56_), .b(new_n45_), .O(z13));
  nand3  g84(.a(new_n56_), .b(x17), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z14));
endmodule


