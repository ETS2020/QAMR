// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x05), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x15), .O(new_n41_));
  nand3  g07(.a(x18), .b(new_n41_), .c(new_n36_), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  oai21  g09(.a(new_n40_), .b(x04), .c(new_n43_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n36_), .c(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z02));
  inv1   g13(.a(x07), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(x06), .c(x05), .O(new_n49_));
  oai21  g15(.a(new_n48_), .b(x06), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(new_n37_), .O(new_n53_));
  oai21  g19(.a(new_n48_), .b(new_n36_), .c(x08), .O(new_n54_));
  inv1   g20(.a(x08), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(x07), .c(x06), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n45_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n53_), .O(z04));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(x04), .b(new_n35_), .c(x03), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n53_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x08), .c(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x05), .O(new_n66_));
  nand3  g32(.a(x08), .b(x07), .c(x06), .O(new_n67_));
  aoi22  g33(.a(new_n67_), .b(x09), .c(new_n66_), .d(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n63_), .O(z05));
  nand2  g35(.a(new_n65_), .b(x10), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(x08), .c(x07), .d(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x05), .O(new_n74_));
  nand2  g40(.a(x10), .b(new_n36_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n63_), .O(z06));
  inv1   g42(.a(x10), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n64_), .O(new_n79_));
  nor3   g45(.a(new_n79_), .b(new_n55_), .c(new_n48_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n40_), .c(x06), .O(new_n81_));
  nand2  g47(.a(new_n72_), .b(x11), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n63_), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n71_), .b(new_n84_), .c(new_n78_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  oai21  g53(.a(new_n80_), .b(new_n84_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x05), .O(new_n89_));
  nand2  g55(.a(x12), .b(new_n36_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n63_), .O(z08));
  inv1   g57(.a(x04), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n45_), .b(new_n93_), .c(new_n84_), .d(new_n78_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n64_), .c(x08), .d(x07), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n92_), .c(x05), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x06), .O(new_n98_));
  nand2  g64(.a(new_n87_), .b(x13), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n98_), .c(new_n63_), .O(z09));
  inv1   g66(.a(x03), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x02), .c(new_n60_), .O(new_n102_));
  inv1   g68(.a(x00), .O(new_n103_));
  nand4  g69(.a(new_n93_), .b(new_n84_), .c(new_n78_), .d(new_n77_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n64_), .c(x08), .d(x07), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n36_), .O(new_n107_));
  aoi21  g73(.a(new_n102_), .b(x14), .c(new_n107_), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n92_), .c(new_n53_), .O(z10));
  nand2  g75(.a(new_n53_), .b(new_n35_), .O(z11));
  oai21  g76(.a(new_n60_), .b(x02), .c(new_n101_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n53_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  nand2  g79(.a(new_n53_), .b(new_n92_), .O(z13));
  inv1   g80(.a(x17), .O(new_n115_));
  nor3   g81(.a(new_n37_), .b(new_n115_), .c(new_n92_), .O(z14));
endmodule


