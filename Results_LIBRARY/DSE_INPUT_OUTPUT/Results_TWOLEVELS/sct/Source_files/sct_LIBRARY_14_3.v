// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n109_, new_n110_, new_n113_;
  inv1   g00(.a(x00), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(x01), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  nand2  g04(.a(x01), .b(x00), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n39_), .c(new_n47_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n39_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n39_), .O(z04));
  aoi21  g27(.a(new_n49_), .b(x16), .c(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  nand2  g29(.a(x16), .b(new_n36_), .O(new_n64_));
  oai21  g30(.a(new_n63_), .b(new_n35_), .c(new_n64_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nand2  g33(.a(x16), .b(x03), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(x04), .O(new_n69_));
  aoi21  g35(.a(new_n65_), .b(x04), .c(new_n69_), .O(new_n70_));
  oai21  g36(.a(new_n44_), .b(x02), .c(new_n48_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  inv1   g39(.a(x09), .O(new_n74_));
  nand3  g40(.a(new_n57_), .b(new_n74_), .c(x08), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n67_), .c(x04), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n73_), .c(new_n35_), .O(new_n77_));
  oai21  g43(.a(new_n70_), .b(x01), .c(new_n77_), .O(z05));
  oai21  g44(.a(new_n73_), .b(new_n44_), .c(new_n39_), .O(new_n79_));
  nand2  g45(.a(new_n75_), .b(x10), .O(new_n80_));
  nor2   g46(.a(x10), .b(x09), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n57_), .c(x08), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n80_), .c(new_n79_), .d(new_n39_), .O(z06));
  inv1   g49(.a(x10), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(new_n66_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n74_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x04), .O(new_n88_));
  aoi21  g54(.a(new_n82_), .b(x11), .c(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n72_), .c(new_n40_), .O(z07));
  nand2  g56(.a(new_n87_), .b(x12), .O(new_n91_));
  nor2   g57(.a(x12), .b(x11), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n81_), .c(new_n86_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n91_), .c(new_n79_), .d(new_n39_), .O(z08));
  nand2  g60(.a(new_n93_), .b(x13), .O(new_n95_));
  inv1   g61(.a(new_n75_), .O(new_n96_));
  nor2   g62(.a(x13), .b(x12), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n96_), .c(new_n85_), .d(new_n84_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n95_), .c(new_n79_), .d(new_n39_), .O(z09));
  nand4  g65(.a(new_n97_), .b(new_n85_), .c(new_n84_), .d(new_n35_), .O(new_n100_));
  oai21  g66(.a(x01), .b(new_n35_), .c(new_n100_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n50_), .c(new_n74_), .d(x08), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(x07), .c(x06), .O(new_n104_));
  nand4  g70(.a(new_n49_), .b(new_n39_), .c(x16), .d(x14), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n44_), .O(z10));
  nand2  g72(.a(new_n39_), .b(new_n36_), .O(z11));
  nand3  g73(.a(new_n39_), .b(x16), .c(new_n36_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n48_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n39_), .O(z12));
  nor2   g77(.a(new_n40_), .b(new_n44_), .O(z13));
  nand3  g78(.a(new_n39_), .b(x17), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z14));
endmodule


