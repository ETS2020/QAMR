// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x12), .b(x11), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(new_n36_), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n46_), .c(new_n45_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n46_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n50_), .c(new_n46_), .d(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  oai21  g27(.a(new_n42_), .b(x02), .c(new_n47_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x16), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n57_), .b(new_n68_), .c(x08), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(new_n46_), .O(z05));
  nand2  g36(.a(new_n69_), .b(x10), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n63_), .c(x04), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nor2   g39(.a(x12), .b(x11), .O(new_n74_));
  nor3   g40(.a(new_n74_), .b(x10), .c(x09), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x08), .c(x07), .d(x06), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n42_), .c(new_n73_), .O(z06));
  nand3  g43(.a(x08), .b(x07), .c(x06), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  nor2   g45(.a(x10), .b(x09), .O(new_n80_));
  oai21  g46(.a(new_n79_), .b(new_n49_), .c(new_n80_), .O(new_n81_));
  inv1   g47(.a(x10), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand4  g49(.a(x12), .b(new_n83_), .c(new_n82_), .d(new_n68_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  aoi21  g51(.a(new_n81_), .b(x11), .c(new_n85_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n42_), .c(new_n65_), .O(z07));
  inv1   g53(.a(x12), .O(new_n88_));
  aoi21  g54(.a(new_n81_), .b(x04), .c(x11), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n88_), .c(new_n65_), .O(z08));
  nor2   g56(.a(x13), .b(new_n42_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n63_), .c(new_n36_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n47_), .b(x02), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(x16), .c(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  nand2  g62(.a(new_n49_), .b(x14), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n93_), .c(new_n97_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n46_), .c(x04), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(z10));
  nor2   g66(.a(new_n36_), .b(new_n35_), .O(z11));
  nand2  g67(.a(x16), .b(new_n35_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n46_), .c(x04), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(z12));
  nor2   g71(.a(new_n79_), .b(new_n49_), .O(new_n106_));
  nand4  g72(.a(new_n102_), .b(new_n82_), .c(new_n68_), .d(new_n47_), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n106_), .c(new_n46_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n76_), .c(new_n42_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n46_), .O(z14));
endmodule


