// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand3  g01(.a(x18), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x05), .O(new_n38_));
  inv1   g04(.a(x18), .O(new_n39_));
  oai21  g05(.a(new_n39_), .b(x15), .c(new_n38_), .O(new_n40_));
  nand2  g06(.a(new_n39_), .b(x01), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(new_n41_), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n41_), .O(z03));
  inv1   g17(.a(x08), .O(new_n52_));
  and2   g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n52_), .b(x07), .c(x06), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(new_n46_), .c(new_n41_), .d(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  inv1   g23(.a(x03), .O(new_n58_));
  inv1   g24(.a(x16), .O(new_n59_));
  nand2  g25(.a(x04), .b(new_n35_), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(new_n58_), .c(new_n59_), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n42_), .c(new_n41_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n53_), .b(new_n65_), .c(x08), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n64_), .c(new_n62_), .d(new_n41_), .O(z05));
  inv1   g33(.a(new_n41_), .O(new_n68_));
  inv1   g34(.a(new_n61_), .O(new_n69_));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n53_), .c(x08), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x04), .O(new_n72_));
  aoi21  g38(.a(new_n66_), .b(x10), .c(new_n72_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n69_), .c(new_n68_), .O(z06));
  nand2  g40(.a(new_n71_), .b(x11), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  inv1   g43(.a(new_n63_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n65_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n75_), .c(new_n62_), .d(new_n41_), .O(z07));
  nand2  g46(.a(new_n79_), .b(x12), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n70_), .b(new_n78_), .c(new_n82_), .d(new_n77_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n81_), .c(new_n62_), .d(new_n41_), .O(z08));
  inv1   g50(.a(x13), .O(new_n85_));
  nand4  g51(.a(new_n46_), .b(new_n85_), .c(new_n82_), .d(new_n77_), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(x10), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n65_), .c(x08), .d(x07), .O(new_n88_));
  nand2  g54(.a(x16), .b(new_n35_), .O(new_n89_));
  oai21  g55(.a(new_n88_), .b(new_n45_), .c(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x04), .O(new_n91_));
  aoi21  g57(.a(x16), .b(x03), .c(new_n42_), .O(new_n92_));
  nand2  g58(.a(new_n83_), .b(x13), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n41_), .O(z09));
  inv1   g60(.a(x14), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  nand4  g62(.a(new_n85_), .b(new_n82_), .c(new_n77_), .d(new_n76_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  and2   g64(.a(new_n98_), .b(new_n46_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n65_), .c(x08), .d(x07), .O(new_n100_));
  oai22  g66(.a(new_n100_), .b(new_n45_), .c(new_n46_), .d(new_n95_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n41_), .c(x04), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(z10));
  nand2  g69(.a(new_n41_), .b(new_n35_), .O(z11));
  nand2  g70(.a(new_n89_), .b(new_n58_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n41_), .c(x04), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(z12));
  nand2  g73(.a(new_n41_), .b(new_n42_), .O(z13));
  nand2  g74(.a(new_n41_), .b(x17), .O(new_n109_));
  nor2   g75(.a(new_n109_), .b(new_n42_), .O(z14));
endmodule


