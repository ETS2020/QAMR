// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n119_;
  nor2   g00(.a(x17), .b(x03), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  or2    g02(.a(x14), .b(x01), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n35_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  oai21  g14(.a(x17), .b(x03), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  nand3  g16(.a(x17), .b(new_n50_), .c(x02), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  oai21  g23(.a(x03), .b(new_n38_), .c(x16), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n58_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n36_), .O(z04));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n59_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n60_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n36_), .O(new_n70_));
  inv1   g36(.a(x17), .O(new_n71_));
  nor3   g37(.a(new_n71_), .b(new_n44_), .c(x02), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(x03), .c(x16), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n70_), .O(z05));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n60_), .c(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  aoi21  g43(.a(new_n68_), .b(x10), .c(new_n77_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n35_), .c(new_n73_), .O(z06));
  inv1   g45(.a(new_n65_), .O(new_n80_));
  nor3   g46(.a(x11), .b(x10), .c(x09), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  aoi21  g49(.a(new_n76_), .b(x11), .c(new_n83_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n35_), .c(new_n73_), .O(z07));
  oai21  g51(.a(x16), .b(new_n44_), .c(x03), .O(new_n86_));
  oai21  g52(.a(new_n48_), .b(x02), .c(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x17), .O(new_n88_));
  nand2  g54(.a(new_n82_), .b(x12), .O(new_n89_));
  nor2   g55(.a(x12), .b(x11), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n75_), .c(new_n80_), .O(new_n91_));
  and2   g57(.a(new_n91_), .b(new_n36_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n89_), .c(new_n88_), .d(new_n86_), .O(z08));
  nand3  g59(.a(new_n80_), .b(x04), .c(x02), .O(new_n94_));
  inv1   g60(.a(x12), .O(new_n95_));
  nor2   g61(.a(new_n71_), .b(x13), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n81_), .c(new_n95_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n94_), .c(x17), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n50_), .O(new_n99_));
  nand4  g65(.a(new_n67_), .b(x07), .c(x06), .d(x04), .O(new_n100_));
  inv1   g66(.a(x13), .O(new_n101_));
  nor2   g67(.a(x11), .b(x10), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n48_), .c(new_n101_), .d(new_n95_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n100_), .c(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n36_), .O(new_n105_));
  nand2  g71(.a(new_n91_), .b(x13), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n105_), .c(new_n99_), .d(new_n73_), .O(z09));
  inv1   g73(.a(x09), .O(new_n108_));
  inv1   g74(.a(x00), .O(new_n109_));
  nand3  g75(.a(new_n102_), .b(new_n101_), .c(new_n95_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n52_), .c(new_n108_), .d(x08), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(x07), .c(x06), .O(new_n114_));
  oai21  g80(.a(new_n71_), .b(x02), .c(new_n50_), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(x16), .c(x14), .O(new_n116_));
  aoi21  g82(.a(new_n116_), .b(new_n114_), .c(new_n44_), .O(z10));
  nor2   g83(.a(new_n35_), .b(new_n38_), .O(z11));
  nand3  g84(.a(x17), .b(x16), .c(new_n38_), .O(new_n119_));
  aoi21  g85(.a(new_n119_), .b(new_n50_), .c(new_n44_), .O(z12));
  nor2   g86(.a(new_n35_), .b(new_n44_), .O(z13));
  oai21  g87(.a(new_n71_), .b(new_n44_), .c(new_n36_), .O(z14));
endmodule


