// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_;
  nor2   g00(.a(x17), .b(x12), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n35_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(new_n35_), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n36_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n56_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  aoi21  g29(.a(x16), .b(new_n36_), .c(new_n45_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  aoi21  g31(.a(new_n63_), .b(x09), .c(new_n65_), .O(new_n66_));
  inv1   g32(.a(x12), .O(new_n67_));
  inv1   g33(.a(x17), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(new_n45_), .c(new_n68_), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n58_), .b(new_n70_), .c(x08), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  and2   g38(.a(x16), .b(x03), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n72_), .c(new_n69_), .O(new_n74_));
  oai21  g40(.a(new_n66_), .b(new_n35_), .c(new_n74_), .O(z05));
  nand2  g41(.a(new_n65_), .b(new_n49_), .O(new_n76_));
  nand3  g42(.a(new_n69_), .b(x16), .c(x03), .O(new_n77_));
  nand2  g43(.a(new_n71_), .b(x10), .O(new_n78_));
  nor4   g44(.a(new_n57_), .b(x10), .c(x09), .d(new_n56_), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n35_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(z06));
  inv1   g47(.a(new_n79_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x11), .O(new_n83_));
  inv1   g49(.a(new_n63_), .O(new_n84_));
  nor2   g50(.a(x11), .b(x10), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n70_), .O(new_n86_));
  and2   g52(.a(new_n86_), .b(new_n49_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n83_), .c(new_n77_), .d(new_n76_), .O(z07));
  inv1   g54(.a(new_n85_), .O(new_n89_));
  nand3  g55(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n70_), .c(new_n45_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n89_), .c(x12), .O(new_n93_));
  nand4  g59(.a(new_n85_), .b(new_n72_), .c(x17), .d(new_n67_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n77_), .d(new_n76_), .O(z08));
  inv1   g61(.a(x11), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n50_), .b(new_n97_), .c(new_n67_), .d(new_n96_), .O(new_n98_));
  nor3   g64(.a(new_n98_), .b(x10), .c(x09), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x08), .c(x07), .d(x06), .O(new_n100_));
  nand2  g66(.a(new_n86_), .b(x13), .O(new_n101_));
  oai21  g67(.a(new_n100_), .b(new_n45_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x17), .O(new_n103_));
  nand2  g69(.a(x13), .b(x12), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n103_), .c(new_n77_), .d(new_n76_), .O(z09));
  inv1   g71(.a(x00), .O(new_n106_));
  and2   g72(.a(new_n50_), .b(new_n70_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(x08), .c(x07), .d(x06), .O(new_n108_));
  oai22  g74(.a(new_n108_), .b(new_n106_), .c(new_n50_), .d(new_n39_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n49_), .O(new_n110_));
  nand4  g76(.a(new_n50_), .b(x17), .c(new_n97_), .d(new_n67_), .O(new_n111_));
  nor3   g77(.a(new_n111_), .b(x11), .c(x10), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n70_), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(new_n114_));
  nand4  g80(.a(new_n114_), .b(x08), .c(x07), .d(x06), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n110_), .c(new_n45_), .O(z10));
  nor2   g82(.a(new_n35_), .b(new_n36_), .O(z11));
  inv1   g83(.a(x16), .O(new_n118_));
  nor3   g84(.a(new_n35_), .b(new_n118_), .c(x02), .O(new_n119_));
  oai21  g85(.a(new_n119_), .b(x03), .c(x04), .O(new_n120_));
  nand2  g86(.a(new_n120_), .b(new_n49_), .O(z12));
  nand4  g87(.a(new_n91_), .b(new_n71_), .c(new_n50_), .d(new_n70_), .O(new_n122_));
  nand2  g88(.a(new_n122_), .b(x12), .O(new_n123_));
  aoi21  g89(.a(new_n123_), .b(new_n68_), .c(new_n45_), .O(z13));
  nor2   g90(.a(new_n68_), .b(new_n45_), .O(z14));
endmodule


