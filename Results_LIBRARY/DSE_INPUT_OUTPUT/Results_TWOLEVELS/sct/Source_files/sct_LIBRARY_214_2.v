// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x01), .O(new_n35_));
  oai21  g01(.a(x02), .b(new_n35_), .c(x14), .O(z00));
  nor2   g02(.a(x14), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x05), .O(new_n38_));
  inv1   g04(.a(x18), .O(new_n39_));
  oai21  g05(.a(new_n39_), .b(x15), .c(new_n38_), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  nand2  g07(.a(x05), .b(new_n41_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n40_), .c(new_n37_), .O(z01));
  inv1   g09(.a(new_n37_), .O(new_n44_));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n45_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n44_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n44_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  and2   g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n55_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n49_), .c(new_n44_), .d(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n56_), .b(new_n63_), .c(x08), .O(new_n64_));
  inv1   g30(.a(x02), .O(new_n65_));
  nand3  g31(.a(x16), .b(x04), .c(new_n65_), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n64_), .c(new_n62_), .d(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n44_), .O(new_n68_));
  inv1   g34(.a(x14), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n41_), .c(x01), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x16), .c(x03), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n68_), .O(z05));
  and2   g38(.a(new_n64_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n56_), .c(x08), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n66_), .c(x04), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n73_), .c(new_n44_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n71_), .O(z06));
  inv1   g44(.a(x10), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(new_n61_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n63_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n66_), .c(x04), .O(new_n83_));
  aoi21  g49(.a(new_n75_), .b(x11), .c(new_n83_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n37_), .c(new_n71_), .O(z07));
  and2   g51(.a(new_n82_), .b(x12), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n74_), .b(new_n81_), .c(new_n87_), .d(new_n80_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n66_), .c(x04), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n86_), .c(new_n44_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n71_), .O(z08));
  nand2  g57(.a(new_n88_), .b(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n66_), .c(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n44_), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  oai21  g61(.a(new_n48_), .b(x01), .c(new_n69_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n95_), .c(new_n87_), .d(new_n80_), .O(new_n97_));
  inv1   g63(.a(new_n97_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n79_), .c(new_n63_), .d(x08), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(x07), .c(x06), .d(x04), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n94_), .c(new_n71_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  nand4  g69(.a(new_n95_), .b(new_n87_), .c(new_n80_), .d(new_n79_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n96_), .c(new_n63_), .d(x08), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(x07), .c(x06), .O(new_n108_));
  nand2  g74(.a(new_n48_), .b(x14), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n41_), .O(z10));
  nor2   g76(.a(new_n37_), .b(new_n65_), .O(z11));
  oai21  g77(.a(new_n47_), .b(x02), .c(new_n46_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n44_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  nand2  g80(.a(new_n44_), .b(new_n41_), .O(z13));
  nand2  g81(.a(x17), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n44_), .O(z14));
endmodule


