// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n112_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(x15), .b(new_n36_), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  nand2  g07(.a(x15), .b(new_n35_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(new_n42_), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n46_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  inv1   g23(.a(x07), .O(new_n58_));
  nor2   g24(.a(new_n58_), .b(new_n46_), .O(new_n59_));
  nand3  g25(.a(new_n57_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(new_n49_), .c(new_n42_), .d(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  nor2   g29(.a(new_n43_), .b(x02), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(x03), .c(x16), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n59_), .b(new_n68_), .c(x08), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n42_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(x04), .O(z05));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n59_), .c(x08), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n42_), .c(x04), .O(new_n74_));
  aoi21  g40(.a(new_n69_), .b(x10), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n65_), .O(z06));
  nand2  g42(.a(new_n73_), .b(x11), .O(new_n77_));
  nor4   g43(.a(new_n66_), .b(x11), .c(x10), .d(x09), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n47_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n77_), .c(new_n65_), .d(x04), .O(z07));
  inv1   g46(.a(new_n78_), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  inv1   g49(.a(new_n66_), .O(new_n84_));
  nand4  g50(.a(new_n72_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n42_), .c(x04), .O(new_n86_));
  aoi21  g52(.a(new_n81_), .b(x12), .c(new_n86_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n65_), .O(z08));
  inv1   g54(.a(x10), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n49_), .b(new_n42_), .c(new_n90_), .d(new_n83_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(x11), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n89_), .c(new_n68_), .d(x08), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n58_), .O(new_n94_));
  inv1   g60(.a(x16), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(x02), .O(new_n96_));
  aoi21  g62(.a(new_n94_), .b(x06), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n85_), .b(x13), .O(new_n98_));
  aoi21  g64(.a(x16), .b(x03), .c(new_n47_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x04), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  nand4  g67(.a(new_n90_), .b(new_n83_), .c(new_n82_), .d(new_n89_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n50_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n68_), .c(x08), .d(x07), .O(new_n104_));
  oai22  g70(.a(new_n104_), .b(new_n46_), .c(new_n49_), .d(new_n36_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n42_), .c(x04), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(z10));
  nor2   g73(.a(new_n47_), .b(new_n48_), .O(z11));
  oai21  g74(.a(new_n96_), .b(x03), .c(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n42_), .O(z12));
  nand2  g76(.a(new_n42_), .b(new_n43_), .O(z13));
  nand2  g77(.a(x17), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n42_), .O(z14));
endmodule


