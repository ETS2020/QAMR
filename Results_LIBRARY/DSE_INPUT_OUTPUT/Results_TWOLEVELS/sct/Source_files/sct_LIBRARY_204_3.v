// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x02), .O(new_n35_));
  oai21  g01(.a(x05), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x04), .O(new_n38_));
  oai21  g04(.a(new_n38_), .b(x01), .c(x05), .O(new_n39_));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n39_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(x04), .O(new_n46_));
  inv1   g12(.a(x01), .O(new_n47_));
  nor2   g13(.a(new_n40_), .b(new_n47_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n45_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n49_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n45_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n49_), .O(z04));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(x04), .b(new_n35_), .c(x03), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n55_), .b(new_n63_), .c(x08), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n62_), .c(new_n49_), .O(new_n68_));
  oai21  g34(.a(new_n40_), .b(new_n47_), .c(new_n38_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n68_), .O(z05));
  nand2  g36(.a(new_n66_), .b(x10), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n55_), .c(x08), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n62_), .c(new_n49_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n69_), .O(z06));
  nand2  g42(.a(new_n73_), .b(x11), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand4  g45(.a(new_n65_), .b(new_n79_), .c(new_n78_), .d(new_n63_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n62_), .c(new_n49_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n69_), .O(z07));
  inv1   g49(.a(new_n61_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n49_), .c(x16), .O(new_n85_));
  nand2  g51(.a(new_n80_), .b(x12), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand3  g53(.a(new_n72_), .b(new_n87_), .c(new_n79_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n65_), .c(new_n48_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n86_), .c(new_n85_), .d(new_n69_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n45_), .b(new_n92_), .c(new_n87_), .d(new_n79_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(x10), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n63_), .c(x08), .d(x07), .O(new_n95_));
  nand2  g61(.a(x16), .b(new_n35_), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n44_), .c(new_n96_), .O(new_n97_));
  inv1   g63(.a(x03), .O(new_n98_));
  nor2   g64(.a(new_n60_), .b(new_n98_), .O(new_n99_));
  aoi21  g65(.a(new_n97_), .b(x04), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n88_), .b(new_n64_), .c(x13), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n100_), .c(new_n69_), .d(new_n49_), .O(z09));
  inv1   g68(.a(x14), .O(new_n103_));
  inv1   g69(.a(x00), .O(new_n104_));
  nand4  g70(.a(new_n92_), .b(new_n87_), .c(new_n79_), .d(new_n78_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  and2   g72(.a(new_n106_), .b(new_n45_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n63_), .c(x08), .d(x07), .O(new_n108_));
  oai22  g74(.a(new_n108_), .b(new_n44_), .c(new_n45_), .d(new_n103_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(new_n49_), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z10));
  nor2   g77(.a(new_n48_), .b(new_n35_), .O(z11));
  nand3  g78(.a(new_n49_), .b(x16), .c(new_n35_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n98_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n49_), .O(z12));
  nor2   g82(.a(new_n48_), .b(new_n38_), .O(z13));
  nand3  g83(.a(new_n49_), .b(x17), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


