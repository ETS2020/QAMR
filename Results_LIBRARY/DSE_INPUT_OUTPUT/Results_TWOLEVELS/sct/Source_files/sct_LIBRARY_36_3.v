// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n122_;
  inv1   g00(.a(x07), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  nand2  g14(.a(x10), .b(x07), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n38_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand3  g18(.a(new_n36_), .b(x07), .c(new_n48_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n48_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(new_n37_), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(x07), .b(x06), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n58_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n50_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n57_), .O(z04));
  inv1   g30(.a(x16), .O(new_n65_));
  aoi21  g31(.a(x04), .b(new_n38_), .c(x03), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n65_), .c(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n57_), .O(new_n68_));
  nor2   g34(.a(new_n58_), .b(new_n48_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(x10), .c(x07), .O(new_n70_));
  nor4   g36(.a(new_n59_), .b(x10), .c(x09), .d(new_n58_), .O(new_n71_));
  aoi21  g37(.a(new_n70_), .b(x09), .c(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n68_), .O(z05));
  nand3  g39(.a(x16), .b(new_n36_), .c(new_n38_), .O(new_n74_));
  oai21  g40(.a(new_n36_), .b(x07), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g42(.a(new_n57_), .b(new_n45_), .O(new_n77_));
  inv1   g43(.a(x03), .O(new_n78_));
  inv1   g44(.a(x09), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x08), .O(new_n80_));
  oai22  g46(.a(new_n80_), .b(new_n59_), .c(new_n65_), .d(new_n78_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n36_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n77_), .c(new_n76_), .O(z06));
  oai21  g49(.a(new_n80_), .b(new_n48_), .c(x11), .O(new_n84_));
  nor2   g50(.a(x11), .b(x09), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n60_), .c(x08), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n36_), .O(new_n88_));
  nand2  g54(.a(x11), .b(new_n35_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n68_), .O(z07));
  inv1   g56(.a(x12), .O(new_n91_));
  aoi21  g57(.a(new_n85_), .b(new_n69_), .c(new_n91_), .O(new_n92_));
  inv1   g58(.a(x11), .O(new_n93_));
  nand3  g59(.a(x08), .b(x07), .c(x06), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n91_), .c(new_n93_), .d(new_n79_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n92_), .c(new_n36_), .O(new_n98_));
  nand2  g64(.a(x12), .b(new_n35_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n98_), .c(new_n68_), .O(z08));
  inv1   g66(.a(x13), .O(new_n101_));
  nand4  g67(.a(new_n50_), .b(new_n101_), .c(new_n91_), .d(new_n93_), .O(new_n102_));
  nor2   g68(.a(new_n102_), .b(x09), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(x08), .c(x06), .d(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n36_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x07), .O(new_n106_));
  nand2  g72(.a(new_n96_), .b(x13), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n106_), .c(new_n68_), .O(z09));
  inv1   g74(.a(new_n50_), .O(new_n109_));
  inv1   g75(.a(x00), .O(new_n110_));
  nand3  g76(.a(new_n101_), .b(new_n91_), .c(new_n93_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g78(.a(new_n112_), .b(new_n50_), .c(new_n79_), .d(x08), .O(new_n113_));
  nor2   g79(.a(new_n113_), .b(new_n35_), .O(new_n114_));
  aoi22  g80(.a(new_n114_), .b(x06), .c(new_n109_), .d(x14), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n45_), .c(new_n57_), .O(z10));
  nor2   g82(.a(new_n37_), .b(new_n38_), .O(z11));
  oai21  g83(.a(new_n65_), .b(x02), .c(new_n78_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n57_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  aoi21  g86(.a(x10), .b(x07), .c(new_n45_), .O(z13));
  nand3  g87(.a(new_n57_), .b(x17), .c(x04), .O(new_n122_));
  inv1   g88(.a(new_n122_), .O(z14));
endmodule


