// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n115_,
    new_n116_, new_n118_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  inv1   g02(.a(x16), .O(new_n37_));
  nand3  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand2  g10(.a(x16), .b(new_n35_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(x02), .c(x01), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x02), .O(new_n54_));
  oai21  g20(.a(x03), .b(new_n54_), .c(x16), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n45_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n55_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n45_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n59_), .b(new_n66_), .c(x08), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n45_), .O(new_n69_));
  oai21  g35(.a(new_n43_), .b(x02), .c(new_n48_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x16), .c(x01), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n69_), .O(z05));
  nand2  g38(.a(new_n45_), .b(new_n43_), .O(z13));
  oai21  g39(.a(new_n70_), .b(new_n35_), .c(x16), .O(new_n74_));
  nand2  g40(.a(new_n67_), .b(x10), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n59_), .c(x08), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n75_), .c(new_n74_), .d(z13), .O(z06));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n64_), .O(new_n81_));
  aoi21  g47(.a(new_n77_), .b(x11), .c(new_n81_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n74_), .c(z13), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nor2   g50(.a(new_n81_), .b(new_n84_), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  inv1   g52(.a(new_n64_), .O(new_n87_));
  nand4  g53(.a(new_n76_), .b(new_n87_), .c(new_n84_), .d(new_n86_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n85_), .c(new_n45_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n71_), .O(z08));
  nor2   g57(.a(x03), .b(new_n54_), .O(new_n92_));
  nor3   g58(.a(x13), .b(x12), .c(x11), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n76_), .c(new_n87_), .d(new_n92_), .O(new_n94_));
  nand2  g60(.a(x16), .b(new_n54_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n94_), .c(new_n35_), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n79_), .b(new_n37_), .c(new_n97_), .d(new_n84_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n67_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n96_), .c(x04), .O(new_n100_));
  aoi21  g66(.a(new_n48_), .b(x01), .c(new_n37_), .O(new_n101_));
  aoi21  g67(.a(new_n88_), .b(x13), .c(new_n101_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n100_), .c(z13), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(x10), .O(new_n105_));
  nand4  g71(.a(new_n97_), .b(new_n84_), .c(new_n86_), .d(new_n105_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n50_), .c(new_n66_), .d(x08), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(x07), .c(x06), .O(new_n110_));
  nor2   g76(.a(new_n55_), .b(new_n36_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x01), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n110_), .c(new_n43_), .O(z10));
  nand2  g79(.a(new_n45_), .b(new_n54_), .O(z11));
  oai21  g80(.a(new_n95_), .b(new_n35_), .c(new_n48_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n45_), .O(z12));
  nand2  g83(.a(x17), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n45_), .O(z14));
endmodule


