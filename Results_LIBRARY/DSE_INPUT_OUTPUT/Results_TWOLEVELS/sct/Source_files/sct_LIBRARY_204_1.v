// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(new_n36_), .b(x05), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g04(.a(x04), .O(new_n39_));
  oai21  g05(.a(new_n39_), .b(new_n35_), .c(x05), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(new_n40_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(x04), .O(new_n48_));
  nor2   g14(.a(new_n41_), .b(x01), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n50_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  and2   g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n55_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n47_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n50_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n46_), .c(x03), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand3  g32(.a(new_n56_), .b(new_n64_), .c(x08), .O(new_n67_));
  oai21  g33(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n63_), .c(new_n50_), .O(new_n69_));
  oai21  g35(.a(new_n41_), .b(x01), .c(new_n39_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(z05));
  nand2  g37(.a(new_n67_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n56_), .c(x08), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n63_), .c(new_n50_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n70_), .O(z06));
  nand2  g43(.a(new_n74_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x10), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  nand4  g46(.a(new_n66_), .b(new_n80_), .c(new_n79_), .d(new_n64_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n63_), .c(new_n50_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n70_), .O(z07));
  inv1   g50(.a(new_n62_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n50_), .c(x16), .O(new_n86_));
  nand2  g52(.a(new_n81_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n73_), .b(new_n88_), .c(new_n80_), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n66_), .c(new_n49_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n87_), .c(new_n86_), .d(new_n70_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n47_), .b(new_n93_), .c(new_n88_), .d(new_n80_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n64_), .c(x08), .d(x07), .O(new_n96_));
  nand2  g62(.a(x16), .b(new_n46_), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n45_), .c(new_n97_), .O(new_n98_));
  and2   g64(.a(x16), .b(x03), .O(new_n99_));
  aoi21  g65(.a(new_n98_), .b(x04), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n89_), .b(new_n65_), .c(x13), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n100_), .c(new_n70_), .d(new_n50_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  nand4  g69(.a(new_n93_), .b(new_n88_), .c(new_n80_), .d(new_n79_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  and2   g71(.a(new_n105_), .b(new_n47_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n64_), .c(x08), .d(x07), .O(new_n107_));
  oai22  g73(.a(new_n107_), .b(new_n45_), .c(new_n47_), .d(new_n36_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(new_n50_), .c(x04), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(z10));
  nor2   g76(.a(new_n49_), .b(new_n46_), .O(z11));
  nor2   g77(.a(new_n49_), .b(new_n61_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n46_), .c(x03), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(new_n39_), .c(new_n50_), .O(z12));
  nor2   g80(.a(new_n49_), .b(new_n39_), .O(z13));
  inv1   g81(.a(x17), .O(new_n116_));
  nor3   g82(.a(new_n49_), .b(new_n116_), .c(new_n39_), .O(z14));
endmodule


