// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_;
  nor2   g00(.a(x15), .b(x11), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(z00));
  aoi21  g07(.a(x11), .b(x05), .c(x15), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(x11), .c(x15), .O(new_n44_));
  oai22  g10(.a(new_n44_), .b(x05), .c(new_n42_), .d(x04), .O(z01));
  inv1   g11(.a(new_n35_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n46_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n46_), .O(z04));
  inv1   g25(.a(x04), .O(new_n60_));
  inv1   g26(.a(x03), .O(new_n61_));
  inv1   g27(.a(x16), .O(new_n62_));
  nand2  g28(.a(x04), .b(new_n36_), .O(new_n63_));
  aoi21  g29(.a(new_n63_), .b(new_n61_), .c(new_n62_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n60_), .c(new_n46_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n55_), .b(new_n68_), .c(x08), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(new_n46_), .O(z05));
  nand2  g36(.a(new_n69_), .b(x10), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x04), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n64_), .c(new_n46_), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(x15), .O(new_n75_));
  nand2  g41(.a(x11), .b(x04), .O(new_n76_));
  oai21  g42(.a(new_n75_), .b(x11), .c(new_n76_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n74_), .c(new_n68_), .d(x08), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(x07), .c(x06), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n73_), .O(z06));
  nand3  g47(.a(x11), .b(x10), .c(x04), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand3  g49(.a(x15), .b(new_n83_), .c(new_n74_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n66_), .c(new_n82_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n68_), .O(new_n86_));
  nand3  g52(.a(new_n69_), .b(x11), .c(x04), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n65_), .O(z07));
  inv1   g54(.a(x12), .O(new_n89_));
  inv1   g55(.a(new_n66_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n74_), .d(new_n68_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x15), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n83_), .O(new_n93_));
  nor2   g59(.a(x11), .b(x10), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n68_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n66_), .c(x12), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n93_), .c(new_n65_), .O(z08));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n48_), .b(new_n98_), .c(new_n89_), .d(new_n83_), .O(new_n99_));
  nor3   g65(.a(new_n99_), .b(x10), .c(x09), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(x08), .c(x07), .d(x06), .O(new_n101_));
  nand2  g67(.a(new_n91_), .b(x13), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n60_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x15), .O(new_n104_));
  nand2  g70(.a(x13), .b(x11), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n65_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  nand4  g73(.a(new_n94_), .b(x15), .c(new_n98_), .d(new_n89_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  and2   g75(.a(new_n109_), .b(new_n48_), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(new_n68_), .c(x08), .d(x07), .O(new_n111_));
  oai22  g77(.a(new_n111_), .b(new_n47_), .c(new_n48_), .d(new_n39_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n46_), .O(z10));
  nor2   g80(.a(new_n35_), .b(new_n36_), .O(z11));
  oai21  g81(.a(new_n62_), .b(x02), .c(new_n61_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n46_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  aoi21  g84(.a(new_n55_), .b(x08), .c(x10), .O(new_n119_));
  nor2   g85(.a(new_n119_), .b(x09), .O(new_n120_));
  oai21  g86(.a(new_n120_), .b(new_n69_), .c(x11), .O(new_n121_));
  aoi21  g87(.a(new_n121_), .b(new_n75_), .c(new_n60_), .O(z13));
  nand2  g88(.a(x17), .b(x04), .O(new_n123_));
  nand2  g89(.a(new_n123_), .b(new_n46_), .O(z14));
endmodule


