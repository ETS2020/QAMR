// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n113_;
  nor2   g00(.a(x14), .b(x01), .O(z00));
  inv1   g01(.a(x18), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x15), .O(new_n37_));
  inv1   g03(.a(x04), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nor2   g05(.a(x02), .b(new_n39_), .O(new_n40_));
  aoi21  g06(.a(x05), .b(new_n38_), .c(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n37_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g08(.a(new_n40_), .O(new_n43_));
  inv1   g09(.a(x06), .O(new_n44_));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x16), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x02), .c(new_n46_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n44_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n43_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n43_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  inv1   g23(.a(x02), .O(new_n58_));
  nor2   g24(.a(new_n46_), .b(new_n45_), .O(new_n59_));
  oai22  g25(.a(new_n59_), .b(new_n58_), .c(x16), .d(x01), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n57_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  oai21  g28(.a(new_n46_), .b(x02), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n39_), .O(new_n64_));
  oai21  g30(.a(new_n59_), .b(new_n38_), .c(x02), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n55_), .b(new_n68_), .c(x08), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nor2   g36(.a(new_n70_), .b(new_n40_), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n67_), .c(new_n65_), .d(new_n64_), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n55_), .c(x08), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x04), .O(new_n75_));
  aoi21  g41(.a(new_n69_), .b(x10), .c(new_n75_), .O(new_n76_));
  nand3  g42(.a(x04), .b(new_n58_), .c(new_n39_), .O(new_n77_));
  oai21  g43(.a(new_n45_), .b(new_n58_), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x16), .O(new_n79_));
  oai21  g45(.a(new_n76_), .b(new_n40_), .c(new_n79_), .O(z06));
  inv1   g46(.a(x10), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  inv1   g48(.a(new_n66_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n82_), .c(new_n81_), .d(new_n68_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x04), .O(new_n85_));
  aoi21  g51(.a(new_n74_), .b(x11), .c(new_n85_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n40_), .c(new_n79_), .O(z07));
  nand2  g53(.a(new_n84_), .b(x12), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n73_), .b(new_n89_), .c(new_n82_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n83_), .c(new_n40_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n88_), .c(new_n65_), .d(new_n64_), .O(z08));
  oai21  g59(.a(new_n90_), .b(new_n66_), .c(x13), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n89_), .c(new_n82_), .d(new_n81_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n70_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n94_), .c(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n43_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n79_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  aoi21  g68(.a(new_n96_), .b(new_n102_), .c(new_n47_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n68_), .c(x08), .d(x07), .O(new_n104_));
  nand2  g70(.a(new_n47_), .b(x14), .O(new_n105_));
  oai21  g71(.a(new_n104_), .b(new_n44_), .c(new_n105_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n43_), .O(z10));
  nand2  g74(.a(new_n58_), .b(new_n39_), .O(z11));
  aoi21  g75(.a(x16), .b(x04), .c(x01), .O(new_n110_));
  oai22  g76(.a(new_n110_), .b(x02), .c(new_n38_), .d(new_n45_), .O(z12));
  nand2  g77(.a(new_n43_), .b(new_n38_), .O(z13));
  inv1   g78(.a(x17), .O(new_n113_));
  nor3   g79(.a(new_n40_), .b(new_n113_), .c(new_n38_), .O(z14));
endmodule


