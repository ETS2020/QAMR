// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n115_;
  nor2   g00(.a(x06), .b(x04), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nor2   g09(.a(x05), .b(new_n43_), .O(new_n44_));
  inv1   g10(.a(x06), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x04), .O(new_n46_));
  oai22  g12(.a(new_n46_), .b(new_n44_), .c(new_n42_), .d(x15), .O(new_n47_));
  nand3  g13(.a(x06), .b(x05), .c(new_n43_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n47_), .O(z01));
  oai21  g15(.a(x03), .b(new_n36_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n35_), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(x06), .c(x04), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(x06), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(z03));
  nand2  g24(.a(x08), .b(new_n54_), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x07), .c(x06), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n59_), .c(new_n43_), .O(new_n62_));
  nor2   g28(.a(new_n60_), .b(x06), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(new_n50_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n53_), .O(z04));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x06), .O(new_n69_));
  inv1   g35(.a(x03), .O(new_n70_));
  inv1   g36(.a(x16), .O(new_n71_));
  aoi21  g37(.a(new_n70_), .b(x02), .c(new_n71_), .O(new_n72_));
  nand3  g38(.a(x08), .b(x07), .c(x06), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(x09), .c(new_n72_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n43_), .c(new_n69_), .O(z05));
  nand2  g41(.a(new_n66_), .b(x08), .O(new_n76_));
  nand2  g42(.a(x07), .b(x06), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n76_), .c(x10), .O(new_n78_));
  nor2   g44(.a(x10), .b(x09), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(x08), .c(x07), .d(x06), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n78_), .c(new_n50_), .d(x04), .O(z06));
  nand2  g47(.a(new_n80_), .b(x11), .O(new_n82_));
  inv1   g48(.a(x10), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  inv1   g50(.a(new_n73_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n66_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n82_), .c(new_n50_), .d(x04), .O(z07));
  nor2   g53(.a(x12), .b(x11), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n83_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n67_), .c(x04), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  aoi21  g57(.a(new_n86_), .b(x12), .c(new_n72_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n43_), .c(new_n91_), .O(z08));
  inv1   g59(.a(new_n46_), .O(new_n94_));
  inv1   g60(.a(x12), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n50_), .b(new_n96_), .c(new_n95_), .d(new_n84_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(x10), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n66_), .c(x08), .d(x07), .O(new_n99_));
  nand3  g65(.a(new_n88_), .b(new_n79_), .c(new_n85_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x13), .c(new_n72_), .O(new_n101_));
  oai21  g67(.a(new_n99_), .b(new_n45_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n94_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  nand4  g71(.a(new_n96_), .b(new_n95_), .c(new_n84_), .d(new_n83_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n72_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n66_), .c(x08), .d(x07), .O(new_n108_));
  oai22  g74(.a(new_n108_), .b(new_n45_), .c(new_n50_), .d(new_n39_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n53_), .O(z10));
  nor2   g77(.a(new_n35_), .b(new_n36_), .O(z11));
  aoi21  g78(.a(x16), .b(new_n36_), .c(x03), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(new_n43_), .c(new_n53_), .O(z12));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n53_), .O(z14));
  buf    g82(.a(x04), .O(z13));
endmodule


