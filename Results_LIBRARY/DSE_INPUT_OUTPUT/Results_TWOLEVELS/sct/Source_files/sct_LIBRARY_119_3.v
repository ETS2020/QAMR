// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x05), .O(new_n35_));
  inv1   g01(.a(x16), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n35_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand3  g11(.a(new_n36_), .b(x05), .c(new_n45_), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(new_n44_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n40_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n38_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x03), .O(new_n53_));
  nand3  g19(.a(new_n35_), .b(new_n53_), .c(x02), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x16), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n49_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n38_), .O(z04));
  oai21  g29(.a(new_n36_), .b(new_n35_), .c(new_n45_), .O(new_n64_));
  oai21  g30(.a(new_n45_), .b(x02), .c(new_n53_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x05), .c(x16), .O(new_n66_));
  nand3  g32(.a(x08), .b(x07), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n59_), .b(new_n69_), .c(x08), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n66_), .d(new_n64_), .O(z05));
  nand2  g37(.a(new_n70_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n59_), .c(x08), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n38_), .O(new_n76_));
  nand3  g42(.a(new_n65_), .b(x16), .c(new_n35_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n64_), .O(z06));
  nand2  g44(.a(new_n74_), .b(x11), .O(new_n79_));
  inv1   g45(.a(new_n67_), .O(new_n80_));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n69_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n79_), .c(new_n66_), .d(new_n64_), .O(z07));
  nand2  g49(.a(new_n82_), .b(x12), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand4  g52(.a(new_n73_), .b(new_n80_), .c(new_n86_), .d(new_n85_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n84_), .c(new_n66_), .d(new_n64_), .O(z08));
  nor2   g54(.a(x03), .b(new_n40_), .O(new_n89_));
  nor3   g55(.a(x13), .b(x12), .c(x11), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n73_), .c(new_n80_), .d(new_n89_), .O(new_n91_));
  nand2  g57(.a(x16), .b(new_n40_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n91_), .c(x05), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n81_), .b(new_n36_), .c(new_n94_), .d(new_n86_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n70_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n93_), .c(x04), .O(new_n97_));
  nand3  g63(.a(new_n87_), .b(new_n38_), .c(x13), .O(new_n98_));
  nand3  g64(.a(x16), .b(new_n35_), .c(x03), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n64_), .O(z09));
  nand2  g66(.a(new_n49_), .b(x00), .O(new_n101_));
  nand4  g67(.a(new_n55_), .b(new_n94_), .c(new_n86_), .d(new_n85_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(x10), .c(new_n101_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n69_), .c(x08), .d(x07), .O(new_n104_));
  inv1   g70(.a(new_n49_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n104_), .b(new_n48_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n38_), .O(z10));
  nand2  g75(.a(new_n38_), .b(new_n40_), .O(z11));
  nand3  g76(.a(x16), .b(new_n35_), .c(new_n40_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n53_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n38_), .O(z12));
  nor2   g80(.a(new_n37_), .b(new_n45_), .O(z13));
  nand2  g81(.a(x17), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n38_), .O(z14));
endmodule


