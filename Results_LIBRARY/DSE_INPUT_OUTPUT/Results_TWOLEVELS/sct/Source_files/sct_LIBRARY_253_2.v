// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_;
  inv1   g00(.a(x15), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x17), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(new_n35_), .O(new_n44_));
  aoi21  g10(.a(new_n35_), .b(x05), .c(new_n44_), .O(new_n45_));
  nor2   g11(.a(x18), .b(x15), .O(new_n46_));
  nor2   g12(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai22  g13(.a(new_n47_), .b(x05), .c(new_n45_), .d(x04), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(new_n36_), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n37_), .c(x16), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n51_), .c(new_n50_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n57_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n51_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n50_), .O(z04));
  inv1   g29(.a(x16), .O(new_n64_));
  aoi21  g30(.a(x04), .b(new_n37_), .c(x03), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  nand3  g35(.a(new_n59_), .b(new_n67_), .c(x08), .O(new_n70_));
  oai21  g36(.a(new_n69_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n66_), .c(new_n50_), .O(new_n72_));
  inv1   g38(.a(x04), .O(new_n73_));
  oai21  g39(.a(x17), .b(new_n35_), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(z05));
  inv1   g41(.a(new_n65_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n50_), .c(x16), .O(new_n77_));
  nand2  g43(.a(new_n70_), .b(x10), .O(new_n78_));
  inv1   g44(.a(x10), .O(new_n79_));
  nand4  g45(.a(new_n59_), .b(new_n79_), .c(new_n67_), .d(x08), .O(new_n80_));
  and2   g46(.a(new_n80_), .b(new_n50_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n78_), .c(new_n77_), .d(new_n74_), .O(z06));
  nand2  g48(.a(new_n80_), .b(x11), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand4  g50(.a(new_n69_), .b(new_n84_), .c(new_n79_), .d(new_n67_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n66_), .c(new_n50_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n74_), .O(z07));
  nand2  g54(.a(new_n85_), .b(x12), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n84_), .c(new_n79_), .d(new_n67_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n68_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n36_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n89_), .c(new_n77_), .d(new_n74_), .O(z08));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n51_), .b(new_n95_), .c(new_n90_), .d(new_n84_), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(x10), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n67_), .c(x08), .d(x07), .O(new_n98_));
  nand2  g64(.a(x16), .b(new_n37_), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n49_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(x16), .b(x03), .O(new_n101_));
  oai21  g67(.a(new_n92_), .b(new_n95_), .c(new_n101_), .O(new_n102_));
  aoi21  g68(.a(new_n100_), .b(x04), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n36_), .c(new_n74_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  nand4  g71(.a(new_n95_), .b(new_n90_), .c(new_n84_), .d(new_n79_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  and2   g73(.a(new_n107_), .b(new_n51_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n67_), .c(x08), .d(x07), .O(new_n109_));
  oai22  g75(.a(new_n109_), .b(new_n49_), .c(new_n51_), .d(new_n40_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(new_n50_), .c(x04), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(z10));
  nand2  g78(.a(new_n50_), .b(new_n37_), .O(z11));
  nor2   g79(.a(new_n36_), .b(new_n64_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n37_), .c(x03), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n73_), .c(new_n50_), .O(z12));
  nor2   g82(.a(new_n36_), .b(new_n73_), .O(z13));
  oai21  g83(.a(new_n43_), .b(new_n73_), .c(new_n50_), .O(z14));
endmodule


