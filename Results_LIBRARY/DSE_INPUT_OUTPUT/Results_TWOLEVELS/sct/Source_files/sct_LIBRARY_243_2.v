// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n109_, new_n112_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x00), .O(new_n36_));
  nor2   g02(.a(x09), .b(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  inv1   g11(.a(new_n37_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n48_), .c(new_n46_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n54_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n54_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n48_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n46_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n35_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n46_), .O(new_n64_));
  nand4  g30(.a(new_n48_), .b(x07), .c(new_n47_), .d(x04), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(x00), .c(new_n68_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n67_), .c(new_n64_), .O(z05));
  inv1   g38(.a(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n68_), .c(x08), .O(new_n74_));
  oai22  g40(.a(new_n74_), .b(new_n55_), .c(new_n70_), .d(new_n73_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n36_), .O(new_n76_));
  nand2  g42(.a(x10), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n64_), .O(z06));
  nand2  g44(.a(new_n73_), .b(x08), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n55_), .c(x11), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand4  g47(.a(new_n70_), .b(new_n81_), .c(new_n73_), .d(new_n68_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n36_), .O(new_n84_));
  nand2  g50(.a(x11), .b(x09), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n64_), .O(z07));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n70_), .b(new_n87_), .c(new_n81_), .d(new_n73_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n36_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n68_), .O(new_n90_));
  nand2  g56(.a(new_n82_), .b(x12), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n64_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n48_), .b(new_n93_), .c(new_n87_), .d(new_n81_), .O(new_n94_));
  nor3   g60(.a(new_n94_), .b(x10), .c(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  nand2  g62(.a(new_n88_), .b(x13), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n43_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n36_), .O(new_n99_));
  nand2  g65(.a(x13), .b(x09), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n99_), .c(new_n64_), .O(z09));
  inv1   g67(.a(new_n48_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x14), .O(new_n103_));
  oai21  g69(.a(new_n96_), .b(x00), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x04), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n46_), .O(z10));
  nand2  g72(.a(new_n46_), .b(new_n35_), .O(z11));
  nor2   g73(.a(new_n37_), .b(new_n61_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n35_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n43_), .c(new_n46_), .O(z12));
  nand2  g76(.a(new_n46_), .b(new_n43_), .O(z13));
  inv1   g77(.a(x17), .O(new_n112_));
  nor3   g78(.a(new_n37_), .b(new_n112_), .c(new_n43_), .O(z14));
endmodule


