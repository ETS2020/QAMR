// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x00), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(x02), .b(x01), .O(new_n37_));
  nand3  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand3  g05(.a(x14), .b(new_n39_), .c(x01), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n38_), .O(z00));
  nor2   g07(.a(x14), .b(new_n35_), .O(new_n42_));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(new_n42_), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n39_), .c(x16), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n50_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n50_), .O(z04));
  nand3  g28(.a(x16), .b(x04), .c(new_n39_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n50_), .O(new_n65_));
  oai21  g31(.a(new_n36_), .b(new_n46_), .c(x00), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x16), .c(x03), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n58_), .b(new_n70_), .c(x08), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n42_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(z05));
  inv1   g40(.a(x10), .O(new_n75_));
  nor2   g41(.a(new_n72_), .b(new_n75_), .O(new_n76_));
  nand4  g42(.a(new_n58_), .b(new_n75_), .c(new_n70_), .d(x08), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n63_), .c(x04), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n76_), .c(new_n50_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n67_), .O(z06));
  inv1   g46(.a(x11), .O(new_n81_));
  inv1   g47(.a(new_n68_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n81_), .c(new_n75_), .d(new_n70_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n63_), .c(x04), .O(new_n84_));
  aoi21  g50(.a(new_n77_), .b(x11), .c(new_n84_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n42_), .c(new_n67_), .O(z07));
  nand2  g52(.a(new_n83_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n81_), .c(new_n75_), .d(new_n70_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n68_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n42_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n87_), .c(new_n67_), .d(new_n65_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nor2   g59(.a(new_n90_), .b(new_n93_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n64_), .c(new_n50_), .O(new_n95_));
  nand3  g61(.a(new_n51_), .b(x04), .c(new_n35_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n36_), .c(x13), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n88_), .c(new_n81_), .d(new_n75_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(x09), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x08), .c(x07), .d(x06), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n95_), .c(new_n67_), .O(z09));
  nand4  g67(.a(new_n51_), .b(new_n93_), .c(new_n88_), .d(new_n81_), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n75_), .c(new_n35_), .O(new_n104_));
  nand2  g70(.a(x14), .b(x00), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(x09), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(x08), .c(x07), .d(x06), .O(new_n107_));
  inv1   g73(.a(new_n51_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x14), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n107_), .c(new_n46_), .O(z10));
  nor2   g76(.a(new_n42_), .b(new_n39_), .O(z11));
  inv1   g77(.a(x03), .O(new_n112_));
  inv1   g78(.a(x16), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(x02), .c(new_n112_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n50_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  nor2   g82(.a(new_n42_), .b(new_n46_), .O(z13));
  inv1   g83(.a(x17), .O(new_n118_));
  nor3   g84(.a(new_n42_), .b(new_n118_), .c(new_n46_), .O(z14));
endmodule


