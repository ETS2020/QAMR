// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x07), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n36_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x17), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x07), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n39_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n49_), .c(new_n47_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand3  g18(.a(x17), .b(x07), .c(new_n47_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nor2   g23(.a(new_n35_), .b(new_n47_), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n37_), .O(z04));
  inv1   g28(.a(x16), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x02), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n37_), .O(new_n65_));
  oai21  g31(.a(x07), .b(new_n44_), .c(new_n48_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x16), .c(x03), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n57_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x17), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x07), .O(new_n71_));
  nand3  g37(.a(x08), .b(x07), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n71_), .c(new_n67_), .d(new_n65_), .O(z05));
  nand2  g40(.a(new_n71_), .b(x10), .O(new_n75_));
  inv1   g41(.a(x09), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  inv1   g43(.a(new_n72_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(x17), .c(new_n77_), .d(new_n76_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n75_), .c(new_n67_), .d(new_n65_), .O(z06));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n76_), .O(new_n82_));
  nor3   g48(.a(new_n82_), .b(new_n57_), .c(new_n47_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n48_), .c(x07), .O(new_n84_));
  inv1   g50(.a(new_n58_), .O(new_n85_));
  nand3  g51(.a(new_n77_), .b(new_n76_), .c(x08), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n85_), .c(x11), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n84_), .c(new_n67_), .d(new_n65_), .O(z07));
  nand2  g54(.a(new_n84_), .b(x12), .O(new_n89_));
  nor2   g55(.a(new_n48_), .b(x12), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n81_), .c(new_n68_), .d(new_n58_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n89_), .c(new_n67_), .d(new_n65_), .O(z08));
  inv1   g58(.a(x11), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n50_), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n96_));
  nor3   g62(.a(new_n96_), .b(x10), .c(x09), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x08), .c(x07), .d(x06), .O(new_n98_));
  nand3  g64(.a(new_n94_), .b(new_n93_), .c(new_n77_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n69_), .c(x13), .O(new_n100_));
  oai21  g66(.a(new_n98_), .b(new_n44_), .c(new_n100_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x17), .O(new_n102_));
  nand2  g68(.a(x13), .b(new_n35_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n102_), .c(new_n67_), .d(new_n65_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  inv1   g71(.a(new_n50_), .O(new_n106_));
  nand3  g72(.a(new_n81_), .b(new_n95_), .c(new_n94_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n105_), .c(new_n106_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n76_), .c(x08), .d(x07), .O(new_n109_));
  nand2  g75(.a(new_n106_), .b(x14), .O(new_n110_));
  oai21  g76(.a(new_n109_), .b(new_n47_), .c(new_n110_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x17), .O(new_n112_));
  nand3  g78(.a(new_n106_), .b(x14), .c(new_n35_), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n112_), .c(new_n44_), .O(z10));
  nor2   g80(.a(new_n36_), .b(new_n39_), .O(z11));
  inv1   g81(.a(x03), .O(new_n116_));
  oai21  g82(.a(new_n63_), .b(x02), .c(new_n116_), .O(new_n117_));
  nand3  g83(.a(new_n117_), .b(new_n37_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z12));
  nor2   g85(.a(new_n36_), .b(new_n44_), .O(z13));
  oai21  g86(.a(new_n48_), .b(new_n44_), .c(new_n37_), .O(z14));
endmodule


