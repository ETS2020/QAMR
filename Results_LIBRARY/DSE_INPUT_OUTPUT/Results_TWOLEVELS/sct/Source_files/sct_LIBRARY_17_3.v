// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n113_, new_n114_, new_n116_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x16), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(x15), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  aoi22  g07(.a(x16), .b(x01), .c(x05), .d(new_n41_), .O(new_n42_));
  oai21  g08(.a(new_n40_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  inv1   g10(.a(x01), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(x02), .c(new_n45_), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n44_), .c(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  nand2  g16(.a(x16), .b(x01), .O(new_n51_));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nor2   g18(.a(x03), .b(new_n35_), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n36_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n51_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n55_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n51_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n59_), .b(new_n66_), .c(x08), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n51_), .O(new_n69_));
  oai21  g35(.a(new_n41_), .b(x02), .c(new_n46_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x16), .c(new_n45_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n69_), .O(z05));
  nand2  g38(.a(new_n51_), .b(new_n41_), .O(z13));
  oai21  g39(.a(new_n70_), .b(x01), .c(x16), .O(new_n74_));
  nand2  g40(.a(new_n67_), .b(x10), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n59_), .c(x08), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n75_), .c(new_n74_), .d(z13), .O(z06));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n64_), .O(new_n81_));
  aoi21  g47(.a(new_n77_), .b(x11), .c(new_n81_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n74_), .c(z13), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nor2   g50(.a(new_n81_), .b(new_n84_), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  inv1   g52(.a(new_n64_), .O(new_n87_));
  nand4  g53(.a(new_n76_), .b(new_n87_), .c(new_n84_), .d(new_n86_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n85_), .c(new_n51_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n71_), .O(z08));
  nor3   g57(.a(x13), .b(x12), .c(x11), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n76_), .c(new_n87_), .d(new_n53_), .O(new_n93_));
  nand2  g59(.a(x16), .b(new_n35_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n93_), .c(x01), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n79_), .b(new_n36_), .c(new_n96_), .d(new_n84_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n67_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n95_), .c(x04), .O(new_n99_));
  aoi21  g65(.a(new_n46_), .b(new_n45_), .c(new_n36_), .O(new_n100_));
  aoi21  g66(.a(new_n88_), .b(x13), .c(new_n100_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n99_), .c(z13), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  inv1   g69(.a(x10), .O(new_n104_));
  nand4  g70(.a(new_n96_), .b(new_n84_), .c(new_n86_), .d(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n48_), .c(new_n66_), .d(x08), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(x07), .c(x06), .O(new_n109_));
  nand3  g75(.a(new_n54_), .b(x14), .c(new_n45_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n41_), .O(z10));
  nand2  g77(.a(new_n51_), .b(new_n35_), .O(z11));
  oai21  g78(.a(new_n94_), .b(x01), .c(new_n46_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n51_), .O(z12));
  nand2  g81(.a(x17), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n51_), .O(z14));
endmodule


