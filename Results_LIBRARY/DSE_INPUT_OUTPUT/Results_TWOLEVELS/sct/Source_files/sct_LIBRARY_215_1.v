// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n119_;
  nor2   g00(.a(x17), .b(x14), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  and2   g03(.a(x14), .b(x01), .O(new_n38_));
  oai21  g04(.a(new_n35_), .b(new_n38_), .c(new_n37_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(x01), .c(new_n39_), .O(z00));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(x17), .b(new_n41_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x03), .O(new_n51_));
  inv1   g17(.a(x16), .O(new_n52_));
  aoi21  g18(.a(new_n51_), .b(x02), .c(new_n52_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n42_), .c(new_n50_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z02));
  xor2a  g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n54_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n42_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(x07), .b(x06), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n60_), .b(x07), .c(x06), .O(new_n63_));
  oai21  g29(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n54_), .c(new_n42_), .d(x04), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(z04));
  aoi21  g32(.a(x16), .b(new_n37_), .c(new_n47_), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n42_), .O(new_n69_));
  oai21  g35(.a(new_n41_), .b(new_n47_), .c(x17), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x16), .c(x03), .O(new_n71_));
  nand3  g37(.a(x08), .b(x07), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x09), .O(new_n73_));
  inv1   g39(.a(x09), .O(new_n74_));
  nand3  g40(.a(new_n62_), .b(new_n74_), .c(x08), .O(new_n75_));
  and2   g41(.a(new_n75_), .b(new_n42_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n73_), .c(new_n71_), .d(new_n69_), .O(z05));
  nand2  g43(.a(new_n75_), .b(x10), .O(new_n78_));
  nor4   g44(.a(new_n61_), .b(x10), .c(x09), .d(new_n60_), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n43_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n78_), .c(new_n71_), .d(new_n69_), .O(z06));
  inv1   g47(.a(new_n79_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x11), .O(new_n83_));
  nor4   g49(.a(new_n72_), .b(x11), .c(x10), .d(x09), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n43_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n83_), .c(new_n71_), .d(new_n69_), .O(z07));
  inv1   g52(.a(new_n84_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x12), .O(new_n88_));
  inv1   g54(.a(x10), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n74_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n72_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n43_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n88_), .c(new_n71_), .d(new_n69_), .O(z08));
  inv1   g61(.a(x13), .O(new_n96_));
  oai21  g62(.a(new_n93_), .b(new_n96_), .c(new_n67_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n42_), .O(new_n98_));
  oai21  g64(.a(new_n53_), .b(x17), .c(new_n41_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n96_), .c(new_n91_), .d(new_n90_), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n89_), .c(new_n74_), .d(x08), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(x07), .c(x06), .d(x04), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n98_), .c(new_n71_), .O(z09));
  inv1   g71(.a(x00), .O(new_n106_));
  nand4  g72(.a(new_n96_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n99_), .c(new_n74_), .d(x08), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(x07), .c(x06), .O(new_n111_));
  nand2  g77(.a(new_n53_), .b(x14), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n111_), .c(new_n47_), .O(z10));
  nor2   g79(.a(new_n43_), .b(new_n37_), .O(z11));
  nor2   g80(.a(new_n43_), .b(new_n52_), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n37_), .c(x03), .O(new_n116_));
  oai21  g82(.a(new_n116_), .b(new_n47_), .c(new_n42_), .O(z12));
  aoi21  g83(.a(x17), .b(new_n41_), .c(new_n47_), .O(z13));
  nand3  g84(.a(x17), .b(x14), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z14));
endmodule


