// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x02), .O(new_n35_));
  oai21  g01(.a(x08), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x08), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(new_n40_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n47_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n47_), .O(z03));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(x08), .c(new_n38_), .O(new_n56_));
  nand3  g22(.a(new_n39_), .b(x07), .c(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n49_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n35_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n47_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  inv1   g31(.a(new_n55_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g33(.a(new_n66_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  aoi22  g34(.a(new_n68_), .b(new_n38_), .c(x09), .d(new_n39_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n64_), .O(z05));
  nand3  g36(.a(new_n65_), .b(x07), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n66_), .c(x08), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n38_), .O(new_n76_));
  nand2  g42(.a(x10), .b(new_n39_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n64_), .O(z06));
  nor4   g44(.a(new_n55_), .b(x11), .c(x10), .d(x09), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(x01), .c(x08), .O(new_n80_));
  nand2  g46(.a(new_n74_), .b(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n64_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(x08), .b(x07), .c(x06), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  nand3  g51(.a(new_n73_), .b(new_n83_), .c(new_n85_), .O(new_n86_));
  oai22  g52(.a(new_n86_), .b(new_n84_), .c(new_n79_), .d(new_n83_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n38_), .O(new_n88_));
  nand2  g54(.a(x12), .b(new_n39_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n64_), .O(z08));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n49_), .b(new_n91_), .c(new_n83_), .d(new_n85_), .O(new_n92_));
  nor3   g58(.a(new_n92_), .b(x10), .c(x09), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(x08), .c(x07), .d(x06), .O(new_n94_));
  inv1   g60(.a(x10), .O(new_n95_));
  nand3  g61(.a(new_n83_), .b(new_n85_), .c(new_n95_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n71_), .c(x13), .O(new_n97_));
  oai21  g63(.a(new_n94_), .b(new_n44_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n38_), .O(new_n99_));
  nand2  g65(.a(x13), .b(new_n39_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n99_), .c(new_n64_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  inv1   g68(.a(new_n49_), .O(new_n103_));
  nand4  g69(.a(new_n91_), .b(new_n83_), .c(new_n85_), .d(new_n95_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n102_), .c(new_n103_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n65_), .c(x08), .d(x07), .O(new_n106_));
  nand2  g72(.a(new_n103_), .b(x14), .O(new_n107_));
  oai21  g73(.a(new_n106_), .b(new_n48_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n38_), .O(new_n109_));
  nand3  g75(.a(new_n103_), .b(x14), .c(new_n39_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n44_), .O(z10));
  nand2  g77(.a(new_n47_), .b(new_n35_), .O(z11));
  nand3  g78(.a(new_n47_), .b(x16), .c(new_n35_), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(x03), .c(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n47_), .O(z12));
  nand2  g82(.a(new_n47_), .b(new_n44_), .O(z13));
  nand3  g83(.a(new_n47_), .b(x17), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


