// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n118_;
  nor2   g00(.a(x07), .b(x05), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x15), .O(new_n43_));
  nand2  g09(.a(x18), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(x07), .c(new_n42_), .O(new_n45_));
  oai21  g11(.a(new_n42_), .b(x04), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(new_n35_), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(z03));
  inv1   g20(.a(x07), .O(new_n55_));
  xnor2a g21(.a(x08), .b(x06), .O(new_n56_));
  nand3  g22(.a(x08), .b(new_n55_), .c(x05), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n49_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n36_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n48_), .O(new_n64_));
  oai21  g30(.a(x09), .b(new_n42_), .c(new_n55_), .O(new_n65_));
  nand2  g31(.a(x08), .b(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(new_n64_), .O(z05));
  oai21  g36(.a(x10), .b(new_n42_), .c(new_n55_), .O(new_n71_));
  nand3  g37(.a(new_n68_), .b(x08), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(x08), .c(x07), .d(x06), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n71_), .d(new_n64_), .O(z06));
  inv1   g42(.a(x11), .O(new_n77_));
  inv1   g43(.a(new_n66_), .O(new_n78_));
  aoi21  g44(.a(new_n74_), .b(new_n78_), .c(new_n77_), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  nand4  g46(.a(new_n78_), .b(new_n77_), .c(new_n80_), .d(new_n68_), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n79_), .c(x07), .O(new_n83_));
  nand3  g49(.a(x11), .b(new_n55_), .c(x05), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n64_), .O(z07));
  oai21  g51(.a(x12), .b(new_n42_), .c(new_n55_), .O(new_n86_));
  nand3  g52(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n74_), .b(new_n88_), .c(new_n77_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  aoi21  g56(.a(new_n81_), .b(x12), .c(new_n90_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n86_), .c(new_n64_), .O(z08));
  inv1   g58(.a(x04), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n49_), .b(new_n94_), .c(new_n88_), .d(new_n77_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(x10), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n68_), .c(x08), .d(x06), .O(new_n97_));
  nand3  g63(.a(new_n88_), .b(new_n77_), .c(new_n80_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n72_), .c(x13), .O(new_n99_));
  oai21  g65(.a(new_n97_), .b(new_n93_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x07), .O(new_n101_));
  nand3  g67(.a(x13), .b(new_n55_), .c(x05), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(new_n64_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand4  g70(.a(new_n94_), .b(new_n88_), .c(new_n77_), .d(new_n80_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n49_), .c(new_n68_), .d(x08), .O(new_n107_));
  oai22  g73(.a(new_n107_), .b(new_n47_), .c(new_n49_), .d(new_n39_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x07), .O(new_n109_));
  inv1   g75(.a(new_n49_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(x14), .c(x05), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n109_), .c(new_n93_), .O(z10));
  nor2   g78(.a(new_n35_), .b(new_n36_), .O(z11));
  nor3   g79(.a(new_n35_), .b(new_n61_), .c(x02), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(x03), .c(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n48_), .O(z12));
  nand2  g82(.a(new_n48_), .b(new_n93_), .O(z13));
  nand2  g83(.a(x17), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n48_), .O(z14));
endmodule


