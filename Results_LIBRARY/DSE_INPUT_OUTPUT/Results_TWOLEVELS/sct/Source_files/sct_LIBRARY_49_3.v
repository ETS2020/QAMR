// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n115_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x16), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(x15), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  inv1   g07(.a(x01), .O(new_n42_));
  nor2   g08(.a(new_n36_), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n40_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(x02), .c(new_n42_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n43_), .O(new_n52_));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  aoi21  g19(.a(new_n47_), .b(x02), .c(new_n36_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n52_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n49_), .c(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  nor2   g29(.a(new_n55_), .b(new_n41_), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n59_), .b(new_n67_), .c(x08), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(x04), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n64_), .c(new_n42_), .O(new_n70_));
  nand2  g36(.a(new_n69_), .b(new_n36_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(z05));
  nand2  g38(.a(new_n68_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand4  g40(.a(new_n59_), .b(new_n74_), .c(new_n67_), .d(x08), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(x04), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n64_), .c(new_n42_), .O(new_n77_));
  nand2  g43(.a(new_n76_), .b(new_n36_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n77_), .O(z06));
  oai21  g45(.a(new_n54_), .b(new_n41_), .c(new_n42_), .O(new_n80_));
  nand2  g46(.a(new_n75_), .b(x11), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  inv1   g48(.a(new_n65_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n82_), .c(new_n74_), .d(new_n67_), .O(new_n84_));
  aoi21  g50(.a(new_n36_), .b(new_n41_), .c(new_n43_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n81_), .d(new_n80_), .O(z07));
  nand2  g52(.a(new_n84_), .b(x12), .O(new_n87_));
  nand2  g53(.a(new_n74_), .b(new_n67_), .O(new_n88_));
  nor4   g54(.a(new_n88_), .b(new_n65_), .c(x12), .d(x11), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n87_), .c(new_n85_), .d(new_n80_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  oai21  g58(.a(new_n89_), .b(new_n92_), .c(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n52_), .O(new_n94_));
  inv1   g60(.a(x12), .O(new_n95_));
  nand4  g61(.a(new_n49_), .b(new_n92_), .c(new_n95_), .d(new_n82_), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(x10), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n67_), .c(x08), .d(x07), .O(new_n98_));
  nand2  g64(.a(new_n54_), .b(new_n42_), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n46_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n94_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  nand4  g69(.a(new_n92_), .b(new_n95_), .c(new_n82_), .d(new_n74_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n49_), .c(new_n67_), .d(x08), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(x07), .c(x06), .O(new_n108_));
  nand3  g74(.a(new_n54_), .b(x14), .c(new_n42_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n41_), .O(z10));
  nor2   g76(.a(new_n43_), .b(new_n35_), .O(z11));
  nand2  g77(.a(new_n36_), .b(x03), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n99_), .c(new_n41_), .O(z12));
  nor2   g79(.a(new_n43_), .b(new_n41_), .O(z13));
  nand3  g80(.a(new_n52_), .b(x17), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z14));
endmodule


