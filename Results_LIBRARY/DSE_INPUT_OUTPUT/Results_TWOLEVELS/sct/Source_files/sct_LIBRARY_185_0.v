// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n125_;
  oai21  g00(.a(x14), .b(x01), .c(x02), .O(z00));
  inv1   g01(.a(x18), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x15), .O(new_n37_));
  inv1   g03(.a(x04), .O(new_n38_));
  nor2   g04(.a(x02), .b(x01), .O(new_n39_));
  aoi21  g05(.a(x05), .b(new_n38_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n37_), .b(x05), .c(new_n40_), .O(z01));
  inv1   g07(.a(x06), .O(new_n42_));
  nand2  g08(.a(x16), .b(x03), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x02), .O(new_n44_));
  inv1   g10(.a(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x01), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n42_), .c(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(new_n39_), .O(new_n53_));
  inv1   g19(.a(x02), .O(new_n54_));
  oai21  g20(.a(x03), .b(new_n54_), .c(x16), .O(new_n55_));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n55_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n53_), .O(z04));
  oai21  g27(.a(new_n45_), .b(x02), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x01), .O(new_n63_));
  nand2  g29(.a(new_n43_), .b(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x02), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n57_), .b(new_n68_), .c(x08), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n63_), .O(z05));
  nand2  g37(.a(new_n69_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n57_), .c(x08), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n65_), .d(new_n63_), .O(z06));
  inv1   g42(.a(new_n66_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n68_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x04), .O(new_n80_));
  aoi21  g46(.a(new_n74_), .b(x11), .c(new_n80_), .O(new_n81_));
  nand2  g47(.a(x03), .b(x02), .O(new_n82_));
  nand3  g48(.a(x04), .b(new_n54_), .c(x01), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x16), .O(new_n85_));
  oai21  g51(.a(new_n81_), .b(new_n39_), .c(new_n85_), .O(z07));
  nand2  g52(.a(new_n79_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand4  g55(.a(new_n73_), .b(new_n77_), .c(new_n89_), .d(new_n88_), .O(new_n90_));
  and2   g56(.a(new_n90_), .b(new_n53_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n87_), .c(new_n65_), .d(new_n63_), .O(z08));
  nand2  g58(.a(new_n90_), .b(x13), .O(new_n93_));
  nor2   g59(.a(new_n42_), .b(new_n38_), .O(new_n94_));
  nand3  g60(.a(new_n68_), .b(x08), .c(x07), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nor3   g62(.a(x16), .b(x13), .c(x12), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n96_), .c(new_n94_), .d(new_n78_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n93_), .c(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n53_), .O(new_n100_));
  nor2   g66(.a(new_n45_), .b(new_n38_), .O(new_n101_));
  inv1   g67(.a(x03), .O(new_n102_));
  nand3  g68(.a(new_n77_), .b(x04), .c(new_n102_), .O(new_n103_));
  inv1   g69(.a(x13), .O(new_n104_));
  nand4  g70(.a(new_n73_), .b(new_n104_), .c(new_n89_), .d(new_n88_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n103_), .c(new_n43_), .O(new_n106_));
  inv1   g72(.a(x01), .O(new_n107_));
  nor2   g73(.a(x02), .b(new_n107_), .O(new_n108_));
  aoi22  g74(.a(new_n108_), .b(new_n101_), .c(new_n106_), .d(x02), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n100_), .O(z09));
  inv1   g76(.a(x00), .O(new_n111_));
  inv1   g77(.a(x10), .O(new_n112_));
  nand4  g78(.a(new_n104_), .b(new_n89_), .c(new_n88_), .d(new_n112_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand4  g80(.a(new_n114_), .b(new_n47_), .c(new_n68_), .d(x08), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(x07), .c(x06), .O(new_n117_));
  oai21  g83(.a(x02), .b(new_n107_), .c(new_n82_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(x16), .c(x14), .O(new_n119_));
  aoi21  g85(.a(new_n119_), .b(new_n117_), .c(new_n38_), .O(z10));
  oai21  g86(.a(new_n45_), .b(x02), .c(new_n102_), .O(new_n121_));
  nand2  g87(.a(new_n121_), .b(x01), .O(new_n122_));
  aoi21  g88(.a(new_n122_), .b(new_n82_), .c(new_n38_), .O(z12));
  nor2   g89(.a(new_n39_), .b(new_n38_), .O(z13));
  inv1   g90(.a(x17), .O(new_n125_));
  nor3   g91(.a(new_n39_), .b(new_n125_), .c(new_n38_), .O(z14));
  buf    g92(.a(x02), .O(z11));
endmodule


