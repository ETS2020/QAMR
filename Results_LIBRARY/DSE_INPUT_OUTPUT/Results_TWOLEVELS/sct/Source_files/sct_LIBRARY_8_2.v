// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(x09), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g04(.a(x09), .O(new_n39_));
  nor2   g05(.a(x14), .b(new_n39_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(new_n40_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n48_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n56_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n47_), .O(z04));
  inv1   g28(.a(x16), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x02), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  oai21  g31(.a(new_n36_), .b(new_n44_), .c(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x16), .c(x03), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(x14), .c(x09), .O(new_n69_));
  nand3  g35(.a(new_n58_), .b(new_n39_), .c(x08), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(z05));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(new_n68_), .O(new_n73_));
  nand3  g39(.a(new_n58_), .b(new_n72_), .c(x08), .O(new_n74_));
  oai21  g40(.a(new_n73_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n39_), .O(new_n76_));
  nand3  g42(.a(x14), .b(x10), .c(x09), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(new_n67_), .d(new_n65_), .O(z06));
  oai21  g44(.a(new_n36_), .b(x11), .c(x09), .O(new_n79_));
  nor3   g45(.a(x11), .b(x10), .c(x09), .O(new_n80_));
  aoi22  g46(.a(new_n80_), .b(new_n73_), .c(new_n74_), .d(x11), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n79_), .c(new_n67_), .d(new_n65_), .O(z07));
  nor2   g48(.a(x11), .b(x10), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n58_), .c(x08), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x12), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n73_), .b(new_n87_), .c(new_n86_), .d(new_n72_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n39_), .O(new_n90_));
  nand3  g56(.a(x14), .b(x12), .c(x09), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n67_), .d(new_n65_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n50_), .b(new_n93_), .c(new_n87_), .d(new_n86_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  nand2  g62(.a(new_n88_), .b(x13), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n44_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n39_), .O(new_n99_));
  nand3  g65(.a(x14), .b(x13), .c(x09), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n67_), .d(new_n65_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand3  g68(.a(new_n83_), .b(new_n93_), .c(new_n87_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n50_), .c(new_n39_), .d(x08), .O(new_n105_));
  inv1   g71(.a(new_n105_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(x07), .c(x06), .O(new_n107_));
  inv1   g73(.a(new_n50_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x14), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n107_), .c(new_n44_), .O(z10));
  nand2  g76(.a(new_n47_), .b(new_n49_), .O(z11));
  nor2   g77(.a(new_n40_), .b(new_n63_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n49_), .c(x03), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(new_n44_), .c(new_n47_), .O(z12));
  nor2   g80(.a(new_n40_), .b(new_n44_), .O(z13));
  inv1   g81(.a(x17), .O(new_n116_));
  nor3   g82(.a(new_n40_), .b(new_n116_), .c(new_n44_), .O(z14));
endmodule


