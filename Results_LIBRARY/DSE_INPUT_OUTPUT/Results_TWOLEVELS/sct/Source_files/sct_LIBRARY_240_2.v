// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_;
  inv1   g00(.a(x12), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n48_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  inv1   g20(.a(x07), .O(new_n55_));
  nand3  g21(.a(x12), .b(new_n55_), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(x06), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n52_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  aoi21  g26(.a(x12), .b(new_n55_), .c(new_n48_), .O(new_n61_));
  nor2   g27(.a(new_n55_), .b(new_n48_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x12), .c(new_n60_), .O(new_n63_));
  oai21  g29(.a(new_n61_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n52_), .c(x04), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(z04));
  nand2  g32(.a(new_n36_), .b(new_n38_), .O(z11));
  aoi21  g33(.a(z11), .b(new_n49_), .c(new_n50_), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n68_), .c(x04), .O(new_n72_));
  nand3  g38(.a(x08), .b(x07), .c(x06), .O(new_n73_));
  nand2  g39(.a(new_n36_), .b(x04), .O(new_n74_));
  aoi21  g40(.a(new_n73_), .b(x09), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n72_), .O(z05));
  inv1   g42(.a(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n69_), .c(x08), .O(new_n78_));
  nor3   g44(.a(new_n78_), .b(new_n55_), .c(new_n48_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n68_), .c(x04), .O(new_n80_));
  nand3  g46(.a(new_n62_), .b(new_n69_), .c(x08), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(x10), .c(new_n74_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n80_), .O(z06));
  nor4   g49(.a(new_n73_), .b(x11), .c(x10), .d(x09), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n68_), .O(new_n85_));
  inv1   g51(.a(new_n62_), .O(new_n86_));
  oai21  g52(.a(new_n78_), .b(new_n86_), .c(x11), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n85_), .c(new_n36_), .d(x04), .O(z07));
  nor2   g54(.a(new_n60_), .b(new_n55_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n51_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x12), .O(new_n91_));
  inv1   g57(.a(x11), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n77_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n69_), .c(x08), .d(x07), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x06), .O(new_n96_));
  aoi21  g62(.a(x12), .b(x09), .c(new_n68_), .O(new_n97_));
  nor2   g63(.a(new_n35_), .b(x06), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n37_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n97_), .c(new_n96_), .d(x04), .O(z08));
  inv1   g66(.a(new_n68_), .O(new_n101_));
  nor2   g67(.a(new_n37_), .b(x13), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(x04), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n49_), .b(x02), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(x16), .c(x09), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(x08), .c(x07), .d(x06), .O(new_n107_));
  nand2  g73(.a(new_n51_), .b(x14), .O(new_n108_));
  oai21  g74(.a(new_n107_), .b(new_n104_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x12), .O(new_n110_));
  nand3  g76(.a(new_n51_), .b(x14), .c(new_n48_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n45_), .O(z10));
  oai21  g78(.a(new_n50_), .b(x02), .c(new_n49_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(new_n36_), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z12));
  nand2  g81(.a(x12), .b(x09), .O(new_n116_));
  oai21  g82(.a(new_n89_), .b(new_n35_), .c(x06), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n52_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n116_), .c(new_n70_), .O(new_n119_));
  oai21  g85(.a(new_n119_), .b(new_n68_), .c(x04), .O(new_n120_));
  nand2  g86(.a(new_n120_), .b(new_n36_), .O(z13));
  nand2  g87(.a(x17), .b(x04), .O(new_n122_));
  nand2  g88(.a(new_n122_), .b(new_n36_), .O(z14));
endmodule


