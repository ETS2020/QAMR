// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n124_;
  nor2   g00(.a(x14), .b(x06), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  and2   g03(.a(x14), .b(x01), .O(new_n38_));
  oai21  g04(.a(new_n35_), .b(new_n38_), .c(new_n37_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(x01), .c(new_n39_), .O(z00));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x06), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n37_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n50_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nand3  g20(.a(x14), .b(new_n54_), .c(x06), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(x06), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n51_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nor2   g25(.a(new_n54_), .b(new_n50_), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n51_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n42_), .O(z04));
  nor3   g30(.a(new_n43_), .b(new_n47_), .c(x02), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x03), .c(x16), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x08), .c(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x14), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nor2   g36(.a(new_n59_), .b(new_n54_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x06), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(x09), .c(new_n47_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n70_), .c(new_n66_), .O(z05));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x14), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x06), .O(new_n78_));
  nand3  g44(.a(new_n60_), .b(new_n67_), .c(x08), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(x10), .c(new_n47_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n66_), .O(z06));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n71_), .c(new_n67_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x14), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x06), .O(new_n85_));
  nand3  g51(.a(new_n75_), .b(new_n60_), .c(x08), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(x11), .c(new_n47_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(new_n66_), .O(z07));
  inv1   g54(.a(x10), .O(new_n89_));
  nor2   g55(.a(x12), .b(x11), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n68_), .c(x14), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x06), .O(new_n93_));
  inv1   g59(.a(new_n72_), .O(new_n94_));
  nand3  g60(.a(new_n82_), .b(new_n94_), .c(new_n67_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(x12), .c(new_n47_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n93_), .c(new_n66_), .O(z08));
  nand3  g63(.a(new_n42_), .b(x16), .c(new_n37_), .O(new_n98_));
  inv1   g64(.a(x12), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n82_), .b(x14), .c(new_n100_), .d(new_n99_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n79_), .c(new_n98_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x04), .O(new_n103_));
  nand3  g69(.a(new_n90_), .b(new_n75_), .c(new_n94_), .O(new_n104_));
  nand2  g70(.a(x16), .b(x03), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n42_), .c(x04), .O(new_n106_));
  aoi21  g72(.a(new_n104_), .b(x13), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n103_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  inv1   g75(.a(x11), .O(new_n110_));
  nand4  g76(.a(new_n100_), .b(new_n99_), .c(new_n110_), .d(new_n89_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n109_), .c(x09), .O(new_n112_));
  nand4  g78(.a(new_n112_), .b(x08), .c(x07), .d(x06), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n51_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(x14), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z10));
  nand2  g82(.a(new_n42_), .b(new_n37_), .O(z11));
  inv1   g83(.a(x03), .O(new_n118_));
  inv1   g84(.a(x16), .O(new_n119_));
  oai21  g85(.a(new_n119_), .b(x02), .c(new_n118_), .O(new_n120_));
  nand3  g86(.a(new_n120_), .b(new_n42_), .c(x04), .O(new_n121_));
  inv1   g87(.a(new_n121_), .O(z12));
  nand2  g88(.a(new_n42_), .b(new_n47_), .O(z13));
  nand2  g89(.a(x17), .b(x04), .O(new_n124_));
  nand2  g90(.a(new_n124_), .b(new_n42_), .O(z14));
endmodule


