// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_, new_n119_, new_n122_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x13), .O(new_n38_));
  oai21  g04(.a(new_n38_), .b(x01), .c(new_n36_), .O(z00));
  nor2   g05(.a(x13), .b(x01), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(new_n40_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n47_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n49_), .c(new_n47_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  and2   g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n55_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n49_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n47_), .O(z04));
  inv1   g26(.a(x03), .O(new_n61_));
  oai21  g27(.a(new_n44_), .b(x02), .c(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x16), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n55_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n56_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(new_n47_), .O(z05));
  nand2  g36(.a(new_n69_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand4  g39(.a(new_n56_), .b(new_n73_), .c(new_n72_), .d(x08), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n71_), .c(new_n65_), .d(new_n47_), .O(z06));
  nand2  g41(.a(new_n74_), .b(x11), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  inv1   g43(.a(new_n66_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n73_), .d(new_n72_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n76_), .c(new_n65_), .d(new_n47_), .O(z07));
  nand2  g46(.a(new_n79_), .b(x12), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n63_), .c(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n47_), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  inv1   g50(.a(x13), .O(new_n85_));
  nand4  g51(.a(new_n49_), .b(new_n85_), .c(x04), .d(x01), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n84_), .c(new_n77_), .d(new_n73_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(x09), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(x08), .c(x07), .d(x06), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n83_), .O(z08));
  nand3  g57(.a(new_n68_), .b(x06), .c(x01), .O(new_n92_));
  nand4  g58(.a(new_n85_), .b(new_n84_), .c(new_n77_), .d(new_n73_), .O(new_n93_));
  oai22  g59(.a(new_n93_), .b(new_n92_), .c(new_n85_), .d(x06), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n49_), .c(x07), .O(new_n95_));
  nand3  g61(.a(new_n47_), .b(x16), .c(new_n35_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  aoi21  g64(.a(x16), .b(x03), .c(new_n44_), .O(new_n99_));
  nand2  g65(.a(new_n68_), .b(x07), .O(new_n100_));
  nand3  g66(.a(new_n84_), .b(new_n77_), .c(new_n73_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n100_), .c(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n99_), .c(new_n98_), .d(new_n47_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n61_), .b(x02), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(x16), .c(x09), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(x08), .c(x07), .d(x06), .O(new_n107_));
  oai22  g73(.a(new_n107_), .b(new_n104_), .c(new_n49_), .d(new_n37_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n47_), .O(new_n109_));
  nand4  g75(.a(new_n49_), .b(new_n85_), .c(new_n84_), .d(new_n77_), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n73_), .c(new_n72_), .d(x08), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(new_n113_));
  nand4  g79(.a(new_n113_), .b(x07), .c(x06), .d(x01), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n109_), .c(new_n44_), .O(z10));
  nor2   g81(.a(new_n40_), .b(new_n35_), .O(z11));
  inv1   g82(.a(x16), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(x02), .c(new_n61_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n47_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  nand2  g86(.a(new_n47_), .b(new_n44_), .O(z13));
  inv1   g87(.a(x17), .O(new_n122_));
  nor3   g88(.a(new_n40_), .b(new_n122_), .c(new_n44_), .O(z14));
endmodule


