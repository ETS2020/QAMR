// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n113_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x09), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  oai21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  nand2  g05(.a(new_n37_), .b(new_n36_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(new_n41_));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n41_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n48_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n40_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n40_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n56_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n40_), .O(z04));
  inv1   g28(.a(x16), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x02), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n40_), .O(new_n65_));
  oai21  g31(.a(new_n37_), .b(new_n45_), .c(new_n36_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x16), .c(x03), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n58_), .b(x14), .c(new_n36_), .d(x08), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(z05));
  inv1   g37(.a(new_n68_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n37_), .c(new_n36_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  nand4  g41(.a(new_n72_), .b(x14), .c(new_n75_), .d(new_n36_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n74_), .c(new_n67_), .d(new_n65_), .O(z06));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n58_), .c(x08), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x14), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n36_), .O(new_n81_));
  nand3  g47(.a(new_n75_), .b(new_n36_), .c(x08), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n57_), .c(x11), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n81_), .c(new_n67_), .d(new_n65_), .O(z07));
  nand2  g50(.a(new_n81_), .b(x12), .O(new_n85_));
  nor2   g51(.a(x09), .b(new_n56_), .O(new_n86_));
  nor2   g52(.a(new_n37_), .b(x12), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n86_), .c(new_n78_), .d(new_n58_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n85_), .c(new_n67_), .d(new_n65_), .O(z08));
  nor3   g55(.a(x12), .b(x11), .c(x10), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n72_), .c(new_n37_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(x09), .c(x13), .O(new_n92_));
  nor2   g58(.a(new_n48_), .b(new_n45_), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand3  g61(.a(x14), .b(new_n95_), .c(new_n94_), .O(new_n96_));
  nor3   g62(.a(new_n96_), .b(x11), .c(x10), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n93_), .c(new_n86_), .d(x07), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n92_), .c(new_n67_), .d(new_n65_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  inv1   g66(.a(x11), .O(new_n101_));
  nand4  g67(.a(new_n95_), .b(new_n94_), .c(new_n101_), .d(new_n75_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n100_), .c(x09), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(x08), .c(x07), .d(x06), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n50_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(x14), .c(x04), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(z10));
  nand2  g73(.a(new_n40_), .b(new_n49_), .O(z11));
  nor3   g74(.a(new_n41_), .b(new_n63_), .c(x02), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(x03), .c(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n40_), .O(z12));
  nor2   g77(.a(new_n41_), .b(new_n45_), .O(z13));
  nand2  g78(.a(new_n40_), .b(x17), .O(new_n113_));
  nor2   g79(.a(new_n113_), .b(new_n45_), .O(z14));
endmodule


