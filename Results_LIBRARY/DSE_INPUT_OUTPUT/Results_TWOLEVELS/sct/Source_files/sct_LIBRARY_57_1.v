// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n126_;
  inv1   g00(.a(x14), .O(new_n35_));
  aoi21  g01(.a(x01), .b(x00), .c(new_n35_), .O(new_n36_));
  oai22  g02(.a(new_n36_), .b(x02), .c(x14), .d(x01), .O(z00));
  nor2   g03(.a(new_n35_), .b(x00), .O(new_n38_));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n38_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(new_n38_), .O(new_n46_));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n46_), .c(new_n45_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n46_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n46_), .O(z04));
  inv1   g25(.a(x03), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x02), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x14), .c(x00), .O(new_n62_));
  nand2  g28(.a(new_n35_), .b(new_n47_), .O(new_n63_));
  aoi21  g29(.a(new_n63_), .b(new_n62_), .c(new_n42_), .O(new_n64_));
  nor2   g30(.a(x14), .b(new_n60_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nand3  g32(.a(x08), .b(x07), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n55_), .b(new_n69_), .c(x08), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n46_), .d(x04), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n66_), .O(z05));
  inv1   g39(.a(x10), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n55_), .c(x08), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x04), .O(new_n78_));
  aoi21  g44(.a(new_n70_), .b(x10), .c(new_n78_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n38_), .c(new_n66_), .O(z06));
  inv1   g46(.a(new_n67_), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n69_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x04), .O(new_n84_));
  aoi21  g50(.a(new_n77_), .b(x11), .c(new_n84_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n38_), .c(new_n66_), .O(z07));
  nand2  g52(.a(new_n46_), .b(new_n42_), .O(new_n87_));
  nand2  g53(.a(new_n83_), .b(x12), .O(new_n88_));
  nor4   g54(.a(new_n75_), .b(new_n67_), .c(x12), .d(x11), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n38_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n88_), .c(new_n87_), .d(new_n66_), .O(z08));
  nand2  g57(.a(new_n63_), .b(new_n62_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x16), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n48_), .b(new_n35_), .c(new_n95_), .d(new_n94_), .O(new_n96_));
  nor3   g62(.a(new_n96_), .b(x11), .c(x10), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n69_), .c(x08), .d(x07), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n45_), .c(new_n93_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x04), .O(new_n100_));
  oai21  g66(.a(new_n89_), .b(new_n95_), .c(x04), .O(new_n101_));
  nand4  g67(.a(x08), .b(x07), .c(x06), .d(x00), .O(new_n102_));
  nor3   g68(.a(x13), .b(x12), .c(x11), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n76_), .O(new_n104_));
  nand3  g70(.a(x16), .b(new_n35_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n104_), .b(new_n102_), .c(new_n105_), .O(new_n106_));
  aoi21  g72(.a(new_n101_), .b(new_n46_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n100_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  nand4  g75(.a(new_n82_), .b(new_n35_), .c(new_n95_), .d(new_n94_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n48_), .c(new_n69_), .d(x08), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(x07), .c(x06), .O(new_n114_));
  nand4  g80(.a(new_n61_), .b(x16), .c(x14), .d(x00), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n114_), .c(new_n42_), .O(z10));
  nor2   g82(.a(new_n38_), .b(new_n47_), .O(z11));
  nand2  g83(.a(x16), .b(x14), .O(new_n118_));
  oai21  g84(.a(new_n118_), .b(x02), .c(new_n60_), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(x00), .O(new_n120_));
  inv1   g86(.a(x16), .O(new_n121_));
  oai21  g87(.a(new_n121_), .b(x02), .c(new_n60_), .O(new_n122_));
  nand2  g88(.a(new_n122_), .b(new_n35_), .O(new_n123_));
  aoi21  g89(.a(new_n123_), .b(new_n120_), .c(new_n42_), .O(z12));
  nor2   g90(.a(new_n38_), .b(new_n42_), .O(z13));
  inv1   g91(.a(x17), .O(new_n126_));
  nor3   g92(.a(new_n38_), .b(new_n126_), .c(new_n42_), .O(z14));
endmodule


