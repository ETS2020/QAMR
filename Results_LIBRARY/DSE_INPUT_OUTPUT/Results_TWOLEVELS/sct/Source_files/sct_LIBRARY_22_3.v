// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n113_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  inv1   g04(.a(x03), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nor2   g10(.a(new_n39_), .b(new_n35_), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n44_), .c(new_n45_), .O(new_n46_));
  oai21  g12(.a(new_n43_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(new_n45_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n39_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n48_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n52_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n48_), .O(z04));
  oai21  g29(.a(new_n50_), .b(new_n39_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n35_), .O(new_n65_));
  oai21  g31(.a(new_n50_), .b(x02), .c(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n39_), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n59_), .b(new_n70_), .c(x08), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n45_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(z05));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n59_), .c(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  aoi21  g43(.a(new_n71_), .b(x10), .c(new_n77_), .O(new_n78_));
  nand3  g44(.a(x04), .b(new_n39_), .c(new_n38_), .O(new_n79_));
  oai21  g45(.a(new_n39_), .b(x01), .c(new_n79_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x16), .O(new_n81_));
  oai21  g47(.a(new_n78_), .b(new_n45_), .c(new_n81_), .O(z06));
  inv1   g48(.a(x10), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  inv1   g50(.a(new_n68_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n70_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x04), .O(new_n87_));
  aoi21  g53(.a(new_n76_), .b(x11), .c(new_n87_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n45_), .c(new_n81_), .O(z07));
  and2   g55(.a(new_n86_), .b(x12), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand4  g57(.a(new_n75_), .b(new_n85_), .c(new_n91_), .d(new_n84_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x04), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n90_), .c(new_n48_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n81_), .O(z08));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n91_), .c(new_n84_), .d(new_n83_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n71_), .c(x04), .O(new_n98_));
  aoi21  g64(.a(new_n92_), .b(x13), .c(new_n98_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n45_), .c(new_n81_), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  aoi21  g67(.a(new_n97_), .b(new_n101_), .c(new_n51_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n70_), .c(x08), .d(x07), .O(new_n103_));
  nand2  g69(.a(new_n51_), .b(x14), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n49_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n48_), .O(z10));
  nor2   g73(.a(new_n45_), .b(new_n38_), .O(z11));
  nand2  g74(.a(x16), .b(x04), .O(new_n109_));
  oai21  g75(.a(x04), .b(x01), .c(x03), .O(new_n110_));
  oai21  g76(.a(new_n109_), .b(new_n40_), .c(new_n110_), .O(z12));
  aoi21  g77(.a(x03), .b(x01), .c(new_n44_), .O(z13));
  nand2  g78(.a(x17), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n48_), .O(z14));
endmodule


