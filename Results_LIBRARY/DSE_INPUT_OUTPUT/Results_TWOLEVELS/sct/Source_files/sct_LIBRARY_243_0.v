// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n119_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x09), .b(x00), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  inv1   g10(.a(new_n36_), .O(new_n45_));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n45_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n47_), .c(new_n45_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n53_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n47_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n45_), .O(z04));
  nand2  g25(.a(x09), .b(new_n46_), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x08), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n47_), .c(x07), .O(new_n64_));
  nand3  g30(.a(new_n45_), .b(x16), .c(new_n35_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x04), .O(new_n67_));
  aoi21  g33(.a(x16), .b(x03), .c(new_n42_), .O(new_n68_));
  nand2  g34(.a(x08), .b(x07), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n67_), .d(new_n45_), .O(z05));
  inv1   g37(.a(x16), .O(new_n72_));
  aoi21  g38(.a(x04), .b(new_n35_), .c(x03), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n72_), .c(x04), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n45_), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand3  g42(.a(x08), .b(x07), .c(x06), .O(new_n77_));
  inv1   g43(.a(new_n77_), .O(new_n78_));
  nand3  g44(.a(new_n76_), .b(new_n61_), .c(x08), .O(new_n79_));
  oai22  g45(.a(new_n79_), .b(new_n54_), .c(new_n78_), .d(new_n76_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x00), .O(new_n81_));
  nand2  g47(.a(x10), .b(x09), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n75_), .O(z06));
  nand2  g49(.a(new_n76_), .b(x08), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n54_), .c(x11), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  nand4  g52(.a(new_n78_), .b(new_n86_), .c(new_n76_), .d(new_n61_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x00), .O(new_n89_));
  nand2  g55(.a(x11), .b(x09), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n75_), .O(z07));
  nor3   g57(.a(x12), .b(x11), .c(x10), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n78_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x00), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n61_), .O(new_n95_));
  nand2  g61(.a(new_n87_), .b(x12), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n95_), .c(new_n75_), .O(z08));
  inv1   g63(.a(x13), .O(new_n98_));
  aoi21  g64(.a(new_n92_), .b(new_n78_), .c(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n55_), .b(new_n61_), .c(x08), .O(new_n100_));
  inv1   g66(.a(x12), .O(new_n101_));
  nand4  g67(.a(new_n98_), .b(new_n101_), .c(new_n86_), .d(new_n76_), .O(new_n102_));
  nor2   g68(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n99_), .c(x00), .O(new_n104_));
  nand2  g70(.a(x13), .b(x09), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n75_), .O(z09));
  nand4  g72(.a(new_n47_), .b(new_n61_), .c(x08), .d(x07), .O(new_n107_));
  inv1   g73(.a(x03), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x02), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(x16), .c(x14), .O(new_n110_));
  oai21  g76(.a(new_n107_), .b(new_n46_), .c(new_n110_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n45_), .O(z10));
  nand2  g79(.a(new_n45_), .b(new_n35_), .O(z11));
  nand2  g80(.a(new_n65_), .b(new_n108_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n45_), .O(z12));
  nand2  g83(.a(new_n45_), .b(new_n42_), .O(z13));
  inv1   g84(.a(x17), .O(new_n119_));
  nor3   g85(.a(new_n36_), .b(new_n119_), .c(new_n42_), .O(z14));
endmodule


