// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n117_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x04), .O(new_n38_));
  inv1   g04(.a(x06), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g06(.a(x02), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x01), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n40_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(x05), .b(new_n38_), .O(new_n45_));
  nor2   g11(.a(new_n39_), .b(x04), .O(new_n46_));
  oai22  g12(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x15), .O(new_n47_));
  nand3  g13(.a(x06), .b(x05), .c(new_n38_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n47_), .O(z01));
  oai21  g15(.a(x03), .b(new_n41_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n39_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n56_), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(x07), .c(x06), .O(new_n59_));
  aoi21  g25(.a(new_n59_), .b(new_n57_), .c(new_n38_), .O(new_n60_));
  nor2   g26(.a(new_n58_), .b(x06), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(new_n50_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n40_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x08), .c(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  inv1   g33(.a(x03), .O(new_n68_));
  inv1   g34(.a(x16), .O(new_n69_));
  aoi21  g35(.a(new_n68_), .b(x02), .c(new_n69_), .O(new_n70_));
  nand3  g36(.a(x08), .b(x07), .c(x06), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(x09), .c(new_n70_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n38_), .c(new_n67_), .O(z05));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(x08), .c(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand4  g43(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n78_));
  aoi21  g44(.a(new_n78_), .b(x10), .c(new_n70_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n38_), .c(new_n77_), .O(z06));
  nand2  g46(.a(x07), .b(x06), .O(new_n81_));
  nand2  g47(.a(new_n74_), .b(x08), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n81_), .c(x11), .O(new_n83_));
  inv1   g49(.a(x10), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(new_n71_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n64_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n83_), .c(new_n50_), .d(x04), .O(z07));
  nor2   g54(.a(x12), .b(x11), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n84_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n65_), .c(x04), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x06), .O(new_n92_));
  aoi21  g58(.a(new_n87_), .b(x12), .c(new_n70_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n38_), .c(new_n92_), .O(z08));
  inv1   g60(.a(new_n46_), .O(new_n95_));
  inv1   g61(.a(x12), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n50_), .b(new_n97_), .c(new_n96_), .d(new_n85_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(x10), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n64_), .c(x08), .d(x07), .O(new_n100_));
  nand3  g66(.a(new_n89_), .b(new_n74_), .c(new_n86_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x13), .c(new_n70_), .O(new_n102_));
  oai21  g68(.a(new_n100_), .b(new_n39_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n95_), .O(z09));
  inv1   g71(.a(x00), .O(new_n106_));
  nand4  g72(.a(new_n97_), .b(new_n96_), .c(new_n85_), .d(new_n84_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n70_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n64_), .c(x08), .d(x07), .O(new_n109_));
  oai22  g75(.a(new_n109_), .b(new_n39_), .c(new_n50_), .d(new_n36_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n40_), .O(z10));
  nand2  g78(.a(new_n40_), .b(new_n41_), .O(z11));
  aoi21  g79(.a(x16), .b(new_n41_), .c(x03), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(new_n38_), .c(new_n40_), .O(z12));
  nand2  g81(.a(x06), .b(new_n38_), .O(z13));
  nand2  g82(.a(x17), .b(x04), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n40_), .O(z14));
endmodule


