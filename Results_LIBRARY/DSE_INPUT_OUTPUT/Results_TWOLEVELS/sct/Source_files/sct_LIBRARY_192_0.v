// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_;
  nor2   g00(.a(x17), .b(x10), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n35_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n40_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n49_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n36_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n36_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n56_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n36_), .O(z04));
  inv1   g28(.a(x03), .O(new_n63_));
  inv1   g29(.a(x16), .O(new_n64_));
  nand2  g30(.a(x04), .b(new_n40_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n63_), .c(new_n64_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n67_), .c(x04), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n66_), .c(new_n36_), .O(new_n71_));
  nand2  g37(.a(x10), .b(x04), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand2  g39(.a(x17), .b(new_n73_), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n72_), .c(x09), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x08), .c(x07), .d(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n71_), .O(z05));
  oai21  g43(.a(new_n66_), .b(new_n46_), .c(new_n36_), .O(new_n78_));
  nand3  g44(.a(x17), .b(new_n67_), .c(x08), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n57_), .c(x17), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n73_), .O(new_n81_));
  nand3  g47(.a(new_n58_), .b(new_n67_), .c(x08), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(x10), .c(x04), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n78_), .O(z06));
  inv1   g50(.a(x11), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n67_), .c(x08), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n57_), .c(x17), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n73_), .O(new_n88_));
  nand3  g54(.a(new_n73_), .b(new_n67_), .c(x08), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n57_), .c(x11), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n78_), .O(z07));
  inv1   g57(.a(x12), .O(new_n92_));
  nand4  g58(.a(new_n69_), .b(new_n92_), .c(new_n85_), .d(new_n67_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x17), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n73_), .O(new_n95_));
  nor2   g61(.a(x11), .b(x10), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n67_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n68_), .c(x12), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n95_), .c(new_n78_), .O(z08));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n50_), .b(new_n100_), .c(new_n92_), .d(new_n85_), .O(new_n101_));
  nor3   g67(.a(new_n101_), .b(x10), .c(x09), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(x08), .c(x07), .d(x06), .O(new_n103_));
  nand2  g69(.a(new_n93_), .b(x13), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n46_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x17), .O(new_n106_));
  nand2  g72(.a(x13), .b(x10), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n106_), .c(new_n78_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  nand4  g75(.a(new_n96_), .b(x17), .c(new_n100_), .d(new_n92_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  and2   g77(.a(new_n111_), .b(new_n50_), .O(new_n112_));
  nand4  g78(.a(new_n112_), .b(new_n67_), .c(x08), .d(x07), .O(new_n113_));
  oai22  g79(.a(new_n113_), .b(new_n49_), .c(new_n50_), .d(new_n38_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n36_), .O(z10));
  nand2  g82(.a(new_n36_), .b(new_n40_), .O(z11));
  oai21  g83(.a(new_n64_), .b(x02), .c(new_n63_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n36_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  inv1   g86(.a(x17), .O(new_n121_));
  nand3  g87(.a(new_n57_), .b(new_n67_), .c(x08), .O(new_n122_));
  oai21  g88(.a(new_n122_), .b(new_n57_), .c(x10), .O(new_n123_));
  aoi21  g89(.a(new_n123_), .b(new_n121_), .c(new_n46_), .O(z13));
  nor2   g90(.a(new_n121_), .b(new_n46_), .O(z14));
endmodule


