// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n117_, new_n119_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(x16), .b(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n38_), .b(x01), .c(new_n36_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(x16), .b(x01), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand3  g13(.a(x16), .b(new_n47_), .c(x02), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x01), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n46_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(new_n43_), .O(new_n54_));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(x03), .b(new_n35_), .c(x16), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n54_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n56_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n54_), .O(z04));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n59_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n60_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n54_), .O(new_n70_));
  oai21  g36(.a(new_n42_), .b(x02), .c(new_n47_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x16), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n70_), .O(z05));
  nand2  g39(.a(new_n54_), .b(new_n42_), .O(z13));
  nand2  g40(.a(new_n68_), .b(x10), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n60_), .c(x08), .O(new_n77_));
  and2   g43(.a(new_n77_), .b(new_n54_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n75_), .c(z13), .d(new_n72_), .O(z06));
  nand2  g45(.a(new_n77_), .b(x11), .O(new_n80_));
  inv1   g46(.a(x09), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(new_n65_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n43_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n80_), .c(z13), .d(new_n72_), .O(z07));
  inv1   g52(.a(x12), .O(new_n87_));
  nor2   g53(.a(new_n84_), .b(new_n87_), .O(new_n88_));
  inv1   g54(.a(x11), .O(new_n89_));
  inv1   g55(.a(new_n65_), .O(new_n90_));
  nand4  g56(.a(new_n76_), .b(new_n90_), .c(new_n87_), .d(new_n89_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x04), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n88_), .c(new_n54_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n72_), .O(z08));
  nand4  g60(.a(new_n67_), .b(x07), .c(x06), .d(x04), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n82_), .b(new_n49_), .c(new_n96_), .d(new_n87_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n95_), .c(x04), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x01), .O(new_n99_));
  nor2   g65(.a(x03), .b(new_n35_), .O(new_n100_));
  nor3   g66(.a(x13), .b(x12), .c(x11), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n76_), .c(new_n90_), .d(new_n100_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(x04), .c(new_n47_), .d(x02), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x16), .O(new_n104_));
  aoi21  g70(.a(new_n91_), .b(x13), .c(new_n43_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n99_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  inv1   g73(.a(x10), .O(new_n108_));
  nand4  g74(.a(new_n96_), .b(new_n87_), .c(new_n89_), .d(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(new_n51_), .c(new_n81_), .d(x08), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(x07), .c(x06), .O(new_n113_));
  or2    g79(.a(new_n56_), .b(new_n37_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n113_), .c(new_n42_), .O(z10));
  nand2  g81(.a(new_n54_), .b(new_n35_), .O(z11));
  aoi21  g82(.a(x16), .b(new_n35_), .c(x03), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(new_n42_), .c(new_n54_), .O(z12));
  nand2  g84(.a(x17), .b(x04), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(new_n54_), .O(z14));
endmodule


