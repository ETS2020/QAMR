// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n117_, new_n120_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(new_n36_), .b(x06), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  nand2  g09(.a(x06), .b(new_n35_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n46_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n44_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n44_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n44_), .O(z04));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(x04), .b(new_n47_), .c(x03), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n44_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand2  g30(.a(x08), .b(x07), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand3  g32(.a(new_n55_), .b(new_n64_), .c(x08), .O(new_n67_));
  oai21  g33(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  aoi22  g34(.a(new_n68_), .b(x01), .c(x09), .d(new_n46_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n63_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  nand3  g37(.a(new_n64_), .b(x08), .c(x07), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n55_), .c(x08), .O(new_n75_));
  oai21  g41(.a(new_n73_), .b(new_n71_), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x01), .O(new_n77_));
  nand2  g43(.a(x10), .b(new_n46_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n63_), .O(z06));
  inv1   g45(.a(x11), .O(new_n80_));
  aoi21  g46(.a(new_n74_), .b(new_n66_), .c(new_n80_), .O(new_n81_));
  nand2  g47(.a(new_n80_), .b(new_n71_), .O(new_n82_));
  inv1   g48(.a(new_n82_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n66_), .c(new_n64_), .d(x06), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n81_), .c(x01), .O(new_n86_));
  nand2  g52(.a(x11), .b(new_n46_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n63_), .O(z07));
  inv1   g54(.a(x12), .O(new_n89_));
  nand4  g55(.a(new_n73_), .b(new_n89_), .c(new_n80_), .d(new_n71_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x01), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x06), .O(new_n92_));
  nand2  g58(.a(new_n84_), .b(x12), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n63_), .O(z08));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n48_), .b(new_n95_), .c(new_n89_), .d(new_n80_), .O(new_n96_));
  nor3   g62(.a(new_n96_), .b(x10), .c(x09), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x08), .c(x07), .d(x06), .O(new_n98_));
  nand2  g64(.a(new_n90_), .b(x13), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n42_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x01), .O(new_n101_));
  nand2  g67(.a(x13), .b(new_n46_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(new_n63_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(new_n48_), .O(new_n105_));
  nand3  g71(.a(new_n83_), .b(new_n95_), .c(new_n89_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n104_), .c(new_n105_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n64_), .c(x08), .d(x07), .O(new_n108_));
  nand2  g74(.a(new_n105_), .b(x14), .O(new_n109_));
  oai21  g75(.a(new_n108_), .b(new_n46_), .c(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x01), .O(new_n111_));
  nand3  g77(.a(new_n105_), .b(x14), .c(new_n46_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n111_), .c(new_n42_), .O(z10));
  nand2  g79(.a(new_n44_), .b(new_n47_), .O(z11));
  inv1   g80(.a(x03), .O(new_n115_));
  oai21  g81(.a(new_n60_), .b(x02), .c(new_n115_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n44_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  nand2  g84(.a(new_n44_), .b(new_n42_), .O(z13));
  nand2  g85(.a(x17), .b(x04), .O(new_n120_));
  nand2  g86(.a(new_n120_), .b(new_n44_), .O(z14));
endmodule


