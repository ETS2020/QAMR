// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n113_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  inv1   g02(.a(x15), .O(new_n37_));
  oai21  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand2  g10(.a(new_n37_), .b(new_n35_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(new_n45_), .O(new_n48_));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  inv1   g24(.a(x07), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(new_n47_), .O(new_n60_));
  nand3  g26(.a(new_n58_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n50_), .c(new_n45_), .d(x04), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  nor2   g30(.a(new_n43_), .b(x02), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x03), .c(x16), .O(new_n66_));
  nand3  g32(.a(x08), .b(x07), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n60_), .b(new_n69_), .c(x08), .O(new_n70_));
  and2   g36(.a(new_n70_), .b(new_n45_), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n68_), .c(new_n66_), .d(x04), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n60_), .c(x08), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n45_), .c(x04), .O(new_n75_));
  aoi21  g41(.a(new_n70_), .b(x10), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n66_), .O(z06));
  nand2  g43(.a(new_n74_), .b(x11), .O(new_n78_));
  nor4   g44(.a(new_n67_), .b(x11), .c(x10), .d(x09), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n48_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n78_), .c(new_n66_), .d(x04), .O(z07));
  inv1   g47(.a(new_n79_), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  inv1   g50(.a(new_n67_), .O(new_n85_));
  nand4  g51(.a(new_n73_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n45_), .c(x04), .O(new_n87_));
  aoi21  g53(.a(new_n82_), .b(x12), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n66_), .O(z08));
  inv1   g55(.a(x10), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n50_), .b(new_n45_), .c(new_n91_), .d(new_n84_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x11), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n90_), .c(new_n69_), .d(x08), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n59_), .O(new_n95_));
  inv1   g61(.a(x16), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(x02), .O(new_n97_));
  aoi21  g63(.a(new_n95_), .b(x06), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n86_), .b(x13), .O(new_n99_));
  aoi21  g65(.a(x16), .b(x03), .c(new_n48_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(x04), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand4  g68(.a(new_n91_), .b(new_n84_), .c(new_n83_), .d(new_n90_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n51_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n69_), .c(x08), .d(x07), .O(new_n105_));
  oai22  g71(.a(new_n105_), .b(new_n47_), .c(new_n50_), .d(new_n36_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n45_), .c(x04), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(z10));
  nor2   g74(.a(new_n48_), .b(new_n49_), .O(z11));
  oai21  g75(.a(new_n97_), .b(x03), .c(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n45_), .O(z12));
  nand2  g77(.a(new_n45_), .b(new_n43_), .O(z13));
  nand2  g78(.a(x17), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n45_), .O(z14));
endmodule


