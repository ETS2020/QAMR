// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_;
  inv1   g00(.a(x05), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g06(.a(x02), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x01), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n40_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x04), .O(new_n44_));
  inv1   g10(.a(x17), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(new_n44_), .c(x05), .O(new_n46_));
  inv1   g12(.a(x18), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x15), .c(new_n35_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n46_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n41_), .c(x16), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n37_), .c(new_n50_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n51_), .c(new_n37_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n37_), .O(z04));
  inv1   g28(.a(x03), .O(new_n63_));
  oai21  g29(.a(new_n44_), .b(x02), .c(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x16), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  nand3  g35(.a(new_n58_), .b(new_n67_), .c(x08), .O(new_n70_));
  oai21  g36(.a(new_n69_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n66_), .c(new_n37_), .O(new_n72_));
  oai21  g38(.a(x17), .b(new_n35_), .c(new_n44_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(z05));
  nand3  g40(.a(new_n64_), .b(new_n37_), .c(x16), .O(new_n75_));
  nand2  g41(.a(new_n70_), .b(x10), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n58_), .c(x08), .O(new_n78_));
  and2   g44(.a(new_n78_), .b(new_n37_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n76_), .c(new_n75_), .d(new_n73_), .O(z06));
  nand2  g46(.a(new_n78_), .b(x11), .O(new_n81_));
  inv1   g47(.a(x10), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand4  g49(.a(new_n69_), .b(new_n83_), .c(new_n82_), .d(new_n67_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n81_), .c(new_n65_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n37_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n73_), .O(z07));
  nand2  g53(.a(new_n84_), .b(x12), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand4  g55(.a(new_n77_), .b(new_n69_), .c(new_n89_), .d(new_n83_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n65_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n37_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n73_), .O(z08));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n51_), .b(new_n94_), .c(new_n89_), .d(new_n83_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(x09), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  oai21  g63(.a(x12), .b(x11), .c(x13), .O(new_n98_));
  oai21  g64(.a(new_n97_), .b(new_n44_), .c(new_n98_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n82_), .O(new_n100_));
  nand2  g66(.a(new_n78_), .b(x13), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n73_), .c(new_n37_), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n100_), .c(new_n75_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  inv1   g71(.a(x16), .O(new_n106_));
  aoi21  g72(.a(new_n63_), .b(x02), .c(new_n106_), .O(new_n107_));
  nand4  g73(.a(new_n94_), .b(new_n89_), .c(new_n83_), .d(new_n82_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n105_), .c(new_n107_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n67_), .c(x08), .d(x07), .O(new_n110_));
  oai22  g76(.a(new_n110_), .b(new_n50_), .c(new_n51_), .d(new_n39_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n37_), .O(z10));
  nor2   g79(.a(new_n36_), .b(new_n41_), .O(z11));
  nor2   g80(.a(new_n36_), .b(new_n106_), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n41_), .c(x03), .O(new_n116_));
  oai21  g82(.a(new_n116_), .b(new_n44_), .c(new_n37_), .O(z12));
  nor2   g83(.a(new_n36_), .b(new_n44_), .O(z13));
  nor2   g84(.a(new_n45_), .b(new_n44_), .O(z14));
endmodule


