// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n115_,
    new_n117_;
  nor2   g00(.a(x16), .b(x15), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  aoi21  g08(.a(x16), .b(x05), .c(x15), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(x16), .c(x15), .O(new_n45_));
  oai22  g11(.a(new_n45_), .b(x05), .c(new_n43_), .d(x04), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n40_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n36_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x03), .O(new_n52_));
  nand3  g18(.a(x16), .b(new_n52_), .c(x02), .O(new_n53_));
  inv1   g19(.a(x16), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x15), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n51_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n56_), .c(x04), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n60_), .b(new_n67_), .c(x08), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n36_), .O(new_n70_));
  inv1   g36(.a(x04), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(x02), .c(new_n52_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x16), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n70_), .O(z05));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n60_), .c(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  aoi21  g43(.a(new_n68_), .b(x10), .c(new_n77_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n35_), .c(new_n73_), .O(z06));
  nand2  g45(.a(new_n76_), .b(x11), .O(new_n80_));
  nor4   g46(.a(new_n65_), .b(x11), .c(x10), .d(x09), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n36_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n73_), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nor2   g52(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(new_n65_), .O(new_n89_));
  nand4  g55(.a(new_n75_), .b(new_n89_), .c(new_n86_), .d(new_n88_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x04), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n87_), .c(new_n36_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n73_), .O(z08));
  nand2  g59(.a(new_n36_), .b(new_n71_), .O(z13));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n48_), .b(new_n95_), .c(new_n86_), .d(new_n88_), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(x10), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n67_), .c(x08), .d(x07), .O(new_n98_));
  nand2  g64(.a(x16), .b(new_n40_), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n47_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x04), .O(new_n101_));
  oai21  g67(.a(new_n54_), .b(new_n52_), .c(new_n36_), .O(new_n102_));
  aoi21  g68(.a(new_n90_), .b(x13), .c(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n101_), .c(z13), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  aoi21  g71(.a(new_n52_), .b(x02), .c(new_n54_), .O(new_n106_));
  inv1   g72(.a(x10), .O(new_n107_));
  nand4  g73(.a(new_n95_), .b(new_n86_), .c(new_n88_), .d(new_n107_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n105_), .c(new_n106_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n67_), .c(x08), .d(x07), .O(new_n110_));
  oai22  g76(.a(new_n110_), .b(new_n47_), .c(new_n48_), .d(new_n38_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n36_), .O(z10));
  nor2   g79(.a(new_n35_), .b(new_n40_), .O(z11));
  aoi21  g80(.a(x16), .b(new_n40_), .c(x03), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n71_), .c(new_n36_), .O(z12));
  inv1   g82(.a(x17), .O(new_n117_));
  nor3   g83(.a(new_n35_), .b(new_n117_), .c(new_n71_), .O(z14));
endmodule


