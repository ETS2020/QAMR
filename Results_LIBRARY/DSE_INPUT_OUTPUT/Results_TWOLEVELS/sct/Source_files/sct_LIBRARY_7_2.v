// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n106_;
  inv1   g00(.a(x04), .O(new_n35_));
  nor2   g01(.a(x06), .b(new_n35_), .O(z02));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(z02), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  aoi21  g11(.a(x06), .b(new_n43_), .c(new_n35_), .O(new_n46_));
  oai22  g12(.a(new_n46_), .b(new_n45_), .c(new_n43_), .d(x04), .O(z01));
  inv1   g13(.a(x07), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n37_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x06), .c(new_n35_), .O(z03));
  xor2a  g17(.a(x08), .b(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(x06), .c(new_n35_), .O(z04));
  inv1   g20(.a(x06), .O(new_n55_));
  aoi21  g21(.a(x16), .b(new_n37_), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x09), .O(new_n58_));
  inv1   g24(.a(x09), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(x08), .c(x07), .O(new_n60_));
  nand3  g26(.a(x16), .b(x06), .c(x03), .O(new_n61_));
  and2   g27(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n58_), .c(new_n56_), .d(x04), .O(z05));
  inv1   g29(.a(x03), .O(new_n64_));
  oai21  g30(.a(new_n35_), .b(x02), .c(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x16), .O(new_n66_));
  nand2  g32(.a(new_n60_), .b(x10), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  inv1   g34(.a(new_n57_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n59_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x04), .O(z06));
  nand2  g39(.a(new_n70_), .b(x11), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  nand4  g41(.a(new_n69_), .b(new_n75_), .c(new_n68_), .d(new_n59_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n66_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x06), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x04), .O(z07));
  nand2  g45(.a(new_n76_), .b(x12), .O(new_n80_));
  inv1   g46(.a(new_n61_), .O(new_n81_));
  nor4   g47(.a(new_n60_), .b(x12), .c(x11), .d(x10), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n80_), .c(new_n56_), .d(x04), .O(z08));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n49_), .b(new_n86_), .c(new_n85_), .d(new_n75_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x10), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n59_), .c(x08), .d(x07), .O(new_n89_));
  nand2  g55(.a(x16), .b(new_n37_), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n89_), .c(new_n35_), .O(new_n91_));
  nand2  g57(.a(x16), .b(x03), .O(new_n92_));
  oai21  g58(.a(new_n82_), .b(new_n86_), .c(new_n92_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n91_), .c(x06), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand4  g62(.a(new_n86_), .b(new_n85_), .c(new_n75_), .d(new_n68_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n49_), .c(new_n59_), .d(x08), .O(new_n99_));
  oai22  g65(.a(new_n99_), .b(new_n48_), .c(new_n49_), .d(new_n40_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(x06), .c(x04), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(z10));
  nor2   g68(.a(z02), .b(new_n37_), .O(z11));
  aoi21  g69(.a(new_n56_), .b(new_n64_), .c(new_n35_), .O(z12));
  nor2   g70(.a(new_n55_), .b(new_n35_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(x06), .c(new_n35_), .O(z14));
endmodule


