// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n116_, new_n119_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  nand2  g01(.a(x10), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n36_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n37_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n36_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand3  g21(.a(new_n53_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n47_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n36_), .O(z04));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(x04), .b(new_n37_), .c(x03), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n36_), .O(new_n63_));
  nand2  g29(.a(x08), .b(x07), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x10), .c(x06), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(x08), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  aoi22  g36(.a(new_n70_), .b(new_n55_), .c(new_n66_), .d(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n63_), .O(z05));
  nand3  g38(.a(x16), .b(new_n68_), .c(new_n37_), .O(new_n73_));
  oai21  g39(.a(new_n68_), .b(x06), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x04), .O(new_n75_));
  nand2  g41(.a(new_n36_), .b(new_n43_), .O(new_n76_));
  oai21  g42(.a(new_n64_), .b(x09), .c(new_n68_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x06), .O(new_n78_));
  nand3  g44(.a(x16), .b(new_n68_), .c(x03), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n76_), .d(new_n75_), .O(z06));
  inv1   g46(.a(x11), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n67_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n64_), .c(new_n68_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nand3  g50(.a(new_n55_), .b(new_n67_), .c(x08), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x11), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n63_), .O(z07));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n81_), .c(new_n67_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n64_), .c(new_n68_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  inv1   g57(.a(new_n55_), .O(new_n92_));
  nand3  g58(.a(new_n81_), .b(new_n67_), .c(x08), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n92_), .c(x12), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n91_), .c(new_n63_), .O(z08));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n47_), .b(new_n96_), .c(new_n88_), .d(new_n81_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(x09), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(x08), .c(x07), .d(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n68_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x06), .O(new_n101_));
  nand2  g67(.a(new_n65_), .b(x06), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n89_), .c(x13), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n101_), .c(new_n63_), .O(z09));
  inv1   g70(.a(new_n47_), .O(new_n105_));
  inv1   g71(.a(x00), .O(new_n106_));
  nand3  g72(.a(new_n96_), .b(new_n88_), .c(new_n81_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n47_), .c(new_n67_), .d(x08), .O(new_n109_));
  nor2   g75(.a(new_n109_), .b(new_n54_), .O(new_n110_));
  aoi22  g76(.a(new_n110_), .b(x06), .c(new_n105_), .d(x14), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n43_), .c(new_n36_), .O(z10));
  nand2  g78(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g79(.a(x03), .O(new_n114_));
  oai21  g80(.a(new_n60_), .b(x02), .c(new_n114_), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(new_n36_), .c(x04), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(z12));
  aoi21  g83(.a(x10), .b(x06), .c(new_n43_), .O(z13));
  nand3  g84(.a(new_n36_), .b(x17), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z14));
endmodule


