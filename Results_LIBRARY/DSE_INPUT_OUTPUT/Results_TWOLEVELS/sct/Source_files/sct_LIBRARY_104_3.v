// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g03(.a(x06), .b(x05), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  oai21  g08(.a(x06), .b(new_n42_), .c(x05), .O(new_n43_));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(new_n43_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n39_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x06), .c(new_n44_), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  oai21  g22(.a(new_n52_), .b(new_n48_), .c(x08), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n49_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n38_), .O(z04));
  inv1   g28(.a(x03), .O(new_n63_));
  oai21  g29(.a(new_n42_), .b(x02), .c(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n38_), .c(x16), .O(new_n65_));
  aoi21  g31(.a(x06), .b(x05), .c(x04), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(x08), .c(x07), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(x05), .c(x06), .O(new_n71_));
  nand3  g37(.a(x08), .b(x07), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n71_), .c(new_n67_), .d(new_n65_), .O(z05));
  inv1   g40(.a(x10), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(x08), .c(x07), .d(x06), .O(new_n77_));
  oai21  g43(.a(new_n70_), .b(new_n75_), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n44_), .O(new_n79_));
  nand2  g45(.a(x10), .b(new_n48_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n67_), .d(new_n65_), .O(z06));
  inv1   g47(.a(x11), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n75_), .c(new_n68_), .O(new_n83_));
  nor3   g49(.a(new_n83_), .b(new_n58_), .c(new_n52_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(x05), .c(x06), .O(new_n85_));
  nand2  g51(.a(new_n77_), .b(x11), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n67_), .d(new_n65_), .O(z07));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n76_), .b(new_n88_), .c(new_n82_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n72_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  oai21  g57(.a(new_n84_), .b(new_n88_), .c(new_n91_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n44_), .O(new_n93_));
  nand2  g59(.a(x12), .b(new_n48_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n67_), .d(new_n65_), .O(z08));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n49_), .b(new_n96_), .c(new_n88_), .d(new_n82_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(x10), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n68_), .c(x08), .d(x07), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n42_), .c(new_n44_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x06), .O(new_n101_));
  aoi21  g67(.a(new_n91_), .b(x13), .c(new_n66_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(new_n65_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(x16), .O(new_n105_));
  aoi21  g71(.a(new_n63_), .b(x02), .c(new_n105_), .O(new_n106_));
  nand4  g72(.a(new_n96_), .b(new_n88_), .c(new_n82_), .d(new_n75_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n104_), .c(new_n106_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n68_), .c(x08), .d(x07), .O(new_n109_));
  oai22  g75(.a(new_n109_), .b(new_n48_), .c(new_n49_), .d(new_n36_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n38_), .O(z10));
  nand2  g78(.a(new_n38_), .b(new_n39_), .O(z11));
  oai21  g79(.a(new_n105_), .b(x02), .c(new_n63_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n38_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  nand2  g82(.a(new_n38_), .b(new_n42_), .O(z13));
  nand3  g83(.a(new_n38_), .b(x17), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


