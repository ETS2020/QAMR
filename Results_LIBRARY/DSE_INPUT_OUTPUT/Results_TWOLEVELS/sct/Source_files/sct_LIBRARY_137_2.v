// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_, new_n119_, new_n122_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand3  g01(.a(x13), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x13), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n39_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n48_), .c(new_n39_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n39_), .O(z04));
  inv1   g25(.a(x03), .O(new_n60_));
  oai21  g26(.a(new_n44_), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x16), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n39_), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n54_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n55_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n66_), .c(new_n64_), .d(new_n39_), .O(z05));
  nand2  g35(.a(new_n68_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand4  g38(.a(new_n55_), .b(new_n72_), .c(new_n71_), .d(x08), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n70_), .c(new_n64_), .d(new_n39_), .O(z06));
  nand2  g40(.a(new_n73_), .b(x11), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  inv1   g42(.a(new_n65_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(new_n72_), .d(new_n71_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n75_), .c(new_n64_), .d(new_n39_), .O(z07));
  nand2  g45(.a(new_n78_), .b(x12), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n62_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n39_), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  inv1   g49(.a(x01), .O(new_n84_));
  nand4  g50(.a(new_n48_), .b(new_n38_), .c(x04), .d(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n38_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n83_), .c(new_n76_), .d(new_n72_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x09), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(x08), .c(x07), .d(x06), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n82_), .O(z08));
  nand3  g56(.a(new_n67_), .b(x06), .c(new_n84_), .O(new_n91_));
  nand4  g57(.a(new_n38_), .b(new_n83_), .c(new_n76_), .d(new_n72_), .O(new_n92_));
  oai22  g58(.a(new_n92_), .b(new_n91_), .c(new_n38_), .d(x06), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n48_), .c(x07), .O(new_n94_));
  nand3  g60(.a(new_n39_), .b(x16), .c(new_n35_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x04), .O(new_n97_));
  aoi21  g63(.a(x16), .b(x03), .c(new_n44_), .O(new_n98_));
  nand2  g64(.a(new_n67_), .b(x07), .O(new_n99_));
  nand3  g65(.a(new_n83_), .b(new_n76_), .c(new_n72_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n99_), .c(x13), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n98_), .c(new_n97_), .d(new_n39_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  inv1   g69(.a(x14), .O(new_n104_));
  nand2  g70(.a(new_n60_), .b(x02), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(x16), .c(x09), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(x08), .c(x07), .d(x06), .O(new_n107_));
  oai22  g73(.a(new_n107_), .b(new_n103_), .c(new_n48_), .d(new_n104_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n39_), .O(new_n109_));
  nand4  g75(.a(new_n48_), .b(new_n38_), .c(new_n83_), .d(new_n76_), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n72_), .c(new_n71_), .d(x08), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(new_n113_));
  nand4  g79(.a(new_n113_), .b(x07), .c(x06), .d(new_n84_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n109_), .c(new_n44_), .O(z10));
  nor2   g81(.a(new_n40_), .b(new_n35_), .O(z11));
  inv1   g82(.a(x16), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(x02), .c(new_n60_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n39_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  nand2  g86(.a(new_n39_), .b(new_n44_), .O(z13));
  nand2  g87(.a(new_n39_), .b(x17), .O(new_n122_));
  nor2   g88(.a(new_n122_), .b(new_n44_), .O(z14));
endmodule


