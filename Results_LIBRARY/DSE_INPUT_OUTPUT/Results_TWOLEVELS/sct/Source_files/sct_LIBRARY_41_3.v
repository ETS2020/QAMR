// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x07), .O(new_n35_));
  inv1   g01(.a(x17), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n37_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  nand2  g14(.a(x17), .b(x07), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n40_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand3  g18(.a(new_n36_), .b(x07), .c(new_n48_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n48_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n38_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nor2   g23(.a(new_n35_), .b(new_n48_), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n38_), .O(z04));
  inv1   g28(.a(x16), .O(new_n63_));
  aoi21  g29(.a(x04), .b(new_n40_), .c(x03), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n63_), .c(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n38_), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n57_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n36_), .c(new_n35_), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nand3  g36(.a(x08), .b(x07), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z05));
  nand2  g39(.a(new_n70_), .b(x10), .O(new_n74_));
  inv1   g40(.a(x09), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  inv1   g42(.a(new_n71_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n36_), .c(new_n76_), .d(new_n75_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n74_), .c(new_n66_), .O(z06));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n75_), .O(new_n81_));
  nor3   g47(.a(new_n81_), .b(new_n57_), .c(new_n48_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(x17), .c(x07), .O(new_n83_));
  inv1   g49(.a(new_n58_), .O(new_n84_));
  nand3  g50(.a(new_n76_), .b(new_n75_), .c(x08), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n84_), .c(x11), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n83_), .c(new_n66_), .O(z07));
  nand2  g53(.a(new_n83_), .b(x12), .O(new_n88_));
  nor2   g54(.a(x17), .b(x12), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n80_), .c(new_n67_), .d(new_n58_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n66_), .O(z08));
  inv1   g57(.a(x11), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n50_), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  nor3   g61(.a(new_n95_), .b(x10), .c(x09), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  nand3  g63(.a(new_n93_), .b(new_n92_), .c(new_n76_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n68_), .c(x13), .O(new_n99_));
  oai21  g65(.a(new_n97_), .b(new_n45_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n36_), .O(new_n101_));
  nand2  g67(.a(x13), .b(new_n35_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(new_n66_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(new_n50_), .O(new_n105_));
  nand3  g71(.a(new_n80_), .b(new_n94_), .c(new_n93_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n104_), .c(new_n105_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n75_), .c(x08), .d(x07), .O(new_n108_));
  nand2  g74(.a(new_n105_), .b(x14), .O(new_n109_));
  oai21  g75(.a(new_n108_), .b(new_n48_), .c(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n36_), .O(new_n111_));
  nand3  g77(.a(new_n105_), .b(x14), .c(new_n35_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n111_), .c(new_n45_), .O(z10));
  nor2   g79(.a(new_n37_), .b(new_n40_), .O(z11));
  inv1   g80(.a(x03), .O(new_n115_));
  oai21  g81(.a(new_n63_), .b(x02), .c(new_n115_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n38_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  aoi21  g84(.a(x17), .b(x07), .c(new_n45_), .O(z13));
  aoi21  g85(.a(new_n35_), .b(new_n45_), .c(new_n36_), .O(z14));
endmodule


