// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n118_, new_n120_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand3  g01(.a(x16), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x01), .O(new_n41_));
  nor2   g07(.a(x16), .b(new_n41_), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n40_), .c(new_n42_), .O(new_n43_));
  oai21  g09(.a(new_n39_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(new_n42_), .O(new_n45_));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  nor2   g13(.a(x03), .b(new_n35_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n45_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n45_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n45_), .O(z04));
  nand2  g27(.a(new_n45_), .b(new_n40_), .O(z13));
  inv1   g28(.a(x03), .O(new_n63_));
  oai21  g29(.a(new_n40_), .b(x02), .c(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x16), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n56_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n57_), .c(new_n42_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(z13), .O(z05));
  nand2  g36(.a(new_n68_), .b(new_n57_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n57_), .c(x08), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(new_n45_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n65_), .d(z13), .O(z06));
  nand2  g42(.a(new_n74_), .b(x11), .O(new_n77_));
  inv1   g43(.a(x09), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n66_), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(new_n40_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n45_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n65_), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nor2   g52(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(new_n66_), .O(new_n89_));
  nand4  g55(.a(new_n73_), .b(new_n89_), .c(new_n86_), .d(new_n88_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x04), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n87_), .c(new_n45_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n65_), .O(z08));
  nand4  g59(.a(new_n68_), .b(x07), .c(x06), .d(x04), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n79_), .b(new_n47_), .c(new_n95_), .d(new_n86_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n94_), .c(x04), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n41_), .O(new_n98_));
  nor3   g64(.a(x13), .b(x12), .c(x11), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n73_), .c(new_n89_), .d(new_n48_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(x04), .c(new_n63_), .d(x02), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x16), .O(new_n102_));
  aoi21  g68(.a(new_n90_), .b(x13), .c(new_n42_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n102_), .c(new_n98_), .O(z09));
  nand3  g70(.a(x16), .b(new_n63_), .c(x02), .O(new_n105_));
  nand2  g71(.a(new_n47_), .b(new_n41_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g73(.a(x00), .O(new_n108_));
  inv1   g74(.a(x10), .O(new_n109_));
  nand4  g75(.a(new_n95_), .b(new_n86_), .c(new_n88_), .d(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n107_), .c(new_n78_), .d(x08), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(x07), .c(x06), .O(new_n114_));
  nand2  g80(.a(new_n49_), .b(x14), .O(new_n115_));
  aoi21  g81(.a(new_n115_), .b(new_n114_), .c(new_n40_), .O(z10));
  nand2  g82(.a(new_n45_), .b(new_n35_), .O(z11));
  aoi21  g83(.a(x16), .b(new_n35_), .c(x03), .O(new_n118_));
  oai21  g84(.a(new_n118_), .b(new_n40_), .c(new_n45_), .O(z12));
  inv1   g85(.a(x17), .O(new_n120_));
  nor3   g86(.a(new_n42_), .b(new_n120_), .c(new_n40_), .O(z14));
endmodule


