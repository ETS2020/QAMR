// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_, new_n119_, new_n122_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g05(.a(x17), .b(x00), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(new_n41_));
  aoi21  g07(.a(new_n39_), .b(new_n36_), .c(new_n41_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n35_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n49_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n40_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n40_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n40_), .O(z04));
  inv1   g27(.a(x00), .O(new_n62_));
  inv1   g28(.a(x03), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x02), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(x16), .c(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand2  g32(.a(x16), .b(new_n35_), .O(new_n67_));
  oai21  g33(.a(new_n66_), .b(new_n62_), .c(new_n67_), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nand2  g36(.a(x16), .b(x03), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(x04), .O(new_n72_));
  aoi21  g38(.a(new_n68_), .b(x04), .c(new_n72_), .O(new_n73_));
  nand2  g39(.a(x04), .b(new_n35_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  and2   g41(.a(new_n75_), .b(x16), .O(new_n76_));
  inv1   g42(.a(x09), .O(new_n77_));
  nand3  g43(.a(new_n57_), .b(new_n77_), .c(x08), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n70_), .c(x04), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n76_), .c(new_n62_), .O(new_n80_));
  oai21  g46(.a(new_n73_), .b(x17), .c(new_n80_), .O(z05));
  inv1   g47(.a(new_n76_), .O(new_n82_));
  nor2   g48(.a(x10), .b(x09), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n57_), .c(x08), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x04), .O(new_n85_));
  aoi21  g51(.a(new_n78_), .b(x10), .c(new_n85_), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n82_), .c(new_n41_), .O(z06));
  oai21  g53(.a(new_n76_), .b(new_n46_), .c(new_n40_), .O(new_n88_));
  nand2  g54(.a(new_n84_), .b(x11), .O(new_n89_));
  inv1   g55(.a(x10), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  inv1   g57(.a(new_n69_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n77_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n89_), .c(new_n88_), .d(new_n40_), .O(z07));
  inv1   g60(.a(x12), .O(new_n95_));
  nand4  g61(.a(new_n83_), .b(new_n92_), .c(new_n95_), .d(new_n91_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x04), .O(new_n97_));
  aoi21  g63(.a(new_n93_), .b(x12), .c(new_n97_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n82_), .c(new_n41_), .O(z08));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n50_), .b(new_n100_), .c(new_n95_), .d(new_n91_), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n90_), .c(new_n77_), .d(x08), .O(new_n103_));
  inv1   g69(.a(new_n103_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(x07), .c(x06), .d(x04), .O(new_n105_));
  aoi21  g71(.a(new_n96_), .b(x13), .c(new_n41_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n105_), .c(new_n88_), .O(z09));
  nand4  g73(.a(new_n100_), .b(new_n95_), .c(new_n91_), .d(new_n90_), .O(new_n108_));
  inv1   g74(.a(x17), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x00), .O(new_n110_));
  aoi22  g76(.a(new_n110_), .b(new_n108_), .c(new_n64_), .d(x16), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n77_), .c(x08), .d(x07), .O(new_n112_));
  oai22  g78(.a(new_n112_), .b(new_n49_), .c(new_n50_), .d(new_n38_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n40_), .O(z10));
  nand2  g81(.a(new_n40_), .b(new_n35_), .O(z11));
  nand3  g82(.a(new_n40_), .b(x16), .c(new_n35_), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n63_), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(x04), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(new_n40_), .O(z12));
  nand2  g86(.a(new_n40_), .b(new_n46_), .O(z13));
  nand3  g87(.a(x17), .b(x04), .c(new_n62_), .O(new_n122_));
  inv1   g88(.a(new_n122_), .O(z14));
endmodule


