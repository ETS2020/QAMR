// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n122_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(new_n36_), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  inv1   g14(.a(new_n37_), .O(new_n49_));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n45_), .O(new_n55_));
  nand3  g21(.a(new_n53_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n46_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n49_), .O(z04));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(x04), .b(new_n35_), .c(x03), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n49_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nor2   g30(.a(new_n53_), .b(new_n54_), .O(new_n65_));
  nand3  g31(.a(new_n64_), .b(x08), .c(x07), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(new_n36_), .b(x09), .c(new_n45_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(z05));
  nand3  g36(.a(new_n55_), .b(new_n64_), .c(x08), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x08), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  aoi22  g40(.a(new_n74_), .b(new_n55_), .c(new_n71_), .d(x10), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n63_), .O(z06));
  inv1   g42(.a(x11), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n36_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  aoi21  g45(.a(new_n72_), .b(new_n65_), .c(new_n77_), .O(new_n80_));
  nand2  g46(.a(new_n65_), .b(x06), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nor2   g48(.a(new_n78_), .b(x09), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n79_), .c(new_n63_), .O(z07));
  oai21  g51(.a(x12), .b(x10), .c(new_n45_), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  aoi21  g53(.a(new_n83_), .b(new_n65_), .c(new_n87_), .O(new_n88_));
  nand3  g54(.a(new_n72_), .b(new_n87_), .c(new_n77_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n81_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n86_), .c(new_n63_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n46_), .b(new_n93_), .c(new_n87_), .d(new_n77_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n36_), .c(new_n64_), .d(x08), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x07), .c(x06), .d(x04), .O(new_n98_));
  aoi21  g64(.a(new_n93_), .b(new_n36_), .c(x06), .O(new_n99_));
  inv1   g65(.a(new_n66_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n87_), .c(new_n77_), .d(new_n36_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x13), .c(new_n99_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n98_), .c(new_n63_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(x03), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(x02), .c(new_n60_), .O(new_n106_));
  nand4  g72(.a(new_n93_), .b(new_n87_), .c(new_n77_), .d(new_n36_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n104_), .c(new_n106_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n64_), .c(x08), .d(x07), .O(new_n109_));
  nand2  g75(.a(new_n106_), .b(x14), .O(new_n110_));
  oai21  g76(.a(new_n109_), .b(new_n45_), .c(new_n110_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n49_), .O(z10));
  nand2  g79(.a(new_n49_), .b(new_n35_), .O(z11));
  aoi21  g80(.a(x16), .b(new_n35_), .c(x03), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n49_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  oai21  g84(.a(new_n106_), .b(x06), .c(new_n115_), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(new_n36_), .O(new_n120_));
  aoi21  g86(.a(new_n120_), .b(new_n45_), .c(new_n42_), .O(z13));
  inv1   g87(.a(x17), .O(new_n122_));
  nor3   g88(.a(new_n37_), .b(new_n122_), .c(new_n42_), .O(z14));
endmodule


