// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x07), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x05), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x15), .O(new_n42_));
  nand2  g08(.a(x18), .b(new_n42_), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n35_), .c(new_n41_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x04), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  nand2  g12(.a(x07), .b(new_n41_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n37_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  nand3  g16(.a(x07), .b(new_n46_), .c(x05), .O(new_n51_));
  oai21  g17(.a(x07), .b(new_n46_), .c(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n48_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(new_n36_), .O(new_n55_));
  inv1   g21(.a(x08), .O(new_n56_));
  nor2   g22(.a(new_n35_), .b(new_n46_), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n48_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n55_), .O(z04));
  inv1   g27(.a(x16), .O(new_n62_));
  aoi21  g28(.a(x04), .b(new_n37_), .c(x03), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n55_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x05), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  aoi22  g35(.a(new_n69_), .b(x09), .c(new_n68_), .d(x07), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n65_), .O(z05));
  inv1   g37(.a(x10), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n66_), .c(x08), .d(x06), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n41_), .c(x07), .O(new_n75_));
  nand3  g41(.a(new_n57_), .b(new_n66_), .c(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n65_), .O(z06));
  inv1   g44(.a(x11), .O(new_n79_));
  inv1   g45(.a(new_n69_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n72_), .d(new_n66_), .O(new_n81_));
  oai21  g47(.a(new_n74_), .b(new_n79_), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x05), .O(new_n83_));
  nand2  g49(.a(x11), .b(new_n35_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n65_), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n79_), .c(new_n72_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n67_), .c(x05), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x07), .O(new_n89_));
  nand2  g55(.a(new_n81_), .b(x12), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n65_), .O(z08));
  inv1   g57(.a(x04), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n48_), .b(new_n93_), .c(new_n86_), .d(new_n79_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n66_), .c(x08), .d(x06), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n92_), .c(x05), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x07), .O(new_n98_));
  nand4  g64(.a(new_n86_), .b(new_n79_), .c(new_n72_), .d(new_n66_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n69_), .c(x13), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n98_), .c(new_n65_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  inv1   g68(.a(x03), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(x02), .c(new_n62_), .O(new_n104_));
  nand4  g70(.a(new_n93_), .b(new_n86_), .c(new_n79_), .d(new_n72_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n102_), .c(new_n104_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n66_), .c(x08), .d(x07), .O(new_n107_));
  nand2  g73(.a(new_n104_), .b(x14), .O(new_n108_));
  oai21  g74(.a(new_n107_), .b(new_n46_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x05), .O(new_n110_));
  nand3  g76(.a(new_n104_), .b(x14), .c(new_n35_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n92_), .O(z10));
  nand2  g78(.a(new_n55_), .b(new_n37_), .O(z11));
  nor2   g79(.a(new_n36_), .b(new_n62_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n37_), .c(x03), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n92_), .c(new_n55_), .O(z12));
  nand2  g82(.a(new_n55_), .b(new_n92_), .O(z13));
  inv1   g83(.a(x17), .O(new_n118_));
  nor3   g84(.a(new_n36_), .b(new_n118_), .c(new_n92_), .O(z14));
endmodule


