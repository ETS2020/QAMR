// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n115_;
  inv1   g00(.a(x05), .O(new_n35_));
  nor2   g01(.a(x16), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n35_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand3  g10(.a(x16), .b(x05), .c(new_n44_), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n43_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n39_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x03), .O(new_n52_));
  nand3  g18(.a(x16), .b(new_n52_), .c(x02), .O(new_n53_));
  inv1   g19(.a(x16), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n35_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n51_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n48_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n37_), .O(z04));
  oai21  g30(.a(x16), .b(new_n35_), .c(new_n44_), .O(new_n65_));
  oai21  g31(.a(new_n44_), .b(x02), .c(new_n52_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x16), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n60_), .b(new_n70_), .c(x08), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n36_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(z05));
  inv1   g40(.a(x10), .O(new_n75_));
  nand4  g41(.a(new_n60_), .b(new_n75_), .c(new_n70_), .d(x08), .O(new_n76_));
  oai21  g42(.a(new_n72_), .b(new_n75_), .c(new_n76_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n37_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n67_), .c(new_n65_), .O(z06));
  nand2  g45(.a(new_n76_), .b(x11), .O(new_n80_));
  nor4   g46(.a(new_n68_), .b(x11), .c(x10), .d(x09), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(new_n36_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n80_), .c(new_n67_), .d(new_n65_), .O(z07));
  inv1   g49(.a(new_n81_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x12), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n86_), .c(new_n75_), .d(new_n70_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(new_n68_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n36_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n85_), .c(new_n67_), .d(new_n65_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n56_), .b(new_n92_), .c(new_n87_), .d(new_n86_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(x10), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n70_), .c(x08), .d(x07), .O(new_n95_));
  nand2  g61(.a(x16), .b(new_n39_), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n47_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  aoi22  g64(.a(new_n90_), .b(x13), .c(x16), .d(x03), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n98_), .c(new_n65_), .O(z09));
  aoi21  g66(.a(new_n55_), .b(new_n53_), .c(x13), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n87_), .c(new_n86_), .d(new_n75_), .O(new_n102_));
  nand2  g68(.a(new_n48_), .b(x00), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(x09), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(x08), .c(x07), .d(x06), .O(new_n105_));
  inv1   g71(.a(new_n48_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x14), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n37_), .O(z10));
  nand2  g76(.a(new_n37_), .b(new_n39_), .O(z11));
  aoi21  g77(.a(x16), .b(new_n39_), .c(x03), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(new_n44_), .c(new_n37_), .O(z12));
  nor2   g79(.a(new_n36_), .b(new_n44_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n37_), .O(z14));
endmodule


