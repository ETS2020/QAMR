// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n118_;
  inv1   g00(.a(x13), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n37_), .c(x16), .O(new_n47_));
  nand4  g13(.a(new_n47_), .b(new_n35_), .c(new_n46_), .d(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  inv1   g15(.a(new_n36_), .O(new_n50_));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n50_), .O(z03));
  inv1   g19(.a(x07), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n46_), .c(x08), .O(new_n55_));
  inv1   g21(.a(x08), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(x07), .c(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n47_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n50_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n37_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nand3  g33(.a(new_n65_), .b(x08), .c(x07), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nand3  g36(.a(new_n35_), .b(x09), .c(new_n46_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n64_), .O(z05));
  oai21  g38(.a(x13), .b(x10), .c(new_n46_), .O(new_n73_));
  nand2  g39(.a(new_n68_), .b(x10), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x08), .c(x07), .d(x06), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n74_), .c(new_n73_), .d(new_n64_), .O(z06));
  inv1   g43(.a(x11), .O(new_n78_));
  aoi21  g44(.a(new_n75_), .b(new_n67_), .c(new_n78_), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  nand2  g46(.a(new_n78_), .b(new_n80_), .O(new_n81_));
  nor3   g47(.a(new_n81_), .b(new_n66_), .c(x09), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n79_), .c(x06), .O(new_n83_));
  nand3  g49(.a(new_n35_), .b(x11), .c(new_n46_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n64_), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nand2  g52(.a(new_n35_), .b(new_n86_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n46_), .O(new_n88_));
  inv1   g54(.a(new_n82_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x12), .O(new_n90_));
  nor2   g56(.a(x12), .b(x11), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n75_), .c(new_n67_), .d(x06), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n90_), .c(new_n88_), .d(new_n64_), .O(z08));
  nand4  g59(.a(new_n47_), .b(new_n35_), .c(new_n86_), .d(new_n78_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n80_), .c(new_n65_), .d(x08), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x07), .c(x06), .d(x04), .O(new_n98_));
  nand2  g64(.a(new_n92_), .b(x13), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n98_), .c(new_n64_), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  oai21  g67(.a(new_n87_), .b(new_n81_), .c(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n47_), .c(new_n65_), .d(x08), .O(new_n103_));
  inv1   g69(.a(new_n47_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x14), .O(new_n105_));
  oai21  g71(.a(new_n103_), .b(new_n54_), .c(new_n105_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x06), .O(new_n107_));
  nand3  g73(.a(new_n104_), .b(x14), .c(new_n35_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n43_), .O(z10));
  nor2   g75(.a(new_n36_), .b(new_n37_), .O(z11));
  aoi21  g76(.a(x16), .b(new_n37_), .c(x03), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n50_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  oai21  g80(.a(new_n104_), .b(x06), .c(new_n111_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n35_), .O(new_n116_));
  aoi21  g82(.a(new_n116_), .b(new_n46_), .c(new_n43_), .O(z13));
  inv1   g83(.a(x17), .O(new_n118_));
  nor3   g84(.a(new_n36_), .b(new_n118_), .c(new_n43_), .O(z14));
endmodule


