// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n107_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x08), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(new_n36_), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  inv1   g14(.a(new_n37_), .O(new_n49_));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(z03));
  xor2a  g18(.a(x08), .b(x07), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n46_), .c(x06), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z04));
  inv1   g21(.a(x16), .O(new_n56_));
  aoi21  g22(.a(x04), .b(new_n35_), .c(x03), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n56_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n49_), .O(new_n59_));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x08), .c(x07), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(x07), .c(new_n61_), .O(new_n62_));
  aoi22  g28(.a(new_n62_), .b(x06), .c(x09), .d(new_n36_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n59_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n60_), .c(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  aoi22  g33(.a(new_n67_), .b(x08), .c(new_n61_), .d(x10), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n59_), .O(z06));
  nand2  g35(.a(new_n66_), .b(x11), .O(new_n70_));
  nor2   g36(.a(x11), .b(x10), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n60_), .c(x08), .d(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(x11), .b(new_n36_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n59_), .O(z07));
  inv1   g42(.a(x12), .O(new_n77_));
  inv1   g43(.a(x07), .O(new_n78_));
  nor2   g44(.a(x09), .b(new_n78_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n71_), .c(new_n77_), .O(new_n80_));
  nor4   g46(.a(new_n61_), .b(x12), .c(x11), .d(x10), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n80_), .c(x06), .O(new_n82_));
  nand2  g48(.a(x12), .b(new_n36_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n59_), .O(z08));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n46_), .b(new_n86_), .c(new_n77_), .d(new_n85_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n65_), .c(new_n60_), .d(x07), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n42_), .c(x06), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x08), .O(new_n91_));
  or2    g57(.a(new_n81_), .b(new_n86_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n59_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand3  g60(.a(new_n71_), .b(new_n86_), .c(new_n77_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n46_), .c(new_n60_), .d(x08), .O(new_n97_));
  inv1   g63(.a(new_n46_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x14), .O(new_n99_));
  oai21  g65(.a(new_n97_), .b(new_n78_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x06), .O(new_n101_));
  nand3  g67(.a(new_n98_), .b(x14), .c(new_n36_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n42_), .O(z10));
  nor2   g69(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g70(.a(x03), .O(new_n105_));
  oai21  g71(.a(new_n56_), .b(x02), .c(new_n105_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n49_), .c(x04), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(z12));
  nand2  g74(.a(new_n49_), .b(new_n42_), .O(z13));
  inv1   g75(.a(x17), .O(new_n110_));
  nor3   g76(.a(new_n37_), .b(new_n110_), .c(new_n42_), .O(z14));
endmodule


