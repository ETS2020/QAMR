// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n122_, new_n125_;
  inv1   g00(.a(x08), .O(new_n35_));
  nand2  g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n40_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n36_), .c(new_n49_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n36_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n49_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n49_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n50_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n36_), .O(z04));
  oai21  g26(.a(x17), .b(x09), .c(new_n35_), .O(new_n61_));
  nand2  g27(.a(x07), .b(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x03), .O(new_n64_));
  oai21  g30(.a(new_n46_), .b(x02), .c(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x16), .O(new_n66_));
  inv1   g32(.a(new_n62_), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n35_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n67_), .c(new_n46_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n66_), .c(new_n63_), .d(new_n61_), .O(z05));
  oai21  g36(.a(x17), .b(x10), .c(new_n35_), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x07), .c(x06), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x08), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n67_), .c(new_n46_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n74_), .c(new_n71_), .d(new_n66_), .O(z06));
  oai21  g45(.a(x17), .b(x11), .c(new_n35_), .O(new_n80_));
  nand2  g46(.a(new_n75_), .b(new_n67_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x11), .O(new_n82_));
  nand3  g48(.a(x08), .b(x07), .c(x06), .O(new_n83_));
  inv1   g49(.a(new_n83_), .O(new_n84_));
  nor3   g50(.a(x11), .b(x10), .c(x09), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(new_n84_), .c(new_n46_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n82_), .c(new_n80_), .d(new_n66_), .O(z07));
  oai21  g53(.a(x17), .b(x12), .c(new_n35_), .O(new_n88_));
  nand2  g54(.a(new_n85_), .b(new_n67_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x12), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand3  g58(.a(new_n75_), .b(new_n92_), .c(new_n91_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n84_), .c(new_n46_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n90_), .c(new_n88_), .d(new_n66_), .O(z08));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n50_), .b(new_n97_), .c(new_n92_), .d(new_n91_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(x10), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n72_), .c(x08), .d(x07), .O(new_n100_));
  nand2  g66(.a(x16), .b(new_n40_), .O(new_n101_));
  oai21  g67(.a(new_n100_), .b(new_n49_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x04), .O(new_n103_));
  oai21  g69(.a(x17), .b(x13), .c(new_n35_), .O(new_n104_));
  inv1   g70(.a(x10), .O(new_n105_));
  nand3  g71(.a(new_n92_), .b(new_n91_), .c(new_n105_), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n73_), .c(x13), .O(new_n107_));
  aoi21  g73(.a(x16), .b(x03), .c(new_n46_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(new_n107_), .c(new_n104_), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n103_), .O(z09));
  inv1   g77(.a(x00), .O(new_n112_));
  nand4  g78(.a(new_n97_), .b(new_n92_), .c(new_n91_), .d(new_n105_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g80(.a(new_n114_), .b(new_n50_), .c(new_n72_), .d(x07), .O(new_n115_));
  oai22  g81(.a(new_n115_), .b(new_n49_), .c(new_n50_), .d(new_n38_), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(x08), .O(new_n117_));
  aoi21  g83(.a(new_n64_), .b(x02), .c(x17), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(x16), .c(x14), .O(new_n119_));
  aoi21  g85(.a(new_n119_), .b(new_n117_), .c(new_n46_), .O(z10));
  nand2  g86(.a(new_n36_), .b(new_n40_), .O(z11));
  aoi21  g87(.a(x16), .b(new_n40_), .c(x03), .O(new_n122_));
  oai21  g88(.a(new_n122_), .b(new_n46_), .c(new_n36_), .O(z12));
  aoi21  g89(.a(x17), .b(new_n35_), .c(new_n46_), .O(z13));
  nand3  g90(.a(x17), .b(x08), .c(x04), .O(new_n125_));
  inv1   g91(.a(new_n125_), .O(z14));
endmodule


