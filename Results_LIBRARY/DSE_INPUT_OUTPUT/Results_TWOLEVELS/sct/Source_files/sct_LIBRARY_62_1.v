// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n115_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  oai21  g06(.a(x06), .b(x05), .c(new_n36_), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n38_), .c(x16), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n45_), .c(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z03));
  inv1   g17(.a(new_n46_), .O(new_n52_));
  inv1   g18(.a(x08), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n45_), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand3  g22(.a(new_n53_), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  aoi21  g24(.a(new_n56_), .b(x04), .c(new_n58_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n52_), .c(new_n37_), .O(z04));
  oai21  g26(.a(x09), .b(new_n36_), .c(new_n45_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nor2   g28(.a(new_n53_), .b(new_n54_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g30(.a(new_n55_), .b(new_n62_), .c(x08), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n46_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n64_), .c(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n61_), .O(z05));
  oai21  g34(.a(x10), .b(new_n36_), .c(new_n45_), .O(new_n69_));
  nand3  g35(.a(new_n62_), .b(x08), .c(x07), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x10), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n55_), .c(x08), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n46_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n69_), .O(z06));
  oai21  g42(.a(x11), .b(new_n36_), .c(new_n45_), .O(new_n77_));
  nand2  g43(.a(new_n72_), .b(new_n63_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x11), .O(new_n79_));
  nand2  g45(.a(new_n63_), .b(x06), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  nor3   g47(.a(x11), .b(x10), .c(x09), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n79_), .c(new_n46_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n77_), .O(z07));
  oai21  g52(.a(x12), .b(new_n36_), .c(new_n45_), .O(new_n87_));
  nand2  g53(.a(new_n82_), .b(new_n63_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x12), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand4  g57(.a(new_n81_), .b(new_n72_), .c(new_n91_), .d(new_n90_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n89_), .c(new_n46_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n87_), .O(z08));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n46_), .b(new_n96_), .c(new_n91_), .d(new_n90_), .O(new_n97_));
  nor3   g63(.a(new_n97_), .b(x10), .c(x09), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(x08), .c(x07), .d(x06), .O(new_n99_));
  nand2  g65(.a(new_n92_), .b(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n46_), .d(x04), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  inv1   g68(.a(x10), .O(new_n103_));
  nand4  g69(.a(new_n96_), .b(new_n91_), .c(new_n90_), .d(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n46_), .c(new_n62_), .d(x08), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n54_), .c(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x06), .O(new_n108_));
  nand3  g74(.a(new_n52_), .b(x14), .c(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n108_), .O(z10));
  nand2  g76(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g77(.a(x16), .b(new_n38_), .c(x03), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(new_n36_), .c(new_n37_), .O(z12));
  nand2  g79(.a(new_n45_), .b(new_n36_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n37_), .O(z14));
endmodule


