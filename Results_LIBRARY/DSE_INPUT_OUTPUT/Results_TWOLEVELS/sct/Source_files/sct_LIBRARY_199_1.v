// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n111_, new_n114_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand3  g01(.a(new_n35_), .b(x01), .c(x00), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x01), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x00), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n40_), .c(new_n42_), .O(new_n43_));
  oai21  g09(.a(new_n39_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(new_n42_), .O(new_n45_));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n45_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n45_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  and2   g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n53_), .b(x07), .c(x06), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n47_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n45_), .O(z04));
  inv1   g24(.a(x03), .O(new_n59_));
  oai21  g25(.a(new_n40_), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x16), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n63_), .c(x04), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n62_), .c(new_n45_), .O(new_n67_));
  nand3  g33(.a(new_n47_), .b(x04), .c(x00), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x01), .c(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n67_), .O(z05));
  nand3  g37(.a(new_n54_), .b(new_n63_), .c(x08), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n54_), .c(x08), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x04), .O(new_n75_));
  aoi21  g41(.a(new_n72_), .b(x10), .c(new_n75_), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n61_), .c(new_n42_), .O(z06));
  nor2   g43(.a(new_n62_), .b(new_n40_), .O(new_n78_));
  nand2  g44(.a(new_n74_), .b(x11), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand4  g47(.a(new_n65_), .b(new_n81_), .c(new_n80_), .d(new_n63_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n79_), .c(new_n78_), .d(new_n45_), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nand4  g50(.a(new_n73_), .b(new_n65_), .c(new_n84_), .d(new_n81_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(new_n86_));
  aoi21  g52(.a(new_n82_), .b(x12), .c(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n61_), .c(new_n42_), .O(z08));
  nand2  g54(.a(new_n85_), .b(x13), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n61_), .c(x04), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n45_), .O(new_n91_));
  inv1   g57(.a(x00), .O(new_n92_));
  nand3  g58(.a(new_n47_), .b(x04), .c(new_n41_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n92_), .c(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n84_), .c(new_n81_), .d(new_n80_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(x09), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n91_), .O(z09));
  nor2   g64(.a(x13), .b(x12), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n81_), .c(new_n80_), .d(new_n41_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n92_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n47_), .c(new_n63_), .d(x08), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(x07), .c(x06), .O(new_n104_));
  nand2  g70(.a(new_n59_), .b(x02), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n45_), .c(x16), .d(x14), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n104_), .c(new_n40_), .O(z10));
  nor2   g73(.a(new_n42_), .b(new_n35_), .O(z11));
  inv1   g74(.a(x16), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(x02), .c(new_n59_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(new_n45_), .c(x04), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(z12));
  nand2  g78(.a(new_n45_), .b(new_n40_), .O(z13));
  nand2  g79(.a(x17), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n45_), .O(z14));
endmodule


