// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n119_;
  inv1   g00(.a(x11), .O(new_n35_));
  nor2   g01(.a(x15), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  aoi21  g08(.a(new_n35_), .b(x05), .c(x15), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n35_), .c(x15), .O(new_n45_));
  oai22  g11(.a(new_n45_), .b(x05), .c(new_n43_), .d(x04), .O(z01));
  inv1   g12(.a(new_n36_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n37_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n47_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n47_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n55_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n49_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n47_), .O(z04));
  inv1   g27(.a(x03), .O(new_n62_));
  inv1   g28(.a(x04), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x02), .c(new_n62_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x16), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n47_), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n57_), .b(new_n70_), .c(x08), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n69_), .c(new_n67_), .d(new_n47_), .O(z05));
  nand2  g38(.a(new_n71_), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n65_), .c(x04), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  inv1   g42(.a(x15), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n63_), .c(x11), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n76_), .c(new_n70_), .d(x08), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(x07), .c(x06), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n75_), .O(z06));
  nand4  g48(.a(new_n57_), .b(new_n76_), .c(new_n70_), .d(x08), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(x15), .c(x11), .d(x04), .O(new_n84_));
  inv1   g50(.a(new_n68_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n35_), .c(new_n76_), .d(new_n70_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n67_), .O(z07));
  oai21  g53(.a(new_n77_), .b(x12), .c(x11), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n35_), .c(new_n76_), .d(new_n70_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n68_), .O(new_n91_));
  aoi21  g57(.a(new_n83_), .b(x12), .c(new_n91_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n88_), .c(new_n67_), .O(z08));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n49_), .b(new_n94_), .c(new_n89_), .d(new_n76_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(x09), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  nand3  g63(.a(new_n89_), .b(new_n76_), .c(new_n70_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n68_), .c(x13), .O(new_n99_));
  oai21  g65(.a(new_n97_), .b(new_n63_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n35_), .O(new_n101_));
  nand3  g67(.a(x15), .b(x13), .c(x11), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(new_n67_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(x16), .O(new_n105_));
  aoi21  g71(.a(new_n62_), .b(x02), .c(new_n105_), .O(new_n106_));
  nand4  g72(.a(new_n94_), .b(new_n89_), .c(new_n35_), .d(new_n76_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n104_), .c(new_n106_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n70_), .c(x08), .d(x07), .O(new_n109_));
  oai22  g75(.a(new_n109_), .b(new_n48_), .c(new_n49_), .d(new_n40_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n47_), .O(z10));
  nor2   g78(.a(new_n36_), .b(new_n37_), .O(z11));
  oai21  g79(.a(new_n105_), .b(x02), .c(new_n62_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n47_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  aoi21  g82(.a(new_n77_), .b(x11), .c(new_n63_), .O(z13));
  nand2  g83(.a(x17), .b(x04), .O(new_n119_));
  nand2  g84(.a(new_n119_), .b(new_n47_), .O(z14));
endmodule


