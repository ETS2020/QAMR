// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x08), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  nand2  g09(.a(x08), .b(x01), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(x08), .b(x01), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n44_), .O(z03));
  inv1   g19(.a(x01), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(x08), .c(new_n54_), .O(new_n56_));
  nand3  g22(.a(new_n36_), .b(x07), .c(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n47_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n35_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n44_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n54_), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  aoi22  g34(.a(new_n68_), .b(x09), .c(new_n67_), .d(x08), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n64_), .O(z05));
  nand2  g36(.a(new_n66_), .b(x10), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(x08), .c(x07), .d(x06), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n54_), .O(new_n75_));
  nand2  g41(.a(x10), .b(new_n36_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n64_), .O(z06));
  nor4   g43(.a(new_n55_), .b(x11), .c(x10), .d(x09), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(x01), .c(x08), .O(new_n79_));
  nand2  g45(.a(new_n73_), .b(x11), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n64_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand3  g49(.a(new_n72_), .b(new_n82_), .c(new_n83_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n68_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  oai21  g52(.a(new_n78_), .b(new_n82_), .c(new_n86_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n54_), .O(new_n88_));
  nand2  g54(.a(x12), .b(new_n36_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n64_), .O(z08));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n47_), .b(new_n91_), .c(new_n82_), .d(new_n83_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x10), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n65_), .c(x07), .d(x06), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n42_), .c(new_n54_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x08), .O(new_n96_));
  nand2  g62(.a(new_n86_), .b(x13), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n96_), .c(new_n64_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  inv1   g65(.a(x03), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x02), .c(new_n61_), .O(new_n101_));
  inv1   g67(.a(x10), .O(new_n102_));
  nand4  g68(.a(new_n91_), .b(new_n82_), .c(new_n83_), .d(new_n102_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n99_), .c(new_n101_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n65_), .c(x08), .d(x07), .O(new_n105_));
  nand2  g71(.a(new_n101_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n46_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n54_), .O(new_n108_));
  nand3  g74(.a(new_n101_), .b(x14), .c(new_n36_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n42_), .O(z10));
  nand2  g76(.a(new_n44_), .b(new_n35_), .O(z11));
  oai21  g77(.a(new_n61_), .b(x02), .c(new_n100_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n44_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  nand2  g80(.a(new_n44_), .b(new_n42_), .O(z13));
  nand3  g81(.a(new_n44_), .b(x17), .c(x04), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(z14));
endmodule


