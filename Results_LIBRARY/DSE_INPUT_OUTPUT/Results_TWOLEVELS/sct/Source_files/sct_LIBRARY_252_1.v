// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n119_, new_n120_,
    new_n123_;
  inv1   g00(.a(x11), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n37_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n35_), .c(new_n49_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand3  g18(.a(new_n35_), .b(x07), .c(new_n49_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(new_n35_), .b(x08), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n49_), .O(new_n60_));
  oai21  g26(.a(new_n57_), .b(new_n49_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n50_), .c(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  inv1   g29(.a(new_n36_), .O(new_n64_));
  inv1   g30(.a(x16), .O(new_n65_));
  aoi21  g31(.a(x04), .b(new_n37_), .c(x03), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n65_), .c(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  oai21  g34(.a(x11), .b(x09), .c(new_n49_), .O(new_n69_));
  nand2  g35(.a(x08), .b(x07), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x09), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nand2  g38(.a(x07), .b(x06), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(x08), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n71_), .c(new_n69_), .d(new_n68_), .O(z05));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  inv1   g43(.a(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n49_), .O(new_n79_));
  aoi21  g45(.a(new_n35_), .b(new_n49_), .c(x10), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n72_), .c(x08), .d(x07), .O(new_n81_));
  nand3  g47(.a(new_n72_), .b(x08), .c(x07), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x10), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n81_), .c(new_n79_), .d(new_n68_), .O(z06));
  nor2   g50(.a(x10), .b(x09), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n74_), .c(x08), .O(new_n86_));
  nand3  g52(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nor2   g54(.a(new_n78_), .b(x09), .O(new_n89_));
  aoi22  g55(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(x11), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n68_), .O(z07));
  nor2   g57(.a(new_n59_), .b(new_n49_), .O(new_n92_));
  aoi21  g58(.a(new_n85_), .b(new_n74_), .c(x11), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n92_), .c(x12), .O(new_n94_));
  inv1   g60(.a(x12), .O(new_n95_));
  nand4  g61(.a(new_n88_), .b(new_n85_), .c(new_n95_), .d(new_n35_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n94_), .c(new_n68_), .O(z08));
  inv1   g63(.a(x10), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n50_), .b(new_n99_), .c(new_n95_), .d(new_n98_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(x09), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(x08), .c(x07), .d(x06), .O(new_n102_));
  nand3  g68(.a(new_n95_), .b(new_n98_), .c(new_n72_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n73_), .c(x13), .O(new_n104_));
  oai21  g70(.a(new_n102_), .b(new_n46_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n35_), .O(new_n106_));
  nand3  g72(.a(new_n58_), .b(x13), .c(x06), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n106_), .c(new_n68_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  inv1   g75(.a(x03), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(x02), .c(new_n65_), .O(new_n111_));
  nand3  g77(.a(new_n77_), .b(new_n99_), .c(new_n95_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n109_), .c(new_n111_), .O(new_n113_));
  nand4  g79(.a(new_n113_), .b(new_n72_), .c(x08), .d(x07), .O(new_n114_));
  oai22  g80(.a(new_n114_), .b(new_n49_), .c(new_n50_), .d(new_n40_), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n64_), .O(z10));
  nand2  g83(.a(new_n64_), .b(new_n37_), .O(z11));
  oai21  g84(.a(new_n65_), .b(x02), .c(new_n110_), .O(new_n119_));
  nand3  g85(.a(new_n119_), .b(new_n64_), .c(x04), .O(new_n120_));
  inv1   g86(.a(new_n120_), .O(z12));
  nand2  g87(.a(new_n64_), .b(new_n46_), .O(z13));
  inv1   g88(.a(x17), .O(new_n123_));
  nor3   g89(.a(new_n36_), .b(new_n123_), .c(new_n46_), .O(z14));
endmodule


