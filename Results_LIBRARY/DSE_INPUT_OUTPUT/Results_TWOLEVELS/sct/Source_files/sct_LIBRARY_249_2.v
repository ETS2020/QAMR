// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n119_, new_n122_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(x16), .b(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n39_), .b(new_n35_), .c(new_n37_), .O(z00));
  nor2   g06(.a(x16), .b(new_n35_), .O(new_n41_));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n41_), .O(z01));
  inv1   g13(.a(new_n41_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  nor2   g15(.a(x03), .b(new_n38_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(x16), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n48_), .O(z03));
  inv1   g23(.a(x03), .O(new_n58_));
  nand3  g24(.a(x16), .b(new_n58_), .c(x02), .O(new_n59_));
  inv1   g25(.a(x16), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n35_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  inv1   g28(.a(x08), .O(new_n63_));
  and2   g29(.a(x07), .b(x06), .O(new_n64_));
  nand3  g30(.a(new_n63_), .b(x07), .c(x06), .O(new_n65_));
  oai21  g31(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n62_), .c(x04), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(z04));
  oai21  g34(.a(new_n51_), .b(new_n45_), .c(x16), .O(new_n69_));
  nand3  g35(.a(x08), .b(x07), .c(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n45_), .b(new_n35_), .O(new_n72_));
  nor2   g38(.a(x09), .b(new_n63_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n64_), .c(new_n41_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n72_), .c(new_n71_), .d(new_n69_), .O(z05));
  nand2  g41(.a(new_n73_), .b(new_n64_), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n64_), .c(x08), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n72_), .c(new_n48_), .O(new_n79_));
  aoi21  g45(.a(new_n76_), .b(x10), .c(new_n79_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n69_), .O(z06));
  nor2   g47(.a(new_n50_), .b(new_n45_), .O(new_n82_));
  nand2  g48(.a(new_n78_), .b(x11), .O(new_n83_));
  inv1   g49(.a(x09), .O(new_n84_));
  inv1   g50(.a(new_n70_), .O(new_n85_));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n83_), .c(x04), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n82_), .c(x16), .O(new_n89_));
  nand2  g55(.a(new_n88_), .b(new_n35_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n89_), .O(z07));
  inv1   g57(.a(x11), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand4  g59(.a(new_n77_), .b(new_n85_), .c(new_n93_), .d(new_n92_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n72_), .c(new_n48_), .O(new_n95_));
  aoi21  g61(.a(new_n87_), .b(x12), .c(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n69_), .O(z08));
  nand4  g63(.a(new_n73_), .b(x07), .c(x06), .d(x04), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n86_), .b(new_n60_), .c(new_n99_), .d(new_n93_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n98_), .c(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n35_), .O(new_n102_));
  nor3   g68(.a(x13), .b(x12), .c(x11), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n77_), .c(new_n85_), .d(new_n50_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(x04), .c(new_n58_), .d(x02), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x16), .O(new_n106_));
  aoi21  g72(.a(new_n94_), .b(x13), .c(new_n41_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n106_), .c(new_n102_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  inv1   g75(.a(x10), .O(new_n110_));
  nand4  g76(.a(new_n99_), .b(new_n93_), .c(new_n92_), .d(new_n110_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand4  g78(.a(new_n112_), .b(new_n62_), .c(new_n84_), .d(x08), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(x07), .c(x06), .O(new_n115_));
  nand3  g81(.a(new_n51_), .b(x16), .c(x14), .O(new_n116_));
  aoi21  g82(.a(new_n116_), .b(new_n115_), .c(new_n45_), .O(z10));
  nor2   g83(.a(new_n41_), .b(new_n38_), .O(z11));
  nand2  g84(.a(new_n48_), .b(x03), .O(new_n119_));
  aoi21  g85(.a(new_n119_), .b(new_n39_), .c(new_n45_), .O(z12));
  nor2   g86(.a(new_n41_), .b(new_n45_), .O(z13));
  inv1   g87(.a(x17), .O(new_n122_));
  nor3   g88(.a(new_n41_), .b(new_n122_), .c(new_n45_), .O(z14));
endmodule


