// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n119_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x09), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  oai21  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(x09), .b(x01), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n43_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n46_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n49_), .c(new_n46_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n55_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n49_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n46_), .O(z04));
  inv1   g27(.a(x16), .O(new_n62_));
  aoi21  g28(.a(x04), .b(new_n48_), .c(x03), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  nand4  g31(.a(new_n49_), .b(x07), .c(new_n47_), .d(x04), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  nor2   g34(.a(x09), .b(new_n55_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x07), .c(x06), .d(x01), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n65_), .O(z05));
  inv1   g37(.a(x10), .O(new_n72_));
  nand3  g38(.a(new_n57_), .b(new_n72_), .c(x08), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n35_), .c(new_n36_), .O(new_n75_));
  nand2  g41(.a(new_n69_), .b(new_n57_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n65_), .O(z06));
  inv1   g44(.a(x11), .O(new_n79_));
  nand3  g45(.a(x08), .b(x07), .c(x06), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n36_), .O(new_n83_));
  oai21  g49(.a(new_n74_), .b(new_n79_), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x01), .O(new_n85_));
  nand2  g51(.a(x11), .b(x09), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n65_), .O(z07));
  nand3  g53(.a(new_n82_), .b(new_n57_), .c(x08), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x12), .O(new_n89_));
  nor2   g55(.a(x12), .b(x11), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n81_), .c(new_n72_), .d(new_n36_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x01), .O(new_n93_));
  nand2  g59(.a(x12), .b(x09), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n65_), .O(z08));
  inv1   g61(.a(x12), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n49_), .b(new_n97_), .c(new_n96_), .d(new_n79_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(x10), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x08), .c(x07), .d(x06), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n42_), .c(x01), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n36_), .O(new_n102_));
  nand2  g68(.a(new_n91_), .b(x13), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n102_), .c(new_n65_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  inv1   g71(.a(x03), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(x02), .c(new_n62_), .O(new_n107_));
  nand3  g73(.a(new_n82_), .b(new_n97_), .c(new_n96_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n105_), .c(new_n107_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n36_), .c(x08), .d(x07), .O(new_n110_));
  oai22  g76(.a(new_n110_), .b(new_n47_), .c(new_n49_), .d(new_n37_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n46_), .O(z10));
  nand2  g79(.a(new_n46_), .b(new_n48_), .O(z11));
  oai21  g80(.a(new_n62_), .b(x02), .c(new_n106_), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(new_n46_), .c(x04), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(z12));
  nor2   g83(.a(new_n43_), .b(new_n42_), .O(z13));
  inv1   g84(.a(x17), .O(new_n119_));
  nor3   g85(.a(new_n43_), .b(new_n119_), .c(new_n42_), .O(z14));
endmodule


