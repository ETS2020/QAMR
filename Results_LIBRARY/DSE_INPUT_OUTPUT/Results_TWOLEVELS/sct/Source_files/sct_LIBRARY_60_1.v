// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n107_, new_n109_;
  inv1   g00(.a(x14), .O(new_n35_));
  nor2   g01(.a(x16), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  inv1   g08(.a(x16), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x01), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n42_), .c(new_n44_), .O(new_n45_));
  oai21  g11(.a(new_n41_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(x02), .c(x01), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(new_n44_), .O(new_n56_));
  oai21  g22(.a(x03), .b(new_n37_), .c(x16), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n57_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n56_), .O(z04));
  aoi21  g29(.a(new_n48_), .b(x02), .c(new_n43_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n42_), .c(x01), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n59_), .b(new_n68_), .c(x08), .O(new_n69_));
  aoi21  g35(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(z05));
  nand2  g37(.a(new_n69_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n59_), .c(x08), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n72_), .c(new_n70_), .d(new_n65_), .O(z06));
  nand2  g41(.a(new_n74_), .b(x11), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  inv1   g44(.a(new_n66_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n68_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n76_), .c(new_n70_), .d(new_n65_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n73_), .b(new_n79_), .c(new_n82_), .d(new_n78_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x04), .O(new_n84_));
  aoi21  g50(.a(new_n80_), .b(x12), .c(new_n84_), .O(new_n85_));
  oai21  g51(.a(new_n57_), .b(new_n42_), .c(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x01), .O(new_n87_));
  oai21  g53(.a(new_n85_), .b(x16), .c(new_n87_), .O(z08));
  nand2  g54(.a(new_n56_), .b(new_n42_), .O(z13));
  nand2  g55(.a(new_n64_), .b(x01), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n57_), .b(new_n91_), .c(new_n82_), .d(new_n78_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x10), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n68_), .c(x08), .d(x07), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n47_), .c(new_n90_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  aoi21  g62(.a(new_n83_), .b(x13), .c(new_n44_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n96_), .c(z13), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nand4  g65(.a(new_n91_), .b(new_n82_), .c(new_n78_), .d(new_n77_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n99_), .c(new_n64_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n68_), .c(x08), .d(x07), .O(new_n102_));
  oai22  g68(.a(new_n102_), .b(new_n47_), .c(new_n57_), .d(new_n35_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n56_), .O(z10));
  nand2  g71(.a(new_n56_), .b(new_n37_), .O(z11));
  nand2  g72(.a(new_n43_), .b(x03), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n90_), .c(new_n42_), .O(z12));
  inv1   g74(.a(x17), .O(new_n109_));
  nor3   g75(.a(new_n44_), .b(new_n109_), .c(new_n42_), .O(z14));
endmodule


