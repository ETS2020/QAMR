// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n117_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  inv1   g02(.a(x17), .O(new_n37_));
  oai21  g03(.a(new_n37_), .b(new_n35_), .c(new_n36_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  nor2   g05(.a(x17), .b(x14), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(new_n40_), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(new_n52_), .c(new_n48_), .d(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(new_n52_), .c(new_n48_), .d(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n59_), .b(new_n66_), .c(x08), .O(new_n67_));
  inv1   g33(.a(x02), .O(new_n68_));
  nand3  g34(.a(x16), .b(x04), .c(new_n68_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n48_), .O(new_n71_));
  oai21  g37(.a(new_n36_), .b(new_n44_), .c(new_n37_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x16), .c(x03), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(z05));
  and2   g40(.a(new_n67_), .b(x10), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n66_), .O(new_n77_));
  inv1   g43(.a(new_n77_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n59_), .c(x08), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n69_), .c(x04), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n75_), .c(new_n48_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n73_), .O(z06));
  nand2  g48(.a(new_n69_), .b(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n48_), .O(new_n84_));
  nand2  g50(.a(new_n79_), .b(x11), .O(new_n85_));
  nor4   g51(.a(new_n64_), .b(x11), .c(x10), .d(x09), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(new_n40_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n85_), .c(new_n84_), .d(new_n73_), .O(z07));
  inv1   g54(.a(x12), .O(new_n89_));
  nor2   g55(.a(new_n86_), .b(new_n89_), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  inv1   g57(.a(new_n64_), .O(new_n92_));
  nand4  g58(.a(new_n78_), .b(new_n92_), .c(new_n89_), .d(new_n91_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n69_), .c(x04), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n90_), .c(new_n48_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n73_), .O(z08));
  and2   g62(.a(new_n93_), .b(x13), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n83_), .c(new_n48_), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  oai21  g65(.a(new_n51_), .b(new_n37_), .c(new_n36_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n89_), .d(new_n91_), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n76_), .c(new_n66_), .d(x08), .O(new_n103_));
  inv1   g69(.a(new_n103_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(x07), .c(x06), .d(x04), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n98_), .c(new_n73_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  nand4  g73(.a(new_n99_), .b(new_n89_), .c(new_n91_), .d(new_n76_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n100_), .c(new_n66_), .d(x08), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(x07), .c(x06), .O(new_n112_));
  nand2  g78(.a(new_n51_), .b(x14), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n112_), .c(new_n44_), .O(z10));
  nor2   g80(.a(new_n40_), .b(new_n68_), .O(z11));
  nor3   g81(.a(new_n40_), .b(new_n50_), .c(x02), .O(new_n116_));
  oai21  g82(.a(new_n116_), .b(x03), .c(x04), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n48_), .O(z12));
  nor2   g84(.a(new_n40_), .b(new_n44_), .O(z13));
  nor2   g85(.a(new_n37_), .b(new_n44_), .O(z14));
endmodule


