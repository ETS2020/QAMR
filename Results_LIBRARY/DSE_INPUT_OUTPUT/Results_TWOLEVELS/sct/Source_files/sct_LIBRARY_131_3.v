// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x00), .O(new_n35_));
  inv1   g01(.a(x17), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  inv1   g07(.a(x02), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x01), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n41_), .c(new_n38_), .O(z00));
  inv1   g10(.a(x18), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x15), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  aoi21  g13(.a(x05), .b(new_n47_), .c(new_n37_), .O(new_n48_));
  oai21  g14(.a(new_n46_), .b(x05), .c(new_n48_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x03), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(x02), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(x16), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n38_), .O(z02));
  xor2a  g21(.a(x07), .b(x06), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n53_), .c(new_n38_), .d(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n53_), .c(new_n38_), .d(x04), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  aoi21  g30(.a(new_n52_), .b(x16), .c(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand2  g32(.a(x16), .b(new_n42_), .O(new_n67_));
  oai21  g33(.a(new_n66_), .b(new_n35_), .c(new_n67_), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nand2  g36(.a(x16), .b(x03), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(x04), .O(new_n72_));
  aoi21  g38(.a(new_n68_), .b(x04), .c(new_n72_), .O(new_n73_));
  inv1   g39(.a(x16), .O(new_n74_));
  nand2  g40(.a(x04), .b(new_n42_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n51_), .c(new_n74_), .O(new_n76_));
  inv1   g42(.a(x09), .O(new_n77_));
  nand3  g43(.a(new_n60_), .b(new_n77_), .c(x08), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n70_), .c(x04), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n76_), .c(new_n35_), .O(new_n80_));
  oai21  g46(.a(new_n73_), .b(x17), .c(new_n80_), .O(z05));
  nor2   g47(.a(new_n76_), .b(new_n47_), .O(new_n82_));
  nand2  g48(.a(new_n78_), .b(x10), .O(new_n83_));
  nor2   g49(.a(x10), .b(x09), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n60_), .c(x08), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n38_), .O(z06));
  inv1   g52(.a(new_n76_), .O(new_n87_));
  inv1   g53(.a(x10), .O(new_n88_));
  inv1   g54(.a(x11), .O(new_n89_));
  inv1   g55(.a(new_n69_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(new_n77_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x04), .O(new_n92_));
  aoi21  g58(.a(new_n85_), .b(x11), .c(new_n92_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n87_), .c(new_n37_), .O(z07));
  nand2  g60(.a(new_n91_), .b(x12), .O(new_n95_));
  inv1   g61(.a(x12), .O(new_n96_));
  nand4  g62(.a(new_n84_), .b(new_n90_), .c(new_n96_), .d(new_n89_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n95_), .c(new_n82_), .d(new_n38_), .O(z08));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n96_), .c(new_n89_), .d(new_n88_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n78_), .c(x04), .O(new_n101_));
  aoi21  g67(.a(new_n97_), .b(x13), .c(new_n101_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n87_), .c(new_n37_), .O(z09));
  nand2  g69(.a(new_n36_), .b(x00), .O(new_n104_));
  aoi22  g70(.a(new_n104_), .b(new_n100_), .c(new_n52_), .d(x16), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n77_), .c(x08), .d(x07), .O(new_n106_));
  oai22  g72(.a(new_n106_), .b(new_n50_), .c(new_n53_), .d(new_n40_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n38_), .O(z10));
  nand2  g75(.a(new_n38_), .b(new_n42_), .O(z11));
  nand2  g76(.a(new_n67_), .b(new_n51_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n38_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  aoi21  g79(.a(x17), .b(x00), .c(new_n47_), .O(z13));
  nand3  g80(.a(x17), .b(x04), .c(new_n35_), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z14));
endmodule


