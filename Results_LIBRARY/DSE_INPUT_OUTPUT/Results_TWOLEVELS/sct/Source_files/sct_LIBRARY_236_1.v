// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_;
  inv1   g00(.a(x08), .O(new_n35_));
  nand2  g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n40_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n49_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n36_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n36_), .O(z03));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n35_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n35_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n36_), .O(z04));
  inv1   g27(.a(x16), .O(new_n62_));
  aoi21  g28(.a(x04), .b(new_n40_), .c(x03), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n36_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x07), .c(x06), .O(new_n67_));
  oai21  g33(.a(new_n57_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x08), .O(new_n69_));
  inv1   g35(.a(x17), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x09), .c(new_n35_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n69_), .c(new_n65_), .O(z05));
  inv1   g38(.a(x10), .O(new_n73_));
  inv1   g39(.a(new_n67_), .O(new_n74_));
  nand3  g40(.a(new_n57_), .b(new_n73_), .c(new_n66_), .O(new_n75_));
  oai21  g41(.a(new_n74_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x08), .O(new_n77_));
  nand3  g43(.a(new_n70_), .b(x10), .c(new_n35_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n65_), .O(z06));
  and2   g45(.a(new_n75_), .b(x11), .O(new_n80_));
  nor4   g46(.a(new_n56_), .b(x11), .c(x10), .d(x09), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n80_), .c(x08), .O(new_n82_));
  nand3  g48(.a(new_n70_), .b(x11), .c(new_n35_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n65_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  nand4  g52(.a(new_n74_), .b(new_n85_), .c(new_n86_), .d(new_n73_), .O(new_n87_));
  oai21  g53(.a(new_n81_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x08), .O(new_n89_));
  nand3  g55(.a(new_n70_), .b(x12), .c(new_n35_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n65_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n50_), .b(new_n92_), .c(new_n85_), .d(new_n86_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n73_), .c(new_n66_), .d(x08), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x07), .c(x06), .d(x04), .O(new_n97_));
  aoi21  g63(.a(new_n70_), .b(new_n92_), .c(x08), .O(new_n98_));
  aoi21  g64(.a(new_n87_), .b(x13), .c(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n97_), .c(new_n65_), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  nand4  g67(.a(new_n92_), .b(new_n85_), .c(new_n86_), .d(new_n73_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n50_), .c(new_n66_), .d(x07), .O(new_n104_));
  oai22  g70(.a(new_n104_), .b(new_n49_), .c(new_n50_), .d(new_n38_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x08), .O(new_n106_));
  inv1   g72(.a(x03), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x02), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n70_), .c(x16), .d(x14), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n106_), .c(new_n46_), .O(z10));
  nand2  g76(.a(new_n36_), .b(new_n40_), .O(z11));
  oai21  g77(.a(new_n62_), .b(x02), .c(new_n107_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n36_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  aoi21  g80(.a(x17), .b(new_n35_), .c(new_n46_), .O(z13));
  aoi21  g81(.a(x08), .b(new_n46_), .c(new_n70_), .O(z14));
endmodule


