// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n111_, new_n114_;
  nand2  g00(.a(x17), .b(x10), .O(new_n35_));
  inv1   g01(.a(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n44_), .c(new_n35_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n39_), .c(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n35_), .c(new_n48_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n35_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n55_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n49_), .c(new_n35_), .d(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(x16), .O(new_n62_));
  aoi21  g28(.a(x04), .b(new_n39_), .c(x03), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n35_), .O(new_n65_));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n57_), .b(new_n68_), .c(x08), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(new_n35_), .O(z05));
  inv1   g36(.a(x17), .O(new_n71_));
  nand3  g37(.a(new_n69_), .b(new_n71_), .c(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand4  g39(.a(new_n57_), .b(new_n73_), .c(new_n68_), .d(x08), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(new_n65_), .O(z06));
  oai21  g41(.a(x17), .b(x11), .c(x10), .O(new_n76_));
  inv1   g42(.a(new_n66_), .O(new_n77_));
  nor3   g43(.a(x11), .b(x10), .c(x09), .O(new_n78_));
  aoi22  g44(.a(new_n78_), .b(new_n77_), .c(new_n69_), .d(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n76_), .c(new_n65_), .O(z07));
  inv1   g46(.a(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n68_), .c(x08), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n56_), .c(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand4  g50(.a(new_n77_), .b(new_n84_), .c(new_n81_), .d(new_n68_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n73_), .O(new_n87_));
  nand3  g53(.a(new_n71_), .b(x12), .c(x10), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n65_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n49_), .b(new_n90_), .c(new_n84_), .d(new_n81_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n73_), .c(new_n68_), .d(x08), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(x07), .c(x06), .d(x04), .O(new_n95_));
  aoi21  g61(.a(new_n71_), .b(new_n90_), .c(new_n73_), .O(new_n96_));
  aoi21  g62(.a(new_n85_), .b(x13), .c(new_n96_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n95_), .c(new_n65_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  inv1   g65(.a(x03), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x02), .c(new_n62_), .O(new_n101_));
  nand4  g67(.a(new_n90_), .b(new_n84_), .c(new_n81_), .d(new_n73_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n99_), .c(new_n101_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n68_), .c(x08), .d(x07), .O(new_n104_));
  oai22  g70(.a(new_n104_), .b(new_n48_), .c(new_n49_), .d(new_n37_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n35_), .O(z10));
  nand2  g73(.a(new_n35_), .b(new_n39_), .O(z11));
  nand3  g74(.a(new_n35_), .b(x16), .c(new_n39_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n100_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n35_), .O(z12));
  aoi21  g78(.a(x17), .b(x10), .c(new_n45_), .O(z13));
  nand3  g79(.a(x17), .b(new_n73_), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z14));
endmodule


