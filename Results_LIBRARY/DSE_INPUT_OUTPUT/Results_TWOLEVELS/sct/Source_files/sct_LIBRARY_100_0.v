// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n116_;
  nor2   g00(.a(x17), .b(x16), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  or2    g02(.a(x14), .b(x01), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n35_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand3  g14(.a(x16), .b(new_n48_), .c(x02), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  nand2  g16(.a(x17), .b(new_n50_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  aoi21  g23(.a(new_n48_), .b(x02), .c(new_n50_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n60_), .b(x07), .c(x06), .O(new_n62_));
  oai21  g28(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n59_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n36_), .O(z04));
  nand2  g31(.a(new_n36_), .b(new_n44_), .O(new_n66_));
  oai21  g32(.a(new_n44_), .b(x02), .c(new_n48_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x16), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nand3  g37(.a(new_n61_), .b(new_n71_), .c(x08), .O(new_n72_));
  and2   g38(.a(new_n72_), .b(new_n36_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n70_), .c(new_n68_), .d(new_n66_), .O(z05));
  inv1   g40(.a(x10), .O(new_n75_));
  nand4  g41(.a(new_n61_), .b(new_n75_), .c(new_n71_), .d(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  aoi21  g43(.a(new_n72_), .b(x10), .c(new_n77_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n35_), .c(new_n68_), .O(z06));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n71_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n69_), .c(x04), .O(new_n82_));
  aoi21  g48(.a(new_n76_), .b(x11), .c(new_n82_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n35_), .c(new_n68_), .O(z07));
  nor2   g50(.a(new_n80_), .b(x09), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n72_), .c(x12), .O(new_n86_));
  inv1   g52(.a(x11), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n87_), .c(new_n75_), .d(new_n71_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n69_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n35_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n86_), .c(new_n68_), .d(new_n66_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  oai21  g59(.a(new_n90_), .b(new_n93_), .c(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n36_), .O(new_n95_));
  nand4  g61(.a(new_n52_), .b(new_n93_), .c(new_n88_), .d(new_n87_), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(x10), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n71_), .c(x08), .d(x07), .O(new_n98_));
  nand2  g64(.a(x16), .b(new_n38_), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n47_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x04), .O(new_n101_));
  nand2  g67(.a(x16), .b(x03), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(new_n95_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand3  g70(.a(new_n80_), .b(new_n93_), .c(new_n88_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n52_), .c(new_n71_), .d(x08), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(x07), .c(x06), .O(new_n109_));
  nand2  g75(.a(new_n58_), .b(x14), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n44_), .O(z10));
  nor2   g77(.a(new_n35_), .b(new_n38_), .O(z11));
  aoi21  g78(.a(x16), .b(new_n38_), .c(x03), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(new_n44_), .c(new_n36_), .O(z12));
  nor2   g80(.a(new_n35_), .b(new_n44_), .O(z13));
  nand2  g81(.a(x17), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n36_), .O(z14));
endmodule


