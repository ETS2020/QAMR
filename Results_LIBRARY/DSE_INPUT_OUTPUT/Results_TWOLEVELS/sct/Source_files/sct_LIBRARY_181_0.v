// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n109_,
    new_n110_, new_n112_;
  inv1   g00(.a(x14), .O(new_n35_));
  nand2  g01(.a(x16), .b(x01), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n37_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nor2   g09(.a(x16), .b(x14), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n43_), .c(new_n44_), .O(new_n45_));
  oai21  g11(.a(new_n42_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand3  g14(.a(x16), .b(new_n48_), .c(x02), .O(new_n49_));
  oai21  g15(.a(x16), .b(new_n35_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand2  g27(.a(new_n48_), .b(x02), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n43_), .c(x16), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n57_), .b(new_n66_), .c(x08), .O(new_n67_));
  aoi21  g33(.a(x14), .b(new_n43_), .c(new_n44_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n67_), .c(new_n65_), .d(new_n63_), .O(z05));
  nand2  g35(.a(new_n67_), .b(x10), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n57_), .c(x08), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n70_), .c(new_n68_), .d(new_n63_), .O(z06));
  nand2  g39(.a(new_n72_), .b(x11), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  inv1   g42(.a(new_n64_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n66_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n74_), .c(new_n68_), .d(new_n63_), .O(z07));
  aoi21  g45(.a(new_n48_), .b(x02), .c(new_n43_), .O(new_n80_));
  nand2  g46(.a(new_n78_), .b(x12), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n71_), .b(new_n77_), .c(new_n82_), .d(new_n76_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(x04), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n80_), .c(x16), .O(new_n85_));
  nand2  g51(.a(new_n84_), .b(x14), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n85_), .O(z08));
  inv1   g53(.a(new_n44_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n43_), .O(z13));
  inv1   g55(.a(x13), .O(new_n90_));
  nand2  g56(.a(new_n49_), .b(new_n35_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n82_), .d(new_n76_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x10), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n66_), .c(x08), .d(x07), .O(new_n94_));
  nand2  g60(.a(new_n62_), .b(x16), .O(new_n95_));
  oai21  g61(.a(new_n94_), .b(new_n47_), .c(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x04), .O(new_n97_));
  aoi21  g63(.a(new_n83_), .b(x13), .c(new_n44_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n97_), .c(z13), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand4  g66(.a(new_n90_), .b(new_n82_), .c(new_n76_), .d(new_n75_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n91_), .c(new_n66_), .d(x08), .O(new_n103_));
  inv1   g69(.a(new_n103_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(x07), .c(x06), .O(new_n105_));
  nand3  g71(.a(new_n62_), .b(x16), .c(x14), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n43_), .O(z10));
  nor2   g73(.a(new_n44_), .b(new_n38_), .O(z11));
  nand2  g74(.a(new_n88_), .b(x03), .O(new_n109_));
  nand2  g75(.a(x16), .b(new_n38_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n43_), .O(z12));
  nand2  g77(.a(x17), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n88_), .O(z14));
endmodule


