// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x11), .O(new_n36_));
  inv1   g02(.a(x12), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n38_), .O(z02));
  xor2a  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n53_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n38_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n53_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n38_), .O(z04));
  oai21  g30(.a(new_n46_), .b(x02), .c(new_n50_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x16), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n38_), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nand3  g37(.a(new_n60_), .b(new_n71_), .c(x08), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n70_), .c(new_n68_), .d(new_n38_), .O(z05));
  nand2  g39(.a(new_n72_), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n66_), .c(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n38_), .O(new_n76_));
  aoi21  g42(.a(new_n37_), .b(new_n36_), .c(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n71_), .c(x08), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(x07), .c(x06), .d(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n76_), .O(z06));
  nand2  g47(.a(x11), .b(x10), .O(new_n82_));
  inv1   g48(.a(x10), .O(new_n83_));
  nand3  g49(.a(x12), .b(new_n36_), .c(new_n83_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n69_), .c(new_n82_), .O(new_n85_));
  aoi22  g51(.a(new_n85_), .b(new_n71_), .c(new_n72_), .d(x11), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n46_), .c(new_n68_), .O(z07));
  nand4  g53(.a(new_n60_), .b(new_n83_), .c(new_n71_), .d(x08), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(x04), .c(x11), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n37_), .c(new_n68_), .O(z08));
  nor2   g56(.a(x13), .b(new_n46_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n66_), .c(new_n42_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n50_), .b(x02), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(x16), .c(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  nand2  g62(.a(new_n52_), .b(x14), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n93_), .c(new_n97_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n38_), .c(x04), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(z10));
  nor2   g66(.a(new_n42_), .b(new_n39_), .O(z11));
  oai21  g67(.a(new_n51_), .b(x02), .c(new_n50_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n38_), .c(x04), .O(new_n103_));
  inv1   g69(.a(new_n103_), .O(z12));
  nand4  g70(.a(new_n77_), .b(x08), .c(x07), .d(x06), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n82_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n71_), .O(new_n107_));
  aoi22  g73(.a(new_n72_), .b(new_n38_), .c(x12), .d(x10), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n46_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n38_), .O(z14));
endmodule


