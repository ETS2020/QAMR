// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n119_, new_n122_;
  inv1   g00(.a(x01), .O(new_n35_));
  aoi21  g01(.a(x14), .b(new_n35_), .c(x02), .O(z00));
  inv1   g02(.a(x02), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n38_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  nand2  g14(.a(new_n47_), .b(new_n37_), .O(new_n49_));
  oai21  g15(.a(new_n48_), .b(new_n35_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(new_n38_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nor2   g30(.a(x09), .b(new_n56_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n57_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  nand2  g34(.a(x03), .b(x01), .O(new_n69_));
  nand2  g35(.a(x04), .b(new_n37_), .O(new_n70_));
  aoi21  g36(.a(new_n70_), .b(new_n69_), .c(new_n47_), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n68_), .O(z05));
  oai21  g39(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x01), .O(new_n75_));
  oai21  g41(.a(x16), .b(new_n42_), .c(new_n37_), .O(new_n76_));
  nand2  g42(.a(new_n66_), .b(x10), .O(new_n77_));
  nor2   g43(.a(x10), .b(x09), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x08), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n57_), .c(new_n38_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(z06));
  nand2  g48(.a(new_n80_), .b(new_n57_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x11), .O(new_n84_));
  inv1   g50(.a(x09), .O(new_n85_));
  inv1   g51(.a(new_n63_), .O(new_n86_));
  nor2   g52(.a(x11), .b(x10), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  and2   g54(.a(new_n88_), .b(new_n62_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n84_), .c(new_n76_), .d(new_n75_), .O(z07));
  nand2  g56(.a(new_n88_), .b(x12), .O(new_n91_));
  nor2   g57(.a(x12), .b(x11), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n78_), .c(new_n86_), .O(new_n93_));
  and2   g59(.a(new_n93_), .b(new_n62_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n91_), .c(new_n76_), .d(new_n75_), .O(z08));
  nand4  g61(.a(new_n57_), .b(x04), .c(new_n46_), .d(x01), .O(new_n96_));
  nor3   g62(.a(x13), .b(x12), .c(x11), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n80_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n96_), .c(x01), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x02), .O(new_n100_));
  nand4  g66(.a(new_n65_), .b(x07), .c(x06), .d(x04), .O(new_n101_));
  inv1   g67(.a(x12), .O(new_n102_));
  inv1   g68(.a(x13), .O(new_n103_));
  nand4  g69(.a(new_n87_), .b(new_n47_), .c(new_n103_), .d(new_n102_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n101_), .c(x04), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n62_), .O(new_n106_));
  aoi21  g72(.a(new_n93_), .b(x13), .c(new_n71_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n106_), .c(new_n100_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  nand3  g75(.a(new_n87_), .b(new_n103_), .c(new_n102_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n50_), .c(new_n85_), .d(x08), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(x07), .c(x06), .O(new_n114_));
  nand2  g80(.a(new_n69_), .b(x02), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(x16), .c(x14), .O(new_n116_));
  aoi21  g82(.a(new_n116_), .b(new_n114_), .c(new_n42_), .O(z10));
  nor2   g83(.a(new_n37_), .b(new_n35_), .O(z11));
  oai21  g84(.a(x16), .b(x03), .c(new_n37_), .O(new_n119_));
  aoi21  g85(.a(new_n119_), .b(new_n69_), .c(new_n42_), .O(z12));
  nand2  g86(.a(new_n62_), .b(new_n42_), .O(z13));
  inv1   g87(.a(x17), .O(new_n122_));
  nor3   g88(.a(new_n38_), .b(new_n122_), .c(new_n42_), .O(z14));
endmodule


