// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n119_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  inv1   g03(.a(x16), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  oai21  g05(.a(new_n39_), .b(x01), .c(new_n36_), .O(z00));
  nor2   g06(.a(new_n38_), .b(x01), .O(new_n41_));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n41_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(x02), .c(x01), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(new_n41_), .O(new_n54_));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  aoi21  g21(.a(new_n49_), .b(x02), .c(new_n38_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n55_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n54_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n60_), .b(x07), .c(x06), .O(new_n62_));
  oai21  g28(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n51_), .c(x04), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  nor2   g31(.a(new_n57_), .b(new_n45_), .O(new_n66_));
  nand3  g32(.a(x08), .b(x07), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n61_), .b(new_n69_), .c(x08), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(x04), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n66_), .c(x01), .O(new_n72_));
  nand2  g38(.a(new_n71_), .b(new_n38_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(z05));
  nand2  g40(.a(new_n70_), .b(x10), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n61_), .c(x08), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(x04), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n66_), .c(x01), .O(new_n79_));
  nand2  g45(.a(new_n78_), .b(new_n38_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n79_), .O(z06));
  nand2  g47(.a(new_n77_), .b(x11), .O(new_n82_));
  inv1   g48(.a(x10), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  inv1   g50(.a(new_n67_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n69_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n82_), .c(x04), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n66_), .c(x01), .O(new_n88_));
  nand2  g54(.a(new_n87_), .b(new_n38_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n88_), .O(z07));
  oai21  g56(.a(new_n56_), .b(new_n45_), .c(x01), .O(new_n91_));
  nand2  g57(.a(new_n86_), .b(x12), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand4  g59(.a(new_n76_), .b(new_n85_), .c(new_n93_), .d(new_n84_), .O(new_n94_));
  aoi21  g60(.a(new_n38_), .b(new_n45_), .c(new_n41_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n92_), .d(new_n91_), .O(z08));
  nand2  g62(.a(new_n54_), .b(new_n45_), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n51_), .b(new_n98_), .c(new_n93_), .d(new_n84_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(x10), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n69_), .c(x08), .d(x07), .O(new_n101_));
  nand2  g67(.a(new_n56_), .b(x01), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n48_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x04), .O(new_n104_));
  aoi21  g70(.a(new_n94_), .b(x13), .c(new_n41_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n97_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  nand4  g73(.a(new_n98_), .b(new_n93_), .c(new_n84_), .d(new_n83_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n51_), .c(new_n69_), .d(x08), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(x07), .c(x06), .O(new_n112_));
  nand3  g78(.a(new_n56_), .b(x14), .c(x01), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n112_), .c(new_n45_), .O(z10));
  nor2   g80(.a(new_n41_), .b(new_n35_), .O(z11));
  nand2  g81(.a(new_n38_), .b(x03), .O(new_n116_));
  aoi21  g82(.a(new_n116_), .b(new_n102_), .c(new_n45_), .O(z12));
  nor2   g83(.a(new_n41_), .b(new_n45_), .O(z13));
  nand2  g84(.a(x17), .b(x04), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(new_n54_), .O(z14));
endmodule


