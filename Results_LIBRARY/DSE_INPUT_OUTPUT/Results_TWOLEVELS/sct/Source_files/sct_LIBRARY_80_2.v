// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n117_;
  inv1   g00(.a(x11), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n36_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n37_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  inv1   g16(.a(x07), .O(new_n51_));
  nand3  g17(.a(new_n46_), .b(new_n51_), .c(x06), .O(new_n52_));
  inv1   g18(.a(x17), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x11), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(x07), .c(new_n47_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n48_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nor2   g25(.a(new_n51_), .b(new_n47_), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n48_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n46_), .O(z04));
  inv1   g30(.a(x03), .O(new_n65_));
  oai21  g31(.a(new_n43_), .b(x02), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x16), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  nand3  g35(.a(x08), .b(x07), .c(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x09), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nand3  g38(.a(new_n60_), .b(new_n72_), .c(x08), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n71_), .c(new_n69_), .d(new_n46_), .O(z05));
  nand2  g40(.a(new_n73_), .b(x10), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x08), .O(new_n77_));
  nor3   g43(.a(new_n77_), .b(new_n51_), .c(new_n47_), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n43_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n75_), .c(new_n67_), .O(new_n80_));
  and2   g46(.a(new_n80_), .b(new_n46_), .O(z06));
  nand4  g47(.a(new_n48_), .b(x07), .c(new_n47_), .d(x04), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n76_), .c(x08), .d(x07), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(x17), .c(x11), .O(new_n84_));
  inv1   g50(.a(x10), .O(new_n85_));
  inv1   g51(.a(new_n70_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n35_), .c(new_n85_), .d(new_n72_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n84_), .c(new_n69_), .O(z07));
  inv1   g54(.a(x12), .O(new_n89_));
  nand4  g55(.a(new_n86_), .b(new_n89_), .c(new_n85_), .d(new_n72_), .O(new_n90_));
  oai21  g56(.a(new_n78_), .b(new_n89_), .c(new_n90_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n35_), .O(new_n92_));
  nand3  g58(.a(x17), .b(x12), .c(x11), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n69_), .O(z08));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n48_), .b(new_n95_), .c(new_n89_), .d(new_n35_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n85_), .c(new_n72_), .d(x08), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x07), .c(x06), .d(x04), .O(new_n100_));
  aoi21  g66(.a(x17), .b(new_n95_), .c(new_n35_), .O(new_n101_));
  aoi21  g67(.a(new_n90_), .b(x13), .c(new_n101_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n100_), .c(new_n69_), .O(z09));
  nor3   g69(.a(new_n96_), .b(x10), .c(x09), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(x08), .c(x07), .d(x06), .O(new_n105_));
  inv1   g71(.a(x00), .O(new_n106_));
  inv1   g72(.a(x16), .O(new_n107_));
  aoi21  g73(.a(new_n65_), .b(x02), .c(new_n107_), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(x09), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(x08), .c(x07), .d(x06), .O(new_n110_));
  nand2  g76(.a(new_n108_), .b(x14), .O(new_n111_));
  oai21  g77(.a(new_n110_), .b(new_n106_), .c(new_n111_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n46_), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n105_), .c(new_n43_), .O(z10));
  nand2  g80(.a(new_n46_), .b(new_n37_), .O(z11));
  nor2   g81(.a(new_n36_), .b(new_n107_), .O(new_n116_));
  aoi21  g82(.a(new_n116_), .b(new_n37_), .c(x03), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(new_n43_), .c(new_n46_), .O(z12));
  nand2  g84(.a(new_n46_), .b(new_n43_), .O(z13));
  nor2   g85(.a(new_n53_), .b(new_n43_), .O(z14));
endmodule


