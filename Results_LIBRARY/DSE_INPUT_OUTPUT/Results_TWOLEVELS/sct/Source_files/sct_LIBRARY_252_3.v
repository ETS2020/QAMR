// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n117_, new_n120_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x11), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z01));
  oai21  g15(.a(x03), .b(new_n38_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n35_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nor2   g19(.a(x11), .b(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n53_), .b(x06), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n50_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  oai21  g24(.a(new_n54_), .b(new_n35_), .c(x08), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nor2   g26(.a(new_n53_), .b(new_n35_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n36_), .c(new_n60_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n50_), .c(x04), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  inv1   g31(.a(new_n37_), .O(new_n66_));
  inv1   g32(.a(x16), .O(new_n67_));
  aoi21  g33(.a(x04), .b(new_n38_), .c(x03), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n67_), .c(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x08), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n53_), .c(new_n36_), .O(new_n73_));
  nor2   g39(.a(new_n60_), .b(new_n53_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  aoi22  g41(.a(new_n75_), .b(x09), .c(new_n73_), .d(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n70_), .O(z05));
  nor2   g43(.a(x10), .b(x09), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(x11), .c(x06), .O(new_n81_));
  inv1   g47(.a(new_n61_), .O(new_n82_));
  oai21  g48(.a(new_n72_), .b(new_n82_), .c(x10), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n70_), .O(z06));
  inv1   g50(.a(new_n69_), .O(new_n85_));
  nand3  g51(.a(new_n78_), .b(new_n61_), .c(x08), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n36_), .O(z07));
  inv1   g53(.a(x12), .O(new_n88_));
  nor2   g54(.a(new_n80_), .b(new_n88_), .O(new_n89_));
  inv1   g55(.a(x10), .O(new_n90_));
  nand3  g56(.a(new_n88_), .b(new_n90_), .c(new_n71_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n75_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n89_), .c(new_n36_), .O(new_n93_));
  nand2  g59(.a(x12), .b(new_n35_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n70_), .O(z08));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n50_), .b(new_n96_), .c(new_n88_), .d(new_n90_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(x09), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(x08), .c(x07), .d(x06), .O(new_n99_));
  inv1   g65(.a(new_n74_), .O(new_n100_));
  oai21  g66(.a(new_n91_), .b(new_n100_), .c(x13), .O(new_n101_));
  oai21  g67(.a(new_n99_), .b(new_n47_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n36_), .O(new_n103_));
  nand2  g69(.a(x13), .b(new_n35_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n103_), .c(new_n70_), .O(z09));
  inv1   g71(.a(x00), .O(new_n106_));
  inv1   g72(.a(x03), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(x02), .c(new_n67_), .O(new_n108_));
  nand4  g74(.a(new_n96_), .b(new_n88_), .c(new_n36_), .d(new_n90_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n106_), .c(new_n108_), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(new_n71_), .c(x08), .d(x07), .O(new_n111_));
  oai22  g77(.a(new_n111_), .b(new_n35_), .c(new_n50_), .d(new_n41_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n66_), .O(z10));
  nand2  g80(.a(new_n66_), .b(new_n38_), .O(z11));
  oai21  g81(.a(new_n67_), .b(x02), .c(new_n107_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n66_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  nand2  g84(.a(new_n66_), .b(new_n47_), .O(z13));
  nand3  g85(.a(new_n66_), .b(x17), .c(x04), .O(new_n120_));
  inv1   g86(.a(new_n120_), .O(z14));
endmodule


