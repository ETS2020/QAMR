// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_, new_n121_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x12), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z01));
  inv1   g15(.a(new_n37_), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n38_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n35_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n50_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n50_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  inv1   g23(.a(x07), .O(new_n58_));
  aoi21  g24(.a(new_n36_), .b(new_n58_), .c(new_n35_), .O(new_n59_));
  nor2   g25(.a(new_n58_), .b(new_n35_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n36_), .c(new_n57_), .O(new_n61_));
  oai21  g27(.a(new_n59_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n51_), .c(x04), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  inv1   g30(.a(x03), .O(new_n65_));
  inv1   g31(.a(x16), .O(new_n66_));
  nand2  g32(.a(x04), .b(new_n38_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n65_), .c(new_n66_), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n47_), .c(new_n50_), .O(new_n69_));
  nor2   g35(.a(new_n57_), .b(new_n58_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(x12), .c(x06), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nand3  g38(.a(new_n36_), .b(new_n72_), .c(x08), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  aoi22  g40(.a(new_n74_), .b(new_n60_), .c(new_n71_), .d(x09), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n69_), .O(z05));
  nand3  g42(.a(new_n72_), .b(x08), .c(x07), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n36_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x10), .O(new_n80_));
  inv1   g46(.a(x10), .O(new_n81_));
  nand2  g47(.a(new_n70_), .b(x06), .O(new_n82_));
  inv1   g48(.a(new_n82_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n36_), .c(new_n81_), .d(new_n72_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n80_), .c(new_n69_), .O(z06));
  inv1   g51(.a(x11), .O(new_n86_));
  nand3  g52(.a(new_n36_), .b(new_n86_), .c(new_n81_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n77_), .c(new_n36_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x06), .O(new_n89_));
  inv1   g55(.a(new_n60_), .O(new_n90_));
  nand3  g56(.a(new_n81_), .b(new_n72_), .c(x08), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n90_), .c(x11), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n89_), .c(new_n69_), .O(z07));
  nand3  g59(.a(new_n86_), .b(new_n81_), .c(new_n72_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n82_), .c(x04), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n68_), .c(new_n36_), .O(new_n96_));
  oai21  g62(.a(new_n36_), .b(x06), .c(new_n96_), .O(z08));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n51_), .b(new_n98_), .c(new_n86_), .d(new_n81_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(x09), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(x08), .c(x07), .d(x06), .O(new_n101_));
  inv1   g67(.a(new_n70_), .O(new_n102_));
  oai21  g68(.a(new_n94_), .b(new_n102_), .c(x13), .O(new_n103_));
  oai21  g69(.a(new_n101_), .b(new_n47_), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n36_), .O(new_n105_));
  nand2  g71(.a(x13), .b(new_n35_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n105_), .c(new_n69_), .O(z09));
  inv1   g73(.a(x00), .O(new_n108_));
  aoi21  g74(.a(new_n65_), .b(x02), .c(new_n66_), .O(new_n109_));
  nand4  g75(.a(new_n98_), .b(new_n36_), .c(new_n86_), .d(new_n81_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n108_), .c(new_n109_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n72_), .c(x08), .d(x07), .O(new_n112_));
  oai22  g78(.a(new_n112_), .b(new_n35_), .c(new_n51_), .d(new_n41_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n50_), .O(z10));
  nor2   g81(.a(new_n37_), .b(new_n38_), .O(z11));
  oai21  g82(.a(new_n66_), .b(x02), .c(new_n65_), .O(new_n117_));
  nand3  g83(.a(new_n117_), .b(new_n50_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z12));
  nand2  g85(.a(new_n50_), .b(new_n47_), .O(z13));
  nand2  g86(.a(x17), .b(x04), .O(new_n121_));
  nand2  g87(.a(new_n121_), .b(new_n50_), .O(z14));
endmodule


