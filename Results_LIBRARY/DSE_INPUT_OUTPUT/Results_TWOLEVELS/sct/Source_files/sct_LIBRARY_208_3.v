// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  oai21  g01(.a(x15), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x15), .b(x01), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n40_), .c(new_n42_), .O(new_n43_));
  oai21  g09(.a(new_n39_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(new_n47_));
  nor2   g13(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n45_), .c(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n45_), .O(new_n56_));
  nand3  g22(.a(new_n54_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n54_), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n46_), .c(new_n41_), .d(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  nor2   g26(.a(new_n40_), .b(x02), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(x03), .c(x16), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n56_), .b(new_n65_), .c(x08), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n41_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n64_), .c(new_n62_), .d(x04), .O(z05));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n56_), .c(x08), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n41_), .c(x04), .O(new_n71_));
  aoi21  g37(.a(new_n66_), .b(x10), .c(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n62_), .O(z06));
  nand2  g39(.a(new_n70_), .b(x11), .O(new_n74_));
  nor4   g40(.a(new_n63_), .b(x11), .c(x10), .d(x09), .O(new_n75_));
  nor2   g41(.a(new_n75_), .b(new_n42_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n74_), .c(new_n62_), .d(x04), .O(z07));
  inv1   g43(.a(new_n75_), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  inv1   g46(.a(new_n63_), .O(new_n81_));
  nand4  g47(.a(new_n69_), .b(new_n81_), .c(new_n80_), .d(new_n79_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n41_), .c(x04), .O(new_n83_));
  aoi21  g49(.a(new_n78_), .b(x12), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n62_), .O(z08));
  inv1   g51(.a(x10), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n46_), .b(new_n41_), .c(new_n87_), .d(new_n80_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(x11), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n86_), .c(new_n65_), .d(x08), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n55_), .O(new_n91_));
  inv1   g57(.a(x16), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(x02), .O(new_n93_));
  aoi21  g59(.a(new_n91_), .b(x06), .c(new_n93_), .O(new_n94_));
  nand2  g60(.a(new_n82_), .b(x13), .O(new_n95_));
  aoi21  g61(.a(x16), .b(x03), .c(new_n42_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(x04), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand4  g64(.a(new_n87_), .b(new_n80_), .c(new_n79_), .d(new_n86_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n47_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n65_), .c(x08), .d(x07), .O(new_n101_));
  nand2  g67(.a(new_n47_), .b(x14), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n45_), .c(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n41_), .c(x04), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(z10));
  nor2   g71(.a(new_n42_), .b(new_n35_), .O(z11));
  oai21  g72(.a(new_n93_), .b(x03), .c(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n41_), .O(z12));
  nand2  g74(.a(new_n41_), .b(new_n40_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n41_), .O(z14));
endmodule


