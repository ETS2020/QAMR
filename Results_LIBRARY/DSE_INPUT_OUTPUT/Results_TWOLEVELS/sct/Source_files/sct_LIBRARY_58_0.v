// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n108_;
  nor2   g00(.a(x06), .b(x04), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nor2   g09(.a(x05), .b(new_n43_), .O(new_n44_));
  inv1   g10(.a(x06), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x04), .O(new_n46_));
  oai22  g12(.a(new_n46_), .b(new_n44_), .c(new_n42_), .d(x15), .O(new_n47_));
  nand3  g13(.a(x06), .b(x05), .c(new_n43_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n47_), .O(z01));
  inv1   g15(.a(x03), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x02), .O(new_n51_));
  inv1   g17(.a(x16), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n43_), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(new_n51_), .c(x06), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(x03), .b(new_n36_), .c(x16), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n59_), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n56_), .c(x04), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n60_), .b(new_n67_), .c(x08), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n66_), .c(new_n56_), .d(x04), .O(z05));
  nand2  g35(.a(new_n68_), .b(x10), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n60_), .c(x08), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n70_), .c(new_n56_), .d(x04), .O(z06));
  nand2  g39(.a(x08), .b(x07), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n67_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n74_), .c(x04), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  aoi21  g45(.a(new_n50_), .b(x02), .c(new_n52_), .O(new_n80_));
  aoi21  g46(.a(new_n72_), .b(x11), .c(new_n80_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n43_), .c(new_n79_), .O(z07));
  oai21  g48(.a(new_n77_), .b(new_n65_), .c(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  inv1   g50(.a(new_n65_), .O(new_n85_));
  nand4  g51(.a(new_n71_), .b(new_n85_), .c(new_n84_), .d(new_n76_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n83_), .c(new_n56_), .d(x04), .O(z08));
  inv1   g53(.a(new_n46_), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand4  g55(.a(new_n56_), .b(new_n89_), .c(new_n84_), .d(new_n76_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(x10), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n67_), .c(x08), .d(x07), .O(new_n92_));
  aoi21  g58(.a(new_n86_), .b(x13), .c(new_n80_), .O(new_n93_));
  oai21  g59(.a(new_n92_), .b(new_n45_), .c(new_n93_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n88_), .O(z09));
  inv1   g62(.a(new_n35_), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand4  g64(.a(new_n89_), .b(new_n84_), .c(new_n76_), .d(new_n75_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n80_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n67_), .c(x08), .d(x07), .O(new_n101_));
  oai22  g67(.a(new_n101_), .b(new_n45_), .c(new_n56_), .d(new_n39_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n97_), .O(z10));
  nor2   g70(.a(new_n35_), .b(new_n36_), .O(z11));
  nand2  g71(.a(x16), .b(new_n36_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n50_), .c(new_n43_), .O(z12));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n97_), .O(z14));
  buf    g75(.a(x04), .O(z13));
endmodule


