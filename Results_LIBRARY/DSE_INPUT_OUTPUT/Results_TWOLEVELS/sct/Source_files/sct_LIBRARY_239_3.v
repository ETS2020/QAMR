// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n71_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(x08), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n45_), .c(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n36_), .O(z03));
  inv1   g17(.a(x08), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x07), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n46_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(x03), .O(new_n57_));
  oai21  g23(.a(new_n42_), .b(x02), .c(new_n57_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x16), .O(new_n59_));
  nor2   g25(.a(x09), .b(new_n42_), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(new_n59_), .c(new_n37_), .O(z05));
  inv1   g27(.a(x10), .O(new_n62_));
  nand4  g28(.a(new_n59_), .b(new_n36_), .c(new_n62_), .d(x04), .O(z06));
  nor2   g29(.a(x11), .b(new_n42_), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(new_n59_), .c(new_n37_), .O(z07));
  nor2   g31(.a(x12), .b(new_n42_), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(new_n59_), .c(new_n37_), .O(z08));
  inv1   g33(.a(x13), .O(new_n68_));
  nand4  g34(.a(new_n59_), .b(new_n36_), .c(new_n68_), .d(x04), .O(z09));
  nand2  g35(.a(new_n57_), .b(x02), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n36_), .c(x16), .d(x14), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(new_n42_), .O(z10));
  nor2   g38(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g39(.a(x16), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(x02), .c(new_n57_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n36_), .c(x04), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(z12));
  nand3  g43(.a(x08), .b(x07), .c(x06), .O(new_n78_));
  aoi22  g44(.a(new_n78_), .b(new_n46_), .c(new_n75_), .d(new_n36_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n42_), .c(new_n36_), .O(z13));
  nand3  g46(.a(new_n36_), .b(x17), .c(x04), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(z14));
endmodule


