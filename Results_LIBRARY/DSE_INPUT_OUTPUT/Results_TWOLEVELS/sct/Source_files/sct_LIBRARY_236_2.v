// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x08), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n36_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n39_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n47_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n47_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n48_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n37_), .O(z04));
  inv1   g24(.a(x16), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x02), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n37_), .O(new_n61_));
  inv1   g27(.a(x17), .O(new_n62_));
  oai21  g28(.a(x08), .b(new_n44_), .c(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x16), .c(x03), .O(new_n64_));
  and2   g30(.a(x07), .b(x06), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n62_), .c(x08), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand4  g34(.a(new_n65_), .b(x17), .c(new_n68_), .d(x08), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n64_), .d(new_n61_), .O(z05));
  nand3  g36(.a(new_n68_), .b(x07), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x17), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x08), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  nand3  g40(.a(x08), .b(x07), .c(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nor2   g42(.a(new_n62_), .b(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n68_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n74_), .c(new_n64_), .d(new_n61_), .O(z06));
  nor2   g45(.a(x10), .b(x09), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n65_), .c(new_n62_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n35_), .c(x11), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand4  g49(.a(new_n80_), .b(new_n76_), .c(x17), .d(new_n83_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n82_), .c(new_n64_), .d(new_n61_), .O(z07));
  nor3   g51(.a(x11), .b(x10), .c(x09), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n65_), .c(new_n62_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n35_), .c(x12), .O(new_n88_));
  nor2   g54(.a(x11), .b(x10), .O(new_n89_));
  nor2   g55(.a(x09), .b(new_n35_), .O(new_n90_));
  nor2   g56(.a(new_n62_), .b(x12), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n65_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n88_), .c(new_n64_), .d(new_n61_), .O(z08));
  inv1   g59(.a(x12), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n48_), .b(x17), .c(new_n95_), .d(new_n94_), .O(new_n96_));
  nor3   g62(.a(new_n96_), .b(x11), .c(x10), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n68_), .c(x07), .d(x06), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n44_), .c(x17), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x08), .O(new_n100_));
  nand3  g66(.a(new_n80_), .b(new_n94_), .c(new_n83_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n75_), .c(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n100_), .c(new_n64_), .d(new_n61_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(new_n48_), .O(new_n105_));
  nand3  g71(.a(new_n89_), .b(new_n95_), .c(new_n94_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n104_), .c(new_n105_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n68_), .c(x08), .d(x07), .O(new_n108_));
  nand2  g74(.a(new_n105_), .b(x14), .O(new_n109_));
  oai21  g75(.a(new_n108_), .b(new_n47_), .c(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x17), .O(new_n111_));
  nand3  g77(.a(new_n105_), .b(x14), .c(new_n35_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n111_), .c(new_n44_), .O(z10));
  nand2  g79(.a(new_n37_), .b(new_n39_), .O(z11));
  inv1   g80(.a(x03), .O(new_n115_));
  oai21  g81(.a(new_n59_), .b(x02), .c(new_n115_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n37_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  nor2   g84(.a(new_n36_), .b(new_n44_), .O(z13));
  oai21  g85(.a(new_n62_), .b(new_n44_), .c(new_n37_), .O(z14));
endmodule


