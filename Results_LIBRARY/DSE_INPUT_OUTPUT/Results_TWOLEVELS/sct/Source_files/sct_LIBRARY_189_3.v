// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n113_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  oai21  g05(.a(x05), .b(x01), .c(x04), .O(new_n40_));
  oai21  g06(.a(new_n39_), .b(x15), .c(new_n40_), .O(new_n41_));
  nand2  g07(.a(x05), .b(new_n36_), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n41_), .O(z01));
  inv1   g09(.a(x01), .O(new_n44_));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(new_n45_), .c(x04), .d(new_n44_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n44_), .c(new_n36_), .O(z03));
  inv1   g17(.a(x08), .O(new_n52_));
  and2   g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n52_), .b(x07), .c(x06), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  aoi21  g22(.a(new_n56_), .b(new_n44_), .c(new_n36_), .O(z04));
  inv1   g23(.a(x03), .O(new_n58_));
  inv1   g24(.a(x16), .O(new_n59_));
  nand2  g25(.a(x04), .b(new_n35_), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(new_n58_), .c(new_n59_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n44_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nor2   g30(.a(new_n36_), .b(new_n44_), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n53_), .b(new_n67_), .c(x08), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(x04), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n66_), .c(new_n64_), .d(new_n62_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n53_), .c(x08), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(x04), .c(new_n44_), .O(new_n75_));
  aoi21  g41(.a(new_n68_), .b(x10), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n62_), .O(z06));
  nand2  g43(.a(new_n74_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  inv1   g45(.a(new_n63_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n71_), .d(new_n67_), .O(new_n81_));
  and2   g47(.a(new_n81_), .b(x04), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n78_), .c(new_n66_), .d(new_n62_), .O(z07));
  nand2  g49(.a(new_n81_), .b(x12), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  nand4  g51(.a(new_n73_), .b(new_n80_), .c(new_n85_), .d(new_n79_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n61_), .c(new_n44_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n46_), .b(new_n90_), .c(new_n85_), .d(new_n79_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(x10), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n67_), .c(x08), .d(x07), .O(new_n93_));
  nand2  g59(.a(x16), .b(new_n35_), .O(new_n94_));
  oai21  g60(.a(new_n93_), .b(new_n45_), .c(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n86_), .b(x13), .O(new_n96_));
  oai21  g62(.a(new_n59_), .b(new_n58_), .c(new_n96_), .O(new_n97_));
  aoi21  g63(.a(new_n95_), .b(x04), .c(new_n97_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(x01), .c(x04), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n71_), .b(new_n44_), .O(new_n101_));
  nand3  g67(.a(new_n90_), .b(new_n85_), .c(new_n79_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n46_), .c(new_n67_), .d(x08), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(x07), .c(x06), .O(new_n106_));
  inv1   g72(.a(new_n46_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(x14), .c(x01), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n106_), .c(new_n36_), .O(z10));
  nor2   g75(.a(new_n65_), .b(new_n35_), .O(z11));
  nand3  g76(.a(new_n94_), .b(new_n58_), .c(new_n44_), .O(new_n111_));
  and2   g77(.a(new_n111_), .b(x04), .O(z12));
  inv1   g78(.a(x17), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n44_), .c(new_n36_), .O(z14));
  buf    g80(.a(x04), .O(z13));
endmodule


