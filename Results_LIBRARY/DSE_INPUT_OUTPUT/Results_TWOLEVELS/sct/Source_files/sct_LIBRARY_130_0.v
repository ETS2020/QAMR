// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x17), .O(new_n36_));
  oai21  g02(.a(new_n36_), .b(x01), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  nand2  g09(.a(new_n36_), .b(new_n35_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n44_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n44_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  and2   g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n53_), .b(x07), .c(x06), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(x16), .b(x03), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x02), .O(new_n58_));
  inv1   g24(.a(x16), .O(new_n59_));
  nand2  g25(.a(x17), .b(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n56_), .c(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  nand2  g29(.a(new_n57_), .b(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x02), .O(new_n65_));
  oai21  g31(.a(new_n59_), .b(new_n42_), .c(x17), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n35_), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n54_), .b(new_n70_), .c(x08), .O(new_n71_));
  nor2   g37(.a(new_n36_), .b(x04), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  and2   g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(z05));
  nand2  g41(.a(new_n71_), .b(x10), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n54_), .c(x08), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(x04), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n44_), .O(new_n80_));
  nand2  g46(.a(x03), .b(x02), .O(new_n81_));
  nand2  g47(.a(x17), .b(x04), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(x02), .c(new_n81_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x16), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n80_), .O(z06));
  nand2  g51(.a(new_n78_), .b(x11), .O(new_n86_));
  inv1   g52(.a(new_n68_), .O(new_n87_));
  nor2   g53(.a(x11), .b(x10), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n70_), .O(new_n89_));
  and2   g55(.a(new_n89_), .b(new_n73_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n86_), .c(new_n67_), .d(new_n65_), .O(z07));
  nand2  g57(.a(new_n89_), .b(x12), .O(new_n92_));
  inv1   g58(.a(x11), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  nand4  g60(.a(new_n77_), .b(new_n87_), .c(new_n94_), .d(new_n93_), .O(new_n95_));
  and2   g61(.a(new_n95_), .b(new_n73_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n92_), .c(new_n67_), .d(new_n65_), .O(z08));
  nand2  g63(.a(new_n95_), .b(x13), .O(new_n98_));
  nor2   g64(.a(new_n46_), .b(new_n42_), .O(new_n99_));
  nand3  g65(.a(new_n70_), .b(x08), .c(x07), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(new_n101_));
  nor3   g67(.a(x16), .b(x13), .c(x12), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n101_), .c(new_n99_), .d(new_n88_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n98_), .c(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n44_), .O(new_n105_));
  inv1   g71(.a(x03), .O(new_n106_));
  nand3  g72(.a(new_n87_), .b(x04), .c(new_n106_), .O(new_n107_));
  inv1   g73(.a(x13), .O(new_n108_));
  nand4  g74(.a(new_n77_), .b(new_n108_), .c(new_n94_), .d(new_n93_), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n107_), .c(new_n57_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x02), .O(new_n111_));
  nand4  g77(.a(x17), .b(x16), .c(x04), .d(new_n35_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n111_), .c(new_n105_), .O(z09));
  inv1   g79(.a(x00), .O(new_n114_));
  inv1   g80(.a(x10), .O(new_n115_));
  nand4  g81(.a(new_n108_), .b(new_n94_), .c(new_n93_), .d(new_n115_), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand4  g83(.a(new_n117_), .b(new_n61_), .c(new_n70_), .d(x08), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(new_n119_));
  nand3  g85(.a(new_n119_), .b(x07), .c(x06), .O(new_n120_));
  oai21  g86(.a(new_n36_), .b(x02), .c(new_n81_), .O(new_n121_));
  nand3  g87(.a(new_n121_), .b(x16), .c(x14), .O(new_n122_));
  aoi21  g88(.a(new_n122_), .b(new_n120_), .c(new_n42_), .O(z10));
  oai21  g89(.a(new_n42_), .b(new_n106_), .c(new_n67_), .O(z12));
  nand2  g90(.a(new_n44_), .b(new_n42_), .O(z13));
  nand2  g91(.a(new_n82_), .b(new_n44_), .O(z14));
  buf    g92(.a(x02), .O(z11));
endmodule


