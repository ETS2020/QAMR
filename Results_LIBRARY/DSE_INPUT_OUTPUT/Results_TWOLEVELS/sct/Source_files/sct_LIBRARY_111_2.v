// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n114_;
  inv1   g00(.a(x08), .O(new_n35_));
  nor2   g01(.a(x17), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n36_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n39_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n37_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n48_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n48_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n49_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n37_), .O(z04));
  inv1   g25(.a(x16), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(x02), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n37_), .O(new_n62_));
  inv1   g28(.a(x17), .O(new_n63_));
  oai21  g29(.a(x08), .b(new_n45_), .c(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x16), .c(x03), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x07), .c(x06), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n63_), .c(x08), .O(new_n69_));
  nand3  g35(.a(x08), .b(x07), .c(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x09), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n69_), .c(new_n65_), .d(new_n62_), .O(z05));
  nand2  g38(.a(new_n69_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(new_n70_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x17), .c(new_n74_), .d(new_n66_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n73_), .c(new_n65_), .d(new_n62_), .O(z06));
  nand2  g43(.a(x07), .b(x06), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n74_), .c(new_n66_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n78_), .c(x17), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x08), .O(new_n82_));
  nand3  g48(.a(new_n74_), .b(new_n66_), .c(x08), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n78_), .c(x11), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n82_), .c(new_n65_), .d(new_n62_), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nand4  g52(.a(new_n68_), .b(new_n86_), .c(new_n79_), .d(new_n74_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x17), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x08), .O(new_n89_));
  oai21  g55(.a(new_n80_), .b(new_n70_), .c(x12), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n65_), .d(new_n62_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n49_), .b(new_n92_), .c(new_n86_), .d(new_n79_), .O(new_n93_));
  nor3   g59(.a(new_n93_), .b(x10), .c(x09), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(x08), .c(x07), .d(x06), .O(new_n95_));
  nand2  g61(.a(new_n87_), .b(x13), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n45_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x17), .O(new_n98_));
  nand2  g64(.a(x13), .b(new_n35_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n98_), .c(new_n65_), .d(new_n62_), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  inv1   g67(.a(new_n49_), .O(new_n102_));
  nand4  g68(.a(new_n92_), .b(new_n86_), .c(new_n79_), .d(new_n74_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n101_), .c(new_n102_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n66_), .c(x08), .d(x07), .O(new_n105_));
  nand2  g71(.a(new_n102_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n48_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x17), .O(new_n108_));
  nand3  g74(.a(new_n102_), .b(x14), .c(new_n35_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n45_), .O(z10));
  nand2  g76(.a(new_n37_), .b(new_n39_), .O(z11));
  inv1   g77(.a(x03), .O(new_n112_));
  oai21  g78(.a(new_n60_), .b(x02), .c(new_n112_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(new_n37_), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z12));
  nor2   g81(.a(new_n36_), .b(new_n45_), .O(z13));
  oai21  g82(.a(new_n63_), .b(new_n45_), .c(new_n37_), .O(z14));
endmodule


