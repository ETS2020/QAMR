// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(x08), .b(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n36_), .c(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z02));
  inv1   g13(.a(new_n37_), .O(new_n48_));
  inv1   g14(.a(x07), .O(new_n49_));
  nand3  g15(.a(x08), .b(new_n49_), .c(x06), .O(new_n50_));
  oai21  g16(.a(new_n49_), .b(x06), .c(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n45_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n48_), .O(z03));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n45_), .c(x08), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(x16), .O(new_n57_));
  aoi21  g23(.a(x04), .b(new_n35_), .c(x03), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n57_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nor2   g27(.a(x09), .b(new_n49_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x06), .O(new_n63_));
  oai21  g29(.a(new_n61_), .b(x07), .c(new_n63_), .O(new_n64_));
  aoi22  g30(.a(new_n64_), .b(x08), .c(x09), .d(new_n36_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n60_), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n61_), .c(x07), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x08), .c(new_n36_), .O(new_n69_));
  aoi21  g35(.a(new_n63_), .b(x10), .c(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n60_), .O(z06));
  nand2  g37(.a(new_n68_), .b(x11), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n61_), .c(x07), .d(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g42(.a(x11), .b(new_n36_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n60_), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  aoi21  g45(.a(new_n73_), .b(new_n62_), .c(new_n79_), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand3  g47(.a(new_n79_), .b(new_n81_), .c(new_n67_), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n63_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n80_), .c(x08), .O(new_n84_));
  nand2  g50(.a(x12), .b(new_n36_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n60_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n45_), .b(new_n87_), .c(new_n79_), .d(new_n81_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(x10), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n61_), .c(x08), .d(x07), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n42_), .c(x08), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x06), .O(new_n92_));
  oai21  g58(.a(new_n82_), .b(new_n63_), .c(x13), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n60_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand3  g61(.a(new_n73_), .b(new_n87_), .c(new_n79_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n45_), .c(new_n61_), .d(x07), .O(new_n98_));
  inv1   g64(.a(new_n45_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x14), .O(new_n100_));
  oai21  g66(.a(new_n98_), .b(new_n36_), .c(new_n100_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x08), .O(new_n102_));
  nand3  g68(.a(new_n99_), .b(x14), .c(new_n36_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n42_), .O(z10));
  nor2   g70(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g71(.a(x03), .O(new_n106_));
  oai21  g72(.a(new_n57_), .b(x02), .c(new_n106_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n48_), .c(x04), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(z12));
  nand2  g75(.a(new_n48_), .b(new_n42_), .O(z13));
  inv1   g76(.a(x17), .O(new_n111_));
  nor3   g77(.a(new_n37_), .b(new_n111_), .c(new_n42_), .O(z14));
endmodule


