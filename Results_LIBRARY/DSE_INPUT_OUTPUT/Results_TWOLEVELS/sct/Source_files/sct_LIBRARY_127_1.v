// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_;
  inv1   g00(.a(x09), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x07), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n36_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(new_n36_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n37_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n48_), .c(new_n49_), .d(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n48_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nand3  g19(.a(new_n35_), .b(new_n53_), .c(x06), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(x06), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n50_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n48_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  nor2   g24(.a(new_n53_), .b(new_n49_), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n50_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n48_), .O(z04));
  inv1   g29(.a(x16), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(x02), .c(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n48_), .O(new_n66_));
  oai21  g32(.a(x09), .b(new_n45_), .c(new_n53_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x16), .c(x03), .O(new_n68_));
  nand3  g34(.a(x08), .b(x07), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nor2   g36(.a(x09), .b(new_n58_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n70_), .c(new_n68_), .d(new_n66_), .O(z05));
  inv1   g39(.a(x10), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n35_), .c(x08), .d(x06), .O(new_n75_));
  oai21  g41(.a(new_n74_), .b(new_n35_), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x07), .O(new_n77_));
  nand3  g43(.a(new_n69_), .b(x10), .c(new_n35_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n68_), .d(new_n66_), .O(z06));
  oai21  g45(.a(x11), .b(x09), .c(new_n53_), .O(new_n80_));
  inv1   g46(.a(new_n69_), .O(new_n81_));
  nor3   g47(.a(x11), .b(x10), .c(x09), .O(new_n82_));
  aoi22  g48(.a(new_n82_), .b(new_n81_), .c(new_n75_), .d(x11), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n80_), .c(new_n68_), .d(new_n66_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  nor3   g51(.a(x12), .b(x11), .c(x10), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n71_), .c(x06), .O(new_n87_));
  oai21  g53(.a(new_n85_), .b(new_n35_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x07), .O(new_n89_));
  nor2   g55(.a(x11), .b(x10), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n59_), .c(x08), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(x12), .c(new_n35_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n89_), .c(new_n68_), .d(new_n66_), .O(z08));
  inv1   g59(.a(x11), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n50_), .b(new_n95_), .c(new_n85_), .d(new_n94_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n74_), .c(new_n35_), .d(x08), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x07), .c(x06), .d(x04), .O(new_n100_));
  aoi21  g66(.a(new_n95_), .b(new_n35_), .c(x07), .O(new_n101_));
  aoi21  g67(.a(new_n87_), .b(x13), .c(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n100_), .c(new_n68_), .d(new_n66_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand3  g70(.a(new_n90_), .b(new_n95_), .c(new_n85_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n50_), .c(new_n35_), .d(x08), .O(new_n107_));
  oai22  g73(.a(new_n107_), .b(new_n49_), .c(new_n50_), .d(new_n40_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x07), .O(new_n109_));
  inv1   g75(.a(new_n50_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(x14), .c(new_n35_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n109_), .c(new_n45_), .O(z10));
  nand2  g78(.a(new_n48_), .b(new_n37_), .O(z11));
  nor2   g79(.a(new_n36_), .b(new_n64_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n37_), .c(x03), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n45_), .c(new_n48_), .O(z12));
  nand3  g82(.a(new_n50_), .b(x07), .c(x06), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n35_), .O(new_n118_));
  aoi21  g84(.a(new_n118_), .b(new_n53_), .c(new_n45_), .O(z13));
  inv1   g85(.a(x17), .O(new_n120_));
  nor3   g86(.a(new_n36_), .b(new_n120_), .c(new_n45_), .O(z14));
endmodule


