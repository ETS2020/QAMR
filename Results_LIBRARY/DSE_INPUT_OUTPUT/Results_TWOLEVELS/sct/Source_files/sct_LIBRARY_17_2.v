// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n117_, new_n119_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand3  g01(.a(x16), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x01), .O(new_n41_));
  nor2   g07(.a(x16), .b(new_n41_), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n40_), .c(new_n42_), .O(new_n43_));
  oai21  g09(.a(new_n39_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  nand3  g12(.a(x16), .b(new_n46_), .c(x02), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n42_), .O(new_n53_));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nor2   g20(.a(x03), .b(new_n35_), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n54_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n53_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n60_), .b(x07), .c(x06), .O(new_n62_));
  oai21  g28(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n57_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n53_), .O(z04));
  nand3  g31(.a(x08), .b(x07), .c(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n60_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  oai21  g37(.a(new_n40_), .b(x02), .c(new_n46_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x16), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(z05));
  nand2  g40(.a(new_n53_), .b(new_n40_), .O(z13));
  nand2  g41(.a(new_n69_), .b(x10), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n61_), .c(x08), .O(new_n78_));
  and2   g44(.a(new_n78_), .b(new_n53_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n76_), .c(z13), .d(new_n73_), .O(z06));
  nand2  g46(.a(new_n78_), .b(x11), .O(new_n81_));
  inv1   g47(.a(x09), .O(new_n82_));
  nor2   g48(.a(x11), .b(x10), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n42_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n81_), .c(z13), .d(new_n73_), .O(z07));
  inv1   g53(.a(x12), .O(new_n88_));
  nor2   g54(.a(new_n85_), .b(new_n88_), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  inv1   g56(.a(new_n66_), .O(new_n91_));
  nand4  g57(.a(new_n77_), .b(new_n91_), .c(new_n88_), .d(new_n90_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x04), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n89_), .c(new_n53_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n73_), .O(z08));
  nand4  g61(.a(new_n68_), .b(x07), .c(x06), .d(x04), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n83_), .b(new_n48_), .c(new_n97_), .d(new_n88_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n96_), .c(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n41_), .O(new_n100_));
  nor3   g66(.a(x13), .b(x12), .c(x11), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n77_), .c(new_n91_), .d(new_n55_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(x04), .c(new_n46_), .d(x02), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x16), .O(new_n104_));
  aoi21  g70(.a(new_n92_), .b(x13), .c(new_n42_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n104_), .c(new_n100_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  inv1   g73(.a(x10), .O(new_n108_));
  nand4  g74(.a(new_n97_), .b(new_n88_), .c(new_n90_), .d(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(new_n50_), .c(new_n82_), .d(x08), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(x07), .c(x06), .O(new_n113_));
  nand2  g79(.a(new_n56_), .b(x14), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n113_), .c(new_n40_), .O(z10));
  nand2  g81(.a(new_n53_), .b(new_n35_), .O(z11));
  aoi21  g82(.a(x16), .b(new_n35_), .c(x03), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(new_n40_), .c(new_n53_), .O(z12));
  nand2  g84(.a(x17), .b(x04), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(new_n53_), .O(z14));
endmodule


