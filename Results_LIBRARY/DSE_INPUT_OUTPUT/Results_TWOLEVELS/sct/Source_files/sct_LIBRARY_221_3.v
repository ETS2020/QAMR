// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n120_,
    new_n123_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x01), .O(new_n38_));
  nor2   g04(.a(new_n35_), .b(new_n38_), .O(new_n39_));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n39_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n48_), .b(new_n35_), .O(new_n50_));
  oai21  g16(.a(new_n49_), .b(x01), .c(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n46_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(new_n39_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  nor2   g31(.a(x09), .b(new_n57_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g35(.a(x03), .b(new_n38_), .O(new_n70_));
  nand2  g36(.a(x04), .b(new_n35_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n70_), .c(new_n48_), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n69_), .O(z05));
  oai21  g40(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n38_), .O(new_n76_));
  oai21  g42(.a(x16), .b(new_n43_), .c(new_n35_), .O(new_n77_));
  nand2  g43(.a(new_n67_), .b(x10), .O(new_n78_));
  nor2   g44(.a(x10), .b(x09), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x08), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n58_), .c(new_n39_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(z06));
  nand2  g49(.a(new_n81_), .b(new_n58_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x11), .O(new_n85_));
  inv1   g51(.a(x09), .O(new_n86_));
  inv1   g52(.a(new_n64_), .O(new_n87_));
  nor2   g53(.a(x11), .b(x10), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(new_n89_));
  and2   g55(.a(new_n89_), .b(new_n63_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n85_), .c(new_n77_), .d(new_n76_), .O(z07));
  nand2  g57(.a(new_n89_), .b(x12), .O(new_n92_));
  nor2   g58(.a(x12), .b(x11), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n79_), .c(new_n87_), .O(new_n94_));
  and2   g60(.a(new_n94_), .b(new_n63_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n92_), .c(new_n77_), .d(new_n76_), .O(z08));
  nand4  g62(.a(new_n58_), .b(x04), .c(new_n47_), .d(new_n38_), .O(new_n97_));
  nor3   g63(.a(x13), .b(x12), .c(x11), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n81_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n97_), .c(new_n38_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x02), .O(new_n101_));
  nand4  g67(.a(new_n66_), .b(x07), .c(x06), .d(x04), .O(new_n102_));
  inv1   g68(.a(x12), .O(new_n103_));
  inv1   g69(.a(x13), .O(new_n104_));
  nand4  g70(.a(new_n88_), .b(new_n48_), .c(new_n104_), .d(new_n103_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n102_), .c(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n63_), .O(new_n107_));
  aoi21  g73(.a(new_n94_), .b(x13), .c(new_n72_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(new_n107_), .c(new_n101_), .O(z09));
  inv1   g75(.a(x00), .O(new_n110_));
  nand3  g76(.a(new_n88_), .b(new_n104_), .c(new_n103_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g78(.a(new_n112_), .b(new_n51_), .c(new_n86_), .d(x08), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(x07), .c(x06), .O(new_n115_));
  nand2  g81(.a(new_n70_), .b(x02), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(x16), .c(x14), .O(new_n117_));
  aoi21  g83(.a(new_n117_), .b(new_n115_), .c(new_n43_), .O(z10));
  nor2   g84(.a(new_n35_), .b(x01), .O(z11));
  oai21  g85(.a(x16), .b(x03), .c(new_n35_), .O(new_n120_));
  aoi21  g86(.a(new_n120_), .b(new_n70_), .c(new_n43_), .O(z12));
  nand2  g87(.a(new_n63_), .b(new_n43_), .O(z13));
  nand3  g88(.a(new_n63_), .b(x17), .c(x04), .O(new_n123_));
  inv1   g89(.a(new_n123_), .O(z14));
endmodule


