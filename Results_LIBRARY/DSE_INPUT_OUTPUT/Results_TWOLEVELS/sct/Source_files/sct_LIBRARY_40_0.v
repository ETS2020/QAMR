// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x17), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  inv1   g06(.a(x01), .O(new_n41_));
  inv1   g07(.a(x14), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n40_), .c(new_n38_), .O(z00));
  inv1   g10(.a(x05), .O(new_n45_));
  inv1   g11(.a(x18), .O(new_n46_));
  oai21  g12(.a(new_n46_), .b(x15), .c(new_n45_), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  nand2  g14(.a(x05), .b(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(new_n47_), .c(new_n38_), .O(z01));
  oai21  g16(.a(x03), .b(new_n39_), .c(x16), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(x17), .c(new_n35_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand3  g19(.a(x17), .b(x07), .c(new_n35_), .O(new_n54_));
  oai21  g20(.a(x07), .b(new_n35_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n51_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand3  g24(.a(x17), .b(x08), .c(new_n35_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n35_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x16), .O(new_n63_));
  aoi21  g29(.a(x04), .b(new_n39_), .c(x03), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n63_), .c(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n37_), .O(new_n66_));
  oai21  g32(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n67_));
  nand2  g33(.a(x08), .b(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n69_), .c(new_n67_), .d(new_n66_), .O(z05));
  inv1   g38(.a(x10), .O(new_n73_));
  nand3  g39(.a(new_n70_), .b(x08), .c(x07), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  inv1   g41(.a(new_n68_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(new_n77_));
  oai21  g43(.a(new_n75_), .b(new_n73_), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand3  g45(.a(x17), .b(x10), .c(new_n35_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n66_), .O(z06));
  and2   g47(.a(new_n77_), .b(x11), .O(new_n82_));
  nor4   g48(.a(new_n68_), .b(x11), .c(x10), .d(x09), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n82_), .c(x06), .O(new_n84_));
  nand3  g50(.a(x17), .b(x11), .c(new_n35_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n66_), .O(z07));
  oai21  g52(.a(new_n36_), .b(x12), .c(new_n35_), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nor2   g54(.a(new_n83_), .b(new_n88_), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  nand4  g56(.a(new_n88_), .b(new_n90_), .c(new_n73_), .d(new_n70_), .O(new_n91_));
  nor3   g57(.a(new_n91_), .b(new_n68_), .c(new_n35_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n87_), .c(new_n66_), .O(z08));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n51_), .b(new_n95_), .c(new_n88_), .d(new_n90_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n73_), .c(new_n70_), .d(x08), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x07), .c(x06), .d(x04), .O(new_n100_));
  aoi21  g66(.a(x17), .b(new_n95_), .c(x06), .O(new_n101_));
  nand4  g67(.a(new_n75_), .b(new_n88_), .c(new_n90_), .d(new_n73_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x13), .c(new_n101_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n100_), .c(new_n66_), .O(z09));
  inv1   g70(.a(x07), .O(new_n105_));
  inv1   g71(.a(x00), .O(new_n106_));
  nand4  g72(.a(new_n95_), .b(new_n88_), .c(new_n90_), .d(new_n73_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n51_), .c(new_n70_), .d(x08), .O(new_n109_));
  oai22  g75(.a(new_n109_), .b(new_n105_), .c(new_n51_), .d(new_n42_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x06), .O(new_n111_));
  inv1   g77(.a(x03), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x02), .O(new_n113_));
  nand4  g79(.a(new_n113_), .b(x17), .c(x16), .d(x14), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n111_), .c(new_n48_), .O(z10));
  nand2  g81(.a(new_n37_), .b(new_n39_), .O(z11));
  nor3   g82(.a(new_n38_), .b(new_n63_), .c(x02), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(x03), .c(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n37_), .O(z12));
  nand2  g85(.a(new_n37_), .b(new_n48_), .O(z13));
  nor2   g86(.a(new_n36_), .b(new_n48_), .O(z14));
endmodule


