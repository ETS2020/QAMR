// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  oai21  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  nand2  g09(.a(new_n36_), .b(x01), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z01));
  inv1   g11(.a(x01), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(x06), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n44_), .O(z03));
  inv1   g18(.a(x07), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n36_), .c(x08), .O(new_n54_));
  inv1   g20(.a(x08), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(x07), .c(x06), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n47_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n44_), .O(z04));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(x04), .b(new_n35_), .c(x03), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n44_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nor2   g30(.a(new_n55_), .b(new_n53_), .O(new_n65_));
  nand3  g31(.a(new_n64_), .b(x08), .c(x07), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(x09), .b(new_n36_), .c(new_n46_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(z05));
  nand2  g36(.a(new_n66_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand3  g38(.a(new_n65_), .b(new_n72_), .c(new_n64_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand3  g41(.a(x10), .b(new_n36_), .c(new_n46_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n63_), .O(z06));
  and2   g43(.a(new_n73_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand4  g45(.a(new_n65_), .b(new_n79_), .c(new_n72_), .d(new_n64_), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n78_), .c(x06), .O(new_n82_));
  nand3  g48(.a(x11), .b(new_n36_), .c(new_n46_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n63_), .O(z07));
  oai21  g50(.a(x12), .b(x01), .c(new_n36_), .O(new_n85_));
  nand2  g51(.a(new_n65_), .b(x06), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n79_), .c(new_n72_), .d(new_n64_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  aoi21  g55(.a(new_n80_), .b(x12), .c(new_n89_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n85_), .c(new_n63_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n47_), .b(new_n92_), .c(new_n87_), .d(new_n79_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(x10), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n64_), .c(x08), .d(x07), .O(new_n95_));
  nand3  g61(.a(new_n87_), .b(new_n79_), .c(new_n72_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n66_), .c(x13), .O(new_n97_));
  oai21  g63(.a(new_n95_), .b(new_n42_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x06), .O(new_n99_));
  nand3  g65(.a(x13), .b(new_n36_), .c(new_n46_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n99_), .c(new_n63_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand4  g68(.a(new_n92_), .b(new_n87_), .c(new_n79_), .d(new_n72_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n47_), .c(new_n64_), .d(x08), .O(new_n105_));
  inv1   g71(.a(new_n47_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x14), .O(new_n107_));
  oai21  g73(.a(new_n105_), .b(new_n53_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x06), .O(new_n109_));
  nand3  g75(.a(new_n106_), .b(x14), .c(new_n46_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n42_), .O(z10));
  nand2  g77(.a(new_n44_), .b(new_n35_), .O(z11));
  inv1   g78(.a(x03), .O(new_n113_));
  oai21  g79(.a(new_n60_), .b(x02), .c(new_n113_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n44_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  nand2  g82(.a(new_n44_), .b(new_n42_), .O(z13));
  nand2  g83(.a(x17), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n44_), .O(z14));
endmodule


