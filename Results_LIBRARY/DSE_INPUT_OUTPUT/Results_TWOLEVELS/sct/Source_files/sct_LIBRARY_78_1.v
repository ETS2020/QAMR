// Benchmark "FAU" written by ABC on Wed Aug 19 15:40:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n116_, new_n119_;
  inv1   g00(.a(x07), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x05), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x15), .O(new_n41_));
  nand2  g07(.a(x18), .b(new_n41_), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(new_n36_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n37_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n47_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n47_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  nor2   g21(.a(new_n35_), .b(new_n48_), .O(new_n56_));
  nand3  g22(.a(new_n55_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n49_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n47_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n37_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n47_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nor2   g31(.a(new_n55_), .b(new_n48_), .O(new_n66_));
  nand3  g32(.a(new_n56_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g33(.a(new_n66_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  aoi22  g34(.a(new_n68_), .b(x05), .c(x09), .d(new_n35_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n64_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  nand3  g37(.a(new_n65_), .b(x08), .c(x06), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n56_), .c(x08), .O(new_n75_));
  oai21  g41(.a(new_n73_), .b(new_n71_), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x05), .O(new_n77_));
  nand2  g43(.a(x10), .b(new_n35_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n64_), .O(z06));
  inv1   g45(.a(x11), .O(new_n80_));
  aoi21  g46(.a(new_n74_), .b(new_n66_), .c(new_n80_), .O(new_n81_));
  nand3  g47(.a(x08), .b(x07), .c(x06), .O(new_n82_));
  nor4   g48(.a(new_n82_), .b(x11), .c(x10), .d(x09), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n81_), .c(x05), .O(new_n84_));
  nand2  g50(.a(x11), .b(new_n35_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n64_), .O(z07));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n73_), .b(new_n87_), .c(new_n80_), .d(new_n71_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x05), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x07), .O(new_n90_));
  inv1   g56(.a(new_n83_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x12), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n90_), .c(new_n64_), .O(z08));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n49_), .b(new_n94_), .c(new_n87_), .d(new_n80_), .O(new_n95_));
  nor3   g61(.a(new_n95_), .b(x10), .c(x09), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  nand2  g63(.a(new_n88_), .b(x13), .O(new_n98_));
  oai21  g64(.a(new_n97_), .b(new_n44_), .c(new_n98_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x05), .O(new_n100_));
  nand2  g66(.a(x13), .b(new_n35_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n100_), .c(new_n64_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  inv1   g69(.a(new_n49_), .O(new_n104_));
  nand4  g70(.a(new_n94_), .b(new_n87_), .c(new_n80_), .d(new_n71_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n103_), .c(new_n104_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n65_), .c(x08), .d(x07), .O(new_n107_));
  nand2  g73(.a(new_n104_), .b(x14), .O(new_n108_));
  oai21  g74(.a(new_n107_), .b(new_n48_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x05), .O(new_n110_));
  nand3  g76(.a(new_n104_), .b(x14), .c(new_n35_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n44_), .O(z10));
  nor2   g78(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g79(.a(x03), .O(new_n114_));
  oai21  g80(.a(new_n61_), .b(x02), .c(new_n114_), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(new_n47_), .c(x04), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(z12));
  nand2  g83(.a(new_n47_), .b(new_n44_), .O(z13));
  nand2  g84(.a(x17), .b(x04), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(new_n47_), .O(z14));
endmodule


