// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n116_;
  nor2   g00(.a(x15), .b(x10), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(z00));
  aoi21  g07(.a(x10), .b(x05), .c(x15), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(x10), .c(x15), .O(new_n44_));
  oai22  g10(.a(new_n44_), .b(x05), .c(new_n42_), .d(x04), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(new_n35_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n54_), .b(x07), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n54_), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(x04), .b(new_n36_), .c(x03), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n47_), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n56_), .b(new_n67_), .c(x08), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n66_), .c(new_n64_), .d(new_n47_), .O(z05));
  nand3  g35(.a(x08), .b(x07), .c(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n48_), .c(x04), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(new_n65_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x15), .c(new_n74_), .d(new_n67_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n73_), .c(new_n64_), .O(z06));
  inv1   g43(.a(x11), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n67_), .c(x08), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n55_), .c(x15), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand3  g47(.a(new_n74_), .b(new_n67_), .c(x08), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n55_), .c(x11), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n64_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n78_), .c(new_n67_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n65_), .c(x15), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n74_), .O(new_n88_));
  nand3  g54(.a(new_n78_), .b(new_n74_), .c(new_n67_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n65_), .c(x12), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n64_), .O(z08));
  inv1   g57(.a(x04), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n48_), .b(new_n93_), .c(new_n85_), .d(new_n78_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n92_), .c(x15), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n74_), .O(new_n98_));
  nand4  g64(.a(new_n85_), .b(new_n78_), .c(new_n74_), .d(new_n67_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n65_), .c(x13), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n98_), .c(new_n64_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  inv1   g68(.a(x03), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(x02), .c(new_n61_), .O(new_n104_));
  nand4  g70(.a(new_n93_), .b(new_n85_), .c(new_n78_), .d(new_n74_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n102_), .c(new_n104_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n67_), .c(x08), .d(x07), .O(new_n107_));
  oai22  g73(.a(new_n107_), .b(new_n46_), .c(new_n48_), .d(new_n39_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n47_), .O(z10));
  nor2   g76(.a(new_n35_), .b(new_n36_), .O(z11));
  nor3   g77(.a(new_n35_), .b(new_n61_), .c(x02), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(x03), .c(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n47_), .O(z12));
  nand2  g80(.a(new_n47_), .b(new_n92_), .O(z13));
  nand2  g81(.a(x17), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n47_), .O(z14));
endmodule


