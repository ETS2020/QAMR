// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n113_;
  inv1   g00(.a(x01), .O(new_n35_));
  aoi21  g01(.a(x14), .b(new_n35_), .c(x02), .O(z00));
  inv1   g02(.a(x18), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x15), .O(new_n38_));
  inv1   g04(.a(x04), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x01), .O(new_n41_));
  aoi21  g07(.a(x05), .b(new_n39_), .c(new_n41_), .O(new_n42_));
  oai21  g08(.a(new_n38_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g09(.a(x03), .O(new_n44_));
  inv1   g10(.a(x06), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(x04), .c(new_n44_), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x01), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n45_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(z02));
  inv1   g17(.a(new_n41_), .O(new_n52_));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  aoi21  g19(.a(new_n44_), .b(x02), .c(new_n49_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n52_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n58_), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  oai22  g27(.a(new_n54_), .b(new_n35_), .c(x16), .d(x02), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  oai21  g30(.a(new_n49_), .b(new_n44_), .c(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x01), .O(new_n66_));
  oai21  g32(.a(x16), .b(new_n39_), .c(new_n40_), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n59_), .b(new_n70_), .c(x08), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n41_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n69_), .c(new_n67_), .d(new_n66_), .O(z05));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n59_), .c(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  aoi21  g43(.a(new_n71_), .b(x10), .c(new_n77_), .O(new_n78_));
  oai22  g44(.a(new_n39_), .b(x02), .c(new_n44_), .d(new_n35_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x16), .O(new_n80_));
  oai21  g46(.a(new_n78_), .b(new_n41_), .c(new_n80_), .O(z06));
  inv1   g47(.a(x10), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  inv1   g49(.a(new_n68_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n70_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(new_n86_));
  aoi21  g52(.a(new_n76_), .b(x11), .c(new_n86_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n41_), .c(new_n80_), .O(z07));
  nand2  g54(.a(new_n85_), .b(x12), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand3  g56(.a(new_n75_), .b(new_n90_), .c(new_n83_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n84_), .c(new_n41_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n89_), .c(new_n67_), .d(new_n66_), .O(z08));
  oai21  g60(.a(new_n91_), .b(new_n68_), .c(x13), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n90_), .c(new_n83_), .d(new_n82_), .O(new_n97_));
  inv1   g63(.a(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n72_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n95_), .c(x04), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n52_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n80_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  aoi21  g69(.a(new_n97_), .b(new_n103_), .c(new_n54_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n70_), .c(x08), .d(x07), .O(new_n105_));
  nand2  g71(.a(new_n54_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n45_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n52_), .O(z10));
  aoi21  g75(.a(x16), .b(new_n40_), .c(x03), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n39_), .c(new_n52_), .O(z12));
  nand2  g77(.a(new_n52_), .b(new_n39_), .O(z13));
  inv1   g78(.a(x17), .O(new_n113_));
  nor3   g79(.a(new_n41_), .b(new_n113_), .c(new_n39_), .O(z14));
  buf    g80(.a(x02), .O(z11));
endmodule


