// Benchmark "FAU" written by ABC on Wed Aug 19 15:41:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand3  g01(.a(x15), .b(new_n35_), .c(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x04), .O(new_n38_));
  nor2   g04(.a(x18), .b(x05), .O(new_n39_));
  aoi21  g05(.a(x05), .b(new_n38_), .c(new_n39_), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x15), .O(new_n42_));
  oai21  g08(.a(new_n40_), .b(x01), .c(new_n42_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  inv1   g11(.a(x01), .O(new_n46_));
  nor2   g12(.a(x15), .b(new_n46_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n45_), .c(new_n44_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n48_), .c(new_n45_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n45_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n48_), .O(z04));
  inv1   g25(.a(x03), .O(new_n60_));
  oai21  g26(.a(new_n38_), .b(x02), .c(new_n60_), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(x16), .c(new_n38_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n55_), .b(new_n65_), .c(x08), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n64_), .c(new_n62_), .d(new_n48_), .O(z05));
  nand2  g33(.a(new_n61_), .b(x16), .O(new_n68_));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n55_), .c(x08), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x04), .O(new_n71_));
  aoi21  g37(.a(new_n66_), .b(x10), .c(new_n71_), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n68_), .c(new_n47_), .O(z06));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  inv1   g41(.a(new_n63_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n65_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x04), .O(new_n78_));
  aoi21  g44(.a(new_n70_), .b(x11), .c(new_n78_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n68_), .c(new_n47_), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  nand4  g47(.a(new_n69_), .b(new_n76_), .c(new_n81_), .d(new_n75_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  aoi21  g49(.a(new_n77_), .b(x12), .c(new_n83_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n68_), .c(new_n47_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n45_), .b(new_n86_), .c(new_n81_), .d(new_n75_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x10), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n65_), .c(x08), .d(x07), .O(new_n89_));
  nand2  g55(.a(x16), .b(new_n35_), .O(new_n90_));
  oai21  g56(.a(new_n89_), .b(new_n44_), .c(new_n90_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x04), .O(new_n92_));
  aoi21  g58(.a(x16), .b(x03), .c(new_n38_), .O(new_n93_));
  nand2  g59(.a(new_n82_), .b(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(new_n48_), .O(z09));
  inv1   g61(.a(x14), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand4  g63(.a(new_n86_), .b(new_n81_), .c(new_n75_), .d(new_n74_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  and2   g65(.a(new_n99_), .b(new_n45_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n65_), .c(x08), .d(x07), .O(new_n101_));
  oai22  g67(.a(new_n101_), .b(new_n44_), .c(new_n45_), .d(new_n96_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n48_), .c(x04), .O(new_n103_));
  inv1   g69(.a(new_n103_), .O(z10));
  nand2  g70(.a(new_n48_), .b(new_n35_), .O(z11));
  inv1   g71(.a(x16), .O(new_n106_));
  nor2   g72(.a(new_n47_), .b(new_n106_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n35_), .c(x03), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n38_), .c(new_n48_), .O(z12));
  nor2   g75(.a(new_n47_), .b(new_n38_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n48_), .O(z14));
endmodule


