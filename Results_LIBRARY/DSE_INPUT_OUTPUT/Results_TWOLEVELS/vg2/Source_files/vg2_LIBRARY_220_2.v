// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n168_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x01), .O(new_n35_));
  inv1   g002(.a(x03), .O(new_n36_));
  inv1   g003(.a(x06), .O(new_n37_));
  inv1   g004(.a(x20), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand3  g012(.a(new_n41_), .b(new_n45_), .c(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n39_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n44_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n43_), .c(new_n38_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n52_));
  aoi21  g019(.a(new_n52_), .b(new_n34_), .c(x11), .O(z0));
  nand3  g020(.a(x04), .b(x03), .c(x01), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand2  g024(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand4  g025(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n59_));
  nand4  g026(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n60_));
  or2    g027(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g028(.a(x09), .O(new_n62_));
  inv1   g029(.a(x11), .O(new_n63_));
  nor3   g030(.a(x04), .b(x03), .c(x01), .O(new_n64_));
  nor2   g031(.a(x08), .b(x06), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(new_n66_));
  nor2   g033(.a(x14), .b(x12), .O(new_n67_));
  nor2   g034(.a(x17), .b(x16), .O(new_n68_));
  inv1   g035(.a(x19), .O(new_n69_));
  nor2   g036(.a(x20), .b(new_n69_), .O(new_n70_));
  nor2   g037(.a(x23), .b(x22), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n68_), .d(new_n67_), .O(new_n72_));
  oai22  g039(.a(new_n72_), .b(new_n66_), .c(new_n61_), .d(new_n58_), .O(new_n73_));
  nand2  g040(.a(new_n47_), .b(new_n39_), .O(new_n74_));
  nand3  g041(.a(new_n41_), .b(x13), .c(x05), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  inv1   g044(.a(x22), .O(new_n78_));
  inv1   g045(.a(x23), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g047(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n80_), .c(x20), .d(x18), .O(new_n83_));
  nor2   g050(.a(x09), .b(x08), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n64_), .c(x07), .d(new_n37_), .O(new_n85_));
  nor2   g052(.a(x12), .b(x11), .O(new_n86_));
  nor2   g053(.a(x16), .b(x14), .O(new_n87_));
  nor2   g054(.a(x20), .b(x17), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n71_), .O(new_n89_));
  oai22  g056(.a(new_n89_), .b(new_n85_), .c(new_n83_), .d(new_n58_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n74_), .c(x24), .O(new_n91_));
  nor2   g058(.a(x02), .b(new_n35_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n57_), .c(x04), .d(x03), .O(new_n93_));
  nand4  g060(.a(new_n82_), .b(new_n80_), .c(x21), .d(x20), .O(new_n94_));
  nor2   g061(.a(x03), .b(x01), .O(new_n95_));
  nor2   g062(.a(x06), .b(x04), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n95_), .c(new_n84_), .d(x00), .O(new_n97_));
  oai22  g064(.a(new_n97_), .b(new_n89_), .c(new_n94_), .d(new_n93_), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n41_), .c(new_n45_), .O(new_n99_));
  nand2  g066(.a(new_n63_), .b(x02), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n99_), .c(new_n91_), .d(new_n77_), .O(z1));
  inv1   g068(.a(x14), .O(new_n102_));
  nand2  g069(.a(x24), .b(x18), .O(new_n103_));
  nand3  g070(.a(new_n41_), .b(x15), .c(x13), .O(new_n104_));
  aoi21  g071(.a(new_n104_), .b(new_n103_), .c(new_n39_), .O(new_n105_));
  nand3  g072(.a(x24), .b(x18), .c(x13), .O(new_n106_));
  nand3  g073(.a(x15), .b(new_n47_), .c(new_n39_), .O(new_n107_));
  nand4  g074(.a(new_n41_), .b(x21), .c(new_n45_), .d(new_n34_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  oai21  g076(.a(new_n109_), .b(new_n105_), .c(x20), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n102_), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(x11), .c(x06), .d(x03), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n35_), .O(z2));
  nand4  g080(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n114_));
  nand4  g081(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n115_));
  nand2  g082(.a(new_n95_), .b(new_n65_), .O(new_n116_));
  nand3  g083(.a(new_n70_), .b(new_n102_), .c(new_n63_), .O(new_n117_));
  oai22  g084(.a(new_n117_), .b(new_n116_), .c(new_n115_), .d(new_n114_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n76_), .O(new_n119_));
  nand4  g086(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n120_));
  nand3  g087(.a(new_n95_), .b(x07), .c(new_n37_), .O(new_n121_));
  inv1   g088(.a(x08), .O(new_n122_));
  nand4  g089(.a(new_n38_), .b(new_n102_), .c(new_n63_), .d(new_n122_), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n121_), .c(new_n120_), .d(new_n114_), .O(new_n124_));
  nand3  g091(.a(new_n124_), .b(new_n74_), .c(x24), .O(new_n125_));
  nand4  g092(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x00), .O(new_n126_));
  nand3  g093(.a(x21), .b(x20), .c(x14), .O(new_n127_));
  nor3   g094(.a(new_n127_), .b(new_n63_), .c(new_n122_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n92_), .c(x06), .d(x03), .O(new_n129_));
  oai21  g096(.a(new_n126_), .b(new_n123_), .c(new_n129_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n41_), .c(new_n45_), .O(new_n131_));
  nand4  g098(.a(new_n131_), .b(new_n125_), .c(new_n119_), .d(new_n100_), .O(z3));
  inv1   g099(.a(x04), .O(new_n133_));
  inv1   g100(.a(x17), .O(new_n134_));
  oai21  g101(.a(x23), .b(new_n133_), .c(new_n134_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n78_), .c(x09), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x16), .c(new_n122_), .O(new_n137_));
  nand4  g104(.a(new_n41_), .b(new_n45_), .c(new_n34_), .d(x00), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n48_), .c(new_n44_), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(new_n43_), .c(new_n137_), .O(new_n140_));
  inv1   g107(.a(x16), .O(new_n141_));
  oai21  g108(.a(new_n79_), .b(x04), .c(x17), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(x22), .c(new_n62_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n141_), .c(x08), .O(new_n144_));
  oai21  g111(.a(new_n109_), .b(new_n105_), .c(new_n144_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n140_), .c(new_n100_), .O(z4));
  nand2  g113(.a(new_n48_), .b(new_n44_), .O(new_n147_));
  oai21  g114(.a(new_n147_), .b(new_n43_), .c(new_n100_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n138_), .O(z5));
  nor2   g116(.a(new_n49_), .b(new_n43_), .O(new_n150_));
  aoi21  g117(.a(x20), .b(new_n102_), .c(x06), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(new_n150_), .c(new_n34_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n63_), .O(new_n153_));
  inv1   g120(.a(x18), .O(new_n154_));
  oai21  g121(.a(x20), .b(new_n102_), .c(x06), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(x11), .c(new_n36_), .O(new_n156_));
  nand2  g123(.a(x07), .b(x03), .O(new_n157_));
  oai21  g124(.a(new_n156_), .b(new_n154_), .c(new_n157_), .O(new_n158_));
  nand3  g125(.a(new_n158_), .b(new_n74_), .c(x24), .O(new_n159_));
  inv1   g126(.a(x15), .O(new_n160_));
  oai22  g127(.a(new_n156_), .b(new_n160_), .c(new_n69_), .d(new_n36_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n76_), .O(new_n162_));
  inv1   g129(.a(x21), .O(new_n163_));
  nand2  g130(.a(x03), .b(x00), .O(new_n164_));
  oai21  g131(.a(new_n156_), .b(new_n163_), .c(new_n164_), .O(new_n165_));
  nand4  g132(.a(new_n165_), .b(new_n41_), .c(new_n45_), .d(new_n34_), .O(new_n166_));
  nand4  g133(.a(new_n166_), .b(new_n162_), .c(new_n159_), .d(new_n153_), .O(z6));
  nand4  g134(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n100_), .O(new_n168_));
  or2    g135(.a(new_n168_), .b(new_n105_), .O(z7));
endmodule


