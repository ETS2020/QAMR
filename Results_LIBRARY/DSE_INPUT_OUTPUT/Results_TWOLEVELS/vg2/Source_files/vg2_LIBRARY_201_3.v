// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n168_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  nand2  g003(.a(x24), .b(x07), .O(new_n37_));
  inv1   g004(.a(x24), .O(new_n38_));
  nand3  g005(.a(new_n38_), .b(x19), .c(x13), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand3  g007(.a(x24), .b(x13), .c(x07), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  nand4  g010(.a(new_n38_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n44_));
  inv1   g011(.a(x05), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n45_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n44_), .c(new_n41_), .O(new_n48_));
  aoi21  g015(.a(new_n40_), .b(x05), .c(new_n48_), .O(new_n49_));
  nor3   g016(.a(new_n49_), .b(x20), .c(x14), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n51_));
  nand2  g018(.a(x23), .b(x02), .O(new_n52_));
  oai21  g019(.a(new_n51_), .b(x01), .c(new_n52_), .O(z0));
  nand2  g020(.a(x04), .b(x03), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n55_), .c(new_n42_), .d(x01), .O(new_n58_));
  nand4  g025(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  and2   g027(.a(x23), .b(x22), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n60_), .c(x20), .d(x17), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nor3   g030(.a(x04), .b(x03), .c(x01), .O(new_n64_));
  nor2   g031(.a(x08), .b(x06), .O(new_n65_));
  nor2   g032(.a(x11), .b(x09), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  inv1   g034(.a(x12), .O(new_n68_));
  inv1   g035(.a(x14), .O(new_n69_));
  inv1   g036(.a(x16), .O(new_n70_));
  inv1   g037(.a(x17), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  inv1   g039(.a(x20), .O(new_n73_));
  nor2   g040(.a(x23), .b(x22), .O(new_n74_));
  nand3  g041(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nor3   g042(.a(new_n75_), .b(new_n72_), .c(new_n67_), .O(new_n76_));
  nor2   g043(.a(x13), .b(x05), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  nand3  g045(.a(new_n38_), .b(x13), .c(x05), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  oai21  g047(.a(new_n76_), .b(new_n63_), .c(new_n80_), .O(new_n81_));
  nand4  g048(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n82_));
  inv1   g049(.a(new_n82_), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n61_), .c(x20), .d(x18), .O(new_n84_));
  nor2   g051(.a(x09), .b(x08), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n64_), .c(x07), .d(new_n35_), .O(new_n86_));
  nand4  g053(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n36_), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n74_), .c(new_n73_), .d(new_n71_), .O(new_n89_));
  oai22  g056(.a(new_n89_), .b(new_n86_), .c(new_n84_), .d(new_n58_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n78_), .c(x24), .O(new_n91_));
  nand3  g058(.a(new_n61_), .b(x21), .c(x20), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(new_n82_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n57_), .c(new_n55_), .d(x01), .O(new_n94_));
  nor2   g061(.a(x03), .b(x01), .O(new_n95_));
  nor2   g062(.a(x06), .b(x04), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n95_), .c(new_n85_), .d(x00), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n89_), .c(new_n94_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n38_), .c(new_n43_), .d(new_n42_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n91_), .c(new_n81_), .O(z1));
  inv1   g067(.a(x01), .O(new_n101_));
  nand2  g068(.a(x24), .b(x18), .O(new_n102_));
  nand3  g069(.a(new_n38_), .b(x15), .c(x13), .O(new_n103_));
  aoi21  g070(.a(new_n103_), .b(new_n102_), .c(new_n45_), .O(new_n104_));
  nand3  g071(.a(x24), .b(x18), .c(x13), .O(new_n105_));
  nand3  g072(.a(x15), .b(new_n46_), .c(new_n45_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  oai21  g074(.a(new_n107_), .b(new_n104_), .c(new_n52_), .O(new_n108_));
  nand4  g075(.a(new_n38_), .b(x21), .c(new_n43_), .d(new_n42_), .O(new_n109_));
  aoi21  g076(.a(new_n109_), .b(new_n108_), .c(new_n73_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(x14), .c(x11), .d(x06), .O(new_n111_));
  nor3   g078(.a(new_n111_), .b(new_n34_), .c(new_n101_), .O(z2));
  inv1   g079(.a(x08), .O(new_n113_));
  nand3  g080(.a(x15), .b(x13), .c(x05), .O(new_n114_));
  nand3  g081(.a(x21), .b(new_n43_), .c(new_n42_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x20), .c(x14), .d(x11), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x06), .c(x03), .d(x01), .O(new_n119_));
  nand3  g086(.a(new_n43_), .b(new_n42_), .c(x00), .O(new_n120_));
  nand3  g087(.a(x19), .b(x13), .c(x05), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(new_n73_), .c(new_n69_), .d(new_n36_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(x08), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n35_), .c(new_n34_), .d(new_n101_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n119_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n38_), .O(new_n127_));
  nand4  g094(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n128_));
  nand4  g095(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n129_));
  nand3  g096(.a(new_n95_), .b(x07), .c(new_n35_), .O(new_n130_));
  nand4  g097(.a(new_n73_), .b(new_n69_), .c(new_n36_), .d(new_n113_), .O(new_n131_));
  oai22  g098(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n128_), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n78_), .c(x24), .O(new_n133_));
  nand4  g100(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n134_));
  nand2  g101(.a(new_n95_), .b(new_n65_), .O(new_n135_));
  nand4  g102(.a(new_n73_), .b(x19), .c(new_n69_), .d(new_n36_), .O(new_n136_));
  oai22  g103(.a(new_n136_), .b(new_n135_), .c(new_n134_), .d(new_n128_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n46_), .c(new_n45_), .O(new_n138_));
  nand4  g105(.a(new_n138_), .b(new_n133_), .c(new_n127_), .d(new_n52_), .O(z3));
  inv1   g106(.a(new_n49_), .O(new_n140_));
  inv1   g107(.a(x22), .O(new_n141_));
  inv1   g108(.a(x04), .O(new_n142_));
  oai21  g109(.a(x23), .b(new_n142_), .c(new_n71_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n141_), .c(x09), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x16), .c(new_n113_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n140_), .O(new_n146_));
  nand2  g113(.a(x22), .b(new_n71_), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(x09), .c(new_n70_), .O(new_n148_));
  inv1   g115(.a(new_n104_), .O(new_n149_));
  nand4  g116(.a(new_n109_), .b(new_n106_), .c(new_n105_), .d(new_n149_), .O(new_n150_));
  oai21  g117(.a(new_n148_), .b(new_n113_), .c(new_n150_), .O(new_n151_));
  nand3  g118(.a(new_n38_), .b(x21), .c(new_n43_), .O(new_n152_));
  nand4  g119(.a(new_n152_), .b(new_n106_), .c(new_n105_), .d(new_n149_), .O(new_n153_));
  nand4  g120(.a(new_n153_), .b(x22), .c(x16), .d(new_n142_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n42_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(x23), .O(new_n156_));
  nand3  g123(.a(new_n156_), .b(new_n151_), .c(new_n146_), .O(z4));
  nand2  g124(.a(new_n40_), .b(x05), .O(new_n158_));
  and2   g125(.a(new_n52_), .b(new_n47_), .O(new_n159_));
  nand4  g126(.a(new_n159_), .b(new_n44_), .c(new_n41_), .d(new_n158_), .O(z5));
  nand2  g127(.a(x20), .b(new_n69_), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n35_), .c(x11), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x03), .c(new_n140_), .O(new_n163_));
  nand2  g130(.a(new_n73_), .b(x14), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(x06), .c(new_n36_), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(new_n34_), .c(new_n150_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n163_), .c(new_n52_), .O(z6));
  inv1   g134(.a(new_n107_), .O(new_n168_));
  nand4  g135(.a(new_n109_), .b(new_n168_), .c(new_n149_), .d(new_n52_), .O(z7));
endmodule


