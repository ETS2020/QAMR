// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n180_, new_n181_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x22), .O(new_n35_));
  inv1   g002(.a(x01), .O(new_n36_));
  inv1   g003(.a(x03), .O(new_n37_));
  inv1   g004(.a(x06), .O(new_n38_));
  nand4  g005(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x00), .O(new_n39_));
  inv1   g006(.a(x14), .O(new_n40_));
  inv1   g007(.a(x20), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nor2   g009(.a(x11), .b(x10), .O(new_n43_));
  nand4  g010(.a(new_n43_), .b(new_n42_), .c(new_n41_), .d(new_n40_), .O(new_n44_));
  oai21  g011(.a(new_n44_), .b(new_n39_), .c(new_n35_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  inv1   g013(.a(x11), .O(new_n47_));
  inv1   g014(.a(x07), .O(new_n48_));
  nand3  g015(.a(new_n42_), .b(x19), .c(x13), .O(new_n49_));
  oai21  g016(.a(new_n42_), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  nand3  g017(.a(x24), .b(x13), .c(x07), .O(new_n51_));
  inv1   g018(.a(x05), .O(new_n52_));
  inv1   g019(.a(x13), .O(new_n53_));
  nand3  g020(.a(x19), .b(new_n53_), .c(new_n52_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  aoi21  g022(.a(new_n50_), .b(x05), .c(new_n55_), .O(new_n56_));
  nor3   g023(.a(new_n56_), .b(x20), .c(x14), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n47_), .c(new_n38_), .d(new_n37_), .O(new_n58_));
  oai21  g025(.a(new_n58_), .b(x01), .c(new_n46_), .O(z0));
  nand4  g026(.a(x04), .b(x03), .c(x02), .d(x01), .O(new_n60_));
  nand4  g027(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g029(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  inv1   g031(.a(x17), .O(new_n65_));
  nor2   g032(.a(new_n41_), .b(new_n65_), .O(new_n66_));
  and2   g033(.a(x23), .b(x22), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n64_), .d(new_n62_), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  nor3   g036(.a(x04), .b(x03), .c(x01), .O(new_n70_));
  nor2   g037(.a(x08), .b(x06), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n47_), .d(new_n69_), .O(new_n72_));
  nor2   g039(.a(x14), .b(x12), .O(new_n73_));
  nor2   g040(.a(x17), .b(x16), .O(new_n74_));
  inv1   g041(.a(x19), .O(new_n75_));
  nor2   g042(.a(x20), .b(new_n75_), .O(new_n76_));
  nor2   g043(.a(x23), .b(x22), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n76_), .c(new_n74_), .d(new_n73_), .O(new_n78_));
  oai21  g045(.a(new_n78_), .b(new_n72_), .c(new_n68_), .O(new_n79_));
  nor2   g046(.a(x13), .b(x05), .O(new_n80_));
  nor2   g047(.a(x24), .b(new_n53_), .O(new_n81_));
  aoi21  g048(.a(new_n81_), .b(x05), .c(new_n80_), .O(new_n82_));
  inv1   g049(.a(new_n82_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  inv1   g051(.a(new_n80_), .O(new_n85_));
  nand4  g052(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  and2   g054(.a(x20), .b(x18), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n87_), .c(new_n67_), .d(new_n62_), .O(new_n89_));
  inv1   g056(.a(x08), .O(new_n90_));
  nor2   g057(.a(new_n48_), .b(x06), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n70_), .c(new_n69_), .d(new_n90_), .O(new_n92_));
  inv1   g059(.a(x12), .O(new_n93_));
  inv1   g060(.a(x16), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n40_), .c(new_n93_), .d(new_n47_), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n77_), .c(new_n41_), .d(new_n65_), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n92_), .c(new_n89_), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n85_), .c(x24), .O(new_n99_));
  nand4  g066(.a(new_n37_), .b(new_n34_), .c(new_n36_), .d(x00), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  inv1   g068(.a(x10), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n69_), .c(new_n90_), .O(new_n103_));
  nor3   g070(.a(new_n103_), .b(x06), .c(x04), .O(new_n104_));
  nand2  g071(.a(new_n41_), .b(new_n65_), .O(new_n105_));
  nor4   g072(.a(new_n105_), .b(x24), .c(x23), .d(x22), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(new_n104_), .c(new_n101_), .d(new_n96_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n99_), .c(new_n84_), .O(z1));
  nand2  g075(.a(x22), .b(new_n34_), .O(new_n109_));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  nand3  g077(.a(new_n42_), .b(x15), .c(x13), .O(new_n111_));
  aoi21  g078(.a(new_n111_), .b(new_n110_), .c(new_n52_), .O(new_n112_));
  nand3  g079(.a(x24), .b(x18), .c(x13), .O(new_n113_));
  nand3  g080(.a(x15), .b(new_n53_), .c(new_n52_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n112_), .c(new_n109_), .O(new_n116_));
  nor2   g083(.a(x10), .b(x02), .O(new_n117_));
  nor2   g084(.a(x24), .b(x22), .O(new_n118_));
  nand3  g085(.a(new_n118_), .b(new_n117_), .c(x21), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(new_n116_), .c(new_n41_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x14), .c(x11), .d(x06), .O(new_n121_));
  nor3   g088(.a(new_n121_), .b(new_n37_), .c(new_n36_), .O(z2));
  nand4  g089(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n123_));
  nor2   g090(.a(new_n40_), .b(new_n47_), .O(new_n124_));
  nand3  g091(.a(new_n124_), .b(x20), .c(x15), .O(new_n125_));
  nand3  g092(.a(new_n71_), .b(new_n37_), .c(new_n36_), .O(new_n126_));
  nand3  g093(.a(new_n76_), .b(new_n40_), .c(new_n47_), .O(new_n127_));
  oai22  g094(.a(new_n127_), .b(new_n126_), .c(new_n125_), .d(new_n123_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n83_), .O(new_n129_));
  nand2  g096(.a(new_n124_), .b(new_n88_), .O(new_n130_));
  nand3  g097(.a(new_n91_), .b(new_n37_), .c(new_n36_), .O(new_n131_));
  nand4  g098(.a(new_n41_), .b(new_n40_), .c(new_n47_), .d(new_n90_), .O(new_n132_));
  oai22  g099(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n123_), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n85_), .c(x24), .O(new_n134_));
  nand3  g101(.a(new_n124_), .b(x21), .c(x20), .O(new_n135_));
  oai22  g102(.a(new_n135_), .b(new_n123_), .c(new_n132_), .d(new_n39_), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n42_), .c(new_n102_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n35_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n34_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n134_), .c(new_n129_), .O(z3));
  oai21  g107(.a(new_n94_), .b(x09), .c(x08), .O(new_n141_));
  nand3  g108(.a(new_n117_), .b(new_n42_), .c(x21), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n114_), .c(new_n113_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n112_), .c(new_n141_), .O(new_n144_));
  nand2  g111(.a(new_n117_), .b(x00), .O(new_n145_));
  nand4  g112(.a(new_n35_), .b(x19), .c(x13), .d(x05), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  inv1   g114(.a(x04), .O(new_n148_));
  oai21  g115(.a(x23), .b(new_n148_), .c(new_n65_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand3  g117(.a(x19), .b(x13), .c(x05), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n145_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(x09), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(new_n150_), .c(x24), .O(new_n154_));
  nand3  g121(.a(new_n85_), .b(x24), .c(x07), .O(new_n155_));
  nand2  g122(.a(new_n149_), .b(new_n35_), .O(new_n156_));
  aoi22  g123(.a(new_n156_), .b(new_n69_), .c(new_n155_), .d(new_n54_), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(new_n154_), .c(new_n94_), .O(new_n158_));
  nand4  g125(.a(new_n42_), .b(new_n102_), .c(x08), .d(x00), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n35_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n34_), .O(new_n161_));
  nand2  g128(.a(x23), .b(new_n148_), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(x17), .c(new_n35_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(x18), .c(x16), .O(new_n164_));
  nand2  g131(.a(x08), .b(x07), .O(new_n165_));
  aoi21  g132(.a(new_n165_), .b(new_n164_), .c(new_n80_), .O(new_n166_));
  nand3  g133(.a(new_n163_), .b(x16), .c(x15), .O(new_n167_));
  nand2  g134(.a(x19), .b(x08), .O(new_n168_));
  aoi21  g135(.a(new_n168_), .b(new_n167_), .c(new_n82_), .O(new_n169_));
  aoi21  g136(.a(new_n166_), .b(x24), .c(new_n169_), .O(new_n170_));
  nand4  g137(.a(new_n170_), .b(new_n161_), .c(new_n158_), .d(new_n144_), .O(z4));
  inv1   g138(.a(new_n109_), .O(new_n172_));
  nand4  g139(.a(new_n118_), .b(new_n102_), .c(new_n34_), .d(x00), .O(new_n173_));
  oai21  g140(.a(new_n172_), .b(new_n56_), .c(new_n173_), .O(z5));
  aoi21  g141(.a(x20), .b(new_n40_), .c(x06), .O(new_n175_));
  oai21  g142(.a(new_n175_), .b(x11), .c(new_n37_), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(z5), .O(new_n177_));
  nand2  g144(.a(new_n119_), .b(new_n116_), .O(z7));
  aoi21  g145(.a(new_n41_), .b(x14), .c(new_n38_), .O(new_n179_));
  oai21  g146(.a(new_n179_), .b(new_n47_), .c(x03), .O(new_n180_));
  nand2  g147(.a(new_n180_), .b(z7), .O(new_n181_));
  nand2  g148(.a(new_n181_), .b(new_n177_), .O(z6));
endmodule


