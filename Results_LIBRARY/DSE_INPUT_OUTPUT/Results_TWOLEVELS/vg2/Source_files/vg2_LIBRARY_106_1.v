// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n198_, new_n199_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x07), .O(new_n35_));
  inv1   g002(.a(x01), .O(new_n36_));
  inv1   g003(.a(x03), .O(new_n37_));
  inv1   g004(.a(x06), .O(new_n38_));
  nand4  g005(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x00), .O(new_n39_));
  inv1   g006(.a(x14), .O(new_n40_));
  inv1   g007(.a(x20), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nor2   g009(.a(x11), .b(x10), .O(new_n43_));
  nand4  g010(.a(new_n43_), .b(new_n42_), .c(new_n41_), .d(new_n40_), .O(new_n44_));
  oai21  g011(.a(new_n44_), .b(new_n39_), .c(new_n35_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  inv1   g013(.a(x11), .O(new_n47_));
  inv1   g014(.a(x05), .O(new_n48_));
  nand2  g015(.a(x24), .b(x07), .O(new_n49_));
  nand3  g016(.a(new_n42_), .b(x19), .c(x13), .O(new_n50_));
  aoi21  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand3  g018(.a(x24), .b(x13), .c(x07), .O(new_n52_));
  inv1   g019(.a(x13), .O(new_n53_));
  nand3  g020(.a(x19), .b(new_n53_), .c(new_n48_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  oai21  g022(.a(new_n55_), .b(new_n51_), .c(new_n41_), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(x14), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n47_), .c(new_n38_), .d(new_n37_), .O(new_n58_));
  oai21  g025(.a(new_n58_), .b(x01), .c(new_n46_), .O(z0));
  inv1   g026(.a(x12), .O(new_n60_));
  nand3  g027(.a(x15), .b(x13), .c(x05), .O(new_n61_));
  inv1   g028(.a(x10), .O(new_n62_));
  nand3  g029(.a(x21), .b(new_n62_), .c(new_n34_), .O(new_n63_));
  nand2  g030(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(x23), .c(x22), .d(x20), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(x17), .c(x16), .d(x14), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(new_n60_), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(x11), .c(x09), .d(x08), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n38_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(x04), .c(x03), .d(x01), .O(new_n71_));
  inv1   g038(.a(x04), .O(new_n72_));
  inv1   g039(.a(x08), .O(new_n73_));
  inv1   g040(.a(x09), .O(new_n74_));
  inv1   g041(.a(x16), .O(new_n75_));
  inv1   g042(.a(x17), .O(new_n76_));
  inv1   g043(.a(x22), .O(new_n77_));
  inv1   g044(.a(x23), .O(new_n78_));
  nand3  g045(.a(new_n62_), .b(new_n34_), .c(x00), .O(new_n79_));
  nand3  g046(.a(x19), .b(x13), .c(x05), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n78_), .c(new_n77_), .d(new_n41_), .O(new_n82_));
  inv1   g049(.a(new_n82_), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n76_), .c(new_n75_), .d(new_n40_), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(x12), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n47_), .c(new_n74_), .d(new_n73_), .O(new_n86_));
  nor2   g053(.a(new_n86_), .b(x06), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n72_), .c(new_n37_), .d(new_n36_), .O(new_n88_));
  aoi21  g055(.a(new_n88_), .b(new_n71_), .c(x24), .O(new_n89_));
  nand4  g056(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n90_));
  nor4   g057(.a(new_n90_), .b(new_n72_), .c(new_n37_), .d(new_n36_), .O(new_n91_));
  nand4  g058(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n92_));
  nor2   g059(.a(new_n78_), .b(new_n77_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(x20), .c(x17), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nor3   g063(.a(x04), .b(x03), .c(x01), .O(new_n97_));
  nor2   g064(.a(x08), .b(x06), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n97_), .c(new_n47_), .d(new_n74_), .O(new_n99_));
  nor2   g066(.a(x14), .b(x12), .O(new_n100_));
  nor2   g067(.a(x17), .b(x16), .O(new_n101_));
  inv1   g068(.a(x19), .O(new_n102_));
  nor2   g069(.a(x20), .b(new_n102_), .O(new_n103_));
  nor2   g070(.a(x23), .b(x22), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n103_), .c(new_n101_), .d(new_n100_), .O(new_n105_));
  oai21  g072(.a(new_n105_), .b(new_n99_), .c(new_n96_), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n53_), .c(new_n48_), .O(new_n107_));
  nand2  g074(.a(new_n53_), .b(new_n48_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x24), .c(x23), .d(x22), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n41_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(x18), .c(x17), .d(x16), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n40_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(x12), .c(x11), .d(x09), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n73_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(x06), .c(x04), .d(x03), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n36_), .c(new_n107_), .O(new_n116_));
  oai21  g083(.a(new_n116_), .b(new_n89_), .c(new_n35_), .O(new_n117_));
  nand3  g084(.a(new_n42_), .b(x13), .c(x05), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n108_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n106_), .O(new_n120_));
  nand4  g087(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n121_));
  inv1   g088(.a(new_n121_), .O(new_n122_));
  inv1   g089(.a(x18), .O(new_n123_));
  nor2   g090(.a(new_n41_), .b(new_n123_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n122_), .c(new_n93_), .d(new_n91_), .O(new_n125_));
  nor2   g092(.a(new_n35_), .b(x06), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(new_n97_), .c(new_n74_), .d(new_n73_), .O(new_n127_));
  nand4  g094(.a(new_n75_), .b(new_n40_), .c(new_n60_), .d(new_n47_), .O(new_n128_));
  inv1   g095(.a(new_n128_), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(new_n104_), .c(new_n41_), .d(new_n76_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n127_), .c(new_n125_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n108_), .c(x24), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n120_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(x02), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n117_), .O(z1));
  nand2  g102(.a(x07), .b(new_n34_), .O(new_n136_));
  nor2   g103(.a(new_n42_), .b(new_n123_), .O(new_n137_));
  inv1   g104(.a(new_n137_), .O(new_n138_));
  nand3  g105(.a(new_n42_), .b(x15), .c(x13), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(new_n138_), .c(new_n48_), .O(new_n140_));
  nand3  g107(.a(x15), .b(new_n53_), .c(new_n48_), .O(new_n141_));
  oai21  g108(.a(new_n138_), .b(new_n53_), .c(new_n141_), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(new_n140_), .c(new_n136_), .O(new_n143_));
  nand3  g110(.a(new_n42_), .b(x21), .c(new_n62_), .O(new_n144_));
  inv1   g111(.a(new_n144_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n35_), .c(new_n34_), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(new_n143_), .c(new_n41_), .O(new_n147_));
  nand4  g114(.a(new_n147_), .b(x14), .c(x11), .d(x06), .O(new_n148_));
  nor3   g115(.a(new_n148_), .b(new_n37_), .c(new_n36_), .O(z2));
  nand4  g116(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n150_));
  nor2   g117(.a(new_n40_), .b(new_n47_), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(x20), .c(x15), .O(new_n152_));
  nand3  g119(.a(new_n98_), .b(new_n37_), .c(new_n36_), .O(new_n153_));
  nand3  g120(.a(new_n103_), .b(new_n40_), .c(new_n47_), .O(new_n154_));
  oai22  g121(.a(new_n154_), .b(new_n153_), .c(new_n152_), .d(new_n150_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n119_), .O(new_n156_));
  nand2  g123(.a(new_n151_), .b(new_n124_), .O(new_n157_));
  nand3  g124(.a(new_n126_), .b(new_n37_), .c(new_n36_), .O(new_n158_));
  nand4  g125(.a(new_n41_), .b(new_n40_), .c(new_n47_), .d(new_n73_), .O(new_n159_));
  oai22  g126(.a(new_n159_), .b(new_n158_), .c(new_n157_), .d(new_n150_), .O(new_n160_));
  nand3  g127(.a(new_n160_), .b(new_n108_), .c(x24), .O(new_n161_));
  nand3  g128(.a(new_n151_), .b(x21), .c(x20), .O(new_n162_));
  oai22  g129(.a(new_n162_), .b(new_n150_), .c(new_n159_), .d(new_n39_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(new_n42_), .c(new_n62_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n35_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n34_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n161_), .c(new_n156_), .O(z3));
  oai21  g134(.a(x23), .b(new_n72_), .c(new_n76_), .O(new_n168_));
  aoi21  g135(.a(new_n168_), .b(new_n77_), .c(x09), .O(new_n169_));
  oai21  g136(.a(new_n169_), .b(x16), .c(new_n73_), .O(new_n170_));
  nand3  g137(.a(new_n136_), .b(new_n119_), .c(x19), .O(new_n171_));
  nand4  g138(.a(new_n108_), .b(x24), .c(x07), .d(x02), .O(new_n172_));
  nor2   g139(.a(x24), .b(x10), .O(new_n173_));
  nand4  g140(.a(new_n173_), .b(new_n35_), .c(new_n34_), .d(x00), .O(new_n174_));
  nand3  g141(.a(new_n174_), .b(new_n172_), .c(new_n171_), .O(z5));
  nand2  g142(.a(z5), .b(new_n170_), .O(new_n176_));
  aoi21  g143(.a(x23), .b(new_n72_), .c(new_n76_), .O(new_n177_));
  oai21  g144(.a(new_n177_), .b(new_n77_), .c(x09), .O(new_n178_));
  aoi21  g145(.a(new_n178_), .b(x16), .c(new_n73_), .O(new_n179_));
  nand2  g146(.a(x07), .b(x02), .O(new_n180_));
  oai21  g147(.a(new_n179_), .b(x07), .c(new_n180_), .O(new_n181_));
  nand4  g148(.a(new_n181_), .b(new_n108_), .c(x24), .d(x18), .O(new_n182_));
  inv1   g149(.a(new_n179_), .O(new_n183_));
  nand3  g150(.a(new_n136_), .b(new_n119_), .c(x15), .O(new_n184_));
  nand2  g151(.a(new_n184_), .b(new_n146_), .O(new_n185_));
  nand2  g152(.a(new_n185_), .b(new_n183_), .O(new_n186_));
  nand3  g153(.a(new_n186_), .b(new_n182_), .c(new_n176_), .O(z4));
  aoi21  g154(.a(x20), .b(new_n40_), .c(x06), .O(new_n188_));
  oai21  g155(.a(new_n188_), .b(x11), .c(new_n37_), .O(new_n189_));
  nand2  g156(.a(new_n189_), .b(z5), .O(new_n190_));
  aoi21  g157(.a(new_n41_), .b(x14), .c(new_n38_), .O(new_n191_));
  oai21  g158(.a(new_n191_), .b(new_n47_), .c(x03), .O(new_n192_));
  nand2  g159(.a(new_n192_), .b(new_n35_), .O(new_n193_));
  nand2  g160(.a(new_n193_), .b(new_n180_), .O(new_n194_));
  nand4  g161(.a(new_n194_), .b(new_n108_), .c(x24), .d(x18), .O(new_n195_));
  nand2  g162(.a(new_n192_), .b(new_n185_), .O(new_n196_));
  nand3  g163(.a(new_n196_), .b(new_n195_), .c(new_n190_), .O(z6));
  nor2   g164(.a(new_n142_), .b(new_n140_), .O(new_n198_));
  oai21  g165(.a(new_n145_), .b(x07), .c(new_n34_), .O(new_n199_));
  nand2  g166(.a(new_n199_), .b(new_n198_), .O(z7));
endmodule


