// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  nor2   g011(.a(x24), .b(x10), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  inv1   g019(.a(x10), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(x04), .O(new_n54_));
  oai21  g021(.a(new_n52_), .b(x01), .c(new_n54_), .O(z0));
  inv1   g022(.a(x01), .O(new_n56_));
  inv1   g023(.a(x04), .O(new_n57_));
  nand4  g024(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n58_));
  nor4   g025(.a(new_n58_), .b(new_n57_), .c(new_n34_), .d(new_n56_), .O(new_n59_));
  nand4  g026(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n60_));
  inv1   g027(.a(x22), .O(new_n61_));
  inv1   g028(.a(x23), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(x20), .c(x17), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n60_), .O(new_n65_));
  nand2  g032(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nor3   g033(.a(x04), .b(x03), .c(x01), .O(new_n67_));
  inv1   g034(.a(x08), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  nand4  g036(.a(new_n36_), .b(new_n69_), .c(new_n68_), .d(new_n35_), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  nand2  g038(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  inv1   g039(.a(x12), .O(new_n73_));
  inv1   g040(.a(x14), .O(new_n74_));
  inv1   g041(.a(x16), .O(new_n75_));
  inv1   g042(.a(x17), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  nor2   g045(.a(x23), .b(x22), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n78_), .c(new_n37_), .d(x19), .O(new_n80_));
  oai21  g047(.a(new_n80_), .b(new_n72_), .c(new_n66_), .O(new_n81_));
  nand2  g048(.a(new_n47_), .b(new_n38_), .O(new_n82_));
  nand3  g049(.a(new_n40_), .b(x13), .c(x05), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  nand4  g052(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  inv1   g054(.a(x18), .O(new_n88_));
  nor2   g055(.a(new_n37_), .b(new_n88_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n87_), .c(new_n63_), .d(new_n59_), .O(new_n90_));
  nor2   g057(.a(x09), .b(x08), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n67_), .c(x07), .d(new_n35_), .O(new_n92_));
  nand4  g059(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n36_), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n79_), .c(new_n37_), .d(new_n76_), .O(new_n95_));
  oai21  g062(.a(new_n95_), .b(new_n92_), .c(new_n90_), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n82_), .c(x24), .O(new_n97_));
  nor2   g064(.a(x03), .b(x02), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n71_), .c(new_n56_), .d(x00), .O(new_n99_));
  nor2   g066(.a(x24), .b(x23), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n78_), .c(new_n61_), .d(new_n37_), .O(new_n101_));
  oai21  g068(.a(new_n101_), .b(new_n99_), .c(new_n57_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n53_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n97_), .c(new_n85_), .O(z1));
  nand2  g071(.a(x24), .b(x18), .O(new_n105_));
  nand3  g072(.a(new_n40_), .b(x15), .c(x13), .O(new_n106_));
  aoi21  g073(.a(new_n106_), .b(new_n105_), .c(new_n38_), .O(new_n107_));
  nand3  g074(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n108_));
  oai21  g075(.a(new_n105_), .b(new_n47_), .c(new_n108_), .O(new_n109_));
  oai21  g076(.a(new_n109_), .b(new_n107_), .c(new_n54_), .O(new_n110_));
  nor2   g077(.a(x04), .b(x02), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(new_n40_), .c(x21), .d(new_n53_), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(x20), .c(x14), .d(x11), .O(new_n114_));
  nor4   g081(.a(new_n114_), .b(new_n35_), .c(new_n34_), .d(new_n56_), .O(z2));
  nand3  g082(.a(x15), .b(x13), .c(x05), .O(new_n116_));
  nand3  g083(.a(x21), .b(new_n53_), .c(new_n44_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x20), .c(x14), .d(x11), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n68_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x06), .c(x03), .d(x01), .O(new_n121_));
  nand3  g088(.a(new_n53_), .b(new_n44_), .c(x00), .O(new_n122_));
  nand3  g089(.a(x19), .b(x13), .c(x05), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n37_), .c(new_n74_), .d(new_n36_), .O(new_n125_));
  nor2   g092(.a(new_n125_), .b(x08), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(new_n35_), .c(new_n34_), .d(new_n56_), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(new_n121_), .c(x24), .O(new_n128_));
  nand4  g095(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n129_));
  nand3  g096(.a(new_n89_), .b(x14), .c(x11), .O(new_n130_));
  nand4  g097(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n56_), .O(new_n131_));
  nand4  g098(.a(new_n37_), .b(new_n74_), .c(new_n36_), .d(new_n68_), .O(new_n132_));
  oai22  g099(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n129_), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n82_), .c(x24), .O(new_n134_));
  nand4  g101(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n135_));
  nand4  g102(.a(new_n68_), .b(new_n35_), .c(new_n34_), .d(new_n56_), .O(new_n136_));
  nand4  g103(.a(new_n37_), .b(x19), .c(new_n74_), .d(new_n36_), .O(new_n137_));
  oai22  g104(.a(new_n137_), .b(new_n136_), .c(new_n135_), .d(new_n129_), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n47_), .c(new_n38_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n134_), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(new_n128_), .c(new_n57_), .O(new_n141_));
  nand2  g108(.a(new_n138_), .b(new_n84_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n134_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(x10), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n141_), .O(z3));
  aoi21  g112(.a(new_n61_), .b(x17), .c(x09), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(x16), .c(new_n68_), .O(new_n147_));
  nand2  g114(.a(new_n48_), .b(new_n43_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n42_), .c(new_n54_), .O(new_n149_));
  nand4  g116(.a(new_n45_), .b(new_n57_), .c(new_n44_), .d(x00), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  aoi21  g119(.a(x22), .b(new_n76_), .c(new_n69_), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n75_), .c(x08), .O(new_n154_));
  nor2   g121(.a(new_n75_), .b(x04), .O(new_n155_));
  aoi22  g122(.a(new_n155_), .b(new_n63_), .c(new_n154_), .d(new_n54_), .O(new_n156_));
  nand3  g123(.a(x10), .b(x07), .c(x04), .O(new_n157_));
  nand2  g124(.a(new_n79_), .b(new_n75_), .O(new_n158_));
  oai22  g125(.a(new_n158_), .b(new_n157_), .c(new_n156_), .d(new_n88_), .O(new_n159_));
  nand3  g126(.a(new_n159_), .b(new_n82_), .c(x24), .O(new_n160_));
  inv1   g127(.a(x15), .O(new_n161_));
  nor2   g128(.a(x16), .b(new_n53_), .O(new_n162_));
  nand4  g129(.a(new_n162_), .b(new_n79_), .c(x19), .d(x04), .O(new_n163_));
  oai21  g130(.a(new_n156_), .b(new_n161_), .c(new_n163_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n84_), .O(new_n165_));
  aoi21  g132(.a(new_n62_), .b(x17), .c(new_n61_), .O(new_n166_));
  oai21  g133(.a(new_n166_), .b(new_n69_), .c(x16), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(x08), .O(new_n168_));
  nand4  g135(.a(new_n168_), .b(new_n40_), .c(x21), .d(new_n53_), .O(new_n169_));
  inv1   g136(.a(new_n169_), .O(new_n170_));
  nand3  g137(.a(new_n170_), .b(new_n57_), .c(new_n44_), .O(new_n171_));
  nand4  g138(.a(new_n171_), .b(new_n165_), .c(new_n160_), .d(new_n152_), .O(z4));
  inv1   g139(.a(new_n42_), .O(new_n173_));
  nand3  g140(.a(new_n40_), .b(new_n44_), .c(x00), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(new_n57_), .O(new_n175_));
  aoi21  g142(.a(new_n175_), .b(new_n53_), .c(new_n148_), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(new_n173_), .O(z5));
  aoi21  g144(.a(x20), .b(new_n74_), .c(x06), .O(new_n178_));
  oai21  g145(.a(new_n178_), .b(x11), .c(new_n34_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n151_), .O(new_n180_));
  aoi21  g147(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n181_));
  oai21  g148(.a(new_n181_), .b(new_n36_), .c(x03), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(new_n113_), .O(new_n183_));
  nand2  g150(.a(new_n183_), .b(new_n180_), .O(z6));
  inv1   g151(.a(new_n107_), .O(new_n185_));
  nand3  g152(.a(new_n40_), .b(x21), .c(new_n44_), .O(new_n186_));
  nand2  g153(.a(new_n186_), .b(new_n57_), .O(new_n187_));
  aoi21  g154(.a(new_n187_), .b(new_n53_), .c(new_n109_), .O(new_n188_));
  nand2  g155(.a(new_n188_), .b(new_n185_), .O(z7));
endmodule


