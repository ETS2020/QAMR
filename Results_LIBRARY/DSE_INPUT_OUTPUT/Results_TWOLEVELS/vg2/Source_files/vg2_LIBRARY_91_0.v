// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x14), .O(new_n37_));
  inv1   g004(.a(x20), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x05), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x05), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n45_), .O(new_n47_));
  and2   g014(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n38_), .c(new_n37_), .d(new_n36_), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n35_), .c(new_n34_), .d(x02), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x01), .O(z0));
  inv1   g020(.a(x02), .O(new_n54_));
  inv1   g021(.a(x01), .O(new_n55_));
  nor2   g022(.a(new_n34_), .b(new_n55_), .O(new_n56_));
  inv1   g023(.a(x04), .O(new_n57_));
  nor2   g024(.a(new_n35_), .b(new_n57_), .O(new_n58_));
  inv1   g025(.a(x08), .O(new_n59_));
  inv1   g026(.a(x09), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g028(.a(new_n36_), .b(x10), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n61_), .c(new_n58_), .d(new_n56_), .O(new_n63_));
  nand4  g030(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n64_));
  inv1   g031(.a(new_n64_), .O(new_n65_));
  inv1   g032(.a(x23), .O(new_n66_));
  nor2   g033(.a(x24), .b(new_n66_), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n65_), .c(x22), .d(x21), .O(new_n68_));
  oai21  g035(.a(new_n68_), .b(new_n63_), .c(x20), .O(new_n69_));
  nand2  g036(.a(new_n69_), .b(new_n54_), .O(new_n70_));
  nand3  g037(.a(x04), .b(x03), .c(x01), .O(new_n71_));
  inv1   g038(.a(new_n71_), .O(new_n72_));
  nor2   g039(.a(new_n59_), .b(new_n35_), .O(new_n73_));
  nand3  g040(.a(new_n73_), .b(x11), .c(x09), .O(new_n74_));
  inv1   g041(.a(new_n74_), .O(new_n75_));
  nand4  g042(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n76_));
  nand4  g043(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  nor3   g046(.a(x04), .b(x03), .c(x01), .O(new_n80_));
  nor2   g047(.a(x11), .b(x09), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n80_), .c(new_n59_), .d(new_n35_), .O(new_n82_));
  nor2   g049(.a(x14), .b(x12), .O(new_n83_));
  nor2   g050(.a(x17), .b(x16), .O(new_n84_));
  inv1   g051(.a(x19), .O(new_n85_));
  nor2   g052(.a(x20), .b(new_n85_), .O(new_n86_));
  nor2   g053(.a(x23), .b(x22), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n86_), .c(new_n84_), .d(new_n83_), .O(new_n88_));
  oai21  g055(.a(new_n88_), .b(new_n82_), .c(new_n79_), .O(new_n89_));
  nand2  g056(.a(new_n46_), .b(new_n45_), .O(new_n90_));
  nand3  g057(.a(new_n40_), .b(x13), .c(x05), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand4  g060(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(new_n64_), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n75_), .c(new_n72_), .O(new_n96_));
  nor2   g063(.a(x09), .b(x08), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n80_), .c(x07), .d(new_n35_), .O(new_n98_));
  nor2   g065(.a(x12), .b(x11), .O(new_n99_));
  nor2   g066(.a(x16), .b(x14), .O(new_n100_));
  nor2   g067(.a(x20), .b(x17), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n87_), .O(new_n102_));
  oai21  g069(.a(new_n102_), .b(new_n98_), .c(new_n96_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n90_), .c(x24), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n93_), .c(new_n70_), .O(z1));
  inv1   g072(.a(x10), .O(new_n106_));
  nand3  g073(.a(new_n56_), .b(new_n106_), .c(x06), .O(new_n107_));
  nor2   g074(.a(new_n37_), .b(new_n36_), .O(new_n108_));
  inv1   g075(.a(x21), .O(new_n109_));
  nor2   g076(.a(x24), .b(new_n109_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  oai21  g078(.a(new_n111_), .b(new_n107_), .c(x20), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n54_), .O(new_n113_));
  nand2  g080(.a(x24), .b(x18), .O(new_n114_));
  nand3  g081(.a(new_n40_), .b(x15), .c(x13), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g083(.a(x24), .b(x18), .c(x13), .O(new_n117_));
  nand3  g084(.a(x15), .b(new_n46_), .c(new_n45_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  aoi21  g086(.a(new_n116_), .b(x05), .c(new_n119_), .O(new_n120_));
  nor3   g087(.a(new_n120_), .b(new_n38_), .c(new_n37_), .O(new_n121_));
  nand4  g088(.a(new_n121_), .b(x11), .c(x06), .d(x03), .O(new_n122_));
  oai21  g089(.a(new_n122_), .b(new_n55_), .c(new_n113_), .O(z2));
  nand2  g090(.a(new_n73_), .b(new_n56_), .O(new_n124_));
  nand3  g091(.a(new_n108_), .b(x20), .c(x15), .O(new_n125_));
  nand4  g092(.a(new_n35_), .b(new_n34_), .c(x02), .d(new_n55_), .O(new_n126_));
  nand4  g093(.a(new_n86_), .b(new_n37_), .c(new_n36_), .d(new_n59_), .O(new_n127_));
  oai22  g094(.a(new_n127_), .b(new_n126_), .c(new_n125_), .d(new_n124_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n92_), .O(new_n129_));
  nand3  g096(.a(new_n108_), .b(x20), .c(x18), .O(new_n130_));
  nor2   g097(.a(x20), .b(x14), .O(new_n131_));
  nand4  g098(.a(new_n131_), .b(new_n36_), .c(new_n59_), .d(x07), .O(new_n132_));
  oai22  g099(.a(new_n132_), .b(new_n126_), .c(new_n130_), .d(new_n124_), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n90_), .c(x24), .O(new_n134_));
  nor2   g101(.a(x02), .b(new_n55_), .O(new_n135_));
  nand2  g102(.a(new_n108_), .b(new_n106_), .O(new_n136_));
  nand2  g103(.a(new_n110_), .b(x20), .O(new_n137_));
  nor2   g104(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand4  g105(.a(new_n138_), .b(new_n135_), .c(new_n73_), .d(x03), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n134_), .c(new_n129_), .O(z3));
  inv1   g107(.a(x22), .O(new_n141_));
  inv1   g108(.a(x17), .O(new_n142_));
  oai21  g109(.a(x23), .b(new_n57_), .c(new_n142_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n141_), .c(x09), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x16), .c(new_n59_), .O(new_n145_));
  inv1   g112(.a(x00), .O(new_n146_));
  nor2   g113(.a(x02), .b(new_n146_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n40_), .c(new_n106_), .O(new_n148_));
  nand4  g115(.a(new_n148_), .b(new_n47_), .c(new_n44_), .d(new_n43_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  inv1   g117(.a(x16), .O(new_n151_));
  oai21  g118(.a(new_n66_), .b(x04), .c(x17), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(x22), .c(new_n60_), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n151_), .c(x08), .O(new_n154_));
  nand2  g121(.a(new_n116_), .b(x05), .O(new_n155_));
  nand3  g122(.a(new_n110_), .b(new_n106_), .c(new_n54_), .O(new_n156_));
  nand4  g123(.a(new_n156_), .b(new_n118_), .c(new_n117_), .d(new_n155_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g125(.a(new_n38_), .b(new_n54_), .O(new_n159_));
  nand3  g126(.a(new_n159_), .b(new_n158_), .c(new_n150_), .O(z4));
  nand2  g127(.a(new_n159_), .b(new_n49_), .O(new_n161_));
  nand4  g128(.a(new_n147_), .b(new_n40_), .c(x20), .d(new_n106_), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n161_), .O(z5));
  oai21  g130(.a(new_n36_), .b(x06), .c(x03), .O(new_n164_));
  nand2  g131(.a(new_n106_), .b(new_n54_), .O(new_n165_));
  inv1   g132(.a(new_n159_), .O(new_n166_));
  oai22  g133(.a(new_n166_), .b(new_n120_), .c(new_n165_), .d(new_n137_), .O(z7));
  nand2  g134(.a(z7), .b(new_n164_), .O(new_n168_));
  oai21  g135(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n169_));
  nor2   g136(.a(new_n38_), .b(x14), .O(new_n170_));
  aoi22  g137(.a(new_n170_), .b(new_n36_), .c(new_n169_), .d(new_n159_), .O(new_n171_));
  nor2   g138(.a(new_n171_), .b(new_n85_), .O(new_n172_));
  nand3  g139(.a(new_n38_), .b(x15), .c(x14), .O(new_n173_));
  nor3   g140(.a(new_n173_), .b(new_n36_), .c(new_n54_), .O(new_n174_));
  oai21  g141(.a(new_n174_), .b(new_n172_), .c(new_n92_), .O(new_n175_));
  inv1   g142(.a(x07), .O(new_n176_));
  nor2   g143(.a(new_n36_), .b(new_n54_), .O(new_n177_));
  nand4  g144(.a(new_n177_), .b(new_n38_), .c(x18), .d(x14), .O(new_n178_));
  oai21  g145(.a(new_n171_), .b(new_n176_), .c(new_n178_), .O(new_n179_));
  nand3  g146(.a(new_n179_), .b(new_n90_), .c(x24), .O(new_n180_));
  oai21  g147(.a(new_n37_), .b(x06), .c(new_n36_), .O(new_n181_));
  nand2  g148(.a(new_n181_), .b(new_n34_), .O(new_n182_));
  nand4  g149(.a(new_n182_), .b(new_n40_), .c(x20), .d(new_n106_), .O(new_n183_));
  inv1   g150(.a(new_n183_), .O(new_n184_));
  nand3  g151(.a(new_n184_), .b(new_n54_), .c(x00), .O(new_n185_));
  nand4  g152(.a(new_n185_), .b(new_n180_), .c(new_n175_), .d(new_n168_), .O(z6));
endmodule


