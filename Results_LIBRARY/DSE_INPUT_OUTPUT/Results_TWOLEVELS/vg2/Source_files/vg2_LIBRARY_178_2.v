// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n39_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n37_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nor3   g016(.a(new_n49_), .b(x20), .c(x14), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n51_));
  inv1   g018(.a(x08), .O(new_n52_));
  nor2   g019(.a(x21), .b(new_n52_), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  oai21  g021(.a(new_n51_), .b(x01), .c(new_n54_), .O(z0));
  inv1   g022(.a(x12), .O(new_n56_));
  inv1   g023(.a(x20), .O(new_n57_));
  nand3  g024(.a(x15), .b(x13), .c(x05), .O(new_n58_));
  oai21  g025(.a(x10), .b(x02), .c(new_n58_), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(x23), .c(x22), .d(x21), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(x17), .c(x16), .d(x14), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(x11), .c(x09), .d(x08), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n35_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(x04), .c(x03), .d(x01), .O(new_n66_));
  inv1   g033(.a(x01), .O(new_n67_));
  inv1   g034(.a(x04), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  inv1   g036(.a(x14), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  inv1   g038(.a(x17), .O(new_n72_));
  inv1   g039(.a(x22), .O(new_n73_));
  inv1   g040(.a(x23), .O(new_n74_));
  nand3  g041(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n75_));
  nand3  g042(.a(x19), .b(x13), .c(x05), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n74_), .c(new_n73_), .d(new_n57_), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(x12), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n36_), .c(new_n69_), .d(new_n52_), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(x06), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n68_), .c(new_n34_), .d(new_n67_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n39_), .O(new_n86_));
  nor2   g053(.a(x13), .b(x05), .O(new_n87_));
  nand4  g054(.a(x06), .b(x04), .c(x03), .d(x01), .O(new_n88_));
  nand4  g055(.a(x12), .b(x11), .c(x09), .d(x08), .O(new_n89_));
  nor2   g056(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand4  g057(.a(x18), .b(x17), .c(x16), .d(x14), .O(new_n91_));
  nand4  g058(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nor3   g061(.a(x04), .b(x03), .c(x01), .O(new_n95_));
  nand4  g062(.a(new_n69_), .b(new_n52_), .c(x07), .d(new_n35_), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n71_), .b(new_n70_), .c(new_n56_), .d(new_n36_), .O(new_n98_));
  nand4  g065(.a(new_n74_), .b(new_n73_), .c(new_n57_), .d(new_n72_), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n97_), .c(new_n95_), .O(new_n101_));
  aoi21  g068(.a(new_n101_), .b(new_n94_), .c(new_n87_), .O(new_n102_));
  nand4  g069(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n92_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n90_), .O(new_n105_));
  nand4  g072(.a(new_n36_), .b(new_n69_), .c(new_n52_), .d(new_n35_), .O(new_n106_));
  inv1   g073(.a(new_n106_), .O(new_n107_));
  nand4  g074(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n56_), .O(new_n108_));
  nand4  g075(.a(new_n74_), .b(new_n73_), .c(new_n57_), .d(x19), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n107_), .c(new_n95_), .O(new_n111_));
  aoi21  g078(.a(new_n111_), .b(new_n105_), .c(x13), .O(new_n112_));
  aoi22  g079(.a(new_n112_), .b(new_n37_), .c(new_n102_), .d(x24), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n86_), .O(z1));
  nand2  g081(.a(x24), .b(x18), .O(new_n115_));
  nand3  g082(.a(new_n39_), .b(x15), .c(x13), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n115_), .c(new_n37_), .O(new_n117_));
  nand3  g084(.a(x24), .b(x18), .c(x13), .O(new_n118_));
  nand3  g085(.a(x15), .b(new_n46_), .c(new_n37_), .O(new_n119_));
  nand4  g086(.a(new_n39_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  oai21  g088(.a(new_n121_), .b(new_n117_), .c(x20), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(new_n70_), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(x11), .c(x06), .d(x03), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(new_n67_), .c(new_n54_), .O(z2));
  nand4  g092(.a(new_n59_), .b(x20), .c(x14), .d(x11), .O(new_n126_));
  nor2   g093(.a(new_n126_), .b(new_n52_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(x06), .c(x03), .d(x01), .O(new_n128_));
  nand4  g095(.a(new_n77_), .b(new_n57_), .c(new_n70_), .d(new_n36_), .O(new_n129_));
  nor2   g096(.a(new_n129_), .b(x08), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(new_n35_), .c(new_n34_), .d(new_n67_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n39_), .O(new_n133_));
  inv1   g100(.a(new_n87_), .O(new_n134_));
  nand4  g101(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n135_));
  nand4  g102(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n136_));
  nand4  g103(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n67_), .O(new_n137_));
  nand4  g104(.a(new_n57_), .b(new_n70_), .c(new_n36_), .d(new_n52_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n135_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n134_), .c(x24), .O(new_n140_));
  nand4  g107(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n141_));
  nand4  g108(.a(new_n52_), .b(new_n35_), .c(new_n34_), .d(new_n67_), .O(new_n142_));
  nand4  g109(.a(new_n57_), .b(x19), .c(new_n70_), .d(new_n36_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n135_), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n46_), .c(new_n37_), .O(new_n145_));
  nand4  g112(.a(new_n145_), .b(new_n140_), .c(new_n133_), .d(new_n54_), .O(z3));
  inv1   g113(.a(new_n49_), .O(new_n147_));
  aoi21  g114(.a(new_n74_), .b(x04), .c(x17), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(x22), .c(new_n69_), .O(new_n149_));
  nand3  g116(.a(new_n149_), .b(new_n147_), .c(new_n71_), .O(new_n150_));
  inv1   g117(.a(x18), .O(new_n151_));
  aoi21  g118(.a(x23), .b(new_n68_), .c(new_n72_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n73_), .c(x09), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(x16), .c(new_n52_), .O(new_n154_));
  nand3  g121(.a(x21), .b(x08), .c(x07), .O(new_n155_));
  oai21  g122(.a(new_n154_), .b(new_n151_), .c(new_n155_), .O(new_n156_));
  nand3  g123(.a(new_n156_), .b(new_n134_), .c(x24), .O(new_n157_));
  nand3  g124(.a(new_n39_), .b(x13), .c(x05), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n134_), .O(new_n159_));
  inv1   g126(.a(x15), .O(new_n160_));
  nand3  g127(.a(x21), .b(x19), .c(x08), .O(new_n161_));
  oai21  g128(.a(new_n154_), .b(new_n160_), .c(new_n161_), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  nand2  g130(.a(new_n153_), .b(x16), .O(new_n164_));
  nor2   g131(.a(new_n52_), .b(x00), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand4  g133(.a(new_n166_), .b(new_n39_), .c(x21), .d(new_n44_), .O(new_n167_));
  inv1   g134(.a(new_n167_), .O(new_n168_));
  aoi21  g135(.a(new_n168_), .b(new_n43_), .c(new_n53_), .O(new_n169_));
  nand4  g136(.a(new_n169_), .b(new_n163_), .c(new_n157_), .d(new_n150_), .O(z4));
  nand4  g137(.a(new_n54_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n171_));
  inv1   g138(.a(x21), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(x08), .O(new_n173_));
  nand4  g140(.a(new_n173_), .b(x19), .c(x13), .d(x05), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(new_n171_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n39_), .O(new_n176_));
  nand3  g143(.a(new_n134_), .b(x24), .c(x07), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n47_), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n173_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n176_), .O(z5));
  nand2  g147(.a(x20), .b(new_n70_), .O(new_n181_));
  aoi21  g148(.a(new_n181_), .b(new_n35_), .c(x11), .O(new_n182_));
  oai21  g149(.a(new_n182_), .b(x03), .c(new_n147_), .O(new_n183_));
  nand2  g150(.a(new_n57_), .b(x14), .O(new_n184_));
  aoi21  g151(.a(new_n184_), .b(x06), .c(new_n36_), .O(new_n185_));
  oai22  g152(.a(new_n185_), .b(new_n34_), .c(new_n121_), .d(new_n117_), .O(new_n186_));
  nand3  g153(.a(new_n186_), .b(new_n183_), .c(new_n54_), .O(z6));
  nand2  g154(.a(new_n119_), .b(new_n118_), .O(new_n188_));
  oai21  g155(.a(new_n188_), .b(new_n117_), .c(new_n54_), .O(new_n189_));
  nand2  g156(.a(new_n189_), .b(new_n120_), .O(z7));
endmodule


