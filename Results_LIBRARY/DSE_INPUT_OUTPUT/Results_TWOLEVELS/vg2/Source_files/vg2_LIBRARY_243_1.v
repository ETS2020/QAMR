// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n180_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x12), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x13), .O(new_n44_));
  nand3  g011(.a(x19), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  oai22  g013(.a(new_n46_), .b(new_n42_), .c(new_n37_), .d(x02), .O(new_n47_));
  inv1   g014(.a(x02), .O(new_n48_));
  inv1   g015(.a(x10), .O(new_n49_));
  nor2   g016(.a(x24), .b(x12), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x00), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n47_), .c(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(z0));
  inv1   g021(.a(x00), .O(new_n55_));
  nor2   g022(.a(x01), .b(new_n55_), .O(new_n56_));
  nor2   g023(.a(x04), .b(x03), .O(new_n57_));
  nor2   g024(.a(x08), .b(x06), .O(new_n58_));
  nor2   g025(.a(x10), .b(x09), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n60_));
  nor2   g027(.a(x14), .b(x11), .O(new_n61_));
  nor2   g028(.a(x17), .b(x16), .O(new_n62_));
  nor2   g029(.a(x22), .b(x20), .O(new_n63_));
  nor2   g030(.a(x24), .b(x23), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n61_), .O(new_n65_));
  oai21  g032(.a(new_n65_), .b(new_n60_), .c(new_n37_), .O(new_n66_));
  nand2  g033(.a(new_n66_), .b(new_n48_), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  inv1   g035(.a(x03), .O(new_n69_));
  inv1   g036(.a(x04), .O(new_n70_));
  nand4  g037(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n71_));
  nor4   g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand4  g039(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n73_));
  inv1   g040(.a(x22), .O(new_n74_));
  inv1   g041(.a(x23), .O(new_n75_));
  nor2   g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(x20), .c(x17), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n72_), .O(new_n79_));
  inv1   g046(.a(x09), .O(new_n80_));
  nor3   g047(.a(x04), .b(x03), .c(x01), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n58_), .c(new_n35_), .d(new_n80_), .O(new_n82_));
  nor2   g049(.a(x14), .b(x12), .O(new_n83_));
  inv1   g050(.a(x19), .O(new_n84_));
  nor2   g051(.a(x20), .b(new_n84_), .O(new_n85_));
  nor2   g052(.a(x23), .b(x22), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n62_), .O(new_n87_));
  oai21  g054(.a(new_n87_), .b(new_n82_), .c(new_n79_), .O(new_n88_));
  nand2  g055(.a(new_n44_), .b(new_n38_), .O(new_n89_));
  nand3  g056(.a(new_n40_), .b(x13), .c(x05), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand4  g059(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  and2   g061(.a(x20), .b(x18), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n94_), .c(new_n76_), .d(new_n72_), .O(new_n96_));
  nor2   g063(.a(x09), .b(x08), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n81_), .c(x07), .d(new_n34_), .O(new_n98_));
  inv1   g065(.a(x17), .O(new_n99_));
  inv1   g066(.a(x20), .O(new_n100_));
  inv1   g067(.a(x16), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n36_), .c(new_n37_), .d(new_n35_), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n86_), .c(new_n100_), .d(new_n99_), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n98_), .c(new_n96_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n89_), .c(x24), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n92_), .c(new_n67_), .O(z1));
  nand4  g074(.a(new_n49_), .b(x06), .c(x03), .d(x01), .O(new_n108_));
  nor2   g075(.a(new_n36_), .b(new_n35_), .O(new_n109_));
  nand2  g076(.a(new_n40_), .b(x21), .O(new_n110_));
  inv1   g077(.a(new_n110_), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n109_), .c(x20), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n108_), .c(new_n37_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n48_), .O(new_n114_));
  nand2  g081(.a(x24), .b(x18), .O(new_n115_));
  nand3  g082(.a(new_n40_), .b(x15), .c(x13), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n115_), .c(new_n38_), .O(new_n117_));
  nand3  g084(.a(x15), .b(new_n44_), .c(new_n38_), .O(new_n118_));
  oai21  g085(.a(new_n115_), .b(new_n44_), .c(new_n118_), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nor3   g087(.a(new_n120_), .b(new_n100_), .c(new_n36_), .O(new_n121_));
  nand4  g088(.a(new_n121_), .b(x11), .c(x06), .d(x03), .O(new_n122_));
  oai21  g089(.a(new_n122_), .b(new_n68_), .c(new_n114_), .O(z2));
  inv1   g090(.a(x08), .O(new_n124_));
  nand3  g091(.a(x15), .b(x13), .c(x05), .O(new_n125_));
  nand3  g092(.a(x21), .b(new_n49_), .c(new_n48_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(x20), .c(x14), .d(x11), .O(new_n128_));
  nor2   g095(.a(new_n128_), .b(new_n124_), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(x06), .c(x03), .d(x01), .O(new_n130_));
  nand3  g097(.a(new_n49_), .b(new_n48_), .c(x00), .O(new_n131_));
  nand3  g098(.a(x19), .b(x13), .c(x05), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand4  g100(.a(new_n133_), .b(new_n100_), .c(new_n36_), .d(new_n35_), .O(new_n134_));
  nor2   g101(.a(new_n134_), .b(x08), .O(new_n135_));
  nand4  g102(.a(new_n135_), .b(new_n34_), .c(new_n69_), .d(new_n68_), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(new_n130_), .c(x24), .O(new_n137_));
  nand4  g104(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n138_));
  nand2  g105(.a(new_n109_), .b(new_n95_), .O(new_n139_));
  nand4  g106(.a(x07), .b(new_n34_), .c(new_n69_), .d(new_n68_), .O(new_n140_));
  nand4  g107(.a(new_n100_), .b(new_n36_), .c(new_n35_), .d(new_n124_), .O(new_n141_));
  oai22  g108(.a(new_n141_), .b(new_n140_), .c(new_n139_), .d(new_n138_), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n89_), .c(x24), .O(new_n143_));
  nand3  g110(.a(new_n109_), .b(x20), .c(x15), .O(new_n144_));
  nand3  g111(.a(new_n58_), .b(new_n69_), .c(new_n68_), .O(new_n145_));
  nand2  g112(.a(new_n85_), .b(new_n61_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n145_), .c(new_n144_), .d(new_n138_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n44_), .c(new_n38_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n143_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n137_), .c(new_n37_), .O(new_n150_));
  nand2  g117(.a(new_n147_), .b(new_n91_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n143_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(x02), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n150_), .O(z3));
  nand2  g121(.a(new_n51_), .b(new_n47_), .O(new_n155_));
  oai21  g122(.a(x23), .b(new_n70_), .c(new_n99_), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n74_), .c(x09), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(x16), .c(new_n124_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  oai21  g126(.a(new_n75_), .b(x04), .c(x17), .O(new_n160_));
  aoi21  g127(.a(new_n160_), .b(x22), .c(new_n80_), .O(new_n161_));
  oai21  g128(.a(new_n161_), .b(new_n101_), .c(x08), .O(new_n162_));
  nor2   g129(.a(new_n37_), .b(x02), .O(new_n163_));
  nand4  g130(.a(new_n111_), .b(new_n37_), .c(new_n49_), .d(new_n48_), .O(new_n164_));
  oai21  g131(.a(new_n120_), .b(new_n163_), .c(new_n164_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n159_), .O(z4));
  nor2   g134(.a(new_n46_), .b(new_n42_), .O(new_n168_));
  nand3  g135(.a(new_n40_), .b(new_n49_), .c(x00), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n37_), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n48_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n168_), .O(z5));
  aoi21  g139(.a(x20), .b(new_n36_), .c(x06), .O(new_n173_));
  oai21  g140(.a(new_n173_), .b(x11), .c(new_n69_), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(new_n155_), .O(new_n175_));
  aoi21  g142(.a(new_n100_), .b(x14), .c(new_n34_), .O(new_n176_));
  oai21  g143(.a(new_n176_), .b(new_n35_), .c(x03), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n165_), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n175_), .O(z6));
  aoi21  g146(.a(new_n111_), .b(new_n49_), .c(x12), .O(new_n180_));
  oai21  g147(.a(new_n180_), .b(x02), .c(new_n120_), .O(z7));
endmodule


