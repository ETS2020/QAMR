// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n176_, new_n177_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x04), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x02), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x13), .O(new_n45_));
  nand3  g012(.a(x19), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g014(.a(new_n47_), .b(new_n43_), .c(new_n38_), .O(new_n48_));
  inv1   g015(.a(x02), .O(new_n49_));
  inv1   g016(.a(x10), .O(new_n50_));
  nand4  g017(.a(new_n41_), .b(new_n50_), .c(new_n49_), .d(x00), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n48_), .c(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(z0));
  nand3  g021(.a(x04), .b(x03), .c(x01), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  inv1   g023(.a(x08), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n34_), .O(new_n58_));
  nand3  g025(.a(new_n58_), .b(x11), .c(x09), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand4  g027(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n61_));
  nand4  g028(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(new_n60_), .c(new_n56_), .O(new_n64_));
  nor3   g031(.a(x04), .b(x03), .c(x01), .O(new_n65_));
  nor2   g032(.a(x08), .b(x06), .O(new_n66_));
  nor2   g033(.a(x11), .b(x09), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  inv1   g035(.a(x20), .O(new_n69_));
  inv1   g036(.a(x12), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  inv1   g038(.a(x17), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n36_), .d(new_n70_), .O(new_n73_));
  inv1   g040(.a(new_n73_), .O(new_n74_));
  nor2   g041(.a(x23), .b(x22), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n74_), .c(new_n69_), .d(x19), .O(new_n76_));
  oai21  g043(.a(new_n76_), .b(new_n68_), .c(new_n64_), .O(new_n77_));
  nor2   g044(.a(x13), .b(x05), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  nand3  g046(.a(new_n41_), .b(x13), .c(x05), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  nand4  g049(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n83_));
  nand4  g050(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n60_), .c(new_n56_), .O(new_n86_));
  nor2   g053(.a(x09), .b(x08), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n65_), .c(x07), .d(new_n34_), .O(new_n88_));
  nand4  g055(.a(new_n71_), .b(new_n36_), .c(new_n70_), .d(new_n35_), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n75_), .c(new_n69_), .d(new_n72_), .O(new_n91_));
  oai21  g058(.a(new_n91_), .b(new_n88_), .c(new_n86_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n79_), .c(x24), .O(new_n93_));
  inv1   g060(.a(x01), .O(new_n94_));
  inv1   g061(.a(x03), .O(new_n95_));
  nand4  g062(.a(new_n34_), .b(new_n95_), .c(new_n94_), .d(x00), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n87_), .c(new_n35_), .d(new_n50_), .O(new_n98_));
  inv1   g065(.a(x22), .O(new_n99_));
  nor2   g066(.a(x24), .b(x23), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n74_), .c(new_n99_), .d(new_n69_), .O(new_n101_));
  oai21  g068(.a(new_n101_), .b(new_n98_), .c(new_n49_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n37_), .O(new_n103_));
  nand4  g070(.a(x04), .b(x03), .c(new_n49_), .d(x01), .O(new_n104_));
  inv1   g071(.a(new_n104_), .O(new_n105_));
  nand3  g072(.a(x11), .b(new_n50_), .c(x09), .O(new_n106_));
  inv1   g073(.a(new_n106_), .O(new_n107_));
  nand2  g074(.a(x21), .b(x20), .O(new_n108_));
  nand3  g075(.a(new_n41_), .b(x23), .c(x22), .O(new_n109_));
  nor3   g076(.a(new_n109_), .b(new_n108_), .c(new_n83_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(new_n107_), .c(new_n105_), .d(new_n58_), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(new_n103_), .c(new_n93_), .d(new_n82_), .O(z1));
  nand2  g079(.a(x24), .b(x18), .O(new_n113_));
  nand3  g080(.a(new_n41_), .b(x15), .c(x13), .O(new_n114_));
  aoi21  g081(.a(new_n114_), .b(new_n113_), .c(new_n39_), .O(new_n115_));
  nand3  g082(.a(x24), .b(x18), .c(x13), .O(new_n116_));
  nand3  g083(.a(x15), .b(new_n45_), .c(new_n39_), .O(new_n117_));
  nand4  g084(.a(new_n41_), .b(x21), .c(new_n50_), .d(new_n49_), .O(new_n118_));
  nand3  g085(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nor3   g087(.a(new_n120_), .b(new_n69_), .c(new_n36_), .O(new_n121_));
  nand4  g088(.a(new_n121_), .b(x11), .c(x06), .d(x03), .O(new_n122_));
  oai21  g089(.a(new_n122_), .b(new_n94_), .c(new_n38_), .O(z2));
  nand3  g090(.a(x15), .b(x13), .c(x05), .O(new_n124_));
  nand3  g091(.a(x21), .b(new_n50_), .c(new_n49_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(x20), .c(x14), .d(x11), .O(new_n127_));
  nor2   g094(.a(new_n127_), .b(new_n57_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(x06), .c(x03), .d(x01), .O(new_n129_));
  nand3  g096(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n130_));
  nand3  g097(.a(x19), .b(x13), .c(x05), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n69_), .c(new_n36_), .d(new_n35_), .O(new_n133_));
  nor2   g100(.a(new_n133_), .b(x08), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n34_), .c(new_n95_), .d(new_n94_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n129_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n41_), .O(new_n137_));
  nand3  g104(.a(new_n58_), .b(x03), .c(x01), .O(new_n138_));
  nand4  g105(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n139_));
  nand4  g106(.a(x07), .b(new_n34_), .c(new_n95_), .d(new_n94_), .O(new_n140_));
  nand4  g107(.a(new_n69_), .b(new_n36_), .c(new_n35_), .d(new_n57_), .O(new_n141_));
  oai22  g108(.a(new_n141_), .b(new_n140_), .c(new_n139_), .d(new_n138_), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n79_), .c(x24), .O(new_n143_));
  nand4  g110(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n144_));
  nand3  g111(.a(new_n66_), .b(new_n95_), .c(new_n94_), .O(new_n145_));
  nand4  g112(.a(new_n69_), .b(x19), .c(new_n36_), .d(new_n35_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n145_), .c(new_n144_), .d(new_n138_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n45_), .c(new_n39_), .O(new_n148_));
  nand4  g115(.a(new_n148_), .b(new_n143_), .c(new_n137_), .d(new_n38_), .O(z3));
  oai21  g116(.a(x23), .b(new_n37_), .c(new_n72_), .O(new_n150_));
  aoi21  g117(.a(new_n150_), .b(new_n99_), .c(x09), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(x16), .c(new_n57_), .O(new_n152_));
  inv1   g119(.a(new_n43_), .O(new_n153_));
  and2   g120(.a(new_n51_), .b(new_n46_), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n44_), .c(new_n153_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  inv1   g123(.a(new_n120_), .O(new_n157_));
  nand2  g124(.a(x22), .b(new_n72_), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(x09), .c(new_n71_), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(new_n57_), .c(new_n157_), .O(new_n160_));
  inv1   g127(.a(new_n115_), .O(new_n161_));
  nand3  g128(.a(new_n41_), .b(x21), .c(new_n50_), .O(new_n162_));
  nand4  g129(.a(new_n162_), .b(new_n117_), .c(new_n116_), .d(new_n161_), .O(new_n163_));
  nand4  g130(.a(new_n163_), .b(x23), .c(x22), .d(x16), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n49_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n37_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n160_), .c(new_n156_), .O(z4));
  nand4  g134(.a(new_n154_), .b(new_n44_), .c(new_n153_), .d(new_n38_), .O(z5));
  nand2  g135(.a(x20), .b(new_n36_), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(new_n34_), .c(x11), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(x03), .c(new_n155_), .O(new_n171_));
  nand2  g138(.a(new_n69_), .b(x14), .O(new_n172_));
  aoi21  g139(.a(new_n172_), .b(x06), .c(new_n35_), .O(new_n173_));
  oai21  g140(.a(new_n173_), .b(new_n95_), .c(new_n157_), .O(new_n174_));
  nand3  g141(.a(new_n174_), .b(new_n171_), .c(new_n38_), .O(z6));
  nand2  g142(.a(new_n117_), .b(new_n116_), .O(new_n176_));
  oai21  g143(.a(new_n176_), .b(new_n115_), .c(new_n38_), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n118_), .O(z7));
endmodule


