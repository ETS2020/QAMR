// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x14), .O(new_n37_));
  inv1   g004(.a(x20), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x05), .O(new_n43_));
  inv1   g010(.a(x05), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  inv1   g012(.a(new_n45_), .O(new_n46_));
  inv1   g013(.a(x19), .O(new_n47_));
  nor2   g014(.a(new_n47_), .b(x13), .O(new_n48_));
  aoi21  g015(.a(new_n48_), .b(new_n44_), .c(new_n46_), .O(new_n49_));
  nand2  g016(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n38_), .c(new_n37_), .d(new_n36_), .O(new_n51_));
  inv1   g018(.a(new_n51_), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(x10), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(x01), .O(z0));
  nand3  g021(.a(x04), .b(x03), .c(x01), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  inv1   g023(.a(x08), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n35_), .O(new_n58_));
  nand3  g025(.a(new_n58_), .b(x11), .c(x09), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand4  g027(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n61_));
  nand4  g028(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(new_n60_), .c(new_n56_), .O(new_n64_));
  inv1   g031(.a(x09), .O(new_n65_));
  inv1   g032(.a(x04), .O(new_n66_));
  nor2   g033(.a(x03), .b(x01), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n35_), .c(new_n66_), .O(new_n68_));
  inv1   g035(.a(new_n68_), .O(new_n69_));
  inv1   g036(.a(x10), .O(new_n70_));
  nor2   g037(.a(x11), .b(new_n70_), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n69_), .c(new_n65_), .d(new_n57_), .O(new_n72_));
  nor2   g039(.a(x14), .b(x12), .O(new_n73_));
  nor2   g040(.a(x17), .b(x16), .O(new_n74_));
  nor2   g041(.a(x20), .b(new_n47_), .O(new_n75_));
  nor2   g042(.a(x23), .b(x22), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  oai21  g044(.a(new_n77_), .b(new_n72_), .c(new_n64_), .O(new_n78_));
  inv1   g045(.a(x13), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n44_), .O(new_n80_));
  nand3  g047(.a(new_n40_), .b(x13), .c(x05), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  nand4  g050(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n84_));
  nand4  g051(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n60_), .c(new_n56_), .O(new_n87_));
  nor2   g054(.a(new_n70_), .b(x09), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n69_), .c(new_n57_), .d(x07), .O(new_n89_));
  nor2   g056(.a(x12), .b(x11), .O(new_n90_));
  nor2   g057(.a(x16), .b(x14), .O(new_n91_));
  nor2   g058(.a(x20), .b(x17), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n76_), .O(new_n93_));
  oai21  g060(.a(new_n93_), .b(new_n89_), .c(new_n87_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n80_), .c(x24), .O(new_n95_));
  inv1   g062(.a(new_n84_), .O(new_n96_));
  inv1   g063(.a(x02), .O(new_n97_));
  nand4  g064(.a(x04), .b(x03), .c(new_n97_), .d(x01), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  nand3  g066(.a(x11), .b(new_n70_), .c(x09), .O(new_n100_));
  nor3   g067(.a(new_n100_), .b(new_n57_), .c(new_n35_), .O(new_n101_));
  inv1   g068(.a(x21), .O(new_n102_));
  nand3  g069(.a(new_n40_), .b(x23), .c(x22), .O(new_n103_));
  nor3   g070(.a(new_n103_), .b(new_n102_), .c(new_n38_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n101_), .c(new_n99_), .d(new_n96_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n95_), .c(new_n83_), .O(z1));
  inv1   g073(.a(x01), .O(new_n107_));
  nand2  g074(.a(x24), .b(x18), .O(new_n108_));
  nand3  g075(.a(new_n40_), .b(x15), .c(x13), .O(new_n109_));
  aoi21  g076(.a(new_n109_), .b(new_n108_), .c(new_n44_), .O(new_n110_));
  nand3  g077(.a(x24), .b(x18), .c(x13), .O(new_n111_));
  nand3  g078(.a(x15), .b(new_n79_), .c(new_n44_), .O(new_n112_));
  nand4  g079(.a(new_n40_), .b(x21), .c(new_n70_), .d(new_n97_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  oai21  g081(.a(new_n114_), .b(new_n110_), .c(x20), .O(new_n115_));
  nor2   g082(.a(new_n115_), .b(new_n37_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x11), .c(x06), .d(x03), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n107_), .O(z2));
  nand3  g085(.a(new_n58_), .b(x03), .c(x01), .O(new_n119_));
  nor2   g086(.a(new_n37_), .b(new_n36_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(x20), .c(x15), .O(new_n121_));
  nand3  g088(.a(new_n67_), .b(new_n57_), .c(new_n35_), .O(new_n122_));
  nand3  g089(.a(new_n75_), .b(new_n71_), .c(new_n37_), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n119_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n82_), .O(new_n125_));
  nand3  g092(.a(new_n120_), .b(x20), .c(x18), .O(new_n126_));
  nand3  g093(.a(new_n67_), .b(x07), .c(new_n35_), .O(new_n127_));
  nor2   g094(.a(x20), .b(x14), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n36_), .c(x10), .d(new_n57_), .O(new_n129_));
  oai22  g096(.a(new_n129_), .b(new_n127_), .c(new_n126_), .d(new_n119_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n80_), .c(x24), .O(new_n131_));
  nor2   g098(.a(x02), .b(new_n107_), .O(new_n132_));
  nand2  g099(.a(new_n120_), .b(new_n70_), .O(new_n133_));
  nor2   g100(.a(x24), .b(new_n102_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(x20), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n132_), .c(new_n58_), .d(x03), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n131_), .c(new_n125_), .O(z3));
  inv1   g105(.a(x22), .O(new_n139_));
  inv1   g106(.a(x17), .O(new_n140_));
  oai21  g107(.a(x23), .b(new_n66_), .c(new_n140_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n139_), .c(x09), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(x16), .c(new_n57_), .O(new_n143_));
  nand2  g110(.a(new_n37_), .b(new_n70_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n50_), .O(new_n145_));
  nor2   g112(.a(x24), .b(new_n37_), .O(new_n146_));
  nand4  g113(.a(new_n146_), .b(new_n70_), .c(new_n97_), .d(x00), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n143_), .O(new_n149_));
  inv1   g116(.a(x16), .O(new_n150_));
  inv1   g117(.a(x23), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(x04), .c(x17), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(x22), .c(new_n65_), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n150_), .c(x08), .O(new_n154_));
  nand2  g121(.a(new_n112_), .b(new_n111_), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(new_n110_), .c(new_n144_), .O(new_n156_));
  nand4  g123(.a(new_n134_), .b(x14), .c(new_n70_), .d(new_n97_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n156_), .O(z7));
  nand2  g125(.a(z7), .b(new_n154_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n149_), .O(z4));
  nand3  g127(.a(new_n40_), .b(new_n97_), .c(x00), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(x14), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n70_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(new_n49_), .c(new_n43_), .O(z5));
  oai21  g131(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n148_), .O(new_n166_));
  inv1   g133(.a(x18), .O(new_n167_));
  oai21  g134(.a(new_n36_), .b(x06), .c(x03), .O(new_n168_));
  nor2   g135(.a(x20), .b(new_n37_), .O(new_n169_));
  aoi22  g136(.a(new_n169_), .b(x11), .c(new_n168_), .d(new_n144_), .O(new_n170_));
  nor2   g137(.a(new_n38_), .b(x14), .O(new_n171_));
  nand4  g138(.a(new_n171_), .b(new_n36_), .c(x10), .d(x07), .O(new_n172_));
  oai21  g139(.a(new_n170_), .b(new_n167_), .c(new_n172_), .O(new_n173_));
  nand3  g140(.a(new_n173_), .b(new_n80_), .c(x24), .O(new_n174_));
  inv1   g141(.a(x15), .O(new_n175_));
  nor2   g142(.a(new_n170_), .b(new_n175_), .O(new_n176_));
  nand3  g143(.a(x20), .b(x19), .c(new_n37_), .O(new_n177_));
  nor3   g144(.a(new_n177_), .b(x11), .c(new_n70_), .O(new_n178_));
  oai21  g145(.a(new_n178_), .b(new_n176_), .c(new_n82_), .O(new_n179_));
  nand2  g146(.a(x20), .b(x06), .O(new_n180_));
  aoi21  g147(.a(new_n180_), .b(x11), .c(new_n34_), .O(new_n181_));
  nor3   g148(.a(new_n181_), .b(x24), .c(new_n102_), .O(new_n182_));
  nand4  g149(.a(new_n182_), .b(x14), .c(new_n70_), .d(new_n97_), .O(new_n183_));
  nand4  g150(.a(new_n183_), .b(new_n179_), .c(new_n174_), .d(new_n166_), .O(z6));
endmodule


