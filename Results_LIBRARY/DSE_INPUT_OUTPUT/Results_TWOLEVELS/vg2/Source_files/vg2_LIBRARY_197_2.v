// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  inv1   g009(.a(new_n42_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x02), .O(new_n45_));
  inv1   g012(.a(x10), .O(new_n46_));
  nand4  g013(.a(new_n40_), .b(new_n46_), .c(new_n45_), .d(x00), .O(new_n47_));
  inv1   g014(.a(x13), .O(new_n48_));
  nand3  g015(.a(x19), .b(new_n48_), .c(new_n38_), .O(new_n49_));
  and2   g016(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g017(.a(new_n50_), .b(new_n44_), .c(new_n43_), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n37_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x14), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n54_));
  nor2   g021(.a(x04), .b(new_n34_), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  oai21  g023(.a(new_n54_), .b(x01), .c(new_n56_), .O(z0));
  inv1   g024(.a(x12), .O(new_n58_));
  nand3  g025(.a(x15), .b(x13), .c(x05), .O(new_n59_));
  nand3  g026(.a(x21), .b(new_n46_), .c(new_n45_), .O(new_n60_));
  nand2  g027(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(x23), .c(x22), .d(x20), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(x17), .c(x16), .d(x14), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(x11), .c(x09), .d(x08), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n35_), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(x04), .c(x03), .d(x01), .O(new_n68_));
  inv1   g035(.a(x01), .O(new_n69_));
  inv1   g036(.a(x04), .O(new_n70_));
  inv1   g037(.a(x08), .O(new_n71_));
  inv1   g038(.a(x09), .O(new_n72_));
  inv1   g039(.a(x14), .O(new_n73_));
  inv1   g040(.a(x16), .O(new_n74_));
  inv1   g041(.a(x17), .O(new_n75_));
  inv1   g042(.a(x22), .O(new_n76_));
  inv1   g043(.a(x23), .O(new_n77_));
  nand3  g044(.a(new_n46_), .b(new_n45_), .c(x00), .O(new_n78_));
  nand3  g045(.a(x19), .b(x13), .c(x05), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n77_), .c(new_n76_), .d(new_n37_), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(x12), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n36_), .c(new_n72_), .d(new_n71_), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(x06), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n70_), .c(new_n34_), .d(new_n69_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n68_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n40_), .O(new_n89_));
  nor2   g056(.a(new_n71_), .b(new_n35_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(x11), .c(x09), .O(new_n91_));
  nor4   g058(.a(new_n91_), .b(new_n70_), .c(new_n34_), .d(new_n69_), .O(new_n92_));
  nand4  g059(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  inv1   g061(.a(x18), .O(new_n95_));
  nor2   g062(.a(new_n37_), .b(new_n95_), .O(new_n96_));
  nor2   g063(.a(new_n77_), .b(new_n76_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n96_), .c(new_n94_), .d(new_n92_), .O(new_n98_));
  nor3   g065(.a(x04), .b(x03), .c(x01), .O(new_n99_));
  nand4  g066(.a(new_n72_), .b(new_n71_), .c(x07), .d(new_n35_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand4  g068(.a(new_n74_), .b(new_n73_), .c(new_n58_), .d(new_n36_), .O(new_n102_));
  nand4  g069(.a(new_n77_), .b(new_n76_), .c(new_n37_), .d(new_n75_), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n101_), .c(new_n99_), .O(new_n105_));
  aoi22  g072(.a(new_n105_), .b(new_n98_), .c(new_n48_), .d(new_n38_), .O(new_n106_));
  nand4  g073(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n107_));
  nand3  g074(.a(new_n97_), .b(x20), .c(x17), .O(new_n108_));
  nor2   g075(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n92_), .O(new_n110_));
  nand4  g077(.a(new_n36_), .b(new_n72_), .c(new_n71_), .d(new_n35_), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  nand4  g079(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n58_), .O(new_n113_));
  nand4  g080(.a(new_n77_), .b(new_n76_), .c(new_n37_), .d(x19), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g082(.a(new_n115_), .b(new_n112_), .c(new_n99_), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n110_), .c(x13), .O(new_n117_));
  aoi22  g084(.a(new_n117_), .b(new_n38_), .c(new_n106_), .d(x24), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n89_), .O(z1));
  nor2   g086(.a(new_n40_), .b(new_n95_), .O(new_n120_));
  inv1   g087(.a(new_n120_), .O(new_n121_));
  nand3  g088(.a(new_n40_), .b(x15), .c(x13), .O(new_n122_));
  aoi21  g089(.a(new_n122_), .b(new_n121_), .c(new_n38_), .O(new_n123_));
  nand2  g090(.a(new_n120_), .b(x13), .O(new_n124_));
  nand3  g091(.a(x15), .b(new_n48_), .c(new_n38_), .O(new_n125_));
  nand4  g092(.a(new_n40_), .b(x21), .c(new_n46_), .d(new_n45_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(new_n127_));
  nor2   g094(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nor3   g095(.a(new_n128_), .b(new_n37_), .c(new_n73_), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(x11), .c(x06), .d(x01), .O(new_n130_));
  aoi21  g097(.a(new_n130_), .b(x04), .c(new_n34_), .O(z2));
  nand4  g098(.a(new_n61_), .b(x20), .c(x14), .d(x11), .O(new_n132_));
  nor2   g099(.a(new_n132_), .b(new_n71_), .O(new_n133_));
  nand4  g100(.a(new_n133_), .b(x06), .c(x03), .d(x01), .O(new_n134_));
  nand4  g101(.a(new_n80_), .b(new_n37_), .c(new_n73_), .d(new_n36_), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(x08), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n35_), .c(new_n34_), .d(new_n69_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n40_), .O(new_n139_));
  nand2  g106(.a(new_n48_), .b(new_n38_), .O(new_n140_));
  nand3  g107(.a(new_n90_), .b(x03), .c(x01), .O(new_n141_));
  nand3  g108(.a(new_n96_), .b(x14), .c(x11), .O(new_n142_));
  nor2   g109(.a(x03), .b(x01), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(x07), .c(new_n35_), .O(new_n144_));
  nand4  g111(.a(new_n37_), .b(new_n73_), .c(new_n36_), .d(new_n71_), .O(new_n145_));
  oai22  g112(.a(new_n145_), .b(new_n144_), .c(new_n142_), .d(new_n141_), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n140_), .c(x24), .O(new_n147_));
  nand3  g114(.a(new_n90_), .b(new_n38_), .c(x01), .O(new_n148_));
  nor2   g115(.a(x13), .b(new_n36_), .O(new_n149_));
  nand4  g116(.a(new_n149_), .b(x20), .c(x15), .d(x14), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(new_n148_), .c(x04), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(x03), .O(new_n152_));
  nor3   g119(.a(x08), .b(x06), .c(x05), .O(new_n153_));
  nand3  g120(.a(new_n37_), .b(x19), .c(new_n73_), .O(new_n154_));
  nor3   g121(.a(new_n154_), .b(x13), .c(x11), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n153_), .c(new_n143_), .O(new_n156_));
  nand4  g123(.a(new_n156_), .b(new_n152_), .c(new_n147_), .d(new_n139_), .O(z3));
  oai21  g124(.a(x23), .b(new_n70_), .c(new_n75_), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(new_n76_), .c(x09), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(x16), .c(new_n71_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n51_), .O(new_n161_));
  inv1   g128(.a(new_n128_), .O(new_n162_));
  oai21  g129(.a(new_n77_), .b(x04), .c(x17), .O(new_n163_));
  aoi21  g130(.a(new_n163_), .b(x22), .c(new_n72_), .O(new_n164_));
  oai21  g131(.a(new_n164_), .b(new_n74_), .c(x08), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n161_), .c(new_n56_), .O(z4));
  nand4  g134(.a(new_n56_), .b(new_n50_), .c(new_n44_), .d(new_n43_), .O(z5));
  nand2  g135(.a(x20), .b(new_n73_), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(new_n35_), .c(x11), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(x03), .c(new_n51_), .O(new_n171_));
  nand2  g138(.a(new_n37_), .b(x14), .O(new_n172_));
  aoi21  g139(.a(new_n172_), .b(x06), .c(new_n36_), .O(new_n173_));
  oai21  g140(.a(new_n173_), .b(new_n34_), .c(new_n162_), .O(new_n174_));
  nand3  g141(.a(new_n174_), .b(new_n171_), .c(new_n56_), .O(z6));
  nor2   g142(.a(new_n128_), .b(new_n55_), .O(z7));
endmodule


