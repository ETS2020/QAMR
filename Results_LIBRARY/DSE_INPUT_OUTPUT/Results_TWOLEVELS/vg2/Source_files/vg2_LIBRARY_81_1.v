// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x01), .O(z0));
  inv1   g020(.a(x14), .O(new_n54_));
  inv1   g021(.a(x00), .O(new_n55_));
  nor2   g022(.a(x01), .b(new_n55_), .O(new_n56_));
  nor2   g023(.a(x04), .b(x03), .O(new_n57_));
  nor2   g024(.a(x08), .b(x06), .O(new_n58_));
  nor2   g025(.a(x10), .b(x09), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n60_));
  nor2   g027(.a(x12), .b(x11), .O(new_n61_));
  nor2   g028(.a(x17), .b(x16), .O(new_n62_));
  nor2   g029(.a(x22), .b(x20), .O(new_n63_));
  nor2   g030(.a(x24), .b(x23), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n61_), .O(new_n65_));
  oai21  g032(.a(new_n65_), .b(new_n60_), .c(new_n54_), .O(new_n66_));
  nand2  g033(.a(new_n66_), .b(new_n44_), .O(new_n67_));
  nand3  g034(.a(x04), .b(x03), .c(x01), .O(new_n68_));
  inv1   g035(.a(new_n68_), .O(new_n69_));
  nand4  g036(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  nand4  g038(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n72_));
  nand4  g039(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g041(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  inv1   g042(.a(x09), .O(new_n76_));
  nor3   g043(.a(x04), .b(x03), .c(x01), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n58_), .c(new_n36_), .d(new_n76_), .O(new_n78_));
  nor2   g045(.a(x14), .b(x12), .O(new_n79_));
  inv1   g046(.a(x19), .O(new_n80_));
  nor2   g047(.a(x20), .b(new_n80_), .O(new_n81_));
  nor2   g048(.a(x23), .b(x22), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n81_), .c(new_n79_), .d(new_n62_), .O(new_n83_));
  oai21  g050(.a(new_n83_), .b(new_n78_), .c(new_n75_), .O(new_n84_));
  nand2  g051(.a(new_n47_), .b(new_n38_), .O(new_n85_));
  nand3  g052(.a(new_n40_), .b(x13), .c(x05), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nand4  g055(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n89_));
  nand4  g056(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n90_));
  nor2   g057(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n71_), .c(new_n69_), .O(new_n92_));
  inv1   g059(.a(x08), .O(new_n93_));
  inv1   g060(.a(x07), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(x06), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n77_), .c(new_n76_), .d(new_n93_), .O(new_n96_));
  nor2   g063(.a(x16), .b(x14), .O(new_n97_));
  nor2   g064(.a(x20), .b(x17), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n97_), .c(new_n82_), .d(new_n61_), .O(new_n99_));
  oai21  g066(.a(new_n99_), .b(new_n96_), .c(new_n92_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n85_), .c(x24), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n88_), .c(new_n67_), .O(z1));
  nand2  g069(.a(x24), .b(x18), .O(new_n103_));
  nand3  g070(.a(new_n40_), .b(x15), .c(x13), .O(new_n104_));
  aoi21  g071(.a(new_n104_), .b(new_n103_), .c(new_n38_), .O(new_n105_));
  nand3  g072(.a(x24), .b(x18), .c(x13), .O(new_n106_));
  nand3  g073(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n105_), .c(x20), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n36_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(x06), .c(x03), .d(x01), .O(new_n111_));
  aoi21  g078(.a(new_n111_), .b(x02), .c(new_n54_), .O(z2));
  nand3  g079(.a(new_n56_), .b(new_n35_), .c(new_n34_), .O(new_n113_));
  nor2   g080(.a(x10), .b(x08), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(new_n40_), .c(new_n37_), .d(new_n36_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n113_), .c(new_n54_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n44_), .O(new_n117_));
  nand4  g084(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n118_));
  nor2   g085(.a(new_n54_), .b(new_n36_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(x20), .c(x15), .O(new_n120_));
  inv1   g087(.a(x01), .O(new_n121_));
  nand3  g088(.a(new_n58_), .b(new_n34_), .c(new_n121_), .O(new_n122_));
  nand3  g089(.a(new_n81_), .b(new_n54_), .c(new_n36_), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n122_), .c(new_n120_), .d(new_n118_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n87_), .O(new_n125_));
  nand3  g092(.a(new_n119_), .b(x20), .c(x18), .O(new_n126_));
  nand3  g093(.a(new_n95_), .b(new_n34_), .c(new_n121_), .O(new_n127_));
  nand4  g094(.a(new_n37_), .b(new_n54_), .c(new_n36_), .d(new_n93_), .O(new_n128_));
  oai22  g095(.a(new_n128_), .b(new_n127_), .c(new_n126_), .d(new_n118_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n85_), .c(x24), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n125_), .c(new_n117_), .O(z3));
  inv1   g098(.a(x22), .O(new_n132_));
  inv1   g099(.a(x04), .O(new_n133_));
  inv1   g100(.a(x17), .O(new_n134_));
  oai21  g101(.a(x23), .b(new_n133_), .c(new_n134_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n132_), .c(x09), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x16), .c(new_n93_), .O(new_n137_));
  nor2   g104(.a(new_n54_), .b(x02), .O(new_n138_));
  nand2  g105(.a(new_n48_), .b(new_n43_), .O(new_n139_));
  nor2   g106(.a(new_n139_), .b(new_n42_), .O(new_n140_));
  nor2   g107(.a(x24), .b(x14), .O(new_n141_));
  nand4  g108(.a(new_n141_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n142_));
  oai21  g109(.a(new_n140_), .b(new_n138_), .c(new_n142_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n137_), .O(new_n144_));
  inv1   g111(.a(x16), .O(new_n145_));
  inv1   g112(.a(x23), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(x04), .c(x17), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(x22), .c(new_n76_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n145_), .c(x08), .O(new_n149_));
  nor2   g116(.a(new_n108_), .b(new_n105_), .O(new_n150_));
  nor2   g117(.a(x10), .b(x02), .O(new_n151_));
  nand4  g118(.a(new_n151_), .b(new_n40_), .c(x21), .d(new_n54_), .O(new_n152_));
  oai21  g119(.a(new_n138_), .b(new_n150_), .c(new_n152_), .O(z7));
  nand2  g120(.a(z7), .b(new_n149_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n144_), .O(z4));
  nand3  g122(.a(new_n40_), .b(new_n45_), .c(x00), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n54_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n44_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n140_), .O(z5));
  oai21  g126(.a(new_n36_), .b(x06), .c(x03), .O(new_n160_));
  nand3  g127(.a(new_n151_), .b(new_n40_), .c(x21), .O(new_n161_));
  nand3  g128(.a(new_n161_), .b(new_n107_), .c(new_n106_), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(new_n105_), .c(new_n160_), .O(new_n163_));
  oai21  g130(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(new_n36_), .c(x03), .O(new_n165_));
  nand3  g132(.a(new_n119_), .b(new_n37_), .c(x15), .O(new_n166_));
  oai21  g133(.a(new_n165_), .b(new_n80_), .c(new_n166_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n87_), .O(new_n168_));
  nand3  g135(.a(new_n119_), .b(new_n37_), .c(x18), .O(new_n169_));
  oai21  g136(.a(new_n165_), .b(new_n94_), .c(new_n169_), .O(new_n170_));
  nand3  g137(.a(new_n170_), .b(new_n85_), .c(x24), .O(new_n171_));
  nor2   g138(.a(x20), .b(x06), .O(new_n172_));
  oai21  g139(.a(new_n172_), .b(x11), .c(new_n34_), .O(new_n173_));
  nand4  g140(.a(new_n173_), .b(new_n40_), .c(new_n45_), .d(x00), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(new_n54_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n44_), .O(new_n176_));
  nand4  g143(.a(new_n176_), .b(new_n171_), .c(new_n168_), .d(new_n163_), .O(z6));
endmodule


