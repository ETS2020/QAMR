// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x20), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand4  g007(.a(new_n40_), .b(new_n39_), .c(x19), .d(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n38_), .c(new_n37_), .O(new_n42_));
  inv1   g009(.a(new_n42_), .O(new_n43_));
  inv1   g010(.a(x13), .O(new_n44_));
  nand3  g011(.a(x19), .b(new_n44_), .c(new_n37_), .O(new_n45_));
  inv1   g012(.a(x02), .O(new_n46_));
  inv1   g013(.a(x10), .O(new_n47_));
  nand4  g014(.a(new_n40_), .b(new_n47_), .c(new_n46_), .d(x00), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand3  g016(.a(x24), .b(x13), .c(x07), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  aoi21  g018(.a(new_n49_), .b(new_n39_), .c(new_n51_), .O(new_n52_));
  aoi21  g019(.a(new_n52_), .b(new_n43_), .c(x14), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n54_));
  nand2  g021(.a(x24), .b(x20), .O(new_n55_));
  oai21  g022(.a(new_n54_), .b(x01), .c(new_n55_), .O(z0));
  inv1   g023(.a(x01), .O(new_n57_));
  inv1   g024(.a(x04), .O(new_n58_));
  nand4  g025(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n59_));
  nor4   g026(.a(new_n59_), .b(new_n58_), .c(new_n34_), .d(new_n57_), .O(new_n60_));
  nand4  g027(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n61_));
  nand4  g028(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g030(.a(new_n63_), .b(new_n60_), .O(new_n64_));
  inv1   g031(.a(x00), .O(new_n65_));
  nor2   g032(.a(x01), .b(new_n65_), .O(new_n66_));
  inv1   g033(.a(x08), .O(new_n67_));
  inv1   g034(.a(x09), .O(new_n68_));
  nand4  g035(.a(new_n36_), .b(new_n68_), .c(new_n67_), .d(new_n35_), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n66_), .c(new_n58_), .d(new_n34_), .O(new_n71_));
  inv1   g038(.a(x22), .O(new_n72_));
  inv1   g039(.a(x12), .O(new_n73_));
  inv1   g040(.a(x14), .O(new_n74_));
  inv1   g041(.a(x16), .O(new_n75_));
  inv1   g042(.a(x17), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  nor2   g045(.a(x24), .b(x23), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n78_), .c(new_n72_), .d(new_n39_), .O(new_n80_));
  oai21  g047(.a(new_n80_), .b(new_n71_), .c(new_n64_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n47_), .c(new_n46_), .O(new_n82_));
  xnor2a g049(.a(x13), .b(x05), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(x23), .c(x22), .d(x17), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(x16), .c(x15), .d(x14), .O(new_n86_));
  nor2   g053(.a(new_n86_), .b(new_n73_), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(x11), .c(x09), .d(x08), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(new_n35_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(x04), .c(x03), .d(x01), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n40_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(x20), .O(new_n92_));
  inv1   g059(.a(x23), .O(new_n93_));
  nor2   g060(.a(x13), .b(x05), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n39_), .c(x19), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n50_), .O(new_n96_));
  oai21  g063(.a(new_n96_), .b(new_n42_), .c(new_n93_), .O(new_n97_));
  nor2   g064(.a(new_n97_), .b(x22), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(x12), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n36_), .c(new_n68_), .d(new_n67_), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(x06), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n58_), .c(new_n34_), .d(new_n57_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n92_), .c(new_n82_), .O(z1));
  nand2  g071(.a(new_n83_), .b(x15), .O(new_n105_));
  nand3  g072(.a(x21), .b(new_n47_), .c(new_n46_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n40_), .c(x20), .d(x14), .O(new_n108_));
  inv1   g075(.a(new_n108_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(x11), .c(x06), .d(x03), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n57_), .O(z2));
  nand4  g078(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n112_));
  nand4  g079(.a(x21), .b(x20), .c(x14), .d(x11), .O(new_n113_));
  nand3  g080(.a(new_n66_), .b(new_n35_), .c(new_n34_), .O(new_n114_));
  nor2   g081(.a(x24), .b(x20), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(new_n74_), .c(new_n36_), .d(new_n67_), .O(new_n116_));
  oai22  g083(.a(new_n116_), .b(new_n114_), .c(new_n113_), .d(new_n112_), .O(new_n117_));
  nand3  g084(.a(new_n117_), .b(new_n47_), .c(new_n46_), .O(new_n118_));
  nor4   g085(.a(new_n105_), .b(new_n74_), .c(new_n36_), .d(new_n67_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(x06), .c(x03), .d(x01), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n40_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(x20), .O(new_n122_));
  nand3  g089(.a(new_n95_), .b(new_n50_), .c(new_n43_), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(new_n74_), .c(new_n36_), .d(new_n67_), .O(new_n124_));
  inv1   g091(.a(new_n124_), .O(new_n125_));
  nand4  g092(.a(new_n125_), .b(new_n35_), .c(new_n34_), .d(new_n57_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n122_), .c(new_n118_), .O(z3));
  aoi21  g094(.a(new_n93_), .b(x04), .c(x17), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(x22), .c(new_n68_), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(new_n75_), .c(x08), .O(new_n130_));
  nor2   g097(.a(new_n40_), .b(x20), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(x07), .O(new_n132_));
  nand3  g099(.a(new_n40_), .b(x19), .c(x13), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n132_), .c(new_n37_), .O(new_n134_));
  nand4  g101(.a(new_n55_), .b(x19), .c(new_n44_), .d(new_n37_), .O(new_n135_));
  nand3  g102(.a(new_n131_), .b(x13), .c(x07), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n135_), .c(new_n48_), .O(new_n137_));
  nor2   g104(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  aoi21  g105(.a(x23), .b(new_n58_), .c(new_n76_), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(new_n72_), .c(x09), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(x16), .c(new_n67_), .O(new_n141_));
  nand2  g108(.a(new_n131_), .b(x18), .O(new_n142_));
  nand3  g109(.a(new_n40_), .b(x15), .c(x13), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n142_), .c(new_n37_), .O(new_n144_));
  nand4  g111(.a(new_n55_), .b(x15), .c(new_n44_), .d(new_n37_), .O(new_n145_));
  nand4  g112(.a(new_n40_), .b(x21), .c(new_n47_), .d(new_n46_), .O(new_n146_));
  nand3  g113(.a(new_n131_), .b(x18), .c(x13), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n146_), .c(new_n145_), .O(new_n148_));
  nor2   g115(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  oai22  g116(.a(new_n149_), .b(new_n141_), .c(new_n138_), .d(new_n130_), .O(z4));
  inv1   g117(.a(new_n138_), .O(z5));
  nand2  g118(.a(x20), .b(new_n74_), .O(new_n152_));
  nand2  g119(.a(new_n40_), .b(x06), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(new_n152_), .c(x11), .O(new_n154_));
  nor2   g121(.a(x24), .b(new_n34_), .O(new_n155_));
  nand3  g122(.a(new_n47_), .b(new_n46_), .c(x00), .O(new_n156_));
  nand3  g123(.a(x19), .b(x13), .c(x05), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  oai21  g125(.a(new_n155_), .b(new_n154_), .c(new_n158_), .O(new_n159_));
  nand2  g126(.a(new_n36_), .b(new_n37_), .O(new_n160_));
  nand3  g127(.a(x19), .b(new_n74_), .c(new_n44_), .O(new_n161_));
  oai21  g128(.a(new_n161_), .b(new_n160_), .c(new_n40_), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(x20), .O(new_n163_));
  oai21  g130(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n164_));
  inv1   g131(.a(new_n94_), .O(new_n165_));
  nand3  g132(.a(new_n165_), .b(x24), .c(x07), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n45_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  oai21  g135(.a(x20), .b(new_n74_), .c(x06), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(x11), .c(new_n34_), .O(new_n170_));
  nand3  g137(.a(new_n40_), .b(x13), .c(x05), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n165_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(x15), .O(new_n173_));
  aoi21  g140(.a(new_n173_), .b(new_n146_), .c(new_n170_), .O(new_n174_));
  oai21  g141(.a(x14), .b(new_n35_), .c(x11), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(x03), .O(new_n176_));
  nand4  g143(.a(new_n176_), .b(new_n165_), .c(x24), .d(x18), .O(new_n177_));
  inv1   g144(.a(new_n177_), .O(new_n178_));
  nor2   g145(.a(new_n178_), .b(new_n174_), .O(new_n179_));
  nand4  g146(.a(new_n179_), .b(new_n168_), .c(new_n163_), .d(new_n159_), .O(z6));
  inv1   g147(.a(new_n149_), .O(z7));
endmodule


