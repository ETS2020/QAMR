// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n180_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  inv1   g019(.a(x08), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  oai21  g022(.a(new_n52_), .b(x01), .c(new_n55_), .O(z0));
  inv1   g023(.a(x01), .O(new_n57_));
  nor2   g024(.a(x02), .b(new_n57_), .O(new_n58_));
  nand2  g025(.a(x04), .b(x03), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand4  g027(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(new_n63_));
  nand4  g030(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n64_));
  inv1   g031(.a(new_n64_), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  inv1   g033(.a(x23), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n65_), .c(x20), .d(x17), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nor3   g037(.a(x04), .b(x03), .c(x01), .O(new_n71_));
  nor2   g038(.a(x08), .b(x06), .O(new_n72_));
  nor2   g039(.a(x11), .b(x09), .O(new_n73_));
  nand3  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  inv1   g041(.a(x12), .O(new_n75_));
  inv1   g042(.a(x14), .O(new_n76_));
  inv1   g043(.a(x16), .O(new_n77_));
  inv1   g044(.a(x17), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  inv1   g046(.a(x19), .O(new_n80_));
  nor2   g047(.a(x20), .b(new_n80_), .O(new_n81_));
  nor2   g048(.a(x23), .b(x22), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor3   g050(.a(new_n83_), .b(new_n79_), .c(new_n74_), .O(new_n84_));
  nand2  g051(.a(new_n47_), .b(new_n38_), .O(new_n85_));
  nand3  g052(.a(new_n40_), .b(x13), .c(x05), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g054(.a(new_n84_), .b(new_n70_), .c(new_n87_), .O(new_n88_));
  nand4  g055(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  and2   g057(.a(x20), .b(x18), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n90_), .c(new_n68_), .O(new_n92_));
  inv1   g059(.a(x07), .O(new_n93_));
  nor2   g060(.a(new_n93_), .b(x06), .O(new_n94_));
  nor2   g061(.a(x09), .b(x08), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n94_), .c(new_n71_), .O(new_n96_));
  nand4  g063(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n36_), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n82_), .c(new_n37_), .d(new_n78_), .O(new_n99_));
  oai22  g066(.a(new_n99_), .b(new_n96_), .c(new_n92_), .d(new_n63_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n85_), .c(x24), .O(new_n101_));
  nor3   g068(.a(new_n61_), .b(new_n59_), .c(new_n57_), .O(new_n102_));
  inv1   g069(.a(x21), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n37_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n102_), .c(new_n90_), .d(new_n68_), .O(new_n105_));
  nor2   g072(.a(x03), .b(x01), .O(new_n106_));
  nor2   g073(.a(x06), .b(x04), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n106_), .c(new_n95_), .d(x00), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n99_), .c(new_n105_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n40_), .c(new_n45_), .d(new_n44_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n101_), .c(new_n88_), .O(z1));
  nand2  g078(.a(x08), .b(x02), .O(new_n112_));
  nand2  g079(.a(x24), .b(x18), .O(new_n113_));
  nand3  g080(.a(new_n40_), .b(x15), .c(x13), .O(new_n114_));
  aoi21  g081(.a(new_n114_), .b(new_n113_), .c(new_n38_), .O(new_n115_));
  nand3  g082(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n116_));
  oai21  g083(.a(new_n113_), .b(new_n47_), .c(new_n116_), .O(new_n117_));
  oai21  g084(.a(new_n117_), .b(new_n115_), .c(new_n112_), .O(new_n118_));
  nand4  g085(.a(new_n40_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(new_n118_), .c(new_n37_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x14), .c(x11), .d(x06), .O(new_n121_));
  nor3   g088(.a(new_n121_), .b(new_n34_), .c(new_n57_), .O(z2));
  nand3  g089(.a(new_n58_), .b(x06), .c(x03), .O(new_n123_));
  nor2   g090(.a(new_n36_), .b(new_n53_), .O(new_n124_));
  inv1   g091(.a(x15), .O(new_n125_));
  nor2   g092(.a(new_n37_), .b(new_n125_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n124_), .c(x14), .O(new_n127_));
  nor2   g094(.a(x14), .b(x11), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n106_), .c(new_n81_), .d(new_n72_), .O(new_n129_));
  oai21  g096(.a(new_n127_), .b(new_n123_), .c(new_n129_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n87_), .O(new_n131_));
  nand3  g098(.a(new_n124_), .b(new_n91_), .c(x14), .O(new_n132_));
  nand2  g099(.a(new_n106_), .b(new_n94_), .O(new_n133_));
  nand4  g100(.a(new_n37_), .b(new_n76_), .c(new_n36_), .d(new_n53_), .O(new_n134_));
  oai22  g101(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n123_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n85_), .c(x24), .O(new_n136_));
  nand4  g103(.a(new_n35_), .b(new_n34_), .c(new_n57_), .d(x00), .O(new_n137_));
  nand4  g104(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n138_));
  nand3  g105(.a(new_n104_), .b(x14), .c(x11), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n138_), .c(new_n137_), .d(new_n134_), .O(new_n140_));
  nand4  g107(.a(new_n140_), .b(new_n40_), .c(new_n45_), .d(new_n44_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n136_), .c(new_n131_), .O(z3));
  inv1   g109(.a(x09), .O(new_n143_));
  aoi21  g110(.a(new_n67_), .b(x04), .c(x17), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x22), .c(new_n143_), .O(new_n145_));
  nand2  g112(.a(new_n48_), .b(new_n43_), .O(new_n146_));
  nor2   g113(.a(new_n146_), .b(new_n42_), .O(new_n147_));
  oai21  g114(.a(new_n147_), .b(x08), .c(new_n46_), .O(new_n148_));
  nand3  g115(.a(new_n148_), .b(new_n145_), .c(new_n77_), .O(new_n149_));
  inv1   g116(.a(x04), .O(new_n150_));
  aoi21  g117(.a(x23), .b(new_n150_), .c(new_n78_), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(new_n66_), .c(x09), .O(new_n152_));
  nand3  g119(.a(new_n152_), .b(x18), .c(x16), .O(new_n153_));
  nand2  g120(.a(x08), .b(x07), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n44_), .O(new_n156_));
  nand2  g123(.a(x18), .b(new_n53_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nand3  g125(.a(new_n158_), .b(new_n85_), .c(x24), .O(new_n159_));
  nand3  g126(.a(new_n152_), .b(x16), .c(x15), .O(new_n160_));
  nand2  g127(.a(x19), .b(x08), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n160_), .c(x02), .O(new_n162_));
  nor2   g129(.a(new_n125_), .b(x08), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(new_n162_), .c(new_n87_), .O(new_n164_));
  aoi21  g131(.a(new_n152_), .b(x16), .c(new_n53_), .O(new_n165_));
  nand2  g132(.a(x08), .b(x00), .O(new_n166_));
  oai21  g133(.a(new_n165_), .b(new_n103_), .c(new_n166_), .O(new_n167_));
  nand4  g134(.a(new_n167_), .b(new_n40_), .c(new_n45_), .d(new_n44_), .O(new_n168_));
  nand4  g135(.a(new_n168_), .b(new_n164_), .c(new_n159_), .d(new_n149_), .O(z4));
  inv1   g136(.a(new_n112_), .O(new_n170_));
  oai21  g137(.a(new_n147_), .b(new_n170_), .c(new_n46_), .O(z5));
  aoi21  g138(.a(x20), .b(new_n76_), .c(x06), .O(new_n172_));
  oai21  g139(.a(new_n172_), .b(x11), .c(new_n34_), .O(new_n173_));
  oai21  g140(.a(new_n147_), .b(new_n54_), .c(new_n46_), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(new_n173_), .O(new_n175_));
  aoi21  g142(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n176_));
  oai21  g143(.a(new_n176_), .b(new_n36_), .c(x03), .O(new_n177_));
  oai21  g144(.a(new_n117_), .b(new_n115_), .c(new_n55_), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n119_), .O(z7));
  nand2  g146(.a(z7), .b(new_n177_), .O(new_n180_));
  nand2  g147(.a(new_n180_), .b(new_n175_), .O(z6));
endmodule


