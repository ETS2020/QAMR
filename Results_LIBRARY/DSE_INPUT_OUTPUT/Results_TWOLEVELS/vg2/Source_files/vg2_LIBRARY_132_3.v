// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n172_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x01), .O(z0));
  inv1   g020(.a(x04), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n34_), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n55_), .c(new_n44_), .d(x01), .O(new_n58_));
  nand4  g025(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  inv1   g027(.a(x22), .O(new_n61_));
  inv1   g028(.a(x23), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n60_), .c(x20), .d(x17), .O(new_n64_));
  nor3   g031(.a(x04), .b(x03), .c(x01), .O(new_n65_));
  inv1   g032(.a(x08), .O(new_n66_));
  inv1   g033(.a(x09), .O(new_n67_));
  nand4  g034(.a(new_n36_), .b(new_n67_), .c(new_n66_), .d(new_n35_), .O(new_n68_));
  inv1   g035(.a(new_n68_), .O(new_n69_));
  inv1   g036(.a(x12), .O(new_n70_));
  inv1   g037(.a(x14), .O(new_n71_));
  inv1   g038(.a(x16), .O(new_n72_));
  inv1   g039(.a(x17), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nor2   g041(.a(x23), .b(x22), .O(new_n75_));
  nand3  g042(.a(new_n75_), .b(new_n37_), .c(x19), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n69_), .c(new_n65_), .O(new_n78_));
  oai21  g045(.a(new_n64_), .b(new_n58_), .c(new_n78_), .O(new_n79_));
  nand2  g046(.a(new_n47_), .b(new_n38_), .O(new_n80_));
  nand3  g047(.a(new_n40_), .b(x13), .c(x05), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand4  g050(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n63_), .c(x20), .d(x18), .O(new_n86_));
  nor2   g053(.a(x09), .b(x08), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n65_), .c(x07), .d(new_n35_), .O(new_n88_));
  nand4  g055(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n36_), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n75_), .c(new_n37_), .d(new_n73_), .O(new_n91_));
  oai22  g058(.a(new_n91_), .b(new_n88_), .c(new_n86_), .d(new_n58_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n80_), .c(x24), .O(new_n93_));
  nand2  g060(.a(new_n55_), .b(x01), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(new_n95_));
  nand3  g062(.a(new_n63_), .b(x21), .c(x20), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n95_), .c(new_n85_), .d(new_n57_), .O(new_n98_));
  nor2   g065(.a(x03), .b(x01), .O(new_n99_));
  nor2   g066(.a(x06), .b(x04), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n99_), .c(new_n87_), .d(x00), .O(new_n101_));
  oai21  g068(.a(new_n101_), .b(new_n91_), .c(new_n98_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n40_), .c(new_n45_), .d(new_n44_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n93_), .c(new_n83_), .O(z1));
  inv1   g071(.a(x01), .O(new_n105_));
  nand2  g072(.a(x24), .b(x18), .O(new_n106_));
  nand3  g073(.a(new_n40_), .b(x15), .c(x13), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(x05), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand3  g077(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n111_));
  nand3  g078(.a(new_n40_), .b(x21), .c(new_n45_), .O(new_n112_));
  nand3  g079(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n109_), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(x20), .c(x14), .d(x11), .O(new_n116_));
  inv1   g083(.a(new_n116_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(x06), .c(x03), .d(new_n44_), .O(new_n118_));
  nor2   g085(.a(new_n118_), .b(new_n105_), .O(z2));
  nand4  g086(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n120_));
  nor2   g087(.a(new_n71_), .b(new_n36_), .O(new_n121_));
  nand3  g088(.a(new_n121_), .b(x20), .c(x15), .O(new_n122_));
  nand3  g089(.a(new_n99_), .b(new_n66_), .c(new_n35_), .O(new_n123_));
  nand4  g090(.a(new_n37_), .b(x19), .c(new_n71_), .d(new_n36_), .O(new_n124_));
  oai22  g091(.a(new_n124_), .b(new_n123_), .c(new_n122_), .d(new_n120_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n82_), .O(new_n126_));
  nand3  g093(.a(new_n121_), .b(x20), .c(x18), .O(new_n127_));
  nand3  g094(.a(new_n99_), .b(x07), .c(new_n35_), .O(new_n128_));
  nand4  g095(.a(new_n37_), .b(new_n71_), .c(new_n36_), .d(new_n66_), .O(new_n129_));
  oai22  g096(.a(new_n129_), .b(new_n128_), .c(new_n127_), .d(new_n120_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n80_), .c(x24), .O(new_n131_));
  nand4  g098(.a(new_n45_), .b(x08), .c(x06), .d(x03), .O(new_n132_));
  inv1   g099(.a(x21), .O(new_n133_));
  nor2   g100(.a(x24), .b(new_n133_), .O(new_n134_));
  nand3  g101(.a(new_n121_), .b(new_n134_), .c(x20), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(new_n132_), .c(new_n44_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(x01), .O(new_n137_));
  inv1   g104(.a(x00), .O(new_n138_));
  nor2   g105(.a(x01), .b(new_n138_), .O(new_n139_));
  nor3   g106(.a(x06), .b(x03), .c(x02), .O(new_n140_));
  nor3   g107(.a(x11), .b(x10), .c(x08), .O(new_n141_));
  nor3   g108(.a(x24), .b(x20), .c(x14), .O(new_n142_));
  nand4  g109(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(new_n139_), .O(new_n143_));
  nand4  g110(.a(new_n143_), .b(new_n137_), .c(new_n131_), .d(new_n126_), .O(z3));
  oai21  g111(.a(x23), .b(new_n54_), .c(new_n73_), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(new_n61_), .c(x09), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(x16), .c(new_n66_), .O(new_n147_));
  nand2  g114(.a(x02), .b(x01), .O(new_n148_));
  nand2  g115(.a(new_n48_), .b(new_n43_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n42_), .c(new_n148_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n46_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  oai21  g119(.a(new_n62_), .b(x04), .c(x17), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(x22), .c(new_n67_), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(new_n72_), .c(x08), .O(new_n155_));
  nand3  g122(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n148_), .O(new_n157_));
  nand3  g124(.a(new_n134_), .b(new_n45_), .c(new_n44_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n152_), .O(z4));
  inv1   g128(.a(new_n42_), .O(new_n162_));
  and2   g129(.a(new_n48_), .b(new_n46_), .O(new_n163_));
  nand4  g130(.a(new_n148_), .b(new_n163_), .c(new_n43_), .d(new_n162_), .O(z5));
  aoi21  g131(.a(x20), .b(new_n71_), .c(x06), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(x11), .c(new_n34_), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n151_), .O(new_n167_));
  aoi21  g134(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n168_));
  oai21  g135(.a(new_n168_), .b(new_n36_), .c(x03), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n159_), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n167_), .O(z6));
  and2   g138(.a(new_n148_), .b(new_n110_), .O(new_n172_));
  nand4  g139(.a(new_n172_), .b(new_n158_), .c(new_n111_), .d(new_n109_), .O(z7));
endmodule


