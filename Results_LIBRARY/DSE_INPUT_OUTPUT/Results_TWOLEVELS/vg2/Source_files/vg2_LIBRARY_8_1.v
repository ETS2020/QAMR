// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x01), .O(z0));
  nand3  g020(.a(x04), .b(x03), .c(x01), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n58_));
  nand4  g025(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n57_), .c(new_n55_), .O(new_n61_));
  inv1   g028(.a(x09), .O(new_n62_));
  nor3   g029(.a(x04), .b(x03), .c(x01), .O(new_n63_));
  nor2   g030(.a(x08), .b(x06), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n63_), .c(new_n36_), .d(new_n62_), .O(new_n65_));
  nor2   g032(.a(x14), .b(x12), .O(new_n66_));
  nor2   g033(.a(x17), .b(x16), .O(new_n67_));
  inv1   g034(.a(x19), .O(new_n68_));
  nor2   g035(.a(x20), .b(new_n68_), .O(new_n69_));
  nor2   g036(.a(x23), .b(x22), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n67_), .d(new_n66_), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n65_), .c(new_n61_), .O(new_n72_));
  nand2  g039(.a(new_n47_), .b(new_n38_), .O(new_n73_));
  nand3  g040(.a(new_n40_), .b(x13), .c(x05), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand4  g043(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n77_));
  inv1   g044(.a(x18), .O(new_n78_));
  nor2   g045(.a(new_n37_), .b(new_n78_), .O(new_n79_));
  nand3  g046(.a(new_n79_), .b(x23), .c(x22), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n57_), .c(new_n55_), .O(new_n82_));
  inv1   g049(.a(x07), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(x06), .O(new_n84_));
  nor2   g051(.a(x09), .b(x08), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n84_), .c(new_n63_), .O(new_n86_));
  inv1   g053(.a(x17), .O(new_n87_));
  inv1   g054(.a(x12), .O(new_n88_));
  inv1   g055(.a(x14), .O(new_n89_));
  inv1   g056(.a(x16), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(new_n36_), .O(new_n91_));
  inv1   g058(.a(new_n91_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n70_), .c(new_n37_), .d(new_n87_), .O(new_n93_));
  oai21  g060(.a(new_n93_), .b(new_n86_), .c(new_n82_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n73_), .c(x24), .O(new_n95_));
  inv1   g062(.a(x00), .O(new_n96_));
  nor2   g063(.a(x01), .b(new_n96_), .O(new_n97_));
  nor2   g064(.a(x03), .b(x02), .O(new_n98_));
  nor2   g065(.a(x06), .b(x04), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n85_), .O(new_n100_));
  inv1   g067(.a(x22), .O(new_n101_));
  nor2   g068(.a(x24), .b(x23), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n92_), .c(new_n101_), .d(new_n87_), .O(new_n103_));
  oai21  g070(.a(new_n103_), .b(new_n100_), .c(new_n37_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n45_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n95_), .c(new_n76_), .O(z1));
  inv1   g073(.a(x01), .O(new_n107_));
  nand2  g074(.a(x24), .b(x18), .O(new_n108_));
  nand3  g075(.a(new_n40_), .b(x15), .c(x13), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(x05), .O(new_n111_));
  nand3  g078(.a(x24), .b(x18), .c(x13), .O(new_n112_));
  inv1   g079(.a(new_n112_), .O(new_n113_));
  inv1   g080(.a(x15), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(x13), .O(new_n115_));
  aoi21  g082(.a(new_n115_), .b(new_n38_), .c(new_n113_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n111_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(x20), .c(x14), .d(x11), .O(new_n118_));
  inv1   g085(.a(new_n118_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(x10), .c(x06), .d(x03), .O(new_n120_));
  nor2   g087(.a(new_n120_), .b(new_n107_), .O(z2));
  nand4  g088(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n122_));
  nand2  g089(.a(x11), .b(x10), .O(new_n123_));
  nand3  g090(.a(x20), .b(x15), .c(x14), .O(new_n124_));
  nor3   g091(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(new_n125_));
  nor2   g092(.a(x03), .b(x01), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  nand3  g094(.a(new_n69_), .b(new_n89_), .c(new_n36_), .O(new_n128_));
  nor2   g095(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g096(.a(new_n129_), .b(new_n125_), .c(new_n75_), .O(new_n130_));
  inv1   g097(.a(new_n123_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n79_), .c(x14), .O(new_n132_));
  nor2   g099(.a(x11), .b(x08), .O(new_n133_));
  nor2   g100(.a(x20), .b(x14), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n133_), .c(new_n126_), .d(new_n84_), .O(new_n135_));
  oai21  g102(.a(new_n132_), .b(new_n122_), .c(new_n135_), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n73_), .c(x24), .O(new_n137_));
  nor3   g104(.a(x06), .b(x03), .c(x02), .O(new_n138_));
  nor3   g105(.a(x11), .b(x10), .c(x08), .O(new_n139_));
  nor3   g106(.a(x24), .b(x20), .c(x14), .O(new_n140_));
  nand4  g107(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n97_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n137_), .c(new_n130_), .O(z3));
  inv1   g109(.a(x04), .O(new_n143_));
  oai21  g110(.a(x23), .b(new_n143_), .c(new_n87_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n101_), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(new_n62_), .c(x16), .O(new_n146_));
  inv1   g113(.a(new_n42_), .O(new_n147_));
  nand2  g114(.a(new_n48_), .b(new_n43_), .O(new_n148_));
  inv1   g115(.a(new_n148_), .O(new_n149_));
  aoi22  g116(.a(new_n149_), .b(new_n147_), .c(x20), .d(new_n45_), .O(new_n150_));
  nand3  g117(.a(new_n40_), .b(new_n37_), .c(new_n45_), .O(new_n151_));
  nor3   g118(.a(new_n151_), .b(x02), .c(new_n96_), .O(new_n152_));
  oai22  g119(.a(new_n152_), .b(new_n150_), .c(new_n146_), .d(x08), .O(new_n153_));
  inv1   g120(.a(x23), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(x04), .c(x17), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(x22), .c(new_n62_), .O(new_n156_));
  oai21  g123(.a(new_n156_), .b(new_n90_), .c(x08), .O(new_n157_));
  aoi22  g124(.a(new_n116_), .b(new_n111_), .c(x20), .d(new_n45_), .O(new_n158_));
  nand3  g125(.a(new_n40_), .b(x21), .c(new_n37_), .O(new_n159_));
  nor3   g126(.a(new_n159_), .b(x10), .c(x02), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n158_), .c(new_n157_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n153_), .O(z4));
  nand3  g129(.a(new_n40_), .b(new_n44_), .c(x00), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n37_), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(new_n45_), .c(new_n148_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n147_), .O(z5));
  oai21  g133(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n167_));
  oai21  g134(.a(new_n49_), .b(new_n42_), .c(new_n167_), .O(new_n168_));
  oai21  g135(.a(x20), .b(new_n89_), .c(x06), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(x11), .c(new_n34_), .O(new_n170_));
  nand4  g137(.a(x20), .b(new_n89_), .c(new_n36_), .d(x07), .O(new_n171_));
  oai21  g138(.a(new_n170_), .b(new_n78_), .c(new_n171_), .O(new_n172_));
  nand3  g139(.a(new_n172_), .b(new_n73_), .c(x24), .O(new_n173_));
  nand4  g140(.a(x20), .b(x19), .c(new_n89_), .d(new_n36_), .O(new_n174_));
  oai21  g141(.a(new_n170_), .b(new_n114_), .c(new_n174_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n75_), .O(new_n176_));
  oai21  g143(.a(x14), .b(new_n35_), .c(x11), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(x03), .O(new_n178_));
  nand4  g145(.a(new_n178_), .b(new_n40_), .c(x21), .d(new_n44_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n37_), .O(new_n180_));
  nand2  g147(.a(new_n180_), .b(new_n45_), .O(new_n181_));
  nand4  g148(.a(new_n181_), .b(new_n176_), .c(new_n173_), .d(new_n168_), .O(z6));
  nand3  g149(.a(new_n40_), .b(x21), .c(new_n44_), .O(new_n183_));
  nand2  g150(.a(new_n183_), .b(new_n37_), .O(new_n184_));
  nand2  g151(.a(new_n184_), .b(new_n45_), .O(new_n185_));
  nand3  g152(.a(new_n185_), .b(new_n116_), .c(new_n111_), .O(z7));
endmodule


