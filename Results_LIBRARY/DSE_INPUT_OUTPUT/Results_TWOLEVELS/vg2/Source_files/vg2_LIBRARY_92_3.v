// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_;
  inv1   g000(.a(x01), .O(new_n34_));
  inv1   g001(.a(x03), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x14), .O(new_n38_));
  inv1   g005(.a(x20), .O(new_n39_));
  inv1   g006(.a(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nor2   g008(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  inv1   g009(.a(new_n42_), .O(new_n43_));
  nand3  g010(.a(new_n41_), .b(x19), .c(x13), .O(new_n44_));
  oai21  g011(.a(new_n43_), .b(x02), .c(new_n44_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(x05), .O(new_n46_));
  inv1   g013(.a(x05), .O(new_n47_));
  inv1   g014(.a(x13), .O(new_n48_));
  nand2  g015(.a(x24), .b(x02), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(x19), .c(new_n48_), .d(new_n47_), .O(new_n50_));
  inv1   g017(.a(x02), .O(new_n51_));
  nand3  g018(.a(x24), .b(x13), .c(x07), .O(new_n52_));
  inv1   g019(.a(x10), .O(new_n53_));
  nand3  g020(.a(new_n41_), .b(new_n53_), .c(x00), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g022(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand3  g023(.a(new_n56_), .b(new_n50_), .c(new_n46_), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(z0));
  inv1   g028(.a(x12), .O(new_n62_));
  nand3  g029(.a(x15), .b(x13), .c(x05), .O(new_n63_));
  nand3  g030(.a(x21), .b(new_n53_), .c(new_n51_), .O(new_n64_));
  nand2  g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(x23), .c(x22), .d(x20), .O(new_n66_));
  inv1   g033(.a(new_n66_), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(x17), .c(x16), .d(x14), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(x11), .c(x09), .d(x08), .O(new_n70_));
  nor2   g037(.a(new_n70_), .b(new_n36_), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(x04), .c(x03), .d(x01), .O(new_n72_));
  inv1   g039(.a(x04), .O(new_n73_));
  inv1   g040(.a(x08), .O(new_n74_));
  inv1   g041(.a(x09), .O(new_n75_));
  inv1   g042(.a(x16), .O(new_n76_));
  inv1   g043(.a(x17), .O(new_n77_));
  inv1   g044(.a(x22), .O(new_n78_));
  inv1   g045(.a(x23), .O(new_n79_));
  nand3  g046(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n80_));
  nand3  g047(.a(x19), .b(x13), .c(x05), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n79_), .c(new_n78_), .d(new_n39_), .O(new_n83_));
  inv1   g050(.a(new_n83_), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n77_), .c(new_n76_), .d(new_n38_), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(x12), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n37_), .c(new_n75_), .d(new_n74_), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(x06), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n73_), .c(new_n35_), .d(new_n34_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n72_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n41_), .O(new_n91_));
  nor2   g058(.a(x13), .b(x05), .O(new_n92_));
  nand4  g059(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n93_));
  nor4   g060(.a(new_n93_), .b(new_n73_), .c(new_n35_), .d(new_n34_), .O(new_n94_));
  nand4  g061(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n95_));
  nand4  g062(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g064(.a(new_n73_), .b(new_n35_), .c(new_n34_), .O(new_n98_));
  nand4  g065(.a(new_n75_), .b(new_n74_), .c(x07), .d(new_n36_), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g067(.a(new_n76_), .b(new_n38_), .c(new_n62_), .d(new_n37_), .O(new_n101_));
  nor2   g068(.a(x23), .b(x22), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n39_), .c(new_n77_), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi22  g071(.a(new_n104_), .b(new_n100_), .c(new_n97_), .d(new_n94_), .O(new_n105_));
  oai21  g072(.a(new_n105_), .b(new_n92_), .c(new_n51_), .O(new_n106_));
  nand4  g073(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n107_));
  nand4  g074(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n108_));
  nor2   g075(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n94_), .O(new_n110_));
  inv1   g077(.a(new_n98_), .O(new_n111_));
  nor2   g078(.a(x08), .b(x06), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(new_n111_), .c(new_n37_), .d(new_n75_), .O(new_n113_));
  nor2   g080(.a(x14), .b(x12), .O(new_n114_));
  nor2   g081(.a(x17), .b(x16), .O(new_n115_));
  inv1   g082(.a(x19), .O(new_n116_));
  nor2   g083(.a(x20), .b(new_n116_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(new_n115_), .c(new_n114_), .d(new_n102_), .O(new_n118_));
  oai21  g085(.a(new_n118_), .b(new_n113_), .c(new_n110_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n48_), .c(new_n47_), .O(new_n120_));
  inv1   g087(.a(new_n120_), .O(new_n121_));
  aoi21  g088(.a(new_n106_), .b(x24), .c(new_n121_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n91_), .O(z1));
  nand2  g090(.a(x24), .b(x18), .O(new_n124_));
  nand3  g091(.a(new_n41_), .b(x15), .c(x13), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(new_n124_), .c(new_n47_), .O(new_n126_));
  nand3  g093(.a(x24), .b(x18), .c(x13), .O(new_n127_));
  nand3  g094(.a(x15), .b(new_n48_), .c(new_n47_), .O(new_n128_));
  nand4  g095(.a(new_n41_), .b(x21), .c(new_n53_), .d(new_n51_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n128_), .c(new_n127_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n126_), .c(x20), .O(new_n131_));
  nor2   g098(.a(new_n131_), .b(new_n38_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(x11), .c(x06), .d(x03), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n34_), .c(new_n49_), .O(z2));
  inv1   g101(.a(new_n92_), .O(new_n135_));
  nand4  g102(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n136_));
  nand4  g103(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n137_));
  nand4  g104(.a(x07), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n138_));
  nand4  g105(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n74_), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n138_), .c(new_n137_), .d(new_n136_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n135_), .c(x24), .O(new_n141_));
  nand3  g108(.a(new_n41_), .b(x21), .c(new_n53_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n128_), .O(new_n143_));
  nand4  g110(.a(new_n143_), .b(x20), .c(x14), .d(x11), .O(new_n144_));
  nor2   g111(.a(new_n144_), .b(new_n74_), .O(new_n145_));
  nand4  g112(.a(new_n145_), .b(x06), .c(x03), .d(x01), .O(new_n146_));
  nand3  g113(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n54_), .O(new_n148_));
  nand4  g115(.a(new_n148_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n149_));
  nor2   g116(.a(new_n149_), .b(x08), .O(new_n150_));
  nand4  g117(.a(new_n150_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(new_n146_), .c(new_n141_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n51_), .O(new_n153_));
  nand2  g120(.a(x13), .b(x05), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n135_), .O(new_n155_));
  nand4  g122(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n156_));
  nand3  g123(.a(new_n112_), .b(new_n35_), .c(new_n34_), .O(new_n157_));
  nand3  g124(.a(new_n117_), .b(new_n38_), .c(new_n37_), .O(new_n158_));
  oai22  g125(.a(new_n158_), .b(new_n157_), .c(new_n156_), .d(new_n136_), .O(new_n159_));
  nand3  g126(.a(new_n159_), .b(new_n155_), .c(new_n41_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n153_), .O(z3));
  oai21  g128(.a(x23), .b(new_n73_), .c(new_n77_), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(new_n78_), .c(x09), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(x16), .c(new_n74_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n57_), .O(new_n165_));
  oai21  g132(.a(new_n79_), .b(x04), .c(x17), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(x22), .c(new_n75_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n76_), .c(x08), .O(new_n168_));
  oai21  g135(.a(new_n124_), .b(x02), .c(new_n125_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(x05), .O(new_n170_));
  nand4  g137(.a(new_n49_), .b(x15), .c(new_n48_), .d(new_n47_), .O(new_n171_));
  nand2  g138(.a(new_n142_), .b(new_n127_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(new_n51_), .O(new_n173_));
  nand3  g140(.a(new_n173_), .b(new_n171_), .c(new_n170_), .O(z7));
  nand2  g141(.a(z7), .b(new_n168_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n165_), .O(z4));
  aoi21  g143(.a(new_n44_), .b(new_n43_), .c(new_n47_), .O(new_n177_));
  inv1   g144(.a(new_n177_), .O(new_n178_));
  oai21  g145(.a(new_n48_), .b(new_n40_), .c(new_n51_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(x24), .O(new_n180_));
  nand4  g147(.a(new_n41_), .b(new_n53_), .c(new_n51_), .d(x00), .O(new_n181_));
  and2   g148(.a(new_n181_), .b(new_n147_), .O(new_n182_));
  nand3  g149(.a(new_n182_), .b(new_n180_), .c(new_n178_), .O(z5));
  nand2  g150(.a(x20), .b(new_n38_), .O(new_n184_));
  aoi21  g151(.a(new_n184_), .b(new_n36_), .c(x11), .O(new_n185_));
  nand2  g152(.a(new_n182_), .b(new_n52_), .O(new_n186_));
  oai22  g153(.a(new_n186_), .b(new_n177_), .c(new_n185_), .d(x03), .O(new_n187_));
  nand2  g154(.a(new_n39_), .b(x14), .O(new_n188_));
  aoi21  g155(.a(new_n188_), .b(x06), .c(new_n37_), .O(new_n189_));
  oai22  g156(.a(new_n189_), .b(new_n35_), .c(new_n130_), .d(new_n126_), .O(new_n190_));
  nand3  g157(.a(new_n190_), .b(new_n187_), .c(new_n49_), .O(z6));
endmodule


