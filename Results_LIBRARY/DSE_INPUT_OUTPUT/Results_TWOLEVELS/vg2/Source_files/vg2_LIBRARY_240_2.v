// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x09), .O(new_n34_));
  inv1   g001(.a(x01), .O(new_n35_));
  inv1   g002(.a(x03), .O(new_n36_));
  inv1   g003(.a(x06), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nor3   g017(.a(new_n50_), .b(x20), .c(x11), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n52_));
  aoi21  g019(.a(new_n52_), .b(new_n34_), .c(x14), .O(z0));
  inv1   g020(.a(x12), .O(new_n54_));
  nand3  g021(.a(x15), .b(x13), .c(x05), .O(new_n55_));
  nand3  g022(.a(x21), .b(new_n45_), .c(new_n44_), .O(new_n56_));
  nand2  g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(x23), .c(x22), .d(x20), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(x17), .c(x16), .d(x14), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(x11), .c(x09), .d(x08), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n37_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(x04), .c(x03), .d(x01), .O(new_n64_));
  inv1   g031(.a(x04), .O(new_n65_));
  inv1   g032(.a(x08), .O(new_n66_));
  inv1   g033(.a(x11), .O(new_n67_));
  inv1   g034(.a(x14), .O(new_n68_));
  inv1   g035(.a(x16), .O(new_n69_));
  inv1   g036(.a(x17), .O(new_n70_));
  inv1   g037(.a(x20), .O(new_n71_));
  inv1   g038(.a(x22), .O(new_n72_));
  inv1   g039(.a(x23), .O(new_n73_));
  nand3  g040(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n74_));
  nand3  g041(.a(x19), .b(x13), .c(x05), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(x12), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n67_), .c(new_n34_), .d(new_n66_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(x06), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n65_), .c(new_n36_), .d(new_n35_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n64_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n40_), .O(new_n85_));
  nor2   g052(.a(x13), .b(x05), .O(new_n86_));
  nand4  g053(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n87_));
  nor4   g054(.a(new_n87_), .b(new_n65_), .c(new_n36_), .d(new_n35_), .O(new_n88_));
  nand4  g055(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  inv1   g057(.a(x18), .O(new_n91_));
  nor2   g058(.a(new_n71_), .b(new_n91_), .O(new_n92_));
  nor2   g059(.a(new_n73_), .b(new_n72_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n92_), .c(new_n90_), .d(new_n88_), .O(new_n94_));
  nor3   g061(.a(x04), .b(x03), .c(x01), .O(new_n95_));
  nor2   g062(.a(x09), .b(x08), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(x07), .c(new_n37_), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand4  g065(.a(new_n69_), .b(new_n68_), .c(new_n54_), .d(new_n67_), .O(new_n99_));
  nand4  g066(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n98_), .c(new_n95_), .O(new_n102_));
  aoi21  g069(.a(new_n102_), .b(new_n94_), .c(new_n86_), .O(new_n103_));
  nand4  g070(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n104_));
  nand3  g071(.a(new_n93_), .b(x20), .c(x17), .O(new_n105_));
  nor2   g072(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  nor2   g074(.a(x08), .b(x06), .O(new_n108_));
  nor2   g075(.a(x11), .b(x09), .O(new_n109_));
  nand4  g076(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n54_), .O(new_n110_));
  nand4  g077(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x19), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(new_n109_), .c(new_n108_), .d(new_n95_), .O(new_n113_));
  aoi21  g080(.a(new_n113_), .b(new_n107_), .c(x13), .O(new_n114_));
  aoi22  g081(.a(new_n114_), .b(new_n38_), .c(new_n103_), .d(x24), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n85_), .O(z1));
  nor2   g083(.a(new_n40_), .b(new_n91_), .O(new_n117_));
  inv1   g084(.a(new_n117_), .O(new_n118_));
  nand3  g085(.a(new_n40_), .b(x15), .c(x13), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(new_n118_), .c(new_n38_), .O(new_n120_));
  nand2  g087(.a(new_n117_), .b(x13), .O(new_n121_));
  nand3  g088(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n122_));
  nand4  g089(.a(new_n40_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n123_));
  nand3  g090(.a(new_n123_), .b(new_n122_), .c(new_n121_), .O(new_n124_));
  nor2   g091(.a(new_n124_), .b(new_n120_), .O(new_n125_));
  nor3   g092(.a(new_n125_), .b(new_n71_), .c(new_n68_), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(x11), .c(x06), .d(x03), .O(new_n127_));
  nor2   g094(.a(x14), .b(new_n34_), .O(new_n128_));
  inv1   g095(.a(new_n128_), .O(new_n129_));
  oai21  g096(.a(new_n127_), .b(new_n35_), .c(new_n129_), .O(z2));
  nand4  g097(.a(new_n57_), .b(x20), .c(x14), .d(x11), .O(new_n131_));
  nor2   g098(.a(new_n131_), .b(new_n66_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(x06), .c(x03), .d(x01), .O(new_n133_));
  nand4  g100(.a(new_n76_), .b(new_n71_), .c(new_n68_), .d(new_n67_), .O(new_n134_));
  nor2   g101(.a(new_n134_), .b(x09), .O(new_n135_));
  nand4  g102(.a(new_n135_), .b(new_n66_), .c(new_n37_), .d(new_n36_), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x01), .c(new_n133_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n40_), .O(new_n138_));
  inv1   g105(.a(new_n86_), .O(new_n139_));
  nand4  g106(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n140_));
  nor2   g107(.a(new_n68_), .b(new_n67_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n92_), .O(new_n142_));
  nand4  g109(.a(x07), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n143_));
  nand4  g110(.a(new_n96_), .b(new_n71_), .c(new_n68_), .d(new_n67_), .O(new_n144_));
  oai22  g111(.a(new_n144_), .b(new_n143_), .c(new_n142_), .d(new_n140_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n139_), .c(x24), .O(new_n146_));
  nand3  g113(.a(new_n141_), .b(x20), .c(x15), .O(new_n147_));
  nand3  g114(.a(new_n108_), .b(new_n36_), .c(new_n35_), .O(new_n148_));
  nand4  g115(.a(new_n109_), .b(new_n71_), .c(x19), .d(new_n68_), .O(new_n149_));
  oai22  g116(.a(new_n149_), .b(new_n148_), .c(new_n147_), .d(new_n140_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n47_), .c(new_n38_), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(new_n146_), .c(new_n138_), .O(z3));
  inv1   g119(.a(x21), .O(new_n153_));
  nand2  g120(.a(x08), .b(x00), .O(new_n154_));
  oai21  g121(.a(new_n153_), .b(x08), .c(new_n154_), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n45_), .c(new_n44_), .O(new_n156_));
  inv1   g123(.a(x15), .O(new_n157_));
  nand2  g124(.a(x19), .b(x08), .O(new_n158_));
  oai21  g125(.a(new_n157_), .b(x08), .c(new_n158_), .O(new_n159_));
  nand3  g126(.a(new_n159_), .b(x13), .c(x05), .O(new_n160_));
  aoi21  g127(.a(new_n160_), .b(new_n156_), .c(x24), .O(new_n161_));
  nand2  g128(.a(x08), .b(x07), .O(new_n162_));
  oai21  g129(.a(new_n91_), .b(x08), .c(new_n162_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(new_n139_), .c(x24), .O(new_n164_));
  nand3  g131(.a(new_n159_), .b(new_n47_), .c(new_n38_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  oai21  g133(.a(new_n166_), .b(new_n161_), .c(new_n129_), .O(new_n167_));
  inv1   g134(.a(new_n50_), .O(new_n168_));
  oai21  g135(.a(x23), .b(new_n65_), .c(new_n70_), .O(new_n169_));
  nand3  g136(.a(new_n169_), .b(new_n72_), .c(new_n34_), .O(new_n170_));
  oai21  g137(.a(new_n68_), .b(new_n34_), .c(new_n170_), .O(new_n171_));
  nand3  g138(.a(new_n171_), .b(new_n168_), .c(new_n69_), .O(new_n172_));
  oai21  g139(.a(new_n73_), .b(x04), .c(x17), .O(new_n173_));
  nand3  g140(.a(new_n173_), .b(x22), .c(x14), .O(new_n174_));
  aoi21  g141(.a(new_n174_), .b(x09), .c(new_n125_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(x16), .O(new_n176_));
  nand3  g143(.a(new_n176_), .b(new_n172_), .c(new_n167_), .O(z4));
  nor2   g144(.a(new_n128_), .b(new_n50_), .O(z5));
  aoi21  g145(.a(new_n67_), .b(x06), .c(x03), .O(new_n179_));
  aoi21  g146(.a(x11), .b(new_n37_), .c(new_n36_), .O(new_n180_));
  oai22  g147(.a(new_n180_), .b(new_n125_), .c(new_n179_), .d(new_n50_), .O(new_n181_));
  nand2  g148(.a(new_n181_), .b(new_n129_), .O(new_n182_));
  nand2  g149(.a(new_n34_), .b(x00), .O(new_n183_));
  nand3  g150(.a(x20), .b(new_n68_), .c(new_n67_), .O(new_n184_));
  nand3  g151(.a(new_n141_), .b(x21), .c(new_n71_), .O(new_n185_));
  oai21  g152(.a(new_n184_), .b(new_n183_), .c(new_n185_), .O(new_n186_));
  nand3  g153(.a(new_n186_), .b(new_n45_), .c(new_n44_), .O(new_n187_));
  nand4  g154(.a(new_n109_), .b(x20), .c(x19), .d(new_n68_), .O(new_n188_));
  nand3  g155(.a(new_n141_), .b(new_n71_), .c(x15), .O(new_n189_));
  nand2  g156(.a(new_n189_), .b(new_n188_), .O(new_n190_));
  nand3  g157(.a(new_n190_), .b(x13), .c(x05), .O(new_n191_));
  nand2  g158(.a(new_n191_), .b(new_n187_), .O(new_n192_));
  nand2  g159(.a(new_n192_), .b(new_n40_), .O(new_n193_));
  nand2  g160(.a(new_n34_), .b(x07), .O(new_n194_));
  nand3  g161(.a(new_n141_), .b(new_n71_), .c(x18), .O(new_n195_));
  oai21  g162(.a(new_n194_), .b(new_n184_), .c(new_n195_), .O(new_n196_));
  nand3  g163(.a(new_n196_), .b(new_n139_), .c(x24), .O(new_n197_));
  nand3  g164(.a(new_n190_), .b(new_n47_), .c(new_n38_), .O(new_n198_));
  nand4  g165(.a(new_n198_), .b(new_n197_), .c(new_n193_), .d(new_n182_), .O(z6));
  nor2   g166(.a(new_n128_), .b(new_n125_), .O(z7));
endmodule


