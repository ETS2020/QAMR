// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n168_, new_n169_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  inv1   g004(.a(x12), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g006(.a(x05), .O(new_n40_));
  nand2  g007(.a(x24), .b(x07), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  aoi21  g010(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n40_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  oai21  g015(.a(new_n48_), .b(new_n44_), .c(new_n39_), .O(new_n49_));
  inv1   g016(.a(x10), .O(new_n50_));
  nor2   g017(.a(x24), .b(new_n38_), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n50_), .c(new_n37_), .d(x00), .O(new_n52_));
  aoi21  g019(.a(new_n52_), .b(new_n49_), .c(x20), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n54_));
  nor3   g021(.a(new_n54_), .b(x03), .c(x01), .O(z0));
  nand4  g022(.a(x06), .b(x04), .c(x03), .d(x01), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nor2   g024(.a(new_n35_), .b(x10), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n57_), .c(x09), .d(x08), .O(new_n59_));
  inv1   g026(.a(x16), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n36_), .O(new_n61_));
  inv1   g028(.a(x17), .O(new_n62_));
  inv1   g029(.a(x20), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g031(.a(x21), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  inv1   g034(.a(x23), .O(new_n68_));
  nor2   g035(.a(x24), .b(new_n68_), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n67_), .c(new_n64_), .d(new_n61_), .O(new_n70_));
  oai21  g037(.a(new_n70_), .b(new_n59_), .c(x12), .O(new_n71_));
  nand2  g038(.a(new_n71_), .b(new_n37_), .O(new_n72_));
  inv1   g039(.a(x01), .O(new_n73_));
  inv1   g040(.a(x03), .O(new_n74_));
  inv1   g041(.a(x04), .O(new_n75_));
  nand4  g042(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n76_));
  nor4   g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand4  g044(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  nor2   g046(.a(new_n68_), .b(new_n66_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n79_), .c(new_n77_), .d(new_n64_), .O(new_n81_));
  inv1   g048(.a(x09), .O(new_n82_));
  nor3   g049(.a(x04), .b(x03), .c(x01), .O(new_n83_));
  nor2   g050(.a(x08), .b(x06), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n83_), .c(new_n35_), .d(new_n82_), .O(new_n85_));
  nor2   g052(.a(x14), .b(x12), .O(new_n86_));
  nor2   g053(.a(x17), .b(x16), .O(new_n87_));
  inv1   g054(.a(x19), .O(new_n88_));
  nor2   g055(.a(x20), .b(new_n88_), .O(new_n89_));
  nor2   g056(.a(x23), .b(x22), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n89_), .c(new_n87_), .d(new_n86_), .O(new_n91_));
  oai21  g058(.a(new_n91_), .b(new_n85_), .c(new_n81_), .O(new_n92_));
  nand2  g059(.a(new_n46_), .b(new_n40_), .O(new_n93_));
  nand3  g060(.a(new_n42_), .b(x13), .c(x05), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nand4  g063(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  and2   g065(.a(x20), .b(x18), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(new_n80_), .d(new_n77_), .O(new_n100_));
  nor2   g067(.a(x09), .b(x08), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n83_), .c(x07), .d(new_n34_), .O(new_n102_));
  nand4  g069(.a(new_n60_), .b(new_n36_), .c(new_n38_), .d(new_n35_), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n90_), .c(new_n63_), .d(new_n62_), .O(new_n105_));
  oai21  g072(.a(new_n105_), .b(new_n102_), .c(new_n100_), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n93_), .c(x24), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n96_), .c(new_n72_), .O(z1));
  nand2  g075(.a(x24), .b(x18), .O(new_n109_));
  nand3  g076(.a(new_n42_), .b(x15), .c(x13), .O(new_n110_));
  aoi21  g077(.a(new_n110_), .b(new_n109_), .c(new_n40_), .O(new_n111_));
  nand3  g078(.a(x24), .b(x18), .c(x13), .O(new_n112_));
  nand3  g079(.a(x15), .b(new_n46_), .c(new_n40_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  oai21  g081(.a(new_n114_), .b(new_n111_), .c(new_n39_), .O(new_n115_));
  nor2   g082(.a(x24), .b(new_n65_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x12), .c(new_n50_), .d(new_n37_), .O(new_n117_));
  aoi21  g084(.a(new_n117_), .b(new_n115_), .c(new_n63_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x14), .c(x11), .d(x06), .O(new_n119_));
  nor3   g086(.a(new_n119_), .b(new_n74_), .c(new_n73_), .O(z2));
  nand4  g087(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n121_));
  nor2   g088(.a(new_n36_), .b(new_n35_), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(x20), .c(x15), .O(new_n123_));
  nand3  g090(.a(new_n84_), .b(new_n74_), .c(new_n73_), .O(new_n124_));
  nand3  g091(.a(new_n89_), .b(new_n36_), .c(new_n35_), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n121_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n95_), .O(new_n127_));
  nand2  g094(.a(new_n122_), .b(new_n99_), .O(new_n128_));
  nand4  g095(.a(x07), .b(new_n34_), .c(new_n74_), .d(new_n73_), .O(new_n129_));
  inv1   g096(.a(x08), .O(new_n130_));
  nand4  g097(.a(new_n63_), .b(new_n36_), .c(new_n35_), .d(new_n130_), .O(new_n131_));
  oai22  g098(.a(new_n131_), .b(new_n129_), .c(new_n128_), .d(new_n121_), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n93_), .c(x24), .O(new_n133_));
  nand4  g100(.a(new_n34_), .b(new_n74_), .c(new_n73_), .d(x00), .O(new_n134_));
  nand3  g101(.a(new_n122_), .b(x21), .c(x20), .O(new_n135_));
  oai22  g102(.a(new_n135_), .b(new_n121_), .c(new_n134_), .d(new_n131_), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n42_), .c(new_n50_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(x12), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n37_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n133_), .c(new_n127_), .O(z3));
  oai21  g107(.a(x23), .b(new_n75_), .c(new_n62_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n66_), .c(x09), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(x16), .c(new_n130_), .O(new_n143_));
  nand4  g110(.a(new_n42_), .b(new_n50_), .c(new_n37_), .d(x00), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n47_), .c(new_n45_), .O(new_n145_));
  or2    g112(.a(new_n145_), .b(new_n44_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n143_), .O(new_n147_));
  oai21  g114(.a(new_n68_), .b(x04), .c(x17), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(x22), .c(new_n82_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n60_), .c(x08), .O(new_n150_));
  nand3  g117(.a(new_n116_), .b(new_n50_), .c(new_n37_), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(new_n113_), .c(new_n112_), .O(new_n152_));
  or2    g119(.a(new_n152_), .b(new_n111_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n147_), .c(new_n39_), .O(z4));
  nor2   g122(.a(new_n48_), .b(new_n44_), .O(new_n156_));
  nand3  g123(.a(new_n42_), .b(new_n50_), .c(x00), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(x12), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n37_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n156_), .O(z5));
  nand2  g127(.a(x20), .b(new_n36_), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n34_), .c(x11), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x03), .c(new_n146_), .O(new_n163_));
  nand2  g130(.a(new_n63_), .b(x14), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(x06), .c(new_n35_), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(new_n74_), .c(new_n153_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n163_), .c(new_n39_), .O(z6));
  nor2   g134(.a(new_n114_), .b(new_n111_), .O(new_n168_));
  aoi21  g135(.a(new_n116_), .b(new_n50_), .c(new_n38_), .O(new_n169_));
  oai21  g136(.a(new_n169_), .b(x02), .c(new_n168_), .O(z7));
endmodule


