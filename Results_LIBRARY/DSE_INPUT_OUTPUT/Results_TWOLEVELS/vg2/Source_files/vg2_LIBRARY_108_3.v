// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n39_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n37_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nor3   g016(.a(new_n49_), .b(x20), .c(x14), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n51_));
  nand2  g018(.a(x10), .b(x02), .O(new_n52_));
  oai21  g019(.a(new_n51_), .b(x01), .c(new_n52_), .O(z0));
  inv1   g020(.a(new_n52_), .O(new_n54_));
  nand3  g021(.a(x04), .b(x03), .c(x01), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  inv1   g026(.a(x17), .O(new_n60_));
  inv1   g027(.a(x20), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g029(.a(x22), .O(new_n63_));
  inv1   g030(.a(x23), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n62_), .c(new_n59_), .d(new_n57_), .O(new_n66_));
  nor3   g033(.a(x04), .b(x03), .c(x01), .O(new_n67_));
  inv1   g034(.a(x08), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  nand4  g036(.a(new_n36_), .b(new_n69_), .c(new_n68_), .d(new_n35_), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  inv1   g038(.a(x12), .O(new_n72_));
  inv1   g039(.a(x14), .O(new_n73_));
  inv1   g040(.a(x16), .O(new_n74_));
  nand4  g041(.a(new_n60_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n75_));
  nor2   g042(.a(x23), .b(x22), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(new_n61_), .c(x19), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n71_), .c(new_n67_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  nor2   g047(.a(x13), .b(x05), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand3  g049(.a(new_n39_), .b(x13), .c(x05), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand4  g051(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n85_));
  inv1   g052(.a(new_n85_), .O(new_n86_));
  and2   g053(.a(x20), .b(x18), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n86_), .c(new_n65_), .d(new_n57_), .O(new_n88_));
  nor2   g055(.a(x09), .b(x08), .O(new_n89_));
  nand3  g056(.a(new_n89_), .b(x07), .c(new_n35_), .O(new_n90_));
  inv1   g057(.a(new_n90_), .O(new_n91_));
  nand4  g058(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n36_), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand3  g060(.a(new_n76_), .b(new_n61_), .c(new_n60_), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n93_), .c(new_n91_), .d(new_n67_), .O(new_n96_));
  aoi21  g063(.a(new_n96_), .b(new_n88_), .c(new_n81_), .O(new_n97_));
  aoi22  g064(.a(new_n97_), .b(x24), .c(new_n84_), .d(new_n80_), .O(new_n98_));
  nand2  g065(.a(new_n95_), .b(new_n93_), .O(new_n99_));
  and2   g066(.a(x21), .b(x20), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n86_), .c(new_n65_), .d(new_n57_), .O(new_n101_));
  nor2   g068(.a(x03), .b(x01), .O(new_n102_));
  nor2   g069(.a(x06), .b(x04), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n102_), .c(new_n89_), .d(x00), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n99_), .c(new_n101_), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n39_), .c(new_n44_), .d(new_n43_), .O(new_n106_));
  oai21  g073(.a(new_n98_), .b(new_n54_), .c(new_n106_), .O(z1));
  inv1   g074(.a(x01), .O(new_n108_));
  nand2  g075(.a(x24), .b(x18), .O(new_n109_));
  nand3  g076(.a(new_n39_), .b(x15), .c(x13), .O(new_n110_));
  aoi21  g077(.a(new_n110_), .b(new_n109_), .c(new_n37_), .O(new_n111_));
  nand3  g078(.a(x24), .b(x18), .c(x13), .O(new_n112_));
  nand3  g079(.a(x15), .b(new_n46_), .c(new_n37_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  oai21  g081(.a(new_n114_), .b(new_n111_), .c(new_n52_), .O(new_n115_));
  nand4  g082(.a(new_n39_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n115_), .c(new_n61_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(x14), .c(x11), .d(x06), .O(new_n118_));
  nor3   g085(.a(new_n118_), .b(new_n34_), .c(new_n108_), .O(z2));
  nand4  g086(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n120_));
  nor2   g087(.a(new_n73_), .b(new_n36_), .O(new_n121_));
  nand3  g088(.a(new_n121_), .b(x20), .c(x15), .O(new_n122_));
  nand3  g089(.a(new_n102_), .b(new_n68_), .c(new_n35_), .O(new_n123_));
  nand4  g090(.a(new_n61_), .b(x19), .c(new_n73_), .d(new_n36_), .O(new_n124_));
  oai22  g091(.a(new_n124_), .b(new_n123_), .c(new_n122_), .d(new_n120_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n84_), .O(new_n126_));
  nand2  g093(.a(new_n121_), .b(new_n87_), .O(new_n127_));
  nand3  g094(.a(new_n102_), .b(x07), .c(new_n35_), .O(new_n128_));
  nand4  g095(.a(new_n61_), .b(new_n73_), .c(new_n36_), .d(new_n68_), .O(new_n129_));
  oai22  g096(.a(new_n129_), .b(new_n128_), .c(new_n127_), .d(new_n120_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n82_), .c(x24), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n126_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n52_), .O(new_n133_));
  nand4  g100(.a(new_n35_), .b(new_n34_), .c(new_n108_), .d(x00), .O(new_n134_));
  nand2  g101(.a(new_n121_), .b(new_n100_), .O(new_n135_));
  oai22  g102(.a(new_n135_), .b(new_n120_), .c(new_n134_), .d(new_n129_), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n39_), .c(new_n44_), .d(new_n43_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n133_), .O(z3));
  inv1   g105(.a(x04), .O(new_n139_));
  oai21  g106(.a(x23), .b(new_n139_), .c(new_n60_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n63_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n69_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n74_), .c(x08), .O(new_n143_));
  oai21  g110(.a(new_n64_), .b(x04), .c(x17), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(x22), .c(new_n69_), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(new_n74_), .c(x08), .O(new_n146_));
  inv1   g113(.a(new_n111_), .O(new_n147_));
  and2   g114(.a(new_n116_), .b(new_n113_), .O(new_n148_));
  nand3  g115(.a(new_n148_), .b(new_n112_), .c(new_n147_), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(new_n146_), .c(new_n54_), .O(new_n150_));
  oai21  g117(.a(new_n143_), .b(new_n49_), .c(new_n150_), .O(z4));
  nand2  g118(.a(new_n47_), .b(new_n42_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n41_), .c(new_n52_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n45_), .O(z5));
  aoi21  g121(.a(x20), .b(new_n73_), .c(x06), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(x11), .c(new_n34_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(z5), .O(new_n157_));
  nand2  g124(.a(new_n116_), .b(new_n115_), .O(new_n158_));
  aoi21  g125(.a(new_n61_), .b(x14), .c(new_n35_), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(new_n36_), .c(x03), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n157_), .O(z6));
  nand4  g129(.a(new_n148_), .b(new_n112_), .c(new_n147_), .d(new_n52_), .O(z7));
endmodule


