// Benchmark "FAU" written by ABC on Wed Aug 19 21:58:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n39_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n37_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nor3   g016(.a(new_n49_), .b(x20), .c(x14), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n51_));
  inv1   g018(.a(x08), .O(new_n52_));
  inv1   g019(.a(x23), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  oai21  g022(.a(new_n51_), .b(x01), .c(new_n55_), .O(z0));
  inv1   g023(.a(x04), .O(new_n57_));
  inv1   g024(.a(x09), .O(new_n58_));
  inv1   g025(.a(x12), .O(new_n59_));
  inv1   g026(.a(x16), .O(new_n60_));
  inv1   g027(.a(x17), .O(new_n61_));
  inv1   g028(.a(x20), .O(new_n62_));
  oai21  g029(.a(new_n48_), .b(new_n41_), .c(new_n53_), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(x22), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(x14), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n59_), .c(new_n36_), .d(new_n58_), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(x08), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n35_), .c(new_n57_), .d(new_n34_), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(x01), .O(z1));
  inv1   g037(.a(x01), .O(new_n71_));
  inv1   g038(.a(x14), .O(new_n72_));
  nand2  g039(.a(x24), .b(x18), .O(new_n73_));
  nand3  g040(.a(new_n39_), .b(x15), .c(x13), .O(new_n74_));
  aoi21  g041(.a(new_n74_), .b(new_n73_), .c(new_n37_), .O(new_n75_));
  nand3  g042(.a(x24), .b(x18), .c(x13), .O(new_n76_));
  nand3  g043(.a(x15), .b(new_n46_), .c(new_n37_), .O(new_n77_));
  nand4  g044(.a(new_n39_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n79_));
  or2    g046(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  oai21  g047(.a(new_n53_), .b(new_n52_), .c(new_n80_), .O(new_n81_));
  nor3   g048(.a(new_n81_), .b(new_n62_), .c(new_n72_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(x11), .c(x06), .d(x03), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(new_n71_), .O(z2));
  nand3  g051(.a(x15), .b(x13), .c(x05), .O(new_n85_));
  nand3  g052(.a(x21), .b(new_n44_), .c(new_n43_), .O(new_n86_));
  aoi21  g053(.a(new_n86_), .b(new_n85_), .c(x23), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(x20), .c(x14), .d(x11), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(new_n52_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(x06), .c(x03), .d(x01), .O(new_n90_));
  nand3  g057(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n91_));
  nand3  g058(.a(x19), .b(x13), .c(x05), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n62_), .c(new_n72_), .d(new_n36_), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(x08), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n35_), .c(new_n34_), .d(new_n71_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n90_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n39_), .O(new_n98_));
  nor2   g065(.a(x03), .b(x01), .O(new_n99_));
  nand4  g066(.a(new_n62_), .b(new_n72_), .c(new_n36_), .d(new_n52_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n99_), .c(x07), .d(new_n35_), .O(new_n102_));
  nand4  g069(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nor2   g071(.a(new_n72_), .b(new_n36_), .O(new_n105_));
  nor2   g072(.a(x23), .b(new_n62_), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(new_n105_), .c(new_n104_), .d(x18), .O(new_n107_));
  aoi22  g074(.a(new_n107_), .b(new_n102_), .c(new_n46_), .d(new_n37_), .O(new_n108_));
  nand4  g075(.a(new_n62_), .b(x19), .c(new_n72_), .d(new_n36_), .O(new_n109_));
  inv1   g076(.a(new_n109_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(new_n99_), .c(new_n52_), .d(new_n35_), .O(new_n111_));
  nand4  g078(.a(new_n106_), .b(new_n105_), .c(new_n104_), .d(x15), .O(new_n112_));
  aoi21  g079(.a(new_n112_), .b(new_n111_), .c(x13), .O(new_n113_));
  aoi22  g080(.a(new_n113_), .b(new_n37_), .c(new_n108_), .d(x24), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n98_), .O(z3));
  inv1   g082(.a(new_n80_), .O(new_n116_));
  nand2  g083(.a(x22), .b(new_n61_), .O(new_n117_));
  aoi21  g084(.a(new_n117_), .b(x09), .c(x23), .O(new_n118_));
  aoi21  g085(.a(new_n118_), .b(x16), .c(new_n52_), .O(new_n119_));
  inv1   g086(.a(x22), .O(new_n120_));
  aoi21  g087(.a(new_n120_), .b(x17), .c(x09), .O(new_n121_));
  nand3  g088(.a(new_n53_), .b(new_n120_), .c(x04), .O(new_n122_));
  oai21  g089(.a(new_n121_), .b(x08), .c(new_n122_), .O(new_n123_));
  aoi22  g090(.a(new_n123_), .b(new_n60_), .c(new_n53_), .d(x08), .O(new_n124_));
  oai22  g091(.a(new_n124_), .b(new_n49_), .c(new_n119_), .d(new_n116_), .O(z4));
  inv1   g092(.a(new_n41_), .O(new_n126_));
  and2   g093(.a(new_n45_), .b(new_n42_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(new_n55_), .c(new_n47_), .d(new_n126_), .O(z5));
  nand2  g095(.a(x20), .b(new_n72_), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(new_n35_), .c(x11), .O(new_n130_));
  oai22  g097(.a(new_n130_), .b(x03), .c(new_n48_), .d(new_n41_), .O(new_n131_));
  nand2  g098(.a(new_n62_), .b(x14), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(x06), .c(new_n36_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n34_), .c(new_n80_), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n131_), .c(new_n55_), .O(z6));
  nor2   g102(.a(new_n116_), .b(new_n54_), .O(z7));
endmodule


