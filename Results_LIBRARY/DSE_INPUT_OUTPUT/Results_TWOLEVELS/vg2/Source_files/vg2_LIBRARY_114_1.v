// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x21), .O(new_n35_));
  inv1   g002(.a(x03), .O(new_n36_));
  inv1   g003(.a(x06), .O(new_n37_));
  inv1   g004(.a(x00), .O(new_n38_));
  nor2   g005(.a(x01), .b(new_n38_), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  inv1   g007(.a(x14), .O(new_n41_));
  inv1   g008(.a(x20), .O(new_n42_));
  inv1   g009(.a(x24), .O(new_n43_));
  nor2   g010(.a(x11), .b(x10), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n43_), .c(new_n42_), .d(new_n41_), .O(new_n45_));
  oai21  g012(.a(new_n45_), .b(new_n40_), .c(new_n35_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n34_), .O(new_n47_));
  inv1   g014(.a(x11), .O(new_n48_));
  inv1   g015(.a(x05), .O(new_n49_));
  nand2  g016(.a(x24), .b(x07), .O(new_n50_));
  nand3  g017(.a(new_n43_), .b(x19), .c(x13), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand3  g019(.a(x24), .b(x13), .c(x07), .O(new_n53_));
  inv1   g020(.a(x13), .O(new_n54_));
  nand3  g021(.a(x19), .b(new_n54_), .c(new_n49_), .O(new_n55_));
  nand2  g022(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  oai21  g023(.a(new_n56_), .b(new_n52_), .c(new_n42_), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(x14), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n48_), .c(new_n37_), .d(new_n36_), .O(new_n59_));
  oai21  g026(.a(new_n59_), .b(x01), .c(new_n47_), .O(z0));
  nand2  g027(.a(x21), .b(new_n34_), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand3  g029(.a(x04), .b(x03), .c(x01), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  nand4  g031(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  nand4  g033(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n67_));
  nand4  g034(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n66_), .c(new_n64_), .O(new_n70_));
  nor3   g037(.a(x04), .b(x03), .c(x01), .O(new_n71_));
  nor2   g038(.a(x08), .b(x06), .O(new_n72_));
  nor2   g039(.a(x11), .b(x09), .O(new_n73_));
  inv1   g040(.a(x12), .O(new_n74_));
  inv1   g041(.a(x16), .O(new_n75_));
  inv1   g042(.a(x17), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n41_), .d(new_n74_), .O(new_n77_));
  inv1   g044(.a(x22), .O(new_n78_));
  inv1   g045(.a(x23), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n78_), .c(new_n42_), .d(x19), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n82_));
  nor2   g049(.a(x13), .b(x05), .O(new_n83_));
  nor2   g050(.a(x24), .b(new_n54_), .O(new_n84_));
  aoi21  g051(.a(new_n84_), .b(x05), .c(new_n83_), .O(new_n85_));
  aoi21  g052(.a(new_n82_), .b(new_n70_), .c(new_n85_), .O(new_n86_));
  nand4  g053(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n87_));
  nand4  g054(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g056(.a(new_n89_), .b(new_n66_), .c(new_n64_), .O(new_n90_));
  inv1   g057(.a(x08), .O(new_n91_));
  inv1   g058(.a(x09), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(x07), .d(new_n37_), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  nand4  g061(.a(new_n75_), .b(new_n41_), .c(new_n74_), .d(new_n48_), .O(new_n95_));
  nand4  g062(.a(new_n79_), .b(new_n78_), .c(new_n42_), .d(new_n76_), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n94_), .c(new_n71_), .O(new_n98_));
  aoi21  g065(.a(new_n98_), .b(new_n90_), .c(new_n83_), .O(new_n99_));
  aoi21  g066(.a(new_n99_), .b(x24), .c(new_n86_), .O(new_n100_));
  nand3  g067(.a(new_n39_), .b(new_n36_), .c(new_n34_), .O(new_n101_));
  inv1   g068(.a(new_n101_), .O(new_n102_));
  inv1   g069(.a(x10), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n92_), .c(new_n91_), .O(new_n104_));
  nor3   g071(.a(new_n104_), .b(x06), .c(x04), .O(new_n105_));
  nand3  g072(.a(new_n76_), .b(new_n75_), .c(new_n41_), .O(new_n106_));
  nor3   g073(.a(new_n106_), .b(x12), .c(x11), .O(new_n107_));
  nand3  g074(.a(new_n43_), .b(new_n79_), .c(new_n78_), .O(new_n108_));
  nor3   g075(.a(new_n108_), .b(x21), .c(x20), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n107_), .c(new_n105_), .d(new_n102_), .O(new_n110_));
  oai21  g077(.a(new_n100_), .b(new_n62_), .c(new_n110_), .O(z1));
  inv1   g078(.a(x01), .O(new_n112_));
  nand2  g079(.a(x24), .b(x18), .O(new_n113_));
  nand3  g080(.a(new_n43_), .b(x15), .c(x13), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(x05), .O(new_n116_));
  nand3  g083(.a(x24), .b(x18), .c(x13), .O(new_n117_));
  inv1   g084(.a(new_n117_), .O(new_n118_));
  inv1   g085(.a(x15), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(x13), .O(new_n120_));
  aoi21  g087(.a(new_n120_), .b(new_n49_), .c(new_n118_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n116_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(new_n61_), .c(x20), .d(x14), .O(new_n123_));
  inv1   g090(.a(new_n123_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x11), .c(x06), .d(x03), .O(new_n125_));
  nor2   g092(.a(new_n125_), .b(new_n112_), .O(z2));
  nand3  g093(.a(new_n72_), .b(new_n39_), .c(new_n36_), .O(new_n127_));
  oai21  g094(.a(new_n127_), .b(new_n45_), .c(new_n35_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n34_), .O(new_n129_));
  inv1   g096(.a(new_n85_), .O(new_n130_));
  nand4  g097(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n131_));
  nand4  g098(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n132_));
  nand3  g099(.a(new_n72_), .b(new_n36_), .c(new_n112_), .O(new_n133_));
  nand4  g100(.a(new_n42_), .b(x19), .c(new_n41_), .d(new_n48_), .O(new_n134_));
  oai22  g101(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n131_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n130_), .O(new_n136_));
  inv1   g103(.a(new_n83_), .O(new_n137_));
  nand4  g104(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n138_));
  nand4  g105(.a(x07), .b(new_n37_), .c(new_n36_), .d(new_n112_), .O(new_n139_));
  nand4  g106(.a(new_n42_), .b(new_n41_), .c(new_n48_), .d(new_n91_), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n131_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n137_), .c(x24), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n136_), .c(new_n129_), .O(z3));
  inv1   g110(.a(x04), .O(new_n144_));
  oai21  g111(.a(x23), .b(new_n144_), .c(new_n76_), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(new_n78_), .c(x09), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(x16), .c(new_n91_), .O(new_n147_));
  inv1   g114(.a(new_n52_), .O(new_n148_));
  nand4  g115(.a(new_n43_), .b(new_n103_), .c(new_n34_), .d(x00), .O(new_n149_));
  nand4  g116(.a(new_n149_), .b(new_n55_), .c(new_n53_), .d(new_n148_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  oai21  g118(.a(new_n79_), .b(x04), .c(x17), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(x22), .c(new_n92_), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n75_), .c(x08), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n122_), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n151_), .c(new_n61_), .O(z4));
  nand3  g123(.a(new_n55_), .b(new_n53_), .c(new_n148_), .O(new_n157_));
  nand2  g124(.a(new_n34_), .b(x00), .O(new_n158_));
  nand3  g125(.a(new_n43_), .b(new_n35_), .c(new_n103_), .O(new_n159_));
  nor2   g126(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  aoi21  g127(.a(new_n61_), .b(new_n157_), .c(new_n160_), .O(new_n161_));
  inv1   g128(.a(new_n161_), .O(z5));
  oai21  g129(.a(new_n42_), .b(x14), .c(new_n37_), .O(new_n163_));
  aoi21  g130(.a(new_n163_), .b(new_n48_), .c(x03), .O(new_n164_));
  aoi21  g131(.a(new_n42_), .b(x14), .c(new_n37_), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(new_n48_), .c(x03), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n122_), .c(new_n61_), .O(new_n167_));
  oai21  g134(.a(new_n164_), .b(new_n161_), .c(new_n167_), .O(z6));
  aoi21  g135(.a(new_n121_), .b(new_n116_), .c(new_n62_), .O(z7));
endmodule


