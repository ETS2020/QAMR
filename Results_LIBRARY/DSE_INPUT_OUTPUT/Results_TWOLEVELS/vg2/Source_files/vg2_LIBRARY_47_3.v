// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n186_, new_n187_,
    new_n188_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nand2  g019(.a(x24), .b(x02), .O(new_n53_));
  oai21  g020(.a(new_n52_), .b(x01), .c(new_n53_), .O(z0));
  inv1   g021(.a(x12), .O(new_n55_));
  nand3  g022(.a(x15), .b(x13), .c(x05), .O(new_n56_));
  nand3  g023(.a(x21), .b(new_n45_), .c(new_n44_), .O(new_n57_));
  nand2  g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(x23), .c(x22), .d(x20), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand4  g027(.a(new_n60_), .b(x17), .c(x16), .d(x14), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n55_), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(x11), .c(x09), .d(x08), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n35_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(x04), .c(x03), .d(x01), .O(new_n65_));
  inv1   g032(.a(x01), .O(new_n66_));
  inv1   g033(.a(x04), .O(new_n67_));
  inv1   g034(.a(x08), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  inv1   g036(.a(x14), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  inv1   g038(.a(x17), .O(new_n72_));
  inv1   g039(.a(x22), .O(new_n73_));
  inv1   g040(.a(x23), .O(new_n74_));
  nand3  g041(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n75_));
  nand3  g042(.a(x19), .b(x13), .c(x05), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n74_), .c(new_n73_), .d(new_n37_), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(x12), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n36_), .c(new_n69_), .d(new_n68_), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(x06), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n67_), .c(new_n34_), .d(new_n66_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n65_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n40_), .O(new_n86_));
  nor2   g053(.a(x13), .b(x05), .O(new_n87_));
  nand4  g054(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n88_));
  nor4   g055(.a(new_n88_), .b(new_n67_), .c(new_n34_), .d(new_n66_), .O(new_n89_));
  nand4  g056(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n90_));
  nand4  g057(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g059(.a(new_n67_), .b(new_n34_), .c(new_n66_), .O(new_n93_));
  nand4  g060(.a(new_n69_), .b(new_n68_), .c(x07), .d(new_n35_), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand4  g062(.a(new_n71_), .b(new_n70_), .c(new_n55_), .d(new_n36_), .O(new_n96_));
  nor2   g063(.a(x23), .b(x22), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n37_), .c(new_n72_), .O(new_n98_));
  nor2   g065(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  aoi22  g066(.a(new_n99_), .b(new_n95_), .c(new_n92_), .d(new_n89_), .O(new_n100_));
  oai21  g067(.a(new_n100_), .b(new_n87_), .c(new_n44_), .O(new_n101_));
  nand4  g068(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n102_));
  nand4  g069(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n89_), .O(new_n105_));
  inv1   g072(.a(new_n93_), .O(new_n106_));
  nor2   g073(.a(x08), .b(x06), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n106_), .c(new_n36_), .d(new_n69_), .O(new_n108_));
  nor2   g075(.a(x14), .b(x12), .O(new_n109_));
  nor2   g076(.a(x17), .b(x16), .O(new_n110_));
  inv1   g077(.a(x19), .O(new_n111_));
  nor2   g078(.a(x20), .b(new_n111_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(new_n110_), .c(new_n109_), .d(new_n97_), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n108_), .c(new_n105_), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(new_n47_), .c(new_n38_), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  aoi21  g083(.a(new_n101_), .b(x24), .c(new_n116_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n86_), .O(z1));
  inv1   g085(.a(x18), .O(new_n119_));
  nor2   g086(.a(new_n40_), .b(new_n119_), .O(new_n120_));
  inv1   g087(.a(new_n120_), .O(new_n121_));
  nand3  g088(.a(new_n40_), .b(x15), .c(x13), .O(new_n122_));
  oai21  g089(.a(new_n121_), .b(x02), .c(new_n122_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(x05), .O(new_n124_));
  nand4  g091(.a(new_n53_), .b(x15), .c(new_n47_), .d(new_n38_), .O(new_n125_));
  nand2  g092(.a(new_n120_), .b(x13), .O(new_n126_));
  nand3  g093(.a(new_n40_), .b(x21), .c(new_n45_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n44_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n125_), .c(new_n124_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(x20), .c(x14), .d(x11), .O(new_n131_));
  inv1   g098(.a(new_n131_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(x06), .c(x03), .d(x01), .O(new_n133_));
  inv1   g100(.a(new_n133_), .O(z2));
  nand4  g101(.a(new_n58_), .b(x20), .c(x14), .d(x11), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(new_n68_), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(x06), .c(x03), .d(x01), .O(new_n137_));
  nand4  g104(.a(new_n77_), .b(new_n37_), .c(new_n70_), .d(new_n36_), .O(new_n138_));
  nor2   g105(.a(new_n138_), .b(x08), .O(new_n139_));
  nand4  g106(.a(new_n139_), .b(new_n35_), .c(new_n34_), .d(new_n66_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n40_), .O(new_n142_));
  inv1   g109(.a(new_n87_), .O(new_n143_));
  nand4  g110(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n144_));
  nand4  g111(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n145_));
  nand4  g112(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n66_), .O(new_n146_));
  nand4  g113(.a(new_n37_), .b(new_n70_), .c(new_n36_), .d(new_n68_), .O(new_n147_));
  oai22  g114(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(new_n144_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n143_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n44_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(x24), .O(new_n151_));
  nand4  g118(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n152_));
  nand3  g119(.a(new_n107_), .b(new_n34_), .c(new_n66_), .O(new_n153_));
  nand3  g120(.a(new_n112_), .b(new_n70_), .c(new_n36_), .O(new_n154_));
  oai22  g121(.a(new_n154_), .b(new_n153_), .c(new_n152_), .d(new_n144_), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n47_), .c(new_n38_), .O(new_n156_));
  nand3  g123(.a(new_n156_), .b(new_n151_), .c(new_n142_), .O(z3));
  oai21  g124(.a(x23), .b(new_n67_), .c(new_n72_), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(new_n73_), .c(x09), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(x16), .c(new_n68_), .O(new_n160_));
  oai21  g127(.a(new_n39_), .b(x02), .c(new_n41_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(x05), .O(new_n162_));
  nand4  g129(.a(new_n53_), .b(x19), .c(new_n47_), .d(new_n38_), .O(new_n163_));
  nand3  g130(.a(new_n40_), .b(new_n45_), .c(x00), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n43_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n44_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n163_), .c(new_n162_), .O(z5));
  nand2  g134(.a(z5), .b(new_n160_), .O(new_n168_));
  oai21  g135(.a(new_n74_), .b(x04), .c(x17), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(x22), .c(new_n69_), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(new_n71_), .c(x08), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n130_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(new_n168_), .O(z4));
  nand2  g140(.a(x20), .b(new_n70_), .O(new_n174_));
  aoi21  g141(.a(new_n174_), .b(new_n35_), .c(x11), .O(new_n175_));
  oai22  g142(.a(new_n175_), .b(x03), .c(new_n49_), .d(new_n42_), .O(new_n176_));
  nand2  g143(.a(new_n37_), .b(x14), .O(new_n177_));
  aoi21  g144(.a(new_n177_), .b(x06), .c(new_n36_), .O(new_n178_));
  aoi21  g145(.a(new_n122_), .b(new_n121_), .c(new_n38_), .O(new_n179_));
  nand3  g146(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n180_));
  nand4  g147(.a(new_n40_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n181_));
  and2   g148(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(new_n126_), .O(new_n183_));
  oai22  g150(.a(new_n183_), .b(new_n179_), .c(new_n178_), .d(new_n34_), .O(new_n184_));
  nand3  g151(.a(new_n184_), .b(new_n176_), .c(new_n53_), .O(z6));
  inv1   g152(.a(new_n179_), .O(new_n186_));
  oai21  g153(.a(new_n119_), .b(new_n47_), .c(new_n44_), .O(new_n187_));
  nand2  g154(.a(new_n187_), .b(x24), .O(new_n188_));
  nand3  g155(.a(new_n188_), .b(new_n182_), .c(new_n186_), .O(z7));
endmodule


