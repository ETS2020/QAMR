// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_;
  inv1   g000(.a(x01), .O(new_n34_));
  inv1   g001(.a(x03), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x14), .O(new_n38_));
  inv1   g005(.a(x20), .O(new_n39_));
  inv1   g006(.a(x02), .O(new_n40_));
  nand3  g007(.a(x24), .b(x07), .c(new_n40_), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(x05), .O(new_n45_));
  inv1   g012(.a(x05), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand2  g014(.a(x24), .b(x02), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(x19), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nand3  g016(.a(x24), .b(x13), .c(x07), .O(new_n50_));
  inv1   g017(.a(x10), .O(new_n51_));
  nand3  g018(.a(new_n42_), .b(new_n51_), .c(x00), .O(new_n52_));
  nand2  g019(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n40_), .O(new_n54_));
  nand3  g021(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(z5));
  nand4  g022(.a(z5), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(z0));
  nand2  g026(.a(new_n47_), .b(new_n46_), .O(new_n60_));
  inv1   g027(.a(x04), .O(new_n61_));
  nand4  g028(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n62_));
  nor4   g029(.a(new_n62_), .b(new_n61_), .c(new_n35_), .d(new_n34_), .O(new_n63_));
  nand4  g030(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n64_));
  inv1   g031(.a(new_n64_), .O(new_n65_));
  inv1   g032(.a(x18), .O(new_n66_));
  nor2   g033(.a(new_n39_), .b(new_n66_), .O(new_n67_));
  inv1   g034(.a(x22), .O(new_n68_));
  inv1   g035(.a(x23), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n63_), .O(new_n71_));
  nor3   g038(.a(x04), .b(x03), .c(x01), .O(new_n72_));
  nor2   g039(.a(x09), .b(x08), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(x07), .d(new_n36_), .O(new_n74_));
  inv1   g041(.a(x17), .O(new_n75_));
  inv1   g042(.a(x12), .O(new_n76_));
  inv1   g043(.a(x16), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n38_), .c(new_n76_), .d(new_n37_), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  nor2   g046(.a(x23), .b(x22), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n79_), .c(new_n39_), .d(new_n75_), .O(new_n81_));
  oai21  g048(.a(new_n81_), .b(new_n74_), .c(new_n71_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n60_), .c(x24), .O(new_n83_));
  nand3  g050(.a(x15), .b(new_n47_), .c(new_n46_), .O(new_n84_));
  nand3  g051(.a(new_n42_), .b(x21), .c(new_n51_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(x23), .c(x22), .d(x20), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(x17), .c(x16), .d(x14), .O(new_n89_));
  nor2   g056(.a(new_n89_), .b(new_n76_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(x11), .c(x09), .d(x08), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n36_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(x04), .c(x03), .d(x01), .O(new_n93_));
  inv1   g060(.a(x08), .O(new_n94_));
  inv1   g061(.a(x09), .O(new_n95_));
  nand3  g062(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n52_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n69_), .c(new_n68_), .d(new_n39_), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n75_), .c(new_n77_), .d(new_n38_), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(x12), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n37_), .c(new_n95_), .d(new_n94_), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(x06), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n61_), .c(new_n35_), .d(new_n34_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n93_), .c(new_n83_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n40_), .O(new_n106_));
  nand2  g073(.a(x13), .b(x05), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n60_), .O(new_n108_));
  nand4  g075(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n109_));
  nand3  g076(.a(new_n70_), .b(x20), .c(x17), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n63_), .O(new_n112_));
  nor2   g079(.a(x08), .b(x06), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(new_n72_), .c(new_n37_), .d(new_n95_), .O(new_n114_));
  nor2   g081(.a(x14), .b(x12), .O(new_n115_));
  nor2   g082(.a(x17), .b(x16), .O(new_n116_));
  inv1   g083(.a(x19), .O(new_n117_));
  nor2   g084(.a(x20), .b(new_n117_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(new_n116_), .c(new_n115_), .d(new_n80_), .O(new_n119_));
  oai21  g086(.a(new_n119_), .b(new_n114_), .c(new_n112_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n108_), .c(new_n42_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n106_), .O(z1));
  nor2   g089(.a(new_n42_), .b(new_n66_), .O(new_n123_));
  inv1   g090(.a(new_n123_), .O(new_n124_));
  nand3  g091(.a(new_n42_), .b(x15), .c(x13), .O(new_n125_));
  oai21  g092(.a(new_n124_), .b(x02), .c(new_n125_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(x05), .O(new_n127_));
  nand4  g094(.a(new_n48_), .b(x15), .c(new_n47_), .d(new_n46_), .O(new_n128_));
  oai21  g095(.a(new_n124_), .b(new_n47_), .c(new_n85_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n40_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n128_), .c(new_n127_), .O(new_n131_));
  nand4  g098(.a(new_n131_), .b(x20), .c(x14), .d(x11), .O(new_n132_));
  inv1   g099(.a(new_n132_), .O(new_n133_));
  nand4  g100(.a(new_n133_), .b(x06), .c(x03), .d(x01), .O(new_n134_));
  inv1   g101(.a(new_n134_), .O(z2));
  nand4  g102(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n136_));
  nand3  g103(.a(new_n67_), .b(x14), .c(x11), .O(new_n137_));
  nand4  g104(.a(x07), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n138_));
  nand4  g105(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n94_), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n138_), .c(new_n137_), .d(new_n136_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n60_), .c(x24), .O(new_n141_));
  nand4  g108(.a(new_n86_), .b(x20), .c(x14), .d(x11), .O(new_n142_));
  nor2   g109(.a(new_n142_), .b(new_n94_), .O(new_n143_));
  nand4  g110(.a(new_n143_), .b(x06), .c(x03), .d(x01), .O(new_n144_));
  nand4  g111(.a(new_n97_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n145_));
  nor2   g112(.a(new_n145_), .b(x08), .O(new_n146_));
  nand4  g113(.a(new_n146_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n144_), .c(new_n141_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n40_), .O(new_n149_));
  nand4  g116(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n150_));
  nand3  g117(.a(new_n113_), .b(new_n35_), .c(new_n34_), .O(new_n151_));
  nand3  g118(.a(new_n118_), .b(new_n38_), .c(new_n37_), .O(new_n152_));
  oai22  g119(.a(new_n152_), .b(new_n151_), .c(new_n150_), .d(new_n136_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n108_), .c(new_n42_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n149_), .O(z3));
  oai21  g122(.a(x23), .b(new_n61_), .c(new_n75_), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n68_), .c(x09), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(x16), .c(new_n94_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(z5), .O(new_n159_));
  oai21  g126(.a(new_n69_), .b(x04), .c(x17), .O(new_n160_));
  aoi21  g127(.a(new_n160_), .b(x22), .c(new_n95_), .O(new_n161_));
  oai21  g128(.a(new_n161_), .b(new_n77_), .c(x08), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n131_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n159_), .O(z4));
  aoi21  g131(.a(x20), .b(new_n38_), .c(x06), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(x11), .c(new_n35_), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(z5), .O(new_n167_));
  aoi21  g134(.a(new_n39_), .b(x14), .c(new_n36_), .O(new_n168_));
  oai21  g135(.a(new_n168_), .b(new_n37_), .c(x03), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n131_), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n167_), .O(z6));
  inv1   g138(.a(new_n125_), .O(new_n172_));
  oai21  g139(.a(new_n172_), .b(new_n123_), .c(x05), .O(new_n173_));
  oai21  g140(.a(new_n66_), .b(new_n47_), .c(new_n40_), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(x24), .O(new_n175_));
  nand4  g142(.a(new_n42_), .b(x21), .c(new_n51_), .d(new_n40_), .O(new_n176_));
  nand4  g143(.a(new_n176_), .b(new_n175_), .c(new_n173_), .d(new_n84_), .O(z7));
endmodule


