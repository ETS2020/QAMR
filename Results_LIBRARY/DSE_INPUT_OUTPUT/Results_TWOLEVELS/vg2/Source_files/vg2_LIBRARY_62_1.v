// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_;
  inv1   g000(.a(x01), .O(new_n34_));
  inv1   g001(.a(x03), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x14), .O(new_n38_));
  inv1   g005(.a(x20), .O(new_n39_));
  nand3  g006(.a(x24), .b(x07), .c(x02), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x05), .O(new_n44_));
  inv1   g011(.a(x05), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n45_), .O(new_n47_));
  nand3  g014(.a(x24), .b(x13), .c(x07), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g016(.a(new_n49_), .b(x02), .O(new_n50_));
  inv1   g017(.a(x02), .O(new_n51_));
  inv1   g018(.a(x10), .O(new_n52_));
  nand3  g019(.a(new_n52_), .b(new_n51_), .c(x00), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n41_), .O(new_n55_));
  nand3  g022(.a(new_n55_), .b(new_n50_), .c(new_n44_), .O(z5));
  nand4  g023(.a(z5), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n57_));
  inv1   g024(.a(new_n57_), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(z0));
  nand3  g027(.a(x04), .b(x03), .c(x01), .O(new_n61_));
  nand4  g028(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g030(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n64_));
  nand4  g031(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g033(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nor2   g034(.a(x03), .b(x01), .O(new_n68_));
  nor2   g035(.a(x06), .b(x04), .O(new_n69_));
  nor2   g036(.a(x09), .b(x08), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(x00), .O(new_n71_));
  nor2   g038(.a(x12), .b(x11), .O(new_n72_));
  nor2   g039(.a(x16), .b(x14), .O(new_n73_));
  nor2   g040(.a(x20), .b(x17), .O(new_n74_));
  nor2   g041(.a(x23), .b(x22), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  oai21  g043(.a(new_n76_), .b(new_n71_), .c(new_n67_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n52_), .c(new_n51_), .O(new_n78_));
  nand2  g045(.a(x13), .b(x05), .O(new_n79_));
  nand2  g046(.a(new_n46_), .b(new_n45_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g048(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n82_));
  nand4  g049(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n63_), .O(new_n85_));
  inv1   g052(.a(x09), .O(new_n86_));
  nor3   g053(.a(x04), .b(x03), .c(x01), .O(new_n87_));
  nor2   g054(.a(x08), .b(x06), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n87_), .c(new_n37_), .d(new_n86_), .O(new_n89_));
  nor2   g056(.a(x14), .b(x12), .O(new_n90_));
  nor2   g057(.a(x17), .b(x16), .O(new_n91_));
  inv1   g058(.a(x19), .O(new_n92_));
  nor2   g059(.a(x20), .b(new_n92_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n91_), .c(new_n90_), .d(new_n75_), .O(new_n94_));
  oai21  g061(.a(new_n94_), .b(new_n89_), .c(new_n85_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n81_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n78_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n41_), .O(new_n98_));
  nand4  g065(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n64_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n63_), .O(new_n101_));
  nand4  g068(.a(new_n87_), .b(new_n70_), .c(x07), .d(new_n36_), .O(new_n102_));
  oai21  g069(.a(new_n102_), .b(new_n76_), .c(new_n101_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n80_), .c(x24), .O(new_n104_));
  nand3  g071(.a(new_n95_), .b(new_n46_), .c(new_n45_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(x02), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n98_), .O(z1));
  nand3  g075(.a(x24), .b(x18), .c(x02), .O(new_n109_));
  nand3  g076(.a(new_n41_), .b(x15), .c(x13), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(x05), .O(new_n112_));
  nand3  g079(.a(x15), .b(new_n46_), .c(new_n45_), .O(new_n113_));
  nand3  g080(.a(x24), .b(x18), .c(x13), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(x02), .O(new_n116_));
  nand3  g083(.a(x21), .b(new_n52_), .c(new_n51_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n41_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n116_), .c(new_n112_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x20), .c(x14), .d(x11), .O(new_n121_));
  inv1   g088(.a(new_n121_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(x06), .c(x03), .d(x01), .O(new_n123_));
  inv1   g090(.a(new_n123_), .O(z2));
  nand4  g091(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(x00), .O(new_n125_));
  inv1   g092(.a(x08), .O(new_n126_));
  nand4  g093(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n126_), .O(new_n127_));
  nand4  g094(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n128_));
  nor2   g095(.a(new_n38_), .b(new_n37_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(x21), .c(x20), .O(new_n130_));
  oai22  g097(.a(new_n130_), .b(new_n128_), .c(new_n127_), .d(new_n125_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n52_), .c(new_n51_), .O(new_n132_));
  nand3  g099(.a(new_n129_), .b(x20), .c(x15), .O(new_n133_));
  nand2  g100(.a(new_n88_), .b(new_n68_), .O(new_n134_));
  nand3  g101(.a(new_n93_), .b(new_n38_), .c(new_n37_), .O(new_n135_));
  oai22  g102(.a(new_n135_), .b(new_n134_), .c(new_n133_), .d(new_n128_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n81_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n132_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n41_), .O(new_n139_));
  nand3  g106(.a(new_n129_), .b(x20), .c(x18), .O(new_n140_));
  nand3  g107(.a(new_n68_), .b(x07), .c(new_n36_), .O(new_n141_));
  oai22  g108(.a(new_n141_), .b(new_n127_), .c(new_n140_), .d(new_n128_), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n80_), .c(x24), .O(new_n143_));
  nand3  g110(.a(new_n136_), .b(new_n46_), .c(new_n45_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(x02), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n139_), .O(z3));
  inv1   g114(.a(x22), .O(new_n148_));
  inv1   g115(.a(x04), .O(new_n149_));
  inv1   g116(.a(x17), .O(new_n150_));
  oai21  g117(.a(x23), .b(new_n149_), .c(new_n150_), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(new_n148_), .c(x09), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(x16), .c(new_n126_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(z5), .O(new_n154_));
  inv1   g121(.a(x16), .O(new_n155_));
  inv1   g122(.a(x23), .O(new_n156_));
  oai21  g123(.a(new_n156_), .b(x04), .c(x17), .O(new_n157_));
  aoi21  g124(.a(new_n157_), .b(x22), .c(new_n86_), .O(new_n158_));
  oai21  g125(.a(new_n158_), .b(new_n155_), .c(x08), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n120_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n154_), .O(z4));
  aoi21  g128(.a(x20), .b(new_n38_), .c(x06), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x11), .c(new_n35_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(z5), .O(new_n164_));
  aoi21  g131(.a(new_n39_), .b(x14), .c(new_n36_), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(new_n37_), .c(x03), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n120_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n164_), .O(z6));
  inv1   g135(.a(new_n115_), .O(new_n169_));
  inv1   g136(.a(x21), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(x10), .c(new_n41_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n51_), .O(new_n172_));
  and2   g139(.a(x24), .b(x18), .O(new_n173_));
  inv1   g140(.a(new_n110_), .O(new_n174_));
  oai21  g141(.a(new_n174_), .b(new_n173_), .c(x05), .O(new_n175_));
  nand3  g142(.a(new_n175_), .b(new_n172_), .c(new_n169_), .O(z7));
endmodule


