// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x13), .O(new_n44_));
  nand3  g011(.a(x19), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  oai21  g013(.a(new_n46_), .b(new_n42_), .c(new_n37_), .O(new_n47_));
  nor2   g014(.a(new_n47_), .b(x14), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n49_));
  inv1   g016(.a(x02), .O(new_n50_));
  nand2  g017(.a(new_n40_), .b(new_n50_), .O(new_n51_));
  oai21  g018(.a(new_n49_), .b(x01), .c(new_n51_), .O(z0));
  inv1   g019(.a(x01), .O(new_n53_));
  inv1   g020(.a(x04), .O(new_n54_));
  nand4  g021(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n55_));
  nor4   g022(.a(new_n55_), .b(new_n54_), .c(new_n34_), .d(new_n53_), .O(new_n56_));
  nand4  g023(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n57_));
  inv1   g024(.a(x22), .O(new_n58_));
  inv1   g025(.a(x23), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(x20), .c(x17), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand2  g029(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  inv1   g030(.a(x09), .O(new_n64_));
  nor3   g031(.a(x04), .b(x03), .c(x01), .O(new_n65_));
  nor2   g032(.a(x08), .b(x06), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n65_), .c(new_n36_), .d(new_n64_), .O(new_n67_));
  nor2   g034(.a(x14), .b(x12), .O(new_n68_));
  nor2   g035(.a(x17), .b(x16), .O(new_n69_));
  inv1   g036(.a(x19), .O(new_n70_));
  nor2   g037(.a(x20), .b(new_n70_), .O(new_n71_));
  nor2   g038(.a(x23), .b(x22), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n69_), .d(new_n68_), .O(new_n73_));
  oai21  g040(.a(new_n73_), .b(new_n67_), .c(new_n63_), .O(new_n74_));
  nand2  g041(.a(new_n44_), .b(new_n38_), .O(new_n75_));
  nand3  g042(.a(new_n40_), .b(x13), .c(x05), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand4  g045(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n79_));
  inv1   g046(.a(new_n79_), .O(new_n80_));
  inv1   g047(.a(x18), .O(new_n81_));
  nor2   g048(.a(new_n37_), .b(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n80_), .c(new_n60_), .d(new_n56_), .O(new_n83_));
  inv1   g050(.a(x08), .O(new_n84_));
  inv1   g051(.a(x07), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(x06), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n65_), .c(new_n64_), .d(new_n84_), .O(new_n87_));
  nor2   g054(.a(x12), .b(x11), .O(new_n88_));
  nor2   g055(.a(x16), .b(x14), .O(new_n89_));
  nor2   g056(.a(x20), .b(x17), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(new_n72_), .O(new_n91_));
  oai21  g058(.a(new_n91_), .b(new_n87_), .c(new_n83_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n75_), .c(x24), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n78_), .c(new_n51_), .O(z1));
  aoi21  g061(.a(new_n76_), .b(new_n75_), .c(new_n50_), .O(new_n95_));
  nand3  g062(.a(x24), .b(new_n44_), .c(new_n38_), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n95_), .c(x15), .O(new_n98_));
  nand3  g065(.a(new_n75_), .b(x24), .c(x18), .O(new_n99_));
  aoi21  g066(.a(new_n99_), .b(new_n98_), .c(new_n37_), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(x14), .c(x11), .d(x06), .O(new_n101_));
  nor3   g068(.a(new_n101_), .b(new_n34_), .c(new_n53_), .O(z2));
  nand4  g069(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n103_));
  nand4  g070(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n104_));
  nand3  g071(.a(new_n66_), .b(new_n34_), .c(new_n53_), .O(new_n105_));
  inv1   g072(.a(x14), .O(new_n106_));
  nand3  g073(.a(new_n71_), .b(new_n106_), .c(new_n36_), .O(new_n107_));
  oai22  g074(.a(new_n107_), .b(new_n105_), .c(new_n104_), .d(new_n103_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n77_), .O(new_n109_));
  nand3  g076(.a(new_n82_), .b(x14), .c(x11), .O(new_n110_));
  nand3  g077(.a(new_n86_), .b(new_n34_), .c(new_n53_), .O(new_n111_));
  nand4  g078(.a(new_n37_), .b(new_n106_), .c(new_n36_), .d(new_n84_), .O(new_n112_));
  oai22  g079(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(new_n103_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n75_), .c(x24), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(new_n109_), .c(new_n51_), .O(z3));
  nor2   g082(.a(new_n97_), .b(new_n95_), .O(new_n116_));
  inv1   g083(.a(x15), .O(new_n117_));
  inv1   g084(.a(x17), .O(new_n118_));
  aoi21  g085(.a(x23), .b(new_n54_), .c(new_n118_), .O(new_n119_));
  oai21  g086(.a(new_n119_), .b(new_n58_), .c(x09), .O(new_n120_));
  aoi21  g087(.a(new_n120_), .b(x16), .c(new_n84_), .O(new_n121_));
  nor2   g088(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  inv1   g089(.a(x16), .O(new_n123_));
  aoi21  g090(.a(new_n59_), .b(x04), .c(x17), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(x22), .c(new_n64_), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(new_n123_), .c(x08), .O(new_n126_));
  nor2   g093(.a(new_n126_), .b(new_n70_), .O(new_n127_));
  nor2   g094(.a(new_n127_), .b(new_n122_), .O(new_n128_));
  oai22  g095(.a(new_n126_), .b(new_n85_), .c(new_n121_), .d(new_n81_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n75_), .c(x24), .O(new_n130_));
  oai21  g097(.a(new_n128_), .b(new_n116_), .c(new_n130_), .O(z4));
  nand3  g098(.a(new_n75_), .b(x24), .c(x07), .O(new_n132_));
  oai21  g099(.a(new_n116_), .b(new_n70_), .c(new_n132_), .O(z5));
  nand2  g100(.a(x20), .b(new_n106_), .O(new_n134_));
  aoi21  g101(.a(new_n134_), .b(new_n35_), .c(x11), .O(new_n135_));
  oai22  g102(.a(new_n135_), .b(x03), .c(new_n46_), .d(new_n42_), .O(new_n136_));
  nand2  g103(.a(new_n37_), .b(x14), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(x06), .c(new_n36_), .O(new_n138_));
  nand2  g105(.a(x24), .b(x18), .O(new_n139_));
  nand3  g106(.a(new_n40_), .b(x15), .c(x13), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(new_n139_), .c(new_n38_), .O(new_n141_));
  nand3  g108(.a(x24), .b(x18), .c(x13), .O(new_n142_));
  nand3  g109(.a(x15), .b(new_n44_), .c(new_n38_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  oai22  g111(.a(new_n144_), .b(new_n141_), .c(new_n138_), .d(new_n34_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n136_), .c(new_n51_), .O(z6));
  inv1   g113(.a(new_n141_), .O(new_n147_));
  nand4  g114(.a(new_n143_), .b(new_n142_), .c(new_n147_), .d(new_n51_), .O(z7));
endmodule


