// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n186_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  inv1   g019(.a(x23), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(x02), .O(new_n54_));
  oai21  g021(.a(new_n52_), .b(x01), .c(new_n54_), .O(z0));
  inv1   g022(.a(x04), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n34_), .O(new_n57_));
  nand4  g024(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand3  g026(.a(new_n59_), .b(new_n57_), .c(x01), .O(new_n60_));
  nand4  g027(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  inv1   g029(.a(x22), .O(new_n63_));
  nor2   g030(.a(new_n53_), .b(new_n63_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n62_), .c(x20), .d(x17), .O(new_n65_));
  inv1   g032(.a(x09), .O(new_n66_));
  nor3   g033(.a(x04), .b(x03), .c(x01), .O(new_n67_));
  nor2   g034(.a(x08), .b(x06), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n36_), .d(new_n66_), .O(new_n69_));
  nor2   g036(.a(x14), .b(x12), .O(new_n70_));
  nor2   g037(.a(x17), .b(x16), .O(new_n71_));
  inv1   g038(.a(x19), .O(new_n72_));
  nor2   g039(.a(x20), .b(new_n72_), .O(new_n73_));
  nor2   g040(.a(x23), .b(x22), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n71_), .d(new_n70_), .O(new_n75_));
  oai22  g042(.a(new_n75_), .b(new_n69_), .c(new_n65_), .d(new_n60_), .O(new_n76_));
  nand2  g043(.a(new_n47_), .b(new_n38_), .O(new_n77_));
  nand3  g044(.a(new_n40_), .b(x13), .c(x05), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand4  g047(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n64_), .c(x20), .d(x18), .O(new_n83_));
  nor2   g050(.a(x09), .b(x08), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n67_), .c(x07), .d(new_n35_), .O(new_n85_));
  nor2   g052(.a(x12), .b(x11), .O(new_n86_));
  nor2   g053(.a(x16), .b(x14), .O(new_n87_));
  nor2   g054(.a(x20), .b(x17), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n74_), .O(new_n89_));
  oai22  g056(.a(new_n89_), .b(new_n85_), .c(new_n83_), .d(new_n60_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n77_), .c(x24), .O(new_n91_));
  nand4  g058(.a(new_n59_), .b(new_n57_), .c(new_n44_), .d(x01), .O(new_n92_));
  nand4  g059(.a(new_n82_), .b(new_n64_), .c(x21), .d(x20), .O(new_n93_));
  nor2   g060(.a(x03), .b(x01), .O(new_n94_));
  nor2   g061(.a(x06), .b(x04), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n94_), .c(new_n84_), .d(x00), .O(new_n96_));
  oai22  g063(.a(new_n96_), .b(new_n89_), .c(new_n93_), .d(new_n92_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n40_), .c(new_n45_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n91_), .c(new_n80_), .d(new_n54_), .O(z1));
  inv1   g066(.a(x01), .O(new_n100_));
  inv1   g067(.a(x14), .O(new_n101_));
  nand2  g068(.a(x24), .b(x18), .O(new_n102_));
  nand3  g069(.a(new_n40_), .b(x15), .c(x13), .O(new_n103_));
  aoi21  g070(.a(new_n103_), .b(new_n102_), .c(new_n38_), .O(new_n104_));
  nand3  g071(.a(x24), .b(x18), .c(x13), .O(new_n105_));
  nand3  g072(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n106_));
  nand4  g073(.a(new_n40_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n104_), .c(x20), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n101_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(x11), .c(x06), .d(x03), .O(new_n111_));
  oai21  g078(.a(new_n111_), .b(new_n100_), .c(new_n54_), .O(z2));
  inv1   g079(.a(x08), .O(new_n113_));
  nand3  g080(.a(x15), .b(x13), .c(x05), .O(new_n114_));
  nand3  g081(.a(x21), .b(new_n45_), .c(new_n44_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x20), .c(x14), .d(x11), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x06), .c(x03), .d(x01), .O(new_n119_));
  nand3  g086(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n120_));
  nand3  g087(.a(x19), .b(x13), .c(x05), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(new_n37_), .c(new_n101_), .d(new_n36_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(x08), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n35_), .c(new_n34_), .d(new_n100_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n119_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n40_), .O(new_n127_));
  nand4  g094(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n128_));
  nand4  g095(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n129_));
  nand3  g096(.a(new_n94_), .b(x07), .c(new_n35_), .O(new_n130_));
  nand4  g097(.a(new_n37_), .b(new_n101_), .c(new_n36_), .d(new_n113_), .O(new_n131_));
  oai22  g098(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n128_), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n77_), .c(x24), .O(new_n133_));
  nand4  g100(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n134_));
  nand2  g101(.a(new_n94_), .b(new_n68_), .O(new_n135_));
  nand3  g102(.a(new_n73_), .b(new_n101_), .c(new_n36_), .O(new_n136_));
  oai22  g103(.a(new_n136_), .b(new_n135_), .c(new_n134_), .d(new_n128_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n47_), .c(new_n38_), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n133_), .c(new_n54_), .O(new_n139_));
  inv1   g106(.a(new_n139_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n127_), .O(z3));
  inv1   g108(.a(x16), .O(new_n142_));
  inv1   g109(.a(x17), .O(new_n143_));
  oai21  g110(.a(x22), .b(new_n143_), .c(new_n66_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n122_), .O(new_n145_));
  inv1   g112(.a(x00), .O(new_n146_));
  oai21  g113(.a(x10), .b(new_n146_), .c(new_n121_), .O(new_n147_));
  nand4  g114(.a(new_n147_), .b(new_n53_), .c(new_n63_), .d(x04), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n142_), .O(new_n150_));
  oai21  g117(.a(new_n53_), .b(x04), .c(x17), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(x22), .c(new_n66_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n142_), .c(x08), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n116_), .O(new_n154_));
  nand2  g121(.a(new_n122_), .b(x08), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n154_), .c(new_n150_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n40_), .O(new_n157_));
  inv1   g124(.a(x07), .O(new_n158_));
  inv1   g125(.a(x18), .O(new_n159_));
  aoi21  g126(.a(x23), .b(new_n56_), .c(new_n143_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n63_), .c(x09), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(x16), .c(new_n113_), .O(new_n162_));
  aoi21  g129(.a(new_n53_), .b(x04), .c(x17), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(x22), .c(new_n66_), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(new_n142_), .c(x08), .O(new_n165_));
  oai22  g132(.a(new_n165_), .b(new_n158_), .c(new_n162_), .d(new_n159_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n77_), .c(x24), .O(new_n167_));
  inv1   g134(.a(x15), .O(new_n168_));
  oai22  g135(.a(new_n165_), .b(new_n72_), .c(new_n162_), .d(new_n168_), .O(new_n169_));
  nand3  g136(.a(new_n169_), .b(new_n47_), .c(new_n38_), .O(new_n170_));
  nand4  g137(.a(new_n170_), .b(new_n167_), .c(new_n157_), .d(new_n54_), .O(z4));
  inv1   g138(.a(new_n42_), .O(new_n172_));
  and2   g139(.a(new_n46_), .b(new_n43_), .O(new_n173_));
  nand4  g140(.a(new_n173_), .b(new_n54_), .c(new_n48_), .d(new_n172_), .O(z5));
  aoi21  g141(.a(x20), .b(new_n101_), .c(x06), .O(new_n175_));
  oai21  g142(.a(new_n175_), .b(x11), .c(new_n34_), .O(new_n176_));
  nand2  g143(.a(new_n48_), .b(new_n43_), .O(new_n177_));
  oai21  g144(.a(new_n177_), .b(new_n42_), .c(new_n54_), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n46_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n176_), .O(new_n180_));
  aoi21  g147(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n181_));
  oai21  g148(.a(new_n181_), .b(new_n36_), .c(x03), .O(new_n182_));
  nand2  g149(.a(new_n106_), .b(new_n105_), .O(new_n183_));
  oai21  g150(.a(new_n183_), .b(new_n104_), .c(new_n54_), .O(new_n184_));
  nand2  g151(.a(new_n184_), .b(new_n107_), .O(z7));
  nand2  g152(.a(z7), .b(new_n182_), .O(new_n186_));
  nand2  g153(.a(new_n186_), .b(new_n180_), .O(z6));
endmodule


