// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n184_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x01), .O(new_n35_));
  inv1   g002(.a(x03), .O(new_n36_));
  inv1   g003(.a(x06), .O(new_n37_));
  nand4  g004(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x00), .O(new_n38_));
  inv1   g005(.a(x14), .O(new_n39_));
  inv1   g006(.a(x20), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nor2   g008(.a(x11), .b(x10), .O(new_n42_));
  nand4  g009(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n39_), .O(new_n43_));
  oai21  g010(.a(new_n43_), .b(new_n38_), .c(x23), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(new_n34_), .O(new_n45_));
  inv1   g012(.a(x11), .O(new_n46_));
  inv1   g013(.a(x05), .O(new_n47_));
  nand2  g014(.a(x24), .b(x07), .O(new_n48_));
  nand3  g015(.a(new_n41_), .b(x19), .c(x13), .O(new_n49_));
  aoi21  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand3  g017(.a(x24), .b(x13), .c(x07), .O(new_n51_));
  inv1   g018(.a(x13), .O(new_n52_));
  nand3  g019(.a(x19), .b(new_n52_), .c(new_n47_), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  oai21  g021(.a(new_n54_), .b(new_n50_), .c(new_n40_), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(x14), .O(new_n56_));
  nand4  g023(.a(new_n56_), .b(new_n46_), .c(new_n37_), .d(new_n36_), .O(new_n57_));
  oai21  g024(.a(new_n57_), .b(x01), .c(new_n45_), .O(z0));
  nor2   g025(.a(new_n36_), .b(new_n35_), .O(new_n59_));
  inv1   g026(.a(x04), .O(new_n60_));
  nor2   g027(.a(new_n37_), .b(new_n60_), .O(new_n61_));
  inv1   g028(.a(x08), .O(new_n62_));
  inv1   g029(.a(x09), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nor2   g031(.a(new_n46_), .b(x10), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n61_), .d(new_n59_), .O(new_n66_));
  nand4  g033(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n67_));
  inv1   g034(.a(new_n67_), .O(new_n68_));
  inv1   g035(.a(x22), .O(new_n69_));
  nor2   g036(.a(x24), .b(new_n69_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n68_), .c(x21), .d(x20), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n66_), .c(x23), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n34_), .O(new_n73_));
  nand3  g040(.a(x04), .b(x03), .c(x01), .O(new_n74_));
  inv1   g041(.a(new_n74_), .O(new_n75_));
  nand4  g042(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n76_));
  inv1   g043(.a(new_n76_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand4  g045(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n79_));
  nand4  g046(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n80_));
  or2    g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nor3   g048(.a(x04), .b(x03), .c(x01), .O(new_n82_));
  nor2   g049(.a(x08), .b(x06), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n82_), .c(new_n46_), .d(new_n63_), .O(new_n84_));
  nor2   g051(.a(x14), .b(x12), .O(new_n85_));
  nor2   g052(.a(x17), .b(x16), .O(new_n86_));
  inv1   g053(.a(x19), .O(new_n87_));
  nor2   g054(.a(x20), .b(new_n87_), .O(new_n88_));
  nor2   g055(.a(x23), .b(x22), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n85_), .O(new_n90_));
  oai22  g057(.a(new_n90_), .b(new_n84_), .c(new_n81_), .d(new_n78_), .O(new_n91_));
  nand2  g058(.a(new_n52_), .b(new_n47_), .O(new_n92_));
  nand3  g059(.a(new_n41_), .b(x13), .c(x05), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  inv1   g062(.a(x18), .O(new_n96_));
  nor2   g063(.a(new_n40_), .b(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n68_), .c(x23), .d(x22), .O(new_n98_));
  nor2   g065(.a(x09), .b(x08), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n82_), .c(x07), .d(new_n37_), .O(new_n100_));
  nor2   g067(.a(x12), .b(x11), .O(new_n101_));
  nor2   g068(.a(x16), .b(x14), .O(new_n102_));
  nor2   g069(.a(x20), .b(x17), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n102_), .c(new_n101_), .d(new_n89_), .O(new_n104_));
  oai22  g071(.a(new_n104_), .b(new_n100_), .c(new_n98_), .d(new_n78_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n92_), .c(x24), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n95_), .c(new_n73_), .O(z1));
  inv1   g074(.a(x10), .O(new_n108_));
  nand3  g075(.a(new_n59_), .b(new_n108_), .c(x06), .O(new_n109_));
  nor2   g076(.a(new_n39_), .b(new_n46_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(new_n41_), .c(x21), .d(x20), .O(new_n111_));
  oai21  g078(.a(new_n111_), .b(new_n109_), .c(x23), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n34_), .O(new_n113_));
  nand2  g080(.a(x24), .b(x18), .O(new_n114_));
  nand3  g081(.a(new_n41_), .b(x15), .c(x13), .O(new_n115_));
  aoi21  g082(.a(new_n115_), .b(new_n114_), .c(new_n47_), .O(new_n116_));
  nand3  g083(.a(x15), .b(new_n52_), .c(new_n47_), .O(new_n117_));
  oai21  g084(.a(new_n114_), .b(new_n52_), .c(new_n117_), .O(new_n118_));
  oai21  g085(.a(new_n118_), .b(new_n116_), .c(x20), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n39_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x11), .c(x06), .d(x03), .O(new_n121_));
  oai21  g088(.a(new_n121_), .b(new_n35_), .c(new_n113_), .O(z2));
  nand3  g089(.a(new_n59_), .b(x08), .c(x06), .O(new_n123_));
  nand3  g090(.a(new_n110_), .b(x20), .c(x15), .O(new_n124_));
  nand3  g091(.a(new_n83_), .b(new_n36_), .c(new_n35_), .O(new_n125_));
  nand3  g092(.a(new_n88_), .b(new_n39_), .c(new_n46_), .O(new_n126_));
  oai22  g093(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n123_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n94_), .O(new_n128_));
  nand2  g095(.a(new_n110_), .b(new_n97_), .O(new_n129_));
  nand4  g096(.a(x07), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n130_));
  nand4  g097(.a(new_n40_), .b(new_n39_), .c(new_n46_), .d(new_n62_), .O(new_n131_));
  oai22  g098(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n123_), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n92_), .c(x24), .O(new_n133_));
  nand3  g100(.a(new_n110_), .b(x21), .c(x20), .O(new_n134_));
  oai22  g101(.a(new_n134_), .b(new_n123_), .c(new_n131_), .d(new_n38_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n41_), .c(new_n108_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(x23), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n34_), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n133_), .c(new_n128_), .O(z3));
  nand2  g106(.a(new_n69_), .b(x17), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(new_n63_), .c(x16), .O(new_n141_));
  inv1   g108(.a(x00), .O(new_n142_));
  nor2   g109(.a(x02), .b(new_n142_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n41_), .c(new_n108_), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n53_), .c(new_n51_), .O(new_n145_));
  oai22  g112(.a(new_n145_), .b(new_n50_), .c(new_n141_), .d(x08), .O(new_n146_));
  inv1   g113(.a(x17), .O(new_n147_));
  aoi21  g114(.a(x23), .b(new_n60_), .c(new_n147_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n69_), .c(x09), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(x16), .c(new_n62_), .O(new_n150_));
  inv1   g117(.a(x16), .O(new_n151_));
  nand4  g118(.a(new_n89_), .b(new_n151_), .c(x07), .d(x04), .O(new_n152_));
  oai21  g119(.a(new_n150_), .b(new_n96_), .c(new_n152_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n92_), .c(x24), .O(new_n154_));
  inv1   g121(.a(x15), .O(new_n155_));
  nand4  g122(.a(new_n89_), .b(x19), .c(new_n151_), .d(x04), .O(new_n156_));
  oai21  g123(.a(new_n150_), .b(new_n155_), .c(new_n156_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n94_), .O(new_n158_));
  nand2  g125(.a(x17), .b(x04), .O(new_n159_));
  aoi21  g126(.a(new_n159_), .b(x22), .c(new_n63_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n151_), .c(x08), .O(new_n161_));
  nand4  g128(.a(new_n161_), .b(new_n41_), .c(x21), .d(new_n108_), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(x23), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n34_), .O(new_n164_));
  nand4  g131(.a(new_n164_), .b(new_n158_), .c(new_n154_), .d(new_n146_), .O(z4));
  nor2   g132(.a(new_n54_), .b(new_n50_), .O(new_n166_));
  nand3  g133(.a(new_n41_), .b(new_n108_), .c(x00), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(x23), .O(new_n168_));
  nand2  g135(.a(new_n168_), .b(new_n34_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n166_), .O(z5));
  aoi21  g137(.a(x20), .b(new_n39_), .c(x06), .O(new_n171_));
  oai21  g138(.a(new_n171_), .b(x11), .c(new_n36_), .O(new_n172_));
  nor2   g139(.a(x23), .b(x02), .O(new_n173_));
  inv1   g140(.a(x23), .O(new_n174_));
  nor2   g141(.a(x24), .b(new_n174_), .O(new_n175_));
  nand3  g142(.a(new_n175_), .b(new_n143_), .c(new_n108_), .O(new_n176_));
  oai21  g143(.a(new_n173_), .b(new_n166_), .c(new_n176_), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n172_), .O(new_n178_));
  aoi21  g145(.a(new_n40_), .b(x14), .c(new_n37_), .O(new_n179_));
  oai21  g146(.a(new_n179_), .b(new_n46_), .c(x03), .O(new_n180_));
  nor2   g147(.a(new_n118_), .b(new_n116_), .O(new_n181_));
  nand4  g148(.a(new_n175_), .b(x21), .c(new_n108_), .d(new_n34_), .O(new_n182_));
  oai21  g149(.a(new_n173_), .b(new_n181_), .c(new_n182_), .O(z7));
  nand2  g150(.a(z7), .b(new_n180_), .O(new_n184_));
  nand2  g151(.a(new_n184_), .b(new_n178_), .O(z6));
endmodule


