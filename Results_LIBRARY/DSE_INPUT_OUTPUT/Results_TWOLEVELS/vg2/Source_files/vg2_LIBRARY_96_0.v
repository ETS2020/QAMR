// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x01), .O(new_n35_));
  inv1   g002(.a(x03), .O(new_n36_));
  inv1   g003(.a(x06), .O(new_n37_));
  nand4  g004(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x00), .O(new_n38_));
  inv1   g005(.a(x14), .O(new_n39_));
  inv1   g006(.a(x20), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nor2   g008(.a(x11), .b(x10), .O(new_n42_));
  nand4  g009(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n39_), .O(new_n43_));
  oai21  g010(.a(new_n43_), .b(new_n38_), .c(x05), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(new_n34_), .O(new_n45_));
  inv1   g012(.a(x11), .O(new_n46_));
  inv1   g013(.a(x05), .O(new_n47_));
  nand2  g014(.a(x24), .b(x07), .O(new_n48_));
  nand3  g015(.a(new_n41_), .b(x19), .c(x13), .O(new_n49_));
  aoi21  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand3  g017(.a(x24), .b(x13), .c(x07), .O(new_n51_));
  inv1   g018(.a(x13), .O(new_n52_));
  nand3  g019(.a(x19), .b(new_n52_), .c(new_n47_), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nor3   g022(.a(new_n55_), .b(x20), .c(x14), .O(new_n56_));
  nand4  g023(.a(new_n56_), .b(new_n46_), .c(new_n37_), .d(new_n36_), .O(new_n57_));
  oai21  g024(.a(new_n57_), .b(x01), .c(new_n45_), .O(z0));
  inv1   g025(.a(x12), .O(new_n59_));
  nand2  g026(.a(x15), .b(x13), .O(new_n60_));
  inv1   g027(.a(x10), .O(new_n61_));
  nand3  g028(.a(x21), .b(new_n61_), .c(new_n34_), .O(new_n62_));
  nand2  g029(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(x23), .c(x22), .d(x20), .O(new_n64_));
  inv1   g031(.a(new_n64_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(x17), .c(x16), .d(x14), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n59_), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(x11), .c(x09), .d(x08), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n37_), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(x04), .c(x03), .d(x01), .O(new_n70_));
  inv1   g037(.a(x04), .O(new_n71_));
  inv1   g038(.a(x08), .O(new_n72_));
  inv1   g039(.a(x09), .O(new_n73_));
  inv1   g040(.a(x16), .O(new_n74_));
  inv1   g041(.a(x17), .O(new_n75_));
  inv1   g042(.a(x22), .O(new_n76_));
  inv1   g043(.a(x23), .O(new_n77_));
  nand3  g044(.a(new_n61_), .b(new_n34_), .c(x00), .O(new_n78_));
  nand2  g045(.a(x19), .b(x13), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n77_), .c(new_n76_), .d(new_n40_), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n75_), .c(new_n74_), .d(new_n39_), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(x12), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n46_), .c(new_n73_), .d(new_n72_), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(x06), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n71_), .c(new_n36_), .d(new_n35_), .O(new_n87_));
  aoi21  g054(.a(new_n87_), .b(new_n70_), .c(x24), .O(new_n88_));
  nand4  g055(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n89_));
  nor4   g056(.a(new_n89_), .b(new_n71_), .c(new_n36_), .d(new_n35_), .O(new_n90_));
  nand4  g057(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n91_));
  nand4  g058(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nor3   g061(.a(x04), .b(x03), .c(x01), .O(new_n95_));
  nand4  g062(.a(new_n73_), .b(new_n72_), .c(x07), .d(new_n37_), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n74_), .b(new_n39_), .c(new_n59_), .d(new_n46_), .O(new_n98_));
  nand4  g065(.a(new_n77_), .b(new_n76_), .c(new_n40_), .d(new_n75_), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n97_), .c(new_n95_), .O(new_n101_));
  aoi21  g068(.a(new_n101_), .b(new_n94_), .c(new_n41_), .O(new_n102_));
  oai21  g069(.a(new_n102_), .b(new_n88_), .c(x05), .O(new_n103_));
  nand3  g070(.a(x15), .b(new_n52_), .c(new_n47_), .O(new_n104_));
  nand2  g071(.a(x24), .b(x18), .O(new_n105_));
  inv1   g072(.a(new_n105_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(x13), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x23), .c(x22), .d(x20), .O(new_n109_));
  inv1   g076(.a(new_n109_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(x17), .c(x16), .d(x14), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n59_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(x11), .c(x09), .d(x08), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n37_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(x04), .c(x03), .d(x01), .O(new_n115_));
  nand4  g082(.a(new_n54_), .b(new_n77_), .c(new_n76_), .d(new_n40_), .O(new_n116_));
  inv1   g083(.a(new_n116_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(new_n75_), .c(new_n74_), .d(new_n39_), .O(new_n118_));
  nor2   g085(.a(new_n118_), .b(x12), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(new_n46_), .c(new_n73_), .d(new_n72_), .O(new_n120_));
  nor2   g087(.a(new_n120_), .b(x06), .O(new_n121_));
  nand4  g088(.a(new_n121_), .b(new_n71_), .c(new_n36_), .d(new_n35_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n115_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(x02), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n103_), .O(z1));
  nand4  g092(.a(new_n61_), .b(x06), .c(x03), .d(x01), .O(new_n126_));
  nor2   g093(.a(new_n39_), .b(new_n46_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(new_n41_), .c(x21), .d(x20), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(new_n126_), .c(x05), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n34_), .O(new_n130_));
  nand3  g097(.a(new_n41_), .b(x15), .c(x13), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(new_n105_), .c(new_n47_), .O(new_n132_));
  oai21  g099(.a(new_n132_), .b(new_n108_), .c(x20), .O(new_n133_));
  nor2   g100(.a(new_n133_), .b(new_n39_), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(x11), .c(x06), .d(x03), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(new_n35_), .c(new_n130_), .O(z2));
  nor2   g103(.a(x13), .b(x05), .O(new_n137_));
  inv1   g104(.a(new_n137_), .O(new_n138_));
  nand4  g105(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n139_));
  nand3  g106(.a(new_n127_), .b(x20), .c(x18), .O(new_n140_));
  nand4  g107(.a(x07), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n141_));
  nand4  g108(.a(new_n40_), .b(new_n39_), .c(new_n46_), .d(new_n72_), .O(new_n142_));
  oai22  g109(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(new_n139_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n138_), .c(x24), .O(new_n144_));
  nand3  g111(.a(new_n127_), .b(x20), .c(x15), .O(new_n145_));
  nand4  g112(.a(new_n72_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n146_));
  nand4  g113(.a(new_n40_), .b(x19), .c(new_n39_), .d(new_n46_), .O(new_n147_));
  oai22  g114(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(new_n139_), .O(new_n148_));
  nand3  g115(.a(new_n41_), .b(x13), .c(x05), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n138_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand3  g118(.a(new_n127_), .b(x21), .c(x20), .O(new_n152_));
  oai22  g119(.a(new_n152_), .b(new_n139_), .c(new_n142_), .d(new_n38_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n41_), .c(new_n61_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(x05), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n34_), .O(new_n156_));
  nand3  g123(.a(new_n156_), .b(new_n151_), .c(new_n144_), .O(z3));
  oai21  g124(.a(x23), .b(new_n71_), .c(new_n75_), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(new_n76_), .c(x09), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(x16), .c(new_n72_), .O(new_n160_));
  nand2  g127(.a(new_n54_), .b(x02), .O(new_n161_));
  inv1   g128(.a(new_n48_), .O(new_n162_));
  aoi21  g129(.a(new_n80_), .b(new_n41_), .c(new_n162_), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(new_n47_), .c(new_n161_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n160_), .O(new_n165_));
  oai21  g132(.a(new_n77_), .b(x04), .c(x17), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(x22), .c(new_n73_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n74_), .c(x08), .O(new_n168_));
  nand2  g135(.a(new_n108_), .b(x02), .O(new_n169_));
  aoi21  g136(.a(new_n63_), .b(new_n41_), .c(new_n106_), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(new_n47_), .c(new_n169_), .O(z7));
  nand2  g138(.a(z7), .b(new_n168_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(new_n165_), .O(z4));
  nand3  g140(.a(new_n41_), .b(new_n61_), .c(x00), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(x05), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n34_), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(new_n55_), .O(z5));
  nand2  g144(.a(x20), .b(new_n39_), .O(new_n178_));
  aoi21  g145(.a(new_n178_), .b(new_n37_), .c(x11), .O(new_n179_));
  nand4  g146(.a(new_n41_), .b(new_n61_), .c(new_n34_), .d(x00), .O(new_n180_));
  nand3  g147(.a(new_n180_), .b(new_n53_), .c(new_n51_), .O(new_n181_));
  oai22  g148(.a(new_n181_), .b(new_n50_), .c(new_n179_), .d(x03), .O(new_n182_));
  nand2  g149(.a(new_n40_), .b(x14), .O(new_n183_));
  aoi21  g150(.a(new_n183_), .b(x06), .c(new_n46_), .O(new_n184_));
  nand4  g151(.a(new_n41_), .b(x21), .c(new_n61_), .d(new_n34_), .O(new_n185_));
  nand3  g152(.a(new_n185_), .b(new_n107_), .c(new_n104_), .O(new_n186_));
  oai22  g153(.a(new_n186_), .b(new_n132_), .c(new_n184_), .d(new_n36_), .O(new_n187_));
  nand2  g154(.a(new_n47_), .b(new_n34_), .O(new_n188_));
  nand3  g155(.a(new_n188_), .b(new_n187_), .c(new_n182_), .O(z6));
endmodule


