// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nand2  g019(.a(x02), .b(x01), .O(new_n53_));
  oai21  g020(.a(new_n52_), .b(x01), .c(new_n53_), .O(z0));
  inv1   g021(.a(x04), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n34_), .O(new_n56_));
  nand4  g023(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n57_));
  inv1   g024(.a(new_n57_), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n56_), .c(new_n44_), .d(x01), .O(new_n59_));
  nand4  g026(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  inv1   g028(.a(x22), .O(new_n62_));
  inv1   g029(.a(x23), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n61_), .c(x20), .d(x17), .O(new_n65_));
  nor3   g032(.a(x04), .b(x03), .c(x01), .O(new_n66_));
  inv1   g033(.a(x08), .O(new_n67_));
  inv1   g034(.a(x09), .O(new_n68_));
  nand4  g035(.a(new_n36_), .b(new_n68_), .c(new_n67_), .d(new_n35_), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  inv1   g037(.a(x12), .O(new_n71_));
  inv1   g038(.a(x14), .O(new_n72_));
  inv1   g039(.a(x16), .O(new_n73_));
  inv1   g040(.a(x17), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nor2   g042(.a(x23), .b(x22), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(new_n37_), .c(x19), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n70_), .c(new_n66_), .O(new_n79_));
  oai21  g046(.a(new_n65_), .b(new_n59_), .c(new_n79_), .O(new_n80_));
  nand2  g047(.a(new_n47_), .b(new_n38_), .O(new_n81_));
  nand3  g048(.a(new_n40_), .b(x13), .c(x05), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand4  g051(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n85_));
  inv1   g052(.a(new_n85_), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n64_), .c(x20), .d(x18), .O(new_n87_));
  nor2   g054(.a(x09), .b(x08), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n66_), .c(x07), .d(new_n35_), .O(new_n89_));
  nand4  g056(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n36_), .O(new_n90_));
  inv1   g057(.a(new_n90_), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n76_), .c(new_n37_), .d(new_n74_), .O(new_n92_));
  oai22  g059(.a(new_n92_), .b(new_n89_), .c(new_n87_), .d(new_n59_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n81_), .c(x24), .O(new_n94_));
  nand2  g061(.a(new_n56_), .b(x01), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  nand3  g063(.a(new_n64_), .b(x21), .c(x20), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n96_), .c(new_n86_), .d(new_n58_), .O(new_n99_));
  nor2   g066(.a(x03), .b(x01), .O(new_n100_));
  nor2   g067(.a(x06), .b(x04), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n100_), .c(new_n88_), .d(x00), .O(new_n102_));
  oai21  g069(.a(new_n102_), .b(new_n92_), .c(new_n99_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n40_), .c(new_n45_), .d(new_n44_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n94_), .c(new_n84_), .O(z1));
  inv1   g072(.a(x01), .O(new_n106_));
  nand2  g073(.a(x24), .b(x18), .O(new_n107_));
  nand3  g074(.a(new_n40_), .b(x15), .c(x13), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(x05), .O(new_n110_));
  nand3  g077(.a(x24), .b(x18), .c(x13), .O(new_n111_));
  nand3  g078(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n112_));
  nand3  g079(.a(new_n40_), .b(x21), .c(new_n45_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  inv1   g081(.a(new_n114_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n110_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x20), .c(x14), .d(x11), .O(new_n117_));
  inv1   g084(.a(new_n117_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x06), .c(x03), .d(new_n44_), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n106_), .O(z2));
  nand4  g087(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n121_));
  nor2   g088(.a(new_n72_), .b(new_n36_), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(x20), .c(x15), .O(new_n123_));
  nand3  g090(.a(new_n100_), .b(new_n67_), .c(new_n35_), .O(new_n124_));
  nand4  g091(.a(new_n37_), .b(x19), .c(new_n72_), .d(new_n36_), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n121_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n83_), .O(new_n127_));
  nand3  g094(.a(new_n122_), .b(x20), .c(x18), .O(new_n128_));
  nand3  g095(.a(new_n100_), .b(x07), .c(new_n35_), .O(new_n129_));
  nand4  g096(.a(new_n37_), .b(new_n72_), .c(new_n36_), .d(new_n67_), .O(new_n130_));
  oai22  g097(.a(new_n130_), .b(new_n129_), .c(new_n128_), .d(new_n121_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n81_), .c(x24), .O(new_n132_));
  nand4  g099(.a(new_n45_), .b(x08), .c(x06), .d(x03), .O(new_n133_));
  inv1   g100(.a(x21), .O(new_n134_));
  nor2   g101(.a(x24), .b(new_n134_), .O(new_n135_));
  nand3  g102(.a(new_n122_), .b(new_n135_), .c(x20), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(new_n133_), .c(new_n44_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(x01), .O(new_n138_));
  inv1   g105(.a(x00), .O(new_n139_));
  nor2   g106(.a(x01), .b(new_n139_), .O(new_n140_));
  nor3   g107(.a(x06), .b(x03), .c(x02), .O(new_n141_));
  nor3   g108(.a(x11), .b(x10), .c(x08), .O(new_n142_));
  nor3   g109(.a(x24), .b(x20), .c(x14), .O(new_n143_));
  nand4  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n140_), .O(new_n144_));
  nand4  g111(.a(new_n144_), .b(new_n138_), .c(new_n132_), .d(new_n127_), .O(z3));
  oai21  g112(.a(x23), .b(new_n55_), .c(new_n74_), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(new_n62_), .c(x09), .O(new_n147_));
  oai21  g114(.a(new_n147_), .b(x16), .c(new_n67_), .O(new_n148_));
  nand2  g115(.a(new_n48_), .b(new_n43_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n42_), .c(new_n53_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n46_), .O(z5));
  nand2  g118(.a(z5), .b(new_n148_), .O(new_n152_));
  oai21  g119(.a(new_n63_), .b(x04), .c(x17), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(x22), .c(new_n68_), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(new_n73_), .c(x08), .O(new_n155_));
  nand3  g122(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n53_), .O(new_n157_));
  nand3  g124(.a(new_n135_), .b(new_n45_), .c(new_n44_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n152_), .O(z4));
  nand2  g128(.a(x20), .b(new_n72_), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(new_n35_), .c(x11), .O(new_n163_));
  oai22  g130(.a(new_n163_), .b(x03), .c(new_n49_), .d(new_n42_), .O(new_n164_));
  nand2  g131(.a(new_n37_), .b(x14), .O(new_n165_));
  aoi21  g132(.a(new_n165_), .b(x06), .c(new_n36_), .O(new_n166_));
  and2   g133(.a(new_n158_), .b(new_n112_), .O(new_n167_));
  nand3  g134(.a(new_n167_), .b(new_n111_), .c(new_n110_), .O(new_n168_));
  oai21  g135(.a(new_n166_), .b(new_n34_), .c(new_n168_), .O(new_n169_));
  nand3  g136(.a(new_n169_), .b(new_n164_), .c(new_n53_), .O(z6));
  nand4  g137(.a(new_n167_), .b(new_n111_), .c(new_n110_), .d(new_n53_), .O(z7));
endmodule


