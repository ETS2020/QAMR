// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x14), .O(new_n37_));
  inv1   g004(.a(x20), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x05), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x05), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n45_), .O(new_n47_));
  and2   g014(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n38_), .c(new_n37_), .d(new_n36_), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n35_), .c(new_n34_), .d(x02), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x01), .O(z0));
  nand3  g020(.a(x04), .b(x03), .c(x01), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  inv1   g022(.a(x08), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n35_), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(x11), .c(x09), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n60_));
  nand4  g027(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n59_), .c(new_n55_), .O(new_n63_));
  inv1   g030(.a(x01), .O(new_n64_));
  inv1   g031(.a(x04), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n34_), .c(x02), .d(new_n64_), .O(new_n66_));
  inv1   g033(.a(new_n66_), .O(new_n67_));
  nor2   g034(.a(x11), .b(x09), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n56_), .d(new_n35_), .O(new_n69_));
  nor2   g036(.a(x14), .b(x12), .O(new_n70_));
  nor2   g037(.a(x17), .b(x16), .O(new_n71_));
  inv1   g038(.a(x19), .O(new_n72_));
  nor2   g039(.a(x20), .b(new_n72_), .O(new_n73_));
  nor2   g040(.a(x23), .b(x22), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n71_), .d(new_n70_), .O(new_n75_));
  oai21  g042(.a(new_n75_), .b(new_n69_), .c(new_n63_), .O(new_n76_));
  nand2  g043(.a(new_n46_), .b(new_n45_), .O(new_n77_));
  nand3  g044(.a(new_n40_), .b(x13), .c(x05), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand4  g047(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n81_));
  nand4  g048(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n59_), .c(new_n55_), .O(new_n84_));
  nor2   g051(.a(x09), .b(x08), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n67_), .c(x07), .d(new_n35_), .O(new_n86_));
  nor2   g053(.a(x12), .b(x11), .O(new_n87_));
  nor2   g054(.a(x16), .b(x14), .O(new_n88_));
  nor2   g055(.a(x20), .b(x17), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n74_), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n86_), .c(new_n84_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n77_), .c(x24), .O(new_n92_));
  inv1   g059(.a(new_n81_), .O(new_n93_));
  inv1   g060(.a(x02), .O(new_n94_));
  nand4  g061(.a(x04), .b(x03), .c(new_n94_), .d(x01), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  inv1   g063(.a(x10), .O(new_n97_));
  nand3  g064(.a(x11), .b(new_n97_), .c(x09), .O(new_n98_));
  nor3   g065(.a(new_n98_), .b(new_n56_), .c(new_n35_), .O(new_n99_));
  inv1   g066(.a(x21), .O(new_n100_));
  nand3  g067(.a(new_n40_), .b(x23), .c(x22), .O(new_n101_));
  nor3   g068(.a(new_n101_), .b(new_n100_), .c(new_n38_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n99_), .c(new_n96_), .d(new_n93_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n92_), .c(new_n80_), .O(z1));
  nand2  g071(.a(x24), .b(x18), .O(new_n105_));
  nand3  g072(.a(new_n40_), .b(x15), .c(x13), .O(new_n106_));
  aoi21  g073(.a(new_n106_), .b(new_n105_), .c(new_n45_), .O(new_n107_));
  nand3  g074(.a(x24), .b(x18), .c(x13), .O(new_n108_));
  nand3  g075(.a(x15), .b(new_n46_), .c(new_n45_), .O(new_n109_));
  nand4  g076(.a(new_n40_), .b(x21), .c(new_n97_), .d(new_n94_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n109_), .c(new_n108_), .O(new_n111_));
  oai21  g078(.a(new_n111_), .b(new_n107_), .c(x20), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n37_), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(x11), .c(x06), .d(x03), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n64_), .O(z2));
  nand3  g082(.a(new_n57_), .b(x03), .c(x01), .O(new_n116_));
  nor2   g083(.a(new_n37_), .b(new_n36_), .O(new_n117_));
  nand3  g084(.a(new_n117_), .b(x20), .c(x15), .O(new_n118_));
  nand4  g085(.a(new_n35_), .b(new_n34_), .c(x02), .d(new_n64_), .O(new_n119_));
  nand4  g086(.a(new_n73_), .b(new_n37_), .c(new_n36_), .d(new_n56_), .O(new_n120_));
  oai22  g087(.a(new_n120_), .b(new_n119_), .c(new_n118_), .d(new_n116_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n79_), .O(new_n122_));
  nand3  g089(.a(new_n117_), .b(x20), .c(x18), .O(new_n123_));
  nor2   g090(.a(x20), .b(x14), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n36_), .c(new_n56_), .d(x07), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n119_), .c(new_n123_), .d(new_n116_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n77_), .c(x24), .O(new_n127_));
  nor2   g094(.a(x02), .b(new_n64_), .O(new_n128_));
  nand2  g095(.a(new_n117_), .b(new_n97_), .O(new_n129_));
  nor2   g096(.a(x24), .b(new_n100_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(x20), .O(new_n131_));
  nor2   g098(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n128_), .c(new_n57_), .d(x03), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n127_), .c(new_n122_), .O(z3));
  inv1   g101(.a(x22), .O(new_n135_));
  inv1   g102(.a(x17), .O(new_n136_));
  oai21  g103(.a(x23), .b(new_n65_), .c(new_n136_), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(new_n135_), .c(x09), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(x16), .c(new_n56_), .O(new_n139_));
  nand4  g106(.a(new_n40_), .b(new_n97_), .c(new_n94_), .d(x00), .O(new_n140_));
  nand4  g107(.a(new_n140_), .b(new_n47_), .c(new_n44_), .d(new_n43_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  inv1   g109(.a(x16), .O(new_n143_));
  inv1   g110(.a(x09), .O(new_n144_));
  inv1   g111(.a(x23), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(x04), .c(x17), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x22), .c(new_n144_), .O(new_n147_));
  oai21  g114(.a(new_n147_), .b(new_n143_), .c(x08), .O(new_n148_));
  oai21  g115(.a(new_n111_), .b(new_n107_), .c(new_n148_), .O(new_n149_));
  nand2  g116(.a(new_n36_), .b(new_n94_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n149_), .c(new_n142_), .O(z4));
  nand3  g118(.a(new_n40_), .b(new_n97_), .c(x00), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(x11), .c(x02), .O(new_n153_));
  or2    g120(.a(new_n153_), .b(new_n49_), .O(z5));
  nand3  g121(.a(new_n97_), .b(new_n94_), .c(x00), .O(new_n155_));
  nand3  g122(.a(x19), .b(x13), .c(x05), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n155_), .c(new_n34_), .O(new_n157_));
  nand3  g124(.a(x15), .b(x13), .c(x05), .O(new_n158_));
  nand3  g125(.a(x21), .b(new_n97_), .c(new_n94_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n34_), .O(new_n161_));
  oai21  g128(.a(x20), .b(new_n37_), .c(x06), .O(new_n162_));
  nand4  g129(.a(x15), .b(x13), .c(x11), .d(x05), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  oai21  g132(.a(new_n38_), .b(x14), .c(new_n35_), .O(new_n166_));
  and2   g133(.a(new_n166_), .b(x19), .O(new_n167_));
  nand4  g134(.a(new_n167_), .b(x13), .c(new_n36_), .d(x05), .O(new_n168_));
  nand3  g135(.a(new_n168_), .b(new_n165_), .c(new_n161_), .O(new_n169_));
  oai21  g136(.a(new_n169_), .b(new_n157_), .c(new_n40_), .O(new_n170_));
  inv1   g137(.a(x07), .O(new_n171_));
  inv1   g138(.a(x18), .O(new_n172_));
  aoi21  g139(.a(new_n162_), .b(x11), .c(new_n34_), .O(new_n173_));
  aoi21  g140(.a(new_n166_), .b(new_n36_), .c(x03), .O(new_n174_));
  oai22  g141(.a(new_n174_), .b(new_n171_), .c(new_n173_), .d(new_n172_), .O(new_n175_));
  nand3  g142(.a(new_n175_), .b(new_n77_), .c(x24), .O(new_n176_));
  inv1   g143(.a(x15), .O(new_n177_));
  oai22  g144(.a(new_n174_), .b(new_n72_), .c(new_n173_), .d(new_n177_), .O(new_n178_));
  nand3  g145(.a(new_n178_), .b(new_n46_), .c(new_n45_), .O(new_n179_));
  nand4  g146(.a(new_n179_), .b(new_n176_), .c(new_n170_), .d(new_n150_), .O(z6));
  aoi21  g147(.a(new_n130_), .b(new_n97_), .c(new_n36_), .O(new_n181_));
  nand2  g148(.a(new_n109_), .b(new_n108_), .O(new_n182_));
  nor2   g149(.a(new_n182_), .b(new_n107_), .O(new_n183_));
  oai21  g150(.a(new_n181_), .b(x02), .c(new_n183_), .O(z7));
endmodule


