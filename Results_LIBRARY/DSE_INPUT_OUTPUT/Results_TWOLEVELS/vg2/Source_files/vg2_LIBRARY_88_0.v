// Benchmark "FAU" written by ABC on Wed Aug 19 21:57:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  inv1   g010(.a(x00), .O(new_n44_));
  nor2   g011(.a(x02), .b(new_n44_), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(new_n39_), .c(new_n43_), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n37_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n42_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n41_), .O(new_n50_));
  nor3   g017(.a(new_n50_), .b(x20), .c(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nor2   g019(.a(x21), .b(x17), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  oai21  g021(.a(new_n52_), .b(x01), .c(new_n54_), .O(z0));
  inv1   g022(.a(x01), .O(new_n56_));
  inv1   g023(.a(x04), .O(new_n57_));
  inv1   g024(.a(x08), .O(new_n58_));
  inv1   g025(.a(x09), .O(new_n59_));
  inv1   g026(.a(x14), .O(new_n60_));
  inv1   g027(.a(x16), .O(new_n61_));
  inv1   g028(.a(x17), .O(new_n62_));
  inv1   g029(.a(x20), .O(new_n63_));
  inv1   g030(.a(x22), .O(new_n64_));
  inv1   g031(.a(x23), .O(new_n65_));
  inv1   g032(.a(x02), .O(new_n66_));
  nand3  g033(.a(new_n43_), .b(new_n66_), .c(x00), .O(new_n67_));
  nand3  g034(.a(x19), .b(x13), .c(x05), .O(new_n68_));
  nand2  g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n72_));
  nor2   g039(.a(new_n72_), .b(x12), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n36_), .c(new_n59_), .d(new_n58_), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(x06), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n57_), .c(new_n34_), .d(new_n56_), .O(new_n76_));
  nand4  g043(.a(x04), .b(x03), .c(new_n66_), .d(x01), .O(new_n77_));
  nand4  g044(.a(new_n43_), .b(x09), .c(x08), .d(x06), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand4  g046(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n80_));
  nor2   g047(.a(new_n65_), .b(new_n64_), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nor4   g049(.a(new_n82_), .b(new_n80_), .c(new_n63_), .d(new_n62_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  aoi21  g051(.a(new_n84_), .b(new_n76_), .c(x24), .O(new_n85_));
  inv1   g052(.a(x12), .O(new_n86_));
  inv1   g053(.a(x07), .O(new_n87_));
  oai21  g054(.a(x13), .b(x05), .c(x24), .O(new_n88_));
  oai21  g055(.a(new_n88_), .b(new_n87_), .c(new_n48_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n90_));
  inv1   g057(.a(new_n90_), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n86_), .c(new_n36_), .d(new_n59_), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(x08), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n35_), .c(new_n57_), .d(new_n34_), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(x01), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n85_), .c(x21), .O(new_n98_));
  nand2  g065(.a(x24), .b(x18), .O(new_n99_));
  nand3  g066(.a(new_n39_), .b(x15), .c(x13), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(x05), .O(new_n102_));
  nand3  g069(.a(x24), .b(x18), .c(x13), .O(new_n103_));
  nand3  g070(.a(x15), .b(new_n47_), .c(new_n37_), .O(new_n104_));
  and2   g071(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(x23), .c(x22), .d(x20), .O(new_n107_));
  inv1   g074(.a(new_n107_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x17), .c(x16), .d(x14), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n86_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(x11), .c(x09), .d(x08), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n35_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(x04), .c(x03), .d(x01), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n98_), .O(z1));
  inv1   g081(.a(new_n106_), .O(new_n115_));
  inv1   g082(.a(x21), .O(new_n116_));
  nor2   g083(.a(x24), .b(new_n116_), .O(new_n117_));
  nand3  g084(.a(new_n117_), .b(new_n43_), .c(new_n66_), .O(new_n118_));
  oai21  g085(.a(new_n115_), .b(new_n53_), .c(new_n118_), .O(new_n119_));
  and2   g086(.a(new_n119_), .b(x20), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x14), .c(x11), .d(x06), .O(new_n121_));
  nor3   g088(.a(new_n121_), .b(new_n34_), .c(new_n56_), .O(z2));
  nand3  g089(.a(x15), .b(x13), .c(x05), .O(new_n123_));
  nand3  g090(.a(x21), .b(new_n43_), .c(new_n66_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand4  g092(.a(new_n125_), .b(x20), .c(x14), .d(x11), .O(new_n126_));
  nor2   g093(.a(new_n126_), .b(new_n58_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(x06), .c(x03), .d(x01), .O(new_n128_));
  nand4  g095(.a(new_n69_), .b(new_n63_), .c(new_n60_), .d(new_n36_), .O(new_n129_));
  nor2   g096(.a(new_n129_), .b(x08), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(new_n35_), .c(new_n34_), .d(new_n56_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n39_), .O(new_n133_));
  nand2  g100(.a(new_n47_), .b(new_n37_), .O(new_n134_));
  nand4  g101(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n135_));
  nand4  g102(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n136_));
  nand4  g103(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n56_), .O(new_n137_));
  nand4  g104(.a(new_n63_), .b(new_n60_), .c(new_n36_), .d(new_n58_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n135_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n134_), .c(x24), .O(new_n140_));
  nand4  g107(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n141_));
  nand4  g108(.a(new_n58_), .b(new_n35_), .c(new_n34_), .d(new_n56_), .O(new_n142_));
  nand4  g109(.a(new_n63_), .b(x19), .c(new_n60_), .d(new_n36_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n135_), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n47_), .c(new_n37_), .O(new_n145_));
  nand4  g112(.a(new_n145_), .b(new_n140_), .c(new_n133_), .d(new_n54_), .O(z3));
  nand2  g113(.a(x22), .b(new_n59_), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(new_n61_), .c(x08), .O(new_n148_));
  nor2   g115(.a(new_n148_), .b(new_n50_), .O(new_n149_));
  oai21  g116(.a(new_n82_), .b(x04), .c(x09), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(x16), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(x08), .c(new_n115_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n149_), .c(x17), .O(new_n153_));
  inv1   g120(.a(new_n42_), .O(new_n154_));
  inv1   g121(.a(new_n48_), .O(new_n155_));
  nor3   g122(.a(new_n155_), .b(new_n154_), .c(new_n41_), .O(new_n156_));
  nand3  g123(.a(new_n65_), .b(new_n64_), .c(x04), .O(new_n157_));
  aoi21  g124(.a(new_n157_), .b(new_n59_), .c(x16), .O(new_n158_));
  nor2   g125(.a(new_n158_), .b(x08), .O(new_n159_));
  nor2   g126(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  inv1   g127(.a(new_n102_), .O(new_n161_));
  nand2  g128(.a(x22), .b(new_n62_), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(x09), .c(new_n61_), .O(new_n163_));
  nand3  g130(.a(new_n39_), .b(new_n43_), .c(new_n66_), .O(new_n164_));
  nand3  g131(.a(new_n164_), .b(new_n104_), .c(new_n103_), .O(new_n165_));
  oai22  g132(.a(new_n165_), .b(new_n161_), .c(new_n163_), .d(new_n58_), .O(new_n166_));
  nand3  g133(.a(new_n81_), .b(x16), .c(new_n57_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n44_), .O(new_n168_));
  nand4  g135(.a(new_n168_), .b(new_n39_), .c(new_n43_), .d(new_n66_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n166_), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(new_n160_), .c(x21), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n153_), .O(z4));
  inv1   g139(.a(new_n41_), .O(new_n173_));
  nor2   g140(.a(new_n53_), .b(new_n155_), .O(new_n174_));
  nand4  g141(.a(new_n174_), .b(new_n46_), .c(new_n42_), .d(new_n173_), .O(z5));
  aoi21  g142(.a(x20), .b(new_n60_), .c(x06), .O(new_n176_));
  oai21  g143(.a(new_n176_), .b(x11), .c(new_n34_), .O(new_n177_));
  nand4  g144(.a(new_n45_), .b(new_n39_), .c(x17), .d(new_n43_), .O(new_n178_));
  oai21  g145(.a(new_n156_), .b(new_n53_), .c(new_n178_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n177_), .O(new_n180_));
  aoi21  g147(.a(new_n63_), .b(x14), .c(new_n35_), .O(new_n181_));
  oai21  g148(.a(new_n181_), .b(new_n36_), .c(x03), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(new_n119_), .O(new_n183_));
  nand3  g150(.a(new_n117_), .b(new_n45_), .c(new_n43_), .O(new_n184_));
  nand3  g151(.a(new_n184_), .b(new_n183_), .c(new_n180_), .O(z6));
  nand4  g152(.a(new_n118_), .b(new_n105_), .c(new_n102_), .d(new_n54_), .O(z7));
endmodule


