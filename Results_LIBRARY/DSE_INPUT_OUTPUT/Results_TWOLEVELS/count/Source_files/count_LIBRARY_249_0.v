// Benchmark "FAU" written by ABC on Wed Aug 19 19:40:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  nor2   g000(.a(x07), .b(x05), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g014(.a(x14), .O(new_n66_));
  aoi21  g015(.a(new_n59_), .b(new_n66_), .c(x18), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n65_), .c(new_n52_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n56_), .O(new_n71_));
  oai21  g020(.a(new_n64_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  aoi21  g023(.a(new_n59_), .b(new_n74_), .c(x18), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n73_), .c(new_n52_), .O(z02));
  oai21  g025(.a(x07), .b(x05), .c(x18), .O(new_n77_));
  inv1   g026(.a(x22), .O(new_n78_));
  aoi21  g027(.a(new_n70_), .b(new_n56_), .c(new_n78_), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n56_), .c(new_n62_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n79_), .c(x16), .O(new_n83_));
  nor2   g032(.a(x16), .b(x12), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n83_), .c(new_n77_), .O(z03));
  inv1   g035(.a(x23), .O(new_n87_));
  oai21  g036(.a(x22), .b(x21), .c(new_n54_), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n64_), .c(new_n87_), .O(new_n89_));
  nand3  g038(.a(new_n62_), .b(new_n54_), .c(new_n53_), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n69_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n89_), .c(x16), .O(new_n94_));
  inv1   g043(.a(x11), .O(new_n95_));
  aoi21  g044(.a(new_n59_), .b(new_n95_), .c(x18), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n94_), .c(new_n52_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  nor2   g047(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n80_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n90_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n99_), .c(x16), .O(new_n103_));
  nor2   g052(.a(x16), .b(x10), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n52_), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n103_), .c(new_n77_), .O(z05));
  inv1   g055(.a(x25), .O(new_n107_));
  nand2  g056(.a(new_n101_), .b(new_n54_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n64_), .c(new_n107_), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n91_), .c(new_n70_), .d(new_n56_), .O(new_n111_));
  inv1   g060(.a(new_n111_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n109_), .c(x16), .O(new_n113_));
  inv1   g062(.a(x09), .O(new_n114_));
  aoi21  g063(.a(new_n59_), .b(new_n114_), .c(x18), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n113_), .c(new_n52_), .O(z06));
  inv1   g065(.a(x26), .O(new_n117_));
  nand3  g066(.a(new_n110_), .b(new_n80_), .c(new_n87_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n54_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n64_), .c(new_n117_), .O(new_n120_));
  nor3   g069(.a(x26), .b(x25), .c(x24), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n91_), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n71_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n120_), .c(x16), .O(new_n124_));
  inv1   g073(.a(x08), .O(new_n125_));
  aoi21  g074(.a(new_n59_), .b(new_n125_), .c(x18), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n124_), .c(new_n52_), .O(z07));
  inv1   g076(.a(x27), .O(new_n128_));
  nor3   g077(.a(x27), .b(x26), .c(x25), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n100_), .O(new_n130_));
  oai22  g079(.a(new_n130_), .b(new_n81_), .c(new_n64_), .d(new_n128_), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(x16), .c(x18), .O(new_n132_));
  inv1   g081(.a(x05), .O(new_n133_));
  inv1   g082(.a(x07), .O(new_n134_));
  oai21  g083(.a(x19), .b(new_n134_), .c(new_n133_), .O(new_n135_));
  nor2   g084(.a(x26), .b(x25), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n91_), .c(new_n98_), .d(new_n69_), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n135_), .c(x27), .d(x16), .O(new_n138_));
  nand3  g087(.a(new_n59_), .b(new_n134_), .c(x05), .O(new_n139_));
  and2   g088(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  oai21  g089(.a(new_n132_), .b(new_n52_), .c(new_n140_), .O(z08));
  inv1   g090(.a(x28), .O(new_n142_));
  nor2   g091(.a(x27), .b(x26), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n110_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n92_), .c(new_n54_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n64_), .c(new_n142_), .O(new_n146_));
  nor2   g095(.a(x28), .b(x27), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n110_), .c(new_n117_), .d(new_n87_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n81_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n146_), .c(x16), .O(new_n150_));
  inv1   g099(.a(x06), .O(new_n151_));
  aoi21  g100(.a(new_n59_), .b(new_n151_), .c(x18), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n150_), .c(new_n52_), .O(z09));
  nand2  g102(.a(new_n90_), .b(x29), .O(new_n154_));
  nor3   g103(.a(x29), .b(x28), .c(x27), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n121_), .c(new_n93_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(x16), .c(x18), .O(new_n158_));
  oai21  g107(.a(x19), .b(new_n133_), .c(new_n134_), .O(new_n159_));
  inv1   g108(.a(new_n101_), .O(new_n160_));
  nand2  g109(.a(new_n147_), .b(new_n136_), .O(new_n161_));
  inv1   g110(.a(new_n161_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n159_), .c(x29), .d(x16), .O(new_n164_));
  nand3  g113(.a(new_n59_), .b(x07), .c(new_n133_), .O(new_n165_));
  and2   g114(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  oai21  g115(.a(new_n158_), .b(new_n52_), .c(new_n166_), .O(z10));
  nand2  g116(.a(new_n156_), .b(x30), .O(new_n168_));
  nand3  g117(.a(new_n91_), .b(new_n64_), .c(new_n69_), .O(new_n169_));
  nor2   g118(.a(x30), .b(x29), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n147_), .c(new_n121_), .O(new_n171_));
  or2    g120(.a(new_n171_), .b(new_n169_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  nor2   g123(.a(x16), .b(x04), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n52_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n174_), .c(new_n77_), .O(z11));
  nand2  g126(.a(new_n160_), .b(new_n64_), .O(new_n178_));
  oai21  g127(.a(new_n171_), .b(new_n169_), .c(x31), .O(new_n179_));
  nor2   g128(.a(x29), .b(x28), .O(new_n180_));
  nor2   g129(.a(x31), .b(x30), .O(new_n181_));
  nand3  g130(.a(new_n181_), .b(new_n180_), .c(new_n129_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n178_), .c(new_n179_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(x16), .O(new_n184_));
  nor2   g133(.a(x16), .b(x03), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n52_), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n184_), .c(new_n77_), .O(z12));
  oai21  g136(.a(new_n182_), .b(new_n178_), .c(x32), .O(new_n188_));
  nor2   g137(.a(x32), .b(x31), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n170_), .c(new_n162_), .d(new_n102_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x16), .O(new_n192_));
  nor2   g141(.a(x16), .b(x02), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n52_), .O(new_n194_));
  nand3  g143(.a(new_n194_), .b(new_n192_), .c(new_n77_), .O(z13));
  inv1   g144(.a(x33), .O(new_n196_));
  inv1   g145(.a(x30), .O(new_n197_));
  nand4  g146(.a(new_n189_), .b(new_n180_), .c(new_n197_), .d(new_n128_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n137_), .c(new_n54_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n64_), .c(new_n196_), .O(new_n200_));
  nand2  g149(.a(new_n180_), .b(new_n143_), .O(new_n201_));
  nor2   g150(.a(x33), .b(x32), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n181_), .O(new_n203_));
  nor3   g152(.a(new_n203_), .b(new_n201_), .c(new_n111_), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n200_), .c(x16), .O(new_n205_));
  inv1   g154(.a(x01), .O(new_n206_));
  aoi21  g155(.a(new_n59_), .b(new_n206_), .c(x18), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n205_), .c(new_n52_), .O(z14));
  inv1   g157(.a(x34), .O(new_n209_));
  nand4  g158(.a(new_n202_), .b(new_n181_), .c(new_n180_), .d(new_n128_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n137_), .c(new_n54_), .O(new_n211_));
  aoi21  g160(.a(new_n211_), .b(new_n64_), .c(new_n209_), .O(new_n212_));
  inv1   g161(.a(x32), .O(new_n213_));
  nand4  g162(.a(new_n181_), .b(new_n209_), .c(new_n196_), .d(new_n213_), .O(new_n214_));
  nor3   g163(.a(new_n214_), .b(new_n201_), .c(new_n111_), .O(new_n215_));
  oai21  g164(.a(new_n215_), .b(new_n212_), .c(x16), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  aoi21  g166(.a(new_n59_), .b(new_n217_), .c(x18), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n216_), .c(new_n52_), .O(z15));
endmodule


