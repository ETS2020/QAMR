// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x02), .O(new_n57_));
  inv1   g006(.a(x10), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nor2   g008(.a(new_n58_), .b(new_n57_), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n60_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n59_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n60_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n59_), .O(z02));
  nand2  g027(.a(x22), .b(x21), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n64_), .c(new_n52_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n66_), .b(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  inv1   g035(.a(x16), .O(new_n87_));
  aoi21  g036(.a(new_n87_), .b(new_n86_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n85_), .c(new_n60_), .O(z03));
  nand2  g038(.a(x23), .b(x21), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n71_), .c(new_n64_), .d(new_n52_), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n90_), .c(x19), .O(new_n93_));
  inv1   g042(.a(x23), .O(new_n94_));
  nor2   g043(.a(x22), .b(x20), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n55_), .c(new_n94_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n93_), .c(x16), .O(new_n97_));
  nor2   g046(.a(x16), .b(x11), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n60_), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n97_), .c(new_n59_), .O(z04));
  oai21  g049(.a(new_n87_), .b(x02), .c(x10), .O(new_n101_));
  nand2  g050(.a(new_n95_), .b(new_n55_), .O(new_n102_));
  nor2   g051(.a(x23), .b(x21), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(x19), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n102_), .c(x24), .O(new_n105_));
  nor2   g054(.a(x24), .b(x23), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n80_), .c(new_n66_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n101_), .O(new_n109_));
  inv1   g058(.a(x18), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  aoi22  g060(.a(new_n111_), .b(new_n58_), .c(x18), .d(new_n57_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n109_), .O(z05));
  nor2   g062(.a(x24), .b(x22), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n55_), .c(new_n64_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n104_), .c(x25), .O(new_n116_));
  nor2   g065(.a(x25), .b(x24), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n91_), .c(new_n72_), .d(new_n55_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g069(.a(x09), .O(new_n121_));
  aoi21  g070(.a(new_n87_), .b(new_n121_), .c(x18), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n120_), .c(new_n60_), .O(z06));
  nand3  g072(.a(new_n117_), .b(new_n66_), .c(new_n83_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n104_), .c(x26), .O(new_n125_));
  nor3   g074(.a(x26), .b(x25), .c(x24), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n91_), .c(new_n72_), .d(new_n55_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  inv1   g078(.a(x08), .O(new_n130_));
  aoi21  g079(.a(new_n87_), .b(new_n130_), .c(x18), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n129_), .c(new_n60_), .O(z07));
  nor2   g081(.a(x26), .b(x25), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n114_), .c(new_n66_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n104_), .c(x27), .O(new_n135_));
  inv1   g084(.a(new_n55_), .O(new_n136_));
  nand2  g085(.a(new_n80_), .b(new_n64_), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nor3   g087(.a(x27), .b(x26), .c(x25), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n138_), .c(new_n106_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n135_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(x16), .O(new_n142_));
  inv1   g091(.a(x07), .O(new_n143_));
  aoi21  g092(.a(new_n87_), .b(new_n143_), .c(x18), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n142_), .c(new_n60_), .O(z08));
  nor2   g094(.a(x27), .b(x26), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n117_), .c(new_n95_), .d(new_n55_), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n104_), .c(x28), .O(new_n148_));
  nor3   g097(.a(x28), .b(x27), .c(x26), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n138_), .c(new_n117_), .d(new_n94_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  inv1   g101(.a(x06), .O(new_n153_));
  aoi21  g102(.a(new_n87_), .b(new_n153_), .c(x18), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n152_), .c(new_n60_), .O(z09));
  inv1   g104(.a(x29), .O(new_n156_));
  inv1   g105(.a(new_n104_), .O(new_n157_));
  inv1   g106(.a(x26), .O(new_n158_));
  nor2   g107(.a(x28), .b(x27), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n117_), .c(new_n158_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n102_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n157_), .c(new_n156_), .O(new_n162_));
  nand3  g111(.a(new_n91_), .b(new_n66_), .c(new_n71_), .O(new_n163_));
  inv1   g112(.a(x27), .O(new_n164_));
  nor2   g113(.a(x29), .b(x28), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n126_), .c(new_n164_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n163_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n162_), .c(x16), .O(new_n168_));
  inv1   g117(.a(x05), .O(new_n169_));
  aoi21  g118(.a(new_n87_), .b(new_n169_), .c(x18), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n168_), .c(new_n60_), .O(z10));
  inv1   g120(.a(x30), .O(new_n172_));
  nand3  g121(.a(new_n165_), .b(new_n133_), .c(new_n164_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n115_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n157_), .c(new_n172_), .O(new_n175_));
  nor2   g124(.a(x30), .b(x29), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n159_), .c(new_n126_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n163_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n175_), .c(x16), .O(new_n179_));
  nor2   g128(.a(x16), .b(x04), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n60_), .O(new_n181_));
  nand3  g130(.a(new_n181_), .b(new_n179_), .c(new_n59_), .O(z11));
  inv1   g131(.a(x31), .O(new_n183_));
  nand3  g132(.a(new_n172_), .b(new_n94_), .c(new_n71_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n53_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n174_), .c(new_n183_), .O(new_n186_));
  nor2   g135(.a(x31), .b(x30), .O(new_n187_));
  nand3  g136(.a(new_n187_), .b(new_n165_), .c(new_n139_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n107_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n186_), .c(x16), .O(new_n190_));
  nor2   g139(.a(x16), .b(x03), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n60_), .O(new_n192_));
  nand3  g141(.a(new_n192_), .b(new_n190_), .c(new_n59_), .O(z12));
  aoi21  g142(.a(x16), .b(new_n58_), .c(new_n57_), .O(new_n194_));
  nand2  g143(.a(new_n187_), .b(new_n103_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n53_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n174_), .O(new_n197_));
  nor2   g146(.a(x32), .b(x31), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n176_), .c(new_n159_), .d(new_n133_), .O(new_n199_));
  nor2   g148(.a(new_n199_), .b(new_n107_), .O(new_n200_));
  aoi21  g149(.a(new_n197_), .b(x32), .c(new_n200_), .O(new_n201_));
  aoi22  g150(.a(new_n111_), .b(new_n57_), .c(x18), .d(new_n58_), .O(new_n202_));
  oai21  g151(.a(new_n201_), .b(new_n194_), .c(new_n202_), .O(z13));
  inv1   g152(.a(x33), .O(new_n204_));
  inv1   g153(.a(x25), .O(new_n205_));
  inv1   g154(.a(x28), .O(new_n206_));
  nor2   g155(.a(x32), .b(x29), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n146_), .c(new_n206_), .d(new_n205_), .O(new_n208_));
  nor2   g157(.a(new_n208_), .b(new_n115_), .O(new_n209_));
  aoi21  g158(.a(new_n209_), .b(new_n196_), .c(new_n204_), .O(new_n210_));
  nand2  g159(.a(new_n165_), .b(new_n146_), .O(new_n211_));
  inv1   g160(.a(x32), .O(new_n212_));
  nand3  g161(.a(new_n187_), .b(new_n204_), .c(new_n212_), .O(new_n213_));
  nor3   g162(.a(new_n213_), .b(new_n211_), .c(new_n118_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n210_), .c(x16), .O(new_n215_));
  nor2   g164(.a(x16), .b(x01), .O(new_n216_));
  nor2   g165(.a(new_n216_), .b(new_n60_), .O(new_n217_));
  nand3  g166(.a(new_n217_), .b(new_n215_), .c(new_n59_), .O(z14));
  inv1   g167(.a(x34), .O(new_n219_));
  inv1   g168(.a(new_n103_), .O(new_n220_));
  nand3  g169(.a(new_n204_), .b(new_n183_), .c(new_n172_), .O(new_n221_));
  oai21  g170(.a(new_n221_), .b(new_n220_), .c(new_n53_), .O(new_n222_));
  aoi21  g171(.a(new_n222_), .b(new_n209_), .c(new_n219_), .O(new_n223_));
  nand4  g172(.a(new_n187_), .b(new_n219_), .c(new_n204_), .d(new_n212_), .O(new_n224_));
  nor3   g173(.a(new_n224_), .b(new_n211_), .c(new_n118_), .O(new_n225_));
  oai21  g174(.a(new_n225_), .b(new_n223_), .c(x16), .O(new_n226_));
  inv1   g175(.a(x00), .O(new_n227_));
  aoi21  g176(.a(new_n87_), .b(new_n227_), .c(x18), .O(new_n228_));
  aoi21  g177(.a(new_n228_), .b(new_n226_), .c(new_n60_), .O(z15));
endmodule


