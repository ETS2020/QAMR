// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  nor2   g000(.a(x15), .b(x00), .O(new_n52_));
  xnor2a g001(.a(x19), .b(x17), .O(new_n53_));
  aoi21  g002(.a(new_n53_), .b(x16), .c(x18), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand3  g005(.a(new_n56_), .b(new_n55_), .c(x00), .O(new_n57_));
  oai21  g006(.a(new_n54_), .b(new_n52_), .c(new_n57_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(x19), .b(x17), .O(new_n60_));
  nor2   g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g012(.a(x14), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(new_n64_), .c(x18), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n52_), .O(z01));
  oai21  g015(.a(x15), .b(x00), .c(x18), .O(new_n67_));
  inv1   g016(.a(x21), .O(new_n68_));
  nand3  g017(.a(new_n60_), .b(new_n68_), .c(new_n59_), .O(new_n69_));
  oai21  g018(.a(new_n62_), .b(new_n68_), .c(new_n69_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  nor2   g020(.a(x16), .b(x13), .O(new_n72_));
  nor2   g021(.a(new_n72_), .b(new_n52_), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n71_), .c(new_n67_), .O(z02));
  nand2  g023(.a(x22), .b(x21), .O(new_n75_));
  inv1   g024(.a(x17), .O(new_n76_));
  nor2   g025(.a(x22), .b(x21), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n59_), .c(new_n76_), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n75_), .c(x19), .O(new_n79_));
  inv1   g028(.a(x22), .O(new_n80_));
  nor2   g029(.a(new_n62_), .b(new_n80_), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n79_), .c(x16), .O(new_n82_));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(new_n83_), .c(x18), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(new_n52_), .O(z03));
  inv1   g034(.a(x23), .O(new_n86_));
  inv1   g035(.a(x19), .O(new_n87_));
  oai21  g036(.a(x22), .b(x21), .c(new_n87_), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n62_), .c(new_n86_), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n62_), .c(new_n68_), .O(new_n91_));
  inv1   g040(.a(new_n91_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n89_), .c(x16), .O(new_n93_));
  inv1   g042(.a(x11), .O(new_n94_));
  aoi21  g043(.a(new_n56_), .b(new_n94_), .c(x18), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n52_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n77_), .c(new_n62_), .O(new_n99_));
  oai21  g048(.a(new_n92_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(x16), .O(new_n101_));
  nor2   g050(.a(x16), .b(x10), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n52_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n101_), .c(new_n67_), .O(z05));
  inv1   g053(.a(x25), .O(new_n105_));
  nand2  g054(.a(new_n98_), .b(new_n77_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n87_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n62_), .c(new_n105_), .O(new_n108_));
  nor2   g057(.a(x25), .b(x24), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n90_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n69_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n108_), .c(x16), .O(new_n112_));
  inv1   g061(.a(x09), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(new_n113_), .c(x18), .O(new_n114_));
  aoi21  g063(.a(new_n114_), .b(new_n112_), .c(new_n52_), .O(z06));
  inv1   g064(.a(x26), .O(new_n116_));
  nor3   g065(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n90_), .O(new_n118_));
  oai22  g067(.a(new_n118_), .b(new_n69_), .c(new_n111_), .d(new_n116_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  nor2   g069(.a(x16), .b(x08), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n120_), .c(new_n67_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  nand2  g073(.a(new_n90_), .b(new_n68_), .O(new_n125_));
  nor2   g074(.a(x26), .b(x25), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n97_), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n125_), .c(new_n87_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n62_), .c(new_n124_), .O(new_n129_));
  nand3  g078(.a(new_n77_), .b(new_n60_), .c(new_n59_), .O(new_n130_));
  nor3   g079(.a(x27), .b(x26), .c(x25), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n98_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n129_), .c(x16), .O(new_n134_));
  inv1   g083(.a(x07), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(new_n135_), .c(x18), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n134_), .c(new_n52_), .O(z08));
  inv1   g086(.a(x28), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n109_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n125_), .c(new_n87_), .O(new_n141_));
  aoi21  g090(.a(new_n141_), .b(new_n62_), .c(new_n138_), .O(new_n142_));
  nand2  g091(.a(new_n109_), .b(new_n86_), .O(new_n143_));
  nor2   g092(.a(x28), .b(x27), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n116_), .O(new_n145_));
  nor3   g094(.a(new_n145_), .b(new_n143_), .c(new_n130_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n142_), .c(x16), .O(new_n147_));
  inv1   g096(.a(x06), .O(new_n148_));
  aoi21  g097(.a(new_n56_), .b(new_n148_), .c(x18), .O(new_n149_));
  aoi21  g098(.a(new_n149_), .b(new_n147_), .c(new_n52_), .O(z09));
  inv1   g099(.a(x29), .O(new_n151_));
  nand3  g100(.a(new_n59_), .b(new_n87_), .c(new_n76_), .O(new_n152_));
  nand4  g101(.a(new_n144_), .b(new_n126_), .c(new_n98_), .d(new_n77_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n87_), .c(new_n152_), .O(new_n154_));
  nor2   g103(.a(x29), .b(x28), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n117_), .c(new_n124_), .O(new_n156_));
  oai22  g105(.a(new_n156_), .b(new_n91_), .c(new_n154_), .d(new_n151_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(x16), .O(new_n158_));
  inv1   g107(.a(x05), .O(new_n159_));
  aoi21  g108(.a(new_n56_), .b(new_n159_), .c(x18), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n158_), .c(new_n52_), .O(z10));
  inv1   g110(.a(x30), .O(new_n162_));
  nand3  g111(.a(new_n155_), .b(new_n126_), .c(new_n124_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n106_), .c(new_n87_), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n62_), .c(new_n162_), .O(new_n165_));
  nor2   g114(.a(x30), .b(x29), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n144_), .c(new_n117_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n91_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n165_), .c(x16), .O(new_n169_));
  inv1   g118(.a(x04), .O(new_n170_));
  aoi21  g119(.a(new_n56_), .b(new_n170_), .c(x18), .O(new_n171_));
  aoi21  g120(.a(new_n171_), .b(new_n169_), .c(new_n52_), .O(z11));
  inv1   g121(.a(x31), .O(new_n173_));
  nor2   g122(.a(x31), .b(x30), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n155_), .c(new_n131_), .O(new_n175_));
  oai22  g124(.a(new_n175_), .b(new_n99_), .c(new_n168_), .d(new_n173_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  nor2   g126(.a(x16), .b(x03), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n52_), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n177_), .c(new_n67_), .O(z12));
  inv1   g129(.a(x32), .O(new_n181_));
  nand3  g130(.a(new_n109_), .b(new_n77_), .c(new_n86_), .O(new_n182_));
  nand4  g131(.a(new_n174_), .b(new_n144_), .c(new_n151_), .d(new_n116_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n182_), .c(new_n87_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n62_), .c(new_n181_), .O(new_n185_));
  nor2   g134(.a(x32), .b(x31), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n166_), .c(new_n144_), .d(new_n126_), .O(new_n187_));
  nor2   g136(.a(new_n187_), .b(new_n99_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n185_), .c(x16), .O(new_n189_));
  inv1   g138(.a(x02), .O(new_n190_));
  aoi21  g139(.a(new_n56_), .b(new_n190_), .c(x18), .O(new_n191_));
  aoi21  g140(.a(new_n191_), .b(new_n189_), .c(new_n52_), .O(z13));
  inv1   g141(.a(x33), .O(new_n193_));
  nand2  g142(.a(new_n155_), .b(new_n139_), .O(new_n194_));
  nand3  g143(.a(new_n174_), .b(new_n193_), .c(new_n181_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n194_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n111_), .O(new_n197_));
  oai21  g146(.a(new_n188_), .b(new_n193_), .c(new_n197_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x16), .O(new_n199_));
  nor2   g148(.a(x16), .b(x01), .O(new_n200_));
  nor2   g149(.a(new_n200_), .b(new_n52_), .O(new_n201_));
  nand3  g150(.a(new_n201_), .b(new_n199_), .c(new_n67_), .O(z14));
  inv1   g151(.a(x00), .O(new_n203_));
  oai21  g152(.a(new_n56_), .b(new_n55_), .c(new_n203_), .O(new_n204_));
  nand2  g153(.a(new_n197_), .b(x34), .O(new_n205_));
  inv1   g154(.a(new_n194_), .O(new_n206_));
  nor3   g155(.a(x34), .b(x33), .c(x32), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n206_), .c(new_n174_), .d(new_n111_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n205_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(x16), .O(new_n210_));
  nand3  g159(.a(new_n210_), .b(new_n204_), .c(new_n67_), .O(z15));
endmodule


