// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n212_, new_n213_, new_n214_,
    new_n215_, new_n216_, new_n217_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n235_, new_n236_, new_n237_, new_n238_, new_n239_, new_n240_,
    new_n241_, new_n242_, new_n243_, new_n244_, new_n245_, new_n246_,
    new_n247_, new_n248_, new_n249_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  nand2  g005(.a(x30), .b(x20), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x18), .O(new_n58_));
  inv1   g007(.a(new_n57_), .O(new_n59_));
  nor2   g008(.a(x16), .b(x15), .O(new_n60_));
  nor2   g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n58_), .c(new_n56_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(x20), .b(x19), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  oai21  g014(.a(new_n55_), .b(new_n63_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n59_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n58_), .O(z01));
  inv1   g019(.a(x16), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  inv1   g021(.a(x30), .O(new_n73_));
  oai21  g022(.a(new_n72_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x20), .O(new_n75_));
  nor2   g024(.a(new_n55_), .b(new_n72_), .O(new_n76_));
  nand3  g025(.a(new_n55_), .b(new_n72_), .c(new_n63_), .O(new_n77_));
  inv1   g026(.a(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  inv1   g028(.a(x13), .O(new_n80_));
  nand2  g029(.a(new_n71_), .b(new_n80_), .O(new_n81_));
  nand4  g030(.a(new_n81_), .b(new_n79_), .c(new_n75_), .d(new_n58_), .O(z02));
  nor2   g031(.a(x16), .b(x12), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(x18), .c(new_n57_), .O(new_n84_));
  inv1   g033(.a(x22), .O(new_n85_));
  nor3   g034(.a(x21), .b(x19), .c(x17), .O(new_n86_));
  nor2   g035(.a(x22), .b(x21), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n55_), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n85_), .c(new_n88_), .O(new_n89_));
  nand3  g038(.a(new_n73_), .b(x22), .c(x20), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n89_), .b(new_n63_), .c(new_n91_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n71_), .c(new_n84_), .O(z03));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(x18), .c(new_n57_), .O(new_n95_));
  nand2  g044(.a(new_n88_), .b(x23), .O(new_n96_));
  nor2   g045(.a(x23), .b(x22), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n55_), .c(new_n72_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n96_), .c(x20), .O(new_n99_));
  nand3  g048(.a(new_n73_), .b(x23), .c(x20), .O(new_n100_));
  inv1   g049(.a(new_n100_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n99_), .c(x16), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n95_), .O(z04));
  nor2   g052(.a(x16), .b(x10), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(x18), .c(new_n57_), .O(new_n105_));
  nand2  g054(.a(new_n98_), .b(x24), .O(new_n106_));
  nand3  g055(.a(new_n72_), .b(new_n53_), .c(new_n52_), .O(new_n107_));
  inv1   g056(.a(x23), .O(new_n108_));
  inv1   g057(.a(x24), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n85_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  inv1   g060(.a(new_n111_), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(new_n106_), .c(x20), .O(new_n113_));
  nand3  g062(.a(new_n73_), .b(x24), .c(x20), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n105_), .O(z05));
  nor2   g066(.a(x16), .b(x09), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(x18), .c(new_n57_), .O(new_n119_));
  oai21  g068(.a(new_n110_), .b(new_n107_), .c(x25), .O(new_n120_));
  nor2   g069(.a(x25), .b(x24), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n97_), .c(new_n86_), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n120_), .c(x20), .O(new_n123_));
  nand3  g072(.a(new_n73_), .b(x25), .c(x20), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n123_), .c(x16), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n119_), .O(z06));
  inv1   g076(.a(x26), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n71_), .c(new_n73_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x20), .O(new_n130_));
  nand2  g079(.a(new_n122_), .b(x26), .O(new_n131_));
  nor3   g080(.a(x26), .b(x25), .c(x24), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n97_), .c(new_n78_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g084(.a(x08), .O(new_n136_));
  nand2  g085(.a(new_n71_), .b(new_n136_), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n135_), .c(new_n130_), .d(new_n58_), .O(z07));
  nor2   g087(.a(x16), .b(x07), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(x18), .c(new_n57_), .O(new_n140_));
  nor2   g089(.a(x24), .b(x23), .O(new_n141_));
  nor2   g090(.a(x26), .b(x25), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n88_), .c(x27), .O(new_n144_));
  nor3   g093(.a(x27), .b(x26), .c(x25), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n141_), .c(new_n87_), .d(new_n55_), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n144_), .c(x20), .O(new_n147_));
  nand3  g096(.a(new_n73_), .b(x27), .c(x20), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n147_), .c(x16), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n140_), .O(z08));
  nand2  g100(.a(x28), .b(x16), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n73_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x20), .O(new_n154_));
  nand2  g103(.a(new_n146_), .b(x28), .O(new_n155_));
  nor3   g104(.a(x22), .b(x21), .c(x20), .O(new_n156_));
  nor3   g105(.a(x25), .b(x24), .c(x23), .O(new_n157_));
  nor3   g106(.a(x28), .b(x27), .c(x26), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n157_), .c(new_n156_), .d(new_n55_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(x16), .O(new_n161_));
  inv1   g110(.a(x06), .O(new_n162_));
  nand2  g111(.a(new_n71_), .b(new_n162_), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n161_), .c(new_n154_), .d(new_n58_), .O(z09));
  nor2   g113(.a(x16), .b(x05), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(x18), .c(new_n57_), .O(new_n166_));
  nor2   g115(.a(x28), .b(x27), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n121_), .c(new_n128_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n98_), .c(x29), .O(new_n169_));
  nor3   g118(.a(x23), .b(x22), .c(x21), .O(new_n170_));
  nor3   g119(.a(x29), .b(x28), .c(x27), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n132_), .c(new_n170_), .d(new_n55_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n169_), .c(x20), .O(new_n173_));
  nand3  g122(.a(new_n73_), .b(x29), .c(x20), .O(new_n174_));
  inv1   g123(.a(new_n174_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n173_), .c(x16), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n166_), .O(z10));
  nand2  g126(.a(new_n172_), .b(x30), .O(new_n178_));
  nand4  g127(.a(new_n97_), .b(new_n64_), .c(new_n72_), .d(new_n52_), .O(new_n179_));
  nor2   g128(.a(x30), .b(x29), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n167_), .c(new_n132_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n179_), .c(new_n178_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x04), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n59_), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n183_), .c(new_n58_), .O(z11));
  nor2   g135(.a(x16), .b(x03), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(x18), .c(new_n57_), .O(new_n188_));
  nand2  g137(.a(x31), .b(x20), .O(new_n189_));
  nor2   g138(.a(x31), .b(x29), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n167_), .c(new_n142_), .d(new_n109_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n179_), .c(new_n189_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n73_), .O(new_n193_));
  nor3   g142(.a(x30), .b(x29), .c(x28), .O(new_n194_));
  nand3  g143(.a(new_n194_), .b(new_n145_), .c(new_n111_), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(x31), .c(new_n63_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n193_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(x16), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n188_), .O(z12));
  nor2   g148(.a(x16), .b(x02), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(x18), .c(new_n57_), .O(new_n201_));
  nand2  g150(.a(x32), .b(x20), .O(new_n202_));
  nand4  g151(.a(new_n141_), .b(new_n87_), .c(new_n64_), .d(new_n52_), .O(new_n203_));
  inv1   g152(.a(x25), .O(new_n204_));
  nor2   g153(.a(x27), .b(x26), .O(new_n205_));
  nor2   g154(.a(x29), .b(x28), .O(new_n206_));
  nor2   g155(.a(x32), .b(x31), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n206_), .c(new_n205_), .d(new_n204_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n203_), .c(new_n202_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n73_), .O(new_n210_));
  nor2   g159(.a(x31), .b(x30), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n206_), .O(new_n212_));
  inv1   g161(.a(new_n212_), .O(new_n213_));
  nand3  g162(.a(new_n213_), .b(new_n145_), .c(new_n111_), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(x32), .c(new_n63_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n210_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(x16), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n201_), .O(z13));
  inv1   g167(.a(x33), .O(new_n219_));
  oai21  g168(.a(new_n219_), .b(new_n71_), .c(new_n73_), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(x20), .O(new_n221_));
  nand3  g170(.a(new_n207_), .b(new_n180_), .c(new_n158_), .O(new_n222_));
  oai21  g171(.a(new_n222_), .b(new_n122_), .c(x33), .O(new_n223_));
  nand2  g172(.a(new_n121_), .b(new_n97_), .O(new_n224_));
  nor2   g173(.a(new_n224_), .b(new_n77_), .O(new_n225_));
  nor2   g174(.a(x33), .b(x32), .O(new_n226_));
  nand4  g175(.a(new_n226_), .b(new_n211_), .c(new_n206_), .d(new_n205_), .O(new_n227_));
  inv1   g176(.a(new_n227_), .O(new_n228_));
  nand2  g177(.a(new_n228_), .b(new_n225_), .O(new_n229_));
  nand2  g178(.a(new_n229_), .b(new_n223_), .O(new_n230_));
  nand2  g179(.a(new_n230_), .b(x16), .O(new_n231_));
  inv1   g180(.a(x01), .O(new_n232_));
  nand2  g181(.a(new_n71_), .b(new_n232_), .O(new_n233_));
  nand4  g182(.a(new_n233_), .b(new_n231_), .c(new_n221_), .d(new_n58_), .O(z14));
  inv1   g183(.a(x34), .O(new_n235_));
  oai21  g184(.a(new_n235_), .b(new_n71_), .c(new_n73_), .O(new_n236_));
  nand2  g185(.a(new_n236_), .b(x20), .O(new_n237_));
  oai21  g186(.a(new_n227_), .b(new_n122_), .c(x34), .O(new_n238_));
  nand2  g187(.a(new_n206_), .b(new_n205_), .O(new_n239_));
  inv1   g188(.a(new_n239_), .O(new_n240_));
  inv1   g189(.a(new_n211_), .O(new_n241_));
  inv1   g190(.a(x32), .O(new_n242_));
  nand3  g191(.a(new_n235_), .b(new_n219_), .c(new_n242_), .O(new_n243_));
  nor2   g192(.a(new_n243_), .b(new_n241_), .O(new_n244_));
  nand3  g193(.a(new_n244_), .b(new_n240_), .c(new_n225_), .O(new_n245_));
  nand2  g194(.a(new_n245_), .b(new_n238_), .O(new_n246_));
  nand2  g195(.a(new_n246_), .b(x16), .O(new_n247_));
  inv1   g196(.a(x00), .O(new_n248_));
  nand2  g197(.a(new_n71_), .b(new_n248_), .O(new_n249_));
  nand4  g198(.a(new_n249_), .b(new_n247_), .c(new_n237_), .d(new_n58_), .O(z15));
endmodule


