// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x03), .O(new_n52_));
  inv1   g001(.a(x18), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  inv1   g004(.a(new_n55_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nor2   g006(.a(x16), .b(x15), .O(new_n58_));
  aoi21  g007(.a(new_n57_), .b(x16), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n59_), .b(new_n52_), .c(new_n53_), .O(z00));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n56_), .b(x20), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x03), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n53_), .O(z01));
  inv1   g017(.a(x17), .O(new_n69_));
  inv1   g018(.a(x19), .O(new_n70_));
  nand2  g019(.a(x20), .b(x03), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n55_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor3   g026(.a(new_n77_), .b(x18), .c(new_n52_), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n61_), .c(new_n78_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  oai21  g029(.a(x21), .b(x20), .c(x03), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n55_), .c(new_n80_), .O(new_n82_));
  nor3   g031(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n55_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n82_), .c(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n61_), .b(new_n87_), .c(x18), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n86_), .c(x03), .O(z03));
  oai21  g038(.a(new_n83_), .b(new_n52_), .c(new_n55_), .O(new_n90_));
  nor3   g039(.a(x23), .b(x22), .c(x21), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n63_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  aoi21  g042(.a(new_n90_), .b(x23), .c(new_n93_), .O(new_n94_));
  nor2   g043(.a(x16), .b(x11), .O(new_n95_));
  nor3   g044(.a(new_n95_), .b(x18), .c(new_n52_), .O(new_n96_));
  oai21  g045(.a(new_n94_), .b(new_n61_), .c(new_n96_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  inv1   g047(.a(new_n91_), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n70_), .c(new_n64_), .O(new_n100_));
  nor2   g049(.a(x22), .b(x21), .O(new_n101_));
  nor2   g050(.a(x24), .b(x23), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n101_), .c(new_n63_), .O(new_n103_));
  oai21  g052(.a(new_n100_), .b(new_n98_), .c(new_n103_), .O(new_n104_));
  nor2   g053(.a(x16), .b(x10), .O(new_n105_));
  aoi21  g054(.a(new_n104_), .b(x16), .c(new_n105_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n52_), .c(new_n53_), .O(z05));
  aoi21  g056(.a(new_n102_), .b(new_n101_), .c(x19), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n64_), .c(x25), .O(new_n109_));
  nor2   g058(.a(x23), .b(x22), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n110_), .c(new_n73_), .d(new_n55_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  nor2   g062(.a(x16), .b(x09), .O(new_n114_));
  aoi21  g063(.a(new_n113_), .b(x16), .c(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n52_), .c(new_n53_), .O(z06));
  inv1   g065(.a(x26), .O(new_n117_));
  nor3   g066(.a(x25), .b(x24), .c(x23), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n101_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n70_), .c(new_n64_), .O(new_n120_));
  nor3   g069(.a(x26), .b(x25), .c(x24), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n110_), .c(new_n75_), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n117_), .c(new_n122_), .O(new_n123_));
  nor2   g072(.a(x16), .b(x08), .O(new_n124_));
  aoi21  g073(.a(new_n123_), .b(x16), .c(new_n124_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n52_), .c(new_n53_), .O(z07));
  inv1   g075(.a(x27), .O(new_n127_));
  inv1   g076(.a(x21), .O(new_n128_));
  nor2   g077(.a(x26), .b(x25), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n110_), .c(new_n98_), .d(new_n128_), .O(new_n130_));
  aoi21  g079(.a(new_n130_), .b(new_n70_), .c(new_n64_), .O(new_n131_));
  inv1   g080(.a(x25), .O(new_n132_));
  nor2   g081(.a(x27), .b(x26), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n102_), .c(new_n132_), .O(new_n134_));
  oai22  g083(.a(new_n134_), .b(new_n84_), .c(new_n131_), .d(new_n127_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x07), .O(new_n136_));
  aoi21  g085(.a(new_n135_), .b(x16), .c(new_n136_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n52_), .c(new_n53_), .O(z08));
  inv1   g087(.a(x28), .O(new_n139_));
  nand4  g088(.a(new_n133_), .b(new_n111_), .c(new_n110_), .d(new_n73_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x03), .O(new_n141_));
  aoi21  g090(.a(new_n141_), .b(new_n55_), .c(new_n139_), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n118_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n84_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n142_), .c(x16), .O(new_n146_));
  inv1   g095(.a(x06), .O(new_n147_));
  aoi21  g096(.a(new_n61_), .b(new_n147_), .c(x18), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(x03), .O(z09));
  inv1   g098(.a(x29), .O(new_n150_));
  nor2   g099(.a(x28), .b(x27), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n129_), .c(new_n102_), .d(new_n101_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n70_), .c(new_n64_), .O(new_n153_));
  nor3   g102(.a(x29), .b(x28), .c(x27), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n121_), .O(new_n155_));
  oai22  g104(.a(new_n155_), .b(new_n92_), .c(new_n153_), .d(new_n150_), .O(new_n156_));
  nor2   g105(.a(x16), .b(x05), .O(new_n157_));
  aoi21  g106(.a(new_n156_), .b(x16), .c(new_n157_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n52_), .c(new_n53_), .O(z10));
  inv1   g108(.a(x30), .O(new_n160_));
  nand3  g109(.a(new_n102_), .b(new_n73_), .c(new_n80_), .O(new_n161_));
  nand2  g110(.a(new_n154_), .b(new_n129_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n161_), .c(x03), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n55_), .c(new_n160_), .O(new_n164_));
  nor2   g113(.a(x30), .b(x29), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n151_), .c(new_n121_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n92_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n164_), .c(x16), .O(new_n168_));
  inv1   g117(.a(x04), .O(new_n169_));
  aoi21  g118(.a(new_n61_), .b(new_n169_), .c(x18), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n168_), .c(x03), .O(z11));
  inv1   g120(.a(x31), .O(new_n172_));
  nand3  g121(.a(new_n165_), .b(new_n133_), .c(new_n139_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n119_), .c(new_n70_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n63_), .c(new_n172_), .O(new_n175_));
  nor2   g124(.a(x29), .b(x28), .O(new_n176_));
  nor2   g125(.a(x31), .b(x30), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n176_), .c(new_n133_), .d(new_n132_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n103_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n175_), .c(x16), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n52_), .c(new_n53_), .O(z12));
  inv1   g130(.a(x32), .O(new_n182_));
  nor3   g131(.a(x31), .b(x30), .c(x29), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n143_), .c(new_n118_), .d(new_n101_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n70_), .c(new_n64_), .O(new_n185_));
  nor2   g134(.a(x32), .b(x31), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n165_), .c(new_n151_), .d(new_n129_), .O(new_n187_));
  oai22  g136(.a(new_n187_), .b(new_n103_), .c(new_n185_), .d(new_n182_), .O(new_n188_));
  nor2   g137(.a(x16), .b(x02), .O(new_n189_));
  aoi21  g138(.a(new_n188_), .b(x16), .c(new_n189_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n52_), .c(new_n53_), .O(z13));
  inv1   g140(.a(x33), .O(new_n192_));
  nor3   g141(.a(x32), .b(x31), .c(x30), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n154_), .c(new_n121_), .d(new_n91_), .O(new_n194_));
  aoi21  g143(.a(new_n194_), .b(new_n70_), .c(new_n64_), .O(new_n195_));
  nor2   g144(.a(x33), .b(x32), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n177_), .c(new_n176_), .d(new_n133_), .O(new_n197_));
  oai22  g146(.a(new_n197_), .b(new_n112_), .c(new_n195_), .d(new_n192_), .O(new_n198_));
  nor2   g147(.a(x16), .b(x01), .O(new_n199_));
  aoi21  g148(.a(new_n198_), .b(x16), .c(new_n199_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n52_), .c(new_n53_), .O(z14));
  inv1   g150(.a(x34), .O(new_n202_));
  nand4  g151(.a(new_n196_), .b(new_n177_), .c(new_n176_), .d(new_n127_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n130_), .c(new_n70_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(new_n63_), .c(new_n202_), .O(new_n205_));
  nand2  g154(.a(new_n176_), .b(new_n133_), .O(new_n206_));
  nand4  g155(.a(new_n177_), .b(new_n202_), .c(new_n192_), .d(new_n182_), .O(new_n207_));
  nor3   g156(.a(new_n207_), .b(new_n206_), .c(new_n112_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n205_), .c(x16), .O(new_n209_));
  inv1   g158(.a(x00), .O(new_n210_));
  nand2  g159(.a(new_n61_), .b(new_n210_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n209_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(x03), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(new_n53_), .O(z15));
endmodule


