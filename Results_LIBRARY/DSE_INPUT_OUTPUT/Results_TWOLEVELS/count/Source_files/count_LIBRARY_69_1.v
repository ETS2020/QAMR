// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_;
  inv1   g000(.a(x15), .O(new_n52_));
  inv1   g001(.a(x16), .O(new_n53_));
  oai21  g002(.a(x18), .b(new_n53_), .c(new_n52_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x16), .O(new_n59_));
  inv1   g008(.a(x05), .O(new_n60_));
  nand2  g009(.a(x15), .b(new_n60_), .O(new_n61_));
  nand2  g010(.a(x18), .b(x05), .O(new_n62_));
  nand4  g011(.a(new_n62_), .b(new_n61_), .c(new_n59_), .d(new_n54_), .O(z00));
  inv1   g012(.a(new_n61_), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(new_n58_), .b(new_n65_), .O(new_n66_));
  nor3   g015(.a(x20), .b(x19), .c(x17), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  inv1   g017(.a(x14), .O(new_n69_));
  aoi21  g018(.a(new_n53_), .b(new_n69_), .c(x18), .O(new_n70_));
  aoi21  g019(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(z01));
  nand2  g020(.a(new_n61_), .b(x18), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  nand4  g022(.a(new_n73_), .b(new_n65_), .c(new_n56_), .d(new_n55_), .O(new_n74_));
  oai21  g023(.a(new_n67_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n64_), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n76_), .c(new_n72_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(new_n74_), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n58_), .c(new_n65_), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n80_), .c(new_n83_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(x16), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  aoi21  g035(.a(new_n53_), .b(new_n86_), .c(x18), .O(new_n87_));
  aoi21  g036(.a(new_n87_), .b(new_n85_), .c(new_n64_), .O(z03));
  nand2  g037(.a(new_n83_), .b(x23), .O(new_n89_));
  nor2   g038(.a(x20), .b(x19), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n90_), .c(new_n73_), .d(new_n55_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(x16), .O(new_n94_));
  nor2   g043(.a(x16), .b(x11), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n64_), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n94_), .c(new_n72_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  nand3  g047(.a(new_n65_), .b(new_n56_), .c(new_n55_), .O(new_n99_));
  inv1   g048(.a(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n80_), .c(new_n73_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n82_), .c(new_n90_), .d(new_n55_), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n98_), .c(new_n104_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  nor2   g055(.a(x16), .b(x10), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n64_), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n106_), .c(new_n72_), .O(z05));
  nand2  g058(.a(new_n104_), .b(x25), .O(new_n110_));
  nor2   g059(.a(x21), .b(x20), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n91_), .c(new_n111_), .d(new_n58_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g064(.a(x09), .O(new_n116_));
  aoi21  g065(.a(new_n53_), .b(new_n116_), .c(x18), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n115_), .c(new_n64_), .O(z06));
  inv1   g067(.a(x26), .O(new_n119_));
  inv1   g068(.a(x25), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n98_), .c(new_n100_), .d(new_n80_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n74_), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n91_), .c(new_n81_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x16), .O(new_n126_));
  nor2   g075(.a(x16), .b(x08), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n64_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n126_), .c(new_n72_), .O(z07));
  nand2  g078(.a(new_n124_), .b(x27), .O(new_n130_));
  nor2   g079(.a(x27), .b(x26), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n103_), .c(new_n120_), .O(new_n132_));
  or2    g081(.a(new_n132_), .b(new_n83_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  nor2   g084(.a(x16), .b(x07), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n64_), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n135_), .c(new_n72_), .O(z08));
  oai21  g087(.a(new_n132_), .b(new_n83_), .c(x28), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n112_), .c(new_n119_), .d(new_n100_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n83_), .c(new_n139_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x16), .O(new_n143_));
  inv1   g092(.a(x06), .O(new_n144_));
  aoi21  g093(.a(new_n53_), .b(new_n144_), .c(x18), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n143_), .c(new_n64_), .O(z09));
  nor2   g095(.a(x28), .b(x25), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n58_), .c(new_n80_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x29), .O(new_n149_));
  nor3   g098(.a(x23), .b(x22), .c(x21), .O(new_n150_));
  nor3   g099(.a(x29), .b(x28), .c(x27), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n123_), .c(new_n150_), .d(new_n67_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n149_), .c(new_n53_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(x18), .c(new_n61_), .O(new_n154_));
  nand2  g103(.a(x16), .b(x05), .O(new_n155_));
  oai21  g104(.a(x15), .b(x05), .c(new_n155_), .O(new_n156_));
  nor2   g105(.a(x23), .b(x21), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n131_), .c(new_n98_), .d(new_n65_), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n156_), .c(x29), .O(new_n159_));
  nand3  g108(.a(new_n53_), .b(new_n52_), .c(new_n60_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n159_), .c(new_n154_), .O(z10));
  nand2  g110(.a(new_n152_), .b(x30), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n140_), .c(new_n123_), .d(new_n102_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n53_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n64_), .O(z11));
  nor2   g118(.a(x26), .b(x25), .O(new_n170_));
  nand4  g119(.a(new_n163_), .b(new_n140_), .c(new_n170_), .d(new_n98_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n92_), .c(x31), .O(new_n172_));
  nand2  g121(.a(new_n103_), .b(new_n82_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  nor3   g123(.a(x27), .b(x26), .c(x25), .O(new_n175_));
  nor2   g124(.a(x29), .b(x28), .O(new_n176_));
  nor2   g125(.a(x31), .b(x30), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n175_), .c(new_n174_), .d(new_n67_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n172_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(x16), .O(new_n182_));
  inv1   g131(.a(x03), .O(new_n183_));
  aoi21  g132(.a(new_n53_), .b(new_n183_), .c(x18), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n182_), .c(new_n64_), .O(z12));
  nand4  g134(.a(new_n177_), .b(new_n176_), .c(new_n131_), .d(new_n120_), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n104_), .c(x32), .O(new_n187_));
  nand2  g136(.a(new_n140_), .b(new_n170_), .O(new_n188_));
  inv1   g137(.a(new_n188_), .O(new_n189_));
  nor2   g138(.a(x32), .b(x31), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n163_), .O(new_n191_));
  inv1   g140(.a(new_n191_), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n189_), .c(new_n174_), .d(new_n67_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n187_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(x16), .O(new_n195_));
  inv1   g144(.a(x02), .O(new_n196_));
  aoi21  g145(.a(new_n53_), .b(new_n196_), .c(x18), .O(new_n197_));
  aoi21  g146(.a(new_n197_), .b(new_n195_), .c(new_n64_), .O(z13));
  nand4  g147(.a(new_n190_), .b(new_n163_), .c(new_n140_), .d(new_n170_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n104_), .c(x33), .O(new_n200_));
  nor2   g149(.a(x33), .b(x32), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n177_), .c(new_n176_), .d(new_n131_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n113_), .c(new_n200_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(x16), .O(new_n204_));
  nor2   g153(.a(x16), .b(x01), .O(new_n205_));
  nor2   g154(.a(new_n205_), .b(new_n64_), .O(new_n206_));
  nand3  g155(.a(new_n206_), .b(new_n204_), .c(new_n72_), .O(z14));
  oai21  g156(.a(new_n202_), .b(new_n113_), .c(x34), .O(new_n208_));
  nand2  g157(.a(new_n176_), .b(new_n131_), .O(new_n209_));
  inv1   g158(.a(new_n209_), .O(new_n210_));
  inv1   g159(.a(x30), .O(new_n211_));
  inv1   g160(.a(x31), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n211_), .O(new_n213_));
  inv1   g162(.a(x32), .O(new_n214_));
  inv1   g163(.a(x33), .O(new_n215_));
  inv1   g164(.a(x34), .O(new_n216_));
  nand3  g165(.a(new_n216_), .b(new_n215_), .c(new_n214_), .O(new_n217_));
  nor2   g166(.a(new_n217_), .b(new_n213_), .O(new_n218_));
  nand3  g167(.a(new_n218_), .b(new_n210_), .c(new_n122_), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(new_n208_), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(x16), .O(new_n221_));
  inv1   g170(.a(x00), .O(new_n222_));
  aoi21  g171(.a(new_n53_), .b(new_n222_), .c(x18), .O(new_n223_));
  aoi21  g172(.a(new_n223_), .b(new_n221_), .c(new_n64_), .O(z15));
endmodule


