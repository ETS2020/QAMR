// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n230_, new_n231_, new_n232_, new_n233_, new_n234_,
    new_n235_, new_n236_, new_n237_;
  inv1   g000(.a(x15), .O(new_n52_));
  inv1   g001(.a(x16), .O(new_n53_));
  oai21  g002(.a(x18), .b(new_n53_), .c(new_n52_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x16), .O(new_n59_));
  inv1   g008(.a(x27), .O(new_n60_));
  nor2   g009(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  aoi21  g010(.a(new_n60_), .b(x18), .c(new_n61_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n59_), .c(new_n54_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n58_), .b(new_n64_), .O(new_n65_));
  nor2   g014(.a(x20), .b(x19), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n55_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n65_), .c(x16), .O(new_n69_));
  inv1   g018(.a(x14), .O(new_n70_));
  aoi21  g019(.a(new_n53_), .b(new_n70_), .c(x18), .O(new_n71_));
  aoi21  g020(.a(new_n71_), .b(new_n69_), .c(new_n61_), .O(z01));
  inv1   g021(.a(new_n61_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x18), .O(new_n74_));
  inv1   g023(.a(x21), .O(new_n75_));
  nor2   g024(.a(x21), .b(x20), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n58_), .O(new_n77_));
  oai21  g026(.a(new_n68_), .b(new_n75_), .c(new_n77_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(x16), .O(new_n79_));
  nor2   g028(.a(x16), .b(x13), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n61_), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n79_), .c(new_n74_), .O(z02));
  nand3  g031(.a(new_n77_), .b(x22), .c(x16), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  inv1   g033(.a(x18), .O(new_n85_));
  oai21  g034(.a(x16), .b(x12), .c(new_n85_), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n84_), .c(new_n73_), .O(new_n87_));
  inv1   g036(.a(x22), .O(new_n88_));
  inv1   g037(.a(x23), .O(new_n89_));
  oai21  g038(.a(x27), .b(new_n89_), .c(new_n52_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x27), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n88_), .c(new_n75_), .d(new_n64_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n56_), .c(new_n55_), .d(x16), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n87_), .O(z03));
  inv1   g044(.a(new_n58_), .O(new_n96_));
  nor2   g045(.a(x22), .b(x21), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n64_), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n89_), .O(new_n100_));
  nor2   g049(.a(x23), .b(x22), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n75_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n67_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n100_), .c(x16), .O(new_n104_));
  inv1   g053(.a(x11), .O(new_n105_));
  aoi21  g054(.a(new_n53_), .b(new_n105_), .c(x18), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n104_), .c(new_n61_), .O(z04));
  inv1   g056(.a(x24), .O(new_n108_));
  nor2   g057(.a(x24), .b(x23), .O(new_n109_));
  nand4  g058(.a(new_n109_), .b(new_n97_), .c(new_n66_), .d(new_n55_), .O(new_n110_));
  oai21  g059(.a(new_n103_), .b(new_n108_), .c(new_n110_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(x16), .O(new_n112_));
  nor2   g061(.a(x16), .b(x10), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n61_), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n112_), .c(new_n74_), .O(z05));
  inv1   g064(.a(x25), .O(new_n116_));
  inv1   g065(.a(new_n110_), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nor2   g067(.a(x25), .b(x24), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n101_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n77_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n118_), .c(x16), .O(new_n122_));
  nor2   g071(.a(x16), .b(x09), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n61_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(new_n74_), .O(z06));
  inv1   g074(.a(x26), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n101_), .c(new_n108_), .O(new_n128_));
  oai22  g077(.a(new_n128_), .b(new_n77_), .c(new_n121_), .d(new_n126_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  nor2   g079(.a(x16), .b(x08), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n61_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n130_), .c(new_n74_), .O(z07));
  oai21  g082(.a(x16), .b(x07), .c(new_n85_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n73_), .O(new_n135_));
  nand3  g084(.a(x27), .b(x23), .c(new_n52_), .O(new_n136_));
  nor2   g085(.a(x27), .b(x26), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n109_), .c(new_n116_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n136_), .c(x22), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n75_), .c(new_n64_), .d(new_n56_), .O(new_n140_));
  nor2   g089(.a(x24), .b(x22), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n127_), .c(new_n76_), .d(new_n58_), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(x27), .c(new_n52_), .O(new_n143_));
  oai21  g092(.a(new_n140_), .b(x17), .c(new_n143_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(x16), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n135_), .O(z08));
  oai21  g095(.a(x16), .b(x06), .c(new_n85_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n73_), .O(new_n148_));
  oai21  g097(.a(new_n128_), .b(new_n77_), .c(x28), .O(new_n149_));
  nor3   g098(.a(x28), .b(x26), .c(x25), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n109_), .c(new_n99_), .O(new_n151_));
  aoi21  g100(.a(new_n151_), .b(new_n149_), .c(x27), .O(new_n152_));
  inv1   g101(.a(x28), .O(new_n153_));
  nor3   g102(.a(new_n153_), .b(new_n60_), .c(x15), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n152_), .c(x16), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n148_), .O(z09));
  nand3  g105(.a(x29), .b(x27), .c(x16), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n85_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n52_), .O(new_n159_));
  nand4  g108(.a(new_n97_), .b(new_n58_), .c(new_n64_), .d(x16), .O(new_n160_));
  nor3   g109(.a(x25), .b(x24), .c(x23), .O(new_n161_));
  nor2   g110(.a(x29), .b(x28), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n161_), .c(new_n126_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n160_), .c(new_n85_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n60_), .O(new_n165_));
  nand3  g114(.a(new_n151_), .b(x29), .c(x16), .O(new_n166_));
  nor2   g115(.a(x16), .b(x05), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n61_), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n166_), .c(new_n165_), .d(new_n159_), .O(z10));
  nand3  g118(.a(x30), .b(x27), .c(x16), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n85_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n52_), .O(new_n172_));
  nor2   g121(.a(x28), .b(x26), .O(new_n173_));
  nor2   g122(.a(x30), .b(x29), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n161_), .c(new_n173_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n160_), .c(new_n85_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n60_), .O(new_n177_));
  nand4  g126(.a(new_n162_), .b(new_n161_), .c(new_n99_), .d(new_n126_), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(x30), .c(x16), .O(new_n179_));
  nor2   g128(.a(x16), .b(x04), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n61_), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n179_), .c(new_n177_), .d(new_n172_), .O(z11));
  nand3  g131(.a(x31), .b(x27), .c(x16), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n85_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n52_), .O(new_n185_));
  nand4  g134(.a(new_n101_), .b(new_n76_), .c(new_n58_), .d(x16), .O(new_n186_));
  nor3   g135(.a(x26), .b(x25), .c(x24), .O(new_n187_));
  nor2   g136(.a(x31), .b(x30), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n162_), .c(new_n187_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n186_), .c(new_n85_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n60_), .O(new_n191_));
  nand4  g140(.a(new_n174_), .b(new_n187_), .c(new_n103_), .d(new_n153_), .O(new_n192_));
  nand3  g141(.a(new_n192_), .b(x31), .c(x16), .O(new_n193_));
  nor2   g142(.a(x16), .b(x03), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n61_), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n193_), .c(new_n191_), .d(new_n185_), .O(z12));
  nand3  g145(.a(x32), .b(x27), .c(x16), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n85_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n52_), .O(new_n199_));
  inv1   g148(.a(x31), .O(new_n200_));
  inv1   g149(.a(x32), .O(new_n201_));
  nand3  g150(.a(new_n174_), .b(new_n201_), .c(new_n200_), .O(new_n202_));
  inv1   g151(.a(new_n202_), .O(new_n203_));
  nand3  g152(.a(new_n203_), .b(new_n173_), .c(new_n119_), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n186_), .c(new_n85_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n60_), .O(new_n206_));
  nand4  g155(.a(new_n188_), .b(new_n162_), .c(new_n187_), .d(new_n103_), .O(new_n207_));
  nand3  g156(.a(new_n207_), .b(x32), .c(x16), .O(new_n208_));
  nor2   g157(.a(x16), .b(x02), .O(new_n209_));
  nor2   g158(.a(new_n209_), .b(new_n61_), .O(new_n210_));
  nand4  g159(.a(new_n210_), .b(new_n208_), .c(new_n206_), .d(new_n199_), .O(z13));
  nand3  g160(.a(x33), .b(x27), .c(x16), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n85_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(new_n52_), .O(new_n214_));
  nor2   g163(.a(x17), .b(new_n53_), .O(new_n215_));
  nand4  g164(.a(new_n215_), .b(new_n109_), .c(new_n97_), .d(new_n66_), .O(new_n216_));
  nor2   g165(.a(x33), .b(x32), .O(new_n217_));
  nand4  g166(.a(new_n217_), .b(new_n188_), .c(new_n162_), .d(new_n127_), .O(new_n218_));
  oai21  g167(.a(new_n218_), .b(new_n216_), .c(new_n85_), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(new_n60_), .O(new_n220_));
  nand3  g169(.a(new_n203_), .b(new_n150_), .c(new_n117_), .O(new_n221_));
  nand3  g170(.a(new_n221_), .b(x33), .c(x16), .O(new_n222_));
  nor2   g171(.a(x16), .b(x01), .O(new_n223_));
  nor2   g172(.a(new_n223_), .b(new_n61_), .O(new_n224_));
  nand4  g173(.a(new_n224_), .b(new_n222_), .c(new_n220_), .d(new_n214_), .O(z14));
  oai21  g174(.a(x16), .b(x00), .c(new_n85_), .O(new_n226_));
  nand2  g175(.a(new_n226_), .b(new_n73_), .O(new_n227_));
  oai21  g176(.a(new_n218_), .b(new_n110_), .c(x34), .O(new_n228_));
  nand2  g177(.a(new_n174_), .b(new_n173_), .O(new_n229_));
  inv1   g178(.a(x33), .O(new_n230_));
  inv1   g179(.a(x34), .O(new_n231_));
  nand4  g180(.a(new_n231_), .b(new_n230_), .c(new_n201_), .d(new_n200_), .O(new_n232_));
  nor2   g181(.a(new_n232_), .b(new_n229_), .O(new_n233_));
  nand2  g182(.a(new_n233_), .b(new_n121_), .O(new_n234_));
  aoi21  g183(.a(new_n234_), .b(new_n228_), .c(x27), .O(new_n235_));
  nor3   g184(.a(new_n231_), .b(new_n60_), .c(x15), .O(new_n236_));
  oai21  g185(.a(new_n236_), .b(new_n235_), .c(x16), .O(new_n237_));
  nand2  g186(.a(new_n237_), .b(new_n227_), .O(z15));
endmodule


