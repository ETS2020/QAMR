// Benchmark "FAU" written by ABC on Wed Aug 19 19:40:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x25), .c(new_n52_), .O(z00));
  inv1   g008(.a(x16), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  aoi21  g010(.a(new_n55_), .b(x20), .c(new_n61_), .O(new_n62_));
  nor2   g011(.a(x16), .b(x14), .O(new_n63_));
  nor2   g012(.a(x25), .b(x18), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  oai21  g015(.a(new_n62_), .b(new_n60_), .c(new_n66_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n54_), .O(new_n70_));
  oai21  g019(.a(new_n61_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  nor2   g020(.a(x16), .b(x13), .O(new_n72_));
  aoi21  g021(.a(new_n71_), .b(x16), .c(new_n72_), .O(new_n73_));
  oai21  g022(.a(new_n73_), .b(x25), .c(new_n52_), .O(z02));
  inv1   g023(.a(x25), .O(new_n75_));
  nand2  g024(.a(new_n70_), .b(x22), .O(new_n76_));
  inv1   g025(.a(x20), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n54_), .c(new_n77_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n76_), .c(new_n60_), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n80_), .c(new_n75_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n52_), .O(z03));
  nand2  g032(.a(new_n79_), .b(x23), .O(new_n84_));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n61_), .O(new_n86_));
  aoi21  g035(.a(new_n86_), .b(new_n84_), .c(new_n60_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x11), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n87_), .c(new_n75_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n52_), .O(z04));
  nand2  g039(.a(new_n86_), .b(x24), .O(new_n91_));
  inv1   g040(.a(x17), .O(new_n92_));
  nor2   g041(.a(x20), .b(x19), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n78_), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n91_), .c(new_n60_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n75_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n52_), .O(z05));
  inv1   g048(.a(x09), .O(new_n100_));
  nand2  g049(.a(new_n60_), .b(new_n100_), .O(new_n101_));
  nor2   g050(.a(x17), .b(new_n60_), .O(new_n102_));
  nor3   g051(.a(x25), .b(x24), .c(x23), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n102_), .c(new_n78_), .d(new_n93_), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n101_), .c(new_n64_), .O(z06));
  nand2  g054(.a(new_n95_), .b(x26), .O(new_n106_));
  nor2   g055(.a(x23), .b(x22), .O(new_n107_));
  nor2   g056(.a(x26), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n107_), .c(new_n69_), .d(new_n54_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n106_), .c(new_n60_), .O(new_n110_));
  nor2   g059(.a(x16), .b(x08), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n110_), .c(new_n75_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n52_), .O(z07));
  inv1   g062(.a(x27), .O(new_n114_));
  inv1   g063(.a(x23), .O(new_n115_));
  nand4  g064(.a(new_n108_), .b(new_n78_), .c(new_n115_), .d(new_n77_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n75_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n54_), .c(new_n114_), .O(new_n118_));
  nor3   g067(.a(x27), .b(x26), .c(x24), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n107_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n70_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n118_), .c(x16), .O(new_n122_));
  nor2   g071(.a(x16), .b(x07), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n65_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n122_), .O(z08));
  inv1   g074(.a(x28), .O(new_n126_));
  nor2   g075(.a(x27), .b(x26), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n94_), .c(new_n78_), .d(new_n77_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n75_), .c(new_n55_), .O(new_n129_));
  nor3   g078(.a(x28), .b(x27), .c(x26), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n94_), .O(new_n131_));
  oai22  g080(.a(new_n131_), .b(new_n79_), .c(new_n129_), .d(new_n126_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(x16), .O(new_n133_));
  nor2   g082(.a(x16), .b(x06), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n65_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n133_), .O(z09));
  inv1   g085(.a(x29), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n108_), .c(new_n107_), .d(new_n69_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n75_), .c(new_n55_), .O(new_n140_));
  nor2   g089(.a(x29), .b(x28), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n108_), .c(new_n114_), .d(new_n115_), .O(new_n142_));
  oai22  g091(.a(new_n142_), .b(new_n79_), .c(new_n140_), .d(new_n137_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  nor2   g093(.a(x16), .b(x05), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n65_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n144_), .O(z10));
  oai21  g096(.a(new_n142_), .b(new_n79_), .c(x30), .O(new_n148_));
  nor3   g097(.a(x30), .b(x29), .c(x28), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n119_), .c(new_n85_), .d(new_n61_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n148_), .c(new_n60_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x04), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n151_), .c(new_n75_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n52_), .O(z11));
  inv1   g103(.a(x31), .O(new_n155_));
  inv1   g104(.a(x22), .O(new_n156_));
  nand3  g105(.a(new_n94_), .b(new_n69_), .c(new_n156_), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n127_), .c(new_n126_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n157_), .c(new_n75_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n54_), .c(new_n155_), .O(new_n161_));
  nor2   g110(.a(x31), .b(x30), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n141_), .c(new_n119_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n86_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n161_), .c(x16), .O(new_n165_));
  nor2   g114(.a(x16), .b(x03), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n65_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n165_), .O(z12));
  inv1   g117(.a(x32), .O(new_n169_));
  inv1   g118(.a(x26), .O(new_n170_));
  nand4  g119(.a(new_n162_), .b(new_n138_), .c(new_n137_), .d(new_n170_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n157_), .c(new_n75_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n54_), .c(new_n169_), .O(new_n173_));
  nor2   g122(.a(x32), .b(x31), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n158_), .c(new_n130_), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n95_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n173_), .c(x16), .O(new_n177_));
  nor2   g126(.a(x16), .b(x02), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n65_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z13));
  inv1   g129(.a(x33), .O(new_n181_));
  inv1   g130(.a(x30), .O(new_n182_));
  nand4  g131(.a(new_n174_), .b(new_n141_), .c(new_n182_), .d(new_n114_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n116_), .c(new_n75_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n54_), .c(new_n181_), .O(new_n185_));
  nor2   g134(.a(x33), .b(x32), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n162_), .c(new_n141_), .d(new_n127_), .O(new_n187_));
  nor2   g136(.a(new_n187_), .b(new_n95_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n185_), .c(x16), .O(new_n189_));
  nor2   g138(.a(x16), .b(x01), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n65_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n189_), .O(z14));
  inv1   g141(.a(x34), .O(new_n193_));
  nand4  g142(.a(new_n186_), .b(new_n162_), .c(new_n141_), .d(new_n114_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n116_), .c(new_n75_), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n54_), .c(new_n193_), .O(new_n196_));
  nor2   g145(.a(x34), .b(x33), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n174_), .c(new_n158_), .d(new_n138_), .O(new_n198_));
  nor2   g147(.a(new_n198_), .b(new_n109_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n196_), .c(x16), .O(new_n200_));
  nor2   g149(.a(x16), .b(x00), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n65_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n200_), .O(z15));
endmodule


