// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g008(.a(x30), .b(x18), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g010(.a(x18), .O(new_n62_));
  inv1   g011(.a(x30), .O(new_n63_));
  inv1   g012(.a(new_n55_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x20), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  nand3  g015(.a(new_n66_), .b(new_n53_), .c(new_n52_), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n65_), .c(new_n58_), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n62_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n55_), .b(new_n72_), .c(new_n66_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n67_), .b(x21), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(new_n60_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g027(.a(new_n75_), .b(new_n58_), .c(new_n78_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(new_n67_), .O(new_n81_));
  nor2   g030(.a(new_n80_), .b(new_n72_), .O(new_n82_));
  nor2   g031(.a(x20), .b(x17), .O(new_n83_));
  nor2   g032(.a(x22), .b(x21), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  oai22  g034(.a(new_n85_), .b(x19), .c(new_n81_), .d(new_n80_), .O(new_n86_));
  nor2   g035(.a(x16), .b(x12), .O(new_n87_));
  aoi21  g036(.a(new_n86_), .b(x16), .c(new_n87_), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(x30), .c(new_n62_), .O(z03));
  nor2   g038(.a(new_n84_), .b(x19), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n67_), .c(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n72_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n67_), .O(new_n94_));
  inv1   g043(.a(new_n94_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n91_), .c(new_n58_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x11), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n96_), .c(new_n63_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n62_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  aoi21  g049(.a(new_n93_), .b(new_n53_), .c(new_n67_), .O(new_n101_));
  nor2   g050(.a(x24), .b(x23), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n84_), .O(new_n103_));
  oai22  g052(.a(new_n103_), .b(new_n67_), .c(new_n101_), .d(new_n100_), .O(new_n104_));
  nor2   g053(.a(x16), .b(x10), .O(new_n105_));
  aoi21  g054(.a(new_n104_), .b(x16), .c(new_n105_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(x30), .c(new_n62_), .O(z05));
  inv1   g056(.a(x25), .O(new_n108_));
  nand2  g057(.a(new_n103_), .b(new_n53_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n81_), .c(new_n108_), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n92_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n73_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n110_), .c(x16), .O(new_n114_));
  nor2   g063(.a(x16), .b(x09), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n76_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n114_), .O(z06));
  inv1   g066(.a(x26), .O(new_n118_));
  nor3   g067(.a(x25), .b(x24), .c(x23), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n84_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n53_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n81_), .c(new_n118_), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n92_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n73_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n122_), .c(x16), .O(new_n126_));
  nor2   g075(.a(x16), .b(x08), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n76_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z07));
  inv1   g078(.a(x27), .O(new_n130_));
  nand3  g079(.a(new_n123_), .b(new_n92_), .c(new_n72_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n53_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n81_), .c(new_n130_), .O(new_n133_));
  nand2  g082(.a(new_n84_), .b(new_n66_), .O(new_n134_));
  nor3   g083(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n102_), .O(new_n136_));
  nor3   g085(.a(new_n136_), .b(new_n134_), .c(new_n64_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n133_), .c(x16), .O(new_n138_));
  nor2   g087(.a(x16), .b(x07), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n76_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(z08));
  inv1   g090(.a(x28), .O(new_n142_));
  nor2   g091(.a(x27), .b(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n111_), .c(new_n92_), .d(new_n72_), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n53_), .c(new_n67_), .O(new_n145_));
  nor2   g094(.a(new_n134_), .b(new_n64_), .O(new_n146_));
  nor3   g095(.a(x28), .b(x27), .c(x26), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n146_), .c(new_n119_), .O(new_n148_));
  oai21  g097(.a(new_n145_), .b(new_n142_), .c(new_n148_), .O(new_n149_));
  nor2   g098(.a(x16), .b(x06), .O(new_n150_));
  aoi21  g099(.a(new_n149_), .b(x16), .c(new_n150_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(x30), .c(new_n62_), .O(z09));
  inv1   g101(.a(x29), .O(new_n153_));
  nor2   g102(.a(x26), .b(x25), .O(new_n154_));
  nor2   g103(.a(x28), .b(x27), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n102_), .d(new_n84_), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n53_), .c(new_n67_), .O(new_n157_));
  nor3   g106(.a(x29), .b(x28), .c(x27), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n123_), .c(new_n94_), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n153_), .c(new_n159_), .O(new_n160_));
  nor2   g109(.a(x16), .b(x05), .O(new_n161_));
  aoi21  g110(.a(new_n160_), .b(x16), .c(new_n161_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(x30), .c(new_n62_), .O(z10));
  nand4  g112(.a(new_n84_), .b(new_n55_), .c(new_n66_), .d(x16), .O(new_n164_));
  nor2   g113(.a(x29), .b(x28), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n143_), .c(new_n119_), .O(new_n166_));
  oai22  g115(.a(new_n166_), .b(new_n164_), .c(x16), .d(x04), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n63_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n62_), .O(z11));
  inv1   g118(.a(x31), .O(new_n170_));
  nand4  g119(.a(new_n158_), .b(new_n154_), .c(new_n102_), .d(new_n84_), .O(new_n171_));
  aoi21  g120(.a(new_n171_), .b(new_n53_), .c(new_n67_), .O(new_n172_));
  nor2   g121(.a(x31), .b(x29), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n155_), .c(new_n123_), .d(new_n94_), .O(new_n174_));
  oai21  g123(.a(new_n172_), .b(new_n170_), .c(new_n174_), .O(new_n175_));
  nor2   g124(.a(x16), .b(x03), .O(new_n176_));
  aoi21  g125(.a(new_n175_), .b(x16), .c(new_n176_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(x30), .c(new_n62_), .O(z12));
  inv1   g127(.a(x32), .O(new_n179_));
  nor3   g128(.a(x31), .b(x29), .c(x28), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n143_), .c(new_n119_), .d(new_n84_), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n53_), .c(new_n67_), .O(new_n182_));
  nor2   g131(.a(new_n103_), .b(new_n67_), .O(new_n183_));
  nor2   g132(.a(x32), .b(x31), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n165_), .c(new_n135_), .d(new_n183_), .O(new_n185_));
  oai21  g134(.a(new_n182_), .b(new_n179_), .c(new_n185_), .O(new_n186_));
  nor2   g135(.a(x16), .b(x02), .O(new_n187_));
  aoi21  g136(.a(new_n186_), .b(x16), .c(new_n187_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(x30), .c(new_n62_), .O(z13));
  inv1   g138(.a(x33), .O(new_n190_));
  nor3   g139(.a(x32), .b(x31), .c(x29), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n147_), .c(new_n119_), .d(new_n84_), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n53_), .c(new_n67_), .O(new_n193_));
  nand2  g142(.a(new_n155_), .b(new_n154_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  nor2   g144(.a(x33), .b(x32), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n173_), .c(new_n195_), .d(new_n183_), .O(new_n197_));
  oai21  g146(.a(new_n193_), .b(new_n190_), .c(new_n197_), .O(new_n198_));
  nor2   g147(.a(x16), .b(x01), .O(new_n199_));
  aoi21  g148(.a(new_n198_), .b(x16), .c(new_n199_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(x30), .c(new_n62_), .O(z14));
  inv1   g150(.a(x34), .O(new_n202_));
  nand3  g151(.a(new_n196_), .b(new_n158_), .c(new_n170_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n131_), .c(new_n53_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(new_n81_), .c(new_n202_), .O(new_n205_));
  nand2  g154(.a(new_n165_), .b(new_n143_), .O(new_n206_));
  nand3  g155(.a(new_n184_), .b(new_n202_), .c(new_n190_), .O(new_n207_));
  nor4   g156(.a(new_n207_), .b(new_n206_), .c(new_n112_), .d(new_n73_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n205_), .c(x16), .O(new_n209_));
  nor2   g158(.a(x16), .b(x00), .O(new_n210_));
  nor2   g159(.a(new_n210_), .b(new_n76_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n209_), .O(z15));
endmodule


