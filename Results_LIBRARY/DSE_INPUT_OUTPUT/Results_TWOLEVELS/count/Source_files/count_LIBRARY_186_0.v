// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  nor2   g000(.a(x12), .b(x08), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g012(.a(new_n62_), .b(new_n54_), .c(new_n53_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n63_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n59_), .b(new_n67_), .c(x18), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n66_), .c(new_n52_), .O(z01));
  oai21  g018(.a(x12), .b(x08), .c(x18), .O(new_n70_));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n56_), .O(new_n73_));
  oai21  g022(.a(new_n65_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n52_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n70_), .O(z02));
  inv1   g027(.a(x08), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  oai21  g029(.a(new_n59_), .b(new_n79_), .c(new_n80_), .O(new_n81_));
  and2   g030(.a(new_n73_), .b(x22), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n56_), .c(new_n62_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n82_), .c(x16), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n81_), .c(new_n70_), .O(z03));
  inv1   g036(.a(x23), .O(new_n88_));
  nor2   g037(.a(new_n85_), .b(new_n88_), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n71_), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n64_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n89_), .c(x16), .O(new_n93_));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n52_), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n93_), .c(new_n70_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n83_), .c(new_n65_), .O(new_n99_));
  oai21  g048(.a(new_n92_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(x16), .O(new_n101_));
  nor2   g050(.a(x16), .b(x10), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n52_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n101_), .c(new_n70_), .O(z05));
  inv1   g053(.a(x25), .O(new_n105_));
  nand2  g054(.a(new_n98_), .b(new_n83_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n64_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n90_), .c(new_n72_), .d(new_n56_), .O(new_n109_));
  oai21  g058(.a(new_n107_), .b(new_n105_), .c(new_n109_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  nor2   g060(.a(x16), .b(x09), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n52_), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n111_), .c(new_n70_), .O(z06));
  inv1   g063(.a(x26), .O(new_n115_));
  nor3   g064(.a(x26), .b(x25), .c(x24), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n90_), .O(new_n117_));
  oai22  g066(.a(new_n117_), .b(new_n73_), .c(new_n65_), .d(new_n115_), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(x16), .c(x18), .O(new_n119_));
  oai21  g068(.a(x19), .b(new_n79_), .c(new_n80_), .O(new_n120_));
  nand3  g069(.a(new_n108_), .b(new_n83_), .c(new_n88_), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n120_), .c(x26), .d(x16), .O(new_n122_));
  nand3  g071(.a(new_n59_), .b(x12), .c(new_n79_), .O(new_n123_));
  and2   g072(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  oai21  g073(.a(new_n119_), .b(new_n52_), .c(new_n124_), .O(z07));
  inv1   g074(.a(x27), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n90_), .c(new_n97_), .d(new_n71_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n54_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n65_), .c(new_n126_), .O(new_n130_));
  nor2   g079(.a(x27), .b(x26), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n98_), .c(new_n105_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n84_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n130_), .c(x16), .O(new_n134_));
  inv1   g083(.a(x07), .O(new_n135_));
  aoi21  g084(.a(new_n59_), .b(new_n135_), .c(x18), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n134_), .c(new_n52_), .O(z08));
  inv1   g086(.a(x28), .O(new_n138_));
  nor2   g087(.a(new_n133_), .b(new_n138_), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n108_), .c(new_n115_), .d(new_n88_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n84_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n139_), .c(x16), .O(new_n143_));
  nor2   g092(.a(x16), .b(x06), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n52_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(new_n70_), .O(z09));
  inv1   g095(.a(x29), .O(new_n147_));
  nand3  g096(.a(new_n90_), .b(new_n65_), .c(new_n71_), .O(new_n148_));
  nor2   g097(.a(x29), .b(x28), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n116_), .c(new_n126_), .O(new_n150_));
  oai22  g099(.a(new_n150_), .b(new_n148_), .c(new_n142_), .d(new_n147_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  nor2   g101(.a(x16), .b(x05), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n52_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(new_n70_), .O(z10));
  inv1   g104(.a(x30), .O(new_n156_));
  nand3  g105(.a(new_n149_), .b(new_n127_), .c(new_n126_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n106_), .c(new_n54_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n65_), .c(new_n156_), .O(new_n159_));
  nor2   g108(.a(x30), .b(x29), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n140_), .c(new_n116_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n148_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n159_), .c(x16), .O(new_n163_));
  inv1   g112(.a(x04), .O(new_n164_));
  aoi21  g113(.a(new_n59_), .b(new_n164_), .c(x18), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n163_), .c(new_n52_), .O(z11));
  inv1   g115(.a(x31), .O(new_n167_));
  nand3  g116(.a(new_n160_), .b(new_n131_), .c(new_n138_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n121_), .c(new_n54_), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n65_), .c(new_n167_), .O(new_n170_));
  nor2   g119(.a(x31), .b(x30), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n149_), .c(new_n131_), .d(new_n105_), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n99_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n170_), .c(x16), .O(new_n174_));
  inv1   g123(.a(x03), .O(new_n175_));
  aoi21  g124(.a(new_n59_), .b(new_n175_), .c(x18), .O(new_n176_));
  aoi21  g125(.a(new_n176_), .b(new_n174_), .c(new_n52_), .O(z12));
  inv1   g126(.a(x32), .O(new_n178_));
  nor2   g127(.a(x32), .b(x31), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n160_), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n140_), .c(new_n127_), .d(new_n107_), .O(new_n182_));
  oai21  g131(.a(new_n173_), .b(new_n178_), .c(new_n182_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(x16), .O(new_n184_));
  nor2   g133(.a(x16), .b(x02), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n52_), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n184_), .c(new_n70_), .O(z13));
  inv1   g136(.a(x33), .O(new_n188_));
  nand4  g137(.a(new_n179_), .b(new_n149_), .c(new_n156_), .d(new_n126_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n128_), .c(new_n54_), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n65_), .c(new_n188_), .O(new_n191_));
  nand2  g140(.a(new_n149_), .b(new_n131_), .O(new_n192_));
  nor2   g141(.a(x33), .b(x32), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n171_), .O(new_n194_));
  nor3   g143(.a(new_n194_), .b(new_n192_), .c(new_n109_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n191_), .c(x16), .O(new_n196_));
  inv1   g145(.a(x01), .O(new_n197_));
  aoi21  g146(.a(new_n59_), .b(new_n197_), .c(x18), .O(new_n198_));
  aoi21  g147(.a(new_n198_), .b(new_n196_), .c(new_n52_), .O(z14));
  inv1   g148(.a(x34), .O(new_n200_));
  nand4  g149(.a(new_n193_), .b(new_n171_), .c(new_n149_), .d(new_n126_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(new_n128_), .c(new_n54_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n65_), .c(new_n200_), .O(new_n203_));
  nand4  g152(.a(new_n171_), .b(new_n200_), .c(new_n188_), .d(new_n178_), .O(new_n204_));
  nor3   g153(.a(new_n204_), .b(new_n192_), .c(new_n109_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n203_), .c(x16), .O(new_n206_));
  inv1   g155(.a(x00), .O(new_n207_));
  aoi21  g156(.a(new_n59_), .b(new_n207_), .c(x18), .O(new_n208_));
  aoi21  g157(.a(new_n208_), .b(new_n206_), .c(new_n52_), .O(z15));
endmodule


