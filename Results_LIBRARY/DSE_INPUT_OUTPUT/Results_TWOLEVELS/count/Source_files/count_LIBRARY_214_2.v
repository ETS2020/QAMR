// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x03), .c(new_n52_), .O(z00));
  inv1   g008(.a(x03), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n54_), .b(new_n61_), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  nand3  g013(.a(new_n61_), .b(new_n64_), .c(new_n63_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n62_), .c(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  inv1   g017(.a(x16), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n68_), .c(x18), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n67_), .c(new_n60_), .O(z01));
  nand2  g020(.a(x20), .b(new_n60_), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n64_), .c(new_n63_), .O(new_n73_));
  inv1   g022(.a(x21), .O(new_n74_));
  nand4  g023(.a(new_n74_), .b(new_n61_), .c(new_n64_), .d(new_n63_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  aoi21  g025(.a(new_n73_), .b(x21), .c(new_n76_), .O(new_n77_));
  nor2   g026(.a(x16), .b(x13), .O(new_n78_));
  nor3   g027(.a(new_n78_), .b(x18), .c(x03), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n69_), .c(new_n79_), .O(z02));
  nand2  g029(.a(new_n75_), .b(x22), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n54_), .c(new_n61_), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n81_), .c(new_n69_), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n84_), .c(new_n60_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n52_), .O(z03));
  nor3   g036(.a(x22), .b(x21), .c(x20), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(x03), .c(new_n54_), .O(new_n89_));
  inv1   g038(.a(x22), .O(new_n90_));
  inv1   g039(.a(x23), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor3   g041(.a(new_n92_), .b(new_n65_), .c(x21), .O(new_n93_));
  aoi21  g042(.a(new_n89_), .b(x23), .c(new_n93_), .O(new_n94_));
  nor2   g043(.a(x16), .b(x11), .O(new_n95_));
  nor3   g044(.a(new_n95_), .b(x18), .c(x03), .O(new_n96_));
  oai21  g045(.a(new_n94_), .b(new_n69_), .c(new_n96_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  nor2   g047(.a(x21), .b(x20), .O(new_n99_));
  nor2   g048(.a(x23), .b(x22), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n60_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n54_), .c(new_n98_), .O(new_n103_));
  nor2   g052(.a(x20), .b(x19), .O(new_n104_));
  nor2   g053(.a(x24), .b(x23), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n82_), .c(new_n104_), .d(new_n63_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n103_), .c(x16), .O(new_n108_));
  inv1   g057(.a(x10), .O(new_n109_));
  aoi21  g058(.a(new_n69_), .b(new_n109_), .c(x18), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n108_), .c(new_n60_), .O(z05));
  nand2  g060(.a(new_n106_), .b(x25), .O(new_n112_));
  nor2   g061(.a(x25), .b(x24), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n100_), .c(new_n99_), .d(new_n54_), .O(new_n114_));
  aoi21  g063(.a(new_n114_), .b(new_n112_), .c(new_n69_), .O(new_n115_));
  nor2   g064(.a(x16), .b(x09), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n115_), .c(new_n60_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n52_), .O(z06));
  inv1   g067(.a(x26), .O(new_n119_));
  nand4  g068(.a(new_n113_), .b(new_n82_), .c(new_n91_), .d(new_n61_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n60_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n54_), .c(new_n119_), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n100_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n75_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n122_), .c(x16), .O(new_n126_));
  inv1   g075(.a(x08), .O(new_n127_));
  aoi21  g076(.a(new_n69_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n126_), .c(new_n60_), .O(z07));
  inv1   g078(.a(x27), .O(new_n130_));
  nor2   g079(.a(x26), .b(x25), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n105_), .c(new_n88_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n60_), .c(new_n55_), .O(new_n133_));
  inv1   g082(.a(x25), .O(new_n134_));
  nor2   g083(.a(x27), .b(x26), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n105_), .c(new_n134_), .O(new_n136_));
  or2    g085(.a(new_n136_), .b(new_n83_), .O(new_n137_));
  oai21  g086(.a(new_n133_), .b(new_n130_), .c(new_n137_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(x16), .O(new_n139_));
  nor2   g088(.a(x16), .b(x07), .O(new_n140_));
  nor3   g089(.a(new_n140_), .b(x18), .c(x03), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n139_), .O(z08));
  oai21  g091(.a(new_n136_), .b(new_n83_), .c(x28), .O(new_n143_));
  nor3   g092(.a(x25), .b(x24), .c(x23), .O(new_n144_));
  nor3   g093(.a(x28), .b(x27), .c(x26), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n144_), .c(new_n88_), .d(new_n54_), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n143_), .c(new_n69_), .O(new_n147_));
  nor2   g096(.a(x16), .b(x06), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n147_), .c(new_n60_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n52_), .O(z09));
  nand2  g099(.a(new_n146_), .b(x29), .O(new_n151_));
  nor2   g100(.a(new_n92_), .b(x21), .O(new_n152_));
  nor3   g101(.a(x29), .b(x28), .c(x27), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n123_), .c(new_n152_), .d(new_n66_), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n151_), .c(new_n69_), .O(new_n155_));
  nor2   g104(.a(x16), .b(x05), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n155_), .c(new_n60_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n52_), .O(z10));
  inv1   g107(.a(x30), .O(new_n159_));
  nand3  g108(.a(new_n105_), .b(new_n99_), .c(new_n90_), .O(new_n160_));
  nor2   g109(.a(x29), .b(x28), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n131_), .c(new_n130_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n160_), .c(new_n60_), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n54_), .c(new_n159_), .O(new_n164_));
  nand2  g113(.a(new_n152_), .b(new_n66_), .O(new_n165_));
  nor2   g114(.a(x28), .b(x27), .O(new_n166_));
  nor2   g115(.a(x30), .b(x29), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n166_), .c(new_n123_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n164_), .c(x16), .O(new_n170_));
  inv1   g119(.a(x04), .O(new_n171_));
  aoi21  g120(.a(new_n69_), .b(new_n171_), .c(x18), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n170_), .c(new_n60_), .O(z11));
  nand4  g122(.a(new_n167_), .b(new_n166_), .c(new_n123_), .d(new_n93_), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n161_), .c(new_n135_), .d(new_n134_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n106_), .c(x16), .O(new_n177_));
  aoi21  g126(.a(new_n174_), .b(x31), .c(new_n177_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(x03), .c(new_n52_), .O(z12));
  inv1   g128(.a(x32), .O(new_n180_));
  inv1   g129(.a(x29), .O(new_n181_));
  nand4  g130(.a(new_n175_), .b(new_n166_), .c(new_n181_), .d(new_n119_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n120_), .c(new_n60_), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n54_), .c(new_n180_), .O(new_n184_));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n167_), .c(new_n166_), .d(new_n131_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n106_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n184_), .c(x16), .O(new_n188_));
  inv1   g137(.a(x02), .O(new_n189_));
  aoi21  g138(.a(new_n69_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand3  g139(.a(new_n190_), .b(new_n188_), .c(new_n60_), .O(z13));
  oai21  g140(.a(new_n186_), .b(new_n106_), .c(x33), .O(new_n192_));
  nand4  g141(.a(new_n134_), .b(new_n98_), .c(new_n91_), .d(new_n90_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n75_), .O(new_n194_));
  nor2   g143(.a(x33), .b(x32), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n175_), .c(new_n161_), .d(new_n135_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n194_), .O(new_n198_));
  aoi21  g147(.a(new_n198_), .b(new_n192_), .c(new_n69_), .O(new_n199_));
  nor2   g148(.a(x16), .b(x01), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n199_), .c(new_n60_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n52_), .O(z14));
  oai21  g151(.a(new_n196_), .b(new_n114_), .c(x34), .O(new_n203_));
  nand2  g152(.a(new_n161_), .b(new_n135_), .O(new_n204_));
  inv1   g153(.a(new_n204_), .O(new_n205_));
  inv1   g154(.a(x31), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n159_), .O(new_n207_));
  inv1   g156(.a(x33), .O(new_n208_));
  inv1   g157(.a(x34), .O(new_n209_));
  nand3  g158(.a(new_n209_), .b(new_n208_), .c(new_n180_), .O(new_n210_));
  nor2   g159(.a(new_n210_), .b(new_n207_), .O(new_n211_));
  nand3  g160(.a(new_n211_), .b(new_n205_), .c(new_n194_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n203_), .c(new_n69_), .O(new_n213_));
  nor2   g162(.a(x16), .b(x00), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n213_), .c(new_n60_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n52_), .O(z15));
endmodule


