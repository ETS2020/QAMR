// Benchmark "FAU" written by ABC on Wed Aug 19 19:40:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_;
  nor2   g000(.a(x15), .b(x07), .O(new_n52_));
  xnor2a g001(.a(x19), .b(x17), .O(new_n53_));
  aoi21  g002(.a(new_n53_), .b(x16), .c(x18), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand3  g005(.a(new_n56_), .b(new_n55_), .c(x07), .O(new_n57_));
  oai21  g006(.a(new_n54_), .b(new_n52_), .c(new_n57_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(x19), .b(x17), .O(new_n60_));
  nor2   g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g012(.a(x14), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(new_n64_), .c(x18), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n52_), .O(z01));
  oai21  g015(.a(x15), .b(x07), .c(x18), .O(new_n67_));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n60_), .O(new_n70_));
  oai21  g019(.a(new_n62_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x16), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  nor2   g022(.a(new_n73_), .b(new_n52_), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n72_), .c(new_n67_), .O(z02));
  inv1   g024(.a(x22), .O(new_n76_));
  inv1   g025(.a(new_n70_), .O(new_n77_));
  nand4  g026(.a(new_n60_), .b(new_n76_), .c(new_n68_), .d(new_n59_), .O(new_n78_));
  oai21  g027(.a(new_n77_), .b(new_n76_), .c(new_n78_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(x16), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n52_), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n80_), .c(new_n67_), .O(z03));
  nand2  g032(.a(new_n78_), .b(x23), .O(new_n84_));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n62_), .c(new_n68_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x16), .O(new_n88_));
  nor2   g037(.a(x16), .b(x11), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n52_), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n88_), .c(new_n67_), .O(z04));
  inv1   g040(.a(x24), .O(new_n92_));
  inv1   g041(.a(x19), .O(new_n93_));
  nand2  g042(.a(new_n85_), .b(new_n68_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n62_), .c(new_n92_), .O(new_n96_));
  inv1   g045(.a(x17), .O(new_n97_));
  nand3  g046(.a(new_n59_), .b(new_n93_), .c(new_n97_), .O(new_n98_));
  inv1   g047(.a(x23), .O(new_n99_));
  nand4  g048(.a(new_n92_), .b(new_n99_), .c(new_n76_), .d(new_n68_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n96_), .c(x16), .O(new_n102_));
  inv1   g051(.a(x10), .O(new_n103_));
  aoi21  g052(.a(new_n56_), .b(new_n103_), .c(x18), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n102_), .c(new_n52_), .O(z05));
  inv1   g054(.a(x25), .O(new_n106_));
  nand2  g055(.a(new_n100_), .b(new_n93_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n62_), .c(new_n106_), .O(new_n108_));
  nor2   g057(.a(x25), .b(x24), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n85_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n70_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n108_), .c(x16), .O(new_n112_));
  inv1   g061(.a(x09), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(new_n113_), .c(x18), .O(new_n114_));
  aoi21  g063(.a(new_n114_), .b(new_n112_), .c(new_n52_), .O(z06));
  inv1   g064(.a(x26), .O(new_n116_));
  nor2   g065(.a(new_n111_), .b(new_n116_), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n85_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n70_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  nor2   g070(.a(x16), .b(x08), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n52_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n121_), .c(new_n67_), .O(z07));
  inv1   g073(.a(x07), .O(new_n125_));
  oai21  g074(.a(new_n56_), .b(new_n55_), .c(new_n125_), .O(new_n126_));
  inv1   g075(.a(x27), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n92_), .c(new_n99_), .O(new_n129_));
  oai22  g078(.a(new_n129_), .b(new_n78_), .c(new_n120_), .d(new_n127_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n126_), .c(new_n67_), .O(z08));
  inv1   g081(.a(x28), .O(new_n133_));
  nor2   g082(.a(x27), .b(x26), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n109_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n94_), .c(new_n93_), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n62_), .c(new_n133_), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n109_), .c(new_n116_), .d(new_n99_), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n78_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n137_), .c(x16), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  aoi21  g091(.a(new_n56_), .b(new_n142_), .c(x18), .O(new_n143_));
  aoi21  g092(.a(new_n143_), .b(new_n141_), .c(new_n52_), .O(z09));
  inv1   g093(.a(x29), .O(new_n145_));
  nor2   g094(.a(new_n94_), .b(new_n98_), .O(new_n146_));
  nor3   g095(.a(x29), .b(x28), .c(x27), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n118_), .c(new_n146_), .O(new_n148_));
  oai21  g097(.a(new_n140_), .b(new_n145_), .c(new_n148_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  nor2   g099(.a(x16), .b(x05), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n52_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n150_), .c(new_n67_), .O(z10));
  nand2  g102(.a(new_n148_), .b(x30), .O(new_n154_));
  nor2   g103(.a(x30), .b(x29), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n138_), .c(new_n118_), .O(new_n156_));
  or2    g105(.a(new_n156_), .b(new_n86_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  nor2   g108(.a(x16), .b(x04), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n52_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n159_), .c(new_n67_), .O(z11));
  oai21  g111(.a(new_n156_), .b(new_n86_), .c(x31), .O(new_n163_));
  nor2   g112(.a(x29), .b(x28), .O(new_n164_));
  nor2   g113(.a(x31), .b(x30), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n164_), .c(new_n128_), .d(new_n101_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n163_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x16), .O(new_n168_));
  nor2   g117(.a(x16), .b(x03), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n52_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n168_), .c(new_n67_), .O(z12));
  nand2  g120(.a(new_n166_), .b(x32), .O(new_n172_));
  nor2   g121(.a(x26), .b(x25), .O(new_n173_));
  nor2   g122(.a(x32), .b(x31), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n155_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n138_), .c(new_n173_), .d(new_n101_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n172_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x16), .O(new_n179_));
  nor2   g128(.a(x16), .b(x02), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n52_), .O(new_n181_));
  nand3  g130(.a(new_n181_), .b(new_n179_), .c(new_n67_), .O(z13));
  nand2  g131(.a(new_n177_), .b(x33), .O(new_n183_));
  nand2  g132(.a(new_n164_), .b(new_n134_), .O(new_n184_));
  inv1   g133(.a(new_n184_), .O(new_n185_));
  nor2   g134(.a(x33), .b(x32), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n185_), .c(new_n165_), .d(new_n111_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n183_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(x16), .O(new_n189_));
  nor2   g138(.a(x16), .b(x01), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n52_), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(new_n189_), .c(new_n67_), .O(z14));
  inv1   g141(.a(x34), .O(new_n193_));
  nand4  g142(.a(new_n173_), .b(new_n85_), .c(new_n92_), .d(new_n68_), .O(new_n194_));
  nand4  g143(.a(new_n186_), .b(new_n165_), .c(new_n164_), .d(new_n127_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n194_), .c(new_n93_), .O(new_n196_));
  aoi21  g145(.a(new_n196_), .b(new_n62_), .c(new_n193_), .O(new_n197_));
  inv1   g146(.a(new_n110_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n77_), .O(new_n199_));
  nor3   g148(.a(x34), .b(x33), .c(x32), .O(new_n200_));
  nand3  g149(.a(new_n200_), .b(new_n185_), .c(new_n165_), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n199_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n197_), .c(x16), .O(new_n203_));
  inv1   g152(.a(x00), .O(new_n204_));
  aoi21  g153(.a(new_n56_), .b(new_n204_), .c(x18), .O(new_n205_));
  aoi21  g154(.a(new_n205_), .b(new_n203_), .c(new_n52_), .O(z15));
endmodule


