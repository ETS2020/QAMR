// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_;
  inv1   g000(.a(x10), .O(new_n52_));
  nor2   g001(.a(x13), .b(new_n52_), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  oai21  g011(.a(x13), .b(new_n52_), .c(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n63_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n57_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(x16), .c(x18), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  nand3  g025(.a(new_n60_), .b(new_n76_), .c(new_n52_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n53_), .c(new_n77_), .O(z02));
  nand2  g027(.a(x22), .b(x21), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n64_), .c(new_n54_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n66_), .b(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n63_), .O(z03));
  inv1   g037(.a(x23), .O(new_n89_));
  oai21  g038(.a(x22), .b(x21), .c(new_n55_), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(new_n66_), .c(new_n89_), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n66_), .c(new_n71_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n91_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n60_), .b(new_n96_), .c(x18), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n53_), .O(z04));
  oai21  g047(.a(x18), .b(new_n60_), .c(new_n52_), .O(new_n99_));
  nor2   g048(.a(x23), .b(x21), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(x19), .O(new_n101_));
  nand3  g050(.a(new_n57_), .b(new_n83_), .c(new_n64_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n101_), .c(x24), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n80_), .c(new_n66_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x16), .O(new_n107_));
  aoi21  g056(.a(x18), .b(x13), .c(new_n53_), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n107_), .c(new_n99_), .O(z05));
  nor2   g058(.a(x24), .b(x22), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n57_), .c(new_n64_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n101_), .c(x25), .O(new_n112_));
  nor2   g061(.a(x25), .b(x24), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n92_), .c(new_n72_), .d(new_n57_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  nor2   g065(.a(x16), .b(x09), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n53_), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n116_), .c(new_n63_), .O(z06));
  nand3  g068(.a(new_n113_), .b(new_n66_), .c(new_n83_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n101_), .c(x26), .O(new_n121_));
  nor3   g070(.a(x26), .b(x25), .c(x24), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n92_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n73_), .c(new_n121_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x16), .O(new_n125_));
  nor2   g074(.a(x16), .b(x08), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n53_), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n125_), .c(new_n63_), .O(z07));
  inv1   g077(.a(x27), .O(new_n129_));
  inv1   g078(.a(x24), .O(new_n130_));
  nor2   g079(.a(x26), .b(x25), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n92_), .c(new_n130_), .d(new_n71_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n55_), .O(new_n133_));
  aoi21  g082(.a(new_n133_), .b(new_n66_), .c(new_n129_), .O(new_n134_));
  nand3  g083(.a(new_n80_), .b(new_n57_), .c(new_n64_), .O(new_n135_));
  inv1   g084(.a(x25), .O(new_n136_));
  nor2   g085(.a(x27), .b(x26), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n104_), .c(new_n136_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n134_), .c(x16), .O(new_n140_));
  inv1   g089(.a(x07), .O(new_n141_));
  aoi21  g090(.a(new_n60_), .b(new_n141_), .c(x18), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n140_), .c(new_n53_), .O(z08));
  nor2   g092(.a(x27), .b(x23), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n71_), .c(x19), .O(new_n145_));
  nand3  g094(.a(new_n131_), .b(new_n110_), .c(new_n66_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n145_), .c(x28), .O(new_n147_));
  inv1   g096(.a(x26), .O(new_n148_));
  nor2   g097(.a(x28), .b(x27), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n113_), .c(new_n148_), .d(new_n89_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n135_), .c(new_n147_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  nor2   g101(.a(x16), .b(x06), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n53_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(new_n63_), .O(z09));
  inv1   g104(.a(x29), .O(new_n156_));
  nand3  g105(.a(new_n64_), .b(new_n55_), .c(new_n54_), .O(new_n157_));
  nand4  g106(.a(new_n149_), .b(new_n131_), .c(new_n104_), .d(new_n80_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n55_), .c(new_n157_), .O(new_n159_));
  nor2   g108(.a(x29), .b(x28), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n122_), .c(new_n129_), .O(new_n161_));
  oai22  g110(.a(new_n161_), .b(new_n93_), .c(new_n159_), .d(new_n156_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  inv1   g112(.a(x05), .O(new_n164_));
  aoi21  g113(.a(new_n60_), .b(new_n164_), .c(x18), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n163_), .c(new_n53_), .O(z10));
  inv1   g115(.a(x30), .O(new_n167_));
  nand2  g116(.a(new_n104_), .b(new_n80_), .O(new_n168_));
  nand3  g117(.a(new_n160_), .b(new_n131_), .c(new_n129_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n168_), .c(new_n55_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n66_), .c(new_n167_), .O(new_n171_));
  nor2   g120(.a(x30), .b(x29), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n149_), .c(new_n122_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n93_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n171_), .c(x16), .O(new_n175_));
  inv1   g124(.a(x04), .O(new_n176_));
  aoi21  g125(.a(new_n60_), .b(new_n176_), .c(x18), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n175_), .c(new_n53_), .O(z11));
  inv1   g127(.a(x31), .O(new_n179_));
  nand3  g128(.a(new_n172_), .b(new_n100_), .c(new_n129_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n55_), .O(new_n181_));
  inv1   g130(.a(x28), .O(new_n182_));
  nand3  g131(.a(new_n113_), .b(new_n182_), .c(new_n148_), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n102_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n181_), .c(new_n179_), .O(new_n185_));
  nor2   g134(.a(x31), .b(x30), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n160_), .c(new_n137_), .d(new_n136_), .O(new_n187_));
  nor2   g136(.a(new_n187_), .b(new_n105_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n185_), .c(x16), .O(new_n189_));
  nor2   g138(.a(x16), .b(x03), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n53_), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(new_n189_), .c(new_n63_), .O(z12));
  inv1   g141(.a(x32), .O(new_n193_));
  nand4  g142(.a(new_n113_), .b(new_n179_), .c(new_n182_), .d(new_n148_), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n102_), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n181_), .c(new_n193_), .O(new_n196_));
  nor2   g145(.a(x32), .b(x31), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n172_), .c(new_n149_), .d(new_n131_), .O(new_n198_));
  nor2   g147(.a(new_n198_), .b(new_n105_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n196_), .c(x16), .O(new_n200_));
  nor2   g149(.a(x16), .b(x02), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n53_), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n200_), .c(new_n63_), .O(z13));
  inv1   g152(.a(x33), .O(new_n204_));
  nand3  g153(.a(new_n197_), .b(new_n131_), .c(new_n182_), .O(new_n205_));
  nor2   g154(.a(new_n205_), .b(new_n111_), .O(new_n206_));
  aoi21  g155(.a(new_n206_), .b(new_n181_), .c(new_n204_), .O(new_n207_));
  nand2  g156(.a(new_n160_), .b(new_n137_), .O(new_n208_));
  nor2   g157(.a(x33), .b(x32), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n186_), .O(new_n210_));
  nor3   g159(.a(new_n210_), .b(new_n208_), .c(new_n114_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n207_), .c(x16), .O(new_n212_));
  nor2   g161(.a(x16), .b(x01), .O(new_n213_));
  nor2   g162(.a(new_n213_), .b(new_n53_), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(new_n212_), .c(new_n63_), .O(z14));
  inv1   g164(.a(x34), .O(new_n216_));
  nand4  g165(.a(new_n209_), .b(new_n186_), .c(new_n160_), .d(new_n129_), .O(new_n217_));
  oai21  g166(.a(new_n217_), .b(new_n132_), .c(new_n55_), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n66_), .c(new_n216_), .O(new_n219_));
  nand4  g168(.a(new_n186_), .b(new_n216_), .c(new_n204_), .d(new_n193_), .O(new_n220_));
  nor3   g169(.a(new_n220_), .b(new_n208_), .c(new_n114_), .O(new_n221_));
  oai21  g170(.a(new_n221_), .b(new_n219_), .c(x16), .O(new_n222_));
  inv1   g171(.a(x00), .O(new_n223_));
  aoi21  g172(.a(new_n60_), .b(new_n223_), .c(x18), .O(new_n224_));
  aoi21  g173(.a(new_n224_), .b(new_n222_), .c(new_n53_), .O(z15));
endmodule


