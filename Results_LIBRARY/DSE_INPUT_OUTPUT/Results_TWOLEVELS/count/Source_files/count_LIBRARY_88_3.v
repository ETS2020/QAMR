// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x05), .O(new_n57_));
  inv1   g006(.a(x11), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nor2   g008(.a(new_n58_), .b(new_n57_), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n60_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n59_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  inv1   g025(.a(x16), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n76_), .c(x18), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n75_), .c(new_n60_), .O(z02));
  nand2  g028(.a(x22), .b(x21), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n64_), .c(new_n52_), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n80_), .c(x19), .O(new_n83_));
  inv1   g032(.a(x22), .O(new_n84_));
  nor2   g033(.a(new_n66_), .b(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n83_), .c(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n77_), .b(new_n87_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n60_), .O(z03));
  inv1   g038(.a(x18), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x16), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n58_), .O(new_n92_));
  inv1   g041(.a(x23), .O(new_n93_));
  inv1   g042(.a(new_n81_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n53_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n66_), .c(new_n93_), .O(new_n96_));
  nand3  g045(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n97_));
  nor2   g046(.a(x23), .b(x22), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n71_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n96_), .c(x16), .O(new_n101_));
  aoi21  g050(.a(x18), .b(new_n57_), .c(new_n60_), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n101_), .c(new_n92_), .O(z04));
  inv1   g052(.a(x24), .O(new_n104_));
  nand2  g053(.a(new_n99_), .b(new_n53_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n66_), .c(new_n104_), .O(new_n106_));
  nand4  g055(.a(new_n81_), .b(new_n66_), .c(new_n104_), .d(new_n93_), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n106_), .c(x16), .O(new_n109_));
  inv1   g058(.a(x10), .O(new_n110_));
  aoi21  g059(.a(new_n77_), .b(new_n110_), .c(x18), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n109_), .c(new_n60_), .O(z05));
  inv1   g061(.a(x25), .O(new_n113_));
  nand3  g062(.a(new_n55_), .b(new_n104_), .c(new_n64_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n105_), .c(new_n113_), .O(new_n116_));
  nor2   g065(.a(x25), .b(x24), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n98_), .c(new_n72_), .d(new_n55_), .O(new_n118_));
  inv1   g067(.a(new_n118_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  nor2   g069(.a(x16), .b(x09), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n60_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n120_), .c(new_n59_), .O(z06));
  inv1   g072(.a(x26), .O(new_n124_));
  nand3  g073(.a(new_n81_), .b(new_n113_), .c(new_n93_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n53_), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n115_), .c(new_n124_), .O(new_n127_));
  nor3   g076(.a(x26), .b(x25), .c(x24), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n98_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n73_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n127_), .c(x16), .O(new_n131_));
  nor2   g080(.a(x16), .b(x08), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n60_), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n131_), .c(new_n59_), .O(z07));
  inv1   g083(.a(x27), .O(new_n135_));
  nor2   g084(.a(x26), .b(x25), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n81_), .c(new_n93_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n53_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n115_), .c(new_n135_), .O(new_n139_));
  nand2  g088(.a(new_n104_), .b(new_n93_), .O(new_n140_));
  nand3  g089(.a(new_n81_), .b(new_n55_), .c(new_n64_), .O(new_n141_));
  nor2   g090(.a(x27), .b(x26), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n113_), .O(new_n143_));
  nor3   g092(.a(new_n143_), .b(new_n141_), .c(new_n140_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n139_), .c(x16), .O(new_n145_));
  nor2   g094(.a(x16), .b(x07), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n60_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n145_), .c(new_n59_), .O(z08));
  inv1   g097(.a(x28), .O(new_n149_));
  oai21  g098(.a(new_n143_), .b(new_n99_), .c(new_n53_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n115_), .c(new_n149_), .O(new_n151_));
  nor2   g100(.a(x28), .b(x27), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n117_), .c(new_n124_), .d(new_n93_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n141_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n151_), .c(x16), .O(new_n155_));
  nor2   g104(.a(x16), .b(x06), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n60_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n155_), .c(new_n59_), .O(z09));
  oai21  g107(.a(new_n77_), .b(x11), .c(x05), .O(new_n159_));
  inv1   g108(.a(x29), .O(new_n160_));
  nand2  g109(.a(new_n152_), .b(new_n136_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n99_), .c(new_n53_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n115_), .c(new_n160_), .O(new_n163_));
  inv1   g112(.a(new_n100_), .O(new_n164_));
  nor2   g113(.a(x29), .b(x28), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n128_), .c(new_n135_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n164_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n163_), .c(new_n159_), .O(new_n168_));
  aoi22  g117(.a(new_n91_), .b(new_n57_), .c(x18), .d(new_n58_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n168_), .O(z10));
  inv1   g119(.a(x30), .O(new_n171_));
  inv1   g120(.a(new_n55_), .O(new_n172_));
  nand3  g121(.a(new_n160_), .b(new_n104_), .c(new_n64_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n162_), .c(new_n171_), .O(new_n175_));
  nor2   g124(.a(x30), .b(x29), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n152_), .c(new_n128_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n164_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n175_), .c(x16), .O(new_n179_));
  nor2   g128(.a(x16), .b(x04), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n60_), .O(new_n181_));
  nand3  g130(.a(new_n181_), .b(new_n179_), .c(new_n59_), .O(z11));
  inv1   g131(.a(x31), .O(new_n183_));
  nand3  g132(.a(new_n142_), .b(new_n171_), .c(new_n149_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n125_), .c(new_n53_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n174_), .c(new_n183_), .O(new_n186_));
  nor2   g135(.a(x31), .b(x30), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n165_), .c(new_n142_), .d(new_n113_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n107_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n186_), .c(x16), .O(new_n190_));
  inv1   g139(.a(x03), .O(new_n191_));
  aoi21  g140(.a(new_n77_), .b(new_n191_), .c(x18), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n190_), .c(new_n60_), .O(z12));
  inv1   g142(.a(x32), .O(new_n194_));
  nand3  g143(.a(new_n183_), .b(new_n160_), .c(new_n104_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n97_), .O(new_n196_));
  aoi21  g145(.a(new_n196_), .b(new_n185_), .c(new_n194_), .O(new_n197_));
  nand3  g146(.a(new_n176_), .b(new_n194_), .c(new_n183_), .O(new_n198_));
  nor3   g147(.a(new_n198_), .b(new_n161_), .c(new_n107_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n197_), .c(x16), .O(new_n200_));
  nor2   g149(.a(x16), .b(x02), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n60_), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n200_), .c(new_n59_), .O(z13));
  inv1   g152(.a(x33), .O(new_n204_));
  nor2   g153(.a(x32), .b(x30), .O(new_n205_));
  nand3  g154(.a(new_n205_), .b(new_n142_), .c(new_n149_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n125_), .c(new_n53_), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n196_), .c(new_n204_), .O(new_n208_));
  nand2  g157(.a(new_n165_), .b(new_n142_), .O(new_n209_));
  nand3  g158(.a(new_n187_), .b(new_n204_), .c(new_n194_), .O(new_n210_));
  nor3   g159(.a(new_n210_), .b(new_n209_), .c(new_n118_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n208_), .c(x16), .O(new_n212_));
  inv1   g161(.a(x01), .O(new_n213_));
  aoi21  g162(.a(new_n77_), .b(new_n213_), .c(x18), .O(new_n214_));
  aoi21  g163(.a(new_n214_), .b(new_n212_), .c(new_n60_), .O(z14));
  inv1   g164(.a(x34), .O(new_n216_));
  nand4  g165(.a(new_n204_), .b(new_n183_), .c(new_n160_), .d(new_n104_), .O(new_n217_));
  nor2   g166(.a(new_n217_), .b(new_n97_), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n207_), .c(new_n216_), .O(new_n219_));
  nand4  g168(.a(new_n187_), .b(new_n216_), .c(new_n204_), .d(new_n194_), .O(new_n220_));
  nor3   g169(.a(new_n220_), .b(new_n209_), .c(new_n118_), .O(new_n221_));
  oai21  g170(.a(new_n221_), .b(new_n219_), .c(x16), .O(new_n222_));
  nor2   g171(.a(x16), .b(x00), .O(new_n223_));
  nor2   g172(.a(new_n223_), .b(new_n60_), .O(new_n224_));
  nand3  g173(.a(new_n224_), .b(new_n222_), .c(new_n59_), .O(z15));
endmodule


