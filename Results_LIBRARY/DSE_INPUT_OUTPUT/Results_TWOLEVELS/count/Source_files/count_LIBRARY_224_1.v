// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n230_, new_n231_, new_n232_;
  inv1   g000(.a(x10), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x07), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  oai21  g011(.a(new_n52_), .b(x07), .c(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n55_), .c(new_n54_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n57_), .O(new_n74_));
  oai21  g023(.a(new_n67_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n60_), .b(new_n77_), .c(x18), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n76_), .c(new_n53_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(new_n74_), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n57_), .c(new_n64_), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n80_), .c(new_n83_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n63_), .O(z03));
  inv1   g037(.a(x23), .O(new_n89_));
  oai21  g038(.a(x22), .b(x21), .c(new_n55_), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(new_n67_), .c(new_n89_), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n72_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n91_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n60_), .b(new_n96_), .c(x18), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n53_), .O(z04));
  oai21  g047(.a(x18), .b(new_n60_), .c(new_n52_), .O(new_n99_));
  nand2  g048(.a(x24), .b(x23), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n73_), .c(new_n80_), .d(new_n54_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n100_), .c(x19), .O(new_n103_));
  and2   g052(.a(new_n83_), .b(x24), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n103_), .c(x16), .O(new_n105_));
  aoi21  g054(.a(x18), .b(x07), .c(new_n53_), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n105_), .c(new_n99_), .O(z05));
  inv1   g056(.a(x25), .O(new_n108_));
  nand2  g057(.a(new_n101_), .b(new_n82_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n55_), .O(new_n110_));
  aoi21  g059(.a(new_n110_), .b(new_n67_), .c(new_n108_), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n92_), .O(new_n113_));
  inv1   g062(.a(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n81_), .O(new_n115_));
  inv1   g064(.a(new_n115_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n111_), .c(x16), .O(new_n117_));
  inv1   g066(.a(x09), .O(new_n118_));
  aoi21  g067(.a(new_n60_), .b(new_n118_), .c(x18), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(new_n53_), .O(z06));
  inv1   g069(.a(x26), .O(new_n121_));
  nand3  g070(.a(new_n112_), .b(new_n82_), .c(new_n89_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n55_), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n67_), .c(new_n121_), .O(new_n124_));
  nor3   g073(.a(x26), .b(x25), .c(x24), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n92_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n74_), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n124_), .c(x16), .O(new_n128_));
  inv1   g077(.a(x08), .O(new_n129_));
  aoi21  g078(.a(new_n60_), .b(new_n129_), .c(x18), .O(new_n130_));
  aoi21  g079(.a(new_n130_), .b(new_n128_), .c(new_n53_), .O(z07));
  inv1   g080(.a(x27), .O(new_n132_));
  nor2   g081(.a(x24), .b(x22), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n72_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n55_), .c(new_n66_), .O(new_n135_));
  nor2   g084(.a(x27), .b(x26), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n101_), .c(new_n108_), .O(new_n137_));
  oai22  g086(.a(new_n137_), .b(new_n83_), .c(new_n135_), .d(new_n132_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(x16), .c(x18), .O(new_n139_));
  inv1   g088(.a(x07), .O(new_n140_));
  oai21  g089(.a(x19), .b(new_n140_), .c(x10), .O(new_n141_));
  nor2   g090(.a(x26), .b(x25), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n89_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n141_), .c(x27), .O(new_n144_));
  inv1   g093(.a(new_n144_), .O(new_n145_));
  nor3   g094(.a(x16), .b(x10), .c(x07), .O(new_n146_));
  aoi21  g095(.a(new_n145_), .b(x16), .c(new_n146_), .O(new_n147_));
  oai21  g096(.a(new_n139_), .b(new_n53_), .c(new_n147_), .O(z08));
  inv1   g097(.a(x28), .O(new_n149_));
  nor2   g098(.a(x25), .b(x23), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n136_), .c(x19), .O(new_n151_));
  inv1   g100(.a(new_n151_), .O(new_n152_));
  nor2   g101(.a(new_n134_), .b(new_n66_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n152_), .c(new_n149_), .O(new_n154_));
  nor2   g103(.a(x28), .b(x27), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n112_), .c(new_n121_), .d(new_n89_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n83_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n154_), .c(x16), .O(new_n158_));
  nor2   g107(.a(x16), .b(x06), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n53_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n158_), .c(new_n63_), .O(z09));
  inv1   g110(.a(x24), .O(new_n162_));
  nand4  g111(.a(new_n82_), .b(new_n67_), .c(new_n149_), .d(new_n162_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n151_), .c(x29), .O(new_n164_));
  nor2   g113(.a(x29), .b(x28), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n125_), .c(new_n94_), .d(new_n132_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n164_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x16), .O(new_n168_));
  nor2   g117(.a(x16), .b(x05), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n53_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n168_), .c(new_n63_), .O(z10));
  inv1   g120(.a(x30), .O(new_n172_));
  nand3  g121(.a(new_n165_), .b(new_n142_), .c(new_n132_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n109_), .c(new_n55_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n67_), .c(new_n172_), .O(new_n175_));
  nor2   g124(.a(x30), .b(x29), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n155_), .c(new_n125_), .O(new_n177_));
  nor3   g126(.a(new_n177_), .b(new_n93_), .c(new_n66_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n175_), .c(x16), .O(new_n179_));
  inv1   g128(.a(x04), .O(new_n180_));
  aoi21  g129(.a(new_n60_), .b(new_n180_), .c(x18), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n179_), .c(new_n53_), .O(z11));
  inv1   g131(.a(x31), .O(new_n183_));
  nand3  g132(.a(new_n176_), .b(new_n136_), .c(new_n149_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n122_), .c(new_n55_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n67_), .c(new_n183_), .O(new_n186_));
  nand3  g135(.a(new_n101_), .b(new_n82_), .c(new_n67_), .O(new_n187_));
  nor2   g136(.a(x31), .b(x30), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n165_), .c(new_n136_), .d(new_n108_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n187_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n186_), .c(x16), .O(new_n191_));
  inv1   g140(.a(x03), .O(new_n192_));
  aoi21  g141(.a(new_n60_), .b(new_n192_), .c(x18), .O(new_n193_));
  aoi21  g142(.a(new_n193_), .b(new_n191_), .c(new_n53_), .O(z12));
  inv1   g143(.a(x32), .O(new_n195_));
  nand2  g144(.a(new_n188_), .b(new_n132_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n143_), .c(new_n55_), .O(new_n197_));
  nand2  g146(.a(new_n165_), .b(new_n133_), .O(new_n198_));
  nor2   g147(.a(new_n198_), .b(new_n74_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n197_), .c(new_n195_), .O(new_n200_));
  nor2   g149(.a(x32), .b(x31), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n176_), .c(new_n155_), .d(new_n142_), .O(new_n202_));
  nor2   g151(.a(new_n202_), .b(new_n187_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n200_), .c(x16), .O(new_n204_));
  nor2   g153(.a(x16), .b(x02), .O(new_n205_));
  nor2   g154(.a(new_n205_), .b(new_n53_), .O(new_n206_));
  nand3  g155(.a(new_n206_), .b(new_n204_), .c(new_n63_), .O(z13));
  inv1   g156(.a(x33), .O(new_n208_));
  nand4  g157(.a(new_n142_), .b(new_n92_), .c(new_n162_), .d(new_n72_), .O(new_n209_));
  nand4  g158(.a(new_n201_), .b(new_n165_), .c(new_n172_), .d(new_n132_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n209_), .c(new_n55_), .O(new_n211_));
  aoi21  g160(.a(new_n211_), .b(new_n67_), .c(new_n208_), .O(new_n212_));
  nor2   g161(.a(x33), .b(x32), .O(new_n213_));
  nand4  g162(.a(new_n213_), .b(new_n188_), .c(new_n165_), .d(new_n136_), .O(new_n214_));
  nor2   g163(.a(new_n214_), .b(new_n115_), .O(new_n215_));
  oai21  g164(.a(new_n215_), .b(new_n212_), .c(x16), .O(new_n216_));
  inv1   g165(.a(x01), .O(new_n217_));
  aoi21  g166(.a(new_n60_), .b(new_n217_), .c(x18), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n216_), .c(new_n53_), .O(z14));
  inv1   g168(.a(x34), .O(new_n220_));
  nand4  g169(.a(new_n208_), .b(new_n183_), .c(new_n172_), .d(new_n132_), .O(new_n221_));
  oai21  g170(.a(new_n221_), .b(new_n143_), .c(new_n55_), .O(new_n222_));
  nor2   g171(.a(x32), .b(x29), .O(new_n223_));
  nand3  g172(.a(new_n223_), .b(new_n133_), .c(new_n149_), .O(new_n224_));
  nor2   g173(.a(new_n224_), .b(new_n74_), .O(new_n225_));
  aoi21  g174(.a(new_n225_), .b(new_n222_), .c(new_n220_), .O(new_n226_));
  nor3   g175(.a(x34), .b(x33), .c(x32), .O(new_n227_));
  nand4  g176(.a(new_n227_), .b(new_n188_), .c(new_n165_), .d(new_n136_), .O(new_n228_));
  nor2   g177(.a(new_n228_), .b(new_n115_), .O(new_n229_));
  oai21  g178(.a(new_n229_), .b(new_n226_), .c(x16), .O(new_n230_));
  nor2   g179(.a(x16), .b(x00), .O(new_n231_));
  nor2   g180(.a(new_n231_), .b(new_n53_), .O(new_n232_));
  nand3  g181(.a(new_n232_), .b(new_n230_), .c(new_n63_), .O(z15));
endmodule


