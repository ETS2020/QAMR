// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x18), .b(x05), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(new_n60_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n58_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  oai21  g020(.a(new_n63_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  aoi21  g022(.a(new_n72_), .b(x16), .c(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(x18), .c(new_n58_), .O(z02));
  oai21  g024(.a(new_n70_), .b(x18), .c(new_n53_), .O(new_n76_));
  inv1   g025(.a(x20), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n54_), .O(new_n80_));
  aoi21  g029(.a(new_n76_), .b(x22), .c(new_n80_), .O(new_n81_));
  inv1   g030(.a(x12), .O(new_n82_));
  aoi21  g031(.a(new_n61_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n61_), .c(new_n83_), .O(z03));
  inv1   g033(.a(new_n79_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(x18), .c(new_n53_), .O(new_n86_));
  nor3   g035(.a(x23), .b(x22), .c(x21), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n64_), .O(new_n89_));
  aoi21  g038(.a(new_n86_), .b(x23), .c(new_n89_), .O(new_n90_));
  inv1   g039(.a(x11), .O(new_n91_));
  aoi21  g040(.a(new_n61_), .b(new_n91_), .c(x18), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n61_), .c(new_n92_), .O(z04));
  inv1   g042(.a(x24), .O(new_n94_));
  inv1   g043(.a(x19), .O(new_n95_));
  aoi21  g044(.a(new_n88_), .b(new_n95_), .c(new_n64_), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n78_), .c(new_n63_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n94_), .c(new_n98_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x10), .O(new_n100_));
  aoi21  g049(.a(new_n99_), .b(x16), .c(new_n100_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(x18), .c(new_n58_), .O(z05));
  aoi21  g051(.a(new_n97_), .b(new_n78_), .c(x19), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n64_), .c(x25), .O(new_n104_));
  nor2   g053(.a(x23), .b(x22), .O(new_n105_));
  nor2   g054(.a(x25), .b(x24), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n105_), .c(new_n70_), .d(new_n53_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  aoi21  g058(.a(new_n108_), .b(x16), .c(new_n109_), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(x18), .c(new_n58_), .O(z06));
  inv1   g060(.a(x26), .O(new_n112_));
  nor3   g061(.a(x25), .b(x24), .c(x23), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n85_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n60_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n53_), .c(new_n112_), .O(new_n116_));
  nor3   g065(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n105_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n71_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  inv1   g069(.a(x08), .O(new_n121_));
  aoi21  g070(.a(new_n61_), .b(new_n121_), .c(x18), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n120_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  nor2   g073(.a(x26), .b(x25), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n97_), .c(new_n85_), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n60_), .c(new_n54_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n97_), .c(new_n80_), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n124_), .c(new_n129_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g080(.a(x07), .O(new_n132_));
  aoi21  g081(.a(new_n61_), .b(new_n132_), .c(x18), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(z08));
  inv1   g083(.a(x28), .O(new_n135_));
  nor2   g084(.a(x27), .b(x26), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n106_), .c(new_n87_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n95_), .c(new_n64_), .O(new_n138_));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n113_), .c(new_n80_), .d(new_n112_), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n135_), .c(new_n140_), .O(new_n141_));
  nor2   g090(.a(x16), .b(x06), .O(new_n142_));
  aoi21  g091(.a(new_n141_), .b(x16), .c(new_n142_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(x18), .c(new_n58_), .O(z09));
  nand2  g093(.a(new_n140_), .b(x29), .O(new_n145_));
  nor3   g094(.a(x29), .b(x28), .c(x27), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n117_), .c(new_n89_), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n145_), .c(new_n61_), .O(new_n148_));
  nor2   g097(.a(x16), .b(x05), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n148_), .c(new_n60_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n58_), .O(z10));
  inv1   g100(.a(x30), .O(new_n152_));
  inv1   g101(.a(x22), .O(new_n153_));
  nand3  g102(.a(new_n97_), .b(new_n70_), .c(new_n153_), .O(new_n154_));
  nand2  g103(.a(new_n146_), .b(new_n125_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n154_), .c(new_n60_), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n53_), .c(new_n152_), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n139_), .c(new_n117_), .O(new_n159_));
  nor3   g108(.a(new_n159_), .b(new_n88_), .c(new_n64_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n157_), .c(x16), .O(new_n161_));
  inv1   g110(.a(x04), .O(new_n162_));
  aoi21  g111(.a(new_n61_), .b(new_n162_), .c(x18), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n161_), .O(z11));
  inv1   g113(.a(x31), .O(new_n165_));
  nor3   g114(.a(x30), .b(x29), .c(x28), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n136_), .c(new_n113_), .d(new_n78_), .O(new_n167_));
  aoi21  g116(.a(new_n167_), .b(new_n95_), .c(new_n64_), .O(new_n168_));
  nor2   g117(.a(x29), .b(x28), .O(new_n169_));
  nor2   g118(.a(x31), .b(x30), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n169_), .c(new_n128_), .O(new_n171_));
  oai22  g120(.a(new_n171_), .b(new_n98_), .c(new_n168_), .d(new_n165_), .O(new_n172_));
  nor2   g121(.a(x16), .b(x03), .O(new_n173_));
  aoi21  g122(.a(new_n172_), .b(x16), .c(new_n173_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(x18), .c(new_n58_), .O(z12));
  inv1   g124(.a(x32), .O(new_n176_));
  inv1   g125(.a(x29), .O(new_n177_));
  nand4  g126(.a(new_n170_), .b(new_n139_), .c(new_n177_), .d(new_n112_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n114_), .c(new_n60_), .O(new_n179_));
  aoi21  g128(.a(new_n179_), .b(new_n53_), .c(new_n176_), .O(new_n180_));
  nor2   g129(.a(x32), .b(x31), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n158_), .c(new_n139_), .d(new_n125_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n98_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n180_), .c(x16), .O(new_n184_));
  inv1   g133(.a(x02), .O(new_n185_));
  aoi21  g134(.a(new_n61_), .b(new_n185_), .c(x18), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n184_), .O(z13));
  inv1   g136(.a(x33), .O(new_n188_));
  nor3   g137(.a(x32), .b(x31), .c(x30), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n146_), .c(new_n117_), .d(new_n87_), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n95_), .c(new_n64_), .O(new_n191_));
  nor2   g140(.a(x33), .b(x32), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n170_), .c(new_n169_), .d(new_n136_), .O(new_n193_));
  oai22  g142(.a(new_n193_), .b(new_n107_), .c(new_n191_), .d(new_n188_), .O(new_n194_));
  nor2   g143(.a(x16), .b(x01), .O(new_n195_));
  aoi21  g144(.a(new_n194_), .b(x16), .c(new_n195_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(x18), .c(new_n58_), .O(z14));
  inv1   g146(.a(x34), .O(new_n198_));
  nand4  g147(.a(new_n125_), .b(new_n105_), .c(new_n94_), .d(new_n69_), .O(new_n199_));
  nand4  g148(.a(new_n192_), .b(new_n170_), .c(new_n169_), .d(new_n124_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n199_), .c(new_n95_), .O(new_n201_));
  aoi21  g150(.a(new_n201_), .b(new_n63_), .c(new_n198_), .O(new_n202_));
  nand2  g151(.a(new_n169_), .b(new_n136_), .O(new_n203_));
  nand4  g152(.a(new_n170_), .b(new_n198_), .c(new_n188_), .d(new_n176_), .O(new_n204_));
  nor3   g153(.a(new_n204_), .b(new_n203_), .c(new_n107_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n202_), .c(x16), .O(new_n206_));
  inv1   g155(.a(x00), .O(new_n207_));
  nand2  g156(.a(new_n61_), .b(new_n207_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n206_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n60_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n58_), .O(z15));
endmodule


