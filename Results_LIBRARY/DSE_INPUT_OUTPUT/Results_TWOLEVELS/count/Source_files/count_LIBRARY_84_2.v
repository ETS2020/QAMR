// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x12), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x08), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x18), .O(new_n59_));
  inv1   g008(.a(new_n58_), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n60_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n59_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n60_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n59_), .O(z02));
  and2   g027(.a(x22), .b(x21), .O(new_n79_));
  nor2   g028(.a(x20), .b(x17), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  nand3  g031(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(x22), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(x19), .c(new_n84_), .O(new_n85_));
  aoi21  g034(.a(new_n85_), .b(x16), .c(x18), .O(new_n86_));
  inv1   g035(.a(x08), .O(new_n87_));
  inv1   g036(.a(x16), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n57_), .c(new_n87_), .O(new_n89_));
  oai21  g038(.a(new_n86_), .b(new_n60_), .c(new_n89_), .O(z03));
  nand2  g039(.a(x23), .b(x21), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n80_), .c(new_n71_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(x19), .O(new_n94_));
  inv1   g043(.a(x23), .O(new_n95_));
  nor2   g044(.a(x22), .b(x20), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n55_), .c(new_n95_), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n94_), .c(x16), .O(new_n98_));
  nor2   g047(.a(x16), .b(x11), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n60_), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n98_), .c(new_n59_), .O(z04));
  nand2  g050(.a(new_n96_), .b(new_n55_), .O(new_n102_));
  nor2   g051(.a(x23), .b(x21), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(x19), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n102_), .c(x24), .O(new_n105_));
  inv1   g054(.a(x24), .O(new_n106_));
  nand4  g055(.a(new_n81_), .b(new_n66_), .c(new_n106_), .d(new_n95_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x16), .O(new_n109_));
  inv1   g058(.a(x10), .O(new_n110_));
  aoi21  g059(.a(new_n88_), .b(new_n110_), .c(x18), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n109_), .c(new_n60_), .O(z05));
  inv1   g061(.a(x22), .O(new_n113_));
  nand4  g062(.a(new_n55_), .b(new_n106_), .c(new_n113_), .d(new_n64_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n104_), .c(x25), .O(new_n115_));
  nor2   g064(.a(x25), .b(x24), .O(new_n116_));
  nand4  g065(.a(new_n116_), .b(new_n92_), .c(new_n72_), .d(new_n55_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x16), .O(new_n119_));
  nor2   g068(.a(x16), .b(x09), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n60_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n119_), .c(new_n59_), .O(z06));
  aoi21  g071(.a(x16), .b(x12), .c(new_n87_), .O(new_n123_));
  nor2   g072(.a(x25), .b(x23), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n71_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n53_), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n55_), .c(new_n106_), .d(new_n64_), .O(new_n127_));
  nor2   g076(.a(x26), .b(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n92_), .c(new_n106_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n73_), .O(new_n130_));
  aoi21  g079(.a(new_n127_), .b(x26), .c(new_n130_), .O(new_n131_));
  inv1   g080(.a(x18), .O(new_n132_));
  nand3  g081(.a(x26), .b(x22), .c(x16), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  aoi22  g083(.a(new_n134_), .b(new_n58_), .c(new_n88_), .d(new_n87_), .O(new_n135_));
  oai21  g084(.a(new_n131_), .b(new_n123_), .c(new_n135_), .O(z07));
  inv1   g085(.a(x27), .O(new_n137_));
  nor2   g086(.a(x26), .b(x24), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n113_), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n83_), .O(new_n140_));
  aoi21  g089(.a(new_n140_), .b(new_n126_), .c(new_n137_), .O(new_n141_));
  nand2  g090(.a(new_n106_), .b(new_n95_), .O(new_n142_));
  nand3  g091(.a(new_n81_), .b(new_n55_), .c(new_n64_), .O(new_n143_));
  inv1   g092(.a(x25), .O(new_n144_));
  nor2   g093(.a(x27), .b(x26), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nor3   g095(.a(new_n146_), .b(new_n143_), .c(new_n142_), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n141_), .c(x16), .O(new_n148_));
  nor2   g097(.a(x16), .b(x07), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n60_), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n148_), .c(new_n59_), .O(z08));
  inv1   g100(.a(x28), .O(new_n152_));
  nand3  g101(.a(new_n103_), .b(new_n137_), .c(new_n144_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n53_), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n140_), .c(new_n152_), .O(new_n155_));
  inv1   g104(.a(x26), .O(new_n156_));
  nor2   g105(.a(x28), .b(x27), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n116_), .c(new_n156_), .d(new_n95_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n143_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n155_), .c(x16), .O(new_n160_));
  nor2   g109(.a(x16), .b(x06), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n60_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n160_), .c(new_n59_), .O(z09));
  inv1   g112(.a(x29), .O(new_n164_));
  nand3  g113(.a(new_n157_), .b(new_n103_), .c(new_n144_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n53_), .O(new_n166_));
  aoi21  g115(.a(new_n166_), .b(new_n140_), .c(new_n164_), .O(new_n167_));
  nand2  g116(.a(new_n128_), .b(new_n106_), .O(new_n168_));
  nand3  g117(.a(new_n92_), .b(new_n66_), .c(new_n71_), .O(new_n169_));
  nor2   g118(.a(x29), .b(x28), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n137_), .O(new_n171_));
  nor3   g120(.a(new_n171_), .b(new_n169_), .c(new_n168_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n167_), .c(x16), .O(new_n173_));
  nor2   g122(.a(x16), .b(x05), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n60_), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n173_), .c(new_n59_), .O(z10));
  inv1   g125(.a(x30), .O(new_n177_));
  oai21  g126(.a(new_n171_), .b(new_n125_), .c(new_n53_), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n140_), .c(new_n177_), .O(new_n179_));
  nor2   g128(.a(x30), .b(x29), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n157_), .O(new_n181_));
  nor3   g130(.a(new_n181_), .b(new_n169_), .c(new_n168_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n179_), .c(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x04), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n60_), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n183_), .c(new_n59_), .O(z11));
  inv1   g135(.a(x31), .O(new_n187_));
  nand4  g136(.a(new_n180_), .b(new_n157_), .c(new_n124_), .d(new_n71_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n53_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n140_), .c(new_n187_), .O(new_n190_));
  nor2   g139(.a(x31), .b(x30), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n170_), .c(new_n145_), .d(new_n144_), .O(new_n192_));
  nor2   g141(.a(new_n192_), .b(new_n107_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n190_), .c(x16), .O(new_n194_));
  inv1   g143(.a(x03), .O(new_n195_));
  aoi21  g144(.a(new_n88_), .b(new_n195_), .c(x18), .O(new_n196_));
  aoi21  g145(.a(new_n196_), .b(new_n194_), .c(new_n60_), .O(z12));
  inv1   g146(.a(x32), .O(new_n198_));
  nand4  g147(.a(new_n187_), .b(new_n156_), .c(new_n106_), .d(new_n113_), .O(new_n199_));
  nor2   g148(.a(new_n199_), .b(new_n83_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n189_), .c(new_n198_), .O(new_n201_));
  nor2   g150(.a(x32), .b(x31), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n180_), .c(new_n157_), .d(new_n128_), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n107_), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n201_), .c(x16), .O(new_n205_));
  inv1   g154(.a(x02), .O(new_n206_));
  aoi21  g155(.a(new_n88_), .b(new_n206_), .c(x18), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n205_), .c(new_n60_), .O(z13));
  inv1   g157(.a(x33), .O(new_n209_));
  nand2  g158(.a(new_n202_), .b(new_n138_), .O(new_n210_));
  nor2   g159(.a(new_n210_), .b(new_n102_), .O(new_n211_));
  aoi21  g160(.a(new_n211_), .b(new_n189_), .c(new_n209_), .O(new_n212_));
  nand2  g161(.a(new_n170_), .b(new_n145_), .O(new_n213_));
  nor2   g162(.a(x33), .b(x32), .O(new_n214_));
  nand2  g163(.a(new_n214_), .b(new_n191_), .O(new_n215_));
  nor3   g164(.a(new_n215_), .b(new_n213_), .c(new_n117_), .O(new_n216_));
  oai21  g165(.a(new_n216_), .b(new_n212_), .c(x16), .O(new_n217_));
  inv1   g166(.a(x01), .O(new_n218_));
  aoi21  g167(.a(new_n88_), .b(new_n218_), .c(x18), .O(new_n219_));
  aoi21  g168(.a(new_n219_), .b(new_n217_), .c(new_n60_), .O(z14));
  inv1   g169(.a(x34), .O(new_n221_));
  nand3  g170(.a(new_n214_), .b(new_n138_), .c(new_n187_), .O(new_n222_));
  nor2   g171(.a(new_n222_), .b(new_n102_), .O(new_n223_));
  aoi21  g172(.a(new_n223_), .b(new_n189_), .c(new_n221_), .O(new_n224_));
  nand4  g173(.a(new_n191_), .b(new_n221_), .c(new_n209_), .d(new_n198_), .O(new_n225_));
  nor3   g174(.a(new_n225_), .b(new_n213_), .c(new_n117_), .O(new_n226_));
  oai21  g175(.a(new_n226_), .b(new_n224_), .c(x16), .O(new_n227_));
  nor2   g176(.a(x16), .b(x00), .O(new_n228_));
  nor2   g177(.a(new_n228_), .b(new_n60_), .O(new_n229_));
  nand3  g178(.a(new_n229_), .b(new_n227_), .c(new_n59_), .O(z15));
endmodule


