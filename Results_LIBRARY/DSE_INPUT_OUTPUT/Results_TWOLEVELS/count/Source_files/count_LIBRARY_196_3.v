// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x14), .O(new_n58_));
  nand2  g007(.a(x18), .b(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n57_), .b(x18), .c(new_n59_), .O(z00));
  inv1   g009(.a(x16), .O(new_n61_));
  oai21  g010(.a(x18), .b(new_n61_), .c(new_n58_), .O(new_n62_));
  inv1   g011(.a(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  inv1   g013(.a(x17), .O(new_n65_));
  inv1   g014(.a(x19), .O(new_n66_));
  nand3  g015(.a(new_n64_), .b(new_n66_), .c(new_n65_), .O(new_n67_));
  oai21  g016(.a(new_n53_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n63_), .c(x16), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n62_), .O(z01));
  nand2  g019(.a(x20), .b(new_n63_), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n66_), .c(new_n65_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n53_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n61_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n61_), .c(new_n78_), .O(z02));
  oai21  g028(.a(new_n73_), .b(x18), .c(new_n53_), .O(new_n80_));
  nor3   g029(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  and2   g030(.a(new_n81_), .b(new_n53_), .O(new_n82_));
  aoi21  g031(.a(new_n80_), .b(x22), .c(new_n82_), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n61_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n61_), .c(new_n85_), .O(z03));
  oai21  g035(.a(new_n81_), .b(x18), .c(new_n53_), .O(new_n87_));
  inv1   g036(.a(x21), .O(new_n88_));
  nor2   g037(.a(x23), .b(x22), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n67_), .O(new_n91_));
  aoi21  g040(.a(new_n87_), .b(x23), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x11), .O(new_n93_));
  aoi21  g042(.a(new_n61_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n61_), .c(new_n94_), .O(z04));
  inv1   g044(.a(x24), .O(new_n96_));
  nand2  g045(.a(new_n89_), .b(new_n73_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n63_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n53_), .c(new_n96_), .O(new_n99_));
  nor2   g048(.a(x22), .b(x21), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n67_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n99_), .c(x16), .O(new_n104_));
  inv1   g053(.a(x10), .O(new_n105_));
  aoi21  g054(.a(new_n61_), .b(new_n105_), .c(x18), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n104_), .O(z05));
  inv1   g056(.a(x25), .O(new_n108_));
  inv1   g057(.a(x22), .O(new_n109_));
  nand3  g058(.a(new_n101_), .b(new_n73_), .c(new_n109_), .O(new_n110_));
  aoi21  g059(.a(new_n110_), .b(new_n63_), .c(new_n54_), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n89_), .c(new_n75_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n108_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g064(.a(x09), .O(new_n116_));
  aoi21  g065(.a(new_n61_), .b(new_n116_), .c(x18), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n115_), .O(z06));
  inv1   g067(.a(x26), .O(new_n119_));
  nor3   g068(.a(x25), .b(x24), .c(x23), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n100_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n66_), .c(new_n67_), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n89_), .c(new_n75_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  nor2   g074(.a(x16), .b(x08), .O(new_n126_));
  aoi21  g075(.a(new_n125_), .b(x16), .c(new_n126_), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(x18), .c(new_n59_), .O(z07));
  inv1   g077(.a(x27), .O(new_n129_));
  nand3  g078(.a(new_n123_), .b(new_n89_), .c(new_n88_), .O(new_n130_));
  aoi21  g079(.a(new_n130_), .b(new_n66_), .c(new_n67_), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n101_), .c(new_n82_), .O(new_n133_));
  oai21  g082(.a(new_n131_), .b(new_n129_), .c(new_n133_), .O(new_n134_));
  nor2   g083(.a(x16), .b(x07), .O(new_n135_));
  aoi21  g084(.a(new_n134_), .b(x16), .c(new_n135_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(x18), .c(new_n59_), .O(z08));
  inv1   g086(.a(x28), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n112_), .c(new_n89_), .d(new_n88_), .O(new_n140_));
  aoi21  g089(.a(new_n140_), .b(new_n66_), .c(new_n67_), .O(new_n141_));
  nor3   g090(.a(x28), .b(x27), .c(x26), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n120_), .c(new_n82_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n138_), .c(new_n143_), .O(new_n144_));
  nor2   g093(.a(x16), .b(x06), .O(new_n145_));
  aoi21  g094(.a(new_n144_), .b(x16), .c(new_n145_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(x18), .c(new_n59_), .O(z09));
  inv1   g096(.a(x29), .O(new_n148_));
  nor2   g097(.a(x26), .b(x25), .O(new_n149_));
  nor2   g098(.a(x28), .b(x27), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n149_), .c(new_n101_), .d(new_n100_), .O(new_n151_));
  aoi21  g100(.a(new_n151_), .b(new_n66_), .c(new_n67_), .O(new_n152_));
  nor3   g101(.a(x29), .b(x28), .c(x27), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n123_), .c(new_n91_), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n148_), .c(new_n154_), .O(new_n155_));
  nor2   g104(.a(x16), .b(x05), .O(new_n156_));
  aoi21  g105(.a(new_n155_), .b(x16), .c(new_n156_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(x18), .c(new_n59_), .O(z10));
  inv1   g107(.a(x30), .O(new_n159_));
  nand2  g108(.a(new_n153_), .b(new_n149_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n110_), .c(new_n63_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n53_), .c(new_n159_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n150_), .c(new_n123_), .O(new_n164_));
  nor3   g113(.a(new_n164_), .b(new_n90_), .c(new_n67_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n162_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n61_), .b(new_n167_), .c(x18), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z11));
  inv1   g118(.a(x31), .O(new_n170_));
  nand3  g119(.a(new_n163_), .b(new_n132_), .c(new_n138_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n110_), .c(new_n63_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n53_), .c(new_n170_), .O(new_n173_));
  nor2   g122(.a(x29), .b(x28), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n174_), .c(new_n132_), .O(new_n176_));
  nor3   g125(.a(new_n176_), .b(new_n102_), .c(new_n67_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n173_), .c(x16), .O(new_n178_));
  inv1   g127(.a(x03), .O(new_n179_));
  aoi21  g128(.a(new_n61_), .b(new_n179_), .c(x18), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n178_), .O(z12));
  inv1   g130(.a(x32), .O(new_n182_));
  nor3   g131(.a(x31), .b(x30), .c(x29), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n142_), .c(new_n120_), .d(new_n100_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n66_), .c(new_n67_), .O(new_n185_));
  nand3  g134(.a(new_n163_), .b(new_n182_), .c(new_n170_), .O(new_n186_));
  inv1   g135(.a(new_n186_), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n150_), .c(new_n149_), .d(new_n103_), .O(new_n188_));
  oai21  g137(.a(new_n185_), .b(new_n182_), .c(new_n188_), .O(new_n189_));
  nor2   g138(.a(x16), .b(x02), .O(new_n190_));
  aoi21  g139(.a(new_n189_), .b(x16), .c(new_n190_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(x18), .c(new_n59_), .O(z13));
  inv1   g141(.a(x33), .O(new_n193_));
  nand2  g142(.a(new_n120_), .b(new_n81_), .O(new_n194_));
  nand2  g143(.a(new_n187_), .b(new_n142_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n194_), .c(new_n63_), .O(new_n196_));
  aoi21  g145(.a(new_n196_), .b(new_n53_), .c(new_n193_), .O(new_n197_));
  nand2  g146(.a(new_n174_), .b(new_n139_), .O(new_n198_));
  nand3  g147(.a(new_n175_), .b(new_n193_), .c(new_n182_), .O(new_n199_));
  nor3   g148(.a(new_n199_), .b(new_n198_), .c(new_n113_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n197_), .c(x16), .O(new_n201_));
  inv1   g150(.a(x01), .O(new_n202_));
  aoi21  g151(.a(new_n61_), .b(new_n202_), .c(x18), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n201_), .O(z14));
  inv1   g153(.a(x34), .O(new_n205_));
  nand3  g154(.a(new_n149_), .b(new_n101_), .c(new_n81_), .O(new_n206_));
  inv1   g155(.a(new_n199_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n153_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n206_), .c(new_n63_), .O(new_n209_));
  aoi21  g158(.a(new_n209_), .b(new_n53_), .c(new_n205_), .O(new_n210_));
  nand4  g159(.a(new_n175_), .b(new_n205_), .c(new_n193_), .d(new_n182_), .O(new_n211_));
  nor3   g160(.a(new_n211_), .b(new_n198_), .c(new_n113_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n210_), .c(x16), .O(new_n213_));
  inv1   g162(.a(x00), .O(new_n214_));
  aoi21  g163(.a(new_n61_), .b(new_n214_), .c(x18), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n213_), .O(z15));
endmodule


