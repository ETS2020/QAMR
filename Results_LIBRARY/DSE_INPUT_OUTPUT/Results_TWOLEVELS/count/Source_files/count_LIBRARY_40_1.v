// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g008(.a(x30), .b(x18), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n55_), .b(new_n62_), .O(new_n63_));
  nand3  g012(.a(new_n62_), .b(new_n53_), .c(new_n52_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n63_), .c(x16), .O(new_n66_));
  inv1   g015(.a(new_n60_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n66_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nand3  g020(.a(new_n55_), .b(new_n71_), .c(new_n62_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n64_), .b(x21), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n67_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n58_), .c(new_n76_), .O(z02));
  nand2  g026(.a(x22), .b(x21), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n62_), .c(new_n52_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(x19), .O(new_n81_));
  and2   g030(.a(new_n64_), .b(x22), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n81_), .c(x16), .O(new_n83_));
  nor2   g032(.a(x16), .b(x12), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n83_), .O(z03));
  inv1   g035(.a(x18), .O(new_n87_));
  inv1   g036(.a(x30), .O(new_n88_));
  nor2   g037(.a(new_n79_), .b(x19), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n64_), .c(x23), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n71_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n64_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  aoi21  g043(.a(new_n94_), .b(new_n90_), .c(new_n58_), .O(new_n95_));
  nor2   g044(.a(x16), .b(x11), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n95_), .c(new_n88_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n87_), .O(z04));
  inv1   g047(.a(x24), .O(new_n99_));
  aoi21  g048(.a(new_n92_), .b(new_n53_), .c(new_n64_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n79_), .O(new_n102_));
  oai22  g051(.a(new_n102_), .b(new_n64_), .c(new_n100_), .d(new_n99_), .O(new_n103_));
  nor2   g052(.a(x16), .b(x10), .O(new_n104_));
  aoi21  g053(.a(new_n103_), .b(x16), .c(new_n104_), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(x30), .c(new_n87_), .O(z05));
  inv1   g055(.a(x25), .O(new_n107_));
  aoi21  g056(.a(new_n102_), .b(new_n53_), .c(new_n64_), .O(new_n108_));
  nor2   g057(.a(x25), .b(x24), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n91_), .c(new_n73_), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n107_), .c(new_n110_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x09), .O(new_n112_));
  aoi21  g061(.a(new_n111_), .b(x16), .c(new_n112_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(x30), .c(new_n87_), .O(z06));
  inv1   g063(.a(x26), .O(new_n115_));
  nor3   g064(.a(x25), .b(x24), .c(x23), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n79_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n53_), .c(new_n64_), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n91_), .c(new_n73_), .O(new_n120_));
  oai21  g069(.a(new_n118_), .b(new_n115_), .c(new_n120_), .O(new_n121_));
  nor2   g070(.a(x16), .b(x08), .O(new_n122_));
  aoi21  g071(.a(new_n121_), .b(x16), .c(new_n122_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(x30), .c(new_n87_), .O(z07));
  inv1   g073(.a(x27), .O(new_n125_));
  nor3   g074(.a(x23), .b(x22), .c(x21), .O(new_n126_));
  nand2  g075(.a(new_n119_), .b(new_n126_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n53_), .c(new_n64_), .O(new_n128_));
  nor3   g077(.a(x22), .b(x21), .c(x20), .O(new_n129_));
  and2   g078(.a(new_n129_), .b(new_n55_), .O(new_n130_));
  nor3   g079(.a(x27), .b(x26), .c(x25), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n130_), .c(new_n101_), .O(new_n132_));
  oai21  g081(.a(new_n128_), .b(new_n125_), .c(new_n132_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x07), .O(new_n134_));
  aoi21  g083(.a(new_n133_), .b(x16), .c(new_n134_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(x30), .c(new_n87_), .O(z08));
  inv1   g085(.a(x28), .O(new_n137_));
  nor2   g086(.a(x27), .b(x26), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n109_), .c(new_n126_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n53_), .c(new_n64_), .O(new_n140_));
  nor3   g089(.a(x28), .b(x27), .c(x26), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n130_), .c(new_n116_), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n137_), .c(new_n142_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  nor2   g093(.a(x16), .b(x06), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n67_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n144_), .O(z09));
  inv1   g096(.a(x29), .O(new_n148_));
  nor2   g097(.a(x26), .b(x25), .O(new_n149_));
  nor2   g098(.a(x28), .b(x27), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n102_), .c(new_n53_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n65_), .c(new_n148_), .O(new_n153_));
  nand4  g102(.a(new_n150_), .b(new_n119_), .c(new_n88_), .d(new_n148_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n94_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n153_), .c(x16), .O(new_n156_));
  nor2   g105(.a(x16), .b(x05), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n67_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n156_), .O(z10));
  nand3  g108(.a(new_n129_), .b(new_n55_), .c(x16), .O(new_n160_));
  nor2   g109(.a(x29), .b(x28), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n138_), .O(new_n162_));
  inv1   g111(.a(new_n162_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n116_), .O(new_n164_));
  oai22  g113(.a(new_n164_), .b(new_n160_), .c(x16), .d(x04), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n88_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n87_), .O(z11));
  inv1   g116(.a(x31), .O(new_n168_));
  nor3   g117(.a(x29), .b(x28), .c(x27), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n149_), .c(new_n101_), .d(new_n79_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n53_), .c(new_n64_), .O(new_n171_));
  nor2   g120(.a(x31), .b(x29), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n150_), .c(new_n119_), .d(new_n93_), .O(new_n173_));
  oai21  g122(.a(new_n171_), .b(new_n168_), .c(new_n173_), .O(new_n174_));
  nor2   g123(.a(x16), .b(x03), .O(new_n175_));
  aoi21  g124(.a(new_n174_), .b(x16), .c(new_n175_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(x30), .c(new_n87_), .O(z12));
  inv1   g126(.a(x32), .O(new_n178_));
  nor3   g127(.a(x31), .b(x29), .c(x28), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n138_), .c(new_n116_), .d(new_n79_), .O(new_n180_));
  aoi21  g129(.a(new_n180_), .b(new_n53_), .c(new_n64_), .O(new_n181_));
  nor2   g130(.a(new_n102_), .b(new_n64_), .O(new_n182_));
  nor2   g131(.a(x32), .b(x31), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n161_), .c(new_n131_), .d(new_n182_), .O(new_n184_));
  oai21  g133(.a(new_n181_), .b(new_n178_), .c(new_n184_), .O(new_n185_));
  nor2   g134(.a(x16), .b(x02), .O(new_n186_));
  aoi21  g135(.a(new_n185_), .b(x16), .c(new_n186_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(x30), .c(new_n87_), .O(z13));
  inv1   g137(.a(x33), .O(new_n189_));
  nor3   g138(.a(x32), .b(x31), .c(x29), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n141_), .c(new_n116_), .d(new_n79_), .O(new_n191_));
  aoi21  g140(.a(new_n191_), .b(new_n53_), .c(new_n64_), .O(new_n192_));
  inv1   g141(.a(new_n151_), .O(new_n193_));
  nor2   g142(.a(x33), .b(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n172_), .c(new_n193_), .d(new_n182_), .O(new_n195_));
  oai21  g144(.a(new_n192_), .b(new_n189_), .c(new_n195_), .O(new_n196_));
  nor2   g145(.a(x16), .b(x01), .O(new_n197_));
  aoi21  g146(.a(new_n196_), .b(x16), .c(new_n197_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(x30), .c(new_n87_), .O(z14));
  inv1   g148(.a(x34), .O(new_n200_));
  nand3  g149(.a(new_n194_), .b(new_n169_), .c(new_n168_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(new_n127_), .c(new_n53_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n65_), .c(new_n200_), .O(new_n203_));
  nand4  g152(.a(new_n183_), .b(new_n163_), .c(new_n200_), .d(new_n189_), .O(new_n204_));
  nor2   g153(.a(new_n204_), .b(new_n110_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n203_), .c(x16), .O(new_n206_));
  nor2   g155(.a(x16), .b(x00), .O(new_n207_));
  nor2   g156(.a(new_n207_), .b(new_n67_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n206_), .O(z15));
endmodule


