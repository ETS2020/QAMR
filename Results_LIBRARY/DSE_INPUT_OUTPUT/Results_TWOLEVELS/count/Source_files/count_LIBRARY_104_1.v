// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x10), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x03), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  inv1   g011(.a(new_n53_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x18), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(new_n57_), .b(new_n65_), .O(new_n66_));
  nor3   g015(.a(x20), .b(x19), .c(x17), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n57_), .O(new_n74_));
  oai21  g023(.a(new_n67_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n53_), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n76_), .c(new_n64_), .O(z02));
  nand2  g028(.a(x22), .b(x21), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n65_), .c(new_n54_), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n80_), .c(x19), .O(new_n83_));
  inv1   g032(.a(x22), .O(new_n84_));
  nor2   g033(.a(new_n67_), .b(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n83_), .c(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n60_), .b(new_n87_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n53_), .O(z03));
  inv1   g038(.a(x23), .O(new_n90_));
  oai21  g039(.a(x22), .b(x21), .c(new_n55_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n67_), .c(new_n90_), .O(new_n92_));
  nand3  g041(.a(new_n65_), .b(new_n55_), .c(new_n54_), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n72_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n92_), .c(x16), .O(new_n97_));
  inv1   g046(.a(x11), .O(new_n98_));
  aoi21  g047(.a(new_n60_), .b(new_n98_), .c(x18), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n97_), .c(new_n53_), .O(z04));
  aoi21  g049(.a(x16), .b(x03), .c(new_n52_), .O(new_n101_));
  aoi21  g050(.a(new_n94_), .b(new_n72_), .c(x19), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n93_), .c(x24), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n81_), .c(new_n67_), .O(new_n105_));
  and2   g054(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  inv1   g055(.a(x18), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x16), .O(new_n108_));
  aoi22  g057(.a(new_n108_), .b(new_n52_), .c(x18), .d(x03), .O(new_n109_));
  oai21  g058(.a(new_n106_), .b(new_n101_), .c(new_n109_), .O(z05));
  nand2  g059(.a(new_n105_), .b(x25), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n94_), .c(new_n73_), .d(new_n57_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  nor2   g064(.a(x16), .b(x09), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n53_), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n115_), .c(new_n64_), .O(z06));
  nand2  g067(.a(new_n113_), .b(x26), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n94_), .c(new_n73_), .d(new_n57_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  nor2   g072(.a(x16), .b(x08), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n53_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n123_), .c(new_n64_), .O(z07));
  nand2  g075(.a(new_n121_), .b(x27), .O(new_n127_));
  nand3  g076(.a(new_n81_), .b(new_n57_), .c(new_n65_), .O(new_n128_));
  inv1   g077(.a(x25), .O(new_n129_));
  nor2   g078(.a(x27), .b(x26), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n104_), .c(new_n129_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n128_), .c(new_n127_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(x16), .O(new_n133_));
  nor2   g082(.a(x16), .b(x07), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n53_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n133_), .c(new_n64_), .O(z08));
  inv1   g085(.a(x28), .O(new_n137_));
  nand2  g086(.a(new_n130_), .b(new_n112_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n95_), .c(new_n55_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n67_), .c(new_n137_), .O(new_n140_));
  inv1   g089(.a(x26), .O(new_n141_));
  nor2   g090(.a(x28), .b(x27), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n112_), .c(new_n141_), .d(new_n90_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n128_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n140_), .c(x16), .O(new_n145_));
  inv1   g094(.a(x06), .O(new_n146_));
  aoi21  g095(.a(new_n60_), .b(new_n146_), .c(x18), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n145_), .c(new_n53_), .O(z09));
  inv1   g097(.a(x29), .O(new_n149_));
  nor3   g098(.a(x29), .b(x28), .c(x27), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n120_), .c(new_n96_), .O(new_n151_));
  oai21  g100(.a(new_n144_), .b(new_n149_), .c(new_n151_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  nor2   g102(.a(x16), .b(x05), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n53_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n153_), .c(new_n64_), .O(z10));
  nand2  g105(.a(new_n151_), .b(x30), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n142_), .c(new_n120_), .d(new_n96_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(x16), .O(new_n161_));
  nor2   g110(.a(x16), .b(x04), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n53_), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n161_), .c(new_n64_), .O(z11));
  nand2  g113(.a(new_n93_), .b(x31), .O(new_n165_));
  nor2   g114(.a(x29), .b(x28), .O(new_n166_));
  nor2   g115(.a(x31), .b(x30), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n166_), .c(new_n130_), .d(new_n129_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n105_), .c(new_n165_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n107_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n63_), .O(new_n172_));
  inv1   g121(.a(x31), .O(new_n173_));
  aoi21  g122(.a(new_n55_), .b(x03), .c(new_n52_), .O(new_n174_));
  nand3  g123(.a(new_n112_), .b(new_n81_), .c(new_n90_), .O(new_n175_));
  nand3  g124(.a(new_n158_), .b(new_n130_), .c(new_n137_), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  nor3   g126(.a(new_n177_), .b(new_n174_), .c(new_n173_), .O(new_n178_));
  nor3   g127(.a(x16), .b(x10), .c(x03), .O(new_n179_));
  aoi21  g128(.a(new_n178_), .b(x16), .c(new_n179_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n172_), .O(z12));
  inv1   g130(.a(x32), .O(new_n182_));
  nand4  g131(.a(new_n167_), .b(new_n142_), .c(new_n149_), .d(new_n141_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n175_), .c(new_n55_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n67_), .c(new_n182_), .O(new_n185_));
  nor2   g134(.a(x26), .b(x25), .O(new_n186_));
  nor2   g135(.a(x32), .b(x31), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n158_), .c(new_n142_), .d(new_n186_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n105_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n185_), .c(x16), .O(new_n190_));
  inv1   g139(.a(x02), .O(new_n191_));
  aoi21  g140(.a(new_n60_), .b(new_n191_), .c(x18), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n190_), .c(new_n53_), .O(z13));
  inv1   g142(.a(x33), .O(new_n194_));
  inv1   g143(.a(x24), .O(new_n195_));
  nand4  g144(.a(new_n186_), .b(new_n94_), .c(new_n195_), .d(new_n72_), .O(new_n196_));
  inv1   g145(.a(x27), .O(new_n197_));
  inv1   g146(.a(x30), .O(new_n198_));
  nand4  g147(.a(new_n187_), .b(new_n166_), .c(new_n198_), .d(new_n197_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n196_), .c(new_n55_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n67_), .c(new_n194_), .O(new_n201_));
  nand2  g150(.a(new_n166_), .b(new_n130_), .O(new_n202_));
  nor2   g151(.a(x33), .b(x32), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n167_), .O(new_n204_));
  nor3   g153(.a(new_n204_), .b(new_n202_), .c(new_n113_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n201_), .c(x16), .O(new_n206_));
  inv1   g155(.a(x01), .O(new_n207_));
  aoi21  g156(.a(new_n60_), .b(new_n207_), .c(x18), .O(new_n208_));
  aoi21  g157(.a(new_n208_), .b(new_n206_), .c(new_n53_), .O(z14));
  inv1   g158(.a(x34), .O(new_n210_));
  nand4  g159(.a(new_n203_), .b(new_n167_), .c(new_n166_), .d(new_n197_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n196_), .c(new_n55_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n67_), .c(new_n210_), .O(new_n213_));
  nand4  g162(.a(new_n167_), .b(new_n210_), .c(new_n194_), .d(new_n182_), .O(new_n214_));
  nor3   g163(.a(new_n214_), .b(new_n202_), .c(new_n113_), .O(new_n215_));
  oai21  g164(.a(new_n215_), .b(new_n213_), .c(x16), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  aoi21  g166(.a(new_n60_), .b(new_n217_), .c(x18), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n216_), .c(new_n53_), .O(z15));
endmodule


