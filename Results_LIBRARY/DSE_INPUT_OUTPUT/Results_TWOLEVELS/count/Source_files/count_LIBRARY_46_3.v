// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x05), .O(new_n58_));
  nand2  g007(.a(x18), .b(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n57_), .b(x18), .c(new_n59_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x16), .O(new_n62_));
  nand2  g011(.a(new_n54_), .b(x20), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n59_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  oai21  g021(.a(new_n64_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  aoi21  g023(.a(new_n73_), .b(x16), .c(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(x18), .c(new_n59_), .O(z02));
  oai21  g025(.a(new_n71_), .b(x18), .c(new_n53_), .O(new_n77_));
  nor3   g026(.a(x22), .b(x21), .c(x20), .O(new_n78_));
  and2   g027(.a(new_n78_), .b(new_n53_), .O(new_n79_));
  aoi21  g028(.a(new_n77_), .b(x22), .c(new_n79_), .O(new_n80_));
  inv1   g029(.a(x12), .O(new_n81_));
  aoi21  g030(.a(new_n62_), .b(new_n81_), .c(x18), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n62_), .c(new_n82_), .O(z03));
  oai21  g032(.a(new_n78_), .b(x18), .c(new_n53_), .O(new_n84_));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  inv1   g034(.a(new_n85_), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n65_), .O(new_n87_));
  aoi21  g036(.a(new_n84_), .b(x23), .c(new_n87_), .O(new_n88_));
  inv1   g037(.a(x11), .O(new_n89_));
  aoi21  g038(.a(new_n62_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n62_), .c(new_n90_), .O(z04));
  inv1   g040(.a(x24), .O(new_n92_));
  inv1   g041(.a(x19), .O(new_n93_));
  aoi21  g042(.a(new_n86_), .b(new_n93_), .c(new_n65_), .O(new_n94_));
  nor2   g043(.a(x22), .b(x21), .O(new_n95_));
  nor2   g044(.a(x24), .b(x23), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n64_), .O(new_n99_));
  oai21  g048(.a(new_n94_), .b(new_n92_), .c(new_n99_), .O(new_n100_));
  nor2   g049(.a(x16), .b(x10), .O(new_n101_));
  aoi21  g050(.a(new_n100_), .b(x16), .c(new_n101_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(x18), .c(new_n59_), .O(z05));
  inv1   g052(.a(x25), .O(new_n104_));
  aoi21  g053(.a(new_n97_), .b(new_n93_), .c(new_n65_), .O(new_n105_));
  nor2   g054(.a(x23), .b(x22), .O(new_n106_));
  nor2   g055(.a(x25), .b(x24), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n106_), .c(new_n71_), .d(new_n53_), .O(new_n108_));
  oai21  g057(.a(new_n105_), .b(new_n104_), .c(new_n108_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x09), .O(new_n110_));
  aoi21  g059(.a(new_n109_), .b(x16), .c(new_n110_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(x18), .c(new_n59_), .O(z06));
  inv1   g061(.a(x26), .O(new_n113_));
  nor3   g062(.a(x25), .b(x24), .c(x23), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n78_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n61_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n53_), .c(new_n113_), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n106_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n72_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  inv1   g070(.a(x08), .O(new_n122_));
  aoi21  g071(.a(new_n62_), .b(new_n122_), .c(x18), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n121_), .O(z07));
  inv1   g073(.a(x27), .O(new_n125_));
  nor2   g074(.a(x26), .b(x25), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n96_), .c(new_n78_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n61_), .c(new_n54_), .O(new_n128_));
  nor3   g077(.a(x27), .b(x26), .c(x25), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n96_), .c(new_n79_), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(new_n125_), .c(new_n130_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(x16), .O(new_n132_));
  inv1   g081(.a(x07), .O(new_n133_));
  aoi21  g082(.a(new_n62_), .b(new_n133_), .c(x18), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  nor2   g085(.a(x27), .b(x26), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n107_), .c(new_n85_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n93_), .c(new_n65_), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n114_), .c(new_n79_), .d(new_n113_), .O(new_n141_));
  oai21  g090(.a(new_n139_), .b(new_n136_), .c(new_n141_), .O(new_n142_));
  nor2   g091(.a(x16), .b(x06), .O(new_n143_));
  aoi21  g092(.a(new_n142_), .b(x16), .c(new_n143_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(x18), .c(new_n59_), .O(z09));
  oai21  g094(.a(x18), .b(new_n62_), .c(new_n58_), .O(new_n146_));
  inv1   g095(.a(x29), .O(new_n147_));
  nand2  g096(.a(new_n140_), .b(new_n126_), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n98_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n93_), .c(new_n65_), .O(new_n151_));
  nor3   g100(.a(x29), .b(x28), .c(x27), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n118_), .c(new_n87_), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n147_), .c(new_n153_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n61_), .c(x16), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n146_), .O(z10));
  inv1   g105(.a(x30), .O(new_n157_));
  inv1   g106(.a(x22), .O(new_n158_));
  nand3  g107(.a(new_n96_), .b(new_n71_), .c(new_n158_), .O(new_n159_));
  nand2  g108(.a(new_n152_), .b(new_n126_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n159_), .c(new_n61_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n53_), .c(new_n157_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n140_), .c(new_n118_), .O(new_n164_));
  nor3   g113(.a(new_n164_), .b(new_n86_), .c(new_n65_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n162_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n62_), .b(new_n167_), .c(x18), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z11));
  inv1   g118(.a(x31), .O(new_n170_));
  nor3   g119(.a(x30), .b(x29), .c(x28), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n137_), .c(new_n114_), .d(new_n95_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n93_), .c(new_n65_), .O(new_n173_));
  nor2   g122(.a(x29), .b(x28), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n174_), .c(new_n129_), .O(new_n176_));
  oai22  g125(.a(new_n176_), .b(new_n99_), .c(new_n173_), .d(new_n170_), .O(new_n177_));
  nor2   g126(.a(x16), .b(x03), .O(new_n178_));
  aoi21  g127(.a(new_n177_), .b(x16), .c(new_n178_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(x18), .c(new_n59_), .O(z12));
  inv1   g129(.a(x32), .O(new_n181_));
  nand4  g130(.a(new_n175_), .b(new_n140_), .c(new_n147_), .d(new_n113_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n115_), .c(new_n61_), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n53_), .c(new_n181_), .O(new_n184_));
  nand4  g133(.a(new_n163_), .b(new_n149_), .c(new_n181_), .d(new_n170_), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n99_), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n184_), .c(x16), .O(new_n187_));
  inv1   g136(.a(x02), .O(new_n188_));
  aoi21  g137(.a(new_n62_), .b(new_n188_), .c(x18), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n187_), .O(z13));
  inv1   g139(.a(x33), .O(new_n191_));
  nor3   g140(.a(x32), .b(x31), .c(x30), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n152_), .c(new_n118_), .d(new_n85_), .O(new_n193_));
  aoi21  g142(.a(new_n193_), .b(new_n93_), .c(new_n65_), .O(new_n194_));
  nor2   g143(.a(x33), .b(x32), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n175_), .c(new_n174_), .d(new_n137_), .O(new_n196_));
  oai22  g145(.a(new_n196_), .b(new_n108_), .c(new_n194_), .d(new_n191_), .O(new_n197_));
  nor2   g146(.a(x16), .b(x01), .O(new_n198_));
  aoi21  g147(.a(new_n197_), .b(x16), .c(new_n198_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(x18), .c(new_n59_), .O(z14));
  inv1   g149(.a(x34), .O(new_n201_));
  nand4  g150(.a(new_n126_), .b(new_n106_), .c(new_n92_), .d(new_n70_), .O(new_n202_));
  nand4  g151(.a(new_n195_), .b(new_n175_), .c(new_n174_), .d(new_n125_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n202_), .c(new_n93_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(new_n64_), .c(new_n201_), .O(new_n205_));
  nand2  g154(.a(new_n174_), .b(new_n137_), .O(new_n206_));
  nand4  g155(.a(new_n175_), .b(new_n201_), .c(new_n191_), .d(new_n181_), .O(new_n207_));
  nor3   g156(.a(new_n207_), .b(new_n206_), .c(new_n108_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n205_), .c(x16), .O(new_n209_));
  inv1   g158(.a(x00), .O(new_n210_));
  nand2  g159(.a(new_n62_), .b(new_n210_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n209_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n61_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(new_n59_), .O(z15));
endmodule


