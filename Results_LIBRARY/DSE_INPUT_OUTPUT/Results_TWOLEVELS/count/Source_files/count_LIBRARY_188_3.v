// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(new_n55_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x20), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n58_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nor2   g017(.a(x27), .b(new_n61_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n65_), .b(x21), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n58_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n58_), .c(new_n77_), .O(z02));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n55_), .c(new_n64_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n73_), .b(x22), .c(new_n81_), .O(new_n82_));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n58_), .b(new_n83_), .c(x18), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n58_), .c(new_n84_), .O(z03));
  nand2  g034(.a(new_n80_), .b(x23), .O(new_n86_));
  inv1   g035(.a(x21), .O(new_n87_));
  nor3   g036(.a(x20), .b(x19), .c(x17), .O(new_n88_));
  nor2   g037(.a(x23), .b(x22), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(new_n86_), .c(new_n58_), .O(new_n91_));
  nor2   g040(.a(x16), .b(x11), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n91_), .c(new_n61_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n70_), .O(z04));
  nor2   g043(.a(x24), .b(x23), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n79_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n65_), .O(new_n97_));
  aoi21  g046(.a(new_n90_), .b(x24), .c(new_n97_), .O(new_n98_));
  inv1   g047(.a(x10), .O(new_n99_));
  aoi21  g048(.a(new_n58_), .b(new_n99_), .c(x18), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n58_), .c(new_n100_), .O(z05));
  nand3  g050(.a(new_n95_), .b(new_n79_), .c(new_n88_), .O(new_n102_));
  nor2   g051(.a(x25), .b(x24), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n89_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  aoi22  g054(.a(new_n105_), .b(new_n74_), .c(new_n102_), .d(x25), .O(new_n106_));
  inv1   g055(.a(x09), .O(new_n107_));
  aoi21  g056(.a(new_n58_), .b(new_n107_), .c(x18), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n58_), .c(new_n108_), .O(z06));
  nand4  g058(.a(new_n103_), .b(new_n89_), .c(new_n72_), .d(new_n55_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x26), .O(new_n111_));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n89_), .c(new_n72_), .d(new_n55_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n111_), .c(new_n58_), .O(new_n114_));
  nor2   g063(.a(x16), .b(x08), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n114_), .c(new_n61_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n70_), .O(z07));
  inv1   g066(.a(x27), .O(new_n118_));
  nand2  g067(.a(new_n112_), .b(new_n89_), .O(new_n119_));
  nand4  g068(.a(new_n72_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n119_), .c(new_n61_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand3  g071(.a(new_n113_), .b(x27), .c(x16), .O(new_n123_));
  inv1   g072(.a(x07), .O(new_n124_));
  nand2  g073(.a(new_n58_), .b(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n61_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n122_), .O(z08));
  inv1   g077(.a(x28), .O(new_n129_));
  nand3  g078(.a(new_n118_), .b(new_n53_), .c(new_n52_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n61_), .O(new_n131_));
  nand2  g080(.a(new_n79_), .b(new_n64_), .O(new_n132_));
  nor2   g081(.a(x26), .b(x25), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n95_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n132_), .c(new_n118_), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n131_), .c(new_n129_), .O(new_n136_));
  nor3   g085(.a(x25), .b(x24), .c(x23), .O(new_n137_));
  nor3   g086(.a(x28), .b(x27), .c(x26), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n137_), .c(new_n81_), .O(new_n139_));
  inv1   g088(.a(new_n139_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n136_), .c(x16), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  nor2   g091(.a(x18), .b(x16), .O(new_n143_));
  aoi21  g092(.a(new_n143_), .b(new_n142_), .c(new_n69_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n141_), .O(z09));
  nor2   g094(.a(x29), .b(x28), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n112_), .c(new_n118_), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n90_), .O(new_n148_));
  aoi21  g097(.a(new_n139_), .b(x29), .c(new_n148_), .O(new_n149_));
  inv1   g098(.a(x05), .O(new_n150_));
  aoi21  g099(.a(new_n58_), .b(new_n150_), .c(x18), .O(new_n151_));
  oai21  g100(.a(new_n149_), .b(new_n58_), .c(new_n151_), .O(z10));
  inv1   g101(.a(x30), .O(new_n153_));
  inv1   g102(.a(x29), .O(new_n154_));
  nand3  g103(.a(new_n55_), .b(new_n154_), .c(new_n118_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n61_), .O(new_n156_));
  nand2  g105(.a(new_n89_), .b(new_n72_), .O(new_n157_));
  inv1   g106(.a(x26), .O(new_n158_));
  nand3  g107(.a(new_n103_), .b(new_n129_), .c(new_n158_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n157_), .c(new_n118_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n156_), .c(new_n153_), .O(new_n161_));
  nor2   g110(.a(x28), .b(x27), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n162_), .c(new_n112_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n90_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n161_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n143_), .b(new_n167_), .c(new_n69_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z11));
  oai21  g118(.a(new_n164_), .b(new_n90_), .c(x31), .O(new_n170_));
  nor3   g119(.a(x27), .b(x26), .c(x25), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n171_), .c(new_n146_), .d(new_n97_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n170_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  inv1   g124(.a(x03), .O(new_n176_));
  aoi21  g125(.a(new_n58_), .b(new_n176_), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z12));
  inv1   g127(.a(x32), .O(new_n179_));
  inv1   g128(.a(x31), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n154_), .c(new_n118_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n62_), .c(new_n61_), .O(new_n182_));
  nor2   g131(.a(x30), .b(x28), .O(new_n183_));
  nand3  g132(.a(new_n183_), .b(new_n103_), .c(new_n158_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n157_), .c(new_n118_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n182_), .c(new_n179_), .O(new_n186_));
  nor2   g135(.a(x32), .b(x31), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n163_), .c(new_n162_), .d(new_n133_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n102_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n186_), .c(x16), .O(new_n190_));
  inv1   g139(.a(x02), .O(new_n191_));
  aoi21  g140(.a(new_n143_), .b(new_n191_), .c(new_n69_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n190_), .O(z13));
  oai21  g142(.a(new_n188_), .b(new_n102_), .c(x33), .O(new_n194_));
  nand3  g143(.a(new_n146_), .b(new_n118_), .c(new_n158_), .O(new_n195_));
  inv1   g144(.a(x33), .O(new_n196_));
  nand3  g145(.a(new_n172_), .b(new_n196_), .c(new_n179_), .O(new_n197_));
  nor2   g146(.a(new_n197_), .b(new_n195_), .O(new_n198_));
  nand3  g147(.a(new_n198_), .b(new_n105_), .c(new_n74_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n194_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(x16), .O(new_n201_));
  inv1   g150(.a(x01), .O(new_n202_));
  aoi21  g151(.a(new_n58_), .b(new_n202_), .c(x18), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n201_), .O(z14));
  inv1   g153(.a(x34), .O(new_n205_));
  nand3  g154(.a(new_n196_), .b(new_n180_), .c(new_n154_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n130_), .c(new_n61_), .O(new_n207_));
  inv1   g156(.a(x22), .O(new_n208_));
  nand3  g157(.a(new_n95_), .b(new_n72_), .c(new_n208_), .O(new_n209_));
  nor2   g158(.a(x32), .b(x30), .O(new_n210_));
  nand3  g159(.a(new_n210_), .b(new_n133_), .c(new_n129_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n209_), .c(new_n118_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n207_), .c(new_n205_), .O(new_n213_));
  nand4  g162(.a(new_n172_), .b(new_n205_), .c(new_n196_), .d(new_n179_), .O(new_n214_));
  nor3   g163(.a(new_n214_), .b(new_n195_), .c(new_n110_), .O(new_n215_));
  oai21  g164(.a(new_n215_), .b(new_n213_), .c(x16), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  aoi21  g166(.a(new_n143_), .b(new_n217_), .c(new_n69_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n216_), .O(z15));
endmodule


