// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x24), .c(new_n52_), .O(z00));
  inv1   g008(.a(x24), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n55_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  nor2   g012(.a(x20), .b(x19), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n60_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n52_), .O(z01));
  inv1   g018(.a(x19), .O(new_n70_));
  nand2  g019(.a(new_n60_), .b(x20), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n70_), .c(new_n63_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n54_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(x24), .b(x18), .O(new_n78_));
  inv1   g027(.a(new_n78_), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  oai21  g029(.a(new_n76_), .b(new_n61_), .c(new_n80_), .O(z02));
  oai21  g030(.a(new_n73_), .b(x24), .c(new_n54_), .O(new_n82_));
  nor3   g031(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  aoi22  g032(.a(new_n83_), .b(new_n54_), .c(new_n82_), .d(x22), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n79_), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n61_), .c(new_n86_), .O(z03));
  nor2   g036(.a(x22), .b(x21), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(x19), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n65_), .c(x23), .O(new_n90_));
  inv1   g039(.a(x21), .O(new_n91_));
  inv1   g040(.a(new_n65_), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  aoi21  g043(.a(new_n94_), .b(new_n90_), .c(new_n61_), .O(new_n95_));
  nor2   g044(.a(x16), .b(x11), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n95_), .c(new_n60_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n52_), .O(z04));
  inv1   g047(.a(x10), .O(new_n99_));
  nand2  g048(.a(new_n61_), .b(new_n99_), .O(new_n100_));
  nor2   g049(.a(x17), .b(new_n61_), .O(new_n101_));
  nor2   g050(.a(x24), .b(x23), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n101_), .c(new_n88_), .d(new_n64_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n100_), .c(new_n78_), .O(z05));
  inv1   g053(.a(x25), .O(new_n105_));
  nand2  g054(.a(new_n93_), .b(new_n73_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n60_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n54_), .c(new_n105_), .O(new_n108_));
  nor2   g057(.a(x25), .b(x23), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n88_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n65_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n108_), .c(x16), .O(new_n112_));
  nor2   g061(.a(x16), .b(x09), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n79_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n112_), .O(z06));
  inv1   g064(.a(x26), .O(new_n116_));
  aoi21  g065(.a(new_n110_), .b(new_n70_), .c(new_n65_), .O(new_n117_));
  nor2   g066(.a(x26), .b(x25), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n93_), .c(new_n75_), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n116_), .c(new_n119_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x08), .O(new_n121_));
  aoi21  g070(.a(new_n120_), .b(x16), .c(new_n121_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(x24), .c(new_n52_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  inv1   g073(.a(x23), .O(new_n125_));
  nand3  g074(.a(new_n118_), .b(new_n88_), .c(new_n125_), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n70_), .c(new_n65_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n93_), .c(new_n75_), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n124_), .c(new_n129_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x07), .O(new_n131_));
  aoi21  g080(.a(new_n130_), .b(x16), .c(new_n131_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(x24), .c(new_n52_), .O(z08));
  inv1   g082(.a(x28), .O(new_n134_));
  nor2   g083(.a(x27), .b(x26), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n93_), .c(new_n105_), .d(new_n91_), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n70_), .c(new_n65_), .O(new_n137_));
  nor3   g086(.a(x28), .b(x27), .c(x26), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n109_), .c(new_n83_), .d(new_n54_), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n134_), .c(new_n139_), .O(new_n140_));
  nor2   g089(.a(x16), .b(x06), .O(new_n141_));
  aoi21  g090(.a(new_n140_), .b(x16), .c(new_n141_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(x24), .c(new_n52_), .O(z09));
  inv1   g092(.a(x29), .O(new_n144_));
  nor2   g093(.a(x28), .b(x27), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n118_), .c(new_n93_), .d(new_n91_), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n70_), .c(new_n65_), .O(new_n147_));
  nor3   g096(.a(x26), .b(x25), .c(x23), .O(new_n148_));
  nor3   g097(.a(x29), .b(x28), .c(x27), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n148_), .c(new_n83_), .d(new_n54_), .O(new_n150_));
  oai21  g099(.a(new_n147_), .b(new_n144_), .c(new_n150_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x05), .O(new_n152_));
  aoi21  g101(.a(new_n151_), .b(x16), .c(new_n152_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(x24), .c(new_n52_), .O(z10));
  inv1   g103(.a(x30), .O(new_n155_));
  nand2  g104(.a(new_n149_), .b(new_n118_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n106_), .c(new_n60_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n54_), .c(new_n155_), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n128_), .c(new_n134_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n94_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n158_), .c(x16), .O(new_n162_));
  nor2   g111(.a(x16), .b(x04), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n79_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(z11));
  inv1   g114(.a(x31), .O(new_n166_));
  inv1   g115(.a(x22), .O(new_n167_));
  nand3  g116(.a(new_n109_), .b(new_n73_), .c(new_n167_), .O(new_n168_));
  nand3  g117(.a(new_n159_), .b(new_n135_), .c(new_n134_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n168_), .c(new_n60_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n54_), .c(new_n166_), .O(new_n171_));
  nor2   g120(.a(x29), .b(x28), .O(new_n172_));
  nor2   g121(.a(x31), .b(x30), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n172_), .c(new_n128_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n94_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n171_), .c(x16), .O(new_n176_));
  nor2   g125(.a(x16), .b(x03), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n79_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n176_), .O(z12));
  inv1   g128(.a(x32), .O(new_n180_));
  nand4  g129(.a(new_n173_), .b(new_n145_), .c(new_n144_), .d(new_n116_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n168_), .c(new_n60_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n54_), .c(new_n180_), .O(new_n183_));
  inv1   g132(.a(new_n111_), .O(new_n184_));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n159_), .c(new_n138_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n184_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n183_), .c(x16), .O(new_n188_));
  nor2   g137(.a(x16), .b(x02), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n79_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(z13));
  inv1   g140(.a(x33), .O(new_n192_));
  nand2  g141(.a(new_n148_), .b(new_n83_), .O(new_n193_));
  nand4  g142(.a(new_n185_), .b(new_n172_), .c(new_n155_), .d(new_n124_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n193_), .c(new_n60_), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n54_), .c(new_n192_), .O(new_n196_));
  nor2   g145(.a(x33), .b(x32), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n173_), .c(new_n172_), .d(new_n135_), .O(new_n198_));
  nor2   g147(.a(new_n198_), .b(new_n184_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n196_), .c(x16), .O(new_n200_));
  nor2   g149(.a(x16), .b(x01), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n79_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n200_), .O(z14));
  inv1   g152(.a(x34), .O(new_n204_));
  nand4  g153(.a(new_n197_), .b(new_n173_), .c(new_n172_), .d(new_n124_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n193_), .c(new_n60_), .O(new_n206_));
  aoi21  g155(.a(new_n206_), .b(new_n54_), .c(new_n204_), .O(new_n207_));
  nor2   g156(.a(x34), .b(x33), .O(new_n208_));
  nand4  g157(.a(new_n208_), .b(new_n185_), .c(new_n159_), .d(new_n145_), .O(new_n209_));
  nor2   g158(.a(new_n209_), .b(new_n119_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n207_), .c(x16), .O(new_n211_));
  nor2   g160(.a(x16), .b(x00), .O(new_n212_));
  nor2   g161(.a(new_n212_), .b(new_n79_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(new_n211_), .O(z15));
endmodule


