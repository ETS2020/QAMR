// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  inv1   g000(.a(x09), .O(new_n52_));
  inv1   g001(.a(x13), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x16), .O(new_n59_));
  inv1   g008(.a(x15), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n60_), .c(x18), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n59_), .c(new_n54_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n58_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n56_), .c(new_n55_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  inv1   g017(.a(x14), .O(new_n69_));
  aoi21  g018(.a(new_n61_), .b(new_n69_), .c(x18), .O(new_n70_));
  aoi21  g019(.a(new_n70_), .b(new_n68_), .c(new_n54_), .O(z01));
  aoi21  g020(.a(x16), .b(new_n52_), .c(new_n53_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n58_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n66_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(x18), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(x16), .O(new_n78_));
  aoi22  g027(.a(new_n78_), .b(new_n53_), .c(x18), .d(new_n52_), .O(new_n79_));
  oai21  g028(.a(new_n76_), .b(new_n72_), .c(new_n79_), .O(z02));
  oai21  g029(.a(new_n53_), .b(new_n52_), .c(x18), .O(new_n81_));
  inv1   g030(.a(x22), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n58_), .c(new_n64_), .O(new_n84_));
  oai21  g033(.a(new_n75_), .b(new_n82_), .c(new_n84_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(x16), .O(new_n86_));
  nor2   g035(.a(x16), .b(x12), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n54_), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n86_), .c(new_n81_), .O(z03));
  inv1   g038(.a(x23), .O(new_n90_));
  oai21  g039(.a(x22), .b(x21), .c(new_n56_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n67_), .c(new_n90_), .O(new_n92_));
  inv1   g041(.a(x21), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n67_), .c(new_n93_), .O(new_n95_));
  inv1   g044(.a(new_n95_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n92_), .c(x16), .O(new_n97_));
  inv1   g046(.a(x11), .O(new_n98_));
  aoi21  g047(.a(new_n61_), .b(new_n98_), .c(x18), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n97_), .c(new_n54_), .O(z04));
  inv1   g049(.a(x24), .O(new_n101_));
  nor2   g050(.a(x24), .b(x23), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n83_), .c(new_n67_), .O(new_n103_));
  oai21  g052(.a(new_n96_), .b(new_n101_), .c(new_n103_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(x16), .O(new_n105_));
  nor2   g054(.a(x16), .b(x10), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n54_), .O(new_n107_));
  nand3  g056(.a(new_n107_), .b(new_n105_), .c(new_n81_), .O(z05));
  nand2  g057(.a(new_n78_), .b(new_n52_), .O(new_n109_));
  inv1   g058(.a(x25), .O(new_n110_));
  nand2  g059(.a(new_n102_), .b(new_n83_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n66_), .O(new_n112_));
  nor2   g061(.a(x25), .b(x24), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n94_), .c(new_n73_), .d(new_n58_), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n110_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  aoi21  g065(.a(x18), .b(new_n53_), .c(new_n54_), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n116_), .c(new_n109_), .O(z06));
  and2   g067(.a(new_n114_), .b(x26), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n94_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n74_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n119_), .c(x16), .O(new_n123_));
  nor2   g072(.a(x16), .b(x08), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n54_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n123_), .c(new_n81_), .O(z07));
  inv1   g075(.a(x27), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n102_), .O(new_n129_));
  oai22  g078(.a(new_n129_), .b(new_n84_), .c(new_n122_), .d(new_n127_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  nor2   g080(.a(x16), .b(x07), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n54_), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n131_), .c(new_n81_), .O(z08));
  oai21  g083(.a(new_n129_), .b(new_n84_), .c(x28), .O(new_n135_));
  inv1   g084(.a(x26), .O(new_n136_));
  nor2   g085(.a(x28), .b(x27), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n113_), .c(new_n136_), .d(new_n90_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n84_), .c(new_n135_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(x16), .O(new_n140_));
  nor2   g089(.a(x16), .b(x06), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n54_), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n140_), .c(new_n81_), .O(z09));
  inv1   g092(.a(x29), .O(new_n144_));
  nor2   g093(.a(x26), .b(x25), .O(new_n145_));
  nand4  g094(.a(new_n137_), .b(new_n145_), .c(new_n102_), .d(new_n83_), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n56_), .c(new_n66_), .O(new_n147_));
  nor2   g096(.a(x29), .b(x28), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n120_), .c(new_n127_), .O(new_n149_));
  oai22  g098(.a(new_n149_), .b(new_n95_), .c(new_n147_), .d(new_n144_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  inv1   g100(.a(x05), .O(new_n152_));
  aoi21  g101(.a(new_n61_), .b(new_n152_), .c(x18), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n151_), .c(new_n54_), .O(z10));
  inv1   g103(.a(x30), .O(new_n155_));
  nand3  g104(.a(new_n148_), .b(new_n145_), .c(new_n127_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n111_), .c(new_n56_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n67_), .c(new_n155_), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n137_), .c(new_n120_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n95_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n158_), .c(x16), .O(new_n162_));
  inv1   g111(.a(x04), .O(new_n163_));
  aoi21  g112(.a(new_n61_), .b(new_n163_), .c(x18), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n162_), .c(new_n54_), .O(z11));
  inv1   g114(.a(x31), .O(new_n166_));
  nand3  g115(.a(new_n113_), .b(new_n83_), .c(new_n90_), .O(new_n167_));
  inv1   g116(.a(x28), .O(new_n168_));
  nor2   g117(.a(x27), .b(x26), .O(new_n169_));
  nand3  g118(.a(new_n159_), .b(new_n169_), .c(new_n168_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(new_n56_), .O(new_n171_));
  aoi21  g120(.a(new_n171_), .b(new_n67_), .c(new_n166_), .O(new_n172_));
  nor2   g121(.a(x31), .b(x30), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n148_), .c(new_n128_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n103_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n172_), .c(x16), .O(new_n176_));
  inv1   g125(.a(x03), .O(new_n177_));
  aoi21  g126(.a(new_n61_), .b(new_n177_), .c(x18), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n176_), .c(new_n54_), .O(z12));
  inv1   g128(.a(x32), .O(new_n180_));
  nand2  g129(.a(new_n137_), .b(new_n145_), .O(new_n181_));
  nand3  g130(.a(new_n159_), .b(new_n180_), .c(new_n166_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n112_), .O(new_n184_));
  oai21  g133(.a(new_n175_), .b(new_n180_), .c(new_n184_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(x16), .O(new_n186_));
  nor2   g135(.a(x16), .b(x02), .O(new_n187_));
  nor2   g136(.a(new_n187_), .b(new_n54_), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n186_), .c(new_n81_), .O(z13));
  inv1   g138(.a(x33), .O(new_n190_));
  aoi21  g139(.a(new_n183_), .b(new_n112_), .c(new_n190_), .O(new_n191_));
  nand2  g140(.a(new_n148_), .b(new_n169_), .O(new_n192_));
  nor2   g141(.a(x33), .b(x32), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n173_), .O(new_n194_));
  nor3   g143(.a(new_n194_), .b(new_n192_), .c(new_n114_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n191_), .c(x16), .O(new_n196_));
  nor2   g145(.a(x16), .b(x01), .O(new_n197_));
  nor2   g146(.a(new_n197_), .b(new_n54_), .O(new_n198_));
  nand3  g147(.a(new_n198_), .b(new_n196_), .c(new_n81_), .O(z14));
  inv1   g148(.a(x34), .O(new_n200_));
  nand4  g149(.a(new_n145_), .b(new_n94_), .c(new_n101_), .d(new_n93_), .O(new_n201_));
  nand4  g150(.a(new_n193_), .b(new_n173_), .c(new_n148_), .d(new_n127_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n201_), .c(new_n56_), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n67_), .c(new_n200_), .O(new_n204_));
  nand4  g153(.a(new_n173_), .b(new_n200_), .c(new_n190_), .d(new_n180_), .O(new_n205_));
  nor3   g154(.a(new_n205_), .b(new_n192_), .c(new_n114_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n204_), .c(x16), .O(new_n207_));
  inv1   g156(.a(x00), .O(new_n208_));
  aoi21  g157(.a(new_n61_), .b(new_n208_), .c(x18), .O(new_n209_));
  aoi21  g158(.a(new_n209_), .b(new_n207_), .c(new_n54_), .O(z15));
endmodule


