// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n230_, new_n231_, new_n232_, new_n233_, new_n234_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x18), .O(new_n58_));
  inv1   g007(.a(x32), .O(new_n59_));
  nor2   g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  inv1   g009(.a(new_n60_), .O(new_n61_));
  oai21  g010(.a(new_n57_), .b(x18), .c(new_n61_), .O(z00));
  inv1   g011(.a(x16), .O(new_n63_));
  nand2  g012(.a(new_n54_), .b(x20), .O(new_n64_));
  inv1   g013(.a(x17), .O(new_n65_));
  nor2   g014(.a(x20), .b(x19), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n64_), .c(new_n63_), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n68_), .c(new_n58_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n61_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  inv1   g021(.a(new_n67_), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n72_), .c(new_n75_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  aoi21  g026(.a(new_n76_), .b(x16), .c(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(x18), .c(new_n61_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(x20), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n81_), .c(new_n65_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(x22), .b(x21), .c(new_n84_), .O(new_n85_));
  oai22  g034(.a(new_n85_), .b(x19), .c(new_n73_), .d(new_n80_), .O(new_n86_));
  nor2   g035(.a(x16), .b(x12), .O(new_n87_));
  aoi21  g036(.a(new_n86_), .b(x16), .c(new_n87_), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(x18), .c(new_n61_), .O(z03));
  inv1   g038(.a(new_n82_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(x20), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(x18), .c(new_n53_), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  nor3   g043(.a(new_n94_), .b(new_n67_), .c(x21), .O(new_n95_));
  aoi21  g044(.a(new_n92_), .b(x23), .c(new_n95_), .O(new_n96_));
  inv1   g045(.a(x11), .O(new_n97_));
  aoi21  g046(.a(new_n63_), .b(new_n97_), .c(x18), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n63_), .c(new_n98_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  nand2  g049(.a(new_n93_), .b(new_n74_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n58_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n53_), .c(new_n100_), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n82_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n67_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n63_), .b(new_n108_), .c(x18), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n107_), .O(z05));
  inv1   g059(.a(x25), .O(new_n111_));
  nand3  g060(.a(new_n104_), .b(new_n74_), .c(new_n80_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n58_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n53_), .c(new_n111_), .O(new_n114_));
  nor2   g063(.a(x25), .b(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n93_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n75_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n114_), .c(x16), .O(new_n118_));
  inv1   g067(.a(x09), .O(new_n119_));
  aoi21  g068(.a(new_n63_), .b(new_n119_), .c(x18), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z06));
  inv1   g070(.a(x26), .O(new_n122_));
  inv1   g071(.a(x23), .O(new_n123_));
  nand2  g072(.a(new_n115_), .b(new_n123_), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n91_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n58_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n53_), .c(new_n122_), .O(new_n128_));
  nor2   g077(.a(x26), .b(x25), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n100_), .O(new_n130_));
  nor3   g079(.a(new_n130_), .b(new_n94_), .c(new_n75_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  inv1   g081(.a(x08), .O(new_n133_));
  aoi21  g082(.a(new_n63_), .b(new_n133_), .c(x18), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z07));
  inv1   g084(.a(x27), .O(new_n136_));
  nand2  g085(.a(new_n129_), .b(new_n104_), .O(new_n137_));
  inv1   g086(.a(new_n137_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n91_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n58_), .O(new_n140_));
  aoi21  g089(.a(new_n140_), .b(new_n53_), .c(new_n136_), .O(new_n141_));
  nand2  g090(.a(new_n91_), .b(new_n53_), .O(new_n142_));
  nor2   g091(.a(x27), .b(x26), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n104_), .c(new_n111_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n141_), .c(x16), .O(new_n146_));
  inv1   g095(.a(x07), .O(new_n147_));
  aoi21  g096(.a(new_n63_), .b(new_n147_), .c(x18), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n146_), .O(z08));
  inv1   g098(.a(x28), .O(new_n150_));
  inv1   g099(.a(x19), .O(new_n151_));
  nand4  g100(.a(new_n143_), .b(new_n115_), .c(new_n93_), .d(new_n72_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n151_), .c(new_n67_), .O(new_n153_));
  nor3   g102(.a(x28), .b(x27), .c(x26), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n125_), .O(new_n155_));
  oai22  g104(.a(new_n155_), .b(new_n142_), .c(new_n153_), .d(new_n150_), .O(new_n156_));
  nor2   g105(.a(x16), .b(x06), .O(new_n157_));
  aoi21  g106(.a(new_n156_), .b(x16), .c(new_n157_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(x18), .c(new_n61_), .O(z09));
  inv1   g108(.a(x29), .O(new_n160_));
  nand2  g109(.a(new_n154_), .b(new_n115_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n101_), .c(new_n58_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n53_), .c(new_n160_), .O(new_n163_));
  inv1   g112(.a(new_n95_), .O(new_n164_));
  nor2   g113(.a(x29), .b(x28), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n136_), .O(new_n166_));
  nor3   g115(.a(new_n166_), .b(new_n130_), .c(new_n164_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n163_), .c(x16), .O(new_n168_));
  inv1   g117(.a(x05), .O(new_n169_));
  aoi21  g118(.a(new_n63_), .b(new_n169_), .c(x18), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n168_), .O(z10));
  inv1   g120(.a(x30), .O(new_n172_));
  inv1   g121(.a(new_n105_), .O(new_n173_));
  inv1   g122(.a(new_n166_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n129_), .c(new_n173_), .O(new_n175_));
  aoi21  g124(.a(new_n175_), .b(new_n151_), .c(new_n67_), .O(new_n176_));
  nor2   g125(.a(x28), .b(x27), .O(new_n177_));
  nor2   g126(.a(x30), .b(x29), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n130_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n95_), .O(new_n181_));
  oai21  g130(.a(new_n176_), .b(new_n172_), .c(new_n181_), .O(new_n182_));
  nor2   g131(.a(x16), .b(x04), .O(new_n183_));
  aoi21  g132(.a(new_n182_), .b(x16), .c(new_n183_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(x18), .c(new_n61_), .O(z11));
  aoi21  g134(.a(new_n59_), .b(x18), .c(x31), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n172_), .c(new_n160_), .d(new_n150_), .O(new_n187_));
  nor2   g136(.a(new_n187_), .b(x27), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n122_), .c(new_n111_), .d(new_n100_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(x23), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n80_), .c(new_n72_), .d(new_n81_), .O(new_n191_));
  nor2   g140(.a(new_n124_), .b(new_n90_), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n178_), .c(new_n143_), .d(new_n150_), .O(new_n193_));
  nand3  g142(.a(new_n193_), .b(x31), .c(new_n58_), .O(new_n194_));
  oai21  g143(.a(new_n191_), .b(x17), .c(new_n194_), .O(new_n195_));
  nand3  g144(.a(new_n67_), .b(x31), .c(new_n58_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  aoi21  g146(.a(new_n195_), .b(new_n151_), .c(new_n197_), .O(new_n198_));
  nor3   g147(.a(x18), .b(x16), .c(x03), .O(new_n199_));
  nor2   g148(.a(new_n199_), .b(new_n60_), .O(new_n200_));
  oai21  g149(.a(new_n198_), .b(new_n63_), .c(new_n200_), .O(z12));
  nand4  g150(.a(new_n173_), .b(new_n66_), .c(new_n65_), .d(x16), .O(new_n202_));
  nor2   g151(.a(x32), .b(x31), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n178_), .c(new_n177_), .d(new_n129_), .O(new_n204_));
  oai22  g153(.a(new_n204_), .b(new_n202_), .c(x16), .d(x02), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n58_), .O(new_n206_));
  nor2   g155(.a(x31), .b(x30), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n192_), .c(new_n154_), .d(new_n160_), .O(new_n208_));
  aoi21  g157(.a(new_n208_), .b(new_n151_), .c(new_n67_), .O(new_n209_));
  nor2   g158(.a(new_n209_), .b(new_n63_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(x18), .c(x32), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n206_), .O(z13));
  aoi21  g161(.a(x33), .b(x16), .c(x18), .O(new_n213_));
  inv1   g162(.a(x33), .O(new_n214_));
  nand3  g163(.a(new_n207_), .b(new_n214_), .c(new_n59_), .O(new_n215_));
  inv1   g164(.a(new_n215_), .O(new_n216_));
  nand4  g165(.a(new_n216_), .b(new_n165_), .c(new_n143_), .d(new_n117_), .O(new_n217_));
  oai21  g166(.a(new_n209_), .b(new_n214_), .c(new_n217_), .O(new_n218_));
  nor2   g167(.a(x16), .b(x01), .O(new_n219_));
  aoi21  g168(.a(new_n218_), .b(x16), .c(new_n219_), .O(new_n220_));
  oai22  g169(.a(new_n220_), .b(x18), .c(new_n213_), .d(new_n59_), .O(z14));
  nand2  g170(.a(x34), .b(x32), .O(new_n222_));
  inv1   g171(.a(x34), .O(new_n223_));
  nand3  g172(.a(new_n203_), .b(new_n223_), .c(new_n214_), .O(new_n224_));
  nor2   g173(.a(new_n224_), .b(new_n179_), .O(new_n225_));
  nand3  g174(.a(new_n225_), .b(new_n138_), .c(new_n84_), .O(new_n226_));
  aoi21  g175(.a(new_n226_), .b(new_n222_), .c(x19), .O(new_n227_));
  nor2   g176(.a(x33), .b(x31), .O(new_n228_));
  nand3  g177(.a(new_n228_), .b(new_n178_), .c(new_n154_), .O(new_n229_));
  oai21  g178(.a(new_n229_), .b(new_n126_), .c(new_n58_), .O(new_n230_));
  aoi21  g179(.a(new_n230_), .b(new_n53_), .c(new_n223_), .O(new_n231_));
  oai21  g180(.a(new_n231_), .b(new_n227_), .c(x16), .O(new_n232_));
  inv1   g181(.a(x00), .O(new_n233_));
  aoi21  g182(.a(new_n63_), .b(new_n233_), .c(x18), .O(new_n234_));
  nand2  g183(.a(new_n234_), .b(new_n232_), .O(z15));
endmodule


