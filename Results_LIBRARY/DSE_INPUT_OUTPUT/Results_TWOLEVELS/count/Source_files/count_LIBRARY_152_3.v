// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g006(.a(x03), .O(new_n58_));
  nand2  g007(.a(x18), .b(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n57_), .b(x18), .c(new_n59_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x16), .O(new_n62_));
  nand2  g011(.a(new_n54_), .b(x20), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n59_), .O(z01));
  inv1   g018(.a(x17), .O(new_n70_));
  inv1   g019(.a(x19), .O(new_n71_));
  nand2  g020(.a(x20), .b(new_n61_), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  aoi21  g025(.a(new_n73_), .b(x21), .c(new_n76_), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n62_), .b(new_n78_), .c(x18), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n62_), .c(new_n79_), .O(z02));
  oai21  g029(.a(new_n74_), .b(x18), .c(new_n53_), .O(new_n81_));
  nor3   g030(.a(x22), .b(x21), .c(x20), .O(new_n82_));
  and2   g031(.a(new_n82_), .b(new_n53_), .O(new_n83_));
  aoi21  g032(.a(new_n81_), .b(x22), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x12), .O(new_n85_));
  aoi21  g034(.a(new_n62_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n62_), .c(new_n86_), .O(z03));
  nor2   g036(.a(x22), .b(x21), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(x19), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n65_), .c(x23), .O(new_n90_));
  inv1   g039(.a(x21), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n64_), .c(new_n91_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n90_), .c(new_n62_), .O(new_n94_));
  nor2   g043(.a(x16), .b(x11), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n94_), .c(new_n61_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n59_), .O(z04));
  aoi21  g046(.a(new_n92_), .b(new_n91_), .c(x19), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n65_), .c(x24), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n88_), .c(new_n64_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x10), .O(new_n103_));
  aoi21  g052(.a(new_n102_), .b(x16), .c(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(x18), .c(new_n59_), .O(z05));
  aoi21  g054(.a(new_n100_), .b(new_n88_), .c(x19), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n65_), .c(x25), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n92_), .c(new_n74_), .d(new_n53_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nor2   g059(.a(x16), .b(x09), .O(new_n111_));
  aoi21  g060(.a(new_n110_), .b(x16), .c(new_n111_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(x18), .c(new_n59_), .O(z06));
  inv1   g062(.a(x26), .O(new_n114_));
  nor3   g063(.a(x25), .b(x24), .c(x23), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n88_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n71_), .c(new_n65_), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n92_), .c(new_n76_), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n114_), .c(new_n119_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x08), .O(new_n121_));
  aoi21  g070(.a(new_n120_), .b(x16), .c(new_n121_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(x18), .c(new_n59_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  nor2   g073(.a(x26), .b(x25), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n100_), .c(new_n82_), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n61_), .c(new_n54_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n100_), .c(new_n83_), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n124_), .c(new_n129_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g080(.a(x07), .O(new_n132_));
  aoi21  g081(.a(new_n62_), .b(new_n132_), .c(x18), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(z08));
  inv1   g083(.a(x28), .O(new_n135_));
  nor2   g084(.a(x27), .b(x26), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n108_), .c(new_n92_), .d(new_n74_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n61_), .c(new_n54_), .O(new_n138_));
  nor3   g087(.a(x28), .b(x27), .c(x26), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n115_), .c(new_n83_), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n135_), .c(new_n140_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(x16), .O(new_n142_));
  inv1   g091(.a(x06), .O(new_n143_));
  aoi21  g092(.a(new_n62_), .b(new_n143_), .c(x18), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n142_), .O(z09));
  inv1   g094(.a(x29), .O(new_n146_));
  nor2   g095(.a(x28), .b(x27), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n125_), .c(new_n100_), .d(new_n88_), .O(new_n148_));
  aoi21  g097(.a(new_n148_), .b(new_n71_), .c(new_n65_), .O(new_n149_));
  nor3   g098(.a(x29), .b(x28), .c(x27), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n118_), .O(new_n151_));
  oai22  g100(.a(new_n151_), .b(new_n93_), .c(new_n149_), .d(new_n146_), .O(new_n152_));
  nor2   g101(.a(x16), .b(x05), .O(new_n153_));
  aoi21  g102(.a(new_n152_), .b(x16), .c(new_n153_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(x18), .c(new_n59_), .O(z10));
  inv1   g104(.a(x30), .O(new_n156_));
  nand4  g105(.a(new_n150_), .b(new_n125_), .c(new_n100_), .d(new_n88_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n71_), .c(new_n65_), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n147_), .c(new_n118_), .O(new_n160_));
  oai22  g109(.a(new_n160_), .b(new_n93_), .c(new_n158_), .d(new_n156_), .O(new_n161_));
  nor2   g110(.a(x16), .b(x04), .O(new_n162_));
  aoi21  g111(.a(new_n161_), .b(x16), .c(new_n162_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(x18), .c(new_n59_), .O(z11));
  inv1   g113(.a(x31), .O(new_n165_));
  inv1   g114(.a(x22), .O(new_n166_));
  nand3  g115(.a(new_n100_), .b(new_n74_), .c(new_n166_), .O(new_n167_));
  nand3  g116(.a(new_n159_), .b(new_n128_), .c(new_n135_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n167_), .c(new_n61_), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n53_), .c(new_n165_), .O(new_n170_));
  nor2   g119(.a(x29), .b(x28), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n171_), .c(new_n128_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n101_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n170_), .c(x16), .O(new_n175_));
  aoi21  g124(.a(new_n62_), .b(new_n58_), .c(x18), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n175_), .O(z12));
  inv1   g126(.a(x32), .O(new_n178_));
  nand2  g127(.a(new_n115_), .b(new_n82_), .O(new_n179_));
  nand3  g128(.a(new_n172_), .b(new_n139_), .c(new_n146_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n179_), .c(new_n61_), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n53_), .c(new_n178_), .O(new_n182_));
  nand2  g131(.a(new_n147_), .b(new_n125_), .O(new_n183_));
  nand3  g132(.a(new_n159_), .b(new_n178_), .c(new_n165_), .O(new_n184_));
  nor3   g133(.a(new_n184_), .b(new_n183_), .c(new_n101_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n182_), .c(x16), .O(new_n186_));
  inv1   g135(.a(x02), .O(new_n187_));
  aoi21  g136(.a(new_n62_), .b(new_n187_), .c(x18), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n186_), .O(z13));
  inv1   g138(.a(x33), .O(new_n190_));
  inv1   g139(.a(new_n184_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n139_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n179_), .c(new_n61_), .O(new_n193_));
  aoi21  g142(.a(new_n193_), .b(new_n53_), .c(new_n190_), .O(new_n194_));
  nand2  g143(.a(new_n171_), .b(new_n136_), .O(new_n195_));
  nor2   g144(.a(x33), .b(x32), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n172_), .O(new_n197_));
  nor3   g146(.a(new_n197_), .b(new_n195_), .c(new_n109_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n194_), .c(x16), .O(new_n199_));
  inv1   g148(.a(x01), .O(new_n200_));
  aoi21  g149(.a(new_n62_), .b(new_n200_), .c(x18), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n199_), .O(z14));
  inv1   g151(.a(x34), .O(new_n203_));
  inv1   g152(.a(x24), .O(new_n204_));
  nand4  g153(.a(new_n125_), .b(new_n92_), .c(new_n204_), .d(new_n91_), .O(new_n205_));
  nand4  g154(.a(new_n196_), .b(new_n172_), .c(new_n171_), .d(new_n124_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n205_), .c(new_n71_), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n64_), .c(new_n203_), .O(new_n208_));
  nand4  g157(.a(new_n172_), .b(new_n203_), .c(new_n190_), .d(new_n178_), .O(new_n209_));
  nor3   g158(.a(new_n209_), .b(new_n195_), .c(new_n109_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n208_), .c(x16), .O(new_n211_));
  inv1   g160(.a(x00), .O(new_n212_));
  nand2  g161(.a(new_n62_), .b(new_n212_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(new_n211_), .O(new_n214_));
  nand2  g163(.a(new_n214_), .b(new_n61_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n59_), .O(z15));
endmodule


