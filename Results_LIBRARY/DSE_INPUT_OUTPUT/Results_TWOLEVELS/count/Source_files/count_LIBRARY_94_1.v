// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x07), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x00), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  oai21  g011(.a(new_n52_), .b(x00), .c(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n63_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n57_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n60_), .b(new_n76_), .c(x18), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n75_), .c(new_n53_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  aoi21  g028(.a(new_n72_), .b(new_n57_), .c(new_n79_), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n57_), .c(new_n64_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n53_), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n84_), .c(new_n63_), .O(z03));
  inv1   g036(.a(x23), .O(new_n88_));
  nor2   g037(.a(x20), .b(x19), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n89_), .c(new_n71_), .d(new_n54_), .O(new_n91_));
  oai21  g040(.a(new_n83_), .b(new_n88_), .c(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n53_), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n93_), .c(new_n63_), .O(z04));
  and2   g045(.a(new_n91_), .b(x24), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n81_), .c(new_n66_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n97_), .c(x16), .O(new_n101_));
  nor2   g050(.a(x16), .b(x10), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n53_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n101_), .c(new_n63_), .O(z05));
  inv1   g053(.a(x25), .O(new_n105_));
  nor2   g054(.a(x25), .b(x24), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n90_), .c(new_n72_), .d(new_n57_), .O(new_n107_));
  oai21  g056(.a(new_n100_), .b(new_n105_), .c(new_n107_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x16), .O(new_n109_));
  nor2   g058(.a(x16), .b(x09), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n53_), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n109_), .c(new_n63_), .O(z06));
  inv1   g061(.a(x26), .O(new_n113_));
  nand3  g062(.a(new_n106_), .b(new_n81_), .c(new_n88_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n55_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n66_), .c(new_n113_), .O(new_n116_));
  nor3   g065(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n90_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n73_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  inv1   g069(.a(x08), .O(new_n121_));
  aoi21  g070(.a(new_n60_), .b(new_n121_), .c(x18), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n120_), .c(new_n53_), .O(z07));
  aoi21  g072(.a(x16), .b(x00), .c(new_n52_), .O(new_n124_));
  nand3  g073(.a(new_n117_), .b(new_n90_), .c(new_n71_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n55_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n66_), .O(new_n127_));
  nor2   g076(.a(x27), .b(x26), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n98_), .c(new_n105_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n82_), .O(new_n130_));
  aoi21  g079(.a(new_n127_), .b(x27), .c(new_n130_), .O(new_n131_));
  inv1   g080(.a(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(x16), .O(new_n133_));
  aoi22  g082(.a(new_n133_), .b(new_n52_), .c(x18), .d(x00), .O(new_n134_));
  oai21  g083(.a(new_n131_), .b(new_n124_), .c(new_n134_), .O(z08));
  nor2   g084(.a(new_n128_), .b(x19), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n107_), .c(x28), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n106_), .c(new_n113_), .d(new_n88_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n82_), .c(new_n137_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x16), .O(new_n141_));
  nor2   g090(.a(x16), .b(x06), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n53_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n141_), .c(new_n63_), .O(z09));
  inv1   g093(.a(x29), .O(new_n145_));
  inv1   g094(.a(new_n136_), .O(new_n146_));
  inv1   g095(.a(x24), .O(new_n147_));
  nor2   g096(.a(x28), .b(x25), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n90_), .c(new_n147_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n73_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n146_), .c(new_n145_), .O(new_n151_));
  inv1   g100(.a(x27), .O(new_n152_));
  nor2   g101(.a(x29), .b(x28), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n117_), .c(new_n152_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n91_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n151_), .c(x16), .O(new_n156_));
  nor2   g105(.a(x16), .b(x05), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n53_), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n156_), .c(new_n63_), .O(z10));
  inv1   g108(.a(x30), .O(new_n160_));
  nand3  g109(.a(new_n153_), .b(new_n98_), .c(new_n105_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n82_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n146_), .c(new_n160_), .O(new_n163_));
  nor2   g112(.a(x30), .b(x29), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n138_), .c(new_n117_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n91_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n163_), .c(x16), .O(new_n167_));
  nor2   g116(.a(x16), .b(x04), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n53_), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n167_), .c(new_n63_), .O(z11));
  inv1   g119(.a(x31), .O(new_n171_));
  inv1   g120(.a(x28), .O(new_n172_));
  nand3  g121(.a(new_n164_), .b(new_n128_), .c(new_n172_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n114_), .c(new_n55_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n66_), .c(new_n171_), .O(new_n175_));
  nor2   g124(.a(x31), .b(x30), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n153_), .c(new_n128_), .d(new_n105_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n99_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n175_), .c(x16), .O(new_n179_));
  inv1   g128(.a(x03), .O(new_n180_));
  aoi21  g129(.a(new_n60_), .b(new_n180_), .c(x18), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n179_), .c(new_n53_), .O(z12));
  inv1   g131(.a(x32), .O(new_n183_));
  nand3  g132(.a(new_n171_), .b(new_n152_), .c(new_n113_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n55_), .O(new_n185_));
  nand4  g134(.a(new_n164_), .b(new_n106_), .c(new_n172_), .d(new_n88_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n82_), .O(new_n187_));
  aoi21  g136(.a(new_n187_), .b(new_n185_), .c(new_n183_), .O(new_n188_));
  nor2   g137(.a(x26), .b(x25), .O(new_n189_));
  nor2   g138(.a(x32), .b(x31), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n164_), .c(new_n138_), .d(new_n189_), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n99_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n188_), .c(x16), .O(new_n193_));
  nor2   g142(.a(x16), .b(x02), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n53_), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n193_), .c(new_n63_), .O(z13));
  inv1   g145(.a(x33), .O(new_n197_));
  nor2   g146(.a(x32), .b(x30), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n148_), .c(new_n145_), .d(new_n147_), .O(new_n199_));
  nor2   g148(.a(new_n199_), .b(new_n91_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n185_), .c(new_n197_), .O(new_n201_));
  nor2   g150(.a(x33), .b(x32), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n176_), .c(new_n153_), .d(new_n128_), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n107_), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n201_), .c(x16), .O(new_n205_));
  nor2   g154(.a(x16), .b(x01), .O(new_n206_));
  nor2   g155(.a(new_n206_), .b(new_n53_), .O(new_n207_));
  nand3  g156(.a(new_n207_), .b(new_n205_), .c(new_n63_), .O(z14));
  inv1   g157(.a(x00), .O(new_n209_));
  oai21  g158(.a(new_n60_), .b(x07), .c(new_n209_), .O(new_n210_));
  inv1   g159(.a(x34), .O(new_n211_));
  nand4  g160(.a(new_n202_), .b(new_n164_), .c(new_n148_), .d(new_n147_), .O(new_n212_));
  nor2   g161(.a(new_n212_), .b(new_n91_), .O(new_n213_));
  aoi21  g162(.a(new_n213_), .b(new_n185_), .c(new_n211_), .O(new_n214_));
  nor3   g163(.a(x34), .b(x33), .c(x32), .O(new_n215_));
  nand4  g164(.a(new_n215_), .b(new_n176_), .c(new_n153_), .d(new_n128_), .O(new_n216_));
  nor2   g165(.a(new_n216_), .b(new_n107_), .O(new_n217_));
  oai21  g166(.a(new_n217_), .b(new_n214_), .c(x16), .O(new_n218_));
  nand3  g167(.a(new_n218_), .b(new_n210_), .c(new_n63_), .O(z15));
endmodule


