// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor2   g011(.a(x20), .b(x19), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n62_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n58_), .b(new_n67_), .c(x18), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n66_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nand3  g019(.a(new_n55_), .b(new_n70_), .c(new_n61_), .O(new_n71_));
  inv1   g020(.a(new_n71_), .O(new_n72_));
  aoi21  g021(.a(new_n64_), .b(x21), .c(new_n72_), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  aoi21  g023(.a(new_n58_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n58_), .c(new_n75_), .O(z02));
  nand2  g025(.a(x22), .b(x21), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n61_), .c(new_n52_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(x19), .O(new_n80_));
  and2   g029(.a(new_n64_), .b(x22), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n58_), .b(new_n83_), .c(x18), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n82_), .O(z03));
  inv1   g034(.a(x18), .O(new_n86_));
  nor2   g035(.a(new_n78_), .b(x19), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(new_n64_), .c(x23), .O(new_n88_));
  nor2   g037(.a(x23), .b(x22), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n70_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n64_), .O(new_n91_));
  inv1   g040(.a(new_n91_), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n88_), .c(new_n58_), .O(new_n93_));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n93_), .c(new_n86_), .O(new_n95_));
  nand2  g044(.a(x33), .b(x18), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n95_), .O(z04));
  nor3   g046(.a(x23), .b(x22), .c(x21), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(x19), .c(new_n65_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n78_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n64_), .O(new_n102_));
  aoi21  g051(.a(new_n99_), .b(x24), .c(new_n102_), .O(new_n103_));
  inv1   g052(.a(x10), .O(new_n104_));
  aoi21  g053(.a(new_n58_), .b(new_n104_), .c(x18), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n58_), .c(new_n105_), .O(z05));
  inv1   g055(.a(x25), .O(new_n107_));
  aoi21  g056(.a(new_n101_), .b(new_n53_), .c(new_n64_), .O(new_n108_));
  nor2   g057(.a(x25), .b(x24), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n89_), .O(new_n110_));
  oai22  g059(.a(new_n110_), .b(new_n71_), .c(new_n108_), .d(new_n107_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x09), .O(new_n112_));
  aoi21  g061(.a(new_n111_), .b(x16), .c(new_n112_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(x18), .c(new_n96_), .O(z06));
  inv1   g063(.a(x26), .O(new_n115_));
  nor3   g064(.a(x25), .b(x24), .c(x23), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n78_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n53_), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(new_n65_), .c(new_n115_), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n89_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n71_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n119_), .c(x16), .O(new_n123_));
  inv1   g072(.a(x08), .O(new_n124_));
  aoi21  g073(.a(new_n58_), .b(new_n124_), .c(x18), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z07));
  inv1   g075(.a(x27), .O(new_n127_));
  nand2  g076(.a(new_n120_), .b(new_n98_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n53_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n65_), .c(new_n127_), .O(new_n130_));
  nand3  g079(.a(new_n78_), .b(new_n55_), .c(new_n61_), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n100_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n130_), .c(x16), .O(new_n135_));
  inv1   g084(.a(x07), .O(new_n136_));
  aoi21  g085(.a(new_n58_), .b(new_n136_), .c(x18), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n135_), .O(z08));
  inv1   g087(.a(x28), .O(new_n139_));
  nor2   g088(.a(x27), .b(x26), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n109_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n90_), .c(new_n53_), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n65_), .c(new_n139_), .O(new_n143_));
  nor2   g092(.a(x28), .b(x27), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n116_), .c(new_n115_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n131_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n143_), .c(x16), .O(new_n147_));
  inv1   g096(.a(x06), .O(new_n148_));
  aoi21  g097(.a(new_n58_), .b(new_n148_), .c(x18), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n147_), .O(z09));
  inv1   g099(.a(x29), .O(new_n151_));
  nor2   g100(.a(x26), .b(x25), .O(new_n152_));
  nand4  g101(.a(new_n144_), .b(new_n152_), .c(new_n100_), .d(new_n78_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n53_), .c(new_n64_), .O(new_n154_));
  nor3   g103(.a(x29), .b(x28), .c(x27), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n120_), .c(new_n91_), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n151_), .c(new_n156_), .O(new_n157_));
  nor2   g106(.a(x16), .b(x05), .O(new_n158_));
  aoi21  g107(.a(new_n157_), .b(x16), .c(new_n158_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(x18), .c(new_n96_), .O(z10));
  inv1   g109(.a(x30), .O(new_n161_));
  nand4  g110(.a(new_n155_), .b(new_n152_), .c(new_n100_), .d(new_n78_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n53_), .c(new_n64_), .O(new_n163_));
  nor2   g112(.a(x30), .b(x29), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n144_), .c(new_n120_), .d(new_n91_), .O(new_n165_));
  oai21  g114(.a(new_n163_), .b(new_n161_), .c(new_n165_), .O(new_n166_));
  nor2   g115(.a(x16), .b(x04), .O(new_n167_));
  aoi21  g116(.a(new_n166_), .b(x16), .c(new_n167_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(x18), .c(new_n96_), .O(z11));
  inv1   g118(.a(x31), .O(new_n170_));
  nor3   g119(.a(x30), .b(x29), .c(x28), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n140_), .c(new_n116_), .d(new_n78_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n53_), .c(new_n64_), .O(new_n173_));
  nor2   g122(.a(x29), .b(x28), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n132_), .d(new_n102_), .O(new_n176_));
  oai21  g125(.a(new_n173_), .b(new_n170_), .c(new_n176_), .O(new_n177_));
  nor2   g126(.a(x16), .b(x03), .O(new_n178_));
  aoi21  g127(.a(new_n177_), .b(x16), .c(new_n178_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(x18), .c(new_n96_), .O(z12));
  inv1   g129(.a(x32), .O(new_n181_));
  nand4  g130(.a(new_n175_), .b(new_n144_), .c(new_n151_), .d(new_n115_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n117_), .c(new_n53_), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n65_), .c(new_n181_), .O(new_n184_));
  nand2  g133(.a(new_n144_), .b(new_n152_), .O(new_n185_));
  nand3  g134(.a(new_n164_), .b(new_n181_), .c(new_n170_), .O(new_n186_));
  nor4   g135(.a(new_n186_), .b(new_n185_), .c(new_n101_), .d(new_n64_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n184_), .c(x16), .O(new_n188_));
  inv1   g137(.a(x02), .O(new_n189_));
  aoi21  g138(.a(new_n58_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(z13));
  inv1   g140(.a(new_n101_), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n63_), .c(new_n52_), .d(x16), .O(new_n193_));
  nor3   g142(.a(x33), .b(x32), .c(x31), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n164_), .c(new_n144_), .d(new_n152_), .O(new_n195_));
  oai22  g144(.a(new_n195_), .b(new_n193_), .c(x16), .d(x01), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n86_), .O(new_n197_));
  nor3   g146(.a(x32), .b(x31), .c(x30), .O(new_n198_));
  nand4  g147(.a(new_n198_), .b(new_n155_), .c(new_n120_), .d(new_n98_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n53_), .c(new_n64_), .O(new_n200_));
  nor2   g149(.a(new_n200_), .b(new_n58_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(x18), .c(x33), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n197_), .O(z14));
  inv1   g152(.a(x34), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n58_), .c(new_n86_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(x33), .O(new_n206_));
  nor2   g155(.a(new_n110_), .b(new_n71_), .O(new_n207_));
  nand2  g156(.a(new_n174_), .b(new_n140_), .O(new_n208_));
  inv1   g157(.a(new_n208_), .O(new_n209_));
  nor3   g158(.a(x34), .b(x33), .c(x32), .O(new_n210_));
  nand4  g159(.a(new_n210_), .b(new_n209_), .c(new_n175_), .d(new_n207_), .O(new_n211_));
  oai21  g160(.a(new_n200_), .b(new_n204_), .c(new_n211_), .O(new_n212_));
  nor2   g161(.a(x16), .b(x00), .O(new_n213_));
  aoi21  g162(.a(new_n212_), .b(x16), .c(new_n213_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(x18), .c(new_n206_), .O(z15));
endmodule


