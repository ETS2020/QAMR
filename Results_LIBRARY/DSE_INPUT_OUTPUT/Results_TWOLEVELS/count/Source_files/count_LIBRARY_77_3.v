// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_;
  inv1   g000(.a(x03), .O(new_n52_));
  inv1   g001(.a(x14), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x16), .O(new_n59_));
  inv1   g008(.a(x15), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n60_), .c(x18), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n59_), .c(new_n54_), .O(z00));
  inv1   g012(.a(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x16), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n53_), .O(new_n66_));
  inv1   g015(.a(x20), .O(new_n67_));
  nor2   g016(.a(new_n58_), .b(new_n67_), .O(new_n68_));
  nand3  g017(.a(new_n67_), .b(new_n56_), .c(new_n55_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  aoi21  g020(.a(x18), .b(new_n52_), .c(new_n54_), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n71_), .c(new_n66_), .O(z01));
  inv1   g022(.a(x21), .O(new_n74_));
  nand3  g023(.a(new_n58_), .b(new_n74_), .c(new_n67_), .O(new_n75_));
  oai21  g024(.a(new_n70_), .b(new_n74_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n61_), .b(new_n78_), .c(x18), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n54_), .O(z02));
  nand2  g029(.a(x22), .b(x21), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n67_), .c(new_n55_), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n81_), .c(x19), .O(new_n84_));
  and2   g033(.a(new_n69_), .b(x22), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n84_), .c(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n61_), .b(new_n87_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n54_), .O(z03));
  oai21  g038(.a(new_n53_), .b(new_n52_), .c(x18), .O(new_n90_));
  nand3  g039(.a(new_n82_), .b(new_n58_), .c(new_n67_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x23), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n70_), .c(new_n74_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  nor2   g045(.a(x16), .b(x11), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n54_), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n96_), .c(new_n90_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  nand2  g049(.a(new_n93_), .b(new_n74_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n56_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n70_), .c(new_n100_), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n82_), .c(new_n70_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n61_), .b(new_n108_), .c(x18), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n107_), .c(new_n54_), .O(z05));
  inv1   g059(.a(x25), .O(new_n111_));
  nand2  g060(.a(new_n104_), .b(new_n82_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n56_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n70_), .c(new_n111_), .O(new_n114_));
  nor2   g063(.a(x25), .b(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n93_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n75_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n114_), .c(x16), .O(new_n118_));
  inv1   g067(.a(x09), .O(new_n119_));
  aoi21  g068(.a(new_n61_), .b(new_n119_), .c(x18), .O(new_n120_));
  aoi21  g069(.a(new_n120_), .b(new_n118_), .c(new_n54_), .O(z06));
  inv1   g070(.a(x26), .O(new_n122_));
  inv1   g071(.a(x23), .O(new_n123_));
  nand3  g072(.a(new_n115_), .b(new_n82_), .c(new_n123_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n56_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n70_), .c(new_n122_), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n100_), .O(new_n128_));
  inv1   g077(.a(new_n128_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n93_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n75_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n126_), .c(x16), .O(new_n132_));
  inv1   g081(.a(x08), .O(new_n133_));
  aoi21  g082(.a(new_n61_), .b(new_n133_), .c(x18), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n132_), .c(new_n54_), .O(z07));
  inv1   g084(.a(x27), .O(new_n136_));
  oai21  g085(.a(new_n128_), .b(new_n101_), .c(new_n56_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n70_), .c(new_n136_), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n104_), .c(new_n111_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n91_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n138_), .c(x16), .O(new_n142_));
  inv1   g091(.a(x07), .O(new_n143_));
  aoi21  g092(.a(new_n61_), .b(new_n143_), .c(x18), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n142_), .c(new_n54_), .O(z08));
  inv1   g094(.a(x28), .O(new_n146_));
  nor2   g095(.a(x28), .b(x27), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n115_), .c(new_n122_), .d(new_n123_), .O(new_n148_));
  oai22  g097(.a(new_n148_), .b(new_n91_), .c(new_n141_), .d(new_n146_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  nor2   g099(.a(x16), .b(x06), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n54_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n150_), .c(new_n90_), .O(z09));
  inv1   g102(.a(x29), .O(new_n154_));
  nand4  g103(.a(new_n147_), .b(new_n127_), .c(new_n104_), .d(new_n82_), .O(new_n155_));
  aoi21  g104(.a(new_n155_), .b(new_n56_), .c(new_n69_), .O(new_n156_));
  nor2   g105(.a(x29), .b(x28), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n129_), .c(new_n136_), .O(new_n158_));
  oai22  g107(.a(new_n158_), .b(new_n94_), .c(new_n156_), .d(new_n154_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  inv1   g109(.a(x05), .O(new_n161_));
  aoi21  g110(.a(new_n61_), .b(new_n161_), .c(x18), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n160_), .c(new_n54_), .O(z10));
  inv1   g112(.a(x30), .O(new_n164_));
  nand3  g113(.a(new_n157_), .b(new_n127_), .c(new_n136_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n112_), .c(new_n56_), .O(new_n166_));
  aoi21  g115(.a(new_n166_), .b(new_n70_), .c(new_n164_), .O(new_n167_));
  nor2   g116(.a(x30), .b(x29), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n147_), .c(new_n129_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n94_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(x16), .O(new_n171_));
  inv1   g120(.a(x04), .O(new_n172_));
  aoi21  g121(.a(new_n61_), .b(new_n172_), .c(x18), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n171_), .c(new_n54_), .O(z11));
  oai21  g123(.a(new_n61_), .b(x14), .c(x03), .O(new_n175_));
  inv1   g124(.a(x31), .O(new_n176_));
  nand3  g125(.a(new_n168_), .b(new_n139_), .c(new_n146_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n124_), .c(new_n56_), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n70_), .c(new_n176_), .O(new_n179_));
  nor2   g128(.a(x31), .b(x30), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n157_), .c(new_n139_), .d(new_n111_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n105_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n179_), .c(new_n175_), .O(new_n183_));
  aoi22  g132(.a(new_n65_), .b(new_n52_), .c(x18), .d(new_n53_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n183_), .O(z12));
  inv1   g134(.a(x32), .O(new_n186_));
  nand4  g135(.a(new_n180_), .b(new_n147_), .c(new_n154_), .d(new_n122_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n124_), .c(new_n56_), .O(new_n188_));
  aoi21  g137(.a(new_n188_), .b(new_n70_), .c(new_n186_), .O(new_n189_));
  nor2   g138(.a(x32), .b(x31), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n168_), .c(new_n147_), .d(new_n127_), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n105_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n189_), .c(x16), .O(new_n193_));
  inv1   g142(.a(x02), .O(new_n194_));
  aoi21  g143(.a(new_n61_), .b(new_n194_), .c(x18), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n193_), .c(new_n54_), .O(z13));
  inv1   g145(.a(x33), .O(new_n197_));
  nand2  g146(.a(new_n157_), .b(new_n139_), .O(new_n198_));
  nand3  g147(.a(new_n180_), .b(new_n197_), .c(new_n186_), .O(new_n199_));
  nor2   g148(.a(new_n199_), .b(new_n198_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n117_), .O(new_n201_));
  oai21  g150(.a(new_n192_), .b(new_n197_), .c(new_n201_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(x16), .O(new_n203_));
  nor2   g152(.a(x16), .b(x01), .O(new_n204_));
  nor2   g153(.a(new_n204_), .b(new_n54_), .O(new_n205_));
  nand3  g154(.a(new_n205_), .b(new_n203_), .c(new_n90_), .O(z14));
  nand2  g155(.a(new_n201_), .b(x34), .O(new_n207_));
  inv1   g156(.a(new_n198_), .O(new_n208_));
  nor3   g157(.a(x34), .b(x33), .c(x32), .O(new_n209_));
  nand4  g158(.a(new_n209_), .b(new_n208_), .c(new_n180_), .d(new_n117_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n207_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(x16), .O(new_n212_));
  nor2   g161(.a(x16), .b(x00), .O(new_n213_));
  nor2   g162(.a(new_n213_), .b(new_n54_), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(new_n212_), .c(new_n90_), .O(z15));
endmodule


