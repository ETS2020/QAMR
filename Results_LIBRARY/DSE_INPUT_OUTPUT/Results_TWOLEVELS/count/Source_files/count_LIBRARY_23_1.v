// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_;
  inv1   g000(.a(x30), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x15), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  inv1   g004(.a(new_n55_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x16), .c(x18), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  nand3  g009(.a(new_n52_), .b(new_n60_), .c(new_n59_), .O(new_n61_));
  oai21  g010(.a(new_n58_), .b(new_n53_), .c(new_n61_), .O(z00));
  inv1   g011(.a(new_n53_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x18), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(new_n55_), .b(new_n65_), .O(new_n66_));
  nor3   g015(.a(x20), .b(x19), .c(x17), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n55_), .b(new_n72_), .c(new_n65_), .O(new_n73_));
  oai21  g022(.a(new_n67_), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n53_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n64_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  inv1   g028(.a(new_n73_), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n55_), .c(new_n65_), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n79_), .c(new_n82_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(x16), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n53_), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n84_), .c(new_n64_), .O(z03));
  inv1   g036(.a(new_n67_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x23), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n67_), .c(new_n72_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n89_), .c(new_n60_), .O(new_n92_));
  inv1   g041(.a(x18), .O(new_n93_));
  oai21  g042(.a(x16), .b(x11), .c(new_n93_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n92_), .c(new_n63_), .O(new_n95_));
  inv1   g044(.a(new_n81_), .O(new_n96_));
  oai21  g045(.a(x30), .b(x19), .c(new_n59_), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n96_), .c(x23), .d(x16), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n95_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  inv1   g049(.a(new_n91_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n81_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n88_), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n102_), .c(x16), .O(new_n106_));
  nor2   g055(.a(x16), .b(x10), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n53_), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n106_), .c(new_n64_), .O(z05));
  inv1   g058(.a(x25), .O(new_n110_));
  nor2   g059(.a(new_n105_), .b(new_n110_), .O(new_n111_));
  nand3  g060(.a(new_n90_), .b(new_n110_), .c(new_n100_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n73_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n111_), .c(x16), .O(new_n114_));
  nor2   g063(.a(x16), .b(x09), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n53_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n114_), .c(new_n64_), .O(z06));
  inv1   g066(.a(x26), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n90_), .O(new_n120_));
  oai22  g069(.a(new_n120_), .b(new_n73_), .c(new_n113_), .d(new_n118_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  nor2   g071(.a(x16), .b(x08), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n53_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(new_n64_), .O(z07));
  inv1   g074(.a(x27), .O(new_n126_));
  nor3   g075(.a(x27), .b(x26), .c(x25), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n103_), .O(new_n128_));
  oai22  g077(.a(new_n128_), .b(new_n82_), .c(new_n67_), .d(new_n126_), .O(new_n129_));
  oai21  g078(.a(x16), .b(x07), .c(new_n93_), .O(new_n130_));
  aoi21  g079(.a(new_n129_), .b(x16), .c(new_n130_), .O(new_n131_));
  nand3  g080(.a(new_n119_), .b(new_n90_), .c(new_n72_), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n97_), .c(x27), .d(x16), .O(new_n133_));
  oai21  g082(.a(new_n131_), .b(new_n53_), .c(new_n133_), .O(z08));
  oai21  g083(.a(new_n128_), .b(new_n82_), .c(x28), .O(new_n135_));
  nor3   g084(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nor2   g085(.a(x28), .b(x27), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n136_), .c(new_n118_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n82_), .c(new_n135_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(x16), .O(new_n140_));
  nor2   g089(.a(x16), .b(x06), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n53_), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n140_), .c(new_n64_), .O(z09));
  inv1   g092(.a(x29), .O(new_n144_));
  nor2   g093(.a(x29), .b(x28), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n119_), .c(new_n126_), .O(new_n146_));
  oai22  g095(.a(new_n146_), .b(new_n91_), .c(new_n67_), .d(new_n144_), .O(new_n147_));
  oai21  g096(.a(x16), .b(x05), .c(new_n93_), .O(new_n148_));
  aoi21  g097(.a(new_n147_), .b(x16), .c(new_n148_), .O(new_n149_));
  inv1   g098(.a(new_n104_), .O(new_n150_));
  nor2   g099(.a(x26), .b(x25), .O(new_n151_));
  nand2  g100(.a(new_n137_), .b(new_n151_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n97_), .c(x29), .d(x16), .O(new_n155_));
  oai21  g104(.a(new_n149_), .b(new_n53_), .c(new_n155_), .O(z10));
  inv1   g105(.a(x19), .O(new_n157_));
  nand3  g106(.a(new_n145_), .b(new_n151_), .c(new_n126_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n104_), .c(new_n157_), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n65_), .c(new_n59_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n56_), .c(x30), .O(new_n161_));
  nor2   g110(.a(x30), .b(x29), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n137_), .c(new_n119_), .d(new_n101_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(x16), .O(new_n165_));
  nor2   g114(.a(x16), .b(x04), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n53_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n165_), .c(new_n64_), .O(z11));
  nor2   g117(.a(x19), .b(new_n60_), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(x31), .c(x30), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n93_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x15), .O(new_n172_));
  aoi21  g121(.a(new_n159_), .b(new_n65_), .c(x30), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n56_), .c(x31), .O(new_n174_));
  inv1   g123(.a(x31), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n52_), .O(new_n176_));
  inv1   g125(.a(new_n176_), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n145_), .c(new_n127_), .d(new_n105_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n174_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(x16), .O(new_n180_));
  aoi21  g129(.a(new_n52_), .b(x18), .c(new_n53_), .O(new_n181_));
  oai21  g130(.a(x16), .b(x03), .c(new_n181_), .O(new_n182_));
  inv1   g131(.a(new_n182_), .O(new_n183_));
  nand3  g132(.a(new_n183_), .b(new_n180_), .c(new_n172_), .O(z12));
  nand3  g133(.a(new_n169_), .b(x32), .c(x30), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n93_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(x15), .O(new_n187_));
  inv1   g136(.a(x17), .O(new_n188_));
  nand3  g137(.a(new_n175_), .b(new_n157_), .c(new_n188_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n173_), .c(x32), .O(new_n190_));
  nor2   g139(.a(x32), .b(x31), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n162_), .c(new_n153_), .d(new_n105_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n190_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(x16), .O(new_n194_));
  oai21  g143(.a(x16), .b(x02), .c(new_n181_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  nand3  g145(.a(new_n196_), .b(new_n194_), .c(new_n187_), .O(z13));
  nand3  g146(.a(new_n169_), .b(x33), .c(x30), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n93_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(x15), .O(new_n200_));
  nand2  g149(.a(new_n191_), .b(new_n55_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(new_n173_), .c(x33), .O(new_n202_));
  nand3  g151(.a(new_n145_), .b(new_n126_), .c(new_n118_), .O(new_n203_));
  inv1   g152(.a(new_n203_), .O(new_n204_));
  nor2   g153(.a(x33), .b(x32), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n204_), .c(new_n177_), .d(new_n113_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n202_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(x16), .O(new_n208_));
  oai21  g157(.a(x16), .b(x01), .c(new_n181_), .O(new_n209_));
  inv1   g158(.a(new_n209_), .O(new_n210_));
  nand3  g159(.a(new_n210_), .b(new_n208_), .c(new_n200_), .O(z14));
  nand3  g160(.a(new_n169_), .b(x34), .c(x30), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n93_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(x15), .O(new_n214_));
  nand3  g163(.a(new_n205_), .b(new_n55_), .c(new_n175_), .O(new_n215_));
  oai21  g164(.a(new_n215_), .b(new_n173_), .c(x34), .O(new_n216_));
  nor3   g165(.a(x34), .b(x33), .c(x32), .O(new_n217_));
  nand4  g166(.a(new_n217_), .b(new_n204_), .c(new_n177_), .d(new_n113_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n216_), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(x16), .O(new_n220_));
  oai21  g169(.a(x16), .b(x00), .c(new_n181_), .O(new_n221_));
  inv1   g170(.a(new_n221_), .O(new_n222_));
  nand3  g171(.a(new_n222_), .b(new_n220_), .c(new_n214_), .O(z15));
endmodule


