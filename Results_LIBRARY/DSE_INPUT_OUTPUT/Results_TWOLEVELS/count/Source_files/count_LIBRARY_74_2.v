// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(new_n55_), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n53_), .c(new_n52_), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  aoi21  g013(.a(new_n61_), .b(x20), .c(new_n64_), .O(new_n65_));
  inv1   g014(.a(x14), .O(new_n66_));
  aoi21  g015(.a(new_n58_), .b(new_n66_), .c(x18), .O(new_n67_));
  oai21  g016(.a(new_n65_), .b(new_n58_), .c(new_n67_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nand3  g018(.a(new_n55_), .b(new_n69_), .c(new_n62_), .O(new_n70_));
  oai21  g019(.a(new_n64_), .b(new_n69_), .c(new_n70_), .O(new_n71_));
  nor2   g020(.a(x16), .b(x13), .O(new_n72_));
  aoi21  g021(.a(new_n71_), .b(x16), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(x32), .b(x18), .O(new_n74_));
  oai21  g023(.a(new_n73_), .b(x18), .c(new_n74_), .O(z02));
  inv1   g024(.a(x22), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nor2   g026(.a(x20), .b(x17), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  oai22  g029(.a(new_n80_), .b(x19), .c(new_n64_), .d(new_n76_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  aoi21  g031(.a(new_n81_), .b(x16), .c(new_n82_), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(x18), .c(new_n74_), .O(z03));
  inv1   g033(.a(x18), .O(new_n85_));
  nor2   g034(.a(new_n79_), .b(x19), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n63_), .c(x23), .O(new_n87_));
  nor3   g036(.a(x23), .b(x22), .c(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n64_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n58_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n90_), .c(new_n85_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n74_), .O(z04));
  oai21  g042(.a(new_n88_), .b(x19), .c(new_n64_), .O(new_n94_));
  nor2   g043(.a(x24), .b(x23), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n79_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n63_), .O(new_n97_));
  aoi21  g046(.a(new_n94_), .b(x24), .c(new_n97_), .O(new_n98_));
  inv1   g047(.a(x10), .O(new_n99_));
  aoi21  g048(.a(new_n58_), .b(new_n99_), .c(x18), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n58_), .c(new_n100_), .O(z05));
  inv1   g050(.a(x25), .O(new_n102_));
  nand2  g051(.a(new_n96_), .b(new_n53_), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n64_), .c(new_n102_), .O(new_n104_));
  nor2   g053(.a(x23), .b(x22), .O(new_n105_));
  nor2   g054(.a(x25), .b(x24), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n70_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n104_), .c(x16), .O(new_n109_));
  inv1   g058(.a(x09), .O(new_n110_));
  aoi21  g059(.a(new_n58_), .b(new_n110_), .c(x18), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n109_), .O(z06));
  inv1   g061(.a(x26), .O(new_n113_));
  nor3   g062(.a(x25), .b(x24), .c(x23), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n79_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n53_), .c(new_n63_), .O(new_n116_));
  nor3   g065(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n105_), .O(new_n118_));
  oai22  g067(.a(new_n118_), .b(new_n70_), .c(new_n116_), .d(new_n113_), .O(new_n119_));
  nor2   g068(.a(x16), .b(x08), .O(new_n120_));
  aoi21  g069(.a(new_n119_), .b(x16), .c(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(x18), .c(new_n74_), .O(z07));
  inv1   g071(.a(x27), .O(new_n123_));
  nand2  g072(.a(new_n117_), .b(new_n88_), .O(new_n124_));
  aoi21  g073(.a(new_n124_), .b(new_n53_), .c(new_n63_), .O(new_n125_));
  nand2  g074(.a(new_n79_), .b(new_n62_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n61_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(new_n95_), .O(new_n129_));
  oai21  g078(.a(new_n125_), .b(new_n123_), .c(new_n129_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x07), .O(new_n131_));
  aoi21  g080(.a(new_n130_), .b(x16), .c(new_n131_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(x18), .c(new_n74_), .O(z08));
  inv1   g082(.a(x28), .O(new_n134_));
  nor2   g083(.a(x27), .b(x26), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n106_), .c(new_n88_), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n53_), .c(new_n63_), .O(new_n137_));
  nor3   g086(.a(x28), .b(x27), .c(x26), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n127_), .c(new_n114_), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n134_), .c(new_n139_), .O(new_n140_));
  nor2   g089(.a(x16), .b(x06), .O(new_n141_));
  aoi21  g090(.a(new_n140_), .b(x16), .c(new_n141_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(x18), .c(new_n74_), .O(z09));
  inv1   g092(.a(x29), .O(new_n144_));
  nor2   g093(.a(x26), .b(x25), .O(new_n145_));
  nor2   g094(.a(x28), .b(x27), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n145_), .c(new_n95_), .d(new_n79_), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n53_), .c(new_n63_), .O(new_n148_));
  nor3   g097(.a(x29), .b(x28), .c(x27), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n117_), .O(new_n150_));
  oai22  g099(.a(new_n150_), .b(new_n89_), .c(new_n148_), .d(new_n144_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x05), .O(new_n152_));
  aoi21  g101(.a(new_n151_), .b(x16), .c(new_n152_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(x18), .c(new_n74_), .O(z10));
  inv1   g103(.a(x30), .O(new_n155_));
  nand2  g104(.a(new_n149_), .b(new_n145_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n96_), .c(new_n53_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n64_), .c(new_n155_), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n146_), .c(new_n117_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n89_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n158_), .c(x16), .O(new_n162_));
  inv1   g111(.a(x04), .O(new_n163_));
  aoi21  g112(.a(new_n58_), .b(new_n163_), .c(x18), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(z11));
  inv1   g114(.a(x31), .O(new_n166_));
  nor3   g115(.a(x30), .b(x29), .c(x28), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n135_), .c(new_n114_), .d(new_n79_), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n53_), .c(new_n63_), .O(new_n169_));
  nor2   g118(.a(x29), .b(x28), .O(new_n170_));
  nor2   g119(.a(x31), .b(x30), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n170_), .c(new_n128_), .d(new_n97_), .O(new_n172_));
  oai21  g121(.a(new_n169_), .b(new_n166_), .c(new_n172_), .O(new_n173_));
  nor2   g122(.a(x16), .b(x03), .O(new_n174_));
  aoi21  g123(.a(new_n173_), .b(x16), .c(new_n174_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(x18), .c(new_n74_), .O(z12));
  inv1   g125(.a(x32), .O(new_n177_));
  nor3   g126(.a(x31), .b(x30), .c(x29), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n138_), .c(new_n114_), .d(new_n79_), .O(new_n179_));
  aoi21  g128(.a(new_n179_), .b(new_n53_), .c(new_n63_), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n177_), .O(new_n181_));
  nand2  g130(.a(new_n146_), .b(new_n145_), .O(new_n182_));
  nand3  g131(.a(new_n159_), .b(new_n177_), .c(new_n166_), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n97_), .c(new_n181_), .O(new_n185_));
  inv1   g134(.a(x02), .O(new_n186_));
  aoi21  g135(.a(new_n58_), .b(new_n186_), .c(x18), .O(new_n187_));
  oai21  g136(.a(new_n185_), .b(new_n58_), .c(new_n187_), .O(z13));
  inv1   g137(.a(x33), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n58_), .c(new_n85_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(x32), .O(new_n191_));
  nand2  g140(.a(new_n170_), .b(new_n135_), .O(new_n192_));
  nand3  g141(.a(new_n171_), .b(new_n189_), .c(new_n177_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n192_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n108_), .O(new_n195_));
  oai21  g144(.a(new_n180_), .b(new_n189_), .c(new_n195_), .O(new_n196_));
  nor2   g145(.a(x16), .b(x01), .O(new_n197_));
  aoi21  g146(.a(new_n196_), .b(x16), .c(new_n197_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(x18), .c(new_n191_), .O(z14));
  inv1   g148(.a(x34), .O(new_n200_));
  inv1   g149(.a(new_n193_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n149_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n124_), .c(new_n53_), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n64_), .c(new_n200_), .O(new_n204_));
  nand4  g153(.a(new_n171_), .b(new_n200_), .c(new_n189_), .d(new_n177_), .O(new_n205_));
  nor4   g154(.a(new_n205_), .b(new_n192_), .c(new_n107_), .d(new_n70_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n204_), .c(x16), .O(new_n207_));
  inv1   g156(.a(x00), .O(new_n208_));
  aoi21  g157(.a(new_n58_), .b(new_n208_), .c(x18), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n207_), .O(z15));
endmodule


