// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x18), .b(x15), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x16), .O(new_n60_));
  inv1   g009(.a(x17), .O(new_n61_));
  inv1   g010(.a(x19), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n54_), .b(x20), .c(new_n65_), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n60_), .b(new_n67_), .c(x18), .O(new_n68_));
  oai21  g017(.a(new_n66_), .b(new_n60_), .c(new_n68_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nand3  g019(.a(new_n53_), .b(new_n70_), .c(new_n63_), .O(new_n71_));
  oai21  g020(.a(new_n65_), .b(new_n70_), .c(new_n71_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  aoi21  g022(.a(new_n72_), .b(x16), .c(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(x18), .c(new_n58_), .O(z02));
  inv1   g024(.a(x22), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n70_), .O(new_n77_));
  nor2   g026(.a(x20), .b(x17), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  oai22  g029(.a(new_n80_), .b(x19), .c(new_n65_), .d(new_n76_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  aoi21  g031(.a(new_n81_), .b(x16), .c(new_n82_), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(x18), .c(new_n58_), .O(z03));
  inv1   g033(.a(x23), .O(new_n85_));
  inv1   g034(.a(x18), .O(new_n86_));
  oai21  g035(.a(new_n79_), .b(x19), .c(new_n63_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n53_), .c(new_n85_), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n70_), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n64_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n89_), .c(x16), .O(new_n93_));
  inv1   g042(.a(x11), .O(new_n94_));
  aoi21  g043(.a(new_n60_), .b(new_n94_), .c(x18), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n93_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  aoi21  g046(.a(new_n91_), .b(new_n62_), .c(new_n64_), .O(new_n98_));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n79_), .c(new_n65_), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  nor2   g050(.a(x16), .b(x10), .O(new_n102_));
  aoi21  g051(.a(new_n101_), .b(x16), .c(new_n102_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(x18), .c(new_n58_), .O(z05));
  nand2  g053(.a(new_n99_), .b(new_n79_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n62_), .c(x20), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(x18), .c(new_n53_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n90_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n71_), .O(new_n110_));
  aoi21  g059(.a(new_n107_), .b(x25), .c(new_n110_), .O(new_n111_));
  inv1   g060(.a(x09), .O(new_n112_));
  aoi21  g061(.a(new_n60_), .b(new_n112_), .c(x18), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n60_), .c(new_n113_), .O(z06));
  nor3   g063(.a(x25), .b(x24), .c(x23), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n79_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n62_), .c(x20), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(x18), .c(new_n53_), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n90_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n71_), .O(new_n121_));
  aoi21  g070(.a(new_n118_), .b(x26), .c(new_n121_), .O(new_n122_));
  inv1   g071(.a(x08), .O(new_n123_));
  aoi21  g072(.a(new_n60_), .b(new_n123_), .c(x18), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n60_), .c(new_n124_), .O(z07));
  inv1   g074(.a(x27), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n90_), .c(new_n97_), .d(new_n70_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n62_), .c(new_n64_), .O(new_n129_));
  inv1   g078(.a(x25), .O(new_n130_));
  nand2  g079(.a(new_n79_), .b(new_n63_), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n54_), .O(new_n132_));
  nor2   g081(.a(x27), .b(x26), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n132_), .c(new_n99_), .d(new_n130_), .O(new_n134_));
  oai21  g083(.a(new_n129_), .b(new_n126_), .c(new_n134_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x07), .O(new_n136_));
  aoi21  g085(.a(new_n135_), .b(x16), .c(new_n136_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(x18), .c(new_n58_), .O(z08));
  inv1   g087(.a(x28), .O(new_n139_));
  nor3   g088(.a(x23), .b(x22), .c(x21), .O(new_n140_));
  nand3  g089(.a(new_n133_), .b(new_n108_), .c(new_n140_), .O(new_n141_));
  aoi21  g090(.a(new_n141_), .b(new_n62_), .c(new_n64_), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n132_), .c(new_n115_), .O(new_n144_));
  oai21  g093(.a(new_n142_), .b(new_n139_), .c(new_n144_), .O(new_n145_));
  nor2   g094(.a(x16), .b(x06), .O(new_n146_));
  aoi21  g095(.a(new_n145_), .b(x16), .c(new_n146_), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(x18), .c(new_n58_), .O(z09));
  nor2   g097(.a(x28), .b(x27), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n127_), .c(new_n99_), .d(new_n79_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n62_), .c(x20), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(x18), .c(new_n53_), .O(new_n152_));
  nor3   g101(.a(x29), .b(x28), .c(x27), .O(new_n153_));
  and2   g102(.a(new_n153_), .b(new_n119_), .O(new_n154_));
  aoi22  g103(.a(new_n154_), .b(new_n92_), .c(new_n152_), .d(x29), .O(new_n155_));
  inv1   g104(.a(x05), .O(new_n156_));
  aoi21  g105(.a(new_n60_), .b(new_n156_), .c(x18), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n60_), .c(new_n157_), .O(z10));
  inv1   g107(.a(x30), .O(new_n159_));
  nand4  g108(.a(new_n153_), .b(new_n127_), .c(new_n99_), .d(new_n79_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n62_), .c(new_n64_), .O(new_n161_));
  nor2   g110(.a(x30), .b(x29), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n149_), .c(new_n119_), .d(new_n92_), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n159_), .c(new_n163_), .O(new_n164_));
  nor2   g113(.a(x16), .b(x04), .O(new_n165_));
  aoi21  g114(.a(new_n164_), .b(x16), .c(new_n165_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(x18), .c(new_n58_), .O(z11));
  nor3   g116(.a(x30), .b(x29), .c(x28), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n133_), .c(new_n115_), .d(new_n79_), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n62_), .c(x20), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(x18), .c(new_n53_), .O(new_n171_));
  nor2   g120(.a(x29), .b(x28), .O(new_n172_));
  nor2   g121(.a(x31), .b(x30), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n172_), .c(new_n133_), .d(new_n130_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n100_), .O(new_n175_));
  aoi21  g124(.a(new_n171_), .b(x31), .c(new_n175_), .O(new_n176_));
  inv1   g125(.a(x03), .O(new_n177_));
  aoi21  g126(.a(new_n60_), .b(new_n177_), .c(x18), .O(new_n178_));
  oai21  g127(.a(new_n176_), .b(new_n60_), .c(new_n178_), .O(z12));
  inv1   g128(.a(x32), .O(new_n180_));
  nor3   g129(.a(x31), .b(x30), .c(x29), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n143_), .c(new_n115_), .d(new_n79_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n62_), .c(new_n64_), .O(new_n183_));
  nor2   g132(.a(x32), .b(x31), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n162_), .c(new_n149_), .d(new_n127_), .O(new_n185_));
  oai22  g134(.a(new_n185_), .b(new_n100_), .c(new_n183_), .d(new_n180_), .O(new_n186_));
  nor2   g135(.a(x16), .b(x02), .O(new_n187_));
  aoi21  g136(.a(new_n186_), .b(x16), .c(new_n187_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(x18), .c(new_n58_), .O(z13));
  inv1   g138(.a(x33), .O(new_n190_));
  nor3   g139(.a(x32), .b(x31), .c(x30), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n153_), .c(new_n119_), .d(new_n140_), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n62_), .c(new_n64_), .O(new_n193_));
  nand2  g142(.a(new_n172_), .b(new_n133_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  nor2   g144(.a(x33), .b(x32), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n195_), .c(new_n173_), .d(new_n110_), .O(new_n197_));
  oai21  g146(.a(new_n193_), .b(new_n190_), .c(new_n197_), .O(new_n198_));
  nor2   g147(.a(x16), .b(x01), .O(new_n199_));
  aoi21  g148(.a(new_n198_), .b(x16), .c(new_n199_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(x18), .c(new_n58_), .O(z14));
  nand4  g150(.a(new_n196_), .b(new_n173_), .c(new_n172_), .d(new_n126_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n128_), .c(new_n62_), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n63_), .c(x18), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n54_), .c(x34), .O(new_n205_));
  nor3   g154(.a(x34), .b(x33), .c(x32), .O(new_n206_));
  nand4  g155(.a(new_n206_), .b(new_n195_), .c(new_n173_), .d(new_n110_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n205_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(x16), .O(new_n209_));
  inv1   g158(.a(x00), .O(new_n210_));
  aoi21  g159(.a(new_n60_), .b(new_n210_), .c(x18), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n209_), .O(z15));
endmodule


