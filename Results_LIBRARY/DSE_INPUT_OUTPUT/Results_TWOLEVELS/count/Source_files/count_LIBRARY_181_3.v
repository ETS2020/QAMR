// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(new_n55_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  aoi21  g011(.a(new_n61_), .b(x20), .c(new_n62_), .O(new_n63_));
  inv1   g012(.a(x14), .O(new_n64_));
  aoi21  g013(.a(new_n58_), .b(new_n64_), .c(x18), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n58_), .c(new_n65_), .O(z01));
  nand2  g015(.a(x21), .b(x20), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n67_), .c(x17), .O(new_n70_));
  inv1   g019(.a(x21), .O(new_n71_));
  nor3   g020(.a(new_n55_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  inv1   g023(.a(x18), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(x17), .O(new_n76_));
  nor2   g025(.a(x18), .b(x16), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n74_), .c(new_n76_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n73_), .O(z02));
  nand2  g028(.a(new_n68_), .b(new_n55_), .O(new_n80_));
  inv1   g029(.a(x20), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g032(.a(new_n83_), .b(new_n61_), .O(new_n84_));
  aoi21  g033(.a(new_n80_), .b(x22), .c(new_n84_), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  aoi21  g035(.a(new_n58_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n58_), .c(new_n87_), .O(z03));
  nand2  g037(.a(x23), .b(x22), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n71_), .c(new_n81_), .d(new_n52_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n89_), .c(x19), .O(new_n92_));
  inv1   g041(.a(x23), .O(new_n93_));
  aoi21  g042(.a(new_n68_), .b(new_n55_), .c(new_n93_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n58_), .b(new_n96_), .c(x18), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n95_), .O(z04));
  inv1   g047(.a(x24), .O(new_n99_));
  nand2  g048(.a(new_n61_), .b(new_n75_), .O(new_n100_));
  nand2  g049(.a(new_n90_), .b(new_n68_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n52_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n82_), .c(new_n62_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n77_), .b(new_n108_), .c(new_n76_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n107_), .O(z05));
  nor3   g059(.a(x24), .b(x21), .c(x20), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n90_), .c(new_n53_), .d(new_n52_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(x25), .O(new_n113_));
  nor2   g062(.a(x25), .b(x24), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n90_), .c(new_n68_), .d(new_n55_), .O(new_n115_));
  and2   g064(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  inv1   g065(.a(x09), .O(new_n117_));
  aoi21  g066(.a(new_n58_), .b(new_n117_), .c(x18), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n58_), .c(new_n118_), .O(z06));
  inv1   g068(.a(x26), .O(new_n120_));
  nand4  g069(.a(new_n114_), .b(new_n82_), .c(new_n93_), .d(new_n81_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n100_), .c(new_n120_), .O(new_n123_));
  nor2   g072(.a(x26), .b(x25), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n90_), .c(new_n99_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n80_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n123_), .c(x16), .O(new_n127_));
  inv1   g076(.a(x08), .O(new_n128_));
  aoi21  g077(.a(new_n77_), .b(new_n128_), .c(new_n76_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(z07));
  inv1   g079(.a(x27), .O(new_n131_));
  inv1   g080(.a(x22), .O(new_n132_));
  inv1   g081(.a(x25), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n93_), .c(new_n132_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n53_), .O(new_n135_));
  nor3   g084(.a(x26), .b(x24), .c(x21), .O(new_n136_));
  and2   g085(.a(new_n136_), .b(new_n62_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n135_), .c(new_n131_), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n104_), .c(new_n133_), .O(new_n140_));
  nor3   g089(.a(new_n140_), .b(new_n83_), .c(new_n61_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n138_), .c(x16), .O(new_n142_));
  inv1   g091(.a(x07), .O(new_n143_));
  aoi21  g092(.a(new_n58_), .b(new_n143_), .c(x18), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n142_), .O(z08));
  nand4  g094(.a(new_n62_), .b(new_n120_), .c(new_n99_), .d(new_n71_), .O(new_n146_));
  nor2   g095(.a(x27), .b(x25), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n90_), .c(x19), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n146_), .c(x28), .O(new_n149_));
  nor3   g098(.a(x28), .b(x27), .c(x26), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n114_), .c(new_n84_), .d(new_n93_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  inv1   g102(.a(x06), .O(new_n154_));
  aoi21  g103(.a(new_n58_), .b(new_n154_), .c(x18), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n153_), .O(z09));
  inv1   g105(.a(x29), .O(new_n157_));
  nor2   g106(.a(x28), .b(x27), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n90_), .c(new_n133_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n53_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n137_), .c(new_n157_), .O(new_n161_));
  nand2  g110(.a(new_n124_), .b(new_n99_), .O(new_n162_));
  nand3  g111(.a(new_n90_), .b(new_n62_), .c(new_n71_), .O(new_n163_));
  nor2   g112(.a(x29), .b(x28), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n131_), .O(new_n165_));
  nor3   g114(.a(new_n165_), .b(new_n163_), .c(new_n162_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n161_), .c(x16), .O(new_n167_));
  inv1   g116(.a(x05), .O(new_n168_));
  aoi21  g117(.a(new_n58_), .b(new_n168_), .c(x18), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n167_), .O(z10));
  inv1   g119(.a(x30), .O(new_n171_));
  oai21  g120(.a(new_n165_), .b(new_n134_), .c(new_n53_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n137_), .c(new_n171_), .O(new_n173_));
  nor2   g122(.a(x30), .b(x29), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n158_), .O(new_n175_));
  nor3   g124(.a(new_n175_), .b(new_n163_), .c(new_n162_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n173_), .c(x16), .O(new_n177_));
  inv1   g126(.a(x04), .O(new_n178_));
  aoi21  g127(.a(new_n58_), .b(new_n178_), .c(x18), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z11));
  inv1   g129(.a(x31), .O(new_n181_));
  oai21  g130(.a(new_n175_), .b(new_n134_), .c(new_n53_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n137_), .c(new_n181_), .O(new_n183_));
  nor2   g132(.a(x31), .b(x30), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n164_), .c(new_n139_), .d(new_n133_), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n105_), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n183_), .c(x16), .O(new_n187_));
  inv1   g136(.a(x03), .O(new_n188_));
  aoi21  g137(.a(new_n58_), .b(new_n188_), .c(x18), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n187_), .O(z12));
  inv1   g139(.a(x32), .O(new_n191_));
  nand4  g140(.a(new_n184_), .b(new_n158_), .c(new_n157_), .d(new_n120_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n121_), .c(new_n52_), .O(new_n193_));
  aoi21  g142(.a(new_n193_), .b(new_n100_), .c(new_n191_), .O(new_n194_));
  nor2   g143(.a(x32), .b(x31), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n174_), .c(new_n158_), .d(new_n124_), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n105_), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n194_), .c(x16), .O(new_n198_));
  inv1   g147(.a(x02), .O(new_n199_));
  aoi21  g148(.a(new_n77_), .b(new_n199_), .c(new_n76_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n198_), .O(z13));
  inv1   g150(.a(x33), .O(new_n202_));
  nand4  g151(.a(new_n195_), .b(new_n174_), .c(new_n158_), .d(new_n120_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n121_), .c(new_n52_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(new_n100_), .c(new_n202_), .O(new_n205_));
  nand2  g154(.a(new_n164_), .b(new_n139_), .O(new_n206_));
  nor2   g155(.a(x33), .b(x32), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n184_), .O(new_n208_));
  nor3   g157(.a(new_n208_), .b(new_n206_), .c(new_n115_), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(new_n205_), .c(x16), .O(new_n210_));
  inv1   g159(.a(x01), .O(new_n211_));
  aoi21  g160(.a(new_n77_), .b(new_n211_), .c(new_n76_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n210_), .O(z14));
  inv1   g162(.a(x34), .O(new_n214_));
  nand4  g163(.a(new_n124_), .b(new_n104_), .c(new_n82_), .d(new_n81_), .O(new_n215_));
  nand4  g164(.a(new_n207_), .b(new_n184_), .c(new_n164_), .d(new_n131_), .O(new_n216_));
  oai21  g165(.a(new_n216_), .b(new_n215_), .c(new_n52_), .O(new_n217_));
  aoi21  g166(.a(new_n217_), .b(new_n100_), .c(new_n214_), .O(new_n218_));
  nand4  g167(.a(new_n184_), .b(new_n214_), .c(new_n202_), .d(new_n191_), .O(new_n219_));
  nor3   g168(.a(new_n219_), .b(new_n206_), .c(new_n115_), .O(new_n220_));
  oai21  g169(.a(new_n220_), .b(new_n218_), .c(x16), .O(new_n221_));
  inv1   g170(.a(x00), .O(new_n222_));
  aoi21  g171(.a(new_n77_), .b(new_n222_), .c(new_n76_), .O(new_n223_));
  nand2  g172(.a(new_n223_), .b(new_n221_), .O(z15));
endmodule


