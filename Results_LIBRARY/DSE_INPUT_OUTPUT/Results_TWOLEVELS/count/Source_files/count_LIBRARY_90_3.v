// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x07), .O(new_n52_));
  inv1   g001(.a(x10), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x16), .O(new_n59_));
  inv1   g008(.a(x15), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n60_), .c(x18), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n59_), .c(new_n54_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n58_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n61_), .b(new_n68_), .c(x18), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n67_), .c(new_n54_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n58_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n61_), .b(new_n76_), .c(x18), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n75_), .c(new_n54_), .O(z02));
  oai21  g027(.a(new_n53_), .b(new_n52_), .c(x18), .O(new_n79_));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(new_n73_), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n58_), .c(new_n64_), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n80_), .c(new_n83_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n54_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n79_), .O(z03));
  and2   g037(.a(new_n83_), .b(x23), .O(new_n89_));
  nand3  g038(.a(new_n64_), .b(new_n56_), .c(new_n55_), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n71_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n89_), .c(x16), .O(new_n94_));
  nor2   g043(.a(x16), .b(x11), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n54_), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n94_), .c(new_n79_), .O(z04));
  aoi21  g046(.a(x16), .b(new_n52_), .c(new_n53_), .O(new_n98_));
  inv1   g047(.a(new_n92_), .O(new_n99_));
  oai21  g048(.a(new_n99_), .b(x19), .c(new_n66_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n82_), .c(new_n66_), .O(new_n102_));
  inv1   g051(.a(new_n102_), .O(new_n103_));
  aoi21  g052(.a(new_n100_), .b(x24), .c(new_n103_), .O(new_n104_));
  inv1   g053(.a(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  aoi22  g055(.a(new_n106_), .b(new_n53_), .c(x18), .d(new_n52_), .O(new_n107_));
  oai21  g056(.a(new_n104_), .b(new_n98_), .c(new_n107_), .O(z05));
  nand2  g057(.a(new_n102_), .b(x25), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n91_), .c(new_n72_), .d(new_n58_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(x16), .O(new_n113_));
  nor2   g062(.a(x16), .b(x09), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n54_), .O(new_n115_));
  nand3  g064(.a(new_n115_), .b(new_n113_), .c(new_n79_), .O(z06));
  and2   g065(.a(new_n111_), .b(x26), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n91_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n73_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  nor2   g070(.a(x16), .b(x08), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n54_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n121_), .c(new_n79_), .O(z07));
  nand2  g073(.a(new_n106_), .b(new_n52_), .O(new_n125_));
  inv1   g074(.a(x27), .O(new_n126_));
  nor3   g075(.a(x27), .b(x26), .c(x25), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n101_), .O(new_n128_));
  oai22  g077(.a(new_n128_), .b(new_n83_), .c(new_n120_), .d(new_n126_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  aoi21  g079(.a(x18), .b(new_n53_), .c(new_n54_), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n130_), .c(new_n125_), .O(z08));
  inv1   g081(.a(x28), .O(new_n133_));
  nor2   g082(.a(x27), .b(x26), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n110_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n92_), .c(new_n56_), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n66_), .c(new_n133_), .O(new_n137_));
  inv1   g086(.a(x23), .O(new_n138_));
  inv1   g087(.a(x26), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n110_), .c(new_n139_), .d(new_n138_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n83_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n137_), .c(x16), .O(new_n143_));
  inv1   g092(.a(x06), .O(new_n144_));
  aoi21  g093(.a(new_n61_), .b(new_n144_), .c(x18), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n143_), .c(new_n54_), .O(z09));
  inv1   g095(.a(x29), .O(new_n147_));
  nor2   g096(.a(x26), .b(x25), .O(new_n148_));
  nand4  g097(.a(new_n140_), .b(new_n148_), .c(new_n101_), .d(new_n82_), .O(new_n149_));
  aoi21  g098(.a(new_n149_), .b(new_n56_), .c(new_n90_), .O(new_n150_));
  nor3   g099(.a(x29), .b(x28), .c(x27), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n118_), .c(new_n93_), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n147_), .c(new_n152_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  inv1   g103(.a(x05), .O(new_n155_));
  aoi21  g104(.a(new_n61_), .b(new_n155_), .c(x18), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n154_), .c(new_n54_), .O(z10));
  nand2  g106(.a(new_n152_), .b(x30), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n140_), .c(new_n118_), .d(new_n93_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  nor2   g111(.a(x16), .b(x04), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n54_), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n162_), .c(new_n79_), .O(z11));
  nand2  g114(.a(new_n99_), .b(new_n66_), .O(new_n166_));
  nand3  g115(.a(new_n159_), .b(new_n140_), .c(new_n118_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n166_), .c(x31), .O(new_n168_));
  nor2   g117(.a(x29), .b(x28), .O(new_n169_));
  nor2   g118(.a(x31), .b(x30), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n169_), .c(new_n127_), .O(new_n171_));
  or2    g120(.a(new_n171_), .b(new_n102_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  nor2   g123(.a(x16), .b(x03), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n54_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n174_), .c(new_n79_), .O(z12));
  oai21  g126(.a(new_n171_), .b(new_n102_), .c(x32), .O(new_n178_));
  nand2  g127(.a(new_n140_), .b(new_n148_), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n159_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n179_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n103_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n178_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(x16), .O(new_n185_));
  nor2   g134(.a(x16), .b(x02), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n54_), .O(new_n187_));
  nand3  g136(.a(new_n187_), .b(new_n185_), .c(new_n79_), .O(z13));
  inv1   g137(.a(x33), .O(new_n189_));
  aoi21  g138(.a(new_n182_), .b(new_n103_), .c(new_n189_), .O(new_n190_));
  nand2  g139(.a(new_n169_), .b(new_n134_), .O(new_n191_));
  nor2   g140(.a(x33), .b(x32), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n170_), .O(new_n193_));
  nor3   g142(.a(new_n193_), .b(new_n191_), .c(new_n111_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n190_), .c(x16), .O(new_n195_));
  nor2   g144(.a(x16), .b(x01), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n54_), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n195_), .c(new_n79_), .O(z14));
  inv1   g147(.a(x34), .O(new_n199_));
  inv1   g148(.a(x24), .O(new_n200_));
  nand4  g149(.a(new_n148_), .b(new_n91_), .c(new_n200_), .d(new_n71_), .O(new_n201_));
  nand4  g150(.a(new_n192_), .b(new_n170_), .c(new_n169_), .d(new_n126_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n201_), .c(new_n56_), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n66_), .c(new_n199_), .O(new_n204_));
  inv1   g153(.a(x32), .O(new_n205_));
  nand4  g154(.a(new_n170_), .b(new_n199_), .c(new_n189_), .d(new_n205_), .O(new_n206_));
  nor3   g155(.a(new_n206_), .b(new_n191_), .c(new_n111_), .O(new_n207_));
  oai21  g156(.a(new_n207_), .b(new_n204_), .c(x16), .O(new_n208_));
  inv1   g157(.a(x00), .O(new_n209_));
  aoi21  g158(.a(new_n61_), .b(new_n209_), .c(x18), .O(new_n210_));
  aoi21  g159(.a(new_n210_), .b(new_n208_), .c(new_n54_), .O(z15));
endmodule


