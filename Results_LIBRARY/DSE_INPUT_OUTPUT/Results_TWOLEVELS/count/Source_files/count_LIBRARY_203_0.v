// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  nor2   g000(.a(x12), .b(x10), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g014(.a(x14), .O(new_n66_));
  aoi21  g015(.a(new_n59_), .b(new_n66_), .c(x18), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n65_), .c(new_n52_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n56_), .O(new_n71_));
  oai21  g020(.a(new_n64_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  aoi21  g023(.a(new_n59_), .b(new_n74_), .c(x18), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n73_), .c(new_n52_), .O(z02));
  inv1   g025(.a(new_n52_), .O(new_n77_));
  nand3  g026(.a(new_n62_), .b(new_n54_), .c(new_n53_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(x22), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n56_), .c(new_n62_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(new_n59_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(x18), .c(new_n77_), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  nand2  g033(.a(new_n59_), .b(new_n84_), .O(new_n85_));
  nand2  g034(.a(x22), .b(x21), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n59_), .c(new_n85_), .O(new_n87_));
  nor2   g036(.a(new_n59_), .b(new_n84_), .O(new_n88_));
  nor2   g037(.a(new_n86_), .b(x19), .O(new_n89_));
  aoi22  g038(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(x10), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n83_), .O(z03));
  inv1   g040(.a(x23), .O(new_n92_));
  oai21  g041(.a(x22), .b(x21), .c(new_n54_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n64_), .c(new_n92_), .O(new_n94_));
  nor2   g043(.a(x23), .b(x22), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n69_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n78_), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n94_), .c(x16), .O(new_n98_));
  inv1   g047(.a(x11), .O(new_n99_));
  aoi21  g048(.a(new_n59_), .b(new_n99_), .c(x18), .O(new_n100_));
  aoi21  g049(.a(new_n100_), .b(new_n98_), .c(new_n52_), .O(z04));
  or2    g050(.a(new_n88_), .b(x10), .O(new_n102_));
  nand2  g051(.a(new_n77_), .b(x18), .O(new_n103_));
  inv1   g052(.a(x24), .O(new_n104_));
  nor2   g053(.a(x24), .b(x23), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n80_), .c(new_n64_), .O(new_n106_));
  oai21  g055(.a(new_n97_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x16), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n103_), .c(new_n102_), .O(z05));
  nand2  g058(.a(new_n106_), .b(x25), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n95_), .c(new_n70_), .d(new_n56_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nor2   g063(.a(x16), .b(x09), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n52_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n114_), .c(new_n103_), .O(z06));
  inv1   g066(.a(x26), .O(new_n118_));
  nand3  g067(.a(new_n111_), .b(new_n80_), .c(new_n92_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n54_), .O(new_n120_));
  aoi21  g069(.a(new_n120_), .b(new_n64_), .c(new_n118_), .O(new_n121_));
  nor3   g070(.a(x26), .b(x25), .c(x24), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n95_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n71_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n121_), .c(x16), .O(new_n125_));
  inv1   g074(.a(x08), .O(new_n126_));
  aoi21  g075(.a(new_n59_), .b(new_n126_), .c(x18), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n125_), .c(new_n52_), .O(z07));
  inv1   g077(.a(x27), .O(new_n129_));
  nor3   g078(.a(x27), .b(x26), .c(x25), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n105_), .O(new_n131_));
  oai22  g080(.a(new_n131_), .b(new_n81_), .c(new_n124_), .d(new_n129_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(x16), .O(new_n133_));
  nor2   g082(.a(x16), .b(x07), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n52_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n133_), .c(new_n103_), .O(z08));
  inv1   g085(.a(x28), .O(new_n137_));
  nor2   g086(.a(x27), .b(x26), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n111_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n96_), .c(new_n54_), .O(new_n140_));
  aoi21  g089(.a(new_n140_), .b(new_n64_), .c(new_n137_), .O(new_n141_));
  nor2   g090(.a(x28), .b(x27), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n111_), .c(new_n118_), .d(new_n92_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n81_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n141_), .c(x16), .O(new_n145_));
  inv1   g094(.a(x06), .O(new_n146_));
  aoi21  g095(.a(new_n59_), .b(new_n146_), .c(x18), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n145_), .c(new_n52_), .O(z09));
  inv1   g097(.a(x29), .O(new_n149_));
  nor3   g098(.a(x29), .b(x28), .c(x27), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n122_), .c(new_n97_), .O(new_n151_));
  oai21  g100(.a(new_n144_), .b(new_n149_), .c(new_n151_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  nor2   g102(.a(x16), .b(x05), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n52_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n153_), .c(new_n103_), .O(z10));
  nand2  g105(.a(new_n151_), .b(x30), .O(new_n157_));
  nand3  g106(.a(new_n95_), .b(new_n64_), .c(new_n69_), .O(new_n158_));
  nor2   g107(.a(x30), .b(x29), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n142_), .c(new_n122_), .O(new_n160_));
  or2    g109(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  nor2   g112(.a(x16), .b(x04), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n52_), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n163_), .c(new_n103_), .O(z11));
  oai21  g115(.a(new_n160_), .b(new_n158_), .c(x31), .O(new_n167_));
  nor2   g116(.a(x29), .b(x28), .O(new_n168_));
  nor2   g117(.a(x31), .b(x30), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n168_), .c(new_n130_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n106_), .c(new_n167_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  nor2   g121(.a(x16), .b(x03), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n52_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n172_), .c(new_n103_), .O(z12));
  inv1   g124(.a(x32), .O(new_n176_));
  nand4  g125(.a(new_n169_), .b(new_n142_), .c(new_n149_), .d(new_n118_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n119_), .c(new_n54_), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n64_), .c(new_n176_), .O(new_n179_));
  nor2   g128(.a(x26), .b(x25), .O(new_n180_));
  nor2   g129(.a(x32), .b(x31), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n159_), .c(new_n142_), .d(new_n180_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n106_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n179_), .c(x16), .O(new_n184_));
  inv1   g133(.a(x02), .O(new_n185_));
  aoi21  g134(.a(new_n59_), .b(new_n185_), .c(x18), .O(new_n186_));
  aoi21  g135(.a(new_n186_), .b(new_n184_), .c(new_n52_), .O(z13));
  inv1   g136(.a(x33), .O(new_n188_));
  nand4  g137(.a(new_n180_), .b(new_n95_), .c(new_n104_), .d(new_n69_), .O(new_n189_));
  inv1   g138(.a(x30), .O(new_n190_));
  nand4  g139(.a(new_n181_), .b(new_n168_), .c(new_n190_), .d(new_n129_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n189_), .c(new_n54_), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n64_), .c(new_n188_), .O(new_n193_));
  nand2  g142(.a(new_n168_), .b(new_n138_), .O(new_n194_));
  nor2   g143(.a(x33), .b(x32), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n169_), .O(new_n196_));
  nor3   g145(.a(new_n196_), .b(new_n194_), .c(new_n112_), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n193_), .c(x16), .O(new_n198_));
  inv1   g147(.a(x01), .O(new_n199_));
  aoi21  g148(.a(new_n59_), .b(new_n199_), .c(x18), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n198_), .c(new_n52_), .O(z14));
  inv1   g150(.a(x34), .O(new_n202_));
  nand4  g151(.a(new_n195_), .b(new_n169_), .c(new_n168_), .d(new_n129_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n189_), .c(new_n54_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(new_n64_), .c(new_n202_), .O(new_n205_));
  nand4  g154(.a(new_n169_), .b(new_n202_), .c(new_n188_), .d(new_n176_), .O(new_n206_));
  nor3   g155(.a(new_n206_), .b(new_n194_), .c(new_n112_), .O(new_n207_));
  oai21  g156(.a(new_n207_), .b(new_n205_), .c(x16), .O(new_n208_));
  inv1   g157(.a(x00), .O(new_n209_));
  aoi21  g158(.a(new_n59_), .b(new_n209_), .c(x18), .O(new_n210_));
  aoi21  g159(.a(new_n210_), .b(new_n208_), .c(new_n52_), .O(z15));
endmodule


