// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  oai21  g010(.a(x19), .b(x17), .c(x20), .O(new_n62_));
  nor2   g011(.a(x20), .b(x19), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x31), .b(new_n61_), .O(new_n68_));
  inv1   g017(.a(new_n68_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor3   g020(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n55_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  aoi21  g025(.a(new_n75_), .b(x16), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(x18), .c(new_n69_), .O(z02));
  nand2  g027(.a(x22), .b(x21), .O(new_n79_));
  inv1   g028(.a(x20), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n80_), .c(new_n52_), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n79_), .c(x19), .O(new_n83_));
  inv1   g032(.a(x22), .O(new_n84_));
  nor2   g033(.a(new_n72_), .b(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n83_), .c(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n58_), .b(new_n87_), .c(x18), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n86_), .O(z03));
  nand3  g038(.a(new_n81_), .b(new_n55_), .c(new_n80_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n63_), .c(new_n71_), .d(new_n52_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(new_n58_), .O(new_n94_));
  nor2   g043(.a(x16), .b(x11), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n94_), .c(new_n61_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n69_), .O(z04));
  nand2  g046(.a(new_n93_), .b(x24), .O(new_n98_));
  inv1   g047(.a(x23), .O(new_n99_));
  inv1   g048(.a(x24), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n99_), .c(new_n84_), .d(new_n71_), .O(new_n101_));
  inv1   g050(.a(new_n101_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n72_), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n98_), .c(new_n58_), .O(new_n104_));
  nor2   g053(.a(x16), .b(x10), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n104_), .c(new_n61_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n69_), .O(z05));
  inv1   g056(.a(x25), .O(new_n108_));
  nand2  g057(.a(new_n101_), .b(new_n53_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n72_), .c(new_n108_), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n92_), .c(new_n73_), .d(new_n55_), .O(new_n112_));
  inv1   g061(.a(new_n112_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n110_), .c(x16), .O(new_n114_));
  inv1   g063(.a(x09), .O(new_n115_));
  aoi21  g064(.a(new_n58_), .b(new_n115_), .c(x18), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n114_), .O(z06));
  inv1   g066(.a(x26), .O(new_n118_));
  nand3  g067(.a(new_n111_), .b(new_n81_), .c(new_n99_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n53_), .c(new_n64_), .O(new_n120_));
  nor3   g069(.a(x26), .b(x25), .c(x24), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n92_), .O(new_n122_));
  oai22  g071(.a(new_n122_), .b(new_n74_), .c(new_n120_), .d(new_n118_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(x16), .O(new_n124_));
  inv1   g073(.a(x08), .O(new_n125_));
  aoi21  g074(.a(new_n58_), .b(new_n125_), .c(x18), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n124_), .O(z07));
  inv1   g076(.a(x27), .O(new_n128_));
  nor3   g077(.a(x23), .b(x22), .c(x21), .O(new_n129_));
  nand2  g078(.a(new_n121_), .b(new_n129_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n53_), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n72_), .c(new_n128_), .O(new_n132_));
  nor2   g081(.a(x27), .b(x26), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n108_), .c(new_n100_), .d(new_n99_), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n90_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n132_), .c(x16), .O(new_n136_));
  inv1   g085(.a(x07), .O(new_n137_));
  aoi21  g086(.a(new_n58_), .b(new_n137_), .c(x18), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n136_), .O(z08));
  oai21  g088(.a(new_n134_), .b(new_n90_), .c(x28), .O(new_n140_));
  nor3   g089(.a(x22), .b(x21), .c(x20), .O(new_n141_));
  nor3   g090(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n55_), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n140_), .c(new_n58_), .O(new_n145_));
  nor2   g094(.a(x16), .b(x06), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n145_), .c(new_n61_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n69_), .O(z09));
  nor2   g097(.a(x28), .b(x27), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n111_), .c(new_n118_), .d(new_n99_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n90_), .c(x29), .O(new_n151_));
  nor3   g100(.a(x29), .b(x28), .c(x27), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n121_), .c(new_n129_), .d(new_n72_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n151_), .c(new_n58_), .O(new_n154_));
  nor2   g103(.a(x16), .b(x05), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n154_), .c(new_n61_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n69_), .O(z10));
  inv1   g106(.a(x30), .O(new_n158_));
  nor2   g107(.a(x26), .b(x25), .O(new_n159_));
  nor2   g108(.a(x29), .b(x28), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n159_), .c(new_n128_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n101_), .c(new_n53_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n72_), .c(new_n158_), .O(new_n163_));
  nor2   g112(.a(x30), .b(x29), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n149_), .c(new_n159_), .d(new_n100_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n93_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n163_), .c(x16), .O(new_n167_));
  inv1   g116(.a(x04), .O(new_n168_));
  aoi21  g117(.a(new_n58_), .b(new_n168_), .c(x18), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n167_), .O(z11));
  inv1   g119(.a(x31), .O(new_n171_));
  inv1   g120(.a(x28), .O(new_n172_));
  nand3  g121(.a(new_n164_), .b(new_n133_), .c(new_n172_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n119_), .c(new_n53_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n72_), .c(new_n171_), .O(new_n175_));
  nor3   g124(.a(x27), .b(x26), .c(x25), .O(new_n176_));
  nor2   g125(.a(x31), .b(x30), .O(new_n177_));
  nand3  g126(.a(new_n177_), .b(new_n160_), .c(new_n176_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n103_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n175_), .c(x16), .O(new_n180_));
  inv1   g129(.a(x03), .O(new_n181_));
  aoi21  g130(.a(new_n58_), .b(new_n181_), .c(x18), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n180_), .O(z12));
  nand3  g132(.a(x32), .b(x31), .c(x16), .O(new_n184_));
  oai21  g133(.a(x16), .b(x02), .c(new_n184_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n61_), .O(new_n186_));
  oai21  g135(.a(new_n165_), .b(new_n93_), .c(x32), .O(new_n187_));
  inv1   g136(.a(x29), .O(new_n188_));
  inv1   g137(.a(x32), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n158_), .c(new_n188_), .d(new_n172_), .O(new_n190_));
  inv1   g139(.a(new_n190_), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n176_), .c(new_n102_), .d(new_n72_), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n187_), .c(new_n58_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(x18), .c(new_n171_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n186_), .O(z13));
  inv1   g144(.a(x33), .O(new_n196_));
  oai21  g145(.a(x32), .b(x31), .c(new_n61_), .O(new_n197_));
  oai21  g146(.a(new_n165_), .b(new_n93_), .c(new_n171_), .O(new_n198_));
  aoi21  g147(.a(new_n198_), .b(new_n197_), .c(new_n196_), .O(new_n199_));
  nand2  g148(.a(new_n160_), .b(new_n133_), .O(new_n200_));
  nand3  g149(.a(new_n177_), .b(new_n196_), .c(new_n189_), .O(new_n201_));
  nor3   g150(.a(new_n201_), .b(new_n200_), .c(new_n112_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n199_), .c(x16), .O(new_n203_));
  inv1   g152(.a(x01), .O(new_n204_));
  nor2   g153(.a(x18), .b(x16), .O(new_n205_));
  aoi21  g154(.a(new_n205_), .b(new_n204_), .c(new_n68_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n203_), .O(z14));
  inv1   g156(.a(x34), .O(new_n208_));
  nand3  g157(.a(new_n196_), .b(new_n189_), .c(new_n171_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n61_), .O(new_n210_));
  aoi21  g159(.a(new_n210_), .b(new_n198_), .c(new_n208_), .O(new_n211_));
  nand4  g160(.a(new_n177_), .b(new_n208_), .c(new_n196_), .d(new_n189_), .O(new_n212_));
  nor3   g161(.a(new_n212_), .b(new_n200_), .c(new_n112_), .O(new_n213_));
  oai21  g162(.a(new_n213_), .b(new_n211_), .c(x16), .O(new_n214_));
  inv1   g163(.a(x00), .O(new_n215_));
  aoi21  g164(.a(new_n205_), .b(new_n215_), .c(new_n68_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n214_), .O(z15));
endmodule


