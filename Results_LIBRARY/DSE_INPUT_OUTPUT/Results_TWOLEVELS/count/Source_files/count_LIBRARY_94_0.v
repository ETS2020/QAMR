// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  nor2   g000(.a(x07), .b(x00), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  oai21  g010(.a(x07), .b(x00), .c(x18), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n56_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n66_), .c(new_n62_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n56_), .O(new_n72_));
  oai21  g021(.a(new_n65_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x16), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  aoi21  g024(.a(new_n59_), .b(new_n75_), .c(x18), .O(new_n76_));
  aoi21  g025(.a(new_n76_), .b(new_n74_), .c(new_n52_), .O(z02));
  inv1   g026(.a(x22), .O(new_n78_));
  inv1   g027(.a(new_n72_), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n56_), .c(new_n63_), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n78_), .c(new_n81_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(x16), .O(new_n83_));
  nor2   g032(.a(x16), .b(x12), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n83_), .c(new_n62_), .O(z03));
  nand2  g035(.a(new_n81_), .b(x23), .O(new_n87_));
  nor2   g036(.a(x20), .b(x19), .O(new_n88_));
  nor2   g037(.a(x23), .b(x22), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n88_), .c(new_n70_), .d(new_n53_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x16), .O(new_n92_));
  nor2   g041(.a(x16), .b(x11), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n52_), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n92_), .c(new_n62_), .O(z04));
  and2   g044(.a(new_n90_), .b(x24), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n80_), .c(new_n65_), .O(new_n98_));
  inv1   g047(.a(new_n98_), .O(new_n99_));
  oai21  g048(.a(new_n99_), .b(new_n96_), .c(x16), .O(new_n100_));
  nor2   g049(.a(x16), .b(x10), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n52_), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n100_), .c(new_n62_), .O(z05));
  inv1   g052(.a(x25), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n89_), .c(new_n71_), .d(new_n56_), .O(new_n106_));
  oai21  g055(.a(new_n99_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x16), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n52_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n108_), .c(new_n62_), .O(z06));
  inv1   g060(.a(x26), .O(new_n112_));
  nand2  g061(.a(new_n88_), .b(new_n53_), .O(new_n113_));
  inv1   g062(.a(x23), .O(new_n114_));
  nand3  g063(.a(new_n105_), .b(new_n80_), .c(new_n114_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n54_), .c(new_n113_), .O(new_n116_));
  nor3   g065(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n89_), .c(new_n79_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n112_), .c(new_n118_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g069(.a(x08), .O(new_n121_));
  aoi21  g070(.a(new_n59_), .b(new_n121_), .c(x18), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n120_), .c(new_n52_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  nor3   g073(.a(x22), .b(x21), .c(x20), .O(new_n125_));
  nor3   g074(.a(x27), .b(x26), .c(x25), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n97_), .c(new_n125_), .d(new_n56_), .O(new_n127_));
  oai21  g076(.a(new_n116_), .b(new_n124_), .c(new_n127_), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(x16), .c(x18), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  nand2  g079(.a(new_n59_), .b(new_n130_), .O(new_n131_));
  nand2  g080(.a(x27), .b(x26), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n59_), .c(new_n131_), .O(new_n133_));
  nor2   g082(.a(new_n59_), .b(new_n130_), .O(new_n134_));
  nor2   g083(.a(new_n132_), .b(x19), .O(new_n135_));
  aoi22  g084(.a(new_n135_), .b(new_n134_), .c(new_n133_), .d(x00), .O(new_n136_));
  oai21  g085(.a(new_n129_), .b(new_n52_), .c(new_n136_), .O(z08));
  nor2   g086(.a(x27), .b(x26), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(x19), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n106_), .c(x28), .O(new_n140_));
  nor2   g089(.a(x28), .b(x27), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n105_), .c(new_n112_), .d(new_n114_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n81_), .c(new_n140_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  nor2   g093(.a(x16), .b(x06), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n52_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n144_), .c(new_n62_), .O(z09));
  inv1   g096(.a(x29), .O(new_n148_));
  inv1   g097(.a(new_n139_), .O(new_n149_));
  inv1   g098(.a(x24), .O(new_n150_));
  nor2   g099(.a(x28), .b(x25), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n89_), .c(new_n150_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n72_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n149_), .c(new_n148_), .O(new_n154_));
  nor2   g103(.a(x29), .b(x28), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n117_), .c(new_n124_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n90_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n154_), .c(x16), .O(new_n158_));
  nor2   g107(.a(x16), .b(x05), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n52_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n158_), .c(new_n62_), .O(z10));
  inv1   g110(.a(x30), .O(new_n162_));
  nand3  g111(.a(new_n155_), .b(new_n97_), .c(new_n104_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n81_), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n149_), .c(new_n162_), .O(new_n165_));
  nor2   g114(.a(x30), .b(x29), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n141_), .c(new_n117_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n90_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n165_), .c(x16), .O(new_n169_));
  nor2   g118(.a(x16), .b(x04), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n52_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n169_), .c(new_n62_), .O(z11));
  inv1   g121(.a(x31), .O(new_n173_));
  inv1   g122(.a(x28), .O(new_n174_));
  nand3  g123(.a(new_n166_), .b(new_n138_), .c(new_n174_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n115_), .c(new_n54_), .O(new_n176_));
  aoi21  g125(.a(new_n176_), .b(new_n65_), .c(new_n173_), .O(new_n177_));
  nor2   g126(.a(x31), .b(x30), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(new_n155_), .c(new_n126_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n98_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n177_), .c(x16), .O(new_n181_));
  inv1   g130(.a(x03), .O(new_n182_));
  aoi21  g131(.a(new_n59_), .b(new_n182_), .c(x18), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n181_), .c(new_n52_), .O(z12));
  inv1   g133(.a(x32), .O(new_n185_));
  nand3  g134(.a(new_n173_), .b(new_n124_), .c(new_n112_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n54_), .O(new_n187_));
  nand4  g136(.a(new_n166_), .b(new_n105_), .c(new_n174_), .d(new_n114_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n81_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n187_), .c(new_n185_), .O(new_n190_));
  nor2   g139(.a(x26), .b(x25), .O(new_n191_));
  nor2   g140(.a(x32), .b(x31), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n166_), .c(new_n141_), .d(new_n191_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n98_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n190_), .c(x16), .O(new_n195_));
  nor2   g144(.a(x16), .b(x02), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n52_), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n195_), .c(new_n62_), .O(z13));
  inv1   g147(.a(x33), .O(new_n199_));
  nor2   g148(.a(x32), .b(x30), .O(new_n200_));
  nand4  g149(.a(new_n200_), .b(new_n151_), .c(new_n148_), .d(new_n150_), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n90_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n187_), .c(new_n199_), .O(new_n203_));
  nor2   g152(.a(x33), .b(x32), .O(new_n204_));
  nand4  g153(.a(new_n204_), .b(new_n178_), .c(new_n155_), .d(new_n138_), .O(new_n205_));
  nor2   g154(.a(new_n205_), .b(new_n106_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n203_), .c(x16), .O(new_n207_));
  nor2   g156(.a(x16), .b(x01), .O(new_n208_));
  nor2   g157(.a(new_n208_), .b(new_n52_), .O(new_n209_));
  nand3  g158(.a(new_n209_), .b(new_n207_), .c(new_n62_), .O(z14));
  or2    g159(.a(new_n134_), .b(x00), .O(new_n211_));
  inv1   g160(.a(x34), .O(new_n212_));
  nand4  g161(.a(new_n204_), .b(new_n166_), .c(new_n151_), .d(new_n150_), .O(new_n213_));
  nor2   g162(.a(new_n213_), .b(new_n90_), .O(new_n214_));
  aoi21  g163(.a(new_n214_), .b(new_n187_), .c(new_n212_), .O(new_n215_));
  nor3   g164(.a(x34), .b(x33), .c(x32), .O(new_n216_));
  nand4  g165(.a(new_n216_), .b(new_n178_), .c(new_n155_), .d(new_n138_), .O(new_n217_));
  nor2   g166(.a(new_n217_), .b(new_n106_), .O(new_n218_));
  oai21  g167(.a(new_n218_), .b(new_n215_), .c(x16), .O(new_n219_));
  nand3  g168(.a(new_n219_), .b(new_n211_), .c(new_n62_), .O(z15));
endmodule


