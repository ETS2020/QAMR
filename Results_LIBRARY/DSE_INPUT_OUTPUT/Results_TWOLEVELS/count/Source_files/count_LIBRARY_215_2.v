// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x01), .O(new_n52_));
  nor2   g001(.a(x10), .b(new_n52_), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  oai21  g011(.a(x10), .b(new_n52_), .c(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n55_), .c(new_n54_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n57_), .O(new_n74_));
  oai21  g023(.a(new_n67_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n60_), .b(new_n77_), .c(x18), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n76_), .c(new_n53_), .O(z02));
  and2   g028(.a(new_n74_), .b(x22), .O(new_n80_));
  inv1   g029(.a(new_n57_), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  nor2   g032(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n80_), .c(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n63_), .O(z03));
  inv1   g037(.a(x23), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n67_), .c(new_n72_), .O(new_n91_));
  oai21  g040(.a(new_n84_), .b(new_n89_), .c(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n53_), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n93_), .c(new_n63_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  nand2  g046(.a(new_n90_), .b(new_n72_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n55_), .c(new_n66_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n82_), .c(new_n67_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(x16), .c(x18), .O(new_n103_));
  inv1   g052(.a(x10), .O(new_n104_));
  nand3  g053(.a(new_n60_), .b(new_n104_), .c(new_n52_), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n53_), .c(new_n105_), .O(z05));
  inv1   g055(.a(x25), .O(new_n107_));
  nand2  g056(.a(new_n100_), .b(new_n82_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n55_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n67_), .c(new_n107_), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n90_), .c(new_n73_), .d(new_n57_), .O(new_n112_));
  inv1   g061(.a(new_n112_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n110_), .c(x16), .O(new_n114_));
  inv1   g063(.a(x09), .O(new_n115_));
  aoi21  g064(.a(new_n60_), .b(new_n115_), .c(x18), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n114_), .c(new_n53_), .O(z06));
  inv1   g066(.a(x26), .O(new_n118_));
  nand3  g067(.a(new_n111_), .b(new_n82_), .c(new_n89_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n55_), .O(new_n120_));
  aoi21  g069(.a(new_n120_), .b(new_n67_), .c(new_n118_), .O(new_n121_));
  nor3   g070(.a(x26), .b(x25), .c(x24), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n90_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n74_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n121_), .c(x16), .O(new_n125_));
  inv1   g074(.a(x08), .O(new_n126_));
  aoi21  g075(.a(new_n60_), .b(new_n126_), .c(x18), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n125_), .c(new_n53_), .O(z07));
  inv1   g077(.a(x27), .O(new_n129_));
  nor2   g078(.a(new_n98_), .b(new_n66_), .O(new_n130_));
  nor2   g079(.a(new_n122_), .b(x19), .O(new_n131_));
  inv1   g080(.a(new_n131_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n130_), .c(new_n129_), .O(new_n133_));
  nor2   g082(.a(x27), .b(x26), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n100_), .c(new_n107_), .O(new_n135_));
  nor3   g084(.a(new_n135_), .b(new_n83_), .c(new_n81_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n133_), .c(x16), .O(new_n137_));
  nor2   g086(.a(x16), .b(x07), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n53_), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n137_), .c(new_n63_), .O(z08));
  nand4  g089(.a(new_n82_), .b(new_n67_), .c(new_n129_), .d(new_n89_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n131_), .c(x28), .O(new_n142_));
  nor3   g091(.a(x25), .b(x24), .c(x23), .O(new_n143_));
  nor2   g092(.a(x28), .b(x27), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n143_), .c(new_n84_), .d(new_n118_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(x16), .O(new_n147_));
  nor2   g096(.a(x16), .b(x06), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n53_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n147_), .c(new_n63_), .O(z09));
  inv1   g099(.a(x29), .O(new_n151_));
  nor2   g100(.a(x26), .b(x25), .O(new_n152_));
  nand4  g101(.a(new_n144_), .b(new_n152_), .c(new_n100_), .d(new_n82_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n55_), .c(new_n66_), .O(new_n154_));
  nor2   g103(.a(x29), .b(x28), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n122_), .c(new_n129_), .O(new_n156_));
  oai22  g105(.a(new_n156_), .b(new_n91_), .c(new_n154_), .d(new_n151_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(x16), .O(new_n158_));
  inv1   g107(.a(x05), .O(new_n159_));
  aoi21  g108(.a(new_n60_), .b(new_n159_), .c(x18), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n158_), .c(new_n53_), .O(z10));
  inv1   g110(.a(x30), .O(new_n162_));
  nand3  g111(.a(new_n155_), .b(new_n152_), .c(new_n129_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n108_), .c(new_n55_), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n67_), .c(new_n162_), .O(new_n165_));
  nor2   g114(.a(x30), .b(x29), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n144_), .c(new_n122_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n91_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n165_), .c(x16), .O(new_n169_));
  inv1   g118(.a(x04), .O(new_n170_));
  aoi21  g119(.a(new_n60_), .b(new_n170_), .c(x18), .O(new_n171_));
  aoi21  g120(.a(new_n171_), .b(new_n169_), .c(new_n53_), .O(z11));
  inv1   g121(.a(x31), .O(new_n173_));
  inv1   g122(.a(x28), .O(new_n174_));
  nand3  g123(.a(new_n166_), .b(new_n134_), .c(new_n174_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n119_), .c(new_n55_), .O(new_n176_));
  aoi21  g125(.a(new_n176_), .b(new_n67_), .c(new_n173_), .O(new_n177_));
  nor2   g126(.a(x31), .b(x30), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n155_), .c(new_n134_), .d(new_n107_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n101_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n177_), .c(x16), .O(new_n181_));
  inv1   g130(.a(x03), .O(new_n182_));
  aoi21  g131(.a(new_n60_), .b(new_n182_), .c(x18), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n181_), .c(new_n53_), .O(z12));
  inv1   g133(.a(x32), .O(new_n185_));
  nand4  g134(.a(new_n178_), .b(new_n144_), .c(new_n151_), .d(new_n118_), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n119_), .c(new_n55_), .O(new_n187_));
  aoi21  g136(.a(new_n187_), .b(new_n67_), .c(new_n185_), .O(new_n188_));
  nor2   g137(.a(x32), .b(x31), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n166_), .c(new_n144_), .d(new_n152_), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n101_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n188_), .c(x16), .O(new_n192_));
  inv1   g141(.a(x02), .O(new_n193_));
  aoi21  g142(.a(new_n60_), .b(new_n193_), .c(x18), .O(new_n194_));
  aoi21  g143(.a(new_n194_), .b(new_n192_), .c(new_n53_), .O(z13));
  aoi21  g144(.a(x16), .b(x10), .c(new_n52_), .O(new_n196_));
  nand4  g145(.a(new_n152_), .b(new_n90_), .c(new_n97_), .d(new_n72_), .O(new_n197_));
  nand4  g146(.a(new_n189_), .b(new_n155_), .c(new_n162_), .d(new_n129_), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(new_n197_), .c(new_n55_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n67_), .O(new_n200_));
  nand2  g149(.a(new_n155_), .b(new_n134_), .O(new_n201_));
  nor2   g150(.a(x33), .b(x32), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n178_), .O(new_n203_));
  nor3   g152(.a(new_n203_), .b(new_n201_), .c(new_n112_), .O(new_n204_));
  aoi21  g153(.a(new_n200_), .b(x33), .c(new_n204_), .O(new_n205_));
  inv1   g154(.a(x18), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(x16), .O(new_n207_));
  aoi22  g156(.a(new_n207_), .b(new_n52_), .c(x18), .d(x10), .O(new_n208_));
  oai21  g157(.a(new_n205_), .b(new_n196_), .c(new_n208_), .O(z14));
  inv1   g158(.a(x34), .O(new_n210_));
  nand4  g159(.a(new_n202_), .b(new_n178_), .c(new_n155_), .d(new_n129_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n197_), .c(new_n55_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n67_), .c(new_n210_), .O(new_n213_));
  nor2   g162(.a(x34), .b(x33), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(new_n178_), .c(new_n185_), .O(new_n215_));
  nor3   g164(.a(new_n215_), .b(new_n201_), .c(new_n112_), .O(new_n216_));
  oai21  g165(.a(new_n216_), .b(new_n213_), .c(x16), .O(new_n217_));
  inv1   g166(.a(x00), .O(new_n218_));
  aoi21  g167(.a(new_n60_), .b(new_n218_), .c(x18), .O(new_n219_));
  aoi21  g168(.a(new_n219_), .b(new_n217_), .c(new_n53_), .O(z15));
endmodule


