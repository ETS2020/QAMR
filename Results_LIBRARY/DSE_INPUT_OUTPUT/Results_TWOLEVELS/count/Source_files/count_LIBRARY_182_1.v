// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g008(.a(x26), .b(x18), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g010(.a(x18), .O(new_n62_));
  inv1   g011(.a(x26), .O(new_n63_));
  oai21  g012(.a(x19), .b(x17), .c(x20), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n63_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n62_), .O(z01));
  inv1   g019(.a(x20), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n55_), .b(new_n72_), .c(new_n71_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n66_), .b(x21), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(new_n60_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g027(.a(new_n75_), .b(new_n58_), .c(new_n78_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  nor2   g030(.a(x20), .b(x17), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  oai22  g033(.a(new_n84_), .b(x19), .c(new_n65_), .d(new_n80_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  aoi21  g035(.a(new_n85_), .b(x16), .c(new_n86_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(x26), .c(new_n62_), .O(z03));
  nor2   g037(.a(new_n83_), .b(x19), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n66_), .c(x23), .O(new_n90_));
  nor3   g039(.a(x23), .b(x22), .c(x21), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n65_), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n90_), .c(new_n58_), .O(new_n93_));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n93_), .c(new_n63_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n62_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  inv1   g046(.a(new_n91_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n53_), .c(new_n66_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n83_), .c(new_n65_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x10), .O(new_n103_));
  aoi21  g052(.a(new_n102_), .b(x16), .c(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(x26), .c(new_n62_), .O(z05));
  inv1   g054(.a(x25), .O(new_n106_));
  nand2  g055(.a(new_n100_), .b(new_n83_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n53_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n65_), .c(new_n106_), .O(new_n109_));
  nor2   g058(.a(x23), .b(x22), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n63_), .c(new_n106_), .d(new_n97_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n73_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n109_), .c(x16), .O(new_n113_));
  nor2   g062(.a(x16), .b(x09), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n76_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n113_), .O(z06));
  nand4  g065(.a(new_n71_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n117_));
  inv1   g066(.a(x23), .O(new_n118_));
  nor2   g067(.a(x25), .b(x24), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n83_), .c(new_n118_), .O(new_n120_));
  oai22  g069(.a(new_n120_), .b(new_n117_), .c(x16), .d(x08), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n62_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  aoi21  g073(.a(new_n120_), .b(new_n53_), .c(new_n66_), .O(new_n125_));
  nor3   g074(.a(x27), .b(x25), .c(x24), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n110_), .c(new_n74_), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n124_), .c(new_n127_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x07), .O(new_n129_));
  aoi21  g078(.a(new_n128_), .b(x16), .c(new_n129_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(x26), .c(new_n62_), .O(z08));
  inv1   g080(.a(x28), .O(new_n132_));
  nand2  g081(.a(new_n126_), .b(new_n91_), .O(new_n133_));
  aoi21  g082(.a(new_n133_), .b(new_n53_), .c(new_n66_), .O(new_n134_));
  nand3  g083(.a(new_n83_), .b(new_n55_), .c(new_n71_), .O(new_n135_));
  nor3   g084(.a(x28), .b(x27), .c(x25), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n100_), .O(new_n137_));
  oai22  g086(.a(new_n137_), .b(new_n135_), .c(new_n134_), .d(new_n132_), .O(new_n138_));
  nor2   g087(.a(x16), .b(x06), .O(new_n139_));
  aoi21  g088(.a(new_n138_), .b(x16), .c(new_n139_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(x26), .c(new_n62_), .O(z09));
  inv1   g090(.a(x29), .O(new_n142_));
  nor2   g091(.a(x28), .b(x27), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n119_), .c(new_n91_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n53_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n65_), .c(new_n142_), .O(new_n146_));
  nor2   g095(.a(x29), .b(x28), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n119_), .c(new_n124_), .d(new_n118_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n135_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n146_), .c(x16), .O(new_n150_));
  nor2   g099(.a(x16), .b(x05), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n76_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z10));
  inv1   g102(.a(x30), .O(new_n154_));
  nor2   g103(.a(x27), .b(x25), .O(new_n155_));
  nand4  g104(.a(new_n147_), .b(new_n155_), .c(new_n100_), .d(new_n83_), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n53_), .c(new_n66_), .O(new_n157_));
  nor3   g106(.a(x30), .b(x29), .c(x28), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n126_), .O(new_n159_));
  oai22  g108(.a(new_n159_), .b(new_n92_), .c(new_n157_), .d(new_n154_), .O(new_n160_));
  nor2   g109(.a(x16), .b(x04), .O(new_n161_));
  aoi21  g110(.a(new_n160_), .b(x16), .c(new_n161_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(x26), .c(new_n62_), .O(z11));
  inv1   g112(.a(x31), .O(new_n164_));
  nand2  g113(.a(new_n158_), .b(new_n155_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n107_), .c(new_n53_), .O(new_n166_));
  aoi21  g115(.a(new_n166_), .b(new_n65_), .c(new_n164_), .O(new_n167_));
  nor2   g116(.a(x31), .b(x30), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n147_), .c(new_n126_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n92_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(x16), .O(new_n171_));
  nor2   g120(.a(x16), .b(x03), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n76_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z12));
  inv1   g123(.a(x32), .O(new_n175_));
  nand3  g124(.a(new_n168_), .b(new_n143_), .c(new_n142_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n120_), .c(new_n53_), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n65_), .c(new_n175_), .O(new_n178_));
  nor2   g127(.a(x30), .b(x29), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n179_), .c(new_n136_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n101_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n178_), .c(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x02), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n76_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n183_), .O(z13));
  inv1   g135(.a(x33), .O(new_n187_));
  nand4  g136(.a(new_n180_), .b(new_n147_), .c(new_n154_), .d(new_n124_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n120_), .c(new_n53_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n65_), .c(new_n187_), .O(new_n190_));
  nor2   g139(.a(x33), .b(x32), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n168_), .c(new_n147_), .d(new_n155_), .O(new_n192_));
  nor2   g141(.a(new_n192_), .b(new_n101_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n190_), .c(x16), .O(new_n194_));
  nor2   g143(.a(x16), .b(x01), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n76_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n194_), .O(z14));
  inv1   g146(.a(x34), .O(new_n198_));
  nor3   g147(.a(x33), .b(x32), .c(x31), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n158_), .c(new_n126_), .d(new_n91_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n53_), .c(new_n66_), .O(new_n201_));
  nand2  g150(.a(new_n179_), .b(new_n143_), .O(new_n202_));
  nand3  g151(.a(new_n180_), .b(new_n198_), .c(new_n187_), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n202_), .O(new_n204_));
  nand4  g153(.a(new_n204_), .b(new_n119_), .c(new_n110_), .d(new_n74_), .O(new_n205_));
  oai21  g154(.a(new_n201_), .b(new_n198_), .c(new_n205_), .O(new_n206_));
  nor2   g155(.a(x16), .b(x00), .O(new_n207_));
  aoi21  g156(.a(new_n206_), .b(x16), .c(new_n207_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(x26), .c(new_n62_), .O(z15));
endmodule


