// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x09), .O(new_n57_));
  nand2  g006(.a(x14), .b(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x18), .O(new_n59_));
  inv1   g008(.a(new_n58_), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(z00));
  nand2  g012(.a(x16), .b(x09), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x14), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  nor2   g015(.a(new_n55_), .b(new_n66_), .O(new_n67_));
  nand3  g016(.a(new_n66_), .b(new_n53_), .c(new_n52_), .O(new_n68_));
  inv1   g017(.a(new_n68_), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(new_n70_));
  inv1   g019(.a(x14), .O(new_n71_));
  inv1   g020(.a(x18), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  aoi22  g022(.a(new_n73_), .b(new_n71_), .c(x18), .d(x09), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n70_), .O(z01));
  inv1   g024(.a(x21), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n66_), .c(new_n53_), .d(new_n52_), .O(new_n77_));
  oai21  g026(.a(new_n69_), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(x16), .O(new_n79_));
  inv1   g028(.a(x13), .O(new_n80_));
  inv1   g029(.a(x16), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n80_), .c(x18), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n79_), .c(new_n60_), .O(z02));
  inv1   g032(.a(x22), .O(new_n84_));
  inv1   g033(.a(new_n77_), .O(new_n85_));
  nor3   g034(.a(x22), .b(x21), .c(x20), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n55_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n84_), .c(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x16), .O(new_n89_));
  inv1   g038(.a(x12), .O(new_n90_));
  aoi21  g039(.a(new_n81_), .b(new_n90_), .c(x18), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n89_), .c(new_n60_), .O(z03));
  inv1   g041(.a(x23), .O(new_n93_));
  aoi21  g042(.a(new_n86_), .b(new_n55_), .c(new_n93_), .O(new_n94_));
  nand3  g043(.a(new_n93_), .b(new_n84_), .c(new_n76_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n68_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  inv1   g046(.a(x11), .O(new_n98_));
  aoi21  g047(.a(new_n81_), .b(new_n98_), .c(x18), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n97_), .c(new_n60_), .O(z04));
  inv1   g049(.a(x24), .O(new_n101_));
  nor2   g050(.a(x20), .b(x19), .O(new_n102_));
  nor2   g051(.a(x22), .b(x21), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n52_), .O(new_n105_));
  oai21  g054(.a(new_n96_), .b(new_n101_), .c(new_n105_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x16), .O(new_n107_));
  nor2   g056(.a(x16), .b(x10), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n60_), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n107_), .c(new_n59_), .O(z05));
  nand2  g059(.a(new_n68_), .b(x25), .O(new_n111_));
  nor2   g060(.a(x21), .b(x20), .O(new_n112_));
  nor2   g061(.a(x23), .b(x22), .O(new_n113_));
  nor2   g062(.a(x25), .b(x24), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(new_n55_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n111_), .c(new_n81_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(x18), .c(new_n58_), .O(new_n117_));
  nand2  g066(.a(new_n104_), .b(new_n103_), .O(new_n118_));
  nand2  g067(.a(new_n71_), .b(new_n57_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n64_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n118_), .c(x25), .O(new_n121_));
  nand3  g070(.a(new_n81_), .b(new_n71_), .c(new_n57_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n121_), .c(new_n117_), .O(z06));
  nand2  g072(.a(new_n115_), .b(x26), .O(new_n124_));
  nor3   g073(.a(x26), .b(x25), .c(x24), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n113_), .c(new_n85_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(x16), .O(new_n128_));
  nor2   g077(.a(x16), .b(x08), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n60_), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n128_), .c(new_n59_), .O(z07));
  nand2  g080(.a(new_n126_), .b(x27), .O(new_n132_));
  nor3   g081(.a(x27), .b(x26), .c(x25), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n104_), .c(new_n86_), .d(new_n55_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(x16), .O(new_n136_));
  nor2   g085(.a(x16), .b(x07), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n60_), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n136_), .c(new_n59_), .O(z08));
  nand2  g088(.a(new_n134_), .b(x28), .O(new_n140_));
  nor3   g089(.a(x25), .b(x24), .c(x23), .O(new_n141_));
  nor3   g090(.a(x28), .b(x27), .c(x26), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n141_), .c(new_n86_), .d(new_n55_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(x16), .O(new_n145_));
  inv1   g094(.a(x06), .O(new_n146_));
  aoi21  g095(.a(new_n81_), .b(new_n146_), .c(x18), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n145_), .c(new_n60_), .O(z09));
  and2   g097(.a(new_n143_), .b(x29), .O(new_n149_));
  nand4  g098(.a(new_n113_), .b(new_n102_), .c(new_n76_), .d(new_n52_), .O(new_n150_));
  inv1   g099(.a(x27), .O(new_n151_));
  nor2   g100(.a(x26), .b(x25), .O(new_n152_));
  nor2   g101(.a(x29), .b(x28), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n152_), .c(new_n151_), .d(new_n101_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n149_), .c(x16), .O(new_n156_));
  nor2   g105(.a(x16), .b(x05), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n60_), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n156_), .c(new_n59_), .O(z10));
  oai21  g108(.a(new_n154_), .b(new_n150_), .c(x30), .O(new_n160_));
  nor2   g109(.a(x28), .b(x27), .O(new_n161_));
  nor2   g110(.a(x30), .b(x29), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n161_), .c(new_n125_), .d(new_n96_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(x16), .O(new_n165_));
  inv1   g114(.a(x04), .O(new_n166_));
  aoi21  g115(.a(new_n81_), .b(new_n166_), .c(x18), .O(new_n167_));
  aoi21  g116(.a(new_n167_), .b(new_n165_), .c(new_n60_), .O(z11));
  nand4  g117(.a(new_n162_), .b(new_n161_), .c(new_n152_), .d(new_n101_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n150_), .c(x31), .O(new_n170_));
  inv1   g119(.a(new_n118_), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n153_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n133_), .c(new_n171_), .d(new_n69_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n170_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  inv1   g126(.a(x03), .O(new_n178_));
  aoi21  g127(.a(new_n81_), .b(new_n178_), .c(x18), .O(new_n179_));
  aoi21  g128(.a(new_n179_), .b(new_n177_), .c(new_n60_), .O(z12));
  nand3  g129(.a(new_n172_), .b(new_n153_), .c(new_n133_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n105_), .c(x32), .O(new_n182_));
  nor2   g131(.a(x32), .b(x31), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n162_), .c(new_n161_), .d(new_n152_), .O(new_n184_));
  or2    g133(.a(new_n184_), .b(new_n105_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n182_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(x16), .O(new_n187_));
  nor2   g136(.a(x16), .b(x02), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n60_), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n187_), .c(new_n59_), .O(z13));
  oai21  g139(.a(new_n184_), .b(new_n105_), .c(x33), .O(new_n191_));
  inv1   g140(.a(x25), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n101_), .c(new_n93_), .d(new_n84_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n77_), .O(new_n194_));
  nor2   g143(.a(x27), .b(x26), .O(new_n195_));
  nor2   g144(.a(x33), .b(x32), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n172_), .c(new_n153_), .d(new_n195_), .O(new_n197_));
  inv1   g146(.a(new_n197_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n194_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n191_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(x16), .O(new_n201_));
  inv1   g150(.a(x01), .O(new_n202_));
  aoi21  g151(.a(new_n81_), .b(new_n202_), .c(x18), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n201_), .c(new_n60_), .O(z14));
  oai21  g153(.a(new_n197_), .b(new_n115_), .c(x34), .O(new_n205_));
  nand2  g154(.a(new_n153_), .b(new_n195_), .O(new_n206_));
  inv1   g155(.a(new_n206_), .O(new_n207_));
  inv1   g156(.a(x30), .O(new_n208_));
  inv1   g157(.a(x31), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n208_), .O(new_n210_));
  inv1   g159(.a(x32), .O(new_n211_));
  inv1   g160(.a(x33), .O(new_n212_));
  inv1   g161(.a(x34), .O(new_n213_));
  nand3  g162(.a(new_n213_), .b(new_n212_), .c(new_n211_), .O(new_n214_));
  nor2   g163(.a(new_n214_), .b(new_n210_), .O(new_n215_));
  nand3  g164(.a(new_n215_), .b(new_n207_), .c(new_n194_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n205_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(x16), .O(new_n218_));
  inv1   g167(.a(x00), .O(new_n219_));
  aoi21  g168(.a(new_n81_), .b(new_n219_), .c(x18), .O(new_n220_));
  aoi21  g169(.a(new_n220_), .b(new_n218_), .c(new_n60_), .O(z15));
endmodule


