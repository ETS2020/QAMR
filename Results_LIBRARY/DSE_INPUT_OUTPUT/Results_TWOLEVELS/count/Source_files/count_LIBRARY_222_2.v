// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x02), .O(new_n52_));
  nor2   g001(.a(x15), .b(new_n52_), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  inv1   g004(.a(new_n55_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x16), .c(x18), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n52_), .O(new_n61_));
  oai21  g010(.a(new_n58_), .b(new_n53_), .c(new_n61_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n60_), .b(new_n67_), .c(x18), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n66_), .c(new_n53_), .O(z01));
  inv1   g018(.a(new_n53_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x18), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n55_), .O(new_n74_));
  oai21  g023(.a(new_n65_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n53_), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n76_), .c(new_n71_), .O(z02));
  nand2  g028(.a(x22), .b(x21), .O(new_n80_));
  inv1   g029(.a(x17), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n63_), .c(new_n81_), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n80_), .c(x19), .O(new_n84_));
  inv1   g033(.a(x22), .O(new_n85_));
  nor2   g034(.a(new_n65_), .b(new_n85_), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n84_), .c(x16), .O(new_n87_));
  inv1   g036(.a(x12), .O(new_n88_));
  aoi21  g037(.a(new_n60_), .b(new_n88_), .c(x18), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n53_), .O(z03));
  nand3  g039(.a(new_n82_), .b(new_n55_), .c(new_n63_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x23), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n65_), .c(new_n72_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  nor2   g045(.a(x16), .b(x11), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n53_), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n96_), .c(new_n71_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  inv1   g049(.a(x19), .O(new_n101_));
  nand3  g050(.a(new_n63_), .b(new_n101_), .c(new_n81_), .O(new_n102_));
  nand2  g051(.a(new_n93_), .b(new_n72_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nor2   g053(.a(x24), .b(x23), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n82_), .c(new_n65_), .O(new_n106_));
  oai21  g055(.a(new_n104_), .b(new_n100_), .c(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x16), .O(new_n108_));
  nor2   g057(.a(x16), .b(x10), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n53_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n108_), .c(new_n71_), .O(z05));
  inv1   g060(.a(x25), .O(new_n112_));
  nand2  g061(.a(new_n105_), .b(new_n82_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n101_), .O(new_n114_));
  aoi21  g063(.a(new_n114_), .b(new_n65_), .c(new_n112_), .O(new_n115_));
  nor2   g064(.a(x25), .b(x24), .O(new_n116_));
  nand4  g065(.a(new_n116_), .b(new_n93_), .c(new_n73_), .d(new_n55_), .O(new_n117_));
  inv1   g066(.a(new_n117_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n115_), .c(x16), .O(new_n119_));
  inv1   g068(.a(x09), .O(new_n120_));
  aoi21  g069(.a(new_n60_), .b(new_n120_), .c(x18), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n119_), .c(new_n53_), .O(z06));
  inv1   g071(.a(x26), .O(new_n123_));
  nor3   g072(.a(x25), .b(x24), .c(x23), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n82_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n101_), .c(new_n102_), .O(new_n126_));
  nor3   g075(.a(x26), .b(x25), .c(x24), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n93_), .c(new_n73_), .d(new_n55_), .O(new_n128_));
  oai21  g077(.a(new_n126_), .b(new_n123_), .c(new_n128_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  inv1   g079(.a(x08), .O(new_n131_));
  aoi21  g080(.a(new_n60_), .b(new_n131_), .c(x18), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n130_), .c(new_n53_), .O(z07));
  nand2  g082(.a(new_n128_), .b(x27), .O(new_n134_));
  nor3   g083(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n105_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n91_), .c(new_n134_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nor2   g087(.a(x16), .b(x07), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n53_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n138_), .c(new_n71_), .O(z08));
  oai21  g090(.a(new_n136_), .b(new_n91_), .c(x28), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n124_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n91_), .c(new_n142_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(x16), .O(new_n146_));
  nor2   g095(.a(x16), .b(x06), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n53_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(new_n71_), .O(z09));
  inv1   g098(.a(x29), .O(new_n150_));
  nor2   g099(.a(x26), .b(x25), .O(new_n151_));
  nor2   g100(.a(x28), .b(x27), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n151_), .c(new_n105_), .d(new_n82_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n101_), .c(new_n102_), .O(new_n154_));
  nor3   g103(.a(x29), .b(x28), .c(x27), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n127_), .c(new_n104_), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n150_), .c(new_n156_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(x16), .O(new_n158_));
  inv1   g107(.a(x05), .O(new_n159_));
  aoi21  g108(.a(new_n60_), .b(new_n159_), .c(x18), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n158_), .c(new_n53_), .O(z10));
  nand2  g110(.a(new_n156_), .b(x30), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n152_), .c(new_n127_), .O(new_n164_));
  or2    g113(.a(new_n164_), .b(new_n94_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(x16), .O(new_n167_));
  nor2   g116(.a(x16), .b(x04), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n53_), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n167_), .c(new_n71_), .O(z11));
  oai21  g119(.a(new_n164_), .b(new_n94_), .c(x31), .O(new_n171_));
  nor2   g120(.a(x29), .b(x28), .O(new_n172_));
  nor2   g121(.a(x31), .b(x30), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n172_), .c(new_n135_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n106_), .c(new_n171_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(x16), .O(new_n176_));
  nor2   g125(.a(x16), .b(x03), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n53_), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(new_n176_), .c(new_n71_), .O(z12));
  oai21  g128(.a(new_n60_), .b(new_n59_), .c(x02), .O(new_n180_));
  inv1   g129(.a(x32), .O(new_n181_));
  nor2   g130(.a(x32), .b(x31), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n163_), .c(new_n152_), .d(new_n151_), .O(new_n183_));
  oai22  g132(.a(new_n183_), .b(new_n106_), .c(new_n55_), .d(new_n181_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n180_), .O(new_n185_));
  nand3  g134(.a(x32), .b(x20), .c(x16), .O(new_n186_));
  inv1   g135(.a(new_n186_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(x18), .c(new_n70_), .O(new_n188_));
  nand3  g137(.a(new_n101_), .b(x16), .c(x15), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(x02), .O(new_n190_));
  nand3  g139(.a(new_n173_), .b(new_n143_), .c(new_n150_), .O(new_n191_));
  or2    g140(.a(new_n191_), .b(new_n125_), .O(new_n192_));
  nand3  g141(.a(new_n192_), .b(new_n190_), .c(x32), .O(new_n193_));
  nand2  g142(.a(new_n60_), .b(new_n52_), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n193_), .c(new_n188_), .d(new_n185_), .O(z13));
  inv1   g144(.a(x33), .O(new_n196_));
  nand4  g145(.a(new_n151_), .b(new_n93_), .c(new_n100_), .d(new_n72_), .O(new_n197_));
  inv1   g146(.a(x27), .O(new_n198_));
  inv1   g147(.a(x30), .O(new_n199_));
  nand4  g148(.a(new_n182_), .b(new_n172_), .c(new_n199_), .d(new_n198_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n197_), .c(new_n101_), .O(new_n201_));
  aoi21  g150(.a(new_n201_), .b(new_n65_), .c(new_n196_), .O(new_n202_));
  nand3  g151(.a(new_n172_), .b(new_n198_), .c(new_n123_), .O(new_n203_));
  nor2   g152(.a(x33), .b(x32), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n173_), .O(new_n205_));
  nor3   g154(.a(new_n205_), .b(new_n203_), .c(new_n117_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n202_), .c(x16), .O(new_n207_));
  inv1   g156(.a(x01), .O(new_n208_));
  aoi21  g157(.a(new_n60_), .b(new_n208_), .c(x18), .O(new_n209_));
  aoi21  g158(.a(new_n209_), .b(new_n207_), .c(new_n53_), .O(z14));
  inv1   g159(.a(x34), .O(new_n211_));
  nand4  g160(.a(new_n204_), .b(new_n173_), .c(new_n172_), .d(new_n198_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n197_), .c(new_n101_), .O(new_n213_));
  aoi21  g162(.a(new_n213_), .b(new_n65_), .c(new_n211_), .O(new_n214_));
  nand4  g163(.a(new_n173_), .b(new_n211_), .c(new_n196_), .d(new_n181_), .O(new_n215_));
  nor3   g164(.a(new_n215_), .b(new_n203_), .c(new_n117_), .O(new_n216_));
  oai21  g165(.a(new_n216_), .b(new_n214_), .c(x16), .O(new_n217_));
  inv1   g166(.a(x00), .O(new_n218_));
  aoi21  g167(.a(new_n60_), .b(new_n218_), .c(x18), .O(new_n219_));
  aoi21  g168(.a(new_n219_), .b(new_n217_), .c(new_n53_), .O(z15));
endmodule


