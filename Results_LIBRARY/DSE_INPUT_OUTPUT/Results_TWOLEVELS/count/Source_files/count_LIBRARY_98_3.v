// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  inv1   g000(.a(x15), .O(new_n52_));
  inv1   g001(.a(x16), .O(new_n53_));
  oai21  g002(.a(x18), .b(new_n53_), .c(new_n52_), .O(new_n54_));
  inv1   g003(.a(x18), .O(new_n55_));
  nand2  g004(.a(x19), .b(x17), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  inv1   g006(.a(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand3  g008(.a(new_n59_), .b(new_n55_), .c(x16), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  nand2  g010(.a(new_n58_), .b(x20), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  aoi21  g013(.a(new_n64_), .b(new_n62_), .c(new_n53_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(new_n55_), .O(new_n67_));
  nand2  g016(.a(x18), .b(new_n52_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n67_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n57_), .O(new_n72_));
  oai21  g021(.a(new_n63_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  aoi21  g023(.a(new_n73_), .b(x16), .c(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(x18), .c(new_n68_), .O(z02));
  inv1   g025(.a(x22), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nor2   g027(.a(x20), .b(x17), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  oai22  g030(.a(new_n81_), .b(x19), .c(new_n63_), .d(new_n77_), .O(new_n82_));
  nor2   g031(.a(x16), .b(x12), .O(new_n83_));
  aoi21  g032(.a(new_n82_), .b(x16), .c(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(x18), .c(new_n68_), .O(z03));
  nor2   g034(.a(new_n80_), .b(x19), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n64_), .c(x23), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n63_), .c(new_n70_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n53_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n90_), .c(new_n55_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n68_), .O(z04));
  inv1   g042(.a(x24), .O(new_n94_));
  nand2  g043(.a(new_n88_), .b(new_n71_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n55_), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n57_), .c(new_n94_), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n80_), .c(new_n63_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n97_), .c(x16), .O(new_n101_));
  inv1   g050(.a(x10), .O(new_n102_));
  aoi21  g051(.a(new_n53_), .b(new_n102_), .c(x18), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n101_), .O(z05));
  inv1   g053(.a(x25), .O(new_n105_));
  nand3  g054(.a(new_n98_), .b(new_n71_), .c(new_n77_), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n55_), .c(new_n58_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n88_), .c(new_n71_), .d(new_n57_), .O(new_n109_));
  oai21  g058(.a(new_n107_), .b(new_n105_), .c(new_n109_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g060(.a(x09), .O(new_n112_));
  aoi21  g061(.a(new_n53_), .b(new_n112_), .c(x18), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n111_), .O(z06));
  inv1   g063(.a(x26), .O(new_n115_));
  inv1   g064(.a(x20), .O(new_n116_));
  inv1   g065(.a(x23), .O(new_n117_));
  nand4  g066(.a(new_n108_), .b(new_n80_), .c(new_n117_), .d(new_n116_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n55_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n57_), .c(new_n115_), .O(new_n120_));
  nor2   g069(.a(x26), .b(x25), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(x24), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n88_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n72_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n120_), .c(x16), .O(new_n126_));
  inv1   g075(.a(x08), .O(new_n127_));
  aoi21  g076(.a(new_n53_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z07));
  inv1   g078(.a(x27), .O(new_n130_));
  inv1   g079(.a(x19), .O(new_n131_));
  nand4  g080(.a(new_n121_), .b(new_n88_), .c(new_n94_), .d(new_n70_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n131_), .c(new_n64_), .O(new_n133_));
  nand2  g082(.a(new_n80_), .b(new_n116_), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n58_), .O(new_n135_));
  nor3   g084(.a(x27), .b(x26), .c(x25), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n135_), .c(new_n98_), .O(new_n137_));
  oai21  g086(.a(new_n133_), .b(new_n130_), .c(new_n137_), .O(new_n138_));
  nor2   g087(.a(x16), .b(x07), .O(new_n139_));
  aoi21  g088(.a(new_n138_), .b(x16), .c(new_n139_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(x18), .c(new_n68_), .O(z08));
  inv1   g090(.a(x28), .O(new_n142_));
  nor2   g091(.a(x27), .b(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n108_), .c(new_n88_), .d(new_n70_), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n131_), .c(new_n64_), .O(new_n145_));
  nand2  g094(.a(new_n142_), .b(new_n130_), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(x26), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n135_), .c(new_n108_), .d(new_n117_), .O(new_n148_));
  oai21  g097(.a(new_n145_), .b(new_n142_), .c(new_n148_), .O(new_n149_));
  nor2   g098(.a(x16), .b(x06), .O(new_n150_));
  aoi21  g099(.a(new_n149_), .b(x16), .c(new_n150_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(x18), .c(new_n68_), .O(z09));
  inv1   g101(.a(x29), .O(new_n153_));
  nand2  g102(.a(new_n147_), .b(new_n108_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n95_), .c(new_n55_), .O(new_n155_));
  aoi21  g104(.a(new_n155_), .b(new_n57_), .c(new_n153_), .O(new_n156_));
  nor3   g105(.a(x29), .b(x28), .c(x27), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n123_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n89_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n156_), .c(x16), .O(new_n160_));
  inv1   g109(.a(x05), .O(new_n161_));
  aoi21  g110(.a(new_n53_), .b(new_n161_), .c(x18), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n160_), .O(z10));
  inv1   g112(.a(x30), .O(new_n164_));
  nand2  g113(.a(new_n157_), .b(new_n121_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n106_), .c(new_n55_), .O(new_n166_));
  aoi21  g115(.a(new_n166_), .b(new_n57_), .c(new_n164_), .O(new_n167_));
  nor2   g116(.a(x30), .b(x29), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n123_), .c(new_n142_), .d(new_n130_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n89_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(x16), .O(new_n171_));
  inv1   g120(.a(x04), .O(new_n172_));
  aoi21  g121(.a(new_n53_), .b(new_n172_), .c(x18), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z11));
  inv1   g123(.a(x31), .O(new_n175_));
  nand3  g124(.a(new_n168_), .b(new_n136_), .c(new_n142_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n106_), .c(new_n55_), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n57_), .c(new_n175_), .O(new_n178_));
  nor2   g127(.a(x29), .b(x28), .O(new_n179_));
  nor2   g128(.a(x31), .b(x30), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n179_), .c(new_n136_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n99_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n178_), .c(x16), .O(new_n183_));
  inv1   g132(.a(x03), .O(new_n184_));
  aoi21  g133(.a(new_n53_), .b(new_n184_), .c(x18), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n183_), .O(z12));
  inv1   g135(.a(x32), .O(new_n187_));
  nand3  g136(.a(new_n180_), .b(new_n147_), .c(new_n153_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n118_), .c(new_n55_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n57_), .c(new_n187_), .O(new_n190_));
  nand3  g139(.a(new_n168_), .b(new_n187_), .c(new_n175_), .O(new_n191_));
  nor4   g140(.a(new_n191_), .b(new_n146_), .c(new_n122_), .d(new_n99_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n190_), .c(x16), .O(new_n193_));
  inv1   g142(.a(x02), .O(new_n194_));
  aoi21  g143(.a(new_n53_), .b(new_n194_), .c(x18), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n193_), .O(z13));
  inv1   g145(.a(x33), .O(new_n197_));
  inv1   g146(.a(new_n191_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n147_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n118_), .c(new_n55_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n57_), .c(new_n197_), .O(new_n201_));
  nand2  g150(.a(new_n179_), .b(new_n143_), .O(new_n202_));
  nor2   g151(.a(x33), .b(x32), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n180_), .O(new_n204_));
  nor3   g153(.a(new_n204_), .b(new_n202_), .c(new_n109_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n201_), .c(x16), .O(new_n206_));
  inv1   g155(.a(x01), .O(new_n207_));
  aoi21  g156(.a(new_n53_), .b(new_n207_), .c(x18), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n206_), .O(z14));
  inv1   g158(.a(x34), .O(new_n210_));
  nand4  g159(.a(new_n203_), .b(new_n180_), .c(new_n179_), .d(new_n130_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n132_), .c(new_n131_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n63_), .c(new_n210_), .O(new_n213_));
  nand4  g162(.a(new_n180_), .b(new_n210_), .c(new_n197_), .d(new_n187_), .O(new_n214_));
  nor3   g163(.a(new_n214_), .b(new_n202_), .c(new_n109_), .O(new_n215_));
  oai21  g164(.a(new_n215_), .b(new_n213_), .c(x16), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  nand2  g166(.a(new_n53_), .b(new_n217_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n216_), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(new_n55_), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n68_), .O(z15));
endmodule


