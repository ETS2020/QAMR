// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_;
  inv1   g000(.a(x00), .O(new_n52_));
  nor2   g001(.a(x15), .b(new_n52_), .O(new_n53_));
  xnor2a g002(.a(x19), .b(x17), .O(new_n54_));
  aoi21  g003(.a(new_n54_), .b(x16), .c(x18), .O(new_n55_));
  inv1   g004(.a(x15), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  nand3  g006(.a(new_n57_), .b(new_n56_), .c(new_n52_), .O(new_n58_));
  oai21  g007(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(x19), .b(x17), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n57_), .b(new_n65_), .c(x18), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n53_), .O(z01));
  oai21  g016(.a(x15), .b(new_n52_), .c(x18), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nand3  g018(.a(new_n61_), .b(new_n69_), .c(new_n60_), .O(new_n70_));
  oai21  g019(.a(new_n63_), .b(new_n69_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x16), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  nor2   g022(.a(new_n73_), .b(new_n53_), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n72_), .c(new_n68_), .O(z02));
  nand2  g024(.a(x22), .b(x21), .O(new_n76_));
  inv1   g025(.a(x17), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n60_), .c(new_n77_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n76_), .c(x19), .O(new_n80_));
  inv1   g029(.a(x22), .O(new_n81_));
  nor2   g030(.a(new_n63_), .b(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n57_), .b(new_n84_), .c(x18), .O(new_n85_));
  aoi21  g034(.a(new_n85_), .b(new_n83_), .c(new_n53_), .O(z03));
  inv1   g035(.a(x23), .O(new_n87_));
  inv1   g036(.a(x19), .O(new_n88_));
  oai21  g037(.a(x22), .b(x21), .c(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n63_), .c(new_n87_), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n63_), .c(new_n69_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n90_), .c(x16), .O(new_n94_));
  inv1   g043(.a(x11), .O(new_n95_));
  aoi21  g044(.a(new_n57_), .b(new_n95_), .c(x18), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n94_), .c(new_n53_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n78_), .c(new_n63_), .O(new_n100_));
  oai21  g049(.a(new_n93_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x16), .O(new_n102_));
  nor2   g051(.a(x16), .b(x10), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n53_), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n102_), .c(new_n68_), .O(z05));
  inv1   g054(.a(x25), .O(new_n106_));
  nand2  g055(.a(new_n99_), .b(new_n78_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n88_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n63_), .c(new_n106_), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n91_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n70_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n109_), .c(x16), .O(new_n113_));
  inv1   g062(.a(x09), .O(new_n114_));
  aoi21  g063(.a(new_n57_), .b(new_n114_), .c(x18), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n113_), .c(new_n53_), .O(z06));
  inv1   g065(.a(x26), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n91_), .O(new_n119_));
  oai22  g068(.a(new_n119_), .b(new_n70_), .c(new_n112_), .d(new_n117_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(x16), .O(new_n121_));
  nor2   g070(.a(x16), .b(x08), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n53_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n121_), .c(new_n68_), .O(z07));
  inv1   g073(.a(x27), .O(new_n125_));
  nand2  g074(.a(new_n91_), .b(new_n69_), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n98_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n126_), .c(new_n88_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n63_), .c(new_n125_), .O(new_n130_));
  nand3  g079(.a(new_n78_), .b(new_n61_), .c(new_n60_), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n99_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n130_), .c(x16), .O(new_n135_));
  inv1   g084(.a(x07), .O(new_n136_));
  aoi21  g085(.a(new_n57_), .b(new_n136_), .c(x18), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n135_), .c(new_n53_), .O(z08));
  inv1   g087(.a(x28), .O(new_n139_));
  nor2   g088(.a(x27), .b(x26), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n110_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n126_), .c(new_n88_), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n63_), .c(new_n139_), .O(new_n143_));
  nand2  g092(.a(new_n110_), .b(new_n87_), .O(new_n144_));
  nor2   g093(.a(x28), .b(x27), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n117_), .O(new_n146_));
  nor3   g095(.a(new_n146_), .b(new_n144_), .c(new_n131_), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n143_), .c(x16), .O(new_n148_));
  inv1   g097(.a(x06), .O(new_n149_));
  aoi21  g098(.a(new_n57_), .b(new_n149_), .c(x18), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n148_), .c(new_n53_), .O(z09));
  inv1   g100(.a(x29), .O(new_n152_));
  nand3  g101(.a(new_n60_), .b(new_n88_), .c(new_n77_), .O(new_n153_));
  nand4  g102(.a(new_n145_), .b(new_n127_), .c(new_n99_), .d(new_n78_), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n88_), .c(new_n153_), .O(new_n155_));
  nor2   g104(.a(x29), .b(x28), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n118_), .c(new_n125_), .O(new_n157_));
  oai22  g106(.a(new_n157_), .b(new_n92_), .c(new_n155_), .d(new_n152_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  inv1   g108(.a(x05), .O(new_n160_));
  aoi21  g109(.a(new_n57_), .b(new_n160_), .c(x18), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n159_), .c(new_n53_), .O(z10));
  inv1   g111(.a(x30), .O(new_n163_));
  nand3  g112(.a(new_n156_), .b(new_n127_), .c(new_n125_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n107_), .c(new_n88_), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n63_), .c(new_n163_), .O(new_n166_));
  nor2   g115(.a(x30), .b(x29), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n145_), .c(new_n118_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n92_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n166_), .c(x16), .O(new_n170_));
  inv1   g119(.a(x04), .O(new_n171_));
  aoi21  g120(.a(new_n57_), .b(new_n171_), .c(x18), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n170_), .c(new_n53_), .O(z11));
  inv1   g122(.a(x31), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n156_), .c(new_n132_), .O(new_n176_));
  oai22  g125(.a(new_n176_), .b(new_n100_), .c(new_n169_), .d(new_n174_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(x16), .O(new_n178_));
  nor2   g127(.a(x16), .b(x03), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n53_), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n178_), .c(new_n68_), .O(z12));
  inv1   g130(.a(x32), .O(new_n182_));
  nand3  g131(.a(new_n110_), .b(new_n78_), .c(new_n87_), .O(new_n183_));
  nand4  g132(.a(new_n175_), .b(new_n145_), .c(new_n152_), .d(new_n117_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n183_), .c(new_n88_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n63_), .c(new_n182_), .O(new_n186_));
  nor2   g135(.a(x32), .b(x31), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n167_), .c(new_n145_), .d(new_n127_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n100_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n186_), .c(x16), .O(new_n190_));
  inv1   g139(.a(x02), .O(new_n191_));
  aoi21  g140(.a(new_n57_), .b(new_n191_), .c(x18), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n190_), .c(new_n53_), .O(z13));
  inv1   g142(.a(x33), .O(new_n194_));
  nand2  g143(.a(new_n156_), .b(new_n140_), .O(new_n195_));
  nand3  g144(.a(new_n175_), .b(new_n194_), .c(new_n182_), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n112_), .O(new_n198_));
  oai21  g147(.a(new_n189_), .b(new_n194_), .c(new_n198_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(x16), .O(new_n200_));
  nor2   g149(.a(x16), .b(x01), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n53_), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n200_), .c(new_n68_), .O(z14));
  oai21  g152(.a(x18), .b(new_n57_), .c(new_n52_), .O(new_n204_));
  nand2  g153(.a(new_n198_), .b(x34), .O(new_n205_));
  inv1   g154(.a(new_n195_), .O(new_n206_));
  nor3   g155(.a(x34), .b(x33), .c(x32), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n206_), .c(new_n175_), .d(new_n112_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n205_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(x16), .O(new_n210_));
  aoi21  g159(.a(x18), .b(x15), .c(new_n53_), .O(new_n211_));
  nand3  g160(.a(new_n211_), .b(new_n210_), .c(new_n204_), .O(z15));
endmodule


