// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n230_, new_n231_, new_n232_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x11), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(x06), .c(x18), .O(new_n58_));
  nor2   g007(.a(new_n57_), .b(x06), .O(new_n59_));
  nor2   g008(.a(x16), .b(x15), .O(new_n60_));
  nor2   g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n58_), .c(new_n56_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n59_), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n66_), .c(new_n58_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n55_), .O(new_n72_));
  oai21  g021(.a(new_n65_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x16), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n59_), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n74_), .c(new_n58_), .O(z02));
  nand2  g026(.a(x22), .b(x21), .O(new_n78_));
  nor2   g027(.a(x20), .b(x17), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n78_), .c(x19), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n65_), .b(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  inv1   g035(.a(x16), .O(new_n87_));
  aoi21  g036(.a(new_n87_), .b(new_n86_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n85_), .c(new_n59_), .O(z03));
  aoi21  g038(.a(x16), .b(x06), .c(new_n57_), .O(new_n90_));
  nand2  g039(.a(x23), .b(x21), .O(new_n91_));
  nor3   g040(.a(x23), .b(x22), .c(x21), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n79_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nor2   g043(.a(x22), .b(x20), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n55_), .O(new_n96_));
  aoi22  g045(.a(new_n96_), .b(x23), .c(new_n94_), .d(new_n53_), .O(new_n97_));
  inv1   g046(.a(x18), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(x16), .O(new_n99_));
  aoi22  g048(.a(new_n99_), .b(new_n57_), .c(x18), .d(x06), .O(new_n100_));
  oai21  g049(.a(new_n97_), .b(new_n90_), .c(new_n100_), .O(z04));
  nand2  g050(.a(x24), .b(x21), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n71_), .c(new_n83_), .d(new_n52_), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n102_), .c(x19), .O(new_n105_));
  inv1   g054(.a(x24), .O(new_n106_));
  nor3   g055(.a(x23), .b(x22), .c(x20), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n55_), .c(new_n106_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n105_), .c(x16), .O(new_n109_));
  inv1   g058(.a(x10), .O(new_n110_));
  aoi21  g059(.a(new_n87_), .b(new_n110_), .c(x18), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n109_), .c(new_n59_), .O(z05));
  nand2  g061(.a(x25), .b(x21), .O(new_n113_));
  nor2   g062(.a(x23), .b(x22), .O(new_n114_));
  nor2   g063(.a(x25), .b(x24), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n114_), .c(new_n71_), .d(new_n52_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n113_), .c(x19), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  nor3   g067(.a(x24), .b(x23), .c(x22), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n65_), .c(new_n118_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  inv1   g070(.a(x09), .O(new_n122_));
  aoi21  g071(.a(new_n87_), .b(new_n122_), .c(x18), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n121_), .c(new_n59_), .O(z06));
  nand2  g073(.a(x26), .b(x21), .O(new_n125_));
  nor2   g074(.a(x26), .b(x25), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n103_), .c(new_n80_), .d(new_n79_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n125_), .c(x19), .O(new_n128_));
  inv1   g077(.a(x26), .O(new_n129_));
  nand2  g078(.a(new_n115_), .b(new_n114_), .O(new_n130_));
  inv1   g079(.a(new_n130_), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n65_), .c(new_n129_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n128_), .c(x16), .O(new_n133_));
  inv1   g082(.a(x08), .O(new_n134_));
  aoi21  g083(.a(new_n87_), .b(new_n134_), .c(x18), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n133_), .c(new_n59_), .O(z07));
  nand2  g085(.a(x27), .b(x21), .O(new_n137_));
  nor3   g086(.a(x27), .b(x26), .c(x25), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n103_), .c(new_n80_), .d(new_n79_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n137_), .c(x19), .O(new_n140_));
  nand4  g089(.a(new_n126_), .b(new_n103_), .c(new_n95_), .d(new_n55_), .O(new_n141_));
  and2   g090(.a(new_n141_), .b(x27), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  nor2   g092(.a(x16), .b(x07), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n59_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(new_n58_), .O(z08));
  inv1   g095(.a(x06), .O(new_n147_));
  oai21  g096(.a(new_n87_), .b(x11), .c(new_n147_), .O(new_n148_));
  nor2   g097(.a(x27), .b(x21), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(x19), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n141_), .c(x28), .O(new_n151_));
  nor3   g100(.a(x22), .b(x21), .c(x20), .O(new_n152_));
  nor3   g101(.a(x25), .b(x24), .c(x23), .O(new_n153_));
  nor3   g102(.a(x28), .b(x27), .c(x26), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n152_), .d(new_n55_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x16), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n148_), .c(new_n58_), .O(z09));
  nor2   g107(.a(x28), .b(x27), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n70_), .c(x19), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n141_), .c(x29), .O(new_n161_));
  nor3   g110(.a(x26), .b(x25), .c(x24), .O(new_n162_));
  nor3   g111(.a(x29), .b(x28), .c(x27), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n162_), .c(new_n92_), .d(new_n65_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(x16), .O(new_n166_));
  inv1   g115(.a(x05), .O(new_n167_));
  aoi21  g116(.a(new_n87_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n59_), .O(z10));
  inv1   g118(.a(x30), .O(new_n170_));
  inv1   g119(.a(x27), .O(new_n171_));
  inv1   g120(.a(x28), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n171_), .c(new_n70_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n53_), .O(new_n174_));
  nor2   g123(.a(x29), .b(x26), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n103_), .c(new_n118_), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n96_), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n174_), .c(new_n170_), .O(new_n178_));
  nand2  g127(.a(new_n92_), .b(new_n65_), .O(new_n179_));
  nor2   g128(.a(x30), .b(x29), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n162_), .c(new_n159_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n179_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n178_), .c(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x04), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n59_), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n183_), .c(new_n58_), .O(z11));
  inv1   g135(.a(x31), .O(new_n187_));
  nand3  g136(.a(new_n149_), .b(new_n170_), .c(new_n172_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n53_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n177_), .c(new_n187_), .O(new_n190_));
  nand3  g139(.a(new_n103_), .b(new_n80_), .c(new_n65_), .O(new_n191_));
  nor2   g140(.a(x29), .b(x28), .O(new_n192_));
  nor2   g141(.a(x31), .b(x30), .O(new_n193_));
  nand3  g142(.a(new_n193_), .b(new_n192_), .c(new_n138_), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n191_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n190_), .c(x16), .O(new_n196_));
  nor2   g145(.a(x16), .b(x03), .O(new_n197_));
  nor2   g146(.a(new_n197_), .b(new_n59_), .O(new_n198_));
  nand3  g147(.a(new_n198_), .b(new_n196_), .c(new_n58_), .O(z12));
  inv1   g148(.a(x32), .O(new_n200_));
  nand3  g149(.a(new_n193_), .b(new_n149_), .c(new_n172_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n53_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n177_), .c(new_n200_), .O(new_n203_));
  nor2   g152(.a(x32), .b(x31), .O(new_n204_));
  nand4  g153(.a(new_n204_), .b(new_n180_), .c(new_n159_), .d(new_n126_), .O(new_n205_));
  nor2   g154(.a(new_n205_), .b(new_n191_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n203_), .c(x16), .O(new_n207_));
  nor2   g156(.a(x16), .b(x02), .O(new_n208_));
  nor2   g157(.a(new_n208_), .b(new_n59_), .O(new_n209_));
  nand3  g158(.a(new_n209_), .b(new_n207_), .c(new_n58_), .O(z13));
  inv1   g159(.a(x33), .O(new_n211_));
  nand3  g160(.a(new_n200_), .b(new_n187_), .c(new_n170_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n173_), .c(new_n53_), .O(new_n213_));
  aoi21  g162(.a(new_n213_), .b(new_n177_), .c(new_n211_), .O(new_n214_));
  nand4  g163(.a(new_n115_), .b(new_n114_), .c(new_n71_), .d(new_n55_), .O(new_n215_));
  nand3  g164(.a(new_n192_), .b(new_n171_), .c(new_n129_), .O(new_n216_));
  nor2   g165(.a(x33), .b(x32), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n193_), .O(new_n218_));
  nor3   g167(.a(new_n218_), .b(new_n216_), .c(new_n215_), .O(new_n219_));
  oai21  g168(.a(new_n219_), .b(new_n214_), .c(x16), .O(new_n220_));
  nor2   g169(.a(x16), .b(x01), .O(new_n221_));
  nor2   g170(.a(new_n221_), .b(new_n59_), .O(new_n222_));
  nand3  g171(.a(new_n222_), .b(new_n220_), .c(new_n58_), .O(z14));
  inv1   g172(.a(x34), .O(new_n224_));
  nand4  g173(.a(new_n217_), .b(new_n193_), .c(new_n159_), .d(new_n70_), .O(new_n225_));
  nand2  g174(.a(new_n225_), .b(new_n53_), .O(new_n226_));
  aoi21  g175(.a(new_n226_), .b(new_n177_), .c(new_n224_), .O(new_n227_));
  nand4  g176(.a(new_n193_), .b(new_n224_), .c(new_n211_), .d(new_n200_), .O(new_n228_));
  nor3   g177(.a(new_n228_), .b(new_n216_), .c(new_n215_), .O(new_n229_));
  oai21  g178(.a(new_n229_), .b(new_n227_), .c(x16), .O(new_n230_));
  nor2   g179(.a(x16), .b(x00), .O(new_n231_));
  nor2   g180(.a(new_n231_), .b(new_n59_), .O(new_n232_));
  nand3  g181(.a(new_n232_), .b(new_n230_), .c(new_n58_), .O(z15));
endmodule


