// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x14), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x09), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(x18), .O(new_n59_));
  inv1   g008(.a(new_n58_), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g012(.a(x16), .O(new_n64_));
  oai21  g013(.a(x19), .b(x17), .c(x20), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  nand3  g015(.a(new_n66_), .b(new_n53_), .c(new_n52_), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(x18), .c(new_n58_), .O(new_n69_));
  inv1   g018(.a(x09), .O(new_n70_));
  nand3  g019(.a(new_n64_), .b(new_n57_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n69_), .O(z01));
  inv1   g021(.a(x21), .O(new_n73_));
  inv1   g022(.a(new_n67_), .O(new_n74_));
  nand4  g023(.a(new_n73_), .b(new_n66_), .c(new_n53_), .d(new_n52_), .O(new_n75_));
  oai21  g024(.a(new_n74_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n64_), .b(new_n78_), .c(x18), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n60_), .O(z02));
  inv1   g029(.a(x22), .O(new_n81_));
  inv1   g030(.a(new_n75_), .O(new_n82_));
  nor3   g031(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n55_), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n81_), .c(new_n84_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n64_), .b(new_n87_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n60_), .O(z03));
  inv1   g038(.a(x23), .O(new_n90_));
  aoi21  g039(.a(new_n83_), .b(new_n55_), .c(new_n90_), .O(new_n91_));
  nand3  g040(.a(new_n90_), .b(new_n81_), .c(new_n73_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n67_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  inv1   g043(.a(x11), .O(new_n95_));
  aoi21  g044(.a(new_n64_), .b(new_n95_), .c(x18), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n94_), .c(new_n60_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  nor2   g047(.a(x20), .b(x19), .O(new_n99_));
  nor2   g048(.a(x22), .b(x21), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n52_), .O(new_n102_));
  oai21  g051(.a(new_n93_), .b(new_n98_), .c(new_n102_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x16), .O(new_n104_));
  nor2   g053(.a(x16), .b(x10), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n60_), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n104_), .c(new_n59_), .O(z05));
  oai21  g056(.a(new_n64_), .b(new_n57_), .c(x09), .O(new_n108_));
  inv1   g057(.a(x25), .O(new_n109_));
  nor3   g058(.a(x21), .b(x19), .c(x17), .O(new_n110_));
  nor3   g059(.a(x24), .b(x23), .c(x22), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  nand4  g061(.a(new_n109_), .b(new_n98_), .c(new_n90_), .d(new_n81_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n75_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n112_), .c(new_n108_), .O(new_n115_));
  inv1   g064(.a(x18), .O(new_n116_));
  nand3  g065(.a(x25), .b(x20), .c(x16), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  aoi22  g067(.a(new_n118_), .b(new_n58_), .c(new_n64_), .d(new_n70_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n115_), .O(z06));
  nor2   g069(.a(x21), .b(x20), .O(new_n121_));
  nor2   g070(.a(x23), .b(x22), .O(new_n122_));
  nor2   g071(.a(x25), .b(x24), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n55_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x26), .O(new_n125_));
  nor3   g074(.a(x26), .b(x25), .c(x24), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n122_), .c(new_n82_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  nor2   g078(.a(x16), .b(x08), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n60_), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n129_), .c(new_n59_), .O(z07));
  nand2  g081(.a(new_n127_), .b(x27), .O(new_n133_));
  nor3   g082(.a(x27), .b(x26), .c(x25), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n101_), .c(new_n83_), .d(new_n55_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  nor2   g086(.a(x16), .b(x07), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n60_), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n137_), .c(new_n59_), .O(z08));
  nand2  g089(.a(new_n135_), .b(x28), .O(new_n141_));
  nor3   g090(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nor3   g091(.a(x28), .b(x27), .c(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n142_), .c(new_n83_), .d(new_n55_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(x16), .O(new_n146_));
  inv1   g095(.a(x06), .O(new_n147_));
  aoi21  g096(.a(new_n64_), .b(new_n147_), .c(x18), .O(new_n148_));
  aoi21  g097(.a(new_n148_), .b(new_n146_), .c(new_n60_), .O(z09));
  and2   g098(.a(new_n144_), .b(x29), .O(new_n150_));
  nand4  g099(.a(new_n122_), .b(new_n99_), .c(new_n73_), .d(new_n52_), .O(new_n151_));
  inv1   g100(.a(x27), .O(new_n152_));
  nor2   g101(.a(x26), .b(x25), .O(new_n153_));
  nor2   g102(.a(x29), .b(x28), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n152_), .d(new_n98_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n150_), .c(x16), .O(new_n157_));
  nor2   g106(.a(x16), .b(x05), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n60_), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n157_), .c(new_n59_), .O(z10));
  oai21  g109(.a(new_n155_), .b(new_n151_), .c(x30), .O(new_n161_));
  nor2   g110(.a(x28), .b(x27), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n162_), .c(new_n126_), .d(new_n93_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n64_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n60_), .O(z11));
  nand4  g118(.a(new_n163_), .b(new_n162_), .c(new_n153_), .d(new_n98_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n151_), .c(x31), .O(new_n171_));
  nand2  g120(.a(new_n101_), .b(new_n100_), .O(new_n172_));
  inv1   g121(.a(new_n172_), .O(new_n173_));
  nor2   g122(.a(x31), .b(x30), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n154_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n134_), .c(new_n173_), .d(new_n74_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n171_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x16), .O(new_n179_));
  inv1   g128(.a(x03), .O(new_n180_));
  aoi21  g129(.a(new_n64_), .b(new_n180_), .c(x18), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n179_), .c(new_n60_), .O(z12));
  nand3  g131(.a(new_n174_), .b(new_n154_), .c(new_n134_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n102_), .c(x32), .O(new_n184_));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n163_), .c(new_n162_), .d(new_n153_), .O(new_n186_));
  or2    g135(.a(new_n186_), .b(new_n102_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n184_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(x16), .O(new_n189_));
  nor2   g138(.a(x16), .b(x02), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n60_), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(new_n189_), .c(new_n59_), .O(z13));
  oai21  g141(.a(new_n186_), .b(new_n102_), .c(x33), .O(new_n193_));
  nor2   g142(.a(x27), .b(x26), .O(new_n194_));
  nor2   g143(.a(x33), .b(x32), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n174_), .c(new_n154_), .d(new_n194_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n114_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n193_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(x16), .O(new_n200_));
  inv1   g149(.a(x01), .O(new_n201_));
  aoi21  g150(.a(new_n64_), .b(new_n201_), .c(x18), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n200_), .c(new_n60_), .O(z14));
  oai21  g152(.a(new_n196_), .b(new_n124_), .c(x34), .O(new_n204_));
  nand2  g153(.a(new_n154_), .b(new_n194_), .O(new_n205_));
  inv1   g154(.a(new_n205_), .O(new_n206_));
  inv1   g155(.a(x30), .O(new_n207_));
  inv1   g156(.a(x31), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n207_), .O(new_n209_));
  inv1   g158(.a(x32), .O(new_n210_));
  inv1   g159(.a(x33), .O(new_n211_));
  inv1   g160(.a(x34), .O(new_n212_));
  nand3  g161(.a(new_n212_), .b(new_n211_), .c(new_n210_), .O(new_n213_));
  nor2   g162(.a(new_n213_), .b(new_n209_), .O(new_n214_));
  nand3  g163(.a(new_n214_), .b(new_n206_), .c(new_n114_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n204_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(x16), .O(new_n217_));
  inv1   g166(.a(x00), .O(new_n218_));
  aoi21  g167(.a(new_n64_), .b(new_n218_), .c(x18), .O(new_n219_));
  aoi21  g168(.a(new_n219_), .b(new_n217_), .c(new_n60_), .O(z15));
endmodule


