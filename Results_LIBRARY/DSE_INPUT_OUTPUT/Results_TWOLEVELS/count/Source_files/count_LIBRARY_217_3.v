// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(new_n55_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x20), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n58_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nor2   g017(.a(x30), .b(new_n61_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  inv1   g020(.a(x20), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  nand3  g022(.a(new_n55_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n65_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n58_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n58_), .c(new_n78_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n73_), .O(new_n81_));
  nor2   g030(.a(x20), .b(x17), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  oai22  g033(.a(new_n84_), .b(x19), .c(new_n64_), .d(new_n80_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  aoi21  g035(.a(new_n85_), .b(x16), .c(new_n86_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(x18), .c(new_n70_), .O(z03));
  nor2   g037(.a(new_n83_), .b(x19), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n65_), .c(x23), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  aoi21  g043(.a(new_n94_), .b(new_n90_), .c(new_n58_), .O(new_n95_));
  nor2   g044(.a(x16), .b(x11), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n95_), .c(new_n61_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n70_), .O(z04));
  inv1   g047(.a(x24), .O(new_n99_));
  aoi21  g048(.a(new_n92_), .b(new_n53_), .c(new_n65_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n83_), .c(new_n64_), .O(new_n102_));
  oai21  g051(.a(new_n100_), .b(new_n99_), .c(new_n102_), .O(new_n103_));
  nor2   g052(.a(x16), .b(x10), .O(new_n104_));
  aoi21  g053(.a(new_n103_), .b(x16), .c(new_n104_), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(x18), .c(new_n70_), .O(z05));
  inv1   g055(.a(x25), .O(new_n107_));
  nand2  g056(.a(new_n101_), .b(new_n83_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n53_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n64_), .c(new_n107_), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n91_), .O(new_n112_));
  inv1   g061(.a(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n75_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n110_), .c(x16), .O(new_n116_));
  inv1   g065(.a(x09), .O(new_n117_));
  aoi21  g066(.a(new_n58_), .b(new_n117_), .c(x18), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(z06));
  inv1   g068(.a(x26), .O(new_n120_));
  nor3   g069(.a(x25), .b(x24), .c(x23), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n83_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n53_), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n64_), .c(new_n120_), .O(new_n124_));
  nor3   g073(.a(x26), .b(x25), .c(x24), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n91_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n74_), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n124_), .c(x16), .O(new_n128_));
  inv1   g077(.a(x08), .O(new_n129_));
  aoi21  g078(.a(new_n58_), .b(new_n129_), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n128_), .O(z07));
  inv1   g080(.a(x27), .O(new_n132_));
  nand2  g081(.a(new_n125_), .b(new_n93_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n53_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n64_), .c(new_n132_), .O(new_n135_));
  nand2  g084(.a(new_n83_), .b(new_n72_), .O(new_n136_));
  nor2   g085(.a(x27), .b(x26), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n101_), .c(new_n107_), .O(new_n138_));
  nor3   g087(.a(new_n138_), .b(new_n136_), .c(new_n62_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n135_), .c(x16), .O(new_n140_));
  inv1   g089(.a(x07), .O(new_n141_));
  aoi21  g090(.a(new_n58_), .b(new_n141_), .c(x18), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(z08));
  inv1   g092(.a(x28), .O(new_n144_));
  nand4  g093(.a(new_n137_), .b(new_n111_), .c(new_n91_), .d(new_n73_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n53_), .c(new_n65_), .O(new_n146_));
  nor2   g095(.a(new_n136_), .b(new_n62_), .O(new_n147_));
  nor3   g096(.a(x28), .b(x27), .c(x26), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n147_), .c(new_n121_), .O(new_n149_));
  oai21  g098(.a(new_n146_), .b(new_n144_), .c(new_n149_), .O(new_n150_));
  nor2   g099(.a(x16), .b(x06), .O(new_n151_));
  aoi21  g100(.a(new_n150_), .b(x16), .c(new_n151_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(x18), .c(new_n70_), .O(z09));
  inv1   g102(.a(x29), .O(new_n154_));
  nor2   g103(.a(x26), .b(x25), .O(new_n155_));
  nor2   g104(.a(x28), .b(x27), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n155_), .c(new_n101_), .d(new_n83_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n53_), .c(new_n65_), .O(new_n158_));
  nor3   g107(.a(x29), .b(x28), .c(x27), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n125_), .O(new_n160_));
  oai22  g109(.a(new_n160_), .b(new_n94_), .c(new_n158_), .d(new_n154_), .O(new_n161_));
  nor2   g110(.a(x16), .b(x05), .O(new_n162_));
  aoi21  g111(.a(new_n161_), .b(x16), .c(new_n162_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(x18), .c(new_n70_), .O(z10));
  inv1   g113(.a(x30), .O(new_n165_));
  nand4  g114(.a(new_n83_), .b(new_n55_), .c(new_n72_), .d(x16), .O(new_n166_));
  nor2   g115(.a(x29), .b(x28), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n137_), .c(new_n121_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n166_), .c(new_n61_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n165_), .O(new_n170_));
  nand4  g119(.a(new_n159_), .b(new_n155_), .c(new_n101_), .d(new_n83_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n53_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n64_), .c(new_n165_), .O(new_n173_));
  nor2   g122(.a(x16), .b(x04), .O(new_n174_));
  aoi21  g123(.a(new_n173_), .b(x16), .c(new_n174_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(x18), .c(new_n170_), .O(z11));
  nand3  g125(.a(new_n165_), .b(new_n53_), .c(new_n52_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n61_), .O(new_n178_));
  aoi21  g127(.a(new_n171_), .b(new_n53_), .c(x20), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(x30), .c(new_n178_), .O(new_n180_));
  nor2   g129(.a(x31), .b(x30), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n167_), .c(new_n137_), .d(new_n107_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n102_), .O(new_n183_));
  aoi21  g132(.a(new_n180_), .b(x31), .c(new_n183_), .O(new_n184_));
  inv1   g133(.a(x03), .O(new_n185_));
  nor2   g134(.a(x18), .b(x16), .O(new_n186_));
  aoi21  g135(.a(new_n186_), .b(new_n185_), .c(new_n69_), .O(new_n187_));
  oai21  g136(.a(new_n184_), .b(new_n58_), .c(new_n187_), .O(z12));
  nor3   g137(.a(x31), .b(x29), .c(x28), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n137_), .c(new_n121_), .d(new_n83_), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n53_), .c(x20), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(x30), .c(new_n178_), .O(new_n192_));
  nor2   g141(.a(x30), .b(x29), .O(new_n193_));
  nor2   g142(.a(x32), .b(x31), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n193_), .c(new_n156_), .d(new_n155_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n102_), .O(new_n196_));
  aoi21  g145(.a(new_n192_), .b(x32), .c(new_n196_), .O(new_n197_));
  inv1   g146(.a(x02), .O(new_n198_));
  aoi21  g147(.a(new_n186_), .b(new_n198_), .c(new_n69_), .O(new_n199_));
  oai21  g148(.a(new_n197_), .b(new_n58_), .c(new_n199_), .O(z13));
  nor3   g149(.a(x32), .b(x31), .c(x29), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n148_), .c(new_n121_), .d(new_n83_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n53_), .c(x20), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(x30), .c(new_n178_), .O(new_n204_));
  nand2  g153(.a(new_n167_), .b(new_n137_), .O(new_n205_));
  inv1   g154(.a(x32), .O(new_n206_));
  inv1   g155(.a(x33), .O(new_n207_));
  nand3  g156(.a(new_n181_), .b(new_n207_), .c(new_n206_), .O(new_n208_));
  nor3   g157(.a(new_n208_), .b(new_n205_), .c(new_n114_), .O(new_n209_));
  aoi21  g158(.a(new_n204_), .b(x33), .c(new_n209_), .O(new_n210_));
  inv1   g159(.a(x01), .O(new_n211_));
  aoi21  g160(.a(new_n186_), .b(new_n211_), .c(new_n69_), .O(new_n212_));
  oai21  g161(.a(new_n210_), .b(new_n58_), .c(new_n212_), .O(z14));
  inv1   g162(.a(x34), .O(new_n214_));
  inv1   g163(.a(new_n208_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n159_), .O(new_n216_));
  oai21  g165(.a(new_n216_), .b(new_n133_), .c(new_n53_), .O(new_n217_));
  aoi21  g166(.a(new_n217_), .b(new_n64_), .c(new_n214_), .O(new_n218_));
  nand4  g167(.a(new_n181_), .b(new_n214_), .c(new_n207_), .d(new_n206_), .O(new_n219_));
  nor3   g168(.a(new_n219_), .b(new_n205_), .c(new_n114_), .O(new_n220_));
  oai21  g169(.a(new_n220_), .b(new_n218_), .c(x16), .O(new_n221_));
  inv1   g170(.a(x00), .O(new_n222_));
  aoi21  g171(.a(new_n58_), .b(new_n222_), .c(x18), .O(new_n223_));
  nand2  g172(.a(new_n223_), .b(new_n221_), .O(z15));
endmodule


