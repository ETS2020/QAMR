// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g005(.a(x16), .b(x15), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x16), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(x25), .c(new_n52_), .O(z00));
  inv1   g008(.a(x25), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n55_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  nor2   g012(.a(x20), .b(x19), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n60_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n52_), .O(z01));
  inv1   g018(.a(x19), .O(new_n70_));
  nand2  g019(.a(new_n60_), .b(x20), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n70_), .c(new_n63_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n54_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nand2  g026(.a(new_n60_), .b(new_n52_), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  oai21  g028(.a(new_n76_), .b(new_n61_), .c(new_n79_), .O(z02));
  oai21  g029(.a(new_n73_), .b(x25), .c(new_n54_), .O(new_n81_));
  inv1   g030(.a(x20), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n55_), .O(new_n85_));
  aoi21  g034(.a(new_n81_), .b(x22), .c(new_n85_), .O(new_n86_));
  nor2   g035(.a(x16), .b(x12), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n78_), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n61_), .c(new_n88_), .O(z03));
  nand2  g038(.a(new_n84_), .b(new_n60_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n54_), .O(new_n91_));
  inv1   g040(.a(x21), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n65_), .O(new_n95_));
  aoi21  g044(.a(new_n91_), .b(x23), .c(new_n95_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x11), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n78_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n61_), .c(new_n98_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  aoi21  g049(.a(new_n94_), .b(new_n70_), .c(new_n65_), .O(new_n101_));
  nor2   g050(.a(x24), .b(x23), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n83_), .c(new_n64_), .d(new_n63_), .O(new_n103_));
  oai21  g052(.a(new_n101_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nor2   g053(.a(x16), .b(x10), .O(new_n105_));
  aoi21  g054(.a(new_n104_), .b(x16), .c(new_n105_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(x25), .c(new_n52_), .O(z05));
  nand2  g056(.a(new_n102_), .b(new_n83_), .O(new_n108_));
  nand3  g057(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n109_));
  oai22  g058(.a(new_n109_), .b(new_n108_), .c(x16), .d(x09), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n60_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n52_), .O(z06));
  inv1   g061(.a(x26), .O(new_n113_));
  inv1   g062(.a(x22), .O(new_n114_));
  nand3  g063(.a(new_n102_), .b(new_n73_), .c(new_n114_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n60_), .c(new_n55_), .O(new_n116_));
  nor2   g065(.a(x26), .b(x24), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n93_), .c(new_n75_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n113_), .c(new_n118_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  nor2   g069(.a(x16), .b(x08), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n78_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n120_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  nor3   g073(.a(x26), .b(x24), .c(x23), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n83_), .c(new_n82_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n60_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n54_), .c(new_n124_), .O(new_n128_));
  nor3   g077(.a(x27), .b(x26), .c(x24), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n93_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n74_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  nor2   g081(.a(x16), .b(x07), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n78_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  inv1   g085(.a(x23), .O(new_n137_));
  nand3  g086(.a(new_n117_), .b(new_n83_), .c(new_n137_), .O(new_n138_));
  nand3  g087(.a(new_n54_), .b(new_n124_), .c(new_n82_), .O(new_n139_));
  aoi21  g088(.a(new_n138_), .b(new_n70_), .c(new_n139_), .O(new_n140_));
  nor3   g089(.a(x28), .b(x27), .c(x26), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n102_), .c(new_n85_), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n136_), .c(new_n142_), .O(new_n143_));
  nor2   g092(.a(x16), .b(x06), .O(new_n144_));
  aoi21  g093(.a(new_n143_), .b(x16), .c(new_n144_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(x25), .c(new_n52_), .O(z09));
  inv1   g095(.a(x29), .O(new_n147_));
  nor2   g096(.a(x28), .b(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n117_), .c(new_n93_), .d(new_n73_), .O(new_n149_));
  aoi21  g098(.a(new_n149_), .b(new_n60_), .c(new_n55_), .O(new_n150_));
  nor2   g099(.a(x29), .b(x28), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n125_), .c(new_n85_), .d(new_n124_), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n147_), .c(new_n152_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  nor2   g103(.a(x16), .b(x05), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n78_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n154_), .O(z10));
  inv1   g106(.a(x30), .O(new_n158_));
  nand4  g107(.a(new_n151_), .b(new_n64_), .c(new_n124_), .d(new_n63_), .O(new_n159_));
  aoi21  g108(.a(new_n138_), .b(new_n70_), .c(new_n159_), .O(new_n160_));
  nor3   g109(.a(x30), .b(x29), .c(x28), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n129_), .c(new_n95_), .O(new_n162_));
  oai21  g111(.a(new_n160_), .b(new_n158_), .c(new_n162_), .O(new_n163_));
  nor2   g112(.a(x16), .b(x04), .O(new_n164_));
  aoi21  g113(.a(new_n163_), .b(x16), .c(new_n164_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(x25), .c(new_n52_), .O(z11));
  inv1   g115(.a(new_n65_), .O(new_n167_));
  nand2  g116(.a(new_n138_), .b(new_n70_), .O(new_n168_));
  nor2   g117(.a(x30), .b(x29), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n148_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n168_), .c(new_n167_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x31), .O(new_n173_));
  nor2   g122(.a(x31), .b(x30), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n151_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n129_), .c(new_n95_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n173_), .O(new_n178_));
  nor2   g127(.a(x16), .b(x03), .O(new_n179_));
  aoi21  g128(.a(new_n178_), .b(x16), .c(new_n179_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(x25), .c(new_n52_), .O(z12));
  inv1   g130(.a(new_n139_), .O(new_n182_));
  nand3  g131(.a(new_n176_), .b(new_n182_), .c(new_n168_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(x32), .O(new_n184_));
  inv1   g133(.a(new_n103_), .O(new_n185_));
  nor2   g134(.a(x32), .b(x31), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n169_), .c(new_n141_), .d(new_n185_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n184_), .O(new_n188_));
  nor2   g137(.a(x16), .b(x02), .O(new_n189_));
  aoi21  g138(.a(new_n188_), .b(x16), .c(new_n189_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(x25), .c(new_n52_), .O(z13));
  nand2  g140(.a(new_n147_), .b(new_n136_), .O(new_n192_));
  inv1   g141(.a(x31), .O(new_n193_));
  inv1   g142(.a(x32), .O(new_n194_));
  nand3  g143(.a(new_n194_), .b(new_n193_), .c(new_n158_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n192_), .O(new_n196_));
  nand3  g145(.a(new_n196_), .b(new_n182_), .c(new_n168_), .O(new_n197_));
  nor2   g146(.a(x27), .b(x26), .O(new_n198_));
  nor2   g147(.a(x33), .b(x32), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n174_), .c(new_n151_), .d(new_n198_), .O(new_n200_));
  nor2   g149(.a(new_n200_), .b(new_n103_), .O(new_n201_));
  aoi21  g150(.a(new_n197_), .b(x33), .c(new_n201_), .O(new_n202_));
  inv1   g151(.a(x01), .O(new_n203_));
  nand2  g152(.a(new_n61_), .b(new_n203_), .O(new_n204_));
  oai21  g153(.a(new_n202_), .b(new_n61_), .c(new_n204_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n60_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n52_), .O(z14));
  inv1   g156(.a(x34), .O(new_n208_));
  nand4  g157(.a(new_n199_), .b(new_n174_), .c(new_n151_), .d(new_n124_), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(new_n126_), .c(new_n60_), .O(new_n210_));
  aoi21  g159(.a(new_n210_), .b(new_n54_), .c(new_n208_), .O(new_n211_));
  nor2   g160(.a(x34), .b(x33), .O(new_n212_));
  nand3  g161(.a(new_n212_), .b(new_n186_), .c(new_n171_), .O(new_n213_));
  nor2   g162(.a(new_n213_), .b(new_n118_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n211_), .c(x16), .O(new_n215_));
  nor2   g164(.a(x16), .b(x00), .O(new_n216_));
  nor2   g165(.a(new_n216_), .b(new_n78_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n215_), .O(z15));
endmodule


