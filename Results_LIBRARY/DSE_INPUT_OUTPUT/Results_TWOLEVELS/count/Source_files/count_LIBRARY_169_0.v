// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  nor2   g005(.a(x27), .b(x26), .O(new_n57_));
  nor2   g006(.a(x16), .b(x15), .O(new_n58_));
  nor3   g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(new_n57_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(x18), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n57_), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n66_), .c(new_n62_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n55_), .O(new_n72_));
  oai21  g021(.a(new_n65_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x16), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  inv1   g024(.a(x16), .O(new_n76_));
  aoi21  g025(.a(new_n76_), .b(new_n75_), .c(x18), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n74_), .c(new_n57_), .O(z02));
  nand2  g027(.a(x22), .b(x21), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n63_), .c(new_n52_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n65_), .b(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n57_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n62_), .O(z03));
  inv1   g037(.a(new_n65_), .O(new_n89_));
  nand3  g038(.a(new_n80_), .b(new_n55_), .c(new_n63_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n70_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n89_), .c(new_n91_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x16), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n76_), .b(new_n96_), .c(x18), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n57_), .O(z04));
  inv1   g047(.a(x24), .O(new_n99_));
  nand2  g048(.a(new_n93_), .b(new_n53_), .O(new_n100_));
  aoi21  g049(.a(new_n100_), .b(new_n65_), .c(new_n99_), .O(new_n101_));
  nor2   g050(.a(x24), .b(x23), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n80_), .c(new_n65_), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nor2   g054(.a(x16), .b(x10), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n57_), .O(new_n107_));
  nand3  g056(.a(new_n107_), .b(new_n105_), .c(new_n62_), .O(z05));
  nand3  g057(.a(new_n103_), .b(x25), .c(x16), .O(new_n109_));
  inv1   g058(.a(x09), .O(new_n110_));
  aoi21  g059(.a(new_n76_), .b(new_n110_), .c(x18), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n61_), .O(new_n113_));
  inv1   g062(.a(x23), .O(new_n114_));
  inv1   g063(.a(x26), .O(new_n115_));
  inv1   g064(.a(x27), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n115_), .c(x25), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n99_), .c(new_n114_), .d(new_n83_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(x21), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n63_), .c(new_n53_), .d(new_n52_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n76_), .c(new_n113_), .O(z06));
  nand4  g070(.a(new_n71_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n122_));
  inv1   g071(.a(x25), .O(new_n123_));
  nand4  g072(.a(new_n92_), .b(x27), .c(new_n123_), .d(new_n99_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n122_), .c(x27), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n115_), .O(new_n126_));
  oai21  g075(.a(x24), .b(x22), .c(new_n53_), .O(new_n127_));
  nor3   g076(.a(x25), .b(x23), .c(x21), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(new_n65_), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(x26), .c(x16), .O(new_n130_));
  inv1   g079(.a(x08), .O(new_n131_));
  nand2  g080(.a(new_n76_), .b(new_n131_), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n130_), .c(new_n126_), .d(new_n62_), .O(z07));
  inv1   g082(.a(x07), .O(new_n134_));
  aoi21  g083(.a(new_n76_), .b(new_n134_), .c(x18), .O(new_n135_));
  nand2  g084(.a(new_n128_), .b(new_n65_), .O(new_n136_));
  nor2   g085(.a(x26), .b(x24), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n83_), .c(x19), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n136_), .c(x27), .O(new_n139_));
  oai22  g088(.a(new_n139_), .b(new_n76_), .c(new_n135_), .d(new_n57_), .O(z08));
  inv1   g089(.a(x18), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  nand2  g091(.a(new_n76_), .b(new_n142_), .O(new_n143_));
  nand2  g092(.a(x28), .b(x16), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n143_), .c(new_n61_), .d(new_n141_), .O(z09));
  inv1   g094(.a(x05), .O(new_n146_));
  nand2  g095(.a(new_n76_), .b(new_n146_), .O(new_n147_));
  aoi21  g096(.a(x29), .b(x16), .c(x18), .O(new_n148_));
  aoi21  g097(.a(new_n148_), .b(new_n147_), .c(new_n57_), .O(z10));
  inv1   g098(.a(x04), .O(new_n150_));
  nand2  g099(.a(new_n76_), .b(new_n150_), .O(new_n151_));
  nand2  g100(.a(x30), .b(x16), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n151_), .c(new_n61_), .d(new_n141_), .O(z11));
  inv1   g102(.a(x03), .O(new_n154_));
  nand2  g103(.a(new_n76_), .b(new_n154_), .O(new_n155_));
  aoi21  g104(.a(x31), .b(x16), .c(x18), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n155_), .c(new_n57_), .O(z12));
  inv1   g106(.a(x02), .O(new_n158_));
  nand2  g107(.a(new_n76_), .b(new_n158_), .O(new_n159_));
  nand2  g108(.a(x32), .b(x16), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n159_), .c(new_n61_), .d(new_n141_), .O(z13));
  inv1   g110(.a(x01), .O(new_n162_));
  nand2  g111(.a(new_n76_), .b(new_n162_), .O(new_n163_));
  nand2  g112(.a(x33), .b(x16), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n163_), .c(new_n61_), .d(new_n141_), .O(z14));
  inv1   g114(.a(x00), .O(new_n166_));
  nand2  g115(.a(new_n76_), .b(new_n166_), .O(new_n167_));
  aoi21  g116(.a(x34), .b(x16), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n167_), .c(new_n57_), .O(z15));
endmodule


