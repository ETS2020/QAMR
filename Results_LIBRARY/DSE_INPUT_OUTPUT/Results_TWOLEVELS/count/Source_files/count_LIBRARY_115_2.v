// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n212_, new_n213_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n229_, new_n230_, new_n231_, new_n232_, new_n233_, new_n234_,
    new_n235_, new_n236_, new_n237_, new_n238_, new_n239_, new_n240_,
    new_n241_, new_n242_;
  inv1   g000(.a(x13), .O(new_n52_));
  nor2   g001(.a(x20), .b(new_n52_), .O(new_n53_));
  nor2   g002(.a(x16), .b(x15), .O(new_n54_));
  inv1   g003(.a(x18), .O(new_n55_));
  nand3  g004(.a(x19), .b(x17), .c(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  inv1   g007(.a(x17), .O(new_n59_));
  inv1   g008(.a(x19), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(x13), .O(new_n62_));
  nand4  g011(.a(new_n62_), .b(new_n60_), .c(new_n59_), .d(x16), .O(new_n63_));
  oai21  g012(.a(new_n58_), .b(new_n53_), .c(new_n63_), .O(z00));
  inv1   g013(.a(x16), .O(new_n65_));
  nor2   g014(.a(x17), .b(new_n65_), .O(new_n66_));
  nand3  g015(.a(new_n66_), .b(new_n61_), .c(new_n60_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n55_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nor2   g018(.a(x19), .b(x17), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n65_), .c(new_n55_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x20), .O(new_n72_));
  nor2   g021(.a(x16), .b(x14), .O(new_n73_));
  nor2   g022(.a(new_n73_), .b(new_n53_), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n72_), .c(new_n69_), .O(z01));
  inv1   g024(.a(new_n53_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x18), .O(new_n77_));
  inv1   g026(.a(x21), .O(new_n78_));
  nor2   g027(.a(new_n70_), .b(new_n78_), .O(new_n79_));
  nand4  g028(.a(new_n78_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(x16), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n79_), .c(new_n52_), .O(new_n82_));
  nand3  g031(.a(x21), .b(x20), .c(x16), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n82_), .c(new_n77_), .O(z02));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n66_), .c(new_n60_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n52_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n61_), .O(new_n88_));
  inv1   g037(.a(x22), .O(new_n89_));
  inv1   g038(.a(new_n80_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g040(.a(x16), .b(x12), .O(new_n92_));
  aoi21  g041(.a(new_n91_), .b(x16), .c(new_n92_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n88_), .c(new_n77_), .O(z03));
  oai21  g043(.a(x16), .b(x11), .c(new_n55_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n76_), .O(new_n96_));
  nand2  g045(.a(new_n85_), .b(new_n70_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(x23), .O(new_n98_));
  nor3   g047(.a(x20), .b(x19), .c(x17), .O(new_n99_));
  nor3   g048(.a(x23), .b(x22), .c(x21), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n98_), .c(x13), .O(new_n102_));
  inv1   g051(.a(x23), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n61_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n102_), .c(x16), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n96_), .O(z04));
  nand2  g055(.a(new_n70_), .b(x16), .O(new_n107_));
  nor2   g056(.a(x24), .b(x23), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n85_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n107_), .c(new_n52_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n61_), .O(new_n111_));
  nand3  g060(.a(new_n101_), .b(x24), .c(x16), .O(new_n112_));
  inv1   g061(.a(x10), .O(new_n113_));
  nand2  g062(.a(new_n65_), .b(new_n113_), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n77_), .O(z05));
  nor2   g064(.a(x21), .b(x19), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n66_), .O(new_n117_));
  inv1   g066(.a(x24), .O(new_n118_));
  inv1   g067(.a(x25), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n118_), .c(new_n103_), .d(new_n89_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n117_), .c(new_n52_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n61_), .O(new_n122_));
  inv1   g071(.a(new_n109_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n99_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(x25), .c(x16), .O(new_n125_));
  inv1   g074(.a(x09), .O(new_n126_));
  nand2  g075(.a(new_n65_), .b(new_n126_), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n125_), .c(new_n122_), .d(new_n77_), .O(z06));
  nor2   g077(.a(x23), .b(x22), .O(new_n129_));
  nor3   g078(.a(x26), .b(x25), .c(x24), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n117_), .c(new_n52_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n61_), .O(new_n133_));
  inv1   g082(.a(x26), .O(new_n134_));
  nor2   g083(.a(new_n120_), .b(new_n80_), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  inv1   g086(.a(x08), .O(new_n138_));
  nand2  g087(.a(new_n65_), .b(new_n138_), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n137_), .c(new_n133_), .d(new_n77_), .O(z07));
  nor2   g089(.a(x27), .b(x26), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n108_), .c(new_n119_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n86_), .c(new_n52_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n61_), .O(new_n144_));
  nand3  g093(.a(new_n130_), .b(new_n129_), .c(new_n90_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(x27), .c(x16), .O(new_n146_));
  inv1   g095(.a(x07), .O(new_n147_));
  nand2  g096(.a(new_n65_), .b(new_n147_), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n146_), .c(new_n144_), .d(new_n77_), .O(z08));
  oai21  g098(.a(x16), .b(x06), .c(new_n55_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n76_), .O(new_n151_));
  oai21  g100(.a(new_n142_), .b(new_n97_), .c(x28), .O(new_n152_));
  nor3   g101(.a(x22), .b(x21), .c(x20), .O(new_n153_));
  nor3   g102(.a(x25), .b(x24), .c(x23), .O(new_n154_));
  nor3   g103(.a(x28), .b(x27), .c(x26), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n153_), .d(new_n70_), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n152_), .c(x13), .O(new_n157_));
  inv1   g106(.a(x28), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n61_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n157_), .c(x16), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n151_), .O(z09));
  oai21  g110(.a(x16), .b(x05), .c(new_n55_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n76_), .O(new_n163_));
  nand3  g112(.a(new_n129_), .b(new_n70_), .c(new_n78_), .O(new_n164_));
  nor2   g113(.a(x25), .b(x24), .O(new_n165_));
  nor2   g114(.a(x28), .b(x27), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n165_), .c(new_n134_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n164_), .c(x29), .O(new_n168_));
  nor3   g117(.a(x29), .b(x28), .c(x27), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n130_), .c(new_n100_), .d(new_n99_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n168_), .c(x13), .O(new_n171_));
  and2   g120(.a(x29), .b(x20), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n171_), .c(x16), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n163_), .O(z10));
  nand3  g123(.a(new_n100_), .b(new_n70_), .c(x16), .O(new_n175_));
  nor2   g124(.a(x30), .b(x29), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n166_), .c(new_n130_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n175_), .c(new_n52_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n61_), .O(new_n179_));
  nand3  g128(.a(new_n170_), .b(x30), .c(x16), .O(new_n180_));
  inv1   g129(.a(x04), .O(new_n181_));
  nand2  g130(.a(new_n65_), .b(new_n181_), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n180_), .c(new_n179_), .d(new_n77_), .O(z11));
  oai21  g132(.a(x16), .b(x03), .c(new_n55_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n76_), .O(new_n185_));
  nand4  g134(.a(new_n116_), .b(new_n108_), .c(new_n89_), .d(new_n59_), .O(new_n186_));
  nand4  g135(.a(new_n176_), .b(new_n141_), .c(new_n158_), .d(new_n119_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n186_), .c(x31), .O(new_n188_));
  nor3   g137(.a(x27), .b(x26), .c(x25), .O(new_n189_));
  nor2   g138(.a(x29), .b(x28), .O(new_n190_));
  nor2   g139(.a(x31), .b(x30), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n189_), .c(new_n123_), .d(new_n99_), .O(new_n194_));
  aoi21  g143(.a(new_n194_), .b(new_n188_), .c(x13), .O(new_n195_));
  inv1   g144(.a(x31), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n61_), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n195_), .c(x16), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n185_), .O(z12));
  oai21  g148(.a(x16), .b(x02), .c(new_n55_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n76_), .O(new_n201_));
  nand4  g150(.a(new_n191_), .b(new_n190_), .c(new_n141_), .d(new_n119_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n186_), .c(x32), .O(new_n203_));
  nand3  g152(.a(new_n166_), .b(new_n134_), .c(new_n119_), .O(new_n204_));
  inv1   g153(.a(new_n204_), .O(new_n205_));
  nor2   g154(.a(x32), .b(x31), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n176_), .O(new_n207_));
  inv1   g156(.a(new_n207_), .O(new_n208_));
  nand4  g157(.a(new_n208_), .b(new_n205_), .c(new_n123_), .d(new_n99_), .O(new_n209_));
  aoi21  g158(.a(new_n209_), .b(new_n203_), .c(x13), .O(new_n210_));
  inv1   g159(.a(x32), .O(new_n211_));
  nor2   g160(.a(new_n211_), .b(new_n61_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n210_), .c(x16), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(new_n201_), .O(z13));
  oai21  g163(.a(x16), .b(x01), .c(new_n55_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n76_), .O(new_n216_));
  nand4  g165(.a(new_n165_), .b(new_n116_), .c(new_n129_), .d(new_n59_), .O(new_n217_));
  nand4  g166(.a(new_n206_), .b(new_n176_), .c(new_n166_), .d(new_n134_), .O(new_n218_));
  oai21  g167(.a(new_n218_), .b(new_n217_), .c(x33), .O(new_n219_));
  nor2   g168(.a(x33), .b(x32), .O(new_n220_));
  nand4  g169(.a(new_n220_), .b(new_n191_), .c(new_n190_), .d(new_n141_), .O(new_n221_));
  inv1   g170(.a(new_n221_), .O(new_n222_));
  nand2  g171(.a(new_n222_), .b(new_n135_), .O(new_n223_));
  aoi21  g172(.a(new_n223_), .b(new_n219_), .c(x13), .O(new_n224_));
  inv1   g173(.a(x33), .O(new_n225_));
  nor2   g174(.a(new_n225_), .b(new_n61_), .O(new_n226_));
  oai21  g175(.a(new_n226_), .b(new_n224_), .c(x16), .O(new_n227_));
  nand2  g176(.a(new_n227_), .b(new_n216_), .O(z14));
  oai21  g177(.a(x16), .b(x00), .c(new_n55_), .O(new_n229_));
  nand2  g178(.a(new_n229_), .b(new_n76_), .O(new_n230_));
  oai21  g179(.a(new_n221_), .b(new_n217_), .c(x34), .O(new_n231_));
  nand2  g180(.a(new_n190_), .b(new_n141_), .O(new_n232_));
  inv1   g181(.a(new_n232_), .O(new_n233_));
  inv1   g182(.a(x30), .O(new_n234_));
  nand2  g183(.a(new_n196_), .b(new_n234_), .O(new_n235_));
  inv1   g184(.a(x34), .O(new_n236_));
  nand3  g185(.a(new_n236_), .b(new_n225_), .c(new_n211_), .O(new_n237_));
  nor2   g186(.a(new_n237_), .b(new_n235_), .O(new_n238_));
  nand3  g187(.a(new_n238_), .b(new_n233_), .c(new_n135_), .O(new_n239_));
  aoi21  g188(.a(new_n239_), .b(new_n231_), .c(x13), .O(new_n240_));
  nor2   g189(.a(new_n236_), .b(new_n61_), .O(new_n241_));
  oai21  g190(.a(new_n241_), .b(new_n240_), .c(x16), .O(new_n242_));
  nand2  g191(.a(new_n242_), .b(new_n230_), .O(z15));
endmodule


