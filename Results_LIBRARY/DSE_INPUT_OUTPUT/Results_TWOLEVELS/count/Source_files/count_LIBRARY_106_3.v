// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x14), .O(new_n61_));
  oai21  g010(.a(x18), .b(new_n61_), .c(new_n58_), .O(new_n62_));
  inv1   g011(.a(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  oai21  g015(.a(new_n55_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand3  g016(.a(new_n67_), .b(new_n63_), .c(x16), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n62_), .O(z01));
  inv1   g018(.a(x13), .O(new_n70_));
  oai21  g019(.a(x18), .b(new_n70_), .c(new_n58_), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n55_), .b(new_n72_), .c(new_n64_), .O(new_n73_));
  oai21  g022(.a(new_n65_), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n63_), .c(x16), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n71_), .O(z02));
  inv1   g025(.a(x12), .O(new_n77_));
  oai21  g026(.a(x18), .b(new_n77_), .c(new_n58_), .O(new_n78_));
  nand2  g027(.a(x22), .b(x21), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n64_), .c(new_n52_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n65_), .b(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n82_), .c(new_n63_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n58_), .c(new_n78_), .O(z03));
  oai21  g035(.a(new_n80_), .b(x19), .c(new_n65_), .O(new_n87_));
  nor3   g036(.a(x23), .b(x22), .c(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n65_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  aoi21  g039(.a(new_n87_), .b(x23), .c(new_n90_), .O(new_n91_));
  inv1   g040(.a(x11), .O(new_n92_));
  aoi21  g041(.a(new_n58_), .b(new_n92_), .c(x18), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n58_), .c(new_n93_), .O(z04));
  inv1   g043(.a(x10), .O(new_n95_));
  oai21  g044(.a(x18), .b(new_n95_), .c(new_n58_), .O(new_n96_));
  inv1   g045(.a(x24), .O(new_n97_));
  inv1   g046(.a(new_n88_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n53_), .c(new_n66_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n80_), .c(new_n65_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n63_), .c(x16), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n96_), .O(z05));
  inv1   g053(.a(x25), .O(new_n105_));
  nand2  g054(.a(new_n100_), .b(new_n80_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n53_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n65_), .c(new_n105_), .O(new_n108_));
  nor2   g057(.a(x23), .b(x22), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n73_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n108_), .c(x16), .O(new_n113_));
  inv1   g062(.a(x09), .O(new_n114_));
  aoi21  g063(.a(new_n58_), .b(new_n114_), .c(x18), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n113_), .O(z06));
  inv1   g065(.a(x08), .O(new_n117_));
  oai21  g066(.a(x18), .b(new_n117_), .c(new_n58_), .O(new_n118_));
  inv1   g067(.a(x26), .O(new_n119_));
  inv1   g068(.a(x23), .O(new_n120_));
  nand3  g069(.a(new_n110_), .b(new_n80_), .c(new_n120_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n53_), .c(new_n66_), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n109_), .O(new_n124_));
  oai22  g073(.a(new_n124_), .b(new_n73_), .c(new_n122_), .d(new_n119_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n63_), .c(x16), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n118_), .O(z07));
  inv1   g076(.a(x27), .O(new_n128_));
  nor2   g077(.a(x26), .b(x25), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n109_), .c(new_n97_), .d(new_n72_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n53_), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n65_), .c(new_n128_), .O(new_n132_));
  inv1   g081(.a(new_n55_), .O(new_n133_));
  nand2  g082(.a(new_n80_), .b(new_n64_), .O(new_n134_));
  nor2   g083(.a(x27), .b(x26), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n100_), .c(new_n105_), .O(new_n136_));
  nor3   g085(.a(new_n136_), .b(new_n134_), .c(new_n133_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n132_), .c(x16), .O(new_n138_));
  inv1   g087(.a(x07), .O(new_n139_));
  aoi21  g088(.a(new_n58_), .b(new_n139_), .c(x18), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(z08));
  inv1   g090(.a(x06), .O(new_n142_));
  oai21  g091(.a(x18), .b(new_n142_), .c(new_n58_), .O(new_n143_));
  inv1   g092(.a(x28), .O(new_n144_));
  nand3  g093(.a(new_n135_), .b(new_n110_), .c(new_n88_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n53_), .c(new_n66_), .O(new_n146_));
  nor2   g095(.a(new_n134_), .b(new_n133_), .O(new_n147_));
  nor3   g096(.a(x28), .b(x27), .c(x26), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n147_), .c(new_n110_), .d(new_n120_), .O(new_n149_));
  oai21  g098(.a(new_n146_), .b(new_n144_), .c(new_n149_), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n63_), .c(x16), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n143_), .O(z09));
  inv1   g101(.a(x05), .O(new_n153_));
  oai21  g102(.a(x18), .b(new_n153_), .c(new_n58_), .O(new_n154_));
  inv1   g103(.a(x29), .O(new_n155_));
  nor2   g104(.a(x28), .b(x27), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n129_), .c(new_n100_), .d(new_n80_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n53_), .c(new_n66_), .O(new_n158_));
  nor3   g107(.a(x29), .b(x28), .c(x27), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n123_), .O(new_n160_));
  oai22  g109(.a(new_n160_), .b(new_n89_), .c(new_n158_), .d(new_n155_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n63_), .c(x16), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n154_), .O(z10));
  inv1   g112(.a(x04), .O(new_n164_));
  oai21  g113(.a(x18), .b(new_n164_), .c(new_n58_), .O(new_n165_));
  inv1   g114(.a(x30), .O(new_n166_));
  nor2   g115(.a(x29), .b(x28), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n129_), .c(new_n128_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n106_), .c(new_n53_), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n65_), .c(new_n166_), .O(new_n170_));
  nor2   g119(.a(x30), .b(x29), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n156_), .c(new_n123_), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n89_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n170_), .c(new_n63_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n58_), .c(new_n165_), .O(z11));
  inv1   g124(.a(x31), .O(new_n176_));
  nand3  g125(.a(new_n171_), .b(new_n135_), .c(new_n144_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n121_), .c(new_n53_), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n65_), .c(new_n176_), .O(new_n179_));
  nor2   g128(.a(x31), .b(x30), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n167_), .c(new_n135_), .d(new_n105_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n101_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n179_), .c(x16), .O(new_n183_));
  inv1   g132(.a(x03), .O(new_n184_));
  aoi21  g133(.a(new_n58_), .b(new_n184_), .c(x18), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n183_), .O(z12));
  inv1   g135(.a(x32), .O(new_n187_));
  nand4  g136(.a(new_n180_), .b(new_n156_), .c(new_n155_), .d(new_n119_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n121_), .c(new_n53_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n65_), .c(new_n187_), .O(new_n190_));
  nor2   g139(.a(x32), .b(x31), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n171_), .c(new_n156_), .d(new_n129_), .O(new_n192_));
  nor2   g141(.a(new_n192_), .b(new_n101_), .O(new_n193_));
  oai21  g142(.a(new_n193_), .b(new_n190_), .c(x16), .O(new_n194_));
  inv1   g143(.a(x02), .O(new_n195_));
  aoi21  g144(.a(new_n58_), .b(new_n195_), .c(x18), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n194_), .O(z13));
  inv1   g146(.a(x01), .O(new_n198_));
  oai21  g147(.a(x18), .b(new_n198_), .c(new_n58_), .O(new_n199_));
  inv1   g148(.a(x33), .O(new_n200_));
  nor3   g149(.a(x32), .b(x31), .c(x30), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n159_), .c(new_n123_), .d(new_n88_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n53_), .c(new_n66_), .O(new_n203_));
  nand2  g152(.a(new_n167_), .b(new_n135_), .O(new_n204_));
  inv1   g153(.a(new_n204_), .O(new_n205_));
  nor2   g154(.a(x33), .b(x32), .O(new_n206_));
  nand4  g155(.a(new_n206_), .b(new_n205_), .c(new_n180_), .d(new_n112_), .O(new_n207_));
  oai21  g156(.a(new_n203_), .b(new_n200_), .c(new_n207_), .O(new_n208_));
  nand3  g157(.a(new_n208_), .b(new_n63_), .c(x16), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n199_), .O(z14));
  inv1   g159(.a(x34), .O(new_n211_));
  nand4  g160(.a(new_n206_), .b(new_n180_), .c(new_n167_), .d(new_n128_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n130_), .c(new_n53_), .O(new_n213_));
  aoi21  g162(.a(new_n213_), .b(new_n65_), .c(new_n211_), .O(new_n214_));
  nand4  g163(.a(new_n180_), .b(new_n211_), .c(new_n200_), .d(new_n187_), .O(new_n215_));
  nor2   g164(.a(new_n215_), .b(new_n204_), .O(new_n216_));
  and2   g165(.a(new_n216_), .b(new_n112_), .O(new_n217_));
  oai21  g166(.a(new_n217_), .b(new_n214_), .c(x16), .O(new_n218_));
  inv1   g167(.a(x00), .O(new_n219_));
  aoi21  g168(.a(new_n58_), .b(new_n219_), .c(x18), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n218_), .O(z15));
endmodule


