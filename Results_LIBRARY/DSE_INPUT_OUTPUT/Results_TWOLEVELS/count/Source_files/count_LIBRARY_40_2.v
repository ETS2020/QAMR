// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nand3  g011(.a(new_n61_), .b(new_n53_), .c(new_n52_), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n62_), .c(x16), .O(new_n65_));
  inv1   g014(.a(x14), .O(new_n66_));
  aoi21  g015(.a(new_n58_), .b(new_n66_), .c(x18), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n65_), .O(z01));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n55_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  aoi21  g020(.a(new_n63_), .b(x21), .c(new_n71_), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n58_), .c(new_n74_), .O(z02));
  nand2  g024(.a(x22), .b(x21), .O(new_n76_));
  nor2   g025(.a(x22), .b(x21), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n61_), .c(new_n52_), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  and2   g028(.a(new_n63_), .b(x22), .O(new_n80_));
  oai21  g029(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  inv1   g030(.a(x12), .O(new_n82_));
  aoi21  g031(.a(new_n58_), .b(new_n82_), .c(x18), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n81_), .O(z03));
  inv1   g033(.a(x18), .O(new_n85_));
  nor2   g034(.a(new_n77_), .b(x19), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n63_), .c(x23), .O(new_n87_));
  nor3   g036(.a(x23), .b(x22), .c(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n64_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n58_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n90_), .c(new_n85_), .O(new_n92_));
  nand2  g041(.a(x30), .b(x18), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(z04));
  nor2   g043(.a(new_n88_), .b(x19), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n63_), .c(x24), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n77_), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n63_), .c(new_n96_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x10), .O(new_n100_));
  aoi21  g049(.a(new_n99_), .b(x16), .c(new_n100_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(x18), .c(new_n93_), .O(z05));
  inv1   g051(.a(x25), .O(new_n103_));
  aoi21  g052(.a(new_n98_), .b(new_n53_), .c(new_n63_), .O(new_n104_));
  nor2   g053(.a(x23), .b(x22), .O(new_n105_));
  nor2   g054(.a(x25), .b(x24), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n71_), .O(new_n109_));
  oai21  g058(.a(new_n104_), .b(new_n103_), .c(new_n109_), .O(new_n110_));
  nor2   g059(.a(x16), .b(x09), .O(new_n111_));
  aoi21  g060(.a(new_n110_), .b(x16), .c(new_n111_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(x18), .c(new_n93_), .O(z06));
  inv1   g062(.a(x26), .O(new_n114_));
  nor3   g063(.a(x25), .b(x24), .c(x23), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n77_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n53_), .c(new_n63_), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n105_), .c(new_n71_), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n114_), .c(new_n119_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x08), .O(new_n121_));
  aoi21  g070(.a(new_n120_), .b(x16), .c(new_n121_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(x18), .c(new_n93_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  nand2  g073(.a(new_n118_), .b(new_n88_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n53_), .c(new_n63_), .O(new_n126_));
  nand3  g075(.a(new_n77_), .b(new_n55_), .c(new_n61_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n97_), .O(new_n129_));
  oai22  g078(.a(new_n129_), .b(new_n127_), .c(new_n126_), .d(new_n124_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x07), .O(new_n131_));
  aoi21  g080(.a(new_n130_), .b(x16), .c(new_n131_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(x18), .c(new_n93_), .O(z08));
  inv1   g082(.a(x28), .O(new_n134_));
  nor2   g083(.a(x27), .b(x26), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n106_), .c(new_n88_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n53_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n64_), .c(new_n134_), .O(new_n138_));
  nor3   g087(.a(x28), .b(x27), .c(x26), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n115_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n127_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n138_), .c(x16), .O(new_n142_));
  inv1   g091(.a(x06), .O(new_n143_));
  aoi21  g092(.a(new_n58_), .b(new_n143_), .c(x18), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n142_), .O(z09));
  inv1   g094(.a(x29), .O(new_n146_));
  inv1   g095(.a(new_n98_), .O(new_n147_));
  nor2   g096(.a(x26), .b(x25), .O(new_n148_));
  nor2   g097(.a(x28), .b(x27), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n53_), .c(new_n63_), .O(new_n153_));
  nor3   g102(.a(x29), .b(x28), .c(x27), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n118_), .O(new_n155_));
  oai22  g104(.a(new_n155_), .b(new_n89_), .c(new_n153_), .d(new_n146_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x16), .O(new_n157_));
  inv1   g106(.a(x05), .O(new_n158_));
  aoi21  g107(.a(new_n58_), .b(new_n158_), .c(x18), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n157_), .O(z10));
  nand4  g109(.a(new_n105_), .b(new_n69_), .c(new_n55_), .d(x16), .O(new_n161_));
  nor2   g110(.a(x30), .b(x29), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n149_), .c(new_n118_), .O(new_n163_));
  oai22  g112(.a(new_n163_), .b(new_n161_), .c(x16), .d(x04), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n85_), .O(new_n165_));
  nand4  g114(.a(new_n154_), .b(new_n148_), .c(new_n97_), .d(new_n77_), .O(new_n166_));
  aoi21  g115(.a(new_n166_), .b(new_n53_), .c(new_n63_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n58_), .c(new_n85_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(x30), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n165_), .O(z11));
  inv1   g119(.a(x31), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n58_), .c(new_n85_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x30), .O(new_n173_));
  nor2   g122(.a(new_n98_), .b(new_n63_), .O(new_n174_));
  nor2   g123(.a(x29), .b(x28), .O(new_n175_));
  nor2   g124(.a(x31), .b(x30), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n128_), .d(new_n174_), .O(new_n177_));
  oai21  g126(.a(new_n167_), .b(new_n171_), .c(new_n177_), .O(new_n178_));
  nor2   g127(.a(x16), .b(x03), .O(new_n179_));
  aoi21  g128(.a(new_n178_), .b(x16), .c(new_n179_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(x18), .c(new_n173_), .O(z12));
  inv1   g130(.a(x32), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n58_), .c(new_n85_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(x30), .O(new_n184_));
  nor3   g133(.a(x31), .b(x29), .c(x28), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n135_), .c(new_n115_), .d(new_n77_), .O(new_n186_));
  aoi21  g135(.a(new_n186_), .b(new_n53_), .c(new_n63_), .O(new_n187_));
  nor2   g136(.a(x32), .b(x31), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n162_), .c(new_n151_), .d(new_n174_), .O(new_n189_));
  oai21  g138(.a(new_n187_), .b(new_n182_), .c(new_n189_), .O(new_n190_));
  nor2   g139(.a(x16), .b(x02), .O(new_n191_));
  aoi21  g140(.a(new_n190_), .b(x16), .c(new_n191_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(x18), .c(new_n184_), .O(z13));
  inv1   g142(.a(x33), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n58_), .c(new_n85_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(x30), .O(new_n196_));
  nor3   g145(.a(x32), .b(x31), .c(x29), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n139_), .c(new_n115_), .d(new_n77_), .O(new_n198_));
  aoi21  g147(.a(new_n198_), .b(new_n53_), .c(new_n63_), .O(new_n199_));
  nand2  g148(.a(new_n175_), .b(new_n135_), .O(new_n200_));
  inv1   g149(.a(new_n200_), .O(new_n201_));
  nand3  g150(.a(new_n176_), .b(new_n194_), .c(new_n182_), .O(new_n202_));
  inv1   g151(.a(new_n202_), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n201_), .c(new_n108_), .d(new_n71_), .O(new_n204_));
  oai21  g153(.a(new_n199_), .b(new_n194_), .c(new_n204_), .O(new_n205_));
  nor2   g154(.a(x16), .b(x01), .O(new_n206_));
  aoi21  g155(.a(new_n205_), .b(x16), .c(new_n206_), .O(new_n207_));
  oai21  g156(.a(new_n207_), .b(x18), .c(new_n196_), .O(z14));
  inv1   g157(.a(x34), .O(new_n209_));
  nand2  g158(.a(new_n203_), .b(new_n154_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n125_), .c(new_n53_), .O(new_n211_));
  aoi21  g160(.a(new_n211_), .b(new_n64_), .c(new_n209_), .O(new_n212_));
  nand4  g161(.a(new_n176_), .b(new_n209_), .c(new_n194_), .d(new_n182_), .O(new_n213_));
  nor3   g162(.a(new_n213_), .b(new_n200_), .c(new_n109_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n212_), .c(x16), .O(new_n215_));
  inv1   g164(.a(x00), .O(new_n216_));
  aoi21  g165(.a(new_n58_), .b(new_n216_), .c(x18), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n215_), .O(z15));
endmodule


