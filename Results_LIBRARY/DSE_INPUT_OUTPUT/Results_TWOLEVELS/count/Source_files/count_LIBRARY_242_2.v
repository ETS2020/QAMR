// Benchmark "FAU" written by ABC on Wed Aug 19 19:40:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_;
  inv1   g000(.a(x07), .O(new_n52_));
  nor2   g001(.a(x15), .b(new_n52_), .O(new_n53_));
  xnor2a g002(.a(x19), .b(x17), .O(new_n54_));
  aoi21  g003(.a(new_n54_), .b(x16), .c(x18), .O(new_n55_));
  inv1   g004(.a(x15), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  nand3  g006(.a(new_n57_), .b(new_n56_), .c(new_n52_), .O(new_n58_));
  oai21  g007(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(x19), .b(x17), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n57_), .b(new_n65_), .c(x18), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n53_), .O(z01));
  oai21  g016(.a(x15), .b(new_n52_), .c(x18), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n61_), .O(new_n71_));
  oai21  g020(.a(new_n63_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  nor2   g023(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  nand3  g024(.a(new_n75_), .b(new_n73_), .c(new_n68_), .O(z02));
  inv1   g025(.a(x22), .O(new_n77_));
  inv1   g026(.a(new_n71_), .O(new_n78_));
  nand4  g027(.a(new_n61_), .b(new_n77_), .c(new_n69_), .d(new_n60_), .O(new_n79_));
  oai21  g028(.a(new_n78_), .b(new_n77_), .c(new_n79_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(x16), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n53_), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n81_), .c(new_n68_), .O(z03));
  nand2  g033(.a(new_n79_), .b(x23), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n63_), .c(new_n69_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x16), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n53_), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n89_), .c(new_n68_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  inv1   g042(.a(x19), .O(new_n94_));
  nand2  g043(.a(new_n86_), .b(new_n69_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n63_), .c(new_n93_), .O(new_n97_));
  inv1   g046(.a(x17), .O(new_n98_));
  nand3  g047(.a(new_n60_), .b(new_n94_), .c(new_n98_), .O(new_n99_));
  inv1   g048(.a(x23), .O(new_n100_));
  nand4  g049(.a(new_n93_), .b(new_n100_), .c(new_n77_), .d(new_n69_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n97_), .c(x16), .O(new_n103_));
  inv1   g052(.a(x10), .O(new_n104_));
  aoi21  g053(.a(new_n57_), .b(new_n104_), .c(x18), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n103_), .c(new_n53_), .O(z05));
  inv1   g055(.a(x25), .O(new_n107_));
  nand2  g056(.a(new_n101_), .b(new_n94_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n63_), .c(new_n107_), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n86_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n71_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n109_), .c(x16), .O(new_n113_));
  inv1   g062(.a(x09), .O(new_n114_));
  aoi21  g063(.a(new_n57_), .b(new_n114_), .c(x18), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n113_), .c(new_n53_), .O(z06));
  inv1   g065(.a(x26), .O(new_n117_));
  nor2   g066(.a(new_n112_), .b(new_n117_), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n86_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n71_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n118_), .c(x16), .O(new_n122_));
  nor2   g071(.a(x16), .b(x08), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n53_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(new_n68_), .O(z07));
  oai21  g074(.a(x18), .b(new_n57_), .c(new_n52_), .O(new_n126_));
  inv1   g075(.a(x27), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n93_), .c(new_n100_), .O(new_n129_));
  oai22  g078(.a(new_n129_), .b(new_n79_), .c(new_n121_), .d(new_n127_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  aoi21  g080(.a(x18), .b(x15), .c(new_n53_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n131_), .c(new_n126_), .O(z08));
  inv1   g082(.a(x28), .O(new_n134_));
  nor2   g083(.a(x27), .b(x26), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n110_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n95_), .c(new_n94_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n63_), .c(new_n134_), .O(new_n138_));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n110_), .c(new_n117_), .d(new_n100_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n79_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n138_), .c(x16), .O(new_n142_));
  inv1   g091(.a(x06), .O(new_n143_));
  aoi21  g092(.a(new_n57_), .b(new_n143_), .c(x18), .O(new_n144_));
  aoi21  g093(.a(new_n144_), .b(new_n142_), .c(new_n53_), .O(z09));
  inv1   g094(.a(x29), .O(new_n146_));
  nor2   g095(.a(new_n95_), .b(new_n99_), .O(new_n147_));
  nor3   g096(.a(x29), .b(x28), .c(x27), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n119_), .c(new_n147_), .O(new_n149_));
  oai21  g098(.a(new_n141_), .b(new_n146_), .c(new_n149_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  nor2   g100(.a(x16), .b(x05), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n53_), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n151_), .c(new_n68_), .O(z10));
  nand2  g103(.a(new_n149_), .b(x30), .O(new_n155_));
  nor2   g104(.a(x30), .b(x29), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n139_), .c(new_n119_), .O(new_n157_));
  or2    g106(.a(new_n157_), .b(new_n87_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  nor2   g109(.a(x16), .b(x04), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n53_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n160_), .c(new_n68_), .O(z11));
  oai21  g112(.a(new_n157_), .b(new_n87_), .c(x31), .O(new_n164_));
  nor2   g113(.a(x29), .b(x28), .O(new_n165_));
  nor2   g114(.a(x31), .b(x30), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n165_), .c(new_n128_), .d(new_n102_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(x16), .O(new_n169_));
  nor2   g118(.a(x16), .b(x03), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n53_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n169_), .c(new_n68_), .O(z12));
  nand2  g121(.a(new_n167_), .b(x32), .O(new_n173_));
  nor2   g122(.a(x26), .b(x25), .O(new_n174_));
  nor2   g123(.a(x32), .b(x31), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n156_), .O(new_n176_));
  inv1   g125(.a(new_n176_), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n139_), .c(new_n174_), .d(new_n102_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n173_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(x16), .O(new_n180_));
  nor2   g129(.a(x16), .b(x02), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n53_), .O(new_n182_));
  nand3  g131(.a(new_n182_), .b(new_n180_), .c(new_n68_), .O(z13));
  nand2  g132(.a(new_n178_), .b(x33), .O(new_n184_));
  nand2  g133(.a(new_n165_), .b(new_n135_), .O(new_n185_));
  inv1   g134(.a(new_n185_), .O(new_n186_));
  nor2   g135(.a(x33), .b(x32), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n186_), .c(new_n166_), .d(new_n112_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n184_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(x16), .O(new_n190_));
  nor2   g139(.a(x16), .b(x01), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n53_), .O(new_n192_));
  nand3  g141(.a(new_n192_), .b(new_n190_), .c(new_n68_), .O(z14));
  inv1   g142(.a(x34), .O(new_n194_));
  nand4  g143(.a(new_n174_), .b(new_n86_), .c(new_n93_), .d(new_n69_), .O(new_n195_));
  nand4  g144(.a(new_n187_), .b(new_n166_), .c(new_n165_), .d(new_n127_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n195_), .c(new_n94_), .O(new_n197_));
  aoi21  g146(.a(new_n197_), .b(new_n63_), .c(new_n194_), .O(new_n198_));
  inv1   g147(.a(new_n111_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n78_), .O(new_n200_));
  nor3   g149(.a(x34), .b(x33), .c(x32), .O(new_n201_));
  nand3  g150(.a(new_n201_), .b(new_n186_), .c(new_n166_), .O(new_n202_));
  nor2   g151(.a(new_n202_), .b(new_n200_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n198_), .c(x16), .O(new_n204_));
  inv1   g153(.a(x00), .O(new_n205_));
  aoi21  g154(.a(new_n57_), .b(new_n205_), .c(x18), .O(new_n206_));
  aoi21  g155(.a(new_n206_), .b(new_n204_), .c(new_n53_), .O(z15));
endmodule


