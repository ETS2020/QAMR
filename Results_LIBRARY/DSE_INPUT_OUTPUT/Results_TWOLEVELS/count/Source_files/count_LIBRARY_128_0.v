// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_;
  nor2   g000(.a(x14), .b(x11), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(new_n52_), .O(new_n62_));
  oai21  g011(.a(x19), .b(x17), .c(x20), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n54_), .c(new_n53_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n59_), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(x18), .c(new_n62_), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  nand3  g017(.a(new_n59_), .b(new_n68_), .c(x11), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  nand2  g019(.a(new_n62_), .b(x18), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nor3   g021(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n56_), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n72_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  nor2   g026(.a(x16), .b(x13), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n52_), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n77_), .c(new_n71_), .O(z02));
  nand2  g029(.a(x22), .b(x21), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n64_), .c(new_n53_), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n81_), .c(x19), .O(new_n84_));
  inv1   g033(.a(x22), .O(new_n85_));
  nor2   g034(.a(new_n73_), .b(new_n85_), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n84_), .c(x16), .O(new_n87_));
  inv1   g036(.a(x12), .O(new_n88_));
  aoi21  g037(.a(new_n59_), .b(new_n88_), .c(x18), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n52_), .O(z03));
  nand2  g039(.a(new_n65_), .b(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n73_), .c(new_n72_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(new_n59_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(x18), .c(new_n62_), .O(new_n95_));
  inv1   g044(.a(new_n82_), .O(new_n96_));
  inv1   g045(.a(x11), .O(new_n97_));
  oai21  g046(.a(x19), .b(new_n97_), .c(new_n68_), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n96_), .c(x23), .d(x16), .O(new_n99_));
  nand3  g048(.a(new_n59_), .b(x14), .c(new_n97_), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n99_), .c(new_n95_), .O(z04));
  inv1   g050(.a(x24), .O(new_n102_));
  nand2  g051(.a(new_n92_), .b(new_n72_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n54_), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n73_), .c(new_n102_), .O(new_n105_));
  nor2   g054(.a(x24), .b(x23), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n82_), .c(new_n73_), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n105_), .c(x16), .O(new_n109_));
  inv1   g058(.a(x10), .O(new_n110_));
  aoi21  g059(.a(new_n59_), .b(new_n110_), .c(x18), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n109_), .c(new_n52_), .O(z05));
  inv1   g061(.a(x25), .O(new_n113_));
  nand2  g062(.a(new_n106_), .b(new_n82_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n54_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n73_), .c(new_n113_), .O(new_n116_));
  nor2   g065(.a(x25), .b(x24), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n92_), .c(new_n74_), .d(new_n56_), .O(new_n118_));
  inv1   g067(.a(new_n118_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  inv1   g069(.a(x09), .O(new_n121_));
  aoi21  g070(.a(new_n59_), .b(new_n121_), .c(x18), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n120_), .c(new_n52_), .O(z06));
  inv1   g072(.a(x26), .O(new_n124_));
  nor2   g073(.a(new_n119_), .b(new_n124_), .O(new_n125_));
  nor3   g074(.a(x26), .b(x25), .c(x24), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n92_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n75_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n125_), .c(x16), .O(new_n129_));
  nor2   g078(.a(x16), .b(x08), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n52_), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n129_), .c(new_n71_), .O(z07));
  inv1   g081(.a(x27), .O(new_n133_));
  nand3  g082(.a(new_n82_), .b(new_n56_), .c(new_n64_), .O(new_n134_));
  nor3   g083(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n106_), .O(new_n136_));
  oai22  g085(.a(new_n136_), .b(new_n134_), .c(new_n128_), .d(new_n133_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nor2   g087(.a(x16), .b(x07), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n52_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n138_), .c(new_n71_), .O(z08));
  inv1   g090(.a(x28), .O(new_n142_));
  nor2   g091(.a(x27), .b(x26), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n117_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n103_), .c(new_n54_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n73_), .c(new_n142_), .O(new_n146_));
  inv1   g095(.a(x23), .O(new_n147_));
  nor2   g096(.a(x28), .b(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n117_), .c(new_n124_), .d(new_n147_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n134_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n146_), .c(x16), .O(new_n151_));
  inv1   g100(.a(x06), .O(new_n152_));
  aoi21  g101(.a(new_n59_), .b(new_n152_), .c(x18), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n151_), .c(new_n52_), .O(z09));
  inv1   g103(.a(x29), .O(new_n155_));
  nor2   g104(.a(new_n103_), .b(new_n65_), .O(new_n156_));
  nor3   g105(.a(x29), .b(x28), .c(x27), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n126_), .c(new_n156_), .O(new_n158_));
  oai21  g107(.a(new_n150_), .b(new_n155_), .c(new_n158_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  nor2   g109(.a(x16), .b(x05), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n52_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n160_), .c(new_n71_), .O(z10));
  nand2  g112(.a(new_n158_), .b(x30), .O(new_n164_));
  nor2   g113(.a(x30), .b(x29), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n148_), .c(new_n126_), .O(new_n166_));
  or2    g115(.a(new_n166_), .b(new_n93_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(x16), .O(new_n169_));
  nor2   g118(.a(x16), .b(x04), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n52_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n169_), .c(new_n71_), .O(z11));
  oai21  g121(.a(new_n166_), .b(new_n93_), .c(x31), .O(new_n173_));
  nor2   g122(.a(x29), .b(x28), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n174_), .c(new_n135_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n107_), .c(new_n173_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(x16), .O(new_n178_));
  nor2   g127(.a(x16), .b(x03), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n52_), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n178_), .c(new_n71_), .O(z12));
  oai21  g130(.a(new_n176_), .b(new_n107_), .c(x32), .O(new_n182_));
  nor2   g131(.a(x26), .b(x25), .O(new_n183_));
  nor2   g132(.a(x32), .b(x31), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n165_), .O(new_n185_));
  inv1   g134(.a(new_n185_), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n148_), .c(new_n183_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n107_), .c(new_n182_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(x16), .O(new_n189_));
  nor2   g138(.a(x16), .b(x02), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n52_), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(new_n189_), .c(new_n71_), .O(z13));
  inv1   g141(.a(x33), .O(new_n193_));
  nand4  g142(.a(new_n183_), .b(new_n92_), .c(new_n102_), .d(new_n72_), .O(new_n194_));
  inv1   g143(.a(x30), .O(new_n195_));
  nand4  g144(.a(new_n184_), .b(new_n174_), .c(new_n195_), .d(new_n133_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n194_), .c(new_n54_), .O(new_n197_));
  aoi21  g146(.a(new_n197_), .b(new_n73_), .c(new_n193_), .O(new_n198_));
  nand2  g147(.a(new_n174_), .b(new_n143_), .O(new_n199_));
  nor2   g148(.a(x33), .b(x32), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n175_), .O(new_n201_));
  nor3   g150(.a(new_n201_), .b(new_n199_), .c(new_n118_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n198_), .c(x16), .O(new_n203_));
  inv1   g152(.a(x01), .O(new_n204_));
  aoi21  g153(.a(new_n59_), .b(new_n204_), .c(x18), .O(new_n205_));
  aoi21  g154(.a(new_n205_), .b(new_n203_), .c(new_n52_), .O(z14));
  inv1   g155(.a(x34), .O(new_n207_));
  nand4  g156(.a(new_n200_), .b(new_n175_), .c(new_n174_), .d(new_n133_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n194_), .c(new_n54_), .O(new_n209_));
  aoi21  g158(.a(new_n209_), .b(new_n73_), .c(new_n207_), .O(new_n210_));
  inv1   g159(.a(x32), .O(new_n211_));
  nand4  g160(.a(new_n175_), .b(new_n207_), .c(new_n193_), .d(new_n211_), .O(new_n212_));
  nor3   g161(.a(new_n212_), .b(new_n199_), .c(new_n118_), .O(new_n213_));
  oai21  g162(.a(new_n213_), .b(new_n210_), .c(x16), .O(new_n214_));
  inv1   g163(.a(x00), .O(new_n215_));
  aoi21  g164(.a(new_n59_), .b(new_n215_), .c(x18), .O(new_n216_));
  aoi21  g165(.a(new_n216_), .b(new_n214_), .c(new_n52_), .O(z15));
endmodule


