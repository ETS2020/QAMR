// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_;
  nor2   g000(.a(x14), .b(x12), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(new_n52_), .O(new_n62_));
  inv1   g011(.a(new_n56_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x20), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n54_), .c(new_n53_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n59_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(x18), .c(new_n62_), .O(new_n68_));
  inv1   g017(.a(x14), .O(new_n69_));
  nand3  g018(.a(new_n59_), .b(new_n69_), .c(x12), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  nand2  g020(.a(new_n62_), .b(x18), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  nor3   g022(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  nor2   g023(.a(x21), .b(x20), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n56_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n73_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(x16), .O(new_n78_));
  nor2   g027(.a(x16), .b(x13), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n52_), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n78_), .c(new_n72_), .O(z02));
  inv1   g030(.a(x12), .O(new_n82_));
  oai21  g031(.a(new_n59_), .b(new_n69_), .c(new_n82_), .O(new_n83_));
  inv1   g032(.a(x22), .O(new_n84_));
  inv1   g033(.a(new_n76_), .O(new_n85_));
  nor2   g034(.a(x22), .b(x21), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n56_), .c(new_n65_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n84_), .c(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x16), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n83_), .c(new_n72_), .O(z03));
  inv1   g039(.a(x23), .O(new_n91_));
  inv1   g040(.a(new_n87_), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n74_), .c(new_n73_), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n91_), .c(new_n94_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  nor2   g045(.a(x16), .b(x11), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n52_), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n96_), .c(new_n72_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  nand2  g049(.a(new_n93_), .b(new_n73_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n54_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n74_), .c(new_n100_), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n86_), .c(new_n74_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n59_), .b(new_n108_), .c(x18), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n107_), .c(new_n52_), .O(z05));
  inv1   g059(.a(x25), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n93_), .c(new_n75_), .d(new_n56_), .O(new_n113_));
  oai21  g062(.a(new_n106_), .b(new_n111_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  nor2   g064(.a(x16), .b(x09), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n52_), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n115_), .c(new_n72_), .O(z06));
  inv1   g067(.a(x26), .O(new_n119_));
  nand3  g068(.a(new_n86_), .b(new_n111_), .c(new_n91_), .O(new_n120_));
  nand3  g069(.a(new_n56_), .b(new_n100_), .c(new_n65_), .O(new_n121_));
  aoi21  g070(.a(new_n120_), .b(new_n54_), .c(new_n121_), .O(new_n122_));
  nor3   g071(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n93_), .c(new_n85_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g075(.a(x08), .O(new_n127_));
  aoi21  g076(.a(new_n59_), .b(new_n127_), .c(x18), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n126_), .c(new_n52_), .O(z07));
  nand2  g078(.a(new_n124_), .b(x27), .O(new_n130_));
  nor3   g079(.a(x27), .b(x26), .c(x25), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n104_), .c(new_n92_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  nor2   g083(.a(x16), .b(x07), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n52_), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n134_), .c(new_n72_), .O(z08));
  nand2  g086(.a(new_n131_), .b(new_n104_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n87_), .c(x28), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n112_), .c(new_n119_), .d(new_n91_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n87_), .c(new_n139_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x16), .O(new_n143_));
  nor2   g092(.a(x16), .b(x06), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n52_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(new_n72_), .O(z09));
  oai21  g095(.a(new_n141_), .b(new_n87_), .c(x29), .O(new_n147_));
  inv1   g096(.a(x27), .O(new_n148_));
  nor2   g097(.a(x29), .b(x28), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n123_), .c(new_n148_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n94_), .c(new_n147_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  nor2   g101(.a(x16), .b(x05), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n52_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(new_n72_), .O(z10));
  inv1   g104(.a(x30), .O(new_n156_));
  nor2   g105(.a(x27), .b(x25), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n149_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n101_), .c(new_n54_), .O(new_n159_));
  nand3  g108(.a(new_n119_), .b(new_n100_), .c(new_n65_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n63_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n159_), .c(new_n156_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n140_), .c(new_n123_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n94_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n162_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n59_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n52_), .O(z11));
  oai21  g118(.a(new_n164_), .b(new_n94_), .c(x31), .O(new_n170_));
  nor2   g119(.a(x31), .b(x30), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n149_), .c(new_n131_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n105_), .c(new_n170_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  nor2   g123(.a(x16), .b(x03), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n52_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n174_), .c(new_n72_), .O(z12));
  oai21  g126(.a(new_n172_), .b(new_n105_), .c(x32), .O(new_n178_));
  nor2   g127(.a(x26), .b(x25), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n163_), .c(new_n140_), .d(new_n179_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n105_), .c(new_n178_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x02), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n52_), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n183_), .c(new_n72_), .O(z13));
  oai21  g135(.a(new_n181_), .b(new_n105_), .c(x33), .O(new_n187_));
  nor2   g136(.a(x27), .b(x26), .O(new_n188_));
  nor2   g137(.a(x33), .b(x32), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n171_), .c(new_n149_), .d(new_n188_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n113_), .c(new_n187_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x16), .O(new_n192_));
  nor2   g141(.a(x16), .b(x01), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n52_), .O(new_n194_));
  nand3  g143(.a(new_n194_), .b(new_n192_), .c(new_n72_), .O(z14));
  inv1   g144(.a(x34), .O(new_n196_));
  nand3  g145(.a(new_n157_), .b(new_n86_), .c(new_n91_), .O(new_n197_));
  inv1   g146(.a(x31), .O(new_n198_));
  nand3  g147(.a(new_n189_), .b(new_n149_), .c(new_n198_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n197_), .c(new_n54_), .O(new_n200_));
  nand3  g149(.a(new_n156_), .b(new_n119_), .c(new_n100_), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n66_), .O(new_n202_));
  aoi21  g151(.a(new_n202_), .b(new_n200_), .c(new_n196_), .O(new_n203_));
  nand2  g152(.a(new_n149_), .b(new_n188_), .O(new_n204_));
  inv1   g153(.a(x32), .O(new_n205_));
  nor2   g154(.a(x34), .b(x33), .O(new_n206_));
  nand3  g155(.a(new_n206_), .b(new_n171_), .c(new_n205_), .O(new_n207_));
  nor3   g156(.a(new_n207_), .b(new_n204_), .c(new_n113_), .O(new_n208_));
  oai21  g157(.a(new_n208_), .b(new_n203_), .c(x16), .O(new_n209_));
  inv1   g158(.a(x00), .O(new_n210_));
  aoi21  g159(.a(new_n59_), .b(new_n210_), .c(x18), .O(new_n211_));
  aoi21  g160(.a(new_n211_), .b(new_n209_), .c(new_n52_), .O(z15));
endmodule


