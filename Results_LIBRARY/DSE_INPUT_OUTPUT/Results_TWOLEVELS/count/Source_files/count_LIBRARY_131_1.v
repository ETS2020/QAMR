// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_;
  inv1   g000(.a(x10), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x09), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  nand3  g013(.a(new_n63_), .b(new_n55_), .c(new_n54_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n64_), .c(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n60_), .b(new_n68_), .c(x18), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n67_), .c(new_n53_), .O(z01));
  oai21  g019(.a(new_n52_), .b(x09), .c(x18), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n57_), .b(new_n72_), .c(new_n63_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n53_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n71_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  inv1   g028(.a(new_n73_), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n57_), .c(new_n63_), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n79_), .c(new_n82_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(x16), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n53_), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n84_), .c(new_n71_), .O(z03));
  and2   g036(.a(new_n82_), .b(x23), .O(new_n88_));
  nor2   g037(.a(x23), .b(x22), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n72_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n65_), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n88_), .c(x16), .O(new_n92_));
  nor2   g041(.a(x16), .b(x11), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n53_), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n92_), .c(new_n71_), .O(z04));
  and2   g044(.a(x16), .b(x09), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n52_), .O(new_n97_));
  inv1   g046(.a(x24), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(new_n55_), .c(new_n65_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n81_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n65_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  inv1   g053(.a(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  aoi22  g055(.a(new_n106_), .b(new_n52_), .c(x18), .d(x09), .O(new_n107_));
  oai21  g056(.a(new_n104_), .b(new_n97_), .c(new_n107_), .O(z05));
  inv1   g057(.a(x25), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n89_), .O(new_n111_));
  oai22  g060(.a(new_n111_), .b(new_n73_), .c(new_n99_), .d(new_n109_), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(x16), .c(x18), .O(new_n113_));
  nand3  g062(.a(x25), .b(x24), .c(x16), .O(new_n114_));
  oai21  g063(.a(x16), .b(x09), .c(new_n114_), .O(new_n115_));
  nor3   g064(.a(new_n109_), .b(new_n98_), .c(x19), .O(new_n116_));
  aoi22  g065(.a(new_n116_), .b(new_n96_), .c(new_n115_), .d(new_n52_), .O(new_n117_));
  oai21  g066(.a(new_n113_), .b(new_n53_), .c(new_n117_), .O(z06));
  inv1   g067(.a(x26), .O(new_n119_));
  nor2   g068(.a(new_n111_), .b(new_n73_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nor3   g070(.a(x26), .b(x25), .c(x24), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n89_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n73_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n121_), .c(x16), .O(new_n125_));
  nor2   g074(.a(x16), .b(x08), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n53_), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n125_), .c(new_n71_), .O(z07));
  inv1   g077(.a(x27), .O(new_n129_));
  nor2   g078(.a(new_n124_), .b(new_n129_), .O(new_n130_));
  nor3   g079(.a(x27), .b(x26), .c(x25), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n101_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n82_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n130_), .c(x16), .O(new_n134_));
  nor2   g083(.a(x16), .b(x07), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n53_), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n134_), .c(new_n71_), .O(z08));
  inv1   g086(.a(x28), .O(new_n138_));
  nor2   g087(.a(new_n133_), .b(new_n138_), .O(new_n139_));
  inv1   g088(.a(x23), .O(new_n140_));
  nand2  g089(.a(new_n110_), .b(new_n140_), .O(new_n141_));
  nor2   g090(.a(x28), .b(x27), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n119_), .O(new_n143_));
  nor3   g092(.a(new_n143_), .b(new_n141_), .c(new_n82_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n139_), .c(x16), .O(new_n145_));
  nor2   g094(.a(x16), .b(x06), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n53_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n145_), .c(new_n71_), .O(z09));
  inv1   g097(.a(x29), .O(new_n149_));
  nor2   g098(.a(x29), .b(x28), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n122_), .c(new_n91_), .d(new_n129_), .O(new_n151_));
  oai21  g100(.a(new_n144_), .b(new_n149_), .c(new_n151_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  nor2   g102(.a(x16), .b(x05), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n53_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n153_), .c(new_n71_), .O(z10));
  inv1   g105(.a(x30), .O(new_n157_));
  nor2   g106(.a(x26), .b(x25), .O(new_n158_));
  nand3  g107(.a(new_n150_), .b(new_n158_), .c(new_n129_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n102_), .c(new_n55_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n66_), .c(new_n157_), .O(new_n161_));
  nand3  g110(.a(new_n89_), .b(new_n66_), .c(new_n72_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n142_), .c(new_n122_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n161_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n60_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n53_), .O(z11));
  inv1   g118(.a(x31), .O(new_n170_));
  nor2   g119(.a(x31), .b(x30), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n150_), .c(new_n131_), .d(new_n103_), .O(new_n172_));
  oai21  g121(.a(new_n165_), .b(new_n170_), .c(new_n172_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  nor2   g123(.a(x16), .b(x03), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n53_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n174_), .c(new_n71_), .O(z12));
  inv1   g126(.a(x32), .O(new_n178_));
  nand3  g127(.a(new_n110_), .b(new_n81_), .c(new_n140_), .O(new_n179_));
  nand4  g128(.a(new_n171_), .b(new_n142_), .c(new_n149_), .d(new_n119_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n179_), .c(new_n55_), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n66_), .c(new_n178_), .O(new_n182_));
  nor2   g131(.a(x32), .b(x31), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n163_), .c(new_n142_), .d(new_n158_), .O(new_n184_));
  nor3   g133(.a(new_n184_), .b(new_n102_), .c(new_n65_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n182_), .c(x16), .O(new_n186_));
  inv1   g135(.a(x02), .O(new_n187_));
  aoi21  g136(.a(new_n60_), .b(new_n187_), .c(x18), .O(new_n188_));
  aoi21  g137(.a(new_n188_), .b(new_n186_), .c(new_n53_), .O(z13));
  inv1   g138(.a(x33), .O(new_n190_));
  nand3  g139(.a(new_n150_), .b(new_n129_), .c(new_n119_), .O(new_n191_));
  nand3  g140(.a(new_n171_), .b(new_n190_), .c(new_n178_), .O(new_n192_));
  nor2   g141(.a(new_n192_), .b(new_n191_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n120_), .O(new_n194_));
  oai21  g143(.a(new_n185_), .b(new_n190_), .c(new_n194_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(x16), .O(new_n196_));
  nor2   g145(.a(x16), .b(x01), .O(new_n197_));
  nor2   g146(.a(new_n197_), .b(new_n53_), .O(new_n198_));
  nand3  g147(.a(new_n198_), .b(new_n196_), .c(new_n71_), .O(z14));
  nand2  g148(.a(new_n194_), .b(x34), .O(new_n200_));
  inv1   g149(.a(new_n191_), .O(new_n201_));
  nor3   g150(.a(x34), .b(x33), .c(x32), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n201_), .c(new_n171_), .d(new_n120_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n200_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(x16), .O(new_n205_));
  nor2   g154(.a(x16), .b(x00), .O(new_n206_));
  nor2   g155(.a(new_n206_), .b(new_n53_), .O(new_n207_));
  nand3  g156(.a(new_n207_), .b(new_n205_), .c(new_n71_), .O(z15));
endmodule


