// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_;
  inv1   g000(.a(x10), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  oai21  g011(.a(new_n52_), .b(x01), .c(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n55_), .c(new_n54_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n57_), .O(new_n74_));
  oai21  g023(.a(new_n67_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n60_), .b(new_n77_), .c(x18), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n76_), .c(new_n53_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  aoi21  g029(.a(new_n73_), .b(new_n57_), .c(new_n80_), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  nor3   g032(.a(new_n83_), .b(x19), .c(x17), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n81_), .c(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n63_), .O(z03));
  inv1   g037(.a(x23), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n72_), .O(new_n91_));
  inv1   g040(.a(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n67_), .O(new_n93_));
  oai21  g042(.a(new_n84_), .b(new_n89_), .c(new_n93_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x16), .O(new_n95_));
  nor2   g044(.a(x16), .b(x11), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n53_), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n95_), .c(new_n63_), .O(z04));
  aoi21  g047(.a(x16), .b(x01), .c(new_n52_), .O(new_n99_));
  oai21  g048(.a(new_n92_), .b(x19), .c(new_n67_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n82_), .c(new_n67_), .O(new_n102_));
  inv1   g051(.a(new_n102_), .O(new_n103_));
  aoi21  g052(.a(new_n100_), .b(x24), .c(new_n103_), .O(new_n104_));
  inv1   g053(.a(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  aoi22  g055(.a(new_n106_), .b(new_n52_), .c(x18), .d(x01), .O(new_n107_));
  oai21  g056(.a(new_n104_), .b(new_n99_), .c(new_n107_), .O(z05));
  inv1   g057(.a(x25), .O(new_n109_));
  nand2  g058(.a(new_n101_), .b(new_n82_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n55_), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n67_), .c(new_n109_), .O(new_n112_));
  nor2   g061(.a(x25), .b(x24), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n90_), .c(new_n73_), .d(new_n57_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n112_), .c(x16), .O(new_n116_));
  inv1   g065(.a(x09), .O(new_n117_));
  aoi21  g066(.a(new_n60_), .b(new_n117_), .c(x18), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(new_n116_), .c(new_n53_), .O(z06));
  inv1   g068(.a(x26), .O(new_n120_));
  nand3  g069(.a(new_n113_), .b(new_n82_), .c(new_n89_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n55_), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n67_), .c(new_n120_), .O(new_n123_));
  nor3   g072(.a(x26), .b(x25), .c(x24), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n90_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n74_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n123_), .c(x16), .O(new_n127_));
  inv1   g076(.a(x08), .O(new_n128_));
  aoi21  g077(.a(new_n60_), .b(new_n128_), .c(x18), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n127_), .c(new_n53_), .O(z07));
  inv1   g079(.a(x27), .O(new_n131_));
  nor2   g080(.a(new_n91_), .b(new_n66_), .O(new_n132_));
  nor2   g081(.a(new_n124_), .b(x19), .O(new_n133_));
  inv1   g082(.a(new_n133_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n132_), .c(new_n131_), .O(new_n135_));
  nor2   g084(.a(x27), .b(x26), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n101_), .c(new_n109_), .O(new_n137_));
  nor4   g086(.a(new_n137_), .b(new_n83_), .c(x19), .d(x17), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n135_), .c(x16), .O(new_n139_));
  nor2   g088(.a(x16), .b(x07), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n53_), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n139_), .c(new_n63_), .O(z08));
  nand4  g091(.a(new_n82_), .b(new_n67_), .c(new_n131_), .d(new_n89_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n133_), .c(x28), .O(new_n144_));
  nor3   g093(.a(x25), .b(x24), .c(x23), .O(new_n145_));
  nor2   g094(.a(x28), .b(x27), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n145_), .c(new_n84_), .d(new_n120_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x16), .O(new_n149_));
  nor2   g098(.a(x16), .b(x06), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n53_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n149_), .c(new_n63_), .O(z09));
  inv1   g101(.a(x29), .O(new_n153_));
  nor2   g102(.a(x26), .b(x25), .O(new_n154_));
  nand4  g103(.a(new_n146_), .b(new_n154_), .c(new_n101_), .d(new_n82_), .O(new_n155_));
  aoi21  g104(.a(new_n155_), .b(new_n55_), .c(new_n66_), .O(new_n156_));
  nor2   g105(.a(x29), .b(x28), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n124_), .c(new_n131_), .O(new_n158_));
  oai22  g107(.a(new_n158_), .b(new_n93_), .c(new_n156_), .d(new_n153_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  inv1   g109(.a(x05), .O(new_n161_));
  aoi21  g110(.a(new_n60_), .b(new_n161_), .c(x18), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n160_), .c(new_n53_), .O(z10));
  inv1   g112(.a(x30), .O(new_n164_));
  nand3  g113(.a(new_n157_), .b(new_n154_), .c(new_n131_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n110_), .c(new_n55_), .O(new_n166_));
  aoi21  g115(.a(new_n166_), .b(new_n67_), .c(new_n164_), .O(new_n167_));
  nor2   g116(.a(x30), .b(x29), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n146_), .c(new_n124_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n93_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(x16), .O(new_n171_));
  inv1   g120(.a(x04), .O(new_n172_));
  aoi21  g121(.a(new_n60_), .b(new_n172_), .c(x18), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n171_), .c(new_n53_), .O(z11));
  inv1   g123(.a(x31), .O(new_n175_));
  inv1   g124(.a(x28), .O(new_n176_));
  nand3  g125(.a(new_n168_), .b(new_n136_), .c(new_n176_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n121_), .c(new_n55_), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n67_), .c(new_n175_), .O(new_n179_));
  nor2   g128(.a(x31), .b(x30), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n157_), .c(new_n136_), .d(new_n109_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n102_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n179_), .c(x16), .O(new_n183_));
  inv1   g132(.a(x03), .O(new_n184_));
  aoi21  g133(.a(new_n60_), .b(new_n184_), .c(x18), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n183_), .c(new_n53_), .O(z12));
  inv1   g135(.a(x32), .O(new_n187_));
  nand4  g136(.a(new_n180_), .b(new_n146_), .c(new_n153_), .d(new_n120_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n121_), .c(new_n55_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n67_), .c(new_n187_), .O(new_n190_));
  nand3  g139(.a(new_n168_), .b(new_n187_), .c(new_n175_), .O(new_n191_));
  inv1   g140(.a(new_n191_), .O(new_n192_));
  nand3  g141(.a(new_n192_), .b(new_n146_), .c(new_n154_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n102_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n190_), .c(x16), .O(new_n195_));
  inv1   g144(.a(x02), .O(new_n196_));
  aoi21  g145(.a(new_n60_), .b(new_n196_), .c(x18), .O(new_n197_));
  aoi21  g146(.a(new_n197_), .b(new_n195_), .c(new_n53_), .O(z13));
  inv1   g147(.a(x33), .O(new_n199_));
  nand3  g148(.a(new_n146_), .b(new_n82_), .c(new_n89_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n55_), .c(new_n66_), .O(new_n201_));
  nor2   g150(.a(x33), .b(x32), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n180_), .c(new_n157_), .d(new_n136_), .O(new_n203_));
  oai22  g152(.a(new_n203_), .b(new_n114_), .c(new_n201_), .d(new_n199_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(x16), .c(x18), .O(new_n205_));
  inv1   g154(.a(x01), .O(new_n206_));
  oai21  g155(.a(x19), .b(new_n206_), .c(x10), .O(new_n207_));
  nand2  g156(.a(new_n192_), .b(new_n124_), .O(new_n208_));
  nand4  g157(.a(new_n208_), .b(new_n207_), .c(x33), .d(x16), .O(new_n209_));
  nand3  g158(.a(new_n60_), .b(new_n52_), .c(new_n206_), .O(new_n210_));
  and2   g159(.a(new_n210_), .b(new_n209_), .O(new_n211_));
  oai21  g160(.a(new_n205_), .b(new_n53_), .c(new_n211_), .O(z14));
  inv1   g161(.a(x34), .O(new_n213_));
  inv1   g162(.a(x24), .O(new_n214_));
  nand4  g163(.a(new_n154_), .b(new_n90_), .c(new_n214_), .d(new_n72_), .O(new_n215_));
  nand4  g164(.a(new_n202_), .b(new_n180_), .c(new_n157_), .d(new_n131_), .O(new_n216_));
  oai21  g165(.a(new_n216_), .b(new_n215_), .c(new_n55_), .O(new_n217_));
  aoi21  g166(.a(new_n217_), .b(new_n67_), .c(new_n213_), .O(new_n218_));
  nand2  g167(.a(new_n157_), .b(new_n136_), .O(new_n219_));
  nand4  g168(.a(new_n180_), .b(new_n213_), .c(new_n199_), .d(new_n187_), .O(new_n220_));
  nor3   g169(.a(new_n220_), .b(new_n219_), .c(new_n114_), .O(new_n221_));
  oai21  g170(.a(new_n221_), .b(new_n218_), .c(x16), .O(new_n222_));
  inv1   g171(.a(x00), .O(new_n223_));
  aoi21  g172(.a(new_n60_), .b(new_n223_), .c(x18), .O(new_n224_));
  aoi21  g173(.a(new_n224_), .b(new_n222_), .c(new_n53_), .O(z15));
endmodule


