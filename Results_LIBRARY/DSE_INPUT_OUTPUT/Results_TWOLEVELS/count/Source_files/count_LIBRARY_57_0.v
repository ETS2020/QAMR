// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_;
  nor2   g000(.a(x13), .b(x09), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g012(.a(new_n62_), .b(new_n54_), .c(new_n53_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n63_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n59_), .b(new_n67_), .c(x18), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n66_), .c(new_n52_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n56_), .O(new_n72_));
  oai21  g021(.a(new_n65_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n73_), .b(x16), .c(x18), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  nand3  g024(.a(new_n59_), .b(new_n75_), .c(x09), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n52_), .c(new_n76_), .O(z02));
  oai21  g026(.a(x13), .b(x09), .c(x18), .O(new_n78_));
  inv1   g027(.a(x22), .O(new_n79_));
  inv1   g028(.a(new_n72_), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n56_), .c(new_n62_), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n79_), .c(new_n82_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(x16), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n52_), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n84_), .c(new_n78_), .O(z03));
  inv1   g036(.a(x23), .O(new_n88_));
  oai21  g037(.a(x22), .b(x21), .c(new_n54_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n65_), .c(new_n88_), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n65_), .c(new_n70_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n90_), .c(x16), .O(new_n94_));
  inv1   g043(.a(x11), .O(new_n95_));
  aoi21  g044(.a(new_n59_), .b(new_n95_), .c(x18), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n94_), .c(new_n52_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n81_), .c(new_n65_), .O(new_n100_));
  oai21  g049(.a(new_n93_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x16), .O(new_n102_));
  nor2   g051(.a(x16), .b(x10), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n52_), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n102_), .c(new_n78_), .O(z05));
  inv1   g054(.a(x09), .O(new_n106_));
  oai21  g055(.a(new_n59_), .b(new_n75_), .c(new_n106_), .O(new_n107_));
  inv1   g056(.a(x25), .O(new_n108_));
  nand2  g057(.a(new_n99_), .b(new_n81_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n64_), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n91_), .c(new_n71_), .d(new_n56_), .O(new_n112_));
  oai21  g061(.a(new_n110_), .b(new_n108_), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n107_), .c(new_n78_), .O(z06));
  and2   g064(.a(new_n112_), .b(x26), .O(new_n116_));
  nor3   g065(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n91_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n72_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  nor2   g069(.a(x16), .b(x08), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n120_), .c(new_n78_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  nor3   g073(.a(x27), .b(x26), .c(x25), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n99_), .O(new_n126_));
  oai22  g075(.a(new_n126_), .b(new_n82_), .c(new_n119_), .d(new_n124_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(x16), .O(new_n128_));
  nor2   g077(.a(x16), .b(x07), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n52_), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n128_), .c(new_n78_), .O(z08));
  oai21  g080(.a(new_n126_), .b(new_n82_), .c(x28), .O(new_n132_));
  inv1   g081(.a(x26), .O(new_n133_));
  nor2   g082(.a(x28), .b(x27), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n111_), .c(new_n133_), .d(new_n88_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n82_), .c(new_n132_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  nor2   g086(.a(x16), .b(x06), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n52_), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n137_), .c(new_n78_), .O(z09));
  inv1   g089(.a(x29), .O(new_n141_));
  nor2   g090(.a(x26), .b(x25), .O(new_n142_));
  nand4  g091(.a(new_n134_), .b(new_n142_), .c(new_n99_), .d(new_n81_), .O(new_n143_));
  aoi21  g092(.a(new_n143_), .b(new_n54_), .c(new_n64_), .O(new_n144_));
  nor2   g093(.a(x29), .b(x28), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n117_), .c(new_n124_), .O(new_n146_));
  oai22  g095(.a(new_n146_), .b(new_n92_), .c(new_n144_), .d(new_n141_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x16), .O(new_n148_));
  inv1   g097(.a(x05), .O(new_n149_));
  aoi21  g098(.a(new_n59_), .b(new_n149_), .c(x18), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n148_), .c(new_n52_), .O(z10));
  inv1   g100(.a(x30), .O(new_n152_));
  nand3  g101(.a(new_n145_), .b(new_n142_), .c(new_n124_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n109_), .c(new_n54_), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n65_), .c(new_n152_), .O(new_n155_));
  nor2   g104(.a(x30), .b(x29), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n134_), .c(new_n117_), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n92_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n155_), .c(x16), .O(new_n159_));
  inv1   g108(.a(x04), .O(new_n160_));
  aoi21  g109(.a(new_n59_), .b(new_n160_), .c(x18), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n159_), .c(new_n52_), .O(z11));
  inv1   g111(.a(x31), .O(new_n163_));
  nand3  g112(.a(new_n111_), .b(new_n81_), .c(new_n88_), .O(new_n164_));
  inv1   g113(.a(x28), .O(new_n165_));
  nor2   g114(.a(x27), .b(x26), .O(new_n166_));
  nand3  g115(.a(new_n156_), .b(new_n166_), .c(new_n165_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n164_), .c(new_n54_), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n65_), .c(new_n163_), .O(new_n169_));
  nor2   g118(.a(x31), .b(x30), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n145_), .c(new_n125_), .O(new_n171_));
  nor2   g120(.a(new_n171_), .b(new_n100_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n169_), .c(x16), .O(new_n173_));
  inv1   g122(.a(x03), .O(new_n174_));
  aoi21  g123(.a(new_n59_), .b(new_n174_), .c(x18), .O(new_n175_));
  aoi21  g124(.a(new_n175_), .b(new_n173_), .c(new_n52_), .O(z12));
  inv1   g125(.a(x32), .O(new_n177_));
  nand2  g126(.a(new_n134_), .b(new_n142_), .O(new_n178_));
  nand3  g127(.a(new_n156_), .b(new_n177_), .c(new_n163_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n178_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n110_), .O(new_n181_));
  oai21  g130(.a(new_n172_), .b(new_n177_), .c(new_n181_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x02), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n52_), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n183_), .c(new_n78_), .O(z13));
  inv1   g135(.a(x33), .O(new_n187_));
  aoi21  g136(.a(new_n180_), .b(new_n110_), .c(new_n187_), .O(new_n188_));
  nand2  g137(.a(new_n145_), .b(new_n166_), .O(new_n189_));
  nor2   g138(.a(x33), .b(x32), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n170_), .O(new_n191_));
  nor3   g140(.a(new_n191_), .b(new_n189_), .c(new_n112_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n188_), .c(x16), .O(new_n193_));
  nor2   g142(.a(x16), .b(x01), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n52_), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n193_), .c(new_n78_), .O(z14));
  inv1   g145(.a(x34), .O(new_n197_));
  nand4  g146(.a(new_n142_), .b(new_n91_), .c(new_n98_), .d(new_n70_), .O(new_n198_));
  nand4  g147(.a(new_n190_), .b(new_n170_), .c(new_n145_), .d(new_n124_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n198_), .c(new_n54_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n65_), .c(new_n197_), .O(new_n201_));
  nand4  g150(.a(new_n170_), .b(new_n197_), .c(new_n187_), .d(new_n177_), .O(new_n202_));
  nor3   g151(.a(new_n202_), .b(new_n189_), .c(new_n112_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n201_), .c(x16), .O(new_n204_));
  inv1   g153(.a(x00), .O(new_n205_));
  aoi21  g154(.a(new_n59_), .b(new_n205_), .c(x18), .O(new_n206_));
  aoi21  g155(.a(new_n206_), .b(new_n204_), .c(new_n52_), .O(z15));
endmodule


