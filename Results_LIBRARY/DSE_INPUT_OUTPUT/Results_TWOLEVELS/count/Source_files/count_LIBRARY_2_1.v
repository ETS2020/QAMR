// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x18), .b(x13), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n58_), .O(z01));
  nand2  g019(.a(x20), .b(new_n60_), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n64_), .c(new_n63_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n53_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n61_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n61_), .c(new_n78_), .O(z02));
  inv1   g028(.a(x21), .O(new_n80_));
  inv1   g029(.a(x22), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g031(.a(x20), .b(x17), .O(new_n83_));
  nor2   g032(.a(x22), .b(x21), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  nand2  g034(.a(new_n66_), .b(x22), .O(new_n86_));
  oai21  g035(.a(new_n85_), .b(x19), .c(new_n86_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x12), .O(new_n88_));
  aoi21  g037(.a(new_n87_), .b(x16), .c(new_n88_), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(x18), .c(new_n58_), .O(z03));
  nor3   g039(.a(x22), .b(x21), .c(x20), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(x18), .c(new_n53_), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n80_), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n66_), .O(new_n95_));
  aoi21  g044(.a(new_n92_), .b(x23), .c(new_n95_), .O(new_n96_));
  inv1   g045(.a(x11), .O(new_n97_));
  aoi21  g046(.a(new_n61_), .b(new_n97_), .c(x18), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n61_), .c(new_n98_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  nand2  g049(.a(new_n93_), .b(new_n73_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n60_), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n53_), .c(new_n100_), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n84_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n66_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n61_), .b(new_n108_), .c(x18), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n107_), .O(z05));
  inv1   g059(.a(x25), .O(new_n111_));
  aoi21  g060(.a(new_n105_), .b(new_n64_), .c(new_n66_), .O(new_n112_));
  nor2   g061(.a(x25), .b(x24), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n93_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n75_), .O(new_n116_));
  oai21  g065(.a(new_n112_), .b(new_n111_), .c(new_n116_), .O(new_n117_));
  nor2   g066(.a(x16), .b(x09), .O(new_n118_));
  aoi21  g067(.a(new_n117_), .b(x16), .c(new_n118_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(x18), .c(new_n58_), .O(z06));
  inv1   g069(.a(x26), .O(new_n121_));
  nor3   g070(.a(x25), .b(x24), .c(x23), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n91_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n60_), .O(new_n124_));
  aoi21  g073(.a(new_n124_), .b(new_n53_), .c(new_n121_), .O(new_n125_));
  nor3   g074(.a(x26), .b(x25), .c(x24), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n93_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n74_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n125_), .c(x16), .O(new_n129_));
  inv1   g078(.a(x08), .O(new_n130_));
  aoi21  g079(.a(new_n61_), .b(new_n130_), .c(x18), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(z07));
  inv1   g081(.a(x27), .O(new_n133_));
  nor3   g082(.a(x23), .b(x22), .c(x21), .O(new_n134_));
  nand2  g083(.a(new_n126_), .b(new_n134_), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n64_), .c(new_n66_), .O(new_n136_));
  nor3   g085(.a(x27), .b(x26), .c(x25), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n104_), .c(new_n91_), .d(new_n53_), .O(new_n138_));
  oai21  g087(.a(new_n136_), .b(new_n133_), .c(new_n138_), .O(new_n139_));
  nor2   g088(.a(x16), .b(x07), .O(new_n140_));
  aoi21  g089(.a(new_n139_), .b(x16), .c(new_n140_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(x18), .c(new_n58_), .O(z08));
  inv1   g091(.a(x28), .O(new_n143_));
  nor2   g092(.a(x27), .b(x26), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n113_), .c(new_n134_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n64_), .c(new_n66_), .O(new_n146_));
  nor3   g095(.a(x28), .b(x27), .c(x26), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n122_), .c(new_n91_), .d(new_n53_), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n143_), .c(new_n148_), .O(new_n149_));
  nor2   g098(.a(x16), .b(x06), .O(new_n150_));
  aoi21  g099(.a(new_n149_), .b(x16), .c(new_n150_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(x18), .c(new_n58_), .O(z09));
  inv1   g101(.a(x29), .O(new_n153_));
  nor2   g102(.a(x26), .b(x25), .O(new_n154_));
  nor2   g103(.a(x28), .b(x27), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n104_), .d(new_n84_), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n64_), .c(new_n66_), .O(new_n157_));
  nor3   g106(.a(x29), .b(x28), .c(x27), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n126_), .c(new_n95_), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n153_), .c(new_n159_), .O(new_n160_));
  nor2   g109(.a(x16), .b(x05), .O(new_n161_));
  aoi21  g110(.a(new_n160_), .b(x16), .c(new_n161_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(x18), .c(new_n58_), .O(z10));
  inv1   g112(.a(x30), .O(new_n164_));
  nand4  g113(.a(new_n158_), .b(new_n154_), .c(new_n104_), .d(new_n84_), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n64_), .c(new_n66_), .O(new_n166_));
  nor2   g115(.a(x30), .b(x29), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n155_), .c(new_n126_), .d(new_n95_), .O(new_n168_));
  oai21  g117(.a(new_n166_), .b(new_n164_), .c(new_n168_), .O(new_n169_));
  nor2   g118(.a(x16), .b(x04), .O(new_n170_));
  aoi21  g119(.a(new_n169_), .b(x16), .c(new_n170_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(x18), .c(new_n58_), .O(z11));
  inv1   g121(.a(x31), .O(new_n173_));
  nand3  g122(.a(new_n104_), .b(new_n73_), .c(new_n81_), .O(new_n174_));
  nand3  g123(.a(new_n167_), .b(new_n137_), .c(new_n143_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n174_), .c(new_n60_), .O(new_n176_));
  aoi21  g125(.a(new_n176_), .b(new_n53_), .c(new_n173_), .O(new_n177_));
  nor2   g126(.a(x29), .b(x28), .O(new_n178_));
  nor2   g127(.a(x31), .b(x30), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n178_), .c(new_n137_), .O(new_n180_));
  nor3   g129(.a(new_n180_), .b(new_n105_), .c(new_n66_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n177_), .c(x16), .O(new_n182_));
  inv1   g131(.a(x03), .O(new_n183_));
  aoi21  g132(.a(new_n61_), .b(new_n183_), .c(x18), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n182_), .O(z12));
  inv1   g134(.a(x32), .O(new_n186_));
  nor3   g135(.a(x31), .b(x30), .c(x29), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n147_), .c(new_n122_), .d(new_n84_), .O(new_n188_));
  aoi21  g137(.a(new_n188_), .b(new_n64_), .c(new_n66_), .O(new_n189_));
  nand2  g138(.a(new_n155_), .b(new_n154_), .O(new_n190_));
  inv1   g139(.a(new_n190_), .O(new_n191_));
  nor2   g140(.a(x32), .b(x31), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n167_), .c(new_n191_), .d(new_n106_), .O(new_n193_));
  oai21  g142(.a(new_n189_), .b(new_n186_), .c(new_n193_), .O(new_n194_));
  nor2   g143(.a(x16), .b(x02), .O(new_n195_));
  aoi21  g144(.a(new_n194_), .b(x16), .c(new_n195_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(x18), .c(new_n58_), .O(z13));
  inv1   g146(.a(x33), .O(new_n198_));
  nor3   g147(.a(x32), .b(x31), .c(x30), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n158_), .c(new_n126_), .d(new_n134_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n64_), .c(new_n66_), .O(new_n201_));
  nand2  g150(.a(new_n178_), .b(new_n144_), .O(new_n202_));
  inv1   g151(.a(new_n202_), .O(new_n203_));
  nand3  g152(.a(new_n179_), .b(new_n198_), .c(new_n186_), .O(new_n204_));
  inv1   g153(.a(new_n204_), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n203_), .c(new_n115_), .d(new_n75_), .O(new_n206_));
  oai21  g155(.a(new_n201_), .b(new_n198_), .c(new_n206_), .O(new_n207_));
  nor2   g156(.a(x16), .b(x01), .O(new_n208_));
  aoi21  g157(.a(new_n207_), .b(x16), .c(new_n208_), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(x18), .c(new_n58_), .O(z14));
  inv1   g159(.a(x34), .O(new_n211_));
  nand3  g160(.a(new_n154_), .b(new_n104_), .c(new_n91_), .O(new_n212_));
  nand2  g161(.a(new_n205_), .b(new_n158_), .O(new_n213_));
  oai21  g162(.a(new_n213_), .b(new_n212_), .c(new_n60_), .O(new_n214_));
  aoi21  g163(.a(new_n214_), .b(new_n53_), .c(new_n211_), .O(new_n215_));
  nand4  g164(.a(new_n179_), .b(new_n211_), .c(new_n198_), .d(new_n186_), .O(new_n216_));
  nor3   g165(.a(new_n216_), .b(new_n202_), .c(new_n116_), .O(new_n217_));
  oai21  g166(.a(new_n217_), .b(new_n215_), .c(x16), .O(new_n218_));
  inv1   g167(.a(x00), .O(new_n219_));
  aoi21  g168(.a(new_n61_), .b(new_n219_), .c(x18), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n218_), .O(z15));
endmodule


