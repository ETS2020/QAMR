// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x12), .O(new_n52_));
  nor2   g001(.a(x14), .b(new_n52_), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  inv1   g011(.a(new_n53_), .O(new_n63_));
  inv1   g012(.a(new_n57_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x20), .O(new_n65_));
  inv1   g014(.a(x20), .O(new_n66_));
  nand3  g015(.a(new_n66_), .b(new_n55_), .c(new_n54_), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n65_), .c(new_n60_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(x18), .c(new_n63_), .O(new_n69_));
  inv1   g018(.a(x14), .O(new_n70_));
  nand3  g019(.a(new_n60_), .b(new_n70_), .c(new_n52_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n69_), .O(z01));
  nand2  g021(.a(new_n63_), .b(x18), .O(new_n73_));
  inv1   g022(.a(x21), .O(new_n74_));
  nor3   g023(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  nor2   g024(.a(x21), .b(x20), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n57_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n74_), .c(new_n77_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(x16), .O(new_n79_));
  nor2   g028(.a(x16), .b(x13), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n53_), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n79_), .c(new_n73_), .O(z02));
  oai21  g031(.a(x18), .b(new_n60_), .c(new_n52_), .O(new_n83_));
  inv1   g032(.a(x22), .O(new_n84_));
  inv1   g033(.a(new_n77_), .O(new_n85_));
  nor2   g034(.a(x22), .b(x21), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n57_), .c(new_n66_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n84_), .c(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x16), .O(new_n89_));
  aoi21  g038(.a(x18), .b(x14), .c(new_n53_), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n89_), .c(new_n83_), .O(z03));
  inv1   g040(.a(x23), .O(new_n92_));
  inv1   g041(.a(new_n87_), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n75_), .c(new_n74_), .O(new_n95_));
  oai21  g044(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x16), .O(new_n97_));
  nor2   g046(.a(x16), .b(x11), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n53_), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n97_), .c(new_n73_), .O(z04));
  inv1   g049(.a(x24), .O(new_n101_));
  nand2  g050(.a(new_n94_), .b(new_n74_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n55_), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n75_), .c(new_n101_), .O(new_n104_));
  nor2   g053(.a(x24), .b(x23), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n86_), .c(new_n75_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n104_), .c(x16), .O(new_n108_));
  inv1   g057(.a(x10), .O(new_n109_));
  aoi21  g058(.a(new_n60_), .b(new_n109_), .c(x18), .O(new_n110_));
  aoi21  g059(.a(new_n110_), .b(new_n108_), .c(new_n53_), .O(z05));
  inv1   g060(.a(x25), .O(new_n112_));
  nor2   g061(.a(x25), .b(x24), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n94_), .c(new_n76_), .d(new_n57_), .O(new_n114_));
  oai21  g063(.a(new_n107_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  nor2   g065(.a(x16), .b(x09), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n53_), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n116_), .c(new_n73_), .O(z06));
  inv1   g068(.a(x26), .O(new_n120_));
  nand3  g069(.a(new_n86_), .b(new_n112_), .c(new_n92_), .O(new_n121_));
  nand3  g070(.a(new_n57_), .b(new_n101_), .c(new_n66_), .O(new_n122_));
  aoi21  g071(.a(new_n121_), .b(new_n55_), .c(new_n122_), .O(new_n123_));
  nor3   g072(.a(x26), .b(x25), .c(x24), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n94_), .c(new_n85_), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n120_), .c(new_n125_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g076(.a(x08), .O(new_n128_));
  aoi21  g077(.a(new_n60_), .b(new_n128_), .c(x18), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n127_), .c(new_n53_), .O(z07));
  nand2  g079(.a(new_n125_), .b(x27), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n105_), .c(new_n93_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  nor2   g084(.a(x16), .b(x07), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n53_), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n135_), .c(new_n73_), .O(z08));
  nand2  g087(.a(new_n132_), .b(new_n105_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n87_), .c(x28), .O(new_n140_));
  nor2   g089(.a(x28), .b(x27), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n113_), .c(new_n120_), .d(new_n92_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n87_), .c(new_n140_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  nor2   g093(.a(x16), .b(x06), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n53_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n144_), .c(new_n73_), .O(z09));
  oai21  g096(.a(new_n142_), .b(new_n87_), .c(x29), .O(new_n148_));
  inv1   g097(.a(x27), .O(new_n149_));
  nor2   g098(.a(x29), .b(x28), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n124_), .c(new_n149_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n95_), .c(new_n148_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  nor2   g102(.a(x16), .b(x05), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n53_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n153_), .c(new_n73_), .O(z10));
  inv1   g105(.a(x30), .O(new_n157_));
  nor2   g106(.a(x27), .b(x25), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n150_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n102_), .c(new_n55_), .O(new_n160_));
  nand3  g109(.a(new_n120_), .b(new_n101_), .c(new_n66_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n64_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n160_), .c(new_n157_), .O(new_n163_));
  nor2   g112(.a(x30), .b(x29), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n141_), .c(new_n124_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n95_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n163_), .c(x16), .O(new_n167_));
  inv1   g116(.a(x04), .O(new_n168_));
  aoi21  g117(.a(new_n60_), .b(new_n168_), .c(x18), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n167_), .c(new_n53_), .O(z11));
  oai21  g119(.a(new_n165_), .b(new_n95_), .c(x31), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n150_), .c(new_n132_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n106_), .c(new_n171_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  nor2   g124(.a(x16), .b(x03), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n53_), .O(new_n177_));
  nand3  g126(.a(new_n177_), .b(new_n175_), .c(new_n73_), .O(z12));
  oai21  g127(.a(new_n173_), .b(new_n106_), .c(x32), .O(new_n179_));
  nor2   g128(.a(x26), .b(x25), .O(new_n180_));
  nor2   g129(.a(x32), .b(x31), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n164_), .c(new_n141_), .d(new_n180_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n106_), .c(new_n179_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(x16), .O(new_n184_));
  nor2   g133(.a(x16), .b(x02), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n53_), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n184_), .c(new_n73_), .O(z13));
  oai21  g136(.a(new_n182_), .b(new_n106_), .c(x33), .O(new_n188_));
  nor2   g137(.a(x27), .b(x26), .O(new_n189_));
  nor2   g138(.a(x33), .b(x32), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n172_), .c(new_n150_), .d(new_n189_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n114_), .c(new_n188_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(x16), .O(new_n193_));
  nor2   g142(.a(x16), .b(x01), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n53_), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n193_), .c(new_n73_), .O(z14));
  inv1   g145(.a(x34), .O(new_n197_));
  nand3  g146(.a(new_n158_), .b(new_n86_), .c(new_n92_), .O(new_n198_));
  inv1   g147(.a(x31), .O(new_n199_));
  nand3  g148(.a(new_n190_), .b(new_n150_), .c(new_n199_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n198_), .c(new_n55_), .O(new_n201_));
  nand3  g150(.a(new_n157_), .b(new_n120_), .c(new_n101_), .O(new_n202_));
  nor2   g151(.a(new_n202_), .b(new_n67_), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n201_), .c(new_n197_), .O(new_n204_));
  nand2  g153(.a(new_n150_), .b(new_n189_), .O(new_n205_));
  inv1   g154(.a(x32), .O(new_n206_));
  nor2   g155(.a(x34), .b(x33), .O(new_n207_));
  nand3  g156(.a(new_n207_), .b(new_n172_), .c(new_n206_), .O(new_n208_));
  nor3   g157(.a(new_n208_), .b(new_n205_), .c(new_n114_), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(new_n204_), .c(x16), .O(new_n210_));
  inv1   g159(.a(x00), .O(new_n211_));
  aoi21  g160(.a(new_n60_), .b(new_n211_), .c(x18), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n210_), .c(new_n53_), .O(z15));
endmodule


