// Benchmark "FAU" written by ABC on Wed Aug 19 19:40:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n212_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n229_, new_n230_, new_n231_, new_n232_, new_n233_, new_n234_,
    new_n235_, new_n236_, new_n237_, new_n238_, new_n239_, new_n240_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x27), .O(new_n57_));
  nor2   g006(.a(new_n57_), .b(x12), .O(new_n58_));
  inv1   g007(.a(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(x18), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n58_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n60_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n55_), .b(new_n72_), .c(new_n64_), .O(new_n73_));
  oai21  g022(.a(new_n67_), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n58_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n60_), .O(z02));
  inv1   g027(.a(x12), .O(new_n79_));
  inv1   g028(.a(x16), .O(new_n80_));
  oai21  g029(.a(x27), .b(new_n80_), .c(new_n79_), .O(new_n81_));
  nand2  g030(.a(x22), .b(x21), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n64_), .c(new_n52_), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(x19), .O(new_n85_));
  and2   g034(.a(new_n66_), .b(x22), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n81_), .c(new_n60_), .O(z03));
  inv1   g037(.a(x23), .O(new_n89_));
  oai21  g038(.a(x22), .b(x21), .c(new_n53_), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(new_n67_), .c(new_n89_), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n72_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n91_), .c(x16), .O(new_n95_));
  nor2   g044(.a(x16), .b(x11), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n58_), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n95_), .c(new_n60_), .O(z04));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n66_), .O(new_n101_));
  aoi21  g050(.a(new_n66_), .b(x24), .c(new_n101_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n80_), .O(new_n103_));
  inv1   g052(.a(x18), .O(new_n104_));
  oai21  g053(.a(x16), .b(x10), .c(new_n104_), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n103_), .c(new_n59_), .O(new_n106_));
  oai21  g055(.a(x27), .b(x19), .c(new_n79_), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n93_), .c(x24), .d(x16), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(z05));
  inv1   g058(.a(x25), .O(new_n110_));
  nand2  g059(.a(new_n100_), .b(new_n53_), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n67_), .c(new_n110_), .O(new_n112_));
  inv1   g061(.a(new_n73_), .O(new_n113_));
  nor2   g062(.a(x25), .b(x24), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n92_), .O(new_n115_));
  inv1   g064(.a(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  inv1   g066(.a(new_n117_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n112_), .c(x16), .O(new_n119_));
  nor2   g068(.a(x16), .b(x09), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n58_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n119_), .c(new_n60_), .O(z06));
  inv1   g071(.a(x26), .O(new_n123_));
  nor3   g072(.a(x25), .b(x24), .c(x23), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n83_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n53_), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n67_), .c(new_n123_), .O(new_n127_));
  nor3   g076(.a(x26), .b(x25), .c(x24), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n92_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n73_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n127_), .c(x16), .O(new_n131_));
  nor2   g080(.a(x16), .b(x08), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n58_), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n131_), .c(new_n60_), .O(z07));
  nand3  g083(.a(new_n128_), .b(new_n92_), .c(new_n72_), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n53_), .c(new_n66_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n57_), .O(new_n137_));
  nand3  g086(.a(new_n83_), .b(new_n55_), .c(new_n64_), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n99_), .c(new_n110_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n137_), .c(x16), .O(new_n142_));
  nor2   g091(.a(x16), .b(x07), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n58_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n142_), .c(new_n60_), .O(z08));
  nand3  g094(.a(x28), .b(new_n53_), .c(x16), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(x12), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x27), .O(new_n148_));
  inv1   g097(.a(x28), .O(new_n149_));
  nor2   g098(.a(new_n136_), .b(new_n149_), .O(new_n150_));
  nor2   g099(.a(x28), .b(x27), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n124_), .c(new_n123_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n138_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n150_), .c(x16), .O(new_n154_));
  inv1   g103(.a(x06), .O(new_n155_));
  nand2  g104(.a(new_n80_), .b(new_n155_), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n154_), .c(new_n148_), .d(new_n60_), .O(z09));
  oai21  g106(.a(x16), .b(x05), .c(new_n104_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n59_), .O(new_n159_));
  inv1   g108(.a(x29), .O(new_n160_));
  nor2   g109(.a(x28), .b(x26), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n114_), .c(new_n92_), .d(new_n72_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n53_), .c(new_n66_), .O(new_n163_));
  nor3   g112(.a(x29), .b(x28), .c(x26), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n124_), .O(new_n165_));
  oai22  g114(.a(new_n165_), .b(new_n138_), .c(new_n163_), .d(new_n160_), .O(new_n166_));
  nor2   g115(.a(new_n160_), .b(new_n57_), .O(new_n167_));
  aoi22  g116(.a(new_n167_), .b(x12), .c(new_n166_), .d(new_n57_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n80_), .c(new_n159_), .O(z10));
  nand3  g118(.a(x30), .b(new_n53_), .c(x16), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x12), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x27), .O(new_n172_));
  inv1   g121(.a(x30), .O(new_n173_));
  nor2   g122(.a(x26), .b(x25), .O(new_n174_));
  nor2   g123(.a(x29), .b(x28), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n100_), .c(new_n53_), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n67_), .c(new_n173_), .O(new_n178_));
  nor2   g127(.a(x30), .b(x29), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n151_), .c(new_n128_), .O(new_n180_));
  nor3   g129(.a(new_n180_), .b(new_n93_), .c(new_n66_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n178_), .c(x16), .O(new_n182_));
  inv1   g131(.a(x04), .O(new_n183_));
  nand2  g132(.a(new_n80_), .b(new_n183_), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n182_), .c(new_n172_), .d(new_n60_), .O(z11));
  oai21  g134(.a(x16), .b(x03), .c(new_n104_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n59_), .O(new_n187_));
  inv1   g136(.a(x31), .O(new_n188_));
  nor3   g137(.a(x30), .b(x29), .c(x28), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n174_), .c(new_n99_), .d(new_n83_), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n53_), .c(new_n66_), .O(new_n191_));
  nor2   g140(.a(x31), .b(x30), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n175_), .c(new_n128_), .d(new_n94_), .O(new_n193_));
  oai21  g142(.a(new_n191_), .b(new_n188_), .c(new_n193_), .O(new_n194_));
  nand3  g143(.a(x31), .b(x27), .c(x12), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  aoi21  g145(.a(new_n194_), .b(new_n57_), .c(new_n196_), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n80_), .c(new_n187_), .O(z12));
  oai21  g147(.a(x16), .b(x02), .c(new_n104_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n59_), .O(new_n200_));
  inv1   g149(.a(x32), .O(new_n201_));
  nor3   g150(.a(x31), .b(x30), .c(x29), .O(new_n202_));
  nand4  g151(.a(new_n202_), .b(new_n161_), .c(new_n124_), .d(new_n83_), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n53_), .c(new_n66_), .O(new_n204_));
  nor2   g153(.a(x32), .b(x31), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n179_), .O(new_n206_));
  inv1   g155(.a(new_n206_), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n161_), .c(new_n101_), .d(new_n110_), .O(new_n208_));
  oai21  g157(.a(new_n204_), .b(new_n201_), .c(new_n208_), .O(new_n209_));
  nand3  g158(.a(x32), .b(x27), .c(x12), .O(new_n210_));
  inv1   g159(.a(new_n210_), .O(new_n211_));
  aoi21  g160(.a(new_n209_), .b(new_n57_), .c(new_n211_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n80_), .c(new_n200_), .O(z13));
  nand3  g162(.a(x33), .b(new_n53_), .c(x16), .O(new_n214_));
  nand2  g163(.a(new_n214_), .b(x12), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(x27), .O(new_n216_));
  inv1   g165(.a(x33), .O(new_n217_));
  nand3  g166(.a(new_n205_), .b(new_n164_), .c(new_n173_), .O(new_n218_));
  oai21  g167(.a(new_n218_), .b(new_n125_), .c(new_n53_), .O(new_n219_));
  aoi21  g168(.a(new_n219_), .b(new_n67_), .c(new_n217_), .O(new_n220_));
  nand2  g169(.a(new_n175_), .b(new_n139_), .O(new_n221_));
  nor2   g170(.a(x33), .b(x32), .O(new_n222_));
  nand2  g171(.a(new_n222_), .b(new_n192_), .O(new_n223_));
  nor3   g172(.a(new_n223_), .b(new_n221_), .c(new_n117_), .O(new_n224_));
  oai21  g173(.a(new_n224_), .b(new_n220_), .c(x16), .O(new_n225_));
  inv1   g174(.a(x01), .O(new_n226_));
  nand2  g175(.a(new_n80_), .b(new_n226_), .O(new_n227_));
  nand4  g176(.a(new_n227_), .b(new_n225_), .c(new_n216_), .d(new_n60_), .O(z14));
  nand3  g177(.a(x34), .b(new_n53_), .c(x16), .O(new_n229_));
  nand2  g178(.a(new_n229_), .b(x12), .O(new_n230_));
  nand2  g179(.a(new_n230_), .b(x27), .O(new_n231_));
  inv1   g180(.a(x34), .O(new_n232_));
  nand3  g181(.a(new_n222_), .b(new_n189_), .c(new_n188_), .O(new_n233_));
  oai21  g182(.a(new_n233_), .b(new_n135_), .c(new_n53_), .O(new_n234_));
  aoi21  g183(.a(new_n234_), .b(new_n67_), .c(new_n232_), .O(new_n235_));
  nand4  g184(.a(new_n192_), .b(new_n232_), .c(new_n217_), .d(new_n201_), .O(new_n236_));
  nor3   g185(.a(new_n236_), .b(new_n221_), .c(new_n117_), .O(new_n237_));
  oai21  g186(.a(new_n237_), .b(new_n235_), .c(x16), .O(new_n238_));
  inv1   g187(.a(x00), .O(new_n239_));
  nand2  g188(.a(new_n80_), .b(new_n239_), .O(new_n240_));
  nand4  g189(.a(new_n240_), .b(new_n238_), .c(new_n231_), .d(new_n60_), .O(z15));
endmodule


