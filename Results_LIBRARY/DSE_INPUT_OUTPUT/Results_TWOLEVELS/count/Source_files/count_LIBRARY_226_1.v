// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_;
  inv1   g000(.a(x25), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x20), .O(new_n53_));
  nor2   g002(.a(x16), .b(x15), .O(new_n54_));
  inv1   g003(.a(x18), .O(new_n55_));
  nand3  g004(.a(x19), .b(x17), .c(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  inv1   g007(.a(x17), .O(new_n59_));
  inv1   g008(.a(x19), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  nand2  g010(.a(x25), .b(new_n61_), .O(new_n62_));
  nand4  g011(.a(new_n62_), .b(new_n60_), .c(new_n59_), .d(x16), .O(new_n63_));
  oai21  g012(.a(new_n58_), .b(new_n53_), .c(new_n63_), .O(z00));
  inv1   g013(.a(new_n53_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(x18), .O(new_n66_));
  nor2   g015(.a(x19), .b(x17), .O(new_n67_));
  nand3  g016(.a(new_n67_), .b(new_n52_), .c(new_n61_), .O(new_n68_));
  oai21  g017(.a(new_n67_), .b(new_n61_), .c(new_n68_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  nor2   g019(.a(x16), .b(x14), .O(new_n71_));
  nor2   g020(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z01));
  nor2   g022(.a(x21), .b(x19), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n59_), .c(x16), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x25), .c(new_n61_), .O(new_n77_));
  inv1   g026(.a(x21), .O(new_n78_));
  nor3   g027(.a(x20), .b(x19), .c(x17), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nor2   g029(.a(x16), .b(x13), .O(new_n81_));
  aoi21  g030(.a(new_n80_), .b(x16), .c(new_n81_), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n77_), .c(new_n66_), .O(z02));
  nor2   g032(.a(x22), .b(x21), .O(new_n84_));
  nand4  g033(.a(new_n84_), .b(new_n60_), .c(new_n59_), .d(x16), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n52_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n61_), .O(new_n87_));
  inv1   g036(.a(x22), .O(new_n88_));
  nand3  g037(.a(new_n67_), .b(new_n78_), .c(new_n61_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nor2   g040(.a(x16), .b(x12), .O(new_n92_));
  aoi21  g041(.a(new_n91_), .b(x16), .c(new_n92_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n87_), .c(new_n66_), .O(z03));
  oai21  g043(.a(x16), .b(x11), .c(new_n55_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n65_), .O(new_n96_));
  inv1   g045(.a(x23), .O(new_n97_));
  nand2  g046(.a(new_n84_), .b(new_n67_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n52_), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n61_), .c(new_n97_), .O(new_n100_));
  inv1   g049(.a(new_n79_), .O(new_n101_));
  nand3  g050(.a(new_n84_), .b(new_n52_), .c(new_n97_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n100_), .c(x16), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n96_), .O(z04));
  oai21  g054(.a(x16), .b(x10), .c(new_n55_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n65_), .O(new_n107_));
  inv1   g056(.a(x24), .O(new_n108_));
  nor2   g057(.a(x23), .b(x22), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n67_), .c(new_n78_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n52_), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n61_), .c(new_n108_), .O(new_n112_));
  nand3  g061(.a(new_n109_), .b(new_n52_), .c(new_n108_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n89_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n107_), .O(z05));
  inv1   g065(.a(x09), .O(new_n117_));
  nor2   g066(.a(new_n52_), .b(new_n61_), .O(new_n118_));
  nor2   g067(.a(x25), .b(x16), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  oai21  g069(.a(new_n118_), .b(new_n114_), .c(x16), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n120_), .c(new_n66_), .O(z06));
  oai21  g071(.a(x16), .b(x08), .c(new_n55_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n65_), .O(new_n124_));
  nor2   g073(.a(x24), .b(x23), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n74_), .c(new_n88_), .d(new_n59_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(x26), .O(new_n127_));
  nor2   g076(.a(x26), .b(x24), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n109_), .O(new_n129_));
  inv1   g078(.a(new_n129_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n90_), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n127_), .c(x25), .O(new_n132_));
  inv1   g081(.a(x26), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n61_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n124_), .O(z07));
  oai21  g085(.a(x16), .b(x07), .c(new_n55_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n65_), .O(new_n138_));
  nand2  g087(.a(new_n74_), .b(new_n59_), .O(new_n139_));
  oai21  g088(.a(new_n129_), .b(new_n139_), .c(x27), .O(new_n140_));
  nor3   g089(.a(x27), .b(x26), .c(x24), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n109_), .c(new_n90_), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n140_), .c(x25), .O(new_n143_));
  inv1   g092(.a(x27), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n61_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n143_), .c(x16), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n138_), .O(z08));
  nor3   g096(.a(x28), .b(x27), .c(x26), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n125_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n85_), .c(new_n52_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n61_), .O(new_n151_));
  nand3  g100(.a(new_n142_), .b(x28), .c(x16), .O(new_n152_));
  inv1   g101(.a(x06), .O(new_n153_));
  inv1   g102(.a(x16), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n152_), .c(new_n151_), .d(new_n66_), .O(z09));
  nor2   g105(.a(x29), .b(x28), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n128_), .c(new_n144_), .d(new_n97_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n85_), .c(new_n52_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n61_), .O(new_n160_));
  nand3  g109(.a(new_n84_), .b(new_n67_), .c(new_n61_), .O(new_n161_));
  or2    g110(.a(new_n161_), .b(new_n149_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(x29), .c(x16), .O(new_n163_));
  inv1   g112(.a(x05), .O(new_n164_));
  nand2  g113(.a(new_n154_), .b(new_n164_), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n163_), .c(new_n160_), .d(new_n66_), .O(z10));
  oai21  g115(.a(x16), .b(x04), .c(new_n55_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n65_), .O(new_n168_));
  nand3  g117(.a(new_n157_), .b(new_n128_), .c(new_n144_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n110_), .c(x30), .O(new_n170_));
  nor3   g119(.a(x23), .b(x22), .c(x21), .O(new_n171_));
  nor3   g120(.a(x30), .b(x29), .c(x28), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n141_), .c(new_n171_), .d(new_n79_), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n170_), .c(x25), .O(new_n174_));
  inv1   g123(.a(x30), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n61_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n174_), .c(x16), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n168_), .O(z11));
  nand3  g127(.a(new_n171_), .b(new_n67_), .c(x16), .O(new_n179_));
  nor2   g128(.a(x31), .b(x30), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n157_), .c(new_n141_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n179_), .c(new_n52_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n61_), .O(new_n183_));
  nand3  g132(.a(new_n173_), .b(x31), .c(x16), .O(new_n184_));
  inv1   g133(.a(x03), .O(new_n185_));
  nand2  g134(.a(new_n154_), .b(new_n185_), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n184_), .c(new_n183_), .d(new_n66_), .O(z12));
  oai21  g136(.a(x16), .b(x02), .c(new_n55_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n65_), .O(new_n189_));
  inv1   g138(.a(x29), .O(new_n190_));
  nor2   g139(.a(x28), .b(x27), .O(new_n191_));
  nand4  g140(.a(new_n180_), .b(new_n191_), .c(new_n190_), .d(new_n133_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n126_), .c(x32), .O(new_n193_));
  nand4  g142(.a(new_n108_), .b(new_n97_), .c(new_n88_), .d(new_n78_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  inv1   g144(.a(x31), .O(new_n196_));
  inv1   g145(.a(x32), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n196_), .c(new_n175_), .d(new_n190_), .O(new_n198_));
  inv1   g147(.a(new_n198_), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n195_), .c(new_n148_), .d(new_n79_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n193_), .c(x25), .O(new_n201_));
  nor2   g150(.a(new_n197_), .b(new_n61_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n201_), .c(x16), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n189_), .O(z13));
  oai21  g153(.a(x16), .b(x01), .c(new_n55_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n65_), .O(new_n206_));
  nor2   g155(.a(x30), .b(x29), .O(new_n207_));
  nor2   g156(.a(x32), .b(x31), .O(new_n208_));
  nand4  g157(.a(new_n208_), .b(new_n207_), .c(new_n191_), .d(new_n133_), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(new_n126_), .c(x33), .O(new_n210_));
  inv1   g159(.a(x28), .O(new_n211_));
  nand4  g160(.a(new_n190_), .b(new_n211_), .c(new_n144_), .d(new_n133_), .O(new_n212_));
  inv1   g161(.a(new_n212_), .O(new_n213_));
  inv1   g162(.a(x33), .O(new_n214_));
  nand4  g163(.a(new_n214_), .b(new_n197_), .c(new_n196_), .d(new_n175_), .O(new_n215_));
  inv1   g164(.a(new_n215_), .O(new_n216_));
  nand4  g165(.a(new_n216_), .b(new_n213_), .c(new_n195_), .d(new_n79_), .O(new_n217_));
  aoi21  g166(.a(new_n217_), .b(new_n210_), .c(x25), .O(new_n218_));
  nor2   g167(.a(new_n214_), .b(new_n61_), .O(new_n219_));
  oai21  g168(.a(new_n219_), .b(new_n218_), .c(x16), .O(new_n220_));
  nand2  g169(.a(new_n220_), .b(new_n206_), .O(z14));
  nand2  g170(.a(new_n130_), .b(new_n76_), .O(new_n222_));
  nor2   g171(.a(x34), .b(x33), .O(new_n223_));
  nand4  g172(.a(new_n223_), .b(new_n208_), .c(new_n207_), .d(new_n191_), .O(new_n224_));
  oai21  g173(.a(new_n224_), .b(new_n222_), .c(new_n52_), .O(new_n225_));
  nand2  g174(.a(new_n225_), .b(new_n61_), .O(new_n226_));
  nand3  g175(.a(new_n217_), .b(x34), .c(x16), .O(new_n227_));
  inv1   g176(.a(x00), .O(new_n228_));
  nand2  g177(.a(new_n154_), .b(new_n228_), .O(new_n229_));
  nand4  g178(.a(new_n229_), .b(new_n227_), .c(new_n226_), .d(new_n66_), .O(z15));
endmodule


