// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  nor2   g005(.a(x26), .b(x24), .O(new_n57_));
  nor2   g006(.a(x16), .b(x15), .O(new_n58_));
  nor3   g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  inv1   g014(.a(x16), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n65_), .c(x18), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n64_), .c(new_n57_), .O(z01));
  inv1   g017(.a(x18), .O(new_n69_));
  nor2   g018(.a(new_n57_), .b(new_n69_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n55_), .b(new_n72_), .c(new_n61_), .O(new_n73_));
  oai21  g022(.a(new_n63_), .b(new_n72_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n57_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n71_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  inv1   g028(.a(new_n73_), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n61_), .O(new_n83_));
  nor3   g032(.a(new_n83_), .b(x19), .c(x17), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n81_), .c(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n57_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n71_), .O(z03));
  inv1   g037(.a(x23), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n63_), .c(new_n72_), .O(new_n91_));
  oai21  g040(.a(new_n84_), .b(new_n89_), .c(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  nor2   g042(.a(x16), .b(x11), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n57_), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n93_), .c(new_n71_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  inv1   g046(.a(x26), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(x25), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n97_), .c(new_n89_), .d(new_n79_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(x21), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n102_));
  nand2  g051(.a(new_n91_), .b(x24), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(x16), .O(new_n105_));
  nor2   g054(.a(x16), .b(x10), .O(new_n106_));
  nor3   g055(.a(new_n106_), .b(new_n57_), .c(x18), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n105_), .O(z05));
  nand4  g057(.a(new_n61_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n109_));
  inv1   g058(.a(x25), .O(new_n110_));
  nand3  g059(.a(new_n82_), .b(new_n110_), .c(new_n89_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n109_), .c(x26), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n97_), .O(new_n113_));
  nand4  g062(.a(new_n82_), .b(new_n63_), .c(new_n97_), .d(new_n89_), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(x25), .c(x16), .O(new_n115_));
  inv1   g064(.a(x09), .O(new_n116_));
  nand2  g065(.a(new_n66_), .b(new_n116_), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n115_), .c(new_n113_), .d(new_n71_), .O(z06));
  inv1   g067(.a(x08), .O(new_n119_));
  aoi21  g068(.a(new_n66_), .b(new_n119_), .c(x18), .O(new_n120_));
  nand4  g069(.a(new_n90_), .b(new_n80_), .c(new_n110_), .d(new_n97_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(x26), .c(x16), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n57_), .c(new_n122_), .O(z07));
  inv1   g072(.a(new_n57_), .O(new_n124_));
  inv1   g073(.a(x07), .O(new_n125_));
  nand2  g074(.a(new_n66_), .b(new_n125_), .O(new_n126_));
  nand2  g075(.a(x27), .b(x16), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n126_), .c(new_n124_), .d(new_n69_), .O(z08));
  inv1   g077(.a(x06), .O(new_n129_));
  nand2  g078(.a(new_n66_), .b(new_n129_), .O(new_n130_));
  nand2  g079(.a(x28), .b(x16), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n130_), .c(new_n124_), .d(new_n69_), .O(z09));
  inv1   g081(.a(x05), .O(new_n133_));
  nand2  g082(.a(new_n66_), .b(new_n133_), .O(new_n134_));
  aoi21  g083(.a(x29), .b(x16), .c(x18), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n134_), .c(new_n57_), .O(z10));
  inv1   g085(.a(x04), .O(new_n137_));
  nand2  g086(.a(new_n66_), .b(new_n137_), .O(new_n138_));
  aoi21  g087(.a(x30), .b(x16), .c(x18), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n138_), .c(new_n57_), .O(z11));
  inv1   g089(.a(x03), .O(new_n141_));
  nand2  g090(.a(new_n66_), .b(new_n141_), .O(new_n142_));
  aoi21  g091(.a(x31), .b(x16), .c(x18), .O(new_n143_));
  aoi21  g092(.a(new_n143_), .b(new_n142_), .c(new_n57_), .O(z12));
  inv1   g093(.a(x02), .O(new_n145_));
  nand2  g094(.a(new_n66_), .b(new_n145_), .O(new_n146_));
  nand2  g095(.a(x32), .b(x16), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n146_), .c(new_n124_), .d(new_n69_), .O(z13));
  inv1   g097(.a(x01), .O(new_n149_));
  nand2  g098(.a(new_n66_), .b(new_n149_), .O(new_n150_));
  nand2  g099(.a(x33), .b(x16), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n150_), .c(new_n124_), .d(new_n69_), .O(z14));
  inv1   g101(.a(x00), .O(new_n153_));
  nand2  g102(.a(new_n66_), .b(new_n153_), .O(new_n154_));
  aoi21  g103(.a(x34), .b(x16), .c(x18), .O(new_n155_));
  aoi21  g104(.a(new_n155_), .b(new_n154_), .c(new_n57_), .O(z15));
endmodule


