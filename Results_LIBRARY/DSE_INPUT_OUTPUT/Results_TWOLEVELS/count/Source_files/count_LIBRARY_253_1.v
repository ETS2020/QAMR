// Benchmark "FAU" written by ABC on Wed Aug 19 19:40:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x18), .b(x00), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x16), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  aoi21  g010(.a(new_n54_), .b(x20), .c(new_n61_), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n60_), .b(new_n63_), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n62_), .b(new_n60_), .c(new_n64_), .O(z01));
  inv1   g014(.a(x17), .O(new_n66_));
  inv1   g015(.a(x19), .O(new_n67_));
  inv1   g016(.a(x18), .O(new_n68_));
  nand2  g017(.a(x20), .b(new_n68_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(new_n70_));
  inv1   g019(.a(x20), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nand3  g021(.a(new_n53_), .b(new_n72_), .c(new_n71_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  aoi21  g023(.a(new_n70_), .b(x21), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n60_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n60_), .c(new_n77_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n72_), .O(new_n80_));
  nor2   g029(.a(x20), .b(x17), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(new_n83_));
  oai22  g032(.a(new_n83_), .b(x19), .c(new_n61_), .d(new_n79_), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  aoi21  g034(.a(new_n84_), .b(x16), .c(new_n85_), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(x18), .c(new_n58_), .O(z03));
  inv1   g036(.a(x23), .O(new_n88_));
  oai21  g037(.a(new_n82_), .b(x19), .c(new_n71_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n68_), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(new_n53_), .c(new_n88_), .O(new_n91_));
  nor3   g040(.a(x23), .b(x22), .c(x21), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n61_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n91_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n60_), .b(new_n96_), .c(x18), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n95_), .O(z04));
  inv1   g047(.a(x24), .O(new_n99_));
  inv1   g048(.a(new_n61_), .O(new_n100_));
  inv1   g049(.a(new_n92_), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n67_), .c(new_n100_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n82_), .c(new_n61_), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n99_), .c(new_n104_), .O(new_n105_));
  nor2   g054(.a(x16), .b(x10), .O(new_n106_));
  aoi21  g055(.a(new_n105_), .b(x16), .c(new_n106_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(x18), .c(new_n58_), .O(z05));
  aoi21  g057(.a(new_n103_), .b(new_n82_), .c(x19), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n100_), .c(x25), .O(new_n110_));
  nor2   g059(.a(x23), .b(x22), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  inv1   g062(.a(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n74_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n110_), .O(new_n116_));
  nor2   g065(.a(x16), .b(x09), .O(new_n117_));
  aoi21  g066(.a(new_n116_), .b(x16), .c(new_n117_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(x18), .c(new_n58_), .O(z06));
  nor3   g068(.a(x25), .b(x24), .c(x23), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n82_), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n67_), .c(x20), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(x18), .c(new_n53_), .O(new_n123_));
  nor3   g072(.a(x26), .b(x25), .c(x24), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n111_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n73_), .O(new_n126_));
  aoi21  g075(.a(new_n123_), .b(x26), .c(new_n126_), .O(new_n127_));
  inv1   g076(.a(x08), .O(new_n128_));
  aoi21  g077(.a(new_n60_), .b(new_n128_), .c(x18), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n60_), .c(new_n129_), .O(z07));
  inv1   g079(.a(x27), .O(new_n131_));
  nand2  g080(.a(new_n124_), .b(new_n92_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n67_), .c(new_n100_), .O(new_n133_));
  nand3  g082(.a(new_n82_), .b(new_n53_), .c(new_n71_), .O(new_n134_));
  nor3   g083(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n103_), .O(new_n136_));
  oai22  g085(.a(new_n136_), .b(new_n134_), .c(new_n133_), .d(new_n131_), .O(new_n137_));
  nor2   g086(.a(x16), .b(x07), .O(new_n138_));
  aoi21  g087(.a(new_n137_), .b(x16), .c(new_n138_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(x18), .c(new_n58_), .O(z08));
  nor2   g089(.a(x27), .b(x26), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n112_), .c(new_n92_), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n67_), .c(x20), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(x18), .c(new_n53_), .O(new_n144_));
  nor3   g093(.a(x28), .b(x27), .c(x26), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n120_), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n134_), .O(new_n147_));
  aoi21  g096(.a(new_n144_), .b(x28), .c(new_n147_), .O(new_n148_));
  inv1   g097(.a(x06), .O(new_n149_));
  aoi21  g098(.a(new_n60_), .b(new_n149_), .c(x18), .O(new_n150_));
  oai21  g099(.a(new_n148_), .b(new_n60_), .c(new_n150_), .O(z09));
  inv1   g100(.a(x29), .O(new_n152_));
  nor2   g101(.a(x26), .b(x25), .O(new_n153_));
  nor2   g102(.a(x28), .b(x27), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n103_), .d(new_n82_), .O(new_n155_));
  aoi21  g104(.a(new_n155_), .b(new_n67_), .c(new_n100_), .O(new_n156_));
  nor3   g105(.a(x29), .b(x28), .c(x27), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n124_), .O(new_n158_));
  oai22  g107(.a(new_n158_), .b(new_n93_), .c(new_n156_), .d(new_n152_), .O(new_n159_));
  nor2   g108(.a(x16), .b(x05), .O(new_n160_));
  aoi21  g109(.a(new_n159_), .b(x16), .c(new_n160_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(x18), .c(new_n58_), .O(z10));
  nand4  g111(.a(new_n157_), .b(new_n153_), .c(new_n103_), .d(new_n82_), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n67_), .c(x20), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(x18), .c(new_n53_), .O(new_n165_));
  nor2   g114(.a(x30), .b(x29), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n154_), .c(new_n124_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n93_), .O(new_n168_));
  aoi21  g117(.a(new_n165_), .b(x30), .c(new_n168_), .O(new_n169_));
  inv1   g118(.a(x04), .O(new_n170_));
  aoi21  g119(.a(new_n60_), .b(new_n170_), .c(x18), .O(new_n171_));
  oai21  g120(.a(new_n169_), .b(new_n60_), .c(new_n171_), .O(z11));
  nor3   g121(.a(x30), .b(x29), .c(x28), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n141_), .c(new_n120_), .d(new_n82_), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n67_), .c(x20), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(x18), .c(new_n53_), .O(new_n176_));
  nor2   g125(.a(x29), .b(x28), .O(new_n177_));
  nor2   g126(.a(x31), .b(x30), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(new_n177_), .c(new_n135_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n104_), .O(new_n180_));
  aoi21  g129(.a(new_n176_), .b(x31), .c(new_n180_), .O(new_n181_));
  inv1   g130(.a(x03), .O(new_n182_));
  aoi21  g131(.a(new_n60_), .b(new_n182_), .c(x18), .O(new_n183_));
  oai21  g132(.a(new_n181_), .b(new_n60_), .c(new_n183_), .O(z12));
  nor3   g133(.a(x31), .b(x30), .c(x29), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n145_), .c(new_n120_), .d(new_n82_), .O(new_n186_));
  aoi21  g135(.a(new_n186_), .b(new_n67_), .c(x20), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(x18), .c(new_n53_), .O(new_n188_));
  nor2   g137(.a(x32), .b(x31), .O(new_n189_));
  nand4  g138(.a(new_n189_), .b(new_n166_), .c(new_n154_), .d(new_n153_), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n104_), .O(new_n191_));
  aoi21  g140(.a(new_n188_), .b(x32), .c(new_n191_), .O(new_n192_));
  inv1   g141(.a(x02), .O(new_n193_));
  aoi21  g142(.a(new_n60_), .b(new_n193_), .c(x18), .O(new_n194_));
  oai21  g143(.a(new_n192_), .b(new_n60_), .c(new_n194_), .O(z13));
  nor3   g144(.a(x32), .b(x31), .c(x30), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n157_), .c(new_n124_), .d(new_n92_), .O(new_n197_));
  aoi21  g146(.a(new_n197_), .b(new_n67_), .c(x20), .O(new_n198_));
  oai21  g147(.a(new_n198_), .b(x18), .c(new_n53_), .O(new_n199_));
  nand2  g148(.a(new_n177_), .b(new_n141_), .O(new_n200_));
  inv1   g149(.a(new_n200_), .O(new_n201_));
  nor2   g150(.a(x33), .b(x32), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n201_), .c(new_n178_), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n115_), .O(new_n204_));
  aoi21  g153(.a(new_n199_), .b(x33), .c(new_n204_), .O(new_n205_));
  inv1   g154(.a(x01), .O(new_n206_));
  aoi21  g155(.a(new_n60_), .b(new_n206_), .c(x18), .O(new_n207_));
  oai21  g156(.a(new_n205_), .b(new_n60_), .c(new_n207_), .O(z14));
  oai21  g157(.a(new_n203_), .b(new_n115_), .c(x34), .O(new_n209_));
  nor2   g158(.a(new_n113_), .b(new_n73_), .O(new_n210_));
  nor3   g159(.a(x34), .b(x33), .c(x32), .O(new_n211_));
  nand4  g160(.a(new_n211_), .b(new_n201_), .c(new_n178_), .d(new_n210_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n209_), .c(new_n60_), .O(new_n213_));
  nor2   g162(.a(x16), .b(x00), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n213_), .c(new_n68_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n58_), .O(z15));
endmodule


