// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x11), .O(new_n52_));
  nor2   g001(.a(x14), .b(new_n52_), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  inv1   g011(.a(new_n53_), .O(new_n63_));
  oai21  g012(.a(x19), .b(x17), .c(x20), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n55_), .c(new_n54_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n60_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(x18), .c(new_n63_), .O(new_n68_));
  inv1   g017(.a(x14), .O(new_n69_));
  nand3  g018(.a(new_n60_), .b(new_n69_), .c(new_n52_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  nand2  g020(.a(new_n63_), .b(x18), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  nor3   g022(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  nor2   g023(.a(x21), .b(x20), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n57_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n73_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(x16), .O(new_n78_));
  nor2   g027(.a(x16), .b(x13), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n53_), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n78_), .c(new_n72_), .O(z02));
  nand2  g030(.a(x22), .b(x21), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n65_), .c(new_n54_), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(x19), .O(new_n85_));
  inv1   g034(.a(x22), .O(new_n86_));
  nor2   g035(.a(new_n74_), .b(new_n86_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  inv1   g037(.a(x12), .O(new_n89_));
  aoi21  g038(.a(new_n60_), .b(new_n89_), .c(x18), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(new_n88_), .c(new_n53_), .O(z03));
  oai21  g040(.a(new_n60_), .b(new_n69_), .c(x11), .O(new_n92_));
  inv1   g041(.a(x23), .O(new_n93_));
  nor2   g042(.a(new_n57_), .b(new_n93_), .O(new_n94_));
  nor2   g043(.a(x23), .b(x22), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n73_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n66_), .O(new_n97_));
  oai21  g046(.a(new_n97_), .b(new_n94_), .c(new_n92_), .O(new_n98_));
  nand3  g047(.a(x23), .b(x20), .c(x16), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(x18), .c(new_n63_), .O(new_n101_));
  nand3  g050(.a(new_n55_), .b(x16), .c(x14), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(x11), .c(new_n83_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x23), .O(new_n104_));
  nand2  g053(.a(new_n60_), .b(new_n52_), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n104_), .c(new_n101_), .d(new_n98_), .O(z04));
  inv1   g055(.a(x24), .O(new_n107_));
  nand2  g056(.a(new_n96_), .b(new_n55_), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n74_), .c(new_n107_), .O(new_n109_));
  nor2   g058(.a(x24), .b(x23), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n83_), .c(new_n74_), .O(new_n111_));
  inv1   g060(.a(new_n111_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n109_), .c(x16), .O(new_n113_));
  inv1   g062(.a(x10), .O(new_n114_));
  aoi21  g063(.a(new_n60_), .b(new_n114_), .c(x18), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n113_), .c(new_n53_), .O(z05));
  inv1   g065(.a(x25), .O(new_n117_));
  nand2  g066(.a(new_n110_), .b(new_n83_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n55_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n74_), .c(new_n117_), .O(new_n120_));
  nor2   g069(.a(x25), .b(x24), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n95_), .c(new_n75_), .d(new_n57_), .O(new_n122_));
  inv1   g071(.a(new_n122_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n120_), .c(x16), .O(new_n124_));
  inv1   g073(.a(x09), .O(new_n125_));
  aoi21  g074(.a(new_n60_), .b(new_n125_), .c(x18), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n124_), .c(new_n53_), .O(z06));
  inv1   g076(.a(x26), .O(new_n128_));
  nor2   g077(.a(new_n123_), .b(new_n128_), .O(new_n129_));
  nor3   g078(.a(x26), .b(x25), .c(x24), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n95_), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n76_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n129_), .c(x16), .O(new_n133_));
  nor2   g082(.a(x16), .b(x08), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n53_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n133_), .c(new_n72_), .O(z07));
  inv1   g085(.a(x27), .O(new_n137_));
  nand3  g086(.a(new_n83_), .b(new_n57_), .c(new_n65_), .O(new_n138_));
  nor3   g087(.a(x27), .b(x26), .c(x25), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n110_), .O(new_n140_));
  oai22  g089(.a(new_n140_), .b(new_n138_), .c(new_n132_), .d(new_n137_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(x16), .O(new_n142_));
  nor2   g091(.a(x16), .b(x07), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n53_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n142_), .c(new_n72_), .O(z08));
  inv1   g094(.a(x28), .O(new_n146_));
  nor2   g095(.a(x27), .b(x26), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n121_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n96_), .c(new_n55_), .O(new_n149_));
  aoi21  g098(.a(new_n149_), .b(new_n74_), .c(new_n146_), .O(new_n150_));
  nor2   g099(.a(x28), .b(x27), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n121_), .c(new_n128_), .d(new_n93_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n138_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n150_), .c(x16), .O(new_n154_));
  inv1   g103(.a(x06), .O(new_n155_));
  aoi21  g104(.a(new_n60_), .b(new_n155_), .c(x18), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n154_), .c(new_n53_), .O(z09));
  inv1   g106(.a(x29), .O(new_n158_));
  nor3   g107(.a(x29), .b(x28), .c(x27), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n130_), .c(new_n97_), .O(new_n160_));
  oai21  g109(.a(new_n153_), .b(new_n158_), .c(new_n160_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  nor2   g111(.a(x16), .b(x05), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n53_), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n162_), .c(new_n72_), .O(z10));
  nand2  g114(.a(new_n160_), .b(x30), .O(new_n166_));
  nand3  g115(.a(new_n95_), .b(new_n74_), .c(new_n73_), .O(new_n167_));
  nor2   g116(.a(x30), .b(x29), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n151_), .c(new_n130_), .O(new_n169_));
  or2    g118(.a(new_n169_), .b(new_n167_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n166_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  nor2   g121(.a(x16), .b(x04), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n53_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n172_), .c(new_n72_), .O(z11));
  oai21  g124(.a(new_n169_), .b(new_n167_), .c(x31), .O(new_n176_));
  nor2   g125(.a(x29), .b(x28), .O(new_n177_));
  nor2   g126(.a(x31), .b(x30), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(new_n177_), .c(new_n139_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n111_), .c(new_n176_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(x16), .O(new_n181_));
  nor2   g130(.a(x16), .b(x03), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n53_), .O(new_n183_));
  nand3  g132(.a(new_n183_), .b(new_n181_), .c(new_n72_), .O(z12));
  oai21  g133(.a(new_n179_), .b(new_n111_), .c(x32), .O(new_n185_));
  nor2   g134(.a(x26), .b(x25), .O(new_n186_));
  nor2   g135(.a(x32), .b(x31), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n168_), .O(new_n188_));
  inv1   g137(.a(new_n188_), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n151_), .c(new_n186_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n111_), .c(new_n185_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x16), .O(new_n192_));
  nor2   g141(.a(x16), .b(x02), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n53_), .O(new_n194_));
  nand3  g143(.a(new_n194_), .b(new_n192_), .c(new_n72_), .O(z13));
  inv1   g144(.a(x33), .O(new_n196_));
  nand4  g145(.a(new_n186_), .b(new_n95_), .c(new_n107_), .d(new_n73_), .O(new_n197_));
  inv1   g146(.a(x30), .O(new_n198_));
  nand4  g147(.a(new_n187_), .b(new_n177_), .c(new_n198_), .d(new_n137_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n197_), .c(new_n55_), .O(new_n200_));
  aoi21  g149(.a(new_n200_), .b(new_n74_), .c(new_n196_), .O(new_n201_));
  nand2  g150(.a(new_n177_), .b(new_n147_), .O(new_n202_));
  nor2   g151(.a(x33), .b(x32), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n178_), .O(new_n204_));
  nor3   g153(.a(new_n204_), .b(new_n202_), .c(new_n122_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n201_), .c(x16), .O(new_n206_));
  inv1   g155(.a(x01), .O(new_n207_));
  aoi21  g156(.a(new_n60_), .b(new_n207_), .c(x18), .O(new_n208_));
  aoi21  g157(.a(new_n208_), .b(new_n206_), .c(new_n53_), .O(z14));
  inv1   g158(.a(x34), .O(new_n210_));
  nand4  g159(.a(new_n203_), .b(new_n178_), .c(new_n177_), .d(new_n137_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n197_), .c(new_n55_), .O(new_n212_));
  aoi21  g161(.a(new_n212_), .b(new_n74_), .c(new_n210_), .O(new_n213_));
  inv1   g162(.a(x32), .O(new_n214_));
  nand4  g163(.a(new_n178_), .b(new_n210_), .c(new_n196_), .d(new_n214_), .O(new_n215_));
  nor3   g164(.a(new_n215_), .b(new_n202_), .c(new_n122_), .O(new_n216_));
  oai21  g165(.a(new_n216_), .b(new_n213_), .c(x16), .O(new_n217_));
  inv1   g166(.a(x00), .O(new_n218_));
  aoi21  g167(.a(new_n60_), .b(new_n218_), .c(x18), .O(new_n219_));
  aoi21  g168(.a(new_n219_), .b(new_n217_), .c(new_n53_), .O(z15));
endmodule


