// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_, new_n226_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand3  g005(.a(new_n56_), .b(new_n52_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  oai21  g008(.a(x18), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(z00));
  inv1   g010(.a(x14), .O(new_n62_));
  oai21  g011(.a(x18), .b(new_n62_), .c(new_n59_), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  inv1   g013(.a(x17), .O(new_n65_));
  inv1   g014(.a(x19), .O(new_n66_));
  nand3  g015(.a(new_n64_), .b(new_n66_), .c(new_n65_), .O(new_n67_));
  oai21  g016(.a(new_n54_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n52_), .c(x16), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n63_), .O(z01));
  nand2  g019(.a(x20), .b(new_n52_), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n66_), .c(new_n65_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n54_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n59_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n59_), .c(new_n78_), .O(z02));
  inv1   g028(.a(x12), .O(new_n80_));
  oai21  g029(.a(x18), .b(new_n80_), .c(new_n59_), .O(new_n81_));
  nand2  g030(.a(x22), .b(x21), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n64_), .c(new_n65_), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(x19), .O(new_n85_));
  inv1   g034(.a(x22), .O(new_n86_));
  inv1   g035(.a(new_n67_), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n85_), .c(new_n52_), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n59_), .c(new_n81_), .O(z03));
  nor3   g039(.a(x22), .b(x21), .c(x20), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(x18), .c(new_n54_), .O(new_n92_));
  inv1   g041(.a(x21), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n87_), .c(new_n93_), .O(new_n95_));
  inv1   g044(.a(new_n95_), .O(new_n96_));
  aoi21  g045(.a(new_n92_), .b(x23), .c(new_n96_), .O(new_n97_));
  inv1   g046(.a(x11), .O(new_n98_));
  aoi21  g047(.a(new_n59_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n59_), .c(new_n99_), .O(z04));
  inv1   g049(.a(x24), .O(new_n101_));
  nand2  g050(.a(new_n94_), .b(new_n73_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n52_), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n54_), .c(new_n101_), .O(new_n104_));
  nor2   g053(.a(x24), .b(x23), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n83_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n67_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n104_), .c(x16), .O(new_n108_));
  inv1   g057(.a(x10), .O(new_n109_));
  aoi21  g058(.a(new_n59_), .b(new_n109_), .c(x18), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n108_), .O(z05));
  inv1   g060(.a(x25), .O(new_n112_));
  nand3  g061(.a(new_n105_), .b(new_n73_), .c(new_n86_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n52_), .c(new_n55_), .O(new_n114_));
  nor2   g063(.a(x25), .b(x24), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n94_), .c(new_n73_), .d(new_n54_), .O(new_n116_));
  oai21  g065(.a(new_n114_), .b(new_n112_), .c(new_n116_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(x16), .O(new_n118_));
  inv1   g067(.a(x09), .O(new_n119_));
  aoi21  g068(.a(new_n59_), .b(new_n119_), .c(x18), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z06));
  inv1   g070(.a(x26), .O(new_n122_));
  nor3   g071(.a(x25), .b(x24), .c(x23), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n91_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n52_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n54_), .c(new_n122_), .O(new_n126_));
  nor3   g075(.a(x26), .b(x25), .c(x24), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n94_), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n74_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n126_), .c(x16), .O(new_n130_));
  inv1   g079(.a(x08), .O(new_n131_));
  aoi21  g080(.a(new_n59_), .b(new_n131_), .c(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n130_), .O(z07));
  inv1   g082(.a(x27), .O(new_n134_));
  nor2   g083(.a(x26), .b(x25), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n105_), .c(new_n91_), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n52_), .c(new_n55_), .O(new_n137_));
  nor3   g086(.a(x27), .b(x26), .c(x25), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n105_), .c(new_n91_), .d(new_n54_), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n134_), .c(new_n139_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x16), .O(new_n141_));
  inv1   g090(.a(x07), .O(new_n142_));
  aoi21  g091(.a(new_n59_), .b(new_n142_), .c(x18), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n141_), .O(z08));
  inv1   g093(.a(x06), .O(new_n145_));
  oai21  g094(.a(x18), .b(new_n145_), .c(new_n59_), .O(new_n146_));
  inv1   g095(.a(x28), .O(new_n147_));
  nor2   g096(.a(x27), .b(x26), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n115_), .c(new_n94_), .d(new_n93_), .O(new_n149_));
  aoi21  g098(.a(new_n149_), .b(new_n66_), .c(new_n67_), .O(new_n150_));
  nor3   g099(.a(x28), .b(x27), .c(x26), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n123_), .c(new_n91_), .d(new_n54_), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n147_), .c(new_n152_), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n52_), .c(x16), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n146_), .O(z09));
  inv1   g104(.a(x29), .O(new_n156_));
  nand2  g105(.a(new_n151_), .b(new_n115_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n102_), .c(new_n52_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n54_), .c(new_n156_), .O(new_n159_));
  nor2   g108(.a(x29), .b(x28), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n127_), .c(new_n134_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n95_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n159_), .c(x16), .O(new_n163_));
  inv1   g112(.a(x05), .O(new_n164_));
  aoi21  g113(.a(new_n59_), .b(new_n164_), .c(x18), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z10));
  inv1   g115(.a(x30), .O(new_n167_));
  nand3  g116(.a(new_n160_), .b(new_n135_), .c(new_n134_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n113_), .c(new_n52_), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n54_), .c(new_n167_), .O(new_n170_));
  nor2   g119(.a(x28), .b(x27), .O(new_n171_));
  nor2   g120(.a(x30), .b(x29), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n171_), .c(new_n127_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n95_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n170_), .c(x16), .O(new_n175_));
  inv1   g124(.a(x04), .O(new_n176_));
  aoi21  g125(.a(new_n59_), .b(new_n176_), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z11));
  inv1   g127(.a(x03), .O(new_n179_));
  oai21  g128(.a(x18), .b(new_n179_), .c(new_n59_), .O(new_n180_));
  inv1   g129(.a(x31), .O(new_n181_));
  inv1   g130(.a(x23), .O(new_n182_));
  nand3  g131(.a(new_n115_), .b(new_n83_), .c(new_n182_), .O(new_n183_));
  nand3  g132(.a(new_n172_), .b(new_n148_), .c(new_n147_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n183_), .c(new_n66_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n87_), .c(new_n181_), .O(new_n186_));
  nor2   g135(.a(x31), .b(x30), .O(new_n187_));
  nand3  g136(.a(new_n187_), .b(new_n160_), .c(new_n138_), .O(new_n188_));
  nor3   g137(.a(new_n188_), .b(new_n106_), .c(new_n67_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n186_), .c(new_n52_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n59_), .c(new_n180_), .O(z12));
  inv1   g140(.a(x02), .O(new_n192_));
  oai21  g141(.a(x18), .b(new_n192_), .c(new_n59_), .O(new_n193_));
  inv1   g142(.a(x32), .O(new_n194_));
  nor3   g143(.a(x31), .b(x30), .c(x29), .O(new_n195_));
  nand4  g144(.a(new_n195_), .b(new_n151_), .c(new_n123_), .d(new_n83_), .O(new_n196_));
  aoi21  g145(.a(new_n196_), .b(new_n66_), .c(new_n67_), .O(new_n197_));
  nor2   g146(.a(x32), .b(x31), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n172_), .O(new_n199_));
  inv1   g148(.a(new_n199_), .O(new_n200_));
  nand4  g149(.a(new_n200_), .b(new_n171_), .c(new_n135_), .d(new_n107_), .O(new_n201_));
  oai21  g150(.a(new_n197_), .b(new_n194_), .c(new_n201_), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n52_), .c(x16), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n193_), .O(z13));
  inv1   g153(.a(x33), .O(new_n205_));
  nand4  g154(.a(new_n198_), .b(new_n172_), .c(new_n171_), .d(new_n122_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n124_), .c(new_n52_), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n54_), .c(new_n205_), .O(new_n208_));
  nand2  g157(.a(new_n160_), .b(new_n148_), .O(new_n209_));
  nor2   g158(.a(x33), .b(x32), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n187_), .O(new_n211_));
  nor3   g160(.a(new_n211_), .b(new_n209_), .c(new_n116_), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n208_), .c(x16), .O(new_n213_));
  inv1   g162(.a(x01), .O(new_n214_));
  aoi21  g163(.a(new_n59_), .b(new_n214_), .c(x18), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n213_), .O(z14));
  inv1   g165(.a(x00), .O(new_n217_));
  oai21  g166(.a(x18), .b(new_n217_), .c(new_n59_), .O(new_n218_));
  inv1   g167(.a(x34), .O(new_n219_));
  nand4  g168(.a(new_n135_), .b(new_n94_), .c(new_n101_), .d(new_n93_), .O(new_n220_));
  nand4  g169(.a(new_n210_), .b(new_n187_), .c(new_n160_), .d(new_n134_), .O(new_n221_));
  oai21  g170(.a(new_n221_), .b(new_n220_), .c(new_n66_), .O(new_n222_));
  aoi21  g171(.a(new_n222_), .b(new_n87_), .c(new_n219_), .O(new_n223_));
  nand4  g172(.a(new_n187_), .b(new_n219_), .c(new_n205_), .d(new_n194_), .O(new_n224_));
  nor3   g173(.a(new_n224_), .b(new_n209_), .c(new_n116_), .O(new_n225_));
  oai21  g174(.a(new_n225_), .b(new_n223_), .c(new_n52_), .O(new_n226_));
  oai21  g175(.a(new_n226_), .b(new_n59_), .c(new_n218_), .O(z15));
endmodule


