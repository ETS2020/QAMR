// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x33), .O(new_n57_));
  nor2   g006(.a(new_n57_), .b(x29), .O(new_n58_));
  inv1   g007(.a(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(x18), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nor2   g014(.a(x20), .b(x19), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n52_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n65_), .c(x16), .O(new_n69_));
  inv1   g018(.a(x14), .O(new_n70_));
  inv1   g019(.a(x16), .O(new_n71_));
  aoi21  g020(.a(new_n71_), .b(new_n70_), .c(x18), .O(new_n72_));
  aoi21  g021(.a(new_n72_), .b(new_n69_), .c(new_n58_), .O(z01));
  inv1   g022(.a(x21), .O(new_n74_));
  nor2   g023(.a(x21), .b(x20), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n55_), .O(new_n76_));
  oai21  g025(.a(new_n68_), .b(new_n74_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(x16), .O(new_n78_));
  nor2   g027(.a(x16), .b(x13), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n58_), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n78_), .c(new_n60_), .O(z02));
  inv1   g030(.a(x22), .O(new_n82_));
  inv1   g031(.a(new_n76_), .O(new_n83_));
  nor2   g032(.a(x22), .b(x21), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n55_), .c(new_n64_), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(x16), .O(new_n87_));
  nor2   g036(.a(x16), .b(x12), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n58_), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n87_), .c(new_n60_), .O(z03));
  nand2  g039(.a(new_n85_), .b(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n68_), .c(new_n74_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x16), .O(new_n95_));
  nor2   g044(.a(x16), .b(x11), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n58_), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n95_), .c(new_n60_), .O(z04));
  inv1   g047(.a(x24), .O(new_n99_));
  inv1   g048(.a(new_n93_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n84_), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n67_), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n100_), .b(new_n99_), .c(new_n104_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g055(.a(x10), .O(new_n107_));
  aoi21  g056(.a(new_n71_), .b(new_n107_), .c(x18), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n106_), .c(new_n58_), .O(z05));
  inv1   g058(.a(x25), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n92_), .c(new_n83_), .O(new_n112_));
  oai21  g061(.a(new_n103_), .b(new_n110_), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nor2   g063(.a(x16), .b(x09), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n58_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n114_), .c(new_n60_), .O(z06));
  nand3  g066(.a(new_n112_), .b(x26), .c(x16), .O(new_n118_));
  inv1   g067(.a(new_n118_), .O(new_n119_));
  inv1   g068(.a(x18), .O(new_n120_));
  oai21  g069(.a(x16), .b(x08), .c(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n119_), .c(new_n59_), .O(new_n122_));
  inv1   g071(.a(x23), .O(new_n123_));
  nor2   g072(.a(new_n58_), .b(x26), .O(new_n124_));
  nand4  g073(.a(new_n124_), .b(new_n110_), .c(new_n99_), .d(new_n123_), .O(new_n125_));
  nor3   g074(.a(new_n125_), .b(x22), .c(x21), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n64_), .c(new_n53_), .d(new_n52_), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n71_), .c(new_n122_), .O(z07));
  nor3   g077(.a(x26), .b(x25), .c(x24), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n92_), .c(new_n83_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x27), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n71_), .O(new_n132_));
  oai21  g081(.a(x16), .b(x07), .c(new_n120_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n132_), .c(new_n59_), .O(new_n134_));
  inv1   g083(.a(x26), .O(new_n135_));
  inv1   g084(.a(x27), .O(new_n136_));
  nand4  g085(.a(new_n59_), .b(new_n136_), .c(new_n135_), .d(new_n110_), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(x24), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n123_), .c(new_n82_), .d(new_n74_), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(x20), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n134_), .O(z08));
  inv1   g091(.a(x28), .O(new_n143_));
  inv1   g092(.a(x29), .O(new_n144_));
  nand2  g093(.a(x33), .b(new_n144_), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n143_), .c(new_n136_), .d(new_n135_), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(x25), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n99_), .c(new_n123_), .d(new_n82_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(x21), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n64_), .c(new_n53_), .d(new_n52_), .O(new_n150_));
  nor3   g099(.a(x27), .b(x26), .c(x25), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n101_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n85_), .c(x28), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(x16), .O(new_n155_));
  inv1   g104(.a(x06), .O(new_n156_));
  aoi21  g105(.a(new_n71_), .b(new_n156_), .c(new_n58_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n155_), .c(new_n60_), .O(z09));
  oai21  g107(.a(x16), .b(x05), .c(new_n120_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n59_), .O(new_n160_));
  nor2   g109(.a(new_n144_), .b(new_n143_), .O(new_n161_));
  nor3   g110(.a(x33), .b(x29), .c(x28), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n161_), .c(new_n136_), .O(new_n163_));
  nand2  g112(.a(x29), .b(x27), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n163_), .c(x26), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n110_), .c(new_n99_), .d(new_n123_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(x22), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n74_), .c(new_n64_), .d(new_n53_), .O(new_n168_));
  nand2  g117(.a(new_n130_), .b(x29), .O(new_n169_));
  oai21  g118(.a(new_n168_), .b(x17), .c(new_n169_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n160_), .O(z10));
  nand4  g121(.a(new_n84_), .b(new_n55_), .c(new_n64_), .d(x16), .O(new_n173_));
  nor2   g122(.a(x27), .b(x26), .O(new_n174_));
  nor2   g123(.a(x30), .b(x28), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n111_), .d(new_n123_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n173_), .c(new_n57_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n144_), .O(new_n178_));
  nor2   g127(.a(x29), .b(x28), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n129_), .c(new_n100_), .d(new_n136_), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(x30), .c(x16), .O(new_n181_));
  inv1   g130(.a(x04), .O(new_n182_));
  nand2  g131(.a(new_n71_), .b(new_n182_), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n181_), .c(new_n178_), .d(new_n60_), .O(z11));
  oai21  g133(.a(x16), .b(x03), .c(new_n120_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n59_), .O(new_n186_));
  inv1   g135(.a(x31), .O(new_n187_));
  nand3  g136(.a(new_n175_), .b(new_n129_), .c(new_n136_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n93_), .c(new_n57_), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n144_), .c(new_n187_), .O(new_n190_));
  nand4  g139(.a(new_n143_), .b(new_n136_), .c(new_n135_), .d(new_n110_), .O(new_n191_));
  inv1   g140(.a(new_n191_), .O(new_n192_));
  nor2   g141(.a(x30), .b(x29), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n192_), .c(new_n57_), .d(new_n187_), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n104_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n190_), .c(x16), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n186_), .O(z12));
  nand4  g146(.a(new_n92_), .b(new_n75_), .c(new_n55_), .d(x16), .O(new_n198_));
  nor2   g147(.a(x32), .b(x31), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n175_), .c(new_n174_), .d(new_n111_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n198_), .c(new_n57_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n144_), .O(new_n202_));
  nor2   g151(.a(x31), .b(x30), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n179_), .c(new_n151_), .d(new_n103_), .O(new_n204_));
  nand3  g153(.a(new_n204_), .b(x32), .c(x16), .O(new_n205_));
  inv1   g154(.a(x02), .O(new_n206_));
  nand2  g155(.a(new_n71_), .b(new_n206_), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n205_), .c(new_n202_), .d(new_n60_), .O(z13));
  nand4  g157(.a(new_n199_), .b(new_n193_), .c(new_n192_), .d(new_n103_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n57_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(x16), .O(new_n211_));
  inv1   g160(.a(x01), .O(new_n212_));
  aoi21  g161(.a(new_n71_), .b(new_n212_), .c(new_n58_), .O(new_n213_));
  nand3  g162(.a(new_n213_), .b(new_n211_), .c(new_n60_), .O(z14));
  inv1   g163(.a(new_n102_), .O(new_n215_));
  nand4  g164(.a(new_n215_), .b(new_n66_), .c(new_n52_), .d(x16), .O(new_n216_));
  nor2   g165(.a(x34), .b(x32), .O(new_n217_));
  nand3  g166(.a(new_n217_), .b(new_n203_), .c(new_n192_), .O(new_n218_));
  oai21  g167(.a(new_n218_), .b(new_n216_), .c(new_n57_), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(new_n144_), .O(new_n220_));
  nand3  g169(.a(new_n209_), .b(x34), .c(x16), .O(new_n221_));
  inv1   g170(.a(x00), .O(new_n222_));
  nand2  g171(.a(new_n71_), .b(new_n222_), .O(new_n223_));
  nand4  g172(.a(new_n223_), .b(new_n221_), .c(new_n220_), .d(new_n60_), .O(z15));
endmodule


