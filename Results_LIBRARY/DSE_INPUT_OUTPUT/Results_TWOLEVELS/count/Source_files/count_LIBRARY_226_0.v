// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n147_, new_n148_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_;
  nor2   g000(.a(x25), .b(x20), .O(new_n52_));
  inv1   g001(.a(new_n52_), .O(new_n53_));
  nor2   g002(.a(x16), .b(x15), .O(new_n54_));
  inv1   g003(.a(x18), .O(new_n55_));
  nand3  g004(.a(x19), .b(x17), .c(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n54_), .c(new_n53_), .O(new_n58_));
  inv1   g007(.a(x17), .O(new_n59_));
  inv1   g008(.a(x19), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  inv1   g010(.a(x25), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g012(.a(new_n63_), .b(new_n60_), .c(new_n59_), .d(x16), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n58_), .O(z00));
  nand2  g014(.a(new_n53_), .b(x18), .O(new_n66_));
  nor2   g015(.a(x19), .b(x17), .O(new_n67_));
  nand4  g016(.a(x25), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n68_));
  oai21  g017(.a(new_n67_), .b(new_n61_), .c(new_n68_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  nor2   g019(.a(x16), .b(x14), .O(new_n71_));
  nor2   g020(.a(new_n71_), .b(new_n52_), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z01));
  inv1   g022(.a(x21), .O(new_n74_));
  nand4  g023(.a(x25), .b(new_n74_), .c(new_n61_), .d(new_n60_), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(x17), .O(new_n76_));
  nor2   g025(.a(x20), .b(x19), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n59_), .c(new_n74_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  nor2   g028(.a(x16), .b(x13), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n52_), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n79_), .c(new_n66_), .O(z02));
  inv1   g031(.a(x22), .O(new_n83_));
  nand4  g032(.a(x25), .b(new_n83_), .c(new_n74_), .d(new_n61_), .O(new_n84_));
  nor3   g033(.a(new_n84_), .b(x19), .c(x17), .O(new_n85_));
  nand3  g034(.a(new_n67_), .b(new_n74_), .c(new_n61_), .O(new_n86_));
  and2   g035(.a(new_n86_), .b(x22), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  nor2   g037(.a(x16), .b(x12), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n52_), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n88_), .c(new_n66_), .O(z03));
  oai21  g040(.a(x16), .b(x11), .c(new_n55_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n53_), .O(new_n93_));
  nor3   g042(.a(x23), .b(x22), .c(x21), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n95_));
  nand3  g044(.a(new_n67_), .b(new_n83_), .c(new_n74_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x23), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n62_), .O(new_n98_));
  inv1   g047(.a(x23), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n61_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n98_), .c(x16), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n93_), .O(z04));
  oai21  g051(.a(x16), .b(x10), .c(new_n55_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n53_), .O(new_n104_));
  inv1   g053(.a(x24), .O(new_n105_));
  nand2  g054(.a(new_n60_), .b(new_n59_), .O(new_n106_));
  nand3  g055(.a(new_n99_), .b(new_n83_), .c(new_n74_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n106_), .c(x25), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n61_), .c(new_n105_), .O(new_n109_));
  nand4  g058(.a(x25), .b(new_n105_), .c(new_n99_), .d(new_n83_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n86_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n109_), .c(x16), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n104_), .O(z05));
  inv1   g062(.a(x09), .O(new_n114_));
  inv1   g063(.a(x16), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n114_), .c(x18), .O(new_n116_));
  nand4  g065(.a(new_n105_), .b(new_n99_), .c(new_n83_), .d(new_n74_), .O(new_n117_));
  inv1   g066(.a(new_n117_), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(x25), .c(x16), .O(new_n120_));
  oai21  g069(.a(new_n116_), .b(new_n52_), .c(new_n120_), .O(z06));
  inv1   g070(.a(x08), .O(new_n122_));
  nand2  g071(.a(new_n115_), .b(new_n122_), .O(new_n123_));
  aoi21  g072(.a(x26), .b(x16), .c(x18), .O(new_n124_));
  aoi21  g073(.a(new_n124_), .b(new_n123_), .c(new_n52_), .O(z07));
  inv1   g074(.a(x07), .O(new_n126_));
  nand2  g075(.a(new_n115_), .b(new_n126_), .O(new_n127_));
  aoi21  g076(.a(x27), .b(x16), .c(x18), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n127_), .c(new_n52_), .O(z08));
  inv1   g078(.a(x06), .O(new_n130_));
  nand2  g079(.a(new_n115_), .b(new_n130_), .O(new_n131_));
  nand2  g080(.a(x28), .b(x16), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n131_), .c(new_n53_), .d(new_n55_), .O(z09));
  inv1   g082(.a(x05), .O(new_n134_));
  nand2  g083(.a(new_n115_), .b(new_n134_), .O(new_n135_));
  nand2  g084(.a(x29), .b(x16), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n135_), .c(new_n53_), .d(new_n55_), .O(z10));
  inv1   g086(.a(x04), .O(new_n138_));
  nand2  g087(.a(new_n115_), .b(new_n138_), .O(new_n139_));
  aoi21  g088(.a(x30), .b(x16), .c(x18), .O(new_n140_));
  aoi21  g089(.a(new_n140_), .b(new_n139_), .c(new_n52_), .O(z11));
  inv1   g090(.a(x03), .O(new_n142_));
  nand2  g091(.a(new_n115_), .b(new_n142_), .O(new_n143_));
  nand2  g092(.a(x31), .b(x16), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n143_), .c(new_n53_), .d(new_n55_), .O(z12));
  inv1   g094(.a(x02), .O(new_n146_));
  nand2  g095(.a(new_n115_), .b(new_n146_), .O(new_n147_));
  aoi21  g096(.a(x32), .b(x16), .c(x18), .O(new_n148_));
  aoi21  g097(.a(new_n148_), .b(new_n147_), .c(new_n52_), .O(z13));
  inv1   g098(.a(x01), .O(new_n150_));
  nand2  g099(.a(new_n115_), .b(new_n150_), .O(new_n151_));
  aoi21  g100(.a(x33), .b(x16), .c(x18), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n151_), .c(new_n52_), .O(z14));
  inv1   g102(.a(x00), .O(new_n154_));
  nand2  g103(.a(new_n115_), .b(new_n154_), .O(new_n155_));
  nand2  g104(.a(x34), .b(x16), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n155_), .c(new_n53_), .d(new_n55_), .O(z15));
endmodule


