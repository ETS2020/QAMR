// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x03), .O(new_n52_));
  inv1   g001(.a(x10), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x16), .O(new_n59_));
  inv1   g008(.a(x15), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n60_), .c(x18), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n59_), .c(new_n54_), .O(z00));
  oai21  g012(.a(new_n53_), .b(new_n52_), .c(x18), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(new_n58_), .b(new_n65_), .O(new_n66_));
  nand3  g015(.a(new_n65_), .b(new_n56_), .c(new_n55_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n66_), .c(x16), .O(new_n69_));
  nor2   g018(.a(x16), .b(x14), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n54_), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n69_), .c(new_n64_), .O(z01));
  inv1   g021(.a(x21), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n58_), .O(new_n75_));
  oai21  g024(.a(new_n68_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  nor2   g026(.a(x16), .b(x13), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n54_), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n77_), .c(new_n64_), .O(z02));
  nand2  g029(.a(x22), .b(x21), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n65_), .c(new_n55_), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n81_), .c(x19), .O(new_n84_));
  and2   g033(.a(new_n67_), .b(x22), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n84_), .c(x16), .O(new_n86_));
  inv1   g035(.a(x12), .O(new_n87_));
  aoi21  g036(.a(new_n61_), .b(new_n87_), .c(x18), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n54_), .O(z03));
  inv1   g038(.a(x23), .O(new_n90_));
  oai21  g039(.a(x22), .b(x21), .c(new_n56_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n68_), .c(new_n90_), .O(new_n92_));
  nor2   g041(.a(x23), .b(x22), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n73_), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n67_), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n92_), .c(x16), .O(new_n96_));
  inv1   g045(.a(x11), .O(new_n97_));
  aoi21  g046(.a(new_n61_), .b(new_n97_), .c(x18), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n96_), .c(new_n54_), .O(z04));
  aoi21  g048(.a(x16), .b(new_n52_), .c(new_n53_), .O(new_n100_));
  aoi21  g049(.a(new_n93_), .b(new_n73_), .c(x19), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n67_), .c(x24), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n82_), .c(new_n68_), .O(new_n104_));
  and2   g053(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  inv1   g054(.a(x18), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x16), .O(new_n107_));
  aoi22  g056(.a(new_n107_), .b(new_n53_), .c(x18), .d(new_n52_), .O(new_n108_));
  oai21  g057(.a(new_n105_), .b(new_n100_), .c(new_n108_), .O(z05));
  nand2  g058(.a(new_n104_), .b(x25), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n93_), .c(new_n74_), .d(new_n58_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nor2   g063(.a(x16), .b(x09), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n54_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n114_), .c(new_n64_), .O(z06));
  nand2  g066(.a(new_n112_), .b(x26), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n93_), .c(new_n74_), .d(new_n58_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  nor2   g071(.a(x16), .b(x08), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n54_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(new_n64_), .O(z07));
  nand2  g074(.a(new_n120_), .b(x27), .O(new_n126_));
  nand3  g075(.a(new_n82_), .b(new_n58_), .c(new_n65_), .O(new_n127_));
  inv1   g076(.a(x25), .O(new_n128_));
  nor2   g077(.a(x27), .b(x26), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n103_), .c(new_n128_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n127_), .c(new_n126_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(x16), .O(new_n132_));
  nor2   g081(.a(x16), .b(x07), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n54_), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n132_), .c(new_n64_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  nand2  g085(.a(new_n129_), .b(new_n111_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n94_), .c(new_n56_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n68_), .c(new_n136_), .O(new_n139_));
  inv1   g088(.a(x26), .O(new_n140_));
  nor2   g089(.a(x28), .b(x27), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n111_), .c(new_n140_), .d(new_n90_), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n127_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n139_), .c(x16), .O(new_n144_));
  inv1   g093(.a(x06), .O(new_n145_));
  aoi21  g094(.a(new_n61_), .b(new_n145_), .c(x18), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n144_), .c(new_n54_), .O(z09));
  inv1   g096(.a(x29), .O(new_n148_));
  nor3   g097(.a(x29), .b(x28), .c(x27), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n119_), .c(new_n95_), .O(new_n150_));
  oai21  g099(.a(new_n143_), .b(new_n148_), .c(new_n150_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  nor2   g101(.a(x16), .b(x05), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n54_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(new_n64_), .O(z10));
  nand2  g104(.a(new_n150_), .b(x30), .O(new_n156_));
  nor2   g105(.a(x30), .b(x29), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n141_), .c(new_n119_), .d(new_n95_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  nor2   g109(.a(x16), .b(x04), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n54_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n160_), .c(new_n64_), .O(z11));
  oai21  g112(.a(new_n61_), .b(x10), .c(x03), .O(new_n164_));
  inv1   g113(.a(x31), .O(new_n165_));
  nand3  g114(.a(new_n111_), .b(new_n82_), .c(new_n90_), .O(new_n166_));
  nand3  g115(.a(new_n157_), .b(new_n129_), .c(new_n136_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n166_), .c(new_n56_), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n68_), .c(new_n165_), .O(new_n169_));
  nor2   g118(.a(x29), .b(x28), .O(new_n170_));
  nor2   g119(.a(x31), .b(x30), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n170_), .c(new_n129_), .d(new_n128_), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n104_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n169_), .c(new_n164_), .O(new_n174_));
  aoi22  g123(.a(new_n107_), .b(new_n52_), .c(x18), .d(new_n53_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n174_), .O(z12));
  inv1   g125(.a(x32), .O(new_n177_));
  nand4  g126(.a(new_n171_), .b(new_n141_), .c(new_n148_), .d(new_n140_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n166_), .c(new_n56_), .O(new_n179_));
  aoi21  g128(.a(new_n179_), .b(new_n68_), .c(new_n177_), .O(new_n180_));
  nor2   g129(.a(x26), .b(x25), .O(new_n181_));
  nor2   g130(.a(x32), .b(x31), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n157_), .c(new_n141_), .d(new_n181_), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n104_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n180_), .c(x16), .O(new_n185_));
  inv1   g134(.a(x02), .O(new_n186_));
  aoi21  g135(.a(new_n61_), .b(new_n186_), .c(x18), .O(new_n187_));
  aoi21  g136(.a(new_n187_), .b(new_n185_), .c(new_n54_), .O(z13));
  inv1   g137(.a(x33), .O(new_n189_));
  inv1   g138(.a(x24), .O(new_n190_));
  nand4  g139(.a(new_n181_), .b(new_n93_), .c(new_n190_), .d(new_n73_), .O(new_n191_));
  inv1   g140(.a(x27), .O(new_n192_));
  inv1   g141(.a(x30), .O(new_n193_));
  nand4  g142(.a(new_n182_), .b(new_n170_), .c(new_n193_), .d(new_n192_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n191_), .c(new_n56_), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n68_), .c(new_n189_), .O(new_n196_));
  nand2  g145(.a(new_n170_), .b(new_n129_), .O(new_n197_));
  nor2   g146(.a(x33), .b(x32), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n171_), .O(new_n199_));
  nor3   g148(.a(new_n199_), .b(new_n197_), .c(new_n112_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n196_), .c(x16), .O(new_n201_));
  inv1   g150(.a(x01), .O(new_n202_));
  aoi21  g151(.a(new_n61_), .b(new_n202_), .c(x18), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n201_), .c(new_n54_), .O(z14));
  inv1   g153(.a(x34), .O(new_n205_));
  nand4  g154(.a(new_n198_), .b(new_n171_), .c(new_n170_), .d(new_n192_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n191_), .c(new_n56_), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n68_), .c(new_n205_), .O(new_n208_));
  nand4  g157(.a(new_n171_), .b(new_n205_), .c(new_n189_), .d(new_n177_), .O(new_n209_));
  nor3   g158(.a(new_n209_), .b(new_n197_), .c(new_n112_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n208_), .c(x16), .O(new_n211_));
  inv1   g160(.a(x00), .O(new_n212_));
  aoi21  g161(.a(new_n61_), .b(new_n212_), .c(x18), .O(new_n213_));
  aoi21  g162(.a(new_n213_), .b(new_n211_), .c(new_n54_), .O(z15));
endmodule


