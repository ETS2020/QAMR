// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(new_n55_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x20), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n58_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  inv1   g017(.a(x07), .O(new_n69_));
  nand2  g018(.a(x18), .b(new_n69_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  inv1   g021(.a(new_n65_), .O(new_n73_));
  nand3  g022(.a(new_n55_), .b(new_n72_), .c(new_n64_), .O(new_n74_));
  oai21  g023(.a(new_n73_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  aoi21  g025(.a(new_n75_), .b(x16), .c(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(x18), .c(new_n70_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n72_), .O(new_n80_));
  nor2   g029(.a(x20), .b(x17), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(new_n83_));
  oai22  g032(.a(new_n83_), .b(x19), .c(new_n73_), .d(new_n79_), .O(new_n84_));
  nor2   g033(.a(x16), .b(x12), .O(new_n85_));
  aoi21  g034(.a(new_n84_), .b(x16), .c(new_n85_), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(x18), .c(new_n70_), .O(z03));
  oai21  g036(.a(new_n82_), .b(x19), .c(new_n73_), .O(new_n88_));
  nor2   g037(.a(x23), .b(x22), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n72_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n65_), .O(new_n91_));
  aoi21  g040(.a(new_n88_), .b(x23), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x11), .O(new_n93_));
  aoi21  g042(.a(new_n58_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n58_), .c(new_n94_), .O(z04));
  inv1   g044(.a(x24), .O(new_n96_));
  aoi21  g045(.a(new_n90_), .b(new_n53_), .c(new_n65_), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n82_), .O(new_n99_));
  oai22  g048(.a(new_n99_), .b(new_n65_), .c(new_n97_), .d(new_n96_), .O(new_n100_));
  nor2   g049(.a(x16), .b(x10), .O(new_n101_));
  aoi21  g050(.a(new_n100_), .b(x16), .c(new_n101_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(x18), .c(new_n70_), .O(z05));
  inv1   g052(.a(x25), .O(new_n104_));
  nand2  g053(.a(new_n99_), .b(new_n53_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n73_), .c(new_n104_), .O(new_n106_));
  nor2   g055(.a(x25), .b(x24), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n89_), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n74_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n106_), .c(x16), .O(new_n110_));
  inv1   g059(.a(x09), .O(new_n111_));
  aoi21  g060(.a(new_n58_), .b(new_n111_), .c(x18), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n110_), .O(z06));
  inv1   g062(.a(x26), .O(new_n114_));
  nor3   g063(.a(x25), .b(x24), .c(x23), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n82_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n53_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n73_), .c(new_n114_), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n89_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n74_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n118_), .c(x16), .O(new_n122_));
  inv1   g071(.a(x08), .O(new_n123_));
  aoi21  g072(.a(new_n58_), .b(new_n123_), .c(x18), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n122_), .O(z07));
  inv1   g074(.a(x27), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n89_), .c(new_n96_), .d(new_n72_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n53_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n73_), .c(new_n126_), .O(new_n130_));
  nand2  g079(.a(new_n82_), .b(new_n64_), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n98_), .O(new_n133_));
  nor3   g082(.a(new_n133_), .b(new_n131_), .c(new_n62_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n130_), .c(x16), .O(new_n135_));
  aoi21  g084(.a(new_n58_), .b(new_n69_), .c(x18), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n135_), .O(z08));
  inv1   g086(.a(x28), .O(new_n138_));
  nor3   g087(.a(x23), .b(x22), .c(x21), .O(new_n139_));
  nor2   g088(.a(x27), .b(x26), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n107_), .c(new_n139_), .O(new_n141_));
  aoi21  g090(.a(new_n141_), .b(new_n53_), .c(new_n65_), .O(new_n142_));
  nor2   g091(.a(new_n131_), .b(new_n62_), .O(new_n143_));
  nor3   g092(.a(x28), .b(x27), .c(x26), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n143_), .c(new_n115_), .O(new_n145_));
  oai21  g094(.a(new_n142_), .b(new_n138_), .c(new_n145_), .O(new_n146_));
  nor2   g095(.a(x16), .b(x06), .O(new_n147_));
  aoi21  g096(.a(new_n146_), .b(x16), .c(new_n147_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(x18), .c(new_n70_), .O(z09));
  inv1   g098(.a(x29), .O(new_n150_));
  nor2   g099(.a(x28), .b(x27), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n127_), .c(new_n98_), .d(new_n82_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n53_), .c(new_n65_), .O(new_n153_));
  nor3   g102(.a(x29), .b(x28), .c(x27), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n119_), .c(new_n91_), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n150_), .c(new_n155_), .O(new_n156_));
  nor2   g105(.a(x16), .b(x05), .O(new_n157_));
  aoi21  g106(.a(new_n156_), .b(x16), .c(new_n157_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(x18), .c(new_n70_), .O(z10));
  inv1   g108(.a(x30), .O(new_n160_));
  nand4  g109(.a(new_n154_), .b(new_n127_), .c(new_n98_), .d(new_n82_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n53_), .c(new_n65_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n151_), .c(new_n119_), .d(new_n91_), .O(new_n164_));
  oai21  g113(.a(new_n162_), .b(new_n160_), .c(new_n164_), .O(new_n165_));
  nor2   g114(.a(x16), .b(x04), .O(new_n166_));
  aoi21  g115(.a(new_n165_), .b(x16), .c(new_n166_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(x18), .c(new_n70_), .O(z11));
  inv1   g117(.a(x31), .O(new_n169_));
  nor3   g118(.a(x30), .b(x29), .c(x28), .O(new_n170_));
  nand4  g119(.a(new_n170_), .b(new_n140_), .c(new_n115_), .d(new_n82_), .O(new_n171_));
  aoi21  g120(.a(new_n171_), .b(new_n53_), .c(new_n65_), .O(new_n172_));
  nor2   g121(.a(new_n99_), .b(new_n65_), .O(new_n173_));
  nor2   g122(.a(x29), .b(x28), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n132_), .d(new_n173_), .O(new_n176_));
  oai21  g125(.a(new_n172_), .b(new_n169_), .c(new_n176_), .O(new_n177_));
  nor2   g126(.a(x16), .b(x03), .O(new_n178_));
  aoi21  g127(.a(new_n177_), .b(x16), .c(new_n178_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(x18), .c(new_n70_), .O(z12));
  inv1   g129(.a(x32), .O(new_n181_));
  nor3   g130(.a(x31), .b(x30), .c(x29), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n144_), .c(new_n115_), .d(new_n82_), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n53_), .c(new_n65_), .O(new_n184_));
  nand2  g133(.a(new_n151_), .b(new_n127_), .O(new_n185_));
  inv1   g134(.a(new_n185_), .O(new_n186_));
  nor2   g135(.a(x32), .b(x31), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n163_), .c(new_n186_), .d(new_n173_), .O(new_n188_));
  oai21  g137(.a(new_n184_), .b(new_n181_), .c(new_n188_), .O(new_n189_));
  nor2   g138(.a(x16), .b(x02), .O(new_n190_));
  aoi21  g139(.a(new_n189_), .b(x16), .c(new_n190_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(x18), .c(new_n70_), .O(z13));
  inv1   g141(.a(x33), .O(new_n193_));
  nor3   g142(.a(x32), .b(x31), .c(x30), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n154_), .c(new_n119_), .d(new_n139_), .O(new_n195_));
  aoi21  g144(.a(new_n195_), .b(new_n53_), .c(new_n65_), .O(new_n196_));
  nand2  g145(.a(new_n174_), .b(new_n140_), .O(new_n197_));
  inv1   g146(.a(new_n197_), .O(new_n198_));
  nor2   g147(.a(x33), .b(x32), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n198_), .c(new_n175_), .d(new_n109_), .O(new_n200_));
  oai21  g149(.a(new_n196_), .b(new_n193_), .c(new_n200_), .O(new_n201_));
  nor2   g150(.a(x16), .b(x01), .O(new_n202_));
  aoi21  g151(.a(new_n201_), .b(x16), .c(new_n202_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(x18), .c(new_n70_), .O(z14));
  nand4  g153(.a(new_n199_), .b(new_n175_), .c(new_n174_), .d(new_n126_), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n128_), .c(new_n53_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n73_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(x34), .O(new_n208_));
  nor3   g157(.a(x34), .b(x33), .c(x32), .O(new_n209_));
  nand4  g158(.a(new_n209_), .b(new_n198_), .c(new_n175_), .d(new_n109_), .O(new_n210_));
  aoi21  g159(.a(new_n210_), .b(new_n208_), .c(new_n58_), .O(new_n211_));
  nor2   g160(.a(x16), .b(x00), .O(new_n212_));
  oai21  g161(.a(new_n212_), .b(new_n211_), .c(new_n61_), .O(new_n213_));
  nand2  g162(.a(new_n213_), .b(new_n70_), .O(z15));
endmodule


