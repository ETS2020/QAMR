// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  nor2   g000(.a(x09), .b(x07), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g012(.a(new_n62_), .b(new_n54_), .c(new_n53_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n63_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n59_), .b(new_n67_), .c(x18), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n66_), .c(new_n52_), .O(z01));
  oai21  g018(.a(x09), .b(x07), .c(x18), .O(new_n70_));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n56_), .O(new_n73_));
  oai21  g022(.a(new_n65_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n52_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n70_), .O(z02));
  nand2  g027(.a(x22), .b(x21), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n62_), .c(new_n53_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n65_), .b(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  aoi21  g035(.a(new_n59_), .b(new_n86_), .c(x18), .O(new_n87_));
  aoi21  g036(.a(new_n87_), .b(new_n85_), .c(new_n52_), .O(z03));
  nor3   g037(.a(x22), .b(x21), .c(x20), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n56_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x23), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n65_), .c(new_n71_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x16), .O(new_n95_));
  nor2   g044(.a(x16), .b(x11), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n52_), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n95_), .c(new_n70_), .O(z04));
  and2   g047(.a(new_n93_), .b(x24), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n80_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n64_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n99_), .c(x16), .O(new_n103_));
  nor2   g052(.a(x16), .b(x10), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n52_), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n103_), .c(new_n70_), .O(z05));
  inv1   g055(.a(x07), .O(new_n107_));
  inv1   g056(.a(x09), .O(new_n108_));
  oai21  g057(.a(new_n59_), .b(new_n107_), .c(new_n108_), .O(new_n109_));
  inv1   g058(.a(x25), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n92_), .c(new_n72_), .d(new_n56_), .O(new_n112_));
  oai21  g061(.a(new_n102_), .b(new_n110_), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n109_), .c(new_n70_), .O(z06));
  inv1   g064(.a(x26), .O(new_n116_));
  inv1   g065(.a(new_n112_), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n92_), .O(new_n119_));
  oai22  g068(.a(new_n119_), .b(new_n73_), .c(new_n117_), .d(new_n116_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(x16), .O(new_n121_));
  nor2   g070(.a(x16), .b(x08), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n52_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n121_), .c(new_n70_), .O(z07));
  inv1   g073(.a(x27), .O(new_n125_));
  inv1   g074(.a(x24), .O(new_n126_));
  nor2   g075(.a(x26), .b(x25), .O(new_n127_));
  nor2   g076(.a(x23), .b(x21), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(new_n129_));
  nand3  g078(.a(new_n56_), .b(new_n83_), .c(new_n62_), .O(new_n130_));
  aoi21  g079(.a(new_n129_), .b(new_n54_), .c(new_n130_), .O(new_n131_));
  nor3   g080(.a(x27), .b(x26), .c(x25), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n100_), .c(new_n89_), .d(new_n56_), .O(new_n133_));
  oai21  g082(.a(new_n131_), .b(new_n125_), .c(new_n133_), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(x16), .c(x18), .O(new_n135_));
  nand3  g084(.a(new_n59_), .b(x09), .c(new_n107_), .O(new_n136_));
  oai21  g085(.a(new_n135_), .b(new_n52_), .c(new_n136_), .O(z08));
  and2   g086(.a(new_n133_), .b(x28), .O(new_n138_));
  inv1   g087(.a(x23), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n111_), .c(new_n116_), .d(new_n139_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n90_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n138_), .c(x16), .O(new_n143_));
  nor2   g092(.a(x16), .b(x06), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n52_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(new_n70_), .O(z09));
  inv1   g095(.a(x29), .O(new_n147_));
  nor2   g096(.a(x29), .b(x28), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n118_), .c(new_n125_), .O(new_n149_));
  oai22  g098(.a(new_n149_), .b(new_n93_), .c(new_n142_), .d(new_n147_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  nor2   g100(.a(x16), .b(x05), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n52_), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n151_), .c(new_n70_), .O(z10));
  inv1   g103(.a(x30), .O(new_n155_));
  nand4  g104(.a(new_n148_), .b(new_n127_), .c(new_n100_), .d(new_n71_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n54_), .O(new_n157_));
  inv1   g106(.a(new_n56_), .O(new_n158_));
  nand3  g107(.a(new_n125_), .b(new_n83_), .c(new_n62_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n157_), .c(new_n155_), .O(new_n161_));
  nor2   g110(.a(x30), .b(x29), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n140_), .c(new_n118_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n93_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n161_), .c(x16), .O(new_n165_));
  inv1   g114(.a(x04), .O(new_n166_));
  aoi21  g115(.a(new_n59_), .b(new_n166_), .c(x18), .O(new_n167_));
  aoi21  g116(.a(new_n167_), .b(new_n165_), .c(new_n52_), .O(z11));
  oai21  g117(.a(new_n163_), .b(new_n93_), .c(x31), .O(new_n169_));
  nor2   g118(.a(x31), .b(x30), .O(new_n170_));
  nand4  g119(.a(new_n170_), .b(new_n148_), .c(new_n132_), .d(new_n102_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n169_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  nor2   g122(.a(x16), .b(x03), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n52_), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n173_), .c(new_n70_), .O(z12));
  inv1   g125(.a(x32), .O(new_n177_));
  nor2   g126(.a(x28), .b(x26), .O(new_n178_));
  nor2   g127(.a(x31), .b(x29), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n178_), .c(new_n128_), .d(new_n111_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n54_), .O(new_n181_));
  nand3  g130(.a(new_n155_), .b(new_n125_), .c(new_n83_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n64_), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n181_), .c(new_n177_), .O(new_n184_));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n162_), .c(new_n140_), .d(new_n127_), .O(new_n186_));
  nor3   g135(.a(new_n186_), .b(new_n101_), .c(new_n64_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(new_n184_), .c(x16), .O(new_n188_));
  inv1   g137(.a(x02), .O(new_n189_));
  aoi21  g138(.a(new_n59_), .b(new_n189_), .c(x18), .O(new_n190_));
  aoi21  g139(.a(new_n190_), .b(new_n188_), .c(new_n52_), .O(z13));
  inv1   g140(.a(x33), .O(new_n192_));
  nor2   g141(.a(x27), .b(x26), .O(new_n193_));
  nand2  g142(.a(new_n148_), .b(new_n193_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  nand3  g144(.a(new_n170_), .b(new_n192_), .c(new_n177_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n195_), .c(new_n117_), .O(new_n198_));
  oai21  g147(.a(new_n187_), .b(new_n192_), .c(new_n198_), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(x16), .O(new_n200_));
  nor2   g149(.a(x16), .b(x01), .O(new_n201_));
  nor2   g150(.a(new_n201_), .b(new_n52_), .O(new_n202_));
  nand3  g151(.a(new_n202_), .b(new_n200_), .c(new_n70_), .O(z14));
  nand2  g152(.a(new_n198_), .b(x34), .O(new_n204_));
  nor3   g153(.a(x34), .b(x33), .c(x32), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n195_), .c(new_n170_), .d(new_n117_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n204_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(x16), .O(new_n208_));
  nor2   g157(.a(x16), .b(x00), .O(new_n209_));
  nor2   g158(.a(new_n209_), .b(new_n52_), .O(new_n210_));
  nand3  g159(.a(new_n210_), .b(new_n208_), .c(new_n70_), .O(z15));
endmodule


