// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_, new_n222_,
    new_n223_, new_n224_, new_n225_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x03), .O(new_n57_));
  nor2   g006(.a(x30), .b(new_n57_), .O(new_n58_));
  inv1   g007(.a(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(x18), .O(new_n60_));
  nor2   g009(.a(x16), .b(x15), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nor2   g014(.a(x20), .b(x19), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n52_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n65_), .c(x16), .O(new_n69_));
  nor2   g018(.a(x16), .b(x14), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n69_), .c(new_n60_), .O(z01));
  inv1   g021(.a(x21), .O(new_n73_));
  nor2   g022(.a(x21), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n55_), .O(new_n75_));
  oai21  g024(.a(new_n68_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  inv1   g027(.a(x16), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n78_), .c(x18), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n77_), .c(new_n58_), .O(z02));
  nand2  g030(.a(x22), .b(x21), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n64_), .c(new_n52_), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(x19), .O(new_n85_));
  and2   g034(.a(new_n67_), .b(x22), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  nor2   g036(.a(x16), .b(x12), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n58_), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n87_), .c(new_n60_), .O(z03));
  inv1   g039(.a(x23), .O(new_n91_));
  oai21  g040(.a(x22), .b(x21), .c(new_n53_), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n68_), .c(new_n91_), .O(new_n93_));
  nor2   g042(.a(x23), .b(x22), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n73_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n67_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n93_), .c(x16), .O(new_n97_));
  nor2   g046(.a(x16), .b(x11), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n58_), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n97_), .c(new_n60_), .O(z04));
  nand2  g049(.a(new_n67_), .b(x24), .O(new_n101_));
  nor2   g050(.a(x24), .b(x23), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n83_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n67_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n101_), .c(new_n79_), .O(new_n106_));
  inv1   g055(.a(x18), .O(new_n107_));
  oai21  g056(.a(x16), .b(x10), .c(new_n107_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n106_), .c(new_n59_), .O(new_n109_));
  inv1   g058(.a(x30), .O(new_n110_));
  oai21  g059(.a(x19), .b(x03), .c(new_n110_), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n95_), .c(x24), .d(x16), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n109_), .O(z05));
  inv1   g062(.a(x25), .O(new_n114_));
  aoi21  g063(.a(new_n103_), .b(new_n53_), .c(new_n67_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nor2   g065(.a(x25), .b(x24), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n94_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n75_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  nor2   g069(.a(x16), .b(x09), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n58_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n120_), .c(new_n60_), .O(z06));
  inv1   g072(.a(x26), .O(new_n124_));
  nor3   g073(.a(x25), .b(x24), .c(x23), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n83_), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n53_), .c(new_n67_), .O(new_n127_));
  nor3   g076(.a(x26), .b(x25), .c(x24), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n94_), .O(new_n129_));
  oai22  g078(.a(new_n129_), .b(new_n75_), .c(new_n127_), .d(new_n124_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  nor2   g080(.a(x16), .b(x08), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n58_), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n131_), .c(new_n60_), .O(z07));
  inv1   g083(.a(x27), .O(new_n135_));
  nand3  g084(.a(new_n83_), .b(new_n55_), .c(new_n64_), .O(new_n136_));
  nor2   g085(.a(x27), .b(x26), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n102_), .c(new_n114_), .O(new_n138_));
  oai22  g087(.a(new_n138_), .b(new_n136_), .c(new_n68_), .d(new_n135_), .O(new_n139_));
  oai21  g088(.a(x16), .b(x07), .c(new_n107_), .O(new_n140_));
  aoi21  g089(.a(new_n139_), .b(x16), .c(new_n140_), .O(new_n141_));
  nand3  g090(.a(new_n128_), .b(new_n94_), .c(new_n73_), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n111_), .c(x27), .d(x16), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n58_), .c(new_n143_), .O(z08));
  inv1   g093(.a(x28), .O(new_n145_));
  nand2  g094(.a(new_n137_), .b(new_n117_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n95_), .c(new_n53_), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n68_), .c(new_n145_), .O(new_n148_));
  nor3   g097(.a(x28), .b(x27), .c(x26), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n125_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n136_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n148_), .c(x16), .O(new_n152_));
  nor2   g101(.a(x16), .b(x06), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n58_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(new_n60_), .O(z09));
  inv1   g104(.a(x29), .O(new_n156_));
  inv1   g105(.a(new_n103_), .O(new_n157_));
  nor2   g106(.a(x26), .b(x25), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n145_), .c(new_n135_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n157_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n53_), .c(new_n67_), .O(new_n162_));
  nor3   g111(.a(x29), .b(x28), .c(x27), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n128_), .c(new_n96_), .O(new_n164_));
  oai21  g113(.a(new_n162_), .b(new_n156_), .c(new_n164_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(x16), .O(new_n166_));
  nor2   g115(.a(x16), .b(x05), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n58_), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n166_), .c(new_n60_), .O(z10));
  oai21  g118(.a(x16), .b(x04), .c(new_n107_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n59_), .O(new_n171_));
  nand2  g120(.a(new_n163_), .b(new_n158_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n103_), .c(new_n53_), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n68_), .c(new_n110_), .O(new_n174_));
  nand4  g123(.a(new_n94_), .b(new_n74_), .c(new_n55_), .d(new_n57_), .O(new_n175_));
  nor2   g124(.a(x30), .b(x29), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n128_), .c(new_n145_), .d(new_n135_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n175_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n174_), .c(x16), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n171_), .O(z11));
  inv1   g129(.a(x31), .O(new_n181_));
  aoi21  g130(.a(new_n173_), .b(new_n68_), .c(new_n181_), .O(new_n182_));
  nor2   g131(.a(x29), .b(x28), .O(new_n183_));
  nor2   g132(.a(x31), .b(x30), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n183_), .c(new_n137_), .d(new_n114_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n105_), .c(x16), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n182_), .c(new_n57_), .O(new_n187_));
  nand3  g136(.a(x31), .b(x30), .c(x16), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n187_), .c(new_n60_), .O(z12));
  oai21  g138(.a(x16), .b(x02), .c(new_n107_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n59_), .O(new_n191_));
  inv1   g140(.a(x32), .O(new_n192_));
  nor3   g141(.a(x31), .b(x29), .c(x28), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n137_), .c(new_n125_), .d(new_n83_), .O(new_n194_));
  aoi21  g143(.a(new_n194_), .b(new_n53_), .c(new_n67_), .O(new_n195_));
  nor2   g144(.a(x32), .b(x31), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n176_), .c(new_n160_), .d(new_n104_), .O(new_n197_));
  oai21  g146(.a(new_n195_), .b(new_n192_), .c(new_n197_), .O(new_n198_));
  nor2   g147(.a(new_n192_), .b(new_n110_), .O(new_n199_));
  aoi21  g148(.a(new_n198_), .b(new_n57_), .c(new_n199_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n79_), .c(new_n191_), .O(z13));
  oai21  g150(.a(x16), .b(x01), .c(new_n107_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n59_), .O(new_n203_));
  inv1   g152(.a(x33), .O(new_n204_));
  nor3   g153(.a(x32), .b(x31), .c(x29), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n149_), .c(new_n125_), .d(new_n83_), .O(new_n206_));
  aoi21  g155(.a(new_n206_), .b(new_n53_), .c(new_n67_), .O(new_n207_));
  nand3  g156(.a(new_n184_), .b(new_n204_), .c(new_n192_), .O(new_n208_));
  inv1   g157(.a(new_n208_), .O(new_n209_));
  nand4  g158(.a(new_n209_), .b(new_n183_), .c(new_n137_), .d(new_n119_), .O(new_n210_));
  oai21  g159(.a(new_n207_), .b(new_n204_), .c(new_n210_), .O(new_n211_));
  nor2   g160(.a(new_n204_), .b(new_n110_), .O(new_n212_));
  aoi21  g161(.a(new_n211_), .b(new_n57_), .c(new_n212_), .O(new_n213_));
  oai21  g162(.a(new_n213_), .b(new_n79_), .c(new_n203_), .O(z14));
  nand4  g163(.a(new_n157_), .b(new_n66_), .c(new_n52_), .d(x16), .O(new_n215_));
  nor3   g164(.a(x34), .b(x33), .c(x32), .O(new_n216_));
  nand4  g165(.a(new_n216_), .b(new_n160_), .c(new_n181_), .d(new_n156_), .O(new_n217_));
  oai21  g166(.a(new_n217_), .b(new_n215_), .c(new_n57_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n110_), .O(new_n219_));
  nand2  g168(.a(new_n209_), .b(new_n163_), .O(new_n220_));
  oai21  g169(.a(new_n220_), .b(new_n142_), .c(new_n53_), .O(new_n221_));
  nand2  g170(.a(new_n221_), .b(new_n68_), .O(new_n222_));
  nand3  g171(.a(new_n222_), .b(x34), .c(x16), .O(new_n223_));
  inv1   g172(.a(x00), .O(new_n224_));
  nand2  g173(.a(new_n79_), .b(new_n224_), .O(new_n225_));
  nand4  g174(.a(new_n225_), .b(new_n223_), .c(new_n219_), .d(new_n60_), .O(z15));
endmodule


