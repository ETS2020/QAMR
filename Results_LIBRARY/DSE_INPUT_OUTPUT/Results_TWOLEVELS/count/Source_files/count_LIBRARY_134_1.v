// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_;
  inv1   g000(.a(x11), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x03), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  oai21  g011(.a(new_n52_), .b(x03), .c(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n55_), .c(new_n54_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(z01));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n57_), .O(new_n74_));
  oai21  g023(.a(new_n67_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n53_), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n76_), .c(new_n63_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  aoi21  g029(.a(new_n73_), .b(new_n57_), .c(new_n80_), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n57_), .c(new_n64_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n81_), .c(x16), .O(new_n85_));
  nor2   g034(.a(x16), .b(x12), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(new_n63_), .O(z03));
  oai21  g037(.a(x18), .b(new_n60_), .c(new_n52_), .O(new_n89_));
  inv1   g038(.a(x23), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n67_), .c(new_n72_), .O(new_n92_));
  oai21  g041(.a(new_n84_), .b(new_n90_), .c(new_n92_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(x16), .O(new_n94_));
  aoi21  g043(.a(x18), .b(x03), .c(new_n53_), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n94_), .c(new_n89_), .O(z04));
  and2   g045(.a(new_n92_), .b(x24), .O(new_n97_));
  inv1   g046(.a(new_n82_), .O(new_n98_));
  inv1   g047(.a(x24), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n90_), .O(new_n100_));
  nor3   g049(.a(new_n100_), .b(new_n98_), .c(new_n66_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n97_), .c(x16), .O(new_n102_));
  nor2   g051(.a(x16), .b(x10), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n53_), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n102_), .c(new_n63_), .O(z05));
  inv1   g054(.a(x25), .O(new_n106_));
  nor2   g055(.a(x25), .b(x24), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n91_), .c(new_n73_), .d(new_n57_), .O(new_n108_));
  oai21  g057(.a(new_n101_), .b(new_n106_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(x16), .O(new_n110_));
  nor2   g059(.a(x16), .b(x09), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n53_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n110_), .c(new_n63_), .O(z06));
  inv1   g062(.a(x26), .O(new_n114_));
  nand3  g063(.a(new_n107_), .b(new_n82_), .c(new_n90_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n55_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n67_), .c(new_n114_), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n91_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n74_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  inv1   g070(.a(x08), .O(new_n122_));
  aoi21  g071(.a(new_n60_), .b(new_n122_), .c(x18), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n121_), .c(new_n53_), .O(z07));
  inv1   g073(.a(x27), .O(new_n125_));
  nand3  g074(.a(new_n57_), .b(new_n114_), .c(new_n64_), .O(new_n126_));
  inv1   g075(.a(new_n126_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n116_), .c(new_n125_), .O(new_n128_));
  nor2   g077(.a(x27), .b(x26), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n106_), .O(new_n130_));
  nor3   g079(.a(new_n130_), .b(new_n100_), .c(new_n83_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  inv1   g081(.a(x07), .O(new_n133_));
  aoi21  g082(.a(new_n60_), .b(new_n133_), .c(x18), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n132_), .c(new_n53_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  inv1   g085(.a(new_n57_), .O(new_n137_));
  nand2  g086(.a(new_n129_), .b(new_n64_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n116_), .c(new_n136_), .O(new_n140_));
  nand2  g089(.a(new_n107_), .b(new_n90_), .O(new_n141_));
  nor2   g090(.a(x28), .b(x27), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n114_), .O(new_n143_));
  nor3   g092(.a(new_n143_), .b(new_n141_), .c(new_n83_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n140_), .c(x16), .O(new_n145_));
  inv1   g094(.a(x06), .O(new_n146_));
  aoi21  g095(.a(new_n60_), .b(new_n146_), .c(x18), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n145_), .c(new_n53_), .O(z09));
  inv1   g097(.a(x29), .O(new_n149_));
  nor2   g098(.a(new_n143_), .b(new_n66_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n116_), .c(new_n149_), .O(new_n151_));
  nor2   g100(.a(x29), .b(x28), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n118_), .c(new_n125_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n92_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n151_), .c(x16), .O(new_n155_));
  inv1   g104(.a(x05), .O(new_n156_));
  aoi21  g105(.a(new_n60_), .b(new_n156_), .c(x18), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n155_), .c(new_n53_), .O(z10));
  inv1   g107(.a(x30), .O(new_n159_));
  nand2  g108(.a(new_n152_), .b(new_n129_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n66_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n116_), .c(new_n159_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n142_), .c(new_n118_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n92_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n162_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n60_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n53_), .O(z11));
  inv1   g118(.a(x03), .O(new_n170_));
  oai21  g119(.a(new_n60_), .b(x11), .c(new_n170_), .O(new_n171_));
  inv1   g120(.a(x31), .O(new_n172_));
  nand3  g121(.a(new_n163_), .b(new_n129_), .c(new_n136_), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n55_), .c(new_n108_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n172_), .O(new_n175_));
  nand4  g124(.a(new_n82_), .b(new_n67_), .c(new_n99_), .d(new_n90_), .O(new_n176_));
  nor2   g125(.a(x31), .b(x30), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n152_), .c(new_n129_), .d(new_n106_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n176_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n175_), .c(x16), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n171_), .c(new_n63_), .O(z12));
  inv1   g130(.a(x32), .O(new_n182_));
  nand2  g131(.a(new_n173_), .b(new_n55_), .O(new_n183_));
  nand4  g132(.a(new_n91_), .b(new_n172_), .c(new_n106_), .d(new_n99_), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n74_), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n183_), .c(new_n182_), .O(new_n186_));
  nor2   g135(.a(x26), .b(x25), .O(new_n187_));
  nor2   g136(.a(x32), .b(x31), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n163_), .c(new_n142_), .d(new_n187_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n176_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n186_), .c(x16), .O(new_n191_));
  nor2   g140(.a(x16), .b(x02), .O(new_n192_));
  nor2   g141(.a(new_n192_), .b(new_n53_), .O(new_n193_));
  nand3  g142(.a(new_n193_), .b(new_n191_), .c(new_n63_), .O(z13));
  inv1   g143(.a(x33), .O(new_n195_));
  nand4  g144(.a(new_n188_), .b(new_n107_), .c(new_n91_), .d(new_n72_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n55_), .O(new_n197_));
  nand2  g146(.a(new_n163_), .b(new_n142_), .O(new_n198_));
  nor2   g147(.a(new_n198_), .b(new_n126_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n197_), .c(new_n195_), .O(new_n200_));
  nand3  g149(.a(new_n177_), .b(new_n195_), .c(new_n182_), .O(new_n201_));
  nor3   g150(.a(new_n201_), .b(new_n160_), .c(new_n108_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n200_), .c(x16), .O(new_n203_));
  inv1   g152(.a(x01), .O(new_n204_));
  aoi21  g153(.a(new_n60_), .b(new_n204_), .c(x18), .O(new_n205_));
  aoi21  g154(.a(new_n205_), .b(new_n203_), .c(new_n53_), .O(z14));
  inv1   g155(.a(x34), .O(new_n207_));
  nor2   g156(.a(x33), .b(x30), .O(new_n208_));
  nand3  g157(.a(new_n208_), .b(new_n142_), .c(new_n149_), .O(new_n209_));
  nor2   g158(.a(new_n209_), .b(new_n126_), .O(new_n210_));
  aoi21  g159(.a(new_n210_), .b(new_n197_), .c(new_n207_), .O(new_n211_));
  nand4  g160(.a(new_n177_), .b(new_n207_), .c(new_n195_), .d(new_n182_), .O(new_n212_));
  nor3   g161(.a(new_n212_), .b(new_n160_), .c(new_n108_), .O(new_n213_));
  oai21  g162(.a(new_n213_), .b(new_n211_), .c(x16), .O(new_n214_));
  inv1   g163(.a(x00), .O(new_n215_));
  aoi21  g164(.a(new_n60_), .b(new_n215_), .c(x18), .O(new_n216_));
  aoi21  g165(.a(new_n216_), .b(new_n214_), .c(new_n53_), .O(z15));
endmodule


