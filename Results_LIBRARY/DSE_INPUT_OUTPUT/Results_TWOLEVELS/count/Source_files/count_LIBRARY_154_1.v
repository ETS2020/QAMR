// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g004(.a(x16), .b(x15), .O(new_n56_));
  aoi21  g005(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  nand2  g006(.a(x18), .b(x03), .O(new_n58_));
  oai21  g007(.a(new_n57_), .b(x18), .c(new_n58_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n54_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n58_), .O(z01));
  nand2  g019(.a(x20), .b(new_n60_), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n64_), .c(new_n63_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n53_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n72_), .b(x21), .c(new_n75_), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n61_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n61_), .c(new_n78_), .O(z02));
  oai21  g028(.a(new_n73_), .b(x18), .c(new_n53_), .O(new_n80_));
  nor3   g029(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  and2   g030(.a(new_n81_), .b(new_n53_), .O(new_n82_));
  aoi21  g031(.a(new_n80_), .b(x22), .c(new_n82_), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n61_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n61_), .c(new_n85_), .O(z03));
  inv1   g035(.a(new_n66_), .O(new_n87_));
  oai21  g036(.a(new_n81_), .b(x18), .c(new_n53_), .O(new_n88_));
  nor3   g037(.a(x23), .b(x22), .c(x21), .O(new_n89_));
  aoi22  g038(.a(new_n89_), .b(new_n87_), .c(new_n88_), .d(x23), .O(new_n90_));
  inv1   g039(.a(x11), .O(new_n91_));
  aoi21  g040(.a(new_n61_), .b(new_n91_), .c(x18), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n61_), .c(new_n92_), .O(z04));
  inv1   g042(.a(x24), .O(new_n94_));
  inv1   g043(.a(new_n89_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n64_), .c(new_n66_), .O(new_n96_));
  nor2   g045(.a(x22), .b(x21), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai22  g048(.a(new_n99_), .b(new_n66_), .c(new_n96_), .d(new_n94_), .O(new_n100_));
  nor2   g049(.a(x16), .b(x10), .O(new_n101_));
  aoi21  g050(.a(new_n100_), .b(x16), .c(new_n101_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(x18), .c(new_n58_), .O(z05));
  inv1   g052(.a(x25), .O(new_n104_));
  aoi21  g053(.a(new_n99_), .b(new_n64_), .c(new_n66_), .O(new_n105_));
  nor2   g054(.a(x23), .b(x22), .O(new_n106_));
  nor2   g055(.a(x25), .b(x24), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  oai22  g057(.a(new_n108_), .b(new_n74_), .c(new_n105_), .d(new_n104_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x09), .O(new_n110_));
  aoi21  g059(.a(new_n109_), .b(x16), .c(new_n110_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(x18), .c(new_n58_), .O(z06));
  inv1   g061(.a(x26), .O(new_n113_));
  nor3   g062(.a(x25), .b(x24), .c(x23), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n97_), .O(new_n115_));
  aoi21  g064(.a(new_n115_), .b(new_n64_), .c(new_n66_), .O(new_n116_));
  nor3   g065(.a(x26), .b(x25), .c(x24), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n106_), .c(new_n75_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n113_), .c(new_n118_), .O(new_n119_));
  nor2   g068(.a(x16), .b(x08), .O(new_n120_));
  aoi21  g069(.a(new_n119_), .b(x16), .c(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(x18), .c(new_n58_), .O(z07));
  inv1   g071(.a(x27), .O(new_n123_));
  nand2  g072(.a(new_n117_), .b(new_n89_), .O(new_n124_));
  aoi21  g073(.a(new_n124_), .b(new_n64_), .c(new_n66_), .O(new_n125_));
  nor3   g074(.a(x27), .b(x26), .c(x25), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n98_), .c(new_n82_), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n123_), .c(new_n127_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x07), .O(new_n129_));
  aoi21  g078(.a(new_n128_), .b(x16), .c(new_n129_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(x18), .c(new_n58_), .O(z08));
  inv1   g080(.a(x28), .O(new_n132_));
  nor2   g081(.a(x27), .b(x26), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n107_), .c(new_n106_), .d(new_n73_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n60_), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n53_), .c(new_n132_), .O(new_n136_));
  inv1   g085(.a(new_n82_), .O(new_n137_));
  nor3   g086(.a(x28), .b(x27), .c(x26), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n114_), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n137_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n136_), .c(x16), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  aoi21  g091(.a(new_n61_), .b(new_n142_), .c(x18), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n141_), .O(z09));
  inv1   g093(.a(x29), .O(new_n145_));
  nand2  g094(.a(new_n89_), .b(new_n87_), .O(new_n146_));
  nor2   g095(.a(x26), .b(x25), .O(new_n147_));
  nor2   g096(.a(x28), .b(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n147_), .c(new_n98_), .d(new_n97_), .O(new_n149_));
  aoi21  g098(.a(new_n149_), .b(new_n64_), .c(new_n66_), .O(new_n150_));
  nor3   g099(.a(x29), .b(x28), .c(x27), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n117_), .O(new_n152_));
  oai22  g101(.a(new_n152_), .b(new_n146_), .c(new_n150_), .d(new_n145_), .O(new_n153_));
  nor2   g102(.a(x16), .b(x05), .O(new_n154_));
  aoi21  g103(.a(new_n153_), .b(x16), .c(new_n154_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(x18), .c(new_n58_), .O(z10));
  inv1   g105(.a(x30), .O(new_n157_));
  inv1   g106(.a(x22), .O(new_n158_));
  nand3  g107(.a(new_n98_), .b(new_n73_), .c(new_n158_), .O(new_n159_));
  nand2  g108(.a(new_n151_), .b(new_n147_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n159_), .c(new_n60_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n53_), .c(new_n157_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n148_), .c(new_n117_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n146_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n162_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x04), .O(new_n167_));
  aoi21  g116(.a(new_n61_), .b(new_n167_), .c(x18), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z11));
  inv1   g118(.a(x31), .O(new_n170_));
  nor3   g119(.a(x30), .b(x29), .c(x28), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n133_), .c(new_n114_), .d(new_n97_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n64_), .c(new_n66_), .O(new_n173_));
  nor2   g122(.a(new_n99_), .b(new_n66_), .O(new_n174_));
  nor2   g123(.a(x29), .b(x28), .O(new_n175_));
  nor2   g124(.a(x31), .b(x30), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n126_), .d(new_n174_), .O(new_n177_));
  oai21  g126(.a(new_n173_), .b(new_n170_), .c(new_n177_), .O(new_n178_));
  nor2   g127(.a(x16), .b(x03), .O(new_n179_));
  aoi21  g128(.a(new_n178_), .b(x16), .c(new_n179_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(x18), .c(new_n58_), .O(z12));
  inv1   g130(.a(x32), .O(new_n182_));
  nand3  g131(.a(new_n53_), .b(new_n170_), .c(new_n65_), .O(new_n183_));
  aoi21  g132(.a(new_n172_), .b(new_n64_), .c(new_n183_), .O(new_n184_));
  nand3  g133(.a(new_n163_), .b(new_n182_), .c(new_n170_), .O(new_n185_));
  inv1   g134(.a(new_n185_), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n148_), .c(new_n147_), .d(new_n174_), .O(new_n187_));
  oai21  g136(.a(new_n184_), .b(new_n182_), .c(new_n187_), .O(new_n188_));
  nor2   g137(.a(x16), .b(x02), .O(new_n189_));
  aoi21  g138(.a(new_n188_), .b(x16), .c(new_n189_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(x18), .c(new_n58_), .O(z13));
  inv1   g140(.a(x33), .O(new_n192_));
  nor3   g141(.a(x32), .b(x30), .c(x29), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n138_), .c(new_n114_), .d(new_n97_), .O(new_n194_));
  aoi21  g143(.a(new_n194_), .b(new_n64_), .c(new_n183_), .O(new_n195_));
  nor2   g144(.a(new_n108_), .b(new_n74_), .O(new_n196_));
  nand2  g145(.a(new_n175_), .b(new_n133_), .O(new_n197_));
  inv1   g146(.a(new_n197_), .O(new_n198_));
  nor2   g147(.a(x33), .b(x32), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n198_), .c(new_n176_), .d(new_n196_), .O(new_n200_));
  oai21  g149(.a(new_n195_), .b(new_n192_), .c(new_n200_), .O(new_n201_));
  nor2   g150(.a(x16), .b(x01), .O(new_n202_));
  aoi21  g151(.a(new_n201_), .b(x16), .c(new_n202_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(x18), .c(new_n58_), .O(z14));
  inv1   g153(.a(x34), .O(new_n205_));
  nor3   g154(.a(x33), .b(x32), .c(x30), .O(new_n206_));
  nand4  g155(.a(new_n206_), .b(new_n151_), .c(new_n117_), .d(new_n89_), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n64_), .c(new_n183_), .O(new_n208_));
  nor3   g157(.a(x34), .b(x33), .c(x32), .O(new_n209_));
  nand4  g158(.a(new_n209_), .b(new_n198_), .c(new_n176_), .d(new_n196_), .O(new_n210_));
  oai21  g159(.a(new_n208_), .b(new_n205_), .c(new_n210_), .O(new_n211_));
  nor2   g160(.a(x16), .b(x00), .O(new_n212_));
  aoi21  g161(.a(new_n211_), .b(x16), .c(new_n212_), .O(new_n213_));
  oai21  g162(.a(new_n213_), .b(x18), .c(new_n58_), .O(z15));
endmodule


