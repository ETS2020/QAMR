// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n204_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n212_, new_n213_, new_n214_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n229_, new_n230_, new_n231_, new_n232_, new_n233_, new_n234_,
    new_n235_, new_n236_, new_n237_;
  nor2   g000(.a(x10), .b(x03), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(new_n52_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x18), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n56_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(new_n63_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n56_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  nor2   g024(.a(x16), .b(x13), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n52_), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(new_n63_), .O(z02));
  nand2  g027(.a(x22), .b(x21), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n64_), .c(new_n53_), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n66_), .b(new_n83_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  aoi21  g035(.a(new_n59_), .b(new_n86_), .c(x18), .O(new_n87_));
  aoi21  g036(.a(new_n87_), .b(new_n85_), .c(new_n52_), .O(z03));
  nand2  g037(.a(x23), .b(x21), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n71_), .c(new_n64_), .d(new_n53_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n89_), .c(x19), .O(new_n92_));
  nand3  g041(.a(new_n56_), .b(new_n83_), .c(new_n64_), .O(new_n93_));
  and2   g042(.a(new_n93_), .b(x23), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n59_), .b(new_n96_), .c(x18), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n52_), .O(z04));
  nand2  g047(.a(x24), .b(x21), .O(new_n99_));
  nand2  g048(.a(new_n72_), .b(new_n53_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n83_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n54_), .O(new_n104_));
  nand3  g053(.a(new_n90_), .b(new_n56_), .c(new_n64_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x24), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n104_), .c(new_n59_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(x18), .c(new_n62_), .O(new_n108_));
  inv1   g057(.a(x10), .O(new_n109_));
  nand3  g058(.a(new_n59_), .b(new_n109_), .c(x03), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n108_), .O(z05));
  inv1   g060(.a(x25), .O(new_n112_));
  inv1   g061(.a(new_n73_), .O(new_n113_));
  nand2  g062(.a(new_n102_), .b(new_n54_), .O(new_n114_));
  aoi21  g063(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(new_n115_));
  nor2   g064(.a(x25), .b(x24), .O(new_n116_));
  nand4  g065(.a(new_n116_), .b(new_n90_), .c(new_n72_), .d(new_n56_), .O(new_n117_));
  inv1   g066(.a(new_n117_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n115_), .c(x16), .O(new_n119_));
  nor2   g068(.a(x16), .b(x09), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n52_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n119_), .c(new_n63_), .O(z06));
  inv1   g071(.a(x26), .O(new_n123_));
  nor3   g072(.a(x25), .b(x21), .c(x20), .O(new_n124_));
  and2   g073(.a(new_n124_), .b(new_n56_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n114_), .c(new_n123_), .O(new_n126_));
  nor3   g075(.a(x26), .b(x25), .c(x24), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n90_), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n73_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n126_), .c(x16), .O(new_n130_));
  nor2   g079(.a(x16), .b(x08), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n52_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n130_), .c(new_n63_), .O(z07));
  inv1   g082(.a(x27), .O(new_n134_));
  nor2   g083(.a(x20), .b(x19), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n53_), .O(new_n136_));
  nor2   g085(.a(x26), .b(x25), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n71_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n114_), .c(new_n134_), .O(new_n140_));
  nand3  g089(.a(new_n80_), .b(new_n56_), .c(new_n64_), .O(new_n141_));
  nor3   g090(.a(x27), .b(x26), .c(x25), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n101_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n140_), .c(x16), .O(new_n145_));
  nor2   g094(.a(x16), .b(x07), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n52_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n145_), .c(new_n63_), .O(z08));
  inv1   g097(.a(x28), .O(new_n149_));
  nor2   g098(.a(x27), .b(x26), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n112_), .c(new_n71_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n54_), .O(new_n152_));
  nor2   g101(.a(new_n102_), .b(new_n136_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n152_), .c(new_n149_), .O(new_n154_));
  inv1   g103(.a(x23), .O(new_n155_));
  nor2   g104(.a(x28), .b(x27), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n116_), .c(new_n123_), .d(new_n155_), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n141_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n154_), .c(x16), .O(new_n159_));
  inv1   g108(.a(x06), .O(new_n160_));
  aoi21  g109(.a(new_n59_), .b(new_n160_), .c(x18), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n159_), .c(new_n52_), .O(z09));
  nor2   g111(.a(x28), .b(x24), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n90_), .c(x19), .O(new_n164_));
  inv1   g113(.a(new_n151_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n66_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n164_), .c(x29), .O(new_n167_));
  nand2  g116(.a(new_n90_), .b(new_n71_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n136_), .O(new_n169_));
  nor2   g118(.a(x29), .b(x28), .O(new_n170_));
  nand4  g119(.a(new_n170_), .b(new_n169_), .c(new_n127_), .d(new_n134_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n167_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  nor2   g122(.a(x16), .b(x05), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n52_), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n173_), .c(new_n63_), .O(z10));
  nor2   g125(.a(x29), .b(x27), .O(new_n177_));
  nand3  g126(.a(new_n177_), .b(new_n137_), .c(new_n113_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n164_), .c(x30), .O(new_n179_));
  nor2   g128(.a(x30), .b(x29), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n169_), .c(new_n156_), .d(new_n127_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n179_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x04), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n52_), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n183_), .c(new_n63_), .O(z11));
  inv1   g135(.a(x31), .O(new_n187_));
  nand4  g136(.a(new_n180_), .b(new_n137_), .c(new_n134_), .d(new_n71_), .O(new_n188_));
  nand4  g137(.a(new_n163_), .b(new_n90_), .c(new_n135_), .d(new_n53_), .O(new_n189_));
  aoi21  g138(.a(new_n188_), .b(new_n54_), .c(new_n189_), .O(new_n190_));
  inv1   g139(.a(x24), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n155_), .c(new_n83_), .d(new_n71_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  inv1   g142(.a(x29), .O(new_n194_));
  inv1   g143(.a(x30), .O(new_n195_));
  nand4  g144(.a(new_n187_), .b(new_n195_), .c(new_n194_), .d(new_n149_), .O(new_n196_));
  inv1   g145(.a(new_n196_), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n193_), .c(new_n142_), .d(new_n66_), .O(new_n198_));
  oai21  g147(.a(new_n190_), .b(new_n187_), .c(new_n198_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(x16), .c(x18), .O(new_n200_));
  inv1   g149(.a(x03), .O(new_n201_));
  nand3  g150(.a(new_n59_), .b(x10), .c(new_n201_), .O(new_n202_));
  oai21  g151(.a(new_n200_), .b(new_n52_), .c(new_n202_), .O(z12));
  inv1   g152(.a(x32), .O(new_n204_));
  nand2  g153(.a(new_n188_), .b(new_n54_), .O(new_n205_));
  nand3  g154(.a(new_n101_), .b(new_n187_), .c(new_n149_), .O(new_n206_));
  nor2   g155(.a(new_n206_), .b(new_n93_), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n205_), .c(new_n204_), .O(new_n208_));
  nor2   g157(.a(x32), .b(x31), .O(new_n209_));
  nand4  g158(.a(new_n209_), .b(new_n180_), .c(new_n156_), .d(new_n137_), .O(new_n210_));
  nor3   g159(.a(new_n210_), .b(new_n192_), .c(new_n136_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n208_), .c(x16), .O(new_n212_));
  inv1   g161(.a(x02), .O(new_n213_));
  aoi21  g162(.a(new_n59_), .b(new_n213_), .c(x18), .O(new_n214_));
  aoi21  g163(.a(new_n214_), .b(new_n212_), .c(new_n52_), .O(z13));
  inv1   g164(.a(x33), .O(new_n216_));
  nand3  g165(.a(new_n209_), .b(new_n101_), .c(new_n149_), .O(new_n217_));
  nor2   g166(.a(new_n217_), .b(new_n93_), .O(new_n218_));
  aoi21  g167(.a(new_n218_), .b(new_n205_), .c(new_n216_), .O(new_n219_));
  nand2  g168(.a(new_n170_), .b(new_n150_), .O(new_n220_));
  nor2   g169(.a(x31), .b(x30), .O(new_n221_));
  nor2   g170(.a(x33), .b(x32), .O(new_n222_));
  nand2  g171(.a(new_n222_), .b(new_n221_), .O(new_n223_));
  nor3   g172(.a(new_n223_), .b(new_n220_), .c(new_n117_), .O(new_n224_));
  oai21  g173(.a(new_n224_), .b(new_n219_), .c(x16), .O(new_n225_));
  inv1   g174(.a(x01), .O(new_n226_));
  aoi21  g175(.a(new_n59_), .b(new_n226_), .c(x18), .O(new_n227_));
  aoi21  g176(.a(new_n227_), .b(new_n225_), .c(new_n52_), .O(z14));
  inv1   g177(.a(x34), .O(new_n229_));
  nand3  g178(.a(new_n222_), .b(new_n163_), .c(new_n187_), .O(new_n230_));
  nor2   g179(.a(new_n230_), .b(new_n105_), .O(new_n231_));
  aoi21  g180(.a(new_n231_), .b(new_n205_), .c(new_n229_), .O(new_n232_));
  nand4  g181(.a(new_n221_), .b(new_n229_), .c(new_n216_), .d(new_n204_), .O(new_n233_));
  nor3   g182(.a(new_n233_), .b(new_n220_), .c(new_n117_), .O(new_n234_));
  oai21  g183(.a(new_n234_), .b(new_n232_), .c(x16), .O(new_n235_));
  inv1   g184(.a(x00), .O(new_n236_));
  aoi21  g185(.a(new_n59_), .b(new_n236_), .c(x18), .O(new_n237_));
  aoi21  g186(.a(new_n237_), .b(new_n235_), .c(new_n52_), .O(z15));
endmodule


