// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x18), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand3  g004(.a(new_n55_), .b(new_n53_), .c(new_n52_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(x16), .O(new_n57_));
  nor2   g006(.a(x18), .b(x16), .O(new_n58_));
  inv1   g007(.a(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n59_), .b(x15), .c(new_n57_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  nor2   g011(.a(new_n62_), .b(x18), .O(new_n63_));
  oai21  g012(.a(new_n54_), .b(new_n61_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(x16), .O(new_n65_));
  oai21  g014(.a(new_n59_), .b(x14), .c(new_n65_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  nor2   g016(.a(new_n62_), .b(new_n67_), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n54_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n52_), .O(new_n71_));
  oai21  g020(.a(new_n71_), .b(new_n68_), .c(x16), .O(new_n72_));
  oai21  g021(.a(new_n59_), .b(x13), .c(new_n72_), .O(z02));
  nand2  g022(.a(x22), .b(x21), .O(new_n74_));
  nor2   g023(.a(x20), .b(x17), .O(new_n75_));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n74_), .c(x19), .O(new_n78_));
  inv1   g027(.a(x22), .O(new_n79_));
  oai21  g028(.a(new_n62_), .b(new_n79_), .c(new_n52_), .O(new_n80_));
  oai21  g029(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  oai21  g030(.a(new_n59_), .b(x12), .c(new_n81_), .O(z03));
  inv1   g031(.a(x16), .O(new_n83_));
  oai21  g032(.a(new_n76_), .b(x19), .c(new_n62_), .O(new_n84_));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  aoi22  g034(.a(new_n85_), .b(new_n62_), .c(new_n84_), .d(x23), .O(new_n86_));
  inv1   g035(.a(x11), .O(new_n87_));
  aoi21  g036(.a(new_n83_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n83_), .c(new_n88_), .O(z04));
  oai21  g038(.a(new_n85_), .b(x19), .c(new_n62_), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n76_), .c(new_n62_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n52_), .O(new_n93_));
  aoi21  g042(.a(new_n90_), .b(x24), .c(new_n93_), .O(new_n94_));
  oai22  g043(.a(new_n94_), .b(new_n83_), .c(new_n59_), .d(x10), .O(z05));
  inv1   g044(.a(x25), .O(new_n96_));
  inv1   g045(.a(x19), .O(new_n97_));
  nand2  g046(.a(new_n91_), .b(new_n76_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n62_), .c(new_n96_), .O(new_n100_));
  nor2   g049(.a(x23), .b(x22), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n101_), .c(new_n69_), .d(new_n54_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n52_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n100_), .c(x16), .O(new_n105_));
  oai21  g054(.a(new_n59_), .b(x09), .c(new_n105_), .O(z06));
  inv1   g055(.a(x26), .O(new_n107_));
  inv1   g056(.a(x23), .O(new_n108_));
  nand3  g057(.a(new_n102_), .b(new_n76_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n97_), .O(new_n110_));
  aoi21  g059(.a(new_n110_), .b(new_n62_), .c(new_n107_), .O(new_n111_));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n101_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n70_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n111_), .c(x16), .O(new_n115_));
  inv1   g064(.a(x08), .O(new_n116_));
  aoi21  g065(.a(new_n83_), .b(new_n116_), .c(x18), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n115_), .O(z07));
  inv1   g067(.a(x27), .O(new_n119_));
  inv1   g068(.a(x24), .O(new_n120_));
  nor2   g069(.a(x26), .b(x25), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n101_), .c(new_n120_), .d(new_n67_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n97_), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n62_), .c(new_n119_), .O(new_n124_));
  nand3  g073(.a(new_n76_), .b(new_n54_), .c(new_n61_), .O(new_n125_));
  nor2   g074(.a(x27), .b(x26), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n91_), .c(new_n96_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n124_), .c(x16), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  aoi21  g079(.a(new_n83_), .b(new_n130_), .c(x18), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(z08));
  inv1   g081(.a(x28), .O(new_n133_));
  nand4  g082(.a(new_n126_), .b(new_n102_), .c(new_n101_), .d(new_n67_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n97_), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n62_), .c(new_n133_), .O(new_n136_));
  nand2  g085(.a(new_n102_), .b(new_n108_), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n107_), .O(new_n139_));
  nor3   g088(.a(new_n139_), .b(new_n125_), .c(new_n137_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n136_), .c(x16), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  aoi21  g091(.a(new_n83_), .b(new_n142_), .c(x18), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n141_), .O(z09));
  inv1   g093(.a(x29), .O(new_n145_));
  nand2  g094(.a(new_n138_), .b(new_n121_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n98_), .c(new_n97_), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n62_), .c(new_n145_), .O(new_n148_));
  nand2  g097(.a(new_n85_), .b(new_n62_), .O(new_n149_));
  nor2   g098(.a(x29), .b(x28), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n112_), .c(new_n119_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n149_), .c(new_n52_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n148_), .c(x16), .O(new_n153_));
  inv1   g102(.a(x05), .O(new_n154_));
  nand2  g103(.a(new_n58_), .b(new_n154_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n153_), .O(z10));
  inv1   g105(.a(x30), .O(new_n157_));
  nand3  g106(.a(new_n150_), .b(new_n121_), .c(new_n119_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n98_), .c(new_n97_), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n62_), .c(new_n157_), .O(new_n160_));
  nor2   g109(.a(x30), .b(x29), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n138_), .c(new_n112_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n149_), .c(new_n52_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n160_), .c(x16), .O(new_n164_));
  inv1   g113(.a(x04), .O(new_n165_));
  nand2  g114(.a(new_n58_), .b(new_n165_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n164_), .O(z11));
  inv1   g116(.a(x31), .O(new_n168_));
  nand3  g117(.a(new_n161_), .b(new_n126_), .c(new_n133_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n109_), .c(new_n97_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n62_), .c(new_n168_), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n150_), .c(new_n126_), .d(new_n96_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n92_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n171_), .c(x16), .O(new_n175_));
  inv1   g124(.a(x03), .O(new_n176_));
  aoi21  g125(.a(new_n83_), .b(new_n176_), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z12));
  inv1   g127(.a(x32), .O(new_n179_));
  nand4  g128(.a(new_n172_), .b(new_n138_), .c(new_n145_), .d(new_n107_), .O(new_n180_));
  oai21  g129(.a(new_n180_), .b(new_n109_), .c(new_n97_), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n62_), .c(new_n179_), .O(new_n182_));
  nor2   g131(.a(x32), .b(x31), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n161_), .c(new_n138_), .d(new_n121_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n92_), .c(new_n52_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n182_), .c(x16), .O(new_n186_));
  inv1   g135(.a(x02), .O(new_n187_));
  nand2  g136(.a(new_n58_), .b(new_n187_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n186_), .O(z13));
  inv1   g138(.a(x33), .O(new_n190_));
  nand4  g139(.a(new_n183_), .b(new_n150_), .c(new_n157_), .d(new_n119_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n122_), .c(new_n97_), .O(new_n192_));
  aoi21  g141(.a(new_n192_), .b(new_n62_), .c(new_n190_), .O(new_n193_));
  nor2   g142(.a(x33), .b(x32), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n172_), .c(new_n150_), .d(new_n126_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n103_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n193_), .c(x16), .O(new_n197_));
  inv1   g146(.a(x01), .O(new_n198_));
  aoi21  g147(.a(new_n83_), .b(new_n198_), .c(x18), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n197_), .O(z14));
  inv1   g149(.a(x34), .O(new_n201_));
  nand4  g150(.a(new_n194_), .b(new_n172_), .c(new_n150_), .d(new_n119_), .O(new_n202_));
  oai21  g151(.a(new_n202_), .b(new_n122_), .c(new_n97_), .O(new_n203_));
  aoi21  g152(.a(new_n203_), .b(new_n62_), .c(new_n201_), .O(new_n204_));
  nor3   g153(.a(x34), .b(x33), .c(x32), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n172_), .c(new_n150_), .d(new_n126_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n103_), .c(new_n52_), .O(new_n207_));
  oai21  g156(.a(new_n207_), .b(new_n204_), .c(x16), .O(new_n208_));
  inv1   g157(.a(x00), .O(new_n209_));
  nand2  g158(.a(new_n58_), .b(new_n209_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n208_), .O(z15));
endmodule


