// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_;
  inv1   g000(.a(x05), .O(new_n52_));
  nor2   g001(.a(x11), .b(new_n52_), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  nor2   g004(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n53_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n60_), .b(new_n67_), .c(x18), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n66_), .c(new_n53_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nand3  g019(.a(new_n57_), .b(new_n70_), .c(new_n63_), .O(new_n71_));
  oai21  g020(.a(new_n65_), .b(new_n70_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  aoi21  g023(.a(new_n60_), .b(new_n74_), .c(x18), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n73_), .c(new_n53_), .O(z02));
  oai21  g025(.a(x11), .b(new_n52_), .c(x18), .O(new_n77_));
  inv1   g026(.a(x22), .O(new_n78_));
  inv1   g027(.a(new_n71_), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n57_), .c(new_n63_), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n78_), .c(new_n81_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(x16), .O(new_n83_));
  nor2   g032(.a(x16), .b(x12), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n53_), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n83_), .c(new_n77_), .O(z03));
  inv1   g035(.a(x11), .O(new_n87_));
  oai21  g036(.a(new_n60_), .b(x05), .c(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n81_), .b(x23), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n65_), .c(new_n70_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n88_), .c(new_n77_), .O(z04));
  inv1   g043(.a(x24), .O(new_n95_));
  inv1   g044(.a(new_n91_), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n80_), .c(new_n65_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n95_), .c(new_n98_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x16), .O(new_n100_));
  nor2   g049(.a(x16), .b(x10), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n53_), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n100_), .c(new_n77_), .O(z05));
  and2   g052(.a(new_n98_), .b(x25), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n90_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n71_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n104_), .c(x16), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n53_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n108_), .c(new_n77_), .O(z06));
  inv1   g060(.a(x26), .O(new_n112_));
  nor3   g061(.a(x26), .b(x25), .c(x24), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n90_), .O(new_n114_));
  oai22  g063(.a(new_n114_), .b(new_n71_), .c(new_n107_), .d(new_n112_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  nor2   g065(.a(x16), .b(x08), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n53_), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n116_), .c(new_n77_), .O(z07));
  inv1   g068(.a(x27), .O(new_n120_));
  nand2  g069(.a(new_n90_), .b(new_n70_), .O(new_n121_));
  nor2   g070(.a(x26), .b(x25), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n95_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n121_), .c(new_n55_), .O(new_n124_));
  aoi21  g073(.a(new_n124_), .b(new_n65_), .c(new_n120_), .O(new_n125_));
  nor3   g074(.a(x27), .b(x26), .c(x25), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n97_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n81_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n125_), .c(x16), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  aoi21  g079(.a(new_n60_), .b(new_n130_), .c(x18), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n129_), .c(new_n53_), .O(z08));
  inv1   g081(.a(x28), .O(new_n133_));
  nor2   g082(.a(x27), .b(x26), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n105_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n121_), .c(new_n55_), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n65_), .c(new_n133_), .O(new_n137_));
  inv1   g086(.a(x23), .O(new_n138_));
  nand2  g087(.a(new_n105_), .b(new_n138_), .O(new_n139_));
  nor2   g088(.a(x28), .b(x27), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n112_), .O(new_n141_));
  nor3   g090(.a(new_n141_), .b(new_n139_), .c(new_n81_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n137_), .c(x16), .O(new_n143_));
  inv1   g092(.a(x06), .O(new_n144_));
  aoi21  g093(.a(new_n60_), .b(new_n144_), .c(x18), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n143_), .c(new_n53_), .O(z09));
  aoi21  g095(.a(x16), .b(x11), .c(new_n52_), .O(new_n147_));
  nand2  g096(.a(new_n97_), .b(new_n80_), .O(new_n148_));
  nand2  g097(.a(new_n140_), .b(new_n122_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n148_), .c(new_n55_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n65_), .O(new_n151_));
  nor2   g100(.a(x29), .b(x28), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n113_), .c(new_n120_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n91_), .O(new_n154_));
  aoi21  g103(.a(new_n151_), .b(x29), .c(new_n154_), .O(new_n155_));
  inv1   g104(.a(x18), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x16), .O(new_n157_));
  aoi22  g106(.a(new_n157_), .b(new_n52_), .c(x18), .d(x11), .O(new_n158_));
  oai21  g107(.a(new_n155_), .b(new_n147_), .c(new_n158_), .O(z10));
  inv1   g108(.a(x30), .O(new_n160_));
  nor2   g109(.a(x30), .b(x29), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n140_), .c(new_n113_), .O(new_n162_));
  oai22  g111(.a(new_n162_), .b(new_n91_), .c(new_n154_), .d(new_n160_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(x16), .O(new_n164_));
  nor2   g113(.a(x16), .b(x04), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n53_), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n164_), .c(new_n77_), .O(z11));
  oai21  g116(.a(new_n162_), .b(new_n91_), .c(x31), .O(new_n168_));
  nor2   g117(.a(x31), .b(x30), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n152_), .c(new_n126_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n98_), .c(new_n168_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  nor2   g121(.a(x16), .b(x03), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n53_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n172_), .c(new_n77_), .O(z12));
  inv1   g124(.a(x32), .O(new_n176_));
  nand3  g125(.a(new_n105_), .b(new_n80_), .c(new_n138_), .O(new_n177_));
  inv1   g126(.a(x29), .O(new_n178_));
  nand4  g127(.a(new_n169_), .b(new_n140_), .c(new_n178_), .d(new_n112_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n177_), .c(new_n55_), .O(new_n180_));
  aoi21  g129(.a(new_n180_), .b(new_n65_), .c(new_n176_), .O(new_n181_));
  nor2   g130(.a(x32), .b(x31), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n161_), .c(new_n140_), .d(new_n122_), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n98_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n181_), .c(x16), .O(new_n185_));
  inv1   g134(.a(x02), .O(new_n186_));
  aoi21  g135(.a(new_n60_), .b(new_n186_), .c(x18), .O(new_n187_));
  aoi21  g136(.a(new_n187_), .b(new_n185_), .c(new_n53_), .O(z13));
  inv1   g137(.a(x33), .O(new_n189_));
  nand2  g138(.a(new_n152_), .b(new_n134_), .O(new_n190_));
  nand3  g139(.a(new_n169_), .b(new_n189_), .c(new_n176_), .O(new_n191_));
  nor2   g140(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n107_), .O(new_n193_));
  oai21  g142(.a(new_n184_), .b(new_n189_), .c(new_n193_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(x16), .O(new_n195_));
  nor2   g144(.a(x16), .b(x01), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n53_), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n195_), .c(new_n77_), .O(z14));
  nand2  g147(.a(new_n193_), .b(x34), .O(new_n199_));
  inv1   g148(.a(new_n190_), .O(new_n200_));
  nor3   g149(.a(x34), .b(x33), .c(x32), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n200_), .c(new_n169_), .d(new_n107_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n199_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(x16), .O(new_n204_));
  nor2   g153(.a(x16), .b(x00), .O(new_n205_));
  nor2   g154(.a(new_n205_), .b(new_n53_), .O(new_n206_));
  nand3  g155(.a(new_n206_), .b(new_n204_), .c(new_n77_), .O(z15));
endmodule


