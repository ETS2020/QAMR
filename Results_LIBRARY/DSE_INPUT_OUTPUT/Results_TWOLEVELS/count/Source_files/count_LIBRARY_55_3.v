// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x15), .O(new_n52_));
  inv1   g001(.a(x16), .O(new_n53_));
  oai21  g002(.a(x18), .b(new_n53_), .c(new_n52_), .O(new_n54_));
  inv1   g003(.a(x18), .O(new_n55_));
  nand2  g004(.a(x19), .b(x17), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  inv1   g006(.a(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand3  g008(.a(new_n59_), .b(new_n55_), .c(x16), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  inv1   g010(.a(x17), .O(new_n62_));
  inv1   g011(.a(x19), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  aoi21  g015(.a(new_n58_), .b(x20), .c(new_n66_), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n53_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n53_), .c(new_n69_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nand3  g020(.a(new_n57_), .b(new_n71_), .c(new_n64_), .O(new_n72_));
  oai21  g021(.a(new_n66_), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  aoi21  g023(.a(new_n73_), .b(x16), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(x18), .b(new_n52_), .O(new_n76_));
  oai21  g025(.a(new_n75_), .b(x18), .c(new_n76_), .O(z02));
  inv1   g026(.a(x22), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  nor2   g028(.a(x20), .b(x17), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  oai22  g031(.a(new_n82_), .b(x19), .c(new_n66_), .d(new_n78_), .O(new_n83_));
  nor2   g032(.a(x16), .b(x12), .O(new_n84_));
  aoi21  g033(.a(new_n83_), .b(x16), .c(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(x18), .c(new_n76_), .O(z03));
  inv1   g035(.a(x23), .O(new_n87_));
  oai21  g036(.a(new_n81_), .b(x19), .c(new_n64_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n55_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n57_), .c(new_n87_), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n71_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n65_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n90_), .c(x16), .O(new_n94_));
  inv1   g043(.a(x11), .O(new_n95_));
  aoi21  g044(.a(new_n53_), .b(new_n95_), .c(x18), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n94_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  aoi21  g047(.a(new_n92_), .b(new_n63_), .c(new_n65_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n81_), .c(new_n66_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x10), .O(new_n103_));
  aoi21  g052(.a(new_n102_), .b(x16), .c(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(x18), .c(new_n76_), .O(z05));
  nand2  g054(.a(new_n100_), .b(new_n81_), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n63_), .c(x20), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(x18), .c(new_n57_), .O(new_n108_));
  nor2   g057(.a(x25), .b(x24), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n91_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n72_), .O(new_n111_));
  aoi21  g060(.a(new_n108_), .b(x25), .c(new_n111_), .O(new_n112_));
  inv1   g061(.a(x09), .O(new_n113_));
  aoi21  g062(.a(new_n53_), .b(new_n113_), .c(x18), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n53_), .c(new_n114_), .O(z06));
  nor3   g064(.a(x25), .b(x24), .c(x23), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n81_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n63_), .c(x20), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(x18), .c(new_n57_), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n91_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n72_), .O(new_n122_));
  aoi21  g071(.a(new_n119_), .b(x26), .c(new_n122_), .O(new_n123_));
  inv1   g072(.a(x08), .O(new_n124_));
  aoi21  g073(.a(new_n53_), .b(new_n124_), .c(x18), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n53_), .c(new_n125_), .O(z07));
  inv1   g075(.a(x27), .O(new_n127_));
  nor2   g076(.a(x26), .b(x25), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n91_), .c(new_n98_), .d(new_n71_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n63_), .c(new_n65_), .O(new_n130_));
  inv1   g079(.a(x25), .O(new_n131_));
  nand2  g080(.a(new_n81_), .b(new_n64_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n58_), .O(new_n133_));
  nor2   g082(.a(x27), .b(x26), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n133_), .c(new_n100_), .d(new_n131_), .O(new_n135_));
  oai21  g084(.a(new_n130_), .b(new_n127_), .c(new_n135_), .O(new_n136_));
  nor2   g085(.a(x16), .b(x07), .O(new_n137_));
  aoi21  g086(.a(new_n136_), .b(x16), .c(new_n137_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(x18), .c(new_n76_), .O(z08));
  inv1   g088(.a(x28), .O(new_n140_));
  nor3   g089(.a(x23), .b(x22), .c(x21), .O(new_n141_));
  nand3  g090(.a(new_n134_), .b(new_n109_), .c(new_n141_), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n63_), .c(new_n65_), .O(new_n143_));
  nor3   g092(.a(x28), .b(x27), .c(x26), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n133_), .c(new_n116_), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n140_), .c(new_n145_), .O(new_n146_));
  nor2   g095(.a(x16), .b(x06), .O(new_n147_));
  aoi21  g096(.a(new_n146_), .b(x16), .c(new_n147_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(x18), .c(new_n76_), .O(z09));
  nor2   g098(.a(x28), .b(x27), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n128_), .c(new_n100_), .d(new_n81_), .O(new_n151_));
  aoi21  g100(.a(new_n151_), .b(new_n63_), .c(x20), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(x18), .c(new_n57_), .O(new_n153_));
  nor3   g102(.a(x29), .b(x28), .c(x27), .O(new_n154_));
  and2   g103(.a(new_n154_), .b(new_n120_), .O(new_n155_));
  aoi22  g104(.a(new_n155_), .b(new_n93_), .c(new_n153_), .d(x29), .O(new_n156_));
  inv1   g105(.a(x05), .O(new_n157_));
  aoi21  g106(.a(new_n53_), .b(new_n157_), .c(x18), .O(new_n158_));
  oai21  g107(.a(new_n156_), .b(new_n53_), .c(new_n158_), .O(z10));
  inv1   g108(.a(x30), .O(new_n160_));
  nand4  g109(.a(new_n154_), .b(new_n128_), .c(new_n100_), .d(new_n81_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n63_), .c(new_n65_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n150_), .c(new_n120_), .d(new_n93_), .O(new_n164_));
  oai21  g113(.a(new_n162_), .b(new_n160_), .c(new_n164_), .O(new_n165_));
  nor2   g114(.a(x16), .b(x04), .O(new_n166_));
  aoi21  g115(.a(new_n165_), .b(x16), .c(new_n166_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(x18), .c(new_n76_), .O(z11));
  nor3   g117(.a(x30), .b(x29), .c(x28), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n134_), .c(new_n116_), .d(new_n81_), .O(new_n170_));
  aoi21  g119(.a(new_n170_), .b(new_n63_), .c(x20), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(x18), .c(new_n57_), .O(new_n172_));
  nor2   g121(.a(x29), .b(x28), .O(new_n173_));
  nor2   g122(.a(x31), .b(x30), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n173_), .c(new_n134_), .d(new_n131_), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n101_), .O(new_n176_));
  aoi21  g125(.a(new_n172_), .b(x31), .c(new_n176_), .O(new_n177_));
  inv1   g126(.a(x03), .O(new_n178_));
  aoi21  g127(.a(new_n53_), .b(new_n178_), .c(x18), .O(new_n179_));
  oai21  g128(.a(new_n177_), .b(new_n53_), .c(new_n179_), .O(z12));
  inv1   g129(.a(x32), .O(new_n181_));
  nor3   g130(.a(x31), .b(x30), .c(x29), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n144_), .c(new_n116_), .d(new_n81_), .O(new_n183_));
  aoi21  g132(.a(new_n183_), .b(new_n63_), .c(new_n65_), .O(new_n184_));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n163_), .c(new_n150_), .d(new_n128_), .O(new_n186_));
  oai22  g135(.a(new_n186_), .b(new_n101_), .c(new_n184_), .d(new_n181_), .O(new_n187_));
  nor2   g136(.a(x16), .b(x02), .O(new_n188_));
  aoi21  g137(.a(new_n187_), .b(x16), .c(new_n188_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(x18), .c(new_n76_), .O(z13));
  inv1   g139(.a(x33), .O(new_n191_));
  nor3   g140(.a(x32), .b(x31), .c(x30), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n154_), .c(new_n120_), .d(new_n141_), .O(new_n193_));
  aoi21  g142(.a(new_n193_), .b(new_n63_), .c(new_n65_), .O(new_n194_));
  nand2  g143(.a(new_n173_), .b(new_n134_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  nor2   g145(.a(x33), .b(x32), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n196_), .c(new_n174_), .d(new_n111_), .O(new_n198_));
  oai21  g147(.a(new_n194_), .b(new_n191_), .c(new_n198_), .O(new_n199_));
  nor2   g148(.a(x16), .b(x01), .O(new_n200_));
  aoi21  g149(.a(new_n199_), .b(x16), .c(new_n200_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(x18), .c(new_n76_), .O(z14));
  nand4  g151(.a(new_n197_), .b(new_n174_), .c(new_n173_), .d(new_n127_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n129_), .c(new_n63_), .O(new_n204_));
  aoi21  g153(.a(new_n204_), .b(new_n64_), .c(x18), .O(new_n205_));
  oai21  g154(.a(new_n205_), .b(new_n58_), .c(x34), .O(new_n206_));
  nor3   g155(.a(x34), .b(x33), .c(x32), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n196_), .c(new_n174_), .d(new_n111_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n206_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(x16), .O(new_n210_));
  inv1   g159(.a(x00), .O(new_n211_));
  aoi21  g160(.a(new_n53_), .b(new_n211_), .c(x18), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n210_), .O(z15));
endmodule


