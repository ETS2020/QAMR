// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_;
  nor2   g000(.a(x10), .b(x09), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n57_), .c(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g014(.a(x14), .O(new_n66_));
  aoi21  g015(.a(new_n59_), .b(new_n66_), .c(x18), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n65_), .c(new_n52_), .O(z01));
  oai21  g017(.a(x10), .b(x09), .c(x18), .O(new_n69_));
  inv1   g018(.a(x21), .O(new_n70_));
  nand3  g019(.a(new_n56_), .b(new_n70_), .c(new_n62_), .O(new_n71_));
  oai21  g020(.a(new_n64_), .b(new_n70_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  nor2   g023(.a(new_n74_), .b(new_n52_), .O(new_n75_));
  nand3  g024(.a(new_n75_), .b(new_n73_), .c(new_n69_), .O(z02));
  nor2   g025(.a(x22), .b(x21), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n56_), .c(new_n62_), .O(new_n78_));
  inv1   g027(.a(new_n78_), .O(new_n79_));
  aoi21  g028(.a(new_n71_), .b(x22), .c(new_n79_), .O(new_n80_));
  or2    g029(.a(new_n80_), .b(new_n59_), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n52_), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n81_), .c(new_n69_), .O(z03));
  inv1   g033(.a(x23), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n64_), .c(new_n70_), .O(new_n87_));
  oai21  g036(.a(new_n79_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x16), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n52_), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n89_), .c(new_n69_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  nand3  g042(.a(new_n62_), .b(new_n54_), .c(new_n53_), .O(new_n94_));
  nand2  g043(.a(new_n86_), .b(new_n70_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n54_), .c(new_n94_), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n77_), .c(new_n64_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n93_), .c(new_n98_), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(x16), .c(x18), .O(new_n100_));
  inv1   g049(.a(x10), .O(new_n101_));
  nand3  g050(.a(new_n59_), .b(new_n101_), .c(x09), .O(new_n102_));
  oai21  g051(.a(new_n100_), .b(new_n52_), .c(new_n102_), .O(z05));
  inv1   g052(.a(x25), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n86_), .O(new_n106_));
  oai22  g055(.a(new_n106_), .b(new_n71_), .c(new_n96_), .d(new_n104_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(x16), .c(x18), .O(new_n108_));
  inv1   g057(.a(x09), .O(new_n109_));
  nand2  g058(.a(new_n59_), .b(new_n109_), .O(new_n110_));
  nand2  g059(.a(x25), .b(x24), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n59_), .c(new_n110_), .O(new_n112_));
  nor2   g061(.a(new_n59_), .b(new_n109_), .O(new_n113_));
  nor2   g062(.a(new_n111_), .b(x19), .O(new_n114_));
  aoi22  g063(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(x10), .O(new_n115_));
  oai21  g064(.a(new_n108_), .b(new_n52_), .c(new_n115_), .O(z06));
  inv1   g065(.a(x26), .O(new_n117_));
  nor2   g066(.a(new_n106_), .b(new_n71_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n86_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n71_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n119_), .c(x16), .O(new_n123_));
  nor2   g072(.a(x16), .b(x08), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n52_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n123_), .c(new_n69_), .O(z07));
  inv1   g075(.a(x27), .O(new_n127_));
  nor2   g076(.a(new_n122_), .b(new_n127_), .O(new_n128_));
  nor3   g077(.a(x27), .b(x26), .c(x25), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n97_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n78_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  nor2   g081(.a(x16), .b(x07), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n52_), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n132_), .c(new_n69_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  nor2   g085(.a(new_n131_), .b(new_n136_), .O(new_n137_));
  nand2  g086(.a(new_n105_), .b(new_n85_), .O(new_n138_));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n117_), .O(new_n140_));
  nor3   g089(.a(new_n140_), .b(new_n138_), .c(new_n78_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n137_), .c(x16), .O(new_n142_));
  nor2   g091(.a(x16), .b(x06), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n52_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n142_), .c(new_n69_), .O(z09));
  inv1   g094(.a(x29), .O(new_n146_));
  nor2   g095(.a(x29), .b(x28), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n120_), .c(new_n127_), .O(new_n148_));
  oai22  g097(.a(new_n148_), .b(new_n87_), .c(new_n141_), .d(new_n146_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  nor2   g099(.a(x16), .b(x05), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n52_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n150_), .c(new_n69_), .O(z10));
  inv1   g102(.a(x30), .O(new_n154_));
  nand2  g103(.a(new_n97_), .b(new_n77_), .O(new_n155_));
  nor2   g104(.a(x26), .b(x25), .O(new_n156_));
  nand3  g105(.a(new_n147_), .b(new_n156_), .c(new_n127_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n155_), .c(new_n54_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n64_), .c(new_n154_), .O(new_n159_));
  nor2   g108(.a(x30), .b(x29), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n139_), .c(new_n120_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n87_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n159_), .c(x16), .O(new_n163_));
  inv1   g112(.a(x04), .O(new_n164_));
  aoi21  g113(.a(new_n59_), .b(new_n164_), .c(x18), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n163_), .c(new_n52_), .O(z11));
  inv1   g115(.a(x31), .O(new_n167_));
  nor2   g116(.a(x31), .b(x30), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n147_), .c(new_n129_), .O(new_n169_));
  oai22  g118(.a(new_n169_), .b(new_n98_), .c(new_n162_), .d(new_n167_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  nor2   g120(.a(x16), .b(x03), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n52_), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n171_), .c(new_n69_), .O(z12));
  inv1   g123(.a(x32), .O(new_n175_));
  nand3  g124(.a(new_n105_), .b(new_n77_), .c(new_n85_), .O(new_n176_));
  nand4  g125(.a(new_n168_), .b(new_n139_), .c(new_n146_), .d(new_n117_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n176_), .c(new_n54_), .O(new_n178_));
  aoi21  g127(.a(new_n178_), .b(new_n64_), .c(new_n175_), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n160_), .c(new_n139_), .d(new_n156_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n98_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n179_), .c(x16), .O(new_n183_));
  inv1   g132(.a(x02), .O(new_n184_));
  aoi21  g133(.a(new_n59_), .b(new_n184_), .c(x18), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n183_), .c(new_n52_), .O(z13));
  inv1   g135(.a(x33), .O(new_n187_));
  nand3  g136(.a(new_n147_), .b(new_n127_), .c(new_n117_), .O(new_n188_));
  nand3  g137(.a(new_n168_), .b(new_n187_), .c(new_n175_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n188_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n118_), .O(new_n191_));
  oai21  g140(.a(new_n182_), .b(new_n187_), .c(new_n191_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(x16), .O(new_n193_));
  nor2   g142(.a(x16), .b(x01), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n52_), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n193_), .c(new_n69_), .O(z14));
  nand2  g145(.a(new_n191_), .b(x34), .O(new_n197_));
  inv1   g146(.a(new_n188_), .O(new_n198_));
  nor3   g147(.a(x34), .b(x33), .c(x32), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n198_), .c(new_n168_), .d(new_n118_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n197_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(x16), .O(new_n202_));
  nor2   g151(.a(x16), .b(x00), .O(new_n203_));
  nor2   g152(.a(new_n203_), .b(new_n52_), .O(new_n204_));
  nand3  g153(.a(new_n204_), .b(new_n202_), .c(new_n69_), .O(z15));
endmodule


