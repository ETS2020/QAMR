// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x11), .O(new_n52_));
  inv1   g001(.a(x18), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  inv1   g004(.a(new_n55_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nor2   g006(.a(x16), .b(x15), .O(new_n58_));
  aoi21  g007(.a(new_n57_), .b(x16), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n59_), .b(new_n52_), .c(new_n53_), .O(z00));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n56_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(x11), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n53_), .O(z01));
  nand2  g019(.a(x20), .b(x11), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n64_), .c(new_n63_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  aoi22  g022(.a(new_n73_), .b(new_n55_), .c(new_n72_), .d(x21), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  nor3   g024(.a(new_n75_), .b(x18), .c(new_n52_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n61_), .c(new_n76_), .O(z02));
  inv1   g026(.a(x22), .O(new_n78_));
  oai21  g027(.a(x21), .b(x20), .c(x11), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n55_), .c(new_n78_), .O(new_n80_));
  nor3   g029(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  and2   g030(.a(new_n81_), .b(new_n55_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n61_), .b(new_n84_), .c(x18), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n83_), .c(x11), .O(z03));
  nor2   g035(.a(x22), .b(x21), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(x19), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n66_), .c(x23), .O(new_n89_));
  inv1   g038(.a(x21), .O(new_n90_));
  inv1   g039(.a(new_n66_), .O(new_n91_));
  nor2   g040(.a(x23), .b(x22), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(x16), .c(x11), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n53_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  nand2  g046(.a(new_n92_), .b(new_n90_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n64_), .c(new_n66_), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n87_), .O(new_n101_));
  oai22  g050(.a(new_n101_), .b(new_n66_), .c(new_n99_), .d(new_n97_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x10), .O(new_n103_));
  aoi21  g052(.a(new_n102_), .b(x16), .c(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n52_), .c(new_n53_), .O(z05));
  inv1   g054(.a(x25), .O(new_n106_));
  aoi21  g055(.a(new_n101_), .b(new_n64_), .c(new_n66_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n92_), .c(new_n73_), .d(new_n55_), .O(new_n109_));
  oai21  g058(.a(new_n107_), .b(new_n106_), .c(new_n109_), .O(new_n110_));
  nor2   g059(.a(x16), .b(x09), .O(new_n111_));
  aoi21  g060(.a(new_n110_), .b(x16), .c(new_n111_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n52_), .c(new_n53_), .O(z06));
  inv1   g062(.a(x26), .O(new_n114_));
  nor3   g063(.a(x25), .b(x24), .c(x23), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n81_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x11), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n55_), .c(new_n114_), .O(new_n118_));
  nand2  g067(.a(new_n73_), .b(new_n55_), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n92_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n118_), .c(x16), .O(new_n123_));
  inv1   g072(.a(x08), .O(new_n124_));
  aoi21  g073(.a(new_n61_), .b(new_n124_), .c(x18), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n123_), .c(x11), .O(z07));
  inv1   g075(.a(x27), .O(new_n127_));
  nor2   g076(.a(x26), .b(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n100_), .c(new_n81_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(x11), .c(new_n56_), .O(new_n130_));
  nor3   g079(.a(x27), .b(x26), .c(x25), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n100_), .c(new_n82_), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n127_), .c(new_n132_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g083(.a(x07), .O(new_n135_));
  aoi21  g084(.a(new_n61_), .b(new_n135_), .c(x18), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n134_), .c(x11), .O(z08));
  inv1   g086(.a(x28), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n108_), .c(new_n92_), .d(new_n90_), .O(new_n140_));
  aoi21  g089(.a(new_n140_), .b(new_n64_), .c(new_n66_), .O(new_n141_));
  nor3   g090(.a(x28), .b(x27), .c(x26), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n115_), .c(new_n82_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n138_), .c(new_n143_), .O(new_n144_));
  nor2   g093(.a(x16), .b(x06), .O(new_n145_));
  aoi21  g094(.a(new_n144_), .b(x16), .c(new_n145_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n52_), .c(new_n53_), .O(z09));
  inv1   g096(.a(x29), .O(new_n148_));
  nand2  g097(.a(new_n92_), .b(new_n73_), .O(new_n149_));
  nand2  g098(.a(new_n142_), .b(new_n108_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n149_), .c(x11), .O(new_n151_));
  aoi21  g100(.a(new_n151_), .b(new_n55_), .c(new_n148_), .O(new_n152_));
  nor3   g101(.a(x29), .b(x28), .c(x27), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n120_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n93_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n152_), .c(x16), .O(new_n156_));
  inv1   g105(.a(x05), .O(new_n157_));
  aoi21  g106(.a(new_n61_), .b(new_n157_), .c(x18), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n156_), .c(x11), .O(z10));
  inv1   g108(.a(x30), .O(new_n160_));
  nand3  g109(.a(new_n100_), .b(new_n73_), .c(new_n78_), .O(new_n161_));
  nand2  g110(.a(new_n153_), .b(new_n128_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n161_), .c(x11), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n55_), .c(new_n160_), .O(new_n164_));
  nor2   g113(.a(x28), .b(x27), .O(new_n165_));
  nor2   g114(.a(x30), .b(x29), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n165_), .c(new_n120_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n93_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n164_), .c(x16), .O(new_n169_));
  inv1   g118(.a(x04), .O(new_n170_));
  aoi21  g119(.a(new_n61_), .b(new_n170_), .c(x18), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n169_), .c(x11), .O(z11));
  inv1   g121(.a(x31), .O(new_n173_));
  nor3   g122(.a(x30), .b(x29), .c(x28), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n139_), .c(new_n115_), .d(new_n87_), .O(new_n175_));
  aoi21  g124(.a(new_n175_), .b(new_n64_), .c(new_n66_), .O(new_n176_));
  nor2   g125(.a(new_n101_), .b(new_n66_), .O(new_n177_));
  nor2   g126(.a(x29), .b(x28), .O(new_n178_));
  nor2   g127(.a(x31), .b(x30), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n178_), .c(new_n131_), .d(new_n177_), .O(new_n180_));
  oai21  g129(.a(new_n176_), .b(new_n173_), .c(new_n180_), .O(new_n181_));
  nor2   g130(.a(x16), .b(x03), .O(new_n182_));
  aoi21  g131(.a(new_n181_), .b(x16), .c(new_n182_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n52_), .c(new_n53_), .O(z12));
  inv1   g133(.a(x32), .O(new_n185_));
  nor3   g134(.a(x31), .b(x30), .c(x29), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n142_), .c(new_n115_), .d(new_n87_), .O(new_n187_));
  aoi21  g136(.a(new_n187_), .b(new_n64_), .c(new_n66_), .O(new_n188_));
  nand3  g137(.a(new_n166_), .b(new_n185_), .c(new_n173_), .O(new_n189_));
  inv1   g138(.a(new_n189_), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n165_), .c(new_n128_), .d(new_n177_), .O(new_n191_));
  oai21  g140(.a(new_n188_), .b(new_n185_), .c(new_n191_), .O(new_n192_));
  nor2   g141(.a(x16), .b(x02), .O(new_n193_));
  aoi21  g142(.a(new_n192_), .b(x16), .c(new_n193_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n52_), .c(new_n53_), .O(z13));
  inv1   g144(.a(x33), .O(new_n196_));
  nand2  g145(.a(new_n190_), .b(new_n142_), .O(new_n197_));
  oai21  g146(.a(new_n197_), .b(new_n116_), .c(x11), .O(new_n198_));
  aoi21  g147(.a(new_n198_), .b(new_n55_), .c(new_n196_), .O(new_n199_));
  nand2  g148(.a(new_n178_), .b(new_n139_), .O(new_n200_));
  nor2   g149(.a(x33), .b(x32), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n179_), .O(new_n202_));
  nor3   g151(.a(new_n202_), .b(new_n200_), .c(new_n109_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n199_), .c(x16), .O(new_n204_));
  inv1   g153(.a(x01), .O(new_n205_));
  aoi21  g154(.a(new_n61_), .b(new_n205_), .c(x18), .O(new_n206_));
  nand3  g155(.a(new_n206_), .b(new_n204_), .c(x11), .O(z14));
  inv1   g156(.a(x34), .O(new_n208_));
  nand4  g157(.a(new_n128_), .b(new_n92_), .c(new_n97_), .d(new_n90_), .O(new_n209_));
  nand4  g158(.a(new_n201_), .b(new_n179_), .c(new_n178_), .d(new_n127_), .O(new_n210_));
  oai21  g159(.a(new_n210_), .b(new_n209_), .c(new_n64_), .O(new_n211_));
  aoi21  g160(.a(new_n211_), .b(new_n91_), .c(new_n208_), .O(new_n212_));
  nand4  g161(.a(new_n179_), .b(new_n208_), .c(new_n196_), .d(new_n185_), .O(new_n213_));
  nor3   g162(.a(new_n213_), .b(new_n200_), .c(new_n109_), .O(new_n214_));
  oai21  g163(.a(new_n214_), .b(new_n212_), .c(x16), .O(new_n215_));
  inv1   g164(.a(x00), .O(new_n216_));
  nand2  g165(.a(new_n61_), .b(new_n216_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n215_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(x11), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(new_n53_), .O(z15));
endmodule


