// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(new_n55_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x20), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  aoi21  g014(.a(new_n65_), .b(new_n63_), .c(new_n58_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  nand2  g017(.a(x18), .b(x14), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n68_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  inv1   g020(.a(x20), .O(new_n72_));
  nand3  g021(.a(new_n55_), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  oai21  g022(.a(new_n64_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  aoi21  g024(.a(new_n74_), .b(x16), .c(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(x18), .c(new_n69_), .O(z02));
  inv1   g026(.a(x22), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  nor2   g028(.a(x20), .b(x17), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  oai22  g031(.a(new_n82_), .b(x19), .c(new_n64_), .d(new_n78_), .O(new_n83_));
  nor2   g032(.a(x16), .b(x12), .O(new_n84_));
  aoi21  g033(.a(new_n83_), .b(x16), .c(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(x18), .c(new_n69_), .O(z03));
  oai21  g035(.a(new_n81_), .b(x19), .c(new_n64_), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n64_), .c(new_n71_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  aoi21  g039(.a(new_n87_), .b(x23), .c(new_n90_), .O(new_n91_));
  inv1   g040(.a(x11), .O(new_n92_));
  aoi21  g041(.a(new_n58_), .b(new_n92_), .c(x18), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n58_), .c(new_n93_), .O(z04));
  aoi21  g043(.a(new_n88_), .b(new_n71_), .c(x19), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n65_), .c(x24), .O(new_n96_));
  nor2   g045(.a(x24), .b(x23), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n81_), .c(new_n64_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x10), .O(new_n100_));
  aoi21  g049(.a(new_n99_), .b(x16), .c(new_n100_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(x18), .c(new_n69_), .O(z05));
  inv1   g051(.a(x25), .O(new_n103_));
  nand2  g052(.a(new_n97_), .b(new_n81_), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n53_), .c(new_n65_), .O(new_n105_));
  nor2   g054(.a(x25), .b(x24), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n73_), .O(new_n108_));
  inv1   g057(.a(new_n108_), .O(new_n109_));
  oai21  g058(.a(new_n105_), .b(new_n103_), .c(new_n109_), .O(new_n110_));
  nor2   g059(.a(x16), .b(x09), .O(new_n111_));
  aoi21  g060(.a(new_n110_), .b(x16), .c(new_n111_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(x18), .c(new_n69_), .O(z06));
  inv1   g062(.a(x26), .O(new_n114_));
  nor3   g063(.a(x25), .b(x24), .c(x23), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n81_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n53_), .c(new_n65_), .O(new_n117_));
  nor3   g066(.a(x26), .b(x25), .c(x24), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n88_), .O(new_n119_));
  oai22  g068(.a(new_n119_), .b(new_n73_), .c(new_n117_), .d(new_n114_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x08), .O(new_n121_));
  aoi21  g070(.a(new_n120_), .b(x16), .c(new_n121_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(x18), .c(new_n69_), .O(z07));
  inv1   g072(.a(x27), .O(new_n124_));
  inv1   g073(.a(x24), .O(new_n125_));
  nor2   g074(.a(x26), .b(x25), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n88_), .c(new_n125_), .d(new_n71_), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n53_), .c(new_n65_), .O(new_n128_));
  nand2  g077(.a(new_n81_), .b(new_n72_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n62_), .O(new_n130_));
  nor3   g079(.a(x27), .b(x26), .c(x25), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n130_), .c(new_n97_), .O(new_n132_));
  oai21  g081(.a(new_n128_), .b(new_n124_), .c(new_n132_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x07), .O(new_n134_));
  aoi21  g083(.a(new_n133_), .b(x16), .c(new_n134_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(x18), .c(new_n69_), .O(z08));
  inv1   g085(.a(x28), .O(new_n137_));
  nor2   g086(.a(x27), .b(x26), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n106_), .c(new_n88_), .d(new_n71_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n53_), .c(new_n65_), .O(new_n140_));
  nor2   g089(.a(x28), .b(x27), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n130_), .c(new_n115_), .d(new_n114_), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n137_), .c(new_n142_), .O(new_n143_));
  nor2   g092(.a(x16), .b(x06), .O(new_n144_));
  aoi21  g093(.a(new_n143_), .b(x16), .c(new_n144_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(x18), .c(new_n69_), .O(z09));
  inv1   g095(.a(x29), .O(new_n147_));
  nand4  g096(.a(new_n141_), .b(new_n126_), .c(new_n97_), .d(new_n81_), .O(new_n148_));
  aoi21  g097(.a(new_n148_), .b(new_n53_), .c(new_n65_), .O(new_n149_));
  nor3   g098(.a(x29), .b(x28), .c(x27), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n118_), .O(new_n151_));
  oai22  g100(.a(new_n151_), .b(new_n89_), .c(new_n149_), .d(new_n147_), .O(new_n152_));
  nor2   g101(.a(x16), .b(x05), .O(new_n153_));
  aoi21  g102(.a(new_n152_), .b(x16), .c(new_n153_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(x18), .c(new_n69_), .O(z10));
  inv1   g104(.a(x30), .O(new_n156_));
  nand2  g105(.a(new_n150_), .b(new_n126_), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n104_), .c(new_n53_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n64_), .c(new_n156_), .O(new_n159_));
  nor2   g108(.a(x30), .b(x29), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n141_), .c(new_n118_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n89_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n159_), .c(x16), .O(new_n163_));
  inv1   g112(.a(x04), .O(new_n164_));
  aoi21  g113(.a(new_n58_), .b(new_n164_), .c(x18), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z11));
  inv1   g115(.a(x31), .O(new_n167_));
  nor3   g116(.a(x30), .b(x29), .c(x28), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n138_), .c(new_n115_), .d(new_n81_), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n53_), .c(new_n65_), .O(new_n170_));
  nor2   g119(.a(x29), .b(x28), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n171_), .c(new_n131_), .O(new_n173_));
  oai22  g122(.a(new_n173_), .b(new_n98_), .c(new_n170_), .d(new_n167_), .O(new_n174_));
  nor2   g123(.a(x16), .b(x03), .O(new_n175_));
  aoi21  g124(.a(new_n174_), .b(x16), .c(new_n175_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(x18), .c(new_n69_), .O(z12));
  inv1   g126(.a(x32), .O(new_n178_));
  nand4  g127(.a(new_n172_), .b(new_n141_), .c(new_n147_), .d(new_n114_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n116_), .c(new_n53_), .O(new_n180_));
  aoi21  g129(.a(new_n180_), .b(new_n64_), .c(new_n178_), .O(new_n181_));
  nor2   g130(.a(x32), .b(x31), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n160_), .c(new_n141_), .d(new_n126_), .O(new_n183_));
  nor2   g132(.a(new_n183_), .b(new_n98_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n181_), .c(x16), .O(new_n185_));
  inv1   g134(.a(x02), .O(new_n186_));
  aoi21  g135(.a(new_n58_), .b(new_n186_), .c(x18), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n185_), .O(z13));
  inv1   g137(.a(x33), .O(new_n189_));
  nand3  g138(.a(new_n182_), .b(new_n150_), .c(new_n156_), .O(new_n190_));
  oai21  g139(.a(new_n190_), .b(new_n127_), .c(new_n53_), .O(new_n191_));
  aoi21  g140(.a(new_n191_), .b(new_n64_), .c(new_n189_), .O(new_n192_));
  nand2  g141(.a(new_n171_), .b(new_n138_), .O(new_n193_));
  nor2   g142(.a(x33), .b(x32), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n172_), .O(new_n195_));
  nor3   g144(.a(new_n195_), .b(new_n193_), .c(new_n109_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n192_), .c(x16), .O(new_n197_));
  inv1   g146(.a(x01), .O(new_n198_));
  aoi21  g147(.a(new_n58_), .b(new_n198_), .c(x18), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n197_), .O(z14));
  nand4  g149(.a(new_n194_), .b(new_n172_), .c(new_n171_), .d(new_n124_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(new_n127_), .c(new_n53_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n64_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(x34), .O(new_n204_));
  inv1   g153(.a(new_n193_), .O(new_n205_));
  nor3   g154(.a(x34), .b(x33), .c(x32), .O(new_n206_));
  nand4  g155(.a(new_n206_), .b(new_n205_), .c(new_n172_), .d(new_n108_), .O(new_n207_));
  aoi21  g156(.a(new_n207_), .b(new_n204_), .c(new_n58_), .O(new_n208_));
  nor2   g157(.a(x16), .b(x00), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(new_n208_), .c(new_n61_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n69_), .O(z15));
endmodule


