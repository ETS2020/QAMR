// Benchmark "FAU" written by ABC on Wed Aug 19 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x25), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand2  g004(.a(x19), .b(x17), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x16), .b(x15), .O(new_n58_));
  aoi21  g007(.a(new_n57_), .b(x16), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n59_), .b(new_n53_), .c(new_n52_), .O(z00));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n55_), .b(x20), .O(new_n62_));
  inv1   g011(.a(x17), .O(new_n63_));
  inv1   g012(.a(x19), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(x25), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n52_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nand4  g020(.a(x25), .b(new_n71_), .c(new_n65_), .d(new_n64_), .O(new_n72_));
  nor2   g021(.a(new_n72_), .b(x17), .O(new_n73_));
  nor3   g022(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  nor2   g023(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n73_), .c(x16), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nand2  g026(.a(x25), .b(new_n52_), .O(new_n78_));
  nor2   g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n76_), .O(z02));
  inv1   g029(.a(x22), .O(new_n81_));
  nand4  g030(.a(x25), .b(new_n81_), .c(new_n71_), .d(new_n65_), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(x19), .O(new_n83_));
  nor2   g032(.a(x21), .b(x20), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n54_), .c(new_n81_), .O(new_n85_));
  aoi21  g034(.a(new_n83_), .b(new_n63_), .c(new_n85_), .O(new_n86_));
  nor2   g035(.a(x16), .b(x12), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n78_), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n61_), .c(new_n88_), .O(z03));
  inv1   g038(.a(x23), .O(new_n90_));
  nand4  g039(.a(x25), .b(new_n90_), .c(new_n81_), .d(new_n71_), .O(new_n91_));
  inv1   g040(.a(new_n91_), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n93_));
  nor2   g042(.a(x22), .b(x21), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n54_), .c(new_n65_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x23), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(x16), .O(new_n98_));
  nor2   g047(.a(x16), .b(x11), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n78_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n98_), .O(z04));
  nand3  g050(.a(new_n90_), .b(new_n81_), .c(new_n71_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n66_), .c(x24), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n94_), .c(new_n74_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n103_), .c(new_n61_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x10), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n106_), .c(x25), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n52_), .O(z05));
  inv1   g058(.a(x24), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n90_), .c(new_n81_), .d(new_n71_), .O(new_n111_));
  inv1   g060(.a(new_n111_), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n113_));
  nor2   g062(.a(x16), .b(x09), .O(new_n114_));
  aoi21  g063(.a(new_n113_), .b(x16), .c(new_n114_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n53_), .c(new_n52_), .O(z06));
  inv1   g065(.a(new_n78_), .O(new_n117_));
  nand2  g066(.a(x26), .b(x16), .O(new_n118_));
  inv1   g067(.a(x08), .O(new_n119_));
  nand2  g068(.a(new_n61_), .b(new_n119_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n118_), .c(new_n117_), .O(z07));
  nand2  g070(.a(x27), .b(x16), .O(new_n122_));
  inv1   g071(.a(x07), .O(new_n123_));
  nand2  g072(.a(new_n61_), .b(new_n123_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(new_n117_), .O(z08));
  nor2   g074(.a(x16), .b(x06), .O(new_n126_));
  and2   g075(.a(x28), .b(x16), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n126_), .c(x25), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n52_), .O(z09));
  nand2  g078(.a(x29), .b(x16), .O(new_n130_));
  inv1   g079(.a(x05), .O(new_n131_));
  nand2  g080(.a(new_n61_), .b(new_n131_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n130_), .c(new_n117_), .O(z10));
  nor2   g082(.a(x16), .b(x04), .O(new_n134_));
  and2   g083(.a(x30), .b(x16), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n134_), .c(x25), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n52_), .O(z11));
  nor2   g086(.a(x16), .b(x03), .O(new_n138_));
  and2   g087(.a(x31), .b(x16), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n138_), .c(x25), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n52_), .O(z12));
  nor2   g090(.a(x16), .b(x02), .O(new_n142_));
  and2   g091(.a(x32), .b(x16), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n142_), .c(x25), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n52_), .O(z13));
  nor2   g094(.a(x16), .b(x01), .O(new_n146_));
  and2   g095(.a(x33), .b(x16), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n146_), .c(x25), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n52_), .O(z14));
  nand2  g098(.a(x34), .b(x16), .O(new_n150_));
  inv1   g099(.a(x00), .O(new_n151_));
  nand2  g100(.a(new_n61_), .b(new_n151_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n150_), .c(new_n117_), .O(z15));
endmodule


