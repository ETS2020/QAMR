// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n148_,
    new_n149_, new_n150_, new_n152_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_;
  nor2   g000(.a(x25), .b(x20), .O(new_n52_));
  inv1   g001(.a(new_n52_), .O(new_n53_));
  nor2   g002(.a(x16), .b(x15), .O(new_n54_));
  inv1   g003(.a(x18), .O(new_n55_));
  nand3  g004(.a(x19), .b(x17), .c(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n54_), .c(new_n53_), .O(new_n58_));
  inv1   g007(.a(x17), .O(new_n59_));
  inv1   g008(.a(x19), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  inv1   g010(.a(x25), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g012(.a(new_n63_), .b(new_n60_), .c(new_n59_), .d(x16), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n58_), .O(z00));
  inv1   g014(.a(x14), .O(new_n66_));
  inv1   g015(.a(x16), .O(new_n67_));
  aoi21  g016(.a(new_n67_), .b(new_n66_), .c(x18), .O(new_n68_));
  nor2   g017(.a(x19), .b(x17), .O(new_n69_));
  nand4  g018(.a(x25), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n70_));
  oai21  g019(.a(new_n69_), .b(new_n61_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g021(.a(new_n68_), .b(new_n52_), .c(new_n72_), .O(z01));
  inv1   g022(.a(x21), .O(new_n74_));
  nand4  g023(.a(x25), .b(new_n74_), .c(new_n61_), .d(new_n60_), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(x17), .O(new_n76_));
  nor2   g025(.a(x20), .b(x19), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n59_), .c(new_n74_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  inv1   g028(.a(x13), .O(new_n80_));
  nand2  g029(.a(new_n67_), .b(new_n80_), .O(new_n81_));
  nand4  g030(.a(new_n81_), .b(new_n79_), .c(new_n53_), .d(new_n55_), .O(z02));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n67_), .b(new_n83_), .c(x18), .O(new_n84_));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  nand4  g034(.a(new_n85_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n86_));
  nand3  g035(.a(new_n74_), .b(new_n60_), .c(new_n59_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x22), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(new_n62_), .O(new_n89_));
  inv1   g038(.a(x22), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n61_), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  oai21  g041(.a(new_n84_), .b(new_n52_), .c(new_n92_), .O(z03));
  oai21  g042(.a(x16), .b(x11), .c(new_n55_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n53_), .O(new_n95_));
  nor3   g044(.a(x23), .b(x22), .c(x21), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n97_));
  nand2  g046(.a(new_n85_), .b(new_n69_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(x23), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n97_), .c(new_n62_), .O(new_n100_));
  inv1   g049(.a(x23), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n61_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n100_), .c(x16), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n95_), .O(z04));
  oai21  g053(.a(x16), .b(x10), .c(new_n55_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n53_), .O(new_n106_));
  inv1   g055(.a(x24), .O(new_n107_));
  nor2   g056(.a(x23), .b(x22), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n69_), .c(new_n74_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(x25), .c(x20), .O(new_n110_));
  nor2   g059(.a(x21), .b(x20), .O(new_n111_));
  nor2   g060(.a(new_n62_), .b(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n111_), .c(new_n108_), .d(new_n69_), .O(new_n113_));
  oai21  g062(.a(new_n110_), .b(new_n107_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n106_), .O(z05));
  nand4  g065(.a(new_n107_), .b(new_n101_), .c(new_n90_), .d(new_n74_), .O(new_n117_));
  inv1   g066(.a(new_n117_), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(x25), .c(x16), .O(new_n120_));
  inv1   g069(.a(x09), .O(new_n121_));
  nand2  g070(.a(new_n67_), .b(new_n121_), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n120_), .c(new_n53_), .d(new_n55_), .O(z06));
  inv1   g072(.a(x08), .O(new_n124_));
  nand2  g073(.a(new_n67_), .b(new_n124_), .O(new_n125_));
  aoi21  g074(.a(x26), .b(x16), .c(x18), .O(new_n126_));
  aoi21  g075(.a(new_n126_), .b(new_n125_), .c(new_n52_), .O(z07));
  inv1   g076(.a(x07), .O(new_n128_));
  nand2  g077(.a(new_n67_), .b(new_n128_), .O(new_n129_));
  aoi21  g078(.a(x27), .b(x16), .c(x18), .O(new_n130_));
  aoi21  g079(.a(new_n130_), .b(new_n129_), .c(new_n52_), .O(z08));
  inv1   g080(.a(x06), .O(new_n132_));
  nand2  g081(.a(new_n67_), .b(new_n132_), .O(new_n133_));
  aoi21  g082(.a(x28), .b(x16), .c(x18), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n133_), .c(new_n52_), .O(z09));
  inv1   g084(.a(x05), .O(new_n136_));
  nand2  g085(.a(new_n67_), .b(new_n136_), .O(new_n137_));
  aoi21  g086(.a(x29), .b(x16), .c(x18), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n137_), .c(new_n52_), .O(z10));
  inv1   g088(.a(x04), .O(new_n140_));
  nand2  g089(.a(new_n67_), .b(new_n140_), .O(new_n141_));
  aoi21  g090(.a(x30), .b(x16), .c(x18), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n141_), .c(new_n52_), .O(z11));
  inv1   g092(.a(x03), .O(new_n144_));
  nand2  g093(.a(new_n67_), .b(new_n144_), .O(new_n145_));
  aoi21  g094(.a(x31), .b(x16), .c(x18), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n145_), .c(new_n52_), .O(z12));
  inv1   g096(.a(x02), .O(new_n148_));
  nand2  g097(.a(new_n67_), .b(new_n148_), .O(new_n149_));
  aoi21  g098(.a(x32), .b(x16), .c(x18), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n149_), .c(new_n52_), .O(z13));
  inv1   g100(.a(x01), .O(new_n152_));
  nand2  g101(.a(new_n67_), .b(new_n152_), .O(new_n153_));
  aoi21  g102(.a(x33), .b(x16), .c(x18), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n153_), .c(new_n52_), .O(z14));
  inv1   g104(.a(x00), .O(new_n156_));
  nand2  g105(.a(new_n67_), .b(new_n156_), .O(new_n157_));
  aoi21  g106(.a(x34), .b(x16), .c(x18), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n157_), .c(new_n52_), .O(z15));
endmodule


