// Benchmark "FAU" written by ABC on Wed Aug 19 19:39:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  inv1   g000(.a(x07), .O(new_n52_));
  inv1   g001(.a(x09), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  oai21  g007(.a(new_n58_), .b(new_n57_), .c(x16), .O(new_n59_));
  inv1   g008(.a(x15), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n60_), .c(x18), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n59_), .c(new_n54_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n58_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n56_), .c(new_n55_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  inv1   g017(.a(x14), .O(new_n69_));
  aoi21  g018(.a(new_n61_), .b(new_n69_), .c(x18), .O(new_n70_));
  aoi21  g019(.a(new_n70_), .b(new_n68_), .c(new_n54_), .O(z01));
  oai21  g020(.a(new_n53_), .b(new_n52_), .c(x18), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  nand3  g022(.a(new_n58_), .b(new_n73_), .c(new_n64_), .O(new_n74_));
  oai21  g023(.a(new_n67_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  nor2   g025(.a(x16), .b(x13), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n54_), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n76_), .c(new_n72_), .O(z02));
  nand2  g028(.a(x22), .b(x21), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n64_), .c(new_n55_), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n80_), .c(x19), .O(new_n83_));
  and2   g032(.a(new_n66_), .b(x22), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  aoi21  g035(.a(new_n61_), .b(new_n86_), .c(x18), .O(new_n87_));
  aoi21  g036(.a(new_n87_), .b(new_n85_), .c(new_n54_), .O(z03));
  nand3  g037(.a(new_n81_), .b(new_n58_), .c(new_n64_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(x23), .O(new_n90_));
  nor2   g039(.a(x23), .b(x22), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n67_), .c(new_n73_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(x16), .O(new_n94_));
  nor2   g043(.a(x16), .b(x11), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n54_), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n94_), .c(new_n72_), .O(z04));
  inv1   g046(.a(x24), .O(new_n98_));
  nand2  g047(.a(new_n91_), .b(new_n73_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n66_), .O(new_n100_));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n81_), .c(new_n67_), .O(new_n102_));
  oai21  g051(.a(new_n100_), .b(new_n98_), .c(new_n102_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x16), .O(new_n104_));
  nor2   g053(.a(x16), .b(x10), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n54_), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n104_), .c(new_n72_), .O(z05));
  inv1   g056(.a(x18), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n53_), .O(new_n110_));
  inv1   g059(.a(x25), .O(new_n111_));
  nand2  g060(.a(new_n101_), .b(new_n81_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n66_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nor2   g063(.a(x25), .b(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n91_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n74_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n114_), .c(x16), .O(new_n118_));
  aoi21  g067(.a(x18), .b(new_n52_), .c(new_n54_), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n118_), .c(new_n110_), .O(z06));
  inv1   g069(.a(x26), .O(new_n121_));
  nor3   g070(.a(x26), .b(x25), .c(x24), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n91_), .O(new_n123_));
  oai22  g072(.a(new_n123_), .b(new_n74_), .c(new_n117_), .d(new_n121_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x16), .O(new_n125_));
  nor2   g074(.a(x16), .b(x08), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n54_), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n125_), .c(new_n72_), .O(z07));
  oai21  g077(.a(new_n61_), .b(x09), .c(x07), .O(new_n129_));
  inv1   g078(.a(x27), .O(new_n130_));
  inv1   g079(.a(new_n122_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n99_), .c(new_n56_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n67_), .c(new_n130_), .O(new_n133_));
  nor3   g082(.a(x27), .b(x26), .c(x25), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n101_), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n89_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n133_), .c(new_n129_), .O(new_n137_));
  aoi22  g086(.a(new_n109_), .b(new_n52_), .c(x18), .d(new_n53_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n137_), .O(z08));
  inv1   g088(.a(x28), .O(new_n140_));
  nor2   g089(.a(new_n136_), .b(new_n140_), .O(new_n141_));
  inv1   g090(.a(x23), .O(new_n142_));
  nand2  g091(.a(new_n115_), .b(new_n142_), .O(new_n143_));
  nor2   g092(.a(x28), .b(x27), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n121_), .O(new_n145_));
  nor3   g094(.a(new_n145_), .b(new_n143_), .c(new_n89_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n141_), .c(x16), .O(new_n147_));
  nor2   g096(.a(x16), .b(x06), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n54_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n147_), .c(new_n72_), .O(z09));
  inv1   g099(.a(x29), .O(new_n151_));
  nor2   g100(.a(x29), .b(x28), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n122_), .c(new_n100_), .d(new_n130_), .O(new_n153_));
  oai21  g102(.a(new_n146_), .b(new_n151_), .c(new_n153_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(x16), .O(new_n155_));
  nor2   g104(.a(x16), .b(x05), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n54_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n155_), .c(new_n72_), .O(z10));
  inv1   g107(.a(x30), .O(new_n159_));
  nor2   g108(.a(x26), .b(x25), .O(new_n160_));
  nand3  g109(.a(new_n152_), .b(new_n160_), .c(new_n130_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n112_), .c(new_n56_), .O(new_n162_));
  aoi21  g111(.a(new_n162_), .b(new_n67_), .c(new_n159_), .O(new_n163_));
  nor2   g112(.a(x30), .b(x29), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n144_), .c(new_n122_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n92_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n163_), .c(x16), .O(new_n167_));
  inv1   g116(.a(x04), .O(new_n168_));
  aoi21  g117(.a(new_n61_), .b(new_n168_), .c(x18), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n167_), .c(new_n54_), .O(z11));
  inv1   g119(.a(x31), .O(new_n171_));
  nor2   g120(.a(x31), .b(x30), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n152_), .c(new_n134_), .d(new_n113_), .O(new_n173_));
  oai21  g122(.a(new_n166_), .b(new_n171_), .c(new_n173_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  nor2   g124(.a(x16), .b(x03), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n54_), .O(new_n177_));
  nand3  g126(.a(new_n177_), .b(new_n175_), .c(new_n72_), .O(z12));
  inv1   g127(.a(x32), .O(new_n179_));
  nand3  g128(.a(new_n115_), .b(new_n81_), .c(new_n142_), .O(new_n180_));
  nand4  g129(.a(new_n172_), .b(new_n144_), .c(new_n151_), .d(new_n121_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n180_), .c(new_n56_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n67_), .c(new_n179_), .O(new_n183_));
  nor2   g132(.a(x32), .b(x31), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n164_), .c(new_n144_), .d(new_n160_), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n102_), .O(new_n186_));
  oai21  g135(.a(new_n186_), .b(new_n183_), .c(x16), .O(new_n187_));
  inv1   g136(.a(x02), .O(new_n188_));
  aoi21  g137(.a(new_n61_), .b(new_n188_), .c(x18), .O(new_n189_));
  aoi21  g138(.a(new_n189_), .b(new_n187_), .c(new_n54_), .O(z13));
  inv1   g139(.a(x33), .O(new_n191_));
  nand3  g140(.a(new_n152_), .b(new_n130_), .c(new_n121_), .O(new_n192_));
  nand3  g141(.a(new_n172_), .b(new_n191_), .c(new_n179_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n192_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n117_), .O(new_n195_));
  oai21  g144(.a(new_n186_), .b(new_n191_), .c(new_n195_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(x16), .O(new_n197_));
  nor2   g146(.a(x16), .b(x01), .O(new_n198_));
  nor2   g147(.a(new_n198_), .b(new_n54_), .O(new_n199_));
  nand3  g148(.a(new_n199_), .b(new_n197_), .c(new_n72_), .O(z14));
  nand2  g149(.a(new_n195_), .b(x34), .O(new_n201_));
  inv1   g150(.a(new_n192_), .O(new_n202_));
  nor3   g151(.a(x34), .b(x33), .c(x32), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n202_), .c(new_n172_), .d(new_n117_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n201_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(x16), .O(new_n206_));
  nor2   g155(.a(x16), .b(x00), .O(new_n207_));
  nor2   g156(.a(new_n207_), .b(new_n54_), .O(new_n208_));
  nand3  g157(.a(new_n208_), .b(new_n206_), .c(new_n72_), .O(z15));
endmodule


