// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n102_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  inv1   g002(.a(x15), .O(new_n48_));
  nor2   g003(.a(x20), .b(new_n48_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g006(.a(z02), .b(new_n46_), .O(z00));
  inv1   g007(.a(x23), .O(new_n53_));
  inv1   g008(.a(x24), .O(new_n54_));
  nand2  g009(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g010(.a(new_n55_), .b(new_n53_), .c(new_n54_), .O(new_n56_));
  inv1   g011(.a(new_n56_), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  inv1   g013(.a(x20), .O(new_n59_));
  inv1   g014(.a(x17), .O(new_n60_));
  inv1   g015(.a(x18), .O(new_n61_));
  inv1   g016(.a(x19), .O(new_n62_));
  nand3  g017(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand4  g018(.a(new_n63_), .b(new_n53_), .c(new_n59_), .d(new_n48_), .O(new_n64_));
  aoi21  g019(.a(new_n64_), .b(new_n58_), .c(x25), .O(z03));
  inv1   g020(.a(x07), .O(new_n66_));
  nand2  g021(.a(x05), .b(x04), .O(new_n67_));
  nand3  g022(.a(new_n67_), .b(new_n50_), .c(new_n66_), .O(new_n68_));
  or2    g023(.a(new_n68_), .b(z03), .O(z01));
  xnor2a g024(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g025(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g026(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g027(.a(x12), .b(x03), .O(new_n73_));
  nand4  g028(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g029(.a(new_n74_), .b(new_n50_), .c(new_n46_), .O(new_n75_));
  inv1   g030(.a(new_n75_), .O(z04));
  nor3   g031(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  inv1   g032(.a(x14), .O(new_n78_));
  nor3   g033(.a(new_n49_), .b(new_n78_), .c(x08), .O(z06));
  nand3  g034(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g035(.a(x24), .b(x22), .c(x21), .O(new_n81_));
  nor3   g036(.a(new_n81_), .b(x19), .c(x18), .O(new_n82_));
  oai21  g037(.a(new_n82_), .b(x25), .c(new_n60_), .O(new_n83_));
  oai21  g038(.a(x25), .b(x24), .c(x23), .O(new_n84_));
  inv1   g039(.a(x22), .O(new_n85_));
  inv1   g040(.a(x21), .O(new_n86_));
  nand3  g041(.a(new_n86_), .b(x19), .c(x18), .O(new_n87_));
  inv1   g042(.a(new_n87_), .O(new_n88_));
  and2   g043(.a(x18), .b(x17), .O(new_n89_));
  nor2   g044(.a(x20), .b(new_n62_), .O(new_n90_));
  nor2   g045(.a(x22), .b(x21), .O(new_n91_));
  nor2   g046(.a(x24), .b(x23), .O(new_n92_));
  nand4  g047(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n93_));
  nand4  g048(.a(new_n93_), .b(new_n88_), .c(new_n54_), .d(new_n85_), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(x25), .O(new_n95_));
  nand3  g050(.a(new_n95_), .b(new_n84_), .c(new_n83_), .O(new_n96_));
  nand2  g051(.a(new_n96_), .b(new_n48_), .O(new_n97_));
  oai21  g052(.a(new_n56_), .b(x25), .c(x20), .O(new_n98_));
  nand2  g053(.a(new_n98_), .b(new_n97_), .O(z08));
  nand4  g054(.a(new_n48_), .b(new_n66_), .c(x05), .d(x04), .O(new_n100_));
  inv1   g055(.a(new_n100_), .O(z09));
  nand4  g056(.a(new_n67_), .b(new_n60_), .c(new_n48_), .d(new_n66_), .O(new_n102_));
  inv1   g057(.a(new_n102_), .O(z10));
  xor2a  g058(.a(x18), .b(x17), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n67_), .c(new_n48_), .d(new_n66_), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(new_n50_), .O(z11));
  nand3  g061(.a(new_n62_), .b(x18), .c(x17), .O(new_n107_));
  oai21  g062(.a(new_n89_), .b(new_n62_), .c(new_n107_), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n67_), .c(new_n48_), .d(new_n66_), .O(new_n109_));
  inv1   g064(.a(new_n109_), .O(z12));
  nand2  g065(.a(new_n90_), .b(new_n89_), .O(new_n111_));
  nand3  g066(.a(new_n66_), .b(x05), .c(x04), .O(new_n112_));
  nand3  g067(.a(new_n112_), .b(new_n111_), .c(new_n66_), .O(new_n113_));
  nand2  g068(.a(new_n113_), .b(new_n48_), .O(new_n114_));
  nand4  g069(.a(x19), .b(x18), .c(x17), .d(new_n48_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(x20), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(new_n114_), .O(z13));
  nand2  g072(.a(new_n112_), .b(new_n66_), .O(new_n118_));
  inv1   g073(.a(new_n118_), .O(new_n119_));
  nand2  g074(.a(new_n111_), .b(x21), .O(new_n120_));
  nor2   g075(.a(x21), .b(x20), .O(new_n121_));
  nand3  g076(.a(new_n121_), .b(new_n89_), .c(x19), .O(new_n122_));
  nand4  g077(.a(new_n122_), .b(new_n120_), .c(new_n119_), .d(new_n48_), .O(z14));
  nand2  g078(.a(new_n122_), .b(x22), .O(new_n124_));
  nand3  g079(.a(x19), .b(x18), .c(x17), .O(new_n125_));
  inv1   g080(.a(new_n125_), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n91_), .c(new_n59_), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n124_), .c(new_n119_), .d(new_n48_), .O(z15));
  oai21  g083(.a(x23), .b(x15), .c(x20), .O(new_n129_));
  nand3  g084(.a(new_n91_), .b(new_n89_), .c(x19), .O(new_n130_));
  nor2   g085(.a(x23), .b(x22), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n126_), .c(new_n121_), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n112_), .c(new_n66_), .O(new_n133_));
  aoi21  g088(.a(new_n130_), .b(x23), .c(new_n133_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(x15), .c(new_n129_), .O(z16));
  nand3  g090(.a(new_n92_), .b(new_n85_), .c(new_n86_), .O(new_n136_));
  nor2   g091(.a(new_n136_), .b(x20), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(x19), .c(x18), .d(x17), .O(new_n138_));
  nand3  g093(.a(new_n131_), .b(new_n126_), .c(new_n86_), .O(new_n139_));
  aoi21  g094(.a(new_n139_), .b(x24), .c(new_n118_), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n48_), .O(new_n142_));
  oai21  g097(.a(x24), .b(x15), .c(x20), .O(new_n143_));
  nand2  g098(.a(new_n143_), .b(new_n142_), .O(z17));
  nand2  g099(.a(new_n92_), .b(new_n91_), .O(new_n145_));
  oai21  g100(.a(new_n125_), .b(new_n145_), .c(x25), .O(new_n146_));
  nor2   g101(.a(x25), .b(x24), .O(new_n147_));
  nand3  g102(.a(new_n91_), .b(new_n147_), .c(new_n53_), .O(new_n148_));
  oai21  g103(.a(new_n148_), .b(new_n111_), .c(new_n112_), .O(new_n149_));
  nor2   g104(.a(new_n149_), .b(x07), .O(new_n150_));
  nand2  g105(.a(x25), .b(x20), .O(new_n151_));
  nand4  g106(.a(new_n151_), .b(new_n150_), .c(new_n146_), .d(new_n48_), .O(z18));
endmodule


