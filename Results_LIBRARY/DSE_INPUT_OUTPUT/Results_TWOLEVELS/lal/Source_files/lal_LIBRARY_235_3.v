// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_;
  inv1   g000(.a(x16), .O(new_n46_));
  nand2  g001(.a(x15), .b(x09), .O(new_n47_));
  nand3  g002(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(z00));
  inv1   g004(.a(new_n47_), .O(new_n50_));
  oai21  g005(.a(x15), .b(x07), .c(x09), .O(new_n51_));
  nand3  g006(.a(new_n51_), .b(x05), .c(x04), .O(new_n52_));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  inv1   g009(.a(x17), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand3  g012(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g013(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g014(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  aoi21  g016(.a(new_n61_), .b(new_n53_), .c(x07), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(new_n50_), .c(new_n52_), .O(z01));
  nand2  g018(.a(new_n47_), .b(new_n46_), .O(z02));
  nand3  g019(.a(new_n61_), .b(new_n47_), .c(new_n53_), .O(new_n65_));
  nand2  g020(.a(new_n65_), .b(new_n47_), .O(z03));
  inv1   g021(.a(x10), .O(new_n67_));
  inv1   g022(.a(x02), .O(new_n68_));
  aoi22  g023(.a(x11), .b(new_n68_), .c(new_n67_), .d(x01), .O(new_n69_));
  oai21  g024(.a(new_n67_), .b(x01), .c(new_n69_), .O(new_n70_));
  inv1   g025(.a(x11), .O(new_n71_));
  nand2  g026(.a(new_n71_), .b(x02), .O(new_n72_));
  inv1   g027(.a(x03), .O(new_n73_));
  nand2  g028(.a(x12), .b(new_n73_), .O(new_n74_));
  inv1   g029(.a(x12), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(x03), .O(new_n76_));
  nand3  g031(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  oai21  g032(.a(new_n77_), .b(new_n70_), .c(new_n47_), .O(new_n78_));
  inv1   g033(.a(x00), .O(new_n79_));
  nor2   g034(.a(x09), .b(new_n79_), .O(new_n80_));
  inv1   g035(.a(x09), .O(new_n81_));
  nor2   g036(.a(x15), .b(new_n81_), .O(new_n82_));
  aoi21  g037(.a(new_n82_), .b(new_n79_), .c(new_n80_), .O(new_n83_));
  aoi21  g038(.a(new_n83_), .b(new_n78_), .c(x08), .O(z04));
  oai21  g039(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g040(.a(x08), .O(new_n86_));
  nand3  g041(.a(new_n47_), .b(x14), .c(new_n86_), .O(new_n87_));
  inv1   g042(.a(new_n87_), .O(z06));
  nand3  g043(.a(new_n47_), .b(new_n86_), .c(x06), .O(z07));
  nand2  g044(.a(new_n57_), .b(new_n56_), .O(new_n90_));
  nand3  g045(.a(x24), .b(x22), .c(x21), .O(new_n91_));
  oai21  g046(.a(new_n91_), .b(new_n90_), .c(new_n53_), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n55_), .O(new_n93_));
  nand2  g048(.a(new_n91_), .b(new_n53_), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(x20), .O(new_n95_));
  oai21  g050(.a(x25), .b(x24), .c(x23), .O(new_n96_));
  inv1   g051(.a(x22), .O(new_n97_));
  inv1   g052(.a(x24), .O(new_n98_));
  inv1   g053(.a(x21), .O(new_n99_));
  nand3  g054(.a(new_n99_), .b(x19), .c(x18), .O(new_n100_));
  inv1   g055(.a(new_n100_), .O(new_n101_));
  nor2   g056(.a(new_n56_), .b(new_n55_), .O(new_n102_));
  nor2   g057(.a(x20), .b(new_n57_), .O(new_n103_));
  nor2   g058(.a(x22), .b(x21), .O(new_n104_));
  nor2   g059(.a(x24), .b(x23), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n104_), .c(new_n103_), .d(new_n102_), .O(new_n106_));
  nand4  g061(.a(new_n106_), .b(new_n101_), .c(new_n98_), .d(new_n97_), .O(new_n107_));
  aoi21  g062(.a(new_n107_), .b(x25), .c(new_n50_), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n96_), .c(new_n95_), .d(new_n93_), .O(z08));
  inv1   g064(.a(x07), .O(new_n110_));
  inv1   g065(.a(x15), .O(new_n111_));
  inv1   g066(.a(x04), .O(new_n112_));
  inv1   g067(.a(x05), .O(new_n113_));
  nor2   g068(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand3  g069(.a(new_n114_), .b(new_n111_), .c(new_n110_), .O(new_n115_));
  inv1   g070(.a(new_n115_), .O(z09));
  inv1   g071(.a(new_n114_), .O(new_n117_));
  nand4  g072(.a(new_n117_), .b(new_n55_), .c(new_n111_), .d(new_n110_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n47_), .O(z10));
  xor2a  g074(.a(x18), .b(x17), .O(new_n120_));
  nand4  g075(.a(new_n120_), .b(new_n117_), .c(new_n111_), .d(new_n110_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n47_), .O(z11));
  nand3  g077(.a(new_n57_), .b(x18), .c(x17), .O(new_n123_));
  oai21  g078(.a(new_n102_), .b(new_n57_), .c(new_n123_), .O(new_n124_));
  nand4  g079(.a(new_n124_), .b(new_n117_), .c(new_n111_), .d(new_n110_), .O(new_n125_));
  inv1   g080(.a(new_n125_), .O(z12));
  nand2  g081(.a(new_n103_), .b(new_n102_), .O(new_n127_));
  oai21  g082(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n111_), .O(new_n129_));
  nand3  g084(.a(x19), .b(x18), .c(x17), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(x20), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n129_), .c(new_n127_), .d(new_n111_), .O(z13));
  nand2  g087(.a(new_n127_), .b(x21), .O(new_n133_));
  nor2   g088(.a(x21), .b(x20), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n102_), .c(x19), .O(new_n135_));
  nand4  g090(.a(new_n135_), .b(new_n133_), .c(new_n129_), .d(new_n111_), .O(z14));
  nand2  g091(.a(new_n135_), .b(x22), .O(new_n137_));
  inv1   g092(.a(new_n130_), .O(new_n138_));
  nand3  g093(.a(new_n138_), .b(new_n104_), .c(new_n54_), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n137_), .c(new_n129_), .d(new_n111_), .O(z15));
  nand2  g095(.a(new_n139_), .b(x23), .O(new_n141_));
  nor2   g096(.a(x23), .b(x22), .O(new_n142_));
  nand3  g097(.a(new_n142_), .b(new_n134_), .c(new_n138_), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n141_), .c(new_n129_), .d(new_n111_), .O(z16));
  nand3  g099(.a(new_n105_), .b(new_n97_), .c(new_n99_), .O(new_n145_));
  nor2   g100(.a(new_n145_), .b(x20), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(x19), .c(x18), .d(x17), .O(new_n147_));
  aoi21  g102(.a(new_n143_), .b(x24), .c(x15), .O(new_n148_));
  nand3  g103(.a(new_n148_), .b(new_n147_), .c(new_n129_), .O(z17));
  nand2  g104(.a(new_n106_), .b(x25), .O(new_n150_));
  nor3   g105(.a(x25), .b(x24), .c(x23), .O(new_n151_));
  nand4  g106(.a(new_n151_), .b(new_n104_), .c(new_n103_), .d(new_n102_), .O(new_n152_));
  nand4  g107(.a(new_n152_), .b(new_n150_), .c(new_n129_), .d(new_n111_), .O(z18));
endmodule


