// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x25), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  inv1   g13(.a(x07), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n47_), .c(new_n59_), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(z01));
  and2   g18(.a(new_n47_), .b(x16), .O(z02));
  inv1   g19(.a(new_n58_), .O(z03));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n47_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g28(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z06));
  aoi22  g30(.a(x25), .b(x15), .c(new_n46_), .d(x06), .O(z07));
  nand3  g31(.a(x24), .b(x22), .c(x21), .O(new_n77_));
  nor3   g32(.a(new_n77_), .b(x19), .c(x18), .O(new_n78_));
  nor2   g33(.a(new_n49_), .b(x15), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n78_), .c(new_n51_), .O(new_n80_));
  inv1   g35(.a(new_n77_), .O(new_n81_));
  oai21  g36(.a(new_n79_), .b(new_n81_), .c(x20), .O(new_n82_));
  oai21  g37(.a(new_n79_), .b(x24), .c(x23), .O(new_n83_));
  nor2   g38(.a(new_n53_), .b(new_n52_), .O(new_n84_));
  nor3   g39(.a(x24), .b(x22), .c(x21), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n84_), .c(x15), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand4  g42(.a(new_n50_), .b(x19), .c(x18), .d(x17), .O(new_n88_));
  nor2   g43(.a(x22), .b(x21), .O(new_n89_));
  nor2   g44(.a(x24), .b(x23), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n88_), .c(new_n87_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n86_), .c(x25), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n83_), .c(new_n82_), .d(new_n80_), .O(z08));
  nand2  g49(.a(new_n87_), .b(new_n59_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n60_), .c(new_n47_), .O(z09));
  nand4  g51(.a(new_n60_), .b(new_n51_), .c(new_n87_), .d(new_n59_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z10));
  xor2a  g53(.a(x18), .b(x17), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n60_), .c(new_n87_), .d(new_n59_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z11));
  nor2   g56(.a(new_n52_), .b(new_n51_), .O(new_n102_));
  nand3  g57(.a(new_n53_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n53_), .c(new_n103_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n60_), .c(new_n87_), .d(new_n59_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(z12));
  nand3  g61(.a(new_n59_), .b(x05), .c(x04), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n59_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n87_), .O(new_n109_));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x20), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n88_), .d(new_n87_), .O(z13));
  nand2  g67(.a(new_n88_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n102_), .c(x19), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n109_), .d(new_n87_), .O(z14));
  nand2  g71(.a(new_n115_), .b(x22), .O(new_n117_));
  inv1   g72(.a(new_n110_), .O(new_n118_));
  nor3   g73(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n117_), .c(new_n109_), .d(new_n87_), .O(z15));
  inv1   g76(.a(x23), .O(new_n122_));
  aoi21  g77(.a(new_n119_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nor2   g78(.a(x23), .b(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n114_), .c(new_n118_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n107_), .c(new_n59_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n123_), .c(new_n87_), .O(new_n127_));
  nand2  g82(.a(new_n49_), .b(x15), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n127_), .O(z16));
  nor2   g84(.a(x20), .b(new_n53_), .O(new_n130_));
  nand4  g85(.a(new_n90_), .b(new_n89_), .c(new_n130_), .d(new_n102_), .O(new_n131_));
  nand2  g86(.a(new_n125_), .b(x24), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n109_), .c(new_n131_), .d(new_n87_), .O(z17));
  nand2  g88(.a(new_n92_), .b(new_n49_), .O(new_n134_));
  aoi21  g89(.a(new_n131_), .b(x25), .c(new_n108_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(x15), .c(new_n134_), .O(z18));
endmodule


