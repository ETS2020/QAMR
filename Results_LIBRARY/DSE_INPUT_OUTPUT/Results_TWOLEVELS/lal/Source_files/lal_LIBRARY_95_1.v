// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x15), .O(new_n53_));
  oai21  g08(.a(x23), .b(x07), .c(new_n53_), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(x05), .c(x04), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  oai21  g11(.a(x25), .b(x24), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  inv1   g13(.a(x25), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x17), .O(new_n61_));
  inv1   g16(.a(x18), .O(new_n62_));
  inv1   g17(.a(x19), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n60_), .O(new_n65_));
  inv1   g20(.a(x21), .O(new_n66_));
  inv1   g21(.a(x22), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(new_n59_), .c(new_n48_), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(new_n58_), .c(new_n55_), .O(z01));
  nor2   g26(.a(new_n49_), .b(x24), .O(new_n72_));
  aoi21  g27(.a(new_n69_), .b(new_n48_), .c(new_n72_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x25), .c(new_n50_), .O(z03));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g32(.a(x12), .b(x03), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g40(.a(x24), .O(new_n86_));
  aoi21  g41(.a(new_n64_), .b(new_n60_), .c(new_n67_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x21), .c(x23), .O(new_n88_));
  nor2   g43(.a(new_n49_), .b(x25), .O(new_n89_));
  oai21  g44(.a(new_n88_), .b(new_n86_), .c(new_n89_), .O(z08));
  nand2  g45(.a(x05), .b(x04), .O(new_n91_));
  nor4   g46(.a(new_n91_), .b(x23), .c(x15), .d(x07), .O(z09));
  nand3  g47(.a(new_n91_), .b(new_n61_), .c(new_n56_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n48_), .c(x15), .O(z10));
  xor2a  g49(.a(x18), .b(x17), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n91_), .c(new_n48_), .d(new_n53_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(x07), .O(z11));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand3  g54(.a(new_n63_), .b(x18), .c(x17), .O(new_n100_));
  oai21  g55(.a(new_n99_), .b(new_n63_), .c(new_n100_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n91_), .c(new_n56_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n48_), .c(x15), .O(z12));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  inv1   g60(.a(new_n91_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n53_), .c(new_n56_), .O(new_n107_));
  nand4  g62(.a(new_n60_), .b(x19), .c(x18), .d(x17), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n105_), .d(new_n56_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n48_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n53_), .O(z13));
  nand3  g66(.a(new_n106_), .b(new_n48_), .c(new_n56_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n48_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n53_), .O(new_n114_));
  nor2   g69(.a(x21), .b(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n99_), .c(x19), .O(new_n116_));
  nand2  g71(.a(new_n48_), .b(x07), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n53_), .O(new_n118_));
  aoi21  g73(.a(new_n108_), .b(x21), .c(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n114_), .O(z14));
  nand3  g75(.a(new_n66_), .b(new_n60_), .c(x19), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n98_), .c(x22), .O(new_n122_));
  inv1   g77(.a(new_n104_), .O(new_n123_));
  nor3   g78(.a(x22), .b(x21), .c(x20), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n122_), .c(new_n107_), .d(new_n56_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n48_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n53_), .O(z15));
  aoi21  g83(.a(new_n124_), .b(new_n123_), .c(x07), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n112_), .c(new_n48_), .d(new_n53_), .O(z16));
  aoi21  g85(.a(new_n124_), .b(new_n123_), .c(new_n86_), .O(new_n131_));
  nand4  g86(.a(new_n115_), .b(new_n123_), .c(new_n86_), .d(new_n67_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n107_), .c(new_n56_), .O(new_n133_));
  oai21  g88(.a(new_n133_), .b(new_n131_), .c(new_n48_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n53_), .O(z17));
  nand2  g90(.a(new_n132_), .b(x25), .O(new_n136_));
  nand4  g91(.a(new_n59_), .b(new_n86_), .c(new_n67_), .d(new_n66_), .O(new_n137_));
  nor2   g92(.a(new_n137_), .b(new_n108_), .O(new_n138_));
  aoi21  g93(.a(new_n48_), .b(x07), .c(new_n138_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n136_), .c(new_n114_), .d(new_n53_), .O(z18));
endmodule


