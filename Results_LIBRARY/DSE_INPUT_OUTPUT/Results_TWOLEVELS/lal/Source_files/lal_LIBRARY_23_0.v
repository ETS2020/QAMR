// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x15), .b(x06), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n48_), .c(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor3   g16(.a(new_n61_), .b(new_n47_), .c(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n47_), .b(new_n64_), .O(z02));
  inv1   g20(.a(new_n59_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n48_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n48_), .O(z06));
  nand2  g31(.a(new_n46_), .b(x06), .O(z07));
  inv1   g32(.a(x24), .O(new_n78_));
  inv1   g33(.a(x22), .O(new_n79_));
  aoi21  g34(.a(new_n55_), .b(new_n51_), .c(new_n79_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(x21), .c(x23), .O(new_n81_));
  nor2   g36(.a(new_n47_), .b(x25), .O(new_n82_));
  oai21  g37(.a(new_n81_), .b(new_n78_), .c(new_n82_), .O(z08));
  inv1   g38(.a(x06), .O(new_n84_));
  nor4   g39(.a(new_n60_), .b(x15), .c(x07), .d(new_n84_), .O(z09));
  inv1   g40(.a(x07), .O(new_n86_));
  nand3  g41(.a(new_n60_), .b(new_n52_), .c(new_n86_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x06), .c(x15), .O(z10));
  inv1   g43(.a(x15), .O(new_n89_));
  xor2a  g44(.a(x18), .b(x17), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n60_), .c(new_n89_), .d(new_n86_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n84_), .O(z11));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand3  g49(.a(new_n54_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n54_), .c(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n60_), .c(new_n86_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(x06), .c(x15), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n61_), .b(new_n89_), .c(new_n86_), .O(new_n101_));
  nand4  g56(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n101_), .c(new_n100_), .d(new_n86_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x06), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n89_), .O(z13));
  oai21  g60(.a(new_n60_), .b(x07), .c(x06), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n89_), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n94_), .c(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n89_), .c(new_n86_), .O(new_n110_));
  aoi21  g65(.a(new_n102_), .b(x21), .c(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n107_), .O(z14));
  inv1   g67(.a(x21), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n51_), .c(x19), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n93_), .c(x22), .O(new_n115_));
  inv1   g70(.a(new_n99_), .O(new_n116_));
  nor3   g71(.a(x22), .b(x21), .c(x20), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n115_), .c(new_n101_), .d(new_n86_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x06), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n89_), .O(z15));
  inv1   g76(.a(x23), .O(new_n122_));
  aoi21  g77(.a(new_n117_), .b(new_n116_), .c(new_n122_), .O(new_n123_));
  nand4  g78(.a(new_n108_), .b(new_n116_), .c(new_n122_), .d(new_n79_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n101_), .c(new_n86_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n123_), .c(x06), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n89_), .O(z16));
  nand2  g82(.a(new_n124_), .b(x24), .O(new_n128_));
  nand4  g83(.a(new_n78_), .b(new_n122_), .c(new_n79_), .d(new_n113_), .O(new_n129_));
  nor2   g84(.a(new_n129_), .b(new_n102_), .O(new_n130_));
  nor2   g85(.a(new_n130_), .b(x15), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n128_), .c(new_n107_), .d(new_n86_), .O(z17));
  oai21  g87(.a(new_n129_), .b(new_n102_), .c(x25), .O(new_n133_));
  nor2   g88(.a(x20), .b(new_n54_), .O(new_n134_));
  nor2   g89(.a(x22), .b(x21), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n135_), .c(new_n134_), .d(new_n94_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n133_), .c(new_n101_), .d(new_n86_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(x06), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n89_), .O(z18));
endmodule


