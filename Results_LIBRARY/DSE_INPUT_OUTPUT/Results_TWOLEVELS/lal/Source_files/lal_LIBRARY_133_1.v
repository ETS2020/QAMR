// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x15), .O(new_n51_));
  oai21  g06(.a(x23), .b(x07), .c(new_n51_), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  nor2   g08(.a(x25), .b(x24), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(x07), .c(new_n49_), .O(new_n55_));
  inv1   g10(.a(x25), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(x22), .c(x21), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n56_), .c(new_n47_), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n55_), .c(new_n53_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n49_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x24), .O(new_n68_));
  nand2  g23(.a(new_n49_), .b(new_n68_), .O(new_n69_));
  nand2  g24(.a(new_n63_), .b(new_n47_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n49_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n49_), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g37(.a(x22), .O(new_n83_));
  aoi21  g38(.a(new_n61_), .b(new_n57_), .c(new_n83_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(x21), .c(x23), .O(new_n85_));
  nor2   g40(.a(new_n48_), .b(x25), .O(new_n86_));
  oai21  g41(.a(new_n85_), .b(new_n68_), .c(new_n86_), .O(z08));
  nand2  g42(.a(x05), .b(x04), .O(new_n88_));
  nor4   g43(.a(new_n88_), .b(x23), .c(x15), .d(x07), .O(z09));
  inv1   g44(.a(x07), .O(new_n90_));
  nand3  g45(.a(new_n88_), .b(new_n58_), .c(new_n90_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n47_), .c(x15), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n88_), .c(new_n47_), .d(new_n51_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(x07), .O(z11));
  aoi21  g50(.a(new_n47_), .b(new_n59_), .c(new_n58_), .O(new_n96_));
  nand3  g51(.a(new_n60_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n60_), .c(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n88_), .c(new_n90_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n47_), .c(x15), .O(z12));
  inv1   g55(.a(new_n88_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n47_), .c(new_n90_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n51_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nor2   g61(.a(new_n59_), .b(new_n58_), .O(new_n107_));
  nor2   g62(.a(x20), .b(new_n60_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g64(.a(new_n47_), .b(x07), .O(new_n110_));
  and2   g65(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n106_), .c(new_n104_), .d(new_n51_), .O(z13));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(x19), .c(x18), .d(x17), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n110_), .c(new_n51_), .O(new_n115_));
  aoi21  g70(.a(new_n109_), .b(x21), .c(new_n115_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n104_), .O(z14));
  nand2  g72(.a(new_n114_), .b(x22), .O(new_n118_));
  nand3  g73(.a(new_n101_), .b(new_n51_), .c(new_n90_), .O(new_n119_));
  inv1   g74(.a(new_n105_), .O(new_n120_));
  nor3   g75(.a(x22), .b(x21), .c(x20), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n119_), .c(new_n118_), .d(new_n90_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n51_), .O(z15));
  aoi21  g80(.a(new_n121_), .b(new_n120_), .c(x07), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n102_), .c(new_n47_), .d(new_n51_), .O(z16));
  aoi21  g82(.a(new_n121_), .b(new_n120_), .c(new_n68_), .O(new_n128_));
  nand4  g83(.a(new_n113_), .b(new_n120_), .c(new_n68_), .d(new_n83_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n119_), .c(new_n90_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n128_), .c(new_n47_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n51_), .O(z17));
  inv1   g87(.a(x21), .O(new_n133_));
  nand4  g88(.a(new_n68_), .b(new_n83_), .c(new_n133_), .d(new_n57_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n105_), .c(x25), .O(new_n135_));
  nor2   g90(.a(x22), .b(x21), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n108_), .c(new_n107_), .d(new_n54_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n119_), .d(new_n90_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n51_), .O(z18));
endmodule


