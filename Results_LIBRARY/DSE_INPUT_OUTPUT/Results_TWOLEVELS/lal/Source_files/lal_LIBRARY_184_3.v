// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  inv1   g002(.a(x18), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  oai21  g005(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  oai21  g006(.a(x15), .b(x07), .c(x18), .O(new_n52_));
  nand3  g007(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  inv1   g008(.a(x07), .O(new_n54_));
  inv1   g009(.a(x23), .O(new_n55_));
  inv1   g010(.a(x24), .O(new_n56_));
  nand2  g011(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g012(.a(new_n57_), .b(new_n55_), .c(new_n56_), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(x25), .c(new_n54_), .O(new_n59_));
  nand2  g014(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  inv1   g015(.a(x20), .O(new_n61_));
  inv1   g016(.a(x25), .O(new_n62_));
  oai21  g017(.a(x19), .b(x17), .c(new_n48_), .O(new_n63_));
  oai21  g018(.a(new_n48_), .b(x15), .c(new_n63_), .O(new_n64_));
  nand4  g019(.a(new_n64_), .b(new_n62_), .c(new_n55_), .d(new_n61_), .O(new_n65_));
  nand3  g020(.a(new_n65_), .b(new_n60_), .c(new_n53_), .O(z01));
  inv1   g021(.a(x16), .O(new_n67_));
  nor2   g022(.a(new_n49_), .b(new_n67_), .O(z02));
  inv1   g023(.a(new_n58_), .O(new_n69_));
  nand2  g024(.a(new_n69_), .b(new_n50_), .O(new_n70_));
  nand3  g025(.a(new_n64_), .b(new_n55_), .c(new_n61_), .O(new_n71_));
  aoi21  g026(.a(new_n71_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g027(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g028(.a(x10), .b(x01), .O(new_n74_));
  xnor2a g029(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g030(.a(x12), .b(x03), .O(new_n76_));
  nand4  g031(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(new_n46_), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n50_), .O(z04));
  inv1   g034(.a(x13), .O(new_n80_));
  nand3  g035(.a(new_n50_), .b(new_n80_), .c(new_n46_), .O(new_n81_));
  inv1   g036(.a(new_n81_), .O(z05));
  nand2  g037(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(new_n50_), .O(z06));
  aoi21  g039(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g040(.a(x17), .O(new_n86_));
  inv1   g041(.a(x19), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(new_n48_), .O(new_n88_));
  nand3  g043(.a(x24), .b(x22), .c(x21), .O(new_n89_));
  nand2  g044(.a(x25), .b(new_n47_), .O(new_n90_));
  oai21  g045(.a(new_n89_), .b(new_n88_), .c(new_n90_), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(new_n86_), .O(new_n92_));
  inv1   g047(.a(new_n90_), .O(new_n93_));
  nand3  g048(.a(x22), .b(x21), .c(x20), .O(new_n94_));
  aoi21  g049(.a(new_n94_), .b(new_n55_), .c(new_n49_), .O(new_n95_));
  oai21  g050(.a(new_n95_), .b(new_n93_), .c(x24), .O(new_n96_));
  inv1   g051(.a(x22), .O(new_n97_));
  nor2   g052(.a(x21), .b(x20), .O(new_n98_));
  nand2  g053(.a(new_n98_), .b(x19), .O(new_n99_));
  inv1   g054(.a(new_n99_), .O(new_n100_));
  nor2   g055(.a(new_n48_), .b(new_n86_), .O(new_n101_));
  nor2   g056(.a(x20), .b(new_n87_), .O(new_n102_));
  nor2   g057(.a(x22), .b(x21), .O(new_n103_));
  nor2   g058(.a(x24), .b(x23), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n100_), .c(new_n55_), .d(new_n97_), .O(new_n106_));
  nand2  g061(.a(new_n106_), .b(new_n47_), .O(new_n107_));
  nand2  g062(.a(new_n107_), .b(x18), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(x25), .O(new_n109_));
  nand3  g064(.a(new_n109_), .b(new_n96_), .c(new_n92_), .O(z08));
  nand4  g065(.a(new_n47_), .b(new_n54_), .c(x05), .d(x04), .O(new_n111_));
  inv1   g066(.a(new_n111_), .O(z09));
  nand2  g067(.a(x05), .b(x04), .O(new_n113_));
  nand4  g068(.a(new_n113_), .b(new_n86_), .c(new_n47_), .d(new_n54_), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n50_), .O(z10));
  xor2a  g070(.a(x18), .b(x17), .O(new_n116_));
  nand4  g071(.a(new_n116_), .b(new_n113_), .c(new_n47_), .d(new_n54_), .O(new_n117_));
  inv1   g072(.a(new_n117_), .O(z11));
  nand3  g073(.a(new_n87_), .b(x18), .c(x17), .O(new_n119_));
  oai21  g074(.a(new_n101_), .b(new_n87_), .c(new_n119_), .O(new_n120_));
  nand4  g075(.a(new_n120_), .b(new_n113_), .c(new_n47_), .d(new_n54_), .O(new_n121_));
  inv1   g076(.a(new_n121_), .O(z12));
  oai21  g077(.a(x20), .b(x15), .c(new_n48_), .O(new_n123_));
  nand2  g078(.a(new_n102_), .b(new_n101_), .O(new_n124_));
  oai21  g079(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n125_));
  nand3  g080(.a(new_n54_), .b(x05), .c(x04), .O(new_n126_));
  nand4  g081(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n54_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n47_), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n123_), .O(z13));
  nand2  g084(.a(new_n126_), .b(new_n54_), .O(new_n130_));
  inv1   g085(.a(new_n130_), .O(new_n131_));
  nand2  g086(.a(new_n124_), .b(x21), .O(new_n132_));
  nand2  g087(.a(new_n101_), .b(new_n100_), .O(new_n133_));
  nand4  g088(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n47_), .O(z14));
  oai21  g089(.a(x22), .b(x15), .c(new_n48_), .O(new_n135_));
  nand3  g090(.a(new_n98_), .b(x19), .c(x17), .O(new_n136_));
  nand3  g091(.a(x19), .b(x18), .c(x17), .O(new_n137_));
  inv1   g092(.a(new_n137_), .O(new_n138_));
  nand3  g093(.a(new_n138_), .b(new_n103_), .c(new_n61_), .O(new_n139_));
  nand3  g094(.a(new_n139_), .b(new_n126_), .c(new_n54_), .O(new_n140_));
  aoi21  g095(.a(new_n136_), .b(x22), .c(new_n140_), .O(new_n141_));
  oai21  g096(.a(new_n141_), .b(x15), .c(new_n135_), .O(z15));
  nand2  g097(.a(new_n139_), .b(x23), .O(new_n143_));
  nor2   g098(.a(x23), .b(x22), .O(new_n144_));
  nand3  g099(.a(new_n144_), .b(new_n138_), .c(new_n98_), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(new_n143_), .c(new_n131_), .d(new_n47_), .O(z16));
  inv1   g101(.a(x21), .O(new_n147_));
  nand3  g102(.a(new_n104_), .b(new_n97_), .c(new_n147_), .O(new_n148_));
  nor2   g103(.a(new_n148_), .b(x20), .O(new_n149_));
  nand4  g104(.a(new_n149_), .b(x19), .c(x18), .d(x17), .O(new_n150_));
  nand4  g105(.a(new_n144_), .b(new_n102_), .c(new_n147_), .d(x17), .O(new_n151_));
  aoi21  g106(.a(new_n151_), .b(x24), .c(new_n130_), .O(new_n152_));
  nand2  g107(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g108(.a(new_n153_), .b(new_n47_), .O(new_n154_));
  oai21  g109(.a(x24), .b(x15), .c(new_n48_), .O(new_n155_));
  nand2  g110(.a(new_n155_), .b(new_n154_), .O(z17));
  oai21  g111(.a(x25), .b(x15), .c(new_n48_), .O(new_n157_));
  nand2  g112(.a(new_n104_), .b(new_n103_), .O(new_n158_));
  nand2  g113(.a(new_n102_), .b(x17), .O(new_n159_));
  oai21  g114(.a(new_n159_), .b(new_n158_), .c(x25), .O(new_n160_));
  nor3   g115(.a(x25), .b(x24), .c(x23), .O(new_n161_));
  nand4  g116(.a(new_n161_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n162_));
  nand4  g117(.a(new_n162_), .b(new_n160_), .c(new_n126_), .d(new_n54_), .O(new_n163_));
  nand2  g118(.a(new_n163_), .b(new_n47_), .O(new_n164_));
  nand2  g119(.a(new_n164_), .b(new_n157_), .O(z18));
endmodule


