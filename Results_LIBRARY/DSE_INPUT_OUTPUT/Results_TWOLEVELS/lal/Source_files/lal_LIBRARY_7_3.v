// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_;
  inv1   g000(.a(x08), .O(new_n46_));
  nand2  g001(.a(x25), .b(x15), .O(new_n47_));
  oai21  g002(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g003(.a(x07), .O(new_n49_));
  inv1   g004(.a(x15), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g006(.a(new_n51_), .b(x25), .O(new_n52_));
  nand3  g007(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  nand2  g008(.a(new_n47_), .b(x07), .O(new_n54_));
  inv1   g009(.a(x25), .O(new_n55_));
  inv1   g010(.a(x20), .O(new_n56_));
  inv1   g011(.a(x17), .O(new_n57_));
  inv1   g012(.a(x18), .O(new_n58_));
  inv1   g013(.a(x19), .O(new_n59_));
  nand3  g014(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g015(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g016(.a(new_n60_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(x23), .c(x24), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  nand3  g019(.a(new_n64_), .b(new_n54_), .c(new_n53_), .O(z01));
  and2   g020(.a(new_n47_), .b(x16), .O(z02));
  nand2  g021(.a(new_n64_), .b(new_n47_), .O(z03));
  xnor2a g022(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g023(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g024(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g025(.a(x12), .b(x03), .O(new_n71_));
  nand4  g026(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand3  g027(.a(new_n72_), .b(new_n47_), .c(new_n46_), .O(new_n73_));
  inv1   g028(.a(new_n73_), .O(z04));
  oai21  g029(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g030(.a(x14), .O(new_n76_));
  oai21  g031(.a(new_n76_), .b(x08), .c(new_n47_), .O(z06));
  aoi22  g032(.a(x25), .b(x15), .c(new_n46_), .d(x06), .O(z07));
  nand2  g033(.a(new_n59_), .b(new_n58_), .O(new_n79_));
  nand3  g034(.a(x24), .b(x22), .c(x21), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n79_), .c(new_n55_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n57_), .O(new_n82_));
  nand2  g037(.a(new_n80_), .b(new_n55_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(x20), .O(new_n84_));
  oai21  g039(.a(x25), .b(x24), .c(x23), .O(new_n85_));
  inv1   g040(.a(x24), .O(new_n86_));
  nor3   g041(.a(new_n59_), .b(new_n58_), .c(x15), .O(new_n87_));
  nor2   g042(.a(x22), .b(x21), .O(new_n88_));
  nor2   g043(.a(new_n59_), .b(new_n58_), .O(new_n89_));
  nor2   g044(.a(new_n57_), .b(x15), .O(new_n90_));
  nor2   g045(.a(x21), .b(x20), .O(new_n91_));
  nor3   g046(.a(x24), .b(x23), .c(x22), .O(new_n92_));
  nand4  g047(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n93_));
  nand4  g048(.a(new_n93_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(x25), .O(new_n95_));
  nand4  g050(.a(new_n95_), .b(new_n85_), .c(new_n84_), .d(new_n82_), .O(z08));
  nand2  g051(.a(x05), .b(x04), .O(new_n97_));
  oai21  g052(.a(new_n97_), .b(new_n51_), .c(new_n47_), .O(z09));
  nand4  g053(.a(new_n97_), .b(new_n57_), .c(new_n50_), .d(new_n49_), .O(new_n99_));
  inv1   g054(.a(new_n99_), .O(z10));
  xor2a  g055(.a(x18), .b(x17), .O(new_n101_));
  nand4  g056(.a(new_n101_), .b(new_n97_), .c(new_n50_), .d(new_n49_), .O(new_n102_));
  nand2  g057(.a(new_n102_), .b(new_n47_), .O(z11));
  nand2  g058(.a(x18), .b(x17), .O(new_n104_));
  inv1   g059(.a(new_n104_), .O(new_n105_));
  nand3  g060(.a(new_n59_), .b(x18), .c(x17), .O(new_n106_));
  oai21  g061(.a(new_n105_), .b(new_n59_), .c(new_n106_), .O(new_n107_));
  nand4  g062(.a(new_n107_), .b(new_n97_), .c(new_n50_), .d(new_n49_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n47_), .O(z12));
  nand3  g064(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(x20), .O(new_n111_));
  nand3  g066(.a(new_n49_), .b(x05), .c(x04), .O(new_n112_));
  nor2   g067(.a(x20), .b(new_n59_), .O(new_n113_));
  nand2  g068(.a(new_n113_), .b(new_n105_), .O(new_n114_));
  nand4  g069(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n49_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n50_), .O(new_n116_));
  nand2  g071(.a(new_n55_), .b(x15), .O(new_n117_));
  nand2  g072(.a(new_n117_), .b(new_n116_), .O(z13));
  inv1   g073(.a(x21), .O(new_n119_));
  aoi21  g074(.a(new_n113_), .b(new_n105_), .c(new_n119_), .O(new_n120_));
  nand3  g075(.a(new_n105_), .b(new_n91_), .c(x19), .O(new_n121_));
  nand3  g076(.a(new_n121_), .b(new_n112_), .c(new_n49_), .O(new_n122_));
  oai21  g077(.a(new_n122_), .b(new_n120_), .c(new_n50_), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(new_n117_), .O(z14));
  nand2  g079(.a(new_n121_), .b(x22), .O(new_n125_));
  inv1   g080(.a(new_n110_), .O(new_n126_));
  nor3   g081(.a(x22), .b(x21), .c(x20), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g083(.a(new_n128_), .b(new_n125_), .c(new_n112_), .d(new_n49_), .O(new_n129_));
  nand2  g084(.a(new_n129_), .b(new_n50_), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(new_n117_), .O(z15));
  inv1   g086(.a(x23), .O(new_n132_));
  aoi21  g087(.a(new_n127_), .b(new_n126_), .c(new_n132_), .O(new_n133_));
  inv1   g088(.a(x22), .O(new_n134_));
  nand4  g089(.a(new_n126_), .b(new_n91_), .c(new_n132_), .d(new_n134_), .O(new_n135_));
  nand3  g090(.a(new_n135_), .b(new_n112_), .c(new_n49_), .O(new_n136_));
  oai21  g091(.a(new_n136_), .b(new_n133_), .c(new_n50_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n117_), .O(z16));
  aoi21  g093(.a(x25), .b(x15), .c(x24), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n132_), .c(new_n134_), .d(new_n119_), .O(new_n140_));
  nor2   g095(.a(new_n140_), .b(x20), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(x19), .c(x18), .d(x17), .O(new_n142_));
  nand2  g097(.a(new_n135_), .b(x24), .O(new_n143_));
  aoi21  g098(.a(x05), .b(x04), .c(x07), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n50_), .O(new_n146_));
  nand3  g101(.a(new_n146_), .b(new_n142_), .c(new_n117_), .O(z17));
  nand3  g102(.a(new_n88_), .b(new_n86_), .c(new_n132_), .O(new_n148_));
  oai21  g103(.a(new_n148_), .b(new_n114_), .c(x25), .O(new_n149_));
  nor3   g104(.a(x25), .b(x24), .c(x23), .O(new_n150_));
  nand4  g105(.a(new_n150_), .b(new_n113_), .c(new_n105_), .d(new_n88_), .O(new_n151_));
  nand4  g106(.a(new_n151_), .b(new_n149_), .c(new_n144_), .d(new_n50_), .O(z18));
endmodule


