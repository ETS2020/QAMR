// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n91_,
    new_n93_, new_n95_, new_n96_, new_n99_, new_n100_, new_n101_,
    new_n104_, new_n105_, new_n107_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x17), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x15), .O(new_n53_));
  nor3   g08(.a(x25), .b(x23), .c(x20), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(new_n53_), .c(x17), .O(new_n55_));
  inv1   g10(.a(x24), .O(new_n56_));
  inv1   g11(.a(x25), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  oai21  g13(.a(x19), .b(x18), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x21), .O(new_n60_));
  inv1   g15(.a(x22), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(new_n56_), .c(new_n57_), .O(new_n64_));
  inv1   g19(.a(x07), .O(new_n65_));
  nand2  g20(.a(x05), .b(x04), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n64_), .c(new_n55_), .O(z01));
  nand2  g24(.a(new_n64_), .b(new_n55_), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n50_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  nor3   g32(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g36(.a(x23), .O(new_n82_));
  nand3  g37(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n82_), .c(new_n56_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(x25), .O(new_n85_));
  nor3   g40(.a(x19), .b(x18), .c(x17), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(x24), .c(x22), .d(x21), .O(new_n87_));
  oai21  g42(.a(new_n85_), .b(new_n49_), .c(new_n87_), .O(z08));
  nor4   g43(.a(new_n66_), .b(x17), .c(x15), .d(x07), .O(z09));
  aoi21  g44(.a(new_n67_), .b(new_n48_), .c(x15), .O(z10));
  nand4  g45(.a(new_n66_), .b(x18), .c(new_n48_), .d(new_n53_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x07), .O(z11));
  nand3  g47(.a(new_n66_), .b(x19), .c(new_n65_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n48_), .c(x15), .O(z12));
  oai21  g49(.a(new_n66_), .b(x07), .c(new_n48_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n53_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n58_), .c(new_n53_), .d(new_n65_), .O(z13));
  nand4  g52(.a(new_n96_), .b(new_n60_), .c(new_n53_), .d(new_n65_), .O(z14));
  nand4  g53(.a(new_n53_), .b(new_n65_), .c(x05), .d(x04), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n61_), .c(new_n65_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n48_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n53_), .O(z15));
  nand4  g57(.a(new_n96_), .b(new_n82_), .c(new_n53_), .d(new_n65_), .O(z16));
  nand3  g58(.a(new_n99_), .b(new_n56_), .c(new_n65_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n48_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n53_), .O(z17));
  aoi21  g61(.a(x25), .b(new_n48_), .c(x15), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n96_), .c(new_n65_), .O(z18));
endmodule


