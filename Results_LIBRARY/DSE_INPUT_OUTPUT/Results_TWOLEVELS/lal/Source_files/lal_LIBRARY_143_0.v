// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_;
  inv1   g000(.a(x08), .O(new_n46_));
  nor2   g001(.a(x15), .b(x02), .O(new_n47_));
  inv1   g002(.a(new_n47_), .O(new_n48_));
  oai21  g003(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g004(.a(x07), .O(new_n50_));
  inv1   g005(.a(x24), .O(new_n51_));
  inv1   g006(.a(x25), .O(new_n52_));
  inv1   g007(.a(x20), .O(new_n53_));
  inv1   g008(.a(x18), .O(new_n54_));
  inv1   g009(.a(x19), .O(new_n55_));
  nand2  g010(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g011(.a(new_n56_), .b(x17), .c(new_n53_), .O(new_n57_));
  inv1   g012(.a(x21), .O(new_n58_));
  inv1   g013(.a(x22), .O(new_n59_));
  nor2   g014(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g015(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g016(.a(new_n61_), .b(new_n51_), .c(new_n52_), .O(new_n62_));
  nand2  g017(.a(x05), .b(x04), .O(new_n63_));
  nand4  g018(.a(new_n63_), .b(new_n62_), .c(new_n48_), .d(new_n50_), .O(z01));
  inv1   g019(.a(x16), .O(new_n65_));
  nand2  g020(.a(new_n48_), .b(new_n65_), .O(z02));
  nand2  g021(.a(new_n62_), .b(new_n48_), .O(z03));
  inv1   g022(.a(x09), .O(new_n68_));
  inv1   g023(.a(x01), .O(new_n69_));
  aoi22  g024(.a(x10), .b(new_n69_), .c(new_n68_), .d(x00), .O(new_n70_));
  oai21  g025(.a(new_n68_), .b(x00), .c(new_n70_), .O(new_n71_));
  inv1   g026(.a(x10), .O(new_n72_));
  nand2  g027(.a(new_n72_), .b(x01), .O(new_n73_));
  inv1   g028(.a(x03), .O(new_n74_));
  nand2  g029(.a(x12), .b(new_n74_), .O(new_n75_));
  inv1   g030(.a(x12), .O(new_n76_));
  nand2  g031(.a(new_n76_), .b(x03), .O(new_n77_));
  nand3  g032(.a(new_n77_), .b(new_n75_), .c(new_n73_), .O(new_n78_));
  oai21  g033(.a(new_n78_), .b(new_n71_), .c(new_n48_), .O(new_n79_));
  inv1   g034(.a(x11), .O(new_n80_));
  inv1   g035(.a(x15), .O(new_n81_));
  nor3   g036(.a(new_n81_), .b(new_n80_), .c(x02), .O(new_n82_));
  aoi21  g037(.a(new_n80_), .b(x02), .c(new_n82_), .O(new_n83_));
  aoi21  g038(.a(new_n83_), .b(new_n79_), .c(x08), .O(z04));
  oai21  g039(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g040(.a(x14), .O(new_n86_));
  nor3   g041(.a(new_n47_), .b(new_n86_), .c(x08), .O(z06));
  nand3  g042(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g043(.a(x17), .O(new_n89_));
  nand3  g044(.a(x24), .b(x22), .c(x21), .O(new_n90_));
  oai21  g045(.a(new_n90_), .b(new_n56_), .c(new_n52_), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g047(.a(new_n90_), .b(new_n52_), .O(new_n93_));
  nand2  g048(.a(new_n93_), .b(x20), .O(new_n94_));
  oai21  g049(.a(x25), .b(x24), .c(x23), .O(new_n95_));
  nand3  g050(.a(new_n58_), .b(x19), .c(x18), .O(new_n96_));
  inv1   g051(.a(new_n96_), .O(new_n97_));
  and2   g052(.a(x18), .b(x17), .O(new_n98_));
  nor2   g053(.a(x20), .b(new_n55_), .O(new_n99_));
  nor2   g054(.a(x22), .b(x21), .O(new_n100_));
  nor2   g055(.a(x24), .b(x23), .O(new_n101_));
  nand4  g056(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n98_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n97_), .c(new_n51_), .d(new_n59_), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(x25), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n95_), .c(new_n94_), .d(new_n92_), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(x02), .O(new_n106_));
  inv1   g061(.a(x23), .O(new_n107_));
  nand3  g062(.a(new_n57_), .b(x22), .c(x21), .O(new_n108_));
  aoi21  g063(.a(new_n108_), .b(new_n107_), .c(new_n51_), .O(new_n109_));
  oai21  g064(.a(new_n109_), .b(x25), .c(x15), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(new_n106_), .O(z08));
  nand3  g066(.a(x05), .b(x04), .c(x02), .O(new_n112_));
  nor3   g067(.a(new_n112_), .b(x15), .c(x07), .O(z09));
  nand3  g068(.a(new_n63_), .b(new_n89_), .c(new_n50_), .O(new_n114_));
  aoi21  g069(.a(new_n114_), .b(x02), .c(x15), .O(z10));
  inv1   g070(.a(x02), .O(new_n116_));
  xor2a  g071(.a(x18), .b(x17), .O(new_n117_));
  nand4  g072(.a(new_n117_), .b(new_n63_), .c(new_n81_), .d(new_n50_), .O(new_n118_));
  nor2   g073(.a(new_n118_), .b(new_n116_), .O(z11));
  nand3  g074(.a(new_n55_), .b(x18), .c(x17), .O(new_n120_));
  oai21  g075(.a(new_n55_), .b(x17), .c(new_n120_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  oai21  g077(.a(x05), .b(new_n116_), .c(x04), .O(new_n123_));
  nand3  g078(.a(new_n123_), .b(x19), .c(new_n54_), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(new_n50_), .O(new_n126_));
  aoi21  g081(.a(new_n126_), .b(x02), .c(x15), .O(z12));
  nand4  g082(.a(new_n50_), .b(x05), .c(x04), .d(x02), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(x02), .O(new_n129_));
  nand2  g084(.a(new_n129_), .b(new_n81_), .O(new_n130_));
  nand3  g085(.a(x19), .b(x18), .c(x17), .O(new_n131_));
  nand2  g086(.a(new_n131_), .b(x20), .O(new_n132_));
  nand2  g087(.a(new_n99_), .b(new_n98_), .O(new_n133_));
  inv1   g088(.a(new_n133_), .O(new_n134_));
  nor2   g089(.a(new_n134_), .b(x15), .O(new_n135_));
  nand4  g090(.a(new_n135_), .b(new_n132_), .c(new_n130_), .d(new_n50_), .O(z13));
  nor2   g091(.a(new_n134_), .b(new_n58_), .O(new_n137_));
  nand4  g092(.a(new_n81_), .b(new_n50_), .c(x05), .d(x04), .O(new_n138_));
  nor2   g093(.a(x21), .b(x20), .O(new_n139_));
  nand3  g094(.a(new_n139_), .b(new_n98_), .c(x19), .O(new_n140_));
  nand3  g095(.a(new_n140_), .b(new_n138_), .c(new_n50_), .O(new_n141_));
  oai21  g096(.a(new_n141_), .b(new_n137_), .c(x02), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(new_n81_), .O(z14));
  inv1   g098(.a(new_n131_), .O(new_n144_));
  nand3  g099(.a(new_n144_), .b(new_n100_), .c(new_n53_), .O(new_n145_));
  nand3  g100(.a(new_n145_), .b(new_n138_), .c(new_n50_), .O(new_n146_));
  aoi21  g101(.a(new_n140_), .b(x22), .c(new_n146_), .O(new_n147_));
  oai21  g102(.a(new_n147_), .b(new_n116_), .c(new_n81_), .O(z15));
  and2   g103(.a(new_n145_), .b(x23), .O(new_n149_));
  nand4  g104(.a(new_n139_), .b(new_n144_), .c(new_n107_), .d(new_n59_), .O(new_n150_));
  nand3  g105(.a(new_n150_), .b(new_n138_), .c(new_n50_), .O(new_n151_));
  oai21  g106(.a(new_n151_), .b(new_n149_), .c(x02), .O(new_n152_));
  nand2  g107(.a(new_n152_), .b(new_n81_), .O(z16));
  nand4  g108(.a(new_n100_), .b(x25), .c(new_n51_), .d(new_n107_), .O(new_n154_));
  oai22  g109(.a(new_n154_), .b(new_n133_), .c(new_n51_), .d(new_n107_), .O(new_n155_));
  nand2  g110(.a(new_n155_), .b(x02), .O(new_n156_));
  nand2  g111(.a(new_n145_), .b(x24), .O(new_n157_));
  nand4  g112(.a(new_n100_), .b(new_n52_), .c(new_n51_), .d(new_n107_), .O(new_n158_));
  oai21  g113(.a(new_n158_), .b(new_n133_), .c(new_n81_), .O(new_n159_));
  nor2   g114(.a(new_n159_), .b(x07), .O(new_n160_));
  nand4  g115(.a(new_n160_), .b(new_n157_), .c(new_n156_), .d(new_n130_), .O(z17));
  nand3  g116(.a(new_n102_), .b(x25), .c(x02), .O(new_n162_));
  nand3  g117(.a(new_n162_), .b(new_n160_), .c(new_n130_), .O(z18));
endmodule


