// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n120_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x03), .O(new_n47_));
  inv1   g002(.a(x15), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  oai21  g005(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g006(.a(x07), .O(new_n52_));
  inv1   g007(.a(x24), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x20), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(x17), .c(new_n55_), .O(new_n59_));
  inv1   g014(.a(x21), .O(new_n60_));
  inv1   g015(.a(x22), .O(new_n61_));
  nor2   g016(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g017(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  oai21  g018(.a(new_n63_), .b(new_n53_), .c(new_n54_), .O(new_n64_));
  inv1   g019(.a(x04), .O(new_n65_));
  inv1   g020(.a(x05), .O(new_n66_));
  nor2   g021(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  inv1   g022(.a(new_n67_), .O(new_n68_));
  nand4  g023(.a(new_n68_), .b(new_n64_), .c(new_n50_), .d(new_n52_), .O(z01));
  inv1   g024(.a(x16), .O(new_n70_));
  nor2   g025(.a(new_n49_), .b(new_n70_), .O(z02));
  nand2  g026(.a(new_n64_), .b(new_n50_), .O(z03));
  nor2   g027(.a(x12), .b(x08), .O(new_n73_));
  oai21  g028(.a(new_n73_), .b(x15), .c(x03), .O(new_n74_));
  inv1   g029(.a(x09), .O(new_n75_));
  inv1   g030(.a(x01), .O(new_n76_));
  aoi22  g031(.a(x10), .b(new_n76_), .c(new_n75_), .d(x00), .O(new_n77_));
  oai21  g032(.a(new_n75_), .b(x00), .c(new_n77_), .O(new_n78_));
  inv1   g033(.a(x10), .O(new_n79_));
  nand2  g034(.a(new_n79_), .b(x01), .O(new_n80_));
  inv1   g035(.a(x02), .O(new_n81_));
  nand2  g036(.a(x11), .b(new_n81_), .O(new_n82_));
  inv1   g037(.a(x11), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(x02), .O(new_n84_));
  nand2  g039(.a(x12), .b(new_n47_), .O(new_n85_));
  nand4  g040(.a(new_n85_), .b(new_n84_), .c(new_n82_), .d(new_n80_), .O(new_n86_));
  oai21  g041(.a(new_n86_), .b(new_n78_), .c(new_n46_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(new_n74_), .O(z04));
  inv1   g043(.a(x13), .O(new_n89_));
  nand3  g044(.a(new_n50_), .b(new_n89_), .c(new_n46_), .O(new_n90_));
  inv1   g045(.a(new_n90_), .O(z05));
  nand2  g046(.a(x14), .b(new_n46_), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n50_), .O(z06));
  nand3  g048(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g049(.a(x17), .O(new_n95_));
  nand3  g050(.a(x24), .b(x22), .c(x21), .O(new_n96_));
  oai21  g051(.a(new_n96_), .b(new_n58_), .c(new_n54_), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g053(.a(new_n96_), .b(new_n54_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(x20), .O(new_n100_));
  oai21  g055(.a(x25), .b(x24), .c(x23), .O(new_n101_));
  nand3  g056(.a(new_n60_), .b(x19), .c(x18), .O(new_n102_));
  inv1   g057(.a(new_n102_), .O(new_n103_));
  and2   g058(.a(x18), .b(x17), .O(new_n104_));
  nor2   g059(.a(x20), .b(new_n57_), .O(new_n105_));
  nor2   g060(.a(x22), .b(x21), .O(new_n106_));
  nor2   g061(.a(x24), .b(x23), .O(new_n107_));
  nand4  g062(.a(new_n107_), .b(new_n106_), .c(new_n105_), .d(new_n104_), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n103_), .c(new_n53_), .d(new_n61_), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(x25), .O(new_n110_));
  nand4  g065(.a(new_n110_), .b(new_n101_), .c(new_n100_), .d(new_n98_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n48_), .O(new_n112_));
  inv1   g067(.a(x23), .O(new_n113_));
  nand3  g068(.a(new_n59_), .b(x22), .c(x21), .O(new_n114_));
  aoi21  g069(.a(new_n114_), .b(new_n113_), .c(new_n53_), .O(new_n115_));
  oai21  g070(.a(new_n115_), .b(x25), .c(new_n47_), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(new_n112_), .O(z08));
  nand3  g072(.a(new_n67_), .b(new_n48_), .c(new_n52_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n50_), .O(z09));
  nand4  g074(.a(new_n68_), .b(new_n95_), .c(new_n48_), .d(new_n52_), .O(new_n120_));
  inv1   g075(.a(new_n120_), .O(z10));
  xor2a  g076(.a(x18), .b(x17), .O(new_n122_));
  nand4  g077(.a(new_n122_), .b(new_n68_), .c(new_n48_), .d(new_n52_), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(new_n50_), .O(z11));
  nand3  g079(.a(new_n57_), .b(x18), .c(x17), .O(new_n125_));
  oai21  g080(.a(new_n104_), .b(new_n57_), .c(new_n125_), .O(new_n126_));
  nand4  g081(.a(new_n126_), .b(new_n68_), .c(new_n48_), .d(new_n52_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n50_), .O(z12));
  nand2  g083(.a(new_n105_), .b(new_n104_), .O(new_n129_));
  nand3  g084(.a(x19), .b(x18), .c(x17), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(x20), .O(new_n131_));
  nand3  g086(.a(new_n52_), .b(x05), .c(x04), .O(new_n132_));
  nand4  g087(.a(new_n132_), .b(new_n131_), .c(new_n129_), .d(new_n52_), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n48_), .O(new_n134_));
  nand2  g089(.a(x15), .b(new_n47_), .O(new_n135_));
  nand2  g090(.a(new_n135_), .b(new_n134_), .O(z13));
  nand2  g091(.a(new_n132_), .b(new_n52_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n48_), .O(new_n138_));
  nand2  g093(.a(new_n129_), .b(x21), .O(new_n139_));
  nor2   g094(.a(x21), .b(x20), .O(new_n140_));
  nand3  g095(.a(new_n140_), .b(new_n104_), .c(x19), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(new_n139_), .c(new_n138_), .d(new_n48_), .O(z14));
  nand2  g097(.a(new_n141_), .b(x22), .O(new_n143_));
  inv1   g098(.a(new_n130_), .O(new_n144_));
  nand3  g099(.a(new_n144_), .b(new_n106_), .c(new_n55_), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(new_n143_), .c(new_n138_), .d(new_n48_), .O(z15));
  nand2  g101(.a(new_n145_), .b(x23), .O(new_n147_));
  nand4  g102(.a(new_n140_), .b(new_n144_), .c(new_n113_), .d(new_n61_), .O(new_n148_));
  nand4  g103(.a(new_n148_), .b(new_n147_), .c(new_n138_), .d(new_n48_), .O(z16));
  nand3  g104(.a(new_n107_), .b(new_n61_), .c(new_n60_), .O(new_n150_));
  nor2   g105(.a(new_n150_), .b(x20), .O(new_n151_));
  nand4  g106(.a(new_n151_), .b(x19), .c(x18), .d(x17), .O(new_n152_));
  aoi21  g107(.a(new_n148_), .b(x24), .c(new_n137_), .O(new_n153_));
  nand2  g108(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  nand2  g109(.a(new_n154_), .b(new_n48_), .O(new_n155_));
  nand2  g110(.a(new_n155_), .b(new_n135_), .O(z17));
  nand2  g111(.a(new_n108_), .b(x25), .O(new_n157_));
  nor3   g112(.a(x25), .b(x24), .c(x23), .O(new_n158_));
  nand4  g113(.a(new_n158_), .b(new_n106_), .c(new_n105_), .d(new_n104_), .O(new_n159_));
  nand4  g114(.a(new_n159_), .b(new_n157_), .c(new_n132_), .d(new_n52_), .O(new_n160_));
  nand2  g115(.a(new_n160_), .b(new_n48_), .O(new_n161_));
  nand2  g116(.a(new_n161_), .b(new_n135_), .O(z18));
endmodule


