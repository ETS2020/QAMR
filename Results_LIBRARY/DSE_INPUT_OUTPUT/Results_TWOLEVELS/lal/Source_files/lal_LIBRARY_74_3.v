// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  aoi21  g17(.a(new_n60_), .b(new_n52_), .c(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n48_), .b(x07), .O(new_n64_));
  oai21  g19(.a(new_n63_), .b(x07), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n49_), .b(new_n66_), .O(z02));
  nand2  g22(.a(new_n60_), .b(new_n52_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n50_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n50_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand3  g32(.a(new_n50_), .b(new_n77_), .c(new_n46_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand3  g37(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n52_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x20), .O(new_n85_));
  nor4   g40(.a(new_n58_), .b(x19), .c(x18), .d(x17), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x23), .c(x24), .O(new_n87_));
  aoi21  g42(.a(x25), .b(new_n53_), .c(new_n49_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n87_), .c(new_n85_), .O(z08));
  nand3  g44(.a(new_n62_), .b(new_n48_), .c(new_n47_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n50_), .O(z09));
  nand4  g46(.a(new_n61_), .b(new_n54_), .c(new_n48_), .d(new_n47_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n61_), .c(new_n48_), .d(new_n47_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n50_), .O(z11));
  nor2   g51(.a(new_n55_), .b(new_n54_), .O(new_n97_));
  nand3  g52(.a(new_n56_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n56_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n61_), .c(new_n48_), .d(new_n47_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z12));
  nand3  g56(.a(new_n48_), .b(x05), .c(x04), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n97_), .b(new_n53_), .c(x19), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n104_), .d(new_n47_), .O(z13));
  nand2  g63(.a(new_n107_), .b(x21), .O(new_n109_));
  inv1   g64(.a(x21), .O(new_n110_));
  nand4  g65(.a(new_n97_), .b(new_n110_), .c(new_n53_), .d(x19), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n104_), .d(new_n47_), .O(z14));
  nor2   g67(.a(x22), .b(x21), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n53_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n105_), .O(new_n115_));
  aoi21  g70(.a(new_n111_), .b(x22), .c(new_n115_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n104_), .c(new_n47_), .O(z15));
  inv1   g72(.a(x23), .O(new_n118_));
  nor2   g73(.a(new_n115_), .b(new_n118_), .O(new_n119_));
  inv1   g74(.a(x22), .O(new_n120_));
  nand4  g75(.a(new_n118_), .b(new_n120_), .c(new_n110_), .d(new_n53_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n105_), .c(new_n47_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n119_), .c(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n104_), .O(z16));
  nand2  g79(.a(x24), .b(x23), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n102_), .c(new_n48_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  inv1   g82(.a(x24), .O(new_n128_));
  nor2   g83(.a(new_n115_), .b(new_n128_), .O(new_n129_));
  nand3  g84(.a(new_n113_), .b(new_n128_), .c(new_n118_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n107_), .c(new_n47_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n129_), .c(new_n48_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n127_), .O(z17));
  oai21  g88(.a(new_n130_), .b(new_n107_), .c(x25), .O(new_n134_));
  inv1   g89(.a(new_n107_), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n113_), .c(new_n135_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n134_), .c(new_n104_), .d(new_n47_), .O(z18));
endmodule


