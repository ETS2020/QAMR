// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x15), .b(x08), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n47_), .c(x07), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n62_), .b(new_n48_), .O(z03));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x11), .b(x02), .O(new_n73_));
  inv1   g28(.a(x12), .O(new_n74_));
  inv1   g29(.a(x15), .O(new_n75_));
  aoi21  g30(.a(new_n74_), .b(x03), .c(new_n75_), .O(new_n76_));
  oai21  g31(.a(new_n74_), .b(x03), .c(new_n76_), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n72_), .c(x08), .O(z04));
  aoi21  g34(.a(x15), .b(x13), .c(x08), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  aoi21  g36(.a(x15), .b(new_n81_), .c(x08), .O(z06));
  nand3  g37(.a(x15), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g38(.a(new_n56_), .b(new_n52_), .c(new_n59_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(x21), .c(x23), .O(new_n85_));
  nor2   g40(.a(new_n47_), .b(x25), .O(new_n86_));
  oai21  g41(.a(new_n85_), .b(new_n50_), .c(new_n86_), .O(z08));
  nor4   g42(.a(new_n63_), .b(x15), .c(new_n46_), .d(x07), .O(z09));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n63_), .b(new_n53_), .c(new_n89_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x08), .c(x15), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n63_), .c(new_n89_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x08), .c(x15), .O(z11));
  nor2   g49(.a(new_n54_), .b(new_n53_), .O(new_n95_));
  nand3  g50(.a(new_n55_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n55_), .c(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n63_), .c(new_n89_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(x08), .c(x15), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n64_), .b(new_n75_), .c(new_n89_), .O(new_n102_));
  nor2   g57(.a(x20), .b(new_n55_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n95_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n89_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x08), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n75_), .O(z13));
  aoi21  g62(.a(new_n103_), .b(new_n95_), .c(new_n58_), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n95_), .c(x19), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n102_), .c(new_n89_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n108_), .c(x08), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n75_), .O(z14));
  oai21  g68(.a(new_n63_), .b(x07), .c(x08), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n75_), .O(new_n115_));
  nand2  g70(.a(new_n110_), .b(x22), .O(new_n116_));
  inv1   g71(.a(new_n100_), .O(new_n117_));
  nor2   g72(.a(x22), .b(x21), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n52_), .O(new_n119_));
  and2   g74(.a(new_n119_), .b(new_n75_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n116_), .c(new_n115_), .d(new_n89_), .O(z15));
  nand2  g76(.a(new_n119_), .b(x23), .O(new_n122_));
  inv1   g77(.a(x23), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n59_), .c(new_n58_), .d(new_n52_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n117_), .c(x15), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n122_), .c(new_n115_), .d(new_n89_), .O(z16));
  oai21  g82(.a(new_n124_), .b(new_n100_), .c(x24), .O(new_n128_));
  nor2   g83(.a(x24), .b(x23), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n118_), .c(new_n103_), .d(new_n95_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n128_), .c(new_n102_), .d(new_n89_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(x08), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n75_), .O(z17));
  nand2  g88(.a(new_n130_), .b(x25), .O(new_n134_));
  inv1   g89(.a(new_n104_), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  and2   g91(.a(new_n136_), .b(new_n118_), .O(new_n137_));
  aoi21  g92(.a(new_n137_), .b(new_n135_), .c(x15), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n134_), .c(new_n115_), .d(new_n89_), .O(z18));
endmodule


