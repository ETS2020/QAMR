// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n105_, new_n106_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  nand2  g002(.a(x23), .b(new_n47_), .O(new_n48_));
  oai21  g003(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  oai21  g004(.a(x23), .b(x07), .c(new_n47_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(x05), .c(x04), .O(new_n51_));
  nor2   g006(.a(x25), .b(x24), .O(new_n52_));
  oai21  g007(.a(new_n52_), .b(x07), .c(new_n48_), .O(new_n53_));
  inv1   g008(.a(x23), .O(new_n54_));
  inv1   g009(.a(x25), .O(new_n55_));
  inv1   g010(.a(x21), .O(new_n56_));
  inv1   g011(.a(x22), .O(new_n57_));
  oai21  g012(.a(new_n57_), .b(new_n56_), .c(x24), .O(new_n58_));
  inv1   g013(.a(x20), .O(new_n59_));
  inv1   g014(.a(x18), .O(new_n60_));
  inv1   g015(.a(x19), .O(new_n61_));
  nand2  g016(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(x17), .c(new_n59_), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  nand3  g019(.a(new_n64_), .b(new_n55_), .c(new_n54_), .O(new_n65_));
  nand3  g020(.a(new_n65_), .b(new_n53_), .c(new_n51_), .O(z01));
  and2   g021(.a(new_n48_), .b(x16), .O(z02));
  inv1   g022(.a(x24), .O(new_n68_));
  nand3  g023(.a(new_n63_), .b(new_n58_), .c(x24), .O(new_n69_));
  aoi22  g024(.a(new_n69_), .b(new_n54_), .c(new_n68_), .d(x15), .O(new_n70_));
  oai21  g025(.a(new_n70_), .b(x25), .c(new_n48_), .O(z03));
  xnor2a g026(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g027(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g028(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g029(.a(x12), .b(x03), .O(new_n75_));
  nand4  g030(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g031(.a(new_n76_), .b(new_n48_), .c(new_n46_), .O(new_n77_));
  inv1   g032(.a(new_n77_), .O(z04));
  oai21  g033(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g034(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g035(.a(new_n80_), .b(new_n48_), .O(z06));
  aoi22  g036(.a(x23), .b(new_n47_), .c(new_n46_), .d(x06), .O(z07));
  oai21  g037(.a(new_n68_), .b(new_n54_), .c(new_n55_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(x15), .O(new_n84_));
  inv1   g039(.a(x17), .O(new_n85_));
  nand3  g040(.a(x24), .b(x22), .c(x21), .O(new_n86_));
  oai21  g041(.a(new_n86_), .b(new_n62_), .c(new_n55_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g043(.a(new_n86_), .b(new_n55_), .O(new_n89_));
  nand2  g044(.a(new_n89_), .b(x20), .O(new_n90_));
  nand3  g045(.a(new_n56_), .b(x19), .c(x18), .O(new_n91_));
  inv1   g046(.a(new_n91_), .O(new_n92_));
  nand3  g047(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  inv1   g048(.a(new_n93_), .O(new_n94_));
  nor2   g049(.a(x21), .b(x20), .O(new_n95_));
  nor2   g050(.a(x24), .b(x22), .O(new_n96_));
  nand3  g051(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n92_), .c(new_n68_), .d(new_n57_), .O(new_n98_));
  nand2  g053(.a(new_n98_), .b(x25), .O(new_n99_));
  nand3  g054(.a(new_n99_), .b(new_n90_), .c(new_n88_), .O(new_n100_));
  nand2  g055(.a(new_n100_), .b(new_n54_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n84_), .O(z08));
  nand2  g057(.a(x05), .b(x04), .O(new_n103_));
  nor4   g058(.a(new_n103_), .b(x23), .c(x15), .d(x07), .O(z09));
  inv1   g059(.a(x07), .O(new_n105_));
  nand3  g060(.a(new_n103_), .b(new_n85_), .c(new_n105_), .O(new_n106_));
  aoi21  g061(.a(new_n106_), .b(new_n54_), .c(x15), .O(z10));
  xor2a  g062(.a(x18), .b(x17), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n103_), .c(new_n54_), .d(new_n47_), .O(new_n109_));
  nor2   g064(.a(new_n109_), .b(x07), .O(z11));
  nor2   g065(.a(new_n60_), .b(new_n85_), .O(new_n111_));
  nand3  g066(.a(new_n61_), .b(x18), .c(x17), .O(new_n112_));
  oai21  g067(.a(new_n111_), .b(new_n61_), .c(new_n112_), .O(new_n113_));
  nand4  g068(.a(new_n113_), .b(new_n103_), .c(new_n54_), .d(new_n47_), .O(new_n114_));
  nor2   g069(.a(new_n114_), .b(x07), .O(z12));
  nand2  g070(.a(new_n93_), .b(x20), .O(new_n116_));
  nand4  g071(.a(new_n47_), .b(new_n105_), .c(x05), .d(x04), .O(new_n117_));
  nor2   g072(.a(x20), .b(new_n61_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n111_), .O(new_n119_));
  nand4  g074(.a(new_n119_), .b(new_n117_), .c(new_n116_), .d(new_n105_), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(new_n54_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n47_), .O(z13));
  aoi21  g077(.a(new_n118_), .b(new_n111_), .c(new_n56_), .O(new_n123_));
  nand3  g078(.a(new_n111_), .b(new_n95_), .c(x19), .O(new_n124_));
  nand3  g079(.a(new_n124_), .b(new_n117_), .c(new_n105_), .O(new_n125_));
  oai21  g080(.a(new_n125_), .b(new_n123_), .c(new_n54_), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n47_), .O(z14));
  nor2   g082(.a(x22), .b(x21), .O(new_n128_));
  nand3  g083(.a(new_n128_), .b(new_n94_), .c(new_n59_), .O(new_n129_));
  nand3  g084(.a(new_n129_), .b(new_n117_), .c(new_n105_), .O(new_n130_));
  aoi21  g085(.a(new_n124_), .b(x22), .c(new_n130_), .O(new_n131_));
  oai21  g086(.a(new_n131_), .b(x23), .c(new_n47_), .O(z15));
  nand2  g087(.a(new_n130_), .b(new_n54_), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n47_), .O(z16));
  oai21  g089(.a(new_n103_), .b(x07), .c(new_n54_), .O(new_n135_));
  nand2  g090(.a(new_n135_), .b(new_n47_), .O(new_n136_));
  nor3   g091(.a(x24), .b(x22), .c(x21), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(new_n59_), .c(x19), .d(x18), .O(new_n138_));
  oai21  g093(.a(new_n138_), .b(new_n85_), .c(new_n105_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n54_), .O(new_n140_));
  aoi21  g095(.a(new_n129_), .b(x24), .c(x15), .O(new_n141_));
  nand3  g096(.a(new_n141_), .b(new_n140_), .c(new_n136_), .O(z17));
  nand4  g097(.a(new_n128_), .b(new_n118_), .c(new_n111_), .d(new_n52_), .O(new_n143_));
  nand3  g098(.a(new_n143_), .b(new_n117_), .c(new_n105_), .O(new_n144_));
  aoi21  g099(.a(new_n97_), .b(x25), .c(new_n144_), .O(new_n145_));
  oai21  g100(.a(new_n145_), .b(x23), .c(new_n47_), .O(z18));
endmodule


