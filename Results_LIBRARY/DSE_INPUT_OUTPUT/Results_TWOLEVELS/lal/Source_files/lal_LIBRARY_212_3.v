// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n99_, new_n100_, new_n102_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nand3  g16(.a(new_n47_), .b(x05), .c(x04), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n61_), .c(new_n47_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n50_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n61_), .b(new_n50_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n50_), .c(new_n46_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g29(.a(new_n50_), .b(x14), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z06));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g32(.a(new_n56_), .b(new_n55_), .O(new_n78_));
  nand3  g33(.a(x24), .b(x22), .c(x21), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n78_), .c(new_n52_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n54_), .O(new_n81_));
  nand2  g36(.a(new_n79_), .b(new_n52_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x20), .O(new_n83_));
  oai21  g38(.a(x25), .b(x24), .c(x23), .O(new_n84_));
  inv1   g39(.a(x22), .O(new_n85_));
  inv1   g40(.a(x24), .O(new_n86_));
  inv1   g41(.a(x21), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x19), .c(x18), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  and2   g44(.a(x18), .b(x17), .O(new_n90_));
  nor2   g45(.a(x20), .b(new_n56_), .O(new_n91_));
  nor2   g46(.a(x22), .b(x21), .O(new_n92_));
  nor2   g47(.a(x24), .b(x23), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n89_), .c(new_n86_), .d(new_n85_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(x25), .c(new_n49_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n84_), .c(new_n83_), .d(new_n81_), .O(z08));
  nand2  g52(.a(x05), .b(x04), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n48_), .c(new_n47_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z09));
  nand4  g56(.a(new_n98_), .b(new_n54_), .c(new_n48_), .d(new_n47_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z10));
  xor2a  g58(.a(x18), .b(x17), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n98_), .c(new_n48_), .d(new_n47_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n50_), .O(z11));
  nand3  g61(.a(new_n56_), .b(x18), .c(x17), .O(new_n107_));
  oai21  g62(.a(new_n90_), .b(new_n56_), .c(new_n107_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n98_), .c(new_n48_), .d(new_n47_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n50_), .O(z12));
  nand2  g65(.a(new_n91_), .b(new_n90_), .O(new_n111_));
  oai21  g66(.a(new_n99_), .b(x15), .c(new_n47_), .O(new_n112_));
  nand3  g67(.a(x19), .b(x18), .c(x17), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x20), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n47_), .O(z13));
  nand2  g70(.a(new_n111_), .b(x21), .O(new_n116_));
  nor2   g71(.a(x21), .b(x20), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n90_), .c(x19), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n116_), .c(new_n112_), .d(new_n47_), .O(z14));
  nand2  g74(.a(new_n118_), .b(x22), .O(new_n120_));
  inv1   g75(.a(new_n113_), .O(new_n121_));
  nor3   g76(.a(x22), .b(x21), .c(x20), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n120_), .c(new_n112_), .d(new_n47_), .O(z15));
  inv1   g79(.a(x23), .O(new_n125_));
  aoi21  g80(.a(new_n122_), .b(new_n121_), .c(new_n125_), .O(new_n126_));
  nand4  g81(.a(new_n117_), .b(new_n121_), .c(new_n125_), .d(new_n85_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n47_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n126_), .c(new_n48_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n112_), .O(z16));
  nand3  g85(.a(new_n93_), .b(new_n85_), .c(new_n87_), .O(new_n131_));
  nor2   g86(.a(new_n131_), .b(x20), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(x19), .c(x18), .d(x17), .O(new_n133_));
  nand2  g88(.a(new_n127_), .b(x24), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n133_), .c(new_n112_), .d(new_n47_), .O(z17));
  nand2  g90(.a(new_n94_), .b(x25), .O(new_n136_));
  nor3   g91(.a(x25), .b(x24), .c(x23), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n136_), .c(new_n112_), .d(new_n47_), .O(z18));
endmodule


