// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g000(.a(x08), .O(new_n46_));
  nor2   g001(.a(x15), .b(x10), .O(new_n47_));
  nor3   g002(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g003(.a(x10), .O(new_n49_));
  inv1   g004(.a(x15), .O(new_n50_));
  oai21  g005(.a(new_n49_), .b(x07), .c(new_n50_), .O(new_n51_));
  nand3  g006(.a(new_n51_), .b(x05), .c(x04), .O(new_n52_));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  inv1   g009(.a(x17), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand3  g012(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g013(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g014(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  aoi21  g016(.a(new_n61_), .b(new_n53_), .c(x07), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(new_n47_), .c(new_n52_), .O(z01));
  inv1   g018(.a(x16), .O(new_n64_));
  nor2   g019(.a(new_n47_), .b(new_n64_), .O(z02));
  inv1   g020(.a(new_n47_), .O(new_n66_));
  nand3  g021(.a(new_n61_), .b(new_n66_), .c(new_n53_), .O(new_n67_));
  nand2  g022(.a(new_n67_), .b(new_n66_), .O(z03));
  inv1   g023(.a(x09), .O(new_n69_));
  inv1   g024(.a(x02), .O(new_n70_));
  aoi22  g025(.a(x11), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g026(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g027(.a(x11), .O(new_n73_));
  nand2  g028(.a(new_n73_), .b(x02), .O(new_n74_));
  inv1   g029(.a(x03), .O(new_n75_));
  nand2  g030(.a(x12), .b(new_n75_), .O(new_n76_));
  inv1   g031(.a(x12), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(x03), .O(new_n78_));
  nand3  g033(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  oai21  g034(.a(new_n79_), .b(new_n72_), .c(new_n66_), .O(new_n80_));
  nor2   g035(.a(new_n49_), .b(x01), .O(new_n81_));
  nor2   g036(.a(new_n50_), .b(x10), .O(new_n82_));
  aoi21  g037(.a(new_n82_), .b(x01), .c(new_n81_), .O(new_n83_));
  aoi21  g038(.a(new_n83_), .b(new_n80_), .c(x08), .O(z04));
  oai21  g039(.a(x13), .b(x08), .c(new_n66_), .O(z05));
  nand2  g040(.a(x14), .b(new_n46_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(new_n66_), .O(z06));
  aoi21  g042(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g043(.a(x24), .O(new_n89_));
  inv1   g044(.a(x22), .O(new_n90_));
  aoi21  g045(.a(new_n58_), .b(new_n54_), .c(new_n90_), .O(new_n91_));
  aoi21  g046(.a(new_n91_), .b(x21), .c(x23), .O(new_n92_));
  nor2   g047(.a(new_n47_), .b(x25), .O(new_n93_));
  oai21  g048(.a(new_n92_), .b(new_n89_), .c(new_n93_), .O(z08));
  nand2  g049(.a(x05), .b(x04), .O(new_n95_));
  nor4   g050(.a(new_n95_), .b(x15), .c(new_n49_), .d(x07), .O(z09));
  nand4  g051(.a(new_n95_), .b(new_n55_), .c(new_n50_), .d(x10), .O(new_n97_));
  nor2   g052(.a(new_n97_), .b(x07), .O(z10));
  inv1   g053(.a(x07), .O(new_n99_));
  xor2a  g054(.a(x18), .b(x17), .O(new_n100_));
  nand3  g055(.a(new_n100_), .b(new_n95_), .c(new_n99_), .O(new_n101_));
  aoi21  g056(.a(new_n101_), .b(x10), .c(x15), .O(z11));
  nor2   g057(.a(new_n56_), .b(new_n55_), .O(new_n103_));
  nand3  g058(.a(new_n57_), .b(x18), .c(x17), .O(new_n104_));
  oai21  g059(.a(new_n103_), .b(new_n57_), .c(new_n104_), .O(new_n105_));
  nand3  g060(.a(new_n105_), .b(new_n95_), .c(new_n99_), .O(new_n106_));
  aoi21  g061(.a(new_n106_), .b(x10), .c(x15), .O(z12));
  nand3  g062(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(x20), .O(new_n109_));
  nand4  g064(.a(new_n50_), .b(new_n99_), .c(x05), .d(x04), .O(new_n110_));
  nor2   g065(.a(x20), .b(new_n57_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n103_), .O(new_n112_));
  nand4  g067(.a(new_n112_), .b(new_n110_), .c(new_n109_), .d(new_n99_), .O(new_n113_));
  nand2  g068(.a(new_n113_), .b(x10), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n50_), .O(z13));
  nand2  g070(.a(new_n110_), .b(new_n99_), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(x10), .O(new_n117_));
  nor2   g072(.a(x21), .b(x20), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n103_), .c(x19), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n50_), .c(x10), .O(new_n120_));
  aoi21  g075(.a(new_n112_), .b(x21), .c(new_n120_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n117_), .O(z14));
  nand2  g077(.a(new_n119_), .b(x22), .O(new_n123_));
  nor2   g078(.a(x22), .b(x21), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n54_), .O(new_n125_));
  nor2   g080(.a(new_n125_), .b(new_n108_), .O(new_n126_));
  nor2   g081(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n123_), .c(new_n117_), .d(new_n50_), .O(z15));
  inv1   g083(.a(x23), .O(new_n129_));
  nor2   g084(.a(new_n126_), .b(new_n129_), .O(new_n130_));
  inv1   g085(.a(new_n108_), .O(new_n131_));
  nand4  g086(.a(new_n118_), .b(new_n131_), .c(new_n129_), .d(new_n90_), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n110_), .c(new_n99_), .O(new_n133_));
  oai21  g088(.a(new_n133_), .b(new_n130_), .c(x10), .O(new_n134_));
  nand2  g089(.a(new_n134_), .b(new_n50_), .O(z16));
  nand2  g090(.a(new_n132_), .b(x24), .O(new_n136_));
  nand3  g091(.a(new_n124_), .b(new_n89_), .c(new_n129_), .O(new_n137_));
  nor2   g092(.a(new_n137_), .b(new_n112_), .O(new_n138_));
  nor2   g093(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n136_), .c(new_n117_), .d(new_n50_), .O(z17));
  oai21  g095(.a(new_n137_), .b(new_n112_), .c(x25), .O(new_n141_));
  nor3   g096(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nand4  g097(.a(new_n142_), .b(new_n124_), .c(new_n111_), .d(new_n103_), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n141_), .c(new_n110_), .d(new_n99_), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(x10), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n50_), .O(z18));
endmodule


