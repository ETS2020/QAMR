// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x16), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  inv1   g002(.a(x24), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g006(.a(new_n51_), .O(z00));
  oai21  g007(.a(x15), .b(x07), .c(x24), .O(new_n53_));
  nand3  g008(.a(new_n53_), .b(x05), .c(x04), .O(new_n54_));
  nand2  g009(.a(new_n50_), .b(x07), .O(new_n55_));
  inv1   g010(.a(x25), .O(new_n56_));
  inv1   g011(.a(x23), .O(new_n57_));
  nor3   g012(.a(x19), .b(x18), .c(x17), .O(new_n58_));
  inv1   g013(.a(x21), .O(new_n59_));
  inv1   g014(.a(x22), .O(new_n60_));
  nor2   g015(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  oai21  g016(.a(new_n58_), .b(x20), .c(new_n61_), .O(new_n62_));
  nand3  g017(.a(new_n62_), .b(new_n57_), .c(new_n47_), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(x24), .O(new_n64_));
  nand2  g019(.a(new_n64_), .b(new_n56_), .O(new_n65_));
  nand3  g020(.a(new_n65_), .b(new_n55_), .c(new_n54_), .O(z01));
  nand2  g021(.a(new_n50_), .b(new_n46_), .O(z02));
  inv1   g022(.a(new_n65_), .O(z03));
  inv1   g023(.a(x08), .O(new_n69_));
  xnor2a g024(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g025(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g026(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g027(.a(x12), .b(x03), .O(new_n73_));
  nand4  g028(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g029(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(new_n50_), .O(z04));
  inv1   g031(.a(x13), .O(new_n77_));
  nand3  g032(.a(new_n50_), .b(new_n77_), .c(new_n69_), .O(new_n78_));
  inv1   g033(.a(new_n78_), .O(z05));
  nand2  g034(.a(x14), .b(new_n69_), .O(new_n80_));
  nand2  g035(.a(new_n80_), .b(new_n50_), .O(z06));
  aoi21  g036(.a(new_n69_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g037(.a(x17), .O(new_n83_));
  nand3  g038(.a(x24), .b(x22), .c(x21), .O(new_n84_));
  nor4   g039(.a(new_n84_), .b(x19), .c(x18), .d(x15), .O(new_n85_));
  nor2   g040(.a(new_n56_), .b(x24), .O(new_n86_));
  oai21  g041(.a(new_n86_), .b(new_n85_), .c(new_n83_), .O(new_n87_));
  nand4  g042(.a(x24), .b(x22), .c(x21), .d(new_n47_), .O(new_n88_));
  inv1   g043(.a(new_n88_), .O(new_n89_));
  oai21  g044(.a(new_n89_), .b(new_n86_), .c(x20), .O(new_n90_));
  nand2  g045(.a(new_n56_), .b(new_n57_), .O(new_n91_));
  nand3  g046(.a(new_n91_), .b(x24), .c(new_n47_), .O(new_n92_));
  nand3  g047(.a(new_n59_), .b(x19), .c(x18), .O(new_n93_));
  inv1   g048(.a(new_n93_), .O(new_n94_));
  nand3  g049(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g050(.a(new_n95_), .O(new_n96_));
  nor2   g051(.a(x21), .b(x20), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n96_), .c(new_n57_), .d(new_n60_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n94_), .c(new_n57_), .d(new_n60_), .O(new_n99_));
  nand3  g054(.a(new_n99_), .b(x25), .c(new_n48_), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n92_), .c(new_n90_), .d(new_n87_), .O(z08));
  inv1   g056(.a(x07), .O(new_n102_));
  nand4  g057(.a(new_n47_), .b(new_n102_), .c(x05), .d(x04), .O(new_n103_));
  inv1   g058(.a(new_n103_), .O(z09));
  nand2  g059(.a(x05), .b(x04), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n83_), .c(new_n47_), .d(new_n102_), .O(new_n106_));
  nand2  g061(.a(new_n106_), .b(new_n50_), .O(z10));
  xor2a  g062(.a(x18), .b(x17), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n105_), .c(new_n47_), .d(new_n102_), .O(new_n109_));
  inv1   g064(.a(new_n109_), .O(z11));
  inv1   g065(.a(x19), .O(new_n111_));
  and2   g066(.a(x18), .b(x17), .O(new_n112_));
  nand3  g067(.a(new_n111_), .b(x18), .c(x17), .O(new_n113_));
  oai21  g068(.a(new_n112_), .b(new_n111_), .c(new_n113_), .O(new_n114_));
  nand4  g069(.a(new_n114_), .b(new_n105_), .c(new_n47_), .d(new_n102_), .O(new_n115_));
  inv1   g070(.a(new_n115_), .O(z12));
  nand3  g071(.a(new_n102_), .b(x05), .c(x04), .O(new_n117_));
  nand2  g072(.a(new_n117_), .b(new_n102_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n47_), .O(new_n119_));
  nand2  g074(.a(new_n95_), .b(x20), .O(new_n120_));
  inv1   g075(.a(x20), .O(new_n121_));
  nand3  g076(.a(new_n112_), .b(new_n121_), .c(x19), .O(new_n122_));
  nand4  g077(.a(new_n122_), .b(new_n120_), .c(new_n119_), .d(new_n47_), .O(z13));
  nand2  g078(.a(new_n122_), .b(x21), .O(new_n124_));
  nand3  g079(.a(new_n112_), .b(new_n97_), .c(x19), .O(new_n125_));
  nand4  g080(.a(new_n125_), .b(new_n124_), .c(new_n119_), .d(new_n47_), .O(z14));
  nand4  g081(.a(new_n96_), .b(new_n60_), .c(new_n59_), .d(new_n121_), .O(new_n127_));
  nand3  g082(.a(new_n127_), .b(new_n117_), .c(new_n102_), .O(new_n128_));
  aoi21  g083(.a(new_n125_), .b(x22), .c(new_n128_), .O(new_n129_));
  nand2  g084(.a(new_n48_), .b(x15), .O(new_n130_));
  oai21  g085(.a(new_n129_), .b(x15), .c(new_n130_), .O(z15));
  nand2  g086(.a(new_n127_), .b(x23), .O(new_n132_));
  nand4  g087(.a(new_n132_), .b(new_n119_), .c(new_n98_), .d(new_n47_), .O(z16));
  nand4  g088(.a(new_n48_), .b(new_n57_), .c(new_n60_), .d(new_n59_), .O(new_n134_));
  nor2   g089(.a(new_n134_), .b(x20), .O(new_n135_));
  nand4  g090(.a(new_n135_), .b(x19), .c(x18), .d(x17), .O(new_n136_));
  nand2  g091(.a(x24), .b(x23), .O(new_n137_));
  nand3  g092(.a(new_n137_), .b(new_n117_), .c(new_n102_), .O(new_n138_));
  aoi22  g093(.a(new_n138_), .b(new_n47_), .c(new_n127_), .d(x24), .O(new_n139_));
  nand3  g094(.a(new_n139_), .b(new_n136_), .c(new_n47_), .O(z17));
  nand2  g095(.a(x25), .b(x24), .O(new_n141_));
  nand3  g096(.a(new_n141_), .b(new_n117_), .c(new_n102_), .O(new_n142_));
  inv1   g097(.a(new_n142_), .O(new_n143_));
  nand2  g098(.a(new_n98_), .b(x25), .O(new_n144_));
  nand4  g099(.a(new_n56_), .b(new_n57_), .c(new_n60_), .d(new_n59_), .O(new_n145_));
  oai21  g100(.a(new_n145_), .b(new_n122_), .c(new_n144_), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(new_n48_), .O(new_n147_));
  nand3  g102(.a(new_n147_), .b(new_n143_), .c(new_n47_), .O(z18));
endmodule


