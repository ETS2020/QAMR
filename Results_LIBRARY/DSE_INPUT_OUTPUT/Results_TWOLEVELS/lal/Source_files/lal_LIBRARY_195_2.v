// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n106_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_;
  inv1   g000(.a(x08), .O(new_n46_));
  aoi21  g001(.a(x16), .b(x15), .c(new_n46_), .O(z00));
  inv1   g002(.a(x07), .O(new_n48_));
  inv1   g003(.a(x15), .O(new_n49_));
  nand2  g004(.a(new_n49_), .b(x08), .O(new_n50_));
  inv1   g005(.a(x25), .O(new_n51_));
  inv1   g006(.a(x20), .O(new_n52_));
  inv1   g007(.a(x17), .O(new_n53_));
  nor2   g008(.a(x19), .b(x18), .O(new_n54_));
  nand2  g009(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g010(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n56_), .O(new_n57_));
  oai21  g012(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand3  g013(.a(new_n58_), .b(new_n50_), .c(new_n51_), .O(new_n59_));
  nand2  g014(.a(x05), .b(x04), .O(new_n60_));
  nand4  g015(.a(new_n60_), .b(new_n59_), .c(new_n50_), .d(new_n48_), .O(z01));
  inv1   g016(.a(x16), .O(new_n62_));
  aoi21  g017(.a(new_n49_), .b(x08), .c(new_n62_), .O(z02));
  inv1   g018(.a(new_n59_), .O(z03));
  xnor2a g019(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g020(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g021(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g022(.a(x12), .b(x03), .O(new_n68_));
  nand4  g023(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g024(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g025(.a(new_n70_), .b(new_n50_), .O(z04));
  oai21  g026(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g027(.a(x14), .O(new_n73_));
  nor2   g028(.a(new_n73_), .b(x08), .O(z06));
  nand2  g029(.a(x15), .b(x08), .O(new_n75_));
  oai21  g030(.a(x08), .b(x06), .c(new_n75_), .O(z07));
  inv1   g031(.a(new_n54_), .O(new_n77_));
  nand3  g032(.a(x24), .b(x22), .c(x21), .O(new_n78_));
  oai21  g033(.a(new_n78_), .b(new_n77_), .c(new_n51_), .O(new_n79_));
  nand2  g034(.a(new_n79_), .b(new_n53_), .O(new_n80_));
  nand2  g035(.a(new_n78_), .b(new_n51_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(x20), .O(new_n82_));
  oai21  g037(.a(x25), .b(x24), .c(x23), .O(new_n83_));
  inv1   g038(.a(x21), .O(new_n84_));
  nand3  g039(.a(new_n84_), .b(x19), .c(x18), .O(new_n85_));
  inv1   g040(.a(x22), .O(new_n86_));
  inv1   g041(.a(x24), .O(new_n87_));
  inv1   g042(.a(x18), .O(new_n88_));
  nor2   g043(.a(new_n88_), .b(new_n53_), .O(new_n89_));
  inv1   g044(.a(x19), .O(new_n90_));
  nor2   g045(.a(x20), .b(new_n90_), .O(new_n91_));
  nor2   g046(.a(x22), .b(x21), .O(new_n92_));
  nor2   g047(.a(x24), .b(x23), .O(new_n93_));
  nand4  g048(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n89_), .O(new_n94_));
  nand3  g049(.a(new_n94_), .b(new_n87_), .c(new_n86_), .O(new_n95_));
  oai21  g050(.a(new_n95_), .b(new_n85_), .c(x25), .O(new_n96_));
  nand4  g051(.a(new_n96_), .b(new_n83_), .c(new_n82_), .d(new_n80_), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(new_n46_), .O(new_n98_));
  inv1   g053(.a(x23), .O(new_n99_));
  nand2  g054(.a(new_n55_), .b(new_n52_), .O(new_n100_));
  nand3  g055(.a(new_n100_), .b(x22), .c(x21), .O(new_n101_));
  aoi21  g056(.a(new_n101_), .b(new_n99_), .c(new_n87_), .O(new_n102_));
  oai21  g057(.a(new_n102_), .b(x25), .c(x15), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n98_), .O(z08));
  nor4   g059(.a(new_n60_), .b(x15), .c(x08), .d(x07), .O(z09));
  nand4  g060(.a(new_n60_), .b(new_n53_), .c(new_n49_), .d(new_n46_), .O(new_n106_));
  nor2   g061(.a(new_n106_), .b(x07), .O(z10));
  xor2a  g062(.a(x18), .b(x17), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n60_), .c(new_n49_), .d(new_n46_), .O(new_n109_));
  nor2   g064(.a(new_n109_), .b(x07), .O(z11));
  nand3  g065(.a(new_n90_), .b(x18), .c(x17), .O(new_n111_));
  oai21  g066(.a(new_n89_), .b(new_n90_), .c(new_n111_), .O(new_n112_));
  nand4  g067(.a(new_n112_), .b(new_n60_), .c(new_n49_), .d(new_n46_), .O(new_n113_));
  nor2   g068(.a(new_n113_), .b(x07), .O(z12));
  oai21  g069(.a(new_n60_), .b(x15), .c(new_n48_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n46_), .O(new_n116_));
  nor2   g071(.a(new_n90_), .b(new_n88_), .O(new_n117_));
  inv1   g072(.a(new_n117_), .O(new_n118_));
  nand3  g073(.a(new_n51_), .b(new_n99_), .c(x15), .O(new_n119_));
  aoi21  g074(.a(new_n119_), .b(new_n118_), .c(new_n53_), .O(new_n120_));
  nand4  g075(.a(new_n77_), .b(new_n51_), .c(new_n99_), .d(x15), .O(new_n121_));
  inv1   g076(.a(new_n121_), .O(new_n122_));
  oai21  g077(.a(new_n122_), .b(new_n120_), .c(new_n52_), .O(new_n123_));
  nor3   g078(.a(new_n78_), .b(new_n77_), .c(new_n49_), .O(new_n124_));
  oai21  g079(.a(new_n124_), .b(x20), .c(new_n53_), .O(new_n125_));
  aoi21  g080(.a(new_n56_), .b(new_n99_), .c(new_n87_), .O(new_n126_));
  oai21  g081(.a(new_n56_), .b(new_n52_), .c(new_n99_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(x24), .O(new_n128_));
  nand3  g083(.a(new_n128_), .b(new_n126_), .c(new_n51_), .O(new_n129_));
  oai21  g084(.a(new_n117_), .b(new_n52_), .c(new_n50_), .O(new_n130_));
  aoi21  g085(.a(new_n129_), .b(x15), .c(new_n130_), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n125_), .c(new_n123_), .d(new_n116_), .O(z13));
  nand3  g087(.a(new_n84_), .b(x18), .c(x17), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n119_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n52_), .c(x19), .O(new_n135_));
  nand4  g090(.a(new_n51_), .b(x24), .c(new_n99_), .d(x21), .O(new_n136_));
  nand2  g091(.a(new_n136_), .b(x15), .O(new_n137_));
  inv1   g092(.a(new_n50_), .O(new_n138_));
  nand2  g093(.a(new_n91_), .b(new_n89_), .O(new_n139_));
  aoi21  g094(.a(new_n139_), .b(x21), .c(new_n138_), .O(new_n140_));
  nand4  g095(.a(new_n140_), .b(new_n137_), .c(new_n135_), .d(new_n116_), .O(z14));
  nand2  g096(.a(new_n92_), .b(new_n89_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(new_n119_), .O(new_n143_));
  nand3  g098(.a(new_n143_), .b(new_n52_), .c(x19), .O(new_n144_));
  nand4  g099(.a(new_n51_), .b(x24), .c(new_n99_), .d(x22), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(x15), .O(new_n146_));
  nand4  g101(.a(new_n89_), .b(new_n84_), .c(new_n52_), .d(x19), .O(new_n147_));
  aoi21  g102(.a(new_n147_), .b(x22), .c(new_n138_), .O(new_n148_));
  nand4  g103(.a(new_n148_), .b(new_n146_), .c(new_n144_), .d(new_n116_), .O(z15));
  nand2  g104(.a(x18), .b(new_n46_), .O(new_n150_));
  nand2  g105(.a(new_n92_), .b(x19), .O(new_n151_));
  nand2  g106(.a(new_n51_), .b(x15), .O(new_n152_));
  oai21  g107(.a(new_n151_), .b(new_n150_), .c(new_n152_), .O(new_n153_));
  nand2  g108(.a(new_n153_), .b(x17), .O(new_n154_));
  nand3  g109(.a(new_n77_), .b(new_n51_), .c(x15), .O(new_n155_));
  aoi21  g110(.a(new_n155_), .b(new_n154_), .c(x20), .O(new_n156_));
  nand3  g111(.a(new_n56_), .b(new_n51_), .c(x15), .O(new_n157_));
  inv1   g112(.a(new_n157_), .O(new_n158_));
  oai21  g113(.a(new_n158_), .b(new_n156_), .c(new_n99_), .O(new_n159_));
  nor2   g114(.a(new_n99_), .b(x08), .O(new_n160_));
  oai21  g115(.a(new_n160_), .b(new_n124_), .c(new_n53_), .O(new_n161_));
  nand4  g116(.a(x24), .b(x22), .c(x21), .d(x15), .O(new_n162_));
  inv1   g117(.a(new_n162_), .O(new_n163_));
  oai21  g118(.a(new_n163_), .b(new_n160_), .c(x20), .O(new_n164_));
  nand3  g119(.a(new_n51_), .b(x24), .c(new_n99_), .O(new_n165_));
  nand2  g120(.a(new_n165_), .b(x15), .O(new_n166_));
  aoi21  g121(.a(new_n117_), .b(new_n92_), .c(new_n99_), .O(new_n167_));
  oai21  g122(.a(new_n167_), .b(new_n115_), .c(new_n46_), .O(new_n168_));
  nand3  g123(.a(new_n168_), .b(new_n166_), .c(new_n164_), .O(new_n169_));
  inv1   g124(.a(new_n169_), .O(new_n170_));
  nand3  g125(.a(new_n170_), .b(new_n161_), .c(new_n159_), .O(z16));
  oai21  g126(.a(new_n60_), .b(x07), .c(new_n46_), .O(new_n172_));
  nand2  g127(.a(new_n172_), .b(new_n49_), .O(new_n173_));
  nand2  g128(.a(x25), .b(x08), .O(new_n174_));
  nand4  g129(.a(new_n174_), .b(new_n87_), .c(new_n84_), .d(new_n52_), .O(new_n175_));
  inv1   g130(.a(new_n175_), .O(new_n176_));
  nand4  g131(.a(new_n176_), .b(x19), .c(x18), .d(x17), .O(new_n177_));
  nand2  g132(.a(new_n177_), .b(new_n152_), .O(new_n178_));
  nand3  g133(.a(new_n178_), .b(new_n99_), .c(new_n86_), .O(new_n179_));
  nand2  g134(.a(x24), .b(x23), .O(new_n180_));
  nand2  g135(.a(new_n180_), .b(new_n48_), .O(new_n181_));
  nand2  g136(.a(new_n181_), .b(new_n46_), .O(new_n182_));
  nand2  g137(.a(new_n117_), .b(x17), .O(new_n183_));
  nand2  g138(.a(new_n92_), .b(new_n52_), .O(new_n184_));
  oai21  g139(.a(new_n184_), .b(new_n183_), .c(x24), .O(new_n185_));
  nand3  g140(.a(new_n185_), .b(new_n182_), .c(new_n166_), .O(new_n186_));
  inv1   g141(.a(new_n186_), .O(new_n187_));
  nand3  g142(.a(new_n187_), .b(new_n179_), .c(new_n173_), .O(z17));
  nor2   g143(.a(x24), .b(x22), .O(new_n189_));
  nand4  g144(.a(new_n189_), .b(new_n84_), .c(x19), .d(x18), .O(new_n190_));
  aoi21  g145(.a(new_n190_), .b(new_n49_), .c(new_n53_), .O(new_n191_));
  nor2   g146(.a(new_n54_), .b(new_n49_), .O(new_n192_));
  oai21  g147(.a(new_n192_), .b(new_n191_), .c(new_n52_), .O(new_n193_));
  nand2  g148(.a(new_n56_), .b(x15), .O(new_n194_));
  aoi21  g149(.a(new_n194_), .b(new_n193_), .c(x23), .O(new_n195_));
  nor2   g150(.a(x24), .b(new_n49_), .O(new_n196_));
  oai21  g151(.a(new_n196_), .b(new_n195_), .c(new_n51_), .O(new_n197_));
  nor2   g152(.a(new_n51_), .b(x08), .O(new_n198_));
  oai21  g153(.a(new_n198_), .b(new_n124_), .c(new_n53_), .O(new_n199_));
  oai21  g154(.a(new_n198_), .b(new_n163_), .c(x20), .O(new_n200_));
  nand2  g155(.a(new_n180_), .b(new_n51_), .O(new_n201_));
  nand2  g156(.a(new_n201_), .b(x15), .O(new_n202_));
  nand2  g157(.a(new_n93_), .b(new_n86_), .O(new_n203_));
  oai21  g158(.a(new_n203_), .b(new_n85_), .c(x25), .O(new_n204_));
  nand2  g159(.a(new_n204_), .b(new_n48_), .O(new_n205_));
  nand2  g160(.a(new_n205_), .b(new_n46_), .O(new_n206_));
  nand4  g161(.a(new_n206_), .b(new_n202_), .c(new_n200_), .d(new_n199_), .O(new_n207_));
  inv1   g162(.a(new_n207_), .O(new_n208_));
  nand3  g163(.a(new_n208_), .b(new_n197_), .c(new_n173_), .O(z18));
endmodule


