// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x07), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  nand2  g18(.a(new_n60_), .b(new_n47_), .O(z03));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n47_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g29(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g30(.a(x22), .O(new_n76_));
  aoi21  g31(.a(new_n56_), .b(new_n52_), .c(new_n76_), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(x21), .c(x23), .O(new_n78_));
  aoi21  g33(.a(x15), .b(x07), .c(x25), .O(new_n79_));
  oai21  g34(.a(new_n78_), .b(new_n50_), .c(new_n79_), .O(z08));
  inv1   g35(.a(x15), .O(new_n81_));
  nand4  g36(.a(new_n81_), .b(new_n49_), .c(x05), .d(x04), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand4  g38(.a(new_n61_), .b(new_n53_), .c(new_n81_), .d(new_n49_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z10));
  xor2a  g40(.a(x18), .b(x17), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n61_), .c(new_n81_), .d(new_n49_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n47_), .O(z11));
  nor2   g43(.a(new_n54_), .b(new_n53_), .O(new_n89_));
  nand3  g44(.a(new_n55_), .b(x18), .c(x17), .O(new_n90_));
  oai21  g45(.a(new_n89_), .b(new_n55_), .c(new_n90_), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n61_), .c(new_n81_), .d(new_n49_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z12));
  aoi21  g48(.a(x05), .b(x04), .c(x15), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x20), .O(new_n96_));
  nor2   g51(.a(x20), .b(new_n55_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n89_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n96_), .c(new_n94_), .d(new_n49_), .O(z13));
  or2    g54(.a(new_n94_), .b(x07), .O(new_n100_));
  nand2  g55(.a(new_n98_), .b(x21), .O(new_n101_));
  nor2   g56(.a(x21), .b(x20), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n89_), .c(x19), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .d(new_n49_), .O(z14));
  nand2  g59(.a(new_n103_), .b(x22), .O(new_n105_));
  inv1   g60(.a(new_n95_), .O(new_n106_));
  nor2   g61(.a(x22), .b(x21), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n52_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n105_), .c(new_n100_), .d(new_n49_), .O(z15));
  nand2  g64(.a(new_n108_), .b(x23), .O(new_n110_));
  inv1   g65(.a(x23), .O(new_n111_));
  nand3  g66(.a(new_n102_), .b(new_n111_), .c(new_n76_), .O(new_n112_));
  inv1   g67(.a(new_n112_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n106_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n110_), .c(new_n100_), .d(new_n49_), .O(z16));
  oai21  g70(.a(new_n112_), .b(new_n95_), .c(x24), .O(new_n116_));
  nor2   g71(.a(x24), .b(x23), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n107_), .c(new_n97_), .d(new_n89_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n49_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n81_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n100_), .O(z17));
  nand2  g76(.a(new_n118_), .b(x25), .O(new_n122_));
  nor3   g77(.a(x25), .b(x24), .c(x23), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n107_), .c(new_n97_), .d(new_n89_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n122_), .c(new_n100_), .d(new_n49_), .O(z18));
endmodule


