// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x22), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  nand2  g08(.a(x05), .b(x04), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n56_), .c(new_n57_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x23), .c(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n54_), .c(new_n50_), .d(new_n53_), .O(z01));
  nand2  g20(.a(new_n64_), .b(new_n50_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z04));
  nor3   g28(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z06));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x22), .O(new_n78_));
  inv1   g33(.a(x23), .O(new_n79_));
  nor2   g34(.a(x19), .b(x18), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n58_), .c(x20), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n57_), .c(new_n79_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x24), .c(x25), .O(new_n83_));
  nand2  g38(.a(x25), .b(new_n48_), .O(new_n84_));
  oai21  g39(.a(new_n83_), .b(new_n78_), .c(new_n84_), .O(z08));
  inv1   g40(.a(new_n54_), .O(new_n86_));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n50_), .O(z09));
  inv1   g44(.a(x15), .O(new_n90_));
  nand4  g45(.a(new_n54_), .b(new_n50_), .c(new_n58_), .d(new_n90_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x07), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n54_), .c(new_n90_), .d(new_n53_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n50_), .O(z11));
  nor2   g50(.a(new_n59_), .b(new_n58_), .O(new_n96_));
  nand3  g51(.a(new_n60_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n60_), .c(new_n97_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n54_), .c(new_n90_), .d(new_n53_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n50_), .O(z12));
  inv1   g55(.a(new_n87_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n50_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n96_), .b(new_n56_), .c(x19), .O(new_n105_));
  and2   g60(.a(new_n105_), .b(new_n50_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n102_), .d(new_n88_), .O(z13));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n96_), .c(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n88_), .c(new_n50_), .O(new_n110_));
  aoi21  g65(.a(new_n105_), .b(x21), .c(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n102_), .O(z14));
  oai21  g67(.a(new_n101_), .b(new_n86_), .c(new_n50_), .O(new_n113_));
  nand4  g68(.a(new_n54_), .b(new_n58_), .c(new_n90_), .d(new_n53_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n108_), .c(x19), .d(x18), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x22), .O(new_n116_));
  inv1   g71(.a(new_n103_), .O(new_n117_));
  nand4  g72(.a(new_n108_), .b(new_n117_), .c(new_n48_), .d(new_n78_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n113_), .O(z15));
  nand3  g74(.a(new_n79_), .b(new_n57_), .c(new_n56_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n103_), .c(new_n48_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n78_), .O(new_n122_));
  nand3  g77(.a(new_n78_), .b(new_n57_), .c(new_n56_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n103_), .c(x23), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n122_), .c(new_n102_), .d(new_n88_), .O(z16));
  nand4  g80(.a(new_n108_), .b(new_n117_), .c(new_n79_), .d(new_n78_), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n87_), .c(new_n54_), .d(new_n48_), .O(z17));
  nand3  g82(.a(new_n108_), .b(new_n55_), .c(new_n79_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n103_), .c(new_n48_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n78_), .O(new_n130_));
  nand2  g85(.a(new_n126_), .b(x25), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n130_), .c(new_n102_), .d(new_n88_), .O(z18));
endmodule


