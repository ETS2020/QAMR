// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  inv1   g15(.a(x04), .O(new_n61_));
  inv1   g16(.a(x05), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g18(.a(new_n60_), .b(new_n52_), .c(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n48_), .b(x07), .O(new_n65_));
  oai21  g20(.a(new_n64_), .b(x07), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n49_), .b(new_n67_), .O(z02));
  nand3  g23(.a(new_n60_), .b(new_n50_), .c(new_n52_), .O(new_n69_));
  inv1   g24(.a(new_n69_), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z04));
  inv1   g32(.a(x13), .O(new_n78_));
  nand3  g33(.a(new_n50_), .b(new_n78_), .c(new_n46_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z05));
  nand2  g35(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n50_), .O(z06));
  nand3  g37(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g38(.a(new_n56_), .b(new_n55_), .O(new_n84_));
  nand3  g39(.a(x24), .b(x22), .c(x21), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n84_), .c(new_n52_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n54_), .O(new_n87_));
  nand2  g42(.a(new_n85_), .b(new_n52_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(x20), .O(new_n89_));
  oai21  g44(.a(x25), .b(x24), .c(x23), .O(new_n90_));
  inv1   g45(.a(x22), .O(new_n91_));
  inv1   g46(.a(x24), .O(new_n92_));
  inv1   g47(.a(x21), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(x19), .c(x18), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor2   g50(.a(new_n55_), .b(new_n54_), .O(new_n96_));
  nor2   g51(.a(x20), .b(new_n56_), .O(new_n97_));
  nor2   g52(.a(x22), .b(x21), .O(new_n98_));
  nor2   g53(.a(x24), .b(x23), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n95_), .c(new_n92_), .d(new_n91_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(x25), .c(new_n49_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n90_), .c(new_n89_), .d(new_n87_), .O(z08));
  nand3  g58(.a(new_n63_), .b(new_n48_), .c(new_n47_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z09));
  inv1   g60(.a(new_n63_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n54_), .c(new_n48_), .d(new_n47_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n50_), .O(z10));
  xor2a  g63(.a(x18), .b(x17), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n106_), .c(new_n48_), .d(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n50_), .O(z11));
  nand3  g66(.a(new_n56_), .b(x18), .c(x17), .O(new_n112_));
  oai21  g67(.a(new_n96_), .b(new_n56_), .c(new_n112_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n106_), .c(new_n48_), .d(new_n47_), .O(new_n114_));
  inv1   g69(.a(new_n114_), .O(z12));
  nand2  g70(.a(new_n97_), .b(new_n96_), .O(new_n116_));
  oai21  g71(.a(new_n62_), .b(new_n61_), .c(new_n48_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n47_), .O(new_n118_));
  nand3  g73(.a(x19), .b(x18), .c(x17), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x20), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n118_), .c(new_n116_), .d(new_n47_), .O(z13));
  nor2   g76(.a(x21), .b(x20), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n96_), .c(x19), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n47_), .O(new_n124_));
  aoi21  g79(.a(new_n116_), .b(x21), .c(new_n124_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(x15), .c(new_n118_), .O(z14));
  inv1   g81(.a(new_n119_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n98_), .c(new_n53_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n47_), .O(new_n129_));
  aoi21  g84(.a(new_n123_), .b(x22), .c(new_n129_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(x15), .c(new_n118_), .O(z15));
  nand2  g86(.a(new_n128_), .b(x23), .O(new_n132_));
  nor2   g87(.a(x23), .b(x22), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n122_), .c(new_n127_), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n132_), .c(new_n118_), .d(new_n47_), .O(z16));
  nand3  g90(.a(new_n99_), .b(new_n91_), .c(new_n93_), .O(new_n136_));
  nor2   g91(.a(new_n136_), .b(x20), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(x19), .c(x18), .d(x17), .O(new_n138_));
  nand2  g93(.a(new_n134_), .b(x24), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n138_), .c(new_n118_), .d(new_n47_), .O(z17));
  nand2  g95(.a(new_n100_), .b(x25), .O(new_n141_));
  nor3   g96(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nand4  g97(.a(new_n142_), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n143_));
  nand4  g98(.a(new_n143_), .b(new_n141_), .c(new_n118_), .d(new_n47_), .O(z18));
endmodule


