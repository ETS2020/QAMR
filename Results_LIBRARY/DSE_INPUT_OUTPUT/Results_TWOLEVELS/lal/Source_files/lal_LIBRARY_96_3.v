// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  aoi21  g17(.a(new_n60_), .b(new_n52_), .c(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n48_), .b(x07), .O(new_n64_));
  oai21  g19(.a(new_n63_), .b(x07), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n50_), .b(new_n66_), .O(z02));
  nand2  g22(.a(new_n60_), .b(new_n52_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n50_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n50_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand3  g35(.a(x24), .b(x22), .c(x21), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n52_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x20), .O(new_n83_));
  nor4   g38(.a(new_n58_), .b(x19), .c(x18), .d(x17), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x23), .c(x24), .O(new_n85_));
  aoi21  g40(.a(x25), .b(new_n53_), .c(new_n49_), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n85_), .c(new_n83_), .O(z08));
  nand3  g42(.a(new_n62_), .b(new_n48_), .c(new_n47_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n50_), .O(z09));
  nand4  g44(.a(new_n61_), .b(new_n54_), .c(new_n48_), .d(new_n47_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n61_), .c(new_n48_), .d(new_n47_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z11));
  nor2   g49(.a(new_n55_), .b(new_n54_), .O(new_n95_));
  nand3  g50(.a(new_n56_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n56_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n61_), .c(new_n48_), .d(new_n47_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n50_), .O(z12));
  nand3  g54(.a(new_n48_), .b(x05), .c(x04), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n48_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n95_), .b(new_n53_), .c(x19), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n104_), .c(new_n102_), .d(new_n47_), .O(z13));
  nand2  g61(.a(new_n105_), .b(x21), .O(new_n107_));
  inv1   g62(.a(x21), .O(new_n108_));
  nand4  g63(.a(new_n95_), .b(new_n108_), .c(new_n53_), .d(x19), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n107_), .c(new_n102_), .d(new_n47_), .O(z14));
  nor2   g65(.a(x22), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n53_), .O(new_n112_));
  nor2   g67(.a(new_n112_), .b(new_n103_), .O(new_n113_));
  aoi21  g68(.a(new_n109_), .b(x22), .c(new_n113_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n102_), .c(new_n47_), .O(z15));
  inv1   g70(.a(x23), .O(new_n116_));
  nor2   g71(.a(new_n113_), .b(new_n116_), .O(new_n117_));
  inv1   g72(.a(x22), .O(new_n118_));
  nand4  g73(.a(new_n116_), .b(new_n118_), .c(new_n108_), .d(new_n53_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n103_), .c(new_n47_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n117_), .c(new_n48_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n102_), .O(z16));
  nand2  g77(.a(x24), .b(x23), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n100_), .c(new_n48_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n47_), .O(new_n125_));
  inv1   g80(.a(x24), .O(new_n126_));
  nor2   g81(.a(new_n113_), .b(new_n126_), .O(new_n127_));
  nand3  g82(.a(new_n111_), .b(new_n126_), .c(new_n116_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n105_), .c(new_n47_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n127_), .c(new_n48_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n125_), .O(z17));
  oai21  g86(.a(new_n128_), .b(new_n105_), .c(x25), .O(new_n132_));
  inv1   g87(.a(new_n105_), .O(new_n133_));
  nor3   g88(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n111_), .c(new_n133_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n132_), .c(new_n102_), .d(new_n47_), .O(z18));
endmodule


