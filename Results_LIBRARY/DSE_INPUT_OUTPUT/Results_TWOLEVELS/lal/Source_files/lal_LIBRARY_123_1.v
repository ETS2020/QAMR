// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x15), .O(new_n51_));
  oai21  g06(.a(x24), .b(x07), .c(new_n51_), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  nand2  g08(.a(new_n49_), .b(x07), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  nor3   g11(.a(x19), .b(x18), .c(x17), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  oai21  g13(.a(new_n57_), .b(x20), .c(new_n58_), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n56_), .c(x15), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n55_), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n54_), .c(new_n53_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n48_), .b(new_n64_), .O(z02));
  inv1   g20(.a(new_n62_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n49_), .c(new_n46_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z06));
  nand3  g31(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x20), .O(new_n78_));
  inv1   g33(.a(x22), .O(new_n79_));
  inv1   g34(.a(new_n57_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n78_), .c(new_n79_), .O(new_n81_));
  nand2  g36(.a(new_n56_), .b(x15), .O(new_n82_));
  aoi21  g37(.a(new_n81_), .b(x21), .c(new_n82_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n47_), .c(new_n55_), .O(z08));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x07), .c(new_n47_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n51_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  inv1   g43(.a(x17), .O(new_n89_));
  nand4  g44(.a(new_n85_), .b(new_n47_), .c(new_n89_), .d(new_n51_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x07), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n85_), .c(new_n47_), .d(new_n51_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(x07), .O(z11));
  inv1   g49(.a(x19), .O(new_n95_));
  and2   g50(.a(x18), .b(x17), .O(new_n96_));
  nand3  g51(.a(new_n95_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n85_), .c(new_n47_), .d(new_n51_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(x07), .O(z12));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x20), .O(new_n102_));
  nor2   g57(.a(x20), .b(new_n95_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n96_), .O(new_n104_));
  nand2  g59(.a(new_n47_), .b(x07), .O(new_n105_));
  and2   g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n102_), .c(new_n87_), .d(new_n51_), .O(z13));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n96_), .c(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n105_), .c(new_n51_), .O(new_n110_));
  aoi21  g65(.a(new_n104_), .b(x21), .c(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n87_), .O(z14));
  inv1   g67(.a(x07), .O(new_n113_));
  nand4  g68(.a(new_n51_), .b(new_n113_), .c(x05), .d(x04), .O(new_n114_));
  inv1   g69(.a(new_n101_), .O(new_n115_));
  nor3   g70(.a(x22), .b(x21), .c(x20), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n114_), .c(new_n113_), .O(new_n118_));
  aoi21  g73(.a(new_n109_), .b(x22), .c(new_n118_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x24), .c(new_n51_), .O(z15));
  aoi21  g75(.a(new_n116_), .b(new_n115_), .c(new_n56_), .O(new_n121_));
  nand4  g76(.a(new_n108_), .b(new_n115_), .c(new_n56_), .d(new_n79_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n114_), .c(new_n113_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n121_), .c(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n51_), .O(z16));
  nand2  g80(.a(new_n122_), .b(new_n113_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n87_), .c(new_n51_), .O(z17));
  nor2   g83(.a(x22), .b(x21), .O(new_n129_));
  nor2   g84(.a(x25), .b(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n129_), .c(new_n103_), .d(new_n96_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n114_), .c(new_n113_), .O(new_n132_));
  aoi21  g87(.a(new_n122_), .b(x25), .c(new_n132_), .O(new_n133_));
  oai21  g88(.a(new_n133_), .b(x24), .c(new_n51_), .O(z18));
endmodule


