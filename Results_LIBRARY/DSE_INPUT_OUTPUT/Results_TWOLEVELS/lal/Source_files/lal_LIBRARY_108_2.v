// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x14), .O(new_n48_));
  nor2   g03(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  inv1   g15(.a(x21), .O(new_n61_));
  inv1   g16(.a(x22), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(x23), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n53_), .c(new_n54_), .O(new_n65_));
  nand2  g20(.a(x05), .b(x04), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  nor3   g22(.a(new_n67_), .b(new_n49_), .c(x07), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n65_), .O(z01));
  nand2  g24(.a(new_n65_), .b(new_n50_), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z04));
  nor3   g32(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  aoi21  g33(.a(x15), .b(x08), .c(new_n48_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g35(.a(new_n59_), .b(new_n55_), .c(new_n62_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x21), .c(x23), .O(new_n82_));
  nor2   g37(.a(new_n49_), .b(x25), .O(new_n83_));
  oai21  g38(.a(new_n82_), .b(new_n53_), .c(new_n83_), .O(z08));
  nor4   g39(.a(new_n66_), .b(x15), .c(x14), .d(x07), .O(z09));
  inv1   g40(.a(x15), .O(new_n86_));
  nand4  g41(.a(new_n66_), .b(new_n56_), .c(new_n86_), .d(new_n48_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x07), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n66_), .c(new_n86_), .d(new_n48_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x07), .O(z11));
  nor2   g46(.a(new_n57_), .b(new_n56_), .O(new_n92_));
  nand3  g47(.a(new_n58_), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n58_), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n66_), .c(new_n86_), .d(new_n48_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z12));
  inv1   g51(.a(x07), .O(new_n97_));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x20), .O(new_n99_));
  nand3  g54(.a(new_n67_), .b(new_n86_), .c(new_n97_), .O(new_n100_));
  nand4  g55(.a(new_n55_), .b(x19), .c(x18), .d(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n97_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n86_), .O(z13));
  nand2  g59(.a(new_n100_), .b(new_n97_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n48_), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n92_), .c(x19), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n86_), .c(new_n48_), .O(new_n109_));
  aoi21  g64(.a(new_n101_), .b(x21), .c(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n106_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  inv1   g67(.a(new_n98_), .O(new_n113_));
  nor3   g68(.a(x22), .b(x21), .c(x20), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(new_n49_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n106_), .d(new_n86_), .O(z15));
  inv1   g71(.a(x23), .O(new_n117_));
  aoi21  g72(.a(new_n114_), .b(new_n113_), .c(new_n117_), .O(new_n118_));
  nand4  g73(.a(new_n107_), .b(new_n113_), .c(new_n117_), .d(new_n62_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n100_), .c(new_n97_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n118_), .c(new_n48_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n86_), .O(z16));
  nand2  g77(.a(new_n119_), .b(x24), .O(new_n123_));
  nand4  g78(.a(new_n53_), .b(new_n117_), .c(new_n62_), .d(new_n61_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n101_), .O(new_n125_));
  nor2   g80(.a(new_n125_), .b(new_n49_), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n123_), .c(new_n106_), .d(new_n86_), .O(z17));
  oai21  g82(.a(new_n124_), .b(new_n101_), .c(x25), .O(new_n128_));
  nor2   g83(.a(x20), .b(new_n58_), .O(new_n129_));
  nor2   g84(.a(x22), .b(x21), .O(new_n130_));
  nor3   g85(.a(x25), .b(x24), .c(x23), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n92_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n128_), .c(new_n100_), .d(new_n97_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n48_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n86_), .O(z18));
endmodule


