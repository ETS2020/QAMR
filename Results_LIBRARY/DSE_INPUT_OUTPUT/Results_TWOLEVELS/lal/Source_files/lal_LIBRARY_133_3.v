// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x08), .O(new_n46_));
  nand2  g001(.a(x23), .b(x15), .O(new_n47_));
  oai21  g002(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  oai21  g003(.a(x15), .b(x07), .c(x23), .O(new_n49_));
  nand3  g004(.a(new_n49_), .b(x05), .c(x04), .O(new_n50_));
  nor2   g005(.a(x25), .b(x24), .O(new_n51_));
  oai21  g006(.a(new_n51_), .b(x07), .c(new_n47_), .O(new_n52_));
  inv1   g007(.a(x23), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x20), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(x17), .c(new_n55_), .O(new_n59_));
  nand3  g014(.a(new_n59_), .b(x22), .c(x21), .O(new_n60_));
  nand3  g015(.a(new_n60_), .b(new_n54_), .c(new_n53_), .O(new_n61_));
  nand3  g016(.a(new_n61_), .b(new_n52_), .c(new_n50_), .O(z01));
  inv1   g017(.a(x16), .O(new_n63_));
  nand2  g018(.a(new_n47_), .b(new_n63_), .O(z02));
  inv1   g019(.a(x24), .O(new_n65_));
  nand2  g020(.a(new_n47_), .b(new_n65_), .O(new_n66_));
  nand2  g021(.a(new_n60_), .b(new_n53_), .O(new_n67_));
  aoi21  g022(.a(new_n67_), .b(new_n66_), .c(x25), .O(z03));
  xnor2a g023(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g024(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g025(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g026(.a(x12), .b(x03), .O(new_n72_));
  nand4  g027(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g028(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g029(.a(new_n74_), .b(new_n47_), .O(z04));
  oai21  g030(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g031(.a(x14), .O(new_n77_));
  oai21  g032(.a(new_n77_), .b(x08), .c(new_n47_), .O(z06));
  aoi22  g033(.a(x23), .b(x15), .c(new_n46_), .d(x06), .O(z07));
  inv1   g034(.a(x15), .O(new_n80_));
  nand2  g035(.a(new_n51_), .b(new_n80_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(x23), .O(new_n82_));
  inv1   g037(.a(x17), .O(new_n83_));
  nand3  g038(.a(x24), .b(x22), .c(x21), .O(new_n84_));
  nor2   g039(.a(new_n84_), .b(new_n58_), .O(new_n85_));
  nor2   g040(.a(new_n54_), .b(x23), .O(new_n86_));
  oai21  g041(.a(new_n86_), .b(new_n85_), .c(new_n83_), .O(new_n87_));
  inv1   g042(.a(new_n84_), .O(new_n88_));
  oai21  g043(.a(new_n86_), .b(new_n88_), .c(x20), .O(new_n89_));
  inv1   g044(.a(x22), .O(new_n90_));
  inv1   g045(.a(x21), .O(new_n91_));
  nand3  g046(.a(new_n91_), .b(x19), .c(x18), .O(new_n92_));
  inv1   g047(.a(new_n92_), .O(new_n93_));
  nand3  g048(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g049(.a(new_n94_), .O(new_n95_));
  nor2   g050(.a(x21), .b(x20), .O(new_n96_));
  nand4  g051(.a(new_n96_), .b(new_n95_), .c(new_n65_), .d(new_n90_), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n93_), .c(new_n65_), .d(new_n90_), .O(new_n98_));
  nand3  g053(.a(new_n98_), .b(x25), .c(new_n53_), .O(new_n99_));
  nand4  g054(.a(new_n99_), .b(new_n89_), .c(new_n87_), .d(new_n82_), .O(z08));
  inv1   g055(.a(x07), .O(new_n101_));
  nand4  g056(.a(new_n80_), .b(new_n101_), .c(x05), .d(x04), .O(new_n102_));
  inv1   g057(.a(new_n102_), .O(z09));
  nand2  g058(.a(x05), .b(x04), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n83_), .c(new_n80_), .d(new_n101_), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(new_n47_), .O(z10));
  xor2a  g061(.a(x18), .b(x17), .O(new_n107_));
  nand4  g062(.a(new_n107_), .b(new_n104_), .c(new_n80_), .d(new_n101_), .O(new_n108_));
  inv1   g063(.a(new_n108_), .O(z11));
  nor2   g064(.a(new_n56_), .b(new_n83_), .O(new_n110_));
  nand3  g065(.a(new_n57_), .b(x18), .c(x17), .O(new_n111_));
  oai21  g066(.a(new_n110_), .b(new_n57_), .c(new_n111_), .O(new_n112_));
  nand4  g067(.a(new_n112_), .b(new_n104_), .c(new_n80_), .d(new_n101_), .O(new_n113_));
  nand2  g068(.a(new_n113_), .b(new_n47_), .O(z12));
  nand3  g069(.a(new_n101_), .b(x05), .c(x04), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n101_), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(new_n80_), .O(new_n117_));
  nand2  g072(.a(new_n94_), .b(x20), .O(new_n118_));
  nor2   g073(.a(x20), .b(new_n57_), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n110_), .O(new_n120_));
  nand4  g075(.a(new_n120_), .b(new_n118_), .c(new_n117_), .d(new_n80_), .O(z13));
  nand2  g076(.a(new_n120_), .b(x21), .O(new_n122_));
  nand3  g077(.a(new_n110_), .b(new_n96_), .c(x19), .O(new_n123_));
  nand4  g078(.a(new_n123_), .b(new_n122_), .c(new_n117_), .d(new_n80_), .O(z14));
  nor2   g079(.a(x22), .b(x21), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n95_), .c(new_n55_), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n115_), .c(new_n101_), .O(new_n127_));
  aoi21  g082(.a(new_n123_), .b(x22), .c(new_n127_), .O(new_n128_));
  nand2  g083(.a(new_n53_), .b(x15), .O(new_n129_));
  oai21  g084(.a(new_n128_), .b(x15), .c(new_n129_), .O(z15));
  nand2  g085(.a(new_n126_), .b(x23), .O(new_n131_));
  nand4  g086(.a(new_n96_), .b(new_n95_), .c(new_n53_), .d(new_n90_), .O(new_n132_));
  nand4  g087(.a(new_n132_), .b(new_n131_), .c(new_n117_), .d(new_n80_), .O(z16));
  nand4  g088(.a(new_n65_), .b(new_n90_), .c(new_n91_), .d(new_n55_), .O(new_n134_));
  inv1   g089(.a(new_n134_), .O(new_n135_));
  nand4  g090(.a(new_n135_), .b(x19), .c(x18), .d(x17), .O(new_n136_));
  aoi21  g091(.a(new_n126_), .b(x24), .c(x15), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g093(.a(new_n138_), .b(new_n53_), .O(new_n139_));
  nand2  g094(.a(x24), .b(x23), .O(new_n140_));
  nand3  g095(.a(new_n140_), .b(new_n115_), .c(new_n101_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n80_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(new_n139_), .O(z17));
  nand2  g098(.a(x25), .b(x23), .O(new_n144_));
  nand3  g099(.a(new_n144_), .b(new_n115_), .c(new_n101_), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n80_), .O(new_n146_));
  nand2  g101(.a(new_n97_), .b(x25), .O(new_n147_));
  nand4  g102(.a(new_n125_), .b(new_n119_), .c(new_n110_), .d(new_n51_), .O(new_n148_));
  nand3  g103(.a(new_n148_), .b(new_n147_), .c(new_n80_), .O(new_n149_));
  nand2  g104(.a(new_n149_), .b(new_n53_), .O(new_n150_));
  nand2  g105(.a(new_n150_), .b(new_n146_), .O(z18));
endmodule


