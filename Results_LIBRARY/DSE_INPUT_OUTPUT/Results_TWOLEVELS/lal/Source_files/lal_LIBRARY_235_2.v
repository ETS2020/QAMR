// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n111_, new_n112_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  inv1   g002(.a(x09), .O(new_n48_));
  nor2   g003(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g006(.a(z02), .b(new_n46_), .O(z00));
  inv1   g007(.a(x15), .O(new_n53_));
  oai21  g008(.a(x09), .b(x07), .c(new_n53_), .O(new_n54_));
  nand3  g009(.a(new_n54_), .b(x05), .c(x04), .O(new_n55_));
  inv1   g010(.a(x25), .O(new_n56_));
  inv1   g011(.a(x20), .O(new_n57_));
  inv1   g012(.a(x17), .O(new_n58_));
  inv1   g013(.a(x18), .O(new_n59_));
  inv1   g014(.a(x19), .O(new_n60_));
  nand3  g015(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g016(.a(x22), .b(x21), .O(new_n62_));
  aoi21  g017(.a(new_n61_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  oai21  g018(.a(new_n63_), .b(x23), .c(x24), .O(new_n64_));
  aoi21  g019(.a(new_n64_), .b(new_n56_), .c(x07), .O(new_n65_));
  oai21  g020(.a(new_n65_), .b(new_n49_), .c(new_n55_), .O(z01));
  nand3  g021(.a(new_n64_), .b(new_n50_), .c(new_n56_), .O(new_n67_));
  nand2  g022(.a(new_n67_), .b(new_n50_), .O(z03));
  inv1   g023(.a(x10), .O(new_n69_));
  inv1   g024(.a(x02), .O(new_n70_));
  aoi22  g025(.a(x11), .b(new_n70_), .c(new_n69_), .d(x01), .O(new_n71_));
  oai21  g026(.a(new_n69_), .b(x01), .c(new_n71_), .O(new_n72_));
  inv1   g027(.a(x11), .O(new_n73_));
  nand2  g028(.a(new_n73_), .b(x02), .O(new_n74_));
  inv1   g029(.a(x03), .O(new_n75_));
  nand2  g030(.a(x12), .b(new_n75_), .O(new_n76_));
  inv1   g031(.a(x12), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(x03), .O(new_n78_));
  nand3  g033(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  oai21  g034(.a(new_n79_), .b(new_n72_), .c(new_n50_), .O(new_n80_));
  nor3   g035(.a(new_n53_), .b(new_n48_), .c(x00), .O(new_n81_));
  aoi21  g036(.a(new_n48_), .b(x00), .c(new_n81_), .O(new_n82_));
  aoi21  g037(.a(new_n82_), .b(new_n80_), .c(x08), .O(z04));
  oai21  g038(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g039(.a(x14), .O(new_n85_));
  nor3   g040(.a(new_n49_), .b(new_n85_), .c(x08), .O(z06));
  nand3  g041(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g042(.a(new_n60_), .b(new_n59_), .O(new_n88_));
  nand3  g043(.a(x24), .b(x22), .c(x21), .O(new_n89_));
  oai21  g044(.a(new_n89_), .b(new_n88_), .c(new_n56_), .O(new_n90_));
  nand2  g045(.a(new_n90_), .b(new_n58_), .O(new_n91_));
  nand2  g046(.a(new_n89_), .b(new_n56_), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(x20), .O(new_n93_));
  inv1   g048(.a(x24), .O(new_n94_));
  nand2  g049(.a(new_n56_), .b(new_n94_), .O(new_n95_));
  nand2  g050(.a(new_n95_), .b(x23), .O(new_n96_));
  inv1   g051(.a(x22), .O(new_n97_));
  inv1   g052(.a(x21), .O(new_n98_));
  nand3  g053(.a(new_n98_), .b(x19), .c(x18), .O(new_n99_));
  inv1   g054(.a(new_n99_), .O(new_n100_));
  nor2   g055(.a(new_n59_), .b(new_n58_), .O(new_n101_));
  nor2   g056(.a(x20), .b(new_n60_), .O(new_n102_));
  nor2   g057(.a(x22), .b(x21), .O(new_n103_));
  nor2   g058(.a(x24), .b(x23), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n100_), .c(new_n94_), .d(new_n97_), .O(new_n106_));
  aoi21  g061(.a(new_n106_), .b(x25), .c(new_n49_), .O(new_n107_));
  nand4  g062(.a(new_n107_), .b(new_n96_), .c(new_n93_), .d(new_n91_), .O(z08));
  nand2  g063(.a(x05), .b(x04), .O(new_n109_));
  nor4   g064(.a(new_n109_), .b(x15), .c(x09), .d(x07), .O(z09));
  inv1   g065(.a(x07), .O(new_n111_));
  nand3  g066(.a(new_n109_), .b(new_n58_), .c(new_n111_), .O(new_n112_));
  aoi21  g067(.a(new_n112_), .b(new_n48_), .c(x15), .O(z10));
  xor2a  g068(.a(x18), .b(x17), .O(new_n114_));
  nand3  g069(.a(new_n114_), .b(new_n109_), .c(new_n111_), .O(new_n115_));
  aoi21  g070(.a(new_n115_), .b(new_n48_), .c(x15), .O(z11));
  nand3  g071(.a(new_n60_), .b(x18), .c(x17), .O(new_n117_));
  oai21  g072(.a(new_n101_), .b(new_n60_), .c(new_n117_), .O(new_n118_));
  nand4  g073(.a(new_n118_), .b(new_n109_), .c(new_n53_), .d(new_n48_), .O(new_n119_));
  nor2   g074(.a(new_n119_), .b(x07), .O(z12));
  oai21  g075(.a(new_n109_), .b(x15), .c(new_n111_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n48_), .O(new_n122_));
  nand3  g077(.a(x19), .b(x18), .c(x17), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(x20), .O(new_n124_));
  nand2  g079(.a(new_n102_), .b(new_n101_), .O(new_n125_));
  inv1   g080(.a(new_n125_), .O(new_n126_));
  nor2   g081(.a(new_n126_), .b(new_n49_), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n124_), .c(new_n122_), .d(new_n53_), .O(z13));
  nor2   g083(.a(x21), .b(x20), .O(new_n129_));
  nand3  g084(.a(new_n129_), .b(new_n101_), .c(x19), .O(new_n130_));
  nand3  g085(.a(new_n130_), .b(new_n53_), .c(new_n48_), .O(new_n131_));
  aoi21  g086(.a(new_n125_), .b(x21), .c(new_n131_), .O(new_n132_));
  nand2  g087(.a(new_n132_), .b(new_n122_), .O(z14));
  nand2  g088(.a(new_n130_), .b(x22), .O(new_n134_));
  inv1   g089(.a(new_n123_), .O(new_n135_));
  nand3  g090(.a(new_n135_), .b(new_n103_), .c(new_n57_), .O(new_n136_));
  and2   g091(.a(new_n136_), .b(new_n50_), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(new_n134_), .c(new_n122_), .d(new_n53_), .O(z15));
  nor2   g093(.a(x23), .b(x22), .O(new_n139_));
  nand3  g094(.a(new_n139_), .b(new_n129_), .c(new_n135_), .O(new_n140_));
  nand3  g095(.a(new_n140_), .b(new_n53_), .c(new_n48_), .O(new_n141_));
  aoi21  g096(.a(new_n136_), .b(x23), .c(new_n141_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(new_n122_), .O(z16));
  nand3  g098(.a(new_n104_), .b(new_n97_), .c(new_n98_), .O(new_n144_));
  nor2   g099(.a(new_n144_), .b(x20), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(x19), .c(x18), .d(x17), .O(new_n146_));
  nand2  g101(.a(new_n53_), .b(new_n48_), .O(new_n147_));
  aoi21  g102(.a(new_n140_), .b(x24), .c(new_n147_), .O(new_n148_));
  nand3  g103(.a(new_n148_), .b(new_n146_), .c(new_n122_), .O(z17));
  nand2  g104(.a(new_n105_), .b(x25), .O(new_n150_));
  inv1   g105(.a(new_n103_), .O(new_n151_));
  nor3   g106(.a(new_n151_), .b(new_n95_), .c(x23), .O(new_n152_));
  aoi21  g107(.a(new_n152_), .b(new_n126_), .c(new_n49_), .O(new_n153_));
  nand4  g108(.a(new_n153_), .b(new_n150_), .c(new_n122_), .d(new_n53_), .O(z18));
endmodule


