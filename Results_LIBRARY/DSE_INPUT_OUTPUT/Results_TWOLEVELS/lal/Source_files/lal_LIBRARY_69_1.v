// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x21), .O(new_n53_));
  inv1   g08(.a(x22), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n55_), .c(x24), .O(new_n62_));
  inv1   g17(.a(x15), .O(new_n63_));
  nor2   g18(.a(x24), .b(new_n63_), .O(new_n64_));
  aoi21  g19(.a(new_n62_), .b(new_n48_), .c(new_n64_), .O(new_n65_));
  nand2  g20(.a(x05), .b(x04), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  nor3   g22(.a(new_n67_), .b(new_n49_), .c(x07), .O(new_n68_));
  oai21  g23(.a(new_n65_), .b(x25), .c(new_n68_), .O(z01));
  nor2   g24(.a(new_n65_), .b(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n50_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z06));
  nand3  g35(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x24), .O(new_n82_));
  aoi21  g37(.a(new_n60_), .b(new_n56_), .c(new_n54_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x21), .c(x23), .O(new_n84_));
  nor2   g39(.a(new_n49_), .b(x25), .O(new_n85_));
  oai21  g40(.a(new_n84_), .b(new_n82_), .c(new_n85_), .O(z08));
  oai21  g41(.a(new_n66_), .b(x07), .c(new_n48_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n63_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand4  g44(.a(new_n66_), .b(new_n48_), .c(new_n57_), .d(new_n63_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x07), .O(z10));
  inv1   g46(.a(x07), .O(new_n92_));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n66_), .c(new_n92_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n48_), .c(x15), .O(z11));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n59_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n59_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n66_), .c(new_n48_), .d(new_n63_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(x07), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nor2   g58(.a(x20), .b(new_n59_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n97_), .c(x15), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n103_), .c(new_n88_), .d(new_n92_), .O(z13));
  aoi21  g61(.a(new_n104_), .b(new_n97_), .c(new_n53_), .O(new_n107_));
  nand3  g62(.a(new_n67_), .b(new_n63_), .c(new_n92_), .O(new_n108_));
  nand4  g63(.a(new_n97_), .b(new_n53_), .c(new_n56_), .d(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n92_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n107_), .c(new_n48_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n63_), .O(z14));
  nand3  g67(.a(new_n53_), .b(new_n56_), .c(x19), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n96_), .c(x22), .O(new_n114_));
  inv1   g69(.a(new_n102_), .O(new_n115_));
  nor2   g70(.a(x22), .b(x21), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n56_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n114_), .c(new_n108_), .d(new_n92_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n63_), .O(z15));
  nand3  g75(.a(new_n117_), .b(new_n108_), .c(new_n92_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n48_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n63_), .O(z16));
  nand2  g78(.a(new_n117_), .b(x24), .O(new_n124_));
  nand4  g79(.a(new_n82_), .b(new_n54_), .c(new_n53_), .d(new_n56_), .O(new_n125_));
  inv1   g80(.a(new_n125_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n115_), .c(x15), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n124_), .c(new_n88_), .d(new_n92_), .O(z17));
  oai21  g83(.a(new_n125_), .b(new_n102_), .c(x25), .O(new_n129_));
  nor2   g84(.a(x25), .b(x24), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n116_), .c(new_n104_), .d(new_n97_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n108_), .d(new_n92_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n48_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n63_), .O(z18));
endmodule


