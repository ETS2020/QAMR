// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x24), .O(new_n48_));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  inv1   g10(.a(x21), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n55_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n48_), .c(new_n49_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  inv1   g17(.a(x15), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x08), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n62_), .c(x07), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n60_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n64_), .b(new_n67_), .O(z02));
  inv1   g23(.a(new_n64_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n60_), .O(z03));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  xor2a  g26(.a(x10), .b(x01), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  inv1   g29(.a(x12), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(x03), .c(x15), .O(new_n76_));
  oai21  g31(.a(new_n75_), .b(x03), .c(new_n76_), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n73_), .c(x08), .O(z04));
  nor3   g34(.a(x15), .b(x13), .c(x08), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  aoi21  g36(.a(new_n63_), .b(new_n81_), .c(x08), .O(z06));
  oai21  g37(.a(x15), .b(x06), .c(new_n46_), .O(z07));
  aoi21  g38(.a(new_n54_), .b(new_n50_), .c(new_n57_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(x21), .c(x23), .O(new_n85_));
  nor2   g40(.a(new_n64_), .b(x25), .O(new_n86_));
  oai21  g41(.a(new_n85_), .b(new_n48_), .c(new_n86_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n62_), .b(new_n63_), .c(new_n88_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n69_), .O(z09));
  nand4  g45(.a(new_n61_), .b(new_n51_), .c(new_n63_), .d(new_n88_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n69_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n61_), .c(new_n63_), .d(new_n88_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n69_), .O(z11));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n53_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n53_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n61_), .c(new_n63_), .d(new_n88_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n69_), .O(z12));
  nand3  g56(.a(new_n88_), .b(x05), .c(x04), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n88_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n63_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nand4  g61(.a(new_n50_), .b(x19), .c(x18), .d(x17), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n104_), .d(new_n63_), .O(z13));
  nand2  g63(.a(new_n107_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n97_), .c(x19), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n104_), .d(new_n63_), .O(z14));
  nand3  g67(.a(new_n56_), .b(new_n50_), .c(x19), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n96_), .c(x22), .O(new_n114_));
  inv1   g69(.a(new_n105_), .O(new_n115_));
  nor3   g70(.a(x22), .b(x21), .c(x20), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n114_), .c(new_n102_), .d(new_n88_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n63_), .O(new_n119_));
  nand2  g74(.a(x15), .b(x08), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(z15));
  inv1   g76(.a(x23), .O(new_n122_));
  aoi21  g77(.a(new_n116_), .b(new_n115_), .c(new_n122_), .O(new_n123_));
  nand4  g78(.a(new_n110_), .b(new_n115_), .c(new_n122_), .d(new_n57_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n102_), .c(new_n88_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n123_), .c(new_n63_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n120_), .O(z16));
  nand2  g82(.a(new_n124_), .b(x24), .O(new_n128_));
  nand4  g83(.a(new_n48_), .b(new_n122_), .c(new_n57_), .d(new_n56_), .O(new_n129_));
  or2    g84(.a(new_n129_), .b(new_n107_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n128_), .c(new_n104_), .d(new_n63_), .O(z17));
  oai21  g86(.a(new_n129_), .b(new_n107_), .c(x25), .O(new_n132_));
  nor2   g87(.a(x20), .b(new_n53_), .O(new_n133_));
  nor2   g88(.a(x22), .b(x21), .O(new_n134_));
  nor3   g89(.a(x25), .b(x24), .c(x23), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n134_), .c(new_n133_), .d(new_n97_), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n132_), .c(new_n102_), .d(new_n88_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n63_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n120_), .O(z18));
endmodule


