// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x20), .b(x15), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n47_), .c(x07), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n47_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n62_), .b(new_n48_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n48_), .O(z04));
  nor3   g31(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n48_), .O(z06));
  nand3  g34(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g35(.a(new_n56_), .b(new_n52_), .c(new_n59_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x21), .c(x23), .O(new_n82_));
  nor2   g37(.a(new_n47_), .b(x25), .O(new_n83_));
  oai21  g38(.a(new_n82_), .b(new_n50_), .c(new_n83_), .O(z08));
  nor4   g39(.a(new_n63_), .b(new_n52_), .c(x15), .d(x07), .O(z09));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n63_), .b(x20), .c(new_n53_), .d(new_n86_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x20), .c(x15), .O(z10));
  nand3  g43(.a(x20), .b(new_n54_), .c(x17), .O(new_n89_));
  oai21  g44(.a(new_n54_), .b(x17), .c(new_n89_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n63_), .c(new_n86_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(x20), .c(x15), .O(z11));
  inv1   g47(.a(x15), .O(new_n93_));
  oai21  g48(.a(new_n54_), .b(new_n53_), .c(x19), .O(new_n94_));
  nand3  g49(.a(new_n55_), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n63_), .c(x20), .d(new_n93_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(x07), .O(z12));
  nand4  g53(.a(new_n63_), .b(x19), .c(x18), .d(x17), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n93_), .c(x07), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n52_), .c(new_n93_), .O(z13));
  oai21  g56(.a(new_n63_), .b(x07), .c(x20), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n93_), .O(new_n103_));
  nand2  g58(.a(x20), .b(x07), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n103_), .c(new_n58_), .d(new_n93_), .O(z14));
  nand3  g60(.a(new_n64_), .b(new_n93_), .c(new_n86_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n59_), .c(new_n86_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n93_), .O(z15));
  nor2   g64(.a(x23), .b(x15), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n104_), .c(new_n103_), .O(z16));
  nand4  g66(.a(new_n104_), .b(new_n103_), .c(new_n50_), .d(new_n93_), .O(z17));
  nand4  g67(.a(new_n104_), .b(new_n103_), .c(new_n51_), .d(new_n93_), .O(z18));
endmodule


