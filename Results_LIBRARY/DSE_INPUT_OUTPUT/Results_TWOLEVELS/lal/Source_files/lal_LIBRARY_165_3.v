// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n112_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_;
  inv1   g000(.a(x16), .O(new_n46_));
  nand2  g001(.a(x15), .b(x07), .O(new_n47_));
  nand3  g002(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(z00));
  inv1   g004(.a(x25), .O(new_n50_));
  inv1   g005(.a(x20), .O(new_n51_));
  inv1   g006(.a(x17), .O(new_n52_));
  nor2   g007(.a(x19), .b(x18), .O(new_n53_));
  nand2  g008(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g009(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n55_), .O(new_n56_));
  oai21  g011(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g012(.a(x05), .b(x04), .O(new_n58_));
  inv1   g013(.a(new_n58_), .O(new_n59_));
  aoi21  g014(.a(new_n57_), .b(new_n50_), .c(new_n59_), .O(new_n60_));
  inv1   g015(.a(x15), .O(new_n61_));
  nand2  g016(.a(new_n61_), .b(x07), .O(new_n62_));
  oai21  g017(.a(new_n60_), .b(x07), .c(new_n62_), .O(z01));
  nand2  g018(.a(new_n47_), .b(new_n46_), .O(z02));
  nand3  g019(.a(new_n57_), .b(new_n47_), .c(new_n50_), .O(new_n65_));
  inv1   g020(.a(new_n65_), .O(z03));
  inv1   g021(.a(x08), .O(new_n67_));
  xnor2a g022(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g023(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g024(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g025(.a(x12), .b(x03), .O(new_n71_));
  nand4  g026(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand3  g027(.a(new_n72_), .b(new_n47_), .c(new_n67_), .O(new_n73_));
  inv1   g028(.a(new_n73_), .O(z04));
  inv1   g029(.a(x13), .O(new_n75_));
  nand3  g030(.a(new_n47_), .b(new_n75_), .c(new_n67_), .O(new_n76_));
  inv1   g031(.a(new_n76_), .O(z05));
  nand3  g032(.a(new_n47_), .b(x14), .c(new_n67_), .O(new_n78_));
  inv1   g033(.a(new_n78_), .O(z06));
  nand3  g034(.a(new_n47_), .b(new_n67_), .c(x06), .O(z07));
  inv1   g035(.a(x07), .O(new_n81_));
  inv1   g036(.a(new_n53_), .O(new_n82_));
  nand3  g037(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  oai21  g038(.a(new_n83_), .b(new_n82_), .c(new_n50_), .O(new_n84_));
  nand2  g039(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  nand2  g040(.a(new_n83_), .b(new_n50_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(x20), .O(new_n87_));
  oai21  g042(.a(x25), .b(x24), .c(x23), .O(new_n88_));
  inv1   g043(.a(x22), .O(new_n89_));
  inv1   g044(.a(x24), .O(new_n90_));
  inv1   g045(.a(x21), .O(new_n91_));
  nand3  g046(.a(new_n91_), .b(x19), .c(x18), .O(new_n92_));
  inv1   g047(.a(new_n92_), .O(new_n93_));
  and2   g048(.a(x18), .b(x17), .O(new_n94_));
  inv1   g049(.a(x19), .O(new_n95_));
  nor2   g050(.a(x20), .b(new_n95_), .O(new_n96_));
  nor2   g051(.a(x22), .b(x21), .O(new_n97_));
  nor2   g052(.a(x24), .b(x23), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(new_n94_), .O(new_n99_));
  nand4  g054(.a(new_n99_), .b(new_n93_), .c(new_n90_), .d(new_n89_), .O(new_n100_));
  nand2  g055(.a(new_n100_), .b(x25), .O(new_n101_));
  nand4  g056(.a(new_n101_), .b(new_n88_), .c(new_n87_), .d(new_n85_), .O(new_n102_));
  nand2  g057(.a(new_n102_), .b(new_n81_), .O(new_n103_));
  inv1   g058(.a(x23), .O(new_n104_));
  nand2  g059(.a(new_n54_), .b(new_n51_), .O(new_n105_));
  nand3  g060(.a(new_n105_), .b(x22), .c(x21), .O(new_n106_));
  aoi21  g061(.a(new_n106_), .b(new_n104_), .c(new_n90_), .O(new_n107_));
  oai21  g062(.a(new_n107_), .b(x25), .c(new_n61_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n103_), .O(z08));
  nand3  g064(.a(new_n59_), .b(new_n61_), .c(new_n81_), .O(new_n110_));
  inv1   g065(.a(new_n110_), .O(z09));
  nand4  g066(.a(new_n58_), .b(new_n52_), .c(new_n61_), .d(new_n81_), .O(new_n112_));
  inv1   g067(.a(new_n112_), .O(z10));
  xor2a  g068(.a(x18), .b(x17), .O(new_n114_));
  nand4  g069(.a(new_n114_), .b(new_n58_), .c(new_n61_), .d(new_n81_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n47_), .O(z11));
  nand3  g071(.a(new_n95_), .b(x18), .c(x17), .O(new_n117_));
  oai21  g072(.a(new_n94_), .b(new_n95_), .c(new_n117_), .O(new_n118_));
  nand4  g073(.a(new_n118_), .b(new_n58_), .c(new_n61_), .d(new_n81_), .O(new_n119_));
  inv1   g074(.a(new_n119_), .O(z12));
  nand2  g075(.a(new_n96_), .b(new_n94_), .O(new_n121_));
  nand2  g076(.a(new_n58_), .b(new_n61_), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(new_n81_), .O(new_n123_));
  nand3  g078(.a(x19), .b(x18), .c(x17), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(x20), .O(new_n125_));
  nand4  g080(.a(new_n125_), .b(new_n123_), .c(new_n121_), .d(new_n81_), .O(z13));
  nor2   g081(.a(x21), .b(x20), .O(new_n127_));
  nand3  g082(.a(new_n127_), .b(new_n94_), .c(x19), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n81_), .O(new_n129_));
  aoi21  g084(.a(new_n121_), .b(x21), .c(new_n129_), .O(new_n130_));
  oai21  g085(.a(new_n130_), .b(x15), .c(new_n123_), .O(z14));
  nand2  g086(.a(new_n97_), .b(new_n51_), .O(new_n132_));
  nor2   g087(.a(new_n132_), .b(new_n124_), .O(new_n133_));
  aoi21  g088(.a(new_n128_), .b(x22), .c(new_n133_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n123_), .c(new_n81_), .O(z15));
  nor2   g090(.a(new_n133_), .b(new_n104_), .O(new_n136_));
  nand3  g091(.a(new_n127_), .b(new_n104_), .c(new_n89_), .O(new_n137_));
  oai21  g092(.a(new_n137_), .b(new_n124_), .c(new_n81_), .O(new_n138_));
  oai21  g093(.a(new_n138_), .b(new_n136_), .c(new_n61_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n123_), .O(z16));
  nand2  g095(.a(x25), .b(new_n81_), .O(new_n141_));
  nand2  g096(.a(new_n50_), .b(new_n61_), .O(new_n142_));
  aoi21  g097(.a(new_n142_), .b(new_n141_), .c(x24), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n104_), .c(new_n89_), .d(new_n91_), .O(new_n144_));
  nor2   g099(.a(new_n144_), .b(x20), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(x19), .c(x18), .d(x17), .O(new_n146_));
  oai21  g101(.a(new_n137_), .b(new_n124_), .c(x24), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n81_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n61_), .O(new_n149_));
  nand3  g104(.a(new_n149_), .b(new_n146_), .c(new_n123_), .O(z17));
  nand4  g105(.a(new_n97_), .b(new_n50_), .c(new_n90_), .d(new_n104_), .O(new_n151_));
  oai21  g106(.a(new_n151_), .b(new_n121_), .c(new_n81_), .O(new_n152_));
  nand2  g107(.a(new_n152_), .b(new_n61_), .O(new_n153_));
  aoi21  g108(.a(new_n99_), .b(x25), .c(new_n122_), .O(new_n154_));
  oai21  g109(.a(new_n154_), .b(x07), .c(new_n153_), .O(z18));
endmodule


