// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x03), .O(new_n47_));
  inv1   g002(.a(x15), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  oai21  g005(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g006(.a(x07), .O(new_n52_));
  inv1   g007(.a(x24), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x20), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(x17), .c(new_n55_), .O(new_n59_));
  inv1   g014(.a(x21), .O(new_n60_));
  inv1   g015(.a(x22), .O(new_n61_));
  nor2   g016(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g017(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  oai21  g018(.a(new_n63_), .b(new_n53_), .c(new_n54_), .O(new_n64_));
  nand2  g019(.a(x05), .b(x04), .O(new_n65_));
  nand4  g020(.a(new_n65_), .b(new_n64_), .c(new_n50_), .d(new_n52_), .O(z01));
  inv1   g021(.a(x16), .O(new_n67_));
  nor2   g022(.a(new_n49_), .b(new_n67_), .O(z02));
  nand2  g023(.a(new_n64_), .b(new_n50_), .O(z03));
  nor2   g024(.a(x12), .b(x08), .O(new_n70_));
  oai21  g025(.a(new_n70_), .b(x15), .c(x03), .O(new_n71_));
  inv1   g026(.a(x09), .O(new_n72_));
  inv1   g027(.a(x01), .O(new_n73_));
  aoi22  g028(.a(x10), .b(new_n73_), .c(new_n72_), .d(x00), .O(new_n74_));
  oai21  g029(.a(new_n72_), .b(x00), .c(new_n74_), .O(new_n75_));
  inv1   g030(.a(x10), .O(new_n76_));
  nand2  g031(.a(new_n76_), .b(x01), .O(new_n77_));
  inv1   g032(.a(x02), .O(new_n78_));
  nand2  g033(.a(x11), .b(new_n78_), .O(new_n79_));
  inv1   g034(.a(x11), .O(new_n80_));
  nand2  g035(.a(new_n80_), .b(x02), .O(new_n81_));
  nand2  g036(.a(x12), .b(new_n47_), .O(new_n82_));
  nand4  g037(.a(new_n82_), .b(new_n81_), .c(new_n79_), .d(new_n77_), .O(new_n83_));
  oai21  g038(.a(new_n83_), .b(new_n75_), .c(new_n46_), .O(new_n84_));
  nand2  g039(.a(new_n84_), .b(new_n71_), .O(z04));
  oai21  g040(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g041(.a(new_n50_), .b(x14), .c(new_n46_), .O(new_n87_));
  inv1   g042(.a(new_n87_), .O(z06));
  nand3  g043(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g044(.a(x17), .O(new_n90_));
  nand3  g045(.a(x24), .b(x22), .c(x21), .O(new_n91_));
  oai21  g046(.a(new_n91_), .b(new_n58_), .c(new_n54_), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g048(.a(new_n91_), .b(new_n54_), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(x20), .O(new_n95_));
  oai21  g050(.a(x25), .b(x24), .c(x23), .O(new_n96_));
  nand3  g051(.a(new_n60_), .b(x19), .c(x18), .O(new_n97_));
  inv1   g052(.a(new_n97_), .O(new_n98_));
  and2   g053(.a(x18), .b(x17), .O(new_n99_));
  nor2   g054(.a(x20), .b(new_n57_), .O(new_n100_));
  nor2   g055(.a(x22), .b(x21), .O(new_n101_));
  nor2   g056(.a(x24), .b(x23), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n101_), .c(new_n100_), .d(new_n99_), .O(new_n103_));
  nand4  g058(.a(new_n103_), .b(new_n98_), .c(new_n53_), .d(new_n61_), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(x25), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n96_), .c(new_n95_), .d(new_n93_), .O(new_n106_));
  nand2  g061(.a(new_n106_), .b(new_n48_), .O(new_n107_));
  inv1   g062(.a(x23), .O(new_n108_));
  nand3  g063(.a(new_n59_), .b(x22), .c(x21), .O(new_n109_));
  aoi21  g064(.a(new_n109_), .b(new_n108_), .c(new_n53_), .O(new_n110_));
  oai21  g065(.a(new_n110_), .b(x25), .c(new_n47_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n107_), .O(z08));
  nand4  g067(.a(new_n48_), .b(new_n52_), .c(x05), .d(x04), .O(new_n113_));
  inv1   g068(.a(new_n113_), .O(z09));
  nand4  g069(.a(new_n65_), .b(new_n90_), .c(new_n48_), .d(new_n52_), .O(new_n115_));
  inv1   g070(.a(new_n115_), .O(z10));
  xor2a  g071(.a(x18), .b(x17), .O(new_n117_));
  nand4  g072(.a(new_n117_), .b(new_n65_), .c(new_n48_), .d(new_n52_), .O(new_n118_));
  inv1   g073(.a(new_n118_), .O(z11));
  nand3  g074(.a(new_n57_), .b(x18), .c(x17), .O(new_n120_));
  oai21  g075(.a(new_n99_), .b(new_n57_), .c(new_n120_), .O(new_n121_));
  nand4  g076(.a(new_n121_), .b(new_n65_), .c(new_n48_), .d(new_n52_), .O(new_n122_));
  inv1   g077(.a(new_n122_), .O(z12));
  nand2  g078(.a(new_n100_), .b(new_n99_), .O(new_n124_));
  nand3  g079(.a(new_n52_), .b(x05), .c(x04), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(new_n52_), .O(new_n126_));
  inv1   g081(.a(new_n126_), .O(new_n127_));
  nand3  g082(.a(x19), .b(x18), .c(x17), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(x20), .O(new_n129_));
  nand4  g084(.a(new_n129_), .b(new_n127_), .c(new_n124_), .d(new_n48_), .O(z13));
  inv1   g085(.a(new_n124_), .O(new_n131_));
  nor2   g086(.a(new_n131_), .b(new_n60_), .O(new_n132_));
  nor2   g087(.a(x21), .b(x20), .O(new_n133_));
  nand3  g088(.a(new_n133_), .b(new_n99_), .c(x19), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n125_), .c(new_n52_), .O(new_n135_));
  oai21  g090(.a(new_n135_), .b(new_n132_), .c(new_n48_), .O(new_n136_));
  nand2  g091(.a(x15), .b(new_n47_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n136_), .O(z14));
  inv1   g093(.a(new_n101_), .O(new_n139_));
  nor3   g094(.a(new_n128_), .b(new_n139_), .c(x20), .O(new_n140_));
  aoi21  g095(.a(new_n134_), .b(x22), .c(new_n140_), .O(new_n141_));
  nand3  g096(.a(new_n141_), .b(new_n127_), .c(new_n48_), .O(z15));
  nor2   g097(.a(new_n140_), .b(new_n108_), .O(new_n143_));
  inv1   g098(.a(new_n128_), .O(new_n144_));
  nand4  g099(.a(new_n133_), .b(new_n144_), .c(new_n108_), .d(new_n61_), .O(new_n145_));
  nand3  g100(.a(new_n145_), .b(new_n125_), .c(new_n52_), .O(new_n146_));
  oai21  g101(.a(new_n146_), .b(new_n143_), .c(new_n48_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n137_), .O(z16));
  nand3  g103(.a(new_n102_), .b(new_n61_), .c(new_n60_), .O(new_n149_));
  nor2   g104(.a(new_n149_), .b(x20), .O(new_n150_));
  nand4  g105(.a(new_n150_), .b(x19), .c(x18), .d(x17), .O(new_n151_));
  aoi21  g106(.a(new_n145_), .b(x24), .c(new_n126_), .O(new_n152_));
  nand2  g107(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nand2  g108(.a(new_n153_), .b(new_n48_), .O(new_n154_));
  nand2  g109(.a(new_n154_), .b(new_n137_), .O(z17));
  nand2  g110(.a(new_n103_), .b(x25), .O(new_n156_));
  inv1   g111(.a(new_n125_), .O(new_n157_));
  nor4   g112(.a(new_n139_), .b(x25), .c(x24), .d(x23), .O(new_n158_));
  aoi21  g113(.a(new_n158_), .b(new_n131_), .c(new_n157_), .O(new_n159_));
  nand4  g114(.a(new_n159_), .b(new_n156_), .c(new_n48_), .d(new_n52_), .O(z18));
endmodule


