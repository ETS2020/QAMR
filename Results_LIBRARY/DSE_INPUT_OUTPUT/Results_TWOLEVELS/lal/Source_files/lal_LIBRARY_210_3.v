// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x15), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(x08), .O(new_n64_));
  nor2   g19(.a(new_n46_), .b(new_n64_), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(x07), .O(new_n66_));
  nand3  g21(.a(new_n66_), .b(new_n63_), .c(new_n62_), .O(z01));
  aoi21  g22(.a(x15), .b(x08), .c(new_n47_), .O(z02));
  inv1   g23(.a(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n62_), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n64_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n69_), .O(z04));
  nor2   g32(.a(x13), .b(x08), .O(z05));
  nand2  g33(.a(x14), .b(new_n64_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n69_), .O(z06));
  nand2  g35(.a(new_n46_), .b(x08), .O(new_n81_));
  oai21  g36(.a(x08), .b(x06), .c(new_n81_), .O(z07));
  aoi21  g37(.a(new_n56_), .b(new_n52_), .c(new_n59_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x21), .c(x23), .O(new_n84_));
  nor2   g39(.a(new_n65_), .b(x25), .O(new_n85_));
  oai21  g40(.a(new_n84_), .b(new_n50_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  nand2  g42(.a(new_n46_), .b(new_n87_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n63_), .c(new_n69_), .O(z09));
  nand4  g44(.a(new_n63_), .b(new_n53_), .c(new_n46_), .d(new_n87_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n69_), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n63_), .c(new_n46_), .d(new_n87_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n69_), .O(z11));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n55_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n55_), .c(new_n97_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n63_), .c(new_n46_), .d(new_n87_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n69_), .O(z12));
  nand3  g55(.a(new_n87_), .b(x05), .c(x04), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n87_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n46_), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nand4  g60(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .d(new_n46_), .O(z13));
  nand2  g62(.a(new_n106_), .b(x21), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n96_), .c(x19), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n108_), .c(new_n103_), .d(new_n46_), .O(z14));
  nand3  g66(.a(new_n58_), .b(new_n52_), .c(x19), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n95_), .c(x22), .O(new_n113_));
  inv1   g68(.a(new_n104_), .O(new_n114_));
  nor3   g69(.a(x22), .b(x21), .c(x20), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n113_), .c(new_n101_), .d(new_n87_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n46_), .O(new_n118_));
  nand2  g73(.a(x15), .b(new_n64_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(z15));
  inv1   g75(.a(x23), .O(new_n121_));
  aoi21  g76(.a(new_n115_), .b(new_n114_), .c(new_n121_), .O(new_n122_));
  nand4  g77(.a(new_n109_), .b(new_n114_), .c(new_n121_), .d(new_n59_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n101_), .c(new_n87_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n122_), .c(new_n46_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n119_), .O(z16));
  nand2  g81(.a(new_n123_), .b(x24), .O(new_n127_));
  nand4  g82(.a(new_n50_), .b(new_n121_), .c(new_n59_), .d(new_n58_), .O(new_n128_));
  or2    g83(.a(new_n128_), .b(new_n106_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n127_), .c(new_n103_), .d(new_n46_), .O(z17));
  oai21  g85(.a(new_n128_), .b(new_n106_), .c(x25), .O(new_n131_));
  nor2   g86(.a(x20), .b(new_n55_), .O(new_n132_));
  nor2   g87(.a(x22), .b(x21), .O(new_n133_));
  nor3   g88(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n96_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n131_), .c(new_n101_), .d(new_n87_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n46_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n119_), .O(z18));
endmodule


