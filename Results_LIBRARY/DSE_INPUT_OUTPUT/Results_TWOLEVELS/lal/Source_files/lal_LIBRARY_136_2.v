// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x01), .O(new_n47_));
  nor2   g02(.a(x15), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(new_n48_), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n51_), .c(new_n50_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor3   g17(.a(new_n62_), .b(new_n48_), .c(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n60_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n60_), .O(z03));
  oai21  g22(.a(x10), .b(x08), .c(x15), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(x01), .O(new_n69_));
  inv1   g24(.a(x09), .O(new_n70_));
  aoi22  g25(.a(x10), .b(new_n47_), .c(new_n70_), .d(x00), .O(new_n71_));
  oai21  g26(.a(new_n70_), .b(x00), .c(new_n71_), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(new_n72_), .c(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n69_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n51_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n51_), .O(z06));
  nand3  g35(.a(new_n51_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x24), .O(new_n82_));
  inv1   g37(.a(x22), .O(new_n83_));
  aoi21  g38(.a(new_n56_), .b(new_n52_), .c(new_n83_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(x21), .c(x23), .O(new_n85_));
  nor2   g40(.a(new_n48_), .b(x25), .O(new_n86_));
  oai21  g41(.a(new_n85_), .b(new_n82_), .c(new_n86_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(x05), .c(x04), .d(new_n47_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(x15), .O(z09));
  inv1   g45(.a(x15), .O(new_n91_));
  nand4  g46(.a(new_n61_), .b(new_n53_), .c(new_n91_), .d(new_n88_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(x01), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n61_), .c(new_n88_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n47_), .c(x15), .O(z11));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n55_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n55_), .c(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n61_), .c(new_n88_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n47_), .c(x15), .O(z12));
  nand3  g57(.a(new_n62_), .b(new_n91_), .c(new_n88_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n88_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(new_n105_));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x20), .O(new_n107_));
  nand4  g62(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nor2   g64(.a(new_n109_), .b(new_n48_), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n107_), .c(new_n105_), .d(new_n91_), .O(z13));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n98_), .c(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n91_), .c(new_n47_), .O(new_n114_));
  aoi21  g69(.a(new_n108_), .b(x21), .c(new_n114_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n105_), .O(z14));
  inv1   g71(.a(x21), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n52_), .c(x19), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n97_), .c(x22), .O(new_n119_));
  inv1   g74(.a(new_n106_), .O(new_n120_));
  nor3   g75(.a(x22), .b(x21), .c(x20), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n119_), .c(new_n103_), .d(new_n88_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n91_), .O(z15));
  inv1   g80(.a(x23), .O(new_n126_));
  aoi21  g81(.a(new_n121_), .b(new_n120_), .c(new_n126_), .O(new_n127_));
  nand4  g82(.a(new_n112_), .b(new_n120_), .c(new_n126_), .d(new_n83_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n103_), .c(new_n88_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n127_), .c(new_n47_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n91_), .O(z16));
  nand2  g86(.a(new_n128_), .b(x24), .O(new_n132_));
  nand4  g87(.a(new_n82_), .b(new_n126_), .c(new_n83_), .d(new_n117_), .O(new_n133_));
  inv1   g88(.a(new_n133_), .O(new_n134_));
  aoi21  g89(.a(new_n134_), .b(new_n109_), .c(new_n48_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n132_), .c(new_n105_), .d(new_n91_), .O(z17));
  oai21  g91(.a(new_n133_), .b(new_n108_), .c(x25), .O(new_n137_));
  nor2   g92(.a(x20), .b(new_n55_), .O(new_n138_));
  nor2   g93(.a(x22), .b(x21), .O(new_n139_));
  nor3   g94(.a(x25), .b(x24), .c(x23), .O(new_n140_));
  nand4  g95(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n98_), .O(new_n141_));
  nand4  g96(.a(new_n141_), .b(new_n137_), .c(new_n103_), .d(new_n88_), .O(new_n142_));
  nand2  g97(.a(new_n142_), .b(new_n47_), .O(new_n143_));
  nand2  g98(.a(new_n143_), .b(new_n91_), .O(z18));
endmodule


