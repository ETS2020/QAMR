// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x18), .b(x15), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x15), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n50_), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(x24), .c(x25), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x07), .c(new_n48_), .O(new_n58_));
  inv1   g13(.a(x20), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  nor2   g15(.a(x19), .b(x17), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n60_), .c(new_n54_), .d(new_n59_), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n58_), .c(new_n53_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n48_), .b(new_n65_), .O(z02));
  nand2  g21(.a(new_n56_), .b(x24), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n48_), .O(new_n68_));
  nand3  g23(.a(new_n62_), .b(new_n54_), .c(new_n59_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z04));
  nor3   g32(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor3   g34(.a(new_n47_), .b(new_n79_), .c(x08), .O(z06));
  nand3  g35(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g36(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n61_), .c(new_n50_), .O(new_n84_));
  oai21  g39(.a(new_n55_), .b(new_n59_), .c(new_n54_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x25), .O(new_n86_));
  oai21  g41(.a(new_n84_), .b(x18), .c(new_n86_), .O(z08));
  nand2  g42(.a(x05), .b(x04), .O(new_n88_));
  nor4   g43(.a(new_n88_), .b(new_n51_), .c(x15), .d(x07), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand3  g45(.a(new_n88_), .b(x18), .c(new_n90_), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(x15), .c(x07), .O(z10));
  inv1   g47(.a(x07), .O(new_n93_));
  inv1   g48(.a(new_n91_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(x18), .c(x15), .O(z11));
  xor2a  g51(.a(x19), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n88_), .c(new_n93_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(x18), .c(x15), .O(z12));
  oai21  g54(.a(new_n88_), .b(x07), .c(x18), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n50_), .O(new_n101_));
  nand2  g56(.a(x19), .b(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nor2   g58(.a(new_n51_), .b(new_n93_), .O(new_n104_));
  nand3  g59(.a(new_n59_), .b(x19), .c(x17), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nor2   g61(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n103_), .c(new_n101_), .d(new_n50_), .O(z13));
  nand4  g63(.a(new_n50_), .b(new_n93_), .c(x05), .d(x04), .O(new_n109_));
  inv1   g64(.a(new_n102_), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n109_), .c(new_n93_), .O(new_n113_));
  aoi21  g68(.a(new_n105_), .b(x21), .c(new_n113_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n51_), .c(new_n50_), .O(z14));
  nand2  g70(.a(new_n112_), .b(x22), .O(new_n116_));
  inv1   g71(.a(x21), .O(new_n117_));
  inv1   g72(.a(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n59_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(new_n102_), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(new_n104_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n116_), .c(new_n101_), .d(new_n50_), .O(z15));
  oai21  g77(.a(new_n119_), .b(new_n102_), .c(x23), .O(new_n123_));
  nor3   g78(.a(x23), .b(x22), .c(x21), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n106_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n123_), .c(new_n109_), .d(new_n93_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(x18), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n50_), .O(z16));
  nand2  g83(.a(new_n125_), .b(x24), .O(new_n129_));
  inv1   g84(.a(x24), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n54_), .c(new_n118_), .d(new_n117_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  aoi21  g87(.a(new_n132_), .b(new_n106_), .c(new_n104_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n129_), .c(new_n101_), .d(new_n50_), .O(z17));
  oai21  g89(.a(new_n131_), .b(new_n105_), .c(x25), .O(new_n135_));
  nor2   g90(.a(x23), .b(x22), .O(new_n136_));
  nor2   g91(.a(x25), .b(x24), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n136_), .c(new_n111_), .d(new_n110_), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n135_), .c(new_n109_), .d(new_n93_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(x18), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n50_), .O(z18));
endmodule


