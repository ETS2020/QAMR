// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x22), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x15), .O(new_n49_));
  inv1   g04(.a(x22), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(x24), .c(new_n49_), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n54_), .c(new_n55_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(x22), .c(new_n53_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n47_), .c(x07), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(x25), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n47_), .b(new_n67_), .O(z02));
  nor2   g23(.a(new_n62_), .b(x25), .O(z03));
  inv1   g24(.a(new_n47_), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n70_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  nor3   g32(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor3   g34(.a(new_n47_), .b(new_n79_), .c(x08), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g36(.a(x24), .b(x23), .c(x25), .O(new_n82_));
  nand2  g37(.a(new_n59_), .b(new_n54_), .O(new_n83_));
  nand4  g38(.a(new_n83_), .b(x24), .c(x22), .d(x21), .O(new_n84_));
  oai21  g39(.a(new_n82_), .b(new_n47_), .c(new_n84_), .O(z08));
  nor4   g40(.a(new_n63_), .b(new_n50_), .c(x15), .d(x07), .O(z09));
  nand4  g41(.a(new_n63_), .b(x22), .c(new_n56_), .d(new_n49_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x07), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n63_), .c(x22), .d(new_n49_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x07), .O(z11));
  nor2   g46(.a(new_n57_), .b(new_n56_), .O(new_n92_));
  nand3  g47(.a(new_n58_), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n58_), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n63_), .c(x22), .d(new_n49_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z12));
  inv1   g51(.a(x07), .O(new_n97_));
  nor2   g52(.a(x20), .b(new_n58_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(x18), .c(x17), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n64_), .b(new_n49_), .c(new_n97_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n101_), .c(new_n99_), .d(new_n97_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x22), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n49_), .O(z13));
  aoi21  g60(.a(new_n98_), .b(new_n92_), .c(new_n55_), .O(new_n106_));
  nand4  g61(.a(new_n92_), .b(new_n55_), .c(new_n54_), .d(x19), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n102_), .c(new_n97_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n106_), .c(x22), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n49_), .O(z14));
  nand4  g65(.a(new_n63_), .b(x19), .c(x18), .d(x17), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n97_), .c(new_n50_), .O(new_n112_));
  inv1   g67(.a(new_n100_), .O(new_n113_));
  nor2   g68(.a(new_n50_), .b(new_n55_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(x20), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n49_), .d(new_n97_), .O(z15));
  oai21  g71(.a(new_n63_), .b(x07), .c(x22), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n49_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n51_), .c(new_n49_), .d(new_n97_), .O(z16));
  nor3   g74(.a(x24), .b(x15), .c(x07), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n118_), .O(z17));
  inv1   g76(.a(x25), .O(new_n122_));
  nand3  g77(.a(new_n102_), .b(new_n122_), .c(new_n97_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(x22), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n49_), .O(z18));
endmodule


