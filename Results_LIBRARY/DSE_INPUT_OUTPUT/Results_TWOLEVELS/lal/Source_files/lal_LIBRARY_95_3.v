// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_;
  inv1   g000(.a(x16), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  inv1   g002(.a(x23), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g006(.a(new_n51_), .O(z00));
  oai21  g007(.a(x15), .b(x07), .c(x23), .O(new_n53_));
  nand3  g008(.a(new_n53_), .b(x05), .c(x04), .O(new_n54_));
  nor2   g009(.a(x25), .b(x24), .O(new_n55_));
  oai21  g010(.a(new_n55_), .b(x07), .c(new_n50_), .O(new_n56_));
  inv1   g011(.a(x20), .O(new_n57_));
  inv1   g012(.a(x17), .O(new_n58_));
  inv1   g013(.a(x18), .O(new_n59_));
  inv1   g014(.a(x19), .O(new_n60_));
  nand3  g015(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g016(.a(x22), .b(x21), .O(new_n62_));
  aoi21  g017(.a(new_n61_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  nor2   g018(.a(new_n63_), .b(x25), .O(new_n64_));
  nand2  g019(.a(new_n64_), .b(new_n48_), .O(new_n65_));
  nand3  g020(.a(new_n65_), .b(new_n56_), .c(new_n54_), .O(z01));
  nand2  g021(.a(new_n50_), .b(new_n46_), .O(z02));
  inv1   g022(.a(x25), .O(new_n68_));
  oai22  g023(.a(new_n63_), .b(x23), .c(new_n49_), .d(x24), .O(new_n69_));
  nand2  g024(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g025(.a(new_n70_), .b(new_n50_), .O(z03));
  inv1   g026(.a(x08), .O(new_n72_));
  xnor2a g027(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g028(.a(x10), .b(x01), .O(new_n74_));
  xnor2a g029(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g030(.a(x12), .b(x03), .O(new_n76_));
  nand4  g031(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n50_), .O(z04));
  oai21  g034(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g035(.a(x14), .b(new_n72_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n50_), .O(z06));
  aoi21  g037(.a(new_n72_), .b(x06), .c(new_n49_), .O(z07));
  nand2  g038(.a(new_n60_), .b(new_n59_), .O(new_n84_));
  nand3  g039(.a(x24), .b(x22), .c(x21), .O(new_n85_));
  oai21  g040(.a(new_n85_), .b(new_n84_), .c(new_n68_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(new_n58_), .O(new_n87_));
  nand2  g042(.a(new_n85_), .b(new_n68_), .O(new_n88_));
  nand2  g043(.a(new_n88_), .b(x20), .O(new_n89_));
  oai21  g044(.a(new_n48_), .b(x15), .c(new_n68_), .O(new_n90_));
  nand2  g045(.a(new_n90_), .b(x24), .O(new_n91_));
  aoi21  g046(.a(new_n68_), .b(new_n47_), .c(new_n48_), .O(new_n92_));
  inv1   g047(.a(x21), .O(new_n93_));
  inv1   g048(.a(x22), .O(new_n94_));
  nor2   g049(.a(new_n60_), .b(new_n59_), .O(new_n95_));
  and2   g050(.a(x18), .b(x17), .O(new_n96_));
  nor2   g051(.a(x20), .b(new_n60_), .O(new_n97_));
  nor2   g052(.a(x22), .b(x21), .O(new_n98_));
  nor2   g053(.a(x24), .b(x23), .O(new_n99_));
  nand4  g054(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n101_));
  aoi21  g056(.a(new_n101_), .b(x25), .c(new_n92_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n91_), .c(new_n89_), .d(new_n87_), .O(z08));
  inv1   g058(.a(x07), .O(new_n104_));
  nand4  g059(.a(new_n47_), .b(new_n104_), .c(x05), .d(x04), .O(new_n105_));
  inv1   g060(.a(new_n105_), .O(z09));
  nand2  g061(.a(x05), .b(x04), .O(new_n107_));
  nand4  g062(.a(new_n107_), .b(new_n58_), .c(new_n47_), .d(new_n104_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n50_), .O(z10));
  xor2a  g064(.a(x18), .b(x17), .O(new_n110_));
  nand4  g065(.a(new_n110_), .b(new_n107_), .c(new_n47_), .d(new_n104_), .O(new_n111_));
  inv1   g066(.a(new_n111_), .O(z11));
  nand3  g067(.a(new_n60_), .b(x18), .c(x17), .O(new_n113_));
  oai21  g068(.a(new_n96_), .b(new_n60_), .c(new_n113_), .O(new_n114_));
  nand4  g069(.a(new_n114_), .b(new_n107_), .c(new_n47_), .d(new_n104_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n50_), .O(z12));
  nand2  g071(.a(new_n97_), .b(new_n96_), .O(new_n117_));
  nand3  g072(.a(x19), .b(x18), .c(x17), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(x20), .O(new_n119_));
  nand3  g074(.a(new_n104_), .b(x05), .c(x04), .O(new_n120_));
  nand4  g075(.a(new_n120_), .b(new_n119_), .c(new_n117_), .d(new_n104_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  nand2  g077(.a(new_n48_), .b(x15), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(new_n122_), .O(z13));
  nand2  g079(.a(new_n120_), .b(new_n104_), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(new_n47_), .O(new_n126_));
  nand2  g081(.a(new_n117_), .b(x21), .O(new_n127_));
  nor2   g082(.a(x21), .b(x20), .O(new_n128_));
  nand3  g083(.a(new_n128_), .b(new_n96_), .c(x19), .O(new_n129_));
  nand4  g084(.a(new_n129_), .b(new_n127_), .c(new_n126_), .d(new_n47_), .O(z14));
  inv1   g085(.a(new_n118_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n98_), .c(new_n57_), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n120_), .c(new_n104_), .O(new_n133_));
  aoi21  g088(.a(new_n129_), .b(x22), .c(new_n133_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(x15), .c(new_n123_), .O(z15));
  nand2  g090(.a(new_n132_), .b(x23), .O(new_n136_));
  nand4  g091(.a(new_n128_), .b(new_n131_), .c(new_n48_), .d(new_n94_), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(new_n136_), .c(new_n126_), .d(new_n47_), .O(z16));
  inv1   g093(.a(x24), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n94_), .c(new_n93_), .d(new_n57_), .O(new_n140_));
  inv1   g095(.a(new_n140_), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(x19), .c(x18), .d(x17), .O(new_n142_));
  aoi21  g097(.a(new_n132_), .b(x24), .c(x15), .O(new_n143_));
  nand2  g098(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(new_n48_), .O(new_n145_));
  nand2  g100(.a(x24), .b(x23), .O(new_n146_));
  nand3  g101(.a(new_n146_), .b(new_n120_), .c(new_n104_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n47_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n145_), .O(z17));
  nand2  g104(.a(new_n100_), .b(x25), .O(new_n150_));
  inv1   g105(.a(new_n117_), .O(new_n151_));
  nand4  g106(.a(new_n98_), .b(new_n151_), .c(new_n55_), .d(new_n48_), .O(new_n152_));
  nand4  g107(.a(new_n152_), .b(new_n150_), .c(new_n126_), .d(new_n47_), .O(z18));
endmodule


