// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  aoi21  g02(.a(x16), .b(new_n47_), .c(new_n46_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n49_), .c(new_n50_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n46_), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x07), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  inv1   g21(.a(new_n63_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n66_), .O(z02));
  nand2  g23(.a(new_n67_), .b(new_n61_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n67_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n67_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n67_), .O(z06));
  nand2  g34(.a(new_n46_), .b(x06), .O(z07));
  aoi21  g35(.a(new_n55_), .b(new_n51_), .c(new_n58_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x21), .c(x23), .O(new_n82_));
  nor2   g37(.a(new_n63_), .b(x25), .O(new_n83_));
  oai21  g38(.a(new_n82_), .b(new_n49_), .c(new_n83_), .O(z08));
  inv1   g39(.a(x07), .O(new_n85_));
  inv1   g40(.a(new_n62_), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n47_), .c(new_n85_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand4  g43(.a(new_n62_), .b(new_n52_), .c(new_n47_), .d(new_n85_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n67_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n62_), .c(new_n47_), .d(new_n85_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n67_), .O(z11));
  nor2   g48(.a(new_n53_), .b(new_n52_), .O(new_n94_));
  nand3  g49(.a(new_n54_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n54_), .c(new_n95_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n62_), .c(new_n47_), .d(new_n85_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n67_), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n85_), .b(x05), .c(x04), .O(new_n101_));
  nor2   g56(.a(x20), .b(new_n54_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n94_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .d(new_n85_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(new_n105_));
  nand2  g60(.a(x15), .b(new_n46_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n105_), .O(z13));
  aoi21  g62(.a(new_n102_), .b(new_n94_), .c(new_n57_), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n94_), .c(x19), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n101_), .c(new_n85_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n108_), .c(new_n47_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n106_), .O(z14));
  nand2  g68(.a(new_n110_), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n86_), .b(new_n47_), .c(new_n85_), .O(new_n115_));
  inv1   g70(.a(new_n99_), .O(new_n116_));
  nor2   g71(.a(x22), .b(x21), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n51_), .O(new_n118_));
  and2   g73(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n114_), .c(new_n47_), .d(new_n85_), .O(z15));
  nand2  g75(.a(new_n118_), .b(x23), .O(new_n121_));
  inv1   g76(.a(new_n115_), .O(new_n122_));
  inv1   g77(.a(x23), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n58_), .c(new_n57_), .d(new_n51_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n116_), .c(new_n122_), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n121_), .c(new_n47_), .d(new_n85_), .O(z16));
  oai21  g82(.a(new_n124_), .b(new_n99_), .c(x24), .O(new_n128_));
  nor2   g83(.a(x24), .b(x23), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n117_), .c(new_n102_), .d(new_n94_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n128_), .c(new_n101_), .d(new_n85_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n47_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n106_), .O(z17));
  nand2  g88(.a(new_n130_), .b(x25), .O(new_n134_));
  inv1   g89(.a(new_n103_), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  and2   g91(.a(new_n136_), .b(new_n117_), .O(new_n137_));
  aoi21  g92(.a(new_n137_), .b(new_n135_), .c(new_n122_), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n134_), .c(new_n47_), .d(new_n85_), .O(z18));
endmodule


