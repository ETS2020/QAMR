// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  nand2  g07(.a(new_n47_), .b(new_n52_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(x24), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(x05), .c(x04), .O(new_n55_));
  nand2  g10(.a(new_n50_), .b(x07), .O(new_n56_));
  inv1   g11(.a(x25), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  nor3   g13(.a(x19), .b(x18), .c(x17), .O(new_n59_));
  inv1   g14(.a(x21), .O(new_n60_));
  inv1   g15(.a(x22), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  oai21  g17(.a(new_n59_), .b(x20), .c(new_n62_), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n58_), .c(new_n47_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(x24), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n57_), .O(new_n66_));
  nand3  g21(.a(new_n66_), .b(new_n56_), .c(new_n55_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n49_), .b(new_n68_), .O(z02));
  inv1   g24(.a(new_n66_), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n50_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z06));
  nand3  g35(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x20), .O(new_n82_));
  inv1   g37(.a(new_n59_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n82_), .c(new_n61_), .O(new_n84_));
  nand2  g39(.a(new_n58_), .b(new_n47_), .O(new_n85_));
  aoi21  g40(.a(new_n84_), .b(x21), .c(new_n85_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n48_), .c(new_n57_), .O(z08));
  nand2  g42(.a(x05), .b(x04), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n53_), .c(new_n50_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand4  g45(.a(new_n88_), .b(new_n90_), .c(new_n47_), .d(new_n52_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n88_), .c(new_n47_), .d(new_n52_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z11));
  inv1   g50(.a(x19), .O(new_n96_));
  and2   g51(.a(x18), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n96_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n88_), .c(new_n47_), .d(new_n52_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z12));
  nand3  g56(.a(new_n52_), .b(x05), .c(x04), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n52_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n97_), .b(new_n82_), .c(x19), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n104_), .d(new_n47_), .O(z13));
  nand2  g63(.a(new_n107_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n97_), .c(x19), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n104_), .d(new_n47_), .O(z14));
  inv1   g67(.a(new_n105_), .O(new_n113_));
  nor3   g68(.a(x22), .b(x21), .c(x20), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n102_), .c(new_n52_), .O(new_n116_));
  aoi21  g71(.a(new_n111_), .b(x22), .c(new_n116_), .O(new_n117_));
  nand2  g72(.a(new_n48_), .b(x15), .O(new_n118_));
  oai21  g73(.a(new_n117_), .b(x15), .c(new_n118_), .O(z15));
  aoi21  g74(.a(new_n114_), .b(new_n113_), .c(new_n58_), .O(new_n120_));
  nand4  g75(.a(new_n110_), .b(new_n113_), .c(new_n58_), .d(new_n61_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n102_), .c(new_n52_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n120_), .c(new_n47_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n118_), .O(z16));
  nand2  g79(.a(new_n121_), .b(x24), .O(new_n125_));
  inv1   g80(.a(new_n107_), .O(new_n126_));
  nor2   g81(.a(x22), .b(x21), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n126_), .c(new_n48_), .d(new_n58_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n125_), .c(new_n104_), .d(new_n47_), .O(z17));
  nor2   g84(.a(x20), .b(new_n96_), .O(new_n130_));
  nor2   g85(.a(x24), .b(x23), .O(new_n131_));
  aoi21  g86(.a(new_n131_), .b(new_n130_), .c(x15), .O(new_n132_));
  aoi21  g87(.a(new_n127_), .b(new_n97_), .c(x24), .O(new_n133_));
  oai21  g88(.a(new_n133_), .b(new_n132_), .c(x25), .O(new_n134_));
  nand3  g89(.a(new_n127_), .b(new_n57_), .c(new_n58_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(new_n107_), .c(new_n47_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n48_), .O(new_n137_));
  nand3  g92(.a(new_n137_), .b(new_n134_), .c(new_n104_), .O(z18));
endmodule


