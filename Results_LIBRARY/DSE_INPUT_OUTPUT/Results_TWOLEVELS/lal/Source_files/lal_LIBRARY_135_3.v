// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x07), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n49_), .O(z01));
  and2   g16(.a(new_n47_), .b(x16), .O(z02));
  nand3  g17(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(z03));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n47_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g27(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z06));
  nand3  g29(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g30(.a(x24), .O(new_n76_));
  inv1   g31(.a(x22), .O(new_n77_));
  aoi21  g32(.a(new_n55_), .b(new_n51_), .c(new_n77_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(x21), .c(x23), .O(new_n79_));
  aoi21  g34(.a(x15), .b(x07), .c(x25), .O(new_n80_));
  oai21  g35(.a(new_n79_), .b(new_n76_), .c(new_n80_), .O(z08));
  inv1   g36(.a(x15), .O(new_n82_));
  nand4  g37(.a(new_n82_), .b(new_n49_), .c(x05), .d(x04), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z09));
  nand4  g39(.a(new_n60_), .b(new_n52_), .c(new_n82_), .d(new_n49_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z10));
  xor2a  g41(.a(x18), .b(x17), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n60_), .c(new_n82_), .d(new_n49_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n47_), .O(z11));
  nor2   g44(.a(new_n53_), .b(new_n52_), .O(new_n90_));
  nand3  g45(.a(new_n54_), .b(x18), .c(x17), .O(new_n91_));
  oai21  g46(.a(new_n90_), .b(new_n54_), .c(new_n91_), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n60_), .c(new_n82_), .d(new_n49_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n47_), .O(z12));
  aoi21  g49(.a(x05), .b(x04), .c(x15), .O(new_n95_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(x20), .O(new_n97_));
  nand4  g52(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .d(new_n49_), .O(z13));
  nand2  g54(.a(new_n98_), .b(x21), .O(new_n100_));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n90_), .c(x19), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n100_), .c(new_n95_), .d(new_n49_), .O(z14));
  or2    g58(.a(new_n95_), .b(x07), .O(new_n104_));
  nand2  g59(.a(new_n102_), .b(x22), .O(new_n105_));
  inv1   g60(.a(new_n96_), .O(new_n106_));
  nor2   g61(.a(x22), .b(x21), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n51_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n105_), .c(new_n104_), .d(new_n49_), .O(z15));
  nand2  g64(.a(new_n108_), .b(x23), .O(new_n110_));
  inv1   g65(.a(x23), .O(new_n111_));
  nand3  g66(.a(new_n101_), .b(new_n111_), .c(new_n77_), .O(new_n112_));
  inv1   g67(.a(new_n112_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n106_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n110_), .c(new_n104_), .d(new_n49_), .O(z16));
  oai21  g70(.a(new_n112_), .b(new_n96_), .c(x24), .O(new_n116_));
  nor2   g71(.a(x20), .b(new_n54_), .O(new_n117_));
  nor2   g72(.a(x24), .b(x23), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n107_), .c(new_n117_), .d(new_n90_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n116_), .c(new_n49_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n82_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n104_), .O(z17));
  nand2  g77(.a(new_n118_), .b(new_n107_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n98_), .c(x25), .O(new_n124_));
  nor3   g79(.a(x25), .b(x24), .c(x23), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n107_), .c(new_n117_), .d(new_n90_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n49_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n82_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n104_), .O(z18));
endmodule


