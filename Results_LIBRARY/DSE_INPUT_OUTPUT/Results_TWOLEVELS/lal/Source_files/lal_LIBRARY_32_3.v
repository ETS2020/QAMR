// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x16), .O(new_n46_));
  nand2  g001(.a(x15), .b(x10), .O(new_n47_));
  nand3  g002(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(z00));
  inv1   g004(.a(new_n47_), .O(new_n50_));
  oai21  g005(.a(x15), .b(x07), .c(x10), .O(new_n51_));
  nand3  g006(.a(new_n51_), .b(x05), .c(x04), .O(new_n52_));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  inv1   g009(.a(x17), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand3  g012(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g013(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g014(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  aoi21  g016(.a(new_n61_), .b(new_n53_), .c(x07), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(new_n50_), .c(new_n52_), .O(z01));
  nor2   g018(.a(new_n50_), .b(new_n46_), .O(z02));
  nand3  g019(.a(new_n61_), .b(new_n47_), .c(new_n53_), .O(new_n65_));
  nand2  g020(.a(new_n65_), .b(new_n47_), .O(z03));
  inv1   g021(.a(x09), .O(new_n67_));
  inv1   g022(.a(x02), .O(new_n68_));
  aoi22  g023(.a(x11), .b(new_n68_), .c(new_n67_), .d(x00), .O(new_n69_));
  oai21  g024(.a(new_n67_), .b(x00), .c(new_n69_), .O(new_n70_));
  inv1   g025(.a(x11), .O(new_n71_));
  nand2  g026(.a(new_n71_), .b(x02), .O(new_n72_));
  inv1   g027(.a(x03), .O(new_n73_));
  nand2  g028(.a(x12), .b(new_n73_), .O(new_n74_));
  inv1   g029(.a(x12), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(x03), .O(new_n76_));
  nand3  g031(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  oai21  g032(.a(new_n77_), .b(new_n70_), .c(new_n47_), .O(new_n78_));
  inv1   g033(.a(x01), .O(new_n79_));
  nor2   g034(.a(x10), .b(new_n79_), .O(new_n80_));
  inv1   g035(.a(x10), .O(new_n81_));
  nor2   g036(.a(x15), .b(new_n81_), .O(new_n82_));
  aoi21  g037(.a(new_n82_), .b(new_n79_), .c(new_n80_), .O(new_n83_));
  aoi21  g038(.a(new_n83_), .b(new_n78_), .c(x08), .O(z04));
  oai21  g039(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g040(.a(x14), .O(new_n86_));
  oai21  g041(.a(new_n86_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g042(.a(x08), .O(new_n88_));
  aoi21  g043(.a(new_n88_), .b(x06), .c(new_n50_), .O(z07));
  inv1   g044(.a(x24), .O(new_n90_));
  inv1   g045(.a(x22), .O(new_n91_));
  aoi21  g046(.a(new_n58_), .b(new_n54_), .c(new_n91_), .O(new_n92_));
  aoi21  g047(.a(new_n92_), .b(x21), .c(x23), .O(new_n93_));
  nor2   g048(.a(new_n50_), .b(x25), .O(new_n94_));
  oai21  g049(.a(new_n93_), .b(new_n90_), .c(new_n94_), .O(z08));
  inv1   g050(.a(x07), .O(new_n96_));
  inv1   g051(.a(x15), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n96_), .c(x05), .d(x04), .O(new_n98_));
  inv1   g053(.a(new_n98_), .O(z09));
  nand2  g054(.a(x05), .b(x04), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n55_), .c(new_n97_), .d(new_n96_), .O(new_n101_));
  inv1   g056(.a(new_n101_), .O(z10));
  xor2a  g057(.a(x18), .b(x17), .O(new_n103_));
  nand4  g058(.a(new_n103_), .b(new_n100_), .c(new_n97_), .d(new_n96_), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(new_n47_), .O(z11));
  nor2   g060(.a(new_n56_), .b(new_n55_), .O(new_n106_));
  nand3  g061(.a(new_n57_), .b(x18), .c(x17), .O(new_n107_));
  oai21  g062(.a(new_n106_), .b(new_n57_), .c(new_n107_), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n100_), .c(new_n97_), .d(new_n96_), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(new_n47_), .O(z12));
  nand3  g065(.a(x19), .b(x18), .c(x17), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(x20), .O(new_n112_));
  nand3  g067(.a(new_n96_), .b(x05), .c(x04), .O(new_n113_));
  nor2   g068(.a(x20), .b(new_n57_), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n106_), .O(new_n115_));
  nand4  g070(.a(new_n115_), .b(new_n113_), .c(new_n112_), .d(new_n96_), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(new_n97_), .O(new_n117_));
  nand2  g072(.a(x15), .b(new_n81_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n117_), .O(z13));
  nand2  g074(.a(new_n113_), .b(new_n96_), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(new_n97_), .O(new_n121_));
  nand2  g076(.a(new_n115_), .b(x21), .O(new_n122_));
  nor2   g077(.a(x21), .b(x20), .O(new_n123_));
  nand3  g078(.a(new_n123_), .b(new_n106_), .c(x19), .O(new_n124_));
  nand4  g079(.a(new_n124_), .b(new_n122_), .c(new_n121_), .d(new_n97_), .O(z14));
  nor2   g080(.a(x22), .b(x21), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n54_), .O(new_n127_));
  nor2   g082(.a(new_n127_), .b(new_n111_), .O(new_n128_));
  aoi21  g083(.a(new_n124_), .b(x22), .c(new_n128_), .O(new_n129_));
  nand3  g084(.a(new_n129_), .b(new_n121_), .c(new_n97_), .O(z15));
  inv1   g085(.a(x23), .O(new_n131_));
  nor2   g086(.a(new_n128_), .b(new_n131_), .O(new_n132_));
  inv1   g087(.a(new_n111_), .O(new_n133_));
  nand4  g088(.a(new_n123_), .b(new_n133_), .c(new_n131_), .d(new_n91_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n113_), .c(new_n96_), .O(new_n135_));
  oai21  g090(.a(new_n135_), .b(new_n132_), .c(new_n97_), .O(new_n136_));
  nand2  g091(.a(new_n136_), .b(new_n118_), .O(z16));
  nand2  g092(.a(new_n134_), .b(x24), .O(new_n138_));
  nand3  g093(.a(new_n126_), .b(new_n90_), .c(new_n131_), .O(new_n139_));
  or2    g094(.a(new_n139_), .b(new_n115_), .O(new_n140_));
  nand4  g095(.a(new_n140_), .b(new_n138_), .c(new_n121_), .d(new_n97_), .O(z17));
  oai21  g096(.a(new_n139_), .b(new_n115_), .c(x25), .O(new_n142_));
  nor3   g097(.a(x25), .b(x24), .c(x23), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n126_), .c(new_n114_), .d(new_n106_), .O(new_n144_));
  nand4  g099(.a(new_n144_), .b(new_n142_), .c(new_n113_), .d(new_n96_), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n97_), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(new_n118_), .O(z18));
endmodule


