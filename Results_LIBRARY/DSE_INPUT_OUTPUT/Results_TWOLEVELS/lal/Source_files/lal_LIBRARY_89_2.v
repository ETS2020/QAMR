// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x07), .O(new_n47_));
  nor2   g002(.a(x15), .b(new_n47_), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x25), .O(new_n51_));
  inv1   g006(.a(x20), .O(new_n52_));
  inv1   g007(.a(x17), .O(new_n53_));
  nor2   g008(.a(x19), .b(x18), .O(new_n54_));
  nand2  g009(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g010(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n56_), .O(new_n57_));
  oai21  g012(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g013(.a(new_n58_), .b(new_n51_), .O(new_n59_));
  nand2  g014(.a(x05), .b(x04), .O(new_n60_));
  nand3  g015(.a(new_n60_), .b(new_n59_), .c(new_n47_), .O(z01));
  inv1   g016(.a(x16), .O(new_n62_));
  nand2  g017(.a(new_n49_), .b(new_n62_), .O(z02));
  nand3  g018(.a(new_n58_), .b(new_n49_), .c(new_n51_), .O(new_n64_));
  inv1   g019(.a(new_n64_), .O(z03));
  xnor2a g020(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g021(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g022(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g023(.a(x12), .b(x03), .O(new_n69_));
  nand4  g024(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand3  g025(.a(new_n70_), .b(new_n49_), .c(new_n46_), .O(new_n71_));
  inv1   g026(.a(new_n71_), .O(z04));
  nor3   g027(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g028(.a(x14), .O(new_n74_));
  nor3   g029(.a(new_n48_), .b(new_n74_), .c(x08), .O(z06));
  aoi21  g030(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g031(.a(x18), .O(new_n77_));
  inv1   g032(.a(x19), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g034(.a(x24), .b(x22), .c(x21), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n79_), .c(new_n51_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n53_), .O(new_n82_));
  nand2  g037(.a(new_n80_), .b(new_n51_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(x20), .O(new_n84_));
  oai21  g039(.a(x25), .b(x24), .c(x23), .O(new_n85_));
  inv1   g040(.a(x21), .O(new_n86_));
  nand3  g041(.a(new_n86_), .b(x19), .c(x18), .O(new_n87_));
  inv1   g042(.a(x22), .O(new_n88_));
  inv1   g043(.a(x24), .O(new_n89_));
  nor2   g044(.a(new_n77_), .b(new_n53_), .O(new_n90_));
  nor2   g045(.a(x20), .b(new_n78_), .O(new_n91_));
  nor2   g046(.a(x22), .b(x21), .O(new_n92_));
  nor2   g047(.a(x24), .b(x23), .O(new_n93_));
  nand4  g048(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  nand3  g049(.a(new_n94_), .b(new_n89_), .c(new_n88_), .O(new_n95_));
  oai21  g050(.a(new_n95_), .b(new_n87_), .c(x25), .O(new_n96_));
  nand4  g051(.a(new_n96_), .b(new_n85_), .c(new_n84_), .d(new_n82_), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(new_n47_), .O(new_n98_));
  inv1   g053(.a(x23), .O(new_n99_));
  nand2  g054(.a(new_n55_), .b(new_n52_), .O(new_n100_));
  nand3  g055(.a(new_n100_), .b(x22), .c(x21), .O(new_n101_));
  aoi21  g056(.a(new_n101_), .b(new_n99_), .c(new_n89_), .O(new_n102_));
  oai21  g057(.a(new_n102_), .b(x25), .c(x15), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n98_), .O(z08));
  aoi21  g059(.a(x05), .b(x04), .c(x07), .O(new_n105_));
  nor2   g060(.a(new_n105_), .b(x15), .O(z09));
  inv1   g061(.a(x15), .O(new_n107_));
  nand4  g062(.a(new_n60_), .b(new_n53_), .c(new_n107_), .d(new_n47_), .O(new_n108_));
  inv1   g063(.a(new_n108_), .O(z10));
  xor2a  g064(.a(x18), .b(x17), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(new_n60_), .O(new_n111_));
  aoi21  g066(.a(new_n111_), .b(new_n47_), .c(x15), .O(z11));
  nand3  g067(.a(new_n78_), .b(x18), .c(x17), .O(new_n113_));
  oai21  g068(.a(new_n90_), .b(new_n78_), .c(new_n113_), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n60_), .O(new_n115_));
  aoi21  g070(.a(new_n115_), .b(new_n47_), .c(x15), .O(z12));
  inv1   g071(.a(z09), .O(new_n117_));
  nor2   g072(.a(new_n78_), .b(new_n77_), .O(new_n118_));
  inv1   g073(.a(new_n118_), .O(new_n119_));
  nand3  g074(.a(new_n51_), .b(new_n99_), .c(x15), .O(new_n120_));
  aoi21  g075(.a(new_n120_), .b(new_n119_), .c(new_n53_), .O(new_n121_));
  nand4  g076(.a(new_n79_), .b(new_n51_), .c(new_n99_), .d(x15), .O(new_n122_));
  inv1   g077(.a(new_n122_), .O(new_n123_));
  oai21  g078(.a(new_n123_), .b(new_n121_), .c(new_n52_), .O(new_n124_));
  nor3   g079(.a(new_n80_), .b(new_n79_), .c(new_n107_), .O(new_n125_));
  oai21  g080(.a(new_n125_), .b(x20), .c(new_n53_), .O(new_n126_));
  aoi21  g081(.a(new_n56_), .b(new_n99_), .c(new_n89_), .O(new_n127_));
  oai21  g082(.a(new_n56_), .b(new_n52_), .c(new_n99_), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(x24), .O(new_n129_));
  nand3  g084(.a(new_n129_), .b(new_n127_), .c(new_n51_), .O(new_n130_));
  aoi22  g085(.a(new_n130_), .b(x15), .c(new_n119_), .d(x20), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n126_), .c(new_n124_), .d(new_n117_), .O(z13));
  nand4  g087(.a(new_n86_), .b(x19), .c(x18), .d(new_n47_), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n120_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n52_), .c(x17), .O(new_n135_));
  oai21  g090(.a(x23), .b(x21), .c(x24), .O(new_n136_));
  nand2  g091(.a(new_n136_), .b(new_n51_), .O(new_n137_));
  nand2  g092(.a(x24), .b(x23), .O(new_n138_));
  nand2  g093(.a(new_n138_), .b(new_n51_), .O(new_n139_));
  inv1   g094(.a(new_n139_), .O(new_n140_));
  nand3  g095(.a(new_n140_), .b(new_n137_), .c(new_n47_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(x15), .O(new_n142_));
  aoi21  g097(.a(new_n91_), .b(new_n90_), .c(new_n86_), .O(new_n143_));
  nand3  g098(.a(new_n107_), .b(x05), .c(x04), .O(new_n144_));
  inv1   g099(.a(new_n144_), .O(new_n145_));
  oai21  g100(.a(new_n145_), .b(new_n143_), .c(new_n47_), .O(new_n146_));
  nand3  g101(.a(new_n146_), .b(new_n142_), .c(new_n135_), .O(z14));
  inv1   g102(.a(new_n90_), .O(new_n148_));
  nand3  g103(.a(new_n88_), .b(new_n52_), .c(x19), .O(new_n149_));
  oai21  g104(.a(new_n149_), .b(new_n148_), .c(new_n120_), .O(new_n150_));
  nand2  g105(.a(new_n150_), .b(new_n86_), .O(new_n151_));
  nand4  g106(.a(new_n51_), .b(x24), .c(new_n99_), .d(x22), .O(new_n152_));
  nand4  g107(.a(new_n90_), .b(new_n86_), .c(new_n52_), .d(x19), .O(new_n153_));
  aoi22  g108(.a(new_n153_), .b(x22), .c(new_n152_), .d(x15), .O(new_n154_));
  nand3  g109(.a(new_n154_), .b(new_n151_), .c(new_n117_), .O(z15));
  nand2  g110(.a(new_n118_), .b(new_n92_), .O(new_n156_));
  nand2  g111(.a(new_n51_), .b(x15), .O(new_n157_));
  aoi21  g112(.a(new_n157_), .b(new_n156_), .c(new_n53_), .O(new_n158_));
  nor3   g113(.a(new_n54_), .b(x25), .c(new_n107_), .O(new_n159_));
  oai21  g114(.a(new_n159_), .b(new_n158_), .c(new_n52_), .O(new_n160_));
  nand3  g115(.a(new_n56_), .b(new_n51_), .c(x15), .O(new_n161_));
  nand2  g116(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  nand2  g117(.a(new_n162_), .b(new_n99_), .O(new_n163_));
  oai21  g118(.a(new_n125_), .b(x23), .c(new_n53_), .O(new_n164_));
  nand4  g119(.a(x24), .b(x22), .c(x21), .d(x15), .O(new_n165_));
  inv1   g120(.a(new_n165_), .O(new_n166_));
  oai21  g121(.a(new_n166_), .b(x23), .c(x20), .O(new_n167_));
  nand3  g122(.a(new_n51_), .b(x24), .c(new_n99_), .O(new_n168_));
  nand2  g123(.a(new_n168_), .b(x15), .O(new_n169_));
  nand2  g124(.a(new_n156_), .b(x23), .O(new_n170_));
  nand3  g125(.a(new_n170_), .b(new_n169_), .c(new_n167_), .O(new_n171_));
  inv1   g126(.a(new_n171_), .O(new_n172_));
  nand4  g127(.a(new_n172_), .b(new_n164_), .c(new_n163_), .d(new_n117_), .O(z16));
  nand2  g128(.a(new_n93_), .b(new_n88_), .O(new_n174_));
  nor3   g129(.a(new_n174_), .b(x21), .c(x20), .O(new_n175_));
  nand4  g130(.a(new_n175_), .b(x19), .c(x18), .d(x17), .O(new_n176_));
  and2   g131(.a(new_n144_), .b(new_n138_), .O(new_n177_));
  nand2  g132(.a(new_n137_), .b(new_n51_), .O(new_n178_));
  nand4  g133(.a(new_n118_), .b(new_n92_), .c(new_n52_), .d(x17), .O(new_n179_));
  aoi22  g134(.a(new_n179_), .b(x24), .c(new_n178_), .d(x15), .O(new_n180_));
  nand4  g135(.a(new_n180_), .b(new_n177_), .c(new_n176_), .d(new_n47_), .O(z17));
  nand3  g136(.a(x19), .b(x18), .c(new_n47_), .O(new_n182_));
  nand3  g137(.a(new_n89_), .b(new_n88_), .c(new_n86_), .O(new_n183_));
  oai21  g138(.a(new_n183_), .b(new_n182_), .c(new_n107_), .O(new_n184_));
  nor2   g139(.a(new_n54_), .b(new_n107_), .O(new_n185_));
  aoi21  g140(.a(new_n184_), .b(x17), .c(new_n185_), .O(new_n186_));
  nand2  g141(.a(new_n56_), .b(x15), .O(new_n187_));
  oai21  g142(.a(new_n186_), .b(x20), .c(new_n187_), .O(new_n188_));
  nor2   g143(.a(x24), .b(new_n107_), .O(new_n189_));
  aoi21  g144(.a(new_n188_), .b(new_n99_), .c(new_n189_), .O(new_n190_));
  nor2   g145(.a(new_n51_), .b(x07), .O(new_n191_));
  oai21  g146(.a(new_n191_), .b(new_n125_), .c(new_n53_), .O(new_n192_));
  oai21  g147(.a(new_n191_), .b(new_n166_), .c(x20), .O(new_n193_));
  nand2  g148(.a(new_n139_), .b(x15), .O(new_n194_));
  oai21  g149(.a(new_n174_), .b(new_n87_), .c(x25), .O(new_n195_));
  nand2  g150(.a(new_n195_), .b(new_n144_), .O(new_n196_));
  nand2  g151(.a(new_n196_), .b(new_n47_), .O(new_n197_));
  nand4  g152(.a(new_n197_), .b(new_n194_), .c(new_n193_), .d(new_n192_), .O(new_n198_));
  inv1   g153(.a(new_n198_), .O(new_n199_));
  oai21  g154(.a(new_n190_), .b(x25), .c(new_n199_), .O(z18));
endmodule


