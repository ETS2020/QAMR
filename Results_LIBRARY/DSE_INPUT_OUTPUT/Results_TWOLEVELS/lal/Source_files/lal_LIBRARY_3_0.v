// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x15), .b(x04), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  inv1   g14(.a(x21), .O(new_n60_));
  inv1   g15(.a(x22), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(new_n52_), .c(new_n53_), .O(new_n64_));
  inv1   g19(.a(x04), .O(new_n65_));
  inv1   g20(.a(x05), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor3   g22(.a(new_n67_), .b(new_n48_), .c(x07), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n64_), .O(z01));
  nand2  g24(.a(new_n64_), .b(new_n49_), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n49_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor3   g34(.a(new_n48_), .b(new_n79_), .c(x08), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g36(.a(new_n58_), .b(new_n54_), .c(new_n61_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x21), .c(x23), .O(new_n83_));
  nor2   g38(.a(new_n48_), .b(x25), .O(new_n84_));
  oai21  g39(.a(new_n83_), .b(new_n52_), .c(new_n84_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand3  g42(.a(new_n67_), .b(new_n87_), .c(new_n86_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n66_), .d(x04), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x17), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n87_), .c(new_n86_), .d(new_n66_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(new_n65_), .O(z11));
  nor2   g49(.a(new_n56_), .b(new_n55_), .O(new_n95_));
  nand3  g50(.a(new_n57_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n57_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n87_), .c(new_n86_), .d(new_n66_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n65_), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nor2   g56(.a(new_n67_), .b(x07), .O(new_n102_));
  nand4  g57(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(new_n48_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n102_), .c(new_n101_), .d(new_n87_), .O(z13));
  nor2   g61(.a(new_n104_), .b(new_n60_), .O(new_n107_));
  nand3  g62(.a(new_n87_), .b(new_n86_), .c(x05), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n95_), .c(x19), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n86_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n107_), .c(x04), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n87_), .O(z14));
  nand2  g68(.a(new_n110_), .b(x22), .O(new_n114_));
  inv1   g69(.a(new_n100_), .O(new_n115_));
  nor3   g70(.a(x22), .b(x21), .c(x20), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n115_), .c(new_n48_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n114_), .c(new_n102_), .d(new_n87_), .O(z15));
  inv1   g73(.a(x23), .O(new_n119_));
  aoi21  g74(.a(new_n116_), .b(new_n115_), .c(new_n119_), .O(new_n120_));
  nand4  g75(.a(new_n109_), .b(new_n115_), .c(new_n119_), .d(new_n61_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n108_), .c(new_n86_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n120_), .c(x04), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n87_), .O(z16));
  nand2  g79(.a(new_n121_), .b(x24), .O(new_n125_));
  nand4  g80(.a(new_n52_), .b(new_n119_), .c(new_n61_), .d(new_n60_), .O(new_n126_));
  inv1   g81(.a(new_n126_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n104_), .c(new_n48_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n125_), .c(new_n102_), .d(new_n87_), .O(z17));
  oai21  g84(.a(new_n126_), .b(new_n103_), .c(x25), .O(new_n130_));
  nor2   g85(.a(x20), .b(new_n57_), .O(new_n131_));
  nor2   g86(.a(x22), .b(x21), .O(new_n132_));
  nor3   g87(.a(x25), .b(x24), .c(x23), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n95_), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n130_), .c(new_n108_), .d(new_n86_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(x04), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n87_), .O(z18));
endmodule


