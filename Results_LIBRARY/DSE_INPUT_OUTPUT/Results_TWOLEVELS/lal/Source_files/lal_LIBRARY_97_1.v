// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x21), .O(new_n47_));
  nor2   g002(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x15), .O(new_n51_));
  oai21  g006(.a(x21), .b(x07), .c(new_n51_), .O(new_n52_));
  nand3  g007(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  nor2   g008(.a(x25), .b(x24), .O(new_n54_));
  oai21  g009(.a(new_n54_), .b(x07), .c(new_n49_), .O(new_n55_));
  inv1   g010(.a(x23), .O(new_n56_));
  inv1   g011(.a(x25), .O(new_n57_));
  inv1   g012(.a(x20), .O(new_n58_));
  inv1   g013(.a(x22), .O(new_n59_));
  inv1   g014(.a(x17), .O(new_n60_));
  inv1   g015(.a(x18), .O(new_n61_));
  inv1   g016(.a(x19), .O(new_n62_));
  nand3  g017(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  aoi21  g018(.a(new_n63_), .b(new_n58_), .c(new_n59_), .O(new_n64_));
  oai21  g019(.a(new_n64_), .b(new_n51_), .c(x21), .O(new_n65_));
  nand3  g020(.a(new_n65_), .b(new_n57_), .c(new_n56_), .O(new_n66_));
  nand3  g021(.a(new_n66_), .b(new_n55_), .c(new_n53_), .O(z01));
  inv1   g022(.a(x16), .O(new_n68_));
  nand2  g023(.a(new_n49_), .b(new_n68_), .O(z02));
  nand2  g024(.a(x24), .b(x23), .O(new_n70_));
  oai21  g025(.a(new_n64_), .b(x23), .c(x24), .O(new_n71_));
  aoi22  g026(.a(new_n71_), .b(x15), .c(new_n70_), .d(new_n47_), .O(new_n72_));
  oai21  g027(.a(new_n72_), .b(x25), .c(new_n49_), .O(z03));
  xnor2a g028(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g029(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g030(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g031(.a(x12), .b(x03), .O(new_n77_));
  nand4  g032(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand3  g033(.a(new_n78_), .b(new_n49_), .c(new_n46_), .O(new_n79_));
  inv1   g034(.a(new_n79_), .O(z04));
  nor3   g035(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g036(.a(x14), .O(new_n82_));
  nor3   g037(.a(new_n48_), .b(new_n82_), .c(x08), .O(z06));
  aoi21  g038(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand3  g039(.a(new_n62_), .b(new_n61_), .c(x15), .O(new_n85_));
  nand3  g040(.a(x24), .b(x22), .c(x21), .O(new_n86_));
  nand2  g041(.a(x25), .b(new_n47_), .O(new_n87_));
  oai21  g042(.a(new_n86_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  nand2  g043(.a(new_n88_), .b(new_n60_), .O(new_n89_));
  nand4  g044(.a(x24), .b(x22), .c(x21), .d(x15), .O(new_n90_));
  nand2  g045(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(x20), .O(new_n92_));
  nand2  g047(.a(new_n70_), .b(new_n57_), .O(new_n93_));
  nand2  g048(.a(new_n93_), .b(x15), .O(new_n94_));
  oai21  g049(.a(x25), .b(x24), .c(x23), .O(new_n95_));
  inv1   g050(.a(x24), .O(new_n96_));
  nor2   g051(.a(new_n62_), .b(new_n61_), .O(new_n97_));
  nand3  g052(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  inv1   g053(.a(new_n98_), .O(new_n99_));
  nor2   g054(.a(x22), .b(x20), .O(new_n100_));
  nor2   g055(.a(x24), .b(x23), .O(new_n101_));
  nand3  g056(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n97_), .c(new_n96_), .d(new_n59_), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(x25), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(new_n95_), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(new_n47_), .O(new_n106_));
  nand4  g061(.a(new_n106_), .b(new_n94_), .c(new_n92_), .d(new_n89_), .O(z08));
  nand2  g062(.a(x05), .b(x04), .O(new_n108_));
  oai21  g063(.a(new_n108_), .b(x07), .c(new_n47_), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(new_n51_), .O(new_n110_));
  inv1   g065(.a(new_n110_), .O(z09));
  inv1   g066(.a(x07), .O(new_n112_));
  nand3  g067(.a(new_n108_), .b(new_n60_), .c(new_n112_), .O(new_n113_));
  aoi21  g068(.a(new_n113_), .b(new_n47_), .c(x15), .O(z10));
  xor2a  g069(.a(x18), .b(x17), .O(new_n115_));
  nand3  g070(.a(new_n115_), .b(new_n108_), .c(new_n112_), .O(new_n116_));
  aoi21  g071(.a(new_n116_), .b(new_n47_), .c(x15), .O(z11));
  nor2   g072(.a(new_n61_), .b(new_n60_), .O(new_n118_));
  nand3  g073(.a(new_n62_), .b(x18), .c(x17), .O(new_n119_));
  oai21  g074(.a(new_n118_), .b(new_n62_), .c(new_n119_), .O(new_n120_));
  nand3  g075(.a(new_n120_), .b(new_n108_), .c(new_n112_), .O(new_n121_));
  aoi21  g076(.a(new_n121_), .b(new_n47_), .c(x15), .O(z12));
  nand2  g077(.a(new_n98_), .b(x20), .O(new_n123_));
  nand4  g078(.a(new_n51_), .b(new_n112_), .c(x05), .d(x04), .O(new_n124_));
  nand3  g079(.a(new_n118_), .b(new_n58_), .c(x19), .O(new_n125_));
  nand4  g080(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n112_), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n51_), .O(z13));
  inv1   g083(.a(new_n125_), .O(new_n129_));
  oai21  g084(.a(new_n129_), .b(x07), .c(new_n47_), .O(new_n130_));
  nand3  g085(.a(new_n130_), .b(new_n110_), .c(new_n51_), .O(z14));
  nor2   g086(.a(new_n129_), .b(new_n59_), .O(new_n132_));
  nand3  g087(.a(new_n118_), .b(new_n100_), .c(x19), .O(new_n133_));
  nand3  g088(.a(new_n133_), .b(new_n124_), .c(new_n112_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(new_n132_), .c(new_n47_), .O(new_n135_));
  nand2  g090(.a(new_n135_), .b(new_n51_), .O(z15));
  nand2  g091(.a(new_n133_), .b(x23), .O(new_n137_));
  nand2  g092(.a(new_n47_), .b(x07), .O(new_n138_));
  nor2   g093(.a(x23), .b(x22), .O(new_n139_));
  nand3  g094(.a(new_n139_), .b(new_n99_), .c(new_n58_), .O(new_n140_));
  and2   g095(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(new_n137_), .c(new_n110_), .d(new_n51_), .O(z16));
  nand3  g097(.a(new_n101_), .b(new_n59_), .c(new_n58_), .O(new_n143_));
  inv1   g098(.a(new_n143_), .O(new_n144_));
  nand4  g099(.a(new_n144_), .b(x19), .c(x18), .d(x17), .O(new_n145_));
  nand2  g100(.a(new_n140_), .b(x24), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(new_n145_), .c(new_n124_), .d(new_n112_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n47_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n51_), .O(z17));
  nand2  g104(.a(new_n102_), .b(x25), .O(new_n150_));
  nand3  g105(.a(new_n139_), .b(new_n129_), .c(new_n54_), .O(new_n151_));
  nand4  g106(.a(new_n151_), .b(new_n150_), .c(new_n124_), .d(new_n112_), .O(new_n152_));
  nand2  g107(.a(new_n152_), .b(new_n47_), .O(new_n153_));
  nand2  g108(.a(new_n153_), .b(new_n51_), .O(z18));
endmodule


