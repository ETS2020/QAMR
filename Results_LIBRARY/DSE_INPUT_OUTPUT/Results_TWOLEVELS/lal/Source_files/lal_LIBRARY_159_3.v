// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x07), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n49_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n47_), .b(new_n62_), .O(z02));
  nand3  g18(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(z03));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(new_n47_), .c(new_n46_), .O(new_n71_));
  inv1   g26(.a(new_n71_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g28(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z06));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g31(.a(x24), .O(new_n77_));
  inv1   g32(.a(x22), .O(new_n78_));
  aoi21  g33(.a(new_n55_), .b(new_n51_), .c(new_n78_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(x21), .c(x23), .O(new_n80_));
  aoi21  g35(.a(x15), .b(x07), .c(x25), .O(new_n81_));
  oai21  g36(.a(new_n80_), .b(new_n77_), .c(new_n81_), .O(z08));
  inv1   g37(.a(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n49_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n60_), .c(new_n47_), .O(z09));
  nand4  g40(.a(new_n60_), .b(new_n52_), .c(new_n83_), .d(new_n49_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  xor2a  g42(.a(x18), .b(x17), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n60_), .c(new_n83_), .d(new_n49_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n47_), .O(z11));
  nor2   g45(.a(new_n53_), .b(new_n52_), .O(new_n91_));
  nand3  g46(.a(new_n54_), .b(x18), .c(x17), .O(new_n92_));
  oai21  g47(.a(new_n91_), .b(new_n54_), .c(new_n92_), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n60_), .c(new_n83_), .d(new_n49_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z12));
  nand3  g50(.a(new_n83_), .b(x05), .c(x04), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n83_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nand4  g55(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n49_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n83_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n98_), .O(z13));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand4  g61(.a(new_n91_), .b(new_n106_), .c(new_n51_), .d(x19), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n105_), .c(new_n98_), .d(new_n49_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  inv1   g64(.a(new_n99_), .O(new_n110_));
  nor3   g65(.a(x22), .b(x21), .c(x20), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n109_), .c(new_n98_), .d(new_n49_), .O(z15));
  inv1   g68(.a(x23), .O(new_n114_));
  aoi21  g69(.a(new_n111_), .b(new_n110_), .c(new_n114_), .O(new_n115_));
  nand4  g70(.a(new_n114_), .b(new_n78_), .c(new_n106_), .d(new_n51_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n99_), .c(new_n49_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n115_), .c(new_n83_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n98_), .O(z16));
  nand2  g74(.a(x24), .b(x23), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n96_), .c(new_n83_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n49_), .O(new_n122_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n77_), .O(new_n123_));
  nor2   g78(.a(x22), .b(x21), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n77_), .c(new_n114_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n101_), .c(new_n49_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n123_), .c(new_n83_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n122_), .O(z17));
  oai21  g83(.a(new_n125_), .b(new_n101_), .c(x25), .O(new_n129_));
  inv1   g84(.a(new_n101_), .O(new_n130_));
  nor3   g85(.a(x25), .b(x24), .c(x23), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n124_), .c(new_n130_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n129_), .c(new_n98_), .d(new_n49_), .O(z18));
endmodule


