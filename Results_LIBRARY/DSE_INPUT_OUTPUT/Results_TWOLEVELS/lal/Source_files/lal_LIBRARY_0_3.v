// Benchmark "FAU" written by ABC on Wed Aug 19 19:04:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  nor2   g07(.a(new_n49_), .b(x24), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  and2   g14(.a(x22), .b(x21), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n59_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n53_), .c(new_n52_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nor2   g18(.a(new_n49_), .b(x07), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n49_), .b(new_n66_), .O(z02));
  inv1   g22(.a(new_n62_), .O(z03));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand3  g28(.a(new_n73_), .b(new_n50_), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z04));
  inv1   g30(.a(x13), .O(new_n76_));
  nand3  g31(.a(new_n50_), .b(new_n76_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z06));
  nand3  g35(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x24), .O(new_n82_));
  nand2  g37(.a(new_n52_), .b(new_n82_), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(x23), .c(new_n47_), .O(new_n84_));
  nand4  g39(.a(new_n59_), .b(x24), .c(x22), .d(x21), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n52_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n48_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n84_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  inv1   g44(.a(new_n63_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n47_), .c(new_n89_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand4  g47(.a(new_n63_), .b(new_n55_), .c(new_n47_), .d(new_n89_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n50_), .O(z10));
  xor2a  g49(.a(x18), .b(x17), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n63_), .c(new_n47_), .d(new_n89_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  nor2   g52(.a(new_n56_), .b(new_n55_), .O(new_n98_));
  nand3  g53(.a(new_n57_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n57_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n63_), .c(new_n47_), .d(new_n89_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z12));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n90_), .b(new_n47_), .c(new_n89_), .O(new_n105_));
  nand4  g60(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  and2   g61(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n104_), .c(new_n47_), .d(new_n89_), .O(z13));
  inv1   g63(.a(x21), .O(new_n109_));
  inv1   g64(.a(new_n106_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g66(.a(new_n89_), .b(x05), .c(x04), .O(new_n112_));
  nand4  g67(.a(new_n98_), .b(new_n109_), .c(new_n54_), .d(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n89_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n111_), .c(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n48_), .b(x15), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(z14));
  nand2  g72(.a(new_n113_), .b(x22), .O(new_n118_));
  inv1   g73(.a(new_n103_), .O(new_n119_));
  nor3   g74(.a(x22), .b(x21), .c(x20), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  and2   g76(.a(new_n121_), .b(new_n105_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n118_), .c(new_n47_), .d(new_n89_), .O(z15));
  nand2  g78(.a(new_n121_), .b(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n48_), .O(new_n125_));
  nand2  g80(.a(new_n112_), .b(new_n89_), .O(new_n126_));
  aoi21  g81(.a(new_n121_), .b(x23), .c(new_n126_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(x15), .c(new_n125_), .O(z16));
  inv1   g83(.a(new_n112_), .O(new_n129_));
  aoi21  g84(.a(x24), .b(x23), .c(new_n129_), .O(new_n130_));
  nand2  g85(.a(new_n121_), .b(x24), .O(new_n131_));
  nor2   g86(.a(x22), .b(x21), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n82_), .c(new_n48_), .O(new_n133_));
  inv1   g88(.a(new_n133_), .O(new_n134_));
  aoi21  g89(.a(new_n134_), .b(new_n110_), .c(x07), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n131_), .c(new_n130_), .d(new_n47_), .O(z17));
  aoi21  g91(.a(new_n82_), .b(new_n48_), .c(x15), .O(new_n137_));
  aoi21  g92(.a(new_n120_), .b(new_n119_), .c(x23), .O(new_n138_));
  oai21  g93(.a(new_n138_), .b(new_n137_), .c(x25), .O(new_n139_));
  nand3  g94(.a(new_n132_), .b(new_n52_), .c(new_n82_), .O(new_n140_));
  oai21  g95(.a(new_n140_), .b(new_n106_), .c(new_n47_), .O(new_n141_));
  aoi22  g96(.a(new_n141_), .b(new_n48_), .c(new_n126_), .d(new_n47_), .O(new_n142_));
  nand2  g97(.a(new_n142_), .b(new_n139_), .O(z18));
endmodule


