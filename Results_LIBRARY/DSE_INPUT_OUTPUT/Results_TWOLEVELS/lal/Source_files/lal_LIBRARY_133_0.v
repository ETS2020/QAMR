// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x23), .b(x15), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x15), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n50_), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  inv1   g08(.a(x07), .O(new_n54_));
  oai21  g09(.a(x25), .b(x24), .c(new_n54_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  inv1   g11(.a(x25), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(x22), .c(x21), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n57_), .c(new_n51_), .d(x15), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n56_), .c(new_n53_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n48_), .b(new_n67_), .O(z02));
  inv1   g23(.a(x24), .O(new_n69_));
  nand2  g24(.a(new_n48_), .b(new_n69_), .O(new_n70_));
  nand3  g25(.a(new_n64_), .b(new_n51_), .c(x15), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  xnor2a g29(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n48_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g35(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n48_), .O(z06));
  aoi21  g37(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g38(.a(x22), .O(new_n84_));
  aoi21  g39(.a(new_n62_), .b(new_n58_), .c(new_n84_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x21), .c(x23), .O(new_n86_));
  nor2   g41(.a(new_n47_), .b(x25), .O(new_n87_));
  oai21  g42(.a(new_n86_), .b(new_n69_), .c(new_n87_), .O(z08));
  nand2  g43(.a(x05), .b(x04), .O(new_n89_));
  nor4   g44(.a(new_n89_), .b(new_n51_), .c(x15), .d(x07), .O(z09));
  nand3  g45(.a(new_n89_), .b(new_n59_), .c(new_n54_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(x23), .c(x15), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n89_), .c(x23), .d(new_n50_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(x07), .O(z11));
  nor2   g50(.a(new_n60_), .b(new_n59_), .O(new_n96_));
  nand2  g51(.a(new_n61_), .b(x18), .O(new_n97_));
  oai22  g52(.a(new_n97_), .b(new_n59_), .c(new_n96_), .d(new_n61_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n89_), .c(new_n54_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(x23), .c(x15), .O(z12));
  oai21  g55(.a(new_n89_), .b(x07), .c(x23), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n50_), .O(new_n102_));
  inv1   g57(.a(x21), .O(new_n103_));
  oai21  g58(.a(x23), .b(x22), .c(new_n103_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(x20), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  oai21  g62(.a(new_n51_), .b(new_n54_), .c(new_n50_), .O(new_n108_));
  aoi21  g63(.a(new_n107_), .b(x20), .c(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n106_), .c(new_n102_), .O(z13));
  nor2   g65(.a(new_n104_), .b(x20), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(x19), .c(x18), .d(x17), .O(new_n112_));
  nand3  g67(.a(new_n96_), .b(new_n58_), .c(x19), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(x21), .c(new_n108_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n102_), .O(z14));
  nand3  g70(.a(new_n103_), .b(new_n58_), .c(x19), .O(new_n116_));
  inv1   g71(.a(new_n116_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n96_), .c(new_n84_), .O(new_n118_));
  nand4  g73(.a(new_n50_), .b(new_n54_), .c(x05), .d(x04), .O(new_n119_));
  inv1   g74(.a(new_n107_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n84_), .c(new_n103_), .d(new_n58_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n54_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n118_), .c(x23), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n50_), .O(z15));
  nand2  g79(.a(x23), .b(new_n60_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n97_), .c(x17), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n89_), .O(new_n127_));
  nand3  g82(.a(x23), .b(x05), .c(x04), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  aoi21  g84(.a(new_n129_), .b(new_n54_), .c(new_n51_), .O(new_n130_));
  aoi21  g85(.a(new_n84_), .b(new_n103_), .c(x20), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(x19), .c(x18), .d(x17), .O(new_n132_));
  aoi21  g87(.a(x23), .b(x07), .c(x20), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n132_), .c(new_n130_), .d(new_n50_), .O(z16));
  nand3  g89(.a(new_n119_), .b(new_n69_), .c(new_n54_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(x23), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n50_), .O(z17));
  nand3  g92(.a(new_n119_), .b(new_n57_), .c(new_n54_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(x23), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n50_), .O(z18));
endmodule


