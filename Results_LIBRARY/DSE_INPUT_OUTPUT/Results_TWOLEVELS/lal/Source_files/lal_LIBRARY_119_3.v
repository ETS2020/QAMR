// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x16), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n50_), .c(new_n52_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n50_), .c(new_n62_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n61_), .O(z01));
  nor2   g21(.a(new_n48_), .b(x15), .O(z02));
  inv1   g22(.a(new_n61_), .O(z03));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n50_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g31(.a(new_n50_), .b(x14), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g34(.a(x24), .O(new_n80_));
  inv1   g35(.a(x22), .O(new_n81_));
  aoi21  g36(.a(new_n57_), .b(new_n53_), .c(new_n81_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x21), .c(x23), .O(new_n83_));
  nor2   g38(.a(new_n49_), .b(x25), .O(new_n84_));
  oai21  g39(.a(new_n83_), .b(new_n80_), .c(new_n84_), .O(z08));
  inv1   g40(.a(new_n63_), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n47_), .c(new_n62_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n50_), .O(z09));
  nand4  g43(.a(new_n63_), .b(new_n54_), .c(new_n47_), .d(new_n62_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n63_), .c(new_n47_), .d(new_n62_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n50_), .O(z11));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nand3  g50(.a(new_n56_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n56_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n63_), .c(new_n47_), .d(new_n62_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n62_), .b(x05), .c(x04), .O(new_n102_));
  nand4  g57(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .d(new_n62_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(new_n105_));
  nand2  g60(.a(new_n48_), .b(x15), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n105_), .O(z13));
  inv1   g62(.a(x21), .O(new_n108_));
  nor2   g63(.a(x20), .b(new_n56_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n95_), .c(new_n108_), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n95_), .c(x19), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n102_), .c(new_n62_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n110_), .c(new_n47_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n106_), .O(z14));
  nand3  g70(.a(new_n108_), .b(new_n53_), .c(x19), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n94_), .c(x22), .O(new_n117_));
  inv1   g72(.a(new_n100_), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n53_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n117_), .c(new_n102_), .d(new_n62_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n106_), .O(z15));
  inv1   g78(.a(x23), .O(new_n124_));
  nand4  g79(.a(new_n111_), .b(new_n118_), .c(new_n124_), .d(new_n81_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n87_), .c(new_n62_), .O(new_n126_));
  aoi21  g81(.a(new_n120_), .b(x23), .c(new_n126_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n47_), .O(z16));
  nand2  g83(.a(new_n125_), .b(x24), .O(new_n129_));
  inv1   g84(.a(new_n87_), .O(new_n130_));
  nand4  g85(.a(new_n80_), .b(new_n124_), .c(new_n81_), .d(new_n108_), .O(new_n131_));
  nor2   g86(.a(new_n131_), .b(new_n103_), .O(new_n132_));
  nor2   g87(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n129_), .c(new_n47_), .d(new_n62_), .O(z17));
  oai21  g89(.a(new_n131_), .b(new_n103_), .c(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n119_), .c(new_n109_), .d(new_n95_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n102_), .d(new_n62_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n106_), .O(z18));
endmodule


