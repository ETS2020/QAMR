// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x08), .O(new_n46_));
  nor2   g001(.a(x16), .b(new_n46_), .O(z00));
  inv1   g002(.a(x15), .O(new_n48_));
  oai21  g003(.a(new_n46_), .b(x07), .c(new_n48_), .O(new_n49_));
  nand3  g004(.a(new_n49_), .b(x05), .c(x04), .O(new_n50_));
  nor2   g005(.a(x15), .b(x08), .O(new_n51_));
  inv1   g006(.a(x25), .O(new_n52_));
  inv1   g007(.a(x20), .O(new_n53_));
  inv1   g008(.a(x17), .O(new_n54_));
  inv1   g009(.a(x18), .O(new_n55_));
  inv1   g010(.a(x19), .O(new_n56_));
  nand3  g011(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g012(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g014(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  aoi21  g015(.a(new_n60_), .b(new_n52_), .c(x07), .O(new_n61_));
  oai21  g016(.a(new_n61_), .b(new_n51_), .c(new_n50_), .O(z01));
  inv1   g017(.a(x16), .O(new_n63_));
  nor2   g018(.a(new_n51_), .b(new_n63_), .O(z02));
  inv1   g019(.a(new_n51_), .O(new_n65_));
  nand3  g020(.a(new_n60_), .b(new_n65_), .c(new_n52_), .O(new_n66_));
  nand2  g021(.a(new_n66_), .b(new_n65_), .O(z03));
  xnor2a g022(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g023(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g024(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g025(.a(x12), .b(x03), .O(new_n71_));
  nand4  g026(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand3  g027(.a(new_n72_), .b(x15), .c(new_n46_), .O(new_n73_));
  inv1   g028(.a(new_n73_), .O(z04));
  inv1   g029(.a(x13), .O(new_n75_));
  nand3  g030(.a(x15), .b(new_n75_), .c(new_n46_), .O(new_n76_));
  inv1   g031(.a(new_n76_), .O(z05));
  nand3  g032(.a(x15), .b(x14), .c(new_n46_), .O(new_n78_));
  inv1   g033(.a(new_n78_), .O(z06));
  nand3  g034(.a(x15), .b(new_n46_), .c(x06), .O(z07));
  nand2  g035(.a(new_n56_), .b(new_n55_), .O(new_n81_));
  nand3  g036(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  oai21  g037(.a(new_n82_), .b(new_n81_), .c(new_n52_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(new_n54_), .O(new_n84_));
  nand2  g039(.a(new_n82_), .b(new_n52_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g041(.a(x24), .O(new_n87_));
  nand2  g042(.a(new_n52_), .b(new_n87_), .O(new_n88_));
  nand2  g043(.a(new_n88_), .b(x23), .O(new_n89_));
  inv1   g044(.a(x22), .O(new_n90_));
  inv1   g045(.a(x21), .O(new_n91_));
  nand3  g046(.a(new_n91_), .b(x19), .c(x18), .O(new_n92_));
  inv1   g047(.a(new_n92_), .O(new_n93_));
  nor2   g048(.a(new_n55_), .b(new_n54_), .O(new_n94_));
  nor2   g049(.a(x20), .b(new_n56_), .O(new_n95_));
  nor2   g050(.a(x22), .b(x21), .O(new_n96_));
  nor2   g051(.a(x24), .b(x23), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n93_), .c(new_n87_), .d(new_n90_), .O(new_n99_));
  aoi21  g054(.a(new_n99_), .b(x25), .c(new_n51_), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n89_), .c(new_n86_), .d(new_n84_), .O(z08));
  inv1   g056(.a(x07), .O(new_n102_));
  and2   g057(.a(x05), .b(x04), .O(new_n103_));
  aoi21  g058(.a(new_n103_), .b(new_n102_), .c(new_n46_), .O(new_n104_));
  nor2   g059(.a(new_n104_), .b(x15), .O(z09));
  inv1   g060(.a(new_n103_), .O(new_n106_));
  nand4  g061(.a(new_n106_), .b(new_n54_), .c(new_n48_), .d(x08), .O(new_n107_));
  nor2   g062(.a(new_n107_), .b(x07), .O(z10));
  xor2a  g063(.a(x18), .b(x17), .O(new_n109_));
  nand3  g064(.a(new_n109_), .b(new_n106_), .c(new_n102_), .O(new_n110_));
  aoi21  g065(.a(new_n110_), .b(x08), .c(x15), .O(z11));
  nand3  g066(.a(new_n56_), .b(x18), .c(x17), .O(new_n112_));
  oai21  g067(.a(new_n94_), .b(new_n56_), .c(new_n112_), .O(new_n113_));
  nand4  g068(.a(new_n113_), .b(new_n106_), .c(new_n48_), .d(x08), .O(new_n114_));
  nor2   g069(.a(new_n114_), .b(x07), .O(z12));
  nand2  g070(.a(new_n95_), .b(new_n94_), .O(new_n116_));
  nand3  g071(.a(x19), .b(x18), .c(x17), .O(new_n117_));
  nand2  g072(.a(new_n117_), .b(x20), .O(new_n118_));
  nand3  g073(.a(new_n103_), .b(new_n48_), .c(new_n102_), .O(new_n119_));
  nand4  g074(.a(new_n119_), .b(new_n118_), .c(new_n116_), .d(new_n102_), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(x08), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n48_), .O(z13));
  nand2  g077(.a(new_n119_), .b(new_n102_), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(x08), .O(new_n124_));
  nor2   g079(.a(x21), .b(x20), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n94_), .c(x19), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n48_), .c(x08), .O(new_n127_));
  aoi21  g082(.a(new_n116_), .b(x21), .c(new_n127_), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n124_), .O(z14));
  nand2  g084(.a(new_n126_), .b(x22), .O(new_n130_));
  inv1   g085(.a(new_n117_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n96_), .c(new_n53_), .O(new_n132_));
  and2   g087(.a(new_n132_), .b(new_n65_), .O(new_n133_));
  nand4  g088(.a(new_n133_), .b(new_n130_), .c(new_n124_), .d(new_n48_), .O(z15));
  nor2   g089(.a(x23), .b(x22), .O(new_n135_));
  nand3  g090(.a(new_n135_), .b(new_n125_), .c(new_n131_), .O(new_n136_));
  nand3  g091(.a(new_n136_), .b(new_n48_), .c(x08), .O(new_n137_));
  aoi21  g092(.a(new_n132_), .b(x23), .c(new_n137_), .O(new_n138_));
  nand2  g093(.a(new_n138_), .b(new_n124_), .O(z16));
  nand3  g094(.a(new_n97_), .b(new_n90_), .c(new_n91_), .O(new_n140_));
  nor2   g095(.a(new_n140_), .b(x20), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(x19), .c(x18), .d(x17), .O(new_n142_));
  nand2  g097(.a(new_n48_), .b(x08), .O(new_n143_));
  aoi21  g098(.a(new_n136_), .b(x24), .c(new_n143_), .O(new_n144_));
  nand3  g099(.a(new_n144_), .b(new_n142_), .c(new_n124_), .O(z17));
  nand2  g100(.a(new_n98_), .b(x25), .O(new_n146_));
  inv1   g101(.a(new_n116_), .O(new_n147_));
  inv1   g102(.a(new_n96_), .O(new_n148_));
  nor3   g103(.a(new_n148_), .b(new_n88_), .c(x23), .O(new_n149_));
  aoi21  g104(.a(new_n149_), .b(new_n147_), .c(new_n51_), .O(new_n150_));
  nand4  g105(.a(new_n150_), .b(new_n146_), .c(new_n124_), .d(new_n48_), .O(z18));
endmodule


