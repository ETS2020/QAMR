// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  inv1   g002(.a(x15), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(x07), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g006(.a(z02), .b(new_n46_), .O(z00));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  inv1   g009(.a(x18), .O(new_n55_));
  inv1   g010(.a(x19), .O(new_n56_));
  nand2  g011(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g012(.a(new_n57_), .b(x17), .c(new_n54_), .O(new_n58_));
  inv1   g013(.a(x21), .O(new_n59_));
  inv1   g014(.a(x22), .O(new_n60_));
  nor2   g015(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  and2   g016(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(x23), .c(x24), .O(new_n63_));
  nand3  g018(.a(new_n63_), .b(new_n50_), .c(new_n53_), .O(new_n64_));
  inv1   g019(.a(x23), .O(new_n65_));
  inv1   g020(.a(x24), .O(new_n66_));
  nand3  g021(.a(new_n58_), .b(x22), .c(x21), .O(new_n67_));
  aoi21  g022(.a(new_n67_), .b(new_n65_), .c(new_n66_), .O(new_n68_));
  oai21  g023(.a(new_n68_), .b(x25), .c(x07), .O(new_n69_));
  inv1   g024(.a(x07), .O(new_n70_));
  inv1   g025(.a(x04), .O(new_n71_));
  inv1   g026(.a(x05), .O(new_n72_));
  nor2   g027(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g028(.a(new_n73_), .b(new_n48_), .c(new_n70_), .O(new_n74_));
  nand3  g029(.a(new_n74_), .b(new_n69_), .c(new_n64_), .O(z01));
  inv1   g030(.a(new_n64_), .O(z03));
  xnor2a g031(.a(x09), .b(x00), .O(new_n77_));
  xnor2a g032(.a(x10), .b(x01), .O(new_n78_));
  xnor2a g033(.a(x11), .b(x02), .O(new_n79_));
  xnor2a g034(.a(x12), .b(x03), .O(new_n80_));
  nand4  g035(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n77_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n46_), .O(new_n82_));
  nand2  g037(.a(new_n82_), .b(new_n50_), .O(z04));
  oai21  g038(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g039(.a(x14), .O(new_n85_));
  nor3   g040(.a(new_n49_), .b(new_n85_), .c(x08), .O(z06));
  nand3  g041(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g042(.a(x17), .O(new_n88_));
  nand3  g043(.a(x24), .b(x22), .c(x21), .O(new_n89_));
  oai21  g044(.a(new_n89_), .b(new_n57_), .c(new_n53_), .O(new_n90_));
  nand2  g045(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g046(.a(new_n89_), .b(new_n53_), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(x20), .O(new_n93_));
  oai21  g048(.a(x25), .b(x24), .c(x23), .O(new_n94_));
  nand3  g049(.a(new_n59_), .b(x19), .c(x18), .O(new_n95_));
  inv1   g050(.a(new_n95_), .O(new_n96_));
  nand4  g051(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n97_));
  inv1   g052(.a(new_n97_), .O(new_n98_));
  nor2   g053(.a(x22), .b(x21), .O(new_n99_));
  nand4  g054(.a(new_n99_), .b(new_n98_), .c(new_n66_), .d(new_n65_), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n96_), .c(new_n66_), .d(new_n60_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(x25), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n94_), .c(new_n93_), .d(new_n91_), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(new_n69_), .O(z08));
  nand3  g060(.a(new_n73_), .b(new_n48_), .c(new_n70_), .O(new_n106_));
  inv1   g061(.a(new_n106_), .O(z09));
  inv1   g062(.a(new_n73_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n88_), .O(new_n109_));
  aoi21  g064(.a(new_n109_), .b(new_n48_), .c(x07), .O(z10));
  xor2a  g065(.a(x18), .b(x17), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n108_), .O(new_n112_));
  aoi21  g067(.a(new_n112_), .b(new_n48_), .c(x07), .O(z11));
  oai21  g068(.a(new_n55_), .b(new_n88_), .c(x19), .O(new_n114_));
  nand3  g069(.a(new_n56_), .b(x18), .c(x17), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g071(.a(new_n116_), .b(new_n108_), .c(new_n48_), .d(new_n70_), .O(new_n117_));
  inv1   g072(.a(new_n117_), .O(z12));
  and2   g073(.a(x19), .b(x18), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n48_), .O(new_n120_));
  nand3  g075(.a(new_n53_), .b(new_n65_), .c(x07), .O(new_n121_));
  aoi21  g076(.a(new_n121_), .b(new_n120_), .c(new_n88_), .O(new_n122_));
  nand4  g077(.a(new_n57_), .b(new_n53_), .c(new_n65_), .d(x07), .O(new_n123_));
  inv1   g078(.a(new_n123_), .O(new_n124_));
  oai21  g079(.a(new_n124_), .b(new_n122_), .c(new_n54_), .O(new_n125_));
  nor2   g080(.a(new_n54_), .b(x15), .O(new_n126_));
  nor3   g081(.a(new_n89_), .b(new_n57_), .c(new_n70_), .O(new_n127_));
  oai21  g082(.a(new_n127_), .b(new_n126_), .c(new_n88_), .O(new_n128_));
  inv1   g083(.a(new_n61_), .O(new_n129_));
  nand2  g084(.a(new_n129_), .b(new_n65_), .O(new_n130_));
  aoi21  g085(.a(new_n61_), .b(x20), .c(x23), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n130_), .c(new_n53_), .d(x24), .O(new_n132_));
  oai21  g087(.a(new_n56_), .b(new_n55_), .c(x20), .O(new_n133_));
  nand3  g088(.a(new_n70_), .b(x05), .c(x04), .O(new_n134_));
  aoi21  g089(.a(new_n134_), .b(new_n133_), .c(x15), .O(new_n135_));
  aoi21  g090(.a(new_n132_), .b(x07), .c(new_n135_), .O(new_n136_));
  nand3  g091(.a(new_n136_), .b(new_n128_), .c(new_n125_), .O(z13));
  oai21  g092(.a(new_n72_), .b(new_n71_), .c(new_n48_), .O(new_n138_));
  nand2  g093(.a(new_n138_), .b(new_n70_), .O(new_n139_));
  nand2  g094(.a(new_n121_), .b(new_n95_), .O(new_n140_));
  nand3  g095(.a(new_n140_), .b(new_n54_), .c(x17), .O(new_n141_));
  nand4  g096(.a(new_n53_), .b(x24), .c(new_n65_), .d(x21), .O(new_n142_));
  aoi22  g097(.a(new_n142_), .b(x07), .c(new_n97_), .d(x21), .O(new_n143_));
  nand3  g098(.a(new_n143_), .b(new_n141_), .c(new_n139_), .O(z14));
  nand4  g099(.a(new_n99_), .b(x19), .c(x18), .d(new_n48_), .O(new_n145_));
  aoi21  g100(.a(new_n145_), .b(new_n121_), .c(new_n88_), .O(new_n146_));
  oai21  g101(.a(new_n146_), .b(new_n124_), .c(new_n54_), .O(new_n147_));
  nand3  g102(.a(new_n58_), .b(x24), .c(x07), .O(new_n148_));
  aoi21  g103(.a(new_n148_), .b(x15), .c(new_n59_), .O(new_n149_));
  nor2   g104(.a(new_n98_), .b(x15), .O(new_n150_));
  oai21  g105(.a(new_n150_), .b(new_n149_), .c(x22), .O(new_n151_));
  nand4  g106(.a(new_n61_), .b(new_n53_), .c(x24), .d(new_n65_), .O(new_n152_));
  nand2  g107(.a(new_n152_), .b(x07), .O(new_n153_));
  nand4  g108(.a(new_n153_), .b(new_n151_), .c(new_n147_), .d(new_n74_), .O(z15));
  oai21  g109(.a(x25), .b(new_n70_), .c(new_n145_), .O(new_n155_));
  nand2  g110(.a(new_n155_), .b(x17), .O(new_n156_));
  nand3  g111(.a(new_n57_), .b(new_n53_), .c(x07), .O(new_n157_));
  aoi21  g112(.a(new_n157_), .b(new_n156_), .c(x20), .O(new_n158_));
  nand3  g113(.a(new_n129_), .b(new_n53_), .c(x07), .O(new_n159_));
  inv1   g114(.a(new_n159_), .O(new_n160_));
  oai21  g115(.a(new_n160_), .b(new_n158_), .c(new_n65_), .O(new_n161_));
  nor2   g116(.a(new_n65_), .b(x15), .O(new_n162_));
  oai21  g117(.a(new_n162_), .b(new_n127_), .c(new_n88_), .O(new_n163_));
  nand4  g118(.a(x24), .b(x22), .c(x21), .d(x07), .O(new_n164_));
  inv1   g119(.a(new_n164_), .O(new_n165_));
  oai21  g120(.a(new_n165_), .b(new_n162_), .c(x20), .O(new_n166_));
  nand3  g121(.a(new_n53_), .b(x24), .c(new_n65_), .O(new_n167_));
  nand2  g122(.a(new_n167_), .b(x07), .O(new_n168_));
  inv1   g123(.a(new_n134_), .O(new_n169_));
  aoi21  g124(.a(new_n119_), .b(new_n99_), .c(new_n65_), .O(new_n170_));
  oai21  g125(.a(new_n170_), .b(new_n169_), .c(new_n48_), .O(new_n171_));
  nand4  g126(.a(new_n171_), .b(new_n168_), .c(new_n166_), .d(new_n163_), .O(new_n172_));
  inv1   g127(.a(new_n172_), .O(new_n173_));
  nand2  g128(.a(new_n173_), .b(new_n161_), .O(z16));
  nand4  g129(.a(new_n61_), .b(new_n56_), .c(new_n55_), .d(x07), .O(new_n175_));
  aoi21  g130(.a(new_n175_), .b(x15), .c(x17), .O(new_n176_));
  nand3  g131(.a(new_n54_), .b(x19), .c(x18), .O(new_n177_));
  nand3  g132(.a(new_n65_), .b(new_n60_), .c(new_n59_), .O(new_n178_));
  oai21  g133(.a(new_n178_), .b(new_n177_), .c(new_n48_), .O(new_n179_));
  oai21  g134(.a(new_n131_), .b(new_n70_), .c(new_n179_), .O(new_n180_));
  oai21  g135(.a(new_n180_), .b(new_n176_), .c(x24), .O(new_n181_));
  nand3  g136(.a(new_n119_), .b(x17), .c(new_n48_), .O(new_n182_));
  nor3   g137(.a(x24), .b(x23), .c(x22), .O(new_n183_));
  nand3  g138(.a(new_n183_), .b(new_n59_), .c(new_n54_), .O(new_n184_));
  oai21  g139(.a(new_n184_), .b(new_n182_), .c(new_n70_), .O(new_n185_));
  nand2  g140(.a(new_n185_), .b(x25), .O(new_n186_));
  nand4  g141(.a(new_n99_), .b(new_n53_), .c(new_n66_), .d(new_n65_), .O(new_n187_));
  oai21  g142(.a(new_n187_), .b(new_n97_), .c(new_n134_), .O(new_n188_));
  nand2  g143(.a(new_n188_), .b(new_n48_), .O(new_n189_));
  nand3  g144(.a(new_n63_), .b(new_n53_), .c(x07), .O(new_n190_));
  nand4  g145(.a(new_n190_), .b(new_n189_), .c(new_n186_), .d(new_n181_), .O(z17));
  nor2   g146(.a(x24), .b(x22), .O(new_n192_));
  nand4  g147(.a(new_n192_), .b(new_n119_), .c(new_n59_), .d(new_n48_), .O(new_n193_));
  aoi21  g148(.a(new_n193_), .b(new_n70_), .c(new_n88_), .O(new_n194_));
  aoi21  g149(.a(new_n56_), .b(new_n55_), .c(new_n70_), .O(new_n195_));
  oai21  g150(.a(new_n195_), .b(new_n194_), .c(new_n54_), .O(new_n196_));
  nand2  g151(.a(new_n129_), .b(x07), .O(new_n197_));
  aoi21  g152(.a(new_n197_), .b(new_n196_), .c(x23), .O(new_n198_));
  nor2   g153(.a(x24), .b(new_n70_), .O(new_n199_));
  oai21  g154(.a(new_n199_), .b(new_n198_), .c(new_n53_), .O(new_n200_));
  nor2   g155(.a(new_n53_), .b(x15), .O(new_n201_));
  oai21  g156(.a(new_n201_), .b(new_n127_), .c(new_n88_), .O(new_n202_));
  oai21  g157(.a(new_n201_), .b(new_n165_), .c(x20), .O(new_n203_));
  oai21  g158(.a(new_n66_), .b(new_n65_), .c(new_n53_), .O(new_n204_));
  nand2  g159(.a(new_n204_), .b(x07), .O(new_n205_));
  nand2  g160(.a(new_n183_), .b(new_n96_), .O(new_n206_));
  nand3  g161(.a(new_n206_), .b(x25), .c(new_n48_), .O(new_n207_));
  nand4  g162(.a(new_n207_), .b(new_n205_), .c(new_n203_), .d(new_n202_), .O(new_n208_));
  inv1   g163(.a(new_n208_), .O(new_n209_));
  nand3  g164(.a(new_n209_), .b(new_n200_), .c(new_n139_), .O(z18));
endmodule


