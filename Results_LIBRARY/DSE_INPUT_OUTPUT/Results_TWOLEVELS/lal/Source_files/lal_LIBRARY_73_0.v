// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x15), .b(x13), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x13), .O(new_n52_));
  inv1   g07(.a(x15), .O(new_n53_));
  oai21  g08(.a(new_n52_), .b(x07), .c(new_n53_), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(x05), .c(x04), .O(new_n55_));
  inv1   g10(.a(x25), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(x22), .b(x21), .O(new_n62_));
  aoi21  g17(.a(new_n61_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x23), .c(x24), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n56_), .c(x07), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(new_n48_), .c(new_n55_), .O(z01));
  nand3  g21(.a(new_n64_), .b(new_n49_), .c(new_n56_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n49_), .O(z03));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n49_), .O(z04));
  nand3  g30(.a(x15), .b(new_n52_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g35(.a(x24), .O(new_n81_));
  inv1   g36(.a(x22), .O(new_n82_));
  aoi21  g37(.a(new_n61_), .b(new_n57_), .c(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x21), .c(x23), .O(new_n84_));
  nor2   g39(.a(new_n48_), .b(x25), .O(new_n85_));
  oai21  g40(.a(new_n84_), .b(new_n81_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  nand2  g42(.a(x05), .b(x04), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n87_), .c(new_n52_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x15), .O(z09));
  nand3  g46(.a(new_n88_), .b(new_n58_), .c(new_n87_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(x13), .c(x15), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n88_), .c(new_n53_), .d(x13), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z11));
  nand3  g51(.a(new_n60_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n60_), .b(x17), .c(new_n97_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n88_), .O(new_n99_));
  oai21  g54(.a(new_n52_), .b(x05), .c(x04), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(x19), .c(new_n59_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n87_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(x13), .c(x15), .O(z12));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n89_), .b(new_n53_), .c(new_n87_), .O(new_n107_));
  nand4  g62(.a(new_n57_), .b(x19), .c(x18), .d(x17), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n87_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x13), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n53_), .O(z13));
  nand2  g66(.a(new_n107_), .b(new_n87_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x13), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(x19), .c(x18), .d(x17), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n53_), .c(x13), .O(new_n116_));
  aoi21  g71(.a(new_n108_), .b(x21), .c(new_n116_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n113_), .O(z14));
  nand2  g73(.a(new_n115_), .b(x22), .O(new_n119_));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n57_), .O(new_n121_));
  nor2   g76(.a(new_n121_), .b(new_n105_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(new_n48_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n119_), .c(new_n113_), .d(new_n53_), .O(z15));
  inv1   g79(.a(x23), .O(new_n125_));
  nor2   g80(.a(new_n122_), .b(new_n125_), .O(new_n126_));
  inv1   g81(.a(new_n105_), .O(new_n127_));
  nand4  g82(.a(new_n114_), .b(new_n127_), .c(new_n125_), .d(new_n82_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n107_), .c(new_n87_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n126_), .c(x13), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n53_), .O(z16));
  nand2  g86(.a(new_n128_), .b(x24), .O(new_n132_));
  inv1   g87(.a(new_n108_), .O(new_n133_));
  nand3  g88(.a(new_n120_), .b(new_n81_), .c(new_n125_), .O(new_n134_));
  inv1   g89(.a(new_n134_), .O(new_n135_));
  aoi21  g90(.a(new_n135_), .b(new_n133_), .c(new_n48_), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n132_), .c(new_n113_), .d(new_n53_), .O(z17));
  oai21  g92(.a(new_n134_), .b(new_n108_), .c(x25), .O(new_n138_));
  nor3   g93(.a(x25), .b(x24), .c(x23), .O(new_n139_));
  nand3  g94(.a(new_n139_), .b(new_n120_), .c(new_n133_), .O(new_n140_));
  nand4  g95(.a(new_n140_), .b(new_n138_), .c(new_n107_), .d(new_n87_), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(x13), .O(new_n142_));
  nand2  g97(.a(new_n142_), .b(new_n53_), .O(z18));
endmodule


