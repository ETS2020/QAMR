// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  nor3   g002(.a(x16), .b(new_n47_), .c(new_n46_), .O(z00));
  oai21  g003(.a(x08), .b(x07), .c(new_n47_), .O(new_n49_));
  nand3  g004(.a(new_n49_), .b(x05), .c(x04), .O(new_n50_));
  nor2   g005(.a(x15), .b(new_n46_), .O(new_n51_));
  inv1   g006(.a(x25), .O(new_n52_));
  inv1   g007(.a(x20), .O(new_n53_));
  inv1   g008(.a(x17), .O(new_n54_));
  inv1   g009(.a(x18), .O(new_n55_));
  inv1   g010(.a(x19), .O(new_n56_));
  nand3  g011(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g012(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g014(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  aoi21  g015(.a(new_n60_), .b(new_n52_), .c(x07), .O(new_n61_));
  oai21  g016(.a(new_n61_), .b(new_n51_), .c(new_n50_), .O(z01));
  inv1   g017(.a(x16), .O(new_n63_));
  aoi21  g018(.a(new_n47_), .b(x08), .c(new_n63_), .O(z02));
  inv1   g019(.a(new_n51_), .O(new_n65_));
  nand3  g020(.a(new_n60_), .b(new_n65_), .c(new_n52_), .O(new_n66_));
  nand2  g021(.a(new_n66_), .b(new_n65_), .O(z03));
  xor2a  g022(.a(x09), .b(x00), .O(new_n68_));
  xor2a  g023(.a(x10), .b(x01), .O(new_n69_));
  nor2   g024(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  xor2a  g025(.a(x11), .b(x02), .O(new_n71_));
  xor2a  g026(.a(x12), .b(x03), .O(new_n72_));
  nor2   g027(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g028(.a(new_n73_), .b(new_n70_), .c(x08), .O(z04));
  nor2   g029(.a(x13), .b(x08), .O(z05));
  inv1   g030(.a(x14), .O(new_n76_));
  nor2   g031(.a(new_n76_), .b(x08), .O(z06));
  nand2  g032(.a(new_n46_), .b(x06), .O(z07));
  nand2  g033(.a(new_n56_), .b(new_n55_), .O(new_n79_));
  nand3  g034(.a(x24), .b(x22), .c(x21), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n79_), .c(new_n52_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n54_), .O(new_n82_));
  nand2  g037(.a(new_n80_), .b(new_n52_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(x20), .O(new_n84_));
  inv1   g039(.a(x24), .O(new_n85_));
  nand2  g040(.a(new_n52_), .b(new_n85_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(x23), .O(new_n87_));
  inv1   g042(.a(x22), .O(new_n88_));
  inv1   g043(.a(x21), .O(new_n89_));
  nand3  g044(.a(new_n89_), .b(x19), .c(x18), .O(new_n90_));
  inv1   g045(.a(new_n90_), .O(new_n91_));
  nor2   g046(.a(new_n55_), .b(new_n54_), .O(new_n92_));
  nor2   g047(.a(x20), .b(new_n56_), .O(new_n93_));
  nor2   g048(.a(x22), .b(x21), .O(new_n94_));
  nor2   g049(.a(x24), .b(x23), .O(new_n95_));
  nand4  g050(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n96_));
  nand4  g051(.a(new_n96_), .b(new_n91_), .c(new_n85_), .d(new_n88_), .O(new_n97_));
  aoi21  g052(.a(new_n97_), .b(x25), .c(new_n51_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n87_), .c(new_n84_), .d(new_n82_), .O(z08));
  inv1   g054(.a(x07), .O(new_n100_));
  and2   g055(.a(x05), .b(x04), .O(new_n101_));
  aoi21  g056(.a(new_n101_), .b(new_n100_), .c(x08), .O(new_n102_));
  nor2   g057(.a(new_n102_), .b(x15), .O(z09));
  inv1   g058(.a(new_n101_), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n54_), .c(new_n47_), .d(new_n46_), .O(new_n105_));
  nor2   g060(.a(new_n105_), .b(x07), .O(z10));
  xor2a  g061(.a(x18), .b(x17), .O(new_n107_));
  nand3  g062(.a(new_n107_), .b(new_n104_), .c(new_n100_), .O(new_n108_));
  aoi21  g063(.a(new_n108_), .b(new_n46_), .c(x15), .O(z11));
  nand3  g064(.a(new_n56_), .b(x18), .c(x17), .O(new_n110_));
  oai21  g065(.a(new_n92_), .b(new_n56_), .c(new_n110_), .O(new_n111_));
  nand4  g066(.a(new_n111_), .b(new_n104_), .c(new_n47_), .d(new_n46_), .O(new_n112_));
  nor2   g067(.a(new_n112_), .b(x07), .O(z12));
  nand2  g068(.a(new_n93_), .b(new_n92_), .O(new_n114_));
  nand3  g069(.a(x19), .b(x18), .c(x17), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(x20), .O(new_n116_));
  nand3  g071(.a(new_n101_), .b(new_n47_), .c(new_n100_), .O(new_n117_));
  nand4  g072(.a(new_n117_), .b(new_n116_), .c(new_n114_), .d(new_n100_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n46_), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n47_), .O(z13));
  nand2  g075(.a(new_n117_), .b(new_n100_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n46_), .O(new_n122_));
  nor2   g077(.a(x21), .b(x20), .O(new_n123_));
  nand3  g078(.a(new_n123_), .b(new_n92_), .c(x19), .O(new_n124_));
  nand3  g079(.a(new_n124_), .b(new_n47_), .c(new_n46_), .O(new_n125_));
  aoi21  g080(.a(new_n114_), .b(x21), .c(new_n125_), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n122_), .O(z14));
  nand2  g082(.a(new_n124_), .b(x22), .O(new_n128_));
  inv1   g083(.a(new_n115_), .O(new_n129_));
  nand3  g084(.a(new_n129_), .b(new_n94_), .c(new_n53_), .O(new_n130_));
  and2   g085(.a(new_n130_), .b(new_n65_), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n128_), .c(new_n122_), .d(new_n47_), .O(z15));
  nor2   g087(.a(x23), .b(x22), .O(new_n133_));
  nand3  g088(.a(new_n133_), .b(new_n123_), .c(new_n129_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n47_), .c(new_n46_), .O(new_n135_));
  aoi21  g090(.a(new_n130_), .b(x23), .c(new_n135_), .O(new_n136_));
  nand2  g091(.a(new_n136_), .b(new_n122_), .O(z16));
  nand3  g092(.a(new_n95_), .b(new_n88_), .c(new_n89_), .O(new_n138_));
  nor2   g093(.a(new_n138_), .b(x20), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(x19), .c(x18), .d(x17), .O(new_n140_));
  nand2  g095(.a(new_n47_), .b(new_n46_), .O(new_n141_));
  aoi21  g096(.a(new_n134_), .b(x24), .c(new_n141_), .O(new_n142_));
  nand3  g097(.a(new_n142_), .b(new_n140_), .c(new_n122_), .O(z17));
  nand2  g098(.a(new_n96_), .b(x25), .O(new_n144_));
  inv1   g099(.a(new_n114_), .O(new_n145_));
  inv1   g100(.a(new_n94_), .O(new_n146_));
  nor3   g101(.a(new_n146_), .b(new_n86_), .c(x23), .O(new_n147_));
  aoi21  g102(.a(new_n147_), .b(new_n145_), .c(new_n51_), .O(new_n148_));
  nand4  g103(.a(new_n148_), .b(new_n144_), .c(new_n122_), .d(new_n47_), .O(z18));
endmodule


