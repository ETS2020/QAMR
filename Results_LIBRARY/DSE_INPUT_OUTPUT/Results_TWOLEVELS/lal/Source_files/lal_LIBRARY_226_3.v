// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  oai21  g06(.a(x15), .b(x07), .c(x24), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  nand2  g08(.a(new_n50_), .b(x07), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  nor3   g11(.a(x19), .b(x18), .c(x17), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  oai21  g15(.a(new_n57_), .b(x20), .c(new_n60_), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n56_), .c(new_n47_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n54_), .c(new_n53_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n49_), .b(new_n66_), .O(z02));
  inv1   g22(.a(new_n64_), .O(z03));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand3  g28(.a(new_n73_), .b(new_n50_), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n50_), .O(z06));
  nand3  g33(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g34(.a(x20), .O(new_n80_));
  inv1   g35(.a(new_n57_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n80_), .c(new_n59_), .O(new_n82_));
  nand2  g37(.a(new_n56_), .b(new_n47_), .O(new_n83_));
  aoi21  g38(.a(new_n82_), .b(x21), .c(new_n83_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n48_), .c(new_n55_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n47_), .b(new_n86_), .c(x05), .d(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  inv1   g43(.a(x17), .O(new_n89_));
  nand2  g44(.a(x05), .b(x04), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n89_), .c(new_n47_), .d(new_n86_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n50_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n90_), .c(new_n47_), .d(new_n86_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n50_), .O(z11));
  inv1   g50(.a(x19), .O(new_n96_));
  and2   g51(.a(x18), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n96_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n90_), .c(new_n47_), .d(new_n86_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n50_), .O(z12));
  nand3  g56(.a(new_n86_), .b(x05), .c(x04), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n86_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nor2   g61(.a(x20), .b(new_n96_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n97_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n104_), .d(new_n47_), .O(z13));
  inv1   g64(.a(new_n108_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n58_), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n97_), .c(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n102_), .c(new_n86_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n111_), .c(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n48_), .b(x15), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(z14));
  inv1   g72(.a(new_n105_), .O(new_n118_));
  nor3   g73(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n102_), .c(new_n86_), .O(new_n121_));
  aoi21  g76(.a(new_n113_), .b(x22), .c(new_n121_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(x15), .c(new_n116_), .O(z15));
  aoi21  g78(.a(new_n119_), .b(new_n118_), .c(new_n56_), .O(new_n124_));
  nand4  g79(.a(new_n112_), .b(new_n118_), .c(new_n56_), .d(new_n59_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n102_), .c(new_n86_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n124_), .c(new_n47_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n116_), .O(z16));
  nand2  g83(.a(new_n125_), .b(x24), .O(new_n129_));
  nor2   g84(.a(x22), .b(x21), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n110_), .c(new_n48_), .d(new_n56_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n104_), .d(new_n47_), .O(z17));
  nand3  g87(.a(new_n107_), .b(new_n48_), .c(new_n56_), .O(new_n133_));
  and2   g88(.a(new_n133_), .b(new_n47_), .O(new_n134_));
  aoi21  g89(.a(new_n130_), .b(new_n97_), .c(x24), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(new_n134_), .c(x25), .O(new_n136_));
  nand3  g91(.a(new_n130_), .b(new_n55_), .c(new_n56_), .O(new_n137_));
  oai21  g92(.a(new_n137_), .b(new_n108_), .c(new_n47_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n48_), .O(new_n139_));
  nand3  g94(.a(new_n139_), .b(new_n136_), .c(new_n104_), .O(z18));
endmodule


