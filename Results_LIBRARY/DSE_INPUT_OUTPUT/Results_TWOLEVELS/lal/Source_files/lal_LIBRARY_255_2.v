// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x07), .O(new_n47_));
  nor2   g002(.a(x15), .b(new_n47_), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x15), .O(new_n51_));
  nand2  g006(.a(new_n51_), .b(x07), .O(new_n52_));
  nand3  g007(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x23), .O(new_n55_));
  inv1   g010(.a(x20), .O(new_n56_));
  inv1   g011(.a(x18), .O(new_n57_));
  inv1   g012(.a(x19), .O(new_n58_));
  nand2  g013(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g014(.a(new_n59_), .b(x17), .c(new_n56_), .O(new_n60_));
  nand3  g015(.a(new_n60_), .b(x22), .c(x21), .O(new_n61_));
  nand2  g016(.a(new_n61_), .b(new_n55_), .O(new_n62_));
  nand2  g017(.a(new_n62_), .b(x24), .O(new_n63_));
  nand3  g018(.a(new_n63_), .b(new_n54_), .c(new_n47_), .O(new_n64_));
  nand2  g019(.a(x15), .b(x07), .O(new_n65_));
  nand3  g020(.a(new_n65_), .b(new_n64_), .c(new_n53_), .O(z01));
  inv1   g021(.a(x16), .O(new_n67_));
  nor2   g022(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g023(.a(new_n63_), .b(new_n54_), .O(new_n69_));
  nand2  g024(.a(new_n69_), .b(new_n49_), .O(z03));
  xnor2a g025(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g026(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g027(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g028(.a(x12), .b(x03), .O(new_n74_));
  nand4  g029(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g031(.a(new_n76_), .b(new_n49_), .O(z04));
  oai21  g032(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g033(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g034(.a(new_n79_), .b(new_n49_), .O(z06));
  nand3  g035(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g036(.a(x17), .O(new_n82_));
  nand3  g037(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  oai21  g038(.a(new_n83_), .b(new_n59_), .c(new_n54_), .O(new_n84_));
  nand2  g039(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g040(.a(new_n83_), .b(new_n54_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(x20), .O(new_n87_));
  oai21  g042(.a(x25), .b(x24), .c(x23), .O(new_n88_));
  inv1   g043(.a(x22), .O(new_n89_));
  inv1   g044(.a(x24), .O(new_n90_));
  inv1   g045(.a(x21), .O(new_n91_));
  nand3  g046(.a(new_n91_), .b(x19), .c(x18), .O(new_n92_));
  inv1   g047(.a(new_n92_), .O(new_n93_));
  and2   g048(.a(x18), .b(x17), .O(new_n94_));
  nor2   g049(.a(x20), .b(new_n58_), .O(new_n95_));
  nor2   g050(.a(x22), .b(x21), .O(new_n96_));
  nor2   g051(.a(x24), .b(x23), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n93_), .c(new_n90_), .d(new_n89_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(x25), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n88_), .c(new_n87_), .d(new_n85_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand3  g057(.a(new_n60_), .b(x22), .c(x21), .O(new_n103_));
  aoi21  g058(.a(new_n103_), .b(new_n55_), .c(new_n90_), .O(new_n104_));
  oai21  g059(.a(new_n104_), .b(x25), .c(x15), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(new_n102_), .O(z08));
  aoi21  g061(.a(x05), .b(x04), .c(x07), .O(new_n107_));
  nor2   g062(.a(new_n107_), .b(x15), .O(z09));
  nand2  g063(.a(x05), .b(x04), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(new_n82_), .O(new_n110_));
  aoi21  g065(.a(new_n110_), .b(new_n47_), .c(x15), .O(z10));
  xor2a  g066(.a(x18), .b(x17), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  aoi21  g068(.a(new_n113_), .b(new_n47_), .c(x15), .O(z11));
  nand3  g069(.a(new_n58_), .b(x18), .c(x17), .O(new_n115_));
  oai21  g070(.a(new_n94_), .b(new_n58_), .c(new_n115_), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(new_n109_), .O(new_n117_));
  aoi21  g072(.a(new_n117_), .b(new_n47_), .c(x15), .O(z12));
  nand2  g073(.a(new_n95_), .b(new_n94_), .O(new_n119_));
  nand3  g074(.a(x19), .b(x18), .c(x17), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(x20), .O(new_n121_));
  nand3  g076(.a(new_n51_), .b(x05), .c(x04), .O(new_n122_));
  nand3  g077(.a(new_n122_), .b(new_n121_), .c(new_n119_), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(new_n47_), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n51_), .O(z13));
  nand4  g080(.a(new_n94_), .b(new_n91_), .c(new_n56_), .d(x19), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  aoi21  g082(.a(new_n119_), .b(x21), .c(new_n127_), .O(new_n128_));
  oai21  g083(.a(new_n128_), .b(x07), .c(new_n51_), .O(z14));
  inv1   g084(.a(z09), .O(new_n130_));
  nand2  g085(.a(new_n126_), .b(x22), .O(new_n131_));
  inv1   g086(.a(new_n120_), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n96_), .c(new_n56_), .O(new_n133_));
  nand4  g088(.a(new_n133_), .b(new_n131_), .c(new_n130_), .d(new_n51_), .O(z15));
  and2   g089(.a(new_n133_), .b(x23), .O(new_n135_));
  nand4  g090(.a(new_n55_), .b(new_n89_), .c(new_n91_), .d(new_n56_), .O(new_n136_));
  oai21  g091(.a(new_n136_), .b(new_n120_), .c(new_n122_), .O(new_n137_));
  oai21  g092(.a(new_n137_), .b(new_n135_), .c(new_n47_), .O(new_n138_));
  nand2  g093(.a(new_n138_), .b(new_n51_), .O(z16));
  nand4  g094(.a(new_n96_), .b(x25), .c(new_n90_), .d(new_n55_), .O(new_n140_));
  oai22  g095(.a(new_n140_), .b(new_n119_), .c(new_n90_), .d(new_n55_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n47_), .O(new_n142_));
  nand2  g097(.a(new_n133_), .b(x24), .O(new_n143_));
  nand4  g098(.a(new_n96_), .b(new_n54_), .c(new_n90_), .d(new_n55_), .O(new_n144_));
  nor2   g099(.a(new_n144_), .b(new_n119_), .O(new_n145_));
  nor2   g100(.a(new_n145_), .b(x15), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(new_n143_), .c(new_n142_), .d(new_n130_), .O(z17));
  nand3  g102(.a(new_n98_), .b(x25), .c(new_n47_), .O(new_n148_));
  nand3  g103(.a(new_n148_), .b(new_n146_), .c(new_n130_), .O(z18));
endmodule


