// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  nor2   g002(.a(x22), .b(new_n47_), .O(new_n48_));
  nor3   g003(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g004(.a(x22), .O(new_n50_));
  inv1   g005(.a(x23), .O(new_n51_));
  nand2  g006(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g007(.a(new_n52_), .b(x24), .c(x15), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  inv1   g009(.a(x21), .O(new_n55_));
  inv1   g010(.a(x17), .O(new_n56_));
  inv1   g011(.a(x18), .O(new_n57_));
  inv1   g012(.a(x19), .O(new_n58_));
  nand3  g013(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  aoi21  g014(.a(new_n59_), .b(new_n54_), .c(new_n55_), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  aoi21  g016(.a(new_n61_), .b(x22), .c(new_n53_), .O(new_n62_));
  nand2  g017(.a(x05), .b(x04), .O(new_n63_));
  inv1   g018(.a(new_n63_), .O(new_n64_));
  nor3   g019(.a(new_n64_), .b(new_n48_), .c(x07), .O(new_n65_));
  oai21  g020(.a(new_n62_), .b(x25), .c(new_n65_), .O(z01));
  inv1   g021(.a(x16), .O(new_n67_));
  nor2   g022(.a(new_n48_), .b(new_n67_), .O(z02));
  nor2   g023(.a(new_n62_), .b(x25), .O(z03));
  inv1   g024(.a(new_n48_), .O(new_n70_));
  xnor2a g025(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g026(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g027(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g028(.a(x12), .b(x03), .O(new_n74_));
  nand4  g029(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g030(.a(new_n75_), .b(new_n70_), .c(new_n46_), .O(new_n76_));
  inv1   g031(.a(new_n76_), .O(z04));
  nor3   g032(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g033(.a(x14), .O(new_n79_));
  nor3   g034(.a(new_n48_), .b(new_n79_), .c(x08), .O(z06));
  aoi21  g035(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand2  g036(.a(new_n58_), .b(new_n57_), .O(new_n82_));
  nand3  g037(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  nand2  g038(.a(x25), .b(new_n47_), .O(new_n84_));
  oai21  g039(.a(new_n83_), .b(new_n82_), .c(new_n84_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(new_n56_), .O(new_n86_));
  nand2  g041(.a(new_n84_), .b(new_n83_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(x20), .O(new_n88_));
  inv1   g043(.a(x24), .O(new_n89_));
  nor2   g044(.a(new_n89_), .b(new_n51_), .O(new_n90_));
  oai21  g045(.a(new_n90_), .b(x25), .c(x22), .O(new_n91_));
  oai21  g046(.a(x25), .b(x24), .c(x23), .O(new_n92_));
  nor2   g047(.a(new_n58_), .b(new_n57_), .O(new_n93_));
  nor2   g048(.a(new_n57_), .b(new_n56_), .O(new_n94_));
  nor2   g049(.a(x20), .b(new_n58_), .O(new_n95_));
  nor2   g050(.a(x22), .b(x21), .O(new_n96_));
  nor2   g051(.a(x24), .b(x23), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n93_), .c(new_n89_), .d(new_n55_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(x25), .O(new_n100_));
  nand2  g055(.a(new_n100_), .b(new_n92_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n91_), .c(new_n88_), .d(new_n86_), .O(z08));
  inv1   g058(.a(x07), .O(new_n104_));
  nand3  g059(.a(new_n64_), .b(new_n47_), .c(new_n104_), .O(new_n105_));
  inv1   g060(.a(new_n105_), .O(z09));
  nand4  g061(.a(new_n63_), .b(new_n56_), .c(new_n47_), .d(new_n104_), .O(new_n107_));
  inv1   g062(.a(new_n107_), .O(z10));
  xor2a  g063(.a(x18), .b(x17), .O(new_n109_));
  nand4  g064(.a(new_n109_), .b(new_n63_), .c(new_n47_), .d(new_n104_), .O(new_n110_));
  inv1   g065(.a(new_n110_), .O(z11));
  nand3  g066(.a(new_n58_), .b(x18), .c(x17), .O(new_n112_));
  oai21  g067(.a(new_n94_), .b(new_n58_), .c(new_n112_), .O(new_n113_));
  nand4  g068(.a(new_n113_), .b(new_n63_), .c(new_n47_), .d(new_n104_), .O(new_n114_));
  inv1   g069(.a(new_n114_), .O(z12));
  nand2  g070(.a(new_n95_), .b(new_n94_), .O(new_n116_));
  nand2  g071(.a(new_n93_), .b(x17), .O(new_n117_));
  nand2  g072(.a(new_n117_), .b(x20), .O(new_n118_));
  nand3  g073(.a(new_n104_), .b(x05), .c(x04), .O(new_n119_));
  nand4  g074(.a(new_n119_), .b(new_n118_), .c(new_n116_), .d(new_n104_), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(new_n47_), .O(new_n121_));
  nand2  g076(.a(x22), .b(x15), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(new_n121_), .O(z13));
  nand2  g078(.a(new_n50_), .b(x15), .O(new_n124_));
  nand4  g079(.a(new_n124_), .b(new_n55_), .c(new_n54_), .d(x19), .O(new_n125_));
  inv1   g080(.a(new_n125_), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(x18), .c(x17), .O(new_n127_));
  inv1   g082(.a(new_n116_), .O(new_n128_));
  aoi21  g083(.a(x05), .b(x04), .c(x07), .O(new_n129_));
  oai21  g084(.a(new_n128_), .b(new_n55_), .c(new_n129_), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(new_n47_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n127_), .c(new_n122_), .O(z14));
  inv1   g087(.a(new_n117_), .O(new_n133_));
  nand3  g088(.a(new_n133_), .b(new_n96_), .c(new_n54_), .O(new_n134_));
  and2   g089(.a(new_n134_), .b(new_n119_), .O(new_n135_));
  nor2   g090(.a(x21), .b(x20), .O(new_n136_));
  nand3  g091(.a(new_n136_), .b(new_n94_), .c(x19), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(x22), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n135_), .c(new_n47_), .d(new_n104_), .O(z15));
  nand2  g094(.a(new_n134_), .b(x23), .O(new_n140_));
  inv1   g095(.a(new_n52_), .O(new_n141_));
  nand3  g096(.a(new_n136_), .b(new_n133_), .c(new_n141_), .O(new_n142_));
  nand4  g097(.a(new_n142_), .b(new_n140_), .c(new_n129_), .d(new_n47_), .O(z16));
  nand3  g098(.a(new_n97_), .b(new_n50_), .c(new_n55_), .O(new_n144_));
  nor2   g099(.a(new_n144_), .b(x20), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(x19), .c(x18), .d(x17), .O(new_n146_));
  inv1   g101(.a(new_n90_), .O(new_n147_));
  nand3  g102(.a(new_n119_), .b(new_n147_), .c(new_n104_), .O(new_n148_));
  inv1   g103(.a(new_n148_), .O(new_n149_));
  nand2  g104(.a(new_n134_), .b(x24), .O(new_n150_));
  nand4  g105(.a(new_n150_), .b(new_n149_), .c(new_n146_), .d(new_n47_), .O(z17));
  oai21  g106(.a(x25), .b(x15), .c(x22), .O(new_n152_));
  nand2  g107(.a(new_n136_), .b(new_n97_), .O(new_n153_));
  oai21  g108(.a(new_n153_), .b(new_n117_), .c(x25), .O(new_n154_));
  nor3   g109(.a(x25), .b(x24), .c(x23), .O(new_n155_));
  nand3  g110(.a(new_n155_), .b(new_n96_), .c(new_n128_), .O(new_n156_));
  nand4  g111(.a(new_n156_), .b(new_n154_), .c(new_n119_), .d(new_n104_), .O(new_n157_));
  nand2  g112(.a(new_n157_), .b(new_n47_), .O(new_n158_));
  nand2  g113(.a(new_n158_), .b(new_n152_), .O(z18));
endmodule


