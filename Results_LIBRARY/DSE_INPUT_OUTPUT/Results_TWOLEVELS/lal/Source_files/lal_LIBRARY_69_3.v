// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_;
  inv1   g000(.a(x16), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  inv1   g002(.a(x23), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g006(.a(new_n51_), .O(z00));
  inv1   g007(.a(x07), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x20), .O(new_n55_));
  inv1   g010(.a(x17), .O(new_n56_));
  nor2   g011(.a(x19), .b(x18), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g013(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g014(.a(new_n58_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  oai22  g015(.a(new_n60_), .b(x23), .c(new_n49_), .d(x24), .O(new_n61_));
  nand2  g016(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  and2   g017(.a(x05), .b(x04), .O(new_n63_));
  inv1   g018(.a(new_n63_), .O(new_n64_));
  nand4  g019(.a(new_n64_), .b(new_n62_), .c(new_n50_), .d(new_n53_), .O(z01));
  nand2  g020(.a(new_n50_), .b(new_n46_), .O(z02));
  inv1   g021(.a(new_n62_), .O(z03));
  inv1   g022(.a(x08), .O(new_n68_));
  xnor2a g023(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g024(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g025(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g026(.a(x12), .b(x03), .O(new_n72_));
  nand4  g027(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand3  g028(.a(new_n73_), .b(new_n50_), .c(new_n68_), .O(new_n74_));
  inv1   g029(.a(new_n74_), .O(z04));
  oai21  g030(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g031(.a(x14), .b(new_n68_), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(new_n50_), .O(z06));
  nand3  g033(.a(new_n50_), .b(new_n68_), .c(x06), .O(z07));
  inv1   g034(.a(x18), .O(new_n80_));
  inv1   g035(.a(x19), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g037(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  nand2  g038(.a(x25), .b(new_n48_), .O(new_n84_));
  oai21  g039(.a(new_n83_), .b(new_n82_), .c(new_n84_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(new_n56_), .O(new_n86_));
  nand2  g041(.a(new_n84_), .b(new_n83_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(x20), .O(new_n88_));
  oai21  g043(.a(x25), .b(x23), .c(x24), .O(new_n89_));
  oai21  g044(.a(x25), .b(x15), .c(x23), .O(new_n90_));
  inv1   g045(.a(x21), .O(new_n91_));
  inv1   g046(.a(x22), .O(new_n92_));
  nor2   g047(.a(new_n81_), .b(new_n80_), .O(new_n93_));
  nand3  g048(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g049(.a(new_n94_), .O(new_n95_));
  nor2   g050(.a(x21), .b(x20), .O(new_n96_));
  nor2   g051(.a(x24), .b(x22), .O(new_n97_));
  nand3  g052(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n93_), .c(new_n92_), .d(new_n91_), .O(new_n99_));
  nand3  g054(.a(new_n99_), .b(x25), .c(new_n48_), .O(new_n100_));
  nand3  g055(.a(new_n100_), .b(new_n90_), .c(new_n89_), .O(new_n101_));
  inv1   g056(.a(new_n101_), .O(new_n102_));
  nand3  g057(.a(new_n102_), .b(new_n88_), .c(new_n86_), .O(z08));
  nand3  g058(.a(new_n63_), .b(new_n47_), .c(new_n53_), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(new_n50_), .O(z09));
  nand4  g060(.a(new_n64_), .b(new_n56_), .c(new_n47_), .d(new_n53_), .O(new_n106_));
  inv1   g061(.a(new_n106_), .O(z10));
  xor2a  g062(.a(x18), .b(x17), .O(new_n108_));
  nand4  g063(.a(new_n108_), .b(new_n64_), .c(new_n47_), .d(new_n53_), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(new_n50_), .O(z11));
  nor2   g065(.a(new_n80_), .b(new_n56_), .O(new_n111_));
  nand3  g066(.a(new_n81_), .b(x18), .c(x17), .O(new_n112_));
  oai21  g067(.a(new_n111_), .b(new_n81_), .c(new_n112_), .O(new_n113_));
  nand4  g068(.a(new_n113_), .b(new_n64_), .c(new_n47_), .d(new_n53_), .O(new_n114_));
  inv1   g069(.a(new_n114_), .O(z12));
  nand2  g070(.a(new_n94_), .b(x20), .O(new_n116_));
  nand3  g071(.a(new_n111_), .b(new_n55_), .c(x19), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(new_n104_), .c(new_n53_), .O(new_n118_));
  inv1   g073(.a(new_n118_), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n116_), .c(new_n47_), .O(z13));
  inv1   g075(.a(new_n117_), .O(new_n121_));
  nor2   g076(.a(new_n121_), .b(new_n91_), .O(new_n122_));
  nand3  g077(.a(new_n53_), .b(x05), .c(x04), .O(new_n123_));
  nand3  g078(.a(new_n111_), .b(new_n96_), .c(x19), .O(new_n124_));
  nand3  g079(.a(new_n124_), .b(new_n123_), .c(new_n53_), .O(new_n125_));
  oai21  g080(.a(new_n125_), .b(new_n122_), .c(new_n47_), .O(new_n126_));
  nand2  g081(.a(new_n48_), .b(x15), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n126_), .O(z14));
  nor2   g083(.a(x22), .b(x21), .O(new_n129_));
  nand3  g084(.a(new_n129_), .b(new_n95_), .c(new_n55_), .O(new_n130_));
  nand3  g085(.a(new_n130_), .b(new_n123_), .c(new_n53_), .O(new_n131_));
  aoi21  g086(.a(new_n124_), .b(x22), .c(new_n131_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(x15), .c(new_n127_), .O(z15));
  nand2  g088(.a(new_n130_), .b(new_n47_), .O(new_n134_));
  nand2  g089(.a(new_n134_), .b(new_n48_), .O(new_n135_));
  nand2  g090(.a(new_n123_), .b(new_n53_), .O(new_n136_));
  aoi21  g091(.a(new_n130_), .b(x23), .c(new_n136_), .O(new_n137_));
  oai21  g092(.a(new_n137_), .b(x15), .c(new_n135_), .O(z16));
  inv1   g093(.a(x24), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n48_), .c(new_n92_), .d(new_n91_), .O(new_n140_));
  nor2   g095(.a(new_n140_), .b(x20), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(x19), .c(x18), .d(x17), .O(new_n142_));
  nand4  g097(.a(new_n96_), .b(new_n95_), .c(new_n48_), .d(new_n92_), .O(new_n143_));
  nand2  g098(.a(new_n104_), .b(new_n53_), .O(new_n144_));
  aoi21  g099(.a(new_n143_), .b(x24), .c(new_n144_), .O(new_n145_));
  nand3  g100(.a(new_n145_), .b(new_n142_), .c(new_n47_), .O(z17));
  nand2  g101(.a(x25), .b(x23), .O(new_n147_));
  nand3  g102(.a(new_n147_), .b(new_n123_), .c(new_n53_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n47_), .O(new_n149_));
  nand2  g104(.a(new_n98_), .b(x25), .O(new_n150_));
  nor2   g105(.a(x25), .b(x24), .O(new_n151_));
  nand3  g106(.a(new_n151_), .b(new_n129_), .c(new_n121_), .O(new_n152_));
  nand3  g107(.a(new_n152_), .b(new_n150_), .c(new_n47_), .O(new_n153_));
  nand2  g108(.a(new_n153_), .b(new_n48_), .O(new_n154_));
  nand2  g109(.a(new_n154_), .b(new_n149_), .O(z18));
endmodule


