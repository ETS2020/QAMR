// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n212_, new_n213_,
    new_n214_, new_n215_, new_n216_, new_n217_;
  inv1   g000(.a(x16), .O(new_n46_));
  inv1   g001(.a(x07), .O(new_n47_));
  inv1   g002(.a(x15), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g006(.a(new_n51_), .O(z00));
  inv1   g007(.a(x25), .O(new_n53_));
  nand2  g008(.a(x15), .b(x07), .O(new_n54_));
  inv1   g009(.a(x23), .O(new_n55_));
  inv1   g010(.a(x17), .O(new_n56_));
  nor2   g011(.a(x19), .b(x18), .O(new_n57_));
  aoi21  g012(.a(new_n57_), .b(new_n56_), .c(x20), .O(new_n58_));
  nand2  g013(.a(x22), .b(x21), .O(new_n59_));
  oai21  g014(.a(new_n59_), .b(new_n58_), .c(new_n55_), .O(new_n60_));
  nand2  g015(.a(new_n60_), .b(x24), .O(new_n61_));
  nand3  g016(.a(new_n61_), .b(new_n54_), .c(new_n53_), .O(new_n62_));
  nand3  g017(.a(new_n47_), .b(x05), .c(x04), .O(new_n63_));
  nand3  g018(.a(new_n63_), .b(new_n62_), .c(new_n47_), .O(z01));
  nand2  g019(.a(new_n50_), .b(new_n46_), .O(z02));
  inv1   g020(.a(new_n62_), .O(z03));
  inv1   g021(.a(x08), .O(new_n67_));
  xnor2a g022(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g023(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g024(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g025(.a(x12), .b(x03), .O(new_n71_));
  nand4  g026(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g027(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  nand2  g028(.a(new_n73_), .b(new_n50_), .O(z04));
  inv1   g029(.a(x13), .O(new_n75_));
  nand3  g030(.a(new_n50_), .b(new_n75_), .c(new_n67_), .O(new_n76_));
  inv1   g031(.a(new_n76_), .O(z05));
  nand2  g032(.a(x14), .b(new_n67_), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n50_), .O(z06));
  aoi21  g034(.a(new_n67_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g035(.a(new_n57_), .O(new_n81_));
  nand3  g036(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  oai21  g037(.a(new_n82_), .b(new_n81_), .c(new_n53_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(new_n56_), .O(new_n84_));
  nand2  g039(.a(new_n82_), .b(new_n53_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(x20), .O(new_n86_));
  oai21  g041(.a(x25), .b(x24), .c(x23), .O(new_n87_));
  inv1   g042(.a(x22), .O(new_n88_));
  inv1   g043(.a(x24), .O(new_n89_));
  inv1   g044(.a(x21), .O(new_n90_));
  nand3  g045(.a(new_n90_), .b(x19), .c(x18), .O(new_n91_));
  inv1   g046(.a(new_n91_), .O(new_n92_));
  inv1   g047(.a(x20), .O(new_n93_));
  nand4  g048(.a(new_n93_), .b(x19), .c(x18), .d(x17), .O(new_n94_));
  inv1   g049(.a(new_n94_), .O(new_n95_));
  nor2   g050(.a(x22), .b(x21), .O(new_n96_));
  nor2   g051(.a(x24), .b(x23), .O(new_n97_));
  nand3  g052(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n92_), .c(new_n89_), .d(new_n88_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(x25), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n87_), .c(new_n86_), .d(new_n84_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  nor2   g057(.a(new_n58_), .b(new_n88_), .O(new_n103_));
  aoi21  g058(.a(new_n103_), .b(x21), .c(x23), .O(new_n104_));
  oai21  g059(.a(new_n104_), .b(new_n89_), .c(new_n53_), .O(new_n105_));
  nand3  g060(.a(new_n105_), .b(x15), .c(new_n47_), .O(new_n106_));
  nand2  g061(.a(new_n106_), .b(new_n102_), .O(z08));
  nand2  g062(.a(x05), .b(x04), .O(new_n108_));
  inv1   g063(.a(new_n108_), .O(new_n109_));
  nand3  g064(.a(new_n109_), .b(new_n48_), .c(new_n47_), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(new_n50_), .O(z09));
  nand4  g066(.a(new_n108_), .b(new_n56_), .c(new_n48_), .d(new_n47_), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(new_n50_), .O(z10));
  xor2a  g068(.a(x18), .b(x17), .O(new_n114_));
  nand4  g069(.a(new_n114_), .b(new_n108_), .c(new_n48_), .d(new_n47_), .O(new_n115_));
  inv1   g070(.a(new_n115_), .O(z11));
  inv1   g071(.a(x18), .O(new_n117_));
  oai21  g072(.a(new_n117_), .b(new_n56_), .c(x19), .O(new_n118_));
  inv1   g073(.a(x19), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(x18), .c(x17), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand4  g076(.a(new_n121_), .b(new_n108_), .c(new_n48_), .d(new_n47_), .O(new_n122_));
  inv1   g077(.a(new_n122_), .O(z12));
  nor2   g078(.a(new_n119_), .b(new_n117_), .O(new_n124_));
  inv1   g079(.a(new_n124_), .O(new_n125_));
  nand4  g080(.a(new_n53_), .b(new_n55_), .c(x15), .d(new_n47_), .O(new_n126_));
  aoi21  g081(.a(new_n126_), .b(new_n125_), .c(new_n56_), .O(new_n127_));
  nand4  g082(.a(new_n81_), .b(new_n53_), .c(new_n55_), .d(x15), .O(new_n128_));
  nor2   g083(.a(new_n128_), .b(x07), .O(new_n129_));
  oai21  g084(.a(new_n129_), .b(new_n127_), .c(new_n93_), .O(new_n130_));
  nand4  g085(.a(x24), .b(x22), .c(x21), .d(new_n119_), .O(new_n131_));
  inv1   g086(.a(new_n131_), .O(new_n132_));
  nand4  g087(.a(new_n132_), .b(new_n117_), .c(x15), .d(new_n47_), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n93_), .O(new_n134_));
  nand2  g089(.a(new_n134_), .b(new_n56_), .O(new_n135_));
  aoi21  g090(.a(new_n59_), .b(new_n55_), .c(new_n89_), .O(new_n136_));
  oai21  g091(.a(new_n59_), .b(new_n93_), .c(new_n55_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(x24), .O(new_n138_));
  nand3  g093(.a(new_n138_), .b(new_n136_), .c(new_n53_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(x15), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(new_n108_), .O(new_n141_));
  aoi22  g096(.a(new_n141_), .b(new_n47_), .c(new_n125_), .d(x20), .O(new_n142_));
  nand4  g097(.a(new_n142_), .b(new_n135_), .c(new_n130_), .d(new_n47_), .O(z13));
  nand3  g098(.a(new_n90_), .b(x18), .c(x17), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(new_n126_), .O(new_n145_));
  nand3  g100(.a(new_n145_), .b(new_n93_), .c(x19), .O(new_n146_));
  nand4  g101(.a(new_n53_), .b(x24), .c(new_n55_), .d(x21), .O(new_n147_));
  aoi21  g102(.a(new_n147_), .b(x15), .c(new_n109_), .O(new_n148_));
  nand2  g103(.a(new_n94_), .b(x21), .O(new_n149_));
  nand4  g104(.a(new_n149_), .b(new_n148_), .c(new_n146_), .d(new_n47_), .O(z14));
  nand4  g105(.a(new_n96_), .b(x19), .c(x18), .d(new_n48_), .O(new_n151_));
  aoi21  g106(.a(new_n151_), .b(new_n126_), .c(new_n56_), .O(new_n152_));
  oai21  g107(.a(new_n152_), .b(new_n129_), .c(new_n93_), .O(new_n153_));
  nor2   g108(.a(new_n58_), .b(new_n89_), .O(new_n154_));
  aoi21  g109(.a(new_n154_), .b(new_n47_), .c(new_n48_), .O(new_n155_));
  nor2   g110(.a(new_n155_), .b(new_n90_), .O(new_n156_));
  nor2   g111(.a(new_n95_), .b(x15), .O(new_n157_));
  oai21  g112(.a(new_n157_), .b(new_n156_), .c(x22), .O(new_n158_));
  nand2  g113(.a(x24), .b(x23), .O(new_n159_));
  nand3  g114(.a(new_n159_), .b(new_n136_), .c(new_n53_), .O(new_n160_));
  nand2  g115(.a(new_n160_), .b(x15), .O(new_n161_));
  aoi21  g116(.a(new_n161_), .b(new_n108_), .c(x07), .O(new_n162_));
  aoi21  g117(.a(new_n48_), .b(x07), .c(new_n162_), .O(new_n163_));
  nand3  g118(.a(new_n163_), .b(new_n158_), .c(new_n153_), .O(z15));
  nand2  g119(.a(new_n124_), .b(new_n96_), .O(new_n165_));
  nand3  g120(.a(new_n53_), .b(x15), .c(new_n47_), .O(new_n166_));
  nand2  g121(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand2  g122(.a(new_n167_), .b(x17), .O(new_n168_));
  nand4  g123(.a(new_n81_), .b(new_n53_), .c(x15), .d(new_n47_), .O(new_n169_));
  aoi21  g124(.a(new_n169_), .b(new_n168_), .c(x20), .O(new_n170_));
  nand4  g125(.a(new_n59_), .b(new_n53_), .c(x15), .d(new_n47_), .O(new_n171_));
  inv1   g126(.a(new_n171_), .O(new_n172_));
  oai21  g127(.a(new_n172_), .b(new_n170_), .c(new_n55_), .O(new_n173_));
  nand2  g128(.a(new_n133_), .b(new_n55_), .O(new_n174_));
  nand2  g129(.a(new_n174_), .b(new_n56_), .O(new_n175_));
  nand2  g130(.a(x15), .b(new_n47_), .O(new_n176_));
  nor2   g131(.a(new_n82_), .b(new_n176_), .O(new_n177_));
  oai21  g132(.a(new_n177_), .b(x23), .c(x20), .O(new_n178_));
  nand3  g133(.a(new_n53_), .b(x24), .c(new_n55_), .O(new_n179_));
  nand2  g134(.a(new_n179_), .b(x15), .O(new_n180_));
  nand2  g135(.a(new_n180_), .b(new_n108_), .O(new_n181_));
  nand2  g136(.a(new_n181_), .b(new_n47_), .O(new_n182_));
  nand2  g137(.a(new_n165_), .b(x23), .O(new_n183_));
  nand4  g138(.a(new_n183_), .b(new_n182_), .c(new_n178_), .d(new_n175_), .O(new_n184_));
  inv1   g139(.a(new_n184_), .O(new_n185_));
  nand3  g140(.a(new_n185_), .b(new_n173_), .c(new_n47_), .O(z16));
  nand4  g141(.a(new_n96_), .b(x25), .c(new_n89_), .d(new_n55_), .O(new_n187_));
  oai21  g142(.a(new_n187_), .b(new_n94_), .c(new_n159_), .O(new_n188_));
  nand2  g143(.a(new_n188_), .b(new_n48_), .O(new_n189_));
  oai21  g144(.a(x23), .b(x22), .c(x24), .O(new_n190_));
  nand3  g145(.a(new_n190_), .b(x15), .c(new_n47_), .O(new_n191_));
  nand2  g146(.a(new_n191_), .b(new_n98_), .O(new_n192_));
  nand2  g147(.a(new_n192_), .b(new_n53_), .O(new_n193_));
  aoi21  g148(.a(x24), .b(x23), .c(x25), .O(new_n194_));
  oai21  g149(.a(new_n194_), .b(new_n48_), .c(new_n108_), .O(new_n195_));
  nand4  g150(.a(new_n124_), .b(new_n96_), .c(new_n93_), .d(x17), .O(new_n196_));
  aoi22  g151(.a(new_n196_), .b(x24), .c(new_n195_), .d(new_n47_), .O(new_n197_));
  nand4  g152(.a(new_n197_), .b(new_n193_), .c(new_n189_), .d(new_n47_), .O(z17));
  nor2   g153(.a(x24), .b(x22), .O(new_n199_));
  nand4  g154(.a(new_n199_), .b(new_n90_), .c(x19), .d(x18), .O(new_n200_));
  aoi21  g155(.a(new_n200_), .b(new_n176_), .c(new_n56_), .O(new_n201_));
  nor3   g156(.a(new_n57_), .b(new_n48_), .c(x07), .O(new_n202_));
  oai21  g157(.a(new_n202_), .b(new_n201_), .c(new_n93_), .O(new_n203_));
  nand3  g158(.a(new_n59_), .b(x15), .c(new_n47_), .O(new_n204_));
  aoi21  g159(.a(new_n204_), .b(new_n203_), .c(x23), .O(new_n205_));
  nand3  g160(.a(new_n89_), .b(x15), .c(new_n47_), .O(new_n206_));
  inv1   g161(.a(new_n206_), .O(new_n207_));
  oai21  g162(.a(new_n207_), .b(new_n205_), .c(new_n53_), .O(new_n208_));
  nor2   g163(.a(new_n53_), .b(x15), .O(new_n209_));
  inv1   g164(.a(new_n209_), .O(new_n210_));
  aoi21  g165(.a(new_n210_), .b(new_n133_), .c(x17), .O(new_n211_));
  nand2  g166(.a(new_n195_), .b(new_n47_), .O(new_n212_));
  oai21  g167(.a(new_n209_), .b(new_n177_), .c(x20), .O(new_n213_));
  nand3  g168(.a(new_n97_), .b(new_n92_), .c(new_n88_), .O(new_n214_));
  nand3  g169(.a(new_n214_), .b(x25), .c(new_n48_), .O(new_n215_));
  nand3  g170(.a(new_n215_), .b(new_n213_), .c(new_n212_), .O(new_n216_));
  nor2   g171(.a(new_n216_), .b(new_n211_), .O(new_n217_));
  nand3  g172(.a(new_n217_), .b(new_n208_), .c(new_n47_), .O(z18));
endmodule


