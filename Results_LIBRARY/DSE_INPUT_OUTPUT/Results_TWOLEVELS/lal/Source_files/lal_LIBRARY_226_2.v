// Benchmark "FAU" written by ABC on Wed Aug 19 19:06:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  oai21  g06(.a(x15), .b(x07), .c(new_n51_), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(x05), .c(x04), .O(new_n53_));
  nand2  g08(.a(new_n49_), .b(x07), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  nor3   g11(.a(x19), .b(x18), .c(x17), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  oai21  g15(.a(new_n57_), .b(x20), .c(new_n60_), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(x24), .c(new_n56_), .O(new_n62_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n55_), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n54_), .c(new_n53_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  inv1   g23(.a(new_n65_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n49_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z06));
  nand3  g34(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g35(.a(x20), .O(new_n81_));
  inv1   g36(.a(new_n57_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n81_), .c(new_n59_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x21), .c(x23), .O(new_n84_));
  nor2   g39(.a(new_n48_), .b(x25), .O(new_n85_));
  oai21  g40(.a(new_n84_), .b(new_n51_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  nand4  g42(.a(new_n47_), .b(new_n87_), .c(x05), .d(x04), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand2  g45(.a(x05), .b(x04), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n90_), .c(new_n47_), .d(new_n87_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n49_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n91_), .c(new_n47_), .d(new_n87_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n49_), .O(z11));
  inv1   g51(.a(x19), .O(new_n97_));
  and2   g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n97_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n91_), .c(new_n47_), .d(new_n87_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n49_), .O(z12));
  nand3  g57(.a(new_n87_), .b(x05), .c(x04), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n87_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(new_n105_));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x20), .O(new_n107_));
  nor2   g62(.a(x20), .b(new_n97_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n98_), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n107_), .c(new_n105_), .d(new_n47_), .O(z13));
  inv1   g65(.a(new_n109_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n58_), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n98_), .c(x19), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n103_), .c(new_n87_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n112_), .c(new_n47_), .O(new_n116_));
  nand2  g71(.a(x24), .b(x15), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(z14));
  inv1   g73(.a(new_n106_), .O(new_n119_));
  nor3   g74(.a(x22), .b(x21), .c(x20), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n103_), .c(new_n87_), .O(new_n122_));
  aoi21  g77(.a(new_n114_), .b(x22), .c(new_n122_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(x15), .c(new_n117_), .O(z15));
  aoi21  g79(.a(new_n120_), .b(new_n119_), .c(new_n56_), .O(new_n125_));
  nand4  g80(.a(new_n113_), .b(new_n119_), .c(new_n56_), .d(new_n59_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n103_), .c(new_n87_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n125_), .c(new_n47_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n117_), .O(z16));
  nand2  g84(.a(new_n126_), .b(x24), .O(new_n130_));
  nor2   g85(.a(x22), .b(x21), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n111_), .c(new_n51_), .d(new_n56_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n130_), .c(new_n105_), .d(new_n47_), .O(z17));
  oai21  g88(.a(x25), .b(x15), .c(x24), .O(new_n134_));
  nand2  g89(.a(new_n126_), .b(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n131_), .c(new_n108_), .d(new_n98_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n103_), .d(new_n87_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n134_), .O(z18));
endmodule


