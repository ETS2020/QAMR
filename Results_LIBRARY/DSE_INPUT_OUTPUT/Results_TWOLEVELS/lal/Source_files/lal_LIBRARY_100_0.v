// Benchmark "FAU" written by ABC on Wed Aug 19 19:05:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_;
  inv1   g000(.a(x08), .O(new_n46_));
  nor2   g001(.a(x15), .b(x02), .O(new_n47_));
  inv1   g002(.a(new_n47_), .O(new_n48_));
  oai21  g003(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g004(.a(x07), .O(new_n50_));
  inv1   g005(.a(x24), .O(new_n51_));
  inv1   g006(.a(x25), .O(new_n52_));
  inv1   g007(.a(x20), .O(new_n53_));
  inv1   g008(.a(x18), .O(new_n54_));
  inv1   g009(.a(x19), .O(new_n55_));
  nand2  g010(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g011(.a(new_n56_), .b(x17), .c(new_n53_), .O(new_n57_));
  inv1   g012(.a(x21), .O(new_n58_));
  inv1   g013(.a(x22), .O(new_n59_));
  nor2   g014(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g015(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g016(.a(new_n61_), .b(new_n51_), .c(new_n52_), .O(new_n62_));
  and2   g017(.a(x05), .b(x04), .O(new_n63_));
  inv1   g018(.a(new_n63_), .O(new_n64_));
  nand4  g019(.a(new_n64_), .b(new_n62_), .c(new_n48_), .d(new_n50_), .O(z01));
  inv1   g020(.a(x16), .O(new_n66_));
  nor2   g021(.a(new_n47_), .b(new_n66_), .O(z02));
  nand2  g022(.a(new_n62_), .b(new_n48_), .O(z03));
  inv1   g023(.a(x15), .O(new_n69_));
  aoi21  g024(.a(x11), .b(new_n46_), .c(new_n69_), .O(new_n70_));
  inv1   g025(.a(x09), .O(new_n71_));
  inv1   g026(.a(x01), .O(new_n72_));
  aoi22  g027(.a(x10), .b(new_n72_), .c(new_n71_), .d(x00), .O(new_n73_));
  oai21  g028(.a(new_n71_), .b(x00), .c(new_n73_), .O(new_n74_));
  inv1   g029(.a(x10), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(x01), .O(new_n76_));
  inv1   g031(.a(x11), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(x02), .O(new_n78_));
  xnor2a g033(.a(x12), .b(x03), .O(new_n79_));
  nand3  g034(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n74_), .c(new_n46_), .O(new_n81_));
  oai21  g036(.a(new_n70_), .b(x02), .c(new_n81_), .O(z04));
  oai21  g037(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g038(.a(x14), .b(new_n46_), .O(new_n84_));
  nand2  g039(.a(new_n84_), .b(new_n48_), .O(z06));
  nand3  g040(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g041(.a(x17), .O(new_n87_));
  nand3  g042(.a(x24), .b(x22), .c(x21), .O(new_n88_));
  oai21  g043(.a(new_n88_), .b(new_n56_), .c(new_n52_), .O(new_n89_));
  nand2  g044(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g045(.a(new_n88_), .b(new_n52_), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(x20), .O(new_n92_));
  oai21  g047(.a(x25), .b(x24), .c(x23), .O(new_n93_));
  nand3  g048(.a(new_n58_), .b(x19), .c(x18), .O(new_n94_));
  inv1   g049(.a(new_n94_), .O(new_n95_));
  and2   g050(.a(x18), .b(x17), .O(new_n96_));
  nor2   g051(.a(x20), .b(new_n55_), .O(new_n97_));
  nor2   g052(.a(x22), .b(x21), .O(new_n98_));
  nor2   g053(.a(x24), .b(x23), .O(new_n99_));
  nand4  g054(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n95_), .c(new_n51_), .d(new_n59_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(x25), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n93_), .c(new_n92_), .d(new_n90_), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(x02), .O(new_n104_));
  inv1   g059(.a(x23), .O(new_n105_));
  nand3  g060(.a(new_n57_), .b(x22), .c(x21), .O(new_n106_));
  aoi21  g061(.a(new_n106_), .b(new_n105_), .c(new_n51_), .O(new_n107_));
  oai21  g062(.a(new_n107_), .b(x25), .c(x15), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n104_), .O(z08));
  nand4  g064(.a(new_n50_), .b(x05), .c(x04), .d(x02), .O(new_n110_));
  aoi21  g065(.a(new_n110_), .b(x02), .c(x15), .O(z09));
  inv1   g066(.a(x02), .O(new_n112_));
  nand4  g067(.a(new_n64_), .b(new_n87_), .c(new_n69_), .d(new_n50_), .O(new_n113_));
  nor2   g068(.a(new_n113_), .b(new_n112_), .O(z10));
  xor2a  g069(.a(x18), .b(x17), .O(new_n115_));
  nand3  g070(.a(new_n115_), .b(new_n64_), .c(new_n50_), .O(new_n116_));
  aoi21  g071(.a(new_n116_), .b(x02), .c(x15), .O(z11));
  nand3  g072(.a(new_n55_), .b(x18), .c(x17), .O(new_n118_));
  oai21  g073(.a(new_n96_), .b(new_n55_), .c(new_n118_), .O(new_n119_));
  nand4  g074(.a(new_n119_), .b(new_n64_), .c(new_n69_), .d(new_n50_), .O(new_n120_));
  nor2   g075(.a(new_n120_), .b(new_n112_), .O(z12));
  inv1   g076(.a(z09), .O(new_n122_));
  nand3  g077(.a(x19), .b(x18), .c(x17), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(x20), .O(new_n124_));
  nand2  g079(.a(new_n97_), .b(new_n96_), .O(new_n125_));
  inv1   g080(.a(new_n125_), .O(new_n126_));
  nor2   g081(.a(new_n126_), .b(x15), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n124_), .c(new_n122_), .d(new_n50_), .O(z13));
  nor2   g083(.a(new_n126_), .b(new_n58_), .O(new_n129_));
  nand3  g084(.a(new_n63_), .b(new_n69_), .c(new_n50_), .O(new_n130_));
  nand4  g085(.a(new_n96_), .b(new_n58_), .c(new_n53_), .d(x19), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n130_), .c(new_n50_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(new_n129_), .c(x02), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n69_), .O(z14));
  inv1   g089(.a(new_n123_), .O(new_n135_));
  nand3  g090(.a(new_n135_), .b(new_n98_), .c(new_n53_), .O(new_n136_));
  nand3  g091(.a(new_n136_), .b(new_n130_), .c(new_n50_), .O(new_n137_));
  aoi21  g092(.a(new_n131_), .b(x22), .c(new_n137_), .O(new_n138_));
  oai21  g093(.a(new_n138_), .b(new_n112_), .c(new_n69_), .O(z15));
  nand2  g094(.a(new_n136_), .b(x23), .O(new_n140_));
  nand4  g095(.a(new_n105_), .b(new_n59_), .c(new_n58_), .d(new_n53_), .O(new_n141_));
  inv1   g096(.a(new_n141_), .O(new_n142_));
  aoi21  g097(.a(new_n142_), .b(new_n135_), .c(x15), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n140_), .c(new_n122_), .d(new_n50_), .O(z16));
  nand3  g099(.a(new_n99_), .b(new_n59_), .c(new_n58_), .O(new_n145_));
  nor2   g100(.a(new_n145_), .b(x20), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(x19), .c(x18), .d(x17), .O(new_n147_));
  oai21  g102(.a(new_n51_), .b(new_n105_), .c(new_n147_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(x02), .O(new_n149_));
  nand2  g104(.a(new_n69_), .b(new_n50_), .O(new_n150_));
  aoi21  g105(.a(new_n136_), .b(x24), .c(new_n150_), .O(new_n151_));
  nand3  g106(.a(new_n151_), .b(new_n149_), .c(new_n122_), .O(z17));
  nand2  g107(.a(new_n100_), .b(x25), .O(new_n153_));
  nor3   g108(.a(x25), .b(x24), .c(x23), .O(new_n154_));
  nand3  g109(.a(new_n154_), .b(new_n98_), .c(new_n126_), .O(new_n155_));
  nand4  g110(.a(new_n155_), .b(new_n153_), .c(new_n130_), .d(new_n50_), .O(new_n156_));
  nand2  g111(.a(new_n156_), .b(x02), .O(new_n157_));
  nand2  g112(.a(new_n157_), .b(new_n69_), .O(z18));
endmodule


