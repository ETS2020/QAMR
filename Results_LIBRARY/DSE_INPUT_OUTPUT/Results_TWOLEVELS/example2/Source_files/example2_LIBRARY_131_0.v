// Benchmark "FAU" written by ABC on Thu Aug 20 02:46:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n166_,
    new_n168_, new_n170_, new_n172_, new_n173_, new_n174_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n197_, new_n198_, new_n200_, new_n201_, new_n203_,
    new_n204_, new_n206_, new_n207_, new_n209_, new_n210_, new_n212_,
    new_n213_, new_n215_, new_n216_, new_n218_, new_n219_, new_n221_,
    new_n222_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n230_, new_n231_, new_n232_, new_n233_, new_n234_,
    new_n235_, new_n236_, new_n237_, new_n238_, new_n240_, new_n241_,
    new_n242_, new_n243_, new_n245_, new_n246_, new_n248_, new_n249_,
    new_n250_, new_n251_, new_n252_, new_n253_, new_n254_, new_n255_,
    new_n256_, new_n258_, new_n259_, new_n260_, new_n262_, new_n264_,
    new_n266_, new_n268_, new_n270_, new_n272_, new_n274_, new_n275_,
    new_n276_, new_n277_, new_n278_, new_n279_, new_n280_, new_n281_,
    new_n282_, new_n283_, new_n284_, new_n285_, new_n287_, new_n288_,
    new_n289_, new_n290_, new_n291_, new_n292_, new_n293_, new_n294_,
    new_n295_, new_n296_, new_n297_, new_n299_, new_n301_, new_n303_,
    new_n305_, new_n307_, new_n309_, new_n311_, new_n313_, new_n315_,
    new_n317_, new_n319_, new_n321_, new_n323_, new_n324_, new_n325_,
    new_n327_, new_n329_, new_n330_, new_n332_, new_n334_, new_n336_,
    new_n338_, new_n340_, new_n341_, new_n342_, new_n343_, new_n344_,
    new_n346_, new_n347_, new_n348_, new_n349_, new_n350_, new_n352_,
    new_n353_, new_n355_, new_n356_, new_n357_, new_n358_, new_n359_,
    new_n360_, new_n362_, new_n363_, new_n364_, new_n365_, new_n366_,
    new_n367_, new_n368_, new_n369_, new_n370_, new_n371_, new_n373_,
    new_n374_, new_n375_, new_n377_, new_n378_, new_n379_, new_n380_,
    new_n382_, new_n383_, new_n384_, new_n385_, new_n387_, new_n389_,
    new_n391_, new_n392_;
  inv1   g000(.a(x06), .O(new_n152_));
  nand2  g001(.a(x52), .b(x40), .O(new_n153_));
  oai21  g002(.a(x40), .b(new_n152_), .c(new_n153_), .O(new_n154_));
  nor2   g003(.a(x77), .b(x01), .O(z54));
  inv1   g004(.a(z54), .O(new_n156_));
  nand2  g005(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  inv1   g006(.a(x01), .O(new_n158_));
  inv1   g007(.a(x79), .O(new_n159_));
  nand2  g008(.a(new_n159_), .b(x78), .O(new_n160_));
  nand4  g009(.a(new_n160_), .b(x77), .c(x40), .d(new_n158_), .O(new_n161_));
  nand2  g010(.a(new_n161_), .b(new_n157_), .O(z00));
  oai21  g011(.a(new_n159_), .b(x78), .c(x77), .O(new_n163_));
  nor2   g012(.a(new_n163_), .b(x01), .O(z01));
  inv1   g013(.a(x78), .O(new_n165_));
  nand3  g014(.a(x79), .b(new_n165_), .c(x66), .O(new_n166_));
  aoi21  g015(.a(new_n166_), .b(x77), .c(x01), .O(z02));
  nand4  g016(.a(x78), .b(x77), .c(x52), .d(new_n158_), .O(new_n168_));
  nor2   g017(.a(new_n168_), .b(x79), .O(z03));
  nand3  g018(.a(new_n159_), .b(x78), .c(x77), .O(new_n170_));
  and2   g019(.a(new_n170_), .b(new_n158_), .O(z04));
  inv1   g020(.a(x40), .O(new_n172_));
  nand2  g021(.a(new_n172_), .b(x23), .O(new_n173_));
  nand2  g022(.a(x65), .b(x40), .O(new_n174_));
  aoi21  g023(.a(new_n174_), .b(new_n173_), .c(z54), .O(z05));
  nand2  g024(.a(x64), .b(x40), .O(new_n176_));
  nand2  g025(.a(new_n172_), .b(x24), .O(new_n177_));
  nand3  g026(.a(new_n177_), .b(new_n176_), .c(new_n156_), .O(z06));
  nand2  g027(.a(new_n172_), .b(x25), .O(new_n179_));
  nand2  g028(.a(x63), .b(x40), .O(new_n180_));
  aoi21  g029(.a(new_n180_), .b(new_n179_), .c(z54), .O(z07));
  nand2  g030(.a(x62), .b(x40), .O(new_n182_));
  nand2  g031(.a(new_n172_), .b(x26), .O(new_n183_));
  nand3  g032(.a(new_n183_), .b(new_n182_), .c(new_n156_), .O(z08));
  nand2  g033(.a(new_n172_), .b(x27), .O(new_n185_));
  nand2  g034(.a(x61), .b(x40), .O(new_n186_));
  aoi21  g035(.a(new_n186_), .b(new_n185_), .c(z54), .O(z09));
  nand2  g036(.a(new_n172_), .b(x28), .O(new_n188_));
  nand2  g037(.a(x60), .b(x40), .O(new_n189_));
  aoi21  g038(.a(new_n189_), .b(new_n188_), .c(z54), .O(z10));
  nand2  g039(.a(x59), .b(x40), .O(new_n191_));
  nand2  g040(.a(new_n172_), .b(x29), .O(new_n192_));
  nand3  g041(.a(new_n192_), .b(new_n191_), .c(new_n156_), .O(z11));
  nand2  g042(.a(new_n172_), .b(x30), .O(new_n194_));
  nand2  g043(.a(x58), .b(x40), .O(new_n195_));
  aoi21  g044(.a(new_n195_), .b(new_n194_), .c(z54), .O(z12));
  nand2  g045(.a(new_n172_), .b(x31), .O(new_n197_));
  nand2  g046(.a(x57), .b(x40), .O(new_n198_));
  aoi21  g047(.a(new_n198_), .b(new_n197_), .c(z54), .O(z13));
  nand2  g048(.a(x51), .b(x40), .O(new_n200_));
  nand2  g049(.a(new_n172_), .b(x32), .O(new_n201_));
  nand3  g050(.a(new_n201_), .b(new_n200_), .c(new_n156_), .O(z14));
  nand2  g051(.a(x50), .b(x40), .O(new_n203_));
  nand2  g052(.a(new_n172_), .b(x33), .O(new_n204_));
  nand3  g053(.a(new_n204_), .b(new_n203_), .c(new_n156_), .O(z15));
  nand2  g054(.a(x49), .b(x40), .O(new_n206_));
  nand2  g055(.a(new_n172_), .b(x34), .O(new_n207_));
  nand3  g056(.a(new_n207_), .b(new_n206_), .c(new_n156_), .O(z16));
  nand2  g057(.a(x48), .b(x40), .O(new_n209_));
  nand2  g058(.a(new_n172_), .b(x35), .O(new_n210_));
  nand3  g059(.a(new_n210_), .b(new_n209_), .c(new_n156_), .O(z17));
  nand2  g060(.a(x47), .b(x40), .O(new_n212_));
  nand2  g061(.a(new_n172_), .b(x36), .O(new_n213_));
  nand3  g062(.a(new_n213_), .b(new_n212_), .c(new_n156_), .O(z18));
  nand2  g063(.a(new_n172_), .b(x37), .O(new_n215_));
  nand2  g064(.a(x46), .b(x40), .O(new_n216_));
  aoi21  g065(.a(new_n216_), .b(new_n215_), .c(z54), .O(z19));
  nand2  g066(.a(x45), .b(x40), .O(new_n218_));
  nand2  g067(.a(new_n172_), .b(x38), .O(new_n219_));
  nand3  g068(.a(new_n219_), .b(new_n218_), .c(new_n156_), .O(z20));
  nand2  g069(.a(x44), .b(x40), .O(new_n221_));
  nand2  g070(.a(new_n172_), .b(x39), .O(new_n222_));
  nand3  g071(.a(new_n222_), .b(new_n221_), .c(new_n156_), .O(z21));
  inv1   g072(.a(x41), .O(new_n224_));
  xnor2a g073(.a(x84), .b(x81), .O(new_n225_));
  nand3  g074(.a(new_n225_), .b(x79), .c(new_n165_), .O(new_n226_));
  inv1   g075(.a(new_n226_), .O(new_n227_));
  nand3  g076(.a(new_n227_), .b(x66), .c(new_n224_), .O(new_n228_));
  inv1   g077(.a(x42), .O(new_n229_));
  inv1   g078(.a(x74), .O(new_n230_));
  nand3  g079(.a(x80), .b(new_n230_), .c(x43), .O(new_n231_));
  inv1   g080(.a(x83), .O(new_n232_));
  nand4  g081(.a(x84), .b(new_n232_), .c(x82), .d(x81), .O(new_n233_));
  oai21  g082(.a(new_n233_), .b(new_n231_), .c(new_n229_), .O(new_n234_));
  nand2  g083(.a(new_n234_), .b(x79), .O(new_n235_));
  nand3  g084(.a(new_n235_), .b(x78), .c(x04), .O(new_n236_));
  nand2  g085(.a(new_n236_), .b(new_n228_), .O(new_n237_));
  nand3  g086(.a(new_n237_), .b(x77), .c(new_n158_), .O(new_n238_));
  inv1   g087(.a(new_n238_), .O(z22));
  inv1   g088(.a(x04), .O(new_n240_));
  nand3  g089(.a(new_n159_), .b(x05), .c(new_n240_), .O(new_n241_));
  nand2  g090(.a(new_n241_), .b(x00), .O(new_n242_));
  nand2  g091(.a(new_n242_), .b(x77), .O(new_n243_));
  nand2  g092(.a(new_n243_), .b(new_n158_), .O(z23));
  aoi21  g093(.a(x79), .b(new_n165_), .c(x43), .O(new_n245_));
  nand3  g094(.a(new_n245_), .b(x05), .c(new_n240_), .O(new_n246_));
  aoi21  g095(.a(new_n246_), .b(x77), .c(x01), .O(z24));
  xnor2a g096(.a(x84), .b(x82), .O(new_n248_));
  nand2  g097(.a(new_n248_), .b(x81), .O(new_n249_));
  inv1   g098(.a(x81), .O(new_n250_));
  xor2a  g099(.a(x84), .b(x82), .O(new_n251_));
  nand2  g100(.a(new_n251_), .b(new_n250_), .O(new_n252_));
  nand2  g101(.a(new_n252_), .b(new_n249_), .O(new_n253_));
  nand3  g102(.a(new_n253_), .b(x79), .c(x78), .O(new_n254_));
  inv1   g103(.a(new_n254_), .O(new_n255_));
  nand4  g104(.a(new_n255_), .b(new_n229_), .c(x05), .d(new_n240_), .O(new_n256_));
  aoi21  g105(.a(new_n256_), .b(x77), .c(x01), .O(z25));
  nand4  g106(.a(new_n253_), .b(x79), .c(x78), .d(x77), .O(new_n258_));
  inv1   g107(.a(new_n258_), .O(new_n259_));
  nand4  g108(.a(new_n259_), .b(x44), .c(new_n229_), .d(new_n240_), .O(new_n260_));
  nor2   g109(.a(new_n260_), .b(x01), .O(z26));
  nand4  g110(.a(new_n259_), .b(x45), .c(new_n229_), .d(new_n240_), .O(new_n262_));
  nor2   g111(.a(new_n262_), .b(x01), .O(z27));
  nand4  g112(.a(new_n255_), .b(x46), .c(new_n229_), .d(new_n240_), .O(new_n264_));
  aoi21  g113(.a(new_n264_), .b(x77), .c(x01), .O(z28));
  nand4  g114(.a(new_n259_), .b(x47), .c(new_n229_), .d(new_n240_), .O(new_n266_));
  nor2   g115(.a(new_n266_), .b(x01), .O(z29));
  nand4  g116(.a(new_n255_), .b(x48), .c(new_n229_), .d(new_n240_), .O(new_n268_));
  aoi21  g117(.a(new_n268_), .b(x77), .c(x01), .O(z30));
  nand4  g118(.a(new_n255_), .b(x49), .c(new_n229_), .d(new_n240_), .O(new_n270_));
  aoi21  g119(.a(new_n270_), .b(x77), .c(x01), .O(z31));
  nand4  g120(.a(new_n255_), .b(x50), .c(new_n229_), .d(new_n240_), .O(new_n272_));
  aoi21  g121(.a(new_n272_), .b(x77), .c(x01), .O(z32));
  xor2a  g122(.a(x83), .b(x81), .O(new_n274_));
  nand3  g123(.a(new_n274_), .b(x42), .c(x05), .O(new_n275_));
  nand3  g124(.a(x81), .b(x51), .c(new_n229_), .O(new_n276_));
  nand2  g125(.a(new_n276_), .b(new_n275_), .O(new_n277_));
  nand2  g126(.a(new_n277_), .b(new_n248_), .O(new_n278_));
  xnor2a g127(.a(x83), .b(x81), .O(new_n279_));
  nand3  g128(.a(new_n279_), .b(x42), .c(x05), .O(new_n280_));
  nand3  g129(.a(new_n250_), .b(x51), .c(new_n229_), .O(new_n281_));
  nand2  g130(.a(new_n281_), .b(new_n280_), .O(new_n282_));
  nand2  g131(.a(new_n282_), .b(new_n251_), .O(new_n283_));
  nand2  g132(.a(new_n283_), .b(new_n278_), .O(new_n284_));
  nand4  g133(.a(new_n284_), .b(x79), .c(x78), .d(new_n240_), .O(new_n285_));
  aoi21  g134(.a(new_n285_), .b(x77), .c(x01), .O(z33));
  aoi21  g135(.a(x83), .b(x42), .c(x81), .O(new_n287_));
  nand3  g136(.a(x83), .b(x81), .c(x42), .O(new_n288_));
  inv1   g137(.a(new_n288_), .O(new_n289_));
  oai21  g138(.a(new_n289_), .b(new_n287_), .c(new_n251_), .O(new_n290_));
  nand2  g139(.a(x83), .b(x42), .O(new_n291_));
  nand2  g140(.a(new_n291_), .b(x81), .O(new_n292_));
  nand3  g141(.a(x83), .b(new_n250_), .c(x42), .O(new_n293_));
  nand2  g142(.a(new_n293_), .b(new_n292_), .O(new_n294_));
  nand2  g143(.a(new_n294_), .b(new_n248_), .O(new_n295_));
  aoi21  g144(.a(new_n295_), .b(new_n290_), .c(new_n159_), .O(new_n296_));
  nand4  g145(.a(new_n296_), .b(x78), .c(x77), .d(x52), .O(new_n297_));
  nor3   g146(.a(new_n297_), .b(x04), .c(x01), .O(z34));
  nand4  g147(.a(new_n296_), .b(x78), .c(x77), .d(x53), .O(new_n299_));
  nor3   g148(.a(new_n299_), .b(x04), .c(x01), .O(z35));
  nand4  g149(.a(new_n296_), .b(x78), .c(x77), .d(x54), .O(new_n301_));
  nor3   g150(.a(new_n301_), .b(x04), .c(x01), .O(z36));
  nand4  g151(.a(new_n296_), .b(x78), .c(x55), .d(new_n240_), .O(new_n303_));
  aoi21  g152(.a(new_n303_), .b(x77), .c(x01), .O(z37));
  nand4  g153(.a(new_n296_), .b(x78), .c(x56), .d(new_n240_), .O(new_n305_));
  aoi21  g154(.a(new_n305_), .b(x77), .c(x01), .O(z38));
  nand4  g155(.a(new_n296_), .b(x78), .c(x77), .d(x57), .O(new_n307_));
  nor3   g156(.a(new_n307_), .b(x04), .c(x01), .O(z39));
  nand4  g157(.a(new_n296_), .b(x78), .c(x58), .d(new_n240_), .O(new_n309_));
  aoi21  g158(.a(new_n309_), .b(x77), .c(x01), .O(z40));
  nand4  g159(.a(new_n296_), .b(x78), .c(x59), .d(new_n240_), .O(new_n311_));
  aoi21  g160(.a(new_n311_), .b(x77), .c(x01), .O(z41));
  nand4  g161(.a(new_n296_), .b(x78), .c(x77), .d(x60), .O(new_n313_));
  nor3   g162(.a(new_n313_), .b(x04), .c(x01), .O(z42));
  nand4  g163(.a(new_n296_), .b(x78), .c(x61), .d(new_n240_), .O(new_n315_));
  aoi21  g164(.a(new_n315_), .b(x77), .c(x01), .O(z43));
  nand4  g165(.a(new_n296_), .b(x78), .c(x62), .d(new_n240_), .O(new_n317_));
  aoi21  g166(.a(new_n317_), .b(x77), .c(x01), .O(z44));
  nand4  g167(.a(new_n296_), .b(x78), .c(x63), .d(new_n240_), .O(new_n319_));
  aoi21  g168(.a(new_n319_), .b(x77), .c(x01), .O(z45));
  nand4  g169(.a(new_n296_), .b(x78), .c(x77), .d(x64), .O(new_n321_));
  nor3   g170(.a(new_n321_), .b(x04), .c(x01), .O(z46));
  inv1   g171(.a(x77), .O(new_n323_));
  or2    g172(.a(x75), .b(x67), .O(new_n324_));
  nand4  g173(.a(new_n324_), .b(new_n225_), .c(x79), .d(new_n165_), .O(new_n325_));
  nor3   g174(.a(new_n325_), .b(new_n323_), .c(x01), .O(z47));
  nand2  g175(.a(new_n227_), .b(x68), .O(new_n327_));
  aoi21  g176(.a(new_n327_), .b(x77), .c(x01), .O(z48));
  inv1   g177(.a(x69), .O(new_n329_));
  nand2  g178(.a(new_n227_), .b(x77), .O(new_n330_));
  nor3   g179(.a(new_n330_), .b(new_n329_), .c(x01), .O(z49));
  inv1   g180(.a(x70), .O(new_n332_));
  nor3   g181(.a(new_n330_), .b(new_n332_), .c(x01), .O(z50));
  nand2  g182(.a(new_n227_), .b(x71), .O(new_n334_));
  aoi21  g183(.a(new_n334_), .b(x77), .c(x01), .O(z51));
  nand2  g184(.a(new_n227_), .b(x72), .O(new_n336_));
  aoi21  g185(.a(new_n336_), .b(x77), .c(x01), .O(z52));
  inv1   g186(.a(x73), .O(new_n338_));
  nor3   g187(.a(new_n330_), .b(new_n338_), .c(x01), .O(z53));
  inv1   g188(.a(x84), .O(new_n340_));
  nor2   g189(.a(x04), .b(x01), .O(new_n341_));
  nand4  g190(.a(new_n341_), .b(x79), .c(x78), .d(x77), .O(new_n342_));
  nor2   g191(.a(new_n342_), .b(x80), .O(new_n343_));
  nand2  g192(.a(new_n343_), .b(new_n250_), .O(new_n344_));
  nor4   g193(.a(new_n344_), .b(new_n340_), .c(new_n232_), .d(x82), .O(z55));
  inv1   g194(.a(x00), .O(new_n346_));
  xor2a  g195(.a(x84), .b(x81), .O(new_n347_));
  aoi21  g196(.a(new_n347_), .b(new_n158_), .c(x76), .O(new_n348_));
  nor2   g197(.a(new_n348_), .b(new_n159_), .O(new_n349_));
  aoi21  g198(.a(new_n349_), .b(new_n165_), .c(new_n346_), .O(new_n350_));
  oai21  g199(.a(new_n350_), .b(new_n323_), .c(new_n158_), .O(z56));
  inv1   g200(.a(x02), .O(new_n352_));
  nand3  g201(.a(x03), .b(new_n352_), .c(x00), .O(new_n353_));
  aoi21  g202(.a(new_n353_), .b(x77), .c(x01), .O(z57));
  nand4  g203(.a(x80), .b(new_n230_), .c(x43), .d(new_n229_), .O(new_n355_));
  oai22  g204(.a(new_n355_), .b(new_n233_), .c(new_n229_), .d(x40), .O(new_n356_));
  nand4  g205(.a(new_n356_), .b(x79), .c(x78), .d(x04), .O(new_n357_));
  nand3  g206(.a(new_n165_), .b(new_n229_), .c(x40), .O(new_n358_));
  nand2  g207(.a(new_n358_), .b(x04), .O(new_n359_));
  aoi21  g208(.a(new_n359_), .b(new_n159_), .c(new_n323_), .O(new_n360_));
  aoi21  g209(.a(new_n360_), .b(new_n357_), .c(x01), .O(z58));
  nand3  g210(.a(new_n159_), .b(new_n165_), .c(x77), .O(new_n362_));
  oai21  g211(.a(new_n165_), .b(new_n240_), .c(new_n362_), .O(new_n363_));
  nand2  g212(.a(new_n363_), .b(x40), .O(new_n364_));
  oai21  g213(.a(new_n165_), .b(new_n323_), .c(x04), .O(new_n365_));
  nand2  g214(.a(new_n365_), .b(new_n159_), .O(new_n366_));
  inv1   g215(.a(new_n231_), .O(new_n367_));
  inv1   g216(.a(new_n233_), .O(new_n368_));
  aoi21  g217(.a(new_n368_), .b(new_n367_), .c(new_n165_), .O(new_n369_));
  nand4  g218(.a(new_n369_), .b(x77), .c(new_n229_), .d(x04), .O(new_n370_));
  nand4  g219(.a(new_n370_), .b(new_n366_), .c(new_n364_), .d(x77), .O(new_n371_));
  and2   g220(.a(new_n371_), .b(new_n158_), .O(z59));
  nand3  g221(.a(new_n347_), .b(x79), .c(new_n165_), .O(new_n373_));
  nand2  g222(.a(new_n159_), .b(new_n240_), .O(new_n374_));
  nand4  g223(.a(new_n374_), .b(new_n373_), .c(new_n236_), .d(x77), .O(new_n375_));
  and2   g224(.a(new_n375_), .b(new_n158_), .O(z60));
  nand2  g225(.a(new_n225_), .b(new_n165_), .O(new_n377_));
  nand2  g226(.a(x78), .b(new_n240_), .O(new_n378_));
  nand2  g227(.a(new_n378_), .b(new_n377_), .O(new_n379_));
  nand4  g228(.a(new_n379_), .b(x80), .c(x79), .d(x77), .O(new_n380_));
  nor2   g229(.a(new_n380_), .b(x01), .O(z61));
  oai21  g230(.a(new_n340_), .b(x78), .c(new_n378_), .O(new_n382_));
  nand3  g231(.a(new_n382_), .b(x81), .c(x79), .O(new_n383_));
  aoi21  g232(.a(new_n234_), .b(x79), .c(new_n165_), .O(new_n384_));
  aoi21  g233(.a(new_n384_), .b(x04), .c(new_n323_), .O(new_n385_));
  aoi21  g234(.a(new_n385_), .b(new_n383_), .c(x01), .O(z62));
  nand3  g235(.a(new_n379_), .b(x82), .c(x79), .O(new_n387_));
  aoi21  g236(.a(new_n387_), .b(x77), .c(x01), .O(z63));
  nand4  g237(.a(new_n379_), .b(x83), .c(x79), .d(x77), .O(new_n389_));
  nor2   g238(.a(new_n389_), .b(x01), .O(z64));
  oai21  g239(.a(new_n250_), .b(x78), .c(new_n378_), .O(new_n391_));
  nand3  g240(.a(new_n391_), .b(x84), .c(x79), .O(new_n392_));
  aoi21  g241(.a(new_n392_), .b(x77), .c(x01), .O(z65));
endmodule


