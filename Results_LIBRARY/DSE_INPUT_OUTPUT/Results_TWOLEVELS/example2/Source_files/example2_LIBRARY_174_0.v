// Benchmark "FAU" written by ABC on Thu Aug 20 02:47:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n175_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n200_, new_n201_,
    new_n203_, new_n204_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n212_, new_n213_, new_n215_, new_n216_, new_n218_, new_n219_,
    new_n221_, new_n222_, new_n224_, new_n225_, new_n227_, new_n228_,
    new_n229_, new_n230_, new_n231_, new_n232_, new_n233_, new_n234_,
    new_n235_, new_n236_, new_n237_, new_n238_, new_n239_, new_n240_,
    new_n241_, new_n244_, new_n245_, new_n246_, new_n248_, new_n249_,
    new_n250_, new_n251_, new_n252_, new_n253_, new_n254_, new_n255_,
    new_n256_, new_n257_, new_n259_, new_n260_, new_n261_, new_n263_,
    new_n265_, new_n267_, new_n269_, new_n271_, new_n273_, new_n275_,
    new_n276_, new_n277_, new_n278_, new_n279_, new_n280_, new_n281_,
    new_n282_, new_n283_, new_n284_, new_n285_, new_n286_, new_n288_,
    new_n289_, new_n290_, new_n291_, new_n292_, new_n293_, new_n294_,
    new_n295_, new_n296_, new_n297_, new_n298_, new_n300_, new_n302_,
    new_n304_, new_n305_, new_n307_, new_n309_, new_n311_, new_n313_,
    new_n315_, new_n317_, new_n319_, new_n321_, new_n323_, new_n325_,
    new_n326_, new_n328_, new_n329_, new_n330_, new_n332_, new_n334_,
    new_n335_, new_n337_, new_n339_, new_n341_, new_n343_, new_n344_,
    new_n345_, new_n346_, new_n347_, new_n349_, new_n350_, new_n351_,
    new_n352_, new_n353_, new_n354_, new_n356_, new_n357_, new_n359_,
    new_n360_, new_n361_, new_n363_, new_n364_, new_n366_, new_n367_,
    new_n368_, new_n369_, new_n370_, new_n371_, new_n372_, new_n373_,
    new_n375_, new_n376_, new_n377_, new_n379_, new_n380_, new_n381_,
    new_n382_, new_n383_, new_n384_, new_n385_, new_n387_, new_n389_,
    new_n391_, new_n392_, new_n393_, new_n394_, new_n395_;
  inv1   g000(.a(x06), .O(new_n152_));
  nand2  g001(.a(x52), .b(x40), .O(new_n153_));
  oai21  g002(.a(x40), .b(new_n152_), .c(new_n153_), .O(new_n154_));
  nand2  g003(.a(new_n154_), .b(x01), .O(new_n155_));
  inv1   g004(.a(x01), .O(z04));
  inv1   g005(.a(x40), .O(new_n157_));
  nand2  g006(.a(new_n157_), .b(new_n152_), .O(new_n158_));
  nand3  g007(.a(new_n158_), .b(x79), .c(z04), .O(new_n159_));
  nand2  g008(.a(new_n159_), .b(new_n155_), .O(z00));
  nand2  g009(.a(x78), .b(x77), .O(new_n161_));
  nor2   g010(.a(x78), .b(x77), .O(new_n162_));
  inv1   g011(.a(new_n162_), .O(new_n163_));
  nand2  g012(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  nand3  g013(.a(new_n164_), .b(x79), .c(z04), .O(new_n165_));
  inv1   g014(.a(new_n165_), .O(z01));
  inv1   g015(.a(x77), .O(new_n167_));
  nand3  g016(.a(x78), .b(new_n167_), .c(x75), .O(new_n168_));
  inv1   g017(.a(x79), .O(new_n169_));
  nor2   g018(.a(x78), .b(new_n167_), .O(new_n170_));
  aoi21  g019(.a(new_n170_), .b(x66), .c(new_n169_), .O(new_n171_));
  aoi21  g020(.a(new_n171_), .b(new_n168_), .c(x01), .O(z02));
  nor2   g021(.a(x79), .b(x01), .O(z54));
  inv1   g022(.a(z54), .O(new_n175_));
  nand2  g023(.a(x65), .b(x40), .O(new_n176_));
  nand2  g024(.a(new_n157_), .b(x23), .O(new_n177_));
  nand3  g025(.a(new_n177_), .b(new_n176_), .c(new_n175_), .O(z05));
  nand2  g026(.a(x64), .b(x40), .O(new_n179_));
  nand2  g027(.a(new_n157_), .b(x24), .O(new_n180_));
  nand3  g028(.a(new_n180_), .b(new_n179_), .c(new_n175_), .O(z06));
  nand2  g029(.a(x63), .b(x40), .O(new_n182_));
  nand2  g030(.a(new_n157_), .b(x25), .O(new_n183_));
  nand3  g031(.a(new_n183_), .b(new_n182_), .c(new_n175_), .O(z07));
  nand2  g032(.a(new_n157_), .b(x26), .O(new_n185_));
  nand2  g033(.a(x62), .b(x40), .O(new_n186_));
  aoi21  g034(.a(new_n186_), .b(new_n185_), .c(z54), .O(z08));
  nand2  g035(.a(new_n157_), .b(x27), .O(new_n188_));
  nand2  g036(.a(x61), .b(x40), .O(new_n189_));
  aoi21  g037(.a(new_n189_), .b(new_n188_), .c(z54), .O(z09));
  nand2  g038(.a(new_n157_), .b(x28), .O(new_n191_));
  nand2  g039(.a(x60), .b(x40), .O(new_n192_));
  aoi21  g040(.a(new_n192_), .b(new_n191_), .c(z54), .O(z10));
  nand2  g041(.a(new_n157_), .b(x29), .O(new_n194_));
  nand2  g042(.a(x59), .b(x40), .O(new_n195_));
  aoi21  g043(.a(new_n195_), .b(new_n194_), .c(z54), .O(z11));
  nand2  g044(.a(x58), .b(x40), .O(new_n197_));
  nand2  g045(.a(new_n157_), .b(x30), .O(new_n198_));
  nand3  g046(.a(new_n198_), .b(new_n197_), .c(new_n175_), .O(z12));
  nand2  g047(.a(x57), .b(x40), .O(new_n200_));
  nand2  g048(.a(new_n157_), .b(x31), .O(new_n201_));
  nand3  g049(.a(new_n201_), .b(new_n200_), .c(new_n175_), .O(z13));
  nand2  g050(.a(x51), .b(x40), .O(new_n203_));
  nand2  g051(.a(new_n157_), .b(x32), .O(new_n204_));
  nand3  g052(.a(new_n204_), .b(new_n203_), .c(new_n175_), .O(z14));
  nand2  g053(.a(new_n157_), .b(x33), .O(new_n206_));
  nand2  g054(.a(x50), .b(x40), .O(new_n207_));
  aoi21  g055(.a(new_n207_), .b(new_n206_), .c(z54), .O(z15));
  nand2  g056(.a(new_n157_), .b(x34), .O(new_n209_));
  nand2  g057(.a(x49), .b(x40), .O(new_n210_));
  aoi21  g058(.a(new_n210_), .b(new_n209_), .c(z54), .O(z16));
  nand2  g059(.a(new_n157_), .b(x35), .O(new_n212_));
  nand2  g060(.a(x48), .b(x40), .O(new_n213_));
  aoi21  g061(.a(new_n213_), .b(new_n212_), .c(z54), .O(z17));
  nand2  g062(.a(x47), .b(x40), .O(new_n215_));
  nand2  g063(.a(new_n157_), .b(x36), .O(new_n216_));
  nand3  g064(.a(new_n216_), .b(new_n215_), .c(new_n175_), .O(z18));
  nand2  g065(.a(new_n157_), .b(x37), .O(new_n218_));
  nand2  g066(.a(x46), .b(x40), .O(new_n219_));
  aoi21  g067(.a(new_n219_), .b(new_n218_), .c(z54), .O(z19));
  nand2  g068(.a(x45), .b(x40), .O(new_n221_));
  nand2  g069(.a(new_n157_), .b(x38), .O(new_n222_));
  nand3  g070(.a(new_n222_), .b(new_n221_), .c(new_n175_), .O(z20));
  nand2  g071(.a(x44), .b(x40), .O(new_n224_));
  nand2  g072(.a(new_n157_), .b(x39), .O(new_n225_));
  nand3  g073(.a(new_n225_), .b(new_n224_), .c(new_n175_), .O(z21));
  inv1   g074(.a(x41), .O(new_n227_));
  xnor2a g075(.a(x84), .b(x81), .O(new_n228_));
  nand2  g076(.a(new_n170_), .b(x66), .O(new_n229_));
  nand2  g077(.a(new_n229_), .b(new_n168_), .O(new_n230_));
  nand3  g078(.a(new_n230_), .b(new_n228_), .c(new_n227_), .O(new_n231_));
  inv1   g079(.a(x42), .O(new_n232_));
  inv1   g080(.a(x78), .O(new_n233_));
  inv1   g081(.a(x74), .O(new_n234_));
  nand3  g082(.a(x80), .b(new_n234_), .c(x43), .O(new_n235_));
  inv1   g083(.a(x83), .O(new_n236_));
  nand4  g084(.a(x84), .b(new_n236_), .c(x82), .d(x81), .O(new_n237_));
  oai21  g085(.a(new_n237_), .b(new_n235_), .c(x79), .O(new_n238_));
  nor2   g086(.a(new_n238_), .b(new_n233_), .O(new_n239_));
  nand4  g087(.a(new_n239_), .b(x77), .c(new_n232_), .d(x04), .O(new_n240_));
  nand3  g088(.a(new_n240_), .b(new_n231_), .c(x79), .O(new_n241_));
  and2   g089(.a(new_n241_), .b(z04), .O(z22));
  nand3  g090(.a(x79), .b(z04), .c(x00), .O(z23));
  inv1   g091(.a(x43), .O(new_n244_));
  nor2   g092(.a(x04), .b(x01), .O(new_n245_));
  nand4  g093(.a(new_n245_), .b(x77), .c(new_n244_), .d(x05), .O(new_n246_));
  nor3   g094(.a(new_n246_), .b(new_n169_), .c(new_n233_), .O(z24));
  inv1   g095(.a(x04), .O(new_n248_));
  xnor2a g096(.a(x84), .b(x82), .O(new_n249_));
  nand2  g097(.a(new_n249_), .b(x81), .O(new_n250_));
  inv1   g098(.a(x81), .O(new_n251_));
  xor2a  g099(.a(x84), .b(x82), .O(new_n252_));
  nand2  g100(.a(new_n252_), .b(new_n251_), .O(new_n253_));
  nand2  g101(.a(new_n253_), .b(new_n250_), .O(new_n254_));
  nand4  g102(.a(new_n254_), .b(x79), .c(x78), .d(x77), .O(new_n255_));
  inv1   g103(.a(new_n255_), .O(new_n256_));
  nand4  g104(.a(new_n256_), .b(new_n232_), .c(x05), .d(new_n248_), .O(new_n257_));
  nor2   g105(.a(new_n257_), .b(x01), .O(z25));
  nand3  g106(.a(new_n254_), .b(x78), .c(x77), .O(new_n259_));
  inv1   g107(.a(new_n259_), .O(new_n260_));
  nand4  g108(.a(new_n260_), .b(x44), .c(new_n232_), .d(new_n248_), .O(new_n261_));
  aoi21  g109(.a(new_n261_), .b(x79), .c(x01), .O(z26));
  nand4  g110(.a(new_n256_), .b(x45), .c(new_n232_), .d(new_n248_), .O(new_n263_));
  nor2   g111(.a(new_n263_), .b(x01), .O(z27));
  nand4  g112(.a(new_n260_), .b(x46), .c(new_n232_), .d(new_n248_), .O(new_n265_));
  aoi21  g113(.a(new_n265_), .b(x79), .c(x01), .O(z28));
  nand4  g114(.a(new_n260_), .b(x47), .c(new_n232_), .d(new_n248_), .O(new_n267_));
  aoi21  g115(.a(new_n267_), .b(x79), .c(x01), .O(z29));
  nand4  g116(.a(new_n260_), .b(x48), .c(new_n232_), .d(new_n248_), .O(new_n269_));
  aoi21  g117(.a(new_n269_), .b(x79), .c(x01), .O(z30));
  nand4  g118(.a(new_n260_), .b(x49), .c(new_n232_), .d(new_n248_), .O(new_n271_));
  aoi21  g119(.a(new_n271_), .b(x79), .c(x01), .O(z31));
  nand4  g120(.a(new_n260_), .b(x50), .c(new_n232_), .d(new_n248_), .O(new_n273_));
  aoi21  g121(.a(new_n273_), .b(x79), .c(x01), .O(z32));
  xor2a  g122(.a(x83), .b(x81), .O(new_n275_));
  nand3  g123(.a(new_n275_), .b(x42), .c(x05), .O(new_n276_));
  nand3  g124(.a(x81), .b(x51), .c(new_n232_), .O(new_n277_));
  nand2  g125(.a(new_n277_), .b(new_n276_), .O(new_n278_));
  nand2  g126(.a(new_n278_), .b(new_n249_), .O(new_n279_));
  xnor2a g127(.a(x83), .b(x81), .O(new_n280_));
  nand3  g128(.a(new_n280_), .b(x42), .c(x05), .O(new_n281_));
  nand3  g129(.a(new_n251_), .b(x51), .c(new_n232_), .O(new_n282_));
  nand2  g130(.a(new_n282_), .b(new_n281_), .O(new_n283_));
  nand2  g131(.a(new_n283_), .b(new_n252_), .O(new_n284_));
  nand2  g132(.a(new_n284_), .b(new_n279_), .O(new_n285_));
  nand4  g133(.a(new_n285_), .b(x78), .c(x77), .d(new_n248_), .O(new_n286_));
  aoi21  g134(.a(new_n286_), .b(x79), .c(x01), .O(z33));
  aoi21  g135(.a(x83), .b(x42), .c(x81), .O(new_n288_));
  nand3  g136(.a(x83), .b(x81), .c(x42), .O(new_n289_));
  inv1   g137(.a(new_n289_), .O(new_n290_));
  oai21  g138(.a(new_n290_), .b(new_n288_), .c(new_n252_), .O(new_n291_));
  nand2  g139(.a(x83), .b(x42), .O(new_n292_));
  nand2  g140(.a(new_n292_), .b(x81), .O(new_n293_));
  nand3  g141(.a(x83), .b(new_n251_), .c(x42), .O(new_n294_));
  nand2  g142(.a(new_n294_), .b(new_n293_), .O(new_n295_));
  nand2  g143(.a(new_n295_), .b(new_n249_), .O(new_n296_));
  aoi21  g144(.a(new_n296_), .b(new_n291_), .c(new_n169_), .O(new_n297_));
  nand4  g145(.a(new_n297_), .b(x78), .c(x77), .d(x52), .O(new_n298_));
  nor3   g146(.a(new_n298_), .b(x04), .c(x01), .O(z34));
  nand4  g147(.a(new_n297_), .b(x78), .c(x77), .d(x53), .O(new_n300_));
  nor3   g148(.a(new_n300_), .b(x04), .c(x01), .O(z35));
  nand4  g149(.a(new_n297_), .b(x78), .c(x77), .d(x54), .O(new_n302_));
  nor3   g150(.a(new_n302_), .b(x04), .c(x01), .O(z36));
  aoi21  g151(.a(new_n296_), .b(new_n291_), .c(new_n233_), .O(new_n304_));
  nand4  g152(.a(new_n304_), .b(x77), .c(x55), .d(new_n248_), .O(new_n305_));
  aoi21  g153(.a(new_n305_), .b(x79), .c(x01), .O(z37));
  nand4  g154(.a(new_n304_), .b(x77), .c(x56), .d(new_n248_), .O(new_n307_));
  aoi21  g155(.a(new_n307_), .b(x79), .c(x01), .O(z38));
  nand4  g156(.a(new_n304_), .b(x77), .c(x57), .d(new_n248_), .O(new_n309_));
  aoi21  g157(.a(new_n309_), .b(x79), .c(x01), .O(z39));
  nand4  g158(.a(new_n304_), .b(x77), .c(x58), .d(new_n248_), .O(new_n311_));
  aoi21  g159(.a(new_n311_), .b(x79), .c(x01), .O(z40));
  nand4  g160(.a(new_n297_), .b(x78), .c(x77), .d(x59), .O(new_n313_));
  nor3   g161(.a(new_n313_), .b(x04), .c(x01), .O(z41));
  nand4  g162(.a(new_n304_), .b(x77), .c(x60), .d(new_n248_), .O(new_n315_));
  aoi21  g163(.a(new_n315_), .b(x79), .c(x01), .O(z42));
  nand4  g164(.a(new_n297_), .b(x78), .c(x77), .d(x61), .O(new_n317_));
  nor3   g165(.a(new_n317_), .b(x04), .c(x01), .O(z43));
  nand4  g166(.a(new_n297_), .b(x78), .c(x77), .d(x62), .O(new_n319_));
  nor3   g167(.a(new_n319_), .b(x04), .c(x01), .O(z44));
  nand4  g168(.a(new_n297_), .b(x78), .c(x77), .d(x63), .O(new_n321_));
  nor3   g169(.a(new_n321_), .b(x04), .c(x01), .O(z45));
  nand4  g170(.a(new_n304_), .b(x77), .c(x64), .d(new_n248_), .O(new_n323_));
  aoi21  g171(.a(new_n323_), .b(x79), .c(x01), .O(z46));
  or2    g172(.a(x75), .b(x67), .O(new_n325_));
  nand4  g173(.a(new_n325_), .b(new_n228_), .c(x79), .d(new_n233_), .O(new_n326_));
  nor3   g174(.a(new_n326_), .b(new_n167_), .c(x01), .O(z47));
  nand3  g175(.a(new_n228_), .b(new_n233_), .c(x77), .O(new_n328_));
  inv1   g176(.a(new_n328_), .O(new_n329_));
  nand2  g177(.a(new_n329_), .b(x68), .O(new_n330_));
  aoi21  g178(.a(new_n330_), .b(x79), .c(x01), .O(z48));
  nand2  g179(.a(new_n329_), .b(x69), .O(new_n332_));
  aoi21  g180(.a(new_n332_), .b(x79), .c(x01), .O(z49));
  inv1   g181(.a(x70), .O(new_n334_));
  nand4  g182(.a(new_n228_), .b(x79), .c(new_n233_), .d(x77), .O(new_n335_));
  nor3   g183(.a(new_n335_), .b(new_n334_), .c(x01), .O(z50));
  inv1   g184(.a(x71), .O(new_n337_));
  nor3   g185(.a(new_n335_), .b(new_n337_), .c(x01), .O(z51));
  inv1   g186(.a(x72), .O(new_n339_));
  nor3   g187(.a(new_n335_), .b(new_n339_), .c(x01), .O(z52));
  inv1   g188(.a(x73), .O(new_n341_));
  nor3   g189(.a(new_n335_), .b(new_n341_), .c(x01), .O(z53));
  inv1   g190(.a(x84), .O(new_n343_));
  inv1   g191(.a(x82), .O(new_n344_));
  nand4  g192(.a(new_n245_), .b(x79), .c(x78), .d(x77), .O(new_n345_));
  nor2   g193(.a(new_n345_), .b(x80), .O(new_n346_));
  nand4  g194(.a(new_n346_), .b(x83), .c(new_n344_), .d(new_n251_), .O(new_n347_));
  nor2   g195(.a(new_n347_), .b(new_n343_), .O(z55));
  and2   g196(.a(new_n161_), .b(x76), .O(new_n349_));
  xor2a  g197(.a(x84), .b(x81), .O(new_n350_));
  xor2a  g198(.a(x78), .b(x77), .O(new_n351_));
  aoi21  g199(.a(new_n351_), .b(new_n350_), .c(new_n162_), .O(new_n352_));
  oai21  g200(.a(new_n352_), .b(x01), .c(x00), .O(new_n353_));
  oai21  g201(.a(new_n353_), .b(new_n349_), .c(x79), .O(new_n354_));
  nand2  g202(.a(new_n354_), .b(z04), .O(z56));
  inv1   g203(.a(x02), .O(new_n356_));
  nand4  g204(.a(x03), .b(new_n356_), .c(z04), .d(x00), .O(new_n357_));
  nor2   g205(.a(new_n357_), .b(new_n169_), .O(z57));
  nand4  g206(.a(x80), .b(new_n234_), .c(x43), .d(new_n232_), .O(new_n359_));
  oai22  g207(.a(new_n359_), .b(new_n237_), .c(new_n232_), .d(x40), .O(new_n360_));
  nand4  g208(.a(new_n360_), .b(x78), .c(x77), .d(x04), .O(new_n361_));
  aoi21  g209(.a(new_n361_), .b(x79), .c(x01), .O(z58));
  oai21  g210(.a(new_n238_), .b(x42), .c(new_n157_), .O(new_n363_));
  nand4  g211(.a(new_n363_), .b(x78), .c(x77), .d(x04), .O(new_n364_));
  aoi21  g212(.a(new_n364_), .b(x79), .c(x01), .O(z59));
  nand2  g213(.a(new_n351_), .b(new_n350_), .O(new_n366_));
  inv1   g214(.a(new_n235_), .O(new_n367_));
  nor2   g215(.a(new_n344_), .b(new_n251_), .O(new_n368_));
  nor2   g216(.a(new_n343_), .b(x83), .O(new_n369_));
  nand3  g217(.a(new_n369_), .b(new_n368_), .c(new_n367_), .O(new_n370_));
  nand4  g218(.a(new_n370_), .b(x78), .c(x77), .d(new_n232_), .O(new_n371_));
  oai21  g219(.a(new_n371_), .b(new_n248_), .c(new_n366_), .O(new_n372_));
  nand3  g220(.a(new_n372_), .b(x79), .c(z04), .O(new_n373_));
  inv1   g221(.a(new_n373_), .O(z60));
  nand2  g222(.a(new_n351_), .b(new_n228_), .O(new_n375_));
  oai21  g223(.a(new_n161_), .b(x04), .c(new_n375_), .O(new_n376_));
  nand4  g224(.a(new_n376_), .b(x80), .c(x79), .d(z04), .O(new_n377_));
  inv1   g225(.a(new_n377_), .O(z61));
  nand3  g226(.a(new_n351_), .b(x84), .c(x81), .O(new_n379_));
  oai21  g227(.a(new_n371_), .b(new_n248_), .c(new_n379_), .O(new_n380_));
  nand2  g228(.a(new_n380_), .b(x79), .O(new_n381_));
  nor2   g229(.a(new_n167_), .b(x04), .O(new_n382_));
  nand2  g230(.a(x81), .b(x78), .O(new_n383_));
  inv1   g231(.a(new_n383_), .O(new_n384_));
  aoi21  g232(.a(new_n384_), .b(new_n382_), .c(new_n169_), .O(new_n385_));
  aoi21  g233(.a(new_n385_), .b(new_n381_), .c(x01), .O(z62));
  nand4  g234(.a(new_n376_), .b(x82), .c(x79), .d(z04), .O(new_n387_));
  inv1   g235(.a(new_n387_), .O(z63));
  nand4  g236(.a(new_n376_), .b(x83), .c(x79), .d(z04), .O(new_n389_));
  inv1   g237(.a(new_n389_), .O(z64));
  nor2   g238(.a(new_n233_), .b(x04), .O(new_n391_));
  nor2   g239(.a(new_n251_), .b(x78), .O(new_n392_));
  oai21  g240(.a(new_n392_), .b(new_n391_), .c(x77), .O(new_n393_));
  oai21  g241(.a(new_n383_), .b(x77), .c(new_n393_), .O(new_n394_));
  nand4  g242(.a(new_n394_), .b(x84), .c(x79), .d(z04), .O(new_n395_));
  inv1   g243(.a(new_n395_), .O(z65));
  zero   g244(.O(z03));
endmodule


