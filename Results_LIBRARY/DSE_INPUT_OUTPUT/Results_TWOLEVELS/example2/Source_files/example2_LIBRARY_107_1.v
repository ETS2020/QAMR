// Benchmark "FAU" written by ABC on Thu Aug 20 02:46:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n169_, new_n172_, new_n173_, new_n174_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n197_, new_n198_, new_n200_, new_n201_, new_n203_,
    new_n204_, new_n206_, new_n207_, new_n209_, new_n210_, new_n212_,
    new_n213_, new_n215_, new_n216_, new_n218_, new_n219_, new_n221_,
    new_n222_, new_n224_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n230_, new_n231_, new_n232_, new_n234_, new_n235_, new_n249_,
    new_n250_, new_n251_, new_n252_, new_n253_, new_n255_, new_n256_,
    new_n257_, new_n258_, new_n260_, new_n261_, new_n262_, new_n263_,
    new_n265_, new_n266_, new_n267_, new_n268_, new_n270_, new_n271_,
    new_n272_, new_n273_, new_n275_, new_n276_, new_n277_, new_n278_,
    new_n280_, new_n281_, new_n282_, new_n283_, new_n285_, new_n286_,
    new_n287_, new_n288_, new_n290_, new_n291_, new_n292_, new_n293_,
    new_n294_, new_n295_, new_n297_, new_n298_, new_n300_, new_n302_,
    new_n304_, new_n305_, new_n306_, new_n308_, new_n310_, new_n311_,
    new_n312_, new_n314_, new_n316_, new_n317_, new_n318_, new_n319_,
    new_n321_, new_n322_, new_n323_;
  inv1   g000(.a(x06), .O(new_n152_));
  nand2  g001(.a(x52), .b(x40), .O(new_n153_));
  oai21  g002(.a(x40), .b(new_n152_), .c(new_n153_), .O(new_n154_));
  nand2  g003(.a(new_n154_), .b(x01), .O(new_n155_));
  inv1   g004(.a(x01), .O(z04));
  inv1   g005(.a(x77), .O(new_n157_));
  inv1   g006(.a(x40), .O(new_n158_));
  nand2  g007(.a(new_n158_), .b(new_n152_), .O(new_n159_));
  nand3  g008(.a(new_n159_), .b(new_n157_), .c(z04), .O(new_n160_));
  nand2  g009(.a(new_n160_), .b(new_n155_), .O(z00));
  inv1   g010(.a(x78), .O(new_n162_));
  aoi21  g011(.a(new_n162_), .b(x04), .c(x79), .O(new_n163_));
  nor2   g012(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  aoi21  g013(.a(new_n164_), .b(new_n157_), .c(x01), .O(z01));
  inv1   g014(.a(x79), .O(new_n166_));
  nand4  g015(.a(x78), .b(new_n157_), .c(x75), .d(z04), .O(new_n167_));
  nor2   g016(.a(new_n167_), .b(new_n166_), .O(z02));
  nand3  g017(.a(new_n166_), .b(x78), .c(x52), .O(new_n169_));
  aoi21  g018(.a(new_n169_), .b(new_n157_), .c(x01), .O(z03));
  nor2   g019(.a(new_n157_), .b(x01), .O(z26));
  inv1   g020(.a(z26), .O(new_n172_));
  nand2  g021(.a(x65), .b(x40), .O(new_n173_));
  nand2  g022(.a(new_n158_), .b(x23), .O(new_n174_));
  nand3  g023(.a(new_n174_), .b(new_n173_), .c(new_n172_), .O(z05));
  nand2  g024(.a(x64), .b(x40), .O(new_n176_));
  nand2  g025(.a(new_n158_), .b(x24), .O(new_n177_));
  nand3  g026(.a(new_n177_), .b(new_n176_), .c(new_n172_), .O(z06));
  nand2  g027(.a(x63), .b(x40), .O(new_n179_));
  nand2  g028(.a(new_n158_), .b(x25), .O(new_n180_));
  nand3  g029(.a(new_n180_), .b(new_n179_), .c(new_n172_), .O(z07));
  nand2  g030(.a(new_n158_), .b(x26), .O(new_n182_));
  nand2  g031(.a(x62), .b(x40), .O(new_n183_));
  aoi21  g032(.a(new_n183_), .b(new_n182_), .c(z26), .O(z08));
  nand2  g033(.a(new_n158_), .b(x27), .O(new_n185_));
  nand2  g034(.a(x61), .b(x40), .O(new_n186_));
  aoi21  g035(.a(new_n186_), .b(new_n185_), .c(z26), .O(z09));
  nand2  g036(.a(x60), .b(x40), .O(new_n188_));
  nand2  g037(.a(new_n158_), .b(x28), .O(new_n189_));
  nand3  g038(.a(new_n189_), .b(new_n188_), .c(new_n172_), .O(z10));
  nand2  g039(.a(x59), .b(x40), .O(new_n191_));
  nand2  g040(.a(new_n158_), .b(x29), .O(new_n192_));
  nand3  g041(.a(new_n192_), .b(new_n191_), .c(new_n172_), .O(z11));
  nand2  g042(.a(new_n158_), .b(x30), .O(new_n194_));
  nand2  g043(.a(x58), .b(x40), .O(new_n195_));
  aoi21  g044(.a(new_n195_), .b(new_n194_), .c(z26), .O(z12));
  nand2  g045(.a(x57), .b(x40), .O(new_n197_));
  nand2  g046(.a(new_n158_), .b(x31), .O(new_n198_));
  nand3  g047(.a(new_n198_), .b(new_n197_), .c(new_n172_), .O(z13));
  nand2  g048(.a(x51), .b(x40), .O(new_n200_));
  nand2  g049(.a(new_n158_), .b(x32), .O(new_n201_));
  nand3  g050(.a(new_n201_), .b(new_n200_), .c(new_n172_), .O(z14));
  nand2  g051(.a(new_n158_), .b(x33), .O(new_n203_));
  nand2  g052(.a(x50), .b(x40), .O(new_n204_));
  aoi21  g053(.a(new_n204_), .b(new_n203_), .c(z26), .O(z15));
  nand2  g054(.a(x49), .b(x40), .O(new_n206_));
  nand2  g055(.a(new_n158_), .b(x34), .O(new_n207_));
  nand3  g056(.a(new_n207_), .b(new_n206_), .c(new_n172_), .O(z16));
  nand2  g057(.a(new_n158_), .b(x35), .O(new_n209_));
  nand2  g058(.a(x48), .b(x40), .O(new_n210_));
  aoi21  g059(.a(new_n210_), .b(new_n209_), .c(z26), .O(z17));
  nand2  g060(.a(x47), .b(x40), .O(new_n212_));
  nand2  g061(.a(new_n158_), .b(x36), .O(new_n213_));
  nand3  g062(.a(new_n213_), .b(new_n212_), .c(new_n172_), .O(z18));
  nand2  g063(.a(new_n158_), .b(x37), .O(new_n215_));
  nand2  g064(.a(x46), .b(x40), .O(new_n216_));
  aoi21  g065(.a(new_n216_), .b(new_n215_), .c(z26), .O(z19));
  nand2  g066(.a(x45), .b(x40), .O(new_n218_));
  nand2  g067(.a(new_n158_), .b(x38), .O(new_n219_));
  nand3  g068(.a(new_n219_), .b(new_n218_), .c(new_n172_), .O(z20));
  nand2  g069(.a(new_n158_), .b(x39), .O(new_n221_));
  nand2  g070(.a(x44), .b(x40), .O(new_n222_));
  aoi21  g071(.a(new_n222_), .b(new_n221_), .c(z26), .O(z21));
  xnor2a g072(.a(x84), .b(x81), .O(new_n224_));
  nand3  g073(.a(new_n224_), .b(x79), .c(x75), .O(new_n225_));
  nand2  g074(.a(new_n166_), .b(x04), .O(new_n226_));
  oai21  g075(.a(new_n225_), .b(x41), .c(new_n226_), .O(new_n227_));
  nand4  g076(.a(new_n227_), .b(x78), .c(new_n157_), .d(z04), .O(new_n228_));
  inv1   g077(.a(new_n228_), .O(z22));
  nand2  g078(.a(new_n166_), .b(x05), .O(new_n230_));
  oai21  g079(.a(new_n230_), .b(x04), .c(x00), .O(new_n231_));
  nand2  g080(.a(new_n231_), .b(new_n157_), .O(new_n232_));
  nand2  g081(.a(new_n232_), .b(z04), .O(z23));
  nor2   g082(.a(x04), .b(x01), .O(new_n234_));
  nand2  g083(.a(new_n234_), .b(x05), .O(new_n235_));
  nor4   g084(.a(new_n235_), .b(x79), .c(x77), .d(x43), .O(z24));
  nand2  g085(.a(x52), .b(x15), .O(new_n249_));
  inv1   g086(.a(x52), .O(new_n250_));
  nand2  g087(.a(new_n250_), .b(x07), .O(new_n251_));
  aoi21  g088(.a(new_n251_), .b(new_n249_), .c(x79), .O(new_n252_));
  nand4  g089(.a(new_n252_), .b(x78), .c(new_n157_), .d(x04), .O(new_n253_));
  nor2   g090(.a(new_n253_), .b(x01), .O(z47));
  nand2  g091(.a(x52), .b(x16), .O(new_n255_));
  nand2  g092(.a(new_n250_), .b(x08), .O(new_n256_));
  aoi21  g093(.a(new_n256_), .b(new_n255_), .c(x79), .O(new_n257_));
  nand4  g094(.a(new_n257_), .b(x78), .c(new_n157_), .d(x04), .O(new_n258_));
  nor2   g095(.a(new_n258_), .b(x01), .O(z48));
  nand2  g096(.a(x52), .b(x17), .O(new_n260_));
  nand2  g097(.a(new_n250_), .b(x09), .O(new_n261_));
  aoi21  g098(.a(new_n261_), .b(new_n260_), .c(x79), .O(new_n262_));
  nand4  g099(.a(new_n262_), .b(x78), .c(new_n157_), .d(x04), .O(new_n263_));
  nor2   g100(.a(new_n263_), .b(x01), .O(z49));
  inv1   g101(.a(x10), .O(new_n265_));
  nand2  g102(.a(x52), .b(x18), .O(new_n266_));
  oai21  g103(.a(x52), .b(new_n265_), .c(new_n266_), .O(new_n267_));
  nand4  g104(.a(new_n267_), .b(new_n166_), .c(x78), .d(x04), .O(new_n268_));
  aoi21  g105(.a(new_n268_), .b(new_n157_), .c(x01), .O(z50));
  inv1   g106(.a(x11), .O(new_n270_));
  nand2  g107(.a(x52), .b(x19), .O(new_n271_));
  oai21  g108(.a(x52), .b(new_n270_), .c(new_n271_), .O(new_n272_));
  nand4  g109(.a(new_n272_), .b(new_n166_), .c(x78), .d(x04), .O(new_n273_));
  aoi21  g110(.a(new_n273_), .b(new_n157_), .c(x01), .O(z51));
  nand2  g111(.a(x52), .b(x20), .O(new_n275_));
  nand2  g112(.a(new_n250_), .b(x12), .O(new_n276_));
  aoi21  g113(.a(new_n276_), .b(new_n275_), .c(x79), .O(new_n277_));
  nand4  g114(.a(new_n277_), .b(x78), .c(new_n157_), .d(x04), .O(new_n278_));
  nor2   g115(.a(new_n278_), .b(x01), .O(z52));
  inv1   g116(.a(x13), .O(new_n280_));
  nand2  g117(.a(x52), .b(x21), .O(new_n281_));
  oai21  g118(.a(x52), .b(new_n280_), .c(new_n281_), .O(new_n282_));
  nand4  g119(.a(new_n282_), .b(new_n166_), .c(x78), .d(x04), .O(new_n283_));
  aoi21  g120(.a(new_n283_), .b(new_n157_), .c(x01), .O(z53));
  nand2  g121(.a(x52), .b(x22), .O(new_n285_));
  nand2  g122(.a(new_n250_), .b(x14), .O(new_n286_));
  aoi21  g123(.a(new_n286_), .b(new_n285_), .c(x79), .O(new_n287_));
  nand4  g124(.a(new_n287_), .b(x78), .c(new_n157_), .d(x04), .O(new_n288_));
  nor2   g125(.a(new_n288_), .b(x01), .O(z54));
  xor2a  g126(.a(x84), .b(x81), .O(new_n290_));
  nand2  g127(.a(new_n290_), .b(x79), .O(new_n291_));
  nand2  g128(.a(new_n291_), .b(x78), .O(new_n292_));
  nand2  g129(.a(x79), .b(x76), .O(new_n293_));
  nand2  g130(.a(new_n293_), .b(x00), .O(new_n294_));
  aoi21  g131(.a(new_n292_), .b(z04), .c(new_n294_), .O(new_n295_));
  oai21  g132(.a(new_n295_), .b(x77), .c(z04), .O(z56));
  inv1   g133(.a(x02), .O(new_n297_));
  nand4  g134(.a(x03), .b(new_n297_), .c(z04), .d(x00), .O(new_n298_));
  nor2   g135(.a(new_n298_), .b(x77), .O(z57));
  nand3  g136(.a(new_n163_), .b(new_n157_), .c(z04), .O(new_n300_));
  inv1   g137(.a(new_n300_), .O(z58));
  nand3  g138(.a(new_n234_), .b(new_n166_), .c(new_n157_), .O(new_n302_));
  inv1   g139(.a(new_n302_), .O(z59));
  aoi21  g140(.a(new_n291_), .b(new_n226_), .c(new_n162_), .O(new_n304_));
  nor2   g141(.a(x79), .b(x04), .O(new_n305_));
  oai21  g142(.a(new_n305_), .b(new_n304_), .c(new_n157_), .O(new_n306_));
  nor2   g143(.a(new_n306_), .b(x01), .O(z60));
  nand4  g144(.a(new_n224_), .b(x80), .c(x79), .d(x78), .O(new_n308_));
  aoi21  g145(.a(new_n308_), .b(new_n157_), .c(x01), .O(z61));
  nand3  g146(.a(x84), .b(x81), .c(x79), .O(new_n310_));
  nand2  g147(.a(new_n310_), .b(new_n226_), .O(new_n311_));
  nand4  g148(.a(new_n311_), .b(x78), .c(new_n157_), .d(z04), .O(new_n312_));
  inv1   g149(.a(new_n312_), .O(z62));
  nand4  g150(.a(new_n224_), .b(x82), .c(x79), .d(x78), .O(new_n314_));
  aoi21  g151(.a(new_n314_), .b(new_n157_), .c(x01), .O(z63));
  nand3  g152(.a(new_n224_), .b(x83), .c(x79), .O(new_n316_));
  nand3  g153(.a(new_n166_), .b(new_n157_), .c(x04), .O(new_n317_));
  nand2  g154(.a(new_n317_), .b(new_n316_), .O(new_n318_));
  nand2  g155(.a(new_n318_), .b(x78), .O(new_n319_));
  aoi21  g156(.a(new_n319_), .b(new_n157_), .c(x01), .O(z64));
  inv1   g157(.a(x84), .O(new_n321_));
  nor2   g158(.a(x77), .b(x01), .O(new_n322_));
  nand4  g159(.a(new_n322_), .b(x81), .c(x79), .d(x78), .O(new_n323_));
  nor2   g160(.a(new_n323_), .b(new_n321_), .O(z65));
  zero   g161(.O(z25));
  zero   g162(.O(z28));
  zero   g163(.O(z31));
  zero   g164(.O(z33));
  zero   g165(.O(z34));
  zero   g166(.O(z35));
  zero   g167(.O(z36));
  zero   g168(.O(z37));
  zero   g169(.O(z38));
  zero   g170(.O(z41));
  zero   g171(.O(z43));
  zero   g172(.O(z44));
  nor2   g173(.a(new_n157_), .b(x01), .O(z27));
  nor2   g174(.a(new_n157_), .b(x01), .O(z29));
  nor2   g175(.a(new_n157_), .b(x01), .O(z30));
  nor2   g176(.a(new_n157_), .b(x01), .O(z32));
  nor2   g177(.a(new_n157_), .b(x01), .O(z39));
  nor2   g178(.a(new_n157_), .b(x01), .O(z40));
  nor2   g179(.a(new_n157_), .b(x01), .O(z42));
  nor2   g180(.a(new_n157_), .b(x01), .O(z45));
  nor2   g181(.a(new_n157_), .b(x01), .O(z46));
  nor2   g182(.a(new_n157_), .b(x01), .O(z55));
endmodule


