// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  oai21  g02(.a(x04), .b(x01), .c(new_n27_), .O(new_n28_));
  nand2  g03(.a(x05), .b(x02), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n29_), .c(new_n28_), .d(new_n26_), .O(z00));
  inv1   g08(.a(x01), .O(new_n34_));
  nand3  g09(.a(x05), .b(new_n30_), .c(new_n26_), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  nor2   g12(.a(x03), .b(new_n27_), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(new_n31_), .c(x04), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n37_), .O(z01));
  nor2   g15(.a(x01), .b(x00), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(new_n30_), .c(new_n26_), .d(new_n27_), .O(new_n42_));
  nor3   g17(.a(new_n42_), .b(x13), .c(new_n31_), .O(z02));
  inv1   g18(.a(x13), .O(new_n44_));
  inv1   g19(.a(x00), .O(new_n45_));
  nor2   g20(.a(x03), .b(new_n45_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n44_), .c(x05), .d(new_n30_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(new_n34_), .c(x02), .O(z03));
  nor2   g23(.a(x02), .b(new_n34_), .O(z04));
  inv1   g24(.a(x07), .O(new_n51_));
  inv1   g25(.a(x12), .O(new_n52_));
  inv1   g26(.a(x08), .O(new_n53_));
  nand2  g27(.a(new_n53_), .b(new_n45_), .O(new_n54_));
  inv1   g28(.a(x09), .O(new_n55_));
  nand2  g29(.a(new_n55_), .b(x00), .O(new_n56_));
  aoi21  g30(.a(new_n56_), .b(new_n54_), .c(new_n44_), .O(new_n57_));
  nand4  g31(.a(new_n57_), .b(new_n52_), .c(new_n51_), .d(x05), .O(new_n58_));
  nor2   g32(.a(new_n58_), .b(new_n30_), .O(new_n59_));
  aoi21  g33(.a(new_n59_), .b(new_n26_), .c(x01), .O(new_n60_));
  nand4  g34(.a(new_n38_), .b(new_n44_), .c(new_n31_), .d(x04), .O(new_n61_));
  oai21  g35(.a(new_n60_), .b(x02), .c(new_n61_), .O(z06));
  aoi21  g36(.a(x01), .b(new_n45_), .c(x13), .O(new_n63_));
  nand4  g37(.a(new_n63_), .b(x05), .c(new_n30_), .d(new_n26_), .O(new_n64_));
  aoi21  g38(.a(new_n64_), .b(new_n34_), .c(x02), .O(z07));
  nand2  g39(.a(x04), .b(new_n26_), .O(new_n66_));
  nand3  g40(.a(x13), .b(new_n52_), .c(x05), .O(new_n67_));
  oai21  g41(.a(new_n67_), .b(new_n66_), .c(new_n34_), .O(new_n68_));
  nand2  g42(.a(new_n68_), .b(new_n27_), .O(new_n69_));
  nand2  g43(.a(new_n69_), .b(new_n61_), .O(z08));
  nand2  g44(.a(x13), .b(x06), .O(new_n71_));
  oai21  g45(.a(x02), .b(new_n34_), .c(new_n71_), .O(z09));
  nand2  g46(.a(new_n27_), .b(new_n34_), .O(new_n73_));
  oai21  g47(.a(x05), .b(new_n27_), .c(new_n73_), .O(new_n74_));
  nand3  g48(.a(new_n74_), .b(x06), .c(new_n26_), .O(new_n75_));
  inv1   g49(.a(new_n75_), .O(z10));
  zero   g50(.O(z05));
endmodule


