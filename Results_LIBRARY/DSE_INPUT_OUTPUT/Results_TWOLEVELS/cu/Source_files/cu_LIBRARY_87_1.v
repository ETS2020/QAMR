// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n79_;
  inv1   g00(.a(x13), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x08), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  aoi21  g03(.a(x04), .b(new_n28_), .c(x03), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nor2   g06(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  aoi21  g07(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n29_), .c(new_n27_), .O(z00));
  inv1   g09(.a(new_n27_), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(x05), .c(new_n30_), .d(new_n28_), .O(new_n36_));
  nand4  g11(.a(new_n35_), .b(new_n31_), .c(x04), .d(x02), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n36_), .c(x03), .O(z01));
  inv1   g13(.a(x03), .O(new_n39_));
  nor3   g14(.a(x02), .b(x01), .c(x00), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(x05), .c(new_n30_), .d(new_n39_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x13), .O(z02));
  inv1   g17(.a(x01), .O(new_n43_));
  nand4  g18(.a(new_n39_), .b(new_n28_), .c(new_n43_), .d(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n26_), .c(x05), .d(new_n30_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z03));
  inv1   g22(.a(x00), .O(new_n48_));
  nand3  g23(.a(new_n28_), .b(x01), .c(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n26_), .b(x05), .c(new_n30_), .d(new_n39_), .O(new_n50_));
  oai21  g25(.a(new_n50_), .b(new_n49_), .c(new_n35_), .O(z04));
  nor3   g26(.a(x02), .b(new_n43_), .c(new_n48_), .O(new_n52_));
  nand4  g27(.a(new_n52_), .b(x05), .c(new_n30_), .d(new_n39_), .O(new_n53_));
  nor2   g28(.a(new_n53_), .b(x13), .O(z05));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  inv1   g31(.a(x11), .O(new_n57_));
  nor2   g32(.a(x09), .b(x01), .O(new_n58_));
  aoi21  g33(.a(new_n57_), .b(x01), .c(new_n58_), .O(new_n59_));
  inv1   g34(.a(x10), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(x01), .c(new_n48_), .O(new_n61_));
  oai21  g36(.a(new_n59_), .b(new_n48_), .c(new_n61_), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(x13), .c(new_n56_), .d(x08), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n55_), .c(x05), .d(new_n28_), .O(new_n65_));
  nand3  g40(.a(new_n26_), .b(new_n31_), .c(x02), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(x04), .c(new_n39_), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(z06));
  nand4  g44(.a(new_n26_), .b(x05), .c(new_n30_), .d(new_n39_), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(x02), .O(z07));
  nand4  g46(.a(x13), .b(new_n56_), .c(x05), .d(new_n28_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n66_), .O(new_n73_));
  nand3  g48(.a(new_n73_), .b(x04), .c(new_n39_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n35_), .O(z08));
  nand3  g50(.a(x13), .b(x08), .c(x06), .O(new_n76_));
  inv1   g51(.a(new_n76_), .O(z09));
  inv1   g52(.a(new_n32_), .O(new_n78_));
  nand4  g53(.a(new_n78_), .b(new_n35_), .c(x06), .d(new_n39_), .O(new_n79_));
  inv1   g54(.a(new_n79_), .O(z10));
endmodule


