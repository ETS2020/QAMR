// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_;
  inv1   g00(.a(x04), .O(new_n26_));
  nor2   g01(.a(x03), .b(x02), .O(new_n27_));
  nand3  g02(.a(new_n27_), .b(x05), .c(new_n26_), .O(z00));
  nand3  g03(.a(new_n27_), .b(x05), .c(new_n26_), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(z01));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x02), .O(new_n32_));
  inv1   g07(.a(x00), .O(new_n33_));
  inv1   g08(.a(x01), .O(new_n34_));
  inv1   g09(.a(x02), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  inv1   g11(.a(x03), .O(new_n37_));
  inv1   g12(.a(x13), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(x05), .c(new_n26_), .d(new_n37_), .O(new_n39_));
  oai21  g14(.a(new_n39_), .b(new_n36_), .c(new_n32_), .O(z02));
  nand3  g15(.a(new_n35_), .b(new_n34_), .c(x00), .O(new_n41_));
  oai21  g16(.a(new_n41_), .b(new_n39_), .c(new_n32_), .O(z03));
  nand3  g17(.a(new_n35_), .b(x01), .c(new_n33_), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n39_), .c(new_n32_), .O(z04));
  nor3   g19(.a(x02), .b(new_n34_), .c(new_n33_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(x05), .c(new_n26_), .d(new_n37_), .O(new_n46_));
  nor2   g21(.a(new_n46_), .b(x13), .O(z05));
  inv1   g22(.a(x07), .O(new_n48_));
  inv1   g23(.a(x12), .O(new_n49_));
  nor2   g24(.a(x09), .b(x01), .O(new_n50_));
  nor2   g25(.a(x11), .b(new_n34_), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n50_), .c(x00), .O(new_n52_));
  nor2   g27(.a(x08), .b(x01), .O(new_n53_));
  nor2   g28(.a(x10), .b(new_n34_), .O(new_n54_));
  oai21  g29(.a(new_n54_), .b(new_n53_), .c(new_n33_), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand4  g31(.a(new_n56_), .b(x13), .c(new_n49_), .d(new_n48_), .O(new_n57_));
  nor2   g32(.a(new_n57_), .b(new_n31_), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(x04), .c(new_n37_), .d(new_n35_), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n32_), .O(z06));
  nand4  g35(.a(new_n38_), .b(x05), .c(new_n26_), .d(new_n37_), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(x02), .O(z07));
  nand4  g37(.a(new_n27_), .b(new_n49_), .c(x05), .d(x04), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(new_n38_), .O(z08));
  nand2  g39(.a(x13), .b(x06), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n32_), .O(z09));
  nand3  g41(.a(x06), .b(new_n37_), .c(new_n35_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n32_), .O(z10));
endmodule


