// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n82_, new_n83_;
  inv1   g00(.a(x01), .O(new_n26_));
  nor2   g01(.a(x04), .b(new_n26_), .O(z05));
  inv1   g02(.a(z05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nor2   g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nor2   g07(.a(x05), .b(x04), .O(new_n33_));
  oai21  g08(.a(new_n33_), .b(new_n32_), .c(new_n26_), .O(new_n34_));
  oai21  g09(.a(x05), .b(new_n30_), .c(x04), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(new_n34_), .c(new_n29_), .O(z00));
  inv1   g11(.a(x04), .O(new_n37_));
  nor2   g12(.a(x02), .b(x01), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(x05), .c(new_n37_), .O(new_n39_));
  nand3  g14(.a(new_n31_), .b(x04), .c(x02), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(new_n39_), .c(x03), .O(z01));
  inv1   g16(.a(x00), .O(new_n42_));
  inv1   g17(.a(x03), .O(new_n43_));
  nor2   g18(.a(x13), .b(new_n31_), .O(new_n44_));
  nand4  g19(.a(new_n44_), .b(new_n38_), .c(new_n43_), .d(new_n42_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n26_), .c(x04), .O(z02));
  inv1   g21(.a(x13), .O(new_n47_));
  nand4  g22(.a(new_n43_), .b(new_n30_), .c(new_n26_), .d(x00), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n47_), .c(x05), .d(new_n37_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z03));
  inv1   g26(.a(x07), .O(new_n53_));
  inv1   g27(.a(x12), .O(new_n54_));
  inv1   g28(.a(x09), .O(new_n55_));
  nand3  g29(.a(new_n55_), .b(x04), .c(new_n26_), .O(new_n56_));
  inv1   g30(.a(x11), .O(new_n57_));
  nand2  g31(.a(new_n57_), .b(x01), .O(new_n58_));
  nand2  g32(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand2  g33(.a(new_n59_), .b(x00), .O(new_n60_));
  inv1   g34(.a(x08), .O(new_n61_));
  nand3  g35(.a(new_n61_), .b(x04), .c(new_n26_), .O(new_n62_));
  inv1   g36(.a(x10), .O(new_n63_));
  nand2  g37(.a(new_n63_), .b(x01), .O(new_n64_));
  nand2  g38(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g39(.a(new_n65_), .b(new_n42_), .O(new_n66_));
  aoi21  g40(.a(new_n66_), .b(new_n60_), .c(new_n47_), .O(new_n67_));
  nand4  g41(.a(new_n67_), .b(new_n54_), .c(new_n53_), .d(x05), .O(new_n68_));
  nand4  g42(.a(new_n47_), .b(new_n31_), .c(x04), .d(x02), .O(new_n69_));
  oai21  g43(.a(new_n68_), .b(x02), .c(new_n69_), .O(new_n70_));
  nand2  g44(.a(new_n70_), .b(new_n43_), .O(new_n71_));
  nand2  g45(.a(new_n71_), .b(new_n28_), .O(z06));
  nand3  g46(.a(new_n44_), .b(new_n37_), .c(new_n43_), .O(new_n73_));
  nor3   g47(.a(new_n73_), .b(x02), .c(x01), .O(z07));
  nand4  g48(.a(x13), .b(new_n54_), .c(x05), .d(new_n30_), .O(new_n75_));
  nand3  g49(.a(new_n47_), .b(new_n31_), .c(x02), .O(new_n76_));
  nand2  g50(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g51(.a(new_n77_), .b(x04), .c(new_n43_), .O(new_n78_));
  inv1   g52(.a(new_n78_), .O(z08));
  inv1   g53(.a(x06), .O(new_n80_));
  nor3   g54(.a(z05), .b(new_n47_), .c(new_n80_), .O(z09));
  inv1   g55(.a(new_n32_), .O(new_n82_));
  nand3  g56(.a(new_n82_), .b(x06), .c(new_n43_), .O(new_n83_));
  nand2  g57(.a(new_n83_), .b(new_n28_), .O(z10));
  zero   g58(.O(z04));
endmodule


