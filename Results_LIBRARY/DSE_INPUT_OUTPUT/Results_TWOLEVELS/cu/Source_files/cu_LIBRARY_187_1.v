// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n88_;
  inv1   g00(.a(x10), .O(new_n26_));
  oai21  g01(.a(new_n26_), .b(x02), .c(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  oai21  g03(.a(x05), .b(new_n28_), .c(x02), .O(new_n29_));
  nor2   g04(.a(new_n28_), .b(x02), .O(new_n30_));
  nor2   g05(.a(x05), .b(x04), .O(new_n31_));
  oai21  g06(.a(new_n31_), .b(new_n30_), .c(new_n26_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(z00));
  inv1   g08(.a(x02), .O(new_n34_));
  inv1   g09(.a(x03), .O(new_n35_));
  nand3  g10(.a(x05), .b(new_n28_), .c(new_n35_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n26_), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  inv1   g13(.a(x05), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(x04), .c(new_n35_), .d(x02), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n38_), .O(z01));
  inv1   g16(.a(x00), .O(new_n42_));
  inv1   g17(.a(x01), .O(new_n43_));
  inv1   g18(.a(x13), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(x05), .c(new_n28_), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n35_), .c(new_n43_), .d(new_n42_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(new_n26_), .c(x02), .O(z02));
  nand4  g23(.a(new_n35_), .b(new_n34_), .c(new_n43_), .d(x00), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n26_), .c(x05), .d(new_n28_), .O(new_n51_));
  nor2   g26(.a(new_n51_), .b(x13), .O(z03));
  nand4  g27(.a(new_n35_), .b(new_n34_), .c(x01), .d(new_n42_), .O(new_n53_));
  inv1   g28(.a(new_n53_), .O(new_n54_));
  nand4  g29(.a(new_n54_), .b(new_n26_), .c(x05), .d(new_n28_), .O(new_n55_));
  nor2   g30(.a(new_n55_), .b(x13), .O(z04));
  nand4  g31(.a(new_n46_), .b(new_n35_), .c(x01), .d(x00), .O(new_n57_));
  aoi21  g32(.a(new_n57_), .b(new_n26_), .c(x02), .O(z05));
  inv1   g33(.a(x07), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  inv1   g35(.a(x08), .O(new_n61_));
  oai21  g36(.a(new_n61_), .b(x01), .c(new_n42_), .O(new_n62_));
  inv1   g37(.a(x09), .O(new_n63_));
  nand3  g38(.a(new_n63_), .b(new_n43_), .c(x00), .O(new_n64_));
  inv1   g39(.a(x11), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(x01), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(x13), .c(new_n60_), .d(new_n26_), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(new_n69_));
  nand4  g44(.a(new_n69_), .b(new_n59_), .c(x05), .d(new_n34_), .O(new_n70_));
  nand3  g45(.a(new_n44_), .b(new_n39_), .c(x02), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(x04), .c(new_n35_), .O(new_n73_));
  inv1   g48(.a(new_n73_), .O(z06));
  oai21  g49(.a(new_n26_), .b(x01), .c(x00), .O(new_n75_));
  oai21  g50(.a(new_n26_), .b(new_n43_), .c(new_n42_), .O(new_n76_));
  aoi21  g51(.a(new_n76_), .b(new_n75_), .c(x13), .O(new_n77_));
  nand4  g52(.a(new_n77_), .b(x05), .c(new_n28_), .d(new_n35_), .O(new_n78_));
  aoi21  g53(.a(new_n78_), .b(new_n26_), .c(x02), .O(z07));
  nand2  g54(.a(x05), .b(new_n34_), .O(new_n80_));
  nand3  g55(.a(x13), .b(new_n60_), .c(new_n26_), .O(new_n81_));
  oai21  g56(.a(new_n81_), .b(new_n80_), .c(new_n71_), .O(new_n82_));
  nand3  g57(.a(new_n82_), .b(x04), .c(new_n35_), .O(new_n83_));
  inv1   g58(.a(new_n83_), .O(z08));
  nand2  g59(.a(x13), .b(x06), .O(new_n85_));
  oai21  g60(.a(new_n26_), .b(x02), .c(new_n85_), .O(z09));
  aoi21  g61(.a(x06), .b(new_n35_), .c(x10), .O(new_n87_));
  nand3  g62(.a(x06), .b(new_n39_), .c(new_n35_), .O(new_n88_));
  oai21  g63(.a(new_n87_), .b(x02), .c(new_n88_), .O(z10));
endmodule


