// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n56_, new_n58_;
  inv1   g00(.a(x03), .O(new_n26_));
  oai21  g01(.a(x13), .b(x02), .c(x05), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nand2  g03(.a(x04), .b(new_n28_), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n29_), .c(new_n27_), .d(new_n26_), .O(z00));
  nand3  g08(.a(new_n31_), .b(x04), .c(x02), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(x05), .c(new_n30_), .d(new_n28_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(new_n34_), .c(x03), .O(z01));
  nor3   g12(.a(x02), .b(x01), .c(x00), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(x05), .c(new_n30_), .d(new_n26_), .O(new_n39_));
  nor2   g14(.a(new_n39_), .b(x13), .O(z02));
  inv1   g15(.a(x00), .O(new_n41_));
  nor2   g16(.a(x01), .b(new_n41_), .O(new_n42_));
  nor3   g17(.a(x04), .b(x03), .c(x02), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(new_n35_), .c(new_n31_), .O(z03));
  nand3  g20(.a(new_n43_), .b(x01), .c(new_n41_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n35_), .c(new_n31_), .O(z04));
  nand2  g22(.a(x01), .b(x00), .O(new_n48_));
  nor2   g23(.a(new_n48_), .b(x02), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(x05), .c(new_n30_), .d(new_n26_), .O(new_n50_));
  nor2   g25(.a(new_n50_), .b(x13), .O(z05));
  nand4  g26(.a(new_n31_), .b(x04), .c(new_n26_), .d(x02), .O(new_n52_));
  nor2   g27(.a(new_n52_), .b(x13), .O(z06));
  nand4  g28(.a(new_n35_), .b(new_n30_), .c(new_n26_), .d(new_n28_), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n35_), .c(new_n31_), .O(z07));
  nand3  g30(.a(x13), .b(x06), .c(new_n31_), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(z09));
  nand3  g32(.a(new_n27_), .b(x06), .c(new_n26_), .O(new_n58_));
  inv1   g33(.a(new_n58_), .O(z10));
  nor2   g34(.a(new_n52_), .b(x13), .O(z08));
endmodule


