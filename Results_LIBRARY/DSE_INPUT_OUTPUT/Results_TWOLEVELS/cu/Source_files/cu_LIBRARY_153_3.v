// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_;
  inv1   g00(.a(x03), .O(new_n26_));
  nor2   g01(.a(x02), .b(x01), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x05), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  nand2  g05(.a(x04), .b(new_n30_), .O(new_n31_));
  inv1   g06(.a(x04), .O(new_n32_));
  inv1   g07(.a(x05), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g09(.a(new_n34_), .b(new_n31_), .c(new_n29_), .d(new_n26_), .O(z00));
  nand3  g10(.a(x05), .b(new_n32_), .c(new_n30_), .O(new_n36_));
  nand3  g11(.a(new_n33_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n26_), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nor2   g15(.a(new_n33_), .b(new_n40_), .O(z04));
  inv1   g16(.a(z04), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n39_), .O(z01));
  inv1   g18(.a(x00), .O(new_n44_));
  nor3   g19(.a(x13), .b(x04), .c(x03), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(new_n27_), .c(new_n44_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n40_), .c(new_n33_), .O(z02));
  nand3  g22(.a(new_n45_), .b(new_n27_), .c(x00), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n40_), .c(new_n33_), .O(z03));
  inv1   g24(.a(x07), .O(new_n51_));
  inv1   g25(.a(x12), .O(new_n52_));
  inv1   g26(.a(x08), .O(new_n53_));
  nand2  g27(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  inv1   g28(.a(x09), .O(new_n55_));
  nand2  g29(.a(new_n55_), .b(x00), .O(new_n56_));
  nand2  g30(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand4  g31(.a(new_n57_), .b(x13), .c(new_n52_), .d(new_n51_), .O(new_n58_));
  inv1   g32(.a(new_n58_), .O(new_n59_));
  nand4  g33(.a(new_n59_), .b(x05), .c(new_n30_), .d(new_n40_), .O(new_n60_));
  inv1   g34(.a(x13), .O(new_n61_));
  nand3  g35(.a(new_n61_), .b(new_n33_), .c(x02), .O(new_n62_));
  nand2  g36(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand3  g37(.a(new_n63_), .b(x04), .c(new_n26_), .O(new_n64_));
  inv1   g38(.a(new_n64_), .O(z06));
  nand4  g39(.a(new_n61_), .b(x05), .c(new_n32_), .d(new_n26_), .O(new_n66_));
  nor3   g40(.a(new_n66_), .b(x02), .c(x01), .O(z07));
  nand3  g41(.a(x13), .b(new_n52_), .c(x05), .O(new_n68_));
  oai21  g42(.a(new_n68_), .b(new_n28_), .c(new_n62_), .O(new_n69_));
  nand3  g43(.a(new_n69_), .b(x04), .c(new_n26_), .O(new_n70_));
  inv1   g44(.a(new_n70_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n72_));
  nand2  g46(.a(new_n72_), .b(new_n42_), .O(z09));
  nand3  g47(.a(new_n29_), .b(x06), .c(new_n26_), .O(new_n74_));
  inv1   g48(.a(new_n74_), .O(z10));
  zero   g49(.O(z05));
endmodule


