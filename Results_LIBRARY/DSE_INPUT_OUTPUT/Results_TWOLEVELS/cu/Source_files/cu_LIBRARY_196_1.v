// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x00), .O(new_n27_));
  oai21  g02(.a(x02), .b(new_n27_), .c(x05), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  nand2  g04(.a(x04), .b(new_n29_), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n30_), .c(new_n28_), .d(new_n26_), .O(z00));
  nand3  g09(.a(x05), .b(new_n31_), .c(new_n29_), .O(new_n35_));
  nand3  g10(.a(new_n32_), .b(x04), .c(x02), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n26_), .O(new_n38_));
  nor2   g13(.a(new_n32_), .b(x00), .O(z02));
  inv1   g14(.a(z02), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n38_), .O(z01));
  inv1   g16(.a(x13), .O(new_n42_));
  inv1   g17(.a(x01), .O(new_n43_));
  nand4  g18(.a(new_n26_), .b(new_n29_), .c(new_n43_), .d(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n42_), .c(x05), .d(new_n31_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z03));
  nor3   g22(.a(x02), .b(new_n43_), .c(new_n27_), .O(new_n49_));
  nand4  g23(.a(new_n49_), .b(x05), .c(new_n31_), .d(new_n26_), .O(new_n50_));
  nor2   g24(.a(new_n50_), .b(x13), .O(z05));
  inv1   g25(.a(x07), .O(new_n52_));
  inv1   g26(.a(x12), .O(new_n53_));
  inv1   g27(.a(x09), .O(new_n54_));
  nand2  g28(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  inv1   g29(.a(x11), .O(new_n56_));
  nand2  g30(.a(new_n56_), .b(x01), .O(new_n57_));
  nand2  g31(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand4  g32(.a(new_n58_), .b(x13), .c(new_n53_), .d(new_n52_), .O(new_n59_));
  inv1   g33(.a(new_n59_), .O(new_n60_));
  nand4  g34(.a(new_n60_), .b(x05), .c(new_n29_), .d(x00), .O(new_n61_));
  nand3  g35(.a(new_n42_), .b(new_n32_), .c(x02), .O(new_n62_));
  nand2  g36(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g37(.a(new_n63_), .b(x04), .c(new_n26_), .O(new_n64_));
  inv1   g38(.a(new_n64_), .O(z06));
  nand4  g39(.a(new_n42_), .b(new_n31_), .c(new_n26_), .d(new_n29_), .O(new_n66_));
  aoi21  g40(.a(new_n66_), .b(x00), .c(new_n32_), .O(z07));
  nand4  g41(.a(x13), .b(new_n53_), .c(x05), .d(new_n29_), .O(new_n68_));
  nand2  g42(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  nand3  g43(.a(new_n69_), .b(x04), .c(new_n26_), .O(new_n70_));
  nand2  g44(.a(new_n70_), .b(new_n40_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n72_));
  nand2  g46(.a(new_n72_), .b(new_n40_), .O(z09));
  nand3  g47(.a(new_n28_), .b(x06), .c(new_n26_), .O(new_n74_));
  inv1   g48(.a(new_n74_), .O(z10));
  zero   g49(.O(z04));
endmodule


