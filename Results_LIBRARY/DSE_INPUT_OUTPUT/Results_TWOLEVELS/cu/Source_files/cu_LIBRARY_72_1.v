// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n79_, new_n80_;
  inv1   g00(.a(x13), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x01), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  aoi21  g03(.a(x04), .b(new_n28_), .c(x03), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nor2   g06(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  aoi21  g07(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n29_), .c(new_n27_), .O(z00));
  inv1   g09(.a(x03), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  nand3  g11(.a(x05), .b(new_n30_), .c(new_n28_), .O(new_n37_));
  nand3  g12(.a(new_n31_), .b(x04), .c(x02), .O(new_n38_));
  aoi22  g13(.a(new_n38_), .b(new_n37_), .c(x13), .d(new_n36_), .O(new_n39_));
  and2   g14(.a(new_n39_), .b(new_n35_), .O(z01));
  nor3   g15(.a(x02), .b(x01), .c(x00), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(x05), .c(new_n30_), .d(new_n35_), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(x13), .O(z02));
  nor2   g18(.a(x03), .b(x02), .O(new_n44_));
  nor2   g19(.a(x13), .b(new_n31_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n44_), .c(new_n30_), .d(x00), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n26_), .c(x01), .O(z03));
  inv1   g22(.a(new_n27_), .O(new_n48_));
  inv1   g23(.a(x00), .O(new_n49_));
  nand3  g24(.a(new_n28_), .b(x01), .c(new_n49_), .O(new_n50_));
  nand3  g25(.a(new_n45_), .b(new_n30_), .c(new_n35_), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(z04));
  nor3   g27(.a(x02), .b(new_n36_), .c(new_n49_), .O(new_n53_));
  nand4  g28(.a(new_n53_), .b(x05), .c(new_n30_), .d(new_n35_), .O(new_n54_));
  nor2   g29(.a(new_n54_), .b(x13), .O(z05));
  inv1   g30(.a(x07), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  inv1   g32(.a(x10), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n49_), .O(new_n59_));
  inv1   g34(.a(x11), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x00), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(x13), .c(new_n57_), .d(new_n56_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(x05), .c(new_n28_), .d(x01), .O(new_n65_));
  nand3  g40(.a(new_n26_), .b(new_n31_), .c(x02), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(x04), .c(new_n35_), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(z06));
  nand3  g44(.a(new_n45_), .b(new_n30_), .c(new_n35_), .O(new_n70_));
  oai21  g45(.a(new_n70_), .b(x02), .c(new_n48_), .O(z07));
  nand2  g46(.a(new_n28_), .b(x01), .O(new_n72_));
  nand3  g47(.a(x13), .b(new_n57_), .c(x05), .O(new_n73_));
  oai21  g48(.a(new_n73_), .b(new_n72_), .c(new_n66_), .O(new_n74_));
  nand3  g49(.a(new_n74_), .b(x04), .c(new_n35_), .O(new_n75_));
  inv1   g50(.a(new_n75_), .O(z08));
  inv1   g51(.a(x06), .O(new_n77_));
  aoi21  g52(.a(new_n77_), .b(x01), .c(new_n26_), .O(z09));
  inv1   g53(.a(new_n32_), .O(new_n79_));
  nand4  g54(.a(new_n79_), .b(new_n48_), .c(x06), .d(new_n35_), .O(new_n80_));
  inv1   g55(.a(new_n80_), .O(z10));
endmodule


