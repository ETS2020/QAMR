// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x01), .O(z03));
  inv1   g02(.a(z03), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(x05), .c(x01), .O(new_n33_));
  oai21  g08(.a(new_n31_), .b(new_n30_), .c(new_n26_), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n33_), .c(new_n29_), .O(z00));
  nand4  g10(.a(x05), .b(new_n31_), .c(new_n30_), .d(x01), .O(new_n36_));
  nand3  g11(.a(new_n26_), .b(x04), .c(x02), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n36_), .c(x03), .O(z01));
  inv1   g13(.a(x13), .O(new_n40_));
  inv1   g14(.a(x00), .O(new_n41_));
  inv1   g15(.a(x03), .O(new_n42_));
  nand4  g16(.a(new_n42_), .b(new_n30_), .c(x01), .d(new_n41_), .O(new_n43_));
  inv1   g17(.a(new_n43_), .O(new_n44_));
  nand4  g18(.a(new_n44_), .b(new_n40_), .c(x05), .d(new_n31_), .O(new_n45_));
  inv1   g19(.a(new_n45_), .O(z04));
  nand2  g20(.a(x01), .b(x00), .O(new_n47_));
  nor2   g21(.a(new_n47_), .b(x02), .O(new_n48_));
  nand4  g22(.a(new_n48_), .b(x05), .c(new_n31_), .d(new_n42_), .O(new_n49_));
  nor2   g23(.a(new_n49_), .b(x13), .O(z05));
  inv1   g24(.a(x07), .O(new_n51_));
  inv1   g25(.a(x12), .O(new_n52_));
  inv1   g26(.a(x10), .O(new_n53_));
  nand2  g27(.a(new_n53_), .b(new_n41_), .O(new_n54_));
  inv1   g28(.a(x11), .O(new_n55_));
  nand2  g29(.a(new_n55_), .b(x00), .O(new_n56_));
  aoi21  g30(.a(new_n56_), .b(new_n54_), .c(new_n40_), .O(new_n57_));
  nand4  g31(.a(new_n57_), .b(new_n52_), .c(new_n51_), .d(x05), .O(new_n58_));
  nand3  g32(.a(new_n40_), .b(new_n26_), .c(x02), .O(new_n59_));
  oai21  g33(.a(new_n58_), .b(x02), .c(new_n59_), .O(new_n60_));
  nand3  g34(.a(new_n60_), .b(x04), .c(new_n42_), .O(new_n61_));
  nand2  g35(.a(new_n61_), .b(new_n28_), .O(z06));
  nand4  g36(.a(new_n40_), .b(new_n31_), .c(new_n42_), .d(new_n30_), .O(new_n63_));
  aoi21  g37(.a(new_n63_), .b(x01), .c(new_n26_), .O(z07));
  nand4  g38(.a(x13), .b(new_n52_), .c(x05), .d(new_n30_), .O(new_n65_));
  nand2  g39(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand3  g40(.a(new_n66_), .b(x04), .c(new_n42_), .O(new_n67_));
  nand2  g41(.a(new_n67_), .b(new_n28_), .O(z08));
  inv1   g42(.a(x06), .O(new_n69_));
  nor3   g43(.a(z03), .b(new_n40_), .c(new_n69_), .O(z09));
  nand2  g44(.a(x05), .b(x02), .O(new_n71_));
  nand3  g45(.a(new_n71_), .b(x06), .c(new_n42_), .O(new_n72_));
  nand2  g46(.a(new_n72_), .b(new_n28_), .O(z10));
  zero   g47(.O(z02));
endmodule


