// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n87_;
  inv1   g00(.a(x03), .O(new_n26_));
  oai21  g01(.a(x06), .b(x02), .c(x05), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nand2  g03(.a(x04), .b(new_n28_), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n29_), .c(new_n27_), .d(new_n26_), .O(z00));
  inv1   g08(.a(x06), .O(new_n34_));
  nand3  g09(.a(x05), .b(new_n30_), .c(new_n28_), .O(new_n35_));
  nand3  g10(.a(new_n31_), .b(x04), .c(x02), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n26_), .O(new_n38_));
  oai21  g13(.a(new_n34_), .b(new_n31_), .c(new_n38_), .O(z01));
  inv1   g14(.a(x00), .O(new_n40_));
  inv1   g15(.a(x01), .O(new_n41_));
  nand4  g16(.a(new_n26_), .b(new_n28_), .c(new_n41_), .d(new_n40_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  nand4  g18(.a(new_n43_), .b(new_n34_), .c(x05), .d(new_n30_), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x13), .O(z02));
  nand4  g20(.a(new_n26_), .b(new_n28_), .c(new_n41_), .d(x00), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(new_n47_));
  nand4  g22(.a(new_n47_), .b(new_n34_), .c(x05), .d(new_n30_), .O(new_n48_));
  nor2   g23(.a(new_n48_), .b(x13), .O(z03));
  nor3   g24(.a(x13), .b(x04), .c(x03), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n28_), .c(x01), .d(new_n40_), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(new_n34_), .c(new_n31_), .O(z04));
  nand4  g27(.a(new_n26_), .b(new_n28_), .c(x01), .d(x00), .O(new_n53_));
  inv1   g28(.a(new_n53_), .O(new_n54_));
  nand4  g29(.a(new_n54_), .b(new_n34_), .c(x05), .d(new_n30_), .O(new_n55_));
  nor2   g30(.a(new_n55_), .b(x13), .O(z05));
  inv1   g31(.a(x07), .O(new_n57_));
  inv1   g32(.a(x09), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n41_), .O(new_n59_));
  inv1   g34(.a(x11), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x01), .O(new_n61_));
  aoi21  g36(.a(new_n61_), .b(new_n59_), .c(new_n40_), .O(new_n62_));
  inv1   g37(.a(x08), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n41_), .O(new_n64_));
  inv1   g39(.a(x10), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(x01), .O(new_n66_));
  aoi21  g41(.a(new_n66_), .b(new_n64_), .c(x00), .O(new_n67_));
  oai21  g42(.a(new_n67_), .b(new_n62_), .c(x13), .O(new_n68_));
  nor2   g43(.a(new_n68_), .b(x12), .O(new_n69_));
  nand4  g44(.a(new_n69_), .b(new_n57_), .c(new_n34_), .d(x05), .O(new_n70_));
  inv1   g45(.a(x13), .O(new_n71_));
  nand3  g46(.a(new_n71_), .b(new_n31_), .c(x02), .O(new_n72_));
  oai21  g47(.a(new_n70_), .b(x02), .c(new_n72_), .O(new_n73_));
  nand3  g48(.a(new_n73_), .b(x04), .c(new_n26_), .O(new_n74_));
  inv1   g49(.a(new_n74_), .O(z06));
  aoi21  g50(.a(x01), .b(new_n40_), .c(new_n34_), .O(new_n76_));
  nor2   g51(.a(new_n76_), .b(x13), .O(new_n77_));
  nand4  g52(.a(new_n77_), .b(new_n30_), .c(new_n26_), .d(new_n28_), .O(new_n78_));
  aoi21  g53(.a(new_n78_), .b(new_n34_), .c(new_n31_), .O(z07));
  nand2  g54(.a(x05), .b(new_n28_), .O(new_n80_));
  inv1   g55(.a(x12), .O(new_n81_));
  nand3  g56(.a(x13), .b(new_n81_), .c(new_n34_), .O(new_n82_));
  oai21  g57(.a(new_n82_), .b(new_n80_), .c(new_n72_), .O(new_n83_));
  nand3  g58(.a(new_n83_), .b(x04), .c(new_n26_), .O(new_n84_));
  inv1   g59(.a(new_n84_), .O(z08));
  aoi21  g60(.a(new_n71_), .b(new_n31_), .c(new_n34_), .O(z09));
  nand3  g61(.a(x06), .b(new_n31_), .c(new_n26_), .O(new_n87_));
  inv1   g62(.a(new_n87_), .O(z10));
endmodule


