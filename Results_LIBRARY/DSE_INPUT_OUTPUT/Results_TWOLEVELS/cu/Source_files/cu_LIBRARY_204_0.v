// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n88_;
  nor2   g00(.a(x08), .b(x01), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g05(.a(x05), .b(x02), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n32_), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n31_), .c(new_n30_), .d(new_n27_), .O(z00));
  inv1   g09(.a(x03), .O(new_n35_));
  inv1   g10(.a(new_n26_), .O(new_n36_));
  nor2   g11(.a(new_n29_), .b(x04), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n32_), .O(new_n38_));
  nand3  g13(.a(new_n29_), .b(x04), .c(x02), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(new_n36_), .c(new_n35_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(z01));
  inv1   g17(.a(x00), .O(new_n43_));
  inv1   g18(.a(x01), .O(new_n44_));
  nand4  g19(.a(new_n35_), .b(new_n32_), .c(new_n44_), .d(new_n43_), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(x08), .c(x05), .d(new_n28_), .O(new_n47_));
  nor2   g22(.a(new_n47_), .b(x13), .O(z02));
  nor2   g23(.a(x03), .b(x02), .O(new_n49_));
  inv1   g24(.a(x08), .O(new_n50_));
  nor2   g25(.a(x13), .b(new_n50_), .O(new_n51_));
  nand4  g26(.a(new_n51_), .b(new_n49_), .c(new_n37_), .d(x00), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(x08), .c(x01), .O(z03));
  inv1   g28(.a(x13), .O(new_n54_));
  nand4  g29(.a(new_n35_), .b(new_n32_), .c(x01), .d(new_n43_), .O(new_n55_));
  inv1   g30(.a(new_n55_), .O(new_n56_));
  nand4  g31(.a(new_n56_), .b(new_n54_), .c(x05), .d(new_n28_), .O(new_n57_));
  inv1   g32(.a(new_n57_), .O(z04));
  nor3   g33(.a(x02), .b(new_n44_), .c(new_n43_), .O(new_n59_));
  nand4  g34(.a(new_n59_), .b(x05), .c(new_n28_), .d(new_n35_), .O(new_n60_));
  nor2   g35(.a(new_n60_), .b(x13), .O(z05));
  inv1   g36(.a(x07), .O(new_n62_));
  inv1   g37(.a(x12), .O(new_n63_));
  inv1   g38(.a(x09), .O(new_n64_));
  nand3  g39(.a(new_n64_), .b(x08), .c(new_n44_), .O(new_n65_));
  inv1   g40(.a(x11), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(x01), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(x00), .O(new_n69_));
  inv1   g44(.a(x10), .O(new_n70_));
  nand3  g45(.a(new_n70_), .b(x01), .c(new_n43_), .O(new_n71_));
  aoi21  g46(.a(new_n71_), .b(new_n69_), .c(new_n54_), .O(new_n72_));
  nand4  g47(.a(new_n72_), .b(new_n63_), .c(new_n62_), .d(x05), .O(new_n73_));
  nand4  g48(.a(new_n36_), .b(new_n54_), .c(new_n29_), .d(x02), .O(new_n74_));
  oai21  g49(.a(new_n73_), .b(x02), .c(new_n74_), .O(new_n75_));
  nand3  g50(.a(new_n75_), .b(x04), .c(new_n35_), .O(new_n76_));
  inv1   g51(.a(new_n76_), .O(z06));
  aoi21  g52(.a(new_n50_), .b(new_n44_), .c(x13), .O(new_n78_));
  nand4  g53(.a(new_n78_), .b(x05), .c(new_n28_), .d(new_n35_), .O(new_n79_));
  nor2   g54(.a(new_n79_), .b(x02), .O(z07));
  nand4  g55(.a(x13), .b(new_n63_), .c(x05), .d(new_n32_), .O(new_n81_));
  nand3  g56(.a(new_n54_), .b(new_n29_), .c(x02), .O(new_n82_));
  nand2  g57(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand4  g58(.a(new_n83_), .b(new_n36_), .c(x04), .d(new_n35_), .O(new_n84_));
  inv1   g59(.a(new_n84_), .O(z08));
  inv1   g60(.a(x06), .O(new_n86_));
  nor3   g61(.a(new_n26_), .b(new_n54_), .c(new_n86_), .O(z09));
  nand4  g62(.a(new_n31_), .b(new_n36_), .c(x06), .d(new_n35_), .O(new_n88_));
  inv1   g63(.a(new_n88_), .O(z10));
endmodule


