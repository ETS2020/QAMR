// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n90_, new_n91_;
  inv1   g00(.a(x06), .O(new_n26_));
  nand2  g01(.a(x11), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  aoi21  g04(.a(new_n29_), .b(x02), .c(x03), .O(new_n30_));
  nor2   g05(.a(x05), .b(x02), .O(new_n31_));
  aoi21  g06(.a(x05), .b(x04), .c(new_n31_), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(z00));
  inv1   g08(.a(x03), .O(new_n34_));
  inv1   g09(.a(x02), .O(new_n35_));
  nand3  g10(.a(x05), .b(new_n29_), .c(new_n35_), .O(new_n36_));
  inv1   g11(.a(x05), .O(new_n37_));
  nand3  g12(.a(new_n37_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n27_), .c(new_n34_), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z01));
  nor3   g16(.a(new_n28_), .b(x13), .c(new_n37_), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(new_n29_), .c(new_n34_), .d(new_n35_), .O(new_n43_));
  nor3   g18(.a(new_n43_), .b(x01), .c(x00), .O(z02));
  inv1   g19(.a(x01), .O(new_n45_));
  nand3  g20(.a(new_n35_), .b(new_n45_), .c(x00), .O(new_n46_));
  inv1   g21(.a(x13), .O(new_n47_));
  nand4  g22(.a(new_n47_), .b(x05), .c(new_n29_), .d(new_n34_), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n46_), .c(new_n27_), .O(z03));
  inv1   g24(.a(x00), .O(new_n50_));
  nand3  g25(.a(new_n35_), .b(x01), .c(new_n50_), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n48_), .c(new_n27_), .O(z04));
  nor3   g27(.a(new_n43_), .b(new_n45_), .c(new_n50_), .O(z05));
  nand4  g28(.a(x05), .b(new_n35_), .c(new_n45_), .d(new_n50_), .O(new_n54_));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nand4  g32(.a(x13), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n58_));
  nand3  g33(.a(new_n47_), .b(new_n37_), .c(x02), .O(new_n59_));
  oai21  g34(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n27_), .O(new_n61_));
  inv1   g36(.a(x09), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n45_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x10), .O(new_n64_));
  nand3  g39(.a(new_n64_), .b(x01), .c(new_n50_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(x06), .O(new_n67_));
  inv1   g42(.a(x11), .O(new_n68_));
  nor2   g43(.a(x10), .b(new_n45_), .O(new_n69_));
  aoi21  g44(.a(x09), .b(new_n45_), .c(new_n50_), .O(new_n70_));
  oai21  g45(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g46(.a(new_n71_), .b(new_n67_), .c(new_n47_), .O(new_n72_));
  nand4  g47(.a(new_n72_), .b(new_n57_), .c(new_n55_), .d(x05), .O(new_n73_));
  oai21  g48(.a(new_n73_), .b(x02), .c(new_n61_), .O(new_n74_));
  nand3  g49(.a(new_n74_), .b(x04), .c(new_n34_), .O(new_n75_));
  inv1   g50(.a(new_n75_), .O(z06));
  xnor2a g51(.a(x01), .b(x00), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n27_), .O(new_n78_));
  xnor2a g53(.a(x01), .b(x00), .O(new_n79_));
  aoi21  g54(.a(new_n79_), .b(new_n78_), .c(x13), .O(new_n80_));
  nand4  g55(.a(new_n80_), .b(x05), .c(new_n29_), .d(new_n34_), .O(new_n81_));
  oai21  g56(.a(new_n81_), .b(x02), .c(new_n27_), .O(z07));
  nand2  g57(.a(x05), .b(new_n35_), .O(new_n83_));
  nand2  g58(.a(x13), .b(new_n57_), .O(new_n84_));
  nand4  g59(.a(new_n27_), .b(new_n47_), .c(new_n37_), .d(x02), .O(new_n85_));
  oai21  g60(.a(new_n84_), .b(new_n83_), .c(new_n85_), .O(new_n86_));
  nand3  g61(.a(new_n86_), .b(x04), .c(new_n34_), .O(new_n87_));
  nand2  g62(.a(new_n87_), .b(new_n27_), .O(z08));
  oai21  g63(.a(new_n47_), .b(new_n26_), .c(new_n27_), .O(z09));
  nand2  g64(.a(x05), .b(x02), .O(new_n90_));
  nand3  g65(.a(new_n90_), .b(x06), .c(new_n34_), .O(new_n91_));
  inv1   g66(.a(new_n91_), .O(z10));
endmodule


