// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_, new_n63_;
  inv1   g00(.a(x12), .O(new_n26_));
  aoi21  g01(.a(x13), .b(new_n26_), .c(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g05(.a(x05), .b(x02), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n32_), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n31_), .c(new_n30_), .d(new_n27_), .O(z00));
  nand2  g09(.a(x13), .b(new_n26_), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(x05), .c(new_n28_), .d(new_n32_), .O(new_n36_));
  nand4  g11(.a(new_n35_), .b(new_n29_), .c(x04), .d(x02), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n36_), .c(x03), .O(z01));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nand3  g15(.a(new_n32_), .b(new_n40_), .c(new_n39_), .O(new_n41_));
  inv1   g16(.a(x03), .O(new_n42_));
  inv1   g17(.a(x13), .O(new_n43_));
  nand4  g18(.a(new_n43_), .b(x05), .c(new_n28_), .d(new_n42_), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n41_), .c(new_n35_), .O(z02));
  nand3  g20(.a(new_n32_), .b(new_n40_), .c(x00), .O(new_n46_));
  oai21  g21(.a(new_n46_), .b(new_n44_), .c(new_n35_), .O(z03));
  nand4  g22(.a(new_n42_), .b(new_n32_), .c(x01), .d(new_n39_), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n43_), .c(x05), .d(new_n28_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z04));
  nand3  g26(.a(new_n32_), .b(x01), .c(x00), .O(new_n52_));
  oai21  g27(.a(new_n52_), .b(new_n44_), .c(new_n35_), .O(z05));
  nand4  g28(.a(new_n29_), .b(x04), .c(new_n42_), .d(x02), .O(new_n54_));
  nor2   g29(.a(new_n54_), .b(x13), .O(z06));
  nand4  g30(.a(new_n43_), .b(x05), .c(new_n28_), .d(new_n42_), .O(new_n56_));
  nor2   g31(.a(new_n56_), .b(x02), .O(z07));
  nand2  g32(.a(new_n42_), .b(x02), .O(new_n58_));
  nand3  g33(.a(new_n43_), .b(new_n29_), .c(x04), .O(new_n59_));
  oai21  g34(.a(new_n59_), .b(new_n58_), .c(new_n35_), .O(z08));
  nand3  g35(.a(x13), .b(x12), .c(x06), .O(new_n61_));
  inv1   g36(.a(new_n61_), .O(z09));
  nand4  g37(.a(new_n31_), .b(new_n35_), .c(x06), .d(new_n42_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(z10));
endmodule


