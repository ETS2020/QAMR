// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x03), .O(new_n26_));
  nor2   g01(.a(x05), .b(x04), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  nand2  g03(.a(x05), .b(x02), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  nand2  g05(.a(x04), .b(new_n30_), .O(new_n31_));
  nand4  g06(.a(new_n31_), .b(new_n29_), .c(new_n28_), .d(new_n26_), .O(z00));
  inv1   g07(.a(x04), .O(new_n33_));
  nand3  g08(.a(x05), .b(new_n33_), .c(new_n30_), .O(new_n34_));
  inv1   g09(.a(x05), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(new_n34_), .c(x03), .O(z01));
  inv1   g12(.a(x00), .O(new_n38_));
  inv1   g13(.a(x01), .O(new_n39_));
  inv1   g14(.a(x13), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(x05), .c(new_n26_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(new_n30_), .c(new_n39_), .d(new_n38_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(x05), .c(x04), .O(z02));
  nand4  g19(.a(new_n26_), .b(new_n30_), .c(new_n39_), .d(x00), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n40_), .c(x05), .d(new_n33_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z03));
  nand4  g23(.a(new_n26_), .b(new_n30_), .c(x01), .d(new_n38_), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n40_), .c(x05), .d(new_n33_), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(z04));
  nand4  g27(.a(new_n42_), .b(new_n30_), .c(x01), .d(x00), .O(new_n53_));
  aoi21  g28(.a(new_n53_), .b(x05), .c(x04), .O(z05));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n39_), .O(new_n57_));
  inv1   g32(.a(x11), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  aoi21  g34(.a(new_n59_), .b(new_n57_), .c(new_n38_), .O(new_n60_));
  inv1   g35(.a(x08), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n39_), .O(new_n62_));
  inv1   g37(.a(x10), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(x01), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n62_), .c(x00), .O(new_n65_));
  oai21  g40(.a(new_n65_), .b(new_n60_), .c(x13), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(x12), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(new_n55_), .c(x05), .d(x04), .O(new_n68_));
  nand3  g43(.a(new_n40_), .b(new_n35_), .c(x02), .O(new_n69_));
  oai21  g44(.a(new_n68_), .b(x02), .c(new_n69_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n26_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n28_), .O(z06));
  nand4  g47(.a(new_n40_), .b(x05), .c(new_n33_), .d(new_n26_), .O(new_n73_));
  nor2   g48(.a(new_n73_), .b(x02), .O(z07));
  inv1   g49(.a(x12), .O(new_n75_));
  nand3  g50(.a(x13), .b(new_n75_), .c(x05), .O(new_n76_));
  oai21  g51(.a(new_n76_), .b(new_n31_), .c(new_n69_), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n26_), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n28_), .O(z08));
  inv1   g54(.a(x06), .O(new_n80_));
  nor3   g55(.a(new_n27_), .b(new_n40_), .c(new_n80_), .O(z09));
  nand2  g56(.a(new_n35_), .b(x04), .O(new_n82_));
  oai21  g57(.a(new_n35_), .b(x02), .c(new_n82_), .O(new_n83_));
  nand3  g58(.a(new_n83_), .b(x06), .c(new_n26_), .O(new_n84_));
  inv1   g59(.a(new_n84_), .O(z10));
endmodule


