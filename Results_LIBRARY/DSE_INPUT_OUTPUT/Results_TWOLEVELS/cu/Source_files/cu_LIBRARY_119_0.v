// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n70_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  inv1   g03(.a(x13), .O(new_n29_));
  oai21  g04(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nand2  g06(.a(x04), .b(new_n31_), .O(new_n32_));
  nand2  g07(.a(x05), .b(x02), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n32_), .c(new_n30_), .d(new_n26_), .O(z00));
  nand3  g09(.a(x05), .b(new_n27_), .c(new_n31_), .O(new_n35_));
  nand3  g10(.a(new_n28_), .b(x04), .c(x02), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n26_), .O(new_n38_));
  nor2   g13(.a(x13), .b(x04), .O(z04));
  inv1   g14(.a(z04), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n38_), .O(z01));
  inv1   g16(.a(x07), .O(new_n44_));
  inv1   g17(.a(x00), .O(new_n45_));
  inv1   g18(.a(x01), .O(new_n46_));
  inv1   g19(.a(x09), .O(new_n47_));
  nand2  g20(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g21(.a(x11), .O(new_n49_));
  nand2  g22(.a(new_n49_), .b(x01), .O(new_n50_));
  aoi21  g23(.a(new_n50_), .b(new_n48_), .c(new_n45_), .O(new_n51_));
  inv1   g24(.a(x08), .O(new_n52_));
  nand2  g25(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  inv1   g26(.a(x10), .O(new_n54_));
  nand2  g27(.a(new_n54_), .b(x01), .O(new_n55_));
  aoi21  g28(.a(new_n55_), .b(new_n53_), .c(x00), .O(new_n56_));
  oai21  g29(.a(new_n56_), .b(new_n51_), .c(x13), .O(new_n57_));
  nor2   g30(.a(new_n57_), .b(x12), .O(new_n58_));
  nand4  g31(.a(new_n58_), .b(new_n44_), .c(x05), .d(x04), .O(new_n59_));
  nand3  g32(.a(new_n29_), .b(new_n28_), .c(x02), .O(new_n60_));
  oai21  g33(.a(new_n59_), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g34(.a(new_n61_), .b(new_n26_), .O(new_n62_));
  nand2  g35(.a(new_n62_), .b(new_n40_), .O(z06));
  inv1   g36(.a(x12), .O(new_n64_));
  nand3  g37(.a(x13), .b(new_n64_), .c(x05), .O(new_n65_));
  oai21  g38(.a(new_n65_), .b(new_n32_), .c(new_n60_), .O(new_n66_));
  nand2  g39(.a(new_n66_), .b(new_n26_), .O(new_n67_));
  nand2  g40(.a(new_n67_), .b(new_n40_), .O(z08));
  and2   g41(.a(x13), .b(x06), .O(z09));
  nand3  g42(.a(new_n33_), .b(x06), .c(new_n26_), .O(new_n70_));
  nand2  g43(.a(new_n70_), .b(new_n40_), .O(z10));
  zero   g44(.O(z02));
  zero   g45(.O(z03));
  nor2   g46(.a(x13), .b(x04), .O(z05));
  nor2   g47(.a(x13), .b(x04), .O(z07));
endmodule


