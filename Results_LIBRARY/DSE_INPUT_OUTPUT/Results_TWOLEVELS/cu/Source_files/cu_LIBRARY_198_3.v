// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_;
  inv1   g00(.a(x10), .O(new_n26_));
  inv1   g01(.a(x13), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  aoi21  g04(.a(x04), .b(new_n29_), .c(x03), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  aoi21  g08(.a(new_n32_), .b(new_n31_), .c(new_n33_), .O(new_n34_));
  aoi21  g09(.a(new_n34_), .b(new_n30_), .c(new_n28_), .O(z00));
  inv1   g10(.a(new_n28_), .O(new_n36_));
  inv1   g11(.a(x03), .O(new_n37_));
  nand3  g12(.a(x05), .b(new_n31_), .c(new_n29_), .O(new_n38_));
  nand3  g13(.a(new_n32_), .b(x04), .c(x02), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n37_), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(new_n36_), .O(z01));
  inv1   g17(.a(x00), .O(new_n43_));
  inv1   g18(.a(x01), .O(new_n44_));
  nand3  g19(.a(new_n29_), .b(new_n44_), .c(new_n43_), .O(new_n45_));
  nand4  g20(.a(new_n27_), .b(x05), .c(new_n31_), .d(new_n37_), .O(new_n46_));
  oai21  g21(.a(new_n46_), .b(new_n45_), .c(new_n36_), .O(z02));
  nand4  g22(.a(new_n37_), .b(new_n29_), .c(new_n44_), .d(x00), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n27_), .c(x05), .d(new_n31_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z03));
  nand4  g26(.a(new_n37_), .b(new_n29_), .c(x01), .d(new_n43_), .O(new_n52_));
  inv1   g27(.a(new_n52_), .O(new_n53_));
  nand4  g28(.a(new_n53_), .b(new_n27_), .c(x05), .d(new_n31_), .O(new_n54_));
  inv1   g29(.a(new_n54_), .O(z04));
  nand3  g30(.a(new_n29_), .b(x01), .c(x00), .O(new_n56_));
  oai21  g31(.a(new_n56_), .b(new_n46_), .c(new_n36_), .O(z05));
  inv1   g32(.a(x07), .O(new_n58_));
  inv1   g33(.a(x12), .O(new_n59_));
  inv1   g34(.a(x08), .O(new_n60_));
  oai21  g35(.a(new_n60_), .b(x01), .c(new_n43_), .O(new_n61_));
  inv1   g36(.a(x09), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n44_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x11), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(x01), .O(new_n65_));
  nand3  g40(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  nand4  g41(.a(new_n66_), .b(x13), .c(new_n59_), .d(new_n26_), .O(new_n67_));
  inv1   g42(.a(new_n67_), .O(new_n68_));
  nand4  g43(.a(new_n68_), .b(new_n58_), .c(x05), .d(new_n29_), .O(new_n69_));
  nand3  g44(.a(new_n27_), .b(new_n32_), .c(x02), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g46(.a(new_n71_), .b(x04), .c(new_n37_), .O(new_n72_));
  inv1   g47(.a(new_n72_), .O(z06));
  nand4  g48(.a(new_n27_), .b(x05), .c(new_n31_), .d(new_n37_), .O(new_n74_));
  nor2   g49(.a(new_n74_), .b(x02), .O(z07));
  nand2  g50(.a(x05), .b(new_n29_), .O(new_n76_));
  nand3  g51(.a(x13), .b(new_n59_), .c(new_n26_), .O(new_n77_));
  oai21  g52(.a(new_n77_), .b(new_n76_), .c(new_n70_), .O(new_n78_));
  nand3  g53(.a(new_n78_), .b(x04), .c(new_n37_), .O(new_n79_));
  inv1   g54(.a(new_n79_), .O(z08));
  inv1   g55(.a(x06), .O(new_n81_));
  aoi21  g56(.a(new_n26_), .b(new_n81_), .c(new_n27_), .O(z09));
  inv1   g57(.a(new_n33_), .O(new_n83_));
  nand3  g58(.a(new_n83_), .b(x06), .c(new_n37_), .O(new_n84_));
  nand2  g59(.a(new_n84_), .b(new_n36_), .O(z10));
endmodule


