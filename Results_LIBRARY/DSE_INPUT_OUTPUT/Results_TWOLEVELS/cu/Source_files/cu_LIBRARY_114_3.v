// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_;
  nand2  g00(.a(x08), .b(x00), .O(new_n26_));
  inv1   g01(.a(new_n26_), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  aoi21  g03(.a(new_n28_), .b(x02), .c(x03), .O(new_n29_));
  nor2   g04(.a(x05), .b(x02), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x04), .c(new_n30_), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(z00));
  inv1   g07(.a(x03), .O(new_n33_));
  inv1   g08(.a(x02), .O(new_n34_));
  inv1   g09(.a(x05), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(x04), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand3  g12(.a(new_n35_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n33_), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n26_), .O(z01));
  nor3   g16(.a(x02), .b(x01), .c(x00), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(x05), .c(new_n28_), .d(new_n33_), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(x13), .O(z02));
  inv1   g19(.a(x00), .O(new_n45_));
  inv1   g20(.a(x08), .O(new_n46_));
  inv1   g21(.a(x13), .O(new_n47_));
  nor3   g22(.a(x03), .b(x02), .c(x01), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(new_n36_), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(z03));
  nand4  g25(.a(new_n33_), .b(new_n34_), .c(x01), .d(new_n45_), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(new_n52_));
  nand4  g27(.a(new_n52_), .b(new_n47_), .c(x05), .d(new_n28_), .O(new_n53_));
  inv1   g28(.a(new_n53_), .O(z04));
  nand4  g29(.a(new_n33_), .b(new_n34_), .c(x01), .d(x00), .O(new_n55_));
  inv1   g30(.a(new_n55_), .O(new_n56_));
  nand4  g31(.a(new_n56_), .b(new_n46_), .c(x05), .d(new_n28_), .O(new_n57_));
  nor2   g32(.a(new_n57_), .b(x13), .O(z05));
  nand4  g33(.a(x04), .b(new_n33_), .c(new_n34_), .d(x01), .O(new_n59_));
  inv1   g34(.a(x07), .O(new_n60_));
  inv1   g35(.a(x11), .O(new_n61_));
  nor2   g36(.a(new_n47_), .b(x12), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(x05), .O(new_n63_));
  oai21  g38(.a(new_n63_), .b(new_n59_), .c(new_n46_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(x00), .O(new_n65_));
  inv1   g40(.a(x12), .O(new_n66_));
  nor2   g41(.a(x08), .b(x01), .O(new_n67_));
  inv1   g42(.a(x01), .O(new_n68_));
  nor2   g43(.a(x10), .b(new_n68_), .O(new_n69_));
  oai21  g44(.a(new_n69_), .b(new_n67_), .c(new_n45_), .O(new_n70_));
  inv1   g45(.a(x09), .O(new_n71_));
  nand3  g46(.a(new_n71_), .b(new_n46_), .c(new_n68_), .O(new_n72_));
  aoi21  g47(.a(new_n72_), .b(new_n70_), .c(new_n47_), .O(new_n73_));
  nand4  g48(.a(new_n73_), .b(new_n66_), .c(new_n60_), .d(x05), .O(new_n74_));
  nand4  g49(.a(new_n26_), .b(new_n47_), .c(new_n35_), .d(x02), .O(new_n75_));
  oai21  g50(.a(new_n74_), .b(x02), .c(new_n75_), .O(new_n76_));
  nand3  g51(.a(new_n76_), .b(x04), .c(new_n33_), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n65_), .O(z06));
  aoi21  g53(.a(x08), .b(x00), .c(x13), .O(new_n79_));
  nand4  g54(.a(new_n79_), .b(x05), .c(new_n28_), .d(new_n33_), .O(new_n80_));
  nor2   g55(.a(new_n80_), .b(x02), .O(z07));
  nand3  g56(.a(new_n62_), .b(x05), .c(new_n34_), .O(new_n82_));
  nand3  g57(.a(new_n47_), .b(new_n35_), .c(x02), .O(new_n83_));
  nand2  g58(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand4  g59(.a(new_n84_), .b(new_n26_), .c(x04), .d(new_n33_), .O(new_n85_));
  inv1   g60(.a(new_n85_), .O(z08));
  nand2  g61(.a(x13), .b(x06), .O(new_n87_));
  nand2  g62(.a(new_n87_), .b(new_n26_), .O(z09));
  nand2  g63(.a(x05), .b(x02), .O(new_n89_));
  nand4  g64(.a(new_n89_), .b(new_n26_), .c(x06), .d(new_n33_), .O(new_n90_));
  inv1   g65(.a(new_n90_), .O(z10));
endmodule


