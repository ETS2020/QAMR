// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_;
  inv1   g00(.a(x01), .O(new_n26_));
  inv1   g01(.a(x13), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nor2   g03(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g07(.a(x05), .b(x02), .O(new_n33_));
  inv1   g08(.a(x02), .O(new_n34_));
  nand2  g09(.a(x04), .b(new_n34_), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(new_n33_), .c(new_n32_), .d(new_n29_), .O(z00));
  nand2  g11(.a(x13), .b(x01), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(x05), .c(new_n30_), .d(new_n34_), .O(new_n38_));
  inv1   g13(.a(new_n28_), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(new_n31_), .c(x04), .d(x02), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(new_n38_), .c(x03), .O(z01));
  inv1   g16(.a(x03), .O(new_n42_));
  nor3   g17(.a(x02), .b(x01), .c(x00), .O(new_n43_));
  nand4  g18(.a(new_n43_), .b(x05), .c(new_n30_), .d(new_n42_), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x13), .O(z02));
  nand4  g20(.a(new_n42_), .b(new_n34_), .c(new_n26_), .d(x00), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(new_n47_));
  nand4  g22(.a(new_n47_), .b(new_n27_), .c(x05), .d(new_n30_), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(z03));
  nor3   g24(.a(x03), .b(x02), .c(x00), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n27_), .c(x05), .d(new_n30_), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(new_n27_), .c(new_n26_), .O(z04));
  inv1   g27(.a(x00), .O(new_n53_));
  nor3   g28(.a(x02), .b(new_n26_), .c(new_n53_), .O(new_n54_));
  nand4  g29(.a(new_n54_), .b(x05), .c(new_n30_), .d(new_n42_), .O(new_n55_));
  nor2   g30(.a(new_n55_), .b(x13), .O(z05));
  inv1   g31(.a(x07), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  inv1   g33(.a(x08), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n53_), .O(new_n60_));
  inv1   g35(.a(x09), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(x00), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(x13), .c(new_n58_), .d(new_n57_), .O(new_n64_));
  inv1   g39(.a(new_n64_), .O(new_n65_));
  nand4  g40(.a(new_n65_), .b(x05), .c(new_n34_), .d(new_n26_), .O(new_n66_));
  nand3  g41(.a(new_n27_), .b(new_n31_), .c(x02), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g43(.a(new_n68_), .b(x04), .c(new_n42_), .O(new_n69_));
  inv1   g44(.a(new_n69_), .O(z06));
  nand4  g45(.a(new_n27_), .b(x05), .c(new_n30_), .d(new_n42_), .O(new_n71_));
  oai21  g46(.a(new_n71_), .b(x02), .c(new_n39_), .O(z07));
  nand2  g47(.a(new_n34_), .b(new_n26_), .O(new_n73_));
  nand3  g48(.a(x13), .b(new_n58_), .c(x05), .O(new_n74_));
  oai21  g49(.a(new_n74_), .b(new_n73_), .c(new_n67_), .O(new_n75_));
  nand3  g50(.a(new_n75_), .b(x04), .c(new_n42_), .O(new_n76_));
  inv1   g51(.a(new_n76_), .O(z08));
  nand3  g52(.a(x13), .b(x06), .c(new_n26_), .O(new_n78_));
  inv1   g53(.a(new_n78_), .O(z09));
  nand4  g54(.a(new_n33_), .b(new_n39_), .c(x06), .d(new_n42_), .O(new_n80_));
  inv1   g55(.a(new_n80_), .O(z10));
endmodule


