// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_;
  aoi21  g00(.a(x08), .b(x07), .c(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g04(.a(x05), .b(x04), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x02), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n30_), .c(new_n29_), .d(new_n26_), .O(z00));
  inv1   g08(.a(x03), .O(new_n34_));
  nand2  g09(.a(x08), .b(x07), .O(new_n35_));
  nand3  g10(.a(x05), .b(new_n31_), .c(new_n27_), .O(new_n36_));
  nand3  g11(.a(new_n28_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(new_n35_), .c(new_n34_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(z01));
  inv1   g15(.a(x13), .O(new_n41_));
  nand4  g16(.a(new_n35_), .b(new_n41_), .c(x05), .d(new_n31_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  nand3  g18(.a(new_n43_), .b(new_n34_), .c(new_n27_), .O(new_n44_));
  nor3   g19(.a(new_n44_), .b(x01), .c(x00), .O(z02));
  inv1   g20(.a(x01), .O(new_n46_));
  nand3  g21(.a(new_n27_), .b(new_n46_), .c(x00), .O(new_n47_));
  nand4  g22(.a(new_n41_), .b(x05), .c(new_n31_), .d(new_n34_), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n47_), .c(new_n35_), .O(z03));
  nor3   g24(.a(new_n44_), .b(new_n46_), .c(x00), .O(z04));
  nand3  g25(.a(new_n27_), .b(x01), .c(x00), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n48_), .c(new_n35_), .O(z05));
  inv1   g27(.a(x12), .O(new_n53_));
  nor2   g28(.a(x09), .b(x01), .O(new_n54_));
  nor2   g29(.a(x11), .b(new_n46_), .O(new_n55_));
  oai21  g30(.a(new_n55_), .b(new_n54_), .c(x00), .O(new_n56_));
  inv1   g31(.a(x00), .O(new_n57_));
  nor2   g32(.a(x08), .b(x01), .O(new_n58_));
  nor2   g33(.a(x10), .b(new_n46_), .O(new_n59_));
  oai21  g34(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  aoi21  g35(.a(new_n60_), .b(new_n56_), .c(new_n41_), .O(new_n61_));
  nand4  g36(.a(new_n61_), .b(new_n53_), .c(x05), .d(new_n27_), .O(new_n62_));
  nand3  g37(.a(new_n41_), .b(new_n28_), .c(x02), .O(new_n63_));
  aoi21  g38(.a(new_n63_), .b(new_n62_), .c(x07), .O(new_n64_));
  inv1   g39(.a(x08), .O(new_n65_));
  nand4  g40(.a(new_n41_), .b(new_n65_), .c(new_n28_), .d(x02), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(new_n67_));
  oai21  g42(.a(new_n67_), .b(new_n64_), .c(x04), .O(new_n68_));
  oai21  g43(.a(new_n68_), .b(x03), .c(new_n35_), .O(z06));
  oai21  g44(.a(new_n35_), .b(x00), .c(new_n41_), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n28_), .O(new_n71_));
  nand4  g46(.a(new_n71_), .b(new_n31_), .c(new_n34_), .d(new_n27_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n35_), .O(z07));
  nand4  g48(.a(x13), .b(new_n53_), .c(x05), .d(new_n27_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  nand4  g50(.a(new_n75_), .b(new_n35_), .c(x04), .d(new_n34_), .O(new_n76_));
  inv1   g51(.a(new_n76_), .O(z08));
  nand2  g52(.a(x13), .b(x06), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n35_), .O(z09));
  nand2  g54(.a(x05), .b(x02), .O(new_n80_));
  nand4  g55(.a(new_n80_), .b(new_n35_), .c(x06), .d(new_n34_), .O(new_n81_));
  inv1   g56(.a(new_n81_), .O(z10));
endmodule


