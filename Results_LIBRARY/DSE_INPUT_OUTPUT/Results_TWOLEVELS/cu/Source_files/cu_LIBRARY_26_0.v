// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_;
  nor2   g00(.a(x13), .b(x00), .O(z02));
  nor2   g01(.a(z02), .b(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g05(.a(x05), .b(x02), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n32_), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n31_), .c(new_n30_), .d(new_n27_), .O(z00));
  inv1   g09(.a(z02), .O(new_n35_));
  inv1   g10(.a(x03), .O(new_n36_));
  nand3  g11(.a(x05), .b(new_n28_), .c(new_n32_), .O(new_n37_));
  nand3  g12(.a(new_n29_), .b(x04), .c(x02), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n36_), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n35_), .O(z01));
  inv1   g16(.a(x13), .O(new_n42_));
  inv1   g17(.a(x01), .O(new_n43_));
  nand4  g18(.a(new_n36_), .b(new_n32_), .c(new_n43_), .d(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n42_), .c(x05), .d(new_n28_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z03));
  inv1   g22(.a(x00), .O(new_n49_));
  nor3   g23(.a(x02), .b(new_n43_), .c(new_n49_), .O(new_n50_));
  nand4  g24(.a(new_n50_), .b(x05), .c(new_n28_), .d(new_n36_), .O(new_n51_));
  nor2   g25(.a(new_n51_), .b(x13), .O(z05));
  inv1   g26(.a(x07), .O(new_n53_));
  inv1   g27(.a(x12), .O(new_n54_));
  nor2   g28(.a(x09), .b(x01), .O(new_n55_));
  nor2   g29(.a(x11), .b(new_n43_), .O(new_n56_));
  oai21  g30(.a(new_n56_), .b(new_n55_), .c(x00), .O(new_n57_));
  nor2   g31(.a(x08), .b(x01), .O(new_n58_));
  nor2   g32(.a(x10), .b(new_n43_), .O(new_n59_));
  oai21  g33(.a(new_n59_), .b(new_n58_), .c(new_n49_), .O(new_n60_));
  aoi21  g34(.a(new_n60_), .b(new_n57_), .c(new_n42_), .O(new_n61_));
  nand4  g35(.a(new_n61_), .b(new_n54_), .c(new_n53_), .d(x05), .O(new_n62_));
  nand4  g36(.a(new_n42_), .b(new_n29_), .c(x02), .d(x00), .O(new_n63_));
  oai21  g37(.a(new_n62_), .b(x02), .c(new_n63_), .O(new_n64_));
  nand3  g38(.a(new_n64_), .b(x04), .c(new_n36_), .O(new_n65_));
  inv1   g39(.a(new_n65_), .O(z06));
  nand4  g40(.a(new_n42_), .b(x05), .c(new_n28_), .d(new_n36_), .O(new_n67_));
  nor3   g41(.a(new_n67_), .b(x02), .c(new_n49_), .O(z07));
  nand4  g42(.a(x13), .b(new_n54_), .c(x05), .d(new_n32_), .O(new_n69_));
  nand2  g43(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand3  g44(.a(new_n70_), .b(x04), .c(new_n36_), .O(new_n71_));
  inv1   g45(.a(new_n71_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n73_));
  nand2  g47(.a(new_n73_), .b(new_n35_), .O(z09));
  nand4  g48(.a(new_n31_), .b(new_n35_), .c(x06), .d(new_n36_), .O(new_n75_));
  inv1   g49(.a(new_n75_), .O(z10));
  zero   g50(.O(z04));
endmodule


