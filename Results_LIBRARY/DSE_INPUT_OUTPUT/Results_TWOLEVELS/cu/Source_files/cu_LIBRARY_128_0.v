// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n84_;
  nor2   g00(.a(x13), .b(x08), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g05(.a(x05), .b(x02), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n32_), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n31_), .c(new_n30_), .d(new_n27_), .O(z00));
  inv1   g09(.a(new_n26_), .O(new_n35_));
  inv1   g10(.a(x03), .O(new_n36_));
  nor2   g11(.a(new_n29_), .b(x04), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n32_), .O(new_n38_));
  nand3  g13(.a(new_n29_), .b(x04), .c(x02), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(new_n35_), .O(z01));
  inv1   g17(.a(x00), .O(new_n43_));
  inv1   g18(.a(x01), .O(new_n44_));
  nand2  g19(.a(new_n37_), .b(new_n36_), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n32_), .c(new_n44_), .d(new_n43_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x08), .c(x13), .O(z02));
  nand4  g23(.a(new_n46_), .b(new_n32_), .c(new_n44_), .d(x00), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(x08), .c(x13), .O(z03));
  nor2   g25(.a(x02), .b(new_n44_), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n46_), .c(new_n43_), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(x08), .c(x13), .O(z04));
  nand3  g28(.a(new_n51_), .b(new_n46_), .c(x00), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(x08), .c(x13), .O(z05));
  inv1   g30(.a(x07), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n44_), .O(new_n58_));
  inv1   g33(.a(x11), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  aoi21  g35(.a(new_n60_), .b(new_n58_), .c(new_n43_), .O(new_n61_));
  inv1   g36(.a(x10), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(x01), .c(new_n43_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  oai21  g39(.a(new_n64_), .b(new_n61_), .c(x13), .O(new_n65_));
  inv1   g40(.a(x08), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(new_n44_), .c(new_n43_), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(new_n65_), .c(x12), .O(new_n68_));
  nand4  g43(.a(new_n68_), .b(new_n56_), .c(x05), .d(new_n32_), .O(new_n69_));
  inv1   g44(.a(x13), .O(new_n70_));
  nand3  g45(.a(new_n70_), .b(new_n29_), .c(x02), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(x04), .c(new_n36_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n35_), .O(z06));
  nand2  g49(.a(new_n46_), .b(new_n32_), .O(new_n75_));
  aoi21  g50(.a(new_n75_), .b(x08), .c(x13), .O(z07));
  inv1   g51(.a(x12), .O(new_n77_));
  nand4  g52(.a(x13), .b(new_n77_), .c(x05), .d(new_n32_), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  nand3  g54(.a(new_n79_), .b(x04), .c(new_n36_), .O(new_n80_));
  nand2  g55(.a(new_n80_), .b(new_n35_), .O(z08));
  nand2  g56(.a(x13), .b(x06), .O(new_n82_));
  nand2  g57(.a(new_n82_), .b(new_n35_), .O(z09));
  nand4  g58(.a(new_n31_), .b(new_n35_), .c(x06), .d(new_n36_), .O(new_n84_));
  inv1   g59(.a(new_n84_), .O(z10));
endmodule


