// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_;
  inv1   g00(.a(x03), .O(new_n26_));
  oai21  g01(.a(x13), .b(x02), .c(x05), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nand2  g03(.a(x04), .b(new_n28_), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n29_), .c(new_n27_), .d(new_n26_), .O(z00));
  nand3  g08(.a(new_n31_), .b(x04), .c(x02), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(x05), .c(new_n30_), .d(new_n28_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(new_n34_), .c(x03), .O(z01));
  inv1   g12(.a(x00), .O(new_n38_));
  inv1   g13(.a(x01), .O(new_n39_));
  nor3   g14(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(new_n35_), .c(new_n31_), .O(z02));
  nand3  g17(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(new_n35_), .c(new_n31_), .O(z03));
  nand4  g19(.a(new_n26_), .b(new_n28_), .c(x01), .d(new_n38_), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n35_), .c(x05), .d(new_n30_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z04));
  nand2  g23(.a(x01), .b(x00), .O(new_n49_));
  nor2   g24(.a(new_n49_), .b(x02), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(x05), .c(new_n30_), .d(new_n26_), .O(new_n51_));
  nor2   g26(.a(new_n51_), .b(x13), .O(z05));
  nand2  g27(.a(new_n26_), .b(x02), .O(new_n53_));
  nand3  g28(.a(new_n35_), .b(new_n31_), .c(x04), .O(new_n54_));
  oai22  g29(.a(new_n54_), .b(new_n53_), .c(new_n35_), .d(new_n31_), .O(z06));
  nand4  g30(.a(new_n35_), .b(new_n30_), .c(new_n26_), .d(new_n28_), .O(new_n56_));
  aoi21  g31(.a(new_n56_), .b(new_n35_), .c(new_n31_), .O(z07));
  nand4  g32(.a(new_n31_), .b(x04), .c(new_n26_), .d(x02), .O(new_n58_));
  nor2   g33(.a(new_n58_), .b(x13), .O(z08));
  inv1   g34(.a(x06), .O(new_n60_));
  aoi21  g35(.a(new_n60_), .b(new_n31_), .c(new_n35_), .O(z09));
  nand3  g36(.a(new_n27_), .b(x06), .c(new_n26_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(z10));
endmodule


