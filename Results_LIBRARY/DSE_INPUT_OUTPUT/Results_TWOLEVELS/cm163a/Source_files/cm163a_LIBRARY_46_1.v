// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x05), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  oai21  g02(.a(new_n23_), .b(x04), .c(new_n22_), .O(new_n24_));
  and2   g03(.a(x03), .b(x02), .O(new_n25_));
  nand3  g04(.a(new_n23_), .b(x03), .c(x02), .O(new_n26_));
  oai21  g05(.a(new_n25_), .b(new_n23_), .c(new_n26_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  nand3  g09(.a(new_n23_), .b(new_n30_), .c(new_n29_), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(new_n28_), .c(new_n24_), .O(z0));
  inv1   g11(.a(x01), .O(new_n33_));
  oai21  g12(.a(x09), .b(new_n33_), .c(new_n30_), .O(new_n34_));
  nand2  g13(.a(x03), .b(x02), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x11), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n23_), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n35_), .c(new_n36_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(x04), .O(new_n40_));
  nand2  g19(.a(x11), .b(x09), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(new_n40_), .c(new_n34_), .d(new_n24_), .O(z1));
  inv1   g21(.a(x06), .O(new_n43_));
  oai21  g22(.a(x09), .b(new_n43_), .c(new_n30_), .O(new_n44_));
  nand3  g23(.a(new_n37_), .b(x03), .c(x02), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(x12), .O(new_n46_));
  nor2   g25(.a(x12), .b(x11), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n25_), .c(new_n23_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(x04), .O(new_n50_));
  nand2  g29(.a(x12), .b(x09), .O(new_n51_));
  nand4  g30(.a(new_n51_), .b(new_n50_), .c(new_n44_), .d(new_n24_), .O(z2));
  inv1   g31(.a(x07), .O(new_n53_));
  oai21  g32(.a(x09), .b(new_n53_), .c(new_n30_), .O(new_n54_));
  inv1   g33(.a(x13), .O(new_n55_));
  aoi21  g34(.a(new_n47_), .b(new_n25_), .c(new_n55_), .O(new_n56_));
  inv1   g35(.a(x12), .O(new_n57_));
  nand3  g36(.a(new_n55_), .b(new_n57_), .c(new_n37_), .O(new_n58_));
  nor2   g37(.a(new_n58_), .b(new_n26_), .O(new_n59_));
  oai21  g38(.a(new_n59_), .b(new_n56_), .c(x04), .O(new_n60_));
  nand2  g39(.a(x13), .b(x09), .O(new_n61_));
  nand4  g40(.a(new_n61_), .b(new_n60_), .c(new_n54_), .d(new_n24_), .O(z3));
  inv1   g41(.a(x03), .O(new_n63_));
  inv1   g42(.a(x15), .O(new_n64_));
  aoi21  g43(.a(x09), .b(new_n30_), .c(new_n64_), .O(new_n65_));
  nand4  g44(.a(new_n65_), .b(x14), .c(x10), .d(x08), .O(new_n66_));
  nor2   g45(.a(new_n66_), .b(new_n63_), .O(z4));
endmodule


