// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_;
  nand2  g00(.a(x09), .b(x01), .O(new_n22_));
  oai21  g01(.a(x04), .b(x00), .c(x05), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x01), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(x09), .c(new_n25_), .O(new_n27_));
  inv1   g06(.a(x09), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(x03), .c(x02), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n24_), .O(z0));
  inv1   g11(.a(x04), .O(new_n33_));
  inv1   g12(.a(x05), .O(new_n34_));
  oai21  g13(.a(new_n34_), .b(new_n33_), .c(new_n25_), .O(new_n35_));
  oai21  g14(.a(x11), .b(x01), .c(x09), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x04), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n26_), .c(x05), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n28_), .O(new_n40_));
  nand3  g19(.a(new_n26_), .b(x11), .c(x04), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(new_n40_), .c(new_n36_), .d(new_n35_), .O(z1));
  nand2  g21(.a(new_n22_), .b(new_n34_), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  oai21  g23(.a(new_n44_), .b(new_n33_), .c(new_n25_), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(x09), .O(new_n46_));
  nand2  g25(.a(x12), .b(x11), .O(new_n47_));
  nand4  g26(.a(new_n44_), .b(new_n37_), .c(x03), .d(x02), .O(new_n48_));
  aoi21  g27(.a(new_n48_), .b(new_n47_), .c(x09), .O(new_n49_));
  inv1   g28(.a(new_n26_), .O(new_n50_));
  nor2   g29(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  oai21  g30(.a(new_n51_), .b(new_n49_), .c(x04), .O(new_n52_));
  inv1   g31(.a(x06), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  nand4  g33(.a(new_n54_), .b(new_n52_), .c(new_n46_), .d(new_n43_), .O(z2));
  inv1   g34(.a(x13), .O(new_n56_));
  oai21  g35(.a(new_n56_), .b(new_n33_), .c(new_n25_), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(x09), .O(new_n58_));
  oai21  g37(.a(x12), .b(x11), .c(new_n28_), .O(new_n59_));
  aoi21  g38(.a(new_n59_), .b(new_n50_), .c(new_n56_), .O(new_n60_));
  nand3  g39(.a(new_n56_), .b(new_n44_), .c(new_n37_), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(new_n29_), .O(new_n62_));
  oai21  g41(.a(new_n62_), .b(new_n60_), .c(x04), .O(new_n63_));
  inv1   g42(.a(x07), .O(new_n64_));
  nand2  g43(.a(new_n64_), .b(new_n33_), .O(new_n65_));
  nand4  g44(.a(new_n65_), .b(new_n63_), .c(new_n58_), .d(new_n43_), .O(z3));
  inv1   g45(.a(x03), .O(new_n67_));
  inv1   g46(.a(x08), .O(new_n68_));
  nand4  g47(.a(new_n22_), .b(x15), .c(x14), .d(x10), .O(new_n69_));
  nor3   g48(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(z4));
endmodule


