// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  nand3  g03(.a(new_n22_), .b(x03), .c(x02), .O(new_n25_));
  oai21  g04(.a(new_n24_), .b(new_n22_), .c(new_n25_), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g06(.a(x00), .O(new_n28_));
  inv1   g07(.a(x04), .O(new_n29_));
  inv1   g08(.a(x05), .O(new_n30_));
  aoi21  g09(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n27_), .O(z0));
  nor2   g11(.a(x11), .b(x09), .O(new_n33_));
  aoi22  g12(.a(new_n33_), .b(new_n24_), .c(new_n25_), .d(x11), .O(new_n34_));
  inv1   g13(.a(x01), .O(new_n35_));
  aoi21  g14(.a(new_n29_), .b(new_n35_), .c(new_n30_), .O(new_n36_));
  oai21  g15(.a(new_n34_), .b(new_n29_), .c(new_n36_), .O(z1));
  aoi21  g16(.a(x12), .b(x04), .c(new_n30_), .O(new_n38_));
  nand2  g17(.a(new_n33_), .b(x03), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(x12), .O(new_n40_));
  inv1   g19(.a(x11), .O(new_n41_));
  inv1   g20(.a(x12), .O(new_n42_));
  nand4  g21(.a(new_n24_), .b(new_n42_), .c(new_n41_), .d(new_n22_), .O(new_n43_));
  aoi21  g22(.a(new_n43_), .b(new_n40_), .c(new_n29_), .O(new_n44_));
  nor2   g23(.a(x06), .b(x04), .O(new_n45_));
  oai21  g24(.a(new_n45_), .b(new_n44_), .c(x05), .O(new_n46_));
  oai21  g25(.a(new_n38_), .b(x02), .c(new_n46_), .O(z2));
  inv1   g26(.a(x02), .O(new_n48_));
  inv1   g27(.a(x13), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n29_), .c(x05), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g30(.a(x13), .b(x12), .O(new_n52_));
  nand3  g31(.a(new_n49_), .b(new_n42_), .c(new_n41_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n23_), .c(new_n52_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n22_), .O(new_n55_));
  nand2  g34(.a(new_n39_), .b(x13), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n55_), .c(new_n29_), .O(new_n57_));
  nor2   g36(.a(x07), .b(x04), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n57_), .c(x05), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n51_), .O(z3));
  inv1   g39(.a(x03), .O(new_n61_));
  inv1   g40(.a(x15), .O(new_n62_));
  aoi21  g41(.a(new_n30_), .b(x02), .c(new_n62_), .O(new_n63_));
  nand4  g42(.a(new_n63_), .b(x14), .c(x10), .d(x08), .O(new_n64_));
  nor2   g43(.a(new_n64_), .b(new_n61_), .O(z4));
endmodule


