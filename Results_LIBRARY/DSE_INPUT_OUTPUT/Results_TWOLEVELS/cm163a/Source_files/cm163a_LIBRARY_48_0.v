// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_;
  inv1   g00(.a(x05), .O(new_n22_));
  oai21  g01(.a(x09), .b(x04), .c(new_n22_), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  and2   g03(.a(x03), .b(x02), .O(new_n25_));
  nand3  g04(.a(new_n24_), .b(x03), .c(x02), .O(new_n26_));
  oai21  g05(.a(new_n25_), .b(new_n24_), .c(new_n26_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  nand3  g09(.a(x09), .b(new_n30_), .c(new_n29_), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(new_n28_), .c(new_n23_), .O(z0));
  nor2   g11(.a(x04), .b(x01), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  nor2   g13(.a(new_n34_), .b(new_n30_), .O(new_n35_));
  oai21  g14(.a(new_n35_), .b(new_n33_), .c(x09), .O(new_n36_));
  nand2  g15(.a(x03), .b(x02), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x11), .O(new_n38_));
  nand2  g17(.a(new_n34_), .b(new_n24_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n37_), .c(new_n38_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(x04), .O(new_n41_));
  nand3  g20(.a(new_n41_), .b(new_n36_), .c(new_n23_), .O(z1));
  nor2   g21(.a(x06), .b(x04), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  nor2   g23(.a(new_n44_), .b(new_n30_), .O(new_n45_));
  oai21  g24(.a(new_n45_), .b(new_n43_), .c(x09), .O(new_n46_));
  nand3  g25(.a(new_n34_), .b(x03), .c(x02), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x12), .O(new_n48_));
  nor2   g27(.a(x12), .b(x11), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(new_n25_), .c(new_n24_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x04), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n46_), .c(new_n23_), .O(z2));
  inv1   g32(.a(x13), .O(new_n54_));
  oai21  g33(.a(new_n54_), .b(new_n24_), .c(x05), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(x04), .O(new_n56_));
  nand2  g35(.a(x09), .b(x07), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n30_), .O(new_n58_));
  aoi21  g37(.a(new_n49_), .b(new_n25_), .c(new_n54_), .O(new_n59_));
  nand3  g38(.a(new_n54_), .b(new_n44_), .c(new_n34_), .O(new_n60_));
  nor2   g39(.a(new_n60_), .b(new_n37_), .O(new_n61_));
  oai21  g40(.a(new_n61_), .b(new_n59_), .c(new_n24_), .O(new_n62_));
  nand2  g41(.a(x09), .b(new_n22_), .O(new_n63_));
  nand4  g42(.a(new_n63_), .b(new_n62_), .c(new_n58_), .d(new_n56_), .O(z3));
  nand2  g43(.a(x08), .b(x03), .O(new_n65_));
  nand3  g44(.a(x15), .b(x14), .c(x10), .O(new_n66_));
  oai22  g45(.a(new_n66_), .b(new_n65_), .c(x09), .d(x04), .O(z4));
endmodule


