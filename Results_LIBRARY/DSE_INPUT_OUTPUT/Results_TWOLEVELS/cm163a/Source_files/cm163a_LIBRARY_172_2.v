// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_;
  inv1   g00(.a(x02), .O(new_n22_));
  inv1   g01(.a(x04), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  oai21  g03(.a(new_n24_), .b(new_n23_), .c(x05), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  inv1   g05(.a(x03), .O(new_n27_));
  nand2  g06(.a(x09), .b(new_n27_), .O(new_n28_));
  nor2   g07(.a(x09), .b(new_n27_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x02), .O(new_n30_));
  aoi21  g09(.a(new_n30_), .b(new_n28_), .c(new_n23_), .O(new_n31_));
  nor2   g10(.a(x04), .b(x00), .O(new_n32_));
  oai21  g11(.a(new_n32_), .b(new_n31_), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n26_), .O(z0));
  inv1   g13(.a(x11), .O(new_n35_));
  oai21  g14(.a(new_n35_), .b(new_n23_), .c(x05), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n22_), .O(new_n37_));
  nand2  g16(.a(new_n24_), .b(x03), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x11), .O(new_n39_));
  nand4  g18(.a(new_n35_), .b(new_n24_), .c(x03), .d(x02), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(new_n39_), .c(new_n23_), .O(new_n41_));
  nor2   g20(.a(x04), .b(x01), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(new_n41_), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n37_), .O(z1));
  nand2  g23(.a(x04), .b(x03), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand3  g25(.a(new_n46_), .b(new_n35_), .c(new_n24_), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n45_), .c(x05), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(x02), .O(new_n49_));
  oai21  g28(.a(new_n46_), .b(new_n23_), .c(x05), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  aoi21  g30(.a(x11), .b(x05), .c(x09), .O(new_n52_));
  aoi21  g31(.a(new_n52_), .b(x03), .c(new_n46_), .O(new_n53_));
  nor2   g32(.a(x06), .b(x04), .O(new_n54_));
  aoi21  g33(.a(new_n53_), .b(x04), .c(new_n54_), .O(new_n55_));
  nand3  g34(.a(new_n55_), .b(new_n51_), .c(new_n49_), .O(z2));
  nand2  g35(.a(x13), .b(x04), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(x05), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(new_n22_), .O(new_n59_));
  nand2  g38(.a(new_n46_), .b(new_n35_), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n38_), .c(x13), .O(new_n61_));
  nor3   g40(.a(x13), .b(x12), .c(x11), .O(new_n62_));
  nand3  g41(.a(new_n62_), .b(new_n29_), .c(x02), .O(new_n63_));
  aoi21  g42(.a(new_n63_), .b(new_n61_), .c(new_n23_), .O(new_n64_));
  nor2   g43(.a(x07), .b(x04), .O(new_n65_));
  oai21  g44(.a(new_n65_), .b(new_n64_), .c(x05), .O(new_n66_));
  nand2  g45(.a(new_n66_), .b(new_n59_), .O(z3));
  nand2  g46(.a(x08), .b(x03), .O(new_n68_));
  nand3  g47(.a(x15), .b(x14), .c(x10), .O(new_n69_));
  oai22  g48(.a(new_n69_), .b(new_n68_), .c(x05), .d(new_n22_), .O(z4));
endmodule


