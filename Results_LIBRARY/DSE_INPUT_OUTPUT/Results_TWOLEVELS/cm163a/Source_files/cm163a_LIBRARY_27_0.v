// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand3  g01(.a(new_n22_), .b(x04), .c(x02), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x05), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x03), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(x09), .c(x04), .O(new_n27_));
  oai21  g06(.a(x04), .b(x00), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x05), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n25_), .O(z0));
  nor2   g09(.a(x11), .b(x09), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(x04), .c(x02), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(x03), .O(new_n34_));
  inv1   g13(.a(x03), .O(new_n35_));
  inv1   g14(.a(x04), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  oai21  g16(.a(new_n37_), .b(new_n36_), .c(x05), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n35_), .O(new_n39_));
  aoi21  g18(.a(new_n22_), .b(x02), .c(new_n37_), .O(new_n40_));
  nor2   g19(.a(x04), .b(x01), .O(new_n41_));
  aoi21  g20(.a(new_n40_), .b(x04), .c(new_n41_), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(new_n39_), .c(new_n34_), .O(z1));
  nand2  g22(.a(x04), .b(x02), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n37_), .c(new_n22_), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n44_), .c(x05), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x03), .O(new_n48_));
  oai21  g27(.a(new_n45_), .b(new_n36_), .c(x05), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n35_), .O(new_n50_));
  aoi21  g29(.a(new_n31_), .b(x02), .c(new_n45_), .O(new_n51_));
  nor2   g30(.a(x06), .b(x04), .O(new_n52_));
  aoi21  g31(.a(new_n51_), .b(x04), .c(new_n52_), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z2));
  inv1   g33(.a(x13), .O(new_n55_));
  nand3  g34(.a(new_n55_), .b(new_n45_), .c(new_n37_), .O(new_n56_));
  oai21  g35(.a(new_n56_), .b(new_n23_), .c(x05), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(x03), .O(new_n58_));
  oai21  g37(.a(new_n55_), .b(new_n36_), .c(x05), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n35_), .O(new_n60_));
  nand4  g39(.a(new_n45_), .b(new_n37_), .c(new_n22_), .d(x02), .O(new_n61_));
  nand3  g40(.a(new_n61_), .b(x13), .c(x04), .O(new_n62_));
  inv1   g41(.a(x07), .O(new_n63_));
  nand2  g42(.a(new_n63_), .b(new_n36_), .O(new_n64_));
  nand4  g43(.a(new_n64_), .b(new_n62_), .c(new_n60_), .d(new_n58_), .O(z3));
  nand2  g44(.a(x08), .b(x03), .O(new_n66_));
  nand3  g45(.a(x15), .b(x14), .c(x10), .O(new_n67_));
  oai22  g46(.a(new_n67_), .b(new_n66_), .c(x05), .d(x03), .O(z4));
endmodule


