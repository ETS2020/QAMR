// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x09), .O(new_n22_));
  inv1   g01(.a(x05), .O(new_n23_));
  inv1   g02(.a(x11), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nand4  g04(.a(new_n25_), .b(new_n22_), .c(x03), .d(x02), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  nand3  g06(.a(new_n27_), .b(x09), .c(x05), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x04), .O(new_n30_));
  inv1   g09(.a(x00), .O(new_n31_));
  nor2   g10(.a(new_n23_), .b(x04), .O(new_n32_));
  aoi22  g11(.a(new_n32_), .b(new_n31_), .c(x11), .d(new_n23_), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n30_), .O(z0));
  nand3  g13(.a(new_n24_), .b(new_n22_), .c(x04), .O(new_n35_));
  oai22  g14(.a(new_n35_), .b(new_n27_), .c(x04), .d(x01), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x05), .O(new_n37_));
  nand3  g16(.a(new_n22_), .b(x03), .c(x02), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(x04), .c(new_n23_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n24_), .c(new_n37_), .O(z1));
  inv1   g19(.a(x04), .O(new_n41_));
  inv1   g20(.a(x12), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(new_n41_), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x11), .O(new_n44_));
  nand2  g23(.a(new_n38_), .b(x12), .O(new_n45_));
  nor2   g24(.a(x12), .b(x11), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(new_n22_), .c(x03), .d(x02), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(new_n48_));
  nor2   g27(.a(x06), .b(x04), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n48_), .c(x05), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n44_), .O(z2));
  nand2  g30(.a(x13), .b(x04), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(x05), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g33(.a(new_n42_), .b(new_n22_), .O(new_n55_));
  oai21  g34(.a(new_n55_), .b(new_n27_), .c(x13), .O(new_n56_));
  inv1   g35(.a(x03), .O(new_n57_));
  nor2   g36(.a(x09), .b(new_n57_), .O(new_n58_));
  nor3   g37(.a(x13), .b(x12), .c(x11), .O(new_n59_));
  nand3  g38(.a(new_n59_), .b(new_n58_), .c(x02), .O(new_n60_));
  aoi21  g39(.a(new_n60_), .b(new_n56_), .c(new_n41_), .O(new_n61_));
  nor2   g40(.a(x07), .b(x04), .O(new_n62_));
  oai21  g41(.a(new_n62_), .b(new_n61_), .c(x05), .O(new_n63_));
  nand2  g42(.a(new_n63_), .b(new_n54_), .O(z3));
  inv1   g43(.a(x15), .O(new_n65_));
  aoi21  g44(.a(new_n24_), .b(new_n23_), .c(new_n65_), .O(new_n66_));
  nand4  g45(.a(new_n66_), .b(x14), .c(x10), .d(x08), .O(new_n67_));
  nor2   g46(.a(new_n67_), .b(new_n57_), .O(z4));
endmodule


