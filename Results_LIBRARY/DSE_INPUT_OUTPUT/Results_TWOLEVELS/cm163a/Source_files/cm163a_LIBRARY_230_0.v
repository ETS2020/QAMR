// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_;
  nor2   g00(.a(x13), .b(x09), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x09), .O(new_n25_));
  inv1   g04(.a(x09), .O(new_n26_));
  nand4  g05(.a(x13), .b(new_n26_), .c(x03), .d(x02), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  or2    g08(.a(x04), .b(x00), .O(new_n30_));
  nand4  g09(.a(new_n30_), .b(new_n29_), .c(new_n23_), .d(x05), .O(z0));
  oai21  g10(.a(x04), .b(x01), .c(x05), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  inv1   g12(.a(x13), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand4  g14(.a(new_n35_), .b(new_n26_), .c(x03), .d(x02), .O(new_n36_));
  nand2  g15(.a(new_n24_), .b(x11), .O(new_n37_));
  aoi21  g16(.a(new_n37_), .b(new_n36_), .c(new_n34_), .O(new_n38_));
  nor2   g17(.a(new_n35_), .b(new_n26_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n38_), .c(x04), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n33_), .O(z1));
  oai21  g20(.a(x06), .b(x04), .c(x05), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n23_), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  nand3  g23(.a(new_n35_), .b(x03), .c(x02), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(x13), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n26_), .c(new_n44_), .O(new_n47_));
  nand3  g26(.a(new_n26_), .b(x03), .c(x02), .O(new_n48_));
  nand3  g27(.a(x13), .b(new_n44_), .c(new_n35_), .O(new_n49_));
  nor2   g28(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  oai21  g29(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n43_), .O(z2));
  nor2   g31(.a(x12), .b(x11), .O(new_n53_));
  nand4  g32(.a(new_n53_), .b(new_n26_), .c(x03), .d(x02), .O(new_n54_));
  oai21  g33(.a(x07), .b(x04), .c(x05), .O(new_n55_));
  aoi21  g34(.a(new_n54_), .b(x04), .c(new_n55_), .O(new_n56_));
  nand2  g35(.a(new_n55_), .b(x09), .O(new_n57_));
  oai21  g36(.a(new_n56_), .b(new_n34_), .c(new_n57_), .O(z3));
  inv1   g37(.a(x03), .O(new_n59_));
  inv1   g38(.a(x15), .O(new_n60_));
  nor2   g39(.a(new_n22_), .b(new_n60_), .O(new_n61_));
  nand4  g40(.a(new_n61_), .b(x14), .c(x10), .d(x08), .O(new_n62_));
  nor2   g41(.a(new_n62_), .b(new_n59_), .O(z4));
endmodule


