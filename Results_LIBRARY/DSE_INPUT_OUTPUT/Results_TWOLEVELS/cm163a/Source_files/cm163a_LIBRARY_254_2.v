// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x11), .O(new_n22_));
  nand2  g01(.a(new_n22_), .b(x07), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n24_), .b(x09), .c(x04), .O(new_n25_));
  inv1   g04(.a(x00), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(new_n25_), .c(x05), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  aoi21  g09(.a(new_n22_), .b(x07), .c(x09), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n30_), .O(z0));
  oai21  g12(.a(x04), .b(x01), .c(x05), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n23_), .O(new_n35_));
  inv1   g14(.a(new_n24_), .O(new_n36_));
  inv1   g15(.a(x09), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(x03), .c(x02), .O(new_n38_));
  nor3   g17(.a(x11), .b(x09), .c(x07), .O(new_n39_));
  aoi22  g18(.a(new_n39_), .b(new_n36_), .c(new_n38_), .d(x11), .O(new_n40_));
  oai21  g19(.a(new_n40_), .b(new_n27_), .c(new_n35_), .O(z1));
  inv1   g20(.a(x05), .O(new_n42_));
  nand2  g21(.a(new_n23_), .b(new_n42_), .O(new_n43_));
  inv1   g22(.a(x07), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n37_), .c(x04), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n24_), .c(new_n44_), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n22_), .O(new_n48_));
  nand3  g27(.a(new_n36_), .b(new_n22_), .c(new_n37_), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(x12), .c(x04), .O(new_n50_));
  inv1   g29(.a(x06), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n27_), .O(new_n52_));
  nand4  g31(.a(new_n52_), .b(new_n50_), .c(new_n48_), .d(new_n43_), .O(z2));
  inv1   g32(.a(x13), .O(new_n54_));
  nor2   g33(.a(x12), .b(x09), .O(new_n55_));
  aoi21  g34(.a(new_n55_), .b(new_n36_), .c(new_n54_), .O(new_n56_));
  nand3  g35(.a(new_n54_), .b(new_n45_), .c(new_n22_), .O(new_n57_));
  oai21  g36(.a(new_n57_), .b(new_n38_), .c(x04), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n56_), .c(new_n44_), .O(new_n59_));
  nand3  g38(.a(x13), .b(x11), .c(x04), .O(new_n60_));
  nand3  g39(.a(new_n60_), .b(new_n59_), .c(new_n43_), .O(z3));
  nand2  g40(.a(x08), .b(x03), .O(new_n62_));
  nand3  g41(.a(x15), .b(x14), .c(x10), .O(new_n63_));
  oai21  g42(.a(new_n63_), .b(new_n62_), .c(new_n23_), .O(z4));
endmodule


