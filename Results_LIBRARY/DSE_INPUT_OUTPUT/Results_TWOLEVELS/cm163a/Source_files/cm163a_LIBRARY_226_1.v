// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(x03), .c(x02), .O(new_n26_));
  aoi21  g05(.a(new_n26_), .b(new_n24_), .c(new_n22_), .O(new_n27_));
  nor2   g06(.a(x04), .b(x00), .O(new_n28_));
  oai21  g07(.a(new_n28_), .b(new_n27_), .c(x05), .O(new_n29_));
  nor2   g08(.a(x11), .b(x05), .O(new_n30_));
  inv1   g09(.a(new_n30_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n29_), .O(z0));
  inv1   g11(.a(x11), .O(new_n33_));
  nand2  g12(.a(new_n25_), .b(x04), .O(new_n34_));
  oai21  g13(.a(new_n34_), .b(new_n23_), .c(x05), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand3  g15(.a(new_n26_), .b(x11), .c(x04), .O(new_n37_));
  oai21  g16(.a(x04), .b(x01), .c(new_n37_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x05), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n36_), .O(z1));
  nand3  g19(.a(x12), .b(x11), .c(x04), .O(new_n41_));
  oai21  g20(.a(x06), .b(x04), .c(new_n41_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(x05), .O(new_n43_));
  inv1   g22(.a(x05), .O(new_n44_));
  nand2  g23(.a(new_n26_), .b(x12), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(new_n25_), .c(x03), .d(x02), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n45_), .c(new_n22_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n44_), .c(new_n33_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n43_), .O(z2));
  inv1   g29(.a(x13), .O(new_n51_));
  oai21  g30(.a(x12), .b(x11), .c(x05), .O(new_n52_));
  nand2  g31(.a(new_n26_), .b(new_n33_), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand3  g33(.a(new_n51_), .b(new_n46_), .c(new_n33_), .O(new_n55_));
  nor2   g34(.a(new_n55_), .b(new_n26_), .O(new_n56_));
  oai21  g35(.a(new_n56_), .b(new_n54_), .c(x04), .O(new_n57_));
  nor2   g36(.a(x07), .b(new_n44_), .O(new_n58_));
  aoi21  g37(.a(new_n58_), .b(new_n22_), .c(new_n30_), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n57_), .O(z3));
  inv1   g39(.a(x03), .O(new_n61_));
  inv1   g40(.a(x15), .O(new_n62_));
  aoi21  g41(.a(x11), .b(new_n44_), .c(new_n62_), .O(new_n63_));
  nand4  g42(.a(new_n63_), .b(x14), .c(x10), .d(x08), .O(new_n64_));
  nor2   g43(.a(new_n64_), .b(new_n61_), .O(z4));
endmodule


