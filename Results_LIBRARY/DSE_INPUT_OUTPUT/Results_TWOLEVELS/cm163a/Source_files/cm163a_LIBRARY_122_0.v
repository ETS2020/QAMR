// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x06), .b(x00), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x05), .O(new_n25_));
  oai21  g04(.a(x06), .b(x04), .c(new_n25_), .O(new_n26_));
  inv1   g05(.a(x09), .O(new_n27_));
  and2   g06(.a(x03), .b(x02), .O(new_n28_));
  nand3  g07(.a(new_n27_), .b(x03), .c(x02), .O(new_n29_));
  oai21  g08(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(new_n26_), .c(new_n24_), .O(z0));
  nand2  g11(.a(x06), .b(x01), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n22_), .O(new_n34_));
  nand2  g13(.a(new_n29_), .b(x11), .O(new_n35_));
  inv1   g14(.a(x11), .O(new_n36_));
  nand3  g15(.a(new_n28_), .b(new_n36_), .c(new_n27_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x04), .O(new_n39_));
  nand3  g18(.a(new_n39_), .b(new_n34_), .c(new_n26_), .O(z1));
  nor3   g19(.a(x12), .b(x11), .c(x09), .O(new_n41_));
  aoi22  g20(.a(new_n41_), .b(new_n28_), .c(new_n37_), .d(x12), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(new_n22_), .c(new_n26_), .O(z2));
  nand2  g22(.a(x07), .b(x06), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n22_), .O(new_n45_));
  inv1   g24(.a(x13), .O(new_n46_));
  aoi21  g25(.a(new_n41_), .b(new_n28_), .c(new_n46_), .O(new_n47_));
  inv1   g26(.a(x12), .O(new_n48_));
  nand3  g27(.a(new_n46_), .b(new_n48_), .c(new_n36_), .O(new_n49_));
  nor2   g28(.a(new_n49_), .b(new_n29_), .O(new_n50_));
  oai21  g29(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n45_), .c(new_n26_), .O(z3));
  inv1   g31(.a(x03), .O(new_n53_));
  inv1   g32(.a(x06), .O(new_n54_));
  inv1   g33(.a(x15), .O(new_n55_));
  aoi21  g34(.a(new_n54_), .b(new_n22_), .c(new_n55_), .O(new_n56_));
  nand4  g35(.a(new_n56_), .b(x14), .c(x10), .d(x08), .O(new_n57_));
  nor2   g36(.a(new_n57_), .b(new_n53_), .O(z4));
endmodule


