// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_;
  inv1   g00(.a(x05), .O(new_n22_));
  inv1   g01(.a(x13), .O(new_n23_));
  inv1   g02(.a(x15), .O(new_n24_));
  nor2   g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g04(.a(new_n25_), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(new_n22_), .O(new_n27_));
  inv1   g06(.a(x09), .O(new_n28_));
  and2   g07(.a(x03), .b(x02), .O(new_n29_));
  nand3  g08(.a(new_n28_), .b(x03), .c(x02), .O(new_n30_));
  oai21  g09(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x04), .O(new_n32_));
  inv1   g11(.a(x00), .O(new_n33_));
  inv1   g12(.a(x04), .O(new_n34_));
  aoi21  g13(.a(new_n34_), .b(new_n33_), .c(new_n25_), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(new_n32_), .c(new_n27_), .O(z0));
  nand2  g15(.a(new_n30_), .b(x11), .O(new_n37_));
  nor2   g16(.a(x11), .b(x09), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n29_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(x04), .O(new_n41_));
  nor2   g20(.a(x04), .b(x01), .O(new_n42_));
  nor2   g21(.a(new_n42_), .b(new_n25_), .O(new_n43_));
  nand3  g22(.a(new_n43_), .b(new_n41_), .c(new_n27_), .O(z1));
  inv1   g23(.a(x12), .O(new_n45_));
  aoi21  g24(.a(new_n38_), .b(new_n29_), .c(new_n45_), .O(new_n46_));
  nand2  g25(.a(x03), .b(x02), .O(new_n47_));
  inv1   g26(.a(x11), .O(new_n48_));
  nand3  g27(.a(new_n45_), .b(new_n48_), .c(new_n28_), .O(new_n49_));
  nor2   g28(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  oai21  g29(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  inv1   g30(.a(x06), .O(new_n52_));
  aoi21  g31(.a(new_n52_), .b(new_n34_), .c(new_n22_), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n51_), .c(new_n25_), .O(z2));
  nor3   g33(.a(x12), .b(x11), .c(x09), .O(new_n55_));
  aoi21  g34(.a(new_n55_), .b(new_n29_), .c(new_n23_), .O(new_n56_));
  nand3  g35(.a(new_n23_), .b(new_n45_), .c(new_n48_), .O(new_n57_));
  nor2   g36(.a(new_n57_), .b(new_n30_), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n56_), .c(x04), .O(new_n59_));
  nor2   g38(.a(x07), .b(x04), .O(new_n60_));
  nor2   g39(.a(new_n60_), .b(new_n25_), .O(new_n61_));
  nand3  g40(.a(new_n61_), .b(new_n59_), .c(new_n27_), .O(z3));
  nand4  g41(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n63_));
  aoi21  g42(.a(new_n63_), .b(new_n23_), .c(new_n24_), .O(z4));
endmodule


