// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x07), .O(new_n22_));
  nor2   g01(.a(x12), .b(new_n22_), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  and2   g04(.a(x03), .b(x02), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  aoi21  g10(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  nor2   g13(.a(new_n23_), .b(x09), .O(new_n35_));
  nand4  g14(.a(new_n35_), .b(x04), .c(x03), .d(x02), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n34_), .O(z0));
  nand3  g16(.a(new_n25_), .b(x03), .c(x02), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(x11), .c(x04), .O(new_n39_));
  inv1   g18(.a(new_n39_), .O(new_n40_));
  oai21  g19(.a(x04), .b(x01), .c(x05), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n40_), .c(new_n24_), .O(new_n42_));
  inv1   g21(.a(x11), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(x07), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n43_), .c(new_n25_), .d(x04), .O(new_n46_));
  inv1   g25(.a(new_n46_), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(x03), .c(x02), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n42_), .O(z1));
  nand2  g28(.a(x12), .b(x11), .O(new_n50_));
  nand3  g29(.a(new_n44_), .b(new_n43_), .c(new_n22_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand4  g31(.a(new_n52_), .b(new_n25_), .c(x03), .d(x02), .O(new_n53_));
  nand2  g32(.a(new_n38_), .b(x12), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(x04), .O(new_n56_));
  oai21  g35(.a(x06), .b(x04), .c(x05), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n24_), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(new_n56_), .O(z2));
  nand2  g38(.a(new_n24_), .b(new_n31_), .O(new_n60_));
  inv1   g39(.a(x13), .O(new_n61_));
  oai21  g40(.a(new_n61_), .b(new_n43_), .c(x04), .O(new_n62_));
  nand2  g41(.a(new_n62_), .b(new_n22_), .O(new_n63_));
  nand2  g42(.a(new_n38_), .b(x13), .O(new_n64_));
  nand4  g43(.a(new_n26_), .b(new_n61_), .c(new_n43_), .d(new_n25_), .O(new_n65_));
  nand3  g44(.a(new_n65_), .b(new_n64_), .c(new_n22_), .O(new_n66_));
  nand2  g45(.a(new_n66_), .b(new_n44_), .O(new_n67_));
  nand3  g46(.a(x13), .b(x12), .c(x04), .O(new_n68_));
  nand4  g47(.a(new_n68_), .b(new_n67_), .c(new_n63_), .d(new_n60_), .O(z3));
  inv1   g48(.a(x03), .O(new_n70_));
  inv1   g49(.a(x15), .O(new_n71_));
  nor2   g50(.a(new_n23_), .b(new_n71_), .O(new_n72_));
  nand4  g51(.a(new_n72_), .b(x14), .c(x10), .d(x08), .O(new_n73_));
  nor2   g52(.a(new_n73_), .b(new_n70_), .O(z4));
endmodule


