// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  aoi21  g00(.a(x05), .b(x00), .c(x04), .O(new_n22_));
  nand3  g01(.a(x04), .b(x03), .c(x02), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x05), .c(x09), .O(new_n24_));
  or2    g03(.a(new_n24_), .b(new_n22_), .O(z0));
  inv1   g04(.a(x04), .O(new_n26_));
  nand2  g05(.a(x05), .b(x01), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g07(.a(x05), .O(new_n29_));
  inv1   g08(.a(x09), .O(new_n30_));
  nand2  g09(.a(x03), .b(x02), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x11), .O(new_n32_));
  inv1   g11(.a(x11), .O(new_n33_));
  nand3  g12(.a(new_n33_), .b(x03), .c(x02), .O(new_n34_));
  aoi21  g13(.a(new_n34_), .b(new_n32_), .c(new_n26_), .O(new_n35_));
  oai21  g14(.a(new_n35_), .b(new_n29_), .c(new_n30_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n28_), .O(z1));
  nand2  g16(.a(x06), .b(x05), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n26_), .O(new_n39_));
  nand2  g18(.a(new_n34_), .b(x12), .O(new_n40_));
  inv1   g19(.a(x12), .O(new_n41_));
  and2   g20(.a(x03), .b(x02), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(new_n41_), .c(new_n33_), .O(new_n43_));
  aoi21  g22(.a(new_n43_), .b(new_n40_), .c(new_n26_), .O(new_n44_));
  oai21  g23(.a(new_n44_), .b(new_n29_), .c(new_n30_), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n39_), .O(z2));
  nand2  g25(.a(x07), .b(x05), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n26_), .O(new_n48_));
  inv1   g27(.a(x13), .O(new_n49_));
  oai21  g28(.a(x12), .b(x11), .c(new_n30_), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n42_), .c(new_n49_), .O(new_n51_));
  nand3  g30(.a(new_n49_), .b(new_n41_), .c(new_n33_), .O(new_n52_));
  oai21  g31(.a(new_n52_), .b(new_n31_), .c(new_n30_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n51_), .c(x04), .O(new_n54_));
  nand2  g33(.a(new_n30_), .b(new_n29_), .O(new_n55_));
  nand3  g34(.a(new_n55_), .b(new_n54_), .c(new_n48_), .O(z3));
  nand2  g35(.a(x08), .b(x03), .O(new_n57_));
  nand3  g36(.a(x15), .b(x14), .c(x10), .O(new_n58_));
  oai22  g37(.a(new_n58_), .b(new_n57_), .c(new_n30_), .d(new_n26_), .O(z4));
endmodule


