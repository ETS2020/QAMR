// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x03), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  oai21  g02(.a(new_n23_), .b(x04), .c(new_n22_), .O(new_n24_));
  inv1   g03(.a(x02), .O(new_n25_));
  nand2  g04(.a(x09), .b(new_n25_), .O(new_n26_));
  nand2  g05(.a(new_n23_), .b(x02), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  inv1   g08(.a(x00), .O(new_n30_));
  inv1   g09(.a(x04), .O(new_n31_));
  inv1   g10(.a(x05), .O(new_n32_));
  aoi21  g11(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  nand3  g12(.a(new_n33_), .b(new_n29_), .c(new_n24_), .O(z0));
  inv1   g13(.a(x11), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x03), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n23_), .O(new_n38_));
  nand3  g17(.a(new_n27_), .b(x11), .c(x04), .O(new_n39_));
  inv1   g18(.a(x01), .O(new_n40_));
  aoi21  g19(.a(new_n31_), .b(new_n40_), .c(new_n32_), .O(new_n41_));
  nand3  g20(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z1));
  inv1   g21(.a(x12), .O(new_n43_));
  nand4  g22(.a(new_n43_), .b(new_n35_), .c(x04), .d(x02), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(x03), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  nor2   g25(.a(x11), .b(x09), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x02), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(x12), .c(x04), .O(new_n49_));
  inv1   g28(.a(x06), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n31_), .c(new_n32_), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n49_), .c(new_n46_), .O(z2));
  nand2  g31(.a(x13), .b(x12), .O(new_n53_));
  inv1   g32(.a(x13), .O(new_n54_));
  nand4  g33(.a(new_n54_), .b(new_n43_), .c(new_n35_), .d(new_n23_), .O(new_n55_));
  aoi21  g34(.a(new_n55_), .b(new_n53_), .c(new_n25_), .O(new_n56_));
  aoi21  g35(.a(new_n47_), .b(x02), .c(new_n54_), .O(new_n57_));
  oai21  g36(.a(new_n57_), .b(new_n56_), .c(x04), .O(new_n58_));
  oai21  g37(.a(x07), .b(x04), .c(x05), .O(new_n59_));
  aoi21  g38(.a(new_n23_), .b(new_n22_), .c(new_n59_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n58_), .O(z3));
  inv1   g40(.a(x15), .O(new_n62_));
  nand4  g41(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n63_));
  nor2   g42(.a(new_n63_), .b(new_n62_), .O(z4));
endmodule


