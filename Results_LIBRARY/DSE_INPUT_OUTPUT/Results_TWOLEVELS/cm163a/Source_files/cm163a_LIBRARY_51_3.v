// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_;
  inv1   g00(.a(x00), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(x03), .c(x02), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(new_n24_), .c(x04), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n22_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x05), .O(z0));
  inv1   g08(.a(x04), .O(new_n30_));
  inv1   g09(.a(new_n23_), .O(new_n31_));
  nor2   g10(.a(x11), .b(x09), .O(new_n32_));
  aoi22  g11(.a(new_n32_), .b(new_n31_), .c(new_n26_), .d(x11), .O(new_n33_));
  oai21  g12(.a(x04), .b(x01), .c(x05), .O(new_n34_));
  nor2   g13(.a(new_n34_), .b(x00), .O(new_n35_));
  oai21  g14(.a(new_n33_), .b(new_n30_), .c(new_n35_), .O(z1));
  nand2  g15(.a(x11), .b(new_n22_), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n31_), .c(new_n25_), .O(new_n38_));
  nor3   g17(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  aoi22  g18(.a(new_n39_), .b(new_n31_), .c(new_n38_), .d(x12), .O(new_n40_));
  oai21  g19(.a(x06), .b(x04), .c(x05), .O(new_n41_));
  nor2   g20(.a(new_n41_), .b(x00), .O(new_n42_));
  oai21  g21(.a(new_n40_), .b(new_n30_), .c(new_n42_), .O(z2));
  inv1   g22(.a(x03), .O(new_n44_));
  nor2   g23(.a(x09), .b(new_n44_), .O(new_n45_));
  oai21  g24(.a(x12), .b(x11), .c(new_n22_), .O(new_n46_));
  nand3  g25(.a(new_n46_), .b(new_n45_), .c(x02), .O(new_n47_));
  inv1   g26(.a(x11), .O(new_n48_));
  inv1   g27(.a(x12), .O(new_n49_));
  inv1   g28(.a(x13), .O(new_n50_));
  nand3  g29(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor2   g30(.a(new_n51_), .b(new_n26_), .O(new_n52_));
  aoi21  g31(.a(new_n47_), .b(x13), .c(new_n52_), .O(new_n53_));
  oai21  g32(.a(x07), .b(x04), .c(x05), .O(new_n54_));
  nor2   g33(.a(new_n54_), .b(x00), .O(new_n55_));
  oai21  g34(.a(new_n53_), .b(new_n30_), .c(new_n55_), .O(z3));
  inv1   g35(.a(x08), .O(new_n57_));
  nand2  g36(.a(x05), .b(x00), .O(new_n58_));
  nand4  g37(.a(new_n58_), .b(x15), .c(x14), .d(x10), .O(new_n59_));
  nor3   g38(.a(new_n59_), .b(new_n57_), .c(new_n44_), .O(z4));
endmodule


