// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand3  g01(.a(new_n22_), .b(x04), .c(x03), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x05), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x02), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(x09), .c(x04), .O(new_n27_));
  oai21  g06(.a(x04), .b(x00), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x05), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n25_), .O(z0));
  inv1   g09(.a(x04), .O(new_n31_));
  nand3  g10(.a(new_n22_), .b(x03), .c(x02), .O(new_n32_));
  inv1   g11(.a(x11), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n22_), .c(x03), .d(x02), .O(new_n34_));
  inv1   g13(.a(new_n34_), .O(new_n35_));
  aoi21  g14(.a(new_n32_), .b(x11), .c(new_n35_), .O(new_n36_));
  inv1   g15(.a(x01), .O(new_n37_));
  inv1   g16(.a(x05), .O(new_n38_));
  aoi21  g17(.a(new_n31_), .b(new_n37_), .c(new_n38_), .O(new_n39_));
  oai21  g18(.a(new_n36_), .b(new_n31_), .c(new_n39_), .O(z1));
  nand2  g19(.a(x04), .b(x03), .O(new_n41_));
  nor2   g20(.a(x12), .b(x11), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n22_), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n41_), .c(x05), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(x02), .O(new_n45_));
  nand3  g24(.a(new_n34_), .b(x12), .c(x04), .O(new_n46_));
  inv1   g25(.a(x06), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(x05), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n45_), .O(z2));
  inv1   g30(.a(new_n32_), .O(new_n52_));
  nand4  g31(.a(new_n42_), .b(new_n22_), .c(x03), .d(x02), .O(new_n53_));
  nor3   g32(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  aoi22  g33(.a(new_n54_), .b(new_n52_), .c(new_n53_), .d(x13), .O(new_n55_));
  inv1   g34(.a(x07), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n31_), .c(new_n38_), .O(new_n57_));
  oai21  g36(.a(new_n55_), .b(new_n31_), .c(new_n57_), .O(z3));
  inv1   g37(.a(x03), .O(new_n59_));
  inv1   g38(.a(x02), .O(new_n60_));
  inv1   g39(.a(x15), .O(new_n61_));
  aoi21  g40(.a(new_n38_), .b(new_n60_), .c(new_n61_), .O(new_n62_));
  nand4  g41(.a(new_n62_), .b(x14), .c(x10), .d(x08), .O(new_n63_));
  nor2   g42(.a(new_n63_), .b(new_n59_), .O(z4));
endmodule


