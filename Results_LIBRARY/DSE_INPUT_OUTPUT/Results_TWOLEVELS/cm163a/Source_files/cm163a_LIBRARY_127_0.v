// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x05), .O(new_n22_));
  nor2   g01(.a(x13), .b(x09), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  inv1   g04(.a(x09), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  inv1   g06(.a(new_n27_), .O(new_n28_));
  nand4  g07(.a(x13), .b(new_n26_), .c(x03), .d(x02), .O(new_n29_));
  oai21  g08(.a(new_n28_), .b(new_n26_), .c(new_n29_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  inv1   g10(.a(x00), .O(new_n32_));
  inv1   g11(.a(x04), .O(new_n33_));
  aoi21  g12(.a(new_n33_), .b(new_n32_), .c(new_n23_), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(new_n31_), .c(new_n25_), .O(z0));
  oai21  g14(.a(x04), .b(x01), .c(x05), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n24_), .O(new_n37_));
  inv1   g16(.a(x13), .O(new_n38_));
  inv1   g17(.a(x11), .O(new_n39_));
  nand4  g18(.a(new_n39_), .b(new_n26_), .c(x03), .d(x02), .O(new_n40_));
  nand2  g19(.a(new_n27_), .b(x11), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nor2   g21(.a(new_n39_), .b(new_n26_), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n42_), .c(x04), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n37_), .O(z1));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  inv1   g25(.a(x12), .O(new_n47_));
  nand3  g26(.a(x13), .b(new_n47_), .c(new_n39_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n46_), .c(x13), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n26_), .O(new_n50_));
  nand3  g29(.a(new_n28_), .b(new_n39_), .c(new_n26_), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(x12), .c(x04), .O(new_n52_));
  inv1   g31(.a(x06), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  nand4  g33(.a(new_n54_), .b(new_n52_), .c(new_n50_), .d(new_n25_), .O(z2));
  nor2   g34(.a(x12), .b(x11), .O(new_n56_));
  nand4  g35(.a(new_n56_), .b(new_n26_), .c(x03), .d(x02), .O(new_n57_));
  oai21  g36(.a(x07), .b(x04), .c(x05), .O(new_n58_));
  aoi21  g37(.a(new_n57_), .b(x04), .c(new_n58_), .O(new_n59_));
  nand2  g38(.a(new_n58_), .b(x09), .O(new_n60_));
  oai21  g39(.a(new_n59_), .b(new_n38_), .c(new_n60_), .O(z3));
  inv1   g40(.a(x03), .O(new_n62_));
  inv1   g41(.a(x15), .O(new_n63_));
  nor2   g42(.a(new_n23_), .b(new_n63_), .O(new_n64_));
  nand4  g43(.a(new_n64_), .b(x14), .c(x10), .d(x08), .O(new_n65_));
  nor2   g44(.a(new_n65_), .b(new_n62_), .O(z4));
endmodule


