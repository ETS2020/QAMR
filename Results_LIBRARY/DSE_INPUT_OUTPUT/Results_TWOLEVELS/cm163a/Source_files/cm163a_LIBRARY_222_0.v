// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x05), .O(new_n22_));
  oai21  g01(.a(x13), .b(x09), .c(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x09), .O(new_n25_));
  inv1   g04(.a(x09), .O(new_n26_));
  inv1   g05(.a(x13), .O(new_n27_));
  oai21  g06(.a(x12), .b(x11), .c(new_n27_), .O(new_n28_));
  nand4  g07(.a(new_n28_), .b(new_n26_), .c(x03), .d(x02), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  nor2   g10(.a(x13), .b(x09), .O(new_n32_));
  nor2   g11(.a(x04), .b(x00), .O(new_n33_));
  nor2   g12(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(new_n31_), .c(new_n23_), .O(z0));
  aoi21  g14(.a(new_n27_), .b(x12), .c(x11), .O(new_n36_));
  nand4  g15(.a(new_n36_), .b(new_n26_), .c(x03), .d(x02), .O(new_n37_));
  nand3  g16(.a(new_n26_), .b(x03), .c(x02), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x11), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(x04), .O(new_n41_));
  nor2   g20(.a(x04), .b(x01), .O(new_n42_));
  nor3   g21(.a(new_n42_), .b(new_n32_), .c(new_n22_), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n41_), .O(z1));
  inv1   g23(.a(x11), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand3  g25(.a(new_n46_), .b(new_n45_), .c(x04), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n24_), .c(x13), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n26_), .O(new_n49_));
  nand4  g28(.a(new_n45_), .b(new_n26_), .c(x03), .d(x02), .O(new_n50_));
  nand3  g29(.a(new_n50_), .b(x12), .c(x04), .O(new_n51_));
  or2    g30(.a(x06), .b(x04), .O(new_n52_));
  nand4  g31(.a(new_n52_), .b(new_n51_), .c(new_n49_), .d(new_n23_), .O(z2));
  nor2   g32(.a(x12), .b(x11), .O(new_n54_));
  nand4  g33(.a(new_n54_), .b(new_n26_), .c(x03), .d(x02), .O(new_n55_));
  oai21  g34(.a(x07), .b(x04), .c(x05), .O(new_n56_));
  aoi21  g35(.a(new_n55_), .b(x04), .c(new_n56_), .O(new_n57_));
  nand2  g36(.a(new_n56_), .b(x09), .O(new_n58_));
  oai21  g37(.a(new_n57_), .b(new_n27_), .c(new_n58_), .O(z3));
  inv1   g38(.a(x03), .O(new_n60_));
  inv1   g39(.a(x15), .O(new_n61_));
  nor2   g40(.a(new_n32_), .b(new_n61_), .O(new_n62_));
  nand4  g41(.a(new_n62_), .b(x14), .c(x10), .d(x08), .O(new_n63_));
  nor2   g42(.a(new_n63_), .b(new_n60_), .O(z4));
endmodule


