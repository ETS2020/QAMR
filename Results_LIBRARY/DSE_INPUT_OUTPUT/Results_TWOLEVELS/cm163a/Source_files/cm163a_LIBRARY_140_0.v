// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_;
  nor2   g00(.a(x08), .b(x06), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  and2   g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n23_), .b(x03), .c(x02), .O(new_n25_));
  oai21  g04(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g06(.a(x00), .O(new_n28_));
  inv1   g07(.a(x04), .O(new_n29_));
  inv1   g08(.a(x05), .O(new_n30_));
  aoi21  g09(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  aoi21  g10(.a(new_n31_), .b(new_n27_), .c(new_n22_), .O(z0));
  inv1   g11(.a(new_n22_), .O(new_n33_));
  nand2  g12(.a(new_n25_), .b(x11), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand3  g14(.a(new_n24_), .b(new_n35_), .c(new_n23_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x04), .O(new_n38_));
  inv1   g17(.a(x01), .O(new_n39_));
  nand2  g18(.a(new_n29_), .b(new_n39_), .O(new_n40_));
  nand4  g19(.a(new_n40_), .b(new_n38_), .c(new_n33_), .d(x05), .O(z1));
  nand2  g20(.a(x12), .b(x11), .O(new_n42_));
  nor2   g21(.a(x12), .b(x11), .O(new_n43_));
  nand4  g22(.a(new_n43_), .b(new_n23_), .c(x03), .d(x02), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n42_), .c(new_n29_), .O(new_n45_));
  oai21  g24(.a(new_n45_), .b(new_n30_), .c(new_n33_), .O(new_n46_));
  inv1   g25(.a(x08), .O(new_n47_));
  nand2  g26(.a(x06), .b(x04), .O(new_n48_));
  oai21  g27(.a(new_n47_), .b(x06), .c(new_n48_), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(new_n25_), .c(x12), .O(new_n50_));
  inv1   g29(.a(x06), .O(new_n51_));
  nand3  g30(.a(x08), .b(new_n51_), .c(new_n29_), .O(new_n52_));
  and2   g31(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n46_), .O(z2));
  inv1   g33(.a(x13), .O(new_n55_));
  nor3   g34(.a(x12), .b(x11), .c(x09), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n24_), .c(new_n55_), .O(new_n57_));
  inv1   g36(.a(x12), .O(new_n58_));
  nand3  g37(.a(new_n55_), .b(new_n58_), .c(new_n35_), .O(new_n59_));
  nor2   g38(.a(new_n59_), .b(new_n25_), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n57_), .c(x04), .O(new_n61_));
  inv1   g40(.a(x07), .O(new_n62_));
  nand2  g41(.a(new_n62_), .b(new_n29_), .O(new_n63_));
  nand4  g42(.a(new_n63_), .b(new_n61_), .c(new_n33_), .d(x05), .O(z3));
  nand2  g43(.a(x08), .b(x03), .O(new_n65_));
  nand3  g44(.a(x15), .b(x14), .c(x10), .O(new_n66_));
  oai21  g45(.a(new_n66_), .b(new_n65_), .c(new_n33_), .O(z4));
endmodule


