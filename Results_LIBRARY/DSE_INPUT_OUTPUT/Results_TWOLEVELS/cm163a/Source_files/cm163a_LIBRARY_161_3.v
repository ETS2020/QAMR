// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x02), .O(new_n22_));
  inv1   g01(.a(x04), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  oai21  g03(.a(new_n23_), .b(new_n22_), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x03), .O(new_n26_));
  inv1   g05(.a(x05), .O(new_n27_));
  nand2  g06(.a(x09), .b(x03), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nor2   g08(.a(x04), .b(x00), .O(new_n30_));
  aoi21  g09(.a(x09), .b(x04), .c(new_n30_), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(new_n29_), .c(new_n26_), .O(z0));
  inv1   g11(.a(x11), .O(new_n33_));
  nand3  g12(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n24_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x03), .O(new_n36_));
  inv1   g15(.a(x03), .O(new_n37_));
  nor2   g16(.a(new_n33_), .b(new_n23_), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n27_), .c(new_n37_), .O(new_n39_));
  oai22  g18(.a(x09), .b(x05), .c(x04), .d(x01), .O(new_n40_));
  aoi21  g19(.a(new_n38_), .b(new_n22_), .c(new_n40_), .O(new_n41_));
  nand3  g20(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z1));
  oai21  g21(.a(x06), .b(x04), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n28_), .O(new_n44_));
  oai21  g23(.a(x11), .b(new_n22_), .c(x12), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(new_n33_), .c(x03), .d(x02), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n45_), .c(x09), .O(new_n48_));
  nor2   g27(.a(new_n46_), .b(x03), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n44_), .O(z2));
  oai21  g30(.a(x07), .b(x04), .c(x05), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n28_), .O(new_n53_));
  nand3  g32(.a(new_n46_), .b(new_n33_), .c(x02), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(x13), .O(new_n55_));
  nor2   g34(.a(x13), .b(x12), .O(new_n56_));
  nand4  g35(.a(new_n56_), .b(new_n33_), .c(x03), .d(x02), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n55_), .c(x09), .O(new_n58_));
  inv1   g37(.a(x13), .O(new_n59_));
  nor2   g38(.a(new_n59_), .b(x03), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n58_), .c(x04), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n53_), .O(z3));
  inv1   g41(.a(x14), .O(new_n63_));
  inv1   g42(.a(x15), .O(new_n64_));
  nand4  g43(.a(x10), .b(new_n24_), .c(x08), .d(x03), .O(new_n65_));
  nor3   g44(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(z4));
endmodule


