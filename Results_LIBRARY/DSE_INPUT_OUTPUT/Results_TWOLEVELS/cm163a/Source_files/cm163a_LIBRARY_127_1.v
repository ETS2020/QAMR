// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand2  g01(.a(x13), .b(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  inv1   g03(.a(new_n24_), .O(new_n25_));
  nand3  g04(.a(new_n22_), .b(x03), .c(x02), .O(new_n26_));
  oai21  g05(.a(new_n25_), .b(new_n22_), .c(new_n26_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(new_n28_), .c(new_n23_), .d(x05), .O(z0));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n23_), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  inv1   g14(.a(x13), .O(new_n36_));
  nand2  g15(.a(new_n24_), .b(new_n36_), .O(new_n37_));
  aoi21  g16(.a(new_n37_), .b(new_n22_), .c(new_n35_), .O(new_n38_));
  nand3  g17(.a(new_n36_), .b(new_n35_), .c(new_n22_), .O(new_n39_));
  nor2   g18(.a(new_n39_), .b(new_n24_), .O(new_n40_));
  oai21  g19(.a(new_n40_), .b(new_n38_), .c(x04), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n34_), .O(z1));
  inv1   g21(.a(x05), .O(new_n43_));
  nand2  g22(.a(new_n23_), .b(new_n43_), .O(new_n44_));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand3  g25(.a(new_n36_), .b(new_n46_), .c(new_n35_), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n45_), .c(new_n36_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n22_), .O(new_n49_));
  nand3  g28(.a(new_n25_), .b(new_n35_), .c(new_n22_), .O(new_n50_));
  nand3  g29(.a(new_n50_), .b(x12), .c(x04), .O(new_n51_));
  inv1   g30(.a(x06), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n30_), .O(new_n53_));
  nand4  g32(.a(new_n53_), .b(new_n51_), .c(new_n49_), .d(new_n44_), .O(z2));
  nand2  g33(.a(x13), .b(x09), .O(new_n55_));
  oai21  g34(.a(new_n47_), .b(new_n26_), .c(new_n55_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(x04), .O(new_n57_));
  inv1   g36(.a(x07), .O(new_n58_));
  oai21  g37(.a(x13), .b(x04), .c(new_n55_), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g39(.a(new_n60_), .b(new_n57_), .c(new_n44_), .O(z3));
  inv1   g40(.a(x03), .O(new_n62_));
  inv1   g41(.a(x15), .O(new_n63_));
  aoi21  g42(.a(x13), .b(new_n22_), .c(new_n63_), .O(new_n64_));
  nand4  g43(.a(new_n64_), .b(x14), .c(x10), .d(x08), .O(new_n65_));
  nor2   g44(.a(new_n65_), .b(new_n62_), .O(z4));
endmodule


