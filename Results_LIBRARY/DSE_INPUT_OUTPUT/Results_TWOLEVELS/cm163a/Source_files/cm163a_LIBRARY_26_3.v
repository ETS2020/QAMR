// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_;
  nand2  g00(.a(x11), .b(x09), .O(new_n22_));
  oai21  g01(.a(x04), .b(x00), .c(x05), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x11), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(new_n25_), .c(x09), .O(new_n27_));
  inv1   g06(.a(x09), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(x03), .c(x02), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n24_), .O(z0));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n22_), .O(new_n34_));
  inv1   g13(.a(new_n26_), .O(new_n35_));
  nand3  g14(.a(new_n25_), .b(x03), .c(x02), .O(new_n36_));
  oai21  g15(.a(new_n35_), .b(new_n25_), .c(new_n36_), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n28_), .c(x04), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n34_), .O(z1));
  oai21  g18(.a(x06), .b(x04), .c(x05), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n22_), .O(new_n41_));
  nand2  g20(.a(x12), .b(x11), .O(new_n42_));
  inv1   g21(.a(x12), .O(new_n43_));
  nand4  g22(.a(new_n43_), .b(new_n25_), .c(x03), .d(x02), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n42_), .c(x09), .O(new_n45_));
  nand3  g24(.a(new_n29_), .b(x12), .c(new_n25_), .O(new_n46_));
  inv1   g25(.a(new_n46_), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n45_), .c(x04), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n41_), .O(z2));
  inv1   g28(.a(x05), .O(new_n50_));
  nand2  g29(.a(new_n22_), .b(new_n50_), .O(new_n51_));
  inv1   g30(.a(x04), .O(new_n52_));
  inv1   g31(.a(x13), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n52_), .c(new_n25_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(x09), .O(new_n55_));
  oai21  g34(.a(x12), .b(x11), .c(new_n28_), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n35_), .c(new_n53_), .O(new_n57_));
  nand3  g36(.a(new_n53_), .b(new_n43_), .c(new_n25_), .O(new_n58_));
  nor2   g37(.a(new_n58_), .b(new_n29_), .O(new_n59_));
  oai21  g38(.a(new_n59_), .b(new_n57_), .c(x04), .O(new_n60_));
  inv1   g39(.a(x07), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n52_), .O(new_n62_));
  nand4  g41(.a(new_n62_), .b(new_n60_), .c(new_n55_), .d(new_n51_), .O(z3));
  nand2  g42(.a(x08), .b(x03), .O(new_n64_));
  nand3  g43(.a(x15), .b(x14), .c(x10), .O(new_n65_));
  oai21  g44(.a(new_n65_), .b(new_n64_), .c(new_n22_), .O(z4));
endmodule


