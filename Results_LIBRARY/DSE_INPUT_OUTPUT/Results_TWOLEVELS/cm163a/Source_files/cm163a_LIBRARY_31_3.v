// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x05), .b(x00), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  and2   g04(.a(x03), .b(x02), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g06(.a(x12), .O(new_n28_));
  nand3  g07(.a(new_n25_), .b(x03), .c(x02), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  oai21  g09(.a(new_n30_), .b(new_n27_), .c(x04), .O(new_n31_));
  nor2   g10(.a(x12), .b(x05), .O(new_n32_));
  inv1   g11(.a(new_n32_), .O(new_n33_));
  nand3  g12(.a(new_n33_), .b(new_n31_), .c(new_n24_), .O(z0));
  nand2  g13(.a(x05), .b(x01), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n22_), .O(new_n36_));
  inv1   g15(.a(x05), .O(new_n37_));
  nand2  g16(.a(new_n29_), .b(x11), .O(new_n38_));
  inv1   g17(.a(x11), .O(new_n39_));
  nand4  g18(.a(new_n39_), .b(new_n25_), .c(x03), .d(x02), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(new_n38_), .c(new_n22_), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n37_), .c(new_n28_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n36_), .O(z1));
  nor3   g22(.a(x12), .b(x11), .c(x09), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n26_), .c(x12), .O(new_n45_));
  aoi21  g24(.a(x06), .b(x05), .c(x04), .O(new_n46_));
  nor2   g25(.a(new_n46_), .b(new_n32_), .O(new_n47_));
  oai21  g26(.a(new_n45_), .b(new_n22_), .c(new_n47_), .O(z2));
  nand2  g27(.a(x07), .b(x05), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n22_), .O(new_n50_));
  nand2  g29(.a(new_n40_), .b(x13), .O(new_n51_));
  nor2   g30(.a(x13), .b(x11), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n26_), .c(new_n25_), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n51_), .c(new_n22_), .O(new_n54_));
  oai21  g33(.a(new_n54_), .b(new_n37_), .c(new_n28_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n50_), .O(z3));
  inv1   g35(.a(x03), .O(new_n57_));
  inv1   g36(.a(x08), .O(new_n58_));
  nand2  g37(.a(x12), .b(x04), .O(new_n59_));
  nand4  g38(.a(new_n59_), .b(x15), .c(x14), .d(x10), .O(new_n60_));
  nor3   g39(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(z4));
endmodule


