// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_;
  nor2   g00(.a(x15), .b(x13), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  and2   g03(.a(x03), .b(x02), .O(new_n25_));
  nor2   g04(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g06(.a(new_n27_), .O(new_n28_));
  oai21  g07(.a(x04), .b(x00), .c(x05), .O(new_n29_));
  oai21  g08(.a(new_n29_), .b(new_n28_), .c(new_n23_), .O(new_n30_));
  nor2   g09(.a(new_n22_), .b(x09), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n30_), .O(z0));
  nand3  g12(.a(new_n24_), .b(x03), .c(x02), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(x11), .c(x04), .O(new_n35_));
  inv1   g14(.a(new_n35_), .O(new_n36_));
  oai21  g15(.a(x04), .b(x01), .c(x05), .O(new_n37_));
  oai21  g16(.a(new_n37_), .b(new_n36_), .c(new_n23_), .O(new_n38_));
  nor3   g17(.a(new_n22_), .b(x11), .c(x09), .O(new_n39_));
  nand4  g18(.a(new_n39_), .b(x04), .c(x03), .d(x02), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n38_), .O(z1));
  inv1   g20(.a(x02), .O(new_n42_));
  inv1   g21(.a(x11), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  nand4  g23(.a(new_n44_), .b(new_n43_), .c(new_n24_), .d(x03), .O(new_n45_));
  nor2   g24(.a(new_n45_), .b(new_n42_), .O(new_n46_));
  nor2   g25(.a(x11), .b(x09), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n25_), .c(new_n44_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n46_), .c(x04), .O(new_n49_));
  inv1   g28(.a(x05), .O(new_n50_));
  nor2   g29(.a(x06), .b(x04), .O(new_n51_));
  nor3   g30(.a(new_n51_), .b(new_n22_), .c(new_n50_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n49_), .O(z2));
  nand2  g32(.a(new_n23_), .b(new_n50_), .O(new_n54_));
  xor2a  g33(.a(x13), .b(x12), .O(new_n55_));
  nand2  g34(.a(x13), .b(x11), .O(new_n56_));
  oai21  g35(.a(new_n55_), .b(x11), .c(new_n56_), .O(new_n57_));
  nand4  g36(.a(new_n57_), .b(new_n24_), .c(x03), .d(x02), .O(new_n58_));
  nand2  g37(.a(new_n34_), .b(x13), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(x04), .O(new_n61_));
  nor2   g40(.a(x07), .b(x04), .O(new_n62_));
  nor2   g41(.a(new_n62_), .b(new_n22_), .O(new_n63_));
  nand3  g42(.a(new_n63_), .b(new_n61_), .c(new_n54_), .O(z3));
  nand2  g43(.a(x08), .b(x03), .O(new_n65_));
  nand3  g44(.a(x15), .b(x14), .c(x10), .O(new_n66_));
  oai21  g45(.a(new_n66_), .b(new_n65_), .c(new_n23_), .O(z4));
endmodule


