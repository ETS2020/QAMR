// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_;
  inv1   g00(.a(x00), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(x03), .O(new_n24_));
  nor2   g03(.a(x09), .b(x04), .O(new_n25_));
  oai21  g04(.a(new_n25_), .b(new_n24_), .c(new_n22_), .O(new_n26_));
  inv1   g05(.a(x03), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  oai21  g07(.a(new_n23_), .b(new_n28_), .c(x05), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand3  g09(.a(x04), .b(x03), .c(x02), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x05), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  nand3  g12(.a(new_n33_), .b(new_n30_), .c(new_n26_), .O(z0));
  nand2  g13(.a(x09), .b(x03), .O(new_n35_));
  oai21  g14(.a(x04), .b(x01), .c(x05), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g16(.a(x02), .O(new_n38_));
  nand2  g17(.a(x11), .b(new_n38_), .O(new_n39_));
  inv1   g18(.a(x11), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(x03), .c(x02), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n39_), .c(x09), .O(new_n42_));
  nor2   g21(.a(new_n40_), .b(x03), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n42_), .c(x04), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n37_), .O(z1));
  oai21  g24(.a(x06), .b(x04), .c(x05), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n35_), .O(new_n47_));
  oai21  g26(.a(x11), .b(new_n38_), .c(x12), .O(new_n48_));
  nor2   g27(.a(x12), .b(x11), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(x03), .c(x02), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n48_), .c(x09), .O(new_n51_));
  inv1   g30(.a(x12), .O(new_n52_));
  nor2   g31(.a(new_n52_), .b(x03), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n51_), .c(x04), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n47_), .O(z2));
  nand2  g34(.a(x04), .b(x02), .O(new_n56_));
  inv1   g35(.a(x13), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(new_n52_), .c(new_n40_), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n56_), .c(new_n23_), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(x03), .O(new_n60_));
  oai21  g39(.a(new_n57_), .b(new_n28_), .c(x05), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n27_), .O(new_n62_));
  oai21  g41(.a(new_n49_), .b(x09), .c(x02), .O(new_n63_));
  nand3  g42(.a(new_n63_), .b(x13), .c(x04), .O(new_n64_));
  nor2   g43(.a(x07), .b(x04), .O(new_n65_));
  nor2   g44(.a(x09), .b(x05), .O(new_n66_));
  nor2   g45(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g46(.a(new_n67_), .b(new_n64_), .c(new_n62_), .d(new_n60_), .O(z3));
  inv1   g47(.a(x14), .O(new_n69_));
  inv1   g48(.a(x15), .O(new_n70_));
  nand4  g49(.a(x10), .b(new_n23_), .c(x08), .d(x03), .O(new_n71_));
  nor3   g50(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(z4));
endmodule


