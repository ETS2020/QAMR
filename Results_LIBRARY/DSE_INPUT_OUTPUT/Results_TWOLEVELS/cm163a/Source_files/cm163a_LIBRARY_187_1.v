// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_;
  inv1   g00(.a(x11), .O(new_n22_));
  nand2  g01(.a(x12), .b(new_n22_), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  nand2  g03(.a(x03), .b(x02), .O(new_n25_));
  inv1   g04(.a(new_n25_), .O(new_n26_));
  nand3  g05(.a(new_n24_), .b(x03), .c(x02), .O(new_n27_));
  oai21  g06(.a(new_n26_), .b(new_n24_), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  inv1   g08(.a(x00), .O(new_n30_));
  inv1   g09(.a(x04), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand4  g11(.a(new_n32_), .b(new_n29_), .c(new_n23_), .d(x05), .O(z0));
  oai21  g12(.a(x04), .b(x01), .c(x05), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n23_), .O(new_n35_));
  nor3   g14(.a(x12), .b(x11), .c(x09), .O(new_n36_));
  aoi22  g15(.a(new_n36_), .b(new_n26_), .c(new_n27_), .d(x11), .O(new_n37_));
  oai21  g16(.a(new_n37_), .b(new_n31_), .c(new_n35_), .O(z1));
  aoi21  g17(.a(new_n36_), .b(new_n26_), .c(x12), .O(new_n39_));
  inv1   g18(.a(x06), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n31_), .O(new_n41_));
  nand3  g20(.a(new_n41_), .b(new_n23_), .c(x05), .O(new_n42_));
  inv1   g21(.a(new_n42_), .O(new_n43_));
  oai21  g22(.a(new_n39_), .b(new_n31_), .c(new_n43_), .O(z2));
  inv1   g23(.a(x05), .O(new_n45_));
  nand2  g24(.a(new_n23_), .b(new_n45_), .O(new_n46_));
  inv1   g25(.a(x12), .O(new_n47_));
  inv1   g26(.a(x13), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n24_), .c(x04), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n25_), .c(new_n47_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  nand3  g30(.a(new_n26_), .b(new_n22_), .c(new_n24_), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(x13), .c(x04), .O(new_n53_));
  inv1   g32(.a(x07), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n31_), .O(new_n55_));
  nand4  g34(.a(new_n55_), .b(new_n53_), .c(new_n51_), .d(new_n46_), .O(z3));
  inv1   g35(.a(x03), .O(new_n57_));
  inv1   g36(.a(x08), .O(new_n58_));
  nand4  g37(.a(new_n23_), .b(x15), .c(x14), .d(x10), .O(new_n59_));
  nor3   g38(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(z4));
endmodule


