// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_;
  nor2   g00(.a(x13), .b(x08), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  and2   g03(.a(x03), .b(x02), .O(new_n25_));
  nand3  g04(.a(new_n24_), .b(x03), .c(x02), .O(new_n26_));
  oai21  g05(.a(new_n25_), .b(new_n24_), .c(new_n26_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  or2    g07(.a(x04), .b(x00), .O(new_n29_));
  nand4  g08(.a(new_n29_), .b(new_n28_), .c(new_n23_), .d(x05), .O(z0));
  nand3  g09(.a(new_n26_), .b(x11), .c(x04), .O(new_n31_));
  inv1   g10(.a(new_n31_), .O(new_n32_));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  oai21  g12(.a(new_n33_), .b(new_n32_), .c(new_n23_), .O(new_n34_));
  nor3   g13(.a(new_n22_), .b(x11), .c(x09), .O(new_n35_));
  nand4  g14(.a(new_n35_), .b(x04), .c(x03), .d(x02), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n34_), .O(z1));
  inv1   g16(.a(x02), .O(new_n38_));
  inv1   g17(.a(x11), .O(new_n39_));
  inv1   g18(.a(x08), .O(new_n40_));
  inv1   g19(.a(x13), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n40_), .c(x12), .O(new_n42_));
  nand4  g21(.a(new_n42_), .b(new_n39_), .c(new_n24_), .d(x03), .O(new_n43_));
  nand3  g22(.a(new_n25_), .b(new_n39_), .c(new_n24_), .O(new_n44_));
  nand3  g23(.a(new_n44_), .b(new_n23_), .c(x12), .O(new_n45_));
  oai21  g24(.a(new_n43_), .b(new_n38_), .c(new_n45_), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g26(.a(x06), .b(x04), .c(x05), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n23_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n47_), .O(z2));
  nand2  g29(.a(x13), .b(x12), .O(new_n51_));
  inv1   g30(.a(x12), .O(new_n52_));
  nand3  g31(.a(new_n41_), .b(new_n52_), .c(x08), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n51_), .c(x11), .O(new_n54_));
  nand4  g33(.a(new_n54_), .b(new_n24_), .c(x03), .d(x02), .O(new_n55_));
  nand2  g34(.a(new_n44_), .b(x13), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(x04), .O(new_n58_));
  inv1   g37(.a(x05), .O(new_n59_));
  nor2   g38(.a(x07), .b(x04), .O(new_n60_));
  nor3   g39(.a(new_n60_), .b(new_n22_), .c(new_n59_), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n58_), .O(z3));
  inv1   g41(.a(x15), .O(new_n63_));
  nand4  g42(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n64_));
  nor2   g43(.a(new_n64_), .b(new_n63_), .O(z4));
endmodule


