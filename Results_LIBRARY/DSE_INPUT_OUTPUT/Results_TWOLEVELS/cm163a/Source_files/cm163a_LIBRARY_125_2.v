// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand2  g01(.a(new_n22_), .b(x06), .O(new_n23_));
  oai21  g02(.a(x04), .b(x00), .c(x05), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  and2   g04(.a(x03), .b(x02), .O(new_n26_));
  inv1   g05(.a(x06), .O(new_n27_));
  nand3  g06(.a(new_n26_), .b(new_n22_), .c(new_n27_), .O(new_n28_));
  oai21  g07(.a(new_n26_), .b(new_n22_), .c(new_n28_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x04), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n25_), .O(z0));
  inv1   g10(.a(x05), .O(new_n32_));
  nand2  g11(.a(new_n23_), .b(new_n32_), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  nand3  g13(.a(new_n26_), .b(new_n34_), .c(x04), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n27_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n22_), .O(new_n37_));
  inv1   g16(.a(x03), .O(new_n38_));
  nor2   g17(.a(x09), .b(new_n38_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(x02), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(x11), .c(x04), .O(new_n41_));
  or2    g20(.a(x04), .b(x01), .O(new_n42_));
  nand4  g21(.a(new_n42_), .b(new_n41_), .c(new_n37_), .d(new_n33_), .O(z1));
  inv1   g22(.a(x12), .O(new_n44_));
  oai21  g23(.a(new_n44_), .b(new_n34_), .c(x04), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n27_), .O(new_n46_));
  nor2   g25(.a(new_n26_), .b(new_n44_), .O(new_n47_));
  nand4  g26(.a(new_n44_), .b(new_n34_), .c(x03), .d(x02), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n27_), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n47_), .c(new_n22_), .O(new_n50_));
  nand3  g29(.a(x12), .b(x09), .c(x04), .O(new_n51_));
  nand4  g30(.a(new_n51_), .b(new_n50_), .c(new_n46_), .d(new_n33_), .O(z2));
  oai21  g31(.a(x07), .b(x04), .c(x05), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n23_), .O(new_n54_));
  nand2  g33(.a(new_n48_), .b(x13), .O(new_n55_));
  nor3   g34(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(new_n39_), .c(x02), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n55_), .c(x06), .O(new_n58_));
  and2   g37(.a(x13), .b(x09), .O(new_n59_));
  oai21  g38(.a(new_n59_), .b(new_n58_), .c(x04), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n54_), .O(z3));
  and2   g40(.a(new_n23_), .b(x15), .O(new_n62_));
  nand4  g41(.a(new_n62_), .b(x14), .c(x10), .d(x08), .O(new_n63_));
  nor2   g42(.a(new_n63_), .b(new_n38_), .O(z4));
endmodule


