// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_;
  nand2  g00(.a(x13), .b(x08), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  inv1   g03(.a(new_n24_), .O(new_n25_));
  nand3  g04(.a(new_n23_), .b(x03), .c(x02), .O(new_n26_));
  oai21  g05(.a(new_n25_), .b(new_n23_), .c(new_n26_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(new_n28_), .c(new_n22_), .d(x05), .O(z0));
  inv1   g11(.a(x02), .O(new_n33_));
  nand3  g12(.a(new_n26_), .b(x11), .c(x04), .O(new_n34_));
  inv1   g13(.a(new_n34_), .O(new_n35_));
  oai21  g14(.a(x04), .b(x01), .c(x05), .O(new_n36_));
  oai21  g15(.a(new_n36_), .b(new_n35_), .c(new_n22_), .O(new_n37_));
  inv1   g16(.a(x08), .O(new_n38_));
  inv1   g17(.a(x12), .O(new_n39_));
  oai21  g18(.a(x13), .b(new_n39_), .c(new_n38_), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(x13), .c(x11), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(new_n23_), .c(x04), .d(x03), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(new_n33_), .c(new_n37_), .O(z1));
  inv1   g22(.a(new_n22_), .O(new_n44_));
  nor2   g23(.a(x11), .b(x09), .O(new_n45_));
  aoi21  g24(.a(new_n45_), .b(new_n25_), .c(new_n39_), .O(new_n46_));
  nor4   g25(.a(new_n24_), .b(x12), .c(x11), .d(x09), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  inv1   g27(.a(x05), .O(new_n49_));
  inv1   g28(.a(x06), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n30_), .c(new_n49_), .O(new_n51_));
  aoi21  g30(.a(new_n51_), .b(new_n48_), .c(new_n44_), .O(z2));
  nand3  g31(.a(x13), .b(x12), .c(new_n38_), .O(new_n53_));
  inv1   g32(.a(x13), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n39_), .O(new_n55_));
  aoi21  g34(.a(new_n55_), .b(new_n53_), .c(x11), .O(new_n56_));
  nand4  g35(.a(new_n56_), .b(new_n23_), .c(x03), .d(x02), .O(new_n57_));
  nand2  g36(.a(new_n45_), .b(new_n25_), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(x13), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(x04), .O(new_n61_));
  inv1   g40(.a(x07), .O(new_n62_));
  nand2  g41(.a(new_n62_), .b(new_n30_), .O(new_n63_));
  nand3  g42(.a(new_n63_), .b(new_n22_), .c(x05), .O(new_n64_));
  inv1   g43(.a(new_n64_), .O(new_n65_));
  nand2  g44(.a(new_n65_), .b(new_n61_), .O(z3));
  nand3  g45(.a(x10), .b(x08), .c(x03), .O(new_n67_));
  inv1   g46(.a(new_n67_), .O(new_n68_));
  nand4  g47(.a(new_n68_), .b(x15), .c(x14), .d(new_n54_), .O(new_n69_));
  inv1   g48(.a(new_n69_), .O(z4));
endmodule


