// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nand3  g02(.a(new_n22_), .b(x03), .c(x02), .O(new_n24_));
  oai21  g03(.a(new_n23_), .b(new_n22_), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g05(.a(x00), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  inv1   g07(.a(x05), .O(new_n29_));
  aoi21  g08(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n26_), .O(z0));
  nor2   g10(.a(x11), .b(x09), .O(new_n32_));
  aoi22  g11(.a(new_n32_), .b(new_n23_), .c(new_n24_), .d(x11), .O(new_n33_));
  inv1   g12(.a(x01), .O(new_n34_));
  aoi21  g13(.a(new_n28_), .b(new_n34_), .c(new_n29_), .O(new_n35_));
  oai21  g14(.a(new_n33_), .b(new_n28_), .c(new_n35_), .O(z1));
  inv1   g15(.a(x03), .O(new_n37_));
  inv1   g16(.a(x12), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n28_), .c(x05), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  inv1   g19(.a(x11), .O(new_n41_));
  nand3  g20(.a(new_n41_), .b(new_n22_), .c(x02), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(x12), .O(new_n43_));
  nor2   g22(.a(x12), .b(x11), .O(new_n44_));
  nand3  g23(.a(new_n44_), .b(new_n23_), .c(new_n22_), .O(new_n45_));
  aoi21  g24(.a(new_n45_), .b(new_n43_), .c(new_n28_), .O(new_n46_));
  nor2   g25(.a(x06), .b(x04), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n46_), .c(x05), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n40_), .O(z2));
  inv1   g28(.a(x13), .O(new_n50_));
  nand2  g29(.a(x03), .b(x02), .O(new_n51_));
  nand3  g30(.a(new_n50_), .b(new_n38_), .c(new_n41_), .O(new_n52_));
  oai22  g31(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n38_), .O(new_n53_));
  aoi21  g32(.a(new_n32_), .b(new_n23_), .c(new_n50_), .O(new_n54_));
  aoi21  g33(.a(new_n53_), .b(new_n22_), .c(new_n54_), .O(new_n55_));
  inv1   g34(.a(x07), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n28_), .c(new_n29_), .O(new_n57_));
  oai21  g36(.a(new_n55_), .b(new_n28_), .c(new_n57_), .O(z3));
  inv1   g37(.a(x14), .O(new_n59_));
  inv1   g38(.a(x15), .O(new_n60_));
  nand4  g39(.a(x10), .b(x08), .c(x05), .d(x03), .O(new_n61_));
  nor3   g40(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(z4));
endmodule


