// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_;
  inv1   g00(.a(x03), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  oai21  g03(.a(x04), .b(x00), .c(x05), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  aoi21  g05(.a(x03), .b(x02), .c(new_n23_), .O(new_n27_));
  nand3  g06(.a(new_n23_), .b(x03), .c(x02), .O(new_n28_));
  inv1   g07(.a(new_n28_), .O(new_n29_));
  oai21  g08(.a(new_n29_), .b(new_n27_), .c(x04), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n26_), .O(z0));
  inv1   g10(.a(x05), .O(new_n32_));
  nand2  g11(.a(new_n24_), .b(new_n32_), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x03), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n23_), .O(new_n37_));
  aoi21  g16(.a(new_n23_), .b(x02), .c(new_n34_), .O(new_n38_));
  nor2   g17(.a(x04), .b(x01), .O(new_n39_));
  aoi21  g18(.a(new_n38_), .b(x04), .c(new_n39_), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n37_), .c(new_n33_), .O(z1));
  nand2  g20(.a(x04), .b(x02), .O(new_n42_));
  inv1   g21(.a(x12), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n34_), .O(new_n44_));
  oai21  g23(.a(new_n44_), .b(new_n42_), .c(x03), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  nor2   g25(.a(x11), .b(x09), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(x02), .c(new_n43_), .O(new_n48_));
  nor2   g27(.a(x06), .b(x04), .O(new_n49_));
  aoi21  g28(.a(new_n48_), .b(x04), .c(new_n49_), .O(new_n50_));
  nand3  g29(.a(new_n50_), .b(new_n46_), .c(new_n33_), .O(z2));
  inv1   g30(.a(x13), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n43_), .c(new_n34_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n42_), .c(x03), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  nand4  g34(.a(new_n43_), .b(new_n34_), .c(new_n23_), .d(x02), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(x13), .c(x04), .O(new_n57_));
  or2    g36(.a(x07), .b(x04), .O(new_n58_));
  nand4  g37(.a(new_n58_), .b(new_n57_), .c(new_n55_), .d(new_n33_), .O(z3));
  nand2  g38(.a(x08), .b(x03), .O(new_n60_));
  nand3  g39(.a(x15), .b(x14), .c(x10), .O(new_n61_));
  oai21  g40(.a(new_n61_), .b(new_n60_), .c(new_n24_), .O(z4));
endmodule


