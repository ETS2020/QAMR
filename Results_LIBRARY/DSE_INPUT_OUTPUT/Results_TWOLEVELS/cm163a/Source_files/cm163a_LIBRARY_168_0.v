// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_;
  nor2   g00(.a(x14), .b(x13), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n24_), .b(x09), .c(x04), .O(new_n25_));
  or2    g04(.a(x04), .b(x00), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(new_n25_), .c(x05), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  nor2   g07(.a(new_n22_), .b(x09), .O(new_n29_));
  nand4  g08(.a(new_n29_), .b(x04), .c(x03), .d(x02), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n28_), .O(z0));
  inv1   g10(.a(x09), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(x03), .c(x02), .O(new_n33_));
  nand3  g12(.a(new_n33_), .b(x11), .c(x04), .O(new_n34_));
  inv1   g13(.a(new_n34_), .O(new_n35_));
  oai21  g14(.a(x04), .b(x01), .c(x05), .O(new_n36_));
  oai21  g15(.a(new_n36_), .b(new_n35_), .c(new_n23_), .O(new_n37_));
  nor3   g16(.a(new_n22_), .b(x11), .c(x09), .O(new_n38_));
  nand4  g17(.a(new_n38_), .b(x04), .c(x03), .d(x02), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n37_), .O(z1));
  inv1   g19(.a(x02), .O(new_n41_));
  inv1   g20(.a(x11), .O(new_n42_));
  inv1   g21(.a(x13), .O(new_n43_));
  inv1   g22(.a(x14), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n43_), .c(x12), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n42_), .c(new_n32_), .d(x03), .O(new_n46_));
  nand4  g25(.a(new_n42_), .b(new_n32_), .c(x03), .d(x02), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n23_), .c(x12), .O(new_n48_));
  oai21  g27(.a(new_n46_), .b(new_n41_), .c(new_n48_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(x04), .O(new_n50_));
  oai21  g29(.a(x06), .b(x04), .c(x05), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n23_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n50_), .O(z2));
  oai21  g32(.a(x07), .b(x04), .c(x05), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  nand2  g34(.a(x13), .b(x12), .O(new_n56_));
  inv1   g35(.a(x12), .O(new_n57_));
  nand3  g36(.a(x14), .b(new_n43_), .c(new_n57_), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n42_), .O(new_n60_));
  nand2  g39(.a(x13), .b(x11), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g41(.a(new_n62_), .b(new_n32_), .c(x03), .d(x02), .O(new_n63_));
  nand2  g42(.a(new_n33_), .b(x13), .O(new_n64_));
  nand2  g43(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g44(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g45(.a(new_n66_), .b(new_n55_), .O(z3));
  nand2  g46(.a(x08), .b(x03), .O(new_n68_));
  nand3  g47(.a(x15), .b(x14), .c(x10), .O(new_n69_));
  oai21  g48(.a(new_n69_), .b(new_n68_), .c(new_n23_), .O(z4));
endmodule


