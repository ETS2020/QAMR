// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nand3  g02(.a(new_n22_), .b(x03), .c(x02), .O(new_n24_));
  oai21  g03(.a(new_n23_), .b(new_n22_), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g05(.a(x00), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  inv1   g07(.a(x05), .O(new_n29_));
  aoi21  g08(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n26_), .O(z0));
  inv1   g10(.a(x12), .O(new_n32_));
  aoi21  g11(.a(new_n32_), .b(new_n29_), .c(x11), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n22_), .c(x03), .d(x02), .O(new_n34_));
  nand3  g13(.a(new_n24_), .b(x11), .c(x05), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x04), .O(new_n37_));
  inv1   g16(.a(x01), .O(new_n38_));
  nor2   g17(.a(new_n29_), .b(x04), .O(new_n39_));
  aoi22  g18(.a(new_n39_), .b(new_n38_), .c(x12), .d(new_n29_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n37_), .O(z1));
  nor2   g20(.a(x06), .b(x04), .O(new_n42_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n43_));
  inv1   g22(.a(new_n43_), .O(new_n44_));
  nor3   g23(.a(x12), .b(x11), .c(x09), .O(new_n45_));
  aoi21  g24(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  nor2   g25(.a(x11), .b(x09), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n23_), .O(new_n48_));
  aoi21  g27(.a(new_n48_), .b(x04), .c(new_n29_), .O(new_n49_));
  oai22  g28(.a(new_n49_), .b(new_n32_), .c(new_n46_), .d(new_n29_), .O(z2));
  inv1   g29(.a(new_n24_), .O(new_n51_));
  nor2   g30(.a(x12), .b(x11), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n23_), .c(new_n22_), .O(new_n53_));
  nor3   g32(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  aoi22  g33(.a(new_n54_), .b(new_n51_), .c(new_n53_), .d(x13), .O(new_n55_));
  inv1   g34(.a(x07), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n28_), .c(new_n29_), .O(new_n57_));
  oai21  g36(.a(new_n55_), .b(new_n28_), .c(new_n57_), .O(z3));
  inv1   g37(.a(x03), .O(new_n59_));
  inv1   g38(.a(x15), .O(new_n60_));
  aoi21  g39(.a(new_n32_), .b(new_n29_), .c(new_n60_), .O(new_n61_));
  nand4  g40(.a(new_n61_), .b(x14), .c(x10), .d(x08), .O(new_n62_));
  nor2   g41(.a(new_n62_), .b(new_n59_), .O(z4));
endmodule


