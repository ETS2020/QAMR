// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_;
  inv1   g00(.a(x09), .O(new_n22_));
  inv1   g01(.a(x12), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  oai21  g03(.a(x04), .b(x00), .c(x05), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand4  g05(.a(x12), .b(new_n22_), .c(x03), .d(x02), .O(new_n27_));
  nand2  g06(.a(x03), .b(x02), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x09), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n26_), .O(z0));
  inv1   g11(.a(x05), .O(new_n33_));
  nand2  g12(.a(new_n24_), .b(new_n33_), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand3  g14(.a(x12), .b(new_n35_), .c(x04), .O(new_n36_));
  oai21  g15(.a(new_n36_), .b(new_n28_), .c(x12), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n22_), .O(new_n38_));
  nand3  g17(.a(new_n22_), .b(x03), .c(x02), .O(new_n39_));
  nand3  g18(.a(new_n39_), .b(x11), .c(x04), .O(new_n40_));
  inv1   g19(.a(x01), .O(new_n41_));
  inv1   g20(.a(x04), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand4  g22(.a(new_n43_), .b(new_n40_), .c(new_n38_), .d(new_n34_), .O(z1));
  nand3  g23(.a(x12), .b(x11), .c(x04), .O(new_n45_));
  oai21  g24(.a(new_n45_), .b(new_n28_), .c(x12), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n22_), .O(new_n47_));
  nand3  g26(.a(new_n39_), .b(x12), .c(x04), .O(new_n48_));
  inv1   g27(.a(x06), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nand4  g29(.a(new_n50_), .b(new_n48_), .c(new_n47_), .d(new_n34_), .O(z2));
  nor2   g30(.a(x07), .b(x04), .O(new_n52_));
  aoi21  g31(.a(x13), .b(x04), .c(new_n52_), .O(new_n53_));
  aoi22  g32(.a(new_n53_), .b(x05), .c(new_n23_), .d(new_n22_), .O(z3));
  nand2  g33(.a(x08), .b(x03), .O(new_n55_));
  nand3  g34(.a(x15), .b(x14), .c(x10), .O(new_n56_));
  oai21  g35(.a(new_n56_), .b(new_n55_), .c(new_n24_), .O(z4));
endmodule


