// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nand3  g02(.a(new_n23_), .b(new_n22_), .c(x04), .O(new_n24_));
  oai21  g03(.a(x04), .b(x00), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x05), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  oai21  g06(.a(new_n23_), .b(new_n27_), .c(x05), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x09), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n26_), .O(z0));
  inv1   g09(.a(x11), .O(new_n31_));
  nand3  g10(.a(new_n22_), .b(x03), .c(x02), .O(new_n32_));
  inv1   g11(.a(new_n32_), .O(new_n33_));
  nand3  g12(.a(new_n23_), .b(new_n31_), .c(new_n22_), .O(new_n34_));
  oai21  g13(.a(new_n33_), .b(new_n31_), .c(new_n34_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x04), .O(new_n36_));
  inv1   g15(.a(x01), .O(new_n37_));
  nand2  g16(.a(new_n27_), .b(new_n37_), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(new_n36_), .c(x05), .O(z1));
  nand2  g18(.a(x12), .b(x04), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(x05), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(x09), .O(new_n42_));
  nand3  g21(.a(new_n31_), .b(x03), .c(x02), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x12), .O(new_n44_));
  nor2   g23(.a(x12), .b(x11), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n23_), .c(new_n22_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(new_n27_), .O(new_n47_));
  nor2   g26(.a(x06), .b(x04), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n47_), .c(x05), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n42_), .O(z2));
  oai21  g29(.a(x12), .b(x11), .c(x05), .O(new_n51_));
  nand4  g30(.a(new_n51_), .b(new_n22_), .c(x03), .d(x02), .O(new_n52_));
  nor3   g31(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  aoi22  g32(.a(new_n53_), .b(new_n33_), .c(new_n52_), .d(x13), .O(new_n54_));
  inv1   g33(.a(x05), .O(new_n55_));
  inv1   g34(.a(x07), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n27_), .c(new_n55_), .O(new_n57_));
  oai21  g36(.a(new_n54_), .b(new_n27_), .c(new_n57_), .O(z3));
  nand2  g37(.a(x08), .b(x03), .O(new_n59_));
  nand3  g38(.a(x15), .b(x14), .c(x10), .O(new_n60_));
  oai22  g39(.a(new_n60_), .b(new_n59_), .c(x09), .d(x05), .O(z4));
endmodule


