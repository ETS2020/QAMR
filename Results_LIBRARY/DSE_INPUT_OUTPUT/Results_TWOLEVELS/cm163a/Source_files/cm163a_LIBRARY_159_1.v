// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x07), .O(new_n22_));
  nand2  g01(.a(x12), .b(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n24_), .b(x09), .c(x04), .O(new_n25_));
  inv1   g04(.a(x00), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(new_n25_), .c(x05), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  inv1   g09(.a(x11), .O(new_n31_));
  oai21  g10(.a(x12), .b(new_n31_), .c(x07), .O(new_n32_));
  aoi21  g11(.a(new_n32_), .b(x12), .c(x09), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(x04), .c(x03), .d(x02), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n30_), .O(z0));
  inv1   g14(.a(new_n23_), .O(new_n36_));
  inv1   g15(.a(x09), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(x03), .c(x02), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x11), .O(new_n39_));
  inv1   g18(.a(new_n24_), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n31_), .c(new_n37_), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(x04), .O(new_n43_));
  inv1   g22(.a(x01), .O(new_n44_));
  inv1   g23(.a(x05), .O(new_n45_));
  aoi21  g24(.a(new_n27_), .b(new_n44_), .c(new_n45_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n43_), .c(new_n36_), .O(z1));
  nand3  g26(.a(x12), .b(x11), .c(x07), .O(new_n48_));
  oai21  g27(.a(x12), .b(x11), .c(new_n48_), .O(new_n49_));
  nand4  g28(.a(new_n49_), .b(new_n37_), .c(x03), .d(x02), .O(new_n50_));
  nand3  g29(.a(new_n38_), .b(x12), .c(x07), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(x04), .O(new_n53_));
  oai21  g32(.a(x06), .b(x04), .c(x05), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n53_), .O(z2));
  nand2  g35(.a(new_n23_), .b(new_n45_), .O(new_n57_));
  oai21  g36(.a(x12), .b(new_n27_), .c(new_n22_), .O(new_n58_));
  nand2  g37(.a(x13), .b(x11), .O(new_n59_));
  nor2   g38(.a(x13), .b(x11), .O(new_n60_));
  nand4  g39(.a(new_n60_), .b(new_n37_), .c(x03), .d(x02), .O(new_n61_));
  aoi21  g40(.a(new_n61_), .b(new_n59_), .c(x12), .O(new_n62_));
  inv1   g41(.a(x13), .O(new_n63_));
  aoi21  g42(.a(x12), .b(x07), .c(x09), .O(new_n64_));
  aoi21  g43(.a(new_n64_), .b(new_n40_), .c(new_n63_), .O(new_n65_));
  oai21  g44(.a(new_n65_), .b(new_n62_), .c(x04), .O(new_n66_));
  nand3  g45(.a(new_n66_), .b(new_n58_), .c(new_n57_), .O(z3));
  inv1   g46(.a(x03), .O(new_n68_));
  and2   g47(.a(new_n23_), .b(x15), .O(new_n69_));
  nand4  g48(.a(new_n69_), .b(x14), .c(x10), .d(x08), .O(new_n70_));
  nor2   g49(.a(new_n70_), .b(new_n68_), .O(z4));
endmodule


