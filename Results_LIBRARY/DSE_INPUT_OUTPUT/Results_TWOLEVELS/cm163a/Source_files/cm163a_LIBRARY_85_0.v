// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(x03), .c(x02), .O(new_n26_));
  aoi21  g05(.a(new_n26_), .b(new_n24_), .c(new_n22_), .O(new_n27_));
  nor2   g06(.a(x04), .b(x00), .O(new_n28_));
  oai21  g07(.a(new_n28_), .b(new_n27_), .c(x05), .O(new_n29_));
  inv1   g08(.a(x05), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x01), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n29_), .O(z0));
  inv1   g11(.a(x01), .O(new_n33_));
  oai21  g12(.a(new_n30_), .b(new_n22_), .c(new_n33_), .O(new_n34_));
  nand2  g13(.a(new_n26_), .b(x11), .O(new_n35_));
  inv1   g14(.a(new_n23_), .O(new_n36_));
  nor2   g15(.a(x11), .b(x09), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n35_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(x04), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n34_), .c(new_n31_), .O(z1));
  inv1   g20(.a(x12), .O(new_n42_));
  aoi21  g21(.a(new_n37_), .b(new_n36_), .c(new_n42_), .O(new_n43_));
  inv1   g22(.a(x11), .O(new_n44_));
  nand3  g23(.a(new_n42_), .b(new_n44_), .c(new_n25_), .O(new_n45_));
  nor2   g24(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n43_), .c(x04), .O(new_n47_));
  inv1   g26(.a(x06), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n22_), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(new_n47_), .c(x05), .O(z2));
  inv1   g29(.a(x13), .O(new_n51_));
  nor3   g30(.a(x12), .b(x11), .c(x09), .O(new_n52_));
  aoi21  g31(.a(new_n52_), .b(new_n36_), .c(new_n51_), .O(new_n53_));
  nand3  g32(.a(new_n51_), .b(new_n42_), .c(new_n44_), .O(new_n54_));
  nor2   g33(.a(new_n54_), .b(new_n26_), .O(new_n55_));
  oai21  g34(.a(new_n55_), .b(new_n53_), .c(x04), .O(new_n56_));
  inv1   g35(.a(x07), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n22_), .O(new_n58_));
  nand3  g37(.a(new_n58_), .b(new_n56_), .c(x05), .O(z3));
  nand2  g38(.a(x08), .b(x03), .O(new_n60_));
  nand3  g39(.a(x15), .b(x14), .c(x10), .O(new_n61_));
  oai22  g40(.a(new_n61_), .b(new_n60_), .c(x05), .d(x01), .O(z4));
endmodule


