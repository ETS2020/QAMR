// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x05), .b(x00), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x02), .O(new_n25_));
  nand2  g04(.a(x09), .b(new_n25_), .O(new_n26_));
  inv1   g05(.a(x09), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x02), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x04), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x03), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(new_n30_), .c(new_n24_), .O(z0));
  nand2  g12(.a(x05), .b(x01), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n22_), .O(new_n35_));
  nand2  g14(.a(new_n28_), .b(x11), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n27_), .c(x02), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(new_n36_), .c(new_n22_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n31_), .c(x03), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n35_), .O(z1));
  nand2  g20(.a(x06), .b(x05), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n22_), .O(new_n43_));
  nand2  g22(.a(new_n38_), .b(x12), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n37_), .c(new_n27_), .d(x02), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(new_n22_), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n31_), .c(x03), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n43_), .O(z2));
  nand2  g28(.a(x07), .b(x05), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  nand2  g30(.a(x13), .b(x12), .O(new_n52_));
  inv1   g31(.a(x13), .O(new_n53_));
  nand4  g32(.a(new_n53_), .b(new_n45_), .c(new_n37_), .d(x02), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n27_), .O(new_n56_));
  nand2  g35(.a(new_n38_), .b(x13), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n56_), .c(new_n22_), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n31_), .c(x03), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n51_), .O(z3));
  inv1   g39(.a(x15), .O(new_n61_));
  nand4  g40(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n62_));
  nor2   g41(.a(new_n62_), .b(new_n61_), .O(z4));
endmodule


