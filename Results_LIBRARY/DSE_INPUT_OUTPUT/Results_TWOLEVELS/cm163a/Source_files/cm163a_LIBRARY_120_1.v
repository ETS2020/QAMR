// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_;
  inv1   g00(.a(x13), .O(new_n22_));
  nand2  g01(.a(x15), .b(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n24_), .b(x09), .c(x04), .O(new_n25_));
  inv1   g04(.a(x00), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(new_n25_), .c(x05), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  aoi21  g09(.a(x15), .b(new_n22_), .c(x09), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n30_), .O(z0));
  inv1   g12(.a(x09), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(x03), .c(x02), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(x11), .c(x04), .O(new_n36_));
  inv1   g15(.a(new_n36_), .O(new_n37_));
  oai21  g16(.a(x04), .b(x01), .c(x05), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n37_), .c(new_n23_), .O(new_n39_));
  inv1   g18(.a(x11), .O(new_n40_));
  nand4  g19(.a(new_n23_), .b(new_n40_), .c(new_n34_), .d(x04), .O(new_n41_));
  inv1   g20(.a(new_n41_), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(x03), .c(x02), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n39_), .O(z1));
  inv1   g23(.a(x02), .O(new_n45_));
  aoi21  g24(.a(x15), .b(new_n22_), .c(x12), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(new_n40_), .c(new_n34_), .d(x03), .O(new_n47_));
  nand2  g26(.a(new_n40_), .b(new_n34_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n24_), .c(x12), .O(new_n49_));
  oai21  g28(.a(new_n47_), .b(new_n45_), .c(new_n49_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(x04), .O(new_n51_));
  inv1   g30(.a(x06), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n27_), .O(new_n53_));
  nand4  g32(.a(new_n53_), .b(new_n51_), .c(new_n23_), .d(x05), .O(z2));
  oai21  g33(.a(x07), .b(x04), .c(x05), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n23_), .O(new_n56_));
  and2   g35(.a(x13), .b(x12), .O(new_n57_));
  nor3   g36(.a(x15), .b(x13), .c(x12), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n57_), .c(new_n40_), .O(new_n59_));
  nand2  g38(.a(x13), .b(x11), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand4  g40(.a(new_n61_), .b(new_n34_), .c(x03), .d(x02), .O(new_n62_));
  nand2  g41(.a(new_n35_), .b(x13), .O(new_n63_));
  nand2  g42(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g43(.a(new_n64_), .b(x04), .O(new_n65_));
  nand2  g44(.a(new_n65_), .b(new_n56_), .O(z3));
  inv1   g45(.a(x15), .O(new_n67_));
  nand4  g46(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n68_));
  aoi21  g47(.a(new_n68_), .b(x13), .c(new_n67_), .O(z4));
endmodule


