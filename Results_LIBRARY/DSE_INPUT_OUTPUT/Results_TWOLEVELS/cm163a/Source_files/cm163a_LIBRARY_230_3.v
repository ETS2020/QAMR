// Benchmark "FAU" written by ABC on Wed Aug 19 17:19:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_;
  nand2  g00(.a(x13), .b(x09), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  and2   g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n23_), .b(x03), .c(x02), .O(new_n25_));
  oai21  g04(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x04), .O(new_n27_));
  or2    g06(.a(x04), .b(x00), .O(new_n28_));
  nand4  g07(.a(new_n28_), .b(new_n27_), .c(new_n22_), .d(x05), .O(z0));
  oai21  g08(.a(x04), .b(x01), .c(x05), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n22_), .O(new_n31_));
  nand2  g10(.a(x03), .b(x02), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(x11), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(x03), .c(x02), .O(new_n35_));
  aoi21  g14(.a(new_n35_), .b(new_n33_), .c(x09), .O(new_n36_));
  inv1   g15(.a(x13), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(x11), .c(x09), .O(new_n38_));
  inv1   g17(.a(new_n38_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n36_), .c(x04), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n31_), .O(z1));
  oai21  g20(.a(x06), .b(x04), .c(x05), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n22_), .O(new_n43_));
  nand2  g22(.a(new_n35_), .b(x12), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand3  g24(.a(new_n24_), .b(new_n45_), .c(new_n34_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(x09), .O(new_n47_));
  nand3  g26(.a(new_n37_), .b(x12), .c(x09), .O(new_n48_));
  inv1   g27(.a(new_n48_), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n43_), .O(z2));
  oai21  g30(.a(x07), .b(x04), .c(x05), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n22_), .O(new_n53_));
  nand2  g32(.a(new_n46_), .b(x13), .O(new_n54_));
  nand4  g33(.a(new_n24_), .b(new_n37_), .c(new_n45_), .d(new_n34_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(new_n23_), .c(x04), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n53_), .O(z3));
  inv1   g37(.a(x03), .O(new_n59_));
  inv1   g38(.a(x08), .O(new_n60_));
  nand4  g39(.a(new_n22_), .b(x15), .c(x14), .d(x10), .O(new_n61_));
  nor3   g40(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(z4));
endmodule


