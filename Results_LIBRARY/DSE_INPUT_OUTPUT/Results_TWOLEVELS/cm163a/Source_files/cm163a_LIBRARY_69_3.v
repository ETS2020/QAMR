// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x01), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  inv1   g03(.a(new_n24_), .O(new_n25_));
  inv1   g04(.a(x03), .O(new_n26_));
  nor2   g05(.a(x09), .b(new_n26_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x02), .O(new_n28_));
  oai21  g07(.a(new_n25_), .b(new_n23_), .c(new_n28_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x04), .O(new_n30_));
  inv1   g09(.a(x00), .O(new_n31_));
  inv1   g10(.a(x04), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n30_), .c(x05), .d(new_n22_), .O(z0));
  inv1   g13(.a(x11), .O(new_n35_));
  nand3  g14(.a(new_n25_), .b(new_n35_), .c(new_n23_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x04), .O(new_n37_));
  aoi21  g16(.a(new_n28_), .b(x11), .c(new_n37_), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(x01), .c(x05), .O(z1));
  inv1   g18(.a(x12), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n35_), .c(new_n23_), .O(new_n41_));
  nor2   g20(.a(new_n41_), .b(new_n24_), .O(new_n42_));
  aoi21  g21(.a(new_n36_), .b(x12), .c(new_n42_), .O(new_n43_));
  oai21  g22(.a(x06), .b(x04), .c(x05), .O(new_n44_));
  nor2   g23(.a(new_n44_), .b(x01), .O(new_n45_));
  oai21  g24(.a(new_n43_), .b(new_n32_), .c(new_n45_), .O(z2));
  oai21  g25(.a(new_n41_), .b(new_n24_), .c(x13), .O(new_n47_));
  nor3   g26(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n27_), .c(x02), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n47_), .c(new_n32_), .O(new_n50_));
  nor2   g29(.a(x07), .b(x04), .O(new_n51_));
  oai21  g30(.a(new_n51_), .b(new_n50_), .c(new_n22_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(x05), .O(z3));
  inv1   g32(.a(x08), .O(new_n54_));
  nand2  g33(.a(x05), .b(x01), .O(new_n55_));
  nand4  g34(.a(new_n55_), .b(x15), .c(x14), .d(x10), .O(new_n56_));
  nor3   g35(.a(new_n56_), .b(new_n54_), .c(new_n26_), .O(z4));
endmodule


