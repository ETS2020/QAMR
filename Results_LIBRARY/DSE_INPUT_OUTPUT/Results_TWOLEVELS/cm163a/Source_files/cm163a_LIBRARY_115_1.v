// Benchmark "FAU" written by ABC on Wed Aug 19 17:18:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x15), .O(new_n22_));
  nor2   g01(.a(new_n22_), .b(x13), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  and2   g04(.a(x03), .b(x02), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(new_n28_), .O(new_n29_));
  oai21  g08(.a(x04), .b(x00), .c(x05), .O(new_n30_));
  oai21  g09(.a(new_n30_), .b(new_n29_), .c(new_n24_), .O(new_n31_));
  nor2   g10(.a(new_n23_), .b(x09), .O(new_n32_));
  nand4  g11(.a(new_n32_), .b(x04), .c(x03), .d(x02), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n31_), .O(z0));
  nand3  g13(.a(new_n25_), .b(x03), .c(x02), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(x11), .c(x04), .O(new_n36_));
  inv1   g15(.a(new_n36_), .O(new_n37_));
  oai21  g16(.a(x04), .b(x01), .c(x05), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n37_), .c(new_n24_), .O(new_n39_));
  nor3   g18(.a(new_n23_), .b(x11), .c(x09), .O(new_n40_));
  nand4  g19(.a(new_n40_), .b(x04), .c(x03), .d(x02), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n39_), .O(z1));
  inv1   g21(.a(x02), .O(new_n43_));
  inv1   g22(.a(x11), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n44_), .c(new_n25_), .d(x03), .O(new_n46_));
  nor2   g25(.a(new_n46_), .b(new_n43_), .O(new_n47_));
  nor2   g26(.a(x11), .b(x09), .O(new_n48_));
  aoi21  g27(.a(new_n48_), .b(new_n26_), .c(new_n45_), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n50_));
  or2    g29(.a(x06), .b(x04), .O(new_n51_));
  nand4  g30(.a(new_n51_), .b(new_n50_), .c(new_n24_), .d(x05), .O(z2));
  inv1   g31(.a(x05), .O(new_n53_));
  nand2  g32(.a(new_n24_), .b(new_n53_), .O(new_n54_));
  xor2a  g33(.a(x13), .b(x12), .O(new_n55_));
  nand2  g34(.a(x13), .b(x11), .O(new_n56_));
  oai21  g35(.a(new_n55_), .b(x11), .c(new_n56_), .O(new_n57_));
  nand4  g36(.a(new_n57_), .b(new_n25_), .c(x03), .d(x02), .O(new_n58_));
  nand2  g37(.a(new_n35_), .b(x13), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(x04), .O(new_n61_));
  nor2   g40(.a(x07), .b(x04), .O(new_n62_));
  nor2   g41(.a(new_n62_), .b(new_n23_), .O(new_n63_));
  nand3  g42(.a(new_n63_), .b(new_n61_), .c(new_n54_), .O(z3));
  nand4  g43(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n65_));
  aoi21  g44(.a(new_n65_), .b(x13), .c(new_n22_), .O(z4));
endmodule


