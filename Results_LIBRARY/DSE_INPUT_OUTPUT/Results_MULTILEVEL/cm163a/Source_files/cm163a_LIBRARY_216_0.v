// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x02), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  oai21  g02(.a(new_n23_), .b(x04), .c(new_n22_), .O(new_n24_));
  nor2   g03(.a(new_n23_), .b(x03), .O(new_n25_));
  inv1   g04(.a(x03), .O(new_n26_));
  nor2   g05(.a(x09), .b(new_n26_), .O(new_n27_));
  oai21  g06(.a(new_n27_), .b(new_n25_), .c(x04), .O(new_n28_));
  or2    g07(.a(x04), .b(x00), .O(new_n29_));
  nand4  g08(.a(new_n29_), .b(new_n28_), .c(new_n24_), .d(x05), .O(z0));
  nor2   g09(.a(x09), .b(x02), .O(new_n31_));
  inv1   g10(.a(new_n31_), .O(new_n32_));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand2  g13(.a(x11), .b(new_n26_), .O(new_n35_));
  inv1   g14(.a(x11), .O(new_n36_));
  nand3  g15(.a(new_n36_), .b(new_n23_), .c(x03), .O(new_n37_));
  aoi21  g16(.a(new_n37_), .b(new_n35_), .c(new_n22_), .O(new_n38_));
  nor2   g17(.a(new_n36_), .b(new_n23_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n38_), .c(x04), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n34_), .O(z1));
  oai21  g20(.a(x06), .b(x04), .c(x05), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n32_), .O(new_n43_));
  oai21  g22(.a(x11), .b(new_n26_), .c(x12), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n36_), .c(new_n23_), .d(x03), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(new_n22_), .O(new_n47_));
  nor2   g26(.a(new_n45_), .b(new_n23_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n43_), .O(z2));
  oai21  g29(.a(x07), .b(x04), .c(x05), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n32_), .O(new_n52_));
  nand3  g31(.a(new_n45_), .b(new_n36_), .c(x03), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(x13), .O(new_n54_));
  nor2   g33(.a(x13), .b(x12), .O(new_n55_));
  nand4  g34(.a(new_n55_), .b(new_n36_), .c(new_n23_), .d(x03), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n54_), .c(new_n22_), .O(new_n57_));
  and2   g36(.a(x13), .b(x09), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n57_), .c(x04), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n52_), .O(z3));
  inv1   g39(.a(x15), .O(new_n61_));
  nor2   g40(.a(new_n31_), .b(new_n61_), .O(new_n62_));
  nand4  g41(.a(new_n62_), .b(x14), .c(x10), .d(x08), .O(new_n63_));
  nor2   g42(.a(new_n63_), .b(new_n26_), .O(z4));
endmodule


