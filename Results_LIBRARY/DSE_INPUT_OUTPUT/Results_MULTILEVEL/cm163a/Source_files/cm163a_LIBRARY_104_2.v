// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_;
  inv1   g00(.a(x03), .O(new_n22_));
  nand2  g01(.a(new_n22_), .b(x02), .O(new_n23_));
  oai21  g02(.a(x04), .b(x00), .c(x05), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g04(.a(x09), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(x03), .c(x02), .O(new_n27_));
  oai21  g06(.a(new_n26_), .b(x02), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n25_), .O(z0));
  oai21  g09(.a(x04), .b(x01), .c(x05), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n23_), .O(new_n32_));
  inv1   g11(.a(x11), .O(new_n33_));
  inv1   g12(.a(x02), .O(new_n34_));
  aoi21  g13(.a(x09), .b(x03), .c(new_n34_), .O(new_n35_));
  nand4  g14(.a(new_n33_), .b(new_n26_), .c(x03), .d(x02), .O(new_n36_));
  oai21  g15(.a(new_n35_), .b(new_n33_), .c(new_n36_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x04), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n32_), .O(z1));
  oai21  g18(.a(x06), .b(x04), .c(x05), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n23_), .O(new_n41_));
  oai21  g20(.a(x11), .b(x09), .c(x12), .O(new_n42_));
  inv1   g21(.a(x12), .O(new_n43_));
  nand4  g22(.a(new_n43_), .b(new_n33_), .c(new_n26_), .d(x02), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n42_), .c(new_n22_), .O(new_n45_));
  nor2   g24(.a(new_n43_), .b(x02), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n45_), .c(x04), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n41_), .O(z2));
  oai21  g27(.a(x07), .b(x04), .c(x05), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n23_), .O(new_n50_));
  nand3  g29(.a(new_n43_), .b(new_n33_), .c(new_n26_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x13), .O(new_n52_));
  nor2   g31(.a(x13), .b(x12), .O(new_n53_));
  nand4  g32(.a(new_n53_), .b(new_n33_), .c(new_n26_), .d(x02), .O(new_n54_));
  aoi21  g33(.a(new_n54_), .b(new_n52_), .c(new_n22_), .O(new_n55_));
  inv1   g34(.a(x13), .O(new_n56_));
  nor2   g35(.a(new_n56_), .b(x02), .O(new_n57_));
  oai21  g36(.a(new_n57_), .b(new_n55_), .c(x04), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(new_n50_), .O(z3));
  inv1   g38(.a(x15), .O(new_n60_));
  nand4  g39(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(new_n60_), .O(z4));
endmodule


