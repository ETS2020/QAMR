// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nand3  g02(.a(new_n22_), .b(x03), .c(x02), .O(new_n24_));
  oai21  g03(.a(new_n23_), .b(new_n22_), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g05(.a(x00), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g08(.a(x13), .b(x10), .O(new_n30_));
  nand4  g09(.a(new_n30_), .b(new_n29_), .c(new_n26_), .d(x05), .O(z0));
  inv1   g10(.a(new_n30_), .O(new_n32_));
  nand2  g11(.a(new_n24_), .b(x11), .O(new_n33_));
  nor2   g12(.a(x11), .b(x09), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n23_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x04), .O(new_n37_));
  inv1   g16(.a(x01), .O(new_n38_));
  inv1   g17(.a(x05), .O(new_n39_));
  aoi21  g18(.a(new_n28_), .b(new_n38_), .c(new_n39_), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(new_n37_), .c(new_n32_), .O(z1));
  nor3   g20(.a(x12), .b(x11), .c(x09), .O(new_n42_));
  aoi22  g21(.a(new_n42_), .b(new_n23_), .c(new_n35_), .d(x12), .O(new_n43_));
  inv1   g22(.a(x06), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n28_), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n30_), .c(x05), .O(new_n46_));
  inv1   g25(.a(new_n46_), .O(new_n47_));
  oai21  g26(.a(new_n43_), .b(new_n28_), .c(new_n47_), .O(z2));
  oai21  g27(.a(x07), .b(x04), .c(x05), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n30_), .O(new_n50_));
  inv1   g29(.a(x10), .O(new_n51_));
  nor2   g30(.a(x12), .b(x11), .O(new_n52_));
  nand4  g31(.a(new_n52_), .b(new_n22_), .c(x03), .d(x02), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(x13), .c(new_n51_), .O(new_n54_));
  inv1   g33(.a(new_n24_), .O(new_n55_));
  nor3   g34(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(x04), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n50_), .O(z3));
  nand3  g39(.a(x10), .b(x08), .c(x03), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(x13), .O(new_n62_));
  nand3  g41(.a(new_n62_), .b(x15), .c(x14), .O(new_n63_));
  inv1   g42(.a(new_n63_), .O(z4));
endmodule


