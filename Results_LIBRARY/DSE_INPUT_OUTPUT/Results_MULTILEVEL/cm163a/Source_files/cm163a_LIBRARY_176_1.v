// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x03), .O(new_n22_));
  nand2  g01(.a(x12), .b(new_n22_), .O(new_n23_));
  oai21  g02(.a(x04), .b(x00), .c(x05), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  xnor2a g04(.a(x09), .b(x02), .O(new_n26_));
  inv1   g05(.a(x12), .O(new_n27_));
  nand3  g06(.a(new_n27_), .b(x09), .c(new_n22_), .O(new_n28_));
  oai21  g07(.a(new_n26_), .b(new_n22_), .c(new_n28_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x04), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n25_), .O(z0));
  oai21  g10(.a(x04), .b(x01), .c(x05), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  inv1   g12(.a(x02), .O(new_n34_));
  oai21  g13(.a(x09), .b(new_n34_), .c(x11), .O(new_n35_));
  inv1   g14(.a(x09), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(new_n35_), .c(new_n22_), .O(new_n39_));
  nand3  g18(.a(new_n27_), .b(x11), .c(new_n22_), .O(new_n40_));
  inv1   g19(.a(new_n40_), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n39_), .c(x04), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n33_), .O(z1));
  oai21  g22(.a(x06), .b(x04), .c(x05), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n23_), .O(new_n45_));
  nand2  g24(.a(new_n38_), .b(x12), .O(new_n46_));
  nand4  g25(.a(new_n27_), .b(new_n37_), .c(new_n36_), .d(x02), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(x04), .c(x03), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n45_), .O(z2));
  inv1   g29(.a(x04), .O(new_n51_));
  aoi21  g30(.a(x12), .b(new_n22_), .c(x07), .O(new_n52_));
  nor2   g31(.a(x12), .b(x05), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  inv1   g33(.a(x05), .O(new_n55_));
  nor2   g34(.a(x13), .b(x12), .O(new_n56_));
  oai21  g35(.a(new_n56_), .b(x03), .c(new_n55_), .O(new_n57_));
  nand2  g36(.a(x13), .b(new_n22_), .O(new_n58_));
  nor2   g37(.a(x13), .b(x11), .O(new_n59_));
  nand4  g38(.a(new_n59_), .b(new_n36_), .c(x03), .d(x02), .O(new_n60_));
  aoi21  g39(.a(new_n60_), .b(new_n58_), .c(x12), .O(new_n61_));
  nand3  g40(.a(new_n47_), .b(x13), .c(x03), .O(new_n62_));
  inv1   g41(.a(new_n62_), .O(new_n63_));
  oai21  g42(.a(new_n63_), .b(new_n61_), .c(x04), .O(new_n64_));
  nand3  g43(.a(new_n64_), .b(new_n57_), .c(new_n54_), .O(z3));
  inv1   g44(.a(x15), .O(new_n66_));
  nand4  g45(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n67_));
  nor2   g46(.a(new_n67_), .b(new_n66_), .O(z4));
endmodule


