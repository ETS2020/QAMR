// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  inv1   g02(.a(x03), .O(new_n24_));
  nor2   g03(.a(x09), .b(new_n24_), .O(new_n25_));
  aoi22  g04(.a(new_n25_), .b(x02), .c(new_n23_), .d(x09), .O(new_n26_));
  nor2   g05(.a(x04), .b(x00), .O(new_n27_));
  inv1   g06(.a(x09), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x08), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x05), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  oai21  g10(.a(new_n26_), .b(new_n22_), .c(new_n31_), .O(z0));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  nand2  g13(.a(new_n23_), .b(x11), .O(new_n35_));
  inv1   g14(.a(new_n23_), .O(new_n36_));
  nor2   g15(.a(x11), .b(x09), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(new_n35_), .c(x08), .O(new_n39_));
  inv1   g18(.a(x11), .O(new_n40_));
  nor2   g19(.a(new_n40_), .b(new_n28_), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n39_), .c(x04), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n34_), .O(z1));
  inv1   g22(.a(x12), .O(new_n44_));
  oai21  g23(.a(new_n44_), .b(new_n22_), .c(x09), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(x08), .O(new_n46_));
  aoi21  g25(.a(new_n37_), .b(new_n36_), .c(new_n44_), .O(new_n47_));
  nand2  g26(.a(new_n44_), .b(new_n40_), .O(new_n48_));
  nor3   g27(.a(new_n48_), .b(new_n23_), .c(x09), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n50_));
  inv1   g29(.a(x06), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  nand4  g31(.a(new_n52_), .b(new_n50_), .c(new_n46_), .d(x05), .O(z2));
  oai21  g32(.a(x07), .b(x04), .c(x05), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n29_), .O(new_n55_));
  oai21  g34(.a(new_n48_), .b(new_n23_), .c(x13), .O(new_n56_));
  nor3   g35(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(new_n25_), .c(x02), .O(new_n58_));
  aoi21  g37(.a(new_n58_), .b(new_n56_), .c(x08), .O(new_n59_));
  and2   g38(.a(x13), .b(x09), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n59_), .c(x04), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n55_), .O(z3));
  inv1   g41(.a(x08), .O(new_n63_));
  nand4  g42(.a(x15), .b(x14), .c(x10), .d(x03), .O(new_n64_));
  aoi21  g43(.a(new_n64_), .b(x09), .c(new_n63_), .O(z4));
endmodule


