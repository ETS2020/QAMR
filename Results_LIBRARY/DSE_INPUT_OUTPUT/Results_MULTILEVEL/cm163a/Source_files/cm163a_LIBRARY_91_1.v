// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_;
  inv1   g00(.a(x00), .O(new_n22_));
  inv1   g01(.a(x04), .O(new_n23_));
  oai21  g02(.a(x11), .b(new_n22_), .c(new_n23_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  inv1   g05(.a(new_n26_), .O(new_n27_));
  nand3  g06(.a(new_n25_), .b(x03), .c(x02), .O(new_n28_));
  oai21  g07(.a(new_n27_), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x04), .O(new_n30_));
  nand3  g09(.a(new_n30_), .b(new_n24_), .c(x05), .O(z0));
  inv1   g10(.a(x01), .O(new_n32_));
  oai21  g11(.a(x11), .b(new_n32_), .c(new_n23_), .O(new_n33_));
  nand2  g12(.a(new_n28_), .b(x11), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand4  g14(.a(new_n27_), .b(new_n35_), .c(new_n25_), .d(x04), .O(new_n36_));
  nand4  g15(.a(new_n36_), .b(new_n34_), .c(new_n33_), .d(x05), .O(z1));
  inv1   g16(.a(x05), .O(new_n38_));
  oai21  g17(.a(new_n35_), .b(x04), .c(new_n38_), .O(new_n39_));
  inv1   g18(.a(x06), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n23_), .O(new_n41_));
  inv1   g20(.a(x12), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(new_n25_), .c(x04), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n26_), .c(new_n41_), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n35_), .O(new_n45_));
  nand3  g24(.a(new_n27_), .b(new_n35_), .c(new_n25_), .O(new_n46_));
  nand3  g25(.a(new_n46_), .b(x12), .c(x04), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n45_), .c(new_n39_), .O(z2));
  inv1   g27(.a(x07), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n23_), .O(new_n50_));
  nand3  g29(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  inv1   g30(.a(x13), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n42_), .c(new_n25_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n35_), .O(new_n55_));
  nand4  g34(.a(new_n27_), .b(new_n42_), .c(new_n35_), .d(new_n25_), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(x13), .c(x04), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(new_n55_), .c(new_n39_), .O(z3));
  inv1   g37(.a(x03), .O(new_n59_));
  inv1   g38(.a(x15), .O(new_n60_));
  aoi21  g39(.a(x11), .b(new_n23_), .c(new_n60_), .O(new_n61_));
  nand4  g40(.a(new_n61_), .b(x14), .c(x10), .d(x08), .O(new_n62_));
  nor2   g41(.a(new_n62_), .b(new_n59_), .O(z4));
endmodule


