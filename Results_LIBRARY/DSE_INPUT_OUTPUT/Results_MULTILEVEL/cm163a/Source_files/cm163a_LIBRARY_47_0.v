// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nand3  g02(.a(new_n22_), .b(x03), .c(x02), .O(new_n24_));
  oai21  g03(.a(new_n23_), .b(new_n22_), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x04), .O(new_n26_));
  nor2   g05(.a(x04), .b(x00), .O(new_n27_));
  inv1   g06(.a(x07), .O(new_n28_));
  inv1   g07(.a(x08), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x05), .O(new_n31_));
  nor2   g10(.a(new_n31_), .b(new_n27_), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n26_), .O(z0));
  inv1   g12(.a(x04), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand4  g14(.a(new_n35_), .b(new_n22_), .c(x03), .d(x02), .O(new_n36_));
  inv1   g15(.a(new_n36_), .O(new_n37_));
  aoi21  g16(.a(new_n24_), .b(x11), .c(new_n37_), .O(new_n38_));
  nor2   g17(.a(x04), .b(x01), .O(new_n39_));
  nor2   g18(.a(new_n39_), .b(new_n31_), .O(new_n40_));
  oai21  g19(.a(new_n38_), .b(new_n34_), .c(new_n40_), .O(z1));
  nor3   g20(.a(x12), .b(x11), .c(x09), .O(new_n42_));
  aoi22  g21(.a(new_n42_), .b(new_n23_), .c(new_n36_), .d(x12), .O(new_n43_));
  inv1   g22(.a(x05), .O(new_n44_));
  oai21  g23(.a(x06), .b(x04), .c(new_n30_), .O(new_n45_));
  nor2   g24(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g25(.a(new_n43_), .b(new_n34_), .c(new_n46_), .O(z2));
  oai21  g26(.a(new_n29_), .b(new_n34_), .c(new_n28_), .O(new_n48_));
  inv1   g27(.a(x13), .O(new_n49_));
  aoi21  g28(.a(new_n42_), .b(new_n23_), .c(new_n49_), .O(new_n50_));
  inv1   g29(.a(x12), .O(new_n51_));
  nand3  g30(.a(new_n49_), .b(new_n51_), .c(new_n35_), .O(new_n52_));
  nor2   g31(.a(new_n52_), .b(new_n24_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand3  g33(.a(new_n54_), .b(new_n48_), .c(x05), .O(z3));
  nand2  g34(.a(x08), .b(x03), .O(new_n56_));
  nand3  g35(.a(x15), .b(x14), .c(x10), .O(new_n57_));
  oai21  g36(.a(new_n57_), .b(new_n56_), .c(new_n30_), .O(z4));
endmodule


