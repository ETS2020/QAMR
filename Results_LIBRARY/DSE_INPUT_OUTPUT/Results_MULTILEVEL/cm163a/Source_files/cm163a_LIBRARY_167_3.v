// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x11), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  nand3  g02(.a(new_n23_), .b(x04), .c(x03), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x02), .O(new_n26_));
  nand3  g05(.a(new_n22_), .b(x03), .c(x02), .O(new_n27_));
  nand3  g06(.a(new_n27_), .b(x09), .c(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  aoi21  g10(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(new_n28_), .c(new_n26_), .O(z0));
  oai21  g12(.a(x04), .b(x01), .c(x05), .O(new_n34_));
  aoi21  g13(.a(x11), .b(x04), .c(new_n34_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n26_), .O(z1));
  nand2  g15(.a(x04), .b(x03), .O(new_n37_));
  inv1   g16(.a(x12), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n23_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n37_), .c(new_n22_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(x02), .O(new_n41_));
  nand4  g20(.a(new_n22_), .b(new_n23_), .c(x03), .d(x02), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(x12), .c(x04), .O(new_n43_));
  inv1   g22(.a(x06), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n30_), .c(new_n31_), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n43_), .c(new_n41_), .O(z2));
  inv1   g25(.a(x13), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n38_), .c(new_n23_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n37_), .c(new_n22_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(x02), .O(new_n50_));
  nor2   g29(.a(x12), .b(x11), .O(new_n51_));
  nand4  g30(.a(new_n51_), .b(new_n23_), .c(x03), .d(x02), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(x13), .c(x04), .O(new_n53_));
  inv1   g32(.a(x07), .O(new_n54_));
  aoi21  g33(.a(new_n54_), .b(new_n30_), .c(new_n31_), .O(new_n55_));
  nand3  g34(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(z3));
  inv1   g35(.a(x03), .O(new_n57_));
  inv1   g36(.a(x08), .O(new_n58_));
  nand2  g37(.a(x11), .b(x02), .O(new_n59_));
  nand4  g38(.a(new_n59_), .b(x15), .c(x14), .d(x10), .O(new_n60_));
  nor3   g39(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(z4));
endmodule


