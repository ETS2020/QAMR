// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x14), .b(x00), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  inv1   g05(.a(new_n26_), .O(new_n27_));
  nand3  g06(.a(new_n25_), .b(x03), .c(x02), .O(new_n28_));
  oai21  g07(.a(new_n27_), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x04), .O(new_n30_));
  nand3  g09(.a(new_n30_), .b(new_n24_), .c(x05), .O(z0));
  nand2  g10(.a(x14), .b(x01), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  nand2  g12(.a(new_n28_), .b(x11), .O(new_n34_));
  nor2   g13(.a(x11), .b(x09), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n27_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x04), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(new_n33_), .c(x05), .O(z1));
  inv1   g18(.a(x05), .O(new_n40_));
  oai21  g19(.a(x14), .b(x04), .c(new_n40_), .O(new_n41_));
  inv1   g20(.a(x12), .O(new_n42_));
  aoi21  g21(.a(new_n35_), .b(new_n27_), .c(new_n42_), .O(new_n43_));
  inv1   g22(.a(x11), .O(new_n44_));
  nand3  g23(.a(new_n42_), .b(new_n44_), .c(new_n25_), .O(new_n45_));
  nor2   g24(.a(new_n45_), .b(new_n26_), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n43_), .c(x04), .O(new_n47_));
  inv1   g26(.a(x06), .O(new_n48_));
  nand3  g27(.a(x14), .b(new_n48_), .c(new_n22_), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(new_n47_), .c(new_n41_), .O(z2));
  nand2  g29(.a(x14), .b(x07), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  inv1   g31(.a(x13), .O(new_n53_));
  nor3   g32(.a(x12), .b(x11), .c(x09), .O(new_n54_));
  aoi21  g33(.a(new_n54_), .b(new_n27_), .c(new_n53_), .O(new_n55_));
  nand3  g34(.a(new_n53_), .b(new_n42_), .c(new_n44_), .O(new_n56_));
  nor2   g35(.a(new_n56_), .b(new_n28_), .O(new_n57_));
  oai21  g36(.a(new_n57_), .b(new_n55_), .c(x04), .O(new_n58_));
  nand3  g37(.a(new_n58_), .b(new_n52_), .c(x05), .O(z3));
  inv1   g38(.a(x15), .O(new_n60_));
  nand4  g39(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(new_n60_), .O(z4));
endmodule


