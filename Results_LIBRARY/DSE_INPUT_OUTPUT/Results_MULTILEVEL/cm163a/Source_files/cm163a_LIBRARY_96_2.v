// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x09), .O(new_n22_));
  inv1   g01(.a(x05), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  aoi21  g03(.a(new_n24_), .b(x04), .c(new_n23_), .O(new_n25_));
  nand3  g04(.a(new_n22_), .b(x03), .c(x02), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(x05), .c(x04), .O(new_n27_));
  inv1   g06(.a(new_n27_), .O(new_n28_));
  oai22  g07(.a(new_n28_), .b(x00), .c(new_n25_), .d(new_n22_), .O(z0));
  nand2  g08(.a(new_n22_), .b(x00), .O(new_n30_));
  oai21  g09(.a(x04), .b(x01), .c(x05), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g11(.a(new_n24_), .b(x11), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  and2   g13(.a(x03), .b(x02), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(new_n34_), .c(new_n22_), .O(new_n36_));
  aoi21  g15(.a(new_n36_), .b(new_n33_), .c(x00), .O(new_n37_));
  nor2   g16(.a(new_n34_), .b(new_n22_), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n37_), .c(x04), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n32_), .O(z1));
  oai21  g19(.a(x06), .b(x04), .c(x05), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n30_), .O(new_n42_));
  nand3  g21(.a(new_n34_), .b(x03), .c(x02), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x12), .O(new_n44_));
  nor2   g23(.a(x12), .b(x11), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n35_), .c(new_n22_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(x00), .O(new_n47_));
  inv1   g26(.a(x12), .O(new_n48_));
  nor2   g27(.a(new_n48_), .b(new_n22_), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n42_), .O(z2));
  inv1   g30(.a(x04), .O(new_n52_));
  inv1   g31(.a(x13), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n52_), .c(x09), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(x00), .O(new_n55_));
  nor3   g34(.a(x12), .b(x11), .c(x09), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n35_), .c(new_n53_), .O(new_n57_));
  nand3  g36(.a(new_n53_), .b(new_n48_), .c(new_n34_), .O(new_n58_));
  nor2   g37(.a(new_n58_), .b(new_n26_), .O(new_n59_));
  oai21  g38(.a(new_n59_), .b(new_n57_), .c(x04), .O(new_n60_));
  inv1   g39(.a(x07), .O(new_n61_));
  aoi21  g40(.a(new_n61_), .b(new_n52_), .c(new_n23_), .O(new_n62_));
  nand3  g41(.a(new_n62_), .b(new_n60_), .c(new_n55_), .O(z3));
  inv1   g42(.a(x03), .O(new_n64_));
  and2   g43(.a(new_n30_), .b(x15), .O(new_n65_));
  nand4  g44(.a(new_n65_), .b(x14), .c(x10), .d(x08), .O(new_n66_));
  nor2   g45(.a(new_n66_), .b(new_n64_), .O(z4));
endmodule


