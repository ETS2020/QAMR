// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nand3  g02(.a(new_n22_), .b(x03), .c(x02), .O(new_n24_));
  oai21  g03(.a(new_n23_), .b(new_n22_), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g05(.a(x00), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g08(.a(x09), .b(x07), .O(new_n30_));
  nand4  g09(.a(new_n30_), .b(new_n29_), .c(new_n26_), .d(x05), .O(z0));
  oai21  g10(.a(x04), .b(x01), .c(x05), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand2  g12(.a(x03), .b(x02), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(x11), .O(new_n35_));
  inv1   g14(.a(x11), .O(new_n36_));
  nand3  g15(.a(new_n36_), .b(x03), .c(x02), .O(new_n37_));
  aoi21  g16(.a(new_n37_), .b(new_n35_), .c(x09), .O(new_n38_));
  nor3   g17(.a(new_n36_), .b(new_n22_), .c(x07), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n38_), .c(x04), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n33_), .O(z1));
  aoi21  g20(.a(x09), .b(x07), .c(x06), .O(new_n42_));
  nor2   g21(.a(x07), .b(x05), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n42_), .c(new_n28_), .O(new_n44_));
  inv1   g23(.a(x05), .O(new_n45_));
  oai21  g24(.a(x12), .b(x07), .c(x09), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g26(.a(new_n37_), .b(x12), .O(new_n48_));
  inv1   g27(.a(x12), .O(new_n49_));
  nand3  g28(.a(new_n23_), .b(new_n49_), .c(new_n36_), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n48_), .c(x09), .O(new_n51_));
  nor3   g30(.a(new_n49_), .b(new_n22_), .c(x07), .O(new_n52_));
  oai21  g31(.a(new_n52_), .b(new_n51_), .c(x04), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(new_n47_), .c(new_n44_), .O(z2));
  oai21  g33(.a(x13), .b(x07), .c(x09), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n45_), .O(new_n56_));
  inv1   g35(.a(x07), .O(new_n57_));
  inv1   g36(.a(x13), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n22_), .c(x04), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g39(.a(new_n50_), .b(x13), .O(new_n61_));
  nand4  g40(.a(new_n23_), .b(new_n58_), .c(new_n49_), .d(new_n36_), .O(new_n62_));
  nand2  g41(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g42(.a(new_n63_), .b(new_n22_), .c(x04), .O(new_n64_));
  nand3  g43(.a(new_n64_), .b(new_n60_), .c(new_n56_), .O(z3));
  inv1   g44(.a(x03), .O(new_n66_));
  inv1   g45(.a(x08), .O(new_n67_));
  nand4  g46(.a(new_n30_), .b(x15), .c(x14), .d(x10), .O(new_n68_));
  nor3   g47(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(z4));
endmodule


