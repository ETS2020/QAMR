// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_;
  nand2  g00(.a(x03), .b(x02), .O(new_n22_));
  nand2  g01(.a(new_n22_), .b(x09), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  nand3  g03(.a(new_n24_), .b(x03), .c(x02), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(new_n23_), .c(x05), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  nand2  g07(.a(x05), .b(new_n28_), .O(new_n29_));
  oai21  g08(.a(new_n29_), .b(x00), .c(new_n27_), .O(z0));
  nand2  g09(.a(new_n25_), .b(x11), .O(new_n31_));
  inv1   g10(.a(new_n22_), .O(new_n32_));
  nor2   g11(.a(x11), .b(x09), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(new_n31_), .c(x05), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x04), .O(new_n36_));
  or2    g15(.a(new_n29_), .b(x01), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n36_), .O(z1));
  inv1   g17(.a(x12), .O(new_n39_));
  aoi21  g18(.a(new_n33_), .b(new_n32_), .c(new_n39_), .O(new_n40_));
  inv1   g19(.a(x11), .O(new_n41_));
  nand3  g20(.a(new_n39_), .b(new_n41_), .c(new_n24_), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(new_n22_), .c(x05), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n40_), .c(x04), .O(new_n44_));
  inv1   g23(.a(x06), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(x05), .c(new_n28_), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n44_), .O(z2));
  inv1   g26(.a(x13), .O(new_n48_));
  nor3   g27(.a(x12), .b(x11), .c(x09), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n32_), .c(new_n48_), .O(new_n50_));
  nand3  g29(.a(new_n48_), .b(new_n39_), .c(new_n41_), .O(new_n51_));
  oai21  g30(.a(new_n51_), .b(new_n25_), .c(x05), .O(new_n52_));
  oai21  g31(.a(new_n52_), .b(new_n50_), .c(x04), .O(new_n53_));
  inv1   g32(.a(x07), .O(new_n54_));
  nand3  g33(.a(new_n54_), .b(x05), .c(new_n28_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n53_), .O(z3));
  inv1   g35(.a(x03), .O(new_n57_));
  inv1   g36(.a(x05), .O(new_n58_));
  inv1   g37(.a(x15), .O(new_n59_));
  aoi21  g38(.a(new_n58_), .b(new_n28_), .c(new_n59_), .O(new_n60_));
  nand4  g39(.a(new_n60_), .b(x14), .c(x10), .d(x08), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(new_n57_), .O(z4));
endmodule


