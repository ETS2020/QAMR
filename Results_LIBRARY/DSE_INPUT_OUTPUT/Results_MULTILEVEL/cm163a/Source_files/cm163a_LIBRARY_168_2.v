// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_;
  inv1   g00(.a(x11), .O(new_n22_));
  nand2  g01(.a(new_n22_), .b(x06), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  and2   g04(.a(x03), .b(x02), .O(new_n26_));
  inv1   g05(.a(x03), .O(new_n27_));
  nor2   g06(.a(x09), .b(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x02), .O(new_n29_));
  oai21  g08(.a(new_n26_), .b(new_n25_), .c(new_n29_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  inv1   g10(.a(x00), .O(new_n32_));
  inv1   g11(.a(x04), .O(new_n33_));
  inv1   g12(.a(x05), .O(new_n34_));
  aoi21  g13(.a(new_n33_), .b(new_n32_), .c(new_n34_), .O(new_n35_));
  aoi21  g14(.a(new_n35_), .b(new_n31_), .c(new_n24_), .O(z0));
  inv1   g15(.a(x06), .O(new_n37_));
  nand3  g16(.a(new_n26_), .b(new_n25_), .c(x04), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n22_), .O(new_n40_));
  nand3  g19(.a(new_n29_), .b(x11), .c(x04), .O(new_n41_));
  inv1   g20(.a(x01), .O(new_n42_));
  aoi21  g21(.a(new_n33_), .b(new_n42_), .c(new_n34_), .O(new_n43_));
  nand3  g22(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  nand2  g23(.a(new_n23_), .b(new_n34_), .O(new_n45_));
  nand2  g24(.a(new_n29_), .b(x12), .O(new_n46_));
  inv1   g25(.a(x12), .O(new_n47_));
  nand4  g26(.a(new_n26_), .b(new_n47_), .c(new_n22_), .d(new_n25_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n46_), .c(x04), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  nand3  g29(.a(x12), .b(x11), .c(x04), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n50_), .c(new_n45_), .O(z2));
  oai21  g31(.a(x07), .b(x04), .c(x05), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n23_), .O(new_n54_));
  nand4  g33(.a(new_n47_), .b(new_n25_), .c(x03), .d(x02), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(x13), .O(new_n56_));
  nor3   g35(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(new_n28_), .c(x02), .O(new_n58_));
  aoi21  g37(.a(new_n58_), .b(new_n56_), .c(x06), .O(new_n59_));
  and2   g38(.a(x13), .b(x11), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n59_), .c(x04), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n54_), .O(z3));
  nand2  g41(.a(x08), .b(x03), .O(new_n63_));
  nand3  g42(.a(x15), .b(x14), .c(x10), .O(new_n64_));
  oai21  g43(.a(new_n64_), .b(new_n63_), .c(new_n23_), .O(z4));
endmodule


