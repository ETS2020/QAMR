// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x02), .O(new_n22_));
  nand2  g01(.a(x03), .b(new_n22_), .O(new_n23_));
  oai21  g02(.a(x04), .b(x00), .c(x05), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g04(.a(x09), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(x03), .c(x02), .O(new_n27_));
  oai21  g06(.a(new_n26_), .b(x03), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n25_), .O(z0));
  oai21  g09(.a(x04), .b(x01), .c(x05), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n23_), .O(new_n32_));
  nand2  g11(.a(x11), .b(x09), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(new_n26_), .c(x03), .O(new_n35_));
  aoi21  g14(.a(new_n35_), .b(new_n33_), .c(new_n22_), .O(new_n36_));
  inv1   g15(.a(x03), .O(new_n37_));
  nand2  g16(.a(x09), .b(x02), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(x11), .c(new_n37_), .O(new_n39_));
  inv1   g18(.a(new_n39_), .O(new_n40_));
  oai21  g19(.a(new_n40_), .b(new_n36_), .c(x04), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n32_), .O(z1));
  oai21  g21(.a(x06), .b(x04), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n23_), .O(new_n44_));
  oai21  g23(.a(x11), .b(x09), .c(x12), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(new_n34_), .c(new_n26_), .d(x03), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n45_), .c(new_n22_), .O(new_n48_));
  nor2   g27(.a(new_n46_), .b(x03), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n44_), .O(z2));
  oai21  g30(.a(x07), .b(x04), .c(x05), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n23_), .O(new_n53_));
  nand3  g32(.a(new_n46_), .b(new_n34_), .c(new_n26_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(x13), .O(new_n55_));
  nor2   g34(.a(x13), .b(x12), .O(new_n56_));
  nand4  g35(.a(new_n56_), .b(new_n34_), .c(new_n26_), .d(x03), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n55_), .c(new_n22_), .O(new_n58_));
  inv1   g37(.a(x13), .O(new_n59_));
  nor2   g38(.a(new_n59_), .b(x03), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n58_), .c(x04), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n53_), .O(z3));
  inv1   g41(.a(x14), .O(new_n63_));
  inv1   g42(.a(x15), .O(new_n64_));
  nand4  g43(.a(x10), .b(x08), .c(x03), .d(x02), .O(new_n65_));
  nor3   g44(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(z4));
endmodule


