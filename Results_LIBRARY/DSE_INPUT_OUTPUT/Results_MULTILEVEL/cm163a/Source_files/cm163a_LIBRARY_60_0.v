// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_;
  nor2   g00(.a(x11), .b(x01), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  and2   g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n23_), .b(x03), .c(x02), .O(new_n25_));
  oai21  g04(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g06(.a(x00), .O(new_n28_));
  inv1   g07(.a(x04), .O(new_n29_));
  inv1   g08(.a(x05), .O(new_n30_));
  aoi21  g09(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  aoi21  g10(.a(new_n31_), .b(new_n27_), .c(new_n22_), .O(z0));
  inv1   g11(.a(x01), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  oai21  g13(.a(new_n34_), .b(new_n29_), .c(new_n33_), .O(new_n35_));
  nand2  g14(.a(new_n25_), .b(x11), .O(new_n36_));
  nand3  g15(.a(new_n24_), .b(new_n34_), .c(new_n23_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x04), .O(new_n39_));
  nand3  g18(.a(new_n39_), .b(new_n35_), .c(x05), .O(z1));
  inv1   g19(.a(new_n22_), .O(new_n41_));
  oai21  g20(.a(x06), .b(x04), .c(x05), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g22(.a(new_n25_), .b(x12), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand4  g24(.a(new_n24_), .b(new_n45_), .c(new_n34_), .d(new_n23_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(new_n33_), .O(new_n47_));
  nor2   g26(.a(new_n45_), .b(new_n34_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n43_), .O(z2));
  oai21  g29(.a(x07), .b(x04), .c(x05), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  nand4  g31(.a(new_n45_), .b(new_n23_), .c(x03), .d(x02), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(x13), .O(new_n54_));
  inv1   g33(.a(x03), .O(new_n55_));
  nor2   g34(.a(x09), .b(new_n55_), .O(new_n56_));
  nor3   g35(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(new_n56_), .c(x02), .O(new_n58_));
  aoi21  g37(.a(new_n58_), .b(new_n54_), .c(new_n33_), .O(new_n59_));
  and2   g38(.a(x13), .b(x11), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n59_), .c(x04), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n52_), .O(z3));
  inv1   g41(.a(x15), .O(new_n63_));
  nor2   g42(.a(new_n22_), .b(new_n63_), .O(new_n64_));
  nand4  g43(.a(new_n64_), .b(x14), .c(x10), .d(x08), .O(new_n65_));
  nor2   g44(.a(new_n65_), .b(new_n55_), .O(z4));
endmodule


