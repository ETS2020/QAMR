// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x05), .b(x00), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x05), .O(new_n25_));
  inv1   g04(.a(x11), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x09), .O(new_n28_));
  inv1   g07(.a(x09), .O(new_n29_));
  nand3  g08(.a(new_n29_), .b(x03), .c(x02), .O(new_n30_));
  aoi21  g09(.a(new_n30_), .b(new_n28_), .c(new_n22_), .O(new_n31_));
  oai21  g10(.a(new_n31_), .b(new_n25_), .c(new_n26_), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n24_), .O(z0));
  aoi21  g12(.a(x05), .b(x01), .c(x04), .O(new_n34_));
  nand4  g13(.a(new_n29_), .b(x04), .c(x03), .d(x02), .O(new_n35_));
  aoi21  g14(.a(new_n35_), .b(x05), .c(x11), .O(new_n36_));
  or2    g15(.a(new_n36_), .b(new_n34_), .O(z1));
  nand2  g16(.a(x06), .b(x05), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n22_), .O(new_n39_));
  nand2  g18(.a(new_n30_), .b(x12), .O(new_n40_));
  inv1   g19(.a(x12), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(new_n29_), .c(x03), .d(x02), .O(new_n42_));
  aoi21  g21(.a(new_n42_), .b(new_n40_), .c(new_n22_), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n25_), .c(new_n26_), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n39_), .O(z2));
  and2   g24(.a(new_n42_), .b(x13), .O(new_n46_));
  inv1   g25(.a(x13), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n41_), .c(new_n29_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n27_), .c(new_n26_), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  inv1   g29(.a(x07), .O(new_n51_));
  aoi21  g30(.a(new_n51_), .b(new_n22_), .c(new_n25_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n50_), .O(z3));
  inv1   g32(.a(x03), .O(new_n54_));
  inv1   g33(.a(x08), .O(new_n55_));
  nand2  g34(.a(x11), .b(x04), .O(new_n56_));
  nand4  g35(.a(new_n56_), .b(x15), .c(x14), .d(x10), .O(new_n57_));
  nor3   g36(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z4));
endmodule


