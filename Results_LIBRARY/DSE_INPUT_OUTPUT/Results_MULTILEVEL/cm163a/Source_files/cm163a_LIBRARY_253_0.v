// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x05), .O(new_n22_));
  oai21  g01(.a(x07), .b(x04), .c(new_n22_), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  nand2  g03(.a(x03), .b(x02), .O(new_n25_));
  inv1   g04(.a(new_n25_), .O(new_n26_));
  nand3  g05(.a(new_n24_), .b(x03), .c(x02), .O(new_n27_));
  oai21  g06(.a(new_n26_), .b(new_n24_), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  inv1   g08(.a(x00), .O(new_n30_));
  inv1   g09(.a(x04), .O(new_n31_));
  nand3  g10(.a(x07), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(new_n29_), .c(new_n23_), .O(z0));
  nand2  g12(.a(x07), .b(x01), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g14(.a(new_n27_), .b(x11), .O(new_n36_));
  nor2   g15(.a(x11), .b(x09), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n26_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(x04), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n35_), .c(x05), .O(z1));
  nand2  g20(.a(x07), .b(x06), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n31_), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  aoi21  g23(.a(new_n37_), .b(new_n26_), .c(new_n44_), .O(new_n45_));
  nor2   g24(.a(x12), .b(x11), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(new_n24_), .c(x03), .d(x02), .O(new_n47_));
  inv1   g26(.a(new_n47_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n45_), .c(x04), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(new_n43_), .c(x05), .O(z2));
  inv1   g29(.a(new_n27_), .O(new_n51_));
  nor3   g30(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  aoi22  g31(.a(new_n52_), .b(new_n51_), .c(new_n47_), .d(x13), .O(new_n53_));
  nor2   g32(.a(x07), .b(x04), .O(new_n54_));
  nor2   g33(.a(new_n54_), .b(new_n22_), .O(new_n55_));
  oai21  g34(.a(new_n53_), .b(new_n31_), .c(new_n55_), .O(z3));
  inv1   g35(.a(x03), .O(new_n57_));
  inv1   g36(.a(x15), .O(new_n58_));
  nor2   g37(.a(new_n54_), .b(new_n58_), .O(new_n59_));
  nand4  g38(.a(new_n59_), .b(x14), .c(x10), .d(x08), .O(new_n60_));
  nor2   g39(.a(new_n60_), .b(new_n57_), .O(z4));
endmodule


