// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_;
  nand2  g00(.a(x11), .b(x09), .O(new_n22_));
  oai21  g01(.a(x04), .b(x00), .c(x05), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x11), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(new_n25_), .c(x09), .O(new_n27_));
  inv1   g06(.a(x09), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(x03), .c(x02), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x04), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n24_), .O(z0));
  nand2  g11(.a(new_n26_), .b(x11), .O(new_n33_));
  nand2  g12(.a(new_n25_), .b(new_n28_), .O(new_n34_));
  oai21  g13(.a(new_n34_), .b(new_n26_), .c(new_n33_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x04), .O(new_n36_));
  inv1   g15(.a(x01), .O(new_n37_));
  inv1   g16(.a(x04), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand4  g18(.a(new_n39_), .b(new_n36_), .c(new_n22_), .d(x05), .O(z1));
  inv1   g19(.a(x12), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n38_), .c(new_n25_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(x09), .O(new_n43_));
  nand3  g22(.a(new_n25_), .b(x03), .c(x02), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(x12), .O(new_n45_));
  and2   g24(.a(x03), .b(x02), .O(new_n46_));
  nor2   g25(.a(x12), .b(x11), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n46_), .c(new_n28_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(x04), .O(new_n50_));
  inv1   g29(.a(x05), .O(new_n51_));
  inv1   g30(.a(x06), .O(new_n52_));
  aoi21  g31(.a(new_n52_), .b(new_n38_), .c(new_n51_), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(new_n50_), .c(new_n43_), .O(z2));
  inv1   g33(.a(x13), .O(new_n55_));
  oai21  g34(.a(new_n55_), .b(new_n38_), .c(new_n25_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(x09), .O(new_n57_));
  aoi21  g36(.a(new_n47_), .b(new_n46_), .c(new_n55_), .O(new_n58_));
  nand3  g37(.a(new_n55_), .b(new_n41_), .c(new_n25_), .O(new_n59_));
  nor2   g38(.a(new_n59_), .b(new_n29_), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n58_), .c(x04), .O(new_n61_));
  inv1   g40(.a(x07), .O(new_n62_));
  aoi21  g41(.a(new_n62_), .b(new_n38_), .c(new_n51_), .O(new_n63_));
  nand3  g42(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(z3));
  inv1   g43(.a(x03), .O(new_n65_));
  inv1   g44(.a(x08), .O(new_n66_));
  nand4  g45(.a(new_n22_), .b(x15), .c(x14), .d(x10), .O(new_n67_));
  nor3   g46(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(z4));
endmodule


