// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  inv1   g03(.a(x03), .O(new_n25_));
  nor2   g04(.a(x09), .b(new_n25_), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x02), .O(new_n27_));
  oai21  g06(.a(new_n24_), .b(new_n22_), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  inv1   g08(.a(x00), .O(new_n30_));
  inv1   g09(.a(x04), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g11(.a(x11), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(x10), .O(new_n34_));
  nand4  g13(.a(new_n34_), .b(new_n32_), .c(new_n29_), .d(x05), .O(z0));
  oai21  g14(.a(x04), .b(x01), .c(x05), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nor3   g16(.a(x11), .b(x10), .c(x09), .O(new_n38_));
  aoi22  g17(.a(new_n38_), .b(new_n24_), .c(new_n27_), .d(x11), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n31_), .c(new_n37_), .O(z1));
  inv1   g19(.a(x12), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n31_), .c(x11), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(x10), .O(new_n43_));
  nor2   g22(.a(x11), .b(x09), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n24_), .c(new_n41_), .O(new_n45_));
  nor4   g24(.a(new_n23_), .b(x12), .c(x11), .d(x09), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n45_), .c(x04), .O(new_n47_));
  inv1   g26(.a(x05), .O(new_n48_));
  inv1   g27(.a(x06), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n31_), .c(new_n48_), .O(new_n50_));
  nand3  g29(.a(new_n50_), .b(new_n47_), .c(new_n43_), .O(z2));
  oai21  g30(.a(x07), .b(x04), .c(x05), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand2  g32(.a(new_n41_), .b(new_n22_), .O(new_n54_));
  oai21  g33(.a(new_n54_), .b(new_n23_), .c(x13), .O(new_n55_));
  nor3   g34(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(new_n26_), .c(x02), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n55_), .c(x10), .O(new_n58_));
  and2   g37(.a(x13), .b(x11), .O(new_n59_));
  oai21  g38(.a(new_n59_), .b(new_n58_), .c(x04), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n53_), .O(z3));
  inv1   g40(.a(x10), .O(new_n62_));
  nand4  g41(.a(x15), .b(x14), .c(x08), .d(x03), .O(new_n63_));
  aoi21  g42(.a(new_n63_), .b(x11), .c(new_n62_), .O(z4));
endmodule


