// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_;
  inv1   g00(.a(x04), .O(new_n22_));
  inv1   g01(.a(x12), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x03), .O(new_n24_));
  oai21  g03(.a(x04), .b(x00), .c(x05), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  oai21  g05(.a(new_n23_), .b(x02), .c(x03), .O(new_n27_));
  nand2  g06(.a(x03), .b(x02), .O(new_n28_));
  inv1   g07(.a(new_n28_), .O(new_n29_));
  nor2   g08(.a(new_n23_), .b(x09), .O(new_n30_));
  aoi22  g09(.a(new_n30_), .b(new_n29_), .c(new_n27_), .d(x09), .O(new_n31_));
  oai21  g10(.a(new_n31_), .b(new_n22_), .c(new_n26_), .O(z0));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  inv1   g14(.a(x02), .O(new_n36_));
  oai21  g15(.a(x09), .b(new_n36_), .c(x12), .O(new_n37_));
  aoi21  g16(.a(new_n37_), .b(x03), .c(new_n35_), .O(new_n38_));
  inv1   g17(.a(x09), .O(new_n39_));
  nand3  g18(.a(x12), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  nor2   g19(.a(new_n40_), .b(new_n28_), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n38_), .c(x04), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n34_), .O(z1));
  oai21  g22(.a(x06), .b(x04), .c(x05), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n24_), .O(new_n45_));
  nand3  g24(.a(new_n29_), .b(new_n35_), .c(new_n39_), .O(new_n46_));
  nand3  g25(.a(new_n46_), .b(x12), .c(x04), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n45_), .O(z2));
  nor2   g27(.a(x07), .b(x04), .O(new_n49_));
  aoi21  g28(.a(x13), .b(x04), .c(new_n49_), .O(new_n50_));
  aoi22  g29(.a(new_n50_), .b(x05), .c(new_n23_), .d(x03), .O(z3));
  inv1   g30(.a(x14), .O(new_n52_));
  inv1   g31(.a(x15), .O(new_n53_));
  nand4  g32(.a(x12), .b(x10), .c(x08), .d(x03), .O(new_n54_));
  nor3   g33(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z4));
endmodule


