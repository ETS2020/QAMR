// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x04), .O(new_n22_));
  inv1   g01(.a(x03), .O(new_n23_));
  nor2   g02(.a(x09), .b(new_n23_), .O(new_n24_));
  oai21  g03(.a(new_n24_), .b(new_n22_), .c(x02), .O(new_n25_));
  inv1   g04(.a(x09), .O(new_n26_));
  and2   g05(.a(x03), .b(x02), .O(new_n27_));
  nor2   g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  inv1   g08(.a(x00), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  aoi21  g10(.a(new_n22_), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(new_n29_), .c(new_n25_), .O(z0));
  inv1   g12(.a(x11), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(new_n26_), .c(x03), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x04), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x02), .O(new_n37_));
  nand2  g16(.a(new_n24_), .b(x02), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(x11), .c(x04), .O(new_n39_));
  inv1   g18(.a(x01), .O(new_n40_));
  aoi21  g19(.a(new_n22_), .b(new_n40_), .c(new_n31_), .O(new_n41_));
  nand3  g20(.a(new_n41_), .b(new_n39_), .c(new_n37_), .O(z1));
  nand2  g21(.a(new_n26_), .b(x03), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n34_), .O(new_n45_));
  oai21  g24(.a(new_n45_), .b(new_n43_), .c(x04), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(x02), .O(new_n47_));
  nand3  g26(.a(new_n27_), .b(new_n34_), .c(new_n26_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(x12), .c(x04), .O(new_n49_));
  inv1   g28(.a(x06), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n22_), .c(new_n31_), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(z2));
  inv1   g31(.a(x13), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(new_n44_), .c(new_n34_), .O(new_n54_));
  oai21  g33(.a(new_n54_), .b(new_n43_), .c(x04), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(x02), .O(new_n56_));
  nand4  g35(.a(new_n27_), .b(new_n44_), .c(new_n34_), .d(new_n26_), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(x13), .c(x04), .O(new_n58_));
  inv1   g37(.a(x07), .O(new_n59_));
  aoi21  g38(.a(new_n59_), .b(new_n22_), .c(new_n31_), .O(new_n60_));
  nand3  g39(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(z3));
  inv1   g40(.a(x15), .O(new_n62_));
  aoi21  g41(.a(new_n22_), .b(x02), .c(new_n62_), .O(new_n63_));
  nand4  g42(.a(new_n63_), .b(x14), .c(x10), .d(x08), .O(new_n64_));
  nor2   g43(.a(new_n64_), .b(new_n23_), .O(z4));
endmodule


