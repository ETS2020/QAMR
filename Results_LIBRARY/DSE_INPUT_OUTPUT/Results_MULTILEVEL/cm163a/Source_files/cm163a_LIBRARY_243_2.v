// Benchmark "FAU" written by ABC on Thu Aug 13 15:20:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nand3  g02(.a(new_n22_), .b(x03), .c(x02), .O(new_n24_));
  oai21  g03(.a(new_n23_), .b(new_n22_), .c(new_n24_), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g05(.a(x05), .O(new_n27_));
  nor2   g06(.a(x04), .b(x00), .O(new_n28_));
  inv1   g07(.a(x07), .O(new_n29_));
  nor2   g08(.a(x09), .b(new_n29_), .O(new_n30_));
  nor3   g09(.a(new_n30_), .b(new_n28_), .c(new_n27_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n26_), .O(z0));
  inv1   g11(.a(new_n30_), .O(new_n33_));
  oai21  g12(.a(x04), .b(x01), .c(x05), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g14(.a(x03), .b(x02), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x11), .O(new_n37_));
  inv1   g16(.a(x11), .O(new_n38_));
  nand3  g17(.a(new_n23_), .b(new_n38_), .c(new_n22_), .O(new_n39_));
  aoi21  g18(.a(new_n39_), .b(new_n37_), .c(x07), .O(new_n40_));
  nor2   g19(.a(new_n38_), .b(new_n22_), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n40_), .c(x04), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n35_), .O(z1));
  oai21  g22(.a(x06), .b(x04), .c(x05), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n33_), .O(new_n45_));
  nand3  g24(.a(new_n38_), .b(x03), .c(x02), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(x12), .O(new_n47_));
  nor2   g26(.a(x12), .b(x11), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n23_), .c(new_n22_), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n47_), .c(x07), .O(new_n50_));
  inv1   g29(.a(x12), .O(new_n51_));
  nor2   g30(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  oai21  g31(.a(new_n52_), .b(new_n50_), .c(x04), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n45_), .O(z2));
  nand2  g33(.a(new_n33_), .b(new_n27_), .O(new_n55_));
  inv1   g34(.a(x13), .O(new_n56_));
  aoi21  g35(.a(new_n48_), .b(new_n23_), .c(new_n56_), .O(new_n57_));
  nand3  g36(.a(new_n56_), .b(new_n51_), .c(new_n38_), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n24_), .c(x04), .O(new_n59_));
  oai21  g38(.a(new_n59_), .b(new_n57_), .c(new_n29_), .O(new_n60_));
  nand3  g39(.a(x13), .b(x09), .c(x04), .O(new_n61_));
  nand3  g40(.a(new_n61_), .b(new_n60_), .c(new_n55_), .O(z3));
  inv1   g41(.a(x03), .O(new_n63_));
  inv1   g42(.a(x15), .O(new_n64_));
  nor2   g43(.a(new_n30_), .b(new_n64_), .O(new_n65_));
  nand4  g44(.a(new_n65_), .b(x14), .c(x10), .d(x08), .O(new_n66_));
  nor2   g45(.a(new_n66_), .b(new_n63_), .O(z4));
endmodule


