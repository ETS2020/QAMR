// Benchmark "FAU" written by ABC on Thu Aug 13 15:19:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x00), .O(new_n22_));
  inv1   g01(.a(x04), .O(new_n23_));
  oai21  g02(.a(x12), .b(new_n22_), .c(new_n23_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  and2   g04(.a(x03), .b(x02), .O(new_n26_));
  nand3  g05(.a(new_n25_), .b(x03), .c(x02), .O(new_n27_));
  oai21  g06(.a(new_n26_), .b(new_n25_), .c(new_n27_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  nand3  g08(.a(new_n29_), .b(new_n24_), .c(x05), .O(z0));
  inv1   g09(.a(x05), .O(new_n31_));
  inv1   g10(.a(x12), .O(new_n32_));
  oai21  g11(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand2  g12(.a(new_n27_), .b(x11), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand3  g14(.a(new_n26_), .b(new_n35_), .c(new_n25_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x04), .O(new_n38_));
  inv1   g17(.a(x01), .O(new_n39_));
  nand3  g18(.a(new_n32_), .b(new_n23_), .c(new_n39_), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n38_), .c(new_n33_), .O(z1));
  inv1   g20(.a(x06), .O(new_n42_));
  oai21  g21(.a(x12), .b(new_n42_), .c(new_n23_), .O(new_n43_));
  nand2  g22(.a(new_n36_), .b(x12), .O(new_n44_));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g24(.a(new_n45_), .O(new_n46_));
  nor3   g25(.a(x12), .b(x11), .c(x09), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n46_), .c(new_n31_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n44_), .c(new_n43_), .O(z2));
  inv1   g28(.a(x13), .O(new_n50_));
  nand3  g29(.a(new_n50_), .b(new_n35_), .c(new_n25_), .O(new_n51_));
  oai22  g30(.a(new_n51_), .b(new_n45_), .c(x07), .d(x04), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n32_), .O(new_n53_));
  nand2  g32(.a(new_n47_), .b(new_n26_), .O(new_n54_));
  nand3  g33(.a(new_n54_), .b(x13), .c(x04), .O(new_n55_));
  nand3  g34(.a(new_n55_), .b(new_n53_), .c(new_n33_), .O(z3));
  inv1   g35(.a(x03), .O(new_n57_));
  inv1   g36(.a(x15), .O(new_n58_));
  aoi21  g37(.a(x12), .b(new_n23_), .c(new_n58_), .O(new_n59_));
  nand4  g38(.a(new_n59_), .b(x14), .c(x10), .d(x08), .O(new_n60_));
  nor2   g39(.a(new_n60_), .b(new_n57_), .O(z4));
endmodule


