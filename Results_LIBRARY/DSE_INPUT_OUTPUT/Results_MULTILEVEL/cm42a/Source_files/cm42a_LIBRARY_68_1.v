// Benchmark "FAU" written by ABC on Thu Aug 13 14:33:12 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n21_, new_n22_, new_n24_, new_n26_,
    new_n27_, new_n28_, new_n33_, new_n34_;
  inv1   g00(.a(x0), .O(new_n15_));
  nor2   g01(.a(x3), .b(x2), .O(new_n16_));
  oai21  g02(.a(new_n16_), .b(x1), .c(new_n15_), .O(z0));
  inv1   g03(.a(x1), .O(new_n18_));
  nand3  g04(.a(new_n16_), .b(new_n18_), .c(x0), .O(z1));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand4  g07(.a(new_n22_), .b(new_n21_), .c(x1), .d(x0), .O(z3));
  nor2   g08(.a(x3), .b(new_n21_), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x1), .c(new_n15_), .O(z4));
  nand3  g10(.a(new_n22_), .b(x2), .c(new_n18_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x0), .O(new_n27_));
  nand2  g12(.a(new_n18_), .b(new_n15_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(z5));
  nand3  g14(.a(new_n24_), .b(x1), .c(x0), .O(z7));
  nand4  g15(.a(x3), .b(new_n21_), .c(new_n18_), .d(new_n15_), .O(z8));
  nand2  g16(.a(x1), .b(x0), .O(new_n33_));
  oai22  g17(.a(new_n22_), .b(x2), .c(new_n18_), .d(x0), .O(new_n34_));
  nand3  g18(.a(new_n34_), .b(new_n28_), .c(new_n33_), .O(z9));
  one    g19(.O(z2));
  one    g20(.O(z6));
endmodule


