// Benchmark "FAU" written by ABC on Thu Aug 13 14:33:17 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n17_, new_n18_, new_n21_, new_n23_, new_n25_, new_n27_, new_n30_;
  inv1   g00(.a(x2), .O(new_n17_));
  inv1   g01(.a(x3), .O(new_n18_));
  nand2  g02(.a(new_n18_), .b(new_n17_), .O(z2));
  nor2   g03(.a(x1), .b(x0), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n17_), .c(new_n18_), .O(z4));
  inv1   g05(.a(x1), .O(new_n23_));
  nand4  g06(.a(new_n18_), .b(x2), .c(new_n23_), .d(x0), .O(z5));
  inv1   g07(.a(x0), .O(new_n25_));
  nand4  g08(.a(new_n18_), .b(x2), .c(x1), .d(new_n25_), .O(z6));
  oai21  g09(.a(new_n23_), .b(new_n25_), .c(x2), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n18_), .O(z7));
  oai21  g11(.a(new_n21_), .b(new_n18_), .c(new_n17_), .O(z8));
  oai21  g12(.a(x1), .b(new_n25_), .c(x3), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n17_), .O(z9));
  one    g14(.O(z0));
  one    g15(.O(z1));
  one    g16(.O(z3));
endmodule


