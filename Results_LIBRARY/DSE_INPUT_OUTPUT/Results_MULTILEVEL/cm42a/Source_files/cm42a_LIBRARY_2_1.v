// Benchmark "FAU" written by ABC on Thu Aug 13 14:32:53 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n20_, new_n22_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n31_, new_n32_, new_n34_, new_n35_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nor2   g02(.a(x2), .b(x1), .O(new_n17_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(new_n15_), .O(z0));
  nor2   g04(.a(new_n16_), .b(x0), .O(new_n19_));
  xor2a  g05(.a(x3), .b(x0), .O(new_n20_));
  oai21  g06(.a(new_n19_), .b(new_n17_), .c(new_n20_), .O(z1));
  inv1   g07(.a(x2), .O(new_n22_));
  nand4  g08(.a(new_n16_), .b(new_n22_), .c(x1), .d(new_n15_), .O(z2));
  nand2  g09(.a(new_n16_), .b(new_n15_), .O(new_n24_));
  nand3  g10(.a(new_n16_), .b(new_n22_), .c(x1), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(x0), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n24_), .O(z3));
  inv1   g13(.a(x1), .O(new_n28_));
  nand4  g14(.a(new_n16_), .b(x2), .c(new_n28_), .d(new_n15_), .O(z4));
  nand4  g15(.a(new_n16_), .b(x2), .c(new_n28_), .d(x0), .O(z5));
  oai21  g16(.a(x1), .b(x0), .c(x2), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n16_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n15_), .O(z6));
  nand3  g19(.a(new_n16_), .b(x2), .c(x1), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x0), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n24_), .O(z7));
  inv1   g22(.a(new_n19_), .O(z8));
  nand3  g23(.a(new_n17_), .b(x3), .c(x0), .O(z9));
endmodule


