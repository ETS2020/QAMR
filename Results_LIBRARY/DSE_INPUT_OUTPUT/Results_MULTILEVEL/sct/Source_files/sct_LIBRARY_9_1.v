// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n106_;
  inv1   g00(.a(x16), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x12), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand3  g13(.a(x12), .b(new_n47_), .c(x02), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n36_), .O(new_n52_));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  aoi21  g19(.a(new_n47_), .b(x02), .c(new_n35_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n52_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n46_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n46_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n55_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n52_), .O(z04));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  and2   g31(.a(x07), .b(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n64_), .c(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nand2  g35(.a(new_n54_), .b(x12), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(z05));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n66_), .c(x08), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  aoi21  g40(.a(new_n67_), .b(x10), .c(new_n74_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n36_), .c(new_n70_), .O(z06));
  inv1   g42(.a(new_n63_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n65_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x04), .O(new_n80_));
  aoi21  g46(.a(new_n73_), .b(x11), .c(new_n80_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n36_), .c(new_n70_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  inv1   g49(.a(new_n79_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(x16), .c(new_n83_), .O(new_n85_));
  aoi21  g51(.a(new_n79_), .b(x12), .c(new_n43_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n55_), .O(z08));
  nand2  g53(.a(new_n52_), .b(new_n43_), .O(new_n88_));
  oai21  g54(.a(new_n54_), .b(x13), .c(x12), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nor2   g56(.a(new_n84_), .b(new_n90_), .O(new_n91_));
  nand3  g57(.a(new_n78_), .b(new_n90_), .c(new_n83_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n67_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n91_), .c(new_n35_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n89_), .c(new_n88_), .O(z09));
  nand2  g61(.a(new_n49_), .b(x00), .O(new_n96_));
  nand4  g62(.a(new_n78_), .b(new_n35_), .c(new_n90_), .d(new_n83_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n96_), .c(x09), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(x08), .c(x07), .d(x06), .O(new_n99_));
  nand3  g65(.a(new_n54_), .b(x14), .c(x12), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n99_), .c(new_n43_), .O(z10));
  nand2  g67(.a(new_n52_), .b(new_n37_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n37_), .c(x03), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n43_), .c(new_n52_), .O(z12));
  nor2   g70(.a(new_n36_), .b(new_n43_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n52_), .O(z14));
endmodule


