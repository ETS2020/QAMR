// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x10), .O(new_n35_));
  nor2   g01(.a(x13), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(new_n36_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n37_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n47_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n46_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n46_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n48_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n47_), .O(z04));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand2  g27(.a(x07), .b(x06), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(x08), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(new_n36_), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n60_), .c(new_n48_), .d(x04), .O(z05));
  nand3  g33(.a(x13), .b(new_n61_), .c(x08), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n62_), .c(x10), .O(new_n69_));
  nand4  g35(.a(new_n63_), .b(new_n35_), .c(new_n61_), .d(x08), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n69_), .c(new_n48_), .d(x04), .O(z06));
  inv1   g37(.a(new_n48_), .O(new_n72_));
  inv1   g38(.a(x13), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(x11), .c(x10), .O(new_n74_));
  oai21  g40(.a(new_n72_), .b(new_n43_), .c(new_n74_), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nor2   g42(.a(new_n65_), .b(new_n76_), .O(new_n77_));
  nand3  g43(.a(new_n76_), .b(new_n61_), .c(x08), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n62_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n77_), .c(new_n35_), .O(new_n80_));
  nand3  g46(.a(x13), .b(x11), .c(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n75_), .O(z07));
  oai21  g48(.a(new_n73_), .b(x12), .c(x10), .O(new_n83_));
  oai21  g49(.a(new_n72_), .b(new_n43_), .c(new_n83_), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(new_n59_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n76_), .d(new_n61_), .O(new_n87_));
  oai21  g53(.a(new_n79_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n35_), .O(new_n89_));
  nand3  g55(.a(x13), .b(x12), .c(x10), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n84_), .O(z08));
  nand2  g57(.a(new_n87_), .b(x13), .O(new_n92_));
  nand4  g58(.a(new_n73_), .b(new_n85_), .c(new_n76_), .d(new_n61_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n86_), .c(x10), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n92_), .c(new_n48_), .d(x04), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  inv1   g63(.a(x03), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x02), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(x16), .c(x09), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(x08), .c(x07), .d(x06), .O(new_n101_));
  nand2  g67(.a(new_n72_), .b(x14), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n97_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  nand4  g70(.a(new_n48_), .b(new_n73_), .c(new_n85_), .d(new_n76_), .O(new_n105_));
  nor3   g71(.a(new_n105_), .b(x10), .c(x09), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(x08), .c(x07), .d(x06), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n104_), .c(new_n43_), .O(z10));
  nor2   g74(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g75(.a(x16), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(x02), .c(new_n98_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n47_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  nor2   g79(.a(new_n36_), .b(new_n43_), .O(z13));
  inv1   g80(.a(x17), .O(new_n115_));
  nor3   g81(.a(new_n36_), .b(new_n115_), .c(new_n43_), .O(z14));
endmodule


