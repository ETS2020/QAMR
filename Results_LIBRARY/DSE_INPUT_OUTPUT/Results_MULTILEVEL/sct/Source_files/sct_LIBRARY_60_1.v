// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n121_;
  inv1   g00(.a(x12), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n36_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n37_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n35_), .c(new_n47_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  inv1   g16(.a(new_n36_), .O(new_n51_));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n48_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n51_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n47_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n47_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n51_), .O(z04));
  oai21  g25(.a(x12), .b(x09), .c(new_n47_), .O(new_n60_));
  inv1   g26(.a(x07), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  nor2   g31(.a(new_n61_), .b(new_n47_), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n62_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n66_), .c(new_n44_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n65_), .c(new_n60_), .d(new_n48_), .O(z05));
  oai21  g35(.a(x12), .b(x10), .c(new_n47_), .O(new_n70_));
  nand2  g36(.a(new_n67_), .b(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n66_), .c(new_n44_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n72_), .c(new_n70_), .d(new_n48_), .O(z06));
  oai21  g43(.a(x12), .b(x11), .c(new_n47_), .O(new_n78_));
  nand2  g44(.a(new_n73_), .b(new_n63_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  nand2  g46(.a(new_n63_), .b(x06), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nor3   g48(.a(x11), .b(x10), .c(x09), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n82_), .c(new_n44_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n80_), .c(new_n78_), .d(new_n48_), .O(z07));
  inv1   g51(.a(new_n48_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n44_), .c(new_n51_), .O(new_n87_));
  inv1   g53(.a(new_n71_), .O(new_n88_));
  aoi21  g54(.a(new_n83_), .b(new_n63_), .c(new_n35_), .O(new_n89_));
  nor3   g55(.a(x12), .b(x11), .c(x10), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n88_), .c(new_n89_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n47_), .c(new_n87_), .O(z08));
  oai21  g58(.a(x13), .b(x12), .c(new_n47_), .O(new_n93_));
  oai21  g59(.a(new_n86_), .b(new_n44_), .c(new_n93_), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  inv1   g61(.a(x11), .O(new_n96_));
  nand3  g62(.a(new_n73_), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  oai22  g63(.a(new_n97_), .b(new_n81_), .c(new_n95_), .d(x06), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n35_), .O(new_n99_));
  nand2  g65(.a(new_n90_), .b(new_n88_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(x13), .c(x06), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n99_), .c(new_n94_), .O(z09));
  inv1   g68(.a(x09), .O(new_n103_));
  inv1   g69(.a(x00), .O(new_n104_));
  inv1   g70(.a(x10), .O(new_n105_));
  nand4  g71(.a(new_n95_), .b(new_n35_), .c(new_n96_), .d(new_n105_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n48_), .c(new_n103_), .d(x08), .O(new_n108_));
  nand2  g74(.a(new_n86_), .b(x14), .O(new_n109_));
  oai21  g75(.a(new_n108_), .b(new_n61_), .c(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x06), .O(new_n111_));
  nand3  g77(.a(new_n86_), .b(x14), .c(new_n35_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n111_), .c(new_n44_), .O(z10));
  nor2   g79(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g80(.a(x03), .O(new_n115_));
  inv1   g81(.a(x16), .O(new_n116_));
  oai21  g82(.a(new_n116_), .b(x02), .c(new_n115_), .O(new_n117_));
  nand3  g83(.a(new_n117_), .b(new_n51_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z12));
  nor2   g85(.a(new_n36_), .b(new_n44_), .O(z13));
  inv1   g86(.a(x17), .O(new_n121_));
  nor3   g87(.a(new_n36_), .b(new_n121_), .c(new_n44_), .O(z14));
endmodule


