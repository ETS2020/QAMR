// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n112_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x06), .b(x04), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nor2   g06(.a(x05), .b(new_n40_), .O(new_n41_));
  inv1   g07(.a(x06), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x04), .O(new_n43_));
  oai22  g09(.a(new_n43_), .b(new_n41_), .c(new_n39_), .d(x15), .O(new_n44_));
  nand3  g10(.a(x06), .b(x05), .c(new_n40_), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n44_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n40_), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n48_), .c(x06), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand2  g18(.a(new_n48_), .b(x16), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n52_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(new_n36_), .O(new_n56_));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x06), .c(x04), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n42_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n56_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  inv1   g32(.a(new_n53_), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x09), .c(new_n67_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n40_), .c(new_n66_), .O(z05));
  nand2  g36(.a(new_n63_), .b(x08), .O(new_n71_));
  nand2  g37(.a(x07), .b(x06), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n71_), .c(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(x08), .c(x07), .d(x06), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n53_), .d(x04), .O(z06));
  nand2  g42(.a(x08), .b(x07), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n63_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n77_), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  aoi21  g47(.a(new_n75_), .b(x11), .c(new_n67_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n40_), .c(new_n81_), .O(z07));
  inv1   g49(.a(x10), .O(new_n84_));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n64_), .c(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x06), .O(new_n88_));
  inv1   g54(.a(new_n68_), .O(new_n89_));
  nand3  g55(.a(new_n78_), .b(new_n89_), .c(new_n63_), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(x12), .c(new_n67_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n40_), .c(new_n88_), .O(z08));
  inv1   g58(.a(x11), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n84_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n64_), .c(x04), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x06), .O(new_n98_));
  nand3  g64(.a(new_n85_), .b(new_n74_), .c(new_n89_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(x13), .c(new_n67_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n40_), .c(new_n98_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n96_), .b(new_n102_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n53_), .c(new_n63_), .d(x08), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(x07), .c(x06), .O(new_n106_));
  nand2  g72(.a(new_n67_), .b(x14), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n40_), .O(z10));
  nor2   g74(.a(new_n36_), .b(new_n35_), .O(z11));
  aoi21  g75(.a(x16), .b(new_n35_), .c(x03), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n40_), .c(new_n56_), .O(z12));
  nand2  g77(.a(x17), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n56_), .O(z14));
  buf    g79(.a(x04), .O(z13));
endmodule


