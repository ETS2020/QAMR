// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n98_, new_n101_;
  inv1   g00(.a(x12), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x07), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n38_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n50_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(z02));
  nand3  g19(.a(x12), .b(x07), .c(new_n50_), .O(new_n54_));
  oai21  g20(.a(x07), .b(new_n50_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n51_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n50_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n50_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n36_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x12), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x07), .O(new_n66_));
  nand4  g32(.a(x12), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(x09), .c(new_n47_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n51_), .O(z05));
  nand2  g35(.a(new_n51_), .b(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n36_), .O(new_n71_));
  nand2  g37(.a(new_n66_), .b(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand3  g39(.a(x08), .b(x07), .c(x06), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x12), .c(new_n73_), .d(new_n63_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n72_), .c(new_n71_), .O(z06));
  inv1   g43(.a(x07), .O(new_n78_));
  and2   g44(.a(x08), .b(x06), .O(new_n79_));
  nor2   g45(.a(x10), .b(x09), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n79_), .c(new_n35_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n78_), .c(x11), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand4  g49(.a(new_n80_), .b(new_n75_), .c(x12), .d(new_n83_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n71_), .O(z07));
  nand3  g51(.a(x12), .b(new_n83_), .c(new_n73_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n64_), .c(x07), .O(new_n87_));
  aoi21  g53(.a(x12), .b(new_n78_), .c(new_n47_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n51_), .O(z08));
  nor2   g55(.a(x13), .b(new_n47_), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n51_), .c(new_n37_), .O(z09));
  nand4  g57(.a(new_n51_), .b(new_n63_), .c(x08), .d(x07), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n50_), .O(new_n93_));
  nor2   g59(.a(new_n51_), .b(new_n41_), .O(new_n94_));
  aoi21  g60(.a(new_n93_), .b(x00), .c(new_n94_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n47_), .c(new_n36_), .O(z10));
  nor2   g62(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n38_), .c(x03), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n47_), .c(new_n36_), .O(z12));
  nor2   g65(.a(new_n37_), .b(new_n47_), .O(z13));
  nand2  g66(.a(new_n36_), .b(x17), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n47_), .O(z14));
endmodule


