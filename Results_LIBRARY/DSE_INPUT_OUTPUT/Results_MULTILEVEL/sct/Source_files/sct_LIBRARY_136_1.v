// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n103_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  oai21  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n36_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(z00));
  inv1   g08(.a(new_n41_), .O(new_n43_));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n39_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n50_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n41_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n51_), .c(new_n41_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n50_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n50_), .c(new_n58_), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n51_), .c(new_n41_), .d(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n63_), .c(new_n51_), .d(x04), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n41_), .O(z05));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n41_), .c(x04), .O(new_n70_));
  aoi21  g36(.a(new_n65_), .b(x10), .c(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n51_), .O(z06));
  inv1   g38(.a(new_n62_), .O(new_n73_));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n64_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x04), .O(new_n76_));
  aoi21  g42(.a(new_n69_), .b(x11), .c(new_n76_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n51_), .c(new_n43_), .O(z07));
  nand2  g44(.a(new_n75_), .b(x12), .O(new_n79_));
  nor2   g45(.a(x12), .b(x11), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n68_), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(new_n62_), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n43_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n79_), .c(new_n51_), .d(x04), .O(z08));
  oai21  g50(.a(new_n82_), .b(x14), .c(new_n36_), .O(new_n85_));
  oai21  g51(.a(new_n81_), .b(new_n62_), .c(x13), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n51_), .d(x04), .O(z09));
  oai21  g53(.a(new_n37_), .b(x13), .c(x00), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nor2   g55(.a(x14), .b(x13), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n74_), .c(new_n89_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n51_), .c(new_n64_), .d(x08), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(x07), .c(x06), .O(new_n95_));
  nor2   g61(.a(new_n51_), .b(new_n37_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x13), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n95_), .c(new_n47_), .O(z10));
  nand2  g64(.a(new_n41_), .b(new_n39_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n39_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n47_), .c(new_n41_), .O(z12));
  nor2   g67(.a(new_n43_), .b(new_n47_), .O(z13));
  nand2  g68(.a(x17), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n41_), .O(z14));
endmodule


