// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n115_;
  nor2   g00(.a(x11), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n35_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand4  g13(.a(new_n47_), .b(x11), .c(new_n46_), .d(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  inv1   g15(.a(new_n35_), .O(new_n50_));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n50_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand3  g20(.a(x11), .b(x08), .c(new_n46_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n46_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n47_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  inv1   g24(.a(x11), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x09), .c(new_n46_), .O(new_n60_));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  nand2  g28(.a(x07), .b(x06), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  inv1   g30(.a(x08), .O(new_n65_));
  nor2   g31(.a(x09), .b(new_n65_), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(new_n64_), .c(new_n43_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n62_), .c(new_n60_), .d(new_n47_), .O(z05));
  oai21  g34(.a(new_n59_), .b(x10), .c(new_n46_), .O(new_n69_));
  nand2  g35(.a(new_n66_), .b(x07), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand4  g39(.a(new_n64_), .b(new_n73_), .c(new_n72_), .d(x08), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(x04), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n71_), .c(new_n69_), .d(new_n47_), .O(z06));
  inv1   g42(.a(new_n47_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n43_), .c(new_n50_), .O(new_n78_));
  nand2  g44(.a(new_n74_), .b(x11), .O(new_n79_));
  inv1   g45(.a(new_n61_), .O(new_n80_));
  nand3  g46(.a(new_n59_), .b(new_n73_), .c(new_n72_), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(x06), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(z07));
  oai21  g50(.a(x12), .b(new_n59_), .c(new_n46_), .O(new_n85_));
  oai21  g51(.a(new_n81_), .b(new_n61_), .c(x12), .O(new_n86_));
  inv1   g52(.a(new_n70_), .O(new_n87_));
  nor3   g53(.a(x12), .b(x11), .c(x10), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(new_n87_), .c(new_n43_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n86_), .c(new_n85_), .d(new_n47_), .O(z08));
  oai21  g56(.a(x13), .b(new_n59_), .c(new_n46_), .O(new_n91_));
  nand2  g57(.a(new_n88_), .b(new_n87_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x13), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n59_), .d(new_n73_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n87_), .c(new_n43_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n93_), .c(new_n91_), .d(new_n47_), .O(z09));
  inv1   g65(.a(x07), .O(new_n100_));
  nand2  g66(.a(x06), .b(x00), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n96_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n47_), .c(new_n72_), .d(x08), .O(new_n103_));
  nand2  g69(.a(new_n77_), .b(x14), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n100_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n50_), .O(z10));
  nor2   g73(.a(new_n35_), .b(new_n36_), .O(z11));
  inv1   g74(.a(x03), .O(new_n109_));
  inv1   g75(.a(x16), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(x02), .c(new_n109_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n50_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  nand2  g79(.a(new_n50_), .b(new_n43_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n50_), .O(z14));
endmodule


