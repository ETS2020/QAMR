// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n117_;
  nand2  g00(.a(x16), .b(x06), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n35_), .O(z02));
  nand3  g18(.a(new_n50_), .b(x07), .c(new_n46_), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand3  g20(.a(new_n48_), .b(new_n54_), .c(x06), .O(new_n55_));
  aoi21  g21(.a(new_n55_), .b(new_n53_), .c(new_n43_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n46_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n46_), .c(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  nand4  g26(.a(x08), .b(new_n46_), .c(new_n47_), .d(x02), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n60_), .c(new_n43_), .O(z04));
  nand2  g28(.a(new_n35_), .b(new_n43_), .O(new_n63_));
  oai21  g29(.a(new_n49_), .b(x09), .c(new_n46_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nand4  g33(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n48_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n64_), .c(new_n63_), .O(z05));
  oai21  g37(.a(new_n49_), .b(x10), .c(new_n46_), .O(new_n72_));
  nand3  g38(.a(new_n65_), .b(x08), .c(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x08), .c(x07), .d(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n48_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n72_), .c(new_n63_), .O(z06));
  oai21  g45(.a(new_n49_), .b(x11), .c(new_n46_), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  aoi21  g47(.a(new_n75_), .b(new_n67_), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n67_), .b(x06), .O(new_n83_));
  inv1   g49(.a(x10), .O(new_n84_));
  nand3  g50(.a(new_n81_), .b(new_n84_), .c(new_n65_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n82_), .c(new_n48_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n80_), .c(new_n63_), .O(z07));
  oai21  g54(.a(new_n49_), .b(x12), .c(new_n46_), .O(new_n89_));
  oai21  g55(.a(new_n85_), .b(new_n66_), .c(x12), .O(new_n90_));
  nor2   g56(.a(x12), .b(x11), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n75_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n83_), .c(new_n90_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n48_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n89_), .c(new_n63_), .O(z08));
  oai21  g61(.a(new_n49_), .b(x13), .c(new_n46_), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n84_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n73_), .c(x13), .O(new_n98_));
  inv1   g64(.a(x12), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n81_), .d(new_n84_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n68_), .c(new_n98_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n96_), .c(new_n63_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  nand2  g71(.a(new_n101_), .b(new_n105_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n48_), .c(new_n65_), .d(x08), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(x07), .c(x06), .O(new_n109_));
  nand3  g75(.a(new_n49_), .b(x14), .c(new_n46_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n43_), .O(z10));
  nand2  g77(.a(new_n35_), .b(new_n37_), .O(z11));
  nand2  g78(.a(new_n35_), .b(x03), .O(new_n113_));
  nand3  g79(.a(x16), .b(new_n46_), .c(new_n37_), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n113_), .c(new_n43_), .O(z12));
  nor2   g81(.a(new_n36_), .b(new_n43_), .O(z13));
  nand3  g82(.a(new_n35_), .b(x17), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z14));
endmodule


