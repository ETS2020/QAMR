// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi22  g05(.a(new_n39_), .b(new_n36_), .c(x18), .d(x04), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  oai21  g07(.a(x18), .b(new_n41_), .c(x04), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  inv1   g09(.a(x15), .O(new_n44_));
  nand3  g10(.a(x18), .b(new_n44_), .c(new_n41_), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(new_n42_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x18), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n35_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  aoi21  g20(.a(new_n54_), .b(new_n49_), .c(new_n43_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n56_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n57_), .b(new_n64_), .c(x08), .O(new_n65_));
  and2   g31(.a(new_n65_), .b(new_n49_), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n63_), .c(new_n50_), .d(x04), .O(z05));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n57_), .c(x08), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n49_), .c(x04), .O(new_n70_));
  aoi21  g36(.a(new_n65_), .b(x10), .c(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n50_), .O(z06));
  nand2  g38(.a(new_n69_), .b(x11), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  inv1   g41(.a(new_n62_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n64_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n73_), .c(new_n50_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n49_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x04), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  nand4  g47(.a(new_n68_), .b(new_n76_), .c(new_n81_), .d(new_n75_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n49_), .c(x04), .O(new_n83_));
  aoi21  g49(.a(new_n77_), .b(x12), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n50_), .O(z08));
  nand2  g51(.a(new_n82_), .b(x13), .O(new_n86_));
  inv1   g52(.a(new_n65_), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n81_), .c(new_n75_), .d(new_n74_), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n86_), .c(new_n50_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n49_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n89_), .b(new_n95_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n50_), .c(new_n64_), .d(x08), .O(new_n97_));
  inv1   g63(.a(new_n97_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(x07), .c(x06), .O(new_n99_));
  nor2   g65(.a(new_n50_), .b(new_n38_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(x18), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n99_), .c(new_n43_), .O(z10));
  aoi21  g68(.a(x18), .b(x04), .c(new_n35_), .O(z11));
  inv1   g69(.a(x03), .O(new_n104_));
  aoi21  g70(.a(x16), .b(new_n35_), .c(x18), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n43_), .O(z12));
  nor2   g72(.a(x18), .b(new_n43_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  aoi21  g74(.a(new_n49_), .b(new_n108_), .c(new_n43_), .O(z14));
endmodule


