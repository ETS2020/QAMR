// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n100_, new_n103_;
  inv1   g00(.a(x18), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x15), .O(new_n36_));
  or2    g02(.a(x14), .b(x01), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  oai21  g07(.a(new_n35_), .b(x15), .c(new_n41_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n36_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n36_), .c(new_n46_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n36_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n46_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n46_), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n50_), .c(new_n36_), .d(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(new_n36_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(x04), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n50_), .c(new_n61_), .O(z05));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x04), .O(new_n71_));
  aoi21  g37(.a(new_n65_), .b(x10), .c(new_n71_), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n50_), .c(new_n61_), .O(z06));
  nand2  g39(.a(new_n70_), .b(x11), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  inv1   g42(.a(new_n62_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n64_), .O(new_n78_));
  and2   g44(.a(new_n78_), .b(new_n36_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n74_), .c(new_n50_), .d(x04), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  nand4  g47(.a(new_n69_), .b(new_n77_), .c(new_n81_), .d(new_n76_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  aoi21  g49(.a(new_n78_), .b(x12), .c(new_n83_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n50_), .c(new_n61_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n81_), .c(new_n76_), .d(new_n75_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n65_), .c(x04), .O(new_n88_));
  aoi21  g54(.a(new_n82_), .b(x13), .c(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n50_), .c(new_n61_), .O(z09));
  inv1   g56(.a(x00), .O(new_n91_));
  aoi21  g57(.a(new_n87_), .b(new_n91_), .c(new_n49_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n64_), .c(x08), .d(x07), .O(new_n93_));
  nand2  g59(.a(new_n49_), .b(x14), .O(new_n94_));
  oai21  g60(.a(new_n93_), .b(new_n46_), .c(new_n94_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n36_), .c(x04), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(z10));
  nor2   g63(.a(new_n61_), .b(new_n38_), .O(z11));
  oai21  g64(.a(new_n48_), .b(x02), .c(new_n47_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n36_), .c(x04), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(z12));
  nor2   g67(.a(new_n61_), .b(new_n43_), .O(z13));
  nand2  g68(.a(new_n36_), .b(x17), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n43_), .O(z14));
endmodule


