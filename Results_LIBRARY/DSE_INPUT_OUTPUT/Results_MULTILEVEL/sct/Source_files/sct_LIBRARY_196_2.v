// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x13), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z01));
  inv1   g15(.a(x06), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n38_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n50_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n36_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n50_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n50_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n51_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n36_), .O(z04));
  nand2  g27(.a(new_n51_), .b(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n36_), .O(new_n63_));
  and2   g29(.a(x08), .b(x07), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n35_), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  and2   g33(.a(x07), .b(x06), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x13), .c(new_n67_), .d(x08), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x13), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  aoi21  g40(.a(new_n69_), .b(x10), .c(new_n47_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n51_), .O(z06));
  nand2  g42(.a(new_n74_), .b(x11), .O(new_n77_));
  nor2   g43(.a(new_n35_), .b(x11), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n71_), .c(new_n64_), .d(x06), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n63_), .O(z07));
  nor3   g46(.a(x11), .b(x10), .c(x09), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n64_), .c(new_n35_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n50_), .c(x12), .O(new_n83_));
  inv1   g49(.a(x10), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand4  g52(.a(x13), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n68_), .c(new_n67_), .d(x08), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n83_), .c(new_n63_), .O(z08));
  nand3  g56(.a(new_n67_), .b(x08), .c(x07), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n87_), .c(x06), .O(new_n92_));
  aoi21  g58(.a(x13), .b(new_n50_), .c(new_n47_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n51_), .O(z09));
  nand4  g60(.a(new_n51_), .b(new_n67_), .c(x08), .d(x07), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n50_), .O(new_n96_));
  nor2   g62(.a(new_n51_), .b(new_n41_), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(x00), .c(new_n97_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n47_), .c(new_n36_), .O(z10));
  nand2  g65(.a(new_n36_), .b(new_n38_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n38_), .c(x03), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n47_), .c(new_n36_), .O(z12));
  nor2   g68(.a(new_n37_), .b(new_n47_), .O(z13));
  nand2  g69(.a(x17), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n36_), .O(z14));
endmodule


