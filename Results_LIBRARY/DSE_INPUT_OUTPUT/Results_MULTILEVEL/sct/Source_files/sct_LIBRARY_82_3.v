// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n112_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x17), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n35_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(new_n37_), .O(new_n54_));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n51_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n54_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n35_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n35_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n54_), .O(z04));
  inv1   g28(.a(x08), .O(new_n63_));
  nor2   g29(.a(x09), .b(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n36_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  and2   g33(.a(x07), .b(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n36_), .c(x08), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(x09), .c(new_n45_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n51_), .O(z05));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x08), .c(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n36_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  inv1   g41(.a(x09), .O(new_n76_));
  nand4  g42(.a(new_n68_), .b(new_n36_), .c(new_n76_), .d(x08), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x10), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n75_), .c(new_n51_), .d(x04), .O(z06));
  oai21  g45(.a(new_n50_), .b(new_n45_), .c(new_n54_), .O(new_n80_));
  nand2  g46(.a(new_n75_), .b(x11), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand3  g48(.a(x08), .b(x07), .c(x06), .O(new_n83_));
  inv1   g49(.a(new_n83_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n72_), .c(new_n36_), .d(new_n82_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n81_), .c(new_n80_), .O(z07));
  inv1   g52(.a(x10), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n82_), .c(new_n87_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n65_), .c(new_n36_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  aoi21  g57(.a(new_n85_), .b(x12), .c(new_n45_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n51_), .O(z08));
  nand2  g59(.a(new_n91_), .b(x13), .O(new_n94_));
  nor2   g60(.a(x11), .b(x10), .O(new_n95_));
  nor3   g61(.a(x17), .b(x13), .c(x12), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n95_), .c(new_n68_), .d(new_n64_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n94_), .c(new_n80_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nor2   g65(.a(x13), .b(x12), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n95_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n99_), .c(new_n50_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n76_), .c(x08), .d(x07), .O(new_n103_));
  nand2  g69(.a(new_n50_), .b(x14), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n35_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n54_), .O(z10));
  nor2   g73(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n38_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n45_), .c(new_n54_), .O(z12));
  oai21  g76(.a(new_n36_), .b(new_n35_), .c(new_n45_), .O(z13));
  nand3  g77(.a(x17), .b(new_n35_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z14));
endmodule


