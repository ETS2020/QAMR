// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n36_), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n36_), .b(x05), .c(new_n42_), .O(new_n45_));
  oai21  g11(.a(new_n44_), .b(new_n41_), .c(new_n45_), .O(z01));
  inv1   g12(.a(new_n37_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n36_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n47_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n36_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n36_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n48_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n60_), .c(new_n48_), .d(x04), .O(z05));
  inv1   g29(.a(x10), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n36_), .O(new_n66_));
  nand3  g32(.a(new_n61_), .b(x08), .c(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x10), .O(new_n68_));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n48_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x04), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n66_), .O(z06));
  oai21  g39(.a(x11), .b(new_n42_), .c(new_n36_), .O(new_n74_));
  nand3  g40(.a(new_n69_), .b(x08), .c(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x11), .O(new_n76_));
  inv1   g42(.a(new_n59_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n61_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n76_), .c(new_n48_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n74_), .O(z07));
  nand2  g48(.a(new_n79_), .b(x12), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  nand4  g51(.a(new_n69_), .b(new_n77_), .c(new_n85_), .d(new_n84_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n83_), .c(new_n48_), .d(x04), .O(z08));
  oai21  g53(.a(x13), .b(new_n42_), .c(new_n36_), .O(new_n88_));
  nand3  g54(.a(new_n85_), .b(new_n84_), .c(new_n64_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n67_), .c(x13), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  inv1   g57(.a(new_n62_), .O(new_n92_));
  nand4  g58(.a(new_n78_), .b(new_n92_), .c(new_n91_), .d(new_n85_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n90_), .c(new_n48_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n88_), .O(z09));
  inv1   g62(.a(x07), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand3  g64(.a(new_n91_), .b(new_n85_), .c(new_n84_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n65_), .c(new_n98_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n48_), .c(new_n61_), .d(x08), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n97_), .c(x04), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x06), .O(new_n103_));
  inv1   g69(.a(new_n48_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(x14), .c(x04), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n103_), .O(z10));
  nor2   g72(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n35_), .c(x03), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(new_n42_), .O(z12));
  and2   g75(.a(x17), .b(x04), .O(z14));
  buf    g76(.a(x04), .O(z13));
endmodule


