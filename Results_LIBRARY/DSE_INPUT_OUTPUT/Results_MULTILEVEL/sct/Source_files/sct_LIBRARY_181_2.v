// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x06), .O(new_n35_));
  nor2   g01(.a(x07), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n37_), .c(x16), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n35_), .c(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  inv1   g14(.a(new_n36_), .O(new_n49_));
  nand4  g15(.a(new_n46_), .b(x07), .c(new_n35_), .d(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n49_), .O(z03));
  inv1   g17(.a(x08), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x07), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n46_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(x07), .O(new_n57_));
  nor2   g23(.a(x09), .b(new_n52_), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n57_), .c(x06), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(x09), .c(new_n43_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n46_), .O(z05));
  inv1   g28(.a(new_n46_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n43_), .c(new_n49_), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x06), .O(new_n67_));
  oai21  g33(.a(new_n58_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x07), .O(new_n69_));
  nand2  g35(.a(x10), .b(new_n35_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(z06));
  nand2  g37(.a(new_n66_), .b(x08), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x11), .O(new_n73_));
  inv1   g39(.a(x09), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n74_), .c(x08), .d(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x07), .O(new_n78_));
  nand2  g44(.a(x11), .b(new_n35_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n64_), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  aoi21  g47(.a(new_n75_), .b(new_n58_), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n58_), .b(x06), .O(new_n83_));
  nor2   g49(.a(x12), .b(x11), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n65_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n82_), .c(x07), .O(new_n87_));
  nand2  g53(.a(x12), .b(new_n35_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n64_), .O(z08));
  inv1   g55(.a(x11), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n81_), .c(new_n90_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n72_), .c(x07), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  inv1   g60(.a(new_n60_), .O(new_n95_));
  nand3  g61(.a(new_n84_), .b(new_n66_), .c(new_n95_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(x13), .c(new_n43_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n94_), .c(new_n46_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nand4  g65(.a(new_n91_), .b(new_n81_), .c(new_n90_), .d(new_n65_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n46_), .c(new_n74_), .d(x08), .O(new_n102_));
  nand2  g68(.a(new_n63_), .b(x14), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n35_), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x07), .O(new_n105_));
  nand3  g71(.a(new_n63_), .b(x14), .c(new_n35_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n43_), .O(z10));
  nor2   g73(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g74(.a(x03), .O(new_n109_));
  inv1   g75(.a(x16), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(x02), .c(new_n109_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n49_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  nand2  g79(.a(new_n49_), .b(new_n43_), .O(z13));
  inv1   g80(.a(x17), .O(new_n115_));
  nor3   g81(.a(new_n36_), .b(new_n115_), .c(new_n43_), .O(z14));
endmodule


