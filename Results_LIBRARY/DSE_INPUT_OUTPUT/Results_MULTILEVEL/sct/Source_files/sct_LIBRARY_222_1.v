// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x07), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x02), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n36_), .c(x06), .O(z02));
  inv1   g15(.a(new_n37_), .O(new_n50_));
  nand4  g16(.a(new_n47_), .b(new_n36_), .c(x06), .d(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n50_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(x07), .c(x06), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(x07), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n47_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  nand2  g23(.a(new_n48_), .b(new_n50_), .O(new_n58_));
  inv1   g24(.a(x09), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(x08), .O(new_n60_));
  nor2   g26(.a(x09), .b(new_n53_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x07), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n60_), .c(x06), .O(new_n64_));
  nand2  g30(.a(x09), .b(new_n36_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n64_), .c(new_n58_), .O(z05));
  inv1   g32(.a(x06), .O(new_n67_));
  oai21  g33(.a(x10), .b(x07), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n62_), .b(x10), .O(new_n69_));
  nand2  g35(.a(x08), .b(x07), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n71_), .c(new_n42_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n69_), .c(new_n68_), .d(new_n47_), .O(z06));
  inv1   g40(.a(x11), .O(new_n75_));
  aoi21  g41(.a(new_n72_), .b(x08), .c(new_n75_), .O(new_n76_));
  nor4   g42(.a(new_n70_), .b(x11), .c(x10), .d(x09), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n76_), .c(x06), .O(new_n78_));
  nand2  g44(.a(x11), .b(new_n36_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n58_), .O(z07));
  oai21  g46(.a(x12), .b(x07), .c(new_n67_), .O(new_n81_));
  inv1   g47(.a(new_n77_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x12), .O(new_n83_));
  nor3   g49(.a(x12), .b(x11), .c(x10), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n63_), .c(new_n42_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n83_), .c(new_n81_), .d(new_n47_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  aoi21  g53(.a(new_n84_), .b(new_n61_), .c(new_n87_), .O(new_n88_));
  inv1   g54(.a(x10), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand4  g56(.a(new_n87_), .b(new_n90_), .c(new_n75_), .d(new_n89_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n62_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n88_), .c(x06), .O(new_n93_));
  nand2  g59(.a(x13), .b(new_n36_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n58_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n47_), .c(new_n59_), .d(x08), .O(new_n98_));
  nand3  g64(.a(new_n46_), .b(x16), .c(x14), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n36_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n50_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g69(.a(x16), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(x02), .c(new_n45_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n50_), .c(x04), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(z12));
  nand2  g73(.a(new_n50_), .b(new_n42_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  nor3   g75(.a(new_n37_), .b(new_n109_), .c(new_n42_), .O(z14));
endmodule


