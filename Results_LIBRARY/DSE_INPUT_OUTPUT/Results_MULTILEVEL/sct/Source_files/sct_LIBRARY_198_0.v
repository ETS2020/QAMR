// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  nor2   g00(.a(x05), .b(x03), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x15), .O(new_n41_));
  nand3  g07(.a(x18), .b(new_n41_), .c(x03), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  oai21  g09(.a(new_n40_), .b(x04), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  nand2  g14(.a(new_n47_), .b(x03), .O(new_n49_));
  oai21  g15(.a(new_n48_), .b(new_n40_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n45_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n45_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n50_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(new_n35_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  nand2  g33(.a(x05), .b(new_n36_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n46_), .c(new_n47_), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n67_), .O(z05));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(x08), .c(x07), .d(x06), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  aoi21  g40(.a(new_n65_), .b(x10), .c(new_n74_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n35_), .c(new_n70_), .O(z06));
  inv1   g42(.a(new_n48_), .O(new_n77_));
  nand2  g43(.a(new_n73_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x10), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(new_n62_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n64_), .O(new_n82_));
  and2   g48(.a(new_n82_), .b(new_n61_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n78_), .c(new_n77_), .d(x04), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  nand4  g51(.a(new_n72_), .b(new_n81_), .c(new_n85_), .d(new_n80_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n61_), .c(x04), .O(new_n87_));
  aoi21  g53(.a(new_n82_), .b(x12), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n77_), .O(z08));
  nand2  g55(.a(new_n86_), .b(x13), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n85_), .c(new_n80_), .d(new_n79_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n65_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n35_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n90_), .c(new_n77_), .d(x04), .O(z09));
  inv1   g61(.a(x04), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n50_), .c(new_n64_), .d(x08), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(x07), .c(x06), .O(new_n101_));
  nand2  g67(.a(new_n69_), .b(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n96_), .O(z10));
  nand2  g69(.a(new_n61_), .b(new_n36_), .O(z11));
  nand3  g70(.a(x16), .b(x05), .c(new_n36_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n46_), .c(new_n96_), .O(z12));
  nand2  g72(.a(new_n61_), .b(new_n96_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  nor3   g74(.a(new_n35_), .b(new_n108_), .c(new_n96_), .O(z14));
endmodule


