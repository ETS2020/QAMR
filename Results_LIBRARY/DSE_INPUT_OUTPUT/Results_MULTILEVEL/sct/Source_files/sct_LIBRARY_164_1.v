// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n115_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x00), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n41_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n38_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x06), .c(x00), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  xor2a  g23(.a(x08), .b(x07), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(x06), .c(x00), .O(new_n59_));
  oai21  g25(.a(new_n57_), .b(x06), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n49_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  oai21  g28(.a(x09), .b(x06), .c(new_n36_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nor2   g32(.a(new_n52_), .b(new_n48_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(x08), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(new_n45_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n65_), .c(new_n63_), .d(new_n49_), .O(z05));
  oai21  g37(.a(x10), .b(x06), .c(new_n36_), .O(new_n72_));
  nand2  g38(.a(new_n68_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x08), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n67_), .c(new_n45_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n73_), .c(new_n72_), .d(new_n49_), .O(z06));
  inv1   g44(.a(x03), .O(new_n79_));
  inv1   g45(.a(x16), .O(new_n80_));
  aoi21  g46(.a(new_n79_), .b(x02), .c(new_n80_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n45_), .c(new_n37_), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nor2   g49(.a(new_n57_), .b(new_n52_), .O(new_n84_));
  aoi21  g50(.a(new_n74_), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  inv1   g51(.a(x10), .O(new_n86_));
  nand3  g52(.a(new_n83_), .b(new_n86_), .c(new_n66_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(new_n64_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n85_), .c(x00), .O(new_n89_));
  nand2  g55(.a(x11), .b(new_n48_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n82_), .O(z07));
  oai21  g57(.a(x12), .b(x06), .c(new_n36_), .O(new_n92_));
  oai21  g58(.a(new_n87_), .b(new_n64_), .c(x12), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  inv1   g60(.a(new_n64_), .O(new_n95_));
  nand4  g61(.a(new_n74_), .b(new_n95_), .c(new_n94_), .d(new_n83_), .O(new_n96_));
  and2   g62(.a(new_n96_), .b(x04), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n93_), .c(new_n92_), .d(new_n49_), .O(z08));
  oai21  g64(.a(x13), .b(x06), .c(new_n36_), .O(new_n99_));
  nand2  g65(.a(new_n96_), .b(x13), .O(new_n100_));
  inv1   g66(.a(x13), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n94_), .c(new_n83_), .d(new_n86_), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n69_), .c(new_n45_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n100_), .c(new_n99_), .d(new_n49_), .O(z09));
  nand4  g71(.a(new_n49_), .b(new_n66_), .c(x08), .d(x07), .O(new_n106_));
  nand2  g72(.a(new_n81_), .b(x14), .O(new_n107_));
  oai21  g73(.a(new_n106_), .b(new_n48_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n37_), .O(z10));
  nor2   g76(.a(new_n41_), .b(new_n38_), .O(z11));
  aoi21  g77(.a(x16), .b(new_n38_), .c(x03), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(new_n45_), .c(new_n37_), .O(z12));
  nand2  g79(.a(new_n37_), .b(new_n45_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n37_), .O(z14));
endmodule


