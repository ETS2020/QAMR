// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n112_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(new_n36_), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  nand3  g14(.a(new_n36_), .b(x07), .c(new_n45_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n45_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand3  g19(.a(new_n36_), .b(x08), .c(new_n45_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n46_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  inv1   g23(.a(new_n46_), .O(new_n58_));
  nor2   g24(.a(x10), .b(x09), .O(new_n59_));
  oai22  g25(.a(new_n59_), .b(x06), .c(new_n58_), .d(new_n42_), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  and2   g27(.a(x08), .b(x07), .O(new_n62_));
  nand3  g28(.a(new_n61_), .b(x08), .c(x07), .O(new_n63_));
  oai21  g29(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nand3  g31(.a(new_n36_), .b(x09), .c(new_n45_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n65_), .c(new_n60_), .O(z05));
  inv1   g33(.a(new_n37_), .O(new_n68_));
  oai21  g34(.a(new_n58_), .b(new_n42_), .c(new_n68_), .O(new_n69_));
  aoi22  g35(.a(new_n63_), .b(x10), .c(new_n62_), .d(new_n59_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n45_), .c(new_n69_), .O(z06));
  oai21  g37(.a(x11), .b(x10), .c(new_n45_), .O(new_n72_));
  nand2  g38(.a(new_n62_), .b(new_n59_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x11), .O(new_n74_));
  nand2  g40(.a(new_n62_), .b(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n36_), .c(new_n61_), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n76_), .c(new_n42_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n74_), .c(new_n72_), .d(new_n46_), .O(z07));
  oai21  g47(.a(x12), .b(x10), .c(new_n45_), .O(new_n82_));
  inv1   g48(.a(new_n62_), .O(new_n83_));
  oai21  g49(.a(new_n78_), .b(new_n83_), .c(x12), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  nand3  g51(.a(new_n59_), .b(new_n85_), .c(new_n77_), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n76_), .c(new_n42_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n84_), .c(new_n82_), .d(new_n46_), .O(z08));
  oai21  g55(.a(x13), .b(x10), .c(new_n45_), .O(new_n90_));
  nand3  g56(.a(new_n85_), .b(new_n77_), .c(new_n36_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n63_), .c(x13), .O(new_n92_));
  nand4  g58(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n85_), .c(new_n77_), .d(new_n36_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(new_n42_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n92_), .c(new_n90_), .d(new_n46_), .O(z09));
  inv1   g64(.a(x07), .O(new_n99_));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n95_), .b(new_n100_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n46_), .c(new_n61_), .d(x08), .O(new_n102_));
  nand2  g68(.a(new_n58_), .b(x14), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n99_), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x06), .O(new_n105_));
  nand3  g71(.a(new_n58_), .b(x14), .c(new_n36_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n42_), .O(z10));
  nand2  g73(.a(new_n68_), .b(new_n35_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n35_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n42_), .c(new_n68_), .O(z12));
  nor2   g76(.a(new_n37_), .b(new_n42_), .O(z13));
  inv1   g77(.a(x17), .O(new_n112_));
  nor3   g78(.a(new_n37_), .b(new_n112_), .c(new_n42_), .O(z14));
endmodule


