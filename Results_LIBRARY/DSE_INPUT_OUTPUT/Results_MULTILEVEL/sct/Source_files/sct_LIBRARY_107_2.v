// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n113_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n36_), .O(z11));
  inv1   g05(.a(x06), .O(new_n40_));
  nand3  g06(.a(new_n37_), .b(new_n40_), .c(new_n35_), .O(new_n41_));
  oai21  g07(.a(z11), .b(new_n35_), .c(new_n41_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n38_), .O(z01));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n40_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n38_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand3  g18(.a(x14), .b(new_n52_), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x07), .O(new_n58_));
  nand3  g24(.a(x14), .b(x08), .c(new_n52_), .O(new_n59_));
  aoi21  g25(.a(new_n59_), .b(new_n58_), .c(new_n40_), .O(new_n60_));
  nor2   g26(.a(new_n57_), .b(x06), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n46_), .c(new_n38_), .O(z04));
  inv1   g29(.a(new_n49_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n46_), .c(new_n38_), .O(new_n65_));
  nor2   g31(.a(new_n57_), .b(new_n52_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n37_), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nor2   g35(.a(new_n52_), .b(new_n40_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x14), .c(new_n69_), .d(x08), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z05));
  nand3  g38(.a(new_n69_), .b(x08), .c(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x14), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x10), .O(new_n76_));
  nor2   g42(.a(new_n37_), .b(x10), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n66_), .c(new_n69_), .d(x06), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(new_n65_), .O(z06));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n66_), .c(new_n69_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x14), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x06), .O(new_n83_));
  aoi21  g49(.a(new_n78_), .b(x11), .c(new_n46_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n49_), .O(z07));
  nand2  g51(.a(new_n83_), .b(x12), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand3  g53(.a(new_n70_), .b(new_n69_), .c(x08), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n80_), .c(x14), .d(new_n87_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n86_), .c(new_n65_), .O(z08));
  inv1   g57(.a(x10), .O(new_n92_));
  inv1   g58(.a(x11), .O(new_n93_));
  nand3  g59(.a(new_n87_), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n73_), .c(x14), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x06), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x13), .O(new_n97_));
  nor2   g63(.a(new_n37_), .b(x13), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n89_), .c(new_n80_), .d(new_n87_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n97_), .c(new_n65_), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  inv1   g67(.a(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n87_), .c(new_n93_), .d(new_n92_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n101_), .c(x09), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(x08), .c(x07), .d(x06), .O(new_n105_));
  nand2  g71(.a(new_n64_), .b(x14), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n38_), .O(z10));
  aoi21  g75(.a(x16), .b(new_n36_), .c(x03), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n46_), .c(new_n38_), .O(z12));
  nand2  g77(.a(new_n38_), .b(new_n46_), .O(z13));
  nand2  g78(.a(new_n38_), .b(x17), .O(new_n113_));
  nor2   g79(.a(new_n113_), .b(new_n46_), .O(z14));
endmodule


