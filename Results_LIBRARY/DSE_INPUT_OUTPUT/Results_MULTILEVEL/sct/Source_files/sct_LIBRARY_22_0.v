// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n119_;
  nor2   g00(.a(x18), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(new_n35_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(x05), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x06), .O(new_n44_));
  nand2  g10(.a(x18), .b(x15), .O(new_n45_));
  oai21  g11(.a(x18), .b(new_n44_), .c(new_n45_), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(new_n43_), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(new_n42_), .O(z01));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(x04), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(x18), .c(x06), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n41_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n44_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n44_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n52_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n41_), .O(z04));
  inv1   g28(.a(x18), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x09), .c(new_n44_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n53_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  and2   g32(.a(x08), .b(x07), .O(new_n67_));
  nand3  g33(.a(new_n66_), .b(x08), .c(x07), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(new_n66_), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nand3  g36(.a(x18), .b(x09), .c(new_n44_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n65_), .O(z05));
  oai21  g38(.a(new_n63_), .b(x10), .c(new_n44_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n53_), .O(new_n74_));
  nand2  g40(.a(new_n68_), .b(x10), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand3  g42(.a(new_n67_), .b(new_n76_), .c(new_n66_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand3  g45(.a(x18), .b(x10), .c(new_n44_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n74_), .O(z06));
  oai21  g47(.a(new_n63_), .b(x11), .c(new_n44_), .O(new_n82_));
  nand2  g48(.a(new_n77_), .b(x11), .O(new_n83_));
  nand2  g49(.a(new_n67_), .b(x06), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nor3   g51(.a(x11), .b(x10), .c(x09), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n85_), .c(new_n40_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n83_), .c(new_n82_), .d(new_n52_), .O(z07));
  oai21  g54(.a(new_n63_), .b(x12), .c(new_n44_), .O(new_n89_));
  nand2  g55(.a(new_n86_), .b(new_n67_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x12), .O(new_n91_));
  inv1   g57(.a(x11), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(new_n76_), .d(new_n66_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n85_), .c(new_n40_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n91_), .c(new_n89_), .d(new_n52_), .O(z08));
  oai21  g63(.a(new_n63_), .b(x13), .c(new_n44_), .O(new_n98_));
  nand3  g64(.a(new_n93_), .b(new_n92_), .c(new_n76_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n68_), .c(x13), .O(new_n100_));
  nand4  g66(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n101_));
  inv1   g67(.a(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n93_), .c(new_n92_), .d(new_n76_), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n40_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n100_), .c(new_n98_), .d(new_n52_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  aoi21  g73(.a(new_n103_), .b(new_n107_), .c(new_n51_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n66_), .c(x08), .d(x07), .O(new_n109_));
  nand2  g75(.a(new_n51_), .b(x14), .O(new_n110_));
  oai21  g76(.a(new_n109_), .b(new_n44_), .c(new_n110_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n41_), .O(z10));
  nand2  g79(.a(new_n41_), .b(new_n36_), .O(z11));
  oai21  g80(.a(new_n50_), .b(x02), .c(new_n49_), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(new_n41_), .c(x04), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(z12));
  nor2   g83(.a(new_n35_), .b(new_n40_), .O(z13));
  nand2  g84(.a(x17), .b(x04), .O(new_n119_));
  nand2  g85(.a(new_n119_), .b(new_n41_), .O(z14));
endmodule


