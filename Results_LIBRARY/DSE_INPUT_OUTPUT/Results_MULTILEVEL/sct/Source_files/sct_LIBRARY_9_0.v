// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n110_;
  nor2   g00(.a(x16), .b(x12), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n35_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  nand3  g12(.a(x16), .b(new_n46_), .c(x02), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x12), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n35_), .O(new_n53_));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  aoi21  g20(.a(new_n46_), .b(x02), .c(new_n48_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(z03));
  xnor2a g24(.a(x08), .b(x07), .O(new_n59_));
  nand2  g25(.a(x08), .b(new_n45_), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n45_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n56_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n53_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n56_), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(x08), .c(x07), .d(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x04), .O(new_n73_));
  aoi21  g39(.a(new_n67_), .b(x10), .c(new_n73_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n35_), .c(new_n56_), .O(z06));
  inv1   g41(.a(new_n64_), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n66_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x04), .O(new_n79_));
  aoi21  g45(.a(new_n72_), .b(x11), .c(new_n79_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n35_), .c(new_n56_), .O(z07));
  oai21  g47(.a(x03), .b(new_n36_), .c(new_n49_), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand2  g49(.a(new_n78_), .b(x16), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g51(.a(new_n78_), .b(x12), .c(new_n42_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n82_), .O(z08));
  nand2  g53(.a(new_n53_), .b(new_n42_), .O(new_n88_));
  nand2  g54(.a(new_n78_), .b(x13), .O(new_n89_));
  inv1   g55(.a(new_n67_), .O(new_n90_));
  nor2   g56(.a(x13), .b(x12), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n77_), .c(new_n90_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n89_), .c(new_n46_), .d(x02), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x16), .O(new_n94_));
  nand2  g60(.a(x13), .b(x12), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n88_), .O(z09));
  nand2  g62(.a(new_n50_), .b(x00), .O(new_n97_));
  nor2   g63(.a(x10), .b(x03), .O(new_n98_));
  nor2   g64(.a(x12), .b(x11), .O(new_n99_));
  nor2   g65(.a(new_n48_), .b(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(x02), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n97_), .c(x09), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(x08), .c(x07), .d(x06), .O(new_n103_));
  nand2  g69(.a(new_n55_), .b(x14), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n42_), .O(z10));
  nand2  g71(.a(new_n53_), .b(new_n36_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n36_), .c(x03), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n42_), .c(new_n53_), .O(z12));
  nor2   g74(.a(new_n35_), .b(new_n42_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n53_), .O(z14));
endmodule


