// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n111_;
  nor2   g00(.a(x10), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n35_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  oai21  g10(.a(x03), .b(new_n36_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(x10), .c(x06), .O(z02));
  inv1   g13(.a(x06), .O(new_n48_));
  nand3  g14(.a(x10), .b(x07), .c(new_n48_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand3  g19(.a(x10), .b(x08), .c(new_n48_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n45_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  inv1   g23(.a(x10), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(x09), .c(new_n48_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n46_), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  and2   g27(.a(x08), .b(x07), .O(new_n62_));
  nand3  g28(.a(new_n61_), .b(x08), .c(x07), .O(new_n63_));
  oai21  g29(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nand3  g31(.a(x10), .b(x09), .c(new_n48_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n65_), .c(new_n60_), .O(z05));
  nand2  g33(.a(new_n63_), .b(x10), .O(new_n68_));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n62_), .c(new_n48_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n45_), .d(x04), .O(z06));
  inv1   g37(.a(new_n35_), .O(new_n72_));
  nand2  g38(.a(new_n46_), .b(new_n72_), .O(new_n73_));
  inv1   g39(.a(x11), .O(new_n74_));
  inv1   g40(.a(new_n63_), .O(new_n75_));
  nand4  g41(.a(new_n62_), .b(new_n74_), .c(new_n58_), .d(new_n61_), .O(new_n76_));
  oai21  g42(.a(new_n75_), .b(new_n74_), .c(new_n76_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x06), .O(new_n78_));
  nand2  g44(.a(x11), .b(x10), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n73_), .O(z07));
  oai21  g46(.a(x12), .b(new_n58_), .c(new_n48_), .O(new_n81_));
  nand2  g47(.a(new_n76_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n75_), .b(new_n83_), .c(new_n74_), .d(new_n58_), .O(new_n84_));
  and2   g50(.a(new_n84_), .b(x04), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n82_), .c(new_n81_), .d(new_n45_), .O(z08));
  oai21  g52(.a(x13), .b(new_n58_), .c(new_n48_), .O(new_n87_));
  nand2  g53(.a(new_n84_), .b(x13), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n83_), .c(new_n74_), .d(new_n58_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n75_), .c(new_n42_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n88_), .c(new_n87_), .d(new_n45_), .O(z09));
  inv1   g59(.a(x07), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n45_), .c(new_n61_), .d(x08), .O(new_n97_));
  inv1   g63(.a(new_n45_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x14), .O(new_n99_));
  oai21  g65(.a(new_n97_), .b(new_n94_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x06), .O(new_n101_));
  nand3  g67(.a(new_n98_), .b(x14), .c(x10), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n42_), .O(z10));
  nand2  g69(.a(new_n72_), .b(new_n36_), .O(z11));
  inv1   g70(.a(x03), .O(new_n105_));
  inv1   g71(.a(x16), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(x02), .c(new_n105_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n72_), .c(x04), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(z12));
  nor2   g75(.a(new_n35_), .b(new_n42_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n72_), .O(z14));
endmodule


