// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n118_;
  nor2   g00(.a(x18), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n35_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(x18), .c(new_n45_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand3  g17(.a(x18), .b(x07), .c(new_n45_), .O(new_n52_));
  oai21  g18(.a(x07), .b(new_n45_), .c(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand3  g22(.a(x18), .b(x08), .c(new_n45_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n45_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n49_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  oai21  g26(.a(new_n40_), .b(x09), .c(new_n45_), .O(new_n61_));
  nand2  g27(.a(new_n49_), .b(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  and2   g30(.a(x08), .b(x07), .O(new_n65_));
  nand3  g31(.a(new_n64_), .b(x08), .c(x07), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(x18), .b(x09), .c(new_n45_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(z05));
  oai21  g36(.a(new_n40_), .b(x10), .c(new_n45_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n62_), .O(new_n72_));
  nand2  g38(.a(new_n66_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand3  g40(.a(new_n65_), .b(new_n74_), .c(new_n64_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand3  g43(.a(x18), .b(x10), .c(new_n45_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n72_), .O(z06));
  oai21  g45(.a(new_n40_), .b(x11), .c(new_n45_), .O(new_n80_));
  nand2  g46(.a(new_n75_), .b(x11), .O(new_n81_));
  nand2  g47(.a(new_n65_), .b(x06), .O(new_n82_));
  inv1   g48(.a(new_n82_), .O(new_n83_));
  nor3   g49(.a(x11), .b(x10), .c(x09), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n83_), .c(new_n42_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n81_), .c(new_n80_), .d(new_n49_), .O(z07));
  oai21  g52(.a(new_n40_), .b(x12), .c(new_n45_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  aoi21  g55(.a(new_n84_), .b(new_n65_), .c(new_n89_), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  nand3  g57(.a(new_n89_), .b(new_n91_), .c(new_n74_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n66_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n90_), .c(x06), .O(new_n94_));
  nand3  g60(.a(x18), .b(x12), .c(new_n45_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n88_), .O(z08));
  oai21  g62(.a(new_n40_), .b(x13), .c(new_n45_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n62_), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n89_), .c(new_n91_), .d(new_n74_), .O(new_n100_));
  oai22  g66(.a(new_n100_), .b(new_n66_), .c(new_n93_), .d(new_n99_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x06), .O(new_n102_));
  nand3  g68(.a(x18), .b(x13), .c(new_n45_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n102_), .c(new_n98_), .O(z09));
  inv1   g70(.a(new_n35_), .O(new_n105_));
  inv1   g71(.a(x00), .O(new_n106_));
  aoi21  g72(.a(new_n100_), .b(new_n106_), .c(new_n48_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n64_), .c(x08), .d(x07), .O(new_n108_));
  nand2  g74(.a(new_n48_), .b(x14), .O(new_n109_));
  oai21  g75(.a(new_n108_), .b(new_n45_), .c(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n105_), .O(z10));
  nor2   g78(.a(new_n35_), .b(new_n36_), .O(z11));
  oai21  g79(.a(new_n47_), .b(x02), .c(new_n46_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n105_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  nor2   g82(.a(new_n35_), .b(new_n42_), .O(z13));
  nand2  g83(.a(x17), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n105_), .O(z14));
endmodule


