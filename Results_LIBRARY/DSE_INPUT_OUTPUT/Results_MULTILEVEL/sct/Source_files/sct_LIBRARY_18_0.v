// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n117_;
  nor2   g00(.a(x12), .b(x07), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n35_), .O(z01));
  inv1   g11(.a(new_n35_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n46_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nand3  g19(.a(x12), .b(new_n53_), .c(x06), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(x06), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n50_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  xnor2a g23(.a(x08), .b(x06), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n53_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n46_), .O(z04));
  inv1   g28(.a(x12), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x09), .c(new_n53_), .O(new_n64_));
  nand2  g30(.a(x08), .b(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nor2   g32(.a(new_n53_), .b(new_n47_), .O(new_n67_));
  inv1   g33(.a(x08), .O(new_n68_));
  nor2   g34(.a(x09), .b(new_n68_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n67_), .c(new_n43_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n66_), .c(new_n64_), .d(new_n50_), .O(z05));
  oai21  g37(.a(new_n63_), .b(x10), .c(new_n53_), .O(new_n72_));
  nand2  g38(.a(new_n50_), .b(x04), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  nand2  g41(.a(new_n69_), .b(x06), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  inv1   g43(.a(x09), .O(new_n78_));
  inv1   g44(.a(new_n65_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n75_), .c(new_n78_), .O(new_n80_));
  oai21  g46(.a(new_n77_), .b(new_n75_), .c(new_n80_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x07), .O(new_n82_));
  nand3  g48(.a(x12), .b(x10), .c(new_n53_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n74_), .O(z06));
  oai21  g50(.a(new_n63_), .b(x11), .c(new_n53_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n73_), .O(new_n86_));
  and2   g52(.a(new_n80_), .b(x11), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  nand4  g54(.a(new_n79_), .b(new_n88_), .c(new_n75_), .d(new_n78_), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n87_), .c(x07), .O(new_n91_));
  nand3  g57(.a(x12), .b(x11), .c(new_n53_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n86_), .O(z07));
  nand2  g59(.a(new_n89_), .b(x12), .O(new_n94_));
  nor3   g60(.a(x12), .b(x11), .c(x10), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n77_), .c(new_n53_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n94_), .c(new_n50_), .d(x04), .O(z08));
  nand2  g63(.a(new_n73_), .b(new_n46_), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nor2   g65(.a(new_n90_), .b(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n99_), .b(new_n63_), .c(new_n88_), .d(new_n75_), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n76_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n100_), .c(x07), .O(new_n103_));
  nand2  g69(.a(x13), .b(x12), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n103_), .c(new_n98_), .O(z09));
  nand2  g71(.a(x07), .b(x00), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n101_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n50_), .c(new_n78_), .d(x08), .O(new_n108_));
  nand3  g74(.a(new_n49_), .b(x16), .c(x14), .O(new_n109_));
  oai21  g75(.a(new_n108_), .b(new_n47_), .c(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n46_), .O(z10));
  nor2   g78(.a(new_n35_), .b(new_n36_), .O(z11));
  aoi21  g79(.a(x16), .b(new_n36_), .c(x03), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(new_n43_), .c(new_n46_), .O(z12));
  nor2   g81(.a(new_n35_), .b(new_n43_), .O(z13));
  inv1   g82(.a(x17), .O(new_n117_));
  nor3   g83(.a(new_n35_), .b(new_n117_), .c(new_n43_), .O(z14));
endmodule


