// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n112_, new_n113_, new_n115_;
  nand2  g00(.a(x16), .b(x06), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n35_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(x04), .c(x16), .O(new_n54_));
  nand4  g20(.a(new_n50_), .b(x07), .c(new_n46_), .d(x04), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n46_), .c(new_n55_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n50_), .b(x08), .c(new_n46_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n46_), .c(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n35_), .O(z04));
  nand2  g27(.a(new_n35_), .b(new_n43_), .O(z13));
  oai21  g28(.a(new_n49_), .b(x09), .c(new_n46_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  and2   g30(.a(x08), .b(x07), .O(new_n65_));
  nor2   g31(.a(new_n53_), .b(new_n46_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(x08), .O(new_n67_));
  oai21  g33(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n48_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n63_), .c(z13), .O(z05));
  oai21  g36(.a(new_n49_), .b(x10), .c(new_n46_), .O(new_n71_));
  nand3  g37(.a(new_n64_), .b(x08), .c(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand4  g40(.a(new_n66_), .b(new_n74_), .c(new_n64_), .d(x08), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n48_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n71_), .c(z13), .O(z06));
  nor3   g44(.a(x11), .b(x10), .c(x09), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n65_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n48_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n47_), .c(x02), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x16), .O(new_n85_));
  aoi21  g51(.a(new_n75_), .b(x11), .c(new_n43_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n82_), .O(z07));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n83_), .c(new_n74_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n72_), .c(new_n48_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand3  g57(.a(new_n88_), .b(new_n47_), .c(x02), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x16), .O(new_n93_));
  nand3  g59(.a(new_n79_), .b(new_n65_), .c(x06), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x12), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n93_), .c(new_n91_), .d(x04), .O(z08));
  oai21  g62(.a(new_n49_), .b(x13), .c(new_n46_), .O(new_n97_));
  oai21  g63(.a(new_n89_), .b(new_n72_), .c(x13), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n88_), .c(new_n83_), .d(new_n74_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n67_), .c(new_n98_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n97_), .c(z13), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  aoi21  g70(.a(new_n100_), .b(new_n104_), .c(x09), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(x08), .c(x07), .d(x06), .O(new_n106_));
  nand2  g72(.a(new_n49_), .b(x14), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n35_), .O(z10));
  nor2   g76(.a(new_n36_), .b(new_n37_), .O(z11));
  nand2  g77(.a(new_n35_), .b(x03), .O(new_n112_));
  nand3  g78(.a(x16), .b(new_n46_), .c(new_n37_), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n112_), .c(new_n43_), .O(z12));
  nand3  g80(.a(new_n35_), .b(x17), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z14));
endmodule


