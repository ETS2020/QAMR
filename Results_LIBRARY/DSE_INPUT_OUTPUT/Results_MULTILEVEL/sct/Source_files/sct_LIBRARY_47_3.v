// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n111_, new_n114_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x11), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(x15), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  inv1   g07(.a(x01), .O(new_n42_));
  nor2   g08(.a(new_n36_), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n40_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n43_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n46_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n47_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n47_), .c(new_n55_), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n48_), .c(new_n46_), .d(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  and2   g27(.a(x07), .b(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(x08), .O(new_n63_));
  and2   g29(.a(new_n63_), .b(new_n46_), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n60_), .c(new_n48_), .d(x04), .O(z05));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n62_), .c(x08), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n46_), .c(x04), .O(new_n68_));
  aoi21  g34(.a(new_n63_), .b(x10), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n48_), .O(z06));
  nand2  g36(.a(new_n66_), .b(x08), .O(new_n71_));
  nand2  g37(.a(new_n62_), .b(new_n42_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n71_), .c(x11), .O(new_n73_));
  inv1   g39(.a(new_n59_), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n61_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n73_), .c(new_n48_), .d(x04), .O(z07));
  oai21  g43(.a(x12), .b(x01), .c(x11), .O(new_n78_));
  nand2  g44(.a(new_n67_), .b(x12), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  nand3  g46(.a(new_n66_), .b(new_n80_), .c(new_n36_), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n74_), .c(new_n41_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n79_), .c(new_n78_), .d(new_n48_), .O(z08));
  inv1   g50(.a(new_n48_), .O(new_n85_));
  oai21  g51(.a(x13), .b(x01), .c(x11), .O(new_n86_));
  oai21  g52(.a(new_n85_), .b(new_n41_), .c(new_n86_), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nor3   g54(.a(x12), .b(x10), .c(x09), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n74_), .c(new_n88_), .O(new_n90_));
  nand3  g56(.a(new_n66_), .b(new_n88_), .c(new_n80_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n59_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n90_), .c(new_n36_), .O(new_n93_));
  nand3  g59(.a(x13), .b(x11), .c(new_n42_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n87_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  inv1   g62(.a(x03), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x02), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x16), .c(x09), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(x08), .c(x07), .d(x06), .O(new_n100_));
  nand2  g66(.a(new_n85_), .b(x14), .O(new_n101_));
  oai21  g67(.a(new_n100_), .b(new_n96_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n46_), .O(new_n103_));
  nand4  g69(.a(new_n48_), .b(new_n88_), .c(new_n80_), .d(new_n36_), .O(new_n104_));
  nor3   g70(.a(new_n104_), .b(x10), .c(x09), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(x08), .c(x07), .d(x06), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n103_), .c(new_n41_), .O(z10));
  nand2  g73(.a(new_n46_), .b(new_n35_), .O(z11));
  inv1   g74(.a(x16), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(x02), .c(new_n97_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(new_n46_), .c(x04), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(z12));
  nand2  g78(.a(new_n46_), .b(new_n41_), .O(z13));
  nand2  g79(.a(x17), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n46_), .O(z14));
endmodule


