// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n118_;
  nor2   g00(.a(x15), .b(x06), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  or2    g02(.a(x14), .b(x01), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(z00));
  aoi21  g06(.a(x06), .b(x05), .c(x15), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(x06), .c(x15), .O(new_n43_));
  oai22  g09(.a(new_n43_), .b(x05), .c(new_n41_), .d(x04), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x16), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x02), .c(new_n46_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x15), .c(x06), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(z03));
  inv1   g19(.a(x06), .O(new_n54_));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand3  g21(.a(x15), .b(x08), .c(new_n54_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  inv1   g25(.a(x15), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(x09), .c(new_n54_), .O(new_n61_));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x04), .O(new_n64_));
  and2   g30(.a(x07), .b(x06), .O(new_n65_));
  inv1   g31(.a(x08), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n66_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n63_), .c(new_n61_), .d(new_n48_), .O(z05));
  oai21  g35(.a(new_n60_), .b(x10), .c(new_n54_), .O(new_n70_));
  nand2  g36(.a(new_n67_), .b(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n65_), .c(new_n64_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n72_), .c(new_n70_), .d(new_n48_), .O(z06));
  oai21  g43(.a(new_n60_), .b(x11), .c(new_n54_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n49_), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(new_n62_), .O(new_n81_));
  aoi21  g47(.a(new_n73_), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  nor4   g48(.a(new_n62_), .b(x11), .c(x10), .d(x09), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n82_), .c(x06), .O(new_n84_));
  nand3  g50(.a(x15), .b(x11), .c(new_n54_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n79_), .O(z07));
  oai21  g52(.a(new_n60_), .b(x12), .c(new_n54_), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  or2    g54(.a(new_n83_), .b(new_n88_), .O(new_n89_));
  nand3  g55(.a(new_n73_), .b(new_n88_), .c(new_n80_), .O(new_n90_));
  nor3   g56(.a(new_n90_), .b(new_n62_), .c(new_n54_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n64_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n89_), .c(new_n87_), .d(new_n48_), .O(z08));
  oai21  g59(.a(new_n60_), .b(x13), .c(new_n54_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n49_), .O(new_n95_));
  inv1   g61(.a(x10), .O(new_n96_));
  nand3  g62(.a(new_n88_), .b(new_n80_), .c(new_n96_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n71_), .c(x13), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n88_), .c(new_n80_), .d(new_n96_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n71_), .c(new_n98_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x06), .O(new_n102_));
  nand3  g68(.a(x15), .b(x13), .c(new_n54_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n102_), .c(new_n95_), .O(z09));
  inv1   g70(.a(x09), .O(new_n105_));
  inv1   g71(.a(x00), .O(new_n106_));
  aoi21  g72(.a(new_n100_), .b(new_n106_), .c(new_n47_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n105_), .c(x08), .d(x07), .O(new_n108_));
  nand2  g74(.a(new_n47_), .b(x14), .O(new_n109_));
  oai21  g75(.a(new_n108_), .b(new_n54_), .c(new_n109_), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n36_), .O(z10));
  nor2   g78(.a(new_n35_), .b(new_n38_), .O(z11));
  oai21  g79(.a(new_n46_), .b(x02), .c(new_n45_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n36_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  nor2   g82(.a(new_n35_), .b(new_n64_), .O(z13));
  nand2  g83(.a(x17), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n36_), .O(z14));
endmodule


