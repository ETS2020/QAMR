// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(x09), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n45_), .c(x04), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(new_n36_), .O(z02));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n36_), .O(z03));
  xnor2a g17(.a(x08), .b(x07), .O(new_n52_));
  nand2  g18(.a(x08), .b(new_n45_), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(new_n45_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n46_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n36_), .O(z04));
  inv1   g22(.a(x09), .O(new_n57_));
  and2   g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x06), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n46_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  oai21  g27(.a(new_n57_), .b(x06), .c(new_n61_), .O(z05));
  inv1   g28(.a(new_n46_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n42_), .c(new_n36_), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  nand2  g31(.a(x07), .b(x06), .O(new_n66_));
  nand2  g32(.a(new_n65_), .b(x08), .O(new_n67_));
  oai22  g33(.a(new_n67_), .b(new_n66_), .c(new_n58_), .d(new_n65_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n57_), .O(new_n69_));
  nand2  g35(.a(x10), .b(new_n45_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(z06));
  inv1   g37(.a(x07), .O(new_n72_));
  oai21  g38(.a(new_n67_), .b(new_n72_), .c(x11), .O(new_n73_));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x08), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n66_), .c(new_n73_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n57_), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n45_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n64_), .O(z07));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(x12), .O(new_n81_));
  nand4  g47(.a(new_n58_), .b(new_n81_), .c(new_n80_), .d(new_n65_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n57_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nand4  g50(.a(new_n74_), .b(new_n58_), .c(new_n57_), .d(x06), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(x12), .c(new_n42_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n46_), .O(z08));
  and2   g53(.a(new_n82_), .b(x13), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n81_), .c(new_n80_), .d(new_n65_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n59_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n88_), .c(new_n57_), .O(new_n92_));
  nand2  g58(.a(x13), .b(new_n45_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n64_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n46_), .c(x08), .d(x07), .O(new_n97_));
  nand2  g63(.a(new_n63_), .b(x14), .O(new_n98_));
  oai21  g64(.a(new_n97_), .b(new_n45_), .c(new_n98_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x04), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n36_), .O(z10));
  nor2   g67(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g68(.a(x03), .O(new_n103_));
  inv1   g69(.a(x16), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(x02), .c(new_n103_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n36_), .c(x04), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(z12));
  nor2   g73(.a(new_n37_), .b(new_n42_), .O(z13));
  nand3  g74(.a(new_n36_), .b(x17), .c(x04), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(z14));
endmodule


