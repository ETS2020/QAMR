// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(x13), .b(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x02), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n36_), .c(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  inv1   g15(.a(x07), .O(new_n50_));
  nand3  g16(.a(x13), .b(new_n50_), .c(x06), .O(new_n51_));
  oai21  g17(.a(new_n50_), .b(x06), .c(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  xor2a  g21(.a(x08), .b(x07), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(x13), .c(x06), .O(new_n57_));
  oai21  g23(.a(new_n55_), .b(x06), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n47_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  nor2   g26(.a(x09), .b(new_n55_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x07), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x13), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x06), .O(new_n64_));
  nor2   g30(.a(new_n50_), .b(new_n36_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x13), .c(x08), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(x09), .c(new_n42_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n64_), .c(new_n47_), .O(z05));
  inv1   g34(.a(new_n37_), .O(new_n69_));
  inv1   g35(.a(new_n47_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n42_), .c(new_n69_), .O(new_n71_));
  nand2  g37(.a(new_n64_), .b(x10), .O(new_n72_));
  inv1   g38(.a(x09), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand3  g40(.a(x08), .b(x07), .c(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(x13), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n72_), .c(new_n71_), .O(z06));
  nand2  g44(.a(x08), .b(x07), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n73_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n79_), .c(x13), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x06), .O(new_n83_));
  aoi21  g49(.a(new_n77_), .b(x11), .c(new_n42_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n47_), .O(z07));
  nand2  g51(.a(new_n83_), .b(x12), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x12), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n80_), .c(new_n65_), .d(new_n61_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n86_), .c(new_n71_), .O(z08));
  nor2   g56(.a(x12), .b(x11), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n74_), .c(new_n73_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n75_), .c(x13), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n71_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  aoi21  g61(.a(new_n46_), .b(x16), .c(x09), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  nand2  g63(.a(new_n70_), .b(x14), .O(new_n98_));
  oai21  g64(.a(new_n97_), .b(new_n95_), .c(new_n98_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x13), .O(new_n100_));
  nand3  g66(.a(new_n70_), .b(x14), .c(new_n36_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n42_), .O(z10));
  nand2  g68(.a(new_n69_), .b(new_n35_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n35_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n42_), .c(new_n69_), .O(z12));
  nor2   g71(.a(new_n37_), .b(new_n42_), .O(z13));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n69_), .O(z14));
endmodule


