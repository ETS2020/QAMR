// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n120_;
  nor2   g00(.a(x15), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  aoi21  g05(.a(x06), .b(x05), .c(x15), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(x06), .c(x15), .O(new_n42_));
  oai22  g08(.a(new_n42_), .b(x05), .c(new_n40_), .d(x04), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n36_), .c(x16), .O(new_n45_));
  nand4  g11(.a(new_n45_), .b(x15), .c(new_n44_), .d(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z02));
  inv1   g13(.a(new_n35_), .O(new_n48_));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n45_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(z03));
  xnor2a g17(.a(x08), .b(x07), .O(new_n52_));
  nand3  g18(.a(x15), .b(x08), .c(new_n44_), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(new_n44_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n45_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(x15), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(x09), .c(new_n44_), .O(new_n58_));
  and2   g24(.a(x08), .b(x07), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x04), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  inv1   g29(.a(x07), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n44_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(x08), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n61_), .c(new_n58_), .d(new_n45_), .O(z05));
  oai21  g35(.a(new_n57_), .b(x10), .c(new_n44_), .O(new_n70_));
  nand3  g36(.a(new_n63_), .b(x08), .c(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n65_), .c(new_n62_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n72_), .c(new_n70_), .d(new_n45_), .O(z06));
  oai21  g43(.a(new_n57_), .b(x11), .c(new_n44_), .O(new_n78_));
  nand2  g44(.a(new_n73_), .b(new_n59_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  nand2  g46(.a(new_n59_), .b(x06), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  inv1   g48(.a(x10), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n63_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n82_), .c(new_n62_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n80_), .c(new_n78_), .d(new_n45_), .O(z07));
  oai21  g54(.a(new_n57_), .b(x12), .c(new_n44_), .O(new_n89_));
  oai21  g55(.a(new_n85_), .b(new_n60_), .c(x12), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand3  g57(.a(new_n73_), .b(new_n91_), .c(new_n84_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n82_), .c(new_n62_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(new_n45_), .O(z08));
  oai21  g61(.a(new_n57_), .b(x13), .c(new_n44_), .O(new_n96_));
  nand3  g62(.a(new_n91_), .b(new_n84_), .c(new_n83_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n71_), .c(x13), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n91_), .c(new_n84_), .d(new_n83_), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n67_), .c(new_n62_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n98_), .c(new_n96_), .d(new_n45_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n100_), .b(new_n104_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n45_), .c(new_n63_), .d(x08), .O(new_n106_));
  inv1   g72(.a(new_n45_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x14), .O(new_n108_));
  oai21  g74(.a(new_n106_), .b(new_n64_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x06), .O(new_n110_));
  nand3  g76(.a(new_n107_), .b(x15), .c(x14), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n62_), .O(z10));
  nand2  g78(.a(new_n48_), .b(new_n36_), .O(z11));
  inv1   g79(.a(x03), .O(new_n114_));
  inv1   g80(.a(x16), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(x02), .c(new_n114_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n48_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  nand2  g84(.a(new_n48_), .b(new_n62_), .O(z13));
  nand2  g85(.a(x17), .b(x04), .O(new_n120_));
  nand2  g86(.a(new_n120_), .b(new_n48_), .O(z14));
endmodule


