// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n111_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x14), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x07), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(x14), .b(x07), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  aoi21  g05(.a(new_n37_), .b(x01), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(x01), .c(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n39_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n37_), .c(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n38_), .c(new_n48_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand3  g17(.a(new_n35_), .b(x07), .c(new_n48_), .O(new_n52_));
  oai21  g18(.a(x07), .b(new_n48_), .c(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xor2a  g21(.a(x08), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n35_), .c(x07), .O(new_n57_));
  inv1   g23(.a(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n49_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n35_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x07), .O(new_n66_));
  nor2   g32(.a(new_n58_), .b(new_n48_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n35_), .c(x08), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n66_), .c(new_n49_), .d(x04), .O(z05));
  nand2  g36(.a(x08), .b(x06), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n63_), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n71_), .c(new_n35_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x07), .O(new_n75_));
  nand4  g41(.a(new_n67_), .b(new_n35_), .c(new_n63_), .d(x08), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n75_), .c(new_n49_), .d(x04), .O(z06));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n71_), .c(new_n35_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x07), .O(new_n82_));
  nand3  g48(.a(x08), .b(x07), .c(x06), .O(new_n83_));
  nand3  g49(.a(new_n35_), .b(new_n72_), .c(new_n63_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n83_), .c(x11), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n82_), .c(new_n49_), .d(x04), .O(z07));
  inv1   g52(.a(new_n49_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n45_), .c(new_n38_), .O(new_n88_));
  nand2  g54(.a(new_n82_), .b(x12), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand3  g56(.a(new_n79_), .b(new_n35_), .c(new_n90_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n67_), .c(new_n63_), .d(x08), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n89_), .c(new_n88_), .O(z08));
  inv1   g60(.a(x11), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n90_), .c(new_n95_), .d(new_n72_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n64_), .c(new_n35_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x07), .O(new_n99_));
  nand2  g65(.a(new_n93_), .b(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n99_), .c(new_n49_), .d(x04), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n97_), .b(new_n102_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n49_), .c(new_n35_), .d(new_n63_), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(x08), .c(x07), .d(x06), .O(new_n106_));
  nand3  g72(.a(new_n87_), .b(x14), .c(new_n58_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n45_), .O(z10));
  nand2  g74(.a(new_n38_), .b(new_n37_), .O(z11));
  inv1   g75(.a(x03), .O(new_n110_));
  inv1   g76(.a(x16), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(x02), .c(new_n110_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n38_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  nor2   g80(.a(new_n39_), .b(new_n45_), .O(z13));
  nand2  g81(.a(x17), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n38_), .O(z14));
endmodule


