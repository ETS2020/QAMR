// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x12), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  aoi21  g12(.a(x05), .b(new_n46_), .c(new_n37_), .O(new_n47_));
  oai21  g13(.a(new_n45_), .b(x05), .c(new_n47_), .O(z01));
  oai21  g14(.a(x03), .b(new_n38_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n35_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n37_), .O(new_n52_));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n52_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n36_), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n56_), .b(x06), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n49_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x08), .c(x07), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n36_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nand4  g31(.a(new_n36_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(x09), .c(new_n46_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n49_), .O(z05));
  nand2  g34(.a(new_n49_), .b(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n52_), .O(new_n70_));
  nand2  g36(.a(new_n65_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(x07), .O(new_n73_));
  nor2   g39(.a(new_n56_), .b(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n36_), .c(new_n72_), .d(new_n62_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n71_), .c(new_n70_), .O(z06));
  nor2   g44(.a(x10), .b(x09), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n74_), .c(x12), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n35_), .c(x11), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand4  g48(.a(new_n79_), .b(new_n76_), .c(new_n36_), .d(new_n82_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n70_), .O(z07));
  nand4  g50(.a(new_n76_), .b(new_n82_), .c(new_n72_), .d(new_n62_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n49_), .c(new_n36_), .d(x04), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n82_), .c(new_n72_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n63_), .c(new_n36_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x06), .O(new_n90_));
  nand2  g56(.a(new_n83_), .b(x13), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n49_), .d(x04), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n49_), .c(new_n62_), .d(x08), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n73_), .O(new_n96_));
  nor2   g62(.a(new_n49_), .b(new_n41_), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(x06), .c(new_n97_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n46_), .c(new_n52_), .O(z10));
  nor2   g65(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n38_), .c(x03), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n46_), .c(new_n52_), .O(z12));
  nor2   g68(.a(new_n37_), .b(new_n46_), .O(z13));
  nand2  g69(.a(x17), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n52_), .O(z14));
endmodule


