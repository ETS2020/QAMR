// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n98_, new_n100_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nor2   g02(.a(x08), .b(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(new_n37_), .O(new_n40_));
  inv1   g06(.a(x08), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(new_n36_), .c(x05), .O(new_n42_));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n42_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(x08), .c(new_n47_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  aoi21  g20(.a(new_n54_), .b(x08), .c(new_n36_), .O(z03));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n50_), .c(x08), .d(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  nand2  g24(.a(x07), .b(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x07), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n50_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x08), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x04), .O(z05));
  nand2  g31(.a(new_n62_), .b(x10), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  inv1   g33(.a(new_n59_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n61_), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(x08), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n66_), .c(new_n50_), .d(x04), .O(z06));
  nand2  g37(.a(new_n69_), .b(x11), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand4  g39(.a(new_n68_), .b(new_n73_), .c(new_n67_), .d(new_n61_), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(x08), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n50_), .d(x04), .O(z07));
  nand2  g42(.a(new_n74_), .b(x12), .O(new_n77_));
  inv1   g43(.a(new_n62_), .O(new_n78_));
  nor3   g44(.a(x12), .b(x11), .c(x10), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n78_), .c(new_n41_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n77_), .c(new_n50_), .d(x04), .O(z08));
  nand2  g47(.a(new_n79_), .b(new_n78_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x13), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  inv1   g50(.a(x13), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n73_), .d(new_n67_), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n78_), .c(new_n41_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n83_), .c(new_n50_), .d(x04), .O(z09));
  inv1   g55(.a(x00), .O(new_n90_));
  nand2  g56(.a(new_n86_), .b(new_n90_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n50_), .c(new_n61_), .d(x07), .O(new_n92_));
  nand3  g58(.a(new_n49_), .b(x16), .c(x14), .O(new_n93_));
  oai21  g59(.a(new_n92_), .b(new_n47_), .c(new_n93_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(x08), .c(x04), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(z10));
  nor2   g62(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n35_), .c(new_n41_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n48_), .c(new_n36_), .O(z12));
  inv1   g65(.a(x17), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x08), .c(new_n36_), .O(z14));
  buf    g67(.a(x04), .O(z13));
endmodule


