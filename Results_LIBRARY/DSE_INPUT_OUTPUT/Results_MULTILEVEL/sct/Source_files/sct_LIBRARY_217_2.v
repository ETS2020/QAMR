// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n117_, new_n118_, new_n120_;
  inv1   g00(.a(x11), .O(new_n35_));
  nor2   g01(.a(x16), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n36_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n37_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x03), .O(new_n52_));
  nand3  g18(.a(x16), .b(new_n52_), .c(x02), .O(new_n53_));
  oai21  g19(.a(x16), .b(x11), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n47_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n47_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n54_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(new_n36_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n63_), .c(new_n48_), .d(x04), .O(z05));
  nand2  g34(.a(new_n65_), .b(x10), .O(new_n69_));
  inv1   g35(.a(x08), .O(new_n70_));
  nand2  g36(.a(x07), .b(x06), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n64_), .O(new_n73_));
  nor3   g39(.a(new_n73_), .b(new_n71_), .c(new_n70_), .O(new_n74_));
  nor2   g40(.a(new_n74_), .b(new_n36_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n69_), .c(new_n48_), .d(x04), .O(z06));
  nand2  g42(.a(new_n46_), .b(new_n43_), .O(z13));
  nor2   g43(.a(x03), .b(new_n37_), .O(new_n78_));
  oai21  g44(.a(new_n74_), .b(new_n35_), .c(new_n78_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x16), .O(new_n80_));
  inv1   g46(.a(new_n62_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n35_), .c(new_n72_), .d(new_n64_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(z13), .O(z07));
  inv1   g49(.a(x16), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(x12), .c(x11), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n43_), .O(new_n86_));
  nand2  g52(.a(x12), .b(x11), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n52_), .c(x02), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x16), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nor2   g56(.a(new_n74_), .b(new_n90_), .O(new_n91_));
  nor4   g57(.a(new_n62_), .b(x12), .c(x10), .d(x09), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n91_), .c(new_n35_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n89_), .c(new_n86_), .O(z08));
  oai21  g60(.a(new_n84_), .b(x13), .c(x11), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n43_), .O(new_n96_));
  nand2  g62(.a(x13), .b(x11), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n52_), .c(x02), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x16), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nor2   g66(.a(new_n92_), .b(new_n100_), .O(new_n101_));
  nor4   g67(.a(new_n73_), .b(new_n62_), .c(x13), .d(x12), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n101_), .c(new_n35_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n99_), .c(new_n96_), .O(z09));
  nand2  g70(.a(new_n48_), .b(x00), .O(new_n105_));
  nand3  g71(.a(new_n35_), .b(new_n52_), .c(x02), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x16), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n100_), .c(new_n90_), .d(new_n72_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n105_), .c(x09), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(x08), .c(x07), .d(x06), .O(new_n110_));
  inv1   g76(.a(new_n48_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x14), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n46_), .O(z10));
  nor2   g81(.a(new_n36_), .b(new_n37_), .O(z11));
  nand2  g82(.a(new_n46_), .b(x03), .O(new_n117_));
  nand2  g83(.a(x16), .b(new_n37_), .O(new_n118_));
  aoi21  g84(.a(new_n118_), .b(new_n117_), .c(new_n43_), .O(z12));
  nand2  g85(.a(x17), .b(x04), .O(new_n120_));
  nand2  g86(.a(new_n120_), .b(new_n46_), .O(z14));
endmodule


