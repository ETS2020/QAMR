// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_;
  inv1   g00(.a(x06), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x04), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x05), .O(new_n45_));
  nor2   g11(.a(x06), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x04), .c(new_n46_), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  nand3  g14(.a(new_n35_), .b(x05), .c(new_n48_), .O(new_n49_));
  oai21  g15(.a(new_n47_), .b(new_n44_), .c(new_n49_), .O(z01));
  inv1   g16(.a(new_n36_), .O(new_n51_));
  oai21  g17(.a(x03), .b(new_n37_), .c(x16), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n35_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n51_), .O(z02));
  nand3  g20(.a(x07), .b(new_n35_), .c(x04), .O(new_n55_));
  oai21  g21(.a(x07), .b(new_n35_), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n51_), .O(z03));
  xnor2a g24(.a(x08), .b(x07), .O(new_n59_));
  nand3  g25(.a(x08), .b(new_n35_), .c(x04), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n35_), .c(new_n60_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n52_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n51_), .O(z04));
  oai21  g29(.a(x09), .b(new_n48_), .c(new_n35_), .O(new_n64_));
  nand2  g30(.a(x08), .b(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n52_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n64_), .O(z05));
  oai21  g37(.a(x10), .b(new_n48_), .c(new_n35_), .O(new_n72_));
  nand3  g38(.a(new_n67_), .b(x08), .c(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x08), .c(x07), .d(x06), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n52_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x04), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n72_), .O(z06));
  oai21  g45(.a(x11), .b(new_n48_), .c(new_n35_), .O(new_n80_));
  nand3  g46(.a(new_n75_), .b(x08), .c(x07), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x11), .O(new_n82_));
  nand3  g48(.a(x08), .b(x07), .c(x06), .O(new_n83_));
  inv1   g49(.a(new_n83_), .O(new_n84_));
  nor2   g50(.a(x11), .b(x10), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n67_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n82_), .c(new_n52_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x04), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n80_), .O(z07));
  oai21  g55(.a(x12), .b(new_n48_), .c(new_n35_), .O(new_n90_));
  nand2  g56(.a(new_n85_), .b(new_n67_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n65_), .c(x12), .O(new_n92_));
  inv1   g58(.a(x11), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  nand4  g60(.a(new_n84_), .b(new_n75_), .c(new_n94_), .d(new_n93_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n92_), .c(new_n52_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x04), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n90_), .O(z08));
  nand2  g64(.a(new_n95_), .b(x13), .O(new_n99_));
  inv1   g65(.a(x10), .O(new_n100_));
  inv1   g66(.a(x13), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n94_), .c(new_n93_), .d(new_n100_), .O(new_n102_));
  or2    g68(.a(new_n102_), .b(new_n68_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n99_), .c(new_n52_), .d(x04), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  nand2  g71(.a(new_n102_), .b(new_n105_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n52_), .c(new_n67_), .d(x08), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(x07), .c(x06), .O(new_n109_));
  or2    g75(.a(new_n52_), .b(new_n40_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n48_), .O(z10));
  nand2  g77(.a(new_n51_), .b(new_n37_), .O(z11));
  aoi21  g78(.a(x16), .b(new_n37_), .c(x03), .O(new_n113_));
  nor2   g79(.a(new_n113_), .b(new_n48_), .O(z12));
  inv1   g80(.a(new_n46_), .O(z13));
  and2   g81(.a(x17), .b(x04), .O(z14));
endmodule


