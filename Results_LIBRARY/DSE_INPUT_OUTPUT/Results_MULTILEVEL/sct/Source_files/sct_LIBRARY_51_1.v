// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x08), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n36_), .O(z01));
  oai21  g12(.a(x03), .b(new_n37_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n35_), .c(x06), .O(z02));
  inv1   g15(.a(new_n36_), .O(new_n50_));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n50_), .O(z03));
  xor2a  g19(.a(x08), .b(x07), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n47_), .c(x06), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(x06), .O(new_n57_));
  oai21  g23(.a(x09), .b(x08), .c(new_n57_), .O(new_n58_));
  nand2  g24(.a(x08), .b(x07), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x08), .c(x07), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n44_), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n60_), .c(new_n58_), .d(new_n47_), .O(z05));
  oai21  g31(.a(x10), .b(x08), .c(new_n57_), .O(new_n66_));
  nand2  g32(.a(new_n62_), .b(x10), .O(new_n67_));
  inv1   g33(.a(new_n59_), .O(new_n68_));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n68_), .c(new_n44_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(new_n66_), .d(new_n47_), .O(z06));
  nand2  g37(.a(new_n48_), .b(new_n50_), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  aoi21  g39(.a(new_n69_), .b(x07), .c(new_n73_), .O(new_n74_));
  nor4   g40(.a(new_n59_), .b(x11), .c(x10), .d(x09), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n74_), .c(x06), .O(new_n76_));
  nand2  g42(.a(x11), .b(new_n35_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(z07));
  oai21  g44(.a(x12), .b(x08), .c(new_n57_), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  or2    g46(.a(new_n75_), .b(new_n80_), .O(new_n81_));
  nor3   g47(.a(x12), .b(x11), .c(x10), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n63_), .c(new_n44_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n81_), .c(new_n79_), .d(new_n47_), .O(z08));
  inv1   g50(.a(x13), .O(new_n85_));
  inv1   g51(.a(x07), .O(new_n86_));
  nor2   g52(.a(x09), .b(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n82_), .c(new_n85_), .O(new_n88_));
  inv1   g54(.a(x10), .O(new_n89_));
  nand4  g55(.a(new_n85_), .b(new_n80_), .c(new_n73_), .d(new_n89_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n62_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n88_), .c(x06), .O(new_n92_));
  nand2  g58(.a(x13), .b(new_n35_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n72_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n47_), .c(new_n61_), .d(x08), .O(new_n97_));
  inv1   g63(.a(new_n47_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x14), .O(new_n99_));
  oai21  g65(.a(new_n97_), .b(new_n86_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x06), .O(new_n101_));
  nand3  g67(.a(new_n98_), .b(x14), .c(new_n35_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n44_), .O(z10));
  nor2   g69(.a(new_n36_), .b(new_n37_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n37_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n44_), .c(new_n50_), .O(z12));
  nand2  g72(.a(new_n50_), .b(new_n44_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  nor3   g74(.a(new_n36_), .b(new_n108_), .c(new_n44_), .O(z14));
endmodule


