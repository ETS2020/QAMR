// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x09), .b(x08), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  inv1   g10(.a(new_n36_), .O(new_n45_));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n45_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n47_), .c(new_n45_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand2  g19(.a(x08), .b(new_n46_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n46_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n47_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n45_), .O(z04));
  inv1   g23(.a(x09), .O(new_n58_));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  inv1   g25(.a(new_n47_), .O(new_n60_));
  nand3  g26(.a(new_n58_), .b(x07), .c(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x08), .c(x04), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  oai21  g29(.a(new_n59_), .b(new_n58_), .c(new_n63_), .O(z05));
  inv1   g30(.a(x08), .O(new_n65_));
  oai21  g31(.a(x10), .b(new_n58_), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n61_), .b(x10), .O(new_n67_));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n59_), .c(new_n42_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n66_), .d(new_n47_), .O(z06));
  oai21  g36(.a(new_n60_), .b(new_n42_), .c(new_n45_), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x07), .c(x06), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x11), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n59_), .c(new_n58_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x08), .O(new_n78_));
  nand2  g44(.a(x11), .b(x09), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n71_), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  aoi21  g47(.a(new_n75_), .b(new_n59_), .c(new_n81_), .O(new_n82_));
  inv1   g48(.a(new_n61_), .O(new_n83_));
  nor3   g49(.a(x12), .b(x11), .c(x10), .O(new_n84_));
  and2   g50(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n82_), .c(x08), .O(new_n86_));
  nand2  g52(.a(x12), .b(x09), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n71_), .O(z08));
  inv1   g54(.a(x13), .O(new_n89_));
  aoi21  g55(.a(new_n84_), .b(new_n59_), .c(new_n89_), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  nand4  g57(.a(new_n89_), .b(new_n81_), .c(new_n91_), .d(new_n72_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n61_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n90_), .c(x08), .O(new_n94_));
  nand2  g60(.a(x13), .b(x09), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n71_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n47_), .c(new_n58_), .d(x07), .O(new_n99_));
  nand2  g65(.a(new_n60_), .b(x14), .O(new_n100_));
  oai21  g66(.a(new_n99_), .b(new_n46_), .c(new_n100_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x04), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n45_), .O(z10));
  nand2  g69(.a(new_n45_), .b(new_n35_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n35_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n42_), .c(new_n45_), .O(z12));
  nand2  g72(.a(new_n45_), .b(new_n42_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  nor3   g74(.a(new_n36_), .b(new_n108_), .c(new_n42_), .O(z14));
endmodule


