// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n117_, new_n118_, new_n119_, new_n122_;
  inv1   g00(.a(x10), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n36_), .O(z01));
  oai21  g12(.a(x03), .b(new_n37_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n35_), .c(x06), .O(z02));
  inv1   g15(.a(x06), .O(new_n50_));
  nand3  g16(.a(new_n35_), .b(x07), .c(new_n50_), .O(new_n51_));
  oai21  g17(.a(x07), .b(new_n50_), .c(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  inv1   g20(.a(new_n36_), .O(new_n55_));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n50_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n50_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n47_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n55_), .O(z04));
  nor2   g26(.a(x10), .b(x09), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(x06), .c(new_n48_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  and2   g29(.a(x08), .b(x07), .O(new_n64_));
  nand3  g30(.a(new_n63_), .b(x08), .c(x07), .O(new_n65_));
  oai21  g31(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g33(.a(new_n35_), .b(x09), .c(new_n50_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n62_), .O(z05));
  nand2  g35(.a(new_n63_), .b(x08), .O(new_n70_));
  nand2  g36(.a(x07), .b(x06), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n70_), .c(x10), .O(new_n72_));
  nand4  g38(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n72_), .c(new_n47_), .d(x04), .O(z06));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(x06), .c(new_n48_), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n63_), .c(x08), .O(new_n78_));
  oai22  g44(.a(new_n78_), .b(new_n71_), .c(new_n77_), .d(x06), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n35_), .O(new_n80_));
  nand2  g46(.a(new_n64_), .b(new_n61_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(x11), .c(x06), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n76_), .O(z07));
  oai21  g49(.a(x12), .b(x10), .c(new_n50_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n48_), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand2  g52(.a(new_n64_), .b(x06), .O(new_n87_));
  nand3  g53(.a(new_n86_), .b(new_n77_), .c(new_n63_), .O(new_n88_));
  oai22  g54(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x06), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n35_), .O(new_n90_));
  nand3  g56(.a(new_n75_), .b(new_n64_), .c(new_n63_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(x12), .c(x06), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n90_), .c(new_n85_), .O(z08));
  oai21  g59(.a(x13), .b(x10), .c(new_n50_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n48_), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n86_), .c(new_n77_), .d(new_n63_), .O(new_n97_));
  oai22  g63(.a(new_n97_), .b(new_n87_), .c(new_n96_), .d(x06), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n35_), .O(new_n99_));
  inv1   g65(.a(new_n65_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n86_), .c(new_n77_), .d(new_n35_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(x13), .c(x06), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n99_), .c(new_n95_), .O(z09));
  inv1   g69(.a(x07), .O(new_n104_));
  inv1   g70(.a(x00), .O(new_n105_));
  nand4  g71(.a(new_n96_), .b(new_n86_), .c(new_n77_), .d(new_n35_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n47_), .c(new_n63_), .d(x08), .O(new_n108_));
  inv1   g74(.a(new_n47_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x14), .O(new_n110_));
  oai21  g76(.a(new_n108_), .b(new_n104_), .c(new_n110_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x06), .O(new_n112_));
  nand3  g78(.a(new_n109_), .b(x14), .c(new_n35_), .O(new_n113_));
  aoi21  g79(.a(new_n113_), .b(new_n112_), .c(new_n44_), .O(z10));
  nand2  g80(.a(new_n55_), .b(new_n37_), .O(z11));
  inv1   g81(.a(x03), .O(new_n116_));
  inv1   g82(.a(x16), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(x02), .c(new_n116_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n55_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  nand2  g86(.a(new_n55_), .b(new_n44_), .O(z13));
  nand2  g87(.a(x17), .b(x04), .O(new_n122_));
  nand2  g88(.a(new_n122_), .b(new_n55_), .O(z14));
endmodule


