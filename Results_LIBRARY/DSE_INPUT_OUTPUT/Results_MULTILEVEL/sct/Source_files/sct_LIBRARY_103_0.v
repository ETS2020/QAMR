// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n120_;
  nor2   g00(.a(x15), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  inv1   g07(.a(x15), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(new_n40_), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n40_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n43_), .O(z01));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(x15), .c(new_n41_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n35_), .O(new_n52_));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n52_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand3  g22(.a(x15), .b(x08), .c(new_n41_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n41_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n49_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  oai21  g26(.a(new_n42_), .b(x09), .c(new_n41_), .O(new_n61_));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x07), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n41_), .O(new_n65_));
  inv1   g31(.a(x08), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n66_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n65_), .c(new_n46_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n63_), .c(new_n61_), .d(new_n49_), .O(z05));
  oai21  g35(.a(new_n42_), .b(x10), .c(new_n41_), .O(new_n70_));
  nand2  g36(.a(new_n67_), .b(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n65_), .c(new_n46_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n72_), .c(new_n70_), .d(new_n49_), .O(z06));
  oai21  g43(.a(new_n42_), .b(x11), .c(new_n41_), .O(new_n78_));
  inv1   g44(.a(new_n73_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n62_), .c(x11), .O(new_n80_));
  nand3  g46(.a(x08), .b(x07), .c(x06), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  inv1   g48(.a(x09), .O(new_n83_));
  inv1   g49(.a(x10), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n82_), .c(new_n46_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n80_), .c(new_n78_), .d(new_n49_), .O(z07));
  oai21  g55(.a(new_n42_), .b(x12), .c(new_n41_), .O(new_n90_));
  oai21  g56(.a(new_n86_), .b(new_n62_), .c(x12), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand3  g58(.a(new_n73_), .b(new_n92_), .c(new_n85_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n82_), .c(new_n46_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n91_), .c(new_n90_), .d(new_n49_), .O(z08));
  inv1   g62(.a(new_n49_), .O(new_n97_));
  oai21  g63(.a(new_n42_), .b(x13), .c(new_n41_), .O(new_n98_));
  oai21  g64(.a(new_n97_), .b(new_n46_), .c(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n92_), .b(new_n85_), .c(new_n84_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n71_), .c(x13), .O(new_n101_));
  inv1   g67(.a(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n92_), .c(new_n85_), .d(new_n84_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n71_), .c(new_n101_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x06), .O(new_n105_));
  nand3  g71(.a(x15), .b(x13), .c(new_n41_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n105_), .c(new_n99_), .O(z09));
  inv1   g73(.a(x00), .O(new_n108_));
  nand2  g74(.a(new_n103_), .b(new_n108_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n49_), .c(new_n83_), .d(x08), .O(new_n110_));
  nand2  g76(.a(new_n97_), .b(x14), .O(new_n111_));
  oai21  g77(.a(new_n110_), .b(new_n64_), .c(new_n111_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x06), .O(new_n113_));
  nand3  g79(.a(new_n97_), .b(x15), .c(x14), .O(new_n114_));
  aoi21  g80(.a(new_n114_), .b(new_n113_), .c(new_n46_), .O(z10));
  nand2  g81(.a(new_n52_), .b(new_n36_), .O(z11));
  aoi21  g82(.a(x16), .b(new_n36_), .c(x03), .O(new_n117_));
  oai21  g83(.a(new_n117_), .b(new_n46_), .c(new_n52_), .O(z12));
  nand2  g84(.a(new_n52_), .b(new_n46_), .O(z13));
  inv1   g85(.a(x17), .O(new_n120_));
  nor3   g86(.a(new_n35_), .b(new_n120_), .c(new_n46_), .O(z14));
endmodule


