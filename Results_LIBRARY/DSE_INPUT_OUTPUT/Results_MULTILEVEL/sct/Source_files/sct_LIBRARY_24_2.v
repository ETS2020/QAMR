// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x18), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x08), .O(new_n36_));
  or2    g02(.a(x14), .b(x01), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  oai21  g07(.a(new_n35_), .b(x15), .c(new_n41_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n36_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n36_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n36_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n46_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n46_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n50_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n36_), .O(z04));
  oai21  g26(.a(new_n49_), .b(new_n43_), .c(new_n36_), .O(new_n61_));
  and2   g27(.a(x07), .b(x06), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n35_), .c(x08), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n62_), .b(x18), .c(new_n65_), .d(x08), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n61_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand3  g34(.a(new_n62_), .b(new_n68_), .c(new_n65_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x18), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x08), .O(new_n71_));
  aoi21  g37(.a(new_n66_), .b(x10), .c(new_n43_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n50_), .O(z06));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n62_), .c(new_n65_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x18), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x08), .O(new_n77_));
  nand3  g43(.a(x08), .b(x07), .c(x06), .O(new_n78_));
  nand3  g44(.a(x18), .b(new_n68_), .c(new_n65_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n78_), .c(x11), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n77_), .c(new_n50_), .d(x04), .O(z07));
  nand3  g47(.a(new_n65_), .b(x07), .c(x06), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n68_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n82_), .c(x18), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x08), .O(new_n87_));
  nand4  g53(.a(x18), .b(new_n83_), .c(new_n68_), .d(new_n65_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n78_), .c(x12), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n87_), .c(new_n50_), .d(x04), .O(z08));
  nand2  g56(.a(new_n87_), .b(x13), .O(new_n91_));
  nand3  g57(.a(new_n62_), .b(new_n65_), .c(x08), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nor2   g59(.a(new_n35_), .b(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n74_), .d(new_n84_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n91_), .c(new_n61_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nor2   g63(.a(x13), .b(x12), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n74_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n97_), .c(new_n49_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n65_), .c(x08), .d(x07), .O(new_n101_));
  nand2  g67(.a(new_n49_), .b(x14), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n46_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n36_), .O(z10));
  nand2  g71(.a(new_n36_), .b(new_n38_), .O(z11));
  oai21  g72(.a(new_n48_), .b(x02), .c(new_n47_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n36_), .c(x04), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(z12));
  aoi21  g75(.a(new_n35_), .b(x08), .c(new_n43_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n36_), .O(z14));
endmodule


