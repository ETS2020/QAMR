// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x06), .O(new_n35_));
  nor2   g01(.a(x13), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n36_), .O(z01));
  inv1   g12(.a(new_n36_), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n35_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x13), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n56_), .b(x06), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(new_n50_), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n44_), .c(new_n47_), .O(new_n63_));
  inv1   g29(.a(x13), .O(new_n64_));
  inv1   g30(.a(x07), .O(new_n65_));
  nor2   g31(.a(new_n56_), .b(new_n65_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n64_), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  nor2   g34(.a(new_n65_), .b(new_n35_), .O(new_n69_));
  nor2   g35(.a(new_n64_), .b(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(x08), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n63_), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n66_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x13), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  aoi21  g42(.a(new_n71_), .b(x10), .c(new_n44_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n50_), .O(z06));
  nand2  g44(.a(new_n76_), .b(x11), .O(new_n79_));
  nor2   g45(.a(new_n64_), .b(x11), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n73_), .c(new_n66_), .d(x06), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n79_), .c(new_n63_), .O(z07));
  nor3   g48(.a(x11), .b(x10), .c(x09), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n66_), .c(new_n64_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n35_), .c(x12), .O(new_n85_));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  nor2   g52(.a(x09), .b(new_n56_), .O(new_n87_));
  nor2   g53(.a(new_n64_), .b(x12), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n69_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n85_), .c(new_n63_), .O(z08));
  nand2  g56(.a(new_n66_), .b(x06), .O(new_n91_));
  nor2   g57(.a(x12), .b(x11), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n73_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n91_), .c(x13), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n63_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  aoi21  g62(.a(new_n49_), .b(x16), .c(x09), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x08), .c(x07), .d(x06), .O(new_n98_));
  nand2  g64(.a(new_n62_), .b(x14), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x13), .O(new_n101_));
  nand3  g67(.a(new_n62_), .b(x14), .c(new_n35_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n44_), .O(z10));
  nor2   g69(.a(new_n36_), .b(new_n37_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n37_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n44_), .c(new_n47_), .O(z12));
  nand2  g72(.a(new_n47_), .b(new_n44_), .O(z13));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n47_), .O(z14));
endmodule


