// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand2  g02(.a(x08), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n43_), .c(new_n37_), .O(z01));
  inv1   g12(.a(x08), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x04), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(new_n47_), .c(x06), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n37_), .O(z03));
  xor2a  g21(.a(x08), .b(x07), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n50_), .c(x06), .d(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  oai21  g24(.a(x09), .b(x08), .c(new_n36_), .O(new_n59_));
  inv1   g25(.a(x07), .O(new_n60_));
  oai21  g26(.a(new_n47_), .b(new_n60_), .c(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x08), .c(x07), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n44_), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n61_), .c(new_n59_), .d(new_n50_), .O(z05));
  oai21  g32(.a(x10), .b(x08), .c(new_n36_), .O(new_n67_));
  nand2  g33(.a(new_n63_), .b(x10), .O(new_n68_));
  inv1   g34(.a(x10), .O(new_n69_));
  nor2   g35(.a(new_n47_), .b(new_n60_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n62_), .O(new_n71_));
  and2   g37(.a(new_n71_), .b(x04), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n68_), .c(new_n67_), .d(new_n50_), .O(z06));
  oai21  g39(.a(x11), .b(x08), .c(new_n36_), .O(new_n74_));
  nand2  g40(.a(new_n71_), .b(x11), .O(new_n75_));
  nor3   g41(.a(x11), .b(x10), .c(x09), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n70_), .c(new_n44_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n75_), .c(new_n74_), .d(new_n50_), .O(z07));
  nand2  g44(.a(new_n51_), .b(new_n37_), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nor2   g47(.a(x09), .b(new_n60_), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(new_n83_));
  nor3   g49(.a(x12), .b(x11), .c(x10), .O(new_n84_));
  and2   g50(.a(new_n84_), .b(new_n64_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n83_), .c(x06), .O(new_n86_));
  nand2  g52(.a(x12), .b(new_n47_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n79_), .O(z08));
  inv1   g54(.a(x13), .O(new_n89_));
  aoi21  g55(.a(new_n84_), .b(new_n82_), .c(new_n89_), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  nand4  g57(.a(new_n89_), .b(new_n80_), .c(new_n91_), .d(new_n69_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n63_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n90_), .c(x06), .O(new_n94_));
  nand2  g60(.a(x13), .b(new_n47_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n79_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n50_), .c(new_n62_), .d(x08), .O(new_n99_));
  nand3  g65(.a(new_n49_), .b(x16), .c(x14), .O(new_n100_));
  oai21  g66(.a(new_n99_), .b(new_n60_), .c(new_n100_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x04), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n37_), .O(z10));
  nand2  g69(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n38_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n44_), .c(new_n37_), .O(z12));
  aoi21  g72(.a(x08), .b(new_n36_), .c(new_n44_), .O(z13));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n37_), .O(z14));
endmodule


