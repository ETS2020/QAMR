// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n107_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nor2   g02(.a(x11), .b(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(new_n37_), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n47_), .c(new_n46_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand2  g19(.a(x08), .b(new_n45_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(new_n47_), .c(new_n46_), .d(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x09), .O(new_n59_));
  inv1   g25(.a(x09), .O(new_n60_));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n60_), .c(x08), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n37_), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n59_), .c(new_n47_), .d(x04), .O(z05));
  inv1   g31(.a(new_n47_), .O(new_n66_));
  inv1   g32(.a(x11), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(x09), .c(x10), .O(new_n68_));
  oai21  g34(.a(new_n66_), .b(new_n42_), .c(new_n68_), .O(new_n69_));
  nand3  g35(.a(new_n62_), .b(x11), .c(x10), .O(new_n70_));
  nand4  g36(.a(new_n61_), .b(new_n36_), .c(new_n60_), .d(x08), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(z06));
  nand4  g38(.a(new_n61_), .b(new_n67_), .c(new_n60_), .d(x08), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n36_), .c(x04), .O(new_n74_));
  aoi21  g40(.a(new_n62_), .b(x11), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n47_), .O(z07));
  oai21  g42(.a(new_n66_), .b(new_n42_), .c(new_n46_), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  inv1   g44(.a(new_n58_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n67_), .d(new_n60_), .O(new_n80_));
  oai21  g46(.a(new_n63_), .b(new_n78_), .c(new_n80_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n36_), .O(new_n82_));
  nand2  g48(.a(x12), .b(x11), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n77_), .O(z08));
  oai21  g50(.a(x13), .b(new_n67_), .c(x10), .O(new_n85_));
  nand2  g51(.a(new_n80_), .b(x13), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n78_), .c(new_n67_), .d(new_n60_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n79_), .c(new_n42_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n86_), .c(new_n85_), .d(new_n47_), .O(z09));
  inv1   g57(.a(x00), .O(new_n92_));
  inv1   g58(.a(x03), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x02), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(x16), .c(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  nand2  g62(.a(new_n66_), .b(x14), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n92_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n46_), .O(new_n99_));
  nand4  g65(.a(new_n47_), .b(new_n87_), .c(new_n78_), .d(new_n67_), .O(new_n100_));
  nor3   g66(.a(new_n100_), .b(x10), .c(x09), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(x08), .c(x07), .d(x06), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n99_), .c(new_n42_), .O(z10));
  nor2   g69(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g70(.a(x16), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(x02), .c(new_n93_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n46_), .c(x04), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(z12));
  nand2  g74(.a(new_n46_), .b(new_n42_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n46_), .O(z14));
endmodule


