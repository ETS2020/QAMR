// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n89_, new_n92_;
  inv1   g00(.a(x10), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n36_), .O(z03));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x10), .c(x06), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n48_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(new_n51_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n45_), .c(new_n36_), .O(new_n64_));
  aoi21  g30(.a(x08), .b(x07), .c(new_n35_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n48_), .c(x09), .O(new_n66_));
  nor2   g32(.a(new_n35_), .b(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n64_), .O(z05));
  inv1   g35(.a(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n64_), .O(z06));
  nor2   g39(.a(x11), .b(new_n45_), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n51_), .c(new_n37_), .O(z07));
  nor2   g41(.a(x12), .b(new_n45_), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n51_), .c(new_n37_), .O(z08));
  nor2   g43(.a(x13), .b(new_n45_), .O(new_n78_));
  aoi21  g44(.a(new_n78_), .b(new_n51_), .c(new_n37_), .O(z09));
  inv1   g45(.a(x00), .O(new_n80_));
  aoi21  g46(.a(new_n50_), .b(x16), .c(x09), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(x08), .c(x07), .d(x06), .O(new_n82_));
  nand2  g48(.a(new_n63_), .b(x14), .O(new_n83_));
  oai21  g49(.a(new_n82_), .b(new_n80_), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x10), .O(new_n85_));
  nand3  g51(.a(new_n63_), .b(x14), .c(new_n48_), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n85_), .c(new_n45_), .O(z10));
  nor2   g53(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g54(.a(x16), .b(new_n38_), .c(x03), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n45_), .c(new_n36_), .O(z12));
  nor2   g56(.a(new_n37_), .b(new_n45_), .O(z13));
  nand2  g57(.a(new_n36_), .b(x17), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n45_), .O(z14));
endmodule


