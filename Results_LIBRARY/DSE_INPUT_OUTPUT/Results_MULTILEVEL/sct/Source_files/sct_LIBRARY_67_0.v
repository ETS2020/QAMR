// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n101_;
  nor2   g00(.a(x13), .b(x10), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n35_), .O(z01));
  inv1   g13(.a(new_n35_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n36_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n49_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n48_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n48_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n49_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n49_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n50_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n48_), .O(z04));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  and2   g30(.a(new_n64_), .b(new_n48_), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n62_), .c(new_n50_), .d(x04), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  nand2  g33(.a(new_n64_), .b(x13), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g35(.a(new_n64_), .b(x10), .c(new_n45_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n50_), .O(z06));
  nand2  g37(.a(new_n50_), .b(x04), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n48_), .O(new_n73_));
  nand2  g39(.a(new_n69_), .b(x11), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  inv1   g41(.a(new_n61_), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(x13), .d(new_n75_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(z07));
  inv1   g45(.a(x12), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n75_), .c(new_n63_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n61_), .c(x13), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  nand2  g49(.a(new_n78_), .b(x12), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n83_), .c(new_n50_), .d(x04), .O(z08));
  nand3  g51(.a(new_n77_), .b(new_n80_), .c(new_n75_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n61_), .c(x13), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n73_), .O(z09));
  nand4  g54(.a(new_n50_), .b(new_n63_), .c(x08), .d(x07), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n49_), .O(new_n90_));
  nor2   g56(.a(new_n50_), .b(new_n39_), .O(new_n91_));
  aoi21  g57(.a(new_n90_), .b(x00), .c(new_n91_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n45_), .c(new_n48_), .O(z10));
  nor2   g59(.a(new_n35_), .b(new_n36_), .O(z11));
  inv1   g60(.a(x03), .O(new_n95_));
  inv1   g61(.a(x16), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(x02), .c(new_n95_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n48_), .c(x04), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(z12));
  nand2  g65(.a(new_n48_), .b(new_n45_), .O(z13));
  inv1   g66(.a(x17), .O(new_n101_));
  nor3   g67(.a(new_n35_), .b(new_n101_), .c(new_n45_), .O(z14));
endmodule


