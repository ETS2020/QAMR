// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n113_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x09), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(new_n36_), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  inv1   g14(.a(new_n37_), .O(new_n49_));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  inv1   g19(.a(x08), .O(new_n54_));
  nor2   g20(.a(x09), .b(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n45_), .O(new_n56_));
  oai21  g22(.a(new_n53_), .b(new_n45_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n46_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  inv1   g25(.a(new_n46_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n42_), .c(new_n49_), .O(new_n61_));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  aoi22  g28(.a(new_n62_), .b(x09), .c(new_n55_), .d(x07), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n45_), .c(new_n61_), .O(z05));
  nor2   g30(.a(x10), .b(x09), .O(new_n65_));
  oai22  g31(.a(new_n65_), .b(x06), .c(new_n60_), .d(new_n42_), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(x06), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n36_), .O(new_n70_));
  nand2  g36(.a(new_n55_), .b(x07), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(x10), .c(x06), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z06));
  oai21  g39(.a(x11), .b(x09), .c(new_n45_), .O(new_n74_));
  inv1   g40(.a(new_n65_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n62_), .c(x11), .O(new_n76_));
  nand3  g42(.a(x08), .b(x07), .c(x06), .O(new_n77_));
  inv1   g43(.a(new_n77_), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n67_), .c(new_n36_), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n78_), .c(new_n42_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n76_), .c(new_n74_), .d(new_n46_), .O(z07));
  oai21  g49(.a(x12), .b(x09), .c(new_n45_), .O(new_n84_));
  oai21  g50(.a(new_n80_), .b(new_n62_), .c(x12), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand3  g52(.a(new_n65_), .b(new_n86_), .c(new_n79_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(new_n78_), .c(new_n42_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n85_), .c(new_n84_), .d(new_n46_), .O(z08));
  oai21  g56(.a(x13), .b(x09), .c(new_n45_), .O(new_n91_));
  nand3  g57(.a(new_n86_), .b(new_n79_), .c(new_n67_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n71_), .c(x13), .O(new_n93_));
  nand3  g59(.a(new_n55_), .b(x07), .c(x06), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n86_), .c(new_n79_), .d(new_n67_), .O(new_n96_));
  nor2   g62(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n42_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n93_), .c(new_n91_), .d(new_n46_), .O(z09));
  inv1   g65(.a(x07), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n96_), .b(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n46_), .c(new_n36_), .d(x08), .O(new_n103_));
  nand2  g69(.a(new_n60_), .b(x14), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n100_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x06), .O(new_n106_));
  nand3  g72(.a(new_n60_), .b(x14), .c(new_n36_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n42_), .O(z10));
  nand2  g74(.a(new_n49_), .b(new_n35_), .O(z11));
  aoi21  g75(.a(x16), .b(new_n35_), .c(x03), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n42_), .c(new_n49_), .O(z12));
  nor2   g77(.a(new_n37_), .b(new_n42_), .O(z13));
  nand2  g78(.a(x17), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n49_), .O(z14));
endmodule


