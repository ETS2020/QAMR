// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x11), .b(x06), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(x11), .c(x06), .O(z02));
  inv1   g13(.a(x06), .O(new_n48_));
  nand3  g14(.a(x11), .b(x07), .c(new_n48_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand3  g19(.a(x11), .b(x08), .c(new_n48_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n45_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  inv1   g23(.a(x11), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(x09), .c(new_n48_), .O(new_n59_));
  nand2  g25(.a(x08), .b(x07), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n48_), .O(new_n63_));
  inv1   g29(.a(x08), .O(new_n64_));
  nor2   g30(.a(x09), .b(new_n64_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n63_), .c(new_n42_), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n61_), .c(new_n59_), .d(new_n45_), .O(z05));
  oai21  g33(.a(new_n58_), .b(x10), .c(new_n48_), .O(new_n68_));
  nand2  g34(.a(new_n65_), .b(x07), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(x08), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n63_), .c(new_n42_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n70_), .c(new_n68_), .d(new_n45_), .O(z06));
  nand2  g42(.a(new_n72_), .b(new_n71_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n60_), .c(x11), .O(new_n78_));
  inv1   g44(.a(new_n60_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n58_), .c(new_n72_), .d(new_n71_), .O(new_n80_));
  and2   g46(.a(new_n80_), .b(x06), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n78_), .c(new_n45_), .d(x04), .O(z07));
  oai21  g48(.a(x12), .b(new_n58_), .c(new_n48_), .O(new_n83_));
  nand2  g49(.a(new_n80_), .b(x12), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(new_n69_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n58_), .d(new_n72_), .O(new_n87_));
  and2   g53(.a(new_n87_), .b(x04), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n84_), .c(new_n83_), .d(new_n45_), .O(z08));
  oai21  g55(.a(x13), .b(new_n58_), .c(new_n48_), .O(new_n90_));
  nand2  g56(.a(new_n87_), .b(x13), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n85_), .c(new_n58_), .d(new_n72_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n86_), .c(new_n42_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n91_), .c(new_n90_), .d(new_n45_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n93_), .b(new_n97_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n45_), .c(new_n71_), .d(x08), .O(new_n99_));
  inv1   g65(.a(new_n45_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x14), .O(new_n101_));
  oai21  g67(.a(new_n99_), .b(new_n62_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x06), .O(new_n103_));
  nand3  g69(.a(new_n100_), .b(x14), .c(x11), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n42_), .O(z10));
  nor2   g71(.a(new_n36_), .b(new_n35_), .O(z11));
  inv1   g72(.a(new_n36_), .O(new_n107_));
  aoi21  g73(.a(x16), .b(new_n35_), .c(x03), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n42_), .c(new_n107_), .O(z12));
  nand2  g75(.a(new_n107_), .b(new_n42_), .O(z13));
  inv1   g76(.a(x17), .O(new_n111_));
  nor3   g77(.a(new_n36_), .b(new_n111_), .c(new_n42_), .O(z14));
endmodule


