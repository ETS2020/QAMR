// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x09), .O(new_n35_));
  inv1   g01(.a(x18), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z00));
  aoi21  g08(.a(new_n35_), .b(x05), .c(new_n36_), .O(new_n43_));
  aoi21  g09(.a(x15), .b(new_n35_), .c(new_n36_), .O(new_n44_));
  oai22  g10(.a(new_n44_), .b(x05), .c(new_n43_), .d(x04), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n38_), .c(new_n46_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n49_), .c(new_n38_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n46_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n46_), .c(new_n56_), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n49_), .c(new_n38_), .d(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  and2   g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n36_), .c(x08), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  nand3  g28(.a(new_n60_), .b(new_n35_), .c(x08), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n62_), .c(new_n49_), .d(x04), .O(z05));
  inv1   g30(.a(x04), .O(new_n65_));
  inv1   g31(.a(x16), .O(new_n66_));
  aoi21  g32(.a(new_n47_), .b(x02), .c(new_n66_), .O(new_n67_));
  oai21  g33(.a(x18), .b(x10), .c(x09), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nand3  g36(.a(x08), .b(x07), .c(x06), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand3  g39(.a(new_n60_), .b(new_n70_), .c(x08), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n73_), .c(new_n35_), .O(new_n76_));
  nand3  g42(.a(new_n36_), .b(x10), .c(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n69_), .O(z06));
  oai21  g44(.a(x18), .b(x11), .c(x09), .O(new_n79_));
  oai21  g45(.a(new_n67_), .b(new_n65_), .c(new_n79_), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand4  g47(.a(new_n60_), .b(new_n81_), .c(new_n70_), .d(x08), .O(new_n82_));
  oai21  g48(.a(new_n75_), .b(new_n81_), .c(new_n82_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n35_), .O(new_n84_));
  nand3  g50(.a(new_n36_), .b(x11), .c(x09), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n80_), .O(z07));
  oai21  g52(.a(x18), .b(x12), .c(x09), .O(new_n87_));
  nand2  g53(.a(new_n82_), .b(x12), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n81_), .c(new_n70_), .d(new_n35_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n72_), .c(new_n65_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n88_), .c(new_n87_), .d(new_n49_), .O(z08));
  oai21  g59(.a(x18), .b(x13), .c(x09), .O(new_n94_));
  nand3  g60(.a(new_n89_), .b(new_n81_), .c(new_n70_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n71_), .c(x13), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n89_), .c(new_n81_), .d(new_n70_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n63_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n65_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n96_), .c(new_n94_), .d(new_n49_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n98_), .b(new_n102_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n49_), .c(x08), .d(x07), .O(new_n104_));
  nand2  g70(.a(new_n67_), .b(x14), .O(new_n105_));
  oai21  g71(.a(new_n104_), .b(new_n46_), .c(new_n105_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n35_), .O(new_n107_));
  nand4  g73(.a(new_n48_), .b(new_n36_), .c(x16), .d(x14), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n65_), .O(z10));
  nor2   g75(.a(new_n37_), .b(new_n40_), .O(z11));
  oai21  g76(.a(new_n66_), .b(x02), .c(new_n47_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n38_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  nand2  g79(.a(new_n38_), .b(new_n65_), .O(z13));
  nand3  g80(.a(new_n38_), .b(x17), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z14));
endmodule


