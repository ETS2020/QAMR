// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n113_;
  inv1   g00(.a(x12), .O(new_n35_));
  nor2   g01(.a(x16), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand3  g13(.a(x16), .b(new_n47_), .c(x02), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n35_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n46_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(new_n36_), .O(new_n54_));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand2  g21(.a(new_n47_), .b(x02), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(x16), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n55_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n54_), .O(z03));
  xnor2a g25(.a(x08), .b(x07), .O(new_n60_));
  nand2  g26(.a(x08), .b(new_n46_), .O(new_n61_));
  oai21  g27(.a(new_n60_), .b(new_n46_), .c(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n57_), .c(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n54_), .O(z04));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n54_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n57_), .O(z05));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(x08), .c(x07), .d(x06), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  aoi21  g40(.a(new_n68_), .b(x10), .c(new_n74_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n36_), .c(new_n57_), .O(z06));
  nand2  g42(.a(new_n73_), .b(x11), .O(new_n77_));
  nor4   g43(.a(new_n65_), .b(x11), .c(x10), .d(x09), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n43_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n54_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n57_), .O(z07));
  nand2  g48(.a(new_n56_), .b(new_n50_), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand3  g50(.a(new_n72_), .b(x16), .c(new_n84_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n65_), .c(x12), .O(new_n86_));
  inv1   g52(.a(new_n65_), .O(new_n87_));
  nand4  g53(.a(new_n72_), .b(new_n87_), .c(new_n35_), .d(new_n84_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n86_), .c(new_n83_), .d(x04), .O(z08));
  oai21  g55(.a(new_n49_), .b(x13), .c(x12), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n43_), .O(new_n91_));
  nand2  g57(.a(x13), .b(x12), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n47_), .c(x02), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nor2   g61(.a(new_n78_), .b(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n72_), .b(new_n95_), .c(new_n84_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n65_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n96_), .c(new_n35_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n94_), .c(new_n91_), .O(z09));
  nand2  g66(.a(new_n51_), .b(x00), .O(new_n101_));
  inv1   g67(.a(x10), .O(new_n102_));
  aoi21  g68(.a(new_n56_), .b(x16), .c(x13), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n35_), .c(new_n84_), .d(new_n102_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n101_), .c(x09), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(x08), .c(x07), .d(x06), .O(new_n106_));
  nand3  g72(.a(new_n56_), .b(x16), .c(x14), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n43_), .O(z10));
  nand2  g74(.a(new_n54_), .b(new_n37_), .O(z11));
  aoi21  g75(.a(x16), .b(new_n37_), .c(x03), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n43_), .c(new_n54_), .O(z12));
  nor2   g77(.a(new_n36_), .b(new_n43_), .O(z13));
  nand2  g78(.a(x17), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n54_), .O(z14));
endmodule


