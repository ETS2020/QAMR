// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n98_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  oai21  g06(.a(x06), .b(x05), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n41_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n36_), .c(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z02));
  xor2a  g13(.a(x07), .b(x06), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n45_), .c(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z03));
  xnor2a g16(.a(x08), .b(x07), .O(new_n51_));
  nand2  g17(.a(x08), .b(new_n36_), .O(new_n52_));
  oai21  g18(.a(new_n51_), .b(new_n36_), .c(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n45_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z04));
  oai21  g21(.a(x09), .b(new_n40_), .c(new_n36_), .O(new_n56_));
  inv1   g22(.a(x09), .O(new_n57_));
  aoi21  g23(.a(x08), .b(x07), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n57_), .b(x08), .c(x07), .d(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n45_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n58_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n56_), .O(z05));
  nand2  g28(.a(new_n59_), .b(x10), .O(new_n63_));
  nor2   g29(.a(x10), .b(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n63_), .c(new_n45_), .d(x04), .O(z06));
  nand2  g32(.a(new_n65_), .b(x11), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  inv1   g34(.a(x11), .O(new_n69_));
  nand3  g35(.a(x08), .b(x07), .c(x06), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n69_), .c(new_n68_), .d(new_n57_), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n67_), .c(new_n45_), .d(x04), .O(z07));
  nand2  g39(.a(new_n72_), .b(x12), .O(new_n74_));
  inv1   g40(.a(x12), .O(new_n75_));
  nand4  g41(.a(new_n71_), .b(new_n64_), .c(new_n75_), .d(new_n69_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n74_), .c(new_n45_), .d(x04), .O(z08));
  oai21  g43(.a(x13), .b(new_n40_), .c(new_n36_), .O(new_n78_));
  nand3  g44(.a(new_n57_), .b(x08), .c(x07), .O(new_n79_));
  nand3  g45(.a(new_n75_), .b(new_n69_), .c(new_n68_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n79_), .c(x13), .O(new_n81_));
  inv1   g47(.a(x13), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n75_), .c(new_n69_), .d(new_n68_), .O(new_n83_));
  or2    g49(.a(new_n83_), .b(new_n59_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n81_), .c(new_n45_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n78_), .O(z09));
  inv1   g53(.a(x00), .O(new_n88_));
  nand2  g54(.a(new_n83_), .b(new_n88_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n45_), .c(new_n57_), .d(x08), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(x07), .c(x06), .O(new_n92_));
  inv1   g58(.a(x14), .O(new_n93_));
  or2    g59(.a(new_n45_), .b(new_n93_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n92_), .c(new_n40_), .O(z10));
  nor2   g61(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g62(.a(new_n37_), .O(new_n97_));
  aoi21  g63(.a(x16), .b(new_n35_), .c(x03), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n40_), .c(new_n97_), .O(z12));
  nand2  g65(.a(new_n36_), .b(new_n40_), .O(z13));
  and2   g66(.a(x17), .b(x04), .O(z14));
endmodule


