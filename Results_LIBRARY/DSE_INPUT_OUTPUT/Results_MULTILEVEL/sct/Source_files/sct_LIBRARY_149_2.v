// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n98_, new_n99_, new_n100_, new_n103_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x12), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x06), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n39_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n40_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n49_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n39_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n39_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n49_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n49_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n50_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n39_), .O(z04));
  nand2  g26(.a(new_n50_), .b(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n39_), .O(new_n62_));
  and2   g28(.a(x08), .b(x07), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n38_), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  nor2   g31(.a(new_n38_), .b(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n62_), .O(z05));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(x08), .c(x07), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x12), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand2  g40(.a(new_n63_), .b(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(x12), .c(new_n74_), .d(new_n69_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n73_), .c(new_n62_), .O(z06));
  nor2   g44(.a(x10), .b(x09), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n63_), .c(new_n38_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n49_), .c(x11), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand4  g48(.a(new_n79_), .b(new_n76_), .c(x12), .d(new_n82_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n62_), .O(z07));
  nand3  g50(.a(x12), .b(new_n82_), .c(new_n74_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n70_), .c(x06), .O(new_n86_));
  aoi21  g52(.a(x12), .b(new_n49_), .c(new_n46_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n50_), .O(z08));
  inv1   g54(.a(x13), .O(new_n89_));
  nand4  g55(.a(new_n50_), .b(new_n39_), .c(new_n89_), .d(x04), .O(z09));
  nand4  g56(.a(new_n50_), .b(new_n69_), .c(x08), .d(x07), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n49_), .O(new_n92_));
  nor2   g58(.a(new_n50_), .b(new_n36_), .O(new_n93_));
  aoi21  g59(.a(new_n92_), .b(x00), .c(new_n93_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n46_), .c(new_n39_), .O(z10));
  nand2  g61(.a(new_n39_), .b(new_n40_), .O(z11));
  inv1   g62(.a(x03), .O(new_n97_));
  inv1   g63(.a(x16), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(x02), .c(new_n97_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n39_), .c(x04), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(z12));
  aoi21  g67(.a(new_n38_), .b(x06), .c(new_n46_), .O(z13));
  nand2  g68(.a(x17), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n39_), .O(z14));
endmodule


