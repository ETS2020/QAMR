// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  nor2   g00(.a(x09), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n35_), .O(z01));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x09), .c(x06), .O(z02));
  inv1   g14(.a(x06), .O(new_n49_));
  nand3  g15(.a(x09), .b(x07), .c(new_n49_), .O(new_n50_));
  oai21  g16(.a(x07), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n46_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(new_n35_), .O(new_n54_));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n49_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n49_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n46_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n54_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  and2   g26(.a(x08), .b(x07), .O(new_n61_));
  inv1   g27(.a(new_n46_), .O(new_n62_));
  nand3  g28(.a(new_n60_), .b(x08), .c(x07), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x06), .c(x04), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  oai21  g31(.a(new_n61_), .b(new_n60_), .c(new_n65_), .O(z05));
  nand2  g32(.a(new_n47_), .b(new_n54_), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  nand3  g34(.a(new_n61_), .b(new_n68_), .c(new_n60_), .O(new_n69_));
  oai21  g35(.a(new_n61_), .b(new_n68_), .c(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nand2  g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n67_), .O(z06));
  nand3  g39(.a(new_n68_), .b(x08), .c(x07), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x11), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n61_), .c(new_n60_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand2  g45(.a(x11), .b(x09), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n67_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  aoi21  g48(.a(new_n76_), .b(new_n61_), .c(new_n82_), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand3  g50(.a(new_n82_), .b(new_n84_), .c(new_n68_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n63_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n83_), .c(x06), .O(new_n87_));
  nand2  g53(.a(x12), .b(x09), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n67_), .O(z08));
  oai21  g55(.a(x13), .b(new_n60_), .c(new_n49_), .O(new_n90_));
  oai21  g56(.a(new_n85_), .b(new_n63_), .c(x13), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n82_), .c(new_n84_), .d(new_n68_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n63_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n43_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n91_), .c(new_n90_), .d(new_n46_), .O(z09));
  inv1   g62(.a(x07), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n46_), .c(new_n60_), .d(x08), .O(new_n100_));
  nand2  g66(.a(new_n62_), .b(x14), .O(new_n101_));
  oai21  g67(.a(new_n100_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n54_), .O(z10));
  nand2  g70(.a(new_n54_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n43_), .c(new_n54_), .O(z12));
  nand2  g73(.a(new_n54_), .b(new_n43_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  nor3   g75(.a(new_n35_), .b(new_n109_), .c(new_n43_), .O(z14));
endmodule


