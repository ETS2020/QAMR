// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  nand2  g01(.a(x06), .b(x00), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n36_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n37_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  inv1   g15(.a(x07), .O(new_n50_));
  inv1   g16(.a(x00), .O(new_n51_));
  nand3  g17(.a(new_n50_), .b(x06), .c(new_n51_), .O(new_n52_));
  oai21  g18(.a(new_n50_), .b(x06), .c(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n47_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  nand3  g21(.a(x08), .b(new_n50_), .c(new_n51_), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x07), .O(new_n58_));
  aoi21  g24(.a(new_n58_), .b(new_n56_), .c(new_n46_), .O(new_n59_));
  nor2   g25(.a(new_n57_), .b(x06), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n59_), .c(new_n47_), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n43_), .c(new_n36_), .O(z04));
  inv1   g28(.a(new_n47_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n43_), .c(new_n36_), .O(new_n64_));
  oai21  g30(.a(new_n57_), .b(new_n50_), .c(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n51_), .O(new_n69_));
  nand2  g35(.a(x09), .b(new_n46_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(z05));
  oai21  g37(.a(x10), .b(x06), .c(x00), .O(new_n72_));
  nand2  g38(.a(new_n67_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(x08), .c(x07), .d(x06), .O(new_n75_));
  and2   g41(.a(new_n75_), .b(x04), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n73_), .c(new_n72_), .d(new_n47_), .O(z06));
  oai21  g43(.a(x11), .b(x06), .c(x00), .O(new_n78_));
  nand2  g44(.a(new_n75_), .b(x11), .O(new_n79_));
  nand3  g45(.a(x08), .b(x07), .c(x06), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n66_), .O(new_n83_));
  and2   g49(.a(new_n83_), .b(x04), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n79_), .c(new_n78_), .d(new_n47_), .O(z07));
  oai21  g51(.a(x12), .b(x06), .c(x00), .O(new_n86_));
  nand2  g52(.a(new_n83_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n74_), .b(new_n89_), .c(new_n88_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n81_), .c(new_n43_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n87_), .c(new_n86_), .d(new_n47_), .O(z08));
  nand3  g59(.a(new_n66_), .b(x08), .c(x07), .O(new_n94_));
  inv1   g60(.a(x10), .O(new_n95_));
  nand3  g61(.a(new_n89_), .b(new_n88_), .c(new_n95_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n94_), .c(x13), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand3  g64(.a(new_n82_), .b(new_n98_), .c(new_n89_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n67_), .c(new_n97_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n51_), .O(new_n101_));
  nand2  g67(.a(x13), .b(new_n46_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n101_), .c(new_n64_), .O(z09));
  nand4  g69(.a(new_n47_), .b(new_n98_), .c(new_n89_), .d(new_n88_), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(x10), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n66_), .c(x08), .d(x07), .O(new_n106_));
  nand2  g72(.a(new_n63_), .b(x14), .O(new_n107_));
  oai21  g73(.a(new_n106_), .b(new_n46_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n36_), .O(z10));
  nand2  g76(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g77(.a(x03), .O(new_n112_));
  inv1   g78(.a(x16), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(x02), .c(new_n112_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n36_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  nand2  g82(.a(new_n36_), .b(new_n43_), .O(z13));
  nand2  g83(.a(x17), .b(x04), .O(new_n118_));
  nand2  g84(.a(new_n118_), .b(new_n36_), .O(z14));
endmodule


