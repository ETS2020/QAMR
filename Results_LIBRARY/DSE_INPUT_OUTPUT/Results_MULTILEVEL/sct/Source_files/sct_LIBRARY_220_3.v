// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n109_, new_n112_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  inv1   g02(.a(x11), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  aoi21  g04(.a(new_n35_), .b(x01), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(x14), .b(x01), .c(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n38_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(new_n38_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n46_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n46_), .c(new_n55_), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  and2   g27(.a(x07), .b(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(x08), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n38_), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n60_), .c(new_n48_), .d(x04), .O(z05));
  inv1   g32(.a(new_n48_), .O(new_n67_));
  oai21  g33(.a(x11), .b(x09), .c(x10), .O(new_n68_));
  oai21  g34(.a(new_n67_), .b(new_n43_), .c(new_n68_), .O(new_n69_));
  nand3  g35(.a(new_n63_), .b(new_n37_), .c(x10), .O(new_n70_));
  nand4  g36(.a(new_n62_), .b(new_n36_), .c(new_n61_), .d(x08), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(z06));
  nand2  g38(.a(new_n71_), .b(x11), .O(new_n73_));
  inv1   g39(.a(new_n59_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n37_), .c(new_n36_), .d(new_n61_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n48_), .d(x04), .O(z07));
  oai21  g42(.a(x12), .b(x11), .c(x10), .O(new_n77_));
  oai21  g43(.a(new_n67_), .b(new_n43_), .c(new_n77_), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n36_), .c(new_n61_), .O(new_n80_));
  oai22  g46(.a(new_n80_), .b(new_n59_), .c(new_n79_), .d(new_n36_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n37_), .O(new_n82_));
  nand4  g48(.a(new_n62_), .b(new_n37_), .c(new_n61_), .d(x08), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(x12), .c(new_n36_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n78_), .O(z08));
  oai21  g51(.a(x13), .b(x11), .c(x10), .O(new_n86_));
  nand3  g52(.a(new_n79_), .b(new_n37_), .c(new_n61_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n59_), .c(x13), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n79_), .c(new_n37_), .d(new_n36_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n64_), .c(new_n43_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n88_), .c(new_n86_), .d(new_n48_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  inv1   g60(.a(x03), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x02), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(x16), .c(x09), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(x08), .c(x07), .d(x06), .O(new_n98_));
  nand2  g64(.a(new_n67_), .b(x14), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n94_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n47_), .O(new_n101_));
  nand4  g67(.a(new_n48_), .b(new_n89_), .c(new_n79_), .d(new_n37_), .O(new_n102_));
  nor3   g68(.a(new_n102_), .b(x10), .c(x09), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(x08), .c(x07), .d(x06), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n101_), .c(new_n43_), .O(z10));
  nor2   g71(.a(new_n38_), .b(new_n35_), .O(z11));
  inv1   g72(.a(x16), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(x02), .c(new_n95_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(new_n47_), .c(x04), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(z12));
  nand2  g76(.a(new_n47_), .b(new_n43_), .O(z13));
  nand2  g77(.a(x17), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n47_), .O(z14));
endmodule


