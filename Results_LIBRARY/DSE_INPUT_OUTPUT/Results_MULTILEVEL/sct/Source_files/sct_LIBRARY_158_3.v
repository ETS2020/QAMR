// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n117_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g13(.a(new_n37_), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n35_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(z02));
  inv1   g20(.a(x07), .O(new_n55_));
  nand3  g21(.a(new_n36_), .b(new_n55_), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(x06), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n52_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x07), .O(new_n61_));
  nand3  g27(.a(new_n36_), .b(x08), .c(new_n55_), .O(new_n62_));
  aoi21  g28(.a(new_n62_), .b(new_n61_), .c(new_n35_), .O(new_n63_));
  nor2   g29(.a(new_n60_), .b(x06), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n63_), .c(new_n52_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n45_), .c(new_n48_), .O(z04));
  oai21  g32(.a(new_n51_), .b(new_n45_), .c(new_n48_), .O(new_n67_));
  nor2   g33(.a(new_n60_), .b(new_n55_), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(x10), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nor2   g37(.a(new_n55_), .b(new_n35_), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n36_), .c(new_n71_), .d(x08), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z05));
  nand3  g40(.a(new_n72_), .b(new_n71_), .c(x08), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n52_), .c(new_n36_), .d(x04), .O(z06));
  nand3  g42(.a(new_n71_), .b(x08), .c(x07), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x11), .O(new_n78_));
  nor2   g44(.a(x11), .b(x09), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n72_), .c(x08), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n36_), .O(new_n82_));
  nand2  g48(.a(x11), .b(new_n35_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n67_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  aoi21  g51(.a(new_n79_), .b(new_n68_), .c(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n68_), .b(x06), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  nand3  g54(.a(new_n85_), .b(new_n88_), .c(new_n71_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n86_), .c(new_n36_), .O(new_n91_));
  nand2  g57(.a(x12), .b(new_n35_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n67_), .O(z08));
  inv1   g59(.a(new_n68_), .O(new_n94_));
  oai21  g60(.a(new_n89_), .b(new_n94_), .c(x13), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand3  g62(.a(new_n79_), .b(new_n96_), .c(new_n85_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n87_), .c(new_n95_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n36_), .O(new_n99_));
  nand2  g65(.a(x13), .b(new_n35_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n99_), .c(new_n67_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n49_), .b(x02), .O(new_n103_));
  nand3  g69(.a(new_n96_), .b(new_n85_), .c(new_n88_), .O(new_n104_));
  aoi22  g70(.a(new_n104_), .b(new_n102_), .c(new_n103_), .d(x16), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n71_), .c(x08), .d(x07), .O(new_n106_));
  nand2  g72(.a(new_n51_), .b(x14), .O(new_n107_));
  oai21  g73(.a(new_n106_), .b(new_n35_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n36_), .O(new_n109_));
  nand3  g75(.a(new_n51_), .b(x14), .c(new_n35_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n45_), .O(z10));
  nand2  g77(.a(new_n48_), .b(new_n38_), .O(z11));
  oai21  g78(.a(new_n50_), .b(x02), .c(new_n49_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(new_n48_), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z12));
  nand2  g81(.a(new_n48_), .b(new_n45_), .O(z13));
  nand2  g82(.a(x17), .b(x04), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n48_), .O(z14));
endmodule


