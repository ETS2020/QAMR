// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n109_, new_n112_;
  inv1   g00(.a(x01), .O(new_n35_));
  oai21  g01(.a(x02), .b(new_n35_), .c(x14), .O(z00));
  inv1   g02(.a(x02), .O(new_n37_));
  nor2   g03(.a(x14), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n38_), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x06), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(x04), .c(new_n45_), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x14), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(z02));
  inv1   g18(.a(new_n38_), .O(new_n53_));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  aoi21  g20(.a(new_n45_), .b(x02), .c(new_n50_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(z03));
  xnor2a g24(.a(x08), .b(x07), .O(new_n59_));
  nand2  g25(.a(x08), .b(new_n46_), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n46_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n56_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n53_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  inv1   g35(.a(x14), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n45_), .c(x02), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x16), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n69_), .O(z05));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(x08), .c(x07), .d(x06), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n53_), .c(x04), .O(new_n76_));
  aoi21  g42(.a(new_n67_), .b(x10), .c(new_n76_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n56_), .O(z06));
  nand2  g44(.a(new_n75_), .b(x11), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  inv1   g47(.a(new_n64_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(new_n66_), .O(new_n83_));
  and2   g49(.a(new_n83_), .b(new_n53_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n79_), .c(new_n56_), .d(x04), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nand4  g52(.a(new_n74_), .b(new_n82_), .c(new_n86_), .d(new_n81_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n53_), .c(x04), .O(new_n88_));
  aoi21  g54(.a(new_n83_), .b(x12), .c(new_n88_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n56_), .O(z08));
  nand2  g56(.a(new_n87_), .b(x13), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n86_), .c(new_n81_), .d(new_n80_), .O(new_n93_));
  or2    g59(.a(new_n93_), .b(new_n67_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n91_), .c(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n53_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n72_), .O(z09));
  oai21  g63(.a(x16), .b(x02), .c(new_n70_), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n93_), .b(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n98_), .c(new_n66_), .d(x08), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(x07), .c(x06), .O(new_n103_));
  nand2  g69(.a(new_n55_), .b(x14), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n42_), .O(z10));
  nor2   g71(.a(new_n70_), .b(new_n37_), .O(z11));
  aoi21  g72(.a(x04), .b(x03), .c(new_n70_), .O(new_n107_));
  nand2  g73(.a(new_n50_), .b(new_n45_), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(x04), .c(new_n37_), .O(new_n109_));
  oai21  g75(.a(new_n107_), .b(new_n37_), .c(new_n109_), .O(z12));
  nand2  g76(.a(new_n53_), .b(new_n42_), .O(z13));
  inv1   g77(.a(x17), .O(new_n112_));
  nor3   g78(.a(new_n38_), .b(new_n112_), .c(new_n42_), .O(z14));
endmodule


