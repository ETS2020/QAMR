// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x06), .b(x04), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x04), .O(new_n39_));
  inv1   g05(.a(x06), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x05), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x16), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x02), .c(new_n46_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n40_), .c(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(new_n36_), .O(new_n54_));
  xor2a  g20(.a(x08), .b(x07), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(x06), .c(x04), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n40_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n54_), .O(z04));
  inv1   g26(.a(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x08), .c(x07), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x06), .O(new_n64_));
  and2   g30(.a(x08), .b(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(x09), .c(new_n47_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n39_), .c(new_n64_), .O(z05));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n65_), .c(new_n39_), .O(new_n70_));
  nand4  g36(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(x10), .c(new_n47_), .O(new_n72_));
  oai22  g38(.a(new_n72_), .b(new_n39_), .c(new_n70_), .d(new_n40_), .O(z06));
  nor3   g39(.a(x11), .b(x10), .c(x09), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n65_), .c(new_n39_), .O(new_n75_));
  nand4  g41(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(x11), .c(new_n47_), .O(new_n77_));
  oai22  g43(.a(new_n77_), .b(new_n39_), .c(new_n75_), .d(new_n40_), .O(z07));
  inv1   g44(.a(x10), .O(new_n79_));
  nor2   g45(.a(x12), .b(x11), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n62_), .c(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x06), .O(new_n83_));
  inv1   g49(.a(new_n66_), .O(new_n84_));
  nand2  g50(.a(new_n74_), .b(new_n84_), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(x12), .c(new_n47_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n39_), .c(new_n83_), .O(z08));
  nor2   g53(.a(x11), .b(x10), .O(new_n88_));
  nor2   g54(.a(x13), .b(x12), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n62_), .c(x04), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x06), .O(new_n92_));
  nand3  g58(.a(new_n80_), .b(new_n69_), .c(new_n84_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(x13), .c(new_n47_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n39_), .c(new_n92_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  aoi21  g62(.a(new_n90_), .b(new_n96_), .c(new_n47_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n61_), .c(x08), .d(x07), .O(new_n98_));
  nand2  g64(.a(new_n47_), .b(x14), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n40_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n54_), .O(z10));
  nor2   g68(.a(new_n36_), .b(new_n35_), .O(z11));
  nand2  g69(.a(x16), .b(new_n35_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n45_), .c(new_n39_), .O(z12));
  and2   g71(.a(x17), .b(x04), .O(z14));
  buf    g72(.a(x04), .O(z13));
endmodule


