// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n111_;
  nor2   g00(.a(x10), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n35_), .O(z01));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x10), .c(x06), .O(z02));
  inv1   g14(.a(new_n35_), .O(new_n49_));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(z03));
  inv1   g18(.a(x06), .O(new_n53_));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand3  g20(.a(x10), .b(x08), .c(new_n53_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n46_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  inv1   g24(.a(x10), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x09), .c(new_n53_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  and2   g28(.a(x08), .b(x07), .O(new_n63_));
  nand3  g29(.a(new_n62_), .b(x08), .c(x07), .O(new_n64_));
  oai21  g30(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  nand3  g32(.a(x10), .b(x09), .c(new_n53_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(z05));
  nand2  g34(.a(new_n47_), .b(new_n49_), .O(new_n69_));
  inv1   g35(.a(x07), .O(new_n70_));
  nor2   g36(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n62_), .c(x08), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  nand4  g39(.a(new_n71_), .b(new_n59_), .c(new_n62_), .d(x08), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n69_), .O(z06));
  nand2  g41(.a(new_n64_), .b(x11), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nand4  g43(.a(new_n63_), .b(new_n77_), .c(new_n59_), .d(new_n62_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x06), .O(new_n80_));
  nand2  g46(.a(x11), .b(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n69_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  nor2   g49(.a(x11), .b(x09), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n63_), .c(new_n83_), .O(new_n85_));
  nor4   g51(.a(new_n64_), .b(x12), .c(x11), .d(x10), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n85_), .c(x06), .O(new_n87_));
  nand2  g53(.a(x12), .b(x10), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n69_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nor3   g56(.a(x12), .b(x11), .c(x09), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n63_), .c(new_n90_), .O(new_n92_));
  nand4  g58(.a(new_n90_), .b(new_n83_), .c(new_n77_), .d(new_n59_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n92_), .c(x06), .O(new_n95_));
  nand2  g61(.a(x13), .b(x10), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n95_), .c(new_n69_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n46_), .c(new_n62_), .d(x08), .O(new_n100_));
  inv1   g66(.a(new_n46_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x14), .O(new_n102_));
  oai21  g68(.a(new_n100_), .b(new_n70_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x06), .O(new_n104_));
  nand3  g70(.a(new_n101_), .b(x14), .c(x10), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n43_), .O(z10));
  nor2   g72(.a(new_n35_), .b(new_n36_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n36_), .c(x03), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n43_), .c(new_n49_), .O(z12));
  nor2   g75(.a(new_n35_), .b(new_n43_), .O(z13));
  inv1   g76(.a(x17), .O(new_n111_));
  nor3   g77(.a(new_n35_), .b(new_n111_), .c(new_n43_), .O(z14));
endmodule


