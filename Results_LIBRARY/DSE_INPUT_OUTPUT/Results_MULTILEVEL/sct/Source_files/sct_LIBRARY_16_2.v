// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n101_;
  inv1   g00(.a(x07), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi22  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .d(x04), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x07), .b(new_n40_), .c(new_n43_), .O(new_n44_));
  oai22  g10(.a(new_n44_), .b(new_n42_), .c(new_n40_), .d(x04), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x07), .c(new_n43_), .O(z02));
  nand4  g17(.a(new_n49_), .b(x07), .c(new_n46_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  nand3  g19(.a(x08), .b(x07), .c(new_n46_), .O(new_n54_));
  oai21  g20(.a(x08), .b(new_n46_), .c(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  aoi21  g22(.a(new_n56_), .b(x07), .c(new_n43_), .O(z04));
  nand2  g23(.a(x08), .b(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x09), .O(new_n59_));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x08), .c(x06), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n35_), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n59_), .c(new_n49_), .d(x04), .O(z05));
  nand2  g30(.a(new_n61_), .b(x10), .O(new_n65_));
  inv1   g31(.a(x10), .O(new_n66_));
  inv1   g32(.a(new_n58_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n60_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n65_), .c(new_n49_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x07), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x04), .O(z06));
  nand2  g37(.a(new_n68_), .b(x11), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand4  g39(.a(new_n67_), .b(new_n73_), .c(new_n66_), .d(new_n60_), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(x07), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n49_), .d(x04), .O(z07));
  nand2  g42(.a(new_n74_), .b(x12), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n73_), .c(new_n66_), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n61_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n35_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n77_), .c(new_n49_), .d(x04), .O(z08));
  oai21  g48(.a(new_n79_), .b(new_n61_), .c(x13), .O(new_n83_));
  inv1   g49(.a(x13), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n78_), .c(new_n73_), .d(new_n66_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n62_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n83_), .c(new_n49_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x07), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x04), .O(z09));
  inv1   g56(.a(x00), .O(new_n91_));
  nand2  g57(.a(new_n85_), .b(new_n91_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n49_), .c(new_n60_), .d(x08), .O(new_n93_));
  nand3  g59(.a(new_n48_), .b(x16), .c(x14), .O(new_n94_));
  oai21  g60(.a(new_n93_), .b(new_n46_), .c(new_n94_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(x07), .c(x04), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(z10));
  oai21  g63(.a(x07), .b(new_n43_), .c(new_n36_), .O(z11));
  aoi21  g64(.a(x16), .b(new_n36_), .c(new_n35_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n47_), .c(new_n43_), .O(z12));
  inv1   g66(.a(x17), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x07), .c(new_n43_), .O(z14));
  buf    g68(.a(x04), .O(z13));
endmodule


