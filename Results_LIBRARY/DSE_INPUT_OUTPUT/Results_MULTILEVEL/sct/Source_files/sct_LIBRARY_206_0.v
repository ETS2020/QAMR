// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x11), .b(x06), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(x11), .c(x06), .O(z02));
  inv1   g13(.a(x06), .O(new_n48_));
  nand3  g14(.a(x11), .b(x07), .c(new_n48_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(new_n36_), .O(new_n53_));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n48_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n48_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n45_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(z04));
  inv1   g24(.a(x11), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x09), .c(new_n48_), .O(new_n60_));
  inv1   g26(.a(x07), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(x09), .O(new_n63_));
  nor2   g29(.a(new_n61_), .b(new_n48_), .O(new_n64_));
  nor2   g30(.a(x09), .b(new_n62_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n64_), .c(new_n42_), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n63_), .c(new_n60_), .d(new_n45_), .O(z05));
  oai21  g33(.a(new_n59_), .b(x10), .c(new_n48_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(x08), .c(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nor2   g39(.a(new_n62_), .b(new_n61_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n70_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand3  g43(.a(x11), .b(x10), .c(new_n48_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n69_), .O(z06));
  nand2  g45(.a(new_n75_), .b(x11), .O(new_n80_));
  nor3   g46(.a(x11), .b(x10), .c(x09), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n74_), .c(new_n48_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n80_), .c(new_n45_), .d(x04), .O(z07));
  nand2  g49(.a(new_n46_), .b(new_n53_), .O(new_n84_));
  and2   g50(.a(new_n75_), .b(x12), .O(new_n85_));
  nor4   g51(.a(new_n71_), .b(x12), .c(x11), .d(x10), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n85_), .c(x06), .O(new_n87_));
  nand2  g53(.a(x12), .b(x11), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n84_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nor3   g56(.a(x12), .b(x10), .c(x09), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n74_), .c(new_n90_), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand4  g59(.a(new_n90_), .b(new_n93_), .c(new_n59_), .d(new_n73_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n71_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n92_), .c(x06), .O(new_n96_));
  nand2  g62(.a(x13), .b(x11), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n96_), .c(new_n84_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n94_), .b(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n45_), .c(new_n70_), .d(x08), .O(new_n101_));
  inv1   g67(.a(new_n45_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x14), .O(new_n103_));
  oai21  g69(.a(new_n101_), .b(new_n61_), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x06), .O(new_n105_));
  nand3  g71(.a(new_n102_), .b(x14), .c(x11), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n42_), .O(z10));
  nand2  g73(.a(new_n53_), .b(new_n35_), .O(z11));
  inv1   g74(.a(x03), .O(new_n109_));
  inv1   g75(.a(x16), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(x02), .c(new_n109_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n53_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  nor2   g79(.a(new_n36_), .b(new_n42_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n53_), .O(z14));
endmodule


