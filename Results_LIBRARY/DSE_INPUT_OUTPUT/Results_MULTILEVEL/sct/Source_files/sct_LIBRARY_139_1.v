// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n116_;
  inv1   g00(.a(x16), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n36_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  aoi21  g12(.a(new_n35_), .b(new_n44_), .c(x06), .O(z02));
  inv1   g13(.a(x07), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x06), .O(new_n52_));
  oai21  g18(.a(new_n48_), .b(x06), .c(new_n52_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n37_), .O(z03));
  xor2a  g21(.a(x08), .b(x07), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n51_), .c(x06), .O(new_n57_));
  inv1   g23(.a(x06), .O(new_n58_));
  nand3  g24(.a(new_n35_), .b(x08), .c(new_n58_), .O(new_n59_));
  aoi21  g25(.a(new_n59_), .b(new_n57_), .c(new_n44_), .O(z04));
  nor2   g26(.a(x16), .b(x09), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(x06), .c(new_n44_), .O(new_n62_));
  nand2  g28(.a(x08), .b(x07), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n51_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(new_n35_), .b(x09), .c(new_n58_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n62_), .O(z05));
  oai21  g36(.a(x16), .b(x10), .c(new_n58_), .O(new_n71_));
  nand2  g37(.a(new_n66_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(x08), .c(x07), .d(x06), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(x04), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n71_), .d(new_n51_), .O(z06));
  nor2   g42(.a(x16), .b(x11), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(x06), .c(new_n44_), .O(new_n78_));
  inv1   g44(.a(new_n63_), .O(new_n79_));
  nand2  g45(.a(new_n73_), .b(new_n79_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x11), .O(new_n81_));
  inv1   g47(.a(x10), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand4  g49(.a(new_n79_), .b(new_n83_), .c(new_n82_), .d(new_n65_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n81_), .c(new_n51_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand3  g52(.a(new_n35_), .b(x11), .c(new_n58_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n78_), .O(z07));
  oai21  g54(.a(x16), .b(x12), .c(new_n58_), .O(new_n89_));
  nand2  g55(.a(new_n84_), .b(x12), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand3  g57(.a(new_n73_), .b(new_n91_), .c(new_n83_), .O(new_n92_));
  nor3   g58(.a(new_n92_), .b(new_n63_), .c(new_n58_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n44_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(new_n51_), .O(z08));
  oai21  g61(.a(x16), .b(x13), .c(new_n58_), .O(new_n96_));
  nand3  g62(.a(new_n91_), .b(new_n83_), .c(new_n82_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n66_), .c(x13), .O(new_n98_));
  nand4  g64(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n91_), .c(new_n83_), .d(new_n82_), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nor2   g68(.a(new_n102_), .b(new_n44_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n98_), .c(new_n96_), .d(new_n51_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  nand2  g71(.a(new_n101_), .b(new_n105_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n51_), .c(new_n65_), .d(x08), .O(new_n107_));
  nand3  g73(.a(new_n50_), .b(x16), .c(x14), .O(new_n108_));
  oai21  g74(.a(new_n107_), .b(new_n48_), .c(new_n108_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(x06), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z10));
  nor2   g77(.a(new_n36_), .b(new_n39_), .O(z11));
  aoi21  g78(.a(x16), .b(new_n39_), .c(x03), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(new_n44_), .c(new_n37_), .O(z12));
  nor2   g80(.a(new_n36_), .b(new_n44_), .O(z13));
  nand2  g81(.a(x17), .b(x04), .O(new_n116_));
  nand2  g82(.a(new_n116_), .b(new_n37_), .O(z14));
endmodule


