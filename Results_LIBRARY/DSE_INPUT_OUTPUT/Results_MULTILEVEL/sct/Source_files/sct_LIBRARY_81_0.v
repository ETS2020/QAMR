// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_;
  nor2   g00(.a(x08), .b(x04), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x08), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x05), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand2  g19(.a(x08), .b(new_n46_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n46_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n47_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x09), .O(new_n59_));
  inv1   g25(.a(x09), .O(new_n60_));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n60_), .c(x08), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n59_), .c(new_n47_), .d(x04), .O(z05));
  nor2   g29(.a(x10), .b(x09), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(new_n61_), .c(new_n40_), .O(new_n65_));
  inv1   g31(.a(new_n47_), .O(new_n66_));
  aoi21  g32(.a(new_n62_), .b(x10), .c(new_n66_), .O(new_n67_));
  oai22  g33(.a(new_n67_), .b(new_n40_), .c(new_n65_), .d(new_n41_), .O(z06));
  nand3  g34(.a(new_n64_), .b(new_n61_), .c(x08), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x11), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  inv1   g37(.a(x11), .O(new_n72_));
  inv1   g38(.a(new_n58_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n60_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n70_), .c(new_n47_), .d(x04), .O(z07));
  nand3  g41(.a(new_n60_), .b(x07), .c(x06), .O(new_n76_));
  inv1   g42(.a(x12), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n72_), .c(new_n71_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n76_), .c(x04), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x08), .O(new_n80_));
  aoi21  g46(.a(new_n74_), .b(x12), .c(new_n66_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n40_), .c(new_n80_), .O(z08));
  nand3  g48(.a(new_n64_), .b(new_n77_), .c(new_n72_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n58_), .c(x13), .O(new_n84_));
  inv1   g50(.a(x13), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n77_), .c(new_n72_), .d(new_n71_), .O(new_n86_));
  or2    g52(.a(new_n86_), .b(new_n62_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n84_), .c(new_n47_), .d(x04), .O(z09));
  inv1   g54(.a(x00), .O(new_n89_));
  nand2  g55(.a(new_n86_), .b(new_n89_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n47_), .c(new_n60_), .d(x08), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(x07), .c(x06), .O(new_n93_));
  nand2  g59(.a(new_n66_), .b(x14), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n93_), .c(new_n40_), .O(z10));
  inv1   g61(.a(new_n35_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n36_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n36_), .c(x03), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n40_), .c(new_n96_), .O(z12));
  nand2  g65(.a(x17), .b(x04), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n96_), .O(z14));
  buf    g67(.a(x04), .O(z13));
endmodule


