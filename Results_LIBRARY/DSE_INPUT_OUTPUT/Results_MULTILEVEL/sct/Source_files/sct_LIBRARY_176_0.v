// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n112_;
  nor2   g00(.a(x11), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n35_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(x11), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  nand3  g14(.a(x11), .b(x07), .c(new_n45_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n45_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand3  g19(.a(x11), .b(x08), .c(new_n45_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n46_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  inv1   g23(.a(x11), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(x09), .c(new_n45_), .O(new_n59_));
  nand2  g25(.a(x08), .b(x07), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  nand2  g27(.a(x07), .b(x06), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  inv1   g29(.a(x08), .O(new_n64_));
  nor2   g30(.a(x09), .b(new_n64_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n63_), .c(new_n42_), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n61_), .c(new_n59_), .d(new_n46_), .O(z05));
  oai21  g33(.a(new_n58_), .b(x10), .c(new_n45_), .O(new_n68_));
  nand2  g34(.a(new_n65_), .b(x07), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand4  g38(.a(new_n63_), .b(new_n72_), .c(new_n71_), .d(x08), .O(new_n73_));
  and2   g39(.a(new_n73_), .b(x04), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n70_), .c(new_n68_), .d(new_n46_), .O(z06));
  inv1   g41(.a(new_n35_), .O(new_n76_));
  inv1   g42(.a(new_n46_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n42_), .c(new_n76_), .O(new_n78_));
  nand2  g44(.a(new_n73_), .b(x11), .O(new_n79_));
  inv1   g45(.a(new_n60_), .O(new_n80_));
  nand3  g46(.a(new_n58_), .b(new_n72_), .c(new_n71_), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(x06), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(z07));
  oai21  g50(.a(x12), .b(new_n58_), .c(new_n45_), .O(new_n85_));
  oai21  g51(.a(new_n81_), .b(new_n60_), .c(x12), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(new_n69_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n87_), .c(new_n58_), .d(new_n72_), .O(new_n89_));
  and2   g55(.a(new_n89_), .b(x04), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n86_), .c(new_n85_), .d(new_n46_), .O(z08));
  oai21  g57(.a(x13), .b(new_n58_), .c(new_n45_), .O(new_n92_));
  nand2  g58(.a(new_n89_), .b(x13), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n87_), .c(new_n58_), .d(new_n72_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n88_), .c(new_n42_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n93_), .c(new_n92_), .d(new_n46_), .O(z09));
  inv1   g64(.a(x07), .O(new_n99_));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n95_), .b(new_n100_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n46_), .c(new_n71_), .d(x08), .O(new_n102_));
  nand2  g68(.a(new_n77_), .b(x14), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n99_), .c(new_n103_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x06), .O(new_n105_));
  nand3  g71(.a(new_n77_), .b(x14), .c(x11), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n42_), .O(z10));
  nand2  g73(.a(new_n76_), .b(new_n36_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n36_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n42_), .c(new_n76_), .O(z12));
  nand2  g76(.a(new_n76_), .b(new_n42_), .O(z13));
  inv1   g77(.a(x17), .O(new_n112_));
  nor3   g78(.a(new_n35_), .b(new_n112_), .c(new_n42_), .O(z14));
endmodule


