// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n36_), .O(z11));
  inv1   g05(.a(x06), .O(new_n40_));
  nand3  g06(.a(new_n37_), .b(new_n40_), .c(new_n35_), .O(new_n41_));
  oai21  g07(.a(z11), .b(new_n35_), .c(new_n41_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n38_), .O(z01));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n40_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n38_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n38_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n40_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n40_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n49_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n38_), .O(z04));
  nand2  g25(.a(new_n49_), .b(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n38_), .O(new_n61_));
  inv1   g27(.a(x07), .O(new_n62_));
  inv1   g28(.a(x08), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n37_), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nor2   g33(.a(new_n62_), .b(new_n40_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x14), .c(new_n67_), .d(x08), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(new_n61_), .O(z05));
  nor2   g36(.a(x09), .b(new_n63_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(x07), .c(new_n37_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n40_), .c(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand2  g40(.a(new_n64_), .b(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(x14), .c(new_n74_), .d(new_n67_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n73_), .c(new_n61_), .O(z06));
  nor2   g44(.a(x10), .b(x09), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n64_), .c(new_n37_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n40_), .c(x11), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand4  g48(.a(new_n79_), .b(new_n76_), .c(x14), .d(new_n82_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n61_), .O(z07));
  nand2  g50(.a(new_n71_), .b(x07), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n82_), .c(new_n74_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n85_), .c(x14), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x06), .O(new_n89_));
  nand2  g55(.a(new_n83_), .b(x12), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n49_), .d(x04), .O(z08));
  nand2  g57(.a(new_n89_), .b(x13), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(x14), .b(new_n93_), .c(new_n86_), .O(new_n94_));
  nor3   g60(.a(new_n94_), .b(x11), .c(x10), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n71_), .c(new_n68_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n92_), .c(new_n61_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand4  g64(.a(new_n93_), .b(new_n86_), .c(new_n82_), .d(new_n74_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(x09), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(x08), .c(x07), .d(x06), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n49_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(x14), .c(x04), .O(new_n103_));
  inv1   g69(.a(new_n103_), .O(z10));
  inv1   g70(.a(x03), .O(new_n105_));
  inv1   g71(.a(x16), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(x02), .c(new_n105_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n38_), .c(x04), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(z12));
  nand2  g75(.a(new_n38_), .b(new_n46_), .O(z13));
  nand2  g76(.a(new_n38_), .b(x17), .O(new_n111_));
  nor2   g77(.a(new_n111_), .b(new_n46_), .O(z14));
endmodule


