// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n117_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x15), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n37_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n35_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n37_), .O(new_n53_));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n53_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  xor2a  g23(.a(x08), .b(x07), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n36_), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n57_), .b(x06), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nor2   g28(.a(x09), .b(new_n57_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n36_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  inv1   g32(.a(x07), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(new_n35_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n36_), .c(x08), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(x09), .c(new_n44_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n66_), .c(new_n50_), .O(z05));
  nor2   g37(.a(new_n57_), .b(new_n67_), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n36_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  inv1   g42(.a(x09), .O(new_n77_));
  nand4  g43(.a(new_n68_), .b(new_n36_), .c(new_n77_), .d(x08), .O(new_n78_));
  aoi21  g44(.a(new_n78_), .b(x10), .c(new_n44_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n76_), .c(new_n50_), .O(z06));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n72_), .c(new_n77_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n36_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nand2  g50(.a(new_n72_), .b(x06), .O(new_n85_));
  inv1   g51(.a(x10), .O(new_n86_));
  nand3  g52(.a(new_n36_), .b(new_n86_), .c(new_n77_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n85_), .c(x11), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n84_), .c(new_n50_), .d(x04), .O(z07));
  inv1   g55(.a(x11), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n86_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n64_), .c(new_n36_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  nand3  g60(.a(new_n73_), .b(new_n36_), .c(new_n90_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n85_), .c(x12), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n94_), .c(new_n50_), .d(x04), .O(z08));
  oai21  g63(.a(new_n49_), .b(new_n44_), .c(new_n53_), .O(new_n98_));
  nand2  g64(.a(new_n94_), .b(x13), .O(new_n99_));
  nor3   g65(.a(x15), .b(x13), .c(x12), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n81_), .c(new_n68_), .d(new_n63_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  nor2   g69(.a(x13), .b(x12), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n81_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n103_), .c(new_n49_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n77_), .c(x08), .d(x07), .O(new_n107_));
  nand2  g73(.a(new_n49_), .b(x14), .O(new_n108_));
  oai21  g74(.a(new_n107_), .b(new_n35_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n36_), .O(new_n110_));
  nand3  g76(.a(new_n49_), .b(x14), .c(new_n35_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n44_), .O(z10));
  nand2  g78(.a(new_n53_), .b(new_n38_), .O(z11));
  aoi21  g79(.a(x16), .b(new_n38_), .c(x03), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(new_n44_), .c(new_n53_), .O(z12));
  nand2  g81(.a(new_n53_), .b(new_n44_), .O(z13));
  nand3  g82(.a(new_n53_), .b(x17), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z14));
endmodule


