// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x05), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x03), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x03), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(new_n41_), .c(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n35_), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n43_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n41_), .b(x02), .c(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n48_), .b(x03), .O(new_n50_));
  oai21  g16(.a(new_n49_), .b(x05), .c(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n47_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n47_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n51_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(new_n36_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  nand2  g34(.a(new_n35_), .b(new_n37_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n41_), .c(new_n48_), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n68_), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(x08), .c(x07), .d(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x04), .O(new_n75_));
  aoi21  g41(.a(new_n66_), .b(x10), .c(new_n75_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n36_), .c(new_n71_), .O(z06));
  inv1   g43(.a(new_n49_), .O(new_n78_));
  nand2  g44(.a(new_n74_), .b(x11), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  inv1   g47(.a(new_n63_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(new_n65_), .O(new_n83_));
  and2   g49(.a(new_n83_), .b(new_n62_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n79_), .c(new_n78_), .d(x04), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nand4  g52(.a(new_n73_), .b(new_n82_), .c(new_n86_), .d(new_n81_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n62_), .c(x04), .O(new_n88_));
  aoi21  g54(.a(new_n83_), .b(x12), .c(new_n88_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n78_), .O(z08));
  nand2  g56(.a(new_n87_), .b(x13), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n86_), .c(new_n81_), .d(new_n80_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n36_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n91_), .c(new_n78_), .d(x04), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n93_), .b(new_n97_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n51_), .c(new_n65_), .d(x08), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(x07), .c(x06), .O(new_n101_));
  nand2  g67(.a(new_n70_), .b(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n42_), .O(z10));
  nand2  g69(.a(new_n62_), .b(new_n37_), .O(z11));
  nand3  g70(.a(x16), .b(new_n35_), .c(new_n37_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n41_), .c(new_n42_), .O(z12));
  nand2  g72(.a(new_n62_), .b(new_n42_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  nor3   g74(.a(new_n36_), .b(new_n108_), .c(new_n42_), .O(z14));
endmodule


