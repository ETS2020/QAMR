// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x11), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x09), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n36_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(new_n36_), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n37_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n49_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n48_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n48_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n50_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n49_), .O(z04));
  nand2  g26(.a(new_n50_), .b(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n49_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  inv1   g31(.a(x07), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(new_n48_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n35_), .c(new_n65_), .d(x08), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n64_), .c(new_n62_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand3  g36(.a(new_n67_), .b(new_n70_), .c(x08), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(x11), .c(new_n65_), .O(new_n73_));
  aoi21  g39(.a(new_n68_), .b(x10), .c(new_n45_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n50_), .O(z06));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n67_), .c(x08), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n50_), .c(new_n35_), .d(x04), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  inv1   g45(.a(new_n63_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n70_), .d(new_n65_), .O(new_n81_));
  oai21  g47(.a(new_n72_), .b(new_n79_), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n35_), .O(new_n83_));
  nand2  g49(.a(x12), .b(x09), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n62_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n79_), .c(new_n70_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n63_), .c(new_n35_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n65_), .O(new_n89_));
  nand3  g55(.a(new_n76_), .b(new_n79_), .c(new_n35_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n63_), .c(x13), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n89_), .c(new_n50_), .d(x04), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n87_), .b(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n50_), .c(new_n65_), .d(x08), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n66_), .O(new_n96_));
  nor2   g62(.a(new_n50_), .b(new_n40_), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(x06), .c(new_n97_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n45_), .c(new_n49_), .O(z10));
  nand2  g65(.a(new_n49_), .b(new_n37_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n37_), .c(x03), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n45_), .c(new_n49_), .O(z12));
  nor2   g68(.a(new_n36_), .b(new_n45_), .O(z13));
  nand2  g69(.a(x17), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n49_), .O(z14));
endmodule


