// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n75_, new_n78_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x08), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n39_), .O(z01));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n36_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n39_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n39_), .O(z03));
  nand2  g22(.a(x08), .b(new_n36_), .O(new_n57_));
  nand3  g23(.a(new_n37_), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n51_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n51_), .b(new_n39_), .c(new_n62_), .d(x04), .O(z05));
  inv1   g29(.a(x10), .O(new_n64_));
  nand4  g30(.a(new_n51_), .b(new_n39_), .c(new_n64_), .d(x04), .O(z06));
  inv1   g31(.a(x11), .O(new_n66_));
  nand4  g32(.a(new_n51_), .b(new_n39_), .c(new_n66_), .d(x04), .O(z07));
  nor2   g33(.a(x12), .b(new_n46_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n51_), .c(new_n38_), .O(z08));
  nor2   g35(.a(x13), .b(new_n46_), .O(new_n70_));
  aoi21  g36(.a(new_n70_), .b(new_n51_), .c(new_n38_), .O(z09));
  nand4  g37(.a(new_n50_), .b(x16), .c(x14), .d(x04), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n39_), .O(z10));
  nand2  g39(.a(new_n39_), .b(new_n40_), .O(z11));
  aoi21  g40(.a(x16), .b(new_n40_), .c(x03), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n46_), .c(new_n39_), .O(z12));
  nor2   g42(.a(new_n38_), .b(new_n46_), .O(z13));
  nand2  g43(.a(x17), .b(x04), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n39_), .O(z14));
endmodule


