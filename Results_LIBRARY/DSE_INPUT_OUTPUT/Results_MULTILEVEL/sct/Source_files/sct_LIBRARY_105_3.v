// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n121_;
  inv1   g00(.a(x08), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  aoi21  g12(.a(x05), .b(new_n46_), .c(new_n37_), .O(new_n47_));
  oai21  g13(.a(new_n45_), .b(x05), .c(new_n47_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(new_n37_), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n38_), .c(x16), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand2  g19(.a(x10), .b(x08), .O(new_n54_));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(new_n54_), .c(new_n51_), .d(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand2  g23(.a(new_n35_), .b(x07), .O(new_n58_));
  inv1   g24(.a(x07), .O(new_n59_));
  nor2   g25(.a(x10), .b(new_n35_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n58_), .c(new_n49_), .O(new_n62_));
  nand2  g28(.a(new_n60_), .b(new_n49_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n62_), .c(new_n51_), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(new_n46_), .O(z04));
  nor2   g32(.a(new_n59_), .b(new_n49_), .O(new_n67_));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n36_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x08), .O(new_n71_));
  nand2  g37(.a(new_n67_), .b(new_n60_), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(x09), .c(new_n46_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n51_), .O(z05));
  inv1   g40(.a(x09), .O(new_n75_));
  nand3  g41(.a(new_n67_), .b(new_n75_), .c(x08), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n51_), .c(x04), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n36_), .O(new_n78_));
  oai21  g44(.a(new_n36_), .b(x08), .c(new_n78_), .O(z06));
  inv1   g45(.a(x11), .O(new_n80_));
  nand3  g46(.a(new_n67_), .b(new_n80_), .c(new_n75_), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(x10), .c(x08), .O(new_n83_));
  nand3  g49(.a(new_n68_), .b(new_n67_), .c(x08), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(x11), .c(new_n46_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n83_), .c(new_n51_), .O(z07));
  nand2  g52(.a(new_n51_), .b(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n50_), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nor2   g55(.a(new_n82_), .b(new_n89_), .O(new_n90_));
  nand3  g56(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nor2   g57(.a(x12), .b(x11), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n75_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n90_), .c(new_n36_), .O(new_n95_));
  nand2  g61(.a(x12), .b(new_n35_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n95_), .c(new_n88_), .O(z08));
  nand3  g63(.a(new_n75_), .b(x07), .c(x06), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n89_), .c(new_n80_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n98_), .c(new_n36_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x08), .O(new_n102_));
  inv1   g68(.a(new_n91_), .O(new_n103_));
  nand3  g69(.a(new_n92_), .b(new_n103_), .c(new_n68_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(x13), .c(new_n46_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n102_), .c(new_n51_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  nand2  g73(.a(new_n100_), .b(new_n107_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n51_), .c(new_n75_), .d(x08), .O(new_n109_));
  nor2   g75(.a(new_n109_), .b(new_n59_), .O(new_n110_));
  nor2   g76(.a(new_n51_), .b(new_n41_), .O(new_n111_));
  aoi21  g77(.a(new_n110_), .b(x06), .c(new_n111_), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(new_n46_), .c(new_n50_), .O(z10));
  nand2  g79(.a(new_n50_), .b(new_n38_), .O(z11));
  inv1   g80(.a(x03), .O(new_n115_));
  inv1   g81(.a(x16), .O(new_n116_));
  oai21  g82(.a(new_n116_), .b(x02), .c(new_n115_), .O(new_n117_));
  nand3  g83(.a(new_n117_), .b(new_n50_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z12));
  nand2  g85(.a(new_n50_), .b(new_n46_), .O(z13));
  nand2  g86(.a(x17), .b(x04), .O(new_n121_));
  nand2  g87(.a(new_n121_), .b(new_n50_), .O(z14));
endmodule


