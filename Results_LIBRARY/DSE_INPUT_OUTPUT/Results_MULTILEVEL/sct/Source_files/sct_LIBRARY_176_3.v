// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n111_, new_n114_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x11), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n37_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n35_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand3  g18(.a(new_n36_), .b(new_n52_), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n36_), .c(x06), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n35_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n49_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n36_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  nor2   g32(.a(new_n52_), .b(new_n35_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n36_), .c(x08), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n66_), .c(new_n49_), .d(x04), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(x08), .c(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n36_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand4  g40(.a(new_n67_), .b(new_n36_), .c(new_n63_), .d(x08), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x10), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n74_), .c(new_n49_), .d(x04), .O(z06));
  nand3  g43(.a(new_n71_), .b(new_n67_), .c(x08), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n49_), .c(x04), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n36_), .O(new_n80_));
  oai21  g46(.a(new_n36_), .b(x06), .c(new_n80_), .O(z07));
  inv1   g47(.a(x10), .O(new_n82_));
  nor2   g48(.a(x12), .b(x11), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n64_), .c(new_n36_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand3  g52(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n36_), .c(new_n82_), .d(new_n63_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(x12), .c(new_n44_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n86_), .c(new_n49_), .O(z08));
  inv1   g57(.a(x12), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n82_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n64_), .c(new_n36_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x06), .O(new_n96_));
  nand3  g62(.a(new_n88_), .b(new_n83_), .c(new_n71_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(x13), .c(new_n44_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(new_n49_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  aoi22  g66(.a(new_n94_), .b(new_n100_), .c(new_n48_), .d(x16), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n63_), .c(x08), .d(x07), .O(new_n102_));
  inv1   g68(.a(new_n49_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x14), .O(new_n104_));
  oai21  g70(.a(new_n102_), .b(new_n35_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n36_), .O(new_n106_));
  nand3  g72(.a(new_n103_), .b(x14), .c(new_n35_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n44_), .O(z10));
  inv1   g74(.a(new_n37_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n38_), .O(z11));
  aoi21  g76(.a(x16), .b(new_n38_), .c(x03), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n44_), .c(new_n109_), .O(z12));
  nand2  g78(.a(new_n109_), .b(new_n44_), .O(z13));
  nand3  g79(.a(new_n109_), .b(x17), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z14));
endmodule


