// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x12), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  oai21  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n36_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n41_), .c(new_n49_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z02));
  xor2a  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n53_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n41_), .O(z03));
  xnor2a g24(.a(x08), .b(x07), .O(new_n59_));
  nand2  g25(.a(x08), .b(new_n49_), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n49_), .c(new_n60_), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(new_n53_), .c(new_n41_), .d(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  inv1   g29(.a(new_n41_), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  and2   g33(.a(x07), .b(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(x08), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(x04), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n53_), .c(new_n64_), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n68_), .c(x08), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n41_), .c(x04), .O(new_n75_));
  aoi21  g41(.a(new_n69_), .b(x10), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n53_), .O(z06));
  inv1   g43(.a(new_n65_), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n67_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x04), .O(new_n81_));
  aoi21  g47(.a(new_n74_), .b(x11), .c(new_n81_), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n53_), .c(new_n64_), .O(z07));
  oai21  g49(.a(new_n52_), .b(new_n46_), .c(new_n41_), .O(new_n84_));
  nand2  g50(.a(new_n80_), .b(x12), .O(new_n85_));
  inv1   g51(.a(new_n69_), .O(new_n86_));
  nand4  g52(.a(new_n79_), .b(new_n86_), .c(new_n37_), .d(new_n36_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(z08));
  and2   g54(.a(new_n80_), .b(new_n37_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(x12), .c(x13), .O(new_n90_));
  nor2   g56(.a(x14), .b(x13), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n79_), .c(new_n36_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n86_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n90_), .c(new_n84_), .O(z09));
  oai21  g61(.a(new_n37_), .b(x12), .c(x00), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n53_), .c(new_n67_), .d(x08), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(x07), .c(x06), .O(new_n100_));
  nand3  g66(.a(new_n52_), .b(x14), .c(x12), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n46_), .O(z10));
  nor2   g68(.a(new_n64_), .b(new_n39_), .O(z11));
  oai21  g69(.a(new_n51_), .b(x02), .c(new_n50_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n41_), .c(x04), .O(new_n105_));
  inv1   g71(.a(new_n105_), .O(z12));
  nor2   g72(.a(new_n64_), .b(new_n46_), .O(z13));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n41_), .O(z14));
endmodule


