// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n93_, new_n94_, new_n95_,
    new_n98_;
  inv1   g00(.a(x06), .O(new_n35_));
  nor2   g01(.a(x10), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n36_), .O(z01));
  inv1   g12(.a(new_n36_), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n35_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nand3  g19(.a(x10), .b(new_n53_), .c(x06), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(x06), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n50_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x07), .O(new_n59_));
  nand3  g25(.a(x10), .b(x08), .c(new_n53_), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(new_n59_), .c(new_n35_), .O(new_n61_));
  nor2   g27(.a(new_n58_), .b(x06), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(new_n50_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n44_), .c(new_n47_), .O(z04));
  inv1   g30(.a(new_n50_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n44_), .c(new_n47_), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  nor2   g33(.a(new_n58_), .b(new_n53_), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n67_), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nor2   g36(.a(new_n67_), .b(x09), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(x08), .c(x07), .d(x06), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z05));
  nand2  g39(.a(new_n71_), .b(new_n68_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  aoi21  g41(.a(x10), .b(new_n35_), .c(new_n44_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n50_), .O(z06));
  nor2   g43(.a(x11), .b(new_n44_), .O(new_n78_));
  aoi21  g44(.a(new_n78_), .b(new_n50_), .c(new_n36_), .O(z07));
  nor2   g45(.a(x12), .b(new_n44_), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n50_), .c(new_n36_), .O(z08));
  nor2   g47(.a(x13), .b(new_n44_), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n50_), .c(new_n36_), .O(z09));
  inv1   g49(.a(x00), .O(new_n84_));
  aoi21  g50(.a(new_n49_), .b(x16), .c(x09), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(x08), .c(x07), .d(x06), .O(new_n86_));
  nand2  g52(.a(new_n65_), .b(x14), .O(new_n87_));
  oai21  g53(.a(new_n86_), .b(new_n84_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x10), .O(new_n89_));
  nand3  g55(.a(new_n65_), .b(x14), .c(new_n35_), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n89_), .c(new_n44_), .O(z10));
  nand2  g57(.a(new_n47_), .b(new_n37_), .O(z11));
  inv1   g58(.a(x16), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(x02), .c(new_n48_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n47_), .c(x04), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(z12));
  nand2  g62(.a(new_n47_), .b(new_n44_), .O(z13));
  nand2  g63(.a(x17), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n47_), .O(z14));
endmodule


