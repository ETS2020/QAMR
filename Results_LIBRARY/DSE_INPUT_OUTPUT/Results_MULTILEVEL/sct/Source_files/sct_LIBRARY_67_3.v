// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x10), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g13(.a(new_n37_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n48_), .O(z02));
  xor2a  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n53_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n48_), .O(z03));
  xnor2a g24(.a(x08), .b(x07), .O(new_n59_));
  nand2  g25(.a(x08), .b(new_n49_), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n49_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n53_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n48_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  and2   g32(.a(x07), .b(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(x08), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(new_n37_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n65_), .c(new_n53_), .d(x04), .O(z05));
  inv1   g37(.a(new_n67_), .O(new_n72_));
  nand3  g38(.a(new_n36_), .b(new_n66_), .c(x08), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n72_), .c(x10), .O(new_n74_));
  nand4  g40(.a(new_n67_), .b(new_n35_), .c(new_n66_), .d(x08), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n74_), .c(new_n53_), .d(x04), .O(z06));
  oai21  g42(.a(x13), .b(x11), .c(x10), .O(new_n77_));
  oai21  g43(.a(new_n52_), .b(new_n45_), .c(new_n77_), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nor2   g45(.a(x11), .b(x09), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n67_), .c(x08), .O(new_n81_));
  oai21  g47(.a(new_n69_), .b(new_n79_), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n35_), .O(new_n83_));
  nand3  g49(.a(new_n36_), .b(x11), .c(x10), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n78_), .O(z07));
  oai21  g51(.a(x13), .b(x12), .c(x10), .O(new_n86_));
  nand2  g52(.a(new_n81_), .b(x12), .O(new_n87_));
  inv1   g53(.a(new_n64_), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n79_), .c(new_n35_), .d(new_n66_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n88_), .c(new_n45_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n87_), .c(new_n86_), .d(new_n53_), .O(z08));
  oai21  g59(.a(new_n52_), .b(new_n45_), .c(new_n48_), .O(new_n94_));
  nand3  g60(.a(new_n89_), .b(new_n79_), .c(new_n66_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n64_), .c(x13), .O(new_n96_));
  nor2   g62(.a(x13), .b(x12), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n80_), .c(new_n88_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n35_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n94_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand3  g68(.a(new_n97_), .b(new_n79_), .c(new_n35_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n52_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n66_), .c(x08), .d(x07), .O(new_n105_));
  nand2  g71(.a(new_n52_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n49_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n48_), .O(z10));
  nor2   g75(.a(new_n37_), .b(new_n38_), .O(z11));
  oai21  g76(.a(new_n51_), .b(x02), .c(new_n50_), .O(new_n111_));
  nand3  g77(.a(new_n111_), .b(new_n48_), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z12));
  nand2  g79(.a(new_n48_), .b(new_n45_), .O(z13));
  nand3  g80(.a(new_n48_), .b(x17), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z14));
endmodule


