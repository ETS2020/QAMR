// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x04), .O(new_n38_));
  nand2  g04(.a(x06), .b(new_n38_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x05), .O(new_n45_));
  nor2   g11(.a(x06), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x04), .c(new_n46_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(x05), .c(new_n38_), .O(new_n49_));
  oai21  g15(.a(new_n47_), .b(new_n44_), .c(new_n49_), .O(z01));
  oai21  g16(.a(x03), .b(new_n40_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n39_), .O(z02));
  nand3  g19(.a(x07), .b(new_n48_), .c(x04), .O(new_n54_));
  oai21  g20(.a(x07), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n39_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n48_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n48_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  oai21  g28(.a(x09), .b(new_n38_), .c(new_n48_), .O(new_n63_));
  nand2  g29(.a(x08), .b(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n51_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n63_), .O(z05));
  oai21  g36(.a(x10), .b(new_n38_), .c(new_n48_), .O(new_n71_));
  nand3  g37(.a(new_n66_), .b(x08), .c(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(x08), .c(x07), .d(x06), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n51_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n71_), .O(z06));
  oai21  g44(.a(x11), .b(new_n38_), .c(new_n48_), .O(new_n79_));
  nand3  g45(.a(new_n74_), .b(x08), .c(x07), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x11), .O(new_n81_));
  nand3  g47(.a(x08), .b(x07), .c(x06), .O(new_n82_));
  inv1   g48(.a(new_n82_), .O(new_n83_));
  nor2   g49(.a(x11), .b(x10), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n66_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n81_), .c(new_n51_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n79_), .O(z07));
  oai21  g54(.a(x12), .b(new_n38_), .c(new_n48_), .O(new_n89_));
  nand2  g55(.a(new_n84_), .b(new_n66_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n64_), .c(x12), .O(new_n91_));
  inv1   g57(.a(x11), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand4  g59(.a(new_n83_), .b(new_n74_), .c(new_n93_), .d(new_n92_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n91_), .c(new_n51_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n89_), .O(z08));
  nand2  g63(.a(new_n94_), .b(x13), .O(new_n98_));
  inv1   g64(.a(x10), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n93_), .c(new_n92_), .d(new_n99_), .O(new_n101_));
  or2    g67(.a(new_n101_), .b(new_n67_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n98_), .c(new_n51_), .d(x04), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n101_), .b(new_n104_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n51_), .c(new_n66_), .d(x08), .O(new_n106_));
  inv1   g72(.a(new_n106_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(x07), .c(x06), .O(new_n108_));
  or2    g74(.a(new_n51_), .b(new_n36_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n38_), .O(z10));
  nand2  g76(.a(new_n39_), .b(new_n40_), .O(z11));
  aoi21  g77(.a(x16), .b(new_n40_), .c(x03), .O(new_n112_));
  nor2   g78(.a(new_n112_), .b(new_n38_), .O(z12));
  inv1   g79(.a(new_n46_), .O(z13));
  and2   g80(.a(x17), .b(x04), .O(z14));
endmodule


