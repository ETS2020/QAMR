// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n92_, new_n93_, new_n96_;
  inv1   g00(.a(x06), .O(new_n35_));
  nor2   g01(.a(x10), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n36_), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n35_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n46_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nand3  g19(.a(x10), .b(new_n53_), .c(x06), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(x06), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n50_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  xor2a  g24(.a(x08), .b(x07), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(x10), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n58_), .b(x06), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n50_), .c(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  oai21  g29(.a(new_n49_), .b(new_n43_), .c(new_n46_), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  nor2   g31(.a(new_n58_), .b(new_n53_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n65_), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  nor2   g34(.a(new_n65_), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(z05));
  nand2  g37(.a(new_n69_), .b(new_n66_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x06), .O(new_n73_));
  aoi21  g39(.a(x10), .b(new_n35_), .c(new_n43_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n50_), .O(z06));
  nor2   g41(.a(x11), .b(new_n43_), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n50_), .c(new_n36_), .O(z07));
  nor2   g43(.a(new_n36_), .b(x12), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n50_), .c(x04), .O(z08));
  nor2   g45(.a(new_n36_), .b(x13), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n50_), .c(x04), .O(z09));
  inv1   g47(.a(x00), .O(new_n82_));
  nand2  g48(.a(new_n47_), .b(x02), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(x16), .c(x09), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(x08), .c(x07), .d(x06), .O(new_n85_));
  nand2  g51(.a(new_n49_), .b(x14), .O(new_n86_));
  oai21  g52(.a(new_n85_), .b(new_n82_), .c(new_n86_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x10), .O(new_n88_));
  nand3  g54(.a(new_n49_), .b(x14), .c(new_n35_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n88_), .c(new_n43_), .O(z10));
  nand2  g56(.a(new_n46_), .b(new_n37_), .O(z11));
  oai21  g57(.a(new_n48_), .b(x02), .c(new_n47_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n46_), .c(x04), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(z12));
  nor2   g60(.a(new_n36_), .b(new_n43_), .O(z13));
  nand2  g61(.a(x17), .b(x04), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n46_), .O(z14));
endmodule


