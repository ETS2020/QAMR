// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n117_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x13), .b(x06), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(x13), .c(x06), .O(z02));
  inv1   g13(.a(x06), .O(new_n48_));
  nand3  g14(.a(x13), .b(x07), .c(new_n48_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n45_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(new_n36_), .O(new_n53_));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n48_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n48_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n45_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(z04));
  inv1   g24(.a(x13), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x09), .c(new_n48_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n46_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand2  g28(.a(x08), .b(x07), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n62_), .b(x08), .c(x07), .O(new_n65_));
  oai21  g31(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g33(.a(x13), .b(x09), .c(new_n48_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n61_), .O(z05));
  oai21  g35(.a(new_n59_), .b(x10), .c(new_n48_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(new_n65_), .O(new_n73_));
  nand3  g39(.a(new_n64_), .b(new_n72_), .c(new_n62_), .O(new_n74_));
  oai21  g40(.a(new_n73_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand3  g42(.a(x13), .b(x10), .c(new_n48_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z06));
  oai21  g44(.a(new_n59_), .b(x11), .c(new_n48_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  and2   g46(.a(new_n74_), .b(x11), .O(new_n81_));
  nor4   g47(.a(new_n63_), .b(x11), .c(x10), .d(x09), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n81_), .c(x06), .O(new_n83_));
  nand3  g49(.a(x13), .b(x11), .c(new_n48_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n80_), .O(z07));
  oai21  g51(.a(new_n59_), .b(x12), .c(new_n48_), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  or2    g53(.a(new_n82_), .b(new_n87_), .O(new_n88_));
  inv1   g54(.a(x11), .O(new_n89_));
  nand4  g55(.a(new_n87_), .b(new_n89_), .c(new_n72_), .d(new_n62_), .O(new_n90_));
  nor3   g56(.a(new_n90_), .b(new_n63_), .c(new_n48_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n42_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n88_), .c(new_n86_), .d(new_n45_), .O(z08));
  nand3  g59(.a(new_n87_), .b(new_n89_), .c(new_n72_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n65_), .c(x13), .O(new_n95_));
  nand4  g61(.a(new_n59_), .b(new_n87_), .c(new_n89_), .d(new_n72_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n73_), .c(new_n48_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n95_), .c(new_n45_), .d(x04), .O(z09));
  inv1   g65(.a(x07), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n96_), .b(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n45_), .c(new_n62_), .d(x08), .O(new_n103_));
  inv1   g69(.a(new_n45_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(x14), .O(new_n105_));
  oai21  g71(.a(new_n103_), .b(new_n100_), .c(new_n105_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x06), .O(new_n107_));
  nand3  g73(.a(new_n104_), .b(x14), .c(x13), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n42_), .O(z10));
  nor2   g75(.a(new_n36_), .b(new_n35_), .O(z11));
  inv1   g76(.a(x03), .O(new_n111_));
  inv1   g77(.a(x16), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(x02), .c(new_n111_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(new_n53_), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z12));
  nor2   g81(.a(new_n36_), .b(new_n42_), .O(z13));
  inv1   g82(.a(x17), .O(new_n117_));
  nor3   g83(.a(new_n36_), .b(new_n117_), .c(new_n42_), .O(z14));
endmodule


