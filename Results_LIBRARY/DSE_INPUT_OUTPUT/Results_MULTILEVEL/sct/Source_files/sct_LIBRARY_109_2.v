// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(x07), .b(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  inv1   g11(.a(new_n37_), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n36_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n46_), .O(z02));
  nand4  g15(.a(new_n47_), .b(x07), .c(new_n36_), .d(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n46_), .O(z03));
  inv1   g17(.a(x08), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x07), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n47_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(x07), .O(new_n57_));
  nor2   g23(.a(x09), .b(new_n52_), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n57_), .c(x06), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(x09), .c(new_n43_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n47_), .O(z05));
  inv1   g28(.a(new_n47_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n43_), .c(new_n46_), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x06), .O(new_n67_));
  oai21  g33(.a(new_n58_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x07), .O(new_n69_));
  nand2  g35(.a(x10), .b(new_n36_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(z06));
  inv1   g37(.a(x11), .O(new_n72_));
  aoi21  g38(.a(new_n66_), .b(x08), .c(new_n72_), .O(new_n73_));
  inv1   g39(.a(x09), .O(new_n74_));
  nand3  g40(.a(new_n72_), .b(new_n65_), .c(new_n74_), .O(new_n75_));
  nor3   g41(.a(new_n75_), .b(new_n52_), .c(new_n36_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n73_), .c(x07), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n36_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n64_), .O(z07));
  inv1   g45(.a(x12), .O(new_n80_));
  nand4  g46(.a(new_n58_), .b(new_n80_), .c(new_n72_), .d(new_n65_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x07), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x06), .O(new_n83_));
  oai21  g49(.a(new_n75_), .b(new_n60_), .c(x12), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n83_), .c(new_n47_), .d(x04), .O(z08));
  and2   g51(.a(new_n81_), .b(x13), .O(new_n86_));
  nand2  g52(.a(new_n58_), .b(x06), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n80_), .c(new_n72_), .d(new_n65_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n86_), .c(x07), .O(new_n91_));
  nand2  g57(.a(x13), .b(new_n36_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n64_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n47_), .c(new_n74_), .d(x08), .O(new_n96_));
  nand2  g62(.a(new_n63_), .b(x14), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n36_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x07), .O(new_n99_));
  nand3  g65(.a(new_n63_), .b(x14), .c(new_n36_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n99_), .c(new_n43_), .O(z10));
  nor2   g67(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n43_), .c(new_n46_), .O(z12));
  nand2  g70(.a(new_n46_), .b(new_n43_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n46_), .O(z14));
endmodule


