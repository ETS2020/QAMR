// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n119_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x14), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n36_), .O(z11));
  inv1   g05(.a(x14), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(x06), .c(new_n35_), .O(new_n41_));
  oai21  g07(.a(z11), .b(new_n35_), .c(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n37_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x14), .c(x06), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n38_), .O(z03));
  inv1   g19(.a(x06), .O(new_n54_));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n54_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n38_), .O(z04));
  oai21  g25(.a(new_n40_), .b(x09), .c(new_n54_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n49_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand2  g28(.a(x08), .b(x07), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n62_), .b(x08), .c(x07), .O(new_n65_));
  oai21  g31(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g33(.a(x14), .b(x09), .c(new_n54_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n61_), .O(z05));
  oai21  g35(.a(new_n40_), .b(x10), .c(new_n54_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n49_), .O(new_n71_));
  nand2  g37(.a(new_n65_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n64_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand3  g42(.a(x14), .b(x10), .c(new_n54_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z06));
  oai21  g44(.a(new_n40_), .b(x11), .c(new_n54_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n49_), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  aoi21  g47(.a(new_n73_), .b(new_n64_), .c(new_n81_), .O(new_n82_));
  inv1   g48(.a(x10), .O(new_n83_));
  nand3  g49(.a(new_n81_), .b(new_n83_), .c(new_n62_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n63_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n82_), .c(x06), .O(new_n86_));
  nand3  g52(.a(x14), .b(x11), .c(new_n54_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n80_), .O(z07));
  oai21  g54(.a(new_n40_), .b(x12), .c(new_n54_), .O(new_n89_));
  oai21  g55(.a(new_n84_), .b(new_n63_), .c(x12), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand3  g57(.a(new_n73_), .b(new_n91_), .c(new_n81_), .O(new_n92_));
  nor3   g58(.a(new_n92_), .b(new_n63_), .c(new_n54_), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n45_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(new_n48_), .O(z08));
  oai21  g61(.a(new_n40_), .b(x13), .c(new_n54_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n49_), .O(new_n97_));
  nand3  g63(.a(new_n91_), .b(new_n81_), .c(new_n83_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n65_), .c(x13), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n91_), .c(new_n81_), .d(new_n83_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n65_), .c(new_n99_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x06), .O(new_n103_));
  nand3  g69(.a(x14), .b(x13), .c(new_n54_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n103_), .c(new_n97_), .O(z09));
  inv1   g71(.a(x00), .O(new_n106_));
  nand2  g72(.a(new_n101_), .b(new_n106_), .O(new_n107_));
  nand4  g73(.a(new_n107_), .b(new_n48_), .c(new_n62_), .d(x08), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(x07), .c(x06), .O(new_n110_));
  or2    g76(.a(new_n48_), .b(new_n40_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n45_), .O(z10));
  inv1   g78(.a(x03), .O(new_n113_));
  inv1   g79(.a(x16), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(x02), .c(new_n113_), .O(new_n115_));
  nand3  g81(.a(new_n115_), .b(new_n38_), .c(x04), .O(new_n116_));
  inv1   g82(.a(new_n116_), .O(z12));
  nand2  g83(.a(new_n38_), .b(new_n45_), .O(z13));
  inv1   g84(.a(x17), .O(new_n119_));
  nor3   g85(.a(new_n37_), .b(new_n119_), .c(new_n45_), .O(z14));
endmodule


