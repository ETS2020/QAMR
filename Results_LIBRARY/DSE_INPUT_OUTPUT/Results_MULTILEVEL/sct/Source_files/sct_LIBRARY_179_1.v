// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n117_;
  inv1   g00(.a(x15), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  nand3  g08(.a(new_n37_), .b(x05), .c(new_n42_), .O(new_n43_));
  nor2   g09(.a(x18), .b(x15), .O(new_n44_));
  aoi21  g10(.a(x15), .b(x06), .c(new_n44_), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  oai21  g12(.a(x03), .b(new_n39_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n35_), .c(x06), .O(z02));
  inv1   g15(.a(x06), .O(new_n50_));
  nand3  g16(.a(new_n35_), .b(x07), .c(new_n50_), .O(new_n51_));
  oai21  g17(.a(x07), .b(new_n50_), .c(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand3  g21(.a(new_n35_), .b(x08), .c(new_n50_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n47_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  oai21  g25(.a(x15), .b(x09), .c(new_n50_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n48_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  and2   g28(.a(x08), .b(x07), .O(new_n63_));
  nand3  g29(.a(new_n62_), .b(x08), .c(x07), .O(new_n64_));
  oai21  g30(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  nand3  g32(.a(new_n35_), .b(x09), .c(new_n50_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(z05));
  oai21  g34(.a(x15), .b(x10), .c(new_n50_), .O(new_n69_));
  nand2  g35(.a(new_n64_), .b(x10), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(x08), .c(x07), .d(x06), .O(new_n72_));
  and2   g38(.a(new_n72_), .b(x04), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n70_), .c(new_n69_), .d(new_n47_), .O(z06));
  oai21  g40(.a(x15), .b(x11), .c(new_n50_), .O(new_n75_));
  nand2  g41(.a(new_n71_), .b(new_n63_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x11), .O(new_n77_));
  nand2  g43(.a(new_n63_), .b(x06), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n62_), .O(new_n82_));
  inv1   g48(.a(new_n82_), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n79_), .c(new_n42_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n77_), .c(new_n75_), .d(new_n47_), .O(z07));
  oai21  g51(.a(x15), .b(x12), .c(new_n50_), .O(new_n86_));
  inv1   g52(.a(new_n63_), .O(new_n87_));
  oai21  g53(.a(new_n82_), .b(new_n87_), .c(x12), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n71_), .b(new_n89_), .c(new_n81_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n79_), .c(new_n42_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n88_), .c(new_n86_), .d(new_n47_), .O(z08));
  oai21  g59(.a(x15), .b(x13), .c(new_n50_), .O(new_n94_));
  nand3  g60(.a(new_n89_), .b(new_n81_), .c(new_n80_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n64_), .c(x13), .O(new_n96_));
  nand4  g62(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n89_), .c(new_n81_), .d(new_n80_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n42_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n96_), .c(new_n94_), .d(new_n47_), .O(z09));
  inv1   g68(.a(x07), .O(new_n103_));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n99_), .b(new_n104_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n47_), .c(new_n62_), .d(x08), .O(new_n106_));
  inv1   g72(.a(new_n47_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x14), .O(new_n108_));
  oai21  g74(.a(new_n106_), .b(new_n103_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x06), .O(new_n110_));
  nand3  g76(.a(new_n107_), .b(new_n35_), .c(x14), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n42_), .O(z10));
  nor2   g78(.a(new_n36_), .b(new_n39_), .O(z11));
  aoi21  g79(.a(x16), .b(new_n39_), .c(x03), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(new_n42_), .c(new_n37_), .O(z12));
  nand2  g81(.a(new_n37_), .b(new_n42_), .O(z13));
  nand2  g82(.a(x17), .b(x04), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n37_), .O(z14));
endmodule


