// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n110_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g03(.a(x13), .b(x07), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n44_), .c(new_n38_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n39_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n38_), .O(z02));
  inv1   g17(.a(x13), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x07), .c(new_n48_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n48_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n48_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n48_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n49_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n38_), .O(z04));
  nand2  g27(.a(new_n49_), .b(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n38_), .O(new_n63_));
  nand2  g29(.a(x08), .b(x06), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x13), .c(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nor2   g33(.a(x13), .b(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n63_), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n52_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x07), .O(new_n74_));
  aoi21  g40(.a(new_n69_), .b(x10), .c(new_n45_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n49_), .O(z06));
  inv1   g42(.a(x09), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n64_), .c(new_n52_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x07), .O(new_n82_));
  nand3  g48(.a(x08), .b(x07), .c(x06), .O(new_n83_));
  nand3  g49(.a(new_n52_), .b(new_n78_), .c(new_n77_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n83_), .c(x11), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n82_), .c(new_n49_), .d(x04), .O(z07));
  nand3  g52(.a(new_n77_), .b(x08), .c(x06), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n79_), .c(new_n78_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n87_), .c(new_n52_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x07), .O(new_n91_));
  nand3  g57(.a(new_n71_), .b(new_n52_), .c(new_n79_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n83_), .c(x12), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n91_), .c(new_n49_), .d(x04), .O(z08));
  inv1   g60(.a(new_n83_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n71_), .c(new_n88_), .d(new_n79_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n49_), .c(new_n52_), .d(x04), .O(z09));
  inv1   g63(.a(x07), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n89_), .b(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n49_), .c(new_n77_), .d(x08), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nor2   g68(.a(new_n49_), .b(new_n36_), .O(new_n103_));
  aoi21  g69(.a(new_n102_), .b(x06), .c(new_n103_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n45_), .c(new_n38_), .O(z10));
  nand2  g71(.a(new_n38_), .b(new_n39_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n39_), .c(x03), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n45_), .c(new_n38_), .O(z12));
  nand2  g74(.a(new_n38_), .b(new_n45_), .O(z13));
  nand3  g75(.a(new_n38_), .b(x17), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z14));
endmodule


