// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_, new_n119_;
  inv1   g00(.a(x05), .O(new_n35_));
  nor2   g01(.a(x06), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n35_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand3  g09(.a(x06), .b(x05), .c(new_n43_), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(new_n42_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n46_), .c(new_n35_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand3  g18(.a(x07), .b(new_n46_), .c(new_n35_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n46_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(new_n36_), .O(new_n57_));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n46_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n46_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n50_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n57_), .O(z04));
  oai21  g28(.a(x09), .b(x05), .c(new_n46_), .O(new_n63_));
  nand2  g29(.a(new_n50_), .b(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand2  g32(.a(x08), .b(x07), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  nand3  g34(.a(new_n66_), .b(x08), .c(x07), .O(new_n69_));
  oai21  g35(.a(new_n68_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nand3  g37(.a(x09), .b(new_n46_), .c(new_n35_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n65_), .O(z05));
  oai21  g39(.a(x10), .b(x05), .c(new_n46_), .O(new_n74_));
  nand2  g40(.a(new_n69_), .b(x10), .O(new_n75_));
  and2   g41(.a(x07), .b(x06), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x08), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n76_), .c(new_n43_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n75_), .c(new_n74_), .d(new_n50_), .O(z06));
  oai21  g47(.a(x11), .b(x05), .c(new_n46_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  aoi21  g50(.a(new_n77_), .b(new_n68_), .c(new_n84_), .O(new_n85_));
  nor4   g51(.a(new_n67_), .b(x11), .c(x10), .d(x09), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n85_), .c(x06), .O(new_n87_));
  nand3  g53(.a(x11), .b(new_n46_), .c(new_n35_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n83_), .O(z07));
  oai21  g55(.a(x12), .b(x05), .c(new_n46_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n64_), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  inv1   g58(.a(x10), .O(new_n93_));
  inv1   g59(.a(new_n69_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n92_), .c(new_n84_), .d(new_n93_), .O(new_n95_));
  oai21  g61(.a(new_n86_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x06), .O(new_n97_));
  nand3  g63(.a(x12), .b(new_n46_), .c(new_n35_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n97_), .c(new_n91_), .O(z08));
  oai21  g65(.a(x13), .b(x05), .c(new_n46_), .O(new_n100_));
  nand2  g66(.a(new_n95_), .b(x13), .O(new_n101_));
  nand3  g67(.a(new_n76_), .b(new_n66_), .c(x08), .O(new_n102_));
  inv1   g68(.a(x13), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n92_), .c(new_n84_), .d(new_n93_), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n43_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n101_), .c(new_n100_), .d(new_n50_), .O(z09));
  inv1   g73(.a(x00), .O(new_n108_));
  aoi21  g74(.a(new_n104_), .b(new_n108_), .c(new_n49_), .O(new_n109_));
  nand4  g75(.a(new_n109_), .b(new_n66_), .c(x08), .d(x07), .O(new_n110_));
  nand2  g76(.a(new_n49_), .b(x14), .O(new_n111_));
  oai21  g77(.a(new_n110_), .b(new_n46_), .c(new_n111_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(x04), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(new_n57_), .O(z10));
  nand2  g80(.a(new_n57_), .b(new_n37_), .O(z11));
  aoi21  g81(.a(x16), .b(new_n37_), .c(x03), .O(new_n116_));
  oai21  g82(.a(new_n116_), .b(new_n43_), .c(new_n57_), .O(z12));
  nand2  g83(.a(new_n57_), .b(new_n43_), .O(z13));
  inv1   g84(.a(x17), .O(new_n119_));
  nor3   g85(.a(new_n36_), .b(new_n119_), .c(new_n43_), .O(z14));
endmodule


