// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  inv1   g00(.a(x09), .O(new_n35_));
  inv1   g01(.a(x11), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n37_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(new_n37_), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n48_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n47_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n47_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n52_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n48_), .O(z04));
  oai21  g28(.a(new_n51_), .b(new_n44_), .c(new_n48_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n36_), .c(x09), .O(new_n65_));
  and2   g31(.a(x07), .b(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n35_), .c(x08), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n63_), .O(z05));
  oai21  g34(.a(x11), .b(x10), .c(x09), .O(new_n69_));
  nand2  g35(.a(new_n64_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand4  g37(.a(new_n66_), .b(new_n71_), .c(new_n35_), .d(x08), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(x04), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n69_), .c(new_n52_), .O(z06));
  nand2  g41(.a(new_n72_), .b(x11), .O(new_n76_));
  inv1   g42(.a(new_n64_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n35_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n76_), .c(new_n52_), .d(x04), .O(z07));
  oai21  g46(.a(x12), .b(x11), .c(x09), .O(new_n81_));
  oai21  g47(.a(new_n51_), .b(new_n44_), .c(new_n81_), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n71_), .c(new_n35_), .O(new_n84_));
  oai22  g50(.a(new_n84_), .b(new_n64_), .c(new_n83_), .d(new_n35_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n36_), .O(new_n86_));
  nand3  g52(.a(new_n78_), .b(new_n66_), .c(x08), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(x12), .c(new_n35_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n86_), .c(new_n82_), .O(z08));
  oai21  g55(.a(x13), .b(x11), .c(x09), .O(new_n90_));
  nand3  g56(.a(new_n83_), .b(new_n36_), .c(new_n71_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n64_), .c(x13), .O(new_n92_));
  nor2   g58(.a(x13), .b(x12), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n78_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n67_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n44_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n92_), .c(new_n90_), .d(new_n52_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  aoi21  g64(.a(new_n94_), .b(new_n98_), .c(new_n51_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n35_), .c(x08), .d(x07), .O(new_n100_));
  nand2  g66(.a(new_n51_), .b(x14), .O(new_n101_));
  oai21  g67(.a(new_n100_), .b(new_n47_), .c(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n48_), .O(z10));
  nand2  g70(.a(new_n48_), .b(new_n38_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n38_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n44_), .c(new_n48_), .O(z12));
  nor2   g73(.a(new_n37_), .b(new_n44_), .O(z13));
  nand2  g74(.a(x17), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n48_), .O(z14));
endmodule


