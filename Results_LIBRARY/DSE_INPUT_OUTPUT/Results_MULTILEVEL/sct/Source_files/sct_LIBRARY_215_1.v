// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x17), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(new_n35_), .c(x06), .O(z02));
  inv1   g16(.a(new_n36_), .O(new_n51_));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n48_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n51_), .O(z03));
  inv1   g20(.a(x06), .O(new_n55_));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n55_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n48_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n51_), .O(z04));
  oai21  g26(.a(x17), .b(x09), .c(new_n55_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n49_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand2  g29(.a(x08), .b(x07), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n63_), .b(x08), .c(x07), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(new_n35_), .b(x09), .c(new_n55_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n62_), .O(z05));
  oai21  g36(.a(x17), .b(x10), .c(new_n55_), .O(new_n71_));
  nand2  g37(.a(new_n66_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(x08), .c(x07), .d(x06), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(x04), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n71_), .d(new_n48_), .O(z06));
  oai21  g42(.a(x17), .b(x11), .c(new_n55_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n49_), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  aoi21  g45(.a(new_n73_), .b(new_n65_), .c(new_n79_), .O(new_n80_));
  nor4   g46(.a(new_n64_), .b(x11), .c(x10), .d(x09), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n80_), .c(x06), .O(new_n82_));
  nand3  g48(.a(new_n35_), .b(x11), .c(new_n55_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n78_), .O(z07));
  oai21  g50(.a(x17), .b(x12), .c(new_n55_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n49_), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(x10), .O(new_n88_));
  inv1   g54(.a(new_n66_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n87_), .c(new_n79_), .d(new_n88_), .O(new_n90_));
  oai21  g56(.a(new_n81_), .b(new_n87_), .c(new_n90_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x06), .O(new_n92_));
  nand3  g58(.a(new_n35_), .b(x12), .c(new_n55_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n86_), .O(z08));
  oai21  g60(.a(x17), .b(x13), .c(new_n55_), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(x13), .O(new_n96_));
  nand4  g62(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n87_), .c(new_n79_), .d(new_n88_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n43_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n96_), .c(new_n95_), .d(new_n48_), .O(z09));
  inv1   g68(.a(x07), .O(new_n103_));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n99_), .b(new_n104_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n48_), .c(new_n63_), .d(x08), .O(new_n106_));
  nand3  g72(.a(new_n47_), .b(x16), .c(x14), .O(new_n107_));
  oai21  g73(.a(new_n106_), .b(new_n103_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x06), .O(new_n109_));
  nand4  g75(.a(new_n47_), .b(new_n35_), .c(x16), .d(x14), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n43_), .O(z10));
  nor2   g77(.a(new_n36_), .b(new_n37_), .O(z11));
  inv1   g78(.a(x16), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(x02), .c(new_n46_), .O(new_n114_));
  nand3  g80(.a(new_n114_), .b(new_n51_), .c(x04), .O(new_n115_));
  inv1   g81(.a(new_n115_), .O(z12));
  oai21  g82(.a(new_n35_), .b(x06), .c(new_n43_), .O(z13));
  nand3  g83(.a(x17), .b(x06), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


