// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n112_;
  nor2   g00(.a(x11), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n35_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x11), .c(x06), .O(z02));
  inv1   g16(.a(new_n35_), .O(new_n51_));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n48_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n51_), .O(z03));
  inv1   g20(.a(x06), .O(new_n55_));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand3  g22(.a(x11), .b(x08), .c(new_n55_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n48_), .c(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x11), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(x09), .c(new_n55_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n49_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  and2   g30(.a(x08), .b(x07), .O(new_n65_));
  nand3  g31(.a(new_n64_), .b(x08), .c(x07), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(x11), .b(x09), .c(new_n55_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(z05));
  oai21  g36(.a(new_n61_), .b(x10), .c(new_n55_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n49_), .O(new_n72_));
  nand2  g38(.a(new_n66_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand3  g40(.a(new_n65_), .b(new_n74_), .c(new_n64_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand3  g43(.a(x11), .b(x10), .c(new_n55_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n72_), .O(z06));
  nand2  g45(.a(new_n75_), .b(x11), .O(new_n80_));
  nor3   g46(.a(x11), .b(x10), .c(x09), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n65_), .c(new_n55_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n80_), .c(new_n48_), .d(x04), .O(z07));
  nand2  g49(.a(new_n49_), .b(new_n51_), .O(new_n84_));
  and2   g50(.a(new_n75_), .b(x12), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n61_), .c(new_n74_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(new_n66_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n85_), .c(x06), .O(new_n89_));
  nand2  g55(.a(x12), .b(x11), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n84_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nor3   g58(.a(x12), .b(x10), .c(x09), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n65_), .c(new_n92_), .O(new_n94_));
  nand4  g60(.a(new_n92_), .b(new_n86_), .c(new_n61_), .d(new_n74_), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n66_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n94_), .c(x06), .O(new_n97_));
  nand2  g63(.a(x13), .b(x11), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n97_), .c(new_n84_), .O(z09));
  inv1   g65(.a(x07), .O(new_n100_));
  nand2  g66(.a(x06), .b(x00), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n95_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n48_), .c(new_n64_), .d(x08), .O(new_n103_));
  nand3  g69(.a(new_n47_), .b(x16), .c(x14), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n100_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n51_), .O(z10));
  nor2   g73(.a(new_n35_), .b(new_n36_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n36_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n43_), .c(new_n51_), .O(z12));
  nand2  g76(.a(new_n51_), .b(new_n43_), .O(z13));
  inv1   g77(.a(x17), .O(new_n112_));
  nor3   g78(.a(new_n35_), .b(new_n112_), .c(new_n43_), .O(z14));
endmodule


