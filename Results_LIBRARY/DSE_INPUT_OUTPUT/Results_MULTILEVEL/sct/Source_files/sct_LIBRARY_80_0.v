// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_, new_n113_;
  nor2   g00(.a(x13), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n35_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(x13), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  nand3  g14(.a(x13), .b(x07), .c(new_n45_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n45_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(new_n35_), .O(new_n53_));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n45_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n45_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n46_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(z04));
  inv1   g24(.a(new_n46_), .O(new_n59_));
  inv1   g25(.a(x13), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(x09), .c(new_n45_), .O(new_n61_));
  oai21  g27(.a(new_n59_), .b(new_n42_), .c(new_n61_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  and2   g29(.a(x08), .b(x07), .O(new_n64_));
  nand3  g30(.a(new_n63_), .b(x08), .c(x07), .O(new_n65_));
  oai21  g31(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g33(.a(x13), .b(x09), .c(new_n45_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n62_), .O(z05));
  oai21  g35(.a(new_n60_), .b(x10), .c(new_n45_), .O(new_n70_));
  nand2  g36(.a(new_n65_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x07), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n63_), .c(x08), .O(new_n74_));
  nor3   g40(.a(new_n74_), .b(new_n72_), .c(new_n45_), .O(new_n75_));
  nor2   g41(.a(new_n75_), .b(new_n42_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n71_), .c(new_n70_), .d(new_n46_), .O(z06));
  oai21  g43(.a(new_n60_), .b(x11), .c(new_n45_), .O(new_n78_));
  nand3  g44(.a(new_n64_), .b(new_n73_), .c(new_n63_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  nand2  g46(.a(new_n64_), .b(x06), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nor3   g48(.a(x11), .b(x10), .c(x09), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n82_), .c(new_n42_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n80_), .c(new_n78_), .d(new_n46_), .O(z07));
  oai21  g51(.a(new_n60_), .b(x12), .c(new_n45_), .O(new_n86_));
  oai21  g52(.a(new_n59_), .b(new_n42_), .c(new_n86_), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  aoi21  g54(.a(new_n83_), .b(new_n64_), .c(new_n88_), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  nand3  g56(.a(new_n88_), .b(new_n90_), .c(new_n73_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n65_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n89_), .c(x06), .O(new_n93_));
  nand3  g59(.a(x13), .b(x12), .c(new_n45_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n87_), .O(z08));
  oai21  g61(.a(new_n91_), .b(new_n65_), .c(x13), .O(new_n96_));
  nand4  g62(.a(new_n60_), .b(new_n88_), .c(new_n90_), .d(new_n73_), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n65_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n45_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n96_), .c(new_n46_), .d(x04), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n97_), .b(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n46_), .c(new_n63_), .d(x08), .O(new_n103_));
  nand2  g69(.a(new_n59_), .b(x14), .O(new_n104_));
  oai21  g70(.a(new_n103_), .b(new_n72_), .c(new_n104_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x06), .O(new_n106_));
  nand3  g72(.a(new_n59_), .b(x14), .c(x13), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n42_), .O(z10));
  nor2   g74(.a(new_n35_), .b(new_n36_), .O(z11));
  aoi21  g75(.a(x16), .b(new_n36_), .c(x03), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n42_), .c(new_n53_), .O(z12));
  nand2  g77(.a(new_n53_), .b(new_n42_), .O(z13));
  inv1   g78(.a(x17), .O(new_n113_));
  nor3   g79(.a(new_n35_), .b(new_n113_), .c(new_n42_), .O(z14));
endmodule


