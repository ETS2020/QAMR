// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_, new_n119_, new_n122_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x09), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x07), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  inv1   g11(.a(new_n37_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n46_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nand3  g20(.a(new_n36_), .b(new_n54_), .c(x06), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(x06), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n51_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x07), .O(new_n60_));
  nand3  g26(.a(new_n36_), .b(x08), .c(new_n54_), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n60_), .c(new_n47_), .O(new_n62_));
  nor3   g28(.a(new_n37_), .b(new_n59_), .c(x06), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(new_n51_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n43_), .O(z04));
  nand2  g31(.a(new_n51_), .b(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n46_), .O(new_n67_));
  nor2   g33(.a(new_n59_), .b(new_n47_), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  nand3  g35(.a(new_n36_), .b(x08), .c(x06), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  aoi21  g37(.a(new_n69_), .b(x09), .c(new_n71_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n54_), .c(new_n67_), .O(z05));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(x07), .c(new_n66_), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(x08), .c(x07), .d(x06), .O(new_n77_));
  oai21  g43(.a(new_n76_), .b(x07), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n36_), .O(new_n79_));
  nand3  g45(.a(new_n70_), .b(x10), .c(x07), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n75_), .O(z06));
  oai21  g47(.a(x11), .b(x09), .c(new_n54_), .O(new_n82_));
  nand2  g48(.a(new_n74_), .b(new_n68_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x11), .O(new_n84_));
  nand3  g50(.a(x08), .b(x07), .c(x06), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  nor3   g52(.a(x11), .b(x10), .c(x09), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n86_), .c(new_n43_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n84_), .c(new_n82_), .d(new_n51_), .O(z07));
  oai21  g55(.a(x12), .b(x09), .c(new_n54_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n66_), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  inv1   g58(.a(x11), .O(new_n93_));
  nand3  g59(.a(new_n92_), .b(new_n93_), .c(new_n76_), .O(new_n94_));
  oai22  g60(.a(new_n94_), .b(new_n85_), .c(new_n92_), .d(x07), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n36_), .O(new_n96_));
  nand2  g62(.a(new_n87_), .b(new_n68_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(x12), .c(x07), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(new_n91_), .O(z08));
  oai21  g65(.a(x13), .b(x09), .c(new_n54_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n66_), .O(new_n101_));
  inv1   g67(.a(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n92_), .c(new_n93_), .d(new_n76_), .O(new_n103_));
  oai22  g69(.a(new_n103_), .b(new_n85_), .c(new_n102_), .d(x07), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n36_), .O(new_n105_));
  inv1   g71(.a(new_n94_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n71_), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(x13), .c(x07), .O(new_n108_));
  nand3  g74(.a(new_n108_), .b(new_n105_), .c(new_n101_), .O(z09));
  inv1   g75(.a(x00), .O(new_n110_));
  aoi21  g76(.a(new_n103_), .b(new_n110_), .c(new_n50_), .O(new_n111_));
  nand4  g77(.a(new_n111_), .b(new_n36_), .c(x08), .d(x07), .O(new_n112_));
  nand2  g78(.a(new_n50_), .b(x14), .O(new_n113_));
  oai21  g79(.a(new_n112_), .b(new_n47_), .c(new_n113_), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n46_), .O(z10));
  nor2   g82(.a(new_n37_), .b(new_n35_), .O(z11));
  oai21  g83(.a(new_n49_), .b(x02), .c(new_n48_), .O(new_n118_));
  nand3  g84(.a(new_n118_), .b(new_n46_), .c(x04), .O(new_n119_));
  inv1   g85(.a(new_n119_), .O(z12));
  nor2   g86(.a(new_n37_), .b(new_n43_), .O(z13));
  nand2  g87(.a(x17), .b(x04), .O(new_n122_));
  nand2  g88(.a(new_n122_), .b(new_n46_), .O(z14));
endmodule


