// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n114_;
  inv1   g00(.a(x09), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x06), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n36_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n35_), .c(new_n46_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand3  g18(.a(new_n35_), .b(x07), .c(new_n46_), .O(new_n53_));
  oai21  g19(.a(x07), .b(new_n46_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nor2   g24(.a(x09), .b(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n46_), .O(new_n60_));
  oai21  g26(.a(new_n57_), .b(new_n46_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n50_), .c(x04), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  and2   g29(.a(x08), .b(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n59_), .b(x07), .c(x06), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n66_), .c(new_n50_), .d(x04), .O(z05));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  oai22  g35(.a(new_n69_), .b(x06), .c(new_n49_), .d(new_n43_), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(x08), .c(x07), .d(x06), .O(new_n72_));
  oai21  g38(.a(new_n71_), .b(x06), .c(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n35_), .O(new_n74_));
  nand2  g40(.a(new_n59_), .b(x07), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(x10), .c(x06), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n70_), .O(z06));
  oai21  g43(.a(x11), .b(x09), .c(new_n46_), .O(new_n78_));
  nand2  g44(.a(new_n69_), .b(new_n64_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  inv1   g46(.a(new_n65_), .O(new_n81_));
  nor3   g47(.a(x11), .b(x10), .c(x09), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n81_), .c(new_n43_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n80_), .c(new_n78_), .d(new_n50_), .O(z07));
  oai21  g50(.a(x12), .b(x09), .c(new_n46_), .O(new_n85_));
  nand2  g51(.a(new_n82_), .b(new_n64_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n69_), .b(new_n89_), .c(new_n88_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n81_), .c(new_n43_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n87_), .c(new_n85_), .d(new_n50_), .O(z08));
  oai21  g59(.a(x13), .b(x09), .c(new_n46_), .O(new_n94_));
  nand3  g60(.a(new_n89_), .b(new_n88_), .c(new_n71_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n75_), .c(x13), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n89_), .c(new_n88_), .d(new_n71_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n67_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n43_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n96_), .c(new_n94_), .d(new_n50_), .O(z09));
  inv1   g67(.a(new_n36_), .O(new_n102_));
  inv1   g68(.a(x00), .O(new_n103_));
  aoi21  g69(.a(new_n98_), .b(new_n103_), .c(new_n49_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n35_), .c(x08), .d(x07), .O(new_n105_));
  nand2  g71(.a(new_n49_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n46_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n102_), .O(z10));
  nor2   g75(.a(new_n36_), .b(new_n37_), .O(z11));
  aoi21  g76(.a(x16), .b(new_n37_), .c(x03), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n43_), .c(new_n102_), .O(z12));
  nand2  g78(.a(new_n102_), .b(new_n43_), .O(z13));
  nand2  g79(.a(x17), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n102_), .O(z14));
endmodule


