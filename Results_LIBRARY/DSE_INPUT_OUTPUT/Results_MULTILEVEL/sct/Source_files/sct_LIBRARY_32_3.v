// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_, new_n112_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x11), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z01));
  inv1   g15(.a(new_n37_), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n38_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n35_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n50_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n50_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  xor2a  g23(.a(x08), .b(x07), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n36_), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n57_), .b(x06), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nand2  g28(.a(new_n51_), .b(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  inv1   g30(.a(x07), .O(new_n65_));
  nor2   g31(.a(new_n57_), .b(new_n65_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(x11), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  nor2   g34(.a(x11), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(z05));
  inv1   g37(.a(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x08), .c(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n36_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x10), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  nand2  g43(.a(new_n66_), .b(x06), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n36_), .c(new_n77_), .d(new_n72_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n76_), .c(new_n64_), .O(z06));
  nor2   g47(.a(x10), .b(x09), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(x08), .c(x07), .d(x06), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n51_), .c(new_n36_), .d(x04), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  aoi21  g51(.a(new_n82_), .b(new_n66_), .c(new_n85_), .O(new_n86_));
  nand3  g52(.a(new_n85_), .b(new_n77_), .c(new_n72_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(new_n78_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n86_), .c(new_n36_), .O(new_n89_));
  nand2  g55(.a(x12), .b(new_n35_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n64_), .O(z08));
  inv1   g57(.a(new_n66_), .O(new_n92_));
  oai21  g58(.a(new_n87_), .b(new_n92_), .c(x13), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand3  g60(.a(new_n82_), .b(new_n94_), .c(new_n85_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n78_), .c(new_n93_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n36_), .O(new_n97_));
  nand2  g63(.a(x13), .b(new_n35_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n97_), .c(new_n64_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand3  g66(.a(new_n94_), .b(new_n85_), .c(new_n77_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n51_), .c(new_n72_), .d(x08), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n65_), .O(new_n104_));
  nor2   g70(.a(new_n51_), .b(new_n41_), .O(new_n105_));
  aoi21  g71(.a(new_n104_), .b(x06), .c(new_n105_), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n47_), .c(new_n50_), .O(z10));
  nor2   g73(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n38_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n47_), .c(new_n50_), .O(z12));
  nand2  g76(.a(new_n50_), .b(new_n47_), .O(z13));
  nand3  g77(.a(new_n50_), .b(x17), .c(x04), .O(new_n112_));
  inv1   g78(.a(new_n112_), .O(z14));
endmodule


