// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n109_, new_n110_, new_n113_;
  inv1   g00(.a(x10), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n37_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(new_n37_), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n38_), .c(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n49_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n47_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n47_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n49_), .c(x04), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n48_), .O(z04));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand2  g28(.a(x07), .b(x06), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(x08), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(new_n37_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n61_), .c(new_n49_), .d(x04), .O(z05));
  nand3  g34(.a(new_n36_), .b(new_n62_), .c(x08), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n63_), .c(x10), .O(new_n70_));
  nand4  g36(.a(new_n64_), .b(new_n35_), .c(new_n62_), .d(x08), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n70_), .c(new_n49_), .d(x04), .O(z06));
  inv1   g38(.a(new_n49_), .O(new_n73_));
  oai21  g39(.a(x13), .b(x11), .c(x10), .O(new_n74_));
  oai21  g40(.a(new_n73_), .b(new_n44_), .c(new_n74_), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nor2   g42(.a(new_n66_), .b(new_n76_), .O(new_n77_));
  nand3  g43(.a(new_n76_), .b(new_n62_), .c(x08), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n63_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n77_), .c(new_n35_), .O(new_n80_));
  nand3  g46(.a(new_n36_), .b(x11), .c(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n75_), .O(z07));
  nor2   g48(.a(x13), .b(x12), .O(new_n83_));
  oai22  g49(.a(new_n83_), .b(new_n35_), .c(new_n73_), .d(new_n44_), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  nor2   g51(.a(new_n79_), .b(new_n85_), .O(new_n86_));
  nor4   g52(.a(new_n60_), .b(x12), .c(x11), .d(x09), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n86_), .c(new_n35_), .O(new_n88_));
  nand3  g54(.a(new_n36_), .b(x12), .c(x10), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n84_), .O(z08));
  nand4  g56(.a(new_n85_), .b(new_n76_), .c(new_n35_), .d(new_n62_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n60_), .c(x13), .O(new_n92_));
  nand4  g58(.a(new_n83_), .b(new_n66_), .c(new_n76_), .d(new_n35_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(new_n49_), .d(x04), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x02), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(x16), .c(x09), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(x08), .c(x07), .d(x06), .O(new_n99_));
  nand2  g65(.a(new_n73_), .b(x14), .O(new_n100_));
  oai21  g66(.a(new_n99_), .b(new_n95_), .c(new_n100_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  nand4  g68(.a(new_n49_), .b(new_n36_), .c(new_n85_), .d(new_n76_), .O(new_n103_));
  nor3   g69(.a(new_n103_), .b(x10), .c(x09), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(x08), .c(x07), .d(x06), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n102_), .c(new_n44_), .O(z10));
  nor2   g72(.a(new_n37_), .b(new_n38_), .O(z11));
  inv1   g73(.a(x16), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(x02), .c(new_n96_), .O(new_n109_));
  nand3  g75(.a(new_n109_), .b(new_n48_), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z12));
  nor2   g77(.a(new_n37_), .b(new_n44_), .O(z13));
  nand3  g78(.a(new_n48_), .b(x17), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z14));
endmodule


