// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n108_;
  inv1   g00(.a(x14), .O(new_n35_));
  aoi21  g01(.a(x15), .b(x01), .c(new_n35_), .O(new_n36_));
  oai22  g02(.a(new_n36_), .b(x02), .c(x14), .d(x01), .O(z00));
  nand2  g03(.a(x15), .b(x05), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x14), .O(new_n39_));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n39_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  nor2   g12(.a(x15), .b(new_n35_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n48_), .c(new_n46_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n48_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n46_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n46_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n52_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n48_), .O(z04));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(x04), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n52_), .c(new_n47_), .O(z05));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n48_), .c(x04), .O(new_n72_));
  aoi21  g38(.a(new_n66_), .b(x10), .c(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n52_), .O(z06));
  inv1   g40(.a(x10), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  inv1   g42(.a(new_n63_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n65_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x04), .O(new_n79_));
  aoi21  g45(.a(new_n71_), .b(x11), .c(new_n79_), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n52_), .c(new_n47_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n70_), .b(new_n77_), .c(new_n82_), .d(new_n76_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x04), .O(new_n84_));
  aoi21  g50(.a(new_n78_), .b(x12), .c(new_n84_), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(new_n52_), .c(new_n47_), .O(z08));
  nand2  g52(.a(new_n83_), .b(x13), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n82_), .c(new_n76_), .d(new_n75_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n66_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n47_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n87_), .c(new_n52_), .d(x04), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  aoi21  g59(.a(new_n89_), .b(new_n93_), .c(new_n51_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n65_), .c(x08), .d(x07), .O(new_n95_));
  nand2  g61(.a(new_n51_), .b(x14), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n46_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x15), .O(new_n98_));
  nand3  g64(.a(new_n94_), .b(new_n35_), .c(new_n65_), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(x08), .c(x07), .d(x06), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n98_), .c(new_n43_), .O(z10));
  inv1   g68(.a(x02), .O(new_n103_));
  nand2  g69(.a(new_n48_), .b(new_n103_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n103_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n43_), .c(new_n48_), .O(z12));
  nor2   g72(.a(new_n47_), .b(new_n43_), .O(z13));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n48_), .O(z14));
endmodule


