// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n80_, new_n83_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x09), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n43_), .c(new_n37_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(z03));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x09), .c(x06), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n47_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  oai21  g28(.a(new_n50_), .b(new_n44_), .c(new_n37_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n63_), .O(z05));
  inv1   g32(.a(new_n37_), .O(new_n67_));
  nor2   g33(.a(x10), .b(new_n44_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n51_), .c(new_n67_), .O(z06));
  inv1   g35(.a(x11), .O(new_n70_));
  nand4  g36(.a(new_n51_), .b(new_n37_), .c(new_n70_), .d(x04), .O(z07));
  inv1   g37(.a(x12), .O(new_n72_));
  nand4  g38(.a(new_n51_), .b(new_n37_), .c(new_n72_), .d(x04), .O(z08));
  inv1   g39(.a(x13), .O(new_n74_));
  nand4  g40(.a(new_n51_), .b(new_n37_), .c(new_n74_), .d(x04), .O(z09));
  nand2  g41(.a(new_n48_), .b(x02), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n37_), .c(x16), .d(x14), .O(new_n77_));
  nor2   g43(.a(new_n77_), .b(new_n44_), .O(z10));
  nand2  g44(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g45(.a(x16), .b(new_n38_), .c(x03), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n44_), .c(new_n37_), .O(z12));
  nor2   g47(.a(new_n67_), .b(new_n44_), .O(z13));
  nand2  g48(.a(x17), .b(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n37_), .O(z14));
endmodule


