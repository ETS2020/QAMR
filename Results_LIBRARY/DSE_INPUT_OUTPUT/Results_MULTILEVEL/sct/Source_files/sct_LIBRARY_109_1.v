// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x07), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n36_), .c(x06), .O(z02));
  inv1   g14(.a(new_n37_), .O(new_n49_));
  nand4  g15(.a(new_n46_), .b(new_n36_), .c(x06), .d(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n49_), .O(z03));
  inv1   g17(.a(x08), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x07), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x07), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n46_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(x06), .O(new_n57_));
  oai21  g23(.a(x09), .b(x07), .c(new_n57_), .O(new_n58_));
  nand2  g24(.a(x08), .b(x07), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x09), .O(new_n60_));
  nor2   g26(.a(x09), .b(new_n52_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x07), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n43_), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n60_), .c(new_n58_), .d(new_n46_), .O(z05));
  nand2  g31(.a(new_n47_), .b(new_n49_), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(x08), .c(x07), .O(new_n69_));
  oai21  g35(.a(new_n61_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nand2  g37(.a(x10), .b(new_n36_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n66_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  aoi21  g40(.a(new_n68_), .b(x08), .c(new_n74_), .O(new_n75_));
  nor4   g41(.a(new_n59_), .b(x11), .c(x10), .d(x09), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n75_), .c(x06), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n36_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n66_), .O(z07));
  oai21  g45(.a(x12), .b(x07), .c(new_n57_), .O(new_n80_));
  inv1   g46(.a(x12), .O(new_n81_));
  or2    g47(.a(new_n76_), .b(new_n81_), .O(new_n82_));
  nor3   g48(.a(x12), .b(x11), .c(x10), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n63_), .c(new_n43_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n82_), .c(new_n80_), .d(new_n46_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  aoi21  g52(.a(new_n83_), .b(new_n61_), .c(new_n86_), .O(new_n87_));
  nand4  g53(.a(new_n86_), .b(new_n81_), .c(new_n74_), .d(new_n67_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(new_n62_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n87_), .c(x06), .O(new_n90_));
  nand2  g56(.a(x13), .b(new_n36_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n66_), .O(z09));
  inv1   g58(.a(x09), .O(new_n93_));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n88_), .b(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n46_), .c(new_n93_), .d(x08), .O(new_n96_));
  inv1   g62(.a(new_n46_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x14), .O(new_n98_));
  oai21  g64(.a(new_n96_), .b(new_n36_), .c(new_n98_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x06), .O(new_n100_));
  nand3  g66(.a(new_n97_), .b(x14), .c(new_n36_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n43_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n35_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n43_), .c(new_n49_), .O(z12));
  nand2  g71(.a(new_n49_), .b(new_n43_), .O(z13));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n49_), .O(z14));
endmodule


