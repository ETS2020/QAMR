// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(x09), .b(x08), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n46_), .c(x04), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n36_), .O(z02));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n47_), .c(new_n36_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  and2   g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n53_), .b(x07), .c(x06), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n47_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n36_), .O(z04));
  inv1   g24(.a(x09), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(new_n47_), .c(new_n59_), .d(x04), .O(z05));
  inv1   g27(.a(x10), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x07), .c(x06), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(x09), .c(x08), .O(new_n65_));
  nand3  g31(.a(new_n54_), .b(new_n59_), .c(x08), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(x10), .c(new_n43_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n47_), .O(z06));
  inv1   g34(.a(new_n47_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n43_), .c(new_n36_), .O(new_n70_));
  inv1   g36(.a(x11), .O(new_n71_));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n54_), .c(x08), .O(new_n73_));
  oai21  g39(.a(new_n64_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n59_), .O(new_n75_));
  nand2  g41(.a(x11), .b(new_n53_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n70_), .O(z07));
  inv1   g43(.a(x12), .O(new_n78_));
  aoi21  g44(.a(new_n72_), .b(new_n54_), .c(new_n78_), .O(new_n79_));
  inv1   g45(.a(new_n60_), .O(new_n80_));
  nor3   g46(.a(x12), .b(x11), .c(x10), .O(new_n81_));
  and2   g47(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n79_), .c(new_n59_), .O(new_n83_));
  nand2  g49(.a(x12), .b(new_n53_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n70_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  aoi21  g52(.a(new_n81_), .b(new_n54_), .c(new_n86_), .O(new_n87_));
  nand4  g53(.a(new_n86_), .b(new_n78_), .c(new_n71_), .d(new_n62_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(new_n60_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n87_), .c(new_n59_), .O(new_n90_));
  nand2  g56(.a(x13), .b(new_n53_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n70_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n47_), .c(x08), .d(x07), .O(new_n95_));
  nand2  g61(.a(new_n69_), .b(x14), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n46_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n36_), .O(z10));
  nand2  g65(.a(new_n36_), .b(new_n35_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n35_), .c(x03), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n43_), .c(new_n36_), .O(z12));
  nand2  g68(.a(new_n36_), .b(new_n43_), .O(z13));
  nand3  g69(.a(new_n36_), .b(x17), .c(x04), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(z14));
endmodule


