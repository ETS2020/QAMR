// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n114_,
    new_n117_;
  nor2   g00(.a(x07), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n35_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  oai21  g10(.a(x03), .b(new_n36_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(x07), .c(x06), .O(z02));
  xor2a  g13(.a(x07), .b(x06), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n45_), .c(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z03));
  inv1   g16(.a(x06), .O(new_n51_));
  xnor2a g17(.a(x08), .b(x07), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  inv1   g19(.a(x08), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  oai21  g22(.a(new_n52_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n45_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  oai21  g25(.a(x09), .b(new_n53_), .c(new_n51_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n46_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nor2   g28(.a(x09), .b(new_n54_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x06), .O(new_n64_));
  oai21  g30(.a(new_n62_), .b(x06), .c(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x07), .O(new_n66_));
  inv1   g32(.a(new_n55_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x09), .c(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n61_), .O(z05));
  oai21  g35(.a(x10), .b(new_n53_), .c(new_n51_), .O(new_n70_));
  nand2  g36(.a(new_n63_), .b(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n62_), .O(new_n74_));
  nor4   g40(.a(new_n74_), .b(new_n54_), .c(new_n53_), .d(new_n51_), .O(new_n75_));
  nor2   g41(.a(new_n75_), .b(new_n42_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n72_), .c(new_n70_), .d(new_n45_), .O(z06));
  oai21  g43(.a(x11), .b(new_n53_), .c(new_n51_), .O(new_n78_));
  oai21  g44(.a(new_n74_), .b(new_n67_), .c(x11), .O(new_n79_));
  nand2  g45(.a(new_n55_), .b(x06), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  nor3   g47(.a(x11), .b(x10), .c(x09), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n81_), .c(new_n42_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n79_), .c(new_n78_), .d(new_n45_), .O(z07));
  oai21  g50(.a(x12), .b(new_n53_), .c(new_n51_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n46_), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  nand3  g54(.a(new_n87_), .b(new_n88_), .c(new_n73_), .O(new_n89_));
  oai22  g55(.a(new_n89_), .b(new_n64_), .c(new_n87_), .d(x06), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x07), .O(new_n91_));
  nand2  g57(.a(new_n82_), .b(new_n55_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(x12), .c(x06), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n91_), .c(new_n86_), .O(z08));
  oai21  g60(.a(x13), .b(new_n53_), .c(new_n51_), .O(new_n95_));
  oai21  g61(.a(new_n89_), .b(new_n71_), .c(x13), .O(new_n96_));
  nand3  g62(.a(new_n63_), .b(x07), .c(x06), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n87_), .c(new_n88_), .d(new_n73_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n42_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n96_), .c(new_n95_), .d(new_n45_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  nand2  g69(.a(new_n99_), .b(new_n103_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n45_), .c(new_n62_), .d(x08), .O(new_n105_));
  inv1   g71(.a(new_n45_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(x14), .O(new_n107_));
  oai21  g73(.a(new_n105_), .b(new_n53_), .c(new_n107_), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(x06), .O(new_n109_));
  nand3  g75(.a(new_n106_), .b(x14), .c(x07), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n42_), .O(z10));
  inv1   g77(.a(new_n35_), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n36_), .O(z11));
  aoi21  g79(.a(x16), .b(new_n36_), .c(x03), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(new_n42_), .c(new_n112_), .O(z12));
  nor2   g81(.a(new_n35_), .b(new_n42_), .O(z13));
  nand2  g82(.a(x17), .b(x04), .O(new_n117_));
  nand2  g83(.a(new_n117_), .b(new_n112_), .O(z14));
endmodule


