// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n107_, new_n108_, new_n110_;
  inv1   g00(.a(x16), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x11), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n36_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x02), .c(new_n35_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n47_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n37_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n37_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n47_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n47_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n50_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n37_), .O(z04));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n37_), .O(new_n66_));
  nand2  g32(.a(new_n49_), .b(x11), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n66_), .O(z05));
  nand2  g34(.a(x07), .b(x06), .O(new_n69_));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x08), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  aoi21  g40(.a(new_n64_), .b(x10), .c(new_n74_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n36_), .c(new_n67_), .O(z06));
  inv1   g42(.a(x11), .O(new_n77_));
  oai21  g43(.a(new_n72_), .b(x16), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n73_), .b(x11), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n50_), .d(x04), .O(z07));
  nand2  g46(.a(new_n37_), .b(new_n44_), .O(z13));
  oai21  g47(.a(new_n49_), .b(x12), .c(x11), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  inv1   g49(.a(new_n61_), .O(new_n84_));
  nand4  g50(.a(new_n70_), .b(new_n84_), .c(new_n83_), .d(new_n77_), .O(new_n85_));
  oai21  g51(.a(new_n72_), .b(new_n83_), .c(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n35_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n82_), .c(z13), .O(z08));
  inv1   g54(.a(x13), .O(new_n89_));
  nand3  g55(.a(new_n70_), .b(new_n89_), .c(new_n83_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n61_), .c(new_n35_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n77_), .O(new_n92_));
  nand3  g58(.a(new_n89_), .b(new_n48_), .c(x02), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x16), .O(new_n94_));
  aoi21  g60(.a(new_n85_), .b(x13), .c(new_n44_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  inv1   g63(.a(x10), .O(new_n98_));
  nand4  g64(.a(new_n89_), .b(new_n83_), .c(new_n77_), .d(new_n98_), .O(new_n99_));
  oai21  g65(.a(new_n49_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n63_), .c(x08), .d(x07), .O(new_n101_));
  nand2  g67(.a(new_n49_), .b(x14), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n47_), .c(new_n102_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n37_), .O(z10));
  nand2  g71(.a(new_n37_), .b(new_n39_), .O(z11));
  nand2  g72(.a(new_n37_), .b(x03), .O(new_n107_));
  nand3  g73(.a(x16), .b(x11), .c(new_n39_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n44_), .O(z12));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n37_), .O(z14));
endmodule


