// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n92_, new_n93_, new_n96_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nor2   g02(.a(x06), .b(new_n36_), .O(z02));
  aoi21  g03(.a(new_n35_), .b(x01), .c(z02), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  oai21  g07(.a(new_n40_), .b(x15), .c(new_n41_), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n36_), .c(z02), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(new_n42_), .O(z01));
  inv1   g10(.a(x07), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x06), .c(new_n36_), .O(z03));
  xor2a  g14(.a(x08), .b(x07), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(new_n46_), .c(x06), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z04));
  inv1   g17(.a(x09), .O(new_n52_));
  and2   g18(.a(x08), .b(x07), .O(new_n53_));
  inv1   g19(.a(new_n46_), .O(new_n54_));
  nand3  g20(.a(new_n52_), .b(x08), .c(x07), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(x06), .c(x04), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  oai21  g23(.a(new_n53_), .b(new_n52_), .c(new_n57_), .O(z05));
  nand2  g24(.a(new_n55_), .b(x10), .O(new_n59_));
  inv1   g25(.a(x10), .O(new_n60_));
  nand3  g26(.a(new_n53_), .b(new_n60_), .c(new_n52_), .O(new_n61_));
  and2   g27(.a(new_n61_), .b(x06), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n59_), .c(new_n46_), .d(x04), .O(z06));
  nand2  g29(.a(new_n61_), .b(x11), .O(new_n64_));
  inv1   g30(.a(x11), .O(new_n65_));
  nand4  g31(.a(new_n53_), .b(new_n65_), .c(new_n60_), .d(new_n52_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n46_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x04), .O(z07));
  nand2  g35(.a(new_n66_), .b(x12), .O(new_n70_));
  inv1   g36(.a(x12), .O(new_n71_));
  inv1   g37(.a(new_n55_), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n71_), .c(new_n65_), .d(new_n60_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n70_), .c(new_n46_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x04), .O(z08));
  nand2  g42(.a(new_n73_), .b(x13), .O(new_n77_));
  inv1   g43(.a(x06), .O(new_n78_));
  inv1   g44(.a(x13), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n71_), .c(new_n65_), .d(new_n60_), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n72_), .c(new_n78_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n77_), .c(new_n46_), .d(x04), .O(z09));
  inv1   g49(.a(x00), .O(new_n84_));
  nand2  g50(.a(new_n80_), .b(new_n84_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n46_), .c(new_n52_), .d(x08), .O(new_n86_));
  nand2  g52(.a(new_n54_), .b(x14), .O(new_n87_));
  oai21  g53(.a(new_n86_), .b(new_n45_), .c(new_n87_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(x06), .c(x04), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(z10));
  nor2   g56(.a(z02), .b(new_n35_), .O(z11));
  inv1   g57(.a(x03), .O(new_n92_));
  aoi21  g58(.a(x16), .b(new_n35_), .c(new_n78_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n92_), .c(new_n36_), .O(z12));
  nor2   g60(.a(new_n78_), .b(new_n36_), .O(z13));
  nand2  g61(.a(z13), .b(x17), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(z14));
endmodule


