// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n73_, new_n75_, new_n78_, new_n79_, new_n82_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x09), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n43_), .c(new_n37_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n51_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n47_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n47_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n51_), .c(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n37_), .O(z04));
  oai21  g27(.a(new_n50_), .b(new_n44_), .c(new_n37_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n62_), .O(z05));
  inv1   g31(.a(new_n37_), .O(new_n66_));
  nor2   g32(.a(x10), .b(new_n44_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n51_), .c(new_n66_), .O(z06));
  nor2   g34(.a(x11), .b(new_n44_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n51_), .c(new_n66_), .O(z07));
  inv1   g36(.a(x12), .O(new_n71_));
  nand4  g37(.a(new_n51_), .b(new_n37_), .c(new_n71_), .d(x04), .O(z08));
  nor2   g38(.a(x13), .b(new_n44_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n51_), .c(new_n66_), .O(z09));
  nand3  g40(.a(new_n50_), .b(x14), .c(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n37_), .O(z10));
  nor2   g42(.a(new_n66_), .b(new_n38_), .O(z11));
  oai21  g43(.a(new_n49_), .b(x02), .c(new_n48_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n37_), .c(x04), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(z12));
  nor2   g46(.a(new_n66_), .b(new_n44_), .O(z13));
  nand2  g47(.a(new_n37_), .b(x17), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n44_), .O(z14));
endmodule


