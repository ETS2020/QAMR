// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n104_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(z00));
  oai21  g07(.a(new_n37_), .b(x05), .c(new_n36_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n37_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x06), .c(x04), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n38_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  xor2a  g23(.a(x08), .b(x07), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(x06), .c(x04), .O(new_n59_));
  oai21  g25(.a(new_n57_), .b(x06), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n49_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n38_), .O(z04));
  nor2   g28(.a(new_n57_), .b(new_n52_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n65_), .c(new_n49_), .d(x04), .O(z05));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n63_), .c(new_n36_), .O(new_n70_));
  aoi21  g36(.a(new_n67_), .b(x10), .c(new_n48_), .O(new_n71_));
  oai22  g37(.a(new_n71_), .b(new_n36_), .c(new_n70_), .d(new_n37_), .O(z06));
  nor3   g38(.a(x11), .b(x10), .c(x09), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n63_), .c(new_n36_), .O(new_n74_));
  nand4  g40(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(x11), .c(new_n48_), .O(new_n76_));
  oai22  g42(.a(new_n76_), .b(new_n36_), .c(new_n74_), .d(new_n37_), .O(z07));
  nand3  g43(.a(new_n66_), .b(x08), .c(x07), .O(new_n78_));
  inv1   g44(.a(x10), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(x12), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n78_), .c(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nand3  g50(.a(new_n73_), .b(new_n63_), .c(x06), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(x12), .c(new_n48_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n36_), .c(new_n84_), .O(z08));
  nand3  g53(.a(new_n69_), .b(new_n81_), .c(new_n80_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n64_), .c(x13), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n81_), .c(new_n80_), .d(new_n79_), .O(new_n91_));
  or2    g57(.a(new_n91_), .b(new_n67_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n89_), .c(new_n49_), .d(x04), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  aoi21  g60(.a(new_n91_), .b(new_n94_), .c(new_n48_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n66_), .c(x08), .d(x07), .O(new_n96_));
  nand2  g62(.a(new_n48_), .b(x14), .O(new_n97_));
  oai21  g63(.a(new_n96_), .b(new_n37_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n38_), .O(z10));
  nand2  g66(.a(new_n38_), .b(new_n39_), .O(z11));
  aoi21  g67(.a(x16), .b(new_n39_), .c(x03), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n36_), .c(new_n38_), .O(z12));
  nand2  g69(.a(x17), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n38_), .O(z14));
  buf    g71(.a(x04), .O(z13));
endmodule


