// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n115_,
    new_n117_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x16), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(new_n45_), .c(new_n37_), .O(z01));
  inv1   g14(.a(x03), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x02), .c(new_n36_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n35_), .c(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand3  g19(.a(new_n51_), .b(x07), .c(new_n35_), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand3  g21(.a(new_n36_), .b(new_n55_), .c(x06), .O(new_n56_));
  aoi21  g22(.a(new_n56_), .b(new_n54_), .c(new_n46_), .O(z03));
  xnor2a g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n35_), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n35_), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n36_), .O(new_n61_));
  nand4  g27(.a(x08), .b(new_n35_), .c(new_n49_), .d(x02), .O(new_n62_));
  aoi21  g28(.a(new_n62_), .b(new_n61_), .c(new_n46_), .O(z04));
  nand2  g29(.a(new_n38_), .b(new_n46_), .O(z13));
  oai21  g30(.a(new_n50_), .b(x09), .c(new_n35_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  and2   g32(.a(x08), .b(x07), .O(new_n67_));
  nand2  g33(.a(x07), .b(x06), .O(new_n68_));
  nand2  g34(.a(new_n66_), .b(x08), .O(new_n69_));
  oai22  g35(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n36_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n65_), .c(z13), .O(z05));
  oai21  g38(.a(new_n50_), .b(x10), .c(new_n35_), .O(new_n73_));
  nand3  g39(.a(new_n66_), .b(x08), .c(x07), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x10), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x08), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n68_), .c(new_n75_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n36_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n73_), .c(z13), .O(z06));
  oai21  g46(.a(new_n50_), .b(x11), .c(new_n35_), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  aoi21  g48(.a(new_n76_), .b(new_n67_), .c(new_n82_), .O(new_n83_));
  nand2  g49(.a(new_n67_), .b(x06), .O(new_n84_));
  inv1   g50(.a(x10), .O(new_n85_));
  nand3  g51(.a(new_n82_), .b(new_n85_), .c(new_n66_), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n83_), .c(new_n36_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n81_), .c(z13), .O(z07));
  inv1   g55(.a(x12), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n82_), .c(new_n85_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n74_), .c(new_n36_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x06), .O(new_n93_));
  nand3  g59(.a(new_n90_), .b(new_n49_), .c(x02), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x16), .O(new_n95_));
  oai21  g61(.a(new_n86_), .b(new_n84_), .c(x12), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n95_), .c(new_n93_), .d(x04), .O(z08));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n90_), .c(new_n82_), .d(new_n85_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n74_), .c(new_n36_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x06), .O(new_n101_));
  nand3  g67(.a(new_n98_), .b(new_n49_), .c(x02), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x16), .O(new_n103_));
  nand3  g69(.a(new_n76_), .b(new_n90_), .c(new_n82_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n84_), .c(x13), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n103_), .c(new_n101_), .d(x04), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  aoi21  g73(.a(new_n99_), .b(new_n107_), .c(x09), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(x08), .c(x07), .d(x06), .O(new_n109_));
  nand2  g75(.a(new_n50_), .b(x14), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n38_), .O(z10));
  nor2   g79(.a(new_n37_), .b(new_n40_), .O(z11));
  aoi21  g80(.a(x16), .b(new_n40_), .c(x03), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n46_), .c(new_n38_), .O(z12));
  nand3  g82(.a(new_n38_), .b(x17), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z14));
endmodule


