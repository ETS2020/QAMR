// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n94_, new_n97_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x06), .O(new_n43_));
  nor2   g09(.a(x05), .b(new_n36_), .O(new_n44_));
  aoi21  g10(.a(new_n43_), .b(new_n36_), .c(new_n44_), .O(new_n45_));
  nand3  g11(.a(new_n43_), .b(x05), .c(new_n36_), .O(new_n46_));
  oai21  g12(.a(new_n45_), .b(new_n42_), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n38_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(new_n43_), .c(x04), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(z02));
  nand3  g16(.a(x07), .b(new_n43_), .c(x04), .O(new_n51_));
  oai21  g17(.a(x07), .b(new_n43_), .c(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n37_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n43_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n43_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n48_), .c(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n61_), .c(new_n48_), .d(x04), .O(z05));
  nand2  g30(.a(new_n63_), .b(x10), .O(new_n65_));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n65_), .c(new_n48_), .d(x04), .O(z06));
  nand2  g34(.a(new_n67_), .b(x11), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  inv1   g36(.a(x11), .O(new_n71_));
  inv1   g37(.a(new_n60_), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n62_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n69_), .c(new_n48_), .d(x04), .O(z07));
  nand2  g40(.a(new_n73_), .b(x12), .O(new_n75_));
  inv1   g41(.a(x12), .O(new_n76_));
  nand4  g42(.a(new_n66_), .b(new_n72_), .c(new_n76_), .d(new_n71_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n75_), .c(new_n48_), .d(x04), .O(z08));
  nand2  g44(.a(new_n77_), .b(x13), .O(new_n79_));
  inv1   g45(.a(x13), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n76_), .c(new_n71_), .d(new_n70_), .O(new_n81_));
  or2    g47(.a(new_n81_), .b(new_n63_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n79_), .c(new_n48_), .d(x04), .O(z09));
  inv1   g49(.a(x07), .O(new_n84_));
  inv1   g50(.a(x00), .O(new_n85_));
  nand2  g51(.a(new_n81_), .b(new_n85_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n48_), .c(new_n62_), .d(x08), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n84_), .c(x04), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x06), .O(new_n89_));
  inv1   g55(.a(new_n48_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(x14), .c(x04), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n89_), .O(z10));
  nand2  g58(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g59(.a(x16), .b(new_n38_), .c(x03), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n36_), .O(z12));
  nand2  g61(.a(new_n43_), .b(new_n36_), .O(z13));
  nand2  g62(.a(x17), .b(x04), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n37_), .O(z14));
endmodule


