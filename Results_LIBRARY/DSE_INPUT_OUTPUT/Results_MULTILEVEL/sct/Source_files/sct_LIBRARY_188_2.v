// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n113_, new_n116_;
  inv1   g00(.a(x09), .O(new_n35_));
  nor2   g01(.a(x18), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  inv1   g04(.a(x02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  nand3  g08(.a(new_n37_), .b(x05), .c(new_n42_), .O(new_n43_));
  nor2   g09(.a(x18), .b(x09), .O(new_n44_));
  aoi21  g10(.a(x18), .b(x15), .c(new_n44_), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n37_), .c(new_n47_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xor2a  g18(.a(x07), .b(x06), .O(new_n53_));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n37_), .d(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n47_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n47_), .c(new_n57_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n50_), .c(new_n37_), .d(x04), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x18), .c(x08), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  nand3  g29(.a(new_n61_), .b(new_n35_), .c(x08), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n63_), .c(new_n50_), .d(x04), .O(z05));
  inv1   g31(.a(x16), .O(new_n66_));
  aoi21  g32(.a(new_n48_), .b(x02), .c(new_n66_), .O(new_n67_));
  inv1   g33(.a(x18), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(x10), .c(x09), .O(new_n69_));
  oai21  g35(.a(new_n67_), .b(new_n42_), .c(new_n69_), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand3  g37(.a(x08), .b(x07), .c(x06), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nor2   g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand3  g40(.a(new_n61_), .b(new_n71_), .c(x08), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n74_), .c(new_n35_), .O(new_n77_));
  nand3  g43(.a(x18), .b(x10), .c(x09), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n70_), .O(z06));
  oai21  g45(.a(new_n68_), .b(x11), .c(x09), .O(new_n80_));
  oai21  g46(.a(new_n67_), .b(new_n42_), .c(new_n80_), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand4  g48(.a(new_n61_), .b(new_n82_), .c(new_n71_), .d(x08), .O(new_n83_));
  oai21  g49(.a(new_n76_), .b(new_n82_), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n35_), .O(new_n85_));
  nand3  g51(.a(x18), .b(x11), .c(x09), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n81_), .O(z07));
  oai21  g53(.a(new_n68_), .b(x12), .c(x09), .O(new_n88_));
  nand2  g54(.a(new_n83_), .b(x12), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n82_), .c(new_n71_), .d(new_n35_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n73_), .c(new_n42_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n89_), .c(new_n88_), .d(new_n50_), .O(z08));
  oai21  g60(.a(new_n68_), .b(x13), .c(x09), .O(new_n95_));
  nand3  g61(.a(new_n90_), .b(new_n82_), .c(new_n71_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n72_), .c(x13), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n90_), .c(new_n82_), .d(new_n71_), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n64_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n42_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n97_), .c(new_n95_), .d(new_n50_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  nand2  g69(.a(new_n99_), .b(new_n103_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n50_), .c(x08), .d(x07), .O(new_n105_));
  nand2  g71(.a(new_n67_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n47_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n35_), .O(new_n108_));
  nand4  g74(.a(new_n49_), .b(x18), .c(x16), .d(x14), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n42_), .O(z10));
  nor2   g76(.a(new_n36_), .b(new_n39_), .O(z11));
  oai21  g77(.a(new_n66_), .b(x02), .c(new_n48_), .O(new_n112_));
  nand3  g78(.a(new_n112_), .b(new_n37_), .c(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z12));
  nand2  g80(.a(new_n37_), .b(new_n42_), .O(z13));
  inv1   g81(.a(x17), .O(new_n116_));
  nor3   g82(.a(new_n36_), .b(new_n116_), .c(new_n42_), .O(z14));
endmodule


