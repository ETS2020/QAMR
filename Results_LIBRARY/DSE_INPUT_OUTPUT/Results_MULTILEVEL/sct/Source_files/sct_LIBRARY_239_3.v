// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n117_;
  inv1   g00(.a(x07), .O(new_n35_));
  inv1   g01(.a(x15), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  aoi21  g06(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n37_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(new_n37_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n48_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n47_), .O(z02));
  xor2a  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(x04), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n47_), .O(z03));
  oai21  g23(.a(x15), .b(x06), .c(x07), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x08), .O(new_n59_));
  nor2   g25(.a(new_n35_), .b(new_n48_), .O(new_n60_));
  nor2   g26(.a(x15), .b(x08), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n52_), .c(x04), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  oai21  g31(.a(new_n51_), .b(new_n44_), .c(new_n47_), .O(new_n66_));
  aoi21  g32(.a(x08), .b(x06), .c(x15), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n35_), .c(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand4  g35(.a(new_n60_), .b(new_n36_), .c(new_n69_), .d(x08), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n66_), .O(z05));
  nand3  g37(.a(new_n69_), .b(x08), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n36_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x07), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x10), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand3  g42(.a(x08), .b(x07), .c(x06), .O(new_n77_));
  inv1   g43(.a(new_n77_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n36_), .c(new_n76_), .d(new_n69_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n75_), .c(new_n66_), .O(z06));
  nand2  g46(.a(x08), .b(x06), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n69_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n81_), .c(new_n36_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x07), .O(new_n85_));
  aoi21  g51(.a(new_n79_), .b(x11), .c(new_n44_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n52_), .O(z07));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n76_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n72_), .c(new_n36_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x07), .O(new_n92_));
  nor2   g58(.a(x15), .b(x11), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n76_), .c(new_n69_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n77_), .c(x12), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n92_), .c(new_n52_), .d(x04), .O(z08));
  nor2   g62(.a(x13), .b(x12), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n82_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n72_), .c(new_n36_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x07), .O(new_n100_));
  nand3  g66(.a(new_n60_), .b(new_n69_), .c(x08), .O(new_n101_));
  nand3  g67(.a(new_n82_), .b(new_n36_), .c(new_n89_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n101_), .c(x13), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n100_), .c(new_n52_), .d(x04), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  aoi21  g71(.a(new_n98_), .b(new_n105_), .c(new_n51_), .O(new_n106_));
  nand4  g72(.a(new_n106_), .b(new_n69_), .c(x08), .d(x07), .O(new_n107_));
  nand2  g73(.a(new_n51_), .b(x14), .O(new_n108_));
  oai21  g74(.a(new_n107_), .b(new_n48_), .c(new_n108_), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n36_), .O(new_n110_));
  nand3  g76(.a(new_n51_), .b(x14), .c(new_n35_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n110_), .c(new_n44_), .O(z10));
  nand2  g78(.a(new_n47_), .b(new_n38_), .O(z11));
  aoi21  g79(.a(x16), .b(new_n38_), .c(x03), .O(new_n114_));
  oai21  g80(.a(new_n114_), .b(new_n44_), .c(new_n47_), .O(z12));
  nor2   g81(.a(new_n37_), .b(new_n44_), .O(z13));
  nand3  g82(.a(new_n47_), .b(x17), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z14));
endmodule


