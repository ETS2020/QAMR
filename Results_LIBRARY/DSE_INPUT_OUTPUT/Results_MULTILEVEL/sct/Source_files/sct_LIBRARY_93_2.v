// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n98_, new_n100_;
  inv1   g00(.a(x04), .O(new_n35_));
  nor2   g01(.a(x06), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x02), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x01), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  aoi21  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(new_n36_), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  oai21  g09(.a(new_n42_), .b(x15), .c(new_n43_), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n35_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n41_), .O(z01));
  inv1   g12(.a(x07), .O(new_n48_));
  inv1   g13(.a(x03), .O(new_n49_));
  nand2  g14(.a(new_n49_), .b(x02), .O(new_n50_));
  nand2  g15(.a(new_n50_), .b(x16), .O(new_n51_));
  nand2  g16(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  aoi21  g17(.a(new_n52_), .b(x06), .c(new_n35_), .O(z03));
  xor2a  g18(.a(x08), .b(x07), .O(new_n54_));
  nand2  g19(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  aoi21  g20(.a(new_n55_), .b(x06), .c(new_n35_), .O(z04));
  inv1   g21(.a(x09), .O(new_n57_));
  and2   g22(.a(x08), .b(x07), .O(new_n58_));
  nor2   g23(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g24(.a(new_n57_), .b(x08), .c(x07), .O(new_n60_));
  nand2  g25(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  oai21  g26(.a(new_n61_), .b(new_n59_), .c(x06), .O(new_n62_));
  nand2  g27(.a(new_n62_), .b(x04), .O(z05));
  nand2  g28(.a(new_n60_), .b(x10), .O(new_n64_));
  inv1   g29(.a(x10), .O(new_n65_));
  nand3  g30(.a(new_n58_), .b(new_n65_), .c(new_n57_), .O(new_n66_));
  and2   g31(.a(new_n66_), .b(x06), .O(new_n67_));
  nand4  g32(.a(new_n67_), .b(new_n64_), .c(new_n51_), .d(x04), .O(z06));
  nand2  g33(.a(new_n66_), .b(x11), .O(new_n69_));
  inv1   g34(.a(x11), .O(new_n70_));
  nand4  g35(.a(new_n58_), .b(new_n70_), .c(new_n65_), .d(new_n57_), .O(new_n71_));
  nand3  g36(.a(new_n71_), .b(new_n69_), .c(new_n51_), .O(new_n72_));
  nand2  g37(.a(new_n72_), .b(x06), .O(new_n73_));
  nand2  g38(.a(new_n73_), .b(x04), .O(z07));
  nand2  g39(.a(new_n71_), .b(x12), .O(new_n75_));
  inv1   g40(.a(x12), .O(new_n76_));
  nand3  g41(.a(new_n76_), .b(new_n70_), .c(new_n65_), .O(new_n77_));
  or2    g42(.a(new_n77_), .b(new_n60_), .O(new_n78_));
  nand3  g43(.a(new_n78_), .b(new_n75_), .c(new_n51_), .O(new_n79_));
  nand2  g44(.a(new_n79_), .b(x06), .O(new_n80_));
  nand2  g45(.a(new_n80_), .b(x04), .O(z08));
  oai21  g46(.a(new_n77_), .b(new_n60_), .c(x13), .O(new_n82_));
  inv1   g47(.a(x13), .O(new_n83_));
  nand4  g48(.a(new_n83_), .b(new_n76_), .c(new_n70_), .d(new_n65_), .O(new_n84_));
  or2    g49(.a(new_n84_), .b(new_n60_), .O(new_n85_));
  nand3  g50(.a(new_n85_), .b(new_n82_), .c(new_n51_), .O(new_n86_));
  nand2  g51(.a(new_n86_), .b(x06), .O(new_n87_));
  nand2  g52(.a(new_n87_), .b(x04), .O(z09));
  inv1   g53(.a(x00), .O(new_n89_));
  nand2  g54(.a(new_n84_), .b(new_n89_), .O(new_n90_));
  nand4  g55(.a(new_n90_), .b(new_n51_), .c(new_n57_), .d(x08), .O(new_n91_));
  nand3  g56(.a(new_n50_), .b(x16), .c(x14), .O(new_n92_));
  oai21  g57(.a(new_n91_), .b(new_n48_), .c(new_n92_), .O(new_n93_));
  nand3  g58(.a(new_n93_), .b(x06), .c(x04), .O(new_n94_));
  inv1   g59(.a(new_n94_), .O(z10));
  nand2  g60(.a(new_n41_), .b(new_n37_), .O(z11));
  inv1   g61(.a(x06), .O(new_n97_));
  aoi21  g62(.a(x16), .b(new_n37_), .c(new_n97_), .O(new_n98_));
  aoi21  g63(.a(new_n98_), .b(new_n49_), .c(new_n35_), .O(z12));
  nand3  g64(.a(x17), .b(x06), .c(x04), .O(new_n100_));
  inv1   g65(.a(new_n100_), .O(z14));
  zero   g66(.O(z02));
  buf    g67(.a(x04), .O(z13));
endmodule


