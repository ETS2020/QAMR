// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n105_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x09), .b(x06), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(x15), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  aoi21  g07(.a(x05), .b(new_n41_), .c(new_n36_), .O(new_n42_));
  oai21  g08(.a(new_n40_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand4  g11(.a(new_n45_), .b(x09), .c(new_n44_), .d(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z02));
  inv1   g13(.a(new_n36_), .O(new_n48_));
  xor2a  g14(.a(x07), .b(x06), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n45_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(z03));
  xnor2a g17(.a(x08), .b(x07), .O(new_n52_));
  nand3  g18(.a(x09), .b(x08), .c(new_n44_), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(new_n44_), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n45_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z04));
  inv1   g22(.a(new_n45_), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n41_), .c(new_n48_), .O(new_n58_));
  nand2  g24(.a(x08), .b(x07), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n44_), .c(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(z05));
  inv1   g29(.a(x10), .O(new_n64_));
  inv1   g30(.a(new_n59_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n64_), .c(new_n61_), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand2  g34(.a(x10), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n58_), .O(z06));
  oai21  g36(.a(x11), .b(new_n61_), .c(new_n44_), .O(new_n71_));
  nand2  g37(.a(new_n66_), .b(x11), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand4  g39(.a(new_n65_), .b(new_n73_), .c(new_n64_), .d(new_n61_), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(x04), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n72_), .c(new_n71_), .d(new_n45_), .O(z07));
  oai21  g42(.a(x12), .b(new_n61_), .c(new_n44_), .O(new_n77_));
  nand2  g43(.a(new_n74_), .b(x12), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(new_n61_), .b(x08), .c(x07), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n79_), .c(new_n73_), .d(new_n64_), .O(new_n82_));
  and2   g48(.a(new_n82_), .b(x04), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n78_), .c(new_n77_), .d(new_n45_), .O(z08));
  oai21  g50(.a(x13), .b(new_n61_), .c(new_n44_), .O(new_n85_));
  nand2  g51(.a(new_n82_), .b(x13), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n79_), .c(new_n73_), .d(new_n64_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n81_), .c(new_n41_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n86_), .c(new_n85_), .d(new_n45_), .O(z09));
  inv1   g57(.a(x07), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n45_), .c(new_n61_), .d(x08), .O(new_n95_));
  nand2  g61(.a(new_n57_), .b(x14), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n92_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x06), .O(new_n98_));
  nand3  g64(.a(new_n57_), .b(x14), .c(x09), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n41_), .O(z10));
  nand2  g66(.a(new_n48_), .b(new_n35_), .O(z11));
  aoi21  g67(.a(x16), .b(new_n35_), .c(x03), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n41_), .c(new_n48_), .O(z12));
  nor2   g69(.a(new_n36_), .b(new_n41_), .O(z13));
  nand2  g70(.a(x17), .b(x04), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n48_), .O(z14));
endmodule


