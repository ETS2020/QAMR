// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n112_, new_n115_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  oai21  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x01), .O(new_n39_));
  nor2   g05(.a(x06), .b(new_n39_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand4  g13(.a(new_n47_), .b(new_n36_), .c(x04), .d(new_n39_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  inv1   g15(.a(new_n40_), .O(new_n50_));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n50_), .O(z03));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n36_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n36_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n47_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n50_), .O(z04));
  oai21  g24(.a(x09), .b(x01), .c(new_n36_), .O(new_n59_));
  nand2  g25(.a(x08), .b(x07), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n44_), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n61_), .c(new_n59_), .d(new_n47_), .O(z05));
  oai21  g32(.a(x10), .b(x01), .c(new_n36_), .O(new_n67_));
  nand2  g33(.a(new_n47_), .b(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nand3  g36(.a(new_n62_), .b(x08), .c(x07), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nand4  g38(.a(new_n70_), .b(new_n62_), .c(x08), .d(x07), .O(new_n73_));
  oai21  g39(.a(new_n72_), .b(new_n70_), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand3  g41(.a(x10), .b(new_n36_), .c(new_n39_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n69_), .O(z06));
  oai21  g43(.a(x11), .b(x01), .c(new_n36_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n68_), .O(new_n79_));
  and2   g45(.a(new_n73_), .b(x11), .O(new_n80_));
  nor4   g46(.a(new_n60_), .b(x11), .c(x10), .d(x09), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n80_), .c(x06), .O(new_n82_));
  nand3  g48(.a(x11), .b(new_n36_), .c(new_n39_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n79_), .O(z07));
  oai21  g50(.a(x12), .b(x01), .c(new_n36_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n68_), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  nand4  g54(.a(new_n72_), .b(new_n87_), .c(new_n88_), .d(new_n70_), .O(new_n89_));
  oai21  g55(.a(new_n81_), .b(new_n87_), .c(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand3  g57(.a(x12), .b(new_n36_), .c(new_n39_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n86_), .O(z08));
  oai21  g59(.a(x13), .b(x01), .c(new_n36_), .O(new_n94_));
  nand2  g60(.a(new_n89_), .b(x13), .O(new_n95_));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n87_), .c(new_n88_), .d(new_n70_), .O(new_n97_));
  inv1   g63(.a(new_n97_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n64_), .c(new_n44_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n95_), .c(new_n94_), .d(new_n47_), .O(z09));
  inv1   g66(.a(x07), .O(new_n101_));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n97_), .b(new_n102_), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n47_), .c(new_n62_), .d(x08), .O(new_n104_));
  inv1   g70(.a(new_n47_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n104_), .b(new_n101_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x06), .O(new_n108_));
  nand3  g74(.a(new_n105_), .b(x14), .c(new_n39_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n108_), .c(new_n44_), .O(z10));
  nand2  g76(.a(new_n50_), .b(new_n35_), .O(z11));
  aoi21  g77(.a(x16), .b(new_n35_), .c(x03), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(new_n44_), .c(new_n50_), .O(z12));
  nand2  g79(.a(new_n50_), .b(new_n44_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n50_), .O(z14));
endmodule


