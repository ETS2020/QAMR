// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n117_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x13), .b(x06), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(x13), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  nand3  g14(.a(x13), .b(x07), .c(new_n45_), .O(new_n49_));
  oai21  g15(.a(x07), .b(new_n45_), .c(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z03));
  inv1   g18(.a(new_n36_), .O(new_n53_));
  xnor2a g19(.a(x08), .b(x07), .O(new_n54_));
  nand2  g20(.a(x08), .b(new_n45_), .O(new_n55_));
  oai21  g21(.a(new_n54_), .b(new_n45_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n46_), .c(x04), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(z04));
  inv1   g24(.a(x13), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x09), .c(new_n45_), .O(new_n60_));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  inv1   g29(.a(x07), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n45_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(x08), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(new_n42_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n62_), .c(new_n60_), .d(new_n46_), .O(z05));
  oai21  g35(.a(new_n59_), .b(x10), .c(new_n45_), .O(new_n70_));
  nand3  g36(.a(new_n63_), .b(x08), .c(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n65_), .c(new_n42_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n72_), .c(new_n70_), .d(new_n46_), .O(z06));
  inv1   g43(.a(new_n46_), .O(new_n78_));
  oai21  g44(.a(new_n59_), .b(x11), .c(new_n45_), .O(new_n79_));
  oai21  g45(.a(new_n78_), .b(new_n42_), .c(new_n79_), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  inv1   g47(.a(new_n61_), .O(new_n82_));
  aoi21  g48(.a(new_n73_), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  inv1   g49(.a(x10), .O(new_n84_));
  nand3  g50(.a(new_n81_), .b(new_n84_), .c(new_n63_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n61_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n83_), .c(x06), .O(new_n87_));
  nand3  g53(.a(x13), .b(x11), .c(new_n45_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n80_), .O(z07));
  oai21  g55(.a(new_n59_), .b(x12), .c(new_n45_), .O(new_n90_));
  oai21  g56(.a(new_n85_), .b(new_n61_), .c(x12), .O(new_n91_));
  nor2   g57(.a(x12), .b(x11), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n73_), .c(new_n82_), .d(x06), .O(new_n93_));
  and2   g59(.a(new_n93_), .b(x04), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n91_), .c(new_n90_), .d(new_n46_), .O(z08));
  oai21  g61(.a(new_n78_), .b(new_n42_), .c(new_n53_), .O(new_n96_));
  nand2  g62(.a(new_n93_), .b(x13), .O(new_n97_));
  inv1   g63(.a(x12), .O(new_n98_));
  nand4  g64(.a(new_n59_), .b(new_n98_), .c(new_n81_), .d(new_n84_), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n67_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n97_), .c(new_n96_), .O(z09));
  nand2  g68(.a(x06), .b(x00), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n46_), .c(new_n63_), .d(x08), .O(new_n105_));
  nand2  g71(.a(new_n78_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n64_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n53_), .O(z10));
  nand2  g75(.a(new_n53_), .b(new_n35_), .O(z11));
  inv1   g76(.a(x03), .O(new_n111_));
  inv1   g77(.a(x16), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(x02), .c(new_n111_), .O(new_n113_));
  nand3  g79(.a(new_n113_), .b(new_n53_), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z12));
  nor2   g81(.a(new_n36_), .b(new_n42_), .O(z13));
  inv1   g82(.a(x17), .O(new_n117_));
  nor3   g83(.a(new_n36_), .b(new_n117_), .c(new_n42_), .O(z14));
endmodule


