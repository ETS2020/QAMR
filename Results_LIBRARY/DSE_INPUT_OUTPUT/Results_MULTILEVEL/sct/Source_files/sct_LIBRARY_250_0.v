// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n120_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x14), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n36_), .O(z11));
  inv1   g05(.a(x14), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(x06), .c(new_n35_), .O(new_n41_));
  oai21  g07(.a(z11), .b(new_n35_), .c(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n37_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand4  g15(.a(new_n49_), .b(x14), .c(new_n48_), .d(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand3  g17(.a(x14), .b(x07), .c(new_n48_), .O(new_n52_));
  oai21  g18(.a(x07), .b(new_n48_), .c(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n49_), .c(x04), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xnor2a g21(.a(x08), .b(x07), .O(new_n56_));
  nand2  g22(.a(x08), .b(new_n48_), .O(new_n57_));
  oai21  g23(.a(new_n56_), .b(new_n48_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n49_), .c(x04), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n38_), .O(z04));
  oai21  g26(.a(new_n40_), .b(x09), .c(new_n48_), .O(new_n61_));
  nand2  g27(.a(new_n49_), .b(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand2  g30(.a(x08), .b(x07), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand3  g32(.a(new_n64_), .b(x08), .c(x07), .O(new_n67_));
  oai21  g33(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x06), .O(new_n69_));
  nand3  g35(.a(x14), .b(x09), .c(new_n48_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n63_), .O(z05));
  oai21  g37(.a(new_n40_), .b(x10), .c(new_n48_), .O(new_n72_));
  nand2  g38(.a(new_n67_), .b(x10), .O(new_n73_));
  and2   g39(.a(x07), .b(x06), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x08), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n74_), .c(new_n45_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n73_), .c(new_n72_), .d(new_n49_), .O(z06));
  oai21  g45(.a(new_n40_), .b(x11), .c(new_n48_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n62_), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  aoi21  g48(.a(new_n75_), .b(new_n66_), .c(new_n82_), .O(new_n83_));
  nor3   g49(.a(x11), .b(x10), .c(x09), .O(new_n84_));
  and2   g50(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n83_), .c(x06), .O(new_n86_));
  nand3  g52(.a(x14), .b(x11), .c(new_n48_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n81_), .O(z07));
  oai21  g54(.a(new_n40_), .b(x12), .c(new_n48_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n62_), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  aoi21  g57(.a(new_n84_), .b(new_n66_), .c(new_n91_), .O(new_n92_));
  inv1   g58(.a(x10), .O(new_n93_));
  nand3  g59(.a(new_n91_), .b(new_n82_), .c(new_n93_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n67_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n92_), .c(x06), .O(new_n96_));
  nand3  g62(.a(x14), .b(x12), .c(new_n48_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n96_), .c(new_n90_), .O(z08));
  oai21  g64(.a(new_n40_), .b(x13), .c(new_n48_), .O(new_n99_));
  oai21  g65(.a(new_n94_), .b(new_n67_), .c(x13), .O(new_n100_));
  nand3  g66(.a(new_n74_), .b(new_n64_), .c(x08), .O(new_n101_));
  inv1   g67(.a(x13), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n91_), .c(new_n82_), .d(new_n93_), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n45_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n100_), .c(new_n99_), .d(new_n49_), .O(z09));
  inv1   g72(.a(x00), .O(new_n107_));
  nand2  g73(.a(new_n103_), .b(new_n107_), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n49_), .c(new_n64_), .d(x08), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(new_n110_));
  nand3  g76(.a(new_n110_), .b(x07), .c(x06), .O(new_n111_));
  or2    g77(.a(new_n49_), .b(new_n40_), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n111_), .c(new_n45_), .O(z10));
  inv1   g79(.a(x03), .O(new_n114_));
  inv1   g80(.a(x16), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(x02), .c(new_n114_), .O(new_n116_));
  nand3  g82(.a(new_n116_), .b(new_n38_), .c(x04), .O(new_n117_));
  inv1   g83(.a(new_n117_), .O(z12));
  nand2  g84(.a(new_n38_), .b(new_n45_), .O(z13));
  inv1   g85(.a(x17), .O(new_n120_));
  nor3   g86(.a(new_n37_), .b(new_n120_), .c(new_n45_), .O(z14));
endmodule


