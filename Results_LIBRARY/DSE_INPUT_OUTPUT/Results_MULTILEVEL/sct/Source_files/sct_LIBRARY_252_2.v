// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n106_, new_n107_, new_n110_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x07), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n44_), .c(new_n41_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n38_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand4  g17(.a(new_n49_), .b(x07), .c(new_n48_), .d(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(z03));
  xor2a  g19(.a(x08), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(z04));
  inv1   g22(.a(x08), .O(new_n57_));
  nor2   g23(.a(x09), .b(new_n57_), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n36_), .c(x06), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(x09), .c(new_n45_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n49_), .O(z05));
  inv1   g28(.a(x09), .O(new_n63_));
  inv1   g29(.a(x10), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n36_), .c(x06), .O(new_n67_));
  nand3  g33(.a(new_n58_), .b(x07), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x10), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n49_), .d(x04), .O(z06));
  inv1   g36(.a(new_n49_), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n45_), .c(new_n37_), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n63_), .c(x08), .d(x06), .O(new_n75_));
  oai21  g41(.a(new_n66_), .b(new_n73_), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x07), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n48_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n72_), .O(z07));
  inv1   g45(.a(x12), .O(new_n80_));
  nand4  g46(.a(new_n58_), .b(new_n80_), .c(new_n73_), .d(new_n64_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x07), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x06), .O(new_n83_));
  inv1   g49(.a(new_n60_), .O(new_n84_));
  nand3  g50(.a(new_n74_), .b(new_n84_), .c(new_n63_), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(x12), .c(new_n45_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n83_), .c(new_n49_), .O(z08));
  and2   g53(.a(new_n81_), .b(x13), .O(new_n88_));
  nand2  g54(.a(new_n58_), .b(x06), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n80_), .c(new_n73_), .d(new_n64_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n88_), .c(x07), .O(new_n93_));
  nand2  g59(.a(x13), .b(new_n48_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n72_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n49_), .c(new_n63_), .d(x08), .O(new_n98_));
  nand2  g64(.a(new_n71_), .b(x14), .O(new_n99_));
  oai21  g65(.a(new_n98_), .b(new_n48_), .c(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n37_), .O(z10));
  nand2  g68(.a(new_n37_), .b(new_n38_), .O(z11));
  inv1   g69(.a(x03), .O(new_n104_));
  inv1   g70(.a(x16), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(x02), .c(new_n104_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n37_), .c(x04), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(z12));
  nand2  g74(.a(new_n37_), .b(new_n45_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n37_), .O(z14));
endmodule


