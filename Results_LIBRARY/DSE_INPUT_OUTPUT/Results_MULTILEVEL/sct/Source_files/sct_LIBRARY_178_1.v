// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n102_,
    new_n105_;
  and2   g00(.a(x14), .b(x13), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x13), .O(new_n38_));
  aoi21  g04(.a(new_n36_), .b(x01), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n35_), .b(x01), .c(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n38_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n38_), .O(new_n46_));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x04), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n46_), .O(z02));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n51_), .c(new_n46_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n57_), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n51_), .c(x04), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n46_), .O(z04));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n58_), .b(new_n65_), .c(x08), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(new_n38_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n64_), .c(new_n51_), .d(x04), .O(z05));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n58_), .c(x08), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x04), .O(new_n72_));
  aoi21  g38(.a(new_n66_), .b(x10), .c(new_n72_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n51_), .c(new_n38_), .O(z06));
  inv1   g40(.a(new_n63_), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n65_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x04), .O(new_n78_));
  aoi21  g44(.a(new_n71_), .b(x11), .c(new_n78_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n51_), .c(new_n38_), .O(z07));
  nor2   g46(.a(x12), .b(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n70_), .c(new_n75_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  aoi21  g49(.a(new_n77_), .b(x12), .c(new_n83_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n51_), .c(new_n38_), .O(z08));
  oai21  g51(.a(new_n50_), .b(new_n43_), .c(new_n46_), .O(new_n86_));
  nand2  g52(.a(new_n82_), .b(x13), .O(new_n87_));
  nor3   g53(.a(x14), .b(x13), .c(x12), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n76_), .c(new_n67_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(z09));
  inv1   g56(.a(x00), .O(new_n91_));
  nor2   g57(.a(x13), .b(x12), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n76_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n91_), .c(new_n50_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n65_), .c(x08), .d(x07), .O(new_n95_));
  nand2  g61(.a(new_n50_), .b(x14), .O(new_n96_));
  oai21  g62(.a(new_n95_), .b(new_n47_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n46_), .O(z10));
  nor2   g65(.a(new_n38_), .b(new_n36_), .O(z11));
  oai21  g66(.a(new_n49_), .b(x02), .c(new_n48_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n46_), .c(x04), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(z12));
  nand2  g69(.a(new_n46_), .b(new_n43_), .O(z13));
  inv1   g70(.a(x17), .O(new_n105_));
  nor3   g71(.a(new_n38_), .b(new_n105_), .c(new_n43_), .O(z14));
endmodule


