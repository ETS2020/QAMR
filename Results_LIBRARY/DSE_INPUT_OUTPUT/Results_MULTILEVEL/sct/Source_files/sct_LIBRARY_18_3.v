// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n110_;
  inv1   g00(.a(x07), .O(new_n35_));
  inv1   g01(.a(x12), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(x02), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g08(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(z00));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x15), .c(new_n44_), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nand2  g13(.a(x05), .b(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z01));
  inv1   g15(.a(new_n37_), .O(new_n50_));
  inv1   g16(.a(x06), .O(new_n51_));
  oai21  g17(.a(x03), .b(new_n38_), .c(x16), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n51_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n50_), .O(z02));
  nand3  g20(.a(new_n36_), .b(x07), .c(new_n51_), .O(new_n55_));
  oai21  g21(.a(x07), .b(new_n51_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n52_), .c(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  xnor2a g24(.a(x08), .b(x07), .O(new_n59_));
  nand2  g25(.a(x08), .b(new_n51_), .O(new_n60_));
  oai21  g26(.a(new_n59_), .b(new_n51_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n52_), .c(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n50_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x08), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n36_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x07), .O(new_n67_));
  nand4  g33(.a(new_n36_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x09), .c(new_n47_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n52_), .O(z05));
  nand2  g36(.a(new_n52_), .b(x04), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n50_), .O(new_n72_));
  nand2  g38(.a(new_n67_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand3  g40(.a(x08), .b(x07), .c(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n36_), .c(new_n74_), .d(new_n64_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n73_), .c(new_n72_), .O(z06));
  nand2  g44(.a(x08), .b(x06), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  nor2   g46(.a(x10), .b(x09), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n80_), .c(x12), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n35_), .c(x11), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand4  g50(.a(new_n81_), .b(new_n76_), .c(new_n36_), .d(new_n84_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n83_), .c(new_n72_), .O(z07));
  nand3  g52(.a(new_n84_), .b(new_n74_), .c(new_n64_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n76_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n52_), .c(new_n36_), .d(x04), .O(z08));
  oai21  g56(.a(new_n87_), .b(new_n79_), .c(x13), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n81_), .b(new_n76_), .c(new_n92_), .d(new_n84_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n36_), .O(new_n95_));
  nand2  g61(.a(x13), .b(new_n35_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n95_), .c(new_n72_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand3  g64(.a(new_n92_), .b(new_n84_), .c(new_n74_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n52_), .c(new_n64_), .d(x08), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n35_), .O(new_n102_));
  nor2   g68(.a(new_n52_), .b(new_n41_), .O(new_n103_));
  aoi21  g69(.a(new_n102_), .b(x06), .c(new_n103_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n47_), .c(new_n50_), .O(z10));
  nor2   g71(.a(new_n37_), .b(new_n38_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n38_), .c(x03), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n47_), .c(new_n50_), .O(z12));
  nor2   g74(.a(new_n37_), .b(new_n47_), .O(z13));
  nand3  g75(.a(new_n50_), .b(x17), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z14));
endmodule


