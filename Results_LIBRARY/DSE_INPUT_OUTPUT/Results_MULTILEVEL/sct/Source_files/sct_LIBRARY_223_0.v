// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n117_, new_n118_, new_n121_;
  nor2   g00(.a(x08), .b(x06), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  aoi21  g04(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n35_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(x08), .c(new_n46_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n35_), .O(new_n53_));
  xor2a  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n50_), .c(x04), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n53_), .O(z03));
  xnor2a g22(.a(x08), .b(x07), .O(new_n57_));
  nand2  g23(.a(x08), .b(new_n46_), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(new_n46_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n50_), .c(x04), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(x08), .O(new_n62_));
  oai21  g28(.a(x09), .b(new_n62_), .c(new_n46_), .O(new_n63_));
  oai21  g29(.a(new_n49_), .b(new_n43_), .c(new_n63_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x07), .c(x06), .O(new_n66_));
  oai21  g32(.a(new_n65_), .b(x06), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x08), .O(new_n68_));
  inv1   g34(.a(x07), .O(new_n69_));
  nor2   g35(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(x09), .c(x06), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n68_), .c(new_n64_), .O(z05));
  oai21  g39(.a(x10), .b(new_n62_), .c(new_n46_), .O(new_n74_));
  nand3  g40(.a(new_n65_), .b(x08), .c(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x10), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(x08), .c(x07), .d(x06), .O(new_n78_));
  and2   g44(.a(new_n78_), .b(x04), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n76_), .c(new_n74_), .d(new_n50_), .O(z06));
  oai21  g46(.a(x11), .b(new_n62_), .c(new_n46_), .O(new_n81_));
  nand2  g47(.a(new_n77_), .b(new_n70_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x11), .O(new_n83_));
  nand2  g49(.a(new_n70_), .b(x06), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nor3   g51(.a(x11), .b(x10), .c(x09), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n85_), .c(new_n43_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n83_), .c(new_n81_), .d(new_n50_), .O(z07));
  oai21  g54(.a(x12), .b(new_n62_), .c(new_n46_), .O(new_n89_));
  nand2  g55(.a(new_n86_), .b(new_n70_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x12), .O(new_n91_));
  inv1   g57(.a(x11), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand3  g59(.a(new_n77_), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n85_), .c(new_n43_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n91_), .c(new_n89_), .d(new_n50_), .O(z08));
  oai21  g63(.a(x13), .b(new_n62_), .c(new_n46_), .O(new_n98_));
  oai21  g64(.a(new_n49_), .b(new_n43_), .c(new_n98_), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  inv1   g66(.a(x10), .O(new_n101_));
  nand4  g67(.a(new_n100_), .b(new_n93_), .c(new_n92_), .d(new_n101_), .O(new_n102_));
  oai22  g68(.a(new_n102_), .b(new_n66_), .c(new_n100_), .d(x06), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x08), .O(new_n104_));
  inv1   g70(.a(new_n75_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n93_), .c(new_n92_), .d(new_n101_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(x13), .c(x06), .O(new_n107_));
  nand3  g73(.a(new_n107_), .b(new_n104_), .c(new_n99_), .O(z09));
  inv1   g74(.a(x00), .O(new_n109_));
  aoi21  g75(.a(new_n102_), .b(new_n109_), .c(new_n49_), .O(new_n110_));
  nand4  g76(.a(new_n110_), .b(new_n65_), .c(x08), .d(x07), .O(new_n111_));
  nand2  g77(.a(new_n49_), .b(x14), .O(new_n112_));
  oai21  g78(.a(new_n111_), .b(new_n46_), .c(new_n112_), .O(new_n113_));
  nand2  g79(.a(new_n113_), .b(x04), .O(new_n114_));
  nand2  g80(.a(new_n114_), .b(new_n53_), .O(z10));
  nor2   g81(.a(new_n35_), .b(new_n36_), .O(z11));
  oai21  g82(.a(new_n48_), .b(x02), .c(new_n47_), .O(new_n117_));
  nand3  g83(.a(new_n117_), .b(new_n53_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z12));
  nand2  g85(.a(new_n53_), .b(new_n43_), .O(z13));
  nand2  g86(.a(x17), .b(x04), .O(new_n121_));
  nand2  g87(.a(new_n121_), .b(new_n53_), .O(z14));
endmodule


