// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n114_;
  inv1   g00(.a(x02), .O(new_n35_));
  nor2   g01(.a(x11), .b(x06), .O(new_n36_));
  aoi21  g02(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x05), .O(new_n39_));
  inv1   g05(.a(x18), .O(new_n40_));
  oai21  g06(.a(new_n40_), .b(x15), .c(new_n39_), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  nand2  g08(.a(x05), .b(new_n42_), .O(new_n43_));
  aoi21  g09(.a(new_n43_), .b(new_n41_), .c(new_n36_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(x11), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  inv1   g14(.a(new_n36_), .O(new_n49_));
  xor2a  g15(.a(x07), .b(x06), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n46_), .c(x04), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(z03));
  xnor2a g18(.a(x08), .b(x07), .O(new_n53_));
  nand3  g19(.a(x11), .b(x08), .c(new_n45_), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n46_), .c(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z04));
  inv1   g23(.a(x11), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(x09), .c(new_n45_), .O(new_n59_));
  inv1   g25(.a(x07), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nand2  g30(.a(x07), .b(x06), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n61_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n66_), .c(new_n42_), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n64_), .c(new_n59_), .d(new_n46_), .O(z05));
  inv1   g35(.a(new_n46_), .O(new_n70_));
  oai21  g36(.a(new_n58_), .b(x10), .c(new_n45_), .O(new_n71_));
  oai21  g37(.a(new_n70_), .b(new_n42_), .c(new_n71_), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand2  g39(.a(new_n67_), .b(x07), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  inv1   g41(.a(x09), .O(new_n76_));
  nand3  g42(.a(new_n62_), .b(new_n73_), .c(new_n76_), .O(new_n77_));
  oai21  g43(.a(new_n75_), .b(new_n73_), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand3  g45(.a(x11), .b(x10), .c(new_n45_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n72_), .O(z06));
  oai21  g47(.a(new_n70_), .b(new_n42_), .c(new_n49_), .O(new_n82_));
  nand3  g48(.a(new_n73_), .b(new_n76_), .c(x08), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n65_), .c(x11), .O(new_n84_));
  nand3  g50(.a(new_n58_), .b(new_n73_), .c(new_n76_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n62_), .c(x06), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n84_), .c(new_n82_), .O(z07));
  oai21  g54(.a(x12), .b(new_n58_), .c(new_n45_), .O(new_n89_));
  oai21  g55(.a(new_n85_), .b(new_n63_), .c(x12), .O(new_n90_));
  nor3   g56(.a(x12), .b(x11), .c(x10), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n75_), .c(new_n42_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n90_), .c(new_n89_), .d(new_n46_), .O(z08));
  inv1   g59(.a(x13), .O(new_n94_));
  nor3   g60(.a(x12), .b(x10), .c(x09), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n62_), .c(new_n94_), .O(new_n96_));
  inv1   g62(.a(x12), .O(new_n97_));
  nand4  g63(.a(new_n94_), .b(new_n97_), .c(new_n58_), .d(new_n73_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n74_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n96_), .c(x06), .O(new_n100_));
  nand2  g66(.a(x13), .b(x11), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n100_), .c(new_n82_), .O(z09));
  nand2  g68(.a(x06), .b(x00), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n46_), .c(new_n76_), .d(x08), .O(new_n105_));
  nand2  g71(.a(new_n70_), .b(x14), .O(new_n106_));
  oai21  g72(.a(new_n105_), .b(new_n60_), .c(new_n106_), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n49_), .O(z10));
  nor2   g75(.a(new_n36_), .b(new_n35_), .O(z11));
  aoi21  g76(.a(x16), .b(new_n35_), .c(x03), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n42_), .c(new_n49_), .O(z12));
  nor2   g78(.a(new_n36_), .b(new_n42_), .O(z13));
  inv1   g79(.a(x17), .O(new_n114_));
  nor3   g80(.a(new_n36_), .b(new_n114_), .c(new_n42_), .O(z14));
endmodule


