// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n103_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(x13), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  nand2  g04(.a(new_n36_), .b(x13), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n47_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n39_), .O(z02));
  xor2a  g17(.a(x07), .b(x06), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n49_), .c(new_n39_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  xnor2a g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(new_n47_), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n47_), .c(new_n56_), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n49_), .c(new_n39_), .d(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n61_), .c(new_n49_), .d(x04), .O(new_n64_));
  and2   g30(.a(new_n64_), .b(new_n39_), .O(z05));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n39_), .c(x04), .O(new_n68_));
  aoi21  g34(.a(new_n63_), .b(x10), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n49_), .O(z06));
  inv1   g36(.a(new_n60_), .O(new_n71_));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n62_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  aoi21  g40(.a(new_n67_), .b(x11), .c(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n49_), .c(new_n40_), .O(z07));
  inv1   g42(.a(x11), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  nand4  g44(.a(new_n66_), .b(new_n71_), .c(new_n78_), .d(new_n77_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n39_), .c(x04), .O(new_n80_));
  aoi21  g46(.a(new_n73_), .b(x12), .c(new_n80_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n49_), .O(z08));
  nand3  g48(.a(new_n72_), .b(x14), .c(new_n78_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n63_), .c(x13), .O(new_n84_));
  inv1   g50(.a(x13), .O(new_n85_));
  inv1   g51(.a(new_n63_), .O(new_n86_));
  nand4  g52(.a(new_n72_), .b(new_n86_), .c(new_n85_), .d(new_n78_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n84_), .c(new_n49_), .d(x04), .O(z09));
  inv1   g54(.a(x00), .O(new_n89_));
  inv1   g55(.a(x10), .O(new_n90_));
  nand3  g56(.a(new_n78_), .b(new_n77_), .c(new_n90_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n49_), .c(new_n85_), .O(new_n93_));
  nand3  g59(.a(x14), .b(x13), .c(x00), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n93_), .c(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(x08), .c(x07), .d(x06), .O(new_n96_));
  or2    g62(.a(new_n49_), .b(new_n36_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n96_), .c(new_n44_), .O(z10));
  nand2  g64(.a(new_n39_), .b(new_n48_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n48_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n44_), .c(new_n39_), .O(z12));
  nor2   g67(.a(new_n40_), .b(new_n44_), .O(z13));
  nand2  g68(.a(x17), .b(x04), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n39_), .O(z14));
endmodule


