// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x18), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi22  g06(.a(new_n40_), .b(new_n37_), .c(new_n35_), .d(x04), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  nand2  g08(.a(x15), .b(new_n42_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n43_), .c(x18), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand4  g14(.a(new_n48_), .b(x18), .c(new_n47_), .d(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  xor2a  g16(.a(x07), .b(x06), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(x18), .c(new_n44_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  and2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n54_), .b(x07), .c(x06), .O(new_n56_));
  oai21  g22(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n48_), .c(x18), .d(x04), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand3  g28(.a(new_n55_), .b(new_n62_), .c(x08), .O(new_n63_));
  and2   g29(.a(new_n63_), .b(x18), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n61_), .c(new_n48_), .d(x04), .O(z05));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n55_), .c(x08), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x18), .c(x04), .O(new_n68_));
  aoi21  g34(.a(new_n63_), .b(x10), .c(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n48_), .O(z06));
  nand2  g36(.a(new_n67_), .b(x11), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  inv1   g39(.a(new_n60_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n62_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n71_), .c(new_n48_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x18), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x04), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  nand4  g45(.a(new_n66_), .b(new_n74_), .c(new_n79_), .d(new_n73_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(x18), .c(x04), .O(new_n81_));
  aoi21  g47(.a(new_n75_), .b(x12), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n48_), .O(z08));
  nand2  g49(.a(new_n80_), .b(x13), .O(new_n84_));
  inv1   g50(.a(new_n63_), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n79_), .c(new_n73_), .d(new_n72_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n84_), .c(new_n48_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x18), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x04), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n87_), .b(new_n93_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n48_), .c(new_n62_), .d(x08), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(x07), .c(x06), .O(new_n97_));
  nor2   g63(.a(new_n48_), .b(new_n39_), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n35_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n97_), .c(new_n44_), .O(z10));
  aoi21  g66(.a(new_n35_), .b(x04), .c(new_n36_), .O(z11));
  inv1   g67(.a(x03), .O(new_n102_));
  aoi21  g68(.a(x16), .b(new_n36_), .c(new_n35_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n44_), .O(z12));
  nor2   g70(.a(new_n35_), .b(new_n44_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  aoi21  g72(.a(x18), .b(new_n106_), .c(new_n44_), .O(z14));
endmodule


