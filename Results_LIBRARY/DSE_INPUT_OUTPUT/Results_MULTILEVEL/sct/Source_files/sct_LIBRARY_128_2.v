// Benchmark "FAU" written by ABC on Fri Aug 14 02:12:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n100_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(x11), .b(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  inv1   g11(.a(new_n37_), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n36_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n46_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand3  g18(.a(x11), .b(new_n52_), .c(x06), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(x06), .c(new_n53_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n49_), .c(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x11), .c(x06), .O(new_n58_));
  nand2  g24(.a(x08), .b(new_n36_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n49_), .c(x04), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x11), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  nand4  g32(.a(x11), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(x09), .c(new_n43_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n49_), .O(z05));
  nand2  g35(.a(x08), .b(x07), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n63_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n70_), .c(x11), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(x07), .b(x06), .O(new_n75_));
  nand3  g41(.a(x11), .b(new_n63_), .c(x08), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n75_), .c(x10), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n74_), .c(new_n49_), .d(x04), .O(z06));
  nand3  g44(.a(x11), .b(new_n71_), .c(new_n63_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n70_), .c(x06), .O(new_n80_));
  aoi21  g46(.a(x11), .b(new_n36_), .c(new_n43_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n49_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n49_), .b(new_n46_), .c(new_n83_), .d(x04), .O(z08));
  inv1   g50(.a(x13), .O(new_n85_));
  nand4  g51(.a(new_n49_), .b(new_n46_), .c(new_n85_), .d(x04), .O(z09));
  inv1   g52(.a(x00), .O(new_n87_));
  aoi21  g53(.a(new_n48_), .b(x16), .c(x09), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(x08), .c(x07), .d(x06), .O(new_n89_));
  inv1   g55(.a(new_n49_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x14), .O(new_n91_));
  oai21  g57(.a(new_n89_), .b(new_n87_), .c(new_n91_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x11), .O(new_n93_));
  nand3  g59(.a(new_n90_), .b(x14), .c(new_n36_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n93_), .c(new_n43_), .O(z10));
  nor2   g61(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n35_), .c(x03), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n43_), .c(new_n46_), .O(z12));
  nand2  g64(.a(new_n46_), .b(new_n43_), .O(z13));
  inv1   g65(.a(x17), .O(new_n100_));
  nor3   g66(.a(new_n37_), .b(new_n100_), .c(new_n43_), .O(z14));
endmodule


