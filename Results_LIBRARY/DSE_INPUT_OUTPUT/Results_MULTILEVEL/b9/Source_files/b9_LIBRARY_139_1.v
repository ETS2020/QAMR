// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_;
  inv1   g000(.a(x04), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  oai21  g002(.a(x27), .b(x10), .c(x40), .O(new_n65_));
  nor2   g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n66_), .b(new_n63_), .c(new_n70_), .O(new_n71_));
  inv1   g009(.a(x36), .O(new_n72_));
  nand2  g010(.a(x40), .b(new_n72_), .O(new_n73_));
  and2   g011(.a(new_n73_), .b(x16), .O(new_n74_));
  oai21  g012(.a(new_n71_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(x27), .c(x04), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n73_), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x40), .O(new_n84_));
  nand4  g022(.a(new_n84_), .b(new_n72_), .c(new_n77_), .d(new_n76_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n80_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n82_), .c(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x36), .O(new_n94_));
  inv1   g032(.a(new_n94_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n91_), .c(new_n89_), .O(z02));
  inv1   g034(.a(new_n73_), .O(new_n97_));
  nand2  g035(.a(new_n69_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n68_), .b(new_n67_), .c(new_n99_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(z03));
  aoi21  g039(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n98_), .c(new_n97_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n73_), .c(new_n68_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  inv1   g045(.a(x15), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n108_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n73_), .O(z07));
  inv1   g056(.a(x40), .O(new_n119_));
  aoi21  g057(.a(new_n64_), .b(x36), .c(new_n119_), .O(z08));
  inv1   g058(.a(x11), .O(new_n121_));
  nand4  g059(.a(new_n73_), .b(x34), .c(x27), .d(x26), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n121_), .O(z09));
  nand3  g061(.a(new_n73_), .b(x37), .c(x06), .O(new_n124_));
  inv1   g062(.a(x07), .O(new_n125_));
  nand2  g063(.a(new_n92_), .b(x36), .O(new_n126_));
  nand3  g064(.a(new_n119_), .b(x35), .c(new_n81_), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(new_n128_));
  nand4  g066(.a(x40), .b(x39), .c(x36), .d(x05), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n128_), .c(new_n63_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n124_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x27), .O(new_n133_));
  nand2  g071(.a(new_n78_), .b(x40), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n64_), .O(new_n135_));
  nand4  g073(.a(new_n135_), .b(x36), .c(x05), .d(new_n63_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(z10));
  nand2  g075(.a(new_n73_), .b(new_n67_), .O(new_n138_));
  oai21  g076(.a(new_n82_), .b(x04), .c(x36), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n138_), .c(x08), .O(new_n140_));
  nand2  g078(.a(x39), .b(x29), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x36), .c(new_n119_), .O(new_n142_));
  nand2  g080(.a(x27), .b(x04), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n83_), .c(new_n142_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n140_), .c(new_n76_), .O(new_n145_));
  nor2   g083(.a(new_n145_), .b(x09), .O(z11));
  nand2  g084(.a(x39), .b(new_n63_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x36), .c(new_n119_), .O(new_n148_));
  nand2  g086(.a(x36), .b(new_n67_), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(x10), .c(new_n148_), .d(new_n70_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  inv1   g089(.a(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n119_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n63_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand2  g094(.a(new_n69_), .b(new_n68_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n154_), .c(new_n67_), .O(new_n159_));
  nand4  g097(.a(new_n78_), .b(x39), .c(new_n151_), .d(new_n63_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x36), .c(new_n119_), .O(new_n161_));
  or2    g099(.a(new_n161_), .b(new_n159_), .O(z13));
  nand3  g100(.a(x39), .b(new_n151_), .c(new_n63_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  nand4  g102(.a(x27), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n157_), .c(new_n164_), .O(new_n167_));
  nand4  g105(.a(x37), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n152_), .c(x28), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(x27), .c(x32), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n76_), .c(new_n167_), .O(new_n171_));
  nand2  g109(.a(new_n166_), .b(new_n157_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n119_), .O(new_n173_));
  oai21  g111(.a(new_n171_), .b(new_n72_), .c(new_n173_), .O(z14));
  nand4  g112(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n73_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand4  g115(.a(new_n73_), .b(new_n177_), .c(x22), .d(x01), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(x23), .c(x22), .d(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n73_), .O(z17));
  nand2  g120(.a(x27), .b(new_n63_), .O(new_n183_));
  nand2  g121(.a(new_n67_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n152_), .O(new_n185_));
  inv1   g123(.a(x09), .O(new_n186_));
  nand2  g124(.a(new_n76_), .b(new_n186_), .O(new_n187_));
  aoi21  g125(.a(new_n185_), .b(new_n81_), .c(new_n187_), .O(new_n188_));
  oai22  g126(.a(new_n188_), .b(new_n97_), .c(new_n94_), .d(new_n88_), .O(z18));
  nand2  g127(.a(x40), .b(x04), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x35), .c(new_n81_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(x36), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n138_), .c(x08), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n144_), .c(new_n76_), .O(new_n194_));
  nor2   g132(.a(new_n194_), .b(x09), .O(z19));
  nor2   g133(.a(new_n152_), .b(new_n67_), .O(new_n196_));
  aoi21  g134(.a(x28), .b(new_n67_), .c(new_n152_), .O(new_n197_));
  nand2  g135(.a(new_n93_), .b(x29), .O(new_n198_));
  inv1   g136(.a(new_n198_), .O(new_n199_));
  oai22  g137(.a(new_n199_), .b(new_n197_), .c(new_n196_), .d(x08), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n76_), .c(new_n186_), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n73_), .O(z20));
endmodule


