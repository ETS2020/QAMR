// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n202_, new_n203_, new_n204_, new_n205_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x19), .O(new_n77_));
  inv1   g015(.a(x26), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n76_), .c(x16), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n83_), .c(new_n82_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g030(.a(new_n66_), .b(x28), .O(new_n93_));
  oai21  g031(.a(x27), .b(x08), .c(new_n93_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x04), .c(new_n72_), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n92_), .c(new_n79_), .O(z02));
  oai21  g034(.a(new_n66_), .b(new_n84_), .c(x27), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n80_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n98_), .b(new_n64_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n97_), .c(new_n79_), .O(z04));
  nor2   g040(.a(new_n84_), .b(new_n64_), .O(new_n103_));
  inv1   g041(.a(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n80_), .c(new_n98_), .O(z05));
  oai21  g043(.a(new_n103_), .b(x37), .c(new_n80_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n63_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n80_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  nand3  g055(.a(new_n80_), .b(x40), .c(x39), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z08));
  inv1   g057(.a(x34), .O(new_n120_));
  nand4  g058(.a(x27), .b(x26), .c(new_n77_), .d(x11), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  nand2  g060(.a(new_n72_), .b(x07), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n86_), .c(x27), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n83_), .b(new_n82_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x40), .c(x39), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n127_), .c(new_n65_), .O(new_n132_));
  nand3  g070(.a(x37), .b(x27), .c(x06), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n132_), .c(new_n79_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  nand3  g073(.a(x40), .b(x39), .c(x29), .O(new_n136_));
  nor2   g074(.a(new_n64_), .b(new_n65_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n85_), .c(new_n136_), .O(new_n138_));
  inv1   g076(.a(x08), .O(new_n139_));
  nand3  g077(.a(new_n93_), .b(x27), .c(new_n65_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(new_n80_), .c(new_n82_), .d(new_n135_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(z11));
  inv1   g082(.a(x10), .O(new_n145_));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n64_), .c(new_n145_), .O(new_n147_));
  oai22  g085(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n147_), .c(new_n79_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  inv1   g088(.a(x40), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n151_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n150_), .d(new_n65_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x20), .O(new_n156_));
  nor2   g094(.a(new_n70_), .b(new_n156_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n154_), .c(new_n64_), .O(new_n159_));
  nor3   g097(.a(new_n130_), .b(x13), .c(x04), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n77_), .O(new_n161_));
  inv1   g099(.a(x39), .O(new_n162_));
  aoi21  g100(.a(new_n152_), .b(x28), .c(new_n64_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n129_), .c(x40), .O(new_n164_));
  nor2   g102(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(new_n78_), .c(new_n150_), .d(new_n65_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n161_), .O(z13));
  aoi21  g105(.a(x20), .b(new_n155_), .c(x19), .O(new_n168_));
  nor2   g106(.a(x26), .b(new_n77_), .O(new_n169_));
  nand4  g107(.a(new_n152_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  nand3  g109(.a(new_n73_), .b(new_n150_), .c(new_n65_), .O(new_n172_));
  oai22  g110(.a(new_n172_), .b(new_n171_), .c(new_n169_), .d(new_n168_), .O(new_n173_));
  nand3  g111(.a(new_n98_), .b(new_n66_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n83_), .c(new_n82_), .O(new_n176_));
  oai21  g114(.a(new_n70_), .b(new_n64_), .c(new_n172_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n77_), .O(new_n179_));
  nand4  g117(.a(new_n83_), .b(new_n82_), .c(new_n64_), .d(new_n78_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n179_), .c(new_n173_), .O(z14));
  nand3  g119(.a(x34), .b(x27), .c(x12), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n77_), .c(new_n78_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(x22), .c(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n80_), .O(z16));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n187_), .b(x23), .c(x22), .d(x01), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n80_), .O(z17));
  oai21  g127(.a(new_n85_), .b(x27), .c(new_n136_), .O(new_n190_));
  nand3  g128(.a(new_n140_), .b(new_n82_), .c(new_n135_), .O(new_n191_));
  aoi21  g129(.a(new_n190_), .b(x08), .c(new_n191_), .O(new_n192_));
  nor2   g130(.a(new_n192_), .b(new_n79_), .O(z18));
  oai21  g131(.a(new_n66_), .b(new_n64_), .c(new_n139_), .O(new_n194_));
  nand2  g132(.a(new_n136_), .b(new_n66_), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n194_), .c(new_n79_), .O(new_n196_));
  oai21  g134(.a(x28), .b(x04), .c(new_n139_), .O(new_n197_));
  oai21  g135(.a(new_n137_), .b(x28), .c(new_n136_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n197_), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n196_), .c(new_n82_), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(x09), .c(new_n80_), .O(z19));
  inv1   g139(.a(new_n136_), .O(new_n202_));
  aoi21  g140(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n202_), .c(new_n194_), .O(new_n204_));
  nand4  g142(.a(new_n204_), .b(new_n80_), .c(new_n82_), .d(new_n135_), .O(new_n205_));
  inv1   g143(.a(new_n205_), .O(z20));
endmodule


