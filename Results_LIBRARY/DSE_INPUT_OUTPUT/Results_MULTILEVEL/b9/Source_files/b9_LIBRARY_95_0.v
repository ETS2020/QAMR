// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n191_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n204_, new_n206_,
    new_n207_, new_n208_, new_n209_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n64_), .d(new_n71_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x35), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(x28), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x36), .c(x27), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n79_), .c(new_n78_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g022(.a(x02), .O(new_n85_));
  inv1   g023(.a(x08), .O(new_n86_));
  inv1   g024(.a(x29), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  inv1   g026(.a(new_n81_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(z08));
  nand3  g031(.a(z08), .b(new_n91_), .c(new_n88_), .O(z02));
  nand2  g032(.a(new_n68_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n67_), .b(new_n66_), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n95_), .c(new_n63_), .O(z03));
  inv1   g036(.a(new_n63_), .O(new_n99_));
  nand2  g037(.a(new_n67_), .b(new_n66_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n95_), .c(new_n99_), .d(new_n96_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n63_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n64_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n99_), .O(z07));
  inv1   g053(.a(x11), .O(new_n116_));
  nand4  g054(.a(new_n99_), .b(x34), .c(x27), .d(x26), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  inv1   g056(.a(x28), .O(new_n119_));
  aoi21  g057(.a(x35), .b(new_n119_), .c(x36), .O(new_n120_));
  xor2a  g058(.a(x40), .b(x39), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x07), .O(new_n122_));
  nand2  g060(.a(z08), .b(x05), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n120_), .O(new_n124_));
  inv1   g062(.a(x06), .O(new_n125_));
  nor3   g063(.a(new_n63_), .b(new_n67_), .c(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n71_), .c(new_n126_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n79_), .b(new_n78_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x40), .c(x39), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n71_), .O(new_n132_));
  oai21  g070(.a(new_n127_), .b(new_n66_), .c(new_n132_), .O(z10));
  oai21  g071(.a(x40), .b(x39), .c(new_n66_), .O(new_n134_));
  nand3  g072(.a(x35), .b(new_n119_), .c(new_n71_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x39), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n134_), .c(x08), .O(new_n137_));
  inv1   g075(.a(x39), .O(new_n138_));
  nand2  g076(.a(x40), .b(new_n138_), .O(new_n139_));
  nand2  g077(.a(x40), .b(x29), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x39), .O(new_n141_));
  nand2  g079(.a(x27), .b(x04), .O(new_n142_));
  aoi22  g080(.a(new_n142_), .b(new_n81_), .c(new_n141_), .d(new_n139_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n137_), .c(new_n78_), .O(new_n144_));
  nor2   g082(.a(new_n144_), .b(x09), .O(z11));
  inv1   g083(.a(x36), .O(new_n146_));
  nand3  g084(.a(new_n67_), .b(new_n146_), .c(new_n80_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  aoi21  g086(.a(x40), .b(new_n71_), .c(new_n69_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(x39), .O(new_n150_));
  nor2   g088(.a(new_n69_), .b(new_n73_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n138_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x28), .c(new_n73_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n154_), .d(new_n71_), .O(new_n157_));
  inv1   g095(.a(x18), .O(new_n158_));
  inv1   g096(.a(x19), .O(new_n159_));
  nand2  g097(.a(new_n68_), .b(new_n67_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x20), .c(new_n159_), .d(new_n158_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nor2   g101(.a(new_n130_), .b(x13), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n71_), .c(new_n63_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n163_), .O(z13));
  inv1   g104(.a(new_n139_), .O(new_n167_));
  nand3  g105(.a(x20), .b(new_n159_), .c(new_n158_), .O(new_n168_));
  nand4  g106(.a(new_n155_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n169_));
  nand3  g107(.a(x40), .b(new_n154_), .c(new_n71_), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n169_), .c(new_n138_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n167_), .c(new_n168_), .O(new_n173_));
  nand3  g111(.a(new_n67_), .b(new_n80_), .c(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n79_), .c(new_n78_), .O(new_n176_));
  oai21  g114(.a(new_n171_), .b(new_n69_), .c(new_n176_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x39), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n173_), .c(new_n152_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nor2   g118(.a(new_n117_), .b(new_n180_), .O(z15));
  inv1   g119(.a(x23), .O(new_n182_));
  nand4  g120(.a(new_n99_), .b(new_n182_), .c(x22), .d(x01), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(z16));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(x23), .c(x22), .d(x01), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n99_), .O(z17));
  oai22  g125(.a(new_n92_), .b(new_n87_), .c(new_n89_), .d(x27), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(x08), .O(new_n189_));
  nor2   g127(.a(new_n66_), .b(x04), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n81_), .c(x09), .O(new_n191_));
  nand4  g129(.a(new_n191_), .b(new_n189_), .c(new_n99_), .d(new_n78_), .O(z18));
  inv1   g130(.a(x09), .O(new_n193_));
  oai21  g131(.a(new_n80_), .b(x04), .c(x39), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n134_), .c(new_n119_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n86_), .O(new_n196_));
  nand2  g134(.a(x39), .b(new_n80_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n142_), .c(new_n119_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n140_), .O(new_n199_));
  nand2  g137(.a(x40), .b(new_n80_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n142_), .c(new_n119_), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n138_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n199_), .c(new_n196_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n78_), .c(new_n193_), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(new_n99_), .O(z19));
  nand2  g143(.a(x28), .b(new_n66_), .O(new_n206_));
  aoi22  g144(.a(new_n206_), .b(x35), .c(new_n141_), .d(new_n139_), .O(new_n207_));
  aoi21  g145(.a(new_n197_), .b(new_n134_), .c(x08), .O(new_n208_));
  oai21  g146(.a(new_n208_), .b(new_n207_), .c(new_n78_), .O(new_n209_));
  nor2   g147(.a(new_n209_), .b(x09), .O(z20));
endmodule


