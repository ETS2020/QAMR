// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x24), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n67_), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n75_), .c(new_n63_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n66_), .d(new_n74_), .O(new_n78_));
  oai21  g016(.a(new_n73_), .b(new_n65_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x28), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n81_), .c(new_n80_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n64_), .c(x04), .O(z01));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n85_), .c(x04), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x39), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n90_), .c(x40), .O(new_n94_));
  nor2   g032(.a(x40), .b(x24), .O(new_n95_));
  inv1   g033(.a(new_n95_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z02));
  nand2  g035(.a(new_n69_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(x27), .O(new_n100_));
  aoi21  g038(.a(new_n68_), .b(new_n100_), .c(new_n99_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n65_), .O(z03));
  aoi21  g040(.a(new_n68_), .b(new_n100_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n98_), .c(new_n65_), .O(z04));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n68_), .c(new_n65_), .O(z05));
  nand3  g044(.a(new_n105_), .b(new_n64_), .c(new_n68_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n66_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n64_), .c(x03), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  nor2   g058(.a(new_n63_), .b(new_n120_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n64_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n64_), .O(z09));
  inv1   g063(.a(x07), .O(new_n126_));
  aoi21  g064(.a(x40), .b(new_n120_), .c(new_n95_), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x05), .O(new_n128_));
  oai21  g066(.a(new_n127_), .b(new_n126_), .c(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n86_), .c(new_n74_), .O(new_n130_));
  nand3  g068(.a(new_n64_), .b(x37), .c(x06), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x27), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor2   g072(.a(new_n134_), .b(new_n120_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x05), .c(new_n74_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand2  g076(.a(new_n121_), .b(x29), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n84_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  inv1   g080(.a(x08), .O(new_n143_));
  nand3  g081(.a(new_n84_), .b(x27), .c(new_n74_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n80_), .c(new_n138_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n64_), .O(z11));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n100_), .c(new_n75_), .O(new_n150_));
  oai21  g088(.a(new_n122_), .b(x04), .c(new_n71_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n150_), .c(new_n64_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x28), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n74_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n70_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n156_), .c(new_n63_), .O(new_n160_));
  inv1   g098(.a(x24), .O(new_n161_));
  nand4  g099(.a(new_n70_), .b(new_n161_), .c(x20), .d(new_n158_), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(x18), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n160_), .c(x27), .O(new_n164_));
  nand3  g102(.a(new_n135_), .b(new_n153_), .c(new_n74_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(z13));
  nand3  g104(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n167_));
  nand4  g105(.a(new_n154_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n168_));
  nand3  g106(.a(x39), .b(new_n153_), .c(new_n74_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n168_), .c(new_n63_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n95_), .c(new_n167_), .O(new_n172_));
  nand3  g110(.a(new_n68_), .b(new_n83_), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n81_), .c(new_n80_), .O(new_n175_));
  oai21  g113(.a(new_n170_), .b(new_n72_), .c(new_n175_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x40), .O(new_n177_));
  nand3  g115(.a(new_n71_), .b(new_n63_), .c(new_n161_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n177_), .c(new_n172_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nand4  g118(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n180_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand4  g121(.a(new_n64_), .b(new_n183_), .c(x22), .d(x01), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(z16));
  nand3  g123(.a(x23), .b(x22), .c(x01), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(x24), .O(z17));
  oai21  g125(.a(new_n85_), .b(x27), .c(new_n139_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(x08), .O(new_n189_));
  nor2   g127(.a(new_n65_), .b(x30), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n189_), .c(new_n144_), .d(new_n138_), .O(z18));
  nor2   g129(.a(x40), .b(x35), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n91_), .c(new_n161_), .O(new_n193_));
  inv1   g131(.a(x28), .O(new_n194_));
  nand2  g132(.a(new_n140_), .b(new_n194_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n139_), .O(new_n196_));
  nand2  g134(.a(x35), .b(x27), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(x04), .c(new_n143_), .O(new_n198_));
  nand2  g136(.a(x39), .b(x29), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n83_), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(new_n198_), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(x40), .O(new_n202_));
  nand2  g140(.a(x28), .b(new_n143_), .O(new_n203_));
  nand4  g141(.a(new_n203_), .b(new_n202_), .c(new_n196_), .d(new_n193_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n80_), .c(new_n138_), .O(new_n205_));
  nand2  g143(.a(new_n205_), .b(new_n64_), .O(z19));
  aoi21  g144(.a(x28), .b(new_n100_), .c(new_n83_), .O(new_n207_));
  nand2  g145(.a(new_n199_), .b(x40), .O(new_n208_));
  aoi21  g146(.a(new_n208_), .b(new_n96_), .c(new_n207_), .O(new_n209_));
  nand2  g147(.a(new_n64_), .b(new_n100_), .O(new_n210_));
  nand2  g148(.a(x40), .b(new_n83_), .O(new_n211_));
  aoi21  g149(.a(new_n211_), .b(new_n210_), .c(x08), .O(new_n212_));
  oai21  g150(.a(new_n212_), .b(new_n209_), .c(new_n80_), .O(new_n213_));
  nor2   g151(.a(new_n213_), .b(x09), .O(z20));
endmodule


