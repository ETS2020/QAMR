// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_;
  inv1   g000(.a(x32), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x30), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g009(.a(new_n71_), .b(x40), .c(x39), .d(new_n68_), .O(new_n72_));
  inv1   g010(.a(x37), .O(new_n73_));
  nand2  g011(.a(x35), .b(x28), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n72_), .c(new_n67_), .O(new_n77_));
  and2   g015(.a(x40), .b(x39), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(x10), .c(new_n68_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n77_), .c(new_n66_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g020(.a(x28), .O(new_n83_));
  aoi21  g021(.a(x35), .b(new_n83_), .c(x36), .O(new_n84_));
  nor3   g022(.a(new_n84_), .b(x30), .c(new_n67_), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(x32), .c(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(new_n69_), .b(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n78_), .d(new_n64_), .O(z02));
  nand2  g031(.a(new_n74_), .b(x27), .O(new_n94_));
  aoi21  g032(.a(new_n73_), .b(new_n67_), .c(new_n65_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(z04));
  nor2   g036(.a(new_n83_), .b(new_n67_), .O(new_n99_));
  inv1   g037(.a(new_n99_), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n64_), .c(new_n73_), .O(z05));
  oai21  g039(.a(new_n99_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n66_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n64_), .O(z07));
  nand2  g051(.a(x40), .b(x39), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n64_), .O(z08));
  inv1   g053(.a(x11), .O(new_n116_));
  nand4  g054(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  nand3  g056(.a(new_n114_), .b(new_n64_), .c(x07), .O(new_n119_));
  inv1   g057(.a(x30), .O(new_n120_));
  nand3  g058(.a(new_n78_), .b(new_n120_), .c(x05), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n119_), .c(new_n84_), .O(new_n122_));
  nor2   g060(.a(new_n65_), .b(new_n73_), .O(new_n123_));
  aoi22  g061(.a(new_n123_), .b(x06), .c(new_n122_), .d(new_n68_), .O(new_n124_));
  nand4  g062(.a(new_n78_), .b(x32), .c(x05), .d(new_n68_), .O(new_n125_));
  oai21  g063(.a(new_n124_), .b(new_n67_), .c(new_n125_), .O(z10));
  inv1   g064(.a(x09), .O(new_n127_));
  nand2  g065(.a(new_n78_), .b(x29), .O(new_n128_));
  oai21  g066(.a(new_n67_), .b(new_n68_), .c(new_n90_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(new_n90_), .b(x27), .c(new_n68_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n120_), .c(new_n127_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(z11));
  inv1   g074(.a(x10), .O(new_n137_));
  nor3   g075(.a(x37), .b(x36), .c(x35), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n67_), .c(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n75_), .b(x27), .O(new_n140_));
  oai21  g078(.a(new_n114_), .b(x04), .c(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n139_), .c(new_n64_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  inv1   g081(.a(x40), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x28), .c(new_n144_), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(x39), .c(new_n143_), .d(new_n68_), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand4  g087(.a(new_n75_), .b(x20), .c(new_n149_), .d(new_n148_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n147_), .c(x30), .O(new_n151_));
  nand4  g089(.a(new_n75_), .b(x32), .c(x20), .d(new_n149_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(x18), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n151_), .c(x27), .O(new_n154_));
  nand4  g092(.a(new_n78_), .b(x32), .c(new_n143_), .d(new_n68_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z13));
  nand3  g094(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n157_));
  nand3  g095(.a(new_n78_), .b(new_n143_), .c(new_n68_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n64_), .O(new_n159_));
  nand4  g097(.a(new_n145_), .b(new_n63_), .c(new_n120_), .d(x28), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand3  g100(.a(new_n73_), .b(new_n69_), .c(x28), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x27), .c(x32), .O(new_n164_));
  nand3  g102(.a(new_n158_), .b(new_n73_), .c(new_n83_), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n164_), .c(new_n120_), .O(new_n167_));
  nand3  g105(.a(new_n158_), .b(new_n140_), .c(x32), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n167_), .c(new_n162_), .O(z14));
  inv1   g107(.a(x12), .O(new_n170_));
  nor2   g108(.a(new_n117_), .b(new_n170_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(x22), .c(x01), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n64_), .O(z16));
  inv1   g112(.a(x01), .O(new_n175_));
  inv1   g113(.a(x24), .O(new_n176_));
  nand4  g114(.a(new_n64_), .b(new_n176_), .c(x23), .d(x22), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z17));
  nand2  g116(.a(new_n90_), .b(new_n67_), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n128_), .c(new_n131_), .O(new_n180_));
  nand2  g118(.a(new_n132_), .b(new_n127_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n180_), .c(new_n120_), .O(new_n182_));
  oai21  g120(.a(new_n63_), .b(new_n120_), .c(new_n182_), .O(z18));
  inv1   g121(.a(new_n128_), .O(new_n184_));
  oai21  g122(.a(new_n69_), .b(new_n67_), .c(new_n131_), .O(new_n185_));
  aoi21  g123(.a(x28), .b(new_n67_), .c(new_n69_), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n184_), .c(new_n185_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n120_), .c(new_n127_), .O(new_n188_));
  inv1   g126(.a(new_n188_), .O(z20));
  inv1   g127(.a(new_n135_), .O(z19));
endmodule


