// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_;
  nor2   g000(.a(x33), .b(x10), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x33), .O(new_n72_));
  oai21  g010(.a(new_n72_), .b(new_n66_), .c(new_n71_), .O(new_n73_));
  nand4  g011(.a(new_n73_), .b(x40), .c(x39), .d(new_n64_), .O(new_n74_));
  oai22  g012(.a(new_n74_), .b(x04), .c(new_n70_), .d(new_n63_), .O(z00));
  inv1   g013(.a(new_n63_), .O(new_n76_));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n78_), .c(new_n77_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n76_), .c(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  nand2  g027(.a(new_n66_), .b(new_n89_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(x35), .c(new_n80_), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  aoi21  g030(.a(new_n91_), .b(x04), .c(new_n92_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n88_), .c(new_n63_), .O(z02));
  nand2  g032(.a(new_n68_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n67_), .b(new_n66_), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n95_), .c(new_n63_), .O(z03));
  aoi21  g036(.a(new_n67_), .b(new_n66_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n63_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n67_), .c(new_n63_), .O(z05));
  nand3  g040(.a(new_n101_), .b(new_n76_), .c(new_n67_), .O(new_n103_));
  inv1   g041(.a(new_n103_), .O(z06));
  inv1   g042(.a(x03), .O(new_n105_));
  inv1   g043(.a(x31), .O(new_n106_));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n64_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand4  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .d(new_n106_), .O(new_n111_));
  inv1   g049(.a(x14), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n109_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n111_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x10), .O(new_n117_));
  nand3  g055(.a(new_n114_), .b(x33), .c(new_n112_), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n117_), .c(new_n105_), .O(z07));
  nand2  g057(.a(new_n92_), .b(new_n76_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n76_), .O(z09));
  inv1   g060(.a(x04), .O(new_n123_));
  nand2  g061(.a(new_n92_), .b(x07), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x05), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n82_), .c(x27), .O(new_n127_));
  nand2  g065(.a(new_n78_), .b(new_n77_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x40), .c(x39), .d(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n123_), .O(new_n131_));
  nand3  g069(.a(x37), .b(x27), .c(x06), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n76_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  inv1   g072(.a(new_n92_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x29), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n80_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand4  g077(.a(x35), .b(new_n80_), .c(x27), .d(new_n123_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n89_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n77_), .c(new_n134_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n76_), .O(z11));
  oai21  g082(.a(new_n92_), .b(x04), .c(new_n68_), .O(new_n145_));
  inv1   g083(.a(x35), .O(new_n146_));
  nand3  g084(.a(new_n79_), .b(new_n146_), .c(new_n71_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n145_), .c(x37), .O(new_n148_));
  nor2   g086(.a(x27), .b(x10), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(x33), .O(new_n150_));
  inv1   g088(.a(new_n69_), .O(new_n151_));
  nand2  g089(.a(new_n135_), .b(new_n123_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n151_), .c(x10), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  inv1   g093(.a(x40), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n156_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n155_), .d(new_n123_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand2  g099(.a(new_n68_), .b(new_n67_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x27), .O(new_n165_));
  nand3  g103(.a(new_n128_), .b(x40), .c(x39), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x13), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n123_), .c(new_n63_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(z13));
  nand3  g107(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n170_));
  nand4  g108(.a(new_n157_), .b(new_n78_), .c(new_n77_), .d(x28), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  nand3  g110(.a(new_n135_), .b(new_n155_), .c(new_n123_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n170_), .O(new_n174_));
  nand3  g112(.a(new_n67_), .b(new_n146_), .c(x28), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(x27), .c(x32), .O(new_n176_));
  aoi22  g114(.a(new_n176_), .b(new_n77_), .c(new_n173_), .d(new_n151_), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n174_), .c(new_n63_), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n76_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n76_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x01), .O(new_n184_));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n76_), .b(new_n185_), .c(x23), .d(x22), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z17));
  oai21  g125(.a(new_n81_), .b(x27), .c(new_n136_), .O(new_n188_));
  nand3  g126(.a(new_n140_), .b(new_n77_), .c(new_n134_), .O(new_n189_));
  aoi21  g127(.a(new_n188_), .b(x08), .c(new_n189_), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n63_), .O(z18));
  nand4  g129(.a(new_n142_), .b(new_n76_), .c(new_n77_), .d(new_n134_), .O(new_n192_));
  inv1   g130(.a(new_n192_), .O(z19));
  inv1   g131(.a(new_n136_), .O(new_n194_));
  oai21  g132(.a(new_n146_), .b(new_n66_), .c(new_n89_), .O(new_n195_));
  aoi21  g133(.a(x28), .b(new_n66_), .c(new_n146_), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n194_), .c(new_n195_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n77_), .c(new_n134_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n76_), .O(z20));
endmodule


