// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n201_,
    new_n202_, new_n203_, new_n204_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x19), .O(new_n77_));
  nand2  g015(.a(x40), .b(new_n77_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(x16), .O(z00));
  inv1   g017(.a(new_n78_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n80_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  inv1   g028(.a(x29), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n84_), .c(x04), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n73_), .d(x19), .O(z02));
  nand2  g033(.a(x35), .b(x28), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n78_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n98_), .b(new_n64_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n97_), .c(new_n80_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n78_), .c(new_n98_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n78_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  inv1   g054(.a(x39), .O(new_n117_));
  inv1   g055(.a(x40), .O(new_n118_));
  aoi21  g056(.a(new_n117_), .b(x19), .c(new_n118_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  nand2  g060(.a(new_n72_), .b(x07), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n85_), .c(x27), .O(new_n126_));
  nand2  g064(.a(new_n82_), .b(new_n81_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(x40), .c(x39), .d(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n65_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n78_), .O(z10));
  nand2  g070(.a(new_n78_), .b(new_n64_), .O(new_n133_));
  nand3  g071(.a(x35), .b(new_n83_), .c(new_n65_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x19), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n133_), .c(x08), .O(new_n136_));
  nor2   g074(.a(new_n66_), .b(x28), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x19), .c(new_n118_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n137_), .c(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n136_), .c(new_n81_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(x09), .O(z11));
  nand3  g081(.a(new_n98_), .b(new_n67_), .c(new_n66_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  nor2   g083(.a(new_n70_), .b(new_n64_), .O(new_n146_));
  aoi21  g084(.a(x39), .b(new_n65_), .c(new_n146_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n145_), .c(x19), .O(new_n148_));
  oai21  g086(.a(new_n70_), .b(new_n64_), .c(new_n118_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n118_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(x19), .d(new_n151_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  aoi21  g093(.a(new_n96_), .b(new_n98_), .c(x40), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x20), .c(new_n77_), .d(new_n155_), .O(new_n157_));
  oai21  g095(.a(new_n154_), .b(x04), .c(new_n157_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nand2  g097(.a(new_n127_), .b(x40), .O(new_n160_));
  nor2   g098(.a(new_n160_), .b(new_n117_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x19), .c(new_n151_), .d(new_n65_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n159_), .O(z13));
  oai21  g101(.a(new_n127_), .b(new_n77_), .c(x40), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n64_), .O(new_n165_));
  nand4  g103(.a(new_n152_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n73_), .c(new_n151_), .d(new_n65_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x19), .O(new_n168_));
  nand2  g106(.a(x20), .b(new_n155_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n70_), .c(new_n118_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n168_), .c(new_n165_), .O(z14));
  nand4  g109(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n78_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(x22), .c(x01), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n78_), .O(z16));
  inv1   g114(.a(x01), .O(new_n177_));
  inv1   g115(.a(x24), .O(new_n178_));
  nand4  g116(.a(new_n78_), .b(new_n178_), .c(x23), .d(x22), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n177_), .O(z17));
  inv1   g118(.a(x09), .O(new_n181_));
  oai22  g119(.a(new_n84_), .b(x27), .c(new_n72_), .d(new_n91_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x08), .O(new_n183_));
  nor2   g121(.a(new_n64_), .b(x04), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n137_), .c(x30), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n183_), .c(new_n78_), .d(new_n181_), .O(z18));
  nand2  g124(.a(x35), .b(x27), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(x19), .O(new_n188_));
  nand2  g126(.a(x40), .b(x04), .O(new_n189_));
  nand2  g127(.a(new_n118_), .b(new_n64_), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n189_), .c(new_n188_), .d(new_n83_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n90_), .O(new_n192_));
  nand2  g130(.a(new_n66_), .b(x19), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n140_), .c(new_n83_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n138_), .O(new_n195_));
  nand2  g133(.a(new_n140_), .b(new_n137_), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n118_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n195_), .c(new_n192_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n81_), .c(new_n181_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n78_), .O(z19));
  aoi21  g138(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n201_));
  nor2   g139(.a(new_n201_), .b(new_n139_), .O(new_n202_));
  aoi21  g140(.a(new_n190_), .b(new_n188_), .c(x08), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n202_), .c(new_n81_), .O(new_n204_));
  nor2   g142(.a(new_n204_), .b(x09), .O(z20));
endmodule


