// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n202_, new_n203_, new_n204_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  nand2  g004(.a(x40), .b(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x39), .c(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(z08));
  nand3  g011(.a(z08), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  nor2   g014(.a(x40), .b(new_n66_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(x16), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  inv1   g021(.a(new_n83_), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x36), .c(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n81_), .c(new_n80_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n83_), .c(x04), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(z08), .O(z02));
  inv1   g031(.a(x40), .O(new_n94_));
  oai21  g032(.a(x35), .b(x27), .c(new_n94_), .O(new_n95_));
  nand2  g033(.a(x35), .b(x28), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n95_), .d(x21), .O(z03));
  nand2  g038(.a(new_n78_), .b(x21), .O(new_n101_));
  oai21  g039(.a(new_n94_), .b(x28), .c(x35), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(x27), .O(new_n103_));
  oai21  g041(.a(new_n94_), .b(x27), .c(x35), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n98_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n101_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n98_), .c(new_n77_), .O(z05));
  nand3  g046(.a(new_n107_), .b(new_n78_), .c(new_n98_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n63_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n78_), .c(x03), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z07));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  nand2  g062(.a(new_n83_), .b(new_n67_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(x39), .c(x05), .O(new_n126_));
  nand2  g064(.a(new_n72_), .b(x36), .O(new_n127_));
  inv1   g065(.a(x39), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(x35), .c(new_n82_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x07), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n126_), .c(new_n64_), .O(new_n132_));
  inv1   g070(.a(x05), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor3   g072(.a(new_n134_), .b(new_n128_), .c(new_n133_), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n132_), .c(new_n65_), .O(new_n136_));
  nor2   g074(.a(new_n98_), .b(new_n64_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(x06), .c(new_n77_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(z10));
  inv1   g077(.a(x09), .O(new_n140_));
  nand2  g078(.a(x39), .b(x29), .O(new_n141_));
  aoi21  g079(.a(x27), .b(x04), .c(x28), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n94_), .c(x35), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  oai21  g082(.a(new_n64_), .b(x08), .c(x40), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n66_), .O(new_n146_));
  inv1   g084(.a(x08), .O(new_n147_));
  nand3  g085(.a(new_n82_), .b(x27), .c(new_n65_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(x40), .c(new_n147_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n146_), .c(new_n144_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n80_), .c(new_n140_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z11));
  inv1   g090(.a(x10), .O(new_n153_));
  nor3   g091(.a(x37), .b(x36), .c(x35), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n64_), .c(new_n153_), .O(new_n155_));
  oai22  g093(.a(new_n70_), .b(new_n64_), .c(new_n128_), .d(x04), .O(new_n156_));
  nand3  g094(.a(x37), .b(new_n66_), .c(x27), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n94_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n156_), .c(new_n155_), .O(z12));
  inv1   g097(.a(x13), .O(new_n160_));
  inv1   g098(.a(x36), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n66_), .c(x28), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x39), .c(new_n160_), .d(new_n65_), .O(new_n163_));
  inv1   g101(.a(x18), .O(new_n164_));
  inv1   g102(.a(x19), .O(new_n165_));
  oai21  g103(.a(new_n98_), .b(x28), .c(new_n96_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x20), .c(new_n165_), .d(new_n164_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n163_), .c(new_n64_), .O(new_n168_));
  oai21  g106(.a(x32), .b(x30), .c(x39), .O(new_n169_));
  nor3   g107(.a(new_n169_), .b(x13), .c(x04), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n168_), .c(x40), .O(new_n171_));
  nand3  g109(.a(x20), .b(new_n165_), .c(new_n164_), .O(new_n172_));
  or2    g110(.a(new_n172_), .b(new_n157_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n171_), .O(z13));
  nand3  g112(.a(x39), .b(new_n160_), .c(new_n65_), .O(new_n175_));
  nand4  g113(.a(x27), .b(x20), .c(new_n165_), .d(new_n164_), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n70_), .c(new_n175_), .O(new_n177_));
  nand2  g115(.a(new_n134_), .b(new_n64_), .O(new_n178_));
  nand4  g116(.a(new_n66_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x40), .O(new_n180_));
  nand4  g118(.a(x37), .b(x20), .c(new_n165_), .d(new_n164_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  nand4  g120(.a(new_n172_), .b(new_n161_), .c(new_n81_), .d(new_n80_), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(x28), .c(new_n77_), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n182_), .c(new_n178_), .d(new_n177_), .O(z14));
  inv1   g124(.a(x12), .O(new_n187_));
  nor2   g125(.a(new_n123_), .b(new_n187_), .O(z15));
  inv1   g126(.a(x23), .O(new_n189_));
  nand4  g127(.a(new_n78_), .b(new_n189_), .c(x22), .d(x01), .O(new_n190_));
  inv1   g128(.a(new_n190_), .O(z16));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(x23), .c(x22), .d(x01), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n78_), .O(z17));
  nand2  g132(.a(new_n84_), .b(new_n64_), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n141_), .c(new_n147_), .O(new_n196_));
  nand3  g134(.a(new_n84_), .b(x27), .c(new_n65_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n80_), .c(new_n140_), .O(new_n198_));
  oai21  g136(.a(new_n198_), .b(new_n196_), .c(x40), .O(new_n199_));
  oai21  g137(.a(x30), .b(x09), .c(new_n66_), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(new_n199_), .O(z18));
  oai21  g139(.a(new_n94_), .b(new_n147_), .c(new_n66_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n149_), .c(new_n144_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n80_), .c(new_n140_), .O(new_n204_));
  inv1   g142(.a(new_n204_), .O(z19));
  oai21  g143(.a(new_n66_), .b(new_n64_), .c(new_n147_), .O(new_n206_));
  oai21  g144(.a(new_n82_), .b(x27), .c(x35), .O(new_n207_));
  nand2  g145(.a(new_n207_), .b(new_n141_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n206_), .c(x40), .O(new_n209_));
  nand3  g147(.a(new_n209_), .b(new_n80_), .c(new_n140_), .O(new_n210_));
  nand2  g148(.a(new_n210_), .b(new_n78_), .O(z20));
endmodule


