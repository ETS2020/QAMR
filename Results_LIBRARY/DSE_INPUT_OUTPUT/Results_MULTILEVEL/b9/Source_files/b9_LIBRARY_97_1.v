// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(x14), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  nor4   g006(.a(new_n68_), .b(x15), .c(new_n67_), .d(x04), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x40), .c(x39), .d(new_n71_), .O(new_n75_));
  nand2  g013(.a(x35), .b(x28), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n75_), .c(x37), .O(new_n77_));
  nand2  g015(.a(x37), .b(x14), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n77_), .c(x27), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(x15), .c(new_n70_), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  oai21  g021(.a(new_n72_), .b(x28), .c(new_n73_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n66_), .c(x04), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(new_n72_), .b(x28), .O(new_n91_));
  oai21  g029(.a(x27), .b(x08), .c(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(new_n68_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n90_), .c(new_n65_), .O(z02));
  inv1   g032(.a(x14), .O(new_n95_));
  inv1   g033(.a(x27), .O(new_n96_));
  oai21  g034(.a(x37), .b(new_n96_), .c(new_n95_), .O(new_n97_));
  nand2  g035(.a(new_n76_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n64_), .b(new_n96_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x21), .O(z03));
  nand2  g038(.a(new_n66_), .b(x21), .O(new_n101_));
  oai21  g039(.a(new_n96_), .b(new_n95_), .c(x37), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n76_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(new_n99_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g043(.a(z06), .O(z05));
  inv1   g044(.a(x03), .O(new_n107_));
  inv1   g045(.a(x31), .O(new_n108_));
  inv1   g046(.a(x33), .O(new_n109_));
  inv1   g047(.a(x15), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  aoi22  g050(.a(new_n112_), .b(x38), .c(x17), .d(new_n110_), .O(new_n113_));
  nand4  g051(.a(new_n113_), .b(new_n109_), .c(new_n108_), .d(x14), .O(new_n114_));
  inv1   g052(.a(x00), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n111_), .c(new_n115_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n64_), .c(new_n95_), .O(new_n117_));
  aoi21  g055(.a(new_n117_), .b(new_n114_), .c(new_n107_), .O(z07));
  inv1   g056(.a(x39), .O(new_n119_));
  inv1   g057(.a(x40), .O(new_n120_));
  nor3   g058(.a(new_n65_), .b(new_n120_), .c(new_n119_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n66_), .O(z09));
  inv1   g061(.a(x05), .O(new_n124_));
  nand2  g062(.a(new_n68_), .b(x07), .O(new_n125_));
  oai21  g063(.a(new_n68_), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n84_), .c(x27), .O(new_n127_));
  nand2  g065(.a(new_n83_), .b(new_n82_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x40), .c(x39), .d(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n66_), .c(new_n71_), .O(new_n131_));
  nand4  g069(.a(x37), .b(x27), .c(x14), .d(x06), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  inv1   g072(.a(new_n68_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x29), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n91_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  inv1   g077(.a(x08), .O(new_n140_));
  nand3  g078(.a(new_n91_), .b(x27), .c(new_n71_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand4  g081(.a(new_n143_), .b(new_n66_), .c(new_n82_), .d(new_n134_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(z11));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n96_), .c(new_n67_), .O(new_n147_));
  nand2  g085(.a(new_n76_), .b(new_n64_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  oai21  g087(.a(new_n68_), .b(x04), .c(new_n149_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n147_), .c(new_n66_), .O(z12));
  inv1   g089(.a(x18), .O(new_n152_));
  inv1   g090(.a(x19), .O(new_n153_));
  nand3  g091(.a(new_n64_), .b(x35), .c(x28), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n78_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n153_), .d(new_n152_), .O(new_n156_));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi22  g096(.a(new_n158_), .b(x28), .c(x37), .d(new_n95_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x40), .c(x39), .d(new_n157_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(x04), .c(new_n156_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x27), .O(new_n162_));
  nand4  g100(.a(new_n128_), .b(new_n66_), .c(x40), .d(x39), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n157_), .c(new_n71_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n162_), .O(z13));
  nand3  g104(.a(new_n135_), .b(new_n157_), .c(new_n71_), .O(new_n167_));
  nand4  g105(.a(x27), .b(x20), .c(new_n153_), .d(new_n152_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n65_), .c(new_n148_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n167_), .O(new_n171_));
  nand3  g109(.a(x20), .b(new_n153_), .c(new_n152_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n73_), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n66_), .O(new_n175_));
  nand2  g113(.a(new_n172_), .b(x14), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x37), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n72_), .c(x28), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n175_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n83_), .c(new_n82_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n171_), .O(z14));
  nand4  g119(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n66_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand4  g122(.a(new_n66_), .b(new_n184_), .c(x22), .d(x01), .O(new_n185_));
  inv1   g123(.a(new_n185_), .O(z16));
  inv1   g124(.a(x01), .O(new_n187_));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n66_), .b(new_n188_), .c(x23), .d(x22), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n187_), .O(z17));
  nand2  g128(.a(new_n91_), .b(new_n96_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n136_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(x08), .O(new_n193_));
  nor2   g131(.a(new_n65_), .b(x30), .O(new_n194_));
  nand4  g132(.a(new_n194_), .b(new_n193_), .c(new_n141_), .d(new_n134_), .O(z18));
  inv1   g133(.a(new_n136_), .O(new_n196_));
  oai21  g134(.a(new_n72_), .b(new_n96_), .c(new_n140_), .O(new_n197_));
  aoi21  g135(.a(x28), .b(new_n96_), .c(new_n72_), .O(new_n198_));
  oai21  g136(.a(new_n198_), .b(new_n196_), .c(new_n197_), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(new_n82_), .c(new_n134_), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(new_n66_), .O(z20));
  inv1   g139(.a(new_n144_), .O(z19));
endmodule


