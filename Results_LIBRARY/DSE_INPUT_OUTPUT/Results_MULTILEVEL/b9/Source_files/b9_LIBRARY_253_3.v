// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n212_, new_n213_;
  nand2  g000(.a(x40), .b(x38), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x38), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n63_), .c(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n82_), .c(x04), .O(new_n91_));
  inv1   g029(.a(x39), .O(new_n92_));
  nor2   g030(.a(new_n74_), .b(new_n92_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n91_), .c(new_n89_), .d(new_n72_), .O(z02));
  nand2  g032(.a(new_n69_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n68_), .b(new_n67_), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n95_), .c(new_n64_), .O(z03));
  aoi21  g036(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n64_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n68_), .c(new_n64_), .O(z05));
  nand3  g040(.a(new_n101_), .b(new_n63_), .c(new_n68_), .O(new_n103_));
  inv1   g041(.a(new_n103_), .O(z06));
  inv1   g042(.a(x03), .O(new_n105_));
  inv1   g043(.a(x31), .O(new_n106_));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n65_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(new_n74_), .b(new_n109_), .c(x00), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x38), .O(new_n111_));
  nand4  g049(.a(new_n111_), .b(new_n108_), .c(new_n107_), .d(new_n106_), .O(new_n112_));
  inv1   g050(.a(x14), .O(new_n113_));
  nor2   g051(.a(x25), .b(x00), .O(new_n114_));
  oai21  g052(.a(new_n114_), .b(x40), .c(x38), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(new_n112_), .c(new_n105_), .O(z07));
  nand3  g055(.a(x40), .b(x39), .c(new_n72_), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n63_), .O(z09));
  inv1   g059(.a(x04), .O(new_n122_));
  aoi21  g060(.a(x35), .b(new_n81_), .c(x36), .O(new_n123_));
  oai21  g061(.a(x39), .b(x38), .c(x40), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x07), .O(new_n125_));
  nand4  g063(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(new_n127_));
  nand3  g065(.a(new_n63_), .b(x37), .c(x06), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n127_), .b(new_n122_), .c(new_n129_), .O(new_n130_));
  nand2  g068(.a(new_n80_), .b(new_n79_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x40), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n92_), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(new_n72_), .c(x05), .d(new_n122_), .O(new_n134_));
  oai21  g072(.a(new_n130_), .b(new_n67_), .c(new_n134_), .O(z10));
  inv1   g073(.a(x08), .O(new_n136_));
  inv1   g074(.a(x09), .O(new_n137_));
  nand4  g075(.a(new_n79_), .b(new_n137_), .c(new_n136_), .d(x04), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n72_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x40), .O(new_n140_));
  nand2  g078(.a(new_n93_), .b(x29), .O(new_n141_));
  nand2  g079(.a(x27), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(x35), .c(new_n81_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  oai21  g082(.a(new_n82_), .b(new_n67_), .c(new_n136_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n79_), .c(new_n137_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n140_), .O(z11));
  inv1   g086(.a(x35), .O(new_n149_));
  inv1   g087(.a(x36), .O(new_n150_));
  nand3  g088(.a(new_n68_), .b(new_n150_), .c(new_n149_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x27), .c(x10), .O(new_n152_));
  aoi21  g090(.a(x39), .b(new_n122_), .c(new_n70_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n152_), .c(new_n72_), .O(new_n154_));
  nand2  g092(.a(new_n69_), .b(new_n68_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x27), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n74_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n154_), .O(z12));
  inv1   g096(.a(x13), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x28), .c(new_n74_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n159_), .d(new_n122_), .O(new_n162_));
  inv1   g100(.a(x18), .O(new_n163_));
  inv1   g101(.a(x19), .O(new_n164_));
  nand4  g102(.a(new_n155_), .b(x20), .c(new_n164_), .d(new_n163_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x27), .O(new_n167_));
  nand4  g105(.a(new_n131_), .b(x39), .c(new_n159_), .d(new_n122_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n72_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x40), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n167_), .O(z13));
  nand3  g109(.a(x20), .b(new_n164_), .c(new_n163_), .O(new_n172_));
  nand4  g110(.a(new_n160_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n173_));
  nand3  g111(.a(x39), .b(new_n159_), .c(new_n122_), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n173_), .c(x38), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n74_), .c(new_n172_), .O(new_n177_));
  nand3  g115(.a(new_n68_), .b(new_n149_), .c(x28), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(x27), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n80_), .c(new_n79_), .O(new_n180_));
  nand2  g118(.a(new_n174_), .b(new_n156_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n72_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n177_), .c(new_n157_), .O(z14));
  inv1   g122(.a(x12), .O(new_n185_));
  nand4  g123(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n185_), .O(z15));
  inv1   g125(.a(x23), .O(new_n188_));
  nand4  g126(.a(new_n63_), .b(new_n188_), .c(x22), .d(x01), .O(new_n189_));
  inv1   g127(.a(new_n189_), .O(z16));
  inv1   g128(.a(x01), .O(new_n191_));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n63_), .b(new_n192_), .c(x23), .d(x22), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(new_n191_), .O(z17));
  oai21  g132(.a(new_n82_), .b(x27), .c(new_n141_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(x08), .O(new_n196_));
  nor2   g134(.a(new_n67_), .b(x04), .O(new_n197_));
  aoi21  g135(.a(new_n197_), .b(new_n83_), .c(x09), .O(new_n198_));
  nand4  g136(.a(new_n198_), .b(new_n196_), .c(new_n63_), .d(new_n79_), .O(z18));
  aoi21  g137(.a(x40), .b(x38), .c(x27), .O(new_n200_));
  nand3  g138(.a(x35), .b(new_n81_), .c(new_n122_), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n72_), .c(new_n200_), .O(new_n202_));
  aoi21  g140(.a(x39), .b(x29), .c(x38), .O(new_n203_));
  oai21  g141(.a(new_n203_), .b(new_n74_), .c(new_n143_), .O(new_n204_));
  oai21  g142(.a(new_n202_), .b(x08), .c(new_n204_), .O(new_n205_));
  nand3  g143(.a(new_n205_), .b(new_n79_), .c(new_n137_), .O(new_n206_));
  inv1   g144(.a(new_n206_), .O(z19));
  nor2   g145(.a(new_n203_), .b(new_n74_), .O(new_n208_));
  aoi21  g146(.a(x28), .b(new_n67_), .c(new_n149_), .O(new_n209_));
  nor2   g147(.a(x38), .b(x35), .O(new_n210_));
  oai21  g148(.a(new_n210_), .b(new_n200_), .c(new_n136_), .O(new_n211_));
  oai21  g149(.a(new_n209_), .b(new_n208_), .c(new_n211_), .O(new_n212_));
  nand3  g150(.a(new_n212_), .b(new_n79_), .c(new_n137_), .O(new_n213_));
  inv1   g151(.a(new_n213_), .O(z20));
endmodule


