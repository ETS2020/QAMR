// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  nand2  g014(.a(x30), .b(new_n65_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n76_), .c(x16), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n82_), .c(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n89_), .c(new_n77_), .d(new_n73_), .O(z02));
  oai21  g030(.a(new_n66_), .b(new_n81_), .c(x27), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n64_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n93_), .c(new_n77_), .d(x21), .O(z03));
  inv1   g034(.a(new_n77_), .O(new_n97_));
  aoi21  g035(.a(new_n94_), .b(new_n64_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n93_), .c(new_n97_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n94_), .c(new_n97_), .O(z05));
  nand3  g039(.a(new_n100_), .b(new_n77_), .c(new_n94_), .O(new_n102_));
  inv1   g040(.a(new_n102_), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n63_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(new_n111_));
  oai22  g049(.a(new_n111_), .b(x14), .c(new_n108_), .d(x31), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n77_), .O(z07));
  nand2  g052(.a(new_n77_), .b(new_n72_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n77_), .O(z09));
  nand2  g055(.a(new_n72_), .b(x07), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n83_), .c(new_n65_), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(x30), .O(new_n123_));
  nand3  g061(.a(x37), .b(x06), .c(x04), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n123_), .c(x27), .O(new_n126_));
  nand2  g064(.a(new_n73_), .b(x32), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(new_n79_), .c(x05), .d(new_n65_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n126_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  nand2  g069(.a(new_n73_), .b(x29), .O(new_n132_));
  nor2   g070(.a(new_n64_), .b(new_n65_), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n82_), .c(new_n132_), .O(new_n134_));
  inv1   g072(.a(x08), .O(new_n135_));
  inv1   g073(.a(new_n82_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x27), .c(new_n65_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n79_), .c(new_n131_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z11));
  nand3  g079(.a(new_n94_), .b(new_n67_), .c(new_n66_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  nor2   g081(.a(new_n70_), .b(new_n64_), .O(new_n144_));
  nor2   g082(.a(new_n144_), .b(new_n73_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n143_), .c(new_n79_), .O(new_n146_));
  oai21  g084(.a(new_n70_), .b(new_n64_), .c(x04), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(z12));
  inv1   g086(.a(x19), .O(new_n149_));
  inv1   g087(.a(new_n70_), .O(new_n150_));
  nand4  g088(.a(new_n77_), .b(new_n150_), .c(x20), .d(new_n149_), .O(new_n151_));
  inv1   g089(.a(x13), .O(new_n152_));
  inv1   g090(.a(x40), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n153_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n79_), .d(new_n152_), .O(new_n156_));
  oai22  g094(.a(new_n156_), .b(x04), .c(new_n151_), .d(x18), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand4  g096(.a(new_n128_), .b(new_n79_), .c(new_n152_), .d(new_n65_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  nand2  g098(.a(x20), .b(new_n149_), .O(new_n161_));
  nand3  g099(.a(new_n154_), .b(new_n80_), .c(x28), .O(new_n162_));
  nor2   g100(.a(new_n72_), .b(x13), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n162_), .c(x30), .O(new_n164_));
  oai22  g102(.a(new_n164_), .b(x04), .c(new_n161_), .d(x18), .O(new_n165_));
  nand3  g103(.a(new_n94_), .b(new_n66_), .c(x28), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(x27), .c(x32), .O(new_n167_));
  nor2   g105(.a(new_n163_), .b(new_n144_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n167_), .c(new_n79_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n165_), .c(new_n147_), .O(z14));
  inv1   g108(.a(x12), .O(new_n171_));
  nand4  g109(.a(new_n77_), .b(x34), .c(x27), .d(x26), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(new_n171_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(x22), .c(x01), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n77_), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(x23), .c(x22), .d(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n77_), .O(z17));
  oai21  g117(.a(new_n82_), .b(x27), .c(new_n132_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x08), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(new_n137_), .c(new_n79_), .d(new_n131_), .O(z18));
  inv1   g120(.a(new_n132_), .O(new_n183_));
  oai21  g121(.a(new_n66_), .b(new_n64_), .c(new_n135_), .O(new_n184_));
  aoi21  g122(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n185_));
  oai21  g123(.a(new_n185_), .b(new_n183_), .c(new_n184_), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n79_), .c(new_n131_), .O(new_n187_));
  inv1   g125(.a(new_n187_), .O(z20));
  inv1   g126(.a(new_n140_), .O(z19));
endmodule


