// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x16), .O(new_n77_));
  inv1   g015(.a(x40), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x32), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n76_), .O(z00));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x27), .c(x30), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x40), .c(new_n82_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n84_), .c(x04), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n89_), .c(x32), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x40), .O(z02));
  oai21  g032(.a(new_n66_), .b(new_n83_), .c(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n64_), .c(new_n79_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n95_), .c(new_n99_), .O(z04));
  inv1   g038(.a(new_n79_), .O(new_n101_));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n101_), .c(new_n96_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n63_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n107_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  oai21  g051(.a(new_n109_), .b(x31), .c(new_n113_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n101_), .O(z07));
  nand3  g054(.a(x40), .b(x39), .c(x32), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n101_), .O(z09));
  nand3  g058(.a(new_n101_), .b(x37), .c(x06), .O(new_n121_));
  oai21  g059(.a(x39), .b(new_n82_), .c(x40), .O(new_n122_));
  nand4  g060(.a(new_n122_), .b(new_n85_), .c(x07), .d(new_n65_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x27), .O(new_n125_));
  nand4  g063(.a(new_n73_), .b(x32), .c(x05), .d(new_n65_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  inv1   g066(.a(x09), .O(new_n129_));
  inv1   g067(.a(x30), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(new_n129_), .c(new_n128_), .d(x04), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x32), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x40), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  nor2   g072(.a(new_n64_), .b(new_n65_), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n84_), .c(new_n134_), .O(new_n136_));
  oai21  g074(.a(new_n84_), .b(new_n64_), .c(new_n128_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n130_), .c(new_n129_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n133_), .O(z11));
  inv1   g078(.a(x10), .O(new_n141_));
  nor3   g079(.a(x37), .b(x36), .c(x35), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n64_), .c(new_n141_), .O(new_n143_));
  oai22  g081(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n143_), .c(new_n101_), .O(z12));
  nand2  g083(.a(new_n73_), .b(x32), .O(new_n146_));
  inv1   g084(.a(x19), .O(new_n147_));
  nor2   g085(.a(new_n79_), .b(new_n70_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x27), .c(x20), .d(new_n147_), .O(new_n149_));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n65_), .O(new_n151_));
  oai22  g089(.a(new_n151_), .b(new_n146_), .c(new_n149_), .d(x18), .O(z13));
  inv1   g090(.a(x18), .O(new_n153_));
  nand4  g091(.a(x27), .b(x20), .c(new_n147_), .d(new_n153_), .O(new_n154_));
  oai22  g092(.a(new_n154_), .b(new_n70_), .c(new_n151_), .d(new_n72_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n101_), .O(z14));
  nand4  g094(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n101_), .O(z15));
  inv1   g096(.a(x23), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(x22), .c(x01), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n101_), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x23), .c(x22), .d(x01), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n101_), .O(z17));
  oai21  g102(.a(new_n84_), .b(x27), .c(new_n134_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x08), .O(new_n166_));
  nand4  g104(.a(x35), .b(new_n83_), .c(x27), .d(new_n65_), .O(new_n167_));
  nor2   g105(.a(new_n79_), .b(x30), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n167_), .c(new_n166_), .d(new_n129_), .O(z18));
  nand2  g107(.a(new_n167_), .b(new_n128_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n136_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n130_), .c(new_n129_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n101_), .O(z19));
  inv1   g111(.a(new_n134_), .O(new_n174_));
  oai21  g112(.a(new_n66_), .b(new_n64_), .c(new_n128_), .O(new_n175_));
  aoi21  g113(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n174_), .c(new_n175_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n130_), .c(new_n129_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n101_), .O(z20));
endmodule


