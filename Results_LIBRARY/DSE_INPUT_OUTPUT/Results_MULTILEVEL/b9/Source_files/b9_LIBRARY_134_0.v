// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n147_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x16), .O(new_n77_));
  nor2   g015(.a(x30), .b(x04), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n76_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n67_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x27), .c(x32), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n81_), .c(x04), .O(new_n86_));
  oai21  g024(.a(new_n81_), .b(x04), .c(new_n86_), .O(z01));
  inv1   g025(.a(new_n78_), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n72_), .c(new_n88_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n83_), .c(x04), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n90_), .O(z02));
  oai21  g031(.a(new_n66_), .b(new_n82_), .c(x27), .O(new_n94_));
  inv1   g032(.a(x21), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n64_), .c(new_n95_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n78_), .O(z03));
  nand2  g036(.a(new_n96_), .b(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n94_), .c(new_n88_), .d(new_n95_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n78_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n63_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n106_), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  oai21  g050(.a(new_n108_), .b(x31), .c(new_n112_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n88_), .O(z07));
  nand2  g053(.a(new_n88_), .b(new_n72_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n88_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  nand4  g057(.a(new_n84_), .b(new_n72_), .c(x27), .d(x07), .O(new_n120_));
  aoi21  g058(.a(new_n73_), .b(x05), .c(new_n81_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n65_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(z10));
  inv1   g063(.a(x09), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  oai21  g065(.a(new_n83_), .b(x27), .c(new_n127_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x08), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(x04), .c(x30), .O(z11));
  inv1   g069(.a(x10), .O(new_n132_));
  nor3   g070(.a(x37), .b(x36), .c(x35), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n64_), .c(new_n132_), .O(new_n134_));
  oai22  g072(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n88_), .O(z12));
  inv1   g074(.a(x19), .O(new_n137_));
  nor2   g075(.a(new_n78_), .b(new_n70_), .O(new_n138_));
  nand4  g076(.a(new_n138_), .b(x27), .c(x20), .d(new_n137_), .O(new_n139_));
  nor2   g077(.a(x13), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n73_), .c(x30), .O(new_n141_));
  oai21  g079(.a(new_n139_), .b(x18), .c(new_n141_), .O(z13));
  oai21  g080(.a(new_n72_), .b(x13), .c(x30), .O(new_n143_));
  nand2  g081(.a(x27), .b(x20), .O(new_n144_));
  nor4   g082(.a(new_n144_), .b(new_n70_), .c(x19), .d(x18), .O(new_n145_));
  aoi21  g083(.a(new_n143_), .b(new_n65_), .c(new_n145_), .O(z14));
  nand4  g084(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n88_), .O(z15));
  inv1   g086(.a(x23), .O(new_n149_));
  nand4  g087(.a(new_n88_), .b(new_n149_), .c(x22), .d(x01), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(z16));
  inv1   g089(.a(x01), .O(new_n152_));
  inv1   g090(.a(x24), .O(new_n153_));
  nand4  g091(.a(new_n88_), .b(new_n153_), .c(x23), .d(x22), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n152_), .O(z17));
  nand4  g093(.a(new_n129_), .b(new_n81_), .c(new_n126_), .d(x04), .O(z18));
  nand4  g094(.a(new_n129_), .b(new_n81_), .c(new_n126_), .d(x04), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z19));
  inv1   g096(.a(new_n127_), .O(new_n159_));
  nor2   g097(.a(new_n66_), .b(new_n64_), .O(new_n160_));
  aoi21  g098(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n161_));
  oai22  g099(.a(new_n161_), .b(new_n159_), .c(new_n160_), .d(x08), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n126_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x04), .c(x30), .O(z20));
endmodule


