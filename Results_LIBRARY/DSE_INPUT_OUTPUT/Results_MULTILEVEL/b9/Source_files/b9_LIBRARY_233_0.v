// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n179_, new_n180_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  nand2  g003(.a(x39), .b(x10), .O(new_n66_));
  nor2   g004(.a(new_n66_), .b(x04), .O(new_n67_));
  aoi21  g005(.a(new_n65_), .b(x27), .c(new_n67_), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  inv1   g007(.a(x40), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n68_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x30), .O(new_n74_));
  inv1   g012(.a(x32), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(new_n74_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x04), .O(z01));
  inv1   g020(.a(x02), .O(new_n83_));
  inv1   g021(.a(x08), .O(new_n84_));
  inv1   g022(.a(x29), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n78_), .c(x04), .O(new_n88_));
  nand2  g026(.a(x40), .b(x39), .O(new_n89_));
  inv1   g027(.a(new_n89_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(z02));
  nand2  g029(.a(x35), .b(x28), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x27), .O(new_n93_));
  inv1   g031(.a(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  inv1   g033(.a(new_n71_), .O(new_n96_));
  aoi21  g034(.a(new_n95_), .b(new_n94_), .c(new_n96_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n93_), .c(new_n99_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n96_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n71_), .c(x03), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z07));
  nand2  g053(.a(new_n89_), .b(new_n71_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n71_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  nand2  g058(.a(x39), .b(x05), .O(new_n121_));
  oai21  g059(.a(x39), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n79_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n75_), .b(new_n74_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(x39), .c(x05), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n123_), .c(x40), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n69_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(z10));
  inv1   g067(.a(x09), .O(new_n130_));
  nand2  g068(.a(x39), .b(x29), .O(new_n131_));
  nor2   g069(.a(new_n94_), .b(new_n69_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n78_), .c(new_n131_), .O(new_n133_));
  oai21  g071(.a(new_n70_), .b(new_n84_), .c(new_n78_), .O(new_n134_));
  oai21  g072(.a(new_n94_), .b(x04), .c(new_n84_), .O(new_n135_));
  nand2  g073(.a(new_n70_), .b(x27), .O(new_n136_));
  nand4  g074(.a(new_n136_), .b(new_n135_), .c(new_n134_), .d(new_n133_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n74_), .c(new_n130_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n71_), .O(z11));
  nor2   g077(.a(new_n64_), .b(new_n94_), .O(new_n140_));
  inv1   g078(.a(x35), .O(new_n141_));
  nand3  g079(.a(new_n95_), .b(new_n76_), .c(new_n141_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  nor2   g081(.a(new_n140_), .b(x39), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x40), .O(new_n145_));
  oai21  g083(.a(new_n140_), .b(new_n69_), .c(new_n145_), .O(z12));
  inv1   g084(.a(x19), .O(new_n147_));
  nand2  g085(.a(new_n92_), .b(new_n95_), .O(new_n148_));
  nand4  g086(.a(new_n71_), .b(new_n148_), .c(x20), .d(new_n147_), .O(new_n149_));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x28), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x40), .c(x39), .d(new_n150_), .O(new_n153_));
  oai22  g091(.a(new_n153_), .b(x04), .c(new_n149_), .d(x18), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x27), .O(new_n155_));
  aoi21  g093(.a(new_n75_), .b(new_n74_), .c(new_n70_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x39), .c(new_n150_), .d(new_n69_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n155_), .O(z13));
  nand3  g096(.a(x39), .b(new_n150_), .c(new_n69_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  nand4  g098(.a(x27), .b(x20), .c(new_n147_), .d(new_n160_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n64_), .c(new_n159_), .O(new_n162_));
  nand3  g100(.a(x20), .b(new_n147_), .c(new_n160_), .O(new_n163_));
  nor2   g101(.a(x37), .b(x35), .O(new_n164_));
  aoi21  g102(.a(new_n163_), .b(new_n151_), .c(new_n164_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n77_), .c(x27), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n75_), .c(new_n74_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n162_), .c(new_n71_), .O(z14));
  inv1   g106(.a(x12), .O(new_n169_));
  nor2   g107(.a(new_n118_), .b(new_n169_), .O(z15));
  inv1   g108(.a(x23), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(x22), .c(x01), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n71_), .O(z16));
  inv1   g111(.a(x24), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(x23), .c(x22), .d(x01), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n71_), .O(z17));
  oai21  g114(.a(new_n78_), .b(new_n94_), .c(x40), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n69_), .O(new_n178_));
  oai22  g116(.a(new_n89_), .b(new_n85_), .c(new_n78_), .d(x27), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x08), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n178_), .c(new_n74_), .d(new_n130_), .O(z18));
  aoi21  g119(.a(x35), .b(new_n77_), .c(new_n70_), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n132_), .c(new_n131_), .O(new_n183_));
  nand2  g121(.a(x40), .b(new_n84_), .O(new_n184_));
  nand3  g122(.a(new_n70_), .b(new_n94_), .c(x04), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n184_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(x28), .O(new_n187_));
  aoi21  g125(.a(new_n70_), .b(x27), .c(new_n69_), .O(new_n188_));
  aoi21  g126(.a(x35), .b(x27), .c(new_n70_), .O(new_n189_));
  oai21  g127(.a(new_n189_), .b(new_n188_), .c(new_n84_), .O(new_n190_));
  oai21  g128(.a(x35), .b(new_n69_), .c(new_n94_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n70_), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(new_n190_), .c(new_n187_), .d(new_n183_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n74_), .c(new_n130_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n71_), .O(z19));
  nand2  g133(.a(x28), .b(new_n94_), .O(new_n196_));
  nand2  g134(.a(new_n131_), .b(x40), .O(new_n197_));
  nand2  g135(.a(new_n70_), .b(x04), .O(new_n198_));
  aoi22  g136(.a(new_n198_), .b(new_n197_), .c(new_n196_), .d(x35), .O(new_n199_));
  nand2  g137(.a(new_n71_), .b(new_n94_), .O(new_n200_));
  nand2  g138(.a(x40), .b(new_n141_), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n200_), .c(x08), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n199_), .c(new_n74_), .O(new_n203_));
  nor2   g141(.a(new_n203_), .b(x09), .O(z20));
endmodule


