// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n210_, new_n211_, new_n212_, new_n213_,
    new_n214_, new_n215_, new_n216_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n74_), .c(new_n63_), .O(new_n80_));
  inv1   g018(.a(x06), .O(new_n81_));
  nand2  g019(.a(new_n76_), .b(new_n81_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n80_), .c(x16), .O(z00));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  nor2   g023(.a(new_n66_), .b(x28), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x36), .c(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g030(.a(x27), .b(x08), .c(new_n86_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x04), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x39), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x40), .O(new_n96_));
  nor2   g034(.a(x40), .b(new_n81_), .O(new_n97_));
  inv1   g035(.a(new_n97_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n96_), .O(z02));
  nand2  g037(.a(new_n71_), .b(x27), .O(new_n100_));
  inv1   g038(.a(new_n82_), .O(new_n101_));
  aoi21  g039(.a(new_n70_), .b(new_n64_), .c(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x21), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  nand3  g042(.a(new_n102_), .b(new_n100_), .c(new_n104_), .O(z04));
  nand2  g043(.a(x28), .b(x27), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n70_), .c(new_n101_), .O(z05));
  nand3  g045(.a(new_n106_), .b(new_n82_), .c(new_n70_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n63_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x03), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n82_), .O(z07));
  inv1   g058(.a(new_n77_), .O(new_n121_));
  nand2  g059(.a(new_n82_), .b(new_n121_), .O(z08));
  inv1   g060(.a(x11), .O(new_n123_));
  nand4  g061(.a(new_n82_), .b(x34), .c(x27), .d(x26), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n123_), .O(z09));
  nor2   g063(.a(new_n86_), .b(x36), .O(new_n126_));
  nor2   g064(.a(new_n76_), .b(x39), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n97_), .c(x07), .O(new_n128_));
  nand2  g066(.a(new_n77_), .b(x05), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n128_), .c(new_n126_), .O(new_n130_));
  oai21  g068(.a(x32), .b(x30), .c(x40), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n75_), .O(new_n132_));
  and2   g070(.a(new_n132_), .b(x05), .O(new_n133_));
  aoi21  g071(.a(new_n130_), .b(x27), .c(new_n133_), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  oai21  g073(.a(new_n134_), .b(x04), .c(new_n135_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  nand2  g075(.a(new_n77_), .b(x29), .O(new_n138_));
  nand2  g076(.a(x27), .b(x04), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n86_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  inv1   g079(.a(x08), .O(new_n142_));
  inv1   g080(.a(x28), .O(new_n143_));
  nand2  g081(.a(x35), .b(new_n143_), .O(new_n144_));
  nand2  g082(.a(x27), .b(new_n65_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n141_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n84_), .c(new_n137_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n82_), .O(z11));
  nand3  g087(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x27), .c(x10), .O(new_n151_));
  nor2   g089(.a(new_n73_), .b(new_n64_), .O(new_n152_));
  aoi21  g090(.a(x39), .b(new_n65_), .c(new_n152_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n151_), .c(x40), .O(new_n154_));
  nor2   g092(.a(new_n152_), .b(x40), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x06), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n154_), .O(z12));
  inv1   g095(.a(x13), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x28), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x39), .c(new_n158_), .d(new_n65_), .O(new_n161_));
  inv1   g099(.a(x18), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  nand4  g101(.a(new_n72_), .b(x20), .c(new_n163_), .d(new_n162_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n161_), .c(new_n76_), .O(new_n165_));
  nor2   g103(.a(new_n164_), .b(new_n81_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n165_), .c(x27), .O(new_n167_));
  nand3  g105(.a(new_n132_), .b(new_n158_), .c(new_n65_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z13));
  nand3  g107(.a(x20), .b(new_n163_), .c(new_n162_), .O(new_n170_));
  nand4  g108(.a(new_n159_), .b(new_n85_), .c(new_n84_), .d(x28), .O(new_n171_));
  nand3  g109(.a(x39), .b(new_n158_), .c(new_n65_), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n171_), .c(new_n76_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n97_), .c(new_n170_), .O(new_n175_));
  nand3  g113(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n85_), .c(new_n84_), .O(new_n178_));
  oai21  g116(.a(new_n173_), .b(new_n152_), .c(new_n178_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x40), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n175_), .c(new_n156_), .O(z14));
  inv1   g119(.a(x12), .O(new_n182_));
  nor2   g120(.a(new_n124_), .b(new_n182_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand4  g122(.a(new_n82_), .b(new_n184_), .c(x22), .d(x01), .O(new_n185_));
  inv1   g123(.a(new_n185_), .O(z16));
  inv1   g124(.a(x01), .O(new_n187_));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n82_), .b(new_n188_), .c(x23), .d(x22), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n187_), .O(z17));
  nand2  g128(.a(new_n64_), .b(x08), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n145_), .c(new_n66_), .O(new_n192_));
  nand2  g130(.a(new_n84_), .b(new_n137_), .O(new_n193_));
  aoi21  g131(.a(new_n192_), .b(new_n143_), .c(new_n193_), .O(new_n194_));
  oai22  g132(.a(new_n194_), .b(new_n101_), .c(new_n91_), .d(new_n121_), .O(z18));
  oai22  g133(.a(x40), .b(x35), .c(x27), .d(x08), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(x06), .O(new_n197_));
  nand2  g135(.a(new_n139_), .b(new_n143_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n138_), .O(new_n199_));
  nand2  g137(.a(x35), .b(x27), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(x04), .c(new_n142_), .O(new_n201_));
  nand2  g139(.a(x39), .b(x29), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n66_), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n201_), .O(new_n204_));
  nand2  g142(.a(new_n204_), .b(x40), .O(new_n205_));
  nand2  g143(.a(x28), .b(new_n142_), .O(new_n206_));
  nand4  g144(.a(new_n206_), .b(new_n205_), .c(new_n199_), .d(new_n197_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n84_), .c(new_n137_), .O(new_n208_));
  nand2  g146(.a(new_n208_), .b(new_n82_), .O(z19));
  aoi21  g147(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n210_));
  nand2  g148(.a(new_n202_), .b(x40), .O(new_n211_));
  aoi21  g149(.a(new_n211_), .b(new_n98_), .c(new_n210_), .O(new_n212_));
  nand2  g150(.a(new_n82_), .b(new_n64_), .O(new_n213_));
  nand2  g151(.a(x40), .b(new_n66_), .O(new_n214_));
  aoi21  g152(.a(new_n214_), .b(new_n213_), .c(x08), .O(new_n215_));
  oai21  g153(.a(new_n215_), .b(new_n212_), .c(new_n84_), .O(new_n216_));
  nor2   g154(.a(new_n216_), .b(x09), .O(z20));
endmodule


