// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n150_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_;
  nor2   g000(.a(x30), .b(x04), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  inv1   g009(.a(x40), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n71_), .c(new_n72_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x39), .c(x30), .d(new_n64_), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(x04), .c(new_n70_), .d(new_n63_), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x27), .c(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(z01));
  inv1   g021(.a(new_n63_), .O(new_n84_));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g025(.a(x28), .O(new_n88_));
  nand2  g026(.a(x35), .b(new_n88_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n91_), .c(new_n87_), .d(new_n84_), .O(z02));
  nand2  g032(.a(new_n68_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n67_), .b(new_n66_), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n95_), .c(new_n63_), .O(z03));
  aoi21  g036(.a(new_n67_), .b(new_n66_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n63_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n63_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x31), .O(new_n104_));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n64_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .d(new_n104_), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n107_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n109_), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n84_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  nand2  g054(.a(new_n92_), .b(new_n84_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n84_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  inv1   g058(.a(x04), .O(new_n121_));
  nand4  g059(.a(new_n92_), .b(new_n80_), .c(x27), .d(x07), .O(new_n122_));
  aoi21  g060(.a(new_n93_), .b(x05), .c(new_n77_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(z10));
  inv1   g065(.a(x09), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  nand3  g067(.a(x35), .b(new_n88_), .c(new_n66_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x08), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(x04), .c(x30), .O(z11));
  nand3  g072(.a(new_n67_), .b(new_n79_), .c(new_n78_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x27), .c(x10), .O(new_n136_));
  nor2   g074(.a(new_n93_), .b(new_n69_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n136_), .c(x30), .O(new_n138_));
  or2    g076(.a(new_n69_), .b(new_n121_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(z12));
  oai21  g078(.a(new_n92_), .b(x13), .c(x30), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n121_), .O(new_n142_));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand4  g082(.a(new_n69_), .b(x20), .c(new_n144_), .d(new_n143_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(z13));
  nand4  g084(.a(x27), .b(x20), .c(new_n144_), .d(new_n143_), .O(new_n147_));
  aoi21  g085(.a(new_n68_), .b(new_n67_), .c(new_n147_), .O(new_n148_));
  aoi21  g086(.a(new_n141_), .b(new_n121_), .c(new_n148_), .O(z14));
  inv1   g087(.a(x12), .O(new_n150_));
  nor2   g088(.a(new_n119_), .b(new_n150_), .O(z15));
  inv1   g089(.a(x23), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(x22), .c(x01), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n84_), .O(z16));
  inv1   g092(.a(x24), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x23), .c(x22), .d(x01), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n84_), .O(z17));
  inv1   g095(.a(x08), .O(new_n158_));
  aoi21  g096(.a(new_n130_), .b(new_n129_), .c(new_n158_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(x09), .c(x04), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n77_), .O(z18));
  nand3  g099(.a(x35), .b(new_n88_), .c(new_n66_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n129_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x08), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n77_), .c(new_n128_), .d(x04), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z19));
  inv1   g104(.a(new_n129_), .O(new_n167_));
  aoi21  g105(.a(x28), .b(new_n66_), .c(new_n78_), .O(new_n168_));
  oai21  g106(.a(new_n78_), .b(new_n66_), .c(new_n158_), .O(new_n169_));
  oai21  g107(.a(new_n168_), .b(new_n167_), .c(new_n169_), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n77_), .c(new_n128_), .d(x04), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z20));
endmodule


