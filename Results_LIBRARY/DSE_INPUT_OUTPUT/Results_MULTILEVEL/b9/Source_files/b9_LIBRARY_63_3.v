// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  nand2  g017(.a(x35), .b(x30), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n79_), .c(x16), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  oai21  g021(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n80_), .c(x04), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n75_), .c(new_n80_), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n82_), .c(new_n66_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n65_), .c(new_n89_), .O(z02));
  oai21  g031(.a(x35), .b(x27), .c(x30), .O(new_n94_));
  nand2  g032(.a(new_n71_), .b(x27), .O(new_n95_));
  nand2  g033(.a(new_n70_), .b(new_n64_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand4  g036(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(new_n98_), .O(z04));
  inv1   g037(.a(new_n80_), .O(new_n100_));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n70_), .c(new_n100_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n63_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(new_n111_));
  oai22  g049(.a(new_n111_), .b(x14), .c(new_n108_), .d(x31), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n80_), .c(x03), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z07));
  nand2  g052(.a(new_n80_), .b(new_n75_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n80_), .O(z09));
  nand2  g055(.a(x05), .b(new_n65_), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n75_), .c(new_n66_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x30), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  nand2  g059(.a(new_n75_), .b(x07), .O(new_n122_));
  oai21  g060(.a(new_n75_), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n84_), .c(x27), .O(new_n124_));
  nand3  g062(.a(new_n76_), .b(x32), .c(x05), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n65_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n120_), .O(z10));
  inv1   g067(.a(x09), .O(new_n130_));
  nand2  g068(.a(new_n76_), .b(x29), .O(new_n131_));
  nor2   g069(.a(new_n66_), .b(x28), .O(new_n132_));
  oai21  g070(.a(new_n64_), .b(new_n65_), .c(new_n132_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  inv1   g072(.a(x08), .O(new_n135_));
  nand3  g073(.a(new_n132_), .b(x27), .c(new_n65_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n82_), .c(new_n130_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  nand3  g078(.a(new_n80_), .b(new_n77_), .c(new_n64_), .O(new_n141_));
  oai21  g079(.a(x30), .b(x28), .c(x35), .O(new_n142_));
  nand2  g080(.a(new_n76_), .b(new_n65_), .O(new_n143_));
  nor2   g081(.a(new_n68_), .b(x10), .O(new_n144_));
  aoi21  g082(.a(new_n143_), .b(new_n142_), .c(new_n144_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(x37), .c(new_n141_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  oai21  g085(.a(x35), .b(x28), .c(x30), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x32), .O(new_n149_));
  nand2  g087(.a(new_n80_), .b(new_n90_), .O(new_n150_));
  nand3  g088(.a(x36), .b(x35), .c(new_n82_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand2  g091(.a(new_n66_), .b(x30), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n153_), .c(new_n149_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x40), .c(x39), .d(new_n147_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand2  g096(.a(new_n72_), .b(new_n82_), .O(new_n159_));
  nand2  g097(.a(x37), .b(new_n66_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n159_), .c(new_n64_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n162_));
  oai21  g100(.a(new_n156_), .b(x04), .c(new_n162_), .O(z13));
  nand3  g101(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n164_));
  nand2  g102(.a(new_n66_), .b(new_n82_), .O(new_n165_));
  oai21  g103(.a(x36), .b(new_n66_), .c(new_n165_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n83_), .c(x28), .O(new_n167_));
  nand3  g105(.a(new_n76_), .b(new_n147_), .c(new_n65_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n167_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n164_), .O(new_n171_));
  nand2  g109(.a(new_n154_), .b(new_n64_), .O(new_n172_));
  nand4  g110(.a(new_n70_), .b(new_n66_), .c(new_n82_), .d(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n83_), .O(new_n175_));
  nand2  g113(.a(new_n72_), .b(x27), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n168_), .c(new_n100_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n175_), .c(new_n171_), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n80_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n80_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n80_), .O(z17));
  nand2  g124(.a(new_n132_), .b(new_n64_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n131_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(x08), .O(new_n189_));
  nand4  g127(.a(new_n189_), .b(new_n136_), .c(new_n82_), .d(new_n130_), .O(z18));
  inv1   g128(.a(new_n131_), .O(new_n191_));
  oai21  g129(.a(new_n66_), .b(new_n64_), .c(new_n135_), .O(new_n192_));
  aoi21  g130(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n191_), .c(new_n192_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n82_), .c(new_n130_), .O(new_n195_));
  inv1   g133(.a(new_n195_), .O(z20));
  inv1   g134(.a(new_n139_), .O(z19));
endmodule


