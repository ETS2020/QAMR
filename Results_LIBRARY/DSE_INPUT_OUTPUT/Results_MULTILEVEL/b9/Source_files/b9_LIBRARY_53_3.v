// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  and2   g012(.a(x40), .b(x39), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(x10), .c(new_n65_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n74_), .c(new_n63_), .O(new_n78_));
  nand2  g016(.a(x30), .b(x27), .O(new_n79_));
  and2   g017(.a(new_n79_), .b(x16), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n78_), .O(z00));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n67_), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(x32), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(x30), .c(x27), .O(new_n86_));
  nor3   g024(.a(x32), .b(x30), .c(x27), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n65_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n86_), .O(z01));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nand2  g028(.a(new_n83_), .b(x04), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n75_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n90_), .c(new_n79_), .O(new_n93_));
  inv1   g031(.a(x08), .O(new_n94_));
  nand3  g032(.a(new_n64_), .b(new_n94_), .c(x04), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n93_), .O(z02));
  inv1   g034(.a(new_n79_), .O(new_n97_));
  inv1   g035(.a(x30), .O(new_n98_));
  nand3  g036(.a(new_n71_), .b(new_n98_), .c(x27), .O(new_n99_));
  inv1   g037(.a(new_n99_), .O(new_n100_));
  aoi21  g038(.a(new_n70_), .b(new_n64_), .c(new_n100_), .O(new_n101_));
  oai21  g039(.a(new_n97_), .b(x21), .c(new_n101_), .O(z03));
  nand2  g040(.a(new_n79_), .b(x21), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n101_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  oai22  g043(.a(new_n105_), .b(x30), .c(new_n70_), .d(x27), .O(z05));
  inv1   g044(.a(new_n105_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n79_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x14), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n111_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  oai21  g055(.a(new_n113_), .b(x31), .c(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n79_), .c(x03), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z07));
  nand3  g058(.a(new_n79_), .b(x40), .c(x39), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z08));
  inv1   g060(.a(x34), .O(new_n123_));
  nand3  g061(.a(x27), .b(x26), .c(x11), .O(new_n124_));
  nor3   g062(.a(new_n124_), .b(new_n123_), .c(x30), .O(z09));
  nand2  g063(.a(x40), .b(x39), .O(new_n126_));
  nand2  g064(.a(x05), .b(new_n65_), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n126_), .c(new_n64_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x30), .O(new_n129_));
  nand2  g067(.a(new_n126_), .b(x07), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x05), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n84_), .c(x27), .O(new_n133_));
  nand3  g071(.a(new_n75_), .b(x32), .c(x05), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n65_), .O(new_n136_));
  nand3  g074(.a(x37), .b(x27), .c(x06), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n129_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  nand2  g077(.a(new_n75_), .b(x29), .O(new_n140_));
  nor2   g078(.a(new_n64_), .b(new_n65_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n83_), .c(new_n140_), .O(new_n142_));
  nor2   g080(.a(new_n66_), .b(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(x27), .c(new_n65_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n94_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n98_), .c(new_n139_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(z11));
  inv1   g086(.a(x10), .O(new_n149_));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n64_), .c(new_n149_), .O(new_n151_));
  nand2  g089(.a(new_n72_), .b(x27), .O(new_n152_));
  oai21  g090(.a(new_n126_), .b(x04), .c(new_n152_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n151_), .c(new_n79_), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  inv1   g093(.a(x40), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n156_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n155_), .d(new_n65_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n72_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n159_), .c(new_n98_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  inv1   g102(.a(x32), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n98_), .c(new_n156_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x39), .c(new_n155_), .d(new_n65_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n164_), .O(z13));
  nand3  g106(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n169_));
  nand4  g107(.a(new_n157_), .b(new_n165_), .c(x28), .d(x27), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  nand3  g109(.a(new_n75_), .b(new_n155_), .c(new_n65_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n171_), .c(new_n169_), .O(new_n173_));
  nand4  g111(.a(new_n70_), .b(new_n66_), .c(new_n165_), .d(x28), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n98_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x27), .O(new_n176_));
  aoi21  g114(.a(new_n172_), .b(new_n152_), .c(new_n87_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n176_), .c(new_n173_), .O(z14));
  nand3  g116(.a(x27), .b(x26), .c(x12), .O(new_n179_));
  nor3   g117(.a(new_n179_), .b(new_n123_), .c(x30), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n79_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n79_), .O(z17));
  nand2  g124(.a(new_n98_), .b(x29), .O(new_n187_));
  oai22  g125(.a(new_n187_), .b(new_n126_), .c(new_n83_), .d(x27), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(x08), .O(new_n189_));
  nand2  g127(.a(new_n144_), .b(new_n139_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n98_), .O(new_n191_));
  nand2  g129(.a(x30), .b(new_n64_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n191_), .c(new_n189_), .O(z18));
  inv1   g131(.a(new_n140_), .O(new_n194_));
  oai21  g132(.a(new_n66_), .b(new_n64_), .c(new_n94_), .O(new_n195_));
  aoi21  g133(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n194_), .c(new_n195_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n98_), .c(new_n139_), .O(new_n198_));
  inv1   g136(.a(new_n198_), .O(z20));
  inv1   g137(.a(new_n147_), .O(z19));
endmodule


