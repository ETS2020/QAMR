// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_;
  inv1   g000(.a(x37), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x35), .O(new_n65_));
  inv1   g003(.a(x36), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g005(.a(new_n67_), .b(x40), .c(x39), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(x40), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x35), .c(x28), .O(new_n70_));
  nand3  g008(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  nor2   g010(.a(new_n69_), .b(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n64_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  aoi21  g013(.a(new_n71_), .b(x27), .c(new_n75_), .O(new_n76_));
  inv1   g014(.a(x16), .O(new_n77_));
  nor2   g015(.a(x40), .b(x35), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  oai21  g017(.a(new_n76_), .b(x15), .c(new_n79_), .O(z00));
  inv1   g018(.a(new_n78_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x27), .c(x04), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  nand2  g024(.a(x35), .b(x28), .O(new_n87_));
  oai21  g025(.a(new_n69_), .b(x35), .c(new_n87_), .O(new_n88_));
  nand4  g026(.a(new_n88_), .b(new_n66_), .c(new_n83_), .d(new_n82_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n86_), .O(z01));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  inv1   g030(.a(x28), .O(new_n93_));
  nand2  g031(.a(x35), .b(new_n93_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n92_), .c(x04), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x39), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n91_), .c(x40), .O(new_n97_));
  oai21  g035(.a(x40), .b(new_n65_), .c(new_n97_), .O(z02));
  nand2  g036(.a(new_n87_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x27), .O(new_n100_));
  nand2  g038(.a(new_n63_), .b(new_n100_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n99_), .c(new_n81_), .d(x21), .O(z03));
  oai21  g040(.a(new_n69_), .b(x27), .c(new_n65_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x21), .O(new_n104_));
  nor2   g042(.a(new_n69_), .b(x35), .O(new_n105_));
  inv1   g043(.a(new_n94_), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(new_n105_), .c(x27), .O(new_n107_));
  nand3  g045(.a(new_n81_), .b(new_n63_), .c(new_n100_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(new_n104_), .O(z04));
  nand2  g047(.a(x28), .b(x27), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n81_), .c(new_n63_), .O(z05));
  inv1   g049(.a(z05), .O(z06));
  inv1   g050(.a(x33), .O(new_n113_));
  inv1   g051(.a(x15), .O(new_n114_));
  nand2  g052(.a(x17), .b(new_n114_), .O(new_n115_));
  inv1   g053(.a(x00), .O(new_n116_));
  oai21  g054(.a(x25), .b(new_n116_), .c(x38), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  inv1   g056(.a(x25), .O(new_n119_));
  nand3  g057(.a(x38), .b(new_n119_), .c(new_n116_), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  oai22  g059(.a(new_n121_), .b(x14), .c(new_n118_), .d(x31), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n81_), .c(x03), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(z07));
  oai21  g062(.a(new_n69_), .b(new_n72_), .c(new_n81_), .O(z08));
  nand4  g063(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n81_), .O(z09));
  oai21  g065(.a(new_n65_), .b(x28), .c(new_n66_), .O(new_n128_));
  inv1   g066(.a(x07), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x05), .O(new_n130_));
  oai21  g068(.a(x39), .b(new_n129_), .c(new_n130_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand2  g070(.a(new_n66_), .b(x28), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n69_), .c(x07), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n132_), .c(new_n100_), .O(new_n135_));
  inv1   g073(.a(x05), .O(new_n136_));
  nand3  g074(.a(new_n84_), .b(x40), .c(x39), .O(new_n137_));
  nor2   g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n135_), .c(new_n64_), .O(new_n139_));
  nor2   g077(.a(new_n63_), .b(new_n100_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(x06), .c(new_n78_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(z10));
  inv1   g080(.a(x09), .O(new_n143_));
  oai21  g081(.a(new_n100_), .b(new_n64_), .c(new_n93_), .O(new_n144_));
  nand2  g082(.a(new_n73_), .b(x29), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  inv1   g084(.a(x08), .O(new_n147_));
  nand2  g085(.a(x39), .b(x29), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n147_), .c(new_n65_), .O(new_n149_));
  nand3  g087(.a(new_n93_), .b(x27), .c(new_n64_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n149_), .c(new_n146_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n82_), .c(new_n143_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n81_), .O(z11));
  inv1   g092(.a(x10), .O(new_n155_));
  nor3   g093(.a(x37), .b(x36), .c(x35), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n100_), .c(new_n155_), .O(new_n157_));
  nand2  g095(.a(new_n87_), .b(new_n63_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  oai21  g097(.a(new_n72_), .b(x04), .c(new_n159_), .O(new_n160_));
  oai21  g098(.a(x37), .b(x28), .c(x35), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n100_), .c(new_n69_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n160_), .c(new_n157_), .O(z12));
  inv1   g101(.a(x13), .O(new_n164_));
  nand2  g102(.a(x36), .b(x35), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x28), .c(new_n69_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x39), .c(new_n164_), .d(new_n64_), .O(new_n167_));
  inv1   g105(.a(x18), .O(new_n168_));
  inv1   g106(.a(x19), .O(new_n169_));
  nand2  g107(.a(new_n70_), .b(new_n63_), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(x20), .c(new_n169_), .d(new_n168_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n167_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x27), .O(new_n173_));
  nor2   g111(.a(new_n137_), .b(x13), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n64_), .c(new_n78_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n173_), .O(z13));
  nand3  g114(.a(x20), .b(new_n169_), .c(new_n168_), .O(new_n177_));
  nand4  g115(.a(new_n165_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n73_), .c(new_n164_), .d(new_n64_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n177_), .O(new_n180_));
  nand3  g118(.a(new_n63_), .b(new_n65_), .c(x28), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(x27), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n83_), .c(new_n82_), .O(new_n183_));
  nand3  g121(.a(x39), .b(new_n164_), .c(new_n64_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n159_), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n183_), .c(new_n180_), .d(new_n162_), .O(z14));
  nand4  g124(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n81_), .O(z15));
  inv1   g126(.a(x23), .O(new_n189_));
  nand4  g127(.a(new_n81_), .b(new_n189_), .c(x22), .d(x01), .O(new_n190_));
  inv1   g128(.a(new_n190_), .O(z16));
  inv1   g129(.a(x24), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(x23), .c(x22), .d(x01), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n81_), .O(z17));
  oai21  g132(.a(new_n94_), .b(x27), .c(new_n145_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(x08), .O(new_n196_));
  oai21  g134(.a(x30), .b(x09), .c(new_n81_), .O(new_n197_));
  nand3  g135(.a(new_n106_), .b(x27), .c(new_n64_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n197_), .c(new_n196_), .O(z18));
  oai21  g137(.a(new_n65_), .b(new_n100_), .c(new_n147_), .O(new_n200_));
  oai21  g138(.a(new_n93_), .b(x27), .c(x35), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n148_), .O(new_n202_));
  nand3  g140(.a(new_n69_), .b(x28), .c(new_n100_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n202_), .c(new_n200_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n82_), .c(new_n143_), .O(new_n205_));
  nand2  g143(.a(new_n205_), .b(new_n81_), .O(z20));
  nand2  g144(.a(new_n153_), .b(new_n81_), .O(z19));
endmodule


