// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_;
  inv1   g000(.a(x10), .O(new_n63_));
  nand2  g001(.a(x19), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(x19), .c(new_n63_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x40), .c(x39), .d(new_n66_), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(x04), .c(new_n72_), .d(new_n65_), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n78_), .c(new_n77_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n81_), .c(x04), .O(new_n90_));
  nand2  g028(.a(x40), .b(x39), .O(new_n91_));
  inv1   g029(.a(new_n91_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n88_), .d(new_n64_), .O(z02));
  nand2  g031(.a(new_n70_), .b(x27), .O(new_n94_));
  nand2  g032(.a(new_n69_), .b(new_n68_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g034(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n65_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n64_), .c(new_n69_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n66_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  oai22  g047(.a(new_n109_), .b(x14), .c(new_n106_), .d(x31), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n64_), .c(x03), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z07));
  inv1   g050(.a(x39), .O(new_n113_));
  inv1   g051(.a(x40), .O(new_n114_));
  nor3   g052(.a(new_n65_), .b(new_n114_), .c(new_n113_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(z09));
  inv1   g055(.a(x04), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  nand2  g057(.a(new_n91_), .b(x07), .O(new_n120_));
  oai21  g058(.a(new_n91_), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n82_), .c(x27), .O(new_n122_));
  nand2  g060(.a(new_n78_), .b(new_n77_), .O(new_n123_));
  nand4  g061(.a(new_n123_), .b(x40), .c(x39), .d(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n118_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n64_), .O(z10));
  inv1   g066(.a(x09), .O(new_n129_));
  nand2  g067(.a(new_n92_), .b(x29), .O(new_n130_));
  nor2   g068(.a(new_n68_), .b(new_n118_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n81_), .c(new_n130_), .O(new_n132_));
  inv1   g070(.a(x08), .O(new_n133_));
  nand4  g071(.a(x35), .b(new_n80_), .c(x27), .d(new_n118_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n77_), .c(new_n129_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n64_), .O(z11));
  inv1   g076(.a(x19), .O(new_n139_));
  inv1   g077(.a(x35), .O(new_n140_));
  nand3  g078(.a(new_n69_), .b(new_n79_), .c(new_n140_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x27), .c(x10), .O(new_n142_));
  aoi21  g080(.a(new_n92_), .b(new_n118_), .c(new_n71_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n142_), .c(new_n139_), .O(new_n144_));
  nand2  g082(.a(new_n143_), .b(x10), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n144_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x28), .c(new_n114_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x39), .c(new_n147_), .d(new_n118_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  nand2  g089(.a(new_n70_), .b(new_n69_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(x20), .c(new_n151_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n150_), .c(new_n68_), .O(new_n154_));
  nand3  g092(.a(new_n123_), .b(x40), .c(x39), .O(new_n155_));
  nor3   g093(.a(new_n155_), .b(x13), .c(x04), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n154_), .c(new_n139_), .O(new_n157_));
  aoi21  g095(.a(new_n148_), .b(x28), .c(new_n68_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n123_), .c(x40), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(new_n113_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(new_n147_), .c(x10), .d(new_n118_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(z13));
  aoi21  g100(.a(x20), .b(new_n151_), .c(x19), .O(new_n163_));
  nor2   g101(.a(new_n139_), .b(new_n63_), .O(new_n164_));
  nand4  g102(.a(new_n148_), .b(new_n78_), .c(new_n77_), .d(x28), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(new_n166_));
  nand3  g104(.a(new_n92_), .b(new_n147_), .c(new_n118_), .O(new_n167_));
  oai22  g105(.a(new_n167_), .b(new_n166_), .c(new_n164_), .d(new_n163_), .O(new_n168_));
  inv1   g106(.a(new_n167_), .O(new_n169_));
  nand3  g107(.a(new_n69_), .b(new_n140_), .c(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n78_), .c(new_n77_), .O(new_n172_));
  oai21  g110(.a(new_n169_), .b(new_n71_), .c(new_n172_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n139_), .O(new_n174_));
  nand4  g112(.a(new_n78_), .b(new_n77_), .c(new_n68_), .d(x10), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(new_n168_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n64_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand4  g117(.a(new_n64_), .b(new_n179_), .c(x22), .d(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z16));
  inv1   g119(.a(x01), .O(new_n182_));
  inv1   g120(.a(x24), .O(new_n183_));
  nand4  g121(.a(new_n64_), .b(new_n183_), .c(x23), .d(x22), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n182_), .O(z17));
  oai21  g123(.a(new_n81_), .b(x27), .c(new_n130_), .O(new_n186_));
  nand3  g124(.a(new_n134_), .b(new_n77_), .c(new_n129_), .O(new_n187_));
  aoi21  g125(.a(new_n186_), .b(x08), .c(new_n187_), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n65_), .O(z18));
  nand4  g127(.a(new_n136_), .b(new_n64_), .c(new_n77_), .d(new_n129_), .O(new_n190_));
  inv1   g128(.a(new_n190_), .O(z19));
  inv1   g129(.a(new_n130_), .O(new_n192_));
  oai21  g130(.a(new_n140_), .b(new_n68_), .c(new_n133_), .O(new_n193_));
  aoi21  g131(.a(x28), .b(new_n68_), .c(new_n140_), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n192_), .c(new_n193_), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n64_), .c(new_n77_), .d(new_n129_), .O(new_n196_));
  inv1   g134(.a(new_n196_), .O(z20));
endmodule


