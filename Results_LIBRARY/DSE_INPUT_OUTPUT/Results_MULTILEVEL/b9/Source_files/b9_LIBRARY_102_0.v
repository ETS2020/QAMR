// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n195_,
    new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x30), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  nor4   g006(.a(new_n68_), .b(x15), .c(new_n67_), .d(x04), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x40), .c(x39), .d(new_n71_), .O(new_n75_));
  nand2  g013(.a(x35), .b(x28), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n75_), .c(new_n64_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x37), .c(x27), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(x15), .c(new_n70_), .O(z00));
  inv1   g017(.a(x27), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x32), .c(x37), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n64_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x04), .O(z01));
  inv1   g024(.a(new_n66_), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(new_n72_), .b(x28), .O(new_n91_));
  oai21  g029(.a(x27), .b(x08), .c(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(new_n68_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n90_), .c(new_n87_), .O(z02));
  nand2  g032(.a(new_n76_), .b(x27), .O(new_n95_));
  oai21  g033(.a(new_n64_), .b(new_n80_), .c(new_n65_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n95_), .c(x21), .O(z03));
  nand2  g035(.a(new_n96_), .b(x21), .O(new_n98_));
  nand3  g036(.a(new_n76_), .b(new_n66_), .c(x27), .O(new_n99_));
  nand3  g037(.a(new_n65_), .b(x30), .c(new_n80_), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(z04));
  nand3  g039(.a(x30), .b(x28), .c(x27), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n65_), .O(z05));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n65_), .c(x30), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  inv1   g045(.a(x15), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n108_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n66_), .c(x03), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z07));
  nand2  g056(.a(new_n68_), .b(new_n66_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n66_), .O(z09));
  nand3  g059(.a(new_n68_), .b(new_n66_), .c(x07), .O(new_n122_));
  inv1   g060(.a(new_n68_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(x37), .c(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n122_), .c(new_n82_), .O(new_n125_));
  inv1   g063(.a(x32), .O(new_n126_));
  oai21  g064(.a(new_n65_), .b(new_n126_), .c(new_n64_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(x40), .c(x39), .d(x05), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n125_), .b(x27), .c(new_n129_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  oai21  g069(.a(new_n130_), .b(x04), .c(new_n131_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  nand2  g072(.a(x27), .b(x04), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n91_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  inv1   g075(.a(x08), .O(new_n138_));
  nand3  g076(.a(new_n91_), .b(x27), .c(new_n71_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n133_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x37), .c(x30), .O(z11));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n80_), .c(new_n67_), .O(new_n145_));
  nand2  g083(.a(new_n76_), .b(new_n65_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  oai21  g085(.a(new_n68_), .b(x04), .c(new_n147_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n145_), .c(new_n66_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  inv1   g088(.a(x40), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n151_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n150_), .d(new_n71_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand3  g094(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n154_), .c(new_n65_), .O(new_n158_));
  nand3  g096(.a(x35), .b(x30), .c(x28), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n158_), .c(x27), .O(new_n161_));
  and2   g099(.a(new_n127_), .b(x40), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x39), .c(new_n150_), .d(new_n71_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n161_), .O(z13));
  nand4  g102(.a(new_n152_), .b(new_n126_), .c(new_n64_), .d(x28), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(new_n166_));
  nand3  g104(.a(new_n123_), .b(new_n150_), .c(new_n71_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n166_), .c(new_n157_), .O(new_n168_));
  oai21  g106(.a(x32), .b(x27), .c(x37), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n64_), .O(new_n170_));
  nand2  g108(.a(new_n167_), .b(new_n147_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n170_), .c(new_n168_), .O(z14));
  inv1   g110(.a(x12), .O(new_n173_));
  nand4  g111(.a(new_n66_), .b(x34), .c(x27), .d(x26), .O(new_n174_));
  nor2   g112(.a(new_n174_), .b(new_n173_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(x22), .c(x01), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n66_), .O(z16));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(x23), .c(x22), .d(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n66_), .O(z17));
  nand2  g119(.a(new_n91_), .b(new_n80_), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n134_), .c(new_n138_), .O(new_n183_));
  nand2  g121(.a(new_n139_), .b(new_n133_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n183_), .c(x37), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n64_), .O(z18));
  inv1   g124(.a(new_n134_), .O(new_n187_));
  aoi21  g125(.a(x37), .b(new_n72_), .c(x28), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n135_), .c(new_n187_), .O(new_n189_));
  oai21  g127(.a(new_n72_), .b(new_n80_), .c(x37), .O(new_n190_));
  nor2   g128(.a(x28), .b(x04), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n190_), .c(x08), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n189_), .c(new_n133_), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(x37), .c(x30), .O(z19));
  oai21  g132(.a(new_n72_), .b(new_n80_), .c(new_n138_), .O(new_n195_));
  aoi21  g133(.a(x28), .b(new_n80_), .c(new_n72_), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n187_), .c(new_n195_), .O(new_n197_));
  nand4  g135(.a(new_n197_), .b(x37), .c(new_n64_), .d(new_n133_), .O(new_n198_));
  inv1   g136(.a(new_n198_), .O(z20));
endmodule


