// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n180_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  and2   g012(.a(x40), .b(x39), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(x10), .c(new_n65_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n74_), .c(new_n63_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  nand2  g017(.a(x32), .b(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n78_), .c(x16), .O(z00));
  inv1   g019(.a(x28), .O(new_n82_));
  aoi21  g020(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  nor3   g021(.a(new_n83_), .b(x32), .c(new_n64_), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x30), .c(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g026(.a(x35), .b(new_n82_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n88_), .c(new_n80_), .d(new_n75_), .O(z02));
  inv1   g030(.a(new_n80_), .O(new_n93_));
  nand2  g031(.a(new_n71_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n70_), .b(new_n64_), .c(new_n95_), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n94_), .c(new_n93_), .O(z03));
  nand2  g035(.a(new_n70_), .b(new_n64_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n94_), .c(new_n80_), .d(new_n95_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n80_), .c(new_n70_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n63_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n80_), .O(z07));
  nand2  g051(.a(x40), .b(x39), .O(new_n114_));
  nand2  g052(.a(new_n80_), .b(new_n114_), .O(z08));
  inv1   g053(.a(x11), .O(new_n116_));
  nand4  g054(.a(new_n80_), .b(x34), .c(x27), .d(x26), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  nand3  g056(.a(new_n80_), .b(new_n114_), .c(x07), .O(new_n119_));
  inv1   g057(.a(x32), .O(new_n120_));
  nand3  g058(.a(new_n75_), .b(new_n120_), .c(x05), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n119_), .c(new_n83_), .O(new_n122_));
  nor2   g060(.a(new_n93_), .b(new_n70_), .O(new_n123_));
  aoi22  g061(.a(new_n123_), .b(x06), .c(new_n122_), .d(new_n65_), .O(new_n124_));
  nand4  g062(.a(new_n75_), .b(x30), .c(x05), .d(new_n65_), .O(new_n125_));
  oai21  g063(.a(new_n124_), .b(new_n64_), .c(new_n125_), .O(z10));
  inv1   g064(.a(x09), .O(new_n127_));
  nand2  g065(.a(new_n75_), .b(x29), .O(new_n128_));
  nor2   g066(.a(new_n64_), .b(new_n65_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n89_), .c(new_n128_), .O(new_n130_));
  inv1   g068(.a(x08), .O(new_n131_));
  nand4  g069(.a(x35), .b(new_n82_), .c(x27), .d(new_n65_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n127_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n120_), .c(x30), .O(z11));
  inv1   g074(.a(x10), .O(new_n137_));
  nor3   g075(.a(x37), .b(x36), .c(x35), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n64_), .c(new_n137_), .O(new_n139_));
  oai22  g077(.a(new_n114_), .b(x04), .c(new_n73_), .d(new_n64_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n139_), .c(new_n93_), .O(z12));
  inv1   g079(.a(x13), .O(new_n142_));
  inv1   g080(.a(x40), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x28), .c(new_n143_), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(x39), .c(new_n142_), .d(new_n65_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand4  g086(.a(new_n72_), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n146_), .c(x32), .O(new_n150_));
  nand4  g088(.a(new_n72_), .b(x30), .c(x20), .d(new_n148_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(x18), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(x27), .O(new_n153_));
  nor2   g091(.a(x13), .b(x04), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n75_), .c(x30), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(z13));
  nand3  g094(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n157_));
  nand3  g095(.a(new_n144_), .b(new_n79_), .c(x28), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  nand2  g097(.a(new_n154_), .b(new_n75_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n157_), .O(new_n161_));
  oai21  g099(.a(new_n73_), .b(new_n64_), .c(new_n160_), .O(new_n162_));
  nand3  g100(.a(new_n70_), .b(new_n66_), .c(x28), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n120_), .c(x27), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n79_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n162_), .c(new_n161_), .O(z14));
  inv1   g104(.a(x12), .O(new_n167_));
  nor2   g105(.a(new_n117_), .b(new_n167_), .O(z15));
  inv1   g106(.a(x23), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(x22), .c(x01), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n80_), .O(z16));
  inv1   g109(.a(x01), .O(new_n172_));
  inv1   g110(.a(x24), .O(new_n173_));
  nand4  g111(.a(new_n80_), .b(new_n173_), .c(x23), .d(x22), .O(new_n174_));
  nor2   g112(.a(new_n174_), .b(new_n172_), .O(z17));
  oai21  g113(.a(new_n89_), .b(x27), .c(new_n128_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x08), .O(new_n177_));
  nor2   g115(.a(x32), .b(x30), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n177_), .c(new_n132_), .d(new_n127_), .O(z18));
  nand4  g117(.a(new_n134_), .b(new_n120_), .c(new_n79_), .d(new_n127_), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z19));
  inv1   g119(.a(new_n128_), .O(new_n182_));
  oai21  g120(.a(new_n66_), .b(new_n64_), .c(new_n131_), .O(new_n183_));
  aoi21  g121(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n182_), .c(new_n183_), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n120_), .c(new_n79_), .d(new_n127_), .O(new_n186_));
  inv1   g124(.a(new_n186_), .O(z20));
endmodule


