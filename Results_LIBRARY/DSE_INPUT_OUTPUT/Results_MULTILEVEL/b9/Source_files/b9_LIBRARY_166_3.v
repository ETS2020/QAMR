// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_;
  nand2  g000(.a(x40), .b(x06), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x27), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  inv1   g010(.a(x06), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n65_), .d(new_n73_), .O(new_n78_));
  oai22  g016(.a(new_n78_), .b(x04), .c(new_n72_), .d(new_n64_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n64_), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  inv1   g025(.a(new_n83_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x04), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x39), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n87_), .c(new_n73_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x40), .O(z02));
  nand2  g031(.a(new_n68_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x27), .O(new_n95_));
  nand2  g033(.a(new_n67_), .b(new_n95_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n94_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g035(.a(new_n67_), .b(new_n95_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n94_), .c(new_n64_), .O(z04));
  inv1   g037(.a(x28), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n95_), .O(new_n101_));
  inv1   g039(.a(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n63_), .c(new_n67_), .O(z05));
  oai21  g041(.a(new_n101_), .b(x37), .c(new_n63_), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n65_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n63_), .O(z07));
  nand3  g053(.a(x40), .b(x39), .c(new_n73_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n63_), .O(z09));
  nor2   g057(.a(new_n83_), .b(x36), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(x05), .O(new_n122_));
  oai21  g060(.a(x39), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n73_), .O(new_n124_));
  nand2  g062(.a(new_n75_), .b(x07), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n120_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  nand2  g065(.a(new_n81_), .b(new_n80_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x40), .c(x39), .d(new_n73_), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  aoi21  g068(.a(new_n126_), .b(x27), .c(new_n130_), .O(new_n131_));
  nand4  g069(.a(new_n75_), .b(x37), .c(x27), .d(x06), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(x04), .c(new_n132_), .O(z10));
  inv1   g071(.a(x08), .O(new_n134_));
  inv1   g072(.a(x09), .O(new_n135_));
  nand4  g073(.a(new_n80_), .b(new_n135_), .c(new_n134_), .d(x04), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n73_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x40), .O(new_n138_));
  nand3  g076(.a(x40), .b(x39), .c(x29), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n83_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  oai21  g080(.a(new_n88_), .b(new_n95_), .c(new_n134_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n80_), .c(new_n135_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n138_), .O(z11));
  nor3   g084(.a(x37), .b(x36), .c(x35), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n95_), .c(new_n74_), .O(new_n148_));
  inv1   g086(.a(x04), .O(new_n149_));
  nand3  g087(.a(x40), .b(x39), .c(new_n149_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n70_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n148_), .c(new_n63_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n75_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n149_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n69_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n156_), .c(x06), .O(new_n160_));
  nand4  g098(.a(new_n69_), .b(new_n75_), .c(x20), .d(new_n158_), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(x18), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n160_), .c(x27), .O(new_n163_));
  inv1   g101(.a(x39), .O(new_n164_));
  nand2  g102(.a(new_n128_), .b(x40), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n153_), .c(new_n73_), .d(new_n149_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n163_), .O(z13));
  nand3  g106(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n169_));
  nand4  g107(.a(new_n154_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n170_));
  nand3  g108(.a(x39), .b(new_n153_), .c(new_n149_), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(new_n170_), .c(x06), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n75_), .c(new_n169_), .O(new_n174_));
  nand3  g112(.a(new_n67_), .b(new_n82_), .c(x28), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x27), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n81_), .c(new_n80_), .O(new_n177_));
  oai21  g115(.a(new_n172_), .b(new_n71_), .c(new_n177_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n73_), .O(new_n179_));
  nand2  g117(.a(new_n70_), .b(new_n75_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n179_), .c(new_n174_), .O(z14));
  inv1   g119(.a(x12), .O(new_n182_));
  nand4  g120(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n182_), .O(z15));
  inv1   g122(.a(x23), .O(new_n185_));
  nand4  g123(.a(new_n63_), .b(new_n185_), .c(x22), .d(x01), .O(new_n186_));
  inv1   g124(.a(new_n186_), .O(z16));
  inv1   g125(.a(x01), .O(new_n188_));
  inv1   g126(.a(x24), .O(new_n189_));
  nand4  g127(.a(new_n63_), .b(new_n189_), .c(x23), .d(x22), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n188_), .O(z17));
  oai21  g129(.a(new_n88_), .b(x27), .c(new_n139_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(x08), .O(new_n193_));
  nor2   g131(.a(new_n95_), .b(x04), .O(new_n194_));
  aoi21  g132(.a(new_n194_), .b(new_n83_), .c(x30), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n193_), .c(new_n63_), .d(new_n135_), .O(z18));
  nand2  g134(.a(x40), .b(x04), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(x35), .c(new_n100_), .O(new_n198_));
  aoi22  g136(.a(new_n198_), .b(new_n73_), .c(new_n63_), .d(new_n95_), .O(new_n199_));
  aoi21  g137(.a(x39), .b(x29), .c(x06), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n75_), .c(new_n141_), .O(new_n201_));
  oai21  g139(.a(new_n199_), .b(x08), .c(new_n201_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n80_), .c(new_n135_), .O(new_n203_));
  inv1   g141(.a(new_n203_), .O(z19));
  inv1   g142(.a(new_n139_), .O(new_n205_));
  oai21  g143(.a(new_n82_), .b(new_n95_), .c(new_n134_), .O(new_n206_));
  aoi21  g144(.a(x28), .b(new_n95_), .c(new_n82_), .O(new_n207_));
  oai21  g145(.a(new_n207_), .b(new_n205_), .c(new_n206_), .O(new_n208_));
  nand3  g146(.a(new_n208_), .b(new_n80_), .c(new_n135_), .O(new_n209_));
  nand2  g147(.a(new_n209_), .b(new_n63_), .O(z20));
endmodule


