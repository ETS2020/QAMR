// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n199_, new_n200_, new_n201_,
    new_n202_;
  nand2  g000(.a(x30), .b(x04), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n79_), .c(new_n67_), .O(new_n82_));
  nor2   g020(.a(x30), .b(new_n72_), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(x32), .c(new_n83_), .O(z01));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n80_), .O(new_n88_));
  inv1   g026(.a(x30), .O(new_n89_));
  nand2  g027(.a(x35), .b(new_n89_), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n88_), .c(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(new_n87_), .O(z02));
  nand2  g032(.a(new_n69_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n68_), .b(new_n67_), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n95_), .c(new_n64_), .O(z03));
  aoi21  g036(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n64_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n68_), .c(new_n64_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x33), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n65_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n106_), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  oai21  g050(.a(new_n108_), .b(x31), .c(new_n112_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n63_), .c(x03), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z07));
  nand2  g053(.a(new_n92_), .b(new_n63_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n72_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x30), .O(new_n122_));
  nand2  g060(.a(new_n81_), .b(new_n79_), .O(new_n123_));
  nand2  g061(.a(new_n92_), .b(x07), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n120_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n123_), .c(x27), .O(new_n126_));
  nand3  g064(.a(new_n93_), .b(x32), .c(x05), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n72_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n122_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(new_n134_));
  nand2  g072(.a(new_n81_), .b(new_n89_), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n135_), .c(new_n134_), .O(new_n137_));
  nand3  g075(.a(x35), .b(new_n80_), .c(x27), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n89_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n72_), .c(x08), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n137_), .c(new_n132_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n63_), .O(z11));
  inv1   g080(.a(x35), .O(new_n143_));
  nand3  g081(.a(new_n68_), .b(new_n79_), .c(new_n143_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  nor2   g083(.a(new_n93_), .b(new_n70_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(new_n72_), .O(new_n147_));
  inv1   g085(.a(new_n70_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n89_), .c(x04), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n74_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n72_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand2  g094(.a(new_n69_), .b(new_n68_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n154_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x27), .O(new_n160_));
  inv1   g098(.a(x32), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(x04), .c(new_n89_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x40), .c(x39), .d(new_n151_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n160_), .c(new_n63_), .O(z13));
  nand3  g102(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n165_));
  nand3  g103(.a(new_n152_), .b(new_n161_), .c(x28), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n72_), .c(x30), .O(new_n167_));
  aoi21  g105(.a(new_n93_), .b(new_n151_), .c(x04), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n167_), .c(new_n165_), .O(new_n169_));
  aoi21  g107(.a(x32), .b(new_n72_), .c(x27), .O(new_n170_));
  nand2  g108(.a(new_n69_), .b(x04), .O(new_n171_));
  nand3  g109(.a(new_n143_), .b(new_n161_), .c(x28), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(new_n171_), .c(x37), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n170_), .c(new_n89_), .O(new_n174_));
  nand2  g112(.a(new_n93_), .b(new_n151_), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n148_), .c(new_n72_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n174_), .c(new_n169_), .O(z14));
  inv1   g115(.a(x12), .O(new_n178_));
  nor2   g116(.a(new_n118_), .b(new_n178_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(x22), .c(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n63_), .O(z16));
  inv1   g120(.a(x24), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(x23), .c(x22), .d(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n63_), .O(z17));
  nand2  g123(.a(new_n139_), .b(new_n72_), .O(new_n186_));
  inv1   g124(.a(x08), .O(new_n187_));
  nand3  g125(.a(x35), .b(new_n80_), .c(new_n67_), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n133_), .c(new_n187_), .O(new_n189_));
  oai21  g127(.a(new_n189_), .b(x09), .c(new_n89_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n186_), .O(z18));
  nand3  g129(.a(new_n136_), .b(x35), .c(new_n80_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n133_), .O(new_n193_));
  nand2  g131(.a(x27), .b(new_n72_), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n81_), .c(new_n187_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n193_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n89_), .c(new_n132_), .O(new_n197_));
  inv1   g135(.a(new_n197_), .O(z19));
  oai21  g136(.a(new_n143_), .b(new_n67_), .c(new_n187_), .O(new_n199_));
  aoi21  g137(.a(x28), .b(new_n67_), .c(new_n143_), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n134_), .c(new_n199_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n89_), .c(new_n132_), .O(new_n202_));
  inv1   g140(.a(new_n202_), .O(z20));
endmodule


