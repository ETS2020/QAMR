// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n184_, new_n185_, new_n186_;
  nand2  g000(.a(x40), .b(x29), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x29), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  nand2  g024(.a(new_n67_), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n82_), .O(new_n88_));
  nand2  g026(.a(x39), .b(x02), .O(new_n89_));
  aoi21  g027(.a(new_n88_), .b(x04), .c(new_n89_), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(x29), .c(x40), .O(z02));
  nand2  g029(.a(new_n69_), .b(x27), .O(new_n92_));
  nand2  g030(.a(new_n68_), .b(new_n67_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n92_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g032(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n92_), .c(new_n64_), .O(z04));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n68_), .c(new_n64_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x33), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n65_), .O(new_n101_));
  inv1   g039(.a(x00), .O(new_n102_));
  oai21  g040(.a(x25), .b(new_n102_), .c(x38), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand3  g044(.a(x38), .b(new_n106_), .c(new_n102_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  oai21  g046(.a(new_n104_), .b(x31), .c(new_n108_), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n63_), .O(z07));
  oai21  g049(.a(x39), .b(x29), .c(x40), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n63_), .O(z09));
  inv1   g053(.a(x04), .O(new_n116_));
  inv1   g054(.a(x28), .O(new_n117_));
  aoi21  g055(.a(x35), .b(new_n117_), .c(x36), .O(new_n118_));
  nand2  g056(.a(new_n112_), .b(x07), .O(new_n119_));
  nand4  g057(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nand3  g059(.a(new_n63_), .b(x37), .c(x06), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n116_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(x39), .O(new_n125_));
  nand2  g063(.a(new_n80_), .b(new_n79_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x40), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(new_n72_), .c(x05), .d(new_n116_), .O(new_n129_));
  oai21  g067(.a(new_n124_), .b(new_n67_), .c(new_n129_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  nand2  g069(.a(x27), .b(x04), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n87_), .c(new_n82_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n79_), .c(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n63_), .O(z11));
  nor3   g073(.a(x37), .b(x36), .c(x35), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n67_), .c(new_n73_), .O(new_n137_));
  inv1   g075(.a(new_n70_), .O(new_n138_));
  nor2   g076(.a(new_n74_), .b(new_n125_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n116_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n137_), .c(new_n63_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x28), .c(new_n74_), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(x39), .c(new_n143_), .d(new_n116_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand2  g086(.a(new_n69_), .b(new_n68_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n146_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x27), .O(new_n152_));
  nand4  g090(.a(new_n126_), .b(x39), .c(new_n143_), .d(new_n116_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n72_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x40), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(z13));
  nand3  g094(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n157_));
  nand4  g095(.a(new_n144_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  nand3  g097(.a(new_n139_), .b(new_n143_), .c(new_n116_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n157_), .O(new_n161_));
  nand3  g099(.a(new_n68_), .b(new_n81_), .c(x28), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n80_), .c(new_n79_), .O(new_n164_));
  aoi21  g102(.a(new_n160_), .b(new_n138_), .c(new_n64_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n164_), .c(new_n161_), .O(z14));
  inv1   g104(.a(x12), .O(new_n167_));
  nand4  g105(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(new_n167_), .O(z15));
  inv1   g107(.a(x23), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(x22), .c(x01), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n63_), .O(z16));
  inv1   g110(.a(x01), .O(new_n173_));
  inv1   g111(.a(x24), .O(new_n174_));
  nand4  g112(.a(new_n63_), .b(new_n174_), .c(x23), .d(x22), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n173_), .O(z17));
  nand2  g114(.a(x27), .b(new_n116_), .O(new_n177_));
  oai21  g115(.a(x27), .b(new_n86_), .c(new_n177_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(x35), .c(new_n117_), .O(new_n179_));
  nor2   g117(.a(x30), .b(x09), .O(new_n180_));
  aoi21  g118(.a(new_n180_), .b(new_n179_), .c(new_n64_), .O(z18));
  nand4  g119(.a(new_n133_), .b(new_n63_), .c(new_n79_), .d(new_n131_), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z19));
  oai21  g121(.a(x28), .b(new_n86_), .c(new_n67_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x35), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n79_), .c(new_n131_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n63_), .O(z20));
endmodule


