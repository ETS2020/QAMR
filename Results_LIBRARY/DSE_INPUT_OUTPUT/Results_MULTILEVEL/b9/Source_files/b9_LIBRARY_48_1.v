// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n165_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x30), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n63_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x39), .c(x30), .d(new_n65_), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x27), .c(x32), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x40), .c(new_n77_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(z01));
  and2   g021(.a(x29), .b(x08), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  inv1   g023(.a(x08), .O(new_n86_));
  nand2  g024(.a(new_n67_), .b(new_n86_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(x35), .c(new_n85_), .O(new_n88_));
  nand3  g026(.a(x40), .b(x39), .c(x30), .O(new_n89_));
  aoi21  g027(.a(new_n88_), .b(x04), .c(new_n89_), .O(new_n90_));
  oai21  g028(.a(new_n84_), .b(x02), .c(new_n90_), .O(z02));
  nand2  g029(.a(new_n69_), .b(x27), .O(new_n92_));
  aoi21  g030(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(z04));
  inv1   g034(.a(new_n64_), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n97_), .c(new_n68_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x31), .O(new_n101_));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n65_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand4  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n104_), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n106_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n97_), .c(x03), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z07));
  inv1   g051(.a(x39), .O(new_n114_));
  aoi21  g052(.a(new_n114_), .b(x30), .c(new_n63_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n97_), .O(z09));
  inv1   g055(.a(x04), .O(new_n118_));
  nand4  g056(.a(new_n89_), .b(new_n80_), .c(x27), .d(x07), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nor2   g060(.a(new_n68_), .b(new_n67_), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(x06), .c(new_n64_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(z10));
  inv1   g063(.a(x09), .O(new_n126_));
  nand2  g064(.a(x27), .b(x04), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(new_n87_), .c(x35), .d(new_n85_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(new_n63_), .c(new_n77_), .d(new_n126_), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(z11));
  nor3   g068(.a(x37), .b(x36), .c(x35), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n67_), .c(new_n72_), .O(new_n132_));
  inv1   g070(.a(new_n70_), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(new_n118_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n132_), .c(new_n97_), .O(z12));
  inv1   g074(.a(x18), .O(new_n137_));
  inv1   g075(.a(x19), .O(new_n138_));
  nand4  g076(.a(new_n70_), .b(x20), .c(new_n138_), .d(new_n137_), .O(new_n139_));
  inv1   g077(.a(x13), .O(new_n140_));
  nand3  g078(.a(x39), .b(new_n140_), .c(new_n118_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x30), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x40), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n139_), .O(z13));
  nand4  g082(.a(x27), .b(x20), .c(new_n138_), .d(new_n137_), .O(new_n145_));
  aoi21  g083(.a(new_n69_), .b(new_n68_), .c(new_n145_), .O(new_n146_));
  aoi21  g084(.a(new_n142_), .b(x40), .c(new_n146_), .O(z14));
  inv1   g085(.a(x12), .O(new_n148_));
  nand4  g086(.a(new_n97_), .b(x34), .c(x27), .d(x26), .O(new_n149_));
  nor2   g087(.a(new_n149_), .b(new_n148_), .O(z15));
  inv1   g088(.a(x23), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(x22), .c(x01), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n97_), .O(z16));
  inv1   g091(.a(x01), .O(new_n154_));
  inv1   g092(.a(x24), .O(new_n155_));
  nand4  g093(.a(new_n97_), .b(new_n155_), .c(x23), .d(x22), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(new_n154_), .O(z17));
  nand2  g095(.a(x27), .b(new_n118_), .O(new_n158_));
  nand2  g096(.a(new_n67_), .b(x08), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n158_), .c(new_n78_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n85_), .c(x09), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(x40), .c(new_n77_), .O(z18));
  oai21  g100(.a(x28), .b(new_n86_), .c(new_n67_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x35), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n63_), .c(new_n77_), .d(new_n126_), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z20));
  inv1   g104(.a(new_n129_), .O(z19));
endmodule


