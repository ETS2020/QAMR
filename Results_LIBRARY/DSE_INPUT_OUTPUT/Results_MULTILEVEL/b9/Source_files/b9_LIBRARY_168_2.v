// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n184_, new_n185_, new_n186_, new_n187_;
  inv1   g000(.a(x04), .O(new_n63_));
  nor2   g001(.a(x30), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x27), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n63_), .O(new_n77_));
  oai21  g015(.a(new_n72_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(z01));
  and2   g019(.a(x29), .b(x08), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  inv1   g021(.a(x08), .O(new_n84_));
  inv1   g022(.a(x27), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g024(.a(new_n86_), .b(x35), .c(x30), .d(new_n83_), .O(new_n87_));
  nand2  g025(.a(x40), .b(x39), .O(new_n88_));
  aoi21  g026(.a(new_n87_), .b(x04), .c(new_n88_), .O(new_n89_));
  oai21  g027(.a(new_n82_), .b(x02), .c(new_n89_), .O(z02));
  nand2  g028(.a(new_n68_), .b(x27), .O(new_n91_));
  aoi21  g029(.a(new_n67_), .b(new_n85_), .c(new_n64_), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  nand3  g032(.a(new_n92_), .b(new_n91_), .c(new_n94_), .O(z04));
  inv1   g033(.a(new_n64_), .O(new_n96_));
  oai21  g034(.a(new_n83_), .b(new_n85_), .c(new_n67_), .O(new_n97_));
  and2   g035(.a(new_n97_), .b(new_n96_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x33), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n65_), .O(new_n101_));
  inv1   g039(.a(x00), .O(new_n102_));
  oai21  g040(.a(x25), .b(new_n102_), .c(x38), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(new_n107_));
  oai22  g045(.a(new_n107_), .b(x14), .c(new_n104_), .d(x31), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n96_), .O(z07));
  inv1   g048(.a(x39), .O(new_n111_));
  nor3   g049(.a(new_n64_), .b(new_n74_), .c(new_n111_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n96_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(x35), .O(new_n116_));
  inv1   g054(.a(x36), .O(new_n117_));
  oai21  g055(.a(new_n116_), .b(x28), .c(new_n117_), .O(new_n118_));
  nand2  g056(.a(new_n88_), .b(x07), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(x04), .O(new_n124_));
  nand3  g062(.a(x37), .b(x30), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n124_), .c(x27), .O(new_n127_));
  oai21  g065(.a(x32), .b(x30), .c(x40), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n111_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x05), .c(new_n63_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nor2   g070(.a(new_n116_), .b(x28), .O(new_n133_));
  inv1   g071(.a(new_n88_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n82_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n133_), .c(new_n86_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n132_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n63_), .c(x30), .O(z11));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n85_), .c(new_n73_), .O(new_n141_));
  oai21  g079(.a(new_n88_), .b(x04), .c(new_n70_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n96_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x28), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(x40), .c(x39), .d(new_n144_), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand4  g087(.a(new_n69_), .b(x20), .c(new_n149_), .d(new_n148_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n147_), .c(x04), .O(new_n151_));
  nand4  g089(.a(new_n69_), .b(x30), .c(x20), .d(new_n149_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(x18), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n151_), .c(x27), .O(new_n154_));
  nand3  g092(.a(new_n129_), .b(new_n144_), .c(new_n63_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z13));
  nand3  g094(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n157_));
  inv1   g095(.a(x32), .O(new_n158_));
  nand4  g096(.a(new_n145_), .b(new_n158_), .c(new_n79_), .d(x28), .O(new_n159_));
  nor2   g097(.a(new_n88_), .b(x13), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n159_), .c(x04), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n80_), .c(new_n157_), .O(new_n162_));
  nand3  g100(.a(new_n67_), .b(new_n116_), .c(x28), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n158_), .c(new_n79_), .O(new_n165_));
  oai21  g103(.a(new_n160_), .b(new_n71_), .c(new_n165_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n63_), .O(new_n167_));
  nand3  g105(.a(new_n70_), .b(x30), .c(x04), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n167_), .c(new_n162_), .O(z14));
  inv1   g107(.a(x12), .O(new_n170_));
  nor2   g108(.a(new_n114_), .b(new_n170_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(x22), .c(x01), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n96_), .O(z16));
  inv1   g112(.a(x01), .O(new_n175_));
  inv1   g113(.a(x24), .O(new_n176_));
  nand4  g114(.a(new_n96_), .b(new_n176_), .c(x23), .d(x22), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z17));
  nand2  g116(.a(new_n134_), .b(x29), .O(new_n179_));
  inv1   g117(.a(new_n179_), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n133_), .c(x08), .O(new_n181_));
  aoi21  g119(.a(new_n133_), .b(x27), .c(x30), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(new_n181_), .c(new_n132_), .d(new_n63_), .O(z18));
  oai21  g121(.a(new_n116_), .b(new_n85_), .c(new_n84_), .O(new_n184_));
  aoi21  g122(.a(x28), .b(new_n85_), .c(new_n116_), .O(new_n185_));
  oai21  g123(.a(new_n185_), .b(new_n180_), .c(new_n184_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n132_), .O(new_n187_));
  aoi21  g125(.a(new_n187_), .b(new_n63_), .c(x30), .O(z20));
  aoi21  g126(.a(new_n138_), .b(new_n63_), .c(x30), .O(z19));
endmodule


