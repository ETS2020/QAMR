// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x27), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  inv1   g005(.a(x28), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x04), .c(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x35), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x40), .c(x39), .d(new_n72_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n71_), .c(new_n67_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x27), .c(new_n66_), .O(new_n78_));
  oai21  g016(.a(new_n65_), .b(x16), .c(new_n78_), .O(z00));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(x40), .c(new_n63_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n68_), .O(new_n84_));
  nand4  g022(.a(new_n84_), .b(new_n74_), .c(new_n83_), .d(new_n82_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n81_), .c(x04), .O(z01));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nand2  g025(.a(new_n84_), .b(x04), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x39), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n87_), .c(x27), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x40), .O(z02));
  nand2  g029(.a(x35), .b(x28), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x27), .O(new_n93_));
  inv1   g031(.a(x40), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x37), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n63_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n93_), .c(x21), .O(z03));
  nand2  g035(.a(new_n64_), .b(x21), .O(new_n98_));
  nand3  g036(.a(new_n94_), .b(new_n67_), .c(new_n63_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(new_n93_), .O(z04));
  oai21  g038(.a(x37), .b(x28), .c(x27), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n95_), .O(z05));
  nand2  g040(.a(new_n68_), .b(x27), .O(new_n103_));
  nor2   g041(.a(x40), .b(x27), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n103_), .c(x37), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x14), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n109_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  oai21  g053(.a(new_n111_), .b(x31), .c(new_n115_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n64_), .O(z07));
  inv1   g056(.a(x39), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(x27), .c(new_n94_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n64_), .O(z09));
  nand3  g060(.a(new_n69_), .b(x27), .c(x07), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x05), .O(new_n124_));
  aoi22  g062(.a(new_n124_), .b(new_n123_), .c(new_n84_), .d(new_n74_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  oai21  g064(.a(x32), .b(x30), .c(x40), .O(new_n127_));
  nor3   g065(.a(new_n127_), .b(new_n119_), .c(new_n126_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n125_), .c(new_n72_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n64_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nand3  g070(.a(x35), .b(new_n68_), .c(new_n72_), .O(new_n133_));
  nand3  g071(.a(x39), .b(x29), .c(x08), .O(new_n134_));
  nor2   g072(.a(x40), .b(new_n72_), .O(new_n135_));
  aoi21  g073(.a(new_n134_), .b(new_n133_), .c(new_n135_), .O(new_n136_));
  nor2   g074(.a(x27), .b(x08), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n84_), .c(new_n94_), .O(new_n138_));
  oai21  g076(.a(new_n136_), .b(new_n63_), .c(new_n138_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n82_), .c(new_n132_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z11));
  oai21  g079(.a(new_n119_), .b(x04), .c(x27), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x40), .O(new_n143_));
  inv1   g081(.a(x35), .O(new_n144_));
  nand4  g082(.a(new_n74_), .b(new_n144_), .c(x27), .d(new_n73_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  aoi21  g084(.a(new_n143_), .b(new_n92_), .c(new_n146_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(x37), .c(new_n105_), .O(z12));
  inv1   g086(.a(x18), .O(new_n149_));
  inv1   g087(.a(x19), .O(new_n150_));
  aoi21  g088(.a(new_n92_), .b(new_n67_), .c(new_n63_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x20), .c(new_n150_), .d(new_n149_), .O(new_n152_));
  inv1   g090(.a(x13), .O(new_n153_));
  aoi21  g091(.a(x36), .b(x35), .c(x32), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n82_), .c(x28), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n72_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x40), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n152_), .O(z13));
  nand3  g097(.a(x20), .b(new_n150_), .c(new_n149_), .O(new_n160_));
  inv1   g098(.a(new_n69_), .O(new_n161_));
  nand3  g099(.a(new_n154_), .b(new_n82_), .c(x28), .O(new_n162_));
  nor2   g100(.a(x13), .b(x04), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n162_), .c(new_n161_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n160_), .O(new_n165_));
  aoi22  g103(.a(new_n163_), .b(new_n161_), .c(x35), .d(x28), .O(new_n166_));
  nand4  g104(.a(new_n144_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(new_n67_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n165_), .c(x27), .O(z14));
  nand4  g108(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z15));
  inv1   g110(.a(x23), .O(new_n173_));
  nand4  g111(.a(new_n64_), .b(new_n173_), .c(x22), .d(x01), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(z16));
  inv1   g113(.a(x01), .O(new_n176_));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n64_), .b(new_n177_), .c(x23), .d(x22), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n176_), .O(z17));
  inv1   g117(.a(x29), .O(new_n180_));
  oai22  g118(.a(new_n84_), .b(x27), .c(new_n69_), .d(new_n180_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(x08), .O(new_n182_));
  inv1   g120(.a(new_n84_), .O(new_n183_));
  nor2   g121(.a(new_n63_), .b(x04), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(x30), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n182_), .c(new_n64_), .d(new_n132_), .O(z18));
  inv1   g124(.a(x08), .O(new_n187_));
  nor2   g125(.a(x35), .b(new_n63_), .O(new_n188_));
  oai21  g126(.a(new_n188_), .b(new_n104_), .c(new_n187_), .O(new_n189_));
  aoi21  g127(.a(x39), .b(x29), .c(new_n63_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n94_), .c(new_n144_), .O(new_n191_));
  nand3  g129(.a(new_n94_), .b(x28), .c(new_n63_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n191_), .c(new_n189_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n82_), .c(new_n132_), .O(new_n194_));
  inv1   g132(.a(new_n194_), .O(z20));
  inv1   g133(.a(new_n140_), .O(z19));
endmodule


