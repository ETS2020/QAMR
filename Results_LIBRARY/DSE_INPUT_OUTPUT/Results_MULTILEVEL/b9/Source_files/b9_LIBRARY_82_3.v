// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n180_, new_n181_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x29), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x15), .O(new_n71_));
  inv1   g009(.a(x29), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x04), .c(new_n70_), .O(z00));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g024(.a(x08), .O(new_n87_));
  inv1   g025(.a(x27), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n83_), .O(new_n90_));
  nand2  g028(.a(x39), .b(x02), .O(new_n91_));
  aoi21  g029(.a(new_n90_), .b(x04), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x29), .c(x40), .O(z02));
  nand2  g031(.a(new_n66_), .b(x27), .O(new_n94_));
  aoi21  g032(.a(new_n65_), .b(new_n88_), .c(new_n79_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n64_), .c(new_n65_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n71_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  oai22  g047(.a(new_n109_), .b(x14), .c(new_n106_), .d(x31), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n64_), .O(z07));
  oai21  g050(.a(x39), .b(x29), .c(x40), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z09));
  inv1   g054(.a(x04), .O(new_n117_));
  inv1   g055(.a(x28), .O(new_n118_));
  aoi21  g056(.a(x35), .b(new_n118_), .c(x36), .O(new_n119_));
  nand2  g057(.a(new_n113_), .b(x07), .O(new_n120_));
  inv1   g058(.a(x39), .O(new_n121_));
  nor2   g059(.a(new_n74_), .b(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n72_), .c(x05), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n120_), .c(new_n119_), .O(new_n124_));
  nand3  g062(.a(new_n64_), .b(x37), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n117_), .c(new_n126_), .O(new_n127_));
  oai21  g065(.a(x32), .b(x30), .c(x40), .O(new_n128_));
  nor3   g066(.a(new_n128_), .b(new_n121_), .c(x29), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x05), .c(new_n117_), .O(new_n130_));
  oai21  g068(.a(new_n127_), .b(new_n88_), .c(new_n130_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nand2  g070(.a(x27), .b(x04), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n89_), .c(new_n83_), .O(new_n134_));
  nand4  g072(.a(new_n134_), .b(new_n64_), .c(new_n80_), .d(new_n132_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(z11));
  nor3   g074(.a(x37), .b(x36), .c(x35), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n88_), .c(new_n73_), .O(new_n138_));
  nand2  g076(.a(new_n122_), .b(new_n117_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n68_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n138_), .c(new_n64_), .O(z12));
  inv1   g079(.a(x13), .O(new_n142_));
  nand2  g080(.a(x36), .b(x35), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x28), .c(new_n74_), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(x39), .c(new_n142_), .d(new_n117_), .O(new_n145_));
  inv1   g083(.a(x18), .O(new_n146_));
  inv1   g084(.a(x19), .O(new_n147_));
  nand4  g085(.a(new_n67_), .b(x20), .c(new_n147_), .d(new_n146_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n145_), .c(x29), .O(new_n149_));
  nand4  g087(.a(new_n67_), .b(new_n74_), .c(x20), .d(new_n147_), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(x18), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(x27), .O(new_n152_));
  nand3  g090(.a(new_n129_), .b(new_n142_), .c(new_n117_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(z13));
  nand3  g092(.a(x20), .b(new_n147_), .c(new_n146_), .O(new_n155_));
  nand4  g093(.a(new_n143_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(new_n157_));
  nand3  g095(.a(new_n122_), .b(new_n142_), .c(new_n117_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n157_), .c(new_n155_), .O(new_n159_));
  nand3  g097(.a(new_n65_), .b(new_n82_), .c(x28), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n81_), .c(new_n80_), .O(new_n162_));
  aoi21  g100(.a(new_n158_), .b(new_n68_), .c(new_n79_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n162_), .c(new_n159_), .O(z14));
  nand4  g102(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n64_), .O(z15));
  inv1   g104(.a(x23), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(x22), .c(x01), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n64_), .O(z16));
  inv1   g107(.a(x01), .O(new_n170_));
  inv1   g108(.a(x24), .O(new_n171_));
  nand4  g109(.a(new_n64_), .b(new_n171_), .c(x23), .d(x22), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(new_n170_), .O(z17));
  nand2  g111(.a(x27), .b(new_n117_), .O(new_n174_));
  oai21  g112(.a(x27), .b(new_n87_), .c(new_n174_), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(x35), .c(new_n118_), .O(new_n176_));
  nor2   g114(.a(x30), .b(x09), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n176_), .c(new_n79_), .O(z18));
  oai21  g116(.a(x28), .b(new_n87_), .c(new_n88_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x35), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n64_), .c(new_n80_), .d(new_n132_), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z20));
  inv1   g120(.a(new_n135_), .O(z19));
endmodule


