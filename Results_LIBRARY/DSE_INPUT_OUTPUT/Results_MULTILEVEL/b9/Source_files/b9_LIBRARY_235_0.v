// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_;
  inv1   g000(.a(x16), .O(new_n63_));
  nor2   g001(.a(x40), .b(x27), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  inv1   g007(.a(x35), .O(new_n70_));
  inv1   g008(.a(x36), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand4  g010(.a(new_n72_), .b(x40), .c(x39), .d(new_n69_), .O(new_n73_));
  inv1   g011(.a(x37), .O(new_n74_));
  nand2  g012(.a(x35), .b(x28), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n73_), .c(new_n68_), .O(new_n78_));
  nand4  g016(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n78_), .c(new_n67_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n66_), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n71_), .O(new_n87_));
  nand2  g025(.a(x40), .b(new_n68_), .O(new_n88_));
  oai21  g026(.a(new_n87_), .b(new_n68_), .c(new_n88_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n84_), .c(new_n83_), .O(new_n90_));
  oai21  g028(.a(new_n64_), .b(x04), .c(new_n90_), .O(z01));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n86_), .c(x04), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x39), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x40), .O(new_n98_));
  oai21  g036(.a(x40), .b(new_n68_), .c(new_n98_), .O(z02));
  nand2  g037(.a(new_n75_), .b(x27), .O(new_n100_));
  nand2  g038(.a(x40), .b(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n68_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x21), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  nand3  g042(.a(new_n102_), .b(new_n100_), .c(new_n104_), .O(z04));
  oai21  g043(.a(x37), .b(x28), .c(x27), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n101_), .O(z05));
  nand2  g045(.a(new_n85_), .b(x27), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n88_), .c(x37), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n67_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x14), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n112_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  oai21  g056(.a(new_n114_), .b(x31), .c(new_n118_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n65_), .O(z07));
  nand2  g059(.a(x40), .b(x39), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n65_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(z09));
  nand2  g063(.a(new_n122_), .b(x07), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x05), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n87_), .c(x27), .O(new_n129_));
  nand2  g067(.a(new_n84_), .b(new_n83_), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(x40), .c(x39), .d(x05), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n69_), .O(new_n133_));
  nand3  g071(.a(x37), .b(x27), .c(x06), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(z10));
  inv1   g073(.a(x09), .O(new_n136_));
  inv1   g074(.a(new_n95_), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  nor2   g076(.a(new_n68_), .b(new_n69_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n86_), .c(new_n138_), .O(new_n140_));
  inv1   g078(.a(x08), .O(new_n141_));
  inv1   g079(.a(x40), .O(new_n142_));
  oai22  g080(.a(new_n86_), .b(x04), .c(new_n142_), .d(new_n141_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n140_), .c(new_n137_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n83_), .c(new_n136_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n65_), .O(z11));
  inv1   g084(.a(x10), .O(new_n147_));
  nand4  g085(.a(new_n74_), .b(new_n71_), .c(new_n70_), .d(x27), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n88_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  inv1   g088(.a(x39), .O(new_n151_));
  oai21  g089(.a(new_n76_), .b(new_n68_), .c(new_n88_), .O(new_n152_));
  oai21  g090(.a(new_n151_), .b(x04), .c(new_n152_), .O(new_n153_));
  nand4  g091(.a(new_n75_), .b(new_n142_), .c(new_n74_), .d(x27), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n153_), .c(new_n150_), .O(z12));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(x36), .b(x35), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x28), .c(new_n142_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x39), .c(new_n156_), .d(new_n69_), .O(new_n159_));
  inv1   g097(.a(x18), .O(new_n160_));
  inv1   g098(.a(x19), .O(new_n161_));
  nand4  g099(.a(new_n76_), .b(x20), .c(new_n161_), .d(new_n160_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  aoi21  g102(.a(new_n84_), .b(new_n83_), .c(new_n142_), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(x39), .c(new_n156_), .d(new_n69_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n164_), .O(z13));
  nand3  g105(.a(x20), .b(new_n161_), .c(new_n160_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  nand4  g107(.a(new_n157_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n170_));
  nand4  g108(.a(x40), .b(x39), .c(new_n156_), .d(new_n69_), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(new_n170_), .c(new_n169_), .O(new_n173_));
  nand2  g111(.a(new_n171_), .b(new_n75_), .O(new_n174_));
  nand4  g112(.a(new_n70_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n174_), .c(x37), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n173_), .c(x27), .O(new_n177_));
  nand4  g115(.a(new_n130_), .b(x39), .c(new_n156_), .d(new_n69_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(x40), .c(new_n68_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n177_), .O(z14));
  nand4  g118(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand4  g121(.a(new_n65_), .b(new_n183_), .c(x22), .d(x01), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(z16));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(x23), .c(x22), .d(x01), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n65_), .O(z17));
  nor2   g126(.a(new_n70_), .b(x28), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n68_), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(new_n138_), .c(new_n141_), .O(new_n191_));
  nand2  g129(.a(new_n83_), .b(new_n136_), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n191_), .c(x40), .O(new_n193_));
  nand2  g131(.a(new_n189_), .b(new_n69_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n83_), .c(new_n136_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(x27), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n193_), .O(z18));
  oai21  g135(.a(new_n70_), .b(new_n68_), .c(new_n141_), .O(new_n198_));
  oai21  g136(.a(new_n85_), .b(x27), .c(x35), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n138_), .O(new_n200_));
  nand2  g138(.a(new_n142_), .b(new_n70_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n200_), .c(new_n198_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n83_), .c(new_n136_), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n65_), .O(z20));
  nand2  g142(.a(new_n145_), .b(new_n65_), .O(z19));
endmodule


