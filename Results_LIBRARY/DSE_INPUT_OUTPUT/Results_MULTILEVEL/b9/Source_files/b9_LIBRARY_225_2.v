// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n186_, new_n187_, new_n188_,
    new_n189_;
  inv1   g000(.a(x04), .O(new_n63_));
  nor2   g001(.a(x30), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n72_), .c(new_n73_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n65_), .d(new_n63_), .O(new_n76_));
  oai21  g014(.a(new_n71_), .b(new_n64_), .c(new_n76_), .O(z00));
  nand2  g015(.a(x30), .b(x04), .O(z01));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  aoi21  g017(.a(x29), .b(x08), .c(x02), .O(new_n80_));
  nand2  g018(.a(x40), .b(x39), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  inv1   g020(.a(x08), .O(new_n83_));
  nand2  g021(.a(new_n67_), .b(new_n83_), .O(new_n84_));
  inv1   g022(.a(x35), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(x28), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(x30), .c(x04), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n82_), .O(z02));
  nand2  g027(.a(new_n69_), .b(x27), .O(new_n90_));
  nand2  g028(.a(new_n68_), .b(new_n67_), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n90_), .c(new_n79_), .d(x21), .O(z03));
  aoi21  g030(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n90_), .c(new_n64_), .O(z04));
  inv1   g032(.a(x28), .O(new_n95_));
  nor2   g033(.a(new_n95_), .b(new_n67_), .O(new_n96_));
  inv1   g034(.a(new_n96_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n79_), .c(new_n68_), .O(z05));
  oai21  g036(.a(new_n96_), .b(x37), .c(new_n79_), .O(z06));
  inv1   g037(.a(x33), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n65_), .O(new_n101_));
  inv1   g039(.a(x00), .O(new_n102_));
  oai21  g040(.a(x25), .b(new_n102_), .c(x38), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(new_n107_));
  oai22  g045(.a(new_n107_), .b(x14), .c(new_n104_), .d(x31), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n79_), .c(x03), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z07));
  inv1   g048(.a(x39), .O(new_n111_));
  nor3   g049(.a(new_n64_), .b(new_n73_), .c(new_n111_), .O(z08));
  nand4  g050(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n79_), .O(z09));
  inv1   g052(.a(x36), .O(new_n115_));
  oai21  g053(.a(new_n85_), .b(x28), .c(new_n115_), .O(new_n116_));
  nand2  g054(.a(new_n81_), .b(x07), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n116_), .c(x27), .O(new_n120_));
  oai21  g058(.a(x32), .b(x30), .c(x40), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n111_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x05), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n79_), .O(z10));
  inv1   g065(.a(x09), .O(new_n128_));
  inv1   g066(.a(x30), .O(new_n129_));
  nand4  g067(.a(x40), .b(x39), .c(x29), .d(x08), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n86_), .c(new_n84_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(new_n129_), .c(new_n128_), .d(new_n63_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  nand3  g072(.a(new_n68_), .b(new_n115_), .c(new_n85_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x27), .c(x10), .O(new_n136_));
  inv1   g074(.a(new_n81_), .O(new_n137_));
  nor2   g075(.a(new_n137_), .b(new_n70_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n136_), .c(new_n63_), .O(new_n139_));
  inv1   g077(.a(new_n70_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x30), .c(x04), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x28), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(x40), .c(x39), .d(new_n143_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand2  g086(.a(new_n69_), .b(new_n68_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n146_), .c(x04), .O(new_n151_));
  nand4  g089(.a(new_n149_), .b(x30), .c(x20), .d(new_n148_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(x18), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n151_), .c(x27), .O(new_n154_));
  nand3  g092(.a(new_n122_), .b(new_n143_), .c(new_n63_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z13));
  nand3  g094(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n157_));
  inv1   g095(.a(x32), .O(new_n158_));
  nand4  g096(.a(new_n144_), .b(new_n158_), .c(new_n129_), .d(x28), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  nand3  g098(.a(new_n137_), .b(new_n143_), .c(new_n63_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n160_), .c(new_n157_), .O(new_n162_));
  nand3  g100(.a(new_n68_), .b(new_n85_), .c(x28), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x27), .c(x32), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(x04), .c(new_n129_), .O(new_n165_));
  nand2  g103(.a(new_n161_), .b(new_n140_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n165_), .c(new_n162_), .O(z14));
  nand4  g105(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n79_), .O(z15));
  inv1   g107(.a(x23), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(x22), .c(x01), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n79_), .O(z16));
  inv1   g110(.a(x01), .O(new_n173_));
  inv1   g111(.a(x24), .O(new_n174_));
  nand4  g112(.a(new_n79_), .b(new_n174_), .c(x23), .d(x22), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n173_), .O(z17));
  nand3  g114(.a(x40), .b(x39), .c(x29), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n86_), .c(x08), .O(new_n179_));
  aoi21  g117(.a(new_n86_), .b(x27), .c(x30), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n179_), .c(new_n128_), .d(new_n63_), .O(z18));
  aoi21  g119(.a(new_n85_), .b(new_n63_), .c(x28), .O(new_n182_));
  oai22  g120(.a(new_n182_), .b(new_n131_), .c(new_n84_), .d(x04), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n128_), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n63_), .c(x30), .O(z19));
  oai21  g123(.a(new_n85_), .b(new_n67_), .c(new_n83_), .O(new_n186_));
  aoi21  g124(.a(x28), .b(new_n67_), .c(new_n85_), .O(new_n187_));
  oai21  g125(.a(new_n187_), .b(new_n178_), .c(new_n186_), .O(new_n188_));
  nand4  g126(.a(new_n188_), .b(new_n129_), .c(new_n128_), .d(new_n63_), .O(new_n189_));
  inv1   g127(.a(new_n189_), .O(z20));
endmodule


