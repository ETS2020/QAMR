// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n181_, new_n182_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  nor2   g017(.a(x30), .b(x28), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n79_), .c(x16), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  nand2  g022(.a(x36), .b(x27), .O(new_n85_));
  nand4  g023(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n86_));
  oai21  g024(.a(new_n80_), .b(x04), .c(new_n86_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  inv1   g029(.a(x28), .O(new_n92_));
  nand2  g030(.a(x35), .b(new_n92_), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n91_), .c(x04), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n90_), .c(new_n81_), .d(new_n76_), .O(z02));
  inv1   g033(.a(x21), .O(new_n96_));
  oai21  g034(.a(new_n83_), .b(x27), .c(new_n92_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g036(.a(new_n83_), .b(x28), .O(new_n99_));
  nor2   g037(.a(x35), .b(new_n92_), .O(new_n100_));
  oai21  g038(.a(new_n100_), .b(new_n99_), .c(x27), .O(new_n101_));
  nand3  g039(.a(new_n81_), .b(new_n70_), .c(new_n64_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n101_), .c(new_n98_), .O(z03));
  nand2  g041(.a(new_n71_), .b(x27), .O(new_n104_));
  nor2   g042(.a(x37), .b(x27), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(new_n106_));
  nand4  g044(.a(new_n106_), .b(new_n104_), .c(new_n81_), .d(new_n96_), .O(z04));
  oai22  g045(.a(new_n105_), .b(new_n92_), .c(new_n70_), .d(new_n83_), .O(z05));
  aoi21  g046(.a(x28), .b(new_n64_), .c(new_n99_), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x37), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n63_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n81_), .O(z07));
  nand2  g059(.a(new_n81_), .b(new_n75_), .O(z08));
  inv1   g060(.a(x11), .O(new_n123_));
  nand4  g061(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n123_), .O(z09));
  oai21  g063(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n75_), .c(x07), .O(new_n127_));
  nand3  g065(.a(new_n76_), .b(x36), .c(x05), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n64_), .O(new_n129_));
  inv1   g067(.a(x05), .O(new_n130_));
  nand2  g068(.a(new_n84_), .b(new_n83_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x40), .c(x39), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n129_), .c(new_n65_), .O(new_n134_));
  nor2   g072(.a(new_n70_), .b(new_n64_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x06), .c(new_n80_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand3  g076(.a(new_n76_), .b(x29), .c(x08), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(x28), .c(x30), .O(z11));
  inv1   g079(.a(x10), .O(new_n142_));
  nor3   g080(.a(x37), .b(x36), .c(x35), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n64_), .c(new_n142_), .O(new_n144_));
  oai22  g082(.a(new_n75_), .b(x04), .c(new_n73_), .d(new_n64_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(new_n81_), .O(z12));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand4  g086(.a(new_n72_), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n149_));
  nor2   g087(.a(new_n66_), .b(x13), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n76_), .c(x36), .d(new_n65_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nor2   g091(.a(x13), .b(x04), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n76_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x30), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n92_), .O(new_n157_));
  nor2   g095(.a(new_n132_), .b(x13), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n65_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n153_), .O(z13));
  nand3  g098(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n161_));
  nand2  g099(.a(x36), .b(x35), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n84_), .c(new_n83_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n155_), .c(new_n161_), .O(new_n165_));
  oai21  g103(.a(x37), .b(x35), .c(x27), .O(new_n166_));
  nand3  g104(.a(new_n154_), .b(new_n131_), .c(new_n76_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  aoi21  g106(.a(new_n154_), .b(new_n76_), .c(x37), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n83_), .c(new_n92_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n168_), .c(new_n165_), .O(z14));
  inv1   g109(.a(x12), .O(new_n172_));
  nor2   g110(.a(new_n124_), .b(new_n172_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand4  g112(.a(new_n81_), .b(new_n174_), .c(x22), .d(x01), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(x23), .c(x22), .d(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n81_), .O(z17));
  inv1   g117(.a(z11), .O(z18));
  nand2  g118(.a(x35), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n139_), .c(new_n138_), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(x28), .c(x30), .O(z20));
  aoi21  g121(.a(new_n140_), .b(x28), .c(x30), .O(z19));
endmodule


