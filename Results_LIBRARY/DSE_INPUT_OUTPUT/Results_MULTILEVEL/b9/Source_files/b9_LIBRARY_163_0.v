// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n205_, new_n206_, new_n207_,
    new_n208_;
  inv1   g000(.a(x37), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  oai21  g006(.a(x36), .b(x35), .c(x39), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  oai21  g008(.a(new_n69_), .b(x04), .c(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(x40), .c(x37), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x39), .O(new_n74_));
  nor2   g012(.a(new_n64_), .b(new_n74_), .O(z08));
  nand3  g013(.a(z08), .b(x10), .c(new_n73_), .O(new_n76_));
  oai21  g014(.a(new_n72_), .b(new_n68_), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n67_), .O(new_n78_));
  oai21  g016(.a(new_n66_), .b(x16), .c(new_n78_), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n66_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  nand2  g028(.a(x35), .b(new_n90_), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n91_), .c(x04), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n89_), .c(x39), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(new_n95_));
  nor2   g033(.a(x40), .b(new_n63_), .O(new_n96_));
  inv1   g034(.a(new_n96_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n95_), .O(z02));
  nand2  g036(.a(new_n70_), .b(x27), .O(new_n99_));
  oai21  g037(.a(new_n64_), .b(new_n68_), .c(new_n63_), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(x21), .O(z03));
  inv1   g039(.a(x21), .O(new_n102_));
  nand3  g040(.a(new_n100_), .b(new_n99_), .c(new_n102_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(x40), .c(new_n63_), .O(z05));
  nand3  g043(.a(new_n104_), .b(x40), .c(new_n63_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n67_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n65_), .O(z07));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x06), .O(new_n122_));
  nor2   g060(.a(new_n83_), .b(x36), .O(new_n123_));
  nor2   g061(.a(new_n64_), .b(x39), .O(new_n124_));
  oai21  g062(.a(new_n124_), .b(new_n96_), .c(x07), .O(new_n125_));
  nand2  g063(.a(z08), .b(x05), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n81_), .b(new_n80_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x40), .c(x39), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  aoi21  g069(.a(new_n127_), .b(x27), .c(new_n131_), .O(new_n132_));
  nand2  g070(.a(x37), .b(x27), .O(new_n133_));
  oai22  g071(.a(new_n133_), .b(new_n122_), .c(new_n132_), .d(x04), .O(z10));
  nand2  g072(.a(new_n65_), .b(new_n68_), .O(new_n135_));
  oai21  g073(.a(new_n91_), .b(x04), .c(x40), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n135_), .c(x08), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x40), .c(new_n96_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n83_), .c(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n137_), .c(new_n80_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(x09), .O(z11));
  inv1   g081(.a(x10), .O(new_n144_));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n68_), .c(new_n144_), .O(new_n146_));
  aoi21  g084(.a(x35), .b(x28), .c(x37), .O(new_n147_));
  oai22  g085(.a(new_n147_), .b(new_n68_), .c(new_n74_), .d(x04), .O(new_n148_));
  nand2  g086(.a(new_n133_), .b(new_n64_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n146_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n64_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n73_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand3  g094(.a(new_n70_), .b(x40), .c(new_n63_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n154_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x27), .O(new_n160_));
  nor2   g098(.a(new_n130_), .b(x13), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n73_), .c(new_n66_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n160_), .O(z13));
  nand3  g101(.a(x39), .b(new_n151_), .c(new_n73_), .O(new_n164_));
  nand4  g102(.a(x27), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n147_), .c(new_n164_), .O(new_n166_));
  nand3  g104(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n167_));
  nor2   g105(.a(x37), .b(x35), .O(new_n168_));
  aoi21  g106(.a(new_n167_), .b(new_n152_), .c(new_n168_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n90_), .c(x27), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n81_), .c(new_n80_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n166_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x40), .O(new_n173_));
  nand3  g111(.a(new_n165_), .b(new_n64_), .c(x37), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n173_), .O(z14));
  nand4  g113(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n65_), .O(z15));
  inv1   g115(.a(x23), .O(new_n178_));
  nand4  g116(.a(new_n65_), .b(new_n178_), .c(x22), .d(x01), .O(new_n179_));
  inv1   g117(.a(new_n179_), .O(z16));
  inv1   g118(.a(x24), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(x23), .c(x22), .d(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n65_), .O(z17));
  inv1   g121(.a(z08), .O(new_n184_));
  nand2  g122(.a(x27), .b(new_n73_), .O(new_n185_));
  nand2  g123(.a(new_n68_), .b(x08), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(new_n82_), .O(new_n187_));
  inv1   g125(.a(x09), .O(new_n188_));
  nand2  g126(.a(new_n80_), .b(new_n188_), .O(new_n189_));
  aoi21  g127(.a(new_n187_), .b(new_n90_), .c(new_n189_), .O(new_n190_));
  oai22  g128(.a(new_n190_), .b(new_n66_), .c(new_n88_), .d(new_n184_), .O(z18));
  inv1   g129(.a(x08), .O(new_n192_));
  oai21  g130(.a(new_n82_), .b(x04), .c(x40), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n135_), .c(new_n90_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n192_), .O(new_n195_));
  nand2  g133(.a(x40), .b(new_n82_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n140_), .c(new_n90_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n138_), .O(new_n198_));
  nand2  g136(.a(x37), .b(new_n82_), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(new_n140_), .c(new_n90_), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(new_n64_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n198_), .c(new_n195_), .O(new_n202_));
  nand3  g140(.a(new_n202_), .b(new_n80_), .c(new_n188_), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(new_n65_), .O(z19));
  aoi21  g142(.a(x28), .b(new_n68_), .c(new_n82_), .O(new_n205_));
  nor2   g143(.a(new_n205_), .b(new_n139_), .O(new_n206_));
  aoi21  g144(.a(new_n196_), .b(new_n135_), .c(x08), .O(new_n207_));
  oai21  g145(.a(new_n207_), .b(new_n206_), .c(new_n80_), .O(new_n208_));
  nor2   g146(.a(new_n208_), .b(x09), .O(z20));
endmodule


