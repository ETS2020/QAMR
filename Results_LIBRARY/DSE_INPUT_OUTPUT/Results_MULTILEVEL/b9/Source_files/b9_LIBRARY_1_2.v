// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_;
  inv1   g000(.a(x30), .O(new_n63_));
  nor2   g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(new_n64_), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n80_), .c(new_n63_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n79_), .c(x04), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n83_), .c(x04), .O(new_n92_));
  nand2  g030(.a(x40), .b(x39), .O(new_n93_));
  inv1   g031(.a(new_n93_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(new_n90_), .O(z02));
  nand2  g033(.a(new_n69_), .b(x27), .O(new_n96_));
  aoi21  g034(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(z04));
  nor2   g038(.a(new_n82_), .b(new_n67_), .O(new_n101_));
  inv1   g039(.a(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n79_), .c(new_n68_), .O(z05));
  oai21  g041(.a(new_n101_), .b(x37), .c(new_n79_), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n65_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n79_), .O(z07));
  nand2  g053(.a(new_n93_), .b(new_n79_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n79_), .O(z09));
  nand2  g056(.a(x39), .b(x05), .O(new_n119_));
  oai21  g057(.a(new_n119_), .b(x04), .c(x40), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(x30), .O(new_n121_));
  inv1   g059(.a(x05), .O(new_n122_));
  nand2  g060(.a(new_n93_), .b(x07), .O(new_n123_));
  oai21  g061(.a(new_n93_), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n84_), .c(x27), .O(new_n125_));
  nand3  g063(.a(new_n94_), .b(x32), .c(x05), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n72_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n121_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  aoi21  g069(.a(x39), .b(x29), .c(x30), .O(new_n132_));
  nand2  g070(.a(x27), .b(x04), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x35), .c(new_n82_), .O(new_n134_));
  oai21  g072(.a(new_n132_), .b(new_n74_), .c(new_n134_), .O(new_n135_));
  inv1   g073(.a(x08), .O(new_n136_));
  nand4  g074(.a(x35), .b(new_n82_), .c(x27), .d(new_n72_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n63_), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n131_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n79_), .O(z11));
  inv1   g079(.a(x35), .O(new_n142_));
  nand3  g080(.a(new_n68_), .b(new_n81_), .c(new_n142_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  aoi21  g082(.a(x39), .b(new_n72_), .c(new_n70_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(x40), .O(new_n146_));
  inv1   g084(.a(new_n70_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n74_), .c(new_n63_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x28), .c(new_n74_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x39), .c(new_n150_), .d(new_n72_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  nand2  g093(.a(new_n69_), .b(new_n68_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x20), .c(new_n155_), .d(new_n154_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nand2  g097(.a(x40), .b(x32), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n63_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x39), .c(new_n150_), .d(new_n72_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n159_), .c(new_n79_), .O(z13));
  nand3  g101(.a(x20), .b(new_n155_), .c(new_n154_), .O(new_n164_));
  nand3  g102(.a(new_n151_), .b(new_n80_), .c(x28), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x40), .c(x30), .O(new_n166_));
  nand3  g104(.a(x39), .b(new_n150_), .c(new_n72_), .O(new_n167_));
  and2   g105(.a(new_n167_), .b(x40), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(new_n164_), .O(new_n169_));
  nand2  g107(.a(new_n160_), .b(new_n67_), .O(new_n170_));
  oai21  g108(.a(x32), .b(new_n82_), .c(x40), .O(new_n171_));
  nor2   g109(.a(x40), .b(x28), .O(new_n172_));
  aoi21  g110(.a(new_n171_), .b(new_n142_), .c(new_n172_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(x37), .c(new_n170_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n63_), .O(new_n175_));
  nand3  g113(.a(new_n167_), .b(new_n147_), .c(x40), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n175_), .c(new_n169_), .O(z14));
  inv1   g115(.a(x12), .O(new_n178_));
  nand4  g116(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n178_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n79_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n184_), .b(x23), .c(x22), .d(x01), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n79_), .O(z17));
  nand4  g124(.a(x35), .b(new_n63_), .c(new_n82_), .d(new_n67_), .O(new_n187_));
  nand2  g125(.a(new_n94_), .b(x29), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x08), .O(new_n190_));
  nand2  g128(.a(new_n137_), .b(new_n131_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n63_), .O(new_n192_));
  nand2  g130(.a(x40), .b(x30), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n192_), .c(new_n190_), .O(z18));
  nand2  g132(.a(x39), .b(x29), .O(new_n195_));
  nand2  g133(.a(new_n134_), .b(new_n195_), .O(new_n196_));
  oai21  g134(.a(new_n74_), .b(new_n136_), .c(new_n142_), .O(new_n197_));
  nand3  g135(.a(new_n82_), .b(x27), .c(new_n72_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n136_), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(new_n197_), .c(new_n196_), .O(new_n200_));
  aoi21  g138(.a(new_n133_), .b(new_n82_), .c(x40), .O(new_n201_));
  aoi21  g139(.a(new_n200_), .b(new_n63_), .c(new_n201_), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(x09), .c(new_n79_), .O(z19));
  inv1   g141(.a(new_n188_), .O(new_n204_));
  oai21  g142(.a(new_n142_), .b(new_n67_), .c(new_n136_), .O(new_n205_));
  aoi21  g143(.a(x28), .b(new_n67_), .c(new_n142_), .O(new_n206_));
  oai21  g144(.a(new_n206_), .b(new_n204_), .c(new_n205_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n63_), .c(new_n131_), .O(new_n208_));
  inv1   g146(.a(new_n208_), .O(z20));
endmodule


