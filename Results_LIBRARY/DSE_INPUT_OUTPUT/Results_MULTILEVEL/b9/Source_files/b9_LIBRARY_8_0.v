// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n149_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x16), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n76_), .O(z00));
  inv1   g018(.a(new_n78_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x04), .O(z01));
  inv1   g020(.a(x02), .O(new_n83_));
  nand2  g021(.a(x29), .b(x08), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n86_), .c(x04), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n85_), .c(new_n81_), .d(new_n73_), .O(z02));
  oai21  g028(.a(new_n66_), .b(new_n87_), .c(x27), .O(new_n91_));
  inv1   g029(.a(x21), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n64_), .c(new_n92_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n91_), .c(new_n78_), .O(z03));
  nand2  g033(.a(new_n93_), .b(new_n64_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n91_), .c(new_n81_), .d(new_n92_), .O(z04));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n81_), .c(new_n93_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x33), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n63_), .O(new_n102_));
  inv1   g040(.a(x00), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n103_), .c(x38), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n103_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  oai21  g047(.a(new_n105_), .b(x31), .c(new_n109_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n81_), .O(z07));
  nand2  g050(.a(new_n81_), .b(new_n72_), .O(z08));
  inv1   g051(.a(x11), .O(new_n114_));
  nand4  g052(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n114_), .O(z09));
  oai21  g054(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n117_));
  nand4  g055(.a(new_n117_), .b(new_n72_), .c(x27), .d(x07), .O(new_n118_));
  nand2  g056(.a(new_n73_), .b(x05), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n65_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(new_n78_), .O(z10));
  inv1   g061(.a(x09), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x29), .O(new_n125_));
  nor2   g063(.a(new_n64_), .b(new_n65_), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n88_), .c(new_n125_), .O(new_n127_));
  inv1   g065(.a(x08), .O(new_n128_));
  nand4  g066(.a(x35), .b(new_n87_), .c(x27), .d(new_n65_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n124_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(x32), .c(x30), .O(z11));
  inv1   g071(.a(x10), .O(new_n134_));
  nor3   g072(.a(x37), .b(x36), .c(x35), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n64_), .c(new_n134_), .O(new_n136_));
  oai22  g074(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n136_), .c(new_n78_), .O(z12));
  inv1   g076(.a(x18), .O(new_n139_));
  inv1   g077(.a(x19), .O(new_n140_));
  nor2   g078(.a(new_n70_), .b(new_n64_), .O(new_n141_));
  nand4  g079(.a(new_n141_), .b(x20), .c(new_n140_), .d(new_n139_), .O(new_n142_));
  nor2   g080(.a(x13), .b(x04), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n73_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n142_), .c(new_n78_), .O(z13));
  nand4  g083(.a(x27), .b(x20), .c(new_n140_), .d(new_n139_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n70_), .c(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n81_), .O(z14));
  inv1   g086(.a(x12), .O(new_n149_));
  nor2   g087(.a(new_n115_), .b(new_n149_), .O(z15));
  inv1   g088(.a(x23), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(x22), .c(x01), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n81_), .O(z16));
  inv1   g091(.a(x01), .O(new_n154_));
  inv1   g092(.a(x24), .O(new_n155_));
  nand4  g093(.a(new_n81_), .b(new_n155_), .c(x23), .d(x22), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(new_n154_), .O(z17));
  oai21  g095(.a(new_n88_), .b(x27), .c(new_n125_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x08), .O(new_n159_));
  inv1   g097(.a(x32), .O(new_n160_));
  nor2   g098(.a(new_n160_), .b(x30), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n159_), .c(new_n129_), .d(new_n124_), .O(z18));
  inv1   g100(.a(x30), .O(new_n163_));
  nand4  g101(.a(new_n131_), .b(x32), .c(new_n163_), .d(new_n124_), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z19));
  inv1   g103(.a(new_n125_), .O(new_n166_));
  oai21  g104(.a(new_n66_), .b(new_n64_), .c(new_n128_), .O(new_n167_));
  aoi21  g105(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n166_), .c(new_n167_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x32), .c(new_n163_), .d(new_n124_), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z20));
endmodule


