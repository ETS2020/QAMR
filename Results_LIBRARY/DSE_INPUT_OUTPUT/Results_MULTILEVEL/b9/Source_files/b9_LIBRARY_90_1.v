// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n192_, new_n194_, new_n195_,
    new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x33), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x10), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x15), .c(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x15), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  inv1   g013(.a(x35), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n63_), .c(x27), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n74_), .c(new_n75_), .O(new_n80_));
  nand4  g018(.a(new_n80_), .b(x39), .c(new_n73_), .d(new_n72_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n71_), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n77_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n84_), .c(new_n83_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n65_), .c(x04), .O(z01));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  inv1   g031(.a(x08), .O(new_n94_));
  inv1   g032(.a(x27), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(x35), .c(new_n85_), .O(new_n97_));
  nand2  g035(.a(x40), .b(x39), .O(new_n98_));
  aoi21  g036(.a(new_n97_), .b(x04), .c(new_n98_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n93_), .c(new_n64_), .O(z02));
  nand2  g038(.a(new_n67_), .b(x27), .O(new_n101_));
  inv1   g039(.a(x21), .O(new_n102_));
  aoi21  g040(.a(new_n66_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n101_), .c(new_n64_), .O(z03));
  aoi21  g042(.a(new_n66_), .b(new_n95_), .c(x21), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n101_), .c(new_n64_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n66_), .c(new_n64_), .O(z05));
  nand3  g046(.a(new_n107_), .b(new_n65_), .c(new_n66_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z06));
  inv1   g048(.a(x03), .O(new_n111_));
  inv1   g049(.a(x31), .O(new_n112_));
  nand2  g050(.a(x17), .b(new_n73_), .O(new_n113_));
  inv1   g051(.a(x00), .O(new_n114_));
  oai21  g052(.a(x25), .b(new_n114_), .c(x38), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(x14), .O(new_n117_));
  inv1   g055(.a(x25), .O(new_n118_));
  nand3  g056(.a(x38), .b(new_n118_), .c(new_n114_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  nand3  g060(.a(new_n119_), .b(new_n117_), .c(x10), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n111_), .O(z07));
  nand2  g062(.a(new_n98_), .b(new_n65_), .O(z08));
  nand4  g063(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n65_), .O(z09));
  nand2  g065(.a(new_n98_), .b(x07), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n87_), .c(x27), .O(new_n131_));
  nand2  g069(.a(new_n84_), .b(new_n83_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(x40), .c(x39), .d(x05), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n72_), .O(new_n135_));
  nand3  g073(.a(x37), .b(x27), .c(x06), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n135_), .c(new_n65_), .O(z10));
  inv1   g075(.a(x09), .O(new_n138_));
  nand3  g076(.a(x40), .b(x39), .c(x29), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x35), .c(new_n85_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand4  g080(.a(x35), .b(new_n85_), .c(x27), .d(new_n72_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n94_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n83_), .c(new_n138_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n65_), .O(z11));
  oai21  g085(.a(new_n98_), .b(x04), .c(new_n67_), .O(new_n148_));
  nand3  g086(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n148_), .c(x37), .O(new_n150_));
  nor2   g088(.a(x27), .b(x10), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n63_), .O(new_n152_));
  inv1   g090(.a(new_n98_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n72_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n69_), .c(x10), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x28), .c(new_n75_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n72_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand4  g100(.a(new_n68_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x27), .O(new_n165_));
  nand3  g103(.a(new_n132_), .b(x40), .c(x39), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x13), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n72_), .c(new_n64_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(z13));
  nand3  g107(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n170_));
  nand4  g108(.a(new_n158_), .b(new_n84_), .c(new_n83_), .d(x28), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  nand3  g110(.a(new_n153_), .b(new_n157_), .c(new_n72_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n170_), .O(new_n174_));
  nand3  g112(.a(new_n66_), .b(new_n76_), .c(x28), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(x27), .c(x32), .O(new_n176_));
  aoi22  g114(.a(new_n176_), .b(new_n83_), .c(new_n173_), .d(new_n69_), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n174_), .c(new_n64_), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n65_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand4  g119(.a(new_n65_), .b(new_n181_), .c(x22), .d(x01), .O(new_n182_));
  inv1   g120(.a(new_n182_), .O(z16));
  inv1   g121(.a(x01), .O(new_n184_));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n65_), .b(new_n185_), .c(x23), .d(x22), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z17));
  oai21  g125(.a(new_n86_), .b(x27), .c(new_n139_), .O(new_n188_));
  nand3  g126(.a(new_n143_), .b(new_n83_), .c(new_n138_), .O(new_n189_));
  aoi21  g127(.a(new_n188_), .b(x08), .c(new_n189_), .O(new_n190_));
  nor2   g128(.a(new_n190_), .b(new_n64_), .O(z18));
  nand4  g129(.a(new_n145_), .b(new_n65_), .c(new_n83_), .d(new_n138_), .O(new_n192_));
  inv1   g130(.a(new_n192_), .O(z19));
  inv1   g131(.a(new_n139_), .O(new_n194_));
  oai21  g132(.a(new_n76_), .b(new_n95_), .c(new_n94_), .O(new_n195_));
  aoi21  g133(.a(x28), .b(new_n95_), .c(new_n76_), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n194_), .c(new_n195_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n83_), .c(new_n138_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n65_), .O(z20));
endmodule


