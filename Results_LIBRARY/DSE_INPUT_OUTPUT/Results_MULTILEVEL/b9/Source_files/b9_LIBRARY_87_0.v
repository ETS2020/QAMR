// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_;
  inv1   g000(.a(x04), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n64_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x39), .c(new_n66_), .d(new_n63_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n72_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x35), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(x28), .c(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand4  g021(.a(new_n83_), .b(new_n79_), .c(new_n78_), .d(x04), .O(new_n84_));
  oai21  g022(.a(new_n64_), .b(x04), .c(new_n84_), .O(z01));
  inv1   g023(.a(x39), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n86_), .c(x40), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  nor4   g027(.a(new_n89_), .b(new_n64_), .c(new_n80_), .d(x28), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n63_), .c(new_n88_), .O(z02));
  inv1   g029(.a(new_n65_), .O(new_n92_));
  nand2  g030(.a(x35), .b(x28), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x21), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n67_), .c(new_n95_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n92_), .O(z03));
  nand2  g036(.a(new_n96_), .b(new_n67_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n94_), .c(new_n65_), .d(new_n95_), .O(z04));
  inv1   g038(.a(x28), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n67_), .O(new_n102_));
  inv1   g040(.a(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n65_), .c(new_n96_), .O(z05));
  oai21  g042(.a(new_n102_), .b(x37), .c(new_n65_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n65_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  nor2   g054(.a(new_n64_), .b(new_n86_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(x39), .b(x05), .O(new_n122_));
  oai21  g060(.a(x39), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n82_), .c(new_n63_), .O(new_n124_));
  nand2  g062(.a(x37), .b(x06), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n64_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x06), .c(x04), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n126_), .c(x27), .O(new_n129_));
  oai21  g067(.a(x32), .b(x30), .c(x40), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n86_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x05), .c(new_n63_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n129_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(x35), .b(new_n101_), .O(new_n135_));
  nand2  g073(.a(x39), .b(x29), .O(new_n136_));
  nor2   g074(.a(new_n67_), .b(new_n63_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n135_), .c(new_n136_), .O(new_n138_));
  inv1   g076(.a(x08), .O(new_n139_));
  oai21  g077(.a(new_n64_), .b(new_n139_), .c(new_n135_), .O(new_n140_));
  oai21  g078(.a(new_n67_), .b(x04), .c(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n64_), .b(x27), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(new_n138_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n78_), .c(new_n134_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n65_), .O(z11));
  nand3  g083(.a(new_n96_), .b(new_n81_), .c(new_n80_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x27), .c(x10), .O(new_n147_));
  nor2   g085(.a(new_n69_), .b(x39), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n147_), .c(x40), .O(new_n149_));
  oai21  g087(.a(new_n69_), .b(new_n63_), .c(new_n149_), .O(z12));
  inv1   g088(.a(x19), .O(new_n151_));
  nand2  g089(.a(new_n93_), .b(new_n96_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(new_n65_), .c(x20), .d(new_n151_), .O(new_n153_));
  inv1   g091(.a(x13), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x28), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x40), .c(x39), .d(new_n154_), .O(new_n157_));
  oai22  g095(.a(new_n157_), .b(x04), .c(new_n153_), .d(x18), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nand3  g097(.a(new_n131_), .b(new_n154_), .c(new_n63_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(z13));
  aoi21  g099(.a(x39), .b(new_n154_), .c(new_n64_), .O(new_n162_));
  inv1   g100(.a(x18), .O(new_n163_));
  nand4  g101(.a(x27), .b(x20), .c(new_n151_), .d(new_n163_), .O(new_n164_));
  oai22  g102(.a(new_n164_), .b(new_n68_), .c(new_n162_), .d(x04), .O(new_n165_));
  nand3  g103(.a(x20), .b(new_n151_), .c(new_n163_), .O(new_n166_));
  nor2   g104(.a(x37), .b(x35), .O(new_n167_));
  aoi21  g105(.a(new_n166_), .b(new_n155_), .c(new_n167_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n101_), .c(x27), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x40), .c(new_n79_), .d(new_n78_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n165_), .O(z14));
  nand4  g109(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n65_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(x22), .c(x01), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n65_), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(x23), .c(x22), .d(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n65_), .O(z17));
  nor3   g117(.a(new_n89_), .b(new_n80_), .c(x28), .O(new_n180_));
  nand3  g118(.a(x39), .b(x29), .c(x08), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n78_), .c(new_n134_), .O(new_n182_));
  aoi21  g120(.a(new_n180_), .b(new_n63_), .c(new_n182_), .O(new_n183_));
  nand4  g121(.a(x35), .b(new_n101_), .c(new_n67_), .d(x08), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n78_), .c(new_n134_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(x04), .O(new_n186_));
  oai21  g124(.a(new_n183_), .b(new_n64_), .c(new_n186_), .O(z18));
  nand3  g125(.a(new_n64_), .b(x28), .c(x04), .O(new_n188_));
  oai21  g126(.a(new_n64_), .b(x08), .c(new_n188_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n67_), .O(new_n190_));
  oai21  g128(.a(new_n80_), .b(x27), .c(new_n64_), .O(new_n191_));
  aoi21  g129(.a(new_n136_), .b(x27), .c(new_n139_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n191_), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(x04), .O(new_n194_));
  nand3  g132(.a(new_n181_), .b(new_n135_), .c(x40), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n194_), .c(new_n190_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n78_), .c(new_n134_), .O(new_n197_));
  inv1   g135(.a(new_n197_), .O(z19));
  nand2  g136(.a(x28), .b(new_n67_), .O(new_n199_));
  nand2  g137(.a(new_n136_), .b(x40), .O(new_n200_));
  nand2  g138(.a(new_n64_), .b(x04), .O(new_n201_));
  aoi22  g139(.a(new_n201_), .b(new_n200_), .c(new_n199_), .d(x35), .O(new_n202_));
  nand2  g140(.a(new_n65_), .b(new_n67_), .O(new_n203_));
  nand2  g141(.a(x40), .b(new_n80_), .O(new_n204_));
  aoi21  g142(.a(new_n204_), .b(new_n203_), .c(x08), .O(new_n205_));
  oai21  g143(.a(new_n205_), .b(new_n202_), .c(new_n78_), .O(new_n206_));
  nor2   g144(.a(new_n206_), .b(x09), .O(z20));
endmodule


