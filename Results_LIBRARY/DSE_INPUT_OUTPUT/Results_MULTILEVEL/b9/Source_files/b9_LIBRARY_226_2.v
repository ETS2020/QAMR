// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n173_, new_n174_,
    new_n175_, new_n178_;
  inv1   g000(.a(x30), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x08), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g009(.a(new_n71_), .b(x40), .c(x39), .d(new_n68_), .O(new_n72_));
  inv1   g010(.a(x37), .O(new_n73_));
  nand2  g011(.a(x35), .b(x28), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n72_), .c(new_n67_), .O(new_n77_));
  nand2  g015(.a(x40), .b(x39), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(x10), .c(new_n68_), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n77_), .c(new_n66_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n70_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x27), .c(x32), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(x08), .c(new_n63_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x04), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n85_), .c(x04), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n79_), .d(new_n64_), .O(z02));
  nand2  g033(.a(new_n74_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n73_), .b(new_n67_), .c(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n96_), .c(new_n65_), .O(z03));
  aoi21  g037(.a(new_n73_), .b(new_n67_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n96_), .c(new_n65_), .O(z04));
  nor2   g039(.a(new_n84_), .b(new_n67_), .O(new_n102_));
  inv1   g040(.a(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n64_), .c(new_n73_), .O(z05));
  oai21  g042(.a(new_n102_), .b(x37), .c(new_n64_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z07));
  nand2  g054(.a(new_n78_), .b(new_n64_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(z09));
  inv1   g057(.a(x05), .O(new_n120_));
  nand2  g058(.a(new_n78_), .b(x07), .O(new_n121_));
  oai21  g059(.a(new_n78_), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n86_), .c(x27), .O(new_n123_));
  inv1   g061(.a(x32), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  nand4  g063(.a(new_n125_), .b(x40), .c(x39), .d(x05), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n68_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n64_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  inv1   g069(.a(x09), .O(new_n132_));
  nand2  g070(.a(x27), .b(new_n68_), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n85_), .c(new_n132_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n131_), .c(x30), .O(z11));
  inv1   g073(.a(x10), .O(new_n136_));
  nor3   g074(.a(x37), .b(x36), .c(x35), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n67_), .c(new_n136_), .O(new_n138_));
  oai22  g076(.a(new_n78_), .b(x04), .c(new_n76_), .d(new_n67_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n64_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  inv1   g079(.a(x40), .O(new_n142_));
  nand2  g080(.a(x36), .b(x35), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x28), .c(new_n142_), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(x39), .c(new_n141_), .d(new_n68_), .O(new_n145_));
  inv1   g083(.a(x18), .O(new_n146_));
  inv1   g084(.a(x19), .O(new_n147_));
  nand4  g085(.a(new_n75_), .b(x20), .c(new_n147_), .d(new_n146_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n145_), .c(x08), .O(new_n149_));
  nand4  g087(.a(new_n75_), .b(x30), .c(x20), .d(new_n147_), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(x18), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(x27), .O(new_n152_));
  nand2  g090(.a(x32), .b(new_n131_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n63_), .c(new_n142_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n141_), .d(new_n68_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(z13));
  nand3  g094(.a(x20), .b(new_n147_), .c(new_n146_), .O(new_n157_));
  nand4  g095(.a(new_n143_), .b(new_n124_), .c(new_n63_), .d(x28), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  nand3  g097(.a(new_n79_), .b(new_n141_), .c(new_n68_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n157_), .O(new_n161_));
  nand3  g099(.a(new_n73_), .b(new_n69_), .c(x28), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(x27), .c(x32), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x08), .c(new_n63_), .O(new_n164_));
  oai21  g102(.a(new_n76_), .b(new_n67_), .c(new_n160_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n164_), .c(new_n161_), .O(z14));
  inv1   g104(.a(x12), .O(new_n167_));
  nand4  g105(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(new_n167_), .O(z15));
  inv1   g107(.a(x23), .O(new_n170_));
  nand4  g108(.a(new_n64_), .b(new_n170_), .c(x22), .d(x01), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z16));
  inv1   g110(.a(x01), .O(new_n173_));
  inv1   g111(.a(x24), .O(new_n174_));
  nand4  g112(.a(new_n64_), .b(new_n174_), .c(x23), .d(x22), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n173_), .O(z17));
  inv1   g114(.a(z11), .O(z18));
  oai21  g115(.a(new_n69_), .b(new_n67_), .c(new_n132_), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n131_), .c(x30), .O(z20));
  aoi21  g117(.a(new_n134_), .b(new_n131_), .c(x30), .O(z19));
endmodule


