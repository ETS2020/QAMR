// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n168_, new_n170_, new_n171_, new_n173_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n203_;
  inv1   g000(.a(x06), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x15), .c(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x15), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n64_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n73_), .d(new_n72_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n71_), .O(z00));
  inv1   g016(.a(new_n65_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n79_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  nand2  g028(.a(x35), .b(new_n90_), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n91_), .c(x04), .O(new_n93_));
  inv1   g031(.a(x39), .O(new_n94_));
  nor2   g032(.a(new_n64_), .b(new_n94_), .O(z08));
  nand3  g033(.a(z08), .b(new_n93_), .c(new_n89_), .O(z02));
  nand2  g034(.a(new_n67_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x27), .O(new_n98_));
  aoi21  g036(.a(new_n66_), .b(new_n98_), .c(new_n79_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n97_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n97_), .c(new_n101_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n79_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n73_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n65_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  nor2   g057(.a(new_n83_), .b(x36), .O(new_n120_));
  nor2   g058(.a(new_n64_), .b(x39), .O(new_n121_));
  nor2   g059(.a(x40), .b(new_n63_), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(new_n121_), .c(x07), .O(new_n123_));
  nand2  g061(.a(z08), .b(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n120_), .O(new_n125_));
  oai21  g063(.a(x32), .b(x30), .c(x40), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n94_), .O(new_n127_));
  and2   g065(.a(new_n127_), .b(x05), .O(new_n128_));
  aoi21  g066(.a(new_n125_), .b(x27), .c(new_n128_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  oai21  g068(.a(new_n129_), .b(x04), .c(new_n130_), .O(z10));
  nand2  g069(.a(new_n65_), .b(new_n98_), .O(new_n132_));
  oai21  g070(.a(new_n91_), .b(x04), .c(x40), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n132_), .c(x08), .O(new_n134_));
  nand2  g072(.a(x39), .b(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x40), .c(new_n122_), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n83_), .c(new_n136_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n134_), .c(new_n80_), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(x09), .O(z11));
  nor3   g078(.a(x37), .b(x36), .c(x35), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n98_), .c(new_n74_), .O(new_n142_));
  inv1   g080(.a(z08), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(x04), .c(new_n69_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n142_), .c(new_n65_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x28), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x39), .c(new_n146_), .d(new_n72_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand4  g089(.a(new_n68_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n149_), .c(new_n64_), .O(new_n153_));
  nor2   g091(.a(new_n152_), .b(new_n63_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n153_), .c(x27), .O(new_n155_));
  nand3  g093(.a(new_n127_), .b(new_n146_), .c(new_n72_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n155_), .O(z13));
  nand3  g095(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n158_));
  nand4  g096(.a(new_n147_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  nand3  g098(.a(z08), .b(new_n146_), .c(new_n72_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n160_), .c(new_n158_), .O(new_n162_));
  nand3  g100(.a(new_n66_), .b(new_n82_), .c(x28), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n81_), .c(new_n80_), .O(new_n165_));
  aoi21  g103(.a(new_n161_), .b(new_n69_), .c(new_n79_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n165_), .c(new_n162_), .O(z14));
  nand4  g105(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n65_), .O(z15));
  inv1   g107(.a(x23), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(x22), .c(x01), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n65_), .O(z16));
  inv1   g110(.a(x01), .O(new_n173_));
  inv1   g111(.a(x24), .O(new_n174_));
  nand4  g112(.a(new_n65_), .b(new_n174_), .c(x23), .d(x22), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n173_), .O(z17));
  nand2  g114(.a(x27), .b(new_n72_), .O(new_n177_));
  nand2  g115(.a(new_n98_), .b(x08), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n177_), .c(new_n82_), .O(new_n179_));
  inv1   g117(.a(x09), .O(new_n180_));
  nand2  g118(.a(new_n80_), .b(new_n180_), .O(new_n181_));
  aoi21  g119(.a(new_n179_), .b(new_n90_), .c(new_n181_), .O(new_n182_));
  oai22  g120(.a(new_n182_), .b(new_n79_), .c(new_n143_), .d(new_n88_), .O(z18));
  nor2   g121(.a(x40), .b(x35), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n92_), .c(x06), .O(new_n185_));
  nand2  g123(.a(new_n137_), .b(new_n90_), .O(new_n186_));
  nand2  g124(.a(z08), .b(x29), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n186_), .O(new_n188_));
  inv1   g126(.a(x08), .O(new_n189_));
  nand2  g127(.a(x35), .b(x27), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(x04), .c(new_n189_), .O(new_n191_));
  nand2  g129(.a(new_n135_), .b(new_n82_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n191_), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(x40), .O(new_n194_));
  nand2  g132(.a(x28), .b(new_n189_), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n194_), .c(new_n188_), .d(new_n185_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n80_), .c(new_n180_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n65_), .O(z19));
  aoi21  g136(.a(x28), .b(new_n98_), .c(new_n82_), .O(new_n199_));
  nor2   g137(.a(new_n199_), .b(new_n136_), .O(new_n200_));
  nand2  g138(.a(x40), .b(new_n82_), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(new_n132_), .c(x08), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n200_), .c(new_n80_), .O(new_n203_));
  nor2   g141(.a(new_n203_), .b(x09), .O(z20));
endmodule


