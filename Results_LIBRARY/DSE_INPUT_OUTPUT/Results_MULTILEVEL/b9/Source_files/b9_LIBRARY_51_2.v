// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n167_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x04), .O(new_n63_));
  nor2   g001(.a(x30), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x15), .c(x16), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n63_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n71_), .O(z00));
  nand2  g016(.a(x30), .b(x04), .O(z01));
  aoi21  g017(.a(x29), .b(x08), .c(x02), .O(new_n80_));
  nand2  g018(.a(x40), .b(x39), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n80_), .c(new_n65_), .O(new_n82_));
  inv1   g020(.a(x08), .O(new_n83_));
  inv1   g021(.a(x27), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(x30), .c(x04), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n82_), .O(z02));
  nand2  g028(.a(new_n67_), .b(x27), .O(new_n91_));
  inv1   g029(.a(x21), .O(new_n92_));
  aoi21  g030(.a(new_n66_), .b(new_n84_), .c(new_n92_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n91_), .c(new_n64_), .O(z03));
  nand2  g032(.a(new_n66_), .b(new_n84_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n91_), .c(new_n65_), .d(new_n92_), .O(z04));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n65_), .c(new_n66_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x33), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n72_), .O(new_n101_));
  inv1   g039(.a(x00), .O(new_n102_));
  oai21  g040(.a(x25), .b(new_n102_), .c(x38), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(new_n107_));
  oai22  g045(.a(new_n107_), .b(x14), .c(new_n104_), .d(x31), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n65_), .c(x03), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z07));
  inv1   g048(.a(x39), .O(new_n111_));
  nor3   g049(.a(new_n64_), .b(new_n74_), .c(new_n111_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(x36), .O(new_n116_));
  oai21  g054(.a(new_n86_), .b(x28), .c(new_n116_), .O(new_n117_));
  nand2  g055(.a(new_n81_), .b(x07), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n117_), .c(x27), .O(new_n121_));
  oai21  g059(.a(x32), .b(x30), .c(x40), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n111_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n63_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n65_), .O(z10));
  inv1   g066(.a(x09), .O(new_n129_));
  inv1   g067(.a(x30), .O(new_n130_));
  nand4  g068(.a(x40), .b(x39), .c(x29), .d(x08), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n87_), .c(new_n85_), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(new_n130_), .c(new_n129_), .d(new_n63_), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(z11));
  nor3   g073(.a(x37), .b(x36), .c(x35), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n84_), .c(new_n73_), .O(new_n137_));
  oai21  g075(.a(new_n81_), .b(x04), .c(new_n69_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n65_), .O(z12));
  inv1   g077(.a(x13), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(x40), .c(x39), .d(new_n140_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand4  g083(.a(new_n68_), .b(x20), .c(new_n145_), .d(new_n144_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n143_), .c(x04), .O(new_n147_));
  nand4  g085(.a(new_n68_), .b(x30), .c(x20), .d(new_n145_), .O(new_n148_));
  nor2   g086(.a(new_n148_), .b(x18), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n147_), .c(x27), .O(new_n150_));
  nand3  g088(.a(new_n123_), .b(new_n140_), .c(new_n63_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z13));
  nand3  g090(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n153_));
  inv1   g091(.a(x32), .O(new_n154_));
  nand4  g092(.a(new_n141_), .b(new_n154_), .c(new_n130_), .d(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  nand4  g094(.a(x40), .b(x39), .c(new_n140_), .d(new_n63_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n156_), .c(new_n153_), .O(new_n158_));
  nand3  g096(.a(new_n66_), .b(new_n86_), .c(x28), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x27), .c(x32), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(x04), .c(new_n130_), .O(new_n161_));
  nand2  g099(.a(new_n157_), .b(new_n69_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n161_), .c(new_n158_), .O(z14));
  inv1   g101(.a(x12), .O(new_n164_));
  nor2   g102(.a(new_n114_), .b(new_n164_), .O(z15));
  inv1   g103(.a(x23), .O(new_n166_));
  nand4  g104(.a(new_n65_), .b(new_n166_), .c(x22), .d(x01), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z16));
  inv1   g106(.a(x24), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(x23), .c(x22), .d(x01), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n65_), .O(z17));
  nand3  g109(.a(x40), .b(x39), .c(x29), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n87_), .c(x08), .O(new_n174_));
  aoi21  g112(.a(new_n87_), .b(x27), .c(x30), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(new_n174_), .c(new_n129_), .d(new_n63_), .O(z18));
  oai21  g114(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n177_));
  aoi21  g115(.a(x28), .b(new_n84_), .c(new_n86_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n173_), .c(new_n177_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n129_), .O(new_n180_));
  aoi21  g118(.a(new_n180_), .b(new_n63_), .c(x30), .O(z20));
  inv1   g119(.a(new_n134_), .O(z19));
endmodule


