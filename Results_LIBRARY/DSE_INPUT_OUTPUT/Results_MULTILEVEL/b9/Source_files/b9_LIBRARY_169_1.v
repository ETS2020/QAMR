// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n197_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x28), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x36), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand4  g007(.a(new_n69_), .b(x40), .c(x39), .d(new_n65_), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  inv1   g009(.a(x37), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n70_), .c(new_n64_), .O(new_n75_));
  nor2   g013(.a(x40), .b(new_n72_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x27), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n75_), .c(new_n63_), .O(new_n79_));
  inv1   g017(.a(x16), .O(new_n80_));
  nand2  g018(.a(x40), .b(new_n64_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n79_), .O(z00));
  inv1   g021(.a(x30), .O(new_n84_));
  inv1   g022(.a(x32), .O(new_n85_));
  oai21  g023(.a(new_n71_), .b(x28), .c(new_n68_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n81_), .c(x04), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g030(.a(x40), .b(x39), .O(new_n93_));
  inv1   g031(.a(new_n93_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(x28), .d(new_n65_), .O(z02));
  oai21  g033(.a(new_n71_), .b(new_n64_), .c(x27), .O(new_n96_));
  nor2   g034(.a(x37), .b(x27), .O(new_n97_));
  inv1   g035(.a(new_n97_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n96_), .c(new_n81_), .d(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand4  g038(.a(new_n98_), .b(new_n96_), .c(new_n81_), .d(new_n100_), .O(z04));
  inv1   g039(.a(new_n76_), .O(new_n102_));
  oai21  g040(.a(new_n97_), .b(new_n64_), .c(new_n102_), .O(z05));
  nand2  g041(.a(new_n102_), .b(new_n64_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n98_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n81_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  inv1   g054(.a(x39), .O(new_n117_));
  inv1   g055(.a(x40), .O(new_n118_));
  aoi21  g056(.a(new_n117_), .b(x28), .c(new_n118_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  aoi21  g060(.a(new_n68_), .b(new_n71_), .c(x28), .O(new_n123_));
  aoi21  g061(.a(x40), .b(x39), .c(new_n68_), .O(new_n124_));
  oai21  g062(.a(new_n124_), .b(new_n123_), .c(x07), .O(new_n125_));
  nand3  g063(.a(new_n94_), .b(x36), .c(x05), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n67_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n85_), .b(new_n84_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x40), .c(x39), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n127_), .c(new_n65_), .O(new_n132_));
  nand3  g070(.a(x37), .b(x27), .c(x06), .O(new_n133_));
  and2   g071(.a(new_n133_), .b(new_n81_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n132_), .O(z10));
  inv1   g073(.a(x09), .O(new_n136_));
  inv1   g074(.a(x08), .O(new_n137_));
  oai21  g075(.a(x28), .b(new_n67_), .c(new_n137_), .O(new_n138_));
  oai21  g076(.a(new_n67_), .b(new_n65_), .c(x35), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n64_), .O(new_n140_));
  nand2  g078(.a(new_n94_), .b(x29), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n140_), .c(new_n138_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n84_), .c(new_n136_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n81_), .O(z11));
  oai21  g083(.a(new_n93_), .b(x04), .c(new_n74_), .O(new_n146_));
  nor3   g084(.a(x37), .b(x36), .c(x35), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n67_), .c(new_n66_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n146_), .c(new_n104_), .O(z12));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand4  g089(.a(new_n73_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n152_));
  nor2   g090(.a(new_n71_), .b(x13), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n94_), .c(x36), .d(new_n65_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n152_), .c(new_n67_), .O(new_n155_));
  nor3   g093(.a(new_n130_), .b(x13), .c(x04), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n155_), .c(x28), .O(new_n157_));
  nand3  g095(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n78_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n157_), .O(z13));
  nand2  g099(.a(x36), .b(x35), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n85_), .c(new_n84_), .O(new_n163_));
  nor2   g101(.a(x13), .b(x04), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n163_), .c(new_n94_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n158_), .O(new_n166_));
  nand3  g104(.a(new_n164_), .b(new_n129_), .c(new_n94_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n74_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n166_), .c(new_n104_), .O(z14));
  inv1   g107(.a(x12), .O(new_n170_));
  nor2   g108(.a(new_n121_), .b(new_n170_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand4  g110(.a(new_n81_), .b(new_n172_), .c(x22), .d(x01), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z16));
  inv1   g112(.a(x01), .O(new_n175_));
  inv1   g113(.a(x24), .O(new_n176_));
  nand4  g114(.a(new_n81_), .b(new_n176_), .c(x23), .d(x22), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z17));
  nand3  g116(.a(new_n94_), .b(x29), .c(x28), .O(new_n179_));
  nor2   g117(.a(x40), .b(new_n71_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n64_), .c(new_n67_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n179_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x08), .O(new_n183_));
  oai21  g121(.a(x30), .b(x09), .c(new_n81_), .O(new_n184_));
  nand4  g122(.a(new_n180_), .b(new_n64_), .c(x27), .d(new_n65_), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n184_), .c(new_n183_), .O(z18));
  oai22  g124(.a(new_n93_), .b(new_n91_), .c(new_n71_), .d(x28), .O(new_n187_));
  nand2  g125(.a(new_n67_), .b(new_n137_), .O(new_n188_));
  nand3  g126(.a(new_n188_), .b(new_n187_), .c(new_n140_), .O(new_n189_));
  nand3  g127(.a(new_n189_), .b(new_n84_), .c(new_n136_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n81_), .O(z19));
  oai21  g129(.a(new_n71_), .b(new_n67_), .c(new_n137_), .O(new_n192_));
  oai21  g130(.a(new_n64_), .b(x27), .c(x35), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n141_), .O(new_n194_));
  nand2  g132(.a(new_n71_), .b(new_n64_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n194_), .c(new_n192_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n84_), .c(new_n136_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n81_), .O(z20));
endmodule


