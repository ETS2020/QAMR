// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x27), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  oai21  g004(.a(x36), .b(x35), .c(x39), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  oai21  g006(.a(new_n67_), .b(x04), .c(new_n68_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x27), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  nand3  g009(.a(x39), .b(x10), .c(new_n71_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x40), .c(new_n66_), .O(new_n74_));
  oai21  g012(.a(new_n65_), .b(x16), .c(new_n74_), .O(z00));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(x40), .c(new_n78_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n77_), .c(new_n76_), .O(new_n83_));
  oai21  g021(.a(new_n65_), .b(x04), .c(new_n83_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x29), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g025(.a(x39), .O(new_n88_));
  aoi21  g026(.a(new_n80_), .b(x04), .c(new_n88_), .O(new_n89_));
  oai21  g027(.a(new_n87_), .b(x02), .c(new_n89_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x40), .O(new_n91_));
  aoi21  g029(.a(new_n85_), .b(x04), .c(new_n63_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x27), .c(new_n91_), .O(z02));
  nand2  g031(.a(x40), .b(x35), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n79_), .c(x27), .O(new_n95_));
  inv1   g033(.a(x27), .O(new_n96_));
  inv1   g034(.a(x37), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n95_), .c(new_n100_), .O(z04));
  nand3  g039(.a(x40), .b(x28), .c(x27), .O(new_n102_));
  oai21  g040(.a(new_n65_), .b(new_n97_), .c(new_n102_), .O(z05));
  nand2  g041(.a(x40), .b(new_n79_), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(x27), .c(x37), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z07));
  nand2  g054(.a(x40), .b(x39), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n64_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n63_), .O(z09));
  nand2  g058(.a(new_n80_), .b(new_n78_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(x39), .b(x05), .O(new_n123_));
  oai21  g061(.a(x39), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n121_), .c(x27), .O(new_n125_));
  oai21  g063(.a(x32), .b(x30), .c(x39), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n71_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n130_), .c(new_n63_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  nand2  g071(.a(x39), .b(x29), .O(new_n134_));
  nor2   g072(.a(new_n96_), .b(new_n71_), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n80_), .c(new_n134_), .O(new_n136_));
  oai21  g074(.a(new_n63_), .b(new_n85_), .c(new_n80_), .O(new_n137_));
  oai21  g075(.a(new_n96_), .b(x04), .c(new_n85_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n136_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n76_), .c(new_n133_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n64_), .O(z11));
  inv1   g079(.a(x10), .O(new_n142_));
  inv1   g080(.a(x35), .O(new_n143_));
  nand4  g081(.a(x40), .b(new_n97_), .c(new_n78_), .d(new_n143_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g084(.a(x35), .b(x28), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(x40), .c(new_n97_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  oai21  g087(.a(new_n88_), .b(x04), .c(new_n149_), .O(new_n150_));
  nand2  g088(.a(new_n63_), .b(new_n96_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n150_), .c(new_n146_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x28), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n71_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  inv1   g096(.a(new_n68_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x27), .O(new_n162_));
  nand3  g100(.a(new_n127_), .b(new_n153_), .c(new_n71_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n162_), .c(new_n63_), .O(z13));
  nand3  g102(.a(x39), .b(new_n153_), .c(new_n71_), .O(new_n165_));
  nand3  g103(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n68_), .c(x40), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x27), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nand2  g107(.a(new_n166_), .b(new_n154_), .O(new_n170_));
  nand2  g108(.a(new_n97_), .b(new_n143_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(x40), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n77_), .c(new_n76_), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n169_), .c(new_n151_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n63_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand4  g117(.a(new_n64_), .b(new_n179_), .c(x22), .d(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(x23), .c(x22), .d(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n64_), .O(z17));
  oai22  g122(.a(new_n117_), .b(new_n86_), .c(new_n80_), .d(x27), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(x08), .O(new_n186_));
  oai21  g124(.a(x30), .b(x09), .c(new_n64_), .O(new_n187_));
  inv1   g125(.a(new_n94_), .O(new_n188_));
  nand4  g126(.a(new_n188_), .b(new_n79_), .c(x27), .d(new_n71_), .O(new_n189_));
  nand3  g127(.a(new_n189_), .b(new_n187_), .c(new_n186_), .O(z18));
  oai21  g128(.a(new_n143_), .b(new_n96_), .c(new_n85_), .O(new_n191_));
  oai21  g129(.a(new_n79_), .b(x27), .c(x35), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n134_), .O(new_n193_));
  nand2  g131(.a(new_n80_), .b(new_n63_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n193_), .c(new_n191_), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n76_), .c(new_n133_), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n64_), .O(z20));
  nand2  g135(.a(new_n140_), .b(new_n64_), .O(z19));
endmodule


