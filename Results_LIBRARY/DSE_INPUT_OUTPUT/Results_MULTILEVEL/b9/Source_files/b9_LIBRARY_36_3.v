// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_;
  nand2  g000(.a(x40), .b(x30), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x27), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  oai21  g013(.a(x36), .b(x35), .c(x27), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(x39), .c(new_n73_), .d(new_n65_), .O(new_n78_));
  oai22  g016(.a(new_n78_), .b(x04), .c(new_n72_), .d(new_n64_), .O(z00));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n73_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g026(.a(x27), .b(x08), .c(new_n82_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x04), .O(new_n90_));
  and2   g028(.a(x40), .b(x39), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n90_), .c(new_n88_), .d(new_n73_), .O(z02));
  nand2  g030(.a(new_n68_), .b(x27), .O(new_n93_));
  inv1   g031(.a(x27), .O(new_n94_));
  aoi21  g032(.a(new_n67_), .b(new_n94_), .c(new_n64_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n93_), .c(new_n97_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n67_), .c(new_n64_), .O(z05));
  nand3  g038(.a(new_n99_), .b(new_n63_), .c(new_n67_), .O(new_n101_));
  inv1   g039(.a(new_n101_), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n65_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n63_), .O(z07));
  nand3  g051(.a(x40), .b(x39), .c(new_n73_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n63_), .O(z09));
  inv1   g055(.a(x04), .O(new_n118_));
  inv1   g056(.a(x28), .O(new_n119_));
  aoi21  g057(.a(x35), .b(new_n119_), .c(x36), .O(new_n120_));
  oai21  g058(.a(x39), .b(x30), .c(x40), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x07), .O(new_n122_));
  nand3  g060(.a(new_n91_), .b(new_n73_), .c(x05), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n120_), .O(new_n124_));
  nor2   g062(.a(new_n64_), .b(new_n67_), .O(new_n125_));
  aoi22  g063(.a(new_n125_), .b(x06), .c(new_n124_), .d(new_n118_), .O(new_n126_));
  nand2  g064(.a(new_n91_), .b(x32), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(new_n73_), .c(x05), .d(new_n118_), .O(new_n129_));
  oai21  g067(.a(new_n126_), .b(new_n94_), .c(new_n129_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  nand2  g069(.a(new_n91_), .b(x29), .O(new_n132_));
  oai21  g070(.a(new_n94_), .b(new_n118_), .c(new_n82_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  inv1   g072(.a(x08), .O(new_n135_));
  nand3  g073(.a(new_n82_), .b(x27), .c(new_n118_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n73_), .c(new_n131_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  nor3   g078(.a(x37), .b(x36), .c(x35), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n94_), .c(new_n74_), .O(new_n142_));
  nand2  g080(.a(new_n91_), .b(new_n118_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n70_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n142_), .c(new_n63_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x28), .c(new_n75_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x39), .c(new_n146_), .d(new_n118_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand4  g089(.a(new_n69_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n149_), .c(x30), .O(new_n153_));
  nand4  g091(.a(new_n69_), .b(new_n75_), .c(x20), .d(new_n151_), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(x18), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n153_), .c(x27), .O(new_n156_));
  nand4  g094(.a(new_n128_), .b(new_n73_), .c(new_n146_), .d(new_n118_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(z13));
  nand3  g096(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n159_));
  nand3  g097(.a(new_n147_), .b(new_n80_), .c(x28), .O(new_n160_));
  nand3  g098(.a(x39), .b(new_n146_), .c(new_n118_), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n160_), .c(x30), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n75_), .c(new_n159_), .O(new_n164_));
  nand3  g102(.a(new_n67_), .b(new_n81_), .c(x28), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x27), .c(x32), .O(new_n166_));
  nor2   g104(.a(new_n162_), .b(new_n71_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n166_), .c(new_n73_), .O(new_n168_));
  nand2  g106(.a(new_n70_), .b(new_n75_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n168_), .c(new_n164_), .O(z14));
  nand4  g108(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n63_), .O(z15));
  inv1   g110(.a(x23), .O(new_n173_));
  nand4  g111(.a(new_n63_), .b(new_n173_), .c(x22), .d(x01), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(z16));
  inv1   g113(.a(x24), .O(new_n176_));
  nand4  g114(.a(new_n176_), .b(x23), .c(x22), .d(x01), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n63_), .O(z17));
  nand2  g116(.a(new_n82_), .b(new_n94_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n132_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x08), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(new_n136_), .c(new_n73_), .d(new_n131_), .O(z18));
  nand2  g120(.a(new_n139_), .b(new_n63_), .O(z19));
  inv1   g121(.a(new_n132_), .O(new_n184_));
  oai21  g122(.a(new_n81_), .b(new_n94_), .c(new_n135_), .O(new_n185_));
  aoi21  g123(.a(x28), .b(new_n94_), .c(new_n81_), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n184_), .c(new_n185_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n73_), .c(new_n131_), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n63_), .O(z20));
endmodule


