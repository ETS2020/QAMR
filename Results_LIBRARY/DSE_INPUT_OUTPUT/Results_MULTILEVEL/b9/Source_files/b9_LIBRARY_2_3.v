// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n199_, new_n200_,
    new_n201_, new_n202_;
  nand2  g000(.a(x40), .b(x23), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x23), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  or2    g020(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n81_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x39), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n86_), .c(new_n72_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x40), .O(z02));
  nand2  g030(.a(new_n69_), .b(x27), .O(new_n93_));
  aoi21  g031(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n93_), .c(new_n96_), .O(z04));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n63_), .c(new_n68_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x33), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n65_), .O(new_n102_));
  inv1   g040(.a(x00), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n103_), .c(x38), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n103_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  oai21  g047(.a(new_n105_), .b(x31), .c(new_n109_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n63_), .O(z07));
  oai21  g050(.a(x39), .b(x23), .c(x40), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n63_), .O(z09));
  inv1   g054(.a(x04), .O(new_n117_));
  nand2  g055(.a(new_n113_), .b(x07), .O(new_n118_));
  nand4  g056(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n118_), .c(new_n82_), .O(new_n120_));
  nand3  g058(.a(new_n63_), .b(x37), .c(x06), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n117_), .c(new_n122_), .O(new_n123_));
  inv1   g061(.a(x39), .O(new_n124_));
  nand2  g062(.a(new_n80_), .b(new_n79_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x40), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(new_n72_), .c(x05), .d(new_n117_), .O(new_n128_));
  oai21  g066(.a(new_n123_), .b(new_n67_), .c(new_n128_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x04), .O(new_n131_));
  inv1   g069(.a(x09), .O(new_n132_));
  nand2  g070(.a(new_n79_), .b(new_n132_), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n131_), .c(new_n72_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nand3  g073(.a(x40), .b(x39), .c(x29), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n81_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  oai21  g077(.a(new_n87_), .b(new_n67_), .c(new_n130_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n79_), .c(new_n132_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n135_), .O(z11));
  inv1   g081(.a(x35), .O(new_n144_));
  inv1   g082(.a(x36), .O(new_n145_));
  nand3  g083(.a(new_n68_), .b(new_n145_), .c(new_n144_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x27), .c(x10), .O(new_n147_));
  aoi21  g085(.a(x39), .b(new_n117_), .c(new_n70_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n147_), .c(new_n72_), .O(new_n149_));
  oai21  g087(.a(new_n70_), .b(x40), .c(new_n149_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n74_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n151_), .d(new_n117_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand2  g094(.a(new_n69_), .b(new_n68_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n154_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x27), .O(new_n160_));
  nand4  g098(.a(new_n125_), .b(x39), .c(new_n151_), .d(new_n117_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n72_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x40), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n160_), .O(z13));
  nand3  g102(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n165_));
  nand4  g103(.a(new_n152_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(new_n167_));
  nand4  g105(.a(x40), .b(x39), .c(new_n151_), .d(new_n117_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n167_), .c(new_n165_), .O(new_n169_));
  nand3  g107(.a(new_n68_), .b(new_n144_), .c(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n80_), .c(new_n79_), .O(new_n172_));
  inv1   g110(.a(new_n70_), .O(new_n173_));
  aoi21  g111(.a(new_n168_), .b(new_n173_), .c(new_n64_), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n172_), .c(new_n169_), .O(z14));
  nand4  g113(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n63_), .O(z15));
  nand2  g115(.a(x22), .b(x01), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(x23), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(x22), .c(x01), .O(new_n181_));
  aoi21  g119(.a(new_n181_), .b(new_n74_), .c(new_n72_), .O(z17));
  nand2  g120(.a(x27), .b(new_n117_), .O(new_n183_));
  nand2  g121(.a(new_n67_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n144_), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n81_), .c(new_n133_), .O(new_n186_));
  inv1   g124(.a(new_n136_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n72_), .c(x08), .O(new_n188_));
  oai21  g126(.a(new_n186_), .b(new_n64_), .c(new_n188_), .O(z18));
  aoi21  g127(.a(x40), .b(x23), .c(x27), .O(new_n190_));
  nand2  g128(.a(x40), .b(x04), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(x35), .c(new_n81_), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n72_), .c(new_n190_), .O(new_n193_));
  aoi21  g131(.a(x39), .b(x29), .c(x23), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n74_), .c(new_n138_), .O(new_n195_));
  oai21  g133(.a(new_n193_), .b(x08), .c(new_n195_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n79_), .c(new_n132_), .O(new_n197_));
  inv1   g135(.a(new_n197_), .O(z19));
  oai21  g136(.a(new_n144_), .b(new_n67_), .c(new_n130_), .O(new_n199_));
  aoi21  g137(.a(x28), .b(new_n67_), .c(new_n144_), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n187_), .c(new_n199_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n79_), .c(new_n132_), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n63_), .O(z20));
endmodule


