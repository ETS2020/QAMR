// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_;
  inv1   g000(.a(x19), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n67_), .c(new_n68_), .O(new_n75_));
  inv1   g013(.a(x04), .O(new_n76_));
  inv1   g014(.a(x10), .O(new_n77_));
  oai21  g015(.a(x36), .b(x35), .c(x27), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n77_), .c(new_n64_), .O(new_n79_));
  nand4  g017(.a(new_n79_), .b(x39), .c(new_n67_), .d(new_n76_), .O(new_n80_));
  oai21  g018(.a(new_n75_), .b(new_n66_), .c(new_n80_), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x36), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n83_), .c(new_n82_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n65_), .c(x04), .O(z01));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor2   g031(.a(x27), .b(x08), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n86_), .c(x04), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n93_), .c(x39), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x40), .O(new_n97_));
  nand2  g035(.a(new_n64_), .b(x19), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n97_), .O(z02));
  nand2  g037(.a(new_n71_), .b(x27), .O(new_n100_));
  aoi21  g038(.a(new_n70_), .b(new_n69_), .c(new_n66_), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n100_), .c(x21), .O(z03));
  inv1   g040(.a(x21), .O(new_n103_));
  nand3  g041(.a(new_n101_), .b(new_n100_), .c(new_n103_), .O(z04));
  nor2   g042(.a(new_n85_), .b(new_n69_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n65_), .c(new_n70_), .O(z05));
  oai21  g045(.a(new_n105_), .b(x37), .c(new_n65_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n67_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n65_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  nor2   g058(.a(new_n64_), .b(new_n120_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n65_), .O(z08));
  inv1   g061(.a(x11), .O(new_n124_));
  nand4  g062(.a(new_n65_), .b(x34), .c(x27), .d(x26), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n124_), .O(z09));
  inv1   g064(.a(x07), .O(new_n127_));
  nand2  g065(.a(x40), .b(new_n120_), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n98_), .c(new_n127_), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x05), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n129_), .c(new_n87_), .O(new_n132_));
  nand3  g070(.a(new_n65_), .b(x37), .c(x06), .O(new_n133_));
  oai21  g071(.a(new_n132_), .b(x04), .c(new_n133_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x27), .O(new_n135_));
  inv1   g073(.a(x05), .O(new_n136_));
  nand2  g074(.a(new_n83_), .b(new_n82_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x40), .c(x39), .O(new_n138_));
  nor2   g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n76_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n135_), .O(z10));
  inv1   g079(.a(x09), .O(new_n142_));
  nand2  g080(.a(new_n121_), .b(x29), .O(new_n143_));
  nor2   g081(.a(new_n69_), .b(new_n76_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n86_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(x08), .O(new_n146_));
  nand2  g084(.a(x27), .b(new_n76_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n86_), .c(new_n146_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n82_), .c(new_n142_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n65_), .O(z11));
  inv1   g089(.a(x35), .O(new_n152_));
  nand3  g090(.a(new_n70_), .b(new_n84_), .c(new_n152_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x27), .c(x10), .O(new_n154_));
  aoi21  g092(.a(x39), .b(new_n76_), .c(new_n74_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n154_), .c(x40), .O(new_n156_));
  nor2   g094(.a(new_n74_), .b(x40), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x19), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n156_), .O(z12));
  inv1   g097(.a(x13), .O(new_n160_));
  nand2  g098(.a(x36), .b(x35), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(x28), .c(new_n64_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x39), .c(new_n160_), .d(new_n76_), .O(new_n163_));
  inv1   g101(.a(x18), .O(new_n164_));
  nand4  g102(.a(new_n72_), .b(x20), .c(new_n63_), .d(new_n164_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n163_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x27), .O(new_n167_));
  nor2   g105(.a(new_n138_), .b(x13), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n76_), .c(new_n66_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n167_), .O(z13));
  nand3  g108(.a(x39), .b(new_n160_), .c(new_n76_), .O(new_n171_));
  nand4  g109(.a(x27), .b(x20), .c(new_n63_), .d(new_n164_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n73_), .c(new_n171_), .O(new_n173_));
  nand3  g111(.a(x20), .b(new_n63_), .c(new_n164_), .O(new_n174_));
  aoi22  g112(.a(new_n174_), .b(new_n161_), .c(new_n70_), .d(new_n152_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n85_), .c(x27), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n83_), .c(new_n82_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n173_), .c(x40), .O(z14));
  nand4  g116(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n65_), .O(z15));
  inv1   g118(.a(x23), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(x22), .c(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n65_), .O(z16));
  inv1   g121(.a(x01), .O(new_n184_));
  inv1   g122(.a(x24), .O(new_n185_));
  nand4  g123(.a(new_n65_), .b(new_n185_), .c(x23), .d(x22), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z17));
  nand2  g125(.a(new_n69_), .b(x08), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n147_), .c(new_n152_), .O(new_n189_));
  nand2  g127(.a(new_n82_), .b(new_n142_), .O(new_n190_));
  aoi21  g128(.a(new_n189_), .b(new_n85_), .c(new_n190_), .O(new_n191_));
  oai22  g129(.a(new_n191_), .b(new_n66_), .c(new_n122_), .d(new_n92_), .O(z18));
  inv1   g130(.a(new_n143_), .O(new_n193_));
  oai21  g131(.a(new_n152_), .b(new_n69_), .c(new_n146_), .O(new_n194_));
  aoi21  g132(.a(x28), .b(new_n69_), .c(new_n152_), .O(new_n195_));
  oai21  g133(.a(new_n195_), .b(new_n193_), .c(new_n194_), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(new_n82_), .c(new_n142_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n65_), .O(z20));
  nand2  g136(.a(new_n150_), .b(new_n65_), .O(z19));
endmodule


