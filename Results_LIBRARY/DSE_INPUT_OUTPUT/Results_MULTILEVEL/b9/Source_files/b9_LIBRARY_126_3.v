// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n180_, new_n181_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x05), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g004(.a(x40), .b(x39), .O(new_n67_));
  oai21  g005(.a(new_n67_), .b(new_n66_), .c(new_n63_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x10), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  inv1   g009(.a(x36), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand4  g011(.a(new_n73_), .b(x40), .c(x39), .d(new_n64_), .O(new_n74_));
  nand2  g012(.a(x35), .b(x28), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(new_n74_), .c(new_n70_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x27), .c(new_n65_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n69_), .c(x16), .O(z00));
  nand2  g016(.a(x10), .b(x05), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nor2   g020(.a(new_n71_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n80_), .O(z01));
  inv1   g024(.a(new_n67_), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  nand2  g029(.a(x35), .b(new_n91_), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n92_), .c(x04), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n90_), .c(new_n79_), .d(new_n87_), .O(z02));
  nand2  g033(.a(new_n75_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x27), .O(new_n97_));
  nand2  g035(.a(new_n70_), .b(new_n97_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n96_), .c(new_n79_), .d(x21), .O(z03));
  aoi21  g037(.a(new_n70_), .b(new_n97_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n96_), .c(new_n80_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n70_), .c(new_n80_), .O(z05));
  nand3  g041(.a(new_n102_), .b(new_n79_), .c(new_n70_), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n65_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n79_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  nand2  g054(.a(new_n79_), .b(new_n67_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n79_), .O(z09));
  nand3  g057(.a(new_n79_), .b(new_n67_), .c(x07), .O(new_n120_));
  inv1   g058(.a(x10), .O(new_n121_));
  nand4  g059(.a(x40), .b(x39), .c(new_n121_), .d(x05), .O(new_n122_));
  aoi22  g060(.a(new_n122_), .b(new_n120_), .c(new_n92_), .d(new_n72_), .O(new_n123_));
  nand3  g061(.a(new_n79_), .b(x37), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n64_), .c(new_n125_), .O(new_n126_));
  inv1   g064(.a(x39), .O(new_n127_));
  oai21  g065(.a(x32), .b(x30), .c(x40), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(new_n121_), .c(x05), .d(new_n64_), .O(new_n130_));
  oai21  g068(.a(new_n126_), .b(new_n97_), .c(new_n130_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  nor2   g071(.a(new_n97_), .b(new_n64_), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(new_n92_), .c(new_n133_), .O(new_n135_));
  inv1   g073(.a(x08), .O(new_n136_));
  nand3  g074(.a(new_n83_), .b(x27), .c(new_n64_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n81_), .c(new_n132_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n79_), .O(z11));
  nor3   g079(.a(x37), .b(x36), .c(x35), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n97_), .c(new_n121_), .O(new_n143_));
  nand2  g081(.a(new_n75_), .b(new_n70_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  oai21  g083(.a(new_n67_), .b(x04), .c(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n143_), .c(new_n79_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  inv1   g086(.a(x40), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x28), .c(new_n149_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n148_), .d(new_n64_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand4  g092(.a(new_n144_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand3  g095(.a(new_n129_), .b(new_n148_), .c(new_n64_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n157_), .c(new_n80_), .O(z13));
  nand3  g097(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n160_));
  nand4  g098(.a(new_n150_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  nand3  g100(.a(new_n87_), .b(new_n148_), .c(new_n64_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n162_), .c(new_n160_), .O(new_n164_));
  nand3  g102(.a(new_n70_), .b(new_n71_), .c(x28), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x27), .c(x32), .O(new_n166_));
  aoi22  g104(.a(new_n166_), .b(new_n81_), .c(new_n163_), .d(new_n145_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n164_), .c(new_n80_), .O(z14));
  inv1   g106(.a(x12), .O(new_n169_));
  nand4  g107(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(new_n169_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(x22), .c(x01), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n79_), .O(z16));
  inv1   g112(.a(x01), .O(new_n175_));
  inv1   g113(.a(x24), .O(new_n176_));
  nand4  g114(.a(new_n79_), .b(new_n176_), .c(x23), .d(x22), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z17));
  oai21  g116(.a(new_n92_), .b(x27), .c(new_n133_), .O(new_n179_));
  nand3  g117(.a(new_n137_), .b(new_n81_), .c(new_n132_), .O(new_n180_));
  aoi21  g118(.a(new_n179_), .b(x08), .c(new_n180_), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n80_), .O(z18));
  oai21  g120(.a(new_n71_), .b(new_n97_), .c(new_n136_), .O(new_n183_));
  nand2  g121(.a(new_n133_), .b(new_n71_), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n80_), .O(new_n185_));
  inv1   g123(.a(new_n133_), .O(new_n186_));
  nor2   g124(.a(x28), .b(x04), .O(new_n187_));
  aoi21  g125(.a(x27), .b(x04), .c(x28), .O(new_n188_));
  oai22  g126(.a(new_n188_), .b(new_n186_), .c(new_n187_), .d(x08), .O(new_n189_));
  oai21  g127(.a(new_n189_), .b(new_n185_), .c(new_n81_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(x09), .c(new_n79_), .O(z19));
  aoi21  g129(.a(x28), .b(new_n97_), .c(new_n71_), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n186_), .c(new_n183_), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(new_n79_), .c(new_n81_), .d(new_n132_), .O(new_n194_));
  inv1   g132(.a(new_n194_), .O(z20));
endmodule


