// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  nand2  g015(.a(x40), .b(x31), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nor2   g019(.a(new_n66_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g023(.a(x31), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(new_n82_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(x04), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n89_), .c(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n86_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(z02));
  inv1   g033(.a(new_n78_), .O(new_n96_));
  inv1   g034(.a(x28), .O(new_n97_));
  oai21  g035(.a(new_n66_), .b(new_n97_), .c(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(x37), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n64_), .c(new_n99_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n96_), .O(z03));
  aoi21  g040(.a(new_n100_), .b(new_n64_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n98_), .c(new_n96_), .O(z04));
  nor2   g042(.a(new_n97_), .b(new_n64_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n78_), .c(new_n100_), .O(z05));
  oai21  g045(.a(new_n105_), .b(x37), .c(new_n78_), .O(z06));
  inv1   g046(.a(x03), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n63_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  inv1   g051(.a(x14), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n111_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  oai21  g055(.a(new_n113_), .b(x33), .c(new_n117_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n86_), .O(new_n119_));
  nand3  g057(.a(new_n116_), .b(new_n73_), .c(new_n114_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(new_n109_), .O(z07));
  nand3  g059(.a(x40), .b(x39), .c(new_n86_), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n78_), .O(z09));
  aoi21  g063(.a(x35), .b(new_n97_), .c(x36), .O(new_n126_));
  oai21  g064(.a(x39), .b(x31), .c(x40), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x07), .O(new_n128_));
  nand3  g066(.a(new_n74_), .b(new_n86_), .c(x05), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n128_), .c(new_n126_), .O(new_n130_));
  nand3  g068(.a(new_n78_), .b(x37), .c(x06), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  aoi21  g070(.a(new_n130_), .b(new_n65_), .c(new_n132_), .O(new_n133_));
  oai21  g071(.a(x32), .b(x30), .c(x40), .O(new_n134_));
  nor3   g072(.a(new_n134_), .b(new_n72_), .c(x31), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x05), .c(new_n65_), .O(new_n136_));
  oai21  g074(.a(new_n133_), .b(new_n64_), .c(new_n136_), .O(z10));
  inv1   g075(.a(x08), .O(new_n138_));
  inv1   g076(.a(x09), .O(new_n139_));
  nand4  g077(.a(new_n80_), .b(new_n139_), .c(new_n138_), .d(x04), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n86_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x40), .O(new_n142_));
  nand2  g080(.a(new_n74_), .b(x29), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(x35), .c(new_n97_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  oai21  g084(.a(new_n90_), .b(new_n64_), .c(new_n138_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n80_), .c(new_n139_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n142_), .O(z11));
  nor2   g088(.a(new_n70_), .b(new_n64_), .O(new_n151_));
  nand3  g089(.a(new_n100_), .b(new_n67_), .c(new_n66_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x27), .c(x10), .O(new_n153_));
  aoi21  g091(.a(x39), .b(new_n65_), .c(new_n151_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n153_), .c(new_n86_), .O(new_n155_));
  oai21  g093(.a(new_n151_), .b(x40), .c(new_n155_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x28), .c(new_n73_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  inv1   g100(.a(new_n70_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n160_), .c(x31), .O(new_n165_));
  nand4  g103(.a(new_n163_), .b(new_n73_), .c(x20), .d(new_n162_), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x18), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n165_), .c(x27), .O(new_n168_));
  nand3  g106(.a(new_n135_), .b(new_n157_), .c(new_n65_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n168_), .O(z13));
  nand3  g108(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n171_));
  nand4  g109(.a(new_n158_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(new_n173_));
  nand3  g111(.a(new_n74_), .b(new_n157_), .c(new_n65_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n173_), .c(new_n171_), .O(new_n175_));
  nand3  g113(.a(new_n100_), .b(new_n66_), .c(x28), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n81_), .c(new_n80_), .O(new_n178_));
  inv1   g116(.a(new_n151_), .O(new_n179_));
  nand2  g117(.a(new_n174_), .b(new_n179_), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n178_), .c(new_n175_), .d(new_n78_), .O(z14));
  nand4  g119(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n78_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand4  g122(.a(new_n78_), .b(new_n184_), .c(x22), .d(x01), .O(new_n185_));
  inv1   g123(.a(new_n185_), .O(z16));
  inv1   g124(.a(x01), .O(new_n187_));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n78_), .b(new_n188_), .c(x23), .d(x22), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n187_), .O(z17));
  oai21  g128(.a(new_n90_), .b(x27), .c(new_n143_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(x08), .O(new_n192_));
  nor2   g130(.a(new_n64_), .b(x04), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(new_n82_), .c(x09), .O(new_n194_));
  nand4  g132(.a(new_n194_), .b(new_n192_), .c(new_n78_), .d(new_n80_), .O(z18));
  nand2  g133(.a(x40), .b(x04), .O(new_n196_));
  nand3  g134(.a(new_n196_), .b(x35), .c(new_n97_), .O(new_n197_));
  aoi22  g135(.a(new_n197_), .b(new_n86_), .c(new_n78_), .d(new_n64_), .O(new_n198_));
  aoi21  g136(.a(x39), .b(x29), .c(x31), .O(new_n199_));
  oai21  g137(.a(new_n199_), .b(new_n73_), .c(new_n145_), .O(new_n200_));
  oai21  g138(.a(new_n198_), .b(x08), .c(new_n200_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n80_), .c(new_n139_), .O(new_n202_));
  inv1   g140(.a(new_n202_), .O(z19));
  inv1   g141(.a(new_n143_), .O(new_n204_));
  oai21  g142(.a(new_n66_), .b(new_n64_), .c(new_n138_), .O(new_n205_));
  aoi21  g143(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n206_));
  oai21  g144(.a(new_n206_), .b(new_n204_), .c(new_n205_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n80_), .c(new_n139_), .O(new_n208_));
  nand2  g146(.a(new_n208_), .b(new_n78_), .O(z20));
endmodule


