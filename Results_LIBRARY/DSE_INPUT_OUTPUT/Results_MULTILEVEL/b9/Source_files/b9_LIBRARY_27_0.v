// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  inv1   g017(.a(x16), .O(new_n80_));
  nor2   g018(.a(x30), .b(x04), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n79_), .O(z00));
  inv1   g021(.a(x30), .O(new_n84_));
  oai21  g022(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x27), .c(x32), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n84_), .c(x04), .O(new_n87_));
  oai21  g025(.a(new_n84_), .b(x04), .c(new_n87_), .O(z01));
  inv1   g026(.a(new_n81_), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g030(.a(x28), .O(new_n93_));
  nand2  g031(.a(x35), .b(new_n93_), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n94_), .c(x04), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n92_), .c(new_n89_), .d(new_n76_), .O(z02));
  nand2  g035(.a(new_n71_), .b(x27), .O(new_n98_));
  aoi21  g036(.a(new_n70_), .b(new_n64_), .c(new_n81_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n98_), .c(new_n101_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n81_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x14), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n108_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  oai21  g052(.a(new_n110_), .b(x31), .c(new_n114_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n89_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  nand2  g055(.a(new_n89_), .b(new_n75_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n89_), .O(z09));
  nand3  g058(.a(new_n89_), .b(x37), .c(x06), .O(new_n121_));
  nand4  g059(.a(new_n85_), .b(new_n75_), .c(x30), .d(x07), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(x04), .c(new_n121_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x27), .O(new_n124_));
  nand4  g062(.a(new_n76_), .b(x30), .c(x05), .d(new_n65_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(z10));
  inv1   g064(.a(x09), .O(new_n127_));
  inv1   g065(.a(x08), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  nand3  g067(.a(x35), .b(new_n93_), .c(new_n64_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n129_), .c(new_n128_), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(x30), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n127_), .c(x04), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  nand3  g072(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x27), .c(x10), .O(new_n136_));
  aoi21  g074(.a(new_n72_), .b(x27), .c(new_n76_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n136_), .c(x30), .O(new_n138_));
  oai21  g076(.a(new_n73_), .b(new_n64_), .c(x04), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(z12));
  inv1   g078(.a(x19), .O(new_n141_));
  nor2   g079(.a(new_n81_), .b(new_n73_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(x27), .c(x20), .d(new_n141_), .O(new_n143_));
  inv1   g081(.a(x13), .O(new_n144_));
  nand4  g082(.a(new_n76_), .b(x30), .c(new_n144_), .d(new_n65_), .O(new_n145_));
  oai21  g083(.a(new_n143_), .b(x18), .c(new_n145_), .O(z13));
  oai21  g084(.a(new_n75_), .b(x13), .c(x30), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  nand4  g086(.a(x27), .b(x20), .c(new_n141_), .d(new_n148_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(new_n150_));
  aoi22  g088(.a(new_n150_), .b(new_n72_), .c(new_n147_), .d(new_n65_), .O(z14));
  inv1   g089(.a(x12), .O(new_n152_));
  nand4  g090(.a(new_n89_), .b(x34), .c(x27), .d(x26), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(new_n152_), .O(z15));
  inv1   g092(.a(x23), .O(new_n155_));
  nand4  g093(.a(new_n89_), .b(new_n155_), .c(x22), .d(x01), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(x23), .c(x22), .d(x01), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n89_), .O(z17));
  oai21  g098(.a(new_n131_), .b(x09), .c(x04), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n84_), .O(z18));
  inv1   g100(.a(new_n129_), .O(new_n163_));
  oai21  g101(.a(new_n66_), .b(new_n64_), .c(new_n128_), .O(new_n164_));
  aoi21  g102(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n163_), .c(new_n164_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n127_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(x04), .c(x30), .O(z20));
  inv1   g106(.a(new_n133_), .O(z19));
endmodule


