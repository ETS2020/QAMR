// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand4  g012(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n74_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x40), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x27), .c(x04), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n79_), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  inv1   g025(.a(new_n87_), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n78_), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n67_), .c(new_n82_), .d(new_n81_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n85_), .O(z01));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n87_), .c(x04), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x39), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x40), .O(new_n98_));
  nand2  g036(.a(new_n78_), .b(x36), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n98_), .O(z02));
  nand2  g038(.a(new_n71_), .b(x27), .O(new_n101_));
  inv1   g039(.a(new_n79_), .O(new_n102_));
  aoi21  g040(.a(new_n70_), .b(new_n64_), .c(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x21), .O(z03));
  inv1   g042(.a(x21), .O(new_n105_));
  nand3  g043(.a(new_n103_), .b(new_n101_), .c(new_n105_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n70_), .c(new_n102_), .O(z05));
  nand3  g046(.a(new_n107_), .b(new_n79_), .c(new_n70_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n63_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n79_), .O(z07));
  nand2  g059(.a(x40), .b(x39), .O(new_n122_));
  nand2  g060(.a(new_n79_), .b(new_n122_), .O(z08));
  nand4  g061(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n79_), .O(z09));
  oai21  g063(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n126_));
  inv1   g064(.a(x07), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x05), .O(new_n128_));
  oai21  g066(.a(x39), .b(new_n127_), .c(new_n128_), .O(new_n129_));
  nor2   g067(.a(x40), .b(new_n127_), .O(new_n130_));
  aoi21  g068(.a(new_n129_), .b(new_n126_), .c(new_n130_), .O(new_n131_));
  nand4  g069(.a(new_n83_), .b(x40), .c(x39), .d(x05), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(new_n64_), .c(new_n132_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n65_), .O(new_n134_));
  nor2   g072(.a(new_n70_), .b(new_n64_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x06), .c(new_n102_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n134_), .O(z10));
  nand2  g075(.a(new_n79_), .b(new_n64_), .O(new_n138_));
  nand3  g076(.a(x35), .b(new_n86_), .c(new_n65_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x40), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n138_), .c(x08), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x40), .O(new_n143_));
  nand2  g081(.a(x27), .b(x04), .O(new_n144_));
  aoi22  g082(.a(new_n144_), .b(new_n88_), .c(new_n143_), .d(new_n99_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n141_), .c(new_n81_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(x09), .O(z11));
  inv1   g085(.a(x10), .O(new_n148_));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n64_), .c(new_n148_), .O(new_n150_));
  oai22  g088(.a(new_n122_), .b(x04), .c(new_n73_), .d(new_n64_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n150_), .c(new_n79_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n78_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n65_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n72_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(new_n83_), .b(x40), .c(x39), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(x13), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n65_), .c(new_n102_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z13));
  nand2  g103(.a(new_n153_), .b(new_n65_), .O(new_n166_));
  nand4  g104(.a(x27), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n167_));
  oai22  g105(.a(new_n167_), .b(new_n73_), .c(new_n166_), .d(new_n122_), .O(new_n168_));
  nand3  g106(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n169_));
  aoi22  g107(.a(new_n169_), .b(new_n154_), .c(new_n70_), .d(new_n66_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n86_), .c(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n82_), .c(new_n81_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n168_), .c(new_n79_), .O(z14));
  inv1   g111(.a(x12), .O(new_n174_));
  nand4  g112(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n174_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(x22), .c(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n79_), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(x23), .c(x22), .d(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n79_), .O(z17));
  nand2  g120(.a(x27), .b(new_n65_), .O(new_n183_));
  nand2  g121(.a(new_n64_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n66_), .O(new_n185_));
  inv1   g123(.a(x09), .O(new_n186_));
  nand2  g124(.a(new_n81_), .b(new_n186_), .O(new_n187_));
  aoi21  g125(.a(new_n185_), .b(new_n86_), .c(new_n187_), .O(new_n188_));
  oai22  g126(.a(new_n188_), .b(new_n102_), .c(new_n93_), .d(new_n122_), .O(z18));
  nand2  g127(.a(x28), .b(new_n64_), .O(new_n190_));
  aoi22  g128(.a(new_n190_), .b(x35), .c(new_n143_), .d(new_n99_), .O(new_n191_));
  nand2  g129(.a(x40), .b(new_n66_), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n138_), .c(x08), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n191_), .c(new_n81_), .O(new_n194_));
  nor2   g132(.a(new_n194_), .b(x09), .O(z20));
  nor2   g133(.a(new_n146_), .b(x09), .O(z19));
endmodule


