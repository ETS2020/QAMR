// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n196_, new_n197_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x31), .O(new_n78_));
  nand2  g016(.a(x40), .b(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g018(.a(new_n79_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n83_), .c(new_n82_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n81_), .O(z01));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n85_), .c(x04), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n74_), .d(x31), .O(z02));
  oai21  g033(.a(new_n66_), .b(new_n84_), .c(x27), .O(new_n96_));
  inv1   g034(.a(x21), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n64_), .c(new_n97_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n81_), .O(z03));
  nand2  g038(.a(new_n98_), .b(new_n64_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n96_), .c(new_n79_), .d(new_n97_), .O(z04));
  oai21  g040(.a(new_n84_), .b(new_n64_), .c(new_n98_), .O(new_n103_));
  and2   g041(.a(new_n103_), .b(new_n79_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand4  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .d(new_n78_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(x14), .c(new_n110_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n79_), .O(z07));
  aoi21  g054(.a(new_n72_), .b(x31), .c(new_n73_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n79_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  nand4  g058(.a(x40), .b(x39), .c(x31), .d(x05), .O(new_n121_));
  oai21  g059(.a(z08), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n86_), .c(new_n65_), .O(new_n123_));
  nand3  g061(.a(new_n79_), .b(x37), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x27), .O(new_n126_));
  oai21  g064(.a(x32), .b(x30), .c(x40), .O(new_n127_));
  nor3   g065(.a(new_n127_), .b(new_n72_), .c(new_n78_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(x05), .c(new_n65_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n126_), .O(z10));
  nand2  g068(.a(new_n79_), .b(new_n64_), .O(new_n131_));
  oai21  g069(.a(new_n85_), .b(x04), .c(x31), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n131_), .c(x08), .O(new_n133_));
  inv1   g071(.a(new_n85_), .O(new_n134_));
  nand2  g072(.a(x39), .b(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x31), .c(new_n73_), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n134_), .c(new_n136_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n133_), .c(new_n82_), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(x09), .O(z11));
  nor2   g078(.a(new_n70_), .b(new_n64_), .O(new_n141_));
  nand3  g079(.a(new_n98_), .b(new_n67_), .c(new_n66_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  aoi21  g081(.a(x39), .b(new_n65_), .c(new_n141_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x31), .O(new_n145_));
  oai21  g083(.a(new_n141_), .b(x40), .c(new_n145_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x28), .c(new_n73_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x39), .c(new_n147_), .d(new_n65_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  inv1   g090(.a(new_n70_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n150_), .c(new_n78_), .O(new_n155_));
  nand4  g093(.a(new_n153_), .b(new_n73_), .c(x20), .d(new_n152_), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(x18), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n155_), .c(x27), .O(new_n158_));
  nand3  g096(.a(new_n128_), .b(new_n147_), .c(new_n65_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  nand3  g098(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n161_));
  nand4  g099(.a(new_n148_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  nand3  g101(.a(new_n74_), .b(new_n147_), .c(new_n65_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n163_), .c(new_n161_), .O(new_n165_));
  nand3  g103(.a(new_n98_), .b(new_n66_), .c(x28), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x27), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n83_), .c(new_n82_), .O(new_n168_));
  inv1   g106(.a(new_n141_), .O(new_n169_));
  nand2  g107(.a(new_n164_), .b(new_n169_), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n168_), .c(new_n165_), .d(new_n79_), .O(z14));
  inv1   g109(.a(x12), .O(new_n172_));
  nand4  g110(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(new_n172_), .O(z15));
  inv1   g112(.a(x23), .O(new_n175_));
  nand4  g113(.a(new_n79_), .b(new_n175_), .c(x22), .d(x01), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(z16));
  inv1   g115(.a(x01), .O(new_n178_));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n79_), .b(new_n179_), .c(x23), .d(x22), .O(new_n180_));
  nor2   g118(.a(new_n180_), .b(new_n178_), .O(z17));
  nand2  g119(.a(new_n74_), .b(x31), .O(new_n182_));
  nand2  g120(.a(x27), .b(new_n65_), .O(new_n183_));
  nand2  g121(.a(new_n64_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n66_), .O(new_n185_));
  inv1   g123(.a(x09), .O(new_n186_));
  nand2  g124(.a(new_n82_), .b(new_n186_), .O(new_n187_));
  aoi21  g125(.a(new_n185_), .b(new_n84_), .c(new_n187_), .O(new_n188_));
  oai22  g126(.a(new_n188_), .b(new_n81_), .c(new_n182_), .d(new_n91_), .O(z18));
  nand2  g127(.a(x40), .b(x04), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(x35), .c(new_n84_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(x31), .O(new_n192_));
  aoi21  g130(.a(new_n192_), .b(new_n131_), .c(x08), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n138_), .c(new_n82_), .O(new_n194_));
  nor2   g132(.a(new_n194_), .b(x09), .O(z19));
  inv1   g133(.a(x08), .O(new_n196_));
  oai21  g134(.a(new_n66_), .b(new_n64_), .c(new_n196_), .O(new_n197_));
  aoi21  g135(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n198_));
  nand2  g136(.a(new_n74_), .b(x29), .O(new_n199_));
  inv1   g137(.a(new_n199_), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(new_n198_), .c(new_n197_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n82_), .c(new_n186_), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n79_), .O(z20));
endmodule


