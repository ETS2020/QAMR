// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_;
  nor2   g000(.a(x32), .b(x30), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x27), .O(new_n65_));
  inv1   g003(.a(x04), .O(new_n66_));
  inv1   g004(.a(x35), .O(new_n67_));
  inv1   g005(.a(x36), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g007(.a(new_n69_), .b(x40), .c(x39), .d(new_n66_), .O(new_n70_));
  inv1   g008(.a(x37), .O(new_n71_));
  nand2  g009(.a(x35), .b(x28), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n70_), .c(new_n65_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n66_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n75_), .c(new_n64_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g019(.a(new_n63_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(z01));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  inv1   g025(.a(x28), .O(new_n88_));
  nand2  g026(.a(x35), .b(new_n88_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n87_), .c(x04), .O(new_n90_));
  nand4  g028(.a(new_n90_), .b(new_n86_), .c(new_n77_), .d(new_n82_), .O(z02));
  nand2  g029(.a(new_n72_), .b(x27), .O(new_n92_));
  aoi21  g030(.a(new_n71_), .b(new_n65_), .c(new_n63_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(z04));
  nor2   g034(.a(new_n88_), .b(new_n65_), .O(new_n97_));
  inv1   g035(.a(new_n97_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n82_), .c(new_n71_), .O(z05));
  oai21  g037(.a(new_n97_), .b(x37), .c(new_n82_), .O(z06));
  inv1   g038(.a(x33), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n64_), .O(new_n102_));
  inv1   g040(.a(x00), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n103_), .c(x38), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand3  g044(.a(x38), .b(new_n106_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(new_n108_));
  oai22  g046(.a(new_n108_), .b(x14), .c(new_n105_), .d(x31), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n82_), .O(z07));
  nand2  g049(.a(new_n76_), .b(new_n82_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n82_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  nand2  g053(.a(new_n89_), .b(new_n68_), .O(new_n116_));
  nand4  g054(.a(new_n116_), .b(new_n76_), .c(x27), .d(x07), .O(new_n117_));
  nand2  g055(.a(new_n77_), .b(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n66_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n120_), .c(new_n63_), .O(z10));
  inv1   g060(.a(x09), .O(new_n123_));
  inv1   g061(.a(x30), .O(new_n124_));
  nand2  g062(.a(new_n77_), .b(x29), .O(new_n125_));
  nor2   g063(.a(new_n65_), .b(new_n66_), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n89_), .c(new_n125_), .O(new_n127_));
  inv1   g065(.a(x08), .O(new_n128_));
  nor2   g066(.a(new_n67_), .b(x28), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x27), .c(new_n66_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n127_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(x32), .c(new_n124_), .d(new_n123_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  inv1   g072(.a(x10), .O(new_n135_));
  nor3   g073(.a(x37), .b(x36), .c(x35), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n65_), .c(new_n135_), .O(new_n137_));
  oai22  g075(.a(new_n76_), .b(x04), .c(new_n74_), .d(new_n65_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n82_), .O(z12));
  inv1   g077(.a(x18), .O(new_n140_));
  inv1   g078(.a(x19), .O(new_n141_));
  nor2   g079(.a(new_n74_), .b(new_n65_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(x20), .c(new_n141_), .d(new_n140_), .O(new_n143_));
  nor2   g081(.a(x13), .b(x04), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n77_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n143_), .c(new_n63_), .O(z13));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n73_), .c(x27), .d(x20), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n145_), .c(new_n82_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(z14));
  inv1   g088(.a(x12), .O(new_n151_));
  nor2   g089(.a(new_n114_), .b(new_n151_), .O(z15));
  inv1   g090(.a(x23), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(x22), .c(x01), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n82_), .O(z16));
  inv1   g093(.a(x01), .O(new_n156_));
  inv1   g094(.a(x24), .O(new_n157_));
  nand4  g095(.a(new_n82_), .b(new_n157_), .c(x23), .d(x22), .O(new_n158_));
  nor2   g096(.a(new_n158_), .b(new_n156_), .O(z17));
  nand2  g097(.a(new_n129_), .b(new_n65_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n125_), .c(new_n128_), .O(new_n161_));
  nand2  g099(.a(new_n130_), .b(new_n123_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n161_), .c(x32), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n124_), .O(z18));
  inv1   g102(.a(new_n125_), .O(new_n165_));
  oai21  g103(.a(new_n67_), .b(new_n65_), .c(new_n128_), .O(new_n166_));
  aoi21  g104(.a(x28), .b(new_n65_), .c(new_n67_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n165_), .c(new_n166_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(x32), .c(new_n124_), .d(new_n123_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z20));
  inv1   g108(.a(new_n133_), .O(z19));
endmodule


