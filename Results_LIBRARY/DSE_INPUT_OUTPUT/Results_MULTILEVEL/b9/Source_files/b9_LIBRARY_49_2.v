// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x33), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x18), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g009(.a(new_n71_), .b(x40), .c(x39), .d(new_n68_), .O(new_n72_));
  aoi21  g010(.a(x35), .b(x28), .c(x37), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(x10), .c(new_n68_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n66_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  oai21  g020(.a(new_n69_), .b(x28), .c(new_n70_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(new_n69_), .b(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(new_n75_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n89_), .c(new_n65_), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  inv1   g034(.a(x37), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n67_), .c(new_n96_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n95_), .c(new_n65_), .O(z03));
  aoi21  g037(.a(new_n97_), .b(new_n67_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n95_), .c(new_n65_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n64_), .c(new_n97_), .O(z05));
  nand2  g041(.a(new_n102_), .b(new_n97_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n64_), .O(z06));
  inv1   g043(.a(x03), .O(new_n106_));
  inv1   g044(.a(x18), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n63_), .O(new_n111_));
  inv1   g049(.a(x14), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n109_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  oai21  g053(.a(new_n111_), .b(x31), .c(new_n115_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n107_), .O(new_n117_));
  nand3  g055(.a(new_n114_), .b(x33), .c(new_n112_), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n117_), .c(new_n106_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  inv1   g058(.a(x40), .O(new_n121_));
  nor3   g059(.a(new_n65_), .b(new_n121_), .c(new_n120_), .O(z08));
  nand4  g060(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n64_), .O(z09));
  nand2  g062(.a(new_n75_), .b(x07), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x05), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n83_), .c(x27), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  inv1   g067(.a(x05), .O(new_n130_));
  nand2  g068(.a(new_n82_), .b(new_n81_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x40), .c(x39), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n129_), .c(new_n68_), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n134_), .c(new_n65_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  nand2  g075(.a(new_n76_), .b(x29), .O(new_n138_));
  oai21  g076(.a(new_n67_), .b(new_n68_), .c(new_n90_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  inv1   g078(.a(x08), .O(new_n141_));
  nand3  g079(.a(new_n90_), .b(x27), .c(new_n68_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n81_), .c(new_n137_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n64_), .O(z11));
  inv1   g084(.a(x10), .O(new_n147_));
  nor3   g085(.a(x37), .b(x36), .c(x35), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n67_), .c(new_n147_), .O(new_n149_));
  oai22  g087(.a(new_n75_), .b(x04), .c(new_n73_), .d(new_n67_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n149_), .c(new_n64_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n121_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n68_), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand2  g094(.a(new_n94_), .b(new_n97_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(x20), .c(new_n156_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n155_), .c(new_n67_), .O(new_n159_));
  nor3   g097(.a(new_n132_), .b(x13), .c(x04), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n107_), .O(new_n161_));
  aoi21  g099(.a(new_n153_), .b(x28), .c(new_n67_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n131_), .c(x40), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(new_n120_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x33), .c(new_n152_), .d(new_n68_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n161_), .O(z13));
  aoi21  g104(.a(x20), .b(new_n156_), .c(x18), .O(new_n167_));
  nor2   g105(.a(new_n63_), .b(new_n107_), .O(new_n168_));
  nand4  g106(.a(new_n153_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  nand3  g108(.a(new_n76_), .b(new_n152_), .c(new_n68_), .O(new_n171_));
  oai22  g109(.a(new_n171_), .b(new_n170_), .c(new_n168_), .d(new_n167_), .O(new_n172_));
  nand3  g110(.a(new_n97_), .b(new_n69_), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n82_), .c(new_n81_), .O(new_n175_));
  oai21  g113(.a(new_n73_), .b(new_n67_), .c(new_n171_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n107_), .O(new_n178_));
  nand4  g116(.a(x33), .b(new_n82_), .c(new_n81_), .d(new_n67_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n178_), .c(new_n172_), .O(z14));
  inv1   g118(.a(x12), .O(new_n181_));
  nand4  g119(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n182_));
  nor2   g120(.a(new_n182_), .b(new_n181_), .O(z15));
  inv1   g121(.a(x23), .O(new_n184_));
  nand4  g122(.a(new_n64_), .b(new_n184_), .c(x22), .d(x01), .O(new_n185_));
  inv1   g123(.a(new_n185_), .O(z16));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n187_), .b(x23), .c(x22), .d(x01), .O(new_n188_));
  nand2  g126(.a(new_n188_), .b(new_n64_), .O(z17));
  nand2  g127(.a(new_n90_), .b(new_n67_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n138_), .O(new_n191_));
  nand3  g129(.a(new_n142_), .b(new_n81_), .c(new_n137_), .O(new_n192_));
  aoi21  g130(.a(new_n191_), .b(x08), .c(new_n192_), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(new_n65_), .O(z18));
  inv1   g132(.a(new_n138_), .O(new_n195_));
  oai21  g133(.a(new_n69_), .b(new_n67_), .c(new_n141_), .O(new_n196_));
  aoi21  g134(.a(x28), .b(new_n67_), .c(new_n69_), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n195_), .c(new_n196_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n81_), .c(new_n137_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n64_), .O(z20));
  nand2  g138(.a(new_n145_), .b(new_n64_), .O(z19));
endmodule


