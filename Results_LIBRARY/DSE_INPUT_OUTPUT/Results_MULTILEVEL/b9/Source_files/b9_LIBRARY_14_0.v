// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_;
  nor2   g000(.a(x30), .b(x04), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n64_), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x10), .O(new_n70_));
  inv1   g008(.a(x40), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n70_), .c(new_n71_), .O(new_n73_));
  nand4  g011(.a(new_n73_), .b(x39), .c(x30), .d(new_n64_), .O(new_n74_));
  oai22  g012(.a(new_n74_), .b(x04), .c(new_n69_), .d(new_n63_), .O(z00));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x35), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  oai21  g016(.a(new_n77_), .b(x28), .c(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x27), .c(x32), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n76_), .c(x04), .O(new_n81_));
  oai21  g019(.a(new_n76_), .b(x04), .c(new_n81_), .O(z01));
  inv1   g020(.a(new_n63_), .O(new_n83_));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x04), .O(new_n90_));
  nand2  g028(.a(x40), .b(x39), .O(new_n91_));
  inv1   g029(.a(new_n91_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n86_), .d(new_n83_), .O(z02));
  oai21  g031(.a(new_n77_), .b(new_n87_), .c(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n66_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n94_), .c(new_n83_), .d(x21), .O(z03));
  aoi21  g035(.a(new_n95_), .b(new_n66_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n94_), .c(new_n63_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n83_), .c(new_n95_), .O(z05));
  nand2  g039(.a(new_n100_), .b(new_n95_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n83_), .O(z06));
  inv1   g041(.a(x31), .O(new_n104_));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n64_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .d(new_n104_), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n107_), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n109_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n83_), .O(z07));
  inv1   g054(.a(x39), .O(new_n117_));
  nor3   g055(.a(new_n63_), .b(new_n71_), .c(new_n117_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n83_), .O(z09));
  inv1   g058(.a(x04), .O(new_n121_));
  nand4  g059(.a(new_n91_), .b(new_n79_), .c(x27), .d(x07), .O(new_n122_));
  aoi21  g060(.a(new_n92_), .b(x05), .c(new_n76_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(z10));
  inv1   g065(.a(x09), .O(new_n128_));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  nand3  g068(.a(x35), .b(new_n87_), .c(new_n66_), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n130_), .c(new_n129_), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(new_n76_), .c(new_n128_), .d(x04), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(z11));
  nor3   g073(.a(x37), .b(x36), .c(x35), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n66_), .c(new_n70_), .O(new_n137_));
  oai22  g075(.a(new_n91_), .b(x04), .c(new_n67_), .d(new_n66_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n83_), .O(z12));
  inv1   g077(.a(x19), .O(new_n140_));
  nor2   g078(.a(new_n67_), .b(new_n63_), .O(new_n141_));
  nand4  g079(.a(new_n141_), .b(x27), .c(x20), .d(new_n140_), .O(new_n142_));
  inv1   g080(.a(x13), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n121_), .O(new_n144_));
  nand2  g082(.a(new_n92_), .b(x30), .O(new_n145_));
  oai22  g083(.a(new_n145_), .b(new_n144_), .c(new_n142_), .d(x18), .O(z13));
  inv1   g084(.a(x18), .O(new_n147_));
  nand4  g085(.a(x27), .b(x20), .c(new_n140_), .d(new_n147_), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n67_), .c(new_n144_), .d(new_n91_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n83_), .O(z14));
  nand4  g088(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n83_), .O(z15));
  inv1   g090(.a(x23), .O(new_n153_));
  nand4  g091(.a(new_n83_), .b(new_n153_), .c(x22), .d(x01), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(z16));
  inv1   g093(.a(x01), .O(new_n156_));
  inv1   g094(.a(x24), .O(new_n157_));
  nand4  g095(.a(new_n83_), .b(new_n157_), .c(x23), .d(x22), .O(new_n158_));
  nor2   g096(.a(new_n158_), .b(new_n156_), .O(z17));
  oai21  g097(.a(new_n132_), .b(x09), .c(x04), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n76_), .O(z18));
  nand3  g099(.a(x35), .b(new_n87_), .c(new_n66_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n130_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x08), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n128_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x04), .c(x30), .O(z19));
  inv1   g104(.a(new_n130_), .O(new_n167_));
  aoi21  g105(.a(x28), .b(new_n66_), .c(new_n77_), .O(new_n168_));
  oai21  g106(.a(new_n77_), .b(new_n66_), .c(new_n129_), .O(new_n169_));
  oai21  g107(.a(new_n168_), .b(new_n167_), .c(new_n169_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n128_), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(x04), .c(x30), .O(z20));
endmodule


