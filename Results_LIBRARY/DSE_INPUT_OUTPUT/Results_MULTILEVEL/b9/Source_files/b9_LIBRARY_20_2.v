// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_;
  inv1   g000(.a(x19), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x10), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(x19), .b(x10), .O(new_n74_));
  oai21  g012(.a(new_n73_), .b(x10), .c(new_n74_), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(x40), .c(x39), .d(new_n66_), .O(new_n76_));
  oai22  g014(.a(new_n76_), .b(x04), .c(new_n72_), .d(new_n65_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n82_), .c(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n91_), .c(new_n89_), .d(new_n64_), .O(z02));
  nand2  g032(.a(new_n70_), .b(x27), .O(new_n95_));
  nand2  g033(.a(new_n69_), .b(new_n68_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n95_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g035(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n95_), .c(new_n65_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n64_), .c(new_n69_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n66_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n64_), .c(x03), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z07));
  inv1   g051(.a(x39), .O(new_n114_));
  inv1   g052(.a(x40), .O(new_n115_));
  nor3   g053(.a(new_n65_), .b(new_n115_), .c(new_n114_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n64_), .O(z09));
  inv1   g056(.a(x04), .O(new_n119_));
  inv1   g057(.a(x05), .O(new_n120_));
  nand2  g058(.a(new_n92_), .b(x07), .O(new_n121_));
  oai21  g059(.a(new_n92_), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n83_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n79_), .b(new_n78_), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(x40), .c(x39), .d(x05), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n119_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n64_), .O(z10));
  inv1   g067(.a(x09), .O(new_n130_));
  nand2  g068(.a(new_n93_), .b(x29), .O(new_n131_));
  nor2   g069(.a(new_n68_), .b(new_n119_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n82_), .c(new_n131_), .O(new_n133_));
  inv1   g071(.a(x08), .O(new_n134_));
  nand4  g072(.a(x35), .b(new_n81_), .c(x27), .d(new_n119_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n78_), .c(new_n130_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n64_), .O(z11));
  nand3  g077(.a(new_n70_), .b(new_n64_), .c(new_n69_), .O(new_n140_));
  oai21  g078(.a(x27), .b(new_n63_), .c(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n92_), .b(x04), .c(new_n141_), .O(new_n142_));
  inv1   g080(.a(x10), .O(new_n143_));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n68_), .c(new_n143_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x28), .c(new_n115_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x39), .c(new_n147_), .d(new_n119_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  nand2  g089(.a(new_n70_), .b(new_n69_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x20), .c(new_n63_), .d(new_n151_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n150_), .c(new_n68_), .O(new_n154_));
  nand3  g092(.a(new_n124_), .b(x40), .c(x39), .O(new_n155_));
  nor3   g093(.a(new_n155_), .b(x13), .c(x04), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n154_), .c(new_n143_), .O(new_n157_));
  aoi21  g095(.a(new_n148_), .b(x28), .c(new_n68_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n124_), .c(x40), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(new_n114_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(x19), .c(new_n147_), .d(new_n119_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(z13));
  aoi21  g100(.a(x20), .b(new_n151_), .c(x10), .O(new_n163_));
  nand4  g101(.a(new_n148_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  nand3  g103(.a(new_n93_), .b(new_n147_), .c(new_n119_), .O(new_n166_));
  oai22  g104(.a(new_n166_), .b(new_n165_), .c(new_n163_), .d(x19), .O(new_n167_));
  inv1   g105(.a(new_n166_), .O(new_n168_));
  inv1   g106(.a(x35), .O(new_n169_));
  nand3  g107(.a(new_n69_), .b(new_n169_), .c(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n79_), .c(new_n78_), .O(new_n172_));
  oai21  g110(.a(new_n168_), .b(new_n71_), .c(new_n172_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n143_), .O(new_n174_));
  nand4  g112(.a(new_n79_), .b(new_n78_), .c(new_n68_), .d(x19), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(new_n167_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n64_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand4  g117(.a(new_n64_), .b(new_n179_), .c(x22), .d(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z16));
  inv1   g119(.a(x01), .O(new_n182_));
  inv1   g120(.a(x24), .O(new_n183_));
  nand4  g121(.a(new_n64_), .b(new_n183_), .c(x23), .d(x22), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n182_), .O(z17));
  oai21  g123(.a(new_n82_), .b(x27), .c(new_n131_), .O(new_n186_));
  nand3  g124(.a(new_n135_), .b(new_n78_), .c(new_n130_), .O(new_n187_));
  aoi21  g125(.a(new_n186_), .b(x08), .c(new_n187_), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n65_), .O(z18));
  nand4  g127(.a(new_n137_), .b(new_n64_), .c(new_n78_), .d(new_n130_), .O(new_n190_));
  inv1   g128(.a(new_n190_), .O(z19));
  inv1   g129(.a(new_n131_), .O(new_n192_));
  oai21  g130(.a(new_n169_), .b(new_n68_), .c(new_n134_), .O(new_n193_));
  aoi21  g131(.a(x28), .b(new_n68_), .c(new_n169_), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n192_), .c(new_n193_), .O(new_n195_));
  nand4  g133(.a(new_n195_), .b(new_n64_), .c(new_n78_), .d(new_n130_), .O(new_n196_));
  inv1   g134(.a(new_n196_), .O(z20));
endmodule


