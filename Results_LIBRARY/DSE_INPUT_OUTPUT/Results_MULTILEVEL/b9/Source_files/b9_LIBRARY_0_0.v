// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n177_, new_n178_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n188_, new_n189_,
    new_n190_, new_n191_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand4  g012(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n74_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x18), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n77_), .c(x16), .O(z00));
  inv1   g019(.a(x28), .O(new_n82_));
  aoi21  g020(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  inv1   g021(.a(new_n83_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x27), .c(x32), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n78_), .c(new_n79_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x04), .O(z01));
  nor2   g025(.a(x30), .b(x18), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  nor2   g030(.a(new_n66_), .b(x28), .O(new_n93_));
  oai21  g031(.a(x27), .b(x08), .c(new_n93_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x04), .c(new_n92_), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n91_), .c(new_n88_), .O(z02));
  nand2  g034(.a(new_n71_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n70_), .b(new_n64_), .c(new_n98_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n97_), .c(new_n88_), .O(z03));
  nand2  g038(.a(new_n70_), .b(new_n64_), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n97_), .c(new_n80_), .d(new_n98_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n88_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n80_), .O(z07));
  nand2  g054(.a(new_n80_), .b(new_n92_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n80_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  nand3  g058(.a(new_n80_), .b(new_n92_), .c(x07), .O(new_n121_));
  nand4  g059(.a(x40), .b(x39), .c(x18), .d(x05), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(new_n83_), .O(new_n123_));
  inv1   g061(.a(x06), .O(new_n124_));
  nor3   g062(.a(new_n88_), .b(new_n70_), .c(new_n124_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n65_), .c(new_n125_), .O(new_n126_));
  inv1   g064(.a(x40), .O(new_n127_));
  nand2  g065(.a(x32), .b(x18), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n79_), .c(new_n127_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(x39), .c(x05), .d(new_n65_), .O(new_n130_));
  oai21  g068(.a(new_n126_), .b(new_n64_), .c(new_n130_), .O(z10));
  inv1   g069(.a(x09), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  nand2  g071(.a(x27), .b(x04), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n93_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  inv1   g074(.a(x08), .O(new_n137_));
  nand3  g075(.a(new_n93_), .b(x27), .c(new_n65_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand4  g078(.a(new_n140_), .b(new_n79_), .c(x18), .d(new_n132_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z11));
  inv1   g080(.a(x10), .O(new_n143_));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n64_), .c(new_n143_), .O(new_n145_));
  oai22  g083(.a(new_n92_), .b(x04), .c(new_n73_), .d(new_n64_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n145_), .c(new_n88_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  oai21  g086(.a(new_n67_), .b(new_n66_), .c(x28), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x40), .c(x39), .d(new_n148_), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand4  g089(.a(new_n72_), .b(x20), .c(new_n151_), .d(new_n78_), .O(new_n152_));
  oai21  g090(.a(new_n150_), .b(x04), .c(new_n152_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  inv1   g092(.a(x39), .O(new_n155_));
  oai21  g093(.a(x32), .b(x30), .c(x40), .O(new_n156_));
  nor3   g094(.a(new_n156_), .b(new_n155_), .c(x13), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n65_), .c(new_n88_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n154_), .O(z13));
  aoi21  g097(.a(new_n149_), .b(x27), .c(x32), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n78_), .c(new_n79_), .O(new_n161_));
  nand2  g099(.a(new_n148_), .b(new_n65_), .O(new_n162_));
  nand4  g100(.a(x27), .b(x20), .c(new_n151_), .d(new_n78_), .O(new_n163_));
  oai22  g101(.a(new_n163_), .b(new_n73_), .c(new_n162_), .d(new_n92_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z14));
  nand4  g103(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n80_), .O(z15));
  inv1   g105(.a(x23), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(x22), .c(x01), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n80_), .O(z16));
  inv1   g108(.a(x01), .O(new_n171_));
  inv1   g109(.a(x24), .O(new_n172_));
  nand4  g110(.a(new_n80_), .b(new_n172_), .c(x23), .d(x22), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(new_n171_), .O(z17));
  nand2  g112(.a(new_n93_), .b(new_n64_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n133_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x08), .O(new_n177_));
  nor2   g115(.a(new_n78_), .b(x09), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n177_), .c(new_n138_), .d(new_n79_), .O(z18));
  inv1   g117(.a(new_n133_), .O(new_n180_));
  aoi21  g118(.a(new_n66_), .b(x18), .c(x28), .O(new_n181_));
  aoi21  g119(.a(new_n181_), .b(new_n134_), .c(new_n180_), .O(new_n182_));
  oai21  g120(.a(new_n66_), .b(new_n64_), .c(x18), .O(new_n183_));
  nor2   g121(.a(x28), .b(x04), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(x08), .O(new_n185_));
  oai21  g123(.a(new_n185_), .b(new_n182_), .c(new_n132_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(x18), .c(x30), .O(z19));
  oai21  g125(.a(new_n66_), .b(new_n64_), .c(new_n137_), .O(new_n188_));
  aoi21  g126(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n189_));
  oai21  g127(.a(new_n189_), .b(new_n180_), .c(new_n188_), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n79_), .c(x18), .d(new_n132_), .O(new_n191_));
  inv1   g129(.a(new_n191_), .O(z20));
endmodule


