// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_;
  nand2  g000(.a(x40), .b(x27), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(new_n65_), .b(x40), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x27), .O(new_n70_));
  aoi22  g008(.a(new_n70_), .b(new_n68_), .c(new_n66_), .d(x27), .O(new_n71_));
  oai22  g009(.a(new_n71_), .b(x15), .c(new_n64_), .d(x16), .O(z00));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x27), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n74_), .c(new_n73_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n63_), .c(x04), .O(z01));
  inv1   g019(.a(x27), .O(new_n82_));
  aoi21  g020(.a(x29), .b(x08), .c(x02), .O(new_n83_));
  inv1   g021(.a(x08), .O(new_n84_));
  oai21  g022(.a(new_n77_), .b(new_n84_), .c(x04), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x39), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n83_), .c(new_n82_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x40), .O(z02));
  inv1   g026(.a(x40), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(x35), .c(x28), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n82_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  nand2  g032(.a(new_n63_), .b(x21), .O(new_n95_));
  nand2  g033(.a(x35), .b(x28), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n89_), .c(x27), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(new_n93_), .O(z04));
  nand3  g036(.a(new_n89_), .b(x28), .c(x27), .O(new_n99_));
  oai21  g037(.a(new_n64_), .b(new_n92_), .c(new_n99_), .O(z05));
  nand2  g038(.a(new_n89_), .b(new_n76_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(x27), .c(x37), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  inv1   g041(.a(x15), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n104_), .O(new_n105_));
  inv1   g043(.a(x00), .O(new_n106_));
  oai21  g044(.a(x25), .b(new_n106_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n106_), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  oai21  g050(.a(new_n108_), .b(x31), .c(new_n112_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n63_), .O(z07));
  inv1   g053(.a(x39), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(new_n82_), .c(new_n89_), .O(z08));
  nand3  g055(.a(x34), .b(x26), .c(x11), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n89_), .c(new_n82_), .O(z09));
  inv1   g057(.a(x04), .O(new_n120_));
  nor2   g058(.a(x32), .b(x30), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nand4  g060(.a(new_n122_), .b(x40), .c(x39), .d(x05), .O(new_n123_));
  nand3  g061(.a(new_n78_), .b(x27), .c(x07), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n120_), .O(new_n126_));
  nand2  g064(.a(x37), .b(x06), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n89_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n126_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  aoi21  g069(.a(x39), .b(x29), .c(x27), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n89_), .c(new_n77_), .O(new_n133_));
  nand2  g071(.a(new_n82_), .b(new_n84_), .O(new_n134_));
  nand3  g072(.a(new_n89_), .b(x27), .c(x04), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n73_), .c(new_n131_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  nor2   g076(.a(new_n65_), .b(new_n82_), .O(new_n139_));
  nand2  g077(.a(x39), .b(x10), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(x04), .c(new_n82_), .O(new_n141_));
  oai21  g079(.a(new_n139_), .b(x40), .c(new_n141_), .O(z12));
  nor3   g080(.a(new_n121_), .b(new_n116_), .c(x13), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n120_), .c(x27), .O(new_n144_));
  inv1   g082(.a(x18), .O(new_n145_));
  inv1   g083(.a(x19), .O(new_n146_));
  nand4  g084(.a(new_n139_), .b(x20), .c(new_n146_), .d(new_n145_), .O(new_n147_));
  oai21  g085(.a(new_n144_), .b(new_n89_), .c(new_n147_), .O(z13));
  nand4  g086(.a(new_n89_), .b(x20), .c(new_n146_), .d(new_n145_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n65_), .c(x27), .O(new_n150_));
  nor2   g088(.a(x13), .b(x04), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(new_n122_), .c(x40), .d(x39), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n82_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(z14));
  nand4  g092(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n155_));
  nor2   g093(.a(new_n155_), .b(x40), .O(z15));
  inv1   g094(.a(x23), .O(new_n157_));
  nand4  g095(.a(new_n63_), .b(new_n157_), .c(x22), .d(x01), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z16));
  inv1   g097(.a(x01), .O(new_n160_));
  inv1   g098(.a(x24), .O(new_n161_));
  nand4  g099(.a(new_n63_), .b(new_n161_), .c(x23), .d(x22), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(new_n160_), .O(z17));
  inv1   g101(.a(x29), .O(new_n164_));
  oai22  g102(.a(new_n77_), .b(x27), .c(new_n69_), .d(new_n164_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x08), .O(new_n166_));
  oai21  g104(.a(new_n77_), .b(x04), .c(new_n89_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x27), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n166_), .c(new_n73_), .d(new_n131_), .O(z18));
  inv1   g107(.a(x35), .O(new_n170_));
  nand2  g108(.a(x39), .b(x29), .O(new_n171_));
  oai21  g109(.a(new_n170_), .b(x28), .c(new_n171_), .O(new_n172_));
  aoi21  g110(.a(new_n89_), .b(x28), .c(new_n84_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n172_), .c(x27), .O(new_n174_));
  nor2   g112(.a(x40), .b(x35), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n174_), .c(new_n73_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(x09), .O(z20));
  inv1   g115(.a(new_n137_), .O(z19));
endmodule


