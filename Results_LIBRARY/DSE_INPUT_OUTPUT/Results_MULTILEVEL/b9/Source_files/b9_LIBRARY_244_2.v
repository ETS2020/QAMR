// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n168_, new_n169_, new_n171_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x39), .O(new_n67_));
  nor3   g005(.a(new_n67_), .b(new_n66_), .c(x04), .O(new_n68_));
  aoi21  g006(.a(new_n65_), .b(x27), .c(new_n68_), .O(new_n69_));
  inv1   g007(.a(x40), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x39), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x30), .O(new_n74_));
  inv1   g012(.a(x32), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(new_n74_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n71_), .c(x04), .O(z01));
  aoi21  g020(.a(x29), .b(x08), .c(x02), .O(new_n83_));
  inv1   g021(.a(x35), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(x28), .O(new_n85_));
  oai21  g023(.a(x27), .b(x08), .c(new_n85_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n83_), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n70_), .c(x39), .O(z02));
  nand2  g026(.a(x35), .b(x28), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  inv1   g028(.a(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  aoi22  g030(.a(new_n70_), .b(x39), .c(new_n92_), .d(new_n91_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n90_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n90_), .c(new_n95_), .O(z04));
  oai21  g034(.a(new_n77_), .b(new_n91_), .c(new_n92_), .O(new_n97_));
  and2   g035(.a(new_n97_), .b(new_n71_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x33), .O(new_n100_));
  inv1   g038(.a(x15), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n101_), .O(new_n102_));
  inv1   g040(.a(x00), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n103_), .c(x38), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(new_n100_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand3  g044(.a(x38), .b(new_n106_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(new_n108_));
  oai22  g046(.a(new_n108_), .b(x14), .c(new_n105_), .d(x31), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n71_), .O(z07));
  inv1   g049(.a(x11), .O(new_n112_));
  nand4  g050(.a(new_n71_), .b(x34), .c(x27), .d(x26), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n112_), .O(z09));
  inv1   g052(.a(x04), .O(new_n115_));
  inv1   g053(.a(x07), .O(new_n116_));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  oai21  g055(.a(x39), .b(new_n116_), .c(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n79_), .c(new_n115_), .O(new_n119_));
  nand3  g057(.a(new_n71_), .b(x37), .c(x06), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x27), .O(new_n122_));
  oai21  g060(.a(x32), .b(x30), .c(x40), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n67_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(x05), .c(new_n115_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(z10));
  inv1   g064(.a(x09), .O(new_n127_));
  nand2  g065(.a(x39), .b(x29), .O(new_n128_));
  nand2  g066(.a(x27), .b(x04), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n85_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  inv1   g069(.a(x08), .O(new_n132_));
  nand2  g070(.a(x39), .b(x04), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(x35), .c(new_n77_), .d(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n74_), .c(new_n127_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n71_), .O(z11));
  nor3   g076(.a(x37), .b(x36), .c(x35), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n91_), .c(new_n66_), .O(new_n140_));
  oai22  g078(.a(new_n64_), .b(new_n91_), .c(new_n67_), .d(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n71_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x28), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(x39), .c(new_n143_), .d(new_n115_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand2  g086(.a(new_n89_), .b(new_n92_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n146_), .c(new_n70_), .O(new_n151_));
  nand4  g089(.a(new_n149_), .b(new_n67_), .c(x20), .d(new_n148_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(x18), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n151_), .c(x27), .O(new_n154_));
  nand3  g092(.a(new_n124_), .b(new_n143_), .c(new_n115_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z13));
  aoi21  g094(.a(new_n143_), .b(new_n115_), .c(new_n70_), .O(new_n157_));
  nand4  g095(.a(x27), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n64_), .c(new_n157_), .d(new_n67_), .O(new_n159_));
  nand3  g097(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n160_));
  nor2   g098(.a(x37), .b(x35), .O(new_n161_));
  aoi21  g099(.a(new_n160_), .b(new_n144_), .c(new_n161_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n77_), .c(x27), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x40), .c(new_n75_), .d(new_n74_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n159_), .O(z14));
  nand4  g103(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n71_), .O(z15));
  inv1   g105(.a(x23), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(x22), .c(x01), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n71_), .O(z16));
  inv1   g108(.a(x01), .O(new_n171_));
  inv1   g109(.a(x24), .O(new_n172_));
  nand4  g110(.a(new_n71_), .b(new_n172_), .c(x23), .d(x22), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(new_n171_), .O(z17));
  oai21  g112(.a(new_n78_), .b(x27), .c(new_n128_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x08), .O(new_n176_));
  nor2   g114(.a(new_n91_), .b(x04), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n85_), .c(x09), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(new_n176_), .c(new_n71_), .d(new_n74_), .O(z18));
  inv1   g117(.a(x29), .O(new_n180_));
  aoi21  g118(.a(x40), .b(new_n180_), .c(new_n67_), .O(new_n181_));
  aoi21  g119(.a(new_n129_), .b(new_n85_), .c(new_n181_), .O(new_n182_));
  nand2  g120(.a(new_n71_), .b(new_n91_), .O(new_n183_));
  nand3  g121(.a(x35), .b(new_n77_), .c(new_n115_), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(x40), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n183_), .c(x08), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n182_), .c(new_n74_), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(x09), .O(z19));
  aoi21  g126(.a(x28), .b(new_n91_), .c(new_n84_), .O(new_n189_));
  nor2   g127(.a(new_n189_), .b(new_n181_), .O(new_n190_));
  nand2  g128(.a(x40), .b(new_n84_), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n183_), .c(x08), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n190_), .c(new_n74_), .O(new_n193_));
  nor2   g131(.a(new_n193_), .b(x09), .O(z20));
  buf    g132(.a(x39), .O(z08));
endmodule


