// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n185_, new_n186_, new_n187_, new_n188_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(z01));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(z01), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand4  g009(.a(x40), .b(x39), .c(x10), .d(z01), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(new_n71_), .c(new_n63_), .O(new_n74_));
  inv1   g012(.a(x30), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n74_), .c(x16), .O(z00));
  nand2  g015(.a(x40), .b(x39), .O(new_n78_));
  aoi21  g016(.a(x29), .b(x08), .c(x02), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  inv1   g018(.a(x08), .O(new_n81_));
  nand2  g019(.a(new_n64_), .b(new_n81_), .O(new_n82_));
  nor2   g020(.a(new_n66_), .b(x28), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(x30), .c(x04), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n80_), .O(z02));
  inv1   g024(.a(new_n76_), .O(new_n87_));
  nand2  g025(.a(x35), .b(x28), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  inv1   g027(.a(x21), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n64_), .c(new_n90_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n89_), .c(new_n87_), .O(z03));
  nand2  g031(.a(new_n91_), .b(new_n64_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n89_), .c(new_n76_), .d(new_n90_), .O(z04));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n91_), .O(new_n97_));
  and2   g035(.a(new_n97_), .b(new_n76_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x33), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n63_), .O(new_n101_));
  inv1   g039(.a(x00), .O(new_n102_));
  oai21  g040(.a(x25), .b(new_n102_), .c(x38), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(new_n107_));
  oai22  g045(.a(new_n107_), .b(x14), .c(new_n104_), .d(x31), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n76_), .O(z07));
  nand2  g048(.a(new_n76_), .b(new_n78_), .O(z08));
  inv1   g049(.a(x11), .O(new_n112_));
  nand4  g050(.a(new_n76_), .b(x34), .c(x27), .d(x26), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n112_), .O(z09));
  oai21  g052(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n115_));
  nand2  g053(.a(new_n78_), .b(x07), .O(new_n116_));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n115_), .c(x27), .O(new_n119_));
  inv1   g057(.a(x39), .O(new_n120_));
  oai21  g058(.a(x32), .b(x30), .c(x40), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x05), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(z01), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n76_), .O(z10));
  inv1   g065(.a(x09), .O(new_n128_));
  nand4  g066(.a(x40), .b(x39), .c(x29), .d(x08), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n83_), .c(new_n82_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(z01), .c(x30), .O(z11));
  inv1   g071(.a(x10), .O(new_n134_));
  nor3   g072(.a(x37), .b(x36), .c(x35), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n64_), .c(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n88_), .b(new_n91_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x27), .O(new_n138_));
  oai21  g076(.a(new_n78_), .b(x04), .c(new_n138_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n136_), .c(new_n76_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  nand2  g079(.a(x36), .b(x35), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x28), .O(new_n143_));
  nand4  g081(.a(new_n143_), .b(x40), .c(x39), .d(new_n141_), .O(new_n144_));
  inv1   g082(.a(x18), .O(new_n145_));
  inv1   g083(.a(x19), .O(new_n146_));
  nand4  g084(.a(new_n137_), .b(x20), .c(new_n146_), .d(new_n145_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n144_), .c(x04), .O(new_n148_));
  nand4  g086(.a(new_n137_), .b(x30), .c(x20), .d(new_n146_), .O(new_n149_));
  nor2   g087(.a(new_n149_), .b(x18), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n148_), .c(x27), .O(new_n151_));
  nand3  g089(.a(new_n122_), .b(new_n141_), .c(z01), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n151_), .O(z13));
  nand3  g091(.a(x20), .b(new_n146_), .c(new_n145_), .O(new_n154_));
  inv1   g092(.a(x32), .O(new_n155_));
  nand4  g093(.a(new_n142_), .b(new_n155_), .c(new_n75_), .d(x28), .O(new_n156_));
  nor2   g094(.a(new_n78_), .b(x13), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n156_), .c(x04), .O(new_n158_));
  nor2   g096(.a(new_n75_), .b(z01), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n158_), .c(new_n154_), .O(new_n160_));
  nand3  g098(.a(new_n91_), .b(new_n66_), .c(x28), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x27), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n155_), .c(new_n75_), .O(new_n163_));
  oai22  g101(.a(new_n78_), .b(x13), .c(new_n70_), .d(new_n64_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(z01), .O(new_n166_));
  nand3  g104(.a(new_n138_), .b(x30), .c(x04), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n166_), .c(new_n160_), .O(z14));
  nand4  g106(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n76_), .O(z15));
  inv1   g108(.a(x23), .O(new_n171_));
  nand4  g109(.a(new_n76_), .b(new_n171_), .c(x22), .d(x01), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z16));
  inv1   g111(.a(x01), .O(new_n174_));
  inv1   g112(.a(x24), .O(new_n175_));
  nand4  g113(.a(new_n76_), .b(new_n175_), .c(x23), .d(x22), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n174_), .O(z17));
  nand3  g115(.a(x40), .b(x39), .c(x29), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n83_), .c(x08), .O(new_n180_));
  aoi21  g118(.a(new_n83_), .b(x27), .c(x30), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(new_n180_), .c(new_n128_), .d(z01), .O(z18));
  nand4  g120(.a(new_n131_), .b(new_n75_), .c(new_n128_), .d(z01), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(z19));
  oai21  g122(.a(new_n66_), .b(new_n64_), .c(new_n81_), .O(new_n185_));
  aoi21  g123(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n179_), .c(new_n185_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n128_), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(z01), .c(x30), .O(z20));
endmodule


