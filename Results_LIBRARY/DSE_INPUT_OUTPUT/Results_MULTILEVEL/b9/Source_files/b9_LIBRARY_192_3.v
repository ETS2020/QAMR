// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n105_, new_n107_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x30), .O(new_n64_));
  inv1   g002(.a(x36), .O(new_n65_));
  nor2   g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g004(.a(new_n66_), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n72_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n71_), .c(x15), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(new_n63_), .c(new_n67_), .O(new_n77_));
  inv1   g015(.a(x15), .O(new_n78_));
  nand2  g016(.a(x36), .b(new_n64_), .O(new_n79_));
  nand2  g017(.a(new_n65_), .b(x35), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g019(.a(new_n81_), .b(x40), .c(x39), .d(x27), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n78_), .c(new_n72_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n77_), .O(z00));
  inv1   g023(.a(x32), .O(new_n86_));
  inv1   g024(.a(x35), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(x28), .c(new_n65_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n86_), .c(new_n64_), .O(new_n90_));
  oai21  g028(.a(new_n66_), .b(x04), .c(new_n90_), .O(z01));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  inv1   g032(.a(x28), .O(new_n95_));
  inv1   g033(.a(x08), .O(new_n96_));
  inv1   g034(.a(x27), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(x35), .c(new_n95_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x04), .c(new_n73_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n94_), .c(new_n66_), .O(z02));
  nand2  g039(.a(new_n69_), .b(x27), .O(new_n102_));
  aoi21  g040(.a(new_n68_), .b(new_n97_), .c(new_n66_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n102_), .c(x21), .O(z03));
  inv1   g042(.a(x21), .O(new_n105_));
  nand3  g043(.a(new_n103_), .b(new_n102_), .c(new_n105_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n67_), .c(new_n68_), .O(z05));
  inv1   g046(.a(z05), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n78_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x14), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n112_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  oai21  g056(.a(new_n114_), .b(x31), .c(new_n118_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n67_), .O(z07));
  nand2  g059(.a(new_n73_), .b(new_n67_), .O(z08));
  nand4  g060(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n67_), .O(z09));
  nand2  g062(.a(x05), .b(new_n72_), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n73_), .c(new_n65_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x30), .O(new_n127_));
  nand2  g065(.a(new_n73_), .b(x07), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n88_), .c(x27), .O(new_n131_));
  nand3  g069(.a(new_n74_), .b(x32), .c(x05), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n72_), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n127_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  nand3  g075(.a(x40), .b(x39), .c(x29), .O(new_n138_));
  nand2  g076(.a(x27), .b(x04), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(x35), .c(new_n95_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand4  g079(.a(x35), .b(new_n95_), .c(x27), .d(new_n72_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n96_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n64_), .c(new_n137_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n67_), .O(z11));
  inv1   g084(.a(x10), .O(new_n147_));
  nor3   g085(.a(x37), .b(x36), .c(x35), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n97_), .c(new_n147_), .O(new_n149_));
  oai21  g087(.a(new_n73_), .b(x04), .c(new_n71_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n149_), .c(new_n67_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  inv1   g090(.a(x40), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n153_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n152_), .d(new_n72_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n70_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  inv1   g099(.a(x39), .O(new_n162_));
  oai21  g100(.a(x32), .b(x30), .c(x40), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n162_), .c(x13), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n72_), .c(new_n66_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n161_), .O(z13));
  nand3  g104(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n167_));
  nand4  g105(.a(new_n154_), .b(new_n86_), .c(new_n64_), .d(x28), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  nand3  g107(.a(new_n74_), .b(new_n152_), .c(new_n72_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n169_), .c(new_n167_), .O(new_n171_));
  nand3  g109(.a(new_n68_), .b(new_n87_), .c(x28), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x27), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n86_), .c(new_n64_), .O(new_n174_));
  aoi21  g112(.a(new_n170_), .b(new_n71_), .c(new_n66_), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(new_n171_), .O(z14));
  inv1   g114(.a(x12), .O(new_n177_));
  nand4  g115(.a(new_n67_), .b(x34), .c(x27), .d(x26), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n177_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand4  g118(.a(new_n67_), .b(new_n180_), .c(x22), .d(x01), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z16));
  inv1   g120(.a(x24), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(x23), .c(x22), .d(x01), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n67_), .O(z17));
  nand3  g123(.a(x35), .b(new_n95_), .c(new_n97_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n138_), .c(new_n96_), .O(new_n187_));
  nand2  g125(.a(new_n142_), .b(new_n137_), .O(new_n188_));
  oai21  g126(.a(new_n188_), .b(new_n187_), .c(new_n64_), .O(new_n189_));
  oai21  g127(.a(x36), .b(new_n64_), .c(new_n189_), .O(z18));
  inv1   g128(.a(new_n138_), .O(new_n191_));
  oai21  g129(.a(new_n87_), .b(new_n97_), .c(new_n96_), .O(new_n192_));
  aoi21  g130(.a(x28), .b(new_n97_), .c(new_n87_), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n191_), .c(new_n192_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n64_), .c(new_n137_), .O(new_n195_));
  inv1   g133(.a(new_n195_), .O(z20));
  nand2  g134(.a(new_n145_), .b(new_n67_), .O(z19));
endmodule


