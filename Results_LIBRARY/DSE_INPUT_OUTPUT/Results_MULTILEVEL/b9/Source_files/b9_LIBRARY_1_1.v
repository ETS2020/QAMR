// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n151_, new_n152_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n169_, new_n170_,
    new_n171_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x30), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n63_), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(x39), .c(x30), .d(new_n65_), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x27), .c(x32), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x40), .c(new_n77_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(z01));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand2  g026(.a(new_n67_), .b(new_n88_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(x35), .c(new_n87_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n91_), .c(new_n86_), .d(x30), .O(z02));
  nand2  g032(.a(new_n69_), .b(x27), .O(new_n95_));
  aoi21  g033(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n95_), .c(new_n98_), .O(z04));
  inv1   g037(.a(new_n64_), .O(new_n100_));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n100_), .c(new_n68_), .O(z05));
  nand2  g040(.a(new_n101_), .b(new_n68_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n100_), .O(z06));
  inv1   g042(.a(x31), .O(new_n105_));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n65_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand4  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .d(new_n105_), .O(new_n110_));
  inv1   g048(.a(x14), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n108_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n100_), .O(z07));
  inv1   g055(.a(x39), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(x30), .c(new_n63_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n100_), .O(z09));
  inv1   g059(.a(x04), .O(new_n122_));
  nand4  g060(.a(new_n92_), .b(new_n80_), .c(x27), .d(x07), .O(new_n123_));
  nand2  g061(.a(new_n93_), .b(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nor2   g064(.a(new_n68_), .b(new_n67_), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(x06), .c(new_n64_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(z10));
  inv1   g067(.a(x09), .O(new_n130_));
  aoi21  g068(.a(new_n67_), .b(x08), .c(new_n122_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n90_), .c(new_n130_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n63_), .c(x30), .O(z11));
  nand3  g071(.a(new_n68_), .b(new_n79_), .c(new_n78_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(x27), .c(x10), .O(new_n135_));
  aoi21  g073(.a(x39), .b(new_n122_), .c(new_n70_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n135_), .c(x30), .O(new_n137_));
  or2    g075(.a(new_n70_), .b(x40), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(z12));
  inv1   g077(.a(x18), .O(new_n140_));
  inv1   g078(.a(x19), .O(new_n141_));
  nand4  g079(.a(new_n70_), .b(x20), .c(new_n141_), .d(new_n140_), .O(new_n142_));
  inv1   g080(.a(x13), .O(new_n143_));
  nand3  g081(.a(x39), .b(new_n143_), .c(new_n122_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x30), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x40), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n142_), .O(z13));
  nand4  g085(.a(x27), .b(x20), .c(new_n141_), .d(new_n140_), .O(new_n148_));
  aoi21  g086(.a(new_n69_), .b(new_n68_), .c(new_n148_), .O(new_n149_));
  aoi21  g087(.a(new_n145_), .b(x40), .c(new_n149_), .O(z14));
  inv1   g088(.a(x12), .O(new_n151_));
  nand4  g089(.a(new_n100_), .b(x34), .c(x27), .d(x26), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(new_n151_), .O(z15));
  inv1   g091(.a(x23), .O(new_n154_));
  nand4  g092(.a(new_n100_), .b(new_n154_), .c(x22), .d(x01), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(z16));
  inv1   g094(.a(x24), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x23), .c(x22), .d(x01), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n100_), .O(z17));
  nand2  g097(.a(new_n67_), .b(x08), .O(new_n160_));
  nand2  g098(.a(x27), .b(new_n122_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n160_), .c(new_n78_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n87_), .c(x09), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x40), .c(new_n77_), .O(z18));
  aoi21  g102(.a(new_n89_), .b(x35), .c(x40), .O(new_n165_));
  oai21  g103(.a(new_n67_), .b(new_n122_), .c(new_n87_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n165_), .c(new_n130_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n63_), .c(x30), .O(z19));
  oai21  g106(.a(x28), .b(new_n88_), .c(new_n67_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x35), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n63_), .c(new_n77_), .d(new_n130_), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z20));
endmodule


