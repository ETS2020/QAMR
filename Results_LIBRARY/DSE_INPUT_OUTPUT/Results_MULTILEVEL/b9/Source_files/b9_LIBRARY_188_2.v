// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n189_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n208_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  inv1   g005(.a(x40), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n67_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x39), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x37), .O(new_n71_));
  nand2  g009(.a(x35), .b(x28), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n70_), .c(new_n64_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(z08));
  nand3  g015(.a(z08), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n75_), .c(new_n63_), .O(new_n80_));
  nand2  g018(.a(new_n68_), .b(x36), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n80_), .c(x16), .O(z00));
  inv1   g020(.a(new_n81_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(new_n64_), .c(new_n65_), .O(new_n85_));
  inv1   g023(.a(x30), .O(new_n86_));
  inv1   g024(.a(x32), .O(new_n87_));
  nor2   g025(.a(new_n66_), .b(x28), .O(new_n88_));
  inv1   g026(.a(new_n88_), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n67_), .c(new_n87_), .d(new_n86_), .O(new_n90_));
  oai21  g028(.a(new_n85_), .b(new_n83_), .c(new_n90_), .O(z01));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g032(.a(x27), .b(x08), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n89_), .c(x04), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(z08), .O(z02));
  nand2  g035(.a(new_n72_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n71_), .b(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n81_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n71_), .b(new_n64_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n83_), .O(z04));
  inv1   g040(.a(x28), .O(new_n103_));
  oai21  g041(.a(new_n103_), .b(new_n64_), .c(new_n71_), .O(new_n104_));
  and2   g042(.a(new_n104_), .b(new_n81_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n63_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n81_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n81_), .O(z09));
  oai21  g057(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(x39), .c(x05), .O(new_n121_));
  inv1   g059(.a(x39), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(x36), .c(x07), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n121_), .c(new_n68_), .O(new_n124_));
  inv1   g062(.a(x07), .O(new_n125_));
  nand4  g063(.a(new_n76_), .b(new_n67_), .c(x35), .d(new_n103_), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n124_), .c(new_n65_), .O(new_n128_));
  nand3  g066(.a(new_n81_), .b(x37), .c(x06), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x27), .O(new_n131_));
  nor3   g069(.a(new_n84_), .b(new_n68_), .c(new_n122_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x05), .c(new_n65_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  nand2  g073(.a(z08), .b(x29), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n88_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  inv1   g077(.a(x08), .O(new_n140_));
  nand2  g078(.a(x27), .b(new_n65_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n89_), .c(new_n140_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n86_), .c(new_n135_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n81_), .O(z11));
  inv1   g083(.a(x10), .O(new_n146_));
  nor3   g084(.a(x37), .b(x36), .c(x35), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n64_), .c(new_n146_), .O(new_n148_));
  nand2  g086(.a(new_n73_), .b(x27), .O(new_n149_));
  oai21  g087(.a(new_n76_), .b(x04), .c(new_n149_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n148_), .c(new_n81_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x28), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n65_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  nand4  g095(.a(new_n73_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n155_), .c(new_n68_), .O(new_n159_));
  nand4  g097(.a(new_n73_), .b(new_n67_), .c(x20), .d(new_n157_), .O(new_n160_));
  nor2   g098(.a(new_n160_), .b(x18), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n159_), .c(x27), .O(new_n162_));
  nand3  g100(.a(new_n132_), .b(new_n152_), .c(new_n65_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n162_), .O(z13));
  nand3  g102(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n165_));
  nand4  g103(.a(new_n153_), .b(new_n87_), .c(new_n86_), .d(x28), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(new_n167_));
  nand3  g105(.a(z08), .b(new_n152_), .c(new_n65_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n167_), .c(new_n165_), .O(new_n169_));
  nand3  g107(.a(new_n71_), .b(new_n66_), .c(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n87_), .c(new_n86_), .O(new_n172_));
  aoi21  g110(.a(new_n168_), .b(new_n149_), .c(new_n83_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n172_), .c(new_n169_), .O(z14));
  inv1   g112(.a(x12), .O(new_n175_));
  nand4  g113(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n175_), .O(z15));
  inv1   g115(.a(x23), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(x22), .c(x01), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n81_), .O(z16));
  inv1   g118(.a(x24), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(x23), .c(x22), .d(x01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n81_), .O(z17));
  nand2  g121(.a(new_n64_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n141_), .c(new_n66_), .O(new_n185_));
  nand2  g123(.a(new_n86_), .b(new_n135_), .O(new_n186_));
  aoi21  g124(.a(new_n185_), .b(new_n103_), .c(new_n186_), .O(new_n187_));
  oai22  g125(.a(new_n187_), .b(new_n83_), .c(new_n93_), .d(new_n76_), .O(z18));
  nand2  g126(.a(new_n81_), .b(new_n64_), .O(new_n189_));
  oai21  g127(.a(new_n66_), .b(x04), .c(x40), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(new_n189_), .c(new_n103_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n140_), .O(new_n192_));
  nand2  g130(.a(x39), .b(x29), .O(new_n193_));
  nand2  g131(.a(x40), .b(new_n66_), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n137_), .c(new_n103_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n193_), .O(new_n196_));
  nand2  g134(.a(new_n67_), .b(new_n66_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n137_), .c(new_n103_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n68_), .O(new_n199_));
  nand3  g137(.a(new_n199_), .b(new_n196_), .c(new_n192_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n86_), .c(new_n135_), .O(new_n201_));
  nand2  g139(.a(new_n201_), .b(new_n81_), .O(z19));
  nand2  g140(.a(x28), .b(new_n64_), .O(new_n203_));
  nand2  g141(.a(new_n193_), .b(x40), .O(new_n204_));
  nand2  g142(.a(new_n68_), .b(new_n67_), .O(new_n205_));
  aoi22  g143(.a(new_n205_), .b(new_n204_), .c(new_n203_), .d(x35), .O(new_n206_));
  aoi21  g144(.a(new_n194_), .b(new_n189_), .c(x08), .O(new_n207_));
  oai21  g145(.a(new_n207_), .b(new_n206_), .c(new_n86_), .O(new_n208_));
  nor2   g146(.a(new_n208_), .b(x09), .O(z20));
endmodule


