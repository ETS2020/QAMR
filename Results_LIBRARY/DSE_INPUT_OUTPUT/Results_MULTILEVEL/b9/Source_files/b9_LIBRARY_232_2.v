// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n175_, new_n176_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_, new_n185_, new_n186_, new_n187_, new_n189_,
    new_n190_, new_n191_;
  inv1   g000(.a(x29), .O(new_n63_));
  nor2   g001(.a(x30), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g008(.a(new_n70_), .b(x40), .c(x39), .d(new_n67_), .O(new_n71_));
  inv1   g009(.a(x37), .O(new_n72_));
  nand2  g010(.a(x35), .b(x28), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n71_), .c(new_n66_), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(x10), .c(new_n67_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n76_), .c(new_n65_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  oai21  g021(.a(new_n68_), .b(x28), .c(new_n69_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x27), .c(x32), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(x29), .c(new_n83_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x04), .O(z01));
  inv1   g025(.a(x08), .O(new_n88_));
  aoi21  g026(.a(x30), .b(new_n88_), .c(new_n63_), .O(new_n89_));
  nor2   g027(.a(new_n68_), .b(x28), .O(new_n90_));
  nand2  g028(.a(new_n66_), .b(new_n88_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x04), .c(new_n77_), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n64_), .c(new_n89_), .d(x02), .O(z02));
  nand2  g032(.a(new_n73_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n72_), .b(new_n66_), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n95_), .c(new_n64_), .O(z03));
  inv1   g036(.a(new_n64_), .O(new_n99_));
  nand2  g037(.a(new_n72_), .b(new_n66_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n95_), .c(new_n99_), .d(new_n96_), .O(z04));
  inv1   g039(.a(x28), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n66_), .O(new_n103_));
  inv1   g041(.a(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n99_), .c(new_n72_), .O(z05));
  oai21  g043(.a(new_n103_), .b(x37), .c(new_n99_), .O(z06));
  inv1   g044(.a(x33), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n65_), .O(new_n108_));
  inv1   g046(.a(x00), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n109_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  oai22  g052(.a(new_n114_), .b(x14), .c(new_n111_), .d(x31), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n99_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  nand2  g055(.a(new_n77_), .b(new_n99_), .O(z08));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n99_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x05), .O(new_n122_));
  nand2  g060(.a(new_n77_), .b(x07), .O(new_n123_));
  oai21  g061(.a(new_n77_), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n84_), .c(x27), .O(new_n125_));
  inv1   g063(.a(x39), .O(new_n126_));
  oai21  g064(.a(x32), .b(x30), .c(x40), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n67_), .O(new_n131_));
  nand3  g069(.a(x37), .b(x27), .c(x06), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n99_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(x39), .b(new_n88_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n66_), .c(new_n67_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n92_), .c(new_n134_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n63_), .c(x30), .O(z11));
  inv1   g076(.a(x10), .O(new_n139_));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n66_), .c(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n74_), .b(x27), .O(new_n142_));
  oai21  g080(.a(new_n77_), .b(x04), .c(new_n142_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n141_), .c(new_n64_), .O(z12));
  inv1   g082(.a(x13), .O(new_n145_));
  inv1   g083(.a(x40), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x28), .c(new_n146_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x39), .c(new_n145_), .d(new_n67_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand4  g089(.a(new_n74_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nor3   g092(.a(new_n127_), .b(new_n126_), .c(x13), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n67_), .c(new_n64_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n154_), .O(z13));
  nand3  g095(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n158_));
  nand3  g096(.a(new_n78_), .b(new_n145_), .c(new_n67_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n99_), .O(new_n160_));
  aoi21  g098(.a(x36), .b(x35), .c(x32), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n83_), .c(new_n63_), .d(x28), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n158_), .O(new_n164_));
  nand3  g102(.a(new_n72_), .b(new_n68_), .c(x28), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(x27), .c(x32), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n83_), .O(new_n167_));
  nand2  g105(.a(new_n159_), .b(new_n142_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n63_), .O(new_n170_));
  nand3  g108(.a(new_n159_), .b(new_n142_), .c(x30), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n170_), .c(new_n164_), .O(z14));
  inv1   g110(.a(x12), .O(new_n173_));
  nor2   g111(.a(new_n120_), .b(new_n173_), .O(z15));
  inv1   g112(.a(x23), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(x22), .c(x01), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n99_), .O(z16));
  inv1   g115(.a(x24), .O(new_n178_));
  nand4  g116(.a(new_n178_), .b(x23), .c(x22), .d(x01), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n99_), .O(z17));
  nand2  g118(.a(x27), .b(new_n67_), .O(new_n181_));
  oai21  g119(.a(x27), .b(new_n88_), .c(new_n181_), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(x35), .c(new_n102_), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(new_n83_), .c(new_n63_), .d(new_n134_), .O(z18));
  nand2  g122(.a(x27), .b(x04), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n91_), .c(new_n90_), .O(new_n186_));
  nand4  g124(.a(new_n186_), .b(new_n83_), .c(new_n63_), .d(new_n134_), .O(new_n187_));
  inv1   g125(.a(new_n187_), .O(z19));
  oai21  g126(.a(x28), .b(new_n88_), .c(new_n66_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x35), .O(new_n190_));
  nand4  g128(.a(new_n190_), .b(new_n83_), .c(new_n63_), .d(new_n134_), .O(new_n191_));
  inv1   g129(.a(new_n191_), .O(z20));
endmodule


