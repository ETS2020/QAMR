// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x10), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x35), .O(new_n67_));
  oai21  g005(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n64_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x36), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n75_), .c(x16), .O(z00));
  inv1   g015(.a(new_n76_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n66_), .c(new_n64_), .O(new_n83_));
  inv1   g021(.a(x36), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  nand4  g024(.a(new_n86_), .b(new_n84_), .c(new_n80_), .d(new_n79_), .O(new_n87_));
  oai21  g025(.a(new_n83_), .b(new_n78_), .c(new_n87_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n86_), .c(x04), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x39), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n84_), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x40), .O(z02));
  nand2  g034(.a(new_n71_), .b(x27), .O(new_n97_));
  aoi21  g035(.a(new_n70_), .b(new_n66_), .c(new_n78_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n97_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n97_), .c(new_n100_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n70_), .c(new_n78_), .O(z05));
  nand3  g041(.a(new_n102_), .b(new_n76_), .c(new_n70_), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x14), .O(new_n111_));
  inv1   g049(.a(x25), .O(new_n112_));
  nand3  g050(.a(x38), .b(new_n112_), .c(new_n108_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  oai21  g052(.a(new_n110_), .b(x31), .c(new_n114_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n76_), .O(z07));
  inv1   g055(.a(x39), .O(new_n118_));
  inv1   g056(.a(x40), .O(new_n119_));
  aoi21  g057(.a(new_n118_), .b(new_n84_), .c(new_n119_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n76_), .O(z09));
  nand2  g060(.a(x27), .b(x07), .O(new_n123_));
  oai21  g061(.a(new_n123_), .b(x04), .c(new_n119_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x36), .O(new_n125_));
  nand2  g063(.a(x40), .b(x39), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x07), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(x35), .c(new_n85_), .d(x27), .O(new_n130_));
  nand4  g068(.a(new_n81_), .b(x40), .c(x39), .d(x05), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n64_), .O(new_n133_));
  nand3  g071(.a(x37), .b(x27), .c(x06), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n125_), .O(z10));
  inv1   g073(.a(x09), .O(new_n136_));
  aoi21  g074(.a(x40), .b(x36), .c(x27), .O(new_n137_));
  nand3  g075(.a(x35), .b(new_n85_), .c(new_n64_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n84_), .c(new_n137_), .O(new_n139_));
  aoi21  g077(.a(x39), .b(x29), .c(x36), .O(new_n140_));
  nand2  g078(.a(x27), .b(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(x35), .c(new_n85_), .O(new_n142_));
  oai21  g080(.a(new_n140_), .b(new_n119_), .c(new_n142_), .O(new_n143_));
  oai21  g081(.a(new_n139_), .b(x08), .c(new_n143_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n79_), .c(new_n136_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(z11));
  nand2  g084(.a(new_n70_), .b(new_n67_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  inv1   g086(.a(new_n126_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(x10), .c(new_n64_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g089(.a(new_n149_), .b(new_n64_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n70_), .c(new_n85_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n151_), .c(new_n76_), .O(z12));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand4  g094(.a(new_n72_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n157_));
  nor2   g095(.a(x13), .b(x04), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n149_), .c(new_n85_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n157_), .c(new_n66_), .O(new_n160_));
  inv1   g098(.a(x13), .O(new_n161_));
  nand4  g099(.a(new_n81_), .b(x39), .c(new_n161_), .d(new_n64_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n84_), .c(new_n119_), .O(new_n163_));
  or2    g101(.a(new_n163_), .b(new_n160_), .O(z13));
  nand4  g102(.a(new_n147_), .b(x20), .c(new_n156_), .d(new_n155_), .O(new_n165_));
  nand2  g103(.a(new_n82_), .b(x28), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n158_), .c(new_n149_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n165_), .O(new_n168_));
  oai21  g106(.a(x37), .b(x28), .c(x27), .O(new_n169_));
  nand2  g107(.a(new_n158_), .b(new_n149_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  aoi21  g109(.a(new_n82_), .b(new_n66_), .c(new_n78_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n171_), .c(new_n168_), .O(z14));
  inv1   g111(.a(x12), .O(new_n174_));
  nand4  g112(.a(new_n76_), .b(x34), .c(x27), .d(x26), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n174_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(x22), .c(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n76_), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(x23), .c(x22), .d(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n76_), .O(z17));
  nand2  g120(.a(x27), .b(new_n64_), .O(new_n183_));
  nand2  g121(.a(new_n66_), .b(x08), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n183_), .c(new_n67_), .O(new_n185_));
  nand2  g123(.a(new_n79_), .b(new_n136_), .O(new_n186_));
  aoi21  g124(.a(new_n185_), .b(new_n85_), .c(new_n186_), .O(new_n187_));
  nand2  g125(.a(new_n149_), .b(new_n84_), .O(new_n188_));
  oai22  g126(.a(new_n188_), .b(new_n90_), .c(new_n187_), .d(new_n78_), .O(z18));
  nor2   g127(.a(new_n140_), .b(new_n119_), .O(new_n190_));
  aoi21  g128(.a(x28), .b(new_n66_), .c(new_n67_), .O(new_n191_));
  aoi21  g129(.a(new_n84_), .b(new_n67_), .c(new_n137_), .O(new_n192_));
  oai22  g130(.a(new_n192_), .b(x08), .c(new_n191_), .d(new_n190_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n79_), .c(new_n136_), .O(new_n194_));
  inv1   g132(.a(new_n194_), .O(z20));
  inv1   g133(.a(new_n145_), .O(z19));
endmodule


