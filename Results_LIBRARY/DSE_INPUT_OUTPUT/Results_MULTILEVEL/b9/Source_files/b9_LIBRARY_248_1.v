// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n64_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x16), .O(z00));
  nand2  g009(.a(x40), .b(new_n63_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  inv1   g011(.a(x30), .O(new_n74_));
  inv1   g012(.a(x32), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(new_n74_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n73_), .O(z01));
  aoi21  g020(.a(x29), .b(x08), .c(x02), .O(new_n83_));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x39), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n83_), .c(x15), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x40), .O(z02));
  nand2  g026(.a(new_n66_), .b(x27), .O(new_n89_));
  inv1   g027(.a(x21), .O(new_n90_));
  inv1   g028(.a(x27), .O(new_n91_));
  aoi21  g029(.a(new_n65_), .b(new_n91_), .c(new_n90_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n89_), .c(new_n73_), .O(z03));
  aoi21  g031(.a(new_n65_), .b(new_n91_), .c(x21), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n89_), .c(new_n73_), .O(z04));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n65_), .c(new_n73_), .O(z05));
  nand3  g035(.a(new_n96_), .b(new_n72_), .c(new_n65_), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(z06));
  inv1   g037(.a(x33), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n63_), .O(new_n101_));
  inv1   g039(.a(x00), .O(new_n102_));
  oai21  g040(.a(x25), .b(new_n102_), .c(x38), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(new_n107_));
  oai22  g045(.a(new_n107_), .b(x14), .c(new_n104_), .d(x31), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n72_), .O(z07));
  inv1   g048(.a(x39), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(x15), .c(new_n64_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n72_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(x04), .O(new_n116_));
  inv1   g054(.a(x05), .O(new_n117_));
  nand2  g055(.a(x40), .b(x39), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x07), .O(new_n119_));
  oai21  g057(.a(new_n118_), .b(new_n117_), .c(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n79_), .c(x27), .O(new_n121_));
  oai21  g059(.a(x32), .b(x30), .c(x40), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n111_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n116_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n72_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x04), .O(new_n130_));
  inv1   g068(.a(x09), .O(new_n131_));
  nand2  g069(.a(new_n74_), .b(new_n131_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n130_), .c(x15), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x40), .O(new_n134_));
  inv1   g072(.a(new_n118_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x29), .O(new_n136_));
  nor2   g074(.a(new_n91_), .b(new_n116_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n78_), .c(new_n136_), .O(new_n138_));
  oai21  g076(.a(new_n78_), .b(new_n91_), .c(new_n129_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n74_), .c(new_n131_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n134_), .O(z11));
  inv1   g080(.a(x10), .O(new_n143_));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n91_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n118_), .b(x04), .c(new_n68_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(new_n72_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x28), .c(new_n64_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x39), .c(new_n148_), .d(new_n116_), .O(new_n151_));
  inv1   g089(.a(x18), .O(new_n152_));
  inv1   g090(.a(x19), .O(new_n153_));
  nand4  g091(.a(new_n67_), .b(x20), .c(new_n153_), .d(new_n152_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n151_), .c(new_n63_), .O(new_n155_));
  nand4  g093(.a(new_n67_), .b(new_n64_), .c(x20), .d(new_n153_), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(x18), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n155_), .c(x27), .O(new_n158_));
  nand4  g096(.a(new_n123_), .b(x15), .c(new_n148_), .d(new_n116_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  nand3  g098(.a(x20), .b(new_n153_), .c(new_n152_), .O(new_n161_));
  nand4  g099(.a(new_n149_), .b(new_n75_), .c(new_n74_), .d(x28), .O(new_n162_));
  nand3  g100(.a(x39), .b(new_n148_), .c(new_n116_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n162_), .c(new_n63_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n64_), .c(new_n161_), .O(new_n166_));
  inv1   g104(.a(x35), .O(new_n167_));
  nand3  g105(.a(new_n65_), .b(new_n167_), .c(x28), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x27), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n75_), .c(new_n74_), .O(new_n170_));
  nand2  g108(.a(new_n163_), .b(new_n68_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x15), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n166_), .c(new_n69_), .O(z14));
  nand4  g112(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n72_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand4  g115(.a(new_n72_), .b(new_n177_), .c(x22), .d(x01), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z16));
  inv1   g117(.a(x01), .O(new_n180_));
  inv1   g118(.a(x24), .O(new_n181_));
  nand4  g119(.a(new_n72_), .b(new_n181_), .c(x23), .d(x22), .O(new_n182_));
  nor2   g120(.a(new_n182_), .b(new_n180_), .O(z17));
  nand2  g121(.a(x27), .b(new_n116_), .O(new_n184_));
  nand2  g122(.a(new_n91_), .b(x08), .O(new_n185_));
  aoi21  g123(.a(new_n185_), .b(new_n184_), .c(new_n167_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n77_), .c(new_n132_), .O(new_n187_));
  inv1   g125(.a(new_n136_), .O(new_n188_));
  nand3  g126(.a(new_n188_), .b(x15), .c(x08), .O(new_n189_));
  oai21  g127(.a(new_n187_), .b(new_n73_), .c(new_n189_), .O(z18));
  oai21  g128(.a(new_n184_), .b(new_n78_), .c(new_n129_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n138_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n74_), .c(new_n131_), .O(new_n193_));
  nand2  g131(.a(new_n193_), .b(new_n72_), .O(z19));
  oai21  g132(.a(new_n167_), .b(new_n91_), .c(new_n129_), .O(new_n195_));
  aoi21  g133(.a(x28), .b(new_n91_), .c(new_n167_), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n188_), .c(new_n195_), .O(new_n197_));
  nand3  g135(.a(new_n197_), .b(new_n74_), .c(new_n131_), .O(new_n198_));
  nand2  g136(.a(new_n198_), .b(new_n72_), .O(z20));
endmodule


