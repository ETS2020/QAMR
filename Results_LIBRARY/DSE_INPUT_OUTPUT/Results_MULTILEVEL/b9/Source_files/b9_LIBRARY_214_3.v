// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n178_,
    new_n179_, new_n181_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n199_;
  nand2  g000(.a(x40), .b(x19), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x19), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  or2    g020(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n81_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x39), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n86_), .c(new_n72_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x40), .O(z02));
  nand2  g030(.a(new_n69_), .b(x27), .O(new_n93_));
  aoi21  g031(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n93_), .c(new_n96_), .O(z04));
  nor2   g035(.a(new_n81_), .b(new_n67_), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n63_), .c(new_n68_), .O(z05));
  oai21  g038(.a(new_n98_), .b(x37), .c(new_n63_), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n65_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n104_), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  oai21  g048(.a(new_n106_), .b(x31), .c(new_n110_), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n63_), .O(z07));
  oai21  g051(.a(x39), .b(x19), .c(x40), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z08));
  inv1   g053(.a(x11), .O(new_n116_));
  nand4  g054(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  inv1   g056(.a(x04), .O(new_n119_));
  nand2  g057(.a(new_n114_), .b(x07), .O(new_n120_));
  nand4  g058(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n120_), .c(new_n82_), .O(new_n122_));
  nand3  g060(.a(new_n63_), .b(x37), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(x39), .O(new_n126_));
  nand2  g064(.a(new_n80_), .b(new_n79_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x40), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(new_n72_), .c(x05), .d(new_n119_), .O(new_n130_));
  oai21  g068(.a(new_n125_), .b(new_n67_), .c(new_n130_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x04), .O(new_n133_));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(new_n79_), .b(new_n134_), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n133_), .c(new_n72_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x40), .O(new_n137_));
  nand3  g075(.a(x40), .b(x39), .c(x29), .O(new_n138_));
  nor2   g076(.a(new_n67_), .b(new_n119_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n87_), .c(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n87_), .b(new_n67_), .c(new_n132_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n79_), .c(new_n134_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n137_), .O(z11));
  inv1   g082(.a(x35), .O(new_n145_));
  inv1   g083(.a(x36), .O(new_n146_));
  nand3  g084(.a(new_n68_), .b(new_n146_), .c(new_n145_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  aoi21  g086(.a(x39), .b(new_n119_), .c(new_n70_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n72_), .O(new_n150_));
  oai21  g088(.a(new_n70_), .b(x40), .c(new_n150_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n74_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n119_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  nand2  g094(.a(new_n69_), .b(new_n68_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x20), .c(new_n72_), .d(new_n156_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x27), .O(new_n160_));
  nand4  g098(.a(new_n127_), .b(x39), .c(new_n152_), .d(new_n119_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n72_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x40), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n160_), .O(z13));
  nand2  g102(.a(x20), .b(new_n156_), .O(new_n165_));
  nand4  g103(.a(new_n153_), .b(new_n80_), .c(new_n79_), .d(x28), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(new_n167_));
  nand4  g105(.a(x40), .b(x39), .c(new_n152_), .d(new_n119_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n167_), .c(new_n165_), .O(new_n169_));
  nand3  g107(.a(new_n68_), .b(new_n145_), .c(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n80_), .c(new_n79_), .O(new_n172_));
  inv1   g110(.a(new_n70_), .O(new_n173_));
  aoi21  g111(.a(new_n168_), .b(new_n173_), .c(x19), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n172_), .c(new_n169_), .O(z14));
  nand4  g113(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n63_), .O(z15));
  inv1   g115(.a(x23), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(x22), .c(x01), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n63_), .O(z16));
  inv1   g118(.a(x01), .O(new_n181_));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n63_), .b(new_n182_), .c(x23), .d(x22), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n181_), .O(z17));
  nand2  g122(.a(x27), .b(new_n119_), .O(new_n185_));
  nand2  g123(.a(new_n67_), .b(x08), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(new_n145_), .O(new_n187_));
  aoi21  g125(.a(new_n187_), .b(new_n81_), .c(new_n135_), .O(new_n188_));
  inv1   g126(.a(new_n138_), .O(new_n189_));
  nand3  g127(.a(new_n189_), .b(new_n72_), .c(x08), .O(new_n190_));
  oai21  g128(.a(new_n188_), .b(new_n64_), .c(new_n190_), .O(z18));
  oai21  g129(.a(new_n185_), .b(new_n87_), .c(new_n132_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n140_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n79_), .c(new_n134_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n63_), .O(z19));
  oai21  g133(.a(new_n145_), .b(new_n67_), .c(new_n132_), .O(new_n196_));
  aoi21  g134(.a(x28), .b(new_n67_), .c(new_n145_), .O(new_n197_));
  oai21  g135(.a(new_n197_), .b(new_n189_), .c(new_n196_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n79_), .c(new_n134_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n63_), .O(z20));
endmodule


