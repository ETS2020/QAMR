// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n189_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x30), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x18), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(x16), .O(z00));
  inv1   g017(.a(x28), .O(new_n80_));
  aoi21  g018(.a(x35), .b(new_n80_), .c(x36), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x27), .c(x32), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x18), .c(new_n77_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g026(.a(x35), .b(new_n80_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n88_), .c(new_n78_), .d(new_n73_), .O(z02));
  inv1   g030(.a(new_n78_), .O(new_n93_));
  oai21  g031(.a(new_n66_), .b(new_n80_), .c(x27), .O(new_n94_));
  inv1   g032(.a(x21), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n64_), .c(new_n95_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n93_), .O(z03));
  nand2  g036(.a(new_n96_), .b(new_n64_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n94_), .c(new_n78_), .d(new_n95_), .O(z04));
  nor2   g038(.a(new_n80_), .b(new_n64_), .O(new_n101_));
  inv1   g039(.a(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n78_), .c(new_n96_), .O(z05));
  oai21  g041(.a(new_n101_), .b(x37), .c(new_n78_), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n63_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n78_), .O(z07));
  inv1   g053(.a(x39), .O(new_n116_));
  inv1   g054(.a(x40), .O(new_n117_));
  nor3   g055(.a(new_n93_), .b(new_n117_), .c(new_n116_), .O(z08));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  nand3  g059(.a(new_n78_), .b(new_n72_), .c(x07), .O(new_n122_));
  inv1   g060(.a(x18), .O(new_n123_));
  nand3  g061(.a(new_n73_), .b(new_n123_), .c(x05), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n122_), .c(new_n81_), .O(new_n125_));
  inv1   g063(.a(x06), .O(new_n126_));
  nand2  g064(.a(new_n78_), .b(x37), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  aoi21  g066(.a(new_n125_), .b(new_n65_), .c(new_n128_), .O(new_n129_));
  nand2  g067(.a(x32), .b(new_n123_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n77_), .c(new_n117_), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(x39), .c(x05), .d(new_n65_), .O(new_n132_));
  oai21  g070(.a(new_n129_), .b(new_n64_), .c(new_n132_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  nor2   g073(.a(new_n64_), .b(new_n65_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n89_), .c(new_n135_), .O(new_n137_));
  inv1   g075(.a(x08), .O(new_n138_));
  nand4  g076(.a(x35), .b(new_n80_), .c(x27), .d(new_n65_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand4  g079(.a(new_n141_), .b(new_n77_), .c(new_n123_), .d(new_n134_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(z11));
  inv1   g081(.a(x10), .O(new_n144_));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n64_), .c(new_n144_), .O(new_n146_));
  oai22  g084(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n146_), .c(new_n93_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand3  g087(.a(new_n73_), .b(new_n149_), .c(new_n65_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x30), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x18), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n117_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n149_), .d(new_n65_), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  inv1   g094(.a(x20), .O(new_n157_));
  nor2   g095(.a(new_n70_), .b(new_n157_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n156_), .c(new_n123_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  inv1   g099(.a(x32), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n77_), .c(new_n117_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x39), .c(new_n149_), .d(new_n65_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n161_), .c(new_n152_), .O(z13));
  nand4  g103(.a(new_n153_), .b(new_n162_), .c(new_n77_), .d(x28), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(new_n167_));
  oai22  g105(.a(new_n167_), .b(new_n150_), .c(new_n157_), .d(x19), .O(new_n168_));
  nand3  g106(.a(new_n96_), .b(new_n66_), .c(x28), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(x27), .c(x32), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(x18), .c(new_n77_), .O(new_n171_));
  nand2  g109(.a(x27), .b(new_n123_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n70_), .c(new_n150_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n171_), .c(new_n168_), .O(z14));
  inv1   g112(.a(x12), .O(new_n175_));
  nor2   g113(.a(new_n120_), .b(new_n175_), .O(z15));
  inv1   g114(.a(x23), .O(new_n177_));
  nand4  g115(.a(new_n78_), .b(new_n177_), .c(x22), .d(x01), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z16));
  inv1   g117(.a(x01), .O(new_n180_));
  inv1   g118(.a(x24), .O(new_n181_));
  nand4  g119(.a(new_n78_), .b(new_n181_), .c(x23), .d(x22), .O(new_n182_));
  nor2   g120(.a(new_n182_), .b(new_n180_), .O(z17));
  nand3  g121(.a(x35), .b(new_n80_), .c(new_n64_), .O(new_n184_));
  aoi21  g122(.a(new_n184_), .b(new_n135_), .c(new_n138_), .O(new_n185_));
  nand2  g123(.a(new_n139_), .b(new_n134_), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n185_), .c(new_n123_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n77_), .O(z18));
  nand2  g126(.a(new_n141_), .b(new_n134_), .O(new_n189_));
  aoi21  g127(.a(new_n189_), .b(new_n123_), .c(x30), .O(z19));
  inv1   g128(.a(new_n135_), .O(new_n191_));
  oai21  g129(.a(new_n66_), .b(new_n64_), .c(new_n138_), .O(new_n192_));
  aoi21  g130(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n193_));
  oai21  g131(.a(new_n193_), .b(new_n191_), .c(new_n192_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n134_), .O(new_n195_));
  aoi21  g133(.a(new_n195_), .b(new_n123_), .c(x30), .O(z20));
endmodule


