// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  inv1   g005(.a(x40), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n67_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x39), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x37), .O(new_n71_));
  nand2  g009(.a(x35), .b(x28), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n70_), .c(new_n64_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n65_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n75_), .c(new_n63_), .O(new_n80_));
  nand2  g018(.a(new_n68_), .b(x36), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n80_), .c(x16), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  oai21  g022(.a(new_n66_), .b(x28), .c(new_n67_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n81_), .c(x04), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g029(.a(new_n66_), .b(x28), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  nor2   g031(.a(x27), .b(x08), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n93_), .c(x04), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n91_), .c(new_n77_), .O(z02));
  inv1   g034(.a(new_n81_), .O(new_n97_));
  nand2  g035(.a(new_n72_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n71_), .b(new_n64_), .c(new_n99_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(z03));
  aoi21  g039(.a(new_n71_), .b(new_n64_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n98_), .c(new_n97_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n71_), .c(new_n97_), .O(z05));
  nand3  g043(.a(new_n104_), .b(new_n81_), .c(new_n71_), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n63_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand3  g051(.a(x38), .b(new_n113_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(x14), .c(new_n112_), .d(x31), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n81_), .O(z07));
  nand2  g056(.a(new_n81_), .b(new_n76_), .O(z08));
  inv1   g057(.a(x11), .O(new_n120_));
  nand4  g058(.a(new_n81_), .b(x34), .c(x27), .d(x26), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n120_), .O(z09));
  nand3  g060(.a(new_n85_), .b(x39), .c(x05), .O(new_n123_));
  inv1   g061(.a(x39), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(x36), .c(x07), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n123_), .c(new_n68_), .O(new_n126_));
  inv1   g064(.a(x07), .O(new_n127_));
  inv1   g065(.a(x28), .O(new_n128_));
  nand4  g066(.a(new_n76_), .b(new_n67_), .c(x35), .d(new_n128_), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n126_), .c(new_n65_), .O(new_n131_));
  nand3  g069(.a(new_n81_), .b(x37), .c(x06), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x27), .O(new_n134_));
  oai21  g072(.a(x32), .b(x30), .c(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(new_n124_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x05), .c(new_n65_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(z10));
  nand2  g076(.a(new_n81_), .b(new_n64_), .O(new_n139_));
  oai21  g077(.a(new_n93_), .b(x04), .c(x40), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n139_), .c(x08), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  nor2   g080(.a(x40), .b(x36), .O(new_n143_));
  aoi21  g081(.a(new_n142_), .b(x40), .c(new_n143_), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n92_), .c(new_n144_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n141_), .c(new_n83_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(x09), .O(z11));
  inv1   g086(.a(x10), .O(new_n149_));
  nor3   g087(.a(x37), .b(x36), .c(x35), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n64_), .c(new_n149_), .O(new_n151_));
  oai22  g089(.a(new_n76_), .b(x04), .c(new_n74_), .d(new_n64_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n151_), .c(new_n81_), .O(z12));
  inv1   g091(.a(x13), .O(new_n154_));
  oai21  g092(.a(new_n67_), .b(new_n66_), .c(x28), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n154_), .d(new_n65_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n73_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n156_), .c(new_n68_), .O(new_n160_));
  nand4  g098(.a(new_n73_), .b(new_n67_), .c(x20), .d(new_n158_), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(x18), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n160_), .c(x27), .O(new_n163_));
  nand3  g101(.a(new_n136_), .b(new_n154_), .c(new_n65_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(z13));
  nand3  g103(.a(x39), .b(new_n154_), .c(new_n65_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(x40), .c(new_n143_), .O(new_n167_));
  nand4  g105(.a(x27), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(new_n74_), .O(new_n169_));
  nand2  g107(.a(x40), .b(new_n66_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x36), .O(new_n171_));
  nand3  g109(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n172_));
  nor2   g110(.a(new_n68_), .b(x37), .O(new_n173_));
  aoi22  g111(.a(new_n173_), .b(new_n66_), .c(new_n172_), .d(new_n171_), .O(new_n174_));
  oai22  g112(.a(new_n174_), .b(new_n128_), .c(new_n68_), .d(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n84_), .c(new_n83_), .O(new_n176_));
  oai21  g114(.a(new_n169_), .b(new_n167_), .c(new_n176_), .O(z14));
  nand4  g115(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n81_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand4  g118(.a(new_n81_), .b(new_n180_), .c(x22), .d(x01), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z16));
  inv1   g120(.a(x01), .O(new_n183_));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n81_), .b(new_n184_), .c(x23), .d(x22), .O(new_n185_));
  nor2   g123(.a(new_n185_), .b(new_n183_), .O(z17));
  nand2  g124(.a(x27), .b(new_n65_), .O(new_n187_));
  nand2  g125(.a(new_n64_), .b(x08), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n187_), .c(new_n66_), .O(new_n189_));
  inv1   g127(.a(x09), .O(new_n190_));
  nand2  g128(.a(new_n83_), .b(new_n190_), .O(new_n191_));
  aoi21  g129(.a(new_n189_), .b(new_n128_), .c(new_n191_), .O(new_n192_));
  oai22  g130(.a(new_n192_), .b(new_n97_), .c(new_n90_), .d(new_n76_), .O(z18));
  aoi21  g131(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n194_));
  nor2   g132(.a(new_n194_), .b(new_n144_), .O(new_n195_));
  aoi21  g133(.a(new_n170_), .b(new_n139_), .c(x08), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n195_), .c(new_n83_), .O(new_n197_));
  nor2   g135(.a(new_n197_), .b(x09), .O(z20));
  nor2   g136(.a(new_n147_), .b(x09), .O(z19));
endmodule


