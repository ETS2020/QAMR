// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n184_, new_n186_, new_n187_,
    new_n188_, new_n190_, new_n191_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x10), .O(new_n63_));
  nand2  g001(.a(x32), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(x15), .c(x16), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n74_), .c(x27), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n63_), .c(new_n73_), .O(new_n79_));
  nand4  g017(.a(new_n79_), .b(x39), .c(new_n72_), .d(new_n71_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n70_), .O(z00));
  inv1   g019(.a(new_n64_), .O(new_n82_));
  inv1   g020(.a(x30), .O(new_n83_));
  nor2   g021(.a(new_n75_), .b(x28), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x36), .c(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n74_), .c(new_n83_), .O(new_n86_));
  oai21  g024(.a(new_n82_), .b(x04), .c(new_n86_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  nand2  g029(.a(x35), .b(new_n91_), .O(new_n92_));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n92_), .c(x04), .O(new_n94_));
  nand2  g032(.a(x40), .b(x39), .O(new_n95_));
  inv1   g033(.a(new_n95_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n94_), .c(new_n90_), .d(new_n64_), .O(z02));
  nand2  g035(.a(new_n66_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(x27), .O(new_n100_));
  aoi21  g038(.a(new_n65_), .b(new_n100_), .c(new_n99_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n82_), .O(z03));
  aoi21  g040(.a(new_n65_), .b(new_n100_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n98_), .c(new_n82_), .O(z04));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n65_), .c(new_n82_), .O(z05));
  nand3  g044(.a(new_n105_), .b(new_n64_), .c(new_n65_), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n72_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  nor3   g058(.a(new_n82_), .b(new_n73_), .c(new_n120_), .O(z08));
  nand4  g059(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n64_), .O(z09));
  aoi21  g061(.a(x35), .b(new_n91_), .c(x36), .O(new_n124_));
  nand3  g062(.a(new_n95_), .b(new_n64_), .c(x07), .O(new_n125_));
  nand3  g063(.a(new_n96_), .b(new_n74_), .c(x05), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(new_n127_));
  inv1   g065(.a(x06), .O(new_n128_));
  nand2  g066(.a(new_n64_), .b(x37), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  aoi21  g068(.a(new_n127_), .b(new_n71_), .c(new_n130_), .O(new_n131_));
  nand2  g069(.a(x32), .b(x10), .O(new_n132_));
  nand2  g070(.a(new_n74_), .b(x30), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n132_), .c(new_n73_), .O(new_n134_));
  nand4  g072(.a(new_n134_), .b(x39), .c(x05), .d(new_n71_), .O(new_n135_));
  oai21  g073(.a(new_n131_), .b(new_n100_), .c(new_n135_), .O(z10));
  inv1   g074(.a(x09), .O(new_n137_));
  nand2  g075(.a(new_n96_), .b(x29), .O(new_n138_));
  nor2   g076(.a(new_n100_), .b(new_n71_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n92_), .c(new_n138_), .O(new_n140_));
  inv1   g078(.a(x08), .O(new_n141_));
  nand3  g079(.a(new_n84_), .b(x27), .c(new_n71_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(new_n64_), .c(new_n83_), .d(new_n137_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(z11));
  oai21  g084(.a(new_n95_), .b(x04), .c(new_n68_), .O(new_n147_));
  nand3  g085(.a(new_n65_), .b(new_n76_), .c(new_n75_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n74_), .c(x27), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n63_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n147_), .O(z12));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n73_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n152_), .d(new_n71_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  nand4  g095(.a(new_n67_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x27), .O(new_n160_));
  oai21  g098(.a(x32), .b(x30), .c(x40), .O(new_n161_));
  nor3   g099(.a(new_n161_), .b(new_n120_), .c(x13), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n71_), .c(new_n82_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n160_), .O(z13));
  nand3  g102(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n165_));
  nor2   g103(.a(x13), .b(x04), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n96_), .c(new_n82_), .O(new_n167_));
  nand4  g105(.a(new_n153_), .b(new_n74_), .c(new_n83_), .d(x28), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n167_), .c(new_n165_), .O(new_n170_));
  nand3  g108(.a(new_n65_), .b(new_n75_), .c(x28), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(x27), .c(x30), .O(new_n172_));
  aoi22  g110(.a(new_n166_), .b(new_n96_), .c(new_n67_), .d(x27), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n74_), .O(new_n174_));
  nand2  g112(.a(new_n173_), .b(x10), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(new_n170_), .O(z14));
  nand4  g114(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n64_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(x22), .c(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n64_), .O(z16));
  inv1   g119(.a(x01), .O(new_n182_));
  inv1   g120(.a(x24), .O(new_n183_));
  nand4  g121(.a(new_n64_), .b(new_n183_), .c(x23), .d(x22), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n182_), .O(z17));
  oai21  g123(.a(new_n92_), .b(x27), .c(new_n138_), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(x08), .O(new_n187_));
  nor2   g125(.a(new_n82_), .b(x30), .O(new_n188_));
  nand4  g126(.a(new_n188_), .b(new_n187_), .c(new_n142_), .d(new_n137_), .O(z18));
  inv1   g127(.a(new_n138_), .O(new_n190_));
  oai21  g128(.a(new_n75_), .b(new_n100_), .c(new_n141_), .O(new_n191_));
  aoi21  g129(.a(x28), .b(new_n100_), .c(new_n75_), .O(new_n192_));
  oai21  g130(.a(new_n192_), .b(new_n190_), .c(new_n191_), .O(new_n193_));
  nand3  g131(.a(new_n193_), .b(new_n83_), .c(new_n137_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n64_), .O(z20));
  inv1   g133(.a(new_n145_), .O(z19));
endmodule


