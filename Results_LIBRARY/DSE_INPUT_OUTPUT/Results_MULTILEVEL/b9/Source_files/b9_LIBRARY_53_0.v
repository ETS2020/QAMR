// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n178_, new_n179_, new_n181_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x16), .O(new_n77_));
  nor2   g015(.a(x30), .b(x27), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n76_), .O(z00));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  aoi21  g020(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n64_), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x30), .c(x04), .O(z01));
  inv1   g023(.a(new_n78_), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g027(.a(new_n66_), .b(x28), .c(x04), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n89_), .c(new_n73_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n86_), .O(new_n92_));
  inv1   g030(.a(x08), .O(new_n93_));
  nand4  g031(.a(x30), .b(new_n64_), .c(new_n93_), .d(x04), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n92_), .O(z02));
  aoi21  g033(.a(x35), .b(x28), .c(new_n64_), .O(new_n96_));
  inv1   g034(.a(x30), .O(new_n97_));
  nor2   g035(.a(x37), .b(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n64_), .c(new_n96_), .O(new_n99_));
  oai21  g037(.a(new_n78_), .b(x21), .c(new_n99_), .O(z03));
  nand2  g038(.a(new_n86_), .b(x21), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n99_), .O(z04));
  oai21  g040(.a(x37), .b(x28), .c(x27), .O(new_n103_));
  nand2  g041(.a(x37), .b(x30), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n103_), .O(z05));
  nand2  g043(.a(new_n104_), .b(new_n64_), .O(new_n106_));
  oai21  g044(.a(x37), .b(x28), .c(new_n106_), .O(z06));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n63_), .O(new_n109_));
  inv1   g047(.a(x00), .O(new_n110_));
  oai21  g048(.a(x25), .b(new_n110_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(x14), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n110_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  oai21  g054(.a(new_n112_), .b(x31), .c(new_n116_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n86_), .c(x03), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z07));
  inv1   g057(.a(x39), .O(new_n120_));
  inv1   g058(.a(x40), .O(new_n121_));
  nor3   g059(.a(new_n78_), .b(new_n121_), .c(new_n120_), .O(z08));
  nand4  g060(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(z09));
  nand2  g062(.a(x05), .b(new_n65_), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n72_), .c(x30), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  nand3  g065(.a(new_n72_), .b(x27), .c(x07), .O(new_n128_));
  nand2  g066(.a(new_n73_), .b(x05), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n128_), .c(new_n83_), .O(new_n130_));
  inv1   g068(.a(x05), .O(new_n131_));
  oai21  g069(.a(x32), .b(x30), .c(x40), .O(new_n132_));
  nor3   g070(.a(new_n132_), .b(new_n120_), .c(new_n131_), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n130_), .c(new_n65_), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n127_), .O(z10));
  nand3  g074(.a(x35), .b(new_n82_), .c(new_n65_), .O(new_n137_));
  nand3  g075(.a(new_n73_), .b(x29), .c(x08), .O(new_n138_));
  nand4  g076(.a(new_n138_), .b(new_n137_), .c(new_n97_), .d(x27), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(x09), .O(z11));
  inv1   g078(.a(x10), .O(new_n141_));
  nor3   g079(.a(x37), .b(x36), .c(x35), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n64_), .c(new_n141_), .O(new_n143_));
  nor2   g081(.a(new_n70_), .b(new_n64_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(new_n145_));
  oai21  g083(.a(new_n72_), .b(x04), .c(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n143_), .c(new_n86_), .O(z12));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand4  g087(.a(new_n144_), .b(x20), .c(new_n149_), .d(new_n148_), .O(new_n150_));
  inv1   g088(.a(x13), .O(new_n151_));
  nand3  g089(.a(new_n73_), .b(new_n151_), .c(new_n65_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x30), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n64_), .O(new_n154_));
  nor2   g092(.a(x30), .b(new_n82_), .O(new_n155_));
  aoi21  g093(.a(x36), .b(x35), .c(x32), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n155_), .c(new_n121_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n151_), .d(new_n65_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n154_), .c(new_n150_), .O(z13));
  nand3  g097(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n160_));
  nand3  g098(.a(new_n156_), .b(new_n97_), .c(x28), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n152_), .c(new_n160_), .O(new_n163_));
  inv1   g101(.a(x37), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n66_), .c(new_n81_), .d(x28), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x27), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n97_), .O(new_n167_));
  nand2  g105(.a(new_n152_), .b(new_n145_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n167_), .c(new_n163_), .O(z14));
  nand4  g107(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand4  g110(.a(new_n86_), .b(new_n172_), .c(x22), .d(x01), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z16));
  inv1   g112(.a(x24), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(x23), .c(x22), .d(x01), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n86_), .O(z17));
  nand2  g115(.a(new_n138_), .b(new_n137_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(x09), .c(x27), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n97_), .O(z18));
  nand4  g118(.a(new_n138_), .b(new_n66_), .c(new_n97_), .d(x27), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(x09), .O(z20));
  nor2   g120(.a(new_n139_), .b(x09), .O(z19));
endmodule


