// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n107_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n207_,
    new_n208_, new_n209_, new_n210_;
  nor2   g000(.a(x40), .b(x36), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n64_), .c(new_n65_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n64_), .d(new_n72_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n63_), .c(new_n77_), .O(z00));
  inv1   g016(.a(new_n63_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x27), .c(x04), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  inv1   g022(.a(x36), .O(new_n85_));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n74_), .O(new_n88_));
  nand4  g026(.a(new_n88_), .b(new_n85_), .c(new_n81_), .d(new_n80_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n84_), .O(z01));
  inv1   g028(.a(x02), .O(new_n91_));
  inv1   g029(.a(x08), .O(new_n92_));
  inv1   g030(.a(x29), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  inv1   g032(.a(new_n87_), .O(new_n95_));
  nor2   g033(.a(x27), .b(x08), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n95_), .c(x04), .O(new_n97_));
  nand2  g035(.a(x40), .b(x39), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(z08));
  nand3  g037(.a(z08), .b(new_n97_), .c(new_n94_), .O(z02));
  nand2  g038(.a(new_n67_), .b(x27), .O(new_n101_));
  inv1   g039(.a(x27), .O(new_n102_));
  aoi21  g040(.a(new_n66_), .b(new_n102_), .c(new_n63_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x21), .O(z03));
  inv1   g042(.a(x21), .O(new_n105_));
  nand3  g043(.a(new_n103_), .b(new_n101_), .c(new_n105_), .O(z04));
  nand2  g044(.a(x28), .b(x27), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n66_), .c(new_n63_), .O(z05));
  nand3  g046(.a(new_n107_), .b(new_n79_), .c(new_n66_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z06));
  inv1   g048(.a(x33), .O(new_n111_));
  nand2  g049(.a(x17), .b(new_n64_), .O(new_n112_));
  inv1   g050(.a(x00), .O(new_n113_));
  oai21  g051(.a(x25), .b(new_n113_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(x25), .O(new_n116_));
  nand3  g054(.a(x38), .b(new_n116_), .c(new_n113_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(x14), .c(new_n115_), .d(x31), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x03), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n79_), .O(z07));
  inv1   g059(.a(x11), .O(new_n122_));
  nand4  g060(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n122_), .O(z09));
  oai21  g062(.a(new_n86_), .b(x28), .c(new_n85_), .O(new_n125_));
  inv1   g063(.a(x07), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x05), .O(new_n127_));
  oai21  g065(.a(x39), .b(new_n126_), .c(new_n127_), .O(new_n128_));
  nor2   g066(.a(x40), .b(new_n126_), .O(new_n129_));
  aoi21  g067(.a(new_n128_), .b(new_n125_), .c(new_n129_), .O(new_n130_));
  nand4  g068(.a(new_n82_), .b(x40), .c(x39), .d(x05), .O(new_n131_));
  oai21  g069(.a(new_n130_), .b(new_n102_), .c(new_n131_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n72_), .O(new_n133_));
  nor2   g071(.a(new_n66_), .b(new_n102_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(x06), .c(new_n63_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n133_), .O(z10));
  oai21  g074(.a(x40), .b(x36), .c(new_n102_), .O(new_n137_));
  inv1   g075(.a(x28), .O(new_n138_));
  nand3  g076(.a(x35), .b(new_n138_), .c(new_n72_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x40), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n137_), .c(x08), .O(new_n141_));
  nand2  g079(.a(x39), .b(x29), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x40), .O(new_n143_));
  nand2  g081(.a(new_n74_), .b(x36), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  aoi22  g083(.a(new_n145_), .b(new_n87_), .c(new_n144_), .d(new_n143_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n141_), .c(new_n80_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(x09), .O(z11));
  nor3   g086(.a(x37), .b(x36), .c(x35), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n102_), .c(new_n73_), .O(new_n150_));
  oai21  g088(.a(new_n98_), .b(x04), .c(new_n69_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n150_), .c(new_n79_), .O(z12));
  inv1   g090(.a(x13), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x28), .c(new_n74_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x39), .c(new_n153_), .d(new_n72_), .O(new_n156_));
  inv1   g094(.a(x18), .O(new_n157_));
  inv1   g095(.a(x19), .O(new_n158_));
  nand4  g096(.a(new_n68_), .b(x20), .c(new_n158_), .d(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(new_n82_), .b(x40), .c(x39), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(x13), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n72_), .c(new_n63_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z13));
  inv1   g103(.a(new_n144_), .O(new_n166_));
  nand3  g104(.a(x20), .b(new_n158_), .c(new_n157_), .O(new_n167_));
  nand4  g105(.a(new_n154_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n168_));
  nand3  g106(.a(x39), .b(new_n153_), .c(new_n72_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n168_), .c(new_n74_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n166_), .c(new_n167_), .O(new_n172_));
  nand3  g110(.a(new_n66_), .b(new_n86_), .c(x28), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n81_), .c(new_n80_), .O(new_n175_));
  oai21  g113(.a(new_n170_), .b(new_n70_), .c(new_n175_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x40), .O(new_n177_));
  nand3  g115(.a(new_n69_), .b(new_n74_), .c(x36), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n177_), .c(new_n172_), .O(z14));
  inv1   g117(.a(x12), .O(new_n180_));
  nor2   g118(.a(new_n123_), .b(new_n180_), .O(z15));
  inv1   g119(.a(x23), .O(new_n182_));
  nand4  g120(.a(new_n79_), .b(new_n182_), .c(x22), .d(x01), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(z16));
  inv1   g122(.a(x01), .O(new_n185_));
  inv1   g123(.a(x24), .O(new_n186_));
  nand4  g124(.a(new_n79_), .b(new_n186_), .c(x23), .d(x22), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(new_n185_), .O(z17));
  oai22  g126(.a(new_n98_), .b(new_n93_), .c(new_n95_), .d(x27), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(x08), .O(new_n190_));
  nor2   g128(.a(new_n102_), .b(x04), .O(new_n191_));
  aoi21  g129(.a(new_n191_), .b(new_n87_), .c(x09), .O(new_n192_));
  nand4  g130(.a(new_n192_), .b(new_n190_), .c(new_n79_), .d(new_n80_), .O(z18));
  inv1   g131(.a(x09), .O(new_n194_));
  oai21  g132(.a(new_n86_), .b(x04), .c(x40), .O(new_n195_));
  nand3  g133(.a(new_n195_), .b(new_n137_), .c(new_n138_), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n92_), .O(new_n197_));
  nand2  g135(.a(x40), .b(new_n86_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n145_), .c(new_n138_), .O(new_n199_));
  nand2  g137(.a(new_n199_), .b(new_n142_), .O(new_n200_));
  nand2  g138(.a(x36), .b(new_n86_), .O(new_n201_));
  nand3  g139(.a(new_n201_), .b(new_n145_), .c(new_n138_), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n74_), .O(new_n203_));
  nand3  g141(.a(new_n203_), .b(new_n200_), .c(new_n197_), .O(new_n204_));
  nand3  g142(.a(new_n204_), .b(new_n80_), .c(new_n194_), .O(new_n205_));
  nand2  g143(.a(new_n205_), .b(new_n79_), .O(z19));
  nand2  g144(.a(x28), .b(new_n102_), .O(new_n207_));
  aoi22  g145(.a(new_n207_), .b(x35), .c(new_n144_), .d(new_n143_), .O(new_n208_));
  aoi21  g146(.a(new_n198_), .b(new_n137_), .c(x08), .O(new_n209_));
  oai21  g147(.a(new_n209_), .b(new_n208_), .c(new_n80_), .O(new_n210_));
  nor2   g148(.a(new_n210_), .b(x09), .O(z20));
endmodule


