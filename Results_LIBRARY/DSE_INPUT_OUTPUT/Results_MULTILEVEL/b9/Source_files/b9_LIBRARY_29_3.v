// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n184_, new_n185_, new_n188_,
    new_n189_, new_n190_, new_n191_, new_n192_;
  nand2  g000(.a(x30), .b(x12), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g008(.a(new_n70_), .b(x40), .c(x39), .d(new_n67_), .O(new_n71_));
  inv1   g009(.a(x37), .O(new_n72_));
  nand2  g010(.a(x35), .b(x28), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n71_), .c(new_n66_), .O(new_n76_));
  and2   g014(.a(x40), .b(x39), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x10), .c(new_n67_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n76_), .c(new_n65_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nor2   g021(.a(new_n68_), .b(x28), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x36), .c(x27), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  oai21  g024(.a(new_n64_), .b(x04), .c(new_n86_), .O(z01));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n84_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n77_), .d(new_n63_), .O(z02));
  nand2  g031(.a(new_n73_), .b(x27), .O(new_n94_));
  nand2  g032(.a(new_n72_), .b(new_n66_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g034(.a(new_n72_), .b(new_n66_), .c(x21), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n64_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n72_), .c(new_n64_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n65_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  oai22  g047(.a(new_n109_), .b(x14), .c(new_n106_), .d(x31), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n63_), .c(x03), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z07));
  nand3  g050(.a(new_n63_), .b(x40), .c(x39), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand4  g053(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  inv1   g055(.a(x28), .O(new_n118_));
  aoi21  g056(.a(x35), .b(new_n118_), .c(x36), .O(new_n119_));
  nand2  g057(.a(x40), .b(x39), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n63_), .c(x07), .O(new_n121_));
  nand3  g059(.a(new_n77_), .b(new_n82_), .c(x05), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(new_n119_), .O(new_n123_));
  nor2   g061(.a(new_n64_), .b(new_n72_), .O(new_n124_));
  aoi22  g062(.a(new_n124_), .b(x06), .c(new_n123_), .d(new_n67_), .O(new_n125_));
  inv1   g063(.a(x12), .O(new_n126_));
  nand2  g064(.a(x30), .b(new_n126_), .O(new_n127_));
  oai21  g065(.a(new_n83_), .b(x30), .c(new_n127_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(x40), .c(x39), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(x05), .c(new_n67_), .O(new_n131_));
  oai21  g069(.a(new_n125_), .b(new_n66_), .c(new_n131_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  nand2  g071(.a(new_n77_), .b(x29), .O(new_n134_));
  oai21  g072(.a(new_n66_), .b(new_n67_), .c(new_n84_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  inv1   g074(.a(x08), .O(new_n137_));
  nand3  g075(.a(new_n84_), .b(x27), .c(new_n67_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n82_), .c(new_n133_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n63_), .O(z11));
  inv1   g080(.a(x10), .O(new_n143_));
  nor3   g081(.a(x37), .b(x36), .c(x35), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n66_), .c(new_n143_), .O(new_n145_));
  oai22  g083(.a(new_n120_), .b(x04), .c(new_n75_), .d(new_n66_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(new_n63_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  inv1   g086(.a(x40), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x28), .c(new_n149_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x39), .c(new_n148_), .d(new_n67_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand4  g092(.a(new_n74_), .b(x20), .c(new_n154_), .d(new_n153_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n152_), .c(x30), .O(new_n156_));
  nor2   g094(.a(new_n155_), .b(x12), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n156_), .c(x27), .O(new_n158_));
  nand3  g096(.a(new_n130_), .b(new_n148_), .c(new_n67_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  nand3  g098(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n161_));
  nor2   g099(.a(x13), .b(x04), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n77_), .c(new_n64_), .O(new_n163_));
  nand4  g101(.a(new_n150_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n163_), .c(new_n161_), .O(new_n166_));
  nand3  g104(.a(new_n72_), .b(new_n68_), .c(x28), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(x27), .c(x32), .O(new_n168_));
  aoi22  g106(.a(new_n162_), .b(new_n77_), .c(new_n74_), .d(x27), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n168_), .c(new_n82_), .O(new_n170_));
  nand2  g108(.a(new_n169_), .b(new_n126_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n170_), .c(new_n166_), .O(z14));
  inv1   g110(.a(x34), .O(new_n173_));
  nand3  g111(.a(x27), .b(x26), .c(x12), .O(new_n174_));
  nor3   g112(.a(new_n174_), .b(new_n173_), .c(x30), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(x22), .c(x01), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n63_), .O(z16));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(x23), .c(x22), .d(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n63_), .O(z17));
  nand2  g119(.a(new_n84_), .b(new_n66_), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n134_), .c(new_n137_), .O(new_n183_));
  nand2  g121(.a(new_n138_), .b(new_n133_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n183_), .c(new_n82_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n127_), .O(z18));
  inv1   g124(.a(new_n141_), .O(z19));
  inv1   g125(.a(new_n134_), .O(new_n188_));
  oai21  g126(.a(new_n68_), .b(new_n66_), .c(new_n137_), .O(new_n189_));
  aoi21  g127(.a(x28), .b(new_n66_), .c(new_n68_), .O(new_n190_));
  oai21  g128(.a(new_n190_), .b(new_n188_), .c(new_n189_), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(new_n82_), .c(new_n133_), .O(new_n192_));
  nand2  g130(.a(new_n192_), .b(new_n63_), .O(z20));
endmodule


