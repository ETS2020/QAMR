// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n197_;
  inv1   g000(.a(x30), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x08), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g008(.a(new_n70_), .b(x40), .c(x39), .d(new_n67_), .O(new_n71_));
  inv1   g009(.a(x37), .O(new_n72_));
  nand2  g010(.a(x35), .b(x28), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n71_), .c(new_n66_), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(x10), .c(new_n67_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n76_), .c(new_n65_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g020(.a(new_n64_), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n69_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n84_), .c(new_n63_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n83_), .c(x04), .O(z01));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  inv1   g031(.a(x08), .O(new_n94_));
  oai21  g032(.a(x27), .b(new_n67_), .c(new_n63_), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  aoi21  g034(.a(new_n86_), .b(x04), .c(new_n77_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n96_), .c(new_n93_), .O(z02));
  nand2  g036(.a(new_n73_), .b(x27), .O(new_n99_));
  inv1   g037(.a(x21), .O(new_n100_));
  aoi21  g038(.a(new_n72_), .b(new_n66_), .c(new_n100_), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n99_), .c(new_n64_), .O(z03));
  aoi21  g040(.a(new_n72_), .b(new_n66_), .c(x21), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n99_), .c(new_n64_), .O(z04));
  nor2   g042(.a(new_n85_), .b(new_n66_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n83_), .c(new_n72_), .O(z05));
  oai21  g045(.a(new_n105_), .b(x37), .c(new_n83_), .O(z06));
  inv1   g046(.a(x33), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n65_), .O(new_n110_));
  inv1   g048(.a(x00), .O(new_n111_));
  oai21  g049(.a(x25), .b(new_n111_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  inv1   g051(.a(x25), .O(new_n114_));
  nand3  g052(.a(x38), .b(new_n114_), .c(new_n111_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai22  g054(.a(new_n116_), .b(x14), .c(new_n113_), .d(x31), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x03), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n83_), .O(z07));
  nand2  g057(.a(new_n77_), .b(new_n83_), .O(z08));
  nand4  g058(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n83_), .O(z09));
  nand2  g060(.a(x05), .b(new_n67_), .O(new_n123_));
  oai21  g061(.a(new_n123_), .b(new_n77_), .c(x08), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x30), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  nand2  g064(.a(new_n77_), .b(x07), .O(new_n127_));
  oai21  g065(.a(new_n77_), .b(new_n126_), .c(new_n127_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n87_), .c(x27), .O(new_n129_));
  nand3  g067(.a(new_n78_), .b(x32), .c(x05), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n67_), .O(new_n132_));
  nand3  g070(.a(x37), .b(x27), .c(x06), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n125_), .O(z10));
  inv1   g072(.a(x09), .O(new_n135_));
  nand2  g073(.a(new_n78_), .b(x29), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x35), .c(new_n85_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n136_), .c(new_n63_), .O(new_n139_));
  nand2  g077(.a(x27), .b(new_n67_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n86_), .c(new_n94_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n135_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n83_), .O(z11));
  inv1   g082(.a(x10), .O(new_n145_));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n66_), .c(new_n145_), .O(new_n147_));
  nand2  g085(.a(new_n74_), .b(x27), .O(new_n148_));
  oai21  g086(.a(new_n77_), .b(x04), .c(new_n148_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n147_), .c(new_n83_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  inv1   g089(.a(x40), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n152_), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x39), .c(new_n151_), .d(new_n67_), .O(new_n155_));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  nand4  g095(.a(new_n74_), .b(x20), .c(new_n157_), .d(new_n156_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n155_), .c(x30), .O(new_n159_));
  nor2   g097(.a(new_n158_), .b(new_n94_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(x27), .O(new_n161_));
  nand2  g099(.a(x30), .b(x08), .O(new_n162_));
  nand2  g100(.a(x32), .b(new_n63_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n162_), .c(new_n152_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(x39), .c(new_n151_), .d(new_n67_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n161_), .O(z13));
  nand3  g104(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n167_));
  nand4  g105(.a(new_n153_), .b(new_n84_), .c(new_n63_), .d(x28), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  nand3  g107(.a(new_n78_), .b(new_n151_), .c(new_n67_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n169_), .c(new_n167_), .O(new_n171_));
  nand3  g109(.a(new_n72_), .b(new_n68_), .c(x28), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x27), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n84_), .c(new_n63_), .O(new_n174_));
  aoi21  g112(.a(new_n170_), .b(new_n148_), .c(new_n64_), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(new_n171_), .O(z14));
  inv1   g114(.a(x12), .O(new_n177_));
  nand4  g115(.a(new_n83_), .b(x34), .c(x27), .d(x26), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n177_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand4  g118(.a(new_n83_), .b(new_n180_), .c(x22), .d(x01), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z16));
  inv1   g120(.a(x01), .O(new_n183_));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n83_), .b(new_n184_), .c(x23), .d(x22), .O(new_n185_));
  nor2   g123(.a(new_n185_), .b(new_n183_), .O(z17));
  nand3  g124(.a(new_n63_), .b(x27), .c(new_n67_), .O(new_n187_));
  oai21  g125(.a(x27), .b(new_n94_), .c(new_n187_), .O(new_n188_));
  nand3  g126(.a(new_n188_), .b(x35), .c(new_n85_), .O(new_n189_));
  nand2  g127(.a(new_n136_), .b(new_n63_), .O(new_n190_));
  aoi22  g128(.a(new_n190_), .b(x08), .c(new_n63_), .d(x09), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n189_), .O(z18));
  oai21  g130(.a(new_n68_), .b(new_n66_), .c(new_n94_), .O(new_n193_));
  oai21  g131(.a(new_n85_), .b(x27), .c(x35), .O(new_n194_));
  nand3  g132(.a(new_n194_), .b(new_n136_), .c(new_n63_), .O(new_n195_));
  nand2  g133(.a(new_n195_), .b(new_n193_), .O(new_n196_));
  nand2  g134(.a(new_n196_), .b(new_n135_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n83_), .O(z20));
  nand2  g136(.a(new_n143_), .b(new_n83_), .O(z19));
endmodule


