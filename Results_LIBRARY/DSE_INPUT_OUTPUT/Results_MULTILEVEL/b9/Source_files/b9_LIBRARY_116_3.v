// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n205_, new_n206_, new_n207_,
    new_n208_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(x21), .O(new_n77_));
  nor2   g015(.a(new_n66_), .b(new_n77_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n76_), .c(x16), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nor2   g020(.a(new_n66_), .b(x28), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n79_), .c(x04), .O(z01));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(new_n83_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n79_), .d(new_n73_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(x27), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(x35), .c(x21), .O(z03));
  inv1   g034(.a(x28), .O(new_n97_));
  nor2   g035(.a(x37), .b(x27), .O(new_n98_));
  aoi21  g036(.a(new_n97_), .b(x27), .c(new_n98_), .O(new_n99_));
  oai21  g037(.a(x27), .b(x21), .c(new_n66_), .O(new_n100_));
  oai21  g038(.a(new_n99_), .b(x21), .c(new_n100_), .O(z04));
  nand2  g039(.a(x28), .b(x27), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n94_), .c(new_n78_), .O(z05));
  nand3  g041(.a(new_n102_), .b(new_n79_), .c(new_n94_), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(z06));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n63_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  oai21  g046(.a(x25), .b(new_n108_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand3  g049(.a(x38), .b(new_n111_), .c(new_n108_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(x14), .c(new_n110_), .d(x31), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n79_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  nand2  g054(.a(new_n79_), .b(new_n72_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n79_), .O(z09));
  nand2  g057(.a(new_n72_), .b(x07), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x05), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(x36), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n82_), .b(new_n81_), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(x40), .c(x39), .d(x05), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n123_), .c(x04), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n126_), .c(new_n79_), .O(new_n129_));
  nand4  g067(.a(new_n122_), .b(x35), .c(new_n97_), .d(x27), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n77_), .c(new_n65_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n129_), .O(z10));
  inv1   g071(.a(x09), .O(new_n134_));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n83_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  inv1   g076(.a(x08), .O(new_n139_));
  nand2  g077(.a(x27), .b(new_n65_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n90_), .c(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n81_), .c(new_n134_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n79_), .O(z11));
  inv1   g082(.a(x10), .O(new_n145_));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n64_), .c(new_n145_), .O(new_n147_));
  oai22  g085(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(new_n79_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  inv1   g088(.a(x40), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n151_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n150_), .d(new_n65_), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  inv1   g094(.a(x20), .O(new_n157_));
  nor2   g095(.a(new_n70_), .b(new_n157_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n156_), .c(new_n155_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n154_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(new_n124_), .b(x40), .c(x39), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(x13), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n65_), .c(new_n78_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z13));
  nand3  g103(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n166_));
  nand3  g104(.a(new_n73_), .b(new_n150_), .c(new_n65_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n79_), .O(new_n168_));
  oai21  g106(.a(x36), .b(x21), .c(x35), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n168_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n166_), .O(new_n172_));
  nand4  g110(.a(new_n124_), .b(new_n73_), .c(new_n150_), .d(new_n65_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n79_), .c(new_n64_), .O(new_n174_));
  oai21  g112(.a(x28), .b(x21), .c(x35), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n167_), .O(new_n176_));
  nand4  g114(.a(new_n66_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n94_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n174_), .c(new_n172_), .O(z14));
  nand4  g118(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n79_), .O(z15));
  inv1   g120(.a(x23), .O(new_n183_));
  nand4  g121(.a(new_n79_), .b(new_n183_), .c(x22), .d(x01), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(z16));
  inv1   g123(.a(x01), .O(new_n186_));
  inv1   g124(.a(x24), .O(new_n187_));
  nand4  g125(.a(new_n79_), .b(new_n187_), .c(x23), .d(x22), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n186_), .O(z17));
  nand3  g127(.a(new_n73_), .b(x29), .c(x08), .O(new_n190_));
  nand3  g128(.a(new_n190_), .b(new_n81_), .c(new_n134_), .O(new_n191_));
  nand2  g129(.a(new_n191_), .b(new_n79_), .O(new_n192_));
  oai21  g130(.a(x27), .b(new_n139_), .c(new_n140_), .O(new_n193_));
  nand4  g131(.a(new_n193_), .b(x35), .c(new_n97_), .d(new_n77_), .O(new_n194_));
  nand2  g132(.a(new_n194_), .b(new_n192_), .O(z18));
  inv1   g133(.a(new_n135_), .O(new_n196_));
  nand2  g134(.a(new_n136_), .b(new_n97_), .O(new_n197_));
  nand2  g135(.a(new_n197_), .b(new_n77_), .O(new_n198_));
  aoi21  g136(.a(new_n198_), .b(x35), .c(new_n196_), .O(new_n199_));
  nand3  g137(.a(new_n97_), .b(x27), .c(new_n65_), .O(new_n200_));
  nand2  g138(.a(new_n200_), .b(new_n77_), .O(new_n201_));
  aoi21  g139(.a(new_n201_), .b(x35), .c(x08), .O(new_n202_));
  oai21  g140(.a(new_n202_), .b(new_n199_), .c(new_n81_), .O(new_n203_));
  nor2   g141(.a(new_n203_), .b(x09), .O(z19));
  oai21  g142(.a(new_n66_), .b(new_n64_), .c(new_n139_), .O(new_n205_));
  aoi21  g143(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n206_));
  oai21  g144(.a(new_n206_), .b(new_n196_), .c(new_n205_), .O(new_n207_));
  nand3  g145(.a(new_n207_), .b(new_n81_), .c(new_n134_), .O(new_n208_));
  nand2  g146(.a(new_n208_), .b(new_n79_), .O(z20));
endmodule


