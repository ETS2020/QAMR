// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n209_, new_n210_, new_n211_, new_n212_, new_n213_;
  inv1   g000(.a(x15), .O(new_n63_));
  oai21  g001(.a(x36), .b(x35), .c(x39), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(x04), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n65_), .c(x27), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand3  g008(.a(x39), .b(x10), .c(new_n70_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(x40), .c(new_n63_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(x16), .O(z00));
  inv1   g012(.a(x30), .O(new_n75_));
  inv1   g013(.a(x32), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n76_), .c(new_n75_), .O(new_n82_));
  inv1   g020(.a(x40), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x16), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n82_), .c(x04), .O(z01));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n79_), .c(x04), .O(new_n90_));
  nand2  g028(.a(x40), .b(x39), .O(new_n91_));
  inv1   g029(.a(new_n91_), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(new_n88_), .O(z02));
  inv1   g031(.a(new_n84_), .O(new_n94_));
  nand2  g032(.a(new_n67_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x21), .O(new_n96_));
  inv1   g034(.a(x27), .O(new_n97_));
  aoi21  g035(.a(new_n66_), .b(new_n97_), .c(new_n96_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n95_), .c(new_n94_), .O(z03));
  aoi21  g037(.a(new_n66_), .b(new_n97_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n95_), .c(new_n94_), .O(z04));
  oai21  g039(.a(new_n78_), .b(new_n97_), .c(new_n66_), .O(new_n102_));
  and2   g040(.a(new_n102_), .b(new_n84_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x33), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n63_), .O(new_n106_));
  inv1   g044(.a(x00), .O(new_n107_));
  oai21  g045(.a(x25), .b(new_n107_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand3  g048(.a(x38), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(new_n112_));
  oai22  g050(.a(new_n112_), .b(x14), .c(new_n109_), .d(x31), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n84_), .O(z07));
  nand2  g053(.a(new_n91_), .b(new_n84_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n84_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  inv1   g058(.a(x39), .O(new_n121_));
  nor2   g059(.a(x40), .b(x16), .O(new_n122_));
  aoi21  g060(.a(x40), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  nand2  g061(.a(new_n92_), .b(x05), .O(new_n124_));
  oai21  g062(.a(new_n123_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n80_), .c(new_n70_), .O(new_n126_));
  nand3  g064(.a(new_n84_), .b(x37), .c(x06), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x27), .O(new_n129_));
  oai21  g067(.a(x32), .b(x30), .c(x40), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n121_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x05), .c(new_n70_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n129_), .O(z10));
  nand2  g071(.a(new_n84_), .b(new_n97_), .O(new_n134_));
  oai21  g072(.a(new_n79_), .b(x04), .c(x40), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n134_), .c(x08), .O(new_n136_));
  inv1   g074(.a(new_n79_), .O(new_n137_));
  nand2  g075(.a(x39), .b(x29), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x40), .c(new_n122_), .O(new_n139_));
  nand2  g077(.a(x27), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n137_), .c(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n136_), .c(new_n75_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(x09), .O(z11));
  inv1   g081(.a(x10), .O(new_n144_));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n97_), .c(new_n144_), .O(new_n146_));
  nand2  g084(.a(new_n68_), .b(x27), .O(new_n147_));
  oai21  g085(.a(new_n91_), .b(x04), .c(new_n147_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n146_), .c(new_n84_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x28), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x39), .c(new_n150_), .d(new_n70_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  nand4  g093(.a(new_n68_), .b(x20), .c(new_n155_), .d(new_n154_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n153_), .c(new_n83_), .O(new_n157_));
  nor2   g095(.a(new_n156_), .b(x16), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n157_), .c(x27), .O(new_n159_));
  nand3  g097(.a(new_n131_), .b(new_n150_), .c(new_n70_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(z13));
  nand3  g099(.a(x20), .b(new_n155_), .c(new_n154_), .O(new_n162_));
  nand4  g100(.a(new_n151_), .b(new_n76_), .c(new_n75_), .d(x28), .O(new_n163_));
  nand3  g101(.a(x39), .b(new_n150_), .c(new_n70_), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n163_), .c(new_n83_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n122_), .c(new_n162_), .O(new_n167_));
  inv1   g105(.a(x35), .O(new_n168_));
  nand3  g106(.a(new_n66_), .b(new_n168_), .c(x28), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x27), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n76_), .c(new_n75_), .O(new_n171_));
  nand2  g109(.a(new_n164_), .b(new_n147_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x40), .O(new_n174_));
  inv1   g112(.a(x16), .O(new_n175_));
  nand3  g113(.a(new_n147_), .b(new_n83_), .c(new_n175_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n174_), .c(new_n167_), .O(z14));
  inv1   g115(.a(x12), .O(new_n178_));
  nor2   g116(.a(new_n118_), .b(new_n178_), .O(z15));
  inv1   g117(.a(x23), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(x22), .c(x01), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n84_), .O(z16));
  inv1   g120(.a(x01), .O(new_n183_));
  inv1   g121(.a(x24), .O(new_n184_));
  nand4  g122(.a(new_n84_), .b(new_n184_), .c(x23), .d(x22), .O(new_n185_));
  nor2   g123(.a(new_n185_), .b(new_n183_), .O(z17));
  nand2  g124(.a(x27), .b(new_n70_), .O(new_n187_));
  nand2  g125(.a(new_n97_), .b(x08), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n187_), .c(new_n168_), .O(new_n189_));
  inv1   g127(.a(x09), .O(new_n190_));
  nand2  g128(.a(new_n75_), .b(new_n190_), .O(new_n191_));
  aoi21  g129(.a(new_n189_), .b(new_n78_), .c(new_n191_), .O(new_n192_));
  oai22  g130(.a(new_n192_), .b(new_n94_), .c(new_n91_), .d(new_n87_), .O(z18));
  nor2   g131(.a(x40), .b(x35), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n89_), .c(new_n175_), .O(new_n195_));
  inv1   g133(.a(x29), .O(new_n196_));
  nand2  g134(.a(new_n140_), .b(new_n78_), .O(new_n197_));
  oai21  g135(.a(new_n91_), .b(new_n196_), .c(new_n197_), .O(new_n198_));
  inv1   g136(.a(x08), .O(new_n199_));
  nand2  g137(.a(x35), .b(x27), .O(new_n200_));
  oai21  g138(.a(new_n200_), .b(x04), .c(new_n199_), .O(new_n201_));
  nand2  g139(.a(new_n138_), .b(new_n168_), .O(new_n202_));
  nand2  g140(.a(new_n202_), .b(new_n201_), .O(new_n203_));
  nand2  g141(.a(new_n203_), .b(x40), .O(new_n204_));
  nand2  g142(.a(x28), .b(new_n199_), .O(new_n205_));
  nand4  g143(.a(new_n205_), .b(new_n204_), .c(new_n198_), .d(new_n195_), .O(new_n206_));
  nand3  g144(.a(new_n206_), .b(new_n75_), .c(new_n190_), .O(new_n207_));
  nand2  g145(.a(new_n207_), .b(new_n84_), .O(z19));
  aoi21  g146(.a(x28), .b(new_n97_), .c(new_n168_), .O(new_n209_));
  nor2   g147(.a(new_n209_), .b(new_n139_), .O(new_n210_));
  nand2  g148(.a(x40), .b(new_n168_), .O(new_n211_));
  aoi21  g149(.a(new_n211_), .b(new_n134_), .c(x08), .O(new_n212_));
  oai21  g150(.a(new_n212_), .b(new_n210_), .c(new_n75_), .O(new_n213_));
  nor2   g151(.a(new_n213_), .b(x09), .O(z20));
endmodule


