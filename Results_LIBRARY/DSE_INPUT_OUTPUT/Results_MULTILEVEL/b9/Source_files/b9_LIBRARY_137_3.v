// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x10), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x35), .O(new_n67_));
  oai21  g005(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n64_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x40), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n75_), .c(x16), .O(z00));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n66_), .c(new_n64_), .O(new_n82_));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  nor2   g022(.a(new_n67_), .b(x28), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  nand4  g024(.a(new_n86_), .b(new_n76_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  oai21  g025(.a(new_n82_), .b(new_n78_), .c(new_n87_), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n86_), .c(x04), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n89_), .c(new_n76_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x40), .O(z02));
  nand2  g032(.a(new_n71_), .b(x27), .O(new_n95_));
  aoi21  g033(.a(new_n70_), .b(new_n66_), .c(new_n78_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n95_), .c(new_n98_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n79_), .c(new_n70_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n63_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  oai22  g048(.a(new_n110_), .b(x14), .c(new_n107_), .d(x31), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n79_), .c(x03), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z07));
  oai21  g051(.a(x39), .b(x36), .c(x40), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n79_), .O(z09));
  nand3  g055(.a(new_n79_), .b(x37), .c(x06), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  inv1   g057(.a(x28), .O(new_n120_));
  nand2  g058(.a(new_n114_), .b(x07), .O(new_n121_));
  nand4  g059(.a(x40), .b(x39), .c(new_n76_), .d(x05), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(x35), .c(new_n120_), .O(new_n124_));
  nand3  g062(.a(new_n77_), .b(x36), .c(x07), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(x04), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n119_), .c(x27), .O(new_n127_));
  inv1   g065(.a(x39), .O(new_n128_));
  nor3   g066(.a(new_n81_), .b(new_n77_), .c(new_n128_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(new_n76_), .c(x05), .d(new_n64_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  inv1   g070(.a(x09), .O(new_n133_));
  nand4  g071(.a(new_n83_), .b(new_n133_), .c(new_n132_), .d(x04), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n76_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x40), .O(new_n136_));
  nor2   g074(.a(new_n77_), .b(new_n128_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x29), .O(new_n138_));
  oai21  g076(.a(new_n66_), .b(new_n64_), .c(new_n85_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n86_), .b(new_n66_), .c(new_n132_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n83_), .c(new_n133_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n136_), .O(z11));
  nor2   g082(.a(x37), .b(x35), .O(new_n145_));
  nand3  g083(.a(new_n137_), .b(x10), .c(new_n64_), .O(new_n146_));
  oai21  g084(.a(new_n145_), .b(new_n66_), .c(new_n146_), .O(new_n147_));
  nand2  g085(.a(new_n137_), .b(new_n64_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n70_), .c(new_n120_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n147_), .c(new_n79_), .O(z12));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand4  g090(.a(new_n72_), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n153_));
  nor2   g091(.a(x13), .b(x04), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n137_), .c(new_n120_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nor3   g095(.a(new_n81_), .b(new_n128_), .c(x13), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n64_), .c(x36), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n77_), .c(new_n157_), .O(z13));
  inv1   g098(.a(new_n145_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x20), .c(new_n152_), .d(new_n151_), .O(new_n162_));
  nand2  g100(.a(new_n81_), .b(x28), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n154_), .c(new_n137_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  oai21  g103(.a(x37), .b(x28), .c(x27), .O(new_n166_));
  nand2  g104(.a(new_n154_), .b(new_n137_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  aoi21  g106(.a(new_n81_), .b(new_n66_), .c(new_n78_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n168_), .c(new_n165_), .O(z14));
  nand4  g108(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n79_), .O(z15));
  inv1   g110(.a(x23), .O(new_n173_));
  nand4  g111(.a(new_n79_), .b(new_n173_), .c(x22), .d(x01), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(z16));
  inv1   g113(.a(x01), .O(new_n176_));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n79_), .b(new_n177_), .c(x23), .d(x22), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n176_), .O(z17));
  oai21  g117(.a(new_n86_), .b(x27), .c(new_n138_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x08), .O(new_n181_));
  nor2   g119(.a(new_n66_), .b(x04), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n85_), .c(x09), .O(new_n183_));
  nand4  g121(.a(new_n183_), .b(new_n181_), .c(new_n79_), .d(new_n83_), .O(z18));
  nor2   g122(.a(new_n78_), .b(x27), .O(new_n185_));
  nand2  g123(.a(new_n85_), .b(new_n64_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n76_), .c(new_n185_), .O(new_n187_));
  nand2  g125(.a(x39), .b(x29), .O(new_n188_));
  aoi21  g126(.a(new_n188_), .b(new_n76_), .c(new_n77_), .O(new_n189_));
  inv1   g127(.a(new_n189_), .O(new_n190_));
  nand2  g128(.a(new_n190_), .b(new_n139_), .O(new_n191_));
  oai21  g129(.a(new_n187_), .b(x08), .c(new_n191_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n83_), .c(new_n133_), .O(new_n193_));
  inv1   g131(.a(new_n193_), .O(z19));
  aoi21  g132(.a(x28), .b(new_n66_), .c(new_n67_), .O(new_n195_));
  nor2   g133(.a(x36), .b(x35), .O(new_n196_));
  oai21  g134(.a(new_n196_), .b(new_n185_), .c(new_n132_), .O(new_n197_));
  oai21  g135(.a(new_n195_), .b(new_n189_), .c(new_n197_), .O(new_n198_));
  nand3  g136(.a(new_n198_), .b(new_n83_), .c(new_n133_), .O(new_n199_));
  inv1   g137(.a(new_n199_), .O(z20));
endmodule


