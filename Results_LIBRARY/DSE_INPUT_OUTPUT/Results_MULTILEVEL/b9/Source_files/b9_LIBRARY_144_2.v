// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n184_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_;
  inv1   g000(.a(x30), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x04), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n67_), .O(new_n73_));
  inv1   g011(.a(x04), .O(z01));
  inv1   g012(.a(x10), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  oai21  g014(.a(x36), .b(x35), .c(x27), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n76_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(x39), .c(new_n66_), .d(z01), .O(new_n79_));
  oai21  g017(.a(new_n73_), .b(new_n65_), .c(new_n79_), .O(z00));
  and2   g018(.a(x29), .b(x08), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  inv1   g020(.a(x08), .O(new_n83_));
  inv1   g021(.a(x27), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand4  g023(.a(new_n85_), .b(x35), .c(x30), .d(new_n82_), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  aoi21  g025(.a(new_n86_), .b(x04), .c(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n81_), .b(x02), .c(new_n88_), .O(z02));
  nand2  g027(.a(new_n69_), .b(x27), .O(new_n90_));
  inv1   g028(.a(x21), .O(new_n91_));
  aoi21  g029(.a(new_n68_), .b(new_n84_), .c(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n90_), .c(new_n65_), .O(z03));
  nand2  g031(.a(new_n68_), .b(new_n84_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n90_), .c(new_n64_), .d(new_n91_), .O(z04));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n64_), .c(new_n68_), .O(z05));
  inv1   g035(.a(z05), .O(z06));
  inv1   g036(.a(x33), .O(new_n99_));
  nand2  g037(.a(x17), .b(new_n66_), .O(new_n100_));
  inv1   g038(.a(x00), .O(new_n101_));
  oai21  g039(.a(x25), .b(new_n101_), .c(x38), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand3  g042(.a(x38), .b(new_n104_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(new_n106_));
  oai22  g044(.a(new_n106_), .b(x14), .c(new_n103_), .d(x31), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x03), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n64_), .O(z07));
  nand2  g047(.a(new_n87_), .b(new_n64_), .O(z08));
  inv1   g048(.a(x11), .O(new_n111_));
  nand4  g049(.a(new_n64_), .b(x34), .c(x27), .d(x26), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n111_), .O(z09));
  inv1   g051(.a(x36), .O(new_n114_));
  nand2  g052(.a(x35), .b(new_n82_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g054(.a(new_n87_), .b(x07), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n120_), .c(x04), .O(new_n122_));
  nand3  g060(.a(x37), .b(x30), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  oai21  g062(.a(new_n124_), .b(new_n122_), .c(x27), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  inv1   g064(.a(x32), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n63_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(x40), .c(x39), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(z01), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n125_), .O(z10));
  inv1   g070(.a(x09), .O(new_n133_));
  inv1   g071(.a(new_n87_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n81_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n115_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n85_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(z01), .c(x30), .O(z11));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n84_), .c(new_n75_), .O(new_n141_));
  oai21  g079(.a(new_n87_), .b(x04), .c(new_n71_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n64_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x28), .c(new_n76_), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(x39), .c(new_n144_), .d(z01), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand4  g087(.a(new_n70_), .b(x20), .c(new_n149_), .d(new_n148_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x27), .O(new_n152_));
  nor2   g090(.a(new_n129_), .b(x13), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(z01), .c(new_n65_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(z13));
  nand3  g093(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n156_));
  nand4  g094(.a(new_n145_), .b(new_n127_), .c(new_n63_), .d(x28), .O(new_n157_));
  nor2   g095(.a(new_n87_), .b(x13), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n157_), .c(x04), .O(new_n159_));
  nor2   g097(.a(new_n63_), .b(z01), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n156_), .O(new_n161_));
  inv1   g099(.a(x35), .O(new_n162_));
  nand3  g100(.a(new_n68_), .b(new_n162_), .c(x28), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n127_), .c(new_n63_), .O(new_n165_));
  oai21  g103(.a(new_n158_), .b(new_n72_), .c(new_n165_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(z01), .O(new_n167_));
  nand3  g105(.a(new_n71_), .b(x30), .c(x04), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n167_), .c(new_n161_), .O(z14));
  nand4  g107(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n64_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand4  g110(.a(new_n64_), .b(new_n172_), .c(x22), .d(x01), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z16));
  inv1   g112(.a(x01), .O(new_n175_));
  inv1   g113(.a(x24), .O(new_n176_));
  nand4  g114(.a(new_n64_), .b(new_n176_), .c(x23), .d(x22), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z17));
  nand2  g116(.a(new_n134_), .b(x29), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n115_), .c(new_n83_), .O(new_n180_));
  oai21  g118(.a(new_n115_), .b(new_n84_), .c(new_n133_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n180_), .c(z01), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n63_), .O(z18));
  nand4  g121(.a(new_n137_), .b(new_n63_), .c(new_n133_), .d(z01), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(z19));
  inv1   g123(.a(new_n179_), .O(new_n186_));
  oai21  g124(.a(new_n162_), .b(new_n84_), .c(new_n83_), .O(new_n187_));
  aoi21  g125(.a(x28), .b(new_n84_), .c(new_n162_), .O(new_n188_));
  oai21  g126(.a(new_n188_), .b(new_n186_), .c(new_n187_), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n133_), .O(new_n190_));
  aoi21  g128(.a(new_n190_), .b(z01), .c(x30), .O(z20));
endmodule


