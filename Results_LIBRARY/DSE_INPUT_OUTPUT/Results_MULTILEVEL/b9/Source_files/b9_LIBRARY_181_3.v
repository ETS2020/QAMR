// Benchmark "FAU" written by ABC on Thu Aug 13 18:18:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n198_, new_n199_, new_n200_,
    new_n201_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x28), .O(new_n70_));
  nor2   g008(.a(new_n66_), .b(new_n70_), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x37), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n69_), .c(new_n64_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(x10), .c(new_n65_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n73_), .c(new_n63_), .O(new_n78_));
  nand2  g016(.a(x40), .b(x37), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n78_), .c(x16), .O(z00));
  inv1   g018(.a(new_n79_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n70_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n83_), .c(new_n82_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n81_), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n84_), .c(x04), .O(new_n93_));
  nor2   g031(.a(new_n74_), .b(x37), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n93_), .c(new_n91_), .O(z02));
  inv1   g033(.a(x21), .O(new_n96_));
  nand2  g034(.a(new_n79_), .b(new_n96_), .O(new_n97_));
  oai21  g035(.a(x40), .b(new_n64_), .c(x37), .O(new_n98_));
  oai21  g036(.a(new_n66_), .b(new_n70_), .c(new_n98_), .O(new_n99_));
  inv1   g037(.a(x37), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n64_), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(z03));
  inv1   g040(.a(x40), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x37), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n64_), .c(new_n97_), .O(new_n105_));
  oai21  g043(.a(new_n71_), .b(new_n64_), .c(new_n105_), .O(z04));
  nand3  g044(.a(new_n100_), .b(x28), .c(x27), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n104_), .O(z05));
  aoi21  g046(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(x17), .b(new_n63_), .O(new_n111_));
  inv1   g049(.a(x00), .O(new_n112_));
  oai21  g050(.a(x25), .b(new_n112_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  inv1   g052(.a(x25), .O(new_n115_));
  nand3  g053(.a(x38), .b(new_n115_), .c(new_n112_), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  oai22  g055(.a(new_n117_), .b(x14), .c(new_n114_), .d(x31), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n79_), .c(x03), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z07));
  nand3  g058(.a(x40), .b(x39), .c(new_n100_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(z08));
  nand4  g060(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n79_), .O(z09));
  nand2  g062(.a(x27), .b(x06), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n103_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x37), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n74_), .b(x07), .O(new_n129_));
  oai21  g067(.a(new_n74_), .b(new_n128_), .c(new_n129_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n85_), .c(x27), .O(new_n131_));
  inv1   g069(.a(x39), .O(new_n132_));
  oai21  g070(.a(x32), .b(x30), .c(x40), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x05), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n65_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n127_), .O(z10));
  inv1   g076(.a(x09), .O(new_n139_));
  aoi21  g077(.a(x40), .b(x37), .c(x27), .O(new_n140_));
  nand3  g078(.a(x35), .b(new_n70_), .c(new_n65_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n100_), .c(new_n140_), .O(new_n142_));
  nand2  g080(.a(x39), .b(x29), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n100_), .c(new_n103_), .O(new_n144_));
  nand2  g082(.a(x27), .b(x04), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(x35), .c(new_n70_), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(new_n147_));
  oai22  g085(.a(new_n147_), .b(new_n144_), .c(new_n142_), .d(x08), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n82_), .c(new_n139_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(z11));
  inv1   g088(.a(x10), .O(new_n151_));
  nor3   g089(.a(x37), .b(x36), .c(x35), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n64_), .c(new_n151_), .O(new_n153_));
  oai21  g091(.a(new_n71_), .b(x37), .c(x27), .O(new_n154_));
  oai21  g092(.a(new_n74_), .b(x04), .c(new_n154_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n153_), .c(new_n98_), .O(z12));
  inv1   g094(.a(x13), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x28), .c(new_n103_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x39), .c(new_n157_), .d(new_n65_), .O(new_n160_));
  inv1   g098(.a(x18), .O(new_n161_));
  inv1   g099(.a(x19), .O(new_n162_));
  nand4  g100(.a(new_n71_), .b(x20), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n160_), .c(x37), .O(new_n164_));
  inv1   g102(.a(x20), .O(new_n165_));
  nand2  g103(.a(new_n162_), .b(new_n161_), .O(new_n166_));
  nor3   g104(.a(new_n166_), .b(new_n104_), .c(new_n165_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n164_), .c(x27), .O(new_n168_));
  nand4  g106(.a(new_n134_), .b(new_n100_), .c(new_n157_), .d(new_n65_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n168_), .O(z13));
  nand3  g108(.a(x20), .b(new_n162_), .c(new_n161_), .O(new_n171_));
  nand4  g109(.a(new_n67_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(new_n94_), .c(new_n157_), .d(new_n65_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  nor2   g112(.a(x37), .b(x35), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x28), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n83_), .c(new_n82_), .O(new_n178_));
  nand3  g116(.a(new_n75_), .b(new_n157_), .c(new_n65_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n154_), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n178_), .c(new_n174_), .d(new_n98_), .O(z14));
  inv1   g119(.a(x12), .O(new_n182_));
  nand4  g120(.a(new_n79_), .b(x34), .c(x27), .d(x26), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n182_), .O(z15));
  inv1   g122(.a(x23), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(x22), .c(x01), .O(new_n186_));
  nand2  g124(.a(new_n186_), .b(new_n79_), .O(z16));
  inv1   g125(.a(x24), .O(new_n188_));
  nand4  g126(.a(new_n188_), .b(x23), .c(x22), .d(x01), .O(new_n189_));
  nand2  g127(.a(new_n189_), .b(new_n79_), .O(z17));
  inv1   g128(.a(new_n94_), .O(new_n191_));
  nand2  g129(.a(x27), .b(new_n65_), .O(new_n192_));
  nand2  g130(.a(new_n64_), .b(x08), .O(new_n193_));
  aoi21  g131(.a(new_n193_), .b(new_n192_), .c(new_n66_), .O(new_n194_));
  nand2  g132(.a(new_n82_), .b(new_n139_), .O(new_n195_));
  aoi21  g133(.a(new_n194_), .b(new_n70_), .c(new_n195_), .O(new_n196_));
  oai22  g134(.a(new_n196_), .b(new_n81_), .c(new_n191_), .d(new_n90_), .O(z18));
  aoi21  g135(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n198_));
  nor2   g136(.a(new_n175_), .b(new_n140_), .O(new_n199_));
  oai22  g137(.a(new_n199_), .b(x08), .c(new_n198_), .d(new_n144_), .O(new_n200_));
  nand3  g138(.a(new_n200_), .b(new_n82_), .c(new_n139_), .O(new_n201_));
  inv1   g139(.a(new_n201_), .O(z20));
  inv1   g140(.a(new_n149_), .O(z19));
endmodule


