// Benchmark "FAU" written by ABC on Thu Aug 13 18:17:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_;
  nand2  g000(.a(x40), .b(x30), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x30), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(x39), .c(new_n72_), .d(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(x04), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x35), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(x28), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x36), .c(x27), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n79_), .c(new_n72_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n63_), .c(x04), .O(z01));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n81_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x04), .O(new_n89_));
  and2   g027(.a(x40), .b(x39), .O(new_n90_));
  nand4  g028(.a(new_n90_), .b(new_n89_), .c(new_n87_), .d(new_n72_), .O(z02));
  nand2  g029(.a(new_n69_), .b(x27), .O(new_n92_));
  nand2  g030(.a(new_n68_), .b(new_n67_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n92_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g032(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n92_), .c(new_n64_), .O(z04));
  inv1   g034(.a(x28), .O(new_n97_));
  nor2   g035(.a(new_n97_), .b(new_n67_), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n63_), .c(new_n68_), .O(z05));
  oai21  g038(.a(new_n98_), .b(x37), .c(new_n63_), .O(z06));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n65_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  oai22  g047(.a(new_n109_), .b(x14), .c(new_n106_), .d(x31), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n63_), .O(z07));
  oai21  g050(.a(x39), .b(x30), .c(x40), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n63_), .O(z09));
  inv1   g054(.a(x04), .O(new_n117_));
  aoi21  g055(.a(x35), .b(new_n97_), .c(x36), .O(new_n118_));
  nand2  g056(.a(new_n113_), .b(x07), .O(new_n119_));
  nand3  g057(.a(new_n90_), .b(new_n72_), .c(x05), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nor2   g059(.a(new_n64_), .b(new_n68_), .O(new_n122_));
  aoi22  g060(.a(new_n122_), .b(x06), .c(new_n121_), .d(new_n117_), .O(new_n123_));
  nand2  g061(.a(new_n90_), .b(x32), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nand4  g063(.a(new_n125_), .b(new_n72_), .c(x05), .d(new_n117_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(new_n67_), .c(new_n126_), .O(z10));
  inv1   g065(.a(x09), .O(new_n128_));
  nand2  g066(.a(new_n90_), .b(x29), .O(new_n129_));
  oai21  g067(.a(new_n67_), .b(new_n117_), .c(new_n81_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(new_n81_), .b(x27), .c(new_n117_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n72_), .c(new_n128_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n63_), .O(z11));
  inv1   g075(.a(x36), .O(new_n138_));
  nand3  g076(.a(new_n68_), .b(new_n138_), .c(new_n80_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x27), .c(x10), .O(new_n140_));
  aoi21  g078(.a(x39), .b(new_n117_), .c(new_n70_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n140_), .c(new_n72_), .O(new_n142_));
  oai21  g080(.a(new_n70_), .b(x40), .c(new_n142_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x28), .c(new_n74_), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(x39), .c(new_n144_), .d(new_n117_), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand2  g087(.a(new_n69_), .b(new_n68_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x20), .c(new_n149_), .d(new_n148_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n147_), .c(x30), .O(new_n152_));
  nand4  g090(.a(new_n150_), .b(new_n74_), .c(x20), .d(new_n149_), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(x18), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n152_), .c(x27), .O(new_n155_));
  nand4  g093(.a(new_n125_), .b(new_n72_), .c(new_n144_), .d(new_n117_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n155_), .O(z13));
  nand3  g095(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n158_));
  nand3  g096(.a(new_n145_), .b(new_n79_), .c(x28), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  nand3  g098(.a(new_n90_), .b(new_n144_), .c(new_n117_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n160_), .c(new_n158_), .O(new_n162_));
  nand3  g100(.a(new_n68_), .b(new_n80_), .c(x28), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n79_), .O(new_n165_));
  inv1   g103(.a(new_n70_), .O(new_n166_));
  nand2  g104(.a(new_n161_), .b(new_n166_), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(new_n165_), .c(new_n162_), .d(new_n63_), .O(z14));
  inv1   g106(.a(x12), .O(new_n169_));
  nand4  g107(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(new_n169_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(x22), .c(x01), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n63_), .O(z16));
  inv1   g112(.a(x01), .O(new_n175_));
  inv1   g113(.a(x24), .O(new_n176_));
  nand4  g114(.a(new_n63_), .b(new_n176_), .c(x23), .d(x22), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z17));
  nand2  g116(.a(new_n81_), .b(new_n67_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n129_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x08), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(new_n133_), .c(new_n72_), .d(new_n128_), .O(z18));
  inv1   g120(.a(new_n129_), .O(new_n183_));
  oai21  g121(.a(new_n80_), .b(new_n67_), .c(new_n132_), .O(new_n184_));
  aoi21  g122(.a(x28), .b(new_n67_), .c(new_n80_), .O(new_n185_));
  oai21  g123(.a(new_n185_), .b(new_n183_), .c(new_n184_), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n72_), .c(new_n128_), .O(new_n187_));
  inv1   g125(.a(new_n187_), .O(z20));
  nand2  g126(.a(new_n136_), .b(new_n63_), .O(z19));
endmodule


