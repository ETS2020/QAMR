// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  nand3  g10(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x14), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(z1));
  xor2a  g15(.a(x12), .b(x11), .O(new_n44_));
  nand4  g16(.a(new_n44_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n45_));
  nor2   g17(.a(x14), .b(x11), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n46_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n45_), .O(z2));
  xnor2a g20(.a(x13), .b(x12), .O(new_n49_));
  nand2  g21(.a(x13), .b(new_n38_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n38_), .c(new_n50_), .O(new_n51_));
  nand4  g23(.a(new_n51_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n46_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z3));
  inv1   g26(.a(x14), .O(new_n55_));
  and2   g27(.a(x13), .b(x12), .O(new_n56_));
  nand3  g28(.a(new_n55_), .b(x13), .c(x12), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n59_));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n41_), .O(z4));
  nor2   g33(.a(new_n46_), .b(new_n30_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x04), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(x14), .c(x13), .d(x12), .O(new_n65_));
  oai21  g37(.a(new_n64_), .b(x14), .c(new_n65_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x11), .O(new_n67_));
  nand3  g39(.a(x13), .b(x12), .c(x11), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(x15), .c(x14), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n63_), .O(z5));
  nand4  g44(.a(x16), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x14), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n38_), .O(new_n75_));
  nand3  g47(.a(new_n56_), .b(x15), .c(x14), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g49(.a(x16), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(x15), .c(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n68_), .c(new_n77_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n81_));
  nand2  g53(.a(x08), .b(x05), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n81_), .c(new_n75_), .O(z6));
  nand2  g55(.a(new_n62_), .b(x06), .O(new_n84_));
  nand2  g56(.a(x17), .b(new_n38_), .O(new_n85_));
  inv1   g57(.a(x17), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(x16), .c(x15), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n68_), .c(new_n85_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(x14), .O(new_n89_));
  nand4  g61(.a(new_n56_), .b(x16), .c(x15), .d(x14), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(x17), .c(x11), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand4  g64(.a(new_n92_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(new_n84_), .O(z7));
  nand2  g66(.a(new_n62_), .b(x07), .O(new_n95_));
  nand2  g67(.a(x18), .b(new_n38_), .O(new_n96_));
  nand4  g68(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n68_), .c(new_n96_), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(x14), .O(new_n99_));
  nand3  g71(.a(x14), .b(x13), .c(x12), .O(new_n100_));
  inv1   g72(.a(new_n100_), .O(new_n101_));
  nand4  g73(.a(new_n101_), .b(x17), .c(x16), .d(x15), .O(new_n102_));
  nand3  g74(.a(new_n102_), .b(x18), .c(x11), .O(new_n103_));
  nand2  g75(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  nand4  g76(.a(new_n104_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n105_));
  nand2  g77(.a(new_n105_), .b(new_n95_), .O(z8));
endmodule


