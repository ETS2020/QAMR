// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x13), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  nand2  g04(.a(x12), .b(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  and2   g06(.a(x15), .b(x14), .O(new_n35_));
  nand3  g07(.a(x18), .b(x17), .c(x16), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n35_), .c(new_n34_), .d(new_n32_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g11(.a(x13), .b(x10), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(new_n32_), .b(new_n42_), .c(new_n29_), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  inv1   g16(.a(x08), .O(new_n45_));
  aoi21  g17(.a(x13), .b(x10), .c(new_n45_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x01), .O(new_n47_));
  xor2a  g19(.a(x12), .b(x11), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n29_), .c(x09), .d(new_n45_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n47_), .O(z2));
  nand2  g22(.a(new_n46_), .b(x02), .O(new_n51_));
  xnor2a g23(.a(x13), .b(x12), .O(new_n52_));
  nand2  g24(.a(x13), .b(new_n42_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n42_), .c(new_n53_), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n29_), .c(x09), .d(new_n45_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n51_), .O(z3));
  inv1   g28(.a(new_n32_), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(x12), .c(x11), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n57_), .c(new_n29_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x13), .O(new_n61_));
  nand3  g33(.a(x13), .b(x12), .c(x11), .O(new_n62_));
  inv1   g34(.a(new_n62_), .O(new_n63_));
  nor2   g35(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n29_), .c(x09), .d(new_n45_), .O(new_n65_));
  nand2  g37(.a(x08), .b(x03), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n65_), .c(new_n61_), .O(z4));
  nand3  g39(.a(x11), .b(x09), .c(new_n45_), .O(new_n68_));
  inv1   g40(.a(x15), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(x14), .c(x12), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n68_), .c(new_n29_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x13), .O(new_n72_));
  nand4  g44(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(x15), .c(new_n29_), .O(new_n74_));
  inv1   g46(.a(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(x09), .c(new_n45_), .O(new_n76_));
  nand2  g48(.a(x08), .b(x04), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(z5));
  nand2  g50(.a(new_n46_), .b(x05), .O(new_n79_));
  nand3  g51(.a(x15), .b(x14), .c(x13), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n33_), .c(x16), .O(new_n81_));
  inv1   g53(.a(x16), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(x15), .c(x14), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n62_), .c(new_n81_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n29_), .c(x09), .d(new_n45_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n79_), .O(z6));
  nand2  g58(.a(new_n34_), .b(new_n32_), .O(new_n87_));
  inv1   g59(.a(x17), .O(new_n88_));
  nand3  g60(.a(new_n35_), .b(new_n88_), .c(x16), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n87_), .c(new_n29_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(x13), .O(new_n91_));
  nand3  g63(.a(x16), .b(x15), .c(x14), .O(new_n92_));
  inv1   g64(.a(new_n92_), .O(new_n93_));
  aoi21  g65(.a(new_n93_), .b(new_n63_), .c(new_n88_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n29_), .c(x09), .d(new_n45_), .O(new_n95_));
  nand2  g67(.a(x08), .b(x06), .O(new_n96_));
  nand3  g68(.a(new_n96_), .b(new_n95_), .c(new_n91_), .O(z7));
  inv1   g69(.a(x18), .O(new_n98_));
  nand4  g70(.a(new_n35_), .b(new_n98_), .c(x17), .d(x16), .O(new_n99_));
  oai21  g71(.a(new_n99_), .b(new_n87_), .c(new_n29_), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(x13), .O(new_n101_));
  nand4  g73(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n102_));
  inv1   g74(.a(new_n102_), .O(new_n103_));
  aoi21  g75(.a(new_n103_), .b(new_n63_), .c(new_n98_), .O(new_n104_));
  nand4  g76(.a(new_n104_), .b(new_n29_), .c(x09), .d(new_n45_), .O(new_n105_));
  nand2  g77(.a(x08), .b(x07), .O(new_n106_));
  nand3  g78(.a(new_n106_), .b(new_n105_), .c(new_n101_), .O(z8));
endmodule


