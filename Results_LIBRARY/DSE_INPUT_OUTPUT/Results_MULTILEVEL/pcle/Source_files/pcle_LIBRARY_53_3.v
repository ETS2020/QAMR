// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x14), .O(new_n29_));
  inv1   g01(.a(x16), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  nand2  g03(.a(x08), .b(x00), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  nor2   g06(.a(x11), .b(x10), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n32_), .c(z0), .O(z1));
  inv1   g09(.a(z0), .O(new_n38_));
  inv1   g10(.a(x08), .O(new_n39_));
  inv1   g11(.a(x10), .O(new_n40_));
  xor2a  g12(.a(x12), .b(x11), .O(new_n41_));
  nand4  g13(.a(new_n41_), .b(new_n40_), .c(x09), .d(new_n39_), .O(new_n42_));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(z2));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x13), .O(new_n46_));
  inv1   g18(.a(x13), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(x12), .c(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n40_), .c(x09), .d(new_n39_), .O(new_n50_));
  nand2  g22(.a(x08), .b(x02), .O(new_n51_));
  aoi21  g23(.a(new_n51_), .b(new_n50_), .c(z0), .O(z3));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x14), .O(new_n54_));
  nand2  g26(.a(new_n29_), .b(x13), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n45_), .c(new_n54_), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(new_n40_), .c(x09), .d(new_n39_), .O(new_n57_));
  aoi21  g29(.a(x08), .b(x03), .c(z0), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n57_), .O(z4));
  nand3  g31(.a(new_n38_), .b(x08), .c(x04), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  aoi21  g33(.a(new_n53_), .b(new_n30_), .c(new_n29_), .O(new_n62_));
  nand3  g34(.a(new_n30_), .b(new_n61_), .c(x14), .O(new_n63_));
  oai22  g35(.a(new_n63_), .b(new_n53_), .c(new_n62_), .d(new_n61_), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n40_), .c(x09), .d(new_n39_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n60_), .O(z5));
  nand3  g38(.a(x15), .b(x14), .c(x13), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n45_), .c(new_n30_), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n40_), .c(x09), .d(new_n39_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(z0), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z6));
  nand2  g43(.a(x08), .b(x06), .O(new_n72_));
  nand3  g44(.a(new_n34_), .b(x17), .c(new_n40_), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n72_), .c(z0), .O(z7));
  nand2  g46(.a(x08), .b(x07), .O(new_n75_));
  nand3  g47(.a(new_n34_), .b(x18), .c(new_n40_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n75_), .c(z0), .O(z8));
endmodule


