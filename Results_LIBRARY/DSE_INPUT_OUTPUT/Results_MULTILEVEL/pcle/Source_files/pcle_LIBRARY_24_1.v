// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x17), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x00), .O(new_n31_));
  oai21  g03(.a(new_n29_), .b(x10), .c(x08), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nor2   g06(.a(x17), .b(x11), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n34_), .c(x09), .d(new_n33_), .O(new_n36_));
  oai21  g08(.a(new_n32_), .b(new_n31_), .c(new_n36_), .O(z1));
  inv1   g09(.a(x01), .O(new_n38_));
  xor2a  g10(.a(x12), .b(x11), .O(new_n39_));
  nand4  g11(.a(new_n39_), .b(new_n29_), .c(new_n34_), .d(x09), .O(new_n40_));
  oai22  g12(.a(new_n40_), .b(x08), .c(new_n32_), .d(new_n38_), .O(z2));
  inv1   g13(.a(x09), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(x13), .O(new_n44_));
  inv1   g16(.a(x13), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(x12), .c(x11), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n44_), .c(new_n42_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n33_), .c(x17), .O(new_n48_));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  oai21  g21(.a(new_n48_), .b(x10), .c(new_n49_), .O(z3));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(x14), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  inv1   g25(.a(new_n43_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n53_), .c(x13), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n52_), .c(new_n42_), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n33_), .c(x17), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  oai21  g30(.a(new_n57_), .b(x10), .c(new_n58_), .O(z4));
  nand2  g31(.a(x14), .b(x13), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n43_), .c(x15), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand4  g34(.a(new_n54_), .b(new_n62_), .c(x14), .d(x13), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n61_), .c(new_n42_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n33_), .c(x17), .O(new_n65_));
  nand2  g37(.a(x08), .b(x04), .O(new_n66_));
  oai21  g38(.a(new_n65_), .b(x10), .c(new_n66_), .O(z5));
  nand3  g39(.a(x15), .b(x14), .c(x13), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n43_), .c(x16), .O(new_n69_));
  inv1   g41(.a(new_n51_), .O(new_n70_));
  nor2   g42(.a(x16), .b(new_n62_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n70_), .c(x14), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n69_), .c(new_n42_), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n33_), .c(x17), .O(new_n74_));
  nand2  g46(.a(x08), .b(x05), .O(new_n75_));
  oai21  g47(.a(new_n74_), .b(x10), .c(new_n75_), .O(z6));
  inv1   g48(.a(x06), .O(new_n77_));
  nand4  g49(.a(new_n54_), .b(new_n34_), .c(x09), .d(new_n33_), .O(new_n78_));
  inv1   g50(.a(new_n60_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n29_), .c(x16), .d(x15), .O(new_n80_));
  oai22  g52(.a(new_n80_), .b(new_n78_), .c(new_n32_), .d(new_n77_), .O(z7));
  nand2  g53(.a(x18), .b(x09), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(x08), .c(new_n29_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n34_), .O(new_n84_));
  nand2  g56(.a(x08), .b(x07), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


