// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  nand2  g09(.a(x09), .b(new_n30_), .O(new_n38_));
  inv1   g10(.a(x11), .O(new_n39_));
  inv1   g11(.a(x14), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(new_n39_), .c(new_n30_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  nand3  g14(.a(x14), .b(new_n39_), .c(new_n31_), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n38_), .c(new_n42_), .O(z1));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n46_));
  aoi22  g18(.a(new_n40_), .b(new_n39_), .c(x08), .d(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  nand2  g20(.a(new_n41_), .b(x02), .O(new_n49_));
  xnor2a g21(.a(x13), .b(x12), .O(new_n50_));
  nand3  g22(.a(x14), .b(x13), .c(new_n39_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n39_), .c(new_n51_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n49_), .O(z3));
  nand2  g26(.a(new_n41_), .b(x03), .O(new_n55_));
  nand3  g27(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x14), .O(new_n57_));
  nand2  g29(.a(x12), .b(x11), .O(new_n58_));
  nand2  g30(.a(new_n40_), .b(x13), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n55_), .O(z4));
  nand3  g34(.a(x15), .b(x14), .c(new_n31_), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n38_), .c(x14), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n39_), .O(new_n65_));
  inv1   g37(.a(x15), .O(new_n66_));
  nand2  g38(.a(x13), .b(x12), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x15), .O(new_n68_));
  nand3  g40(.a(new_n66_), .b(x13), .c(x12), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(x14), .c(x11), .O(new_n71_));
  oai21  g43(.a(new_n66_), .b(x14), .c(new_n71_), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x04), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(new_n65_), .O(z5));
  nand2  g47(.a(new_n41_), .b(x05), .O(new_n76_));
  nand2  g48(.a(x16), .b(new_n39_), .O(new_n77_));
  inv1   g49(.a(x16), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(x15), .c(x13), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n58_), .c(new_n77_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x14), .O(new_n81_));
  inv1   g53(.a(new_n67_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(x15), .c(x14), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(x16), .c(x11), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n76_), .O(z6));
  nand2  g59(.a(new_n41_), .b(x06), .O(new_n88_));
  nand2  g60(.a(x17), .b(new_n39_), .O(new_n89_));
  inv1   g61(.a(x17), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(x16), .c(x15), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n56_), .c(new_n89_), .O(new_n92_));
  nand2  g64(.a(new_n92_), .b(x14), .O(new_n93_));
  nand4  g65(.a(new_n82_), .b(x16), .c(x15), .d(x14), .O(new_n94_));
  nand3  g66(.a(new_n94_), .b(x17), .c(x11), .O(new_n95_));
  nand2  g67(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand4  g68(.a(new_n96_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n88_), .O(z7));
  nand2  g70(.a(new_n41_), .b(x07), .O(new_n99_));
  nand2  g71(.a(x18), .b(new_n39_), .O(new_n100_));
  nand4  g72(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n101_));
  oai21  g73(.a(new_n101_), .b(new_n56_), .c(new_n100_), .O(new_n102_));
  nand2  g74(.a(new_n102_), .b(x14), .O(new_n103_));
  nand3  g75(.a(x14), .b(x13), .c(x12), .O(new_n104_));
  inv1   g76(.a(new_n104_), .O(new_n105_));
  nand4  g77(.a(new_n105_), .b(x17), .c(x16), .d(x15), .O(new_n106_));
  nand3  g78(.a(new_n106_), .b(x18), .c(x11), .O(new_n107_));
  nand2  g79(.a(new_n107_), .b(new_n103_), .O(new_n108_));
  nand4  g80(.a(new_n108_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n109_));
  nand2  g81(.a(new_n109_), .b(new_n99_), .O(z8));
endmodule


