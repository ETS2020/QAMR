// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  oai21  g10(.a(x12), .b(new_n31_), .c(x08), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n41_));
  oai21  g13(.a(new_n39_), .b(new_n38_), .c(new_n41_), .O(z1));
  inv1   g14(.a(x01), .O(new_n43_));
  xor2a  g15(.a(x12), .b(x11), .O(new_n44_));
  nand4  g16(.a(new_n44_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n45_));
  oai21  g17(.a(new_n39_), .b(new_n43_), .c(new_n45_), .O(z2));
  nand2  g18(.a(x13), .b(new_n40_), .O(new_n47_));
  inv1   g19(.a(x13), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(x12), .c(x11), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n47_), .c(x10), .O(new_n50_));
  nor2   g22(.a(new_n48_), .b(x12), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(new_n50_), .c(x09), .O(new_n52_));
  inv1   g24(.a(x12), .O(new_n53_));
  aoi22  g25(.a(new_n53_), .b(x10), .c(x08), .d(x02), .O(new_n54_));
  oai21  g26(.a(new_n52_), .b(x08), .c(new_n54_), .O(z3));
  nand2  g27(.a(x14), .b(x09), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(x08), .c(new_n31_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g30(.a(x13), .b(x11), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x14), .O(new_n60_));
  nand2  g32(.a(x12), .b(x11), .O(new_n61_));
  inv1   g33(.a(x14), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x13), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n65_));
  nand2  g37(.a(x08), .b(x03), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n65_), .c(new_n58_), .O(z4));
  inv1   g39(.a(x04), .O(new_n68_));
  inv1   g40(.a(x15), .O(new_n69_));
  nand4  g41(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n70_));
  inv1   g42(.a(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n69_), .b(x14), .c(x13), .O(new_n72_));
  oai22  g44(.a(new_n72_), .b(new_n61_), .c(new_n71_), .d(new_n69_), .O(new_n73_));
  nand4  g45(.a(new_n73_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n74_));
  oai21  g46(.a(new_n39_), .b(new_n68_), .c(new_n74_), .O(z5));
  nand2  g47(.a(x16), .b(x09), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(x08), .c(new_n31_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n53_), .O(new_n78_));
  nand2  g50(.a(x15), .b(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n59_), .c(x16), .O(new_n80_));
  nand3  g52(.a(x13), .b(x12), .c(x11), .O(new_n81_));
  inv1   g53(.a(x16), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(x15), .c(x14), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x05), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n85_), .c(new_n78_), .O(z6));
  nand2  g59(.a(x17), .b(x09), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(x08), .c(new_n31_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n53_), .O(new_n90_));
  nand3  g62(.a(x16), .b(x15), .c(x14), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n59_), .c(x17), .O(new_n92_));
  inv1   g64(.a(x17), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(x16), .c(x15), .d(x14), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n81_), .c(new_n92_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n96_));
  nand2  g68(.a(x08), .b(x06), .O(new_n97_));
  nand3  g69(.a(new_n97_), .b(new_n96_), .c(new_n90_), .O(z7));
  inv1   g70(.a(x07), .O(new_n99_));
  nand4  g71(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n100_));
  oai21  g72(.a(new_n100_), .b(new_n81_), .c(x18), .O(new_n101_));
  nor2   g73(.a(new_n82_), .b(new_n69_), .O(new_n102_));
  nand4  g74(.a(new_n102_), .b(new_n71_), .c(new_n29_), .d(x17), .O(new_n103_));
  nand2  g75(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand4  g76(.a(new_n104_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n105_));
  oai21  g77(.a(new_n39_), .b(new_n99_), .c(new_n105_), .O(z8));
endmodule


