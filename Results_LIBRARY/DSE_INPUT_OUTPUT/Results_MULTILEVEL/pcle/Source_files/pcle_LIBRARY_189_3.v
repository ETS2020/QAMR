// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n70_, new_n71_;
  inv1   g00(.a(x00), .O(new_n30_));
  inv1   g01(.a(x09), .O(new_n31_));
  inv1   g02(.a(x15), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x08), .O(new_n33_));
  inv1   g04(.a(x08), .O(new_n34_));
  nand2  g05(.a(x09), .b(new_n34_), .O(new_n35_));
  inv1   g06(.a(x10), .O(new_n36_));
  inv1   g07(.a(x11), .O(new_n37_));
  nand3  g08(.a(new_n32_), .b(new_n37_), .c(new_n36_), .O(new_n38_));
  oai22  g09(.a(new_n38_), .b(new_n35_), .c(new_n33_), .d(new_n30_), .O(z1));
  inv1   g10(.a(x01), .O(new_n40_));
  xor2a  g11(.a(x12), .b(x11), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n32_), .c(new_n36_), .d(x09), .O(new_n42_));
  oai22  g13(.a(new_n42_), .b(x08), .c(new_n33_), .d(new_n40_), .O(z2));
  nand2  g14(.a(x12), .b(x11), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x13), .O(new_n45_));
  inv1   g16(.a(x13), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x12), .c(x11), .O(new_n47_));
  aoi21  g18(.a(new_n47_), .b(new_n45_), .c(x10), .O(new_n48_));
  aoi21  g19(.a(new_n48_), .b(new_n34_), .c(x15), .O(new_n49_));
  nand2  g20(.a(x08), .b(x02), .O(new_n50_));
  oai21  g21(.a(new_n49_), .b(new_n31_), .c(new_n50_), .O(z3));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x14), .O(new_n53_));
  inv1   g24(.a(x14), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x13), .c(x12), .d(x11), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(new_n53_), .c(x10), .O(new_n56_));
  aoi21  g27(.a(new_n56_), .b(new_n34_), .c(x15), .O(new_n57_));
  nand2  g28(.a(x08), .b(x03), .O(new_n58_));
  oai21  g29(.a(new_n57_), .b(new_n31_), .c(new_n58_), .O(z4));
  inv1   g30(.a(x04), .O(new_n60_));
  nand4  g31(.a(x11), .b(new_n36_), .c(x09), .d(new_n34_), .O(new_n61_));
  nand4  g32(.a(new_n32_), .b(x14), .c(x13), .d(x12), .O(new_n62_));
  oai22  g33(.a(new_n62_), .b(new_n61_), .c(new_n33_), .d(new_n60_), .O(z5));
  inv1   g34(.a(x05), .O(new_n64_));
  nand3  g35(.a(x16), .b(new_n32_), .c(new_n36_), .O(new_n65_));
  oai22  g36(.a(new_n65_), .b(new_n35_), .c(new_n33_), .d(new_n64_), .O(z6));
  inv1   g37(.a(x06), .O(new_n67_));
  nand3  g38(.a(x17), .b(new_n32_), .c(new_n36_), .O(new_n68_));
  oai22  g39(.a(new_n68_), .b(new_n35_), .c(new_n33_), .d(new_n67_), .O(z7));
  inv1   g40(.a(x07), .O(new_n70_));
  nand3  g41(.a(x18), .b(new_n32_), .c(new_n36_), .O(new_n71_));
  oai22  g42(.a(new_n71_), .b(new_n35_), .c(new_n33_), .d(new_n70_), .O(z8));
  zero   g43(.O(z0));
endmodule


