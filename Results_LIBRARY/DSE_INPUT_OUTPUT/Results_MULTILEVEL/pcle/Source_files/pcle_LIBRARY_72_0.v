// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_;
  inv1   g00(.a(x09), .O(new_n29_));
  nand2  g01(.a(x12), .b(x11), .O(new_n30_));
  nor4   g02(.a(new_n30_), .b(x10), .c(new_n29_), .d(x08), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  inv1   g04(.a(x17), .O(new_n33_));
  inv1   g05(.a(x18), .O(new_n34_));
  nand3  g06(.a(x15), .b(x14), .c(x13), .O(new_n35_));
  nor4   g07(.a(new_n35_), .b(new_n34_), .c(new_n33_), .d(new_n32_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n31_), .O(new_n37_));
  oai21  g09(.a(x09), .b(x03), .c(new_n37_), .O(z0));
  inv1   g10(.a(x03), .O(new_n39_));
  inv1   g11(.a(x08), .O(new_n40_));
  aoi21  g12(.a(new_n29_), .b(new_n39_), .c(new_n40_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  inv1   g14(.a(x10), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  nand4  g16(.a(new_n44_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n42_), .O(z1));
  nand2  g18(.a(new_n41_), .b(x01), .O(new_n47_));
  xor2a  g19(.a(x12), .b(x11), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n47_), .O(z2));
  nand2  g22(.a(new_n41_), .b(x02), .O(new_n51_));
  xnor2a g23(.a(x13), .b(x12), .O(new_n52_));
  nand2  g24(.a(x13), .b(new_n44_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n44_), .c(new_n53_), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n51_), .O(z3));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x13), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n30_), .c(new_n58_), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n62_));
  oai21  g34(.a(new_n40_), .b(new_n39_), .c(new_n62_), .O(z4));
  nand2  g35(.a(new_n41_), .b(x04), .O(new_n64_));
  nand2  g36(.a(new_n57_), .b(x15), .O(new_n65_));
  inv1   g37(.a(x15), .O(new_n66_));
  nand4  g38(.a(new_n66_), .b(x13), .c(x12), .d(x11), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(x14), .O(new_n69_));
  nand2  g41(.a(x15), .b(new_n59_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n64_), .O(z5));
  nand2  g45(.a(new_n41_), .b(x05), .O(new_n74_));
  oai21  g46(.a(new_n35_), .b(new_n30_), .c(x16), .O(new_n75_));
  nand3  g47(.a(new_n32_), .b(x15), .c(x14), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n57_), .c(new_n75_), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n74_), .O(z6));
  nand2  g51(.a(new_n41_), .b(x06), .O(new_n80_));
  nand3  g52(.a(x16), .b(x15), .c(x14), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n57_), .c(x17), .O(new_n82_));
  nand4  g54(.a(new_n33_), .b(x16), .c(x15), .d(x14), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n57_), .c(new_n82_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n80_), .O(z7));
  nand2  g58(.a(new_n41_), .b(x07), .O(new_n87_));
  nand4  g59(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n57_), .c(x18), .O(new_n89_));
  nand4  g61(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n90_));
  nor2   g62(.a(new_n32_), .b(new_n66_), .O(new_n91_));
  nand3  g63(.a(new_n91_), .b(new_n34_), .c(x17), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(new_n43_), .c(x09), .d(new_n40_), .O(new_n94_));
  nand2  g66(.a(new_n94_), .b(new_n87_), .O(z8));
endmodule


