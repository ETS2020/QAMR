// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  nand2  g00(.a(x04), .b(x01), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  nand2  g02(.a(x12), .b(x11), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x10), .O(new_n32_));
  nand3  g04(.a(x15), .b(x14), .c(x13), .O(new_n33_));
  nand3  g05(.a(x18), .b(x17), .c(x16), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n32_), .c(x09), .d(new_n30_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n29_), .O(z0));
  nand2  g09(.a(x08), .b(x00), .O(new_n38_));
  inv1   g10(.a(x10), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n39_), .c(x09), .d(new_n30_), .O(new_n41_));
  nand3  g13(.a(new_n41_), .b(new_n38_), .c(new_n29_), .O(z1));
  xor2a  g14(.a(x12), .b(x11), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n29_), .c(new_n39_), .d(x09), .O(new_n44_));
  inv1   g16(.a(x04), .O(new_n45_));
  nand3  g17(.a(x08), .b(new_n45_), .c(x01), .O(new_n46_));
  oai21  g18(.a(new_n44_), .b(x08), .c(new_n46_), .O(z2));
  inv1   g19(.a(x02), .O(new_n48_));
  oai21  g20(.a(new_n30_), .b(new_n48_), .c(new_n45_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(x01), .O(new_n50_));
  nand2  g22(.a(new_n31_), .b(x13), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n39_), .c(x09), .d(new_n30_), .O(new_n55_));
  inv1   g27(.a(x01), .O(new_n56_));
  nand3  g28(.a(x08), .b(x02), .c(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n55_), .c(new_n50_), .O(z3));
  nand2  g30(.a(x04), .b(x01), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(x08), .c(x03), .O(new_n60_));
  nand3  g32(.a(x13), .b(x12), .c(x11), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x14), .O(new_n62_));
  inv1   g34(.a(x14), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x13), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n31_), .c(new_n62_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n39_), .c(x09), .d(new_n30_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n60_), .c(new_n29_), .O(z4));
  inv1   g39(.a(x15), .O(new_n68_));
  nand4  g40(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n68_), .b(x14), .c(x13), .O(new_n71_));
  oai22  g43(.a(new_n71_), .b(new_n31_), .c(new_n70_), .d(new_n68_), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(new_n29_), .c(new_n39_), .d(x09), .O(new_n73_));
  nand3  g45(.a(x08), .b(x04), .c(new_n56_), .O(new_n74_));
  oai21  g46(.a(new_n73_), .b(x08), .c(new_n74_), .O(z5));
  oai21  g47(.a(new_n33_), .b(new_n31_), .c(x16), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(x15), .c(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n61_), .c(new_n76_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n39_), .c(x09), .d(new_n30_), .O(new_n80_));
  inv1   g52(.a(new_n29_), .O(new_n81_));
  aoi21  g53(.a(x08), .b(x05), .c(new_n81_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n80_), .O(z6));
  nand3  g55(.a(x16), .b(x15), .c(x14), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n61_), .c(x17), .O(new_n85_));
  inv1   g57(.a(x17), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(x16), .c(x15), .d(x14), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n61_), .c(new_n85_), .O(new_n88_));
  nand4  g60(.a(new_n88_), .b(new_n39_), .c(x09), .d(new_n30_), .O(new_n89_));
  nand2  g61(.a(x08), .b(x06), .O(new_n90_));
  aoi21  g62(.a(new_n90_), .b(new_n89_), .c(new_n81_), .O(z7));
  nand4  g63(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n61_), .c(x18), .O(new_n93_));
  nor2   g65(.a(new_n77_), .b(new_n68_), .O(new_n94_));
  nor2   g66(.a(x18), .b(new_n86_), .O(new_n95_));
  nand3  g67(.a(new_n95_), .b(new_n94_), .c(new_n70_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand4  g69(.a(new_n97_), .b(new_n39_), .c(x09), .d(new_n30_), .O(new_n98_));
  nand2  g70(.a(x08), .b(x07), .O(new_n99_));
  aoi21  g71(.a(new_n99_), .b(new_n98_), .c(new_n81_), .O(z8));
endmodule


