// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x17), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(x11), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand4  g13(.a(new_n41_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g15(.a(new_n39_), .b(x08), .O(new_n44_));
  inv1   g16(.a(new_n44_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand3  g19(.a(x17), .b(new_n47_), .c(x11), .O(new_n48_));
  oai21  g20(.a(new_n47_), .b(x11), .c(new_n48_), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n46_), .O(z2));
  nand2  g23(.a(x09), .b(new_n30_), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(x12), .c(new_n31_), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n52_), .c(x17), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x11), .O(new_n56_));
  aoi21  g28(.a(x12), .b(x11), .c(new_n53_), .O(new_n57_));
  nand4  g29(.a(new_n57_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n58_));
  nand2  g30(.a(x08), .b(x02), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(z3));
  nand3  g32(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n61_));
  inv1   g33(.a(x14), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(x13), .c(x12), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n61_), .c(x17), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x11), .O(new_n65_));
  nand3  g37(.a(x13), .b(x12), .c(x11), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  nor2   g39(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n69_));
  nand2  g41(.a(x08), .b(x03), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n69_), .c(new_n65_), .O(z4));
  nand2  g43(.a(new_n45_), .b(x04), .O(new_n72_));
  inv1   g44(.a(x15), .O(new_n73_));
  nand3  g45(.a(x14), .b(x13), .c(x12), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(x17), .c(new_n41_), .O(new_n75_));
  nand3  g47(.a(x17), .b(new_n73_), .c(x14), .O(new_n76_));
  oai22  g48(.a(new_n76_), .b(new_n66_), .c(new_n75_), .d(new_n73_), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n72_), .O(z5));
  nand2  g51(.a(new_n45_), .b(x05), .O(new_n80_));
  inv1   g52(.a(x16), .O(new_n81_));
  nand4  g53(.a(x15), .b(x14), .c(x13), .d(x12), .O(new_n82_));
  aoi21  g54(.a(new_n82_), .b(x17), .c(new_n41_), .O(new_n83_));
  nor2   g55(.a(new_n73_), .b(new_n62_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n67_), .c(x17), .d(new_n81_), .O(new_n85_));
  oai21  g57(.a(new_n83_), .b(new_n81_), .c(new_n85_), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n80_), .O(z6));
  nand2  g60(.a(new_n45_), .b(x06), .O(new_n89_));
  nand3  g61(.a(x16), .b(x15), .c(x14), .O(new_n90_));
  inv1   g62(.a(new_n90_), .O(new_n91_));
  aoi21  g63(.a(new_n91_), .b(new_n67_), .c(new_n38_), .O(new_n92_));
  nand4  g64(.a(new_n92_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(new_n89_), .O(z7));
  nand2  g66(.a(new_n45_), .b(x07), .O(new_n95_));
  nand2  g67(.a(x13), .b(x12), .O(new_n96_));
  oai21  g68(.a(new_n90_), .b(new_n96_), .c(x18), .O(new_n97_));
  nand4  g69(.a(new_n29_), .b(x16), .c(x15), .d(x14), .O(new_n98_));
  oai21  g70(.a(new_n98_), .b(new_n66_), .c(new_n97_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(x17), .O(new_n100_));
  nand2  g72(.a(x18), .b(new_n41_), .O(new_n101_));
  nand2  g73(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g74(.a(new_n102_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n103_));
  nand2  g75(.a(new_n103_), .b(new_n95_), .O(z8));
endmodule


