// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  nand2  g10(.a(x09), .b(new_n30_), .O(new_n39_));
  oai21  g11(.a(x12), .b(x10), .c(x08), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand3  g13(.a(x12), .b(new_n41_), .c(new_n31_), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(new_n39_), .c(new_n40_), .d(new_n38_), .O(z1));
  nand3  g15(.a(new_n41_), .b(x09), .c(new_n30_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x12), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n31_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  inv1   g20(.a(x02), .O(new_n49_));
  xor2a  g21(.a(x13), .b(x11), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(x12), .c(new_n31_), .d(x09), .O(new_n51_));
  oai22  g23(.a(new_n51_), .b(x08), .c(new_n40_), .d(new_n49_), .O(z3));
  inv1   g24(.a(x12), .O(new_n53_));
  inv1   g25(.a(x09), .O(new_n54_));
  nand2  g26(.a(x13), .b(x11), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x14), .O(new_n56_));
  inv1   g28(.a(x14), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(x13), .c(x11), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n30_), .c(new_n53_), .O(new_n60_));
  nand2  g32(.a(x08), .b(x03), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(x10), .c(new_n61_), .O(z4));
  inv1   g34(.a(x04), .O(new_n63_));
  nand3  g35(.a(x14), .b(x13), .c(x11), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x15), .O(new_n65_));
  inv1   g37(.a(x15), .O(new_n66_));
  inv1   g38(.a(new_n55_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(x14), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n65_), .c(new_n53_), .O(new_n69_));
  nand4  g41(.a(new_n69_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n70_));
  oai21  g42(.a(new_n40_), .b(new_n63_), .c(new_n70_), .O(z5));
  nand2  g43(.a(x15), .b(x14), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n55_), .c(x16), .O(new_n73_));
  inv1   g45(.a(x16), .O(new_n74_));
  nand4  g46(.a(new_n67_), .b(new_n74_), .c(x15), .d(x14), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n73_), .c(new_n54_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n30_), .c(new_n53_), .O(new_n77_));
  nand2  g49(.a(x08), .b(x05), .O(new_n78_));
  oai21  g50(.a(new_n77_), .b(x10), .c(new_n78_), .O(z6));
  nand3  g51(.a(x16), .b(x15), .c(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n55_), .c(x17), .O(new_n81_));
  inv1   g53(.a(new_n64_), .O(new_n82_));
  nor2   g54(.a(x17), .b(new_n74_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(x15), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n81_), .c(new_n54_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n30_), .c(new_n53_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x06), .O(new_n87_));
  oai21  g59(.a(new_n86_), .b(x10), .c(new_n87_), .O(z7));
  inv1   g60(.a(x07), .O(new_n89_));
  nand3  g61(.a(x17), .b(x16), .c(x15), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n64_), .c(x18), .O(new_n91_));
  nand4  g63(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n64_), .c(new_n91_), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(x12), .c(new_n31_), .d(x09), .O(new_n94_));
  oai22  g66(.a(new_n94_), .b(x08), .c(new_n40_), .d(new_n89_), .O(z8));
endmodule


