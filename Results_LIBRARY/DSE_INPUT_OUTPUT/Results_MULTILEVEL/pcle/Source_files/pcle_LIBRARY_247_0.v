// Benchmark "FAU" written by ABC on Fri Aug 14 01:58:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n31_), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n39_), .O(z1));
  inv1   g13(.a(x01), .O(new_n42_));
  nand2  g14(.a(x09), .b(new_n30_), .O(new_n43_));
  nand2  g15(.a(new_n39_), .b(x08), .O(new_n44_));
  inv1   g16(.a(x12), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(x11), .c(new_n31_), .O(new_n46_));
  oai22  g18(.a(new_n46_), .b(new_n43_), .c(new_n44_), .d(new_n42_), .O(z2));
  inv1   g19(.a(x02), .O(new_n48_));
  xor2a  g20(.a(x13), .b(x12), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(x11), .c(new_n31_), .d(x09), .O(new_n50_));
  oai22  g22(.a(new_n50_), .b(x08), .c(new_n44_), .d(new_n48_), .O(z3));
  inv1   g23(.a(x03), .O(new_n52_));
  nand2  g24(.a(x13), .b(x12), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x14), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(x13), .c(x12), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n54_), .c(new_n38_), .O(new_n57_));
  nand4  g29(.a(new_n57_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n58_));
  oai21  g30(.a(new_n44_), .b(new_n52_), .c(new_n58_), .O(z4));
  inv1   g31(.a(x09), .O(new_n60_));
  nand3  g32(.a(x14), .b(x13), .c(x12), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x15), .O(new_n62_));
  inv1   g34(.a(x15), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(x14), .c(x13), .d(x12), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n30_), .c(new_n38_), .O(new_n66_));
  nand2  g38(.a(x08), .b(x04), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(x10), .c(new_n67_), .O(z5));
  inv1   g40(.a(x05), .O(new_n69_));
  nand2  g41(.a(x15), .b(x14), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n53_), .c(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(x15), .c(x14), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n53_), .c(new_n71_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(x11), .c(new_n31_), .d(x09), .O(new_n75_));
  oai22  g47(.a(new_n75_), .b(x08), .c(new_n44_), .d(new_n69_), .O(z6));
  nand3  g48(.a(x16), .b(x15), .c(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n53_), .c(x17), .O(new_n78_));
  inv1   g50(.a(new_n61_), .O(new_n79_));
  nor2   g51(.a(x17), .b(new_n72_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n79_), .c(x15), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n78_), .c(new_n60_), .O(new_n82_));
  aoi21  g54(.a(new_n82_), .b(new_n30_), .c(new_n38_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x06), .O(new_n84_));
  oai21  g56(.a(new_n83_), .b(x10), .c(new_n84_), .O(z7));
  nand3  g57(.a(x17), .b(x16), .c(x15), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n61_), .c(x18), .O(new_n87_));
  and2   g59(.a(x16), .b(x15), .O(new_n88_));
  nand4  g60(.a(new_n88_), .b(new_n79_), .c(new_n29_), .d(x17), .O(new_n89_));
  aoi21  g61(.a(new_n89_), .b(new_n87_), .c(new_n60_), .O(new_n90_));
  aoi21  g62(.a(new_n90_), .b(new_n30_), .c(new_n38_), .O(new_n91_));
  nand2  g63(.a(x08), .b(x07), .O(new_n92_));
  oai21  g64(.a(new_n91_), .b(x10), .c(new_n92_), .O(z8));
endmodule


