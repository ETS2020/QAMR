// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  nand2  g09(.a(x12), .b(x10), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x09), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(x08), .O(new_n41_));
  nor2   g13(.a(x11), .b(x10), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z1));
  inv1   g16(.a(x12), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(x11), .c(new_n31_), .O(new_n46_));
  oai21  g18(.a(new_n45_), .b(x11), .c(new_n46_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(x09), .c(new_n30_), .O(new_n48_));
  inv1   g20(.a(new_n38_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x01), .c(new_n49_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n48_), .O(z2));
  xnor2a g23(.a(x13), .b(x11), .O(new_n52_));
  nand3  g24(.a(x13), .b(new_n45_), .c(new_n31_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n45_), .c(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(x09), .c(new_n30_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x02), .c(new_n49_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n55_), .O(z3));
  inv1   g29(.a(x03), .O(new_n58_));
  nand2  g30(.a(new_n38_), .b(x08), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand3  g32(.a(x13), .b(x12), .c(x11), .O(new_n61_));
  inv1   g33(.a(new_n61_), .O(new_n62_));
  nand2  g34(.a(x12), .b(x11), .O(new_n63_));
  nand2  g35(.a(new_n60_), .b(x13), .O(new_n64_));
  oai22  g36(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n60_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n66_));
  oai21  g38(.a(new_n59_), .b(new_n58_), .c(new_n66_), .O(z4));
  nand3  g39(.a(x11), .b(x09), .c(new_n30_), .O(new_n68_));
  inv1   g40(.a(x15), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(x14), .c(x13), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n68_), .c(new_n31_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x12), .O(new_n72_));
  nand4  g44(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(x15), .c(new_n31_), .O(new_n74_));
  inv1   g46(.a(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(x09), .c(new_n30_), .O(new_n76_));
  nand2  g48(.a(x08), .b(x04), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(z5));
  inv1   g50(.a(x05), .O(new_n79_));
  nand3  g51(.a(x15), .b(x14), .c(x13), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n63_), .c(x16), .O(new_n81_));
  inv1   g53(.a(x16), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(x15), .c(x14), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n61_), .c(new_n81_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n85_));
  oai21  g57(.a(new_n59_), .b(new_n79_), .c(new_n85_), .O(z6));
  inv1   g58(.a(x06), .O(new_n87_));
  nand3  g59(.a(x16), .b(x15), .c(x14), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n61_), .c(x17), .O(new_n89_));
  nor2   g61(.a(new_n69_), .b(new_n60_), .O(new_n90_));
  nor2   g62(.a(x17), .b(new_n82_), .O(new_n91_));
  nand3  g63(.a(new_n91_), .b(new_n90_), .c(new_n62_), .O(new_n92_));
  nand2  g64(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n94_));
  oai21  g66(.a(new_n59_), .b(new_n87_), .c(new_n94_), .O(z7));
  nand3  g67(.a(new_n41_), .b(x13), .c(x11), .O(new_n96_));
  nand4  g68(.a(new_n90_), .b(new_n29_), .c(x17), .d(x16), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n96_), .c(new_n31_), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(x12), .O(new_n99_));
  nand4  g71(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n100_));
  inv1   g72(.a(new_n100_), .O(new_n101_));
  aoi21  g73(.a(new_n101_), .b(new_n62_), .c(new_n29_), .O(new_n102_));
  nand4  g74(.a(new_n102_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n103_));
  nand2  g75(.a(x08), .b(x07), .O(new_n104_));
  nand3  g76(.a(new_n104_), .b(new_n103_), .c(new_n99_), .O(z8));
endmodule


