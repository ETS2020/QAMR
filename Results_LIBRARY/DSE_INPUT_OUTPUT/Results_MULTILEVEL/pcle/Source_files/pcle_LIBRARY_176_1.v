// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x10), .O(new_n29_));
  nand2  g01(.a(x14), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(z0));
  inv1   g03(.a(x00), .O(new_n32_));
  nand2  g04(.a(new_n30_), .b(x08), .O(new_n33_));
  inv1   g05(.a(x09), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(x08), .O(new_n35_));
  nor3   g07(.a(x14), .b(x11), .c(x10), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  oai21  g09(.a(new_n33_), .b(new_n32_), .c(new_n37_), .O(z1));
  inv1   g10(.a(x01), .O(new_n39_));
  inv1   g11(.a(x14), .O(new_n40_));
  xor2a  g12(.a(x12), .b(x11), .O(new_n41_));
  nand4  g13(.a(new_n41_), .b(new_n40_), .c(new_n29_), .d(x09), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(x08), .c(new_n33_), .d(new_n39_), .O(z2));
  inv1   g15(.a(x08), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x13), .O(new_n46_));
  inv1   g18(.a(x13), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(x12), .c(x11), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n46_), .c(new_n34_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n44_), .c(x14), .O(new_n50_));
  nand2  g22(.a(x08), .b(x02), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(x10), .c(new_n51_), .O(z3));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  inv1   g25(.a(new_n53_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n35_), .c(x14), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(x10), .c(new_n56_), .O(z4));
  inv1   g29(.a(x04), .O(new_n58_));
  nand4  g30(.a(new_n35_), .b(x15), .c(new_n40_), .d(new_n29_), .O(new_n59_));
  oai21  g31(.a(new_n33_), .b(new_n58_), .c(new_n59_), .O(z5));
  inv1   g32(.a(x05), .O(new_n61_));
  nand4  g33(.a(new_n35_), .b(x16), .c(new_n40_), .d(new_n29_), .O(new_n62_));
  oai21  g34(.a(new_n33_), .b(new_n61_), .c(new_n62_), .O(z6));
  inv1   g35(.a(x06), .O(new_n64_));
  nand4  g36(.a(new_n35_), .b(x17), .c(new_n40_), .d(new_n29_), .O(new_n65_));
  oai21  g37(.a(new_n33_), .b(new_n64_), .c(new_n65_), .O(z7));
  inv1   g38(.a(x07), .O(new_n67_));
  nand4  g39(.a(new_n35_), .b(x18), .c(new_n40_), .d(new_n29_), .O(new_n68_));
  oai21  g40(.a(new_n33_), .b(new_n67_), .c(new_n68_), .O(z8));
endmodule


