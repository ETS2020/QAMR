// Benchmark "FAU" written by ABC on Fri Aug 14 01:58:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x08), .O(new_n30_));
  inv1   g01(.a(x10), .O(new_n31_));
  aoi21  g02(.a(x15), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x00), .O(new_n33_));
  nor2   g04(.a(x15), .b(x11), .O(new_n34_));
  nand4  g05(.a(new_n34_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n33_), .O(z1));
  nand2  g07(.a(new_n32_), .b(x01), .O(new_n37_));
  inv1   g08(.a(x15), .O(new_n38_));
  xor2a  g09(.a(x12), .b(x11), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(new_n38_), .c(new_n31_), .d(x09), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(x08), .c(new_n37_), .O(z2));
  nand2  g12(.a(new_n32_), .b(x02), .O(new_n42_));
  inv1   g13(.a(x11), .O(new_n43_));
  xnor2a g14(.a(x13), .b(x12), .O(new_n44_));
  nand2  g15(.a(x13), .b(new_n43_), .O(new_n45_));
  oai21  g16(.a(new_n44_), .b(new_n43_), .c(new_n45_), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(new_n38_), .c(new_n31_), .d(x09), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(x08), .c(new_n42_), .O(z3));
  nand2  g19(.a(new_n32_), .b(x03), .O(new_n49_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x14), .O(new_n51_));
  inv1   g22(.a(x14), .O(new_n52_));
  nand4  g23(.a(new_n52_), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  aoi21  g24(.a(new_n53_), .b(new_n51_), .c(x15), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n49_), .O(z4));
  nand2  g27(.a(new_n32_), .b(x04), .O(new_n57_));
  nand4  g28(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n58_));
  nand4  g29(.a(new_n38_), .b(x14), .c(x13), .d(x12), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(z5));
  nand2  g31(.a(x16), .b(x09), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(x08), .c(new_n38_), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n31_), .O(new_n63_));
  nand2  g34(.a(x08), .b(x05), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(z6));
  nand2  g36(.a(x17), .b(x09), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(x08), .c(new_n38_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n31_), .O(new_n68_));
  nand2  g39(.a(x08), .b(x06), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(z7));
  nand2  g41(.a(x18), .b(x09), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(x08), .c(new_n38_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n31_), .O(new_n73_));
  nand2  g44(.a(x08), .b(x07), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n73_), .O(z8));
  zero   g46(.O(z0));
endmodule


