// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x13), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(x11), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(x09), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(x08), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n31_), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g17(.a(new_n39_), .b(x08), .O(new_n46_));
  inv1   g18(.a(new_n46_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x01), .O(new_n48_));
  inv1   g20(.a(x12), .O(new_n49_));
  nand3  g21(.a(x13), .b(new_n49_), .c(x11), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(x11), .c(new_n50_), .O(new_n51_));
  nand4  g23(.a(new_n51_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n48_), .O(z2));
  inv1   g25(.a(x02), .O(new_n54_));
  nand2  g26(.a(x12), .b(x11), .O(new_n55_));
  nand4  g27(.a(new_n55_), .b(x13), .c(new_n31_), .d(x09), .O(new_n56_));
  oai22  g28(.a(new_n56_), .b(x08), .c(new_n46_), .d(new_n54_), .O(z3));
  nand2  g29(.a(new_n47_), .b(x03), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(x12), .c(x11), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(x12), .c(new_n60_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x13), .O(new_n62_));
  oai21  g34(.a(new_n59_), .b(x11), .c(new_n62_), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n58_), .O(z4));
  nand3  g37(.a(x15), .b(new_n59_), .c(x13), .O(new_n66_));
  oai21  g38(.a(x15), .b(new_n59_), .c(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  nand2  g40(.a(x12), .b(x11), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x15), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x04), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n72_), .c(new_n39_), .O(z5));
  nand2  g46(.a(new_n47_), .b(x05), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nand3  g48(.a(x15), .b(x14), .c(x12), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(x16), .O(new_n78_));
  nand3  g50(.a(new_n76_), .b(x15), .c(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n69_), .c(new_n78_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x13), .O(new_n81_));
  oai21  g53(.a(new_n76_), .b(x11), .c(new_n81_), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n75_), .O(z6));
  nand3  g56(.a(new_n43_), .b(x12), .c(new_n31_), .O(new_n85_));
  inv1   g57(.a(x17), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(x16), .c(x15), .d(x14), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n85_), .c(x13), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(x11), .O(new_n89_));
  nand3  g61(.a(x16), .b(x15), .c(x14), .O(new_n90_));
  nor2   g62(.a(new_n90_), .b(new_n69_), .O(new_n91_));
  nor2   g63(.a(new_n91_), .b(new_n86_), .O(new_n92_));
  nand4  g64(.a(new_n92_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n93_));
  nand2  g65(.a(x08), .b(x06), .O(new_n94_));
  nand3  g66(.a(new_n94_), .b(new_n93_), .c(new_n89_), .O(z7));
  nand2  g67(.a(new_n47_), .b(x07), .O(new_n96_));
  nand2  g68(.a(x14), .b(x12), .O(new_n97_));
  nand3  g69(.a(x17), .b(x16), .c(x15), .O(new_n98_));
  oai21  g70(.a(new_n98_), .b(new_n97_), .c(x18), .O(new_n99_));
  nand3  g71(.a(x14), .b(x12), .c(x11), .O(new_n100_));
  nand4  g72(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n101_));
  oai21  g73(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  nand2  g74(.a(new_n102_), .b(x13), .O(new_n103_));
  nand2  g75(.a(x18), .b(new_n41_), .O(new_n104_));
  nand2  g76(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g77(.a(new_n105_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n106_));
  nand2  g78(.a(new_n106_), .b(new_n96_), .O(z8));
endmodule


