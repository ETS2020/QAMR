// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x15), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  and2   g03(.a(x12), .b(x11), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(x13), .O(new_n34_));
  inv1   g06(.a(x14), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(x18), .c(x17), .d(x16), .O(new_n37_));
  or2    g09(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  aoi21  g11(.a(x15), .b(x10), .c(new_n31_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand4  g14(.a(new_n42_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n41_), .O(z1));
  nand2  g16(.a(new_n40_), .b(x01), .O(new_n45_));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n45_), .O(z2));
  nand3  g20(.a(new_n34_), .b(x12), .c(x11), .O(new_n49_));
  oai21  g21(.a(new_n32_), .b(new_n34_), .c(new_n49_), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n51_));
  aoi22  g23(.a(x15), .b(x10), .c(x08), .d(x02), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(z3));
  nand2  g25(.a(new_n40_), .b(x03), .O(new_n54_));
  nand3  g26(.a(x13), .b(x12), .c(x11), .O(new_n55_));
  inv1   g27(.a(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n32_), .b(new_n35_), .c(x13), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(new_n35_), .c(new_n57_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n54_), .O(z4));
  nand2  g32(.a(new_n40_), .b(x04), .O(new_n61_));
  nand2  g33(.a(new_n55_), .b(x15), .O(new_n62_));
  nand4  g34(.a(new_n30_), .b(x13), .c(x12), .d(x11), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x14), .O(new_n65_));
  nand2  g37(.a(x15), .b(new_n35_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g39(.a(new_n67_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n61_), .O(z5));
  nand3  g41(.a(x11), .b(x09), .c(new_n31_), .O(new_n70_));
  inv1   g42(.a(x16), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(x14), .c(x13), .d(x12), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n70_), .c(new_n29_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x15), .O(new_n74_));
  nand3  g46(.a(x15), .b(x14), .c(x13), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n32_), .c(new_n71_), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x05), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n74_), .O(z6));
  inv1   g52(.a(x17), .O(new_n81_));
  nand3  g53(.a(new_n36_), .b(new_n81_), .c(x16), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n33_), .c(new_n29_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(x15), .O(new_n84_));
  nand3  g56(.a(x16), .b(x15), .c(x14), .O(new_n85_));
  inv1   g57(.a(new_n85_), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n56_), .c(new_n81_), .O(new_n87_));
  nand4  g59(.a(new_n87_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n88_));
  nand2  g60(.a(x08), .b(x06), .O(new_n89_));
  nand3  g61(.a(new_n89_), .b(new_n88_), .c(new_n84_), .O(z7));
  nand2  g62(.a(new_n40_), .b(x07), .O(new_n91_));
  nand4  g63(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n55_), .c(x18), .O(new_n93_));
  nor2   g65(.a(new_n71_), .b(new_n30_), .O(new_n94_));
  nor2   g66(.a(x18), .b(new_n81_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(new_n94_), .c(new_n36_), .d(new_n32_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand4  g69(.a(new_n97_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(new_n91_), .O(z8));
endmodule


