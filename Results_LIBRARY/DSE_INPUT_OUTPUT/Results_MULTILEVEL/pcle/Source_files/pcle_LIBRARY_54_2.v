// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x13), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x12), .O(new_n34_));
  inv1   g06(.a(x14), .O(new_n35_));
  nand3  g07(.a(x18), .b(x17), .c(x16), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(x15), .c(new_n29_), .O(z0));
  nor2   g11(.a(x15), .b(new_n29_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand4  g14(.a(new_n42_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n46_), .c(new_n40_), .O(z2));
  nand2  g20(.a(x12), .b(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(x13), .O(new_n50_));
  nand3  g22(.a(new_n29_), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n40_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n53_), .O(z3));
  inv1   g27(.a(new_n49_), .O(new_n56_));
  nand3  g28(.a(new_n35_), .b(x12), .c(x11), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(new_n35_), .c(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x13), .O(new_n59_));
  oai21  g31(.a(new_n35_), .b(x13), .c(new_n59_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n61_));
  aoi21  g33(.a(x08), .b(x03), .c(new_n40_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z4));
  inv1   g35(.a(x15), .O(new_n64_));
  nand3  g36(.a(x13), .b(x12), .c(x11), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n35_), .O(new_n67_));
  oai21  g39(.a(new_n64_), .b(x13), .c(new_n50_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(x14), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n71_));
  aoi21  g43(.a(x08), .b(x04), .c(new_n40_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(z5));
  nand3  g45(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n74_));
  inv1   g46(.a(x16), .O(new_n75_));
  nand3  g47(.a(new_n56_), .b(new_n75_), .c(x14), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n74_), .c(x15), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(x13), .O(new_n78_));
  nand3  g50(.a(new_n56_), .b(x14), .c(x13), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(x16), .c(new_n31_), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(x09), .c(new_n30_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x05), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(new_n78_), .O(z6));
  inv1   g56(.a(x17), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(x16), .c(x14), .d(x12), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n32_), .c(x15), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(x13), .O(new_n88_));
  nor4   g60(.a(new_n49_), .b(new_n75_), .c(new_n35_), .d(new_n29_), .O(new_n89_));
  nor2   g61(.a(new_n89_), .b(new_n85_), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n91_));
  nand2  g63(.a(x08), .b(x06), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(new_n91_), .c(new_n88_), .O(z7));
  inv1   g65(.a(new_n40_), .O(new_n94_));
  nand3  g66(.a(new_n94_), .b(x08), .c(x07), .O(new_n95_));
  nand3  g67(.a(x17), .b(x16), .c(x14), .O(new_n96_));
  oai21  g68(.a(new_n96_), .b(new_n49_), .c(x18), .O(new_n97_));
  inv1   g69(.a(x18), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(x17), .c(x16), .d(x14), .O(new_n99_));
  oai21  g71(.a(new_n99_), .b(new_n65_), .c(new_n97_), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(x15), .O(new_n101_));
  nand2  g73(.a(x18), .b(new_n29_), .O(new_n102_));
  nand2  g74(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g75(.a(new_n103_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n104_));
  nand2  g76(.a(new_n104_), .b(new_n95_), .O(z8));
endmodule


