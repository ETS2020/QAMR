// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x09), .O(new_n29_));
  nor2   g01(.a(x10), .b(x08), .O(new_n30_));
  nand2  g02(.a(x14), .b(x13), .O(new_n31_));
  nand3  g03(.a(x18), .b(x16), .c(x15), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(new_n30_), .c(x12), .d(x11), .O(new_n34_));
  aoi21  g06(.a(new_n34_), .b(x17), .c(new_n29_), .O(z0));
  inv1   g07(.a(x08), .O(new_n36_));
  inv1   g08(.a(x17), .O(new_n37_));
  nor2   g09(.a(x11), .b(x10), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n36_), .c(new_n37_), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  oai21  g12(.a(new_n39_), .b(new_n29_), .c(new_n40_), .O(z1));
  inv1   g13(.a(x10), .O(new_n42_));
  xor2a  g14(.a(x12), .b(x11), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n42_), .c(new_n36_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x17), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x09), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  nand2  g20(.a(x12), .b(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(x13), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(x12), .c(x11), .O(new_n52_));
  aoi21  g24(.a(new_n52_), .b(new_n50_), .c(x10), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n36_), .c(new_n37_), .O(new_n54_));
  nand2  g26(.a(x08), .b(x02), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n29_), .c(new_n55_), .O(z3));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand4  g31(.a(new_n59_), .b(x13), .c(x12), .d(x11), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n58_), .c(x10), .O(new_n61_));
  aoi21  g33(.a(new_n61_), .b(new_n36_), .c(new_n37_), .O(new_n62_));
  nand2  g34(.a(x08), .b(x03), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(new_n29_), .c(new_n63_), .O(z4));
  aoi21  g36(.a(new_n37_), .b(x09), .c(new_n36_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(x04), .O(new_n66_));
  oai21  g38(.a(new_n31_), .b(new_n49_), .c(x15), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(x14), .c(x13), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n49_), .c(new_n67_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(x17), .c(new_n42_), .d(x09), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(x08), .c(new_n66_), .O(z5));
  nand2  g44(.a(new_n65_), .b(x05), .O(new_n73_));
  nand3  g45(.a(x15), .b(x14), .c(x13), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n49_), .c(x16), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x15), .c(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n57_), .c(new_n75_), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(x17), .c(new_n42_), .d(x09), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(x08), .c(new_n73_), .O(z6));
  nand3  g52(.a(x16), .b(x15), .c(x14), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n57_), .c(new_n42_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(x08), .c(x17), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(x09), .O(new_n84_));
  nand2  g56(.a(x08), .b(x06), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z7));
  nand2  g58(.a(new_n65_), .b(x07), .O(new_n87_));
  oai21  g59(.a(new_n81_), .b(new_n57_), .c(x18), .O(new_n88_));
  inv1   g60(.a(x18), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(x16), .c(x15), .d(x14), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n57_), .c(new_n88_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(x17), .c(new_n42_), .d(x09), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(x08), .c(new_n87_), .O(z8));
endmodule


