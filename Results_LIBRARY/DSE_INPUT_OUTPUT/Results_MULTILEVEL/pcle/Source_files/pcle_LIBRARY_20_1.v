// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x13), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x00), .O(new_n31_));
  oai21  g03(.a(new_n29_), .b(x10), .c(x08), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  nor3   g06(.a(x13), .b(x11), .c(x10), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  oai21  g08(.a(new_n32_), .b(new_n31_), .c(new_n36_), .O(z1));
  inv1   g09(.a(x10), .O(new_n38_));
  inv1   g10(.a(x08), .O(new_n39_));
  xor2a  g11(.a(x12), .b(x11), .O(new_n40_));
  nand3  g12(.a(new_n40_), .b(x09), .c(new_n39_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n29_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(z2));
  nand4  g17(.a(x12), .b(x11), .c(x09), .d(new_n39_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n29_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n38_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(z3));
  nand2  g22(.a(x14), .b(x09), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(x08), .c(new_n29_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n38_), .O(new_n53_));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n53_), .O(z4));
  inv1   g27(.a(x04), .O(new_n56_));
  nand4  g28(.a(new_n34_), .b(x15), .c(new_n29_), .d(new_n38_), .O(new_n57_));
  oai21  g29(.a(new_n32_), .b(new_n56_), .c(new_n57_), .O(z5));
  inv1   g30(.a(x05), .O(new_n59_));
  nand4  g31(.a(new_n34_), .b(x16), .c(new_n29_), .d(new_n38_), .O(new_n60_));
  oai21  g32(.a(new_n32_), .b(new_n59_), .c(new_n60_), .O(z6));
  nand2  g33(.a(x17), .b(x09), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(x08), .c(new_n29_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n38_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x06), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z7));
  inv1   g38(.a(x07), .O(new_n67_));
  nand4  g39(.a(new_n34_), .b(x18), .c(new_n29_), .d(new_n38_), .O(new_n68_));
  oai21  g40(.a(new_n32_), .b(new_n67_), .c(new_n68_), .O(z8));
endmodule


