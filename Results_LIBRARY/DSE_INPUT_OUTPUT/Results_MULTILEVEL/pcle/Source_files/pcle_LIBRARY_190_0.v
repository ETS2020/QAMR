// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  nor2   g09(.a(x17), .b(x15), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x09), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(x08), .O(new_n41_));
  nor2   g13(.a(x11), .b(x10), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z1));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n46_), .c(new_n38_), .O(z2));
  inv1   g20(.a(x13), .O(new_n49_));
  nand2  g21(.a(x12), .b(x11), .O(new_n50_));
  inv1   g22(.a(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n49_), .b(x12), .c(x11), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  nand4  g25(.a(new_n53_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n38_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z3));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x13), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n50_), .c(new_n58_), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n62_));
  nand2  g34(.a(x08), .b(x03), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n62_), .c(new_n38_), .O(z4));
  inv1   g36(.a(new_n38_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(x08), .c(x04), .O(new_n66_));
  nand2  g38(.a(new_n57_), .b(x15), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand3  g40(.a(x17), .b(new_n68_), .c(x13), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n50_), .c(new_n67_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x14), .O(new_n71_));
  nand2  g43(.a(x15), .b(new_n59_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand4  g45(.a(new_n73_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n66_), .O(z5));
  nand3  g47(.a(new_n51_), .b(x14), .c(x13), .O(new_n76_));
  nand4  g48(.a(new_n76_), .b(x16), .c(new_n31_), .d(x09), .O(new_n77_));
  nand2  g49(.a(x08), .b(x05), .O(new_n78_));
  oai21  g50(.a(new_n77_), .b(x08), .c(new_n78_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n65_), .O(new_n80_));
  inv1   g52(.a(x16), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(x15), .c(x14), .O(new_n82_));
  nand3  g54(.a(x17), .b(x16), .c(new_n68_), .O(new_n83_));
  oai21  g55(.a(new_n82_), .b(new_n57_), .c(new_n83_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n80_), .O(z6));
  nand3  g58(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(x17), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n68_), .O(new_n89_));
  nand3  g61(.a(x16), .b(x14), .c(x13), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n50_), .c(x17), .O(new_n91_));
  inv1   g63(.a(x17), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(x16), .c(x14), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n57_), .c(new_n91_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n95_));
  nand2  g67(.a(x08), .b(x06), .O(new_n96_));
  nand3  g68(.a(new_n96_), .b(new_n95_), .c(new_n89_), .O(z7));
  nand3  g69(.a(new_n41_), .b(x18), .c(new_n31_), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(x17), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(new_n68_), .O(new_n100_));
  nand3  g72(.a(x17), .b(x16), .c(x14), .O(new_n101_));
  oai21  g73(.a(new_n101_), .b(new_n57_), .c(x18), .O(new_n102_));
  nand4  g74(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n103_));
  oai21  g75(.a(new_n103_), .b(new_n76_), .c(new_n102_), .O(new_n104_));
  nand4  g76(.a(new_n104_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n105_));
  nand2  g77(.a(x08), .b(x07), .O(new_n106_));
  nand3  g78(.a(new_n106_), .b(new_n105_), .c(new_n100_), .O(z8));
endmodule


