// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  inv1   g10(.a(x14), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(x11), .c(x08), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(x09), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(x08), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n39_), .c(new_n41_), .d(new_n31_), .O(new_n44_));
  oai21  g16(.a(new_n40_), .b(new_n38_), .c(new_n44_), .O(z1));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n47_));
  nor2   g19(.a(new_n39_), .b(x11), .O(new_n48_));
  aoi21  g20(.a(x08), .b(x01), .c(new_n48_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n47_), .O(z2));
  xnor2a g22(.a(x13), .b(x12), .O(new_n51_));
  nand2  g23(.a(x13), .b(new_n41_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n41_), .c(new_n52_), .O(new_n53_));
  nand4  g25(.a(new_n53_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n48_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z3));
  nand2  g28(.a(x13), .b(x12), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nand4  g30(.a(new_n39_), .b(x13), .c(x12), .d(x11), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(new_n39_), .c(new_n59_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n61_));
  aoi21  g33(.a(x08), .b(x03), .c(new_n48_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z4));
  inv1   g35(.a(x15), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(x13), .c(x12), .O(new_n65_));
  oai21  g37(.a(new_n58_), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x14), .O(new_n67_));
  oai21  g39(.a(new_n64_), .b(x14), .c(new_n67_), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x04), .c(new_n48_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z5));
  inv1   g43(.a(new_n40_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x05), .O(new_n73_));
  inv1   g45(.a(x16), .O(new_n74_));
  nand3  g46(.a(x15), .b(x13), .c(x12), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x16), .O(new_n76_));
  nand3  g48(.a(new_n74_), .b(x15), .c(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n57_), .c(new_n76_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(x11), .O(new_n79_));
  oai21  g51(.a(new_n74_), .b(x14), .c(new_n79_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n73_), .O(z6));
  nand3  g54(.a(new_n43_), .b(x12), .c(new_n31_), .O(new_n83_));
  inv1   g55(.a(x17), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(x16), .c(x15), .d(x13), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n83_), .c(x11), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(x14), .O(new_n87_));
  nand3  g59(.a(x16), .b(x15), .c(x14), .O(new_n88_));
  inv1   g60(.a(new_n88_), .O(new_n89_));
  aoi21  g61(.a(new_n89_), .b(new_n58_), .c(new_n84_), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n91_));
  nand2  g63(.a(x08), .b(x06), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(new_n91_), .c(new_n87_), .O(z7));
  nand2  g65(.a(new_n72_), .b(x07), .O(new_n94_));
  nand3  g66(.a(x17), .b(x16), .c(x15), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n57_), .c(x18), .O(new_n96_));
  nand3  g68(.a(x14), .b(x13), .c(x12), .O(new_n97_));
  nand4  g69(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n98_));
  oai21  g70(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(x11), .O(new_n100_));
  nand2  g72(.a(x18), .b(new_n39_), .O(new_n101_));
  nand2  g73(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g74(.a(new_n102_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n103_));
  nand2  g75(.a(new_n103_), .b(new_n94_), .O(z8));
endmodule


