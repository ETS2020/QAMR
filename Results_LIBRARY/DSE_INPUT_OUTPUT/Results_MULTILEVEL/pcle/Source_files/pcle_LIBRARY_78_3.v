// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x13), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand4  g03(.a(x12), .b(x11), .c(x09), .d(new_n31_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x14), .O(new_n34_));
  inv1   g06(.a(x15), .O(new_n35_));
  nand3  g07(.a(x18), .b(x17), .c(x16), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g11(.a(new_n30_), .b(new_n29_), .O(new_n40_));
  inv1   g12(.a(new_n40_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(x13), .b(new_n47_), .O(new_n48_));
  oai22  g20(.a(new_n48_), .b(new_n43_), .c(new_n46_), .d(x10), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(x09), .c(new_n31_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x01), .c(new_n40_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z2));
  nand3  g24(.a(new_n30_), .b(x12), .c(new_n29_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n48_), .c(new_n43_), .O(new_n54_));
  nor2   g26(.a(new_n30_), .b(x11), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n54_), .c(x09), .O(new_n56_));
  aoi21  g28(.a(x08), .b(x02), .c(new_n40_), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(x08), .c(new_n57_), .O(z3));
  nand2  g30(.a(x09), .b(new_n31_), .O(new_n59_));
  nand3  g31(.a(new_n34_), .b(x12), .c(x11), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n59_), .c(new_n29_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x13), .O(new_n62_));
  nand3  g34(.a(x13), .b(x12), .c(x11), .O(new_n63_));
  inv1   g35(.a(new_n63_), .O(new_n64_));
  nor2   g36(.a(new_n64_), .b(new_n34_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n66_));
  nand2  g38(.a(x08), .b(x03), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n62_), .O(z4));
  nand3  g40(.a(new_n41_), .b(x08), .c(x04), .O(new_n69_));
  nand2  g41(.a(x12), .b(x11), .O(new_n70_));
  nand4  g42(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n71_));
  inv1   g43(.a(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n35_), .b(x14), .c(x13), .O(new_n73_));
  oai22  g45(.a(new_n73_), .b(new_n70_), .c(new_n72_), .d(new_n35_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n69_), .O(z5));
  nand3  g48(.a(x11), .b(x09), .c(new_n31_), .O(new_n77_));
  inv1   g49(.a(x16), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(x15), .c(x14), .d(x12), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n77_), .c(new_n29_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x13), .O(new_n81_));
  nand3  g53(.a(x15), .b(x14), .c(x13), .O(new_n82_));
  nor2   g54(.a(new_n82_), .b(new_n70_), .O(new_n83_));
  nor2   g55(.a(new_n83_), .b(new_n78_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x05), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n85_), .c(new_n81_), .O(z6));
  inv1   g59(.a(x17), .O(new_n88_));
  nand4  g60(.a(new_n88_), .b(x16), .c(x15), .d(x14), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n32_), .c(new_n29_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(x13), .O(new_n91_));
  nand3  g63(.a(x16), .b(x15), .c(x14), .O(new_n92_));
  inv1   g64(.a(new_n92_), .O(new_n93_));
  aoi21  g65(.a(new_n93_), .b(new_n64_), .c(new_n88_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n95_));
  nand2  g67(.a(x08), .b(x06), .O(new_n96_));
  nand3  g68(.a(new_n96_), .b(new_n95_), .c(new_n91_), .O(z7));
  nand3  g69(.a(new_n41_), .b(x08), .c(x07), .O(new_n98_));
  nand4  g70(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n99_));
  oai21  g71(.a(new_n99_), .b(new_n63_), .c(x18), .O(new_n100_));
  nor2   g72(.a(new_n78_), .b(new_n35_), .O(new_n101_));
  nor2   g73(.a(x18), .b(new_n88_), .O(new_n102_));
  nand3  g74(.a(new_n102_), .b(new_n101_), .c(new_n72_), .O(new_n103_));
  nand2  g75(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand4  g76(.a(new_n104_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n105_));
  nand2  g77(.a(new_n105_), .b(new_n98_), .O(z8));
endmodule


