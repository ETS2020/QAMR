// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x11), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand4  g03(.a(x13), .b(x12), .c(x09), .d(new_n31_), .O(new_n32_));
  inv1   g04(.a(x14), .O(new_n33_));
  inv1   g05(.a(x15), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x18), .c(x17), .d(x16), .O(new_n36_));
  or2    g08(.a(new_n36_), .b(new_n32_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g10(.a(new_n30_), .b(new_n29_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  nand4  g13(.a(new_n30_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(z1));
  nand3  g15(.a(x12), .b(new_n30_), .c(new_n29_), .O(new_n44_));
  oai21  g16(.a(x12), .b(new_n30_), .c(new_n44_), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(x09), .c(new_n31_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n39_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  inv1   g20(.a(x02), .O(new_n49_));
  nand2  g21(.a(new_n40_), .b(x08), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  and2   g23(.a(x12), .b(x11), .O(new_n52_));
  nand3  g24(.a(new_n51_), .b(x12), .c(x11), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n55_));
  oai21  g27(.a(new_n50_), .b(new_n49_), .c(new_n55_), .O(z3));
  inv1   g28(.a(new_n50_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x03), .O(new_n58_));
  nand3  g30(.a(x13), .b(x12), .c(x11), .O(new_n59_));
  inv1   g31(.a(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n52_), .b(new_n33_), .c(x13), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n33_), .c(new_n61_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n58_), .O(z4));
  nand2  g36(.a(new_n57_), .b(x04), .O(new_n65_));
  nand2  g37(.a(new_n59_), .b(x15), .O(new_n66_));
  nand4  g38(.a(new_n34_), .b(x13), .c(x12), .d(x11), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(x14), .O(new_n69_));
  nand2  g41(.a(x15), .b(new_n33_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n65_), .O(z5));
  nand3  g45(.a(x12), .b(x09), .c(new_n31_), .O(new_n74_));
  inv1   g46(.a(x16), .O(new_n75_));
  nor2   g47(.a(new_n33_), .b(new_n51_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n75_), .c(x15), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n74_), .c(new_n29_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(x11), .O(new_n79_));
  nand3  g51(.a(x15), .b(x14), .c(x13), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n52_), .c(new_n75_), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x05), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n83_), .c(new_n79_), .O(z6));
  inv1   g57(.a(x17), .O(new_n86_));
  nand3  g58(.a(new_n35_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n32_), .c(new_n29_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(x11), .O(new_n89_));
  nand3  g61(.a(x16), .b(x15), .c(x14), .O(new_n90_));
  inv1   g62(.a(new_n90_), .O(new_n91_));
  aoi21  g63(.a(new_n91_), .b(new_n60_), .c(new_n86_), .O(new_n92_));
  nand4  g64(.a(new_n92_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n93_));
  nand2  g65(.a(x08), .b(x06), .O(new_n94_));
  nand3  g66(.a(new_n94_), .b(new_n93_), .c(new_n89_), .O(z7));
  nand2  g67(.a(new_n57_), .b(x07), .O(new_n96_));
  nand4  g68(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n59_), .c(x18), .O(new_n98_));
  nor2   g70(.a(new_n75_), .b(new_n34_), .O(new_n99_));
  nor2   g71(.a(x18), .b(new_n86_), .O(new_n100_));
  nand4  g72(.a(new_n100_), .b(new_n99_), .c(new_n76_), .d(new_n52_), .O(new_n101_));
  nand2  g73(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nand4  g74(.a(new_n102_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n103_));
  nand2  g75(.a(new_n103_), .b(new_n96_), .O(z8));
endmodule


