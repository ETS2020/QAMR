// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  nand2  g04(.a(x12), .b(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  inv1   g06(.a(x15), .O(new_n35_));
  inv1   g07(.a(x16), .O(new_n36_));
  inv1   g08(.a(x17), .O(new_n37_));
  nand2  g09(.a(x14), .b(x13), .O(new_n38_));
  nor4   g10(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n34_), .c(new_n32_), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g13(.a(new_n30_), .b(new_n29_), .O(new_n42_));
  inv1   g14(.a(new_n42_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x00), .O(new_n44_));
  inv1   g16(.a(x11), .O(new_n45_));
  nand3  g17(.a(new_n32_), .b(new_n45_), .c(new_n29_), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(z1));
  inv1   g19(.a(x08), .O(new_n48_));
  xor2a  g20(.a(x12), .b(x11), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n29_), .c(x09), .d(new_n48_), .O(new_n50_));
  nand2  g22(.a(x08), .b(x01), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n50_), .c(new_n43_), .O(z2));
  xnor2a g24(.a(x13), .b(x12), .O(new_n53_));
  nand2  g25(.a(x13), .b(new_n45_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  nand4  g27(.a(new_n55_), .b(new_n29_), .c(x09), .d(new_n48_), .O(new_n56_));
  aoi21  g28(.a(x08), .b(x02), .c(new_n42_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n56_), .O(z3));
  nor2   g30(.a(new_n42_), .b(new_n48_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x03), .O(new_n60_));
  nand3  g32(.a(x13), .b(x12), .c(x11), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x14), .O(new_n62_));
  inv1   g34(.a(x14), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x13), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n33_), .c(new_n62_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n29_), .c(x09), .d(new_n48_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n60_), .O(z4));
  nand2  g39(.a(new_n59_), .b(x04), .O(new_n68_));
  nand2  g40(.a(new_n61_), .b(x15), .O(new_n69_));
  nand4  g41(.a(new_n35_), .b(x13), .c(x12), .d(x11), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x14), .O(new_n72_));
  nand2  g44(.a(x15), .b(new_n63_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(new_n29_), .c(x09), .d(new_n48_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n68_), .O(z5));
  nand3  g48(.a(x15), .b(x14), .c(x13), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n33_), .c(x16), .O(new_n78_));
  nand3  g50(.a(new_n36_), .b(x15), .c(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n61_), .c(new_n78_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n29_), .c(x09), .d(new_n48_), .O(new_n81_));
  aoi21  g53(.a(x08), .b(x05), .c(new_n42_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(z6));
  nand2  g55(.a(new_n59_), .b(x06), .O(new_n84_));
  nand3  g56(.a(x16), .b(x15), .c(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n61_), .c(x17), .O(new_n86_));
  nor2   g58(.a(new_n35_), .b(new_n63_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n37_), .c(x16), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n61_), .c(new_n86_), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(new_n29_), .c(x09), .d(new_n48_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n84_), .O(z7));
  nand4  g63(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n61_), .c(x18), .O(new_n93_));
  nand4  g65(.a(x13), .b(x12), .c(x11), .d(new_n29_), .O(new_n94_));
  nand4  g66(.a(new_n87_), .b(new_n30_), .c(x17), .d(x16), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  nand3  g68(.a(new_n96_), .b(x09), .c(new_n48_), .O(new_n97_));
  aoi21  g69(.a(x08), .b(x07), .c(new_n42_), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(new_n97_), .O(z8));
endmodule


