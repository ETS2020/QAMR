// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x12), .O(new_n30_));
  inv1   g02(.a(x00), .O(new_n31_));
  nand2  g03(.a(x05), .b(new_n31_), .O(new_n32_));
  nand4  g04(.a(new_n32_), .b(x18), .c(x17), .d(x16), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x15), .c(x14), .d(x13), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(x11), .c(new_n29_), .d(x09), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(x08), .O(z0));
  nor2   g10(.a(x11), .b(x10), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(x09), .c(x08), .O(new_n40_));
  inv1   g12(.a(new_n32_), .O(new_n41_));
  inv1   g13(.a(x09), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(x08), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(new_n41_), .O(new_n44_));
  oai21  g16(.a(new_n40_), .b(new_n31_), .c(new_n44_), .O(z1));
  inv1   g17(.a(x08), .O(new_n46_));
  xor2a  g18(.a(x12), .b(x11), .O(new_n47_));
  nand4  g19(.a(new_n47_), .b(new_n29_), .c(x09), .d(new_n46_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x01), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n48_), .c(new_n41_), .O(z2));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(x12), .b(x11), .O(new_n52_));
  inv1   g24(.a(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n51_), .b(x12), .c(x11), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n51_), .c(new_n54_), .O(new_n55_));
  nand4  g27(.a(new_n55_), .b(new_n29_), .c(x09), .d(new_n46_), .O(new_n56_));
  aoi21  g28(.a(x08), .b(x02), .c(new_n41_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n56_), .O(z3));
  nand3  g30(.a(x13), .b(x12), .c(x11), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x14), .O(new_n60_));
  inv1   g32(.a(x14), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x13), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n52_), .c(new_n60_), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(new_n29_), .c(x09), .d(new_n46_), .O(new_n64_));
  aoi21  g36(.a(x08), .b(x03), .c(new_n41_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z4));
  inv1   g38(.a(x15), .O(new_n67_));
  nand3  g39(.a(new_n53_), .b(x14), .c(x13), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n67_), .b(x14), .c(x13), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n52_), .c(new_n69_), .d(new_n67_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n29_), .c(x09), .d(new_n46_), .O(new_n72_));
  aoi21  g44(.a(x08), .b(x04), .c(new_n41_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(z5));
  oai21  g46(.a(x08), .b(new_n31_), .c(x05), .O(new_n75_));
  nor2   g47(.a(new_n67_), .b(new_n61_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n53_), .c(x13), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(x16), .O(new_n78_));
  inv1   g50(.a(x16), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(x15), .c(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n59_), .c(new_n78_), .O(new_n81_));
  nand4  g53(.a(new_n81_), .b(new_n29_), .c(x09), .d(new_n46_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n75_), .O(z6));
  nor2   g55(.a(new_n79_), .b(new_n67_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n59_), .c(x17), .O(new_n86_));
  nor2   g58(.a(x17), .b(new_n79_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n76_), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n59_), .c(new_n86_), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(new_n29_), .c(x09), .d(new_n46_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x06), .O(new_n91_));
  aoi21  g63(.a(new_n91_), .b(new_n90_), .c(new_n41_), .O(z7));
  nand3  g64(.a(new_n76_), .b(x17), .c(x16), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n59_), .c(x18), .O(new_n94_));
  inv1   g66(.a(x18), .O(new_n95_));
  nand4  g67(.a(new_n84_), .b(new_n69_), .c(new_n95_), .d(x17), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand4  g69(.a(new_n97_), .b(new_n29_), .c(x09), .d(new_n46_), .O(new_n98_));
  aoi21  g70(.a(x08), .b(x07), .c(new_n41_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(new_n98_), .O(z8));
endmodule


