// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x09), .O(new_n29_));
  inv1   g01(.a(x17), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  inv1   g03(.a(x08), .O(new_n32_));
  nor2   g04(.a(x11), .b(x10), .O(new_n33_));
  aoi21  g05(.a(new_n33_), .b(new_n32_), .c(x17), .O(new_n34_));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  oai21  g07(.a(new_n34_), .b(new_n29_), .c(new_n35_), .O(z1));
  inv1   g08(.a(x10), .O(new_n37_));
  xor2a  g09(.a(x12), .b(x11), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(new_n37_), .c(new_n32_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n30_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x09), .O(new_n41_));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(z2));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x13), .O(new_n45_));
  inv1   g17(.a(x13), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(x12), .c(x11), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n45_), .c(x10), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n32_), .c(x17), .O(new_n49_));
  nand2  g21(.a(x08), .b(x02), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n29_), .c(new_n50_), .O(z3));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(x14), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(x13), .c(x12), .d(x11), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n53_), .c(x10), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n32_), .c(x17), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  oai21  g30(.a(new_n57_), .b(new_n29_), .c(new_n58_), .O(z4));
  nor2   g31(.a(z0), .b(new_n32_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x04), .O(new_n61_));
  nand2  g33(.a(x14), .b(x13), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n44_), .c(x15), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(x14), .c(x13), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n44_), .c(new_n63_), .O(new_n66_));
  nand4  g38(.a(new_n66_), .b(new_n30_), .c(new_n37_), .d(x09), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(x08), .c(new_n61_), .O(z5));
  nand2  g40(.a(new_n60_), .b(x05), .O(new_n69_));
  nand3  g41(.a(x15), .b(x14), .c(x13), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n44_), .c(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(x15), .c(x14), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n52_), .c(new_n71_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(new_n30_), .c(new_n37_), .d(x09), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(x08), .c(new_n69_), .O(z6));
  nand4  g48(.a(x12), .b(x11), .c(new_n37_), .d(new_n32_), .O(new_n77_));
  nand4  g49(.a(x16), .b(x15), .c(x14), .d(x13), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n77_), .c(new_n30_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(x09), .O(new_n80_));
  nand2  g52(.a(x08), .b(x06), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(z7));
  nand2  g54(.a(new_n60_), .b(x07), .O(new_n83_));
  nand2  g55(.a(x09), .b(new_n32_), .O(new_n84_));
  nand3  g56(.a(x18), .b(new_n30_), .c(new_n37_), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z8));
endmodule


