// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  nand2  g10(.a(x16), .b(x08), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  inv1   g12(.a(x09), .O(new_n41_));
  nor2   g13(.a(new_n41_), .b(x08), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n40_), .c(new_n31_), .O(new_n43_));
  oai21  g15(.a(new_n39_), .b(new_n38_), .c(new_n43_), .O(z1));
  inv1   g16(.a(x01), .O(new_n45_));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n47_));
  oai21  g19(.a(new_n39_), .b(new_n45_), .c(new_n47_), .O(z2));
  inv1   g20(.a(x02), .O(new_n49_));
  xnor2a g21(.a(x13), .b(x12), .O(new_n50_));
  nand2  g22(.a(x13), .b(new_n40_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n40_), .c(new_n51_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n53_));
  oai21  g25(.a(new_n39_), .b(new_n49_), .c(new_n53_), .O(z3));
  inv1   g26(.a(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nand2  g30(.a(x12), .b(x11), .O(new_n59_));
  nand2  g31(.a(new_n56_), .b(x13), .O(new_n60_));
  oai22  g32(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n56_), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n62_));
  oai21  g34(.a(new_n39_), .b(new_n55_), .c(new_n62_), .O(z4));
  inv1   g35(.a(x16), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(x04), .c(x08), .O(new_n65_));
  nand2  g37(.a(new_n57_), .b(x15), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x13), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n59_), .c(new_n66_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x14), .O(new_n70_));
  nand2  g42(.a(x15), .b(new_n56_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n65_), .O(z5));
  inv1   g46(.a(x05), .O(new_n75_));
  nand3  g47(.a(x15), .b(x14), .c(x13), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n59_), .c(x16), .O(new_n77_));
  nand3  g49(.a(new_n64_), .b(x15), .c(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n57_), .c(new_n77_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n80_));
  oai21  g52(.a(new_n39_), .b(new_n75_), .c(new_n80_), .O(z6));
  and2   g53(.a(x08), .b(x06), .O(new_n82_));
  nand3  g54(.a(new_n42_), .b(x11), .c(new_n31_), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  nand2  g56(.a(x13), .b(x12), .O(new_n85_));
  nor4   g57(.a(new_n85_), .b(x17), .c(new_n67_), .d(new_n56_), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n84_), .c(new_n82_), .O(new_n87_));
  inv1   g59(.a(x17), .O(new_n88_));
  nand3  g60(.a(x16), .b(x15), .c(x14), .O(new_n89_));
  inv1   g61(.a(new_n89_), .O(new_n90_));
  aoi21  g62(.a(new_n90_), .b(new_n58_), .c(new_n88_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n92_));
  oai21  g64(.a(new_n87_), .b(new_n64_), .c(new_n92_), .O(z7));
  oai21  g65(.a(new_n64_), .b(x07), .c(x08), .O(new_n94_));
  nand3  g66(.a(x17), .b(x15), .c(x14), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n57_), .c(x18), .O(new_n96_));
  nor2   g68(.a(new_n64_), .b(new_n67_), .O(new_n97_));
  nand4  g69(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n98_));
  inv1   g70(.a(new_n98_), .O(new_n99_));
  nor2   g71(.a(x18), .b(new_n88_), .O(new_n100_));
  nand3  g72(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(new_n101_));
  aoi21  g73(.a(new_n101_), .b(new_n96_), .c(x08), .O(new_n102_));
  nor2   g74(.a(new_n29_), .b(x16), .O(new_n103_));
  oai21  g75(.a(new_n103_), .b(new_n102_), .c(new_n31_), .O(new_n104_));
  oai21  g76(.a(new_n104_), .b(new_n41_), .c(new_n94_), .O(z8));
endmodule


