// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x09), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nor3   g02(.a(new_n30_), .b(new_n29_), .c(x08), .O(new_n31_));
  inv1   g03(.a(x14), .O(new_n32_));
  inv1   g04(.a(x15), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  inv1   g06(.a(x17), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n34_), .c(new_n31_), .d(x16), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(x02), .c(x10), .O(z0));
  inv1   g11(.a(x02), .O(new_n40_));
  inv1   g12(.a(x08), .O(new_n41_));
  inv1   g13(.a(x10), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n40_), .c(new_n41_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(x00), .O(new_n44_));
  nand2  g16(.a(new_n41_), .b(x02), .O(new_n45_));
  inv1   g17(.a(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n42_), .c(x09), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(z1));
  xor2a  g20(.a(x12), .b(x11), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(x09), .c(new_n41_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x02), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n42_), .O(new_n52_));
  nand2  g24(.a(x08), .b(x01), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z2));
  inv1   g26(.a(x12), .O(new_n55_));
  nand3  g27(.a(x13), .b(new_n55_), .c(new_n41_), .O(new_n56_));
  inv1   g28(.a(x13), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x12), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n56_), .c(new_n46_), .O(new_n59_));
  nor2   g31(.a(new_n57_), .b(x11), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n59_), .c(x09), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x02), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n42_), .O(new_n63_));
  nand2  g35(.a(x08), .b(x02), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z3));
  nand2  g37(.a(new_n43_), .b(x03), .O(new_n66_));
  nand2  g38(.a(new_n30_), .b(x14), .O(new_n67_));
  nand2  g39(.a(x12), .b(x11), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n32_), .c(x13), .O(new_n70_));
  aoi21  g42(.a(new_n70_), .b(new_n67_), .c(x10), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(x09), .c(new_n41_), .d(x02), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n66_), .O(z4));
  nand4  g45(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(x15), .O(new_n75_));
  nand4  g47(.a(new_n69_), .b(new_n33_), .c(x14), .d(x13), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n75_), .c(new_n29_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n41_), .c(new_n40_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x04), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(x10), .c(new_n79_), .O(z5));
  nand3  g52(.a(x15), .b(x14), .c(x13), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n68_), .c(x16), .O(new_n82_));
  inv1   g54(.a(x16), .O(new_n83_));
  inv1   g55(.a(new_n30_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n83_), .c(x15), .d(x14), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n82_), .c(new_n29_), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n41_), .c(new_n40_), .O(new_n87_));
  nand2  g59(.a(x08), .b(x05), .O(new_n88_));
  oai21  g60(.a(new_n87_), .b(x10), .c(new_n88_), .O(z6));
  nand2  g61(.a(new_n43_), .b(x06), .O(new_n90_));
  nand3  g62(.a(x16), .b(x15), .c(x14), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n30_), .c(x17), .O(new_n92_));
  nand3  g64(.a(new_n34_), .b(new_n35_), .c(x16), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n30_), .c(new_n92_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n42_), .c(x09), .d(new_n41_), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n40_), .c(new_n90_), .O(z7));
  nand2  g68(.a(new_n43_), .b(x07), .O(new_n97_));
  nand4  g69(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n98_));
  oai21  g70(.a(new_n98_), .b(new_n30_), .c(x18), .O(new_n99_));
  inv1   g71(.a(new_n74_), .O(new_n100_));
  nor2   g72(.a(new_n83_), .b(new_n33_), .O(new_n101_));
  nand4  g73(.a(new_n101_), .b(new_n100_), .c(new_n36_), .d(x17), .O(new_n102_));
  aoi21  g74(.a(new_n102_), .b(new_n99_), .c(x10), .O(new_n103_));
  nand4  g75(.a(new_n103_), .b(x09), .c(new_n41_), .d(x02), .O(new_n104_));
  nand2  g76(.a(new_n104_), .b(new_n97_), .O(z8));
endmodule


