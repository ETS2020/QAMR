// Benchmark "FAU" written by ABC on Fri Aug 14 01:58:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  inv1   g04(.a(x12), .O(new_n33_));
  inv1   g05(.a(x13), .O(new_n34_));
  nand3  g06(.a(x17), .b(x16), .c(x14), .O(new_n35_));
  nor3   g07(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n32_), .c(x11), .d(new_n30_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x15), .c(new_n29_), .O(z0));
  nor2   g10(.a(new_n29_), .b(x15), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(new_n32_), .b(new_n42_), .c(new_n30_), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  inv1   g16(.a(x08), .O(new_n45_));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n30_), .c(x09), .d(new_n45_), .O(new_n47_));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n47_), .c(new_n40_), .O(z2));
  nand2  g21(.a(x12), .b(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x13), .O(new_n51_));
  nand3  g23(.a(new_n34_), .b(x12), .c(x11), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand4  g25(.a(new_n53_), .b(new_n30_), .c(x09), .d(new_n45_), .O(new_n54_));
  nand2  g26(.a(x08), .b(x02), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n54_), .c(new_n39_), .O(z3));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x13), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n50_), .c(new_n58_), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n30_), .c(x09), .d(new_n45_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x03), .c(new_n39_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z4));
  nor2   g36(.a(new_n39_), .b(new_n45_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(x04), .O(new_n66_));
  nand4  g38(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x15), .O(new_n68_));
  inv1   g40(.a(x15), .O(new_n69_));
  nand3  g41(.a(new_n29_), .b(new_n69_), .c(x14), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n57_), .c(new_n68_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n30_), .c(x09), .d(new_n45_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n66_), .O(z5));
  nand2  g45(.a(new_n65_), .b(x05), .O(new_n74_));
  nand2  g46(.a(new_n67_), .b(x16), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x14), .c(x13), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n50_), .c(new_n75_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(x15), .O(new_n79_));
  nand3  g51(.a(new_n29_), .b(x16), .c(new_n69_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g53(.a(new_n81_), .b(new_n30_), .c(x09), .d(new_n45_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n74_), .O(z6));
  nand3  g55(.a(new_n32_), .b(x17), .c(new_n30_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n29_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  nand3  g58(.a(x16), .b(x14), .c(x13), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n50_), .c(x17), .O(new_n88_));
  inv1   g60(.a(x17), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(x16), .c(x15), .d(x14), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n57_), .c(new_n88_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(new_n30_), .c(x09), .d(new_n45_), .O(new_n92_));
  nand2  g64(.a(x08), .b(x06), .O(new_n93_));
  nand3  g65(.a(new_n93_), .b(new_n92_), .c(new_n86_), .O(z7));
  nand2  g66(.a(new_n65_), .b(x07), .O(new_n95_));
  oai21  g67(.a(new_n57_), .b(new_n35_), .c(x18), .O(new_n96_));
  nand4  g68(.a(new_n29_), .b(x17), .c(x16), .d(x14), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n57_), .c(new_n96_), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(x15), .c(new_n30_), .d(x09), .O(new_n99_));
  oai21  g71(.a(new_n99_), .b(x08), .c(new_n95_), .O(z8));
endmodule


