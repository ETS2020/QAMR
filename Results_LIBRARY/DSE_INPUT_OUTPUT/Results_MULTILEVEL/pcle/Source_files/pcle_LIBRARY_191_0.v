// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x08), .O(new_n29_));
  nand2  g01(.a(x12), .b(x11), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  inv1   g04(.a(x17), .O(new_n33_));
  inv1   g05(.a(x18), .O(new_n34_));
  nand2  g06(.a(x15), .b(x14), .O(new_n35_));
  nor4   g07(.a(new_n35_), .b(new_n34_), .c(new_n33_), .d(new_n32_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n31_), .c(x09), .d(new_n29_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x13), .c(x10), .O(z0));
  inv1   g10(.a(x10), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand3  g12(.a(new_n40_), .b(x09), .c(new_n29_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x13), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x00), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(z1));
  inv1   g17(.a(x01), .O(new_n46_));
  oai21  g18(.a(x13), .b(x10), .c(x08), .O(new_n47_));
  xor2a  g19(.a(x12), .b(x11), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(x13), .c(new_n39_), .d(x09), .O(new_n49_));
  oai22  g21(.a(new_n49_), .b(x08), .c(new_n47_), .d(new_n46_), .O(z2));
  inv1   g22(.a(x02), .O(new_n51_));
  nand2  g23(.a(x12), .b(x11), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(x13), .c(new_n39_), .d(x09), .O(new_n53_));
  oai22  g25(.a(new_n53_), .b(x08), .c(new_n47_), .d(new_n51_), .O(z3));
  inv1   g26(.a(x13), .O(new_n55_));
  inv1   g27(.a(x09), .O(new_n56_));
  nand2  g28(.a(new_n30_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(x12), .c(x11), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n29_), .c(new_n55_), .O(new_n61_));
  nand2  g33(.a(x08), .b(x03), .O(new_n62_));
  oai21  g34(.a(new_n61_), .b(x10), .c(new_n62_), .O(z4));
  nand2  g35(.a(new_n30_), .b(x15), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(x12), .c(x11), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n67_));
  nor2   g39(.a(new_n65_), .b(x14), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n67_), .c(x09), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(x08), .c(x13), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n39_), .O(new_n71_));
  nand2  g43(.a(x08), .b(x04), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(z5));
  inv1   g45(.a(x05), .O(new_n74_));
  oai21  g46(.a(new_n35_), .b(new_n30_), .c(x16), .O(new_n75_));
  nand3  g47(.a(new_n32_), .b(x15), .c(x14), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n30_), .c(new_n75_), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(x13), .c(new_n39_), .d(x09), .O(new_n78_));
  oai22  g50(.a(new_n78_), .b(x08), .c(new_n47_), .d(new_n74_), .O(z6));
  nand3  g51(.a(x16), .b(x15), .c(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n30_), .c(x17), .O(new_n81_));
  nand3  g53(.a(x14), .b(x12), .c(x11), .O(new_n82_));
  inv1   g54(.a(new_n82_), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n33_), .c(x16), .d(x15), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n81_), .c(new_n56_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n29_), .c(new_n55_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x06), .O(new_n87_));
  oai21  g59(.a(new_n86_), .b(x10), .c(new_n87_), .O(z7));
  nand3  g60(.a(x17), .b(x16), .c(x15), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n82_), .c(x18), .O(new_n90_));
  nor2   g62(.a(new_n32_), .b(new_n65_), .O(new_n91_));
  nand4  g63(.a(new_n83_), .b(new_n91_), .c(new_n34_), .d(x17), .O(new_n92_));
  aoi21  g64(.a(new_n92_), .b(new_n90_), .c(new_n56_), .O(new_n93_));
  aoi21  g65(.a(new_n93_), .b(new_n29_), .c(new_n55_), .O(new_n94_));
  nand2  g66(.a(x08), .b(x07), .O(new_n95_));
  oai21  g67(.a(new_n94_), .b(x10), .c(new_n95_), .O(z8));
endmodule


