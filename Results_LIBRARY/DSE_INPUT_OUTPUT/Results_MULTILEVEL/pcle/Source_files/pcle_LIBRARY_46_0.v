// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_;
  nor2   g00(.a(x17), .b(x09), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand2  g03(.a(x12), .b(x11), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(x10), .O(new_n33_));
  nand3  g05(.a(x15), .b(x14), .c(x13), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n33_), .c(x09), .d(new_n31_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n30_), .O(z0));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x10), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand4  g13(.a(new_n41_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n39_), .c(new_n30_), .O(z1));
  xor2a  g15(.a(x12), .b(x11), .O(new_n44_));
  nand4  g16(.a(new_n44_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n29_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z2));
  xnor2a g19(.a(x13), .b(x12), .O(new_n48_));
  nand2  g20(.a(x13), .b(new_n41_), .O(new_n49_));
  oai21  g21(.a(new_n48_), .b(new_n41_), .c(new_n49_), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n29_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(z3));
  inv1   g25(.a(x14), .O(new_n54_));
  nand3  g26(.a(x13), .b(x12), .c(x11), .O(new_n55_));
  inv1   g27(.a(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n54_), .b(x13), .O(new_n57_));
  oai22  g29(.a(new_n57_), .b(new_n32_), .c(new_n56_), .d(new_n54_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n29_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z4));
  nor2   g33(.a(new_n29_), .b(new_n31_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x04), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand4  g36(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n64_), .b(x14), .c(x13), .O(new_n67_));
  oai22  g39(.a(new_n67_), .b(new_n32_), .c(new_n66_), .d(new_n64_), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n63_), .O(z5));
  nand2  g42(.a(new_n62_), .b(x05), .O(new_n71_));
  oai21  g43(.a(new_n34_), .b(new_n32_), .c(x16), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(x15), .c(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n55_), .c(new_n72_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n71_), .O(z6));
  inv1   g49(.a(x17), .O(new_n78_));
  nand4  g50(.a(x12), .b(x11), .c(new_n40_), .d(new_n31_), .O(new_n79_));
  nand4  g51(.a(x16), .b(x15), .c(x14), .d(x13), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n79_), .c(x09), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand3  g54(.a(x16), .b(x15), .c(x14), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n56_), .c(new_n78_), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x06), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n82_), .O(z7));
  nand2  g60(.a(new_n62_), .b(x07), .O(new_n89_));
  nand4  g61(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n55_), .c(x18), .O(new_n91_));
  nor2   g63(.a(new_n73_), .b(new_n64_), .O(new_n92_));
  nor2   g64(.a(x18), .b(new_n78_), .O(new_n93_));
  nand3  g65(.a(new_n93_), .b(new_n92_), .c(new_n66_), .O(new_n94_));
  nand2  g66(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(new_n40_), .c(x09), .d(new_n31_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n89_), .O(z8));
endmodule


