// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x09), .O(new_n30_));
  nand2  g02(.a(x12), .b(x11), .O(new_n31_));
  nor4   g03(.a(new_n31_), .b(x10), .c(new_n30_), .d(x08), .O(new_n32_));
  inv1   g04(.a(x16), .O(new_n33_));
  inv1   g05(.a(x17), .O(new_n34_));
  inv1   g06(.a(x18), .O(new_n35_));
  nand3  g07(.a(x15), .b(x14), .c(x13), .O(new_n36_));
  nor4   g08(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(new_n33_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n32_), .O(new_n38_));
  oai21  g10(.a(x09), .b(new_n29_), .c(new_n38_), .O(z0));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x10), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n41_), .c(new_n29_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n40_), .c(new_n30_), .O(z1));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n41_), .c(new_n29_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n46_), .c(new_n30_), .O(z2));
  xnor2a g20(.a(x13), .b(x12), .O(new_n49_));
  nand2  g21(.a(x13), .b(new_n42_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n42_), .c(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n41_), .c(new_n29_), .O(new_n52_));
  nand2  g24(.a(x08), .b(x02), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n52_), .c(new_n30_), .O(z3));
  oai21  g26(.a(new_n30_), .b(x03), .c(x08), .O(new_n55_));
  nand3  g27(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x13), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n31_), .c(new_n57_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n41_), .c(x09), .d(new_n29_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n55_), .O(z4));
  nand2  g34(.a(new_n56_), .b(x15), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(x13), .c(x12), .d(x11), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x14), .O(new_n67_));
  nand2  g39(.a(x15), .b(new_n58_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n41_), .c(new_n29_), .O(new_n70_));
  nand2  g42(.a(x08), .b(x04), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n70_), .c(new_n30_), .O(z5));
  oai21  g44(.a(new_n30_), .b(x05), .c(x08), .O(new_n73_));
  oai21  g45(.a(new_n36_), .b(new_n31_), .c(x16), .O(new_n74_));
  nand3  g46(.a(new_n33_), .b(x15), .c(x14), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n56_), .c(new_n74_), .O(new_n76_));
  nand4  g48(.a(new_n76_), .b(new_n41_), .c(x09), .d(new_n29_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n73_), .O(z6));
  nand3  g50(.a(x16), .b(x15), .c(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n56_), .c(x17), .O(new_n80_));
  nand4  g52(.a(new_n34_), .b(x16), .c(x15), .d(x14), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n56_), .c(new_n80_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n41_), .c(new_n29_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x06), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n83_), .c(new_n30_), .O(z7));
  nand4  g57(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n56_), .c(x18), .O(new_n87_));
  nand4  g59(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n88_));
  nand4  g60(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n41_), .c(new_n29_), .O(new_n91_));
  nand2  g63(.a(x08), .b(x07), .O(new_n92_));
  aoi21  g64(.a(new_n92_), .b(new_n91_), .c(new_n30_), .O(z8));
endmodule


