// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x15), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand2  g03(.a(x12), .b(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n31_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x13), .O(new_n36_));
  inv1   g08(.a(x14), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g10(.a(x17), .O(new_n39_));
  inv1   g11(.a(x18), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand4  g13(.a(new_n41_), .b(new_n38_), .c(new_n35_), .d(x16), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g15(.a(x15), .b(x10), .O(new_n44_));
  nand2  g16(.a(x08), .b(x00), .O(new_n45_));
  inv1   g17(.a(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(z1));
  nand2  g20(.a(new_n44_), .b(x08), .O(new_n49_));
  inv1   g21(.a(new_n49_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x01), .O(new_n51_));
  xor2a  g23(.a(x12), .b(x11), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n51_), .O(z2));
  nand3  g26(.a(new_n36_), .b(x12), .c(x11), .O(new_n55_));
  oai21  g27(.a(new_n33_), .b(new_n36_), .c(new_n55_), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x02), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n57_), .c(new_n44_), .O(z3));
  nand2  g31(.a(new_n50_), .b(x03), .O(new_n60_));
  nand3  g32(.a(x13), .b(x12), .c(x11), .O(new_n61_));
  inv1   g33(.a(new_n61_), .O(new_n62_));
  nand2  g34(.a(new_n37_), .b(x13), .O(new_n63_));
  oai22  g35(.a(new_n63_), .b(new_n32_), .c(new_n62_), .d(new_n37_), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n60_), .O(z4));
  nand2  g38(.a(new_n50_), .b(x04), .O(new_n67_));
  nand2  g39(.a(new_n61_), .b(x15), .O(new_n68_));
  nand4  g40(.a(new_n30_), .b(x13), .c(x12), .d(x11), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x14), .O(new_n71_));
  nand2  g43(.a(x15), .b(new_n37_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand4  g45(.a(new_n73_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n67_), .O(z5));
  nand2  g47(.a(new_n50_), .b(x05), .O(new_n76_));
  nand3  g48(.a(x15), .b(x14), .c(x13), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n32_), .c(x16), .O(new_n78_));
  inv1   g50(.a(x16), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(x15), .c(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n61_), .c(new_n78_), .O(new_n81_));
  nand4  g53(.a(new_n81_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n76_), .O(z6));
  nand3  g55(.a(new_n38_), .b(new_n39_), .c(x16), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n34_), .c(new_n29_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(x15), .O(new_n86_));
  nand3  g58(.a(x16), .b(x15), .c(x14), .O(new_n87_));
  inv1   g59(.a(new_n87_), .O(new_n88_));
  aoi21  g60(.a(new_n88_), .b(new_n62_), .c(new_n39_), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x06), .O(new_n91_));
  nand3  g63(.a(new_n91_), .b(new_n90_), .c(new_n86_), .O(z7));
  nand4  g64(.a(new_n38_), .b(new_n40_), .c(x17), .d(x16), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n34_), .c(new_n29_), .O(new_n94_));
  nand2  g66(.a(new_n94_), .b(x15), .O(new_n95_));
  nand4  g67(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n96_));
  inv1   g68(.a(new_n96_), .O(new_n97_));
  aoi21  g69(.a(new_n97_), .b(new_n62_), .c(new_n40_), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n99_));
  nand2  g71(.a(x08), .b(x07), .O(new_n100_));
  nand3  g72(.a(new_n100_), .b(new_n99_), .c(new_n95_), .O(z8));
endmodule


