// Benchmark "FAU" written by ABC on Fri Aug 14 01:57:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  inv1   g10(.a(x16), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x08), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(x09), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(x08), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n31_), .O(new_n44_));
  oai21  g16(.a(new_n40_), .b(new_n38_), .c(new_n44_), .O(z1));
  inv1   g17(.a(x01), .O(new_n46_));
  xor2a  g18(.a(x12), .b(x11), .O(new_n47_));
  nand4  g19(.a(new_n47_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n48_));
  oai21  g20(.a(new_n40_), .b(new_n46_), .c(new_n48_), .O(z2));
  inv1   g21(.a(x02), .O(new_n50_));
  xnor2a g22(.a(x13), .b(x12), .O(new_n51_));
  nand2  g23(.a(x13), .b(new_n41_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n41_), .c(new_n52_), .O(new_n53_));
  nand4  g25(.a(new_n53_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n54_));
  oai21  g26(.a(new_n40_), .b(new_n50_), .c(new_n54_), .O(z3));
  inv1   g27(.a(x03), .O(new_n56_));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  nand2  g30(.a(x12), .b(x11), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x13), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n63_));
  oai21  g35(.a(new_n40_), .b(new_n56_), .c(new_n63_), .O(z4));
  oai21  g36(.a(x16), .b(x04), .c(x08), .O(new_n65_));
  inv1   g37(.a(new_n59_), .O(new_n66_));
  inv1   g38(.a(x13), .O(new_n67_));
  nor2   g39(.a(x15), .b(new_n67_), .O(new_n68_));
  aoi22  g40(.a(new_n68_), .b(new_n66_), .c(new_n57_), .d(x15), .O(new_n69_));
  nand2  g41(.a(x15), .b(new_n60_), .O(new_n70_));
  oai21  g42(.a(new_n69_), .b(new_n60_), .c(new_n70_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n65_), .O(z5));
  nand3  g45(.a(new_n43_), .b(x11), .c(new_n31_), .O(new_n74_));
  nand2  g46(.a(x15), .b(x14), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x13), .c(x12), .O(new_n77_));
  nor2   g49(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  aoi21  g50(.a(x08), .b(x05), .c(new_n78_), .O(new_n79_));
  nor3   g51(.a(new_n75_), .b(new_n59_), .c(new_n67_), .O(new_n80_));
  nor2   g52(.a(new_n80_), .b(new_n39_), .O(new_n81_));
  nand4  g53(.a(new_n81_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n82_));
  oai21  g54(.a(new_n79_), .b(x16), .c(new_n82_), .O(z6));
  nand2  g55(.a(x08), .b(x06), .O(new_n84_));
  nand3  g56(.a(new_n43_), .b(x17), .c(new_n31_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n39_), .O(new_n87_));
  inv1   g59(.a(x17), .O(new_n88_));
  nand3  g60(.a(new_n76_), .b(new_n88_), .c(x16), .O(new_n89_));
  oai22  g61(.a(new_n89_), .b(new_n57_), .c(new_n80_), .d(new_n88_), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n87_), .O(z7));
  oai21  g64(.a(x16), .b(x07), .c(x08), .O(new_n93_));
  nand4  g65(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n57_), .c(x18), .O(new_n95_));
  nand3  g67(.a(new_n66_), .b(x14), .c(x13), .O(new_n96_));
  nand4  g68(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n97_));
  oai22  g69(.a(new_n97_), .b(new_n96_), .c(new_n95_), .d(x08), .O(new_n98_));
  nand3  g70(.a(new_n98_), .b(new_n31_), .c(x09), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(new_n93_), .O(z8));
endmodule


