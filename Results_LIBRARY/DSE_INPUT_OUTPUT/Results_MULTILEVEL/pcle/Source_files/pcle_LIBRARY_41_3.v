// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  nand2  g10(.a(x15), .b(x10), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x08), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(x09), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(x08), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n31_), .O(new_n44_));
  oai21  g16(.a(new_n40_), .b(new_n38_), .c(new_n44_), .O(z1));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n47_));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n47_), .c(new_n39_), .O(z2));
  xnor2a g21(.a(x13), .b(x12), .O(new_n50_));
  nand2  g22(.a(x13), .b(new_n41_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n41_), .c(new_n51_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n53_));
  inv1   g25(.a(new_n39_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n54_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n53_), .O(z3));
  inv1   g28(.a(x03), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand3  g30(.a(x13), .b(x12), .c(x11), .O(new_n59_));
  inv1   g31(.a(new_n59_), .O(new_n60_));
  nand2  g32(.a(x12), .b(x11), .O(new_n61_));
  nand2  g33(.a(new_n58_), .b(x13), .O(new_n62_));
  oai22  g34(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n58_), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n64_));
  oai21  g36(.a(new_n40_), .b(new_n57_), .c(new_n64_), .O(z4));
  inv1   g37(.a(new_n61_), .O(new_n66_));
  and2   g38(.a(x14), .b(x13), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(x15), .O(new_n69_));
  nand2  g41(.a(new_n66_), .b(new_n31_), .O(new_n70_));
  inv1   g42(.a(x15), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(x14), .c(x13), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(x09), .c(new_n30_), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x04), .c(new_n54_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z5));
  nand3  g48(.a(x11), .b(x09), .c(new_n30_), .O(new_n77_));
  inv1   g49(.a(x16), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(x14), .c(x13), .d(x12), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n77_), .c(new_n31_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x15), .O(new_n81_));
  nor2   g53(.a(new_n71_), .b(new_n58_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(x13), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n66_), .c(new_n78_), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x05), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n81_), .O(z6));
  inv1   g60(.a(x06), .O(new_n89_));
  nand3  g61(.a(x16), .b(x15), .c(x14), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n59_), .c(x17), .O(new_n91_));
  nor2   g63(.a(x17), .b(new_n78_), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(new_n82_), .c(new_n60_), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n95_));
  oai21  g67(.a(new_n40_), .b(new_n89_), .c(new_n95_), .O(z7));
  nand2  g68(.a(new_n66_), .b(new_n43_), .O(new_n97_));
  nand4  g69(.a(new_n67_), .b(new_n29_), .c(x17), .d(x16), .O(new_n98_));
  oai21  g70(.a(new_n98_), .b(new_n97_), .c(new_n31_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(x15), .O(new_n100_));
  nand4  g72(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n101_));
  inv1   g73(.a(new_n101_), .O(new_n102_));
  aoi21  g74(.a(new_n102_), .b(new_n60_), .c(new_n29_), .O(new_n103_));
  nand4  g75(.a(new_n103_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n104_));
  nand2  g76(.a(x08), .b(x07), .O(new_n105_));
  nand3  g77(.a(new_n105_), .b(new_n104_), .c(new_n100_), .O(z8));
endmodule


