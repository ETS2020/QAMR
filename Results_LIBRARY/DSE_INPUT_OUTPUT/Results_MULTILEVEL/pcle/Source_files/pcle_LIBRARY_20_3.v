// Benchmark "FAU" written by ABC on Fri Aug 14 01:56:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x13), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand4  g03(.a(x12), .b(x11), .c(x09), .d(new_n31_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x14), .O(new_n34_));
  inv1   g06(.a(x15), .O(new_n35_));
  nand3  g07(.a(x18), .b(x17), .c(x16), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g11(.a(x09), .b(new_n31_), .O(new_n40_));
  nor2   g12(.a(new_n30_), .b(new_n29_), .O(new_n41_));
  nor2   g13(.a(new_n41_), .b(new_n31_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x00), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n29_), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n40_), .c(new_n43_), .O(z1));
  xnor2a g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(x13), .b(new_n48_), .O(new_n49_));
  oai22  g21(.a(new_n49_), .b(new_n44_), .c(new_n47_), .d(x10), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(x09), .c(new_n31_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x01), .c(new_n41_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(z2));
  nand3  g25(.a(new_n30_), .b(x12), .c(new_n29_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n49_), .c(new_n44_), .O(new_n55_));
  nor2   g27(.a(new_n30_), .b(x11), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n55_), .c(x09), .O(new_n57_));
  aoi21  g29(.a(x08), .b(x02), .c(new_n41_), .O(new_n58_));
  oai21  g30(.a(new_n57_), .b(x08), .c(new_n58_), .O(z3));
  nand3  g31(.a(new_n34_), .b(x12), .c(x11), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n40_), .c(new_n29_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x13), .O(new_n62_));
  nand3  g34(.a(x13), .b(x12), .c(x11), .O(new_n63_));
  inv1   g35(.a(new_n63_), .O(new_n64_));
  nor2   g36(.a(new_n64_), .b(new_n34_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n66_));
  nand2  g38(.a(x08), .b(x03), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n62_), .O(z4));
  nand2  g40(.a(new_n42_), .b(x04), .O(new_n69_));
  nand2  g41(.a(x12), .b(x11), .O(new_n70_));
  nand4  g42(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n71_));
  inv1   g43(.a(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n35_), .b(x14), .c(x13), .O(new_n73_));
  oai22  g45(.a(new_n73_), .b(new_n70_), .c(new_n72_), .d(new_n35_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n69_), .O(z5));
  nand2  g48(.a(new_n42_), .b(x05), .O(new_n77_));
  nand3  g49(.a(x15), .b(x14), .c(x13), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n70_), .c(x16), .O(new_n79_));
  inv1   g51(.a(x16), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(x15), .c(x14), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n63_), .c(new_n79_), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n77_), .O(z6));
  inv1   g56(.a(x17), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(x16), .c(x15), .d(x14), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n32_), .c(new_n29_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(x13), .O(new_n88_));
  nand3  g60(.a(x16), .b(x15), .c(x14), .O(new_n89_));
  inv1   g61(.a(new_n89_), .O(new_n90_));
  aoi21  g62(.a(new_n90_), .b(new_n64_), .c(new_n85_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n92_));
  nand2  g64(.a(x08), .b(x06), .O(new_n93_));
  nand3  g65(.a(new_n93_), .b(new_n92_), .c(new_n88_), .O(z7));
  nand2  g66(.a(new_n42_), .b(x07), .O(new_n95_));
  nand4  g67(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n96_));
  oai21  g68(.a(new_n96_), .b(new_n63_), .c(x18), .O(new_n97_));
  nor2   g69(.a(new_n80_), .b(new_n35_), .O(new_n98_));
  nor2   g70(.a(x18), .b(new_n85_), .O(new_n99_));
  nand3  g71(.a(new_n99_), .b(new_n98_), .c(new_n72_), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  nand4  g73(.a(new_n101_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n102_));
  nand2  g74(.a(new_n102_), .b(new_n95_), .O(z8));
endmodule


