// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_,
    new_n89_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  inv1   g02(.a(x10), .O(new_n27_));
  nand3  g03(.a(new_n27_), .b(new_n26_), .c(x02), .O(new_n28_));
  oai21  g04(.a(x02), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nand4  g08(.a(new_n32_), .b(new_n27_), .c(x02), .d(x01), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(x03), .O(new_n35_));
  nand2  g11(.a(new_n26_), .b(new_n35_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand4  g13(.a(new_n37_), .b(new_n36_), .c(new_n27_), .d(x02), .O(new_n38_));
  inv1   g14(.a(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n39_), .c(x01), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g17(.a(x08), .O(new_n42_));
  nor2   g18(.a(new_n27_), .b(x03), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(x10), .b(new_n44_), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(new_n46_));
  inv1   g22(.a(x00), .O(new_n47_));
  nand2  g23(.a(new_n45_), .b(new_n47_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n46_), .c(x07), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nor3   g26(.a(new_n50_), .b(x03), .c(x00), .O(new_n51_));
  oai21  g27(.a(new_n51_), .b(new_n49_), .c(new_n25_), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  nand2  g29(.a(new_n42_), .b(new_n53_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n27_), .c(x00), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(x02), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n52_), .O(z2));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  nand2  g34(.a(x10), .b(x02), .O(new_n59_));
  nand2  g35(.a(x12), .b(new_n47_), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(z3));
  nand3  g38(.a(x09), .b(x08), .c(x00), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n53_), .O(new_n64_));
  inv1   g40(.a(x11), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(x07), .O(new_n66_));
  aoi22  g42(.a(new_n66_), .b(new_n64_), .c(x10), .d(x02), .O(z4));
  inv1   g43(.a(x12), .O(new_n68_));
  nor2   g44(.a(x07), .b(x00), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(x13), .c(new_n68_), .d(new_n27_), .O(z5));
  nand2  g46(.a(x08), .b(x00), .O(new_n71_));
  nand2  g47(.a(x09), .b(new_n35_), .O(new_n72_));
  nand2  g48(.a(new_n44_), .b(x03), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n25_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n71_), .c(new_n39_), .O(new_n75_));
  aoi21  g51(.a(x14), .b(x01), .c(x00), .O(new_n76_));
  nor2   g52(.a(x08), .b(new_n47_), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(new_n76_), .c(x02), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n27_), .c(new_n53_), .O(new_n80_));
  inv1   g56(.a(new_n80_), .O(z6));
  nand3  g57(.a(x03), .b(new_n39_), .c(new_n25_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n42_), .O(new_n83_));
  nand4  g59(.a(x09), .b(x03), .c(new_n39_), .d(new_n25_), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n83_), .c(new_n27_), .d(new_n53_), .O(z7));
  nand4  g61(.a(new_n68_), .b(x03), .c(new_n39_), .d(new_n25_), .O(new_n86_));
  oai21  g62(.a(x09), .b(x00), .c(new_n53_), .O(new_n87_));
  aoi21  g63(.a(new_n86_), .b(x09), .c(new_n87_), .O(new_n88_));
  nand2  g64(.a(x10), .b(new_n39_), .O(new_n89_));
  oai21  g65(.a(new_n88_), .b(x10), .c(new_n89_), .O(z8));
endmodule


