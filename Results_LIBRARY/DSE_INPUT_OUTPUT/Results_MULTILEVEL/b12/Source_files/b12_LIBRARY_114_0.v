// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x03), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x02), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n27_), .c(new_n25_), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n28_), .c(x02), .O(new_n32_));
  nor2   g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g09(.a(new_n33_), .b(new_n30_), .c(x11), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(x00), .O(z0));
  nand2  g11(.a(new_n28_), .b(new_n25_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  inv1   g14(.a(x11), .O(new_n39_));
  nor2   g15(.a(new_n25_), .b(x02), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(x01), .c(new_n39_), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n38_), .c(x00), .O(z1));
  oai21  g18(.a(x08), .b(x07), .c(x00), .O(new_n43_));
  nand3  g19(.a(x10), .b(new_n25_), .c(new_n31_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n26_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nand2  g22(.a(x07), .b(new_n25_), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n47_), .c(x00), .O(new_n51_));
  inv1   g27(.a(x08), .O(new_n52_));
  nand4  g28(.a(new_n49_), .b(x09), .c(new_n52_), .d(new_n48_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n51_), .c(new_n31_), .O(new_n55_));
  inv1   g31(.a(x00), .O(new_n56_));
  nand2  g32(.a(new_n39_), .b(new_n56_), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n55_), .c(new_n46_), .O(z2));
  inv1   g34(.a(x12), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(x00), .c(new_n48_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(x11), .O(z3));
  nand3  g37(.a(x09), .b(x08), .c(x00), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n48_), .O(new_n63_));
  oai21  g39(.a(x07), .b(new_n56_), .c(new_n39_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n63_), .O(z4));
  nand3  g41(.a(x13), .b(new_n59_), .c(new_n49_), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(x11), .c(new_n48_), .d(new_n56_), .O(z5));
  nand2  g44(.a(new_n52_), .b(x00), .O(new_n69_));
  nand3  g45(.a(x11), .b(new_n31_), .c(new_n56_), .O(new_n70_));
  xnor2a g46(.a(x09), .b(x03), .O(new_n71_));
  aoi22  g47(.a(new_n71_), .b(new_n26_), .c(new_n70_), .d(new_n69_), .O(new_n72_));
  nand2  g48(.a(x14), .b(x02), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(x11), .c(new_n56_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n69_), .c(new_n31_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n72_), .c(new_n49_), .O(new_n76_));
  nor2   g52(.a(new_n76_), .b(x07), .O(z6));
  nand2  g53(.a(new_n40_), .b(new_n31_), .O(new_n78_));
  nand4  g54(.a(x09), .b(x03), .c(new_n26_), .d(new_n31_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n49_), .c(new_n48_), .O(new_n80_));
  aoi21  g56(.a(new_n78_), .b(new_n52_), .c(new_n80_), .O(new_n81_));
  aoi21  g57(.a(new_n39_), .b(new_n56_), .c(new_n81_), .O(z7));
  nand2  g58(.a(x11), .b(x09), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n56_), .O(new_n84_));
  nand4  g60(.a(new_n59_), .b(x03), .c(new_n26_), .d(new_n31_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(x09), .O(new_n86_));
  nand4  g62(.a(new_n86_), .b(new_n84_), .c(new_n49_), .d(new_n48_), .O(z8));
endmodule


