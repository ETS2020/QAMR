// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  inv1   g11(.a(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n27_), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  inv1   g18(.a(x08), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x00), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n42_), .O(z1));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n46_), .c(x03), .O(new_n48_));
  nand3  g24(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n49_));
  inv1   g25(.a(new_n49_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n48_), .c(new_n25_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n27_), .c(x00), .O(z2));
  inv1   g28(.a(x11), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  oai22  g30(.a(new_n54_), .b(x00), .c(new_n53_), .d(new_n46_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n44_), .O(z3));
  oai21  g32(.a(new_n46_), .b(x00), .c(new_n43_), .O(new_n57_));
  inv1   g33(.a(x09), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n34_), .c(new_n46_), .O(new_n59_));
  nand2  g35(.a(new_n53_), .b(x07), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(z4));
  nand4  g37(.a(x13), .b(new_n54_), .c(new_n47_), .d(new_n46_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n34_), .O(new_n63_));
  oai21  g39(.a(new_n43_), .b(new_n34_), .c(new_n63_), .O(z5));
  nand2  g40(.a(x14), .b(x02), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(x01), .O(new_n66_));
  xor2a  g42(.a(x09), .b(x03), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(x02), .c(new_n25_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n47_), .c(new_n46_), .d(new_n34_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n44_), .O(z6));
  nand4  g47(.a(x03), .b(new_n27_), .c(new_n25_), .d(new_n34_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n43_), .O(new_n73_));
  nor2   g49(.a(x02), .b(x01), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(x09), .c(x03), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n73_), .c(new_n47_), .d(new_n46_), .O(z7));
  oai21  g52(.a(new_n58_), .b(x00), .c(new_n43_), .O(new_n77_));
  nand3  g53(.a(new_n74_), .b(new_n54_), .c(x03), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x09), .O(new_n79_));
  aoi21  g55(.a(new_n58_), .b(new_n34_), .c(x10), .O(new_n80_));
  nand4  g56(.a(new_n80_), .b(new_n79_), .c(new_n77_), .d(new_n46_), .O(z8));
endmodule


