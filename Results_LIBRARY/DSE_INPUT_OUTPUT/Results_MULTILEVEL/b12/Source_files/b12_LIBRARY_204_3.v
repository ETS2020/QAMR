// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_,
    new_n89_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  nand2  g06(.a(x10), .b(x00), .O(new_n31_));
  oai21  g07(.a(new_n30_), .b(x00), .c(new_n31_), .O(z0));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x03), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n25_), .c(x01), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n33_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n31_), .O(z1));
  inv1   g18(.a(x01), .O(new_n43_));
  nand2  g19(.a(x07), .b(new_n34_), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n44_), .c(x00), .O(new_n48_));
  nand2  g24(.a(new_n46_), .b(x09), .O(new_n49_));
  inv1   g25(.a(x08), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  oai22  g27(.a(new_n51_), .b(new_n49_), .c(new_n46_), .d(x03), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n48_), .c(new_n43_), .O(new_n53_));
  aoi21  g29(.a(new_n25_), .b(new_n33_), .c(new_n46_), .O(new_n54_));
  nand2  g30(.a(new_n51_), .b(x00), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(x02), .c(new_n54_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(z2));
  nand2  g33(.a(new_n46_), .b(x00), .O(new_n58_));
  inv1   g34(.a(x12), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n33_), .O(new_n60_));
  aoi22  g36(.a(new_n60_), .b(new_n58_), .c(x11), .d(x07), .O(z3));
  oai21  g37(.a(new_n45_), .b(x00), .c(x10), .O(new_n62_));
  nand3  g38(.a(x09), .b(x08), .c(x00), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n45_), .O(new_n64_));
  inv1   g40(.a(x11), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(x07), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(z4));
  inv1   g43(.a(x13), .O(new_n68_));
  nor2   g44(.a(new_n68_), .b(x12), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n46_), .c(new_n45_), .d(new_n33_), .O(z5));
  oai21  g46(.a(new_n51_), .b(new_n43_), .c(new_n46_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x00), .O(new_n72_));
  aoi21  g48(.a(x14), .b(x02), .c(new_n43_), .O(new_n73_));
  nand2  g49(.a(x09), .b(new_n34_), .O(new_n74_));
  inv1   g50(.a(x09), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x03), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n74_), .c(new_n25_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n43_), .c(new_n73_), .O(new_n78_));
  nand3  g54(.a(new_n77_), .b(new_n50_), .c(new_n43_), .O(new_n79_));
  oai21  g55(.a(new_n78_), .b(x00), .c(new_n79_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n46_), .c(new_n45_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n72_), .O(z6));
  nand3  g58(.a(x03), .b(new_n25_), .c(new_n43_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n50_), .O(new_n84_));
  nand4  g60(.a(x09), .b(x03), .c(new_n25_), .d(new_n43_), .O(new_n85_));
  nand4  g61(.a(new_n85_), .b(new_n84_), .c(new_n46_), .d(new_n45_), .O(z7));
  nand4  g62(.a(new_n59_), .b(x03), .c(new_n25_), .d(new_n43_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(x09), .O(new_n88_));
  aoi21  g64(.a(new_n75_), .b(new_n33_), .c(x10), .O(new_n89_));
  nand3  g65(.a(new_n89_), .b(new_n88_), .c(new_n45_), .O(z8));
endmodule


