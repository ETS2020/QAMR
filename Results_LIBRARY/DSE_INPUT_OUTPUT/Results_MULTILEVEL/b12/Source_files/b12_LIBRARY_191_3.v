// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  nor2   g14(.a(new_n34_), .b(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(x01), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(z1));
  oai21  g17(.a(x08), .b(x07), .c(x00), .O(new_n42_));
  nand3  g18(.a(x10), .b(new_n34_), .c(new_n25_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n27_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g21(.a(x07), .b(new_n34_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n46_), .c(x00), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  nand4  g27(.a(new_n48_), .b(x09), .c(new_n51_), .d(new_n47_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n50_), .c(new_n25_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n45_), .O(z2));
  inv1   g31(.a(x12), .O(new_n56_));
  nand2  g32(.a(x11), .b(x07), .O(new_n57_));
  oai21  g33(.a(new_n56_), .b(x00), .c(new_n57_), .O(new_n58_));
  nand2  g34(.a(x08), .b(x00), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n58_), .O(z3));
  and2   g36(.a(new_n59_), .b(new_n57_), .O(z4));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n56_), .d(new_n48_), .O(z5));
  nand2  g39(.a(x09), .b(new_n34_), .O(new_n64_));
  inv1   g40(.a(x09), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(x03), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n64_), .c(new_n27_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n59_), .c(new_n25_), .O(new_n68_));
  aoi21  g44(.a(x14), .b(x02), .c(x00), .O(new_n69_));
  inv1   g45(.a(x00), .O(new_n70_));
  nor2   g46(.a(x08), .b(new_n70_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n69_), .c(x01), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n48_), .c(new_n47_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(z6));
  oai21  g51(.a(x10), .b(x07), .c(new_n59_), .O(new_n76_));
  nand3  g52(.a(new_n39_), .b(new_n25_), .c(new_n70_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(x08), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x09), .O(new_n79_));
  nand2  g55(.a(new_n39_), .b(new_n25_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n51_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n79_), .c(new_n76_), .O(z7));
  oai21  g58(.a(new_n65_), .b(x00), .c(x08), .O(new_n83_));
  nand4  g59(.a(new_n56_), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(x09), .O(new_n85_));
  aoi21  g61(.a(new_n65_), .b(new_n70_), .c(x10), .O(new_n86_));
  nand4  g62(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n47_), .O(z8));
endmodule


