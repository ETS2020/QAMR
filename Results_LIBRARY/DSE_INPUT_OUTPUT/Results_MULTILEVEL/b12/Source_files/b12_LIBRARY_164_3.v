// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n27_), .c(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g16(.a(x00), .O(new_n41_));
  nor3   g17(.a(x10), .b(x08), .c(x07), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n41_), .c(x02), .O(new_n43_));
  oai21  g19(.a(x10), .b(x07), .c(new_n34_), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n44_), .c(x00), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  nand4  g25(.a(new_n46_), .b(x09), .c(new_n49_), .d(new_n45_), .O(new_n50_));
  inv1   g26(.a(new_n50_), .O(new_n51_));
  oai21  g27(.a(new_n51_), .b(new_n48_), .c(new_n25_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n43_), .O(z2));
  nand2  g29(.a(new_n46_), .b(x00), .O(new_n54_));
  inv1   g30(.a(x12), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n41_), .O(new_n56_));
  aoi22  g32(.a(new_n56_), .b(new_n54_), .c(x11), .d(x07), .O(z3));
  oai21  g33(.a(new_n45_), .b(x00), .c(x10), .O(new_n58_));
  nand3  g34(.a(x09), .b(x08), .c(x00), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n45_), .O(new_n60_));
  inv1   g36(.a(x11), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(x07), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(z4));
  nand4  g39(.a(x13), .b(new_n55_), .c(new_n46_), .d(new_n45_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n41_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n54_), .O(z5));
  nand2  g42(.a(x08), .b(x00), .O(new_n67_));
  nand2  g43(.a(x09), .b(new_n34_), .O(new_n68_));
  inv1   g44(.a(x09), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x03), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n68_), .c(new_n27_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n67_), .c(new_n25_), .O(new_n72_));
  aoi21  g48(.a(x14), .b(x02), .c(x00), .O(new_n73_));
  nor2   g49(.a(x08), .b(new_n41_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n46_), .c(new_n45_), .O(new_n77_));
  inv1   g53(.a(new_n77_), .O(z6));
  nand3  g54(.a(x03), .b(new_n27_), .c(new_n25_), .O(new_n79_));
  nand4  g55(.a(x09), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n45_), .O(new_n81_));
  aoi21  g57(.a(new_n79_), .b(new_n49_), .c(new_n81_), .O(new_n82_));
  nand2  g58(.a(x10), .b(new_n41_), .O(new_n83_));
  oai21  g59(.a(new_n82_), .b(x10), .c(new_n83_), .O(z7));
  nand4  g60(.a(new_n55_), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(x09), .O(new_n86_));
  aoi21  g62(.a(new_n69_), .b(new_n41_), .c(x10), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n86_), .c(new_n45_), .O(z8));
endmodule


