// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(new_n25_), .O(new_n31_));
  aoi22  g07(.a(new_n31_), .b(x01), .c(new_n29_), .d(x03), .O(new_n32_));
  inv1   g08(.a(x10), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x00), .O(new_n34_));
  oai21  g10(.a(new_n32_), .b(x00), .c(new_n34_), .O(z0));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(new_n27_), .b(new_n36_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(x02), .O(new_n39_));
  nor2   g15(.a(new_n36_), .b(x02), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(x01), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n39_), .c(x00), .O(z1));
  inv1   g18(.a(x01), .O(new_n43_));
  nor2   g19(.a(x08), .b(x07), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n36_), .c(new_n43_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(x10), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x00), .O(new_n47_));
  inv1   g23(.a(x00), .O(new_n48_));
  oai21  g24(.a(new_n44_), .b(new_n48_), .c(x02), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  nand2  g26(.a(new_n33_), .b(new_n50_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n36_), .c(new_n48_), .O(new_n52_));
  nand3  g28(.a(new_n33_), .b(x09), .c(new_n50_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n49_), .c(new_n47_), .O(z2));
  nand2  g32(.a(x10), .b(x00), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(new_n57_), .c(x11), .d(x07), .O(z3));
  aoi21  g36(.a(x09), .b(x08), .c(x07), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n33_), .c(x00), .O(new_n62_));
  nor2   g38(.a(x11), .b(new_n50_), .O(new_n63_));
  aoi21  g39(.a(new_n50_), .b(new_n48_), .c(new_n63_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n62_), .O(z4));
  inv1   g41(.a(x13), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(x12), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n33_), .c(new_n50_), .d(new_n48_), .O(z5));
  nand2  g44(.a(x14), .b(x02), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x01), .O(new_n70_));
  xor2a  g46(.a(x09), .b(x03), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(x02), .c(new_n43_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand4  g49(.a(new_n73_), .b(new_n33_), .c(new_n50_), .d(new_n48_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(z6));
  aoi21  g51(.a(new_n40_), .b(new_n43_), .c(x08), .O(new_n76_));
  nand4  g52(.a(x09), .b(x03), .c(new_n25_), .d(new_n43_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n50_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n76_), .c(new_n48_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n33_), .O(z7));
  nand4  g56(.a(new_n58_), .b(new_n33_), .c(x09), .d(new_n50_), .O(new_n81_));
  inv1   g57(.a(new_n81_), .O(new_n82_));
  nand4  g58(.a(new_n82_), .b(new_n40_), .c(new_n43_), .d(new_n48_), .O(z8));
endmodule


