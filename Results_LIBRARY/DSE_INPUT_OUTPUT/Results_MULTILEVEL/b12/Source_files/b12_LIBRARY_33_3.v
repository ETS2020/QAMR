// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_;
  nand2  g00(.a(x14), .b(x08), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x02), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(new_n31_));
  inv1   g07(.a(x01), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n29_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n31_), .c(new_n25_), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(x00), .O(z0));
  inv1   g12(.a(x00), .O(new_n37_));
  nand2  g13(.a(new_n29_), .b(new_n26_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand3  g16(.a(x03), .b(new_n27_), .c(x01), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n37_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n25_), .O(z1));
  nor2   g20(.a(x08), .b(x07), .O(new_n45_));
  nand3  g21(.a(x10), .b(new_n26_), .c(new_n32_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n27_), .O(new_n47_));
  oai21  g23(.a(new_n45_), .b(new_n37_), .c(new_n47_), .O(new_n48_));
  nand2  g24(.a(x07), .b(new_n26_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n49_), .c(x00), .O(new_n53_));
  nand3  g29(.a(new_n45_), .b(new_n51_), .c(x09), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n53_), .c(new_n32_), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n48_), .c(new_n25_), .O(z2));
  inv1   g33(.a(x11), .O(new_n58_));
  inv1   g34(.a(x12), .O(new_n59_));
  oai22  g35(.a(new_n59_), .b(x00), .c(new_n58_), .d(new_n50_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n25_), .O(z3));
  nand3  g37(.a(new_n25_), .b(new_n58_), .c(x07), .O(new_n62_));
  inv1   g38(.a(x08), .O(new_n63_));
  aoi21  g39(.a(x09), .b(x00), .c(x14), .O(new_n64_));
  oai21  g40(.a(new_n64_), .b(new_n63_), .c(new_n50_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n62_), .O(z4));
  inv1   g42(.a(x14), .O(new_n67_));
  oai21  g43(.a(new_n45_), .b(new_n67_), .c(x00), .O(new_n68_));
  nand2  g44(.a(new_n25_), .b(x07), .O(new_n69_));
  nand2  g45(.a(new_n67_), .b(new_n37_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x08), .O(new_n71_));
  nand3  g47(.a(x13), .b(new_n59_), .c(new_n51_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n50_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n69_), .c(new_n68_), .O(z5));
  nand2  g50(.a(new_n63_), .b(new_n32_), .O(new_n75_));
  xnor2a g51(.a(x09), .b(x03), .O(new_n76_));
  aoi22  g52(.a(new_n76_), .b(new_n27_), .c(new_n75_), .d(new_n70_), .O(new_n77_));
  oai21  g53(.a(new_n27_), .b(x00), .c(new_n63_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n70_), .c(new_n32_), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n77_), .c(new_n51_), .O(new_n80_));
  nor2   g56(.a(new_n80_), .b(x07), .O(z6));
  oai21  g57(.a(x10), .b(x07), .c(new_n25_), .O(new_n82_));
  nand4  g58(.a(new_n67_), .b(x03), .c(new_n27_), .d(new_n32_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(x08), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(x09), .O(new_n85_));
  nand3  g61(.a(x03), .b(new_n27_), .c(new_n32_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n63_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n85_), .c(new_n82_), .O(z7));
  inv1   g64(.a(new_n25_), .O(new_n89_));
  nand4  g65(.a(new_n59_), .b(x03), .c(new_n27_), .d(new_n32_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(x09), .O(new_n91_));
  nor2   g67(.a(x09), .b(x00), .O(new_n92_));
  nor3   g68(.a(new_n92_), .b(x10), .c(x07), .O(new_n93_));
  aoi21  g69(.a(new_n93_), .b(new_n91_), .c(new_n89_), .O(z8));
endmodule


