// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  oai21  g03(.a(x05), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n27_), .b(x03), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n26_), .c(x03), .O(new_n32_));
  nor2   g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g09(.a(new_n33_), .b(new_n30_), .c(new_n25_), .O(new_n34_));
  inv1   g10(.a(x05), .O(new_n35_));
  nand2  g11(.a(x08), .b(new_n35_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(z0));
  nand3  g13(.a(new_n36_), .b(new_n26_), .c(x01), .O(new_n38_));
  nand2  g14(.a(x06), .b(x01), .O(new_n39_));
  nor2   g15(.a(x08), .b(x05), .O(new_n40_));
  aoi21  g16(.a(new_n39_), .b(x05), .c(new_n40_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n26_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x03), .O(new_n43_));
  nor2   g19(.a(new_n41_), .b(new_n27_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x02), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n43_), .c(x00), .O(z1));
  inv1   g22(.a(x03), .O(new_n47_));
  nand3  g23(.a(x07), .b(new_n47_), .c(new_n31_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n26_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  nand2  g26(.a(x10), .b(new_n47_), .O(new_n51_));
  inv1   g27(.a(x07), .O(new_n52_));
  inv1   g28(.a(x10), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(x05), .c(new_n31_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n25_), .O(new_n58_));
  inv1   g34(.a(x08), .O(new_n59_));
  nor2   g35(.a(new_n53_), .b(x03), .O(new_n60_));
  inv1   g36(.a(x09), .O(new_n61_));
  nor2   g37(.a(x10), .b(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n60_), .c(new_n31_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n26_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n59_), .c(new_n52_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n58_), .O(z2));
  inv1   g42(.a(x12), .O(new_n67_));
  nand2  g43(.a(x11), .b(x07), .O(new_n68_));
  oai21  g44(.a(new_n67_), .b(x00), .c(new_n68_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n36_), .O(z3));
  nand2  g46(.a(new_n68_), .b(new_n59_), .O(new_n71_));
  inv1   g47(.a(x11), .O(new_n72_));
  aoi21  g48(.a(x09), .b(x00), .c(x07), .O(new_n73_));
  aoi21  g49(.a(new_n72_), .b(x07), .c(new_n73_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n35_), .c(new_n71_), .O(z4));
  inv1   g51(.a(new_n36_), .O(new_n76_));
  nor3   g52(.a(new_n76_), .b(x07), .c(x00), .O(new_n77_));
  nand4  g53(.a(new_n77_), .b(x13), .c(new_n67_), .d(new_n53_), .O(z5));
  nand2  g54(.a(x08), .b(x00), .O(new_n79_));
  nand2  g55(.a(x09), .b(new_n47_), .O(new_n80_));
  nand2  g56(.a(new_n61_), .b(x03), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n80_), .c(new_n26_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n79_), .c(new_n31_), .O(new_n83_));
  aoi21  g59(.a(x14), .b(x02), .c(x00), .O(new_n84_));
  nor2   g60(.a(x08), .b(new_n25_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n84_), .c(x01), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n53_), .c(new_n52_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(new_n36_), .O(z6));
  nand3  g65(.a(x03), .b(new_n26_), .c(new_n31_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(x08), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(x09), .O(new_n92_));
  nand2  g68(.a(new_n90_), .b(new_n59_), .O(new_n93_));
  nor2   g69(.a(new_n76_), .b(x10), .O(new_n94_));
  nand4  g70(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(new_n52_), .O(z7));
  nand4  g71(.a(new_n67_), .b(x03), .c(new_n26_), .d(new_n31_), .O(new_n96_));
  nand2  g72(.a(new_n96_), .b(x09), .O(new_n97_));
  nor2   g73(.a(x09), .b(x00), .O(new_n98_));
  nor3   g74(.a(new_n98_), .b(x10), .c(x07), .O(new_n99_));
  aoi21  g75(.a(new_n99_), .b(new_n97_), .c(new_n76_), .O(z8));
endmodule


