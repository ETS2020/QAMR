// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(x10), .b(new_n28_), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(new_n25_), .c(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(x10), .c(x02), .d(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  aoi21  g13(.a(new_n26_), .b(new_n37_), .c(x01), .O(new_n38_));
  aoi21  g14(.a(x06), .b(x05), .c(new_n26_), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand3  g16(.a(new_n29_), .b(x06), .c(x05), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(x03), .c(x01), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n36_), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(x01), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n44_), .O(z1));
  oai21  g23(.a(x08), .b(x07), .c(x00), .O(new_n48_));
  nand3  g24(.a(x10), .b(new_n37_), .c(new_n25_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n28_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand3  g27(.a(x07), .b(new_n37_), .c(new_n25_), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  nand3  g29(.a(new_n45_), .b(x09), .c(new_n53_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n36_), .O(new_n56_));
  inv1   g32(.a(x08), .O(new_n57_));
  nand3  g33(.a(x09), .b(new_n57_), .c(new_n53_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n25_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n45_), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n56_), .c(new_n51_), .O(z2));
  nand2  g37(.a(x11), .b(x07), .O(new_n62_));
  nand2  g38(.a(x12), .b(new_n36_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n46_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(z3));
  nand3  g41(.a(x09), .b(x08), .c(x00), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n53_), .O(new_n67_));
  inv1   g43(.a(x11), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(x07), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n67_), .c(new_n46_), .O(z4));
  inv1   g46(.a(x12), .O(new_n71_));
  nor3   g47(.a(x07), .b(x01), .c(x00), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(x13), .c(new_n71_), .d(new_n45_), .O(z5));
  nand2  g49(.a(x08), .b(x00), .O(new_n74_));
  nand2  g50(.a(x09), .b(new_n37_), .O(new_n75_));
  inv1   g51(.a(x09), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x03), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n75_), .c(new_n28_), .O(new_n78_));
  nand4  g54(.a(new_n78_), .b(new_n74_), .c(new_n45_), .d(new_n53_), .O(new_n79_));
  nor2   g55(.a(new_n79_), .b(x01), .O(z6));
  aoi21  g56(.a(x03), .b(new_n28_), .c(x08), .O(new_n81_));
  nand2  g57(.a(x09), .b(x03), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(x02), .c(new_n53_), .O(new_n83_));
  oai21  g59(.a(new_n83_), .b(new_n81_), .c(new_n25_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n45_), .O(z7));
  nand3  g61(.a(new_n71_), .b(x03), .c(new_n28_), .O(new_n86_));
  oai21  g62(.a(x09), .b(x00), .c(new_n53_), .O(new_n87_));
  aoi21  g63(.a(new_n86_), .b(x09), .c(new_n87_), .O(new_n88_));
  oai21  g64(.a(new_n88_), .b(x01), .c(new_n45_), .O(z8));
endmodule


