// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x09), .O(new_n31_));
  inv1   g07(.a(x10), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g09(.a(new_n30_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  inv1   g11(.a(x01), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(x02), .O(new_n41_));
  nand2  g17(.a(x03), .b(new_n25_), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n36_), .c(new_n41_), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n33_), .c(new_n35_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(z1));
  oai21  g21(.a(x08), .b(x07), .c(x00), .O(new_n46_));
  nor2   g22(.a(x03), .b(x01), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(x02), .c(new_n46_), .O(new_n48_));
  aoi21  g24(.a(x08), .b(x00), .c(x07), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n36_), .c(new_n31_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(x10), .c(new_n48_), .O(z2));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  nand2  g28(.a(x12), .b(new_n35_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n33_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  nor2   g31(.a(x07), .b(x00), .O(new_n56_));
  inv1   g32(.a(x07), .O(new_n57_));
  nor2   g33(.a(x11), .b(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n33_), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  nand3  g36(.a(x09), .b(new_n60_), .c(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  nor2   g38(.a(new_n32_), .b(x09), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n62_), .c(new_n57_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n59_), .O(z4));
  nand2  g41(.a(new_n33_), .b(x07), .O(new_n66_));
  inv1   g42(.a(x13), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(x12), .c(x09), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n32_), .c(x00), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n63_), .c(new_n57_), .O(new_n70_));
  nand2  g46(.a(x09), .b(x00), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n66_), .O(z5));
  nand2  g48(.a(x08), .b(x00), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n42_), .c(new_n36_), .O(new_n74_));
  aoi21  g50(.a(x14), .b(x02), .c(x00), .O(new_n75_));
  aoi21  g51(.a(new_n60_), .b(x00), .c(new_n75_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n36_), .c(new_n74_), .O(new_n77_));
  nand4  g53(.a(new_n77_), .b(new_n32_), .c(x09), .d(new_n57_), .O(new_n78_));
  inv1   g54(.a(new_n78_), .O(z6));
  nor2   g55(.a(x10), .b(new_n31_), .O(new_n80_));
  nand3  g56(.a(x03), .b(new_n25_), .c(new_n36_), .O(new_n81_));
  nand4  g57(.a(new_n81_), .b(new_n80_), .c(x08), .d(new_n57_), .O(z7));
  inv1   g58(.a(x12), .O(new_n83_));
  nor2   g59(.a(x07), .b(new_n37_), .O(new_n84_));
  nor2   g60(.a(x02), .b(x01), .O(new_n85_));
  nand4  g61(.a(new_n85_), .b(new_n84_), .c(new_n80_), .d(new_n83_), .O(z8));
endmodule


