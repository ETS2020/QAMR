// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  or2    g09(.a(x04), .b(x03), .O(new_n34_));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(new_n34_), .c(x02), .O(new_n36_));
  nand3  g12(.a(x03), .b(new_n27_), .c(x01), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n36_), .c(x00), .O(z1));
  inv1   g14(.a(x07), .O(new_n39_));
  inv1   g15(.a(x10), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n39_), .c(x03), .O(new_n41_));
  nand3  g17(.a(new_n40_), .b(x09), .c(new_n39_), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(new_n43_));
  oai21  g19(.a(new_n43_), .b(new_n41_), .c(new_n25_), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n27_), .c(x00), .O(z2));
  inv1   g21(.a(x12), .O(new_n46_));
  nand2  g22(.a(x11), .b(x07), .O(new_n47_));
  oai21  g23(.a(new_n46_), .b(x00), .c(new_n47_), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n48_), .O(z3));
  inv1   g27(.a(x00), .O(new_n52_));
  nand2  g28(.a(new_n47_), .b(new_n52_), .O(new_n53_));
  inv1   g29(.a(x09), .O(new_n54_));
  nor2   g30(.a(x11), .b(new_n39_), .O(new_n55_));
  aoi21  g31(.a(new_n54_), .b(new_n39_), .c(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n49_), .c(new_n53_), .O(z4));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n46_), .d(new_n40_), .O(z5));
  nand2  g35(.a(x14), .b(x02), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(x01), .O(new_n61_));
  xor2a  g37(.a(x09), .b(x03), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(x02), .c(new_n25_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n40_), .c(new_n39_), .d(new_n52_), .O(new_n65_));
  inv1   g41(.a(new_n65_), .O(z6));
  oai21  g42(.a(x10), .b(x07), .c(new_n50_), .O(new_n67_));
  nand4  g43(.a(x08), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n68_));
  oai21  g44(.a(x08), .b(x00), .c(new_n68_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x09), .O(new_n70_));
  nand3  g46(.a(x03), .b(new_n27_), .c(new_n25_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n49_), .c(new_n52_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n67_), .O(z7));
  oai21  g49(.a(new_n54_), .b(x00), .c(new_n49_), .O(new_n74_));
  nand4  g50(.a(new_n46_), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x09), .O(new_n76_));
  aoi21  g52(.a(new_n54_), .b(new_n52_), .c(x10), .O(new_n77_));
  nand4  g53(.a(new_n77_), .b(new_n76_), .c(new_n74_), .d(new_n39_), .O(z8));
endmodule


