// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_;
  inv1   g00(.a(x09), .O(new_n25_));
  nand2  g01(.a(x10), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x02), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n30_), .c(x02), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n32_), .c(new_n26_), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(x00), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  nand2  g14(.a(new_n30_), .b(new_n27_), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(x02), .O(new_n41_));
  nand2  g17(.a(x03), .b(new_n28_), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n33_), .c(new_n41_), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n26_), .c(new_n38_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(z1));
  inv1   g21(.a(x10), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(x07), .c(x09), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  nand3  g24(.a(new_n46_), .b(x09), .c(new_n48_), .O(new_n49_));
  oai21  g25(.a(new_n47_), .b(x03), .c(new_n49_), .O(new_n50_));
  aoi22  g26(.a(new_n50_), .b(new_n33_), .c(new_n26_), .d(x02), .O(new_n51_));
  inv1   g27(.a(x08), .O(new_n52_));
  aoi21  g28(.a(x10), .b(x03), .c(x01), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(x02), .c(x09), .O(new_n54_));
  oai21  g30(.a(x10), .b(new_n28_), .c(new_n54_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n52_), .c(new_n48_), .O(new_n56_));
  oai21  g32(.a(new_n51_), .b(x00), .c(new_n56_), .O(z2));
  inv1   g33(.a(x11), .O(new_n58_));
  inv1   g34(.a(x12), .O(new_n59_));
  oai22  g35(.a(new_n59_), .b(x00), .c(new_n58_), .d(new_n48_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n26_), .O(z3));
  nand2  g37(.a(x09), .b(new_n52_), .O(new_n62_));
  nand2  g38(.a(new_n46_), .b(new_n25_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n62_), .c(new_n38_), .O(new_n64_));
  aoi21  g40(.a(x10), .b(new_n25_), .c(x00), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n64_), .c(new_n48_), .O(new_n66_));
  nand3  g42(.a(new_n26_), .b(new_n58_), .c(x07), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n66_), .O(z4));
  oai21  g44(.a(x10), .b(x07), .c(new_n25_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x00), .O(new_n70_));
  nand2  g46(.a(new_n26_), .b(x07), .O(new_n71_));
  inv1   g47(.a(x13), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(x12), .c(new_n46_), .O(new_n73_));
  oai21  g49(.a(new_n46_), .b(new_n25_), .c(new_n73_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n48_), .c(new_n38_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n71_), .c(new_n70_), .O(z5));
  nand2  g52(.a(x08), .b(x00), .O(new_n77_));
  aoi21  g53(.a(x09), .b(new_n27_), .c(x02), .O(new_n78_));
  nand2  g54(.a(new_n25_), .b(x03), .O(new_n79_));
  oai21  g55(.a(new_n78_), .b(x10), .c(new_n79_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n77_), .c(new_n33_), .O(new_n81_));
  nand2  g57(.a(x14), .b(x02), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n38_), .O(new_n83_));
  oai21  g59(.a(x08), .b(new_n38_), .c(new_n83_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n46_), .c(x01), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n48_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(new_n26_), .O(z6));
  oai21  g64(.a(new_n42_), .b(x01), .c(new_n52_), .O(new_n89_));
  nand4  g65(.a(x09), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n90_));
  nand4  g66(.a(new_n90_), .b(new_n89_), .c(new_n46_), .d(new_n48_), .O(z7));
  nand4  g67(.a(new_n59_), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  aoi21  g69(.a(new_n25_), .b(new_n38_), .c(x10), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n93_), .c(new_n48_), .O(z8));
endmodule


