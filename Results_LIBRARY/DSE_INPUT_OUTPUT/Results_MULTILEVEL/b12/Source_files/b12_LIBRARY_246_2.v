// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x12), .O(new_n31_));
  nor2   g07(.a(x13), .b(new_n31_), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(new_n33_));
  oai21  g09(.a(new_n30_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand3  g16(.a(x03), .b(new_n25_), .c(x01), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n33_), .c(new_n35_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  inv1   g21(.a(x01), .O(new_n46_));
  nand3  g22(.a(x10), .b(new_n36_), .c(new_n46_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n25_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand2  g25(.a(x07), .b(new_n36_), .O(new_n50_));
  inv1   g26(.a(x07), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n50_), .c(x00), .O(new_n54_));
  inv1   g30(.a(x08), .O(new_n55_));
  nand4  g31(.a(new_n52_), .b(x09), .c(new_n55_), .d(new_n51_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n54_), .c(new_n46_), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n49_), .c(new_n32_), .O(z2));
  inv1   g35(.a(x11), .O(new_n60_));
  oai22  g36(.a(new_n31_), .b(x00), .c(new_n60_), .d(new_n51_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n33_), .O(z3));
  nand2  g38(.a(x09), .b(x08), .O(new_n63_));
  oai22  g39(.a(new_n63_), .b(new_n35_), .c(x13), .d(new_n31_), .O(new_n64_));
  nand3  g40(.a(new_n33_), .b(new_n60_), .c(x07), .O(new_n65_));
  oai21  g41(.a(new_n64_), .b(x07), .c(new_n65_), .O(z4));
  oai21  g42(.a(x07), .b(x00), .c(new_n33_), .O(new_n67_));
  inv1   g43(.a(x13), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x10), .c(new_n31_), .O(new_n69_));
  oai21  g45(.a(new_n68_), .b(new_n31_), .c(new_n69_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n51_), .c(new_n35_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n67_), .O(z5));
  nand2  g48(.a(x08), .b(x00), .O(new_n73_));
  nand2  g49(.a(x09), .b(new_n36_), .O(new_n74_));
  inv1   g50(.a(x09), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x03), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n74_), .c(new_n25_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n73_), .c(new_n46_), .O(new_n78_));
  aoi21  g54(.a(x14), .b(x02), .c(x00), .O(new_n79_));
  nor2   g55(.a(x08), .b(new_n35_), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(new_n79_), .c(x01), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n52_), .c(new_n51_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n33_), .O(z6));
  nand3  g60(.a(x03), .b(new_n25_), .c(new_n46_), .O(new_n85_));
  nand4  g61(.a(x09), .b(x03), .c(new_n25_), .d(new_n46_), .O(new_n86_));
  nand3  g62(.a(new_n86_), .b(new_n52_), .c(new_n51_), .O(new_n87_));
  aoi21  g63(.a(new_n85_), .b(new_n55_), .c(new_n87_), .O(new_n88_));
  nor2   g64(.a(new_n88_), .b(new_n32_), .O(z7));
  nor2   g65(.a(new_n32_), .b(x00), .O(new_n90_));
  nor2   g66(.a(x10), .b(x07), .O(new_n91_));
  nor2   g67(.a(new_n91_), .b(new_n68_), .O(new_n92_));
  oai21  g68(.a(new_n92_), .b(new_n90_), .c(new_n75_), .O(new_n93_));
  nand2  g69(.a(new_n85_), .b(x09), .O(new_n94_));
  nand2  g70(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  nand2  g71(.a(new_n95_), .b(new_n31_), .O(new_n96_));
  nand3  g72(.a(x13), .b(x12), .c(x09), .O(new_n97_));
  nand3  g73(.a(new_n97_), .b(new_n96_), .c(new_n93_), .O(z8));
endmodule


