// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_;
  inv1   g00(.a(x10), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(x03), .b(new_n27_), .O(new_n28_));
  nand2  g04(.a(x05), .b(x02), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x03), .c(x02), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(new_n33_));
  oai21  g09(.a(new_n33_), .b(new_n30_), .c(new_n25_), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  inv1   g11(.a(x05), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x04), .O(new_n37_));
  nand4  g13(.a(new_n37_), .b(new_n35_), .c(x02), .d(x01), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n34_), .c(x00), .O(z0));
  nand2  g15(.a(new_n25_), .b(x03), .O(new_n40_));
  oai21  g16(.a(new_n31_), .b(x03), .c(new_n40_), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x01), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(x02), .O(new_n43_));
  nand4  g19(.a(new_n25_), .b(x03), .c(new_n27_), .d(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(x00), .O(z1));
  inv1   g21(.a(x08), .O(new_n46_));
  nor2   g22(.a(new_n25_), .b(x03), .O(new_n47_));
  inv1   g23(.a(x09), .O(new_n48_));
  nor2   g24(.a(x10), .b(new_n48_), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n47_), .c(new_n46_), .O(new_n50_));
  inv1   g26(.a(x00), .O(new_n51_));
  nand2  g27(.a(new_n49_), .b(new_n51_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n50_), .c(x07), .O(new_n53_));
  nor2   g29(.a(x10), .b(x07), .O(new_n54_));
  nor3   g30(.a(new_n54_), .b(x03), .c(x00), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n53_), .c(new_n26_), .O(new_n56_));
  nand2  g32(.a(x10), .b(x03), .O(new_n57_));
  oai21  g33(.a(x08), .b(x07), .c(x00), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n57_), .c(x02), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n56_), .O(z2));
  inv1   g36(.a(x07), .O(new_n61_));
  inv1   g37(.a(x11), .O(new_n62_));
  inv1   g38(.a(x12), .O(new_n63_));
  oai22  g39(.a(new_n63_), .b(x00), .c(new_n62_), .d(new_n61_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n57_), .O(z3));
  nand2  g41(.a(x09), .b(x08), .O(new_n66_));
  oai22  g42(.a(new_n66_), .b(new_n51_), .c(new_n25_), .d(new_n35_), .O(new_n67_));
  nand3  g43(.a(new_n57_), .b(new_n62_), .c(x07), .O(new_n68_));
  oai21  g44(.a(new_n67_), .b(x07), .c(new_n68_), .O(z4));
  oai21  g45(.a(x07), .b(x00), .c(new_n57_), .O(new_n70_));
  aoi21  g46(.a(x13), .b(new_n63_), .c(x10), .O(new_n71_));
  or2    g47(.a(new_n71_), .b(new_n47_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n61_), .c(new_n51_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n70_), .O(z5));
  nand2  g50(.a(x08), .b(x00), .O(new_n75_));
  nand2  g51(.a(x09), .b(new_n35_), .O(new_n76_));
  nand2  g52(.a(new_n48_), .b(x03), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n27_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n75_), .c(new_n26_), .O(new_n79_));
  aoi21  g55(.a(x14), .b(x02), .c(x00), .O(new_n80_));
  nor2   g56(.a(x08), .b(new_n51_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(new_n80_), .c(x01), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand3  g59(.a(new_n83_), .b(new_n25_), .c(new_n61_), .O(new_n84_));
  inv1   g60(.a(new_n84_), .O(z6));
  oai21  g61(.a(new_n28_), .b(x01), .c(new_n46_), .O(new_n86_));
  nand4  g62(.a(x09), .b(x03), .c(new_n27_), .d(new_n26_), .O(new_n87_));
  nand4  g63(.a(new_n87_), .b(new_n86_), .c(new_n25_), .d(new_n61_), .O(z7));
  oai21  g64(.a(x10), .b(x09), .c(new_n35_), .O(new_n89_));
  nor2   g65(.a(x12), .b(x02), .O(new_n90_));
  aoi21  g66(.a(new_n90_), .b(new_n26_), .c(new_n48_), .O(new_n91_));
  oai21  g67(.a(x09), .b(x00), .c(new_n61_), .O(new_n92_));
  oai21  g68(.a(new_n92_), .b(new_n91_), .c(new_n25_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(new_n89_), .O(z8));
endmodule


