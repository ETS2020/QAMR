// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x10), .O(new_n31_));
  nand2  g07(.a(x14), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x01), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand2  g16(.a(x03), .b(new_n25_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n35_), .c(new_n40_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n32_), .c(new_n34_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  nor2   g21(.a(x14), .b(x10), .O(new_n46_));
  aoi22  g22(.a(new_n46_), .b(x09), .c(x10), .d(new_n36_), .O(new_n47_));
  nand3  g23(.a(new_n46_), .b(x09), .c(new_n34_), .O(new_n48_));
  oai21  g24(.a(new_n47_), .b(x08), .c(new_n48_), .O(new_n49_));
  oai21  g25(.a(x14), .b(new_n45_), .c(new_n31_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n36_), .c(new_n34_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  aoi21  g28(.a(new_n49_), .b(new_n45_), .c(new_n52_), .O(new_n53_));
  oai21  g29(.a(x08), .b(x07), .c(x00), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n32_), .c(x02), .O(new_n55_));
  oai21  g31(.a(new_n53_), .b(x01), .c(new_n55_), .O(z2));
  inv1   g32(.a(x11), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  oai22  g34(.a(new_n58_), .b(x00), .c(new_n57_), .d(new_n45_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n32_), .O(z3));
  nand3  g36(.a(x09), .b(x08), .c(x00), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n45_), .O(new_n62_));
  nand2  g38(.a(new_n57_), .b(x07), .O(new_n63_));
  aoi22  g39(.a(new_n63_), .b(new_n62_), .c(x14), .d(new_n31_), .O(z4));
  inv1   g40(.a(x14), .O(new_n65_));
  nor3   g41(.a(x10), .b(x07), .c(x00), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n65_), .c(x13), .d(new_n58_), .O(z5));
  nand2  g43(.a(x08), .b(x00), .O(new_n68_));
  nor2   g44(.a(x02), .b(x01), .O(new_n69_));
  nand2  g45(.a(x09), .b(new_n36_), .O(new_n70_));
  inv1   g46(.a(x09), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x03), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n68_), .c(new_n45_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n65_), .c(x10), .O(z6));
  inv1   g51(.a(x08), .O(new_n76_));
  oai21  g52(.a(new_n41_), .b(x01), .c(new_n76_), .O(new_n77_));
  nand3  g53(.a(new_n69_), .b(x09), .c(x03), .O(new_n78_));
  nand4  g54(.a(new_n78_), .b(new_n77_), .c(new_n46_), .d(new_n45_), .O(z7));
  nand3  g55(.a(new_n69_), .b(new_n58_), .c(x03), .O(new_n80_));
  oai21  g56(.a(x09), .b(x00), .c(new_n45_), .O(new_n81_));
  aoi21  g57(.a(new_n80_), .b(x09), .c(new_n81_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(x14), .c(new_n31_), .O(z8));
endmodule


