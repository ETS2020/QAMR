// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x08), .O(new_n31_));
  inv1   g07(.a(x12), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g09(.a(new_n30_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand3  g16(.a(x03), .b(new_n25_), .c(x01), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n33_), .c(new_n35_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  inv1   g20(.a(x01), .O(new_n45_));
  oai21  g21(.a(x12), .b(x08), .c(x07), .O(new_n46_));
  nand2  g22(.a(x10), .b(x08), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n46_), .c(x03), .O(new_n48_));
  inv1   g24(.a(x07), .O(new_n49_));
  inv1   g25(.a(x10), .O(new_n50_));
  nand4  g26(.a(new_n50_), .b(x09), .c(x08), .d(new_n49_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n48_), .c(new_n35_), .O(new_n53_));
  nand2  g29(.a(new_n50_), .b(x09), .O(new_n54_));
  oai21  g30(.a(new_n50_), .b(x03), .c(new_n54_), .O(new_n55_));
  nand4  g31(.a(new_n55_), .b(x12), .c(new_n31_), .d(new_n49_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n45_), .O(new_n58_));
  nand2  g34(.a(x12), .b(x07), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n31_), .c(x00), .O(new_n60_));
  nor2   g36(.a(new_n32_), .b(x08), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n49_), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n60_), .c(x02), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n58_), .O(z2));
  inv1   g41(.a(x11), .O(new_n66_));
  oai22  g42(.a(new_n32_), .b(x00), .c(new_n66_), .d(new_n49_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n33_), .O(z3));
  nand3  g44(.a(new_n33_), .b(new_n66_), .c(x07), .O(new_n69_));
  aoi21  g45(.a(x09), .b(x00), .c(new_n31_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n61_), .c(new_n49_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n69_), .O(z4));
  nor2   g48(.a(x12), .b(x10), .O(new_n73_));
  nor2   g49(.a(x07), .b(x00), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n73_), .c(x13), .d(x08), .O(z5));
  nand2  g51(.a(x12), .b(new_n31_), .O(new_n76_));
  nand2  g52(.a(x08), .b(new_n35_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g54(.a(x09), .b(new_n36_), .O(new_n79_));
  inv1   g55(.a(x09), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x03), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n79_), .c(new_n25_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n78_), .c(new_n45_), .O(new_n83_));
  inv1   g59(.a(new_n83_), .O(new_n84_));
  nand2  g60(.a(x14), .b(x02), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n33_), .c(new_n35_), .O(new_n86_));
  nand2  g62(.a(new_n61_), .b(x00), .O(new_n87_));
  aoi21  g63(.a(new_n87_), .b(new_n86_), .c(new_n45_), .O(new_n88_));
  oai21  g64(.a(new_n88_), .b(new_n84_), .c(new_n50_), .O(new_n89_));
  nor2   g65(.a(new_n89_), .b(x07), .O(z6));
  oai21  g66(.a(x10), .b(x07), .c(new_n33_), .O(new_n91_));
  nand4  g67(.a(x08), .b(x03), .c(new_n25_), .d(new_n45_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(new_n76_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(x09), .O(new_n94_));
  nand3  g70(.a(x03), .b(new_n25_), .c(new_n45_), .O(new_n95_));
  nand3  g71(.a(new_n95_), .b(x12), .c(new_n31_), .O(new_n96_));
  nand3  g72(.a(new_n96_), .b(new_n94_), .c(new_n91_), .O(z7));
  oai21  g73(.a(x09), .b(new_n31_), .c(new_n32_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n35_), .O(new_n99_));
  nand2  g75(.a(new_n95_), .b(x08), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n32_), .O(new_n101_));
  nand2  g77(.a(new_n101_), .b(x09), .O(new_n102_));
  nand3  g78(.a(new_n102_), .b(new_n99_), .c(new_n91_), .O(z8));
endmodule


