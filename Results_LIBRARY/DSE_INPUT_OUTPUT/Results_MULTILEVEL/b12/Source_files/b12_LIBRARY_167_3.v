// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x08), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x05), .O(new_n27_));
  aoi21  g03(.a(new_n27_), .b(x04), .c(new_n25_), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nand2  g05(.a(new_n26_), .b(new_n25_), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(new_n29_), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(new_n28_), .c(x01), .O(new_n32_));
  inv1   g08(.a(x04), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x03), .c(x02), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n32_), .c(x00), .O(z0));
  nand2  g11(.a(x08), .b(x04), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n25_), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n33_), .c(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(x03), .O(new_n41_));
  nand4  g17(.a(new_n38_), .b(new_n26_), .c(x04), .d(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n41_), .c(x00), .O(z1));
  inv1   g19(.a(x00), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  nand3  g21(.a(x07), .b(new_n29_), .c(new_n45_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n36_), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(x09), .O(new_n50_));
  oai22  g26(.a(new_n50_), .b(x07), .c(new_n49_), .d(x03), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n33_), .c(new_n45_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  inv1   g30(.a(x07), .O(new_n55_));
  oai21  g31(.a(new_n49_), .b(x03), .c(new_n50_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n45_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n25_), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n26_), .c(new_n55_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n54_), .O(z2));
  inv1   g36(.a(x11), .O(new_n61_));
  inv1   g37(.a(new_n36_), .O(new_n62_));
  aoi21  g38(.a(x12), .b(new_n44_), .c(new_n62_), .O(new_n63_));
  oai21  g39(.a(new_n61_), .b(new_n55_), .c(new_n63_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(z3));
  oai21  g41(.a(x08), .b(new_n55_), .c(x04), .O(new_n66_));
  nand3  g42(.a(x09), .b(x08), .c(x00), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n55_), .O(new_n68_));
  nand2  g44(.a(new_n61_), .b(x07), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(z4));
  inv1   g46(.a(x12), .O(new_n71_));
  nand3  g47(.a(x13), .b(new_n71_), .c(new_n49_), .O(new_n72_));
  nor3   g48(.a(new_n72_), .b(x07), .c(x00), .O(new_n73_));
  nor2   g49(.a(new_n73_), .b(new_n62_), .O(z5));
  aoi21  g50(.a(new_n33_), .b(new_n44_), .c(new_n26_), .O(new_n75_));
  nand2  g51(.a(x14), .b(x02), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x01), .O(new_n77_));
  xor2a  g53(.a(x09), .b(x03), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(x02), .c(new_n45_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  nand3  g56(.a(new_n26_), .b(x01), .c(x00), .O(new_n81_));
  inv1   g57(.a(new_n81_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n80_), .c(new_n49_), .O(new_n83_));
  nor2   g59(.a(new_n83_), .b(x07), .O(z6));
  oai21  g60(.a(x10), .b(x07), .c(new_n36_), .O(new_n85_));
  nand4  g61(.a(new_n33_), .b(x03), .c(new_n25_), .d(new_n45_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(x08), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(x09), .O(new_n88_));
  nand3  g64(.a(x03), .b(new_n25_), .c(new_n45_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n26_), .O(new_n90_));
  nand3  g66(.a(new_n90_), .b(new_n88_), .c(new_n85_), .O(z7));
  nand4  g67(.a(new_n71_), .b(x03), .c(new_n25_), .d(new_n45_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  inv1   g69(.a(x09), .O(new_n94_));
  aoi21  g70(.a(new_n94_), .b(new_n44_), .c(x10), .O(new_n95_));
  nand4  g71(.a(new_n95_), .b(new_n93_), .c(new_n36_), .d(new_n55_), .O(z8));
endmodule


