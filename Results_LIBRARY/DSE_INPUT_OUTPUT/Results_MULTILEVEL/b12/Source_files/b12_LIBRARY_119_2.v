// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x10), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x02), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand4  g13(.a(new_n37_), .b(new_n36_), .c(x10), .d(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n25_), .c(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g16(.a(x01), .O(new_n41_));
  inv1   g17(.a(x08), .O(new_n42_));
  nor2   g18(.a(new_n31_), .b(x03), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(x10), .b(new_n44_), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(new_n46_));
  inv1   g22(.a(x00), .O(new_n47_));
  nand2  g23(.a(new_n45_), .b(new_n47_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n46_), .c(x07), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nor3   g26(.a(new_n50_), .b(x03), .c(x00), .O(new_n51_));
  oai21  g27(.a(new_n51_), .b(new_n49_), .c(new_n41_), .O(new_n52_));
  oai21  g28(.a(x08), .b(x07), .c(x10), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n47_), .c(x02), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n52_), .O(z2));
  inv1   g31(.a(x07), .O(new_n56_));
  inv1   g32(.a(x11), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  oai22  g34(.a(new_n58_), .b(x00), .c(new_n57_), .d(new_n56_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n32_), .O(z3));
  nand2  g36(.a(x09), .b(x08), .O(new_n61_));
  oai22  g37(.a(new_n61_), .b(new_n47_), .c(x10), .d(new_n25_), .O(new_n62_));
  nand3  g38(.a(new_n32_), .b(new_n57_), .c(x07), .O(new_n63_));
  oai21  g39(.a(new_n62_), .b(x07), .c(new_n63_), .O(z4));
  nand4  g40(.a(x13), .b(new_n58_), .c(new_n56_), .d(new_n47_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n25_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n31_), .O(z5));
  nand2  g43(.a(x08), .b(x00), .O(new_n68_));
  nand2  g44(.a(x09), .b(new_n34_), .O(new_n69_));
  nand2  g45(.a(new_n44_), .b(x03), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n41_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n68_), .c(new_n31_), .d(new_n56_), .O(new_n72_));
  nor2   g48(.a(new_n72_), .b(x02), .O(z6));
  aoi21  g49(.a(x03), .b(new_n41_), .c(x08), .O(new_n74_));
  nand2  g50(.a(x09), .b(x03), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(x01), .c(new_n56_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n74_), .c(new_n25_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n31_), .O(z7));
  nand3  g54(.a(new_n58_), .b(x03), .c(new_n41_), .O(new_n79_));
  oai21  g55(.a(x09), .b(x00), .c(new_n56_), .O(new_n80_));
  aoi21  g56(.a(new_n79_), .b(x09), .c(new_n80_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(x02), .c(new_n31_), .O(z8));
endmodule


