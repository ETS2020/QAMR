// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  inv1   g11(.a(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n27_), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  nand2  g18(.a(x11), .b(x00), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(z1));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  nand3  g21(.a(x10), .b(new_n35_), .c(new_n25_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n27_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g24(.a(x07), .b(new_n35_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n49_), .c(x00), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  nand4  g30(.a(new_n51_), .b(x09), .c(new_n54_), .d(new_n50_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n53_), .c(new_n25_), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n48_), .c(new_n43_), .O(z2));
  inv1   g34(.a(x11), .O(new_n59_));
  inv1   g35(.a(x12), .O(new_n60_));
  oai21  g36(.a(new_n59_), .b(new_n50_), .c(new_n60_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n34_), .O(z3));
  oai21  g38(.a(new_n59_), .b(x00), .c(x07), .O(new_n63_));
  nand3  g39(.a(new_n59_), .b(x09), .c(x08), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(x00), .O(new_n65_));
  nand2  g41(.a(new_n50_), .b(new_n34_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n63_), .O(z4));
  inv1   g43(.a(x13), .O(new_n68_));
  nor2   g44(.a(new_n68_), .b(x12), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n51_), .c(new_n50_), .d(new_n34_), .O(z5));
  oai21  g46(.a(x11), .b(x08), .c(x00), .O(new_n71_));
  nand2  g47(.a(x09), .b(new_n35_), .O(new_n72_));
  inv1   g48(.a(x09), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x03), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n72_), .c(new_n27_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n71_), .c(new_n25_), .O(new_n76_));
  nand2  g52(.a(x14), .b(x02), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n34_), .O(new_n78_));
  nand3  g54(.a(new_n59_), .b(new_n54_), .c(x00), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x01), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n76_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n51_), .c(new_n50_), .O(new_n83_));
  inv1   g59(.a(new_n83_), .O(z6));
  inv1   g60(.a(new_n43_), .O(new_n85_));
  nand3  g61(.a(x03), .b(new_n27_), .c(new_n25_), .O(new_n86_));
  nand4  g62(.a(x09), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n51_), .c(new_n50_), .O(new_n88_));
  aoi21  g64(.a(new_n86_), .b(new_n54_), .c(new_n88_), .O(new_n89_));
  nor2   g65(.a(new_n89_), .b(new_n85_), .O(z7));
  oai21  g66(.a(x11), .b(new_n34_), .c(new_n73_), .O(new_n91_));
  nand4  g67(.a(new_n60_), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  nor2   g69(.a(new_n85_), .b(x10), .O(new_n94_));
  nand4  g70(.a(new_n94_), .b(new_n93_), .c(new_n91_), .d(new_n50_), .O(z8));
endmodule


