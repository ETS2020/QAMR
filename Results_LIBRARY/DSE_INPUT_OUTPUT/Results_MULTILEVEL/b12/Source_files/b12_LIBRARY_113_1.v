// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nand2  g07(.a(x08), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x04), .O(new_n34_));
  nand2  g10(.a(x06), .b(x05), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g12(.a(x08), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n31_), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n36_), .c(new_n34_), .O(new_n39_));
  nand3  g15(.a(new_n37_), .b(x03), .c(new_n31_), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n39_), .c(x02), .O(new_n42_));
  nand3  g18(.a(x06), .b(x05), .c(x02), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(x03), .c(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n42_), .c(x00), .O(z1));
  inv1   g21(.a(x03), .O(new_n46_));
  nand3  g22(.a(x07), .b(new_n46_), .c(new_n31_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n25_), .c(x00), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nor2   g25(.a(new_n49_), .b(x03), .O(new_n50_));
  inv1   g26(.a(x09), .O(new_n51_));
  nor2   g27(.a(x10), .b(new_n51_), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n50_), .c(new_n31_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n25_), .c(x07), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n48_), .c(new_n37_), .O(new_n55_));
  inv1   g31(.a(x00), .O(new_n56_));
  nand3  g32(.a(x02), .b(x01), .c(new_n56_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n55_), .O(z2));
  inv1   g34(.a(x07), .O(new_n59_));
  inv1   g35(.a(x11), .O(new_n60_));
  inv1   g36(.a(x12), .O(new_n61_));
  oai22  g37(.a(new_n61_), .b(x00), .c(new_n60_), .d(new_n59_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n32_), .O(z3));
  oai21  g39(.a(x08), .b(new_n59_), .c(new_n31_), .O(new_n64_));
  nand3  g40(.a(x09), .b(x08), .c(x00), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand2  g42(.a(new_n60_), .b(x07), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(z4));
  nand3  g44(.a(x13), .b(new_n61_), .c(new_n49_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n32_), .c(new_n59_), .d(new_n56_), .O(z5));
  aoi21  g47(.a(x14), .b(x02), .c(x00), .O(new_n72_));
  nor2   g48(.a(x08), .b(new_n56_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  nand2  g50(.a(x09), .b(new_n46_), .O(new_n75_));
  nand2  g51(.a(new_n51_), .b(x03), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n25_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n37_), .c(new_n31_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n49_), .c(new_n59_), .O(new_n80_));
  inv1   g56(.a(new_n80_), .O(z6));
  nor2   g57(.a(x09), .b(x08), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(x03), .c(new_n25_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n31_), .O(new_n84_));
  aoi21  g60(.a(new_n37_), .b(x01), .c(x10), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n84_), .c(new_n59_), .O(z7));
  oai21  g62(.a(new_n82_), .b(x01), .c(new_n56_), .O(new_n87_));
  oai21  g63(.a(x10), .b(x07), .c(new_n32_), .O(new_n88_));
  nand3  g64(.a(new_n61_), .b(x03), .c(new_n25_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n37_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n31_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(x09), .O(new_n92_));
  nand3  g68(.a(new_n92_), .b(new_n88_), .c(new_n87_), .O(z8));
endmodule


