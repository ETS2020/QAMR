// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x08), .O(new_n31_));
  nand2  g07(.a(x14), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  inv1   g11(.a(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n25_), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n32_), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  aoi21  g21(.a(x14), .b(new_n31_), .c(new_n45_), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nor2   g23(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n46_), .c(new_n35_), .O(new_n49_));
  nand4  g25(.a(new_n47_), .b(x09), .c(x08), .d(new_n45_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n51_));
  inv1   g27(.a(x14), .O(new_n52_));
  nand2  g28(.a(new_n47_), .b(x09), .O(new_n53_));
  nand2  g29(.a(x10), .b(new_n35_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand4  g31(.a(new_n55_), .b(new_n52_), .c(new_n31_), .d(new_n45_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n51_), .c(new_n44_), .O(new_n58_));
  aoi21  g34(.a(new_n52_), .b(x07), .c(x08), .O(new_n59_));
  nand2  g35(.a(new_n52_), .b(new_n31_), .O(new_n60_));
  oai22  g36(.a(new_n60_), .b(x07), .c(new_n59_), .d(x00), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(x02), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n58_), .O(z2));
  inv1   g39(.a(x11), .O(new_n64_));
  inv1   g40(.a(x12), .O(new_n65_));
  oai22  g41(.a(new_n65_), .b(x00), .c(new_n64_), .d(new_n45_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n32_), .O(z3));
  inv1   g43(.a(new_n59_), .O(new_n68_));
  inv1   g44(.a(x09), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n34_), .c(new_n45_), .O(new_n70_));
  nand2  g46(.a(new_n64_), .b(x07), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n68_), .O(z4));
  nand3  g48(.a(x13), .b(new_n65_), .c(new_n47_), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n32_), .c(new_n45_), .d(new_n34_), .O(z5));
  oai21  g51(.a(x01), .b(x00), .c(x08), .O(new_n76_));
  xor2a  g52(.a(x09), .b(x03), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(x02), .c(new_n76_), .O(new_n78_));
  oai21  g54(.a(new_n52_), .b(new_n25_), .c(x01), .O(new_n79_));
  nand2  g55(.a(new_n35_), .b(new_n25_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n52_), .c(new_n69_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n34_), .O(new_n83_));
  nand2  g59(.a(new_n31_), .b(x01), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n83_), .c(new_n78_), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n47_), .c(new_n45_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n32_), .O(z6));
  oai21  g63(.a(x10), .b(x07), .c(new_n32_), .O(new_n88_));
  nand4  g64(.a(x08), .b(x03), .c(new_n25_), .d(new_n44_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(x09), .O(new_n91_));
  nand3  g67(.a(x03), .b(new_n25_), .c(new_n44_), .O(new_n92_));
  nand3  g68(.a(new_n92_), .b(new_n52_), .c(new_n31_), .O(new_n93_));
  nand3  g69(.a(new_n93_), .b(new_n91_), .c(new_n88_), .O(z7));
  nand4  g70(.a(new_n65_), .b(x03), .c(new_n25_), .d(new_n44_), .O(new_n95_));
  nand2  g71(.a(new_n95_), .b(x09), .O(new_n96_));
  aoi21  g72(.a(new_n69_), .b(new_n34_), .c(x07), .O(new_n97_));
  nand4  g73(.a(new_n97_), .b(new_n96_), .c(new_n32_), .d(new_n47_), .O(z8));
endmodule


