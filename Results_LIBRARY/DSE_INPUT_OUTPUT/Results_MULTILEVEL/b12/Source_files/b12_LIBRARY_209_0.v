// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n72_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(new_n25_), .O(new_n31_));
  aoi22  g07(.a(new_n31_), .b(x01), .c(new_n29_), .d(x03), .O(new_n32_));
  nor2   g08(.a(x10), .b(x01), .O(new_n33_));
  inv1   g09(.a(new_n33_), .O(new_n34_));
  oai21  g10(.a(new_n32_), .b(x00), .c(new_n34_), .O(z0));
  nand2  g11(.a(x06), .b(x05), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g13(.a(x01), .O(new_n38_));
  nand2  g14(.a(x10), .b(new_n38_), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n37_), .c(new_n27_), .O(new_n40_));
  inv1   g16(.a(x03), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nor3   g18(.a(new_n42_), .b(new_n41_), .c(x01), .O(new_n43_));
  oai21  g19(.a(new_n43_), .b(new_n40_), .c(x02), .O(new_n44_));
  nand3  g20(.a(x06), .b(x05), .c(x02), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(x03), .c(x01), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n44_), .c(x00), .O(z1));
  aoi21  g23(.a(new_n41_), .b(new_n38_), .c(x02), .O(new_n48_));
  inv1   g24(.a(x00), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n48_), .c(new_n34_), .O(z2));
  inv1   g29(.a(x11), .O(new_n54_));
  aoi21  g30(.a(x12), .b(new_n49_), .c(new_n33_), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand3  g33(.a(x09), .b(x08), .c(x00), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g35(.a(new_n54_), .b(x07), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(new_n33_), .O(z4));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nor2   g38(.a(new_n38_), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(x13), .d(new_n50_), .O(z5));
  nand2  g40(.a(x14), .b(x02), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n49_), .O(new_n66_));
  oai21  g42(.a(x08), .b(new_n49_), .c(new_n66_), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n42_), .c(new_n50_), .d(x01), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(z6));
  oai21  g45(.a(new_n51_), .b(x07), .c(x01), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n42_), .O(z7));
  nor2   g47(.a(x10), .b(x09), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n50_), .c(x01), .d(x00), .O(z8));
endmodule


