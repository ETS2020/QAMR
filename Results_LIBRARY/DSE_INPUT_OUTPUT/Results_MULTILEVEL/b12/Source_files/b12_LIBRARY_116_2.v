// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x10), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x09), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x01), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand2  g16(.a(x03), .b(new_n25_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n35_), .c(new_n40_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n32_), .c(new_n34_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  nand3  g21(.a(x10), .b(new_n36_), .c(new_n35_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand4  g24(.a(x07), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n32_), .O(z2));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  nand2  g27(.a(x12), .b(new_n34_), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n32_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(z3));
  inv1   g30(.a(x11), .O(new_n55_));
  nand3  g31(.a(new_n32_), .b(new_n55_), .c(x07), .O(new_n56_));
  inv1   g32(.a(x09), .O(new_n57_));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(x10), .c(new_n57_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(x07), .c(new_n56_), .O(z4));
  inv1   g36(.a(x07), .O(new_n61_));
  inv1   g37(.a(x12), .O(new_n62_));
  nand4  g38(.a(x13), .b(new_n62_), .c(new_n61_), .d(new_n34_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n31_), .O(z5));
  nand2  g41(.a(new_n36_), .b(new_n25_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n58_), .c(new_n35_), .O(new_n67_));
  inv1   g43(.a(x08), .O(new_n68_));
  aoi21  g44(.a(x14), .b(x02), .c(x00), .O(new_n69_));
  aoi21  g45(.a(new_n68_), .b(x00), .c(new_n69_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n35_), .c(new_n67_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n31_), .c(new_n57_), .d(new_n61_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(z6));
  oai21  g49(.a(new_n41_), .b(x01), .c(new_n68_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n31_), .c(new_n57_), .d(new_n61_), .O(z7));
  nand4  g51(.a(new_n31_), .b(new_n57_), .c(new_n61_), .d(x00), .O(z8));
endmodule


