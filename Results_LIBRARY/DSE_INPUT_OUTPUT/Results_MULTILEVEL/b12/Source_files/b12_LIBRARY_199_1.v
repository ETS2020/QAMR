// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(new_n27_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(x01), .c(x10), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x10), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand2  g16(.a(x03), .b(new_n27_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n25_), .c(new_n40_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n35_), .c(new_n34_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(x02), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  nor2   g23(.a(new_n47_), .b(x03), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(new_n49_), .b(x07), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n48_), .c(new_n34_), .O(new_n51_));
  inv1   g27(.a(x08), .O(new_n52_));
  nand2  g28(.a(x10), .b(new_n36_), .O(new_n53_));
  nand2  g29(.a(new_n35_), .b(x09), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n52_), .c(new_n47_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n51_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n25_), .O(new_n58_));
  nand2  g34(.a(x10), .b(new_n34_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n58_), .c(new_n46_), .O(z2));
  inv1   g36(.a(x12), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n35_), .O(new_n62_));
  aoi22  g38(.a(new_n62_), .b(new_n34_), .c(x11), .d(x07), .O(z3));
  nand3  g39(.a(x09), .b(x08), .c(x00), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  inv1   g41(.a(x11), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(x07), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n65_), .c(new_n59_), .O(z4));
  inv1   g44(.a(x13), .O(new_n69_));
  nor2   g45(.a(new_n69_), .b(x12), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n35_), .c(new_n47_), .d(new_n34_), .O(z5));
  nand2  g47(.a(x08), .b(x00), .O(new_n72_));
  nand2  g48(.a(x09), .b(new_n36_), .O(new_n73_));
  nand2  g49(.a(new_n49_), .b(x03), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n27_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n72_), .c(new_n25_), .O(new_n76_));
  aoi21  g52(.a(x14), .b(x02), .c(x00), .O(new_n77_));
  nor2   g53(.a(x08), .b(new_n34_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n77_), .c(x01), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n35_), .c(new_n47_), .O(new_n81_));
  inv1   g57(.a(new_n81_), .O(z6));
  oai21  g58(.a(new_n41_), .b(x01), .c(new_n52_), .O(new_n83_));
  nand4  g59(.a(x09), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n83_), .c(new_n47_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n35_), .O(new_n86_));
  nand2  g62(.a(x10), .b(x00), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(new_n86_), .O(z7));
  nand4  g64(.a(new_n61_), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(x09), .O(new_n90_));
  aoi21  g66(.a(new_n49_), .b(new_n34_), .c(x10), .O(new_n91_));
  nand3  g67(.a(new_n91_), .b(new_n90_), .c(new_n47_), .O(z8));
endmodule


