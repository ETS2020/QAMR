// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(new_n25_), .O(new_n31_));
  aoi22  g07(.a(new_n31_), .b(x01), .c(new_n29_), .d(x03), .O(new_n32_));
  nand2  g08(.a(x08), .b(x00), .O(new_n33_));
  oai21  g09(.a(new_n32_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(x03), .O(new_n35_));
  nand2  g11(.a(new_n27_), .b(new_n35_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n25_), .c(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g16(.a(x00), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x01), .O(new_n43_));
  nand3  g19(.a(x10), .b(new_n35_), .c(new_n43_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  oai21  g21(.a(new_n42_), .b(new_n41_), .c(new_n45_), .O(new_n46_));
  nand3  g22(.a(x07), .b(new_n35_), .c(new_n41_), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(x09), .c(new_n42_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n46_), .c(new_n33_), .O(z2));
  inv1   g28(.a(x12), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  oai21  g30(.a(new_n53_), .b(x00), .c(new_n54_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n33_), .O(z3));
  nand3  g32(.a(new_n33_), .b(x11), .c(x07), .O(z4));
  oai21  g33(.a(x08), .b(new_n42_), .c(x00), .O(new_n58_));
  inv1   g34(.a(x13), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(x12), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n58_), .c(new_n48_), .d(new_n42_), .O(z5));
  inv1   g37(.a(x08), .O(new_n62_));
  nand3  g38(.a(new_n48_), .b(new_n42_), .c(x01), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(x00), .O(new_n65_));
  nand2  g41(.a(x14), .b(x02), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(x01), .O(new_n67_));
  xor2a  g43(.a(x09), .b(x03), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x02), .c(new_n43_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n48_), .c(new_n42_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n65_), .O(z6));
  nand3  g48(.a(x03), .b(new_n25_), .c(new_n43_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x08), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(x09), .O(new_n75_));
  nand3  g51(.a(new_n33_), .b(new_n48_), .c(new_n42_), .O(new_n76_));
  aoi21  g52(.a(new_n73_), .b(new_n62_), .c(new_n76_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n75_), .O(z7));
  inv1   g54(.a(x09), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(x00), .c(x08), .O(new_n80_));
  nand4  g56(.a(new_n53_), .b(x03), .c(new_n25_), .d(new_n43_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(x09), .O(new_n82_));
  aoi21  g58(.a(new_n79_), .b(new_n41_), .c(x10), .O(new_n83_));
  nand4  g59(.a(new_n83_), .b(new_n82_), .c(new_n80_), .d(new_n42_), .O(z8));
endmodule


