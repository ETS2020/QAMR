// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n77_, new_n78_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  nand2  g02(.a(x03), .b(new_n26_), .O(new_n27_));
  nand2  g03(.a(x05), .b(x02), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(new_n27_), .c(new_n25_), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x03), .c(x02), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(new_n29_), .c(x10), .O(new_n33_));
  inv1   g09(.a(x03), .O(new_n34_));
  inv1   g10(.a(x05), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x04), .O(new_n36_));
  nand4  g12(.a(new_n36_), .b(new_n34_), .c(x02), .d(x01), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n33_), .c(x00), .O(z0));
  nand2  g14(.a(x10), .b(x03), .O(new_n39_));
  oai21  g15(.a(new_n30_), .b(x03), .c(new_n39_), .O(new_n40_));
  nand3  g16(.a(x06), .b(x05), .c(x01), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n40_), .c(x02), .O(new_n42_));
  nand4  g18(.a(x10), .b(x03), .c(new_n26_), .d(x01), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n42_), .c(x00), .O(z1));
  inv1   g20(.a(x00), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nor2   g24(.a(x10), .b(x09), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(x01), .c(new_n26_), .O(new_n50_));
  aoi22  g26(.a(new_n50_), .b(new_n34_), .c(x10), .d(x02), .O(new_n51_));
  nand4  g27(.a(x07), .b(new_n34_), .c(new_n25_), .d(new_n45_), .O(new_n52_));
  oai21  g28(.a(new_n51_), .b(new_n48_), .c(new_n52_), .O(z2));
  inv1   g29(.a(x11), .O(new_n54_));
  inv1   g30(.a(x12), .O(new_n55_));
  oai22  g31(.a(new_n55_), .b(x00), .c(new_n54_), .d(new_n46_), .O(new_n56_));
  inv1   g32(.a(x10), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(x03), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n56_), .O(z3));
  nand2  g35(.a(x09), .b(x08), .O(new_n60_));
  oai22  g36(.a(new_n60_), .b(new_n45_), .c(x10), .d(new_n34_), .O(new_n61_));
  nand3  g37(.a(new_n58_), .b(new_n54_), .c(x07), .O(new_n62_));
  oai21  g38(.a(new_n61_), .b(x07), .c(new_n62_), .O(z4));
  nand4  g39(.a(x13), .b(new_n55_), .c(new_n46_), .d(new_n45_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n34_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n57_), .O(z5));
  inv1   g42(.a(x09), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n26_), .O(new_n68_));
  nand2  g44(.a(x08), .b(x00), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n25_), .O(new_n70_));
  aoi21  g46(.a(x14), .b(x02), .c(x00), .O(new_n71_));
  aoi21  g47(.a(new_n47_), .b(x00), .c(new_n71_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n25_), .c(new_n70_), .O(new_n73_));
  nand4  g49(.a(new_n73_), .b(new_n57_), .c(new_n46_), .d(new_n34_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(z6));
  nand4  g51(.a(new_n57_), .b(x08), .c(new_n46_), .d(new_n34_), .O(z7));
  nand3  g52(.a(new_n67_), .b(new_n46_), .c(x00), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n34_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n57_), .O(z8));
endmodule


