// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x10), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x09), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  inv1   g11(.a(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n25_), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n32_), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  oai21  g21(.a(x09), .b(new_n45_), .c(new_n31_), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n35_), .c(new_n44_), .O(new_n47_));
  nand2  g23(.a(new_n32_), .b(x02), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n34_), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  nand3  g27(.a(x10), .b(new_n35_), .c(new_n44_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n51_), .c(new_n45_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n50_), .O(z2));
  inv1   g31(.a(x12), .O(new_n56_));
  nand2  g32(.a(x11), .b(x07), .O(new_n57_));
  oai21  g33(.a(new_n56_), .b(x00), .c(new_n57_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n32_), .O(z3));
  inv1   g35(.a(x09), .O(new_n60_));
  nand2  g36(.a(new_n57_), .b(new_n60_), .O(new_n61_));
  nand2  g37(.a(x08), .b(x00), .O(new_n62_));
  nor2   g38(.a(x11), .b(new_n45_), .O(new_n63_));
  aoi21  g39(.a(new_n62_), .b(new_n45_), .c(new_n63_), .O(new_n64_));
  oai21  g40(.a(new_n64_), .b(new_n31_), .c(new_n61_), .O(z4));
  nor3   g41(.a(x09), .b(x07), .c(x00), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(x13), .c(new_n56_), .d(new_n31_), .O(z5));
  nand2  g43(.a(new_n35_), .b(new_n25_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n62_), .c(new_n44_), .O(new_n69_));
  aoi21  g45(.a(x14), .b(x02), .c(x00), .O(new_n70_));
  aoi21  g46(.a(new_n51_), .b(x00), .c(new_n70_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n44_), .c(new_n69_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n31_), .c(new_n60_), .d(new_n45_), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(z6));
  nand3  g50(.a(x03), .b(new_n25_), .c(new_n44_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n51_), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n31_), .c(new_n60_), .d(new_n45_), .O(z7));
  nand4  g53(.a(new_n31_), .b(new_n60_), .c(new_n45_), .d(x00), .O(z8));
endmodule


