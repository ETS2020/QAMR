// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  nand2  g06(.a(x08), .b(x00), .O(new_n31_));
  oai21  g07(.a(new_n30_), .b(x00), .c(new_n31_), .O(z0));
  inv1   g08(.a(x03), .O(new_n33_));
  inv1   g09(.a(x04), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n35_), .c(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n25_), .c(x01), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n37_), .c(x00), .O(z1));
  inv1   g15(.a(x00), .O(new_n40_));
  inv1   g16(.a(x07), .O(new_n41_));
  inv1   g17(.a(x01), .O(new_n42_));
  nand3  g18(.a(x10), .b(new_n33_), .c(new_n42_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  oai21  g20(.a(new_n41_), .b(new_n40_), .c(new_n44_), .O(new_n45_));
  inv1   g21(.a(new_n31_), .O(new_n46_));
  nand3  g22(.a(x07), .b(new_n33_), .c(new_n40_), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(x09), .c(new_n41_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n42_), .c(new_n46_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n45_), .O(z2));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(x08), .b(new_n40_), .O(new_n54_));
  aoi21  g30(.a(new_n53_), .b(new_n40_), .c(new_n54_), .O(new_n55_));
  aoi21  g31(.a(x11), .b(x07), .c(new_n55_), .O(z3));
  aoi21  g32(.a(x11), .b(x07), .c(new_n46_), .O(z4));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n53_), .d(new_n48_), .O(z5));
  nand2  g35(.a(x09), .b(new_n33_), .O(new_n60_));
  inv1   g36(.a(x09), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(x03), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n60_), .c(new_n25_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n31_), .c(new_n42_), .O(new_n64_));
  aoi21  g40(.a(x14), .b(x02), .c(x00), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n54_), .c(x01), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n48_), .c(new_n41_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(z6));
  nand3  g45(.a(x03), .b(new_n25_), .c(new_n42_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x08), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x09), .O(new_n72_));
  inv1   g48(.a(x08), .O(new_n73_));
  nand3  g49(.a(new_n31_), .b(new_n48_), .c(new_n41_), .O(new_n74_));
  aoi21  g50(.a(new_n70_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n72_), .O(z7));
  oai21  g52(.a(new_n61_), .b(x08), .c(x00), .O(new_n77_));
  nand4  g53(.a(new_n53_), .b(x03), .c(new_n25_), .d(new_n42_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g55(.a(new_n48_), .b(new_n41_), .O(new_n80_));
  aoi22  g56(.a(new_n80_), .b(new_n31_), .c(new_n61_), .d(new_n40_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n79_), .O(z8));
endmodule


