// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x14), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x01), .O(new_n34_));
  oai21  g10(.a(x04), .b(x03), .c(new_n34_), .O(new_n35_));
  nand2  g11(.a(x06), .b(x05), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(x14), .c(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(x02), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x02), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(x14), .c(x03), .d(x01), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n39_), .c(x00), .O(z1));
  oai21  g18(.a(x08), .b(x07), .c(x00), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(x03), .c(new_n25_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  inv1   g22(.a(x00), .O(new_n47_));
  inv1   g23(.a(x03), .O(new_n48_));
  nand2  g24(.a(x07), .b(new_n48_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  nand3  g26(.a(new_n44_), .b(x09), .c(new_n50_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  nand4  g30(.a(new_n44_), .b(x09), .c(new_n54_), .d(new_n50_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n53_), .c(new_n46_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n34_), .O(new_n57_));
  nand3  g33(.a(new_n43_), .b(x14), .c(x02), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n57_), .O(z2));
  inv1   g35(.a(x11), .O(new_n60_));
  inv1   g36(.a(x12), .O(new_n61_));
  oai22  g37(.a(new_n61_), .b(x00), .c(new_n60_), .d(new_n50_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n32_), .O(z3));
  nand3  g39(.a(x09), .b(x08), .c(x00), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n50_), .O(new_n65_));
  nand2  g41(.a(new_n60_), .b(x07), .O(new_n66_));
  aoi22  g42(.a(new_n66_), .b(new_n65_), .c(new_n31_), .d(x01), .O(z4));
  nand3  g43(.a(x13), .b(new_n61_), .c(new_n44_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n32_), .c(new_n50_), .d(new_n47_), .O(z5));
  nand2  g46(.a(new_n25_), .b(new_n47_), .O(new_n71_));
  nand2  g47(.a(new_n54_), .b(x00), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(x14), .c(x01), .O(new_n74_));
  nand2  g50(.a(x08), .b(x00), .O(new_n75_));
  nand2  g51(.a(x09), .b(new_n48_), .O(new_n76_));
  inv1   g52(.a(x09), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(x03), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n76_), .c(new_n25_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n75_), .c(new_n34_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n44_), .c(new_n50_), .O(new_n82_));
  inv1   g58(.a(new_n82_), .O(z6));
  oai21  g59(.a(new_n31_), .b(new_n54_), .c(x01), .O(new_n84_));
  oai21  g60(.a(new_n48_), .b(x02), .c(new_n54_), .O(new_n85_));
  nor2   g61(.a(x02), .b(x01), .O(new_n86_));
  nor2   g62(.a(new_n77_), .b(new_n48_), .O(new_n87_));
  aoi21  g63(.a(new_n87_), .b(new_n86_), .c(x10), .O(new_n88_));
  nand4  g64(.a(new_n88_), .b(new_n85_), .c(new_n84_), .d(new_n50_), .O(z7));
  oai21  g65(.a(new_n31_), .b(x09), .c(x01), .O(new_n90_));
  nand3  g66(.a(new_n61_), .b(x03), .c(new_n25_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(x09), .O(new_n92_));
  aoi21  g68(.a(new_n77_), .b(new_n47_), .c(x10), .O(new_n93_));
  nand4  g69(.a(new_n93_), .b(new_n92_), .c(new_n90_), .d(new_n50_), .O(z8));
endmodule


