// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nand2  g07(.a(x10), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x04), .O(new_n34_));
  nand2  g10(.a(x06), .b(x05), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g12(.a(x10), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n31_), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n36_), .c(new_n34_), .O(new_n39_));
  nand3  g15(.a(new_n37_), .b(x03), .c(new_n31_), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n39_), .c(x02), .O(new_n42_));
  nand3  g18(.a(x06), .b(x05), .c(x02), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(x03), .c(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n42_), .c(x00), .O(z1));
  inv1   g21(.a(x00), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x08), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g25(.a(x03), .O(new_n50_));
  nand2  g26(.a(x07), .b(new_n50_), .O(new_n51_));
  nand2  g27(.a(x09), .b(new_n47_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n51_), .c(x00), .O(new_n53_));
  nand3  g29(.a(x09), .b(new_n48_), .c(new_n47_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n53_), .c(new_n31_), .O(new_n56_));
  oai21  g32(.a(new_n49_), .b(new_n25_), .c(new_n56_), .O(z2));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  nand2  g34(.a(x12), .b(new_n46_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n58_), .c(new_n32_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(z3));
  nand2  g37(.a(x09), .b(x08), .O(new_n62_));
  oai22  g38(.a(new_n62_), .b(new_n46_), .c(new_n37_), .d(x01), .O(new_n63_));
  inv1   g39(.a(x11), .O(new_n64_));
  nand3  g40(.a(new_n32_), .b(new_n64_), .c(x07), .O(new_n65_));
  oai21  g41(.a(new_n63_), .b(x07), .c(new_n65_), .O(z4));
  oai21  g42(.a(x07), .b(x00), .c(new_n32_), .O(new_n67_));
  inv1   g43(.a(x13), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x12), .c(new_n37_), .O(new_n69_));
  oai21  g45(.a(new_n37_), .b(new_n31_), .c(new_n69_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n47_), .c(new_n46_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n67_), .O(z5));
  nand2  g48(.a(x08), .b(x00), .O(new_n73_));
  nand2  g49(.a(x09), .b(new_n50_), .O(new_n74_));
  inv1   g50(.a(x09), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x03), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n74_), .c(new_n25_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n73_), .c(new_n31_), .O(new_n78_));
  nand2  g54(.a(x14), .b(x02), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g56(.a(new_n48_), .b(x00), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n37_), .c(x01), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n78_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n47_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n32_), .O(z6));
  nand3  g62(.a(x03), .b(new_n25_), .c(new_n31_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(new_n48_), .O(new_n88_));
  nand4  g64(.a(x09), .b(x03), .c(new_n25_), .d(new_n31_), .O(new_n89_));
  nand4  g65(.a(new_n89_), .b(new_n88_), .c(new_n37_), .d(new_n47_), .O(z7));
  inv1   g66(.a(x12), .O(new_n91_));
  nand4  g67(.a(new_n91_), .b(x03), .c(new_n25_), .d(new_n31_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  aoi21  g69(.a(new_n75_), .b(new_n46_), .c(x10), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n93_), .c(new_n47_), .O(z8));
endmodule


