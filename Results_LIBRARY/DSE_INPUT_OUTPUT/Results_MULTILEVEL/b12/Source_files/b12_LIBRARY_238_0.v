// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand3  g03(.a(x10), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(x01), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(x03), .O(new_n35_));
  nand2  g11(.a(new_n25_), .b(new_n35_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  nand4  g14(.a(x10), .b(x03), .c(new_n27_), .d(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g16(.a(x01), .O(new_n41_));
  aoi21  g17(.a(new_n35_), .b(new_n41_), .c(x02), .O(new_n42_));
  inv1   g18(.a(x00), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  nor2   g22(.a(x10), .b(x02), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  oai21  g24(.a(new_n46_), .b(new_n42_), .c(new_n48_), .O(z2));
  inv1   g25(.a(x11), .O(new_n50_));
  aoi21  g26(.a(x12), .b(new_n43_), .c(new_n47_), .O(new_n51_));
  oai21  g27(.a(new_n50_), .b(new_n44_), .c(new_n51_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z3));
  nand3  g29(.a(x09), .b(x08), .c(x00), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n44_), .O(new_n55_));
  nand2  g31(.a(new_n50_), .b(x07), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n55_), .c(new_n47_), .O(z4));
  nor2   g33(.a(x12), .b(x10), .O(new_n58_));
  nor2   g34(.a(new_n27_), .b(x00), .O(new_n59_));
  nand4  g35(.a(new_n59_), .b(new_n58_), .c(x13), .d(new_n44_), .O(z5));
  aoi21  g36(.a(x14), .b(x01), .c(x00), .O(new_n61_));
  nor2   g37(.a(x08), .b(new_n43_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n61_), .c(new_n44_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(x02), .c(x10), .O(z6));
  inv1   g40(.a(x10), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(x08), .c(new_n44_), .d(x02), .O(z7));
  nor2   g42(.a(x10), .b(x09), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n44_), .c(x02), .d(x00), .O(z8));
endmodule


