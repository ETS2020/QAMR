// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(new_n25_), .O(new_n31_));
  aoi22  g07(.a(new_n31_), .b(x01), .c(new_n29_), .d(x03), .O(new_n32_));
  inv1   g08(.a(x08), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x00), .O(new_n34_));
  oai21  g10(.a(new_n32_), .b(x00), .c(new_n34_), .O(z0));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(new_n27_), .b(new_n36_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n25_), .c(x01), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n39_), .c(x00), .O(z1));
  inv1   g17(.a(x01), .O(new_n42_));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(x03), .O(new_n45_));
  nand3  g21(.a(new_n44_), .b(x09), .c(new_n43_), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n25_), .c(x00), .O(z2));
  inv1   g25(.a(x12), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  oai21  g27(.a(new_n50_), .b(x00), .c(new_n51_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n34_), .O(z3));
  inv1   g29(.a(x00), .O(new_n54_));
  nand2  g30(.a(new_n51_), .b(new_n54_), .O(new_n55_));
  inv1   g31(.a(x09), .O(new_n56_));
  nor2   g32(.a(x11), .b(new_n43_), .O(new_n57_));
  aoi21  g33(.a(new_n56_), .b(new_n43_), .c(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n33_), .c(new_n55_), .O(z4));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(x13), .c(new_n50_), .d(new_n44_), .O(z5));
  nand2  g37(.a(x14), .b(x02), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(x01), .O(new_n63_));
  xor2a  g39(.a(x09), .b(x03), .O(new_n64_));
  oai21  g40(.a(new_n64_), .b(x02), .c(new_n42_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n44_), .c(new_n43_), .d(new_n54_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(z6));
  nand4  g44(.a(x03), .b(new_n25_), .c(new_n42_), .d(new_n54_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n33_), .O(new_n70_));
  nor2   g46(.a(x02), .b(x01), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(x09), .c(x03), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n70_), .c(new_n44_), .d(new_n43_), .O(z7));
  oai21  g49(.a(new_n56_), .b(new_n33_), .c(x00), .O(new_n74_));
  nand3  g50(.a(new_n71_), .b(new_n50_), .c(x03), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  oai21  g52(.a(x10), .b(x07), .c(new_n34_), .O(new_n77_));
  nand2  g53(.a(new_n56_), .b(new_n54_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(z8));
endmodule


