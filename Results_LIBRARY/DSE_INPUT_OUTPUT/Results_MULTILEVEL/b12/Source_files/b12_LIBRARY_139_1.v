// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(new_n27_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(x01), .c(x08), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  nor2   g14(.a(new_n34_), .b(x02), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(x01), .c(x08), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(z1));
  oai21  g17(.a(x08), .b(x07), .c(x00), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x02), .O(new_n43_));
  inv1   g19(.a(x00), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand3  g21(.a(x07), .b(new_n34_), .c(new_n25_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g24(.a(x07), .O(new_n49_));
  inv1   g25(.a(x09), .O(new_n50_));
  nand2  g26(.a(x10), .b(new_n34_), .O(new_n51_));
  oai21  g27(.a(x10), .b(new_n50_), .c(new_n51_), .O(new_n52_));
  nand4  g28(.a(new_n52_), .b(new_n45_), .c(new_n49_), .d(new_n25_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n48_), .c(new_n43_), .O(z2));
  inv1   g30(.a(x11), .O(new_n55_));
  inv1   g31(.a(x12), .O(new_n56_));
  oai22  g32(.a(new_n56_), .b(x00), .c(new_n55_), .d(new_n49_), .O(new_n57_));
  nand2  g33(.a(x08), .b(new_n44_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n57_), .O(z3));
  nand3  g35(.a(x09), .b(x08), .c(x00), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n49_), .O(new_n61_));
  nand2  g37(.a(new_n55_), .b(x07), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n61_), .c(new_n58_), .O(z4));
  inv1   g39(.a(x10), .O(new_n64_));
  nand3  g40(.a(x13), .b(new_n56_), .c(new_n64_), .O(new_n65_));
  inv1   g41(.a(new_n65_), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n45_), .c(new_n49_), .d(new_n44_), .O(z5));
  nand2  g43(.a(x14), .b(x02), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x00), .c(x01), .O(new_n69_));
  xor2a  g45(.a(x09), .b(x03), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(x02), .c(new_n25_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n64_), .c(new_n45_), .d(new_n49_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n58_), .O(z6));
  nand2  g50(.a(new_n39_), .b(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x09), .O(new_n77_));
  nand2  g53(.a(new_n75_), .b(new_n45_), .O(new_n78_));
  nand2  g54(.a(new_n58_), .b(new_n64_), .O(new_n79_));
  nor2   g55(.a(new_n79_), .b(x07), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(z7));
  nand2  g57(.a(x09), .b(x00), .O(new_n82_));
  oai21  g58(.a(x08), .b(x00), .c(new_n82_), .O(new_n83_));
  nand4  g59(.a(new_n56_), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g61(.a(x10), .b(x07), .c(new_n58_), .O(new_n86_));
  nand3  g62(.a(new_n50_), .b(new_n45_), .c(new_n44_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(z8));
endmodule


