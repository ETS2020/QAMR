// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x08), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x06), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x02), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n30_), .c(x02), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n32_), .c(new_n26_), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(x00), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  inv1   g14(.a(x06), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(new_n28_), .c(x01), .O(new_n40_));
  nand3  g16(.a(x06), .b(x05), .c(x01), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n40_), .c(new_n27_), .O(new_n43_));
  nand3  g19(.a(new_n41_), .b(x04), .c(x02), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(new_n43_), .c(new_n38_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n26_), .O(z1));
  oai21  g23(.a(x08), .b(x07), .c(x00), .O(new_n48_));
  nand3  g24(.a(x10), .b(new_n27_), .c(new_n33_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n28_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g27(.a(x07), .b(new_n27_), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  inv1   g29(.a(x10), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n52_), .c(x00), .O(new_n56_));
  nand4  g32(.a(new_n54_), .b(x09), .c(new_n25_), .d(new_n53_), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n33_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n51_), .c(new_n26_), .O(z2));
  inv1   g36(.a(x11), .O(new_n61_));
  inv1   g37(.a(x12), .O(new_n62_));
  oai22  g38(.a(new_n62_), .b(x00), .c(new_n61_), .d(new_n53_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n26_), .O(z3));
  oai21  g40(.a(new_n53_), .b(x06), .c(new_n25_), .O(new_n65_));
  inv1   g41(.a(x09), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(new_n38_), .c(new_n53_), .O(new_n67_));
  nand2  g43(.a(new_n61_), .b(x07), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(z4));
  nand3  g45(.a(x13), .b(new_n62_), .c(new_n54_), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n26_), .c(new_n53_), .d(new_n38_), .O(z5));
  nor2   g48(.a(x09), .b(x06), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(x08), .c(x02), .O(new_n74_));
  nand3  g50(.a(new_n26_), .b(new_n66_), .c(x03), .O(new_n75_));
  nand3  g51(.a(x09), .b(x08), .c(new_n27_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  nand2  g53(.a(x14), .b(x02), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n26_), .c(x01), .O(new_n79_));
  inv1   g55(.a(new_n79_), .O(new_n80_));
  aoi21  g56(.a(new_n77_), .b(new_n33_), .c(new_n80_), .O(new_n81_));
  oai21  g57(.a(new_n27_), .b(x02), .c(x09), .O(new_n82_));
  nor2   g58(.a(new_n82_), .b(x01), .O(new_n83_));
  oai21  g59(.a(x03), .b(x02), .c(new_n66_), .O(new_n84_));
  aoi21  g60(.a(new_n84_), .b(new_n33_), .c(new_n38_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n83_), .c(new_n25_), .O(new_n86_));
  oai22  g62(.a(new_n86_), .b(x06), .c(new_n81_), .d(x00), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n54_), .c(new_n53_), .O(new_n88_));
  inv1   g64(.a(new_n88_), .O(z6));
  oai21  g65(.a(x10), .b(x07), .c(new_n26_), .O(new_n90_));
  nand4  g66(.a(x08), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n91_));
  oai21  g67(.a(x08), .b(x06), .c(new_n91_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  nand3  g69(.a(x03), .b(new_n28_), .c(new_n33_), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n25_), .c(new_n39_), .O(new_n95_));
  nand3  g71(.a(new_n95_), .b(new_n93_), .c(new_n90_), .O(z7));
  nand4  g72(.a(new_n62_), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n97_));
  nand2  g73(.a(new_n97_), .b(x09), .O(new_n98_));
  aoi21  g74(.a(new_n66_), .b(new_n38_), .c(x10), .O(new_n99_));
  nand4  g75(.a(new_n99_), .b(new_n98_), .c(new_n26_), .d(new_n53_), .O(z8));
endmodule


