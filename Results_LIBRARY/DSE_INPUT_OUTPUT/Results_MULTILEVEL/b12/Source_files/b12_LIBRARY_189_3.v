// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  nand2  g06(.a(x08), .b(x05), .O(new_n31_));
  oai21  g07(.a(new_n30_), .b(x00), .c(new_n31_), .O(z0));
  inv1   g08(.a(x01), .O(new_n33_));
  inv1   g09(.a(x08), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n25_), .c(new_n28_), .O(new_n35_));
  nand2  g11(.a(x06), .b(x01), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n34_), .c(new_n28_), .O(new_n37_));
  oai22  g13(.a(new_n37_), .b(new_n25_), .c(new_n35_), .d(new_n33_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nor2   g16(.a(new_n37_), .b(new_n40_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n39_), .c(x00), .O(z1));
  inv1   g19(.a(x00), .O(new_n44_));
  inv1   g20(.a(x03), .O(new_n45_));
  nand3  g21(.a(x07), .b(new_n45_), .c(new_n33_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  nand2  g24(.a(x10), .b(new_n45_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n28_), .c(new_n33_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n44_), .O(new_n56_));
  nor2   g32(.a(new_n51_), .b(x03), .O(new_n57_));
  inv1   g33(.a(x09), .O(new_n58_));
  nor2   g34(.a(x10), .b(new_n58_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n57_), .c(new_n33_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n25_), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n34_), .c(new_n50_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n56_), .O(z2));
  inv1   g39(.a(x12), .O(new_n64_));
  nand2  g40(.a(x11), .b(x07), .O(new_n65_));
  oai21  g41(.a(new_n64_), .b(x00), .c(new_n65_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n31_), .O(z3));
  nand2  g43(.a(new_n65_), .b(new_n34_), .O(new_n68_));
  inv1   g44(.a(x11), .O(new_n69_));
  aoi21  g45(.a(x09), .b(x00), .c(x07), .O(new_n70_));
  aoi21  g46(.a(new_n69_), .b(x07), .c(new_n70_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(x05), .c(new_n68_), .O(z4));
  nand3  g48(.a(x13), .b(new_n64_), .c(new_n51_), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n31_), .c(new_n50_), .d(new_n44_), .O(z5));
  nand2  g51(.a(x08), .b(x00), .O(new_n76_));
  nand2  g52(.a(x09), .b(new_n45_), .O(new_n77_));
  nand2  g53(.a(new_n58_), .b(x03), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n77_), .c(new_n25_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n76_), .c(new_n33_), .O(new_n80_));
  aoi21  g56(.a(x14), .b(x02), .c(x00), .O(new_n81_));
  nor2   g57(.a(x08), .b(new_n44_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n81_), .c(x01), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n51_), .c(new_n50_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n31_), .O(z6));
  nand3  g62(.a(x03), .b(new_n25_), .c(new_n33_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(x08), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(x09), .O(new_n89_));
  nand3  g65(.a(new_n31_), .b(new_n51_), .c(new_n50_), .O(new_n90_));
  aoi21  g66(.a(new_n87_), .b(new_n34_), .c(new_n90_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n89_), .O(z7));
  inv1   g68(.a(new_n31_), .O(new_n93_));
  nand4  g69(.a(new_n64_), .b(x03), .c(new_n25_), .d(new_n33_), .O(new_n94_));
  nand2  g70(.a(new_n94_), .b(x09), .O(new_n95_));
  nor2   g71(.a(x09), .b(x00), .O(new_n96_));
  nor3   g72(.a(new_n96_), .b(x10), .c(x07), .O(new_n97_));
  aoi21  g73(.a(new_n97_), .b(new_n95_), .c(new_n93_), .O(z8));
endmodule


