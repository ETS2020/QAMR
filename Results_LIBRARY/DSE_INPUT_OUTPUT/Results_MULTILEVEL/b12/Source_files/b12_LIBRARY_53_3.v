// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_,
    new_n89_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(x04), .c(new_n25_), .O(new_n29_));
  aoi22  g05(.a(new_n29_), .b(x01), .c(new_n27_), .d(x03), .O(new_n30_));
  nand2  g06(.a(x14), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n30_), .b(x00), .c(new_n31_), .O(z0));
  inv1   g08(.a(x01), .O(new_n33_));
  oai21  g09(.a(x04), .b(x03), .c(new_n33_), .O(new_n34_));
  inv1   g10(.a(x14), .O(new_n35_));
  nand2  g11(.a(x06), .b(x05), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n35_), .c(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(x02), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x02), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n35_), .c(x03), .d(x01), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n39_), .c(x00), .O(z1));
  oai21  g18(.a(x08), .b(x07), .c(x00), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(x03), .c(new_n25_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  inv1   g22(.a(x00), .O(new_n47_));
  inv1   g23(.a(x03), .O(new_n48_));
  nand2  g24(.a(x07), .b(new_n48_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  nand3  g26(.a(new_n44_), .b(x09), .c(new_n50_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  nand4  g30(.a(new_n44_), .b(x09), .c(new_n54_), .d(new_n50_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n53_), .c(new_n46_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n33_), .O(new_n57_));
  nand3  g33(.a(new_n43_), .b(new_n35_), .c(x02), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n57_), .O(z2));
  inv1   g35(.a(x11), .O(new_n60_));
  inv1   g36(.a(x12), .O(new_n61_));
  oai22  g37(.a(new_n61_), .b(x00), .c(new_n60_), .d(new_n50_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n31_), .O(z3));
  inv1   g39(.a(new_n31_), .O(new_n64_));
  nand3  g40(.a(x09), .b(x08), .c(x00), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n50_), .O(new_n66_));
  nand2  g42(.a(new_n60_), .b(x07), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(z4));
  nand3  g44(.a(x13), .b(new_n61_), .c(new_n44_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n31_), .c(new_n50_), .d(new_n47_), .O(z5));
  nand2  g47(.a(x08), .b(x00), .O(new_n72_));
  xor2a  g48(.a(x09), .b(x03), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(x02), .c(new_n33_), .O(new_n74_));
  nand2  g50(.a(new_n35_), .b(x01), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n72_), .c(new_n44_), .d(new_n50_), .O(new_n77_));
  inv1   g53(.a(new_n77_), .O(z6));
  oai21  g54(.a(x14), .b(new_n54_), .c(x01), .O(new_n79_));
  oai21  g55(.a(new_n48_), .b(x02), .c(new_n54_), .O(new_n80_));
  nor2   g56(.a(x02), .b(x01), .O(new_n81_));
  inv1   g57(.a(x09), .O(new_n82_));
  nor2   g58(.a(new_n82_), .b(new_n48_), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n81_), .c(x10), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n80_), .c(new_n79_), .d(new_n50_), .O(z7));
  oai21  g61(.a(x14), .b(x09), .c(x01), .O(new_n86_));
  nand3  g62(.a(new_n61_), .b(x03), .c(new_n25_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(x09), .O(new_n88_));
  aoi21  g64(.a(new_n82_), .b(new_n47_), .c(x10), .O(new_n89_));
  nand4  g65(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n50_), .O(z8));
endmodule


