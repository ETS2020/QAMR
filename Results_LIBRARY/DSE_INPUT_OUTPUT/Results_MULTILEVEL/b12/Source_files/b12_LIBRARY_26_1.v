// Benchmark "FAU" written by ABC on Thu Aug 13 17:51:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  oai21  g03(.a(x05), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n27_), .b(x03), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n26_), .c(x03), .O(new_n32_));
  nor2   g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g09(.a(new_n33_), .b(new_n30_), .c(new_n25_), .O(new_n34_));
  inv1   g10(.a(x05), .O(new_n35_));
  nand2  g11(.a(x10), .b(new_n35_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(z0));
  nand3  g13(.a(new_n36_), .b(new_n26_), .c(x01), .O(new_n38_));
  nand2  g14(.a(x06), .b(x01), .O(new_n39_));
  nor2   g15(.a(x10), .b(x05), .O(new_n40_));
  aoi21  g16(.a(new_n39_), .b(x05), .c(new_n40_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n26_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x03), .O(new_n43_));
  nor2   g19(.a(new_n41_), .b(new_n27_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x02), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n43_), .c(x00), .O(z1));
  oai21  g22(.a(x08), .b(x07), .c(x00), .O(new_n47_));
  inv1   g23(.a(x03), .O(new_n48_));
  nand3  g24(.a(x10), .b(new_n48_), .c(new_n31_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n26_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand2  g27(.a(x07), .b(new_n48_), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  inv1   g29(.a(x10), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n52_), .c(x00), .O(new_n56_));
  inv1   g32(.a(x08), .O(new_n57_));
  nand4  g33(.a(new_n54_), .b(x09), .c(new_n57_), .d(new_n53_), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n56_), .c(new_n31_), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n51_), .c(new_n36_), .O(z2));
  inv1   g37(.a(x11), .O(new_n62_));
  inv1   g38(.a(x12), .O(new_n63_));
  oai22  g39(.a(new_n63_), .b(x00), .c(new_n62_), .d(new_n53_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n36_), .O(z3));
  nand2  g41(.a(x09), .b(x08), .O(new_n66_));
  oai22  g42(.a(new_n66_), .b(new_n25_), .c(new_n54_), .d(x05), .O(new_n67_));
  nand3  g43(.a(new_n36_), .b(new_n62_), .c(x07), .O(new_n68_));
  oai21  g44(.a(new_n67_), .b(x07), .c(new_n68_), .O(z4));
  oai21  g45(.a(x07), .b(x00), .c(new_n36_), .O(new_n70_));
  nand2  g46(.a(x10), .b(x05), .O(new_n71_));
  inv1   g47(.a(x13), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(x12), .c(new_n54_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n53_), .c(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n70_), .O(z5));
  nand2  g52(.a(x08), .b(x00), .O(new_n77_));
  nand2  g53(.a(x09), .b(new_n48_), .O(new_n78_));
  inv1   g54(.a(x09), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(x03), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n78_), .c(new_n26_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n77_), .c(new_n31_), .O(new_n82_));
  aoi21  g58(.a(x14), .b(x02), .c(x00), .O(new_n83_));
  nor2   g59(.a(x08), .b(new_n25_), .O(new_n84_));
  oai21  g60(.a(new_n84_), .b(new_n83_), .c(x01), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand3  g62(.a(new_n86_), .b(new_n54_), .c(new_n53_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(new_n36_), .O(z6));
  nand3  g64(.a(x03), .b(new_n26_), .c(new_n31_), .O(new_n89_));
  nand4  g65(.a(x09), .b(x03), .c(new_n26_), .d(new_n31_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n53_), .O(new_n91_));
  aoi21  g67(.a(new_n89_), .b(new_n57_), .c(new_n91_), .O(new_n92_));
  oai21  g68(.a(new_n92_), .b(x10), .c(new_n71_), .O(z7));
  nand4  g69(.a(new_n63_), .b(x03), .c(new_n26_), .d(new_n31_), .O(new_n94_));
  oai21  g70(.a(x09), .b(x00), .c(new_n53_), .O(new_n95_));
  aoi21  g71(.a(new_n94_), .b(x09), .c(new_n95_), .O(new_n96_));
  oai21  g72(.a(new_n96_), .b(x10), .c(new_n71_), .O(z8));
endmodule


