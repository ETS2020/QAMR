// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  oai21  g03(.a(x05), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n27_), .b(x03), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n26_), .c(x03), .O(new_n32_));
  nor2   g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g09(.a(new_n33_), .b(new_n30_), .c(new_n25_), .O(new_n34_));
  nor2   g10(.a(x10), .b(x03), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(z0));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n27_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(x02), .O(new_n41_));
  nand3  g17(.a(x03), .b(new_n26_), .c(x01), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n36_), .O(z1));
  oai21  g21(.a(x08), .b(x07), .c(x00), .O(new_n46_));
  nand3  g22(.a(x10), .b(new_n38_), .c(new_n31_), .O(new_n47_));
  oai21  g23(.a(new_n35_), .b(new_n26_), .c(new_n47_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand2  g27(.a(x08), .b(x00), .O(new_n52_));
  nand4  g28(.a(new_n52_), .b(new_n51_), .c(x09), .d(new_n50_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(x03), .c(new_n31_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n49_), .O(z2));
  inv1   g32(.a(x11), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  oai22  g34(.a(new_n58_), .b(x00), .c(new_n57_), .d(new_n50_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n36_), .O(z3));
  nand3  g36(.a(x09), .b(x08), .c(x00), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n50_), .O(new_n62_));
  nand2  g38(.a(new_n57_), .b(x07), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n36_), .O(z4));
  nand4  g40(.a(x13), .b(new_n58_), .c(new_n50_), .d(new_n25_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(x03), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n51_), .O(z5));
  nand2  g43(.a(x09), .b(new_n26_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n52_), .c(new_n31_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand2  g46(.a(x14), .b(x02), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n25_), .O(new_n72_));
  inv1   g48(.a(x08), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x00), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n72_), .c(new_n31_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n70_), .c(new_n50_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(x03), .c(x10), .O(z6));
  oai21  g53(.a(x02), .b(x01), .c(new_n73_), .O(new_n78_));
  nor2   g54(.a(x07), .b(new_n38_), .O(new_n79_));
  nand3  g55(.a(x09), .b(new_n26_), .c(new_n31_), .O(new_n80_));
  nand4  g56(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n51_), .O(z7));
  inv1   g57(.a(x09), .O(new_n82_));
  nor2   g58(.a(x12), .b(x02), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n31_), .c(new_n82_), .O(new_n84_));
  oai21  g60(.a(x09), .b(x00), .c(new_n50_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n84_), .c(x03), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n51_), .O(z8));
endmodule


