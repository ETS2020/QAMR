// Benchmark "FAU" written by ABC on Thu Aug 13 17:52:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  oai21  g03(.a(x05), .b(new_n27_), .c(x01), .O(new_n28_));
  nand3  g04(.a(x06), .b(x05), .c(x01), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(new_n31_));
  nand3  g07(.a(x03), .b(new_n26_), .c(x01), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(new_n33_));
  oai21  g09(.a(new_n33_), .b(new_n31_), .c(new_n25_), .O(new_n34_));
  nor2   g10(.a(x10), .b(new_n27_), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(z0));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n27_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n29_), .c(x02), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n32_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n36_), .O(z1));
  oai21  g19(.a(x08), .b(x07), .c(x00), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  nand3  g21(.a(x10), .b(new_n38_), .c(new_n45_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n26_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand2  g24(.a(x07), .b(new_n38_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n49_), .c(x00), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  nand4  g30(.a(new_n51_), .b(x09), .c(new_n54_), .d(new_n50_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n53_), .c(new_n45_), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n48_), .c(new_n36_), .O(z2));
  inv1   g34(.a(x11), .O(new_n59_));
  inv1   g35(.a(x12), .O(new_n60_));
  oai22  g36(.a(new_n60_), .b(x00), .c(new_n59_), .d(new_n50_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n36_), .O(z3));
  nand3  g38(.a(x09), .b(x08), .c(x00), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  nand2  g40(.a(new_n59_), .b(x07), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n64_), .c(new_n35_), .O(z4));
  nor3   g42(.a(x07), .b(x04), .c(x00), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(x13), .c(new_n60_), .d(new_n51_), .O(z5));
  nand2  g44(.a(x08), .b(x00), .O(new_n69_));
  nand2  g45(.a(x09), .b(new_n38_), .O(new_n70_));
  inv1   g46(.a(x09), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x03), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n26_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n69_), .c(new_n45_), .O(new_n74_));
  aoi21  g50(.a(x14), .b(x02), .c(x00), .O(new_n75_));
  nor2   g51(.a(x08), .b(new_n25_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n50_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n27_), .c(x10), .O(z6));
  nand3  g56(.a(x03), .b(new_n26_), .c(new_n45_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n54_), .O(new_n82_));
  nor2   g58(.a(x07), .b(x04), .O(new_n83_));
  nand4  g59(.a(x09), .b(x03), .c(new_n26_), .d(new_n45_), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n51_), .O(z7));
  nand4  g61(.a(new_n60_), .b(x03), .c(new_n26_), .d(new_n45_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(x09), .O(new_n87_));
  aoi21  g63(.a(new_n71_), .b(new_n25_), .c(x10), .O(new_n88_));
  nand3  g64(.a(new_n88_), .b(new_n87_), .c(new_n83_), .O(z8));
endmodule


