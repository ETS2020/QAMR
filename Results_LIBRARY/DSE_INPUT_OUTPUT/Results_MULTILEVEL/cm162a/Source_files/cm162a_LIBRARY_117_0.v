// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  inv1   g02(.a(x12), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  oai21  g04(.a(x03), .b(x00), .c(x05), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  and2   g08(.a(x04), .b(x02), .O(new_n28_));
  nor2   g09(.a(new_n22_), .b(x08), .O(new_n29_));
  aoi22  g10(.a(new_n29_), .b(new_n28_), .c(new_n27_), .d(x08), .O(new_n30_));
  oai21  g11(.a(new_n30_), .b(new_n20_), .c(new_n25_), .O(z0));
  oai21  g12(.a(x03), .b(x01), .c(x05), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand3  g15(.a(x12), .b(new_n34_), .c(new_n21_), .O(new_n35_));
  nand3  g16(.a(x13), .b(x09), .c(x08), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(x04), .c(x02), .O(new_n38_));
  inv1   g19(.a(new_n38_), .O(new_n39_));
  nand2  g20(.a(new_n27_), .b(x12), .O(new_n40_));
  aoi21  g21(.a(new_n40_), .b(new_n21_), .c(new_n34_), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n39_), .c(x03), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n33_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n34_), .c(new_n21_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n26_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  nand3  g28(.a(new_n21_), .b(x04), .c(x02), .O(new_n48_));
  nand3  g29(.a(new_n26_), .b(x12), .c(new_n34_), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n48_), .c(x11), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x03), .O(new_n52_));
  inv1   g33(.a(x06), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n20_), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n52_), .c(new_n23_), .d(x05), .O(z2));
  inv1   g36(.a(x09), .O(new_n56_));
  inv1   g37(.a(x13), .O(new_n57_));
  aoi21  g38(.a(x04), .b(x02), .c(x12), .O(new_n58_));
  nor3   g39(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  inv1   g40(.a(new_n45_), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(new_n28_), .c(new_n22_), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(new_n59_), .c(x03), .O(new_n62_));
  inv1   g43(.a(x07), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n20_), .O(new_n64_));
  nand4  g45(.a(new_n64_), .b(new_n62_), .c(new_n23_), .d(x05), .O(z3));
  inv1   g46(.a(x04), .O(new_n66_));
  oai21  g47(.a(new_n26_), .b(new_n66_), .c(new_n23_), .O(z4));
endmodule


