// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_;
  inv1   g00(.a(x06), .O(new_n20_));
  inv1   g01(.a(x09), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(x03), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n25_), .c(new_n23_), .O(new_n28_));
  oai21  g09(.a(x03), .b(x00), .c(x05), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(new_n22_), .O(new_n30_));
  nand2  g11(.a(x08), .b(x03), .O(new_n31_));
  nand2  g12(.a(x13), .b(x09), .O(new_n32_));
  oai21  g13(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z0));
  nand2  g14(.a(new_n27_), .b(x10), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n34_), .c(new_n23_), .O(new_n37_));
  oai21  g18(.a(x03), .b(x01), .c(x05), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n37_), .c(new_n22_), .O(new_n39_));
  inv1   g20(.a(x10), .O(new_n40_));
  nand2  g21(.a(new_n24_), .b(new_n40_), .O(new_n41_));
  nand4  g22(.a(new_n41_), .b(x13), .c(x09), .d(x03), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n39_), .O(z1));
  nand2  g24(.a(new_n36_), .b(x11), .O(new_n44_));
  inv1   g25(.a(new_n24_), .O(new_n45_));
  nor3   g26(.a(x11), .b(x10), .c(x08), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  aoi22  g28(.a(new_n47_), .b(new_n44_), .c(new_n21_), .d(new_n20_), .O(new_n48_));
  oai21  g29(.a(new_n45_), .b(x11), .c(x13), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(x05), .c(new_n21_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n48_), .c(x03), .O(new_n51_));
  inv1   g32(.a(x05), .O(new_n52_));
  nor2   g33(.a(new_n21_), .b(x06), .O(new_n53_));
  aoi22  g34(.a(new_n53_), .b(new_n23_), .c(x06), .d(new_n52_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n51_), .O(z2));
  inv1   g36(.a(x02), .O(new_n56_));
  nor2   g37(.a(x12), .b(x11), .O(new_n57_));
  aoi22  g38(.a(new_n57_), .b(new_n35_), .c(x13), .d(x09), .O(new_n58_));
  nand3  g39(.a(x13), .b(x12), .c(x09), .O(new_n59_));
  oai21  g40(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  inv1   g41(.a(x12), .O(new_n61_));
  aoi21  g42(.a(new_n46_), .b(new_n45_), .c(new_n61_), .O(new_n62_));
  aoi21  g43(.a(new_n60_), .b(x04), .c(new_n62_), .O(new_n63_));
  oai21  g44(.a(x07), .b(x03), .c(new_n22_), .O(new_n64_));
  nor2   g45(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  oai21  g46(.a(new_n63_), .b(new_n23_), .c(new_n65_), .O(z3));
  inv1   g47(.a(x04), .O(new_n67_));
  oai21  g48(.a(new_n32_), .b(new_n67_), .c(new_n22_), .O(z4));
endmodule


