// Benchmark "FAU" written by ABC on Thu Aug 13 15:05:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand3  g01(.a(new_n20_), .b(x03), .c(x02), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x12), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x04), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  nand4  g05(.a(new_n24_), .b(x12), .c(x04), .d(x02), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x08), .c(x03), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand4  g10(.a(new_n29_), .b(new_n26_), .c(new_n23_), .d(x05), .O(z0));
  inv1   g11(.a(x12), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x04), .O(new_n32_));
  oai21  g13(.a(x03), .b(x01), .c(x05), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  oai21  g15(.a(x10), .b(x08), .c(new_n24_), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  nand3  g17(.a(new_n24_), .b(new_n20_), .c(x02), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x10), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n36_), .c(new_n31_), .O(new_n39_));
  inv1   g20(.a(x10), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(x04), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n39_), .c(x03), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n34_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n40_), .c(new_n20_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n24_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  nand4  g28(.a(new_n24_), .b(new_n40_), .c(new_n20_), .d(x02), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x11), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n47_), .c(new_n28_), .O(new_n50_));
  inv1   g31(.a(x06), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n28_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x05), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n50_), .c(x12), .O(new_n54_));
  inv1   g35(.a(x04), .O(new_n55_));
  nand2  g36(.a(x11), .b(x03), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n52_), .c(x05), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n54_), .O(z2));
  aoi21  g40(.a(x13), .b(x09), .c(x11), .O(new_n60_));
  nand4  g41(.a(new_n60_), .b(new_n40_), .c(new_n20_), .d(x02), .O(new_n61_));
  nor2   g42(.a(new_n31_), .b(x04), .O(new_n62_));
  aoi21  g43(.a(new_n61_), .b(x04), .c(new_n62_), .O(new_n63_));
  inv1   g44(.a(x07), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n28_), .O(new_n65_));
  nand3  g46(.a(new_n65_), .b(new_n32_), .c(x05), .O(new_n66_));
  inv1   g47(.a(new_n66_), .O(new_n67_));
  oai21  g48(.a(new_n63_), .b(new_n28_), .c(new_n67_), .O(z3));
  nand4  g49(.a(x13), .b(x12), .c(x09), .d(x04), .O(new_n69_));
  inv1   g50(.a(new_n69_), .O(z4));
endmodule


