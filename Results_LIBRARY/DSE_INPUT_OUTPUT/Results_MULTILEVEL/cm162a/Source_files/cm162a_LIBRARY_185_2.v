// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  inv1   g00(.a(x11), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x04), .O(new_n21_));
  oai21  g02(.a(x03), .b(x00), .c(x05), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g04(.a(x04), .O(new_n24_));
  nand2  g05(.a(x13), .b(x09), .O(new_n25_));
  aoi21  g06(.a(new_n25_), .b(x02), .c(new_n20_), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n24_), .c(x08), .O(new_n27_));
  inv1   g08(.a(x08), .O(new_n28_));
  nand4  g09(.a(x11), .b(new_n28_), .c(x04), .d(x02), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x03), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n23_), .O(z0));
  oai21  g13(.a(x03), .b(x01), .c(x05), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n21_), .O(new_n34_));
  oai21  g15(.a(x10), .b(x08), .c(new_n25_), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  nand3  g17(.a(new_n25_), .b(new_n28_), .c(x02), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x10), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n36_), .c(new_n20_), .O(new_n39_));
  inv1   g20(.a(x10), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(x04), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n39_), .c(x03), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n34_), .O(z1));
  inv1   g24(.a(x03), .O(new_n44_));
  nand4  g25(.a(new_n25_), .b(new_n40_), .c(new_n28_), .d(x02), .O(new_n45_));
  nor2   g26(.a(new_n20_), .b(x04), .O(new_n46_));
  aoi21  g27(.a(new_n45_), .b(x04), .c(new_n46_), .O(new_n47_));
  inv1   g28(.a(x06), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n21_), .c(x05), .O(new_n50_));
  inv1   g31(.a(new_n50_), .O(new_n51_));
  oai21  g32(.a(new_n47_), .b(new_n44_), .c(new_n51_), .O(z2));
  inv1   g33(.a(x07), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  nand2  g35(.a(x12), .b(x03), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n54_), .c(x05), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n21_), .O(new_n57_));
  nand3  g38(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  nand3  g39(.a(x13), .b(x11), .c(x09), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(z3));
  nand4  g41(.a(x13), .b(x11), .c(x09), .d(x04), .O(new_n61_));
  inv1   g42(.a(new_n61_), .O(z4));
endmodule


