// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_;
  inv1   g00(.a(x00), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  oai21  g02(.a(x08), .b(new_n20_), .c(new_n21_), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand4  g07(.a(new_n26_), .b(x04), .c(x03), .d(x02), .O(new_n27_));
  nand4  g08(.a(new_n27_), .b(new_n25_), .c(new_n22_), .d(x05), .O(z0));
  oai21  g09(.a(x10), .b(x08), .c(new_n23_), .O(new_n29_));
  nand3  g10(.a(new_n29_), .b(x04), .c(x02), .O(new_n30_));
  nand2  g11(.a(new_n24_), .b(x10), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(x03), .O(new_n33_));
  aoi21  g14(.a(new_n26_), .b(x01), .c(x03), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n26_), .c(x05), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n33_), .O(z1));
  inv1   g19(.a(x11), .O(new_n39_));
  nand3  g20(.a(new_n39_), .b(new_n35_), .c(new_n26_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n23_), .O(new_n41_));
  nand3  g22(.a(new_n41_), .b(x04), .c(x02), .O(new_n42_));
  nand2  g23(.a(x04), .b(x02), .O(new_n43_));
  nand3  g24(.a(new_n23_), .b(new_n35_), .c(new_n26_), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(new_n43_), .c(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n42_), .c(x05), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x03), .O(new_n47_));
  oai21  g28(.a(x06), .b(x03), .c(x05), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n26_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n47_), .O(z2));
  inv1   g31(.a(x02), .O(new_n51_));
  nor2   g32(.a(x10), .b(x08), .O(new_n52_));
  nor2   g33(.a(x12), .b(x11), .O(new_n53_));
  aoi22  g34(.a(new_n53_), .b(new_n52_), .c(x13), .d(x09), .O(new_n54_));
  nand3  g35(.a(x13), .b(x12), .c(x09), .O(new_n55_));
  oai21  g36(.a(new_n54_), .b(new_n51_), .c(new_n55_), .O(new_n56_));
  oai21  g37(.a(new_n43_), .b(new_n40_), .c(x12), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x05), .O(new_n58_));
  aoi21  g39(.a(new_n56_), .b(x04), .c(new_n58_), .O(new_n59_));
  oai21  g40(.a(x07), .b(x03), .c(x05), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n26_), .O(new_n61_));
  oai21  g42(.a(new_n59_), .b(new_n21_), .c(new_n61_), .O(z3));
  nand2  g43(.a(x08), .b(new_n21_), .O(new_n63_));
  nand4  g44(.a(new_n63_), .b(x13), .c(x09), .d(x04), .O(new_n64_));
  inv1   g45(.a(new_n64_), .O(z4));
endmodule


