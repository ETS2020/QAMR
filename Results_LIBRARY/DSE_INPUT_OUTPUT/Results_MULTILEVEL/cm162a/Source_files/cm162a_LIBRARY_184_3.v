// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_;
  nand2  g00(.a(x07), .b(x04), .O(new_n20_));
  oai21  g01(.a(x03), .b(x00), .c(x05), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n25_), .c(x07), .O(new_n28_));
  nor2   g09(.a(new_n26_), .b(x04), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(x03), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n22_), .O(z0));
  oai21  g12(.a(x03), .b(x01), .c(x05), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n20_), .O(new_n33_));
  oai21  g14(.a(x10), .b(x08), .c(new_n23_), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand3  g16(.a(new_n23_), .b(new_n26_), .c(x02), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x10), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n35_), .c(x07), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  nor2   g20(.a(new_n39_), .b(x04), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n38_), .c(x03), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n33_), .O(z1));
  inv1   g23(.a(x07), .O(new_n43_));
  inv1   g24(.a(x03), .O(new_n44_));
  inv1   g25(.a(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n39_), .c(new_n26_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n23_), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(x04), .c(x02), .O(new_n48_));
  nand2  g29(.a(new_n26_), .b(x02), .O(new_n49_));
  nand2  g30(.a(new_n23_), .b(new_n39_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n49_), .c(x11), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n48_), .c(new_n44_), .O(new_n52_));
  inv1   g33(.a(x06), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x05), .O(new_n55_));
  oai21  g36(.a(new_n55_), .b(new_n52_), .c(new_n43_), .O(new_n56_));
  inv1   g37(.a(x04), .O(new_n57_));
  nand2  g38(.a(x11), .b(x03), .O(new_n58_));
  nand3  g39(.a(new_n58_), .b(new_n54_), .c(x05), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n56_), .O(z2));
  inv1   g42(.a(x12), .O(new_n62_));
  nand4  g43(.a(new_n62_), .b(new_n45_), .c(new_n39_), .d(new_n26_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n23_), .O(new_n64_));
  nand3  g45(.a(new_n64_), .b(x04), .c(x02), .O(new_n65_));
  nand3  g46(.a(new_n23_), .b(new_n45_), .c(new_n39_), .O(new_n66_));
  oai21  g47(.a(new_n66_), .b(new_n49_), .c(x12), .O(new_n67_));
  nand4  g48(.a(new_n67_), .b(new_n65_), .c(x05), .d(x03), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n43_), .O(new_n69_));
  oai21  g50(.a(new_n62_), .b(new_n44_), .c(x05), .O(new_n70_));
  nand2  g51(.a(new_n70_), .b(new_n57_), .O(new_n71_));
  nand2  g52(.a(new_n71_), .b(new_n69_), .O(z3));
  aoi21  g53(.a(new_n23_), .b(new_n43_), .c(new_n57_), .O(z4));
endmodule


