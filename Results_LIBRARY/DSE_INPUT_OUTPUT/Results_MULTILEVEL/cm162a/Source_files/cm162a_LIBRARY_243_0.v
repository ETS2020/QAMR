// Benchmark "FAU" written by ABC on Thu Aug 13 15:07:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x04), .c(x02), .O(new_n22_));
  inv1   g03(.a(x04), .O(new_n23_));
  nor2   g04(.a(x08), .b(new_n23_), .O(new_n24_));
  aoi22  g05(.a(new_n24_), .b(x02), .c(new_n22_), .d(x08), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  inv1   g07(.a(x05), .O(new_n27_));
  aoi21  g08(.a(new_n20_), .b(new_n26_), .c(new_n27_), .O(new_n28_));
  oai21  g09(.a(new_n25_), .b(new_n20_), .c(new_n28_), .O(z0));
  oai21  g10(.a(x10), .b(x08), .c(new_n21_), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(x04), .c(x02), .O(new_n31_));
  nand2  g12(.a(new_n22_), .b(x10), .O(new_n32_));
  aoi21  g13(.a(new_n32_), .b(new_n31_), .c(new_n27_), .O(new_n33_));
  inv1   g14(.a(x08), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  nor2   g16(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n33_), .c(x03), .O(new_n37_));
  inv1   g18(.a(x01), .O(new_n38_));
  nor2   g19(.a(new_n27_), .b(x03), .O(new_n39_));
  nor2   g20(.a(new_n34_), .b(x05), .O(new_n40_));
  aoi21  g21(.a(new_n39_), .b(new_n38_), .c(new_n40_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n37_), .O(z1));
  inv1   g23(.a(x11), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n35_), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(x08), .c(new_n21_), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(x04), .c(x02), .O(new_n46_));
  nand2  g27(.a(x04), .b(x02), .O(new_n47_));
  nand2  g28(.a(new_n21_), .b(new_n35_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n47_), .c(x11), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n46_), .c(new_n27_), .O(new_n50_));
  nor2   g31(.a(new_n43_), .b(new_n34_), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n50_), .c(x03), .O(new_n52_));
  nor2   g33(.a(x06), .b(new_n27_), .O(new_n53_));
  aoi21  g34(.a(new_n53_), .b(new_n20_), .c(new_n40_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n52_), .O(z2));
  inv1   g36(.a(x02), .O(new_n56_));
  inv1   g37(.a(x12), .O(new_n57_));
  nand4  g38(.a(new_n57_), .b(new_n43_), .c(new_n35_), .d(new_n34_), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n21_), .c(new_n56_), .O(new_n59_));
  nand3  g40(.a(x13), .b(x12), .c(x09), .O(new_n60_));
  inv1   g41(.a(new_n60_), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(new_n59_), .c(x04), .O(new_n62_));
  oai21  g43(.a(new_n47_), .b(new_n44_), .c(x12), .O(new_n63_));
  aoi21  g44(.a(new_n63_), .b(new_n62_), .c(new_n27_), .O(new_n64_));
  nor2   g45(.a(new_n57_), .b(new_n34_), .O(new_n65_));
  oai21  g46(.a(new_n65_), .b(new_n64_), .c(x03), .O(new_n66_));
  nor2   g47(.a(x07), .b(new_n27_), .O(new_n67_));
  aoi21  g48(.a(new_n67_), .b(new_n20_), .c(new_n40_), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n66_), .O(z3));
  inv1   g50(.a(x09), .O(new_n70_));
  oai21  g51(.a(x08), .b(x05), .c(x13), .O(new_n71_));
  nor3   g52(.a(new_n71_), .b(new_n70_), .c(new_n23_), .O(z4));
endmodule


