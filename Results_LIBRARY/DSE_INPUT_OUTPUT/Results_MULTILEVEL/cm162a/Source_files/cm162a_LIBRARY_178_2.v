// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand3  g01(.a(new_n20_), .b(x03), .c(x02), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x11), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x04), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(new_n25_), .c(x11), .O(new_n27_));
  nand3  g08(.a(new_n27_), .b(x08), .c(x03), .O(new_n28_));
  inv1   g09(.a(x00), .O(new_n29_));
  inv1   g10(.a(x03), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(new_n28_), .c(new_n23_), .d(x05), .O(z0));
  oai21  g13(.a(x10), .b(x08), .c(new_n26_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  nand3  g15(.a(new_n26_), .b(x11), .c(new_n20_), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n24_), .c(x10), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x03), .O(new_n38_));
  nor2   g19(.a(x03), .b(x01), .O(new_n39_));
  inv1   g20(.a(x11), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x04), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x05), .O(new_n42_));
  nor2   g23(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n38_), .O(z1));
  oai21  g25(.a(x06), .b(x03), .c(x05), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n41_), .O(new_n46_));
  inv1   g27(.a(x10), .O(new_n47_));
  nand4  g28(.a(new_n26_), .b(new_n25_), .c(new_n47_), .d(new_n20_), .O(new_n48_));
  nand3  g29(.a(new_n48_), .b(x11), .c(x03), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n46_), .O(z2));
  inv1   g31(.a(x07), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n30_), .O(new_n52_));
  nand2  g33(.a(x12), .b(x03), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n52_), .c(x05), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n41_), .O(new_n55_));
  nand3  g36(.a(x04), .b(x03), .c(x02), .O(new_n56_));
  nand3  g37(.a(x13), .b(x11), .c(x09), .O(new_n57_));
  oai21  g38(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z3));
  nand4  g39(.a(x13), .b(x11), .c(x09), .d(x04), .O(new_n59_));
  inv1   g40(.a(new_n59_), .O(z4));
endmodule


