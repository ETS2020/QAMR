// Benchmark "FAU" written by ABC on Thu Aug 13 15:07:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  nand3  g00(.a(x13), .b(x08), .c(x03), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x10), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x09), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  and2   g04(.a(x04), .b(x02), .O(new_n24_));
  nand3  g05(.a(new_n23_), .b(x04), .c(x02), .O(new_n25_));
  oai21  g06(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(new_n27_), .c(new_n22_), .O(z0));
  inv1   g13(.a(x10), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x09), .O(new_n34_));
  oai21  g15(.a(x03), .b(x01), .c(x05), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g17(.a(x13), .b(x09), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n24_), .c(new_n23_), .O(new_n38_));
  nor3   g19(.a(x10), .b(x09), .c(x08), .O(new_n39_));
  aoi22  g20(.a(new_n39_), .b(new_n24_), .c(new_n38_), .d(x10), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n29_), .c(new_n36_), .O(z1));
  inv1   g22(.a(x04), .O(new_n42_));
  inv1   g23(.a(x09), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand4  g25(.a(new_n44_), .b(new_n33_), .c(new_n43_), .d(new_n23_), .O(new_n45_));
  nand3  g26(.a(x13), .b(x10), .c(x09), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n45_), .c(new_n42_), .O(new_n47_));
  nand2  g28(.a(new_n25_), .b(new_n43_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  aoi22  g30(.a(new_n49_), .b(x11), .c(new_n47_), .d(x02), .O(new_n50_));
  oai21  g31(.a(x06), .b(x03), .c(x05), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  oai21  g33(.a(new_n50_), .b(new_n29_), .c(new_n52_), .O(z2));
  inv1   g34(.a(x12), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n44_), .c(new_n33_), .d(new_n23_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(x04), .c(x02), .O(new_n57_));
  nand3  g38(.a(new_n44_), .b(new_n33_), .c(new_n43_), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n25_), .c(x12), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(x03), .O(new_n61_));
  inv1   g42(.a(x07), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(new_n29_), .c(new_n30_), .O(new_n63_));
  nand3  g44(.a(new_n63_), .b(new_n61_), .c(new_n34_), .O(z3));
  nand4  g45(.a(x13), .b(x10), .c(x09), .d(x04), .O(new_n65_));
  inv1   g46(.a(new_n65_), .O(z4));
endmodule


