// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_;
  inv1   g00(.a(x13), .O(new_n20_));
  nor2   g01(.a(new_n20_), .b(x06), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  inv1   g03(.a(x03), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n25_), .c(new_n23_), .O(new_n28_));
  oai21  g09(.a(x03), .b(x00), .c(x05), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(new_n22_), .O(new_n30_));
  nand2  g11(.a(x13), .b(x09), .O(new_n31_));
  inv1   g12(.a(new_n31_), .O(new_n32_));
  nand4  g13(.a(new_n32_), .b(x08), .c(x06), .d(x03), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n30_), .O(z0));
  inv1   g15(.a(x04), .O(new_n35_));
  nor2   g16(.a(x10), .b(x08), .O(new_n36_));
  inv1   g17(.a(new_n36_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n31_), .c(new_n35_), .O(new_n38_));
  inv1   g19(.a(new_n24_), .O(new_n39_));
  nand3  g20(.a(new_n31_), .b(new_n39_), .c(new_n26_), .O(new_n40_));
  aoi22  g21(.a(new_n40_), .b(x10), .c(new_n38_), .d(x02), .O(new_n41_));
  inv1   g22(.a(x05), .O(new_n42_));
  nor2   g23(.a(x03), .b(x01), .O(new_n43_));
  nor3   g24(.a(new_n43_), .b(new_n21_), .c(new_n42_), .O(new_n44_));
  oai21  g25(.a(new_n41_), .b(new_n23_), .c(new_n44_), .O(z1));
  oai21  g26(.a(new_n37_), .b(new_n24_), .c(x11), .O(new_n46_));
  nor3   g27(.a(x11), .b(x10), .c(x08), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n39_), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n46_), .c(new_n21_), .O(new_n49_));
  inv1   g30(.a(x11), .O(new_n50_));
  nand2  g31(.a(new_n24_), .b(new_n50_), .O(new_n51_));
  nand4  g32(.a(new_n51_), .b(x13), .c(x09), .d(x06), .O(new_n52_));
  oai21  g33(.a(x13), .b(x05), .c(new_n52_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n49_), .c(x03), .O(new_n54_));
  nor2   g35(.a(x13), .b(x06), .O(new_n55_));
  aoi22  g36(.a(new_n55_), .b(new_n23_), .c(x06), .d(new_n42_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n54_), .O(z2));
  inv1   g38(.a(x02), .O(new_n58_));
  nor2   g39(.a(x12), .b(x11), .O(new_n59_));
  aoi22  g40(.a(new_n59_), .b(new_n36_), .c(x13), .d(x09), .O(new_n60_));
  nand3  g41(.a(x13), .b(x12), .c(x09), .O(new_n61_));
  oai21  g42(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  inv1   g43(.a(x12), .O(new_n63_));
  aoi21  g44(.a(new_n47_), .b(new_n39_), .c(new_n63_), .O(new_n64_));
  aoi21  g45(.a(new_n62_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g46(.a(x07), .O(new_n66_));
  nand2  g47(.a(new_n66_), .b(new_n23_), .O(new_n67_));
  nand3  g48(.a(new_n67_), .b(new_n22_), .c(x05), .O(new_n68_));
  inv1   g49(.a(new_n68_), .O(new_n69_));
  oai21  g50(.a(new_n65_), .b(new_n23_), .c(new_n69_), .O(z3));
  nand2  g51(.a(x09), .b(x04), .O(new_n71_));
  aoi21  g52(.a(new_n71_), .b(x06), .c(new_n20_), .O(z4));
endmodule


