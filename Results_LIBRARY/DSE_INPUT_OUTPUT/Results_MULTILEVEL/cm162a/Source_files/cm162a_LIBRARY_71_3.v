// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x10), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand3  g03(.a(new_n22_), .b(x04), .c(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n24_), .c(new_n21_), .O(new_n27_));
  nor2   g08(.a(x03), .b(x00), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(new_n20_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x05), .O(z0));
  nand2  g11(.a(new_n22_), .b(x08), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(x04), .c(x03), .d(x02), .O(new_n32_));
  oai21  g13(.a(x03), .b(x01), .c(new_n32_), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n20_), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(x05), .O(z1));
  oai21  g16(.a(x11), .b(x08), .c(new_n22_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(x04), .c(x02), .O(new_n37_));
  and2   g18(.a(x04), .b(x02), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n22_), .c(new_n25_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x11), .O(new_n40_));
  aoi21  g21(.a(new_n40_), .b(new_n37_), .c(new_n21_), .O(new_n41_));
  nor2   g22(.a(x06), .b(x03), .O(new_n42_));
  oai21  g23(.a(new_n42_), .b(new_n41_), .c(new_n20_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x05), .O(z2));
  inv1   g25(.a(x04), .O(new_n45_));
  inv1   g26(.a(new_n22_), .O(new_n46_));
  nor3   g27(.a(x12), .b(x11), .c(x08), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n46_), .c(x02), .O(new_n48_));
  nand3  g29(.a(x13), .b(x12), .c(x09), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(new_n50_));
  inv1   g31(.a(x12), .O(new_n51_));
  nor2   g32(.a(x11), .b(x08), .O(new_n52_));
  aoi21  g33(.a(new_n38_), .b(new_n52_), .c(new_n51_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n50_), .c(x03), .O(new_n54_));
  inv1   g35(.a(x05), .O(new_n55_));
  nor2   g36(.a(x07), .b(x03), .O(new_n56_));
  nor3   g37(.a(new_n56_), .b(x10), .c(new_n55_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n54_), .O(z3));
  oai22  g39(.a(new_n22_), .b(new_n45_), .c(new_n20_), .d(new_n55_), .O(z4));
endmodule


