// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_;
  nand2  g00(.a(x09), .b(x01), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(x04), .c(x02), .O(new_n25_));
  aoi21  g06(.a(new_n25_), .b(new_n23_), .c(new_n21_), .O(new_n26_));
  oai21  g07(.a(x03), .b(x00), .c(x05), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n26_), .c(new_n20_), .O(new_n28_));
  inv1   g09(.a(x01), .O(new_n29_));
  nand2  g10(.a(x13), .b(x09), .O(new_n30_));
  inv1   g11(.a(new_n30_), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(x08), .c(x03), .d(new_n29_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n28_), .O(z0));
  oai21  g14(.a(x09), .b(new_n21_), .c(x01), .O(new_n34_));
  nand2  g15(.a(new_n25_), .b(x10), .O(new_n35_));
  inv1   g16(.a(x10), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n24_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n22_), .c(new_n35_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  inv1   g20(.a(x05), .O(new_n40_));
  nand2  g21(.a(new_n20_), .b(new_n40_), .O(new_n41_));
  nand2  g22(.a(new_n22_), .b(new_n36_), .O(new_n42_));
  nand3  g23(.a(new_n42_), .b(x13), .c(x09), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x03), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n29_), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n41_), .c(new_n39_), .O(z1));
  inv1   g27(.a(x11), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n36_), .c(new_n24_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(x04), .c(x02), .O(new_n50_));
  nand3  g31(.a(new_n30_), .b(new_n36_), .c(new_n24_), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n22_), .c(x11), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x03), .O(new_n54_));
  inv1   g35(.a(x06), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n21_), .c(new_n40_), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n54_), .c(new_n20_), .O(z2));
  inv1   g38(.a(x04), .O(new_n58_));
  inv1   g39(.a(x02), .O(new_n59_));
  inv1   g40(.a(x12), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand4  g42(.a(new_n61_), .b(x13), .c(x09), .d(new_n29_), .O(new_n62_));
  nor2   g43(.a(x12), .b(x11), .O(new_n63_));
  nand4  g44(.a(new_n63_), .b(new_n36_), .c(new_n24_), .d(x02), .O(new_n64_));
  aoi21  g45(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(new_n65_));
  nor2   g46(.a(new_n48_), .b(new_n22_), .O(new_n66_));
  nor2   g47(.a(new_n66_), .b(new_n60_), .O(new_n67_));
  oai21  g48(.a(new_n67_), .b(new_n65_), .c(x03), .O(new_n68_));
  inv1   g49(.a(x07), .O(new_n69_));
  nand2  g50(.a(new_n69_), .b(new_n21_), .O(new_n70_));
  nand3  g51(.a(new_n70_), .b(new_n20_), .c(x05), .O(new_n71_));
  inv1   g52(.a(new_n71_), .O(new_n72_));
  nand2  g53(.a(new_n72_), .b(new_n68_), .O(z3));
  nand3  g54(.a(new_n31_), .b(x04), .c(new_n29_), .O(new_n74_));
  inv1   g55(.a(new_n74_), .O(z4));
endmodule


