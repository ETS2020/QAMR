// Benchmark "FAU" written by ABC on Thu Aug 13 15:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x11), .O(new_n20_));
  nand2  g01(.a(x13), .b(new_n20_), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n24_), .c(new_n22_), .O(new_n27_));
  oai21  g08(.a(x03), .b(x00), .c(x05), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(new_n21_), .O(new_n29_));
  nand2  g10(.a(x08), .b(x03), .O(new_n30_));
  nand3  g11(.a(x13), .b(x11), .c(x09), .O(new_n31_));
  oai21  g12(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(z0));
  inv1   g13(.a(x04), .O(new_n33_));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  nand2  g15(.a(x13), .b(x09), .O(new_n35_));
  inv1   g16(.a(new_n35_), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  inv1   g20(.a(new_n23_), .O(new_n40_));
  aoi21  g21(.a(x13), .b(x09), .c(x08), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  aoi21  g23(.a(new_n38_), .b(x02), .c(new_n42_), .O(new_n43_));
  nor2   g24(.a(x03), .b(x01), .O(new_n44_));
  nand2  g25(.a(new_n21_), .b(x05), .O(new_n45_));
  nor2   g26(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g27(.a(new_n43_), .b(new_n22_), .c(new_n46_), .O(z1));
  inv1   g28(.a(x13), .O(new_n48_));
  nand2  g29(.a(x03), .b(x02), .O(new_n49_));
  nand2  g30(.a(new_n34_), .b(x04), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n20_), .O(new_n52_));
  aoi21  g33(.a(new_n34_), .b(new_n40_), .c(new_n20_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n36_), .c(x03), .O(new_n54_));
  inv1   g35(.a(x06), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n22_), .O(new_n56_));
  nand4  g37(.a(new_n56_), .b(new_n54_), .c(new_n52_), .d(x05), .O(z2));
  nor2   g38(.a(x13), .b(x12), .O(new_n58_));
  nand3  g39(.a(new_n58_), .b(new_n34_), .c(new_n20_), .O(new_n59_));
  aoi21  g40(.a(new_n59_), .b(new_n31_), .c(new_n33_), .O(new_n60_));
  nand4  g41(.a(new_n39_), .b(new_n25_), .c(x04), .d(x02), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n48_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(new_n20_), .O(new_n63_));
  aoi22  g44(.a(new_n63_), .b(x12), .c(new_n60_), .d(x02), .O(new_n64_));
  oai21  g45(.a(x07), .b(x03), .c(x05), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n21_), .O(new_n66_));
  oai21  g47(.a(new_n64_), .b(new_n22_), .c(new_n66_), .O(z3));
  nand4  g48(.a(x13), .b(x11), .c(x09), .d(x04), .O(new_n68_));
  inv1   g49(.a(new_n68_), .O(z4));
endmodule


