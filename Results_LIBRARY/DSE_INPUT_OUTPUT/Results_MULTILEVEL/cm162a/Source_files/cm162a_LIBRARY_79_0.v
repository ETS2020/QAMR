// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_;
  nor2   g00(.a(x13), .b(x06), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n24_), .c(new_n22_), .O(new_n27_));
  oai21  g08(.a(x03), .b(x00), .c(x05), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(new_n21_), .O(new_n29_));
  nand2  g10(.a(x08), .b(x03), .O(new_n30_));
  nand2  g11(.a(x13), .b(x09), .O(new_n31_));
  oai21  g12(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(z0));
  inv1   g13(.a(x04), .O(new_n33_));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  inv1   g15(.a(new_n34_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n31_), .c(new_n33_), .O(new_n36_));
  inv1   g17(.a(new_n23_), .O(new_n37_));
  nand3  g18(.a(new_n31_), .b(new_n37_), .c(new_n25_), .O(new_n38_));
  aoi22  g19(.a(new_n38_), .b(x10), .c(new_n36_), .d(x02), .O(new_n39_));
  inv1   g20(.a(x05), .O(new_n40_));
  nor2   g21(.a(x03), .b(x01), .O(new_n41_));
  nor3   g22(.a(new_n41_), .b(new_n20_), .c(new_n40_), .O(new_n42_));
  oai21  g23(.a(new_n39_), .b(new_n22_), .c(new_n42_), .O(z1));
  oai21  g24(.a(new_n35_), .b(new_n23_), .c(x11), .O(new_n44_));
  nor3   g25(.a(x11), .b(x10), .c(x08), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n37_), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n44_), .c(new_n20_), .O(new_n47_));
  inv1   g28(.a(x13), .O(new_n48_));
  oai21  g29(.a(new_n37_), .b(x11), .c(x09), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(x05), .c(new_n48_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n47_), .c(x03), .O(new_n51_));
  nor2   g32(.a(new_n48_), .b(x06), .O(new_n52_));
  aoi22  g33(.a(new_n52_), .b(new_n22_), .c(x06), .d(new_n40_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(z2));
  inv1   g35(.a(x02), .O(new_n55_));
  nor2   g36(.a(x12), .b(x11), .O(new_n56_));
  aoi22  g37(.a(new_n56_), .b(new_n34_), .c(x13), .d(x09), .O(new_n57_));
  nand3  g38(.a(x13), .b(x12), .c(x09), .O(new_n58_));
  oai21  g39(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  inv1   g40(.a(x12), .O(new_n60_));
  aoi21  g41(.a(new_n45_), .b(new_n37_), .c(new_n60_), .O(new_n61_));
  aoi21  g42(.a(new_n59_), .b(x04), .c(new_n61_), .O(new_n62_));
  nor2   g43(.a(x07), .b(x03), .O(new_n63_));
  nor3   g44(.a(new_n63_), .b(new_n20_), .c(new_n40_), .O(new_n64_));
  oai21  g45(.a(new_n62_), .b(new_n22_), .c(new_n64_), .O(z3));
  oai21  g46(.a(new_n31_), .b(new_n33_), .c(new_n21_), .O(z4));
endmodule


