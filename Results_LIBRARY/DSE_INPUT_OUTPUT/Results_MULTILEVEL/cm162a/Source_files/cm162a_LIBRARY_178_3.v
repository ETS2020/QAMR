// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_;
  inv1   g00(.a(x11), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x03), .c(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x04), .O(new_n24_));
  nand2  g05(.a(x13), .b(x09), .O(new_n25_));
  nand4  g06(.a(new_n25_), .b(new_n20_), .c(x04), .d(x02), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x08), .c(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(new_n27_), .c(new_n24_), .O(z0));
  inv1   g13(.a(x04), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n21_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n25_), .c(new_n33_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x02), .O(new_n37_));
  nand2  g18(.a(x04), .b(x02), .O(new_n38_));
  nand3  g19(.a(new_n25_), .b(new_n20_), .c(new_n21_), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(x10), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n37_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x03), .O(new_n42_));
  nor2   g23(.a(x03), .b(x01), .O(new_n43_));
  oai21  g24(.a(new_n20_), .b(new_n33_), .c(x05), .O(new_n44_));
  nor2   g25(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n42_), .O(z1));
  nand3  g27(.a(new_n36_), .b(x03), .c(x02), .O(new_n47_));
  inv1   g28(.a(new_n47_), .O(new_n48_));
  oai21  g29(.a(x06), .b(x03), .c(x05), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n48_), .c(new_n20_), .O(new_n50_));
  aoi21  g31(.a(x06), .b(new_n29_), .c(new_n20_), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n30_), .c(new_n33_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n50_), .O(z2));
  inv1   g34(.a(x02), .O(new_n54_));
  inv1   g35(.a(x12), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n34_), .c(new_n21_), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n25_), .c(new_n54_), .O(new_n57_));
  nand3  g38(.a(x13), .b(x12), .c(x09), .O(new_n58_));
  inv1   g39(.a(new_n58_), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n57_), .c(x04), .O(new_n60_));
  oai21  g41(.a(new_n35_), .b(new_n54_), .c(x12), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(new_n60_), .c(new_n29_), .O(new_n62_));
  inv1   g43(.a(x07), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n29_), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(x05), .O(new_n65_));
  oai21  g46(.a(new_n65_), .b(new_n62_), .c(new_n20_), .O(new_n66_));
  nand2  g47(.a(x12), .b(x03), .O(new_n67_));
  nand3  g48(.a(new_n67_), .b(new_n64_), .c(x05), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n33_), .O(new_n69_));
  nand2  g50(.a(new_n69_), .b(new_n66_), .O(z3));
  nand4  g51(.a(x13), .b(new_n20_), .c(x09), .d(x04), .O(new_n71_));
  inv1   g52(.a(new_n71_), .O(z4));
endmodule


