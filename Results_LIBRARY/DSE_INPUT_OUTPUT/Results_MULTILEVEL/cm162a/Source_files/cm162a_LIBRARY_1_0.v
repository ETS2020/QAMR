// Benchmark "FAU" written by ABC on Thu Aug 13 15:05:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(new_n21_), .c(x12), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nand2  g05(.a(x13), .b(x09), .O(new_n25_));
  aoi21  g06(.a(new_n25_), .b(x02), .c(new_n22_), .O(new_n26_));
  nand3  g07(.a(new_n22_), .b(x04), .c(x02), .O(new_n27_));
  inv1   g08(.a(new_n27_), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g10(.a(x00), .O(new_n30_));
  inv1   g11(.a(x05), .O(new_n31_));
  aoi21  g12(.a(new_n21_), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(new_n29_), .c(new_n24_), .O(z0));
  inv1   g14(.a(x12), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n20_), .c(x01), .O(new_n35_));
  nor2   g16(.a(new_n34_), .b(x05), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n35_), .c(new_n21_), .O(new_n37_));
  inv1   g18(.a(new_n25_), .O(new_n38_));
  nor2   g19(.a(x10), .b(x08), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand3  g21(.a(new_n25_), .b(new_n22_), .c(x02), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x10), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n40_), .c(new_n20_), .O(new_n43_));
  inv1   g24(.a(x10), .O(new_n44_));
  nor3   g25(.a(new_n34_), .b(new_n44_), .c(x04), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n43_), .c(x03), .O(new_n46_));
  oai21  g27(.a(new_n34_), .b(x10), .c(new_n20_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  nand3  g29(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z1));
  nor3   g30(.a(x11), .b(x10), .c(x08), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n38_), .c(x02), .O(new_n51_));
  nand2  g32(.a(new_n22_), .b(x02), .O(new_n52_));
  nand2  g33(.a(new_n25_), .b(new_n44_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n52_), .c(x11), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n51_), .c(new_n21_), .O(new_n55_));
  oai21  g36(.a(x06), .b(x03), .c(x05), .O(new_n56_));
  oai21  g37(.a(new_n56_), .b(new_n55_), .c(x04), .O(new_n57_));
  aoi21  g38(.a(x06), .b(x05), .c(x03), .O(new_n58_));
  nand3  g39(.a(x11), .b(new_n20_), .c(x03), .O(new_n59_));
  oai21  g40(.a(x11), .b(x05), .c(new_n59_), .O(new_n60_));
  oai21  g41(.a(new_n60_), .b(new_n58_), .c(x12), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n57_), .O(z2));
  inv1   g43(.a(x02), .O(new_n63_));
  inv1   g44(.a(x11), .O(new_n64_));
  nand3  g45(.a(new_n39_), .b(new_n34_), .c(new_n64_), .O(new_n65_));
  nand3  g46(.a(x13), .b(x09), .c(x04), .O(new_n66_));
  aoi21  g47(.a(new_n66_), .b(new_n65_), .c(new_n63_), .O(new_n67_));
  nand3  g48(.a(new_n66_), .b(new_n64_), .c(new_n44_), .O(new_n68_));
  oai21  g49(.a(new_n68_), .b(new_n52_), .c(x12), .O(new_n69_));
  nand2  g50(.a(new_n69_), .b(x04), .O(new_n70_));
  oai21  g51(.a(new_n70_), .b(new_n67_), .c(x03), .O(new_n71_));
  aoi21  g52(.a(x12), .b(x07), .c(x04), .O(new_n72_));
  oai21  g53(.a(x07), .b(x03), .c(x05), .O(new_n73_));
  nor2   g54(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g55(.a(new_n74_), .b(new_n71_), .O(z3));
  inv1   g56(.a(new_n66_), .O(z4));
endmodule


