// Benchmark "FAU" written by ABC on Thu Aug 13 15:05:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  inv1   g02(.a(x04), .O(new_n22_));
  nor2   g03(.a(x08), .b(new_n22_), .O(new_n23_));
  aoi22  g04(.a(new_n23_), .b(x02), .c(new_n21_), .d(x08), .O(new_n24_));
  nor2   g05(.a(x03), .b(x00), .O(new_n25_));
  inv1   g06(.a(x09), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x05), .O(new_n27_));
  nor2   g08(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  oai21  g09(.a(new_n24_), .b(new_n20_), .c(new_n28_), .O(z0));
  nand2  g10(.a(new_n23_), .b(x02), .O(new_n30_));
  inv1   g11(.a(x08), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand4  g13(.a(new_n32_), .b(new_n31_), .c(x04), .d(x02), .O(new_n33_));
  inv1   g14(.a(new_n33_), .O(new_n34_));
  aoi21  g15(.a(new_n30_), .b(x10), .c(new_n34_), .O(new_n35_));
  nor2   g16(.a(x03), .b(x01), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  oai21  g18(.a(new_n35_), .b(new_n20_), .c(new_n37_), .O(z1));
  nand2  g19(.a(new_n33_), .b(x11), .O(new_n39_));
  nor2   g20(.a(x11), .b(x10), .O(new_n40_));
  nand4  g21(.a(new_n40_), .b(new_n31_), .c(x04), .d(x02), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n39_), .c(new_n20_), .O(new_n42_));
  nor2   g23(.a(x06), .b(x03), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(new_n42_), .c(new_n26_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x05), .O(z2));
  inv1   g26(.a(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n32_), .c(new_n31_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n21_), .c(x12), .O(new_n48_));
  nor3   g29(.a(x12), .b(x11), .c(x10), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n23_), .c(x02), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n48_), .c(new_n20_), .O(new_n51_));
  nor2   g32(.a(x07), .b(x03), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n51_), .c(new_n26_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x05), .O(z3));
  aoi21  g35(.a(x13), .b(x04), .c(x05), .O(new_n55_));
  nor2   g36(.a(new_n55_), .b(new_n26_), .O(z4));
endmodule


