// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x00), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  oai21  g04(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(x02), .c(new_n23_), .O(new_n27_));
  nand3  g08(.a(new_n23_), .b(x04), .c(x02), .O(new_n28_));
  inv1   g09(.a(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  inv1   g11(.a(x05), .O(new_n31_));
  aoi21  g12(.a(new_n22_), .b(new_n21_), .c(new_n31_), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(new_n30_), .c(new_n25_), .O(z0));
  inv1   g14(.a(new_n26_), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  nor2   g16(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n20_), .O(new_n37_));
  inv1   g18(.a(x10), .O(new_n38_));
  inv1   g19(.a(x02), .O(new_n39_));
  nor2   g20(.a(new_n20_), .b(new_n39_), .O(new_n40_));
  nor2   g21(.a(new_n34_), .b(x08), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  aoi21  g23(.a(new_n37_), .b(x02), .c(new_n42_), .O(new_n43_));
  nor2   g24(.a(x03), .b(x01), .O(new_n44_));
  nand2  g25(.a(new_n20_), .b(x00), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(x05), .O(new_n46_));
  nor2   g27(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g28(.a(new_n43_), .b(new_n22_), .c(new_n47_), .O(z1));
  inv1   g29(.a(x11), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n38_), .c(new_n23_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n26_), .O(new_n51_));
  nand3  g32(.a(new_n51_), .b(x04), .c(x02), .O(new_n52_));
  inv1   g33(.a(new_n52_), .O(new_n53_));
  nand3  g34(.a(new_n26_), .b(new_n38_), .c(new_n23_), .O(new_n54_));
  inv1   g35(.a(new_n54_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n40_), .c(new_n49_), .O(new_n56_));
  oai21  g37(.a(new_n56_), .b(new_n53_), .c(x03), .O(new_n57_));
  inv1   g38(.a(x06), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n22_), .c(new_n31_), .O(new_n59_));
  nand3  g40(.a(new_n59_), .b(new_n57_), .c(new_n45_), .O(z2));
  nor2   g41(.a(x12), .b(x11), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(new_n35_), .c(new_n34_), .O(new_n62_));
  nand2  g43(.a(new_n23_), .b(x02), .O(new_n63_));
  nand3  g44(.a(new_n26_), .b(new_n49_), .c(new_n38_), .O(new_n64_));
  oai21  g45(.a(new_n64_), .b(new_n63_), .c(x12), .O(new_n65_));
  oai21  g46(.a(new_n62_), .b(new_n39_), .c(new_n65_), .O(new_n66_));
  oai21  g47(.a(x07), .b(x03), .c(x05), .O(new_n67_));
  aoi21  g48(.a(new_n66_), .b(x03), .c(new_n67_), .O(new_n68_));
  aoi21  g49(.a(x07), .b(x05), .c(x03), .O(new_n69_));
  nand3  g50(.a(x12), .b(new_n20_), .c(x03), .O(new_n70_));
  oai21  g51(.a(x12), .b(x05), .c(new_n70_), .O(new_n71_));
  oai21  g52(.a(new_n71_), .b(new_n69_), .c(new_n21_), .O(new_n72_));
  oai21  g53(.a(new_n68_), .b(new_n20_), .c(new_n72_), .O(z3));
  oai21  g54(.a(new_n26_), .b(new_n20_), .c(new_n45_), .O(z4));
endmodule


