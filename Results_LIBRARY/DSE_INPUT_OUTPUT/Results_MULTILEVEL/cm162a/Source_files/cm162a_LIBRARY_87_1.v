// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x04), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  aoi21  g02(.a(new_n21_), .b(x08), .c(new_n20_), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(x04), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n22_), .c(x03), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  inv1   g07(.a(x03), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g09(.a(x02), .O(new_n29_));
  nand2  g10(.a(x04), .b(new_n29_), .O(new_n30_));
  nand4  g11(.a(new_n30_), .b(new_n28_), .c(new_n25_), .d(x05), .O(z0));
  oai21  g12(.a(x03), .b(x01), .c(x05), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  inv1   g14(.a(new_n21_), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(x04), .O(new_n36_));
  oai21  g17(.a(new_n34_), .b(x08), .c(x10), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n36_), .c(new_n29_), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  nor2   g20(.a(new_n39_), .b(x04), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n38_), .c(x03), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n33_), .O(z1));
  oai21  g23(.a(x06), .b(x03), .c(x05), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n30_), .O(new_n44_));
  inv1   g25(.a(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n39_), .c(new_n23_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n21_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(x04), .O(new_n48_));
  nand3  g29(.a(new_n21_), .b(new_n39_), .c(new_n23_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(x11), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n48_), .c(new_n29_), .O(new_n51_));
  nor2   g32(.a(new_n45_), .b(x04), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n51_), .c(x03), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n44_), .O(z2));
  oai21  g35(.a(x07), .b(x03), .c(x05), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nor2   g37(.a(x12), .b(x11), .O(new_n57_));
  aoi22  g38(.a(new_n57_), .b(new_n35_), .c(x13), .d(x09), .O(new_n58_));
  nand2  g39(.a(new_n46_), .b(x12), .O(new_n59_));
  oai21  g40(.a(new_n58_), .b(new_n20_), .c(new_n59_), .O(new_n60_));
  inv1   g41(.a(x12), .O(new_n61_));
  nor2   g42(.a(new_n61_), .b(x04), .O(new_n62_));
  aoi21  g43(.a(new_n60_), .b(x02), .c(new_n62_), .O(new_n63_));
  oai21  g44(.a(new_n63_), .b(new_n27_), .c(new_n56_), .O(z3));
  nand3  g45(.a(new_n34_), .b(x04), .c(x02), .O(new_n65_));
  inv1   g46(.a(new_n65_), .O(z4));
endmodule


