// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_;
  or2    g00(.a(x13), .b(x09), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  inv1   g04(.a(x04), .O(new_n24_));
  nor2   g05(.a(x08), .b(new_n24_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x02), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n23_), .c(new_n21_), .O(new_n27_));
  oai21  g08(.a(x03), .b(x00), .c(x05), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(new_n20_), .O(new_n29_));
  nand4  g10(.a(x13), .b(x09), .c(x08), .d(x03), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n29_), .O(z0));
  nand2  g12(.a(x13), .b(x09), .O(new_n32_));
  inv1   g13(.a(x08), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n32_), .c(new_n24_), .O(new_n36_));
  nand4  g17(.a(new_n32_), .b(new_n33_), .c(x04), .d(x02), .O(new_n37_));
  aoi22  g18(.a(new_n37_), .b(x10), .c(new_n36_), .d(x02), .O(new_n38_));
  nor2   g19(.a(x03), .b(x01), .O(new_n39_));
  nand2  g20(.a(new_n20_), .b(x05), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  oai21  g22(.a(new_n38_), .b(new_n21_), .c(new_n41_), .O(z1));
  inv1   g23(.a(x11), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n34_), .c(new_n33_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n32_), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(x04), .c(x02), .O(new_n46_));
  nand3  g27(.a(new_n32_), .b(new_n34_), .c(new_n33_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n22_), .c(x11), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(x03), .O(new_n50_));
  inv1   g31(.a(x06), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  nand4  g33(.a(new_n52_), .b(new_n50_), .c(new_n20_), .d(x05), .O(z2));
  oai21  g34(.a(new_n44_), .b(new_n22_), .c(x12), .O(new_n54_));
  nor3   g35(.a(x12), .b(x11), .c(x10), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n25_), .c(x02), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n54_), .c(new_n21_), .O(new_n57_));
  oai21  g38(.a(x07), .b(x03), .c(x05), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n57_), .c(new_n20_), .O(new_n59_));
  inv1   g40(.a(x12), .O(new_n60_));
  nand2  g41(.a(new_n22_), .b(new_n60_), .O(new_n61_));
  nand4  g42(.a(new_n61_), .b(x13), .c(x09), .d(x03), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(new_n59_), .O(z3));
  oai21  g44(.a(new_n32_), .b(new_n24_), .c(new_n20_), .O(z4));
endmodule


