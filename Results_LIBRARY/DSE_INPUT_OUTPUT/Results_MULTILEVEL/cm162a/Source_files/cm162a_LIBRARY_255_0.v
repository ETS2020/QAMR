// Benchmark "FAU" written by ABC on Thu Aug 13 15:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x09), .O(new_n21_));
  inv1   g02(.a(x13), .O(new_n22_));
  nor2   g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(x04), .c(x02), .O(new_n25_));
  inv1   g06(.a(x04), .O(new_n26_));
  nor2   g07(.a(x08), .b(new_n26_), .O(new_n27_));
  aoi22  g08(.a(new_n27_), .b(x02), .c(new_n25_), .d(x08), .O(new_n28_));
  nor2   g09(.a(x03), .b(x00), .O(new_n29_));
  inv1   g10(.a(x12), .O(new_n30_));
  nand2  g11(.a(new_n22_), .b(new_n30_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x05), .O(new_n32_));
  nor2   g13(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  oai21  g14(.a(new_n28_), .b(new_n20_), .c(new_n33_), .O(z0));
  inv1   g15(.a(x08), .O(new_n35_));
  inv1   g16(.a(x10), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n24_), .c(new_n26_), .O(new_n38_));
  nand2  g19(.a(x04), .b(x02), .O(new_n39_));
  inv1   g20(.a(new_n39_), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n24_), .c(new_n35_), .O(new_n41_));
  aoi22  g22(.a(new_n41_), .b(x10), .c(new_n38_), .d(x02), .O(new_n42_));
  nor2   g23(.a(x03), .b(x01), .O(new_n43_));
  nor2   g24(.a(new_n43_), .b(new_n32_), .O(new_n44_));
  oai21  g25(.a(new_n42_), .b(new_n20_), .c(new_n44_), .O(z1));
  oai21  g26(.a(new_n39_), .b(new_n37_), .c(x11), .O(new_n46_));
  inv1   g27(.a(x11), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n36_), .c(new_n35_), .O(new_n48_));
  nor2   g29(.a(new_n48_), .b(new_n39_), .O(new_n49_));
  inv1   g30(.a(new_n49_), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n46_), .c(new_n20_), .O(new_n51_));
  oai21  g32(.a(x06), .b(x03), .c(x05), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n51_), .c(new_n31_), .O(new_n53_));
  nand2  g34(.a(new_n39_), .b(new_n47_), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(x13), .c(x09), .d(x03), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n53_), .O(z2));
  nand2  g37(.a(new_n48_), .b(new_n21_), .O(new_n57_));
  nand4  g38(.a(new_n57_), .b(new_n30_), .c(x04), .d(x02), .O(new_n58_));
  nand2  g39(.a(x12), .b(x09), .O(new_n59_));
  aoi21  g40(.a(new_n59_), .b(new_n58_), .c(new_n22_), .O(new_n60_));
  nor2   g41(.a(new_n49_), .b(new_n30_), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(new_n60_), .c(x03), .O(new_n62_));
  oai21  g43(.a(x07), .b(x03), .c(x05), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n31_), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n62_), .O(z3));
  nand2  g46(.a(new_n23_), .b(x04), .O(new_n66_));
  inv1   g47(.a(new_n66_), .O(z4));
endmodule


