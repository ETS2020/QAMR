// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_;
  nand3  g00(.a(x09), .b(x08), .c(x03), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x11), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x13), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  and2   g04(.a(x04), .b(x02), .O(new_n24_));
  nand3  g05(.a(new_n23_), .b(x04), .c(x02), .O(new_n25_));
  oai21  g06(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(new_n27_), .c(new_n22_), .O(z0));
  inv1   g13(.a(x04), .O(new_n33_));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  nand2  g15(.a(x13), .b(x09), .O(new_n35_));
  inv1   g16(.a(new_n35_), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  inv1   g19(.a(x10), .O(new_n39_));
  nor2   g20(.a(new_n36_), .b(x08), .O(new_n40_));
  aoi21  g21(.a(new_n40_), .b(new_n24_), .c(new_n39_), .O(new_n41_));
  aoi21  g22(.a(new_n38_), .b(x02), .c(new_n41_), .O(new_n42_));
  nor2   g23(.a(x03), .b(x01), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand2  g25(.a(x13), .b(new_n44_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(x05), .O(new_n46_));
  nor2   g27(.a(new_n46_), .b(new_n43_), .O(new_n47_));
  oai21  g28(.a(new_n42_), .b(new_n29_), .c(new_n47_), .O(z1));
  inv1   g29(.a(x13), .O(new_n49_));
  nand2  g30(.a(x03), .b(x02), .O(new_n50_));
  nand2  g31(.a(new_n34_), .b(x04), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n44_), .O(new_n53_));
  aoi21  g34(.a(new_n34_), .b(new_n24_), .c(new_n44_), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n36_), .c(x03), .O(new_n55_));
  inv1   g36(.a(x06), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n29_), .c(new_n30_), .O(new_n57_));
  nand3  g38(.a(new_n57_), .b(new_n55_), .c(new_n53_), .O(z2));
  inv1   g39(.a(x12), .O(new_n59_));
  nand3  g40(.a(x13), .b(x11), .c(x09), .O(new_n60_));
  nand3  g41(.a(new_n34_), .b(new_n49_), .c(new_n44_), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g43(.a(new_n62_), .b(new_n59_), .c(x04), .d(x02), .O(new_n63_));
  aoi21  g44(.a(new_n34_), .b(new_n24_), .c(x13), .O(new_n64_));
  oai21  g45(.a(new_n64_), .b(x11), .c(x12), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g47(.a(new_n66_), .b(x03), .O(new_n67_));
  oai21  g48(.a(x07), .b(x03), .c(x05), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n45_), .O(new_n69_));
  nand2  g50(.a(new_n69_), .b(new_n67_), .O(z3));
  nand2  g51(.a(x09), .b(x04), .O(new_n71_));
  aoi21  g52(.a(new_n71_), .b(x11), .c(new_n49_), .O(z4));
endmodule


