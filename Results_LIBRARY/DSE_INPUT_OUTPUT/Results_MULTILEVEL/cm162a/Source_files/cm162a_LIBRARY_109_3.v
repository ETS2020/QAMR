// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x12), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x03), .c(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x04), .O(new_n24_));
  nand2  g05(.a(x13), .b(x09), .O(new_n25_));
  nand4  g06(.a(new_n25_), .b(new_n20_), .c(x04), .d(x02), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x08), .c(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand4  g11(.a(new_n30_), .b(new_n27_), .c(new_n24_), .d(x05), .O(z0));
  oai21  g12(.a(x10), .b(x08), .c(new_n25_), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(x04), .c(x02), .O(new_n33_));
  nand2  g14(.a(x04), .b(x02), .O(new_n34_));
  nand3  g15(.a(new_n25_), .b(new_n20_), .c(new_n21_), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(x10), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x03), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  nand2  g20(.a(new_n29_), .b(new_n39_), .O(new_n40_));
  nand2  g21(.a(x12), .b(x04), .O(new_n41_));
  nand4  g22(.a(new_n41_), .b(new_n40_), .c(new_n38_), .d(x05), .O(z1));
  inv1   g23(.a(x10), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n43_), .c(new_n21_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n25_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  nand4  g28(.a(new_n25_), .b(new_n43_), .c(new_n21_), .d(x02), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x11), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n47_), .c(new_n29_), .O(new_n50_));
  inv1   g31(.a(x06), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n29_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x05), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n50_), .c(new_n20_), .O(new_n54_));
  inv1   g35(.a(x04), .O(new_n55_));
  nand2  g36(.a(x11), .b(x03), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n52_), .c(x05), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n54_), .O(z2));
  nand3  g40(.a(x13), .b(new_n20_), .c(x09), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(new_n45_), .c(new_n55_), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(x02), .c(x12), .O(new_n62_));
  inv1   g43(.a(x07), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n29_), .O(new_n64_));
  nand3  g45(.a(new_n64_), .b(new_n41_), .c(x05), .O(new_n65_));
  inv1   g46(.a(new_n65_), .O(new_n66_));
  oai21  g47(.a(new_n62_), .b(new_n29_), .c(new_n66_), .O(z3));
  nand4  g48(.a(x13), .b(new_n20_), .c(x09), .d(x04), .O(new_n68_));
  inv1   g49(.a(new_n68_), .O(z4));
endmodule


