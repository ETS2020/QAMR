// Benchmark "FAU" written by ABC on Thu Aug 13 15:07:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand3  g01(.a(x04), .b(x03), .c(x02), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x12), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(x04), .c(x02), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x08), .c(x03), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  inv1   g09(.a(x05), .O(new_n29_));
  aoi21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(new_n26_), .c(new_n23_), .O(z0));
  oai21  g12(.a(x10), .b(x08), .c(new_n24_), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(x04), .c(x02), .O(new_n33_));
  inv1   g14(.a(new_n33_), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  and2   g16(.a(x04), .b(x02), .O(new_n36_));
  nand3  g17(.a(new_n24_), .b(x12), .c(new_n20_), .O(new_n37_));
  inv1   g18(.a(new_n37_), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n34_), .c(x03), .O(new_n40_));
  inv1   g21(.a(x01), .O(new_n41_));
  nand2  g22(.a(new_n28_), .b(new_n41_), .O(new_n42_));
  inv1   g23(.a(x12), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n20_), .O(new_n44_));
  nand4  g25(.a(new_n44_), .b(new_n42_), .c(new_n40_), .d(x05), .O(z1));
  nand2  g26(.a(x03), .b(x02), .O(new_n46_));
  nand3  g27(.a(x13), .b(x09), .c(x04), .O(new_n47_));
  oai22  g28(.a(new_n47_), .b(new_n46_), .c(x06), .d(x03), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n29_), .c(new_n44_), .O(new_n49_));
  inv1   g30(.a(x11), .O(new_n50_));
  nand4  g31(.a(new_n24_), .b(new_n35_), .c(x04), .d(x02), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(x12), .c(x08), .O(new_n52_));
  inv1   g33(.a(x04), .O(new_n53_));
  nor2   g34(.a(x08), .b(new_n53_), .O(new_n54_));
  nor2   g35(.a(new_n43_), .b(x11), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n54_), .c(new_n35_), .d(x02), .O(new_n56_));
  oai21  g37(.a(new_n52_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x03), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n49_), .O(z2));
  inv1   g40(.a(x09), .O(new_n60_));
  inv1   g41(.a(x13), .O(new_n61_));
  aoi21  g42(.a(x04), .b(x02), .c(x12), .O(new_n62_));
  nor3   g43(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nor3   g44(.a(x11), .b(x10), .c(x08), .O(new_n64_));
  aoi21  g45(.a(new_n64_), .b(new_n36_), .c(new_n43_), .O(new_n65_));
  oai21  g46(.a(new_n65_), .b(new_n63_), .c(x03), .O(new_n66_));
  inv1   g47(.a(x07), .O(new_n67_));
  nand2  g48(.a(new_n67_), .b(new_n28_), .O(new_n68_));
  nand4  g49(.a(new_n68_), .b(new_n66_), .c(new_n44_), .d(x05), .O(z3));
  nand2  g50(.a(new_n47_), .b(new_n44_), .O(z4));
endmodule


