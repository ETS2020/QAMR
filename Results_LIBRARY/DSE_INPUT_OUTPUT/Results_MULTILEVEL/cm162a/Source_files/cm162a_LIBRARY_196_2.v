// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x09), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x04), .c(x02), .O(new_n22_));
  inv1   g03(.a(x04), .O(new_n23_));
  nor2   g04(.a(x08), .b(new_n23_), .O(new_n24_));
  aoi22  g05(.a(new_n24_), .b(x02), .c(new_n22_), .d(x08), .O(new_n25_));
  nor2   g06(.a(x03), .b(x00), .O(new_n26_));
  inv1   g07(.a(x13), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x08), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x05), .O(new_n29_));
  nor2   g10(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  oai21  g11(.a(new_n25_), .b(new_n20_), .c(new_n30_), .O(z0));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n21_), .b(x08), .O(new_n33_));
  nand4  g14(.a(new_n33_), .b(new_n32_), .c(x04), .d(x02), .O(new_n34_));
  inv1   g15(.a(x08), .O(new_n35_));
  nand2  g16(.a(x13), .b(x09), .O(new_n36_));
  nand4  g17(.a(new_n36_), .b(new_n35_), .c(x04), .d(x02), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x10), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  nor2   g21(.a(x03), .b(x01), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(new_n29_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n40_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(x09), .b(x04), .c(x02), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n44_), .c(new_n35_), .O(new_n46_));
  nand3  g27(.a(new_n44_), .b(new_n32_), .c(new_n35_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n36_), .O(new_n48_));
  nand3  g29(.a(new_n48_), .b(x04), .c(x02), .O(new_n49_));
  nand4  g30(.a(new_n36_), .b(new_n32_), .c(x04), .d(x02), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x11), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n46_), .c(x03), .O(new_n53_));
  inv1   g34(.a(x06), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n53_), .c(new_n28_), .d(x05), .O(z2));
  inv1   g37(.a(x02), .O(new_n57_));
  inv1   g38(.a(new_n36_), .O(new_n58_));
  nor2   g39(.a(x10), .b(x08), .O(new_n59_));
  nor2   g40(.a(x12), .b(x11), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand3  g42(.a(x13), .b(x12), .c(x09), .O(new_n62_));
  oai21  g43(.a(new_n61_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  inv1   g44(.a(x12), .O(new_n64_));
  nand2  g45(.a(new_n28_), .b(new_n23_), .O(new_n65_));
  nand3  g46(.a(new_n44_), .b(new_n32_), .c(x02), .O(new_n66_));
  and2   g47(.a(x13), .b(x08), .O(new_n67_));
  aoi21  g48(.a(new_n66_), .b(new_n35_), .c(new_n67_), .O(new_n68_));
  aoi21  g49(.a(new_n68_), .b(new_n65_), .c(new_n64_), .O(new_n69_));
  aoi21  g50(.a(new_n63_), .b(x04), .c(new_n69_), .O(new_n70_));
  inv1   g51(.a(new_n28_), .O(new_n71_));
  oai22  g52(.a(new_n71_), .b(x07), .c(new_n27_), .d(x05), .O(new_n72_));
  nand2  g53(.a(x13), .b(new_n64_), .O(new_n73_));
  aoi21  g54(.a(new_n73_), .b(x08), .c(x05), .O(new_n74_));
  aoi21  g55(.a(new_n72_), .b(new_n20_), .c(new_n74_), .O(new_n75_));
  oai21  g56(.a(new_n70_), .b(new_n20_), .c(new_n75_), .O(z3));
  nand2  g57(.a(new_n58_), .b(x04), .O(new_n77_));
  inv1   g58(.a(new_n77_), .O(z4));
endmodule


