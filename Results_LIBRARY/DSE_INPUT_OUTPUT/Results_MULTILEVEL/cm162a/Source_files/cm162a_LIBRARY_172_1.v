// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_, new_n61_;
  inv1   g00(.a(x04), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  aoi21  g02(.a(new_n21_), .b(x08), .c(new_n20_), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(x04), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n22_), .c(x03), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  inv1   g07(.a(x03), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand4  g09(.a(new_n28_), .b(new_n25_), .c(x05), .d(x02), .O(z0));
  inv1   g10(.a(x10), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n23_), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(new_n21_), .c(new_n20_), .O(new_n32_));
  inv1   g13(.a(new_n21_), .O(new_n33_));
  nor2   g14(.a(new_n33_), .b(x08), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(x04), .c(new_n30_), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n32_), .c(x03), .O(new_n36_));
  inv1   g17(.a(x02), .O(new_n37_));
  oai21  g18(.a(x03), .b(x01), .c(x05), .O(new_n38_));
  nor2   g19(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n36_), .O(z1));
  nor2   g21(.a(x11), .b(x10), .O(new_n41_));
  aoi22  g22(.a(new_n41_), .b(new_n23_), .c(x13), .d(x09), .O(new_n42_));
  nand4  g23(.a(new_n21_), .b(new_n30_), .c(new_n23_), .d(x04), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x11), .O(new_n44_));
  oai21  g25(.a(new_n42_), .b(new_n20_), .c(new_n44_), .O(new_n45_));
  nor2   g26(.a(x06), .b(x03), .O(new_n46_));
  aoi21  g27(.a(new_n45_), .b(x03), .c(new_n46_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n37_), .c(x05), .O(z2));
  inv1   g29(.a(x11), .O(new_n49_));
  inv1   g30(.a(x12), .O(new_n50_));
  nand4  g31(.a(new_n50_), .b(new_n49_), .c(new_n30_), .d(new_n23_), .O(new_n51_));
  nand2  g32(.a(new_n33_), .b(x02), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n51_), .c(new_n20_), .O(new_n53_));
  nor2   g34(.a(x08), .b(new_n20_), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n41_), .c(new_n50_), .O(new_n55_));
  oai21  g36(.a(new_n55_), .b(new_n53_), .c(x03), .O(new_n56_));
  inv1   g37(.a(x07), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n27_), .O(new_n58_));
  nand4  g39(.a(new_n58_), .b(new_n56_), .c(x05), .d(x02), .O(z3));
  nand2  g40(.a(x05), .b(new_n37_), .O(new_n60_));
  nand4  g41(.a(new_n60_), .b(x13), .c(x09), .d(x04), .O(new_n61_));
  inv1   g42(.a(new_n61_), .O(z4));
endmodule


