// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x04), .c(x02), .O(new_n22_));
  inv1   g03(.a(x04), .O(new_n23_));
  nor2   g04(.a(x08), .b(new_n23_), .O(new_n24_));
  aoi22  g05(.a(new_n24_), .b(x02), .c(new_n22_), .d(x08), .O(new_n25_));
  nor2   g06(.a(x03), .b(x00), .O(new_n26_));
  inv1   g07(.a(x09), .O(new_n27_));
  nand2  g08(.a(x12), .b(new_n27_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x05), .O(new_n29_));
  nor2   g10(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  oai21  g11(.a(new_n25_), .b(new_n20_), .c(new_n30_), .O(z0));
  inv1   g12(.a(x08), .O(new_n32_));
  inv1   g13(.a(x10), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n21_), .c(new_n23_), .O(new_n35_));
  and2   g16(.a(x04), .b(x02), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n21_), .c(new_n32_), .O(new_n37_));
  aoi22  g18(.a(new_n37_), .b(x10), .c(new_n35_), .d(x02), .O(new_n38_));
  nor2   g19(.a(x03), .b(x01), .O(new_n39_));
  nor2   g20(.a(new_n39_), .b(new_n29_), .O(new_n40_));
  oai21  g21(.a(new_n38_), .b(new_n20_), .c(new_n40_), .O(z1));
  nand4  g22(.a(new_n33_), .b(new_n32_), .c(x04), .d(x02), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(x11), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand4  g25(.a(new_n36_), .b(new_n44_), .c(new_n33_), .d(new_n32_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n43_), .c(new_n20_), .O(new_n46_));
  oai21  g27(.a(x06), .b(x03), .c(x05), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n46_), .c(new_n28_), .O(new_n48_));
  or2    g29(.a(new_n36_), .b(x11), .O(new_n49_));
  nand4  g30(.a(new_n49_), .b(x13), .c(x09), .d(x03), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n48_), .O(z2));
  inv1   g32(.a(x12), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n44_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n34_), .c(new_n21_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(x04), .c(x02), .O(new_n55_));
  nor3   g36(.a(x13), .b(x11), .c(x10), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n24_), .c(x02), .O(new_n57_));
  nand3  g38(.a(new_n57_), .b(x12), .c(x09), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(x03), .O(new_n60_));
  oai21  g41(.a(x07), .b(x03), .c(x05), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n28_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(new_n60_), .O(z3));
  oai21  g44(.a(new_n21_), .b(new_n23_), .c(new_n28_), .O(z4));
endmodule


