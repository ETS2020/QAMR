// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(x08), .c(new_n21_), .O(new_n23_));
  oai21  g04(.a(new_n23_), .b(new_n20_), .c(x02), .O(new_n24_));
  inv1   g05(.a(x00), .O(new_n25_));
  nand2  g06(.a(new_n21_), .b(new_n25_), .O(new_n26_));
  inv1   g07(.a(x02), .O(new_n27_));
  nand3  g08(.a(x08), .b(x03), .c(new_n27_), .O(new_n28_));
  nand4  g09(.a(new_n28_), .b(new_n26_), .c(new_n24_), .d(x05), .O(z0));
  oai21  g10(.a(x03), .b(x01), .c(x05), .O(new_n30_));
  oai21  g11(.a(x04), .b(new_n27_), .c(new_n30_), .O(new_n31_));
  inv1   g12(.a(new_n22_), .O(new_n32_));
  nor2   g13(.a(x10), .b(x08), .O(new_n33_));
  oai21  g14(.a(new_n33_), .b(new_n32_), .c(x02), .O(new_n34_));
  oai21  g15(.a(new_n32_), .b(x08), .c(x10), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n34_), .c(new_n20_), .O(new_n36_));
  inv1   g17(.a(x10), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(x02), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n36_), .c(x03), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n31_), .O(z1));
  nor3   g21(.a(x11), .b(x10), .c(x08), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n32_), .c(x02), .O(new_n42_));
  inv1   g23(.a(x08), .O(new_n43_));
  nand3  g24(.a(new_n22_), .b(new_n37_), .c(new_n43_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x11), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n42_), .c(new_n21_), .O(new_n46_));
  inv1   g27(.a(x06), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n21_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x05), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n46_), .c(x04), .O(new_n50_));
  nand2  g31(.a(x11), .b(x03), .O(new_n51_));
  nand3  g32(.a(new_n51_), .b(new_n48_), .c(x05), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n27_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n50_), .O(z2));
  nor2   g35(.a(x12), .b(x11), .O(new_n55_));
  aoi22  g36(.a(new_n55_), .b(new_n33_), .c(x13), .d(x09), .O(new_n56_));
  inv1   g37(.a(x11), .O(new_n57_));
  nand4  g38(.a(new_n22_), .b(new_n57_), .c(new_n37_), .d(new_n43_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x12), .O(new_n59_));
  oai21  g40(.a(new_n56_), .b(new_n27_), .c(new_n59_), .O(new_n60_));
  inv1   g41(.a(x07), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n21_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(x05), .O(new_n63_));
  aoi21  g44(.a(new_n60_), .b(x03), .c(new_n63_), .O(new_n64_));
  nand2  g45(.a(x12), .b(x03), .O(new_n65_));
  nand3  g46(.a(new_n65_), .b(new_n62_), .c(x05), .O(new_n66_));
  nand2  g47(.a(new_n66_), .b(new_n27_), .O(new_n67_));
  oai21  g48(.a(new_n64_), .b(new_n20_), .c(new_n67_), .O(z3));
  nand2  g49(.a(new_n32_), .b(x04), .O(new_n69_));
  inv1   g50(.a(new_n69_), .O(z4));
endmodule


