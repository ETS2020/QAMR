// Benchmark "FAU" written by ABC on Thu Aug 13 15:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_;
  inv1   g00(.a(x13), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x11), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n24_), .c(new_n22_), .O(new_n27_));
  oai21  g08(.a(x03), .b(x00), .c(x05), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(new_n21_), .O(new_n29_));
  nand2  g10(.a(x13), .b(x09), .O(new_n30_));
  inv1   g11(.a(new_n30_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(x08), .c(x03), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n29_), .O(z0));
  inv1   g14(.a(x04), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  nor2   g16(.a(new_n35_), .b(new_n31_), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand4  g18(.a(new_n30_), .b(new_n25_), .c(x04), .d(x02), .O(new_n38_));
  aoi22  g19(.a(new_n38_), .b(x10), .c(new_n37_), .d(x02), .O(new_n39_));
  nor2   g20(.a(x03), .b(x01), .O(new_n40_));
  nand2  g21(.a(new_n21_), .b(x05), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  oai21  g23(.a(new_n39_), .b(new_n22_), .c(new_n42_), .O(z1));
  inv1   g24(.a(x10), .O(new_n44_));
  inv1   g25(.a(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n44_), .c(new_n25_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n30_), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(x04), .c(x02), .O(new_n48_));
  inv1   g29(.a(x09), .O(new_n49_));
  nand3  g30(.a(new_n44_), .b(new_n49_), .c(new_n25_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n23_), .c(x11), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x03), .O(new_n53_));
  inv1   g34(.a(x06), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n22_), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n53_), .c(new_n21_), .d(x05), .O(z2));
  inv1   g37(.a(x02), .O(new_n57_));
  nor2   g38(.a(x12), .b(x11), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n35_), .c(new_n31_), .O(new_n59_));
  nand3  g40(.a(x13), .b(x12), .c(x09), .O(new_n60_));
  oai21  g41(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  inv1   g42(.a(x12), .O(new_n62_));
  nand2  g43(.a(new_n21_), .b(new_n34_), .O(new_n63_));
  nand3  g44(.a(new_n44_), .b(new_n25_), .c(x02), .O(new_n64_));
  and2   g45(.a(x13), .b(x11), .O(new_n65_));
  aoi21  g46(.a(new_n64_), .b(new_n45_), .c(new_n65_), .O(new_n66_));
  aoi21  g47(.a(new_n66_), .b(new_n63_), .c(new_n62_), .O(new_n67_));
  aoi21  g48(.a(new_n61_), .b(x04), .c(new_n67_), .O(new_n68_));
  inv1   g49(.a(new_n21_), .O(new_n69_));
  oai22  g50(.a(new_n69_), .b(x07), .c(new_n20_), .d(x05), .O(new_n70_));
  nand2  g51(.a(x13), .b(new_n62_), .O(new_n71_));
  aoi21  g52(.a(new_n71_), .b(x11), .c(x05), .O(new_n72_));
  aoi21  g53(.a(new_n70_), .b(new_n22_), .c(new_n72_), .O(new_n73_));
  oai21  g54(.a(new_n68_), .b(new_n22_), .c(new_n73_), .O(z3));
  nand2  g55(.a(new_n31_), .b(x04), .O(new_n75_));
  inv1   g56(.a(new_n75_), .O(z4));
endmodule


