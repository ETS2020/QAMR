// Benchmark "FAU" written by ABC on Thu Aug 13 15:05:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_;
  inv1   g00(.a(x04), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  inv1   g03(.a(x12), .O(new_n23_));
  oai21  g04(.a(new_n22_), .b(new_n21_), .c(new_n23_), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(x02), .c(new_n22_), .O(new_n27_));
  nand3  g08(.a(new_n22_), .b(x04), .c(x02), .O(new_n28_));
  inv1   g09(.a(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  inv1   g11(.a(x00), .O(new_n31_));
  inv1   g12(.a(x05), .O(new_n32_));
  aoi21  g13(.a(new_n21_), .b(new_n31_), .c(new_n32_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(new_n30_), .c(new_n25_), .O(z0));
  aoi21  g15(.a(x12), .b(new_n20_), .c(x01), .O(new_n35_));
  nor2   g16(.a(x12), .b(x05), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n35_), .c(new_n21_), .O(new_n37_));
  inv1   g18(.a(new_n26_), .O(new_n38_));
  nor2   g19(.a(x10), .b(x08), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand3  g21(.a(new_n26_), .b(new_n22_), .c(x02), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x10), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n40_), .c(new_n20_), .O(new_n43_));
  nand3  g24(.a(new_n23_), .b(x10), .c(new_n20_), .O(new_n44_));
  inv1   g25(.a(new_n44_), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n43_), .c(x03), .O(new_n46_));
  nor2   g27(.a(x12), .b(x10), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(x04), .c(new_n32_), .O(new_n48_));
  nand3  g29(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(z1));
  inv1   g30(.a(x02), .O(new_n50_));
  nor2   g31(.a(x11), .b(x10), .O(new_n51_));
  aoi22  g32(.a(new_n51_), .b(new_n22_), .c(x13), .d(x09), .O(new_n52_));
  inv1   g33(.a(x10), .O(new_n53_));
  nand4  g34(.a(new_n26_), .b(new_n53_), .c(new_n22_), .d(x02), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x11), .O(new_n55_));
  oai21  g36(.a(new_n52_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g37(.a(x06), .b(x03), .c(x05), .O(new_n57_));
  aoi21  g38(.a(new_n56_), .b(x03), .c(new_n57_), .O(new_n58_));
  aoi21  g39(.a(x06), .b(x05), .c(x03), .O(new_n59_));
  nand3  g40(.a(x11), .b(new_n20_), .c(x03), .O(new_n60_));
  oai21  g41(.a(x11), .b(x05), .c(new_n60_), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(new_n59_), .c(new_n23_), .O(new_n62_));
  oai21  g43(.a(new_n58_), .b(new_n20_), .c(new_n62_), .O(z2));
  nand2  g44(.a(x12), .b(new_n20_), .O(new_n64_));
  nand2  g45(.a(new_n51_), .b(new_n22_), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n26_), .O(new_n66_));
  nand4  g47(.a(new_n66_), .b(new_n23_), .c(x04), .d(x02), .O(new_n67_));
  nand2  g48(.a(new_n22_), .b(x02), .O(new_n68_));
  inv1   g49(.a(x11), .O(new_n69_));
  nand3  g50(.a(new_n26_), .b(new_n69_), .c(new_n53_), .O(new_n70_));
  oai21  g51(.a(new_n70_), .b(new_n68_), .c(x12), .O(new_n71_));
  nand2  g52(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g53(.a(new_n72_), .b(x03), .O(new_n73_));
  inv1   g54(.a(x07), .O(new_n74_));
  nand2  g55(.a(new_n74_), .b(new_n21_), .O(new_n75_));
  nand4  g56(.a(new_n75_), .b(new_n73_), .c(new_n64_), .d(x05), .O(z3));
  nand2  g57(.a(new_n38_), .b(x04), .O(new_n77_));
  inv1   g58(.a(new_n77_), .O(z4));
endmodule


