// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x05), .O(new_n20_));
  aoi21  g01(.a(x08), .b(x03), .c(new_n20_), .O(new_n21_));
  inv1   g02(.a(x03), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x04), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n25_), .c(new_n22_), .O(new_n28_));
  nor2   g09(.a(x03), .b(x00), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n28_), .c(x05), .O(new_n30_));
  oai21  g11(.a(new_n21_), .b(x02), .c(new_n30_), .O(z0));
  inv1   g12(.a(x04), .O(new_n32_));
  nor2   g13(.a(x10), .b(x08), .O(new_n33_));
  inv1   g14(.a(new_n33_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n23_), .c(new_n32_), .O(new_n35_));
  inv1   g16(.a(x02), .O(new_n36_));
  nor2   g17(.a(new_n32_), .b(new_n36_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n23_), .c(new_n26_), .O(new_n38_));
  aoi22  g19(.a(new_n38_), .b(x10), .c(new_n35_), .d(x02), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  aoi21  g21(.a(new_n22_), .b(new_n40_), .c(new_n20_), .O(new_n41_));
  oai21  g22(.a(new_n39_), .b(new_n22_), .c(new_n41_), .O(z1));
  inv1   g23(.a(x10), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n43_), .c(new_n26_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  nand4  g28(.a(new_n23_), .b(new_n43_), .c(new_n26_), .d(x04), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x11), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n47_), .c(new_n20_), .O(new_n50_));
  nor2   g31(.a(new_n44_), .b(x02), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n50_), .c(x03), .O(new_n52_));
  nor2   g33(.a(x06), .b(new_n20_), .O(new_n53_));
  aoi22  g34(.a(new_n53_), .b(new_n22_), .c(new_n20_), .d(new_n36_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n52_), .O(z2));
  nor2   g36(.a(x12), .b(x11), .O(new_n56_));
  nand3  g37(.a(x13), .b(x09), .c(x05), .O(new_n57_));
  inv1   g38(.a(new_n57_), .O(new_n58_));
  aoi21  g39(.a(new_n56_), .b(new_n33_), .c(new_n58_), .O(new_n59_));
  nand4  g40(.a(x13), .b(x12), .c(x09), .d(x05), .O(new_n60_));
  oai21  g41(.a(new_n59_), .b(new_n36_), .c(new_n60_), .O(new_n61_));
  inv1   g42(.a(x12), .O(new_n62_));
  inv1   g43(.a(new_n45_), .O(new_n63_));
  aoi21  g44(.a(new_n63_), .b(new_n37_), .c(new_n62_), .O(new_n64_));
  aoi21  g45(.a(new_n61_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g46(.a(x07), .O(new_n66_));
  aoi21  g47(.a(new_n66_), .b(new_n22_), .c(new_n20_), .O(new_n67_));
  oai21  g48(.a(new_n65_), .b(new_n22_), .c(new_n67_), .O(z3));
  nand2  g49(.a(new_n20_), .b(x02), .O(new_n69_));
  nand4  g50(.a(new_n69_), .b(x13), .c(x09), .d(x04), .O(new_n70_));
  inv1   g51(.a(new_n70_), .O(z4));
endmodule


