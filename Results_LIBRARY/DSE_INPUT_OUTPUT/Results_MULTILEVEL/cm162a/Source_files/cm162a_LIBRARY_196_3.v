// Benchmark "FAU" written by ABC on Thu Aug 13 15:06:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(x08), .c(new_n21_), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  aoi22  g05(.a(new_n24_), .b(x08), .c(new_n23_), .d(x02), .O(new_n25_));
  nor2   g06(.a(x03), .b(x00), .O(new_n26_));
  nand2  g07(.a(x13), .b(x08), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x05), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  oai21  g10(.a(new_n25_), .b(new_n20_), .c(new_n29_), .O(z0));
  oai21  g11(.a(x10), .b(x08), .c(new_n22_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(x04), .c(x02), .O(new_n32_));
  inv1   g13(.a(new_n32_), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nor2   g15(.a(x08), .b(new_n21_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(x02), .c(new_n34_), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n33_), .c(x03), .O(new_n37_));
  nor2   g18(.a(x03), .b(x01), .O(new_n38_));
  nor2   g19(.a(new_n38_), .b(new_n28_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n37_), .O(z1));
  inv1   g21(.a(x11), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  oai21  g23(.a(new_n42_), .b(x08), .c(new_n22_), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(x04), .c(x02), .O(new_n44_));
  inv1   g25(.a(x08), .O(new_n45_));
  nand3  g26(.a(new_n22_), .b(new_n34_), .c(new_n45_), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(new_n24_), .c(x11), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x03), .O(new_n49_));
  inv1   g30(.a(x06), .O(new_n50_));
  inv1   g31(.a(new_n27_), .O(new_n51_));
  aoi21  g32(.a(new_n50_), .b(new_n20_), .c(new_n51_), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(new_n49_), .c(x05), .O(z2));
  inv1   g34(.a(x02), .O(new_n54_));
  inv1   g35(.a(x12), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n41_), .c(new_n34_), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n22_), .c(new_n54_), .O(new_n57_));
  nand3  g38(.a(x13), .b(x12), .c(x09), .O(new_n58_));
  inv1   g39(.a(new_n58_), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n57_), .c(x04), .O(new_n60_));
  oai21  g41(.a(new_n42_), .b(new_n24_), .c(x12), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(new_n60_), .c(x08), .O(new_n62_));
  nor3   g43(.a(new_n35_), .b(x13), .c(new_n55_), .O(new_n63_));
  oai21  g44(.a(new_n63_), .b(new_n62_), .c(x03), .O(new_n64_));
  inv1   g45(.a(x05), .O(new_n65_));
  oai22  g46(.a(new_n51_), .b(x07), .c(x13), .d(x05), .O(new_n66_));
  oai21  g47(.a(x13), .b(x12), .c(x08), .O(new_n67_));
  aoi22  g48(.a(new_n67_), .b(new_n65_), .c(new_n66_), .d(new_n20_), .O(new_n68_));
  nand2  g49(.a(new_n68_), .b(new_n64_), .O(z3));
  inv1   g50(.a(new_n22_), .O(new_n70_));
  nand2  g51(.a(new_n35_), .b(new_n70_), .O(new_n71_));
  inv1   g52(.a(new_n71_), .O(z4));
endmodule


