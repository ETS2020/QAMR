// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n187_,
    new_n188_, new_n189_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x03), .O(new_n36_));
  inv1   g003(.a(x10), .O(new_n37_));
  inv1   g004(.a(x00), .O(new_n38_));
  nor2   g005(.a(x01), .b(new_n38_), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  inv1   g007(.a(x11), .O(new_n41_));
  inv1   g008(.a(x14), .O(new_n42_));
  nor2   g009(.a(x24), .b(x20), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  oai21  g011(.a(new_n44_), .b(new_n40_), .c(new_n35_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  inv1   g014(.a(x05), .O(new_n48_));
  nand2  g015(.a(x24), .b(x07), .O(new_n49_));
  inv1   g016(.a(x24), .O(new_n50_));
  nand3  g017(.a(new_n50_), .b(x19), .c(x13), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  nand3  g019(.a(x24), .b(x13), .c(x07), .O(new_n53_));
  inv1   g020(.a(x13), .O(new_n54_));
  nand3  g021(.a(x19), .b(new_n54_), .c(new_n48_), .O(new_n55_));
  nand2  g022(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  oai21  g023(.a(new_n56_), .b(new_n52_), .c(new_n47_), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(x14), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n41_), .c(new_n35_), .d(new_n36_), .O(new_n59_));
  oai21  g026(.a(new_n59_), .b(x01), .c(new_n46_), .O(z0));
  nand4  g027(.a(x04), .b(x03), .c(x02), .d(x01), .O(new_n61_));
  nand4  g028(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g030(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n64_));
  inv1   g031(.a(new_n64_), .O(new_n65_));
  inv1   g032(.a(x17), .O(new_n66_));
  nor2   g033(.a(new_n47_), .b(new_n66_), .O(new_n67_));
  inv1   g034(.a(x22), .O(new_n68_));
  inv1   g035(.a(x23), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n63_), .O(new_n71_));
  inv1   g038(.a(x09), .O(new_n72_));
  nor3   g039(.a(x04), .b(x03), .c(x01), .O(new_n73_));
  nor2   g040(.a(x08), .b(x06), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n41_), .d(new_n72_), .O(new_n75_));
  nor2   g042(.a(x14), .b(x12), .O(new_n76_));
  nor2   g043(.a(x17), .b(x16), .O(new_n77_));
  inv1   g044(.a(x19), .O(new_n78_));
  nor2   g045(.a(x20), .b(new_n78_), .O(new_n79_));
  nor2   g046(.a(x23), .b(x22), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n79_), .c(new_n77_), .d(new_n76_), .O(new_n81_));
  oai21  g048(.a(new_n81_), .b(new_n75_), .c(new_n71_), .O(new_n82_));
  nand2  g049(.a(new_n54_), .b(new_n48_), .O(new_n83_));
  nand3  g050(.a(new_n50_), .b(x13), .c(x05), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand4  g053(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  and2   g055(.a(x20), .b(x18), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n88_), .c(new_n70_), .d(new_n63_), .O(new_n90_));
  nor2   g057(.a(x09), .b(x08), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n73_), .c(x07), .d(new_n35_), .O(new_n92_));
  inv1   g059(.a(x12), .O(new_n93_));
  inv1   g060(.a(x16), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n42_), .c(new_n93_), .d(new_n41_), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n80_), .c(new_n47_), .d(new_n66_), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n92_), .c(new_n90_), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n83_), .c(x24), .O(new_n99_));
  nand3  g066(.a(new_n39_), .b(new_n36_), .c(new_n34_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  inv1   g068(.a(x08), .O(new_n102_));
  nand3  g069(.a(new_n37_), .b(new_n72_), .c(new_n102_), .O(new_n103_));
  nor3   g070(.a(new_n103_), .b(x06), .c(x04), .O(new_n104_));
  nand3  g071(.a(new_n50_), .b(new_n69_), .c(new_n68_), .O(new_n105_));
  nor3   g072(.a(new_n105_), .b(x20), .c(x17), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(new_n104_), .c(new_n101_), .d(new_n96_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n99_), .c(new_n86_), .O(z1));
  nand2  g075(.a(x24), .b(x18), .O(new_n109_));
  nand3  g076(.a(new_n50_), .b(x15), .c(x13), .O(new_n110_));
  aoi21  g077(.a(new_n110_), .b(new_n109_), .c(new_n48_), .O(new_n111_));
  nand3  g078(.a(x24), .b(x18), .c(x13), .O(new_n112_));
  nand3  g079(.a(x15), .b(new_n54_), .c(new_n48_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  oai21  g081(.a(new_n114_), .b(new_n111_), .c(x20), .O(new_n115_));
  nor2   g082(.a(new_n115_), .b(new_n42_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x11), .c(x03), .d(x01), .O(new_n117_));
  aoi21  g084(.a(new_n117_), .b(x02), .c(new_n35_), .O(z2));
  nand3  g085(.a(new_n39_), .b(new_n102_), .c(new_n36_), .O(new_n119_));
  nand4  g086(.a(new_n43_), .b(new_n42_), .c(new_n41_), .d(new_n37_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n119_), .c(new_n35_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n34_), .O(new_n122_));
  nand4  g089(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n123_));
  nand4  g090(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n124_));
  inv1   g091(.a(x01), .O(new_n125_));
  nand3  g092(.a(new_n74_), .b(new_n36_), .c(new_n125_), .O(new_n126_));
  nand3  g093(.a(new_n79_), .b(new_n42_), .c(new_n41_), .O(new_n127_));
  oai22  g094(.a(new_n127_), .b(new_n126_), .c(new_n124_), .d(new_n123_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n85_), .O(new_n129_));
  nand3  g096(.a(new_n89_), .b(x14), .c(x11), .O(new_n130_));
  nand4  g097(.a(x07), .b(new_n35_), .c(new_n36_), .d(new_n125_), .O(new_n131_));
  nand4  g098(.a(new_n47_), .b(new_n42_), .c(new_n41_), .d(new_n102_), .O(new_n132_));
  oai22  g099(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n123_), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n83_), .c(x24), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n129_), .c(new_n122_), .O(z3));
  inv1   g102(.a(x04), .O(new_n136_));
  oai21  g103(.a(x23), .b(new_n136_), .c(new_n66_), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(new_n68_), .c(x09), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(x16), .c(new_n102_), .O(new_n139_));
  nand4  g106(.a(new_n50_), .b(new_n37_), .c(new_n34_), .d(x00), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n55_), .c(new_n53_), .O(new_n141_));
  or2    g108(.a(new_n141_), .b(new_n52_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  oai21  g110(.a(new_n69_), .b(x04), .c(x17), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(x22), .c(new_n72_), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(new_n94_), .c(x08), .O(new_n146_));
  nand4  g113(.a(new_n50_), .b(x21), .c(new_n37_), .d(new_n34_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n113_), .c(new_n112_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n111_), .c(new_n146_), .O(new_n149_));
  nor2   g116(.a(new_n35_), .b(x02), .O(new_n150_));
  inv1   g117(.a(new_n150_), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(new_n149_), .c(new_n143_), .O(z4));
  nor2   g119(.a(new_n56_), .b(new_n52_), .O(new_n153_));
  nand3  g120(.a(new_n50_), .b(new_n37_), .c(x00), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n35_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n34_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n153_), .O(z5));
  nand3  g124(.a(x20), .b(new_n42_), .c(new_n41_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n36_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n142_), .O(new_n160_));
  inv1   g127(.a(new_n83_), .O(new_n161_));
  nor2   g128(.a(new_n150_), .b(x03), .O(new_n162_));
  nand3  g129(.a(new_n47_), .b(x14), .c(x03), .O(new_n163_));
  aoi21  g130(.a(new_n163_), .b(x06), .c(new_n41_), .O(new_n164_));
  oai21  g131(.a(new_n164_), .b(new_n162_), .c(x18), .O(new_n165_));
  nand3  g132(.a(new_n41_), .b(x07), .c(x06), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(new_n165_), .c(new_n161_), .O(new_n167_));
  nand4  g134(.a(new_n47_), .b(x15), .c(x14), .d(new_n54_), .O(new_n168_));
  nor4   g135(.a(new_n168_), .b(new_n41_), .c(x05), .d(new_n36_), .O(new_n169_));
  oai21  g136(.a(new_n169_), .b(new_n167_), .c(x24), .O(new_n170_));
  inv1   g137(.a(new_n147_), .O(new_n171_));
  inv1   g138(.a(x15), .O(new_n172_));
  aoi21  g139(.a(new_n84_), .b(new_n83_), .c(new_n172_), .O(new_n173_));
  oai22  g140(.a(new_n173_), .b(new_n171_), .c(x11), .d(new_n36_), .O(new_n174_));
  nor2   g141(.a(new_n174_), .b(x06), .O(new_n175_));
  nand3  g142(.a(x15), .b(new_n36_), .c(x02), .O(new_n176_));
  nand3  g143(.a(x19), .b(new_n41_), .c(x06), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n85_), .O(new_n179_));
  xnor2a g146(.a(x13), .b(x05), .O(new_n180_));
  nand4  g147(.a(new_n180_), .b(new_n50_), .c(new_n47_), .d(x15), .O(new_n181_));
  inv1   g148(.a(new_n181_), .O(new_n182_));
  nand4  g149(.a(new_n182_), .b(x14), .c(x11), .d(x02), .O(new_n183_));
  nand3  g150(.a(new_n183_), .b(new_n179_), .c(new_n151_), .O(new_n184_));
  nor2   g151(.a(new_n184_), .b(new_n175_), .O(new_n185_));
  nand3  g152(.a(new_n185_), .b(new_n170_), .c(new_n160_), .O(z6));
  oai21  g153(.a(new_n114_), .b(new_n111_), .c(new_n151_), .O(new_n187_));
  nor2   g154(.a(x06), .b(x02), .O(new_n188_));
  nand4  g155(.a(new_n188_), .b(new_n50_), .c(x21), .d(new_n37_), .O(new_n189_));
  nand2  g156(.a(new_n189_), .b(new_n187_), .O(z7));
endmodule


