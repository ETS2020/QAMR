// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n186_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x03), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x20), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nor2   g011(.a(x24), .b(x10), .O(new_n45_));
  inv1   g012(.a(x19), .O(new_n46_));
  nor2   g013(.a(new_n46_), .b(x13), .O(new_n47_));
  aoi22  g014(.a(new_n47_), .b(new_n39_), .c(new_n45_), .d(x00), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n43_), .c(new_n38_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n52_));
  aoi21  g019(.a(new_n52_), .b(new_n34_), .c(x01), .O(z0));
  inv1   g020(.a(x09), .O(new_n54_));
  nand3  g021(.a(x04), .b(x03), .c(x01), .O(new_n55_));
  nand2  g022(.a(x08), .b(x06), .O(new_n56_));
  nor4   g023(.a(new_n56_), .b(new_n55_), .c(new_n37_), .d(new_n54_), .O(new_n57_));
  nand4  g024(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(x22), .O(new_n59_));
  inv1   g026(.a(x23), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g028(.a(new_n61_), .b(x20), .c(x17), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand2  g030(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  inv1   g031(.a(x01), .O(new_n65_));
  inv1   g032(.a(x04), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n35_), .c(new_n34_), .d(new_n65_), .O(new_n67_));
  inv1   g034(.a(new_n67_), .O(new_n68_));
  nor2   g035(.a(x08), .b(x06), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n68_), .c(new_n37_), .d(new_n54_), .O(new_n70_));
  nor2   g037(.a(x14), .b(x12), .O(new_n71_));
  nor2   g038(.a(x17), .b(x16), .O(new_n72_));
  nor2   g039(.a(x20), .b(new_n46_), .O(new_n73_));
  nor2   g040(.a(x23), .b(x22), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  oai21  g042(.a(new_n75_), .b(new_n70_), .c(new_n64_), .O(new_n76_));
  inv1   g043(.a(x13), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n39_), .O(new_n78_));
  nand3  g045(.a(new_n41_), .b(x13), .c(x05), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n76_), .O(new_n81_));
  nand4  g048(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n82_));
  inv1   g049(.a(new_n82_), .O(new_n83_));
  inv1   g050(.a(x18), .O(new_n84_));
  nor2   g051(.a(new_n38_), .b(new_n84_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n83_), .c(new_n61_), .d(new_n57_), .O(new_n86_));
  nor2   g053(.a(x09), .b(x08), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n68_), .c(x07), .d(new_n36_), .O(new_n88_));
  nor2   g055(.a(x12), .b(x11), .O(new_n89_));
  nor2   g056(.a(x16), .b(x14), .O(new_n90_));
  nor2   g057(.a(x20), .b(x17), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n74_), .O(new_n92_));
  oai21  g059(.a(new_n92_), .b(new_n88_), .c(new_n86_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n78_), .c(x24), .O(new_n94_));
  inv1   g061(.a(x10), .O(new_n95_));
  nand3  g062(.a(new_n61_), .b(x21), .c(x20), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n82_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n57_), .O(new_n98_));
  nor2   g065(.a(x03), .b(x01), .O(new_n99_));
  nor2   g066(.a(x06), .b(x04), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n99_), .c(new_n87_), .d(x00), .O(new_n101_));
  oai21  g068(.a(new_n101_), .b(new_n92_), .c(new_n98_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n41_), .c(new_n95_), .d(new_n34_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n94_), .c(new_n81_), .O(z1));
  inv1   g071(.a(x14), .O(new_n105_));
  nand2  g072(.a(x24), .b(x18), .O(new_n106_));
  nand3  g073(.a(new_n41_), .b(x15), .c(x13), .O(new_n107_));
  aoi21  g074(.a(new_n107_), .b(new_n106_), .c(new_n39_), .O(new_n108_));
  nand3  g075(.a(x24), .b(x18), .c(x13), .O(new_n109_));
  nand3  g076(.a(x15), .b(new_n77_), .c(new_n39_), .O(new_n110_));
  nand4  g077(.a(new_n41_), .b(x21), .c(new_n95_), .d(new_n34_), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n108_), .c(x20), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n105_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(x11), .c(x06), .d(x03), .O(new_n115_));
  nand2  g082(.a(x02), .b(new_n65_), .O(new_n116_));
  oai21  g083(.a(new_n115_), .b(new_n65_), .c(new_n116_), .O(z2));
  nand4  g084(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n118_));
  nor2   g085(.a(new_n105_), .b(new_n37_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(x20), .c(x15), .O(new_n120_));
  nand2  g087(.a(new_n99_), .b(new_n69_), .O(new_n121_));
  nand3  g088(.a(new_n73_), .b(new_n105_), .c(new_n37_), .O(new_n122_));
  oai22  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .d(new_n118_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n80_), .O(new_n124_));
  nand2  g091(.a(new_n119_), .b(new_n85_), .O(new_n125_));
  nand3  g092(.a(new_n99_), .b(x07), .c(new_n36_), .O(new_n126_));
  inv1   g093(.a(x08), .O(new_n127_));
  nand4  g094(.a(new_n38_), .b(new_n105_), .c(new_n37_), .d(new_n127_), .O(new_n128_));
  oai22  g095(.a(new_n128_), .b(new_n126_), .c(new_n125_), .d(new_n118_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n78_), .c(x24), .O(new_n130_));
  nand3  g097(.a(new_n69_), .b(new_n35_), .c(x00), .O(new_n131_));
  nor2   g098(.a(x11), .b(x10), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n41_), .c(new_n38_), .d(new_n105_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n131_), .c(new_n34_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n65_), .O(new_n135_));
  nor4   g102(.a(new_n56_), .b(new_n35_), .c(x02), .d(new_n65_), .O(new_n136_));
  nand3  g103(.a(new_n41_), .b(x21), .c(x20), .O(new_n137_));
  inv1   g104(.a(new_n137_), .O(new_n138_));
  nand4  g105(.a(new_n138_), .b(new_n136_), .c(new_n119_), .d(new_n95_), .O(new_n139_));
  nand4  g106(.a(new_n139_), .b(new_n135_), .c(new_n130_), .d(new_n124_), .O(z3));
  inv1   g107(.a(x07), .O(new_n141_));
  nand2  g108(.a(new_n116_), .b(x08), .O(new_n142_));
  inv1   g109(.a(x16), .O(new_n143_));
  aoi21  g110(.a(new_n60_), .b(x04), .c(x17), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x22), .c(new_n54_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n143_), .c(new_n127_), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(new_n142_), .c(new_n141_), .O(new_n147_));
  inv1   g114(.a(x17), .O(new_n148_));
  aoi21  g115(.a(x23), .b(new_n66_), .c(new_n148_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n59_), .c(x09), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(x16), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(x08), .c(new_n84_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n147_), .c(new_n78_), .O(new_n153_));
  nand4  g120(.a(new_n47_), .b(x08), .c(new_n39_), .d(x01), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(x24), .O(new_n156_));
  nand2  g123(.a(new_n80_), .b(x15), .O(new_n157_));
  aoi22  g124(.a(new_n157_), .b(new_n111_), .c(new_n151_), .d(x08), .O(new_n158_));
  nand2  g125(.a(x24), .b(x08), .O(new_n159_));
  nand3  g126(.a(new_n159_), .b(new_n77_), .c(new_n39_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n79_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(x19), .O(new_n162_));
  nand3  g129(.a(new_n45_), .b(new_n34_), .c(x00), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand3  g131(.a(new_n164_), .b(new_n145_), .c(new_n143_), .O(new_n165_));
  nand2  g132(.a(x13), .b(x05), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n78_), .O(new_n167_));
  nand3  g134(.a(new_n167_), .b(new_n41_), .c(x19), .O(new_n168_));
  oai21  g135(.a(new_n48_), .b(x02), .c(new_n168_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(x08), .O(new_n170_));
  nand3  g137(.a(new_n170_), .b(new_n165_), .c(new_n116_), .O(new_n171_));
  nor2   g138(.a(new_n171_), .b(new_n158_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(new_n156_), .O(z4));
  nand3  g140(.a(x19), .b(new_n77_), .c(new_n39_), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(new_n44_), .O(new_n175_));
  oai21  g142(.a(new_n175_), .b(new_n43_), .c(new_n116_), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(new_n163_), .O(z5));
  aoi21  g144(.a(x20), .b(new_n105_), .c(x06), .O(new_n178_));
  oai21  g145(.a(new_n178_), .b(x11), .c(new_n35_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(z5), .O(new_n180_));
  aoi21  g147(.a(new_n38_), .b(x14), .c(new_n36_), .O(new_n181_));
  oai21  g148(.a(new_n181_), .b(new_n37_), .c(x03), .O(new_n182_));
  nand2  g149(.a(new_n110_), .b(new_n109_), .O(new_n183_));
  oai21  g150(.a(new_n183_), .b(new_n108_), .c(new_n116_), .O(new_n184_));
  nand2  g151(.a(new_n184_), .b(new_n111_), .O(z7));
  nand2  g152(.a(z7), .b(new_n182_), .O(new_n186_));
  nand2  g153(.a(new_n186_), .b(new_n180_), .O(z6));
endmodule


