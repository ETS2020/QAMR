// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n199_,
    new_n200_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x20), .O(new_n35_));
  inv1   g002(.a(x03), .O(new_n36_));
  inv1   g003(.a(x06), .O(new_n37_));
  inv1   g004(.a(x00), .O(new_n38_));
  nor2   g005(.a(x01), .b(new_n38_), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  inv1   g007(.a(x10), .O(new_n41_));
  inv1   g008(.a(x11), .O(new_n42_));
  inv1   g009(.a(x14), .O(new_n43_));
  inv1   g010(.a(x24), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n43_), .c(new_n42_), .d(new_n41_), .O(new_n45_));
  oai21  g012(.a(new_n45_), .b(new_n40_), .c(new_n35_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n34_), .O(new_n47_));
  inv1   g014(.a(x05), .O(new_n48_));
  nand2  g015(.a(x24), .b(x07), .O(new_n49_));
  nand3  g016(.a(new_n44_), .b(x19), .c(x13), .O(new_n50_));
  aoi21  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand3  g018(.a(x24), .b(x13), .c(x07), .O(new_n52_));
  inv1   g019(.a(x13), .O(new_n53_));
  nand3  g020(.a(x19), .b(new_n53_), .c(new_n48_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  oai21  g022(.a(new_n55_), .b(new_n51_), .c(new_n35_), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(x14), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n42_), .c(new_n37_), .d(new_n36_), .O(new_n58_));
  oai21  g025(.a(new_n58_), .b(x01), .c(new_n47_), .O(z0));
  nor2   g026(.a(x04), .b(x03), .O(new_n60_));
  nor2   g027(.a(x08), .b(x06), .O(new_n61_));
  nor2   g028(.a(x10), .b(x09), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n39_), .O(new_n63_));
  inv1   g030(.a(x17), .O(new_n64_));
  inv1   g031(.a(x22), .O(new_n65_));
  inv1   g032(.a(x12), .O(new_n66_));
  inv1   g033(.a(x16), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n43_), .c(new_n66_), .d(new_n42_), .O(new_n68_));
  inv1   g035(.a(new_n68_), .O(new_n69_));
  nor2   g036(.a(x24), .b(x23), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n65_), .d(new_n64_), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n63_), .c(new_n35_), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n34_), .O(new_n73_));
  inv1   g040(.a(x01), .O(new_n74_));
  inv1   g041(.a(x04), .O(new_n75_));
  nand4  g042(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n76_));
  nor4   g043(.a(new_n76_), .b(new_n75_), .c(new_n36_), .d(new_n74_), .O(new_n77_));
  nand4  g044(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n78_));
  nand4  g045(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  inv1   g048(.a(x09), .O(new_n82_));
  nor3   g049(.a(x04), .b(x03), .c(x01), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n61_), .c(new_n42_), .d(new_n82_), .O(new_n84_));
  nor2   g051(.a(x14), .b(x12), .O(new_n85_));
  nor2   g052(.a(x17), .b(x16), .O(new_n86_));
  inv1   g053(.a(x19), .O(new_n87_));
  nor2   g054(.a(x20), .b(new_n87_), .O(new_n88_));
  nor2   g055(.a(x23), .b(x22), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n85_), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n84_), .c(new_n81_), .O(new_n91_));
  nor2   g058(.a(x13), .b(x05), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand3  g060(.a(new_n44_), .b(x13), .c(x05), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand4  g063(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n97_));
  and2   g064(.a(x20), .b(x18), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(x23), .c(x22), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n77_), .O(new_n101_));
  inv1   g068(.a(x08), .O(new_n102_));
  inv1   g069(.a(x07), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(x06), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n83_), .c(new_n82_), .d(new_n102_), .O(new_n105_));
  nand4  g072(.a(new_n89_), .b(new_n69_), .c(new_n35_), .d(new_n64_), .O(new_n106_));
  oai21  g073(.a(new_n106_), .b(new_n105_), .c(new_n101_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n93_), .c(x24), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n96_), .c(new_n73_), .O(z1));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  nand3  g077(.a(new_n44_), .b(x15), .c(x13), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(x05), .O(new_n113_));
  nand3  g080(.a(x24), .b(x18), .c(x13), .O(new_n114_));
  nand3  g081(.a(x15), .b(new_n53_), .c(new_n48_), .O(new_n115_));
  nand3  g082(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x20), .c(x14), .d(x11), .O(new_n117_));
  inv1   g084(.a(new_n117_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x06), .c(x03), .d(x02), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n74_), .O(z2));
  nand4  g087(.a(x06), .b(x03), .c(x02), .d(x01), .O(new_n121_));
  nand2  g088(.a(x11), .b(x08), .O(new_n122_));
  nand3  g089(.a(x20), .b(x15), .c(x14), .O(new_n123_));
  nor3   g090(.a(new_n123_), .b(new_n122_), .c(new_n121_), .O(new_n124_));
  nand2  g091(.a(new_n36_), .b(new_n74_), .O(new_n125_));
  inv1   g092(.a(new_n125_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n61_), .O(new_n127_));
  nand3  g094(.a(new_n88_), .b(new_n43_), .c(new_n42_), .O(new_n128_));
  nor2   g095(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g096(.a(new_n129_), .b(new_n124_), .c(new_n95_), .O(new_n130_));
  inv1   g097(.a(new_n122_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n98_), .c(x14), .O(new_n132_));
  nor2   g099(.a(x11), .b(x08), .O(new_n133_));
  nor2   g100(.a(x20), .b(x14), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n133_), .c(new_n126_), .d(new_n104_), .O(new_n135_));
  oai21  g102(.a(new_n132_), .b(new_n121_), .c(new_n135_), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n93_), .c(x24), .O(new_n137_));
  nor3   g104(.a(x06), .b(x03), .c(x02), .O(new_n138_));
  nor3   g105(.a(x11), .b(x10), .c(x08), .O(new_n139_));
  nor3   g106(.a(x24), .b(x20), .c(x14), .O(new_n140_));
  nand4  g107(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n39_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n137_), .c(new_n130_), .O(z3));
  oai21  g109(.a(x23), .b(new_n75_), .c(new_n64_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n65_), .c(x09), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x16), .c(new_n102_), .O(new_n145_));
  nand4  g112(.a(new_n44_), .b(new_n41_), .c(new_n34_), .d(x00), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n54_), .c(new_n52_), .O(new_n147_));
  or2    g114(.a(new_n147_), .b(new_n51_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  inv1   g116(.a(x23), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(x04), .c(x17), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(x22), .c(new_n82_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n67_), .c(x08), .O(new_n153_));
  nor2   g120(.a(x10), .b(x02), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n44_), .c(x21), .O(new_n155_));
  nand4  g122(.a(new_n155_), .b(new_n115_), .c(new_n114_), .d(new_n113_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand2  g124(.a(x20), .b(new_n34_), .O(new_n158_));
  nand3  g125(.a(new_n158_), .b(new_n157_), .c(new_n149_), .O(z4));
  nor2   g126(.a(new_n55_), .b(new_n51_), .O(new_n160_));
  nand3  g127(.a(new_n44_), .b(new_n41_), .c(x00), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n35_), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n34_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n160_), .O(z5));
  oai21  g131(.a(x11), .b(new_n37_), .c(new_n36_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n148_), .O(new_n166_));
  nor2   g133(.a(new_n35_), .b(x02), .O(new_n167_));
  nor2   g134(.a(new_n167_), .b(x03), .O(new_n168_));
  nand2  g135(.a(new_n37_), .b(x03), .O(new_n169_));
  nand2  g136(.a(new_n35_), .b(x14), .O(new_n170_));
  aoi21  g137(.a(new_n170_), .b(new_n169_), .c(new_n42_), .O(new_n171_));
  oai21  g138(.a(new_n171_), .b(new_n168_), .c(x18), .O(new_n172_));
  nand4  g139(.a(x20), .b(new_n43_), .c(new_n42_), .d(x07), .O(new_n173_));
  aoi21  g140(.a(new_n173_), .b(new_n172_), .c(new_n92_), .O(new_n174_));
  inv1   g141(.a(x15), .O(new_n175_));
  nand3  g142(.a(new_n37_), .b(new_n48_), .c(x03), .O(new_n176_));
  nor4   g143(.a(new_n176_), .b(new_n175_), .c(x13), .d(new_n42_), .O(new_n177_));
  oai21  g144(.a(new_n177_), .b(new_n174_), .c(x24), .O(new_n178_));
  inv1   g145(.a(new_n168_), .O(new_n179_));
  nand3  g146(.a(new_n35_), .b(x14), .c(x11), .O(new_n180_));
  aoi21  g147(.a(new_n180_), .b(new_n179_), .c(new_n175_), .O(new_n181_));
  nand4  g148(.a(x20), .b(x19), .c(new_n43_), .d(new_n42_), .O(new_n182_));
  inv1   g149(.a(new_n182_), .O(new_n183_));
  oai21  g150(.a(new_n183_), .b(new_n181_), .c(new_n95_), .O(new_n184_));
  xnor2a g151(.a(x13), .b(x05), .O(new_n185_));
  nand3  g152(.a(new_n185_), .b(new_n158_), .c(x15), .O(new_n186_));
  nand3  g153(.a(new_n154_), .b(x21), .c(new_n35_), .O(new_n187_));
  aoi21  g154(.a(new_n187_), .b(new_n186_), .c(x06), .O(new_n188_));
  inv1   g155(.a(new_n154_), .O(new_n189_));
  inv1   g156(.a(x21), .O(new_n190_));
  nor2   g157(.a(new_n190_), .b(x20), .O(new_n191_));
  nand2  g158(.a(new_n191_), .b(x14), .O(new_n192_));
  nor2   g159(.a(new_n192_), .b(new_n189_), .O(new_n193_));
  oai21  g160(.a(new_n193_), .b(new_n188_), .c(x11), .O(new_n194_));
  nand4  g161(.a(new_n191_), .b(new_n41_), .c(new_n36_), .d(new_n34_), .O(new_n195_));
  nand2  g162(.a(new_n195_), .b(new_n194_), .O(new_n196_));
  aoi21  g163(.a(new_n196_), .b(new_n44_), .c(new_n167_), .O(new_n197_));
  nand4  g164(.a(new_n197_), .b(new_n184_), .c(new_n178_), .d(new_n166_), .O(z6));
  nand2  g165(.a(new_n158_), .b(new_n116_), .O(new_n199_));
  nand4  g166(.a(new_n154_), .b(new_n44_), .c(x21), .d(new_n35_), .O(new_n200_));
  nand2  g167(.a(new_n200_), .b(new_n199_), .O(z7));
endmodule


