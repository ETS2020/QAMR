// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x01), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x10), .O(new_n37_));
  inv1   g004(.a(x00), .O(new_n38_));
  nor2   g005(.a(x03), .b(new_n38_), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  inv1   g007(.a(x11), .O(new_n41_));
  inv1   g008(.a(x14), .O(new_n42_));
  inv1   g009(.a(x20), .O(new_n43_));
  inv1   g010(.a(x24), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n43_), .c(new_n42_), .d(new_n41_), .O(new_n45_));
  oai21  g012(.a(new_n45_), .b(new_n40_), .c(new_n35_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n34_), .O(new_n47_));
  inv1   g014(.a(x03), .O(new_n48_));
  inv1   g015(.a(x05), .O(new_n49_));
  nand2  g016(.a(x24), .b(x07), .O(new_n50_));
  nand3  g017(.a(new_n44_), .b(x19), .c(x13), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand3  g019(.a(x24), .b(x13), .c(x07), .O(new_n53_));
  inv1   g020(.a(x13), .O(new_n54_));
  nand3  g021(.a(x19), .b(new_n54_), .c(new_n49_), .O(new_n55_));
  nand2  g022(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  oai21  g023(.a(new_n56_), .b(new_n52_), .c(new_n43_), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(x14), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n41_), .c(new_n36_), .d(new_n48_), .O(new_n59_));
  oai21  g026(.a(new_n59_), .b(x01), .c(new_n47_), .O(z0));
  nor2   g027(.a(x06), .b(x04), .O(new_n61_));
  nor2   g028(.a(x09), .b(x08), .O(new_n62_));
  nor2   g029(.a(x11), .b(x10), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n39_), .O(new_n64_));
  inv1   g031(.a(x22), .O(new_n65_));
  inv1   g032(.a(x12), .O(new_n66_));
  inv1   g033(.a(x16), .O(new_n67_));
  inv1   g034(.a(x17), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n42_), .d(new_n66_), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nor2   g037(.a(x24), .b(x23), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n65_), .d(new_n43_), .O(new_n72_));
  oai21  g039(.a(new_n72_), .b(new_n64_), .c(new_n35_), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n34_), .O(new_n74_));
  nand3  g041(.a(x04), .b(x03), .c(x01), .O(new_n75_));
  inv1   g042(.a(new_n75_), .O(new_n76_));
  nand4  g043(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  nand4  g045(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n79_));
  nand4  g046(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n78_), .c(new_n76_), .O(new_n82_));
  inv1   g049(.a(x09), .O(new_n83_));
  nor3   g050(.a(x04), .b(x03), .c(x01), .O(new_n84_));
  nor2   g051(.a(x08), .b(x06), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n84_), .c(new_n41_), .d(new_n83_), .O(new_n86_));
  nor2   g053(.a(x23), .b(x22), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n70_), .c(new_n43_), .d(x19), .O(new_n88_));
  oai21  g055(.a(new_n88_), .b(new_n86_), .c(new_n82_), .O(new_n89_));
  nand2  g056(.a(new_n54_), .b(new_n49_), .O(new_n90_));
  nand3  g057(.a(new_n44_), .b(x13), .c(x05), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand4  g060(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n94_));
  and2   g061(.a(x20), .b(x18), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(x23), .c(x22), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n78_), .c(new_n76_), .O(new_n98_));
  inv1   g065(.a(x07), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(x06), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n84_), .c(new_n62_), .O(new_n101_));
  nand4  g068(.a(new_n67_), .b(new_n42_), .c(new_n66_), .d(new_n41_), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n87_), .c(new_n43_), .d(new_n68_), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n101_), .c(new_n98_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n90_), .c(x24), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n93_), .c(new_n74_), .O(z1));
  nand2  g074(.a(x24), .b(x18), .O(new_n108_));
  nand3  g075(.a(new_n44_), .b(x15), .c(x13), .O(new_n109_));
  aoi21  g076(.a(new_n109_), .b(new_n108_), .c(new_n49_), .O(new_n110_));
  nand3  g077(.a(x15), .b(new_n54_), .c(new_n49_), .O(new_n111_));
  oai21  g078(.a(new_n108_), .b(new_n54_), .c(new_n111_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n110_), .c(x20), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n42_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(x11), .c(x06), .d(x03), .O(new_n115_));
  aoi21  g082(.a(new_n115_), .b(x02), .c(new_n35_), .O(z2));
  nand4  g083(.a(x06), .b(x03), .c(x02), .d(x01), .O(new_n117_));
  nand2  g084(.a(x11), .b(x08), .O(new_n118_));
  nand3  g085(.a(x20), .b(x15), .c(x14), .O(new_n119_));
  nor3   g086(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  nor2   g087(.a(x03), .b(x01), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n85_), .O(new_n122_));
  nand4  g089(.a(new_n43_), .b(x19), .c(new_n42_), .d(new_n41_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(new_n120_), .c(new_n92_), .O(new_n125_));
  inv1   g092(.a(new_n118_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n95_), .c(x14), .O(new_n127_));
  nor2   g094(.a(x11), .b(x08), .O(new_n128_));
  nor2   g095(.a(x20), .b(x14), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(new_n128_), .c(new_n121_), .d(new_n100_), .O(new_n130_));
  oai21  g097(.a(new_n127_), .b(new_n117_), .c(new_n130_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n90_), .c(x24), .O(new_n132_));
  nor2   g099(.a(x01), .b(new_n38_), .O(new_n133_));
  nor3   g100(.a(x06), .b(x03), .c(x02), .O(new_n134_));
  nor3   g101(.a(x11), .b(x10), .c(x08), .O(new_n135_));
  nor3   g102(.a(x24), .b(x20), .c(x14), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n135_), .c(new_n134_), .d(new_n133_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n132_), .c(new_n125_), .O(z3));
  inv1   g105(.a(x04), .O(new_n139_));
  oai21  g106(.a(x23), .b(new_n139_), .c(new_n68_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n65_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n83_), .c(x16), .O(new_n142_));
  nor2   g109(.a(new_n56_), .b(new_n52_), .O(new_n143_));
  nor2   g110(.a(x02), .b(new_n35_), .O(new_n144_));
  nand4  g111(.a(new_n133_), .b(new_n44_), .c(new_n37_), .d(new_n34_), .O(new_n145_));
  oai21  g112(.a(new_n144_), .b(new_n143_), .c(new_n145_), .O(new_n146_));
  oai21  g113(.a(new_n142_), .b(x08), .c(new_n146_), .O(new_n147_));
  inv1   g114(.a(x23), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(x04), .c(x17), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(x22), .c(new_n83_), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(new_n67_), .c(x08), .O(new_n151_));
  nor2   g118(.a(new_n112_), .b(new_n110_), .O(new_n152_));
  nor2   g119(.a(x02), .b(x01), .O(new_n153_));
  nand4  g120(.a(new_n153_), .b(new_n44_), .c(x21), .d(new_n37_), .O(new_n154_));
  oai21  g121(.a(new_n144_), .b(new_n152_), .c(new_n154_), .O(z7));
  nand2  g122(.a(z7), .b(new_n151_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n147_), .O(z4));
  nand3  g124(.a(new_n44_), .b(new_n37_), .c(x00), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n35_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n34_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n143_), .O(z5));
  aoi21  g128(.a(x20), .b(new_n42_), .c(x06), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x11), .c(new_n48_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n146_), .O(new_n164_));
  aoi21  g131(.a(new_n43_), .b(x14), .c(new_n36_), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(new_n41_), .c(x03), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(z7), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n164_), .O(z6));
endmodule


