// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n186_, new_n187_,
    new_n188_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x01), .O(z0));
  nand3  g020(.a(x04), .b(x03), .c(x01), .O(new_n54_));
  nand4  g021(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g023(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n57_));
  and2   g024(.a(x23), .b(x22), .O(new_n58_));
  nand3  g025(.a(new_n58_), .b(x20), .c(x17), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g027(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  inv1   g028(.a(x09), .O(new_n62_));
  nor3   g029(.a(x04), .b(x03), .c(x01), .O(new_n63_));
  nor2   g030(.a(x08), .b(x06), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n63_), .c(new_n36_), .d(new_n62_), .O(new_n65_));
  nor2   g032(.a(x14), .b(x12), .O(new_n66_));
  nor2   g033(.a(x17), .b(x16), .O(new_n67_));
  inv1   g034(.a(x19), .O(new_n68_));
  nor2   g035(.a(x20), .b(new_n68_), .O(new_n69_));
  nor2   g036(.a(x23), .b(x22), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n67_), .d(new_n66_), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n65_), .c(new_n61_), .O(new_n72_));
  nand2  g039(.a(new_n47_), .b(new_n38_), .O(new_n73_));
  nand3  g040(.a(new_n40_), .b(x13), .c(x05), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand4  g043(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  inv1   g045(.a(x18), .O(new_n79_));
  nor2   g046(.a(new_n37_), .b(new_n79_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n78_), .c(new_n58_), .d(new_n56_), .O(new_n81_));
  inv1   g048(.a(x07), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(x06), .O(new_n83_));
  nor2   g050(.a(x09), .b(x08), .O(new_n84_));
  nand3  g051(.a(new_n84_), .b(new_n83_), .c(new_n63_), .O(new_n85_));
  inv1   g052(.a(x12), .O(new_n86_));
  inv1   g053(.a(x14), .O(new_n87_));
  inv1   g054(.a(x16), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n36_), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nor2   g057(.a(x20), .b(x17), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n90_), .c(new_n70_), .O(new_n92_));
  oai21  g059(.a(new_n92_), .b(new_n85_), .c(new_n81_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n73_), .c(x24), .O(new_n94_));
  inv1   g061(.a(x00), .O(new_n95_));
  nor2   g062(.a(x01), .b(new_n95_), .O(new_n96_));
  nor2   g063(.a(x03), .b(x02), .O(new_n97_));
  nor2   g064(.a(x06), .b(x04), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(new_n84_), .O(new_n99_));
  nor3   g066(.a(x24), .b(x23), .c(x22), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n91_), .c(new_n90_), .O(new_n101_));
  oai21  g068(.a(new_n101_), .b(new_n99_), .c(new_n37_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n45_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n94_), .c(new_n76_), .O(z1));
  nand2  g071(.a(x24), .b(x18), .O(new_n105_));
  nand3  g072(.a(new_n40_), .b(x15), .c(x13), .O(new_n106_));
  aoi21  g073(.a(new_n106_), .b(new_n105_), .c(new_n38_), .O(new_n107_));
  nand3  g074(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n108_));
  oai21  g075(.a(new_n105_), .b(new_n47_), .c(new_n108_), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nor3   g077(.a(new_n110_), .b(new_n87_), .c(new_n36_), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(x06), .c(x03), .d(x01), .O(new_n112_));
  aoi21  g079(.a(new_n112_), .b(x10), .c(new_n37_), .O(z2));
  nand4  g080(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n114_));
  nand2  g081(.a(x11), .b(x10), .O(new_n115_));
  nand3  g082(.a(x20), .b(x15), .c(x14), .O(new_n116_));
  nor3   g083(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(new_n117_));
  inv1   g084(.a(x01), .O(new_n118_));
  nand2  g085(.a(new_n34_), .b(new_n118_), .O(new_n119_));
  inv1   g086(.a(new_n119_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n64_), .O(new_n121_));
  nand3  g088(.a(new_n69_), .b(new_n87_), .c(new_n36_), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  oai21  g090(.a(new_n123_), .b(new_n117_), .c(new_n75_), .O(new_n124_));
  inv1   g091(.a(new_n115_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n80_), .c(x14), .O(new_n126_));
  nor2   g093(.a(x11), .b(x08), .O(new_n127_));
  nor2   g094(.a(x20), .b(x14), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n127_), .c(new_n120_), .d(new_n83_), .O(new_n129_));
  oai21  g096(.a(new_n126_), .b(new_n114_), .c(new_n129_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n73_), .c(x24), .O(new_n131_));
  nor3   g098(.a(x06), .b(x03), .c(x02), .O(new_n132_));
  nor3   g099(.a(x11), .b(x10), .c(x08), .O(new_n133_));
  nor3   g100(.a(x24), .b(x20), .c(x14), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n96_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n131_), .c(new_n124_), .O(z3));
  inv1   g103(.a(x08), .O(new_n137_));
  inv1   g104(.a(x23), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(x04), .c(x17), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(x22), .c(new_n62_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n88_), .c(new_n137_), .O(new_n141_));
  oai21  g108(.a(x20), .b(new_n137_), .c(new_n141_), .O(new_n142_));
  oai21  g109(.a(new_n49_), .b(new_n42_), .c(new_n142_), .O(new_n143_));
  oai21  g110(.a(new_n138_), .b(x04), .c(x17), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(x22), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(x09), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x16), .c(new_n137_), .O(new_n147_));
  nand3  g114(.a(x10), .b(x08), .c(x07), .O(new_n148_));
  oai21  g115(.a(new_n147_), .b(new_n79_), .c(new_n148_), .O(new_n149_));
  nand3  g116(.a(new_n149_), .b(new_n73_), .c(x24), .O(new_n150_));
  inv1   g117(.a(x15), .O(new_n151_));
  nand3  g118(.a(x19), .b(x10), .c(x08), .O(new_n152_));
  oai21  g119(.a(new_n147_), .b(new_n151_), .c(new_n152_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n75_), .O(new_n154_));
  aoi21  g121(.a(new_n144_), .b(x22), .c(new_n62_), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(new_n88_), .c(x08), .O(new_n156_));
  nand4  g123(.a(new_n156_), .b(new_n40_), .c(x21), .d(new_n44_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n37_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n45_), .O(new_n159_));
  nand4  g126(.a(new_n159_), .b(new_n154_), .c(new_n150_), .d(new_n143_), .O(z4));
  nand2  g127(.a(x20), .b(new_n45_), .O(new_n161_));
  nand2  g128(.a(new_n48_), .b(new_n43_), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(new_n42_), .c(new_n161_), .O(new_n163_));
  nor2   g130(.a(x24), .b(x20), .O(new_n164_));
  nand4  g131(.a(new_n164_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n163_), .O(z5));
  oai21  g133(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(z5), .O(new_n168_));
  oai21  g135(.a(new_n36_), .b(x06), .c(x03), .O(new_n169_));
  nor2   g136(.a(x20), .b(new_n87_), .O(new_n170_));
  aoi22  g137(.a(new_n170_), .b(x11), .c(new_n169_), .d(new_n161_), .O(new_n171_));
  nor2   g138(.a(new_n45_), .b(new_n82_), .O(new_n172_));
  nand4  g139(.a(new_n172_), .b(x20), .c(new_n87_), .d(new_n36_), .O(new_n173_));
  oai21  g140(.a(new_n171_), .b(new_n79_), .c(new_n173_), .O(new_n174_));
  nand3  g141(.a(new_n174_), .b(new_n73_), .c(x24), .O(new_n175_));
  nor2   g142(.a(new_n171_), .b(new_n151_), .O(new_n176_));
  nand3  g143(.a(x20), .b(x19), .c(new_n87_), .O(new_n177_));
  nor3   g144(.a(new_n177_), .b(x11), .c(new_n45_), .O(new_n178_));
  oai21  g145(.a(new_n178_), .b(new_n176_), .c(new_n75_), .O(new_n179_));
  oai21  g146(.a(x14), .b(new_n35_), .c(x11), .O(new_n180_));
  nand2  g147(.a(new_n180_), .b(x03), .O(new_n181_));
  nand4  g148(.a(new_n181_), .b(new_n40_), .c(x21), .d(new_n37_), .O(new_n182_));
  inv1   g149(.a(new_n182_), .O(new_n183_));
  nand3  g150(.a(new_n183_), .b(new_n45_), .c(new_n44_), .O(new_n184_));
  nand4  g151(.a(new_n184_), .b(new_n179_), .c(new_n175_), .d(new_n168_), .O(z6));
  inv1   g152(.a(new_n161_), .O(new_n186_));
  nor2   g153(.a(x10), .b(x02), .O(new_n187_));
  nand4  g154(.a(new_n187_), .b(new_n40_), .c(x21), .d(new_n37_), .O(new_n188_));
  oai21  g155(.a(new_n186_), .b(new_n110_), .c(new_n188_), .O(z7));
endmodule


