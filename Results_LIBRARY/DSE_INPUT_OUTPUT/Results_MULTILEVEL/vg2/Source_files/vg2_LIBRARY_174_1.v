// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  inv1   g019(.a(x08), .O(new_n53_));
  nand2  g020(.a(x24), .b(new_n53_), .O(new_n54_));
  oai21  g021(.a(new_n52_), .b(x01), .c(new_n54_), .O(z0));
  inv1   g022(.a(x12), .O(new_n56_));
  nand3  g023(.a(x15), .b(x13), .c(x05), .O(new_n57_));
  nand3  g024(.a(x21), .b(new_n45_), .c(new_n44_), .O(new_n58_));
  nand2  g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(x23), .c(x22), .d(x20), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(x17), .c(x16), .d(x14), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(x11), .c(x09), .d(x08), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n35_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(x04), .c(x03), .d(x01), .O(new_n66_));
  inv1   g033(.a(x01), .O(new_n67_));
  inv1   g034(.a(x04), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  inv1   g036(.a(x14), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  inv1   g038(.a(x17), .O(new_n72_));
  inv1   g039(.a(x22), .O(new_n73_));
  inv1   g040(.a(x23), .O(new_n74_));
  inv1   g041(.a(x19), .O(new_n75_));
  xor2a  g042(.a(x13), .b(x05), .O(new_n76_));
  nand3  g043(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n77_));
  oai21  g044(.a(new_n76_), .b(new_n75_), .c(new_n77_), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n74_), .c(new_n73_), .d(new_n37_), .O(new_n79_));
  inv1   g046(.a(new_n79_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(x12), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n36_), .c(new_n69_), .d(new_n53_), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(x06), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n68_), .c(new_n34_), .d(new_n67_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n66_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n40_), .O(new_n87_));
  nand2  g054(.a(new_n47_), .b(new_n38_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(x24), .c(x18), .O(new_n89_));
  nand3  g056(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(x23), .c(x22), .d(x20), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(x17), .c(x16), .d(x14), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(new_n56_), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(x11), .c(x09), .d(x08), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n35_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(x04), .c(x03), .d(x01), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n87_), .O(z1));
  inv1   g066(.a(x18), .O(new_n100_));
  nor2   g067(.a(new_n40_), .b(new_n100_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(x08), .O(new_n102_));
  nand3  g069(.a(new_n40_), .b(x15), .c(x13), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(x05), .O(new_n105_));
  nand4  g072(.a(new_n54_), .b(x15), .c(new_n47_), .d(new_n38_), .O(new_n106_));
  nand4  g073(.a(new_n40_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n107_));
  nand3  g074(.a(new_n101_), .b(x13), .c(x08), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n105_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(x20), .c(x14), .d(x11), .O(new_n110_));
  inv1   g077(.a(new_n110_), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(x06), .c(x03), .d(x01), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n54_), .O(z2));
  nand4  g080(.a(new_n59_), .b(x20), .c(x14), .d(x11), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n53_), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(x06), .c(x03), .d(x01), .O(new_n116_));
  nand4  g083(.a(new_n78_), .b(new_n37_), .c(new_n70_), .d(new_n36_), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(x08), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(new_n35_), .c(new_n34_), .d(new_n67_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n40_), .O(new_n121_));
  nand4  g088(.a(new_n91_), .b(x20), .c(x14), .d(x11), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(new_n53_), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(x06), .c(x03), .d(x01), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n121_), .O(z3));
  inv1   g092(.a(x07), .O(new_n126_));
  aoi21  g093(.a(x23), .b(new_n68_), .c(new_n72_), .O(new_n127_));
  oai21  g094(.a(new_n127_), .b(new_n73_), .c(x09), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(x18), .c(x16), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n88_), .c(x24), .O(new_n131_));
  nand3  g098(.a(new_n40_), .b(x13), .c(x05), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n88_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(x19), .O(new_n134_));
  and2   g101(.a(new_n128_), .b(x16), .O(new_n135_));
  nand4  g102(.a(new_n135_), .b(x15), .c(new_n47_), .d(new_n38_), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n134_), .c(new_n131_), .d(new_n46_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(x08), .O(new_n138_));
  aoi21  g105(.a(new_n74_), .b(x04), .c(x17), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(x22), .c(new_n69_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n78_), .c(new_n71_), .O(new_n141_));
  inv1   g108(.a(new_n58_), .O(new_n142_));
  inv1   g109(.a(new_n76_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(x15), .c(new_n142_), .O(new_n144_));
  nor2   g111(.a(new_n135_), .b(new_n53_), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(new_n144_), .c(new_n141_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n40_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n138_), .O(z4));
  nand3  g115(.a(x24), .b(x08), .c(x07), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(new_n41_), .c(new_n38_), .O(new_n150_));
  inv1   g117(.a(new_n150_), .O(new_n151_));
  nand4  g118(.a(new_n54_), .b(x19), .c(new_n47_), .d(new_n38_), .O(new_n152_));
  oai21  g119(.a(new_n47_), .b(new_n126_), .c(x08), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(x24), .O(new_n154_));
  nand4  g121(.a(new_n154_), .b(new_n152_), .c(new_n151_), .d(new_n46_), .O(z5));
  aoi21  g122(.a(x20), .b(new_n70_), .c(x06), .O(new_n156_));
  oai21  g123(.a(new_n156_), .b(x11), .c(new_n34_), .O(new_n157_));
  nand4  g124(.a(x24), .b(x13), .c(x08), .d(x07), .O(new_n158_));
  nand3  g125(.a(new_n158_), .b(new_n152_), .c(new_n46_), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(new_n150_), .c(new_n157_), .O(new_n160_));
  aoi21  g127(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n161_));
  oai21  g128(.a(new_n161_), .b(new_n36_), .c(x03), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n109_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n160_), .O(z6));
  inv1   g131(.a(new_n103_), .O(new_n165_));
  oai21  g132(.a(new_n165_), .b(new_n101_), .c(x05), .O(new_n166_));
  oai21  g133(.a(new_n100_), .b(new_n47_), .c(x08), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(x24), .O(new_n168_));
  nand4  g135(.a(new_n168_), .b(new_n166_), .c(new_n107_), .d(new_n90_), .O(z7));
endmodule


