// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n197_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x04), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x02), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x13), .O(new_n45_));
  nand3  g012(.a(x19), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g014(.a(new_n47_), .b(new_n43_), .c(new_n38_), .O(new_n48_));
  inv1   g015(.a(x02), .O(new_n49_));
  inv1   g016(.a(x10), .O(new_n50_));
  nand4  g017(.a(new_n41_), .b(new_n50_), .c(new_n49_), .d(x00), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n48_), .c(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(z0));
  nand3  g021(.a(x04), .b(x03), .c(x01), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  inv1   g026(.a(x17), .O(new_n60_));
  inv1   g027(.a(x20), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g029(.a(x22), .O(new_n63_));
  inv1   g030(.a(x23), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n62_), .c(new_n59_), .d(new_n57_), .O(new_n66_));
  inv1   g033(.a(x09), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  inv1   g035(.a(x03), .O(new_n69_));
  nand4  g036(.a(new_n37_), .b(new_n69_), .c(new_n49_), .d(new_n68_), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  nor2   g038(.a(x08), .b(x06), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n35_), .d(new_n67_), .O(new_n73_));
  nor2   g040(.a(x14), .b(x12), .O(new_n74_));
  nor2   g041(.a(x17), .b(x16), .O(new_n75_));
  inv1   g042(.a(x19), .O(new_n76_));
  nor2   g043(.a(x20), .b(new_n76_), .O(new_n77_));
  nor2   g044(.a(x23), .b(x22), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n75_), .d(new_n74_), .O(new_n79_));
  oai21  g046(.a(new_n79_), .b(new_n73_), .c(new_n66_), .O(new_n80_));
  nor2   g047(.a(x13), .b(x05), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand3  g049(.a(new_n41_), .b(x13), .c(x05), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n80_), .O(new_n85_));
  nand4  g052(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  inv1   g054(.a(x18), .O(new_n88_));
  nor2   g055(.a(new_n61_), .b(new_n88_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n87_), .c(new_n65_), .d(new_n57_), .O(new_n90_));
  nor2   g057(.a(x09), .b(x08), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n71_), .c(x07), .d(new_n34_), .O(new_n92_));
  nor2   g059(.a(x12), .b(x11), .O(new_n93_));
  nor2   g060(.a(x16), .b(x14), .O(new_n94_));
  nor2   g061(.a(x20), .b(x17), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n78_), .O(new_n96_));
  oai21  g063(.a(new_n96_), .b(new_n92_), .c(new_n90_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n82_), .c(x24), .O(new_n98_));
  inv1   g065(.a(x21), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n61_), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n87_), .c(new_n65_), .d(new_n57_), .O(new_n101_));
  nor2   g068(.a(x03), .b(x01), .O(new_n102_));
  nor2   g069(.a(x06), .b(x04), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n102_), .c(new_n91_), .d(x00), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n96_), .c(new_n101_), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n41_), .c(new_n50_), .d(new_n49_), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(new_n98_), .c(new_n85_), .d(new_n38_), .O(z1));
  nand2  g074(.a(x24), .b(x18), .O(new_n108_));
  nand3  g075(.a(new_n41_), .b(x15), .c(x13), .O(new_n109_));
  aoi21  g076(.a(new_n109_), .b(new_n108_), .c(new_n39_), .O(new_n110_));
  nand3  g077(.a(x24), .b(x18), .c(x13), .O(new_n111_));
  nand3  g078(.a(x15), .b(new_n45_), .c(new_n39_), .O(new_n112_));
  nand4  g079(.a(new_n41_), .b(x21), .c(new_n50_), .d(new_n49_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  oai21  g081(.a(new_n114_), .b(new_n110_), .c(x20), .O(new_n115_));
  nor2   g082(.a(new_n115_), .b(new_n36_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x11), .c(x06), .d(x03), .O(new_n117_));
  oai21  g084(.a(new_n117_), .b(new_n68_), .c(new_n38_), .O(z2));
  inv1   g085(.a(x08), .O(new_n119_));
  nand3  g086(.a(x15), .b(x13), .c(x05), .O(new_n120_));
  nand3  g087(.a(x21), .b(new_n50_), .c(new_n49_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(x20), .c(x14), .d(x11), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x06), .c(x03), .d(x01), .O(new_n125_));
  nand3  g092(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n126_));
  nand3  g093(.a(x19), .b(x13), .c(x05), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n61_), .c(new_n36_), .d(new_n35_), .O(new_n129_));
  nor2   g096(.a(new_n129_), .b(x08), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(new_n34_), .c(new_n69_), .d(new_n68_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n125_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n41_), .O(new_n133_));
  nand4  g100(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n134_));
  nand3  g101(.a(new_n89_), .b(x14), .c(x11), .O(new_n135_));
  nand3  g102(.a(new_n102_), .b(x07), .c(new_n34_), .O(new_n136_));
  nand4  g103(.a(new_n61_), .b(new_n36_), .c(new_n35_), .d(new_n119_), .O(new_n137_));
  oai22  g104(.a(new_n137_), .b(new_n136_), .c(new_n135_), .d(new_n134_), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n82_), .c(x24), .O(new_n139_));
  nand4  g106(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n140_));
  nand2  g107(.a(new_n102_), .b(new_n72_), .O(new_n141_));
  nand3  g108(.a(new_n77_), .b(new_n36_), .c(new_n35_), .O(new_n142_));
  oai22  g109(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(new_n134_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n45_), .c(new_n39_), .O(new_n144_));
  nand4  g111(.a(new_n144_), .b(new_n139_), .c(new_n133_), .d(new_n38_), .O(z3));
  aoi21  g112(.a(new_n64_), .b(x04), .c(x17), .O(new_n146_));
  aoi21  g113(.a(new_n127_), .b(new_n126_), .c(new_n146_), .O(new_n147_));
  xnor2a g114(.a(x13), .b(x05), .O(new_n148_));
  nand4  g115(.a(new_n148_), .b(new_n64_), .c(x19), .d(x02), .O(new_n149_));
  inv1   g116(.a(new_n149_), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(new_n147_), .c(new_n63_), .O(new_n151_));
  nand2  g118(.a(new_n128_), .b(x09), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(new_n151_), .c(x16), .O(new_n153_));
  aoi21  g120(.a(x22), .b(new_n60_), .c(new_n67_), .O(new_n154_));
  aoi21  g121(.a(new_n121_), .b(new_n120_), .c(new_n154_), .O(new_n155_));
  oai21  g122(.a(new_n99_), .b(x10), .c(new_n120_), .O(new_n156_));
  nand4  g123(.a(new_n156_), .b(x23), .c(x22), .d(new_n37_), .O(new_n157_));
  inv1   g124(.a(new_n157_), .O(new_n158_));
  oai21  g125(.a(new_n158_), .b(new_n155_), .c(x16), .O(new_n159_));
  nand2  g126(.a(x08), .b(x00), .O(new_n160_));
  nand2  g127(.a(x21), .b(new_n119_), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n160_), .c(x10), .O(new_n162_));
  nand2  g129(.a(x15), .b(new_n119_), .O(new_n163_));
  nand2  g130(.a(x19), .b(x08), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(new_n163_), .c(new_n45_), .O(new_n165_));
  aoi22  g132(.a(new_n165_), .b(x05), .c(new_n162_), .d(new_n49_), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n159_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n153_), .c(new_n41_), .O(new_n168_));
  inv1   g135(.a(x07), .O(new_n169_));
  aoi21  g136(.a(x23), .b(new_n37_), .c(new_n60_), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(new_n63_), .c(x09), .O(new_n171_));
  aoi21  g138(.a(new_n171_), .b(x16), .c(new_n119_), .O(new_n172_));
  inv1   g139(.a(x16), .O(new_n173_));
  oai21  g140(.a(new_n146_), .b(x22), .c(new_n67_), .O(new_n174_));
  aoi21  g141(.a(new_n174_), .b(new_n173_), .c(x08), .O(new_n175_));
  oai22  g142(.a(new_n175_), .b(new_n169_), .c(new_n172_), .d(new_n88_), .O(new_n176_));
  nand3  g143(.a(new_n176_), .b(new_n82_), .c(x24), .O(new_n177_));
  nand2  g144(.a(new_n65_), .b(x16), .O(new_n178_));
  oai21  g145(.a(new_n178_), .b(new_n112_), .c(new_n49_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n37_), .O(new_n180_));
  oai21  g147(.a(new_n154_), .b(new_n173_), .c(x08), .O(new_n181_));
  nand2  g148(.a(new_n181_), .b(x15), .O(new_n182_));
  oai21  g149(.a(new_n175_), .b(new_n76_), .c(new_n182_), .O(new_n183_));
  nand3  g150(.a(new_n183_), .b(new_n45_), .c(new_n39_), .O(new_n184_));
  nand4  g151(.a(new_n184_), .b(new_n180_), .c(new_n177_), .d(new_n168_), .O(z4));
  inv1   g152(.a(new_n43_), .O(new_n186_));
  and2   g153(.a(new_n51_), .b(new_n46_), .O(new_n187_));
  nand4  g154(.a(new_n187_), .b(new_n44_), .c(new_n186_), .d(new_n38_), .O(z5));
  nand2  g155(.a(x20), .b(new_n36_), .O(new_n189_));
  aoi21  g156(.a(new_n189_), .b(new_n34_), .c(x11), .O(new_n190_));
  nand3  g157(.a(new_n187_), .b(new_n44_), .c(new_n186_), .O(new_n191_));
  oai21  g158(.a(new_n190_), .b(x03), .c(new_n191_), .O(new_n192_));
  nand2  g159(.a(new_n61_), .b(x14), .O(new_n193_));
  aoi21  g160(.a(new_n193_), .b(x06), .c(new_n35_), .O(new_n194_));
  oai22  g161(.a(new_n194_), .b(new_n69_), .c(new_n114_), .d(new_n110_), .O(new_n195_));
  nand3  g162(.a(new_n195_), .b(new_n192_), .c(new_n38_), .O(z6));
  nand4  g163(.a(new_n113_), .b(new_n112_), .c(new_n111_), .d(new_n38_), .O(new_n197_));
  or2    g164(.a(new_n197_), .b(new_n110_), .O(z7));
endmodule


