// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x09), .O(new_n37_));
  inv1   g004(.a(x10), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g006(.a(x05), .O(new_n40_));
  nand2  g007(.a(x24), .b(x07), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  aoi21  g010(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n40_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  oai21  g015(.a(new_n48_), .b(new_n44_), .c(new_n39_), .O(new_n49_));
  inv1   g016(.a(x02), .O(new_n50_));
  nor2   g017(.a(x24), .b(x10), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(x09), .c(new_n50_), .d(x00), .O(new_n52_));
  aoi21  g019(.a(new_n52_), .b(new_n49_), .c(x20), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n54_));
  nor3   g021(.a(new_n54_), .b(x03), .c(x01), .O(z0));
  inv1   g022(.a(x03), .O(new_n56_));
  inv1   g023(.a(x04), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g025(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  inv1   g027(.a(x08), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n34_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(x11), .c(x09), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  nand4  g031(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n65_));
  nand4  g032(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n64_), .c(new_n60_), .O(new_n68_));
  nor3   g035(.a(x04), .b(x03), .c(x01), .O(new_n69_));
  nor2   g036(.a(x08), .b(x06), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n35_), .d(new_n37_), .O(new_n71_));
  nor2   g038(.a(x14), .b(x12), .O(new_n72_));
  nor2   g039(.a(x17), .b(x16), .O(new_n73_));
  inv1   g040(.a(x19), .O(new_n74_));
  nor2   g041(.a(x20), .b(new_n74_), .O(new_n75_));
  nor2   g042(.a(x23), .b(x22), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n73_), .d(new_n72_), .O(new_n77_));
  oai21  g044(.a(new_n77_), .b(new_n71_), .c(new_n68_), .O(new_n78_));
  nor2   g045(.a(x13), .b(x05), .O(new_n79_));
  inv1   g046(.a(new_n79_), .O(new_n80_));
  nand3  g047(.a(new_n42_), .b(x13), .c(x05), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  nand4  g050(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n84_));
  nand4  g051(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n64_), .c(new_n60_), .O(new_n87_));
  nor2   g054(.a(x09), .b(x08), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n69_), .c(x07), .d(new_n34_), .O(new_n89_));
  nor2   g056(.a(x12), .b(x11), .O(new_n90_));
  nor2   g057(.a(x16), .b(x14), .O(new_n91_));
  nor2   g058(.a(x20), .b(x17), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n76_), .O(new_n93_));
  oai21  g060(.a(new_n93_), .b(new_n89_), .c(new_n87_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n80_), .c(x24), .O(new_n95_));
  nand3  g062(.a(new_n62_), .b(x12), .c(x11), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n58_), .c(new_n50_), .d(x01), .O(new_n98_));
  nand4  g065(.a(x20), .b(x17), .c(x16), .d(x14), .O(new_n99_));
  inv1   g066(.a(new_n99_), .O(new_n100_));
  inv1   g067(.a(x21), .O(new_n101_));
  inv1   g068(.a(x22), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n100_), .c(new_n42_), .d(x23), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n98_), .c(x09), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n38_), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n95_), .c(new_n83_), .O(z1));
  inv1   g074(.a(x01), .O(new_n108_));
  inv1   g075(.a(x20), .O(new_n109_));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  nand3  g077(.a(new_n42_), .b(x15), .c(x13), .O(new_n111_));
  aoi21  g078(.a(new_n111_), .b(new_n110_), .c(new_n40_), .O(new_n112_));
  nand3  g079(.a(x24), .b(x18), .c(x13), .O(new_n113_));
  nand3  g080(.a(x15), .b(new_n46_), .c(new_n40_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n112_), .c(new_n39_), .O(new_n116_));
  nor2   g083(.a(x24), .b(new_n101_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(new_n38_), .c(x09), .d(new_n50_), .O(new_n118_));
  aoi21  g085(.a(new_n118_), .b(new_n116_), .c(new_n109_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(x14), .c(x11), .d(x06), .O(new_n120_));
  nor3   g087(.a(new_n120_), .b(new_n56_), .c(new_n108_), .O(z2));
  nand3  g088(.a(x15), .b(x13), .c(x05), .O(new_n122_));
  nand3  g089(.a(x21), .b(new_n38_), .c(new_n50_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x20), .c(x14), .d(x11), .O(new_n125_));
  nor2   g092(.a(new_n125_), .b(new_n61_), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(x06), .c(x03), .d(x01), .O(new_n127_));
  nand3  g094(.a(new_n38_), .b(new_n50_), .c(x00), .O(new_n128_));
  nand3  g095(.a(x19), .b(x13), .c(x05), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(new_n109_), .c(new_n36_), .d(new_n35_), .O(new_n131_));
  nor2   g098(.a(new_n131_), .b(x08), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n34_), .c(new_n56_), .d(new_n108_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n127_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n42_), .O(new_n135_));
  nand3  g102(.a(new_n62_), .b(x03), .c(x01), .O(new_n136_));
  nand4  g103(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n137_));
  nand4  g104(.a(x07), .b(new_n34_), .c(new_n56_), .d(new_n108_), .O(new_n138_));
  nand4  g105(.a(new_n109_), .b(new_n36_), .c(new_n35_), .d(new_n61_), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n138_), .c(new_n137_), .d(new_n136_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n80_), .c(x24), .O(new_n141_));
  nand4  g108(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n142_));
  nand3  g109(.a(new_n70_), .b(new_n56_), .c(new_n108_), .O(new_n143_));
  nand3  g110(.a(new_n75_), .b(new_n36_), .c(new_n35_), .O(new_n144_));
  oai22  g111(.a(new_n144_), .b(new_n143_), .c(new_n142_), .d(new_n136_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n46_), .c(new_n40_), .O(new_n146_));
  nand4  g113(.a(new_n146_), .b(new_n141_), .c(new_n135_), .d(new_n39_), .O(z3));
  inv1   g114(.a(x16), .O(new_n148_));
  nand3  g115(.a(new_n51_), .b(new_n50_), .c(x00), .O(new_n149_));
  nand3  g116(.a(new_n149_), .b(new_n47_), .c(new_n45_), .O(new_n150_));
  oai22  g117(.a(new_n150_), .b(new_n44_), .c(new_n148_), .d(x08), .O(new_n151_));
  nand2  g118(.a(x23), .b(new_n57_), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(x17), .c(new_n102_), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(x16), .c(new_n61_), .O(new_n154_));
  nand3  g121(.a(new_n117_), .b(new_n38_), .c(new_n50_), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n114_), .c(new_n113_), .O(new_n156_));
  nor2   g123(.a(new_n156_), .b(new_n112_), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(new_n154_), .c(new_n151_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(x09), .O(new_n159_));
  inv1   g126(.a(new_n44_), .O(new_n160_));
  inv1   g127(.a(new_n48_), .O(new_n161_));
  inv1   g128(.a(x17), .O(new_n162_));
  oai21  g129(.a(x23), .b(new_n57_), .c(new_n162_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(new_n102_), .c(new_n148_), .O(new_n164_));
  aoi22  g131(.a(new_n164_), .b(new_n61_), .c(new_n161_), .d(new_n160_), .O(new_n165_));
  nor2   g132(.a(new_n115_), .b(new_n112_), .O(new_n166_));
  aoi21  g133(.a(x16), .b(new_n37_), .c(new_n61_), .O(new_n167_));
  nor2   g134(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  oai21  g135(.a(new_n168_), .b(new_n165_), .c(x10), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n159_), .O(z4));
  nand2  g137(.a(new_n37_), .b(x08), .O(new_n171_));
  nand4  g138(.a(new_n171_), .b(new_n42_), .c(new_n50_), .d(x00), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(x09), .O(new_n173_));
  nand2  g140(.a(new_n173_), .b(new_n38_), .O(new_n174_));
  nand3  g141(.a(new_n174_), .b(new_n161_), .c(new_n160_), .O(z5));
  nand2  g142(.a(new_n52_), .b(new_n49_), .O(new_n176_));
  aoi21  g143(.a(x20), .b(new_n36_), .c(x06), .O(new_n177_));
  oai21  g144(.a(new_n177_), .b(x11), .c(new_n56_), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n176_), .O(new_n179_));
  nand2  g146(.a(new_n118_), .b(new_n116_), .O(z7));
  aoi21  g147(.a(new_n109_), .b(x14), .c(new_n34_), .O(new_n181_));
  oai21  g148(.a(new_n181_), .b(new_n35_), .c(x03), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(z7), .O(new_n183_));
  nand2  g150(.a(new_n183_), .b(new_n179_), .O(z6));
endmodule


