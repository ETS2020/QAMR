// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n178_, new_n179_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nor2   g006(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g007(.a(x19), .O(new_n41_));
  nor2   g008(.a(x24), .b(new_n41_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x13), .O(new_n43_));
  inv1   g010(.a(new_n43_), .O(new_n44_));
  oai21  g011(.a(new_n44_), .b(new_n40_), .c(x05), .O(new_n45_));
  nand3  g012(.a(x24), .b(x13), .c(x07), .O(new_n46_));
  inv1   g013(.a(x02), .O(new_n47_));
  inv1   g014(.a(x10), .O(new_n48_));
  nand4  g015(.a(new_n39_), .b(new_n48_), .c(new_n47_), .d(x00), .O(new_n49_));
  inv1   g016(.a(x05), .O(new_n50_));
  inv1   g017(.a(x13), .O(new_n51_));
  nand3  g018(.a(x19), .b(new_n51_), .c(new_n50_), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n49_), .c(new_n46_), .d(new_n45_), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(x14), .O(new_n55_));
  nand4  g022(.a(new_n55_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(x01), .O(z0));
  inv1   g024(.a(x12), .O(new_n58_));
  inv1   g025(.a(x14), .O(new_n59_));
  inv1   g026(.a(x23), .O(new_n60_));
  nand3  g027(.a(x15), .b(x13), .c(x05), .O(new_n61_));
  nand3  g028(.a(x21), .b(new_n48_), .c(new_n47_), .O(new_n62_));
  aoi21  g029(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(x22), .c(x17), .d(x16), .O(new_n64_));
  nor3   g031(.a(new_n64_), .b(new_n59_), .c(new_n58_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(x11), .c(x09), .d(x08), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n35_), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(x04), .c(x03), .d(x01), .O(new_n68_));
  inv1   g035(.a(x04), .O(new_n69_));
  inv1   g036(.a(x09), .O(new_n70_));
  inv1   g037(.a(x17), .O(new_n71_));
  inv1   g038(.a(x22), .O(new_n72_));
  nand3  g039(.a(new_n48_), .b(new_n47_), .c(x00), .O(new_n73_));
  nand3  g040(.a(x19), .b(x13), .c(x05), .O(new_n74_));
  aoi21  g041(.a(new_n74_), .b(new_n73_), .c(x23), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n72_), .c(new_n37_), .d(new_n71_), .O(new_n76_));
  nor3   g043(.a(new_n76_), .b(x16), .c(x14), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n58_), .c(new_n36_), .d(new_n70_), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(x08), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n35_), .c(new_n69_), .d(new_n34_), .O(new_n80_));
  oai21  g047(.a(new_n80_), .b(x01), .c(new_n68_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n39_), .O(new_n82_));
  nand2  g049(.a(new_n51_), .b(new_n50_), .O(new_n83_));
  inv1   g050(.a(x01), .O(new_n84_));
  nand4  g051(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n85_));
  nor4   g052(.a(new_n85_), .b(new_n69_), .c(new_n34_), .d(new_n84_), .O(new_n86_));
  inv1   g053(.a(x16), .O(new_n87_));
  nand4  g054(.a(x23), .b(x22), .c(x18), .d(x17), .O(new_n88_));
  nor4   g055(.a(new_n88_), .b(new_n87_), .c(new_n59_), .d(new_n58_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  inv1   g057(.a(x08), .O(new_n91_));
  nor3   g058(.a(x04), .b(x03), .c(x01), .O(new_n92_));
  nor2   g059(.a(new_n38_), .b(x06), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n92_), .c(new_n70_), .d(new_n91_), .O(new_n94_));
  nand4  g061(.a(new_n87_), .b(new_n59_), .c(new_n58_), .d(new_n36_), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  nor2   g063(.a(x23), .b(x22), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n96_), .c(new_n37_), .d(new_n71_), .O(new_n98_));
  oai21  g065(.a(new_n98_), .b(new_n94_), .c(new_n90_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n83_), .c(x24), .O(new_n100_));
  nand3  g067(.a(x15), .b(x14), .c(x12), .O(new_n101_));
  nand4  g068(.a(x23), .b(x22), .c(x17), .d(x16), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n86_), .O(new_n104_));
  nor2   g071(.a(x08), .b(x06), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n92_), .c(new_n36_), .d(new_n70_), .O(new_n106_));
  nor2   g073(.a(x14), .b(x12), .O(new_n107_));
  nor2   g074(.a(x17), .b(x16), .O(new_n108_));
  nor2   g075(.a(x20), .b(new_n41_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n108_), .c(new_n107_), .d(new_n97_), .O(new_n110_));
  oai21  g077(.a(new_n110_), .b(new_n106_), .c(new_n104_), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n51_), .c(new_n50_), .O(new_n112_));
  nor2   g079(.a(x20), .b(new_n35_), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(new_n112_), .c(new_n100_), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n82_), .O(z1));
  nand2  g084(.a(x24), .b(x18), .O(new_n118_));
  nand3  g085(.a(new_n39_), .b(x15), .c(x13), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(new_n118_), .c(new_n50_), .O(new_n120_));
  nand3  g087(.a(x24), .b(x18), .c(x13), .O(new_n121_));
  inv1   g088(.a(new_n121_), .O(new_n122_));
  nand3  g089(.a(x15), .b(new_n51_), .c(new_n50_), .O(new_n123_));
  nand4  g090(.a(new_n39_), .b(x21), .c(new_n48_), .d(new_n47_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nor3   g092(.a(new_n125_), .b(new_n122_), .c(new_n120_), .O(new_n126_));
  nor2   g093(.a(new_n126_), .b(new_n59_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(x11), .c(x03), .d(x01), .O(new_n128_));
  aoi21  g095(.a(new_n128_), .b(x20), .c(new_n35_), .O(z2));
  nand2  g096(.a(new_n62_), .b(new_n61_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(x20), .c(x14), .d(x11), .O(new_n131_));
  nor2   g098(.a(new_n131_), .b(new_n91_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(x06), .c(x03), .d(x01), .O(new_n133_));
  nand2  g100(.a(new_n74_), .b(new_n73_), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n37_), .c(new_n59_), .d(new_n36_), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(x08), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n35_), .c(new_n34_), .d(new_n84_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n39_), .O(new_n139_));
  nand4  g106(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n140_));
  nand4  g107(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n141_));
  nand3  g108(.a(new_n93_), .b(new_n34_), .c(new_n84_), .O(new_n142_));
  nand4  g109(.a(new_n37_), .b(new_n59_), .c(new_n36_), .d(new_n91_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n140_), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n83_), .c(x24), .O(new_n145_));
  nand4  g112(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n146_));
  nand3  g113(.a(new_n105_), .b(new_n34_), .c(new_n84_), .O(new_n147_));
  nand3  g114(.a(new_n109_), .b(new_n59_), .c(new_n36_), .O(new_n148_));
  oai22  g115(.a(new_n148_), .b(new_n147_), .c(new_n146_), .d(new_n140_), .O(new_n149_));
  nand3  g116(.a(new_n149_), .b(new_n51_), .c(new_n50_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n145_), .c(new_n139_), .O(z3));
  oai21  g118(.a(x23), .b(new_n69_), .c(new_n71_), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(new_n72_), .c(x09), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(x16), .c(new_n91_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n53_), .O(new_n155_));
  inv1   g122(.a(new_n126_), .O(new_n156_));
  oai21  g123(.a(new_n60_), .b(x04), .c(x17), .O(new_n157_));
  aoi21  g124(.a(new_n157_), .b(x22), .c(new_n70_), .O(new_n158_));
  oai21  g125(.a(new_n158_), .b(new_n87_), .c(x08), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  nand3  g127(.a(new_n160_), .b(new_n155_), .c(new_n114_), .O(z4));
  and2   g128(.a(new_n49_), .b(new_n46_), .O(new_n162_));
  nand4  g129(.a(new_n162_), .b(new_n114_), .c(new_n52_), .d(new_n45_), .O(z5));
  oai21  g130(.a(new_n36_), .b(x06), .c(x03), .O(new_n164_));
  oai21  g131(.a(new_n125_), .b(new_n120_), .c(new_n164_), .O(new_n165_));
  aoi21  g132(.a(new_n42_), .b(x05), .c(new_n40_), .O(new_n166_));
  aoi21  g133(.a(x20), .b(new_n59_), .c(x06), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(x11), .c(new_n34_), .O(new_n168_));
  inv1   g135(.a(new_n168_), .O(new_n169_));
  oai22  g136(.a(new_n36_), .b(x06), .c(x05), .d(x03), .O(new_n170_));
  nand3  g137(.a(new_n170_), .b(x24), .c(x18), .O(new_n171_));
  oai21  g138(.a(new_n169_), .b(new_n166_), .c(new_n171_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(x13), .O(new_n173_));
  nand2  g140(.a(new_n40_), .b(x05), .O(new_n174_));
  nand3  g141(.a(new_n174_), .b(new_n52_), .c(new_n49_), .O(new_n175_));
  aoi21  g142(.a(new_n175_), .b(new_n168_), .c(new_n113_), .O(new_n176_));
  nand3  g143(.a(new_n176_), .b(new_n173_), .c(new_n165_), .O(z6));
  inv1   g144(.a(new_n120_), .O(new_n178_));
  and2   g145(.a(new_n123_), .b(new_n121_), .O(new_n179_));
  nand4  g146(.a(new_n179_), .b(new_n124_), .c(new_n178_), .d(new_n114_), .O(z7));
endmodule


