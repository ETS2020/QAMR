// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n177_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  inv1   g011(.a(x00), .O(new_n45_));
  nor2   g012(.a(x02), .b(new_n45_), .O(new_n46_));
  nand3  g013(.a(new_n46_), .b(new_n40_), .c(new_n44_), .O(new_n47_));
  inv1   g014(.a(x13), .O(new_n48_));
  nand3  g015(.a(x19), .b(new_n48_), .c(new_n38_), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n47_), .c(new_n43_), .O(new_n50_));
  oai21  g017(.a(new_n50_), .b(new_n42_), .c(new_n37_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(x14), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  inv1   g020(.a(x16), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n44_), .O(new_n55_));
  oai21  g022(.a(new_n53_), .b(x01), .c(new_n55_), .O(z0));
  inv1   g023(.a(x04), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n34_), .O(new_n58_));
  nand2  g025(.a(new_n58_), .b(x01), .O(new_n59_));
  nand4  g026(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g028(.a(x12), .O(new_n62_));
  inv1   g029(.a(x14), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g031(.a(new_n64_), .b(x16), .c(x15), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  inv1   g033(.a(x17), .O(new_n67_));
  nor2   g034(.a(new_n37_), .b(new_n67_), .O(new_n68_));
  inv1   g035(.a(x22), .O(new_n69_));
  inv1   g036(.a(x23), .O(new_n70_));
  nor2   g037(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n68_), .c(new_n66_), .d(new_n61_), .O(new_n72_));
  inv1   g039(.a(x09), .O(new_n73_));
  nor3   g040(.a(x04), .b(x03), .c(x01), .O(new_n74_));
  nor2   g041(.a(x08), .b(x06), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n74_), .c(new_n36_), .d(new_n73_), .O(new_n76_));
  nor2   g043(.a(x14), .b(x12), .O(new_n77_));
  nor2   g044(.a(x17), .b(x16), .O(new_n78_));
  inv1   g045(.a(x19), .O(new_n79_));
  nor2   g046(.a(x20), .b(new_n79_), .O(new_n80_));
  nor2   g047(.a(x23), .b(x22), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n80_), .c(new_n78_), .d(new_n77_), .O(new_n82_));
  oai21  g049(.a(new_n82_), .b(new_n76_), .c(new_n72_), .O(new_n83_));
  nand2  g050(.a(new_n48_), .b(new_n38_), .O(new_n84_));
  nand3  g051(.a(new_n40_), .b(x13), .c(x05), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand3  g054(.a(new_n64_), .b(x17), .c(x16), .O(new_n88_));
  inv1   g055(.a(new_n88_), .O(new_n89_));
  and2   g056(.a(x20), .b(x18), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n89_), .c(new_n71_), .d(new_n61_), .O(new_n91_));
  nor2   g058(.a(x09), .b(x08), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n74_), .c(x07), .d(new_n35_), .O(new_n93_));
  nand4  g060(.a(new_n54_), .b(new_n63_), .c(new_n62_), .d(new_n36_), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n81_), .c(new_n37_), .d(new_n67_), .O(new_n96_));
  oai21  g063(.a(new_n96_), .b(new_n93_), .c(new_n91_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n84_), .c(x24), .O(new_n98_));
  inv1   g065(.a(x02), .O(new_n99_));
  inv1   g066(.a(new_n60_), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n58_), .c(new_n99_), .d(x01), .O(new_n101_));
  and2   g068(.a(x22), .b(x21), .O(new_n102_));
  nor2   g069(.a(x24), .b(new_n70_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n102_), .c(new_n68_), .d(new_n64_), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n44_), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n98_), .c(new_n87_), .O(z1));
  inv1   g074(.a(x01), .O(new_n108_));
  nand2  g075(.a(x24), .b(x18), .O(new_n109_));
  nand3  g076(.a(new_n40_), .b(x15), .c(x13), .O(new_n110_));
  aoi21  g077(.a(new_n110_), .b(new_n109_), .c(new_n38_), .O(new_n111_));
  nand3  g078(.a(x15), .b(new_n48_), .c(new_n38_), .O(new_n112_));
  oai21  g079(.a(new_n109_), .b(new_n48_), .c(new_n112_), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n111_), .c(new_n55_), .O(new_n114_));
  nor2   g081(.a(x10), .b(x02), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(new_n40_), .c(x21), .d(x16), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n114_), .c(new_n37_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(x14), .c(x11), .d(x06), .O(new_n118_));
  nor3   g085(.a(new_n118_), .b(new_n34_), .c(new_n108_), .O(z2));
  inv1   g086(.a(x08), .O(new_n120_));
  nand3  g087(.a(x15), .b(x13), .c(x05), .O(new_n121_));
  nand4  g088(.a(x21), .b(x16), .c(new_n44_), .d(new_n99_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(x20), .c(x14), .d(x11), .O(new_n124_));
  nor2   g091(.a(new_n124_), .b(new_n120_), .O(new_n125_));
  nand4  g092(.a(new_n125_), .b(x06), .c(x03), .d(x01), .O(new_n126_));
  nand2  g093(.a(new_n115_), .b(x00), .O(new_n127_));
  nand3  g094(.a(x19), .b(x13), .c(x05), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(new_n37_), .c(new_n63_), .d(new_n36_), .O(new_n130_));
  nor2   g097(.a(new_n130_), .b(x08), .O(new_n131_));
  nand4  g098(.a(new_n131_), .b(new_n35_), .c(new_n34_), .d(new_n108_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n126_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n40_), .O(new_n134_));
  nand4  g101(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n135_));
  nand3  g102(.a(new_n90_), .b(x14), .c(x11), .O(new_n136_));
  nand4  g103(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n108_), .O(new_n137_));
  nand4  g104(.a(new_n37_), .b(new_n63_), .c(new_n36_), .d(new_n120_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n135_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n84_), .c(x24), .O(new_n140_));
  nand4  g107(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n141_));
  nand3  g108(.a(new_n75_), .b(new_n34_), .c(new_n108_), .O(new_n142_));
  nand3  g109(.a(new_n80_), .b(new_n63_), .c(new_n36_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n135_), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n48_), .c(new_n38_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n140_), .c(new_n55_), .O(new_n146_));
  inv1   g113(.a(new_n146_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n134_), .O(z3));
  nand2  g115(.a(new_n49_), .b(new_n43_), .O(new_n149_));
  inv1   g116(.a(new_n55_), .O(new_n150_));
  aoi21  g117(.a(new_n70_), .b(x04), .c(x17), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(x22), .c(new_n73_), .O(new_n152_));
  nand3  g119(.a(new_n152_), .b(new_n54_), .c(x10), .O(new_n153_));
  oai21  g120(.a(new_n150_), .b(new_n120_), .c(new_n153_), .O(new_n154_));
  oai21  g121(.a(new_n149_), .b(new_n42_), .c(new_n154_), .O(new_n155_));
  nor2   g122(.a(new_n150_), .b(x08), .O(new_n156_));
  nand2  g123(.a(x23), .b(new_n57_), .O(new_n157_));
  aoi21  g124(.a(new_n157_), .b(x17), .c(new_n69_), .O(new_n158_));
  nor2   g125(.a(new_n158_), .b(new_n73_), .O(new_n159_));
  nor2   g126(.a(new_n159_), .b(new_n54_), .O(new_n160_));
  oai22  g127(.a(new_n160_), .b(new_n156_), .c(new_n113_), .d(new_n111_), .O(new_n161_));
  nand2  g128(.a(x09), .b(x08), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(new_n158_), .c(x21), .O(new_n163_));
  nand2  g130(.a(x08), .b(x00), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(new_n163_), .c(x24), .O(new_n165_));
  nand4  g132(.a(new_n165_), .b(x16), .c(new_n44_), .d(new_n99_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n161_), .c(new_n155_), .O(z4));
  oai21  g134(.a(new_n149_), .b(new_n42_), .c(new_n55_), .O(new_n168_));
  nand4  g135(.a(new_n46_), .b(new_n40_), .c(x16), .d(new_n44_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n168_), .O(z5));
  aoi21  g137(.a(x20), .b(new_n63_), .c(x06), .O(new_n171_));
  oai21  g138(.a(new_n171_), .b(x11), .c(new_n34_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(z5), .O(new_n173_));
  nand2  g140(.a(new_n116_), .b(new_n114_), .O(z7));
  aoi21  g141(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n175_));
  oai21  g142(.a(new_n175_), .b(new_n36_), .c(x03), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(z7), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n173_), .O(z6));
endmodule


