// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n183_;
  inv1   g000(.a(x11), .O(new_n34_));
  inv1   g001(.a(x14), .O(new_n35_));
  inv1   g002(.a(x20), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  inv1   g004(.a(x04), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g006(.a(x05), .O(new_n40_));
  nand2  g007(.a(x24), .b(x07), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  aoi21  g010(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n40_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  oai21  g015(.a(new_n48_), .b(new_n44_), .c(new_n39_), .O(new_n49_));
  nor2   g016(.a(x24), .b(x10), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(x04), .c(new_n37_), .d(x00), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n49_), .O(z5));
  nand4  g019(.a(z5), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor4   g020(.a(new_n53_), .b(x06), .c(x03), .d(x01), .O(z0));
  nand3  g021(.a(x04), .b(x03), .c(x01), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  inv1   g023(.a(x06), .O(new_n57_));
  inv1   g024(.a(x08), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g026(.a(new_n59_), .b(x11), .c(x09), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  nand2  g028(.a(new_n61_), .b(new_n56_), .O(new_n62_));
  nand4  g029(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n63_));
  and2   g030(.a(x23), .b(x22), .O(new_n64_));
  nand3  g031(.a(new_n64_), .b(x20), .c(x17), .O(new_n65_));
  or2    g032(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  inv1   g033(.a(x09), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  inv1   g035(.a(x03), .O(new_n69_));
  nand4  g036(.a(new_n38_), .b(new_n69_), .c(x02), .d(new_n68_), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  nor2   g038(.a(x08), .b(x06), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n34_), .d(new_n67_), .O(new_n73_));
  nor2   g040(.a(x14), .b(x12), .O(new_n74_));
  nor2   g041(.a(x17), .b(x16), .O(new_n75_));
  inv1   g042(.a(x19), .O(new_n76_));
  nor2   g043(.a(x20), .b(new_n76_), .O(new_n77_));
  nor2   g044(.a(x23), .b(x22), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n75_), .d(new_n74_), .O(new_n79_));
  oai22  g046(.a(new_n79_), .b(new_n73_), .c(new_n66_), .d(new_n62_), .O(new_n80_));
  nand2  g047(.a(new_n46_), .b(new_n40_), .O(new_n81_));
  nand3  g048(.a(new_n42_), .b(x13), .c(x05), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand4  g051(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n85_));
  inv1   g052(.a(new_n85_), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n64_), .c(x20), .d(x18), .O(new_n87_));
  inv1   g054(.a(x07), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(x06), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n71_), .c(new_n67_), .d(new_n58_), .O(new_n90_));
  nor2   g057(.a(x12), .b(x11), .O(new_n91_));
  nor2   g058(.a(x16), .b(x14), .O(new_n92_));
  nor2   g059(.a(x20), .b(x17), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n78_), .O(new_n94_));
  oai22  g061(.a(new_n94_), .b(new_n90_), .c(new_n87_), .d(new_n62_), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n81_), .c(x24), .O(new_n96_));
  nand4  g063(.a(x04), .b(x03), .c(new_n37_), .d(x01), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  inv1   g065(.a(x10), .O(new_n99_));
  nand3  g066(.a(x11), .b(new_n99_), .c(x09), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand2  g068(.a(x21), .b(x20), .O(new_n102_));
  nand3  g069(.a(new_n42_), .b(x23), .c(x22), .O(new_n103_));
  nor3   g070(.a(new_n103_), .b(new_n102_), .c(new_n85_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n101_), .c(new_n98_), .d(new_n59_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n96_), .c(new_n84_), .O(z1));
  nand2  g073(.a(x24), .b(x18), .O(new_n107_));
  nand3  g074(.a(new_n42_), .b(x15), .c(x13), .O(new_n108_));
  aoi21  g075(.a(new_n108_), .b(new_n107_), .c(new_n40_), .O(new_n109_));
  nand3  g076(.a(x15), .b(new_n46_), .c(new_n40_), .O(new_n110_));
  oai21  g077(.a(new_n107_), .b(new_n46_), .c(new_n110_), .O(new_n111_));
  oai21  g078(.a(new_n111_), .b(new_n109_), .c(new_n39_), .O(new_n112_));
  nand3  g079(.a(new_n42_), .b(x21), .c(new_n99_), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(x04), .c(new_n37_), .O(new_n115_));
  aoi21  g082(.a(new_n115_), .b(new_n112_), .c(new_n36_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x14), .c(x11), .d(x06), .O(new_n117_));
  nor3   g084(.a(new_n117_), .b(new_n69_), .c(new_n68_), .O(z2));
  nand3  g085(.a(x15), .b(x13), .c(x05), .O(new_n119_));
  nand3  g086(.a(x21), .b(new_n99_), .c(new_n37_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g088(.a(new_n121_), .b(x20), .c(x14), .d(x11), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(new_n58_), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(x06), .c(x03), .d(x01), .O(new_n124_));
  nand3  g091(.a(new_n99_), .b(new_n37_), .c(x00), .O(new_n125_));
  nand3  g092(.a(x19), .b(x13), .c(x05), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n128_));
  nor2   g095(.a(new_n128_), .b(x08), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(new_n57_), .c(new_n69_), .d(new_n68_), .O(new_n130_));
  aoi21  g097(.a(new_n130_), .b(new_n124_), .c(x24), .O(new_n131_));
  nand3  g098(.a(new_n59_), .b(x03), .c(x01), .O(new_n132_));
  nand4  g099(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n133_));
  nand3  g100(.a(new_n89_), .b(new_n69_), .c(new_n68_), .O(new_n134_));
  nand4  g101(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(new_n58_), .O(new_n135_));
  oai22  g102(.a(new_n135_), .b(new_n134_), .c(new_n133_), .d(new_n132_), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n81_), .c(x24), .O(new_n137_));
  nand4  g104(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n138_));
  nand3  g105(.a(new_n72_), .b(new_n69_), .c(new_n68_), .O(new_n139_));
  nand3  g106(.a(new_n77_), .b(new_n35_), .c(new_n34_), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n132_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n46_), .c(new_n40_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n137_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n131_), .c(x04), .O(new_n144_));
  nand2  g111(.a(new_n141_), .b(new_n83_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n137_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(x02), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n144_), .O(z3));
  nand2  g115(.a(new_n115_), .b(new_n112_), .O(new_n149_));
  inv1   g116(.a(x16), .O(new_n150_));
  inv1   g117(.a(x17), .O(new_n151_));
  aoi21  g118(.a(x22), .b(new_n151_), .c(new_n67_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n150_), .c(x08), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n149_), .O(new_n154_));
  inv1   g121(.a(x22), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(x17), .c(x09), .O(new_n156_));
  oai21  g123(.a(new_n156_), .b(x16), .c(new_n58_), .O(new_n157_));
  nor2   g124(.a(x16), .b(new_n38_), .O(new_n158_));
  aoi22  g125(.a(new_n158_), .b(new_n78_), .c(new_n157_), .d(new_n39_), .O(new_n159_));
  nand3  g126(.a(x15), .b(new_n38_), .c(x02), .O(new_n160_));
  nand2  g127(.a(new_n64_), .b(x16), .O(new_n161_));
  oai22  g128(.a(new_n161_), .b(new_n160_), .c(new_n159_), .d(new_n76_), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n83_), .O(new_n163_));
  nor2   g130(.a(new_n150_), .b(x04), .O(new_n164_));
  nand4  g131(.a(new_n164_), .b(new_n64_), .c(x18), .d(x02), .O(new_n165_));
  oai21  g132(.a(new_n159_), .b(new_n88_), .c(new_n165_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n81_), .c(x24), .O(new_n167_));
  aoi21  g134(.a(x23), .b(new_n151_), .c(x22), .O(new_n168_));
  oai21  g135(.a(new_n168_), .b(x09), .c(new_n150_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n58_), .O(new_n170_));
  nand4  g137(.a(new_n170_), .b(new_n42_), .c(new_n99_), .d(x04), .O(new_n171_));
  inv1   g138(.a(new_n171_), .O(new_n172_));
  nand3  g139(.a(new_n172_), .b(new_n37_), .c(x00), .O(new_n173_));
  nand4  g140(.a(new_n173_), .b(new_n167_), .c(new_n163_), .d(new_n154_), .O(z4));
  aoi21  g141(.a(x20), .b(new_n35_), .c(x06), .O(new_n175_));
  oai21  g142(.a(new_n175_), .b(x11), .c(new_n69_), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(z5), .O(new_n177_));
  aoi21  g144(.a(new_n36_), .b(x14), .c(new_n57_), .O(new_n178_));
  oai21  g145(.a(new_n178_), .b(new_n34_), .c(x03), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n149_), .O(new_n180_));
  nand2  g147(.a(new_n180_), .b(new_n177_), .O(z6));
  nor2   g148(.a(new_n111_), .b(new_n109_), .O(new_n182_));
  oai21  g149(.a(new_n114_), .b(new_n38_), .c(new_n37_), .O(new_n183_));
  nand2  g150(.a(new_n183_), .b(new_n182_), .O(z7));
endmodule


