// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  nand2  g004(.a(x21), .b(new_n37_), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x13), .O(new_n45_));
  nand3  g012(.a(x19), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g014(.a(new_n47_), .b(new_n43_), .c(new_n38_), .O(new_n48_));
  inv1   g015(.a(x10), .O(new_n49_));
  nor2   g016(.a(x24), .b(x21), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n49_), .c(new_n37_), .d(x00), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n48_), .c(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(z0));
  inv1   g021(.a(new_n38_), .O(new_n55_));
  nand3  g022(.a(x04), .b(x03), .c(x01), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n60_));
  nand4  g027(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n59_), .c(new_n57_), .O(new_n63_));
  nor3   g030(.a(x04), .b(x03), .c(x01), .O(new_n64_));
  nor2   g031(.a(x08), .b(x06), .O(new_n65_));
  nor2   g032(.a(x11), .b(x09), .O(new_n66_));
  inv1   g033(.a(x12), .O(new_n67_));
  inv1   g034(.a(x16), .O(new_n68_));
  inv1   g035(.a(x17), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n68_), .c(new_n36_), .d(new_n67_), .O(new_n70_));
  inv1   g037(.a(x20), .O(new_n71_));
  inv1   g038(.a(x22), .O(new_n72_));
  inv1   g039(.a(x23), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x19), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n76_));
  nor2   g043(.a(x13), .b(x05), .O(new_n77_));
  nor2   g044(.a(x24), .b(new_n45_), .O(new_n78_));
  aoi21  g045(.a(new_n78_), .b(x05), .c(new_n77_), .O(new_n79_));
  aoi21  g046(.a(new_n76_), .b(new_n63_), .c(new_n79_), .O(new_n80_));
  nand4  g047(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n81_));
  nand4  g048(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n59_), .c(new_n57_), .O(new_n84_));
  inv1   g051(.a(x08), .O(new_n85_));
  inv1   g052(.a(x09), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n85_), .c(x07), .d(new_n34_), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand4  g055(.a(new_n68_), .b(new_n36_), .c(new_n67_), .d(new_n35_), .O(new_n89_));
  nand4  g056(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n69_), .O(new_n90_));
  nor2   g057(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n88_), .c(new_n64_), .O(new_n92_));
  aoi21  g059(.a(new_n92_), .b(new_n84_), .c(new_n77_), .O(new_n93_));
  aoi21  g060(.a(new_n93_), .b(x24), .c(new_n80_), .O(new_n94_));
  inv1   g061(.a(x01), .O(new_n95_));
  inv1   g062(.a(x03), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n37_), .c(new_n95_), .d(x00), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand3  g065(.a(new_n49_), .b(new_n86_), .c(new_n85_), .O(new_n99_));
  nor3   g066(.a(new_n99_), .b(x06), .c(x04), .O(new_n100_));
  nand3  g067(.a(new_n69_), .b(new_n68_), .c(new_n36_), .O(new_n101_));
  nor3   g068(.a(new_n101_), .b(x12), .c(x11), .O(new_n102_));
  nand3  g069(.a(new_n41_), .b(new_n73_), .c(new_n72_), .O(new_n103_));
  nor3   g070(.a(new_n103_), .b(x21), .c(x20), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n102_), .c(new_n100_), .d(new_n98_), .O(new_n105_));
  oai21  g072(.a(new_n94_), .b(new_n55_), .c(new_n105_), .O(z1));
  nand2  g073(.a(x24), .b(x18), .O(new_n107_));
  nand3  g074(.a(new_n41_), .b(x15), .c(x13), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(x05), .O(new_n110_));
  nand3  g077(.a(x24), .b(x18), .c(x13), .O(new_n111_));
  nand3  g078(.a(x15), .b(new_n45_), .c(new_n39_), .O(new_n112_));
  nand3  g079(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(x20), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n36_), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(x11), .c(x06), .d(x03), .O(new_n116_));
  oai21  g083(.a(new_n116_), .b(new_n95_), .c(new_n38_), .O(z2));
  nand4  g084(.a(new_n65_), .b(new_n96_), .c(new_n95_), .d(x00), .O(new_n118_));
  nand3  g085(.a(new_n41_), .b(new_n71_), .c(new_n36_), .O(new_n119_));
  nor4   g086(.a(new_n119_), .b(new_n118_), .c(x11), .d(x10), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(x21), .c(new_n37_), .O(new_n121_));
  inv1   g088(.a(new_n79_), .O(new_n122_));
  nand4  g089(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n123_));
  nand4  g090(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n124_));
  nand3  g091(.a(new_n65_), .b(new_n96_), .c(new_n95_), .O(new_n125_));
  nand4  g092(.a(new_n71_), .b(x19), .c(new_n36_), .d(new_n35_), .O(new_n126_));
  oai22  g093(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n123_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n122_), .O(new_n128_));
  inv1   g095(.a(new_n77_), .O(new_n129_));
  nand4  g096(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n130_));
  nand4  g097(.a(x07), .b(new_n34_), .c(new_n96_), .d(new_n95_), .O(new_n131_));
  nand4  g098(.a(new_n71_), .b(new_n36_), .c(new_n35_), .d(new_n85_), .O(new_n132_));
  oai22  g099(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n123_), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n129_), .c(x24), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n128_), .c(new_n121_), .O(z3));
  aoi21  g102(.a(new_n73_), .b(x04), .c(x17), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x22), .c(new_n86_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n68_), .c(new_n85_), .O(new_n138_));
  oai21  g105(.a(x21), .b(new_n85_), .c(new_n138_), .O(new_n139_));
  nand4  g106(.a(new_n41_), .b(new_n49_), .c(new_n37_), .d(x00), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n46_), .c(new_n44_), .O(new_n141_));
  or2    g108(.a(new_n141_), .b(new_n43_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  inv1   g110(.a(x18), .O(new_n144_));
  inv1   g111(.a(x04), .O(new_n145_));
  aoi21  g112(.a(x23), .b(new_n145_), .c(new_n69_), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n72_), .c(x09), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(x16), .c(new_n85_), .O(new_n148_));
  nand3  g115(.a(x08), .b(x07), .c(x02), .O(new_n149_));
  oai21  g116(.a(new_n148_), .b(new_n144_), .c(new_n149_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n129_), .c(x24), .O(new_n151_));
  inv1   g118(.a(x15), .O(new_n152_));
  nand3  g119(.a(x19), .b(x08), .c(x02), .O(new_n153_));
  oai21  g120(.a(new_n148_), .b(new_n152_), .c(new_n153_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n122_), .O(new_n155_));
  nand4  g122(.a(new_n155_), .b(new_n151_), .c(new_n143_), .d(new_n38_), .O(z4));
  nand2  g123(.a(new_n51_), .b(new_n48_), .O(z5));
  nand2  g124(.a(x20), .b(new_n36_), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(new_n34_), .c(x11), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(x03), .c(new_n142_), .O(new_n160_));
  nand2  g127(.a(new_n71_), .b(x14), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(x06), .c(new_n35_), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(new_n96_), .c(new_n113_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(new_n160_), .c(new_n38_), .O(z6));
  nand4  g131(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(new_n38_), .O(z7));
endmodule


