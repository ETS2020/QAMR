// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n178_, new_n179_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  inv1   g004(.a(x04), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g006(.a(x05), .O(new_n40_));
  nand2  g007(.a(x24), .b(x07), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  aoi21  g010(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n40_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  oai21  g015(.a(new_n48_), .b(new_n44_), .c(new_n39_), .O(new_n49_));
  nor2   g016(.a(x24), .b(x10), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(x04), .c(new_n37_), .d(x00), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n49_), .c(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(z0));
  inv1   g021(.a(x10), .O(new_n55_));
  nand4  g022(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  inv1   g024(.a(x12), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n35_), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n57_), .c(new_n55_), .d(x09), .O(new_n60_));
  inv1   g027(.a(x16), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n36_), .O(new_n62_));
  inv1   g029(.a(x17), .O(new_n63_));
  inv1   g030(.a(x20), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g032(.a(x21), .O(new_n66_));
  inv1   g033(.a(x22), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g035(.a(x23), .O(new_n69_));
  nor2   g036(.a(x24), .b(new_n69_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n68_), .c(new_n65_), .d(new_n62_), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n60_), .c(x04), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n37_), .O(new_n73_));
  inv1   g040(.a(x01), .O(new_n74_));
  inv1   g041(.a(x03), .O(new_n75_));
  nand4  g042(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n76_));
  nor4   g043(.a(new_n76_), .b(new_n38_), .c(new_n75_), .d(new_n74_), .O(new_n77_));
  nand4  g044(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  nor2   g046(.a(new_n69_), .b(new_n67_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n79_), .c(new_n77_), .d(new_n65_), .O(new_n81_));
  inv1   g048(.a(x09), .O(new_n82_));
  nor3   g049(.a(x04), .b(x03), .c(x01), .O(new_n83_));
  nor2   g050(.a(x08), .b(x06), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n83_), .c(new_n35_), .d(new_n82_), .O(new_n85_));
  nor2   g052(.a(x14), .b(x12), .O(new_n86_));
  nor2   g053(.a(x17), .b(x16), .O(new_n87_));
  inv1   g054(.a(x19), .O(new_n88_));
  nor2   g055(.a(x20), .b(new_n88_), .O(new_n89_));
  nor2   g056(.a(x23), .b(x22), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n89_), .c(new_n87_), .d(new_n86_), .O(new_n91_));
  oai21  g058(.a(new_n91_), .b(new_n85_), .c(new_n81_), .O(new_n92_));
  nand2  g059(.a(new_n46_), .b(new_n40_), .O(new_n93_));
  nand3  g060(.a(new_n42_), .b(x13), .c(x05), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nand4  g063(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  and2   g065(.a(x20), .b(x18), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(new_n80_), .d(new_n77_), .O(new_n100_));
  inv1   g067(.a(x08), .O(new_n101_));
  inv1   g068(.a(x07), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(x06), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n83_), .c(new_n82_), .d(new_n101_), .O(new_n104_));
  nand4  g071(.a(new_n61_), .b(new_n36_), .c(new_n58_), .d(new_n35_), .O(new_n105_));
  inv1   g072(.a(new_n105_), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(new_n90_), .c(new_n64_), .d(new_n63_), .O(new_n107_));
  oai21  g074(.a(new_n107_), .b(new_n104_), .c(new_n100_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n93_), .c(x24), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n96_), .c(new_n73_), .O(z1));
  nand4  g077(.a(new_n55_), .b(x06), .c(x03), .d(x01), .O(new_n111_));
  nor2   g078(.a(new_n36_), .b(new_n35_), .O(new_n112_));
  nor2   g079(.a(x24), .b(new_n66_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n112_), .c(x20), .O(new_n114_));
  oai21  g081(.a(new_n114_), .b(new_n111_), .c(x04), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n37_), .O(new_n116_));
  nand2  g083(.a(x24), .b(x18), .O(new_n117_));
  nand3  g084(.a(new_n42_), .b(x15), .c(x13), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(x05), .O(new_n120_));
  nand3  g087(.a(x24), .b(x18), .c(x13), .O(new_n121_));
  nand3  g088(.a(x15), .b(new_n46_), .c(new_n40_), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(x20), .c(x14), .d(x11), .O(new_n124_));
  inv1   g091(.a(new_n124_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(x06), .c(x03), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(new_n74_), .c(new_n116_), .O(z2));
  nand3  g094(.a(new_n112_), .b(x20), .c(x15), .O(new_n128_));
  nand3  g095(.a(new_n84_), .b(new_n75_), .c(new_n74_), .O(new_n129_));
  nand3  g096(.a(new_n89_), .b(new_n36_), .c(new_n35_), .O(new_n130_));
  oai22  g097(.a(new_n130_), .b(new_n129_), .c(new_n128_), .d(new_n56_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n95_), .O(new_n132_));
  nand3  g099(.a(new_n112_), .b(new_n99_), .c(new_n57_), .O(new_n133_));
  nand3  g100(.a(new_n103_), .b(new_n75_), .c(new_n74_), .O(new_n134_));
  nand4  g101(.a(new_n64_), .b(new_n36_), .c(new_n35_), .d(new_n101_), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n93_), .c(x24), .O(new_n137_));
  nand4  g104(.a(new_n34_), .b(new_n75_), .c(new_n74_), .d(x00), .O(new_n138_));
  nand3  g105(.a(new_n112_), .b(x21), .c(x20), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n56_), .c(new_n138_), .d(new_n135_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n42_), .c(new_n55_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(x04), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n37_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n137_), .c(new_n132_), .O(z3));
  nand2  g111(.a(x22), .b(new_n63_), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x09), .c(new_n61_), .O(new_n146_));
  nand3  g113(.a(new_n113_), .b(new_n55_), .c(new_n37_), .O(new_n147_));
  nand4  g114(.a(new_n147_), .b(new_n122_), .c(new_n121_), .d(new_n120_), .O(new_n148_));
  oai21  g115(.a(new_n146_), .b(new_n101_), .c(new_n148_), .O(new_n149_));
  aoi21  g116(.a(new_n69_), .b(x04), .c(x17), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(x22), .c(new_n82_), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(new_n61_), .c(x08), .O(new_n152_));
  nand4  g119(.a(new_n80_), .b(x16), .c(x15), .d(new_n38_), .O(new_n153_));
  oai21  g120(.a(new_n152_), .b(new_n88_), .c(new_n153_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n95_), .O(new_n155_));
  nand4  g122(.a(new_n80_), .b(x18), .c(x16), .d(new_n38_), .O(new_n156_));
  oai21  g123(.a(new_n152_), .b(new_n102_), .c(new_n156_), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(new_n93_), .c(x24), .O(new_n158_));
  aoi21  g125(.a(x23), .b(new_n63_), .c(x22), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(x09), .c(new_n61_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n101_), .O(new_n161_));
  nand4  g128(.a(new_n161_), .b(new_n42_), .c(new_n55_), .d(x00), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(x04), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n37_), .O(new_n164_));
  nand4  g131(.a(new_n164_), .b(new_n158_), .c(new_n155_), .d(new_n149_), .O(z4));
  nor2   g132(.a(new_n48_), .b(new_n44_), .O(new_n166_));
  aoi21  g133(.a(new_n50_), .b(x00), .c(new_n38_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(x02), .c(new_n166_), .O(z5));
  nand2  g135(.a(x20), .b(new_n36_), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(new_n34_), .c(x11), .O(new_n170_));
  nand3  g137(.a(new_n50_), .b(new_n37_), .c(x00), .O(new_n171_));
  nand3  g138(.a(new_n171_), .b(new_n47_), .c(new_n45_), .O(new_n172_));
  oai22  g139(.a(new_n172_), .b(new_n44_), .c(new_n170_), .d(x03), .O(new_n173_));
  nand2  g140(.a(new_n64_), .b(x14), .O(new_n174_));
  aoi21  g141(.a(new_n174_), .b(x06), .c(new_n35_), .O(new_n175_));
  oai21  g142(.a(new_n175_), .b(new_n75_), .c(new_n148_), .O(new_n176_));
  nand3  g143(.a(new_n176_), .b(new_n173_), .c(new_n39_), .O(z6));
  inv1   g144(.a(new_n123_), .O(new_n178_));
  aoi21  g145(.a(new_n113_), .b(new_n55_), .c(new_n38_), .O(new_n179_));
  oai21  g146(.a(new_n179_), .b(x02), .c(new_n178_), .O(z7));
endmodule


