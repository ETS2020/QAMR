// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n181_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x01), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x20), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nor2   g011(.a(x24), .b(x10), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n39_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n44_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n43_), .c(new_n38_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n52_));
  aoi21  g019(.a(new_n52_), .b(new_n34_), .c(x03), .O(z0));
  nand3  g020(.a(x04), .b(x03), .c(x01), .O(new_n54_));
  nand2  g021(.a(x08), .b(x06), .O(new_n55_));
  nand2  g022(.a(x11), .b(x09), .O(new_n56_));
  nor3   g023(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand4  g024(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  inv1   g026(.a(x17), .O(new_n60_));
  nor2   g027(.a(new_n38_), .b(new_n60_), .O(new_n61_));
  inv1   g028(.a(x22), .O(new_n62_));
  inv1   g029(.a(x23), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n61_), .c(new_n59_), .d(new_n57_), .O(new_n65_));
  inv1   g032(.a(x09), .O(new_n66_));
  inv1   g033(.a(x03), .O(new_n67_));
  inv1   g034(.a(x04), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n34_), .d(new_n35_), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nor2   g037(.a(x08), .b(x06), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n37_), .d(new_n66_), .O(new_n72_));
  nor2   g039(.a(x14), .b(x12), .O(new_n73_));
  nor2   g040(.a(x17), .b(x16), .O(new_n74_));
  inv1   g041(.a(x19), .O(new_n75_));
  nor2   g042(.a(x20), .b(new_n75_), .O(new_n76_));
  nor2   g043(.a(x23), .b(x22), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n76_), .c(new_n74_), .d(new_n73_), .O(new_n78_));
  oai21  g045(.a(new_n78_), .b(new_n72_), .c(new_n65_), .O(new_n79_));
  nand2  g046(.a(new_n47_), .b(new_n39_), .O(new_n80_));
  nand3  g047(.a(new_n41_), .b(x13), .c(x05), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand4  g050(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  inv1   g052(.a(x18), .O(new_n86_));
  nor2   g053(.a(new_n38_), .b(new_n86_), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n85_), .c(new_n64_), .d(new_n57_), .O(new_n88_));
  nor2   g055(.a(x09), .b(x08), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n70_), .c(x07), .d(new_n36_), .O(new_n90_));
  nor2   g057(.a(x12), .b(x11), .O(new_n91_));
  nor2   g058(.a(x16), .b(x14), .O(new_n92_));
  nor2   g059(.a(x20), .b(x17), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n77_), .O(new_n94_));
  oai21  g061(.a(new_n94_), .b(new_n90_), .c(new_n88_), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n80_), .c(x24), .O(new_n96_));
  inv1   g063(.a(x10), .O(new_n97_));
  and2   g064(.a(x21), .b(x20), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n85_), .c(new_n64_), .d(new_n57_), .O(new_n99_));
  nor2   g066(.a(x03), .b(x01), .O(new_n100_));
  nor2   g067(.a(x06), .b(x04), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n100_), .c(new_n89_), .d(x00), .O(new_n102_));
  oai21  g069(.a(new_n102_), .b(new_n94_), .c(new_n99_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n41_), .c(new_n97_), .d(new_n34_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n96_), .c(new_n83_), .O(z1));
  inv1   g072(.a(x14), .O(new_n106_));
  nand2  g073(.a(x24), .b(x18), .O(new_n107_));
  nand3  g074(.a(new_n41_), .b(x15), .c(x13), .O(new_n108_));
  aoi21  g075(.a(new_n108_), .b(new_n107_), .c(new_n39_), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand3  g077(.a(x15), .b(new_n47_), .c(new_n39_), .O(new_n111_));
  nand4  g078(.a(new_n41_), .b(x21), .c(new_n97_), .d(new_n34_), .O(new_n112_));
  nand3  g079(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n109_), .c(x20), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n106_), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(x11), .c(x06), .d(x03), .O(new_n116_));
  nand2  g083(.a(new_n67_), .b(x02), .O(new_n117_));
  oai21  g084(.a(new_n116_), .b(new_n35_), .c(new_n117_), .O(z2));
  nand4  g085(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n119_));
  nor2   g086(.a(new_n106_), .b(new_n37_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(x20), .c(x15), .O(new_n121_));
  nand2  g088(.a(new_n100_), .b(new_n71_), .O(new_n122_));
  nand3  g089(.a(new_n76_), .b(new_n106_), .c(new_n37_), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n119_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n82_), .O(new_n125_));
  nand2  g092(.a(new_n120_), .b(new_n87_), .O(new_n126_));
  nand3  g093(.a(new_n100_), .b(x07), .c(new_n36_), .O(new_n127_));
  inv1   g094(.a(x08), .O(new_n128_));
  nand4  g095(.a(new_n38_), .b(new_n106_), .c(new_n37_), .d(new_n128_), .O(new_n129_));
  oai22  g096(.a(new_n129_), .b(new_n127_), .c(new_n126_), .d(new_n119_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n80_), .c(x24), .O(new_n131_));
  nand3  g098(.a(new_n71_), .b(new_n35_), .c(x00), .O(new_n132_));
  nor2   g099(.a(x11), .b(x10), .O(new_n133_));
  nand4  g100(.a(new_n133_), .b(new_n41_), .c(new_n38_), .d(new_n106_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n132_), .c(new_n34_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n67_), .O(new_n136_));
  nor4   g103(.a(new_n55_), .b(new_n67_), .c(x02), .d(new_n35_), .O(new_n137_));
  nand2  g104(.a(new_n41_), .b(x21), .O(new_n138_));
  nand2  g105(.a(new_n120_), .b(new_n97_), .O(new_n139_));
  nor3   g106(.a(new_n139_), .b(new_n138_), .c(new_n38_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand4  g108(.a(new_n141_), .b(new_n136_), .c(new_n131_), .d(new_n125_), .O(z3));
  oai21  g109(.a(x23), .b(new_n68_), .c(new_n60_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n62_), .c(x09), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x16), .c(new_n128_), .O(new_n145_));
  nand2  g112(.a(new_n48_), .b(new_n44_), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n43_), .c(new_n117_), .O(new_n147_));
  nand3  g114(.a(new_n45_), .b(new_n34_), .c(x00), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n147_), .O(z5));
  nand2  g116(.a(z5), .b(new_n145_), .O(new_n150_));
  inv1   g117(.a(x16), .O(new_n151_));
  oai21  g118(.a(new_n63_), .b(x04), .c(x17), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(x22), .c(new_n66_), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n151_), .c(x08), .O(new_n154_));
  nand2  g121(.a(new_n111_), .b(new_n110_), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(new_n109_), .c(new_n117_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n112_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n150_), .O(z4));
  oai21  g126(.a(new_n38_), .b(x14), .c(new_n36_), .O(new_n160_));
  nand3  g127(.a(new_n45_), .b(new_n67_), .c(x00), .O(new_n161_));
  nand3  g128(.a(new_n161_), .b(new_n48_), .c(new_n44_), .O(new_n162_));
  or2    g129(.a(new_n162_), .b(new_n43_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(new_n160_), .c(new_n37_), .O(new_n164_));
  oai21  g131(.a(x20), .b(new_n106_), .c(x06), .O(new_n165_));
  aoi21  g132(.a(new_n165_), .b(x11), .c(new_n67_), .O(new_n166_));
  nand2  g133(.a(x07), .b(x03), .O(new_n167_));
  oai21  g134(.a(new_n166_), .b(new_n86_), .c(new_n167_), .O(new_n168_));
  nand3  g135(.a(new_n168_), .b(new_n80_), .c(x24), .O(new_n169_));
  inv1   g136(.a(x15), .O(new_n170_));
  oai22  g137(.a(new_n166_), .b(new_n170_), .c(new_n75_), .d(new_n67_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n82_), .O(new_n172_));
  oai21  g139(.a(new_n138_), .b(x10), .c(new_n34_), .O(new_n173_));
  nand2  g140(.a(new_n173_), .b(new_n67_), .O(new_n174_));
  nand3  g141(.a(new_n165_), .b(x21), .c(x11), .O(new_n175_));
  nand2  g142(.a(x03), .b(x00), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  nand4  g144(.a(new_n177_), .b(new_n41_), .c(new_n97_), .d(new_n34_), .O(new_n178_));
  and2   g145(.a(new_n178_), .b(new_n174_), .O(new_n179_));
  nand4  g146(.a(new_n179_), .b(new_n172_), .c(new_n169_), .d(new_n164_), .O(z6));
  nor2   g147(.a(new_n113_), .b(new_n109_), .O(new_n181_));
  nand2  g148(.a(new_n174_), .b(new_n181_), .O(z7));
endmodule


