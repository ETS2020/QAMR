// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n181_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x10), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x02), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x13), .O(new_n45_));
  nand3  g012(.a(x19), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g014(.a(new_n47_), .b(new_n43_), .c(new_n38_), .O(new_n48_));
  inv1   g015(.a(x02), .O(new_n49_));
  nand4  g016(.a(new_n41_), .b(new_n37_), .c(new_n49_), .d(x00), .O(new_n50_));
  aoi21  g017(.a(new_n50_), .b(new_n48_), .c(x20), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nor3   g019(.a(new_n52_), .b(x03), .c(x01), .O(z0));
  nor2   g020(.a(x13), .b(x05), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  nand3  g022(.a(new_n41_), .b(x13), .c(x05), .O(new_n56_));
  nand2  g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g024(.a(x01), .O(new_n58_));
  inv1   g025(.a(x03), .O(new_n59_));
  inv1   g026(.a(x04), .O(new_n60_));
  inv1   g027(.a(x08), .O(new_n61_));
  inv1   g028(.a(x09), .O(new_n62_));
  inv1   g029(.a(x16), .O(new_n63_));
  inv1   g030(.a(x17), .O(new_n64_));
  inv1   g031(.a(x19), .O(new_n65_));
  inv1   g032(.a(x20), .O(new_n66_));
  inv1   g033(.a(x22), .O(new_n67_));
  inv1   g034(.a(x23), .O(new_n68_));
  nand4  g035(.a(new_n38_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n64_), .c(new_n63_), .d(new_n36_), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(x12), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n35_), .c(new_n62_), .d(new_n61_), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(x06), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n75_));
  nor2   g042(.a(new_n61_), .b(new_n34_), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(x11), .c(x09), .O(new_n77_));
  nor4   g044(.a(new_n77_), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n78_));
  nand4  g045(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n79_));
  nor2   g046(.a(new_n68_), .b(new_n67_), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(x20), .c(x17), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n75_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n57_), .O(new_n85_));
  inv1   g052(.a(x12), .O(new_n86_));
  nor2   g053(.a(new_n69_), .b(x17), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n63_), .c(new_n36_), .d(new_n86_), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(x11), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n62_), .c(new_n61_), .d(x07), .O(new_n90_));
  nor2   g057(.a(new_n90_), .b(x06), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n92_));
  nand4  g059(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  and2   g061(.a(x20), .b(x18), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n94_), .c(new_n80_), .d(new_n78_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n55_), .c(x24), .O(new_n98_));
  inv1   g065(.a(x21), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n66_), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n94_), .c(new_n80_), .d(new_n78_), .O(new_n101_));
  nand4  g068(.a(new_n59_), .b(new_n49_), .c(new_n58_), .d(x00), .O(new_n102_));
  nand4  g069(.a(new_n62_), .b(new_n61_), .c(new_n34_), .d(new_n60_), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g071(.a(new_n63_), .b(new_n36_), .c(new_n86_), .d(new_n35_), .O(new_n105_));
  nand4  g072(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n64_), .O(new_n106_));
  nor2   g073(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  aoi21  g075(.a(new_n108_), .b(new_n101_), .c(x24), .O(new_n109_));
  oai21  g076(.a(new_n109_), .b(x02), .c(new_n37_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n98_), .c(new_n85_), .O(z1));
  nand2  g078(.a(x24), .b(x18), .O(new_n112_));
  nand3  g079(.a(new_n41_), .b(x15), .c(x13), .O(new_n113_));
  aoi21  g080(.a(new_n113_), .b(new_n112_), .c(new_n39_), .O(new_n114_));
  inv1   g081(.a(new_n114_), .O(new_n115_));
  nand3  g082(.a(x24), .b(x18), .c(x13), .O(new_n116_));
  nand3  g083(.a(x15), .b(new_n45_), .c(new_n39_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  inv1   g085(.a(new_n118_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nor2   g087(.a(x10), .b(x02), .O(new_n121_));
  nor2   g088(.a(x24), .b(new_n99_), .O(new_n122_));
  aoi22  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .d(new_n38_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n66_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x14), .c(x11), .d(x06), .O(new_n125_));
  nor3   g092(.a(new_n125_), .b(new_n59_), .c(new_n58_), .O(z2));
  nor2   g093(.a(new_n59_), .b(new_n58_), .O(new_n127_));
  nor2   g094(.a(new_n36_), .b(new_n35_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n127_), .c(new_n95_), .d(new_n76_), .O(new_n129_));
  nor2   g096(.a(x03), .b(x01), .O(new_n130_));
  nand4  g097(.a(new_n66_), .b(new_n36_), .c(new_n35_), .d(new_n61_), .O(new_n131_));
  inv1   g098(.a(new_n131_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n130_), .c(x07), .d(new_n34_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n129_), .c(new_n54_), .O(new_n134_));
  nand3  g101(.a(new_n127_), .b(new_n76_), .c(new_n39_), .O(new_n135_));
  nand3  g102(.a(x20), .b(x15), .c(x14), .O(new_n136_));
  nor4   g103(.a(new_n136_), .b(new_n135_), .c(x13), .d(new_n35_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n134_), .c(x24), .O(new_n138_));
  inv1   g105(.a(x00), .O(new_n139_));
  nand3  g106(.a(x19), .b(x13), .c(x05), .O(new_n140_));
  oai21  g107(.a(x10), .b(new_n139_), .c(new_n140_), .O(new_n141_));
  nand4  g108(.a(new_n141_), .b(new_n66_), .c(new_n36_), .d(new_n35_), .O(new_n142_));
  nor2   g109(.a(new_n142_), .b(x08), .O(new_n143_));
  nand4  g110(.a(new_n143_), .b(new_n34_), .c(new_n59_), .d(new_n58_), .O(new_n144_));
  nand2  g111(.a(x13), .b(x05), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n55_), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n38_), .c(x15), .O(new_n147_));
  nand3  g114(.a(x21), .b(new_n37_), .c(new_n49_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand4  g116(.a(new_n149_), .b(x20), .c(x14), .d(x11), .O(new_n150_));
  nor2   g117(.a(new_n150_), .b(new_n61_), .O(new_n151_));
  nand4  g118(.a(new_n151_), .b(x06), .c(x03), .d(x01), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n144_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n41_), .O(new_n154_));
  nor3   g121(.a(x08), .b(x06), .c(x05), .O(new_n155_));
  nand3  g122(.a(new_n66_), .b(x19), .c(new_n36_), .O(new_n156_));
  nor3   g123(.a(new_n156_), .b(x13), .c(x11), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(new_n155_), .c(new_n130_), .O(new_n158_));
  nand4  g125(.a(new_n158_), .b(new_n154_), .c(new_n138_), .d(new_n38_), .O(z3));
  nand2  g126(.a(new_n50_), .b(new_n48_), .O(z5));
  oai21  g127(.a(x23), .b(new_n60_), .c(new_n64_), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n67_), .c(x09), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x16), .c(new_n61_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(z5), .O(new_n164_));
  oai21  g131(.a(new_n68_), .b(x04), .c(x17), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(x22), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(x09), .O(new_n167_));
  aoi21  g134(.a(new_n167_), .b(x16), .c(new_n61_), .O(new_n168_));
  oai21  g135(.a(new_n168_), .b(new_n123_), .c(new_n164_), .O(z4));
  nand2  g136(.a(x20), .b(new_n36_), .O(new_n170_));
  aoi21  g137(.a(new_n170_), .b(new_n34_), .c(x11), .O(new_n171_));
  nand3  g138(.a(new_n41_), .b(new_n37_), .c(x00), .O(new_n172_));
  nand3  g139(.a(new_n172_), .b(new_n46_), .c(new_n44_), .O(new_n173_));
  oai22  g140(.a(new_n173_), .b(new_n43_), .c(new_n171_), .d(x03), .O(new_n174_));
  nand2  g141(.a(new_n66_), .b(x14), .O(new_n175_));
  aoi21  g142(.a(new_n175_), .b(x06), .c(new_n35_), .O(new_n176_));
  nand2  g143(.a(new_n122_), .b(new_n37_), .O(new_n177_));
  nand3  g144(.a(new_n177_), .b(new_n117_), .c(new_n116_), .O(new_n178_));
  oai22  g145(.a(new_n178_), .b(new_n114_), .c(new_n176_), .d(new_n59_), .O(new_n179_));
  nand3  g146(.a(new_n179_), .b(new_n174_), .c(new_n38_), .O(z6));
  oai21  g147(.a(new_n122_), .b(x02), .c(new_n37_), .O(new_n181_));
  nand3  g148(.a(new_n181_), .b(new_n119_), .c(new_n115_), .O(z7));
endmodule


