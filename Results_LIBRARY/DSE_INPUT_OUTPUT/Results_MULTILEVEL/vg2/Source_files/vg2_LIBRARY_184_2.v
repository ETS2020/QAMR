// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x04), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x02), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x13), .O(new_n45_));
  nand3  g012(.a(x19), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g014(.a(new_n47_), .b(new_n43_), .c(new_n38_), .O(new_n48_));
  inv1   g015(.a(x02), .O(new_n49_));
  inv1   g016(.a(x10), .O(new_n50_));
  nand4  g017(.a(new_n41_), .b(new_n50_), .c(new_n49_), .d(x00), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n48_), .c(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(z0));
  inv1   g021(.a(x01), .O(new_n55_));
  inv1   g022(.a(x03), .O(new_n56_));
  nand4  g023(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n57_));
  nor4   g024(.a(new_n57_), .b(new_n37_), .c(new_n56_), .d(new_n55_), .O(new_n58_));
  nand4  g025(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n59_));
  and2   g026(.a(x23), .b(x22), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(x20), .c(x17), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand2  g029(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  inv1   g030(.a(x08), .O(new_n64_));
  nand4  g031(.a(new_n37_), .b(new_n56_), .c(new_n49_), .d(new_n55_), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  nor2   g033(.a(x11), .b(x09), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n64_), .d(new_n34_), .O(new_n68_));
  nor2   g035(.a(x14), .b(x12), .O(new_n69_));
  nor2   g036(.a(x17), .b(x16), .O(new_n70_));
  inv1   g037(.a(x19), .O(new_n71_));
  nor2   g038(.a(x20), .b(new_n71_), .O(new_n72_));
  nor2   g039(.a(x23), .b(x22), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n70_), .d(new_n69_), .O(new_n74_));
  oai21  g041(.a(new_n74_), .b(new_n68_), .c(new_n63_), .O(new_n75_));
  nor2   g042(.a(x13), .b(x05), .O(new_n76_));
  inv1   g043(.a(new_n76_), .O(new_n77_));
  nand3  g044(.a(new_n41_), .b(x13), .c(x05), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand4  g047(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  inv1   g049(.a(x18), .O(new_n83_));
  inv1   g050(.a(x20), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n82_), .c(new_n60_), .d(new_n58_), .O(new_n86_));
  nor2   g053(.a(x09), .b(x08), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n66_), .c(x07), .d(new_n34_), .O(new_n88_));
  nor2   g055(.a(x12), .b(x11), .O(new_n89_));
  nor2   g056(.a(x16), .b(x14), .O(new_n90_));
  nor2   g057(.a(x20), .b(x17), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n73_), .O(new_n92_));
  oai21  g059(.a(new_n92_), .b(new_n88_), .c(new_n86_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n77_), .c(x24), .O(new_n94_));
  nand3  g061(.a(new_n60_), .b(x21), .c(x20), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n81_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n58_), .O(new_n97_));
  nor2   g064(.a(x03), .b(x01), .O(new_n98_));
  nor2   g065(.a(x06), .b(x04), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(new_n87_), .d(x00), .O(new_n100_));
  oai21  g067(.a(new_n100_), .b(new_n92_), .c(new_n97_), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n41_), .c(new_n50_), .d(new_n49_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n94_), .c(new_n80_), .O(z1));
  nand2  g070(.a(x24), .b(x18), .O(new_n104_));
  nand3  g071(.a(new_n41_), .b(x15), .c(x13), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(x05), .O(new_n107_));
  nand3  g074(.a(x24), .b(x18), .c(x13), .O(new_n108_));
  nand3  g075(.a(x15), .b(new_n45_), .c(new_n39_), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n38_), .O(new_n111_));
  nand4  g078(.a(new_n41_), .b(x21), .c(new_n50_), .d(new_n49_), .O(new_n112_));
  aoi21  g079(.a(new_n112_), .b(new_n111_), .c(new_n84_), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(x14), .c(x11), .d(x06), .O(new_n114_));
  nor3   g081(.a(new_n114_), .b(new_n56_), .c(new_n55_), .O(z2));
  nand3  g082(.a(x15), .b(x13), .c(x05), .O(new_n116_));
  nand3  g083(.a(x21), .b(new_n50_), .c(new_n49_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x20), .c(x14), .d(x11), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n64_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x06), .c(x03), .d(x01), .O(new_n121_));
  nand3  g088(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n122_));
  nand3  g089(.a(x19), .b(x13), .c(x05), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n84_), .c(new_n36_), .d(new_n35_), .O(new_n125_));
  nor2   g092(.a(new_n125_), .b(x08), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(new_n34_), .c(new_n56_), .d(new_n55_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n121_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n41_), .O(new_n129_));
  nand4  g096(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n130_));
  nand3  g097(.a(new_n85_), .b(x14), .c(x11), .O(new_n131_));
  nand3  g098(.a(new_n98_), .b(x07), .c(new_n34_), .O(new_n132_));
  nand4  g099(.a(new_n84_), .b(new_n36_), .c(new_n35_), .d(new_n64_), .O(new_n133_));
  oai22  g100(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n130_), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n77_), .c(x24), .O(new_n135_));
  nand4  g102(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n136_));
  nand3  g103(.a(new_n98_), .b(new_n64_), .c(new_n34_), .O(new_n137_));
  nand3  g104(.a(new_n72_), .b(new_n36_), .c(new_n35_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n130_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n45_), .c(new_n39_), .O(new_n140_));
  nand4  g107(.a(new_n140_), .b(new_n135_), .c(new_n129_), .d(new_n38_), .O(z3));
  inv1   g108(.a(new_n116_), .O(new_n142_));
  aoi21  g109(.a(x21), .b(new_n50_), .c(new_n142_), .O(new_n143_));
  inv1   g110(.a(x22), .O(new_n144_));
  inv1   g111(.a(x17), .O(new_n145_));
  aoi21  g112(.a(x23), .b(new_n37_), .c(new_n145_), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n144_), .c(x09), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(x16), .c(new_n64_), .O(new_n148_));
  inv1   g115(.a(x00), .O(new_n149_));
  oai21  g116(.a(x10), .b(new_n149_), .c(new_n123_), .O(new_n150_));
  aoi21  g117(.a(new_n144_), .b(x17), .c(x09), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(x16), .c(new_n64_), .O(new_n152_));
  nand3  g119(.a(new_n50_), .b(x04), .c(x00), .O(new_n153_));
  inv1   g120(.a(x16), .O(new_n154_));
  nand2  g121(.a(new_n73_), .b(new_n154_), .O(new_n155_));
  nor2   g122(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  aoi21  g123(.a(new_n152_), .b(new_n150_), .c(new_n156_), .O(new_n157_));
  oai21  g124(.a(new_n148_), .b(new_n143_), .c(new_n157_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n41_), .O(new_n159_));
  nand2  g126(.a(new_n152_), .b(x07), .O(new_n160_));
  oai21  g127(.a(new_n148_), .b(new_n83_), .c(new_n160_), .O(new_n161_));
  nand3  g128(.a(new_n161_), .b(new_n77_), .c(x24), .O(new_n162_));
  inv1   g129(.a(x15), .O(new_n163_));
  nand2  g130(.a(new_n152_), .b(x19), .O(new_n164_));
  oai21  g131(.a(new_n148_), .b(new_n163_), .c(new_n164_), .O(new_n165_));
  nand3  g132(.a(new_n165_), .b(new_n45_), .c(new_n39_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n162_), .c(new_n159_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n49_), .O(new_n168_));
  nor2   g135(.a(new_n47_), .b(new_n43_), .O(new_n169_));
  aoi21  g136(.a(x23), .b(new_n145_), .c(x22), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(x09), .c(new_n154_), .O(new_n171_));
  aoi21  g138(.a(new_n171_), .b(new_n64_), .c(new_n169_), .O(new_n172_));
  inv1   g139(.a(x09), .O(new_n173_));
  aoi21  g140(.a(x22), .b(new_n145_), .c(new_n173_), .O(new_n174_));
  oai21  g141(.a(new_n174_), .b(new_n154_), .c(x08), .O(new_n175_));
  and2   g142(.a(new_n175_), .b(new_n110_), .O(new_n176_));
  oai21  g143(.a(new_n176_), .b(new_n172_), .c(x04), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n168_), .O(z4));
  nand2  g145(.a(new_n51_), .b(new_n48_), .O(z5));
  aoi21  g146(.a(x20), .b(new_n36_), .c(x06), .O(new_n180_));
  oai21  g147(.a(new_n180_), .b(x11), .c(new_n56_), .O(new_n181_));
  nand2  g148(.a(new_n181_), .b(z5), .O(new_n182_));
  nand2  g149(.a(new_n112_), .b(new_n111_), .O(new_n183_));
  aoi21  g150(.a(new_n84_), .b(x14), .c(new_n34_), .O(new_n184_));
  oai21  g151(.a(new_n184_), .b(new_n35_), .c(x03), .O(new_n185_));
  nand2  g152(.a(new_n185_), .b(new_n183_), .O(new_n186_));
  nand2  g153(.a(new_n186_), .b(new_n182_), .O(z6));
  and2   g154(.a(new_n108_), .b(new_n38_), .O(new_n188_));
  nand4  g155(.a(new_n188_), .b(new_n112_), .c(new_n109_), .d(new_n107_), .O(z7));
endmodule


