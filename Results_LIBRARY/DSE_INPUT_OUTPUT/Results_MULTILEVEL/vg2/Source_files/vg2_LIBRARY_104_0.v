// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n202_, new_n203_, new_n204_, new_n205_,
    new_n206_, new_n207_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x15), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nor4   g017(.a(new_n50_), .b(x20), .c(new_n37_), .d(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(x01), .O(z0));
  inv1   g020(.a(x12), .O(new_n54_));
  nand3  g021(.a(x15), .b(x13), .c(x05), .O(new_n55_));
  nand3  g022(.a(x21), .b(new_n45_), .c(new_n44_), .O(new_n56_));
  nand2  g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(x23), .c(x22), .d(x20), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(x17), .c(x16), .d(x14), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(x11), .c(x09), .d(x08), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n35_), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(x04), .c(x03), .d(x01), .O(new_n64_));
  inv1   g031(.a(x01), .O(new_n65_));
  inv1   g032(.a(x04), .O(new_n66_));
  inv1   g033(.a(x08), .O(new_n67_));
  inv1   g034(.a(x09), .O(new_n68_));
  inv1   g035(.a(x14), .O(new_n69_));
  inv1   g036(.a(x16), .O(new_n70_));
  inv1   g037(.a(x20), .O(new_n71_));
  inv1   g038(.a(x22), .O(new_n72_));
  inv1   g039(.a(x23), .O(new_n73_));
  nand3  g040(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n74_));
  nand3  g041(.a(x19), .b(x13), .c(x05), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(x17), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n70_), .c(x15), .d(new_n69_), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(x12), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n36_), .c(new_n68_), .d(new_n67_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(x06), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n66_), .c(new_n34_), .d(new_n65_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n64_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n40_), .O(new_n85_));
  nor2   g052(.a(x13), .b(x05), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  nand4  g054(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n88_));
  nor4   g055(.a(new_n88_), .b(new_n66_), .c(new_n34_), .d(new_n65_), .O(new_n89_));
  nand4  g056(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n90_));
  inv1   g057(.a(new_n90_), .O(new_n91_));
  inv1   g058(.a(x18), .O(new_n92_));
  nor2   g059(.a(new_n71_), .b(new_n92_), .O(new_n93_));
  nor2   g060(.a(new_n73_), .b(new_n72_), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n93_), .c(new_n91_), .d(new_n89_), .O(new_n95_));
  nor2   g062(.a(x03), .b(x01), .O(new_n96_));
  nor2   g063(.a(x06), .b(x04), .O(new_n97_));
  inv1   g064(.a(x07), .O(new_n98_));
  nor2   g065(.a(x08), .b(new_n98_), .O(new_n99_));
  nor2   g066(.a(x11), .b(x09), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n99_), .c(new_n97_), .d(new_n96_), .O(new_n101_));
  nor2   g068(.a(x14), .b(x12), .O(new_n102_));
  nor2   g069(.a(x16), .b(new_n37_), .O(new_n103_));
  nor2   g070(.a(x20), .b(x17), .O(new_n104_));
  nor2   g071(.a(x23), .b(x22), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n104_), .c(new_n103_), .d(new_n102_), .O(new_n106_));
  oai21  g073(.a(new_n106_), .b(new_n101_), .c(new_n95_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n87_), .c(x24), .O(new_n108_));
  nand3  g075(.a(new_n94_), .b(x20), .c(x17), .O(new_n109_));
  nor4   g076(.a(new_n109_), .b(new_n70_), .c(new_n69_), .d(new_n54_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n89_), .O(new_n111_));
  nor3   g078(.a(x04), .b(x03), .c(x01), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(new_n100_), .c(new_n67_), .d(new_n35_), .O(new_n113_));
  nor2   g080(.a(x17), .b(x16), .O(new_n114_));
  inv1   g081(.a(x19), .O(new_n115_));
  nor2   g082(.a(x20), .b(new_n115_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(new_n114_), .c(new_n105_), .d(new_n102_), .O(new_n117_));
  oai21  g084(.a(new_n117_), .b(new_n113_), .c(new_n111_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x15), .c(new_n47_), .d(new_n38_), .O(new_n119_));
  and2   g086(.a(new_n119_), .b(new_n108_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n85_), .O(z1));
  nor2   g088(.a(new_n40_), .b(new_n92_), .O(new_n122_));
  inv1   g089(.a(new_n122_), .O(new_n123_));
  nand3  g090(.a(new_n40_), .b(x15), .c(x13), .O(new_n124_));
  aoi21  g091(.a(new_n124_), .b(new_n123_), .c(new_n38_), .O(new_n125_));
  nand2  g092(.a(new_n122_), .b(x13), .O(new_n126_));
  nand3  g093(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n127_));
  nand4  g094(.a(new_n40_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(new_n129_));
  oai21  g096(.a(new_n129_), .b(new_n125_), .c(x20), .O(new_n130_));
  nor2   g097(.a(new_n130_), .b(new_n69_), .O(new_n131_));
  nand4  g098(.a(new_n131_), .b(x11), .c(x06), .d(x03), .O(new_n132_));
  nor2   g099(.a(new_n132_), .b(new_n65_), .O(z2));
  nand4  g100(.a(new_n57_), .b(x20), .c(x14), .d(x11), .O(new_n134_));
  nor2   g101(.a(new_n134_), .b(new_n67_), .O(new_n135_));
  nand4  g102(.a(new_n135_), .b(x06), .c(x03), .d(x01), .O(new_n136_));
  nand4  g103(.a(new_n76_), .b(new_n71_), .c(x15), .d(new_n69_), .O(new_n137_));
  nor2   g104(.a(new_n137_), .b(x11), .O(new_n138_));
  nand4  g105(.a(new_n138_), .b(new_n67_), .c(new_n35_), .d(new_n34_), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(x01), .c(new_n136_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n40_), .O(new_n141_));
  nand4  g108(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n142_));
  nand3  g109(.a(new_n93_), .b(x14), .c(x11), .O(new_n143_));
  nand3  g110(.a(new_n96_), .b(x07), .c(new_n35_), .O(new_n144_));
  nor2   g111(.a(x11), .b(x08), .O(new_n145_));
  nand4  g112(.a(new_n145_), .b(new_n71_), .c(x15), .d(new_n69_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n144_), .c(new_n143_), .d(new_n142_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n87_), .c(x24), .O(new_n148_));
  nand3  g115(.a(x06), .b(x03), .c(x01), .O(new_n149_));
  nand4  g116(.a(x20), .b(x14), .c(x11), .d(x08), .O(new_n150_));
  nand3  g117(.a(new_n96_), .b(new_n67_), .c(new_n35_), .O(new_n151_));
  nand3  g118(.a(new_n116_), .b(new_n69_), .c(new_n36_), .O(new_n152_));
  oai22  g119(.a(new_n152_), .b(new_n151_), .c(new_n150_), .d(new_n149_), .O(new_n153_));
  nand4  g120(.a(new_n153_), .b(x15), .c(new_n47_), .d(new_n38_), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n148_), .c(new_n141_), .O(z3));
  nand4  g122(.a(x24), .b(new_n47_), .c(new_n67_), .d(new_n38_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(x15), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n71_), .O(new_n158_));
  inv1   g125(.a(new_n50_), .O(new_n159_));
  inv1   g126(.a(x17), .O(new_n160_));
  oai21  g127(.a(x23), .b(new_n66_), .c(new_n160_), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n72_), .c(x09), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x16), .c(new_n67_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nor2   g131(.a(x24), .b(new_n47_), .O(new_n165_));
  aoi21  g132(.a(new_n165_), .b(x05), .c(new_n86_), .O(new_n166_));
  aoi21  g133(.a(x23), .b(new_n66_), .c(new_n160_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n72_), .c(x09), .O(new_n168_));
  aoi21  g135(.a(new_n168_), .b(x16), .c(new_n67_), .O(new_n169_));
  nand2  g136(.a(x13), .b(x05), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n87_), .O(new_n171_));
  nand3  g138(.a(new_n171_), .b(new_n40_), .c(x19), .O(new_n172_));
  oai21  g139(.a(new_n169_), .b(new_n166_), .c(new_n172_), .O(new_n173_));
  nand2  g140(.a(new_n173_), .b(x15), .O(new_n174_));
  inv1   g141(.a(new_n169_), .O(new_n175_));
  nand2  g142(.a(new_n87_), .b(x24), .O(new_n176_));
  oai21  g143(.a(new_n176_), .b(new_n92_), .c(new_n128_), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n175_), .O(new_n178_));
  nand4  g145(.a(new_n178_), .b(new_n174_), .c(new_n164_), .d(new_n158_), .O(z4));
  inv1   g146(.a(new_n42_), .O(new_n180_));
  and2   g147(.a(new_n46_), .b(new_n43_), .O(new_n181_));
  nand2  g148(.a(new_n71_), .b(new_n37_), .O(new_n182_));
  nand4  g149(.a(new_n182_), .b(new_n181_), .c(new_n48_), .d(new_n180_), .O(z5));
  oai21  g150(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n184_));
  oai21  g151(.a(new_n176_), .b(new_n98_), .c(new_n46_), .O(new_n185_));
  nand2  g152(.a(new_n185_), .b(new_n184_), .O(new_n186_));
  inv1   g153(.a(new_n166_), .O(new_n187_));
  nand2  g154(.a(new_n71_), .b(x14), .O(new_n188_));
  aoi21  g155(.a(new_n188_), .b(x06), .c(new_n36_), .O(new_n189_));
  nand2  g156(.a(new_n115_), .b(x03), .O(new_n190_));
  oai21  g157(.a(new_n190_), .b(new_n189_), .c(new_n187_), .O(new_n191_));
  inv1   g158(.a(new_n128_), .O(new_n192_));
  oai22  g159(.a(new_n189_), .b(new_n34_), .c(new_n192_), .d(new_n122_), .O(new_n193_));
  nand3  g160(.a(new_n193_), .b(new_n191_), .c(new_n186_), .O(new_n194_));
  nand2  g161(.a(new_n194_), .b(x15), .O(new_n195_));
  oai21  g162(.a(new_n36_), .b(x06), .c(x03), .O(new_n196_));
  and2   g163(.a(new_n196_), .b(new_n177_), .O(new_n197_));
  oai21  g164(.a(new_n69_), .b(x06), .c(new_n36_), .O(new_n198_));
  aoi21  g165(.a(new_n198_), .b(new_n34_), .c(new_n50_), .O(new_n199_));
  oai21  g166(.a(new_n199_), .b(new_n197_), .c(x20), .O(new_n200_));
  nand2  g167(.a(new_n200_), .b(new_n195_), .O(z6));
  nand4  g168(.a(new_n182_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n202_));
  nand2  g169(.a(new_n202_), .b(new_n55_), .O(new_n203_));
  nand2  g170(.a(new_n203_), .b(new_n40_), .O(new_n204_));
  oai21  g171(.a(new_n122_), .b(new_n86_), .c(x15), .O(new_n205_));
  nor2   g172(.a(new_n176_), .b(new_n71_), .O(new_n206_));
  nand2  g173(.a(new_n206_), .b(x18), .O(new_n207_));
  nand3  g174(.a(new_n207_), .b(new_n205_), .c(new_n204_), .O(z7));
endmodule


