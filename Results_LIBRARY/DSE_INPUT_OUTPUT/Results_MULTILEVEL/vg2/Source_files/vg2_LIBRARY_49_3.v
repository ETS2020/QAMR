// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x01), .O(new_n34_));
  inv1   g001(.a(x03), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x14), .O(new_n38_));
  inv1   g005(.a(x20), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand3  g007(.a(x24), .b(new_n40_), .c(x07), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(x05), .O(new_n45_));
  inv1   g012(.a(x05), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand2  g014(.a(x24), .b(x10), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(x19), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nand3  g016(.a(x24), .b(x13), .c(x07), .O(new_n50_));
  inv1   g017(.a(x02), .O(new_n51_));
  nand3  g018(.a(new_n42_), .b(new_n51_), .c(x00), .O(new_n52_));
  nand2  g019(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n40_), .O(new_n54_));
  nand3  g021(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(new_n55_));
  nand4  g022(.a(new_n55_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(z0));
  nand3  g026(.a(x04), .b(x03), .c(x01), .O(new_n60_));
  nand4  g027(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g029(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  and2   g031(.a(x21), .b(x20), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  inv1   g033(.a(x23), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(new_n69_));
  nor2   g036(.a(x03), .b(x01), .O(new_n70_));
  nor2   g037(.a(x06), .b(x04), .O(new_n71_));
  nor2   g038(.a(x09), .b(x08), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(x00), .O(new_n73_));
  nor2   g040(.a(x12), .b(x11), .O(new_n74_));
  nor2   g041(.a(x16), .b(x14), .O(new_n75_));
  nor2   g042(.a(x20), .b(x17), .O(new_n76_));
  nor2   g043(.a(x23), .b(x22), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  oai21  g045(.a(new_n78_), .b(new_n73_), .c(new_n69_), .O(new_n79_));
  nand3  g046(.a(new_n79_), .b(new_n42_), .c(new_n51_), .O(new_n80_));
  nand2  g047(.a(new_n47_), .b(new_n46_), .O(new_n81_));
  inv1   g048(.a(x18), .O(new_n82_));
  nor2   g049(.a(new_n39_), .b(new_n82_), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n68_), .c(new_n64_), .d(new_n62_), .O(new_n84_));
  nor3   g051(.a(x04), .b(x03), .c(x01), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n72_), .c(x07), .d(new_n36_), .O(new_n86_));
  oai21  g053(.a(new_n86_), .b(new_n78_), .c(new_n84_), .O(new_n87_));
  nand3  g054(.a(new_n87_), .b(new_n81_), .c(x24), .O(new_n88_));
  nand4  g055(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  inv1   g057(.a(x17), .O(new_n91_));
  nor2   g058(.a(new_n39_), .b(new_n91_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n90_), .c(new_n68_), .d(new_n62_), .O(new_n93_));
  inv1   g060(.a(x09), .O(new_n94_));
  nor2   g061(.a(x08), .b(x06), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n85_), .c(new_n37_), .d(new_n94_), .O(new_n96_));
  nor2   g063(.a(x14), .b(x12), .O(new_n97_));
  nor2   g064(.a(x17), .b(x16), .O(new_n98_));
  inv1   g065(.a(x19), .O(new_n99_));
  nor2   g066(.a(x20), .b(new_n99_), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n98_), .c(new_n97_), .d(new_n77_), .O(new_n101_));
  oai21  g068(.a(new_n101_), .b(new_n96_), .c(new_n93_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n47_), .c(new_n46_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n88_), .c(new_n80_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n40_), .O(new_n105_));
  nand2  g072(.a(x13), .b(x05), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n81_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n102_), .c(new_n42_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n105_), .O(z1));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(x10), .O(new_n111_));
  nand3  g078(.a(new_n42_), .b(x15), .c(x13), .O(new_n112_));
  inv1   g079(.a(new_n112_), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n111_), .c(x05), .O(new_n114_));
  nand4  g081(.a(new_n48_), .b(x15), .c(new_n47_), .d(new_n46_), .O(new_n115_));
  nand3  g082(.a(x24), .b(x18), .c(x13), .O(new_n116_));
  nand3  g083(.a(new_n42_), .b(x21), .c(new_n51_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n40_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n115_), .c(new_n114_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x20), .c(x14), .d(x11), .O(new_n121_));
  inv1   g088(.a(new_n121_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(x06), .c(x03), .d(x01), .O(new_n123_));
  inv1   g090(.a(new_n123_), .O(z2));
  inv1   g091(.a(x08), .O(new_n125_));
  nand3  g092(.a(x15), .b(x13), .c(x05), .O(new_n126_));
  nand3  g093(.a(x21), .b(new_n40_), .c(new_n51_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(x20), .c(x14), .d(x11), .O(new_n129_));
  nor2   g096(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(x06), .c(x03), .d(x01), .O(new_n131_));
  nand3  g098(.a(new_n40_), .b(new_n51_), .c(x00), .O(new_n132_));
  nand3  g099(.a(x19), .b(x13), .c(x05), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(x08), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n131_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n42_), .O(new_n139_));
  nand4  g106(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n140_));
  nand3  g107(.a(new_n83_), .b(x14), .c(x11), .O(new_n141_));
  nand3  g108(.a(new_n70_), .b(x07), .c(new_n36_), .O(new_n142_));
  nand4  g109(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n125_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n140_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n81_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n40_), .O(new_n146_));
  nand4  g113(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n147_));
  nand2  g114(.a(new_n95_), .b(new_n70_), .O(new_n148_));
  nand3  g115(.a(new_n100_), .b(new_n38_), .c(new_n37_), .O(new_n149_));
  oai22  g116(.a(new_n149_), .b(new_n148_), .c(new_n147_), .d(new_n140_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n47_), .c(new_n46_), .O(new_n151_));
  inv1   g118(.a(new_n151_), .O(new_n152_));
  aoi21  g119(.a(new_n146_), .b(x24), .c(new_n152_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n139_), .O(z3));
  inv1   g121(.a(x04), .O(new_n155_));
  oai21  g122(.a(x23), .b(new_n155_), .c(new_n91_), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n66_), .c(x09), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(x16), .c(new_n125_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n55_), .O(new_n159_));
  inv1   g126(.a(x16), .O(new_n160_));
  oai21  g127(.a(new_n67_), .b(x04), .c(x17), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(x22), .c(new_n94_), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(new_n160_), .c(x08), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n120_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n159_), .O(z4));
  inv1   g132(.a(x07), .O(new_n166_));
  oai21  g133(.a(new_n42_), .b(new_n166_), .c(new_n43_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(x05), .O(new_n168_));
  oai21  g135(.a(new_n47_), .b(new_n166_), .c(new_n40_), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(x24), .O(new_n170_));
  nand3  g137(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n171_));
  nand4  g138(.a(new_n42_), .b(new_n40_), .c(new_n51_), .d(x00), .O(new_n172_));
  nand4  g139(.a(new_n172_), .b(new_n171_), .c(new_n170_), .d(new_n168_), .O(z5));
  aoi21  g140(.a(x20), .b(new_n38_), .c(x06), .O(new_n174_));
  oai21  g141(.a(new_n174_), .b(x11), .c(new_n35_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n55_), .O(new_n176_));
  aoi21  g143(.a(new_n39_), .b(x14), .c(new_n36_), .O(new_n177_));
  oai21  g144(.a(new_n177_), .b(new_n37_), .c(x03), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n120_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n176_), .O(z6));
  nand2  g147(.a(new_n112_), .b(new_n110_), .O(new_n181_));
  nand2  g148(.a(new_n181_), .b(x05), .O(new_n182_));
  oai21  g149(.a(new_n82_), .b(new_n47_), .c(new_n40_), .O(new_n183_));
  nand2  g150(.a(new_n183_), .b(x24), .O(new_n184_));
  nand3  g151(.a(x15), .b(new_n47_), .c(new_n46_), .O(new_n185_));
  nand4  g152(.a(new_n42_), .b(x21), .c(new_n40_), .d(new_n51_), .O(new_n186_));
  nand4  g153(.a(new_n186_), .b(new_n185_), .c(new_n184_), .d(new_n182_), .O(z7));
endmodule


