// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n174_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x14), .O(new_n37_));
  inv1   g004(.a(x20), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x05), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x05), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n45_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n44_), .c(new_n43_), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(new_n38_), .c(new_n37_), .d(new_n36_), .O(new_n49_));
  inv1   g016(.a(new_n49_), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n35_), .c(new_n34_), .d(x02), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(x01), .O(z0));
  inv1   g019(.a(x02), .O(new_n53_));
  and2   g020(.a(x04), .b(x03), .O(new_n54_));
  inv1   g021(.a(x08), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n35_), .O(new_n56_));
  inv1   g023(.a(x09), .O(new_n57_));
  nor2   g024(.a(x10), .b(new_n57_), .O(new_n58_));
  inv1   g025(.a(x12), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n36_), .O(new_n60_));
  nand4  g027(.a(new_n60_), .b(new_n58_), .c(new_n56_), .d(new_n54_), .O(new_n61_));
  nand4  g028(.a(x20), .b(x17), .c(x16), .d(x14), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  inv1   g030(.a(x23), .O(new_n64_));
  nor2   g031(.a(x24), .b(new_n64_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n63_), .c(x22), .d(x21), .O(new_n66_));
  oai21  g033(.a(new_n66_), .b(new_n61_), .c(x01), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n53_), .O(new_n68_));
  nand2  g035(.a(new_n54_), .b(x01), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand3  g037(.a(new_n56_), .b(x11), .c(x09), .O(new_n71_));
  inv1   g038(.a(new_n71_), .O(new_n72_));
  nand4  g039(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n73_));
  nand4  g040(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g042(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  nor3   g043(.a(x04), .b(x03), .c(x01), .O(new_n77_));
  nor2   g044(.a(x11), .b(x09), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n55_), .d(new_n35_), .O(new_n79_));
  nor2   g046(.a(x14), .b(x12), .O(new_n80_));
  nor2   g047(.a(x17), .b(x16), .O(new_n81_));
  inv1   g048(.a(x19), .O(new_n82_));
  nor2   g049(.a(x20), .b(new_n82_), .O(new_n83_));
  nor2   g050(.a(x23), .b(x22), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n83_), .c(new_n81_), .d(new_n80_), .O(new_n85_));
  oai21  g052(.a(new_n85_), .b(new_n79_), .c(new_n76_), .O(new_n86_));
  nand2  g053(.a(new_n46_), .b(new_n45_), .O(new_n87_));
  nand3  g054(.a(new_n40_), .b(x13), .c(x05), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand4  g057(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n91_));
  nand4  g058(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n72_), .c(new_n70_), .O(new_n94_));
  nor2   g061(.a(x09), .b(x08), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n77_), .c(x07), .d(new_n35_), .O(new_n96_));
  nor2   g063(.a(x12), .b(x11), .O(new_n97_));
  nor2   g064(.a(x16), .b(x14), .O(new_n98_));
  nor2   g065(.a(x20), .b(x17), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n84_), .O(new_n100_));
  oai21  g067(.a(new_n100_), .b(new_n96_), .c(new_n94_), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n87_), .c(x24), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n90_), .c(new_n68_), .O(z1));
  inv1   g070(.a(x01), .O(new_n104_));
  inv1   g071(.a(x10), .O(new_n105_));
  nand4  g072(.a(x11), .b(new_n105_), .c(x06), .d(x03), .O(new_n106_));
  inv1   g073(.a(x21), .O(new_n107_));
  nor2   g074(.a(x24), .b(new_n107_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(x20), .c(x14), .O(new_n109_));
  oai21  g076(.a(new_n109_), .b(new_n106_), .c(x01), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n53_), .O(new_n111_));
  nand2  g078(.a(x24), .b(x18), .O(new_n112_));
  nand3  g079(.a(new_n40_), .b(x15), .c(x13), .O(new_n113_));
  aoi21  g080(.a(new_n113_), .b(new_n112_), .c(new_n45_), .O(new_n114_));
  nand3  g081(.a(x15), .b(new_n46_), .c(new_n45_), .O(new_n115_));
  oai21  g082(.a(new_n112_), .b(new_n46_), .c(new_n115_), .O(new_n116_));
  nor2   g083(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nor3   g084(.a(new_n117_), .b(new_n38_), .c(new_n37_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x11), .c(x06), .d(x03), .O(new_n119_));
  oai21  g086(.a(new_n119_), .b(new_n104_), .c(new_n111_), .O(z2));
  nand3  g087(.a(new_n56_), .b(x03), .c(x01), .O(new_n121_));
  nor2   g088(.a(new_n37_), .b(new_n36_), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(x20), .c(x15), .O(new_n123_));
  nand4  g090(.a(new_n35_), .b(new_n34_), .c(x02), .d(new_n104_), .O(new_n124_));
  nand4  g091(.a(new_n83_), .b(new_n37_), .c(new_n36_), .d(new_n55_), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n121_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n89_), .O(new_n127_));
  nand3  g094(.a(new_n122_), .b(x20), .c(x18), .O(new_n128_));
  nor2   g095(.a(x20), .b(x14), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(new_n36_), .c(new_n55_), .d(x07), .O(new_n130_));
  oai22  g097(.a(new_n130_), .b(new_n124_), .c(new_n128_), .d(new_n121_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n87_), .c(x24), .O(new_n132_));
  nor2   g099(.a(x02), .b(new_n104_), .O(new_n133_));
  nand2  g100(.a(new_n122_), .b(new_n105_), .O(new_n134_));
  nand2  g101(.a(new_n108_), .b(x20), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n133_), .c(new_n56_), .d(x03), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n132_), .c(new_n127_), .O(z3));
  inv1   g105(.a(x16), .O(new_n139_));
  aoi21  g106(.a(new_n64_), .b(x04), .c(x17), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(x22), .c(new_n57_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n139_), .c(new_n55_), .O(new_n142_));
  oai21  g109(.a(new_n55_), .b(new_n104_), .c(new_n142_), .O(new_n143_));
  nor2   g110(.a(x24), .b(x10), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n53_), .c(x00), .O(new_n145_));
  nand4  g112(.a(new_n145_), .b(new_n47_), .c(new_n44_), .d(new_n43_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n143_), .O(new_n147_));
  oai21  g114(.a(new_n64_), .b(x04), .c(x17), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(x22), .c(new_n57_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n139_), .c(x08), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(x18), .O(new_n151_));
  nand3  g118(.a(x08), .b(x07), .c(x02), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n87_), .c(x24), .O(new_n154_));
  nand2  g121(.a(new_n150_), .b(x15), .O(new_n155_));
  nand3  g122(.a(x19), .b(x08), .c(x02), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n89_), .O(new_n158_));
  nand4  g125(.a(new_n150_), .b(new_n40_), .c(x21), .d(new_n105_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(x01), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n53_), .O(new_n161_));
  nand4  g128(.a(new_n161_), .b(new_n158_), .c(new_n154_), .d(new_n147_), .O(z4));
  oai21  g129(.a(x02), .b(x01), .c(new_n48_), .O(new_n163_));
  nand4  g130(.a(new_n144_), .b(new_n53_), .c(x01), .d(x00), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n163_), .O(z5));
  aoi21  g132(.a(x20), .b(new_n37_), .c(x06), .O(new_n166_));
  oai21  g133(.a(new_n166_), .b(x11), .c(new_n34_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(z5), .O(new_n168_));
  aoi21  g135(.a(new_n38_), .b(x14), .c(new_n35_), .O(new_n169_));
  oai21  g136(.a(new_n169_), .b(new_n36_), .c(x03), .O(new_n170_));
  nor2   g137(.a(x02), .b(x01), .O(new_n171_));
  nand3  g138(.a(new_n133_), .b(new_n108_), .c(new_n105_), .O(new_n172_));
  oai21  g139(.a(new_n171_), .b(new_n117_), .c(new_n172_), .O(z7));
  nand2  g140(.a(z7), .b(new_n170_), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(new_n168_), .O(z6));
endmodule


