// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n199_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  inv1   g011(.a(x00), .O(new_n45_));
  nor2   g012(.a(x02), .b(new_n45_), .O(new_n46_));
  nand3  g013(.a(new_n46_), .b(new_n40_), .c(new_n44_), .O(new_n47_));
  inv1   g014(.a(x13), .O(new_n48_));
  nand3  g015(.a(x19), .b(new_n48_), .c(new_n38_), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n47_), .c(new_n43_), .O(new_n50_));
  oai21  g017(.a(new_n50_), .b(new_n42_), .c(new_n37_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(x14), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(x01), .O(z0));
  nand4  g021(.a(x04), .b(x03), .c(x02), .d(x01), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  inv1   g026(.a(x17), .O(new_n60_));
  nor2   g027(.a(new_n37_), .b(new_n60_), .O(new_n61_));
  inv1   g028(.a(x22), .O(new_n62_));
  inv1   g029(.a(x23), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n61_), .c(new_n59_), .d(new_n57_), .O(new_n65_));
  inv1   g032(.a(x09), .O(new_n66_));
  nor3   g033(.a(x04), .b(x03), .c(x01), .O(new_n67_));
  nor2   g034(.a(x08), .b(x06), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n36_), .d(new_n66_), .O(new_n69_));
  nor2   g036(.a(x14), .b(x12), .O(new_n70_));
  nor2   g037(.a(x17), .b(x16), .O(new_n71_));
  inv1   g038(.a(x19), .O(new_n72_));
  nor2   g039(.a(x20), .b(new_n72_), .O(new_n73_));
  nor2   g040(.a(x23), .b(x22), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n71_), .d(new_n70_), .O(new_n75_));
  oai21  g042(.a(new_n75_), .b(new_n69_), .c(new_n65_), .O(new_n76_));
  nand2  g043(.a(new_n48_), .b(new_n38_), .O(new_n77_));
  nand3  g044(.a(new_n40_), .b(x13), .c(x05), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand4  g047(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  inv1   g049(.a(x18), .O(new_n83_));
  nor2   g050(.a(new_n37_), .b(new_n83_), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n82_), .c(new_n64_), .d(new_n57_), .O(new_n85_));
  inv1   g052(.a(x08), .O(new_n86_));
  inv1   g053(.a(x07), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(x06), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n67_), .c(new_n66_), .d(new_n86_), .O(new_n89_));
  inv1   g056(.a(x12), .O(new_n90_));
  inv1   g057(.a(x14), .O(new_n91_));
  inv1   g058(.a(x16), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n36_), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n74_), .c(new_n37_), .d(new_n60_), .O(new_n95_));
  oai21  g062(.a(new_n95_), .b(new_n89_), .c(new_n85_), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n77_), .c(x24), .O(new_n97_));
  inv1   g064(.a(x01), .O(new_n98_));
  inv1   g065(.a(x02), .O(new_n99_));
  nand4  g066(.a(new_n34_), .b(new_n99_), .c(new_n98_), .d(x00), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand3  g068(.a(new_n44_), .b(new_n66_), .c(new_n86_), .O(new_n102_));
  nor3   g069(.a(new_n102_), .b(x06), .c(x04), .O(new_n103_));
  nand3  g070(.a(new_n40_), .b(new_n63_), .c(new_n62_), .O(new_n104_));
  nor3   g071(.a(new_n104_), .b(x20), .c(x17), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n103_), .c(new_n101_), .d(new_n94_), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n97_), .c(new_n80_), .O(z1));
  nand2  g074(.a(x24), .b(x18), .O(new_n108_));
  nand3  g075(.a(new_n40_), .b(x15), .c(x13), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(x05), .O(new_n111_));
  nand3  g078(.a(x24), .b(x18), .c(x13), .O(new_n112_));
  nand3  g079(.a(x15), .b(new_n48_), .c(new_n38_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(x20), .c(x14), .d(x11), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(x06), .c(x03), .d(x02), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n98_), .O(z2));
  nand4  g085(.a(new_n35_), .b(new_n34_), .c(new_n98_), .d(x00), .O(new_n119_));
  nor2   g086(.a(x10), .b(x08), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(new_n40_), .c(new_n37_), .d(new_n36_), .O(new_n121_));
  oai21  g088(.a(new_n121_), .b(new_n119_), .c(new_n91_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n99_), .O(new_n123_));
  nand4  g090(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n124_));
  nand4  g091(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n125_));
  nand3  g092(.a(new_n68_), .b(new_n34_), .c(new_n98_), .O(new_n126_));
  nand3  g093(.a(new_n73_), .b(new_n91_), .c(new_n36_), .O(new_n127_));
  oai22  g094(.a(new_n127_), .b(new_n126_), .c(new_n125_), .d(new_n124_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n79_), .O(new_n129_));
  nand3  g096(.a(new_n84_), .b(x14), .c(x11), .O(new_n130_));
  nand3  g097(.a(new_n88_), .b(new_n34_), .c(new_n98_), .O(new_n131_));
  nand4  g098(.a(new_n37_), .b(new_n91_), .c(new_n36_), .d(new_n86_), .O(new_n132_));
  oai22  g099(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n124_), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n77_), .c(x24), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n129_), .c(new_n123_), .O(z3));
  inv1   g102(.a(x04), .O(new_n136_));
  oai21  g103(.a(x23), .b(new_n136_), .c(new_n60_), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(new_n62_), .c(x09), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(x16), .c(new_n86_), .O(new_n139_));
  nand2  g106(.a(x14), .b(new_n99_), .O(new_n140_));
  inv1   g107(.a(new_n140_), .O(new_n141_));
  nand2  g108(.a(new_n49_), .b(new_n43_), .O(new_n142_));
  nor2   g109(.a(new_n142_), .b(new_n42_), .O(new_n143_));
  nand4  g110(.a(new_n46_), .b(new_n40_), .c(new_n91_), .d(new_n44_), .O(new_n144_));
  oai21  g111(.a(new_n143_), .b(new_n141_), .c(new_n144_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n139_), .O(new_n146_));
  oai21  g113(.a(new_n63_), .b(x04), .c(x17), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(x22), .c(new_n66_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n92_), .c(x08), .O(new_n149_));
  nand2  g116(.a(new_n140_), .b(new_n114_), .O(new_n150_));
  nor2   g117(.a(x10), .b(x02), .O(new_n151_));
  nand4  g118(.a(new_n151_), .b(new_n40_), .c(x21), .d(new_n91_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n150_), .O(z7));
  nand2  g120(.a(z7), .b(new_n149_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n146_), .O(z4));
  nand3  g122(.a(new_n40_), .b(new_n44_), .c(x00), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n91_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n99_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n143_), .O(z5));
  nand2  g126(.a(new_n151_), .b(x00), .O(new_n160_));
  nand3  g127(.a(x19), .b(x13), .c(x05), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  oai21  g129(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand4  g131(.a(x19), .b(new_n91_), .c(x13), .d(x05), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n160_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(x20), .c(new_n36_), .O(new_n167_));
  oai21  g134(.a(new_n36_), .b(x06), .c(x03), .O(new_n168_));
  nand4  g135(.a(new_n140_), .b(x15), .c(x13), .d(x05), .O(new_n169_));
  nand3  g136(.a(new_n151_), .b(x21), .c(new_n91_), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  aoi21  g139(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n173_));
  nand2  g140(.a(new_n91_), .b(new_n35_), .O(new_n174_));
  oai21  g141(.a(new_n173_), .b(new_n99_), .c(new_n174_), .O(new_n175_));
  nand3  g142(.a(new_n175_), .b(new_n48_), .c(new_n38_), .O(new_n176_));
  nor2   g143(.a(x20), .b(new_n91_), .O(new_n177_));
  nand4  g144(.a(new_n177_), .b(x13), .c(x05), .d(x02), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n176_), .O(new_n179_));
  nand3  g146(.a(new_n179_), .b(x15), .c(x11), .O(new_n180_));
  nand4  g147(.a(new_n180_), .b(new_n172_), .c(new_n167_), .d(new_n164_), .O(new_n181_));
  nand2  g148(.a(new_n181_), .b(new_n40_), .O(new_n182_));
  oai21  g149(.a(x20), .b(new_n91_), .c(x06), .O(new_n183_));
  nand3  g150(.a(new_n183_), .b(x11), .c(x03), .O(new_n184_));
  nand2  g151(.a(new_n140_), .b(new_n34_), .O(new_n185_));
  aoi21  g152(.a(new_n185_), .b(new_n184_), .c(new_n83_), .O(new_n186_));
  oai21  g153(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n187_));
  aoi21  g154(.a(new_n187_), .b(new_n36_), .c(x03), .O(new_n188_));
  nor2   g155(.a(new_n188_), .b(new_n87_), .O(new_n189_));
  oai21  g156(.a(new_n189_), .b(new_n186_), .c(new_n77_), .O(new_n190_));
  nand4  g157(.a(new_n183_), .b(x15), .c(new_n48_), .d(x11), .O(new_n191_));
  inv1   g158(.a(new_n191_), .O(new_n192_));
  nand3  g159(.a(new_n192_), .b(new_n38_), .c(x03), .O(new_n193_));
  nand2  g160(.a(new_n193_), .b(new_n190_), .O(new_n194_));
  nand3  g161(.a(new_n140_), .b(x15), .c(new_n34_), .O(new_n195_));
  oai21  g162(.a(new_n188_), .b(new_n72_), .c(new_n195_), .O(new_n196_));
  nand3  g163(.a(new_n196_), .b(new_n48_), .c(new_n38_), .O(new_n197_));
  nand2  g164(.a(new_n197_), .b(new_n140_), .O(new_n198_));
  aoi21  g165(.a(new_n194_), .b(x24), .c(new_n198_), .O(new_n199_));
  nand2  g166(.a(new_n199_), .b(new_n182_), .O(z6));
endmodule


