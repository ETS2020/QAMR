// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x01), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x13), .O(new_n44_));
  nand3  g011(.a(x19), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  oai21  g013(.a(new_n46_), .b(new_n42_), .c(new_n37_), .O(new_n47_));
  inv1   g014(.a(x02), .O(new_n48_));
  inv1   g015(.a(x10), .O(new_n49_));
  nand4  g016(.a(new_n40_), .b(new_n49_), .c(new_n48_), .d(x00), .O(new_n50_));
  aoi21  g017(.a(new_n50_), .b(new_n47_), .c(x20), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nand2  g019(.a(new_n49_), .b(x01), .O(new_n53_));
  oai21  g020(.a(new_n52_), .b(x03), .c(new_n53_), .O(z0));
  nand4  g021(.a(x06), .b(x04), .c(x03), .d(x01), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  nor2   g023(.a(new_n35_), .b(new_n49_), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(x09), .c(x08), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n60_));
  nand4  g027(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(new_n63_));
  inv1   g030(.a(x09), .O(new_n64_));
  nor3   g031(.a(x04), .b(x03), .c(x01), .O(new_n65_));
  nor2   g032(.a(x08), .b(x06), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n65_), .c(new_n35_), .d(new_n64_), .O(new_n67_));
  nor2   g034(.a(x14), .b(x12), .O(new_n68_));
  nor2   g035(.a(x17), .b(x16), .O(new_n69_));
  inv1   g036(.a(x19), .O(new_n70_));
  nor2   g037(.a(x20), .b(new_n70_), .O(new_n71_));
  nor2   g038(.a(x23), .b(x22), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n69_), .d(new_n68_), .O(new_n73_));
  oai21  g040(.a(new_n73_), .b(new_n67_), .c(new_n63_), .O(new_n74_));
  nand2  g041(.a(new_n44_), .b(new_n38_), .O(new_n75_));
  nand3  g042(.a(new_n40_), .b(x13), .c(x05), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand4  g045(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n79_));
  and2   g046(.a(x20), .b(x18), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(x23), .c(x22), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n59_), .c(new_n56_), .O(new_n83_));
  inv1   g050(.a(x08), .O(new_n84_));
  inv1   g051(.a(x07), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(x06), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n65_), .c(new_n64_), .d(new_n84_), .O(new_n87_));
  inv1   g054(.a(x17), .O(new_n88_));
  inv1   g055(.a(x20), .O(new_n89_));
  inv1   g056(.a(x12), .O(new_n90_));
  inv1   g057(.a(x16), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n36_), .c(new_n90_), .d(new_n35_), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n72_), .c(new_n89_), .d(new_n88_), .O(new_n94_));
  oai21  g061(.a(new_n94_), .b(new_n87_), .c(new_n83_), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n75_), .c(x24), .O(new_n96_));
  inv1   g063(.a(x03), .O(new_n97_));
  inv1   g064(.a(x00), .O(new_n98_));
  nor2   g065(.a(x01), .b(new_n98_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n97_), .c(new_n48_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand3  g068(.a(new_n49_), .b(new_n64_), .c(new_n84_), .O(new_n102_));
  nor3   g069(.a(new_n102_), .b(x06), .c(x04), .O(new_n103_));
  inv1   g070(.a(x22), .O(new_n104_));
  inv1   g071(.a(x23), .O(new_n105_));
  nand3  g072(.a(new_n40_), .b(new_n105_), .c(new_n104_), .O(new_n106_));
  nor3   g073(.a(new_n106_), .b(x20), .c(x17), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n103_), .c(new_n101_), .d(new_n93_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n96_), .c(new_n78_), .O(z1));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  nand3  g077(.a(new_n40_), .b(x15), .c(x13), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(x05), .O(new_n113_));
  nand3  g080(.a(x24), .b(x18), .c(x13), .O(new_n114_));
  nand3  g081(.a(x15), .b(new_n44_), .c(new_n38_), .O(new_n115_));
  and2   g082(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(x20), .c(x14), .d(x11), .O(new_n118_));
  inv1   g085(.a(new_n118_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(x10), .c(x06), .d(x03), .O(new_n120_));
  nor2   g087(.a(new_n120_), .b(new_n37_), .O(z2));
  nand4  g088(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n122_));
  nand3  g089(.a(x20), .b(x15), .c(x14), .O(new_n123_));
  nor4   g090(.a(new_n123_), .b(new_n122_), .c(new_n35_), .d(new_n49_), .O(new_n124_));
  nor2   g091(.a(x03), .b(x01), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n66_), .O(new_n126_));
  nand3  g093(.a(new_n71_), .b(new_n36_), .c(new_n35_), .O(new_n127_));
  nor2   g094(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(new_n124_), .c(new_n77_), .O(new_n129_));
  nand3  g096(.a(new_n80_), .b(new_n57_), .c(x14), .O(new_n130_));
  nor2   g097(.a(x11), .b(x08), .O(new_n131_));
  nor2   g098(.a(x20), .b(x14), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n131_), .c(new_n125_), .d(new_n86_), .O(new_n133_));
  oai21  g100(.a(new_n130_), .b(new_n122_), .c(new_n133_), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n75_), .c(x24), .O(new_n135_));
  nor3   g102(.a(x06), .b(x03), .c(x02), .O(new_n136_));
  nor3   g103(.a(x11), .b(x10), .c(x08), .O(new_n137_));
  nor3   g104(.a(x24), .b(x20), .c(x14), .O(new_n138_));
  nand4  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n99_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n135_), .c(new_n129_), .O(z3));
  inv1   g107(.a(x04), .O(new_n141_));
  oai21  g108(.a(x23), .b(new_n141_), .c(new_n88_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n104_), .c(x09), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(x16), .c(new_n84_), .O(new_n144_));
  inv1   g111(.a(new_n42_), .O(new_n145_));
  nand4  g112(.a(new_n50_), .b(new_n45_), .c(new_n43_), .d(new_n145_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  oai21  g114(.a(new_n105_), .b(x04), .c(x17), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(x22), .c(new_n64_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n91_), .c(x08), .O(new_n150_));
  nand4  g117(.a(new_n40_), .b(x21), .c(new_n49_), .d(new_n48_), .O(new_n151_));
  nand4  g118(.a(new_n151_), .b(new_n115_), .c(new_n114_), .d(new_n113_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n147_), .c(new_n53_), .O(z4));
  nand3  g121(.a(new_n40_), .b(new_n48_), .c(x00), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n37_), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n49_), .c(new_n46_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n145_), .O(z5));
  nand2  g125(.a(x20), .b(new_n36_), .O(new_n159_));
  aoi21  g126(.a(new_n159_), .b(new_n34_), .c(x11), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(x03), .c(new_n146_), .O(new_n161_));
  nand2  g128(.a(new_n89_), .b(x14), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(x06), .c(new_n35_), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(new_n97_), .c(new_n152_), .O(new_n164_));
  nand3  g131(.a(new_n164_), .b(new_n161_), .c(new_n53_), .O(z6));
  nand3  g132(.a(new_n40_), .b(x21), .c(new_n48_), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n37_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n49_), .O(new_n168_));
  nand3  g135(.a(new_n168_), .b(new_n116_), .c(new_n113_), .O(z7));
endmodule


