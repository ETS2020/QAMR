// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  inv1   g004(.a(x10), .O(new_n38_));
  inv1   g005(.a(x13), .O(new_n39_));
  nand2  g006(.a(x21), .b(new_n39_), .O(new_n40_));
  nand4  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .d(x00), .O(new_n41_));
  nand3  g008(.a(x19), .b(x13), .c(x05), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n41_), .c(x24), .O(new_n43_));
  inv1   g010(.a(x05), .O(new_n44_));
  oai21  g011(.a(x21), .b(new_n44_), .c(new_n39_), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(x24), .c(x07), .O(new_n46_));
  inv1   g013(.a(x21), .O(new_n47_));
  nor2   g014(.a(x13), .b(x05), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(x19), .O(new_n49_));
  nand2  g016(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nor3   g018(.a(new_n51_), .b(x20), .c(x14), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(x01), .O(z0));
  inv1   g021(.a(x04), .O(new_n55_));
  inv1   g022(.a(x09), .O(new_n56_));
  inv1   g023(.a(x12), .O(new_n57_));
  inv1   g024(.a(x16), .O(new_n58_));
  inv1   g025(.a(x17), .O(new_n59_));
  inv1   g026(.a(x20), .O(new_n60_));
  inv1   g027(.a(x23), .O(new_n61_));
  oai21  g028(.a(new_n50_), .b(new_n43_), .c(new_n61_), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(x22), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(x14), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n57_), .c(new_n36_), .d(new_n56_), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(x08), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n35_), .c(new_n55_), .d(new_n34_), .O(new_n68_));
  nand3  g035(.a(x24), .b(new_n47_), .c(x18), .O(new_n69_));
  inv1   g036(.a(x24), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(x15), .c(x13), .O(new_n71_));
  nand2  g038(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(x05), .O(new_n73_));
  nand2  g040(.a(x24), .b(x18), .O(new_n74_));
  nand4  g041(.a(new_n70_), .b(x21), .c(new_n38_), .d(new_n37_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(x13), .O(new_n77_));
  nand3  g044(.a(new_n48_), .b(new_n47_), .c(x15), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n77_), .c(new_n73_), .O(z7));
  nand4  g046(.a(z7), .b(x23), .c(x22), .d(x20), .O(new_n80_));
  inv1   g047(.a(new_n80_), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(x17), .c(x16), .d(x14), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(new_n57_), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(x11), .c(x09), .d(x08), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n35_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(x04), .c(x03), .d(x01), .O(new_n86_));
  oai21  g053(.a(new_n68_), .b(x01), .c(new_n86_), .O(z1));
  inv1   g054(.a(x01), .O(new_n88_));
  inv1   g055(.a(x14), .O(new_n89_));
  nand2  g056(.a(new_n74_), .b(new_n71_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(x05), .O(new_n91_));
  nand3  g058(.a(x24), .b(x18), .c(x13), .O(new_n92_));
  nand3  g059(.a(x15), .b(new_n39_), .c(new_n44_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n75_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(x20), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n89_), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(x11), .c(x06), .d(x03), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n88_), .c(new_n40_), .O(z2));
  inv1   g065(.a(x08), .O(new_n99_));
  nand3  g066(.a(x15), .b(x13), .c(x05), .O(new_n100_));
  nand3  g067(.a(x21), .b(new_n38_), .c(new_n37_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(x20), .c(x14), .d(x11), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(x06), .c(x03), .d(x01), .O(new_n105_));
  nand3  g072(.a(new_n38_), .b(new_n37_), .c(x00), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n42_), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n60_), .c(new_n89_), .d(new_n36_), .O(new_n108_));
  nor2   g075(.a(new_n108_), .b(x08), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n35_), .c(new_n34_), .d(new_n88_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n105_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n70_), .O(new_n112_));
  inv1   g079(.a(new_n48_), .O(new_n113_));
  nand4  g080(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n114_));
  nand4  g081(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n115_));
  nand4  g082(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n88_), .O(new_n116_));
  nand4  g083(.a(new_n60_), .b(new_n89_), .c(new_n36_), .d(new_n99_), .O(new_n117_));
  oai22  g084(.a(new_n117_), .b(new_n116_), .c(new_n115_), .d(new_n114_), .O(new_n118_));
  nand3  g085(.a(new_n118_), .b(new_n113_), .c(x24), .O(new_n119_));
  nand4  g086(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n120_));
  nand4  g087(.a(new_n99_), .b(new_n35_), .c(new_n34_), .d(new_n88_), .O(new_n121_));
  nand4  g088(.a(new_n60_), .b(x19), .c(new_n89_), .d(new_n36_), .O(new_n122_));
  oai22  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .d(new_n114_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n44_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n47_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n39_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n119_), .c(new_n112_), .O(z3));
  inv1   g094(.a(x22), .O(new_n128_));
  oai21  g095(.a(x23), .b(new_n55_), .c(new_n59_), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(new_n128_), .c(x09), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x16), .c(new_n99_), .O(new_n131_));
  nand2  g098(.a(x24), .b(x07), .O(new_n132_));
  nand3  g099(.a(new_n70_), .b(x19), .c(x13), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(x05), .O(new_n135_));
  nand3  g102(.a(x24), .b(x13), .c(x07), .O(new_n136_));
  nand4  g103(.a(new_n70_), .b(new_n38_), .c(new_n37_), .d(x00), .O(new_n137_));
  nand3  g104(.a(x19), .b(new_n39_), .c(new_n44_), .O(new_n138_));
  nand4  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n135_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n131_), .O(new_n140_));
  oai21  g107(.a(new_n61_), .b(x04), .c(x17), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(x22), .c(new_n56_), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(new_n58_), .c(x08), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n94_), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n140_), .c(new_n40_), .O(z4));
  inv1   g112(.a(x19), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(x05), .c(new_n47_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n39_), .O(new_n148_));
  nand4  g115(.a(new_n148_), .b(new_n137_), .c(new_n136_), .d(new_n135_), .O(z5));
  aoi21  g116(.a(new_n60_), .b(x14), .c(new_n35_), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(new_n36_), .c(x03), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(z7), .O(new_n152_));
  oai21  g119(.a(new_n60_), .b(x14), .c(new_n35_), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(new_n36_), .c(x03), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(new_n51_), .c(new_n152_), .O(z6));
endmodule


