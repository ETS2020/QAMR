// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n173_, new_n174_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x22), .O(new_n37_));
  nor2   g004(.a(new_n37_), .b(x08), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x02), .O(new_n45_));
  inv1   g012(.a(x10), .O(new_n46_));
  nand4  g013(.a(new_n41_), .b(new_n46_), .c(new_n45_), .d(x00), .O(new_n47_));
  inv1   g014(.a(x13), .O(new_n48_));
  nand3  g015(.a(x19), .b(new_n48_), .c(new_n39_), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n47_), .c(new_n44_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nor4   g018(.a(new_n51_), .b(new_n38_), .c(x20), .d(x14), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(x01), .O(z0));
  inv1   g021(.a(x14), .O(new_n55_));
  inv1   g022(.a(x16), .O(new_n56_));
  inv1   g023(.a(x23), .O(new_n57_));
  nand3  g024(.a(x15), .b(x13), .c(x05), .O(new_n58_));
  nand3  g025(.a(x21), .b(new_n46_), .c(new_n45_), .O(new_n59_));
  aoi21  g026(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand4  g027(.a(new_n60_), .b(x22), .c(x20), .d(x17), .O(new_n61_));
  nor3   g028(.a(new_n61_), .b(new_n56_), .c(new_n55_), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(x12), .c(x11), .d(x09), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n35_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(x04), .c(x03), .d(x01), .O(new_n65_));
  inv1   g032(.a(x04), .O(new_n66_));
  inv1   g033(.a(x09), .O(new_n67_));
  inv1   g034(.a(x12), .O(new_n68_));
  inv1   g035(.a(x17), .O(new_n69_));
  inv1   g036(.a(x20), .O(new_n70_));
  nand3  g037(.a(new_n46_), .b(new_n45_), .c(x00), .O(new_n71_));
  nand3  g038(.a(x19), .b(x13), .c(x05), .O(new_n72_));
  aoi21  g039(.a(new_n72_), .b(new_n71_), .c(x23), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n37_), .c(new_n70_), .d(new_n69_), .O(new_n74_));
  nor3   g041(.a(new_n74_), .b(x16), .c(x14), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n68_), .c(new_n36_), .d(new_n67_), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(x08), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n35_), .c(new_n66_), .d(new_n34_), .O(new_n78_));
  oai21  g045(.a(new_n78_), .b(x01), .c(new_n65_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n41_), .O(new_n80_));
  inv1   g047(.a(new_n38_), .O(new_n81_));
  nand2  g048(.a(new_n48_), .b(new_n39_), .O(new_n82_));
  inv1   g049(.a(x01), .O(new_n83_));
  nand4  g050(.a(x12), .b(x11), .c(x09), .d(x06), .O(new_n84_));
  nor4   g051(.a(new_n84_), .b(new_n66_), .c(new_n34_), .d(new_n83_), .O(new_n85_));
  nand3  g052(.a(x17), .b(x16), .c(x14), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  and2   g054(.a(x20), .b(x18), .O(new_n88_));
  nor2   g055(.a(new_n57_), .b(new_n37_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n85_), .O(new_n90_));
  nor3   g057(.a(x04), .b(x03), .c(x01), .O(new_n91_));
  nor2   g058(.a(x09), .b(x08), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(x07), .d(new_n35_), .O(new_n93_));
  nand4  g060(.a(new_n56_), .b(new_n55_), .c(new_n68_), .d(new_n36_), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(new_n95_));
  nor2   g062(.a(x23), .b(x22), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n95_), .c(new_n70_), .d(new_n69_), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n93_), .c(new_n90_), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n82_), .c(x24), .O(new_n99_));
  nand3  g066(.a(x16), .b(x15), .c(x14), .O(new_n100_));
  nand3  g067(.a(new_n89_), .b(x20), .c(x17), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n85_), .O(new_n103_));
  nor2   g070(.a(x08), .b(x06), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n91_), .c(new_n36_), .d(new_n67_), .O(new_n105_));
  nor2   g072(.a(x14), .b(x12), .O(new_n106_));
  nor2   g073(.a(x17), .b(x16), .O(new_n107_));
  inv1   g074(.a(x19), .O(new_n108_));
  nor2   g075(.a(x20), .b(new_n108_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n107_), .c(new_n106_), .d(new_n96_), .O(new_n110_));
  oai21  g077(.a(new_n110_), .b(new_n105_), .c(new_n103_), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n48_), .c(new_n39_), .O(new_n112_));
  nand3  g079(.a(new_n112_), .b(new_n99_), .c(new_n81_), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n80_), .O(z1));
  nand2  g082(.a(x24), .b(x18), .O(new_n116_));
  nand3  g083(.a(new_n41_), .b(x15), .c(x13), .O(new_n117_));
  aoi21  g084(.a(new_n117_), .b(new_n116_), .c(new_n39_), .O(new_n118_));
  nand3  g085(.a(x24), .b(x18), .c(x13), .O(new_n119_));
  nand3  g086(.a(x15), .b(new_n48_), .c(new_n39_), .O(new_n120_));
  nand4  g087(.a(new_n41_), .b(x21), .c(new_n46_), .d(new_n45_), .O(new_n121_));
  nand3  g088(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(new_n118_), .O(new_n123_));
  nor4   g090(.a(new_n123_), .b(new_n38_), .c(new_n70_), .d(new_n55_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x11), .c(x06), .d(x03), .O(new_n125_));
  nor2   g092(.a(new_n125_), .b(new_n83_), .O(z2));
  inv1   g093(.a(x08), .O(new_n127_));
  nand2  g094(.a(new_n59_), .b(new_n58_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(x20), .c(x14), .d(x11), .O(new_n129_));
  nor2   g096(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(x06), .c(x03), .d(x01), .O(new_n131_));
  nand2  g098(.a(new_n72_), .b(new_n71_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n70_), .c(new_n55_), .d(new_n36_), .O(new_n133_));
  nor2   g100(.a(new_n133_), .b(x08), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n35_), .c(new_n34_), .d(new_n83_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n41_), .O(new_n137_));
  nand4  g104(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n138_));
  nand3  g105(.a(new_n88_), .b(x14), .c(x11), .O(new_n139_));
  nand4  g106(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n83_), .O(new_n140_));
  nand4  g107(.a(new_n70_), .b(new_n55_), .c(new_n36_), .d(new_n127_), .O(new_n141_));
  oai22  g108(.a(new_n141_), .b(new_n140_), .c(new_n139_), .d(new_n138_), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n82_), .c(x24), .O(new_n143_));
  nand4  g110(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n144_));
  nand3  g111(.a(new_n104_), .b(new_n34_), .c(new_n83_), .O(new_n145_));
  nand3  g112(.a(new_n109_), .b(new_n55_), .c(new_n36_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n145_), .c(new_n144_), .d(new_n138_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n48_), .c(new_n39_), .O(new_n148_));
  nand4  g115(.a(new_n148_), .b(new_n143_), .c(new_n137_), .d(new_n81_), .O(z3));
  inv1   g116(.a(new_n51_), .O(new_n150_));
  nand2  g117(.a(new_n57_), .b(x04), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(new_n69_), .c(new_n67_), .O(new_n152_));
  nand3  g119(.a(new_n152_), .b(new_n37_), .c(new_n56_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n127_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  inv1   g122(.a(new_n123_), .O(new_n156_));
  aoi21  g123(.a(x23), .b(new_n66_), .c(new_n69_), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(new_n37_), .c(x09), .O(new_n158_));
  nand3  g125(.a(new_n158_), .b(x16), .c(x08), .O(new_n159_));
  oai21  g126(.a(x22), .b(x08), .c(new_n159_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n155_), .O(z4));
  inv1   g129(.a(new_n43_), .O(new_n163_));
  and2   g130(.a(new_n47_), .b(new_n44_), .O(new_n164_));
  nand4  g131(.a(new_n164_), .b(new_n49_), .c(new_n163_), .d(new_n81_), .O(z5));
  nand2  g132(.a(x20), .b(new_n55_), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(new_n35_), .c(x11), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(x03), .c(new_n150_), .O(new_n168_));
  nand2  g135(.a(new_n70_), .b(x14), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(x06), .c(new_n36_), .O(new_n170_));
  oai21  g137(.a(new_n170_), .b(new_n34_), .c(new_n156_), .O(new_n171_));
  aoi21  g138(.a(new_n171_), .b(new_n168_), .c(new_n38_), .O(z6));
  inv1   g139(.a(new_n118_), .O(new_n173_));
  and2   g140(.a(new_n120_), .b(new_n119_), .O(new_n174_));
  nand4  g141(.a(new_n174_), .b(new_n121_), .c(new_n173_), .d(new_n81_), .O(z7));
endmodule


