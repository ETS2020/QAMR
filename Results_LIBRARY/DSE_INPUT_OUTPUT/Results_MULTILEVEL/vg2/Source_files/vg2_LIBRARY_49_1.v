// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x01), .O(new_n34_));
  inv1   g001(.a(x03), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x14), .O(new_n38_));
  inv1   g005(.a(x20), .O(new_n39_));
  nand3  g006(.a(x24), .b(x10), .c(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x05), .O(new_n44_));
  inv1   g011(.a(x05), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  inv1   g013(.a(x10), .O(new_n47_));
  nand2  g014(.a(x24), .b(new_n47_), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(x19), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g016(.a(x00), .O(new_n50_));
  nor2   g017(.a(x02), .b(new_n50_), .O(new_n51_));
  nor2   g018(.a(x24), .b(x10), .O(new_n52_));
  nand4  g019(.a(x24), .b(x13), .c(x10), .d(x07), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  aoi21  g021(.a(new_n52_), .b(new_n51_), .c(new_n54_), .O(new_n55_));
  nand3  g022(.a(new_n55_), .b(new_n49_), .c(new_n44_), .O(new_n56_));
  nand4  g023(.a(new_n56_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n57_));
  inv1   g024(.a(new_n57_), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(z0));
  inv1   g027(.a(x02), .O(new_n61_));
  nand3  g028(.a(x04), .b(x03), .c(x01), .O(new_n62_));
  nand4  g029(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g031(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n65_));
  nand4  g032(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nor2   g035(.a(x03), .b(x01), .O(new_n69_));
  nor2   g036(.a(x06), .b(x04), .O(new_n70_));
  nor2   g037(.a(x09), .b(x08), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n72_));
  nor2   g039(.a(x12), .b(x11), .O(new_n73_));
  nor2   g040(.a(x16), .b(x14), .O(new_n74_));
  nor2   g041(.a(x20), .b(x17), .O(new_n75_));
  nor2   g042(.a(x23), .b(x22), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  oai21  g044(.a(new_n77_), .b(new_n72_), .c(new_n68_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n47_), .c(new_n61_), .O(new_n79_));
  nand2  g046(.a(x13), .b(x05), .O(new_n80_));
  nor2   g047(.a(x13), .b(x05), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand4  g050(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n84_));
  nand4  g051(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n64_), .O(new_n87_));
  inv1   g054(.a(x09), .O(new_n88_));
  nor3   g055(.a(x04), .b(x03), .c(x01), .O(new_n89_));
  nor2   g056(.a(x08), .b(x06), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n89_), .c(new_n37_), .d(new_n88_), .O(new_n91_));
  nor2   g058(.a(x14), .b(x12), .O(new_n92_));
  nor2   g059(.a(x17), .b(x16), .O(new_n93_));
  inv1   g060(.a(x19), .O(new_n94_));
  nor2   g061(.a(x20), .b(new_n94_), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n93_), .c(new_n92_), .d(new_n76_), .O(new_n96_));
  oai21  g063(.a(new_n96_), .b(new_n91_), .c(new_n87_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n83_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n79_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n41_), .O(new_n100_));
  nand4  g067(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n65_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n64_), .O(new_n103_));
  nand4  g070(.a(new_n89_), .b(new_n71_), .c(x07), .d(new_n36_), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n77_), .c(new_n103_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n82_), .c(x24), .O(new_n106_));
  nand3  g073(.a(new_n97_), .b(new_n46_), .c(new_n45_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(x10), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n100_), .O(z1));
  nand3  g077(.a(x24), .b(x18), .c(x10), .O(new_n111_));
  nand3  g078(.a(new_n41_), .b(x15), .c(x13), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(x05), .O(new_n114_));
  nand4  g081(.a(new_n48_), .b(x15), .c(new_n46_), .d(new_n45_), .O(new_n115_));
  nor2   g082(.a(x10), .b(x02), .O(new_n116_));
  inv1   g083(.a(x21), .O(new_n117_));
  nor2   g084(.a(x24), .b(new_n117_), .O(new_n118_));
  nand4  g085(.a(x24), .b(x18), .c(x13), .d(x10), .O(new_n119_));
  inv1   g086(.a(new_n119_), .O(new_n120_));
  aoi21  g087(.a(new_n118_), .b(new_n116_), .c(new_n120_), .O(new_n121_));
  nand3  g088(.a(new_n121_), .b(new_n115_), .c(new_n114_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(x20), .c(x14), .d(x11), .O(new_n123_));
  inv1   g090(.a(new_n123_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x06), .c(x03), .d(x01), .O(new_n125_));
  inv1   g092(.a(new_n125_), .O(z2));
  nor2   g093(.a(x06), .b(x03), .O(new_n127_));
  inv1   g094(.a(x08), .O(new_n128_));
  nand4  g095(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n128_), .O(new_n129_));
  inv1   g096(.a(new_n129_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(new_n127_), .c(new_n34_), .d(x00), .O(new_n131_));
  nand4  g098(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n132_));
  inv1   g099(.a(new_n132_), .O(new_n133_));
  nor2   g100(.a(new_n38_), .b(new_n37_), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n133_), .c(new_n118_), .d(x20), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n131_), .c(x02), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x24), .c(new_n47_), .O(new_n137_));
  nand3  g104(.a(new_n134_), .b(x20), .c(x15), .O(new_n138_));
  nand2  g105(.a(new_n90_), .b(new_n69_), .O(new_n139_));
  nand3  g106(.a(new_n95_), .b(new_n38_), .c(new_n37_), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n132_), .O(new_n141_));
  nand3  g108(.a(new_n41_), .b(x13), .c(x05), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n82_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand3  g111(.a(new_n134_), .b(x20), .c(x18), .O(new_n145_));
  nand3  g112(.a(new_n69_), .b(x07), .c(new_n36_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n129_), .c(new_n145_), .d(new_n132_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n82_), .c(x24), .O(new_n148_));
  nand3  g115(.a(new_n148_), .b(new_n144_), .c(new_n137_), .O(z3));
  inv1   g116(.a(x22), .O(new_n150_));
  inv1   g117(.a(x04), .O(new_n151_));
  inv1   g118(.a(x17), .O(new_n152_));
  oai21  g119(.a(x23), .b(new_n151_), .c(new_n152_), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(new_n150_), .c(x09), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(x16), .c(new_n128_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n56_), .O(new_n156_));
  inv1   g123(.a(x16), .O(new_n157_));
  inv1   g124(.a(x23), .O(new_n158_));
  oai21  g125(.a(new_n158_), .b(x04), .c(x17), .O(new_n159_));
  aoi21  g126(.a(new_n159_), .b(x22), .c(new_n88_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n157_), .c(x08), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n122_), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(new_n156_), .O(z4));
  nand2  g130(.a(x24), .b(x07), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n42_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(x05), .O(new_n166_));
  oai21  g133(.a(new_n51_), .b(x24), .c(new_n47_), .O(new_n167_));
  nor2   g134(.a(new_n41_), .b(new_n46_), .O(new_n168_));
  nor2   g135(.a(new_n94_), .b(x13), .O(new_n169_));
  aoi22  g136(.a(new_n169_), .b(new_n45_), .c(new_n168_), .d(x07), .O(new_n170_));
  nand3  g137(.a(new_n170_), .b(new_n167_), .c(new_n166_), .O(z5));
  aoi21  g138(.a(x20), .b(new_n38_), .c(x06), .O(new_n172_));
  oai21  g139(.a(new_n172_), .b(x11), .c(new_n35_), .O(new_n173_));
  nand2  g140(.a(new_n173_), .b(new_n56_), .O(new_n174_));
  aoi21  g141(.a(new_n39_), .b(x14), .c(new_n36_), .O(new_n175_));
  oai21  g142(.a(new_n175_), .b(new_n37_), .c(x03), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(new_n122_), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n174_), .O(z6));
  and2   g145(.a(x24), .b(x18), .O(new_n179_));
  inv1   g146(.a(new_n112_), .O(new_n180_));
  oai21  g147(.a(new_n180_), .b(new_n179_), .c(x05), .O(new_n181_));
  oai21  g148(.a(new_n117_), .b(x02), .c(new_n41_), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(new_n47_), .O(new_n183_));
  inv1   g150(.a(x15), .O(new_n184_));
  nor2   g151(.a(new_n184_), .b(x13), .O(new_n185_));
  aoi22  g152(.a(new_n185_), .b(new_n45_), .c(new_n179_), .d(x13), .O(new_n186_));
  nand3  g153(.a(new_n186_), .b(new_n183_), .c(new_n181_), .O(z7));
endmodule


