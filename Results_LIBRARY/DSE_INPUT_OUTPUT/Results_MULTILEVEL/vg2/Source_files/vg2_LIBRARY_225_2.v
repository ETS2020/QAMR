// Benchmark "FAU" written by ABC on Fri Aug 14 02:36:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  inv1   g005(.a(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nor2   g007(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  inv1   g008(.a(new_n41_), .O(new_n42_));
  nand3  g009(.a(new_n40_), .b(x19), .c(x13), .O(new_n43_));
  aoi21  g010(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  inv1   g012(.a(x02), .O(new_n46_));
  nand3  g013(.a(new_n40_), .b(new_n46_), .c(x00), .O(new_n47_));
  inv1   g014(.a(x13), .O(new_n48_));
  nand3  g015(.a(x19), .b(new_n48_), .c(new_n38_), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(new_n50_));
  oai21  g017(.a(new_n50_), .b(new_n44_), .c(new_n37_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(x14), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nand2  g020(.a(new_n40_), .b(x10), .O(new_n54_));
  oai21  g021(.a(new_n53_), .b(x01), .c(new_n54_), .O(z0));
  inv1   g022(.a(x10), .O(new_n56_));
  inv1   g023(.a(x12), .O(new_n57_));
  inv1   g024(.a(x21), .O(new_n58_));
  nand3  g025(.a(x15), .b(x13), .c(x05), .O(new_n59_));
  oai21  g026(.a(new_n58_), .b(x02), .c(new_n59_), .O(new_n60_));
  nand4  g027(.a(new_n60_), .b(x23), .c(x22), .d(x20), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(x17), .c(x16), .d(x14), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(x11), .c(x09), .d(x08), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(new_n35_), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(x04), .c(x03), .d(x01), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  inv1   g035(.a(x04), .O(new_n69_));
  inv1   g036(.a(x08), .O(new_n70_));
  inv1   g037(.a(x09), .O(new_n71_));
  inv1   g038(.a(x14), .O(new_n72_));
  inv1   g039(.a(x16), .O(new_n73_));
  inv1   g040(.a(x17), .O(new_n74_));
  inv1   g041(.a(x22), .O(new_n75_));
  inv1   g042(.a(x23), .O(new_n76_));
  inv1   g043(.a(x00), .O(new_n77_));
  nand3  g044(.a(x19), .b(x13), .c(x05), .O(new_n78_));
  oai21  g045(.a(x02), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n76_), .c(new_n75_), .d(new_n37_), .O(new_n80_));
  inv1   g047(.a(new_n80_), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(x12), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n36_), .c(new_n71_), .d(new_n70_), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(x06), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n69_), .c(new_n34_), .d(new_n68_), .O(new_n86_));
  aoi21  g053(.a(new_n86_), .b(new_n67_), .c(x24), .O(new_n87_));
  nand4  g054(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n88_));
  nor4   g055(.a(new_n88_), .b(new_n69_), .c(new_n34_), .d(new_n68_), .O(new_n89_));
  nand4  g056(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n90_));
  nor2   g057(.a(new_n76_), .b(new_n75_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(x20), .c(x17), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  nor3   g061(.a(x04), .b(x03), .c(x01), .O(new_n95_));
  nor2   g062(.a(x08), .b(x06), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n95_), .c(new_n36_), .d(new_n71_), .O(new_n97_));
  nor2   g064(.a(x14), .b(x12), .O(new_n98_));
  nor2   g065(.a(x17), .b(x16), .O(new_n99_));
  inv1   g066(.a(x19), .O(new_n100_));
  nor2   g067(.a(x20), .b(new_n100_), .O(new_n101_));
  nor2   g068(.a(x23), .b(x22), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n101_), .c(new_n99_), .d(new_n98_), .O(new_n103_));
  oai21  g070(.a(new_n103_), .b(new_n97_), .c(new_n94_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n48_), .c(new_n38_), .O(new_n105_));
  inv1   g072(.a(new_n105_), .O(new_n106_));
  oai21  g073(.a(new_n106_), .b(new_n87_), .c(new_n56_), .O(new_n107_));
  nand4  g074(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n108_));
  inv1   g075(.a(new_n108_), .O(new_n109_));
  inv1   g076(.a(x18), .O(new_n110_));
  nor2   g077(.a(new_n37_), .b(new_n110_), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(new_n109_), .c(new_n91_), .d(new_n89_), .O(new_n112_));
  nor2   g079(.a(new_n39_), .b(x06), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(new_n95_), .c(new_n71_), .d(new_n70_), .O(new_n114_));
  nand4  g081(.a(new_n73_), .b(new_n72_), .c(new_n57_), .d(new_n36_), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  nand4  g083(.a(new_n116_), .b(new_n102_), .c(new_n37_), .d(new_n74_), .O(new_n117_));
  oai21  g084(.a(new_n117_), .b(new_n114_), .c(new_n112_), .O(new_n118_));
  nand2  g085(.a(new_n48_), .b(new_n38_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n105_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(x24), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n107_), .O(z1));
  nor2   g090(.a(new_n40_), .b(new_n110_), .O(new_n124_));
  inv1   g091(.a(new_n124_), .O(new_n125_));
  nand3  g092(.a(new_n40_), .b(x15), .c(x13), .O(new_n126_));
  aoi21  g093(.a(new_n126_), .b(new_n125_), .c(new_n38_), .O(new_n127_));
  nand2  g094(.a(new_n124_), .b(x13), .O(new_n128_));
  nand3  g095(.a(x15), .b(new_n48_), .c(new_n38_), .O(new_n129_));
  nand3  g096(.a(new_n40_), .b(x21), .c(new_n46_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n129_), .c(new_n128_), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(new_n127_), .c(x20), .O(new_n132_));
  nor2   g099(.a(new_n132_), .b(new_n72_), .O(new_n133_));
  nand4  g100(.a(new_n133_), .b(x11), .c(x06), .d(x03), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n68_), .c(new_n54_), .O(z2));
  nand4  g102(.a(new_n60_), .b(x20), .c(x14), .d(x11), .O(new_n136_));
  nor2   g103(.a(new_n136_), .b(new_n70_), .O(new_n137_));
  nand4  g104(.a(new_n137_), .b(x06), .c(x03), .d(x01), .O(new_n138_));
  nand4  g105(.a(new_n79_), .b(new_n37_), .c(new_n72_), .d(new_n36_), .O(new_n139_));
  nor2   g106(.a(new_n139_), .b(x08), .O(new_n140_));
  nand4  g107(.a(new_n140_), .b(new_n35_), .c(new_n34_), .d(new_n68_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n138_), .c(new_n56_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n40_), .O(new_n143_));
  nand4  g110(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n144_));
  nand3  g111(.a(new_n111_), .b(x14), .c(x11), .O(new_n145_));
  nand3  g112(.a(new_n113_), .b(new_n34_), .c(new_n68_), .O(new_n146_));
  nand4  g113(.a(new_n37_), .b(new_n72_), .c(new_n36_), .d(new_n70_), .O(new_n147_));
  oai22  g114(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(new_n144_), .O(new_n148_));
  nand3  g115(.a(new_n148_), .b(new_n119_), .c(x24), .O(new_n149_));
  nand4  g116(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n150_));
  nand3  g117(.a(new_n96_), .b(new_n34_), .c(new_n68_), .O(new_n151_));
  nand3  g118(.a(new_n101_), .b(new_n72_), .c(new_n36_), .O(new_n152_));
  oai22  g119(.a(new_n152_), .b(new_n151_), .c(new_n150_), .d(new_n144_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n48_), .c(new_n38_), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n149_), .c(new_n143_), .O(z3));
  oai21  g122(.a(x23), .b(new_n69_), .c(new_n74_), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n75_), .c(x09), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(x16), .c(new_n70_), .O(new_n158_));
  nand4  g125(.a(new_n40_), .b(x19), .c(x13), .d(new_n56_), .O(new_n159_));
  inv1   g126(.a(new_n159_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n41_), .c(x05), .O(new_n161_));
  nand4  g128(.a(new_n54_), .b(x19), .c(new_n48_), .d(new_n38_), .O(new_n162_));
  nor2   g129(.a(x02), .b(new_n77_), .O(new_n163_));
  nand3  g130(.a(new_n163_), .b(new_n40_), .c(new_n56_), .O(new_n164_));
  nand4  g131(.a(new_n164_), .b(new_n162_), .c(new_n161_), .d(new_n45_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n158_), .O(new_n166_));
  oai21  g133(.a(new_n76_), .b(x04), .c(x17), .O(new_n167_));
  aoi21  g134(.a(new_n167_), .b(x22), .c(new_n71_), .O(new_n168_));
  oai21  g135(.a(new_n168_), .b(new_n73_), .c(x08), .O(new_n169_));
  nand4  g136(.a(new_n40_), .b(x15), .c(x13), .d(new_n56_), .O(new_n170_));
  inv1   g137(.a(new_n170_), .O(new_n171_));
  oai21  g138(.a(new_n171_), .b(new_n124_), .c(x05), .O(new_n172_));
  nand4  g139(.a(new_n54_), .b(x15), .c(new_n48_), .d(new_n38_), .O(new_n173_));
  nand4  g140(.a(new_n40_), .b(x21), .c(new_n56_), .d(new_n46_), .O(new_n174_));
  nand4  g141(.a(new_n174_), .b(new_n173_), .c(new_n172_), .d(new_n128_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n169_), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(new_n166_), .O(z4));
  inv1   g144(.a(new_n44_), .O(new_n178_));
  oai21  g145(.a(new_n163_), .b(x10), .c(new_n40_), .O(new_n179_));
  nand4  g146(.a(new_n179_), .b(new_n49_), .c(new_n45_), .d(new_n178_), .O(z5));
  aoi21  g147(.a(x20), .b(new_n72_), .c(x06), .O(new_n181_));
  oai21  g148(.a(new_n181_), .b(x11), .c(new_n34_), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(new_n165_), .O(new_n183_));
  aoi21  g150(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n184_));
  oai21  g151(.a(new_n184_), .b(new_n36_), .c(x03), .O(new_n185_));
  nand2  g152(.a(new_n185_), .b(new_n175_), .O(new_n186_));
  nand2  g153(.a(new_n186_), .b(new_n183_), .O(z6));
  inv1   g154(.a(new_n127_), .O(new_n188_));
  oai21  g155(.a(new_n58_), .b(x02), .c(new_n56_), .O(new_n189_));
  nand2  g156(.a(new_n189_), .b(new_n40_), .O(new_n190_));
  nand4  g157(.a(new_n190_), .b(new_n129_), .c(new_n128_), .d(new_n188_), .O(z7));
endmodule


