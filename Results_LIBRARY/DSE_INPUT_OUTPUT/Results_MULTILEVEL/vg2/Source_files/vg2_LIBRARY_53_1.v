// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n185_;
  inv1   g000(.a(x02), .O(new_n34_));
  inv1   g001(.a(x10), .O(new_n35_));
  inv1   g002(.a(x01), .O(new_n36_));
  inv1   g003(.a(x03), .O(new_n37_));
  inv1   g004(.a(x06), .O(new_n38_));
  nand4  g005(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x00), .O(new_n39_));
  inv1   g006(.a(x11), .O(new_n40_));
  inv1   g007(.a(x14), .O(new_n41_));
  inv1   g008(.a(x20), .O(new_n42_));
  inv1   g009(.a(x24), .O(new_n43_));
  nand4  g010(.a(new_n43_), .b(new_n42_), .c(new_n41_), .d(new_n40_), .O(new_n44_));
  oai21  g011(.a(new_n44_), .b(new_n39_), .c(new_n35_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  nand2  g013(.a(x24), .b(x07), .O(new_n47_));
  nand3  g014(.a(new_n43_), .b(x19), .c(x13), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g016(.a(x24), .b(x13), .c(x07), .O(new_n50_));
  inv1   g017(.a(x05), .O(new_n51_));
  inv1   g018(.a(x13), .O(new_n52_));
  nand3  g019(.a(x19), .b(new_n52_), .c(new_n51_), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  aoi21  g021(.a(new_n49_), .b(x05), .c(new_n54_), .O(new_n55_));
  nor3   g022(.a(new_n55_), .b(x20), .c(x14), .O(new_n56_));
  nand4  g023(.a(new_n56_), .b(new_n40_), .c(new_n38_), .d(new_n37_), .O(new_n57_));
  oai21  g024(.a(new_n57_), .b(x01), .c(new_n46_), .O(z0));
  inv1   g025(.a(x04), .O(new_n59_));
  nand4  g026(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n60_));
  nor4   g027(.a(new_n60_), .b(new_n59_), .c(new_n37_), .d(new_n36_), .O(new_n61_));
  nand4  g028(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n62_));
  inv1   g029(.a(x22), .O(new_n63_));
  inv1   g030(.a(x23), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(x20), .c(x17), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n62_), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n61_), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  nor3   g036(.a(x04), .b(x03), .c(x01), .O(new_n70_));
  nor2   g037(.a(x08), .b(x06), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n40_), .d(new_n69_), .O(new_n72_));
  nor2   g039(.a(x14), .b(x12), .O(new_n73_));
  nor2   g040(.a(x17), .b(x16), .O(new_n74_));
  inv1   g041(.a(x19), .O(new_n75_));
  nor2   g042(.a(x20), .b(new_n75_), .O(new_n76_));
  nor2   g043(.a(x23), .b(x22), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n76_), .c(new_n74_), .d(new_n73_), .O(new_n78_));
  oai21  g045(.a(new_n78_), .b(new_n72_), .c(new_n68_), .O(new_n79_));
  nand2  g046(.a(new_n52_), .b(new_n51_), .O(new_n80_));
  nand3  g047(.a(new_n43_), .b(x13), .c(x05), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand4  g050(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  and2   g052(.a(x20), .b(x18), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n85_), .c(new_n65_), .d(new_n61_), .O(new_n87_));
  nor2   g054(.a(x09), .b(x08), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n70_), .c(x07), .d(new_n38_), .O(new_n89_));
  inv1   g056(.a(x12), .O(new_n90_));
  inv1   g057(.a(x16), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n41_), .c(new_n90_), .d(new_n40_), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  inv1   g060(.a(x17), .O(new_n94_));
  nand3  g061(.a(new_n77_), .b(new_n42_), .c(new_n94_), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n89_), .c(new_n87_), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n80_), .c(x24), .O(new_n99_));
  inv1   g066(.a(x21), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n42_), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n85_), .c(new_n65_), .d(new_n61_), .O(new_n102_));
  nor2   g069(.a(x03), .b(x01), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(x00), .O(new_n104_));
  nand3  g071(.a(new_n88_), .b(new_n38_), .c(new_n59_), .O(new_n105_));
  nor2   g072(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n96_), .c(new_n93_), .O(new_n107_));
  aoi21  g074(.a(new_n107_), .b(new_n102_), .c(x24), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(x10), .c(new_n34_), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n99_), .c(new_n83_), .O(z1));
  nand4  g077(.a(x11), .b(x06), .c(x03), .d(x01), .O(new_n111_));
  nor2   g078(.a(x24), .b(new_n100_), .O(new_n112_));
  nand3  g079(.a(new_n112_), .b(x20), .c(x14), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n111_), .c(new_n35_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n34_), .O(new_n115_));
  nand2  g082(.a(x24), .b(x18), .O(new_n116_));
  nand3  g083(.a(new_n43_), .b(x15), .c(x13), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g085(.a(x24), .b(x18), .c(x13), .O(new_n119_));
  nand3  g086(.a(x15), .b(new_n52_), .c(new_n51_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  aoi21  g088(.a(new_n118_), .b(x05), .c(new_n121_), .O(new_n122_));
  nor3   g089(.a(new_n122_), .b(new_n42_), .c(new_n41_), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(x11), .c(x06), .d(x03), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(new_n36_), .c(new_n115_), .O(z2));
  inv1   g092(.a(x08), .O(new_n126_));
  nand3  g093(.a(x15), .b(x13), .c(x05), .O(new_n127_));
  oai21  g094(.a(new_n100_), .b(x02), .c(new_n127_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(x20), .c(x14), .d(x11), .O(new_n129_));
  nor2   g096(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(x06), .c(x03), .d(x01), .O(new_n131_));
  inv1   g098(.a(x00), .O(new_n132_));
  nand3  g099(.a(x19), .b(x13), .c(x05), .O(new_n133_));
  oai21  g100(.a(x02), .b(new_n132_), .c(new_n133_), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(new_n42_), .c(new_n41_), .d(new_n40_), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(x08), .O(new_n136_));
  nand4  g103(.a(new_n136_), .b(new_n38_), .c(new_n37_), .d(new_n36_), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(new_n131_), .c(x24), .O(new_n138_));
  nand4  g105(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n139_));
  nand3  g106(.a(new_n86_), .b(x14), .c(x11), .O(new_n140_));
  nand3  g107(.a(new_n103_), .b(x07), .c(new_n38_), .O(new_n141_));
  nand4  g108(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n126_), .O(new_n142_));
  oai22  g109(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(new_n139_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n80_), .c(x24), .O(new_n144_));
  nand4  g111(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n145_));
  nand2  g112(.a(new_n103_), .b(new_n71_), .O(new_n146_));
  nand3  g113(.a(new_n76_), .b(new_n41_), .c(new_n40_), .O(new_n147_));
  oai22  g114(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(new_n139_), .O(new_n148_));
  nand3  g115(.a(new_n148_), .b(new_n52_), .c(new_n51_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n144_), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(new_n138_), .c(new_n35_), .O(new_n151_));
  nand2  g118(.a(new_n148_), .b(new_n82_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n144_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(x02), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n151_), .O(z3));
  oai21  g122(.a(x23), .b(new_n59_), .c(new_n94_), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n63_), .c(x09), .O(new_n157_));
  oai21  g124(.a(new_n157_), .b(x16), .c(new_n126_), .O(new_n158_));
  nand2  g125(.a(new_n49_), .b(x05), .O(new_n159_));
  nand3  g126(.a(new_n43_), .b(new_n34_), .c(x00), .O(new_n160_));
  nand4  g127(.a(new_n160_), .b(new_n53_), .c(new_n50_), .d(new_n159_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n158_), .O(new_n162_));
  oai21  g129(.a(new_n64_), .b(x04), .c(x17), .O(new_n163_));
  aoi21  g130(.a(new_n163_), .b(x22), .c(new_n69_), .O(new_n164_));
  oai21  g131(.a(new_n164_), .b(new_n91_), .c(x08), .O(new_n165_));
  nand2  g132(.a(new_n118_), .b(x05), .O(new_n166_));
  nand2  g133(.a(new_n112_), .b(new_n34_), .O(new_n167_));
  nand4  g134(.a(new_n167_), .b(new_n120_), .c(new_n119_), .d(new_n166_), .O(new_n168_));
  nor2   g135(.a(new_n35_), .b(x02), .O(new_n169_));
  aoi21  g136(.a(new_n168_), .b(new_n165_), .c(new_n169_), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n162_), .O(z4));
  aoi21  g138(.a(new_n43_), .b(x00), .c(x10), .O(new_n172_));
  oai21  g139(.a(new_n172_), .b(x02), .c(new_n55_), .O(z5));
  aoi21  g140(.a(x20), .b(new_n41_), .c(x06), .O(new_n174_));
  oai21  g141(.a(new_n174_), .b(x11), .c(new_n37_), .O(new_n175_));
  nand4  g142(.a(new_n43_), .b(new_n35_), .c(new_n34_), .d(x00), .O(new_n176_));
  oai21  g143(.a(new_n169_), .b(new_n55_), .c(new_n176_), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n175_), .O(new_n178_));
  aoi21  g145(.a(new_n42_), .b(x14), .c(new_n38_), .O(new_n179_));
  oai21  g146(.a(new_n179_), .b(new_n40_), .c(x03), .O(new_n180_));
  nand3  g147(.a(new_n112_), .b(new_n35_), .c(new_n34_), .O(new_n181_));
  oai21  g148(.a(new_n169_), .b(new_n122_), .c(new_n181_), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(new_n180_), .O(new_n183_));
  nand2  g150(.a(new_n183_), .b(new_n178_), .O(z6));
  oai21  g151(.a(new_n112_), .b(x10), .c(new_n34_), .O(new_n185_));
  nand2  g152(.a(new_n185_), .b(new_n122_), .O(z7));
endmodule


