// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n190_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  nand2  g004(.a(x17), .b(new_n37_), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(x13), .O(new_n45_));
  nand3  g012(.a(x19), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g014(.a(new_n47_), .b(new_n43_), .c(new_n38_), .O(new_n48_));
  inv1   g015(.a(x10), .O(new_n49_));
  nor2   g016(.a(x24), .b(x17), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n49_), .c(new_n37_), .d(x00), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n48_), .c(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(z0));
  nand4  g021(.a(x04), .b(x03), .c(x02), .d(x01), .O(new_n55_));
  nand4  g022(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  inv1   g026(.a(x17), .O(new_n60_));
  inv1   g027(.a(x20), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  and2   g029(.a(x23), .b(x22), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n59_), .d(new_n57_), .O(new_n64_));
  inv1   g031(.a(x09), .O(new_n65_));
  nor3   g032(.a(x04), .b(x03), .c(x01), .O(new_n66_));
  nor2   g033(.a(x08), .b(x06), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n35_), .d(new_n65_), .O(new_n68_));
  nor2   g035(.a(x14), .b(x12), .O(new_n69_));
  nor2   g036(.a(x17), .b(x16), .O(new_n70_));
  inv1   g037(.a(x19), .O(new_n71_));
  nor2   g038(.a(x20), .b(new_n71_), .O(new_n72_));
  nor2   g039(.a(x23), .b(x22), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n70_), .d(new_n69_), .O(new_n74_));
  oai21  g041(.a(new_n74_), .b(new_n68_), .c(new_n64_), .O(new_n75_));
  nand2  g042(.a(new_n45_), .b(new_n39_), .O(new_n76_));
  nand3  g043(.a(new_n41_), .b(x13), .c(x05), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand4  g046(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n80_));
  inv1   g047(.a(new_n80_), .O(new_n81_));
  inv1   g048(.a(x18), .O(new_n82_));
  nor2   g049(.a(new_n61_), .b(new_n82_), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n81_), .c(new_n63_), .d(new_n57_), .O(new_n84_));
  nor2   g051(.a(x09), .b(x08), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n66_), .c(x07), .d(new_n34_), .O(new_n86_));
  inv1   g053(.a(x12), .O(new_n87_));
  inv1   g054(.a(x16), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n36_), .c(new_n87_), .d(new_n35_), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n73_), .c(new_n61_), .d(new_n60_), .O(new_n91_));
  oai21  g058(.a(new_n91_), .b(new_n86_), .c(new_n84_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n76_), .c(x24), .O(new_n93_));
  inv1   g060(.a(x01), .O(new_n94_));
  inv1   g061(.a(x03), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n37_), .c(new_n94_), .d(x00), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  inv1   g064(.a(x08), .O(new_n98_));
  nand3  g065(.a(new_n49_), .b(new_n65_), .c(new_n98_), .O(new_n99_));
  nor3   g066(.a(new_n99_), .b(x06), .c(x04), .O(new_n100_));
  nand2  g067(.a(new_n61_), .b(new_n60_), .O(new_n101_));
  nor4   g068(.a(new_n101_), .b(x24), .c(x23), .d(x22), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n100_), .c(new_n97_), .d(new_n90_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n93_), .c(new_n79_), .O(z1));
  nand4  g071(.a(new_n49_), .b(x06), .c(x03), .d(x01), .O(new_n105_));
  nor2   g072(.a(new_n36_), .b(new_n35_), .O(new_n106_));
  nand2  g073(.a(new_n41_), .b(x21), .O(new_n107_));
  inv1   g074(.a(new_n107_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n106_), .c(x20), .O(new_n109_));
  oai21  g076(.a(new_n109_), .b(new_n105_), .c(new_n60_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n37_), .O(new_n111_));
  nand2  g078(.a(x24), .b(x18), .O(new_n112_));
  nand3  g079(.a(new_n41_), .b(x15), .c(x13), .O(new_n113_));
  aoi21  g080(.a(new_n113_), .b(new_n112_), .c(new_n39_), .O(new_n114_));
  nand3  g081(.a(x24), .b(x18), .c(x13), .O(new_n115_));
  nand3  g082(.a(x15), .b(new_n45_), .c(new_n39_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  nor3   g085(.a(new_n118_), .b(new_n61_), .c(new_n36_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(x11), .c(x06), .d(x03), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n94_), .c(new_n111_), .O(z2));
  nand3  g088(.a(x15), .b(x13), .c(x05), .O(new_n122_));
  nand3  g089(.a(x21), .b(new_n49_), .c(new_n37_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x20), .c(x14), .d(x11), .O(new_n125_));
  nor2   g092(.a(new_n125_), .b(new_n98_), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(x06), .c(x03), .d(x01), .O(new_n127_));
  nand3  g094(.a(new_n49_), .b(new_n37_), .c(x00), .O(new_n128_));
  nand3  g095(.a(x19), .b(x13), .c(x05), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(new_n61_), .c(new_n36_), .d(new_n35_), .O(new_n131_));
  nor2   g098(.a(new_n131_), .b(x08), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n34_), .c(new_n95_), .d(new_n94_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n127_), .c(x24), .O(new_n134_));
  nand4  g101(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n135_));
  nand2  g102(.a(new_n106_), .b(new_n83_), .O(new_n136_));
  nand4  g103(.a(x07), .b(new_n34_), .c(new_n95_), .d(new_n94_), .O(new_n137_));
  nand4  g104(.a(new_n61_), .b(new_n36_), .c(new_n35_), .d(new_n98_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n135_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n76_), .c(x24), .O(new_n140_));
  nand3  g107(.a(new_n106_), .b(x20), .c(x15), .O(new_n141_));
  nand3  g108(.a(new_n67_), .b(new_n95_), .c(new_n94_), .O(new_n142_));
  nand3  g109(.a(new_n72_), .b(new_n36_), .c(new_n35_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n135_), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n45_), .c(new_n39_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n140_), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n134_), .c(new_n60_), .O(new_n147_));
  nand2  g114(.a(new_n144_), .b(new_n78_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n140_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(x02), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n147_), .O(z3));
  nand2  g118(.a(new_n73_), .b(x04), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(new_n65_), .c(x16), .O(new_n153_));
  nand4  g120(.a(new_n41_), .b(new_n49_), .c(new_n37_), .d(x00), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n46_), .c(new_n44_), .O(new_n155_));
  or2    g122(.a(new_n155_), .b(new_n43_), .O(new_n156_));
  oai21  g123(.a(new_n153_), .b(x08), .c(new_n156_), .O(new_n157_));
  inv1   g124(.a(x22), .O(new_n158_));
  inv1   g125(.a(x04), .O(new_n159_));
  aoi21  g126(.a(x23), .b(new_n159_), .c(new_n60_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n158_), .c(x09), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(x16), .c(new_n98_), .O(new_n162_));
  nand4  g129(.a(new_n158_), .b(x17), .c(new_n88_), .d(x07), .O(new_n163_));
  oai21  g130(.a(new_n162_), .b(new_n82_), .c(new_n163_), .O(new_n164_));
  nand3  g131(.a(new_n164_), .b(new_n76_), .c(x24), .O(new_n165_));
  inv1   g132(.a(x15), .O(new_n166_));
  nand4  g133(.a(new_n158_), .b(x19), .c(x17), .d(new_n88_), .O(new_n167_));
  oai21  g134(.a(new_n162_), .b(new_n166_), .c(new_n167_), .O(new_n168_));
  nand2  g135(.a(new_n168_), .b(new_n78_), .O(new_n169_));
  oai21  g136(.a(x22), .b(new_n65_), .c(x16), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(x08), .O(new_n171_));
  nand4  g138(.a(new_n171_), .b(new_n41_), .c(x21), .d(new_n49_), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(new_n60_), .O(new_n173_));
  nand2  g140(.a(new_n173_), .b(new_n37_), .O(new_n174_));
  nand4  g141(.a(new_n174_), .b(new_n169_), .c(new_n165_), .d(new_n157_), .O(z4));
  nor2   g142(.a(new_n47_), .b(new_n43_), .O(new_n176_));
  nand3  g143(.a(new_n41_), .b(new_n49_), .c(x00), .O(new_n177_));
  nand2  g144(.a(new_n177_), .b(new_n60_), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(new_n37_), .O(new_n179_));
  nand2  g146(.a(new_n179_), .b(new_n176_), .O(z5));
  nand2  g147(.a(x20), .b(new_n36_), .O(new_n181_));
  aoi21  g148(.a(new_n181_), .b(new_n34_), .c(x11), .O(new_n182_));
  oai21  g149(.a(new_n182_), .b(x03), .c(new_n156_), .O(new_n183_));
  nand2  g150(.a(new_n61_), .b(x14), .O(new_n184_));
  aoi21  g151(.a(new_n184_), .b(x06), .c(new_n35_), .O(new_n185_));
  nand3  g152(.a(new_n108_), .b(new_n49_), .c(new_n37_), .O(new_n186_));
  nand3  g153(.a(new_n186_), .b(new_n116_), .c(new_n115_), .O(new_n187_));
  oai22  g154(.a(new_n187_), .b(new_n114_), .c(new_n185_), .d(new_n95_), .O(new_n188_));
  nand3  g155(.a(new_n188_), .b(new_n183_), .c(new_n38_), .O(z6));
  aoi21  g156(.a(new_n108_), .b(new_n49_), .c(x17), .O(new_n190_));
  oai21  g157(.a(new_n190_), .b(x02), .c(new_n118_), .O(z7));
endmodule


