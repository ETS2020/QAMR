// Benchmark "FAU" written by ABC on Fri Aug 14 02:35:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x13), .O(new_n47_));
  nand3  g014(.a(x19), .b(new_n47_), .c(new_n38_), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g016(.a(new_n49_), .b(new_n42_), .c(new_n37_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n52_));
  nand2  g019(.a(x20), .b(x02), .O(new_n53_));
  oai21  g020(.a(new_n52_), .b(x01), .c(new_n53_), .O(z0));
  inv1   g021(.a(x01), .O(new_n55_));
  nor2   g022(.a(x02), .b(new_n55_), .O(new_n56_));
  nand2  g023(.a(x04), .b(x03), .O(new_n57_));
  inv1   g024(.a(new_n57_), .O(new_n58_));
  nand4  g025(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(new_n61_));
  nand4  g028(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  inv1   g030(.a(x22), .O(new_n64_));
  inv1   g031(.a(x23), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n63_), .c(x20), .d(x17), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(new_n61_), .O(new_n68_));
  nor3   g035(.a(x04), .b(x03), .c(x01), .O(new_n69_));
  nor2   g036(.a(x08), .b(x06), .O(new_n70_));
  nor2   g037(.a(x11), .b(x09), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  inv1   g039(.a(x12), .O(new_n73_));
  inv1   g040(.a(x14), .O(new_n74_));
  inv1   g041(.a(x16), .O(new_n75_));
  inv1   g042(.a(x17), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  inv1   g044(.a(x19), .O(new_n78_));
  nor2   g045(.a(x20), .b(new_n78_), .O(new_n79_));
  nor2   g046(.a(x23), .b(x22), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nor3   g048(.a(new_n81_), .b(new_n77_), .c(new_n72_), .O(new_n82_));
  nand2  g049(.a(new_n47_), .b(new_n38_), .O(new_n83_));
  nand3  g050(.a(new_n40_), .b(x13), .c(x05), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g052(.a(new_n82_), .b(new_n68_), .c(new_n85_), .O(new_n86_));
  nand4  g053(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  inv1   g055(.a(x18), .O(new_n89_));
  nor2   g056(.a(new_n37_), .b(new_n89_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n88_), .c(new_n66_), .O(new_n91_));
  inv1   g058(.a(x07), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(x06), .O(new_n93_));
  nor2   g060(.a(x09), .b(x08), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n93_), .c(new_n69_), .O(new_n95_));
  nand4  g062(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n36_), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n80_), .c(new_n37_), .d(new_n76_), .O(new_n98_));
  oai22  g065(.a(new_n98_), .b(new_n95_), .c(new_n91_), .d(new_n61_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n83_), .c(x24), .O(new_n100_));
  nand3  g067(.a(new_n66_), .b(x21), .c(x20), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n87_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n60_), .c(new_n58_), .d(x01), .O(new_n103_));
  nor2   g070(.a(x03), .b(x01), .O(new_n104_));
  nor2   g071(.a(x06), .b(x04), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n104_), .c(new_n94_), .d(x00), .O(new_n106_));
  oai21  g073(.a(new_n106_), .b(new_n98_), .c(new_n103_), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n40_), .c(new_n45_), .d(new_n44_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n100_), .c(new_n86_), .O(z1));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  nand3  g077(.a(new_n40_), .b(x15), .c(x13), .O(new_n111_));
  aoi21  g078(.a(new_n111_), .b(new_n110_), .c(new_n38_), .O(new_n112_));
  nand3  g079(.a(x24), .b(x18), .c(x13), .O(new_n113_));
  nand3  g080(.a(x15), .b(new_n47_), .c(new_n38_), .O(new_n114_));
  nand3  g081(.a(new_n40_), .b(x21), .c(new_n45_), .O(new_n115_));
  nand3  g082(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  oai21  g083(.a(new_n116_), .b(new_n112_), .c(x14), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n36_), .O(new_n118_));
  nand4  g085(.a(new_n118_), .b(x06), .c(x03), .d(x01), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(new_n44_), .c(new_n37_), .O(z2));
  nand3  g087(.a(new_n56_), .b(x06), .c(x03), .O(new_n121_));
  inv1   g088(.a(x08), .O(new_n122_));
  nor2   g089(.a(new_n36_), .b(new_n122_), .O(new_n123_));
  inv1   g090(.a(x15), .O(new_n124_));
  nor2   g091(.a(new_n37_), .b(new_n124_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n123_), .c(x14), .O(new_n126_));
  nor2   g093(.a(x14), .b(x11), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(new_n104_), .c(new_n79_), .d(new_n70_), .O(new_n128_));
  oai21  g095(.a(new_n126_), .b(new_n121_), .c(new_n128_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n85_), .O(new_n130_));
  nand3  g097(.a(new_n123_), .b(new_n90_), .c(x14), .O(new_n131_));
  nand2  g098(.a(new_n104_), .b(new_n93_), .O(new_n132_));
  nand4  g099(.a(new_n37_), .b(new_n74_), .c(new_n36_), .d(new_n122_), .O(new_n133_));
  oai22  g100(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n121_), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n83_), .c(x24), .O(new_n135_));
  nand4  g102(.a(new_n35_), .b(new_n34_), .c(new_n55_), .d(x00), .O(new_n136_));
  nand4  g103(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n137_));
  nand4  g104(.a(x21), .b(x20), .c(x14), .d(x11), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n133_), .O(new_n139_));
  nand4  g106(.a(new_n139_), .b(new_n40_), .c(new_n45_), .d(new_n44_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n135_), .c(new_n130_), .O(z3));
  inv1   g108(.a(x04), .O(new_n142_));
  oai21  g109(.a(x23), .b(new_n142_), .c(new_n76_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n64_), .c(x09), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x16), .c(new_n122_), .O(new_n145_));
  nand2  g112(.a(new_n48_), .b(new_n43_), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n42_), .c(new_n53_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n46_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  inv1   g116(.a(x09), .O(new_n150_));
  oai21  g117(.a(new_n65_), .b(x04), .c(x17), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(x22), .c(new_n150_), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(new_n75_), .c(x08), .O(new_n153_));
  nand2  g120(.a(new_n114_), .b(new_n113_), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(new_n112_), .c(new_n53_), .O(new_n155_));
  nand4  g122(.a(new_n40_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n149_), .O(z4));
  inv1   g126(.a(new_n42_), .O(new_n160_));
  and2   g127(.a(new_n53_), .b(new_n48_), .O(new_n161_));
  nand4  g128(.a(new_n161_), .b(new_n46_), .c(new_n43_), .d(new_n160_), .O(z5));
  oai21  g129(.a(x11), .b(new_n35_), .c(new_n34_), .O(new_n163_));
  oai21  g130(.a(new_n49_), .b(new_n42_), .c(new_n163_), .O(new_n164_));
  oai21  g131(.a(x20), .b(new_n74_), .c(x06), .O(new_n165_));
  aoi21  g132(.a(new_n165_), .b(x11), .c(new_n34_), .O(new_n166_));
  nand4  g133(.a(x20), .b(new_n74_), .c(new_n36_), .d(x07), .O(new_n167_));
  oai21  g134(.a(new_n166_), .b(new_n89_), .c(new_n167_), .O(new_n168_));
  nand3  g135(.a(new_n168_), .b(new_n83_), .c(x24), .O(new_n169_));
  nand3  g136(.a(new_n127_), .b(x20), .c(x19), .O(new_n170_));
  oai21  g137(.a(new_n166_), .b(new_n124_), .c(new_n170_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n85_), .O(new_n172_));
  inv1   g139(.a(new_n53_), .O(new_n173_));
  xnor2a g140(.a(x13), .b(x05), .O(new_n174_));
  nand3  g141(.a(new_n174_), .b(x15), .c(x02), .O(new_n175_));
  nand4  g142(.a(x21), .b(new_n37_), .c(new_n45_), .d(new_n44_), .O(new_n176_));
  aoi21  g143(.a(new_n176_), .b(new_n175_), .c(new_n74_), .O(new_n177_));
  nand4  g144(.a(x21), .b(new_n45_), .c(new_n35_), .d(new_n44_), .O(new_n178_));
  inv1   g145(.a(new_n178_), .O(new_n179_));
  oai21  g146(.a(new_n179_), .b(new_n177_), .c(x11), .O(new_n180_));
  nand4  g147(.a(x20), .b(new_n74_), .c(new_n36_), .d(x00), .O(new_n181_));
  nand3  g148(.a(x21), .b(new_n34_), .c(new_n44_), .O(new_n182_));
  nand2  g149(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand2  g150(.a(new_n183_), .b(new_n45_), .O(new_n184_));
  nand2  g151(.a(new_n184_), .b(new_n180_), .O(new_n185_));
  aoi21  g152(.a(new_n185_), .b(new_n40_), .c(new_n173_), .O(new_n186_));
  nand4  g153(.a(new_n186_), .b(new_n172_), .c(new_n169_), .d(new_n164_), .O(z6));
  inv1   g154(.a(new_n112_), .O(new_n188_));
  inv1   g155(.a(new_n154_), .O(new_n189_));
  nand4  g156(.a(new_n156_), .b(new_n189_), .c(new_n188_), .d(new_n53_), .O(z7));
endmodule


