// Benchmark "FAU" written by ABC on Thu Aug 13 18:21:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n226_, new_n227_, new_n228_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n234_, new_n235_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n240_, new_n241_, new_n242_,
    new_n243_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n252_, new_n253_, new_n255_,
    new_n256_;
  inv1   g000(.a(x2), .O(new_n17_));
  inv1   g001(.a(x5), .O(new_n18_));
  inv1   g002(.a(x6), .O(new_n19_));
  nor2   g003(.a(x7), .b(new_n19_), .O(new_n20_));
  oai21  g004(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand2  g005(.a(new_n21_), .b(x4), .O(new_n22_));
  inv1   g006(.a(x7), .O(new_n23_));
  nand2  g007(.a(new_n23_), .b(x6), .O(new_n24_));
  nand2  g008(.a(new_n24_), .b(new_n18_), .O(new_n25_));
  nor2   g009(.a(x5), .b(x4), .O(new_n26_));
  aoi22  g010(.a(new_n26_), .b(new_n20_), .c(new_n25_), .d(new_n17_), .O(new_n27_));
  aoi21  g011(.a(new_n27_), .b(new_n22_), .c(x0), .O(new_n28_));
  inv1   g012(.a(x0), .O(new_n29_));
  oai21  g013(.a(new_n19_), .b(x4), .c(x7), .O(new_n30_));
  nand2  g014(.a(new_n30_), .b(x2), .O(new_n31_));
  nand2  g015(.a(x7), .b(new_n18_), .O(new_n32_));
  oai21  g016(.a(new_n32_), .b(x4), .c(x6), .O(new_n33_));
  nand2  g017(.a(new_n33_), .b(new_n17_), .O(new_n34_));
  aoi21  g018(.a(new_n34_), .b(new_n31_), .c(new_n29_), .O(new_n35_));
  oai21  g019(.a(new_n35_), .b(new_n28_), .c(x8), .O(new_n36_));
  inv1   g020(.a(x8), .O(new_n37_));
  nand3  g021(.a(x6), .b(new_n18_), .c(x2), .O(new_n38_));
  oai21  g022(.a(x6), .b(x2), .c(new_n38_), .O(new_n39_));
  nand3  g023(.a(new_n39_), .b(new_n23_), .c(x4), .O(new_n40_));
  nand2  g024(.a(new_n18_), .b(new_n17_), .O(new_n41_));
  oai21  g025(.a(x4), .b(new_n17_), .c(new_n41_), .O(new_n42_));
  nor2   g026(.a(new_n18_), .b(x4), .O(new_n43_));
  aoi22  g027(.a(new_n43_), .b(x2), .c(new_n42_), .d(new_n24_), .O(new_n44_));
  aoi21  g028(.a(new_n44_), .b(new_n40_), .c(x0), .O(new_n45_));
  inv1   g029(.a(x4), .O(new_n46_));
  aoi21  g030(.a(new_n19_), .b(x5), .c(x7), .O(new_n47_));
  nand2  g031(.a(x7), .b(new_n19_), .O(new_n48_));
  oai21  g032(.a(new_n47_), .b(new_n46_), .c(new_n48_), .O(new_n49_));
  nand2  g033(.a(new_n49_), .b(x2), .O(new_n50_));
  nor2   g034(.a(new_n23_), .b(x5), .O(new_n51_));
  aoi21  g035(.a(new_n23_), .b(x5), .c(x6), .O(new_n52_));
  oai22  g036(.a(new_n52_), .b(new_n46_), .c(new_n51_), .d(new_n19_), .O(new_n53_));
  nand2  g037(.a(new_n53_), .b(new_n17_), .O(new_n54_));
  aoi21  g038(.a(new_n54_), .b(new_n50_), .c(new_n29_), .O(new_n55_));
  oai21  g039(.a(new_n55_), .b(new_n45_), .c(new_n37_), .O(new_n56_));
  nand2  g040(.a(x4), .b(x2), .O(new_n57_));
  nor2   g041(.a(new_n57_), .b(x0), .O(new_n58_));
  nand4  g042(.a(new_n58_), .b(new_n23_), .c(new_n19_), .d(x5), .O(new_n59_));
  nand3  g043(.a(new_n59_), .b(new_n56_), .c(new_n36_), .O(new_n60_));
  nand2  g044(.a(new_n60_), .b(x9), .O(new_n61_));
  inv1   g045(.a(x9), .O(new_n62_));
  nand2  g046(.a(new_n19_), .b(x2), .O(new_n63_));
  nand2  g047(.a(new_n20_), .b(new_n17_), .O(new_n64_));
  aoi21  g048(.a(new_n64_), .b(new_n63_), .c(x0), .O(new_n65_));
  nand2  g049(.a(x6), .b(x0), .O(new_n66_));
  aoi21  g050(.a(new_n66_), .b(new_n23_), .c(new_n17_), .O(new_n67_));
  oai21  g051(.a(new_n67_), .b(new_n65_), .c(new_n46_), .O(new_n68_));
  aoi21  g052(.a(x4), .b(new_n17_), .c(x7), .O(new_n69_));
  nand2  g053(.a(x7), .b(x2), .O(new_n70_));
  oai21  g054(.a(new_n69_), .b(new_n19_), .c(new_n70_), .O(new_n71_));
  nand2  g055(.a(new_n71_), .b(x0), .O(new_n72_));
  nand2  g056(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nand3  g057(.a(new_n73_), .b(new_n62_), .c(x5), .O(new_n74_));
  nand2  g058(.a(new_n74_), .b(new_n61_), .O(z0));
  inv1   g059(.a(x1), .O(new_n76_));
  nand4  g060(.a(x8), .b(new_n23_), .c(x6), .d(new_n18_), .O(new_n77_));
  inv1   g061(.a(x3), .O(new_n78_));
  nand2  g062(.a(x9), .b(x5), .O(new_n79_));
  inv1   g063(.a(new_n79_), .O(new_n80_));
  nand3  g064(.a(new_n80_), .b(new_n78_), .c(x2), .O(new_n81_));
  aoi21  g065(.a(new_n81_), .b(new_n77_), .c(x0), .O(new_n82_));
  nand2  g066(.a(new_n23_), .b(new_n17_), .O(new_n83_));
  nand3  g067(.a(new_n83_), .b(new_n18_), .c(x0), .O(new_n84_));
  aoi21  g068(.a(new_n84_), .b(new_n81_), .c(new_n37_), .O(new_n85_));
  nand2  g069(.a(new_n17_), .b(x0), .O(new_n86_));
  nand2  g070(.a(new_n80_), .b(x3), .O(new_n87_));
  nand3  g071(.a(new_n62_), .b(new_n23_), .c(new_n78_), .O(new_n88_));
  oai21  g072(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(new_n89_));
  oai21  g073(.a(new_n89_), .b(new_n85_), .c(x6), .O(new_n90_));
  aoi21  g074(.a(new_n79_), .b(x6), .c(new_n29_), .O(new_n91_));
  nor2   g075(.a(new_n79_), .b(x2), .O(new_n92_));
  oai21  g076(.a(new_n92_), .b(new_n91_), .c(new_n37_), .O(new_n93_));
  oai21  g077(.a(x9), .b(x6), .c(new_n93_), .O(new_n94_));
  nand2  g078(.a(new_n94_), .b(x3), .O(new_n95_));
  nand2  g079(.a(new_n95_), .b(new_n90_), .O(new_n96_));
  oai21  g080(.a(new_n96_), .b(new_n82_), .c(new_n46_), .O(new_n97_));
  nand2  g081(.a(x4), .b(x3), .O(new_n98_));
  nand3  g082(.a(x6), .b(new_n78_), .c(x0), .O(new_n99_));
  oai21  g083(.a(new_n98_), .b(x0), .c(new_n99_), .O(new_n100_));
  nand2  g084(.a(new_n100_), .b(new_n83_), .O(new_n101_));
  nand3  g085(.a(new_n19_), .b(x4), .c(x3), .O(new_n102_));
  nand2  g086(.a(new_n78_), .b(new_n17_), .O(new_n103_));
  oai21  g087(.a(new_n103_), .b(new_n24_), .c(new_n102_), .O(new_n104_));
  nand2  g088(.a(new_n104_), .b(new_n29_), .O(new_n105_));
  nand2  g089(.a(new_n23_), .b(new_n19_), .O(new_n106_));
  aoi21  g090(.a(new_n23_), .b(x0), .c(new_n19_), .O(new_n107_));
  oai21  g091(.a(new_n107_), .b(x2), .c(new_n106_), .O(new_n108_));
  nand3  g092(.a(new_n108_), .b(x4), .c(x3), .O(new_n109_));
  nand3  g093(.a(new_n109_), .b(new_n105_), .c(new_n101_), .O(new_n110_));
  nand2  g094(.a(new_n110_), .b(new_n18_), .O(new_n111_));
  nand2  g095(.a(new_n46_), .b(x3), .O(new_n112_));
  oai21  g096(.a(x7), .b(new_n17_), .c(x0), .O(new_n113_));
  nor2   g097(.a(x6), .b(x3), .O(new_n114_));
  aoi22  g098(.a(new_n114_), .b(new_n17_), .c(new_n113_), .d(new_n112_), .O(new_n115_));
  nand4  g099(.a(new_n19_), .b(x4), .c(x3), .d(new_n17_), .O(new_n116_));
  oai21  g100(.a(new_n115_), .b(new_n18_), .c(new_n116_), .O(new_n117_));
  nand2  g101(.a(new_n117_), .b(x9), .O(new_n118_));
  nand2  g102(.a(new_n118_), .b(new_n111_), .O(new_n119_));
  nand2  g103(.a(new_n119_), .b(x8), .O(new_n120_));
  nand2  g104(.a(new_n24_), .b(new_n78_), .O(new_n121_));
  nor2   g105(.a(new_n121_), .b(new_n17_), .O(new_n122_));
  nand2  g106(.a(new_n17_), .b(new_n29_), .O(new_n123_));
  aoi21  g107(.a(new_n123_), .b(new_n24_), .c(new_n78_), .O(new_n124_));
  oai21  g108(.a(new_n124_), .b(new_n122_), .c(new_n37_), .O(new_n125_));
  nand2  g109(.a(x7), .b(x6), .O(new_n126_));
  inv1   g110(.a(new_n126_), .O(new_n127_));
  nand3  g111(.a(new_n127_), .b(new_n78_), .c(x0), .O(new_n128_));
  aoi21  g112(.a(new_n128_), .b(new_n125_), .c(new_n46_), .O(new_n129_));
  nand2  g113(.a(new_n70_), .b(x8), .O(new_n130_));
  nand4  g114(.a(new_n130_), .b(new_n19_), .c(new_n78_), .d(x0), .O(new_n131_));
  inv1   g115(.a(new_n131_), .O(new_n132_));
  oai21  g116(.a(new_n132_), .b(new_n129_), .c(new_n18_), .O(new_n133_));
  nand3  g117(.a(new_n133_), .b(new_n120_), .c(new_n97_), .O(new_n134_));
  nand2  g118(.a(new_n134_), .b(new_n76_), .O(new_n135_));
  oai21  g119(.a(new_n37_), .b(x2), .c(x0), .O(new_n136_));
  nand3  g120(.a(new_n37_), .b(x5), .c(new_n17_), .O(new_n137_));
  aoi21  g121(.a(new_n137_), .b(new_n136_), .c(x6), .O(new_n138_));
  nand3  g122(.a(x6), .b(new_n46_), .c(new_n29_), .O(new_n139_));
  oai21  g123(.a(x8), .b(new_n46_), .c(new_n139_), .O(new_n140_));
  nand2  g124(.a(new_n140_), .b(x2), .O(new_n141_));
  inv1   g125(.a(new_n26_), .O(new_n142_));
  nand2  g126(.a(x5), .b(x4), .O(new_n143_));
  nand2  g127(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand3  g128(.a(new_n144_), .b(new_n37_), .c(x6), .O(new_n145_));
  nand3  g129(.a(new_n145_), .b(new_n141_), .c(x9), .O(new_n146_));
  oai21  g130(.a(new_n146_), .b(new_n138_), .c(x7), .O(new_n147_));
  nand2  g131(.a(x9), .b(x8), .O(new_n148_));
  oai21  g132(.a(x6), .b(x5), .c(new_n148_), .O(new_n149_));
  nand2  g133(.a(new_n149_), .b(new_n57_), .O(new_n150_));
  oai21  g134(.a(x4), .b(new_n17_), .c(new_n37_), .O(new_n151_));
  nand3  g135(.a(new_n151_), .b(x9), .c(x5), .O(new_n152_));
  aoi21  g136(.a(new_n152_), .b(new_n150_), .c(x0), .O(new_n153_));
  oai21  g137(.a(x5), .b(x0), .c(x9), .O(new_n154_));
  nor3   g138(.a(new_n154_), .b(new_n37_), .c(new_n17_), .O(new_n155_));
  oai21  g139(.a(new_n155_), .b(new_n153_), .c(new_n23_), .O(new_n156_));
  aoi21  g140(.a(x5), .b(x2), .c(new_n29_), .O(new_n157_));
  oai21  g141(.a(new_n157_), .b(new_n37_), .c(x9), .O(new_n158_));
  nand3  g142(.a(new_n158_), .b(x6), .c(new_n46_), .O(new_n159_));
  nand3  g143(.a(new_n159_), .b(new_n156_), .c(new_n147_), .O(new_n160_));
  nand2  g144(.a(new_n160_), .b(x1), .O(new_n161_));
  oai21  g145(.a(new_n63_), .b(new_n29_), .c(x9), .O(new_n162_));
  nand3  g146(.a(new_n162_), .b(x7), .c(new_n46_), .O(new_n163_));
  nand2  g147(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  nand2  g148(.a(new_n164_), .b(x3), .O(new_n165_));
  nand3  g149(.a(new_n20_), .b(new_n18_), .c(x4), .O(new_n166_));
  nand3  g150(.a(x9), .b(new_n19_), .c(new_n46_), .O(new_n167_));
  aoi21  g151(.a(new_n167_), .b(new_n166_), .c(new_n17_), .O(new_n168_));
  aoi21  g152(.a(new_n148_), .b(new_n41_), .c(x6), .O(new_n169_));
  oai21  g153(.a(new_n169_), .b(new_n168_), .c(new_n78_), .O(new_n170_));
  nand2  g154(.a(new_n127_), .b(new_n26_), .O(new_n171_));
  nand2  g155(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand2  g156(.a(new_n172_), .b(new_n29_), .O(new_n173_));
  aoi21  g157(.a(x9), .b(new_n17_), .c(x4), .O(new_n174_));
  nor2   g158(.a(new_n174_), .b(new_n51_), .O(new_n175_));
  oai21  g159(.a(new_n23_), .b(x2), .c(new_n18_), .O(new_n176_));
  nand2  g160(.a(x9), .b(x0), .O(new_n177_));
  nand2  g161(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  oai21  g162(.a(new_n178_), .b(new_n175_), .c(new_n37_), .O(new_n179_));
  nand4  g163(.a(new_n32_), .b(x9), .c(new_n17_), .d(x0), .O(new_n180_));
  nand2  g164(.a(new_n62_), .b(x4), .O(new_n181_));
  and2   g165(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  aoi21  g166(.a(new_n182_), .b(new_n179_), .c(x3), .O(new_n183_));
  nand2  g167(.a(x4), .b(x0), .O(new_n184_));
  aoi21  g168(.a(new_n184_), .b(x9), .c(new_n23_), .O(new_n185_));
  oai21  g169(.a(new_n185_), .b(new_n183_), .c(x6), .O(new_n186_));
  nand2  g170(.a(new_n103_), .b(x7), .O(new_n187_));
  nand4  g171(.a(new_n187_), .b(x9), .c(x8), .d(new_n19_), .O(new_n188_));
  nand3  g172(.a(new_n188_), .b(new_n186_), .c(new_n173_), .O(new_n189_));
  nand3  g173(.a(x9), .b(new_n23_), .c(new_n19_), .O(new_n190_));
  nand2  g174(.a(new_n62_), .b(new_n18_), .O(new_n191_));
  oai21  g175(.a(new_n190_), .b(new_n143_), .c(new_n191_), .O(new_n192_));
  aoi21  g176(.a(new_n189_), .b(x1), .c(new_n192_), .O(new_n193_));
  nand3  g177(.a(new_n193_), .b(new_n165_), .c(new_n135_), .O(z1));
  nor2   g178(.a(x3), .b(x1), .O(new_n195_));
  nand2  g179(.a(x3), .b(x1), .O(new_n196_));
  inv1   g180(.a(new_n196_), .O(new_n197_));
  nor2   g181(.a(new_n197_), .b(new_n195_), .O(new_n198_));
  nand2  g182(.a(new_n198_), .b(new_n191_), .O(z2));
  nand2  g183(.a(new_n196_), .b(new_n191_), .O(z3));
  nand2  g184(.a(new_n24_), .b(x8), .O(new_n201_));
  nand3  g185(.a(new_n201_), .b(x3), .c(x1), .O(new_n202_));
  nand2  g186(.a(new_n195_), .b(new_n20_), .O(new_n203_));
  nand2  g187(.a(new_n203_), .b(new_n202_), .O(new_n204_));
  nand2  g188(.a(x2), .b(x0), .O(new_n205_));
  nand2  g189(.a(new_n205_), .b(new_n123_), .O(new_n206_));
  nand2  g190(.a(new_n206_), .b(new_n204_), .O(new_n207_));
  nand3  g191(.a(new_n37_), .b(x6), .c(new_n76_), .O(new_n208_));
  aoi21  g192(.a(new_n208_), .b(new_n48_), .c(new_n17_), .O(new_n209_));
  nand2  g193(.a(new_n37_), .b(new_n19_), .O(new_n210_));
  oai21  g194(.a(new_n126_), .b(x2), .c(new_n210_), .O(new_n211_));
  oai21  g195(.a(new_n211_), .b(new_n209_), .c(x0), .O(new_n212_));
  nor2   g196(.a(new_n19_), .b(new_n76_), .O(new_n213_));
  nor2   g197(.a(x8), .b(x2), .O(new_n214_));
  oai21  g198(.a(new_n214_), .b(new_n213_), .c(x7), .O(new_n215_));
  nand2  g199(.a(new_n215_), .b(new_n212_), .O(new_n216_));
  inv1   g200(.a(new_n210_), .O(new_n217_));
  nand2  g201(.a(x3), .b(x0), .O(new_n218_));
  aoi21  g202(.a(new_n218_), .b(x8), .c(new_n23_), .O(new_n219_));
  oai21  g203(.a(new_n219_), .b(new_n217_), .c(new_n17_), .O(new_n220_));
  nor2   g204(.a(new_n220_), .b(new_n76_), .O(new_n221_));
  aoi21  g205(.a(new_n216_), .b(new_n78_), .c(new_n221_), .O(new_n222_));
  aoi21  g206(.a(new_n222_), .b(new_n207_), .c(x4), .O(new_n223_));
  oai21  g207(.a(x3), .b(new_n76_), .c(x2), .O(new_n224_));
  aoi21  g208(.a(x3), .b(new_n29_), .c(new_n19_), .O(new_n225_));
  aoi21  g209(.a(new_n225_), .b(new_n224_), .c(x7), .O(new_n226_));
  oai21  g210(.a(new_n114_), .b(new_n76_), .c(new_n29_), .O(new_n227_));
  nand3  g211(.a(new_n196_), .b(new_n19_), .c(new_n17_), .O(new_n228_));
  nand2  g212(.a(new_n228_), .b(new_n227_), .O(new_n229_));
  oai21  g213(.a(new_n229_), .b(new_n226_), .c(x4), .O(new_n230_));
  oai21  g214(.a(new_n210_), .b(new_n103_), .c(new_n230_), .O(new_n231_));
  oai21  g215(.a(new_n231_), .b(new_n223_), .c(x5), .O(new_n232_));
  nand3  g216(.a(new_n197_), .b(x7), .c(new_n19_), .O(new_n233_));
  nand4  g217(.a(new_n37_), .b(x6), .c(new_n18_), .d(x0), .O(new_n234_));
  aoi21  g218(.a(new_n234_), .b(new_n233_), .c(x4), .O(new_n235_));
  inv1   g219(.a(new_n64_), .O(new_n236_));
  oai21  g220(.a(x6), .b(x2), .c(x7), .O(new_n237_));
  aoi22  g221(.a(new_n237_), .b(new_n210_), .c(new_n98_), .d(new_n76_), .O(new_n238_));
  oai21  g222(.a(new_n238_), .b(new_n236_), .c(x0), .O(new_n239_));
  nand2  g223(.a(new_n217_), .b(x2), .O(new_n240_));
  aoi21  g224(.a(new_n240_), .b(new_n126_), .c(new_n46_), .O(new_n241_));
  nand2  g225(.a(new_n20_), .b(new_n78_), .O(new_n242_));
  oai21  g226(.a(new_n48_), .b(new_n78_), .c(new_n242_), .O(new_n243_));
  oai21  g227(.a(new_n243_), .b(new_n241_), .c(x1), .O(new_n244_));
  nand2  g228(.a(new_n20_), .b(new_n29_), .O(new_n245_));
  nand2  g229(.a(new_n37_), .b(x3), .O(new_n246_));
  aoi21  g230(.a(new_n246_), .b(new_n245_), .c(new_n17_), .O(new_n247_));
  nand3  g231(.a(new_n20_), .b(x3), .c(new_n76_), .O(new_n248_));
  inv1   g232(.a(new_n248_), .O(new_n249_));
  oai21  g233(.a(new_n249_), .b(new_n247_), .c(x4), .O(new_n250_));
  nand3  g234(.a(new_n37_), .b(new_n23_), .c(x6), .O(new_n251_));
  nand4  g235(.a(new_n251_), .b(new_n250_), .c(new_n244_), .d(new_n239_), .O(new_n252_));
  aoi21  g236(.a(new_n252_), .b(new_n18_), .c(new_n235_), .O(new_n253_));
  aoi21  g237(.a(new_n253_), .b(new_n232_), .c(new_n62_), .O(z4));
  inv1   g238(.a(new_n198_), .O(new_n255_));
  nand3  g239(.a(new_n206_), .b(new_n255_), .c(new_n191_), .O(new_n256_));
  inv1   g240(.a(new_n256_), .O(z5));
endmodule


