// Benchmark "FAU" written by ABC on Thu Aug 13 18:21:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n212_, new_n213_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n226_, new_n227_, new_n228_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n234_, new_n235_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n240_, new_n241_, new_n242_,
    new_n243_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n252_, new_n253_, new_n254_,
    new_n255_, new_n256_, new_n257_, new_n258_, new_n259_, new_n260_,
    new_n261_, new_n262_, new_n263_, new_n264_, new_n265_, new_n266_,
    new_n267_, new_n268_, new_n269_, new_n270_, new_n271_, new_n272_,
    new_n273_, new_n274_, new_n275_, new_n276_, new_n277_, new_n278_,
    new_n280_, new_n281_;
  inv1   g000(.a(x8), .O(new_n17_));
  inv1   g001(.a(x0), .O(new_n18_));
  inv1   g002(.a(x2), .O(new_n19_));
  inv1   g003(.a(x4), .O(new_n20_));
  nand2  g004(.a(x5), .b(new_n20_), .O(new_n21_));
  inv1   g005(.a(x5), .O(new_n22_));
  inv1   g006(.a(x7), .O(new_n23_));
  nand2  g007(.a(new_n23_), .b(x6), .O(new_n24_));
  inv1   g008(.a(new_n24_), .O(new_n25_));
  nand3  g009(.a(new_n25_), .b(new_n22_), .c(x4), .O(new_n26_));
  aoi21  g010(.a(new_n26_), .b(new_n21_), .c(new_n19_), .O(new_n27_));
  nand2  g011(.a(new_n24_), .b(new_n22_), .O(new_n28_));
  inv1   g012(.a(x6), .O(new_n29_));
  nand3  g013(.a(new_n23_), .b(new_n29_), .c(x4), .O(new_n30_));
  aoi21  g014(.a(new_n30_), .b(new_n28_), .c(x2), .O(new_n31_));
  oai21  g015(.a(new_n31_), .b(new_n27_), .c(x9), .O(new_n32_));
  inv1   g016(.a(x9), .O(new_n33_));
  nand2  g017(.a(x6), .b(new_n20_), .O(new_n34_));
  nand2  g018(.a(new_n34_), .b(x5), .O(new_n35_));
  nand3  g019(.a(new_n35_), .b(new_n23_), .c(new_n19_), .O(new_n36_));
  nor2   g020(.a(new_n23_), .b(x6), .O(new_n37_));
  aoi21  g021(.a(new_n37_), .b(x2), .c(new_n20_), .O(new_n38_));
  oai21  g022(.a(new_n38_), .b(x5), .c(new_n36_), .O(new_n39_));
  nand2  g023(.a(new_n39_), .b(new_n33_), .O(new_n40_));
  nand3  g024(.a(new_n24_), .b(new_n20_), .c(x2), .O(new_n41_));
  nand3  g025(.a(new_n41_), .b(new_n40_), .c(new_n32_), .O(new_n42_));
  nand2  g026(.a(new_n42_), .b(new_n18_), .O(new_n43_));
  aoi21  g027(.a(new_n29_), .b(x5), .c(x7), .O(new_n44_));
  nor2   g028(.a(new_n44_), .b(new_n20_), .O(new_n45_));
  oai21  g029(.a(new_n45_), .b(new_n37_), .c(x9), .O(new_n46_));
  nand2  g030(.a(new_n34_), .b(new_n23_), .O(new_n47_));
  nand3  g031(.a(new_n47_), .b(new_n33_), .c(x5), .O(new_n48_));
  aoi21  g032(.a(new_n48_), .b(new_n46_), .c(new_n18_), .O(new_n49_));
  nor2   g033(.a(x9), .b(new_n23_), .O(new_n50_));
  nand2  g034(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  inv1   g035(.a(new_n51_), .O(new_n52_));
  oai21  g036(.a(new_n52_), .b(new_n49_), .c(x2), .O(new_n53_));
  nand3  g037(.a(x9), .b(new_n23_), .c(x5), .O(new_n54_));
  aoi21  g038(.a(new_n54_), .b(new_n29_), .c(new_n20_), .O(new_n55_));
  nor2   g039(.a(new_n23_), .b(x5), .O(new_n56_));
  inv1   g040(.a(new_n56_), .O(new_n57_));
  nand3  g041(.a(new_n57_), .b(x9), .c(x6), .O(new_n58_));
  nand2  g042(.a(new_n50_), .b(new_n22_), .O(new_n59_));
  nand2  g043(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  oai21  g044(.a(new_n60_), .b(new_n55_), .c(new_n19_), .O(new_n61_));
  nand2  g045(.a(new_n50_), .b(x6), .O(new_n62_));
  nand2  g046(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g047(.a(new_n63_), .b(x0), .O(new_n64_));
  nand4  g048(.a(new_n33_), .b(x6), .c(new_n22_), .d(new_n19_), .O(new_n65_));
  nand4  g049(.a(new_n65_), .b(new_n64_), .c(new_n53_), .d(new_n43_), .O(new_n66_));
  nand2  g050(.a(new_n66_), .b(new_n17_), .O(new_n67_));
  nand2  g051(.a(new_n24_), .b(x2), .O(new_n68_));
  aoi21  g052(.a(new_n68_), .b(new_n22_), .c(new_n20_), .O(new_n69_));
  nand2  g053(.a(new_n28_), .b(new_n19_), .O(new_n70_));
  nor2   g054(.a(x5), .b(x4), .O(new_n71_));
  nand2  g055(.a(new_n71_), .b(new_n25_), .O(new_n72_));
  nand2  g056(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g057(.a(new_n73_), .b(new_n69_), .c(new_n18_), .O(new_n74_));
  aoi21  g058(.a(new_n34_), .b(x7), .c(new_n19_), .O(new_n75_));
  nand2  g059(.a(new_n56_), .b(new_n20_), .O(new_n76_));
  aoi21  g060(.a(new_n76_), .b(x6), .c(x2), .O(new_n77_));
  oai21  g061(.a(new_n77_), .b(new_n75_), .c(x0), .O(new_n78_));
  aoi21  g062(.a(new_n78_), .b(new_n74_), .c(new_n17_), .O(new_n79_));
  nand2  g063(.a(x4), .b(x2), .O(new_n80_));
  nand3  g064(.a(new_n23_), .b(new_n29_), .c(x5), .O(new_n81_));
  nor3   g065(.a(new_n81_), .b(new_n80_), .c(x0), .O(new_n82_));
  oai21  g066(.a(new_n82_), .b(new_n79_), .c(x9), .O(new_n83_));
  nand2  g067(.a(new_n83_), .b(new_n67_), .O(z0));
  inv1   g068(.a(x3), .O(new_n85_));
  nand2  g069(.a(x8), .b(new_n19_), .O(new_n86_));
  nand3  g070(.a(new_n86_), .b(x9), .c(x0), .O(new_n87_));
  nand3  g071(.a(new_n17_), .b(x5), .c(new_n19_), .O(new_n88_));
  aoi21  g072(.a(new_n88_), .b(new_n87_), .c(x6), .O(new_n89_));
  nand3  g073(.a(x9), .b(new_n17_), .c(x4), .O(new_n90_));
  oai21  g074(.a(new_n34_), .b(x0), .c(new_n90_), .O(new_n91_));
  nand2  g075(.a(new_n91_), .b(x2), .O(new_n92_));
  nor2   g076(.a(x8), .b(new_n29_), .O(new_n93_));
  nand2  g077(.a(new_n93_), .b(x4), .O(new_n94_));
  nand2  g078(.a(new_n94_), .b(x9), .O(new_n95_));
  nand2  g079(.a(new_n95_), .b(x5), .O(new_n96_));
  nand2  g080(.a(new_n93_), .b(new_n71_), .O(new_n97_));
  nand3  g081(.a(new_n97_), .b(new_n96_), .c(new_n92_), .O(new_n98_));
  oai21  g082(.a(new_n98_), .b(new_n89_), .c(x7), .O(new_n99_));
  nand3  g083(.a(x9), .b(new_n29_), .c(new_n22_), .O(new_n100_));
  aoi22  g084(.a(new_n100_), .b(new_n17_), .c(x4), .d(x2), .O(new_n101_));
  nand3  g085(.a(x9), .b(new_n20_), .c(x2), .O(new_n102_));
  aoi21  g086(.a(new_n102_), .b(new_n17_), .c(new_n22_), .O(new_n103_));
  oai21  g087(.a(new_n103_), .b(new_n101_), .c(new_n18_), .O(new_n104_));
  nand2  g088(.a(new_n22_), .b(new_n18_), .O(new_n105_));
  nand3  g089(.a(new_n105_), .b(x8), .c(x2), .O(new_n106_));
  nand2  g090(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g091(.a(x8), .b(x2), .O(new_n108_));
  aoi21  g092(.a(new_n108_), .b(x9), .c(new_n22_), .O(new_n109_));
  nor2   g093(.a(new_n17_), .b(x0), .O(new_n110_));
  oai21  g094(.a(new_n110_), .b(new_n109_), .c(x6), .O(new_n111_));
  nor2   g095(.a(new_n111_), .b(x4), .O(new_n112_));
  aoi21  g096(.a(new_n107_), .b(new_n23_), .c(new_n112_), .O(new_n113_));
  aoi21  g097(.a(new_n113_), .b(new_n99_), .c(new_n85_), .O(new_n114_));
  nand3  g098(.a(new_n23_), .b(new_n22_), .c(new_n18_), .O(new_n115_));
  aoi21  g099(.a(new_n115_), .b(x8), .c(new_n19_), .O(new_n116_));
  oai21  g100(.a(new_n56_), .b(x8), .c(x9), .O(new_n117_));
  oai21  g101(.a(new_n117_), .b(new_n116_), .c(x4), .O(new_n118_));
  aoi21  g102(.a(x9), .b(new_n19_), .c(new_n22_), .O(new_n119_));
  oai22  g103(.a(new_n119_), .b(x4), .c(new_n33_), .d(new_n18_), .O(new_n120_));
  nand2  g104(.a(new_n120_), .b(new_n17_), .O(new_n121_));
  nand4  g105(.a(new_n57_), .b(x9), .c(new_n19_), .d(x0), .O(new_n122_));
  nor2   g106(.a(x9), .b(x5), .O(new_n123_));
  inv1   g107(.a(new_n123_), .O(new_n124_));
  nand4  g108(.a(new_n124_), .b(new_n122_), .c(new_n121_), .d(new_n118_), .O(new_n125_));
  nand3  g109(.a(x9), .b(new_n22_), .c(new_n18_), .O(new_n126_));
  aoi21  g110(.a(new_n126_), .b(new_n17_), .c(x2), .O(new_n127_));
  aoi21  g111(.a(new_n102_), .b(new_n17_), .c(x0), .O(new_n128_));
  oai21  g112(.a(new_n128_), .b(new_n127_), .c(new_n29_), .O(new_n129_));
  nand2  g113(.a(new_n129_), .b(new_n59_), .O(new_n130_));
  aoi21  g114(.a(new_n125_), .b(x6), .c(new_n130_), .O(new_n131_));
  aoi21  g115(.a(new_n71_), .b(new_n18_), .c(new_n33_), .O(new_n132_));
  oai21  g116(.a(new_n20_), .b(new_n18_), .c(new_n132_), .O(new_n133_));
  nand3  g117(.a(new_n133_), .b(x7), .c(x6), .O(new_n134_));
  inv1   g118(.a(new_n134_), .O(new_n135_));
  nor2   g119(.a(new_n17_), .b(x7), .O(new_n136_));
  aoi21  g120(.a(new_n136_), .b(new_n29_), .c(new_n135_), .O(new_n137_));
  oai21  g121(.a(new_n131_), .b(x3), .c(new_n137_), .O(new_n138_));
  oai21  g122(.a(new_n138_), .b(new_n114_), .c(x1), .O(new_n139_));
  inv1   g123(.a(x1), .O(new_n140_));
  nand3  g124(.a(new_n37_), .b(new_n20_), .c(x0), .O(new_n141_));
  nand3  g125(.a(x8), .b(x4), .c(new_n18_), .O(new_n142_));
  nand2  g126(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g127(.a(new_n143_), .b(x2), .O(new_n144_));
  aoi21  g128(.a(x6), .b(new_n19_), .c(new_n17_), .O(new_n145_));
  oai22  g129(.a(new_n145_), .b(new_n18_), .c(x8), .d(x2), .O(new_n146_));
  nand3  g130(.a(new_n146_), .b(x5), .c(new_n20_), .O(new_n147_));
  oai21  g131(.a(x2), .b(x0), .c(new_n24_), .O(new_n148_));
  nand4  g132(.a(new_n148_), .b(new_n17_), .c(new_n22_), .d(x4), .O(new_n149_));
  nand2  g133(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g134(.a(new_n150_), .b(x9), .O(new_n151_));
  nand2  g135(.a(x4), .b(new_n19_), .O(new_n152_));
  nand2  g136(.a(new_n136_), .b(new_n22_), .O(new_n153_));
  nor2   g137(.a(x8), .b(x6), .O(new_n154_));
  nand2  g138(.a(new_n154_), .b(new_n20_), .O(new_n155_));
  oai21  g139(.a(new_n153_), .b(new_n152_), .c(new_n155_), .O(new_n156_));
  nand2  g140(.a(new_n156_), .b(x0), .O(new_n157_));
  nand2  g141(.a(new_n29_), .b(new_n19_), .O(new_n158_));
  oai21  g142(.a(new_n25_), .b(x0), .c(new_n158_), .O(new_n159_));
  nand3  g143(.a(new_n159_), .b(x8), .c(x4), .O(new_n160_));
  nand2  g144(.a(new_n57_), .b(x4), .O(new_n161_));
  nand3  g145(.a(new_n161_), .b(new_n33_), .c(new_n29_), .O(new_n162_));
  nand3  g146(.a(new_n162_), .b(new_n160_), .c(new_n157_), .O(new_n163_));
  inv1   g147(.a(new_n163_), .O(new_n164_));
  nand3  g148(.a(new_n164_), .b(new_n151_), .c(new_n144_), .O(new_n165_));
  nand2  g149(.a(new_n165_), .b(x3), .O(new_n166_));
  nand2  g150(.a(new_n23_), .b(new_n19_), .O(new_n167_));
  nand3  g151(.a(new_n167_), .b(new_n22_), .c(x0), .O(new_n168_));
  nand3  g152(.a(new_n80_), .b(new_n23_), .c(new_n18_), .O(new_n169_));
  nand3  g153(.a(x5), .b(new_n20_), .c(x2), .O(new_n170_));
  nand3  g154(.a(new_n170_), .b(new_n169_), .c(new_n168_), .O(new_n171_));
  nand2  g155(.a(new_n171_), .b(x6), .O(new_n172_));
  nand2  g156(.a(new_n23_), .b(x2), .O(new_n173_));
  nand3  g157(.a(new_n173_), .b(new_n158_), .c(x0), .O(new_n174_));
  nand2  g158(.a(new_n174_), .b(x5), .O(new_n175_));
  nand2  g159(.a(new_n175_), .b(new_n172_), .O(new_n176_));
  nand2  g160(.a(new_n176_), .b(x8), .O(new_n177_));
  nand2  g161(.a(x7), .b(x2), .O(new_n178_));
  aoi21  g162(.a(new_n178_), .b(x8), .c(x6), .O(new_n179_));
  nand3  g163(.a(x7), .b(x6), .c(x4), .O(new_n180_));
  inv1   g164(.a(new_n180_), .O(new_n181_));
  oai21  g165(.a(new_n181_), .b(new_n179_), .c(x0), .O(new_n182_));
  nand4  g166(.a(new_n24_), .b(new_n17_), .c(x4), .d(x2), .O(new_n183_));
  nand2  g167(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  nand4  g168(.a(x5), .b(new_n20_), .c(x2), .d(new_n18_), .O(new_n185_));
  inv1   g169(.a(new_n185_), .O(new_n186_));
  aoi21  g170(.a(new_n184_), .b(new_n22_), .c(new_n186_), .O(new_n187_));
  aoi21  g171(.a(new_n187_), .b(new_n177_), .c(new_n33_), .O(new_n188_));
  oai22  g172(.a(new_n37_), .b(x5), .c(new_n24_), .d(x4), .O(new_n189_));
  nand3  g173(.a(new_n189_), .b(new_n33_), .c(new_n17_), .O(new_n190_));
  inv1   g174(.a(new_n190_), .O(new_n191_));
  oai21  g175(.a(new_n191_), .b(new_n188_), .c(new_n85_), .O(new_n192_));
  nand2  g176(.a(new_n167_), .b(x0), .O(new_n193_));
  oai21  g177(.a(x7), .b(x0), .c(new_n193_), .O(new_n194_));
  nand4  g178(.a(new_n194_), .b(x6), .c(new_n22_), .d(new_n20_), .O(new_n195_));
  nand2  g179(.a(new_n173_), .b(x0), .O(new_n196_));
  nand3  g180(.a(new_n196_), .b(x5), .c(x4), .O(new_n197_));
  nand2  g181(.a(new_n197_), .b(new_n195_), .O(new_n198_));
  aoi22  g182(.a(new_n198_), .b(x8), .c(new_n123_), .d(new_n20_), .O(new_n199_));
  nand3  g183(.a(new_n199_), .b(new_n192_), .c(new_n166_), .O(new_n200_));
  nand2  g184(.a(new_n200_), .b(new_n140_), .O(new_n201_));
  nor2   g185(.a(x6), .b(new_n20_), .O(new_n202_));
  nand2  g186(.a(new_n202_), .b(new_n136_), .O(new_n203_));
  aoi21  g187(.a(new_n203_), .b(new_n51_), .c(new_n85_), .O(new_n204_));
  nor2   g188(.a(x9), .b(new_n17_), .O(new_n205_));
  inv1   g189(.a(new_n205_), .O(new_n206_));
  nand2  g190(.a(x5), .b(x4), .O(new_n207_));
  nand3  g191(.a(x9), .b(new_n23_), .c(new_n29_), .O(new_n208_));
  oai21  g192(.a(new_n208_), .b(new_n207_), .c(new_n206_), .O(new_n209_));
  nor2   g193(.a(new_n209_), .b(new_n204_), .O(new_n210_));
  nand3  g194(.a(new_n210_), .b(new_n201_), .c(new_n139_), .O(z1));
  nand2  g195(.a(x3), .b(x1), .O(new_n212_));
  nand2  g196(.a(new_n85_), .b(new_n140_), .O(new_n213_));
  aoi21  g197(.a(new_n213_), .b(new_n212_), .c(new_n205_), .O(z2));
  nor3   g198(.a(new_n205_), .b(new_n85_), .c(new_n140_), .O(z3));
  nand2  g199(.a(new_n213_), .b(new_n212_), .O(new_n216_));
  nand4  g200(.a(new_n216_), .b(x6), .c(new_n20_), .d(x0), .O(new_n217_));
  oai21  g201(.a(x3), .b(new_n140_), .c(x4), .O(new_n218_));
  nand2  g202(.a(new_n218_), .b(new_n217_), .O(new_n219_));
  nand2  g203(.a(new_n219_), .b(x2), .O(new_n220_));
  nand3  g204(.a(new_n20_), .b(x3), .c(x1), .O(new_n221_));
  nand2  g205(.a(new_n221_), .b(new_n213_), .O(new_n222_));
  nand3  g206(.a(new_n222_), .b(x6), .c(new_n19_), .O(new_n223_));
  nand2  g207(.a(x4), .b(x3), .O(new_n224_));
  nand2  g208(.a(new_n224_), .b(new_n223_), .O(new_n225_));
  aoi21  g209(.a(new_n225_), .b(new_n18_), .c(new_n202_), .O(new_n226_));
  aoi21  g210(.a(new_n226_), .b(new_n220_), .c(x7), .O(new_n227_));
  nand3  g211(.a(new_n93_), .b(new_n85_), .c(new_n19_), .O(new_n228_));
  aoi21  g212(.a(new_n228_), .b(new_n20_), .c(x1), .O(new_n229_));
  nand2  g213(.a(new_n202_), .b(new_n85_), .O(new_n230_));
  inv1   g214(.a(new_n230_), .O(new_n231_));
  oai21  g215(.a(new_n231_), .b(new_n229_), .c(new_n18_), .O(new_n232_));
  nand3  g216(.a(new_n24_), .b(new_n17_), .c(x1), .O(new_n233_));
  nand4  g217(.a(x7), .b(x6), .c(new_n85_), .d(x0), .O(new_n234_));
  aoi21  g218(.a(new_n234_), .b(new_n233_), .c(x4), .O(new_n235_));
  oai21  g219(.a(new_n17_), .b(x4), .c(new_n85_), .O(new_n236_));
  nand2  g220(.a(x4), .b(new_n140_), .O(new_n237_));
  aoi21  g221(.a(new_n237_), .b(new_n236_), .c(x6), .O(new_n238_));
  oai21  g222(.a(new_n238_), .b(new_n235_), .c(new_n19_), .O(new_n239_));
  oai22  g223(.a(x8), .b(new_n18_), .c(new_n29_), .d(x3), .O(new_n240_));
  nand2  g224(.a(new_n240_), .b(x1), .O(new_n241_));
  oai21  g225(.a(x6), .b(new_n19_), .c(x8), .O(new_n242_));
  nand3  g226(.a(new_n242_), .b(new_n85_), .c(x0), .O(new_n243_));
  aoi21  g227(.a(new_n243_), .b(new_n241_), .c(new_n23_), .O(new_n244_));
  nand3  g228(.a(new_n154_), .b(new_n85_), .c(x0), .O(new_n245_));
  inv1   g229(.a(new_n245_), .O(new_n246_));
  oai21  g230(.a(new_n246_), .b(new_n244_), .c(new_n20_), .O(new_n247_));
  nand3  g231(.a(new_n247_), .b(new_n239_), .c(new_n232_), .O(new_n248_));
  oai21  g232(.a(new_n248_), .b(new_n227_), .c(x5), .O(new_n249_));
  inv1   g233(.a(new_n154_), .O(new_n250_));
  nand2  g234(.a(new_n158_), .b(x7), .O(new_n251_));
  aoi22  g235(.a(new_n251_), .b(new_n250_), .c(new_n224_), .d(new_n140_), .O(new_n252_));
  nand2  g236(.a(new_n17_), .b(new_n20_), .O(new_n253_));
  aoi21  g237(.a(new_n253_), .b(new_n167_), .c(new_n29_), .O(new_n254_));
  oai21  g238(.a(new_n254_), .b(new_n252_), .c(new_n22_), .O(new_n255_));
  nand2  g239(.a(x7), .b(x6), .O(new_n256_));
  oai21  g240(.a(new_n256_), .b(x2), .c(new_n250_), .O(new_n257_));
  nand3  g241(.a(new_n257_), .b(new_n20_), .c(x1), .O(new_n258_));
  nand2  g242(.a(new_n258_), .b(new_n255_), .O(new_n259_));
  nand2  g243(.a(new_n259_), .b(x0), .O(new_n260_));
  nand2  g244(.a(new_n154_), .b(x2), .O(new_n261_));
  aoi21  g245(.a(new_n261_), .b(new_n256_), .c(new_n20_), .O(new_n262_));
  nand2  g246(.a(new_n37_), .b(x3), .O(new_n263_));
  oai21  g247(.a(new_n24_), .b(x3), .c(new_n263_), .O(new_n264_));
  oai21  g248(.a(new_n264_), .b(new_n262_), .c(x1), .O(new_n265_));
  nand2  g249(.a(new_n25_), .b(new_n18_), .O(new_n266_));
  nand2  g250(.a(new_n17_), .b(x3), .O(new_n267_));
  aoi21  g251(.a(new_n267_), .b(new_n266_), .c(new_n19_), .O(new_n268_));
  nand3  g252(.a(new_n25_), .b(x3), .c(new_n140_), .O(new_n269_));
  inv1   g253(.a(new_n269_), .O(new_n270_));
  oai21  g254(.a(new_n270_), .b(new_n268_), .c(x4), .O(new_n271_));
  nand3  g255(.a(new_n17_), .b(new_n23_), .c(x6), .O(new_n272_));
  nand3  g256(.a(new_n272_), .b(new_n271_), .c(new_n265_), .O(new_n273_));
  nand2  g257(.a(new_n273_), .b(new_n22_), .O(new_n274_));
  inv1   g258(.a(new_n212_), .O(new_n275_));
  nand3  g259(.a(new_n275_), .b(new_n37_), .c(new_n20_), .O(new_n276_));
  nand3  g260(.a(new_n276_), .b(new_n274_), .c(new_n260_), .O(new_n277_));
  inv1   g261(.a(new_n277_), .O(new_n278_));
  aoi21  g262(.a(new_n278_), .b(new_n249_), .c(new_n33_), .O(z4));
  xnor2a g263(.a(x2), .b(x0), .O(new_n280_));
  nand3  g264(.a(new_n280_), .b(new_n216_), .c(new_n206_), .O(new_n281_));
  inv1   g265(.a(new_n281_), .O(z5));
endmodule


