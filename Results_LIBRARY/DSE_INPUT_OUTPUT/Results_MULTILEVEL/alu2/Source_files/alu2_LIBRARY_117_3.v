// Benchmark "FAU" written by ABC on Thu Aug 13 18:21:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n189_;
  inv1   g000(.a(x8), .O(new_n17_));
  inv1   g001(.a(x0), .O(new_n18_));
  inv1   g002(.a(x9), .O(new_n19_));
  inv1   g003(.a(x7), .O(new_n20_));
  inv1   g004(.a(x2), .O(new_n21_));
  inv1   g005(.a(x5), .O(new_n22_));
  nand3  g006(.a(x6), .b(new_n22_), .c(x4), .O(new_n23_));
  inv1   g007(.a(x6), .O(new_n24_));
  nand2  g008(.a(new_n24_), .b(x5), .O(new_n25_));
  aoi21  g009(.a(new_n25_), .b(new_n23_), .c(new_n21_), .O(new_n26_));
  nand3  g010(.a(new_n24_), .b(x4), .c(new_n21_), .O(new_n27_));
  inv1   g011(.a(new_n27_), .O(new_n28_));
  oai21  g012(.a(new_n28_), .b(new_n26_), .c(new_n20_), .O(new_n29_));
  nand2  g013(.a(new_n20_), .b(x6), .O(new_n30_));
  inv1   g014(.a(new_n30_), .O(new_n31_));
  nor2   g015(.a(new_n31_), .b(x5), .O(new_n32_));
  nor2   g016(.a(new_n22_), .b(x4), .O(new_n33_));
  nand2  g017(.a(new_n33_), .b(x2), .O(new_n34_));
  inv1   g018(.a(new_n34_), .O(new_n35_));
  aoi21  g019(.a(new_n32_), .b(new_n21_), .c(new_n35_), .O(new_n36_));
  aoi21  g020(.a(new_n36_), .b(new_n29_), .c(new_n19_), .O(new_n37_));
  inv1   g021(.a(x4), .O(new_n38_));
  nand2  g022(.a(new_n30_), .b(new_n38_), .O(new_n39_));
  nor2   g023(.a(new_n39_), .b(new_n21_), .O(new_n40_));
  oai21  g024(.a(new_n40_), .b(new_n37_), .c(new_n18_), .O(new_n41_));
  aoi21  g025(.a(new_n20_), .b(x4), .c(x6), .O(new_n42_));
  oai21  g026(.a(new_n42_), .b(new_n22_), .c(new_n30_), .O(new_n43_));
  nand2  g027(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  aoi21  g028(.a(new_n25_), .b(new_n20_), .c(new_n38_), .O(new_n45_));
  nor2   g029(.a(new_n20_), .b(x6), .O(new_n46_));
  oai21  g030(.a(new_n46_), .b(new_n45_), .c(x2), .O(new_n47_));
  aoi21  g031(.a(new_n47_), .b(new_n44_), .c(new_n19_), .O(new_n48_));
  nand2  g032(.a(x6), .b(x4), .O(new_n49_));
  nor2   g033(.a(new_n49_), .b(x2), .O(new_n50_));
  oai21  g034(.a(new_n50_), .b(new_n48_), .c(x0), .O(new_n51_));
  nand2  g035(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  nand2  g036(.a(new_n52_), .b(new_n17_), .O(new_n53_));
  nand2  g037(.a(x6), .b(new_n38_), .O(new_n54_));
  nand2  g038(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  nand3  g039(.a(new_n55_), .b(x5), .c(x2), .O(new_n56_));
  oai21  g040(.a(new_n20_), .b(x5), .c(new_n49_), .O(new_n57_));
  nand2  g041(.a(new_n57_), .b(new_n21_), .O(new_n58_));
  nand2  g042(.a(x7), .b(x6), .O(new_n59_));
  nand3  g043(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(new_n60_));
  nand2  g044(.a(new_n60_), .b(x0), .O(new_n61_));
  nor2   g045(.a(new_n24_), .b(x5), .O(new_n62_));
  inv1   g046(.a(new_n62_), .O(new_n63_));
  aoi21  g047(.a(new_n54_), .b(x5), .c(x7), .O(new_n64_));
  nand2  g048(.a(new_n64_), .b(new_n18_), .O(new_n65_));
  nand2  g049(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g050(.a(new_n66_), .b(new_n21_), .O(new_n67_));
  oai21  g051(.a(new_n20_), .b(x5), .c(x4), .O(new_n68_));
  nand3  g052(.a(new_n68_), .b(new_n24_), .c(x2), .O(new_n69_));
  nand2  g053(.a(new_n22_), .b(new_n38_), .O(new_n70_));
  nand2  g054(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g055(.a(new_n71_), .b(new_n18_), .O(new_n72_));
  nand3  g056(.a(x7), .b(new_n38_), .c(x2), .O(new_n73_));
  nand4  g057(.a(new_n73_), .b(new_n72_), .c(new_n67_), .d(new_n61_), .O(new_n74_));
  nand2  g058(.a(new_n74_), .b(new_n19_), .O(new_n75_));
  nand2  g059(.a(new_n75_), .b(new_n53_), .O(z0));
  aoi21  g060(.a(x6), .b(x5), .c(new_n18_), .O(new_n77_));
  nor2   g061(.a(new_n38_), .b(new_n21_), .O(new_n78_));
  oai21  g062(.a(new_n78_), .b(new_n77_), .c(x9), .O(new_n79_));
  inv1   g063(.a(new_n70_), .O(new_n80_));
  aoi21  g064(.a(x5), .b(x0), .c(new_n21_), .O(new_n81_));
  oai21  g065(.a(new_n81_), .b(new_n80_), .c(x6), .O(new_n82_));
  oai21  g066(.a(x6), .b(x2), .c(x9), .O(new_n83_));
  oai21  g067(.a(new_n83_), .b(x4), .c(x5), .O(new_n84_));
  nand3  g068(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(new_n85_));
  nand2  g069(.a(new_n85_), .b(x7), .O(new_n86_));
  nand2  g070(.a(x4), .b(new_n21_), .O(new_n87_));
  nand2  g071(.a(new_n87_), .b(new_n70_), .O(new_n88_));
  nand3  g072(.a(new_n88_), .b(new_n20_), .c(new_n18_), .O(new_n89_));
  nand2  g073(.a(x5), .b(x4), .O(new_n90_));
  aoi21  g074(.a(new_n90_), .b(new_n89_), .c(x6), .O(new_n91_));
  nand3  g075(.a(new_n20_), .b(x5), .c(new_n38_), .O(new_n92_));
  nor3   g076(.a(new_n92_), .b(new_n21_), .c(x0), .O(new_n93_));
  oai21  g077(.a(new_n93_), .b(new_n91_), .c(x9), .O(new_n94_));
  nand3  g078(.a(new_n33_), .b(new_n19_), .c(x6), .O(new_n95_));
  nand3  g079(.a(new_n95_), .b(new_n94_), .c(new_n86_), .O(new_n96_));
  nand2  g080(.a(new_n96_), .b(x1), .O(new_n97_));
  inv1   g081(.a(x1), .O(new_n98_));
  nor2   g082(.a(x9), .b(new_n20_), .O(new_n99_));
  nand2  g083(.a(x9), .b(x5), .O(new_n100_));
  aoi21  g084(.a(new_n100_), .b(x6), .c(new_n18_), .O(new_n101_));
  oai22  g085(.a(new_n100_), .b(x2), .c(x9), .d(x6), .O(new_n102_));
  oai21  g086(.a(new_n102_), .b(new_n101_), .c(new_n38_), .O(new_n103_));
  nand2  g087(.a(new_n21_), .b(new_n18_), .O(new_n104_));
  aoi21  g088(.a(new_n104_), .b(new_n30_), .c(new_n19_), .O(new_n105_));
  aoi22  g089(.a(new_n99_), .b(new_n24_), .c(new_n105_), .d(x4), .O(new_n106_));
  oai21  g090(.a(new_n106_), .b(x5), .c(new_n103_), .O(new_n107_));
  aoi22  g091(.a(new_n107_), .b(new_n98_), .c(new_n99_), .d(new_n38_), .O(new_n108_));
  nand2  g092(.a(new_n108_), .b(new_n97_), .O(new_n109_));
  nand2  g093(.a(new_n109_), .b(x3), .O(new_n110_));
  inv1   g094(.a(x3), .O(new_n111_));
  nand2  g095(.a(new_n98_), .b(new_n18_), .O(new_n112_));
  nand2  g096(.a(new_n38_), .b(x2), .O(new_n113_));
  nand3  g097(.a(x6), .b(new_n21_), .c(x1), .O(new_n114_));
  oai21  g098(.a(new_n113_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  nand2  g099(.a(new_n115_), .b(x5), .O(new_n116_));
  oai21  g100(.a(x5), .b(x2), .c(new_n113_), .O(new_n117_));
  nand3  g101(.a(new_n117_), .b(new_n24_), .c(new_n18_), .O(new_n118_));
  aoi21  g102(.a(x7), .b(x4), .c(x2), .O(new_n119_));
  oai21  g103(.a(new_n119_), .b(x0), .c(x6), .O(new_n120_));
  nand2  g104(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g105(.a(new_n121_), .b(x1), .O(new_n122_));
  aoi21  g106(.a(x7), .b(x4), .c(new_n24_), .O(new_n123_));
  nand3  g107(.a(new_n30_), .b(x4), .c(x2), .O(new_n124_));
  oai21  g108(.a(new_n123_), .b(new_n18_), .c(new_n124_), .O(new_n125_));
  nand3  g109(.a(new_n125_), .b(new_n22_), .c(new_n98_), .O(new_n126_));
  nand3  g110(.a(new_n126_), .b(new_n122_), .c(new_n116_), .O(new_n127_));
  nand2  g111(.a(new_n127_), .b(x9), .O(new_n128_));
  nand2  g112(.a(new_n57_), .b(x1), .O(new_n129_));
  nand2  g113(.a(new_n64_), .b(new_n98_), .O(new_n130_));
  nand3  g114(.a(new_n130_), .b(new_n129_), .c(new_n63_), .O(new_n131_));
  nor3   g115(.a(new_n33_), .b(new_n24_), .c(new_n21_), .O(new_n132_));
  aoi22  g116(.a(new_n132_), .b(x1), .c(new_n131_), .d(new_n19_), .O(new_n133_));
  nand2  g117(.a(new_n133_), .b(new_n128_), .O(new_n134_));
  nor2   g118(.a(x7), .b(x6), .O(new_n135_));
  inv1   g119(.a(new_n135_), .O(new_n136_));
  oai21  g120(.a(new_n136_), .b(new_n90_), .c(new_n17_), .O(new_n137_));
  nand2  g121(.a(new_n137_), .b(x9), .O(new_n138_));
  inv1   g122(.a(new_n59_), .O(new_n139_));
  nor2   g123(.a(new_n70_), .b(x1), .O(new_n140_));
  aoi21  g124(.a(new_n139_), .b(x1), .c(new_n140_), .O(new_n141_));
  oai21  g125(.a(new_n141_), .b(x9), .c(new_n138_), .O(new_n142_));
  aoi21  g126(.a(new_n134_), .b(new_n111_), .c(new_n142_), .O(new_n143_));
  nand2  g127(.a(new_n143_), .b(new_n110_), .O(z1));
  nand2  g128(.a(x9), .b(x8), .O(new_n145_));
  nor2   g129(.a(x3), .b(x1), .O(new_n146_));
  nor2   g130(.a(new_n111_), .b(new_n98_), .O(new_n147_));
  nor2   g131(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand2  g132(.a(new_n148_), .b(new_n145_), .O(z2));
  oai21  g133(.a(new_n111_), .b(new_n98_), .c(new_n145_), .O(z3));
  nand3  g134(.a(new_n38_), .b(x3), .c(x1), .O(new_n151_));
  nand2  g135(.a(new_n146_), .b(new_n31_), .O(new_n152_));
  aoi21  g136(.a(new_n152_), .b(new_n151_), .c(new_n21_), .O(new_n153_));
  aoi21  g137(.a(new_n136_), .b(new_n39_), .c(x3), .O(new_n154_));
  oai21  g138(.a(new_n154_), .b(new_n153_), .c(x0), .O(new_n155_));
  nand3  g139(.a(x3), .b(new_n21_), .c(new_n18_), .O(new_n156_));
  oai21  g140(.a(new_n59_), .b(x3), .c(new_n156_), .O(new_n157_));
  nand3  g141(.a(new_n157_), .b(new_n38_), .c(x1), .O(new_n158_));
  nor2   g142(.a(x6), .b(x2), .O(new_n159_));
  oai21  g143(.a(new_n159_), .b(new_n18_), .c(new_n98_), .O(new_n160_));
  nor2   g144(.a(x6), .b(x3), .O(new_n161_));
  nor2   g145(.a(x7), .b(new_n111_), .O(new_n162_));
  oai21  g146(.a(new_n162_), .b(new_n161_), .c(new_n18_), .O(new_n163_));
  nand2  g147(.a(new_n135_), .b(x3), .O(new_n164_));
  nand3  g148(.a(new_n164_), .b(new_n163_), .c(new_n160_), .O(new_n165_));
  nand2  g149(.a(new_n165_), .b(x4), .O(new_n166_));
  nand2  g150(.a(new_n112_), .b(x6), .O(new_n167_));
  nand3  g151(.a(new_n167_), .b(new_n111_), .c(new_n21_), .O(new_n168_));
  nand4  g152(.a(new_n168_), .b(new_n166_), .c(new_n158_), .d(new_n155_), .O(new_n169_));
  nand3  g153(.a(x4), .b(x3), .c(x2), .O(new_n170_));
  inv1   g154(.a(new_n170_), .O(new_n171_));
  oai21  g155(.a(new_n171_), .b(new_n62_), .c(new_n20_), .O(new_n172_));
  nand2  g156(.a(new_n104_), .b(x3), .O(new_n173_));
  oai21  g157(.a(new_n159_), .b(new_n98_), .c(new_n173_), .O(new_n174_));
  nand2  g158(.a(new_n174_), .b(x4), .O(new_n175_));
  nand2  g159(.a(new_n46_), .b(x3), .O(new_n176_));
  nand2  g160(.a(new_n176_), .b(new_n18_), .O(new_n177_));
  nand2  g161(.a(new_n177_), .b(x1), .O(new_n178_));
  nand2  g162(.a(x4), .b(new_n111_), .O(new_n179_));
  nand3  g163(.a(new_n179_), .b(x6), .c(x0), .O(new_n180_));
  nand3  g164(.a(new_n180_), .b(new_n178_), .c(new_n175_), .O(new_n181_));
  nand2  g165(.a(new_n181_), .b(new_n22_), .O(new_n182_));
  nand2  g166(.a(new_n30_), .b(x0), .O(new_n183_));
  aoi21  g167(.a(new_n183_), .b(new_n176_), .c(x4), .O(new_n184_));
  aoi21  g168(.a(new_n184_), .b(x1), .c(x8), .O(new_n185_));
  nand3  g169(.a(new_n185_), .b(new_n182_), .c(new_n172_), .O(new_n186_));
  aoi21  g170(.a(new_n169_), .b(x5), .c(new_n186_), .O(new_n187_));
  nor2   g171(.a(new_n187_), .b(new_n19_), .O(z4));
  xor2a  g172(.a(x2), .b(x0), .O(new_n189_));
  oai21  g173(.a(new_n189_), .b(new_n148_), .c(new_n145_), .O(z5));
endmodule


