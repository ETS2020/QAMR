// Benchmark "FAU" written by ABC on Thu Aug 13 18:22:29 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n186_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n192_, new_n193_, new_n194_;
  inv1   g000(.a(x0), .O(new_n17_));
  inv1   g001(.a(x8), .O(new_n18_));
  inv1   g002(.a(x4), .O(new_n19_));
  inv1   g003(.a(x5), .O(new_n20_));
  inv1   g004(.a(x6), .O(new_n21_));
  nor2   g005(.a(x7), .b(new_n21_), .O(new_n22_));
  inv1   g006(.a(new_n22_), .O(new_n23_));
  nand2  g007(.a(new_n23_), .b(x2), .O(new_n24_));
  aoi21  g008(.a(new_n24_), .b(new_n20_), .c(new_n19_), .O(new_n25_));
  aoi21  g009(.a(new_n23_), .b(new_n20_), .c(x2), .O(new_n26_));
  oai21  g010(.a(new_n26_), .b(new_n25_), .c(x9), .O(new_n27_));
  nand4  g011(.a(new_n22_), .b(new_n20_), .c(new_n19_), .d(x2), .O(new_n28_));
  aoi21  g012(.a(new_n28_), .b(new_n27_), .c(new_n18_), .O(new_n29_));
  inv1   g013(.a(x9), .O(new_n30_));
  nand2  g014(.a(x7), .b(new_n20_), .O(new_n31_));
  nand2  g015(.a(new_n31_), .b(x4), .O(new_n32_));
  nand3  g016(.a(new_n32_), .b(new_n21_), .c(x2), .O(new_n33_));
  inv1   g017(.a(x2), .O(new_n34_));
  inv1   g018(.a(x7), .O(new_n35_));
  nand2  g019(.a(x6), .b(new_n19_), .O(new_n36_));
  nand2  g020(.a(new_n36_), .b(x5), .O(new_n37_));
  nand3  g021(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nand2  g022(.a(new_n20_), .b(new_n19_), .O(new_n39_));
  nand3  g023(.a(new_n39_), .b(new_n38_), .c(new_n33_), .O(new_n40_));
  and2   g024(.a(new_n40_), .b(new_n30_), .O(new_n41_));
  oai21  g025(.a(new_n41_), .b(new_n29_), .c(new_n17_), .O(new_n42_));
  aoi21  g026(.a(new_n36_), .b(new_n35_), .c(new_n20_), .O(new_n43_));
  nand2  g027(.a(new_n43_), .b(x2), .O(new_n44_));
  oai21  g028(.a(new_n21_), .b(new_n19_), .c(new_n31_), .O(new_n45_));
  nand2  g029(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  nand2  g030(.a(x7), .b(x6), .O(new_n47_));
  nand3  g031(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n48_));
  nand2  g032(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  inv1   g033(.a(new_n31_), .O(new_n50_));
  aoi21  g034(.a(x6), .b(new_n19_), .c(new_n35_), .O(new_n51_));
  nand2  g035(.a(new_n21_), .b(new_n34_), .O(new_n52_));
  oai21  g036(.a(new_n51_), .b(new_n34_), .c(new_n52_), .O(new_n53_));
  nor2   g037(.a(x4), .b(x2), .O(new_n54_));
  aoi22  g038(.a(new_n54_), .b(new_n50_), .c(new_n53_), .d(x9), .O(new_n55_));
  oai21  g039(.a(new_n55_), .b(new_n18_), .c(new_n49_), .O(new_n56_));
  nand3  g040(.a(x6), .b(new_n20_), .c(new_n34_), .O(new_n57_));
  nand2  g041(.a(x7), .b(new_n19_), .O(new_n58_));
  inv1   g042(.a(new_n58_), .O(new_n59_));
  nand2  g043(.a(new_n59_), .b(x2), .O(new_n60_));
  aoi21  g044(.a(new_n60_), .b(new_n57_), .c(x9), .O(new_n61_));
  aoi21  g045(.a(new_n56_), .b(x0), .c(new_n61_), .O(new_n62_));
  nand2  g046(.a(new_n62_), .b(new_n42_), .O(z0));
  nand3  g047(.a(new_n35_), .b(x6), .c(new_n20_), .O(new_n64_));
  inv1   g048(.a(x3), .O(new_n65_));
  nand2  g049(.a(new_n65_), .b(x1), .O(new_n66_));
  inv1   g050(.a(x1), .O(new_n67_));
  nand3  g051(.a(x9), .b(x3), .c(new_n67_), .O(new_n68_));
  oai21  g052(.a(new_n66_), .b(new_n64_), .c(new_n68_), .O(new_n69_));
  nand2  g053(.a(new_n69_), .b(x2), .O(new_n70_));
  oai21  g054(.a(new_n22_), .b(new_n65_), .c(new_n20_), .O(new_n71_));
  nand3  g055(.a(new_n71_), .b(x9), .c(new_n67_), .O(new_n72_));
  aoi21  g056(.a(new_n72_), .b(new_n70_), .c(new_n19_), .O(new_n73_));
  oai21  g057(.a(x3), .b(x2), .c(x4), .O(new_n74_));
  nand4  g058(.a(new_n74_), .b(x6), .c(new_n20_), .d(new_n67_), .O(new_n75_));
  nand3  g059(.a(new_n20_), .b(x4), .c(x2), .O(new_n76_));
  nand4  g060(.a(new_n76_), .b(x9), .c(x3), .d(x1), .O(new_n77_));
  nand2  g061(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g062(.a(new_n78_), .b(new_n35_), .O(new_n79_));
  nand2  g063(.a(x5), .b(new_n67_), .O(new_n80_));
  oai21  g064(.a(x6), .b(new_n67_), .c(new_n80_), .O(new_n81_));
  nand3  g065(.a(new_n81_), .b(x9), .c(new_n65_), .O(new_n82_));
  aoi21  g066(.a(x5), .b(new_n65_), .c(new_n35_), .O(new_n83_));
  nand4  g067(.a(new_n83_), .b(x6), .c(new_n19_), .d(x1), .O(new_n84_));
  nand3  g068(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(new_n85_));
  oai21  g069(.a(new_n85_), .b(new_n73_), .c(new_n17_), .O(new_n86_));
  nand3  g070(.a(x3), .b(new_n34_), .c(new_n67_), .O(new_n87_));
  nand3  g071(.a(x9), .b(new_n35_), .c(new_n20_), .O(new_n88_));
  oai22  g072(.a(new_n88_), .b(new_n87_), .c(new_n47_), .d(new_n67_), .O(new_n89_));
  nand2  g073(.a(new_n89_), .b(x4), .O(new_n90_));
  and2   g074(.a(x6), .b(x5), .O(new_n91_));
  nand4  g075(.a(new_n91_), .b(new_n19_), .c(x3), .d(new_n34_), .O(new_n92_));
  nand4  g076(.a(x7), .b(new_n20_), .c(new_n65_), .d(x2), .O(new_n93_));
  aoi21  g077(.a(new_n93_), .b(new_n92_), .c(x1), .O(new_n94_));
  nand3  g078(.a(new_n47_), .b(x3), .c(x2), .O(new_n95_));
  nand3  g079(.a(new_n31_), .b(new_n65_), .c(new_n34_), .O(new_n96_));
  aoi21  g080(.a(new_n96_), .b(new_n95_), .c(new_n67_), .O(new_n97_));
  oai21  g081(.a(new_n97_), .b(new_n94_), .c(x9), .O(new_n98_));
  nand2  g082(.a(x4), .b(x3), .O(new_n99_));
  nand2  g083(.a(new_n35_), .b(new_n34_), .O(new_n100_));
  nand4  g084(.a(new_n100_), .b(new_n99_), .c(x6), .d(new_n20_), .O(new_n101_));
  nor2   g085(.a(new_n65_), .b(new_n34_), .O(new_n102_));
  nor2   g086(.a(new_n35_), .b(x6), .O(new_n103_));
  nand3  g087(.a(new_n103_), .b(new_n102_), .c(new_n19_), .O(new_n104_));
  nand2  g088(.a(new_n104_), .b(new_n101_), .O(new_n105_));
  nand2  g089(.a(new_n105_), .b(new_n67_), .O(new_n106_));
  nand3  g090(.a(new_n106_), .b(new_n98_), .c(new_n90_), .O(new_n107_));
  nand2  g091(.a(new_n107_), .b(x0), .O(new_n108_));
  oai22  g092(.a(new_n47_), .b(x4), .c(new_n30_), .d(x7), .O(new_n109_));
  nand3  g093(.a(new_n109_), .b(x3), .c(x1), .O(new_n110_));
  nand2  g094(.a(new_n35_), .b(x4), .O(new_n111_));
  oai21  g095(.a(new_n51_), .b(x3), .c(new_n111_), .O(new_n112_));
  nand3  g096(.a(new_n112_), .b(x9), .c(new_n67_), .O(new_n113_));
  aoi21  g097(.a(new_n113_), .b(new_n110_), .c(new_n34_), .O(new_n114_));
  nand2  g098(.a(new_n34_), .b(new_n67_), .O(new_n115_));
  nor4   g099(.a(new_n115_), .b(new_n30_), .c(x6), .d(x3), .O(new_n116_));
  oai21  g100(.a(new_n116_), .b(new_n114_), .c(x5), .O(new_n117_));
  aoi21  g101(.a(new_n65_), .b(new_n34_), .c(new_n35_), .O(new_n118_));
  nand2  g102(.a(new_n115_), .b(x7), .O(new_n119_));
  nand3  g103(.a(new_n119_), .b(x4), .c(x3), .O(new_n120_));
  oai21  g104(.a(new_n118_), .b(new_n67_), .c(new_n120_), .O(new_n121_));
  nand3  g105(.a(new_n121_), .b(x9), .c(new_n21_), .O(new_n122_));
  nand4  g106(.a(new_n122_), .b(new_n117_), .c(new_n108_), .d(new_n86_), .O(new_n123_));
  nand2  g107(.a(new_n123_), .b(x8), .O(new_n124_));
  nand2  g108(.a(new_n43_), .b(x3), .O(new_n125_));
  nand2  g109(.a(new_n45_), .b(new_n65_), .O(new_n126_));
  nand3  g110(.a(new_n126_), .b(new_n125_), .c(new_n47_), .O(new_n127_));
  nand2  g111(.a(new_n127_), .b(x1), .O(new_n128_));
  nand3  g112(.a(new_n37_), .b(new_n35_), .c(new_n67_), .O(new_n129_));
  oai21  g113(.a(new_n21_), .b(x5), .c(new_n129_), .O(new_n130_));
  nand2  g114(.a(new_n130_), .b(new_n65_), .O(new_n131_));
  nand3  g115(.a(new_n32_), .b(new_n21_), .c(x3), .O(new_n132_));
  nand2  g116(.a(new_n132_), .b(new_n39_), .O(new_n133_));
  nand2  g117(.a(new_n133_), .b(new_n67_), .O(new_n134_));
  nand2  g118(.a(new_n59_), .b(x3), .O(new_n135_));
  nand4  g119(.a(new_n135_), .b(new_n134_), .c(new_n131_), .d(new_n128_), .O(new_n136_));
  nand2  g120(.a(new_n136_), .b(new_n30_), .O(new_n137_));
  nand2  g121(.a(new_n137_), .b(new_n124_), .O(z1));
  xor2a  g122(.a(x3), .b(x1), .O(new_n139_));
  aoi21  g123(.a(x9), .b(new_n18_), .c(new_n139_), .O(z2));
  nand2  g124(.a(x9), .b(new_n18_), .O(new_n141_));
  nand2  g125(.a(x3), .b(x1), .O(new_n142_));
  nand2  g126(.a(new_n142_), .b(new_n141_), .O(z3));
  nand2  g127(.a(new_n65_), .b(x2), .O(new_n144_));
  nand3  g128(.a(new_n20_), .b(x4), .c(x3), .O(new_n145_));
  nand3  g129(.a(x8), .b(new_n35_), .c(x5), .O(new_n146_));
  oai21  g130(.a(new_n146_), .b(new_n144_), .c(new_n145_), .O(new_n147_));
  nand2  g131(.a(new_n147_), .b(new_n67_), .O(new_n148_));
  aoi21  g132(.a(new_n58_), .b(x5), .c(new_n67_), .O(new_n149_));
  nand4  g133(.a(x7), .b(x5), .c(new_n19_), .d(new_n65_), .O(new_n150_));
  oai21  g134(.a(x7), .b(x5), .c(new_n150_), .O(new_n151_));
  oai21  g135(.a(new_n151_), .b(new_n149_), .c(new_n34_), .O(new_n152_));
  inv1   g136(.a(new_n102_), .O(new_n153_));
  oai21  g137(.a(new_n146_), .b(new_n153_), .c(new_n31_), .O(new_n154_));
  nand2  g138(.a(new_n154_), .b(x1), .O(new_n155_));
  nand3  g139(.a(new_n155_), .b(new_n152_), .c(new_n148_), .O(new_n156_));
  nand2  g140(.a(new_n156_), .b(x0), .O(new_n157_));
  nand2  g141(.a(new_n34_), .b(new_n17_), .O(new_n158_));
  nand3  g142(.a(x8), .b(x5), .c(x3), .O(new_n159_));
  oai22  g143(.a(new_n159_), .b(new_n158_), .c(x5), .d(x3), .O(new_n160_));
  nand2  g144(.a(new_n160_), .b(x1), .O(new_n161_));
  nand3  g145(.a(x8), .b(x5), .c(new_n65_), .O(new_n162_));
  oai21  g146(.a(new_n162_), .b(new_n115_), .c(new_n76_), .O(new_n163_));
  nand2  g147(.a(new_n163_), .b(new_n17_), .O(new_n164_));
  nand4  g148(.a(new_n20_), .b(x4), .c(x3), .d(new_n67_), .O(new_n165_));
  nand3  g149(.a(new_n165_), .b(new_n164_), .c(new_n161_), .O(new_n166_));
  oai21  g150(.a(new_n34_), .b(x0), .c(new_n35_), .O(new_n167_));
  nand3  g151(.a(new_n167_), .b(new_n20_), .c(x4), .O(new_n168_));
  aoi21  g152(.a(new_n168_), .b(new_n150_), .c(new_n67_), .O(new_n169_));
  aoi21  g153(.a(new_n166_), .b(new_n35_), .c(new_n169_), .O(new_n170_));
  nand2  g154(.a(new_n170_), .b(new_n157_), .O(new_n171_));
  nand2  g155(.a(new_n171_), .b(x6), .O(new_n172_));
  nand2  g156(.a(new_n66_), .b(x2), .O(new_n173_));
  aoi21  g157(.a(x3), .b(new_n17_), .c(new_n21_), .O(new_n174_));
  aoi21  g158(.a(new_n174_), .b(new_n173_), .c(x7), .O(new_n175_));
  aoi21  g159(.a(new_n21_), .b(new_n65_), .c(new_n67_), .O(new_n176_));
  nand3  g160(.a(new_n142_), .b(new_n21_), .c(new_n34_), .O(new_n177_));
  oai21  g161(.a(new_n176_), .b(x0), .c(new_n177_), .O(new_n178_));
  oai21  g162(.a(new_n178_), .b(new_n175_), .c(x5), .O(new_n179_));
  nand4  g163(.a(new_n103_), .b(new_n102_), .c(new_n20_), .d(x0), .O(new_n180_));
  nand2  g164(.a(new_n180_), .b(new_n179_), .O(new_n181_));
  nand2  g165(.a(new_n20_), .b(x1), .O(new_n182_));
  nand4  g166(.a(new_n21_), .b(x5), .c(new_n19_), .d(new_n65_), .O(new_n183_));
  aoi21  g167(.a(new_n183_), .b(new_n182_), .c(new_n34_), .O(new_n184_));
  nand2  g168(.a(x5), .b(x4), .O(new_n185_));
  nand4  g169(.a(new_n185_), .b(new_n21_), .c(x3), .d(x1), .O(new_n186_));
  inv1   g170(.a(new_n186_), .O(new_n187_));
  aoi21  g171(.a(new_n184_), .b(x0), .c(new_n187_), .O(new_n188_));
  oai21  g172(.a(new_n188_), .b(new_n35_), .c(x8), .O(new_n189_));
  aoi21  g173(.a(new_n181_), .b(x4), .c(new_n189_), .O(new_n190_));
  aoi21  g174(.a(new_n190_), .b(new_n172_), .c(new_n30_), .O(z4));
  nand2  g175(.a(x2), .b(x0), .O(new_n192_));
  aoi21  g176(.a(new_n192_), .b(new_n158_), .c(new_n139_), .O(new_n193_));
  nand2  g177(.a(new_n193_), .b(new_n141_), .O(new_n194_));
  inv1   g178(.a(new_n194_), .O(z5));
endmodule


