// Benchmark "FAU" written by ABC on Thu Aug 13 18:21:09 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n193_, new_n196_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n226_, new_n227_, new_n228_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n234_, new_n235_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n240_, new_n241_, new_n242_,
    new_n243_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n252_, new_n253_, new_n254_,
    new_n255_, new_n256_, new_n257_, new_n258_, new_n259_, new_n260_,
    new_n261_, new_n262_, new_n264_, new_n265_;
  inv1   g000(.a(x2), .O(new_n17_));
  inv1   g001(.a(x5), .O(new_n18_));
  nand2  g002(.a(x8), .b(x7), .O(new_n19_));
  oai21  g003(.a(new_n19_), .b(x4), .c(x9), .O(new_n20_));
  nand2  g004(.a(new_n20_), .b(x0), .O(new_n21_));
  inv1   g005(.a(x0), .O(new_n22_));
  inv1   g006(.a(x8), .O(new_n23_));
  inv1   g007(.a(x9), .O(new_n24_));
  inv1   g008(.a(x6), .O(new_n25_));
  nor2   g009(.a(x7), .b(new_n25_), .O(new_n26_));
  nor2   g010(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  nand3  g011(.a(new_n27_), .b(new_n23_), .c(new_n22_), .O(new_n28_));
  nand2  g012(.a(new_n24_), .b(x6), .O(new_n29_));
  nand3  g013(.a(new_n29_), .b(new_n28_), .c(new_n21_), .O(new_n30_));
  nand2  g014(.a(new_n30_), .b(new_n18_), .O(new_n31_));
  inv1   g015(.a(x7), .O(new_n32_));
  nand4  g016(.a(new_n23_), .b(new_n32_), .c(x4), .d(new_n22_), .O(new_n33_));
  nor2   g017(.a(new_n24_), .b(new_n23_), .O(new_n34_));
  nand2  g018(.a(new_n34_), .b(x0), .O(new_n35_));
  nand2  g019(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g020(.a(new_n36_), .b(new_n25_), .O(new_n37_));
  nand2  g021(.a(new_n32_), .b(x6), .O(new_n38_));
  oai21  g022(.a(new_n24_), .b(new_n18_), .c(new_n38_), .O(new_n39_));
  nand3  g023(.a(new_n39_), .b(x8), .c(new_n22_), .O(new_n40_));
  inv1   g024(.a(x4), .O(new_n41_));
  aoi21  g025(.a(new_n32_), .b(x5), .c(x6), .O(new_n42_));
  nand2  g026(.a(x7), .b(new_n18_), .O(new_n43_));
  nand2  g027(.a(new_n43_), .b(x6), .O(new_n44_));
  oai21  g028(.a(new_n42_), .b(new_n41_), .c(new_n44_), .O(new_n45_));
  nand3  g029(.a(new_n45_), .b(new_n23_), .c(x0), .O(new_n46_));
  nand4  g030(.a(new_n46_), .b(new_n40_), .c(new_n37_), .d(new_n31_), .O(new_n47_));
  nand2  g031(.a(new_n47_), .b(new_n17_), .O(new_n48_));
  nand2  g032(.a(new_n32_), .b(new_n25_), .O(new_n49_));
  aoi21  g033(.a(new_n49_), .b(x4), .c(new_n18_), .O(new_n50_));
  nand3  g034(.a(new_n26_), .b(new_n18_), .c(x4), .O(new_n51_));
  oai21  g035(.a(new_n26_), .b(x4), .c(new_n51_), .O(new_n52_));
  oai21  g036(.a(new_n52_), .b(new_n50_), .c(new_n23_), .O(new_n53_));
  nand3  g037(.a(new_n27_), .b(x8), .c(x4), .O(new_n54_));
  nand3  g038(.a(new_n24_), .b(new_n25_), .c(new_n18_), .O(new_n55_));
  nand3  g039(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g040(.a(new_n56_), .b(new_n22_), .O(new_n57_));
  nand2  g041(.a(x9), .b(x7), .O(new_n58_));
  oai21  g042(.a(x6), .b(new_n18_), .c(new_n58_), .O(new_n59_));
  nor2   g043(.a(new_n58_), .b(x6), .O(new_n60_));
  aoi21  g044(.a(new_n59_), .b(x4), .c(new_n60_), .O(new_n61_));
  oai21  g045(.a(new_n25_), .b(x4), .c(x7), .O(new_n62_));
  aoi22  g046(.a(new_n62_), .b(x8), .c(new_n24_), .d(x5), .O(new_n63_));
  oai21  g047(.a(new_n61_), .b(x8), .c(new_n63_), .O(new_n64_));
  nand2  g048(.a(new_n64_), .b(x0), .O(new_n65_));
  nand2  g049(.a(new_n24_), .b(new_n41_), .O(new_n66_));
  nand3  g050(.a(new_n66_), .b(new_n65_), .c(new_n57_), .O(new_n67_));
  nand2  g051(.a(new_n67_), .b(x2), .O(new_n68_));
  nor2   g052(.a(new_n23_), .b(x7), .O(new_n69_));
  nand2  g053(.a(new_n69_), .b(x6), .O(new_n70_));
  nand2  g054(.a(new_n70_), .b(x9), .O(new_n71_));
  nand3  g055(.a(new_n71_), .b(new_n18_), .c(new_n41_), .O(new_n72_));
  nand3  g056(.a(new_n34_), .b(x5), .c(x4), .O(new_n73_));
  nand2  g057(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g058(.a(x6), .b(x0), .O(new_n75_));
  aoi21  g059(.a(new_n75_), .b(x7), .c(x9), .O(new_n76_));
  aoi21  g060(.a(new_n74_), .b(new_n22_), .c(new_n76_), .O(new_n77_));
  nand3  g061(.a(new_n77_), .b(new_n68_), .c(new_n48_), .O(z0));
  inv1   g062(.a(x1), .O(new_n79_));
  inv1   g063(.a(x3), .O(new_n80_));
  oai21  g064(.a(x6), .b(new_n80_), .c(x4), .O(new_n81_));
  nand2  g065(.a(new_n81_), .b(new_n24_), .O(new_n82_));
  nand3  g066(.a(new_n69_), .b(x6), .c(new_n80_), .O(new_n83_));
  nand4  g067(.a(x9), .b(new_n23_), .c(x4), .d(x3), .O(new_n84_));
  aoi21  g068(.a(new_n84_), .b(new_n83_), .c(x2), .O(new_n85_));
  nand3  g069(.a(new_n69_), .b(x6), .c(new_n41_), .O(new_n86_));
  inv1   g070(.a(new_n86_), .O(new_n87_));
  oai21  g071(.a(new_n87_), .b(new_n85_), .c(new_n22_), .O(new_n88_));
  nand2  g072(.a(x7), .b(x6), .O(new_n89_));
  inv1   g073(.a(new_n89_), .O(new_n90_));
  nand2  g074(.a(new_n90_), .b(new_n80_), .O(new_n91_));
  nand3  g075(.a(new_n69_), .b(x3), .c(new_n17_), .O(new_n92_));
  aoi21  g076(.a(new_n92_), .b(new_n91_), .c(new_n22_), .O(new_n93_));
  nand4  g077(.a(new_n38_), .b(x9), .c(new_n80_), .d(x2), .O(new_n94_));
  nand2  g078(.a(new_n26_), .b(x3), .O(new_n95_));
  aoi21  g079(.a(new_n95_), .b(new_n94_), .c(x8), .O(new_n96_));
  oai21  g080(.a(new_n96_), .b(new_n93_), .c(x4), .O(new_n97_));
  nand2  g081(.a(x4), .b(x3), .O(new_n98_));
  nand2  g082(.a(new_n32_), .b(new_n17_), .O(new_n99_));
  nand4  g083(.a(new_n99_), .b(new_n98_), .c(x8), .d(x6), .O(new_n100_));
  oai21  g084(.a(new_n32_), .b(new_n17_), .c(x8), .O(new_n101_));
  nand4  g085(.a(new_n101_), .b(x9), .c(new_n25_), .d(new_n80_), .O(new_n102_));
  nand2  g086(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g087(.a(new_n103_), .b(x0), .O(new_n104_));
  nand4  g088(.a(new_n104_), .b(new_n97_), .c(new_n88_), .d(new_n82_), .O(new_n105_));
  nand2  g089(.a(new_n105_), .b(new_n18_), .O(new_n106_));
  aoi21  g090(.a(x6), .b(new_n17_), .c(new_n23_), .O(new_n107_));
  oai22  g091(.a(new_n107_), .b(new_n22_), .c(x8), .d(x2), .O(new_n108_));
  nand2  g092(.a(new_n108_), .b(x3), .O(new_n109_));
  oai21  g093(.a(new_n23_), .b(new_n25_), .c(x0), .O(new_n110_));
  nand4  g094(.a(new_n110_), .b(x9), .c(new_n80_), .d(x2), .O(new_n111_));
  aoi21  g095(.a(new_n111_), .b(new_n109_), .c(x4), .O(new_n112_));
  oai22  g096(.a(new_n24_), .b(x0), .c(x7), .d(new_n17_), .O(new_n113_));
  oai21  g097(.a(x4), .b(new_n80_), .c(new_n113_), .O(new_n114_));
  nand4  g098(.a(x9), .b(new_n25_), .c(new_n80_), .d(new_n17_), .O(new_n115_));
  aoi21  g099(.a(new_n115_), .b(new_n114_), .c(new_n23_), .O(new_n116_));
  oai21  g100(.a(new_n116_), .b(new_n112_), .c(x5), .O(new_n117_));
  nand3  g101(.a(new_n101_), .b(new_n41_), .c(x0), .O(new_n118_));
  nand2  g102(.a(x2), .b(x0), .O(new_n119_));
  nand4  g103(.a(new_n119_), .b(x9), .c(x8), .d(x4), .O(new_n120_));
  aoi21  g104(.a(new_n120_), .b(new_n118_), .c(x6), .O(new_n121_));
  nand4  g105(.a(new_n99_), .b(x9), .c(x8), .d(x4), .O(new_n122_));
  nor2   g106(.a(new_n122_), .b(x0), .O(new_n123_));
  oai21  g107(.a(new_n123_), .b(new_n121_), .c(x3), .O(new_n124_));
  nand3  g108(.a(new_n124_), .b(new_n117_), .c(new_n106_), .O(new_n125_));
  nand2  g109(.a(new_n125_), .b(new_n79_), .O(new_n126_));
  nor2   g110(.a(new_n18_), .b(new_n80_), .O(new_n127_));
  oai21  g111(.a(x5), .b(x3), .c(new_n25_), .O(new_n128_));
  oai21  g112(.a(new_n128_), .b(new_n127_), .c(new_n24_), .O(new_n129_));
  nand2  g113(.a(new_n69_), .b(new_n22_), .O(new_n130_));
  nor2   g114(.a(x6), .b(new_n18_), .O(new_n131_));
  nand3  g115(.a(new_n131_), .b(new_n23_), .c(x7), .O(new_n132_));
  nand2  g116(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g117(.a(new_n133_), .b(new_n17_), .O(new_n134_));
  nand2  g118(.a(x8), .b(new_n17_), .O(new_n135_));
  nand3  g119(.a(new_n135_), .b(new_n25_), .c(x0), .O(new_n136_));
  nand3  g120(.a(new_n23_), .b(x4), .c(x2), .O(new_n137_));
  aoi21  g121(.a(new_n137_), .b(new_n136_), .c(new_n24_), .O(new_n138_));
  oai22  g122(.a(x8), .b(x5), .c(new_n17_), .d(x0), .O(new_n139_));
  nand2  g123(.a(new_n139_), .b(new_n41_), .O(new_n140_));
  nand3  g124(.a(new_n23_), .b(x5), .c(x4), .O(new_n141_));
  aoi21  g125(.a(new_n141_), .b(new_n140_), .c(new_n25_), .O(new_n142_));
  oai21  g126(.a(new_n142_), .b(new_n138_), .c(x7), .O(new_n143_));
  aoi21  g127(.a(x7), .b(new_n25_), .c(x0), .O(new_n144_));
  nand3  g128(.a(x6), .b(x5), .c(x2), .O(new_n145_));
  inv1   g129(.a(new_n145_), .O(new_n146_));
  oai21  g130(.a(new_n146_), .b(new_n144_), .c(new_n41_), .O(new_n147_));
  nor2   g131(.a(x5), .b(x0), .O(new_n148_));
  nor2   g132(.a(new_n148_), .b(new_n17_), .O(new_n149_));
  nor2   g133(.a(new_n18_), .b(x0), .O(new_n150_));
  oai21  g134(.a(new_n150_), .b(new_n149_), .c(new_n32_), .O(new_n151_));
  nand2  g135(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g136(.a(x5), .b(x2), .O(new_n153_));
  oai21  g137(.a(x6), .b(x5), .c(new_n153_), .O(new_n154_));
  nand4  g138(.a(new_n154_), .b(new_n32_), .c(new_n41_), .d(new_n22_), .O(new_n155_));
  inv1   g139(.a(new_n155_), .O(new_n156_));
  aoi21  g140(.a(new_n152_), .b(x8), .c(new_n156_), .O(new_n157_));
  nand3  g141(.a(new_n157_), .b(new_n143_), .c(new_n134_), .O(new_n158_));
  nand2  g142(.a(new_n158_), .b(x3), .O(new_n159_));
  nor2   g143(.a(x7), .b(x5), .O(new_n160_));
  aoi21  g144(.a(new_n160_), .b(new_n22_), .c(new_n23_), .O(new_n161_));
  nand2  g145(.a(new_n43_), .b(new_n23_), .O(new_n162_));
  oai21  g146(.a(new_n161_), .b(new_n17_), .c(new_n162_), .O(new_n163_));
  aoi21  g147(.a(x5), .b(x2), .c(x4), .O(new_n164_));
  oai21  g148(.a(new_n164_), .b(x0), .c(new_n23_), .O(new_n165_));
  nand3  g149(.a(new_n43_), .b(new_n17_), .c(x0), .O(new_n166_));
  nand2  g150(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  aoi21  g151(.a(new_n163_), .b(x4), .c(new_n167_), .O(new_n168_));
  oai21  g152(.a(new_n148_), .b(new_n34_), .c(new_n17_), .O(new_n169_));
  oai21  g153(.a(x4), .b(new_n17_), .c(new_n23_), .O(new_n170_));
  nand3  g154(.a(new_n170_), .b(x9), .c(new_n22_), .O(new_n171_));
  nand2  g155(.a(new_n171_), .b(new_n169_), .O(new_n172_));
  nand2  g156(.a(new_n172_), .b(new_n25_), .O(new_n173_));
  oai21  g157(.a(new_n168_), .b(new_n25_), .c(new_n173_), .O(new_n174_));
  oai22  g158(.a(new_n89_), .b(x4), .c(new_n49_), .d(x2), .O(new_n175_));
  nand3  g159(.a(new_n175_), .b(new_n18_), .c(new_n22_), .O(new_n176_));
  nor2   g160(.a(new_n41_), .b(new_n22_), .O(new_n177_));
  aoi22  g161(.a(new_n177_), .b(new_n90_), .c(new_n69_), .d(new_n25_), .O(new_n178_));
  nand2  g162(.a(new_n178_), .b(new_n176_), .O(new_n179_));
  aoi21  g163(.a(new_n174_), .b(new_n80_), .c(new_n179_), .O(new_n180_));
  nand3  g164(.a(new_n180_), .b(new_n159_), .c(new_n129_), .O(new_n181_));
  nand2  g165(.a(new_n181_), .b(x1), .O(new_n182_));
  nand3  g166(.a(new_n69_), .b(new_n25_), .c(x4), .O(new_n183_));
  aoi21  g167(.a(new_n183_), .b(new_n66_), .c(new_n80_), .O(new_n184_));
  nand2  g168(.a(new_n131_), .b(x4), .O(new_n185_));
  aoi21  g169(.a(new_n185_), .b(x9), .c(x7), .O(new_n186_));
  nand4  g170(.a(new_n24_), .b(x6), .c(new_n18_), .d(new_n80_), .O(new_n187_));
  inv1   g171(.a(new_n187_), .O(new_n188_));
  nor3   g172(.a(new_n188_), .b(new_n186_), .c(new_n184_), .O(new_n189_));
  nand3  g173(.a(new_n189_), .b(new_n182_), .c(new_n126_), .O(z1));
  nor2   g174(.a(x9), .b(x7), .O(new_n191_));
  inv1   g175(.a(new_n191_), .O(new_n192_));
  xnor2a g176(.a(x3), .b(x1), .O(new_n193_));
  and2   g177(.a(new_n193_), .b(new_n192_), .O(z2));
  nor3   g178(.a(new_n191_), .b(new_n80_), .c(new_n79_), .O(z3));
  nand3  g179(.a(new_n193_), .b(new_n23_), .c(x2), .O(new_n196_));
  nand3  g180(.a(x7), .b(new_n80_), .c(new_n17_), .O(new_n197_));
  aoi21  g181(.a(new_n197_), .b(new_n196_), .c(new_n22_), .O(new_n198_));
  nand2  g182(.a(x7), .b(new_n80_), .O(new_n199_));
  nor2   g183(.a(x2), .b(x0), .O(new_n200_));
  nand3  g184(.a(new_n200_), .b(new_n23_), .c(x3), .O(new_n201_));
  aoi21  g185(.a(new_n201_), .b(new_n199_), .c(new_n79_), .O(new_n202_));
  oai21  g186(.a(new_n202_), .b(new_n198_), .c(x6), .O(new_n203_));
  nand3  g187(.a(new_n101_), .b(new_n80_), .c(x0), .O(new_n204_));
  nand3  g188(.a(new_n23_), .b(new_n17_), .c(x1), .O(new_n205_));
  nand2  g189(.a(new_n205_), .b(new_n204_), .O(new_n206_));
  nand2  g190(.a(new_n206_), .b(new_n25_), .O(new_n207_));
  aoi21  g191(.a(new_n207_), .b(new_n203_), .c(x4), .O(new_n208_));
  nand2  g192(.a(new_n25_), .b(x4), .O(new_n209_));
  nand4  g193(.a(new_n23_), .b(x6), .c(new_n80_), .d(new_n22_), .O(new_n210_));
  aoi21  g194(.a(new_n210_), .b(new_n209_), .c(x1), .O(new_n211_));
  oai21  g195(.a(new_n23_), .b(x4), .c(new_n25_), .O(new_n212_));
  nor2   g196(.a(new_n212_), .b(x3), .O(new_n213_));
  oai21  g197(.a(new_n213_), .b(new_n211_), .c(new_n17_), .O(new_n214_));
  oai21  g198(.a(x6), .b(x3), .c(x1), .O(new_n215_));
  nand3  g199(.a(new_n215_), .b(x4), .c(new_n22_), .O(new_n216_));
  nand2  g200(.a(new_n216_), .b(new_n214_), .O(new_n217_));
  oai21  g201(.a(new_n217_), .b(new_n208_), .c(x9), .O(new_n218_));
  nand4  g202(.a(new_n193_), .b(x6), .c(new_n41_), .d(x0), .O(new_n219_));
  oai21  g203(.a(x3), .b(new_n79_), .c(x4), .O(new_n220_));
  aoi21  g204(.a(new_n220_), .b(new_n219_), .c(new_n17_), .O(new_n221_));
  nand3  g205(.a(new_n41_), .b(x3), .c(x1), .O(new_n222_));
  oai21  g206(.a(x3), .b(x1), .c(new_n222_), .O(new_n223_));
  nand3  g207(.a(new_n223_), .b(x6), .c(new_n17_), .O(new_n224_));
  nand2  g208(.a(new_n224_), .b(new_n98_), .O(new_n225_));
  nand2  g209(.a(new_n225_), .b(new_n22_), .O(new_n226_));
  nand2  g210(.a(new_n226_), .b(new_n209_), .O(new_n227_));
  oai21  g211(.a(new_n227_), .b(new_n221_), .c(new_n32_), .O(new_n228_));
  nand2  g212(.a(new_n228_), .b(new_n218_), .O(new_n229_));
  nand2  g213(.a(new_n229_), .b(x5), .O(new_n230_));
  oai21  g214(.a(x6), .b(x2), .c(x7), .O(new_n231_));
  nand2  g215(.a(new_n23_), .b(new_n25_), .O(new_n232_));
  aoi22  g216(.a(new_n232_), .b(new_n231_), .c(new_n98_), .d(new_n79_), .O(new_n233_));
  nand3  g217(.a(new_n23_), .b(x6), .c(new_n41_), .O(new_n234_));
  inv1   g218(.a(new_n234_), .O(new_n235_));
  oai21  g219(.a(new_n235_), .b(new_n233_), .c(new_n18_), .O(new_n236_));
  oai21  g220(.a(new_n89_), .b(x2), .c(new_n232_), .O(new_n237_));
  nand3  g221(.a(new_n237_), .b(new_n41_), .c(x1), .O(new_n238_));
  aoi21  g222(.a(new_n238_), .b(new_n236_), .c(new_n24_), .O(new_n239_));
  nand3  g223(.a(new_n26_), .b(new_n18_), .c(new_n17_), .O(new_n240_));
  inv1   g224(.a(new_n240_), .O(new_n241_));
  oai21  g225(.a(new_n241_), .b(new_n239_), .c(x0), .O(new_n242_));
  nand3  g226(.a(new_n23_), .b(new_n25_), .c(x2), .O(new_n243_));
  aoi21  g227(.a(new_n243_), .b(new_n89_), .c(new_n41_), .O(new_n244_));
  nand3  g228(.a(x7), .b(new_n25_), .c(x3), .O(new_n245_));
  inv1   g229(.a(new_n245_), .O(new_n246_));
  oai21  g230(.a(new_n246_), .b(new_n244_), .c(x9), .O(new_n247_));
  nand2  g231(.a(new_n26_), .b(new_n80_), .O(new_n248_));
  aoi21  g232(.a(new_n248_), .b(new_n247_), .c(new_n79_), .O(new_n249_));
  nand2  g233(.a(new_n26_), .b(new_n22_), .O(new_n250_));
  nand3  g234(.a(x9), .b(new_n23_), .c(x3), .O(new_n251_));
  aoi21  g235(.a(new_n251_), .b(new_n250_), .c(new_n17_), .O(new_n252_));
  nand3  g236(.a(new_n26_), .b(x3), .c(new_n79_), .O(new_n253_));
  inv1   g237(.a(new_n253_), .O(new_n254_));
  oai21  g238(.a(new_n254_), .b(new_n252_), .c(x4), .O(new_n255_));
  nand3  g239(.a(new_n23_), .b(new_n32_), .c(x6), .O(new_n256_));
  nand2  g240(.a(new_n256_), .b(new_n255_), .O(new_n257_));
  oai21  g241(.a(new_n257_), .b(new_n249_), .c(new_n18_), .O(new_n258_));
  inv1   g242(.a(new_n222_), .O(new_n259_));
  aoi21  g243(.a(new_n259_), .b(new_n60_), .c(new_n191_), .O(new_n260_));
  nand3  g244(.a(new_n260_), .b(new_n258_), .c(new_n242_), .O(new_n261_));
  inv1   g245(.a(new_n261_), .O(new_n262_));
  nand2  g246(.a(new_n262_), .b(new_n230_), .O(z4));
  inv1   g247(.a(new_n119_), .O(new_n264_));
  oai21  g248(.a(new_n200_), .b(new_n264_), .c(new_n193_), .O(new_n265_));
  nand2  g249(.a(new_n265_), .b(new_n192_), .O(z5));
endmodule


