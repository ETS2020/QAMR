// Benchmark "FAU" written by ABC on Fri Aug 14 01:49:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n36_, new_n37_, new_n39_, new_n42_, new_n44_, new_n46_,
    new_n50_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n66_;
  inv1   g00(.a(x09), .O(new_n34_));
  aoi21  g01(.a(x08), .b(x00), .c(new_n34_), .O(z00));
  inv1   g02(.a(x10), .O(new_n36_));
  nand2  g03(.a(x08), .b(x00), .O(new_n37_));
  nand2  g04(.a(new_n37_), .b(new_n36_), .O(z01));
  inv1   g05(.a(x11), .O(new_n39_));
  aoi21  g06(.a(x08), .b(x00), .c(new_n39_), .O(z02));
  and2   g07(.a(new_n37_), .b(x12), .O(z03));
  inv1   g08(.a(x13), .O(new_n42_));
  nand2  g09(.a(new_n37_), .b(new_n42_), .O(z04));
  inv1   g10(.a(x14), .O(new_n44_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n44_), .O(z05));
  inv1   g12(.a(x15), .O(new_n46_));
  nand2  g13(.a(new_n37_), .b(new_n46_), .O(z06));
  and2   g14(.a(new_n37_), .b(x16), .O(z07));
  oai21  g15(.a(new_n34_), .b(x08), .c(new_n37_), .O(z08));
  nand2  g16(.a(x08), .b(x01), .O(new_n50_));
  oai22  g17(.a(new_n50_), .b(x00), .c(new_n36_), .d(x08), .O(z09));
  nand2  g18(.a(x08), .b(x02), .O(new_n52_));
  oai22  g19(.a(new_n52_), .b(x00), .c(new_n39_), .d(x08), .O(z10));
  oai21  g20(.a(x03), .b(x00), .c(x08), .O(new_n54_));
  inv1   g21(.a(x08), .O(new_n55_));
  nand2  g22(.a(x12), .b(new_n55_), .O(new_n56_));
  nand2  g23(.a(new_n56_), .b(new_n54_), .O(z11));
  oai21  g24(.a(x04), .b(x00), .c(x08), .O(new_n58_));
  nand2  g25(.a(x13), .b(new_n55_), .O(new_n59_));
  nand2  g26(.a(new_n59_), .b(new_n58_), .O(z12));
  nand2  g27(.a(x08), .b(x05), .O(new_n61_));
  oai22  g28(.a(new_n61_), .b(x00), .c(new_n44_), .d(x08), .O(z13));
  nand2  g29(.a(x08), .b(x06), .O(new_n63_));
  oai22  g30(.a(new_n63_), .b(x00), .c(new_n46_), .d(x08), .O(z14));
  oai21  g31(.a(x07), .b(x00), .c(x08), .O(new_n65_));
  nand2  g32(.a(x16), .b(new_n55_), .O(new_n66_));
  nand2  g33(.a(new_n66_), .b(new_n65_), .O(z15));
endmodule


