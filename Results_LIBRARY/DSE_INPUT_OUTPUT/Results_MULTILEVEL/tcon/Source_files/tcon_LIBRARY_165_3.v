// Benchmark "FAU" written by ABC on Fri Aug 14 01:50:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n36_, new_n38_, new_n40_, new_n42_, new_n44_, new_n46_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n67_;
  nand2  g00(.a(x08), .b(x02), .O(new_n34_));
  and2   g01(.a(new_n34_), .b(x09), .O(z00));
  inv1   g02(.a(x10), .O(new_n36_));
  nand2  g03(.a(new_n34_), .b(new_n36_), .O(z01));
  inv1   g04(.a(x11), .O(new_n38_));
  aoi21  g05(.a(x08), .b(x02), .c(new_n38_), .O(z02));
  inv1   g06(.a(x12), .O(new_n40_));
  nand2  g07(.a(new_n34_), .b(new_n40_), .O(z03));
  inv1   g08(.a(x13), .O(new_n42_));
  nand2  g09(.a(new_n34_), .b(new_n42_), .O(z04));
  inv1   g10(.a(x14), .O(new_n44_));
  aoi21  g11(.a(x08), .b(x02), .c(new_n44_), .O(z05));
  inv1   g12(.a(x15), .O(new_n46_));
  nand2  g13(.a(new_n34_), .b(new_n46_), .O(z06));
  and2   g14(.a(new_n34_), .b(x16), .O(z07));
  inv1   g15(.a(x08), .O(new_n49_));
  nand2  g16(.a(x09), .b(new_n49_), .O(new_n50_));
  inv1   g17(.a(x02), .O(new_n51_));
  nand3  g18(.a(x08), .b(new_n51_), .c(x00), .O(new_n52_));
  nand2  g19(.a(new_n52_), .b(new_n50_), .O(z08));
  nand2  g20(.a(x10), .b(new_n49_), .O(new_n54_));
  nand3  g21(.a(x08), .b(new_n51_), .c(x01), .O(new_n55_));
  nand2  g22(.a(new_n55_), .b(new_n54_), .O(z09));
  oai21  g23(.a(new_n38_), .b(x08), .c(new_n34_), .O(z10));
  nand2  g24(.a(x08), .b(x03), .O(new_n58_));
  oai22  g25(.a(new_n58_), .b(x02), .c(new_n40_), .d(x08), .O(z11));
  nand2  g26(.a(x08), .b(x04), .O(new_n60_));
  oai22  g27(.a(new_n60_), .b(x02), .c(new_n42_), .d(x08), .O(z12));
  nand2  g28(.a(x08), .b(x05), .O(new_n62_));
  oai22  g29(.a(new_n62_), .b(x02), .c(new_n44_), .d(x08), .O(z13));
  nand2  g30(.a(x08), .b(x06), .O(new_n64_));
  oai22  g31(.a(new_n64_), .b(x02), .c(new_n46_), .d(x08), .O(z14));
  oai21  g32(.a(x07), .b(x02), .c(x08), .O(new_n66_));
  nand2  g33(.a(x16), .b(new_n49_), .O(new_n67_));
  nand2  g34(.a(new_n67_), .b(new_n66_), .O(z15));
endmodule


