// Benchmark "FAU" written by ABC on Fri Aug 14 01:50:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n35_, new_n36_, new_n39_, new_n41_, new_n43_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n77_;
  inv1   g00(.a(x09), .O(new_n34_));
  nand2  g01(.a(x12), .b(x00), .O(new_n35_));
  inv1   g02(.a(new_n35_), .O(new_n36_));
  nor2   g03(.a(new_n36_), .b(new_n34_), .O(z00));
  and2   g04(.a(new_n35_), .b(x10), .O(z01));
  inv1   g05(.a(x11), .O(new_n39_));
  nor2   g06(.a(new_n36_), .b(new_n39_), .O(z02));
  inv1   g07(.a(x13), .O(new_n41_));
  nand2  g08(.a(new_n35_), .b(new_n41_), .O(z04));
  inv1   g09(.a(x14), .O(new_n43_));
  nand2  g10(.a(new_n35_), .b(new_n43_), .O(z05));
  and2   g11(.a(new_n35_), .b(x15), .O(z06));
  and2   g12(.a(new_n35_), .b(x16), .O(z07));
  oai21  g13(.a(x12), .b(x08), .c(x00), .O(new_n47_));
  oai21  g14(.a(new_n34_), .b(x08), .c(new_n47_), .O(z08));
  nand2  g15(.a(x08), .b(x01), .O(new_n49_));
  inv1   g16(.a(x08), .O(new_n50_));
  nand2  g17(.a(x10), .b(new_n50_), .O(new_n51_));
  aoi21  g18(.a(new_n51_), .b(new_n49_), .c(new_n36_), .O(z09));
  inv1   g19(.a(x00), .O(new_n53_));
  nand2  g20(.a(x08), .b(x02), .O(new_n54_));
  nand2  g21(.a(x12), .b(x11), .O(new_n55_));
  oai21  g22(.a(new_n55_), .b(x08), .c(new_n54_), .O(new_n56_));
  nand2  g23(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  inv1   g24(.a(x12), .O(new_n58_));
  oai21  g25(.a(new_n39_), .b(x08), .c(new_n54_), .O(new_n59_));
  nand2  g26(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g27(.a(new_n60_), .b(new_n57_), .O(z10));
  nand2  g28(.a(x08), .b(x03), .O(new_n62_));
  oai21  g29(.a(new_n58_), .b(x08), .c(new_n62_), .O(new_n63_));
  nand2  g30(.a(new_n63_), .b(new_n53_), .O(new_n64_));
  nand3  g31(.a(new_n58_), .b(x08), .c(x03), .O(new_n65_));
  nand2  g32(.a(new_n65_), .b(new_n64_), .O(z11));
  nand2  g33(.a(x13), .b(new_n50_), .O(new_n67_));
  nand2  g34(.a(x08), .b(x04), .O(new_n68_));
  nand3  g35(.a(new_n68_), .b(new_n67_), .c(new_n35_), .O(z12));
  nand2  g36(.a(x14), .b(new_n50_), .O(new_n70_));
  nand2  g37(.a(x08), .b(x05), .O(new_n71_));
  nand3  g38(.a(new_n71_), .b(new_n70_), .c(new_n35_), .O(z13));
  nand2  g39(.a(x08), .b(x06), .O(new_n73_));
  nand2  g40(.a(x15), .b(new_n50_), .O(new_n74_));
  aoi21  g41(.a(new_n74_), .b(new_n73_), .c(new_n36_), .O(z14));
  nand2  g42(.a(x16), .b(new_n50_), .O(new_n76_));
  nand2  g43(.a(x08), .b(x07), .O(new_n77_));
  nand3  g44(.a(new_n77_), .b(new_n76_), .c(new_n35_), .O(z15));
  buf    g45(.a(x12), .O(z03));
endmodule


