// Benchmark "FAU" written by ABC on Fri Aug 14 01:49:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n36_, new_n40_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n70_, new_n71_;
  nand2  g00(.a(x09), .b(x05), .O(new_n34_));
  and2   g01(.a(new_n34_), .b(x10), .O(z01));
  inv1   g02(.a(x11), .O(new_n36_));
  nand2  g03(.a(new_n34_), .b(new_n36_), .O(z02));
  and2   g04(.a(new_n34_), .b(x12), .O(z03));
  and2   g05(.a(new_n34_), .b(x13), .O(z04));
  inv1   g06(.a(x14), .O(new_n40_));
  nand2  g07(.a(new_n34_), .b(new_n40_), .O(z05));
  and2   g08(.a(new_n34_), .b(x15), .O(z06));
  and2   g09(.a(new_n34_), .b(x16), .O(z07));
  nand3  g10(.a(new_n34_), .b(x08), .c(x00), .O(new_n44_));
  inv1   g11(.a(x05), .O(new_n45_));
  inv1   g12(.a(x08), .O(new_n46_));
  nand3  g13(.a(x09), .b(new_n46_), .c(new_n45_), .O(new_n47_));
  nand2  g14(.a(new_n47_), .b(new_n44_), .O(z08));
  nand2  g15(.a(x10), .b(new_n46_), .O(new_n49_));
  nand2  g16(.a(x08), .b(x01), .O(new_n50_));
  nand3  g17(.a(new_n50_), .b(new_n49_), .c(new_n34_), .O(z09));
  nand2  g18(.a(x11), .b(new_n46_), .O(new_n52_));
  nand2  g19(.a(x08), .b(x02), .O(new_n53_));
  nand3  g20(.a(new_n53_), .b(new_n52_), .c(new_n34_), .O(z10));
  inv1   g21(.a(new_n34_), .O(new_n55_));
  nand2  g22(.a(x08), .b(x03), .O(new_n56_));
  nand2  g23(.a(x12), .b(new_n46_), .O(new_n57_));
  aoi21  g24(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z11));
  nand2  g25(.a(x13), .b(new_n46_), .O(new_n59_));
  nand2  g26(.a(x08), .b(x04), .O(new_n60_));
  nand3  g27(.a(new_n60_), .b(new_n59_), .c(new_n34_), .O(z12));
  inv1   g28(.a(x09), .O(new_n62_));
  nand2  g29(.a(new_n40_), .b(new_n46_), .O(new_n63_));
  nand3  g30(.a(new_n63_), .b(new_n62_), .c(x05), .O(new_n64_));
  nand3  g31(.a(x14), .b(new_n46_), .c(new_n45_), .O(new_n65_));
  nand2  g32(.a(new_n65_), .b(new_n64_), .O(z13));
  nand2  g33(.a(x15), .b(new_n46_), .O(new_n67_));
  nand2  g34(.a(x08), .b(x06), .O(new_n68_));
  nand3  g35(.a(new_n68_), .b(new_n67_), .c(new_n34_), .O(z14));
  nand2  g36(.a(x08), .b(x07), .O(new_n70_));
  nand2  g37(.a(x16), .b(new_n46_), .O(new_n71_));
  aoi21  g38(.a(new_n71_), .b(new_n70_), .c(new_n55_), .O(z15));
  buf    g39(.a(x09), .O(z00));
endmodule


