// Benchmark "FAU" written by ABC on Fri Aug 14 01:49:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n38_, new_n40_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_;
  nand2  g00(.a(x14), .b(x11), .O(new_n34_));
  and2   g01(.a(new_n34_), .b(x09), .O(z00));
  and2   g02(.a(new_n34_), .b(x10), .O(z01));
  and2   g03(.a(new_n34_), .b(x12), .O(z03));
  inv1   g04(.a(x13), .O(new_n38_));
  nand2  g05(.a(new_n34_), .b(new_n38_), .O(z04));
  inv1   g06(.a(x15), .O(new_n40_));
  nand2  g07(.a(new_n34_), .b(new_n40_), .O(z06));
  and2   g08(.a(new_n34_), .b(x16), .O(z07));
  and2   g09(.a(x14), .b(x11), .O(new_n43_));
  nand2  g10(.a(x08), .b(x00), .O(new_n44_));
  inv1   g11(.a(x08), .O(new_n45_));
  nand2  g12(.a(x09), .b(new_n45_), .O(new_n46_));
  aoi21  g13(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(z08));
  nand2  g14(.a(x08), .b(x01), .O(new_n48_));
  nand2  g15(.a(x10), .b(new_n45_), .O(new_n49_));
  aoi21  g16(.a(new_n49_), .b(new_n48_), .c(new_n43_), .O(z09));
  nand3  g17(.a(new_n34_), .b(x08), .c(x02), .O(new_n51_));
  inv1   g18(.a(x14), .O(new_n52_));
  nand3  g19(.a(new_n52_), .b(x11), .c(new_n45_), .O(new_n53_));
  nand2  g20(.a(new_n53_), .b(new_n51_), .O(z10));
  nand3  g21(.a(new_n34_), .b(x08), .c(x03), .O(new_n55_));
  nand2  g22(.a(x14), .b(x11), .O(new_n56_));
  nand3  g23(.a(new_n56_), .b(x12), .c(new_n45_), .O(new_n57_));
  nand2  g24(.a(new_n57_), .b(new_n55_), .O(z11));
  nand3  g25(.a(new_n34_), .b(x08), .c(x04), .O(new_n59_));
  nand2  g26(.a(x14), .b(x11), .O(new_n60_));
  nand3  g27(.a(new_n60_), .b(x13), .c(new_n45_), .O(new_n61_));
  nand2  g28(.a(new_n61_), .b(new_n59_), .O(z12));
  nand3  g29(.a(new_n34_), .b(x08), .c(x05), .O(new_n63_));
  inv1   g30(.a(x11), .O(new_n64_));
  nand3  g31(.a(x14), .b(new_n64_), .c(new_n45_), .O(new_n65_));
  nand2  g32(.a(new_n65_), .b(new_n63_), .O(z13));
  nand2  g33(.a(x08), .b(x06), .O(new_n67_));
  nand2  g34(.a(x15), .b(new_n45_), .O(new_n68_));
  aoi21  g35(.a(new_n68_), .b(new_n67_), .c(new_n43_), .O(z14));
  nand2  g36(.a(x16), .b(new_n45_), .O(new_n70_));
  nand2  g37(.a(x08), .b(x07), .O(new_n71_));
  nand3  g38(.a(new_n71_), .b(new_n70_), .c(new_n34_), .O(z15));
  buf    g39(.a(x11), .O(z02));
  buf    g40(.a(x14), .O(z05));
endmodule


