// Benchmark "FAU" written by ABC on Fri Aug 14 01:49:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n35_, new_n38_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n69_, new_n70_;
  inv1   g00(.a(x09), .O(new_n34_));
  nand2  g01(.a(x15), .b(x05), .O(new_n35_));
  nand2  g02(.a(new_n35_), .b(new_n34_), .O(z00));
  and2   g03(.a(new_n35_), .b(x10), .O(z01));
  inv1   g04(.a(x11), .O(new_n38_));
  nand2  g05(.a(new_n35_), .b(new_n38_), .O(z02));
  and2   g06(.a(new_n35_), .b(x12), .O(z03));
  and2   g07(.a(new_n35_), .b(x13), .O(z04));
  and2   g08(.a(new_n35_), .b(x14), .O(z05));
  and2   g09(.a(new_n35_), .b(x16), .O(z07));
  inv1   g10(.a(x08), .O(new_n44_));
  nand2  g11(.a(x09), .b(new_n44_), .O(new_n45_));
  nand2  g12(.a(x08), .b(x00), .O(new_n46_));
  nand3  g13(.a(new_n46_), .b(new_n45_), .c(new_n35_), .O(z08));
  nand2  g14(.a(x10), .b(new_n44_), .O(new_n48_));
  nand2  g15(.a(x08), .b(x01), .O(new_n49_));
  nand3  g16(.a(new_n49_), .b(new_n48_), .c(new_n35_), .O(z09));
  and2   g17(.a(x15), .b(x05), .O(new_n51_));
  nand2  g18(.a(x08), .b(x02), .O(new_n52_));
  nand2  g19(.a(x11), .b(new_n44_), .O(new_n53_));
  aoi21  g20(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(z10));
  nand2  g21(.a(x12), .b(new_n44_), .O(new_n55_));
  nand2  g22(.a(x08), .b(x03), .O(new_n56_));
  nand3  g23(.a(new_n56_), .b(new_n55_), .c(new_n35_), .O(z11));
  nand2  g24(.a(x15), .b(x05), .O(new_n58_));
  nand3  g25(.a(new_n58_), .b(x08), .c(x04), .O(new_n59_));
  nand3  g26(.a(new_n35_), .b(x13), .c(new_n44_), .O(new_n60_));
  nand2  g27(.a(new_n60_), .b(new_n59_), .O(z12));
  nand3  g28(.a(new_n35_), .b(x14), .c(new_n44_), .O(new_n62_));
  inv1   g29(.a(x15), .O(new_n63_));
  nand3  g30(.a(new_n63_), .b(x08), .c(x05), .O(new_n64_));
  nand2  g31(.a(new_n64_), .b(new_n62_), .O(z13));
  oai21  g32(.a(new_n44_), .b(x05), .c(x15), .O(new_n66_));
  nand2  g33(.a(x08), .b(x06), .O(new_n67_));
  nand2  g34(.a(new_n67_), .b(new_n66_), .O(z14));
  nand2  g35(.a(x08), .b(x07), .O(new_n69_));
  nand2  g36(.a(x16), .b(new_n44_), .O(new_n70_));
  aoi21  g37(.a(new_n70_), .b(new_n69_), .c(new_n51_), .O(z15));
  buf    g38(.a(x15), .O(z06));
endmodule


