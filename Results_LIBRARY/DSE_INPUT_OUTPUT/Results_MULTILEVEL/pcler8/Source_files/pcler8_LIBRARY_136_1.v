// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x21), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  nand2  g002(.a(x20), .b(x19), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x10), .O(new_n48_));
  nand2  g004(.a(x23), .b(x22), .O(new_n49_));
  nand3  g005(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(new_n48_), .c(x09), .d(new_n46_), .O(new_n52_));
  aoi21  g008(.a(new_n52_), .b(x13), .c(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nor2   g010(.a(new_n45_), .b(x13), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n57_), .b(new_n59_), .O(z02));
  inv1   g016(.a(new_n55_), .O(new_n61_));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(z03));
  and2   g019(.a(new_n56_), .b(x03), .O(z04));
  and2   g020(.a(new_n56_), .b(x04), .O(z05));
  nand2  g021(.a(new_n56_), .b(x05), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z06));
  nand2  g023(.a(x08), .b(x06), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n61_), .O(z07));
  nand2  g025(.a(new_n56_), .b(x07), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z08));
  inv1   g027(.a(x10), .O(new_n72_));
  inv1   g028(.a(x19), .O(new_n73_));
  nand4  g029(.a(new_n73_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n74_));
  oai21  g030(.a(new_n46_), .b(new_n54_), .c(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n61_), .O(new_n76_));
  inv1   g032(.a(new_n49_), .O(new_n77_));
  inv1   g033(.a(new_n50_), .O(new_n78_));
  nand3  g034(.a(new_n72_), .b(x09), .c(new_n46_), .O(new_n79_));
  nand3  g035(.a(x20), .b(x13), .c(x11), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n78_), .c(new_n77_), .d(x21), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n76_), .O(z09));
  xor2a  g039(.a(x20), .b(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n85_));
  oai21  g041(.a(new_n46_), .b(new_n59_), .c(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n61_), .O(new_n87_));
  nand3  g043(.a(x19), .b(x13), .c(x12), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n79_), .O(new_n89_));
  inv1   g045(.a(x20), .O(new_n90_));
  nand2  g046(.a(x26), .b(x25), .O(new_n91_));
  nand2  g047(.a(x22), .b(x21), .O(new_n92_));
  nand2  g048(.a(x24), .b(x23), .O(new_n93_));
  nor4   g049(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n89_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n87_), .O(z10));
  inv1   g052(.a(new_n47_), .O(new_n97_));
  oai21  g053(.a(new_n50_), .b(new_n49_), .c(x21), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(x20), .c(x19), .O(new_n99_));
  oai21  g055(.a(new_n97_), .b(new_n45_), .c(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n62_), .c(new_n61_), .O(z11));
  inv1   g058(.a(x22), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(x20), .c(x19), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n79_), .c(x13), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x21), .O(new_n106_));
  and2   g062(.a(x23), .b(x14), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n78_), .c(new_n45_), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n97_), .c(new_n103_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n110_));
  nand2  g066(.a(x08), .b(x03), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(new_n110_), .c(new_n106_), .O(z12));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x23), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x20), .c(x19), .O(new_n115_));
  nand2  g071(.a(new_n47_), .b(x23), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x22), .c(x21), .O(new_n118_));
  nand2  g074(.a(new_n92_), .b(x23), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n121_));
  aoi21  g077(.a(x08), .b(x04), .c(new_n55_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n121_), .O(z13));
  inv1   g079(.a(x13), .O(new_n124_));
  nand3  g080(.a(x26), .b(x25), .c(x16), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n97_), .c(x23), .d(x22), .O(new_n126_));
  nand3  g082(.a(x21), .b(x20), .c(x19), .O(new_n127_));
  inv1   g083(.a(x24), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x22), .O(new_n129_));
  nor2   g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  aoi21  g086(.a(new_n126_), .b(x24), .c(new_n130_), .O(new_n131_));
  nand2  g087(.a(x24), .b(new_n45_), .O(new_n132_));
  oai21  g088(.a(new_n131_), .b(new_n124_), .c(new_n132_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n66_), .O(z14));
  nand2  g091(.a(new_n56_), .b(x06), .O(new_n136_));
  nand3  g092(.a(x22), .b(x20), .c(x19), .O(new_n137_));
  nand2  g093(.a(x26), .b(x17), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x24), .c(x23), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n137_), .c(x25), .O(new_n140_));
  inv1   g096(.a(new_n127_), .O(new_n141_));
  nor2   g097(.a(x25), .b(new_n128_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n141_), .c(new_n77_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x13), .O(new_n145_));
  nand2  g101(.a(x25), .b(new_n45_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n136_), .O(z15));
  inv1   g105(.a(x26), .O(new_n150_));
  nor2   g106(.a(new_n47_), .b(x18), .O(new_n151_));
  nand4  g107(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(new_n151_), .c(new_n150_), .O(new_n154_));
  nand4  g110(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n155_));
  nand4  g111(.a(new_n150_), .b(x25), .c(x24), .d(x23), .O(new_n156_));
  nor2   g112(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n154_), .c(x13), .O(new_n158_));
  nand2  g114(.a(x26), .b(new_n45_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n70_), .O(z16));
endmodule


