// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x25), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x20), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  and2   g005(.a(x22), .b(x21), .O(new_n50_));
  inv1   g006(.a(x24), .O(new_n51_));
  inv1   g007(.a(x26), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(new_n50_), .c(new_n49_), .d(x23), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(x19), .c(new_n45_), .O(z00));
  nor2   g011(.a(new_n45_), .b(x19), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n46_), .O(new_n57_));
  and2   g013(.a(new_n57_), .b(x00), .O(z01));
  inv1   g014(.a(new_n56_), .O(new_n59_));
  nand2  g015(.a(x08), .b(x01), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(z02));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n59_), .O(z03));
  nand2  g019(.a(new_n57_), .b(x03), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z04));
  nand2  g021(.a(x08), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n59_), .O(z05));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n59_), .O(z06));
  and2   g025(.a(new_n57_), .b(x06), .O(z07));
  nand2  g026(.a(new_n57_), .b(x07), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z08));
  nand3  g028(.a(new_n50_), .b(x20), .c(x11), .O(new_n73_));
  inv1   g029(.a(x23), .O(new_n74_));
  nor2   g030(.a(new_n51_), .b(new_n74_), .O(new_n75_));
  and2   g031(.a(x26), .b(x25), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n73_), .c(x19), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n79_));
  aoi21  g035(.a(x08), .b(x00), .c(new_n56_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n79_), .O(z09));
  inv1   g037(.a(x19), .O(new_n82_));
  oai21  g038(.a(new_n49_), .b(x25), .c(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n50_), .b(x12), .O(new_n84_));
  oai22  g040(.a(new_n84_), .b(new_n77_), .c(x20), .d(new_n82_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n83_), .c(new_n60_), .O(z10));
  xor2a  g043(.a(x21), .b(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  nand2  g045(.a(x21), .b(new_n82_), .O(new_n90_));
  inv1   g046(.a(new_n77_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(x22), .c(x20), .d(x13), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n62_), .c(new_n59_), .O(z11));
  nand4  g051(.a(new_n76_), .b(x24), .c(x23), .d(x14), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x21), .c(x20), .O(new_n97_));
  inv1   g053(.a(x22), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(x21), .c(x20), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  aoi21  g056(.a(new_n97_), .b(x22), .c(new_n100_), .O(new_n101_));
  nand3  g057(.a(new_n45_), .b(x22), .c(new_n82_), .O(new_n102_));
  oai21  g058(.a(new_n101_), .b(new_n82_), .c(new_n102_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n64_), .O(z12));
  nand2  g061(.a(new_n57_), .b(x04), .O(new_n106_));
  nand4  g062(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x23), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x21), .c(x20), .O(new_n109_));
  nand2  g065(.a(x21), .b(x20), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x23), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n109_), .c(new_n98_), .O(new_n112_));
  nor2   g068(.a(new_n74_), .b(x22), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n112_), .c(x19), .O(new_n114_));
  nand3  g070(.a(new_n45_), .b(x23), .c(new_n82_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n106_), .O(z13));
  nand2  g074(.a(new_n57_), .b(x05), .O(new_n119_));
  inv1   g075(.a(new_n110_), .O(new_n120_));
  nand2  g076(.a(new_n76_), .b(x16), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n120_), .c(x23), .d(x22), .O(new_n122_));
  nand3  g078(.a(new_n51_), .b(x23), .c(x22), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n110_), .O(new_n124_));
  aoi21  g080(.a(new_n122_), .b(x24), .c(new_n124_), .O(new_n125_));
  nor2   g081(.a(x25), .b(new_n51_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n82_), .O(new_n127_));
  oai21  g083(.a(new_n125_), .b(new_n82_), .c(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n119_), .O(z14));
  nand2  g086(.a(new_n50_), .b(x20), .O(new_n131_));
  nand2  g087(.a(x26), .b(x17), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x24), .c(x23), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n131_), .c(x25), .O(new_n134_));
  nor2   g090(.a(new_n74_), .b(new_n98_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n126_), .c(new_n120_), .d(x19), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n138_));
  aoi21  g094(.a(x08), .b(x06), .c(new_n56_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n138_), .O(z15));
  nand3  g096(.a(x24), .b(x23), .c(x22), .O(new_n141_));
  nor3   g097(.a(new_n141_), .b(new_n110_), .c(x18), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n52_), .O(new_n143_));
  nand3  g099(.a(new_n75_), .b(new_n52_), .c(x25), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n131_), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n143_), .c(x19), .O(new_n146_));
  nand2  g102(.a(x26), .b(new_n45_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n71_), .O(z16));
endmodule


