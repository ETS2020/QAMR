// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x10), .O(new_n46_));
  inv1   g002(.a(x09), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x08), .O(new_n48_));
  nand3  g004(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x22), .c(x21), .d(x20), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x25), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x21), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x00), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z01));
  nand2  g015(.a(x08), .b(x01), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n57_), .O(z02));
  inv1   g017(.a(x08), .O(new_n62_));
  nor2   g018(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x02), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n57_), .O(z04));
  and2   g023(.a(new_n63_), .b(x04), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n57_), .O(z06));
  nand2  g026(.a(x08), .b(x06), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n57_), .O(z07));
  nand2  g028(.a(new_n63_), .b(x07), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(z08));
  inv1   g030(.a(x19), .O(new_n75_));
  nand3  g031(.a(new_n48_), .b(new_n75_), .c(new_n46_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n58_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n57_), .O(new_n78_));
  nand2  g034(.a(x20), .b(x11), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x10), .O(new_n80_));
  inv1   g036(.a(x22), .O(new_n81_));
  inv1   g037(.a(x23), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x21), .O(new_n84_));
  nand3  g040(.a(x26), .b(x25), .c(x24), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n80_), .c(new_n48_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n78_), .O(z09));
  nand2  g044(.a(new_n83_), .b(x12), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n85_), .c(x20), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x19), .O(new_n91_));
  nand2  g047(.a(x20), .b(new_n75_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n46_), .c(x09), .d(new_n62_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n60_), .c(new_n57_), .O(z10));
  inv1   g051(.a(x21), .O(new_n96_));
  nand3  g052(.a(new_n55_), .b(new_n96_), .c(x20), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(x20), .c(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(x19), .O(new_n99_));
  nand2  g055(.a(new_n83_), .b(x13), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n85_), .c(x19), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x21), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n99_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n46_), .c(x09), .d(new_n62_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n64_), .O(z11));
  nand2  g061(.a(new_n63_), .b(x03), .O(new_n106_));
  nand2  g062(.a(x23), .b(x14), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n85_), .c(x20), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n75_), .c(x22), .O(new_n109_));
  nand3  g065(.a(new_n81_), .b(x20), .c(x19), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x21), .O(new_n112_));
  nand3  g068(.a(new_n55_), .b(x22), .c(new_n96_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n46_), .c(x09), .d(new_n62_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n106_), .O(z12));
  inv1   g072(.a(x20), .O(new_n117_));
  nand2  g073(.a(new_n82_), .b(x21), .O(new_n118_));
  nand4  g074(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  nand2  g076(.a(x20), .b(x19), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nor2   g078(.a(new_n122_), .b(new_n82_), .O(new_n123_));
  aoi22  g079(.a(new_n123_), .b(x21), .c(new_n120_), .d(x19), .O(new_n124_));
  oai21  g080(.a(new_n81_), .b(new_n96_), .c(x23), .O(new_n125_));
  oai21  g081(.a(new_n124_), .b(new_n81_), .c(new_n125_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n46_), .c(x09), .d(new_n62_), .O(new_n127_));
  aoi21  g083(.a(x08), .b(x04), .c(new_n56_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n127_), .O(z13));
  nand2  g085(.a(new_n63_), .b(x05), .O(new_n130_));
  nand3  g086(.a(x26), .b(x25), .c(x16), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n122_), .c(x23), .d(x22), .O(new_n132_));
  inv1   g088(.a(x24), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x23), .c(x22), .O(new_n134_));
  nor2   g090(.a(new_n134_), .b(new_n121_), .O(new_n135_));
  aoi21  g091(.a(new_n132_), .b(x24), .c(new_n135_), .O(new_n136_));
  nor2   g092(.a(x25), .b(new_n133_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n96_), .O(new_n138_));
  oai21  g094(.a(new_n136_), .b(new_n96_), .c(new_n138_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n46_), .c(x09), .d(new_n62_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n130_), .O(z14));
  nand2  g097(.a(new_n63_), .b(x06), .O(new_n142_));
  nand3  g098(.a(x22), .b(x20), .c(x19), .O(new_n143_));
  nand2  g099(.a(x26), .b(x17), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(x24), .c(x23), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n143_), .c(x25), .O(new_n146_));
  inv1   g102(.a(new_n143_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n137_), .c(x23), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n146_), .c(new_n96_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n46_), .c(x09), .d(new_n62_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n142_), .O(z15));
  nor2   g107(.a(new_n121_), .b(x18), .O(new_n152_));
  nand3  g108(.a(x24), .b(x23), .c(x22), .O(new_n153_));
  inv1   g109(.a(new_n153_), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n152_), .c(new_n45_), .O(new_n155_));
  nand4  g111(.a(new_n45_), .b(x25), .c(x24), .d(x23), .O(new_n156_));
  nor2   g112(.a(new_n156_), .b(new_n143_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n155_), .c(x21), .O(new_n158_));
  oai21  g114(.a(new_n45_), .b(x25), .c(new_n158_), .O(new_n159_));
  nand4  g115(.a(new_n159_), .b(new_n46_), .c(x09), .d(new_n62_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n73_), .O(z16));
endmodule


