// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(x24), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n54_), .O(z01));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n58_), .b(new_n60_), .O(z02));
  inv1   g017(.a(x02), .O(new_n62_));
  nor2   g018(.a(new_n58_), .b(new_n62_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n57_), .O(z04));
  inv1   g021(.a(x04), .O(new_n66_));
  oai21  g022(.a(new_n46_), .b(new_n66_), .c(new_n57_), .O(z05));
  inv1   g023(.a(x05), .O(new_n68_));
  nor2   g024(.a(new_n58_), .b(new_n68_), .O(z06));
  inv1   g025(.a(x06), .O(new_n70_));
  nor2   g026(.a(new_n58_), .b(new_n70_), .O(z07));
  nand3  g027(.a(new_n57_), .b(x08), .c(x07), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  nand2  g029(.a(x21), .b(x20), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x11), .O(new_n76_));
  nand2  g032(.a(x26), .b(x25), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(x23), .c(x22), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n76_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n81_));
  aoi21  g037(.a(x08), .b(x00), .c(new_n56_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(z09));
  inv1   g039(.a(x20), .O(new_n84_));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  nand3  g041(.a(x26), .b(x25), .c(x23), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n85_), .c(x20), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x19), .O(new_n88_));
  oai21  g044(.a(new_n84_), .b(x19), .c(new_n88_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n90_));
  aoi21  g046(.a(x08), .b(x01), .c(new_n56_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(z10));
  inv1   g048(.a(x19), .O(new_n93_));
  nand2  g049(.a(x22), .b(x13), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n86_), .c(x21), .O(new_n95_));
  inv1   g051(.a(x21), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(x20), .O(new_n97_));
  aoi21  g053(.a(new_n95_), .b(x20), .c(new_n97_), .O(new_n98_));
  nand2  g054(.a(x21), .b(new_n93_), .O(new_n99_));
  oai21  g055(.a(new_n98_), .b(new_n93_), .c(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n101_));
  aoi21  g057(.a(x08), .b(x02), .c(new_n56_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n101_), .O(z11));
  nand2  g059(.a(x09), .b(new_n46_), .O(new_n104_));
  nand3  g060(.a(x22), .b(new_n93_), .c(new_n47_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n104_), .c(new_n64_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n57_), .O(new_n107_));
  inv1   g063(.a(x24), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(x23), .c(x22), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x21), .c(x20), .O(new_n110_));
  nand4  g066(.a(new_n74_), .b(x24), .c(x23), .d(x22), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x19), .O(new_n113_));
  nor2   g069(.a(new_n75_), .b(x23), .O(new_n114_));
  nand2  g070(.a(x23), .b(x14), .O(new_n115_));
  nor3   g071(.a(new_n115_), .b(new_n77_), .c(new_n108_), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n114_), .c(x22), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n107_), .O(z12));
  nand3  g076(.a(x26), .b(x25), .c(x15), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n75_), .c(x22), .d(x19), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x24), .c(x23), .O(new_n123_));
  nand2  g079(.a(x20), .b(x19), .O(new_n124_));
  nand3  g080(.a(new_n55_), .b(x22), .c(x21), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n124_), .c(new_n123_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  oai21  g083(.a(new_n58_), .b(new_n66_), .c(new_n127_), .O(z13));
  nand3  g084(.a(x21), .b(x20), .c(x19), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  nand2  g086(.a(new_n78_), .b(x16), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n130_), .c(x23), .d(x22), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(x24), .c(new_n47_), .d(x09), .O(new_n133_));
  oai22  g089(.a(new_n133_), .b(x08), .c(new_n58_), .d(new_n68_), .O(z14));
  nand4  g090(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n135_));
  inv1   g091(.a(x25), .O(new_n136_));
  nand3  g092(.a(new_n75_), .b(new_n136_), .c(x22), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n135_), .c(x24), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(x23), .O(new_n139_));
  nand2  g095(.a(x26), .b(x17), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(x23), .c(x22), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n130_), .c(new_n136_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n144_));
  nand2  g100(.a(x08), .b(x06), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n144_), .c(new_n139_), .O(z15));
  nor2   g102(.a(new_n124_), .b(x18), .O(new_n147_));
  nand3  g103(.a(x25), .b(x22), .c(x21), .O(new_n148_));
  inv1   g104(.a(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n147_), .c(new_n45_), .O(new_n150_));
  nand4  g106(.a(new_n45_), .b(x25), .c(x23), .d(x22), .O(new_n151_));
  nor2   g107(.a(new_n151_), .b(new_n129_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x24), .O(new_n153_));
  nand2  g109(.a(x26), .b(new_n55_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n72_), .O(z16));
endmodule


