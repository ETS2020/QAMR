// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nor2   g010(.a(x23), .b(x22), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g014(.a(new_n55_), .O(new_n59_));
  nand2  g015(.a(x08), .b(x01), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(z02));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n59_), .O(z03));
  and2   g019(.a(new_n56_), .b(x03), .O(z04));
  inv1   g020(.a(x04), .O(new_n65_));
  nor2   g021(.a(new_n57_), .b(new_n65_), .O(z05));
  nand2  g022(.a(new_n56_), .b(x05), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z06));
  nand2  g024(.a(new_n56_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  nand2  g026(.a(x08), .b(x07), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n59_), .O(z08));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x09), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(x08), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n73_), .c(new_n47_), .O(new_n76_));
  oai21  g032(.a(new_n46_), .b(new_n54_), .c(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n59_), .O(new_n78_));
  nand2  g034(.a(x20), .b(x11), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x10), .O(new_n80_));
  inv1   g036(.a(x21), .O(new_n81_));
  inv1   g037(.a(x22), .O(new_n82_));
  inv1   g038(.a(x23), .O(new_n83_));
  nand3  g039(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  nor4   g040(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n80_), .c(new_n75_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n78_), .O(z09));
  xor2a  g043(.a(x20), .b(x19), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n59_), .O(new_n91_));
  nand2  g047(.a(x19), .b(x12), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x10), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n85_), .c(new_n75_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n91_), .O(z10));
  xnor2a g051(.a(x21), .b(x20), .O(new_n96_));
  nand2  g052(.a(x21), .b(new_n73_), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n73_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n62_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n59_), .O(new_n101_));
  nand2  g057(.a(x19), .b(x13), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(x10), .O(new_n103_));
  nand3  g059(.a(x23), .b(x22), .c(x20), .O(new_n104_));
  nor2   g060(.a(new_n104_), .b(new_n84_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n103_), .c(new_n75_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n101_), .O(z11));
  nand3  g063(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n108_));
  nand3  g064(.a(x21), .b(x20), .c(x19), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n108_), .c(x23), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n82_), .O(new_n111_));
  nand2  g067(.a(x20), .b(x19), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  inv1   g069(.a(new_n84_), .O(new_n114_));
  and2   g070(.a(x23), .b(x14), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(new_n81_), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n113_), .c(new_n82_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n118_));
  nand2  g074(.a(x08), .b(x03), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n118_), .c(new_n111_), .O(z12));
  inv1   g076(.a(new_n109_), .O(new_n121_));
  nand4  g077(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n121_), .c(x22), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x23), .O(new_n124_));
  nand4  g080(.a(new_n113_), .b(new_n83_), .c(x22), .d(x21), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  oai21  g083(.a(new_n57_), .b(new_n65_), .c(new_n127_), .O(z13));
  nand2  g084(.a(x24), .b(new_n82_), .O(new_n129_));
  inv1   g085(.a(x24), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x22), .c(x21), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n112_), .c(new_n129_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x23), .O(new_n133_));
  nand3  g089(.a(x26), .b(x25), .c(x16), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n113_), .c(x23), .d(x21), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x24), .c(x22), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n67_), .O(z14));
  nand2  g095(.a(x25), .b(new_n82_), .O(new_n140_));
  inv1   g096(.a(x25), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(x24), .c(x22), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n109_), .c(new_n140_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x23), .O(new_n144_));
  nand2  g100(.a(x26), .b(x17), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n121_), .c(x24), .d(x23), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(x25), .c(x22), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n69_), .O(z15));
  nand2  g106(.a(new_n56_), .b(x07), .O(new_n151_));
  nand2  g107(.a(x26), .b(new_n82_), .O(new_n152_));
  nand4  g108(.a(new_n45_), .b(x25), .c(x24), .d(x22), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n109_), .c(new_n152_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(x23), .O(new_n155_));
  nor2   g111(.a(new_n112_), .b(x18), .O(new_n156_));
  nor2   g112(.a(new_n141_), .b(new_n130_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n156_), .c(x23), .d(x21), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(x26), .c(x22), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n151_), .O(z16));
endmodule


