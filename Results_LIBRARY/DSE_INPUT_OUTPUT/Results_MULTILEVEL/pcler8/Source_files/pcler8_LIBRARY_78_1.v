// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x24), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x20), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x21), .O(new_n50_));
  inv1   g006(.a(x22), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g008(.a(x25), .O(new_n53_));
  inv1   g009(.a(x26), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand4  g011(.a(new_n55_), .b(new_n52_), .c(new_n49_), .d(x23), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(x19), .c(new_n45_), .O(z00));
  nor2   g013(.a(new_n45_), .b(x19), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(new_n59_));
  nand2  g015(.a(x08), .b(x00), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(z01));
  inv1   g017(.a(x01), .O(new_n62_));
  nor2   g018(.a(new_n58_), .b(new_n46_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nor2   g020(.a(new_n64_), .b(new_n62_), .O(z02));
  inv1   g021(.a(x02), .O(new_n66_));
  nor2   g022(.a(new_n64_), .b(new_n66_), .O(z03));
  nand2  g023(.a(x08), .b(x03), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n59_), .O(z04));
  nand2  g025(.a(x08), .b(x04), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n59_), .O(z05));
  nand2  g027(.a(x08), .b(x05), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n59_), .O(z06));
  nand2  g029(.a(x08), .b(x06), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n59_), .O(z07));
  nand2  g031(.a(new_n63_), .b(x07), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(z08));
  nand2  g033(.a(new_n63_), .b(x00), .O(new_n78_));
  nand2  g034(.a(x21), .b(x20), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  inv1   g036(.a(x23), .O(new_n81_));
  nand3  g037(.a(x26), .b(x25), .c(x24), .O(new_n82_));
  nor3   g038(.a(new_n82_), .b(new_n81_), .c(new_n51_), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n80_), .c(x19), .d(x11), .O(new_n84_));
  oai21  g040(.a(x24), .b(x19), .c(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n78_), .O(z09));
  nand2  g043(.a(new_n52_), .b(x12), .O(new_n88_));
  nand3  g044(.a(new_n55_), .b(x24), .c(x23), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x19), .O(new_n91_));
  inv1   g047(.a(x19), .O(new_n92_));
  nand3  g048(.a(new_n45_), .b(x20), .c(new_n92_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  oai21  g051(.a(new_n64_), .b(new_n62_), .c(new_n95_), .O(z10));
  nand3  g052(.a(x23), .b(x22), .c(x13), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n82_), .c(x21), .O(new_n98_));
  nor2   g054(.a(new_n50_), .b(x20), .O(new_n99_));
  aoi21  g055(.a(new_n98_), .b(x20), .c(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n45_), .b(x21), .c(new_n92_), .O(new_n101_));
  oai21  g057(.a(new_n100_), .b(new_n92_), .c(new_n101_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n103_));
  oai21  g059(.a(new_n64_), .b(new_n66_), .c(new_n103_), .O(z11));
  nand2  g060(.a(new_n63_), .b(x03), .O(new_n105_));
  inv1   g061(.a(x20), .O(new_n106_));
  nand2  g062(.a(x23), .b(x14), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n82_), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n106_), .c(x22), .O(new_n109_));
  nand3  g065(.a(new_n51_), .b(x21), .c(x20), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x19), .O(new_n112_));
  nand3  g068(.a(new_n45_), .b(x22), .c(new_n92_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n105_), .O(z12));
  nand2  g072(.a(new_n63_), .b(x04), .O(new_n117_));
  nand4  g073(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x23), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x21), .c(x20), .O(new_n120_));
  nand2  g076(.a(new_n79_), .b(x23), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n120_), .c(new_n51_), .O(new_n122_));
  nor2   g078(.a(new_n81_), .b(x22), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n122_), .c(x19), .O(new_n124_));
  nor2   g080(.a(x24), .b(new_n81_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n92_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n117_), .O(z13));
  nand2  g085(.a(new_n55_), .b(x16), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x23), .c(x22), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n79_), .c(x24), .O(new_n132_));
  nand4  g088(.a(new_n125_), .b(new_n80_), .c(x22), .d(x19), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n72_), .c(new_n59_), .O(z14));
  nand2  g092(.a(new_n63_), .b(x06), .O(new_n137_));
  nand2  g093(.a(x26), .b(x17), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n80_), .c(x23), .d(x22), .O(new_n139_));
  nand3  g095(.a(x22), .b(x21), .c(x20), .O(new_n140_));
  nand3  g096(.a(new_n53_), .b(x24), .c(x23), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  aoi21  g098(.a(new_n139_), .b(x25), .c(new_n142_), .O(new_n143_));
  oai22  g099(.a(new_n143_), .b(new_n92_), .c(new_n53_), .d(x24), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n137_), .O(z15));
  nand3  g102(.a(x25), .b(x23), .c(x22), .O(new_n147_));
  nor3   g103(.a(new_n147_), .b(new_n79_), .c(x18), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n54_), .O(new_n149_));
  nand4  g105(.a(new_n54_), .b(x25), .c(x24), .d(x23), .O(new_n150_));
  nor2   g106(.a(new_n150_), .b(new_n140_), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n149_), .c(x19), .O(new_n152_));
  nand2  g108(.a(x26), .b(new_n45_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n76_), .O(z16));
endmodule


