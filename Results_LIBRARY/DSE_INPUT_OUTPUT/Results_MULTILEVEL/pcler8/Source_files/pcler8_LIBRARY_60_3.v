// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n55_, new_n57_, new_n59_, new_n60_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  inv1   g02(.a(x25), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(x00), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z01));
  nand2  g07(.a(new_n50_), .b(x01), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(z02));
  nand2  g09(.a(new_n50_), .b(x02), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(z03));
  nand2  g11(.a(new_n50_), .b(x03), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(z04));
  inv1   g13(.a(new_n49_), .O(new_n59_));
  nand2  g14(.a(x08), .b(x04), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(z05));
  nand2  g16(.a(new_n50_), .b(x05), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(z06));
  nand2  g18(.a(new_n50_), .b(x06), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(z07));
  nand2  g20(.a(new_n50_), .b(x07), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z08));
  nand2  g22(.a(x08), .b(x00), .O(new_n68_));
  inv1   g23(.a(x10), .O(new_n69_));
  inv1   g24(.a(x19), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(x09), .d(new_n46_), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n68_), .c(new_n59_), .O(z09));
  xor2a  g27(.a(x20), .b(x19), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n69_), .c(x09), .d(new_n46_), .O(new_n74_));
  nand2  g29(.a(x08), .b(x01), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n74_), .c(new_n59_), .O(z10));
  inv1   g31(.a(x20), .O(new_n77_));
  xnor2a g32(.a(x21), .b(x19), .O(new_n78_));
  nand2  g33(.a(x21), .b(new_n77_), .O(new_n79_));
  oai21  g34(.a(new_n78_), .b(new_n77_), .c(new_n79_), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n69_), .c(x09), .d(new_n46_), .O(new_n81_));
  aoi21  g36(.a(x08), .b(x02), .c(new_n49_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(z11));
  nand2  g38(.a(x25), .b(x23), .O(new_n84_));
  inv1   g39(.a(x22), .O(new_n85_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand2  g42(.a(x20), .b(x19), .O(new_n88_));
  nand2  g43(.a(new_n85_), .b(x21), .O(new_n89_));
  oai22  g44(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n85_), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n84_), .c(new_n69_), .d(x09), .O(new_n91_));
  aoi21  g46(.a(x08), .b(x03), .c(new_n49_), .O(new_n92_));
  oai21  g47(.a(new_n91_), .b(x08), .c(new_n92_), .O(z12));
  nand2  g48(.a(new_n50_), .b(x04), .O(new_n94_));
  nand4  g49(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n48_), .c(x23), .O(new_n96_));
  nand3  g51(.a(new_n47_), .b(x22), .c(x21), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n88_), .c(new_n96_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n69_), .c(x09), .d(new_n46_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n94_), .O(z13));
  inv1   g55(.a(x24), .O(new_n101_));
  aoi21  g56(.a(new_n95_), .b(new_n48_), .c(new_n47_), .O(new_n102_));
  and2   g57(.a(x23), .b(x22), .O(new_n103_));
  nor2   g58(.a(x25), .b(x24), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n87_), .O(new_n105_));
  oai21  g60(.a(new_n102_), .b(new_n101_), .c(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n69_), .c(x09), .d(new_n46_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n62_), .O(z14));
  nand4  g63(.a(new_n48_), .b(x24), .c(x23), .d(x22), .O(new_n109_));
  oai22  g64(.a(new_n109_), .b(new_n86_), .c(new_n48_), .d(x23), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n69_), .c(x09), .d(new_n46_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n64_), .O(z15));
  nand2  g67(.a(x08), .b(x07), .O(new_n113_));
  nand4  g68(.a(x26), .b(new_n69_), .c(x09), .d(new_n46_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(new_n49_), .O(z16));
  zero   g70(.O(z00));
endmodule


