// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x13), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  inv1   g011(.a(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(x13), .O(new_n57_));
  nor3   g013(.a(new_n57_), .b(new_n47_), .c(new_n55_), .O(z01));
  nor2   g014(.a(new_n57_), .b(new_n47_), .O(new_n59_));
  and2   g015(.a(new_n59_), .b(x01), .O(z02));
  inv1   g016(.a(new_n57_), .O(new_n61_));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n61_), .O(z04));
  nand2  g021(.a(x08), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n61_), .O(z05));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n61_), .O(z06));
  nand2  g025(.a(x08), .b(x06), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n61_), .O(z07));
  and2   g027(.a(new_n59_), .b(x07), .O(z08));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x09), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(x08), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n73_), .c(new_n48_), .O(new_n76_));
  oai21  g032(.a(new_n47_), .b(new_n55_), .c(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n61_), .O(new_n78_));
  nand3  g034(.a(x20), .b(x13), .c(x11), .O(new_n79_));
  nor4   g035(.a(new_n79_), .b(x10), .c(new_n74_), .d(x08), .O(new_n80_));
  and2   g036(.a(x23), .b(x22), .O(new_n81_));
  inv1   g037(.a(x25), .O(new_n82_));
  nor2   g038(.a(new_n45_), .b(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x24), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n81_), .c(new_n80_), .d(x21), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n78_), .O(z09));
  inv1   g043(.a(x20), .O(new_n88_));
  nand3  g044(.a(x22), .b(x21), .c(x12), .O(new_n89_));
  nand4  g045(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(x19), .O(new_n92_));
  oai21  g048(.a(new_n88_), .b(x19), .c(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n94_));
  aoi21  g050(.a(x08), .b(x01), .c(new_n57_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n94_), .O(z10));
  inv1   g052(.a(x21), .O(new_n97_));
  nand2  g053(.a(x20), .b(x19), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n97_), .b(x20), .c(x19), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n62_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n61_), .O(new_n104_));
  nand2  g060(.a(x21), .b(x13), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(x10), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n85_), .c(new_n81_), .d(new_n75_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n104_), .O(z11));
  nand3  g064(.a(new_n75_), .b(x14), .c(new_n48_), .O(new_n109_));
  nand2  g065(.a(new_n83_), .b(new_n81_), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n109_), .c(x13), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x24), .O(new_n112_));
  nand3  g068(.a(x21), .b(x20), .c(x19), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  nand2  g070(.a(new_n46_), .b(x21), .O(new_n115_));
  oai22  g071(.a(new_n115_), .b(new_n98_), .c(new_n114_), .d(new_n46_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n112_), .c(new_n64_), .O(z12));
  nand2  g074(.a(x22), .b(x21), .O(new_n119_));
  oai21  g075(.a(new_n98_), .b(new_n119_), .c(x23), .O(new_n120_));
  inv1   g076(.a(x23), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x22), .c(x21), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n98_), .c(new_n120_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n66_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n61_), .O(new_n126_));
  nand3  g082(.a(new_n75_), .b(x13), .c(new_n48_), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n85_), .c(x23), .d(x15), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n126_), .O(z13));
  nand3  g086(.a(x26), .b(x25), .c(x16), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x23), .c(x22), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n113_), .c(x24), .O(new_n133_));
  nand4  g089(.a(new_n114_), .b(new_n56_), .c(x23), .d(x22), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n68_), .c(new_n61_), .O(z14));
  nand2  g093(.a(new_n59_), .b(x06), .O(new_n138_));
  nand2  g094(.a(x26), .b(x17), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x23), .c(x22), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n113_), .c(x25), .O(new_n141_));
  nand4  g097(.a(new_n114_), .b(new_n81_), .c(new_n82_), .d(x24), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x13), .O(new_n144_));
  nand2  g100(.a(x25), .b(new_n56_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n138_), .O(z15));
  nand3  g104(.a(new_n99_), .b(new_n75_), .c(new_n48_), .O(new_n149_));
  inv1   g105(.a(new_n119_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n45_), .c(x25), .d(x23), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n149_), .c(x13), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x24), .O(new_n153_));
  inv1   g109(.a(x18), .O(new_n154_));
  nand4  g110(.a(x21), .b(x20), .c(x19), .d(new_n154_), .O(new_n155_));
  nand3  g111(.a(new_n81_), .b(x25), .c(x24), .O(new_n156_));
  nor2   g112(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nor2   g113(.a(new_n157_), .b(new_n45_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n159_));
  nand2  g115(.a(x08), .b(x07), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n159_), .c(new_n153_), .O(z16));
endmodule


