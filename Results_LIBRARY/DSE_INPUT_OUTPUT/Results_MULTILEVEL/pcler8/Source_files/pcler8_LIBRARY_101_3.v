// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x14), .O(new_n45_));
  inv1   g001(.a(x19), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand2  g004(.a(x21), .b(x20), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n51_));
  and2   g007(.a(x23), .b(x22), .O(new_n52_));
  nand3  g008(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  or2    g011(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n45_), .c(new_n46_), .O(z00));
  nor2   g013(.a(new_n46_), .b(new_n45_), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x00), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z01));
  nand2  g017(.a(new_n59_), .b(x01), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z02));
  inv1   g019(.a(new_n58_), .O(new_n64_));
  nand2  g020(.a(x08), .b(x02), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n64_), .O(z03));
  nand2  g022(.a(x08), .b(x03), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n64_), .O(z04));
  nand2  g024(.a(new_n59_), .b(x04), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z05));
  nand2  g026(.a(new_n59_), .b(x05), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z06));
  nand2  g028(.a(x08), .b(x06), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n64_), .O(z07));
  nand2  g030(.a(new_n59_), .b(x07), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(z08));
  and2   g032(.a(x22), .b(x21), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n81_));
  aoi21  g037(.a(x08), .b(x00), .c(new_n58_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(z09));
  inv1   g039(.a(x20), .O(new_n84_));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n79_), .c(x20), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(x19), .c(new_n45_), .O(new_n87_));
  oai21  g043(.a(new_n84_), .b(x19), .c(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n62_), .O(z10));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n53_), .c(x21), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(x20), .c(x19), .O(new_n93_));
  oai21  g049(.a(new_n84_), .b(new_n46_), .c(x21), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n65_), .c(new_n64_), .O(z11));
  nand2  g053(.a(new_n59_), .b(x03), .O(new_n98_));
  inv1   g054(.a(x22), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  oai21  g056(.a(new_n50_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n45_), .O(new_n102_));
  oai21  g058(.a(new_n99_), .b(x19), .c(new_n102_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n98_), .O(z12));
  nand3  g061(.a(new_n49_), .b(x23), .c(new_n45_), .O(new_n106_));
  nand4  g062(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x23), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x21), .c(x20), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x22), .c(x19), .O(new_n111_));
  oai21  g067(.a(new_n99_), .b(new_n46_), .c(x23), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n114_));
  aoi21  g070(.a(x08), .b(x04), .c(new_n58_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(z13));
  nand4  g072(.a(x20), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n117_));
  inv1   g073(.a(x24), .O(new_n118_));
  nand3  g074(.a(new_n77_), .b(new_n118_), .c(x23), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n117_), .c(new_n45_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x19), .O(new_n121_));
  nand2  g077(.a(new_n50_), .b(x19), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(x26), .b(x25), .c(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(x22), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n123_), .c(new_n118_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n128_));
  nand2  g084(.a(x08), .b(x05), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n128_), .c(new_n121_), .O(z14));
  inv1   g086(.a(x25), .O(new_n131_));
  nand4  g087(.a(new_n77_), .b(new_n131_), .c(x24), .d(x23), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n117_), .c(new_n45_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x19), .O(new_n134_));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n52_), .c(x24), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n123_), .c(new_n131_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n134_), .c(new_n73_), .O(z15));
  inv1   g096(.a(x26), .O(new_n141_));
  nand4  g097(.a(new_n52_), .b(new_n141_), .c(x25), .d(x24), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n51_), .c(new_n45_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x19), .O(new_n144_));
  inv1   g100(.a(x18), .O(new_n145_));
  nand3  g101(.a(new_n50_), .b(x19), .c(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n52_), .b(x25), .c(x24), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n141_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n150_));
  nand2  g106(.a(x08), .b(x07), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n150_), .c(new_n144_), .O(z16));
endmodule


