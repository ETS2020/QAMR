// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n53_, new_n55_,
    new_n57_, new_n59_, new_n61_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_;
  inv1   g00(.a(x10), .O(new_n45_));
  nand2  g01(.a(x22), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(z00));
  inv1   g03(.a(x00), .O(new_n48_));
  nand2  g04(.a(new_n46_), .b(x08), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(z01));
  nand2  g06(.a(x08), .b(x01), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n46_), .O(z02));
  nand2  g08(.a(x08), .b(x02), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n46_), .O(z03));
  inv1   g10(.a(x03), .O(new_n55_));
  nor2   g11(.a(new_n49_), .b(new_n55_), .O(z04));
  nand2  g12(.a(x08), .b(x04), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z05));
  inv1   g14(.a(x05), .O(new_n59_));
  nor2   g15(.a(new_n49_), .b(new_n59_), .O(z06));
  nand2  g16(.a(x08), .b(x06), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n46_), .O(z07));
  inv1   g18(.a(x07), .O(new_n63_));
  nor2   g19(.a(new_n49_), .b(new_n63_), .O(z08));
  inv1   g20(.a(x08), .O(new_n65_));
  nor2   g21(.a(x22), .b(x19), .O(new_n66_));
  nand4  g22(.a(new_n66_), .b(new_n45_), .c(x09), .d(new_n65_), .O(new_n67_));
  oai21  g23(.a(new_n49_), .b(new_n48_), .c(new_n67_), .O(z09));
  inv1   g24(.a(x22), .O(new_n69_));
  xor2a  g25(.a(x20), .b(x19), .O(new_n70_));
  nand3  g26(.a(new_n70_), .b(x09), .c(new_n65_), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n45_), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n51_), .O(z10));
  inv1   g30(.a(x09), .O(new_n75_));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(x21), .O(new_n77_));
  inv1   g33(.a(x21), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(x20), .c(x19), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n65_), .c(x22), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(x10), .c(new_n53_), .O(z11));
  nand3  g38(.a(new_n45_), .b(x09), .c(new_n65_), .O(new_n83_));
  nand4  g39(.a(new_n69_), .b(x21), .c(x20), .d(x19), .O(new_n84_));
  oai22  g40(.a(new_n84_), .b(new_n83_), .c(new_n49_), .d(new_n55_), .O(z12));
  nand2  g41(.a(x23), .b(x09), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(x08), .c(new_n69_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n45_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n57_), .O(z13));
  nand2  g45(.a(x24), .b(x09), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(x08), .c(new_n69_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n45_), .O(new_n92_));
  oai21  g48(.a(new_n65_), .b(new_n59_), .c(new_n92_), .O(z14));
  nand2  g49(.a(x25), .b(x09), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(x08), .c(new_n69_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n45_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n61_), .O(z15));
  nand2  g53(.a(x26), .b(x09), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x08), .c(new_n69_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n45_), .O(new_n100_));
  oai21  g56(.a(new_n65_), .b(new_n63_), .c(new_n100_), .O(z16));
endmodule


