// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x24), .b(x02), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nand2  g011(.a(x08), .b(x00), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g015(.a(x02), .O(new_n60_));
  nor2   g016(.a(new_n46_), .b(new_n60_), .O(z03));
  inv1   g017(.a(x03), .O(new_n62_));
  nand2  g018(.a(new_n55_), .b(x08), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n62_), .O(z04));
  inv1   g020(.a(x04), .O(new_n65_));
  nor2   g021(.a(new_n63_), .b(new_n65_), .O(z05));
  nand2  g022(.a(x08), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n55_), .O(z06));
  nand2  g024(.a(x08), .b(x06), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n55_), .O(z07));
  inv1   g026(.a(x07), .O(new_n71_));
  nor2   g027(.a(new_n63_), .b(new_n71_), .O(z08));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x09), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(x08), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n73_), .c(new_n47_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n56_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand2  g034(.a(x20), .b(x11), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x10), .O(new_n80_));
  inv1   g036(.a(x22), .O(new_n81_));
  inv1   g037(.a(x23), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x21), .O(new_n84_));
  nand3  g040(.a(x26), .b(x25), .c(x24), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n80_), .c(new_n75_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n78_), .O(z09));
  inv1   g044(.a(x20), .O(new_n89_));
  nand3  g045(.a(x22), .b(x21), .c(x12), .O(new_n90_));
  nand4  g046(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n90_), .c(x20), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(x19), .O(new_n93_));
  oai21  g049(.a(new_n89_), .b(x19), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n58_), .c(new_n55_), .O(z10));
  inv1   g052(.a(z03), .O(new_n97_));
  inv1   g053(.a(x24), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(x08), .c(new_n60_), .O(new_n99_));
  inv1   g055(.a(x21), .O(new_n100_));
  nor2   g056(.a(new_n89_), .b(new_n73_), .O(new_n101_));
  nand3  g057(.a(new_n100_), .b(x20), .c(x19), .O(new_n102_));
  oai21  g058(.a(new_n101_), .b(new_n100_), .c(new_n102_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  nor2   g060(.a(new_n81_), .b(new_n100_), .O(new_n105_));
  inv1   g061(.a(new_n91_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n105_), .c(x13), .d(new_n46_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n47_), .c(x09), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n97_), .O(z11));
  inv1   g066(.a(new_n101_), .O(new_n111_));
  nand2  g067(.a(x23), .b(x14), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n85_), .c(x21), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x22), .O(new_n114_));
  nand3  g070(.a(new_n101_), .b(new_n81_), .c(x21), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n117_));
  aoi21  g073(.a(x08), .b(x03), .c(new_n54_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n117_), .O(z12));
  nand4  g075(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x22), .c(x21), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n111_), .c(x23), .O(new_n122_));
  nand4  g078(.a(new_n101_), .b(new_n82_), .c(x22), .d(x21), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n125_));
  aoi21  g081(.a(x08), .b(x04), .c(new_n54_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n125_), .O(z13));
  nand3  g083(.a(new_n75_), .b(x19), .c(new_n47_), .O(new_n128_));
  nor2   g084(.a(new_n100_), .b(new_n89_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n98_), .c(x23), .d(x22), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n128_), .c(new_n67_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x02), .O(new_n132_));
  nand3  g088(.a(x21), .b(x20), .c(x19), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand3  g090(.a(x26), .b(x25), .c(x16), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n134_), .c(x23), .d(x22), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n67_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(x24), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n132_), .O(z14));
  nand3  g096(.a(new_n75_), .b(x25), .c(new_n47_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x02), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n98_), .O(new_n143_));
  nand2  g099(.a(x26), .b(x17), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(x23), .c(x22), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n133_), .c(x25), .O(new_n146_));
  inv1   g102(.a(x25), .O(new_n147_));
  nand4  g103(.a(new_n134_), .b(new_n83_), .c(new_n147_), .d(x24), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n143_), .c(new_n69_), .O(z15));
  nand3  g107(.a(new_n75_), .b(x26), .c(new_n47_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x02), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n98_), .O(new_n154_));
  inv1   g110(.a(x18), .O(new_n155_));
  nand3  g111(.a(x20), .b(x19), .c(new_n155_), .O(new_n156_));
  nand4  g112(.a(x25), .b(x23), .c(x22), .d(x21), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n156_), .c(x26), .O(new_n158_));
  nor2   g114(.a(new_n98_), .b(new_n82_), .O(new_n159_));
  nor2   g115(.a(x26), .b(new_n147_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n101_), .c(new_n159_), .d(new_n105_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n158_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n163_));
  nand2  g119(.a(x08), .b(x07), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n163_), .c(new_n154_), .O(z16));
endmodule


