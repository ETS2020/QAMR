// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n67_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nor2   g010(.a(x24), .b(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n54_), .O(z01));
  and2   g014(.a(new_n56_), .b(x01), .O(z02));
  aoi21  g015(.a(x08), .b(x02), .c(new_n55_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z03));
  inv1   g017(.a(x03), .O(new_n62_));
  nor2   g018(.a(new_n57_), .b(new_n62_), .O(z04));
  nand2  g019(.a(new_n56_), .b(x04), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z05));
  inv1   g021(.a(x05), .O(new_n66_));
  inv1   g022(.a(new_n55_), .O(new_n67_));
  oai21  g023(.a(new_n46_), .b(new_n66_), .c(new_n67_), .O(z06));
  and2   g024(.a(new_n56_), .b(x06), .O(z07));
  nand2  g025(.a(x08), .b(x07), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n67_), .O(z08));
  inv1   g027(.a(x19), .O(new_n72_));
  inv1   g028(.a(x09), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(x08), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(new_n72_), .c(new_n47_), .O(new_n75_));
  oai21  g031(.a(new_n46_), .b(new_n54_), .c(new_n75_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n67_), .O(new_n77_));
  nand2  g033(.a(x20), .b(x11), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(x10), .O(new_n79_));
  inv1   g035(.a(x21), .O(new_n80_));
  inv1   g036(.a(x22), .O(new_n81_));
  inv1   g037(.a(x23), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand3  g040(.a(x26), .b(x25), .c(x24), .O(new_n85_));
  nor3   g041(.a(new_n85_), .b(new_n84_), .c(new_n80_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n79_), .c(new_n74_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n77_), .O(z09));
  nand3  g044(.a(x22), .b(x21), .c(x12), .O(new_n89_));
  nand4  g045(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(x19), .O(new_n92_));
  nand2  g048(.a(x20), .b(new_n72_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  aoi21  g051(.a(x08), .b(x01), .c(new_n55_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n95_), .O(z10));
  nand3  g053(.a(x23), .b(x22), .c(x13), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n85_), .c(x21), .O(new_n99_));
  nor2   g055(.a(new_n80_), .b(x20), .O(new_n100_));
  aoi21  g056(.a(new_n99_), .b(x20), .c(new_n100_), .O(new_n101_));
  nand2  g057(.a(x21), .b(new_n72_), .O(new_n102_));
  oai21  g058(.a(new_n101_), .b(new_n72_), .c(new_n102_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n60_), .O(z11));
  nand3  g061(.a(x21), .b(x20), .c(x19), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x22), .O(new_n107_));
  nand4  g063(.a(new_n81_), .b(x21), .c(x20), .d(x19), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n110_));
  oai21  g066(.a(new_n46_), .b(new_n62_), .c(new_n110_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n67_), .O(new_n112_));
  nor2   g068(.a(new_n85_), .b(new_n84_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n74_), .c(x14), .d(new_n47_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n112_), .O(z12));
  nand2  g071(.a(x20), .b(x19), .O(new_n116_));
  nand3  g072(.a(new_n82_), .b(x22), .c(x21), .O(new_n117_));
  nand4  g073(.a(x26), .b(x25), .c(x23), .d(x15), .O(new_n118_));
  oai21  g074(.a(new_n117_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x24), .O(new_n120_));
  oai21  g076(.a(new_n106_), .b(new_n81_), .c(x23), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n64_), .O(z13));
  nand3  g080(.a(x26), .b(x25), .c(x16), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x23), .c(x22), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n106_), .c(x24), .O(new_n127_));
  inv1   g083(.a(x24), .O(new_n128_));
  inv1   g084(.a(new_n106_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n128_), .c(x23), .d(x22), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n132_));
  oai21  g088(.a(new_n57_), .b(new_n66_), .c(new_n132_), .O(z14));
  nand3  g089(.a(new_n74_), .b(x25), .c(new_n47_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x24), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n82_), .O(new_n136_));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x24), .c(x22), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n106_), .c(x25), .O(new_n139_));
  inv1   g095(.a(x25), .O(new_n140_));
  nand4  g096(.a(new_n129_), .b(new_n83_), .c(new_n140_), .d(x24), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n143_));
  nand2  g099(.a(x08), .b(x06), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n143_), .c(new_n136_), .O(z15));
  nand2  g101(.a(new_n56_), .b(x07), .O(new_n146_));
  nand2  g102(.a(x26), .b(new_n82_), .O(new_n147_));
  nand4  g103(.a(new_n45_), .b(x25), .c(x23), .d(x22), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n106_), .c(new_n147_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x24), .O(new_n150_));
  nor2   g106(.a(new_n116_), .b(x18), .O(new_n151_));
  nor2   g107(.a(new_n140_), .b(new_n128_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n151_), .c(x22), .d(x21), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(x26), .c(x23), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n146_), .O(z16));
endmodule


