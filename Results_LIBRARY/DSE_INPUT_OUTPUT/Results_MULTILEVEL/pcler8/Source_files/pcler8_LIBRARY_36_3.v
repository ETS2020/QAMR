// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nand2  g010(.a(x26), .b(x10), .O(new_n55_));
  oai21  g011(.a(new_n46_), .b(new_n54_), .c(new_n55_), .O(z01));
  nand2  g012(.a(new_n55_), .b(x08), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n55_), .O(z03));
  nand2  g018(.a(new_n58_), .b(x03), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z04));
  nand2  g020(.a(new_n58_), .b(x04), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z05));
  nand2  g022(.a(new_n58_), .b(x05), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z06));
  nand2  g024(.a(new_n58_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  nand2  g026(.a(x08), .b(x07), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n55_), .O(z08));
  nand4  g028(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  nand4  g029(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand4  g031(.a(new_n75_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n76_));
  oai21  g032(.a(new_n57_), .b(new_n54_), .c(new_n76_), .O(z09));
  inv1   g033(.a(x20), .O(new_n78_));
  nand3  g034(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n74_), .c(x20), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(x19), .O(new_n81_));
  oai21  g037(.a(new_n78_), .b(x19), .c(new_n81_), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n59_), .O(z10));
  nand2  g040(.a(new_n58_), .b(x02), .O(new_n85_));
  inv1   g041(.a(x19), .O(new_n86_));
  nand3  g042(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  nand3  g043(.a(x26), .b(x25), .c(x24), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n87_), .c(x21), .O(new_n89_));
  inv1   g045(.a(x21), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(x20), .O(new_n91_));
  aoi21  g047(.a(new_n89_), .b(x20), .c(new_n91_), .O(new_n92_));
  nand2  g048(.a(x21), .b(new_n86_), .O(new_n93_));
  oai21  g049(.a(new_n92_), .b(new_n86_), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n85_), .O(z11));
  nand3  g052(.a(x14), .b(x09), .c(new_n46_), .O(new_n97_));
  inv1   g053(.a(x22), .O(new_n98_));
  inv1   g054(.a(x23), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(x25), .c(x24), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n97_), .c(new_n47_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(x26), .O(new_n103_));
  nand3  g059(.a(x21), .b(x20), .c(x19), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nor2   g061(.a(new_n78_), .b(new_n86_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n98_), .c(x21), .O(new_n107_));
  oai21  g063(.a(new_n105_), .b(new_n98_), .c(new_n107_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n109_));
  nand2  g065(.a(x08), .b(x03), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(new_n109_), .c(new_n103_), .O(z12));
  inv1   g067(.a(new_n106_), .O(new_n112_));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x22), .c(x21), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x23), .O(new_n115_));
  nand4  g071(.a(new_n106_), .b(new_n99_), .c(x22), .d(x21), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n65_), .O(z13));
  nand3  g075(.a(x26), .b(x25), .c(x16), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x23), .c(x22), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n104_), .c(x24), .O(new_n122_));
  nor2   g078(.a(x24), .b(new_n99_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n105_), .c(x22), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n67_), .O(z14));
  nand2  g083(.a(x26), .b(x17), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(x24), .c(x23), .d(x22), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n104_), .c(x25), .O(new_n130_));
  inv1   g086(.a(x25), .O(new_n131_));
  nand4  g087(.a(new_n105_), .b(new_n100_), .c(new_n131_), .d(x24), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n69_), .O(z15));
  inv1   g091(.a(x18), .O(new_n136_));
  nand4  g092(.a(x21), .b(x20), .c(x19), .d(new_n136_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n101_), .c(x26), .O(new_n138_));
  nand4  g094(.a(x21), .b(x20), .c(x19), .d(new_n47_), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  nor2   g096(.a(x26), .b(new_n131_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n140_), .c(new_n100_), .d(x24), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n138_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x09), .c(new_n46_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n71_), .c(new_n55_), .O(z16));
endmodule


