// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x23), .O(new_n54_));
  nand2  g010(.a(x24), .b(new_n54_), .O(new_n55_));
  nand2  g011(.a(x08), .b(x00), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n55_), .O(z02));
  nand2  g015(.a(x08), .b(x02), .O(new_n60_));
  and2   g016(.a(new_n60_), .b(new_n55_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(z03));
  aoi21  g018(.a(x24), .b(new_n54_), .c(new_n46_), .O(new_n63_));
  and2   g019(.a(new_n63_), .b(x03), .O(z04));
  inv1   g020(.a(x04), .O(new_n65_));
  inv1   g021(.a(new_n63_), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n65_), .O(z05));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n55_), .O(z06));
  and2   g025(.a(new_n63_), .b(x06), .O(z07));
  and2   g026(.a(new_n63_), .b(x07), .O(z08));
  inv1   g027(.a(x20), .O(new_n72_));
  inv1   g028(.a(x21), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n56_), .c(new_n55_), .O(z09));
  nand3  g035(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  nand3  g036(.a(x26), .b(x25), .c(x24), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n80_), .c(x20), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x19), .O(new_n83_));
  oai21  g039(.a(new_n72_), .b(x19), .c(new_n83_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n58_), .c(new_n55_), .O(z10));
  inv1   g042(.a(x19), .O(new_n87_));
  nand2  g043(.a(x22), .b(x13), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n81_), .c(x21), .O(new_n89_));
  nor2   g045(.a(new_n73_), .b(x20), .O(new_n90_));
  aoi21  g046(.a(new_n89_), .b(x20), .c(new_n90_), .O(new_n91_));
  nand2  g047(.a(x21), .b(new_n87_), .O(new_n92_));
  oai21  g048(.a(new_n91_), .b(new_n87_), .c(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n61_), .O(z11));
  nand3  g051(.a(x21), .b(x20), .c(x19), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(x22), .O(new_n97_));
  nand2  g053(.a(x20), .b(x19), .O(new_n98_));
  inv1   g054(.a(x22), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x21), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n102_));
  nand2  g058(.a(x08), .b(x03), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n55_), .O(new_n105_));
  inv1   g061(.a(x09), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(x08), .O(new_n107_));
  nor3   g063(.a(new_n81_), .b(new_n54_), .c(new_n99_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n107_), .c(x14), .d(new_n47_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n105_), .O(z12));
  aoi21  g066(.a(x21), .b(x20), .c(new_n99_), .O(new_n111_));
  nand4  g067(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x22), .c(x19), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x23), .O(new_n114_));
  inv1   g070(.a(x24), .O(new_n115_));
  inv1   g071(.a(new_n96_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n115_), .c(new_n54_), .d(x22), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n119_));
  oai21  g075(.a(new_n66_), .b(new_n65_), .c(new_n119_), .O(z13));
  nand2  g076(.a(new_n63_), .b(x05), .O(new_n121_));
  nand3  g077(.a(x26), .b(x25), .c(x16), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x22), .c(x21), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n98_), .c(x24), .O(new_n124_));
  inv1   g080(.a(new_n98_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n115_), .c(x22), .d(x21), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n124_), .c(new_n54_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n121_), .O(z14));
  nand3  g085(.a(new_n107_), .b(x19), .c(new_n47_), .O(new_n130_));
  inv1   g086(.a(x25), .O(new_n131_));
  nand3  g087(.a(new_n74_), .b(new_n131_), .c(x22), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n130_), .c(x23), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x24), .O(new_n134_));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x24), .c(x22), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n116_), .c(new_n131_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n139_));
  nand2  g095(.a(x08), .b(x06), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n139_), .c(new_n134_), .O(z15));
  nand4  g097(.a(new_n74_), .b(new_n45_), .c(x25), .d(x22), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n130_), .c(x23), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x24), .O(new_n144_));
  nand4  g100(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n145_));
  nor3   g101(.a(new_n145_), .b(new_n98_), .c(x18), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n45_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n148_));
  nand2  g104(.a(x08), .b(x07), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n148_), .c(new_n144_), .O(z16));
endmodule


