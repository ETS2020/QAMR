// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x18), .O(new_n45_));
  inv1   g01(.a(x09), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(x08), .O(new_n47_));
  nand3  g03(.a(x21), .b(x20), .c(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x22), .O(new_n50_));
  inv1   g06(.a(x23), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g08(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand4  g10(.a(new_n54_), .b(new_n52_), .c(new_n49_), .d(new_n47_), .O(new_n55_));
  aoi21  g11(.a(new_n55_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g12(.a(x00), .O(new_n57_));
  inv1   g13(.a(x10), .O(new_n58_));
  nand2  g14(.a(x18), .b(new_n58_), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(x08), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n57_), .O(z01));
  inv1   g17(.a(x01), .O(new_n62_));
  nor2   g18(.a(new_n60_), .b(new_n62_), .O(z02));
  nand2  g19(.a(x08), .b(x02), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n59_), .O(z03));
  nand2  g21(.a(x08), .b(x03), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n59_), .O(z04));
  inv1   g23(.a(x04), .O(new_n68_));
  nor2   g24(.a(new_n60_), .b(new_n68_), .O(z05));
  inv1   g25(.a(x05), .O(new_n70_));
  nor2   g26(.a(new_n60_), .b(new_n70_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n59_), .O(z07));
  inv1   g29(.a(x07), .O(new_n74_));
  nor2   g30(.a(new_n60_), .b(new_n74_), .O(z08));
  inv1   g31(.a(x21), .O(new_n76_));
  nor2   g32(.a(new_n50_), .b(new_n76_), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g36(.a(new_n80_), .b(new_n45_), .c(new_n58_), .d(x09), .O(new_n81_));
  oai22  g37(.a(new_n81_), .b(x08), .c(new_n60_), .d(new_n57_), .O(z09));
  inv1   g38(.a(x08), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n79_), .c(x19), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g42(.a(x20), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(x19), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n46_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n83_), .c(x18), .O(new_n90_));
  oai22  g46(.a(new_n90_), .b(x10), .c(new_n83_), .d(new_n62_), .O(z10));
  nand3  g47(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n53_), .c(x21), .O(new_n93_));
  nor2   g49(.a(new_n76_), .b(x19), .O(new_n94_));
  aoi21  g50(.a(new_n93_), .b(x19), .c(new_n94_), .O(new_n95_));
  nand2  g51(.a(x21), .b(new_n87_), .O(new_n96_));
  oai21  g52(.a(new_n95_), .b(new_n87_), .c(new_n96_), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(x09), .c(new_n83_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n45_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n58_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n64_), .O(z11));
  nand2  g57(.a(x20), .b(x19), .O(new_n102_));
  nand2  g58(.a(x23), .b(x14), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n53_), .c(x21), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n102_), .c(x22), .O(new_n105_));
  inv1   g61(.a(new_n102_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n50_), .c(x21), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n105_), .c(new_n46_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n83_), .c(x18), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(x10), .c(new_n66_), .O(z12));
  nand4  g66(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(x22), .c(x21), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n102_), .c(x23), .O(new_n113_));
  nand4  g69(.a(new_n106_), .b(new_n51_), .c(x22), .d(x21), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n113_), .c(x18), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n58_), .c(x09), .d(new_n83_), .O(new_n116_));
  oai21  g72(.a(new_n60_), .b(new_n68_), .c(new_n116_), .O(z13));
  nand3  g73(.a(x26), .b(x25), .c(x16), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(x23), .c(x22), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n48_), .c(x24), .O(new_n120_));
  inv1   g76(.a(x24), .O(new_n121_));
  nand4  g77(.a(new_n49_), .b(new_n121_), .c(x23), .d(x22), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n120_), .c(new_n46_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n83_), .c(x18), .O(new_n124_));
  oai22  g80(.a(new_n124_), .b(x10), .c(new_n83_), .d(new_n70_), .O(z14));
  nand2  g81(.a(x26), .b(x17), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n52_), .c(x24), .O(new_n127_));
  oai21  g83(.a(new_n127_), .b(new_n48_), .c(x25), .O(new_n128_));
  inv1   g84(.a(x25), .O(new_n129_));
  nand4  g85(.a(new_n52_), .b(new_n49_), .c(new_n129_), .d(x24), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n128_), .c(new_n46_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n83_), .c(x18), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(x10), .c(new_n72_), .O(z15));
  nand3  g89(.a(new_n52_), .b(x25), .c(x24), .O(new_n134_));
  oai21  g90(.a(new_n134_), .b(new_n48_), .c(x26), .O(new_n135_));
  nor2   g91(.a(new_n121_), .b(new_n51_), .O(new_n136_));
  nor2   g92(.a(x26), .b(new_n129_), .O(new_n137_));
  nand4  g93(.a(new_n137_), .b(new_n106_), .c(new_n136_), .d(new_n77_), .O(new_n138_));
  aoi21  g94(.a(new_n138_), .b(new_n135_), .c(new_n46_), .O(new_n139_));
  aoi21  g95(.a(new_n139_), .b(new_n83_), .c(x18), .O(new_n140_));
  nand2  g96(.a(x08), .b(x07), .O(new_n141_));
  oai21  g97(.a(new_n140_), .b(x10), .c(new_n141_), .O(z16));
endmodule


