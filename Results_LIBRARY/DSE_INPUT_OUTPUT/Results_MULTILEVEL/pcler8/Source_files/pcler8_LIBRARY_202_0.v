// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  nor2   g000(.a(x24), .b(x11), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand2  g004(.a(x20), .b(x19), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand3  g012(.a(new_n56_), .b(new_n54_), .c(x21), .O(new_n57_));
  oai21  g013(.a(new_n57_), .b(new_n51_), .c(new_n46_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  nand2  g015(.a(new_n46_), .b(x08), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n59_), .O(z01));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n46_), .O(z02));
  inv1   g019(.a(x02), .O(new_n64_));
  nor2   g020(.a(new_n60_), .b(new_n64_), .O(z03));
  inv1   g021(.a(x03), .O(new_n66_));
  nor2   g022(.a(new_n60_), .b(new_n66_), .O(z04));
  nand2  g023(.a(x08), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n46_), .O(z05));
  inv1   g025(.a(x05), .O(new_n70_));
  oai21  g026(.a(new_n47_), .b(new_n70_), .c(new_n46_), .O(z06));
  nand2  g027(.a(x08), .b(x06), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n46_), .O(z07));
  nand2  g029(.a(x08), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n46_), .O(z08));
  and2   g031(.a(x22), .b(x21), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  nand4  g033(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n80_));
  aoi21  g036(.a(x08), .b(x00), .c(new_n45_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(z09));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n78_), .c(x20), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x19), .O(new_n85_));
  inv1   g041(.a(x19), .O(new_n86_));
  nand2  g042(.a(x20), .b(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n62_), .c(new_n46_), .O(z10));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n55_), .c(x21), .O(new_n92_));
  inv1   g048(.a(x21), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(x20), .O(new_n94_));
  aoi21  g050(.a(new_n92_), .b(x20), .c(new_n94_), .O(new_n95_));
  nand2  g051(.a(x21), .b(new_n86_), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n86_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n98_));
  aoi21  g054(.a(x08), .b(x02), .c(new_n45_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(z11));
  nand2  g056(.a(x23), .b(x14), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n55_), .c(x21), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n49_), .c(x22), .O(new_n103_));
  nand3  g059(.a(new_n50_), .b(new_n52_), .c(x21), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n106_));
  aoi21  g062(.a(x08), .b(x03), .c(new_n45_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n106_), .O(z12));
  nand4  g064(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x22), .c(x21), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n49_), .c(x23), .O(new_n111_));
  nand4  g067(.a(new_n50_), .b(new_n53_), .c(x22), .d(x21), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n68_), .c(new_n46_), .O(z13));
  nand3  g071(.a(x21), .b(x20), .c(x19), .O(new_n116_));
  nand3  g072(.a(x26), .b(x25), .c(x16), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x23), .c(x22), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x24), .O(new_n119_));
  nor2   g075(.a(x24), .b(new_n53_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n76_), .c(new_n50_), .d(x11), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n123_));
  oai21  g079(.a(new_n60_), .b(new_n70_), .c(new_n123_), .O(z14));
  inv1   g080(.a(new_n116_), .O(new_n125_));
  nand2  g081(.a(x26), .b(x17), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n125_), .c(x23), .d(x22), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(x25), .c(new_n48_), .d(x09), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(x08), .c(new_n72_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n46_), .O(new_n130_));
  inv1   g086(.a(x24), .O(new_n131_));
  nand3  g087(.a(x25), .b(new_n131_), .c(x11), .O(new_n132_));
  nor2   g088(.a(x25), .b(new_n131_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n125_), .c(new_n54_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n130_), .O(z15));
  inv1   g093(.a(x18), .O(new_n138_));
  and2   g094(.a(x25), .b(x23), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n76_), .c(new_n50_), .d(new_n138_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(x26), .c(new_n48_), .d(x09), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(x08), .c(new_n74_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n46_), .O(new_n143_));
  nand3  g099(.a(x26), .b(new_n131_), .c(x11), .O(new_n144_));
  nand2  g100(.a(new_n76_), .b(new_n50_), .O(new_n145_));
  inv1   g101(.a(x26), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(x25), .c(x24), .d(x23), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n145_), .c(new_n144_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n143_), .O(z16));
endmodule


