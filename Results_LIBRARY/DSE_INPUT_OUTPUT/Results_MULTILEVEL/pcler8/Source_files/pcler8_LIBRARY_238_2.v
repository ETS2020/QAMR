// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x19), .O(new_n54_));
  nor2   g010(.a(x23), .b(new_n54_), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  and2   g012(.a(new_n56_), .b(x00), .O(z01));
  and2   g013(.a(new_n56_), .b(x01), .O(z02));
  and2   g014(.a(new_n56_), .b(x02), .O(z03));
  inv1   g015(.a(x03), .O(new_n60_));
  inv1   g016(.a(new_n56_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n60_), .O(z04));
  inv1   g018(.a(new_n55_), .O(new_n63_));
  nand2  g019(.a(x08), .b(x04), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n63_), .O(z05));
  nand2  g021(.a(x08), .b(x05), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n63_), .O(z06));
  nand2  g023(.a(new_n56_), .b(x06), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z07));
  nand2  g025(.a(x08), .b(x07), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n63_), .O(z08));
  and2   g027(.a(x21), .b(x20), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(x11), .O(new_n73_));
  nand4  g029(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand4  g031(.a(new_n75_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n76_));
  aoi21  g032(.a(x08), .b(x00), .c(new_n55_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n76_), .O(z09));
  inv1   g034(.a(x20), .O(new_n79_));
  nand3  g035(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  nand3  g036(.a(x26), .b(x25), .c(x24), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n80_), .c(x20), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x19), .O(new_n83_));
  oai21  g039(.a(new_n79_), .b(x19), .c(new_n83_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n85_));
  aoi21  g041(.a(x08), .b(x01), .c(new_n55_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(z10));
  nand2  g043(.a(x22), .b(x13), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n81_), .c(x21), .O(new_n89_));
  inv1   g045(.a(x21), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(x20), .O(new_n91_));
  aoi21  g047(.a(new_n89_), .b(x20), .c(new_n91_), .O(new_n92_));
  nand2  g048(.a(x21), .b(new_n54_), .O(new_n93_));
  oai21  g049(.a(new_n92_), .b(new_n54_), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  aoi21  g051(.a(x08), .b(x02), .c(new_n55_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n95_), .O(z11));
  inv1   g053(.a(x22), .O(new_n98_));
  nand4  g054(.a(x26), .b(x25), .c(x24), .d(x14), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(x21), .c(x20), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(x23), .c(new_n54_), .O(new_n101_));
  nor2   g057(.a(new_n79_), .b(new_n54_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(x23), .c(new_n98_), .d(x21), .O(new_n103_));
  oai21  g059(.a(new_n101_), .b(new_n98_), .c(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n105_));
  oai21  g061(.a(new_n61_), .b(new_n60_), .c(new_n105_), .O(z12));
  inv1   g062(.a(x09), .O(new_n107_));
  nor2   g063(.a(new_n107_), .b(x08), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x15), .c(new_n47_), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n74_), .c(x23), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x19), .O(new_n111_));
  inv1   g067(.a(x23), .O(new_n112_));
  nand3  g068(.a(x21), .b(x20), .c(x19), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(x22), .c(new_n112_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n111_), .c(new_n64_), .O(z13));
  nand3  g073(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n118_));
  inv1   g074(.a(x24), .O(new_n119_));
  nand3  g075(.a(new_n72_), .b(new_n119_), .c(x22), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n118_), .c(x23), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x19), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x22), .c(x21), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n102_), .c(new_n119_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n122_), .c(new_n66_), .O(z14));
  nand2  g084(.a(x26), .b(x17), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n72_), .c(x24), .d(x22), .O(new_n130_));
  inv1   g086(.a(x25), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x24), .c(x22), .O(new_n132_));
  nor2   g088(.a(new_n132_), .b(new_n113_), .O(new_n133_));
  aoi21  g089(.a(new_n130_), .b(x25), .c(new_n133_), .O(new_n134_));
  nand2  g090(.a(x25), .b(new_n54_), .O(new_n135_));
  oai21  g091(.a(new_n134_), .b(new_n112_), .c(new_n135_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n68_), .O(z15));
  nand3  g094(.a(new_n108_), .b(x20), .c(new_n47_), .O(new_n139_));
  nor2   g095(.a(new_n98_), .b(new_n90_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(new_n45_), .c(x25), .d(x24), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n139_), .c(x23), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x19), .O(new_n143_));
  nor3   g099(.a(new_n79_), .b(new_n54_), .c(x18), .O(new_n144_));
  nand3  g100(.a(new_n140_), .b(x25), .c(x24), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n144_), .c(new_n45_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n143_), .c(new_n70_), .O(z16));
endmodule


