// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n59_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x24), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  and2   g012(.a(new_n56_), .b(x00), .O(z01));
  and2   g013(.a(new_n56_), .b(x01), .O(z02));
  inv1   g014(.a(new_n55_), .O(new_n59_));
  nand2  g015(.a(x08), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(z03));
  nand2  g017(.a(x08), .b(x03), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n59_), .O(z04));
  nand2  g019(.a(x08), .b(x04), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(z05));
  inv1   g021(.a(x05), .O(new_n66_));
  inv1   g022(.a(new_n56_), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n66_), .O(z06));
  nand2  g024(.a(new_n56_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  and2   g026(.a(new_n56_), .b(x07), .O(z08));
  inv1   g027(.a(x20), .O(new_n72_));
  inv1   g028(.a(x21), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n78_));
  aoi21  g034(.a(x08), .b(x00), .c(new_n55_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n78_), .O(z09));
  nand3  g036(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  nand3  g037(.a(x26), .b(x25), .c(x24), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n81_), .c(x20), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x19), .O(new_n84_));
  oai21  g040(.a(new_n72_), .b(x19), .c(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  aoi21  g042(.a(x08), .b(x01), .c(new_n55_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(z10));
  nand2  g044(.a(x20), .b(x19), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand3  g046(.a(new_n73_), .b(x20), .c(x19), .O(new_n91_));
  oai21  g047(.a(new_n90_), .b(new_n73_), .c(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n60_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n59_), .O(new_n95_));
  inv1   g051(.a(x09), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(x08), .O(new_n97_));
  nand2  g053(.a(x21), .b(x13), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(x10), .O(new_n99_));
  inv1   g055(.a(x22), .O(new_n100_));
  inv1   g056(.a(x23), .O(new_n101_));
  nor3   g057(.a(new_n82_), .b(new_n101_), .c(new_n100_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n99_), .c(new_n97_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n95_), .O(z11));
  nand3  g060(.a(x21), .b(x20), .c(x19), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand2  g062(.a(new_n100_), .b(x21), .O(new_n107_));
  oai22  g063(.a(new_n107_), .b(new_n89_), .c(new_n106_), .d(new_n100_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n62_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n59_), .O(new_n111_));
  nand4  g067(.a(new_n102_), .b(new_n97_), .c(x14), .d(new_n47_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(z12));
  nand3  g069(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n114_));
  nor2   g070(.a(new_n100_), .b(new_n73_), .O(new_n115_));
  nand2  g071(.a(new_n90_), .b(new_n115_), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n114_), .c(new_n54_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n101_), .O(new_n118_));
  nand2  g074(.a(x26), .b(x25), .O(new_n119_));
  nand2  g075(.a(x24), .b(x15), .O(new_n120_));
  oai22  g076(.a(new_n120_), .b(new_n119_), .c(new_n106_), .d(new_n101_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x22), .O(new_n122_));
  nand2  g078(.a(x23), .b(new_n100_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n118_), .c(new_n64_), .O(z13));
  nand3  g082(.a(x26), .b(x25), .c(x16), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x22), .c(x21), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n89_), .c(x24), .O(new_n129_));
  nand4  g085(.a(new_n90_), .b(new_n54_), .c(x22), .d(x21), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n129_), .c(new_n101_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n132_));
  oai21  g088(.a(new_n67_), .b(new_n66_), .c(new_n132_), .O(z14));
  nand2  g089(.a(x26), .b(x17), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n90_), .c(x22), .d(x21), .O(new_n135_));
  inv1   g091(.a(x25), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x24), .c(x22), .O(new_n137_));
  nor2   g093(.a(new_n137_), .b(new_n105_), .O(new_n138_));
  aoi21  g094(.a(new_n135_), .b(x25), .c(new_n138_), .O(new_n139_));
  nand2  g095(.a(x25), .b(new_n54_), .O(new_n140_));
  oai21  g096(.a(new_n139_), .b(new_n101_), .c(new_n140_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n69_), .O(z15));
  nand3  g099(.a(new_n97_), .b(x19), .c(new_n47_), .O(new_n144_));
  nand4  g100(.a(new_n74_), .b(new_n45_), .c(x25), .d(x22), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n144_), .c(x23), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x24), .O(new_n147_));
  nor2   g103(.a(new_n89_), .b(x18), .O(new_n148_));
  nand3  g104(.a(new_n115_), .b(x25), .c(x24), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n148_), .c(new_n45_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n152_));
  nand2  g108(.a(x08), .b(x07), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n152_), .c(new_n147_), .O(z16));
endmodule


