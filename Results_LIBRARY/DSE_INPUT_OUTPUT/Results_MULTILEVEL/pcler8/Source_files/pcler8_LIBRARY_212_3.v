// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n55_, new_n57_, new_n59_, new_n61_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n108_,
    new_n109_;
  inv1   g00(.a(x23), .O(new_n45_));
  inv1   g01(.a(x24), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(z00));
  inv1   g03(.a(z00), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x08), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(x00), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(z01));
  nand2  g08(.a(x08), .b(x01), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n48_), .O(z02));
  nand2  g10(.a(new_n50_), .b(x02), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z03));
  nand2  g12(.a(new_n50_), .b(x03), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z04));
  nand2  g14(.a(new_n50_), .b(x04), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z05));
  inv1   g16(.a(x05), .O(new_n61_));
  inv1   g17(.a(x08), .O(new_n62_));
  oai21  g18(.a(new_n62_), .b(new_n61_), .c(new_n48_), .O(z06));
  nand2  g19(.a(new_n50_), .b(x06), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z07));
  nand2  g21(.a(new_n50_), .b(x07), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z08));
  nand2  g23(.a(x08), .b(x00), .O(new_n68_));
  inv1   g24(.a(x10), .O(new_n69_));
  inv1   g25(.a(x19), .O(new_n70_));
  inv1   g26(.a(x09), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(x08), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(new_n73_));
  aoi21  g29(.a(new_n73_), .b(new_n68_), .c(z00), .O(z09));
  xor2a  g30(.a(x20), .b(x19), .O(new_n75_));
  nand4  g31(.a(new_n75_), .b(new_n69_), .c(x09), .d(new_n62_), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n53_), .c(z00), .O(z10));
  xnor2a g33(.a(x21), .b(x20), .O(new_n78_));
  nand2  g34(.a(x21), .b(new_n70_), .O(new_n79_));
  oai21  g35(.a(new_n78_), .b(new_n70_), .c(new_n79_), .O(new_n80_));
  nand4  g36(.a(new_n80_), .b(new_n69_), .c(x09), .d(new_n62_), .O(new_n81_));
  nand2  g37(.a(x08), .b(x02), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n81_), .c(z00), .O(z11));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(x22), .O(new_n85_));
  inv1   g41(.a(x22), .O(new_n86_));
  nand4  g42(.a(new_n86_), .b(x21), .c(x20), .d(x19), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g44(.a(new_n88_), .b(new_n69_), .c(x09), .d(new_n62_), .O(new_n89_));
  aoi21  g45(.a(x08), .b(x03), .c(z00), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n89_), .O(z12));
  nand2  g47(.a(new_n84_), .b(x23), .O(new_n92_));
  nand4  g48(.a(new_n45_), .b(x21), .c(x20), .d(x19), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(x22), .O(new_n95_));
  nand2  g51(.a(x23), .b(new_n86_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n69_), .c(x09), .d(new_n62_), .O(new_n98_));
  aoi21  g54(.a(x08), .b(x04), .c(z00), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(z13));
  nand3  g56(.a(new_n46_), .b(x23), .c(x22), .O(new_n101_));
  oai22  g57(.a(new_n101_), .b(new_n84_), .c(new_n46_), .d(x23), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(new_n69_), .c(x09), .d(new_n62_), .O(new_n103_));
  oai21  g59(.a(new_n49_), .b(new_n61_), .c(new_n103_), .O(z14));
  nand2  g60(.a(x08), .b(x06), .O(new_n105_));
  nand3  g61(.a(new_n72_), .b(x25), .c(new_n69_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n105_), .c(z00), .O(z15));
  nand2  g63(.a(x08), .b(x07), .O(new_n108_));
  nand3  g64(.a(new_n72_), .b(x26), .c(new_n69_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n108_), .c(z00), .O(z16));
endmodule


