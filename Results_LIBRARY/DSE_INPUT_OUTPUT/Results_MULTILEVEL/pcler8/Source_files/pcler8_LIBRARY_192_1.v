// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n66_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x13), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  inv1   g011(.a(x19), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(x13), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(new_n59_));
  nor2   g015(.a(new_n59_), .b(new_n55_), .O(z01));
  inv1   g016(.a(new_n57_), .O(new_n61_));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(z02));
  and2   g019(.a(new_n58_), .b(x02), .O(z03));
  and2   g020(.a(new_n58_), .b(x03), .O(z04));
  aoi21  g021(.a(x08), .b(x04), .c(new_n57_), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  and2   g023(.a(new_n58_), .b(x05), .O(z06));
  nand2  g024(.a(new_n58_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  nand2  g026(.a(x08), .b(x07), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n61_), .O(z08));
  nand4  g028(.a(x21), .b(x20), .c(x13), .d(x11), .O(new_n73_));
  nand2  g029(.a(x23), .b(x22), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  nand3  g031(.a(x26), .b(x25), .c(x24), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n73_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n80_));
  oai21  g036(.a(new_n59_), .b(new_n55_), .c(new_n80_), .O(z09));
  nand3  g037(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  nand4  g038(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n82_), .c(x20), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x19), .O(new_n85_));
  nand2  g041(.a(x20), .b(new_n56_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n62_), .c(new_n61_), .O(z10));
  oai21  g045(.a(new_n76_), .b(new_n74_), .c(x21), .O(new_n90_));
  nand3  g046(.a(x23), .b(x22), .c(x12), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n76_), .c(x20), .O(new_n92_));
  aoi22  g048(.a(new_n92_), .b(x21), .c(new_n90_), .d(x20), .O(new_n93_));
  nand2  g049(.a(x21), .b(new_n56_), .O(new_n94_));
  oai21  g050(.a(new_n93_), .b(new_n56_), .c(new_n94_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n96_));
  aoi21  g052(.a(x08), .b(x02), .c(new_n57_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n96_), .O(z11));
  nand2  g054(.a(x23), .b(x14), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n76_), .c(x22), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(x21), .c(x20), .d(x19), .O(new_n101_));
  nand3  g057(.a(x21), .b(x20), .c(x19), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n46_), .c(new_n101_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n105_));
  aoi21  g061(.a(x08), .b(x03), .c(new_n57_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n105_), .O(z12));
  nand4  g063(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x23), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x22), .O(new_n110_));
  nand2  g066(.a(x23), .b(new_n46_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(x21), .c(x20), .d(x19), .O(new_n113_));
  nand2  g069(.a(new_n102_), .b(x23), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n66_), .O(z13));
  nand4  g073(.a(x20), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n118_));
  inv1   g074(.a(x24), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(x23), .c(x22), .d(x21), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n118_), .c(x13), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x19), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n103_), .c(new_n119_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand2  g083(.a(x08), .b(x05), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n127_), .c(new_n122_), .O(z14));
  nand3  g085(.a(x22), .b(x21), .c(x20), .O(new_n130_));
  nand2  g086(.a(x26), .b(x17), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x24), .c(x23), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n130_), .c(x25), .O(new_n133_));
  nor2   g089(.a(x25), .b(new_n119_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n103_), .c(new_n75_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x13), .O(new_n137_));
  nand2  g093(.a(x25), .b(new_n56_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n69_), .O(z15));
  nand2  g097(.a(new_n58_), .b(x07), .O(new_n142_));
  nand2  g098(.a(x21), .b(x20), .O(new_n143_));
  nor2   g099(.a(new_n143_), .b(x18), .O(new_n144_));
  nand4  g100(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n144_), .c(new_n45_), .O(new_n147_));
  nand4  g103(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n148_));
  nand4  g104(.a(new_n45_), .b(x25), .c(x24), .d(x23), .O(new_n149_));
  nor2   g105(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n147_), .c(x13), .O(new_n151_));
  nand2  g107(.a(x26), .b(new_n56_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n142_), .O(z16));
endmodule


