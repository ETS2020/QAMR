// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x19), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x10), .O(new_n48_));
  inv1   g004(.a(x21), .O(new_n49_));
  inv1   g005(.a(x22), .O(new_n50_));
  nand3  g006(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  nor3   g007(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(new_n48_), .c(x09), .d(new_n46_), .O(new_n53_));
  aoi21  g009(.a(new_n53_), .b(x20), .c(new_n45_), .O(z00));
  nor2   g010(.a(new_n45_), .b(x20), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  and2   g012(.a(new_n56_), .b(x00), .O(z01));
  inv1   g013(.a(new_n55_), .O(new_n58_));
  nand2  g014(.a(x08), .b(x01), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n58_), .O(z03));
  nand2  g018(.a(x08), .b(x03), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(z04));
  nand2  g020(.a(new_n56_), .b(x04), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z05));
  and2   g022(.a(new_n56_), .b(x05), .O(z06));
  nand2  g023(.a(new_n56_), .b(x06), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z07));
  nand2  g025(.a(x08), .b(x07), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n58_), .O(z08));
  inv1   g027(.a(x10), .O(new_n72_));
  nor2   g028(.a(new_n50_), .b(new_n49_), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(x11), .O(new_n74_));
  inv1   g030(.a(x25), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x24), .c(x23), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n74_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n80_));
  aoi21  g036(.a(x08), .b(x00), .c(new_n55_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(z09));
  nand2  g038(.a(new_n56_), .b(x01), .O(new_n83_));
  nand2  g039(.a(new_n73_), .b(x12), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n78_), .c(x19), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g042(.a(x20), .O(new_n87_));
  nand3  g043(.a(new_n45_), .b(new_n87_), .c(x19), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n83_), .O(z10));
  nand2  g047(.a(new_n56_), .b(x02), .O(new_n92_));
  nand3  g048(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n51_), .c(x21), .O(new_n94_));
  nor2   g050(.a(new_n49_), .b(x19), .O(new_n95_));
  aoi21  g051(.a(new_n94_), .b(x19), .c(new_n95_), .O(new_n96_));
  nand3  g052(.a(new_n45_), .b(x21), .c(new_n87_), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n87_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n92_), .O(z11));
  nand2  g056(.a(new_n56_), .b(x03), .O(new_n101_));
  nand2  g057(.a(x23), .b(x14), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n51_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n47_), .c(x22), .O(new_n104_));
  nand3  g060(.a(new_n50_), .b(x21), .c(x19), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x20), .O(new_n107_));
  nand3  g063(.a(new_n45_), .b(x22), .c(new_n87_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n101_), .O(z12));
  nand4  g067(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(x23), .c(new_n49_), .O(new_n113_));
  nand2  g069(.a(x21), .b(x19), .O(new_n114_));
  aoi22  g070(.a(new_n114_), .b(x23), .c(new_n113_), .d(x19), .O(new_n115_));
  nand2  g071(.a(x23), .b(new_n50_), .O(new_n116_));
  oai21  g072(.a(new_n115_), .b(new_n50_), .c(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(x20), .c(new_n72_), .d(x09), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(x08), .c(new_n65_), .O(z13));
  nand3  g075(.a(new_n72_), .b(x09), .c(new_n46_), .O(new_n120_));
  inv1   g076(.a(x24), .O(new_n121_));
  inv1   g077(.a(new_n114_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n121_), .c(x22), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n120_), .c(x20), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x23), .O(new_n125_));
  nand2  g081(.a(new_n77_), .b(x16), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x23), .c(x22), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n122_), .c(new_n121_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n130_));
  nand2  g086(.a(x08), .b(x05), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n130_), .c(new_n125_), .O(z14));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n122_), .c(x24), .d(x22), .O(new_n134_));
  nand3  g090(.a(x22), .b(x21), .c(x19), .O(new_n135_));
  nand3  g091(.a(new_n75_), .b(x24), .c(x23), .O(new_n136_));
  nor2   g092(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi21  g093(.a(new_n134_), .b(x25), .c(new_n137_), .O(new_n138_));
  oai22  g094(.a(new_n138_), .b(new_n87_), .c(new_n75_), .d(x23), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n68_), .O(z15));
  nand2  g097(.a(new_n56_), .b(x07), .O(new_n142_));
  nor2   g098(.a(new_n114_), .b(x18), .O(new_n143_));
  nand3  g099(.a(x25), .b(x24), .c(x22), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n143_), .c(new_n76_), .O(new_n146_));
  nand4  g102(.a(new_n76_), .b(x25), .c(x24), .d(x23), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n135_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n146_), .c(x20), .O(new_n149_));
  oai21  g105(.a(new_n76_), .b(x23), .c(new_n149_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n72_), .c(x09), .d(new_n46_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n142_), .O(z16));
endmodule


