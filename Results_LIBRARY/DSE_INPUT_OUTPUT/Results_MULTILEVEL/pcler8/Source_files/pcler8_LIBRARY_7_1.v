// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(new_n45_), .b(x24), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  and2   g011(.a(new_n55_), .b(x00), .O(z01));
  inv1   g012(.a(new_n54_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z02));
  nand2  g015(.a(x08), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n57_), .O(z03));
  inv1   g017(.a(x03), .O(new_n62_));
  inv1   g018(.a(new_n55_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n62_), .O(z04));
  nand2  g020(.a(x08), .b(x04), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n57_), .O(z05));
  inv1   g022(.a(x05), .O(new_n67_));
  nor2   g023(.a(new_n63_), .b(new_n67_), .O(z06));
  nand2  g024(.a(new_n55_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  aoi21  g026(.a(x08), .b(x07), .c(new_n54_), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z08));
  nand3  g028(.a(x21), .b(x20), .c(x11), .O(new_n73_));
  inv1   g029(.a(x22), .O(new_n74_));
  inv1   g030(.a(x23), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x25), .O(new_n77_));
  nor2   g033(.a(new_n45_), .b(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n73_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n81_));
  aoi21  g037(.a(x08), .b(x00), .c(new_n54_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(z09));
  inv1   g039(.a(x20), .O(new_n84_));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  nand3  g041(.a(x26), .b(x25), .c(x23), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n85_), .c(x20), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x19), .O(new_n88_));
  oai21  g044(.a(new_n84_), .b(x19), .c(new_n88_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n58_), .c(new_n57_), .O(z10));
  inv1   g047(.a(x19), .O(new_n92_));
  xnor2a g048(.a(x21), .b(x20), .O(new_n93_));
  nand2  g049(.a(x21), .b(new_n92_), .O(new_n94_));
  oai21  g050(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n60_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n57_), .O(new_n98_));
  inv1   g054(.a(x09), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(x08), .O(new_n100_));
  nand2  g056(.a(x19), .b(x13), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(x10), .O(new_n102_));
  nand2  g058(.a(new_n76_), .b(x20), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  nand2  g060(.a(new_n78_), .b(x24), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n104_), .c(new_n102_), .d(new_n100_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n98_), .O(z11));
  nand3  g064(.a(x21), .b(x20), .c(x19), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand2  g066(.a(x20), .b(x19), .O(new_n111_));
  nand2  g067(.a(new_n74_), .b(x21), .O(new_n112_));
  oai22  g068(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(new_n74_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n114_));
  oai21  g070(.a(new_n46_), .b(new_n62_), .c(new_n114_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n57_), .O(new_n116_));
  inv1   g072(.a(x14), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(x10), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n106_), .c(new_n76_), .d(new_n100_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n116_), .O(z12));
  nand3  g076(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n121_));
  nand3  g077(.a(x25), .b(x23), .c(x15), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(x24), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x26), .O(new_n124_));
  and2   g080(.a(x22), .b(x21), .O(new_n125_));
  inv1   g081(.a(new_n111_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x23), .O(new_n128_));
  nand3  g084(.a(new_n75_), .b(x22), .c(x21), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n111_), .c(new_n128_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n124_), .c(new_n65_), .O(z13));
  nand3  g088(.a(x26), .b(x25), .c(x16), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x23), .c(x22), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n109_), .c(x24), .O(new_n135_));
  inv1   g091(.a(x24), .O(new_n136_));
  nand4  g092(.a(new_n110_), .b(new_n76_), .c(new_n45_), .d(new_n136_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n139_));
  oai21  g095(.a(new_n63_), .b(new_n67_), .c(new_n139_), .O(z14));
  nand2  g096(.a(x26), .b(x17), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n110_), .c(x23), .d(x22), .O(new_n142_));
  nand3  g098(.a(new_n77_), .b(x23), .c(x22), .O(new_n143_));
  nor2   g099(.a(new_n143_), .b(new_n109_), .O(new_n144_));
  aoi21  g100(.a(new_n142_), .b(x25), .c(new_n144_), .O(new_n145_));
  nor2   g101(.a(x26), .b(new_n77_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n136_), .O(new_n147_));
  oai21  g103(.a(new_n145_), .b(new_n136_), .c(new_n147_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n69_), .O(z15));
  inv1   g106(.a(x18), .O(new_n151_));
  nand2  g107(.a(new_n126_), .b(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n125_), .b(x25), .c(x23), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n152_), .c(x26), .O(new_n154_));
  nand3  g110(.a(new_n146_), .b(x24), .c(x23), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n127_), .c(new_n154_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n71_), .O(z16));
endmodule


