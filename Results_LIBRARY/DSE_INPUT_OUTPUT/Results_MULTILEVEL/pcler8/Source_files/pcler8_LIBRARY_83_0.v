// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x24), .b(x23), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nand2  g011(.a(x08), .b(x00), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n55_), .O(z01));
  inv1   g013(.a(x01), .O(new_n58_));
  nor2   g014(.a(new_n54_), .b(new_n46_), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n58_), .O(z02));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n55_), .O(z04));
  and2   g021(.a(new_n59_), .b(x04), .O(z05));
  inv1   g022(.a(x05), .O(new_n67_));
  oai21  g023(.a(new_n46_), .b(new_n67_), .c(new_n55_), .O(z06));
  nand2  g024(.a(new_n59_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  nand2  g026(.a(x08), .b(x07), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n55_), .O(z08));
  nand4  g028(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  inv1   g029(.a(x23), .O(new_n74_));
  inv1   g030(.a(x24), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x25), .O(new_n77_));
  nor2   g033(.a(new_n45_), .b(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n73_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n56_), .c(new_n55_), .O(z09));
  xor2a  g038(.a(x20), .b(x19), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n84_));
  oai21  g040(.a(new_n46_), .b(new_n58_), .c(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n55_), .O(new_n86_));
  inv1   g042(.a(x09), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(x08), .O(new_n88_));
  nand2  g044(.a(x19), .b(x12), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(x10), .O(new_n90_));
  inv1   g046(.a(x21), .O(new_n91_));
  nand2  g047(.a(x23), .b(x22), .O(new_n92_));
  nand2  g048(.a(new_n78_), .b(x24), .O(new_n93_));
  nor3   g049(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n90_), .c(new_n88_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n86_), .O(z10));
  inv1   g052(.a(x19), .O(new_n97_));
  xnor2a g053(.a(x21), .b(x20), .O(new_n98_));
  nand2  g054(.a(x21), .b(new_n97_), .O(new_n99_));
  oai21  g055(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n62_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n55_), .O(new_n103_));
  nand2  g059(.a(x19), .b(x13), .O(new_n104_));
  nor2   g060(.a(new_n104_), .b(x10), .O(new_n105_));
  inv1   g061(.a(x20), .O(new_n106_));
  nor3   g062(.a(new_n93_), .b(new_n92_), .c(new_n106_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n105_), .c(new_n88_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n103_), .O(z11));
  inv1   g065(.a(x22), .O(new_n110_));
  nand3  g066(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n110_), .b(x21), .c(x20), .d(x19), .O(new_n113_));
  oai21  g069(.a(new_n112_), .b(new_n110_), .c(new_n113_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n64_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n55_), .O(new_n117_));
  nor2   g073(.a(new_n93_), .b(new_n92_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n88_), .c(x14), .d(new_n47_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n117_), .O(z12));
  nand3  g076(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n121_));
  nand4  g077(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(x24), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n74_), .O(new_n124_));
  nand4  g080(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n112_), .c(x22), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x23), .c(new_n47_), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x09), .c(new_n46_), .O(new_n129_));
  nand2  g085(.a(x08), .b(x04), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(new_n124_), .O(z13));
  nand3  g087(.a(x26), .b(x25), .c(x16), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x23), .c(x22), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n111_), .c(x24), .O(new_n134_));
  nand4  g090(.a(new_n112_), .b(new_n75_), .c(x23), .d(x22), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n137_));
  oai21  g093(.a(new_n60_), .b(new_n67_), .c(new_n137_), .O(z14));
  nand2  g094(.a(x25), .b(new_n74_), .O(new_n139_));
  nand3  g095(.a(new_n77_), .b(x23), .c(x22), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n111_), .c(new_n139_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x24), .O(new_n142_));
  nand2  g098(.a(x26), .b(x17), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n112_), .c(x24), .d(x22), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(x25), .c(x23), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n69_), .O(z15));
  nand3  g104(.a(new_n88_), .b(x26), .c(new_n47_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x24), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n74_), .O(new_n151_));
  inv1   g107(.a(x18), .O(new_n152_));
  nand3  g108(.a(x20), .b(x19), .c(new_n152_), .O(new_n153_));
  nand4  g109(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n153_), .c(x26), .O(new_n155_));
  nand3  g111(.a(new_n76_), .b(new_n45_), .c(x25), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n122_), .c(new_n155_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n151_), .c(new_n71_), .O(z16));
endmodule


