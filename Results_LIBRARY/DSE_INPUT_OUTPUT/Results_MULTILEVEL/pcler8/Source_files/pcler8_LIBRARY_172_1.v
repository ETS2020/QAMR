// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x24), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(x20), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  and2   g012(.a(new_n56_), .b(x00), .O(z01));
  inv1   g013(.a(new_n55_), .O(new_n58_));
  nand2  g014(.a(x08), .b(x01), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n58_), .O(z03));
  nand2  g018(.a(x08), .b(x03), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(z04));
  nand2  g020(.a(new_n56_), .b(x04), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z05));
  nand2  g022(.a(new_n56_), .b(x05), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z06));
  nand2  g024(.a(x08), .b(x06), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n58_), .O(z07));
  and2   g026(.a(new_n56_), .b(x07), .O(z08));
  and2   g027(.a(x22), .b(x21), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(x11), .O(new_n73_));
  inv1   g029(.a(x25), .O(new_n74_));
  nor2   g030(.a(new_n45_), .b(new_n74_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(x24), .c(x23), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(x19), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n78_));
  aoi21  g034(.a(x08), .b(x00), .c(new_n55_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n78_), .O(z09));
  nand2  g036(.a(new_n56_), .b(x01), .O(new_n81_));
  nand2  g037(.a(new_n72_), .b(x12), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n76_), .c(x19), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x20), .O(new_n84_));
  inv1   g040(.a(x20), .O(new_n85_));
  nand3  g041(.a(new_n54_), .b(new_n85_), .c(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n81_), .O(z10));
  nand3  g045(.a(x23), .b(x22), .c(x13), .O(new_n90_));
  nand3  g046(.a(x26), .b(x25), .c(x24), .O(new_n91_));
  oai22  g047(.a(new_n91_), .b(new_n90_), .c(x21), .d(new_n85_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(x19), .O(new_n93_));
  inv1   g049(.a(x19), .O(new_n94_));
  oai21  g050(.a(new_n85_), .b(new_n94_), .c(x21), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n61_), .c(new_n58_), .O(z11));
  nand2  g054(.a(new_n56_), .b(x03), .O(new_n99_));
  nand2  g055(.a(x23), .b(x14), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n91_), .c(x21), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n94_), .c(x22), .O(new_n102_));
  inv1   g058(.a(x22), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(x21), .c(x19), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x20), .O(new_n106_));
  nand3  g062(.a(new_n54_), .b(x22), .c(new_n85_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n99_), .O(z12));
  nand4  g066(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x23), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x21), .c(x19), .O(new_n113_));
  nand2  g069(.a(x21), .b(x19), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x23), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n113_), .c(new_n103_), .O(new_n116_));
  inv1   g072(.a(x23), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(x22), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x20), .O(new_n119_));
  nand3  g075(.a(new_n54_), .b(x23), .c(new_n85_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n65_), .O(z13));
  nand2  g079(.a(new_n75_), .b(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(x22), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n114_), .c(x24), .O(new_n126_));
  inv1   g082(.a(new_n114_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n54_), .c(x23), .d(x22), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n126_), .c(new_n85_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n67_), .O(z14));
  nand4  g087(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n132_));
  nand3  g088(.a(new_n72_), .b(new_n74_), .c(x23), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n132_), .c(x20), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x24), .O(new_n135_));
  nand2  g091(.a(new_n72_), .b(x19), .O(new_n136_));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x24), .c(x23), .O(new_n138_));
  nor2   g094(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nor2   g095(.a(new_n139_), .b(new_n74_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n135_), .c(new_n69_), .O(z15));
  nand4  g098(.a(new_n72_), .b(new_n45_), .c(x25), .d(x23), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n132_), .c(x20), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x24), .O(new_n145_));
  nand4  g101(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n146_));
  nor3   g102(.a(new_n146_), .b(new_n114_), .c(x18), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n45_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n149_));
  nand2  g105(.a(x08), .b(x07), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n149_), .c(new_n145_), .O(z16));
endmodule


