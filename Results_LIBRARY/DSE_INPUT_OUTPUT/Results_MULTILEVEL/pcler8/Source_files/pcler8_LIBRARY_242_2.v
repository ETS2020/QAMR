// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x23), .O(new_n54_));
  nor2   g010(.a(x24), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  nand2  g014(.a(x08), .b(x01), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g016(.a(x02), .O(new_n61_));
  nand2  g017(.a(new_n56_), .b(x08), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n61_), .O(z03));
  inv1   g019(.a(x03), .O(new_n64_));
  nor2   g020(.a(new_n62_), .b(new_n64_), .O(z04));
  inv1   g021(.a(x04), .O(new_n66_));
  nor2   g022(.a(new_n62_), .b(new_n66_), .O(z05));
  inv1   g023(.a(x05), .O(new_n68_));
  nor2   g024(.a(new_n62_), .b(new_n68_), .O(z06));
  inv1   g025(.a(x06), .O(new_n70_));
  nor2   g026(.a(new_n62_), .b(new_n70_), .O(z07));
  nand3  g027(.a(new_n56_), .b(x08), .c(x07), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  inv1   g029(.a(x19), .O(new_n74_));
  inv1   g030(.a(x09), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(x08), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n74_), .c(new_n47_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n57_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n56_), .O(new_n79_));
  nand4  g035(.a(new_n76_), .b(x20), .c(x11), .d(new_n47_), .O(new_n80_));
  nand2  g036(.a(x23), .b(x22), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  inv1   g038(.a(x25), .O(new_n83_));
  nor2   g039(.a(new_n45_), .b(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x24), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n82_), .c(x21), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n80_), .c(new_n79_), .O(z09));
  inv1   g044(.a(x20), .O(new_n89_));
  nand3  g045(.a(x22), .b(x21), .c(x12), .O(new_n90_));
  nand3  g046(.a(x26), .b(x25), .c(x23), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n90_), .c(x20), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(x19), .O(new_n93_));
  oai21  g049(.a(new_n89_), .b(x19), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n59_), .c(new_n56_), .O(z10));
  inv1   g052(.a(x21), .O(new_n97_));
  nand2  g053(.a(x20), .b(x19), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n97_), .b(x20), .c(x19), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n102_));
  oai21  g058(.a(new_n46_), .b(new_n61_), .c(new_n102_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n56_), .O(new_n104_));
  nand2  g060(.a(x21), .b(x13), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(x10), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n86_), .c(new_n82_), .d(new_n76_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n104_), .O(z11));
  nand3  g064(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n109_));
  nand3  g065(.a(new_n84_), .b(x22), .c(x14), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n109_), .c(x24), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x23), .O(new_n112_));
  inv1   g068(.a(x22), .O(new_n113_));
  nand3  g069(.a(x21), .b(x20), .c(x19), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand2  g071(.a(new_n113_), .b(x21), .O(new_n116_));
  oai22  g072(.a(new_n116_), .b(new_n98_), .c(new_n115_), .d(new_n113_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n118_));
  nand2  g074(.a(x08), .b(x03), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n118_), .c(new_n112_), .O(z12));
  nand2  g076(.a(new_n84_), .b(x15), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n115_), .c(x22), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x23), .O(new_n123_));
  nand4  g079(.a(new_n99_), .b(new_n54_), .c(x22), .d(x21), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n126_));
  aoi21  g082(.a(x08), .b(x04), .c(new_n55_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n126_), .O(z13));
  nand2  g084(.a(new_n84_), .b(x16), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n115_), .c(x23), .d(x22), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(x24), .c(new_n47_), .d(x09), .O(new_n131_));
  oai22  g087(.a(new_n131_), .b(x08), .c(new_n62_), .d(new_n68_), .O(z14));
  nand3  g088(.a(new_n76_), .b(x19), .c(new_n47_), .O(new_n133_));
  nand4  g089(.a(new_n83_), .b(x22), .c(x21), .d(x20), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n133_), .c(x24), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x23), .O(new_n136_));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x23), .c(x22), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n115_), .c(new_n83_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n141_));
  nand2  g097(.a(x08), .b(x06), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n141_), .c(new_n136_), .O(z15));
  nor2   g099(.a(new_n98_), .b(x18), .O(new_n144_));
  nand3  g100(.a(x25), .b(x22), .c(x21), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n144_), .c(new_n45_), .O(new_n147_));
  nand4  g103(.a(new_n45_), .b(x25), .c(x23), .d(x22), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n114_), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n147_), .c(x24), .O(new_n150_));
  nand2  g106(.a(x26), .b(new_n54_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n72_), .O(z16));
endmodule


