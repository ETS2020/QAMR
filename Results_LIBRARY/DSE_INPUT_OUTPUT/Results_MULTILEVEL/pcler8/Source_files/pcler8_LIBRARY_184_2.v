// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(x24), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x09), .O(new_n49_));
  nor2   g005(.a(new_n49_), .b(x08), .O(new_n50_));
  inv1   g006(.a(x19), .O(new_n51_));
  inv1   g007(.a(x20), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  and2   g009(.a(x23), .b(x22), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(x21), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g013(.a(new_n57_), .b(new_n53_), .c(new_n50_), .d(new_n48_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n47_), .O(z00));
  nand2  g015(.a(x08), .b(x00), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n47_), .O(z01));
  inv1   g017(.a(x08), .O(new_n62_));
  nor2   g018(.a(new_n46_), .b(new_n62_), .O(new_n63_));
  and2   g019(.a(new_n63_), .b(x01), .O(z02));
  and2   g020(.a(new_n63_), .b(x02), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n47_), .O(z04));
  nand2  g023(.a(x08), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n47_), .O(z05));
  nand2  g025(.a(x08), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n47_), .O(z06));
  nand2  g027(.a(x08), .b(x06), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n47_), .O(z07));
  nand2  g029(.a(new_n63_), .b(x07), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(z08));
  nand3  g031(.a(new_n50_), .b(new_n51_), .c(new_n48_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n60_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n47_), .O(new_n78_));
  nand2  g034(.a(x20), .b(x11), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x10), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n57_), .c(new_n50_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n78_), .O(z09));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  nand4  g039(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(x19), .O(new_n86_));
  oai21  g042(.a(new_n52_), .b(x19), .c(new_n86_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n88_));
  aoi21  g044(.a(x08), .b(x01), .c(new_n46_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(z10));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n56_), .c(x21), .O(new_n92_));
  inv1   g048(.a(x21), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(x20), .O(new_n94_));
  aoi21  g050(.a(new_n92_), .b(x20), .c(new_n94_), .O(new_n95_));
  nand2  g051(.a(x21), .b(new_n51_), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n51_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n98_));
  aoi21  g054(.a(x08), .b(x02), .c(new_n46_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(z11));
  inv1   g056(.a(new_n53_), .O(new_n101_));
  nand2  g057(.a(x23), .b(x14), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n56_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n101_), .c(x22), .O(new_n104_));
  nor2   g060(.a(x22), .b(new_n93_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n53_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n66_), .c(new_n47_), .O(z12));
  nand4  g065(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x22), .c(x21), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n101_), .c(x23), .O(new_n112_));
  inv1   g068(.a(x23), .O(new_n113_));
  nand4  g069(.a(new_n53_), .b(new_n113_), .c(x22), .d(x21), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n68_), .c(new_n47_), .O(z13));
  inv1   g073(.a(x24), .O(new_n118_));
  nand3  g074(.a(new_n50_), .b(x19), .c(new_n48_), .O(new_n119_));
  nand3  g075(.a(new_n54_), .b(x21), .c(x20), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n119_), .c(new_n45_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand3  g078(.a(x21), .b(x20), .c(x19), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand3  g080(.a(x26), .b(x25), .c(x16), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x23), .c(x22), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n124_), .c(new_n118_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n122_), .c(new_n70_), .O(z14));
  nand2  g086(.a(x26), .b(x17), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n124_), .c(x23), .d(x22), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(x25), .c(new_n48_), .d(x09), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(x08), .c(new_n72_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n47_), .O(new_n135_));
  nand3  g091(.a(x25), .b(new_n118_), .c(new_n45_), .O(new_n136_));
  inv1   g092(.a(x25), .O(new_n137_));
  nand3  g093(.a(new_n54_), .b(new_n137_), .c(x24), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n123_), .c(new_n136_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n135_), .O(z15));
  inv1   g097(.a(x26), .O(new_n142_));
  nand3  g098(.a(x25), .b(x23), .c(x22), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n123_), .c(new_n142_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x18), .O(new_n145_));
  nand4  g101(.a(new_n124_), .b(new_n54_), .c(new_n142_), .d(x25), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x24), .O(new_n148_));
  nand4  g104(.a(new_n124_), .b(new_n54_), .c(x25), .d(x24), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(x26), .c(new_n45_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n74_), .O(z16));
endmodule


