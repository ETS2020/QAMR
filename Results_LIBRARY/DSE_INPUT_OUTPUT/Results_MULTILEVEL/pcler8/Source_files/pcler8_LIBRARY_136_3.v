// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x13), .O(new_n45_));
  inv1   g001(.a(x21), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  nand2  g003(.a(x20), .b(x19), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x10), .O(new_n49_));
  nand2  g005(.a(x23), .b(x22), .O(new_n50_));
  nand3  g006(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(new_n49_), .c(x09), .d(new_n47_), .O(new_n53_));
  aoi21  g009(.a(new_n53_), .b(new_n45_), .c(new_n46_), .O(z00));
  nor2   g010(.a(new_n46_), .b(new_n45_), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(z01));
  nand2  g014(.a(new_n56_), .b(x01), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z02));
  inv1   g016(.a(new_n55_), .O(new_n61_));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(z03));
  nand2  g019(.a(new_n56_), .b(x03), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z04));
  nand2  g021(.a(new_n56_), .b(x04), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(new_n56_), .b(x05), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z06));
  nand2  g025(.a(x08), .b(x06), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n61_), .O(z07));
  nand2  g027(.a(new_n56_), .b(x07), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  nand2  g029(.a(x08), .b(x00), .O(new_n74_));
  inv1   g030(.a(x10), .O(new_n75_));
  inv1   g031(.a(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n61_), .O(new_n79_));
  nand3  g035(.a(new_n75_), .b(x09), .c(new_n47_), .O(new_n80_));
  nand3  g036(.a(x20), .b(new_n45_), .c(x11), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor3   g038(.a(new_n51_), .b(new_n50_), .c(new_n46_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n79_), .O(z09));
  xor2a  g041(.a(x20), .b(x19), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n87_));
  nand2  g043(.a(x08), .b(x01), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n61_), .O(new_n90_));
  nand3  g046(.a(x19), .b(new_n45_), .c(x12), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n80_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n83_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n90_), .O(z10));
  inv1   g050(.a(new_n48_), .O(new_n95_));
  nand3  g051(.a(new_n46_), .b(x20), .c(x19), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n46_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n62_), .c(new_n61_), .O(z11));
  inv1   g055(.a(x22), .O(new_n100_));
  nand2  g056(.a(x23), .b(x14), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n51_), .c(x22), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(x21), .c(x20), .d(x19), .O(new_n103_));
  nand3  g059(.a(x21), .b(x20), .c(x19), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n100_), .c(new_n103_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n107_));
  aoi21  g063(.a(x08), .b(x03), .c(new_n55_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n107_), .O(z12));
  nand4  g065(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x23), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x22), .O(new_n112_));
  nand2  g068(.a(x23), .b(new_n100_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(x21), .c(x20), .d(x19), .O(new_n115_));
  nand2  g071(.a(new_n104_), .b(x23), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n118_));
  aoi21  g074(.a(x08), .b(x04), .c(new_n55_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n118_), .O(z13));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n95_), .c(x23), .d(x22), .O(new_n122_));
  inv1   g078(.a(x24), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  nor2   g080(.a(new_n124_), .b(new_n104_), .O(new_n125_));
  aoi21  g081(.a(new_n122_), .b(x24), .c(new_n125_), .O(new_n126_));
  nand2  g082(.a(x24), .b(new_n46_), .O(new_n127_));
  oai21  g083(.a(new_n126_), .b(x13), .c(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n68_), .O(z14));
  nand2  g086(.a(new_n56_), .b(x06), .O(new_n131_));
  nand3  g087(.a(x22), .b(x20), .c(x19), .O(new_n132_));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x24), .c(x23), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n132_), .c(x25), .O(new_n135_));
  nor2   g091(.a(x25), .b(new_n123_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n105_), .c(x23), .d(x22), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n45_), .O(new_n139_));
  nand2  g095(.a(x25), .b(new_n46_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n131_), .O(z15));
  inv1   g099(.a(x26), .O(new_n144_));
  nor2   g100(.a(new_n48_), .b(x18), .O(new_n145_));
  nand4  g101(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n145_), .c(new_n144_), .O(new_n148_));
  nand4  g104(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n149_));
  nand4  g105(.a(new_n144_), .b(x25), .c(x24), .d(x23), .O(new_n150_));
  nor2   g106(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n148_), .c(new_n45_), .O(new_n152_));
  nand2  g108(.a(x26), .b(new_n46_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n72_), .O(z16));
endmodule


