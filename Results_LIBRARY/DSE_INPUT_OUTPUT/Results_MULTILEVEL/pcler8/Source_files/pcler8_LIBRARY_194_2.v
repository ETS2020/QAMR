// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n67_, new_n69_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x10), .O(new_n45_));
  nor2   g001(.a(x22), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x08), .O(new_n48_));
  inv1   g004(.a(x19), .O(new_n49_));
  inv1   g005(.a(x20), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n52_));
  inv1   g008(.a(x22), .O(new_n53_));
  inv1   g009(.a(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n55_), .c(x21), .O(new_n58_));
  oai21  g014(.a(new_n58_), .b(new_n52_), .c(new_n47_), .O(z00));
  inv1   g015(.a(x00), .O(new_n60_));
  nor2   g016(.a(new_n46_), .b(new_n48_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n60_), .O(z01));
  inv1   g019(.a(x01), .O(new_n64_));
  nor2   g020(.a(new_n62_), .b(new_n64_), .O(z02));
  and2   g021(.a(new_n61_), .b(x02), .O(z03));
  inv1   g022(.a(x03), .O(new_n67_));
  oai21  g023(.a(new_n48_), .b(new_n67_), .c(new_n47_), .O(z04));
  nand2  g024(.a(new_n61_), .b(x04), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z05));
  and2   g026(.a(new_n61_), .b(x05), .O(z06));
  and2   g027(.a(new_n61_), .b(x06), .O(z07));
  inv1   g028(.a(x07), .O(new_n73_));
  oai21  g029(.a(new_n48_), .b(new_n73_), .c(new_n47_), .O(z08));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n53_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  nand4  g033(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n80_));
  oai21  g036(.a(new_n62_), .b(new_n60_), .c(new_n80_), .O(z09));
  nand3  g037(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n78_), .c(x20), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x19), .O(new_n84_));
  oai21  g040(.a(new_n50_), .b(x19), .c(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n86_));
  oai21  g042(.a(new_n62_), .b(new_n64_), .c(new_n86_), .O(z10));
  nand3  g043(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n56_), .c(x21), .O(new_n89_));
  nor2   g045(.a(new_n75_), .b(x20), .O(new_n90_));
  aoi21  g046(.a(new_n89_), .b(x20), .c(new_n90_), .O(new_n91_));
  nand2  g047(.a(x21), .b(new_n49_), .O(new_n92_));
  oai21  g048(.a(new_n91_), .b(new_n49_), .c(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n94_));
  aoi21  g050(.a(x08), .b(x02), .c(new_n46_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n94_), .O(z11));
  inv1   g052(.a(new_n51_), .O(new_n97_));
  nand2  g053(.a(x23), .b(x14), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n56_), .c(x21), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n97_), .c(x22), .O(new_n100_));
  nand3  g056(.a(new_n51_), .b(new_n53_), .c(x21), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n103_));
  oai21  g059(.a(new_n62_), .b(new_n67_), .c(new_n103_), .O(z12));
  nand4  g060(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x23), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(x21), .c(x20), .d(x19), .O(new_n107_));
  nand3  g063(.a(x21), .b(x20), .c(x19), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x23), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x22), .O(new_n111_));
  nand2  g067(.a(x23), .b(new_n53_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n69_), .O(z13));
  nand2  g071(.a(x24), .b(x09), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(x08), .c(new_n45_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n53_), .O(new_n118_));
  nand3  g074(.a(x26), .b(x25), .c(x16), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x23), .c(x21), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n97_), .c(x24), .O(new_n121_));
  inv1   g077(.a(x24), .O(new_n122_));
  inv1   g078(.a(new_n108_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n122_), .c(x23), .d(x22), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n126_));
  nand2  g082(.a(x08), .b(x05), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n126_), .c(new_n118_), .O(z14));
  nand2  g084(.a(x25), .b(x09), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(x08), .c(new_n45_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n53_), .O(new_n131_));
  nand2  g087(.a(x26), .b(x17), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x24), .c(x23), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n108_), .c(x25), .O(new_n134_));
  inv1   g090(.a(x25), .O(new_n135_));
  nand4  g091(.a(new_n123_), .b(new_n55_), .c(new_n135_), .d(x24), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n138_));
  nand2  g094(.a(x08), .b(x06), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n138_), .c(new_n131_), .O(z15));
  inv1   g096(.a(x18), .O(new_n141_));
  nand4  g097(.a(x21), .b(x20), .c(x19), .d(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n55_), .b(x25), .c(x24), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n142_), .c(x26), .O(new_n144_));
  nor2   g100(.a(new_n122_), .b(new_n54_), .O(new_n145_));
  nor2   g101(.a(x26), .b(new_n135_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n145_), .c(new_n76_), .d(new_n51_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n149_));
  oai21  g105(.a(new_n62_), .b(new_n73_), .c(new_n149_), .O(z16));
endmodule


