// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x25), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(x10), .O(new_n51_));
  inv1   g007(.a(x21), .O(new_n52_));
  nand2  g008(.a(x23), .b(x22), .O(new_n53_));
  inv1   g009(.a(x26), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(x24), .O(new_n56_));
  nor3   g012(.a(new_n56_), .b(new_n53_), .c(new_n52_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n51_), .c(new_n49_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n47_), .O(z00));
  nand2  g015(.a(x08), .b(x00), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n47_), .O(z01));
  inv1   g017(.a(x01), .O(new_n62_));
  nand2  g018(.a(new_n47_), .b(x08), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n62_), .O(z02));
  nand2  g020(.a(x08), .b(x02), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n47_), .O(z03));
  nand2  g022(.a(x08), .b(x03), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n47_), .O(z04));
  nand2  g024(.a(x08), .b(x04), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n47_), .O(z05));
  inv1   g026(.a(x05), .O(new_n71_));
  nor2   g027(.a(new_n63_), .b(new_n71_), .O(z06));
  nand2  g028(.a(x08), .b(x06), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n47_), .O(z07));
  nand2  g030(.a(x08), .b(x07), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n47_), .O(z08));
  inv1   g032(.a(x08), .O(new_n77_));
  inv1   g033(.a(x10), .O(new_n78_));
  nand4  g034(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n79_));
  inv1   g035(.a(x24), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n45_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n55_), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n79_), .c(x19), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n78_), .c(x09), .d(new_n77_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n60_), .c(new_n47_), .O(z09));
  xor2a  g041(.a(x20), .b(x19), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n78_), .c(x09), .d(new_n77_), .O(new_n87_));
  oai21  g043(.a(new_n77_), .b(new_n62_), .c(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n47_), .O(new_n89_));
  nand2  g045(.a(x19), .b(x12), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(x10), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n57_), .c(new_n49_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n89_), .O(z10));
  inv1   g049(.a(x19), .O(new_n94_));
  xnor2a g050(.a(x21), .b(x20), .O(new_n95_));
  nand2  g051(.a(x21), .b(new_n94_), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n94_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n78_), .c(x09), .d(new_n77_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n65_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n47_), .O(new_n100_));
  nand2  g056(.a(x19), .b(x13), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(x10), .O(new_n102_));
  inv1   g058(.a(x20), .O(new_n103_));
  nor3   g059(.a(new_n56_), .b(new_n53_), .c(new_n103_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n102_), .c(new_n49_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n100_), .O(z11));
  nand3  g062(.a(x21), .b(x20), .c(x19), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x22), .O(new_n108_));
  inv1   g064(.a(x22), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x21), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n50_), .c(new_n108_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n78_), .c(x09), .d(new_n77_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n67_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n47_), .O(new_n114_));
  nor2   g070(.a(new_n56_), .b(new_n53_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n49_), .c(x14), .d(new_n78_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n114_), .O(z12));
  nand3  g073(.a(new_n78_), .b(x09), .c(new_n77_), .O(new_n118_));
  nand4  g074(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n118_), .c(x25), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n45_), .O(new_n121_));
  inv1   g077(.a(new_n107_), .O(new_n122_));
  nand4  g078(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n122_), .c(x22), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(new_n78_), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x09), .c(new_n77_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n121_), .c(new_n69_), .O(z13));
  nand3  g084(.a(new_n49_), .b(x24), .c(new_n78_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x25), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n45_), .O(new_n131_));
  nand3  g087(.a(x26), .b(x25), .c(x16), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x22), .c(x21), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n50_), .c(x24), .O(new_n134_));
  nand4  g090(.a(new_n122_), .b(new_n80_), .c(x23), .d(x22), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n78_), .c(x09), .d(new_n77_), .O(new_n137_));
  nand2  g093(.a(x08), .b(x05), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n137_), .c(new_n131_), .O(z14));
  nand2  g095(.a(new_n118_), .b(x25), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n45_), .O(new_n141_));
  nand2  g097(.a(x26), .b(x17), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(x24), .c(x22), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n107_), .c(x25), .O(new_n144_));
  nand4  g100(.a(new_n122_), .b(new_n46_), .c(x24), .d(x22), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n78_), .c(x09), .d(new_n77_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n141_), .c(new_n73_), .O(z15));
  nand3  g104(.a(new_n49_), .b(x26), .c(new_n78_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x25), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n45_), .O(new_n151_));
  inv1   g107(.a(x18), .O(new_n152_));
  nand3  g108(.a(x20), .b(x19), .c(new_n152_), .O(new_n153_));
  nand4  g109(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n153_), .c(x26), .O(new_n155_));
  nand3  g111(.a(new_n81_), .b(new_n54_), .c(x25), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n119_), .c(new_n155_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n78_), .c(x09), .d(new_n77_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n151_), .c(new_n75_), .O(z16));
endmodule


