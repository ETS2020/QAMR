// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(new_n48_), .b(x09), .c(new_n47_), .d(x03), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  inv1   g011(.a(x23), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(x03), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(new_n59_));
  nor2   g015(.a(new_n59_), .b(new_n55_), .O(z01));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n59_), .b(new_n61_), .O(z02));
  aoi21  g018(.a(x08), .b(x02), .c(new_n57_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z03));
  inv1   g020(.a(new_n57_), .O(new_n65_));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n65_), .O(z04));
  nand2  g023(.a(new_n58_), .b(x04), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z05));
  nand2  g025(.a(new_n58_), .b(x05), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z06));
  nand2  g027(.a(new_n58_), .b(x06), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z07));
  nand2  g029(.a(x08), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n65_), .O(z08));
  inv1   g031(.a(x09), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(x08), .O(new_n77_));
  nor2   g033(.a(x19), .b(x10), .O(new_n78_));
  aoi22  g034(.a(new_n78_), .b(new_n77_), .c(x08), .d(x00), .O(new_n79_));
  nand2  g035(.a(x20), .b(x11), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x10), .O(new_n81_));
  nor2   g037(.a(new_n56_), .b(new_n46_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x21), .O(new_n83_));
  nand3  g039(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n81_), .c(new_n77_), .d(x03), .O(new_n86_));
  oai21  g042(.a(new_n79_), .b(new_n57_), .c(new_n86_), .O(z09));
  xor2a  g043(.a(x20), .b(x19), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n89_));
  oai21  g045(.a(new_n47_), .b(new_n61_), .c(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n65_), .O(new_n91_));
  nand2  g047(.a(x19), .b(x12), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x10), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n85_), .c(new_n77_), .d(x03), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n91_), .O(z10));
  inv1   g051(.a(new_n84_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n82_), .c(x13), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(x21), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(x20), .c(x19), .O(new_n99_));
  inv1   g055(.a(x21), .O(new_n100_));
  and2   g056(.a(x20), .b(x19), .O(new_n101_));
  or2    g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n99_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n63_), .O(z11));
  inv1   g061(.a(x03), .O(new_n106_));
  nand2  g062(.a(new_n56_), .b(new_n47_), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n106_), .c(x22), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  nand2  g065(.a(x23), .b(new_n106_), .O(new_n110_));
  nand3  g066(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(new_n110_), .c(new_n47_), .O(new_n112_));
  nand4  g068(.a(new_n96_), .b(x23), .c(x14), .d(x03), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x22), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n109_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n48_), .c(x09), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n66_), .O(z12));
  inv1   g074(.a(new_n111_), .O(new_n119_));
  nand4  g075(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n119_), .c(x22), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x03), .O(new_n122_));
  nand4  g078(.a(new_n101_), .b(new_n56_), .c(x22), .d(x21), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n68_), .O(z13));
  nand3  g082(.a(x26), .b(x25), .c(x16), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n101_), .c(x22), .d(x21), .O(new_n128_));
  inv1   g084(.a(x24), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x23), .c(x22), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n111_), .O(new_n131_));
  aoi21  g087(.a(new_n128_), .b(x24), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(x24), .b(new_n56_), .O(new_n133_));
  oai21  g089(.a(new_n132_), .b(new_n106_), .c(new_n133_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n70_), .O(z14));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x24), .c(x22), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n111_), .c(x25), .O(new_n139_));
  nor2   g095(.a(x25), .b(new_n129_), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n119_), .c(new_n82_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x03), .O(new_n143_));
  nand2  g099(.a(x25), .b(new_n56_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n72_), .O(z15));
  nand3  g103(.a(new_n101_), .b(new_n77_), .c(new_n48_), .O(new_n148_));
  nor2   g104(.a(new_n46_), .b(new_n100_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n45_), .c(x25), .d(x24), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n148_), .c(x03), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x23), .O(new_n152_));
  inv1   g108(.a(x18), .O(new_n153_));
  nand4  g109(.a(x21), .b(x20), .c(x19), .d(new_n153_), .O(new_n154_));
  nand3  g110(.a(new_n82_), .b(x25), .c(x24), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nor2   g112(.a(new_n156_), .b(new_n45_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n152_), .c(new_n74_), .O(z16));
endmodule


