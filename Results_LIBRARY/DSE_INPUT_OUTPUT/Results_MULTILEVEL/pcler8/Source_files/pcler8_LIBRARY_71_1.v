// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x24), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(x23), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n55_), .b(new_n46_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n59_), .O(z02));
  nand2  g018(.a(x08), .b(x02), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n56_), .O(z03));
  inv1   g020(.a(x03), .O(new_n65_));
  nor2   g021(.a(new_n61_), .b(new_n65_), .O(z04));
  nand2  g022(.a(x08), .b(x04), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n56_), .O(z05));
  and2   g024(.a(new_n60_), .b(x05), .O(z06));
  nand2  g025(.a(x08), .b(x06), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n56_), .O(z07));
  nand2  g027(.a(x08), .b(x07), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n56_), .O(z08));
  nand3  g029(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  inv1   g030(.a(x25), .O(new_n75_));
  nor2   g031(.a(new_n45_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x24), .c(x22), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(x19), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n57_), .c(new_n56_), .O(z09));
  xor2a  g036(.a(x20), .b(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n82_));
  oai21  g038(.a(new_n46_), .b(new_n59_), .c(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n56_), .O(new_n84_));
  inv1   g040(.a(x09), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(x08), .O(new_n86_));
  nand2  g042(.a(x19), .b(x12), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(x10), .O(new_n88_));
  inv1   g044(.a(x21), .O(new_n89_));
  nand2  g045(.a(x23), .b(x22), .O(new_n90_));
  nand2  g046(.a(new_n76_), .b(x24), .O(new_n91_));
  nor3   g047(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n88_), .c(new_n86_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n84_), .O(z10));
  inv1   g050(.a(x19), .O(new_n95_));
  xnor2a g051(.a(x21), .b(x20), .O(new_n96_));
  nand2  g052(.a(x21), .b(new_n95_), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n63_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n56_), .O(new_n101_));
  nand2  g057(.a(x19), .b(x13), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(x10), .O(new_n103_));
  inv1   g059(.a(x20), .O(new_n104_));
  nor3   g060(.a(new_n91_), .b(new_n90_), .c(new_n104_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n103_), .c(new_n86_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n101_), .O(z11));
  inv1   g063(.a(x22), .O(new_n108_));
  nand3  g064(.a(x21), .b(x20), .c(x19), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand2  g066(.a(x20), .b(x19), .O(new_n111_));
  nand2  g067(.a(new_n108_), .b(x21), .O(new_n112_));
  oai22  g068(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(new_n108_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n114_));
  oai21  g070(.a(new_n46_), .b(new_n65_), .c(new_n114_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n56_), .O(new_n116_));
  nor2   g072(.a(new_n91_), .b(new_n90_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n86_), .c(x14), .d(new_n47_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n116_), .O(z12));
  nand2  g075(.a(new_n60_), .b(x04), .O(new_n120_));
  nand3  g076(.a(new_n76_), .b(x24), .c(x15), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n110_), .c(x22), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x23), .O(new_n123_));
  inv1   g079(.a(x23), .O(new_n124_));
  nand4  g080(.a(new_n110_), .b(new_n54_), .c(new_n124_), .d(x22), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n120_), .O(z13));
  nand3  g084(.a(x26), .b(x25), .c(x16), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x22), .c(x21), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n111_), .c(x24), .O(new_n131_));
  nand4  g087(.a(new_n110_), .b(new_n54_), .c(x23), .d(x22), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n134_));
  aoi21  g090(.a(x08), .b(x05), .c(new_n55_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(z14));
  nand2  g092(.a(new_n60_), .b(x06), .O(new_n137_));
  inv1   g093(.a(new_n111_), .O(new_n138_));
  nand2  g094(.a(x26), .b(x17), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n138_), .c(x22), .d(x21), .O(new_n140_));
  nand3  g096(.a(new_n75_), .b(x24), .c(x22), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n109_), .O(new_n142_));
  aoi21  g098(.a(new_n140_), .b(x25), .c(new_n142_), .O(new_n143_));
  nand2  g099(.a(x25), .b(new_n54_), .O(new_n144_));
  oai21  g100(.a(new_n143_), .b(new_n124_), .c(new_n144_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n137_), .O(z15));
  nand2  g103(.a(new_n60_), .b(x07), .O(new_n148_));
  nor2   g104(.a(new_n111_), .b(x18), .O(new_n149_));
  nand3  g105(.a(x25), .b(x22), .c(x21), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n149_), .c(new_n45_), .O(new_n152_));
  nand4  g108(.a(new_n45_), .b(x25), .c(x24), .d(x22), .O(new_n153_));
  nor2   g109(.a(new_n153_), .b(new_n109_), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n152_), .c(x23), .O(new_n155_));
  nand2  g111(.a(x26), .b(new_n54_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n148_), .O(z16));
endmodule


