// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x20), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x21), .O(new_n50_));
  inv1   g006(.a(x22), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g008(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(x19), .c(new_n45_), .O(z00));
  nor2   g012(.a(new_n45_), .b(x19), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n46_), .O(new_n58_));
  and2   g014(.a(new_n58_), .b(x00), .O(z01));
  inv1   g015(.a(new_n57_), .O(new_n60_));
  nand2  g016(.a(x08), .b(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(z02));
  inv1   g018(.a(x02), .O(new_n63_));
  inv1   g019(.a(new_n58_), .O(new_n64_));
  nor2   g020(.a(new_n64_), .b(new_n63_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n60_), .O(z04));
  nand2  g023(.a(x08), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n60_), .O(z05));
  nand2  g025(.a(new_n58_), .b(x05), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z06));
  nand2  g027(.a(x08), .b(x06), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n60_), .O(z07));
  and2   g029(.a(new_n58_), .b(x07), .O(z08));
  nand3  g030(.a(new_n52_), .b(x20), .c(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n78_));
  aoi21  g034(.a(x08), .b(x00), .c(new_n57_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n78_), .O(z09));
  inv1   g036(.a(x19), .O(new_n81_));
  oai21  g037(.a(new_n49_), .b(x23), .c(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n52_), .b(x12), .O(new_n83_));
  oai22  g039(.a(new_n83_), .b(new_n76_), .c(x20), .d(new_n81_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n82_), .c(new_n61_), .O(z10));
  nand3  g042(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n53_), .c(x21), .O(new_n88_));
  nor2   g044(.a(new_n50_), .b(x20), .O(new_n89_));
  aoi21  g045(.a(new_n88_), .b(x20), .c(new_n89_), .O(new_n90_));
  nand3  g046(.a(new_n45_), .b(x21), .c(new_n81_), .O(new_n91_));
  oai21  g047(.a(new_n90_), .b(new_n81_), .c(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n93_));
  oai21  g049(.a(new_n64_), .b(new_n63_), .c(new_n93_), .O(z11));
  nand2  g050(.a(new_n58_), .b(x03), .O(new_n95_));
  inv1   g051(.a(x20), .O(new_n96_));
  nand2  g052(.a(x23), .b(x14), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n53_), .c(x21), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n96_), .c(x22), .O(new_n99_));
  nand3  g055(.a(new_n51_), .b(x21), .c(x20), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x19), .O(new_n102_));
  nand3  g058(.a(new_n45_), .b(x22), .c(new_n81_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n95_), .O(z12));
  nand2  g062(.a(new_n58_), .b(x04), .O(new_n107_));
  nand4  g063(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(x23), .c(new_n50_), .O(new_n109_));
  nand2  g065(.a(x21), .b(x20), .O(new_n110_));
  aoi22  g066(.a(new_n110_), .b(x23), .c(new_n109_), .d(x20), .O(new_n111_));
  nand2  g067(.a(x23), .b(new_n51_), .O(new_n112_));
  oai21  g068(.a(new_n111_), .b(new_n51_), .c(new_n112_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(x19), .c(new_n47_), .d(x09), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(x08), .c(new_n107_), .O(z13));
  inv1   g071(.a(x24), .O(new_n116_));
  inv1   g072(.a(new_n110_), .O(new_n117_));
  nand3  g073(.a(x26), .b(x25), .c(x16), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n117_), .c(x22), .O(new_n119_));
  nand3  g075(.a(new_n116_), .b(x23), .c(x22), .O(new_n120_));
  nor2   g076(.a(new_n120_), .b(new_n110_), .O(new_n121_));
  aoi21  g077(.a(new_n119_), .b(x24), .c(new_n121_), .O(new_n122_));
  oai22  g078(.a(new_n122_), .b(new_n81_), .c(new_n116_), .d(x23), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n70_), .O(z14));
  nand2  g081(.a(new_n58_), .b(x06), .O(new_n126_));
  inv1   g082(.a(x25), .O(new_n127_));
  nand2  g083(.a(x26), .b(x17), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n117_), .c(x24), .d(x22), .O(new_n129_));
  nand3  g085(.a(x22), .b(x21), .c(x20), .O(new_n130_));
  nand3  g086(.a(new_n127_), .b(x24), .c(x23), .O(new_n131_));
  nor2   g087(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  aoi21  g088(.a(new_n129_), .b(x25), .c(new_n132_), .O(new_n133_));
  oai22  g089(.a(new_n133_), .b(new_n81_), .c(new_n127_), .d(x23), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n126_), .O(z15));
  inv1   g092(.a(x26), .O(new_n137_));
  nand4  g093(.a(new_n52_), .b(new_n137_), .c(x25), .d(x24), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n48_), .c(x19), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x23), .O(new_n140_));
  nand4  g096(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n141_));
  nor3   g097(.a(new_n141_), .b(new_n110_), .c(x18), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n137_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n144_));
  nand2  g100(.a(x08), .b(x07), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n144_), .c(new_n140_), .O(z16));
endmodule


