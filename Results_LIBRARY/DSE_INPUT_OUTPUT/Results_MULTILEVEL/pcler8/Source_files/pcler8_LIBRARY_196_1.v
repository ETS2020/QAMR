// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  inv1   g010(.a(x25), .O(new_n55_));
  nand2  g011(.a(x26), .b(new_n55_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x08), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n54_), .O(z01));
  nand2  g014(.a(x08), .b(x01), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n56_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  and2   g017(.a(new_n61_), .b(new_n56_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z03));
  inv1   g019(.a(x03), .O(new_n64_));
  nor2   g020(.a(new_n57_), .b(new_n64_), .O(z04));
  nand2  g021(.a(x08), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n56_), .O(z05));
  inv1   g023(.a(x05), .O(new_n68_));
  nor2   g024(.a(new_n57_), .b(new_n68_), .O(z06));
  inv1   g025(.a(x06), .O(new_n70_));
  nor2   g026(.a(new_n57_), .b(new_n70_), .O(z07));
  inv1   g027(.a(x07), .O(new_n72_));
  nor2   g028(.a(new_n57_), .b(new_n72_), .O(z08));
  inv1   g029(.a(x20), .O(new_n74_));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x11), .O(new_n77_));
  inv1   g033(.a(x22), .O(new_n78_));
  inv1   g034(.a(x23), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x26), .c(x24), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n77_), .c(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n83_));
  nand2  g039(.a(x08), .b(x00), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n83_), .c(new_n56_), .O(z09));
  nand3  g041(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  nand3  g042(.a(x26), .b(x24), .c(x23), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  oai21  g045(.a(new_n74_), .b(x19), .c(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n59_), .c(new_n56_), .O(z10));
  inv1   g048(.a(x19), .O(new_n93_));
  nand2  g049(.a(x22), .b(x13), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n87_), .c(x21), .O(new_n95_));
  nor2   g051(.a(new_n75_), .b(x20), .O(new_n96_));
  aoi21  g052(.a(new_n95_), .b(x20), .c(new_n96_), .O(new_n97_));
  nand2  g053(.a(x21), .b(new_n93_), .O(new_n98_));
  oai21  g054(.a(new_n97_), .b(new_n93_), .c(new_n98_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n62_), .O(z11));
  nand3  g057(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n102_));
  inv1   g058(.a(x24), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n79_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(x22), .c(x14), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n102_), .c(x25), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x26), .O(new_n107_));
  nand3  g063(.a(x21), .b(x20), .c(x19), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand2  g065(.a(x20), .b(x19), .O(new_n110_));
  nand2  g066(.a(new_n78_), .b(x21), .O(new_n111_));
  oai22  g067(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(new_n78_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n113_));
  nand2  g069(.a(x08), .b(x03), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n113_), .c(new_n107_), .O(z12));
  nand4  g071(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x23), .O(new_n117_));
  nand3  g073(.a(new_n79_), .b(x22), .c(x21), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n110_), .c(new_n117_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n66_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n56_), .O(new_n122_));
  nand2  g078(.a(x09), .b(new_n46_), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand3  g080(.a(new_n104_), .b(x26), .c(x25), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n124_), .c(x15), .d(new_n47_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n122_), .O(z13));
  nand3  g084(.a(x24), .b(x16), .c(new_n47_), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n123_), .c(x25), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x26), .O(new_n131_));
  nand3  g087(.a(x23), .b(x22), .c(x21), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n110_), .c(x24), .O(new_n133_));
  nand3  g089(.a(new_n103_), .b(x23), .c(x22), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n108_), .c(new_n133_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n136_));
  nand2  g092(.a(x08), .b(x05), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n136_), .c(new_n131_), .O(z14));
  nand3  g094(.a(new_n124_), .b(x19), .c(new_n47_), .O(new_n139_));
  nand3  g095(.a(x24), .b(x23), .c(x22), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n76_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n139_), .c(new_n45_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n55_), .O(new_n144_));
  oai21  g100(.a(new_n140_), .b(new_n108_), .c(x25), .O(new_n145_));
  nand2  g101(.a(x26), .b(x17), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n148_));
  nand2  g104(.a(x08), .b(x06), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n148_), .c(new_n144_), .O(z15));
  inv1   g106(.a(x18), .O(new_n151_));
  nand3  g107(.a(x20), .b(x19), .c(new_n151_), .O(new_n152_));
  nand4  g108(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n152_), .c(x26), .O(new_n154_));
  nand4  g110(.a(new_n109_), .b(new_n80_), .c(new_n45_), .d(x24), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n154_), .c(new_n55_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n157_));
  oai21  g113(.a(new_n57_), .b(new_n72_), .c(new_n157_), .O(z16));
endmodule


