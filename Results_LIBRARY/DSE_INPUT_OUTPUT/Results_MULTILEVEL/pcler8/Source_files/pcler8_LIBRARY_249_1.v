// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n65_, new_n66_, new_n68_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x24), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(x19), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  and2   g012(.a(new_n56_), .b(x00), .O(z01));
  inv1   g013(.a(new_n55_), .O(new_n58_));
  nand2  g014(.a(x08), .b(x01), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n58_), .O(z03));
  and2   g018(.a(new_n56_), .b(x03), .O(z04));
  and2   g019(.a(new_n56_), .b(x04), .O(z05));
  inv1   g020(.a(x05), .O(new_n65_));
  inv1   g021(.a(new_n56_), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n65_), .O(z06));
  nand2  g023(.a(new_n56_), .b(x06), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z07));
  and2   g025(.a(new_n56_), .b(x07), .O(z08));
  inv1   g026(.a(x21), .O(new_n71_));
  inv1   g027(.a(x22), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g029(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  inv1   g030(.a(x25), .O(new_n75_));
  nor2   g031(.a(new_n45_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x24), .c(x23), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(x19), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n79_));
  aoi21  g035(.a(x08), .b(x00), .c(new_n55_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n79_), .O(z09));
  inv1   g037(.a(x19), .O(new_n82_));
  inv1   g038(.a(x09), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(x08), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(x20), .c(new_n47_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n54_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n82_), .O(new_n87_));
  nand2  g043(.a(new_n73_), .b(x12), .O(new_n88_));
  oai22  g044(.a(new_n88_), .b(new_n77_), .c(x20), .d(new_n82_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n87_), .c(new_n59_), .O(z10));
  xor2a  g047(.a(x21), .b(x20), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(x19), .O(new_n93_));
  nand2  g049(.a(x21), .b(new_n82_), .O(new_n94_));
  inv1   g050(.a(new_n77_), .O(new_n95_));
  inv1   g051(.a(x20), .O(new_n96_));
  nor2   g052(.a(new_n72_), .b(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n95_), .c(x13), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n94_), .c(new_n93_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n61_), .c(new_n58_), .O(z11));
  nand3  g057(.a(new_n84_), .b(x22), .c(new_n47_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n54_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n82_), .O(new_n104_));
  nand2  g060(.a(x23), .b(x14), .O(new_n105_));
  nand3  g061(.a(x26), .b(x25), .c(x24), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n105_), .c(x21), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n96_), .c(x22), .O(new_n108_));
  nand4  g064(.a(new_n72_), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n111_));
  nand2  g067(.a(x08), .b(x03), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n111_), .c(new_n104_), .O(z12));
  nand3  g069(.a(new_n84_), .b(x23), .c(new_n47_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n54_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n82_), .O(new_n116_));
  inv1   g072(.a(x23), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x19), .O(new_n118_));
  nand4  g074(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n118_), .c(new_n71_), .O(new_n120_));
  nand2  g076(.a(x21), .b(x20), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x19), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  aoi21  g079(.a(new_n120_), .b(x20), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(x23), .b(new_n72_), .O(new_n125_));
  oai21  g081(.a(new_n124_), .b(new_n72_), .c(new_n125_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  nand2  g083(.a(x08), .b(x04), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n127_), .c(new_n116_), .O(z13));
  nand3  g085(.a(x26), .b(x25), .c(x16), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x23), .c(x22), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n121_), .c(x24), .O(new_n132_));
  inv1   g088(.a(new_n121_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n54_), .c(x23), .d(x22), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n132_), .c(new_n82_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n136_));
  oai21  g092(.a(new_n66_), .b(new_n65_), .c(new_n136_), .O(z14));
  nand2  g093(.a(x26), .b(x17), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n133_), .c(x23), .d(x22), .O(new_n139_));
  nand3  g095(.a(x22), .b(x21), .c(x20), .O(new_n140_));
  nand3  g096(.a(new_n75_), .b(x24), .c(x23), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  aoi21  g098(.a(new_n139_), .b(x25), .c(new_n142_), .O(new_n143_));
  nand2  g099(.a(x25), .b(new_n54_), .O(new_n144_));
  oai21  g100(.a(new_n143_), .b(new_n82_), .c(new_n144_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n68_), .O(z15));
  nand4  g103(.a(new_n73_), .b(new_n45_), .c(x25), .d(x23), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n85_), .c(x19), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x24), .O(new_n150_));
  nand4  g106(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n151_));
  nor3   g107(.a(new_n151_), .b(new_n121_), .c(x18), .O(new_n152_));
  nor2   g108(.a(new_n152_), .b(new_n45_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n154_));
  nand2  g110(.a(x08), .b(x07), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(new_n154_), .c(new_n150_), .O(z16));
endmodule


