// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_;
  inv1   g00(.a(x26), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand4  g03(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  inv1   g10(.a(x23), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x08), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  oai21  g15(.a(new_n46_), .b(new_n59_), .c(new_n56_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n56_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n57_), .b(new_n63_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  nor2   g21(.a(new_n57_), .b(new_n65_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  nor2   g23(.a(new_n57_), .b(new_n67_), .O(z06));
  inv1   g24(.a(x06), .O(new_n69_));
  nor2   g25(.a(new_n57_), .b(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n56_), .O(z08));
  nand3  g28(.a(x21), .b(x20), .c(x11), .O(new_n73_));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x09), .c(new_n46_), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(x23), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n47_), .O(new_n78_));
  oai21  g34(.a(new_n46_), .b(new_n54_), .c(new_n78_), .O(z09));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  nand3  g36(.a(x26), .b(x25), .c(x24), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n80_), .c(x20), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(x19), .O(new_n83_));
  inv1   g39(.a(x19), .O(new_n84_));
  nand2  g40(.a(x20), .b(new_n84_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(new_n55_), .O(new_n86_));
  nand4  g42(.a(new_n86_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n87_));
  oai21  g43(.a(new_n57_), .b(new_n59_), .c(new_n87_), .O(z10));
  inv1   g44(.a(x09), .O(new_n89_));
  nand2  g45(.a(x22), .b(x13), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n81_), .c(x21), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(x20), .c(x19), .O(new_n92_));
  nand2  g48(.a(x20), .b(x19), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(x21), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(new_n89_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n46_), .c(new_n55_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(x10), .c(new_n61_), .O(z11));
  nand4  g53(.a(x26), .b(x25), .c(x24), .d(x14), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(x21), .c(x20), .d(x19), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(x22), .O(new_n100_));
  inv1   g56(.a(x22), .O(new_n101_));
  inv1   g57(.a(new_n93_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n101_), .c(x21), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n100_), .c(new_n89_), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n46_), .c(new_n55_), .O(new_n105_));
  nand2  g61(.a(x08), .b(x03), .O(new_n106_));
  oai21  g62(.a(new_n105_), .b(x10), .c(new_n106_), .O(z12));
  nand3  g63(.a(x21), .b(x20), .c(x19), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nand4  g65(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(x22), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(x09), .c(new_n46_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n47_), .O(new_n114_));
  oai21  g70(.a(new_n46_), .b(new_n65_), .c(new_n114_), .O(z13));
  nand3  g71(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(x22), .c(x21), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n93_), .c(x24), .O(new_n118_));
  inv1   g74(.a(x24), .O(new_n119_));
  nand4  g75(.a(new_n102_), .b(new_n119_), .c(x22), .d(x21), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n118_), .c(new_n89_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n46_), .c(new_n55_), .O(new_n122_));
  nand2  g78(.a(x08), .b(x05), .O(new_n123_));
  oai21  g79(.a(new_n122_), .b(x10), .c(new_n123_), .O(z14));
  nand2  g80(.a(x26), .b(x17), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(x24), .c(x22), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n108_), .c(x25), .O(new_n127_));
  nor2   g83(.a(x25), .b(new_n119_), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n109_), .c(x22), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n127_), .c(new_n55_), .O(new_n130_));
  nand4  g86(.a(new_n130_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n131_));
  oai21  g87(.a(new_n57_), .b(new_n69_), .c(new_n131_), .O(z15));
  inv1   g88(.a(x18), .O(new_n133_));
  nand3  g89(.a(x20), .b(x19), .c(new_n133_), .O(new_n134_));
  nand4  g90(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n135_));
  oai21  g91(.a(new_n135_), .b(new_n134_), .c(x26), .O(new_n136_));
  nor2   g92(.a(new_n119_), .b(new_n101_), .O(new_n137_));
  nand4  g93(.a(new_n109_), .b(new_n137_), .c(new_n45_), .d(x25), .O(new_n138_));
  aoi21  g94(.a(new_n138_), .b(new_n136_), .c(new_n89_), .O(new_n139_));
  aoi21  g95(.a(new_n139_), .b(new_n46_), .c(new_n55_), .O(new_n140_));
  oai21  g96(.a(new_n140_), .b(x10), .c(new_n71_), .O(z16));
endmodule


