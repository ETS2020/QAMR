// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x25), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(x23), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor3   g015(.a(new_n55_), .b(new_n46_), .c(new_n59_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n56_), .O(z03));
  nand2  g018(.a(x08), .b(x03), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n56_), .O(z04));
  nor2   g020(.a(new_n55_), .b(new_n46_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(x04), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n56_), .O(z06));
  and2   g025(.a(new_n65_), .b(x06), .O(z07));
  and2   g026(.a(new_n65_), .b(x07), .O(z08));
  and2   g027(.a(x21), .b(x20), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(x11), .O(new_n73_));
  nor2   g029(.a(new_n45_), .b(new_n54_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(x24), .c(x22), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n73_), .c(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n57_), .c(new_n56_), .O(z09));
  xor2a  g034(.a(x20), .b(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n80_));
  oai21  g036(.a(new_n46_), .b(new_n59_), .c(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n56_), .O(new_n82_));
  inv1   g038(.a(x09), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(x08), .O(new_n84_));
  nand2  g040(.a(x19), .b(x12), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(x10), .O(new_n86_));
  inv1   g042(.a(x21), .O(new_n87_));
  inv1   g043(.a(x22), .O(new_n88_));
  inv1   g044(.a(x23), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand3  g047(.a(x26), .b(x25), .c(x24), .O(new_n92_));
  nor3   g048(.a(new_n92_), .b(new_n91_), .c(new_n87_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n86_), .c(new_n84_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n82_), .O(z10));
  nand2  g051(.a(x22), .b(x13), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n92_), .c(x21), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(x20), .c(x19), .O(new_n98_));
  nand2  g054(.a(x20), .b(x19), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n87_), .c(new_n98_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n61_), .c(new_n56_), .O(z11));
  nand3  g059(.a(x21), .b(x20), .c(x19), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n88_), .b(x21), .O(new_n106_));
  oai22  g062(.a(new_n106_), .b(new_n99_), .c(new_n105_), .d(new_n88_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n63_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n56_), .O(new_n110_));
  nor2   g066(.a(new_n92_), .b(new_n91_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n84_), .c(x14), .d(new_n47_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n110_), .O(z12));
  nand3  g069(.a(new_n74_), .b(x24), .c(x15), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n105_), .c(x22), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x23), .O(new_n116_));
  nand4  g072(.a(new_n105_), .b(new_n54_), .c(new_n89_), .d(x22), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n66_), .O(z13));
  nand2  g076(.a(new_n65_), .b(x05), .O(new_n121_));
  nand3  g077(.a(x26), .b(x25), .c(x16), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n100_), .c(x22), .d(x21), .O(new_n123_));
  inv1   g079(.a(x24), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x22), .c(x21), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n99_), .O(new_n126_));
  aoi21  g082(.a(new_n123_), .b(x24), .c(new_n126_), .O(new_n127_));
  nand3  g083(.a(new_n54_), .b(x24), .c(new_n89_), .O(new_n128_));
  oai21  g084(.a(new_n127_), .b(new_n89_), .c(new_n128_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n121_), .O(z14));
  nand2  g087(.a(x26), .b(x17), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x24), .c(x22), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n104_), .c(x25), .O(new_n134_));
  nand4  g090(.a(new_n105_), .b(new_n90_), .c(new_n54_), .d(x24), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n137_));
  aoi21  g093(.a(x08), .b(x06), .c(new_n55_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n137_), .O(z15));
  nand3  g095(.a(new_n84_), .b(x19), .c(new_n47_), .O(new_n140_));
  nand4  g096(.a(new_n72_), .b(new_n45_), .c(x24), .d(x22), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n140_), .c(x23), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x25), .O(new_n143_));
  nand4  g099(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n144_));
  nor3   g100(.a(new_n144_), .b(new_n99_), .c(x18), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n45_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n147_));
  nand2  g103(.a(x08), .b(x07), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n147_), .c(new_n143_), .O(z16));
endmodule


