// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x10), .O(new_n46_));
  inv1   g002(.a(x09), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x08), .O(new_n48_));
  nand3  g004(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x20), .O(new_n51_));
  inv1   g007(.a(x21), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g009(.a(x24), .O(new_n54_));
  inv1   g010(.a(x26), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g012(.a(new_n56_), .b(new_n53_), .c(new_n50_), .d(x22), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(x25), .c(new_n45_), .O(z00));
  inv1   g014(.a(x25), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x23), .O(new_n60_));
  nand2  g016(.a(x08), .b(x00), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(z01));
  inv1   g018(.a(x01), .O(new_n63_));
  nand2  g019(.a(new_n60_), .b(x08), .O(new_n64_));
  nor2   g020(.a(new_n64_), .b(new_n63_), .O(z02));
  nand2  g021(.a(x08), .b(x02), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n60_), .O(z03));
  nand2  g023(.a(x08), .b(x03), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n60_), .O(z04));
  nand2  g025(.a(x08), .b(x04), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n60_), .O(z05));
  inv1   g027(.a(x05), .O(new_n72_));
  nor2   g028(.a(new_n64_), .b(new_n72_), .O(z06));
  nand2  g029(.a(x08), .b(x06), .O(new_n74_));
  and2   g030(.a(new_n74_), .b(new_n60_), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(z07));
  nand2  g032(.a(x08), .b(x07), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n60_), .O(z08));
  inv1   g034(.a(x08), .O(new_n79_));
  nand2  g035(.a(new_n53_), .b(x11), .O(new_n80_));
  inv1   g036(.a(x22), .O(new_n81_));
  nor2   g037(.a(new_n45_), .b(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n56_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n80_), .c(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n46_), .c(x09), .d(new_n79_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n61_), .c(new_n60_), .O(z09));
  xor2a  g042(.a(x20), .b(x19), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n46_), .c(x09), .d(new_n79_), .O(new_n88_));
  oai21  g044(.a(new_n79_), .b(new_n63_), .c(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  nand2  g046(.a(x19), .b(x12), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(x10), .O(new_n92_));
  inv1   g048(.a(new_n82_), .O(new_n93_));
  nand3  g049(.a(x26), .b(x25), .c(x24), .O(new_n94_));
  nor3   g050(.a(new_n94_), .b(new_n93_), .c(new_n52_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n92_), .c(new_n48_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n90_), .O(z10));
  inv1   g053(.a(x19), .O(new_n98_));
  xnor2a g054(.a(x21), .b(x20), .O(new_n99_));
  nand2  g055(.a(x21), .b(new_n98_), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n46_), .c(x09), .d(new_n79_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n66_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n60_), .O(new_n104_));
  nand2  g060(.a(x19), .b(x13), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(x10), .O(new_n106_));
  nor3   g062(.a(new_n94_), .b(new_n93_), .c(new_n51_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n106_), .c(new_n48_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n104_), .O(z11));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x22), .O(new_n111_));
  nand2  g067(.a(x20), .b(x19), .O(new_n112_));
  nand2  g068(.a(new_n81_), .b(x21), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n46_), .c(x09), .d(new_n79_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n68_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n60_), .O(new_n117_));
  nor2   g073(.a(new_n94_), .b(new_n93_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n48_), .c(x14), .d(new_n46_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n117_), .O(z12));
  nand3  g076(.a(x26), .b(x24), .c(x15), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x22), .c(x21), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n112_), .c(x23), .O(new_n123_));
  nand3  g079(.a(new_n45_), .b(x22), .c(x21), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n112_), .c(new_n123_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n46_), .c(x09), .d(new_n79_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n70_), .c(new_n60_), .O(z13));
  nand3  g083(.a(new_n53_), .b(new_n54_), .c(x22), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n49_), .c(x25), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x23), .O(new_n130_));
  inv1   g086(.a(new_n110_), .O(new_n131_));
  nand2  g087(.a(x26), .b(x16), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x23), .c(x22), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n131_), .c(new_n54_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n46_), .c(x09), .d(new_n79_), .O(new_n136_));
  nand2  g092(.a(x08), .b(x05), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n136_), .c(new_n130_), .O(z14));
  aoi21  g094(.a(x26), .b(x17), .c(new_n54_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n131_), .c(new_n82_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(x25), .c(new_n46_), .d(x09), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(x08), .c(new_n75_), .O(z15));
  nand4  g098(.a(new_n53_), .b(new_n55_), .c(x24), .d(x22), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n49_), .c(x25), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x23), .O(new_n145_));
  nand4  g101(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n146_));
  nor3   g102(.a(new_n146_), .b(new_n112_), .c(x18), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n55_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n46_), .c(x09), .d(new_n79_), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n145_), .c(new_n77_), .O(z16));
endmodule


