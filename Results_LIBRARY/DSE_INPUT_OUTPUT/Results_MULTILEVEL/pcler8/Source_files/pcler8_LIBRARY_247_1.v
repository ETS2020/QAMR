// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n54_, new_n56_,
    new_n58_, new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x10), .O(new_n46_));
  nand2  g01(.a(x19), .b(new_n46_), .O(new_n47_));
  nand2  g02(.a(x08), .b(x00), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(new_n47_), .O(z01));
  nand2  g04(.a(x08), .b(x01), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nand3  g06(.a(new_n47_), .b(x08), .c(x02), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(z03));
  nand2  g08(.a(x08), .b(x03), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n47_), .O(z04));
  nand3  g10(.a(new_n47_), .b(x08), .c(x04), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(z05));
  nand3  g12(.a(new_n47_), .b(x08), .c(x05), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(z06));
  nand2  g14(.a(x08), .b(x06), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n47_), .O(z07));
  nand3  g16(.a(new_n47_), .b(x08), .c(x07), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(z08));
  nand3  g18(.a(new_n47_), .b(x08), .c(x00), .O(new_n64_));
  inv1   g19(.a(x19), .O(new_n65_));
  inv1   g20(.a(x09), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(x08), .O(new_n67_));
  nand3  g22(.a(new_n67_), .b(new_n65_), .c(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n64_), .O(z09));
  nand2  g24(.a(x20), .b(x09), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x08), .c(new_n65_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z10));
  inv1   g28(.a(x21), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(x19), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n67_), .c(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n52_), .O(z11));
  nand3  g32(.a(new_n47_), .b(x08), .c(x03), .O(new_n78_));
  inv1   g33(.a(x22), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(x19), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n67_), .c(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n78_), .O(z12));
  inv1   g37(.a(x23), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x19), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n67_), .c(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n56_), .O(z13));
  inv1   g41(.a(x24), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x19), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n67_), .c(new_n46_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n58_), .O(z14));
  nand3  g45(.a(new_n47_), .b(x08), .c(x06), .O(new_n91_));
  inv1   g46(.a(x25), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(x19), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n67_), .c(new_n46_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n91_), .O(z15));
  inv1   g50(.a(x26), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(x19), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n67_), .c(new_n46_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n62_), .O(z16));
  zero   g54(.O(z00));
endmodule


