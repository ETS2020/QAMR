// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x23), .b(x18), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nand2  g011(.a(x08), .b(x00), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n55_), .O(z01));
  inv1   g013(.a(x01), .O(new_n58_));
  nor3   g014(.a(new_n54_), .b(new_n46_), .c(new_n58_), .O(z02));
  nand2  g015(.a(x08), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n55_), .O(z03));
  nand2  g017(.a(x08), .b(x03), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(z04));
  nand2  g019(.a(x08), .b(x04), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n55_), .O(z05));
  nor2   g021(.a(new_n54_), .b(new_n46_), .O(new_n66_));
  and2   g022(.a(new_n66_), .b(x05), .O(z06));
  nand2  g023(.a(new_n66_), .b(x06), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z07));
  nand2  g025(.a(x08), .b(x07), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n55_), .O(z08));
  nand4  g027(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n72_));
  inv1   g028(.a(x25), .O(new_n73_));
  nor2   g029(.a(new_n45_), .b(new_n73_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(x24), .c(x23), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n56_), .c(new_n55_), .O(z09));
  xor2a  g034(.a(x20), .b(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n80_));
  oai21  g036(.a(new_n46_), .b(new_n58_), .c(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n55_), .O(new_n82_));
  inv1   g038(.a(x09), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(x08), .O(new_n84_));
  nand2  g040(.a(x19), .b(x12), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(x10), .O(new_n86_));
  nand3  g042(.a(x23), .b(x22), .c(x21), .O(new_n87_));
  nand3  g043(.a(x26), .b(x25), .c(x24), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n84_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n82_), .O(z10));
  nand3  g047(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n88_), .c(x21), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(x20), .c(x19), .O(new_n94_));
  nand2  g050(.a(x20), .b(x19), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(x21), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n60_), .c(new_n55_), .O(z11));
  nand2  g055(.a(x23), .b(x14), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n88_), .c(x21), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n95_), .c(x22), .O(new_n102_));
  inv1   g058(.a(x22), .O(new_n103_));
  inv1   g059(.a(new_n95_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(x21), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n62_), .c(new_n55_), .O(z12));
  nand2  g064(.a(new_n66_), .b(x04), .O(new_n109_));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  nand3  g067(.a(new_n74_), .b(x24), .c(x15), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n111_), .c(x22), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x23), .O(new_n114_));
  nor2   g070(.a(x23), .b(new_n103_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n104_), .c(x21), .d(x18), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n109_), .O(z13));
  inv1   g075(.a(x23), .O(new_n120_));
  nand3  g076(.a(new_n84_), .b(x24), .c(new_n47_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x18), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand3  g079(.a(x26), .b(x25), .c(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x22), .c(x21), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n95_), .c(x24), .O(new_n126_));
  nor2   g082(.a(x24), .b(new_n120_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n111_), .c(x22), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n130_));
  nand2  g086(.a(x08), .b(x05), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n130_), .c(new_n123_), .O(z14));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n111_), .c(x24), .d(x22), .O(new_n134_));
  nand3  g090(.a(new_n73_), .b(x24), .c(x22), .O(new_n135_));
  nor2   g091(.a(new_n135_), .b(new_n110_), .O(new_n136_));
  aoi21  g092(.a(new_n134_), .b(x25), .c(new_n136_), .O(new_n137_));
  nand3  g093(.a(x25), .b(new_n120_), .c(x18), .O(new_n138_));
  oai21  g094(.a(new_n137_), .b(new_n120_), .c(new_n138_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n68_), .O(z15));
  nand2  g097(.a(new_n66_), .b(x07), .O(new_n142_));
  nand3  g098(.a(x25), .b(x24), .c(x22), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n110_), .c(x26), .O(new_n144_));
  nand4  g100(.a(new_n45_), .b(x25), .c(x24), .d(x22), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n110_), .c(new_n144_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x23), .O(new_n147_));
  nand2  g103(.a(x26), .b(x18), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n142_), .O(z16));
endmodule


