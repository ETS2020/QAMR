// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x14), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  inv1   g011(.a(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(x14), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(x08), .O(new_n59_));
  nor2   g015(.a(new_n59_), .b(new_n55_), .O(z01));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n59_), .b(new_n61_), .O(z02));
  nand2  g018(.a(x08), .b(x02), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(z03));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n58_), .O(z04));
  inv1   g022(.a(x04), .O(new_n67_));
  nor2   g023(.a(new_n59_), .b(new_n67_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n58_), .O(z06));
  inv1   g026(.a(x06), .O(new_n71_));
  nor2   g027(.a(new_n59_), .b(new_n71_), .O(z07));
  nand2  g028(.a(x08), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n58_), .O(z08));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n46_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  nand4  g033(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n80_));
  aoi21  g036(.a(x08), .b(x00), .c(new_n57_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(z09));
  inv1   g038(.a(x20), .O(new_n83_));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n78_), .c(x20), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(x19), .O(new_n86_));
  oai21  g042(.a(new_n83_), .b(x19), .c(new_n86_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n88_));
  aoi21  g044(.a(x08), .b(x01), .c(new_n57_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(z10));
  nand2  g046(.a(x20), .b(x19), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n75_), .b(x20), .c(x19), .O(new_n93_));
  oai21  g049(.a(new_n92_), .b(new_n75_), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n63_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n58_), .O(new_n97_));
  inv1   g053(.a(x09), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(x08), .O(new_n99_));
  nand2  g055(.a(x14), .b(x13), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(x10), .O(new_n101_));
  inv1   g057(.a(x23), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n46_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  inv1   g060(.a(x25), .O(new_n105_));
  nor2   g061(.a(new_n45_), .b(new_n105_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x24), .O(new_n107_));
  nor3   g063(.a(new_n107_), .b(new_n104_), .c(new_n75_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n101_), .c(new_n99_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n97_), .O(z11));
  nand3  g066(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand2  g068(.a(new_n46_), .b(x21), .O(new_n113_));
  oai22  g069(.a(new_n113_), .b(new_n91_), .c(new_n112_), .d(new_n46_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n65_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n58_), .O(new_n117_));
  nor2   g073(.a(new_n107_), .b(new_n104_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n99_), .c(x14), .d(new_n48_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n117_), .O(z12));
  nand3  g076(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n121_));
  nand3  g077(.a(new_n106_), .b(x23), .c(x15), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(x14), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x24), .O(new_n124_));
  nand2  g080(.a(new_n92_), .b(new_n76_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x23), .O(new_n126_));
  nand3  g082(.a(new_n102_), .b(x22), .c(x21), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n91_), .c(new_n126_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n129_));
  nand2  g085(.a(x08), .b(x04), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(new_n124_), .O(z13));
  nand3  g087(.a(x26), .b(x25), .c(x16), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x23), .c(x22), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n111_), .c(x24), .O(new_n134_));
  nand4  g090(.a(new_n112_), .b(new_n56_), .c(x23), .d(x22), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n69_), .c(new_n58_), .O(z14));
  nand3  g094(.a(new_n99_), .b(x19), .c(new_n48_), .O(new_n139_));
  nor2   g095(.a(new_n75_), .b(new_n83_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(new_n105_), .c(x23), .d(x22), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n139_), .c(x14), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x24), .O(new_n143_));
  nand2  g099(.a(x26), .b(x17), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n103_), .c(x24), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n112_), .c(new_n105_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n148_));
  nand2  g104(.a(x08), .b(x06), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n148_), .c(new_n143_), .O(z15));
  nand3  g106(.a(new_n92_), .b(new_n99_), .c(new_n48_), .O(new_n151_));
  nand4  g107(.a(new_n76_), .b(new_n45_), .c(x25), .d(x23), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n151_), .c(x14), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x24), .O(new_n154_));
  inv1   g110(.a(x18), .O(new_n155_));
  nand3  g111(.a(new_n140_), .b(x19), .c(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n103_), .b(x25), .c(x24), .O(new_n157_));
  nor2   g113(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nor2   g114(.a(new_n158_), .b(new_n45_), .O(new_n159_));
  nand4  g115(.a(new_n159_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n154_), .c(new_n73_), .O(z16));
endmodule


