// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x20), .O(new_n50_));
  inv1   g006(.a(x21), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g008(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(x22), .c(new_n45_), .O(z00));
  inv1   g012(.a(x00), .O(new_n57_));
  inv1   g013(.a(x22), .O(new_n58_));
  nand2  g014(.a(x23), .b(new_n58_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x08), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n57_), .O(z01));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  and2   g018(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z02));
  inv1   g020(.a(x02), .O(new_n65_));
  nor2   g021(.a(new_n60_), .b(new_n65_), .O(z03));
  nand2  g022(.a(x08), .b(x03), .O(new_n67_));
  and2   g023(.a(new_n67_), .b(new_n59_), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z04));
  inv1   g025(.a(x04), .O(new_n70_));
  nor2   g026(.a(new_n60_), .b(new_n70_), .O(z05));
  nand2  g027(.a(x08), .b(x05), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n59_), .O(z06));
  nand2  g029(.a(x08), .b(x06), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n59_), .O(z07));
  inv1   g031(.a(x07), .O(new_n76_));
  nor2   g032(.a(new_n60_), .b(new_n76_), .O(z08));
  nand2  g033(.a(new_n52_), .b(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n81_));
  inv1   g037(.a(new_n59_), .O(new_n82_));
  aoi21  g038(.a(x08), .b(x00), .c(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n81_), .O(z09));
  nand3  g040(.a(x23), .b(x21), .c(x12), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n53_), .c(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  oai21  g043(.a(new_n50_), .b(x19), .c(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n63_), .O(z10));
  inv1   g046(.a(x19), .O(new_n91_));
  nand2  g047(.a(x23), .b(x13), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n53_), .c(x21), .O(new_n93_));
  nor2   g049(.a(new_n51_), .b(x20), .O(new_n94_));
  aoi21  g050(.a(new_n93_), .b(x20), .c(new_n94_), .O(new_n95_));
  nand2  g051(.a(x21), .b(new_n91_), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n91_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n98_));
  aoi21  g054(.a(x08), .b(x02), .c(new_n82_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(z11));
  nand3  g056(.a(x21), .b(x20), .c(x19), .O(new_n101_));
  oai21  g057(.a(x23), .b(x22), .c(new_n101_), .O(new_n102_));
  nor2   g058(.a(new_n50_), .b(new_n91_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n58_), .c(x21), .O(new_n104_));
  nand3  g060(.a(new_n54_), .b(x23), .c(x14), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n68_), .O(z12));
  nand4  g064(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n103_), .c(x21), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x23), .O(new_n111_));
  nand4  g067(.a(new_n103_), .b(new_n45_), .c(x22), .d(x21), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n114_));
  aoi21  g070(.a(x08), .b(x04), .c(new_n82_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(z13));
  nand3  g072(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n117_));
  inv1   g073(.a(x24), .O(new_n118_));
  nand3  g074(.a(new_n103_), .b(new_n118_), .c(x21), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n117_), .c(x22), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x23), .O(new_n121_));
  nand3  g077(.a(x26), .b(x25), .c(x16), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x23), .c(x21), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n103_), .c(new_n118_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n121_), .c(new_n72_), .O(z14));
  inv1   g083(.a(x25), .O(new_n128_));
  nand3  g084(.a(new_n52_), .b(new_n128_), .c(x24), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n48_), .c(x22), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x23), .O(new_n131_));
  nand2  g087(.a(x26), .b(x17), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x24), .c(x23), .O(new_n133_));
  nor2   g089(.a(new_n133_), .b(new_n101_), .O(new_n134_));
  nor2   g090(.a(new_n134_), .b(new_n128_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n131_), .c(new_n74_), .O(z15));
  inv1   g093(.a(x26), .O(new_n138_));
  nand4  g094(.a(new_n52_), .b(new_n138_), .c(x25), .d(x24), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n48_), .c(x22), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x23), .O(new_n141_));
  nor3   g097(.a(new_n50_), .b(new_n91_), .c(x18), .O(new_n142_));
  nand4  g098(.a(x25), .b(x24), .c(x23), .d(x21), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n142_), .c(new_n138_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n146_));
  nand2  g102(.a(x08), .b(x07), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n146_), .c(new_n141_), .O(z16));
endmodule


