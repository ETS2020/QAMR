// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x16), .O(new_n45_));
  nor2   g001(.a(x23), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x09), .O(new_n49_));
  nor2   g005(.a(new_n49_), .b(x08), .O(new_n50_));
  nand2  g006(.a(x20), .b(x19), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  inv1   g008(.a(x22), .O(new_n53_));
  inv1   g009(.a(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(x21), .O(new_n56_));
  nand3  g012(.a(x26), .b(x25), .c(x24), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand4  g014(.a(new_n58_), .b(new_n52_), .c(new_n50_), .d(new_n48_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n47_), .O(z00));
  inv1   g016(.a(x00), .O(new_n61_));
  inv1   g017(.a(x08), .O(new_n62_));
  nor3   g018(.a(new_n46_), .b(new_n62_), .c(new_n61_), .O(z01));
  nand2  g019(.a(x08), .b(x01), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n47_), .O(z02));
  nor2   g021(.a(new_n46_), .b(new_n62_), .O(new_n66_));
  and2   g022(.a(new_n66_), .b(x02), .O(z03));
  nand2  g023(.a(x08), .b(x03), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n47_), .O(z04));
  nand2  g025(.a(x08), .b(x04), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n47_), .O(z05));
  nand2  g027(.a(x08), .b(x05), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n47_), .O(z06));
  and2   g029(.a(new_n66_), .b(x06), .O(z07));
  and2   g030(.a(new_n66_), .b(x07), .O(z08));
  inv1   g031(.a(x19), .O(new_n76_));
  nand3  g032(.a(new_n50_), .b(new_n76_), .c(new_n48_), .O(new_n77_));
  oai21  g033(.a(new_n62_), .b(new_n61_), .c(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n47_), .O(new_n79_));
  nand2  g035(.a(x20), .b(x11), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x10), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n58_), .c(new_n50_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n79_), .O(z09));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  nand4  g040(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  nand2  g043(.a(x20), .b(new_n76_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n64_), .c(new_n47_), .O(z10));
  nand3  g047(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n57_), .c(x21), .O(new_n93_));
  inv1   g049(.a(x21), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(x20), .O(new_n95_));
  aoi21  g051(.a(new_n93_), .b(x20), .c(new_n95_), .O(new_n96_));
  nand2  g052(.a(x21), .b(new_n76_), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n76_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n99_));
  aoi21  g055(.a(x08), .b(x02), .c(new_n46_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n99_), .O(z11));
  nand2  g057(.a(x23), .b(x14), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n57_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n51_), .c(x22), .O(new_n104_));
  nand3  g060(.a(new_n52_), .b(new_n53_), .c(x21), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n68_), .c(new_n47_), .O(z12));
  nand2  g064(.a(new_n66_), .b(x04), .O(new_n109_));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  nand4  g067(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n111_), .c(x22), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x23), .O(new_n114_));
  nor2   g070(.a(x23), .b(new_n53_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n52_), .c(x21), .d(new_n45_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n109_), .O(z13));
  nand2  g075(.a(new_n66_), .b(x05), .O(new_n120_));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n52_), .c(x22), .d(x21), .O(new_n122_));
  inv1   g078(.a(x24), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x22), .c(x21), .O(new_n124_));
  nor2   g080(.a(new_n124_), .b(new_n51_), .O(new_n125_));
  aoi21  g081(.a(new_n122_), .b(x24), .c(new_n125_), .O(new_n126_));
  nand3  g082(.a(x24), .b(new_n54_), .c(new_n45_), .O(new_n127_));
  oai21  g083(.a(new_n126_), .b(new_n54_), .c(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n120_), .O(z14));
  nand3  g086(.a(new_n50_), .b(x25), .c(new_n48_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n45_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n54_), .O(new_n133_));
  nand2  g089(.a(x26), .b(x17), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x24), .c(x22), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n110_), .c(x25), .O(new_n136_));
  inv1   g092(.a(x25), .O(new_n137_));
  nand4  g093(.a(new_n111_), .b(new_n55_), .c(new_n137_), .d(x24), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n140_));
  nand2  g096(.a(x08), .b(x06), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n140_), .c(new_n133_), .O(z15));
  nand3  g098(.a(new_n50_), .b(x26), .c(new_n48_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n45_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n54_), .O(new_n145_));
  inv1   g101(.a(x18), .O(new_n146_));
  nand2  g102(.a(new_n52_), .b(new_n146_), .O(new_n147_));
  nor2   g103(.a(new_n53_), .b(new_n94_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x25), .c(x24), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n147_), .c(x26), .O(new_n150_));
  nor2   g106(.a(new_n123_), .b(new_n54_), .O(new_n151_));
  nor2   g107(.a(x26), .b(new_n137_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n151_), .c(new_n148_), .d(new_n52_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n48_), .c(x09), .d(new_n62_), .O(new_n155_));
  nand2  g111(.a(x08), .b(x07), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n155_), .c(new_n145_), .O(z16));
endmodule


