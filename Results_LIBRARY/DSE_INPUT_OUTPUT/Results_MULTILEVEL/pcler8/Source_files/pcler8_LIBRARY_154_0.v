// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n61_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x24), .b(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  and2   g011(.a(new_n55_), .b(x00), .O(z01));
  inv1   g012(.a(new_n54_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z02));
  and2   g015(.a(new_n55_), .b(x02), .O(z03));
  inv1   g016(.a(x03), .O(new_n61_));
  inv1   g017(.a(new_n55_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n61_), .O(z04));
  nand2  g019(.a(x08), .b(x04), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n57_), .O(z05));
  inv1   g021(.a(x05), .O(new_n66_));
  nor2   g022(.a(new_n62_), .b(new_n66_), .O(z06));
  nand2  g023(.a(x08), .b(x06), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n57_), .O(z07));
  and2   g025(.a(new_n55_), .b(x07), .O(z08));
  nand4  g026(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n71_));
  nand4  g027(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n71_), .c(x19), .O(new_n73_));
  nand4  g029(.a(new_n73_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n74_));
  aoi21  g030(.a(x08), .b(x00), .c(new_n54_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(z09));
  inv1   g032(.a(x20), .O(new_n77_));
  nand3  g033(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n72_), .c(x20), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(x19), .O(new_n80_));
  oai21  g036(.a(new_n77_), .b(x19), .c(new_n80_), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n58_), .c(new_n57_), .O(z10));
  inv1   g039(.a(x21), .O(new_n84_));
  and2   g040(.a(x20), .b(x19), .O(new_n85_));
  nand3  g041(.a(new_n84_), .b(x20), .c(x19), .O(new_n86_));
  oai21  g042(.a(new_n85_), .b(new_n84_), .c(new_n86_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n88_));
  nand2  g044(.a(x08), .b(x02), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n57_), .O(new_n91_));
  inv1   g047(.a(x09), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x08), .O(new_n93_));
  nand2  g049(.a(x21), .b(x13), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(x10), .O(new_n95_));
  inv1   g051(.a(x22), .O(new_n96_));
  inv1   g052(.a(x23), .O(new_n97_));
  nand3  g053(.a(x26), .b(x25), .c(x24), .O(new_n98_));
  nor3   g054(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n95_), .c(new_n93_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n91_), .O(z11));
  nand3  g057(.a(x21), .b(x20), .c(x19), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(x22), .O(new_n103_));
  nand4  g059(.a(new_n96_), .b(x21), .c(x20), .d(x19), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n106_));
  oai21  g062(.a(new_n46_), .b(new_n61_), .c(new_n106_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n57_), .O(new_n108_));
  nand4  g064(.a(new_n99_), .b(new_n93_), .c(x14), .d(new_n47_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(z12));
  nand3  g066(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n111_));
  nand4  g067(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n111_), .c(x24), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n97_), .O(new_n114_));
  inv1   g070(.a(new_n102_), .O(new_n115_));
  nand4  g071(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n115_), .c(x22), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x23), .c(new_n47_), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x09), .c(new_n46_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n114_), .c(new_n64_), .O(z13));
  nand3  g077(.a(x26), .b(x25), .c(x16), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x23), .c(x22), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n102_), .c(x24), .O(new_n124_));
  inv1   g080(.a(x24), .O(new_n125_));
  nand4  g081(.a(new_n115_), .b(new_n125_), .c(x23), .d(x22), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n128_));
  oai21  g084(.a(new_n62_), .b(new_n66_), .c(new_n128_), .O(z14));
  nand2  g085(.a(new_n55_), .b(x06), .O(new_n130_));
  nand2  g086(.a(x25), .b(new_n97_), .O(new_n131_));
  inv1   g087(.a(x25), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x23), .c(x22), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n102_), .c(new_n131_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x24), .O(new_n135_));
  nand2  g091(.a(x26), .b(x17), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n115_), .c(x24), .d(x22), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x25), .c(x23), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n130_), .O(z15));
  nand3  g097(.a(new_n93_), .b(x26), .c(new_n47_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x24), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n97_), .O(new_n144_));
  inv1   g100(.a(x18), .O(new_n145_));
  nand2  g101(.a(new_n85_), .b(new_n145_), .O(new_n146_));
  nand4  g102(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n146_), .c(x26), .O(new_n148_));
  nor2   g104(.a(new_n125_), .b(new_n97_), .O(new_n149_));
  inv1   g105(.a(new_n112_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n149_), .c(new_n45_), .d(x25), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n153_));
  nand2  g109(.a(x08), .b(x07), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(new_n153_), .c(new_n144_), .O(z16));
endmodule


