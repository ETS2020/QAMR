// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x18), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x18), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x00), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z01));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n56_), .b(new_n47_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n60_), .O(z02));
  nand2  g019(.a(x08), .b(x02), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n57_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n57_), .O(z04));
  inv1   g023(.a(x04), .O(new_n68_));
  oai21  g024(.a(new_n47_), .b(new_n68_), .c(new_n57_), .O(z05));
  and2   g025(.a(new_n61_), .b(x05), .O(z06));
  nand2  g026(.a(new_n61_), .b(x06), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z07));
  nand2  g028(.a(x08), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n57_), .O(z08));
  nand4  g030(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  inv1   g031(.a(x25), .O(new_n76_));
  nor2   g032(.a(new_n45_), .b(new_n76_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x24), .c(x23), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n75_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n58_), .c(new_n57_), .O(z09));
  xor2a  g037(.a(x20), .b(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n83_));
  oai21  g039(.a(new_n47_), .b(new_n60_), .c(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n57_), .O(new_n85_));
  inv1   g041(.a(x09), .O(new_n86_));
  nor2   g042(.a(x10), .b(new_n86_), .O(new_n87_));
  and2   g043(.a(x19), .b(x18), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x12), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  and2   g046(.a(x23), .b(x22), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(x21), .O(new_n92_));
  nand3  g048(.a(x26), .b(x25), .c(x24), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n90_), .c(new_n87_), .d(new_n47_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n85_), .O(z10));
  inv1   g052(.a(x21), .O(new_n97_));
  nand3  g053(.a(x23), .b(x22), .c(x13), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n93_), .c(x21), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(x20), .c(x19), .O(new_n100_));
  and2   g056(.a(x20), .b(x19), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n97_), .c(new_n100_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n64_), .c(new_n57_), .O(z11));
  nor2   g060(.a(new_n86_), .b(x08), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x14), .c(new_n48_), .O(new_n106_));
  nand3  g062(.a(new_n77_), .b(x24), .c(x22), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n106_), .c(x18), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x23), .O(new_n109_));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  nand3  g067(.a(new_n101_), .b(new_n46_), .c(x21), .O(new_n112_));
  oai21  g068(.a(new_n111_), .b(new_n46_), .c(new_n112_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n109_), .c(new_n66_), .O(z12));
  nand4  g071(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n101_), .c(x22), .d(x21), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x23), .c(x18), .O(new_n118_));
  nand4  g074(.a(new_n101_), .b(new_n55_), .c(x22), .d(x21), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n121_));
  oai21  g077(.a(new_n62_), .b(new_n68_), .c(new_n121_), .O(z13));
  nand3  g078(.a(new_n105_), .b(x19), .c(new_n48_), .O(new_n123_));
  inv1   g079(.a(x24), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(x22), .c(x21), .d(x20), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n123_), .c(x18), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x23), .O(new_n127_));
  nand3  g083(.a(x26), .b(x25), .c(x16), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x22), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n111_), .c(new_n124_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n132_));
  nand2  g088(.a(x08), .b(x05), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n132_), .c(new_n127_), .O(z14));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x24), .c(x22), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n110_), .c(x25), .O(new_n137_));
  nand4  g093(.a(new_n111_), .b(new_n91_), .c(new_n76_), .d(x24), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x18), .O(new_n140_));
  nand2  g096(.a(x25), .b(new_n55_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n71_), .O(z15));
  nand3  g100(.a(new_n105_), .b(x26), .c(new_n48_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n73_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n57_), .O(new_n147_));
  nand3  g103(.a(x22), .b(x21), .c(x20), .O(new_n148_));
  nor4   g104(.a(new_n148_), .b(new_n76_), .c(new_n124_), .d(new_n55_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n88_), .c(new_n105_), .d(new_n48_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n147_), .O(z16));
endmodule


