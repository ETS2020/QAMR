// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n67_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x10), .O(new_n45_));
  nor2   g001(.a(x21), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x08), .O(new_n48_));
  inv1   g004(.a(x19), .O(new_n49_));
  inv1   g005(.a(x20), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n52_));
  inv1   g008(.a(x22), .O(new_n53_));
  inv1   g009(.a(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n55_), .c(x21), .O(new_n58_));
  oai21  g014(.a(new_n58_), .b(new_n52_), .c(new_n47_), .O(z00));
  nand2  g015(.a(x08), .b(x00), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n47_), .O(z01));
  inv1   g017(.a(x01), .O(new_n62_));
  oai21  g018(.a(new_n48_), .b(new_n62_), .c(new_n47_), .O(z02));
  nor2   g019(.a(new_n46_), .b(new_n48_), .O(new_n64_));
  and2   g020(.a(new_n64_), .b(x02), .O(z03));
  and2   g021(.a(new_n64_), .b(x03), .O(z04));
  nand2  g022(.a(new_n64_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  and2   g024(.a(new_n64_), .b(x05), .O(z06));
  inv1   g025(.a(x06), .O(new_n70_));
  oai21  g026(.a(new_n48_), .b(new_n70_), .c(new_n47_), .O(z07));
  nand2  g027(.a(x08), .b(x07), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n47_), .O(z08));
  inv1   g029(.a(x21), .O(new_n74_));
  nor2   g030(.a(new_n53_), .b(new_n74_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  nand4  g032(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n60_), .c(new_n47_), .O(z09));
  inv1   g036(.a(new_n64_), .O(new_n81_));
  nand3  g037(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n77_), .c(x20), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x19), .O(new_n84_));
  oai21  g040(.a(new_n50_), .b(x19), .c(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n86_));
  oai21  g042(.a(new_n81_), .b(new_n62_), .c(new_n86_), .O(z10));
  nand3  g043(.a(x22), .b(x20), .c(x13), .O(new_n88_));
  oai22  g044(.a(new_n88_), .b(new_n77_), .c(new_n74_), .d(x20), .O(new_n89_));
  aoi22  g045(.a(new_n89_), .b(new_n45_), .c(new_n74_), .d(x20), .O(new_n90_));
  nand3  g046(.a(x21), .b(new_n49_), .c(new_n45_), .O(new_n91_));
  oai21  g047(.a(new_n90_), .b(new_n49_), .c(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(x09), .c(new_n48_), .O(new_n93_));
  aoi21  g049(.a(x08), .b(x02), .c(new_n46_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n93_), .O(z11));
  nand2  g051(.a(x22), .b(x09), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(x08), .c(new_n45_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n74_), .O(new_n98_));
  nand2  g054(.a(x23), .b(x14), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n56_), .c(x20), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n49_), .c(x22), .O(new_n101_));
  nand3  g057(.a(new_n51_), .b(new_n53_), .c(x21), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n104_));
  nand2  g060(.a(x08), .b(x03), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(new_n98_), .O(z12));
  nand4  g062(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x23), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x23), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x22), .O(new_n113_));
  nand2  g069(.a(x23), .b(new_n53_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n67_), .O(z13));
  nand2  g073(.a(x24), .b(x09), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(x08), .c(new_n45_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n74_), .O(new_n120_));
  inv1   g076(.a(new_n51_), .O(new_n121_));
  nand3  g077(.a(x26), .b(x25), .c(x16), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x23), .c(x22), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n121_), .c(x24), .O(new_n124_));
  inv1   g080(.a(x24), .O(new_n125_));
  inv1   g081(.a(new_n110_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n125_), .c(x23), .d(x22), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n129_));
  nand2  g085(.a(x08), .b(x05), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(new_n120_), .O(z14));
  nand2  g087(.a(x26), .b(x17), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n55_), .c(x24), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n110_), .c(x25), .O(new_n134_));
  inv1   g090(.a(x25), .O(new_n135_));
  nand4  g091(.a(new_n126_), .b(new_n55_), .c(new_n135_), .d(x24), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n138_));
  oai21  g094(.a(new_n81_), .b(new_n70_), .c(new_n138_), .O(z15));
  nand2  g095(.a(x26), .b(x09), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(x08), .c(new_n45_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n74_), .O(new_n142_));
  inv1   g098(.a(x18), .O(new_n143_));
  nand2  g099(.a(new_n51_), .b(new_n143_), .O(new_n144_));
  nand3  g100(.a(new_n55_), .b(x25), .c(x24), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n144_), .c(x26), .O(new_n146_));
  nor2   g102(.a(new_n125_), .b(new_n54_), .O(new_n147_));
  nor2   g103(.a(x26), .b(new_n135_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n147_), .c(new_n75_), .d(new_n51_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n45_), .c(x09), .d(new_n48_), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n142_), .c(new_n72_), .O(z16));
endmodule


