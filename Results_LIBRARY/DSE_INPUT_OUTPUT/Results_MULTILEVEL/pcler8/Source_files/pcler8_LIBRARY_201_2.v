// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x23), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x03), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  nor2   g008(.a(new_n45_), .b(new_n52_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(x21), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g012(.a(new_n56_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n46_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  nand2  g015(.a(new_n46_), .b(x08), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n59_), .O(z01));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n46_), .O(z02));
  nand2  g019(.a(x08), .b(x02), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n46_), .O(z03));
  inv1   g021(.a(x03), .O(new_n66_));
  inv1   g022(.a(x08), .O(new_n67_));
  aoi21  g023(.a(x23), .b(new_n67_), .c(new_n66_), .O(z04));
  nand2  g024(.a(x08), .b(x04), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n46_), .O(z05));
  inv1   g026(.a(x05), .O(new_n71_));
  nor2   g027(.a(new_n60_), .b(new_n71_), .O(z06));
  inv1   g028(.a(x06), .O(new_n73_));
  nor2   g029(.a(new_n60_), .b(new_n73_), .O(z07));
  inv1   g030(.a(x07), .O(new_n75_));
  nor2   g031(.a(new_n60_), .b(new_n75_), .O(z08));
  nand4  g032(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n77_));
  inv1   g033(.a(x24), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x26), .c(x25), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n77_), .c(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n47_), .c(x09), .d(new_n67_), .O(new_n82_));
  nand2  g038(.a(x08), .b(x00), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n82_), .c(new_n46_), .O(z09));
  xor2a  g040(.a(x20), .b(x19), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n67_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n62_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n46_), .O(new_n88_));
  nand2  g044(.a(x19), .b(x12), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(x10), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n56_), .c(new_n49_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n88_), .O(z10));
  inv1   g048(.a(x19), .O(new_n93_));
  xnor2a g049(.a(x21), .b(x20), .O(new_n94_));
  nand2  g050(.a(x21), .b(new_n93_), .O(new_n95_));
  oai21  g051(.a(new_n94_), .b(new_n93_), .c(new_n95_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n47_), .c(x09), .d(new_n67_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n64_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n46_), .O(new_n99_));
  nand2  g055(.a(x19), .b(x13), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(x10), .O(new_n101_));
  nand2  g057(.a(new_n53_), .b(x20), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n55_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n101_), .c(new_n49_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n99_), .O(z11));
  inv1   g061(.a(z04), .O(new_n106_));
  nand2  g062(.a(x23), .b(x14), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n55_), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n50_), .c(x22), .O(new_n109_));
  nand3  g065(.a(new_n51_), .b(new_n52_), .c(x21), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n47_), .c(x09), .d(new_n67_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n106_), .O(z12));
  nand3  g069(.a(new_n47_), .b(x09), .c(new_n67_), .O(new_n114_));
  nand4  g070(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n114_), .c(new_n66_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n45_), .O(new_n117_));
  nand3  g073(.a(x21), .b(x20), .c(x19), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand4  g075(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n119_), .c(x22), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(new_n47_), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x09), .c(new_n67_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(new_n117_), .c(new_n69_), .O(z13));
  nand3  g081(.a(new_n49_), .b(x24), .c(new_n47_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n66_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n45_), .O(new_n128_));
  nand3  g084(.a(x26), .b(x25), .c(x16), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x22), .c(x21), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n50_), .c(x24), .O(new_n131_));
  nand4  g087(.a(new_n119_), .b(new_n78_), .c(x23), .d(x22), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n47_), .c(x09), .d(new_n67_), .O(new_n134_));
  nand2  g090(.a(x08), .b(x05), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n134_), .c(new_n128_), .O(z14));
  nand3  g092(.a(new_n49_), .b(x25), .c(new_n47_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n66_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n45_), .O(new_n139_));
  nand2  g095(.a(x26), .b(x17), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(x24), .c(x22), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n118_), .c(x25), .O(new_n142_));
  nor2   g098(.a(x25), .b(new_n78_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n119_), .c(new_n53_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n47_), .c(x09), .d(new_n67_), .O(new_n146_));
  nand2  g102(.a(x08), .b(x06), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n146_), .c(new_n139_), .O(z15));
  nand3  g104(.a(new_n49_), .b(x26), .c(new_n47_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n66_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n45_), .O(new_n151_));
  inv1   g107(.a(x18), .O(new_n152_));
  nand2  g108(.a(new_n51_), .b(new_n152_), .O(new_n153_));
  nand4  g109(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n153_), .c(x26), .O(new_n155_));
  inv1   g111(.a(x26), .O(new_n156_));
  inv1   g112(.a(new_n115_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n79_), .c(new_n156_), .d(x25), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand4  g115(.a(new_n159_), .b(new_n47_), .c(x09), .d(new_n67_), .O(new_n160_));
  nand2  g116(.a(x08), .b(x07), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(new_n160_), .c(new_n151_), .O(z16));
endmodule


