// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nor3   g010(.a(x25), .b(new_n46_), .c(new_n54_), .O(z01));
  nand2  g011(.a(x08), .b(x01), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(x25), .O(z02));
  inv1   g013(.a(x02), .O(new_n58_));
  inv1   g014(.a(x25), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(new_n46_), .O(z03));
  inv1   g016(.a(x03), .O(new_n61_));
  nor3   g017(.a(x25), .b(new_n46_), .c(new_n61_), .O(z04));
  nand2  g018(.a(x08), .b(x04), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(x25), .O(z05));
  inv1   g020(.a(x05), .O(new_n65_));
  aoi21  g021(.a(new_n59_), .b(new_n65_), .c(new_n46_), .O(z06));
  nand2  g022(.a(x08), .b(x06), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(x25), .O(z07));
  nand2  g024(.a(x08), .b(x07), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(x25), .O(z08));
  inv1   g026(.a(x21), .O(new_n71_));
  inv1   g027(.a(x22), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g029(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  inv1   g030(.a(x23), .O(new_n75_));
  inv1   g031(.a(x24), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x26), .c(x25), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n74_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n80_));
  nand2  g036(.a(new_n59_), .b(x08), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n54_), .c(new_n80_), .O(z09));
  oai21  g038(.a(x25), .b(x01), .c(x08), .O(new_n83_));
  xnor2a g039(.a(x20), .b(x19), .O(new_n84_));
  nand3  g040(.a(new_n73_), .b(x19), .c(x12), .O(new_n85_));
  oai22  g041(.a(new_n85_), .b(new_n78_), .c(new_n84_), .d(x08), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n47_), .c(x09), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n83_), .O(z10));
  inv1   g044(.a(x19), .O(new_n89_));
  nand3  g045(.a(x23), .b(x22), .c(x13), .O(new_n90_));
  nand3  g046(.a(x26), .b(x25), .c(x24), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n90_), .c(x21), .O(new_n92_));
  nor2   g048(.a(new_n71_), .b(x20), .O(new_n93_));
  aoi21  g049(.a(new_n92_), .b(x20), .c(new_n93_), .O(new_n94_));
  nand2  g050(.a(x21), .b(new_n89_), .O(new_n95_));
  oai21  g051(.a(new_n94_), .b(new_n89_), .c(new_n95_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n97_));
  oai21  g053(.a(new_n81_), .b(new_n58_), .c(new_n97_), .O(z11));
  nand2  g054(.a(x20), .b(x19), .O(new_n99_));
  nand2  g055(.a(x23), .b(x14), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n91_), .c(x21), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n99_), .c(x22), .O(new_n102_));
  inv1   g058(.a(new_n99_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n72_), .c(x21), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n106_));
  oai21  g062(.a(new_n81_), .b(new_n61_), .c(new_n106_), .O(z12));
  inv1   g063(.a(x09), .O(new_n108_));
  oai21  g064(.a(x25), .b(x04), .c(x08), .O(new_n109_));
  nand2  g065(.a(new_n75_), .b(new_n46_), .O(new_n110_));
  nand4  g066(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(x21), .c(x20), .d(x19), .O(new_n113_));
  nand3  g069(.a(x21), .b(x20), .c(x19), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x23), .c(new_n46_), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n113_), .c(new_n72_), .O(new_n116_));
  nand3  g072(.a(x23), .b(new_n72_), .c(new_n46_), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(new_n47_), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n108_), .c(new_n109_), .O(z13));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x22), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n114_), .c(x24), .O(new_n123_));
  inv1   g079(.a(new_n114_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n76_), .c(x23), .d(x22), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  oai21  g083(.a(new_n81_), .b(new_n65_), .c(new_n127_), .O(z14));
  nand4  g084(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  nand2  g086(.a(x21), .b(x20), .O(new_n131_));
  nand2  g087(.a(new_n77_), .b(x22), .O(new_n132_));
  nor2   g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  aoi22  g089(.a(new_n133_), .b(new_n130_), .c(x08), .d(x06), .O(new_n134_));
  nor2   g090(.a(new_n75_), .b(new_n72_), .O(new_n135_));
  aoi21  g091(.a(x26), .b(x17), .c(new_n76_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n124_), .c(new_n135_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(x25), .c(new_n47_), .d(x09), .O(new_n138_));
  oai22  g094(.a(new_n138_), .b(x08), .c(new_n134_), .d(x25), .O(z15));
  oai21  g095(.a(x25), .b(x07), .c(x08), .O(new_n140_));
  inv1   g096(.a(x18), .O(new_n141_));
  nand4  g097(.a(x21), .b(x20), .c(x19), .d(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n135_), .b(x25), .c(x24), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n142_), .c(x26), .O(new_n144_));
  nor2   g100(.a(x26), .b(new_n59_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n103_), .c(new_n77_), .d(new_n73_), .O(new_n146_));
  oai21  g102(.a(new_n144_), .b(x08), .c(new_n146_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n47_), .c(x09), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n140_), .O(z16));
endmodule


