// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x10), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x19), .O(new_n48_));
  inv1   g004(.a(x20), .O(new_n49_));
  nor2   g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g006(.a(x21), .O(new_n51_));
  inv1   g007(.a(x23), .O(new_n52_));
  nand3  g008(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  nor3   g009(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand4  g010(.a(new_n54_), .b(new_n50_), .c(x09), .d(new_n47_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n45_), .c(new_n46_), .O(z00));
  inv1   g012(.a(x00), .O(new_n57_));
  nor2   g013(.a(new_n46_), .b(new_n45_), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(new_n59_));
  oai21  g015(.a(new_n47_), .b(new_n57_), .c(new_n59_), .O(z01));
  nor2   g016(.a(new_n58_), .b(new_n47_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z02));
  aoi21  g019(.a(x08), .b(x02), .c(new_n58_), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n59_), .O(z04));
  nand2  g023(.a(new_n61_), .b(x04), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z05));
  nand2  g025(.a(new_n61_), .b(x05), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z06));
  inv1   g027(.a(x06), .O(new_n72_));
  oai21  g028(.a(new_n47_), .b(new_n72_), .c(new_n59_), .O(z07));
  inv1   g029(.a(x07), .O(new_n74_));
  oai21  g030(.a(new_n47_), .b(new_n74_), .c(new_n59_), .O(z08));
  inv1   g031(.a(new_n61_), .O(new_n76_));
  nor2   g032(.a(new_n46_), .b(new_n51_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n81_));
  oai21  g037(.a(new_n76_), .b(new_n57_), .c(new_n81_), .O(z09));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n79_), .c(x20), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x19), .O(new_n85_));
  oai21  g041(.a(new_n49_), .b(x19), .c(new_n85_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n62_), .O(z10));
  xor2a  g044(.a(x21), .b(x20), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n45_), .O(new_n90_));
  inv1   g046(.a(new_n79_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(x22), .c(x20), .d(x13), .O(new_n92_));
  aoi21  g048(.a(new_n92_), .b(new_n90_), .c(new_n48_), .O(new_n93_));
  nand3  g049(.a(x21), .b(new_n48_), .c(new_n45_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n93_), .c(x09), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(x08), .c(new_n64_), .O(z11));
  inv1   g053(.a(new_n50_), .O(new_n98_));
  nand2  g054(.a(x23), .b(x14), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n53_), .c(x21), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n98_), .c(x22), .O(new_n101_));
  nor2   g057(.a(new_n48_), .b(x10), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n46_), .c(x21), .d(x20), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(x09), .c(new_n47_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n66_), .c(new_n59_), .O(z12));
  nand4  g062(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x23), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x23), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x22), .O(new_n113_));
  nand2  g069(.a(x23), .b(new_n46_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n68_), .O(z13));
  nand3  g073(.a(x19), .b(x09), .c(new_n47_), .O(new_n118_));
  inv1   g074(.a(x24), .O(new_n119_));
  nor2   g075(.a(new_n51_), .b(new_n49_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n119_), .c(x23), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n118_), .c(new_n45_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x22), .O(new_n123_));
  inv1   g079(.a(new_n110_), .O(new_n124_));
  nand3  g080(.a(x26), .b(x25), .c(x16), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x23), .c(x22), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n124_), .c(new_n119_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n129_));
  nand2  g085(.a(x08), .b(x05), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(new_n123_), .O(z14));
  nor2   g087(.a(new_n52_), .b(new_n46_), .O(new_n132_));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n132_), .c(x24), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n110_), .c(x25), .O(new_n135_));
  inv1   g091(.a(x25), .O(new_n136_));
  nand4  g092(.a(new_n132_), .b(new_n124_), .c(new_n136_), .d(x24), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n139_));
  oai21  g095(.a(new_n76_), .b(new_n72_), .c(new_n139_), .O(z15));
  nor2   g096(.a(new_n48_), .b(x18), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n120_), .O(new_n142_));
  nand3  g098(.a(new_n132_), .b(x25), .c(x24), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n142_), .c(x26), .O(new_n144_));
  nor2   g100(.a(new_n119_), .b(new_n52_), .O(new_n145_));
  nor2   g101(.a(x26), .b(new_n136_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n145_), .c(new_n77_), .d(new_n50_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n149_));
  oai21  g105(.a(new_n76_), .b(new_n74_), .c(new_n149_), .O(z16));
endmodule


