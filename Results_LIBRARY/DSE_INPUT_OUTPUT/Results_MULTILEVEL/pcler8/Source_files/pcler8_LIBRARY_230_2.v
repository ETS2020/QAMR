// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nand3  g009(.a(x25), .b(x08), .c(x00), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(z01));
  nand3  g011(.a(x25), .b(x08), .c(x01), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(z02));
  inv1   g013(.a(x02), .O(new_n58_));
  aoi21  g014(.a(x25), .b(new_n58_), .c(new_n46_), .O(z03));
  nand3  g015(.a(x25), .b(x08), .c(x03), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z04));
  nand3  g017(.a(x25), .b(x08), .c(x04), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z05));
  inv1   g019(.a(x05), .O(new_n64_));
  aoi21  g020(.a(x25), .b(new_n64_), .c(new_n46_), .O(z06));
  nand3  g021(.a(x25), .b(x08), .c(x06), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z07));
  nand3  g023(.a(x25), .b(x08), .c(x07), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z08));
  inv1   g025(.a(x00), .O(new_n70_));
  inv1   g026(.a(x21), .O(new_n71_));
  inv1   g027(.a(x22), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g029(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  nand4  g030(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n77_));
  nand2  g033(.a(x25), .b(x08), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n70_), .c(new_n77_), .O(z09));
  inv1   g035(.a(x25), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(x01), .c(x08), .O(new_n81_));
  inv1   g037(.a(x20), .O(new_n82_));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n75_), .c(x20), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x19), .O(new_n85_));
  oai21  g041(.a(new_n82_), .b(x19), .c(new_n85_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n81_), .O(z10));
  inv1   g044(.a(x19), .O(new_n89_));
  nand3  g045(.a(x23), .b(x22), .c(x13), .O(new_n90_));
  nand3  g046(.a(x26), .b(x25), .c(x24), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n90_), .c(x21), .O(new_n92_));
  nor2   g048(.a(new_n71_), .b(x20), .O(new_n93_));
  aoi21  g049(.a(new_n92_), .b(x20), .c(new_n93_), .O(new_n94_));
  nand2  g050(.a(x21), .b(new_n89_), .O(new_n95_));
  oai21  g051(.a(new_n94_), .b(new_n89_), .c(new_n95_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n97_));
  oai21  g053(.a(new_n78_), .b(new_n58_), .c(new_n97_), .O(z11));
  nand4  g054(.a(x14), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n99_));
  inv1   g055(.a(x23), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n72_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(x26), .c(x24), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n99_), .O(new_n103_));
  aoi21  g059(.a(x08), .b(x03), .c(new_n103_), .O(new_n104_));
  nand3  g060(.a(x21), .b(x20), .c(x19), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nor2   g062(.a(new_n82_), .b(new_n89_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n72_), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n106_), .b(new_n72_), .c(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n110_));
  oai21  g066(.a(new_n104_), .b(new_n80_), .c(new_n110_), .O(z12));
  oai21  g067(.a(new_n80_), .b(x04), .c(x08), .O(new_n112_));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x23), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(x21), .c(x20), .d(x19), .O(new_n115_));
  nand2  g071(.a(new_n105_), .b(x23), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x22), .O(new_n118_));
  nand2  g074(.a(x23), .b(new_n72_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n112_), .O(z13));
  nand3  g078(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n123_));
  nand3  g079(.a(x26), .b(x24), .c(x16), .O(new_n124_));
  oai22  g080(.a(new_n124_), .b(new_n123_), .c(new_n46_), .d(new_n64_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x25), .O(new_n126_));
  nand3  g082(.a(new_n107_), .b(new_n101_), .c(x21), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x24), .O(new_n128_));
  inv1   g084(.a(x24), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x23), .c(x22), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n105_), .c(new_n128_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n126_), .O(z14));
  inv1   g089(.a(x06), .O(new_n134_));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n101_), .c(x24), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n105_), .c(x25), .O(new_n137_));
  nand4  g093(.a(new_n106_), .b(new_n101_), .c(new_n80_), .d(x24), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n140_));
  oai21  g096(.a(new_n78_), .b(new_n134_), .c(new_n140_), .O(z15));
  inv1   g097(.a(x09), .O(new_n142_));
  oai21  g098(.a(new_n80_), .b(x07), .c(x08), .O(new_n143_));
  inv1   g099(.a(x18), .O(new_n144_));
  nand3  g100(.a(x20), .b(x19), .c(new_n144_), .O(new_n145_));
  nand4  g101(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n145_), .c(x26), .O(new_n147_));
  nor2   g103(.a(new_n129_), .b(new_n100_), .O(new_n148_));
  nor2   g104(.a(x26), .b(new_n80_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n107_), .c(new_n148_), .d(new_n73_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n147_), .c(x08), .O(new_n151_));
  nor2   g107(.a(new_n45_), .b(x25), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n151_), .c(new_n47_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n142_), .c(new_n143_), .O(z16));
endmodule


