// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x21), .O(new_n54_));
  nor2   g010(.a(x24), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n55_), .b(new_n46_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n59_), .O(z02));
  inv1   g018(.a(x02), .O(new_n63_));
  nor2   g019(.a(new_n61_), .b(new_n63_), .O(z03));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n56_), .O(z04));
  nand2  g022(.a(x08), .b(x04), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n56_), .O(z05));
  aoi21  g024(.a(x08), .b(x05), .c(new_n55_), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z06));
  and2   g026(.a(new_n60_), .b(x06), .O(z07));
  nand2  g027(.a(new_n60_), .b(x07), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  nand3  g029(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  nand2  g030(.a(x23), .b(x22), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(new_n76_));
  inv1   g032(.a(x25), .O(new_n77_));
  nor2   g033(.a(new_n45_), .b(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n74_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n57_), .c(new_n56_), .O(z09));
  nor2   g038(.a(x24), .b(new_n54_), .O(new_n83_));
  xnor2a g039(.a(x20), .b(x19), .O(new_n84_));
  nand4  g040(.a(x22), .b(x21), .c(x19), .d(x12), .O(new_n85_));
  nand3  g041(.a(new_n78_), .b(x24), .c(x23), .O(new_n86_));
  oai22  g042(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n88_));
  oai21  g044(.a(new_n61_), .b(new_n59_), .c(new_n88_), .O(z10));
  inv1   g045(.a(x19), .O(new_n90_));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  nand3  g047(.a(x26), .b(x25), .c(x24), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n91_), .c(x21), .O(new_n93_));
  inv1   g049(.a(x24), .O(new_n94_));
  nor3   g050(.a(new_n94_), .b(new_n54_), .c(x20), .O(new_n95_));
  aoi21  g051(.a(new_n93_), .b(x20), .c(new_n95_), .O(new_n96_));
  nand3  g052(.a(x24), .b(x21), .c(new_n90_), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n90_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n99_));
  oai21  g055(.a(new_n61_), .b(new_n63_), .c(new_n99_), .O(z11));
  nand2  g056(.a(new_n60_), .b(x03), .O(new_n101_));
  inv1   g057(.a(x22), .O(new_n102_));
  nand3  g058(.a(new_n78_), .b(x23), .c(x14), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(x20), .c(x19), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(x24), .c(new_n54_), .O(new_n105_));
  nand2  g061(.a(x20), .b(x19), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(x24), .c(new_n102_), .d(x21), .O(new_n108_));
  oai21  g064(.a(new_n105_), .b(new_n102_), .c(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n101_), .O(z12));
  nand2  g067(.a(new_n60_), .b(x04), .O(new_n112_));
  nand3  g068(.a(x26), .b(x25), .c(x15), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x23), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x20), .c(x19), .O(new_n115_));
  nand2  g071(.a(new_n106_), .b(x23), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n115_), .c(new_n102_), .O(new_n117_));
  inv1   g073(.a(x23), .O(new_n118_));
  nor2   g074(.a(new_n118_), .b(x22), .O(new_n119_));
  aoi21  g075(.a(new_n117_), .b(x21), .c(new_n119_), .O(new_n120_));
  nand2  g076(.a(x23), .b(new_n54_), .O(new_n121_));
  oai21  g077(.a(new_n120_), .b(new_n94_), .c(new_n121_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n112_), .O(z13));
  nand3  g080(.a(x21), .b(x20), .c(x19), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand2  g082(.a(new_n78_), .b(x16), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n126_), .c(x23), .d(x22), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(x24), .c(new_n47_), .d(x09), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(x08), .c(new_n69_), .O(z14));
  nand4  g086(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n131_));
  nand4  g087(.a(new_n77_), .b(x23), .c(x22), .d(x20), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n131_), .c(x24), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x21), .O(new_n134_));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x23), .c(x22), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n126_), .c(new_n77_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n139_));
  nand2  g095(.a(x08), .b(x06), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n139_), .c(new_n134_), .O(z15));
  nand3  g097(.a(x25), .b(x23), .c(x22), .O(new_n142_));
  nor3   g098(.a(new_n142_), .b(new_n106_), .c(x18), .O(new_n143_));
  nor2   g099(.a(new_n143_), .b(new_n45_), .O(new_n144_));
  nor4   g100(.a(new_n125_), .b(new_n75_), .c(x26), .d(new_n77_), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n144_), .c(x24), .O(new_n146_));
  nand2  g102(.a(x26), .b(new_n54_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n72_), .O(z16));
endmodule


