// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_;
  inv1   g00(.a(x26), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand4  g03(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n45_), .O(z00));
  nand2  g09(.a(x08), .b(x00), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z01));
  inv1   g11(.a(x23), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(new_n57_));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n57_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n57_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n57_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n57_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n57_), .O(z08));
  nand3  g27(.a(x21), .b(x20), .c(x11), .O(new_n72_));
  inv1   g28(.a(x22), .O(new_n73_));
  inv1   g29(.a(x24), .O(new_n74_));
  nor2   g30(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g31(.a(x25), .O(new_n76_));
  nor2   g32(.a(new_n45_), .b(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n72_), .c(x19), .O(new_n79_));
  nand4  g35(.a(new_n79_), .b(x23), .c(new_n47_), .d(x09), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(x08), .c(new_n54_), .O(z09));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  nand3  g38(.a(x26), .b(x25), .c(x24), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n82_), .c(x20), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(x19), .O(new_n85_));
  inv1   g41(.a(x19), .O(new_n86_));
  nand2  g42(.a(x20), .b(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n85_), .c(new_n56_), .O(new_n88_));
  nand4  g44(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n58_), .O(z10));
  nand2  g46(.a(x22), .b(x13), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n83_), .c(x20), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n86_), .c(x21), .O(new_n93_));
  inv1   g49(.a(x21), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(x20), .c(x19), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n93_), .c(x10), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(x09), .c(new_n56_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(x08), .c(new_n60_), .O(z11));
  nand2  g54(.a(x20), .b(x19), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(new_n100_));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x14), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n100_), .c(x21), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x22), .O(new_n103_));
  nand3  g59(.a(new_n100_), .b(new_n73_), .c(x21), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n103_), .c(x10), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(x09), .c(new_n56_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(x08), .c(new_n62_), .O(z12));
  nand3  g63(.a(x21), .b(x20), .c(x19), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nand3  g65(.a(new_n77_), .b(x24), .c(x15), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(x22), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n47_), .c(x09), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n46_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n64_), .O(z13));
  nand3  g71(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(x22), .c(x21), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n99_), .c(x24), .O(new_n118_));
  nand4  g74(.a(new_n100_), .b(new_n74_), .c(x22), .d(x21), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n118_), .c(new_n56_), .O(new_n120_));
  nand4  g76(.a(new_n120_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n66_), .O(z14));
  nand2  g78(.a(x26), .b(x17), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(x24), .c(x22), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n108_), .c(x25), .O(new_n125_));
  nand4  g81(.a(new_n109_), .b(new_n76_), .c(x24), .d(x22), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n125_), .c(new_n56_), .O(new_n127_));
  nand4  g83(.a(new_n127_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n68_), .O(z15));
  inv1   g85(.a(x18), .O(new_n130_));
  nand2  g86(.a(new_n100_), .b(new_n130_), .O(new_n131_));
  nand4  g87(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n131_), .c(x26), .O(new_n133_));
  nand4  g89(.a(new_n109_), .b(new_n75_), .c(new_n45_), .d(x25), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n133_), .c(new_n56_), .O(new_n135_));
  nand4  g91(.a(new_n135_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n70_), .O(z16));
endmodule


