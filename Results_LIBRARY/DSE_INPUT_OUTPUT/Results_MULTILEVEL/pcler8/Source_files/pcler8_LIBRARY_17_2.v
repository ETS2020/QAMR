// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x07), .O(new_n54_));
  nor2   g010(.a(x23), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor3   g015(.a(new_n55_), .b(new_n46_), .c(new_n59_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n56_), .O(z03));
  nand2  g018(.a(x08), .b(x03), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n56_), .O(z04));
  nor2   g020(.a(new_n55_), .b(new_n46_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(x04), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n56_), .O(z06));
  nand2  g025(.a(x08), .b(x06), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n56_), .O(z07));
  nand3  g027(.a(x23), .b(x08), .c(x07), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  inv1   g029(.a(x23), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(x07), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(x08), .c(x00), .O(new_n76_));
  nand4  g032(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n77_));
  inv1   g033(.a(x24), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  inv1   g035(.a(x25), .O(new_n80_));
  nor2   g036(.a(new_n45_), .b(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai22  g038(.a(new_n82_), .b(new_n77_), .c(new_n55_), .d(x19), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n76_), .O(z09));
  xor2a  g041(.a(x20), .b(x19), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n87_));
  oai21  g043(.a(new_n46_), .b(new_n59_), .c(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n56_), .O(new_n89_));
  inv1   g045(.a(x09), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(x08), .O(new_n91_));
  nand2  g047(.a(x19), .b(x12), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x10), .O(new_n93_));
  inv1   g049(.a(x22), .O(new_n94_));
  nor2   g050(.a(new_n74_), .b(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(x21), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(new_n97_));
  nand3  g053(.a(x26), .b(x25), .c(x24), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n97_), .c(new_n93_), .d(new_n91_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n89_), .O(z10));
  inv1   g057(.a(x19), .O(new_n102_));
  xnor2a g058(.a(x21), .b(x20), .O(new_n103_));
  nand2  g059(.a(x21), .b(new_n102_), .O(new_n104_));
  oai21  g060(.a(new_n103_), .b(new_n102_), .c(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n61_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n56_), .O(new_n108_));
  nand2  g064(.a(x19), .b(x13), .O(new_n109_));
  nor2   g065(.a(new_n109_), .b(x10), .O(new_n110_));
  nand2  g066(.a(new_n95_), .b(x20), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n110_), .c(new_n99_), .d(new_n91_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n108_), .O(z11));
  nand2  g070(.a(x20), .b(x19), .O(new_n115_));
  nand2  g071(.a(x23), .b(x14), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n98_), .c(x21), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n115_), .c(x22), .O(new_n118_));
  inv1   g074(.a(new_n115_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n94_), .c(x21), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n63_), .c(new_n56_), .O(z12));
  nand3  g079(.a(x21), .b(x20), .c(x19), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n81_), .b(x24), .c(x15), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n125_), .c(x22), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x23), .O(new_n128_));
  nor2   g084(.a(x23), .b(new_n94_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n119_), .c(x21), .d(new_n54_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n66_), .O(z13));
  nand2  g089(.a(new_n65_), .b(x05), .O(new_n134_));
  nand3  g090(.a(x26), .b(x25), .c(x16), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n119_), .c(x22), .d(x21), .O(new_n136_));
  nand3  g092(.a(new_n78_), .b(x22), .c(x21), .O(new_n137_));
  nor2   g093(.a(new_n137_), .b(new_n115_), .O(new_n138_));
  aoi21  g094(.a(new_n136_), .b(x24), .c(new_n138_), .O(new_n139_));
  nand3  g095(.a(x24), .b(new_n74_), .c(new_n54_), .O(new_n140_));
  oai21  g096(.a(new_n139_), .b(new_n74_), .c(new_n140_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n134_), .O(z14));
  nand3  g099(.a(new_n91_), .b(x25), .c(new_n47_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n54_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n74_), .O(new_n146_));
  nand2  g102(.a(x26), .b(x17), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(x24), .c(x22), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n124_), .c(x25), .O(new_n149_));
  nand4  g105(.a(new_n125_), .b(new_n95_), .c(new_n80_), .d(x24), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n146_), .c(new_n70_), .O(z15));
  oai21  g109(.a(new_n74_), .b(x08), .c(x07), .O(new_n154_));
  inv1   g110(.a(x18), .O(new_n155_));
  nand4  g111(.a(x21), .b(x20), .c(x19), .d(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n95_), .b(x25), .c(x24), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n156_), .c(x26), .O(new_n158_));
  nand3  g114(.a(new_n119_), .b(x22), .c(x21), .O(new_n159_));
  nand3  g115(.a(new_n79_), .b(new_n45_), .c(x25), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n159_), .c(new_n158_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n154_), .O(z16));
endmodule


