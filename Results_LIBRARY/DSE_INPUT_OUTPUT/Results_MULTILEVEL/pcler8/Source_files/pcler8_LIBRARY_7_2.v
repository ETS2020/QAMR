// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  inv1   g010(.a(x24), .O(new_n55_));
  nor2   g011(.a(x26), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n54_), .O(z01));
  nand2  g015(.a(x08), .b(x01), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n57_), .O(z02));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n57_), .O(z03));
  inv1   g019(.a(x03), .O(new_n64_));
  nor2   g020(.a(new_n58_), .b(new_n64_), .O(z04));
  nand2  g021(.a(x08), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n57_), .O(z05));
  inv1   g023(.a(x05), .O(new_n68_));
  nor2   g024(.a(new_n58_), .b(new_n68_), .O(z06));
  nand3  g025(.a(new_n57_), .b(x08), .c(x06), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z07));
  aoi21  g027(.a(x08), .b(x07), .c(new_n56_), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  inv1   g029(.a(x20), .O(new_n74_));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x11), .O(new_n77_));
  inv1   g033(.a(x22), .O(new_n78_));
  inv1   g034(.a(x23), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x25), .c(x24), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n77_), .c(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n83_));
  aoi21  g039(.a(x08), .b(x00), .c(new_n56_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(z09));
  nand3  g041(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  nand3  g042(.a(x25), .b(x24), .c(x23), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  oai21  g045(.a(new_n74_), .b(x19), .c(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n60_), .c(new_n57_), .O(z10));
  nand2  g048(.a(x20), .b(x19), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand3  g050(.a(new_n75_), .b(x20), .c(x19), .O(new_n95_));
  oai21  g051(.a(new_n94_), .b(new_n75_), .c(new_n95_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n62_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n57_), .O(new_n99_));
  inv1   g055(.a(x09), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(x08), .O(new_n101_));
  nand2  g057(.a(x21), .b(x13), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(x10), .O(new_n103_));
  nand3  g059(.a(x26), .b(x25), .c(x24), .O(new_n104_));
  nor3   g060(.a(new_n104_), .b(new_n79_), .c(new_n78_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n103_), .c(new_n101_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n99_), .O(z11));
  nand3  g063(.a(x21), .b(x20), .c(x19), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand2  g065(.a(new_n78_), .b(x21), .O(new_n110_));
  oai22  g066(.a(new_n110_), .b(new_n93_), .c(new_n109_), .d(new_n78_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n112_));
  nand2  g068(.a(x08), .b(x03), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n57_), .O(new_n115_));
  nand4  g071(.a(new_n105_), .b(new_n101_), .c(x14), .d(new_n47_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(z12));
  nand3  g073(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n118_));
  nand3  g074(.a(x25), .b(x23), .c(x15), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n118_), .c(x26), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x24), .O(new_n121_));
  nand4  g077(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(new_n79_), .b(x22), .c(x21), .O(new_n124_));
  oai22  g080(.a(new_n124_), .b(new_n93_), .c(new_n123_), .d(new_n79_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n121_), .c(new_n66_), .O(z13));
  nand2  g083(.a(x25), .b(x16), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x22), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n108_), .c(x26), .O(new_n130_));
  nand4  g086(.a(new_n109_), .b(new_n55_), .c(x23), .d(x22), .O(new_n131_));
  oai21  g087(.a(new_n130_), .b(new_n55_), .c(new_n131_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n133_));
  oai21  g089(.a(new_n58_), .b(new_n68_), .c(new_n133_), .O(z14));
  inv1   g090(.a(x25), .O(new_n135_));
  inv1   g091(.a(x17), .O(new_n136_));
  nand3  g092(.a(x23), .b(x22), .c(x21), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n94_), .c(new_n136_), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(x26), .c(new_n55_), .O(new_n140_));
  nor2   g096(.a(new_n45_), .b(x25), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n123_), .c(x24), .d(x23), .O(new_n142_));
  oai21  g098(.a(new_n140_), .b(new_n135_), .c(new_n142_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n70_), .O(z15));
  inv1   g101(.a(new_n81_), .O(new_n146_));
  inv1   g102(.a(x19), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(x18), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n146_), .c(new_n76_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(x26), .c(new_n47_), .d(x09), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(x08), .c(new_n72_), .O(z16));
endmodule


