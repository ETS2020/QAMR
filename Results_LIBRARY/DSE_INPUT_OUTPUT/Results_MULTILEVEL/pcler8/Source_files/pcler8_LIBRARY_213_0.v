// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  nor2   g000(.a(x24), .b(x03), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x08), .O(new_n48_));
  nand2  g004(.a(x09), .b(new_n48_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand2  g006(.a(x20), .b(x19), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  and2   g008(.a(x23), .b(x22), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(x21), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g012(.a(new_n56_), .b(new_n52_), .c(new_n50_), .d(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n46_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  nand2  g015(.a(new_n46_), .b(x08), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n59_), .O(z01));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n46_), .O(z02));
  inv1   g019(.a(x02), .O(new_n64_));
  nor2   g020(.a(new_n60_), .b(new_n64_), .O(z03));
  and2   g021(.a(x08), .b(x03), .O(z04));
  nand2  g022(.a(x08), .b(x04), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n46_), .O(z05));
  inv1   g024(.a(x05), .O(new_n69_));
  nor2   g025(.a(new_n60_), .b(new_n69_), .O(z06));
  inv1   g026(.a(x06), .O(new_n71_));
  nor2   g027(.a(new_n60_), .b(new_n71_), .O(z07));
  nand2  g028(.a(x08), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n46_), .O(z08));
  nor2   g030(.a(x19), .b(x10), .O(new_n75_));
  aoi22  g031(.a(new_n75_), .b(new_n50_), .c(x08), .d(x00), .O(new_n76_));
  nand2  g032(.a(x20), .b(x11), .O(new_n77_));
  nor3   g033(.a(new_n77_), .b(new_n49_), .c(x10), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n56_), .O(new_n79_));
  oai21  g035(.a(new_n76_), .b(new_n45_), .c(new_n79_), .O(z09));
  xor2a  g036(.a(x20), .b(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n62_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n46_), .O(new_n84_));
  nand2  g040(.a(x19), .b(x12), .O(new_n85_));
  nor3   g041(.a(new_n85_), .b(new_n49_), .c(x10), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n56_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n84_), .O(z10));
  inv1   g044(.a(x19), .O(new_n89_));
  nand3  g045(.a(x23), .b(x22), .c(x13), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n55_), .c(x20), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n89_), .c(x21), .O(new_n92_));
  inv1   g048(.a(x21), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(x20), .c(x19), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n96_));
  aoi21  g052(.a(x08), .b(x02), .c(new_n45_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n96_), .O(z11));
  inv1   g054(.a(x24), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(x03), .c(x08), .O(new_n100_));
  nand2  g056(.a(x23), .b(x14), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n55_), .c(x21), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n51_), .c(x22), .O(new_n103_));
  inv1   g059(.a(x22), .O(new_n104_));
  nand3  g060(.a(new_n52_), .b(new_n104_), .c(x21), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n100_), .c(new_n46_), .O(z12));
  nand2  g064(.a(x22), .b(x21), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n51_), .c(x23), .O(new_n110_));
  inv1   g066(.a(x23), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(x22), .c(x21), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n51_), .c(new_n110_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n67_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n46_), .O(new_n116_));
  nand4  g072(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n50_), .c(x15), .d(new_n47_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n116_), .O(z13));
  nand3  g076(.a(new_n50_), .b(x19), .c(new_n47_), .O(new_n121_));
  nand3  g077(.a(new_n53_), .b(x21), .c(x20), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(x03), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n99_), .O(new_n124_));
  nand3  g080(.a(x21), .b(x20), .c(x19), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand3  g082(.a(x26), .b(x25), .c(x16), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x23), .c(x22), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n126_), .c(new_n99_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n131_));
  nand2  g087(.a(x08), .b(x05), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n131_), .c(new_n124_), .O(z14));
  nand2  g089(.a(x25), .b(new_n47_), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n49_), .c(x03), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n99_), .O(new_n136_));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x23), .c(x22), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n125_), .c(x25), .O(new_n139_));
  nor2   g095(.a(x25), .b(new_n99_), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n126_), .c(new_n53_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n143_));
  nand2  g099(.a(x08), .b(x06), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n143_), .c(new_n136_), .O(z15));
  nand3  g101(.a(x26), .b(new_n99_), .c(new_n47_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n49_), .c(new_n73_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x03), .O(new_n148_));
  inv1   g104(.a(x18), .O(new_n149_));
  nand3  g105(.a(x20), .b(x19), .c(new_n149_), .O(new_n150_));
  nand4  g106(.a(x25), .b(x23), .c(x22), .d(x21), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n150_), .c(x26), .O(new_n152_));
  inv1   g108(.a(x26), .O(new_n153_));
  nand4  g109(.a(new_n126_), .b(new_n53_), .c(new_n153_), .d(x25), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n73_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x24), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n148_), .O(z16));
endmodule


