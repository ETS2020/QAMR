// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x21), .O(new_n54_));
  nor2   g010(.a(x23), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n55_), .b(new_n46_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n59_), .O(z02));
  inv1   g018(.a(x02), .O(new_n63_));
  nor2   g019(.a(new_n61_), .b(new_n63_), .O(z03));
  inv1   g020(.a(x03), .O(new_n65_));
  oai21  g021(.a(new_n46_), .b(new_n65_), .c(new_n56_), .O(z04));
  nand2  g022(.a(x08), .b(x04), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n56_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n56_), .O(z06));
  nand2  g026(.a(new_n60_), .b(x06), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z07));
  and2   g028(.a(new_n60_), .b(x07), .O(z08));
  nand3  g029(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  inv1   g030(.a(x25), .O(new_n75_));
  nor2   g031(.a(new_n45_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x24), .c(x22), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(x19), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n57_), .c(new_n56_), .O(z09));
  nor2   g036(.a(x23), .b(new_n54_), .O(new_n81_));
  xnor2a g037(.a(x20), .b(x19), .O(new_n82_));
  nand4  g038(.a(x22), .b(x21), .c(x19), .d(x12), .O(new_n83_));
  nand3  g039(.a(new_n76_), .b(x24), .c(x23), .O(new_n84_));
  oai22  g040(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  oai21  g042(.a(new_n61_), .b(new_n59_), .c(new_n86_), .O(z10));
  inv1   g043(.a(x19), .O(new_n88_));
  nand3  g044(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  nand3  g045(.a(x26), .b(x25), .c(x24), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n89_), .c(x21), .O(new_n91_));
  nand2  g047(.a(x23), .b(x21), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x20), .O(new_n93_));
  aoi21  g049(.a(new_n91_), .b(x20), .c(new_n93_), .O(new_n94_));
  nand3  g050(.a(x23), .b(x21), .c(new_n88_), .O(new_n95_));
  oai21  g051(.a(new_n94_), .b(new_n88_), .c(new_n95_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n97_));
  oai21  g053(.a(new_n61_), .b(new_n63_), .c(new_n97_), .O(z11));
  inv1   g054(.a(x22), .O(new_n99_));
  nand4  g055(.a(x26), .b(x25), .c(x24), .d(x14), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(x20), .c(x19), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(x23), .c(new_n54_), .O(new_n102_));
  nand2  g058(.a(x20), .b(x19), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(x23), .c(new_n99_), .d(x21), .O(new_n105_));
  oai21  g061(.a(new_n102_), .b(new_n99_), .c(new_n105_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n107_));
  oai21  g063(.a(new_n61_), .b(new_n65_), .c(new_n107_), .O(z12));
  inv1   g064(.a(x09), .O(new_n109_));
  nor2   g065(.a(new_n109_), .b(x08), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x15), .c(new_n47_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n77_), .c(x23), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x21), .O(new_n113_));
  inv1   g069(.a(x23), .O(new_n114_));
  nand3  g070(.a(x21), .b(x20), .c(x19), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(x22), .c(new_n114_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n113_), .c(new_n67_), .O(z13));
  nand3  g075(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n120_));
  inv1   g076(.a(x24), .O(new_n121_));
  nand3  g077(.a(new_n104_), .b(new_n121_), .c(x22), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n120_), .c(x23), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x21), .O(new_n124_));
  nand3  g080(.a(x26), .b(x25), .c(x16), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x22), .c(x21), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n104_), .c(new_n121_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n124_), .c(new_n69_), .O(z14));
  nand2  g086(.a(x26), .b(x17), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n104_), .c(x24), .d(x22), .O(new_n132_));
  nand3  g088(.a(new_n75_), .b(x24), .c(x22), .O(new_n133_));
  nor2   g089(.a(new_n133_), .b(new_n115_), .O(new_n134_));
  aoi21  g090(.a(new_n132_), .b(x25), .c(new_n134_), .O(new_n135_));
  nand2  g091(.a(x25), .b(new_n54_), .O(new_n136_));
  oai21  g092(.a(new_n135_), .b(new_n114_), .c(new_n136_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n71_), .O(z15));
  nand3  g095(.a(new_n110_), .b(x19), .c(new_n47_), .O(new_n140_));
  nor2   g096(.a(x26), .b(new_n75_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(x24), .c(x22), .d(x20), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x23), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x21), .O(new_n144_));
  nand4  g100(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n145_));
  nor3   g101(.a(new_n145_), .b(new_n103_), .c(x18), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n45_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n148_));
  nand2  g104(.a(x08), .b(x07), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n148_), .c(new_n144_), .O(z16));
endmodule


