// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  nor2   g000(.a(x24), .b(x23), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  inv1   g005(.a(x19), .O(new_n50_));
  inv1   g006(.a(x20), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(new_n53_));
  inv1   g009(.a(x22), .O(new_n54_));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g012(.a(x26), .b(x25), .c(x24), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n56_), .c(x21), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(new_n53_), .c(new_n46_), .O(z00));
  inv1   g016(.a(x00), .O(new_n61_));
  nand2  g017(.a(new_n46_), .b(x08), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n61_), .O(z01));
  inv1   g019(.a(x01), .O(new_n64_));
  nor2   g020(.a(new_n62_), .b(new_n64_), .O(z02));
  inv1   g021(.a(x02), .O(new_n66_));
  nor2   g022(.a(new_n62_), .b(new_n66_), .O(z03));
  inv1   g023(.a(x03), .O(new_n68_));
  nor2   g024(.a(new_n62_), .b(new_n68_), .O(z04));
  inv1   g025(.a(x04), .O(new_n70_));
  nor2   g026(.a(new_n62_), .b(new_n70_), .O(z05));
  inv1   g027(.a(x05), .O(new_n72_));
  nor2   g028(.a(new_n62_), .b(new_n72_), .O(z06));
  nand2  g029(.a(x08), .b(x06), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n46_), .O(z07));
  inv1   g031(.a(x07), .O(new_n76_));
  nor2   g032(.a(new_n62_), .b(new_n76_), .O(z08));
  inv1   g033(.a(x08), .O(new_n78_));
  nand4  g034(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n79_));
  nand4  g035(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n79_), .c(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n47_), .c(x09), .d(new_n78_), .O(new_n82_));
  aoi21  g038(.a(x08), .b(x00), .c(new_n45_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n82_), .O(z09));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n80_), .c(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  oai21  g043(.a(new_n51_), .b(x19), .c(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n78_), .O(new_n89_));
  aoi21  g045(.a(x08), .b(x01), .c(new_n45_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(z10));
  nand3  g047(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n57_), .c(x21), .O(new_n93_));
  inv1   g049(.a(x21), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(x20), .O(new_n95_));
  aoi21  g051(.a(new_n93_), .b(x20), .c(new_n95_), .O(new_n96_));
  nand2  g052(.a(x21), .b(new_n50_), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n50_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n78_), .O(new_n99_));
  aoi21  g055(.a(x08), .b(x02), .c(new_n45_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n99_), .O(z11));
  inv1   g057(.a(new_n52_), .O(new_n102_));
  nand2  g058(.a(x23), .b(x14), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n57_), .c(x21), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n102_), .c(x22), .O(new_n105_));
  nand3  g061(.a(new_n52_), .b(new_n54_), .c(x21), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n47_), .c(x09), .d(new_n78_), .O(new_n108_));
  aoi21  g064(.a(x08), .b(x03), .c(new_n45_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(z12));
  nand3  g066(.a(new_n47_), .b(x09), .c(new_n78_), .O(new_n111_));
  nand3  g067(.a(new_n52_), .b(x22), .c(x21), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n111_), .c(x24), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n55_), .O(new_n114_));
  nand3  g070(.a(x21), .b(x20), .c(x19), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand4  g072(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n116_), .c(x22), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x23), .c(new_n47_), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x09), .c(new_n78_), .O(new_n121_));
  nand2  g077(.a(x08), .b(x04), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n121_), .c(new_n114_), .O(z13));
  nand3  g079(.a(x26), .b(x25), .c(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(x22), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n115_), .c(x24), .O(new_n126_));
  inv1   g082(.a(x24), .O(new_n127_));
  nand4  g083(.a(new_n116_), .b(new_n127_), .c(x23), .d(x22), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n78_), .O(new_n130_));
  oai21  g086(.a(new_n62_), .b(new_n72_), .c(new_n130_), .O(z14));
  nand3  g087(.a(new_n49_), .b(x25), .c(new_n47_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x24), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n55_), .O(new_n134_));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x24), .c(x22), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n115_), .c(x25), .O(new_n137_));
  nor2   g093(.a(x25), .b(new_n127_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n116_), .c(new_n56_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n137_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(new_n47_), .c(x09), .d(new_n78_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n134_), .c(new_n74_), .O(z15));
  nand3  g098(.a(new_n49_), .b(x26), .c(new_n47_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x24), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n55_), .O(new_n145_));
  inv1   g101(.a(x18), .O(new_n146_));
  nand3  g102(.a(x20), .b(x19), .c(new_n146_), .O(new_n147_));
  nand4  g103(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n147_), .c(x26), .O(new_n149_));
  inv1   g105(.a(x26), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(x25), .c(x24), .d(x23), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n112_), .c(new_n149_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n47_), .c(x09), .d(new_n78_), .O(new_n153_));
  nand2  g109(.a(x08), .b(x07), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(new_n153_), .c(new_n145_), .O(z16));
endmodule


