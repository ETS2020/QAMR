// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n64_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x25), .b(x24), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  and2   g011(.a(new_n55_), .b(x00), .O(z01));
  inv1   g012(.a(new_n54_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z02));
  nand2  g015(.a(x08), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n57_), .O(z03));
  and2   g017(.a(new_n55_), .b(x03), .O(z04));
  and2   g018(.a(new_n55_), .b(x04), .O(z05));
  nand2  g019(.a(new_n55_), .b(x05), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z06));
  and2   g021(.a(new_n55_), .b(x06), .O(z07));
  nand2  g022(.a(x08), .b(x07), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n57_), .O(z08));
  inv1   g024(.a(x21), .O(new_n69_));
  inv1   g025(.a(x22), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(x20), .c(x11), .O(new_n72_));
  nand4  g028(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand4  g030(.a(new_n74_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n75_));
  aoi21  g031(.a(x08), .b(x00), .c(new_n54_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n75_), .O(z09));
  inv1   g033(.a(x20), .O(new_n78_));
  nand3  g034(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n73_), .c(x20), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(x19), .O(new_n81_));
  oai21  g037(.a(new_n78_), .b(x19), .c(new_n81_), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n58_), .c(new_n57_), .O(z10));
  nand2  g040(.a(x20), .b(x19), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand3  g042(.a(new_n69_), .b(x20), .c(x19), .O(new_n87_));
  oai21  g043(.a(new_n86_), .b(new_n69_), .c(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n57_), .O(new_n91_));
  nand2  g047(.a(x21), .b(x13), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x10), .O(new_n93_));
  inv1   g049(.a(x23), .O(new_n94_));
  nand3  g050(.a(x26), .b(x25), .c(x24), .O(new_n95_));
  nor3   g051(.a(new_n95_), .b(new_n94_), .c(new_n70_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n93_), .c(x09), .d(new_n46_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n91_), .O(z11));
  nand2  g054(.a(x23), .b(x14), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n95_), .c(x21), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n85_), .c(x22), .O(new_n101_));
  nand3  g057(.a(new_n86_), .b(new_n70_), .c(x21), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n104_));
  aoi21  g060(.a(x08), .b(x03), .c(new_n54_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n104_), .O(z12));
  nand4  g062(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x22), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n85_), .c(x23), .O(new_n109_));
  nand4  g065(.a(new_n86_), .b(new_n94_), .c(x22), .d(x21), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n112_));
  aoi21  g068(.a(x08), .b(x04), .c(new_n54_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(z13));
  nand3  g070(.a(x21), .b(x20), .c(x19), .O(new_n115_));
  inv1   g071(.a(x24), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x23), .c(x22), .O(new_n117_));
  nand3  g073(.a(x26), .b(x24), .c(x16), .O(new_n118_));
  oai21  g074(.a(new_n117_), .b(new_n115_), .c(new_n118_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x25), .O(new_n120_));
  nand3  g076(.a(x23), .b(x22), .c(x21), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n85_), .c(x24), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n64_), .O(z14));
  nand3  g081(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x25), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n116_), .O(new_n128_));
  nand2  g084(.a(x26), .b(x17), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x23), .c(x22), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n115_), .c(x25), .O(new_n131_));
  inv1   g087(.a(x25), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x23), .c(x22), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n115_), .c(new_n131_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n135_));
  nand2  g091(.a(x08), .b(x06), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n135_), .c(new_n128_), .O(z15));
  nand2  g093(.a(new_n55_), .b(x07), .O(new_n138_));
  nand2  g094(.a(x26), .b(new_n132_), .O(new_n139_));
  nand4  g095(.a(new_n45_), .b(x25), .c(x23), .d(x22), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n115_), .c(new_n139_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x24), .O(new_n142_));
  nor2   g098(.a(new_n85_), .b(x18), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n71_), .c(x24), .d(x23), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(x26), .c(x25), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n138_), .O(z16));
endmodule


