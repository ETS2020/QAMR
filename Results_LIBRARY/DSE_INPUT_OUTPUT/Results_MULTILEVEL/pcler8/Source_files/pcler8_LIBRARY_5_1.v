// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  inv1   g010(.a(x24), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x23), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n54_), .O(z01));
  nand2  g015(.a(x08), .b(x01), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n57_), .O(z02));
  inv1   g017(.a(x02), .O(new_n62_));
  nor2   g018(.a(new_n58_), .b(new_n62_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n57_), .O(z04));
  inv1   g021(.a(x04), .O(new_n66_));
  oai21  g022(.a(new_n46_), .b(new_n66_), .c(new_n57_), .O(z05));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n57_), .O(z06));
  inv1   g025(.a(x06), .O(new_n70_));
  nor2   g026(.a(new_n58_), .b(new_n70_), .O(z07));
  nand2  g027(.a(x08), .b(x07), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n57_), .O(z08));
  inv1   g029(.a(x20), .O(new_n74_));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x11), .O(new_n77_));
  inv1   g033(.a(x25), .O(new_n78_));
  nor2   g034(.a(new_n45_), .b(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x24), .c(x22), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n77_), .c(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n82_));
  aoi21  g038(.a(x08), .b(x00), .c(new_n56_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n82_), .O(z09));
  xor2a  g040(.a(x20), .b(x19), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n60_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n57_), .O(new_n88_));
  inv1   g044(.a(x09), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(x08), .O(new_n90_));
  nand2  g046(.a(x19), .b(x12), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(x10), .O(new_n92_));
  nand2  g048(.a(x23), .b(x22), .O(new_n93_));
  nand2  g049(.a(new_n79_), .b(x24), .O(new_n94_));
  nor3   g050(.a(new_n94_), .b(new_n93_), .c(new_n75_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n92_), .c(new_n90_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n88_), .O(z10));
  inv1   g053(.a(x19), .O(new_n98_));
  xnor2a g054(.a(x21), .b(x20), .O(new_n99_));
  nand2  g055(.a(x21), .b(new_n98_), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n102_));
  nand2  g058(.a(x08), .b(x02), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n57_), .O(new_n105_));
  nand2  g061(.a(x19), .b(x13), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(x10), .O(new_n107_));
  nor3   g063(.a(new_n94_), .b(new_n93_), .c(new_n74_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n107_), .c(new_n90_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n105_), .O(z11));
  nand3  g066(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x22), .O(new_n112_));
  nand2  g068(.a(x20), .b(x19), .O(new_n113_));
  inv1   g069(.a(x22), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x21), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n64_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n57_), .O(new_n119_));
  nor2   g075(.a(new_n94_), .b(new_n93_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n90_), .c(x14), .d(new_n47_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n119_), .O(z12));
  inv1   g078(.a(new_n111_), .O(new_n123_));
  nand4  g079(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(new_n123_), .c(x22), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x23), .O(new_n126_));
  inv1   g082(.a(x23), .O(new_n127_));
  nand4  g083(.a(new_n123_), .b(new_n55_), .c(new_n127_), .d(x22), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n130_));
  oai21  g086(.a(new_n58_), .b(new_n66_), .c(new_n130_), .O(z13));
  nand3  g087(.a(new_n57_), .b(x08), .c(x05), .O(new_n132_));
  nand3  g088(.a(x26), .b(x25), .c(x16), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x22), .c(x21), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n113_), .c(x24), .O(new_n135_));
  inv1   g091(.a(new_n113_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n55_), .c(x22), .d(x21), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n135_), .c(new_n127_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n132_), .O(z14));
  nand3  g096(.a(new_n90_), .b(x19), .c(new_n47_), .O(new_n141_));
  nand3  g097(.a(new_n76_), .b(new_n78_), .c(x22), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n141_), .c(x23), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x24), .O(new_n144_));
  nand2  g100(.a(x26), .b(x17), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(x24), .c(x22), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n123_), .c(new_n78_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n149_));
  nand2  g105(.a(x08), .b(x06), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n149_), .c(new_n144_), .O(z15));
  nand4  g107(.a(new_n76_), .b(new_n45_), .c(x25), .d(x22), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n141_), .c(x23), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x24), .O(new_n154_));
  nand4  g110(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n155_));
  nor3   g111(.a(new_n155_), .b(new_n113_), .c(x18), .O(new_n156_));
  nor2   g112(.a(new_n156_), .b(new_n45_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n154_), .c(new_n72_), .O(z16));
endmodule


