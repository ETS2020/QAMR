// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x13), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n45_), .O(z00));
  nor2   g011(.a(new_n46_), .b(new_n49_), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x00), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(z01));
  nand2  g015(.a(new_n57_), .b(x01), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z02));
  inv1   g017(.a(new_n56_), .O(new_n62_));
  nand2  g018(.a(x08), .b(x02), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n62_), .O(z03));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n62_), .O(z04));
  nand2  g022(.a(new_n57_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n62_), .O(z06));
  nand2  g026(.a(x08), .b(x06), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n62_), .O(z07));
  nand2  g028(.a(new_n57_), .b(x07), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(z08));
  nand2  g030(.a(x08), .b(x00), .O(new_n75_));
  inv1   g031(.a(x19), .O(new_n76_));
  inv1   g032(.a(x09), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(x08), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n76_), .c(new_n48_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n62_), .O(new_n81_));
  nand3  g037(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  inv1   g039(.a(x20), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(x13), .O(new_n85_));
  inv1   g041(.a(x23), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n46_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x21), .O(new_n88_));
  nand3  g044(.a(x26), .b(x25), .c(x24), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n85_), .c(new_n83_), .d(x11), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n81_), .O(z09));
  xor2a  g048(.a(x20), .b(x19), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n94_));
  nand2  g050(.a(x08), .b(x01), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n62_), .O(new_n97_));
  nor2   g053(.a(new_n76_), .b(x13), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n90_), .c(new_n83_), .d(x12), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n97_), .O(z10));
  xnor2a g056(.a(x21), .b(x20), .O(new_n101_));
  nand2  g057(.a(x21), .b(new_n76_), .O(new_n102_));
  oai21  g058(.a(new_n101_), .b(new_n76_), .c(new_n102_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n63_), .c(new_n56_), .O(z11));
  nand2  g061(.a(x20), .b(x19), .O(new_n106_));
  nand2  g062(.a(x23), .b(x14), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n89_), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n106_), .c(x22), .O(new_n109_));
  inv1   g065(.a(new_n106_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(new_n46_), .c(x21), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n65_), .c(new_n62_), .O(z12));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n110_), .c(x21), .O(new_n116_));
  nand3  g072(.a(new_n86_), .b(x22), .c(x21), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(new_n106_), .O(new_n118_));
  aoi21  g074(.a(new_n116_), .b(x23), .c(new_n118_), .O(new_n119_));
  nand2  g075(.a(x23), .b(new_n46_), .O(new_n120_));
  oai21  g076(.a(new_n119_), .b(x13), .c(new_n120_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n67_), .O(z13));
  nand2  g079(.a(new_n57_), .b(x05), .O(new_n124_));
  nand3  g080(.a(x26), .b(x25), .c(x16), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n110_), .c(x23), .d(x21), .O(new_n126_));
  nand3  g082(.a(x21), .b(x20), .c(x19), .O(new_n127_));
  inv1   g083(.a(x24), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x22), .O(new_n129_));
  nor2   g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  aoi21  g086(.a(new_n126_), .b(x24), .c(new_n130_), .O(new_n131_));
  nand2  g087(.a(x24), .b(new_n46_), .O(new_n132_));
  oai21  g088(.a(new_n131_), .b(x13), .c(new_n132_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n124_), .O(z14));
  nand3  g091(.a(new_n78_), .b(x19), .c(new_n48_), .O(new_n136_));
  inv1   g092(.a(x25), .O(new_n137_));
  inv1   g093(.a(x21), .O(new_n138_));
  nor2   g094(.a(new_n138_), .b(new_n84_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n137_), .c(x24), .d(x23), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n136_), .c(new_n49_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x22), .O(new_n142_));
  nand2  g098(.a(x26), .b(x17), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n87_), .c(x24), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n127_), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n137_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n142_), .c(new_n71_), .O(z15));
  nand3  g104(.a(new_n110_), .b(new_n78_), .c(new_n48_), .O(new_n149_));
  nor2   g105(.a(new_n86_), .b(new_n138_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n45_), .c(x25), .d(x24), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n149_), .c(new_n49_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x22), .O(new_n153_));
  nor2   g109(.a(new_n76_), .b(x18), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n139_), .O(new_n155_));
  nand3  g111(.a(new_n87_), .b(x25), .c(x24), .O(new_n156_));
  nor2   g112(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nor2   g113(.a(new_n157_), .b(new_n45_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n159_));
  nand2  g115(.a(x08), .b(x07), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n159_), .c(new_n153_), .O(z16));
endmodule


