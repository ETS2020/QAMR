// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  inv1   g010(.a(x19), .O(new_n55_));
  inv1   g011(.a(x25), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n54_), .O(z01));
  inv1   g015(.a(x01), .O(new_n60_));
  oai21  g016(.a(new_n46_), .b(new_n60_), .c(new_n57_), .O(z02));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n57_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n57_), .O(z04));
  nand2  g021(.a(x08), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n57_), .O(z05));
  inv1   g023(.a(x05), .O(new_n68_));
  oai21  g024(.a(new_n46_), .b(new_n68_), .c(new_n57_), .O(z06));
  inv1   g025(.a(x06), .O(new_n70_));
  nor2   g026(.a(new_n58_), .b(new_n70_), .O(z07));
  nand2  g027(.a(x08), .b(x07), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n57_), .O(z08));
  nand2  g029(.a(x21), .b(x20), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x11), .O(new_n76_));
  inv1   g032(.a(x22), .O(new_n77_));
  inv1   g033(.a(x23), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x26), .c(x24), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n76_), .c(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(x25), .c(new_n47_), .d(x09), .O(new_n82_));
  oai22  g038(.a(new_n82_), .b(x08), .c(new_n58_), .d(new_n54_), .O(z09));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  nand4  g040(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  nand3  g043(.a(x25), .b(x20), .c(new_n55_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n90_));
  oai21  g046(.a(new_n58_), .b(new_n60_), .c(new_n90_), .O(z10));
  inv1   g047(.a(x09), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x08), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(x21), .c(new_n47_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x25), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n55_), .O(new_n96_));
  nand3  g052(.a(x23), .b(x22), .c(x13), .O(new_n97_));
  nand3  g053(.a(x26), .b(x25), .c(x24), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n97_), .c(x21), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x20), .O(new_n100_));
  inv1   g056(.a(x20), .O(new_n101_));
  nand2  g057(.a(x21), .b(new_n101_), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(new_n100_), .c(new_n55_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n96_), .c(new_n62_), .O(z11));
  nand3  g061(.a(new_n93_), .b(x22), .c(new_n47_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x25), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n55_), .O(new_n108_));
  nand2  g064(.a(x23), .b(x14), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n98_), .c(x21), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n101_), .c(x22), .O(new_n111_));
  nand4  g067(.a(new_n77_), .b(x21), .c(x20), .d(x19), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n108_), .c(new_n64_), .O(z12));
  nand3  g071(.a(new_n93_), .b(x23), .c(new_n47_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x25), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n55_), .O(new_n118_));
  nand4  g074(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x21), .c(x20), .O(new_n121_));
  nand2  g077(.a(new_n74_), .b(x23), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x22), .c(x19), .O(new_n124_));
  nand2  g080(.a(x23), .b(new_n77_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n118_), .c(new_n66_), .O(z13));
  inv1   g084(.a(x24), .O(new_n129_));
  nand2  g085(.a(x26), .b(x16), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x19), .O(new_n131_));
  nand4  g087(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n132_));
  aoi22  g088(.a(new_n132_), .b(x19), .c(new_n131_), .d(x25), .O(new_n133_));
  nand3  g089(.a(x21), .b(x20), .c(x19), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n129_), .c(x23), .d(x22), .O(new_n136_));
  oai21  g092(.a(new_n133_), .b(new_n129_), .c(new_n136_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n138_));
  oai21  g094(.a(new_n58_), .b(new_n68_), .c(new_n138_), .O(z14));
  nand2  g095(.a(x26), .b(x17), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n79_), .c(x24), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n134_), .c(x25), .O(new_n142_));
  nand4  g098(.a(new_n135_), .b(new_n79_), .c(new_n56_), .d(x24), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n145_));
  oai21  g101(.a(new_n58_), .b(new_n70_), .c(new_n145_), .O(z15));
  nand3  g102(.a(new_n93_), .b(x26), .c(new_n47_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x25), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n55_), .O(new_n149_));
  inv1   g105(.a(x18), .O(new_n150_));
  nand2  g106(.a(new_n75_), .b(new_n150_), .O(new_n151_));
  nand3  g107(.a(new_n79_), .b(x25), .c(x24), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n151_), .c(x26), .O(new_n153_));
  nand4  g109(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n154_));
  nand4  g110(.a(new_n45_), .b(x25), .c(x24), .d(x23), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n154_), .c(new_n153_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n149_), .c(new_n72_), .O(z16));
endmodule


