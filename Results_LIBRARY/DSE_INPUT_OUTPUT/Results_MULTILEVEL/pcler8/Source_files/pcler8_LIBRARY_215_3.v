// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x10), .O(new_n45_));
  inv1   g001(.a(x21), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  nand2  g003(.a(x20), .b(x19), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand3  g005(.a(new_n49_), .b(x09), .c(new_n47_), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand3  g012(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n45_), .c(new_n46_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  nor2   g015(.a(new_n46_), .b(new_n45_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  oai21  g017(.a(new_n47_), .b(new_n59_), .c(new_n61_), .O(z01));
  nand2  g018(.a(x08), .b(x01), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n61_), .O(z02));
  nor2   g020(.a(new_n60_), .b(new_n47_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(x02), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z03));
  nand2  g023(.a(new_n65_), .b(x03), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z04));
  nand2  g025(.a(new_n65_), .b(x04), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z05));
  nand2  g027(.a(new_n65_), .b(x05), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z06));
  nand2  g029(.a(x08), .b(x06), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n61_), .O(z07));
  nand2  g031(.a(new_n65_), .b(x07), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(z08));
  nand2  g033(.a(new_n65_), .b(x00), .O(new_n78_));
  nor2   g034(.a(new_n52_), .b(new_n46_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  inv1   g036(.a(x24), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n53_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x26), .c(x25), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n80_), .c(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n78_), .O(z09));
  xnor2a g042(.a(x20), .b(x19), .O(new_n87_));
  nand3  g043(.a(new_n79_), .b(x19), .c(x12), .O(new_n88_));
  oai22  g044(.a(new_n88_), .b(new_n83_), .c(new_n87_), .d(x10), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(x09), .c(new_n47_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n63_), .c(new_n61_), .O(z10));
  inv1   g047(.a(x19), .O(new_n92_));
  nand3  g048(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n55_), .c(x20), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n92_), .c(x21), .O(new_n95_));
  nand3  g051(.a(new_n46_), .b(x20), .c(x19), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n66_), .O(z11));
  nand2  g055(.a(x23), .b(x14), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n55_), .c(x21), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n48_), .c(x22), .O(new_n102_));
  nand3  g058(.a(new_n49_), .b(new_n52_), .c(x21), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n68_), .O(z12));
  nand4  g062(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x23), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x23), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x22), .O(new_n113_));
  nand2  g069(.a(x23), .b(new_n52_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n70_), .O(z13));
  nand3  g073(.a(x19), .b(x09), .c(new_n47_), .O(new_n118_));
  nand4  g074(.a(new_n81_), .b(x23), .c(x22), .d(x20), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n118_), .c(new_n45_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x21), .O(new_n121_));
  inv1   g077(.a(new_n110_), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n122_), .c(new_n81_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand2  g083(.a(x08), .b(x05), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n127_), .c(new_n121_), .O(z14));
  inv1   g085(.a(x25), .O(new_n130_));
  nand3  g086(.a(new_n54_), .b(new_n130_), .c(x24), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n50_), .c(new_n45_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x21), .O(new_n133_));
  nand2  g089(.a(x26), .b(x17), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n54_), .c(x24), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n122_), .c(new_n130_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n133_), .c(new_n74_), .O(z15));
  inv1   g095(.a(x18), .O(new_n140_));
  nand4  g096(.a(x21), .b(x20), .c(x19), .d(new_n140_), .O(new_n141_));
  nand3  g097(.a(new_n54_), .b(x25), .c(x24), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n141_), .c(x26), .O(new_n143_));
  nor2   g099(.a(x26), .b(new_n130_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n82_), .c(new_n79_), .d(new_n49_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n76_), .O(z16));
endmodule


