// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x10), .O(new_n45_));
  inv1   g001(.a(x21), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  and2   g003(.a(x20), .b(x19), .O(new_n48_));
  nand3  g004(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  and2   g006(.a(x23), .b(x22), .O(new_n51_));
  nand3  g007(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n45_), .c(new_n46_), .O(z00));
  nor2   g011(.a(new_n46_), .b(new_n45_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x00), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z01));
  inv1   g015(.a(x01), .O(new_n60_));
  oai21  g016(.a(new_n47_), .b(new_n60_), .c(new_n57_), .O(z02));
  nor2   g017(.a(new_n56_), .b(new_n47_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(x02), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z03));
  nand2  g020(.a(new_n62_), .b(x03), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z04));
  nand2  g022(.a(new_n62_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(new_n62_), .b(x05), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z06));
  inv1   g026(.a(x06), .O(new_n71_));
  oai21  g027(.a(new_n47_), .b(new_n71_), .c(new_n57_), .O(z07));
  nand2  g028(.a(x08), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n57_), .O(z08));
  nand4  g030(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai22  g032(.a(new_n76_), .b(new_n75_), .c(x19), .d(x10), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x09), .c(new_n47_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n58_), .c(new_n57_), .O(z09));
  inv1   g035(.a(new_n62_), .O(new_n80_));
  inv1   g036(.a(x20), .O(new_n81_));
  nand3  g037(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n76_), .c(x20), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x19), .O(new_n84_));
  oai21  g040(.a(new_n81_), .b(x19), .c(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n86_));
  oai21  g042(.a(new_n80_), .b(new_n60_), .c(new_n86_), .O(z10));
  nand3  g043(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n52_), .c(x21), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(x20), .c(x19), .d(new_n45_), .O(new_n90_));
  oai21  g046(.a(new_n48_), .b(new_n46_), .c(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(x09), .c(new_n47_), .O(new_n92_));
  aoi21  g048(.a(x08), .b(x02), .c(new_n56_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n92_), .O(z11));
  nand2  g050(.a(x09), .b(new_n47_), .O(new_n95_));
  inv1   g051(.a(x22), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x20), .c(x19), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n95_), .c(new_n45_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(x21), .O(new_n99_));
  and2   g055(.a(x23), .b(x14), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n53_), .c(new_n46_), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n48_), .c(new_n96_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n103_));
  nand2  g059(.a(x08), .b(x03), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(new_n99_), .O(z12));
  nand4  g061(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x23), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(x21), .c(x20), .d(x19), .O(new_n108_));
  nand3  g064(.a(x21), .b(x20), .c(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x23), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x22), .O(new_n112_));
  nand2  g068(.a(x23), .b(new_n96_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n67_), .O(z13));
  nand3  g072(.a(x19), .b(x09), .c(new_n47_), .O(new_n117_));
  inv1   g073(.a(x24), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(x23), .c(x22), .d(x20), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n117_), .c(new_n45_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x21), .O(new_n121_));
  inv1   g077(.a(new_n109_), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n122_), .c(new_n118_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand2  g083(.a(x08), .b(x05), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n127_), .c(new_n121_), .O(z14));
  nand2  g085(.a(x26), .b(x17), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n51_), .c(x24), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n109_), .c(x25), .O(new_n132_));
  nor2   g088(.a(x25), .b(new_n118_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n122_), .c(new_n51_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n136_));
  oai21  g092(.a(new_n80_), .b(new_n71_), .c(new_n136_), .O(z15));
  inv1   g093(.a(x26), .O(new_n138_));
  nand4  g094(.a(new_n51_), .b(new_n138_), .c(x25), .d(x24), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n49_), .c(new_n45_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x21), .O(new_n141_));
  inv1   g097(.a(x18), .O(new_n142_));
  nand4  g098(.a(x21), .b(x20), .c(x19), .d(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n51_), .b(x25), .c(x24), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n138_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n141_), .c(new_n73_), .O(z16));
endmodule


