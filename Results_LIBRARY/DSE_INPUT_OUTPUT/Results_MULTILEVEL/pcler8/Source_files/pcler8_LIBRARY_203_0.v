// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x23), .b(x22), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nand2  g011(.a(x08), .b(x00), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n55_), .O(z02));
  aoi21  g015(.a(x08), .b(x02), .c(new_n54_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z03));
  aoi21  g017(.a(x08), .b(x03), .c(new_n54_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z04));
  inv1   g019(.a(x04), .O(new_n64_));
  nand2  g020(.a(new_n55_), .b(x08), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(z05));
  nand2  g022(.a(x08), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n55_), .O(z06));
  nand2  g024(.a(x08), .b(x06), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n55_), .O(z07));
  nand2  g026(.a(x08), .b(x07), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n55_), .O(z08));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x09), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(x08), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n73_), .c(new_n47_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n56_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand2  g034(.a(x20), .b(x11), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x10), .O(new_n80_));
  inv1   g036(.a(x22), .O(new_n81_));
  inv1   g037(.a(x23), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x21), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand3  g041(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n85_), .c(new_n80_), .d(new_n75_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n78_), .O(z09));
  inv1   g045(.a(x20), .O(new_n90_));
  nand3  g046(.a(x22), .b(x21), .c(x12), .O(new_n91_));
  nand4  g047(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n91_), .c(x20), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(x19), .O(new_n94_));
  oai21  g050(.a(new_n90_), .b(x19), .c(new_n94_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n58_), .c(new_n55_), .O(z10));
  nand3  g053(.a(x23), .b(x22), .c(x13), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n86_), .c(x21), .O(new_n99_));
  inv1   g055(.a(x21), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(x20), .O(new_n101_));
  aoi21  g057(.a(new_n99_), .b(x20), .c(new_n101_), .O(new_n102_));
  nand2  g058(.a(x21), .b(new_n73_), .O(new_n103_));
  oai21  g059(.a(new_n102_), .b(new_n73_), .c(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n60_), .O(z11));
  nand3  g062(.a(x21), .b(x20), .c(x19), .O(new_n107_));
  oai21  g063(.a(new_n82_), .b(x22), .c(new_n107_), .O(new_n108_));
  nand2  g064(.a(x20), .b(x19), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nor2   g066(.a(x22), .b(new_n100_), .O(new_n111_));
  and2   g067(.a(x22), .b(x14), .O(new_n112_));
  aoi22  g068(.a(new_n112_), .b(new_n87_), .c(new_n111_), .d(new_n110_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n82_), .c(new_n108_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n62_), .O(z12));
  inv1   g072(.a(new_n107_), .O(new_n117_));
  nand4  g073(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n117_), .c(x22), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand4  g076(.a(new_n110_), .b(new_n82_), .c(x22), .d(x21), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n123_));
  oai21  g079(.a(new_n65_), .b(new_n64_), .c(new_n123_), .O(z13));
  nand3  g080(.a(new_n75_), .b(x24), .c(new_n47_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x23), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n81_), .O(new_n127_));
  nand3  g083(.a(x26), .b(x25), .c(x16), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x21), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n109_), .c(x24), .O(new_n130_));
  inv1   g086(.a(x24), .O(new_n131_));
  nand4  g087(.a(new_n117_), .b(new_n131_), .c(x23), .d(x22), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n127_), .c(new_n67_), .O(z14));
  nand3  g091(.a(new_n75_), .b(x25), .c(new_n47_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x23), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n81_), .O(new_n138_));
  nand2  g094(.a(x26), .b(x17), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x24), .c(x23), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n107_), .c(x25), .O(new_n141_));
  inv1   g097(.a(x25), .O(new_n142_));
  nand4  g098(.a(new_n117_), .b(new_n83_), .c(new_n142_), .d(x24), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n138_), .c(new_n69_), .O(z15));
  nand3  g102(.a(new_n55_), .b(x08), .c(x07), .O(new_n147_));
  nand2  g103(.a(x26), .b(new_n81_), .O(new_n148_));
  nand4  g104(.a(new_n45_), .b(x25), .c(x24), .d(x22), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n107_), .c(new_n148_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x23), .O(new_n151_));
  nor2   g107(.a(new_n109_), .b(x18), .O(new_n152_));
  nor2   g108(.a(new_n142_), .b(new_n131_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n152_), .c(x23), .d(x21), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x26), .c(x22), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n147_), .O(z16));
endmodule


