// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(new_n48_), .b(x09), .c(new_n47_), .d(x06), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x06), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  and2   g013(.a(new_n57_), .b(x00), .O(z01));
  inv1   g014(.a(new_n56_), .O(new_n59_));
  nand2  g015(.a(x08), .b(x01), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(z02));
  and2   g017(.a(new_n57_), .b(x02), .O(z03));
  and2   g018(.a(new_n57_), .b(x03), .O(z04));
  inv1   g019(.a(x04), .O(new_n64_));
  inv1   g020(.a(new_n57_), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(z05));
  nand2  g022(.a(x08), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n59_), .O(z06));
  inv1   g024(.a(x06), .O(new_n69_));
  oai21  g025(.a(new_n47_), .b(new_n69_), .c(new_n59_), .O(z07));
  and2   g026(.a(new_n57_), .b(x07), .O(z08));
  inv1   g027(.a(x21), .O(new_n72_));
  nor2   g028(.a(new_n46_), .b(new_n72_), .O(new_n73_));
  nand3  g029(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  inv1   g030(.a(x25), .O(new_n75_));
  nor2   g031(.a(new_n45_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x24), .c(x23), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(x19), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n79_));
  aoi21  g035(.a(x08), .b(x00), .c(new_n56_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n79_), .O(z09));
  xor2a  g037(.a(x20), .b(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n60_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n59_), .O(new_n85_));
  inv1   g041(.a(x09), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(x08), .O(new_n87_));
  nand2  g043(.a(x19), .b(x12), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(x10), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n87_), .c(x06), .O(new_n90_));
  nor2   g046(.a(new_n55_), .b(new_n46_), .O(new_n91_));
  nand3  g047(.a(x26), .b(x25), .c(x24), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(x21), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n90_), .c(new_n85_), .O(z10));
  nand3  g051(.a(x23), .b(x22), .c(x13), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n92_), .c(x21), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(x20), .c(x19), .O(new_n98_));
  and2   g054(.a(x20), .b(x19), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n72_), .c(new_n98_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n101_));
  aoi21  g057(.a(x08), .b(x02), .c(new_n56_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n101_), .O(z11));
  nand3  g059(.a(new_n87_), .b(x14), .c(new_n48_), .O(new_n104_));
  nand3  g060(.a(new_n76_), .b(x24), .c(x22), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n104_), .c(x06), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x23), .O(new_n107_));
  nand3  g063(.a(x21), .b(x20), .c(x19), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand3  g065(.a(new_n99_), .b(new_n46_), .c(x21), .O(new_n110_));
  oai21  g066(.a(new_n109_), .b(new_n46_), .c(new_n110_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n112_));
  nand2  g068(.a(x08), .b(x03), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n112_), .c(new_n107_), .O(z12));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n99_), .c(x22), .d(x21), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x23), .c(x06), .O(new_n117_));
  nand4  g073(.a(new_n99_), .b(new_n55_), .c(x22), .d(x21), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n120_));
  oai21  g076(.a(new_n65_), .b(new_n64_), .c(new_n120_), .O(z13));
  nand2  g077(.a(new_n57_), .b(x05), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n99_), .c(x22), .d(x21), .O(new_n124_));
  inv1   g080(.a(x24), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x23), .c(x22), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n108_), .O(new_n127_));
  aoi21  g083(.a(new_n124_), .b(x24), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(x24), .b(new_n55_), .O(new_n129_));
  oai21  g085(.a(new_n128_), .b(new_n69_), .c(new_n129_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n122_), .O(z14));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x24), .c(x22), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n108_), .c(x25), .O(new_n135_));
  nand4  g091(.a(new_n109_), .b(new_n91_), .c(new_n75_), .d(x24), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n135_), .c(x10), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(x09), .c(x08), .O(new_n138_));
  nand4  g094(.a(new_n87_), .b(x25), .c(new_n55_), .d(new_n48_), .O(new_n139_));
  oai21  g095(.a(new_n138_), .b(new_n69_), .c(new_n139_), .O(z15));
  nand3  g096(.a(new_n99_), .b(new_n87_), .c(new_n48_), .O(new_n141_));
  nand4  g097(.a(new_n73_), .b(new_n45_), .c(x25), .d(x24), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n141_), .c(x06), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x23), .O(new_n144_));
  inv1   g100(.a(x18), .O(new_n145_));
  nand4  g101(.a(x21), .b(x20), .c(x19), .d(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n91_), .b(x25), .c(x24), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n45_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n150_));
  nand2  g106(.a(x08), .b(x07), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n150_), .c(new_n144_), .O(z16));
endmodule


