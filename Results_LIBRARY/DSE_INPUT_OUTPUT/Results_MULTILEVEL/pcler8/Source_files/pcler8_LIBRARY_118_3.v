// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nand2  g009(.a(x25), .b(x10), .O(new_n54_));
  nand2  g010(.a(x08), .b(x00), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(new_n54_), .O(z01));
  nand2  g012(.a(x08), .b(x01), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g014(.a(x02), .O(new_n59_));
  oai21  g015(.a(new_n46_), .b(new_n59_), .c(new_n54_), .O(z03));
  inv1   g016(.a(x03), .O(new_n61_));
  oai21  g017(.a(new_n46_), .b(new_n61_), .c(new_n54_), .O(z04));
  nand2  g018(.a(new_n54_), .b(x08), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(x04), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z05));
  nand2  g022(.a(new_n64_), .b(x05), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z06));
  inv1   g024(.a(x06), .O(new_n69_));
  oai21  g025(.a(new_n46_), .b(new_n69_), .c(new_n54_), .O(z07));
  nand2  g026(.a(new_n64_), .b(x07), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z08));
  nand2  g028(.a(x22), .b(x21), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(x20), .c(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai22  g032(.a(new_n76_), .b(new_n75_), .c(x19), .d(x10), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x09), .c(new_n46_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n55_), .c(new_n54_), .O(z09));
  xnor2a g035(.a(x20), .b(x19), .O(new_n80_));
  nand3  g036(.a(new_n74_), .b(x19), .c(x12), .O(new_n81_));
  oai22  g037(.a(new_n81_), .b(new_n76_), .c(new_n80_), .d(x10), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x09), .c(new_n46_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n57_), .c(new_n54_), .O(z10));
  inv1   g040(.a(x19), .O(new_n85_));
  nand3  g041(.a(x23), .b(x22), .c(x13), .O(new_n86_));
  nand3  g042(.a(x26), .b(x25), .c(x24), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n85_), .c(x21), .O(new_n89_));
  inv1   g045(.a(x21), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(x20), .c(x19), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n93_));
  oai21  g049(.a(new_n63_), .b(new_n59_), .c(new_n93_), .O(z11));
  nand2  g050(.a(x20), .b(x19), .O(new_n95_));
  nand2  g051(.a(x23), .b(x14), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n87_), .c(x21), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n95_), .c(x22), .O(new_n98_));
  inv1   g054(.a(x22), .O(new_n99_));
  inv1   g055(.a(new_n95_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n99_), .c(x21), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n103_));
  oai21  g059(.a(new_n63_), .b(new_n61_), .c(new_n103_), .O(z12));
  nand3  g060(.a(x15), .b(x09), .c(new_n46_), .O(new_n105_));
  nand3  g061(.a(x26), .b(x24), .c(x23), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n105_), .c(new_n47_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x25), .O(new_n108_));
  oai21  g064(.a(new_n95_), .b(new_n73_), .c(x23), .O(new_n109_));
  inv1   g065(.a(x23), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x22), .c(x21), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n95_), .c(new_n109_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n113_));
  nand2  g069(.a(x08), .b(x04), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n113_), .c(new_n108_), .O(z13));
  nand3  g071(.a(x21), .b(x20), .c(x19), .O(new_n116_));
  nand3  g072(.a(x26), .b(x25), .c(x16), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x23), .c(x22), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x24), .O(new_n119_));
  inv1   g075(.a(new_n116_), .O(new_n120_));
  nor2   g076(.a(x24), .b(new_n110_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n120_), .c(x22), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n67_), .O(z14));
  nand2  g081(.a(x26), .b(x17), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(x24), .c(x23), .d(x22), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n116_), .c(x25), .O(new_n128_));
  inv1   g084(.a(x25), .O(new_n129_));
  nor2   g085(.a(new_n110_), .b(new_n99_), .O(new_n130_));
  nand4  g086(.a(new_n120_), .b(new_n130_), .c(new_n129_), .d(x24), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n133_));
  oai21  g089(.a(new_n63_), .b(new_n69_), .c(new_n133_), .O(z15));
  nand3  g090(.a(new_n100_), .b(x09), .c(new_n46_), .O(new_n135_));
  nand4  g091(.a(new_n74_), .b(new_n45_), .c(x24), .d(x23), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n135_), .c(new_n47_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x25), .O(new_n138_));
  inv1   g094(.a(x18), .O(new_n139_));
  nand4  g095(.a(x21), .b(x20), .c(x19), .d(new_n139_), .O(new_n140_));
  nand4  g096(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n45_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n144_));
  nand2  g100(.a(x08), .b(x07), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n144_), .c(new_n138_), .O(z16));
endmodule


