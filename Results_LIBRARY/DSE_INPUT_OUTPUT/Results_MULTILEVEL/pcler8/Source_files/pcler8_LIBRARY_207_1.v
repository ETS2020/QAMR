// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n69_,
    new_n71_, new_n73_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x10), .O(new_n46_));
  inv1   g002(.a(x09), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x08), .O(new_n48_));
  and2   g004(.a(x20), .b(x19), .O(new_n49_));
  nand3  g005(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(new_n50_));
  inv1   g006(.a(x21), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g009(.a(x26), .b(x25), .c(x24), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  or2    g012(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(x16), .c(new_n45_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  inv1   g015(.a(x08), .O(new_n60_));
  nor2   g016(.a(new_n45_), .b(x16), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g020(.a(new_n61_), .O(new_n65_));
  nand2  g021(.a(x08), .b(x01), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n65_), .O(z02));
  and2   g023(.a(new_n62_), .b(x02), .O(z03));
  nand2  g024(.a(x08), .b(x03), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n65_), .O(z04));
  inv1   g026(.a(x04), .O(new_n71_));
  oai21  g027(.a(new_n60_), .b(new_n71_), .c(new_n65_), .O(z05));
  nand2  g028(.a(x08), .b(x05), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n65_), .O(z06));
  and2   g030(.a(new_n62_), .b(x06), .O(z07));
  and2   g031(.a(new_n62_), .b(x07), .O(z08));
  inv1   g032(.a(x19), .O(new_n77_));
  nand3  g033(.a(new_n48_), .b(new_n77_), .c(new_n46_), .O(new_n78_));
  oai21  g034(.a(new_n60_), .b(new_n59_), .c(new_n78_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n65_), .O(new_n80_));
  nand3  g036(.a(x20), .b(x16), .c(x11), .O(new_n81_));
  nor4   g037(.a(new_n81_), .b(x10), .c(new_n47_), .d(x08), .O(new_n82_));
  nor2   g038(.a(new_n45_), .b(new_n52_), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n82_), .c(new_n55_), .d(x21), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n80_), .O(z09));
  nand3  g041(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  nand4  g042(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n86_), .c(x19), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x20), .O(new_n89_));
  oai21  g045(.a(x20), .b(new_n77_), .c(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n46_), .c(x09), .d(new_n60_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n66_), .c(new_n65_), .O(z10));
  inv1   g048(.a(x20), .O(new_n93_));
  nand3  g049(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n54_), .c(x21), .O(new_n95_));
  nor2   g051(.a(new_n51_), .b(x19), .O(new_n96_));
  aoi21  g052(.a(new_n95_), .b(x19), .c(new_n96_), .O(new_n97_));
  nand2  g053(.a(x21), .b(new_n93_), .O(new_n98_));
  oai21  g054(.a(new_n97_), .b(new_n93_), .c(new_n98_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n46_), .c(x09), .d(new_n60_), .O(new_n100_));
  aoi21  g056(.a(x08), .b(x02), .c(new_n61_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n100_), .O(z11));
  nand3  g058(.a(new_n48_), .b(x14), .c(new_n46_), .O(new_n103_));
  nand4  g059(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n103_), .c(x16), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x23), .O(new_n106_));
  nand3  g062(.a(x21), .b(x20), .c(x19), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  nand3  g064(.a(new_n49_), .b(new_n52_), .c(x21), .O(new_n109_));
  oai21  g065(.a(new_n108_), .b(new_n52_), .c(new_n109_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n46_), .c(x09), .d(new_n60_), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(new_n106_), .c(new_n69_), .O(z12));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n49_), .c(x22), .d(x21), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x23), .c(x16), .O(new_n115_));
  nand4  g071(.a(new_n49_), .b(new_n45_), .c(x22), .d(x21), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n46_), .c(x09), .d(new_n60_), .O(new_n118_));
  oai21  g074(.a(new_n63_), .b(new_n71_), .c(new_n118_), .O(z13));
  nand2  g075(.a(new_n62_), .b(x05), .O(new_n120_));
  inv1   g076(.a(x16), .O(new_n121_));
  nand2  g077(.a(x26), .b(x25), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n49_), .c(x22), .d(x21), .O(new_n123_));
  inv1   g079(.a(x24), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(x22), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n107_), .O(new_n126_));
  aoi21  g082(.a(new_n123_), .b(x24), .c(new_n126_), .O(new_n127_));
  nand2  g083(.a(x24), .b(new_n45_), .O(new_n128_));
  oai21  g084(.a(new_n127_), .b(new_n121_), .c(new_n128_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n46_), .c(x09), .d(new_n60_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n120_), .O(z14));
  nand3  g087(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n132_));
  inv1   g088(.a(x25), .O(new_n133_));
  nor2   g089(.a(new_n51_), .b(new_n93_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n133_), .c(x24), .d(x22), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n132_), .c(x16), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x23), .O(new_n137_));
  nand2  g093(.a(x26), .b(x17), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n83_), .c(x24), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n108_), .c(new_n133_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n46_), .c(x09), .d(new_n60_), .O(new_n142_));
  nand2  g098(.a(x08), .b(x06), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n142_), .c(new_n137_), .O(z15));
  inv1   g100(.a(x26), .O(new_n145_));
  nand4  g101(.a(new_n53_), .b(new_n145_), .c(x25), .d(x24), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n50_), .c(x16), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x23), .O(new_n148_));
  nor2   g104(.a(new_n77_), .b(x18), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n134_), .O(new_n150_));
  nand3  g106(.a(new_n83_), .b(x25), .c(x24), .O(new_n151_));
  nor2   g107(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nor2   g108(.a(new_n152_), .b(new_n145_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n46_), .c(x09), .d(new_n60_), .O(new_n154_));
  nand2  g110(.a(x08), .b(x07), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(new_n154_), .c(new_n148_), .O(z16));
endmodule


