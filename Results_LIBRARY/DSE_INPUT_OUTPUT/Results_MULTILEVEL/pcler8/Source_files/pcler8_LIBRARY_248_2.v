// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x23), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x08), .O(new_n48_));
  nand2  g004(.a(x09), .b(new_n48_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand2  g006(.a(x20), .b(x19), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  inv1   g008(.a(x21), .O(new_n53_));
  nand2  g009(.a(x23), .b(x22), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  nor3   g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand4  g012(.a(new_n56_), .b(new_n52_), .c(new_n50_), .d(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n46_), .O(z00));
  nand2  g014(.a(x08), .b(x00), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n46_), .O(z01));
  nand3  g016(.a(x23), .b(x08), .c(x01), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(z02));
  nand2  g018(.a(x08), .b(x02), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n46_), .O(z03));
  inv1   g020(.a(x03), .O(new_n65_));
  nand2  g021(.a(new_n46_), .b(x08), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n65_), .O(z04));
  nand2  g023(.a(x08), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n46_), .O(z05));
  inv1   g025(.a(x05), .O(new_n70_));
  nor2   g026(.a(new_n66_), .b(new_n70_), .O(z06));
  inv1   g027(.a(x06), .O(new_n72_));
  nor2   g028(.a(new_n66_), .b(new_n72_), .O(z07));
  nand2  g029(.a(x08), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n46_), .O(z08));
  inv1   g031(.a(x22), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n53_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n59_), .c(new_n46_), .O(z09));
  oai21  g038(.a(new_n45_), .b(x08), .c(x01), .O(new_n83_));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n79_), .c(x20), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(x19), .O(new_n86_));
  inv1   g042(.a(x19), .O(new_n87_));
  nand2  g043(.a(x20), .b(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n83_), .O(z10));
  nand3  g047(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n55_), .c(x21), .O(new_n93_));
  nor2   g049(.a(new_n53_), .b(x20), .O(new_n94_));
  aoi21  g050(.a(new_n93_), .b(x20), .c(new_n94_), .O(new_n95_));
  nand2  g051(.a(x21), .b(new_n87_), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n87_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n63_), .c(new_n46_), .O(z11));
  nand3  g055(.a(x21), .b(x20), .c(x19), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(new_n101_));
  nand2  g057(.a(new_n76_), .b(x21), .O(new_n102_));
  oai22  g058(.a(new_n102_), .b(new_n51_), .c(new_n101_), .d(new_n76_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n104_));
  oai21  g060(.a(new_n48_), .b(new_n65_), .c(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n46_), .O(new_n106_));
  nor2   g062(.a(new_n55_), .b(new_n54_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n50_), .c(x14), .d(new_n47_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n106_), .O(z12));
  inv1   g065(.a(x01), .O(new_n110_));
  nand3  g066(.a(new_n47_), .b(x09), .c(new_n48_), .O(new_n111_));
  nand2  g067(.a(new_n77_), .b(new_n52_), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n45_), .O(new_n114_));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n101_), .c(x22), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x23), .c(new_n47_), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x09), .c(new_n48_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n114_), .c(new_n68_), .O(z13));
  nand2  g076(.a(x24), .b(new_n47_), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n49_), .c(new_n110_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n45_), .O(new_n123_));
  nand3  g079(.a(x26), .b(x25), .c(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x22), .c(x21), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n51_), .c(x24), .O(new_n126_));
  inv1   g082(.a(x24), .O(new_n127_));
  nand4  g083(.a(new_n101_), .b(new_n127_), .c(x23), .d(x22), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n130_));
  nand2  g086(.a(x08), .b(x05), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n130_), .c(new_n123_), .O(z14));
  nand2  g088(.a(x08), .b(x06), .O(new_n133_));
  nand3  g089(.a(x25), .b(new_n45_), .c(new_n47_), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n49_), .c(new_n133_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n110_), .O(new_n136_));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x24), .c(x22), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n100_), .c(x25), .O(new_n139_));
  nor2   g095(.a(x25), .b(new_n127_), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n101_), .c(x22), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n133_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x23), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n136_), .O(z15));
  nand3  g102(.a(x26), .b(new_n45_), .c(new_n47_), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n49_), .c(new_n74_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n110_), .O(new_n149_));
  inv1   g105(.a(x18), .O(new_n150_));
  nand3  g106(.a(x20), .b(x19), .c(new_n150_), .O(new_n151_));
  nand4  g107(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n151_), .c(x26), .O(new_n153_));
  inv1   g109(.a(x26), .O(new_n154_));
  nor2   g110(.a(new_n127_), .b(new_n76_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n101_), .c(new_n154_), .d(x25), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n47_), .c(x09), .d(new_n48_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n74_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(x23), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n149_), .O(z16));
endmodule


