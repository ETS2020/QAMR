// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  inv1   g010(.a(x24), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x19), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  oai21  g013(.a(new_n46_), .b(new_n54_), .c(new_n57_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n56_), .b(new_n46_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n59_), .O(z02));
  inv1   g018(.a(x02), .O(new_n63_));
  nor2   g019(.a(new_n61_), .b(new_n63_), .O(z03));
  nand2  g020(.a(new_n60_), .b(x03), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z04));
  nand2  g022(.a(new_n60_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n57_), .O(z06));
  nand2  g026(.a(x08), .b(x06), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n57_), .O(z07));
  nand2  g028(.a(x08), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n57_), .O(z08));
  nand2  g030(.a(x21), .b(x20), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x19), .c(x11), .O(new_n77_));
  inv1   g033(.a(x22), .O(new_n78_));
  inv1   g034(.a(x23), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g036(.a(x26), .b(x25), .c(x24), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  oai22  g039(.a(new_n83_), .b(new_n77_), .c(x24), .d(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n85_));
  oai21  g041(.a(new_n61_), .b(new_n54_), .c(new_n85_), .O(z09));
  inv1   g042(.a(x21), .O(new_n87_));
  nor2   g043(.a(new_n78_), .b(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x12), .O(new_n89_));
  inv1   g045(.a(x25), .O(new_n90_));
  nor2   g046(.a(new_n45_), .b(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(x24), .c(x23), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n89_), .c(x20), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(x19), .O(new_n94_));
  inv1   g050(.a(x19), .O(new_n95_));
  nand3  g051(.a(new_n55_), .b(x20), .c(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n98_));
  oai21  g054(.a(new_n61_), .b(new_n59_), .c(new_n98_), .O(z10));
  nand3  g055(.a(x23), .b(x22), .c(x13), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n81_), .c(x21), .O(new_n101_));
  nor2   g057(.a(new_n87_), .b(x20), .O(new_n102_));
  aoi21  g058(.a(new_n101_), .b(x20), .c(new_n102_), .O(new_n103_));
  nand3  g059(.a(new_n55_), .b(x21), .c(new_n95_), .O(new_n104_));
  oai21  g060(.a(new_n103_), .b(new_n95_), .c(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n106_));
  oai21  g062(.a(new_n61_), .b(new_n63_), .c(new_n106_), .O(z11));
  inv1   g063(.a(x20), .O(new_n108_));
  nand2  g064(.a(x23), .b(x14), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n81_), .c(x21), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n108_), .c(x22), .O(new_n111_));
  nand3  g067(.a(new_n78_), .b(x21), .c(x20), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x19), .O(new_n114_));
  nand3  g070(.a(new_n55_), .b(x22), .c(new_n95_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n65_), .O(z12));
  nand4  g074(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x21), .c(x20), .O(new_n121_));
  nand2  g077(.a(new_n75_), .b(x23), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n121_), .c(new_n78_), .O(new_n123_));
  nor2   g079(.a(new_n79_), .b(x22), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n123_), .c(x19), .O(new_n125_));
  nand3  g081(.a(new_n55_), .b(x23), .c(new_n95_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n67_), .O(z13));
  nand2  g085(.a(new_n60_), .b(x05), .O(new_n130_));
  nand2  g086(.a(new_n91_), .b(x16), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x23), .c(x22), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n75_), .c(x24), .O(new_n133_));
  nand4  g089(.a(new_n76_), .b(new_n55_), .c(x23), .d(x22), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n133_), .c(new_n95_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n130_), .O(z14));
  nand4  g093(.a(x20), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n138_));
  nand3  g094(.a(new_n88_), .b(new_n90_), .c(x23), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n138_), .c(x19), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x24), .O(new_n141_));
  nand2  g097(.a(new_n88_), .b(x20), .O(new_n142_));
  nand2  g098(.a(x26), .b(x17), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x24), .c(x23), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n90_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n141_), .c(new_n71_), .O(z15));
  nand4  g104(.a(new_n88_), .b(new_n45_), .c(x25), .d(x23), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n138_), .c(x19), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x24), .O(new_n151_));
  nand3  g107(.a(new_n80_), .b(x25), .c(x24), .O(new_n152_));
  nor3   g108(.a(new_n152_), .b(new_n75_), .c(x18), .O(new_n153_));
  nor2   g109(.a(new_n153_), .b(new_n45_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(new_n151_), .c(new_n73_), .O(z16));
endmodule


