// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_;
  inv1   g00(.a(x26), .O(new_n45_));
  inv1   g01(.a(x22), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  inv1   g04(.a(x18), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n45_), .O(z00));
  nand2  g11(.a(x18), .b(new_n48_), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  oai21  g17(.a(new_n47_), .b(new_n61_), .c(new_n56_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nand2  g19(.a(new_n56_), .b(x08), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n63_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n56_), .O(z05));
  inv1   g23(.a(x05), .O(new_n68_));
  nor2   g24(.a(new_n64_), .b(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n56_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n56_), .O(z08));
  inv1   g29(.a(x21), .O(new_n74_));
  nor2   g30(.a(new_n46_), .b(new_n74_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(x09), .c(new_n47_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n49_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n48_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n57_), .O(z09));
  inv1   g38(.a(x09), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n77_), .c(x20), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x19), .O(new_n86_));
  inv1   g42(.a(x19), .O(new_n87_));
  nand2  g43(.a(x20), .b(new_n87_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n83_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n47_), .c(x18), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(x10), .c(new_n59_), .O(z10));
  nand3  g47(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  nand3  g48(.a(x26), .b(x25), .c(x24), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n92_), .c(x20), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n87_), .c(x21), .O(new_n95_));
  nand3  g51(.a(new_n74_), .b(x20), .c(x19), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n95_), .c(x18), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n98_));
  oai21  g54(.a(new_n64_), .b(new_n61_), .c(new_n98_), .O(z11));
  nand2  g55(.a(x20), .b(x19), .O(new_n100_));
  nand2  g56(.a(x23), .b(x14), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n93_), .c(x21), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(x22), .O(new_n103_));
  inv1   g59(.a(new_n100_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n46_), .c(x21), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n103_), .c(new_n83_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n47_), .c(x18), .O(new_n107_));
  nand2  g63(.a(x08), .b(x03), .O(new_n108_));
  oai21  g64(.a(new_n107_), .b(x10), .c(new_n108_), .O(z12));
  nand4  g65(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(x22), .c(x21), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n100_), .c(x23), .O(new_n112_));
  inv1   g68(.a(x23), .O(new_n113_));
  nand4  g69(.a(new_n104_), .b(new_n113_), .c(x22), .d(x21), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n83_), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n47_), .c(x18), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(x10), .c(new_n66_), .O(z13));
  nand3  g73(.a(x21), .b(x20), .c(x19), .O(new_n118_));
  nand3  g74(.a(x26), .b(x25), .c(x16), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(x23), .c(x22), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n118_), .c(x24), .O(new_n121_));
  inv1   g77(.a(x24), .O(new_n122_));
  inv1   g78(.a(new_n118_), .O(new_n123_));
  nand4  g79(.a(new_n123_), .b(new_n122_), .c(x23), .d(x22), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n121_), .c(x18), .O(new_n125_));
  nand4  g81(.a(new_n125_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n126_));
  oai21  g82(.a(new_n64_), .b(new_n68_), .c(new_n126_), .O(z14));
  nand2  g83(.a(x26), .b(x17), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(x24), .c(x23), .d(x22), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n118_), .c(x25), .O(new_n130_));
  inv1   g86(.a(x25), .O(new_n131_));
  nor2   g87(.a(new_n113_), .b(new_n46_), .O(new_n132_));
  nand4  g88(.a(new_n123_), .b(new_n132_), .c(new_n131_), .d(x24), .O(new_n133_));
  aoi21  g89(.a(new_n133_), .b(new_n130_), .c(new_n83_), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n47_), .c(x18), .O(new_n135_));
  oai21  g91(.a(new_n135_), .b(x10), .c(new_n70_), .O(z15));
  nand4  g92(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n137_));
  oai21  g93(.a(new_n137_), .b(new_n118_), .c(x26), .O(new_n138_));
  nor2   g94(.a(new_n122_), .b(new_n113_), .O(new_n139_));
  nor2   g95(.a(x26), .b(new_n131_), .O(new_n140_));
  nand4  g96(.a(new_n140_), .b(new_n104_), .c(new_n139_), .d(new_n75_), .O(new_n141_));
  aoi21  g97(.a(new_n141_), .b(new_n138_), .c(new_n83_), .O(new_n142_));
  aoi21  g98(.a(new_n142_), .b(new_n47_), .c(x18), .O(new_n143_));
  oai21  g99(.a(new_n143_), .b(x10), .c(new_n72_), .O(z16));
endmodule


