// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nor2   g010(.a(x21), .b(new_n47_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  oai21  g012(.a(new_n46_), .b(new_n54_), .c(new_n56_), .O(z01));
  inv1   g013(.a(x01), .O(new_n58_));
  nor2   g014(.a(new_n55_), .b(new_n46_), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n58_), .O(z02));
  aoi21  g017(.a(x08), .b(x02), .c(new_n55_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n56_), .O(z04));
  nand2  g021(.a(x08), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n56_), .O(z05));
  and2   g023(.a(new_n59_), .b(x05), .O(z06));
  and2   g024(.a(new_n59_), .b(x06), .O(z07));
  nand2  g025(.a(x08), .b(x07), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n56_), .O(z08));
  inv1   g027(.a(x21), .O(new_n72_));
  inv1   g028(.a(x22), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(x20), .c(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n78_));
  oai21  g034(.a(new_n60_), .b(new_n54_), .c(new_n78_), .O(z09));
  inv1   g035(.a(x20), .O(new_n80_));
  nand3  g036(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n76_), .c(x20), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x19), .O(new_n83_));
  oai21  g039(.a(new_n80_), .b(x19), .c(new_n83_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n85_));
  oai21  g041(.a(new_n60_), .b(new_n58_), .c(new_n85_), .O(z10));
  inv1   g042(.a(x19), .O(new_n87_));
  nand3  g043(.a(x22), .b(x20), .c(x13), .O(new_n88_));
  oai22  g044(.a(new_n88_), .b(new_n76_), .c(new_n72_), .d(x20), .O(new_n89_));
  aoi22  g045(.a(new_n89_), .b(new_n47_), .c(new_n72_), .d(x20), .O(new_n90_));
  nand3  g046(.a(x21), .b(new_n87_), .c(new_n47_), .O(new_n91_));
  oai21  g047(.a(new_n90_), .b(new_n87_), .c(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(x09), .c(new_n46_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n62_), .O(z11));
  nand2  g050(.a(x22), .b(x09), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(x08), .c(new_n47_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n72_), .O(new_n97_));
  nand2  g053(.a(x23), .b(x14), .O(new_n98_));
  nand3  g054(.a(x26), .b(x25), .c(x24), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n87_), .c(x22), .O(new_n101_));
  nor2   g057(.a(new_n80_), .b(new_n87_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n73_), .c(x21), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n97_), .c(new_n64_), .O(z12));
  nand2  g062(.a(new_n59_), .b(x04), .O(new_n107_));
  nand4  g063(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x23), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(x21), .c(x20), .d(x19), .O(new_n110_));
  nand3  g066(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x23), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x22), .O(new_n114_));
  nand2  g070(.a(x23), .b(new_n73_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n107_), .O(z13));
  nand2  g074(.a(x24), .b(x09), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(x08), .c(new_n47_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n72_), .O(new_n121_));
  inv1   g077(.a(new_n102_), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x24), .O(new_n125_));
  inv1   g081(.a(x24), .O(new_n126_));
  inv1   g082(.a(new_n111_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n126_), .c(x23), .d(x22), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n130_));
  nand2  g086(.a(x08), .b(x05), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n130_), .c(new_n121_), .O(z14));
  nand2  g088(.a(x25), .b(x09), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(x08), .c(new_n47_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n72_), .O(new_n135_));
  nand3  g091(.a(x22), .b(x20), .c(x19), .O(new_n136_));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x24), .c(x23), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x25), .O(new_n139_));
  inv1   g095(.a(x25), .O(new_n140_));
  inv1   g096(.a(x23), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n73_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n127_), .c(new_n140_), .d(x24), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n139_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n145_));
  nand2  g101(.a(x08), .b(x06), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n145_), .c(new_n135_), .O(z15));
  nand2  g103(.a(x26), .b(x09), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(x08), .c(new_n47_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n72_), .O(new_n150_));
  inv1   g106(.a(x18), .O(new_n151_));
  nand2  g107(.a(new_n102_), .b(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n142_), .b(x25), .c(x24), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n152_), .c(x26), .O(new_n154_));
  nor2   g110(.a(new_n126_), .b(new_n141_), .O(new_n155_));
  nor2   g111(.a(x26), .b(new_n140_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n102_), .c(new_n155_), .d(new_n74_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n150_), .c(new_n70_), .O(z16));
endmodule


