// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x25), .b(x20), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nand2  g011(.a(x08), .b(x00), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n55_), .O(z02));
  nand2  g015(.a(x08), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n55_), .O(z03));
  nor2   g017(.a(new_n54_), .b(new_n46_), .O(new_n62_));
  and2   g018(.a(new_n62_), .b(x03), .O(z04));
  aoi21  g019(.a(x08), .b(x04), .c(new_n54_), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z05));
  nand2  g021(.a(new_n62_), .b(x05), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z06));
  nand2  g023(.a(x08), .b(x06), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n55_), .O(z07));
  and2   g025(.a(new_n62_), .b(x07), .O(z08));
  inv1   g026(.a(x19), .O(new_n71_));
  inv1   g027(.a(x09), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(x08), .O(new_n73_));
  nand3  g029(.a(new_n73_), .b(new_n71_), .c(new_n47_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n56_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n55_), .O(new_n76_));
  nand2  g032(.a(x20), .b(x11), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(x10), .O(new_n78_));
  inv1   g034(.a(x22), .O(new_n79_));
  inv1   g035(.a(x23), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(x21), .O(new_n82_));
  nand3  g038(.a(x26), .b(x25), .c(x24), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n78_), .c(new_n73_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n76_), .O(z09));
  inv1   g042(.a(x20), .O(new_n87_));
  nand3  g043(.a(x22), .b(x21), .c(x12), .O(new_n88_));
  nand4  g044(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x19), .O(new_n91_));
  oai21  g047(.a(new_n87_), .b(x19), .c(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n58_), .c(new_n55_), .O(z10));
  nand3  g050(.a(x21), .b(new_n71_), .c(new_n47_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(new_n96_));
  aoi22  g052(.a(new_n96_), .b(new_n73_), .c(x08), .d(x02), .O(new_n97_));
  nand3  g053(.a(x23), .b(x22), .c(x13), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n83_), .c(x21), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x20), .O(new_n100_));
  nand3  g056(.a(x25), .b(x21), .c(new_n87_), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n100_), .c(new_n71_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n103_));
  oai21  g059(.a(new_n97_), .b(new_n54_), .c(new_n103_), .O(z11));
  nand3  g060(.a(new_n73_), .b(x22), .c(new_n47_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x25), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n87_), .O(new_n107_));
  nand2  g063(.a(x23), .b(x14), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n83_), .c(x21), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n71_), .c(x22), .O(new_n110_));
  nand4  g066(.a(new_n79_), .b(x21), .c(x20), .d(x19), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n113_));
  nand2  g069(.a(x08), .b(x03), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n113_), .c(new_n107_), .O(z12));
  nand4  g071(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x23), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x21), .c(x19), .O(new_n118_));
  nand2  g074(.a(x21), .b(x19), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x22), .c(x20), .O(new_n122_));
  oai21  g078(.a(new_n79_), .b(new_n87_), .c(x23), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n64_), .O(z13));
  inv1   g082(.a(x24), .O(new_n127_));
  inv1   g083(.a(x25), .O(new_n128_));
  nand2  g084(.a(x26), .b(x16), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(x20), .c(new_n128_), .O(new_n130_));
  inv1   g086(.a(new_n119_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n81_), .O(new_n132_));
  aoi21  g088(.a(new_n132_), .b(x20), .c(new_n130_), .O(new_n133_));
  nand3  g089(.a(x21), .b(x20), .c(x19), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n127_), .c(x23), .d(x22), .O(new_n136_));
  oai21  g092(.a(new_n133_), .b(new_n127_), .c(new_n136_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n66_), .O(z14));
  nand2  g095(.a(new_n62_), .b(x06), .O(new_n140_));
  nand2  g096(.a(x26), .b(x17), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n81_), .c(x24), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n134_), .c(x25), .O(new_n143_));
  nand4  g099(.a(new_n135_), .b(new_n81_), .c(new_n128_), .d(x24), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n140_), .O(z15));
  nand3  g103(.a(new_n73_), .b(x26), .c(new_n47_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x25), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n87_), .O(new_n150_));
  inv1   g106(.a(x18), .O(new_n151_));
  nand2  g107(.a(new_n131_), .b(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n81_), .b(x25), .c(x24), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n152_), .c(x26), .O(new_n154_));
  nor2   g110(.a(new_n127_), .b(new_n80_), .O(new_n155_));
  nand4  g111(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n155_), .c(new_n45_), .d(x25), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n154_), .O(new_n159_));
  nand4  g115(.a(new_n159_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n160_));
  nand2  g116(.a(x08), .b(x07), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(new_n160_), .c(new_n150_), .O(z16));
endmodule


