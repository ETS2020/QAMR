// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x04), .O(new_n54_));
  nor2   g010(.a(x23), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  nand2  g014(.a(x08), .b(x01), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g016(.a(x02), .O(new_n61_));
  nand2  g017(.a(new_n56_), .b(x08), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n61_), .O(z03));
  inv1   g019(.a(x03), .O(new_n64_));
  oai21  g020(.a(x23), .b(new_n54_), .c(x08), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(z04));
  nand3  g022(.a(x23), .b(x08), .c(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  inv1   g024(.a(x05), .O(new_n69_));
  nor2   g025(.a(new_n62_), .b(new_n69_), .O(z06));
  nand2  g026(.a(x08), .b(x06), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n56_), .O(z07));
  inv1   g028(.a(x07), .O(new_n73_));
  nor2   g029(.a(new_n62_), .b(new_n73_), .O(z08));
  nand2  g030(.a(x09), .b(new_n46_), .O(new_n75_));
  inv1   g031(.a(x19), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n47_), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n75_), .c(new_n57_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n56_), .O(new_n79_));
  nand2  g035(.a(x20), .b(x11), .O(new_n80_));
  nor3   g036(.a(new_n80_), .b(new_n75_), .c(x10), .O(new_n81_));
  inv1   g037(.a(x22), .O(new_n82_));
  inv1   g038(.a(x23), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g040(.a(x26), .b(x25), .c(x24), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n84_), .c(new_n81_), .d(x21), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n79_), .O(z09));
  inv1   g044(.a(x20), .O(new_n89_));
  nand3  g045(.a(x22), .b(x21), .c(x12), .O(new_n90_));
  nand4  g046(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n90_), .c(x20), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(x19), .O(new_n93_));
  oai21  g049(.a(new_n89_), .b(x19), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n59_), .c(new_n56_), .O(z10));
  inv1   g052(.a(x21), .O(new_n97_));
  nor2   g053(.a(new_n89_), .b(new_n76_), .O(new_n98_));
  nand3  g054(.a(new_n97_), .b(x20), .c(x19), .O(new_n99_));
  oai21  g055(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n101_));
  oai21  g057(.a(new_n46_), .b(new_n61_), .c(new_n101_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n56_), .O(new_n103_));
  inv1   g059(.a(new_n75_), .O(new_n104_));
  nand2  g060(.a(x21), .b(x13), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(x10), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n86_), .c(new_n84_), .d(new_n104_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n103_), .O(z11));
  inv1   g064(.a(new_n98_), .O(new_n109_));
  nand2  g065(.a(x23), .b(x14), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n85_), .c(x21), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(x22), .O(new_n112_));
  nand3  g068(.a(new_n98_), .b(new_n82_), .c(x21), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n115_));
  aoi21  g071(.a(x08), .b(x03), .c(new_n55_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(z12));
  oai21  g073(.a(new_n83_), .b(x08), .c(x04), .O(new_n118_));
  nand3  g074(.a(x21), .b(x20), .c(x19), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  nand4  g076(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n120_), .c(x22), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x23), .O(new_n123_));
  nand4  g079(.a(new_n98_), .b(new_n83_), .c(x22), .d(x21), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n118_), .O(z13));
  nand2  g083(.a(x24), .b(new_n47_), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n75_), .c(new_n54_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n83_), .O(new_n130_));
  nand3  g086(.a(x26), .b(x25), .c(x16), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x22), .c(x21), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n109_), .c(x24), .O(new_n133_));
  inv1   g089(.a(x24), .O(new_n134_));
  nand4  g090(.a(new_n120_), .b(new_n134_), .c(x23), .d(x22), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n137_));
  nand2  g093(.a(x08), .b(x05), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n137_), .c(new_n130_), .O(z14));
  nand3  g095(.a(x25), .b(new_n83_), .c(new_n47_), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n75_), .c(new_n71_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n54_), .O(new_n142_));
  nand2  g098(.a(x26), .b(x17), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x24), .c(x22), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n119_), .c(x25), .O(new_n145_));
  nor2   g101(.a(x25), .b(new_n134_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n120_), .c(x22), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n71_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x23), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n142_), .O(z15));
  nand2  g108(.a(x08), .b(x07), .O(new_n153_));
  nand3  g109(.a(x26), .b(new_n83_), .c(new_n47_), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n75_), .c(new_n153_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n54_), .O(new_n156_));
  inv1   g112(.a(x18), .O(new_n157_));
  nand3  g113(.a(x20), .b(x19), .c(new_n157_), .O(new_n158_));
  nand4  g114(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n158_), .c(x26), .O(new_n160_));
  nor2   g116(.a(new_n134_), .b(new_n82_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n120_), .c(new_n45_), .d(x25), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n153_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(x23), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n156_), .O(z16));
endmodule


