// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n64_, new_n65_, new_n67_, new_n69_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x10), .O(new_n46_));
  inv1   g002(.a(x09), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x08), .O(new_n48_));
  nand3  g004(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x22), .c(x21), .d(x20), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  inv1   g011(.a(x08), .O(new_n56_));
  inv1   g012(.a(x18), .O(new_n57_));
  nor2   g013(.a(x21), .b(new_n57_), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n55_), .O(z01));
  and2   g017(.a(new_n59_), .b(x01), .O(z02));
  and2   g018(.a(new_n59_), .b(x02), .O(z03));
  inv1   g019(.a(new_n58_), .O(new_n64_));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n64_), .O(z04));
  nand2  g022(.a(new_n59_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(new_n59_), .b(x05), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z06));
  and2   g026(.a(new_n59_), .b(x06), .O(z07));
  inv1   g027(.a(x07), .O(new_n72_));
  nor2   g028(.a(new_n60_), .b(new_n72_), .O(z08));
  inv1   g029(.a(x19), .O(new_n74_));
  nand3  g030(.a(new_n48_), .b(new_n74_), .c(new_n46_), .O(new_n75_));
  oai21  g031(.a(new_n56_), .b(new_n55_), .c(new_n75_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n64_), .O(new_n77_));
  nand2  g033(.a(x20), .b(x11), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(x10), .O(new_n79_));
  inv1   g035(.a(x22), .O(new_n80_));
  inv1   g036(.a(x23), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x21), .O(new_n83_));
  nand3  g039(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n79_), .c(new_n48_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n77_), .O(z09));
  inv1   g043(.a(x20), .O(new_n88_));
  nand3  g044(.a(x22), .b(x21), .c(x12), .O(new_n89_));
  nand4  g045(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(x19), .O(new_n92_));
  oai21  g048(.a(new_n88_), .b(x19), .c(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n94_));
  aoi21  g050(.a(x08), .b(x01), .c(new_n58_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n94_), .O(z10));
  xnor2a g052(.a(x21), .b(x20), .O(new_n97_));
  nand2  g053(.a(new_n82_), .b(x13), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n84_), .c(x19), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x21), .O(new_n100_));
  oai21  g056(.a(new_n97_), .b(new_n74_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n102_));
  aoi21  g058(.a(x08), .b(x02), .c(new_n58_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(z11));
  nand2  g060(.a(new_n59_), .b(x03), .O(new_n105_));
  nand2  g061(.a(x23), .b(x14), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n84_), .c(x20), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n74_), .c(x22), .O(new_n108_));
  nand3  g064(.a(new_n80_), .b(x20), .c(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x21), .O(new_n111_));
  inv1   g067(.a(x21), .O(new_n112_));
  nand3  g068(.a(x22), .b(new_n112_), .c(new_n57_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n105_), .O(z12));
  nand4  g072(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x23), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x20), .c(x19), .O(new_n119_));
  oai21  g075(.a(new_n88_), .b(new_n74_), .c(x23), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n119_), .c(new_n80_), .O(new_n121_));
  nor2   g077(.a(new_n81_), .b(x22), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(x21), .O(new_n123_));
  nand3  g079(.a(x23), .b(new_n112_), .c(new_n57_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n67_), .O(z13));
  nor2   g083(.a(new_n88_), .b(new_n74_), .O(new_n128_));
  nand3  g084(.a(x26), .b(x25), .c(x16), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n128_), .c(x23), .d(x22), .O(new_n130_));
  inv1   g086(.a(x24), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x23), .c(x22), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  aoi22  g089(.a(new_n133_), .b(new_n128_), .c(new_n130_), .d(x24), .O(new_n134_));
  nand3  g090(.a(x24), .b(new_n112_), .c(new_n57_), .O(new_n135_));
  oai21  g091(.a(new_n134_), .b(new_n112_), .c(new_n135_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n69_), .O(z14));
  nand3  g094(.a(new_n48_), .b(x25), .c(new_n46_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n57_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n112_), .O(new_n141_));
  nand3  g097(.a(x22), .b(x20), .c(x19), .O(new_n142_));
  nand2  g098(.a(x26), .b(x17), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x24), .c(x23), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n142_), .c(x25), .O(new_n145_));
  nand3  g101(.a(x21), .b(x20), .c(x19), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  nor2   g103(.a(x25), .b(new_n131_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n147_), .c(new_n82_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n151_));
  nand2  g107(.a(x08), .b(x06), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n151_), .c(new_n141_), .O(z15));
  nand4  g109(.a(new_n45_), .b(x25), .c(x24), .d(x23), .O(new_n154_));
  oai22  g110(.a(new_n154_), .b(new_n142_), .c(new_n45_), .d(new_n57_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x21), .O(new_n156_));
  nand4  g112(.a(new_n147_), .b(new_n82_), .c(x25), .d(x24), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(x26), .c(new_n57_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  nand4  g115(.a(new_n159_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n160_));
  oai21  g116(.a(new_n60_), .b(new_n72_), .c(new_n160_), .O(z16));
endmodule


