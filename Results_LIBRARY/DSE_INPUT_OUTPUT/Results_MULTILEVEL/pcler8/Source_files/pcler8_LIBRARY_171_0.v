// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n61_, new_n63_,
    new_n65_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x23), .b(x17), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  and2   g011(.a(new_n55_), .b(x00), .O(z01));
  inv1   g012(.a(new_n54_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z02));
  and2   g015(.a(new_n55_), .b(x02), .O(z03));
  nand2  g016(.a(x08), .b(x03), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n57_), .O(z04));
  nand2  g018(.a(new_n55_), .b(x04), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z05));
  nand2  g020(.a(x08), .b(x05), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n57_), .O(z06));
  and2   g022(.a(new_n55_), .b(x06), .O(z07));
  nand2  g023(.a(x08), .b(x07), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n57_), .O(z08));
  inv1   g025(.a(x21), .O(new_n70_));
  inv1   g026(.a(x22), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g028(.a(new_n72_), .b(x20), .c(x11), .O(new_n73_));
  inv1   g029(.a(x23), .O(new_n74_));
  inv1   g030(.a(x24), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x25), .O(new_n77_));
  nor2   g033(.a(new_n45_), .b(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n73_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n81_));
  aoi21  g037(.a(x08), .b(x00), .c(new_n54_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(z09));
  xor2a  g039(.a(x20), .b(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n58_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n57_), .O(new_n87_));
  inv1   g043(.a(x09), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(x08), .O(new_n89_));
  nand2  g045(.a(x19), .b(x12), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(x10), .O(new_n91_));
  nor2   g047(.a(new_n74_), .b(new_n71_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand3  g049(.a(x26), .b(x25), .c(x24), .O(new_n94_));
  nor3   g050(.a(new_n94_), .b(new_n93_), .c(new_n70_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n91_), .c(new_n89_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n87_), .O(z10));
  nand3  g053(.a(x23), .b(x22), .c(x13), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n94_), .c(x21), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(x20), .c(x19), .O(new_n100_));
  nand2  g056(.a(x20), .b(x19), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n70_), .c(new_n100_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n104_));
  aoi21  g060(.a(x08), .b(x02), .c(new_n54_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n104_), .O(z11));
  nand3  g062(.a(x21), .b(x20), .c(x19), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  nand2  g064(.a(new_n71_), .b(x21), .O(new_n109_));
  oai22  g065(.a(new_n109_), .b(new_n101_), .c(new_n108_), .d(new_n71_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n61_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n57_), .O(new_n113_));
  nor2   g069(.a(new_n94_), .b(new_n93_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n89_), .c(x14), .d(new_n47_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n113_), .O(z12));
  nand3  g072(.a(new_n78_), .b(x24), .c(x15), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n108_), .c(x22), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x23), .O(new_n119_));
  nor2   g075(.a(x23), .b(new_n71_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n102_), .c(x21), .d(x17), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n63_), .O(z13));
  nand2  g080(.a(new_n55_), .b(x05), .O(new_n125_));
  nand3  g081(.a(x26), .b(x25), .c(x16), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n102_), .c(x22), .d(x21), .O(new_n127_));
  nand3  g083(.a(new_n75_), .b(x22), .c(x21), .O(new_n128_));
  nor2   g084(.a(new_n128_), .b(new_n101_), .O(new_n129_));
  aoi21  g085(.a(new_n127_), .b(x24), .c(new_n129_), .O(new_n130_));
  nand3  g086(.a(x24), .b(new_n74_), .c(x17), .O(new_n131_));
  oai21  g087(.a(new_n130_), .b(new_n74_), .c(new_n131_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n125_), .O(z14));
  nand3  g090(.a(new_n89_), .b(x25), .c(new_n47_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x17), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n74_), .O(new_n137_));
  nand2  g093(.a(x26), .b(x17), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x24), .c(x22), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n107_), .c(x25), .O(new_n140_));
  nand4  g096(.a(new_n108_), .b(new_n92_), .c(new_n77_), .d(x24), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n143_));
  nand2  g099(.a(x08), .b(x06), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n143_), .c(new_n137_), .O(z15));
  nand3  g101(.a(new_n89_), .b(x26), .c(new_n47_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x17), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n74_), .O(new_n148_));
  inv1   g104(.a(x18), .O(new_n149_));
  nand2  g105(.a(new_n102_), .b(new_n149_), .O(new_n150_));
  nand3  g106(.a(new_n72_), .b(x25), .c(x24), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n150_), .c(x26), .O(new_n152_));
  nor2   g108(.a(x26), .b(new_n77_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n102_), .c(new_n76_), .d(new_n72_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n148_), .c(new_n68_), .O(z16));
endmodule


