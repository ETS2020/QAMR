// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  nor2   g000(.a(x23), .b(x22), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x21), .O(new_n52_));
  inv1   g008(.a(x22), .O(new_n53_));
  inv1   g009(.a(x23), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  nor4   g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n56_));
  nand4  g012(.a(new_n56_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n46_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  inv1   g015(.a(x08), .O(new_n60_));
  nor2   g016(.a(new_n45_), .b(new_n60_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g019(.a(x01), .O(new_n64_));
  nor2   g020(.a(new_n62_), .b(new_n64_), .O(z02));
  and2   g021(.a(new_n61_), .b(x02), .O(z03));
  nand2  g022(.a(new_n61_), .b(x03), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z04));
  inv1   g024(.a(x04), .O(new_n69_));
  oai21  g025(.a(new_n60_), .b(new_n69_), .c(new_n46_), .O(z05));
  nand2  g026(.a(x08), .b(x05), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n46_), .O(z06));
  nand2  g028(.a(x08), .b(x06), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n46_), .O(z07));
  nand2  g030(.a(x08), .b(x07), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n46_), .O(z08));
  inv1   g032(.a(x19), .O(new_n77_));
  nand3  g033(.a(new_n49_), .b(new_n77_), .c(new_n47_), .O(new_n78_));
  oai21  g034(.a(new_n60_), .b(new_n59_), .c(new_n78_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g036(.a(x20), .b(x11), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(x10), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n56_), .c(new_n49_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n80_), .O(z09));
  xor2a  g040(.a(x20), .b(x19), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n60_), .O(new_n86_));
  oai21  g042(.a(new_n60_), .b(new_n64_), .c(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n46_), .O(new_n88_));
  nand2  g044(.a(x19), .b(x12), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(x10), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n56_), .c(new_n49_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n88_), .O(z10));
  xnor2a g048(.a(x21), .b(x20), .O(new_n93_));
  nand2  g049(.a(x21), .b(new_n77_), .O(new_n94_));
  oai21  g050(.a(new_n93_), .b(new_n77_), .c(new_n94_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n47_), .c(x09), .d(new_n60_), .O(new_n96_));
  nand2  g052(.a(x08), .b(x02), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n46_), .O(new_n99_));
  nand2  g055(.a(x19), .b(x13), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(x10), .O(new_n101_));
  nand3  g057(.a(x23), .b(x22), .c(x20), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n55_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n101_), .c(new_n49_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n99_), .O(z11));
  nor2   g061(.a(x22), .b(new_n52_), .O(new_n106_));
  nand2  g062(.a(x22), .b(x14), .O(new_n107_));
  nor2   g063(.a(new_n107_), .b(new_n55_), .O(new_n108_));
  aoi21  g064(.a(new_n106_), .b(new_n51_), .c(new_n108_), .O(new_n109_));
  nand3  g065(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x22), .O(new_n111_));
  oai21  g067(.a(new_n109_), .b(new_n54_), .c(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n47_), .c(x09), .d(new_n60_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n67_), .O(z12));
  inv1   g070(.a(new_n110_), .O(new_n115_));
  nand4  g071(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n115_), .c(x22), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x23), .O(new_n118_));
  nand4  g074(.a(new_n51_), .b(new_n54_), .c(x22), .d(x21), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n47_), .c(x09), .d(new_n60_), .O(new_n121_));
  oai21  g077(.a(new_n62_), .b(new_n69_), .c(new_n121_), .O(z13));
  nand2  g078(.a(new_n61_), .b(x05), .O(new_n123_));
  nand2  g079(.a(x24), .b(new_n53_), .O(new_n124_));
  inv1   g080(.a(x24), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x22), .c(x21), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n50_), .c(new_n124_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x23), .O(new_n128_));
  nand3  g084(.a(x26), .b(x25), .c(x16), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n51_), .c(x23), .d(x21), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x24), .c(x22), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n47_), .c(x09), .d(new_n60_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n123_), .O(z14));
  nand2  g090(.a(new_n61_), .b(x06), .O(new_n135_));
  nand2  g091(.a(x25), .b(new_n53_), .O(new_n136_));
  inv1   g092(.a(x25), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x24), .c(x22), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n110_), .c(new_n136_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x23), .O(new_n140_));
  nand2  g096(.a(x26), .b(x17), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n115_), .c(x24), .d(x23), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(x25), .c(x22), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n47_), .c(x09), .d(new_n60_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n135_), .O(z15));
  nand3  g102(.a(new_n49_), .b(x26), .c(new_n47_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x23), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n53_), .O(new_n149_));
  inv1   g105(.a(x18), .O(new_n150_));
  nand2  g106(.a(new_n51_), .b(new_n150_), .O(new_n151_));
  nand4  g107(.a(x25), .b(x24), .c(x23), .d(x21), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n151_), .c(x26), .O(new_n153_));
  nor2   g109(.a(new_n53_), .b(new_n52_), .O(new_n154_));
  nor2   g110(.a(new_n125_), .b(new_n54_), .O(new_n155_));
  nor2   g111(.a(x26), .b(new_n137_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n155_), .c(new_n154_), .d(new_n51_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n47_), .c(x09), .d(new_n60_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n149_), .c(new_n75_), .O(z16));
endmodule


