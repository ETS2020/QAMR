// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x10), .O(new_n46_));
  inv1   g002(.a(x09), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x08), .O(new_n48_));
  nand3  g004(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x22), .c(x21), .d(x20), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x21), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x00), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z01));
  inv1   g015(.a(x01), .O(new_n60_));
  inv1   g016(.a(x08), .O(new_n61_));
  nor3   g017(.a(new_n56_), .b(new_n61_), .c(new_n60_), .O(z02));
  nor2   g018(.a(new_n56_), .b(new_n61_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x02), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n57_), .O(z04));
  nand2  g023(.a(x08), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n57_), .O(z05));
  nand2  g025(.a(x08), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n57_), .O(z06));
  nand2  g027(.a(new_n63_), .b(x06), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z07));
  and2   g029(.a(new_n63_), .b(x07), .O(z08));
  inv1   g030(.a(x20), .O(new_n75_));
  inv1   g031(.a(x22), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(x11), .O(new_n78_));
  inv1   g034(.a(x25), .O(new_n79_));
  nor2   g035(.a(new_n45_), .b(new_n79_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x24), .c(x23), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n78_), .c(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n58_), .c(new_n57_), .O(z09));
  xor2a  g040(.a(x20), .b(x19), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n86_));
  oai21  g042(.a(new_n61_), .b(new_n60_), .c(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n57_), .O(new_n88_));
  nand2  g044(.a(x19), .b(x12), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(x10), .O(new_n90_));
  nor2   g046(.a(new_n55_), .b(new_n76_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(x21), .O(new_n92_));
  nand3  g048(.a(x26), .b(x25), .c(x24), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n90_), .c(new_n48_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n88_), .O(z10));
  inv1   g052(.a(x21), .O(new_n97_));
  nand3  g053(.a(new_n55_), .b(new_n97_), .c(x20), .O(new_n98_));
  oai21  g054(.a(new_n97_), .b(x20), .c(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x19), .O(new_n100_));
  nand2  g056(.a(new_n91_), .b(x13), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n93_), .c(x19), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(x21), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n64_), .O(z11));
  nand2  g062(.a(new_n63_), .b(x03), .O(new_n107_));
  inv1   g063(.a(x19), .O(new_n108_));
  nand2  g064(.a(x23), .b(x14), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n93_), .c(x20), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n108_), .c(x22), .O(new_n111_));
  nand3  g067(.a(new_n76_), .b(x20), .c(x19), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x21), .O(new_n114_));
  nand3  g070(.a(new_n55_), .b(x22), .c(new_n97_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n107_), .O(z12));
  nand4  g074(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x20), .c(x19), .O(new_n121_));
  nor2   g077(.a(new_n75_), .b(new_n108_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n55_), .c(new_n121_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x22), .c(x21), .O(new_n124_));
  nand2  g080(.a(x23), .b(new_n76_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n68_), .c(new_n57_), .O(z13));
  nand3  g084(.a(new_n46_), .b(x09), .c(new_n61_), .O(new_n129_));
  inv1   g085(.a(x24), .O(new_n130_));
  nand3  g086(.a(new_n122_), .b(new_n130_), .c(x22), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n129_), .c(x21), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x23), .O(new_n133_));
  nand2  g089(.a(new_n80_), .b(x16), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x23), .c(x22), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n122_), .c(new_n130_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n133_), .c(new_n70_), .O(z14));
  aoi21  g095(.a(x26), .b(x17), .c(new_n130_), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n122_), .c(x22), .O(new_n141_));
  nand3  g097(.a(x22), .b(x20), .c(x19), .O(new_n142_));
  nand3  g098(.a(new_n79_), .b(x24), .c(x23), .O(new_n143_));
  nor2   g099(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  aoi21  g100(.a(new_n141_), .b(x25), .c(new_n144_), .O(new_n145_));
  oai22  g101(.a(new_n145_), .b(new_n97_), .c(new_n79_), .d(x23), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n72_), .O(z15));
  nand3  g104(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n149_));
  nand4  g105(.a(new_n77_), .b(new_n45_), .c(x25), .d(x24), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n149_), .c(x21), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x23), .O(new_n152_));
  nor3   g108(.a(new_n75_), .b(new_n108_), .c(x18), .O(new_n153_));
  nand3  g109(.a(new_n91_), .b(x25), .c(x24), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n153_), .c(new_n45_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n46_), .c(x09), .d(new_n61_), .O(new_n157_));
  nand2  g113(.a(x08), .b(x07), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n157_), .c(new_n152_), .O(z16));
endmodule


