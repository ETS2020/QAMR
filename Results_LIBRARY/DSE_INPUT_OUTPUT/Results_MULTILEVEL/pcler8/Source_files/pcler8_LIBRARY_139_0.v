// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  nor2   g000(.a(x23), .b(x20), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(x21), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g013(.a(new_n57_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n46_), .O(z00));
  nand2  g015(.a(x08), .b(x00), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n46_), .O(z01));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n46_), .O(z02));
  inv1   g019(.a(x08), .O(new_n64_));
  nor2   g020(.a(new_n45_), .b(new_n64_), .O(new_n65_));
  and2   g021(.a(new_n65_), .b(x02), .O(z03));
  nand2  g022(.a(x08), .b(x03), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n46_), .O(z04));
  nand2  g024(.a(x08), .b(x04), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n46_), .O(z05));
  nand2  g026(.a(x08), .b(x05), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n46_), .O(z06));
  and2   g028(.a(new_n65_), .b(x06), .O(z07));
  nand2  g029(.a(x08), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n46_), .O(z08));
  inv1   g031(.a(x19), .O(new_n76_));
  nand3  g032(.a(new_n49_), .b(new_n76_), .c(new_n47_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n60_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g035(.a(x20), .b(x11), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x10), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n57_), .c(new_n49_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n79_), .O(z09));
  nand2  g039(.a(new_n65_), .b(x01), .O(new_n84_));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  nand4  g041(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n85_), .c(x19), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x20), .O(new_n88_));
  inv1   g044(.a(x20), .O(new_n89_));
  nand3  g045(.a(x23), .b(new_n89_), .c(x19), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n47_), .c(x09), .d(new_n64_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n84_), .O(z10));
  nand3  g049(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n56_), .c(x21), .O(new_n95_));
  inv1   g051(.a(x21), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(x19), .O(new_n97_));
  aoi21  g053(.a(new_n95_), .b(x19), .c(new_n97_), .O(new_n98_));
  nand2  g054(.a(x21), .b(new_n89_), .O(new_n99_));
  oai21  g055(.a(new_n98_), .b(new_n89_), .c(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n47_), .c(x09), .d(new_n64_), .O(new_n101_));
  aoi21  g057(.a(x08), .b(x02), .c(new_n45_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n101_), .O(z11));
  nand3  g059(.a(new_n49_), .b(x22), .c(new_n47_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(x23), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n89_), .O(new_n106_));
  nand2  g062(.a(x23), .b(x14), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n56_), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n76_), .c(x22), .O(new_n109_));
  nand3  g065(.a(new_n51_), .b(new_n52_), .c(x21), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n47_), .c(x09), .d(new_n64_), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n106_), .c(new_n67_), .O(z12));
  nand3  g069(.a(new_n47_), .b(x09), .c(new_n64_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x23), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n89_), .O(new_n116_));
  nor2   g072(.a(new_n96_), .b(new_n76_), .O(new_n117_));
  nand4  g073(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n117_), .c(x22), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand3  g076(.a(new_n117_), .b(new_n53_), .c(x22), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n47_), .c(x09), .d(new_n64_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n116_), .c(new_n69_), .O(z13));
  nand2  g080(.a(new_n65_), .b(x05), .O(new_n125_));
  nand2  g081(.a(x24), .b(new_n89_), .O(new_n126_));
  inv1   g082(.a(x24), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x22), .c(x21), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n50_), .c(new_n126_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x23), .O(new_n130_));
  nand3  g086(.a(x26), .b(x25), .c(x16), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n117_), .c(x23), .d(x22), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x24), .c(x20), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n47_), .c(x09), .d(new_n64_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n125_), .O(z14));
  nand3  g092(.a(new_n49_), .b(x25), .c(new_n47_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x23), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n89_), .O(new_n139_));
  nand3  g095(.a(x22), .b(x21), .c(x19), .O(new_n140_));
  nand2  g096(.a(x26), .b(x17), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(x24), .c(x23), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x25), .O(new_n143_));
  nand3  g099(.a(x21), .b(x20), .c(x19), .O(new_n144_));
  inv1   g100(.a(x25), .O(new_n145_));
  nand3  g101(.a(new_n54_), .b(new_n145_), .c(x24), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n144_), .c(new_n143_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n47_), .c(x09), .d(new_n64_), .O(new_n148_));
  nand2  g104(.a(x08), .b(x06), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n148_), .c(new_n139_), .O(z15));
  nand2  g106(.a(new_n65_), .b(x07), .O(new_n151_));
  nand2  g107(.a(x26), .b(new_n89_), .O(new_n152_));
  inv1   g108(.a(x26), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(x25), .c(x24), .d(x22), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n144_), .c(new_n152_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x23), .O(new_n156_));
  inv1   g112(.a(x18), .O(new_n157_));
  nor2   g113(.a(new_n145_), .b(new_n127_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n117_), .c(new_n54_), .d(new_n157_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(x26), .c(x20), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n47_), .c(x09), .d(new_n64_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n151_), .O(z16));
endmodule


