// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nor2   g010(.a(x23), .b(x22), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n57_), .b(new_n59_), .O(z02));
  inv1   g016(.a(new_n55_), .O(new_n61_));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n61_), .O(z04));
  and2   g021(.a(new_n56_), .b(x04), .O(z05));
  nand2  g022(.a(x08), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n61_), .O(z06));
  nand2  g024(.a(x08), .b(x06), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n61_), .O(z07));
  nand2  g026(.a(new_n56_), .b(x07), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z08));
  inv1   g028(.a(x09), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(x08), .O(new_n74_));
  nor2   g030(.a(x19), .b(x10), .O(new_n75_));
  aoi22  g031(.a(new_n75_), .b(new_n74_), .c(x08), .d(x00), .O(new_n76_));
  nand2  g032(.a(x20), .b(x11), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(x10), .O(new_n78_));
  inv1   g034(.a(x22), .O(new_n79_));
  inv1   g035(.a(x23), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(x21), .O(new_n82_));
  nand3  g038(.a(x26), .b(x25), .c(x24), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n78_), .c(new_n74_), .O(new_n85_));
  oai21  g041(.a(new_n76_), .b(new_n55_), .c(new_n85_), .O(z09));
  xor2a  g042(.a(x20), .b(x19), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n88_));
  oai21  g044(.a(new_n46_), .b(new_n59_), .c(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n61_), .O(new_n90_));
  nand2  g046(.a(x19), .b(x12), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(x10), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n84_), .c(new_n74_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n90_), .O(z10));
  inv1   g050(.a(x21), .O(new_n95_));
  nand3  g051(.a(x23), .b(x22), .c(x13), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n83_), .c(x21), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(x20), .c(x19), .O(new_n98_));
  nand2  g054(.a(x20), .b(x19), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n95_), .c(new_n98_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n62_), .c(new_n61_), .O(z11));
  nand2  g059(.a(new_n56_), .b(x03), .O(new_n104_));
  inv1   g060(.a(new_n83_), .O(new_n105_));
  nor2   g061(.a(x22), .b(new_n95_), .O(new_n106_));
  and2   g062(.a(x22), .b(x14), .O(new_n107_));
  aoi22  g063(.a(new_n107_), .b(new_n105_), .c(new_n106_), .d(new_n100_), .O(new_n108_));
  nand3  g064(.a(x21), .b(x20), .c(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x22), .O(new_n110_));
  oai21  g066(.a(new_n108_), .b(new_n80_), .c(new_n110_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n104_), .O(z12));
  nand3  g069(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x23), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n79_), .O(new_n116_));
  nand4  g072(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n110_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x23), .O(new_n119_));
  nand3  g075(.a(new_n100_), .b(new_n80_), .c(x21), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n122_));
  nand2  g078(.a(x08), .b(x04), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n122_), .c(new_n116_), .O(z13));
  nand2  g080(.a(new_n56_), .b(x05), .O(new_n125_));
  nand2  g081(.a(x24), .b(new_n79_), .O(new_n126_));
  inv1   g082(.a(x24), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x22), .c(x21), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n99_), .c(new_n126_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x23), .O(new_n130_));
  nand3  g086(.a(x26), .b(x25), .c(x16), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n100_), .c(x23), .d(x21), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x24), .c(x22), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n125_), .O(z14));
  nand3  g092(.a(new_n74_), .b(x25), .c(new_n47_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x23), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n79_), .O(new_n139_));
  nand2  g095(.a(x26), .b(x17), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(x24), .c(x23), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n109_), .c(x25), .O(new_n142_));
  inv1   g098(.a(x25), .O(new_n143_));
  nand3  g099(.a(new_n81_), .b(new_n143_), .c(x24), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n109_), .c(new_n142_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n139_), .c(new_n69_), .O(z15));
  nand2  g103(.a(x26), .b(new_n79_), .O(new_n148_));
  nand4  g104(.a(new_n45_), .b(x25), .c(x24), .d(x22), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n109_), .c(new_n148_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x23), .O(new_n151_));
  nor2   g107(.a(new_n99_), .b(x18), .O(new_n152_));
  nor2   g108(.a(new_n143_), .b(new_n127_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n152_), .c(x23), .d(x21), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x26), .c(x22), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n71_), .O(z16));
endmodule


