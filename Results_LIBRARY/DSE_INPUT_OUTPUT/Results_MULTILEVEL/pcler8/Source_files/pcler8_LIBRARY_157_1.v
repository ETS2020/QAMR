// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x12), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x12), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  and2   g013(.a(new_n57_), .b(x00), .O(z01));
  inv1   g014(.a(new_n56_), .O(new_n59_));
  nand2  g015(.a(x08), .b(x01), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(z02));
  and2   g017(.a(new_n57_), .b(x02), .O(z03));
  and2   g018(.a(new_n57_), .b(x03), .O(z04));
  aoi21  g019(.a(x08), .b(x04), .c(new_n56_), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z05));
  nand2  g021(.a(new_n57_), .b(x05), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z06));
  and2   g023(.a(new_n57_), .b(x06), .O(z07));
  and2   g024(.a(new_n57_), .b(x07), .O(z08));
  inv1   g025(.a(x21), .O(new_n70_));
  nor2   g026(.a(new_n46_), .b(new_n70_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(x20), .c(x11), .O(new_n72_));
  inv1   g028(.a(x25), .O(new_n73_));
  nor2   g029(.a(new_n45_), .b(new_n73_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(x24), .c(x23), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n77_));
  aoi21  g033(.a(x08), .b(x00), .c(new_n56_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n77_), .O(z09));
  inv1   g035(.a(x19), .O(new_n80_));
  nand3  g036(.a(x23), .b(x22), .c(x21), .O(new_n81_));
  nand3  g037(.a(x26), .b(x25), .c(x24), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n81_), .c(x19), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x20), .O(new_n84_));
  oai21  g040(.a(x20), .b(new_n80_), .c(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n60_), .c(new_n59_), .O(z10));
  inv1   g043(.a(x09), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(x08), .O(new_n89_));
  inv1   g045(.a(new_n82_), .O(new_n90_));
  nand2  g046(.a(x20), .b(x13), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(x10), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n90_), .c(new_n71_), .d(new_n89_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x23), .O(new_n95_));
  inv1   g051(.a(x20), .O(new_n96_));
  xnor2a g052(.a(x21), .b(x19), .O(new_n97_));
  nand2  g053(.a(x21), .b(new_n96_), .O(new_n98_));
  oai21  g054(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n100_));
  nand2  g056(.a(x08), .b(x02), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n100_), .c(new_n95_), .O(z11));
  nand3  g058(.a(new_n89_), .b(x14), .c(new_n48_), .O(new_n103_));
  nand3  g059(.a(new_n74_), .b(x24), .c(x22), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n103_), .c(x12), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x23), .O(new_n106_));
  nand3  g062(.a(x21), .b(x20), .c(x19), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  and2   g064(.a(x20), .b(x19), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n46_), .c(x21), .O(new_n110_));
  oai21  g066(.a(new_n108_), .b(new_n46_), .c(new_n110_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n112_));
  nand2  g068(.a(x08), .b(x03), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n112_), .c(new_n106_), .O(z12));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x23), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x21), .c(x19), .O(new_n117_));
  oai21  g073(.a(new_n70_), .b(new_n80_), .c(x23), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x22), .c(x20), .O(new_n120_));
  oai21  g076(.a(new_n46_), .b(new_n96_), .c(x23), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n64_), .O(z13));
  inv1   g080(.a(x12), .O(new_n125_));
  nand3  g081(.a(x26), .b(x25), .c(x16), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n109_), .c(x22), .d(x21), .O(new_n127_));
  inv1   g083(.a(x24), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x22), .O(new_n129_));
  nor2   g085(.a(new_n129_), .b(new_n107_), .O(new_n130_));
  aoi21  g086(.a(new_n127_), .b(x24), .c(new_n130_), .O(new_n131_));
  nand2  g087(.a(x24), .b(new_n55_), .O(new_n132_));
  oai21  g088(.a(new_n131_), .b(new_n125_), .c(new_n132_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n66_), .O(z14));
  nand3  g091(.a(new_n89_), .b(x19), .c(new_n48_), .O(new_n136_));
  nor2   g092(.a(new_n70_), .b(new_n96_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n73_), .c(x24), .d(x22), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x12), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x23), .O(new_n140_));
  nor2   g096(.a(new_n55_), .b(new_n46_), .O(new_n141_));
  nand2  g097(.a(x26), .b(x17), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n141_), .c(x24), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n108_), .c(new_n73_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n146_));
  nand2  g102(.a(x08), .b(x06), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n146_), .c(new_n140_), .O(z15));
  nand3  g104(.a(new_n109_), .b(new_n89_), .c(new_n48_), .O(new_n149_));
  nand4  g105(.a(new_n71_), .b(new_n45_), .c(x25), .d(x24), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n149_), .c(x12), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x23), .O(new_n152_));
  nor2   g108(.a(new_n80_), .b(x18), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n137_), .O(new_n154_));
  nand3  g110(.a(new_n141_), .b(x25), .c(x24), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nor2   g112(.a(new_n156_), .b(new_n45_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n158_));
  nand2  g114(.a(x08), .b(x07), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n158_), .c(new_n152_), .O(z16));
endmodule


