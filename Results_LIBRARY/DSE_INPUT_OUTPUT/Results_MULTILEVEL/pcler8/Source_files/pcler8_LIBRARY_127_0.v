// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  nor2   g000(.a(x23), .b(x22), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(new_n52_));
  inv1   g008(.a(x22), .O(new_n53_));
  inv1   g009(.a(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n55_), .c(x21), .O(new_n58_));
  oai21  g014(.a(new_n58_), .b(new_n52_), .c(new_n46_), .O(z00));
  inv1   g015(.a(x00), .O(new_n60_));
  nand2  g016(.a(new_n46_), .b(x08), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n60_), .O(z01));
  nand2  g018(.a(x08), .b(x01), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n46_), .O(z02));
  inv1   g020(.a(x02), .O(new_n65_));
  nor2   g021(.a(new_n61_), .b(new_n65_), .O(z03));
  nand2  g022(.a(x08), .b(x03), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n46_), .O(z04));
  inv1   g024(.a(x04), .O(new_n69_));
  nor2   g025(.a(new_n61_), .b(new_n69_), .O(z05));
  nand2  g026(.a(x08), .b(x05), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n46_), .O(z06));
  nand2  g028(.a(x08), .b(x06), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n46_), .O(z07));
  inv1   g030(.a(x07), .O(new_n75_));
  nor2   g031(.a(new_n61_), .b(new_n75_), .O(z08));
  inv1   g032(.a(x08), .O(new_n77_));
  inv1   g033(.a(x21), .O(new_n78_));
  nor2   g034(.a(new_n53_), .b(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  nand4  g036(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n80_), .c(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n83_));
  aoi21  g039(.a(x08), .b(x00), .c(new_n45_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(z09));
  nand3  g041(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n81_), .c(x20), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x19), .O(new_n88_));
  inv1   g044(.a(x19), .O(new_n89_));
  nand2  g045(.a(x20), .b(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n63_), .c(new_n46_), .O(z10));
  nand3  g049(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n56_), .c(x21), .O(new_n95_));
  nor2   g051(.a(new_n78_), .b(x20), .O(new_n96_));
  aoi21  g052(.a(new_n95_), .b(x20), .c(new_n96_), .O(new_n97_));
  nand2  g053(.a(x21), .b(new_n89_), .O(new_n98_));
  oai21  g054(.a(new_n97_), .b(new_n89_), .c(new_n98_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n100_));
  aoi21  g056(.a(x08), .b(x02), .c(new_n45_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n100_), .O(z11));
  nand3  g058(.a(x21), .b(x20), .c(x19), .O(new_n103_));
  oai21  g059(.a(new_n54_), .b(x22), .c(new_n103_), .O(new_n104_));
  nand3  g060(.a(new_n51_), .b(new_n53_), .c(x21), .O(new_n105_));
  nand3  g061(.a(new_n57_), .b(new_n55_), .c(x14), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n67_), .c(new_n46_), .O(z12));
  nand3  g065(.a(new_n47_), .b(x09), .c(new_n77_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x23), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n53_), .O(new_n112_));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  aoi21  g070(.a(new_n103_), .b(x22), .c(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n51_), .b(new_n54_), .c(x21), .O(new_n116_));
  oai21  g072(.a(new_n115_), .b(new_n54_), .c(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n118_));
  nand2  g074(.a(x08), .b(x04), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n118_), .c(new_n112_), .O(z13));
  nand3  g076(.a(new_n49_), .b(x24), .c(new_n47_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x23), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n53_), .O(new_n123_));
  nand3  g079(.a(x26), .b(x25), .c(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(x21), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n50_), .c(x24), .O(new_n126_));
  inv1   g082(.a(x24), .O(new_n127_));
  inv1   g083(.a(new_n103_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n127_), .c(x23), .d(x22), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n123_), .c(new_n71_), .O(z14));
  nand3  g088(.a(new_n49_), .b(x25), .c(new_n47_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x23), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n53_), .O(new_n135_));
  nand2  g091(.a(x26), .b(x17), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x24), .c(x23), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n103_), .c(x25), .O(new_n138_));
  inv1   g094(.a(x25), .O(new_n139_));
  nand4  g095(.a(new_n128_), .b(new_n55_), .c(new_n139_), .d(x24), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n135_), .c(new_n73_), .O(z15));
  nand3  g099(.a(new_n49_), .b(x26), .c(new_n47_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x23), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n53_), .O(new_n146_));
  inv1   g102(.a(x18), .O(new_n147_));
  nand2  g103(.a(new_n51_), .b(new_n147_), .O(new_n148_));
  nand4  g104(.a(x25), .b(x24), .c(x23), .d(x21), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n148_), .c(x26), .O(new_n150_));
  nor2   g106(.a(new_n127_), .b(new_n54_), .O(new_n151_));
  nor2   g107(.a(x26), .b(new_n139_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n151_), .c(new_n79_), .d(new_n51_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n47_), .c(x09), .d(new_n77_), .O(new_n155_));
  nand2  g111(.a(x08), .b(x07), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n155_), .c(new_n146_), .O(z16));
endmodule


