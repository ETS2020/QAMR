// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  nor2   g000(.a(x25), .b(x24), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(x21), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g013(.a(new_n57_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n46_), .O(z00));
  nand2  g015(.a(x08), .b(x00), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n46_), .O(z01));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n46_), .O(z02));
  nand2  g019(.a(x08), .b(x02), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n46_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n46_), .O(z04));
  nand2  g023(.a(x08), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n46_), .O(z05));
  nand2  g025(.a(x08), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n46_), .O(z06));
  inv1   g027(.a(x08), .O(new_n72_));
  nor2   g028(.a(new_n45_), .b(new_n72_), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(x06), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(z07));
  nand2  g031(.a(x08), .b(x07), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n46_), .O(z08));
  inv1   g033(.a(x19), .O(new_n78_));
  nand3  g034(.a(new_n49_), .b(new_n78_), .c(new_n47_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n60_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n46_), .O(new_n81_));
  nand2  g037(.a(x20), .b(x11), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(x10), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n57_), .c(new_n49_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n81_), .O(z09));
  xor2a  g041(.a(x20), .b(x19), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n47_), .c(x09), .d(new_n72_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n46_), .O(new_n89_));
  nand2  g045(.a(x19), .b(x12), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(x10), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n57_), .c(new_n49_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n89_), .O(z10));
  nand3  g049(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n56_), .c(x20), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n78_), .c(x21), .O(new_n96_));
  inv1   g052(.a(x21), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(x20), .c(x19), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n47_), .c(x09), .d(new_n72_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n64_), .c(new_n46_), .O(z11));
  nand2  g057(.a(x23), .b(x14), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n56_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n50_), .c(x22), .O(new_n104_));
  nand3  g060(.a(new_n51_), .b(new_n52_), .c(x21), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n47_), .c(x09), .d(new_n72_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n66_), .c(new_n46_), .O(z12));
  nand4  g064(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x23), .O(new_n110_));
  nand3  g066(.a(new_n53_), .b(x22), .c(x21), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n50_), .c(new_n110_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n47_), .c(x09), .d(new_n72_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n68_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n46_), .O(new_n115_));
  nand4  g071(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n49_), .c(x15), .d(new_n47_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n115_), .O(z13));
  nand2  g075(.a(new_n73_), .b(x05), .O(new_n120_));
  nand3  g076(.a(x21), .b(x20), .c(x19), .O(new_n121_));
  inv1   g077(.a(x24), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x23), .c(x22), .O(new_n123_));
  nand3  g079(.a(x26), .b(x24), .c(x16), .O(new_n124_));
  oai21  g080(.a(new_n123_), .b(new_n121_), .c(new_n124_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x25), .O(new_n126_));
  oai21  g082(.a(new_n55_), .b(new_n50_), .c(x24), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n47_), .c(x09), .d(new_n72_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n120_), .O(z14));
  nand2  g086(.a(x26), .b(x17), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n54_), .c(x24), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n121_), .c(x25), .O(new_n133_));
  inv1   g089(.a(x25), .O(new_n134_));
  inv1   g090(.a(new_n121_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n54_), .c(new_n134_), .d(x24), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n72_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n74_), .O(z15));
  nand2  g095(.a(new_n73_), .b(x07), .O(new_n140_));
  nand2  g096(.a(x26), .b(new_n122_), .O(new_n141_));
  inv1   g097(.a(x26), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(x24), .c(x23), .d(x22), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n121_), .c(new_n141_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x25), .O(new_n145_));
  nor2   g101(.a(new_n50_), .b(x18), .O(new_n146_));
  nor2   g102(.a(new_n134_), .b(new_n53_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n146_), .c(x22), .d(x21), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x26), .c(x24), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n47_), .c(x09), .d(new_n72_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n140_), .O(z16));
endmodule


