// Benchmark "FAU" written by ABC on Fri Aug 14 02:19:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(new_n48_), .b(x09), .c(new_n47_), .d(x06), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x06), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  and2   g013(.a(new_n57_), .b(x00), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  inv1   g015(.a(new_n57_), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n59_), .O(z02));
  inv1   g017(.a(x02), .O(new_n62_));
  nor2   g018(.a(new_n60_), .b(new_n62_), .O(z03));
  inv1   g019(.a(new_n56_), .O(new_n64_));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n64_), .O(z04));
  inv1   g022(.a(x04), .O(new_n67_));
  oai21  g023(.a(new_n47_), .b(new_n67_), .c(new_n64_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n64_), .O(z06));
  inv1   g026(.a(x06), .O(new_n71_));
  oai21  g027(.a(new_n47_), .b(new_n71_), .c(new_n64_), .O(z07));
  nand2  g028(.a(x08), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n64_), .O(z08));
  and2   g030(.a(x22), .b(x21), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  inv1   g032(.a(x25), .O(new_n77_));
  nor2   g033(.a(new_n45_), .b(new_n77_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(x24), .c(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n76_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n81_));
  aoi21  g037(.a(x08), .b(x00), .c(new_n56_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(z09));
  xor2a  g039(.a(x20), .b(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n85_));
  oai21  g041(.a(new_n47_), .b(new_n59_), .c(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n64_), .O(new_n87_));
  nand3  g043(.a(x09), .b(new_n47_), .c(x06), .O(new_n88_));
  nand2  g044(.a(x19), .b(x12), .O(new_n89_));
  nor3   g045(.a(new_n89_), .b(new_n88_), .c(x10), .O(new_n90_));
  nor2   g046(.a(new_n55_), .b(new_n46_), .O(new_n91_));
  nand2  g047(.a(new_n78_), .b(x24), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n91_), .c(new_n90_), .d(x21), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n87_), .O(z10));
  inv1   g051(.a(x19), .O(new_n96_));
  xnor2a g052(.a(x21), .b(x20), .O(new_n97_));
  nand2  g053(.a(x21), .b(new_n96_), .O(new_n98_));
  oai21  g054(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n100_));
  oai21  g056(.a(new_n47_), .b(new_n62_), .c(new_n100_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n64_), .O(new_n102_));
  nand2  g058(.a(x19), .b(x13), .O(new_n103_));
  nor3   g059(.a(new_n103_), .b(new_n88_), .c(x10), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n93_), .c(new_n91_), .d(x20), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n102_), .O(z11));
  nand4  g062(.a(x14), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n107_));
  nand3  g063(.a(new_n78_), .b(x24), .c(x22), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n107_), .c(x06), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x23), .O(new_n110_));
  nand3  g066(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand2  g068(.a(x20), .b(x19), .O(new_n113_));
  nand2  g069(.a(new_n46_), .b(x21), .O(new_n114_));
  oai22  g070(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(new_n46_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n110_), .c(new_n65_), .O(z12));
  inv1   g073(.a(new_n113_), .O(new_n118_));
  nand4  g074(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n118_), .c(x22), .d(x21), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x23), .c(x06), .O(new_n121_));
  nand4  g077(.a(new_n118_), .b(new_n55_), .c(x22), .d(x21), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n124_));
  oai21  g080(.a(new_n60_), .b(new_n67_), .c(new_n124_), .O(z13));
  nand2  g081(.a(new_n57_), .b(x05), .O(new_n126_));
  nand3  g082(.a(x26), .b(x25), .c(x16), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n118_), .c(x22), .d(x21), .O(new_n128_));
  inv1   g084(.a(x24), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x23), .c(x22), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n111_), .O(new_n131_));
  aoi21  g087(.a(new_n128_), .b(x24), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(x24), .b(new_n55_), .O(new_n133_));
  oai21  g089(.a(new_n132_), .b(new_n71_), .c(new_n133_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n126_), .O(z14));
  oai21  g092(.a(x23), .b(x06), .c(x08), .O(new_n137_));
  nand3  g093(.a(new_n118_), .b(new_n48_), .c(x09), .O(new_n138_));
  nand3  g094(.a(new_n75_), .b(new_n77_), .c(x24), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n138_), .c(x06), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x23), .O(new_n141_));
  nand2  g097(.a(x26), .b(x17), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n91_), .c(x24), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n112_), .c(new_n77_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n141_), .c(new_n137_), .O(z15));
  nand2  g103(.a(new_n57_), .b(x07), .O(new_n148_));
  nor2   g104(.a(new_n113_), .b(x18), .O(new_n149_));
  nand4  g105(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n149_), .c(new_n45_), .O(new_n152_));
  nand4  g108(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n153_));
  nand4  g109(.a(new_n45_), .b(x25), .c(x24), .d(x23), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n152_), .c(x06), .O(new_n156_));
  nand2  g112(.a(x26), .b(new_n55_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n148_), .O(z16));
endmodule


