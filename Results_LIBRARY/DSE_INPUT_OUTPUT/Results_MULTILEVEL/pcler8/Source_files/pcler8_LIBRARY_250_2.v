// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x07), .O(new_n47_));
  inv1   g003(.a(x08), .O(new_n48_));
  inv1   g004(.a(x10), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n45_), .O(z00));
  nor2   g011(.a(x10), .b(new_n47_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x00), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z01));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n56_), .b(new_n48_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n60_), .O(z02));
  nand2  g019(.a(new_n61_), .b(x02), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n57_), .O(z04));
  inv1   g023(.a(x04), .O(new_n68_));
  oai21  g024(.a(new_n48_), .b(new_n68_), .c(new_n57_), .O(z05));
  nand2  g025(.a(x08), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n57_), .O(z06));
  nand2  g027(.a(new_n61_), .b(x06), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z07));
  nand3  g029(.a(x10), .b(x08), .c(x07), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(z08));
  nand2  g031(.a(new_n49_), .b(x07), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x08), .c(x00), .O(new_n77_));
  inv1   g033(.a(x21), .O(new_n78_));
  nor2   g034(.a(new_n46_), .b(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  nand4  g036(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n80_), .c(x19), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x09), .c(new_n48_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n47_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n49_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n77_), .O(z09));
  nand3  g042(.a(x22), .b(x21), .c(x12), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n81_), .c(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  inv1   g045(.a(x19), .O(new_n90_));
  nand2  g046(.a(x20), .b(new_n90_), .O(new_n91_));
  aoi21  g047(.a(new_n91_), .b(new_n89_), .c(x10), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n93_));
  oai21  g049(.a(new_n62_), .b(new_n60_), .c(new_n93_), .O(z10));
  nand3  g050(.a(x23), .b(x22), .c(x13), .O(new_n95_));
  nand3  g051(.a(x26), .b(x25), .c(x24), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n95_), .c(x21), .O(new_n97_));
  nor2   g053(.a(new_n78_), .b(x20), .O(new_n98_));
  aoi21  g054(.a(new_n97_), .b(x20), .c(new_n98_), .O(new_n99_));
  nand2  g055(.a(x21), .b(new_n90_), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n90_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(x07), .c(new_n64_), .O(z11));
  inv1   g059(.a(x09), .O(new_n104_));
  nand2  g060(.a(x20), .b(x19), .O(new_n105_));
  nand2  g061(.a(x23), .b(x14), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n96_), .c(x21), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n105_), .c(x22), .O(new_n108_));
  inv1   g064(.a(new_n105_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n46_), .c(x21), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n108_), .c(new_n104_), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n48_), .c(x07), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(x10), .c(new_n66_), .O(z12));
  nand4  g069(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x22), .c(x21), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n105_), .c(x23), .O(new_n116_));
  inv1   g072(.a(x23), .O(new_n117_));
  nand4  g073(.a(new_n109_), .b(new_n117_), .c(x22), .d(x21), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n116_), .c(x10), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n120_));
  oai21  g076(.a(new_n62_), .b(new_n68_), .c(new_n120_), .O(z13));
  nand3  g077(.a(x21), .b(x20), .c(x19), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x24), .O(new_n125_));
  inv1   g081(.a(x24), .O(new_n126_));
  inv1   g082(.a(new_n122_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n126_), .c(x23), .d(x22), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n125_), .c(new_n104_), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n48_), .c(x07), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(x10), .c(new_n70_), .O(z14));
  nor2   g087(.a(new_n117_), .b(new_n46_), .O(new_n132_));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n132_), .c(x24), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n122_), .c(x25), .O(new_n135_));
  inv1   g091(.a(x25), .O(new_n136_));
  nand4  g092(.a(new_n127_), .b(new_n132_), .c(new_n136_), .d(x24), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n135_), .c(x10), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n72_), .O(z15));
  oai21  g096(.a(new_n49_), .b(x08), .c(x07), .O(new_n141_));
  inv1   g097(.a(x18), .O(new_n142_));
  nand4  g098(.a(x21), .b(x20), .c(x19), .d(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n132_), .b(x25), .c(x24), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n143_), .c(x26), .O(new_n145_));
  nor2   g101(.a(new_n126_), .b(new_n117_), .O(new_n146_));
  nor2   g102(.a(x26), .b(new_n136_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n109_), .c(new_n146_), .d(new_n79_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n141_), .O(z16));
endmodule


