// Benchmark "FAU" written by ABC on Fri Aug 14 02:20:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n62_, new_n64_,
    new_n66_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x23), .b(x06), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  and2   g011(.a(new_n55_), .b(x00), .O(z01));
  inv1   g012(.a(new_n54_), .O(new_n57_));
  nand2  g013(.a(x08), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n57_), .O(z02));
  and2   g015(.a(new_n55_), .b(x02), .O(z03));
  and2   g016(.a(new_n55_), .b(x03), .O(z04));
  nand2  g017(.a(new_n55_), .b(x04), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z05));
  nand2  g019(.a(x08), .b(x05), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n57_), .O(z06));
  nand2  g021(.a(x08), .b(x06), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n57_), .O(z07));
  and2   g023(.a(new_n55_), .b(x07), .O(z08));
  inv1   g024(.a(x21), .O(new_n69_));
  inv1   g025(.a(x22), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(x20), .c(x11), .O(new_n72_));
  inv1   g028(.a(x23), .O(new_n73_));
  inv1   g029(.a(x24), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g031(.a(x25), .O(new_n76_));
  nor2   g032(.a(new_n45_), .b(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n72_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n80_));
  aoi21  g036(.a(x08), .b(x00), .c(new_n54_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(z09));
  xor2a  g038(.a(x20), .b(x19), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n58_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n57_), .O(new_n86_));
  inv1   g042(.a(x09), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(x08), .O(new_n88_));
  nand2  g044(.a(x19), .b(x12), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(x10), .O(new_n90_));
  nand3  g046(.a(x26), .b(x25), .c(x24), .O(new_n91_));
  nor4   g047(.a(new_n91_), .b(new_n73_), .c(new_n70_), .d(new_n69_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n88_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n86_), .O(z10));
  nand3  g050(.a(x23), .b(x22), .c(x13), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n91_), .c(x21), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x20), .c(x19), .O(new_n97_));
  nand2  g053(.a(x20), .b(x19), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n69_), .c(new_n97_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n101_));
  aoi21  g057(.a(x08), .b(x02), .c(new_n54_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n101_), .O(z11));
  nand2  g059(.a(x23), .b(x14), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n91_), .c(x21), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n98_), .c(x22), .O(new_n106_));
  nand3  g062(.a(new_n99_), .b(new_n70_), .c(x21), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n109_));
  aoi21  g065(.a(x08), .b(x03), .c(new_n54_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(z12));
  nand3  g067(.a(x21), .b(x20), .c(x19), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand3  g069(.a(new_n77_), .b(x24), .c(x15), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n113_), .c(x22), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x23), .O(new_n116_));
  nor2   g072(.a(x23), .b(new_n70_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n99_), .c(x21), .d(x06), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n62_), .O(z13));
  nand2  g077(.a(new_n55_), .b(x05), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n99_), .c(x22), .d(x21), .O(new_n124_));
  nand3  g080(.a(new_n74_), .b(x22), .c(x21), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n98_), .O(new_n126_));
  aoi21  g082(.a(new_n124_), .b(x24), .c(new_n126_), .O(new_n127_));
  nand3  g083(.a(x24), .b(new_n73_), .c(x06), .O(new_n128_));
  oai21  g084(.a(new_n127_), .b(new_n73_), .c(new_n128_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n122_), .O(z14));
  nand4  g087(.a(x25), .b(new_n73_), .c(new_n47_), .d(x09), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n46_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x06), .O(new_n134_));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x24), .c(x22), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n112_), .c(x25), .O(new_n137_));
  nand4  g093(.a(new_n113_), .b(new_n76_), .c(x24), .d(x22), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n137_), .c(new_n73_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n134_), .O(z15));
  nand3  g097(.a(new_n88_), .b(x26), .c(new_n47_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x06), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n73_), .O(new_n144_));
  inv1   g100(.a(x18), .O(new_n145_));
  nand2  g101(.a(new_n99_), .b(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n71_), .b(x25), .c(x24), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n146_), .c(x26), .O(new_n148_));
  nor2   g104(.a(x26), .b(new_n76_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n99_), .c(new_n75_), .d(new_n71_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n152_));
  nand2  g108(.a(x08), .b(x07), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n152_), .c(new_n144_), .O(z16));
endmodule


