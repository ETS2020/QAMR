// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n137_, new_n139_, new_n140_, new_n143_;
  inv1   g000(.a(x11), .O(new_n44_));
  nor3   g001(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  inv1   g002(.a(x17), .O(new_n46_));
  nor2   g003(.a(x19), .b(x18), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand2  g006(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  aoi21  g007(.a(new_n50_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g008(.a(x00), .O(new_n52_));
  inv1   g009(.a(x02), .O(new_n53_));
  nand4  g010(.a(new_n49_), .b(x09), .c(new_n53_), .d(new_n52_), .O(new_n54_));
  aoi21  g011(.a(new_n54_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g012(.a(x18), .O(new_n56_));
  inv1   g013(.a(x09), .O(new_n57_));
  inv1   g014(.a(x01), .O(new_n58_));
  nand3  g015(.a(new_n53_), .b(new_n58_), .c(new_n52_), .O(new_n59_));
  nor3   g016(.a(new_n59_), .b(x10), .c(new_n57_), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n56_), .c(new_n46_), .d(new_n44_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x19), .O(z02));
  nand2  g019(.a(new_n53_), .b(new_n52_), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nand3  g021(.a(new_n64_), .b(x18), .c(new_n46_), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n63_), .c(new_n44_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  nand3  g024(.a(new_n57_), .b(x02), .c(x00), .O(new_n68_));
  nand3  g025(.a(x12), .b(x11), .c(x10), .O(new_n69_));
  oai21  g026(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(z03));
  nand3  g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  inv1   g028(.a(new_n71_), .O(new_n72_));
  nand3  g029(.a(new_n72_), .b(x10), .c(new_n57_), .O(new_n73_));
  nor3   g030(.a(new_n73_), .b(x12), .c(x11), .O(z04));
  inv1   g031(.a(x10), .O(new_n75_));
  nor3   g032(.a(new_n71_), .b(new_n75_), .c(new_n57_), .O(z05));
  inv1   g033(.a(new_n73_), .O(new_n77_));
  nand2  g034(.a(new_n77_), .b(x11), .O(new_n78_));
  inv1   g035(.a(new_n78_), .O(z06));
  inv1   g036(.a(x12), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(x11), .c(x10), .d(new_n57_), .O(new_n81_));
  nand2  g038(.a(new_n81_), .b(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(x11), .b(new_n58_), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n83_), .O(z07));
  inv1   g042(.a(x05), .O(new_n86_));
  inv1   g043(.a(x06), .O(new_n87_));
  inv1   g044(.a(x07), .O(new_n88_));
  inv1   g045(.a(x04), .O(new_n89_));
  inv1   g046(.a(x03), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(x02), .c(new_n58_), .d(new_n52_), .O(new_n91_));
  nor2   g048(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(x08), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n56_), .c(x17), .d(new_n44_), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(new_n64_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nor2   g055(.a(new_n53_), .b(x01), .O(new_n99_));
  nand3  g056(.a(new_n99_), .b(x12), .c(new_n44_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nor2   g059(.a(x14), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g061(.a(x20), .O(new_n105_));
  nor2   g062(.a(new_n56_), .b(new_n58_), .O(new_n106_));
  nand3  g063(.a(new_n106_), .b(new_n105_), .c(new_n64_), .O(new_n107_));
  oai21  g064(.a(new_n104_), .b(new_n100_), .c(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n98_), .c(new_n97_), .d(new_n52_), .O(new_n109_));
  inv1   g066(.a(new_n109_), .O(z09));
  nand3  g067(.a(new_n64_), .b(x18), .c(x01), .O(new_n111_));
  nor4   g068(.a(new_n111_), .b(new_n98_), .c(new_n97_), .d(x20), .O(new_n112_));
  nand2  g069(.a(new_n103_), .b(x12), .O(new_n113_));
  nor3   g070(.a(new_n113_), .b(new_n53_), .c(x01), .O(new_n114_));
  nand3  g071(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n115_));
  nor3   g072(.a(new_n115_), .b(new_n102_), .c(new_n101_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n114_), .c(new_n112_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n84_), .O(z10));
  nand4  g075(.a(new_n106_), .b(x21), .c(new_n105_), .d(new_n64_), .O(new_n119_));
  nor2   g076(.a(x13), .b(new_n80_), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n99_), .c(new_n44_), .O(new_n121_));
  nor2   g078(.a(new_n101_), .b(x14), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n97_), .c(x20), .d(new_n102_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(new_n121_), .c(new_n119_), .O(new_n124_));
  nand3  g081(.a(new_n124_), .b(new_n98_), .c(new_n52_), .O(new_n125_));
  inv1   g082(.a(new_n125_), .O(z11));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n64_), .b(x17), .c(new_n53_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand4  g088(.a(new_n131_), .b(new_n44_), .c(new_n58_), .d(new_n52_), .O(new_n132_));
  aoi21  g089(.a(new_n132_), .b(new_n128_), .c(x24), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(x09), .O(new_n134_));
  inv1   g091(.a(new_n134_), .O(z12));
  nor4   g092(.a(new_n59_), .b(x19), .c(new_n46_), .d(x11), .O(z13));
  nand4  g093(.a(new_n47_), .b(new_n45_), .c(new_n46_), .d(new_n75_), .O(new_n137_));
  aoi21  g094(.a(new_n137_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g095(.a(new_n75_), .b(x01), .c(new_n53_), .O(new_n139_));
  aoi21  g096(.a(x19), .b(new_n53_), .c(x11), .O(new_n140_));
  oai22  g097(.a(new_n140_), .b(x01), .c(new_n139_), .d(new_n52_), .O(z15));
  oai21  g098(.a(new_n58_), .b(x00), .c(new_n84_), .O(z16));
  nand2  g099(.a(x02), .b(new_n52_), .O(new_n143_));
  aoi21  g100(.a(new_n143_), .b(new_n44_), .c(x01), .O(z17));
endmodule


