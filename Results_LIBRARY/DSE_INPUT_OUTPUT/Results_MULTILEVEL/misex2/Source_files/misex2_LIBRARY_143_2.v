// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n146_, new_n148_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  nand2  g008(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g009(.a(new_n52_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x19), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  nand4  g013(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(x19), .O(z02));
  nand3  g015(.a(new_n46_), .b(x02), .c(x00), .O(new_n59_));
  nand3  g016(.a(x12), .b(x11), .c(x10), .O(new_n60_));
  oai21  g017(.a(new_n60_), .b(new_n59_), .c(x17), .O(new_n61_));
  nand2  g018(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g019(.a(x00), .O(new_n63_));
  inv1   g020(.a(x02), .O(new_n64_));
  inv1   g021(.a(x19), .O(new_n65_));
  nand2  g022(.a(new_n65_), .b(x18), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n44_), .c(new_n64_), .d(new_n63_), .O(new_n68_));
  nand2  g025(.a(new_n68_), .b(new_n62_), .O(z03));
  inv1   g026(.a(x11), .O(new_n70_));
  inv1   g027(.a(x12), .O(new_n71_));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  nor2   g029(.a(new_n72_), .b(x09), .O(new_n73_));
  nand4  g030(.a(new_n73_), .b(new_n71_), .c(new_n70_), .d(x10), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(new_n44_), .O(z04));
  nor4   g032(.a(new_n72_), .b(new_n44_), .c(new_n56_), .d(new_n46_), .O(z05));
  inv1   g033(.a(x01), .O(new_n77_));
  nor2   g034(.a(new_n64_), .b(new_n63_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x11), .c(x10), .d(new_n46_), .O(new_n79_));
  aoi21  g036(.a(new_n79_), .b(x17), .c(new_n77_), .O(z06));
  nand2  g037(.a(x10), .b(new_n46_), .O(new_n81_));
  nand3  g038(.a(x17), .b(new_n71_), .c(x11), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(new_n81_), .c(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g041(.a(new_n84_), .O(z07));
  nand2  g042(.a(new_n44_), .b(x01), .O(new_n86_));
  inv1   g043(.a(x03), .O(new_n87_));
  nor2   g044(.a(new_n64_), .b(x01), .O(new_n88_));
  inv1   g045(.a(x04), .O(new_n89_));
  nor2   g046(.a(x05), .b(new_n89_), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(new_n88_), .c(new_n87_), .d(new_n63_), .O(new_n91_));
  nor3   g048(.a(x08), .b(x07), .c(x06), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(x19), .c(new_n45_), .d(x17), .O(new_n93_));
  oai21  g050(.a(new_n93_), .b(new_n91_), .c(new_n86_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand3  g053(.a(new_n88_), .b(x12), .c(new_n70_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  nor2   g058(.a(x20), .b(x19), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x18), .c(x17), .d(x01), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n63_), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(z09));
  inv1   g063(.a(x20), .O(new_n107_));
  nand3  g064(.a(x22), .b(x21), .c(new_n107_), .O(new_n108_));
  nor3   g065(.a(new_n108_), .b(new_n66_), .c(new_n77_), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n70_), .O(new_n111_));
  inv1   g068(.a(new_n111_), .O(new_n112_));
  nand2  g069(.a(new_n112_), .b(new_n88_), .O(new_n113_));
  inv1   g070(.a(new_n113_), .O(new_n114_));
  nand2  g071(.a(x16), .b(x15), .O(new_n115_));
  nand3  g072(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n116_));
  nor3   g073(.a(new_n116_), .b(new_n115_), .c(x14), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n114_), .c(new_n109_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(x00), .c(new_n86_), .O(z10));
  nand3  g076(.a(x18), .b(x17), .c(x01), .O(new_n120_));
  nand3  g077(.a(x21), .b(new_n107_), .c(new_n65_), .O(new_n121_));
  nor2   g078(.a(new_n98_), .b(x14), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n95_), .c(x20), .d(new_n99_), .O(new_n123_));
  oai22  g080(.a(new_n123_), .b(new_n113_), .c(new_n121_), .d(new_n120_), .O(new_n124_));
  nand3  g081(.a(new_n124_), .b(new_n96_), .c(new_n63_), .O(new_n125_));
  inv1   g082(.a(new_n125_), .O(z11));
  inv1   g083(.a(x24), .O(new_n127_));
  nand2  g084(.a(x10), .b(x02), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(x01), .c(x00), .O(new_n129_));
  nor2   g086(.a(x19), .b(x02), .O(new_n130_));
  oai21  g087(.a(new_n130_), .b(x23), .c(x17), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n77_), .c(new_n63_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n129_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n127_), .c(x09), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(new_n86_), .O(z12));
  nand2  g094(.a(new_n47_), .b(x17), .O(new_n138_));
  nor2   g095(.a(new_n138_), .b(x19), .O(z13));
  nor3   g096(.a(x09), .b(x02), .c(x00), .O(new_n140_));
  nand4  g097(.a(new_n140_), .b(new_n65_), .c(new_n45_), .d(new_n56_), .O(new_n141_));
  aoi21  g098(.a(new_n141_), .b(new_n77_), .c(x17), .O(z14));
  aoi22  g099(.a(new_n86_), .b(x00), .c(x19), .d(new_n77_), .O(new_n143_));
  nand4  g100(.a(x17), .b(new_n56_), .c(x01), .d(x00), .O(new_n144_));
  oai21  g101(.a(new_n143_), .b(x02), .c(new_n144_), .O(z15));
  nand3  g102(.a(x17), .b(x01), .c(new_n63_), .O(new_n146_));
  inv1   g103(.a(new_n146_), .O(z16));
  nand3  g104(.a(x02), .b(new_n77_), .c(new_n63_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


