// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n139_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g06(.a(x17), .O(new_n50_));
  nor3   g07(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(x10), .d(x09), .O(new_n52_));
  nor3   g09(.a(new_n52_), .b(x19), .c(x18), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n47_), .b(new_n54_), .c(x09), .d(new_n45_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g13(.a(x01), .O(new_n57_));
  nor2   g14(.a(x09), .b(new_n45_), .O(new_n58_));
  nand3  g15(.a(x12), .b(x11), .c(x10), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nor2   g19(.a(x02), .b(x01), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n62_), .c(x18), .d(new_n50_), .O(new_n64_));
  oai21  g21(.a(new_n61_), .b(new_n44_), .c(new_n64_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nand4  g24(.a(new_n58_), .b(new_n67_), .c(new_n66_), .d(x10), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(x01), .c(new_n44_), .O(z04));
  nand3  g26(.a(x10), .b(x09), .c(x02), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x01), .c(new_n44_), .O(z05));
  nand3  g28(.a(new_n58_), .b(x11), .c(x10), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n44_), .O(z06));
  nand3  g30(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(x11), .c(x10), .d(new_n46_), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(x12), .O(z07));
  nor2   g34(.a(x03), .b(new_n45_), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x04), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  inv1   g39(.a(x18), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n83_), .c(x17), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n82_), .c(new_n81_), .d(new_n78_), .O(new_n86_));
  aoi21  g43(.a(new_n86_), .b(new_n44_), .c(x01), .O(z08));
  nor2   g44(.a(x14), .b(x13), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x12), .c(new_n66_), .d(x02), .O(new_n89_));
  inv1   g46(.a(x15), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nor2   g48(.a(x22), .b(x21), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(x20), .c(new_n91_), .d(new_n90_), .O(new_n93_));
  oai21  g50(.a(new_n93_), .b(new_n89_), .c(new_n44_), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n57_), .O(new_n95_));
  and2   g52(.a(x18), .b(x01), .O(new_n96_));
  nor2   g53(.a(x20), .b(x19), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(new_n92_), .d(new_n44_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n95_), .O(z09));
  nand2  g56(.a(x22), .b(x21), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n62_), .c(x18), .d(x01), .O(new_n102_));
  nand2  g59(.a(new_n92_), .b(x20), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  nor2   g61(.a(new_n45_), .b(x01), .O(new_n105_));
  inv1   g62(.a(x13), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(x12), .c(new_n66_), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nor3   g65(.a(new_n91_), .b(new_n90_), .c(x14), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n108_), .c(new_n105_), .d(new_n104_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n102_), .c(x00), .O(z10));
  inv1   g68(.a(x22), .O(new_n112_));
  nor2   g69(.a(x13), .b(new_n67_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n105_), .c(new_n66_), .O(new_n114_));
  inv1   g71(.a(x20), .O(new_n115_));
  nand4  g72(.a(new_n96_), .b(x21), .c(new_n115_), .d(new_n62_), .O(new_n116_));
  nor2   g73(.a(new_n90_), .b(x14), .O(new_n117_));
  nor2   g74(.a(x21), .b(new_n115_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n117_), .c(new_n91_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n114_), .c(new_n116_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n112_), .c(new_n44_), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z11));
  inv1   g79(.a(x24), .O(new_n123_));
  aoi21  g80(.a(x10), .b(x02), .c(new_n44_), .O(new_n124_));
  oai21  g81(.a(x19), .b(x02), .c(x23), .O(new_n125_));
  nand3  g82(.a(new_n62_), .b(x17), .c(new_n45_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n125_), .c(x01), .O(new_n127_));
  oai21  g84(.a(new_n127_), .b(new_n124_), .c(new_n123_), .O(new_n128_));
  nand2  g85(.a(new_n57_), .b(x00), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(new_n46_), .c(new_n129_), .O(z12));
  nand2  g87(.a(new_n51_), .b(x17), .O(new_n131_));
  nor2   g88(.a(new_n131_), .b(x19), .O(z13));
  nand4  g89(.a(new_n51_), .b(new_n50_), .c(new_n54_), .d(new_n46_), .O(new_n133_));
  nor3   g90(.a(new_n133_), .b(x19), .c(x18), .O(z14));
  nand2  g91(.a(x10), .b(x02), .O(new_n135_));
  nand3  g92(.a(new_n135_), .b(x01), .c(x00), .O(new_n136_));
  nand4  g93(.a(x19), .b(new_n45_), .c(new_n57_), .d(new_n44_), .O(new_n137_));
  nand2  g94(.a(new_n137_), .b(new_n136_), .O(z15));
  nand2  g95(.a(x01), .b(new_n44_), .O(new_n139_));
  nand2  g96(.a(new_n139_), .b(new_n129_), .O(z16));
  aoi21  g97(.a(new_n45_), .b(new_n44_), .c(x01), .O(z17));
endmodule


