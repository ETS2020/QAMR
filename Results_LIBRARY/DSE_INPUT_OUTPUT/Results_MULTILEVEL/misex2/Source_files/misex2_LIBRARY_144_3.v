// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n142_;
  nand2  g00(.a(x22), .b(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  nand3  g14(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n58_));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n59_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n58_), .c(new_n44_), .O(z02));
  inv1   g18(.a(x22), .O(new_n62_));
  nand3  g19(.a(new_n46_), .b(x02), .c(x00), .O(new_n63_));
  nand3  g20(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x01), .O(new_n66_));
  nor3   g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n51_), .c(x18), .d(new_n49_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n66_), .O(z03));
  inv1   g26(.a(x01), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  inv1   g29(.a(new_n63_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x10), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(new_n62_), .c(new_n70_), .O(z04));
  inv1   g32(.a(x00), .O(new_n76_));
  nor2   g33(.a(new_n45_), .b(new_n76_), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x10), .c(x09), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(new_n62_), .c(new_n70_), .O(z05));
  nand4  g36(.a(new_n77_), .b(x11), .c(x10), .d(new_n46_), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(new_n62_), .c(new_n70_), .O(z06));
  nand4  g38(.a(new_n72_), .b(x11), .c(x10), .d(new_n46_), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n44_), .O(z07));
  nor2   g42(.a(new_n45_), .b(x01), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n76_), .O(new_n87_));
  inv1   g44(.a(x03), .O(new_n88_));
  inv1   g45(.a(x05), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(x04), .c(new_n88_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nor3   g48(.a(x08), .b(x07), .c(x06), .O(new_n92_));
  nor2   g49(.a(new_n51_), .b(x18), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(x17), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n44_), .O(z08));
  inv1   g52(.a(x21), .O(new_n96_));
  nand4  g53(.a(x12), .b(new_n71_), .c(x02), .d(new_n70_), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nor2   g56(.a(x14), .b(x13), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g58(.a(x20), .O(new_n102_));
  and2   g59(.a(x18), .b(x01), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n51_), .O(new_n104_));
  oai21  g61(.a(new_n101_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n62_), .c(new_n96_), .d(new_n76_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(z09));
  nor2   g64(.a(x13), .b(new_n72_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n71_), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n87_), .O(new_n110_));
  nor3   g67(.a(new_n99_), .b(new_n98_), .c(x14), .O(new_n111_));
  nor2   g68(.a(x22), .b(x21), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(x20), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n44_), .O(z10));
  nand4  g71(.a(new_n103_), .b(x21), .c(new_n102_), .d(new_n51_), .O(new_n115_));
  nand3  g72(.a(new_n108_), .b(new_n86_), .c(new_n71_), .O(new_n116_));
  nor2   g73(.a(new_n98_), .b(x14), .O(new_n117_));
  nor2   g74(.a(x21), .b(new_n102_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n117_), .c(new_n99_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n115_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n62_), .c(new_n76_), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z11));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(x10), .b(x02), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(x01), .c(x00), .O(new_n125_));
  oai21  g82(.a(x19), .b(x02), .c(x23), .O(new_n126_));
  nand3  g83(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n70_), .c(new_n76_), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand3  g87(.a(new_n130_), .b(new_n123_), .c(x09), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(new_n44_), .O(z12));
  inv1   g89(.a(new_n47_), .O(new_n133_));
  oai21  g90(.a(new_n127_), .b(new_n133_), .c(new_n44_), .O(z13));
  oai21  g91(.a(new_n60_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g92(.a(x10), .b(new_n70_), .c(x02), .O(new_n136_));
  nand2  g93(.a(new_n136_), .b(x00), .O(new_n137_));
  nand3  g94(.a(x19), .b(new_n45_), .c(new_n70_), .O(new_n138_));
  nand3  g95(.a(new_n138_), .b(new_n137_), .c(new_n44_), .O(z15));
  nand3  g96(.a(new_n62_), .b(x01), .c(new_n76_), .O(new_n140_));
  inv1   g97(.a(new_n140_), .O(z16));
  nand2  g98(.a(new_n47_), .b(x02), .O(new_n142_));
  inv1   g99(.a(new_n142_), .O(z17));
endmodule


