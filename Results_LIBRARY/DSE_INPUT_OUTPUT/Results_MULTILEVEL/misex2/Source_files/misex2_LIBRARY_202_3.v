// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_;
  nand2  g000(.a(x23), .b(x09), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor2   g003(.a(x01), .b(x00), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g005(.a(x17), .O(new_n49_));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x19), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  inv1   g010(.a(x23), .O(new_n54_));
  inv1   g011(.a(new_n52_), .O(new_n55_));
  nor3   g012(.a(x02), .b(x01), .c(x00), .O(new_n56_));
  nand2  g013(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g014(.a(new_n57_), .b(new_n54_), .c(new_n46_), .O(z01));
  nand3  g015(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x10), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x23), .O(z02));
  nor2   g019(.a(x19), .b(new_n50_), .O(new_n63_));
  nand3  g020(.a(new_n63_), .b(new_n56_), .c(new_n49_), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  inv1   g023(.a(x10), .O(new_n67_));
  nor2   g024(.a(new_n67_), .b(x09), .O(new_n68_));
  nand4  g025(.a(new_n68_), .b(new_n66_), .c(x12), .d(x11), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(new_n64_), .c(new_n44_), .O(z03));
  inv1   g027(.a(x11), .O(new_n71_));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  nor3   g029(.a(new_n72_), .b(new_n67_), .c(x09), .O(new_n73_));
  nand2  g030(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(x12), .O(z04));
  nor4   g032(.a(new_n72_), .b(x23), .c(new_n67_), .d(new_n46_), .O(z05));
  nand2  g033(.a(new_n73_), .b(x11), .O(new_n77_));
  inv1   g034(.a(new_n77_), .O(z06));
  inv1   g035(.a(x12), .O(new_n79_));
  nand3  g036(.a(new_n68_), .b(new_n79_), .c(x11), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n44_), .O(z07));
  inv1   g040(.a(x00), .O(new_n84_));
  nor2   g041(.a(new_n45_), .b(x01), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g043(.a(x03), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  nand3  g045(.a(new_n88_), .b(x04), .c(new_n87_), .O(new_n89_));
  nor2   g046(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nor3   g047(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nor2   g048(.a(new_n51_), .b(x18), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(x17), .O(new_n93_));
  nand2  g050(.a(new_n93_), .b(new_n44_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  inv1   g053(.a(x01), .O(new_n97_));
  nand4  g054(.a(x12), .b(new_n71_), .c(x02), .d(new_n97_), .O(new_n98_));
  inv1   g055(.a(x15), .O(new_n99_));
  inv1   g056(.a(x16), .O(new_n100_));
  nor2   g057(.a(x14), .b(x13), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x20), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g059(.a(x20), .O(new_n103_));
  and2   g060(.a(x18), .b(x01), .O(new_n104_));
  nand3  g061(.a(new_n104_), .b(new_n103_), .c(new_n51_), .O(new_n105_));
  oai21  g062(.a(new_n102_), .b(new_n98_), .c(new_n105_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n96_), .c(new_n95_), .d(new_n84_), .O(new_n107_));
  nand2  g064(.a(new_n107_), .b(new_n44_), .O(z09));
  nor2   g065(.a(new_n96_), .b(new_n95_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n63_), .c(new_n103_), .d(x01), .O(new_n110_));
  nor2   g067(.a(x13), .b(new_n79_), .O(new_n111_));
  nand3  g068(.a(new_n111_), .b(new_n85_), .c(new_n71_), .O(new_n112_));
  inv1   g069(.a(x14), .O(new_n113_));
  nor2   g070(.a(new_n100_), .b(new_n99_), .O(new_n114_));
  nor2   g071(.a(x22), .b(x21), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(new_n114_), .c(x20), .d(new_n113_), .O(new_n116_));
  oai21  g073(.a(new_n116_), .b(new_n112_), .c(new_n110_), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(new_n44_), .c(new_n84_), .O(new_n118_));
  inv1   g075(.a(new_n118_), .O(z10));
  nand4  g076(.a(new_n104_), .b(x21), .c(new_n103_), .d(new_n51_), .O(new_n120_));
  nor2   g077(.a(new_n99_), .b(x14), .O(new_n121_));
  nor2   g078(.a(x21), .b(new_n103_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n121_), .c(new_n100_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(new_n112_), .c(new_n120_), .O(new_n124_));
  nand4  g081(.a(new_n124_), .b(new_n44_), .c(new_n96_), .d(new_n84_), .O(new_n125_));
  inv1   g082(.a(new_n125_), .O(z11));
  inv1   g083(.a(x24), .O(new_n127_));
  inv1   g084(.a(new_n47_), .O(new_n128_));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(x01), .c(x00), .O(new_n130_));
  nand3  g087(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n131_));
  oai21  g088(.a(new_n131_), .b(new_n128_), .c(new_n130_), .O(new_n132_));
  nand4  g089(.a(new_n132_), .b(new_n127_), .c(new_n54_), .d(x09), .O(new_n133_));
  inv1   g090(.a(new_n133_), .O(z12));
  nand3  g091(.a(new_n44_), .b(new_n51_), .c(x17), .O(new_n135_));
  nor4   g092(.a(new_n135_), .b(x02), .c(x01), .d(x00), .O(z13));
  nand3  g093(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n137_));
  inv1   g094(.a(new_n137_), .O(new_n138_));
  nand4  g095(.a(new_n138_), .b(new_n50_), .c(new_n49_), .d(new_n67_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x19), .O(z14));
  oai21  g097(.a(x10), .b(new_n97_), .c(x02), .O(new_n141_));
  nand2  g098(.a(new_n141_), .b(x00), .O(new_n142_));
  nand3  g099(.a(x19), .b(new_n45_), .c(new_n97_), .O(new_n143_));
  nand3  g100(.a(new_n143_), .b(new_n142_), .c(new_n44_), .O(z15));
  oai21  g101(.a(new_n97_), .b(x00), .c(new_n44_), .O(z16));
  nand2  g102(.a(new_n86_), .b(new_n44_), .O(z17));
endmodule


