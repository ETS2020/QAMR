// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_;
  nor3   g000(.a(x02), .b(x01), .c(x00), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  inv1   g002(.a(x18), .O(new_n46_));
  inv1   g003(.a(x19), .O(new_n47_));
  nand4  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand2  g006(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  aoi21  g007(.a(new_n50_), .b(x23), .c(x09), .O(z00));
  inv1   g008(.a(x09), .O(new_n52_));
  inv1   g009(.a(x23), .O(new_n53_));
  nand2  g010(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g011(.a(x00), .O(new_n55_));
  inv1   g012(.a(x01), .O(new_n56_));
  inv1   g013(.a(x02), .O(new_n57_));
  nand4  g014(.a(x09), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n48_), .c(new_n54_), .O(z01));
  inv1   g016(.a(x10), .O(new_n60_));
  nand4  g017(.a(x09), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n46_), .c(new_n45_), .d(new_n60_), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x19), .O(z02));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand3  g022(.a(x12), .b(x11), .c(x10), .O(new_n66_));
  oai21  g023(.a(new_n66_), .b(new_n65_), .c(x23), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nor2   g025(.a(x19), .b(new_n46_), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(new_n44_), .c(new_n45_), .O(new_n70_));
  nand2  g027(.a(new_n70_), .b(new_n68_), .O(z03));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g029(.a(new_n72_), .O(new_n73_));
  nand3  g030(.a(new_n73_), .b(x10), .c(new_n52_), .O(new_n74_));
  nor4   g031(.a(new_n74_), .b(new_n53_), .c(x12), .d(x11), .O(z04));
  nor3   g032(.a(new_n72_), .b(new_n60_), .c(new_n52_), .O(z05));
  inv1   g033(.a(x11), .O(new_n77_));
  nor3   g034(.a(new_n74_), .b(new_n53_), .c(new_n77_), .O(z06));
  inv1   g035(.a(x12), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x11), .c(x10), .d(new_n52_), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n54_), .O(z07));
  nor2   g040(.a(new_n57_), .b(x01), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n55_), .O(new_n85_));
  inv1   g042(.a(x03), .O(new_n86_));
  inv1   g043(.a(x05), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(x04), .c(new_n86_), .O(new_n88_));
  nor2   g045(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nor3   g046(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nor2   g047(.a(new_n47_), .b(x18), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x17), .O(new_n92_));
  nand2  g049(.a(new_n92_), .b(new_n54_), .O(z08));
  inv1   g050(.a(x21), .O(new_n94_));
  inv1   g051(.a(x22), .O(new_n95_));
  nand4  g052(.a(x12), .b(new_n77_), .c(x02), .d(new_n56_), .O(new_n96_));
  inv1   g053(.a(x15), .O(new_n97_));
  inv1   g054(.a(x16), .O(new_n98_));
  nor2   g055(.a(x14), .b(x13), .O(new_n99_));
  nand4  g056(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g057(.a(x20), .O(new_n101_));
  and2   g058(.a(x18), .b(x01), .O(new_n102_));
  nand3  g059(.a(new_n102_), .b(new_n101_), .c(new_n47_), .O(new_n103_));
  oai21  g060(.a(new_n100_), .b(new_n96_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n95_), .c(new_n94_), .d(new_n55_), .O(new_n105_));
  nand2  g062(.a(new_n105_), .b(new_n54_), .O(z09));
  nor2   g063(.a(new_n95_), .b(new_n94_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n69_), .c(new_n101_), .d(x01), .O(new_n108_));
  nor2   g065(.a(x13), .b(new_n79_), .O(new_n109_));
  nand3  g066(.a(new_n109_), .b(new_n84_), .c(new_n77_), .O(new_n110_));
  inv1   g067(.a(x14), .O(new_n111_));
  nor2   g068(.a(new_n98_), .b(new_n97_), .O(new_n112_));
  nor2   g069(.a(x22), .b(x21), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n112_), .c(x20), .d(new_n111_), .O(new_n114_));
  oai21  g071(.a(new_n114_), .b(new_n110_), .c(new_n108_), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(new_n54_), .c(new_n55_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(z10));
  nand4  g074(.a(new_n102_), .b(x21), .c(new_n101_), .d(new_n47_), .O(new_n118_));
  nor2   g075(.a(new_n97_), .b(x14), .O(new_n119_));
  nor2   g076(.a(x21), .b(new_n101_), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n119_), .c(new_n98_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(new_n110_), .c(new_n118_), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n54_), .c(new_n95_), .d(new_n55_), .O(new_n123_));
  inv1   g080(.a(new_n123_), .O(z11));
  inv1   g081(.a(x24), .O(new_n125_));
  nand2  g082(.a(x10), .b(x02), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  oai21  g084(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g085(.a(new_n47_), .b(x17), .c(new_n57_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(new_n56_), .c(new_n55_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n127_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n125_), .c(x09), .O(new_n133_));
  inv1   g090(.a(new_n133_), .O(z12));
  aoi21  g091(.a(new_n53_), .b(new_n52_), .c(x19), .O(new_n135_));
  nand4  g092(.a(new_n135_), .b(x17), .c(new_n57_), .d(new_n56_), .O(new_n136_));
  nor2   g093(.a(new_n136_), .b(x00), .O(z13));
  nand3  g094(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n138_));
  nor3   g095(.a(new_n138_), .b(x10), .c(x09), .O(new_n139_));
  nand4  g096(.a(new_n139_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(new_n53_), .O(z14));
  oai21  g098(.a(x10), .b(new_n56_), .c(x02), .O(new_n142_));
  nand2  g099(.a(new_n142_), .b(x00), .O(new_n143_));
  nand3  g100(.a(x19), .b(new_n57_), .c(new_n56_), .O(new_n144_));
  nand3  g101(.a(new_n144_), .b(new_n143_), .c(new_n54_), .O(z15));
  oai21  g102(.a(new_n56_), .b(x00), .c(new_n54_), .O(z16));
  nand2  g103(.a(new_n85_), .b(new_n54_), .O(z17));
endmodule


