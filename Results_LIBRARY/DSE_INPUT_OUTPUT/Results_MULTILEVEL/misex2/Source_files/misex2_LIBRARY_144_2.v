// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n127_, new_n128_, new_n129_, new_n131_, new_n133_;
  inv1   g00(.a(x22), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  nand3  g15(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n59_));
  inv1   g16(.a(x10), .O(new_n60_));
  nand4  g17(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n60_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n59_), .c(new_n45_), .O(z02));
  nand3  g19(.a(new_n47_), .b(x02), .c(x00), .O(new_n63_));
  nand3  g20(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(x22), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x01), .O(new_n66_));
  nor3   g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n52_), .c(x18), .d(new_n50_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n66_), .O(z03));
  inv1   g26(.a(x01), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  inv1   g29(.a(new_n63_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x10), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x22), .c(new_n70_), .O(z04));
  inv1   g32(.a(x00), .O(new_n76_));
  nor2   g33(.a(new_n46_), .b(new_n76_), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x10), .c(x09), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x22), .c(new_n70_), .O(z05));
  nand4  g36(.a(new_n77_), .b(x11), .c(x10), .d(new_n47_), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(x22), .c(new_n70_), .O(z06));
  nand4  g38(.a(new_n72_), .b(x11), .c(x10), .d(new_n47_), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n45_), .O(z07));
  inv1   g42(.a(x04), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n70_), .c(new_n76_), .O(new_n87_));
  nor4   g44(.a(new_n87_), .b(x05), .c(new_n86_), .d(x03), .O(new_n88_));
  nor3   g45(.a(x08), .b(x07), .c(x06), .O(new_n89_));
  nor2   g46(.a(new_n52_), .b(x18), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(x17), .O(new_n91_));
  nand2  g48(.a(new_n91_), .b(new_n45_), .O(z08));
  inv1   g49(.a(x16), .O(new_n93_));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  nand4  g53(.a(new_n71_), .b(x02), .c(new_n70_), .d(new_n76_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n96_), .c(new_n95_), .d(x12), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(x15), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n94_), .c(x20), .d(new_n93_), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(x22), .O(z09));
  nand2  g59(.a(x18), .b(x01), .O(new_n103_));
  nor4   g60(.a(new_n103_), .b(new_n94_), .c(x20), .d(x19), .O(new_n104_));
  nand3  g61(.a(new_n96_), .b(new_n95_), .c(x12), .O(new_n105_));
  nor3   g62(.a(new_n105_), .b(x11), .c(new_n46_), .O(new_n106_));
  inv1   g63(.a(x15), .O(new_n107_));
  nand3  g64(.a(new_n44_), .b(new_n94_), .c(x20), .O(new_n108_));
  nor3   g65(.a(new_n108_), .b(new_n93_), .c(new_n107_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n106_), .c(new_n104_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n45_), .O(z10));
  nor2   g68(.a(new_n99_), .b(new_n107_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n94_), .c(x20), .d(new_n93_), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(x22), .O(z11));
  nand2  g71(.a(x10), .b(x02), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(x01), .c(x00), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n70_), .c(new_n76_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n116_), .c(x24), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x09), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n45_), .O(z12));
  inv1   g80(.a(new_n48_), .O(new_n124_));
  oai21  g81(.a(new_n118_), .b(new_n124_), .c(new_n45_), .O(z13));
  oai21  g82(.a(new_n61_), .b(new_n49_), .c(new_n45_), .O(z14));
  oai21  g83(.a(x10), .b(new_n70_), .c(x02), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x00), .O(new_n128_));
  nand3  g85(.a(x19), .b(new_n46_), .c(new_n70_), .O(new_n129_));
  nand3  g86(.a(new_n129_), .b(new_n128_), .c(new_n45_), .O(z15));
  nand3  g87(.a(x22), .b(x01), .c(new_n76_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z16));
  nand2  g89(.a(new_n48_), .b(x02), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z17));
endmodule


