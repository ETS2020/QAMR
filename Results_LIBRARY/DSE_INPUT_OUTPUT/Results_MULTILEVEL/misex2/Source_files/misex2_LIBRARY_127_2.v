// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x07), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x09), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  inv1   g010(.a(x09), .O(new_n54_));
  nor2   g011(.a(new_n50_), .b(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  nand4  g015(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x19), .O(z02));
  inv1   g017(.a(x19), .O(new_n61_));
  nand2  g018(.a(new_n47_), .b(new_n46_), .O(new_n62_));
  nand3  g019(.a(x18), .b(new_n44_), .c(new_n48_), .O(new_n63_));
  oai21  g020(.a(new_n63_), .b(new_n62_), .c(new_n49_), .O(new_n64_));
  nand2  g021(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand3  g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nor2   g023(.a(new_n58_), .b(x09), .O(new_n67_));
  nand3  g024(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  oai21  g025(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(z03));
  nand2  g026(.a(new_n61_), .b(x07), .O(new_n70_));
  inv1   g027(.a(x11), .O(new_n71_));
  inv1   g028(.a(x12), .O(new_n72_));
  nand3  g029(.a(new_n67_), .b(new_n72_), .c(new_n71_), .O(new_n73_));
  oai21  g030(.a(new_n73_), .b(new_n66_), .c(new_n70_), .O(z04));
  nand2  g031(.a(x01), .b(x00), .O(new_n75_));
  nand3  g032(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n75_), .c(new_n70_), .O(z05));
  inv1   g034(.a(new_n70_), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x10), .c(new_n54_), .d(x02), .O(new_n80_));
  nor3   g037(.a(new_n80_), .b(new_n47_), .c(new_n46_), .O(z06));
  nand3  g038(.a(new_n67_), .b(new_n72_), .c(x11), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n70_), .O(z07));
  inv1   g042(.a(x04), .O(new_n86_));
  nor2   g043(.a(new_n48_), .b(x01), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n46_), .O(new_n88_));
  nor4   g045(.a(new_n88_), .b(x05), .c(new_n86_), .d(x03), .O(new_n89_));
  nor3   g046(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nor2   g047(.a(new_n61_), .b(x18), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x17), .O(new_n92_));
  nand2  g049(.a(new_n92_), .b(new_n70_), .O(z08));
  inv1   g050(.a(x21), .O(new_n94_));
  inv1   g051(.a(x22), .O(new_n95_));
  nand3  g052(.a(new_n87_), .b(x12), .c(new_n71_), .O(new_n96_));
  inv1   g053(.a(x15), .O(new_n97_));
  inv1   g054(.a(x16), .O(new_n98_));
  nor2   g055(.a(x14), .b(x13), .O(new_n99_));
  nand4  g056(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  nor2   g057(.a(new_n45_), .b(new_n47_), .O(new_n101_));
  nor2   g058(.a(x20), .b(x19), .O(new_n102_));
  nand2  g059(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  oai21  g060(.a(new_n100_), .b(new_n96_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n95_), .c(new_n94_), .d(new_n46_), .O(new_n105_));
  nand2  g062(.a(new_n105_), .b(new_n70_), .O(z09));
  inv1   g063(.a(x13), .O(new_n107_));
  inv1   g064(.a(x14), .O(new_n108_));
  nand4  g065(.a(new_n70_), .b(new_n95_), .c(new_n94_), .d(x20), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(new_n98_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(x15), .c(new_n108_), .d(new_n107_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n72_), .O(new_n112_));
  nand4  g069(.a(new_n112_), .b(new_n71_), .c(x02), .d(new_n47_), .O(new_n113_));
  nor2   g070(.a(new_n45_), .b(x07), .O(new_n114_));
  nor2   g071(.a(new_n95_), .b(new_n94_), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(new_n114_), .c(new_n102_), .d(x01), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n113_), .c(x00), .O(z10));
  nor2   g074(.a(new_n94_), .b(x20), .O(new_n118_));
  nand3  g075(.a(new_n118_), .b(new_n101_), .c(new_n61_), .O(new_n119_));
  nand4  g076(.a(new_n87_), .b(new_n107_), .c(x12), .d(new_n71_), .O(new_n120_));
  nor2   g077(.a(new_n97_), .b(x14), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n94_), .c(x20), .d(new_n98_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n120_), .c(new_n119_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n95_), .c(new_n46_), .O(new_n124_));
  nand2  g081(.a(new_n124_), .b(new_n70_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand4  g084(.a(new_n127_), .b(new_n70_), .c(x01), .d(x00), .O(new_n128_));
  nand3  g085(.a(new_n61_), .b(x17), .c(new_n48_), .O(new_n129_));
  nand2  g086(.a(x23), .b(x02), .O(new_n130_));
  aoi21  g087(.a(new_n130_), .b(new_n129_), .c(x07), .O(new_n131_));
  and2   g088(.a(x23), .b(x19), .O(new_n132_));
  oai21  g089(.a(new_n132_), .b(new_n131_), .c(new_n47_), .O(new_n133_));
  oai21  g090(.a(new_n133_), .b(x00), .c(new_n128_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n126_), .c(x09), .O(new_n135_));
  inv1   g092(.a(new_n135_), .O(z12));
  nor3   g093(.a(new_n50_), .b(x19), .c(new_n44_), .O(z13));
  nor3   g094(.a(x02), .b(x01), .c(x00), .O(new_n138_));
  nor2   g095(.a(x18), .b(x17), .O(new_n139_));
  nand4  g096(.a(new_n139_), .b(new_n138_), .c(new_n58_), .d(new_n54_), .O(new_n140_));
  aoi21  g097(.a(new_n140_), .b(new_n49_), .c(x19), .O(z14));
  oai21  g098(.a(x10), .b(new_n47_), .c(x02), .O(new_n142_));
  nand2  g099(.a(new_n142_), .b(x00), .O(new_n143_));
  nand3  g100(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n144_));
  nand3  g101(.a(new_n144_), .b(new_n143_), .c(new_n70_), .O(z15));
  nor3   g102(.a(new_n78_), .b(new_n47_), .c(x00), .O(z16));
  nand2  g103(.a(new_n88_), .b(new_n70_), .O(z17));
endmodule


