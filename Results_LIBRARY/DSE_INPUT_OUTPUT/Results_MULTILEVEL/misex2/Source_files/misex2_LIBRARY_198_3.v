// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n145_, new_n147_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(new_n49_), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nand2  g009(.a(x19), .b(x01), .O(new_n53_));
  nand3  g010(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n54_));
  nor2   g011(.a(x19), .b(x18), .O(new_n55_));
  nand3  g012(.a(new_n55_), .b(new_n44_), .c(x10), .O(new_n56_));
  oai21  g013(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  nand3  g015(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n59_));
  inv1   g016(.a(new_n59_), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x19), .O(z02));
  nand3  g019(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g020(.a(new_n63_), .O(new_n64_));
  nand2  g021(.a(x10), .b(new_n47_), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n64_), .c(x12), .d(x11), .O(new_n67_));
  nand4  g024(.a(new_n48_), .b(x18), .c(new_n44_), .d(new_n46_), .O(new_n68_));
  aoi21  g025(.a(new_n68_), .b(new_n67_), .c(x19), .O(z03));
  inv1   g026(.a(x11), .O(new_n70_));
  inv1   g027(.a(x12), .O(new_n71_));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  nor2   g029(.a(new_n72_), .b(x09), .O(new_n73_));
  nand4  g030(.a(new_n73_), .b(new_n71_), .c(new_n70_), .d(x10), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(x19), .O(z04));
  nor4   g032(.a(new_n72_), .b(x19), .c(new_n58_), .d(new_n47_), .O(z05));
  inv1   g033(.a(x01), .O(new_n77_));
  inv1   g034(.a(x19), .O(new_n78_));
  inv1   g035(.a(x00), .O(new_n79_));
  nor2   g036(.a(new_n46_), .b(new_n79_), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(x11), .c(x10), .d(new_n47_), .O(new_n81_));
  aoi21  g038(.a(new_n81_), .b(new_n78_), .c(new_n77_), .O(z06));
  nand3  g039(.a(new_n78_), .b(new_n71_), .c(x11), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(new_n65_), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(z07));
  inv1   g043(.a(x08), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  inv1   g045(.a(x06), .O(new_n89_));
  inv1   g046(.a(x03), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(x02), .c(new_n77_), .d(new_n79_), .O(new_n91_));
  inv1   g048(.a(new_n91_), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n89_), .c(new_n88_), .d(x04), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(x07), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n45_), .c(x17), .d(new_n87_), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(new_n78_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nor2   g055(.a(new_n46_), .b(x01), .O(new_n99_));
  nand3  g056(.a(new_n99_), .b(x12), .c(new_n70_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nor2   g059(.a(x14), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g061(.a(x20), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(x18), .c(x01), .O(new_n106_));
  oai21  g063(.a(new_n104_), .b(new_n100_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n98_), .c(new_n97_), .d(new_n79_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n53_), .O(z09));
  nand3  g066(.a(x22), .b(x21), .c(new_n105_), .O(new_n110_));
  nor3   g067(.a(new_n110_), .b(new_n45_), .c(new_n77_), .O(new_n111_));
  inv1   g068(.a(x13), .O(new_n112_));
  nand3  g069(.a(new_n112_), .b(x12), .c(new_n70_), .O(new_n113_));
  inv1   g070(.a(new_n113_), .O(new_n114_));
  nand2  g071(.a(new_n114_), .b(new_n99_), .O(new_n115_));
  inv1   g072(.a(new_n115_), .O(new_n116_));
  nand2  g073(.a(x16), .b(x15), .O(new_n117_));
  nand3  g074(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n118_));
  nor3   g075(.a(new_n118_), .b(new_n117_), .c(x14), .O(new_n119_));
  aoi21  g076(.a(new_n119_), .b(new_n116_), .c(new_n111_), .O(new_n120_));
  oai21  g077(.a(new_n120_), .b(x00), .c(new_n53_), .O(z10));
  nand4  g078(.a(x21), .b(new_n105_), .c(x18), .d(x01), .O(new_n122_));
  nor2   g079(.a(new_n101_), .b(x14), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n97_), .c(x20), .d(new_n102_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n115_), .c(new_n122_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n98_), .c(new_n79_), .O(new_n126_));
  nand2  g083(.a(new_n126_), .b(new_n53_), .O(z11));
  nand2  g084(.a(x10), .b(x02), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(x01), .c(x00), .O(new_n129_));
  nand3  g086(.a(new_n48_), .b(x17), .c(new_n46_), .O(new_n130_));
  aoi21  g087(.a(new_n130_), .b(new_n129_), .c(x19), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nor3   g089(.a(new_n132_), .b(x01), .c(x00), .O(new_n133_));
  nor2   g090(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nor3   g091(.a(new_n134_), .b(x24), .c(new_n47_), .O(z12));
  inv1   g092(.a(new_n48_), .O(new_n136_));
  nand3  g093(.a(new_n78_), .b(x17), .c(new_n46_), .O(new_n137_));
  oai21  g094(.a(new_n137_), .b(new_n136_), .c(new_n53_), .O(z13));
  nor2   g095(.a(x09), .b(x02), .O(new_n139_));
  nor2   g096(.a(x17), .b(x10), .O(new_n140_));
  nand4  g097(.a(new_n140_), .b(new_n139_), .c(new_n55_), .d(new_n48_), .O(new_n141_));
  nand2  g098(.a(new_n141_), .b(new_n53_), .O(z14));
  aoi21  g099(.a(new_n58_), .b(x01), .c(new_n46_), .O(new_n143_));
  oai22  g100(.a(new_n143_), .b(new_n79_), .c(new_n99_), .d(new_n78_), .O(z15));
  nand3  g101(.a(new_n78_), .b(x01), .c(new_n79_), .O(new_n145_));
  inv1   g102(.a(new_n145_), .O(z16));
  nand2  g103(.a(new_n99_), .b(new_n79_), .O(new_n147_));
  nand2  g104(.a(new_n147_), .b(new_n53_), .O(z17));
endmodule


