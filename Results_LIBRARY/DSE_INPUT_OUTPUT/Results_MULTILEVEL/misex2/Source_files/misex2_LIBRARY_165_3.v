// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n136_,
    new_n138_, new_n140_, new_n142_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand3  g09(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  nand2  g13(.a(x19), .b(x01), .O(new_n57_));
  nand3  g14(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n58_));
  inv1   g15(.a(x10), .O(new_n59_));
  inv1   g16(.a(x19), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n58_), .c(new_n57_), .O(z02));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nor2   g21(.a(new_n59_), .b(x09), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(x12), .d(x11), .O(new_n66_));
  nand4  g23(.a(new_n48_), .b(x18), .c(new_n44_), .d(new_n46_), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(new_n66_), .c(x19), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(x09), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(x19), .O(z04));
  nor4   g31(.a(new_n71_), .b(x19), .c(new_n59_), .d(new_n47_), .O(z05));
  inv1   g32(.a(x01), .O(new_n76_));
  inv1   g33(.a(x00), .O(new_n77_));
  nor2   g34(.a(new_n46_), .b(new_n77_), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n47_), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(new_n60_), .c(new_n76_), .O(z06));
  inv1   g37(.a(new_n65_), .O(new_n81_));
  nand3  g38(.a(new_n60_), .b(new_n70_), .c(x11), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(new_n81_), .c(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(z07));
  inv1   g42(.a(x04), .O(new_n86_));
  nand2  g43(.a(x02), .b(new_n77_), .O(new_n87_));
  nor4   g44(.a(new_n87_), .b(x05), .c(new_n86_), .d(x03), .O(new_n88_));
  nand2  g45(.a(new_n45_), .b(x17), .O(new_n89_));
  nor4   g46(.a(new_n89_), .b(x08), .c(x07), .d(x06), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(new_n76_), .c(new_n60_), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nor2   g51(.a(new_n46_), .b(x01), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n69_), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x18), .c(x01), .O(new_n102_));
  oai21  g59(.a(new_n100_), .b(new_n96_), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n94_), .c(new_n93_), .d(new_n77_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n57_), .O(z09));
  nor3   g62(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n60_), .c(x18), .d(x01), .O(new_n107_));
  inv1   g64(.a(x13), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(x12), .c(new_n69_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nor3   g67(.a(new_n98_), .b(new_n97_), .c(x14), .O(new_n111_));
  nand3  g68(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  nand4  g70(.a(new_n113_), .b(new_n111_), .c(new_n110_), .d(new_n95_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n107_), .c(x00), .O(z10));
  nand2  g72(.a(new_n110_), .b(new_n95_), .O(new_n116_));
  nand4  g73(.a(x21), .b(new_n101_), .c(x18), .d(x01), .O(new_n117_));
  nor2   g74(.a(new_n97_), .b(x14), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n93_), .c(x20), .d(new_n98_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n117_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n94_), .c(new_n77_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n57_), .O(z11));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(x10), .b(x02), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(x01), .c(x00), .O(new_n125_));
  nand3  g82(.a(new_n60_), .b(x17), .c(new_n46_), .O(new_n126_));
  nand2  g83(.a(x23), .b(x02), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(new_n126_), .c(x01), .O(new_n128_));
  and2   g85(.a(x23), .b(x19), .O(new_n129_));
  oai21  g86(.a(new_n129_), .b(new_n128_), .c(new_n77_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n125_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n123_), .c(x09), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(new_n57_), .O(z12));
  inv1   g90(.a(new_n48_), .O(new_n134_));
  oai21  g91(.a(new_n126_), .b(new_n134_), .c(new_n57_), .O(z13));
  nand3  g92(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n136_));
  oai21  g93(.a(new_n136_), .b(new_n61_), .c(new_n57_), .O(z14));
  aoi21  g94(.a(new_n59_), .b(x01), .c(new_n46_), .O(new_n138_));
  oai22  g95(.a(new_n138_), .b(new_n77_), .c(new_n95_), .d(new_n60_), .O(z15));
  nand3  g96(.a(new_n60_), .b(x01), .c(new_n77_), .O(new_n140_));
  inv1   g97(.a(new_n140_), .O(z16));
  nand2  g98(.a(new_n95_), .b(new_n77_), .O(new_n142_));
  nand2  g99(.a(new_n142_), .b(new_n57_), .O(z17));
endmodule


