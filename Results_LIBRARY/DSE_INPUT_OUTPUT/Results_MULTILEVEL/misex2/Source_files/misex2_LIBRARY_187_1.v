// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n146_;
  inv1   g000(.a(x11), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  nor3   g002(.a(x09), .b(x02), .c(x00), .O(new_n46_));
  nor2   g003(.a(x19), .b(x18), .O(new_n47_));
  nand4  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g005(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x10), .O(new_n51_));
  inv1   g008(.a(x00), .O(new_n52_));
  inv1   g009(.a(x01), .O(new_n53_));
  inv1   g010(.a(x02), .O(new_n54_));
  nand4  g011(.a(x09), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand4  g013(.a(new_n56_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(x19), .O(z01));
  nor2   g015(.a(new_n55_), .b(x10), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x19), .O(z02));
  nand3  g018(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n62_));
  inv1   g019(.a(x19), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(x18), .c(new_n45_), .d(new_n44_), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(x09), .O(new_n66_));
  nand4  g023(.a(x12), .b(x11), .c(x10), .d(new_n66_), .O(new_n67_));
  oai22  g024(.a(new_n67_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(z03));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  inv1   g026(.a(new_n69_), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n44_), .c(x10), .d(new_n66_), .O(new_n71_));
  nor2   g028(.a(new_n71_), .b(x12), .O(z04));
  nor3   g029(.a(new_n69_), .b(new_n51_), .c(new_n66_), .O(z05));
  nor4   g030(.a(new_n65_), .b(new_n44_), .c(new_n51_), .d(x09), .O(z06));
  nand2  g031(.a(new_n66_), .b(x01), .O(new_n75_));
  inv1   g032(.a(x12), .O(new_n76_));
  nand3  g033(.a(new_n76_), .b(x11), .c(x10), .O(new_n77_));
  oai22  g034(.a(new_n77_), .b(new_n75_), .c(x11), .d(x01), .O(new_n78_));
  nand3  g035(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  inv1   g036(.a(new_n79_), .O(z07));
  nand2  g037(.a(x02), .b(new_n52_), .O(new_n81_));
  inv1   g038(.a(x05), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(x04), .O(new_n83_));
  nor3   g040(.a(new_n83_), .b(new_n81_), .c(x03), .O(new_n84_));
  nor3   g041(.a(x08), .b(x07), .c(x06), .O(new_n85_));
  nand3  g042(.a(x19), .b(new_n50_), .c(x17), .O(new_n86_));
  inv1   g043(.a(new_n86_), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(new_n88_));
  aoi21  g045(.a(new_n88_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g046(.a(x21), .O(new_n90_));
  inv1   g047(.a(x22), .O(new_n91_));
  inv1   g048(.a(x13), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(x12), .c(x02), .d(new_n53_), .O(new_n93_));
  inv1   g050(.a(x14), .O(new_n94_));
  inv1   g051(.a(x15), .O(new_n95_));
  inv1   g052(.a(x16), .O(new_n96_));
  nand4  g053(.a(x20), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g054(.a(x20), .O(new_n98_));
  and2   g055(.a(x18), .b(x01), .O(new_n99_));
  nand3  g056(.a(new_n99_), .b(new_n98_), .c(new_n63_), .O(new_n100_));
  oai21  g057(.a(new_n97_), .b(new_n93_), .c(new_n100_), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(new_n91_), .c(new_n90_), .d(new_n52_), .O(new_n102_));
  nand2  g059(.a(x11), .b(new_n53_), .O(new_n103_));
  nand2  g060(.a(new_n103_), .b(new_n102_), .O(z09));
  nand3  g061(.a(new_n63_), .b(x18), .c(x01), .O(new_n105_));
  nand3  g062(.a(x22), .b(x21), .c(new_n98_), .O(new_n106_));
  nor2   g063(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g064(.a(x02), .b(new_n53_), .O(new_n108_));
  nand3  g065(.a(new_n94_), .b(new_n92_), .c(x12), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g067(.a(x16), .b(x15), .O(new_n111_));
  nand3  g068(.a(new_n91_), .b(new_n90_), .c(x20), .O(new_n112_));
  nor2   g069(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  aoi21  g070(.a(new_n113_), .b(new_n110_), .c(new_n107_), .O(new_n114_));
  oai21  g071(.a(new_n114_), .b(x00), .c(new_n103_), .O(z10));
  nand4  g072(.a(new_n99_), .b(x21), .c(new_n98_), .d(new_n63_), .O(new_n116_));
  nor2   g073(.a(new_n54_), .b(x01), .O(new_n117_));
  nor2   g074(.a(x13), .b(new_n76_), .O(new_n118_));
  nand3  g075(.a(new_n118_), .b(new_n117_), .c(new_n44_), .O(new_n119_));
  nor2   g076(.a(new_n95_), .b(x14), .O(new_n120_));
  nor2   g077(.a(x21), .b(new_n98_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n120_), .c(new_n96_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n119_), .c(new_n116_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n91_), .c(new_n52_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  aoi21  g083(.a(x10), .b(x02), .c(new_n53_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n63_), .b(x17), .c(new_n54_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n53_), .c(new_n52_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n126_), .c(x09), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n103_), .O(z12));
  nand3  g092(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n136_));
  nor4   g093(.a(new_n136_), .b(x19), .c(new_n45_), .d(x11), .O(z13));
  nand4  g094(.a(new_n66_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n138_));
  nor2   g095(.a(new_n138_), .b(x10), .O(new_n139_));
  nand4  g096(.a(new_n139_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(x19), .O(z14));
  aoi21  g098(.a(new_n44_), .b(new_n54_), .c(new_n127_), .O(new_n142_));
  nand4  g099(.a(x19), .b(new_n44_), .c(new_n54_), .d(new_n53_), .O(new_n143_));
  oai21  g100(.a(new_n142_), .b(new_n52_), .c(new_n143_), .O(z15));
  nor2   g101(.a(new_n53_), .b(x00), .O(z16));
  nand4  g102(.a(new_n44_), .b(x02), .c(new_n53_), .d(new_n52_), .O(new_n146_));
  inv1   g103(.a(new_n146_), .O(z17));
endmodule


