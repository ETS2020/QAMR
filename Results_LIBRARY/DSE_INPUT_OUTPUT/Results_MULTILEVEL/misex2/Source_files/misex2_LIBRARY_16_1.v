// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n137_, new_n138_, new_n140_, new_n141_;
  inv1   g000(.a(x11), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  inv1   g002(.a(x18), .O(new_n46_));
  inv1   g003(.a(x10), .O(new_n47_));
  inv1   g004(.a(x00), .O(new_n48_));
  inv1   g005(.a(x01), .O(new_n49_));
  inv1   g006(.a(x02), .O(new_n50_));
  inv1   g007(.a(x09), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x19), .O(z00));
  nand4  g012(.a(x09), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z01));
  nor2   g016(.a(new_n56_), .b(x10), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x19), .O(z02));
  nand3  g019(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nand4  g021(.a(new_n64_), .b(x18), .c(new_n45_), .d(new_n44_), .O(new_n65_));
  nand3  g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g023(.a(x12), .b(x11), .c(x10), .d(new_n51_), .O(new_n67_));
  oai22  g024(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n63_), .O(z03));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  inv1   g026(.a(new_n69_), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n44_), .c(x10), .d(new_n51_), .O(new_n71_));
  nor2   g028(.a(new_n71_), .b(x12), .O(z04));
  nand2  g029(.a(x01), .b(x00), .O(new_n73_));
  nand2  g030(.a(x11), .b(new_n49_), .O(new_n74_));
  nand3  g031(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  oai21  g032(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(z05));
  nor4   g033(.a(new_n66_), .b(new_n44_), .c(new_n47_), .d(x09), .O(z06));
  nand2  g034(.a(new_n51_), .b(x01), .O(new_n78_));
  inv1   g035(.a(x12), .O(new_n79_));
  nand3  g036(.a(new_n79_), .b(x11), .c(x10), .O(new_n80_));
  oai22  g037(.a(new_n80_), .b(new_n78_), .c(x11), .d(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  inv1   g039(.a(new_n82_), .O(z07));
  nand2  g040(.a(x02), .b(new_n48_), .O(new_n84_));
  inv1   g041(.a(x03), .O(new_n85_));
  inv1   g042(.a(x05), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x04), .c(new_n85_), .O(new_n87_));
  nor2   g044(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nor3   g045(.a(x08), .b(x07), .c(x06), .O(new_n89_));
  nor2   g046(.a(new_n64_), .b(x18), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(x17), .O(new_n91_));
  aoi21  g048(.a(new_n91_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g049(.a(x21), .O(new_n93_));
  inv1   g050(.a(x22), .O(new_n94_));
  nand4  g051(.a(x12), .b(new_n44_), .c(x02), .d(new_n49_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g056(.a(x20), .O(new_n100_));
  and2   g057(.a(x18), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(new_n100_), .c(new_n64_), .O(new_n102_));
  oai21  g059(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n94_), .c(new_n93_), .d(new_n48_), .O(new_n104_));
  inv1   g061(.a(new_n104_), .O(z09));
  nor3   g062(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n64_), .c(x18), .d(x01), .O(new_n107_));
  nor2   g064(.a(new_n50_), .b(x01), .O(new_n108_));
  nor2   g065(.a(x13), .b(new_n79_), .O(new_n109_));
  nand2  g066(.a(new_n109_), .b(new_n44_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nor3   g068(.a(new_n97_), .b(new_n96_), .c(x14), .O(new_n112_));
  nand3  g069(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n113_));
  inv1   g070(.a(new_n113_), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n108_), .O(new_n115_));
  aoi21  g072(.a(new_n115_), .b(new_n107_), .c(x00), .O(z10));
  nand3  g073(.a(new_n109_), .b(new_n108_), .c(new_n44_), .O(new_n117_));
  nand4  g074(.a(new_n101_), .b(x21), .c(new_n100_), .d(new_n64_), .O(new_n118_));
  nor2   g075(.a(new_n96_), .b(x14), .O(new_n119_));
  nor2   g076(.a(x21), .b(new_n100_), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n119_), .c(new_n97_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(new_n117_), .c(new_n118_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n94_), .c(new_n48_), .O(new_n123_));
  inv1   g080(.a(new_n123_), .O(z11));
  inv1   g081(.a(x24), .O(new_n125_));
  aoi21  g082(.a(x10), .b(x02), .c(new_n49_), .O(new_n126_));
  nand2  g083(.a(new_n126_), .b(x00), .O(new_n127_));
  oai21  g084(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g085(.a(new_n64_), .b(x17), .c(new_n50_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(new_n49_), .c(new_n48_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n127_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n125_), .c(x09), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n74_), .O(z12));
  nand4  g091(.a(new_n64_), .b(x17), .c(new_n50_), .d(new_n48_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(new_n44_), .c(x01), .O(z13));
  nor2   g093(.a(new_n52_), .b(x10), .O(new_n137_));
  nand4  g094(.a(new_n137_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n138_));
  nor2   g095(.a(new_n138_), .b(x19), .O(z14));
  aoi21  g096(.a(new_n44_), .b(new_n50_), .c(new_n126_), .O(new_n140_));
  nand4  g097(.a(x19), .b(new_n44_), .c(new_n50_), .d(new_n49_), .O(new_n141_));
  oai21  g098(.a(new_n140_), .b(new_n48_), .c(new_n141_), .O(z15));
  nor2   g099(.a(new_n49_), .b(x00), .O(z16));
  aoi21  g100(.a(new_n84_), .b(new_n44_), .c(x01), .O(z17));
endmodule


