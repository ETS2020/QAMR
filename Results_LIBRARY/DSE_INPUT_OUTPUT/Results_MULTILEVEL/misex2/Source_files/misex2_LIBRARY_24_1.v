// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n132_, new_n134_, new_n135_, new_n138_;
  inv1   g00(.a(x06), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor3   g02(.a(x09), .b(x02), .c(x00), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  inv1   g09(.a(x01), .O(new_n53_));
  inv1   g10(.a(x02), .O(new_n54_));
  nand4  g11(.a(new_n44_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n50_), .c(new_n45_), .d(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n50_), .c(new_n45_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  nand3  g18(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x18), .c(new_n45_), .d(new_n44_), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n51_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand2  g25(.a(x01), .b(x00), .O(new_n69_));
  nor4   g26(.a(new_n69_), .b(new_n59_), .c(x09), .d(new_n54_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(x12), .O(z04));
  nand2  g29(.a(x06), .b(new_n53_), .O(new_n73_));
  nand3  g30(.a(x10), .b(x09), .c(x02), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n69_), .c(new_n73_), .O(z05));
  nand2  g32(.a(new_n70_), .b(x11), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z06));
  inv1   g34(.a(x12), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n51_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n73_), .O(z07));
  inv1   g39(.a(x08), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x02), .c(new_n53_), .d(new_n52_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n44_), .c(new_n84_), .d(x04), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(x07), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n50_), .c(x17), .d(new_n83_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(new_n63_), .O(z08));
  inv1   g48(.a(x21), .O(new_n92_));
  inv1   g49(.a(x22), .O(new_n93_));
  nand4  g50(.a(x12), .b(new_n68_), .c(x02), .d(new_n53_), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nor2   g53(.a(x14), .b(x13), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  nor2   g56(.a(new_n50_), .b(new_n53_), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(new_n63_), .O(new_n101_));
  oai21  g58(.a(new_n98_), .b(new_n94_), .c(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n93_), .c(new_n92_), .d(new_n52_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n73_), .O(z09));
  nor3   g61(.a(new_n93_), .b(new_n92_), .c(x20), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n63_), .c(x18), .d(x01), .O(new_n106_));
  inv1   g63(.a(x13), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(x12), .c(new_n68_), .O(new_n108_));
  nor4   g65(.a(new_n108_), .b(x06), .c(new_n54_), .d(x01), .O(new_n109_));
  nor3   g66(.a(new_n96_), .b(new_n95_), .c(x14), .O(new_n110_));
  nor2   g67(.a(x22), .b(x21), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(x20), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n106_), .c(x00), .O(z10));
  nand4  g70(.a(new_n100_), .b(x21), .c(new_n99_), .d(new_n63_), .O(new_n114_));
  inv1   g71(.a(new_n108_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(x02), .c(new_n53_), .O(new_n116_));
  nor2   g73(.a(new_n95_), .b(x14), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n92_), .c(x20), .d(new_n96_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n93_), .c(new_n52_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n73_), .O(z11));
  aoi21  g78(.a(x10), .b(x02), .c(new_n53_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x00), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n63_), .b(x17), .c(new_n54_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g83(.a(new_n126_), .b(new_n44_), .c(new_n53_), .d(new_n52_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(new_n123_), .c(x24), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x09), .O(new_n129_));
  inv1   g86(.a(new_n129_), .O(z12));
  nor3   g87(.a(new_n55_), .b(x19), .c(new_n45_), .O(z13));
  nand4  g88(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n59_), .O(new_n132_));
  aoi21  g89(.a(new_n132_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g90(.a(new_n44_), .b(new_n54_), .c(new_n122_), .O(new_n134_));
  nand4  g91(.a(x19), .b(new_n44_), .c(new_n54_), .d(new_n53_), .O(new_n135_));
  oai21  g92(.a(new_n134_), .b(new_n52_), .c(new_n135_), .O(z15));
  oai21  g93(.a(new_n53_), .b(x00), .c(new_n73_), .O(z16));
  nand4  g94(.a(new_n44_), .b(x02), .c(new_n53_), .d(new_n52_), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z17));
endmodule


