// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_;
  nand2  g00(.a(x21), .b(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  nor2   g06(.a(x19), .b(x18), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(x10), .O(new_n51_));
  oai21  g08(.a(new_n51_), .b(new_n48_), .c(new_n44_), .O(z00));
  inv1   g09(.a(x18), .O(new_n53_));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n53_), .c(new_n49_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n53_), .c(new_n49_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  inv1   g17(.a(x21), .O(new_n61_));
  nand3  g18(.a(new_n46_), .b(x02), .c(x00), .O(new_n62_));
  nand3  g19(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x01), .O(new_n65_));
  inv1   g22(.a(x19), .O(new_n66_));
  nor3   g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x18), .d(new_n49_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n65_), .O(z03));
  inv1   g26(.a(x01), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  inv1   g29(.a(new_n62_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x10), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(new_n61_), .c(new_n70_), .O(z04));
  nand4  g32(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(new_n61_), .c(new_n70_), .O(z05));
  nand3  g34(.a(x02), .b(x01), .c(x00), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x11), .c(x10), .d(new_n46_), .O(new_n80_));
  nor2   g37(.a(new_n80_), .b(x21), .O(z06));
  nand4  g38(.a(new_n72_), .b(x11), .c(x10), .d(new_n46_), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n44_), .O(z07));
  inv1   g42(.a(x00), .O(new_n86_));
  nor2   g43(.a(new_n45_), .b(x01), .O(new_n87_));
  nand2  g44(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g45(.a(x05), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(x04), .O(new_n90_));
  nor3   g47(.a(new_n90_), .b(new_n88_), .c(x03), .O(new_n91_));
  nor3   g48(.a(x08), .b(x07), .c(x06), .O(new_n92_));
  nor2   g49(.a(new_n66_), .b(x18), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(x17), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n44_), .O(z08));
  inv1   g52(.a(x22), .O(new_n96_));
  inv1   g53(.a(x20), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n66_), .c(x18), .d(x01), .O(new_n98_));
  inv1   g55(.a(x13), .O(new_n99_));
  nand4  g56(.a(new_n87_), .b(new_n99_), .c(x12), .d(new_n71_), .O(new_n100_));
  inv1   g57(.a(x16), .O(new_n101_));
  nor2   g58(.a(x15), .b(x14), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n61_), .c(x20), .d(new_n101_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n100_), .c(new_n98_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n96_), .c(new_n86_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n44_), .O(z09));
  inv1   g63(.a(x15), .O(new_n107_));
  inv1   g64(.a(x14), .O(new_n108_));
  nand4  g65(.a(new_n71_), .b(x02), .c(new_n70_), .d(new_n86_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n108_), .c(new_n99_), .d(x12), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n107_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n61_), .c(x20), .d(x16), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(x22), .O(z10));
  nand4  g71(.a(new_n112_), .b(new_n61_), .c(x20), .d(new_n101_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(x22), .O(z11));
  nand2  g73(.a(x10), .b(x02), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n61_), .c(x01), .d(x00), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n66_), .b(x17), .c(new_n45_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n70_), .c(new_n86_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n118_), .c(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z12));
  inv1   g82(.a(new_n47_), .O(new_n126_));
  oai21  g83(.a(new_n120_), .b(new_n126_), .c(new_n44_), .O(z13));
  nand3  g84(.a(new_n50_), .b(new_n49_), .c(new_n58_), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n48_), .c(new_n44_), .O(z14));
  aoi22  g86(.a(new_n44_), .b(x00), .c(x19), .d(new_n70_), .O(new_n130_));
  nand4  g87(.a(new_n61_), .b(new_n58_), .c(x01), .d(x00), .O(new_n131_));
  oai21  g88(.a(new_n130_), .b(x02), .c(new_n131_), .O(z15));
  aoi21  g89(.a(new_n61_), .b(x00), .c(new_n70_), .O(z16));
  nand2  g90(.a(new_n88_), .b(new_n44_), .O(z17));
endmodule


