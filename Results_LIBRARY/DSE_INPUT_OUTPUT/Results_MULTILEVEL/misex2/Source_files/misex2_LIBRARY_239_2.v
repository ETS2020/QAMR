// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n130_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(x09), .c(new_n45_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n47_), .b(new_n57_), .c(x09), .d(new_n45_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  nor2   g17(.a(x09), .b(new_n45_), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nor2   g22(.a(x02), .b(x01), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n65_), .c(x18), .d(new_n50_), .O(new_n67_));
  oai21  g24(.a(new_n64_), .b(new_n44_), .c(new_n67_), .O(z03));
  nor2   g25(.a(x12), .b(x11), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n61_), .c(x10), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x01), .c(new_n44_), .O(z04));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n44_), .O(z05));
  nand3  g30(.a(new_n61_), .b(x11), .c(x10), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x01), .c(new_n44_), .O(z06));
  nand3  g32(.a(x02), .b(x01), .c(x00), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(x11), .c(x10), .d(new_n46_), .O(new_n78_));
  nor2   g35(.a(new_n78_), .b(x12), .O(z07));
  nor2   g36(.a(x03), .b(new_n45_), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(x04), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n51_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(x08), .c(x07), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(new_n83_), .c(new_n80_), .O(new_n86_));
  aoi21  g43(.a(new_n86_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g44(.a(x11), .O(new_n88_));
  nor2   g45(.a(x14), .b(x13), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(x12), .c(new_n88_), .d(x02), .O(new_n90_));
  nor2   g47(.a(x16), .b(x15), .O(new_n91_));
  nor2   g48(.a(x22), .b(x21), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(x20), .O(new_n93_));
  oai21  g50(.a(new_n93_), .b(new_n90_), .c(new_n44_), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n60_), .O(new_n95_));
  nor3   g52(.a(new_n51_), .b(new_n60_), .c(x00), .O(new_n96_));
  nor2   g53(.a(x20), .b(x19), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(new_n92_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n95_), .O(z09));
  nand4  g56(.a(new_n92_), .b(x20), .c(x16), .d(x15), .O(new_n100_));
  oai21  g57(.a(new_n100_), .b(new_n90_), .c(new_n44_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n60_), .O(new_n102_));
  nand4  g59(.a(new_n97_), .b(new_n96_), .c(x22), .d(x21), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n102_), .O(z10));
  inv1   g61(.a(x16), .O(new_n105_));
  nand4  g62(.a(new_n92_), .b(x20), .c(new_n105_), .d(x15), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n90_), .c(new_n44_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n60_), .O(new_n108_));
  inv1   g65(.a(x22), .O(new_n109_));
  nand4  g66(.a(new_n97_), .b(new_n96_), .c(new_n109_), .d(x21), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n108_), .O(z11));
  inv1   g68(.a(x24), .O(new_n112_));
  nand2  g69(.a(x10), .b(x02), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(x01), .c(x00), .O(new_n114_));
  oai21  g71(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g72(.a(new_n65_), .b(x17), .c(new_n45_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n60_), .c(new_n44_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n114_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n112_), .c(x09), .O(new_n120_));
  inv1   g77(.a(new_n120_), .O(z12));
  aoi21  g78(.a(new_n116_), .b(new_n44_), .c(x01), .O(z13));
  nand3  g79(.a(new_n52_), .b(new_n46_), .c(new_n45_), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(new_n124_));
  nand4  g81(.a(new_n124_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n125_));
  nor2   g82(.a(new_n125_), .b(x19), .O(z14));
  nand3  g83(.a(new_n52_), .b(x19), .c(new_n45_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n114_), .O(z15));
  xor2a  g85(.a(x01), .b(x00), .O(z16));
  nand2  g86(.a(new_n52_), .b(x02), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z17));
endmodule


