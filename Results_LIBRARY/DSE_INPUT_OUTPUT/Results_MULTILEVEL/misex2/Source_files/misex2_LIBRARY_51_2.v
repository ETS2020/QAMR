// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n139_, new_n141_, new_n142_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  inv1   g010(.a(x06), .O(new_n54_));
  nand2  g011(.a(new_n54_), .b(x01), .O(new_n55_));
  nand4  g012(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n56_));
  inv1   g013(.a(x19), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(z01));
  inv1   g016(.a(x10), .O(new_n60_));
  nand4  g017(.a(new_n57_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n61_));
  oai21  g018(.a(new_n61_), .b(new_n56_), .c(new_n55_), .O(z02));
  nand3  g019(.a(new_n49_), .b(x02), .c(x00), .O(new_n63_));
  nand3  g020(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  oai21  g021(.a(new_n64_), .b(new_n63_), .c(x06), .O(new_n65_));
  nand2  g022(.a(new_n65_), .b(x01), .O(new_n66_));
  nor3   g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n57_), .c(x18), .d(new_n44_), .O(new_n68_));
  nand2  g025(.a(new_n68_), .b(new_n66_), .O(z03));
  inv1   g026(.a(x11), .O(new_n70_));
  nand4  g027(.a(x06), .b(x02), .c(x01), .d(x00), .O(new_n71_));
  nor3   g028(.a(new_n71_), .b(new_n60_), .c(x09), .O(new_n72_));
  nand2  g029(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(x12), .O(z04));
  nand4  g031(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n75_));
  aoi21  g032(.a(new_n75_), .b(x06), .c(new_n47_), .O(z05));
  nand2  g033(.a(new_n72_), .b(x11), .O(new_n77_));
  inv1   g034(.a(new_n77_), .O(z06));
  inv1   g035(.a(x12), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x11), .c(x10), .d(new_n49_), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n55_), .O(z07));
  nand2  g040(.a(x02), .b(new_n46_), .O(new_n84_));
  inv1   g041(.a(x05), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x04), .O(new_n86_));
  nor3   g043(.a(new_n86_), .b(new_n84_), .c(x03), .O(new_n87_));
  nand3  g044(.a(x19), .b(new_n45_), .c(x17), .O(new_n88_));
  nor3   g045(.a(new_n88_), .b(x08), .c(x07), .O(new_n89_));
  nand2  g046(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  aoi21  g047(.a(new_n90_), .b(new_n47_), .c(x06), .O(z08));
  inv1   g048(.a(x21), .O(new_n92_));
  inv1   g049(.a(x22), .O(new_n93_));
  nand4  g050(.a(x12), .b(new_n70_), .c(x02), .d(new_n47_), .O(new_n94_));
  inv1   g051(.a(x15), .O(new_n95_));
  inv1   g052(.a(x16), .O(new_n96_));
  nor2   g053(.a(x14), .b(x13), .O(new_n97_));
  nand4  g054(.a(new_n97_), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  nor2   g055(.a(x20), .b(x19), .O(new_n99_));
  nand4  g056(.a(new_n99_), .b(x18), .c(x06), .d(x01), .O(new_n100_));
  oai21  g057(.a(new_n98_), .b(new_n94_), .c(new_n100_), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(new_n93_), .c(new_n92_), .d(new_n46_), .O(new_n102_));
  inv1   g059(.a(new_n102_), .O(z09));
  nand3  g060(.a(x18), .b(x06), .c(x01), .O(new_n104_));
  inv1   g061(.a(new_n104_), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n99_), .c(x22), .d(x21), .O(new_n106_));
  nand2  g063(.a(x02), .b(new_n47_), .O(new_n107_));
  inv1   g064(.a(x13), .O(new_n108_));
  nand3  g065(.a(new_n108_), .b(x12), .c(new_n70_), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nor3   g067(.a(new_n96_), .b(new_n95_), .c(x14), .O(new_n111_));
  nor2   g068(.a(x22), .b(x21), .O(new_n112_));
  nand4  g069(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(x20), .O(new_n113_));
  aoi21  g070(.a(new_n113_), .b(new_n106_), .c(x00), .O(z10));
  nor2   g071(.a(new_n48_), .b(x01), .O(new_n115_));
  nor2   g072(.a(x13), .b(new_n79_), .O(new_n116_));
  nand3  g073(.a(new_n116_), .b(new_n115_), .c(new_n70_), .O(new_n117_));
  inv1   g074(.a(x20), .O(new_n118_));
  and2   g075(.a(x18), .b(x01), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(x21), .c(new_n118_), .d(new_n57_), .O(new_n120_));
  nor2   g077(.a(new_n95_), .b(x14), .O(new_n121_));
  nor2   g078(.a(x21), .b(new_n118_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n121_), .c(new_n96_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(new_n117_), .c(new_n120_), .O(new_n124_));
  nand3  g081(.a(new_n124_), .b(new_n93_), .c(new_n46_), .O(new_n125_));
  nand2  g082(.a(new_n125_), .b(new_n55_), .O(z11));
  inv1   g083(.a(x24), .O(new_n127_));
  nand2  g084(.a(x10), .b(x02), .O(new_n128_));
  nand4  g085(.a(new_n128_), .b(x06), .c(x01), .d(x00), .O(new_n129_));
  oai21  g086(.a(x19), .b(x02), .c(x23), .O(new_n130_));
  nand3  g087(.a(new_n57_), .b(x17), .c(new_n48_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n47_), .c(new_n46_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n129_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n127_), .c(x09), .O(new_n135_));
  inv1   g092(.a(new_n135_), .O(z12));
  nand3  g093(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n137_));
  nor3   g094(.a(new_n137_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g095(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x19), .O(z14));
  aoi22  g097(.a(new_n55_), .b(x00), .c(x19), .d(new_n47_), .O(new_n141_));
  nand4  g098(.a(new_n60_), .b(x06), .c(x01), .d(x00), .O(new_n142_));
  oai21  g099(.a(new_n141_), .b(x02), .c(new_n142_), .O(z15));
  aoi21  g100(.a(x06), .b(x00), .c(new_n47_), .O(z16));
  oai21  g101(.a(new_n107_), .b(x00), .c(new_n55_), .O(z17));
endmodule


