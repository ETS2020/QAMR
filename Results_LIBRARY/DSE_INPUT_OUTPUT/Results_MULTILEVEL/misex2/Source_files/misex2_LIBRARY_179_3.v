// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n124_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand3  g09(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x19), .O(z02));
  nand4  g16(.a(new_n48_), .b(x18), .c(new_n44_), .d(new_n46_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z03));
  nand2  g18(.a(x01), .b(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(z04));
  inv1   g20(.a(x00), .O(new_n65_));
  inv1   g21(.a(x01), .O(new_n66_));
  aoi21  g22(.a(new_n46_), .b(new_n66_), .c(new_n65_), .O(z07));
  inv1   g23(.a(x03), .O(new_n68_));
  nor2   g24(.a(new_n46_), .b(x01), .O(new_n69_));
  inv1   g25(.a(x04), .O(new_n70_));
  nor2   g26(.a(x05), .b(new_n70_), .O(new_n71_));
  nand4  g27(.a(new_n71_), .b(new_n69_), .c(new_n68_), .d(new_n65_), .O(new_n72_));
  nor3   g28(.a(x08), .b(x07), .c(x06), .O(new_n73_));
  nand4  g29(.a(new_n73_), .b(x19), .c(new_n45_), .d(x17), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n72_), .c(new_n62_), .O(z08));
  inv1   g31(.a(x21), .O(new_n76_));
  inv1   g32(.a(x22), .O(new_n77_));
  inv1   g33(.a(x11), .O(new_n78_));
  nand4  g34(.a(x12), .b(new_n78_), .c(x02), .d(new_n66_), .O(new_n79_));
  inv1   g35(.a(x15), .O(new_n80_));
  inv1   g36(.a(x16), .O(new_n81_));
  nor2   g37(.a(x14), .b(x13), .O(new_n82_));
  nand4  g38(.a(new_n82_), .b(x20), .c(new_n81_), .d(new_n80_), .O(new_n83_));
  inv1   g39(.a(x19), .O(new_n84_));
  inv1   g40(.a(x20), .O(new_n85_));
  and2   g41(.a(x18), .b(x01), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(new_n87_));
  oai21  g43(.a(new_n83_), .b(new_n79_), .c(new_n87_), .O(new_n88_));
  nand4  g44(.a(new_n88_), .b(new_n77_), .c(new_n76_), .d(new_n65_), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(z09));
  nor3   g46(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n91_));
  nand4  g47(.a(new_n91_), .b(new_n84_), .c(x18), .d(x01), .O(new_n92_));
  inv1   g48(.a(x13), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(x12), .c(new_n78_), .O(new_n94_));
  inv1   g50(.a(new_n94_), .O(new_n95_));
  nor3   g51(.a(new_n81_), .b(new_n80_), .c(x14), .O(new_n96_));
  nand3  g52(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n97_));
  inv1   g53(.a(new_n97_), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(new_n96_), .c(new_n95_), .d(new_n69_), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(new_n92_), .c(x00), .O(z10));
  inv1   g56(.a(x12), .O(new_n101_));
  nor2   g57(.a(x13), .b(new_n101_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n69_), .c(new_n78_), .O(new_n103_));
  nand4  g59(.a(new_n86_), .b(x21), .c(new_n85_), .d(new_n84_), .O(new_n104_));
  nor2   g60(.a(new_n80_), .b(x14), .O(new_n105_));
  nor2   g61(.a(x21), .b(new_n85_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n105_), .c(new_n81_), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n103_), .c(new_n104_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n77_), .c(new_n65_), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(z11));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g67(.a(new_n84_), .b(x17), .c(new_n46_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n111_), .c(x24), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(x09), .c(new_n66_), .d(new_n65_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n62_), .O(z12));
  inv1   g71(.a(new_n48_), .O(new_n116_));
  oai21  g72(.a(new_n112_), .b(new_n116_), .c(new_n62_), .O(z13));
  nand3  g73(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n118_));
  nand4  g74(.a(new_n84_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n118_), .c(new_n62_), .O(z14));
  nor2   g76(.a(x19), .b(x00), .O(new_n121_));
  nor3   g77(.a(new_n121_), .b(x02), .c(x01), .O(z15));
  nor2   g78(.a(new_n66_), .b(x00), .O(z16));
  nand2  g79(.a(new_n48_), .b(x02), .O(new_n124_));
  inv1   g80(.a(new_n124_), .O(z17));
  zero   g81(.O(z06));
  inv1   g82(.a(new_n62_), .O(z05));
endmodule


