// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n144_;
  nand2  g000(.a(x17), .b(x01), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor2   g003(.a(x01), .b(x00), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g005(.a(x17), .O(new_n49_));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x19), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g010(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  nor3   g013(.a(x02), .b(x01), .c(x00), .O(new_n57_));
  nand2  g014(.a(new_n57_), .b(x09), .O(new_n58_));
  inv1   g015(.a(new_n58_), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x19), .O(z02));
  nand3  g018(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  inv1   g019(.a(new_n62_), .O(new_n63_));
  nor2   g020(.a(new_n56_), .b(x09), .O(new_n64_));
  nand4  g021(.a(new_n64_), .b(new_n63_), .c(x12), .d(x11), .O(new_n65_));
  nand4  g022(.a(new_n47_), .b(new_n51_), .c(x18), .d(new_n45_), .O(new_n66_));
  aoi21  g023(.a(new_n66_), .b(new_n65_), .c(x17), .O(z03));
  inv1   g024(.a(x00), .O(new_n68_));
  inv1   g025(.a(x01), .O(new_n69_));
  nor2   g026(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(x10), .c(new_n46_), .d(x02), .O(new_n71_));
  nor4   g028(.a(new_n71_), .b(x17), .c(x12), .d(x11), .O(z04));
  nand4  g029(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n73_));
  aoi21  g030(.a(new_n73_), .b(new_n49_), .c(new_n69_), .O(z05));
  inv1   g031(.a(x11), .O(new_n75_));
  nor3   g032(.a(new_n71_), .b(x17), .c(new_n75_), .O(z06));
  inv1   g033(.a(x12), .O(new_n77_));
  nand3  g034(.a(new_n64_), .b(new_n77_), .c(x11), .O(new_n78_));
  nand2  g035(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g036(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(new_n44_), .O(z07));
  inv1   g038(.a(x08), .O(new_n82_));
  inv1   g039(.a(x05), .O(new_n83_));
  inv1   g040(.a(x06), .O(new_n84_));
  inv1   g041(.a(x03), .O(new_n85_));
  nand4  g042(.a(new_n85_), .b(x02), .c(new_n69_), .d(new_n68_), .O(new_n86_));
  inv1   g043(.a(new_n86_), .O(new_n87_));
  nand4  g044(.a(new_n87_), .b(new_n84_), .c(new_n83_), .d(x04), .O(new_n88_));
  nor2   g045(.a(new_n88_), .b(x07), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(new_n50_), .c(x17), .d(new_n82_), .O(new_n90_));
  nor2   g047(.a(new_n90_), .b(new_n51_), .O(z08));
  inv1   g048(.a(x21), .O(new_n92_));
  inv1   g049(.a(x22), .O(new_n93_));
  nor2   g050(.a(new_n45_), .b(x01), .O(new_n94_));
  nand3  g051(.a(new_n94_), .b(x12), .c(new_n75_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  nor2   g056(.a(x20), .b(x19), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x18), .c(new_n49_), .d(x01), .O(new_n101_));
  oai21  g058(.a(new_n99_), .b(new_n95_), .c(new_n101_), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(new_n93_), .c(new_n92_), .d(new_n68_), .O(new_n103_));
  inv1   g060(.a(new_n103_), .O(z09));
  nand3  g061(.a(x18), .b(new_n49_), .c(x01), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n100_), .c(x22), .d(x21), .O(new_n107_));
  nor2   g064(.a(x13), .b(new_n77_), .O(new_n108_));
  nand3  g065(.a(new_n108_), .b(new_n94_), .c(new_n75_), .O(new_n109_));
  nor3   g066(.a(new_n97_), .b(new_n96_), .c(x14), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n93_), .c(new_n92_), .d(x20), .O(new_n111_));
  or2    g068(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  aoi21  g069(.a(new_n112_), .b(new_n107_), .c(x00), .O(z10));
  nor2   g070(.a(new_n92_), .b(x20), .O(new_n114_));
  nand3  g071(.a(new_n114_), .b(new_n106_), .c(new_n51_), .O(new_n115_));
  nor2   g072(.a(new_n96_), .b(x14), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n92_), .c(x20), .d(new_n97_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(new_n109_), .c(new_n115_), .O(new_n118_));
  nand3  g075(.a(new_n118_), .b(new_n93_), .c(new_n68_), .O(new_n119_));
  inv1   g076(.a(new_n119_), .O(z11));
  inv1   g077(.a(x24), .O(new_n121_));
  nand2  g078(.a(x10), .b(x02), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n49_), .c(x01), .d(x00), .O(new_n123_));
  inv1   g080(.a(x23), .O(new_n124_));
  nor2   g081(.a(x19), .b(x02), .O(new_n125_));
  oai21  g082(.a(new_n125_), .b(x23), .c(x17), .O(new_n126_));
  oai21  g083(.a(new_n125_), .b(new_n124_), .c(new_n126_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n69_), .c(new_n68_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n123_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n121_), .c(x09), .O(new_n130_));
  inv1   g087(.a(new_n130_), .O(z12));
  nand2  g088(.a(new_n57_), .b(x17), .O(new_n132_));
  nor2   g089(.a(new_n132_), .b(x19), .O(z13));
  nand2  g090(.a(new_n57_), .b(new_n46_), .O(new_n134_));
  inv1   g091(.a(new_n134_), .O(new_n135_));
  nand4  g092(.a(new_n135_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n136_));
  nor2   g093(.a(new_n136_), .b(x19), .O(z14));
  oai21  g094(.a(x10), .b(new_n69_), .c(x02), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(x00), .O(new_n139_));
  nand3  g096(.a(x19), .b(new_n45_), .c(new_n69_), .O(new_n140_));
  nand3  g097(.a(new_n140_), .b(new_n139_), .c(new_n44_), .O(z15));
  nand3  g098(.a(new_n49_), .b(x01), .c(new_n68_), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(z16));
  nand2  g100(.a(new_n94_), .b(new_n68_), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(new_n44_), .O(z17));
endmodule


