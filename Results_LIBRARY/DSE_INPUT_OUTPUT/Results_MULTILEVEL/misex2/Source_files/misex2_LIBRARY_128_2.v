// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n133_, new_n135_, new_n136_, new_n139_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  inv1   g09(.a(x12), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(x01), .O(new_n54_));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  inv1   g12(.a(x19), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n55_), .c(new_n54_), .O(z02));
  nand2  g18(.a(x02), .b(x00), .O(new_n62_));
  nand3  g19(.a(x11), .b(x10), .c(new_n47_), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(x12), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x01), .O(new_n65_));
  nor3   g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n56_), .c(x18), .d(new_n44_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n65_), .O(z03));
  inv1   g25(.a(new_n54_), .O(z04));
  inv1   g26(.a(x01), .O(new_n70_));
  nand4  g27(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x12), .c(new_n70_), .O(z05));
  nand3  g29(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(x11), .c(x10), .d(new_n47_), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n53_), .O(z06));
  nand3  g33(.a(x02), .b(new_n70_), .c(x00), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z07));
  inv1   g35(.a(x00), .O(new_n79_));
  inv1   g36(.a(x03), .O(new_n80_));
  nand2  g37(.a(x02), .b(new_n70_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  inv1   g39(.a(x04), .O(new_n83_));
  nor2   g40(.a(x05), .b(new_n83_), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n82_), .c(new_n80_), .d(new_n79_), .O(new_n85_));
  nor3   g42(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x19), .c(new_n45_), .d(x17), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n85_), .c(new_n54_), .O(z08));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  inv1   g47(.a(x11), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x02), .d(new_n70_), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand4  g53(.a(x20), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n56_), .c(x18), .d(x01), .O(new_n99_));
  oai21  g56(.a(new_n97_), .b(new_n93_), .c(new_n99_), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n90_), .c(new_n89_), .d(x12), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(x00), .O(z09));
  nand3  g59(.a(new_n56_), .b(x18), .c(x01), .O(new_n103_));
  nand3  g60(.a(x22), .b(x21), .c(new_n98_), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g62(.a(new_n92_), .b(x12), .c(new_n91_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n81_), .O(new_n107_));
  nand3  g64(.a(x16), .b(x15), .c(new_n94_), .O(new_n108_));
  nand3  g65(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n107_), .c(new_n105_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n54_), .O(z10));
  and2   g69(.a(x18), .b(x01), .O(new_n113_));
  nand4  g70(.a(new_n113_), .b(x21), .c(new_n98_), .d(new_n56_), .O(new_n114_));
  nor2   g71(.a(new_n95_), .b(x14), .O(new_n115_));
  nor2   g72(.a(x21), .b(new_n98_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n115_), .c(new_n96_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n93_), .c(new_n114_), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n90_), .c(x12), .d(new_n79_), .O(new_n119_));
  inv1   g76(.a(new_n119_), .O(z11));
  inv1   g77(.a(x24), .O(new_n121_));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(x01), .c(x00), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n56_), .b(x17), .c(new_n46_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n70_), .c(new_n79_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n121_), .c(x09), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n54_), .O(z12));
  inv1   g87(.a(new_n48_), .O(new_n131_));
  oai21  g88(.a(new_n125_), .b(new_n131_), .c(new_n54_), .O(z13));
  nand3  g89(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n133_));
  oai21  g90(.a(new_n133_), .b(new_n60_), .c(new_n54_), .O(z14));
  aoi22  g91(.a(new_n54_), .b(x00), .c(x19), .d(new_n70_), .O(new_n135_));
  nand4  g92(.a(x12), .b(new_n59_), .c(x01), .d(x00), .O(new_n136_));
  oai21  g93(.a(new_n135_), .b(x02), .c(new_n136_), .O(z15));
  aoi21  g94(.a(x12), .b(x00), .c(new_n70_), .O(z16));
  nand2  g95(.a(new_n48_), .b(x02), .O(new_n139_));
  inv1   g96(.a(new_n139_), .O(z17));
endmodule


