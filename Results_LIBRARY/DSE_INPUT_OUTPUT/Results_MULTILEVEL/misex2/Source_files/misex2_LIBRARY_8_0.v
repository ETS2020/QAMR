// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n127_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  inv1   g03(.a(x00), .O(new_n47_));
  inv1   g04(.a(x01), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nor3   g07(.a(new_n50_), .b(new_n46_), .c(x09), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x11), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  inv1   g10(.a(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x02), .O(new_n55_));
  nor2   g12(.a(x17), .b(new_n46_), .O(new_n56_));
  nor2   g13(.a(x19), .b(x18), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n56_), .c(new_n55_), .d(new_n48_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(x11), .c(x00), .O(z01));
  nor3   g16(.a(new_n50_), .b(x10), .c(new_n54_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(x11), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x19), .O(z02));
  nand2  g19(.a(new_n49_), .b(new_n48_), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x18), .c(new_n44_), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n63_), .c(x11), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n47_), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g25(.a(new_n46_), .b(x09), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  inv1   g28(.a(x11), .O(new_n72_));
  nand3  g29(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(new_n72_), .c(x10), .d(new_n54_), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(x12), .O(z04));
  nand2  g33(.a(x01), .b(x00), .O(new_n77_));
  nand2  g34(.a(new_n72_), .b(new_n47_), .O(new_n78_));
  nand3  g35(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n77_), .c(new_n78_), .O(z05));
  nand3  g37(.a(x11), .b(x10), .c(new_n54_), .O(new_n81_));
  oai21  g38(.a(new_n81_), .b(new_n68_), .c(new_n78_), .O(z06));
  nor2   g39(.a(x12), .b(new_n72_), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n69_), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x01), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n78_), .O(z07));
  inv1   g44(.a(x05), .O(new_n88_));
  inv1   g45(.a(x06), .O(new_n89_));
  inv1   g46(.a(x07), .O(new_n90_));
  inv1   g47(.a(x04), .O(new_n91_));
  inv1   g48(.a(x03), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(new_n88_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(x08), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n45_), .c(x17), .d(x11), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n64_), .O(z08));
  inv1   g55(.a(x20), .O(new_n99_));
  nand3  g56(.a(x11), .b(x01), .c(new_n47_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z16));
  nand4  g58(.a(z16), .b(new_n99_), .c(new_n64_), .d(x18), .O(new_n102_));
  nor3   g59(.a(new_n102_), .b(x22), .c(x21), .O(z09));
  inv1   g60(.a(x22), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  or2    g62(.a(new_n102_), .b(new_n105_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n104_), .O(z10));
  nor2   g64(.a(new_n106_), .b(x22), .O(z11));
  nand2  g65(.a(x10), .b(x02), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x01), .c(x00), .O(new_n110_));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g68(.a(new_n64_), .b(x17), .c(new_n49_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n48_), .c(new_n47_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n110_), .c(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n78_), .O(z12));
  nand4  g74(.a(new_n64_), .b(x17), .c(new_n49_), .d(new_n48_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(x11), .c(x00), .O(z13));
  nor3   g76(.a(x09), .b(x02), .c(x01), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n57_), .c(new_n44_), .d(new_n46_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(x11), .c(x00), .O(z14));
  oai21  g79(.a(x10), .b(new_n48_), .c(x02), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x00), .O(new_n124_));
  nand3  g81(.a(x19), .b(new_n49_), .c(new_n48_), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(new_n124_), .c(new_n78_), .O(z15));
  nand2  g83(.a(x02), .b(new_n48_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(x11), .c(x00), .O(z17));
endmodule


