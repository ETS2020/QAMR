// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x13), .O(new_n44_));
  nor3   g001(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  inv1   g002(.a(x10), .O(new_n46_));
  nor2   g003(.a(new_n46_), .b(x09), .O(new_n47_));
  nor2   g004(.a(x18), .b(x17), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  aoi21  g006(.a(new_n49_), .b(new_n44_), .c(x19), .O(z00));
  nand4  g007(.a(new_n48_), .b(new_n45_), .c(x10), .d(x09), .O(new_n51_));
  aoi21  g008(.a(new_n51_), .b(new_n44_), .c(x19), .O(z01));
  nand4  g009(.a(new_n48_), .b(new_n45_), .c(new_n46_), .d(x09), .O(new_n53_));
  aoi21  g010(.a(new_n53_), .b(new_n44_), .c(x19), .O(z02));
  inv1   g011(.a(x19), .O(new_n55_));
  inv1   g012(.a(x00), .O(new_n56_));
  inv1   g013(.a(x01), .O(new_n57_));
  nand2  g014(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  inv1   g015(.a(x02), .O(new_n59_));
  inv1   g016(.a(x17), .O(new_n60_));
  nand3  g017(.a(x18), .b(new_n60_), .c(new_n59_), .O(new_n61_));
  oai21  g018(.a(new_n61_), .b(new_n58_), .c(new_n44_), .O(new_n62_));
  nand2  g019(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  nand3  g020(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand3  g021(.a(new_n47_), .b(x12), .c(x11), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(z03));
  inv1   g023(.a(x09), .O(new_n67_));
  inv1   g024(.a(x11), .O(new_n68_));
  inv1   g025(.a(x12), .O(new_n69_));
  nor2   g026(.a(x19), .b(new_n44_), .O(new_n70_));
  inv1   g027(.a(new_n70_), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n69_), .c(new_n68_), .d(x10), .O(new_n72_));
  inv1   g029(.a(new_n72_), .O(new_n73_));
  nand4  g030(.a(new_n73_), .b(new_n67_), .c(x02), .d(x01), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(new_n56_), .O(z04));
  nand2  g032(.a(x01), .b(x00), .O(new_n76_));
  nand3  g033(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g034(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z05));
  nor2   g035(.a(new_n70_), .b(new_n68_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x10), .c(new_n67_), .d(x02), .O(new_n80_));
  nor3   g037(.a(new_n80_), .b(new_n57_), .c(new_n56_), .O(z06));
  nand3  g038(.a(new_n47_), .b(new_n69_), .c(x11), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n71_), .O(z07));
  inv1   g042(.a(x08), .O(new_n86_));
  inv1   g043(.a(x18), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  inv1   g045(.a(x06), .O(new_n89_));
  inv1   g046(.a(x03), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(x02), .c(new_n57_), .d(new_n56_), .O(new_n91_));
  inv1   g048(.a(new_n91_), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n89_), .c(new_n88_), .d(x04), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(x07), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n87_), .c(x17), .d(new_n86_), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(new_n55_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nand4  g055(.a(x12), .b(new_n68_), .c(x02), .d(new_n57_), .O(new_n99_));
  inv1   g056(.a(x14), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nand4  g059(.a(x20), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  nor2   g061(.a(new_n87_), .b(new_n57_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n104_), .c(new_n55_), .O(new_n106_));
  oai21  g063(.a(new_n103_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n98_), .c(new_n97_), .d(new_n44_), .O(new_n108_));
  nor2   g065(.a(new_n108_), .b(x00), .O(z09));
  nand3  g066(.a(new_n55_), .b(x18), .c(x01), .O(new_n110_));
  nand3  g067(.a(x22), .b(x21), .c(new_n104_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g069(.a(x02), .b(new_n57_), .O(new_n113_));
  nand3  g070(.a(new_n44_), .b(x12), .c(new_n68_), .O(new_n114_));
  nor2   g071(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g072(.a(x16), .b(x15), .c(new_n100_), .O(new_n116_));
  nand3  g073(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n117_));
  nor2   g074(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  aoi21  g075(.a(new_n118_), .b(new_n115_), .c(new_n112_), .O(new_n119_));
  oai21  g076(.a(new_n119_), .b(x00), .c(new_n71_), .O(z10));
  nand4  g077(.a(new_n105_), .b(x21), .c(new_n104_), .d(new_n55_), .O(new_n121_));
  nor2   g078(.a(new_n101_), .b(x14), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n97_), .c(x20), .d(new_n102_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(new_n99_), .c(new_n121_), .O(new_n124_));
  nand4  g081(.a(new_n124_), .b(new_n98_), .c(new_n44_), .d(new_n56_), .O(new_n125_));
  inv1   g082(.a(new_n125_), .O(z11));
  inv1   g083(.a(x24), .O(new_n127_));
  nand2  g084(.a(x10), .b(x02), .O(new_n128_));
  nand4  g085(.a(new_n128_), .b(new_n71_), .c(x01), .d(x00), .O(new_n129_));
  nand3  g086(.a(new_n55_), .b(x17), .c(new_n59_), .O(new_n130_));
  nand2  g087(.a(x23), .b(x02), .O(new_n131_));
  aoi21  g088(.a(new_n131_), .b(new_n130_), .c(x13), .O(new_n132_));
  and2   g089(.a(x23), .b(x19), .O(new_n133_));
  oai21  g090(.a(new_n133_), .b(new_n132_), .c(new_n57_), .O(new_n134_));
  oai21  g091(.a(new_n134_), .b(x00), .c(new_n129_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n127_), .c(x09), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(z12));
  nor2   g094(.a(new_n58_), .b(x02), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(x17), .c(new_n44_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x19), .O(z13));
  nand2  g097(.a(new_n138_), .b(new_n67_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x10), .O(new_n142_));
  nand4  g099(.a(new_n142_), .b(new_n87_), .c(new_n60_), .d(new_n44_), .O(new_n143_));
  nor2   g100(.a(new_n143_), .b(x19), .O(z14));
  oai21  g101(.a(x10), .b(new_n57_), .c(x02), .O(new_n145_));
  nand2  g102(.a(new_n145_), .b(x00), .O(new_n146_));
  nand3  g103(.a(x19), .b(new_n59_), .c(new_n57_), .O(new_n147_));
  nand3  g104(.a(new_n147_), .b(new_n146_), .c(new_n71_), .O(z15));
  nor3   g105(.a(new_n70_), .b(new_n57_), .c(x00), .O(z16));
  oai21  g106(.a(new_n113_), .b(x00), .c(new_n71_), .O(z17));
endmodule


