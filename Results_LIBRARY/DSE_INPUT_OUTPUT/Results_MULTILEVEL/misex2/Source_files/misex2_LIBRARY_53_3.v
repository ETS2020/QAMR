// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n142_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  nand2  g010(.a(x14), .b(x01), .O(new_n54_));
  nand4  g011(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n55_));
  inv1   g012(.a(x19), .O(new_n56_));
  nand4  g013(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n57_));
  oai21  g014(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z01));
  inv1   g015(.a(x10), .O(new_n59_));
  nand4  g016(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n60_));
  oai21  g017(.a(new_n60_), .b(new_n55_), .c(new_n54_), .O(z02));
  inv1   g018(.a(x14), .O(new_n62_));
  nand3  g019(.a(new_n49_), .b(x02), .c(x00), .O(new_n63_));
  nand3  g020(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  oai21  g021(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  nand2  g022(.a(new_n65_), .b(x01), .O(new_n66_));
  nor3   g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g024(.a(x19), .b(new_n45_), .O(new_n68_));
  nand3  g025(.a(new_n68_), .b(new_n67_), .c(new_n44_), .O(new_n69_));
  nand2  g026(.a(new_n69_), .b(new_n66_), .O(z03));
  inv1   g027(.a(x11), .O(new_n71_));
  inv1   g028(.a(x12), .O(new_n72_));
  inv1   g029(.a(new_n63_), .O(new_n73_));
  nand4  g030(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x10), .O(new_n74_));
  aoi21  g031(.a(new_n74_), .b(new_n62_), .c(new_n47_), .O(z04));
  nand3  g032(.a(x02), .b(x01), .c(x00), .O(new_n76_));
  nor4   g033(.a(new_n76_), .b(x14), .c(new_n59_), .d(new_n49_), .O(z05));
  inv1   g034(.a(new_n76_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x11), .c(x10), .d(new_n49_), .O(new_n79_));
  nor2   g036(.a(new_n79_), .b(x14), .O(z06));
  nand2  g037(.a(x10), .b(new_n49_), .O(new_n81_));
  nand3  g038(.a(new_n62_), .b(new_n72_), .c(x11), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(new_n81_), .c(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g041(.a(new_n84_), .O(z07));
  nor2   g042(.a(new_n48_), .b(x01), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(new_n46_), .O(new_n87_));
  inv1   g044(.a(x03), .O(new_n88_));
  inv1   g045(.a(x05), .O(new_n89_));
  nand3  g046(.a(new_n89_), .b(x04), .c(new_n88_), .O(new_n90_));
  nor2   g047(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nor3   g048(.a(x08), .b(x07), .c(x06), .O(new_n92_));
  nor2   g049(.a(new_n56_), .b(x18), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(x17), .O(new_n94_));
  nand2  g051(.a(new_n94_), .b(new_n54_), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  inv1   g053(.a(x22), .O(new_n97_));
  nand4  g054(.a(x12), .b(new_n71_), .c(x02), .d(new_n47_), .O(new_n98_));
  inv1   g055(.a(x13), .O(new_n99_));
  inv1   g056(.a(x15), .O(new_n100_));
  inv1   g057(.a(x16), .O(new_n101_));
  nand4  g058(.a(x20), .b(new_n101_), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g059(.a(x20), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n56_), .c(x18), .d(x01), .O(new_n104_));
  oai21  g061(.a(new_n102_), .b(new_n98_), .c(new_n104_), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n97_), .c(new_n96_), .d(new_n62_), .O(new_n106_));
  nor2   g063(.a(new_n106_), .b(x00), .O(z09));
  nor2   g064(.a(new_n97_), .b(new_n96_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n68_), .c(new_n103_), .d(x01), .O(new_n109_));
  nand4  g066(.a(new_n86_), .b(new_n99_), .c(x12), .d(new_n71_), .O(new_n110_));
  nor2   g067(.a(x22), .b(x21), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(x20), .c(x16), .d(x15), .O(new_n112_));
  oai21  g069(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(new_n62_), .c(new_n46_), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(z10));
  and2   g072(.a(x18), .b(x01), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(x21), .c(new_n103_), .d(new_n56_), .O(new_n117_));
  nor2   g074(.a(new_n100_), .b(x13), .O(new_n118_));
  nor2   g075(.a(x21), .b(new_n103_), .O(new_n119_));
  nand3  g076(.a(new_n119_), .b(new_n118_), .c(new_n101_), .O(new_n120_));
  oai21  g077(.a(new_n120_), .b(new_n98_), .c(new_n117_), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n97_), .c(new_n62_), .d(new_n46_), .O(new_n122_));
  inv1   g079(.a(new_n122_), .O(z11));
  inv1   g080(.a(x24), .O(new_n124_));
  nand2  g081(.a(x10), .b(x02), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  oai21  g083(.a(x19), .b(x02), .c(x23), .O(new_n127_));
  nand3  g084(.a(new_n56_), .b(x17), .c(new_n48_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n47_), .c(new_n46_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n124_), .c(x09), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n54_), .O(z12));
  nand3  g090(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n134_));
  nor3   g091(.a(new_n134_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g092(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n136_));
  nor2   g093(.a(new_n136_), .b(x19), .O(z14));
  oai21  g094(.a(x10), .b(new_n47_), .c(x02), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(x00), .O(new_n139_));
  nand3  g096(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n140_));
  nand3  g097(.a(new_n140_), .b(new_n139_), .c(new_n54_), .O(z15));
  nand3  g098(.a(new_n62_), .b(x01), .c(new_n46_), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(z16));
  nand2  g100(.a(new_n87_), .b(new_n54_), .O(z17));
endmodule


