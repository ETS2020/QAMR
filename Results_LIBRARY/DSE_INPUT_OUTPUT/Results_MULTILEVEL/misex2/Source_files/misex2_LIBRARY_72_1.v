// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n101_, new_n103_, new_n104_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor3   g02(.a(x02), .b(x01), .c(x00), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(x10), .d(new_n44_), .O(new_n47_));
  nor3   g04(.a(new_n47_), .b(x19), .c(x18), .O(z00));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x00), .O(new_n50_));
  nor3   g07(.a(x19), .b(x18), .c(x17), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(x10), .c(x09), .d(new_n50_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n49_), .c(x01), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n51_), .b(new_n54_), .c(x09), .d(new_n50_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n49_), .c(x01), .O(z02));
  inv1   g13(.a(x01), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(x18), .c(new_n45_), .d(new_n50_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n49_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand3  g18(.a(new_n44_), .b(x02), .c(x00), .O(new_n62_));
  nand3  g19(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z03));
  nor2   g21(.a(x12), .b(x11), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(x10), .c(new_n44_), .d(x00), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(x01), .c(new_n49_), .O(z04));
  nand3  g24(.a(x10), .b(x09), .c(x00), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(x01), .c(new_n49_), .O(z05));
  nand3  g26(.a(x11), .b(x10), .c(new_n44_), .O(new_n70_));
  nor4   g27(.a(new_n70_), .b(new_n49_), .c(new_n57_), .d(new_n50_), .O(z06));
  nand3  g28(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(x11), .c(x10), .d(new_n44_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(x12), .O(z07));
  nand2  g32(.a(x02), .b(new_n57_), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z08));
  nand3  g34(.a(x18), .b(x01), .c(new_n50_), .O(new_n78_));
  inv1   g35(.a(x20), .O(new_n79_));
  inv1   g36(.a(x21), .O(new_n80_));
  inv1   g37(.a(x22), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n58_), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(new_n78_), .c(new_n76_), .O(z09));
  nand4  g40(.a(new_n58_), .b(x18), .c(x01), .d(new_n50_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x22), .c(x21), .d(new_n79_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(z10));
  nand4  g44(.a(new_n81_), .b(x21), .c(new_n79_), .d(new_n58_), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n78_), .c(new_n76_), .O(z11));
  inv1   g46(.a(x24), .O(new_n90_));
  nand2  g47(.a(x10), .b(x02), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x01), .c(x00), .O(new_n92_));
  nand2  g49(.a(x23), .b(x19), .O(new_n93_));
  oai21  g50(.a(x19), .b(new_n45_), .c(new_n93_), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n49_), .c(new_n57_), .d(new_n50_), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n90_), .c(x09), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(z12));
  nand3  g55(.a(new_n58_), .b(x17), .c(new_n50_), .O(new_n99_));
  aoi21  g56(.a(new_n99_), .b(new_n49_), .c(x01), .O(z13));
  nand4  g57(.a(new_n51_), .b(new_n54_), .c(new_n44_), .d(new_n50_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n49_), .c(x01), .O(z14));
  nand2  g59(.a(new_n91_), .b(x00), .O(new_n103_));
  oai21  g60(.a(x19), .b(x02), .c(new_n57_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n103_), .O(z15));
  oai21  g62(.a(new_n57_), .b(x00), .c(new_n76_), .O(z16));
  inv1   g63(.a(new_n76_), .O(z17));
endmodule


