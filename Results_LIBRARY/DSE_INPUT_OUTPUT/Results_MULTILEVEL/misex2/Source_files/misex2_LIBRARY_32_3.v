// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n120_, new_n122_, new_n124_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand3  g09(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x01), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n57_), .O(z04));
  inv1   g16(.a(z04), .O(new_n60_));
  nand3  g17(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n61_));
  inv1   g18(.a(x10), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n45_), .c(new_n44_), .d(new_n62_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n61_), .c(new_n60_), .O(z02));
  nand3  g22(.a(new_n46_), .b(new_n58_), .c(new_n57_), .O(new_n66_));
  nand2  g23(.a(new_n63_), .b(x18), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n44_), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n66_), .c(new_n60_), .O(z03));
  aoi21  g27(.a(new_n46_), .b(new_n58_), .c(new_n57_), .O(z07));
  inv1   g28(.a(x08), .O(new_n73_));
  inv1   g29(.a(x05), .O(new_n74_));
  inv1   g30(.a(x06), .O(new_n75_));
  inv1   g31(.a(x03), .O(new_n76_));
  nand4  g32(.a(new_n76_), .b(x02), .c(new_n58_), .d(new_n57_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand4  g34(.a(new_n78_), .b(new_n75_), .c(new_n74_), .d(x04), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(x07), .O(new_n80_));
  nand4  g36(.a(new_n80_), .b(new_n45_), .c(x17), .d(new_n73_), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n63_), .O(z08));
  inv1   g38(.a(x21), .O(new_n83_));
  inv1   g39(.a(x22), .O(new_n84_));
  inv1   g40(.a(x11), .O(new_n85_));
  nor2   g41(.a(new_n46_), .b(x01), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(x12), .c(new_n85_), .O(new_n87_));
  inv1   g43(.a(x15), .O(new_n88_));
  inv1   g44(.a(x16), .O(new_n89_));
  nor2   g45(.a(x14), .b(x13), .O(new_n90_));
  nand4  g46(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  inv1   g47(.a(x20), .O(new_n92_));
  nand4  g48(.a(new_n92_), .b(new_n63_), .c(x18), .d(x01), .O(new_n93_));
  oai21  g49(.a(new_n91_), .b(new_n87_), .c(new_n93_), .O(new_n94_));
  nand4  g50(.a(new_n94_), .b(new_n84_), .c(new_n83_), .d(new_n57_), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(z09));
  nor2   g52(.a(new_n84_), .b(new_n83_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n68_), .c(new_n92_), .d(x01), .O(new_n98_));
  inv1   g54(.a(x13), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(x12), .c(new_n85_), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  nor3   g57(.a(new_n89_), .b(new_n88_), .c(x14), .O(new_n102_));
  nand3  g58(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  nand4  g60(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n86_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n98_), .c(x00), .O(z10));
  nand3  g62(.a(new_n84_), .b(x21), .c(new_n92_), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n67_), .c(new_n57_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(x01), .O(new_n109_));
  nor4   g65(.a(new_n103_), .b(x16), .c(new_n88_), .d(x14), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(new_n101_), .c(new_n86_), .d(new_n57_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n109_), .O(z11));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n113_));
  nand3  g69(.a(new_n63_), .b(x17), .c(new_n46_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n113_), .c(x24), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(x09), .c(new_n58_), .O(new_n116_));
  nor2   g72(.a(new_n116_), .b(x00), .O(z12));
  inv1   g73(.a(new_n48_), .O(new_n118_));
  oai21  g74(.a(new_n114_), .b(new_n118_), .c(new_n60_), .O(z13));
  nand4  g75(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n62_), .O(new_n120_));
  nor2   g76(.a(new_n120_), .b(x19), .O(z14));
  nand3  g77(.a(x19), .b(new_n46_), .c(new_n58_), .O(new_n122_));
  oai21  g78(.a(new_n86_), .b(new_n57_), .c(new_n122_), .O(z15));
  nand2  g79(.a(new_n48_), .b(x02), .O(new_n124_));
  inv1   g80(.a(new_n124_), .O(z17));
  zero   g81(.O(z06));
  nor2   g82(.a(new_n58_), .b(new_n57_), .O(z05));
  buf    g83(.a(x01), .O(z16));
endmodule


