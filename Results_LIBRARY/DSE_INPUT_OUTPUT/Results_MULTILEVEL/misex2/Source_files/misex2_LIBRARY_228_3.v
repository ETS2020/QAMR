// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n140_, new_n141_, new_n142_, new_n145_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  nand4  g008(.a(new_n47_), .b(new_n44_), .c(x10), .d(x09), .O(new_n52_));
  nor3   g009(.a(new_n52_), .b(x19), .c(x18), .O(z01));
  nand2  g010(.a(x17), .b(x13), .O(new_n54_));
  inv1   g011(.a(x02), .O(new_n55_));
  nor2   g012(.a(x01), .b(x00), .O(new_n56_));
  nand3  g013(.a(new_n56_), .b(x09), .c(new_n55_), .O(new_n57_));
  inv1   g014(.a(x10), .O(new_n58_));
  inv1   g015(.a(x19), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n60_));
  oai21  g017(.a(new_n60_), .b(new_n57_), .c(new_n54_), .O(z02));
  nor3   g018(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n59_), .c(x18), .d(new_n44_), .O(new_n63_));
  nand3  g020(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g021(.a(new_n64_), .O(new_n65_));
  nor2   g022(.a(new_n58_), .b(x09), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n65_), .c(x12), .d(x11), .O(new_n67_));
  nand3  g024(.a(new_n67_), .b(new_n63_), .c(new_n54_), .O(z03));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  nand3  g027(.a(new_n66_), .b(new_n70_), .c(new_n69_), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n64_), .c(new_n54_), .O(z04));
  inv1   g029(.a(x00), .O(new_n73_));
  inv1   g030(.a(x01), .O(new_n74_));
  nand4  g031(.a(new_n54_), .b(x10), .c(x09), .d(x02), .O(new_n75_));
  nor3   g032(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(z05));
  nand3  g033(.a(x11), .b(x10), .c(new_n46_), .O(new_n77_));
  oai21  g034(.a(new_n77_), .b(new_n64_), .c(new_n54_), .O(z06));
  nand3  g035(.a(new_n66_), .b(new_n70_), .c(x11), .O(new_n79_));
  nand2  g036(.a(new_n79_), .b(x01), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(new_n54_), .c(x02), .d(x00), .O(new_n81_));
  inv1   g038(.a(new_n81_), .O(z07));
  inv1   g039(.a(x13), .O(new_n83_));
  inv1   g040(.a(x03), .O(new_n84_));
  nand3  g041(.a(x04), .b(new_n84_), .c(x02), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(new_n86_));
  nor3   g043(.a(x07), .b(x06), .c(x05), .O(new_n87_));
  inv1   g044(.a(x08), .O(new_n88_));
  nand3  g045(.a(x19), .b(new_n45_), .c(new_n88_), .O(new_n89_));
  inv1   g046(.a(new_n89_), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(new_n87_), .c(new_n86_), .d(new_n56_), .O(new_n91_));
  aoi21  g048(.a(new_n91_), .b(new_n83_), .c(new_n44_), .O(z08));
  inv1   g049(.a(x21), .O(new_n93_));
  inv1   g050(.a(x22), .O(new_n94_));
  inv1   g051(.a(x20), .O(new_n95_));
  nand4  g052(.a(new_n54_), .b(new_n95_), .c(new_n59_), .d(x18), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n69_), .c(x02), .d(new_n74_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  oai22  g058(.a(new_n101_), .b(new_n97_), .c(new_n96_), .d(new_n74_), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(new_n94_), .c(new_n93_), .d(new_n73_), .O(new_n103_));
  inv1   g060(.a(new_n103_), .O(z09));
  nand4  g061(.a(new_n54_), .b(x22), .c(x21), .d(new_n95_), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n59_), .c(x18), .d(x01), .O(new_n107_));
  nor2   g064(.a(new_n55_), .b(x01), .O(new_n108_));
  nor2   g065(.a(x13), .b(new_n70_), .O(new_n109_));
  nand2  g066(.a(new_n109_), .b(new_n69_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nor3   g068(.a(new_n99_), .b(new_n98_), .c(x14), .O(new_n112_));
  nand3  g069(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n113_));
  inv1   g070(.a(new_n113_), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n108_), .O(new_n115_));
  aoi21  g072(.a(new_n115_), .b(new_n107_), .c(x00), .O(z10));
  nand3  g073(.a(new_n109_), .b(new_n108_), .c(new_n69_), .O(new_n117_));
  and2   g074(.a(x18), .b(x01), .O(new_n118_));
  nand4  g075(.a(new_n118_), .b(x21), .c(new_n95_), .d(new_n59_), .O(new_n119_));
  nor2   g076(.a(new_n98_), .b(x14), .O(new_n120_));
  nor2   g077(.a(x21), .b(new_n95_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n120_), .c(new_n99_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n117_), .c(new_n119_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n94_), .c(new_n73_), .O(new_n124_));
  nand2  g081(.a(new_n124_), .b(new_n54_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n59_), .b(x17), .c(new_n55_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n74_), .c(new_n73_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n126_), .c(x09), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n54_), .O(z12));
  nand3  g092(.a(new_n47_), .b(x17), .c(new_n83_), .O(new_n136_));
  nor2   g093(.a(new_n136_), .b(x19), .O(z13));
  nand4  g094(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n138_));
  nor2   g095(.a(new_n138_), .b(x19), .O(z14));
  oai21  g096(.a(x10), .b(new_n74_), .c(x02), .O(new_n140_));
  nand2  g097(.a(new_n140_), .b(x00), .O(new_n141_));
  nand3  g098(.a(x19), .b(new_n55_), .c(new_n74_), .O(new_n142_));
  nand3  g099(.a(new_n142_), .b(new_n141_), .c(new_n54_), .O(z15));
  oai21  g100(.a(new_n74_), .b(x00), .c(new_n54_), .O(z16));
  nand2  g101(.a(new_n108_), .b(new_n73_), .O(new_n145_));
  nand2  g102(.a(new_n145_), .b(new_n54_), .O(z17));
endmodule


