// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n144_;
  inv1   g000(.a(x20), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x11), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(x17), .O(new_n51_));
  nor2   g008(.a(x19), .b(x18), .O(new_n52_));
  nand3  g009(.a(new_n52_), .b(new_n51_), .c(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n50_), .c(new_n45_), .O(z00));
  inv1   g011(.a(x10), .O(new_n55_));
  inv1   g012(.a(x18), .O(new_n56_));
  inv1   g013(.a(new_n45_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(x19), .O(new_n58_));
  nand3  g015(.a(new_n58_), .b(new_n56_), .c(new_n51_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x00), .O(z01));
  nor2   g019(.a(new_n59_), .b(x10), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n64_));
  nor2   g021(.a(new_n64_), .b(x00), .O(z02));
  nand3  g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand3  g023(.a(x12), .b(x10), .c(new_n49_), .O(new_n67_));
  oai21  g024(.a(new_n67_), .b(new_n66_), .c(x20), .O(new_n68_));
  nand2  g025(.a(new_n68_), .b(x11), .O(new_n69_));
  inv1   g026(.a(x19), .O(new_n70_));
  nor3   g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n70_), .c(x18), .d(new_n51_), .O(new_n72_));
  nand2  g029(.a(new_n72_), .b(new_n69_), .O(z03));
  nor2   g030(.a(new_n47_), .b(new_n46_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(x10), .c(new_n49_), .d(x02), .O(new_n75_));
  nor3   g032(.a(new_n75_), .b(x12), .c(x11), .O(z04));
  nor2   g033(.a(new_n57_), .b(new_n55_), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(x09), .c(x02), .d(x01), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n46_), .O(z05));
  inv1   g036(.a(x11), .O(new_n80_));
  nor3   g037(.a(new_n75_), .b(new_n44_), .c(new_n80_), .O(z06));
  inv1   g038(.a(x12), .O(new_n82_));
  nand4  g039(.a(new_n82_), .b(x11), .c(x10), .d(new_n49_), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n45_), .O(z07));
  inv1   g043(.a(x03), .O(new_n87_));
  inv1   g044(.a(x04), .O(new_n88_));
  inv1   g045(.a(x05), .O(new_n89_));
  inv1   g046(.a(x06), .O(new_n90_));
  inv1   g047(.a(x07), .O(new_n91_));
  nand4  g048(.a(new_n45_), .b(x19), .c(new_n56_), .d(x17), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(x08), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n87_), .c(x02), .d(new_n47_), .O(new_n96_));
  nor2   g053(.a(new_n96_), .b(x00), .O(z08));
  inv1   g054(.a(x21), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  inv1   g056(.a(x13), .O(new_n100_));
  nor2   g057(.a(new_n48_), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(new_n100_), .c(x12), .O(new_n102_));
  inv1   g059(.a(x14), .O(new_n103_));
  inv1   g060(.a(x15), .O(new_n104_));
  inv1   g061(.a(x16), .O(new_n105_));
  nand4  g062(.a(x20), .b(new_n105_), .c(new_n104_), .d(new_n103_), .O(new_n106_));
  nor2   g063(.a(new_n56_), .b(new_n47_), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(new_n44_), .c(new_n70_), .O(new_n108_));
  oai21  g065(.a(new_n106_), .b(new_n102_), .c(new_n108_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n99_), .c(new_n98_), .d(new_n80_), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(x00), .O(z09));
  nand3  g068(.a(new_n70_), .b(x18), .c(x01), .O(new_n112_));
  nand3  g069(.a(x22), .b(x21), .c(new_n44_), .O(new_n113_));
  nand4  g070(.a(new_n101_), .b(new_n103_), .c(new_n100_), .d(x12), .O(new_n114_));
  nor2   g071(.a(x22), .b(x21), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(x20), .c(x16), .d(x15), .O(new_n116_));
  oai22  g073(.a(new_n116_), .b(new_n114_), .c(new_n113_), .d(new_n112_), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(new_n80_), .c(new_n46_), .O(new_n118_));
  inv1   g075(.a(new_n118_), .O(z10));
  nand4  g076(.a(new_n107_), .b(x21), .c(new_n44_), .d(new_n70_), .O(new_n120_));
  nor2   g077(.a(new_n104_), .b(x14), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n98_), .c(x20), .d(new_n105_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n102_), .c(new_n120_), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n99_), .c(new_n80_), .d(new_n46_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  nand2  g082(.a(x10), .b(x02), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  oai21  g084(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g085(.a(new_n70_), .b(x17), .c(new_n48_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(new_n47_), .c(new_n46_), .O(new_n131_));
  aoi21  g088(.a(new_n131_), .b(new_n127_), .c(x24), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(x09), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n45_), .O(z12));
  nand4  g091(.a(new_n58_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n135_));
  nor2   g092(.a(new_n135_), .b(x00), .O(z13));
  nand3  g093(.a(new_n52_), .b(new_n51_), .c(new_n55_), .O(new_n137_));
  oai21  g094(.a(new_n137_), .b(new_n50_), .c(new_n45_), .O(z14));
  oai21  g095(.a(x10), .b(new_n47_), .c(x02), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(x00), .O(new_n140_));
  nand3  g097(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n141_));
  nand3  g098(.a(new_n141_), .b(new_n140_), .c(new_n45_), .O(z15));
  oai21  g099(.a(new_n47_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g100(.a(new_n101_), .b(new_n46_), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(new_n45_), .O(z17));
endmodule


