// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n78_, new_n79_;
  nor2   g00(.a(x01), .b(x00), .O(z00));
  inv1   g01(.a(x12), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nand3  g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(x11), .c(x10), .d(new_n46_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n45_), .O(z03));
  nand3  g07(.a(new_n48_), .b(x10), .c(new_n46_), .O(new_n51_));
  nor3   g08(.a(new_n51_), .b(x12), .c(x11), .O(z04));
  inv1   g09(.a(z00), .O(new_n53_));
  nand2  g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(x10), .b(x09), .c(x02), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z05));
  inv1   g13(.a(new_n49_), .O(z06));
  nand4  g14(.a(new_n45_), .b(x11), .c(x10), .d(new_n46_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(x01), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x02), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(z07));
  inv1   g18(.a(x20), .O(new_n62_));
  inv1   g19(.a(x21), .O(new_n63_));
  inv1   g20(.a(x22), .O(new_n64_));
  inv1   g21(.a(x00), .O(z16));
  inv1   g22(.a(x19), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(x18), .c(x01), .d(z16), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(z09));
  nand4  g27(.a(new_n68_), .b(x22), .c(x21), .d(new_n62_), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(z10));
  nand4  g29(.a(new_n68_), .b(new_n64_), .c(x21), .d(new_n62_), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(z11));
  aoi21  g31(.a(x10), .b(x02), .c(x24), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(x09), .c(x01), .d(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n53_), .O(z12));
  inv1   g34(.a(x10), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(x02), .c(z16), .O(z15));
  zero   g37(.O(z17));
  nor2   g38(.a(x01), .b(x00), .O(z01));
  nor2   g39(.a(x01), .b(x00), .O(z02));
  nor2   g40(.a(x01), .b(x00), .O(z08));
  nor2   g41(.a(x01), .b(x00), .O(z13));
  nor2   g42(.a(x01), .b(x00), .O(z14));
endmodule


