// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n149_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(new_n49_), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nand3  g009(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g010(.a(new_n53_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(x19), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand4  g014(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z02));
  nand3  g016(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g017(.a(new_n60_), .O(new_n61_));
  nor2   g018(.a(new_n57_), .b(x09), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n61_), .c(x12), .d(x11), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nand4  g021(.a(new_n48_), .b(new_n64_), .c(x18), .d(new_n46_), .O(new_n65_));
  aoi21  g022(.a(new_n65_), .b(new_n63_), .c(x17), .O(z03));
  inv1   g023(.a(x01), .O(new_n67_));
  nor2   g024(.a(x09), .b(new_n46_), .O(new_n68_));
  nor2   g025(.a(x12), .b(x11), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n68_), .c(x10), .d(x00), .O(new_n70_));
  aoi21  g027(.a(new_n70_), .b(new_n44_), .c(new_n67_), .O(z04));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  nor4   g029(.a(new_n72_), .b(x17), .c(new_n57_), .d(new_n47_), .O(z05));
  inv1   g030(.a(new_n72_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(x11), .c(x10), .d(new_n47_), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(x17), .O(z06));
  inv1   g033(.a(x11), .O(new_n77_));
  nor2   g034(.a(x12), .b(new_n77_), .O(new_n78_));
  nand2  g035(.a(new_n78_), .b(new_n62_), .O(new_n79_));
  nand2  g036(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g037(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g038(.a(x17), .b(x01), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n81_), .O(z07));
  inv1   g040(.a(x00), .O(new_n84_));
  nand2  g041(.a(x02), .b(new_n84_), .O(new_n85_));
  inv1   g042(.a(x05), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(x04), .O(new_n87_));
  nor3   g044(.a(new_n87_), .b(new_n85_), .c(x03), .O(new_n88_));
  inv1   g045(.a(x08), .O(new_n89_));
  nand3  g046(.a(x19), .b(new_n45_), .c(new_n89_), .O(new_n90_));
  nor3   g047(.a(new_n90_), .b(x07), .c(x06), .O(new_n91_));
  nand2  g048(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  aoi21  g049(.a(new_n92_), .b(new_n67_), .c(new_n44_), .O(z08));
  inv1   g050(.a(x21), .O(new_n94_));
  inv1   g051(.a(x22), .O(new_n95_));
  nor2   g052(.a(new_n46_), .b(x01), .O(new_n96_));
  nand3  g053(.a(new_n96_), .b(x12), .c(new_n77_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  nor2   g058(.a(x20), .b(x19), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x18), .c(new_n44_), .d(x01), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n95_), .c(new_n94_), .d(new_n84_), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(z09));
  nand3  g063(.a(new_n64_), .b(x18), .c(x01), .O(new_n107_));
  nand2  g064(.a(x22), .b(x21), .O(new_n108_));
  nor3   g065(.a(new_n108_), .b(new_n107_), .c(x20), .O(new_n109_));
  nand2  g066(.a(x02), .b(new_n67_), .O(new_n110_));
  inv1   g067(.a(x13), .O(new_n111_));
  nand3  g068(.a(new_n111_), .b(x12), .c(new_n77_), .O(new_n112_));
  nor2   g069(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g070(.a(x16), .b(x15), .O(new_n114_));
  nand3  g071(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n115_));
  nor3   g072(.a(new_n115_), .b(new_n114_), .c(x14), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n113_), .c(new_n109_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n82_), .O(z10));
  inv1   g075(.a(new_n112_), .O(new_n119_));
  nand2  g076(.a(new_n119_), .b(new_n96_), .O(new_n120_));
  nor2   g077(.a(new_n45_), .b(x17), .O(new_n121_));
  nor2   g078(.a(new_n94_), .b(x20), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n121_), .c(new_n64_), .d(x01), .O(new_n123_));
  nor2   g080(.a(new_n98_), .b(x14), .O(new_n124_));
  nand4  g081(.a(new_n124_), .b(new_n94_), .c(x20), .d(new_n99_), .O(new_n125_));
  oai21  g082(.a(new_n125_), .b(new_n120_), .c(new_n123_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n95_), .c(new_n84_), .O(new_n127_));
  inv1   g084(.a(new_n127_), .O(z11));
  inv1   g085(.a(x24), .O(new_n129_));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n64_), .b(x17), .c(new_n46_), .O(new_n133_));
  oai21  g090(.a(new_n132_), .b(x01), .c(new_n133_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n84_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n129_), .c(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n82_), .O(z12));
  nand3  g095(.a(new_n64_), .b(new_n46_), .c(new_n84_), .O(new_n139_));
  aoi21  g096(.a(new_n139_), .b(new_n67_), .c(new_n44_), .O(z13));
  nor2   g097(.a(x09), .b(x02), .O(new_n141_));
  nor2   g098(.a(x17), .b(x10), .O(new_n142_));
  nor2   g099(.a(x19), .b(x18), .O(new_n143_));
  nand4  g100(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n48_), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(new_n82_), .O(z14));
  aoi22  g102(.a(new_n82_), .b(x00), .c(x19), .d(new_n67_), .O(new_n146_));
  nand3  g103(.a(new_n142_), .b(x01), .c(x00), .O(new_n147_));
  oai21  g104(.a(new_n146_), .b(x02), .c(new_n147_), .O(z15));
  nand3  g105(.a(new_n44_), .b(x01), .c(new_n84_), .O(new_n149_));
  inv1   g106(.a(new_n149_), .O(z16));
  oai21  g107(.a(new_n110_), .b(x00), .c(new_n82_), .O(z17));
endmodule


