// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n100_, new_n101_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n122_;
  inv1   g00(.a(x12), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor3   g02(.a(x09), .b(x02), .c(x00), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  inv1   g08(.a(x10), .O(new_n52_));
  inv1   g09(.a(x00), .O(new_n53_));
  inv1   g10(.a(x01), .O(new_n54_));
  inv1   g11(.a(x02), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n52_), .c(new_n51_), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x19), .O(z01));
  nor2   g16(.a(new_n51_), .b(x02), .O(new_n60_));
  nor2   g17(.a(x17), .b(x10), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n60_), .c(new_n47_), .d(new_n53_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(new_n44_), .c(x01), .O(z02));
  nand3  g20(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(x18), .c(new_n45_), .d(new_n44_), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nand4  g24(.a(x12), .b(x11), .c(x10), .d(new_n51_), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n64_), .O(z03));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(x10), .c(new_n51_), .O(new_n72_));
  nor3   g29(.a(new_n72_), .b(x12), .c(x11), .O(z04));
  nor3   g30(.a(new_n70_), .b(new_n52_), .c(new_n51_), .O(z05));
  nand2  g31(.a(x12), .b(new_n54_), .O(new_n75_));
  nand3  g32(.a(x11), .b(x10), .c(new_n51_), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n67_), .c(new_n75_), .O(z06));
  nand4  g34(.a(new_n44_), .b(x11), .c(x10), .d(new_n51_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n75_), .O(z07));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x07), .O(new_n84_));
  inv1   g41(.a(x04), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n54_), .d(new_n53_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x08), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n50_), .c(x17), .d(new_n44_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n65_), .O(z08));
  nand3  g49(.a(x18), .b(x01), .c(new_n53_), .O(new_n93_));
  inv1   g50(.a(x20), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(new_n65_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n93_), .c(new_n75_), .O(z09));
  nor2   g55(.a(new_n54_), .b(x00), .O(z16));
  nand3  g56(.a(z16), .b(new_n65_), .c(x18), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(x22), .c(x21), .d(new_n94_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z10));
  nand4  g60(.a(new_n96_), .b(x21), .c(new_n94_), .d(new_n65_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n93_), .c(new_n75_), .O(z11));
  nand2  g62(.a(x10), .b(x02), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(x01), .c(x00), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n65_), .b(x17), .c(new_n55_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n54_), .c(new_n53_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n107_), .c(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n75_), .O(z12));
  nor4   g71(.a(new_n56_), .b(x19), .c(new_n45_), .d(x12), .O(z13));
  nor3   g72(.a(new_n56_), .b(x10), .c(x09), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(x19), .O(z14));
  aoi21  g75(.a(new_n52_), .b(x01), .c(new_n55_), .O(new_n119_));
  aoi21  g76(.a(x19), .b(new_n55_), .c(x12), .O(new_n120_));
  oai22  g77(.a(new_n120_), .b(x01), .c(new_n119_), .d(new_n53_), .O(z15));
  nand2  g78(.a(x02), .b(new_n53_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n44_), .c(x01), .O(z17));
endmodule


