// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n119_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x21), .c(x01), .O(z00));
  inv1   g05(.a(x21), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  inv1   g08(.a(x10), .O(new_n52_));
  inv1   g09(.a(x00), .O(new_n53_));
  inv1   g10(.a(x01), .O(new_n54_));
  inv1   g11(.a(x02), .O(new_n55_));
  nand4  g12(.a(x09), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n49_), .O(z01));
  nor2   g16(.a(new_n56_), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n49_), .O(z02));
  nand2  g19(.a(new_n55_), .b(new_n53_), .O(new_n63_));
  nand3  g20(.a(new_n51_), .b(x18), .c(new_n44_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(x21), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n54_), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g24(.a(new_n52_), .b(x09), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z03));
  nand2  g27(.a(new_n49_), .b(new_n54_), .O(new_n71_));
  nor2   g28(.a(x12), .b(x11), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n67_), .c(new_n71_), .O(z04));
  nand2  g31(.a(x01), .b(x00), .O(new_n75_));
  nand3  g32(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n71_), .O(z05));
  nor3   g34(.a(new_n75_), .b(x09), .c(new_n55_), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x11), .c(x10), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z06));
  inv1   g37(.a(x12), .O(new_n81_));
  nand3  g38(.a(new_n68_), .b(new_n81_), .c(x11), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n71_), .O(z07));
  nand2  g42(.a(x02), .b(new_n53_), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  nand2  g44(.a(new_n87_), .b(x04), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n86_), .c(x03), .O(new_n89_));
  nor3   g46(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nor2   g47(.a(new_n51_), .b(x18), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x17), .O(new_n92_));
  aoi21  g49(.a(new_n92_), .b(x21), .c(x01), .O(z08));
  nor2   g50(.a(x22), .b(x20), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n51_), .c(x18), .d(new_n53_), .O(new_n95_));
  aoi21  g52(.a(new_n95_), .b(x01), .c(x21), .O(z09));
  nand4  g53(.a(new_n51_), .b(x18), .c(x01), .d(new_n53_), .O(new_n97_));
  nor3   g54(.a(new_n97_), .b(new_n49_), .c(x20), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(x22), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(z10));
  inv1   g57(.a(x22), .O(new_n101_));
  nand2  g58(.a(new_n98_), .b(new_n101_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z11));
  inv1   g60(.a(x24), .O(new_n104_));
  aoi21  g61(.a(x10), .b(x02), .c(new_n54_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(x00), .O(new_n106_));
  oai21  g63(.a(x19), .b(x02), .c(x23), .O(new_n107_));
  nand3  g64(.a(new_n51_), .b(x17), .c(new_n55_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(x21), .c(new_n54_), .d(new_n53_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n106_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n104_), .c(x09), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(z12));
  nand4  g70(.a(new_n51_), .b(x17), .c(new_n55_), .d(new_n53_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(x21), .c(x01), .O(z13));
  nand4  g72(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n52_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(x21), .c(x01), .O(z14));
  aoi21  g74(.a(x21), .b(new_n55_), .c(new_n105_), .O(new_n118_));
  nand4  g75(.a(x21), .b(x19), .c(new_n55_), .d(new_n54_), .O(new_n119_));
  oai21  g76(.a(new_n118_), .b(new_n53_), .c(new_n119_), .O(z15));
  oai21  g77(.a(new_n54_), .b(x00), .c(new_n71_), .O(z16));
  aoi21  g78(.a(new_n86_), .b(x21), .c(x01), .O(z17));
endmodule


