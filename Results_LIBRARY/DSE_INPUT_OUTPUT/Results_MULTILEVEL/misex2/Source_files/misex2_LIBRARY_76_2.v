// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n135_, new_n137_,
    new_n138_, new_n141_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(x10), .c(x09), .d(new_n46_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n47_), .c(x02), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n57_), .c(x09), .d(new_n46_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n47_), .c(x02), .O(z02));
  nand3  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(x18), .c(new_n44_), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand4  g20(.a(x12), .b(x11), .c(x10), .d(new_n49_), .O(new_n64_));
  oai22  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n60_), .O(z03));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(x10), .c(new_n49_), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(x12), .c(x11), .O(z04));
  nor3   g26(.a(new_n66_), .b(new_n57_), .c(new_n49_), .O(z05));
  inv1   g27(.a(new_n68_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(x11), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(z06));
  inv1   g30(.a(x12), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(x11), .c(x10), .d(new_n49_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x01), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x02), .c(x00), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z07));
  inv1   g35(.a(x08), .O(new_n79_));
  inv1   g36(.a(x05), .O(new_n80_));
  inv1   g37(.a(x06), .O(new_n81_));
  inv1   g38(.a(x03), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n81_), .c(new_n80_), .d(x04), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n45_), .c(x17), .d(new_n79_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n61_), .O(z08));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  inv1   g47(.a(x11), .O(new_n91_));
  nor2   g48(.a(new_n48_), .b(x01), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n91_), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  nor2   g52(.a(x14), .b(x13), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(x20), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  nor2   g55(.a(new_n45_), .b(new_n47_), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n61_), .O(new_n100_));
  oai21  g57(.a(new_n97_), .b(new_n93_), .c(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n90_), .c(new_n89_), .d(new_n46_), .O(new_n102_));
  nand2  g59(.a(new_n48_), .b(x01), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n102_), .O(z09));
  nand3  g61(.a(new_n61_), .b(x18), .c(x01), .O(new_n105_));
  nand3  g62(.a(x22), .b(x21), .c(new_n98_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g64(.a(x13), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(x12), .c(new_n91_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n92_), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  nand2  g69(.a(x16), .b(x15), .O(new_n113_));
  nand3  g70(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n114_));
  nor3   g71(.a(new_n114_), .b(new_n113_), .c(x14), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n112_), .c(new_n107_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(x00), .c(new_n103_), .O(z10));
  nand4  g74(.a(new_n99_), .b(x21), .c(new_n98_), .d(new_n61_), .O(new_n118_));
  nor2   g75(.a(new_n94_), .b(x14), .O(new_n119_));
  nand4  g76(.a(new_n119_), .b(new_n89_), .c(x20), .d(new_n95_), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(new_n111_), .c(new_n118_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n90_), .c(new_n46_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n103_), .O(z11));
  inv1   g80(.a(x24), .O(new_n124_));
  nand4  g81(.a(new_n124_), .b(new_n57_), .c(x09), .d(x00), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x01), .O(new_n127_));
  oai21  g84(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g85(.a(new_n61_), .b(x17), .c(new_n48_), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(x01), .c(new_n129_), .O(new_n130_));
  nand4  g87(.a(new_n130_), .b(new_n124_), .c(x09), .d(new_n46_), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(new_n127_), .O(z12));
  nand3  g89(.a(new_n61_), .b(x17), .c(new_n46_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n47_), .c(x02), .O(z13));
  nand4  g91(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n135_));
  nor2   g92(.a(new_n135_), .b(x19), .O(z14));
  aoi21  g93(.a(new_n57_), .b(x01), .c(new_n48_), .O(new_n137_));
  oai21  g94(.a(x19), .b(x01), .c(new_n48_), .O(new_n138_));
  oai21  g95(.a(new_n137_), .b(new_n46_), .c(new_n138_), .O(z15));
  aoi21  g96(.a(x02), .b(x00), .c(new_n47_), .O(z16));
  nand2  g97(.a(new_n92_), .b(new_n46_), .O(new_n141_));
  nand2  g98(.a(new_n141_), .b(new_n103_), .O(z17));
endmodule


