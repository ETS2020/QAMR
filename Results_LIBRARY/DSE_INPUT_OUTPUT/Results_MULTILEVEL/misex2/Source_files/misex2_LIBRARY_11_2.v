// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n146_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x05), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nor2   g006(.a(new_n49_), .b(x09), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  inv1   g009(.a(x09), .O(new_n53_));
  nor2   g010(.a(new_n49_), .b(new_n53_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(x19), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nor3   g014(.a(x02), .b(x01), .c(x00), .O(new_n58_));
  nor2   g015(.a(x19), .b(x17), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(x09), .O(new_n60_));
  aoi21  g017(.a(new_n60_), .b(new_n47_), .c(x18), .O(z02));
  nand2  g018(.a(new_n45_), .b(x05), .O(new_n62_));
  nor2   g019(.a(x19), .b(new_n45_), .O(new_n63_));
  nand3  g020(.a(new_n63_), .b(new_n58_), .c(new_n44_), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  nor2   g023(.a(new_n57_), .b(x09), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n66_), .c(x12), .d(x11), .O(new_n68_));
  nand3  g025(.a(new_n68_), .b(new_n64_), .c(new_n62_), .O(z03));
  inv1   g026(.a(x11), .O(new_n70_));
  inv1   g027(.a(x12), .O(new_n71_));
  nand3  g028(.a(new_n67_), .b(new_n71_), .c(new_n70_), .O(new_n72_));
  oai21  g029(.a(new_n72_), .b(new_n65_), .c(new_n62_), .O(z04));
  nand2  g030(.a(x01), .b(x00), .O(new_n74_));
  nand3  g031(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  oai21  g032(.a(new_n75_), .b(new_n74_), .c(new_n62_), .O(z05));
  inv1   g033(.a(x00), .O(new_n77_));
  inv1   g034(.a(x01), .O(new_n78_));
  aoi21  g035(.a(new_n45_), .b(x05), .c(new_n70_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x10), .c(new_n53_), .d(x02), .O(new_n80_));
  nor3   g037(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(z06));
  nand3  g038(.a(new_n67_), .b(new_n71_), .c(x11), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n62_), .O(z07));
  inv1   g042(.a(x19), .O(new_n86_));
  inv1   g043(.a(x08), .O(new_n87_));
  inv1   g044(.a(x03), .O(new_n88_));
  nand3  g045(.a(new_n48_), .b(new_n88_), .c(x02), .O(new_n89_));
  inv1   g046(.a(new_n89_), .O(new_n90_));
  nand3  g047(.a(new_n90_), .b(new_n47_), .c(x04), .O(new_n91_));
  nor3   g048(.a(new_n91_), .b(x07), .c(x06), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n45_), .c(x17), .d(new_n87_), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(new_n86_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  inv1   g053(.a(x14), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  inv1   g056(.a(x20), .O(new_n100_));
  aoi21  g057(.a(new_n45_), .b(x05), .c(new_n100_), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n102_));
  nor2   g059(.a(new_n102_), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x12), .c(new_n70_), .d(x02), .O(new_n104_));
  nor2   g061(.a(new_n45_), .b(new_n78_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n100_), .c(new_n86_), .O(new_n106_));
  oai21  g063(.a(new_n104_), .b(x01), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n96_), .c(new_n95_), .d(new_n77_), .O(new_n108_));
  inv1   g065(.a(new_n108_), .O(z09));
  inv1   g066(.a(x13), .O(new_n110_));
  nand4  g067(.a(new_n62_), .b(new_n96_), .c(new_n95_), .d(x20), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n99_), .O(new_n112_));
  nand4  g069(.a(new_n112_), .b(x15), .c(new_n97_), .d(new_n110_), .O(new_n113_));
  nor2   g070(.a(new_n113_), .b(new_n71_), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n70_), .c(x02), .d(new_n78_), .O(new_n115_));
  nor2   g072(.a(new_n96_), .b(new_n95_), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n63_), .c(new_n100_), .d(x01), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n115_), .c(x00), .O(z10));
  nand4  g075(.a(new_n105_), .b(x21), .c(new_n100_), .d(new_n86_), .O(new_n119_));
  nor2   g076(.a(new_n46_), .b(x01), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n110_), .c(x12), .d(new_n70_), .O(new_n121_));
  nor2   g078(.a(new_n98_), .b(x14), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n95_), .c(x20), .d(new_n99_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(new_n121_), .c(new_n119_), .O(new_n124_));
  nand3  g081(.a(new_n124_), .b(new_n96_), .c(new_n77_), .O(new_n125_));
  nand2  g082(.a(new_n125_), .b(new_n62_), .O(z11));
  inv1   g083(.a(x24), .O(new_n127_));
  nand2  g084(.a(x10), .b(x02), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(x01), .c(x00), .O(new_n129_));
  oai21  g086(.a(x19), .b(x02), .c(x23), .O(new_n130_));
  nand3  g087(.a(new_n86_), .b(x17), .c(new_n46_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n78_), .c(new_n77_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n129_), .O(new_n134_));
  nand4  g091(.a(new_n134_), .b(new_n62_), .c(new_n127_), .d(x09), .O(new_n135_));
  inv1   g092(.a(new_n135_), .O(z12));
  inv1   g093(.a(new_n48_), .O(new_n137_));
  oai21  g094(.a(new_n131_), .b(new_n137_), .c(new_n62_), .O(z13));
  nand4  g095(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x19), .O(z14));
  oai21  g097(.a(x10), .b(new_n78_), .c(x02), .O(new_n141_));
  nand2  g098(.a(new_n141_), .b(x00), .O(new_n142_));
  nand3  g099(.a(x19), .b(new_n46_), .c(new_n78_), .O(new_n143_));
  nand3  g100(.a(new_n143_), .b(new_n142_), .c(new_n62_), .O(z15));
  oai21  g101(.a(new_n78_), .b(x00), .c(new_n62_), .O(z16));
  nand2  g102(.a(new_n120_), .b(new_n77_), .O(new_n146_));
  nand2  g103(.a(new_n146_), .b(new_n62_), .O(z17));
endmodule


