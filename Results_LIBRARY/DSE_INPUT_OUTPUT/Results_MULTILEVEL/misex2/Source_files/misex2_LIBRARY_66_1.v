// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n133_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  nand3  g09(.a(x09), .b(new_n46_), .c(new_n45_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n54_), .b(new_n48_), .c(new_n57_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x18), .c(new_n46_), .d(new_n45_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n44_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n60_), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n66_));
  oai21  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand3  g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n68_), .c(x10), .d(new_n47_), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(x12), .O(z04));
  nor3   g29(.a(new_n69_), .b(new_n57_), .c(new_n47_), .O(z05));
  nand2  g30(.a(x17), .b(new_n60_), .O(new_n74_));
  nand3  g31(.a(x11), .b(x10), .c(new_n47_), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n65_), .c(new_n74_), .O(z06));
  nand2  g33(.a(new_n47_), .b(x01), .O(new_n77_));
  inv1   g34(.a(x12), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x11), .c(x10), .O(new_n79_));
  oai22  g36(.a(new_n79_), .b(new_n77_), .c(x17), .d(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z07));
  inv1   g39(.a(new_n74_), .O(z08));
  inv1   g40(.a(x21), .O(new_n84_));
  inv1   g41(.a(x22), .O(new_n85_));
  nand4  g42(.a(x12), .b(new_n68_), .c(x02), .d(new_n60_), .O(new_n86_));
  inv1   g43(.a(x15), .O(new_n87_));
  inv1   g44(.a(x16), .O(new_n88_));
  nor2   g45(.a(x14), .b(x13), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(x20), .c(new_n88_), .d(new_n87_), .O(new_n90_));
  inv1   g47(.a(x20), .O(new_n91_));
  and2   g48(.a(x18), .b(x01), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(new_n61_), .O(new_n93_));
  oai21  g50(.a(new_n90_), .b(new_n86_), .c(new_n93_), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n85_), .c(new_n84_), .d(new_n45_), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n74_), .O(z09));
  nand3  g53(.a(new_n61_), .b(x18), .c(x01), .O(new_n97_));
  nand3  g54(.a(x22), .b(x21), .c(new_n91_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g56(.a(x02), .b(new_n60_), .O(new_n100_));
  inv1   g57(.a(x13), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x12), .c(new_n68_), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  inv1   g60(.a(x14), .O(new_n104_));
  nand3  g61(.a(x16), .b(x15), .c(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n103_), .c(new_n99_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n74_), .O(z10));
  nor2   g66(.a(new_n46_), .b(x01), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n101_), .c(x12), .d(new_n68_), .O(new_n111_));
  nand4  g68(.a(new_n92_), .b(x21), .c(new_n91_), .d(new_n61_), .O(new_n112_));
  nor2   g69(.a(x16), .b(new_n87_), .O(new_n113_));
  nor2   g70(.a(x21), .b(new_n91_), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n113_), .c(new_n44_), .d(new_n104_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n111_), .c(new_n112_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n85_), .c(new_n45_), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(z11));
  nand2  g75(.a(x10), .b(x02), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(x01), .c(x00), .O(new_n120_));
  nand2  g77(.a(new_n61_), .b(new_n46_), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(x23), .c(new_n60_), .d(new_n45_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n120_), .c(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n74_), .O(z12));
  nor3   g82(.a(x02), .b(x01), .c(x00), .O(new_n126_));
  nand4  g83(.a(new_n126_), .b(new_n44_), .c(new_n57_), .d(new_n47_), .O(new_n127_));
  nor3   g84(.a(new_n127_), .b(x19), .c(x18), .O(z14));
  aoi21  g85(.a(new_n57_), .b(x01), .c(new_n46_), .O(new_n129_));
  aoi21  g86(.a(x19), .b(new_n46_), .c(x17), .O(new_n130_));
  oai22  g87(.a(new_n130_), .b(x01), .c(new_n129_), .d(new_n45_), .O(z15));
  oai21  g88(.a(new_n60_), .b(x00), .c(new_n74_), .O(z16));
  nand2  g89(.a(x02), .b(new_n45_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n44_), .c(x01), .O(z17));
  inv1   g91(.a(new_n74_), .O(z13));
endmodule


