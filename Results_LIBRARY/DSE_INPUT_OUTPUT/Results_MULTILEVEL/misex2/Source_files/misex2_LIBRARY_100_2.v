// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n133_;
  inv1   g00(.a(x22), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  inv1   g16(.a(x00), .O(new_n60_));
  inv1   g17(.a(x01), .O(new_n61_));
  nand3  g18(.a(new_n46_), .b(new_n61_), .c(new_n60_), .O(new_n62_));
  nand3  g19(.a(new_n52_), .b(x18), .c(new_n50_), .O(new_n63_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n47_), .c(x02), .O(new_n65_));
  nand4  g22(.a(x22), .b(x12), .c(x11), .d(x10), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n65_), .c(new_n63_), .d(new_n62_), .O(z03));
  nand4  g24(.a(new_n64_), .b(x10), .c(new_n47_), .d(x02), .O(new_n68_));
  nor4   g25(.a(new_n68_), .b(new_n44_), .c(x12), .d(x11), .O(z04));
  nand4  g26(.a(new_n64_), .b(x10), .c(x09), .d(x02), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n44_), .O(z05));
  inv1   g28(.a(x11), .O(new_n72_));
  nor3   g29(.a(new_n68_), .b(new_n44_), .c(new_n72_), .O(z06));
  nand2  g30(.a(x10), .b(new_n47_), .O(new_n74_));
  inv1   g31(.a(x12), .O(new_n75_));
  nand3  g32(.a(x22), .b(new_n75_), .c(x11), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n74_), .c(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(z07));
  inv1   g36(.a(x08), .O(new_n80_));
  inv1   g37(.a(x05), .O(new_n81_));
  inv1   g38(.a(x06), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(x02), .c(new_n61_), .d(new_n60_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n82_), .c(new_n81_), .d(x04), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x07), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n51_), .c(x17), .d(new_n80_), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n52_), .O(z08));
  inv1   g46(.a(x16), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  nand4  g50(.a(new_n72_), .b(x02), .c(new_n61_), .d(new_n60_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n93_), .c(new_n92_), .d(x12), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(x15), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n91_), .c(x20), .d(new_n90_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x22), .O(z09));
  nor3   g56(.a(new_n44_), .b(new_n91_), .c(x20), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n52_), .c(x18), .d(x01), .O(new_n101_));
  nand3  g58(.a(new_n92_), .b(x12), .c(new_n72_), .O(new_n102_));
  nor3   g59(.a(new_n102_), .b(new_n46_), .c(x01), .O(new_n103_));
  inv1   g60(.a(x15), .O(new_n104_));
  nor3   g61(.a(new_n90_), .b(new_n104_), .c(x14), .O(new_n105_));
  nor2   g62(.a(x22), .b(x21), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n105_), .c(new_n103_), .d(x20), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n101_), .c(x00), .O(z10));
  nor3   g65(.a(new_n102_), .b(new_n46_), .c(x00), .O(new_n109_));
  nand3  g66(.a(new_n91_), .b(x20), .c(new_n90_), .O(new_n110_));
  nor3   g67(.a(new_n110_), .b(new_n104_), .c(x14), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n61_), .c(x22), .O(z11));
  nand2  g70(.a(x10), .b(x02), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(x22), .c(x01), .d(x00), .O(new_n115_));
  oai21  g72(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  nand3  g73(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n61_), .c(new_n60_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n115_), .c(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z12));
  nand3  g79(.a(new_n48_), .b(x17), .c(new_n46_), .O(new_n123_));
  nor2   g80(.a(new_n123_), .b(x19), .O(z13));
  nand3  g81(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n125_));
  inv1   g82(.a(new_n125_), .O(new_n126_));
  nand4  g83(.a(new_n126_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(x19), .O(z14));
  aoi22  g85(.a(new_n45_), .b(x00), .c(x19), .d(new_n61_), .O(new_n129_));
  nand3  g86(.a(new_n64_), .b(x22), .c(new_n57_), .O(new_n130_));
  oai21  g87(.a(new_n129_), .b(x02), .c(new_n130_), .O(z15));
  aoi21  g88(.a(x22), .b(x00), .c(new_n61_), .O(z16));
  nand2  g89(.a(new_n48_), .b(x02), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z17));
endmodule


