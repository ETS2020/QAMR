// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n137_,
    new_n139_;
  inv1   g00(.a(x21), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nand3  g16(.a(new_n47_), .b(x02), .c(x00), .O(new_n60_));
  nand3  g17(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n60_), .c(x21), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x01), .O(new_n63_));
  nor3   g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n52_), .c(x18), .d(new_n50_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n63_), .O(z03));
  inv1   g23(.a(x00), .O(new_n67_));
  inv1   g24(.a(x01), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(x10), .c(new_n47_), .d(x02), .O(new_n70_));
  nor4   g27(.a(new_n70_), .b(new_n44_), .c(x12), .d(x11), .O(z04));
  nand4  g28(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x21), .c(new_n68_), .O(z05));
  inv1   g30(.a(x11), .O(new_n74_));
  nor3   g31(.a(new_n70_), .b(new_n44_), .c(new_n74_), .O(z06));
  inv1   g32(.a(x12), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x11), .c(x10), .d(new_n47_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n45_), .O(z07));
  inv1   g37(.a(x08), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(x02), .c(new_n68_), .d(new_n67_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n83_), .c(new_n82_), .d(x04), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(x07), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n51_), .c(x17), .d(new_n81_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n52_), .O(z08));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n74_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x20), .c(new_n94_), .O(new_n96_));
  nor3   g53(.a(new_n96_), .b(x15), .c(x14), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n93_), .c(x02), .d(new_n67_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n68_), .c(x21), .O(z09));
  nor2   g56(.a(new_n51_), .b(new_n68_), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand3  g58(.a(x22), .b(new_n101_), .c(new_n52_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  inv1   g60(.a(x14), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n91_), .c(x12), .O(new_n105_));
  nor3   g62(.a(new_n105_), .b(x11), .c(new_n46_), .O(new_n106_));
  inv1   g63(.a(x15), .O(new_n107_));
  nand3  g64(.a(new_n95_), .b(new_n44_), .c(x20), .O(new_n108_));
  nor3   g65(.a(new_n108_), .b(new_n94_), .c(new_n107_), .O(new_n109_));
  aoi22  g66(.a(new_n109_), .b(new_n106_), .c(new_n103_), .d(new_n100_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n45_), .O(z10));
  nand4  g68(.a(new_n100_), .b(x21), .c(new_n101_), .d(new_n52_), .O(new_n112_));
  nand3  g69(.a(new_n93_), .b(x02), .c(new_n68_), .O(new_n113_));
  nor2   g70(.a(new_n107_), .b(x14), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n44_), .c(x20), .d(new_n94_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n95_), .c(new_n67_), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(z11));
  nand2  g75(.a(x10), .b(x02), .O(new_n119_));
  nand4  g76(.a(new_n119_), .b(x21), .c(x01), .d(x00), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n68_), .c(new_n67_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n120_), .c(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z12));
  inv1   g84(.a(new_n48_), .O(new_n128_));
  oai21  g85(.a(new_n122_), .b(new_n128_), .c(new_n45_), .O(z13));
  nand3  g86(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(new_n131_));
  nand4  g88(.a(new_n131_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n132_));
  nor2   g89(.a(new_n132_), .b(x19), .O(z14));
  aoi22  g90(.a(new_n45_), .b(x00), .c(x19), .d(new_n68_), .O(new_n134_));
  nand3  g91(.a(new_n69_), .b(x21), .c(new_n57_), .O(new_n135_));
  oai21  g92(.a(new_n134_), .b(x02), .c(new_n135_), .O(z15));
  nand3  g93(.a(x21), .b(x01), .c(new_n67_), .O(new_n137_));
  inv1   g94(.a(new_n137_), .O(z16));
  nand3  g95(.a(x02), .b(new_n68_), .c(new_n67_), .O(new_n139_));
  nand2  g96(.a(new_n139_), .b(new_n45_), .O(z17));
endmodule


