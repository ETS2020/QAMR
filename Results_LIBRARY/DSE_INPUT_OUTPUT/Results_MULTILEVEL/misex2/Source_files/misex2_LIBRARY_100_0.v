// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n100_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n121_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(x22), .c(x01), .O(z00));
  nand3  g09(.a(x09), .b(new_n45_), .c(new_n44_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(x22), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n48_), .c(new_n47_), .d(new_n57_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(x22), .c(x01), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  nand3  g17(.a(new_n45_), .b(new_n60_), .c(new_n44_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(x22), .b(new_n62_), .c(x18), .d(new_n47_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n67_), .c(x10), .d(new_n46_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  nor3   g28(.a(new_n68_), .b(new_n57_), .c(new_n46_), .O(z05));
  nor4   g29(.a(new_n64_), .b(new_n67_), .c(new_n57_), .d(x09), .O(z06));
  inv1   g30(.a(x22), .O(new_n74_));
  nand2  g31(.a(new_n46_), .b(x01), .O(new_n75_));
  inv1   g32(.a(x12), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x11), .c(x10), .O(new_n77_));
  oai22  g34(.a(new_n77_), .b(new_n75_), .c(new_n74_), .d(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z07));
  inv1   g37(.a(x18), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x07), .O(new_n84_));
  inv1   g41(.a(x04), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n60_), .d(new_n44_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x08), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x19), .c(new_n81_), .d(x17), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n74_), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  nand2  g50(.a(x01), .b(new_n44_), .O(new_n94_));
  nor4   g51(.a(new_n94_), .b(x20), .c(x19), .d(new_n81_), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n74_), .c(new_n93_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(z09));
  nand3  g54(.a(new_n95_), .b(x22), .c(x21), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(z10));
  nor2   g56(.a(new_n93_), .b(x20), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n62_), .c(x18), .d(new_n44_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(x01), .c(x22), .O(z11));
  aoi21  g59(.a(x10), .b(x02), .c(new_n60_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(x00), .O(new_n104_));
  oai21  g61(.a(x19), .b(x02), .c(x23), .O(new_n105_));
  nand3  g62(.a(new_n62_), .b(x17), .c(new_n45_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(x22), .c(new_n60_), .d(new_n44_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n104_), .c(x24), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(x09), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z12));
  nand3  g68(.a(new_n45_), .b(new_n60_), .c(new_n44_), .O(new_n112_));
  nor4   g69(.a(new_n112_), .b(new_n74_), .c(x19), .d(new_n47_), .O(z13));
  nor3   g70(.a(new_n112_), .b(x10), .c(x09), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n62_), .c(new_n81_), .d(new_n47_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n74_), .O(z14));
  aoi21  g73(.a(x22), .b(new_n45_), .c(new_n103_), .O(new_n117_));
  nand4  g74(.a(x22), .b(x19), .c(new_n45_), .d(new_n60_), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n44_), .c(new_n118_), .O(z15));
  oai21  g76(.a(x22), .b(x01), .c(new_n94_), .O(z16));
  nand4  g77(.a(x22), .b(x02), .c(new_n60_), .d(new_n44_), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z17));
endmodule


