// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n132_, new_n134_, new_n135_, new_n138_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  or2    g06(.a(new_n49_), .b(x09), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(x10), .c(x09), .d(new_n48_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n46_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n57_), .c(x09), .d(new_n48_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n46_), .c(x01), .O(z02));
  nor2   g16(.a(x09), .b(new_n48_), .O(new_n60_));
  nand3  g17(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(new_n60_), .c(new_n47_), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nor2   g21(.a(x02), .b(x01), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(x18), .d(new_n44_), .O(new_n66_));
  oai21  g23(.a(new_n63_), .b(new_n46_), .c(new_n66_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand3  g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(new_n57_), .c(x09), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(x12), .O(z04));
  nand3  g29(.a(x10), .b(x09), .c(x02), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x01), .c(new_n46_), .O(z05));
  nand2  g31(.a(new_n70_), .b(x11), .O(new_n75_));
  inv1   g32(.a(new_n75_), .O(z06));
  nor2   g33(.a(x12), .b(new_n68_), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(new_n60_), .c(x10), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x01), .c(new_n46_), .O(z07));
  inv1   g36(.a(x08), .O(new_n80_));
  inv1   g37(.a(x05), .O(new_n81_));
  inv1   g38(.a(x06), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n82_), .c(new_n81_), .d(x04), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x07), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n45_), .c(x17), .d(new_n80_), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n64_), .O(z08));
  nor2   g46(.a(x14), .b(x13), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x12), .c(new_n68_), .d(x02), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nor2   g50(.a(x22), .b(x21), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(x20), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n93_), .c(new_n92_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n91_), .c(new_n46_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n47_), .O(new_n99_));
  nor2   g56(.a(new_n45_), .b(new_n47_), .O(new_n100_));
  nor2   g57(.a(x20), .b(x19), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(new_n94_), .d(new_n46_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n99_), .O(z09));
  inv1   g60(.a(x21), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nor3   g62(.a(new_n105_), .b(new_n104_), .c(x20), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n64_), .c(x18), .d(x01), .O(new_n107_));
  inv1   g64(.a(x13), .O(new_n108_));
  nor2   g65(.a(new_n48_), .b(x01), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n108_), .c(x12), .d(new_n68_), .O(new_n110_));
  nor3   g67(.a(new_n93_), .b(new_n92_), .c(x14), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n96_), .O(new_n112_));
  or2    g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n107_), .c(x00), .O(z10));
  nor2   g71(.a(new_n104_), .b(x20), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n100_), .c(new_n64_), .O(new_n116_));
  nor2   g73(.a(new_n92_), .b(x14), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n104_), .c(x20), .d(new_n93_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n110_), .c(new_n116_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n105_), .c(new_n46_), .O(new_n120_));
  inv1   g77(.a(new_n120_), .O(z11));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(x01), .c(x00), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n64_), .b(x17), .c(new_n48_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n47_), .c(new_n46_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(new_n123_), .c(x24), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x09), .O(new_n129_));
  inv1   g86(.a(new_n129_), .O(z12));
  nor3   g87(.a(new_n49_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g88(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n132_));
  nor2   g89(.a(new_n132_), .b(x19), .O(z14));
  aoi21  g90(.a(x19), .b(new_n48_), .c(x00), .O(new_n134_));
  nand2  g91(.a(new_n122_), .b(x00), .O(new_n135_));
  oai21  g92(.a(new_n134_), .b(x01), .c(new_n135_), .O(z15));
  xor2a  g93(.a(x01), .b(x00), .O(z16));
  nand3  g94(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z17));
endmodule


