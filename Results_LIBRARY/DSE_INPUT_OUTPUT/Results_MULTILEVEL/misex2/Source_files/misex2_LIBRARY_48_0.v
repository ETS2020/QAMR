// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x11), .O(new_n44_));
  inv1   g001(.a(x20), .O(new_n45_));
  nand2  g002(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g003(.a(x00), .O(new_n47_));
  inv1   g004(.a(x01), .O(new_n48_));
  inv1   g005(.a(x02), .O(new_n49_));
  inv1   g006(.a(x09), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g008(.a(x17), .O(new_n52_));
  nor2   g009(.a(x19), .b(x18), .O(new_n53_));
  nand3  g010(.a(new_n53_), .b(new_n52_), .c(x10), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n51_), .c(new_n46_), .O(z00));
  inv1   g012(.a(x10), .O(new_n56_));
  inv1   g013(.a(x18), .O(new_n57_));
  inv1   g014(.a(new_n46_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(new_n59_));
  nand3  g016(.a(new_n59_), .b(new_n57_), .c(new_n52_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(x09), .c(new_n49_), .d(new_n48_), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x00), .O(z01));
  nor2   g020(.a(new_n60_), .b(x10), .O(new_n64_));
  nand4  g021(.a(new_n64_), .b(x09), .c(new_n49_), .d(new_n48_), .O(new_n65_));
  nor2   g022(.a(new_n65_), .b(x00), .O(z02));
  inv1   g023(.a(x19), .O(new_n67_));
  nor3   g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nand4  g025(.a(new_n68_), .b(new_n67_), .c(x18), .d(new_n52_), .O(new_n69_));
  nand3  g026(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g027(.a(new_n70_), .O(new_n71_));
  nor2   g028(.a(new_n56_), .b(x09), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n71_), .c(x12), .d(x11), .O(new_n73_));
  nand3  g030(.a(new_n73_), .b(new_n69_), .c(new_n46_), .O(z03));
  nand3  g031(.a(x02), .b(x01), .c(x00), .O(new_n75_));
  inv1   g032(.a(new_n75_), .O(new_n76_));
  nand3  g033(.a(new_n76_), .b(x10), .c(new_n50_), .O(new_n77_));
  nor4   g034(.a(new_n77_), .b(new_n45_), .c(x12), .d(x11), .O(z04));
  nor2   g035(.a(new_n58_), .b(new_n56_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x09), .c(x02), .d(x01), .O(new_n80_));
  nor2   g037(.a(new_n80_), .b(new_n47_), .O(z05));
  inv1   g038(.a(new_n77_), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(x11), .O(new_n83_));
  inv1   g040(.a(new_n83_), .O(z06));
  inv1   g041(.a(x12), .O(new_n85_));
  nand3  g042(.a(new_n72_), .b(new_n85_), .c(x11), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(x01), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(x02), .c(x00), .O(new_n88_));
  nand2  g045(.a(new_n88_), .b(new_n46_), .O(z07));
  inv1   g046(.a(x03), .O(new_n90_));
  inv1   g047(.a(x04), .O(new_n91_));
  inv1   g048(.a(x05), .O(new_n92_));
  inv1   g049(.a(x06), .O(new_n93_));
  inv1   g050(.a(x07), .O(new_n94_));
  nand4  g051(.a(new_n46_), .b(x19), .c(new_n57_), .d(x17), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(x08), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n97_));
  nor2   g054(.a(new_n97_), .b(new_n91_), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(new_n90_), .c(x02), .d(new_n48_), .O(new_n99_));
  nor2   g056(.a(new_n99_), .b(x00), .O(z08));
  inv1   g057(.a(x21), .O(new_n101_));
  inv1   g058(.a(x22), .O(new_n102_));
  nor2   g059(.a(new_n49_), .b(x01), .O(new_n103_));
  nand3  g060(.a(new_n103_), .b(x12), .c(new_n44_), .O(new_n104_));
  inv1   g061(.a(x15), .O(new_n105_));
  inv1   g062(.a(x16), .O(new_n106_));
  nor2   g063(.a(x14), .b(x13), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(x20), .c(new_n106_), .d(new_n105_), .O(new_n108_));
  nor2   g065(.a(x20), .b(x19), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(x18), .c(x11), .d(x01), .O(new_n110_));
  oai21  g067(.a(new_n108_), .b(new_n104_), .c(new_n110_), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(new_n102_), .c(new_n101_), .d(new_n47_), .O(new_n112_));
  inv1   g069(.a(new_n112_), .O(z09));
  nand3  g070(.a(x18), .b(x11), .c(x01), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(new_n109_), .c(x22), .d(x21), .O(new_n116_));
  nor2   g073(.a(x13), .b(new_n85_), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(new_n103_), .c(new_n44_), .O(new_n118_));
  inv1   g075(.a(new_n118_), .O(new_n119_));
  nor3   g076(.a(new_n106_), .b(new_n105_), .c(x14), .O(new_n120_));
  nor2   g077(.a(x22), .b(x21), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n120_), .c(new_n119_), .d(x20), .O(new_n122_));
  aoi21  g079(.a(new_n122_), .b(new_n116_), .c(x00), .O(z10));
  nand4  g080(.a(new_n115_), .b(x21), .c(new_n45_), .d(new_n67_), .O(new_n124_));
  nor2   g081(.a(new_n105_), .b(x14), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n101_), .c(x20), .d(new_n106_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n118_), .c(new_n124_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n102_), .c(new_n47_), .O(new_n128_));
  inv1   g085(.a(new_n128_), .O(z11));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n67_), .b(x17), .c(new_n49_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n48_), .c(new_n47_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(new_n131_), .c(x24), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n46_), .O(z12));
  nand4  g095(.a(new_n59_), .b(x17), .c(new_n49_), .d(new_n48_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x00), .O(z13));
  nand3  g097(.a(new_n53_), .b(new_n52_), .c(new_n56_), .O(new_n141_));
  oai21  g098(.a(new_n141_), .b(new_n51_), .c(new_n46_), .O(z14));
  oai21  g099(.a(x10), .b(new_n48_), .c(x02), .O(new_n143_));
  nand2  g100(.a(new_n143_), .b(x00), .O(new_n144_));
  nand3  g101(.a(x19), .b(new_n49_), .c(new_n48_), .O(new_n145_));
  nand3  g102(.a(new_n145_), .b(new_n144_), .c(new_n46_), .O(z15));
  oai21  g103(.a(new_n48_), .b(x00), .c(new_n46_), .O(z16));
  nand2  g104(.a(new_n103_), .b(new_n47_), .O(new_n148_));
  nand2  g105(.a(new_n148_), .b(new_n46_), .O(z17));
endmodule


