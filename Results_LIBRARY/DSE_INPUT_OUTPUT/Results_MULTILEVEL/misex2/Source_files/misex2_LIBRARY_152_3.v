// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n135_, new_n137_,
    new_n138_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nand4  g10(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand2  g17(.a(x02), .b(x00), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n49_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x01), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nor3   g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n65_), .c(x18), .d(new_n44_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n64_), .O(z03));
  nor2   g25(.a(x12), .b(x11), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(x10), .c(x02), .d(x00), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(new_n49_), .c(new_n47_), .O(z04));
  nand2  g28(.a(x09), .b(x01), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(z05));
  nand3  g30(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(x11), .c(x10), .d(new_n49_), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z06));
  inv1   g34(.a(x12), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n49_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z07));
  nor2   g39(.a(new_n48_), .b(x01), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n46_), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(x04), .c(new_n85_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nor3   g45(.a(x08), .b(x07), .c(x06), .O(new_n89_));
  nor2   g46(.a(new_n65_), .b(x18), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(x17), .O(new_n91_));
  nand2  g48(.a(new_n91_), .b(new_n72_), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  inv1   g51(.a(x11), .O(new_n95_));
  nand4  g52(.a(x12), .b(new_n95_), .c(x02), .d(new_n47_), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  nor2   g57(.a(x20), .b(x19), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(x18), .c(new_n49_), .d(x01), .O(new_n102_));
  oai21  g59(.a(new_n100_), .b(new_n96_), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n94_), .c(new_n93_), .d(new_n46_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z09));
  nor2   g62(.a(new_n45_), .b(x09), .O(new_n106_));
  nor2   g63(.a(new_n94_), .b(new_n93_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n106_), .c(new_n101_), .d(x01), .O(new_n108_));
  inv1   g65(.a(x13), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x12), .c(new_n95_), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nor3   g68(.a(new_n98_), .b(new_n97_), .c(x14), .O(new_n112_));
  nand3  g69(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n83_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n108_), .c(x00), .O(z10));
  nor2   g73(.a(x13), .b(new_n78_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n83_), .c(new_n95_), .O(new_n118_));
  inv1   g75(.a(x20), .O(new_n119_));
  and2   g76(.a(x18), .b(x01), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(x21), .c(new_n119_), .d(new_n65_), .O(new_n121_));
  nor2   g78(.a(new_n97_), .b(x14), .O(new_n122_));
  nor2   g79(.a(x21), .b(new_n119_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n122_), .c(new_n98_), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n118_), .c(new_n121_), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(new_n94_), .c(new_n46_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n72_), .O(z11));
  oai21  g84(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g85(.a(new_n65_), .b(x17), .c(new_n48_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(new_n128_), .c(x24), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n46_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n47_), .c(new_n49_), .O(z12));
  nand2  g89(.a(new_n47_), .b(new_n46_), .O(new_n133_));
  oai21  g90(.a(new_n129_), .b(new_n133_), .c(new_n72_), .O(z13));
  nand4  g91(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n135_));
  nor2   g92(.a(new_n135_), .b(x19), .O(z14));
  aoi22  g93(.a(new_n72_), .b(x00), .c(x19), .d(new_n47_), .O(new_n137_));
  nand4  g94(.a(new_n58_), .b(new_n49_), .c(x01), .d(x00), .O(new_n138_));
  oai21  g95(.a(new_n137_), .b(x02), .c(new_n138_), .O(z15));
  aoi21  g96(.a(new_n49_), .b(x00), .c(new_n47_), .O(z16));
  nand2  g97(.a(new_n84_), .b(new_n72_), .O(z17));
endmodule


