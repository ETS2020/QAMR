// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n142_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x05), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x09), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  inv1   g010(.a(x09), .O(new_n54_));
  nor2   g011(.a(new_n50_), .b(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  inv1   g015(.a(x19), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n60_));
  inv1   g017(.a(new_n60_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(x09), .c(new_n48_), .d(new_n46_), .O(new_n62_));
  aoi21  g019(.a(new_n62_), .b(new_n49_), .c(x01), .O(z02));
  nand2  g020(.a(new_n48_), .b(new_n46_), .O(new_n64_));
  nand3  g021(.a(new_n59_), .b(x18), .c(new_n44_), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n64_), .c(new_n49_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n47_), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g025(.a(new_n58_), .b(x09), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g027(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  inv1   g028(.a(x11), .O(new_n72_));
  nand3  g029(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  inv1   g030(.a(new_n73_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(new_n72_), .c(x10), .d(new_n54_), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(x12), .O(z04));
  nand2  g033(.a(x01), .b(x00), .O(new_n77_));
  nand2  g034(.a(x05), .b(new_n47_), .O(new_n78_));
  nand3  g035(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(new_n77_), .c(new_n78_), .O(z05));
  nand3  g037(.a(x11), .b(x10), .c(new_n54_), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(new_n68_), .c(new_n78_), .O(z06));
  nor2   g039(.a(x12), .b(new_n72_), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n69_), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(x01), .O(new_n85_));
  nand3  g042(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(new_n78_), .O(z07));
  inv1   g044(.a(x08), .O(new_n88_));
  inv1   g045(.a(x06), .O(new_n89_));
  inv1   g046(.a(x03), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n91_));
  inv1   g048(.a(new_n91_), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n89_), .c(new_n49_), .d(x04), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(x07), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n45_), .c(x17), .d(new_n88_), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(new_n59_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nor2   g055(.a(new_n48_), .b(x01), .O(new_n99_));
  nand3  g056(.a(new_n99_), .b(x12), .c(new_n72_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nor2   g059(.a(x14), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g061(.a(x20), .O(new_n105_));
  nor2   g062(.a(new_n45_), .b(new_n47_), .O(new_n106_));
  nand3  g063(.a(new_n106_), .b(new_n105_), .c(new_n59_), .O(new_n107_));
  oai21  g064(.a(new_n104_), .b(new_n100_), .c(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n98_), .c(new_n97_), .d(new_n46_), .O(new_n109_));
  nand2  g066(.a(new_n109_), .b(new_n78_), .O(z09));
  nor3   g067(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(new_n59_), .c(x18), .d(x01), .O(new_n112_));
  inv1   g069(.a(x13), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(x12), .c(new_n72_), .O(new_n114_));
  nor4   g071(.a(new_n114_), .b(x05), .c(new_n48_), .d(x01), .O(new_n115_));
  nor3   g072(.a(new_n102_), .b(new_n101_), .c(x14), .O(new_n116_));
  nor2   g073(.a(x22), .b(x21), .O(new_n117_));
  nand4  g074(.a(new_n117_), .b(new_n116_), .c(new_n115_), .d(x20), .O(new_n118_));
  aoi21  g075(.a(new_n118_), .b(new_n112_), .c(x00), .O(z10));
  nand4  g076(.a(new_n106_), .b(x21), .c(new_n105_), .d(new_n59_), .O(new_n120_));
  nand3  g077(.a(x12), .b(new_n72_), .c(new_n49_), .O(new_n121_));
  inv1   g078(.a(new_n121_), .O(new_n122_));
  inv1   g079(.a(x14), .O(new_n123_));
  nand3  g080(.a(x15), .b(new_n123_), .c(new_n113_), .O(new_n124_));
  nand3  g081(.a(new_n97_), .b(x20), .c(new_n102_), .O(new_n125_));
  nor2   g082(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n122_), .c(new_n99_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n120_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n98_), .c(new_n46_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  nand2  g087(.a(x10), .b(x02), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(x01), .c(x00), .O(new_n132_));
  oai21  g089(.a(x19), .b(x02), .c(x23), .O(new_n133_));
  nand3  g090(.a(new_n59_), .b(x17), .c(new_n48_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand4  g092(.a(new_n135_), .b(new_n49_), .c(new_n47_), .d(new_n46_), .O(new_n136_));
  aoi21  g093(.a(new_n136_), .b(new_n132_), .c(x24), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(x09), .O(new_n138_));
  inv1   g095(.a(new_n138_), .O(z12));
  nand4  g096(.a(new_n59_), .b(x17), .c(new_n48_), .d(new_n46_), .O(new_n140_));
  aoi21  g097(.a(new_n140_), .b(new_n49_), .c(x01), .O(z13));
  nand4  g098(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x19), .O(z14));
  aoi21  g100(.a(new_n58_), .b(x01), .c(new_n48_), .O(new_n144_));
  aoi21  g101(.a(x19), .b(new_n48_), .c(x05), .O(new_n145_));
  oai22  g102(.a(new_n145_), .b(x01), .c(new_n144_), .d(new_n46_), .O(z15));
  nor2   g103(.a(new_n47_), .b(x00), .O(z16));
  nand4  g104(.a(new_n49_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


