// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n122_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(x22), .c(x01), .O(z00));
  nand4  g10(.a(new_n51_), .b(x09), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(x22), .c(x01), .O(z01));
  inv1   g12(.a(x22), .O(new_n56_));
  inv1   g13(.a(x01), .O(new_n57_));
  nand3  g14(.a(new_n45_), .b(new_n57_), .c(new_n44_), .O(new_n58_));
  nor3   g15(.a(new_n58_), .b(x10), .c(new_n46_), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n56_), .O(z02));
  nand2  g18(.a(new_n45_), .b(new_n44_), .O(new_n62_));
  nand3  g19(.a(new_n49_), .b(x18), .c(new_n47_), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(x22), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n57_), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(x10), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(x09), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n66_), .c(new_n65_), .O(z03));
  inv1   g27(.a(x11), .O(new_n71_));
  nand2  g28(.a(x01), .b(x00), .O(new_n72_));
  nor4   g29(.a(new_n72_), .b(new_n67_), .c(x09), .d(new_n45_), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(x12), .O(z04));
  nand2  g32(.a(new_n56_), .b(new_n57_), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n72_), .c(new_n76_), .O(z05));
  nand2  g35(.a(new_n73_), .b(x11), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z06));
  nor2   g37(.a(x12), .b(new_n71_), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n68_), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n76_), .O(z07));
  nand2  g42(.a(x02), .b(new_n44_), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  nand2  g44(.a(new_n87_), .b(x04), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n86_), .c(x03), .O(new_n89_));
  nor3   g46(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nand3  g47(.a(x19), .b(new_n48_), .c(x17), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  aoi21  g50(.a(new_n93_), .b(x22), .c(x01), .O(z08));
  inv1   g51(.a(x21), .O(new_n95_));
  nor2   g52(.a(new_n57_), .b(x00), .O(z16));
  nand3  g53(.a(z16), .b(new_n49_), .c(x18), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(x20), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n56_), .c(new_n95_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(z09));
  nand3  g57(.a(new_n98_), .b(x22), .c(x21), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(z10));
  nand3  g59(.a(new_n98_), .b(new_n56_), .c(x21), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(z11));
  inv1   g61(.a(x24), .O(new_n105_));
  nand2  g62(.a(x10), .b(x02), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(x01), .c(x00), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n49_), .b(x17), .c(new_n45_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n57_), .c(new_n44_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n107_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n105_), .c(x09), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n76_), .O(z12));
  nor4   g71(.a(new_n58_), .b(new_n56_), .c(x19), .d(new_n47_), .O(z13));
  nor3   g72(.a(new_n58_), .b(x10), .c(x09), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(new_n56_), .O(z14));
  aoi21  g75(.a(new_n67_), .b(x01), .c(new_n45_), .O(new_n119_));
  aoi21  g76(.a(x19), .b(new_n45_), .c(new_n56_), .O(new_n120_));
  oai22  g77(.a(new_n120_), .b(x01), .c(new_n119_), .d(new_n44_), .O(z15));
  nand4  g78(.a(x22), .b(x02), .c(new_n57_), .d(new_n44_), .O(new_n122_));
  inv1   g79(.a(new_n122_), .O(z17));
endmodule


