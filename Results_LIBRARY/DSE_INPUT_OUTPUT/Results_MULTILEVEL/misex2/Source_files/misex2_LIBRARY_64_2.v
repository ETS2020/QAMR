// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  nand2  g008(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g009(.a(new_n52_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x19), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  nand4  g013(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(x19), .O(z02));
  nand3  g015(.a(x02), .b(x01), .c(x00), .O(new_n59_));
  inv1   g016(.a(new_n59_), .O(new_n60_));
  nor2   g017(.a(new_n56_), .b(x09), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n60_), .c(x12), .d(x11), .O(new_n62_));
  inv1   g019(.a(x00), .O(new_n63_));
  inv1   g020(.a(x01), .O(new_n64_));
  inv1   g021(.a(x02), .O(new_n65_));
  nor2   g022(.a(x19), .b(x17), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  aoi21  g024(.a(new_n67_), .b(new_n62_), .c(new_n45_), .O(z03));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  nand3  g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nor2   g028(.a(new_n71_), .b(x09), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(new_n45_), .O(z04));
  nor4   g031(.a(new_n71_), .b(new_n45_), .c(new_n56_), .d(new_n46_), .O(z05));
  nor2   g032(.a(new_n65_), .b(new_n63_), .O(new_n76_));
  nand4  g033(.a(new_n76_), .b(x11), .c(x10), .d(new_n46_), .O(new_n77_));
  aoi21  g034(.a(new_n77_), .b(x18), .c(new_n64_), .O(z06));
  inv1   g035(.a(new_n61_), .O(new_n79_));
  nand3  g036(.a(x18), .b(new_n70_), .c(x11), .O(new_n80_));
  oai21  g037(.a(new_n80_), .b(new_n79_), .c(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  inv1   g039(.a(new_n82_), .O(z07));
  nand2  g040(.a(x02), .b(new_n63_), .O(new_n84_));
  inv1   g041(.a(x05), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x04), .O(new_n86_));
  nor3   g043(.a(new_n86_), .b(new_n84_), .c(x03), .O(new_n87_));
  nor2   g044(.a(x07), .b(x06), .O(new_n88_));
  inv1   g045(.a(x19), .O(new_n89_));
  nor3   g046(.a(new_n89_), .b(new_n44_), .c(x08), .O(new_n90_));
  nand3  g047(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(new_n91_));
  aoi21  g048(.a(new_n91_), .b(new_n64_), .c(x18), .O(z08));
  inv1   g049(.a(x21), .O(new_n93_));
  inv1   g050(.a(x22), .O(new_n94_));
  nand4  g051(.a(x12), .b(new_n69_), .c(x02), .d(new_n64_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g056(.a(x20), .O(new_n100_));
  and2   g057(.a(x18), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(new_n100_), .c(new_n89_), .O(new_n102_));
  oai21  g059(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n94_), .c(new_n93_), .d(new_n63_), .O(new_n104_));
  inv1   g061(.a(new_n104_), .O(z09));
  nor3   g062(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n89_), .c(x18), .d(x01), .O(new_n107_));
  nand2  g064(.a(x02), .b(new_n64_), .O(new_n108_));
  inv1   g065(.a(x13), .O(new_n109_));
  nand3  g066(.a(new_n109_), .b(x12), .c(new_n69_), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nor3   g068(.a(new_n97_), .b(new_n96_), .c(x14), .O(new_n112_));
  nor2   g069(.a(x22), .b(x21), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n112_), .c(new_n111_), .d(x20), .O(new_n114_));
  aoi21  g071(.a(new_n114_), .b(new_n107_), .c(x00), .O(z10));
  nor2   g072(.a(new_n65_), .b(x01), .O(new_n116_));
  nor2   g073(.a(x13), .b(new_n70_), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(new_n116_), .c(new_n69_), .O(new_n118_));
  nand4  g075(.a(new_n101_), .b(x21), .c(new_n100_), .d(new_n89_), .O(new_n119_));
  nor2   g076(.a(new_n96_), .b(x14), .O(new_n120_));
  nor2   g077(.a(x21), .b(new_n100_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n120_), .c(new_n97_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n118_), .c(new_n119_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n94_), .c(new_n63_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand4  g084(.a(new_n127_), .b(x18), .c(x01), .d(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n89_), .b(x17), .c(new_n65_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n64_), .c(new_n63_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n126_), .c(x09), .O(new_n134_));
  inv1   g091(.a(new_n134_), .O(z12));
  nand2  g092(.a(new_n47_), .b(x17), .O(new_n136_));
  nor2   g093(.a(new_n136_), .b(x19), .O(z13));
  nor2   g094(.a(x09), .b(x02), .O(new_n138_));
  nand4  g095(.a(new_n138_), .b(new_n66_), .c(new_n56_), .d(new_n63_), .O(new_n139_));
  aoi21  g096(.a(new_n139_), .b(new_n64_), .c(x18), .O(z14));
  nand2  g097(.a(new_n45_), .b(x01), .O(new_n141_));
  aoi22  g098(.a(new_n141_), .b(x00), .c(x19), .d(new_n64_), .O(new_n142_));
  nand4  g099(.a(x18), .b(new_n56_), .c(x01), .d(x00), .O(new_n143_));
  oai21  g100(.a(new_n142_), .b(x02), .c(new_n143_), .O(z15));
  aoi21  g101(.a(x18), .b(x00), .c(new_n64_), .O(z16));
  oai21  g102(.a(new_n108_), .b(x00), .c(new_n141_), .O(z17));
endmodule


