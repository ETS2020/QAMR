// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_,
    new_n128_;
  inv1   g00(.a(x12), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  inv1   g04(.a(x00), .O(new_n48_));
  inv1   g05(.a(x01), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z00));
  nand3  g12(.a(x09), .b(new_n50_), .c(new_n48_), .O(new_n56_));
  inv1   g13(.a(new_n56_), .O(new_n57_));
  nor2   g14(.a(x19), .b(x18), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n57_), .c(new_n45_), .d(x10), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n44_), .c(x01), .O(z01));
  nand3  g17(.a(new_n58_), .b(new_n45_), .c(new_n47_), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(new_n44_), .c(x01), .O(z02));
  nand2  g21(.a(new_n50_), .b(new_n48_), .O(new_n65_));
  inv1   g22(.a(x19), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(x18), .c(new_n45_), .O(new_n67_));
  oai21  g24(.a(new_n67_), .b(new_n65_), .c(new_n44_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n49_), .O(new_n69_));
  nand3  g26(.a(new_n51_), .b(x02), .c(x00), .O(new_n70_));
  nand3  g27(.a(x12), .b(x11), .c(x10), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(z03));
  nand2  g29(.a(x12), .b(new_n49_), .O(new_n73_));
  nand3  g30(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  inv1   g31(.a(x11), .O(new_n75_));
  nand4  g32(.a(new_n44_), .b(new_n75_), .c(x10), .d(new_n51_), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(z04));
  nand2  g34(.a(x01), .b(x00), .O(new_n78_));
  nand3  g35(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(new_n73_), .O(z05));
  nand3  g37(.a(x02), .b(x01), .c(x00), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(x11), .c(x10), .d(new_n51_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(z06));
  nand2  g41(.a(x11), .b(x10), .O(new_n85_));
  oai21  g42(.a(new_n85_), .b(x09), .c(x01), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n44_), .c(x02), .d(x00), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(z07));
  nand2  g45(.a(x02), .b(new_n48_), .O(new_n89_));
  inv1   g46(.a(x05), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(x04), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n89_), .c(x03), .O(new_n92_));
  nor3   g49(.a(x08), .b(x07), .c(x06), .O(new_n93_));
  nand3  g50(.a(x19), .b(new_n46_), .c(x17), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(new_n96_));
  aoi21  g53(.a(new_n96_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g54(.a(x20), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand4  g57(.a(new_n66_), .b(x18), .c(x01), .d(new_n48_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n100_), .c(new_n99_), .d(new_n98_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(z09));
  nand3  g61(.a(x18), .b(x01), .c(new_n48_), .O(new_n105_));
  nand4  g62(.a(x22), .b(x21), .c(new_n98_), .d(new_n66_), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n105_), .c(new_n73_), .O(z10));
  nand4  g64(.a(new_n102_), .b(new_n100_), .c(x21), .d(new_n98_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(z11));
  inv1   g66(.a(x24), .O(new_n110_));
  aoi21  g67(.a(x10), .b(x02), .c(new_n49_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(x00), .O(new_n112_));
  oai21  g69(.a(x19), .b(x02), .c(x23), .O(new_n113_));
  nand3  g70(.a(new_n66_), .b(x17), .c(new_n50_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n49_), .c(new_n48_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n112_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n110_), .c(x09), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n73_), .O(z12));
  nand4  g76(.a(new_n66_), .b(x17), .c(new_n50_), .d(new_n48_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g78(.a(new_n62_), .b(new_n51_), .c(new_n50_), .d(new_n48_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g80(.a(new_n44_), .b(new_n50_), .c(new_n111_), .O(new_n124_));
  nand4  g81(.a(x19), .b(new_n44_), .c(new_n50_), .d(new_n49_), .O(new_n125_));
  oai21  g82(.a(new_n124_), .b(new_n48_), .c(new_n125_), .O(z15));
  oai21  g83(.a(new_n49_), .b(x00), .c(new_n73_), .O(z16));
  nand4  g84(.a(new_n44_), .b(x02), .c(new_n49_), .d(new_n48_), .O(new_n128_));
  inv1   g85(.a(new_n128_), .O(z17));
endmodule


