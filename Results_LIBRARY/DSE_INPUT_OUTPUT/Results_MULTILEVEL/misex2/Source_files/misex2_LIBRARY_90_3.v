// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  nand2  g008(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g009(.a(new_n52_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x19), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  nand4  g013(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(x19), .O(z02));
  inv1   g015(.a(x03), .O(new_n59_));
  nand3  g016(.a(new_n46_), .b(x01), .c(x00), .O(new_n60_));
  nand3  g017(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  oai21  g018(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g019(.a(new_n62_), .b(x02), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(new_n64_), .c(x18), .d(new_n44_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g024(.a(x02), .O(new_n68_));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  inv1   g027(.a(new_n60_), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n72_));
  aoi21  g029(.a(new_n72_), .b(new_n59_), .c(new_n68_), .O(z04));
  inv1   g030(.a(x00), .O(new_n74_));
  inv1   g031(.a(x01), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g033(.a(new_n76_), .b(x02), .O(new_n77_));
  inv1   g034(.a(new_n77_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x10), .c(x09), .d(new_n59_), .O(new_n79_));
  inv1   g036(.a(new_n79_), .O(z05));
  nand4  g037(.a(new_n76_), .b(x11), .c(x10), .d(new_n46_), .O(new_n81_));
  aoi21  g038(.a(new_n81_), .b(new_n59_), .c(new_n68_), .O(z06));
  nand4  g039(.a(new_n70_), .b(x11), .c(x10), .d(new_n46_), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(x01), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(x00), .O(new_n85_));
  aoi21  g042(.a(new_n85_), .b(new_n59_), .c(new_n68_), .O(z07));
  inv1   g043(.a(x08), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  inv1   g045(.a(x06), .O(new_n89_));
  nand4  g046(.a(new_n59_), .b(x02), .c(new_n75_), .d(new_n74_), .O(new_n90_));
  inv1   g047(.a(new_n90_), .O(z17));
  nand4  g048(.a(z17), .b(new_n89_), .c(new_n88_), .d(x04), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(x07), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n45_), .c(x17), .d(new_n87_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(new_n64_), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  inv1   g053(.a(x22), .O(new_n97_));
  inv1   g054(.a(x20), .O(new_n98_));
  nand2  g055(.a(x03), .b(x02), .O(new_n99_));
  nand4  g056(.a(new_n99_), .b(new_n98_), .c(new_n64_), .d(x18), .O(new_n100_));
  nor2   g057(.a(new_n68_), .b(x01), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x12), .c(new_n69_), .d(new_n59_), .O(new_n102_));
  inv1   g059(.a(x15), .O(new_n103_));
  inv1   g060(.a(x16), .O(new_n104_));
  nor2   g061(.a(x14), .b(x13), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(x20), .c(new_n104_), .d(new_n103_), .O(new_n106_));
  oai22  g063(.a(new_n106_), .b(new_n102_), .c(new_n100_), .d(new_n75_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n97_), .c(new_n96_), .d(new_n74_), .O(new_n108_));
  inv1   g065(.a(new_n108_), .O(z09));
  nand3  g066(.a(new_n64_), .b(x18), .c(x01), .O(new_n110_));
  nand3  g067(.a(x22), .b(x21), .c(new_n98_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  inv1   g069(.a(x13), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(x12), .c(new_n69_), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nand2  g072(.a(new_n115_), .b(new_n101_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(new_n117_));
  nand2  g074(.a(x16), .b(x15), .O(new_n118_));
  nand3  g075(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n119_));
  nor3   g076(.a(new_n119_), .b(new_n118_), .c(x14), .O(new_n120_));
  aoi21  g077(.a(new_n120_), .b(new_n117_), .c(new_n112_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(x00), .c(new_n99_), .O(z10));
  nor2   g079(.a(new_n45_), .b(new_n75_), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(x21), .c(new_n98_), .d(new_n64_), .O(new_n124_));
  nor2   g081(.a(new_n103_), .b(x14), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n96_), .c(x20), .d(new_n104_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n116_), .c(new_n124_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n97_), .c(new_n74_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n99_), .O(z11));
  inv1   g086(.a(x24), .O(new_n130_));
  oai21  g087(.a(x10), .b(x03), .c(x02), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(x01), .c(x00), .O(new_n132_));
  nand2  g089(.a(new_n64_), .b(x17), .O(new_n133_));
  nand2  g090(.a(x23), .b(x19), .O(new_n134_));
  aoi21  g091(.a(new_n134_), .b(new_n133_), .c(x02), .O(new_n135_));
  nand3  g092(.a(x23), .b(new_n59_), .c(x02), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(new_n137_));
  oai21  g094(.a(new_n137_), .b(new_n135_), .c(new_n75_), .O(new_n138_));
  oai21  g095(.a(new_n138_), .b(x00), .c(new_n132_), .O(new_n139_));
  nand3  g096(.a(new_n139_), .b(new_n130_), .c(x09), .O(new_n140_));
  inv1   g097(.a(new_n140_), .O(z12));
  nand2  g098(.a(new_n47_), .b(x17), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x19), .O(z13));
  nand4  g100(.a(new_n46_), .b(new_n68_), .c(new_n75_), .d(new_n74_), .O(new_n144_));
  nand4  g101(.a(new_n64_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n145_));
  oai21  g102(.a(new_n145_), .b(new_n144_), .c(new_n99_), .O(z14));
  oai21  g103(.a(x10), .b(new_n75_), .c(x02), .O(new_n147_));
  nand2  g104(.a(new_n147_), .b(x00), .O(new_n148_));
  nand3  g105(.a(x19), .b(new_n68_), .c(new_n75_), .O(new_n149_));
  nand3  g106(.a(new_n149_), .b(new_n148_), .c(new_n99_), .O(z15));
  oai21  g107(.a(new_n75_), .b(x00), .c(new_n99_), .O(z16));
endmodule


