// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n127_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  inv1   g07(.a(x13), .O(new_n51_));
  nand2  g08(.a(x20), .b(new_n51_), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x00), .O(z00));
  nand4  g13(.a(new_n54_), .b(x09), .c(new_n45_), .d(new_n44_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x00), .O(z01));
  inv1   g15(.a(x00), .O(new_n59_));
  nand4  g16(.a(x09), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n60_), .c(new_n52_), .O(z02));
  inv1   g19(.a(x20), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(x13), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nor2   g23(.a(new_n47_), .b(x09), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x12), .d(x11), .O(new_n68_));
  nor3   g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n50_), .c(x18), .d(new_n48_), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(z03));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand4  g30(.a(new_n52_), .b(new_n73_), .c(new_n72_), .d(x10), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(new_n46_), .c(x02), .d(x01), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n59_), .O(z04));
  nand2  g34(.a(x01), .b(x00), .O(new_n78_));
  nand3  g35(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(new_n52_), .O(z05));
  nor2   g37(.a(new_n64_), .b(new_n72_), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(x10), .c(new_n46_), .d(x02), .O(new_n82_));
  nor3   g39(.a(new_n82_), .b(new_n44_), .c(new_n59_), .O(z06));
  nand3  g40(.a(new_n67_), .b(new_n73_), .c(x11), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x01), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n52_), .O(z07));
  nand3  g44(.a(x02), .b(new_n44_), .c(new_n59_), .O(new_n88_));
  inv1   g45(.a(x05), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(x04), .O(new_n90_));
  nor3   g47(.a(new_n90_), .b(new_n88_), .c(x03), .O(new_n91_));
  nor3   g48(.a(x08), .b(x07), .c(x06), .O(new_n92_));
  nor2   g49(.a(new_n50_), .b(x18), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(x17), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n52_), .O(z08));
  nand3  g52(.a(x18), .b(x01), .c(new_n59_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n97_), .c(new_n63_), .d(new_n50_), .O(new_n99_));
  oai21  g56(.a(new_n99_), .b(new_n96_), .c(new_n52_), .O(z09));
  nand4  g57(.a(new_n50_), .b(x18), .c(x01), .d(new_n59_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(x22), .c(x21), .d(new_n63_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(z10));
  nand4  g61(.a(new_n98_), .b(x21), .c(new_n63_), .d(new_n50_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n96_), .c(new_n52_), .O(z11));
  inv1   g63(.a(x24), .O(new_n107_));
  nand2  g64(.a(x10), .b(x02), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(x01), .c(x00), .O(new_n109_));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n110_));
  nand3  g67(.a(new_n50_), .b(x17), .c(new_n45_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n44_), .c(new_n59_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n109_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n107_), .c(x09), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n52_), .O(z12));
  nand2  g73(.a(new_n44_), .b(new_n59_), .O(new_n117_));
  oai21  g74(.a(new_n111_), .b(new_n117_), .c(new_n52_), .O(z13));
  nor2   g75(.a(new_n53_), .b(x10), .O(new_n119_));
  nand4  g76(.a(new_n119_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n120_));
  nor2   g77(.a(new_n120_), .b(x00), .O(z14));
  oai21  g78(.a(x10), .b(new_n44_), .c(x02), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x00), .O(new_n123_));
  nand3  g80(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n123_), .c(new_n64_), .O(z15));
  oai21  g82(.a(new_n44_), .b(x00), .c(new_n52_), .O(z16));
  nand4  g83(.a(new_n52_), .b(x02), .c(new_n44_), .d(new_n59_), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z17));
endmodule


