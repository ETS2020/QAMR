// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n120_, new_n122_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand3  g09(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  nand2  g13(.a(x01), .b(x00), .O(new_n57_));
  nand3  g14(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n58_));
  inv1   g15(.a(x10), .O(new_n59_));
  inv1   g16(.a(x19), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n58_), .c(new_n57_), .O(z02));
  inv1   g19(.a(x00), .O(new_n63_));
  inv1   g20(.a(x01), .O(new_n64_));
  nand3  g21(.a(new_n46_), .b(new_n64_), .c(new_n63_), .O(new_n65_));
  nand3  g22(.a(new_n60_), .b(x18), .c(new_n44_), .O(new_n66_));
  oai21  g23(.a(new_n66_), .b(new_n65_), .c(new_n57_), .O(z03));
  inv1   g24(.a(new_n57_), .O(z04));
  nand3  g25(.a(x02), .b(new_n64_), .c(x00), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z07));
  nand3  g27(.a(x02), .b(new_n64_), .c(new_n63_), .O(new_n72_));
  inv1   g28(.a(x03), .O(new_n73_));
  inv1   g29(.a(x05), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(x04), .c(new_n73_), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nor3   g32(.a(x08), .b(x07), .c(x06), .O(new_n77_));
  nor2   g33(.a(new_n60_), .b(x18), .O(new_n78_));
  nand4  g34(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(x17), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n57_), .O(z08));
  inv1   g36(.a(x21), .O(new_n81_));
  inv1   g37(.a(x22), .O(new_n82_));
  inv1   g38(.a(x11), .O(new_n83_));
  nand4  g39(.a(x12), .b(new_n83_), .c(x02), .d(new_n64_), .O(new_n84_));
  inv1   g40(.a(x15), .O(new_n85_));
  inv1   g41(.a(x16), .O(new_n86_));
  nor2   g42(.a(x14), .b(x13), .O(new_n87_));
  nand4  g43(.a(new_n87_), .b(x20), .c(new_n86_), .d(new_n85_), .O(new_n88_));
  inv1   g44(.a(x20), .O(new_n89_));
  nand4  g45(.a(new_n89_), .b(new_n60_), .c(x18), .d(x01), .O(new_n90_));
  oai21  g46(.a(new_n88_), .b(new_n84_), .c(new_n90_), .O(new_n91_));
  nand4  g47(.a(new_n91_), .b(new_n82_), .c(new_n81_), .d(new_n63_), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(z09));
  nand2  g49(.a(new_n60_), .b(x18), .O(new_n94_));
  nand3  g50(.a(x22), .b(x21), .c(new_n89_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n94_), .c(new_n63_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(x01), .O(new_n97_));
  inv1   g53(.a(x13), .O(new_n98_));
  inv1   g54(.a(new_n72_), .O(new_n99_));
  nand4  g55(.a(new_n99_), .b(new_n98_), .c(x12), .d(new_n83_), .O(new_n100_));
  inv1   g56(.a(x14), .O(new_n101_));
  nand3  g57(.a(new_n82_), .b(new_n81_), .c(x20), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  nand4  g59(.a(new_n103_), .b(x16), .c(x15), .d(new_n101_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n100_), .c(new_n97_), .O(z10));
  nand3  g61(.a(new_n82_), .b(x21), .c(new_n89_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n94_), .c(new_n63_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(x01), .O(new_n108_));
  nand4  g64(.a(new_n103_), .b(new_n86_), .c(x15), .d(new_n101_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n100_), .c(new_n108_), .O(z11));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g67(.a(new_n60_), .b(x17), .c(new_n46_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n111_), .c(x24), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(x09), .c(new_n64_), .d(new_n63_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n57_), .O(z12));
  nand3  g71(.a(new_n48_), .b(x17), .c(new_n46_), .O(new_n116_));
  nor2   g72(.a(new_n116_), .b(x19), .O(z13));
  nand3  g73(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n61_), .c(new_n57_), .O(z14));
  nor2   g75(.a(x19), .b(x00), .O(new_n120_));
  nor3   g76(.a(new_n120_), .b(x02), .c(x01), .O(z15));
  nand2  g77(.a(new_n48_), .b(x02), .O(new_n122_));
  inv1   g78(.a(new_n122_), .O(z17));
  zero   g79(.O(z05));
  inv1   g80(.a(new_n57_), .O(z06));
  buf    g81(.a(x01), .O(z16));
endmodule


