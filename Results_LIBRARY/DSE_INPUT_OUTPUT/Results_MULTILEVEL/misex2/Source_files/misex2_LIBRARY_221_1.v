// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n110_, new_n112_,
    new_n113_, new_n116_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x09), .O(new_n47_));
  nor3   g04(.a(x19), .b(x18), .c(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(new_n44_), .c(x00), .O(z00));
  nand4  g07(.a(new_n48_), .b(x10), .c(x09), .d(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x00), .O(z01));
  nand4  g09(.a(new_n48_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g11(.a(x00), .O(new_n55_));
  nand3  g12(.a(new_n45_), .b(new_n44_), .c(new_n55_), .O(new_n56_));
  inv1   g13(.a(x17), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(x18), .c(new_n57_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand3  g17(.a(new_n47_), .b(x12), .c(x11), .O(new_n61_));
  oai22  g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n56_), .O(z03));
  inv1   g19(.a(x09), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(x10), .c(new_n63_), .O(new_n66_));
  nor3   g23(.a(new_n66_), .b(x12), .c(x11), .O(z04));
  nand3  g24(.a(x10), .b(x09), .c(x02), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(x00), .c(new_n44_), .O(z05));
  inv1   g26(.a(new_n66_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(x11), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(z06));
  inv1   g29(.a(x11), .O(new_n73_));
  nor2   g30(.a(x12), .b(new_n73_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x01), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x02), .c(x00), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z07));
  nor2   g35(.a(x03), .b(new_n45_), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  inv1   g37(.a(x18), .O(new_n81_));
  nand3  g38(.a(x19), .b(new_n81_), .c(x17), .O(new_n82_));
  nor3   g39(.a(new_n82_), .b(x08), .c(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n80_), .c(new_n79_), .d(x04), .O(new_n84_));
  aoi21  g41(.a(new_n84_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g42(.a(x16), .O(new_n86_));
  inv1   g43(.a(x21), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  inv1   g45(.a(x14), .O(new_n89_));
  nand4  g46(.a(new_n73_), .b(x02), .c(new_n44_), .d(new_n55_), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n89_), .c(new_n88_), .d(x12), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(x15), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n87_), .c(x20), .d(new_n86_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(x22), .O(z09));
  inv1   g52(.a(x15), .O(new_n96_));
  nor2   g53(.a(new_n92_), .b(new_n96_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n87_), .c(x20), .d(x16), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x22), .O(z10));
  nand4  g56(.a(new_n97_), .b(new_n87_), .c(x20), .d(new_n86_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(x22), .O(z11));
  inv1   g58(.a(x24), .O(new_n102_));
  aoi21  g59(.a(x10), .b(x02), .c(new_n44_), .O(new_n103_));
  oai21  g60(.a(x19), .b(x02), .c(x23), .O(new_n104_));
  nand3  g61(.a(new_n58_), .b(x17), .c(new_n45_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n104_), .c(x00), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n103_), .c(new_n102_), .O(new_n107_));
  oai22  g64(.a(new_n107_), .b(new_n63_), .c(new_n44_), .d(x00), .O(z12));
  aoi21  g65(.a(new_n105_), .b(new_n44_), .c(x00), .O(z13));
  nand4  g66(.a(new_n48_), .b(new_n46_), .c(new_n63_), .d(new_n45_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n44_), .c(x00), .O(z14));
  aoi21  g68(.a(new_n46_), .b(x01), .c(new_n45_), .O(new_n112_));
  nand3  g69(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n113_));
  oai21  g70(.a(new_n112_), .b(new_n55_), .c(new_n113_), .O(z15));
  nor2   g71(.a(new_n44_), .b(x00), .O(z16));
  nand3  g72(.a(x02), .b(new_n44_), .c(new_n55_), .O(new_n116_));
  inv1   g73(.a(new_n116_), .O(z17));
endmodule


