// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x15), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x07), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  nor2   g007(.a(x19), .b(x18), .O(new_n51_));
  nand3  g008(.a(new_n51_), .b(new_n50_), .c(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g010(.a(x01), .O(new_n54_));
  inv1   g011(.a(x10), .O(new_n55_));
  inv1   g012(.a(x18), .O(new_n56_));
  inv1   g013(.a(x19), .O(new_n57_));
  nand4  g014(.a(new_n45_), .b(new_n57_), .c(new_n56_), .d(new_n50_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(x09), .c(new_n46_), .d(new_n54_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x00), .O(z01));
  nor2   g018(.a(new_n58_), .b(x10), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(x09), .c(new_n46_), .d(new_n54_), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x00), .O(z02));
  inv1   g021(.a(x07), .O(new_n65_));
  nor2   g022(.a(x15), .b(new_n65_), .O(new_n66_));
  nand3  g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g024(.a(new_n67_), .O(new_n68_));
  nor2   g025(.a(new_n55_), .b(x09), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n68_), .c(x12), .d(x11), .O(new_n70_));
  nor3   g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n57_), .c(x18), .d(new_n50_), .O(new_n72_));
  aoi21  g029(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z03));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand3  g032(.a(new_n69_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n67_), .c(new_n45_), .O(z04));
  inv1   g034(.a(x00), .O(new_n78_));
  nor2   g035(.a(new_n66_), .b(new_n55_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x09), .c(x02), .d(x01), .O(new_n80_));
  nor2   g037(.a(new_n80_), .b(new_n78_), .O(z05));
  nor2   g038(.a(new_n66_), .b(new_n74_), .O(new_n82_));
  nand4  g039(.a(new_n82_), .b(x10), .c(new_n47_), .d(x02), .O(new_n83_));
  nor3   g040(.a(new_n83_), .b(new_n54_), .c(new_n78_), .O(z06));
  nand3  g041(.a(new_n69_), .b(new_n75_), .c(x11), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n45_), .O(z07));
  inv1   g045(.a(x08), .O(new_n89_));
  inv1   g046(.a(x05), .O(new_n90_));
  inv1   g047(.a(x06), .O(new_n91_));
  inv1   g048(.a(x03), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(x02), .c(new_n54_), .d(new_n78_), .O(new_n93_));
  inv1   g050(.a(new_n93_), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n91_), .c(new_n90_), .d(x04), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(x07), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(new_n56_), .c(x17), .d(new_n89_), .O(new_n97_));
  nor2   g054(.a(new_n97_), .b(new_n57_), .O(z08));
  inv1   g055(.a(x21), .O(new_n99_));
  inv1   g056(.a(x22), .O(new_n100_));
  nor2   g057(.a(new_n66_), .b(x20), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(new_n57_), .c(x18), .d(x01), .O(new_n102_));
  nand2  g059(.a(x02), .b(new_n54_), .O(new_n103_));
  nand3  g060(.a(x12), .b(new_n74_), .c(new_n65_), .O(new_n104_));
  nor2   g061(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nor2   g062(.a(x14), .b(x13), .O(new_n106_));
  inv1   g063(.a(x20), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(x16), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n106_), .c(new_n105_), .d(new_n44_), .O(new_n109_));
  nand2  g066(.a(new_n109_), .b(new_n102_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n100_), .c(new_n99_), .d(new_n78_), .O(new_n111_));
  inv1   g068(.a(new_n111_), .O(z09));
  nand4  g069(.a(new_n45_), .b(x22), .c(x21), .d(new_n107_), .O(new_n113_));
  inv1   g070(.a(new_n113_), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n57_), .c(x18), .d(x01), .O(new_n115_));
  nor2   g072(.a(x13), .b(new_n75_), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n74_), .c(x02), .d(new_n54_), .O(new_n117_));
  inv1   g074(.a(x16), .O(new_n118_));
  nor3   g075(.a(new_n118_), .b(new_n44_), .c(x14), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(new_n100_), .c(new_n99_), .d(x20), .O(new_n120_));
  or2    g077(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  aoi21  g078(.a(new_n121_), .b(new_n115_), .c(x00), .O(z10));
  nor2   g079(.a(new_n56_), .b(new_n54_), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(x21), .c(new_n107_), .d(new_n57_), .O(new_n124_));
  nor2   g081(.a(new_n44_), .b(x14), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n99_), .c(x20), .d(new_n118_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n117_), .c(new_n124_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n100_), .c(new_n78_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n45_), .O(z11));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n57_), .b(x17), .c(new_n46_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n54_), .c(new_n78_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(new_n131_), .c(x24), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n45_), .O(z12));
  inv1   g095(.a(new_n48_), .O(new_n139_));
  oai21  g096(.a(new_n133_), .b(new_n139_), .c(new_n45_), .O(z13));
  nand3  g097(.a(new_n51_), .b(new_n50_), .c(new_n55_), .O(new_n141_));
  oai21  g098(.a(new_n141_), .b(new_n49_), .c(new_n45_), .O(z14));
  oai21  g099(.a(x10), .b(new_n54_), .c(x02), .O(new_n143_));
  nand2  g100(.a(new_n143_), .b(x00), .O(new_n144_));
  nand3  g101(.a(x19), .b(new_n46_), .c(new_n54_), .O(new_n145_));
  aoi21  g102(.a(new_n145_), .b(new_n144_), .c(new_n66_), .O(z15));
  oai21  g103(.a(new_n54_), .b(x00), .c(new_n45_), .O(z16));
  nand4  g104(.a(new_n45_), .b(x02), .c(new_n54_), .d(new_n78_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


