// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n135_, new_n137_,
    new_n138_, new_n139_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand2  g09(.a(x16), .b(x01), .O(new_n53_));
  nand3  g10(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n54_));
  inv1   g11(.a(x19), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand3  g15(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x19), .O(z02));
  inv1   g19(.a(x16), .O(new_n63_));
  nand3  g20(.a(new_n47_), .b(x02), .c(x00), .O(new_n64_));
  nand3  g21(.a(x12), .b(x11), .c(x10), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(x01), .O(new_n67_));
  nor3   g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n55_), .c(x18), .d(new_n44_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n67_), .O(z03));
  inv1   g27(.a(x01), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  inv1   g30(.a(new_n64_), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(x10), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(new_n63_), .c(new_n71_), .O(z04));
  inv1   g33(.a(x00), .O(new_n77_));
  nor2   g34(.a(new_n46_), .b(new_n77_), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x10), .c(x09), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(new_n63_), .c(new_n71_), .O(z05));
  nand4  g37(.a(new_n78_), .b(x11), .c(x10), .d(new_n47_), .O(new_n81_));
  aoi21  g38(.a(new_n81_), .b(new_n63_), .c(new_n71_), .O(z06));
  nand4  g39(.a(new_n73_), .b(x11), .c(x10), .d(new_n47_), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(x01), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(new_n53_), .O(z07));
  nand3  g43(.a(x02), .b(new_n71_), .c(new_n77_), .O(new_n87_));
  inv1   g44(.a(x05), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(x04), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n87_), .c(x03), .O(new_n90_));
  nor3   g47(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nor2   g48(.a(new_n55_), .b(x18), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(x17), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n53_), .O(z08));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(x12), .b(new_n72_), .c(x02), .d(new_n71_), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x20), .c(new_n63_), .d(new_n98_), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n55_), .c(x18), .d(x01), .O(new_n102_));
  oai21  g59(.a(new_n100_), .b(new_n97_), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n96_), .c(new_n95_), .d(new_n77_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n53_), .O(z09));
  nand3  g62(.a(new_n55_), .b(x18), .c(x01), .O(new_n106_));
  nand3  g63(.a(x22), .b(x21), .c(new_n101_), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g65(.a(new_n72_), .b(x02), .O(new_n109_));
  nand2  g66(.a(new_n99_), .b(x12), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g68(.a(x16), .b(x15), .O(new_n112_));
  nand3  g69(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n111_), .c(new_n108_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(x00), .c(new_n53_), .O(z10));
  nand2  g73(.a(x18), .b(x01), .O(new_n117_));
  nand3  g74(.a(x21), .b(new_n101_), .c(new_n55_), .O(new_n118_));
  nand4  g75(.a(new_n99_), .b(new_n95_), .c(x20), .d(x15), .O(new_n119_));
  oai22  g76(.a(new_n119_), .b(new_n97_), .c(new_n118_), .d(new_n117_), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n96_), .c(new_n63_), .d(new_n77_), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z11));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(x10), .b(x02), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(x01), .c(x00), .O(new_n125_));
  oai21  g82(.a(x19), .b(x02), .c(x23), .O(new_n126_));
  nand3  g83(.a(new_n55_), .b(x17), .c(new_n46_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n71_), .c(new_n77_), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand3  g87(.a(new_n130_), .b(new_n123_), .c(x09), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(new_n53_), .O(z12));
  inv1   g89(.a(new_n48_), .O(new_n133_));
  oai21  g90(.a(new_n127_), .b(new_n133_), .c(new_n53_), .O(z13));
  nand4  g91(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n135_));
  nor2   g92(.a(new_n135_), .b(x19), .O(z14));
  oai21  g93(.a(x10), .b(new_n71_), .c(x02), .O(new_n137_));
  nand2  g94(.a(new_n137_), .b(x00), .O(new_n138_));
  nand3  g95(.a(x19), .b(new_n46_), .c(new_n71_), .O(new_n139_));
  nand3  g96(.a(new_n139_), .b(new_n138_), .c(new_n53_), .O(z15));
  aoi21  g97(.a(new_n63_), .b(x00), .c(new_n71_), .O(z16));
  nand2  g98(.a(new_n87_), .b(new_n53_), .O(z17));
endmodule


