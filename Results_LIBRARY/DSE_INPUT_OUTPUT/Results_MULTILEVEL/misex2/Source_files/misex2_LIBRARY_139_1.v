// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n132_, new_n133_, new_n135_, new_n136_, new_n139_;
  inv1   g00(.a(x13), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z00));
  nand4  g11(.a(new_n52_), .b(x09), .c(new_n46_), .d(new_n45_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g13(.a(x01), .O(new_n57_));
  nand3  g14(.a(new_n46_), .b(new_n57_), .c(new_n45_), .O(new_n58_));
  nor3   g15(.a(new_n58_), .b(x10), .c(new_n47_), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n49_), .c(new_n48_), .d(new_n44_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  nand3  g18(.a(new_n46_), .b(new_n57_), .c(new_n45_), .O(new_n62_));
  nand4  g19(.a(new_n50_), .b(x18), .c(new_n48_), .d(new_n44_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(z03));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x10), .c(new_n47_), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(x12), .c(x11), .O(z04));
  inv1   g27(.a(x10), .O(new_n71_));
  nor3   g28(.a(new_n67_), .b(new_n71_), .c(new_n47_), .O(z05));
  inv1   g29(.a(new_n69_), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(x11), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(z06));
  inv1   g32(.a(x12), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x11), .c(x10), .d(new_n47_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(x13), .b(new_n57_), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n79_), .O(z07));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x07), .O(new_n84_));
  inv1   g41(.a(x04), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n57_), .d(new_n45_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x08), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n49_), .c(x17), .d(new_n44_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n50_), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  inv1   g51(.a(x11), .O(new_n95_));
  nor2   g52(.a(new_n46_), .b(x01), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n95_), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nor2   g56(.a(x14), .b(x13), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g58(.a(x20), .O(new_n102_));
  nor2   g59(.a(new_n49_), .b(new_n57_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n50_), .O(new_n104_));
  oai21  g61(.a(new_n101_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n94_), .c(new_n93_), .d(new_n45_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(z09));
  nor3   g64(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n50_), .c(x18), .d(x01), .O(new_n109_));
  nand4  g66(.a(new_n96_), .b(new_n44_), .c(x12), .d(new_n95_), .O(new_n110_));
  nor3   g67(.a(new_n99_), .b(new_n98_), .c(x14), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n94_), .c(new_n93_), .d(x20), .O(new_n112_));
  or2    g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n109_), .c(x00), .O(z10));
  nand4  g71(.a(new_n103_), .b(x21), .c(new_n102_), .d(new_n50_), .O(new_n115_));
  nor2   g72(.a(new_n98_), .b(x14), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n93_), .c(x20), .d(new_n99_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n110_), .c(new_n115_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n94_), .c(new_n45_), .O(new_n119_));
  inv1   g76(.a(new_n119_), .O(z11));
  aoi21  g77(.a(x10), .b(x02), .c(new_n57_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x00), .O(new_n122_));
  oai21  g79(.a(x19), .b(x02), .c(x23), .O(new_n123_));
  nand3  g80(.a(new_n50_), .b(x17), .c(new_n46_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(new_n57_), .c(new_n45_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n122_), .c(x24), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x09), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n80_), .O(z12));
  nand4  g86(.a(new_n50_), .b(x17), .c(new_n46_), .d(new_n45_), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(new_n44_), .c(x01), .O(z13));
  nor3   g88(.a(new_n58_), .b(x10), .c(x09), .O(new_n132_));
  nand4  g89(.a(new_n132_), .b(new_n49_), .c(new_n48_), .d(new_n44_), .O(new_n133_));
  nor2   g90(.a(new_n133_), .b(x19), .O(z14));
  aoi21  g91(.a(new_n44_), .b(new_n46_), .c(new_n121_), .O(new_n135_));
  nand4  g92(.a(x19), .b(new_n44_), .c(new_n46_), .d(new_n57_), .O(new_n136_));
  oai21  g93(.a(new_n135_), .b(new_n45_), .c(new_n136_), .O(z15));
  oai21  g94(.a(new_n57_), .b(x00), .c(new_n80_), .O(z16));
  nand2  g95(.a(x02), .b(new_n45_), .O(new_n139_));
  aoi21  g96(.a(new_n139_), .b(new_n44_), .c(x01), .O(z17));
endmodule


