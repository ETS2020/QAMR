// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n150_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  inv1   g008(.a(x10), .O(new_n52_));
  nand2  g009(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g010(.a(x02), .O(new_n54_));
  nor2   g011(.a(x01), .b(x00), .O(new_n55_));
  nand3  g012(.a(new_n55_), .b(x09), .c(new_n54_), .O(new_n56_));
  nor2   g013(.a(x19), .b(x18), .O(new_n57_));
  nand3  g014(.a(new_n57_), .b(new_n44_), .c(x10), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(z01));
  nand2  g016(.a(new_n47_), .b(x09), .O(new_n60_));
  inv1   g017(.a(new_n60_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n45_), .c(new_n44_), .d(new_n52_), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x19), .O(z02));
  nand2  g020(.a(x02), .b(x00), .O(new_n64_));
  nand3  g021(.a(x12), .b(x11), .c(new_n46_), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n64_), .c(x10), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(x01), .O(new_n67_));
  inv1   g024(.a(x19), .O(new_n68_));
  nor3   g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n68_), .c(x18), .d(new_n44_), .O(new_n70_));
  nand2  g027(.a(new_n70_), .b(new_n67_), .O(z03));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g029(.a(new_n72_), .O(new_n73_));
  nand3  g030(.a(new_n73_), .b(x10), .c(new_n46_), .O(new_n74_));
  nor3   g031(.a(new_n74_), .b(x12), .c(x11), .O(z04));
  inv1   g032(.a(x01), .O(new_n76_));
  nand3  g033(.a(x09), .b(x02), .c(x00), .O(new_n77_));
  aoi21  g034(.a(new_n77_), .b(x10), .c(new_n76_), .O(z05));
  inv1   g035(.a(new_n74_), .O(new_n79_));
  nand2  g036(.a(new_n79_), .b(x11), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(z06));
  inv1   g038(.a(x12), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x11), .c(new_n46_), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n53_), .O(z07));
  inv1   g043(.a(x08), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  inv1   g045(.a(x06), .O(new_n89_));
  inv1   g046(.a(x00), .O(new_n90_));
  inv1   g047(.a(x03), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(x02), .c(new_n76_), .d(new_n90_), .O(new_n92_));
  inv1   g049(.a(new_n92_), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n89_), .c(new_n88_), .d(x04), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(x07), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n45_), .c(x17), .d(new_n87_), .O(new_n96_));
  nor2   g053(.a(new_n96_), .b(new_n68_), .O(z08));
  inv1   g054(.a(x21), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  inv1   g056(.a(x11), .O(new_n100_));
  nor2   g057(.a(new_n54_), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(x12), .c(new_n100_), .O(new_n102_));
  inv1   g059(.a(x15), .O(new_n103_));
  inv1   g060(.a(x16), .O(new_n104_));
  nor2   g061(.a(x14), .b(x13), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(x20), .c(new_n104_), .d(new_n103_), .O(new_n106_));
  inv1   g063(.a(x20), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n68_), .c(x18), .d(x01), .O(new_n108_));
  oai21  g065(.a(new_n106_), .b(new_n102_), .c(new_n108_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n99_), .c(new_n98_), .d(new_n90_), .O(new_n110_));
  nand2  g067(.a(new_n110_), .b(new_n53_), .O(z09));
  nand3  g068(.a(new_n68_), .b(x18), .c(x01), .O(new_n112_));
  nand3  g069(.a(x22), .b(x21), .c(new_n107_), .O(new_n113_));
  nor2   g070(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  inv1   g071(.a(x13), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(x12), .c(new_n100_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(new_n117_));
  nand2  g074(.a(new_n117_), .b(new_n101_), .O(new_n118_));
  inv1   g075(.a(new_n118_), .O(new_n119_));
  nand2  g076(.a(x16), .b(x15), .O(new_n120_));
  nand3  g077(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n121_));
  nor3   g078(.a(new_n121_), .b(new_n120_), .c(x14), .O(new_n122_));
  aoi21  g079(.a(new_n122_), .b(new_n119_), .c(new_n114_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(x00), .c(new_n53_), .O(z10));
  nand3  g081(.a(x18), .b(x10), .c(x01), .O(new_n125_));
  nand3  g082(.a(x21), .b(new_n107_), .c(new_n68_), .O(new_n126_));
  nor2   g083(.a(new_n103_), .b(x14), .O(new_n127_));
  nand4  g084(.a(new_n127_), .b(new_n98_), .c(x20), .d(new_n104_), .O(new_n128_));
  oai22  g085(.a(new_n128_), .b(new_n118_), .c(new_n126_), .d(new_n125_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n99_), .c(new_n90_), .O(new_n130_));
  inv1   g087(.a(new_n130_), .O(z11));
  inv1   g088(.a(x24), .O(new_n132_));
  nand4  g089(.a(new_n132_), .b(x09), .c(new_n54_), .d(x00), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(x10), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(x01), .O(new_n135_));
  oai21  g092(.a(x19), .b(x02), .c(x23), .O(new_n136_));
  nand3  g093(.a(new_n68_), .b(x17), .c(new_n54_), .O(new_n137_));
  aoi21  g094(.a(new_n137_), .b(new_n136_), .c(x24), .O(new_n138_));
  nand4  g095(.a(new_n138_), .b(x09), .c(new_n76_), .d(new_n90_), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(new_n135_), .O(z12));
  nand2  g097(.a(new_n47_), .b(x17), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z13));
  nor2   g099(.a(x09), .b(x02), .O(new_n143_));
  nand4  g100(.a(new_n143_), .b(new_n57_), .c(new_n44_), .d(new_n90_), .O(new_n144_));
  aoi21  g101(.a(new_n144_), .b(new_n76_), .c(x10), .O(z14));
  nand2  g102(.a(new_n53_), .b(x00), .O(new_n146_));
  nand2  g103(.a(x19), .b(new_n76_), .O(new_n147_));
  aoi21  g104(.a(new_n147_), .b(new_n146_), .c(x02), .O(z15));
  aoi21  g105(.a(x10), .b(x00), .c(new_n76_), .O(z16));
  nand2  g106(.a(new_n55_), .b(x02), .O(new_n150_));
  inv1   g107(.a(new_n150_), .O(z17));
endmodule


