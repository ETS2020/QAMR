// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n130_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand2  g09(.a(x01), .b(x00), .O(new_n53_));
  nand3  g10(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n54_));
  nor2   g11(.a(x19), .b(x18), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(new_n44_), .c(x10), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z01));
  nand3  g14(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x10), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(new_n45_), .c(new_n44_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  nand4  g18(.a(new_n48_), .b(x18), .c(new_n44_), .d(new_n46_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x19), .O(z03));
  inv1   g20(.a(new_n53_), .O(z04));
  inv1   g21(.a(x01), .O(new_n67_));
  nand3  g22(.a(x02), .b(new_n67_), .c(x00), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(z07));
  inv1   g24(.a(x00), .O(new_n70_));
  nor2   g25(.a(new_n46_), .b(x01), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(new_n73_));
  inv1   g28(.a(x03), .O(new_n74_));
  inv1   g29(.a(x05), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(x04), .c(new_n74_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(new_n77_));
  nor3   g32(.a(x08), .b(x07), .c(x06), .O(new_n78_));
  nand3  g33(.a(x19), .b(new_n45_), .c(x17), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n78_), .c(new_n77_), .d(new_n73_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n53_), .O(z08));
  inv1   g37(.a(x21), .O(new_n83_));
  inv1   g38(.a(x22), .O(new_n84_));
  inv1   g39(.a(x11), .O(new_n85_));
  nand4  g40(.a(x12), .b(new_n85_), .c(x02), .d(new_n67_), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  inv1   g42(.a(x16), .O(new_n88_));
  nor2   g43(.a(x14), .b(x13), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(x20), .c(new_n88_), .d(new_n87_), .O(new_n90_));
  inv1   g45(.a(x19), .O(new_n91_));
  inv1   g46(.a(x20), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n91_), .c(x18), .d(x01), .O(new_n93_));
  oai21  g48(.a(new_n90_), .b(new_n86_), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n84_), .c(new_n83_), .d(new_n70_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z09));
  nand2  g51(.a(new_n91_), .b(x18), .O(new_n97_));
  nand3  g52(.a(x22), .b(x21), .c(new_n92_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n97_), .c(new_n70_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x01), .O(new_n100_));
  inv1   g55(.a(x13), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(x12), .c(new_n85_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nor3   g58(.a(new_n88_), .b(new_n87_), .c(x14), .O(new_n104_));
  nand3  g59(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n103_), .d(new_n73_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n100_), .O(z10));
  nand3  g63(.a(new_n84_), .b(x21), .c(new_n92_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n97_), .c(new_n70_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x01), .O(new_n111_));
  nand2  g66(.a(new_n88_), .b(x15), .O(new_n112_));
  nor3   g67(.a(new_n112_), .b(new_n105_), .c(x14), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n103_), .c(new_n73_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n111_), .O(z11));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  nand3  g71(.a(new_n91_), .b(x17), .c(new_n46_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n116_), .c(x24), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(x09), .c(new_n67_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(x00), .O(z12));
  inv1   g75(.a(new_n48_), .O(new_n121_));
  oai21  g76(.a(new_n117_), .b(new_n121_), .c(new_n53_), .O(z13));
  nor2   g77(.a(x09), .b(x02), .O(new_n123_));
  nor2   g78(.a(x17), .b(x10), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n123_), .c(new_n55_), .d(new_n48_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n53_), .O(z14));
  nand3  g81(.a(x19), .b(new_n46_), .c(new_n67_), .O(new_n127_));
  oai21  g82(.a(new_n71_), .b(new_n70_), .c(new_n127_), .O(z15));
  nor2   g83(.a(new_n67_), .b(x00), .O(z16));
  nand2  g84(.a(new_n48_), .b(x02), .O(new_n130_));
  inv1   g85(.a(new_n130_), .O(z17));
  zero   g86(.O(z05));
  zero   g87(.O(z06));
endmodule


