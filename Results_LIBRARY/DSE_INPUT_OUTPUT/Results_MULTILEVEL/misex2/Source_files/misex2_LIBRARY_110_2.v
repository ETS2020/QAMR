// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n140_, new_n142_, new_n143_,
    new_n145_;
  inv1   g000(.a(x16), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  nor2   g007(.a(x19), .b(x18), .O(new_n51_));
  nand3  g008(.a(new_n51_), .b(new_n50_), .c(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g010(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n52_), .c(new_n45_), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  nand3  g013(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n57_));
  inv1   g014(.a(new_n57_), .O(new_n58_));
  nand3  g015(.a(new_n58_), .b(new_n50_), .c(new_n56_), .O(new_n59_));
  nor3   g016(.a(new_n59_), .b(x19), .c(x18), .O(z02));
  inv1   g017(.a(x00), .O(new_n61_));
  inv1   g018(.a(x01), .O(new_n62_));
  nand3  g019(.a(new_n46_), .b(new_n62_), .c(new_n61_), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nand3  g021(.a(new_n64_), .b(x18), .c(new_n50_), .O(new_n65_));
  nand2  g022(.a(x01), .b(x00), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  nand3  g024(.a(new_n67_), .b(new_n47_), .c(x02), .O(new_n68_));
  nand4  g025(.a(x16), .b(x12), .c(x11), .d(x10), .O(new_n69_));
  oai22  g026(.a(new_n69_), .b(new_n68_), .c(new_n65_), .d(new_n63_), .O(z03));
  inv1   g027(.a(x11), .O(new_n71_));
  inv1   g028(.a(x12), .O(new_n72_));
  nor3   g029(.a(new_n66_), .b(x09), .c(new_n46_), .O(new_n73_));
  nand4  g030(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x10), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(new_n44_), .O(z04));
  nand4  g032(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n76_));
  aoi21  g033(.a(new_n76_), .b(x16), .c(new_n62_), .O(z05));
  nand3  g034(.a(new_n73_), .b(x11), .c(x10), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n44_), .O(z06));
  nand4  g036(.a(new_n72_), .b(x11), .c(x10), .d(new_n47_), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n45_), .O(z07));
  nor2   g040(.a(new_n46_), .b(x01), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n61_), .O(new_n85_));
  inv1   g042(.a(x03), .O(new_n86_));
  inv1   g043(.a(x05), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(x04), .c(new_n86_), .O(new_n88_));
  nor2   g045(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nor3   g046(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nor2   g047(.a(new_n64_), .b(x18), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x17), .O(new_n92_));
  nand2  g049(.a(new_n92_), .b(new_n45_), .O(z08));
  inv1   g050(.a(x21), .O(new_n94_));
  inv1   g051(.a(x22), .O(new_n95_));
  inv1   g052(.a(x13), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(x12), .c(new_n71_), .d(x02), .O(new_n97_));
  inv1   g054(.a(x14), .O(new_n98_));
  inv1   g055(.a(x15), .O(new_n99_));
  nand4  g056(.a(x20), .b(new_n44_), .c(new_n99_), .d(new_n98_), .O(new_n100_));
  inv1   g057(.a(x20), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(new_n64_), .c(x18), .d(x01), .O(new_n102_));
  oai21  g059(.a(new_n100_), .b(new_n97_), .c(new_n102_), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n95_), .c(new_n94_), .d(new_n61_), .O(new_n104_));
  nand2  g061(.a(new_n104_), .b(new_n45_), .O(z09));
  nand3  g062(.a(new_n64_), .b(x18), .c(x01), .O(new_n106_));
  nand3  g063(.a(x22), .b(x21), .c(new_n101_), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g065(.a(x02), .b(new_n62_), .O(new_n109_));
  nand3  g066(.a(new_n96_), .b(x12), .c(new_n71_), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g068(.a(x16), .b(x15), .c(new_n98_), .O(new_n112_));
  nand3  g069(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n113_));
  nor2   g070(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  aoi21  g071(.a(new_n114_), .b(new_n111_), .c(new_n108_), .O(new_n115_));
  oai21  g072(.a(new_n115_), .b(x00), .c(new_n45_), .O(z10));
  nor2   g073(.a(x13), .b(new_n72_), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(new_n84_), .c(new_n71_), .O(new_n118_));
  nand3  g075(.a(x18), .b(x16), .c(x01), .O(new_n119_));
  inv1   g076(.a(new_n119_), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(x21), .c(new_n101_), .d(new_n64_), .O(new_n121_));
  nor2   g078(.a(new_n99_), .b(x14), .O(new_n122_));
  nor2   g079(.a(x21), .b(new_n101_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n122_), .c(new_n44_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n118_), .c(new_n121_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n95_), .c(new_n61_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(z11));
  inv1   g084(.a(x24), .O(new_n128_));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand4  g086(.a(new_n129_), .b(x16), .c(x01), .d(x00), .O(new_n130_));
  oai21  g087(.a(x19), .b(x02), .c(x23), .O(new_n131_));
  nand3  g088(.a(new_n64_), .b(x17), .c(new_n46_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n62_), .c(new_n61_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n130_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n128_), .c(x09), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(z12));
  inv1   g094(.a(new_n48_), .O(new_n138_));
  oai21  g095(.a(new_n132_), .b(new_n138_), .c(new_n45_), .O(z13));
  nand3  g096(.a(new_n51_), .b(new_n50_), .c(new_n56_), .O(new_n140_));
  oai21  g097(.a(new_n140_), .b(new_n49_), .c(new_n45_), .O(z14));
  aoi22  g098(.a(new_n45_), .b(x00), .c(x19), .d(new_n62_), .O(new_n142_));
  nand3  g099(.a(new_n67_), .b(x16), .c(new_n56_), .O(new_n143_));
  oai21  g100(.a(new_n142_), .b(x02), .c(new_n143_), .O(z15));
  nand3  g101(.a(x16), .b(x01), .c(new_n61_), .O(new_n145_));
  inv1   g102(.a(new_n145_), .O(z16));
  nand2  g103(.a(new_n85_), .b(new_n45_), .O(z17));
endmodule


