// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n147_;
  inv1   g000(.a(x06), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x19), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g011(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g012(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g013(.a(x01), .O(new_n57_));
  nand4  g014(.a(new_n45_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x10), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(x09), .c(new_n46_), .d(new_n57_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x00), .O(z02));
  inv1   g018(.a(x03), .O(new_n62_));
  nor2   g019(.a(x06), .b(new_n62_), .O(new_n63_));
  nand3  g020(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g021(.a(new_n64_), .O(new_n65_));
  inv1   g022(.a(x10), .O(new_n66_));
  nor2   g023(.a(new_n66_), .b(x09), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n65_), .c(x12), .d(x11), .O(new_n68_));
  nor3   g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g026(.a(x19), .b(new_n51_), .O(new_n70_));
  nand3  g027(.a(new_n70_), .b(new_n69_), .c(new_n50_), .O(new_n71_));
  aoi21  g028(.a(new_n71_), .b(new_n68_), .c(new_n63_), .O(z03));
  inv1   g029(.a(x00), .O(new_n73_));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand4  g032(.a(new_n45_), .b(new_n75_), .c(new_n74_), .d(x10), .O(new_n76_));
  inv1   g033(.a(new_n76_), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(new_n47_), .c(x02), .d(x01), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n73_), .O(z04));
  nor2   g036(.a(new_n63_), .b(new_n66_), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(x09), .c(x02), .d(x01), .O(new_n81_));
  nor2   g038(.a(new_n81_), .b(new_n73_), .O(z05));
  nor2   g039(.a(new_n63_), .b(new_n74_), .O(new_n83_));
  nand4  g040(.a(new_n83_), .b(x10), .c(new_n47_), .d(x02), .O(new_n84_));
  nor3   g041(.a(new_n84_), .b(new_n57_), .c(new_n73_), .O(z06));
  nand3  g042(.a(new_n67_), .b(new_n75_), .c(x11), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(x01), .O(new_n87_));
  nand4  g044(.a(new_n87_), .b(new_n45_), .c(x02), .d(x00), .O(new_n88_));
  inv1   g045(.a(new_n88_), .O(z07));
  inv1   g046(.a(x04), .O(new_n90_));
  nor2   g047(.a(x05), .b(new_n90_), .O(new_n91_));
  nand3  g048(.a(x19), .b(new_n51_), .c(x17), .O(new_n92_));
  nor3   g049(.a(new_n92_), .b(x08), .c(x07), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n91_), .c(new_n48_), .d(x02), .O(new_n94_));
  aoi21  g051(.a(new_n94_), .b(new_n62_), .c(x06), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  inv1   g053(.a(x22), .O(new_n97_));
  nor2   g054(.a(new_n46_), .b(x01), .O(new_n98_));
  nand3  g055(.a(new_n98_), .b(x12), .c(new_n74_), .O(new_n99_));
  inv1   g056(.a(x15), .O(new_n100_));
  inv1   g057(.a(x16), .O(new_n101_));
  nor2   g058(.a(x14), .b(x13), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x20), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  and2   g061(.a(x18), .b(x01), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n104_), .c(new_n52_), .O(new_n106_));
  oai21  g063(.a(new_n103_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n97_), .c(new_n96_), .d(new_n73_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n45_), .O(z09));
  nor2   g066(.a(new_n97_), .b(new_n96_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n70_), .c(new_n104_), .d(x01), .O(new_n111_));
  nor2   g068(.a(x13), .b(new_n75_), .O(new_n112_));
  nand3  g069(.a(new_n112_), .b(new_n98_), .c(new_n74_), .O(new_n113_));
  inv1   g070(.a(x14), .O(new_n114_));
  nor2   g071(.a(new_n101_), .b(new_n100_), .O(new_n115_));
  nor2   g072(.a(x22), .b(x21), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n115_), .c(x20), .d(new_n114_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(new_n113_), .c(new_n111_), .O(new_n118_));
  nand3  g075(.a(new_n118_), .b(new_n45_), .c(new_n73_), .O(new_n119_));
  inv1   g076(.a(new_n119_), .O(z10));
  nand4  g077(.a(new_n105_), .b(x21), .c(new_n104_), .d(new_n52_), .O(new_n121_));
  nor2   g078(.a(new_n100_), .b(x14), .O(new_n122_));
  nor2   g079(.a(x21), .b(new_n104_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n122_), .c(new_n101_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n113_), .c(new_n121_), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n45_), .c(new_n97_), .d(new_n73_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(z11));
  inv1   g084(.a(x24), .O(new_n128_));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(x01), .c(x00), .O(new_n130_));
  oai21  g087(.a(x19), .b(x02), .c(x23), .O(new_n131_));
  nand3  g088(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n57_), .c(new_n73_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n130_), .O(new_n135_));
  nand4  g092(.a(new_n135_), .b(new_n45_), .c(new_n128_), .d(x09), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(z12));
  inv1   g094(.a(new_n48_), .O(new_n138_));
  oai21  g095(.a(new_n132_), .b(new_n138_), .c(new_n45_), .O(z13));
  nand4  g096(.a(new_n59_), .b(new_n47_), .c(new_n46_), .d(new_n57_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(x00), .O(z14));
  oai21  g098(.a(x10), .b(new_n57_), .c(x02), .O(new_n142_));
  nand2  g099(.a(new_n142_), .b(x00), .O(new_n143_));
  nand3  g100(.a(x19), .b(new_n46_), .c(new_n57_), .O(new_n144_));
  aoi21  g101(.a(new_n144_), .b(new_n143_), .c(new_n63_), .O(z15));
  nor3   g102(.a(new_n63_), .b(new_n57_), .c(x00), .O(z16));
  nand2  g103(.a(new_n98_), .b(new_n73_), .O(new_n147_));
  nand2  g104(.a(new_n147_), .b(new_n45_), .O(z17));
endmodule


