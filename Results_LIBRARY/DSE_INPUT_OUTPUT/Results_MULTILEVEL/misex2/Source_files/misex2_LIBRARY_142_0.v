// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n131_, new_n133_, new_n134_, new_n137_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  nand4  g05(.a(x08), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x09), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(x09), .c(new_n48_), .d(new_n46_), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(x08), .c(x01), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  inv1   g15(.a(x09), .O(new_n59_));
  nor2   g16(.a(new_n49_), .b(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x19), .O(z02));
  nand2  g19(.a(new_n48_), .b(new_n46_), .O(new_n63_));
  nand3  g20(.a(new_n53_), .b(x18), .c(new_n44_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g24(.a(new_n58_), .b(x09), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z03));
  nor2   g27(.a(x08), .b(x01), .O(z08));
  inv1   g28(.a(z08), .O(new_n72_));
  inv1   g29(.a(x11), .O(new_n73_));
  inv1   g30(.a(x12), .O(new_n74_));
  nand3  g31(.a(new_n68_), .b(new_n74_), .c(new_n73_), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n67_), .c(new_n72_), .O(z04));
  nand4  g33(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n58_), .O(z05));
  nor4   g35(.a(new_n67_), .b(new_n73_), .c(new_n58_), .d(x09), .O(z06));
  inv1   g36(.a(x08), .O(new_n80_));
  nand2  g37(.a(new_n59_), .b(x01), .O(new_n81_));
  nand3  g38(.a(new_n74_), .b(x11), .c(x10), .O(new_n82_));
  oai22  g39(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(z07));
  inv1   g42(.a(x21), .O(new_n86_));
  inv1   g43(.a(x22), .O(new_n87_));
  nor2   g44(.a(new_n48_), .b(x01), .O(new_n88_));
  nor2   g45(.a(new_n74_), .b(x11), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(new_n88_), .c(x08), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nor2   g49(.a(x14), .b(x13), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(x20), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  inv1   g51(.a(x20), .O(new_n95_));
  nor2   g52(.a(new_n45_), .b(new_n47_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n53_), .O(new_n97_));
  oai21  g54(.a(new_n94_), .b(new_n90_), .c(new_n97_), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n87_), .c(new_n86_), .d(new_n46_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(z09));
  nor3   g57(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n53_), .c(x18), .d(x01), .O(new_n102_));
  nor3   g59(.a(new_n80_), .b(new_n48_), .c(x01), .O(new_n103_));
  inv1   g60(.a(x13), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(x12), .c(new_n73_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nor3   g63(.a(new_n92_), .b(new_n91_), .c(x14), .O(new_n107_));
  nand3  g64(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n107_), .c(new_n106_), .d(new_n103_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n102_), .c(x00), .O(z10));
  nand4  g68(.a(new_n96_), .b(x21), .c(new_n95_), .d(new_n53_), .O(new_n112_));
  nand4  g69(.a(new_n88_), .b(new_n104_), .c(x12), .d(new_n73_), .O(new_n113_));
  nor2   g70(.a(new_n91_), .b(x14), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n86_), .c(x20), .d(new_n92_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n87_), .c(new_n46_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n72_), .O(z11));
  inv1   g75(.a(x24), .O(new_n119_));
  aoi21  g76(.a(x10), .b(x02), .c(new_n47_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x00), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(x23), .O(new_n122_));
  nand3  g79(.a(new_n53_), .b(x17), .c(new_n48_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g81(.a(new_n124_), .b(x08), .c(new_n47_), .d(new_n46_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n119_), .c(x09), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z12));
  nand4  g85(.a(new_n53_), .b(x17), .c(new_n48_), .d(new_n46_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(x08), .c(x01), .O(z13));
  nand4  g87(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n131_));
  nor2   g88(.a(new_n131_), .b(x19), .O(z14));
  aoi21  g89(.a(x08), .b(new_n48_), .c(new_n120_), .O(new_n133_));
  nand4  g90(.a(x19), .b(x08), .c(new_n48_), .d(new_n47_), .O(new_n134_));
  oai21  g91(.a(new_n133_), .b(new_n46_), .c(new_n134_), .O(z15));
  nor2   g92(.a(new_n47_), .b(x00), .O(z16));
  nand2  g93(.a(x02), .b(new_n46_), .O(new_n137_));
  aoi21  g94(.a(new_n137_), .b(x08), .c(x01), .O(z17));
endmodule


