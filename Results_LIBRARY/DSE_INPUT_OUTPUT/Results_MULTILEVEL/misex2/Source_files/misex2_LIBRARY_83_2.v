// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n75_, new_n76_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n128_, new_n130_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  inv1   g12(.a(x00), .O(new_n56_));
  inv1   g13(.a(x02), .O(new_n57_));
  inv1   g14(.a(x10), .O(new_n58_));
  nor3   g15(.a(x19), .b(x18), .c(x17), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n58_), .c(x09), .d(new_n57_), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(new_n56_), .c(x01), .O(z02));
  inv1   g18(.a(x01), .O(new_n62_));
  nand3  g19(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  nor3   g20(.a(new_n63_), .b(x09), .c(new_n57_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n62_), .c(x00), .O(new_n65_));
  inv1   g22(.a(x19), .O(new_n66_));
  nor2   g23(.a(x02), .b(x01), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x18), .d(new_n44_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n65_), .O(z03));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(x10), .c(new_n46_), .O(new_n72_));
  nor3   g29(.a(new_n72_), .b(x12), .c(x11), .O(z04));
  nor3   g30(.a(new_n70_), .b(new_n58_), .c(new_n46_), .O(z05));
  inv1   g31(.a(x11), .O(new_n75_));
  nand3  g32(.a(x02), .b(x01), .c(x00), .O(new_n76_));
  nor4   g33(.a(new_n76_), .b(new_n75_), .c(new_n58_), .d(x09), .O(z06));
  nor3   g34(.a(new_n72_), .b(x12), .c(new_n75_), .O(z07));
  nor2   g35(.a(x03), .b(new_n57_), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(x04), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n45_), .c(x17), .O(new_n83_));
  nor3   g40(.a(new_n83_), .b(x08), .c(x07), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(new_n85_));
  aoi21  g42(.a(new_n85_), .b(new_n56_), .c(x01), .O(z08));
  nor2   g43(.a(x14), .b(x13), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x12), .c(new_n75_), .d(x02), .O(new_n88_));
  inv1   g45(.a(x15), .O(new_n89_));
  inv1   g46(.a(x16), .O(new_n90_));
  nor2   g47(.a(x22), .b(x21), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(x20), .c(new_n90_), .d(new_n89_), .O(new_n92_));
  oai21  g49(.a(new_n92_), .b(new_n88_), .c(new_n56_), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n62_), .O(new_n94_));
  nor3   g51(.a(new_n45_), .b(new_n62_), .c(x00), .O(new_n95_));
  nor2   g52(.a(x20), .b(x19), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n91_), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n94_), .O(z09));
  nand2  g55(.a(x22), .b(x21), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(x20), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n66_), .c(x18), .d(x01), .O(new_n101_));
  nand2  g58(.a(new_n91_), .b(x20), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  inv1   g60(.a(x13), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(x12), .c(new_n75_), .O(new_n105_));
  nor3   g62(.a(new_n105_), .b(new_n57_), .c(x01), .O(new_n106_));
  nor3   g63(.a(new_n90_), .b(new_n89_), .c(x14), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n106_), .c(new_n103_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n101_), .c(x00), .O(z10));
  nand4  g66(.a(new_n91_), .b(x20), .c(new_n90_), .d(x15), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n88_), .c(new_n56_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n62_), .O(new_n112_));
  inv1   g69(.a(x22), .O(new_n113_));
  nand4  g70(.a(new_n96_), .b(new_n95_), .c(new_n113_), .d(x21), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n112_), .O(z11));
  inv1   g72(.a(x24), .O(new_n116_));
  nand2  g73(.a(x10), .b(x02), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(x01), .c(x00), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n66_), .b(x17), .c(new_n57_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n62_), .c(new_n56_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n118_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n116_), .c(x09), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z12));
  nand2  g82(.a(new_n47_), .b(x17), .O(new_n126_));
  nor2   g83(.a(new_n126_), .b(x19), .O(z13));
  nand4  g84(.a(new_n59_), .b(new_n58_), .c(new_n46_), .d(new_n57_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n56_), .c(x01), .O(z14));
  nand4  g86(.a(x19), .b(new_n57_), .c(new_n62_), .d(new_n56_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n118_), .O(z15));
  nor2   g88(.a(new_n62_), .b(x00), .O(z16));
  aoi21  g89(.a(new_n57_), .b(new_n56_), .c(x01), .O(z17));
endmodule


