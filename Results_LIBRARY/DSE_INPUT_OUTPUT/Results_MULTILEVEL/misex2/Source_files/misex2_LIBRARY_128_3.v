// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n140_, new_n141_, new_n144_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(new_n49_), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nand2  g009(.a(x12), .b(x01), .O(new_n53_));
  nand3  g010(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n54_));
  inv1   g011(.a(x19), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  oai21  g013(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  nand4  g015(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  oai21  g016(.a(new_n59_), .b(new_n54_), .c(new_n53_), .O(z02));
  inv1   g017(.a(x00), .O(new_n61_));
  inv1   g018(.a(x01), .O(new_n62_));
  nand3  g019(.a(new_n46_), .b(new_n62_), .c(new_n61_), .O(new_n63_));
  nand3  g020(.a(new_n55_), .b(x18), .c(new_n44_), .O(new_n64_));
  oai21  g021(.a(new_n64_), .b(new_n63_), .c(new_n53_), .O(z03));
  inv1   g022(.a(x12), .O(new_n66_));
  inv1   g023(.a(x11), .O(new_n67_));
  nor2   g024(.a(new_n46_), .b(new_n61_), .O(new_n68_));
  nand4  g025(.a(new_n68_), .b(new_n67_), .c(x10), .d(new_n47_), .O(new_n69_));
  aoi21  g026(.a(new_n69_), .b(new_n66_), .c(new_n62_), .O(z04));
  nand3  g027(.a(new_n68_), .b(x10), .c(x09), .O(new_n71_));
  aoi21  g028(.a(new_n71_), .b(new_n66_), .c(new_n62_), .O(z05));
  nand3  g029(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  inv1   g030(.a(new_n73_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(x11), .c(x10), .d(new_n47_), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(x12), .O(z06));
  nand4  g033(.a(new_n66_), .b(x11), .c(x10), .d(new_n47_), .O(new_n77_));
  nand2  g034(.a(new_n77_), .b(x01), .O(new_n78_));
  nand3  g035(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  inv1   g036(.a(new_n79_), .O(z07));
  inv1   g037(.a(x03), .O(new_n81_));
  nor2   g038(.a(new_n46_), .b(x01), .O(new_n82_));
  inv1   g039(.a(x04), .O(new_n83_));
  nor2   g040(.a(x05), .b(new_n83_), .O(new_n84_));
  nand4  g041(.a(new_n84_), .b(new_n82_), .c(new_n81_), .d(new_n61_), .O(new_n85_));
  nor3   g042(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nand4  g043(.a(new_n86_), .b(x19), .c(new_n45_), .d(x17), .O(new_n87_));
  oai21  g044(.a(new_n87_), .b(new_n85_), .c(new_n53_), .O(z08));
  inv1   g045(.a(x21), .O(new_n89_));
  inv1   g046(.a(x22), .O(new_n90_));
  nand3  g047(.a(new_n82_), .b(x12), .c(new_n67_), .O(new_n91_));
  inv1   g048(.a(x15), .O(new_n92_));
  inv1   g049(.a(x16), .O(new_n93_));
  nor2   g050(.a(x14), .b(x13), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(x20), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  nor2   g052(.a(x20), .b(x19), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(x18), .c(new_n66_), .d(x01), .O(new_n97_));
  oai21  g054(.a(new_n95_), .b(new_n91_), .c(new_n97_), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(new_n90_), .c(new_n89_), .d(new_n61_), .O(new_n99_));
  inv1   g056(.a(new_n99_), .O(z09));
  nand3  g057(.a(new_n55_), .b(x18), .c(x01), .O(new_n101_));
  inv1   g058(.a(x20), .O(new_n102_));
  nand3  g059(.a(x22), .b(x21), .c(new_n102_), .O(new_n103_));
  nor2   g060(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand2  g061(.a(new_n67_), .b(x02), .O(new_n105_));
  nand2  g062(.a(new_n94_), .b(x12), .O(new_n106_));
  nor2   g063(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g064(.a(x16), .b(x15), .O(new_n108_));
  nand3  g065(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g067(.a(new_n110_), .b(new_n107_), .c(new_n104_), .O(new_n111_));
  oai21  g068(.a(new_n111_), .b(x00), .c(new_n53_), .O(z10));
  nor2   g069(.a(new_n45_), .b(x12), .O(new_n113_));
  nor2   g070(.a(new_n89_), .b(x20), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n113_), .c(new_n55_), .d(x01), .O(new_n115_));
  inv1   g072(.a(x13), .O(new_n116_));
  nand3  g073(.a(new_n116_), .b(x12), .c(new_n67_), .O(new_n117_));
  inv1   g074(.a(new_n117_), .O(new_n118_));
  nor2   g075(.a(new_n92_), .b(x14), .O(new_n119_));
  nand3  g076(.a(new_n89_), .b(x20), .c(new_n93_), .O(new_n120_));
  inv1   g077(.a(new_n120_), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n119_), .c(new_n118_), .d(new_n82_), .O(new_n122_));
  nand2  g079(.a(new_n122_), .b(new_n115_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n90_), .c(new_n61_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n55_), .b(x17), .c(new_n46_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n62_), .c(new_n61_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n126_), .c(x09), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n53_), .O(z12));
  inv1   g092(.a(new_n48_), .O(new_n136_));
  oai21  g093(.a(new_n130_), .b(new_n136_), .c(new_n53_), .O(z13));
  nand3  g094(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n138_));
  oai21  g095(.a(new_n138_), .b(new_n59_), .c(new_n53_), .O(z14));
  aoi22  g096(.a(new_n53_), .b(x00), .c(x19), .d(new_n62_), .O(new_n140_));
  nand4  g097(.a(new_n66_), .b(new_n58_), .c(x01), .d(x00), .O(new_n141_));
  oai21  g098(.a(new_n140_), .b(x02), .c(new_n141_), .O(z15));
  aoi21  g099(.a(new_n66_), .b(x00), .c(new_n62_), .O(z16));
  nand2  g100(.a(new_n48_), .b(x02), .O(new_n144_));
  inv1   g101(.a(new_n144_), .O(z17));
endmodule


