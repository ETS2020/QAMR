// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_;
  nand2  g00(.a(x22), .b(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nand3  g19(.a(new_n45_), .b(new_n62_), .c(new_n61_), .O(new_n63_));
  nand3  g20(.a(new_n51_), .b(x18), .c(new_n49_), .O(new_n64_));
  nor2   g21(.a(x09), .b(new_n45_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(x22), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(x12), .c(x11), .d(x10), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n66_), .c(new_n64_), .d(new_n63_), .O(z03));
  nor2   g26(.a(x12), .b(x11), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n65_), .c(x10), .d(x00), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(new_n67_), .c(new_n62_), .O(z04));
  nor2   g29(.a(new_n45_), .b(new_n61_), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(x10), .c(x09), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(new_n67_), .c(new_n62_), .O(z05));
  nand4  g32(.a(new_n73_), .b(x11), .c(x10), .d(new_n46_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(new_n67_), .c(new_n62_), .O(z06));
  inv1   g34(.a(x12), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n46_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n44_), .O(z07));
  nand3  g39(.a(x02), .b(new_n62_), .c(new_n61_), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x04), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n83_), .c(x03), .O(new_n86_));
  nor3   g43(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  nor2   g44(.a(new_n51_), .b(x18), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x17), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(new_n44_), .O(z08));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x11), .O(new_n92_));
  nand4  g49(.a(x12), .b(new_n92_), .c(x02), .d(new_n62_), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  nor2   g52(.a(x14), .b(x13), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(x20), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n51_), .c(x18), .d(x01), .O(new_n99_));
  oai21  g56(.a(new_n97_), .b(new_n93_), .c(new_n99_), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n67_), .c(new_n91_), .d(new_n61_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(z09));
  inv1   g59(.a(x13), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x12), .c(new_n92_), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n83_), .O(new_n105_));
  nor3   g62(.a(new_n95_), .b(new_n94_), .c(x14), .O(new_n106_));
  nand3  g63(.a(new_n67_), .b(new_n91_), .c(x20), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n44_), .O(z10));
  nor2   g67(.a(new_n50_), .b(new_n62_), .O(new_n111_));
  nand3  g68(.a(x21), .b(new_n98_), .c(new_n51_), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  nand2  g70(.a(x02), .b(new_n62_), .O(new_n114_));
  nor2   g71(.a(new_n104_), .b(new_n114_), .O(new_n115_));
  inv1   g72(.a(x14), .O(new_n116_));
  nand3  g73(.a(new_n95_), .b(x15), .c(new_n116_), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(new_n107_), .O(new_n118_));
  aoi22  g75(.a(new_n118_), .b(new_n115_), .c(new_n113_), .d(new_n111_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(x00), .c(new_n44_), .O(z11));
  inv1   g77(.a(x24), .O(new_n121_));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(new_n67_), .c(x01), .d(x00), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n62_), .c(new_n61_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n121_), .c(x09), .O(new_n129_));
  inv1   g86(.a(new_n129_), .O(z12));
  inv1   g87(.a(new_n47_), .O(new_n131_));
  oai21  g88(.a(new_n125_), .b(new_n131_), .c(new_n44_), .O(z13));
  nand3  g89(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(new_n134_));
  nand4  g91(.a(new_n134_), .b(new_n50_), .c(new_n49_), .d(new_n58_), .O(new_n135_));
  nor2   g92(.a(new_n135_), .b(x19), .O(z14));
  aoi22  g93(.a(new_n44_), .b(x00), .c(x19), .d(new_n62_), .O(new_n137_));
  nand4  g94(.a(new_n67_), .b(new_n58_), .c(x01), .d(x00), .O(new_n138_));
  oai21  g95(.a(new_n137_), .b(x02), .c(new_n138_), .O(z15));
  aoi21  g96(.a(new_n67_), .b(x00), .c(new_n62_), .O(z16));
  nand2  g97(.a(new_n83_), .b(new_n44_), .O(z17));
endmodule


