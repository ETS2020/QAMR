// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n126_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nor2   g02(.a(x09), .b(x02), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  oai21  g08(.a(new_n51_), .b(new_n47_), .c(new_n44_), .O(z00));
  inv1   g09(.a(x02), .O(new_n53_));
  nand3  g10(.a(new_n45_), .b(x09), .c(new_n53_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  nand3  g14(.a(new_n45_), .b(x09), .c(new_n53_), .O(new_n58_));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n59_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n58_), .c(new_n44_), .O(z02));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x01), .O(new_n63_));
  nand3  g20(.a(new_n53_), .b(new_n63_), .c(new_n62_), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n49_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n48_), .O(new_n66_));
  oai21  g23(.a(new_n66_), .b(new_n64_), .c(new_n44_), .O(z03));
  inv1   g24(.a(new_n44_), .O(z04));
  nand3  g25(.a(x02), .b(new_n63_), .c(x00), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z07));
  nor2   g27(.a(new_n53_), .b(x01), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n62_), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(new_n74_));
  inv1   g30(.a(x03), .O(new_n75_));
  inv1   g31(.a(x05), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(x04), .c(new_n75_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nor3   g34(.a(x08), .b(x07), .c(x06), .O(new_n79_));
  nand3  g35(.a(x19), .b(new_n49_), .c(x17), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand4  g37(.a(new_n81_), .b(new_n79_), .c(new_n78_), .d(new_n74_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n44_), .O(z08));
  inv1   g39(.a(new_n65_), .O(new_n84_));
  inv1   g40(.a(x20), .O(new_n85_));
  inv1   g41(.a(x21), .O(new_n86_));
  inv1   g42(.a(x22), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n84_), .c(new_n62_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(x01), .O(new_n90_));
  inv1   g46(.a(x11), .O(new_n91_));
  inv1   g47(.a(x13), .O(new_n92_));
  nand3  g48(.a(new_n92_), .b(x12), .c(new_n91_), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nor3   g50(.a(x16), .b(x15), .c(x14), .O(new_n95_));
  nand3  g51(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n95_), .c(new_n94_), .d(new_n74_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n90_), .O(z09));
  nor2   g55(.a(new_n87_), .b(new_n86_), .O(new_n100_));
  nand4  g56(.a(new_n100_), .b(new_n65_), .c(new_n85_), .d(x01), .O(new_n101_));
  inv1   g57(.a(x15), .O(new_n102_));
  inv1   g58(.a(x16), .O(new_n103_));
  nor3   g59(.a(new_n103_), .b(new_n102_), .c(x14), .O(new_n104_));
  nand4  g60(.a(new_n104_), .b(new_n97_), .c(new_n94_), .d(new_n72_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n101_), .c(x00), .O(z10));
  inv1   g62(.a(x12), .O(new_n107_));
  nor2   g63(.a(x13), .b(new_n107_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n72_), .c(new_n91_), .O(new_n109_));
  and2   g65(.a(x18), .b(x01), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(x21), .c(new_n85_), .d(new_n50_), .O(new_n111_));
  nor2   g67(.a(new_n102_), .b(x14), .O(new_n112_));
  nor2   g68(.a(x21), .b(new_n85_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n112_), .c(new_n103_), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n109_), .c(new_n111_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n87_), .c(new_n62_), .O(new_n116_));
  inv1   g72(.a(new_n116_), .O(z11));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g74(.a(new_n50_), .b(x17), .c(new_n53_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n118_), .c(x24), .O(new_n120_));
  nand4  g76(.a(new_n120_), .b(x09), .c(new_n63_), .d(new_n62_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n44_), .O(z12));
  inv1   g78(.a(new_n45_), .O(new_n123_));
  oai21  g79(.a(new_n119_), .b(new_n123_), .c(new_n44_), .O(z13));
  oai21  g80(.a(new_n60_), .b(new_n47_), .c(new_n44_), .O(z14));
  nand3  g81(.a(x19), .b(new_n53_), .c(new_n63_), .O(new_n126_));
  oai21  g82(.a(new_n72_), .b(new_n62_), .c(new_n126_), .O(z15));
  nand2  g83(.a(new_n73_), .b(new_n44_), .O(z17));
  zero   g84(.O(z05));
  inv1   g85(.a(new_n44_), .O(z06));
  buf    g86(.a(x01), .O(z16));
endmodule


