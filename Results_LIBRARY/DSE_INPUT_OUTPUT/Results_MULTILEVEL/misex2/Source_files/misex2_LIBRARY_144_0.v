// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n125_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x22), .c(x01), .O(z00));
  inv1   g05(.a(x22), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  inv1   g08(.a(x10), .O(new_n52_));
  inv1   g09(.a(x00), .O(new_n53_));
  inv1   g10(.a(x01), .O(new_n54_));
  inv1   g11(.a(x02), .O(new_n55_));
  nand4  g12(.a(x09), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n49_), .O(z01));
  nand3  g16(.a(new_n46_), .b(new_n44_), .c(new_n52_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x09), .c(new_n55_), .d(new_n53_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(x22), .c(x01), .O(z02));
  nand2  g20(.a(new_n55_), .b(new_n53_), .O(new_n64_));
  nand3  g21(.a(new_n51_), .b(x18), .c(new_n44_), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n64_), .c(x22), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n54_), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g25(.a(new_n52_), .b(x09), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  nand2  g28(.a(new_n49_), .b(new_n54_), .O(new_n72_));
  nor2   g29(.a(x12), .b(x11), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n68_), .c(new_n72_), .O(z04));
  nand2  g32(.a(x01), .b(x00), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(z05));
  inv1   g35(.a(x09), .O(new_n79_));
  nand3  g36(.a(x11), .b(x10), .c(new_n79_), .O(new_n80_));
  oai21  g37(.a(new_n80_), .b(new_n68_), .c(new_n72_), .O(z06));
  inv1   g38(.a(x12), .O(new_n82_));
  nand3  g39(.a(new_n69_), .b(new_n82_), .c(x11), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(x01), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(new_n72_), .O(z07));
  nand2  g43(.a(x02), .b(new_n53_), .O(new_n87_));
  inv1   g44(.a(x05), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(x04), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n87_), .c(x03), .O(new_n90_));
  nor3   g47(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nand3  g48(.a(x19), .b(new_n50_), .c(x17), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(x22), .c(x01), .O(z08));
  inv1   g52(.a(x21), .O(new_n96_));
  nor2   g53(.a(new_n54_), .b(x00), .O(z16));
  nand3  g54(.a(z16), .b(new_n51_), .c(x18), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x20), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n49_), .c(new_n96_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z09));
  nand3  g58(.a(x18), .b(x01), .c(new_n53_), .O(new_n102_));
  inv1   g59(.a(x20), .O(new_n103_));
  nand4  g60(.a(x22), .b(x21), .c(new_n103_), .d(new_n51_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n102_), .c(new_n72_), .O(z10));
  nand3  g62(.a(new_n99_), .b(new_n49_), .c(x21), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(z11));
  inv1   g64(.a(x24), .O(new_n108_));
  nand2  g65(.a(x10), .b(x02), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x01), .c(x00), .O(new_n110_));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g68(.a(new_n51_), .b(x17), .c(new_n55_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n54_), .c(new_n53_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n108_), .c(x09), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n72_), .O(z12));
  nand4  g74(.a(new_n51_), .b(x17), .c(new_n55_), .d(new_n53_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(x22), .c(x01), .O(z13));
  nand2  g76(.a(new_n61_), .b(new_n45_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(x22), .c(x01), .O(z14));
  aoi21  g78(.a(new_n52_), .b(x01), .c(new_n55_), .O(new_n122_));
  aoi21  g79(.a(x19), .b(new_n55_), .c(new_n49_), .O(new_n123_));
  oai22  g80(.a(new_n123_), .b(x01), .c(new_n122_), .d(new_n53_), .O(z15));
  nand4  g81(.a(x22), .b(x02), .c(new_n54_), .d(new_n53_), .O(new_n125_));
  inv1   g82(.a(new_n125_), .O(z17));
endmodule


