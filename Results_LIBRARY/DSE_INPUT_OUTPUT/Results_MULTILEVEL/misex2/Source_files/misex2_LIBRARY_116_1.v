// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_;
  inv1   g00(.a(x02), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  nand4  g09(.a(x09), .b(new_n44_), .c(new_n52_), .d(new_n45_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n47_), .b(new_n57_), .c(x09), .d(new_n45_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g16(.a(x19), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(x18), .c(new_n50_), .d(new_n45_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n44_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n52_), .O(new_n63_));
  nand3  g20(.a(new_n46_), .b(x02), .c(x00), .O(new_n64_));
  nand3  g21(.a(x12), .b(x11), .c(x10), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(new_n57_), .c(x09), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  nand3  g28(.a(x10), .b(x09), .c(x00), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n44_), .O(z05));
  nand2  g30(.a(new_n69_), .b(x11), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(z06));
  nor2   g32(.a(x12), .b(new_n67_), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x10), .c(new_n46_), .d(x00), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x01), .c(new_n44_), .O(z07));
  nand2  g35(.a(x02), .b(new_n52_), .O(new_n80_));
  nand3  g36(.a(x18), .b(x01), .c(new_n45_), .O(new_n81_));
  inv1   g37(.a(x20), .O(new_n82_));
  inv1   g38(.a(x21), .O(new_n83_));
  inv1   g39(.a(x22), .O(new_n84_));
  nand4  g40(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n60_), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n81_), .c(new_n80_), .O(z09));
  nor2   g42(.a(new_n52_), .b(x00), .O(z16));
  nand3  g43(.a(z16), .b(new_n60_), .c(x18), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  nand4  g45(.a(new_n89_), .b(x22), .c(x21), .d(new_n82_), .O(new_n90_));
  inv1   g46(.a(new_n90_), .O(z10));
  nand4  g47(.a(new_n84_), .b(x21), .c(new_n82_), .d(new_n60_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n81_), .c(new_n80_), .O(z11));
  inv1   g49(.a(x24), .O(new_n94_));
  nand2  g50(.a(x10), .b(x02), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(x01), .c(x00), .O(new_n96_));
  nand2  g52(.a(x23), .b(x19), .O(new_n97_));
  oai21  g53(.a(x19), .b(new_n50_), .c(new_n97_), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(new_n44_), .c(new_n52_), .d(new_n45_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n94_), .c(x09), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(z12));
  nand3  g58(.a(new_n60_), .b(x17), .c(new_n45_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g60(.a(new_n46_), .b(new_n44_), .c(new_n52_), .d(new_n45_), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n107_));
  nor2   g63(.a(new_n107_), .b(x19), .O(z14));
  aoi21  g64(.a(new_n57_), .b(x01), .c(new_n44_), .O(new_n109_));
  nand3  g65(.a(x19), .b(new_n44_), .c(new_n52_), .O(new_n110_));
  oai21  g66(.a(new_n109_), .b(new_n45_), .c(new_n110_), .O(z15));
  zero   g67(.O(z08));
  zero   g68(.O(z17));
endmodule


