// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n140_, new_n142_;
  inv1   g00(.a(x05), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n51_), .c(new_n50_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x01), .O(new_n63_));
  nand3  g20(.a(new_n46_), .b(new_n63_), .c(new_n62_), .O(new_n64_));
  nand3  g21(.a(new_n52_), .b(x18), .c(new_n50_), .O(new_n65_));
  nor2   g22(.a(new_n63_), .b(new_n62_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(x05), .c(x02), .O(new_n67_));
  nand4  g24(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n67_), .c(new_n65_), .d(new_n64_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  nand3  g27(.a(new_n66_), .b(x05), .c(x02), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n59_), .c(x09), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(x12), .O(z04));
  nand4  g31(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x05), .c(new_n63_), .O(z05));
  nand2  g33(.a(new_n72_), .b(x11), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z06));
  inv1   g35(.a(x12), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x11), .c(x10), .d(new_n47_), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n45_), .O(z07));
  inv1   g40(.a(x08), .O(new_n84_));
  inv1   g41(.a(x06), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n63_), .d(new_n62_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n85_), .c(new_n44_), .d(x04), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x07), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n51_), .c(x17), .d(new_n84_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n52_), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nor2   g51(.a(new_n46_), .b(x01), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n70_), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  nor2   g57(.a(x20), .b(x19), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x18), .c(x01), .O(new_n102_));
  oai21  g59(.a(new_n100_), .b(new_n96_), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n94_), .c(new_n93_), .d(new_n62_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n45_), .O(z09));
  nand3  g62(.a(x18), .b(x05), .c(x01), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n101_), .c(x22), .d(x21), .O(new_n108_));
  nor2   g65(.a(x13), .b(new_n79_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n95_), .c(new_n70_), .O(new_n110_));
  nor3   g67(.a(new_n98_), .b(new_n97_), .c(x14), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n94_), .c(new_n93_), .d(x20), .O(new_n112_));
  or2    g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n108_), .c(x00), .O(z10));
  nor2   g71(.a(new_n93_), .b(x20), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n107_), .c(new_n52_), .O(new_n116_));
  nor2   g73(.a(new_n97_), .b(x14), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n93_), .c(x20), .d(new_n98_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n110_), .c(new_n116_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n94_), .c(new_n62_), .O(new_n120_));
  inv1   g77(.a(new_n120_), .O(z11));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(x01), .c(x00), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n63_), .c(new_n62_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(new_n123_), .c(x24), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x09), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n45_), .O(z12));
  inv1   g87(.a(new_n48_), .O(new_n131_));
  oai21  g88(.a(new_n125_), .b(new_n131_), .c(new_n45_), .O(z13));
  nand3  g89(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(new_n134_));
  nand4  g91(.a(new_n134_), .b(new_n51_), .c(new_n50_), .d(new_n59_), .O(new_n135_));
  nor2   g92(.a(new_n135_), .b(x19), .O(z14));
  aoi22  g93(.a(new_n45_), .b(x00), .c(x19), .d(new_n63_), .O(new_n137_));
  nand3  g94(.a(new_n66_), .b(new_n59_), .c(x05), .O(new_n138_));
  oai21  g95(.a(new_n137_), .b(x02), .c(new_n138_), .O(z15));
  nand3  g96(.a(x05), .b(x01), .c(new_n62_), .O(new_n140_));
  inv1   g97(.a(new_n140_), .O(z16));
  nand2  g98(.a(new_n95_), .b(new_n62_), .O(new_n142_));
  nand2  g99(.a(new_n142_), .b(new_n45_), .O(z17));
endmodule


