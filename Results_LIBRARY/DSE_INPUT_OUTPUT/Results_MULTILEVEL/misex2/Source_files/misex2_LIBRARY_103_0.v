// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n133_, new_n135_, new_n136_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x10), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n44_), .c(x15), .O(new_n52_));
  nor3   g09(.a(new_n52_), .b(x19), .c(x18), .O(z00));
  nand3  g10(.a(x09), .b(new_n48_), .c(new_n46_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nor2   g12(.a(x19), .b(x18), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n55_), .c(new_n44_), .d(x10), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(x15), .c(x01), .O(z01));
  nand3  g15(.a(new_n56_), .b(new_n44_), .c(new_n45_), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n55_), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(x15), .c(x01), .O(z02));
  nand3  g19(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(x18), .c(new_n44_), .d(x15), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g23(.a(x12), .b(x11), .c(x10), .d(new_n49_), .O(new_n67_));
  oai22  g24(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n63_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n69_), .c(x10), .d(new_n49_), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(x12), .O(z04));
  nor3   g30(.a(new_n70_), .b(new_n45_), .c(new_n49_), .O(z05));
  inv1   g31(.a(x15), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(new_n47_), .O(new_n76_));
  nand3  g33(.a(x11), .b(x10), .c(new_n49_), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n66_), .c(new_n76_), .O(z06));
  nand2  g35(.a(new_n49_), .b(x01), .O(new_n79_));
  inv1   g36(.a(x12), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x11), .c(x10), .O(new_n81_));
  oai22  g38(.a(new_n81_), .b(new_n79_), .c(new_n75_), .d(x01), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(z07));
  nand2  g41(.a(x02), .b(new_n46_), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(x04), .c(new_n86_), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nor3   g46(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nor2   g47(.a(new_n64_), .b(x18), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x17), .O(new_n92_));
  aoi21  g49(.a(new_n92_), .b(x15), .c(x01), .O(z08));
  and2   g50(.a(x18), .b(x01), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n46_), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(new_n64_), .O(new_n99_));
  oai21  g56(.a(new_n99_), .b(new_n95_), .c(new_n76_), .O(z09));
  nor3   g57(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n64_), .c(x18), .d(x01), .O(new_n102_));
  nor2   g59(.a(new_n48_), .b(x01), .O(new_n103_));
  nor2   g60(.a(x13), .b(new_n80_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n69_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  inv1   g63(.a(x16), .O(new_n107_));
  nor3   g64(.a(new_n107_), .b(new_n75_), .c(x14), .O(new_n108_));
  nand3  g65(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n108_), .c(new_n106_), .d(new_n103_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n102_), .c(x00), .O(z10));
  nand3  g69(.a(new_n104_), .b(new_n103_), .c(new_n69_), .O(new_n113_));
  nand4  g70(.a(new_n94_), .b(x21), .c(new_n96_), .d(new_n64_), .O(new_n114_));
  nor2   g71(.a(new_n75_), .b(x14), .O(new_n115_));
  nor2   g72(.a(x21), .b(new_n96_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n115_), .c(new_n107_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n113_), .c(new_n114_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n98_), .c(new_n46_), .O(new_n119_));
  inv1   g76(.a(new_n119_), .O(z11));
  inv1   g77(.a(x24), .O(new_n121_));
  aoi21  g78(.a(x10), .b(x02), .c(new_n47_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x00), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n64_), .b(x17), .c(new_n48_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n47_), .c(new_n46_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n121_), .c(x09), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n76_), .O(z12));
  nand3  g87(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n131_));
  nor4   g88(.a(new_n131_), .b(x19), .c(new_n44_), .d(new_n75_), .O(z13));
  nand4  g89(.a(new_n60_), .b(new_n49_), .c(new_n48_), .d(new_n46_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(x15), .c(x01), .O(z14));
  aoi21  g91(.a(x15), .b(new_n48_), .c(new_n122_), .O(new_n135_));
  nand4  g92(.a(x19), .b(x15), .c(new_n48_), .d(new_n47_), .O(new_n136_));
  oai21  g93(.a(new_n135_), .b(new_n46_), .c(new_n136_), .O(z15));
  oai21  g94(.a(new_n47_), .b(x00), .c(new_n76_), .O(z16));
  aoi21  g95(.a(new_n85_), .b(x15), .c(x01), .O(z17));
endmodule


