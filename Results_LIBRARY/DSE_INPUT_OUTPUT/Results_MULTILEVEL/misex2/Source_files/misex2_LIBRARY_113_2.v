// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n146_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  nand4  g010(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n54_));
  inv1   g011(.a(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  nand4  g015(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x19), .O(z02));
  nand3  g017(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n61_));
  inv1   g018(.a(x19), .O(new_n62_));
  nand3  g019(.a(new_n62_), .b(x18), .c(new_n44_), .O(new_n63_));
  nor2   g020(.a(new_n47_), .b(new_n46_), .O(new_n64_));
  nor2   g021(.a(x09), .b(new_n48_), .O(new_n65_));
  nand2  g022(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand4  g023(.a(x17), .b(x12), .c(x11), .d(x10), .O(new_n67_));
  oai22  g024(.a(new_n67_), .b(new_n66_), .c(new_n63_), .d(new_n61_), .O(z03));
  nor2   g025(.a(x12), .b(x11), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n65_), .c(x10), .d(x00), .O(new_n70_));
  aoi21  g027(.a(new_n70_), .b(x17), .c(new_n47_), .O(z04));
  nand2  g028(.a(new_n64_), .b(x02), .O(new_n72_));
  nor4   g029(.a(new_n72_), .b(new_n44_), .c(new_n58_), .d(new_n49_), .O(z05));
  inv1   g030(.a(new_n72_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(x11), .c(x10), .d(new_n49_), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(new_n44_), .O(z06));
  inv1   g033(.a(x12), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(x11), .c(x10), .d(new_n49_), .O(new_n78_));
  nand2  g035(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g036(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g037(.a(new_n44_), .b(x01), .O(new_n81_));
  nand2  g038(.a(new_n81_), .b(new_n80_), .O(z07));
  nor2   g039(.a(new_n48_), .b(x01), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n46_), .O(new_n84_));
  inv1   g041(.a(x05), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x04), .O(new_n86_));
  nor3   g043(.a(new_n86_), .b(new_n84_), .c(x03), .O(new_n87_));
  nor3   g044(.a(x08), .b(x07), .c(x06), .O(new_n88_));
  nor2   g045(.a(new_n62_), .b(x18), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(x17), .O(new_n90_));
  nand2  g047(.a(new_n90_), .b(new_n81_), .O(z08));
  inv1   g048(.a(x21), .O(new_n92_));
  inv1   g049(.a(x22), .O(new_n93_));
  inv1   g050(.a(x11), .O(new_n94_));
  nand3  g051(.a(new_n83_), .b(x12), .c(new_n94_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  nor2   g056(.a(x20), .b(x19), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x18), .c(x17), .d(x01), .O(new_n101_));
  oai21  g058(.a(new_n99_), .b(new_n95_), .c(new_n101_), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(new_n93_), .c(new_n92_), .d(new_n46_), .O(new_n103_));
  inv1   g060(.a(new_n103_), .O(z09));
  nand3  g061(.a(new_n62_), .b(x18), .c(x01), .O(new_n105_));
  inv1   g062(.a(x20), .O(new_n106_));
  nand3  g063(.a(x22), .b(x21), .c(new_n106_), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  inv1   g065(.a(x13), .O(new_n109_));
  nand3  g066(.a(new_n109_), .b(x12), .c(new_n94_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nand2  g068(.a(new_n111_), .b(new_n83_), .O(new_n112_));
  inv1   g069(.a(new_n112_), .O(new_n113_));
  nand2  g070(.a(x16), .b(x15), .O(new_n114_));
  nand3  g071(.a(new_n93_), .b(new_n92_), .c(x20), .O(new_n115_));
  nor3   g072(.a(new_n115_), .b(new_n114_), .c(x14), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n113_), .c(new_n108_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n81_), .O(z10));
  nand3  g075(.a(x18), .b(x17), .c(x01), .O(new_n119_));
  nand3  g076(.a(x21), .b(new_n106_), .c(new_n62_), .O(new_n120_));
  nor2   g077(.a(new_n96_), .b(x14), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n92_), .c(x20), .d(new_n97_), .O(new_n122_));
  oai22  g079(.a(new_n122_), .b(new_n112_), .c(new_n120_), .d(new_n119_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n93_), .c(new_n46_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  nor2   g085(.a(x19), .b(x02), .O(new_n129_));
  oai21  g086(.a(new_n129_), .b(x23), .c(x17), .O(new_n130_));
  oai21  g087(.a(x19), .b(x02), .c(x23), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n47_), .c(new_n46_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n128_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n126_), .c(x09), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n81_), .O(z12));
  nand2  g093(.a(new_n47_), .b(new_n46_), .O(new_n137_));
  nand3  g094(.a(new_n62_), .b(x17), .c(new_n48_), .O(new_n138_));
  oai21  g095(.a(new_n138_), .b(new_n137_), .c(new_n81_), .O(z13));
  nor3   g096(.a(x09), .b(x02), .c(x00), .O(new_n140_));
  nand4  g097(.a(new_n140_), .b(new_n62_), .c(new_n45_), .d(new_n58_), .O(new_n141_));
  aoi21  g098(.a(new_n141_), .b(new_n47_), .c(x17), .O(z14));
  aoi22  g099(.a(new_n81_), .b(x00), .c(x19), .d(new_n47_), .O(new_n143_));
  nand3  g100(.a(new_n64_), .b(x17), .c(new_n58_), .O(new_n144_));
  oai21  g101(.a(new_n143_), .b(x02), .c(new_n144_), .O(z15));
  nand3  g102(.a(x17), .b(x01), .c(new_n46_), .O(new_n146_));
  inv1   g103(.a(new_n146_), .O(z16));
  nand2  g104(.a(new_n84_), .b(new_n81_), .O(z17));
endmodule


