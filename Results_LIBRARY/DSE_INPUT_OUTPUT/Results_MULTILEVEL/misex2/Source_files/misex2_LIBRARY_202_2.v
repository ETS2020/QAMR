// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_;
  inv1   g000(.a(x23), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x09), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(x17), .O(new_n51_));
  inv1   g008(.a(x18), .O(new_n52_));
  inv1   g009(.a(x19), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z00));
  inv1   g012(.a(new_n54_), .O(new_n56_));
  nor3   g013(.a(x02), .b(x01), .c(x00), .O(new_n57_));
  nand2  g014(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g015(.a(new_n58_), .b(x23), .c(new_n49_), .O(z01));
  nand3  g016(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n60_));
  nor3   g017(.a(new_n60_), .b(x10), .c(new_n49_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(new_n44_), .O(z02));
  nor2   g020(.a(x19), .b(new_n52_), .O(new_n64_));
  nand3  g021(.a(new_n64_), .b(new_n57_), .c(new_n51_), .O(new_n65_));
  nand3  g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  inv1   g024(.a(x10), .O(new_n68_));
  nor2   g025(.a(new_n68_), .b(x09), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n67_), .c(x12), .d(x11), .O(new_n70_));
  nand3  g027(.a(new_n70_), .b(new_n65_), .c(new_n45_), .O(z03));
  inv1   g028(.a(x11), .O(new_n72_));
  nand3  g029(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  nor3   g030(.a(new_n73_), .b(new_n68_), .c(x09), .O(new_n74_));
  nand2  g031(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(x12), .O(z04));
  nor4   g033(.a(new_n73_), .b(new_n44_), .c(new_n68_), .d(new_n49_), .O(z05));
  nand2  g034(.a(new_n74_), .b(x11), .O(new_n78_));
  inv1   g035(.a(new_n78_), .O(z06));
  inv1   g036(.a(x12), .O(new_n80_));
  nand3  g037(.a(new_n69_), .b(new_n80_), .c(x11), .O(new_n81_));
  nand2  g038(.a(new_n81_), .b(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n45_), .O(z07));
  nor2   g041(.a(new_n48_), .b(x01), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n46_), .O(new_n86_));
  inv1   g043(.a(x03), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  nand3  g045(.a(new_n88_), .b(x04), .c(new_n87_), .O(new_n89_));
  nor2   g046(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nor3   g047(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nor2   g048(.a(new_n53_), .b(x18), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(x17), .O(new_n93_));
  nand2  g050(.a(new_n93_), .b(new_n45_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n72_), .c(x02), .d(new_n47_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g058(.a(x20), .O(new_n102_));
  and2   g059(.a(x18), .b(x01), .O(new_n103_));
  nand3  g060(.a(new_n103_), .b(new_n102_), .c(new_n53_), .O(new_n104_));
  oai21  g061(.a(new_n101_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n96_), .c(new_n95_), .d(new_n46_), .O(new_n106_));
  nand2  g063(.a(new_n106_), .b(new_n45_), .O(z09));
  nor2   g064(.a(new_n96_), .b(new_n95_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n64_), .c(new_n102_), .d(x01), .O(new_n109_));
  nor2   g066(.a(x13), .b(new_n80_), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(new_n85_), .c(new_n72_), .O(new_n111_));
  inv1   g068(.a(x14), .O(new_n112_));
  nor2   g069(.a(new_n99_), .b(new_n98_), .O(new_n113_));
  nor2   g070(.a(x22), .b(x21), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n113_), .c(x20), .d(new_n112_), .O(new_n115_));
  oai21  g072(.a(new_n115_), .b(new_n111_), .c(new_n109_), .O(new_n116_));
  nand3  g073(.a(new_n116_), .b(new_n45_), .c(new_n46_), .O(new_n117_));
  inv1   g074(.a(new_n117_), .O(z10));
  nand4  g075(.a(new_n103_), .b(x21), .c(new_n102_), .d(new_n53_), .O(new_n119_));
  nor2   g076(.a(new_n98_), .b(x14), .O(new_n120_));
  nor2   g077(.a(x21), .b(new_n102_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n120_), .c(new_n99_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n111_), .c(new_n119_), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n45_), .c(new_n96_), .d(new_n46_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  nand2  g082(.a(x10), .b(x02), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  nand3  g084(.a(new_n53_), .b(new_n51_), .c(new_n48_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n47_), .c(new_n46_), .O(new_n129_));
  aoi21  g086(.a(new_n129_), .b(new_n127_), .c(x24), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x23), .c(x09), .O(new_n131_));
  inv1   g088(.a(new_n131_), .O(z12));
  aoi21  g089(.a(new_n44_), .b(x09), .c(x19), .O(new_n133_));
  nand4  g090(.a(new_n133_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n134_));
  nor2   g091(.a(new_n134_), .b(x00), .O(z13));
  nand4  g092(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(new_n137_));
  nand4  g094(.a(new_n137_), .b(new_n52_), .c(new_n51_), .d(new_n68_), .O(new_n138_));
  nor2   g095(.a(new_n138_), .b(x19), .O(z14));
  oai21  g096(.a(x10), .b(new_n47_), .c(x02), .O(new_n140_));
  nand2  g097(.a(new_n140_), .b(x00), .O(new_n141_));
  nand3  g098(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n142_));
  nand3  g099(.a(new_n142_), .b(new_n141_), .c(new_n45_), .O(z15));
  oai21  g100(.a(new_n47_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g101(.a(new_n86_), .b(new_n45_), .O(z17));
endmodule


