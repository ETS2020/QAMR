// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n135_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  inv1   g03(.a(x00), .O(new_n47_));
  inv1   g04(.a(x01), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  or2    g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n45_), .c(new_n44_), .d(x16), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x19), .O(z00));
  inv1   g11(.a(x09), .O(new_n55_));
  nor3   g12(.a(new_n50_), .b(new_n46_), .c(new_n55_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x16), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x19), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n46_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x09), .c(new_n49_), .d(new_n47_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(x16), .c(x01), .O(z02));
  nand2  g20(.a(new_n49_), .b(new_n47_), .O(new_n64_));
  nand3  g21(.a(new_n59_), .b(x18), .c(new_n44_), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n48_), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g25(.a(new_n46_), .b(x09), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  inv1   g28(.a(x16), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n48_), .O(new_n73_));
  nor2   g30(.a(x12), .b(x11), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n68_), .c(new_n73_), .O(z04));
  nand4  g33(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n46_), .O(z05));
  nand3  g35(.a(x11), .b(x10), .c(new_n55_), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n68_), .c(new_n73_), .O(z06));
  nand2  g37(.a(new_n55_), .b(x01), .O(new_n81_));
  inv1   g38(.a(x12), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x11), .c(x10), .O(new_n83_));
  oai22  g40(.a(new_n83_), .b(new_n81_), .c(new_n72_), .d(x01), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z07));
  inv1   g43(.a(x05), .O(new_n87_));
  inv1   g44(.a(x06), .O(new_n88_));
  inv1   g45(.a(x07), .O(new_n89_));
  inv1   g46(.a(x04), .O(new_n90_));
  inv1   g47(.a(x03), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n89_), .c(new_n88_), .d(new_n87_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(x08), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n45_), .c(x17), .d(x16), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n59_), .O(z08));
  inv1   g54(.a(x20), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nor2   g57(.a(new_n48_), .b(x00), .O(z16));
  nand3  g58(.a(z16), .b(new_n59_), .c(x18), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n100_), .c(new_n99_), .d(new_n98_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z09));
  nand3  g62(.a(new_n59_), .b(x18), .c(x01), .O(new_n106_));
  nor4   g63(.a(new_n106_), .b(new_n100_), .c(new_n99_), .d(x20), .O(new_n107_));
  nand2  g64(.a(x02), .b(new_n48_), .O(new_n108_));
  nor4   g65(.a(new_n108_), .b(x13), .c(new_n82_), .d(x11), .O(new_n109_));
  inv1   g66(.a(x15), .O(new_n110_));
  nand3  g67(.a(new_n100_), .b(new_n99_), .c(x20), .O(new_n111_));
  nor3   g68(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(new_n107_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x00), .c(new_n73_), .O(z10));
  nand3  g71(.a(x18), .b(x01), .c(new_n47_), .O(new_n115_));
  nand4  g72(.a(new_n100_), .b(x21), .c(new_n98_), .d(new_n59_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n115_), .c(new_n73_), .O(z11));
  aoi21  g74(.a(x10), .b(x02), .c(new_n48_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x00), .O(new_n119_));
  oai21  g76(.a(x19), .b(x02), .c(x23), .O(new_n120_));
  nand3  g77(.a(new_n59_), .b(x17), .c(new_n49_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(x16), .c(new_n48_), .d(new_n47_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n119_), .c(x24), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x09), .O(new_n125_));
  inv1   g82(.a(new_n125_), .O(z12));
  nand4  g83(.a(new_n59_), .b(x17), .c(new_n49_), .d(new_n47_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(x16), .c(x01), .O(z13));
  nor2   g85(.a(new_n51_), .b(x10), .O(new_n129_));
  nand4  g86(.a(new_n129_), .b(new_n45_), .c(new_n44_), .d(x16), .O(new_n130_));
  nor2   g87(.a(new_n130_), .b(x19), .O(z14));
  aoi21  g88(.a(x16), .b(new_n49_), .c(new_n118_), .O(new_n132_));
  nand4  g89(.a(x19), .b(x16), .c(new_n49_), .d(new_n48_), .O(new_n133_));
  oai21  g90(.a(new_n132_), .b(new_n47_), .c(new_n133_), .O(z15));
  nand4  g91(.a(x16), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n135_));
  inv1   g92(.a(new_n135_), .O(z17));
endmodule


