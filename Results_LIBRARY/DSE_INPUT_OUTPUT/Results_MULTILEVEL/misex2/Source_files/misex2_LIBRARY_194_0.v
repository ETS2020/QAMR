// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n147_;
  inv1   g000(.a(x17), .O(new_n44_));
  nor3   g001(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g002(.a(x19), .b(x18), .O(new_n46_));
  nand4  g003(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g004(.a(new_n47_), .b(x04), .c(x01), .O(z00));
  inv1   g005(.a(x18), .O(new_n49_));
  inv1   g006(.a(x09), .O(new_n50_));
  inv1   g007(.a(x00), .O(new_n51_));
  inv1   g008(.a(x01), .O(new_n52_));
  inv1   g009(.a(x02), .O(new_n53_));
  nand4  g010(.a(x04), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  nand4  g015(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x19), .O(z02));
  nand3  g017(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n61_));
  inv1   g018(.a(x19), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(x18), .c(new_n44_), .d(x04), .O(new_n63_));
  nand3  g020(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g021(.a(x12), .b(x11), .c(x10), .d(new_n50_), .O(new_n65_));
  oai22  g022(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  inv1   g023(.a(x11), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g025(.a(new_n68_), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n67_), .c(x10), .d(new_n50_), .O(new_n70_));
  nor2   g027(.a(new_n70_), .b(x12), .O(z04));
  nand2  g028(.a(x01), .b(x00), .O(new_n72_));
  inv1   g029(.a(x04), .O(new_n73_));
  nand2  g030(.a(new_n73_), .b(new_n52_), .O(new_n74_));
  nand3  g031(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  oai21  g032(.a(new_n75_), .b(new_n72_), .c(new_n74_), .O(z05));
  nand3  g033(.a(x11), .b(x10), .c(new_n50_), .O(new_n77_));
  oai21  g034(.a(new_n77_), .b(new_n64_), .c(new_n74_), .O(z06));
  inv1   g035(.a(x12), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x11), .c(x10), .d(new_n50_), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n74_), .O(z07));
  inv1   g040(.a(x08), .O(new_n84_));
  inv1   g041(.a(x05), .O(new_n85_));
  inv1   g042(.a(x06), .O(new_n86_));
  inv1   g043(.a(x03), .O(new_n87_));
  nand4  g044(.a(new_n87_), .b(x02), .c(new_n52_), .d(new_n51_), .O(new_n88_));
  inv1   g045(.a(new_n88_), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(new_n86_), .c(new_n85_), .d(x04), .O(new_n90_));
  nor2   g047(.a(new_n90_), .b(x07), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n49_), .c(x17), .d(new_n84_), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(new_n62_), .O(z08));
  inv1   g050(.a(x21), .O(new_n94_));
  inv1   g051(.a(x22), .O(new_n95_));
  nor2   g052(.a(new_n53_), .b(x01), .O(new_n96_));
  nor2   g053(.a(new_n79_), .b(x11), .O(new_n97_));
  nand2  g054(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  inv1   g055(.a(x15), .O(new_n99_));
  inv1   g056(.a(x16), .O(new_n100_));
  nor2   g057(.a(x14), .b(x13), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x20), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g059(.a(x20), .O(new_n103_));
  nor2   g060(.a(new_n49_), .b(new_n52_), .O(new_n104_));
  nand3  g061(.a(new_n104_), .b(new_n103_), .c(new_n62_), .O(new_n105_));
  oai21  g062(.a(new_n102_), .b(new_n98_), .c(new_n105_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n95_), .c(new_n94_), .d(new_n51_), .O(new_n107_));
  nand2  g064(.a(new_n107_), .b(new_n74_), .O(z09));
  nand3  g065(.a(new_n62_), .b(x18), .c(x01), .O(new_n109_));
  nand3  g066(.a(x22), .b(x21), .c(new_n103_), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  inv1   g068(.a(x13), .O(new_n112_));
  nand3  g069(.a(new_n112_), .b(x12), .c(new_n67_), .O(new_n113_));
  nor3   g070(.a(new_n113_), .b(new_n53_), .c(x01), .O(new_n114_));
  inv1   g071(.a(x14), .O(new_n115_));
  nand3  g072(.a(x16), .b(x15), .c(new_n115_), .O(new_n116_));
  nand3  g073(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n117_));
  nor2   g074(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  aoi21  g075(.a(new_n118_), .b(new_n114_), .c(new_n111_), .O(new_n119_));
  oai21  g076(.a(new_n119_), .b(x00), .c(new_n74_), .O(z10));
  nand4  g077(.a(new_n104_), .b(x21), .c(new_n103_), .d(new_n62_), .O(new_n121_));
  nand3  g078(.a(x15), .b(new_n115_), .c(new_n112_), .O(new_n122_));
  nand3  g079(.a(new_n94_), .b(x20), .c(new_n100_), .O(new_n123_));
  nor2   g080(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g081(.a(new_n124_), .b(new_n97_), .c(new_n96_), .d(x04), .O(new_n125_));
  nand2  g082(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n95_), .c(new_n51_), .O(new_n127_));
  inv1   g084(.a(new_n127_), .O(z11));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(x01), .c(x00), .O(new_n130_));
  oai21  g087(.a(x19), .b(x02), .c(x23), .O(new_n131_));
  nand3  g088(.a(new_n62_), .b(x17), .c(new_n53_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand4  g090(.a(new_n133_), .b(x04), .c(new_n52_), .d(new_n51_), .O(new_n134_));
  aoi21  g091(.a(new_n134_), .b(new_n130_), .c(x24), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(x09), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(z12));
  nand4  g094(.a(new_n62_), .b(x17), .c(new_n53_), .d(new_n51_), .O(new_n138_));
  aoi21  g095(.a(new_n138_), .b(x04), .c(x01), .O(z13));
  nor2   g096(.a(new_n54_), .b(x09), .O(new_n140_));
  nand4  g097(.a(new_n140_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z14));
  aoi21  g099(.a(new_n58_), .b(x01), .c(new_n53_), .O(new_n143_));
  aoi21  g100(.a(x19), .b(new_n53_), .c(new_n73_), .O(new_n144_));
  oai22  g101(.a(new_n144_), .b(x01), .c(new_n143_), .d(new_n51_), .O(z15));
  oai21  g102(.a(new_n52_), .b(x00), .c(new_n74_), .O(z16));
  nand2  g103(.a(x02), .b(new_n51_), .O(new_n147_));
  aoi21  g104(.a(new_n147_), .b(x04), .c(x01), .O(z17));
endmodule


