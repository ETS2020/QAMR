// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n137_, new_n139_, new_n140_, new_n141_;
  inv1   g000(.a(x13), .O(new_n44_));
  nor3   g001(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  inv1   g002(.a(x10), .O(new_n46_));
  nor2   g003(.a(new_n46_), .b(x09), .O(new_n47_));
  nor2   g004(.a(x19), .b(x18), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  aoi21  g006(.a(new_n49_), .b(new_n44_), .c(x17), .O(z00));
  inv1   g007(.a(x17), .O(new_n51_));
  inv1   g008(.a(x18), .O(new_n52_));
  inv1   g009(.a(x09), .O(new_n53_));
  inv1   g010(.a(x00), .O(new_n54_));
  inv1   g011(.a(x01), .O(new_n55_));
  inv1   g012(.a(x02), .O(new_n56_));
  nand3  g013(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  or2    g014(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(new_n46_), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n52_), .c(new_n51_), .d(new_n44_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x19), .O(z01));
  nor2   g018(.a(new_n58_), .b(x10), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n52_), .c(new_n51_), .d(new_n44_), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x19), .O(z02));
  nor2   g021(.a(x17), .b(new_n44_), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(x12), .c(x11), .d(x10), .O(new_n67_));
  nor2   g024(.a(new_n67_), .b(x09), .O(new_n68_));
  nand4  g025(.a(new_n68_), .b(x02), .c(x01), .d(x00), .O(new_n69_));
  nor2   g026(.a(x19), .b(new_n52_), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n45_), .c(new_n51_), .d(new_n44_), .O(new_n71_));
  nand2  g028(.a(new_n71_), .b(new_n69_), .O(z03));
  nand3  g029(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand3  g032(.a(new_n47_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n73_), .c(new_n66_), .O(z04));
  nor2   g034(.a(new_n65_), .b(new_n46_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x09), .c(x02), .d(x01), .O(new_n79_));
  nor2   g036(.a(new_n79_), .b(new_n54_), .O(z05));
  nand3  g037(.a(x11), .b(x10), .c(new_n53_), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(new_n73_), .c(new_n66_), .O(z06));
  nand3  g039(.a(new_n47_), .b(new_n75_), .c(x11), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(x01), .O(new_n84_));
  nand4  g041(.a(new_n84_), .b(new_n66_), .c(x02), .d(x00), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(z07));
  nor2   g043(.a(new_n56_), .b(x01), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n54_), .O(new_n88_));
  inv1   g045(.a(x05), .O(new_n89_));
  nand2  g046(.a(new_n89_), .b(x04), .O(new_n90_));
  nor3   g047(.a(new_n90_), .b(new_n88_), .c(x03), .O(new_n91_));
  nor3   g048(.a(x08), .b(x07), .c(x06), .O(new_n92_));
  inv1   g049(.a(x19), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(x18), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n92_), .c(new_n91_), .d(x17), .O(new_n95_));
  nand2  g052(.a(new_n95_), .b(new_n66_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nand3  g055(.a(new_n87_), .b(x12), .c(new_n74_), .O(new_n99_));
  inv1   g056(.a(x15), .O(new_n100_));
  inv1   g057(.a(x16), .O(new_n101_));
  nor2   g058(.a(x14), .b(x13), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x20), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  nor2   g061(.a(new_n52_), .b(new_n55_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n104_), .c(new_n93_), .O(new_n106_));
  oai21  g063(.a(new_n103_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n98_), .c(new_n97_), .d(new_n54_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n66_), .O(z09));
  nand4  g066(.a(new_n66_), .b(x22), .c(x21), .d(new_n104_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(new_n93_), .c(x18), .d(x01), .O(new_n112_));
  nand4  g069(.a(new_n87_), .b(new_n44_), .c(x12), .d(new_n74_), .O(new_n113_));
  nor3   g070(.a(new_n101_), .b(new_n100_), .c(x14), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n98_), .c(new_n97_), .d(x20), .O(new_n115_));
  or2    g072(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n112_), .c(x00), .O(z10));
  nand4  g074(.a(new_n105_), .b(x21), .c(new_n104_), .d(new_n93_), .O(new_n118_));
  nor2   g075(.a(new_n100_), .b(x14), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(new_n97_), .c(x20), .d(new_n101_), .O(new_n120_));
  oai21  g077(.a(new_n120_), .b(new_n113_), .c(new_n118_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n98_), .c(new_n54_), .O(new_n122_));
  nand2  g079(.a(new_n122_), .b(new_n66_), .O(z11));
  inv1   g080(.a(x24), .O(new_n124_));
  nand2  g081(.a(x10), .b(x02), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n66_), .c(x01), .d(x00), .O(new_n126_));
  nor2   g083(.a(x19), .b(x02), .O(new_n127_));
  oai21  g084(.a(new_n127_), .b(x13), .c(new_n51_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(x23), .O(new_n129_));
  nand3  g086(.a(new_n93_), .b(x17), .c(new_n56_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n55_), .c(new_n54_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n126_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n124_), .c(x09), .O(new_n134_));
  inv1   g091(.a(new_n134_), .O(z12));
  nor3   g092(.a(new_n57_), .b(x19), .c(new_n51_), .O(z13));
  nand4  g093(.a(new_n48_), .b(new_n45_), .c(new_n46_), .d(new_n53_), .O(new_n137_));
  aoi21  g094(.a(new_n137_), .b(new_n44_), .c(x17), .O(z14));
  oai21  g095(.a(x10), .b(new_n55_), .c(x02), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(x00), .O(new_n140_));
  nand3  g097(.a(x19), .b(new_n56_), .c(new_n55_), .O(new_n141_));
  aoi21  g098(.a(new_n141_), .b(new_n140_), .c(new_n65_), .O(z15));
  nor3   g099(.a(new_n65_), .b(new_n55_), .c(x00), .O(z16));
  nand2  g100(.a(new_n88_), .b(new_n66_), .O(z17));
endmodule


