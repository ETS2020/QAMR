// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n130_, new_n132_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  nand4  g06(.a(new_n47_), .b(x10), .c(x09), .d(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  nand4  g09(.a(new_n47_), .b(new_n52_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g11(.a(x01), .O(new_n55_));
  nand3  g12(.a(new_n45_), .b(new_n55_), .c(new_n44_), .O(new_n56_));
  inv1   g13(.a(x17), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(x18), .c(new_n57_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand4  g17(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n61_));
  oai22  g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n56_), .O(z03));
  nor2   g19(.a(new_n55_), .b(new_n44_), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x10), .c(new_n46_), .d(x02), .O(new_n64_));
  nor3   g21(.a(new_n64_), .b(x12), .c(x11), .O(z04));
  nand3  g22(.a(x10), .b(x09), .c(x02), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(x01), .c(new_n44_), .O(z05));
  inv1   g24(.a(x11), .O(new_n68_));
  nor4   g25(.a(new_n60_), .b(new_n68_), .c(new_n52_), .d(x09), .O(z06));
  nor3   g26(.a(new_n64_), .b(x12), .c(new_n68_), .O(z07));
  nor2   g27(.a(x03), .b(new_n45_), .O(new_n71_));
  nor2   g28(.a(x06), .b(x05), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(x04), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(new_n74_));
  nor2   g31(.a(x08), .b(x07), .O(new_n75_));
  inv1   g32(.a(x18), .O(new_n76_));
  nand3  g33(.a(x19), .b(new_n76_), .c(x17), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(new_n75_), .c(new_n74_), .d(new_n71_), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g37(.a(x21), .O(new_n81_));
  inv1   g38(.a(x22), .O(new_n82_));
  nand4  g39(.a(x12), .b(new_n68_), .c(x02), .d(new_n55_), .O(new_n83_));
  inv1   g40(.a(x15), .O(new_n84_));
  inv1   g41(.a(x16), .O(new_n85_));
  nor2   g42(.a(x14), .b(x13), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x20), .c(new_n85_), .d(new_n84_), .O(new_n87_));
  inv1   g44(.a(x20), .O(new_n88_));
  and2   g45(.a(x18), .b(x01), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(new_n88_), .c(new_n58_), .O(new_n90_));
  oai21  g47(.a(new_n87_), .b(new_n83_), .c(new_n90_), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n82_), .c(new_n81_), .d(new_n44_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(z09));
  nor3   g50(.a(new_n82_), .b(new_n81_), .c(x20), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n58_), .c(x18), .d(x01), .O(new_n95_));
  nor2   g52(.a(new_n45_), .b(x01), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(x12), .c(new_n68_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nor3   g56(.a(new_n85_), .b(new_n84_), .c(x14), .O(new_n100_));
  nand3  g57(.a(new_n82_), .b(new_n81_), .c(x20), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n100_), .c(new_n99_), .d(new_n96_), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n95_), .c(x00), .O(z10));
  inv1   g61(.a(x12), .O(new_n105_));
  nor2   g62(.a(x13), .b(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n96_), .c(new_n68_), .O(new_n107_));
  nand4  g64(.a(new_n89_), .b(x21), .c(new_n88_), .d(new_n58_), .O(new_n108_));
  nor2   g65(.a(new_n84_), .b(x14), .O(new_n109_));
  nor2   g66(.a(x21), .b(new_n88_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n109_), .c(new_n85_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(new_n107_), .c(new_n108_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n82_), .c(new_n44_), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z11));
  inv1   g71(.a(x24), .O(new_n115_));
  aoi21  g72(.a(x10), .b(x02), .c(new_n44_), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n58_), .b(x17), .c(new_n45_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n117_), .c(x01), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n115_), .O(new_n120_));
  nand2  g77(.a(new_n55_), .b(x00), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n46_), .c(new_n121_), .O(z12));
  aoi21  g79(.a(new_n118_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g80(.a(new_n47_), .b(new_n52_), .c(new_n46_), .d(new_n45_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n44_), .c(x01), .O(z14));
  nand2  g82(.a(x10), .b(x02), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  nand4  g84(.a(x19), .b(new_n45_), .c(new_n55_), .d(new_n44_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n127_), .O(z15));
  nand2  g86(.a(x01), .b(new_n44_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n121_), .O(z16));
  nand3  g88(.a(x02), .b(new_n55_), .c(new_n44_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z17));
endmodule


