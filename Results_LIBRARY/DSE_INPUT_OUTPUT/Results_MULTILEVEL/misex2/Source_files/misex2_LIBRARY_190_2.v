// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x18), .b(x17), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x19), .O(z00));
  nand3  g09(.a(x09), .b(new_n46_), .c(new_n45_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x19), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n54_), .b(new_n48_), .c(new_n57_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n44_), .c(x19), .O(z02));
  nand3  g16(.a(new_n46_), .b(new_n44_), .c(new_n45_), .O(new_n60_));
  inv1   g17(.a(x17), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x18), .c(new_n61_), .O(new_n63_));
  nor2   g20(.a(x09), .b(new_n46_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x19), .b(x12), .c(x11), .d(x10), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n65_), .c(new_n63_), .d(new_n60_), .O(z03));
  nor2   g24(.a(x12), .b(x11), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n64_), .c(x10), .d(x00), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x19), .c(new_n44_), .O(z04));
  nor2   g27(.a(new_n46_), .b(new_n45_), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(x10), .c(x09), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x19), .c(new_n44_), .O(z05));
  nand4  g30(.a(new_n71_), .b(x11), .c(x10), .d(new_n47_), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x19), .c(new_n44_), .O(z06));
  inv1   g32(.a(x12), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x11), .c(x10), .d(new_n47_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nor2   g36(.a(x19), .b(new_n44_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n79_), .O(z07));
  inv1   g39(.a(x04), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n44_), .c(new_n45_), .O(new_n84_));
  nor4   g41(.a(new_n84_), .b(x05), .c(new_n83_), .d(x03), .O(new_n85_));
  nor3   g42(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nor2   g43(.a(new_n62_), .b(x18), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(x17), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n81_), .O(z08));
  inv1   g46(.a(x16), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x11), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(x02), .c(new_n44_), .d(new_n45_), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n93_), .c(new_n92_), .d(x12), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(x15), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n91_), .c(x20), .d(new_n90_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(x22), .O(z09));
  inv1   g57(.a(x15), .O(new_n101_));
  nor2   g58(.a(new_n97_), .b(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n91_), .c(x20), .d(x16), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(x22), .O(z10));
  nor4   g61(.a(new_n84_), .b(x13), .c(new_n76_), .d(x11), .O(new_n105_));
  nand3  g62(.a(new_n90_), .b(x15), .c(new_n93_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nor2   g64(.a(x22), .b(x21), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n107_), .c(new_n105_), .d(x20), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n81_), .O(z11));
  nand2  g67(.a(x10), .b(x02), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(x01), .c(x00), .O(new_n112_));
  oai21  g69(.a(x19), .b(x02), .c(x23), .O(new_n113_));
  nand3  g70(.a(new_n62_), .b(x17), .c(new_n46_), .O(new_n114_));
  oai21  g71(.a(new_n113_), .b(x01), .c(new_n114_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n45_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n112_), .c(x24), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x09), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n81_), .O(z12));
  nand3  g76(.a(x17), .b(new_n46_), .c(new_n45_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n44_), .c(x19), .O(z13));
  nor3   g78(.a(x02), .b(x01), .c(x00), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(new_n61_), .c(new_n57_), .d(new_n47_), .O(new_n123_));
  nor3   g80(.a(new_n123_), .b(x19), .c(x18), .O(z14));
  nor2   g81(.a(new_n80_), .b(new_n45_), .O(new_n125_));
  nor2   g82(.a(new_n62_), .b(x01), .O(new_n126_));
  oai21  g83(.a(new_n126_), .b(new_n125_), .c(new_n46_), .O(new_n127_));
  nand4  g84(.a(x19), .b(new_n57_), .c(x01), .d(x00), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n127_), .O(z15));
  aoi21  g86(.a(x19), .b(x00), .c(new_n44_), .O(z16));
  nand2  g87(.a(new_n84_), .b(new_n81_), .O(z17));
endmodule


