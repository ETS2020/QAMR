// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n128_, new_n130_,
    new_n131_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(x10), .c(x09), .d(new_n48_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n46_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n57_), .c(x09), .d(new_n48_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n46_), .c(x01), .O(z02));
  nand3  g16(.a(x12), .b(x11), .c(x10), .O(new_n60_));
  nor3   g17(.a(new_n60_), .b(x09), .c(new_n48_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n47_), .c(x00), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nor2   g20(.a(x02), .b(x01), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n63_), .c(x18), .d(new_n44_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n62_), .O(z03));
  nor2   g23(.a(new_n47_), .b(new_n46_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(x10), .c(new_n49_), .d(x02), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(x12), .c(x11), .O(z04));
  nand3  g26(.a(x10), .b(x09), .c(x02), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x01), .c(new_n46_), .O(z05));
  nand4  g28(.a(x11), .b(x10), .c(new_n49_), .d(x02), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n46_), .O(z06));
  inv1   g30(.a(x11), .O(new_n74_));
  nor3   g31(.a(new_n68_), .b(x12), .c(new_n74_), .O(z07));
  nor2   g32(.a(x03), .b(new_n48_), .O(new_n76_));
  nor2   g33(.a(x06), .b(x05), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x04), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand3  g37(.a(x19), .b(new_n45_), .c(x17), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n80_), .c(new_n79_), .d(new_n76_), .O(new_n83_));
  aoi21  g40(.a(new_n83_), .b(new_n46_), .c(x01), .O(z08));
  inv1   g41(.a(x21), .O(new_n85_));
  inv1   g42(.a(x22), .O(new_n86_));
  nand4  g43(.a(x12), .b(new_n74_), .c(x02), .d(new_n47_), .O(new_n87_));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(x16), .O(new_n89_));
  nor2   g46(.a(x14), .b(x13), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  inv1   g48(.a(x20), .O(new_n92_));
  and2   g49(.a(x18), .b(x01), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n63_), .O(new_n94_));
  oai21  g51(.a(new_n91_), .b(new_n87_), .c(new_n94_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n86_), .c(new_n85_), .d(new_n46_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(z09));
  nor3   g54(.a(new_n86_), .b(new_n85_), .c(x20), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n63_), .c(x18), .d(x01), .O(new_n99_));
  nor2   g56(.a(new_n48_), .b(x01), .O(new_n100_));
  inv1   g57(.a(x13), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x12), .c(new_n74_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nor3   g60(.a(new_n89_), .b(new_n88_), .c(x14), .O(new_n104_));
  nand3  g61(.a(new_n86_), .b(new_n85_), .c(x20), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n104_), .c(new_n103_), .d(new_n100_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n99_), .c(x00), .O(z10));
  inv1   g65(.a(x12), .O(new_n109_));
  nor2   g66(.a(x13), .b(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n100_), .c(new_n74_), .O(new_n111_));
  nand4  g68(.a(new_n93_), .b(x21), .c(new_n92_), .d(new_n63_), .O(new_n112_));
  nor2   g69(.a(new_n88_), .b(x14), .O(new_n113_));
  nor2   g70(.a(x21), .b(new_n92_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n113_), .c(new_n89_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n111_), .c(new_n112_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n86_), .c(new_n46_), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(z11));
  inv1   g75(.a(x24), .O(new_n119_));
  aoi21  g76(.a(x10), .b(x02), .c(new_n46_), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n63_), .b(x17), .c(new_n48_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n121_), .c(x01), .O(new_n123_));
  oai21  g80(.a(new_n123_), .b(new_n120_), .c(new_n119_), .O(new_n124_));
  nand2  g81(.a(new_n47_), .b(x00), .O(new_n125_));
  oai21  g82(.a(new_n124_), .b(new_n49_), .c(new_n125_), .O(z12));
  aoi21  g83(.a(new_n122_), .b(new_n46_), .c(x01), .O(z13));
  nand4  g84(.a(new_n54_), .b(new_n57_), .c(new_n49_), .d(new_n48_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n46_), .c(x01), .O(z14));
  nand2  g86(.a(x19), .b(new_n48_), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(new_n46_), .c(x01), .O(new_n131_));
  or2    g88(.a(new_n131_), .b(new_n120_), .O(z15));
  nor2   g89(.a(new_n47_), .b(x00), .O(z16));
  aoi21  g90(.a(new_n48_), .b(new_n46_), .c(x01), .O(z17));
endmodule


