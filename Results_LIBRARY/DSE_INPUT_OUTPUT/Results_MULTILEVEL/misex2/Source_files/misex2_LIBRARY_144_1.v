// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n142_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x22), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  nor3   g002(.a(x09), .b(x02), .c(x00), .O(new_n46_));
  nor2   g003(.a(x19), .b(x18), .O(new_n47_));
  nand4  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g005(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x19), .O(new_n51_));
  inv1   g008(.a(x10), .O(new_n52_));
  inv1   g009(.a(x00), .O(new_n53_));
  inv1   g010(.a(x01), .O(new_n54_));
  inv1   g011(.a(x02), .O(new_n55_));
  nand4  g012(.a(x09), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(new_n45_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x22), .O(z01));
  nand3  g016(.a(new_n47_), .b(new_n45_), .c(new_n52_), .O(new_n60_));
  inv1   g017(.a(new_n60_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(x09), .c(new_n55_), .d(new_n53_), .O(new_n62_));
  aoi21  g019(.a(new_n62_), .b(new_n44_), .c(x01), .O(z02));
  nand2  g020(.a(new_n55_), .b(new_n53_), .O(new_n64_));
  nand3  g021(.a(new_n51_), .b(x18), .c(new_n45_), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n64_), .c(new_n44_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n54_), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g025(.a(new_n52_), .b(x09), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g027(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  nand2  g028(.a(x22), .b(new_n54_), .O(new_n72_));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  nand3  g031(.a(new_n69_), .b(new_n74_), .c(new_n73_), .O(new_n75_));
  oai21  g032(.a(new_n75_), .b(new_n68_), .c(new_n72_), .O(z04));
  nand2  g033(.a(x01), .b(x00), .O(new_n77_));
  nand3  g034(.a(x10), .b(x09), .c(x02), .O(new_n78_));
  oai21  g035(.a(new_n78_), .b(new_n77_), .c(new_n72_), .O(z05));
  inv1   g036(.a(x09), .O(new_n80_));
  nand3  g037(.a(x11), .b(x10), .c(new_n80_), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(new_n68_), .c(new_n72_), .O(z06));
  nand3  g039(.a(new_n69_), .b(new_n74_), .c(x11), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n72_), .O(z07));
  nand2  g043(.a(x02), .b(new_n53_), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  nand2  g045(.a(new_n88_), .b(x04), .O(new_n89_));
  nor3   g046(.a(new_n89_), .b(new_n87_), .c(x03), .O(new_n90_));
  nor3   g047(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nand3  g048(.a(x19), .b(new_n50_), .c(x17), .O(new_n92_));
  inv1   g049(.a(new_n92_), .O(new_n93_));
  nand3  g050(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(new_n94_));
  aoi21  g051(.a(new_n94_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n73_), .c(x02), .d(new_n54_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g058(.a(x20), .O(new_n102_));
  and2   g059(.a(x18), .b(x01), .O(new_n103_));
  nand3  g060(.a(new_n103_), .b(new_n102_), .c(new_n51_), .O(new_n104_));
  oai21  g061(.a(new_n101_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n44_), .c(new_n96_), .d(new_n53_), .O(new_n106_));
  inv1   g063(.a(new_n106_), .O(z09));
  nor2   g064(.a(x19), .b(new_n50_), .O(new_n108_));
  nand2  g065(.a(x02), .b(new_n54_), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n73_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  inv1   g069(.a(x14), .O(new_n113_));
  nand2  g070(.a(x15), .b(new_n113_), .O(new_n114_));
  nand3  g071(.a(new_n96_), .b(x20), .c(x16), .O(new_n115_));
  nor2   g072(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nor3   g073(.a(new_n44_), .b(new_n96_), .c(x20), .O(new_n117_));
  aoi22  g074(.a(new_n117_), .b(new_n108_), .c(new_n116_), .d(new_n112_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(x00), .c(new_n72_), .O(z10));
  nor2   g076(.a(new_n55_), .b(x01), .O(new_n120_));
  nor2   g077(.a(x13), .b(new_n74_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n120_), .c(new_n73_), .O(new_n122_));
  nand4  g079(.a(new_n103_), .b(x21), .c(new_n102_), .d(new_n51_), .O(new_n123_));
  nor2   g080(.a(new_n98_), .b(x14), .O(new_n124_));
  nor2   g081(.a(x21), .b(new_n102_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n124_), .c(new_n99_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n122_), .c(new_n123_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n44_), .c(new_n53_), .O(new_n128_));
  inv1   g085(.a(new_n128_), .O(z11));
  inv1   g086(.a(x24), .O(new_n130_));
  nand2  g087(.a(x10), .b(x02), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(x01), .c(x00), .O(new_n132_));
  oai21  g089(.a(x19), .b(x02), .c(x23), .O(new_n133_));
  nand3  g090(.a(new_n51_), .b(x17), .c(new_n55_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n54_), .c(new_n53_), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand3  g094(.a(new_n137_), .b(new_n130_), .c(x09), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n72_), .O(z12));
  nand4  g096(.a(new_n51_), .b(x17), .c(new_n55_), .d(new_n53_), .O(new_n140_));
  aoi21  g097(.a(new_n140_), .b(new_n44_), .c(x01), .O(z13));
  nand2  g098(.a(new_n61_), .b(new_n46_), .O(new_n142_));
  aoi21  g099(.a(new_n142_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g100(.a(new_n52_), .b(x01), .c(new_n55_), .O(new_n144_));
  aoi21  g101(.a(x19), .b(new_n55_), .c(x22), .O(new_n145_));
  oai22  g102(.a(new_n145_), .b(x01), .c(new_n144_), .d(new_n53_), .O(z15));
  nor2   g103(.a(new_n54_), .b(x00), .O(z16));
  nand4  g104(.a(new_n44_), .b(x02), .c(new_n54_), .d(new_n53_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


