// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nand2  g10(.a(x01), .b(x00), .O(new_n54_));
  nand4  g11(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n55_));
  inv1   g12(.a(x19), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n55_), .c(new_n54_), .O(z02));
  nand3  g18(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n62_));
  nor4   g19(.a(new_n62_), .b(x19), .c(new_n45_), .d(x17), .O(z03));
  inv1   g20(.a(new_n54_), .O(z05));
  aoi21  g21(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(z07));
  nand3  g22(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n68_));
  inv1   g23(.a(x03), .O(new_n69_));
  inv1   g24(.a(x05), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(x04), .c(new_n69_), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  nor3   g27(.a(x08), .b(x07), .c(x06), .O(new_n73_));
  nor2   g28(.a(new_n56_), .b(x18), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(x17), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n54_), .O(z08));
  nand2  g31(.a(new_n56_), .b(x18), .O(new_n77_));
  inv1   g32(.a(x20), .O(new_n78_));
  inv1   g33(.a(x21), .O(new_n79_));
  inv1   g34(.a(x22), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n77_), .c(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x01), .O(new_n83_));
  inv1   g38(.a(x11), .O(new_n84_));
  inv1   g39(.a(x13), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(x12), .c(new_n84_), .O(new_n86_));
  nor2   g41(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  nor3   g42(.a(x16), .b(x15), .c(x14), .O(new_n88_));
  nand3  g43(.a(new_n80_), .b(new_n79_), .c(x20), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n83_), .O(z09));
  nand3  g47(.a(x22), .b(x21), .c(new_n78_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n77_), .c(new_n46_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(x01), .O(new_n95_));
  nand2  g50(.a(x16), .b(x15), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(x14), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n90_), .c(new_n87_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n95_), .O(z10));
  nand3  g54(.a(new_n80_), .b(x21), .c(new_n78_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n77_), .c(new_n46_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x01), .O(new_n102_));
  inv1   g57(.a(x14), .O(new_n103_));
  inv1   g58(.a(x15), .O(new_n104_));
  nor2   g59(.a(x16), .b(new_n104_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n90_), .c(new_n87_), .d(new_n103_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n102_), .O(z11));
  oai21  g62(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g63(.a(new_n56_), .b(x17), .c(new_n48_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n108_), .c(x24), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(x09), .c(new_n47_), .d(new_n46_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n54_), .O(z12));
  nor3   g67(.a(new_n62_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g68(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(x19), .O(z14));
  oai21  g70(.a(new_n48_), .b(x01), .c(x00), .O(new_n116_));
  nand3  g71(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(z15));
  nand2  g73(.a(new_n68_), .b(new_n54_), .O(z17));
  zero   g74(.O(z04));
  zero   g75(.O(z06));
  buf    g76(.a(x01), .O(z16));
endmodule


