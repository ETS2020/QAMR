// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n100_, new_n101_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n123_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  inv1   g03(.a(x00), .O(new_n47_));
  inv1   g04(.a(x01), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n45_), .c(new_n44_), .d(x13), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x19), .O(z00));
  nand3  g11(.a(x09), .b(new_n49_), .c(new_n47_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nor2   g13(.a(x19), .b(x18), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n56_), .c(new_n44_), .d(x10), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(x13), .c(x01), .O(z01));
  nand4  g16(.a(new_n57_), .b(new_n56_), .c(new_n44_), .d(new_n46_), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(x13), .c(x01), .O(z02));
  nand3  g18(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x18), .c(new_n44_), .d(x13), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n50_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(z03));
  inv1   g24(.a(x13), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n48_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n50_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n65_), .c(new_n69_), .O(z04));
  nand4  g30(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n46_), .O(z05));
  nor4   g32(.a(new_n65_), .b(new_n70_), .c(new_n46_), .d(x09), .O(z06));
  nand2  g33(.a(new_n50_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n71_), .b(x11), .c(x10), .O(new_n78_));
  oai22  g35(.a(new_n78_), .b(new_n77_), .c(new_n68_), .d(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(z07));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x07), .O(new_n84_));
  inv1   g41(.a(x04), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x08), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n45_), .c(x17), .d(x13), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n63_), .O(z08));
  nand3  g49(.a(x18), .b(x01), .c(new_n47_), .O(new_n93_));
  inv1   g50(.a(x20), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(new_n63_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n93_), .c(new_n69_), .O(z09));
  nor2   g55(.a(new_n48_), .b(x00), .O(z16));
  nand3  g56(.a(z16), .b(new_n63_), .c(x18), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(x22), .c(x21), .d(new_n94_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z10));
  nand4  g60(.a(new_n96_), .b(x21), .c(new_n94_), .d(new_n63_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n93_), .c(new_n69_), .O(z11));
  aoi21  g62(.a(x10), .b(x02), .c(new_n48_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(x00), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n63_), .b(x17), .c(new_n49_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(x13), .c(new_n48_), .d(new_n47_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n107_), .c(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z12));
  nand4  g71(.a(new_n63_), .b(x17), .c(new_n49_), .d(new_n47_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(x13), .c(x01), .O(z13));
  nor2   g73(.a(new_n51_), .b(x10), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n45_), .c(new_n44_), .d(x13), .O(new_n118_));
  nor2   g75(.a(new_n118_), .b(x19), .O(z14));
  aoi21  g76(.a(x13), .b(new_n49_), .c(new_n106_), .O(new_n120_));
  nand4  g77(.a(x19), .b(x13), .c(new_n49_), .d(new_n48_), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n47_), .c(new_n121_), .O(z15));
  nand4  g79(.a(x13), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(z17));
endmodule


