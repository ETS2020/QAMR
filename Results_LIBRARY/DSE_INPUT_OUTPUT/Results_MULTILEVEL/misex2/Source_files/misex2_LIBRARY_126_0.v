// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_;
  inv1   g000(.a(x10), .O(new_n44_));
  inv1   g001(.a(x16), .O(new_n45_));
  nand2  g002(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g003(.a(x00), .O(new_n47_));
  inv1   g004(.a(x01), .O(new_n48_));
  inv1   g005(.a(x02), .O(new_n49_));
  inv1   g006(.a(x09), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g008(.a(x17), .O(new_n52_));
  nor2   g009(.a(x19), .b(x18), .O(new_n53_));
  nand3  g010(.a(new_n53_), .b(new_n52_), .c(x10), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n51_), .c(new_n46_), .O(z00));
  inv1   g012(.a(x18), .O(new_n56_));
  nand4  g013(.a(x09), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n57_));
  inv1   g014(.a(new_n57_), .O(new_n58_));
  nand4  g015(.a(new_n58_), .b(new_n56_), .c(new_n52_), .d(x10), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x19), .O(z01));
  nor2   g017(.a(new_n57_), .b(x10), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n56_), .c(new_n52_), .d(x16), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x19), .O(z02));
  nor2   g020(.a(x16), .b(x10), .O(new_n64_));
  nor2   g021(.a(new_n64_), .b(x19), .O(new_n65_));
  nand3  g022(.a(new_n65_), .b(x18), .c(new_n52_), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n68_));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  inv1   g026(.a(new_n69_), .O(new_n70_));
  nor2   g027(.a(new_n44_), .b(x09), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n70_), .c(x12), .d(x11), .O(new_n72_));
  nand2  g029(.a(new_n72_), .b(new_n68_), .O(z03));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand3  g032(.a(new_n71_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n69_), .c(new_n46_), .O(z04));
  nand2  g034(.a(x01), .b(x00), .O(new_n78_));
  nand3  g035(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(new_n78_), .c(new_n46_), .O(z05));
  nand3  g037(.a(x11), .b(x10), .c(new_n50_), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(new_n69_), .c(new_n46_), .O(z06));
  nand3  g039(.a(new_n71_), .b(new_n75_), .c(x11), .O(new_n83_));
  oai21  g040(.a(new_n64_), .b(x01), .c(new_n83_), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(z07));
  nor2   g043(.a(new_n49_), .b(x01), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n47_), .O(new_n88_));
  inv1   g045(.a(x03), .O(new_n89_));
  inv1   g046(.a(x05), .O(new_n90_));
  nand3  g047(.a(new_n90_), .b(x04), .c(new_n89_), .O(new_n91_));
  nor2   g048(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nor3   g049(.a(x08), .b(x07), .c(x06), .O(new_n93_));
  inv1   g050(.a(x19), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(x18), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n93_), .c(new_n92_), .d(x17), .O(new_n96_));
  nand2  g053(.a(new_n96_), .b(new_n46_), .O(z08));
  inv1   g054(.a(x21), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  nor2   g056(.a(new_n64_), .b(x20), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(new_n94_), .c(x18), .d(x01), .O(new_n101_));
  nand3  g058(.a(x12), .b(new_n74_), .c(x10), .O(new_n102_));
  inv1   g059(.a(new_n102_), .O(new_n103_));
  nor2   g060(.a(x14), .b(x13), .O(new_n104_));
  inv1   g061(.a(x15), .O(new_n105_));
  nand3  g062(.a(x20), .b(new_n45_), .c(new_n105_), .O(new_n106_));
  inv1   g063(.a(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n104_), .c(new_n103_), .d(new_n87_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n101_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n99_), .c(new_n98_), .d(new_n47_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(z09));
  nand3  g068(.a(new_n46_), .b(x22), .c(x21), .O(new_n112_));
  nor2   g069(.a(new_n112_), .b(x20), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n94_), .c(x18), .d(x01), .O(new_n114_));
  inv1   g071(.a(x13), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(x12), .c(new_n74_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(new_n117_));
  nor3   g074(.a(new_n45_), .b(new_n105_), .c(x14), .O(new_n118_));
  nand3  g075(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n119_));
  inv1   g076(.a(new_n119_), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n118_), .c(new_n117_), .d(new_n87_), .O(new_n121_));
  aoi21  g078(.a(new_n121_), .b(new_n114_), .c(x00), .O(z10));
  nand2  g079(.a(new_n117_), .b(new_n87_), .O(new_n123_));
  nor2   g080(.a(new_n98_), .b(x20), .O(new_n124_));
  nand4  g081(.a(new_n124_), .b(new_n94_), .c(x18), .d(x01), .O(new_n125_));
  nor2   g082(.a(new_n105_), .b(x14), .O(new_n126_));
  nand4  g083(.a(new_n126_), .b(new_n98_), .c(x20), .d(new_n45_), .O(new_n127_));
  oai21  g084(.a(new_n127_), .b(new_n123_), .c(new_n125_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n99_), .c(new_n47_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n46_), .O(z11));
  inv1   g087(.a(x24), .O(new_n131_));
  nand2  g088(.a(x10), .b(x02), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(x01), .c(x00), .O(new_n133_));
  oai21  g090(.a(x19), .b(x02), .c(x23), .O(new_n134_));
  nand3  g091(.a(new_n94_), .b(x17), .c(new_n49_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n48_), .c(new_n47_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n131_), .c(x09), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(new_n46_), .O(z12));
  nand4  g097(.a(new_n65_), .b(x17), .c(new_n49_), .d(new_n48_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x00), .O(z13));
  nor3   g099(.a(x02), .b(x01), .c(x00), .O(new_n143_));
  nand4  g100(.a(new_n143_), .b(new_n53_), .c(new_n52_), .d(new_n50_), .O(new_n144_));
  aoi21  g101(.a(new_n144_), .b(x16), .c(x10), .O(z14));
  oai21  g102(.a(x10), .b(new_n48_), .c(x02), .O(new_n146_));
  nand2  g103(.a(new_n146_), .b(x00), .O(new_n147_));
  nand3  g104(.a(x19), .b(new_n49_), .c(new_n48_), .O(new_n148_));
  nand3  g105(.a(new_n148_), .b(new_n147_), .c(new_n46_), .O(z15));
  oai21  g106(.a(new_n48_), .b(x00), .c(new_n46_), .O(z16));
  nand2  g107(.a(new_n88_), .b(new_n46_), .O(z17));
endmodule


