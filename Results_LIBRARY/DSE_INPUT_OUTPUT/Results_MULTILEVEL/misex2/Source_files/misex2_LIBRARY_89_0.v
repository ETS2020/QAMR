// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n140_, new_n141_, new_n144_;
  nor3   g000(.a(x09), .b(x02), .c(x00), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  nor2   g002(.a(x19), .b(x18), .O(new_n46_));
  nand3  g003(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g004(.a(new_n47_), .O(new_n48_));
  nand2  g005(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  aoi21  g006(.a(new_n49_), .b(x12), .c(x01), .O(z00));
  inv1   g007(.a(x00), .O(new_n51_));
  inv1   g008(.a(x02), .O(new_n52_));
  nand3  g009(.a(x09), .b(new_n52_), .c(new_n51_), .O(new_n53_));
  inv1   g010(.a(new_n53_), .O(new_n54_));
  nand2  g011(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  aoi21  g012(.a(new_n55_), .b(x12), .c(x01), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand3  g014(.a(new_n46_), .b(new_n45_), .c(new_n57_), .O(new_n58_));
  inv1   g015(.a(new_n58_), .O(new_n59_));
  nand2  g016(.a(new_n59_), .b(new_n54_), .O(new_n60_));
  aoi21  g017(.a(new_n60_), .b(x12), .c(x01), .O(z02));
  inv1   g018(.a(x12), .O(new_n62_));
  inv1   g019(.a(x09), .O(new_n63_));
  nand3  g020(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g021(.a(new_n64_), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(x11), .c(x10), .d(new_n63_), .O(new_n66_));
  nor3   g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g024(.a(x18), .O(new_n68_));
  nor2   g025(.a(x19), .b(new_n68_), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(new_n67_), .c(new_n45_), .O(new_n70_));
  aoi21  g027(.a(new_n70_), .b(new_n66_), .c(new_n62_), .O(z03));
  inv1   g028(.a(x11), .O(new_n72_));
  nand3  g029(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  inv1   g030(.a(new_n73_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(new_n72_), .c(x10), .d(new_n63_), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(x12), .O(z04));
  nor3   g033(.a(new_n73_), .b(new_n57_), .c(new_n63_), .O(z05));
  inv1   g034(.a(new_n66_), .O(z06));
  nand2  g035(.a(new_n63_), .b(x01), .O(new_n79_));
  nand3  g036(.a(new_n62_), .b(x11), .c(x10), .O(new_n80_));
  oai22  g037(.a(new_n80_), .b(new_n79_), .c(new_n62_), .d(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  inv1   g039(.a(new_n82_), .O(z07));
  nand2  g040(.a(x02), .b(new_n51_), .O(new_n84_));
  inv1   g041(.a(x05), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x04), .O(new_n86_));
  nor3   g043(.a(new_n86_), .b(new_n84_), .c(x03), .O(new_n87_));
  nor3   g044(.a(x08), .b(x07), .c(x06), .O(new_n88_));
  inv1   g045(.a(x19), .O(new_n89_));
  nor2   g046(.a(new_n89_), .b(x18), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(new_n88_), .c(new_n87_), .d(x17), .O(new_n91_));
  aoi21  g048(.a(new_n91_), .b(x12), .c(x01), .O(z08));
  inv1   g049(.a(x21), .O(new_n93_));
  inv1   g050(.a(x22), .O(new_n94_));
  nor2   g051(.a(new_n52_), .b(x01), .O(new_n95_));
  nand3  g052(.a(new_n95_), .b(x12), .c(new_n72_), .O(new_n96_));
  inv1   g053(.a(x15), .O(new_n97_));
  inv1   g054(.a(x16), .O(new_n98_));
  nor2   g055(.a(x14), .b(x13), .O(new_n99_));
  nand4  g056(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g057(.a(x20), .O(new_n101_));
  inv1   g058(.a(x01), .O(new_n102_));
  nor2   g059(.a(new_n68_), .b(new_n102_), .O(new_n103_));
  nand3  g060(.a(new_n103_), .b(new_n101_), .c(new_n89_), .O(new_n104_));
  oai21  g061(.a(new_n100_), .b(new_n96_), .c(new_n104_), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n94_), .c(new_n93_), .d(new_n51_), .O(new_n106_));
  inv1   g063(.a(new_n106_), .O(z09));
  nor2   g064(.a(new_n94_), .b(new_n93_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n69_), .c(new_n101_), .d(x01), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n72_), .O(new_n111_));
  inv1   g068(.a(new_n111_), .O(new_n112_));
  nor3   g069(.a(new_n98_), .b(new_n97_), .c(x14), .O(new_n113_));
  nand3  g070(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(new_n113_), .c(new_n112_), .d(new_n95_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n109_), .c(x00), .O(z10));
  nand4  g074(.a(new_n103_), .b(x21), .c(new_n101_), .d(new_n89_), .O(new_n118_));
  nand3  g075(.a(new_n95_), .b(new_n110_), .c(new_n72_), .O(new_n119_));
  nor2   g076(.a(new_n97_), .b(x14), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n93_), .c(x20), .d(new_n98_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n94_), .c(new_n51_), .O(new_n123_));
  nand2  g080(.a(new_n62_), .b(new_n102_), .O(new_n124_));
  nand2  g081(.a(new_n124_), .b(new_n123_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n89_), .b(x17), .c(new_n52_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand4  g088(.a(new_n131_), .b(x12), .c(new_n102_), .d(new_n51_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n126_), .c(x09), .O(new_n134_));
  inv1   g091(.a(new_n134_), .O(z12));
  nand4  g092(.a(new_n89_), .b(x17), .c(new_n52_), .d(new_n51_), .O(new_n136_));
  aoi21  g093(.a(new_n136_), .b(x12), .c(x01), .O(z13));
  nand2  g094(.a(new_n59_), .b(new_n44_), .O(new_n138_));
  aoi21  g095(.a(new_n138_), .b(x12), .c(x01), .O(z14));
  aoi21  g096(.a(new_n57_), .b(x01), .c(new_n52_), .O(new_n140_));
  aoi21  g097(.a(x19), .b(new_n52_), .c(new_n62_), .O(new_n141_));
  oai22  g098(.a(new_n141_), .b(x01), .c(new_n140_), .d(new_n51_), .O(z15));
  oai21  g099(.a(new_n102_), .b(x00), .c(new_n124_), .O(z16));
  nand4  g100(.a(x12), .b(x02), .c(new_n102_), .d(new_n51_), .O(new_n144_));
  inv1   g101(.a(new_n144_), .O(z17));
endmodule


