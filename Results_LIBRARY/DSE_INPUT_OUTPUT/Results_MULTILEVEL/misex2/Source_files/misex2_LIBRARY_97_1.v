// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n134_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nor2   g10(.a(new_n49_), .b(x01), .O(z01));
  nand2  g11(.a(new_n48_), .b(new_n46_), .O(new_n55_));
  inv1   g12(.a(x19), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(x18), .c(new_n44_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n55_), .c(new_n49_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(x10), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x09), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x12), .c(x11), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z03));
  inv1   g21(.a(z01), .O(new_n65_));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nand3  g24(.a(new_n62_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n60_), .c(new_n65_), .O(z04));
  nand3  g26(.a(x10), .b(x02), .c(x00), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x01), .c(new_n49_), .O(z05));
  nand3  g28(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(x11), .c(x10), .d(new_n49_), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(z06));
  nand3  g32(.a(new_n62_), .b(new_n67_), .c(x11), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n65_), .O(z07));
  inv1   g36(.a(x05), .O(new_n80_));
  inv1   g37(.a(x06), .O(new_n81_));
  inv1   g38(.a(x07), .O(new_n82_));
  inv1   g39(.a(x04), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n82_), .c(new_n81_), .d(new_n80_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(x08), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n45_), .c(x17), .d(new_n49_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n56_), .O(z08));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n47_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(x12), .c(new_n66_), .d(new_n49_), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nor2   g54(.a(x14), .b(x13), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nor2   g57(.a(new_n45_), .b(new_n47_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n56_), .O(new_n102_));
  oai21  g59(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n92_), .c(new_n91_), .d(new_n46_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z09));
  nand3  g62(.a(new_n56_), .b(x18), .c(x01), .O(new_n106_));
  nor4   g63(.a(new_n106_), .b(new_n92_), .c(new_n91_), .d(x20), .O(new_n107_));
  nor2   g64(.a(x13), .b(new_n67_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n94_), .c(new_n66_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n92_), .b(new_n91_), .c(x20), .O(new_n111_));
  nor4   g68(.a(new_n111_), .b(new_n97_), .c(new_n96_), .d(x14), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n110_), .c(new_n107_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x00), .c(new_n65_), .O(z10));
  nand4  g71(.a(new_n101_), .b(x21), .c(new_n100_), .d(new_n56_), .O(new_n115_));
  nor2   g72(.a(new_n96_), .b(x14), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n91_), .c(x20), .d(new_n97_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n109_), .c(new_n115_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n92_), .c(new_n46_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n65_), .O(z11));
  inv1   g77(.a(x24), .O(new_n121_));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(new_n121_), .c(x09), .d(x01), .O(new_n123_));
  nor2   g80(.a(new_n123_), .b(new_n46_), .O(z12));
  nand4  g81(.a(new_n56_), .b(x17), .c(new_n48_), .d(new_n46_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n49_), .c(x01), .O(z13));
  nor3   g83(.a(x10), .b(x02), .c(x00), .O(new_n127_));
  nand4  g84(.a(new_n127_), .b(new_n56_), .c(new_n45_), .d(new_n44_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n49_), .c(x01), .O(z14));
  aoi22  g86(.a(new_n122_), .b(x01), .c(new_n49_), .d(new_n48_), .O(new_n130_));
  nand4  g87(.a(x19), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n131_));
  oai21  g88(.a(new_n130_), .b(new_n46_), .c(new_n131_), .O(z15));
  nor2   g89(.a(new_n47_), .b(x00), .O(z16));
  nand4  g90(.a(new_n49_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z17));
  nor2   g92(.a(new_n49_), .b(x01), .O(z02));
endmodule


