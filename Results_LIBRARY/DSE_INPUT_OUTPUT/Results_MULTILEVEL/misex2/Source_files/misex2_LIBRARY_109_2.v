// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n127_, new_n128_, new_n130_,
    new_n131_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z02));
  inv1   g15(.a(x01), .O(new_n59_));
  inv1   g16(.a(x02), .O(new_n60_));
  nand3  g17(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(x09), .c(new_n60_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n59_), .c(x00), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nor2   g21(.a(x02), .b(x01), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(x18), .d(new_n44_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x00), .O(new_n68_));
  nor2   g25(.a(new_n59_), .b(new_n68_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(x10), .c(new_n46_), .d(x02), .O(new_n70_));
  nor3   g27(.a(new_n70_), .b(x12), .c(x11), .O(z04));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n68_), .O(z05));
  nand4  g30(.a(x11), .b(x10), .c(new_n46_), .d(x02), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x01), .c(new_n68_), .O(z06));
  inv1   g32(.a(x11), .O(new_n76_));
  nor3   g33(.a(new_n70_), .b(x12), .c(new_n76_), .O(z07));
  nor2   g34(.a(x03), .b(new_n60_), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x04), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nand3  g38(.a(x19), .b(new_n45_), .c(x17), .O(new_n82_));
  nor3   g39(.a(new_n82_), .b(x08), .c(x07), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(new_n81_), .c(new_n78_), .O(new_n84_));
  aoi21  g41(.a(new_n84_), .b(new_n68_), .c(x01), .O(z08));
  nor2   g42(.a(x14), .b(x13), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x12), .c(new_n76_), .d(x02), .O(new_n87_));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(x16), .O(new_n89_));
  nor2   g46(.a(x22), .b(x21), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  oai21  g48(.a(new_n91_), .b(new_n87_), .c(new_n68_), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n59_), .O(new_n93_));
  nor3   g50(.a(new_n45_), .b(new_n59_), .c(x00), .O(new_n94_));
  nor2   g51(.a(x20), .b(x19), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n90_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n93_), .O(z09));
  nand2  g54(.a(x22), .b(x21), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x20), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n64_), .c(x18), .d(x01), .O(new_n100_));
  nand2  g57(.a(new_n90_), .b(x20), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  inv1   g59(.a(x13), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x12), .c(new_n76_), .O(new_n104_));
  nor3   g61(.a(new_n104_), .b(new_n60_), .c(x01), .O(new_n105_));
  nor3   g62(.a(new_n89_), .b(new_n88_), .c(x14), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n102_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n100_), .c(x00), .O(z10));
  nand4  g65(.a(new_n90_), .b(x20), .c(new_n89_), .d(x15), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(new_n87_), .c(new_n68_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n59_), .O(new_n111_));
  inv1   g68(.a(x22), .O(new_n112_));
  nand4  g69(.a(new_n95_), .b(new_n94_), .c(new_n112_), .d(x21), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n111_), .O(z11));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(x10), .b(x02), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(x01), .c(x00), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n64_), .b(x17), .c(new_n60_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n59_), .c(new_n68_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n115_), .c(x09), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(z12));
  nand2  g81(.a(new_n47_), .b(x17), .O(new_n125_));
  nor2   g82(.a(new_n125_), .b(x19), .O(z13));
  nor3   g83(.a(x10), .b(x09), .c(x02), .O(new_n127_));
  nand4  g84(.a(new_n127_), .b(new_n64_), .c(new_n45_), .d(new_n44_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n68_), .c(x01), .O(z14));
  aoi21  g86(.a(x19), .b(new_n60_), .c(x00), .O(new_n130_));
  nand2  g87(.a(new_n116_), .b(x00), .O(new_n131_));
  oai21  g88(.a(new_n130_), .b(x01), .c(new_n131_), .O(z15));
  xor2a  g89(.a(x01), .b(x00), .O(z16));
  aoi21  g90(.a(new_n60_), .b(new_n68_), .c(x01), .O(z17));
endmodule


