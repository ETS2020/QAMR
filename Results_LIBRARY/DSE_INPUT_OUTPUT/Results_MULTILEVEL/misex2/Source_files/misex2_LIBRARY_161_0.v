// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n82_, new_n83_;
  nor2   g00(.a(x01), .b(x00), .O(z02));
  inv1   g01(.a(x12), .O(new_n47_));
  inv1   g02(.a(x09), .O(new_n48_));
  nand3  g03(.a(x02), .b(x01), .c(x00), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand4  g05(.a(new_n50_), .b(x11), .c(x10), .d(new_n48_), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(new_n47_), .O(z03));
  inv1   g07(.a(x11), .O(new_n53_));
  nand4  g08(.a(new_n50_), .b(new_n53_), .c(x10), .d(new_n48_), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(x12), .O(z04));
  inv1   g10(.a(z02), .O(new_n56_));
  nand2  g11(.a(x01), .b(x00), .O(new_n57_));
  nand3  g12(.a(x10), .b(x09), .c(x02), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(z05));
  nand3  g14(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand3  g15(.a(x11), .b(x10), .c(new_n48_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n60_), .c(new_n56_), .O(z06));
  nand4  g17(.a(new_n47_), .b(x11), .c(x10), .d(new_n48_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(x01), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(x02), .c(x00), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z07));
  inv1   g21(.a(x18), .O(new_n67_));
  nor2   g22(.a(x19), .b(new_n67_), .O(new_n68_));
  nor3   g23(.a(x22), .b(x21), .c(x20), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(x01), .c(x00), .O(z09));
  inv1   g26(.a(x20), .O(new_n72_));
  nand4  g27(.a(new_n68_), .b(x22), .c(x21), .d(new_n72_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(x01), .c(x00), .O(z10));
  inv1   g29(.a(x22), .O(new_n75_));
  nand4  g30(.a(new_n68_), .b(new_n75_), .c(x21), .d(new_n72_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(x01), .c(x00), .O(z11));
  inv1   g32(.a(x00), .O(z16));
  aoi21  g33(.a(x10), .b(x02), .c(x24), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(x09), .c(x01), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(z16), .O(z12));
  inv1   g36(.a(x10), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x01), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x02), .c(z16), .O(z15));
  zero   g39(.O(z00));
  zero   g40(.O(z01));
  nor2   g41(.a(x01), .b(x00), .O(z08));
  nor2   g42(.a(x01), .b(x00), .O(z13));
  nor2   g43(.a(x01), .b(x00), .O(z14));
  nor2   g44(.a(x01), .b(x00), .O(z17));
endmodule


