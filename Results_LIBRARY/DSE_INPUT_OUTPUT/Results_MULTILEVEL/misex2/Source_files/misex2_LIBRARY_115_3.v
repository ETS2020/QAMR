// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n143_,
    new_n144_, new_n145_;
  inv1   g000(.a(x09), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  nor3   g002(.a(x02), .b(x01), .c(x00), .O(new_n46_));
  nand4  g003(.a(new_n46_), .b(new_n45_), .c(x10), .d(new_n44_), .O(new_n47_));
  nor3   g004(.a(new_n47_), .b(x19), .c(x18), .O(z00));
  nand4  g005(.a(new_n46_), .b(new_n45_), .c(x10), .d(x09), .O(new_n49_));
  nor3   g006(.a(new_n49_), .b(x19), .c(x18), .O(z01));
  nand2  g007(.a(x03), .b(x01), .O(new_n51_));
  inv1   g008(.a(x02), .O(new_n52_));
  nor2   g009(.a(x01), .b(x00), .O(new_n53_));
  nand3  g010(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g011(.a(x10), .O(new_n55_));
  inv1   g012(.a(x18), .O(new_n56_));
  inv1   g013(.a(x19), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n56_), .c(new_n45_), .d(new_n55_), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n54_), .c(new_n51_), .O(z02));
  inv1   g016(.a(x03), .O(new_n60_));
  nand3  g017(.a(new_n44_), .b(x02), .c(x00), .O(new_n61_));
  nand3  g018(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  oai21  g019(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand2  g020(.a(new_n63_), .b(x01), .O(new_n64_));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(new_n57_), .c(x18), .d(new_n45_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n64_), .O(z03));
  inv1   g024(.a(x11), .O(new_n68_));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g026(.a(new_n69_), .b(x03), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n68_), .c(x10), .d(new_n44_), .O(new_n71_));
  nor2   g028(.a(new_n71_), .b(x12), .O(z04));
  inv1   g029(.a(x01), .O(new_n73_));
  inv1   g030(.a(x00), .O(new_n74_));
  nor2   g031(.a(new_n52_), .b(new_n74_), .O(new_n75_));
  nand3  g032(.a(new_n75_), .b(x10), .c(x09), .O(new_n76_));
  aoi21  g033(.a(new_n76_), .b(new_n60_), .c(new_n73_), .O(z05));
  nand4  g034(.a(new_n75_), .b(x11), .c(x10), .d(new_n44_), .O(new_n78_));
  aoi21  g035(.a(new_n78_), .b(new_n60_), .c(new_n73_), .O(z06));
  nand2  g036(.a(new_n44_), .b(new_n60_), .O(new_n80_));
  inv1   g037(.a(x12), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x11), .c(x10), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(new_n80_), .c(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g041(.a(new_n84_), .O(z07));
  nor2   g042(.a(new_n52_), .b(x01), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(new_n74_), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  nand3  g045(.a(new_n88_), .b(x04), .c(new_n60_), .O(new_n89_));
  nor2   g046(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nor3   g047(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nor2   g048(.a(new_n57_), .b(x18), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(x17), .O(new_n93_));
  nand2  g050(.a(new_n93_), .b(new_n51_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n68_), .c(x02), .d(new_n73_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g058(.a(x20), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(new_n57_), .c(x18), .d(x01), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n74_), .O(new_n105_));
  nand2  g062(.a(new_n105_), .b(new_n51_), .O(z09));
  nand3  g063(.a(new_n57_), .b(x18), .c(x01), .O(new_n107_));
  nand3  g064(.a(x22), .b(x21), .c(new_n102_), .O(new_n108_));
  nor2   g065(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g066(.a(x02), .b(new_n73_), .O(new_n110_));
  inv1   g067(.a(x13), .O(new_n111_));
  nand3  g068(.a(new_n111_), .b(x12), .c(new_n68_), .O(new_n112_));
  nor2   g069(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  inv1   g070(.a(x14), .O(new_n114_));
  nand3  g071(.a(x16), .b(x15), .c(new_n114_), .O(new_n115_));
  nand3  g072(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n116_));
  nor2   g073(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n113_), .c(new_n109_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(x00), .c(new_n51_), .O(z10));
  nand4  g076(.a(new_n86_), .b(new_n111_), .c(x12), .d(new_n68_), .O(new_n120_));
  nor2   g077(.a(new_n56_), .b(x03), .O(new_n121_));
  nor2   g078(.a(new_n95_), .b(x20), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(new_n121_), .c(new_n57_), .d(x01), .O(new_n123_));
  nor2   g080(.a(new_n98_), .b(x14), .O(new_n124_));
  nand4  g081(.a(new_n124_), .b(new_n95_), .c(x20), .d(new_n99_), .O(new_n125_));
  oai21  g082(.a(new_n125_), .b(new_n120_), .c(new_n123_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n96_), .c(new_n74_), .O(new_n127_));
  inv1   g084(.a(new_n127_), .O(z11));
  inv1   g085(.a(x24), .O(new_n129_));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n57_), .b(x17), .c(new_n52_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n73_), .c(new_n74_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n129_), .c(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n51_), .O(z12));
  nand2  g095(.a(new_n46_), .b(x17), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x19), .O(z13));
  nand3  g097(.a(new_n53_), .b(new_n44_), .c(new_n52_), .O(new_n141_));
  oai21  g098(.a(new_n141_), .b(new_n58_), .c(new_n51_), .O(z14));
  oai21  g099(.a(x10), .b(new_n73_), .c(x02), .O(new_n143_));
  nand2  g100(.a(new_n143_), .b(x00), .O(new_n144_));
  nand3  g101(.a(x19), .b(new_n52_), .c(new_n73_), .O(new_n145_));
  nand3  g102(.a(new_n145_), .b(new_n144_), .c(new_n51_), .O(z15));
  aoi21  g103(.a(new_n60_), .b(x00), .c(new_n73_), .O(z16));
  nand2  g104(.a(new_n87_), .b(new_n51_), .O(z17));
endmodule


