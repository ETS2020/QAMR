// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_;
  inv1   g00(.a(x22), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n51_), .c(new_n50_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x01), .O(new_n63_));
  nand3  g20(.a(new_n46_), .b(new_n63_), .c(new_n62_), .O(new_n64_));
  nand3  g21(.a(new_n52_), .b(x18), .c(new_n50_), .O(new_n65_));
  nor2   g22(.a(x09), .b(new_n46_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(x01), .c(x00), .O(new_n67_));
  nand4  g24(.a(x22), .b(x12), .c(x11), .d(x10), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n67_), .c(new_n65_), .d(new_n64_), .O(z03));
  nor2   g26(.a(x12), .b(x11), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n66_), .c(x10), .d(x00), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x22), .c(new_n63_), .O(z04));
  nor2   g29(.a(new_n46_), .b(new_n62_), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(x10), .c(x09), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x22), .c(new_n63_), .O(z05));
  nand4  g32(.a(new_n73_), .b(x11), .c(x10), .d(new_n47_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x22), .c(new_n63_), .O(z06));
  inv1   g34(.a(x12), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n47_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n45_), .O(z07));
  inv1   g39(.a(x04), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n63_), .c(new_n62_), .O(new_n84_));
  nor4   g41(.a(new_n84_), .b(x05), .c(new_n83_), .d(x03), .O(new_n85_));
  nor3   g42(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nor2   g43(.a(new_n52_), .b(x18), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(x17), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n45_), .O(z08));
  inv1   g46(.a(x16), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x11), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(x02), .c(new_n63_), .d(new_n62_), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n93_), .c(new_n92_), .d(x12), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(x15), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n91_), .c(x20), .d(new_n90_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(x22), .O(z09));
  nand2  g57(.a(x18), .b(x01), .O(new_n101_));
  nor4   g58(.a(new_n101_), .b(new_n91_), .c(x20), .d(x19), .O(new_n102_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(x12), .O(new_n103_));
  nor3   g60(.a(new_n103_), .b(x11), .c(new_n46_), .O(new_n104_));
  inv1   g61(.a(x15), .O(new_n105_));
  nand3  g62(.a(new_n44_), .b(new_n91_), .c(x20), .O(new_n106_));
  nor3   g63(.a(new_n106_), .b(new_n90_), .c(new_n105_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n104_), .c(new_n102_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n45_), .O(z10));
  nand3  g66(.a(new_n92_), .b(x12), .c(new_n94_), .O(new_n110_));
  nor3   g67(.a(new_n110_), .b(new_n46_), .c(x00), .O(new_n111_));
  nand3  g68(.a(new_n91_), .b(x20), .c(new_n90_), .O(new_n112_));
  nor3   g69(.a(new_n112_), .b(new_n105_), .c(x14), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n63_), .c(x22), .O(z11));
  nand2  g72(.a(x10), .b(x02), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(x22), .c(x01), .d(x00), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n63_), .c(new_n62_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n117_), .c(x24), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x09), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(z12));
  inv1   g81(.a(new_n48_), .O(new_n125_));
  oai21  g82(.a(new_n119_), .b(new_n125_), .c(new_n45_), .O(z13));
  nand3  g83(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(new_n128_));
  nand4  g85(.a(new_n128_), .b(new_n51_), .c(new_n50_), .d(new_n59_), .O(new_n129_));
  nor2   g86(.a(new_n129_), .b(x19), .O(z14));
  aoi22  g87(.a(new_n45_), .b(x00), .c(x19), .d(new_n63_), .O(new_n131_));
  nand4  g88(.a(x22), .b(new_n59_), .c(x01), .d(x00), .O(new_n132_));
  oai21  g89(.a(new_n131_), .b(x02), .c(new_n132_), .O(z15));
  aoi21  g90(.a(x22), .b(x00), .c(new_n63_), .O(z16));
  nand2  g91(.a(new_n84_), .b(new_n45_), .O(z17));
endmodule


