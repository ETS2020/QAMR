// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n126_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(x00), .O(new_n59_));
  inv1   g16(.a(x01), .O(new_n60_));
  nand3  g17(.a(new_n45_), .b(new_n60_), .c(new_n59_), .O(new_n61_));
  nand3  g18(.a(new_n51_), .b(x18), .c(new_n49_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n44_), .O(z03));
  inv1   g20(.a(new_n44_), .O(z04));
  aoi21  g21(.a(new_n45_), .b(new_n60_), .c(new_n59_), .O(z07));
  nor2   g22(.a(new_n45_), .b(x01), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n59_), .O(new_n69_));
  inv1   g24(.a(new_n69_), .O(new_n70_));
  inv1   g25(.a(x03), .O(new_n71_));
  inv1   g26(.a(x05), .O(new_n72_));
  nand3  g27(.a(new_n72_), .b(x04), .c(new_n71_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  nor3   g29(.a(x08), .b(x07), .c(x06), .O(new_n75_));
  nand3  g30(.a(x19), .b(new_n50_), .c(x17), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n75_), .c(new_n74_), .d(new_n70_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n44_), .O(z08));
  inv1   g34(.a(x21), .O(new_n80_));
  inv1   g35(.a(x22), .O(new_n81_));
  inv1   g36(.a(x11), .O(new_n82_));
  nand4  g37(.a(x12), .b(new_n82_), .c(x02), .d(new_n60_), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  inv1   g39(.a(x16), .O(new_n85_));
  nor2   g40(.a(x14), .b(x13), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(x20), .c(new_n85_), .d(new_n84_), .O(new_n87_));
  inv1   g42(.a(x20), .O(new_n88_));
  and2   g43(.a(x18), .b(x01), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n88_), .c(new_n51_), .O(new_n90_));
  oai21  g45(.a(new_n87_), .b(new_n83_), .c(new_n90_), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n81_), .c(new_n80_), .d(new_n59_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand2  g48(.a(new_n51_), .b(x18), .O(new_n94_));
  nand3  g49(.a(x22), .b(x21), .c(new_n88_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n94_), .c(new_n59_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(x01), .O(new_n97_));
  inv1   g52(.a(x12), .O(new_n98_));
  nor2   g53(.a(x13), .b(new_n98_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n82_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nor3   g56(.a(new_n85_), .b(new_n84_), .c(x14), .O(new_n102_));
  nand3  g57(.a(new_n81_), .b(new_n80_), .c(x20), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n70_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n97_), .O(z10));
  nand4  g61(.a(new_n89_), .b(x21), .c(new_n88_), .d(new_n51_), .O(new_n107_));
  nand3  g62(.a(new_n99_), .b(new_n68_), .c(new_n82_), .O(new_n108_));
  nor2   g63(.a(new_n84_), .b(x14), .O(new_n109_));
  nor2   g64(.a(x21), .b(new_n88_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n85_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n108_), .c(new_n107_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n81_), .c(new_n59_), .O(new_n113_));
  inv1   g68(.a(new_n113_), .O(z11));
  oai21  g69(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g70(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n115_), .c(x24), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(x09), .c(new_n60_), .O(new_n118_));
  nor2   g73(.a(new_n118_), .b(x00), .O(z12));
  inv1   g74(.a(new_n47_), .O(new_n120_));
  oai21  g75(.a(new_n116_), .b(new_n120_), .c(new_n44_), .O(z13));
  nand3  g76(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(x19), .O(z14));
  nor2   g80(.a(x19), .b(x00), .O(new_n126_));
  nor3   g81(.a(new_n126_), .b(x02), .c(x01), .O(z15));
  nand2  g82(.a(new_n69_), .b(new_n44_), .O(z17));
  zero   g83(.O(z05));
  zero   g84(.O(z06));
  buf    g85(.a(x01), .O(z16));
endmodule


