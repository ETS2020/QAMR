// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(new_n49_), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nor2   g009(.a(x21), .b(new_n44_), .O(new_n53_));
  inv1   g010(.a(new_n53_), .O(new_n54_));
  nand3  g011(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  inv1   g012(.a(x19), .O(new_n56_));
  nand4  g013(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n57_));
  oai21  g014(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z01));
  inv1   g015(.a(x10), .O(new_n59_));
  nand4  g016(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n60_));
  oai21  g017(.a(new_n60_), .b(new_n55_), .c(new_n54_), .O(z02));
  nand4  g018(.a(new_n54_), .b(x12), .c(x11), .d(x10), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x09), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(x02), .c(x01), .d(x00), .O(new_n64_));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nor2   g022(.a(x19), .b(new_n45_), .O(new_n66_));
  nand3  g023(.a(new_n66_), .b(new_n65_), .c(new_n44_), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n64_), .O(z03));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  inv1   g026(.a(x11), .O(new_n70_));
  inv1   g027(.a(x12), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n47_), .O(new_n72_));
  oai21  g029(.a(new_n72_), .b(new_n69_), .c(new_n54_), .O(z04));
  inv1   g030(.a(x00), .O(new_n74_));
  nor2   g031(.a(new_n53_), .b(new_n59_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(x09), .c(x02), .d(x01), .O(new_n76_));
  nor2   g033(.a(new_n76_), .b(new_n74_), .O(z05));
  nand3  g034(.a(x11), .b(x10), .c(new_n47_), .O(new_n78_));
  oai21  g035(.a(new_n78_), .b(new_n69_), .c(new_n54_), .O(z06));
  nand4  g036(.a(new_n71_), .b(x11), .c(x10), .d(new_n47_), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n54_), .O(z07));
  inv1   g040(.a(x03), .O(new_n84_));
  nand3  g041(.a(x04), .b(new_n84_), .c(x02), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(new_n86_));
  nor3   g043(.a(x07), .b(x06), .c(x05), .O(new_n87_));
  inv1   g044(.a(x08), .O(new_n88_));
  nand3  g045(.a(x19), .b(new_n45_), .c(new_n88_), .O(new_n89_));
  inv1   g046(.a(new_n89_), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(new_n87_), .c(new_n86_), .d(new_n48_), .O(new_n91_));
  aoi21  g048(.a(new_n91_), .b(x21), .c(new_n44_), .O(z08));
  inv1   g049(.a(x22), .O(new_n93_));
  nor2   g050(.a(new_n46_), .b(x01), .O(new_n94_));
  nand3  g051(.a(new_n94_), .b(x12), .c(new_n70_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g056(.a(x20), .O(new_n100_));
  inv1   g057(.a(x01), .O(new_n101_));
  nor2   g058(.a(new_n45_), .b(new_n101_), .O(new_n102_));
  nand3  g059(.a(new_n102_), .b(new_n100_), .c(new_n56_), .O(new_n103_));
  oai21  g060(.a(new_n99_), .b(new_n95_), .c(new_n103_), .O(new_n104_));
  nand3  g061(.a(new_n104_), .b(new_n93_), .c(new_n74_), .O(new_n105_));
  aoi21  g062(.a(new_n105_), .b(new_n44_), .c(x21), .O(z09));
  inv1   g063(.a(x21), .O(new_n107_));
  nor2   g064(.a(new_n93_), .b(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n66_), .c(new_n100_), .d(x01), .O(new_n109_));
  nand3  g066(.a(new_n70_), .b(x02), .c(new_n101_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nand3  g068(.a(new_n44_), .b(x16), .c(x15), .O(new_n112_));
  nand3  g069(.a(new_n93_), .b(new_n107_), .c(x20), .O(new_n113_));
  nor2   g070(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n111_), .c(new_n98_), .d(x12), .O(new_n115_));
  aoi21  g072(.a(new_n115_), .b(new_n109_), .c(x00), .O(z10));
  nand4  g073(.a(new_n102_), .b(x21), .c(new_n100_), .d(new_n56_), .O(new_n117_));
  nor2   g074(.a(x13), .b(new_n71_), .O(new_n118_));
  nand3  g075(.a(new_n118_), .b(new_n94_), .c(new_n70_), .O(new_n119_));
  nor2   g076(.a(new_n96_), .b(x14), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n107_), .c(x20), .d(new_n97_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(new_n119_), .c(new_n117_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n93_), .c(new_n74_), .O(new_n123_));
  nand2  g080(.a(new_n123_), .b(new_n54_), .O(z11));
  inv1   g081(.a(x24), .O(new_n125_));
  nand2  g082(.a(x10), .b(x02), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  nand3  g084(.a(new_n48_), .b(x23), .c(x02), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n54_), .O(new_n130_));
  nor2   g087(.a(x19), .b(x02), .O(new_n131_));
  oai21  g088(.a(new_n131_), .b(x23), .c(x21), .O(new_n132_));
  nand3  g089(.a(x23), .b(x19), .c(new_n44_), .O(new_n133_));
  oai21  g090(.a(new_n132_), .b(new_n44_), .c(new_n133_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n101_), .c(new_n74_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n130_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n125_), .c(x09), .O(new_n137_));
  inv1   g094(.a(new_n137_), .O(z12));
  nand2  g095(.a(new_n131_), .b(new_n48_), .O(new_n139_));
  aoi21  g096(.a(new_n139_), .b(x21), .c(new_n44_), .O(z13));
  nand4  g097(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z14));
  oai21  g099(.a(x10), .b(new_n101_), .c(x02), .O(new_n143_));
  nand2  g100(.a(new_n143_), .b(x00), .O(new_n144_));
  nand3  g101(.a(x19), .b(new_n46_), .c(new_n101_), .O(new_n145_));
  nand3  g102(.a(new_n145_), .b(new_n144_), .c(new_n54_), .O(z15));
  nor3   g103(.a(new_n53_), .b(new_n101_), .c(x00), .O(z16));
  nand4  g104(.a(new_n54_), .b(x02), .c(new_n101_), .d(new_n74_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


