// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n136_,
    new_n138_, new_n139_;
  inv1   g00(.a(x21), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor3   g02(.a(x09), .b(x02), .c(x00), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  inv1   g08(.a(x10), .O(new_n52_));
  inv1   g09(.a(x00), .O(new_n53_));
  inv1   g10(.a(x01), .O(new_n54_));
  inv1   g11(.a(x02), .O(new_n55_));
  nand4  g12(.a(x09), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(new_n45_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x21), .O(z01));
  nor2   g16(.a(new_n56_), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n51_), .c(new_n50_), .d(new_n45_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x21), .O(z02));
  nand2  g19(.a(new_n55_), .b(new_n53_), .O(new_n63_));
  nand3  g20(.a(new_n51_), .b(x18), .c(new_n45_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(new_n44_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n54_), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g24(.a(new_n52_), .b(x09), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z03));
  nand2  g27(.a(x21), .b(new_n54_), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n68_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n67_), .c(new_n71_), .O(z04));
  nand2  g32(.a(x01), .b(x00), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z05));
  nor3   g35(.a(new_n76_), .b(x09), .c(new_n55_), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x11), .c(x10), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(z06));
  nand3  g38(.a(new_n68_), .b(new_n73_), .c(x11), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n71_), .O(z07));
  nand2  g42(.a(x02), .b(new_n53_), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  nand2  g44(.a(new_n87_), .b(x04), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n86_), .c(x03), .O(new_n89_));
  nor3   g46(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nor2   g47(.a(new_n51_), .b(x18), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x17), .O(new_n92_));
  aoi21  g49(.a(new_n92_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g50(.a(x22), .O(new_n94_));
  nor2   g51(.a(new_n55_), .b(x01), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n72_), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nor2   g58(.a(new_n50_), .b(new_n54_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n44_), .c(new_n101_), .d(new_n51_), .O(new_n103_));
  oai21  g60(.a(new_n100_), .b(new_n96_), .c(new_n103_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n94_), .c(new_n53_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n71_), .O(z09));
  nor3   g63(.a(new_n94_), .b(new_n44_), .c(x20), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n51_), .c(x18), .d(x01), .O(new_n108_));
  inv1   g65(.a(x13), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x12), .c(new_n72_), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nor3   g68(.a(new_n98_), .b(new_n97_), .c(x14), .O(new_n112_));
  nand3  g69(.a(new_n94_), .b(new_n44_), .c(x20), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n95_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n108_), .c(x00), .O(z10));
  nand4  g73(.a(new_n95_), .b(new_n109_), .c(x12), .d(new_n72_), .O(new_n117_));
  nand4  g74(.a(new_n102_), .b(x21), .c(new_n101_), .d(new_n51_), .O(new_n118_));
  nor2   g75(.a(new_n97_), .b(x14), .O(new_n119_));
  nand4  g76(.a(new_n119_), .b(new_n44_), .c(x20), .d(new_n98_), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(new_n117_), .c(new_n118_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n94_), .c(new_n53_), .O(new_n122_));
  inv1   g79(.a(new_n122_), .O(z11));
  inv1   g80(.a(x24), .O(new_n124_));
  aoi21  g81(.a(x10), .b(x02), .c(new_n54_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x00), .O(new_n126_));
  oai21  g83(.a(x19), .b(x02), .c(x23), .O(new_n127_));
  nand3  g84(.a(new_n51_), .b(x17), .c(new_n55_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g86(.a(new_n129_), .b(new_n44_), .c(new_n54_), .d(new_n53_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n124_), .c(x09), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z12));
  nand4  g90(.a(new_n51_), .b(x17), .c(new_n55_), .d(new_n53_), .O(new_n134_));
  aoi21  g91(.a(new_n134_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g92(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n52_), .O(new_n136_));
  aoi21  g93(.a(new_n136_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g94(.a(new_n44_), .b(new_n55_), .c(new_n125_), .O(new_n138_));
  nand4  g95(.a(new_n44_), .b(x19), .c(new_n55_), .d(new_n54_), .O(new_n139_));
  oai21  g96(.a(new_n138_), .b(new_n53_), .c(new_n139_), .O(z15));
  oai21  g97(.a(new_n54_), .b(x00), .c(new_n71_), .O(z16));
  aoi21  g98(.a(new_n86_), .b(new_n44_), .c(x01), .O(z17));
endmodule


