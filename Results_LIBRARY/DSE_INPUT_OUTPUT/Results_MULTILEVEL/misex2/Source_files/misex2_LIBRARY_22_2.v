// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n126_, new_n128_, new_n129_, new_n132_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z02));
  inv1   g15(.a(x00), .O(new_n59_));
  inv1   g16(.a(x01), .O(new_n60_));
  inv1   g17(.a(x02), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(x18), .c(new_n44_), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(z03));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x10), .c(new_n46_), .O(new_n70_));
  nor3   g27(.a(new_n70_), .b(x12), .c(x11), .O(z04));
  nor3   g28(.a(new_n68_), .b(new_n56_), .c(new_n46_), .O(z05));
  inv1   g29(.a(x11), .O(new_n73_));
  nor4   g30(.a(new_n65_), .b(new_n73_), .c(new_n56_), .d(x09), .O(z06));
  nor3   g31(.a(new_n70_), .b(x12), .c(new_n73_), .O(z07));
  nor2   g32(.a(x03), .b(new_n61_), .O(new_n76_));
  nor2   g33(.a(x06), .b(x05), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x04), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nand3  g36(.a(x19), .b(new_n45_), .c(x17), .O(new_n80_));
  nor3   g37(.a(new_n80_), .b(x08), .c(x07), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(new_n79_), .c(new_n76_), .O(new_n82_));
  aoi21  g39(.a(new_n82_), .b(new_n59_), .c(x01), .O(z08));
  inv1   g40(.a(x21), .O(new_n84_));
  inv1   g41(.a(x22), .O(new_n85_));
  nand4  g42(.a(x12), .b(new_n73_), .c(x02), .d(new_n60_), .O(new_n86_));
  inv1   g43(.a(x15), .O(new_n87_));
  inv1   g44(.a(x16), .O(new_n88_));
  nor2   g45(.a(x14), .b(x13), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(x20), .c(new_n88_), .d(new_n87_), .O(new_n90_));
  nor2   g47(.a(x20), .b(x19), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x18), .c(x01), .O(new_n92_));
  oai21  g49(.a(new_n90_), .b(new_n86_), .c(new_n92_), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n85_), .c(new_n84_), .d(new_n59_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(z09));
  nor3   g52(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n63_), .c(x18), .d(x01), .O(new_n97_));
  inv1   g54(.a(x13), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(x12), .c(new_n73_), .O(new_n99_));
  nor3   g56(.a(new_n99_), .b(new_n61_), .c(x01), .O(new_n100_));
  nor3   g57(.a(new_n88_), .b(new_n87_), .c(x14), .O(new_n101_));
  nor2   g58(.a(x22), .b(x21), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(x20), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n101_), .c(new_n100_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n97_), .c(x00), .O(z10));
  nand4  g63(.a(new_n89_), .b(x12), .c(new_n73_), .d(x02), .O(new_n107_));
  nand4  g64(.a(new_n102_), .b(x20), .c(new_n88_), .d(x15), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(new_n107_), .c(new_n59_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n60_), .O(new_n110_));
  nor3   g67(.a(new_n45_), .b(new_n60_), .c(x00), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n91_), .c(new_n85_), .d(x21), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n110_), .O(z11));
  inv1   g70(.a(x24), .O(new_n114_));
  nand2  g71(.a(x10), .b(x02), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(x01), .c(x00), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n63_), .b(x17), .c(new_n61_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n60_), .c(new_n59_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n114_), .c(x09), .O(new_n122_));
  inv1   g79(.a(new_n122_), .O(z12));
  nand2  g80(.a(new_n47_), .b(x17), .O(new_n124_));
  nor2   g81(.a(new_n124_), .b(x19), .O(z13));
  nand4  g82(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n126_));
  nor2   g83(.a(new_n126_), .b(x19), .O(z14));
  aoi21  g84(.a(x19), .b(new_n61_), .c(x00), .O(new_n128_));
  nand2  g85(.a(new_n115_), .b(x00), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(x01), .c(new_n129_), .O(z15));
  xor2a  g87(.a(x01), .b(x00), .O(z16));
  nand3  g88(.a(x02), .b(new_n60_), .c(new_n59_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z17));
endmodule


