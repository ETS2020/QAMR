// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n137_, new_n139_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x05), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  inv1   g10(.a(x09), .O(new_n54_));
  nor2   g11(.a(new_n50_), .b(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand3  g17(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x18), .c(new_n44_), .d(new_n49_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n54_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n67_), .c(x10), .d(new_n54_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  nor3   g28(.a(new_n68_), .b(new_n58_), .c(new_n54_), .O(z05));
  nand2  g29(.a(x05), .b(new_n48_), .O(new_n73_));
  nand3  g30(.a(x11), .b(x10), .c(new_n54_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n64_), .c(new_n73_), .O(z06));
  inv1   g32(.a(x12), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x11), .c(x10), .d(new_n54_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(new_n48_), .b(x01), .O(new_n82_));
  inv1   g39(.a(x04), .O(new_n83_));
  nor2   g40(.a(x05), .b(new_n83_), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n82_), .c(new_n81_), .d(new_n46_), .O(new_n85_));
  nor3   g42(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x19), .c(new_n45_), .d(x17), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n85_), .c(new_n73_), .O(z08));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  nand3  g47(.a(new_n82_), .b(x12), .c(new_n67_), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nor2   g50(.a(x14), .b(x13), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(x20), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n62_), .c(x18), .d(x01), .O(new_n97_));
  oai21  g54(.a(new_n95_), .b(new_n91_), .c(new_n97_), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n90_), .c(new_n89_), .d(new_n46_), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n73_), .O(z09));
  nand4  g57(.a(new_n73_), .b(x22), .c(x21), .d(new_n96_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n62_), .c(x18), .d(x01), .O(new_n103_));
  nor2   g60(.a(x13), .b(new_n76_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n82_), .c(new_n67_), .O(new_n105_));
  nor3   g62(.a(new_n93_), .b(new_n92_), .c(x14), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n90_), .c(new_n89_), .d(x20), .O(new_n107_));
  or2    g64(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n103_), .c(x00), .O(z10));
  aoi21  g66(.a(x05), .b(new_n48_), .c(new_n89_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n96_), .c(new_n62_), .d(x18), .O(new_n111_));
  nor2   g68(.a(new_n92_), .b(x14), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n89_), .c(x20), .d(new_n93_), .O(new_n113_));
  oai22  g70(.a(new_n113_), .b(new_n105_), .c(new_n111_), .d(new_n47_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n90_), .c(new_n46_), .O(new_n115_));
  inv1   g72(.a(new_n115_), .O(z11));
  inv1   g73(.a(x24), .O(new_n117_));
  nand2  g74(.a(new_n49_), .b(new_n48_), .O(new_n118_));
  oai21  g75(.a(x10), .b(new_n48_), .c(new_n118_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(x01), .c(x00), .O(new_n120_));
  nand3  g77(.a(new_n62_), .b(x17), .c(new_n48_), .O(new_n121_));
  nand2  g78(.a(x23), .b(x19), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n121_), .c(x05), .O(new_n123_));
  and2   g80(.a(x23), .b(x02), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n123_), .c(new_n47_), .O(new_n125_));
  oai21  g82(.a(new_n125_), .b(x00), .c(new_n120_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n117_), .c(x09), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z12));
  nor3   g85(.a(new_n50_), .b(x19), .c(new_n44_), .O(z13));
  nor3   g86(.a(x09), .b(x01), .c(x00), .O(new_n130_));
  nor2   g87(.a(x19), .b(x18), .O(new_n131_));
  nand4  g88(.a(new_n131_), .b(new_n130_), .c(new_n44_), .d(new_n58_), .O(new_n132_));
  aoi21  g89(.a(new_n132_), .b(new_n49_), .c(x02), .O(z14));
  aoi21  g90(.a(new_n58_), .b(x01), .c(new_n48_), .O(new_n134_));
  aoi21  g91(.a(x19), .b(new_n47_), .c(x05), .O(new_n135_));
  oai22  g92(.a(new_n135_), .b(x02), .c(new_n134_), .d(new_n46_), .O(z15));
  nand2  g93(.a(new_n73_), .b(x01), .O(new_n137_));
  nor2   g94(.a(new_n137_), .b(x00), .O(z16));
  nand3  g95(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n139_));
  inv1   g96(.a(new_n139_), .O(z17));
endmodule


