// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n55_,
    new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n81_, new_n82_, new_n86_,
    new_n87_;
  inv1   g00(.a(x12), .O(new_n47_));
  inv1   g01(.a(x09), .O(new_n48_));
  nand3  g02(.a(x02), .b(x01), .c(x00), .O(new_n49_));
  inv1   g03(.a(new_n49_), .O(new_n50_));
  nand4  g04(.a(new_n50_), .b(x11), .c(x10), .d(new_n48_), .O(new_n51_));
  nor2   g05(.a(new_n51_), .b(new_n47_), .O(z03));
  nand3  g06(.a(new_n50_), .b(x10), .c(new_n48_), .O(new_n53_));
  nor3   g07(.a(new_n53_), .b(x12), .c(x11), .O(z04));
  nand2  g08(.a(x01), .b(x00), .O(new_n55_));
  nor2   g09(.a(x01), .b(x00), .O(z08));
  inv1   g10(.a(z08), .O(new_n57_));
  nand3  g11(.a(x10), .b(x09), .c(x02), .O(new_n58_));
  oai21  g12(.a(new_n58_), .b(new_n55_), .c(new_n57_), .O(z05));
  inv1   g13(.a(new_n51_), .O(z06));
  inv1   g14(.a(x00), .O(new_n61_));
  inv1   g15(.a(x01), .O(new_n62_));
  oai21  g16(.a(x02), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  nand3  g17(.a(new_n48_), .b(x02), .c(x00), .O(new_n64_));
  nand3  g18(.a(new_n47_), .b(x11), .c(x10), .O(new_n65_));
  oai21  g19(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(z07));
  inv1   g20(.a(x20), .O(new_n67_));
  inv1   g21(.a(x21), .O(new_n68_));
  inv1   g22(.a(x22), .O(new_n69_));
  inv1   g23(.a(x19), .O(new_n70_));
  nand4  g24(.a(new_n70_), .b(x18), .c(x01), .d(new_n61_), .O(new_n71_));
  inv1   g25(.a(new_n71_), .O(new_n72_));
  nand4  g26(.a(new_n72_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n73_));
  inv1   g27(.a(new_n73_), .O(z09));
  inv1   g28(.a(x18), .O(new_n75_));
  nor2   g29(.a(x19), .b(new_n75_), .O(new_n76_));
  nand4  g30(.a(new_n76_), .b(x22), .c(x21), .d(new_n67_), .O(new_n77_));
  aoi21  g31(.a(new_n77_), .b(x01), .c(x00), .O(z10));
  nand4  g32(.a(new_n76_), .b(new_n69_), .c(x21), .d(new_n67_), .O(new_n79_));
  aoi21  g33(.a(new_n79_), .b(x01), .c(x00), .O(z11));
  aoi21  g34(.a(x10), .b(x02), .c(x24), .O(new_n81_));
  nand4  g35(.a(new_n81_), .b(x09), .c(x01), .d(x00), .O(new_n82_));
  nand2  g36(.a(new_n82_), .b(new_n57_), .O(z12));
  inv1   g37(.a(x10), .O(new_n86_));
  nand2  g38(.a(new_n86_), .b(x01), .O(new_n87_));
  aoi21  g39(.a(new_n87_), .b(x02), .c(new_n61_), .O(z15));
  nor2   g40(.a(new_n62_), .b(x00), .O(z16));
  zero   g41(.O(z00));
  zero   g42(.O(z01));
  zero   g43(.O(z02));
  zero   g44(.O(z13));
  zero   g45(.O(z14));
  zero   g46(.O(z17));
endmodule


