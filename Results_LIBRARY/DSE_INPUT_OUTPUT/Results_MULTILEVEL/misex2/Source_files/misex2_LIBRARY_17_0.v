// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n131_, new_n133_, new_n134_, new_n137_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  nand4  g05(.a(x03), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(x10), .d(new_n44_), .O(new_n51_));
  nor3   g08(.a(new_n51_), .b(x19), .c(x18), .O(z00));
  nand3  g09(.a(x09), .b(new_n48_), .c(new_n46_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nor2   g11(.a(x19), .b(x18), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n54_), .c(new_n45_), .d(x10), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(x03), .c(x01), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand3  g15(.a(new_n55_), .b(new_n45_), .c(new_n58_), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(x03), .c(x01), .O(z02));
  nand3  g19(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(x18), .c(new_n45_), .d(x03), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nor2   g23(.a(new_n58_), .b(x09), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n66_), .c(new_n65_), .d(new_n63_), .O(z03));
  inv1   g26(.a(x03), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n47_), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n67_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n66_), .c(new_n71_), .O(z04));
  nand3  g32(.a(x02), .b(x01), .c(x00), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n58_), .c(new_n44_), .O(z05));
  inv1   g34(.a(new_n76_), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n44_), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z06));
  nand3  g37(.a(new_n67_), .b(new_n73_), .c(x11), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(x01), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n71_), .O(z07));
  inv1   g41(.a(x21), .O(new_n86_));
  inv1   g42(.a(x22), .O(new_n87_));
  nand4  g43(.a(x12), .b(new_n72_), .c(x02), .d(new_n47_), .O(new_n88_));
  inv1   g44(.a(x15), .O(new_n89_));
  inv1   g45(.a(x16), .O(new_n90_));
  nor2   g46(.a(x14), .b(x13), .O(new_n91_));
  nand4  g47(.a(new_n91_), .b(x20), .c(new_n90_), .d(new_n89_), .O(new_n92_));
  inv1   g48(.a(x20), .O(new_n93_));
  and2   g49(.a(x18), .b(x01), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(new_n95_));
  oai21  g51(.a(new_n92_), .b(new_n88_), .c(new_n95_), .O(new_n96_));
  nand4  g52(.a(new_n96_), .b(new_n87_), .c(new_n86_), .d(new_n46_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n71_), .O(z09));
  nor3   g54(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n99_));
  nand4  g55(.a(new_n99_), .b(new_n64_), .c(x18), .d(x01), .O(new_n100_));
  nor3   g56(.a(new_n70_), .b(new_n48_), .c(x01), .O(new_n101_));
  nor2   g57(.a(x13), .b(new_n73_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n72_), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  nor3   g60(.a(new_n90_), .b(new_n89_), .c(x14), .O(new_n105_));
  nand3  g61(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  nand4  g63(.a(new_n107_), .b(new_n105_), .c(new_n104_), .d(new_n101_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n100_), .c(x00), .O(z10));
  nand4  g65(.a(new_n94_), .b(x21), .c(new_n93_), .d(new_n64_), .O(new_n110_));
  nor2   g66(.a(new_n48_), .b(x01), .O(new_n111_));
  nand3  g67(.a(new_n102_), .b(new_n111_), .c(new_n72_), .O(new_n112_));
  nor2   g68(.a(new_n89_), .b(x14), .O(new_n113_));
  nor2   g69(.a(x21), .b(new_n93_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n113_), .c(new_n90_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n112_), .c(new_n110_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n87_), .c(new_n46_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n71_), .O(z11));
  inv1   g74(.a(x24), .O(new_n119_));
  aoi21  g75(.a(x10), .b(x02), .c(new_n47_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(x00), .O(new_n121_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n122_));
  nand3  g78(.a(new_n64_), .b(x17), .c(new_n48_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n47_), .c(new_n46_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n119_), .c(x09), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n71_), .O(z12));
  nand4  g84(.a(new_n64_), .b(x17), .c(new_n48_), .d(new_n46_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(x03), .c(x01), .O(z13));
  nand4  g86(.a(new_n60_), .b(new_n44_), .c(new_n48_), .d(new_n46_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(x03), .c(x01), .O(z14));
  aoi21  g88(.a(x03), .b(new_n48_), .c(new_n120_), .O(new_n133_));
  nand4  g89(.a(x19), .b(x03), .c(new_n48_), .d(new_n47_), .O(new_n134_));
  oai21  g90(.a(new_n133_), .b(new_n46_), .c(new_n134_), .O(z15));
  oai21  g91(.a(new_n47_), .b(x00), .c(new_n71_), .O(z16));
  nand2  g92(.a(x02), .b(new_n46_), .O(new_n137_));
  aoi21  g93(.a(new_n137_), .b(x03), .c(x01), .O(z17));
  zero   g94(.O(z08));
endmodule


