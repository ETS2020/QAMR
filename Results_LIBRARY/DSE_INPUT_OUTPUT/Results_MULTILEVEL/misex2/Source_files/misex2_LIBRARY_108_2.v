// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n149_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor4   g005(.a(new_n48_), .b(x09), .c(x06), .d(x02), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  inv1   g008(.a(x02), .O(new_n52_));
  inv1   g009(.a(new_n48_), .O(new_n53_));
  nand3  g010(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g011(.a(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  inv1   g014(.a(x09), .O(new_n58_));
  nand2  g015(.a(new_n58_), .b(x06), .O(new_n59_));
  nand3  g016(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n60_));
  inv1   g017(.a(x10), .O(new_n61_));
  inv1   g018(.a(x19), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n45_), .c(new_n44_), .d(new_n61_), .O(new_n63_));
  oai21  g020(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z02));
  nand4  g021(.a(new_n59_), .b(new_n62_), .c(x18), .d(new_n44_), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n52_), .c(new_n47_), .d(new_n46_), .O(new_n67_));
  nand2  g024(.a(x01), .b(x00), .O(new_n68_));
  inv1   g025(.a(new_n68_), .O(new_n69_));
  nor2   g026(.a(x06), .b(new_n52_), .O(new_n70_));
  nor2   g027(.a(new_n61_), .b(x09), .O(new_n71_));
  inv1   g028(.a(x11), .O(new_n72_));
  inv1   g029(.a(x12), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n75_));
  nand2  g032(.a(new_n75_), .b(new_n67_), .O(z03));
  nor3   g033(.a(new_n68_), .b(x06), .c(new_n52_), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(new_n72_), .c(x10), .d(new_n58_), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(x12), .O(z04));
  nand3  g036(.a(x10), .b(x09), .c(x02), .O(new_n80_));
  oai21  g037(.a(new_n80_), .b(new_n68_), .c(new_n59_), .O(z05));
  inv1   g038(.a(x06), .O(new_n82_));
  nand4  g039(.a(new_n69_), .b(x11), .c(x10), .d(x02), .O(new_n83_));
  aoi21  g040(.a(new_n83_), .b(new_n82_), .c(x09), .O(z06));
  nand3  g041(.a(new_n71_), .b(new_n73_), .c(x11), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n59_), .O(z07));
  inv1   g045(.a(x08), .O(new_n89_));
  inv1   g046(.a(x05), .O(new_n90_));
  inv1   g047(.a(x03), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n92_));
  inv1   g049(.a(new_n92_), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n82_), .c(new_n90_), .d(x04), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(x07), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n45_), .c(x17), .d(new_n89_), .O(new_n96_));
  nor2   g053(.a(new_n96_), .b(new_n62_), .O(z08));
  inv1   g054(.a(x21), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  nor2   g056(.a(new_n52_), .b(x01), .O(new_n100_));
  nand3  g057(.a(new_n100_), .b(x12), .c(new_n72_), .O(new_n101_));
  inv1   g058(.a(x15), .O(new_n102_));
  inv1   g059(.a(x16), .O(new_n103_));
  nor2   g060(.a(x14), .b(x13), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(x20), .c(new_n103_), .d(new_n102_), .O(new_n105_));
  inv1   g062(.a(x20), .O(new_n106_));
  nor2   g063(.a(new_n45_), .b(new_n47_), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(new_n106_), .c(new_n62_), .O(new_n108_));
  oai21  g065(.a(new_n105_), .b(new_n101_), .c(new_n108_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n59_), .c(new_n99_), .d(new_n98_), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(x00), .O(z09));
  nand3  g068(.a(new_n62_), .b(x18), .c(x01), .O(new_n112_));
  nand3  g069(.a(x22), .b(x21), .c(new_n106_), .O(new_n113_));
  nor2   g070(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  inv1   g071(.a(x13), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(x12), .c(new_n72_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(new_n117_));
  nand2  g074(.a(new_n117_), .b(new_n100_), .O(new_n118_));
  inv1   g075(.a(new_n118_), .O(new_n119_));
  nand2  g076(.a(x16), .b(x15), .O(new_n120_));
  nand3  g077(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n121_));
  nor3   g078(.a(new_n121_), .b(new_n120_), .c(x14), .O(new_n122_));
  aoi21  g079(.a(new_n122_), .b(new_n119_), .c(new_n114_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(x00), .c(new_n59_), .O(z10));
  nand4  g081(.a(new_n107_), .b(x21), .c(new_n106_), .d(new_n62_), .O(new_n125_));
  nor2   g082(.a(new_n102_), .b(x14), .O(new_n126_));
  nand4  g083(.a(new_n126_), .b(new_n98_), .c(x20), .d(new_n103_), .O(new_n127_));
  oai21  g084(.a(new_n127_), .b(new_n118_), .c(new_n125_), .O(new_n128_));
  nand4  g085(.a(new_n128_), .b(new_n59_), .c(new_n99_), .d(new_n46_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  nand2  g087(.a(x10), .b(x02), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(x01), .c(x00), .O(new_n132_));
  oai21  g089(.a(x19), .b(x02), .c(x23), .O(new_n133_));
  nand3  g090(.a(new_n62_), .b(x17), .c(new_n52_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n47_), .c(new_n46_), .O(new_n136_));
  aoi21  g093(.a(new_n136_), .b(new_n132_), .c(x24), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(x09), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n59_), .O(z12));
  oai21  g096(.a(new_n134_), .b(new_n48_), .c(new_n59_), .O(z13));
  inv1   g097(.a(new_n63_), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(new_n52_), .c(new_n47_), .d(new_n46_), .O(new_n142_));
  aoi21  g099(.a(new_n142_), .b(new_n82_), .c(x09), .O(z14));
  oai21  g100(.a(x10), .b(new_n47_), .c(x02), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(x00), .O(new_n145_));
  nand3  g102(.a(x19), .b(new_n52_), .c(new_n47_), .O(new_n146_));
  nand3  g103(.a(new_n146_), .b(new_n145_), .c(new_n59_), .O(z15));
  oai21  g104(.a(new_n47_), .b(x00), .c(new_n59_), .O(z16));
  nand4  g105(.a(new_n59_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n149_));
  inv1   g106(.a(new_n149_), .O(z17));
endmodule


