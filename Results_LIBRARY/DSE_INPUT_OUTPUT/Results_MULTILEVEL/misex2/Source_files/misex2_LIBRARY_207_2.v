// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n122_, new_n124_, new_n125_,
    new_n126_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x18), .b(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(new_n44_), .c(x19), .O(z00));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand3  g10(.a(new_n53_), .b(new_n44_), .c(new_n52_), .O(new_n54_));
  or2    g11(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n50_), .c(new_n49_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  nand3  g18(.a(new_n51_), .b(x02), .c(x00), .O(new_n62_));
  nand3  g19(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(x19), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x01), .O(new_n65_));
  inv1   g22(.a(x19), .O(new_n66_));
  nor2   g23(.a(x02), .b(x00), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x18), .d(new_n49_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n65_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  inv1   g28(.a(new_n62_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(x10), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x19), .c(new_n44_), .O(z04));
  nor2   g31(.a(new_n53_), .b(new_n52_), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x10), .c(x09), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x19), .c(new_n44_), .O(z05));
  nand4  g34(.a(new_n75_), .b(x11), .c(x10), .d(new_n51_), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x19), .c(new_n44_), .O(z06));
  nand2  g36(.a(x10), .b(new_n51_), .O(new_n80_));
  nand3  g37(.a(x19), .b(new_n71_), .c(x11), .O(new_n81_));
  oai21  g38(.a(new_n81_), .b(new_n80_), .c(x01), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(z07));
  inv1   g41(.a(x08), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  inv1   g43(.a(x06), .O(new_n87_));
  inv1   g44(.a(x03), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x02), .c(new_n44_), .d(new_n52_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n87_), .c(new_n86_), .d(x04), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(x07), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n50_), .c(x17), .d(new_n85_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n66_), .O(z08));
  nand2  g51(.a(new_n66_), .b(x01), .O(new_n95_));
  nand3  g52(.a(x02), .b(new_n44_), .c(new_n52_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nor2   g54(.a(x13), .b(new_n71_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n70_), .O(new_n99_));
  nor3   g56(.a(x16), .b(x15), .c(x14), .O(new_n100_));
  nor2   g57(.a(x22), .b(x21), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(x20), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n99_), .c(new_n95_), .O(z09));
  inv1   g62(.a(x14), .O(new_n106_));
  nand4  g63(.a(new_n103_), .b(x16), .c(x15), .d(new_n106_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n99_), .c(new_n95_), .O(z10));
  inv1   g65(.a(x16), .O(new_n109_));
  nand4  g66(.a(new_n103_), .b(new_n109_), .c(x15), .d(new_n106_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n99_), .c(new_n95_), .O(z11));
  nand2  g68(.a(x10), .b(x02), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(x01), .c(x00), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g71(.a(new_n66_), .b(x17), .c(new_n53_), .O(new_n115_));
  oai21  g72(.a(new_n114_), .b(x01), .c(new_n115_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n52_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n113_), .c(x24), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x09), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n95_), .O(z12));
  nor3   g77(.a(new_n54_), .b(x19), .c(new_n49_), .O(z13));
  nand3  g78(.a(new_n46_), .b(new_n45_), .c(new_n59_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n44_), .c(x19), .O(z14));
  oai21  g80(.a(x10), .b(new_n44_), .c(x02), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x00), .O(new_n125_));
  nand3  g82(.a(x19), .b(new_n53_), .c(new_n44_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n125_), .c(new_n95_), .O(z15));
  aoi21  g84(.a(x19), .b(x00), .c(new_n44_), .O(z16));
  nand2  g85(.a(new_n96_), .b(new_n95_), .O(z17));
endmodule


