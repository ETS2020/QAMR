// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n124_,
    new_n125_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  nand4  g06(.a(new_n47_), .b(x10), .c(x09), .d(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  nand4  g09(.a(new_n47_), .b(new_n52_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g11(.a(x01), .O(new_n55_));
  nor2   g12(.a(x09), .b(new_n45_), .O(new_n56_));
  nand3  g13(.a(x12), .b(x11), .c(x10), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(new_n59_));
  inv1   g16(.a(x17), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nor2   g18(.a(x02), .b(x01), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n61_), .c(x18), .d(new_n60_), .O(new_n63_));
  oai21  g20(.a(new_n59_), .b(new_n44_), .c(new_n63_), .O(z03));
  nor2   g21(.a(x12), .b(x11), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n56_), .c(x10), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(x01), .c(new_n44_), .O(z04));
  nand3  g24(.a(x10), .b(x09), .c(x02), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(x01), .c(new_n44_), .O(z05));
  inv1   g26(.a(x11), .O(new_n70_));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nor4   g28(.a(new_n71_), .b(new_n70_), .c(new_n52_), .d(x09), .O(z06));
  nand3  g29(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(x11), .c(x10), .d(new_n46_), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(x12), .O(z07));
  nor2   g33(.a(x03), .b(new_n45_), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x04), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  inv1   g38(.a(x18), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n82_), .c(x17), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n81_), .c(new_n80_), .d(new_n77_), .O(new_n85_));
  aoi21  g42(.a(new_n85_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g43(.a(x21), .O(new_n87_));
  inv1   g44(.a(x22), .O(new_n88_));
  nand4  g45(.a(x12), .b(new_n70_), .c(x02), .d(new_n55_), .O(new_n89_));
  inv1   g46(.a(x15), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nor2   g48(.a(x14), .b(x13), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(x20), .c(new_n91_), .d(new_n90_), .O(new_n93_));
  nor2   g50(.a(x20), .b(x19), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x18), .c(x01), .O(new_n95_));
  oai21  g52(.a(new_n93_), .b(new_n89_), .c(new_n95_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n88_), .c(new_n87_), .d(new_n44_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(z09));
  nand4  g55(.a(new_n92_), .b(x12), .c(new_n70_), .d(x02), .O(new_n99_));
  nor2   g56(.a(x22), .b(x21), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(x20), .c(x16), .d(x15), .O(new_n101_));
  oai21  g58(.a(new_n101_), .b(new_n99_), .c(new_n44_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n55_), .O(new_n103_));
  nor3   g60(.a(new_n82_), .b(new_n55_), .c(x00), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n94_), .c(x22), .d(x21), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n103_), .O(z10));
  nand4  g63(.a(new_n100_), .b(x20), .c(new_n91_), .d(x15), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n99_), .c(new_n44_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n55_), .O(new_n109_));
  nand4  g66(.a(new_n104_), .b(new_n94_), .c(new_n88_), .d(x21), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n109_), .O(z11));
  inv1   g68(.a(x24), .O(new_n112_));
  aoi21  g69(.a(x10), .b(x02), .c(new_n44_), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g71(.a(new_n61_), .b(x17), .c(new_n45_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n114_), .c(x01), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n113_), .c(new_n112_), .O(new_n117_));
  nand2  g74(.a(new_n55_), .b(x00), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n46_), .c(new_n118_), .O(z12));
  nand4  g76(.a(x17), .b(new_n45_), .c(new_n55_), .d(new_n44_), .O(new_n120_));
  nor2   g77(.a(new_n120_), .b(x19), .O(z13));
  nand4  g78(.a(new_n47_), .b(new_n52_), .c(new_n46_), .d(new_n45_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n44_), .c(x01), .O(z14));
  nand2  g80(.a(x19), .b(new_n45_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n44_), .c(x01), .O(new_n125_));
  or2    g82(.a(new_n125_), .b(new_n113_), .O(z15));
  nor2   g83(.a(new_n55_), .b(x00), .O(z16));
  aoi21  g84(.a(new_n45_), .b(new_n44_), .c(x01), .O(z17));
endmodule


