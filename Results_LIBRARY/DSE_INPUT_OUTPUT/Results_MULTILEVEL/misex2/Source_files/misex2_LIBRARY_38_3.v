// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n143_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  nand2  g008(.a(x19), .b(x17), .O(new_n52_));
  inv1   g009(.a(x00), .O(new_n53_));
  inv1   g010(.a(x01), .O(new_n54_));
  inv1   g011(.a(x02), .O(new_n55_));
  nand4  g012(.a(x09), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  inv1   g013(.a(x19), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n52_), .O(z01));
  inv1   g016(.a(x10), .O(new_n60_));
  nand2  g017(.a(new_n47_), .b(x09), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x19), .O(z02));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nor2   g022(.a(x19), .b(new_n45_), .O(new_n66_));
  nand3  g023(.a(new_n66_), .b(new_n65_), .c(new_n44_), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g025(.a(new_n68_), .O(new_n69_));
  nor2   g026(.a(new_n60_), .b(x09), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n69_), .c(x12), .d(x11), .O(new_n71_));
  nand3  g028(.a(new_n71_), .b(new_n67_), .c(new_n52_), .O(z03));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  nand4  g031(.a(new_n52_), .b(new_n74_), .c(new_n73_), .d(x10), .O(new_n75_));
  inv1   g032(.a(new_n75_), .O(new_n76_));
  nand4  g033(.a(new_n76_), .b(new_n46_), .c(x02), .d(x01), .O(new_n77_));
  nor2   g034(.a(new_n77_), .b(new_n53_), .O(z04));
  nand4  g035(.a(new_n52_), .b(x10), .c(x09), .d(x02), .O(new_n79_));
  nor3   g036(.a(new_n79_), .b(new_n54_), .c(new_n53_), .O(z05));
  nand4  g037(.a(new_n52_), .b(x11), .c(x10), .d(new_n46_), .O(new_n81_));
  inv1   g038(.a(new_n81_), .O(new_n82_));
  nand4  g039(.a(new_n82_), .b(x02), .c(x01), .d(x00), .O(new_n83_));
  inv1   g040(.a(new_n83_), .O(z06));
  nand3  g041(.a(new_n70_), .b(new_n74_), .c(x11), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x01), .O(new_n86_));
  nand4  g043(.a(new_n86_), .b(new_n52_), .c(x02), .d(x00), .O(new_n87_));
  inv1   g044(.a(new_n87_), .O(z07));
  inv1   g045(.a(new_n52_), .O(z08));
  inv1   g046(.a(x21), .O(new_n90_));
  inv1   g047(.a(x22), .O(new_n91_));
  inv1   g048(.a(x15), .O(new_n92_));
  inv1   g049(.a(x16), .O(new_n93_));
  nand4  g050(.a(new_n52_), .b(x20), .c(new_n93_), .d(new_n92_), .O(new_n94_));
  nor3   g051(.a(new_n94_), .b(x14), .c(x13), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(x12), .c(new_n73_), .d(x02), .O(new_n96_));
  inv1   g053(.a(x20), .O(new_n97_));
  nor2   g054(.a(new_n45_), .b(new_n54_), .O(new_n98_));
  nand3  g055(.a(new_n98_), .b(new_n97_), .c(new_n57_), .O(new_n99_));
  oai21  g056(.a(new_n96_), .b(x01), .c(new_n99_), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(new_n91_), .c(new_n90_), .d(new_n53_), .O(new_n101_));
  inv1   g058(.a(new_n101_), .O(z09));
  inv1   g059(.a(x13), .O(new_n103_));
  inv1   g060(.a(x14), .O(new_n104_));
  nand4  g061(.a(new_n52_), .b(new_n91_), .c(new_n90_), .d(x20), .O(new_n105_));
  nor2   g062(.a(new_n105_), .b(new_n93_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(x15), .c(new_n104_), .d(new_n103_), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(new_n74_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n73_), .c(x02), .d(new_n54_), .O(new_n109_));
  nor2   g066(.a(new_n91_), .b(new_n90_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n66_), .c(new_n97_), .d(x01), .O(new_n111_));
  aoi21  g068(.a(new_n111_), .b(new_n109_), .c(x00), .O(z10));
  nand4  g069(.a(new_n98_), .b(x21), .c(new_n97_), .d(new_n57_), .O(new_n113_));
  nor2   g070(.a(new_n55_), .b(x01), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n103_), .c(x12), .d(new_n73_), .O(new_n115_));
  nor2   g072(.a(new_n92_), .b(x14), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n90_), .c(x20), .d(new_n93_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  nand3  g075(.a(new_n118_), .b(new_n91_), .c(new_n53_), .O(new_n119_));
  nand2  g076(.a(new_n119_), .b(new_n52_), .O(z11));
  nand2  g077(.a(x10), .b(x02), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n52_), .c(x01), .d(x00), .O(new_n122_));
  nand2  g079(.a(x23), .b(x02), .O(new_n123_));
  oai21  g080(.a(new_n44_), .b(x02), .c(new_n123_), .O(new_n124_));
  nand2  g081(.a(new_n124_), .b(new_n57_), .O(new_n125_));
  nand3  g082(.a(x23), .b(x19), .c(new_n44_), .O(new_n126_));
  nand2  g083(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n54_), .c(new_n53_), .O(new_n128_));
  aoi21  g085(.a(new_n128_), .b(new_n122_), .c(x24), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(x09), .O(new_n130_));
  inv1   g087(.a(new_n130_), .O(z12));
  nand2  g088(.a(new_n47_), .b(x17), .O(new_n132_));
  nor2   g089(.a(new_n132_), .b(x19), .O(z13));
  nand4  g090(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n134_));
  nor2   g091(.a(new_n134_), .b(x19), .O(z14));
  oai21  g092(.a(x17), .b(new_n54_), .c(x19), .O(new_n136_));
  nor2   g093(.a(z08), .b(x10), .O(new_n137_));
  aoi22  g094(.a(new_n137_), .b(x01), .c(new_n136_), .d(new_n55_), .O(new_n138_));
  nand4  g095(.a(x19), .b(new_n44_), .c(new_n55_), .d(new_n54_), .O(new_n139_));
  oai21  g096(.a(new_n138_), .b(new_n53_), .c(new_n139_), .O(z15));
  nand3  g097(.a(new_n52_), .b(x01), .c(new_n53_), .O(new_n141_));
  inv1   g098(.a(new_n141_), .O(z16));
  nand4  g099(.a(new_n52_), .b(x02), .c(new_n54_), .d(new_n53_), .O(new_n143_));
  inv1   g100(.a(new_n143_), .O(z17));
endmodule


