// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n145_, new_n147_, new_n148_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x07), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x09), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  nand3  g010(.a(x09), .b(new_n48_), .c(new_n46_), .O(new_n54_));
  inv1   g011(.a(new_n54_), .O(new_n55_));
  nor2   g012(.a(x19), .b(x18), .O(new_n56_));
  nand4  g013(.a(new_n56_), .b(new_n55_), .c(new_n44_), .d(x10), .O(new_n57_));
  aoi21  g014(.a(new_n57_), .b(new_n49_), .c(x01), .O(z01));
  inv1   g015(.a(x10), .O(new_n59_));
  nand3  g016(.a(new_n56_), .b(new_n44_), .c(new_n59_), .O(new_n60_));
  inv1   g017(.a(new_n60_), .O(new_n61_));
  nand2  g018(.a(new_n61_), .b(new_n55_), .O(new_n62_));
  aoi21  g019(.a(new_n62_), .b(new_n49_), .c(x01), .O(z02));
  nand2  g020(.a(new_n48_), .b(new_n46_), .O(new_n64_));
  inv1   g021(.a(x19), .O(new_n65_));
  nand3  g022(.a(new_n65_), .b(x18), .c(new_n44_), .O(new_n66_));
  oai21  g023(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n47_), .O(new_n68_));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g026(.a(new_n59_), .b(x09), .O(new_n70_));
  nand3  g027(.a(new_n70_), .b(x12), .c(x11), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(z03));
  nand2  g029(.a(x07), .b(new_n47_), .O(new_n73_));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand3  g032(.a(new_n70_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n69_), .c(new_n73_), .O(z04));
  nand2  g034(.a(x01), .b(x00), .O(new_n78_));
  nand3  g035(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(new_n78_), .c(new_n73_), .O(z05));
  inv1   g037(.a(x09), .O(new_n81_));
  nand3  g038(.a(x11), .b(x10), .c(new_n81_), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(new_n69_), .c(new_n73_), .O(z06));
  nand2  g040(.a(new_n81_), .b(x01), .O(new_n84_));
  nand3  g041(.a(new_n75_), .b(x11), .c(x10), .O(new_n85_));
  oai22  g042(.a(new_n85_), .b(new_n84_), .c(x07), .d(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  inv1   g044(.a(new_n87_), .O(z07));
  nand2  g045(.a(x02), .b(new_n46_), .O(new_n89_));
  inv1   g046(.a(x05), .O(new_n90_));
  nand2  g047(.a(new_n90_), .b(x04), .O(new_n91_));
  nor3   g048(.a(new_n91_), .b(new_n89_), .c(x03), .O(new_n92_));
  nor2   g049(.a(x08), .b(x06), .O(new_n93_));
  nor2   g050(.a(new_n65_), .b(x18), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(x17), .O(new_n95_));
  aoi21  g052(.a(new_n95_), .b(new_n49_), .c(x01), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nand4  g055(.a(x12), .b(new_n74_), .c(x02), .d(new_n47_), .O(new_n99_));
  inv1   g056(.a(x15), .O(new_n100_));
  inv1   g057(.a(x16), .O(new_n101_));
  nor2   g058(.a(x14), .b(x13), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x20), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  nor2   g061(.a(new_n45_), .b(new_n47_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n104_), .c(new_n65_), .O(new_n106_));
  oai21  g063(.a(new_n103_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n98_), .c(new_n97_), .d(new_n46_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n73_), .O(z09));
  nand3  g066(.a(new_n65_), .b(x18), .c(x01), .O(new_n110_));
  nand3  g067(.a(x22), .b(x21), .c(new_n104_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g069(.a(x02), .b(new_n47_), .O(new_n113_));
  inv1   g070(.a(x13), .O(new_n114_));
  nand3  g071(.a(new_n114_), .b(x12), .c(new_n74_), .O(new_n115_));
  nor2   g072(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  inv1   g073(.a(x14), .O(new_n117_));
  nand3  g074(.a(x16), .b(x15), .c(new_n117_), .O(new_n118_));
  nand3  g075(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n119_));
  nor2   g076(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  aoi21  g077(.a(new_n120_), .b(new_n116_), .c(new_n112_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(x00), .c(new_n73_), .O(z10));
  nand4  g079(.a(new_n105_), .b(x21), .c(new_n104_), .d(new_n65_), .O(new_n123_));
  nand3  g080(.a(x12), .b(new_n74_), .c(new_n49_), .O(new_n124_));
  nor2   g081(.a(new_n124_), .b(new_n113_), .O(new_n125_));
  nand3  g082(.a(x15), .b(new_n117_), .c(new_n114_), .O(new_n126_));
  nand3  g083(.a(new_n97_), .b(x20), .c(new_n101_), .O(new_n127_));
  nor2   g084(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n123_), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(new_n98_), .c(new_n46_), .O(new_n131_));
  inv1   g088(.a(new_n131_), .O(z11));
  inv1   g089(.a(x24), .O(new_n133_));
  nand2  g090(.a(x10), .b(x02), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(x01), .c(x00), .O(new_n135_));
  oai21  g092(.a(x19), .b(x02), .c(x23), .O(new_n136_));
  nand3  g093(.a(new_n65_), .b(x17), .c(new_n48_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand4  g095(.a(new_n138_), .b(new_n49_), .c(new_n47_), .d(new_n46_), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand3  g097(.a(new_n140_), .b(new_n133_), .c(x09), .O(new_n141_));
  inv1   g098(.a(new_n141_), .O(z12));
  nand4  g099(.a(new_n65_), .b(x17), .c(new_n48_), .d(new_n46_), .O(new_n143_));
  aoi21  g100(.a(new_n143_), .b(new_n49_), .c(x01), .O(z13));
  nand4  g101(.a(new_n61_), .b(new_n81_), .c(new_n48_), .d(new_n46_), .O(new_n145_));
  aoi21  g102(.a(new_n145_), .b(new_n49_), .c(x01), .O(z14));
  aoi21  g103(.a(new_n59_), .b(x01), .c(new_n48_), .O(new_n147_));
  aoi21  g104(.a(x19), .b(new_n48_), .c(x07), .O(new_n148_));
  oai22  g105(.a(new_n148_), .b(x01), .c(new_n147_), .d(new_n46_), .O(z15));
  oai21  g106(.a(new_n47_), .b(x00), .c(new_n73_), .O(z16));
  aoi21  g107(.a(new_n89_), .b(new_n49_), .c(x01), .O(z17));
endmodule


