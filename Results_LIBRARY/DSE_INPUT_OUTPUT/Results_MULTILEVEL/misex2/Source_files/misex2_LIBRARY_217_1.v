// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n122_;
  inv1   g00(.a(x12), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  inv1   g04(.a(x00), .O(new_n48_));
  inv1   g05(.a(x01), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z00));
  nand4  g12(.a(x09), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x19), .O(z01));
  nor2   g16(.a(new_n56_), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x19), .O(z02));
  nand2  g19(.a(new_n50_), .b(new_n48_), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x18), .c(new_n45_), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n63_), .c(new_n44_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n49_), .O(new_n67_));
  nand3  g24(.a(new_n51_), .b(x02), .c(x00), .O(new_n68_));
  nand3  g25(.a(x12), .b(x11), .c(x10), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(z03));
  nand2  g27(.a(x12), .b(new_n49_), .O(new_n71_));
  nand3  g28(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g29(.a(x11), .O(new_n73_));
  nand4  g30(.a(new_n44_), .b(new_n73_), .c(x10), .d(new_n51_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n72_), .c(new_n71_), .O(z04));
  nand4  g32(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n47_), .O(z05));
  nand3  g34(.a(x11), .b(x10), .c(new_n51_), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n72_), .c(new_n71_), .O(z06));
  nand2  g36(.a(new_n78_), .b(x01), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n44_), .c(x02), .d(x00), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z07));
  inv1   g39(.a(x05), .O(new_n83_));
  inv1   g40(.a(x06), .O(new_n84_));
  inv1   g41(.a(x07), .O(new_n85_));
  inv1   g42(.a(x04), .O(new_n86_));
  inv1   g43(.a(x03), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x02), .c(new_n49_), .d(new_n48_), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(x08), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n46_), .c(x17), .d(new_n44_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n64_), .O(z08));
  nand3  g50(.a(x18), .b(x01), .c(new_n48_), .O(new_n94_));
  nor2   g51(.a(x20), .b(x19), .O(new_n95_));
  nor2   g52(.a(x22), .b(x21), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n94_), .c(new_n71_), .O(z09));
  nand3  g55(.a(new_n95_), .b(x22), .c(x21), .O(new_n99_));
  oai21  g56(.a(new_n99_), .b(new_n94_), .c(new_n71_), .O(z10));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n95_), .b(new_n101_), .c(x21), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n94_), .c(new_n71_), .O(z11));
  aoi21  g60(.a(x10), .b(x02), .c(new_n49_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(x00), .O(new_n105_));
  oai21  g62(.a(x19), .b(x02), .c(x23), .O(new_n106_));
  nand3  g63(.a(new_n64_), .b(x17), .c(new_n50_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n49_), .c(new_n48_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n105_), .c(x24), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(x09), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n71_), .O(z12));
  nand4  g69(.a(new_n64_), .b(x17), .c(new_n50_), .d(new_n48_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n44_), .c(x01), .O(z13));
  nor2   g71(.a(new_n52_), .b(x10), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n116_));
  nor2   g73(.a(new_n116_), .b(x19), .O(z14));
  aoi21  g74(.a(new_n44_), .b(new_n50_), .c(new_n104_), .O(new_n118_));
  nand4  g75(.a(x19), .b(new_n44_), .c(new_n50_), .d(new_n49_), .O(new_n119_));
  oai21  g76(.a(new_n118_), .b(new_n48_), .c(new_n119_), .O(z15));
  oai21  g77(.a(new_n49_), .b(x00), .c(new_n71_), .O(z16));
  nand2  g78(.a(x02), .b(new_n48_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n44_), .c(x01), .O(z17));
endmodule


