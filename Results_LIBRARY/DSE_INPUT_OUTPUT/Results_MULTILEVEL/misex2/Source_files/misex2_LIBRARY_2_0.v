// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n122_, new_n124_, new_n125_, new_n127_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x11), .c(x01), .O(z00));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x10), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  inv1   g09(.a(x01), .O(new_n53_));
  inv1   g10(.a(x02), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nor3   g12(.a(new_n55_), .b(new_n51_), .c(new_n50_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n49_), .c(new_n44_), .d(x11), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  nand3  g15(.a(new_n46_), .b(new_n44_), .c(new_n51_), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(x09), .c(new_n54_), .d(new_n52_), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(x11), .c(x01), .O(z02));
  nand2  g19(.a(new_n54_), .b(new_n52_), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x18), .c(new_n44_), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n63_), .c(x11), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n53_), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g25(.a(new_n51_), .b(x09), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  nand2  g28(.a(x01), .b(x00), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(x10), .c(new_n50_), .d(x02), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(x12), .c(x11), .O(z04));
  inv1   g32(.a(x11), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n53_), .O(new_n77_));
  nand3  g34(.a(x10), .b(x09), .c(x02), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n72_), .c(new_n77_), .O(z05));
  inv1   g36(.a(new_n74_), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(x11), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z06));
  nor2   g39(.a(x12), .b(new_n76_), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n69_), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x01), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n77_), .O(z07));
  inv1   g44(.a(x05), .O(new_n88_));
  inv1   g45(.a(x06), .O(new_n89_));
  inv1   g46(.a(x07), .O(new_n90_));
  inv1   g47(.a(x04), .O(new_n91_));
  inv1   g48(.a(x03), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(x02), .c(new_n53_), .d(new_n52_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(new_n88_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(x08), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n49_), .c(x17), .d(x11), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n64_), .O(z08));
  inv1   g55(.a(x22), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nor2   g57(.a(new_n53_), .b(x00), .O(z16));
  nand4  g58(.a(z16), .b(new_n100_), .c(new_n64_), .d(x18), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(x21), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z09));
  inv1   g62(.a(x21), .O(new_n106_));
  nor2   g63(.a(new_n102_), .b(new_n106_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(x22), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(z10));
  nand2  g66(.a(new_n107_), .b(new_n99_), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z11));
  nand2  g68(.a(x10), .b(x02), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(x01), .c(x00), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g71(.a(new_n64_), .b(x17), .c(new_n54_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(x11), .c(new_n53_), .d(new_n52_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n113_), .c(x24), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x09), .O(new_n119_));
  inv1   g76(.a(new_n119_), .O(z12));
  nor4   g77(.a(new_n55_), .b(x19), .c(new_n44_), .d(new_n76_), .O(z13));
  nand2  g78(.a(new_n60_), .b(new_n45_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(x11), .c(x01), .O(z14));
  aoi21  g80(.a(new_n51_), .b(x01), .c(new_n54_), .O(new_n124_));
  aoi21  g81(.a(x19), .b(new_n54_), .c(new_n76_), .O(new_n125_));
  oai22  g82(.a(new_n125_), .b(x01), .c(new_n124_), .d(new_n52_), .O(z15));
  nand2  g83(.a(x02), .b(new_n52_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(x11), .c(x01), .O(z17));
endmodule


