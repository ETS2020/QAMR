// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n118_,
    new_n119_, new_n122_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  inv1   g03(.a(x00), .O(new_n47_));
  inv1   g04(.a(x01), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nor3   g07(.a(new_n50_), .b(new_n46_), .c(x09), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x11), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  inv1   g10(.a(x09), .O(new_n54_));
  nor3   g11(.a(new_n50_), .b(new_n46_), .c(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x11), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x19), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n45_), .c(new_n44_), .d(new_n46_), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(x09), .c(new_n49_), .d(new_n47_), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(x11), .c(x01), .O(z02));
  inv1   g19(.a(x11), .O(new_n63_));
  inv1   g20(.a(x12), .O(new_n64_));
  nor3   g21(.a(new_n64_), .b(new_n46_), .c(x09), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(x02), .c(x01), .d(x00), .O(new_n66_));
  nor3   g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n58_), .c(x18), .d(new_n44_), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(new_n66_), .c(new_n63_), .O(z03));
  nand3  g26(.a(new_n64_), .b(x10), .c(new_n54_), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(x02), .c(x00), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(x11), .O(z04));
  nand3  g30(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n46_), .c(new_n54_), .O(z05));
  inv1   g32(.a(new_n74_), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x11), .c(x10), .d(new_n54_), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z06));
  nand2  g35(.a(new_n70_), .b(x01), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x11), .c(x02), .d(x00), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(z07));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x07), .O(new_n84_));
  inv1   g41(.a(x04), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x08), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n45_), .c(x17), .d(x11), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n58_), .O(z08));
  nand2  g49(.a(new_n63_), .b(new_n48_), .O(new_n93_));
  nand3  g50(.a(x18), .b(x01), .c(new_n47_), .O(new_n94_));
  inv1   g51(.a(x20), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(new_n95_), .d(new_n58_), .O(new_n98_));
  oai21  g55(.a(new_n98_), .b(new_n94_), .c(new_n93_), .O(z09));
  nand4  g56(.a(x22), .b(x21), .c(new_n95_), .d(new_n58_), .O(new_n100_));
  oai21  g57(.a(new_n100_), .b(new_n94_), .c(new_n93_), .O(z10));
  nand4  g58(.a(new_n58_), .b(x18), .c(x01), .d(new_n47_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n97_), .c(x21), .d(new_n95_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z11));
  aoi21  g62(.a(x10), .b(x02), .c(new_n48_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(x00), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n58_), .b(x17), .c(new_n49_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n48_), .c(new_n47_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n107_), .c(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n93_), .O(z12));
  nor4   g71(.a(new_n50_), .b(x19), .c(new_n44_), .d(new_n63_), .O(z13));
  nand4  g72(.a(new_n60_), .b(new_n54_), .c(new_n49_), .d(new_n47_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(x11), .c(x01), .O(z14));
  aoi21  g74(.a(x11), .b(new_n49_), .c(new_n106_), .O(new_n118_));
  nand4  g75(.a(x19), .b(x11), .c(new_n49_), .d(new_n48_), .O(new_n119_));
  oai21  g76(.a(new_n118_), .b(new_n47_), .c(new_n119_), .O(z15));
  oai21  g77(.a(new_n48_), .b(x00), .c(new_n93_), .O(z16));
  nand4  g78(.a(x11), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n122_));
  inv1   g79(.a(new_n122_), .O(z17));
endmodule


