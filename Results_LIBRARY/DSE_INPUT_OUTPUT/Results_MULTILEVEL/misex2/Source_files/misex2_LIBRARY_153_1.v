// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n134_, new_n135_;
  inv1   g00(.a(x11), .O(new_n44_));
  nand2  g01(.a(x19), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  inv1   g16(.a(x00), .O(new_n60_));
  nor2   g17(.a(x02), .b(x01), .O(new_n61_));
  nor2   g18(.a(x19), .b(new_n51_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n61_), .c(new_n50_), .d(new_n60_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nor2   g22(.a(new_n57_), .b(x09), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n65_), .c(x12), .d(x11), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n63_), .c(new_n45_), .O(z03));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(new_n65_), .b(new_n69_), .c(x10), .d(new_n47_), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(new_n52_), .c(x11), .O(z04));
  nand2  g28(.a(x01), .b(x00), .O(new_n72_));
  nand3  g29(.a(x10), .b(x09), .c(x02), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n45_), .O(z05));
  nand3  g31(.a(x11), .b(x10), .c(new_n47_), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n64_), .c(new_n45_), .O(z06));
  nand3  g33(.a(new_n66_), .b(new_n69_), .c(x11), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n45_), .O(z07));
  inv1   g37(.a(x05), .O(new_n81_));
  inv1   g38(.a(x06), .O(new_n82_));
  inv1   g39(.a(x07), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  inv1   g41(.a(x01), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n85_), .d(new_n60_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x08), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n51_), .c(x17), .d(x11), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n52_), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n85_), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n44_), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nor2   g56(.a(x14), .b(x13), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g58(.a(x20), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(x18), .c(x01), .O(new_n103_));
  oai21  g60(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n94_), .c(new_n93_), .d(new_n52_), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(x00), .O(z09));
  nand3  g63(.a(new_n52_), .b(x18), .c(x01), .O(new_n107_));
  nor4   g64(.a(new_n107_), .b(new_n94_), .c(new_n93_), .d(x20), .O(new_n108_));
  nor2   g65(.a(x13), .b(new_n69_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n96_), .c(new_n44_), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nand3  g68(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n112_));
  nor4   g69(.a(new_n112_), .b(new_n99_), .c(new_n98_), .d(x14), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n111_), .c(new_n108_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x00), .c(new_n45_), .O(z10));
  nand4  g72(.a(x21), .b(new_n102_), .c(x18), .d(x01), .O(new_n116_));
  nor2   g73(.a(new_n98_), .b(x14), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n93_), .c(x20), .d(new_n99_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n110_), .c(new_n116_), .O(new_n119_));
  nand4  g76(.a(new_n119_), .b(new_n94_), .c(new_n52_), .d(new_n60_), .O(new_n120_));
  inv1   g77(.a(new_n120_), .O(z11));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(x01), .c(x00), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n85_), .c(new_n60_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(new_n123_), .c(x24), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x09), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n45_), .O(z12));
  nand3  g87(.a(new_n48_), .b(x17), .c(new_n46_), .O(new_n131_));
  nor2   g88(.a(new_n131_), .b(x19), .O(z13));
  oai21  g89(.a(new_n58_), .b(new_n49_), .c(new_n45_), .O(z14));
  aoi21  g90(.a(new_n57_), .b(x01), .c(new_n46_), .O(new_n134_));
  oai21  g91(.a(new_n61_), .b(new_n44_), .c(x19), .O(new_n135_));
  oai21  g92(.a(new_n134_), .b(new_n60_), .c(new_n135_), .O(z15));
  oai21  g93(.a(new_n85_), .b(x00), .c(new_n45_), .O(z16));
  oai21  g94(.a(new_n95_), .b(x00), .c(new_n45_), .O(z17));
endmodule


