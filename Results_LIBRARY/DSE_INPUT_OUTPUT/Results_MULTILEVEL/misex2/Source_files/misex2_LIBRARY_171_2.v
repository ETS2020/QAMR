// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n128_, new_n129_;
  inv1   g00(.a(x22), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nand3  g16(.a(new_n47_), .b(x02), .c(x00), .O(new_n60_));
  nand3  g17(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n60_), .c(x22), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x01), .O(new_n63_));
  nor3   g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n51_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n64_), .c(new_n50_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x00), .O(new_n68_));
  inv1   g25(.a(x01), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(x10), .c(new_n47_), .d(x02), .O(new_n71_));
  nor4   g28(.a(new_n71_), .b(new_n44_), .c(x12), .d(x11), .O(z04));
  nand4  g29(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x22), .c(new_n69_), .O(z05));
  inv1   g31(.a(x11), .O(new_n75_));
  nor3   g32(.a(new_n71_), .b(new_n44_), .c(new_n75_), .O(z06));
  inv1   g33(.a(x12), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(x11), .c(x10), .d(new_n47_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n45_), .O(z07));
  inv1   g38(.a(x08), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  inv1   g40(.a(x06), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x02), .c(new_n69_), .d(new_n68_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n84_), .c(new_n83_), .d(x04), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(x07), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n51_), .c(x17), .d(new_n82_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(new_n52_), .O(z08));
  inv1   g48(.a(x16), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  nand4  g52(.a(new_n75_), .b(x02), .c(new_n69_), .d(new_n68_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n95_), .c(new_n94_), .d(x12), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x15), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n93_), .c(x20), .d(new_n92_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(x22), .O(z09));
  nor3   g58(.a(new_n44_), .b(new_n93_), .c(x20), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n65_), .c(x01), .O(new_n103_));
  nor2   g60(.a(new_n46_), .b(x01), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n94_), .b(x12), .c(new_n75_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g64(.a(x15), .O(new_n108_));
  nor3   g65(.a(new_n92_), .b(new_n108_), .c(x14), .O(new_n109_));
  nor2   g66(.a(x22), .b(x21), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n109_), .c(new_n107_), .d(x20), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n103_), .c(x00), .O(z10));
  nor2   g69(.a(new_n98_), .b(new_n108_), .O(new_n113_));
  nand4  g70(.a(new_n113_), .b(new_n93_), .c(x20), .d(new_n92_), .O(new_n114_));
  nor2   g71(.a(new_n114_), .b(x22), .O(z11));
  nand2  g72(.a(x10), .b(x02), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(x22), .c(x01), .d(x00), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n69_), .c(new_n68_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n117_), .c(x24), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x09), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(z12));
  inv1   g81(.a(new_n48_), .O(new_n125_));
  oai21  g82(.a(new_n119_), .b(new_n125_), .c(new_n45_), .O(z13));
  oai21  g83(.a(new_n58_), .b(new_n49_), .c(new_n45_), .O(z14));
  aoi22  g84(.a(new_n45_), .b(x00), .c(x19), .d(new_n69_), .O(new_n128_));
  nand3  g85(.a(new_n70_), .b(x22), .c(new_n57_), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(x02), .c(new_n129_), .O(z15));
  aoi21  g87(.a(x22), .b(x00), .c(new_n69_), .O(z16));
  oai21  g88(.a(new_n105_), .b(x00), .c(new_n45_), .O(z17));
endmodule


