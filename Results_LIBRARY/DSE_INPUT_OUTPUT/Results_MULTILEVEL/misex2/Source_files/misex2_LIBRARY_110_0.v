// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n128_, new_n130_, new_n131_;
  nor3   g00(.a(x09), .b(x02), .c(x00), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(x16), .c(x01), .O(z00));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x02), .O(new_n52_));
  nand4  g09(.a(new_n48_), .b(x09), .c(new_n52_), .d(new_n51_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(x16), .c(x01), .O(z01));
  inv1   g11(.a(x01), .O(new_n55_));
  nand4  g12(.a(x09), .b(new_n52_), .c(new_n55_), .d(new_n51_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x10), .O(new_n57_));
  nand3  g14(.a(new_n57_), .b(new_n45_), .c(x16), .O(new_n58_));
  nor3   g15(.a(new_n58_), .b(x19), .c(x18), .O(z02));
  nand3  g16(.a(new_n52_), .b(new_n55_), .c(new_n51_), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x18), .c(new_n45_), .d(x16), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g20(.a(x09), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n64_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n63_), .c(new_n62_), .d(new_n60_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand2  g24(.a(x01), .b(x00), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(x09), .c(new_n52_), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n67_), .c(x10), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  inv1   g28(.a(x16), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  nand3  g30(.a(x10), .b(x09), .c(x02), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n68_), .c(new_n73_), .O(z05));
  nand3  g32(.a(new_n69_), .b(x11), .c(x10), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z06));
  inv1   g34(.a(x12), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n64_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n73_), .O(z07));
  nand2  g39(.a(x02), .b(new_n51_), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x04), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n83_), .c(x03), .O(new_n86_));
  nor3   g43(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  nor2   g44(.a(new_n61_), .b(x18), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x17), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(x16), .c(x01), .O(z08));
  nand3  g47(.a(x18), .b(x01), .c(new_n51_), .O(new_n91_));
  inv1   g48(.a(x20), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(new_n61_), .O(new_n95_));
  oai21  g52(.a(new_n95_), .b(new_n91_), .c(new_n73_), .O(z09));
  nand3  g53(.a(new_n61_), .b(x18), .c(x01), .O(new_n97_));
  nand3  g54(.a(x22), .b(x21), .c(new_n92_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g56(.a(x02), .b(new_n55_), .O(new_n100_));
  inv1   g57(.a(x13), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x12), .c(new_n67_), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  inv1   g60(.a(x14), .O(new_n104_));
  nand2  g61(.a(x15), .b(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n103_), .c(new_n99_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n73_), .O(z10));
  nor2   g66(.a(new_n55_), .b(x00), .O(z16));
  nand3  g67(.a(z16), .b(new_n61_), .c(x18), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n94_), .c(x21), .d(new_n92_), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z11));
  inv1   g71(.a(x24), .O(new_n115_));
  aoi21  g72(.a(x10), .b(x02), .c(new_n55_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(x00), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n61_), .b(x17), .c(new_n52_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(x16), .c(new_n55_), .d(new_n51_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n115_), .c(x09), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(z12));
  nand4  g81(.a(new_n61_), .b(x17), .c(new_n52_), .d(new_n51_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(x16), .c(x01), .O(z13));
  nor2   g83(.a(x17), .b(x10), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(new_n46_), .c(new_n44_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(x16), .c(x01), .O(z14));
  aoi21  g86(.a(x16), .b(new_n52_), .c(new_n116_), .O(new_n130_));
  nand4  g87(.a(x19), .b(x16), .c(new_n52_), .d(new_n55_), .O(new_n131_));
  oai21  g88(.a(new_n130_), .b(new_n51_), .c(new_n131_), .O(z15));
  aoi21  g89(.a(new_n83_), .b(x16), .c(x01), .O(z17));
endmodule


