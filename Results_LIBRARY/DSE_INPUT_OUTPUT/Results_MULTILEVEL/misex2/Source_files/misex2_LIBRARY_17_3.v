// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n136_,
    new_n138_, new_n139_, new_n142_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand2  g09(.a(x03), .b(x01), .O(new_n53_));
  nand3  g10(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n54_));
  inv1   g11(.a(x19), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n54_), .c(new_n53_), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nand3  g19(.a(new_n46_), .b(new_n62_), .c(new_n61_), .O(new_n63_));
  nand3  g20(.a(new_n55_), .b(x18), .c(new_n44_), .O(new_n64_));
  inv1   g21(.a(x03), .O(new_n65_));
  nor2   g22(.a(new_n62_), .b(new_n61_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n65_), .c(x02), .O(new_n67_));
  nand4  g24(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n67_), .c(new_n64_), .d(new_n63_), .O(z03));
  nor2   g26(.a(x09), .b(new_n46_), .O(new_n70_));
  nor2   g27(.a(x12), .b(x11), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(x00), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(new_n65_), .c(new_n62_), .O(z04));
  nand3  g30(.a(new_n66_), .b(new_n65_), .c(x02), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x10), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z05));
  inv1   g34(.a(x11), .O(new_n78_));
  nor4   g35(.a(new_n74_), .b(new_n78_), .c(new_n58_), .d(x09), .O(z06));
  inv1   g36(.a(x12), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(x11), .c(x10), .d(new_n47_), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(x01), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n53_), .O(z07));
  inv1   g41(.a(x08), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  inv1   g43(.a(x06), .O(new_n87_));
  nand4  g44(.a(new_n65_), .b(x02), .c(new_n62_), .d(new_n61_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n87_), .c(new_n86_), .d(x04), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(x07), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n45_), .c(x17), .d(new_n85_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n55_), .O(z08));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nor2   g52(.a(new_n46_), .b(x01), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n78_), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nor2   g56(.a(x14), .b(x13), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  nor2   g58(.a(new_n45_), .b(new_n62_), .O(new_n102_));
  inv1   g59(.a(x20), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n55_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  oai21  g63(.a(new_n101_), .b(new_n97_), .c(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n95_), .c(new_n94_), .d(new_n61_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n53_), .O(z09));
  nor2   g66(.a(new_n45_), .b(x03), .O(new_n110_));
  nor2   g67(.a(new_n95_), .b(new_n94_), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n110_), .c(new_n105_), .d(x01), .O(new_n112_));
  nor2   g69(.a(x13), .b(new_n80_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n96_), .c(new_n78_), .O(new_n114_));
  nor3   g71(.a(new_n99_), .b(new_n98_), .c(x14), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n95_), .c(new_n94_), .d(x20), .O(new_n116_));
  or2    g73(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n112_), .c(x00), .O(z10));
  nand4  g75(.a(new_n102_), .b(x21), .c(new_n103_), .d(new_n55_), .O(new_n119_));
  nor2   g76(.a(new_n98_), .b(x14), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n94_), .c(x20), .d(new_n99_), .O(new_n121_));
  oai21  g78(.a(new_n121_), .b(new_n114_), .c(new_n119_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n95_), .c(new_n61_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n53_), .O(z11));
  nand2  g81(.a(x10), .b(x02), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  oai21  g83(.a(x19), .b(x02), .c(x23), .O(new_n127_));
  nand3  g84(.a(new_n55_), .b(x17), .c(new_n46_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g86(.a(new_n129_), .b(new_n62_), .c(new_n61_), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(new_n126_), .c(x24), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x09), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(new_n53_), .O(z12));
  inv1   g90(.a(new_n48_), .O(new_n134_));
  oai21  g91(.a(new_n128_), .b(new_n134_), .c(new_n53_), .O(z13));
  nand3  g92(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n136_));
  oai21  g93(.a(new_n136_), .b(new_n59_), .c(new_n53_), .O(z14));
  aoi22  g94(.a(new_n53_), .b(x00), .c(x19), .d(new_n62_), .O(new_n138_));
  nand3  g95(.a(new_n66_), .b(new_n58_), .c(new_n65_), .O(new_n139_));
  oai21  g96(.a(new_n138_), .b(x02), .c(new_n139_), .O(z15));
  aoi21  g97(.a(new_n65_), .b(x00), .c(new_n62_), .O(z16));
  nand2  g98(.a(new_n96_), .b(new_n61_), .O(new_n142_));
  nand2  g99(.a(new_n142_), .b(new_n53_), .O(z17));
endmodule


