// Benchmark "FAU" written by ABC on Fri Aug 14 02:09:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n127_, new_n129_, new_n130_,
    new_n133_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x06), .c(x01), .O(z00));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand4  g10(.a(x06), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand3  g17(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x18), .c(new_n44_), .d(x06), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n50_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand2  g24(.a(x01), .b(x00), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(x09), .c(new_n53_), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n67_), .c(x10), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  inv1   g28(.a(x06), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n52_), .O(new_n73_));
  nand3  g30(.a(x10), .b(x09), .c(x02), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n68_), .c(new_n73_), .O(z05));
  nand3  g32(.a(new_n69_), .b(x11), .c(x10), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z06));
  inv1   g34(.a(x12), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n50_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n73_), .O(z07));
  inv1   g39(.a(x21), .O(new_n84_));
  inv1   g40(.a(x22), .O(new_n85_));
  nor2   g41(.a(new_n53_), .b(x01), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(x12), .c(new_n67_), .O(new_n87_));
  inv1   g43(.a(x15), .O(new_n88_));
  inv1   g44(.a(x16), .O(new_n89_));
  nor2   g45(.a(x14), .b(x13), .O(new_n90_));
  nand4  g46(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  inv1   g47(.a(x20), .O(new_n92_));
  nor2   g48(.a(new_n49_), .b(new_n52_), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(new_n92_), .c(new_n62_), .O(new_n94_));
  oai21  g50(.a(new_n91_), .b(new_n87_), .c(new_n94_), .O(new_n95_));
  nand4  g51(.a(new_n95_), .b(new_n85_), .c(new_n84_), .d(new_n51_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n73_), .O(z09));
  nor3   g53(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(new_n62_), .c(x18), .d(x01), .O(new_n99_));
  nor3   g55(.a(new_n72_), .b(new_n53_), .c(x01), .O(new_n100_));
  inv1   g56(.a(x13), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(x12), .c(new_n67_), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  nor3   g59(.a(new_n89_), .b(new_n88_), .c(x14), .O(new_n104_));
  nand3  g60(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n104_), .c(new_n103_), .d(new_n100_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n99_), .c(x00), .O(z10));
  nand4  g64(.a(new_n93_), .b(x21), .c(new_n92_), .d(new_n62_), .O(new_n109_));
  nand4  g65(.a(new_n86_), .b(new_n101_), .c(x12), .d(new_n67_), .O(new_n110_));
  nor2   g66(.a(new_n88_), .b(x14), .O(new_n111_));
  nand4  g67(.a(new_n111_), .b(new_n84_), .c(x20), .d(new_n89_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n85_), .c(new_n51_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n73_), .O(z11));
  inv1   g71(.a(x24), .O(new_n116_));
  aoi21  g72(.a(x10), .b(x02), .c(new_n52_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(x00), .O(new_n118_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g75(.a(new_n62_), .b(x17), .c(new_n53_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(x06), .c(new_n52_), .d(new_n51_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n118_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n116_), .c(x09), .O(new_n124_));
  inv1   g80(.a(new_n124_), .O(z12));
  nor3   g81(.a(new_n54_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g82(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(x06), .c(x01), .O(z14));
  aoi21  g84(.a(x06), .b(new_n53_), .c(new_n117_), .O(new_n129_));
  nand4  g85(.a(x19), .b(x06), .c(new_n53_), .d(new_n52_), .O(new_n130_));
  oai21  g86(.a(new_n129_), .b(new_n51_), .c(new_n130_), .O(z15));
  oai21  g87(.a(new_n52_), .b(x00), .c(new_n73_), .O(z16));
  nand4  g88(.a(x06), .b(x02), .c(new_n52_), .d(new_n51_), .O(new_n133_));
  inv1   g89(.a(new_n133_), .O(z17));
  zero   g90(.O(z08));
endmodule


