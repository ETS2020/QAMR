// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(x10), .c(x09), .d(new_n48_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n46_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n57_), .c(x09), .d(new_n48_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n46_), .c(x01), .O(z02));
  nand3  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(x18), .c(new_n44_), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand4  g20(.a(x12), .b(x11), .c(x10), .d(new_n49_), .O(new_n64_));
  oai22  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n60_), .O(z03));
  nor2   g22(.a(new_n47_), .b(new_n46_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(x10), .c(new_n49_), .d(x02), .O(new_n67_));
  nor3   g24(.a(new_n67_), .b(x12), .c(x11), .O(z04));
  nand3  g25(.a(x10), .b(x09), .c(x02), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x01), .c(new_n46_), .O(z05));
  inv1   g27(.a(x11), .O(new_n71_));
  nor4   g28(.a(new_n63_), .b(new_n71_), .c(new_n57_), .d(x09), .O(z06));
  nor3   g29(.a(new_n67_), .b(x12), .c(new_n71_), .O(z07));
  nor2   g30(.a(x03), .b(new_n48_), .O(new_n74_));
  nor2   g31(.a(x06), .b(x05), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x04), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(new_n77_));
  nor2   g34(.a(x08), .b(x07), .O(new_n78_));
  nand3  g35(.a(x19), .b(new_n45_), .c(x17), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n78_), .c(new_n77_), .d(new_n74_), .O(new_n81_));
  aoi21  g38(.a(new_n81_), .b(new_n46_), .c(x01), .O(z08));
  nor2   g39(.a(x14), .b(x13), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(x12), .c(new_n71_), .d(x02), .O(new_n84_));
  inv1   g41(.a(x15), .O(new_n85_));
  inv1   g42(.a(x16), .O(new_n86_));
  nor2   g43(.a(x22), .b(x21), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x20), .c(new_n86_), .d(new_n85_), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n84_), .c(new_n46_), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(new_n47_), .O(new_n90_));
  nor3   g47(.a(new_n45_), .b(new_n47_), .c(x00), .O(new_n91_));
  nor2   g48(.a(x20), .b(x19), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(new_n87_), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n90_), .O(z09));
  nand4  g51(.a(new_n87_), .b(x20), .c(x16), .d(x15), .O(new_n95_));
  oai21  g52(.a(new_n95_), .b(new_n84_), .c(new_n46_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n47_), .O(new_n97_));
  nand4  g54(.a(new_n92_), .b(new_n91_), .c(x22), .d(x21), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n97_), .O(z10));
  inv1   g56(.a(x22), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  and2   g58(.a(x18), .b(x01), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(x21), .c(new_n101_), .d(new_n61_), .O(new_n103_));
  nor2   g60(.a(new_n48_), .b(x01), .O(new_n104_));
  inv1   g61(.a(x12), .O(new_n105_));
  nor2   g62(.a(x13), .b(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n104_), .c(new_n71_), .O(new_n107_));
  nor2   g64(.a(new_n85_), .b(x14), .O(new_n108_));
  nor2   g65(.a(x21), .b(new_n101_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n108_), .c(new_n86_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n107_), .c(new_n103_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n100_), .c(new_n46_), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(z11));
  inv1   g70(.a(x24), .O(new_n114_));
  aoi21  g71(.a(x10), .b(x02), .c(new_n46_), .O(new_n115_));
  oai21  g72(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  nand3  g73(.a(new_n61_), .b(x17), .c(new_n48_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(x01), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n115_), .c(new_n114_), .O(new_n119_));
  nand2  g76(.a(new_n47_), .b(x00), .O(new_n120_));
  oai21  g77(.a(new_n119_), .b(new_n49_), .c(new_n120_), .O(z12));
  nand3  g78(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n122_));
  nor3   g79(.a(new_n122_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g80(.a(new_n54_), .b(new_n57_), .c(new_n49_), .d(new_n48_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n46_), .c(x01), .O(z14));
  nand2  g82(.a(x10), .b(x02), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  nand4  g84(.a(x19), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n127_), .O(z15));
  nor2   g86(.a(new_n47_), .b(x00), .O(z16));
  aoi21  g87(.a(new_n48_), .b(new_n46_), .c(x01), .O(z17));
endmodule


