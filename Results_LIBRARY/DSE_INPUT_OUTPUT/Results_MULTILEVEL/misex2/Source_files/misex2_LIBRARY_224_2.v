// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n138_,
    new_n140_;
  inv1   g00(.a(x15), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n58_));
  inv1   g15(.a(new_n58_), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x01), .O(new_n63_));
  nand3  g20(.a(new_n46_), .b(new_n63_), .c(new_n62_), .O(new_n64_));
  nand3  g21(.a(new_n52_), .b(x18), .c(new_n50_), .O(new_n65_));
  nor2   g22(.a(new_n63_), .b(new_n62_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n47_), .c(x02), .O(new_n67_));
  nand4  g24(.a(x15), .b(x12), .c(x11), .d(x10), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n67_), .c(new_n65_), .d(new_n64_), .O(z03));
  nand4  g26(.a(new_n66_), .b(x10), .c(new_n47_), .d(x02), .O(new_n70_));
  nor4   g27(.a(new_n70_), .b(new_n44_), .c(x12), .d(x11), .O(z04));
  nand4  g28(.a(new_n66_), .b(x10), .c(x09), .d(x02), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n44_), .O(z05));
  inv1   g30(.a(x11), .O(new_n74_));
  nor3   g31(.a(new_n70_), .b(new_n44_), .c(new_n74_), .O(z06));
  nand2  g32(.a(x10), .b(new_n47_), .O(new_n76_));
  inv1   g33(.a(x12), .O(new_n77_));
  nand3  g34(.a(x15), .b(new_n77_), .c(x11), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n76_), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(z07));
  inv1   g38(.a(x08), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  inv1   g40(.a(x06), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x02), .c(new_n63_), .d(new_n62_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n84_), .c(new_n83_), .d(x04), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(x07), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n51_), .c(x17), .d(new_n82_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(new_n52_), .O(z08));
  inv1   g48(.a(x21), .O(new_n92_));
  inv1   g49(.a(x22), .O(new_n93_));
  nor2   g50(.a(x13), .b(new_n77_), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n74_), .c(x02), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(new_n44_), .d(new_n96_), .O(new_n98_));
  nand2  g55(.a(x18), .b(x01), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n52_), .O(new_n101_));
  oai22  g58(.a(new_n101_), .b(new_n99_), .c(new_n98_), .d(new_n95_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n93_), .c(new_n92_), .d(new_n62_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n45_), .O(z09));
  nand3  g61(.a(new_n52_), .b(x18), .c(x01), .O(new_n105_));
  nand3  g62(.a(x22), .b(x21), .c(new_n100_), .O(new_n106_));
  nand4  g63(.a(new_n94_), .b(new_n74_), .c(x02), .d(new_n63_), .O(new_n107_));
  nor2   g64(.a(x22), .b(x21), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(x20), .c(x16), .d(new_n96_), .O(new_n109_));
  oai22  g66(.a(new_n109_), .b(new_n107_), .c(new_n106_), .d(new_n105_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(x15), .c(new_n62_), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(z10));
  nand3  g69(.a(x21), .b(new_n100_), .c(new_n52_), .O(new_n113_));
  nand4  g70(.a(x12), .b(new_n74_), .c(x02), .d(new_n63_), .O(new_n114_));
  nor2   g71(.a(x14), .b(x13), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n92_), .c(x20), .d(new_n97_), .O(new_n116_));
  oai22  g73(.a(new_n116_), .b(new_n114_), .c(new_n113_), .d(new_n99_), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n93_), .c(x15), .d(new_n62_), .O(new_n118_));
  inv1   g75(.a(new_n118_), .O(z11));
  nand2  g76(.a(x10), .b(x02), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(x15), .c(x01), .d(x00), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(x23), .O(new_n122_));
  nand3  g79(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(new_n63_), .c(new_n62_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n121_), .c(x24), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x09), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z12));
  inv1   g85(.a(new_n48_), .O(new_n129_));
  oai21  g86(.a(new_n123_), .b(new_n129_), .c(new_n45_), .O(z13));
  nand3  g87(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(new_n132_));
  nand4  g89(.a(new_n132_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n133_));
  nor2   g90(.a(new_n133_), .b(x19), .O(z14));
  aoi22  g91(.a(new_n45_), .b(x00), .c(x19), .d(new_n63_), .O(new_n135_));
  nand3  g92(.a(new_n66_), .b(x15), .c(new_n57_), .O(new_n136_));
  oai21  g93(.a(new_n135_), .b(x02), .c(new_n136_), .O(z15));
  nand3  g94(.a(x15), .b(x01), .c(new_n62_), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z16));
  nand2  g96(.a(new_n48_), .b(x02), .O(new_n140_));
  inv1   g97(.a(new_n140_), .O(z17));
endmodule


