// Benchmark "FAU" written by ABC on Fri Aug 14 02:11:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n146_, new_n148_, new_n149_,
    new_n150_, new_n151_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  nand4  g010(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n54_));
  inv1   g011(.a(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  nor3   g014(.a(x02), .b(x01), .c(x00), .O(new_n58_));
  nor2   g015(.a(x19), .b(x18), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n58_), .c(new_n44_), .d(x09), .O(new_n60_));
  aoi21  g017(.a(new_n60_), .b(x21), .c(x10), .O(z02));
  inv1   g018(.a(x19), .O(new_n62_));
  inv1   g019(.a(x10), .O(new_n63_));
  inv1   g020(.a(x21), .O(new_n64_));
  nand2  g021(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(new_n62_), .c(x18), .d(new_n44_), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n68_));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g026(.a(new_n63_), .b(x09), .O(new_n70_));
  nand3  g027(.a(new_n70_), .b(x12), .c(x11), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(z03));
  inv1   g029(.a(x11), .O(new_n73_));
  nand3  g030(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  inv1   g031(.a(new_n74_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(new_n73_), .c(x10), .d(new_n49_), .O(new_n76_));
  nor2   g033(.a(new_n76_), .b(x12), .O(z04));
  nor3   g034(.a(new_n74_), .b(new_n63_), .c(new_n49_), .O(z05));
  nand3  g035(.a(x11), .b(x10), .c(new_n49_), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(new_n69_), .c(new_n65_), .O(z06));
  nor2   g037(.a(x21), .b(x10), .O(new_n81_));
  nor2   g038(.a(x12), .b(new_n73_), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n70_), .O(new_n83_));
  oai21  g040(.a(new_n81_), .b(x01), .c(new_n83_), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(z07));
  nor2   g043(.a(new_n48_), .b(x01), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n46_), .O(new_n88_));
  inv1   g045(.a(x03), .O(new_n89_));
  inv1   g046(.a(x05), .O(new_n90_));
  nand3  g047(.a(new_n90_), .b(x04), .c(new_n89_), .O(new_n91_));
  nor2   g048(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nor3   g049(.a(x08), .b(x07), .c(x06), .O(new_n93_));
  nor2   g050(.a(new_n62_), .b(x18), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(x17), .O(new_n95_));
  nand2  g052(.a(new_n95_), .b(new_n65_), .O(z08));
  inv1   g053(.a(x22), .O(new_n97_));
  nand4  g054(.a(x12), .b(new_n73_), .c(x02), .d(new_n47_), .O(new_n98_));
  inv1   g055(.a(x15), .O(new_n99_));
  inv1   g056(.a(x16), .O(new_n100_));
  nor2   g057(.a(x14), .b(x13), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x20), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g059(.a(x20), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n62_), .c(x18), .d(x01), .O(new_n104_));
  oai21  g061(.a(new_n102_), .b(new_n98_), .c(new_n104_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n97_), .c(new_n46_), .O(new_n106_));
  aoi21  g063(.a(new_n106_), .b(x10), .c(x21), .O(z09));
  nor3   g064(.a(new_n97_), .b(new_n64_), .c(x20), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n62_), .c(x18), .d(x01), .O(new_n109_));
  nor3   g066(.a(new_n63_), .b(new_n48_), .c(x01), .O(new_n110_));
  inv1   g067(.a(x13), .O(new_n111_));
  nand3  g068(.a(new_n111_), .b(x12), .c(new_n73_), .O(new_n112_));
  inv1   g069(.a(new_n112_), .O(new_n113_));
  nor3   g070(.a(new_n100_), .b(new_n99_), .c(x14), .O(new_n114_));
  nand3  g071(.a(new_n97_), .b(new_n64_), .c(x20), .O(new_n115_));
  inv1   g072(.a(new_n115_), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n114_), .c(new_n113_), .d(new_n110_), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n109_), .c(x00), .O(z10));
  nor2   g075(.a(new_n45_), .b(new_n47_), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(x21), .c(new_n103_), .d(new_n62_), .O(new_n120_));
  nand3  g077(.a(x12), .b(new_n73_), .c(x10), .O(new_n121_));
  inv1   g078(.a(new_n121_), .O(new_n122_));
  inv1   g079(.a(x14), .O(new_n123_));
  nand3  g080(.a(x15), .b(new_n123_), .c(new_n111_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(new_n125_));
  nand3  g082(.a(new_n64_), .b(x20), .c(new_n100_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(new_n127_));
  nand4  g084(.a(new_n127_), .b(new_n125_), .c(new_n122_), .d(new_n87_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n120_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n97_), .c(new_n46_), .O(new_n130_));
  inv1   g087(.a(new_n130_), .O(z11));
  inv1   g088(.a(x24), .O(new_n132_));
  nand2  g089(.a(x10), .b(new_n48_), .O(new_n133_));
  nand2  g090(.a(x21), .b(new_n63_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(x01), .c(x00), .O(new_n136_));
  oai21  g093(.a(x19), .b(x02), .c(x23), .O(new_n137_));
  nand3  g094(.a(new_n62_), .b(x17), .c(new_n48_), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand4  g096(.a(new_n139_), .b(new_n65_), .c(new_n47_), .d(new_n46_), .O(new_n140_));
  nand2  g097(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  nand3  g098(.a(new_n141_), .b(new_n132_), .c(x09), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(z12));
  nand2  g100(.a(new_n47_), .b(new_n46_), .O(new_n144_));
  oai21  g101(.a(new_n138_), .b(new_n144_), .c(new_n65_), .O(z13));
  nand4  g102(.a(new_n59_), .b(new_n58_), .c(new_n44_), .d(new_n49_), .O(new_n146_));
  aoi21  g103(.a(new_n146_), .b(x21), .c(x10), .O(z14));
  nand2  g104(.a(x01), .b(x00), .O(new_n148_));
  oai21  g105(.a(new_n64_), .b(x01), .c(new_n63_), .O(new_n149_));
  nor2   g106(.a(new_n81_), .b(new_n62_), .O(new_n150_));
  aoi22  g107(.a(new_n150_), .b(new_n47_), .c(new_n149_), .d(x00), .O(new_n151_));
  oai22  g108(.a(new_n151_), .b(x02), .c(new_n134_), .d(new_n148_), .O(z15));
  nor3   g109(.a(new_n81_), .b(new_n47_), .c(x00), .O(z16));
  nand2  g110(.a(new_n88_), .b(new_n65_), .O(z17));
endmodule


