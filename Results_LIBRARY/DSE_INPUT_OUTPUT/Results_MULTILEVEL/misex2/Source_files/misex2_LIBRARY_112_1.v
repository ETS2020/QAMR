// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n145_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(new_n49_), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nand3  g009(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g010(.a(new_n53_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(x19), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand4  g014(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z02));
  inv1   g016(.a(x12), .O(new_n60_));
  inv1   g017(.a(x11), .O(new_n61_));
  nor3   g018(.a(new_n61_), .b(new_n57_), .c(x09), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(x02), .c(x01), .d(x00), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(new_n64_), .c(x18), .d(new_n44_), .O(new_n66_));
  aoi21  g023(.a(new_n66_), .b(new_n63_), .c(new_n60_), .O(z03));
  nand2  g024(.a(x01), .b(x00), .O(new_n68_));
  nor3   g025(.a(new_n68_), .b(x09), .c(new_n46_), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n60_), .c(new_n61_), .d(x10), .O(new_n70_));
  nor2   g027(.a(new_n70_), .b(x18), .O(z04));
  nor2   g028(.a(new_n45_), .b(x12), .O(new_n72_));
  inv1   g029(.a(new_n72_), .O(new_n73_));
  nand3  g030(.a(x10), .b(x09), .c(x02), .O(new_n74_));
  oai21  g031(.a(new_n74_), .b(new_n68_), .c(new_n73_), .O(z05));
  inv1   g032(.a(x00), .O(new_n76_));
  inv1   g033(.a(x01), .O(new_n77_));
  nor2   g034(.a(new_n72_), .b(new_n61_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x10), .c(new_n47_), .d(x02), .O(new_n79_));
  nor3   g036(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(z06));
  nand4  g037(.a(new_n60_), .b(x11), .c(x10), .d(new_n47_), .O(new_n81_));
  nand2  g038(.a(new_n81_), .b(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n73_), .O(z07));
  inv1   g041(.a(x08), .O(new_n85_));
  inv1   g042(.a(x05), .O(new_n86_));
  inv1   g043(.a(x06), .O(new_n87_));
  inv1   g044(.a(x03), .O(new_n88_));
  nand4  g045(.a(new_n88_), .b(x02), .c(new_n77_), .d(new_n76_), .O(new_n89_));
  inv1   g046(.a(new_n89_), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(new_n87_), .c(new_n86_), .d(x04), .O(new_n91_));
  nor2   g048(.a(new_n91_), .b(x07), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n45_), .c(x17), .d(new_n85_), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(new_n64_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  inv1   g053(.a(x13), .O(new_n97_));
  nor2   g054(.a(new_n46_), .b(x01), .O(new_n98_));
  nand3  g055(.a(new_n98_), .b(new_n97_), .c(new_n61_), .O(new_n99_));
  inv1   g056(.a(x14), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nand4  g059(.a(x20), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  nor2   g061(.a(new_n45_), .b(new_n77_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n104_), .c(new_n64_), .O(new_n106_));
  oai21  g063(.a(new_n103_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n96_), .c(new_n95_), .d(x12), .O(new_n108_));
  nor2   g065(.a(new_n108_), .b(x00), .O(z09));
  nand3  g066(.a(new_n64_), .b(x18), .c(x01), .O(new_n110_));
  nand3  g067(.a(x22), .b(x21), .c(new_n104_), .O(new_n111_));
  nand4  g068(.a(new_n98_), .b(new_n100_), .c(new_n97_), .d(new_n61_), .O(new_n112_));
  nor2   g069(.a(x22), .b(x21), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(x20), .c(x16), .d(x15), .O(new_n114_));
  oai22  g071(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n110_), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(x12), .c(new_n76_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(z10));
  nand4  g074(.a(new_n105_), .b(x21), .c(new_n104_), .d(new_n64_), .O(new_n118_));
  nor2   g075(.a(new_n101_), .b(x14), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(new_n95_), .c(x20), .d(new_n102_), .O(new_n120_));
  oai21  g077(.a(new_n120_), .b(new_n99_), .c(new_n118_), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n96_), .c(x12), .d(new_n76_), .O(new_n122_));
  inv1   g079(.a(new_n122_), .O(z11));
  inv1   g080(.a(x24), .O(new_n124_));
  nand2  g081(.a(x10), .b(x02), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  oai21  g083(.a(x19), .b(x02), .c(x23), .O(new_n127_));
  nand3  g084(.a(new_n64_), .b(x17), .c(new_n46_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n77_), .c(new_n76_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand4  g088(.a(new_n131_), .b(new_n73_), .c(new_n124_), .d(x09), .O(new_n132_));
  inv1   g089(.a(new_n132_), .O(z12));
  nor2   g090(.a(new_n72_), .b(x19), .O(new_n134_));
  nand4  g091(.a(new_n134_), .b(x17), .c(new_n46_), .d(new_n77_), .O(new_n135_));
  nor2   g092(.a(new_n135_), .b(x00), .O(z13));
  nand3  g093(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n137_));
  nand4  g094(.a(new_n64_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n138_));
  oai21  g095(.a(new_n138_), .b(new_n137_), .c(new_n73_), .O(z14));
  oai21  g096(.a(x10), .b(new_n77_), .c(x02), .O(new_n140_));
  nand2  g097(.a(new_n140_), .b(x00), .O(new_n141_));
  nand3  g098(.a(x19), .b(new_n46_), .c(new_n77_), .O(new_n142_));
  nand3  g099(.a(new_n142_), .b(new_n141_), .c(new_n73_), .O(z15));
  oai21  g100(.a(new_n77_), .b(x00), .c(new_n73_), .O(z16));
  nand4  g101(.a(new_n73_), .b(x02), .c(new_n77_), .d(new_n76_), .O(new_n145_));
  inv1   g102(.a(new_n145_), .O(z17));
endmodule


