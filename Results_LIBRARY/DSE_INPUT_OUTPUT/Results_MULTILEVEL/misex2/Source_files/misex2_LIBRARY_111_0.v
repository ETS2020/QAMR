// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x01), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  inv1   g003(.a(x10), .O(new_n47_));
  inv1   g004(.a(x17), .O(new_n48_));
  inv1   g005(.a(x18), .O(new_n49_));
  nor2   g006(.a(x22), .b(x08), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(new_n51_));
  nand3  g008(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x00), .O(z00));
  nand4  g012(.a(new_n53_), .b(x09), .c(new_n45_), .d(new_n44_), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x00), .O(z01));
  nor2   g014(.a(new_n52_), .b(x10), .O(new_n58_));
  nand4  g015(.a(new_n58_), .b(x09), .c(new_n45_), .d(new_n44_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x00), .O(z02));
  nand3  g017(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  nor2   g019(.a(new_n47_), .b(x09), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(new_n62_), .c(x12), .d(x11), .O(new_n64_));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nor2   g022(.a(x19), .b(new_n49_), .O(new_n66_));
  nand3  g023(.a(new_n66_), .b(new_n65_), .c(new_n48_), .O(new_n67_));
  aoi21  g024(.a(new_n67_), .b(new_n64_), .c(new_n50_), .O(z03));
  inv1   g025(.a(x00), .O(new_n69_));
  inv1   g026(.a(x11), .O(new_n70_));
  inv1   g027(.a(x12), .O(new_n71_));
  inv1   g028(.a(new_n50_), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(x10), .O(new_n73_));
  inv1   g030(.a(new_n73_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(new_n46_), .c(x02), .d(x01), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(new_n69_), .O(z04));
  nand2  g033(.a(x01), .b(x00), .O(new_n77_));
  nand3  g034(.a(x10), .b(x09), .c(x02), .O(new_n78_));
  oai21  g035(.a(new_n78_), .b(new_n77_), .c(new_n72_), .O(z05));
  nor3   g036(.a(new_n50_), .b(new_n70_), .c(new_n47_), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(new_n46_), .c(x02), .d(x01), .O(new_n81_));
  nor2   g038(.a(new_n81_), .b(new_n69_), .O(z06));
  nand3  g039(.a(new_n63_), .b(new_n71_), .c(x11), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(x01), .O(new_n84_));
  nand4  g041(.a(new_n84_), .b(new_n72_), .c(x02), .d(x00), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(z07));
  inv1   g043(.a(x22), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  inv1   g045(.a(x06), .O(new_n89_));
  inv1   g046(.a(x07), .O(new_n90_));
  inv1   g047(.a(x04), .O(new_n91_));
  inv1   g048(.a(x03), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(x02), .c(new_n44_), .d(new_n69_), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(new_n88_), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(x08), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(x19), .c(new_n49_), .d(x17), .O(new_n97_));
  nor2   g054(.a(new_n97_), .b(new_n87_), .O(z08));
  inv1   g055(.a(x21), .O(new_n99_));
  nand4  g056(.a(x12), .b(new_n70_), .c(x02), .d(new_n44_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nor2   g059(.a(x14), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g061(.a(x19), .O(new_n105_));
  inv1   g062(.a(x20), .O(new_n106_));
  nor2   g063(.a(new_n49_), .b(new_n44_), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  oai21  g065(.a(new_n104_), .b(new_n100_), .c(new_n108_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n87_), .c(new_n99_), .d(x08), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(x00), .O(z09));
  nor2   g068(.a(new_n87_), .b(new_n99_), .O(new_n112_));
  nand4  g069(.a(new_n112_), .b(new_n66_), .c(new_n106_), .d(x01), .O(new_n113_));
  nand2  g070(.a(x08), .b(x02), .O(new_n114_));
  inv1   g071(.a(x13), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(x12), .c(new_n70_), .O(new_n116_));
  nor3   g073(.a(new_n116_), .b(new_n114_), .c(x01), .O(new_n117_));
  nor3   g074(.a(new_n102_), .b(new_n101_), .c(x14), .O(new_n118_));
  nor2   g075(.a(x22), .b(x21), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(new_n118_), .c(new_n117_), .d(x20), .O(new_n120_));
  aoi21  g077(.a(new_n120_), .b(new_n113_), .c(x00), .O(z10));
  nand4  g078(.a(new_n107_), .b(x21), .c(new_n106_), .d(new_n105_), .O(new_n122_));
  inv1   g079(.a(new_n116_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(x02), .c(new_n44_), .O(new_n124_));
  nor2   g081(.a(new_n101_), .b(x14), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n99_), .c(x20), .d(new_n102_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n124_), .c(new_n122_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n69_), .O(new_n128_));
  aoi21  g085(.a(new_n128_), .b(x08), .c(x22), .O(z11));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n105_), .b(x17), .c(new_n45_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n44_), .c(new_n69_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(new_n131_), .c(x24), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n72_), .O(z12));
  nand4  g095(.a(new_n51_), .b(x17), .c(new_n45_), .d(new_n44_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x00), .O(z13));
  nand4  g097(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n69_), .O(new_n141_));
  nand4  g098(.a(new_n105_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n142_));
  oai21  g099(.a(new_n142_), .b(new_n141_), .c(new_n72_), .O(z14));
  oai21  g100(.a(x10), .b(new_n44_), .c(x02), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(x00), .O(new_n145_));
  nand3  g102(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n146_));
  aoi21  g103(.a(new_n146_), .b(new_n145_), .c(new_n50_), .O(z15));
  oai21  g104(.a(new_n44_), .b(x00), .c(new_n72_), .O(z16));
  nor4   g105(.a(new_n50_), .b(new_n45_), .c(x01), .d(x00), .O(z17));
endmodule


