// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n132_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z02));
  nand3  g15(.a(new_n46_), .b(x02), .c(x00), .O(new_n59_));
  nand3  g16(.a(x12), .b(x11), .c(x10), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n59_), .c(x19), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nor2   g20(.a(x02), .b(x00), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n63_), .c(x18), .d(new_n44_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n62_), .O(z03));
  inv1   g23(.a(x01), .O(new_n67_));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  inv1   g26(.a(new_n59_), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x19), .c(new_n67_), .O(z04));
  nand3  g29(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  nor4   g30(.a(new_n73_), .b(new_n63_), .c(new_n56_), .d(new_n46_), .O(z05));
  inv1   g31(.a(new_n73_), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(x11), .c(x10), .d(new_n46_), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n63_), .O(z06));
  nand2  g34(.a(x10), .b(new_n46_), .O(new_n78_));
  nand3  g35(.a(x19), .b(new_n69_), .c(x11), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z07));
  inv1   g39(.a(x08), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  inv1   g41(.a(x06), .O(new_n85_));
  inv1   g42(.a(x00), .O(new_n86_));
  inv1   g43(.a(x03), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x02), .c(new_n67_), .d(new_n86_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n85_), .c(new_n84_), .d(x04), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(x07), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n45_), .c(x17), .d(new_n83_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n63_), .O(z08));
  inv1   g50(.a(x16), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  nand4  g54(.a(new_n68_), .b(x02), .c(new_n67_), .d(new_n86_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n97_), .c(new_n96_), .d(x12), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(x15), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n95_), .c(x20), .d(new_n94_), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(x22), .O(z09));
  inv1   g60(.a(x15), .O(new_n104_));
  nor2   g61(.a(new_n100_), .b(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n95_), .c(x20), .d(x16), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(x22), .O(z10));
  nand4  g64(.a(new_n105_), .b(new_n95_), .c(x20), .d(new_n94_), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(x22), .O(z11));
  nand2  g66(.a(x10), .b(x02), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(x01), .c(x00), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  inv1   g69(.a(x02), .O(new_n113_));
  nand3  g70(.a(new_n63_), .b(x17), .c(new_n113_), .O(new_n114_));
  oai21  g71(.a(new_n112_), .b(x01), .c(new_n114_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n86_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n111_), .c(x24), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x09), .O(new_n118_));
  oai21  g75(.a(x19), .b(new_n67_), .c(new_n118_), .O(z12));
  nand2  g76(.a(new_n47_), .b(x17), .O(new_n120_));
  nor2   g77(.a(new_n120_), .b(x19), .O(z13));
  nor3   g78(.a(x09), .b(x02), .c(x00), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n67_), .c(x19), .O(z14));
  aoi21  g81(.a(new_n63_), .b(x01), .c(new_n86_), .O(new_n125_));
  nor2   g82(.a(new_n63_), .b(x01), .O(new_n126_));
  oai21  g83(.a(new_n126_), .b(new_n125_), .c(new_n113_), .O(new_n127_));
  nand4  g84(.a(x19), .b(new_n56_), .c(x01), .d(x00), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n127_), .O(z15));
  nand3  g86(.a(x19), .b(x01), .c(new_n86_), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z16));
  nand3  g88(.a(x02), .b(new_n67_), .c(new_n86_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z17));
endmodule


