// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n131_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor3   g02(.a(x02), .b(x01), .c(x00), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(x10), .d(new_n44_), .O(new_n47_));
  nor3   g04(.a(new_n47_), .b(x19), .c(x18), .O(z00));
  nand2  g05(.a(x21), .b(x01), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nor2   g09(.a(x19), .b(x18), .O(new_n53_));
  nand3  g10(.a(new_n53_), .b(new_n45_), .c(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n46_), .b(new_n45_), .c(new_n56_), .d(x09), .O(new_n57_));
  nor3   g14(.a(new_n57_), .b(x19), .c(x18), .O(z02));
  inv1   g15(.a(x21), .O(new_n59_));
  nand3  g16(.a(new_n44_), .b(x02), .c(x00), .O(new_n60_));
  nand3  g17(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x01), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nor3   g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(x18), .d(new_n45_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x10), .c(new_n44_), .O(new_n70_));
  nor4   g27(.a(new_n70_), .b(x21), .c(x12), .d(x11), .O(z04));
  nor4   g28(.a(new_n68_), .b(x21), .c(new_n56_), .d(new_n44_), .O(z05));
  inv1   g29(.a(x11), .O(new_n73_));
  nor3   g30(.a(new_n70_), .b(x21), .c(new_n73_), .O(z06));
  nand2  g31(.a(x10), .b(new_n44_), .O(new_n75_));
  inv1   g32(.a(x12), .O(new_n76_));
  nand3  g33(.a(new_n59_), .b(new_n76_), .c(x11), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n75_), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z07));
  inv1   g37(.a(x00), .O(new_n81_));
  inv1   g38(.a(x01), .O(new_n82_));
  nand3  g39(.a(x02), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x04), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n83_), .c(x03), .O(new_n86_));
  nor3   g43(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  nor2   g44(.a(new_n64_), .b(x18), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x17), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(new_n49_), .O(z08));
  inv1   g47(.a(x22), .O(new_n91_));
  nand4  g48(.a(x12), .b(new_n73_), .c(x02), .d(new_n82_), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nor2   g51(.a(x14), .b(x13), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(x20), .c(new_n94_), .d(new_n93_), .O(new_n96_));
  inv1   g53(.a(x20), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n64_), .c(x18), .d(x01), .O(new_n98_));
  oai21  g55(.a(new_n96_), .b(new_n92_), .c(new_n98_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n91_), .c(new_n59_), .d(new_n81_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z09));
  inv1   g58(.a(new_n83_), .O(new_n102_));
  nor2   g59(.a(x13), .b(new_n76_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n73_), .O(new_n104_));
  inv1   g61(.a(x14), .O(new_n105_));
  nand3  g62(.a(new_n91_), .b(new_n59_), .c(x20), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(x16), .c(x15), .d(new_n105_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(new_n104_), .c(new_n49_), .O(z10));
  nand4  g66(.a(new_n107_), .b(new_n94_), .c(x15), .d(new_n105_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n104_), .c(new_n49_), .O(z11));
  inv1   g68(.a(x24), .O(new_n112_));
  nand2  g69(.a(x10), .b(x02), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(x01), .c(x00), .O(new_n114_));
  oai21  g71(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g72(.a(new_n64_), .b(x17), .c(new_n50_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n82_), .c(new_n81_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n114_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n112_), .c(x09), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n49_), .O(z12));
  nand2  g78(.a(new_n46_), .b(x17), .O(new_n122_));
  nor2   g79(.a(new_n122_), .b(x19), .O(z13));
  nor2   g80(.a(x09), .b(x02), .O(new_n124_));
  nor2   g81(.a(x17), .b(x10), .O(new_n125_));
  nand4  g82(.a(new_n125_), .b(new_n124_), .c(new_n53_), .d(new_n51_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n49_), .O(z14));
  aoi22  g84(.a(new_n49_), .b(x00), .c(x19), .d(new_n82_), .O(new_n128_));
  nand4  g85(.a(new_n59_), .b(new_n56_), .c(x01), .d(x00), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(x02), .c(new_n129_), .O(z15));
  nand3  g87(.a(new_n59_), .b(x01), .c(new_n81_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z16));
  nand2  g89(.a(new_n83_), .b(new_n49_), .O(z17));
endmodule


