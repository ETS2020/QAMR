// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n143_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  nand4  g005(.a(x08), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nor2   g006(.a(new_n49_), .b(x09), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  inv1   g009(.a(x09), .O(new_n53_));
  nor2   g010(.a(new_n53_), .b(x02), .O(new_n54_));
  inv1   g011(.a(x10), .O(new_n55_));
  nor2   g012(.a(x17), .b(new_n55_), .O(new_n56_));
  nor2   g013(.a(x19), .b(x18), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n56_), .c(new_n54_), .d(new_n46_), .O(new_n58_));
  aoi21  g015(.a(new_n58_), .b(x08), .c(x01), .O(z01));
  nor2   g016(.a(new_n49_), .b(new_n53_), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(new_n55_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x19), .O(z02));
  nand2  g019(.a(new_n48_), .b(new_n46_), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nand3  g021(.a(new_n64_), .b(x18), .c(new_n44_), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n63_), .c(x08), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n47_), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g025(.a(new_n55_), .b(x09), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g027(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  nor2   g028(.a(x08), .b(x01), .O(z08));
  inv1   g029(.a(z08), .O(new_n73_));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand3  g032(.a(new_n69_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n68_), .c(new_n73_), .O(z04));
  nand3  g034(.a(x02), .b(x01), .c(x00), .O(new_n78_));
  nor3   g035(.a(new_n78_), .b(new_n55_), .c(new_n53_), .O(z05));
  inv1   g036(.a(new_n78_), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(x11), .c(x10), .d(new_n53_), .O(new_n81_));
  inv1   g038(.a(new_n81_), .O(z06));
  nand3  g039(.a(new_n69_), .b(new_n75_), .c(x11), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n73_), .O(z07));
  inv1   g043(.a(x21), .O(new_n87_));
  inv1   g044(.a(x22), .O(new_n88_));
  nor2   g045(.a(new_n48_), .b(x01), .O(new_n89_));
  nand3  g046(.a(new_n89_), .b(x12), .c(new_n74_), .O(new_n90_));
  inv1   g047(.a(x15), .O(new_n91_));
  inv1   g048(.a(x16), .O(new_n92_));
  nor2   g049(.a(x14), .b(x13), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(x20), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  inv1   g051(.a(x20), .O(new_n95_));
  nor2   g052(.a(new_n45_), .b(new_n47_), .O(new_n96_));
  nand3  g053(.a(new_n96_), .b(new_n95_), .c(new_n64_), .O(new_n97_));
  oai21  g054(.a(new_n94_), .b(new_n90_), .c(new_n97_), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(new_n88_), .c(new_n87_), .d(new_n46_), .O(new_n99_));
  nand2  g056(.a(new_n99_), .b(new_n73_), .O(z09));
  nor3   g057(.a(new_n88_), .b(new_n87_), .c(x20), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(new_n64_), .c(x18), .d(x01), .O(new_n102_));
  inv1   g059(.a(x08), .O(new_n103_));
  nor3   g060(.a(new_n103_), .b(new_n48_), .c(x01), .O(new_n104_));
  inv1   g061(.a(x13), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(x12), .c(new_n74_), .O(new_n106_));
  inv1   g063(.a(new_n106_), .O(new_n107_));
  nor3   g064(.a(new_n92_), .b(new_n91_), .c(x14), .O(new_n108_));
  nand3  g065(.a(new_n88_), .b(new_n87_), .c(x20), .O(new_n109_));
  inv1   g066(.a(new_n109_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n108_), .c(new_n107_), .d(new_n104_), .O(new_n111_));
  aoi21  g068(.a(new_n111_), .b(new_n102_), .c(x00), .O(z10));
  nand4  g069(.a(new_n96_), .b(x21), .c(new_n95_), .d(new_n64_), .O(new_n113_));
  nand3  g070(.a(x12), .b(new_n74_), .c(x08), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  inv1   g072(.a(x14), .O(new_n116_));
  nand3  g073(.a(x15), .b(new_n116_), .c(new_n105_), .O(new_n117_));
  inv1   g074(.a(new_n117_), .O(new_n118_));
  nand3  g075(.a(new_n87_), .b(x20), .c(new_n92_), .O(new_n119_));
  inv1   g076(.a(new_n119_), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n118_), .c(new_n115_), .d(new_n89_), .O(new_n121_));
  nand2  g078(.a(new_n121_), .b(new_n113_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n88_), .c(new_n46_), .O(new_n123_));
  inv1   g080(.a(new_n123_), .O(z11));
  inv1   g081(.a(x24), .O(new_n125_));
  nand2  g082(.a(x10), .b(x02), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  oai21  g084(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g085(.a(new_n64_), .b(x17), .c(new_n48_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand4  g087(.a(new_n130_), .b(x08), .c(new_n47_), .d(new_n46_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n127_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n125_), .c(x09), .O(new_n133_));
  inv1   g090(.a(new_n133_), .O(z12));
  nor3   g091(.a(new_n49_), .b(x19), .c(new_n44_), .O(z13));
  nor3   g092(.a(x09), .b(x02), .c(x00), .O(new_n136_));
  nand4  g093(.a(new_n136_), .b(new_n57_), .c(new_n44_), .d(new_n55_), .O(new_n137_));
  aoi21  g094(.a(new_n137_), .b(x08), .c(x01), .O(z14));
  aoi21  g095(.a(new_n55_), .b(x01), .c(new_n48_), .O(new_n139_));
  aoi21  g096(.a(x19), .b(new_n48_), .c(new_n103_), .O(new_n140_));
  oai22  g097(.a(new_n140_), .b(x01), .c(new_n139_), .d(new_n46_), .O(z15));
  nor2   g098(.a(new_n47_), .b(x00), .O(z16));
  nand4  g099(.a(x08), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n143_));
  inv1   g100(.a(new_n143_), .O(z17));
endmodule


