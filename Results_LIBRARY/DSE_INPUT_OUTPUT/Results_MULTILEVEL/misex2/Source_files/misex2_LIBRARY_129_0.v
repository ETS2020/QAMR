// Benchmark "FAU" written by ABC on Fri Aug 14 02:10:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n125_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(x14), .c(x01), .O(z00));
  nand4  g10(.a(new_n51_), .b(x09), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(x14), .c(x01), .O(z01));
  inv1   g12(.a(x01), .O(new_n56_));
  nand4  g13(.a(x09), .b(new_n45_), .c(new_n56_), .d(new_n44_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x10), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n48_), .c(new_n47_), .d(x14), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n61_));
  nand3  g18(.a(new_n49_), .b(x18), .c(new_n47_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(x14), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n56_), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n66_));
  oai21  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z03));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x10), .c(new_n46_), .O(new_n70_));
  nor3   g27(.a(new_n70_), .b(x12), .c(x11), .O(z04));
  inv1   g28(.a(x10), .O(new_n72_));
  nor3   g29(.a(new_n68_), .b(new_n72_), .c(new_n46_), .O(z05));
  inv1   g30(.a(x14), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n56_), .O(new_n75_));
  nand3  g32(.a(x11), .b(x10), .c(new_n46_), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n65_), .c(new_n75_), .O(z06));
  nand2  g34(.a(new_n46_), .b(x01), .O(new_n78_));
  inv1   g35(.a(x12), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x11), .c(x10), .O(new_n80_));
  oai22  g37(.a(new_n80_), .b(new_n78_), .c(new_n74_), .d(x01), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(z07));
  nand2  g40(.a(x02), .b(new_n44_), .O(new_n84_));
  inv1   g41(.a(x05), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(x04), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n84_), .c(x03), .O(new_n87_));
  nor3   g44(.a(x08), .b(x07), .c(x06), .O(new_n88_));
  nand3  g45(.a(x19), .b(new_n48_), .c(x17), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(x14), .c(x01), .O(z08));
  nand3  g49(.a(x18), .b(x01), .c(new_n44_), .O(new_n93_));
  inv1   g50(.a(x20), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(new_n49_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n93_), .c(new_n75_), .O(z09));
  nand4  g55(.a(new_n49_), .b(x18), .c(x01), .d(new_n44_), .O(new_n99_));
  nor3   g56(.a(new_n99_), .b(new_n95_), .c(x20), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(x22), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(z10));
  nand2  g59(.a(new_n100_), .b(new_n96_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(z11));
  inv1   g61(.a(x24), .O(new_n105_));
  nand2  g62(.a(x10), .b(x02), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(x01), .c(x00), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n49_), .b(x17), .c(new_n45_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n56_), .c(new_n44_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n107_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n105_), .c(x09), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n75_), .O(z12));
  nand3  g71(.a(new_n45_), .b(new_n56_), .c(new_n44_), .O(new_n115_));
  nor4   g72(.a(new_n115_), .b(x19), .c(new_n47_), .d(new_n74_), .O(z13));
  nand4  g73(.a(new_n46_), .b(new_n45_), .c(new_n56_), .d(new_n44_), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(x10), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n48_), .c(new_n47_), .d(x14), .O(new_n119_));
  nor2   g76(.a(new_n119_), .b(x19), .O(z14));
  aoi21  g77(.a(new_n72_), .b(x01), .c(new_n45_), .O(new_n121_));
  aoi21  g78(.a(x19), .b(new_n45_), .c(new_n74_), .O(new_n122_));
  oai22  g79(.a(new_n122_), .b(x01), .c(new_n121_), .d(new_n44_), .O(z15));
  oai21  g80(.a(new_n56_), .b(x00), .c(new_n75_), .O(z16));
  nand4  g81(.a(x14), .b(x02), .c(new_n56_), .d(new_n44_), .O(new_n125_));
  inv1   g82(.a(new_n125_), .O(z17));
endmodule


