// Benchmark "FAU" written by ABC on Thu Aug 13 18:12:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n140_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n178_, new_n179_, new_n180_, new_n182_,
    new_n183_, new_n184_, new_n186_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n209_, new_n210_, new_n211_,
    new_n212_, new_n214_, new_n215_, new_n216_, new_n217_, new_n219_,
    new_n220_, new_n221_, new_n222_, new_n224_, new_n225_, new_n226_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n233_, new_n234_,
    new_n235_, new_n236_;
  inv1   g000(.a(x10), .O(new_n84_));
  inv1   g001(.a(x11), .O(new_n85_));
  nand2  g002(.a(x07), .b(x04), .O(new_n86_));
  oai21  g003(.a(new_n85_), .b(x07), .c(new_n86_), .O(new_n87_));
  nand3  g004(.a(new_n87_), .b(x26), .c(new_n84_), .O(new_n88_));
  inv1   g005(.a(new_n88_), .O(z00));
  nand2  g006(.a(x07), .b(x05), .O(new_n90_));
  inv1   g007(.a(x07), .O(new_n91_));
  inv1   g008(.a(x26), .O(new_n92_));
  aoi21  g009(.a(x12), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  aoi21  g010(.a(new_n93_), .b(new_n90_), .c(x10), .O(z01));
  inv1   g011(.a(x13), .O(new_n95_));
  nand2  g012(.a(x07), .b(x06), .O(new_n96_));
  oai21  g013(.a(new_n95_), .b(x07), .c(new_n96_), .O(new_n97_));
  nand3  g014(.a(new_n97_), .b(x26), .c(new_n84_), .O(new_n98_));
  inv1   g015(.a(new_n98_), .O(z02));
  nand2  g016(.a(x07), .b(x01), .O(new_n100_));
  aoi21  g017(.a(x14), .b(new_n91_), .c(new_n92_), .O(new_n101_));
  aoi21  g018(.a(new_n101_), .b(new_n100_), .c(x10), .O(z03));
  inv1   g019(.a(x15), .O(new_n103_));
  nand2  g020(.a(x07), .b(x02), .O(new_n104_));
  oai21  g021(.a(new_n103_), .b(x07), .c(new_n104_), .O(new_n105_));
  nand3  g022(.a(new_n105_), .b(x26), .c(new_n84_), .O(new_n106_));
  inv1   g023(.a(new_n106_), .O(z04));
  nand2  g024(.a(x07), .b(x03), .O(new_n108_));
  aoi21  g025(.a(x16), .b(new_n91_), .c(new_n92_), .O(new_n109_));
  aoi21  g026(.a(new_n109_), .b(new_n108_), .c(x10), .O(z05));
  inv1   g027(.a(x17), .O(new_n111_));
  nand2  g028(.a(x18), .b(x08), .O(new_n112_));
  oai21  g029(.a(new_n111_), .b(x08), .c(new_n112_), .O(new_n113_));
  nand3  g030(.a(new_n113_), .b(x26), .c(new_n84_), .O(new_n114_));
  inv1   g031(.a(new_n114_), .O(z06));
  inv1   g032(.a(x08), .O(new_n116_));
  nand2  g033(.a(x18), .b(new_n116_), .O(new_n117_));
  aoi21  g034(.a(x19), .b(x08), .c(new_n92_), .O(new_n118_));
  aoi21  g035(.a(new_n118_), .b(new_n117_), .c(x10), .O(z07));
  nand2  g036(.a(x19), .b(new_n116_), .O(new_n120_));
  aoi21  g037(.a(x20), .b(x08), .c(new_n92_), .O(new_n121_));
  aoi21  g038(.a(new_n121_), .b(new_n120_), .c(x10), .O(z08));
  inv1   g039(.a(x20), .O(new_n123_));
  nand2  g040(.a(x21), .b(x08), .O(new_n124_));
  oai21  g041(.a(new_n123_), .b(x08), .c(new_n124_), .O(new_n125_));
  nand3  g042(.a(new_n125_), .b(x26), .c(new_n84_), .O(new_n126_));
  inv1   g043(.a(new_n126_), .O(z09));
  nand2  g044(.a(x21), .b(new_n116_), .O(new_n128_));
  aoi21  g045(.a(x22), .b(x08), .c(new_n92_), .O(new_n129_));
  aoi21  g046(.a(new_n129_), .b(new_n128_), .c(x10), .O(z10));
  nand2  g047(.a(x22), .b(new_n116_), .O(new_n131_));
  aoi21  g048(.a(x23), .b(x08), .c(new_n92_), .O(new_n132_));
  aoi21  g049(.a(new_n132_), .b(new_n131_), .c(x10), .O(z11));
  nand2  g050(.a(x23), .b(new_n116_), .O(new_n134_));
  aoi21  g051(.a(x24), .b(x08), .c(new_n92_), .O(new_n135_));
  aoi21  g052(.a(new_n135_), .b(new_n134_), .c(x10), .O(z12));
  nand2  g053(.a(x24), .b(new_n116_), .O(new_n137_));
  aoi21  g054(.a(x25), .b(x08), .c(new_n92_), .O(new_n138_));
  aoi21  g055(.a(new_n138_), .b(new_n137_), .c(x10), .O(z13));
  nor2   g056(.a(new_n92_), .b(x25), .O(new_n140_));
  aoi21  g057(.a(new_n140_), .b(new_n116_), .c(x10), .O(z14));
  oai21  g058(.a(x27), .b(new_n116_), .c(x26), .O(new_n142_));
  nor2   g059(.a(new_n142_), .b(x10), .O(z15));
  inv1   g060(.a(x27), .O(new_n144_));
  nand2  g061(.a(x28), .b(x08), .O(new_n145_));
  oai21  g062(.a(new_n144_), .b(x08), .c(new_n145_), .O(new_n146_));
  nand3  g063(.a(new_n146_), .b(x26), .c(new_n84_), .O(new_n147_));
  inv1   g064(.a(new_n147_), .O(z16));
  nand2  g065(.a(x28), .b(new_n116_), .O(new_n149_));
  nand2  g066(.a(x29), .b(x08), .O(new_n150_));
  nand3  g067(.a(new_n150_), .b(new_n149_), .c(x26), .O(new_n151_));
  and2   g068(.a(new_n151_), .b(new_n84_), .O(z17));
  nand2  g069(.a(x29), .b(new_n116_), .O(new_n153_));
  nand2  g070(.a(x30), .b(x08), .O(new_n154_));
  nand3  g071(.a(new_n154_), .b(new_n153_), .c(x26), .O(new_n155_));
  and2   g072(.a(new_n155_), .b(new_n84_), .O(z18));
  nand2  g073(.a(x08), .b(x00), .O(new_n157_));
  aoi21  g074(.a(x30), .b(new_n116_), .c(new_n92_), .O(new_n158_));
  aoi21  g075(.a(new_n158_), .b(new_n157_), .c(x10), .O(z19));
  inv1   g076(.a(x31), .O(new_n160_));
  nand2  g077(.a(x32), .b(x09), .O(new_n161_));
  oai21  g078(.a(new_n160_), .b(x09), .c(new_n161_), .O(new_n162_));
  nand3  g079(.a(new_n162_), .b(x26), .c(new_n84_), .O(new_n163_));
  inv1   g080(.a(new_n163_), .O(z20));
  inv1   g081(.a(x09), .O(new_n165_));
  nand2  g082(.a(x32), .b(new_n165_), .O(new_n166_));
  nand2  g083(.a(x33), .b(x09), .O(new_n167_));
  nand3  g084(.a(new_n167_), .b(new_n166_), .c(x26), .O(new_n168_));
  and2   g085(.a(new_n168_), .b(new_n84_), .O(z21));
  nand2  g086(.a(x33), .b(new_n165_), .O(new_n170_));
  nand2  g087(.a(x34), .b(x09), .O(new_n171_));
  nand3  g088(.a(new_n171_), .b(new_n170_), .c(x26), .O(new_n172_));
  and2   g089(.a(new_n172_), .b(new_n84_), .O(z22));
  nand2  g090(.a(x34), .b(new_n165_), .O(new_n174_));
  nand2  g091(.a(x35), .b(x09), .O(new_n175_));
  nand3  g092(.a(new_n175_), .b(new_n174_), .c(x26), .O(new_n176_));
  and2   g093(.a(new_n176_), .b(new_n84_), .O(z23));
  nand2  g094(.a(x35), .b(new_n165_), .O(new_n178_));
  nand2  g095(.a(x36), .b(x09), .O(new_n179_));
  nand3  g096(.a(new_n179_), .b(new_n178_), .c(x26), .O(new_n180_));
  and2   g097(.a(new_n180_), .b(new_n84_), .O(z24));
  nand2  g098(.a(x36), .b(new_n165_), .O(new_n182_));
  nand2  g099(.a(x37), .b(x09), .O(new_n183_));
  nand3  g100(.a(new_n183_), .b(new_n182_), .c(x26), .O(new_n184_));
  and2   g101(.a(new_n184_), .b(new_n84_), .O(z25));
  nand2  g102(.a(x37), .b(new_n165_), .O(new_n186_));
  nand2  g103(.a(x38), .b(x09), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n186_), .c(x26), .O(new_n188_));
  and2   g105(.a(new_n188_), .b(new_n84_), .O(z26));
  nand2  g106(.a(x14), .b(x00), .O(new_n190_));
  inv1   g107(.a(x14), .O(new_n191_));
  nand2  g108(.a(x39), .b(new_n191_), .O(new_n192_));
  aoi21  g109(.a(new_n192_), .b(new_n190_), .c(new_n165_), .O(new_n193_));
  inv1   g110(.a(x38), .O(new_n194_));
  nor2   g111(.a(new_n194_), .b(x09), .O(new_n195_));
  oai21  g112(.a(new_n195_), .b(new_n193_), .c(x26), .O(new_n196_));
  nor2   g113(.a(new_n196_), .b(x10), .O(z27));
  nand2  g114(.a(new_n191_), .b(x09), .O(new_n198_));
  nand2  g115(.a(new_n198_), .b(x39), .O(new_n199_));
  inv1   g116(.a(x40), .O(new_n200_));
  nor2   g117(.a(new_n200_), .b(x14), .O(new_n201_));
  aoi21  g118(.a(new_n201_), .b(x09), .c(new_n92_), .O(new_n202_));
  aoi21  g119(.a(new_n202_), .b(new_n199_), .c(x10), .O(z28));
  nand2  g120(.a(new_n198_), .b(x40), .O(new_n204_));
  inv1   g121(.a(x41), .O(new_n205_));
  nor2   g122(.a(new_n205_), .b(x14), .O(new_n206_));
  aoi21  g123(.a(new_n206_), .b(x09), .c(new_n92_), .O(new_n207_));
  aoi21  g124(.a(new_n207_), .b(new_n204_), .c(x10), .O(z29));
  nor2   g125(.a(x14), .b(new_n165_), .O(new_n209_));
  nand3  g126(.a(x42), .b(new_n191_), .c(x09), .O(new_n210_));
  oai21  g127(.a(new_n209_), .b(new_n205_), .c(new_n210_), .O(new_n211_));
  nand3  g128(.a(new_n211_), .b(x26), .c(new_n84_), .O(new_n212_));
  inv1   g129(.a(new_n212_), .O(z30));
  nand2  g130(.a(new_n198_), .b(x42), .O(new_n214_));
  nand3  g131(.a(x43), .b(new_n191_), .c(x09), .O(new_n215_));
  nand2  g132(.a(new_n215_), .b(new_n214_), .O(new_n216_));
  nand3  g133(.a(new_n216_), .b(x26), .c(new_n84_), .O(new_n217_));
  inv1   g134(.a(new_n217_), .O(z31));
  nand2  g135(.a(new_n198_), .b(x43), .O(new_n219_));
  inv1   g136(.a(x44), .O(new_n220_));
  nor2   g137(.a(new_n220_), .b(x14), .O(new_n221_));
  aoi21  g138(.a(new_n221_), .b(x09), .c(new_n92_), .O(new_n222_));
  aoi21  g139(.a(new_n222_), .b(new_n219_), .c(x10), .O(z32));
  nand3  g140(.a(x45), .b(new_n191_), .c(x09), .O(new_n224_));
  oai21  g141(.a(new_n209_), .b(new_n220_), .c(new_n224_), .O(new_n225_));
  nand3  g142(.a(new_n225_), .b(x26), .c(new_n84_), .O(new_n226_));
  inv1   g143(.a(new_n226_), .O(z33));
  nand2  g144(.a(new_n198_), .b(x45), .O(new_n228_));
  nand3  g145(.a(x46), .b(new_n191_), .c(x09), .O(new_n229_));
  nand2  g146(.a(new_n229_), .b(new_n228_), .O(new_n230_));
  nand3  g147(.a(new_n230_), .b(x26), .c(new_n84_), .O(new_n231_));
  inv1   g148(.a(new_n231_), .O(z34));
  nand2  g149(.a(new_n198_), .b(x46), .O(new_n233_));
  nand2  g150(.a(new_n209_), .b(x00), .O(new_n234_));
  nand2  g151(.a(new_n234_), .b(new_n233_), .O(new_n235_));
  nand3  g152(.a(new_n235_), .b(x26), .c(new_n84_), .O(new_n236_));
  inv1   g153(.a(new_n236_), .O(z35));
endmodule


