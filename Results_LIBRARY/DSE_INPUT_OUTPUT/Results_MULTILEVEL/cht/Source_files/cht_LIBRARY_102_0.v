// Benchmark "FAU" written by ABC on Thu Aug 13 18:12:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n124_, new_n126_, new_n128_,
    new_n130_, new_n132_, new_n134_, new_n136_, new_n138_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n148_,
    new_n149_, new_n150_, new_n152_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n217_, new_n218_;
  inv1   g000(.a(x10), .O(new_n84_));
  inv1   g001(.a(x11), .O(new_n85_));
  nand2  g002(.a(x07), .b(x04), .O(new_n86_));
  oai21  g003(.a(new_n85_), .b(x07), .c(new_n86_), .O(new_n87_));
  nand3  g004(.a(new_n87_), .b(new_n84_), .c(x08), .O(new_n88_));
  inv1   g005(.a(new_n88_), .O(z00));
  inv1   g006(.a(x12), .O(new_n90_));
  nand2  g007(.a(x07), .b(x05), .O(new_n91_));
  oai21  g008(.a(new_n90_), .b(x07), .c(new_n91_), .O(new_n92_));
  nand3  g009(.a(new_n92_), .b(new_n84_), .c(x08), .O(new_n93_));
  inv1   g010(.a(new_n93_), .O(z01));
  inv1   g011(.a(x13), .O(new_n95_));
  nand2  g012(.a(x07), .b(x06), .O(new_n96_));
  oai21  g013(.a(new_n95_), .b(x07), .c(new_n96_), .O(new_n97_));
  nand3  g014(.a(new_n97_), .b(new_n84_), .c(x08), .O(new_n98_));
  inv1   g015(.a(new_n98_), .O(z02));
  nand2  g016(.a(x07), .b(x01), .O(new_n100_));
  inv1   g017(.a(x07), .O(new_n101_));
  inv1   g018(.a(x08), .O(new_n102_));
  aoi21  g019(.a(x14), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  aoi21  g020(.a(new_n103_), .b(new_n100_), .c(x10), .O(z03));
  nand2  g021(.a(x07), .b(x02), .O(new_n105_));
  aoi21  g022(.a(x15), .b(new_n101_), .c(new_n102_), .O(new_n106_));
  aoi21  g023(.a(new_n106_), .b(new_n105_), .c(x10), .O(z04));
  inv1   g024(.a(x16), .O(new_n108_));
  nand2  g025(.a(x07), .b(x03), .O(new_n109_));
  oai21  g026(.a(new_n108_), .b(x07), .c(new_n109_), .O(new_n110_));
  nand3  g027(.a(new_n110_), .b(new_n84_), .c(x08), .O(new_n111_));
  inv1   g028(.a(new_n111_), .O(z05));
  inv1   g029(.a(x18), .O(new_n113_));
  aoi21  g030(.a(new_n113_), .b(x08), .c(x10), .O(z06));
  inv1   g031(.a(x19), .O(new_n115_));
  aoi21  g032(.a(new_n115_), .b(x08), .c(x10), .O(z07));
  inv1   g033(.a(x20), .O(new_n117_));
  aoi21  g034(.a(new_n117_), .b(x08), .c(x10), .O(z08));
  nor2   g035(.a(x10), .b(new_n102_), .O(new_n119_));
  nand2  g036(.a(new_n119_), .b(x21), .O(new_n120_));
  inv1   g037(.a(new_n120_), .O(z09));
  nand2  g038(.a(new_n119_), .b(x22), .O(new_n122_));
  inv1   g039(.a(new_n122_), .O(z10));
  inv1   g040(.a(x23), .O(new_n124_));
  aoi21  g041(.a(new_n124_), .b(x08), .c(x10), .O(z11));
  inv1   g042(.a(x24), .O(new_n126_));
  aoi21  g043(.a(new_n126_), .b(x08), .c(x10), .O(z12));
  nand2  g044(.a(new_n119_), .b(x25), .O(new_n128_));
  inv1   g045(.a(new_n128_), .O(z13));
  inv1   g046(.a(x26), .O(new_n130_));
  aoi21  g047(.a(new_n130_), .b(x08), .c(x10), .O(z14));
  inv1   g048(.a(x27), .O(new_n132_));
  aoi21  g049(.a(new_n132_), .b(x08), .c(x10), .O(z15));
  nand2  g050(.a(new_n119_), .b(x28), .O(new_n134_));
  inv1   g051(.a(new_n134_), .O(z16));
  nand2  g052(.a(new_n119_), .b(x29), .O(new_n136_));
  inv1   g053(.a(new_n136_), .O(z17));
  nand2  g054(.a(new_n119_), .b(x30), .O(new_n138_));
  inv1   g055(.a(new_n138_), .O(z18));
  nand2  g056(.a(x08), .b(x00), .O(new_n140_));
  nor2   g057(.a(new_n140_), .b(x10), .O(z19));
  inv1   g058(.a(x09), .O(new_n142_));
  inv1   g059(.a(x32), .O(new_n143_));
  nand2  g060(.a(x31), .b(new_n142_), .O(new_n144_));
  oai21  g061(.a(new_n143_), .b(new_n142_), .c(new_n144_), .O(new_n145_));
  nand3  g062(.a(new_n145_), .b(new_n84_), .c(x08), .O(new_n146_));
  inv1   g063(.a(new_n146_), .O(z20));
  nand2  g064(.a(x33), .b(x09), .O(new_n148_));
  oai21  g065(.a(new_n143_), .b(x09), .c(new_n148_), .O(new_n149_));
  nand3  g066(.a(new_n149_), .b(new_n84_), .c(x08), .O(new_n150_));
  inv1   g067(.a(new_n150_), .O(z21));
  nand2  g068(.a(x33), .b(new_n142_), .O(new_n152_));
  nand2  g069(.a(x34), .b(x09), .O(new_n153_));
  nand3  g070(.a(new_n153_), .b(new_n152_), .c(x08), .O(new_n154_));
  and2   g071(.a(new_n154_), .b(new_n84_), .O(z22));
  inv1   g072(.a(x35), .O(new_n156_));
  nand2  g073(.a(x34), .b(new_n142_), .O(new_n157_));
  oai21  g074(.a(new_n156_), .b(new_n142_), .c(new_n157_), .O(new_n158_));
  nand3  g075(.a(new_n158_), .b(new_n84_), .c(x08), .O(new_n159_));
  inv1   g076(.a(new_n159_), .O(z23));
  nand2  g077(.a(x36), .b(x09), .O(new_n161_));
  oai21  g078(.a(new_n156_), .b(x09), .c(new_n161_), .O(new_n162_));
  nand3  g079(.a(new_n162_), .b(new_n84_), .c(x08), .O(new_n163_));
  inv1   g080(.a(new_n163_), .O(z24));
  nand2  g081(.a(x36), .b(new_n142_), .O(new_n165_));
  nand2  g082(.a(x37), .b(x09), .O(new_n166_));
  nand3  g083(.a(new_n166_), .b(new_n165_), .c(x08), .O(new_n167_));
  and2   g084(.a(new_n167_), .b(new_n84_), .O(z25));
  inv1   g085(.a(x38), .O(new_n169_));
  nand2  g086(.a(x37), .b(new_n142_), .O(new_n170_));
  oai21  g087(.a(new_n169_), .b(new_n142_), .c(new_n170_), .O(new_n171_));
  nand3  g088(.a(new_n171_), .b(new_n84_), .c(x08), .O(new_n172_));
  inv1   g089(.a(new_n172_), .O(z26));
  nand2  g090(.a(x14), .b(x00), .O(new_n174_));
  inv1   g091(.a(x14), .O(new_n175_));
  nand2  g092(.a(x39), .b(new_n175_), .O(new_n176_));
  aoi21  g093(.a(new_n176_), .b(new_n174_), .c(new_n142_), .O(new_n177_));
  nor2   g094(.a(new_n169_), .b(x09), .O(new_n178_));
  oai21  g095(.a(new_n178_), .b(new_n177_), .c(new_n84_), .O(new_n179_));
  nor2   g096(.a(new_n179_), .b(new_n102_), .O(z27));
  nand2  g097(.a(new_n175_), .b(x09), .O(new_n181_));
  nand2  g098(.a(new_n181_), .b(x39), .O(new_n182_));
  nand3  g099(.a(x40), .b(new_n175_), .c(x09), .O(new_n183_));
  nand2  g100(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  nand3  g101(.a(new_n184_), .b(new_n84_), .c(x08), .O(new_n185_));
  inv1   g102(.a(new_n185_), .O(z28));
  nand2  g103(.a(new_n181_), .b(x40), .O(new_n187_));
  inv1   g104(.a(x41), .O(new_n188_));
  nor2   g105(.a(new_n188_), .b(x14), .O(new_n189_));
  aoi21  g106(.a(new_n189_), .b(x09), .c(new_n102_), .O(new_n190_));
  aoi21  g107(.a(new_n190_), .b(new_n187_), .c(x10), .O(z29));
  inv1   g108(.a(new_n181_), .O(new_n192_));
  nand3  g109(.a(x42), .b(new_n175_), .c(x09), .O(new_n193_));
  oai21  g110(.a(new_n192_), .b(new_n188_), .c(new_n193_), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n84_), .c(x08), .O(new_n195_));
  inv1   g112(.a(new_n195_), .O(z30));
  nand2  g113(.a(new_n181_), .b(x42), .O(new_n197_));
  nand3  g114(.a(x43), .b(new_n175_), .c(x09), .O(new_n198_));
  nand2  g115(.a(new_n198_), .b(new_n197_), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n84_), .c(x08), .O(new_n200_));
  inv1   g117(.a(new_n200_), .O(z31));
  nand2  g118(.a(new_n181_), .b(x43), .O(new_n202_));
  nand3  g119(.a(x44), .b(new_n175_), .c(x09), .O(new_n203_));
  nand2  g120(.a(new_n203_), .b(new_n202_), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n84_), .c(x08), .O(new_n205_));
  inv1   g122(.a(new_n205_), .O(z32));
  nand2  g123(.a(new_n181_), .b(x44), .O(new_n207_));
  nand3  g124(.a(x45), .b(new_n175_), .c(x09), .O(new_n208_));
  nand2  g125(.a(new_n208_), .b(new_n207_), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n84_), .c(x08), .O(new_n210_));
  inv1   g127(.a(new_n210_), .O(z33));
  nand2  g128(.a(new_n181_), .b(x45), .O(new_n212_));
  nand3  g129(.a(x46), .b(new_n175_), .c(x09), .O(new_n213_));
  nand2  g130(.a(new_n213_), .b(new_n212_), .O(new_n214_));
  nand3  g131(.a(new_n214_), .b(new_n84_), .c(x08), .O(new_n215_));
  inv1   g132(.a(new_n215_), .O(z34));
  nand2  g133(.a(new_n181_), .b(x46), .O(new_n217_));
  aoi21  g134(.a(new_n192_), .b(x00), .c(new_n102_), .O(new_n218_));
  aoi21  g135(.a(new_n218_), .b(new_n217_), .c(x10), .O(z35));
endmodule


