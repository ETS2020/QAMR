// Benchmark "FAU" written by ABC on Thu Aug 13 18:12:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n196_, new_n197_, new_n198_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n205_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n212_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n218_, new_n219_, new_n220_, new_n221_, new_n223_,
    new_n224_, new_n225_, new_n226_, new_n228_, new_n229_, new_n230_,
    new_n231_, new_n233_, new_n234_, new_n235_, new_n236_, new_n238_,
    new_n239_, new_n240_, new_n241_, new_n243_, new_n244_, new_n245_,
    new_n246_, new_n248_, new_n249_, new_n250_, new_n251_;
  inv1   g000(.a(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  inv1   g002(.a(x10), .O(new_n86_));
  nand4  g003(.a(x11), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n87_));
  inv1   g004(.a(new_n87_), .O(z00));
  nand2  g005(.a(x07), .b(x05), .O(new_n89_));
  nand2  g006(.a(x12), .b(new_n85_), .O(new_n90_));
  nand3  g007(.a(new_n90_), .b(new_n89_), .c(new_n84_), .O(new_n91_));
  and2   g008(.a(new_n91_), .b(new_n86_), .O(z01));
  nand2  g009(.a(x07), .b(x06), .O(new_n93_));
  nand2  g010(.a(x13), .b(new_n85_), .O(new_n94_));
  nand3  g011(.a(new_n94_), .b(new_n93_), .c(new_n84_), .O(new_n95_));
  and2   g012(.a(new_n95_), .b(new_n86_), .O(z02));
  nand2  g013(.a(x07), .b(x01), .O(new_n97_));
  nand2  g014(.a(x14), .b(new_n85_), .O(new_n98_));
  nand3  g015(.a(new_n98_), .b(new_n97_), .c(new_n84_), .O(new_n99_));
  and2   g016(.a(new_n99_), .b(new_n86_), .O(z03));
  inv1   g017(.a(x15), .O(new_n101_));
  nand2  g018(.a(x07), .b(x02), .O(new_n102_));
  oai21  g019(.a(new_n101_), .b(x07), .c(new_n102_), .O(new_n103_));
  nand3  g020(.a(new_n103_), .b(new_n86_), .c(new_n84_), .O(new_n104_));
  inv1   g021(.a(new_n104_), .O(z04));
  inv1   g022(.a(x16), .O(new_n106_));
  nand2  g023(.a(x07), .b(x03), .O(new_n107_));
  oai21  g024(.a(new_n106_), .b(x07), .c(new_n107_), .O(new_n108_));
  nand3  g025(.a(new_n108_), .b(new_n86_), .c(new_n84_), .O(new_n109_));
  inv1   g026(.a(new_n109_), .O(z05));
  inv1   g027(.a(x08), .O(new_n111_));
  nand2  g028(.a(x17), .b(new_n111_), .O(new_n112_));
  nand2  g029(.a(x18), .b(x08), .O(new_n113_));
  nand3  g030(.a(new_n113_), .b(new_n112_), .c(new_n84_), .O(new_n114_));
  and2   g031(.a(new_n114_), .b(new_n86_), .O(z06));
  inv1   g032(.a(x19), .O(new_n116_));
  nand2  g033(.a(x18), .b(new_n111_), .O(new_n117_));
  oai21  g034(.a(new_n116_), .b(new_n111_), .c(new_n117_), .O(new_n118_));
  nand3  g035(.a(new_n118_), .b(new_n86_), .c(new_n84_), .O(new_n119_));
  inv1   g036(.a(new_n119_), .O(z07));
  nand2  g037(.a(x20), .b(x08), .O(new_n121_));
  oai21  g038(.a(new_n116_), .b(x08), .c(new_n121_), .O(new_n122_));
  nand3  g039(.a(new_n122_), .b(new_n86_), .c(new_n84_), .O(new_n123_));
  inv1   g040(.a(new_n123_), .O(z08));
  nand2  g041(.a(x20), .b(new_n111_), .O(new_n125_));
  nand2  g042(.a(x21), .b(x08), .O(new_n126_));
  nand3  g043(.a(new_n126_), .b(new_n125_), .c(new_n84_), .O(new_n127_));
  and2   g044(.a(new_n127_), .b(new_n86_), .O(z09));
  nand2  g045(.a(x21), .b(new_n111_), .O(new_n129_));
  nand2  g046(.a(x22), .b(x08), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n129_), .c(new_n84_), .O(new_n131_));
  and2   g048(.a(new_n131_), .b(new_n86_), .O(z10));
  nand2  g049(.a(x22), .b(new_n111_), .O(new_n133_));
  nand2  g050(.a(x23), .b(x08), .O(new_n134_));
  nand3  g051(.a(new_n134_), .b(new_n133_), .c(new_n84_), .O(new_n135_));
  and2   g052(.a(new_n135_), .b(new_n86_), .O(z11));
  nand2  g053(.a(x23), .b(new_n111_), .O(new_n137_));
  nand2  g054(.a(x24), .b(x08), .O(new_n138_));
  nand3  g055(.a(new_n138_), .b(new_n137_), .c(new_n84_), .O(new_n139_));
  and2   g056(.a(new_n139_), .b(new_n86_), .O(z12));
  inv1   g057(.a(x25), .O(new_n141_));
  nand2  g058(.a(x24), .b(new_n111_), .O(new_n142_));
  oai21  g059(.a(new_n141_), .b(new_n111_), .c(new_n142_), .O(new_n143_));
  nand3  g060(.a(new_n143_), .b(new_n86_), .c(new_n84_), .O(new_n144_));
  inv1   g061(.a(new_n144_), .O(z13));
  nand2  g062(.a(x26), .b(x08), .O(new_n146_));
  oai21  g063(.a(new_n141_), .b(x08), .c(new_n146_), .O(new_n147_));
  nand3  g064(.a(new_n147_), .b(new_n86_), .c(new_n84_), .O(new_n148_));
  inv1   g065(.a(new_n148_), .O(z14));
  inv1   g066(.a(x26), .O(new_n150_));
  nand2  g067(.a(x27), .b(x08), .O(new_n151_));
  oai21  g068(.a(new_n150_), .b(x08), .c(new_n151_), .O(new_n152_));
  nand3  g069(.a(new_n152_), .b(new_n86_), .c(new_n84_), .O(new_n153_));
  inv1   g070(.a(new_n153_), .O(z15));
  nand2  g071(.a(x27), .b(new_n111_), .O(new_n155_));
  nand2  g072(.a(x28), .b(x08), .O(new_n156_));
  nand3  g073(.a(new_n156_), .b(new_n155_), .c(new_n84_), .O(new_n157_));
  and2   g074(.a(new_n157_), .b(new_n86_), .O(z16));
  inv1   g075(.a(x29), .O(new_n159_));
  nand2  g076(.a(x28), .b(new_n111_), .O(new_n160_));
  oai21  g077(.a(new_n159_), .b(new_n111_), .c(new_n160_), .O(new_n161_));
  nand3  g078(.a(new_n161_), .b(new_n86_), .c(new_n84_), .O(new_n162_));
  inv1   g079(.a(new_n162_), .O(z17));
  nand2  g080(.a(x30), .b(x08), .O(new_n164_));
  oai21  g081(.a(new_n159_), .b(x08), .c(new_n164_), .O(new_n165_));
  nand3  g082(.a(new_n165_), .b(new_n86_), .c(new_n84_), .O(new_n166_));
  inv1   g083(.a(new_n166_), .O(z18));
  nand2  g084(.a(x08), .b(x00), .O(new_n168_));
  nand2  g085(.a(x30), .b(new_n111_), .O(new_n169_));
  nand3  g086(.a(new_n169_), .b(new_n168_), .c(new_n84_), .O(new_n170_));
  and2   g087(.a(new_n170_), .b(new_n86_), .O(z19));
  inv1   g088(.a(x31), .O(new_n172_));
  nand2  g089(.a(x32), .b(x09), .O(new_n173_));
  oai21  g090(.a(new_n172_), .b(x09), .c(new_n173_), .O(new_n174_));
  nand3  g091(.a(new_n174_), .b(new_n86_), .c(new_n84_), .O(new_n175_));
  inv1   g092(.a(new_n175_), .O(z20));
  inv1   g093(.a(x09), .O(new_n177_));
  nand2  g094(.a(x32), .b(new_n177_), .O(new_n178_));
  nand2  g095(.a(x33), .b(x09), .O(new_n179_));
  nand3  g096(.a(new_n179_), .b(new_n178_), .c(new_n84_), .O(new_n180_));
  and2   g097(.a(new_n180_), .b(new_n86_), .O(z21));
  inv1   g098(.a(x34), .O(new_n182_));
  nand2  g099(.a(x33), .b(new_n177_), .O(new_n183_));
  oai21  g100(.a(new_n182_), .b(new_n177_), .c(new_n183_), .O(new_n184_));
  nand3  g101(.a(new_n184_), .b(new_n86_), .c(new_n84_), .O(new_n185_));
  inv1   g102(.a(new_n185_), .O(z22));
  nand2  g103(.a(x35), .b(x09), .O(new_n187_));
  oai21  g104(.a(new_n182_), .b(x09), .c(new_n187_), .O(new_n188_));
  nand3  g105(.a(new_n188_), .b(new_n86_), .c(new_n84_), .O(new_n189_));
  inv1   g106(.a(new_n189_), .O(z23));
  inv1   g107(.a(x35), .O(new_n191_));
  nand2  g108(.a(x36), .b(x09), .O(new_n192_));
  oai21  g109(.a(new_n191_), .b(x09), .c(new_n192_), .O(new_n193_));
  nand3  g110(.a(new_n193_), .b(new_n86_), .c(new_n84_), .O(new_n194_));
  inv1   g111(.a(new_n194_), .O(z24));
  nand2  g112(.a(x36), .b(new_n177_), .O(new_n196_));
  nand2  g113(.a(x37), .b(x09), .O(new_n197_));
  nand3  g114(.a(new_n197_), .b(new_n196_), .c(new_n84_), .O(new_n198_));
  and2   g115(.a(new_n198_), .b(new_n86_), .O(z25));
  inv1   g116(.a(x38), .O(new_n200_));
  nand2  g117(.a(x37), .b(new_n177_), .O(new_n201_));
  oai21  g118(.a(new_n200_), .b(new_n177_), .c(new_n201_), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n86_), .c(new_n84_), .O(new_n203_));
  inv1   g120(.a(new_n203_), .O(z26));
  nand2  g121(.a(x14), .b(x00), .O(new_n205_));
  inv1   g122(.a(x14), .O(new_n206_));
  nand2  g123(.a(x39), .b(new_n206_), .O(new_n207_));
  aoi21  g124(.a(new_n207_), .b(new_n205_), .c(new_n177_), .O(new_n208_));
  nor2   g125(.a(new_n200_), .b(x09), .O(new_n209_));
  oai21  g126(.a(new_n209_), .b(new_n208_), .c(new_n86_), .O(new_n210_));
  nor2   g127(.a(new_n210_), .b(x04), .O(z27));
  nand2  g128(.a(new_n206_), .b(x09), .O(new_n212_));
  nand2  g129(.a(new_n212_), .b(x39), .O(new_n213_));
  nand3  g130(.a(x40), .b(new_n206_), .c(x09), .O(new_n214_));
  nand2  g131(.a(new_n214_), .b(new_n213_), .O(new_n215_));
  nand3  g132(.a(new_n215_), .b(new_n86_), .c(new_n84_), .O(new_n216_));
  inv1   g133(.a(new_n216_), .O(z28));
  nand2  g134(.a(new_n212_), .b(x40), .O(new_n218_));
  nand3  g135(.a(x41), .b(new_n206_), .c(x09), .O(new_n219_));
  nand2  g136(.a(new_n219_), .b(new_n218_), .O(new_n220_));
  nand3  g137(.a(new_n220_), .b(new_n86_), .c(new_n84_), .O(new_n221_));
  inv1   g138(.a(new_n221_), .O(z29));
  nand2  g139(.a(new_n212_), .b(x41), .O(new_n223_));
  nand3  g140(.a(x42), .b(new_n206_), .c(x09), .O(new_n224_));
  nand2  g141(.a(new_n224_), .b(new_n223_), .O(new_n225_));
  nand3  g142(.a(new_n225_), .b(new_n86_), .c(new_n84_), .O(new_n226_));
  inv1   g143(.a(new_n226_), .O(z30));
  nand2  g144(.a(new_n212_), .b(x42), .O(new_n228_));
  inv1   g145(.a(x43), .O(new_n229_));
  nor2   g146(.a(new_n229_), .b(x14), .O(new_n230_));
  aoi21  g147(.a(new_n230_), .b(x09), .c(x04), .O(new_n231_));
  aoi21  g148(.a(new_n231_), .b(new_n228_), .c(x10), .O(z31));
  nand2  g149(.a(new_n212_), .b(x43), .O(new_n233_));
  inv1   g150(.a(x44), .O(new_n234_));
  nor2   g151(.a(new_n234_), .b(x14), .O(new_n235_));
  aoi21  g152(.a(new_n235_), .b(x09), .c(x04), .O(new_n236_));
  aoi21  g153(.a(new_n236_), .b(new_n233_), .c(x10), .O(z32));
  nand2  g154(.a(new_n212_), .b(x44), .O(new_n238_));
  inv1   g155(.a(x45), .O(new_n239_));
  nor2   g156(.a(new_n239_), .b(x14), .O(new_n240_));
  aoi21  g157(.a(new_n240_), .b(x09), .c(x04), .O(new_n241_));
  aoi21  g158(.a(new_n241_), .b(new_n238_), .c(x10), .O(z33));
  nand2  g159(.a(new_n212_), .b(x45), .O(new_n243_));
  nand3  g160(.a(x46), .b(new_n206_), .c(x09), .O(new_n244_));
  nand2  g161(.a(new_n244_), .b(new_n243_), .O(new_n245_));
  nand3  g162(.a(new_n245_), .b(new_n86_), .c(new_n84_), .O(new_n246_));
  inv1   g163(.a(new_n246_), .O(z34));
  inv1   g164(.a(x00), .O(new_n248_));
  nand2  g165(.a(new_n212_), .b(x46), .O(new_n249_));
  oai21  g166(.a(new_n212_), .b(new_n248_), .c(new_n249_), .O(new_n250_));
  nand3  g167(.a(new_n250_), .b(new_n86_), .c(new_n84_), .O(new_n251_));
  inv1   g168(.a(new_n251_), .O(z35));
endmodule


