// Benchmark "FAU" written by ABC on Thu Aug 13 18:12:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n192_, new_n193_, new_n195_,
    new_n196_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n211_, new_n212_, new_n214_, new_n215_, new_n216_, new_n217_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n224_, new_n225_,
    new_n226_, new_n227_, new_n229_, new_n230_, new_n231_, new_n232_,
    new_n234_, new_n235_, new_n236_, new_n237_, new_n239_, new_n240_,
    new_n241_, new_n242_;
  inv1   g000(.a(x10), .O(new_n84_));
  inv1   g001(.a(x11), .O(new_n85_));
  nand2  g002(.a(x07), .b(x04), .O(new_n86_));
  oai21  g003(.a(new_n85_), .b(x07), .c(new_n86_), .O(new_n87_));
  nand2  g004(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nand2  g005(.a(x10), .b(x09), .O(new_n89_));
  nand2  g006(.a(new_n89_), .b(new_n88_), .O(z00));
  nand2  g007(.a(x07), .b(x05), .O(new_n91_));
  inv1   g008(.a(x07), .O(new_n92_));
  nand2  g009(.a(x12), .b(new_n92_), .O(new_n93_));
  aoi21  g010(.a(new_n93_), .b(new_n91_), .c(x10), .O(z01));
  inv1   g011(.a(x13), .O(new_n95_));
  nand2  g012(.a(x07), .b(x06), .O(new_n96_));
  oai21  g013(.a(new_n95_), .b(x07), .c(new_n96_), .O(new_n97_));
  nand2  g014(.a(new_n97_), .b(new_n84_), .O(new_n98_));
  nand2  g015(.a(new_n98_), .b(new_n89_), .O(z02));
  inv1   g016(.a(x14), .O(new_n100_));
  nand2  g017(.a(x07), .b(x01), .O(new_n101_));
  oai21  g018(.a(new_n100_), .b(x07), .c(new_n101_), .O(new_n102_));
  nand2  g019(.a(new_n102_), .b(new_n84_), .O(new_n103_));
  nand2  g020(.a(new_n103_), .b(new_n89_), .O(z03));
  inv1   g021(.a(x15), .O(new_n105_));
  nand2  g022(.a(x07), .b(x02), .O(new_n106_));
  oai21  g023(.a(new_n105_), .b(x07), .c(new_n106_), .O(new_n107_));
  nand2  g024(.a(new_n107_), .b(new_n84_), .O(new_n108_));
  nand2  g025(.a(new_n108_), .b(new_n89_), .O(z04));
  inv1   g026(.a(x16), .O(new_n110_));
  nand2  g027(.a(x07), .b(x03), .O(new_n111_));
  oai21  g028(.a(new_n110_), .b(x07), .c(new_n111_), .O(new_n112_));
  nand2  g029(.a(new_n112_), .b(new_n84_), .O(new_n113_));
  nand2  g030(.a(new_n113_), .b(new_n89_), .O(z05));
  inv1   g031(.a(x17), .O(new_n115_));
  nand2  g032(.a(x18), .b(x08), .O(new_n116_));
  oai21  g033(.a(new_n115_), .b(x08), .c(new_n116_), .O(new_n117_));
  nand2  g034(.a(new_n117_), .b(new_n84_), .O(new_n118_));
  nand2  g035(.a(new_n118_), .b(new_n89_), .O(z06));
  inv1   g036(.a(x18), .O(new_n120_));
  nand2  g037(.a(x19), .b(x08), .O(new_n121_));
  oai21  g038(.a(new_n120_), .b(x08), .c(new_n121_), .O(new_n122_));
  nand2  g039(.a(new_n122_), .b(new_n84_), .O(new_n123_));
  nand2  g040(.a(new_n123_), .b(new_n89_), .O(z07));
  inv1   g041(.a(x19), .O(new_n125_));
  nand2  g042(.a(x20), .b(x08), .O(new_n126_));
  oai21  g043(.a(new_n125_), .b(x08), .c(new_n126_), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(new_n84_), .O(new_n128_));
  nand2  g045(.a(new_n128_), .b(new_n89_), .O(z08));
  inv1   g046(.a(x08), .O(new_n130_));
  nand2  g047(.a(x20), .b(new_n130_), .O(new_n131_));
  nand2  g048(.a(x21), .b(x08), .O(new_n132_));
  aoi21  g049(.a(new_n132_), .b(new_n131_), .c(x10), .O(z09));
  inv1   g050(.a(x21), .O(new_n134_));
  nand2  g051(.a(x22), .b(x08), .O(new_n135_));
  oai21  g052(.a(new_n134_), .b(x08), .c(new_n135_), .O(new_n136_));
  nand2  g053(.a(new_n136_), .b(new_n84_), .O(new_n137_));
  nand2  g054(.a(new_n137_), .b(new_n89_), .O(z10));
  inv1   g055(.a(x22), .O(new_n139_));
  nand2  g056(.a(x23), .b(x08), .O(new_n140_));
  oai21  g057(.a(new_n139_), .b(x08), .c(new_n140_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n84_), .O(new_n142_));
  nand2  g059(.a(new_n142_), .b(new_n89_), .O(z11));
  nand2  g060(.a(x23), .b(new_n130_), .O(new_n144_));
  nand2  g061(.a(x24), .b(x08), .O(new_n145_));
  aoi21  g062(.a(new_n145_), .b(new_n144_), .c(x10), .O(z12));
  nand2  g063(.a(x24), .b(new_n130_), .O(new_n147_));
  nand2  g064(.a(x25), .b(x08), .O(new_n148_));
  aoi21  g065(.a(new_n148_), .b(new_n147_), .c(x10), .O(z13));
  inv1   g066(.a(x25), .O(new_n150_));
  nand2  g067(.a(x26), .b(x08), .O(new_n151_));
  oai21  g068(.a(new_n150_), .b(x08), .c(new_n151_), .O(new_n152_));
  nand2  g069(.a(new_n152_), .b(new_n84_), .O(new_n153_));
  nand2  g070(.a(new_n153_), .b(new_n89_), .O(z14));
  inv1   g071(.a(x26), .O(new_n155_));
  nand2  g072(.a(x27), .b(x08), .O(new_n156_));
  oai21  g073(.a(new_n155_), .b(x08), .c(new_n156_), .O(new_n157_));
  nand2  g074(.a(new_n157_), .b(new_n84_), .O(new_n158_));
  nand2  g075(.a(new_n158_), .b(new_n89_), .O(z15));
  nand2  g076(.a(x27), .b(new_n130_), .O(new_n160_));
  nand2  g077(.a(x28), .b(x08), .O(new_n161_));
  aoi21  g078(.a(new_n161_), .b(new_n160_), .c(x10), .O(z16));
  nand2  g079(.a(x28), .b(new_n130_), .O(new_n163_));
  nand2  g080(.a(x29), .b(x08), .O(new_n164_));
  aoi21  g081(.a(new_n164_), .b(new_n163_), .c(x10), .O(z17));
  inv1   g082(.a(x29), .O(new_n166_));
  nand2  g083(.a(x30), .b(x08), .O(new_n167_));
  oai21  g084(.a(new_n166_), .b(x08), .c(new_n167_), .O(new_n168_));
  nand2  g085(.a(new_n168_), .b(new_n84_), .O(new_n169_));
  nand2  g086(.a(new_n169_), .b(new_n89_), .O(z18));
  inv1   g087(.a(x30), .O(new_n171_));
  nand2  g088(.a(x08), .b(x00), .O(new_n172_));
  oai21  g089(.a(new_n171_), .b(x08), .c(new_n172_), .O(new_n173_));
  nand2  g090(.a(new_n173_), .b(new_n84_), .O(new_n174_));
  nand2  g091(.a(new_n174_), .b(new_n89_), .O(z19));
  inv1   g092(.a(x09), .O(new_n176_));
  nand2  g093(.a(x31), .b(new_n176_), .O(new_n177_));
  nand2  g094(.a(x32), .b(x09), .O(new_n178_));
  aoi21  g095(.a(new_n178_), .b(new_n177_), .c(x10), .O(z20));
  oai21  g096(.a(x33), .b(x10), .c(x09), .O(new_n180_));
  nand3  g097(.a(x32), .b(new_n84_), .c(new_n176_), .O(new_n181_));
  nand2  g098(.a(new_n181_), .b(new_n180_), .O(z21));
  nand2  g099(.a(x33), .b(new_n176_), .O(new_n183_));
  nand2  g100(.a(x34), .b(x09), .O(new_n184_));
  aoi21  g101(.a(new_n184_), .b(new_n183_), .c(x10), .O(z22));
  nand2  g102(.a(x34), .b(new_n176_), .O(new_n186_));
  nand2  g103(.a(x35), .b(x09), .O(new_n187_));
  aoi21  g104(.a(new_n187_), .b(new_n186_), .c(x10), .O(z23));
  oai21  g105(.a(x36), .b(x10), .c(x09), .O(new_n189_));
  nand3  g106(.a(x35), .b(new_n84_), .c(new_n176_), .O(new_n190_));
  nand2  g107(.a(new_n190_), .b(new_n189_), .O(z24));
  nand2  g108(.a(x36), .b(new_n176_), .O(new_n192_));
  nand2  g109(.a(x37), .b(x09), .O(new_n193_));
  aoi21  g110(.a(new_n193_), .b(new_n192_), .c(x10), .O(z25));
  oai21  g111(.a(x38), .b(x10), .c(x09), .O(new_n195_));
  nand3  g112(.a(x37), .b(new_n84_), .c(new_n176_), .O(new_n196_));
  nand2  g113(.a(new_n196_), .b(new_n195_), .O(z26));
  nand2  g114(.a(x14), .b(x00), .O(new_n198_));
  nand2  g115(.a(x39), .b(new_n100_), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n198_), .c(new_n84_), .O(new_n200_));
  nand2  g117(.a(new_n200_), .b(x09), .O(new_n201_));
  nand3  g118(.a(x38), .b(new_n84_), .c(new_n176_), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(new_n201_), .O(z27));
  nor2   g120(.a(x10), .b(x09), .O(new_n204_));
  nor2   g121(.a(new_n100_), .b(new_n176_), .O(new_n205_));
  oai21  g122(.a(new_n205_), .b(new_n204_), .c(x39), .O(new_n206_));
  inv1   g123(.a(x40), .O(new_n207_));
  oai21  g124(.a(new_n207_), .b(x14), .c(new_n84_), .O(new_n208_));
  nand2  g125(.a(new_n208_), .b(x09), .O(new_n209_));
  nand2  g126(.a(new_n209_), .b(new_n206_), .O(z28));
  oai21  g127(.a(x14), .b(new_n176_), .c(x40), .O(new_n211_));
  nand3  g128(.a(x41), .b(new_n100_), .c(x09), .O(new_n212_));
  aoi21  g129(.a(new_n212_), .b(new_n211_), .c(x10), .O(z29));
  oai21  g130(.a(new_n205_), .b(new_n204_), .c(x41), .O(new_n214_));
  inv1   g131(.a(x42), .O(new_n215_));
  oai21  g132(.a(new_n215_), .b(x14), .c(new_n84_), .O(new_n216_));
  nand2  g133(.a(new_n216_), .b(x09), .O(new_n217_));
  nand2  g134(.a(new_n217_), .b(new_n214_), .O(z30));
  oai21  g135(.a(new_n205_), .b(new_n204_), .c(x42), .O(new_n219_));
  inv1   g136(.a(x43), .O(new_n220_));
  oai21  g137(.a(new_n220_), .b(x14), .c(new_n84_), .O(new_n221_));
  nand2  g138(.a(new_n221_), .b(x09), .O(new_n222_));
  nand2  g139(.a(new_n222_), .b(new_n219_), .O(z31));
  oai21  g140(.a(new_n205_), .b(new_n204_), .c(x43), .O(new_n224_));
  inv1   g141(.a(x44), .O(new_n225_));
  oai21  g142(.a(new_n225_), .b(x14), .c(new_n84_), .O(new_n226_));
  nand2  g143(.a(new_n226_), .b(x09), .O(new_n227_));
  nand2  g144(.a(new_n227_), .b(new_n224_), .O(z32));
  oai21  g145(.a(new_n205_), .b(new_n204_), .c(x44), .O(new_n229_));
  inv1   g146(.a(x45), .O(new_n230_));
  oai21  g147(.a(new_n230_), .b(x14), .c(new_n84_), .O(new_n231_));
  nand2  g148(.a(new_n231_), .b(x09), .O(new_n232_));
  nand2  g149(.a(new_n232_), .b(new_n229_), .O(z33));
  oai21  g150(.a(new_n205_), .b(new_n204_), .c(x45), .O(new_n234_));
  inv1   g151(.a(x46), .O(new_n235_));
  oai21  g152(.a(new_n235_), .b(x14), .c(new_n84_), .O(new_n236_));
  nand2  g153(.a(new_n236_), .b(x09), .O(new_n237_));
  nand2  g154(.a(new_n237_), .b(new_n234_), .O(z34));
  oai21  g155(.a(new_n205_), .b(new_n204_), .c(x46), .O(new_n239_));
  inv1   g156(.a(x00), .O(new_n240_));
  oai21  g157(.a(x14), .b(new_n240_), .c(new_n84_), .O(new_n241_));
  nand2  g158(.a(new_n241_), .b(x09), .O(new_n242_));
  nand2  g159(.a(new_n242_), .b(new_n239_), .O(z35));
endmodule


