// Benchmark "FAU" written by ABC on Thu Aug 13 18:12:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n202_, new_n203_, new_n204_, new_n206_,
    new_n207_, new_n209_, new_n210_, new_n211_, new_n212_, new_n213_,
    new_n215_, new_n216_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n224_, new_n225_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  nand2  g002(.a(x11), .b(new_n85_), .O(new_n86_));
  aoi21  g003(.a(new_n86_), .b(new_n84_), .c(x10), .O(z00));
  nand2  g004(.a(x07), .b(x05), .O(new_n88_));
  nand2  g005(.a(x12), .b(new_n85_), .O(new_n89_));
  aoi21  g006(.a(new_n89_), .b(new_n88_), .c(x10), .O(z01));
  nand2  g007(.a(x07), .b(x06), .O(new_n91_));
  nand2  g008(.a(x13), .b(new_n85_), .O(new_n92_));
  aoi21  g009(.a(new_n92_), .b(new_n91_), .c(x10), .O(z02));
  nand2  g010(.a(x07), .b(x01), .O(new_n94_));
  nand2  g011(.a(x14), .b(new_n85_), .O(new_n95_));
  aoi21  g012(.a(new_n95_), .b(new_n94_), .c(x10), .O(z03));
  inv1   g013(.a(x10), .O(new_n97_));
  inv1   g014(.a(x15), .O(new_n98_));
  nand2  g015(.a(x07), .b(x02), .O(new_n99_));
  oai21  g016(.a(new_n98_), .b(x07), .c(new_n99_), .O(new_n100_));
  nand2  g017(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  nand2  g018(.a(x25), .b(x10), .O(new_n102_));
  nand2  g019(.a(new_n102_), .b(new_n101_), .O(z04));
  nand2  g020(.a(x07), .b(x03), .O(new_n104_));
  nand2  g021(.a(x16), .b(new_n85_), .O(new_n105_));
  aoi21  g022(.a(new_n105_), .b(new_n104_), .c(x10), .O(z05));
  inv1   g023(.a(x08), .O(new_n107_));
  nand2  g024(.a(x17), .b(new_n107_), .O(new_n108_));
  nand2  g025(.a(x18), .b(x08), .O(new_n109_));
  aoi21  g026(.a(new_n109_), .b(new_n108_), .c(x10), .O(z06));
  inv1   g027(.a(x18), .O(new_n111_));
  nand2  g028(.a(x19), .b(x08), .O(new_n112_));
  oai21  g029(.a(new_n111_), .b(x08), .c(new_n112_), .O(new_n113_));
  nand2  g030(.a(new_n113_), .b(new_n97_), .O(new_n114_));
  nand2  g031(.a(new_n114_), .b(new_n102_), .O(z07));
  nand2  g032(.a(x19), .b(new_n107_), .O(new_n116_));
  nand2  g033(.a(x20), .b(x08), .O(new_n117_));
  aoi21  g034(.a(new_n117_), .b(new_n116_), .c(x10), .O(z08));
  nand2  g035(.a(x20), .b(new_n107_), .O(new_n119_));
  nand2  g036(.a(x21), .b(x08), .O(new_n120_));
  aoi21  g037(.a(new_n120_), .b(new_n119_), .c(x10), .O(z09));
  inv1   g038(.a(x21), .O(new_n122_));
  nand2  g039(.a(x22), .b(x08), .O(new_n123_));
  oai21  g040(.a(new_n122_), .b(x08), .c(new_n123_), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(new_n97_), .O(new_n125_));
  nand2  g042(.a(new_n125_), .b(new_n102_), .O(z10));
  nand2  g043(.a(x22), .b(new_n107_), .O(new_n127_));
  nand2  g044(.a(x23), .b(x08), .O(new_n128_));
  aoi21  g045(.a(new_n128_), .b(new_n127_), .c(x10), .O(z11));
  nand2  g046(.a(x23), .b(new_n107_), .O(new_n130_));
  nand2  g047(.a(x24), .b(x08), .O(new_n131_));
  aoi21  g048(.a(new_n131_), .b(new_n130_), .c(x10), .O(z12));
  oai21  g049(.a(x10), .b(x08), .c(x25), .O(new_n133_));
  nand3  g050(.a(x24), .b(new_n97_), .c(new_n107_), .O(new_n134_));
  nand2  g051(.a(new_n134_), .b(new_n133_), .O(z13));
  oai21  g052(.a(x26), .b(new_n107_), .c(x25), .O(new_n136_));
  nand2  g053(.a(x26), .b(x08), .O(new_n137_));
  aoi21  g054(.a(new_n137_), .b(new_n136_), .c(x10), .O(z14));
  inv1   g055(.a(x26), .O(new_n139_));
  nand2  g056(.a(x27), .b(x08), .O(new_n140_));
  oai21  g057(.a(new_n139_), .b(x08), .c(new_n140_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n97_), .O(new_n142_));
  nand2  g059(.a(new_n142_), .b(new_n102_), .O(z15));
  nand2  g060(.a(x27), .b(new_n107_), .O(new_n144_));
  nand2  g061(.a(x28), .b(x08), .O(new_n145_));
  aoi21  g062(.a(new_n145_), .b(new_n144_), .c(x10), .O(z16));
  nand2  g063(.a(x28), .b(new_n107_), .O(new_n147_));
  nand2  g064(.a(x29), .b(x08), .O(new_n148_));
  aoi21  g065(.a(new_n148_), .b(new_n147_), .c(x10), .O(z17));
  inv1   g066(.a(x29), .O(new_n150_));
  nand2  g067(.a(x30), .b(x08), .O(new_n151_));
  oai21  g068(.a(new_n150_), .b(x08), .c(new_n151_), .O(new_n152_));
  nand2  g069(.a(new_n152_), .b(new_n97_), .O(new_n153_));
  nand2  g070(.a(new_n153_), .b(new_n102_), .O(z18));
  nand2  g071(.a(x08), .b(x00), .O(new_n155_));
  nand2  g072(.a(x30), .b(new_n107_), .O(new_n156_));
  aoi21  g073(.a(new_n156_), .b(new_n155_), .c(x10), .O(z19));
  inv1   g074(.a(x09), .O(new_n158_));
  inv1   g075(.a(x32), .O(new_n159_));
  nand2  g076(.a(x31), .b(new_n158_), .O(new_n160_));
  oai21  g077(.a(new_n159_), .b(new_n158_), .c(new_n160_), .O(new_n161_));
  nand2  g078(.a(new_n161_), .b(new_n97_), .O(new_n162_));
  nand2  g079(.a(new_n162_), .b(new_n102_), .O(z20));
  nand2  g080(.a(x33), .b(x09), .O(new_n164_));
  oai21  g081(.a(new_n159_), .b(x09), .c(new_n164_), .O(new_n165_));
  nand2  g082(.a(new_n165_), .b(new_n97_), .O(new_n166_));
  nand2  g083(.a(new_n166_), .b(new_n102_), .O(z21));
  nand2  g084(.a(x33), .b(new_n158_), .O(new_n168_));
  nand2  g085(.a(x34), .b(x09), .O(new_n169_));
  aoi21  g086(.a(new_n169_), .b(new_n168_), .c(x10), .O(z22));
  nand2  g087(.a(x34), .b(new_n158_), .O(new_n171_));
  nand2  g088(.a(x35), .b(x09), .O(new_n172_));
  aoi21  g089(.a(new_n172_), .b(new_n171_), .c(x10), .O(z23));
  nand2  g090(.a(x35), .b(new_n158_), .O(new_n174_));
  nand2  g091(.a(x36), .b(x09), .O(new_n175_));
  aoi21  g092(.a(new_n175_), .b(new_n174_), .c(x10), .O(z24));
  nand2  g093(.a(x36), .b(new_n158_), .O(new_n177_));
  nand2  g094(.a(x37), .b(x09), .O(new_n178_));
  aoi21  g095(.a(new_n178_), .b(new_n177_), .c(x10), .O(z25));
  nand2  g096(.a(x37), .b(new_n158_), .O(new_n180_));
  nand2  g097(.a(x38), .b(x09), .O(new_n181_));
  aoi21  g098(.a(new_n181_), .b(new_n180_), .c(x10), .O(z26));
  inv1   g099(.a(x39), .O(new_n183_));
  nand2  g100(.a(x14), .b(x00), .O(new_n184_));
  oai21  g101(.a(new_n183_), .b(x14), .c(new_n184_), .O(new_n185_));
  inv1   g102(.a(x38), .O(new_n186_));
  nor2   g103(.a(new_n186_), .b(x09), .O(new_n187_));
  aoi21  g104(.a(new_n185_), .b(x09), .c(new_n187_), .O(new_n188_));
  oai21  g105(.a(new_n188_), .b(x10), .c(new_n102_), .O(z27));
  inv1   g106(.a(x14), .O(new_n190_));
  aoi21  g107(.a(new_n190_), .b(x09), .c(new_n183_), .O(new_n191_));
  nand3  g108(.a(x40), .b(new_n190_), .c(x09), .O(new_n192_));
  inv1   g109(.a(new_n192_), .O(new_n193_));
  oai21  g110(.a(new_n193_), .b(new_n191_), .c(new_n97_), .O(new_n194_));
  nand2  g111(.a(new_n194_), .b(new_n102_), .O(z28));
  inv1   g112(.a(x40), .O(new_n196_));
  aoi21  g113(.a(new_n190_), .b(x09), .c(new_n196_), .O(new_n197_));
  nand3  g114(.a(x41), .b(new_n190_), .c(x09), .O(new_n198_));
  inv1   g115(.a(new_n198_), .O(new_n199_));
  oai21  g116(.a(new_n199_), .b(new_n197_), .c(new_n97_), .O(new_n200_));
  nand2  g117(.a(new_n200_), .b(new_n102_), .O(z29));
  nand2  g118(.a(new_n190_), .b(x09), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(x41), .O(new_n203_));
  nand3  g120(.a(x42), .b(new_n190_), .c(x09), .O(new_n204_));
  aoi21  g121(.a(new_n204_), .b(new_n203_), .c(x10), .O(z30));
  nand2  g122(.a(new_n202_), .b(x42), .O(new_n206_));
  nand3  g123(.a(x43), .b(new_n190_), .c(x09), .O(new_n207_));
  aoi21  g124(.a(new_n207_), .b(new_n206_), .c(x10), .O(z31));
  inv1   g125(.a(x43), .O(new_n209_));
  aoi21  g126(.a(new_n190_), .b(x09), .c(new_n209_), .O(new_n210_));
  nand3  g127(.a(x44), .b(new_n190_), .c(x09), .O(new_n211_));
  inv1   g128(.a(new_n211_), .O(new_n212_));
  oai21  g129(.a(new_n212_), .b(new_n210_), .c(new_n97_), .O(new_n213_));
  nand2  g130(.a(new_n213_), .b(new_n102_), .O(z32));
  nand2  g131(.a(new_n202_), .b(x44), .O(new_n215_));
  nand3  g132(.a(x45), .b(new_n190_), .c(x09), .O(new_n216_));
  aoi21  g133(.a(new_n216_), .b(new_n215_), .c(x10), .O(z33));
  inv1   g134(.a(x45), .O(new_n218_));
  aoi21  g135(.a(new_n190_), .b(x09), .c(new_n218_), .O(new_n219_));
  nand3  g136(.a(x46), .b(new_n190_), .c(x09), .O(new_n220_));
  inv1   g137(.a(new_n220_), .O(new_n221_));
  oai21  g138(.a(new_n221_), .b(new_n219_), .c(new_n97_), .O(new_n222_));
  nand2  g139(.a(new_n222_), .b(new_n102_), .O(z34));
  nand2  g140(.a(new_n202_), .b(x46), .O(new_n224_));
  nand3  g141(.a(new_n190_), .b(x09), .c(x00), .O(new_n225_));
  aoi21  g142(.a(new_n225_), .b(new_n224_), .c(x10), .O(z35));
endmodule


