// Benchmark "FAU" written by ABC on Thu Aug 13 18:12:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n173_, new_n174_, new_n176_,
    new_n177_, new_n178_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n204_, new_n205_, new_n206_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n213_, new_n214_,
    new_n215_, new_n216_, new_n217_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n225_, new_n226_, new_n227_, new_n228_,
    new_n230_, new_n231_, new_n232_, new_n233_, new_n234_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n240_, new_n242_, new_n243_,
    new_n244_, new_n245_, new_n247_, new_n248_, new_n249_, new_n250_,
    new_n251_, new_n253_, new_n254_, new_n255_, new_n256_, new_n257_,
    new_n259_, new_n260_, new_n261_, new_n262_, new_n263_;
  inv1   g000(.a(x10), .O(new_n84_));
  inv1   g001(.a(x14), .O(new_n85_));
  inv1   g002(.a(x22), .O(new_n86_));
  nand2  g003(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g004(.a(x11), .O(new_n88_));
  nand2  g005(.a(x07), .b(x04), .O(new_n89_));
  oai21  g006(.a(new_n88_), .b(x07), .c(new_n89_), .O(new_n90_));
  nand3  g007(.a(new_n90_), .b(new_n87_), .c(new_n84_), .O(new_n91_));
  inv1   g008(.a(new_n91_), .O(z00));
  inv1   g009(.a(x12), .O(new_n93_));
  nand2  g010(.a(x07), .b(x05), .O(new_n94_));
  oai21  g011(.a(new_n93_), .b(x07), .c(new_n94_), .O(new_n95_));
  nand3  g012(.a(new_n95_), .b(new_n87_), .c(new_n84_), .O(new_n96_));
  inv1   g013(.a(new_n96_), .O(z01));
  inv1   g014(.a(x13), .O(new_n98_));
  nand2  g015(.a(x07), .b(x06), .O(new_n99_));
  oai21  g016(.a(new_n98_), .b(x07), .c(new_n99_), .O(new_n100_));
  nand2  g017(.a(new_n100_), .b(new_n84_), .O(new_n101_));
  nand2  g018(.a(new_n101_), .b(new_n87_), .O(z02));
  inv1   g019(.a(x07), .O(new_n103_));
  oai21  g020(.a(new_n86_), .b(new_n103_), .c(new_n85_), .O(new_n104_));
  nand2  g021(.a(new_n104_), .b(x01), .O(new_n105_));
  nand2  g022(.a(x14), .b(new_n103_), .O(new_n106_));
  aoi21  g023(.a(new_n106_), .b(new_n105_), .c(x10), .O(z03));
  inv1   g024(.a(x15), .O(new_n108_));
  nand2  g025(.a(x07), .b(x02), .O(new_n109_));
  oai21  g026(.a(new_n108_), .b(x07), .c(new_n109_), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(new_n84_), .O(new_n111_));
  nand2  g028(.a(new_n111_), .b(new_n87_), .O(z04));
  inv1   g029(.a(x16), .O(new_n113_));
  nand2  g030(.a(x07), .b(x03), .O(new_n114_));
  oai21  g031(.a(new_n113_), .b(x07), .c(new_n114_), .O(new_n115_));
  nand2  g032(.a(new_n115_), .b(new_n84_), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n87_), .O(z05));
  inv1   g034(.a(x08), .O(new_n118_));
  inv1   g035(.a(x18), .O(new_n119_));
  nand2  g036(.a(x17), .b(new_n118_), .O(new_n120_));
  oai21  g037(.a(new_n119_), .b(new_n118_), .c(new_n120_), .O(new_n121_));
  nand3  g038(.a(new_n121_), .b(new_n87_), .c(new_n84_), .O(new_n122_));
  inv1   g039(.a(new_n122_), .O(z06));
  nand2  g040(.a(x19), .b(x08), .O(new_n124_));
  oai21  g041(.a(new_n119_), .b(x08), .c(new_n124_), .O(new_n125_));
  nand3  g042(.a(new_n125_), .b(new_n87_), .c(new_n84_), .O(new_n126_));
  inv1   g043(.a(new_n126_), .O(z07));
  inv1   g044(.a(x20), .O(new_n128_));
  nand2  g045(.a(x19), .b(new_n118_), .O(new_n129_));
  oai21  g046(.a(new_n128_), .b(new_n118_), .c(new_n129_), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n87_), .c(new_n84_), .O(new_n131_));
  inv1   g048(.a(new_n131_), .O(z08));
  nand2  g049(.a(x21), .b(x08), .O(new_n133_));
  oai21  g050(.a(new_n128_), .b(x08), .c(new_n133_), .O(new_n134_));
  nand2  g051(.a(new_n134_), .b(new_n84_), .O(new_n135_));
  nand2  g052(.a(new_n135_), .b(new_n87_), .O(z09));
  oai21  g053(.a(new_n85_), .b(x08), .c(new_n86_), .O(new_n137_));
  nand2  g054(.a(new_n137_), .b(x21), .O(new_n138_));
  nand2  g055(.a(x22), .b(x08), .O(new_n139_));
  aoi21  g056(.a(new_n139_), .b(new_n138_), .c(x10), .O(z10));
  oai21  g057(.a(x23), .b(new_n118_), .c(x22), .O(new_n141_));
  nand3  g058(.a(x23), .b(x14), .c(x08), .O(new_n142_));
  aoi21  g059(.a(new_n142_), .b(new_n141_), .c(x10), .O(z11));
  inv1   g060(.a(x24), .O(new_n144_));
  nand2  g061(.a(x23), .b(new_n118_), .O(new_n145_));
  oai21  g062(.a(new_n144_), .b(new_n118_), .c(new_n145_), .O(new_n146_));
  nand3  g063(.a(new_n146_), .b(new_n87_), .c(new_n84_), .O(new_n147_));
  inv1   g064(.a(new_n147_), .O(z12));
  nand2  g065(.a(x25), .b(x08), .O(new_n149_));
  oai21  g066(.a(new_n144_), .b(x08), .c(new_n149_), .O(new_n150_));
  nand2  g067(.a(new_n150_), .b(new_n84_), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(new_n87_), .O(z13));
  inv1   g069(.a(x26), .O(new_n153_));
  nand2  g070(.a(x25), .b(new_n118_), .O(new_n154_));
  oai21  g071(.a(new_n153_), .b(new_n118_), .c(new_n154_), .O(new_n155_));
  nand3  g072(.a(new_n155_), .b(new_n87_), .c(new_n84_), .O(new_n156_));
  inv1   g073(.a(new_n156_), .O(z14));
  nand2  g074(.a(x27), .b(x08), .O(new_n158_));
  oai21  g075(.a(new_n153_), .b(x08), .c(new_n158_), .O(new_n159_));
  nand2  g076(.a(new_n159_), .b(new_n84_), .O(new_n160_));
  nand2  g077(.a(new_n160_), .b(new_n87_), .O(z15));
  inv1   g078(.a(x28), .O(new_n162_));
  nand2  g079(.a(x27), .b(new_n118_), .O(new_n163_));
  oai21  g080(.a(new_n162_), .b(new_n118_), .c(new_n163_), .O(new_n164_));
  nand2  g081(.a(new_n164_), .b(new_n84_), .O(new_n165_));
  nand2  g082(.a(new_n165_), .b(new_n87_), .O(z16));
  nand2  g083(.a(x29), .b(x08), .O(new_n167_));
  oai21  g084(.a(new_n162_), .b(x08), .c(new_n167_), .O(new_n168_));
  nand2  g085(.a(new_n168_), .b(new_n84_), .O(new_n169_));
  nand2  g086(.a(new_n169_), .b(new_n87_), .O(z17));
  inv1   g087(.a(x30), .O(new_n171_));
  nand2  g088(.a(x29), .b(new_n118_), .O(new_n172_));
  oai21  g089(.a(new_n171_), .b(new_n118_), .c(new_n172_), .O(new_n173_));
  nand2  g090(.a(new_n173_), .b(new_n84_), .O(new_n174_));
  nand2  g091(.a(new_n174_), .b(new_n87_), .O(z18));
  nand2  g092(.a(x08), .b(x00), .O(new_n176_));
  oai21  g093(.a(new_n171_), .b(x08), .c(new_n176_), .O(new_n177_));
  nand3  g094(.a(new_n177_), .b(new_n87_), .c(new_n84_), .O(new_n178_));
  inv1   g095(.a(new_n178_), .O(z19));
  inv1   g096(.a(x09), .O(new_n180_));
  inv1   g097(.a(x32), .O(new_n181_));
  nand2  g098(.a(x31), .b(new_n180_), .O(new_n182_));
  oai21  g099(.a(new_n181_), .b(new_n180_), .c(new_n182_), .O(new_n183_));
  nand3  g100(.a(new_n183_), .b(new_n87_), .c(new_n84_), .O(new_n184_));
  inv1   g101(.a(new_n184_), .O(z20));
  nand2  g102(.a(x33), .b(x09), .O(new_n186_));
  oai21  g103(.a(new_n181_), .b(x09), .c(new_n186_), .O(new_n187_));
  nand2  g104(.a(new_n187_), .b(new_n84_), .O(new_n188_));
  nand2  g105(.a(new_n188_), .b(new_n87_), .O(z21));
  inv1   g106(.a(x34), .O(new_n190_));
  nand2  g107(.a(x33), .b(new_n180_), .O(new_n191_));
  oai21  g108(.a(new_n190_), .b(new_n180_), .c(new_n191_), .O(new_n192_));
  nand2  g109(.a(new_n192_), .b(new_n84_), .O(new_n193_));
  nand2  g110(.a(new_n193_), .b(new_n87_), .O(z22));
  nand2  g111(.a(x35), .b(x09), .O(new_n195_));
  oai21  g112(.a(new_n190_), .b(x09), .c(new_n195_), .O(new_n196_));
  nand2  g113(.a(new_n196_), .b(new_n84_), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(new_n87_), .O(z23));
  inv1   g115(.a(x36), .O(new_n199_));
  nand2  g116(.a(x35), .b(new_n180_), .O(new_n200_));
  oai21  g117(.a(new_n199_), .b(new_n180_), .c(new_n200_), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n87_), .c(new_n84_), .O(new_n202_));
  inv1   g119(.a(new_n202_), .O(z24));
  nand2  g120(.a(x37), .b(x09), .O(new_n204_));
  oai21  g121(.a(new_n199_), .b(x09), .c(new_n204_), .O(new_n205_));
  nand2  g122(.a(new_n205_), .b(new_n84_), .O(new_n206_));
  nand2  g123(.a(new_n206_), .b(new_n87_), .O(z25));
  inv1   g124(.a(x37), .O(new_n208_));
  nand2  g125(.a(x38), .b(x09), .O(new_n209_));
  oai21  g126(.a(new_n208_), .b(x09), .c(new_n209_), .O(new_n210_));
  nand3  g127(.a(new_n210_), .b(new_n87_), .c(new_n84_), .O(new_n211_));
  inv1   g128(.a(new_n211_), .O(z26));
  nand2  g129(.a(x14), .b(x00), .O(new_n213_));
  nand2  g130(.a(x39), .b(x22), .O(new_n214_));
  oai21  g131(.a(new_n214_), .b(x14), .c(new_n213_), .O(new_n215_));
  nand2  g132(.a(new_n215_), .b(x09), .O(new_n216_));
  nand3  g133(.a(new_n87_), .b(x38), .c(new_n180_), .O(new_n217_));
  aoi21  g134(.a(new_n217_), .b(new_n216_), .c(x10), .O(z27));
  nand3  g135(.a(x40), .b(new_n84_), .c(x09), .O(new_n219_));
  nand2  g136(.a(new_n219_), .b(x22), .O(new_n220_));
  nand2  g137(.a(new_n220_), .b(new_n85_), .O(new_n221_));
  nand2  g138(.a(new_n85_), .b(x09), .O(new_n222_));
  nand3  g139(.a(new_n222_), .b(x39), .c(new_n84_), .O(new_n223_));
  nand2  g140(.a(new_n223_), .b(new_n221_), .O(z28));
  nand3  g141(.a(x41), .b(new_n84_), .c(x09), .O(new_n225_));
  nand2  g142(.a(new_n225_), .b(x22), .O(new_n226_));
  nand2  g143(.a(new_n226_), .b(new_n85_), .O(new_n227_));
  nand3  g144(.a(new_n222_), .b(x40), .c(new_n84_), .O(new_n228_));
  nand2  g145(.a(new_n228_), .b(new_n227_), .O(z29));
  nand2  g146(.a(x41), .b(new_n180_), .O(new_n230_));
  nand3  g147(.a(x42), .b(new_n85_), .c(x09), .O(new_n231_));
  nand2  g148(.a(new_n231_), .b(new_n230_), .O(new_n232_));
  nand2  g149(.a(new_n232_), .b(x22), .O(new_n233_));
  nand2  g150(.a(x41), .b(x14), .O(new_n234_));
  aoi21  g151(.a(new_n234_), .b(new_n233_), .c(x10), .O(z30));
  nand2  g152(.a(x42), .b(new_n180_), .O(new_n236_));
  nand3  g153(.a(x43), .b(new_n85_), .c(x09), .O(new_n237_));
  nand2  g154(.a(new_n237_), .b(new_n236_), .O(new_n238_));
  nand2  g155(.a(new_n238_), .b(x22), .O(new_n239_));
  nand2  g156(.a(x42), .b(x14), .O(new_n240_));
  aoi21  g157(.a(new_n240_), .b(new_n239_), .c(x10), .O(z31));
  nand3  g158(.a(x44), .b(new_n84_), .c(x09), .O(new_n242_));
  nand2  g159(.a(new_n242_), .b(x22), .O(new_n243_));
  nand2  g160(.a(new_n243_), .b(new_n85_), .O(new_n244_));
  nand3  g161(.a(new_n222_), .b(x43), .c(new_n84_), .O(new_n245_));
  nand2  g162(.a(new_n245_), .b(new_n244_), .O(z32));
  nand2  g163(.a(x44), .b(new_n180_), .O(new_n247_));
  nand3  g164(.a(x45), .b(new_n85_), .c(x09), .O(new_n248_));
  nand2  g165(.a(new_n248_), .b(new_n247_), .O(new_n249_));
  nand2  g166(.a(new_n249_), .b(x22), .O(new_n250_));
  nand2  g167(.a(x44), .b(x14), .O(new_n251_));
  aoi21  g168(.a(new_n251_), .b(new_n250_), .c(x10), .O(z33));
  nand2  g169(.a(x45), .b(new_n180_), .O(new_n253_));
  nand3  g170(.a(x46), .b(new_n85_), .c(x09), .O(new_n254_));
  nand2  g171(.a(new_n254_), .b(new_n253_), .O(new_n255_));
  nand2  g172(.a(new_n255_), .b(x22), .O(new_n256_));
  nand2  g173(.a(x45), .b(x14), .O(new_n257_));
  aoi21  g174(.a(new_n257_), .b(new_n256_), .c(x10), .O(z34));
  nand3  g175(.a(new_n85_), .b(x09), .c(x00), .O(new_n259_));
  nand2  g176(.a(x46), .b(new_n180_), .O(new_n260_));
  nand2  g177(.a(new_n260_), .b(new_n259_), .O(new_n261_));
  nand2  g178(.a(new_n261_), .b(x22), .O(new_n262_));
  nand2  g179(.a(x46), .b(x14), .O(new_n263_));
  aoi21  g180(.a(new_n263_), .b(new_n262_), .c(x10), .O(z35));
endmodule


