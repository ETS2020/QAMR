// Benchmark "FAU" written by ABC on Thu Aug 13 18:12:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n180_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n209_, new_n210_, new_n211_,
    new_n212_, new_n213_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n221_, new_n222_, new_n223_, new_n224_, new_n225_,
    new_n227_, new_n228_, new_n230_, new_n231_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  nand2  g002(.a(x11), .b(new_n85_), .O(new_n86_));
  aoi21  g003(.a(new_n86_), .b(new_n84_), .c(x10), .O(z00));
  nand2  g004(.a(x07), .b(x05), .O(new_n88_));
  nand2  g005(.a(x12), .b(new_n85_), .O(new_n89_));
  aoi21  g006(.a(new_n89_), .b(new_n88_), .c(x10), .O(z01));
  nand2  g007(.a(x07), .b(x06), .O(new_n91_));
  nand2  g008(.a(x13), .b(new_n85_), .O(new_n92_));
  aoi21  g009(.a(new_n92_), .b(new_n91_), .c(x10), .O(z02));
  nand2  g010(.a(x07), .b(x01), .O(new_n94_));
  nand2  g011(.a(x14), .b(new_n85_), .O(new_n95_));
  aoi21  g012(.a(new_n95_), .b(new_n94_), .c(x10), .O(z03));
  nand2  g013(.a(x07), .b(x02), .O(new_n97_));
  nand2  g014(.a(x15), .b(new_n85_), .O(new_n98_));
  aoi21  g015(.a(new_n98_), .b(new_n97_), .c(x10), .O(z04));
  nand2  g016(.a(x07), .b(x03), .O(new_n100_));
  nand2  g017(.a(x16), .b(new_n85_), .O(new_n101_));
  aoi21  g018(.a(new_n101_), .b(new_n100_), .c(x10), .O(z05));
  inv1   g019(.a(x10), .O(new_n103_));
  inv1   g020(.a(x17), .O(new_n104_));
  nand2  g021(.a(x18), .b(x08), .O(new_n105_));
  oai21  g022(.a(new_n104_), .b(x08), .c(new_n105_), .O(new_n106_));
  nand2  g023(.a(new_n106_), .b(new_n103_), .O(new_n107_));
  inv1   g024(.a(x15), .O(new_n108_));
  nand2  g025(.a(new_n108_), .b(x10), .O(new_n109_));
  nand2  g026(.a(new_n109_), .b(new_n107_), .O(z06));
  inv1   g027(.a(x08), .O(new_n111_));
  nand2  g028(.a(x18), .b(new_n111_), .O(new_n112_));
  nand2  g029(.a(x19), .b(x08), .O(new_n113_));
  aoi21  g030(.a(new_n113_), .b(new_n112_), .c(x10), .O(z07));
  nand2  g031(.a(x19), .b(new_n111_), .O(new_n115_));
  nand2  g032(.a(x20), .b(x08), .O(new_n116_));
  aoi21  g033(.a(new_n116_), .b(new_n115_), .c(x10), .O(z08));
  nand2  g034(.a(x20), .b(new_n111_), .O(new_n118_));
  nand2  g035(.a(x21), .b(x08), .O(new_n119_));
  aoi21  g036(.a(new_n119_), .b(new_n118_), .c(x10), .O(z09));
  nand2  g037(.a(x21), .b(new_n111_), .O(new_n121_));
  nand2  g038(.a(x22), .b(x08), .O(new_n122_));
  aoi21  g039(.a(new_n122_), .b(new_n121_), .c(x10), .O(z10));
  inv1   g040(.a(x23), .O(new_n124_));
  nand2  g041(.a(x22), .b(new_n111_), .O(new_n125_));
  oai21  g042(.a(new_n124_), .b(new_n111_), .c(new_n125_), .O(new_n126_));
  nand2  g043(.a(new_n126_), .b(new_n103_), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(new_n109_), .O(z11));
  nand2  g045(.a(x24), .b(x08), .O(new_n129_));
  oai21  g046(.a(new_n124_), .b(x08), .c(new_n129_), .O(new_n130_));
  nand2  g047(.a(new_n130_), .b(new_n103_), .O(new_n131_));
  nand2  g048(.a(new_n131_), .b(new_n109_), .O(z12));
  nand2  g049(.a(x24), .b(new_n111_), .O(new_n133_));
  nand2  g050(.a(x25), .b(x08), .O(new_n134_));
  aoi21  g051(.a(new_n134_), .b(new_n133_), .c(x10), .O(z13));
  nand2  g052(.a(x25), .b(new_n111_), .O(new_n136_));
  nand2  g053(.a(x26), .b(x08), .O(new_n137_));
  aoi21  g054(.a(new_n137_), .b(new_n136_), .c(x10), .O(z14));
  inv1   g055(.a(x26), .O(new_n139_));
  nand2  g056(.a(x27), .b(x08), .O(new_n140_));
  oai21  g057(.a(new_n139_), .b(x08), .c(new_n140_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n103_), .O(new_n142_));
  nand2  g059(.a(new_n142_), .b(new_n109_), .O(z15));
  nand2  g060(.a(x27), .b(new_n111_), .O(new_n144_));
  nand2  g061(.a(x28), .b(x08), .O(new_n145_));
  aoi21  g062(.a(new_n145_), .b(new_n144_), .c(x10), .O(z16));
  inv1   g063(.a(x28), .O(new_n147_));
  nand2  g064(.a(x29), .b(x08), .O(new_n148_));
  oai21  g065(.a(new_n147_), .b(x08), .c(new_n148_), .O(new_n149_));
  nand2  g066(.a(new_n149_), .b(new_n103_), .O(new_n150_));
  nand2  g067(.a(new_n150_), .b(new_n109_), .O(z17));
  nand2  g068(.a(x29), .b(new_n111_), .O(new_n152_));
  nand2  g069(.a(x30), .b(x08), .O(new_n153_));
  aoi21  g070(.a(new_n153_), .b(new_n152_), .c(x10), .O(z18));
  inv1   g071(.a(x30), .O(new_n155_));
  nand2  g072(.a(x08), .b(x00), .O(new_n156_));
  oai21  g073(.a(new_n155_), .b(x08), .c(new_n156_), .O(new_n157_));
  nand2  g074(.a(new_n157_), .b(new_n103_), .O(new_n158_));
  nand2  g075(.a(new_n158_), .b(new_n109_), .O(z19));
  inv1   g076(.a(x09), .O(new_n160_));
  nand2  g077(.a(x31), .b(new_n160_), .O(new_n161_));
  nand2  g078(.a(x32), .b(x09), .O(new_n162_));
  aoi21  g079(.a(new_n162_), .b(new_n161_), .c(x10), .O(z20));
  nand2  g080(.a(x32), .b(new_n160_), .O(new_n164_));
  nand2  g081(.a(x33), .b(x09), .O(new_n165_));
  aoi21  g082(.a(new_n165_), .b(new_n164_), .c(x10), .O(z21));
  nand2  g083(.a(x33), .b(new_n160_), .O(new_n167_));
  nand2  g084(.a(x34), .b(x09), .O(new_n168_));
  aoi21  g085(.a(new_n168_), .b(new_n167_), .c(x10), .O(z22));
  inv1   g086(.a(x35), .O(new_n170_));
  nand2  g087(.a(x34), .b(new_n160_), .O(new_n171_));
  oai21  g088(.a(new_n170_), .b(new_n160_), .c(new_n171_), .O(new_n172_));
  nand2  g089(.a(new_n172_), .b(new_n103_), .O(new_n173_));
  nand2  g090(.a(new_n173_), .b(new_n109_), .O(z23));
  nand2  g091(.a(x36), .b(x09), .O(new_n175_));
  oai21  g092(.a(new_n170_), .b(x09), .c(new_n175_), .O(new_n176_));
  nand2  g093(.a(new_n176_), .b(new_n103_), .O(new_n177_));
  nand2  g094(.a(new_n177_), .b(new_n109_), .O(z24));
  nand2  g095(.a(x36), .b(new_n160_), .O(new_n179_));
  nand2  g096(.a(x37), .b(x09), .O(new_n180_));
  aoi21  g097(.a(new_n180_), .b(new_n179_), .c(x10), .O(z25));
  inv1   g098(.a(x38), .O(new_n182_));
  nand2  g099(.a(x37), .b(new_n160_), .O(new_n183_));
  oai21  g100(.a(new_n182_), .b(new_n160_), .c(new_n183_), .O(new_n184_));
  nand2  g101(.a(new_n184_), .b(new_n103_), .O(new_n185_));
  nand2  g102(.a(new_n185_), .b(new_n109_), .O(z26));
  inv1   g103(.a(x39), .O(new_n187_));
  nand2  g104(.a(x14), .b(x00), .O(new_n188_));
  oai21  g105(.a(new_n187_), .b(x14), .c(new_n188_), .O(new_n189_));
  nor2   g106(.a(new_n182_), .b(x09), .O(new_n190_));
  aoi21  g107(.a(new_n189_), .b(x09), .c(new_n190_), .O(new_n191_));
  oai21  g108(.a(new_n191_), .b(x10), .c(new_n109_), .O(z27));
  inv1   g109(.a(x14), .O(new_n193_));
  aoi21  g110(.a(new_n193_), .b(x09), .c(new_n187_), .O(new_n194_));
  nand3  g111(.a(x40), .b(new_n193_), .c(x09), .O(new_n195_));
  inv1   g112(.a(new_n195_), .O(new_n196_));
  oai21  g113(.a(new_n196_), .b(new_n194_), .c(new_n103_), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(new_n109_), .O(z28));
  inv1   g115(.a(x40), .O(new_n199_));
  aoi21  g116(.a(new_n193_), .b(x09), .c(new_n199_), .O(new_n200_));
  nand3  g117(.a(x41), .b(new_n193_), .c(x09), .O(new_n201_));
  inv1   g118(.a(new_n201_), .O(new_n202_));
  oai21  g119(.a(new_n202_), .b(new_n200_), .c(new_n103_), .O(new_n203_));
  nand2  g120(.a(new_n203_), .b(new_n109_), .O(z29));
  nand2  g121(.a(new_n193_), .b(x09), .O(new_n205_));
  nand2  g122(.a(new_n205_), .b(x41), .O(new_n206_));
  nand3  g123(.a(x42), .b(new_n193_), .c(x09), .O(new_n207_));
  aoi21  g124(.a(new_n207_), .b(new_n206_), .c(x10), .O(z30));
  inv1   g125(.a(x42), .O(new_n209_));
  aoi21  g126(.a(new_n193_), .b(x09), .c(new_n209_), .O(new_n210_));
  nand3  g127(.a(x43), .b(new_n193_), .c(x09), .O(new_n211_));
  inv1   g128(.a(new_n211_), .O(new_n212_));
  oai21  g129(.a(new_n212_), .b(new_n210_), .c(new_n103_), .O(new_n213_));
  nand2  g130(.a(new_n213_), .b(new_n109_), .O(z31));
  inv1   g131(.a(x43), .O(new_n215_));
  aoi21  g132(.a(new_n193_), .b(x09), .c(new_n215_), .O(new_n216_));
  nand3  g133(.a(x44), .b(new_n193_), .c(x09), .O(new_n217_));
  inv1   g134(.a(new_n217_), .O(new_n218_));
  oai21  g135(.a(new_n218_), .b(new_n216_), .c(new_n103_), .O(new_n219_));
  nand2  g136(.a(new_n219_), .b(new_n109_), .O(z32));
  inv1   g137(.a(x44), .O(new_n221_));
  aoi21  g138(.a(new_n193_), .b(x09), .c(new_n221_), .O(new_n222_));
  nand3  g139(.a(x45), .b(new_n193_), .c(x09), .O(new_n223_));
  inv1   g140(.a(new_n223_), .O(new_n224_));
  oai21  g141(.a(new_n224_), .b(new_n222_), .c(new_n103_), .O(new_n225_));
  nand2  g142(.a(new_n225_), .b(new_n109_), .O(z33));
  nand2  g143(.a(new_n205_), .b(x45), .O(new_n227_));
  nand3  g144(.a(x46), .b(new_n193_), .c(x09), .O(new_n228_));
  aoi21  g145(.a(new_n228_), .b(new_n227_), .c(x10), .O(z34));
  nand2  g146(.a(new_n205_), .b(x46), .O(new_n230_));
  nand3  g147(.a(new_n193_), .b(x09), .c(x00), .O(new_n231_));
  aoi21  g148(.a(new_n231_), .b(new_n230_), .c(x10), .O(z35));
endmodule


