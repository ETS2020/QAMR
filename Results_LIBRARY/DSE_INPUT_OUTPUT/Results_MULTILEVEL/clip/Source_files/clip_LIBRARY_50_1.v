// Benchmark "FAU" written by ABC on Thu Aug 13 15:23:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8,
    z0, z1, z2, z3, z4  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8;
  output z0, z1, z2, z3, z4;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g000(.a(x0), .O(new_n15_));
  inv1   g001(.a(x5), .O(new_n16_));
  inv1   g002(.a(x6), .O(new_n17_));
  inv1   g003(.a(x4), .O(new_n18_));
  inv1   g004(.a(x7), .O(new_n19_));
  nand2  g005(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nor2   g006(.a(x8), .b(x3), .O(new_n21_));
  inv1   g007(.a(new_n21_), .O(new_n22_));
  nand4  g008(.a(new_n22_), .b(new_n20_), .c(new_n17_), .d(new_n16_), .O(new_n23_));
  aoi21  g009(.a(new_n23_), .b(x2), .c(new_n15_), .O(new_n24_));
  inv1   g010(.a(x3), .O(new_n25_));
  nand2  g011(.a(x7), .b(new_n18_), .O(new_n26_));
  inv1   g012(.a(new_n26_), .O(new_n27_));
  oai21  g013(.a(new_n27_), .b(x8), .c(new_n25_), .O(new_n28_));
  nand3  g014(.a(x8), .b(x7), .c(new_n18_), .O(new_n29_));
  nor2   g015(.a(new_n17_), .b(new_n16_), .O(new_n30_));
  and2   g016(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  aoi21  g017(.a(new_n31_), .b(new_n28_), .c(x2), .O(new_n32_));
  oai21  g018(.a(new_n32_), .b(new_n24_), .c(x1), .O(new_n33_));
  inv1   g019(.a(x2), .O(new_n34_));
  aoi21  g020(.a(x7), .b(x4), .c(x8), .O(new_n35_));
  nand3  g021(.a(x8), .b(x7), .c(x4), .O(new_n36_));
  oai21  g022(.a(new_n35_), .b(new_n25_), .c(new_n36_), .O(new_n37_));
  nand3  g023(.a(new_n37_), .b(new_n16_), .c(x0), .O(new_n38_));
  oai21  g024(.a(new_n34_), .b(x1), .c(new_n38_), .O(new_n39_));
  nand2  g025(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand2  g026(.a(new_n40_), .b(new_n33_), .O(z0));
  nor2   g027(.a(x7), .b(new_n18_), .O(new_n42_));
  nand2  g028(.a(x5), .b(new_n15_), .O(new_n43_));
  nand4  g029(.a(new_n43_), .b(x6), .c(x2), .d(x1), .O(new_n44_));
  nand2  g030(.a(x2), .b(x1), .O(new_n45_));
  nand2  g031(.a(new_n45_), .b(new_n17_), .O(new_n46_));
  nand2  g032(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g033(.a(new_n42_), .b(new_n27_), .c(new_n47_), .O(new_n48_));
  nand2  g034(.a(x7), .b(x4), .O(new_n49_));
  nand2  g035(.a(new_n49_), .b(new_n20_), .O(new_n50_));
  nand4  g036(.a(new_n50_), .b(x6), .c(new_n34_), .d(x1), .O(new_n51_));
  inv1   g037(.a(new_n20_), .O(new_n52_));
  nand2  g038(.a(x8), .b(x3), .O(new_n53_));
  oai21  g039(.a(new_n21_), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  nand3  g040(.a(new_n54_), .b(new_n17_), .c(new_n16_), .O(new_n55_));
  nand2  g041(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g042(.a(new_n56_), .b(x0), .O(new_n57_));
  nand2  g043(.a(x8), .b(new_n25_), .O(new_n58_));
  nand2  g044(.a(new_n58_), .b(x5), .O(new_n59_));
  nand3  g045(.a(new_n59_), .b(x6), .c(new_n34_), .O(new_n60_));
  nand2  g046(.a(new_n17_), .b(x2), .O(new_n61_));
  nand2  g047(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g048(.a(new_n62_), .b(new_n50_), .O(new_n63_));
  nand3  g049(.a(x8), .b(new_n19_), .c(x4), .O(new_n64_));
  nand2  g050(.a(new_n64_), .b(new_n26_), .O(new_n65_));
  nand2  g051(.a(new_n65_), .b(new_n25_), .O(new_n66_));
  nand2  g052(.a(new_n66_), .b(new_n29_), .O(new_n67_));
  nand3  g053(.a(new_n67_), .b(x6), .c(x2), .O(new_n68_));
  nand2  g054(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g055(.a(new_n69_), .b(x1), .O(new_n70_));
  nand3  g056(.a(new_n70_), .b(new_n57_), .c(new_n48_), .O(z1));
  nand3  g057(.a(new_n20_), .b(x2), .c(x1), .O(new_n72_));
  nand2  g058(.a(new_n72_), .b(new_n49_), .O(new_n73_));
  nand2  g059(.a(new_n73_), .b(new_n17_), .O(new_n74_));
  nand2  g060(.a(new_n19_), .b(x4), .O(new_n75_));
  oai21  g061(.a(new_n27_), .b(x2), .c(new_n75_), .O(new_n76_));
  nand3  g062(.a(new_n76_), .b(new_n43_), .c(x6), .O(new_n77_));
  nand2  g063(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g064(.a(new_n53_), .b(new_n22_), .O(new_n79_));
  nand2  g065(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nor2   g066(.a(x8), .b(new_n25_), .O(new_n81_));
  inv1   g067(.a(new_n81_), .O(new_n82_));
  nand2  g068(.a(new_n82_), .b(new_n58_), .O(new_n83_));
  oai21  g069(.a(x6), .b(x2), .c(x1), .O(new_n84_));
  nand3  g070(.a(new_n84_), .b(new_n83_), .c(new_n49_), .O(new_n85_));
  nand2  g071(.a(new_n19_), .b(new_n34_), .O(new_n86_));
  inv1   g072(.a(x8), .O(new_n87_));
  nand3  g073(.a(new_n43_), .b(new_n87_), .c(x3), .O(new_n88_));
  nand2  g074(.a(new_n88_), .b(new_n58_), .O(new_n89_));
  nand3  g075(.a(new_n89_), .b(new_n86_), .c(x6), .O(new_n90_));
  nand3  g076(.a(new_n83_), .b(new_n19_), .c(new_n17_), .O(new_n91_));
  nand2  g077(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g078(.a(new_n92_), .b(new_n18_), .O(new_n93_));
  nand4  g079(.a(new_n87_), .b(x7), .c(x6), .d(x2), .O(new_n94_));
  oai21  g080(.a(x6), .b(x5), .c(new_n94_), .O(new_n95_));
  nand2  g081(.a(new_n95_), .b(x3), .O(new_n96_));
  nand3  g082(.a(x8), .b(new_n17_), .c(new_n16_), .O(new_n97_));
  nand2  g083(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g084(.a(new_n87_), .b(new_n16_), .c(x3), .O(new_n99_));
  nand2  g085(.a(new_n99_), .b(new_n58_), .O(new_n100_));
  nand3  g086(.a(new_n100_), .b(x7), .c(x2), .O(new_n101_));
  aoi21  g087(.a(new_n101_), .b(x1), .c(new_n17_), .O(new_n102_));
  aoi21  g088(.a(new_n98_), .b(x0), .c(new_n102_), .O(new_n103_));
  nand4  g089(.a(new_n103_), .b(new_n93_), .c(new_n85_), .d(new_n80_), .O(z2));
  nand2  g090(.a(new_n73_), .b(new_n22_), .O(new_n105_));
  aoi21  g091(.a(new_n105_), .b(new_n53_), .c(x0), .O(new_n106_));
  aoi21  g092(.a(x7), .b(x4), .c(x2), .O(new_n107_));
  oai21  g093(.a(new_n107_), .b(new_n52_), .c(new_n53_), .O(new_n108_));
  nor2   g094(.a(new_n21_), .b(new_n16_), .O(new_n109_));
  aoi21  g095(.a(new_n109_), .b(new_n108_), .c(new_n15_), .O(new_n110_));
  oai21  g096(.a(new_n110_), .b(new_n106_), .c(new_n17_), .O(new_n111_));
  inv1   g097(.a(new_n58_), .O(new_n112_));
  nand2  g098(.a(new_n75_), .b(x2), .O(new_n113_));
  aoi21  g099(.a(new_n113_), .b(new_n26_), .c(new_n81_), .O(new_n114_));
  oai21  g100(.a(new_n114_), .b(new_n112_), .c(x0), .O(new_n115_));
  aoi21  g101(.a(x7), .b(new_n18_), .c(x2), .O(new_n116_));
  oai21  g102(.a(new_n116_), .b(new_n42_), .c(new_n58_), .O(new_n117_));
  nand2  g103(.a(new_n117_), .b(new_n82_), .O(new_n118_));
  nand3  g104(.a(new_n118_), .b(new_n16_), .c(new_n15_), .O(new_n119_));
  nand3  g105(.a(new_n119_), .b(new_n115_), .c(x1), .O(new_n120_));
  nand2  g106(.a(new_n120_), .b(x6), .O(new_n121_));
  inv1   g107(.a(x1), .O(new_n122_));
  nand4  g108(.a(new_n53_), .b(new_n49_), .c(new_n122_), .d(x0), .O(new_n123_));
  nand3  g109(.a(new_n123_), .b(new_n121_), .c(new_n111_), .O(z3));
  nand2  g110(.a(new_n16_), .b(x1), .O(new_n125_));
  aoi21  g111(.a(new_n118_), .b(new_n15_), .c(new_n125_), .O(new_n126_));
  nand3  g112(.a(new_n53_), .b(new_n45_), .c(new_n49_), .O(new_n127_));
  oai21  g113(.a(new_n52_), .b(new_n87_), .c(new_n25_), .O(new_n128_));
  nand3  g114(.a(new_n87_), .b(new_n19_), .c(new_n18_), .O(new_n129_));
  nand4  g115(.a(new_n129_), .b(new_n128_), .c(new_n127_), .d(x0), .O(new_n130_));
  nand2  g116(.a(new_n130_), .b(x5), .O(new_n131_));
  oai21  g117(.a(new_n126_), .b(new_n17_), .c(new_n131_), .O(z4));
endmodule


