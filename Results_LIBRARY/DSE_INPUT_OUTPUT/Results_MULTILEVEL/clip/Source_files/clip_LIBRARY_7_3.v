// Benchmark "FAU" written by ABC on Thu Aug 13 15:23:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8,
    z0, z1, z2, z3, z4  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8;
  output z0, z1, z2, z3, z4;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_;
  inv1   g000(.a(x2), .O(new_n15_));
  nor2   g001(.a(new_n15_), .b(x1), .O(new_n16_));
  nand2  g002(.a(new_n15_), .b(x1), .O(new_n17_));
  inv1   g003(.a(new_n17_), .O(new_n18_));
  inv1   g004(.a(x8), .O(new_n19_));
  nor2   g005(.a(new_n19_), .b(x3), .O(new_n20_));
  inv1   g006(.a(new_n20_), .O(new_n21_));
  nand3  g007(.a(new_n21_), .b(x6), .c(x5), .O(new_n22_));
  oai21  g008(.a(new_n18_), .b(new_n16_), .c(new_n22_), .O(new_n23_));
  nand2  g009(.a(new_n19_), .b(x3), .O(new_n24_));
  inv1   g010(.a(x4), .O(new_n25_));
  oai21  g011(.a(x7), .b(new_n25_), .c(x2), .O(new_n26_));
  nand2  g012(.a(x7), .b(new_n25_), .O(new_n27_));
  oai22  g013(.a(new_n27_), .b(new_n17_), .c(new_n26_), .d(x1), .O(new_n28_));
  nand2  g014(.a(new_n28_), .b(new_n24_), .O(new_n29_));
  inv1   g015(.a(x1), .O(new_n30_));
  nor2   g016(.a(x7), .b(x4), .O(new_n31_));
  nand2  g017(.a(x7), .b(x4), .O(new_n32_));
  oai21  g018(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  nor2   g019(.a(x8), .b(x3), .O(new_n34_));
  inv1   g020(.a(new_n34_), .O(new_n35_));
  nand2  g021(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g022(.a(x8), .b(x3), .O(new_n37_));
  aoi21  g023(.a(new_n37_), .b(new_n36_), .c(x6), .O(new_n38_));
  oai21  g024(.a(new_n38_), .b(x5), .c(x0), .O(new_n39_));
  nand3  g025(.a(new_n39_), .b(new_n29_), .c(new_n23_), .O(z0));
  inv1   g026(.a(x5), .O(new_n41_));
  inv1   g027(.a(x7), .O(new_n42_));
  nand2  g028(.a(new_n42_), .b(x4), .O(new_n43_));
  nand2  g029(.a(new_n27_), .b(new_n43_), .O(new_n44_));
  xnor2a g030(.a(x6), .b(x2), .O(new_n45_));
  oai21  g031(.a(new_n45_), .b(new_n30_), .c(new_n44_), .O(new_n46_));
  inv1   g032(.a(new_n31_), .O(new_n47_));
  nand2  g033(.a(new_n32_), .b(new_n47_), .O(new_n48_));
  nand2  g034(.a(x6), .b(new_n15_), .O(new_n49_));
  inv1   g035(.a(x6), .O(new_n50_));
  nand2  g036(.a(new_n50_), .b(x2), .O(new_n51_));
  nand2  g037(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand3  g038(.a(new_n52_), .b(new_n48_), .c(x1), .O(new_n53_));
  oai21  g039(.a(new_n34_), .b(new_n31_), .c(new_n37_), .O(new_n54_));
  nand3  g040(.a(new_n54_), .b(new_n50_), .c(x0), .O(new_n55_));
  nand3  g041(.a(new_n55_), .b(new_n53_), .c(new_n46_), .O(new_n56_));
  nand2  g042(.a(new_n56_), .b(new_n41_), .O(new_n57_));
  inv1   g043(.a(x0), .O(new_n58_));
  inv1   g044(.a(x3), .O(new_n59_));
  nand4  g045(.a(x8), .b(x6), .c(new_n59_), .d(new_n15_), .O(new_n60_));
  nand2  g046(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  nand3  g047(.a(new_n61_), .b(new_n48_), .c(x1), .O(new_n62_));
  nand2  g048(.a(x2), .b(x1), .O(new_n63_));
  nand3  g049(.a(new_n63_), .b(new_n44_), .c(new_n50_), .O(new_n64_));
  oai21  g050(.a(new_n50_), .b(new_n15_), .c(x1), .O(new_n65_));
  nand3  g051(.a(x8), .b(new_n42_), .c(x4), .O(new_n66_));
  aoi21  g052(.a(new_n66_), .b(new_n27_), .c(x3), .O(new_n67_));
  nor3   g053(.a(new_n19_), .b(new_n42_), .c(x4), .O(new_n68_));
  oai21  g054(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(new_n69_));
  nand3  g055(.a(new_n69_), .b(new_n64_), .c(new_n62_), .O(new_n70_));
  nand2  g056(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand2  g057(.a(new_n71_), .b(new_n57_), .O(z1));
  nand2  g058(.a(new_n35_), .b(x0), .O(new_n73_));
  oai21  g059(.a(x7), .b(x4), .c(x2), .O(new_n74_));
  oai21  g060(.a(new_n74_), .b(new_n30_), .c(new_n32_), .O(new_n75_));
  nand3  g061(.a(new_n75_), .b(new_n19_), .c(new_n59_), .O(new_n76_));
  aoi21  g062(.a(new_n76_), .b(new_n73_), .c(x6), .O(new_n77_));
  aoi21  g063(.a(x7), .b(new_n25_), .c(x2), .O(new_n78_));
  nand2  g064(.a(new_n78_), .b(x1), .O(new_n79_));
  nand2  g065(.a(new_n79_), .b(new_n43_), .O(new_n80_));
  nand2  g066(.a(new_n37_), .b(new_n35_), .O(new_n81_));
  nand2  g067(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  xor2a  g068(.a(x8), .b(x3), .O(new_n83_));
  nand3  g069(.a(new_n83_), .b(new_n43_), .c(new_n17_), .O(new_n84_));
  nand3  g070(.a(new_n83_), .b(x7), .c(new_n25_), .O(new_n85_));
  and2   g071(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g072(.a(new_n86_), .b(new_n82_), .c(new_n50_), .O(new_n87_));
  oai21  g073(.a(new_n87_), .b(new_n77_), .c(new_n41_), .O(new_n88_));
  nand2  g074(.a(new_n83_), .b(new_n15_), .O(new_n89_));
  oai21  g075(.a(new_n24_), .b(x1), .c(new_n89_), .O(new_n90_));
  nand2  g076(.a(new_n90_), .b(new_n32_), .O(new_n91_));
  nand2  g077(.a(x4), .b(x1), .O(new_n92_));
  nand3  g078(.a(new_n92_), .b(x8), .c(new_n59_), .O(new_n93_));
  nand3  g079(.a(new_n19_), .b(new_n25_), .c(x3), .O(new_n94_));
  aoi21  g080(.a(new_n94_), .b(new_n93_), .c(x7), .O(new_n95_));
  aoi21  g081(.a(new_n81_), .b(new_n75_), .c(new_n95_), .O(new_n96_));
  aoi21  g082(.a(new_n96_), .b(new_n91_), .c(x6), .O(new_n97_));
  nand2  g083(.a(x7), .b(x6), .O(new_n98_));
  aoi21  g084(.a(new_n98_), .b(x4), .c(x1), .O(new_n99_));
  aoi21  g085(.a(new_n27_), .b(new_n26_), .c(new_n50_), .O(new_n100_));
  oai21  g086(.a(new_n100_), .b(new_n99_), .c(x8), .O(new_n101_));
  nor2   g087(.a(new_n101_), .b(x3), .O(new_n102_));
  oai21  g088(.a(new_n102_), .b(new_n97_), .c(new_n58_), .O(new_n103_));
  nand2  g089(.a(new_n103_), .b(new_n88_), .O(z2));
  nor2   g090(.a(x7), .b(new_n25_), .O(new_n105_));
  aoi21  g091(.a(new_n78_), .b(x1), .c(new_n105_), .O(new_n106_));
  oai21  g092(.a(new_n106_), .b(new_n20_), .c(new_n24_), .O(new_n107_));
  nand3  g093(.a(new_n107_), .b(x6), .c(new_n58_), .O(new_n108_));
  nand3  g094(.a(new_n43_), .b(new_n24_), .c(new_n17_), .O(new_n109_));
  nand2  g095(.a(new_n27_), .b(new_n19_), .O(new_n110_));
  nand2  g096(.a(new_n110_), .b(new_n59_), .O(new_n111_));
  nor2   g097(.a(new_n68_), .b(new_n50_), .O(new_n112_));
  nand3  g098(.a(new_n112_), .b(new_n111_), .c(new_n109_), .O(new_n113_));
  nand2  g099(.a(new_n113_), .b(x0), .O(new_n114_));
  nand2  g100(.a(new_n114_), .b(new_n108_), .O(new_n115_));
  nand2  g101(.a(new_n115_), .b(new_n41_), .O(new_n116_));
  nand2  g102(.a(new_n75_), .b(new_n35_), .O(new_n117_));
  nand2  g103(.a(new_n117_), .b(new_n37_), .O(new_n118_));
  nand3  g104(.a(new_n118_), .b(new_n50_), .c(new_n58_), .O(new_n119_));
  nand2  g105(.a(new_n119_), .b(new_n116_), .O(z3));
  nand2  g106(.a(new_n108_), .b(new_n41_), .O(z4));
endmodule


