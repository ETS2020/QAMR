// Benchmark "FAU" written by ABC on Thu Aug 13 15:23:29 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8,
    z0, z1, z2, z3, z4  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8;
  output z0, z1, z2, z3, z4;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_;
  inv1   g000(.a(x0), .O(new_n15_));
  inv1   g001(.a(x5), .O(new_n16_));
  inv1   g002(.a(x6), .O(new_n17_));
  inv1   g003(.a(x2), .O(new_n18_));
  nor2   g004(.a(new_n18_), .b(x1), .O(new_n19_));
  nand2  g005(.a(new_n18_), .b(x1), .O(new_n20_));
  inv1   g006(.a(new_n20_), .O(new_n21_));
  nor2   g007(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  nor2   g008(.a(new_n22_), .b(new_n17_), .O(new_n23_));
  inv1   g009(.a(x1), .O(new_n24_));
  nor2   g010(.a(x7), .b(x4), .O(new_n25_));
  nand2  g011(.a(x7), .b(x4), .O(new_n26_));
  oai21  g012(.a(new_n25_), .b(new_n24_), .c(new_n26_), .O(new_n27_));
  inv1   g013(.a(x3), .O(new_n28_));
  inv1   g014(.a(x8), .O(new_n29_));
  nand2  g015(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g016(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  nand2  g017(.a(x8), .b(x3), .O(new_n32_));
  aoi21  g018(.a(new_n32_), .b(new_n31_), .c(x6), .O(new_n33_));
  aoi21  g019(.a(new_n33_), .b(new_n16_), .c(new_n23_), .O(new_n34_));
  nand3  g020(.a(x8), .b(x6), .c(new_n28_), .O(new_n35_));
  aoi21  g021(.a(new_n35_), .b(x5), .c(new_n22_), .O(new_n36_));
  nor2   g022(.a(x8), .b(new_n28_), .O(new_n37_));
  inv1   g023(.a(x7), .O(new_n38_));
  nand2  g024(.a(new_n38_), .b(x4), .O(new_n39_));
  nand3  g025(.a(new_n39_), .b(x2), .c(new_n24_), .O(new_n40_));
  inv1   g026(.a(x4), .O(new_n41_));
  nand2  g027(.a(x7), .b(new_n41_), .O(new_n42_));
  inv1   g028(.a(new_n42_), .O(new_n43_));
  nand2  g029(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  aoi21  g030(.a(new_n44_), .b(new_n40_), .c(new_n37_), .O(new_n45_));
  aoi21  g031(.a(new_n45_), .b(x6), .c(new_n36_), .O(new_n46_));
  oai21  g032(.a(new_n34_), .b(new_n15_), .c(new_n46_), .O(z0));
  nor2   g033(.a(x7), .b(new_n41_), .O(new_n48_));
  xnor2a g034(.a(x6), .b(x2), .O(new_n49_));
  oai22  g035(.a(new_n49_), .b(new_n24_), .c(new_n43_), .d(new_n48_), .O(new_n50_));
  inv1   g036(.a(new_n25_), .O(new_n51_));
  nand2  g037(.a(new_n26_), .b(new_n51_), .O(new_n52_));
  xor2a  g038(.a(x6), .b(x2), .O(new_n53_));
  nand3  g039(.a(new_n53_), .b(new_n52_), .c(x1), .O(new_n54_));
  nand2  g040(.a(new_n30_), .b(new_n51_), .O(new_n55_));
  nand2  g041(.a(new_n55_), .b(new_n32_), .O(new_n56_));
  nand3  g042(.a(new_n56_), .b(new_n17_), .c(x0), .O(new_n57_));
  nand3  g043(.a(new_n57_), .b(new_n54_), .c(new_n50_), .O(new_n58_));
  nand2  g044(.a(new_n58_), .b(new_n16_), .O(new_n59_));
  nand3  g045(.a(new_n52_), .b(new_n18_), .c(x1), .O(new_n60_));
  nand3  g046(.a(new_n20_), .b(new_n38_), .c(x4), .O(new_n61_));
  nand2  g047(.a(x8), .b(new_n28_), .O(new_n62_));
  aoi22  g048(.a(new_n62_), .b(new_n15_), .c(new_n61_), .d(new_n60_), .O(new_n63_));
  nand2  g049(.a(new_n37_), .b(new_n15_), .O(new_n64_));
  nand4  g050(.a(new_n64_), .b(new_n20_), .c(x7), .d(new_n41_), .O(new_n65_));
  inv1   g051(.a(new_n65_), .O(new_n66_));
  oai21  g052(.a(new_n66_), .b(new_n63_), .c(x6), .O(new_n67_));
  nand2  g053(.a(new_n67_), .b(new_n59_), .O(z1));
  nand2  g054(.a(new_n37_), .b(x0), .O(new_n69_));
  aoi21  g055(.a(new_n69_), .b(new_n62_), .c(new_n21_), .O(new_n70_));
  nand4  g056(.a(new_n29_), .b(new_n16_), .c(x3), .d(x2), .O(new_n71_));
  inv1   g057(.a(new_n71_), .O(new_n72_));
  oai21  g058(.a(new_n72_), .b(new_n70_), .c(new_n39_), .O(new_n73_));
  nand2  g059(.a(x5), .b(new_n15_), .O(new_n74_));
  nand3  g060(.a(new_n42_), .b(new_n18_), .c(x1), .O(new_n75_));
  aoi22  g061(.a(new_n75_), .b(new_n39_), .c(new_n32_), .d(new_n30_), .O(new_n76_));
  nand4  g062(.a(new_n29_), .b(x7), .c(new_n41_), .d(x3), .O(new_n77_));
  inv1   g063(.a(new_n77_), .O(new_n78_));
  oai21  g064(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  nand3  g065(.a(x8), .b(new_n41_), .c(new_n28_), .O(new_n80_));
  nand4  g066(.a(new_n29_), .b(new_n16_), .c(x3), .d(new_n24_), .O(new_n81_));
  nand2  g067(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g068(.a(new_n82_), .b(x7), .O(new_n83_));
  nand3  g069(.a(new_n83_), .b(new_n79_), .c(new_n73_), .O(new_n84_));
  nand2  g070(.a(new_n84_), .b(x6), .O(new_n85_));
  nand2  g071(.a(new_n30_), .b(x0), .O(new_n86_));
  nand2  g072(.a(new_n29_), .b(x3), .O(new_n87_));
  nand2  g073(.a(new_n62_), .b(new_n87_), .O(new_n88_));
  aoi21  g074(.a(x7), .b(x4), .c(x2), .O(new_n89_));
  aoi21  g075(.a(x4), .b(x1), .c(x7), .O(new_n90_));
  oai21  g076(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  nand2  g077(.a(new_n32_), .b(new_n30_), .O(new_n92_));
  oai21  g078(.a(x7), .b(x4), .c(x2), .O(new_n93_));
  oai21  g079(.a(new_n93_), .b(new_n24_), .c(new_n26_), .O(new_n94_));
  nand2  g080(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand3  g081(.a(new_n95_), .b(new_n91_), .c(new_n86_), .O(new_n96_));
  nand2  g082(.a(new_n96_), .b(new_n17_), .O(new_n97_));
  nand3  g083(.a(new_n88_), .b(new_n41_), .c(new_n24_), .O(new_n98_));
  nand2  g084(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g085(.a(new_n99_), .b(new_n16_), .O(new_n100_));
  nand2  g086(.a(new_n100_), .b(new_n85_), .O(z2));
  inv1   g087(.a(new_n62_), .O(new_n102_));
  aoi21  g088(.a(x7), .b(new_n41_), .c(x2), .O(new_n103_));
  aoi21  g089(.a(new_n103_), .b(x1), .c(new_n48_), .O(new_n104_));
  oai21  g090(.a(new_n104_), .b(new_n102_), .c(new_n87_), .O(new_n105_));
  nand3  g091(.a(new_n105_), .b(new_n16_), .c(new_n15_), .O(new_n106_));
  nand3  g092(.a(new_n39_), .b(new_n87_), .c(new_n20_), .O(new_n107_));
  oai21  g093(.a(new_n43_), .b(x8), .c(new_n28_), .O(new_n108_));
  nand3  g094(.a(x8), .b(x7), .c(new_n41_), .O(new_n109_));
  nand3  g095(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  nand2  g096(.a(new_n110_), .b(x0), .O(new_n111_));
  nand2  g097(.a(new_n111_), .b(new_n106_), .O(new_n112_));
  nand2  g098(.a(new_n112_), .b(x6), .O(new_n113_));
  nand2  g099(.a(new_n94_), .b(new_n30_), .O(new_n114_));
  nand3  g100(.a(new_n114_), .b(new_n32_), .c(new_n15_), .O(new_n115_));
  nand3  g101(.a(new_n115_), .b(new_n17_), .c(new_n16_), .O(new_n116_));
  nand2  g102(.a(new_n116_), .b(new_n113_), .O(z3));
  nand2  g103(.a(new_n105_), .b(new_n15_), .O(new_n118_));
  aoi21  g104(.a(new_n118_), .b(new_n16_), .c(new_n17_), .O(z4));
endmodule


