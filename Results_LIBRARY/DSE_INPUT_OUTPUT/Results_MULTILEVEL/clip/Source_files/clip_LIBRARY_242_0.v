// Benchmark "FAU" written by ABC on Thu Aug 13 15:23:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8,
    z0, z1, z2, z3, z4  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8;
  output z0, z1, z2, z3, z4;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g000(.a(x0), .O(new_n15_));
  inv1   g001(.a(x5), .O(new_n16_));
  inv1   g002(.a(x1), .O(new_n17_));
  nor2   g003(.a(x7), .b(x4), .O(new_n18_));
  nand2  g004(.a(x7), .b(x4), .O(new_n19_));
  oai21  g005(.a(new_n18_), .b(new_n17_), .c(new_n19_), .O(new_n20_));
  inv1   g006(.a(x3), .O(new_n21_));
  inv1   g007(.a(x8), .O(new_n22_));
  nand2  g008(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g009(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nand2  g010(.a(x8), .b(x3), .O(new_n25_));
  aoi21  g011(.a(new_n25_), .b(new_n24_), .c(x6), .O(new_n26_));
  nand2  g012(.a(x2), .b(new_n17_), .O(new_n27_));
  inv1   g013(.a(x2), .O(new_n28_));
  nand2  g014(.a(new_n28_), .b(x1), .O(new_n29_));
  nand2  g015(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  aoi21  g016(.a(new_n26_), .b(new_n16_), .c(new_n30_), .O(new_n31_));
  nand2  g017(.a(x8), .b(new_n21_), .O(new_n32_));
  aoi22  g018(.a(new_n32_), .b(x6), .c(new_n29_), .d(new_n27_), .O(new_n33_));
  inv1   g019(.a(x7), .O(new_n34_));
  aoi21  g020(.a(new_n34_), .b(x4), .c(new_n28_), .O(new_n35_));
  nand2  g021(.a(new_n35_), .b(new_n17_), .O(new_n36_));
  nor2   g022(.a(new_n34_), .b(x4), .O(new_n37_));
  nand3  g023(.a(new_n37_), .b(new_n28_), .c(x1), .O(new_n38_));
  aoi22  g024(.a(new_n38_), .b(new_n36_), .c(new_n22_), .d(x3), .O(new_n39_));
  oai21  g025(.a(new_n39_), .b(new_n33_), .c(x5), .O(new_n40_));
  oai21  g026(.a(new_n31_), .b(new_n15_), .c(new_n40_), .O(z0));
  nand2  g027(.a(new_n34_), .b(x4), .O(new_n42_));
  inv1   g028(.a(x4), .O(new_n43_));
  nand2  g029(.a(x7), .b(new_n43_), .O(new_n44_));
  nand2  g030(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  xnor2a g031(.a(x6), .b(x2), .O(new_n46_));
  oai21  g032(.a(new_n46_), .b(new_n17_), .c(new_n45_), .O(new_n47_));
  inv1   g033(.a(new_n18_), .O(new_n48_));
  nand2  g034(.a(new_n19_), .b(new_n48_), .O(new_n49_));
  nand2  g035(.a(x6), .b(new_n28_), .O(new_n50_));
  inv1   g036(.a(x6), .O(new_n51_));
  nand2  g037(.a(new_n51_), .b(x2), .O(new_n52_));
  nand2  g038(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand3  g039(.a(new_n53_), .b(new_n49_), .c(x1), .O(new_n54_));
  nand2  g040(.a(new_n23_), .b(new_n48_), .O(new_n55_));
  nand2  g041(.a(new_n55_), .b(new_n25_), .O(new_n56_));
  nand3  g042(.a(new_n56_), .b(new_n51_), .c(new_n16_), .O(new_n57_));
  nand3  g043(.a(new_n57_), .b(new_n54_), .c(new_n47_), .O(new_n58_));
  nand2  g044(.a(new_n58_), .b(x0), .O(new_n59_));
  nand4  g045(.a(x8), .b(x6), .c(new_n21_), .d(new_n28_), .O(new_n60_));
  nand2  g046(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nand3  g047(.a(new_n61_), .b(new_n49_), .c(x1), .O(new_n62_));
  nand2  g048(.a(x2), .b(x1), .O(new_n63_));
  nand3  g049(.a(new_n63_), .b(new_n45_), .c(new_n51_), .O(new_n64_));
  oai21  g050(.a(new_n51_), .b(new_n28_), .c(x1), .O(new_n65_));
  nand3  g051(.a(x8), .b(new_n34_), .c(x4), .O(new_n66_));
  aoi21  g052(.a(new_n66_), .b(new_n44_), .c(x3), .O(new_n67_));
  nor3   g053(.a(new_n22_), .b(new_n34_), .c(x4), .O(new_n68_));
  oai21  g054(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(new_n69_));
  nand3  g055(.a(new_n69_), .b(new_n64_), .c(new_n62_), .O(new_n70_));
  nand2  g056(.a(new_n70_), .b(x5), .O(new_n71_));
  nand2  g057(.a(new_n71_), .b(new_n59_), .O(z1));
  oai21  g058(.a(x7), .b(x4), .c(x2), .O(new_n73_));
  oai21  g059(.a(new_n73_), .b(new_n17_), .c(new_n19_), .O(new_n74_));
  nand3  g060(.a(new_n74_), .b(new_n22_), .c(new_n21_), .O(new_n75_));
  nand2  g061(.a(new_n23_), .b(new_n16_), .O(new_n76_));
  aoi21  g062(.a(new_n76_), .b(new_n75_), .c(x6), .O(new_n77_));
  nand3  g063(.a(new_n44_), .b(new_n28_), .c(x1), .O(new_n78_));
  nand2  g064(.a(new_n78_), .b(new_n42_), .O(new_n79_));
  nand2  g065(.a(new_n25_), .b(new_n23_), .O(new_n80_));
  nand2  g066(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  xor2a  g067(.a(x8), .b(x3), .O(new_n82_));
  nand3  g068(.a(new_n82_), .b(new_n42_), .c(new_n29_), .O(new_n83_));
  nand3  g069(.a(new_n82_), .b(x7), .c(new_n43_), .O(new_n84_));
  and2   g070(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g071(.a(new_n85_), .b(new_n81_), .c(new_n51_), .O(new_n86_));
  oai21  g072(.a(new_n86_), .b(new_n77_), .c(x0), .O(new_n87_));
  nand2  g073(.a(new_n82_), .b(new_n28_), .O(new_n88_));
  nand3  g074(.a(new_n22_), .b(x3), .c(new_n17_), .O(new_n89_));
  nand2  g075(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g076(.a(new_n90_), .b(new_n19_), .O(new_n91_));
  nand2  g077(.a(x4), .b(x1), .O(new_n92_));
  nand3  g078(.a(new_n92_), .b(x8), .c(new_n21_), .O(new_n93_));
  nand3  g079(.a(new_n22_), .b(new_n43_), .c(x3), .O(new_n94_));
  nand2  g080(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  aoi22  g081(.a(new_n95_), .b(new_n34_), .c(new_n80_), .d(new_n74_), .O(new_n96_));
  aoi21  g082(.a(new_n96_), .b(new_n91_), .c(x6), .O(new_n97_));
  nand2  g083(.a(x7), .b(x6), .O(new_n98_));
  aoi21  g084(.a(new_n98_), .b(x4), .c(x1), .O(new_n99_));
  oai21  g085(.a(x7), .b(new_n43_), .c(x2), .O(new_n100_));
  aoi21  g086(.a(new_n44_), .b(new_n100_), .c(new_n51_), .O(new_n101_));
  oai21  g087(.a(new_n101_), .b(new_n99_), .c(x8), .O(new_n102_));
  nor2   g088(.a(new_n102_), .b(x3), .O(new_n103_));
  oai21  g089(.a(new_n103_), .b(new_n97_), .c(x5), .O(new_n104_));
  nand2  g090(.a(new_n104_), .b(new_n87_), .O(z2));
  aoi21  g091(.a(x7), .b(x4), .c(x2), .O(new_n106_));
  aoi21  g092(.a(x4), .b(x1), .c(x7), .O(new_n107_));
  oai21  g093(.a(new_n107_), .b(new_n106_), .c(new_n25_), .O(new_n108_));
  oai21  g094(.a(x4), .b(x1), .c(x3), .O(new_n109_));
  aoi21  g095(.a(new_n109_), .b(new_n22_), .c(new_n16_), .O(new_n110_));
  aoi21  g096(.a(new_n110_), .b(new_n108_), .c(new_n15_), .O(new_n111_));
  nand2  g097(.a(new_n74_), .b(new_n23_), .O(new_n112_));
  aoi21  g098(.a(new_n112_), .b(new_n25_), .c(new_n16_), .O(new_n113_));
  aoi21  g099(.a(new_n113_), .b(new_n15_), .c(new_n111_), .O(new_n114_));
  inv1   g100(.a(new_n99_), .O(new_n115_));
  nand2  g101(.a(new_n44_), .b(new_n22_), .O(new_n116_));
  oai21  g102(.a(new_n116_), .b(new_n35_), .c(x6), .O(new_n117_));
  aoi21  g103(.a(new_n117_), .b(new_n115_), .c(x3), .O(new_n118_));
  aoi21  g104(.a(new_n42_), .b(new_n29_), .c(new_n37_), .O(new_n119_));
  nor3   g105(.a(new_n119_), .b(new_n22_), .c(new_n51_), .O(new_n120_));
  oai21  g106(.a(new_n120_), .b(new_n118_), .c(x0), .O(new_n121_));
  oai21  g107(.a(new_n114_), .b(x6), .c(new_n121_), .O(z3));
  nand3  g108(.a(new_n63_), .b(new_n25_), .c(new_n19_), .O(new_n123_));
  oai21  g109(.a(new_n18_), .b(new_n22_), .c(new_n21_), .O(new_n124_));
  nand3  g110(.a(new_n22_), .b(new_n34_), .c(new_n43_), .O(new_n125_));
  nor2   g111(.a(x6), .b(new_n15_), .O(new_n126_));
  nand4  g112(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n123_), .O(new_n127_));
  and2   g113(.a(new_n127_), .b(x5), .O(z4));
endmodule


