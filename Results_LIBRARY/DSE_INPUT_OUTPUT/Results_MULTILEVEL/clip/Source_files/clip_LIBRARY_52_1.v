// Benchmark "FAU" written by ABC on Thu Aug 13 15:23:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8,
    z0, z1, z2, z3, z4  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8;
  output z0, z1, z2, z3, z4;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g000(.a(x0), .O(new_n15_));
  inv1   g001(.a(x5), .O(new_n16_));
  inv1   g002(.a(x6), .O(new_n17_));
  inv1   g003(.a(x4), .O(new_n18_));
  inv1   g004(.a(x7), .O(new_n19_));
  nand2  g005(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g006(.a(x3), .O(new_n21_));
  inv1   g007(.a(x8), .O(new_n22_));
  nand2  g008(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand4  g009(.a(new_n23_), .b(new_n20_), .c(new_n17_), .d(new_n16_), .O(new_n24_));
  aoi21  g010(.a(new_n24_), .b(x2), .c(new_n15_), .O(new_n25_));
  nor2   g011(.a(new_n19_), .b(x4), .O(new_n26_));
  oai21  g012(.a(new_n26_), .b(x8), .c(new_n21_), .O(new_n27_));
  nand3  g013(.a(x8), .b(x7), .c(new_n18_), .O(new_n28_));
  nor2   g014(.a(new_n17_), .b(new_n16_), .O(new_n29_));
  and2   g015(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  aoi21  g016(.a(new_n30_), .b(new_n27_), .c(x2), .O(new_n31_));
  oai21  g017(.a(new_n31_), .b(new_n25_), .c(x1), .O(new_n32_));
  inv1   g018(.a(x1), .O(new_n33_));
  inv1   g019(.a(x2), .O(new_n34_));
  aoi21  g020(.a(x7), .b(x4), .c(x8), .O(new_n35_));
  nand3  g021(.a(x8), .b(x7), .c(x4), .O(new_n36_));
  oai21  g022(.a(new_n35_), .b(new_n21_), .c(new_n36_), .O(new_n37_));
  nand3  g023(.a(new_n37_), .b(new_n16_), .c(x0), .O(new_n38_));
  nand3  g024(.a(new_n38_), .b(new_n17_), .c(new_n34_), .O(new_n39_));
  nand2  g025(.a(new_n39_), .b(new_n33_), .O(new_n40_));
  nor2   g026(.a(new_n21_), .b(new_n15_), .O(new_n41_));
  nand4  g027(.a(new_n41_), .b(x8), .c(new_n17_), .d(new_n16_), .O(new_n42_));
  nand3  g028(.a(new_n42_), .b(new_n40_), .c(new_n32_), .O(z0));
  nor2   g029(.a(x7), .b(new_n18_), .O(new_n44_));
  nand2  g030(.a(x5), .b(new_n15_), .O(new_n45_));
  nand4  g031(.a(new_n45_), .b(x6), .c(x2), .d(x1), .O(new_n46_));
  nand2  g032(.a(x2), .b(x1), .O(new_n47_));
  nand2  g033(.a(new_n47_), .b(new_n17_), .O(new_n48_));
  nand2  g034(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  oai21  g035(.a(new_n44_), .b(new_n26_), .c(new_n49_), .O(new_n50_));
  nand2  g036(.a(x7), .b(x4), .O(new_n51_));
  nand2  g037(.a(new_n51_), .b(new_n20_), .O(new_n52_));
  nand4  g038(.a(new_n52_), .b(x6), .c(new_n34_), .d(x1), .O(new_n53_));
  nand2  g039(.a(new_n23_), .b(new_n20_), .O(new_n54_));
  nand2  g040(.a(x8), .b(x3), .O(new_n55_));
  nand2  g041(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g042(.a(new_n56_), .b(new_n17_), .c(new_n16_), .O(new_n57_));
  nand2  g043(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g044(.a(new_n58_), .b(x0), .O(new_n59_));
  nand2  g045(.a(x8), .b(new_n21_), .O(new_n60_));
  nand2  g046(.a(new_n60_), .b(x5), .O(new_n61_));
  nand3  g047(.a(new_n61_), .b(x6), .c(new_n34_), .O(new_n62_));
  nand2  g048(.a(new_n17_), .b(x2), .O(new_n63_));
  nand2  g049(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g050(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  nand2  g051(.a(x7), .b(new_n18_), .O(new_n66_));
  nand3  g052(.a(x8), .b(new_n19_), .c(x4), .O(new_n67_));
  nand2  g053(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g054(.a(new_n68_), .b(new_n21_), .O(new_n69_));
  nand2  g055(.a(new_n69_), .b(new_n28_), .O(new_n70_));
  nand3  g056(.a(new_n70_), .b(x6), .c(x2), .O(new_n71_));
  nand2  g057(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  nand2  g058(.a(new_n72_), .b(x1), .O(new_n73_));
  nand3  g059(.a(new_n73_), .b(new_n59_), .c(new_n50_), .O(z1));
  nand3  g060(.a(new_n23_), .b(new_n16_), .c(x0), .O(new_n75_));
  oai21  g061(.a(x7), .b(x4), .c(x2), .O(new_n76_));
  oai21  g062(.a(new_n76_), .b(new_n33_), .c(new_n51_), .O(new_n77_));
  nand2  g063(.a(new_n55_), .b(new_n23_), .O(new_n78_));
  nand2  g064(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g065(.a(new_n22_), .b(x3), .O(new_n80_));
  nand2  g066(.a(new_n80_), .b(new_n60_), .O(new_n81_));
  nand3  g067(.a(new_n81_), .b(new_n47_), .c(new_n51_), .O(new_n82_));
  nand3  g068(.a(new_n81_), .b(new_n19_), .c(new_n18_), .O(new_n83_));
  nand4  g069(.a(new_n83_), .b(new_n82_), .c(new_n79_), .d(new_n75_), .O(new_n84_));
  nand2  g070(.a(new_n84_), .b(new_n17_), .O(new_n85_));
  aoi21  g071(.a(new_n19_), .b(x4), .c(new_n34_), .O(new_n86_));
  nand3  g072(.a(new_n45_), .b(new_n22_), .c(x3), .O(new_n87_));
  nand2  g073(.a(new_n87_), .b(new_n60_), .O(new_n88_));
  oai21  g074(.a(new_n86_), .b(new_n26_), .c(new_n88_), .O(new_n89_));
  aoi21  g075(.a(x7), .b(new_n18_), .c(x2), .O(new_n90_));
  nor2   g076(.a(new_n90_), .b(new_n44_), .O(new_n91_));
  nand2  g077(.a(new_n78_), .b(new_n45_), .O(new_n92_));
  oai21  g078(.a(new_n92_), .b(new_n91_), .c(new_n89_), .O(new_n93_));
  nand3  g079(.a(new_n93_), .b(x6), .c(x1), .O(new_n94_));
  nand2  g080(.a(new_n94_), .b(new_n85_), .O(z2));
  nand3  g081(.a(new_n55_), .b(new_n47_), .c(new_n51_), .O(new_n96_));
  nand2  g082(.a(new_n20_), .b(x8), .O(new_n97_));
  nand2  g083(.a(new_n97_), .b(new_n21_), .O(new_n98_));
  nand3  g084(.a(new_n22_), .b(new_n19_), .c(new_n18_), .O(new_n99_));
  nand4  g085(.a(new_n99_), .b(new_n98_), .c(new_n96_), .d(x5), .O(new_n100_));
  nand2  g086(.a(new_n100_), .b(new_n17_), .O(new_n101_));
  oai21  g087(.a(new_n86_), .b(new_n26_), .c(new_n80_), .O(new_n102_));
  nand2  g088(.a(new_n102_), .b(new_n60_), .O(new_n103_));
  nand3  g089(.a(new_n103_), .b(x6), .c(x1), .O(new_n104_));
  nand2  g090(.a(new_n104_), .b(new_n101_), .O(new_n105_));
  nand2  g091(.a(new_n105_), .b(x0), .O(new_n106_));
  nand2  g092(.a(new_n77_), .b(new_n23_), .O(new_n107_));
  nand2  g093(.a(new_n107_), .b(new_n55_), .O(new_n108_));
  nand2  g094(.a(new_n108_), .b(new_n17_), .O(new_n109_));
  oai21  g095(.a(new_n90_), .b(new_n44_), .c(new_n60_), .O(new_n110_));
  nand2  g096(.a(new_n110_), .b(new_n80_), .O(new_n111_));
  nand4  g097(.a(new_n111_), .b(x6), .c(new_n16_), .d(x1), .O(new_n112_));
  nand2  g098(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  nand2  g099(.a(new_n113_), .b(new_n15_), .O(new_n114_));
  nand2  g100(.a(new_n114_), .b(new_n106_), .O(z3));
  aoi21  g101(.a(new_n110_), .b(new_n80_), .c(x0), .O(new_n116_));
  nand2  g102(.a(new_n16_), .b(x1), .O(new_n117_));
  oai21  g103(.a(new_n117_), .b(new_n116_), .c(x6), .O(new_n118_));
  nand4  g104(.a(new_n99_), .b(new_n98_), .c(new_n96_), .d(x0), .O(new_n119_));
  nand2  g105(.a(new_n119_), .b(x5), .O(new_n120_));
  nand2  g106(.a(new_n120_), .b(new_n118_), .O(z4));
endmodule


