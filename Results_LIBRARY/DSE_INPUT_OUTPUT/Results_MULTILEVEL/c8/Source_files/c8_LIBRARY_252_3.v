// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_;
  inv1   g000(.a(x08), .O(new_n47_));
  oai21  g001(.a(x18), .b(new_n47_), .c(x27), .O(new_n48_));
  oai21  g002(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g003(.a(x18), .O(new_n50_));
  nand2  g004(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(x27), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  oai21  g008(.a(x18), .b(new_n54_), .c(x27), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  nand2  g010(.a(new_n50_), .b(x11), .O(new_n57_));
  nand2  g011(.a(new_n57_), .b(x27), .O(new_n58_));
  oai21  g012(.a(x27), .b(x22), .c(new_n58_), .O(z03));
  inv1   g013(.a(x27), .O(new_n60_));
  nor2   g014(.a(new_n60_), .b(x18), .O(new_n61_));
  inv1   g015(.a(new_n61_), .O(new_n62_));
  oai22  g016(.a(new_n62_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g017(.a(new_n62_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g018(.a(new_n62_), .b(x14), .c(x27), .d(x25), .O(z06));
  nand2  g019(.a(new_n50_), .b(x15), .O(new_n66_));
  nand2  g020(.a(new_n66_), .b(x27), .O(new_n67_));
  oai21  g021(.a(x27), .b(x26), .c(new_n67_), .O(z07));
  inv1   g022(.a(x16), .O(new_n69_));
  inv1   g023(.a(x00), .O(new_n70_));
  nand2  g024(.a(new_n60_), .b(x18), .O(new_n71_));
  oai22  g025(.a(new_n71_), .b(new_n70_), .c(x18), .d(new_n47_), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  nand2  g027(.a(x27), .b(x18), .O(new_n74_));
  xnor2a g028(.a(x19), .b(x17), .O(new_n75_));
  nand3  g029(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  nand2  g030(.a(new_n76_), .b(new_n73_), .O(z09));
  nand2  g031(.a(x18), .b(x01), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n51_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n69_), .O(new_n80_));
  inv1   g034(.a(x20), .O(new_n81_));
  nor2   g035(.a(x19), .b(x17), .O(new_n82_));
  nor2   g036(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor3   g037(.a(x20), .b(x19), .c(x17), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n80_), .c(new_n74_), .O(z10));
  inv1   g040(.a(x02), .O(new_n87_));
  oai22  g041(.a(new_n71_), .b(new_n87_), .c(x18), .d(new_n54_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n69_), .O(new_n89_));
  inv1   g043(.a(x21), .O(new_n90_));
  nor2   g044(.a(x21), .b(x20), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n82_), .O(new_n92_));
  oai21  g046(.a(new_n84_), .b(new_n90_), .c(new_n92_), .O(new_n93_));
  nand3  g047(.a(new_n93_), .b(new_n74_), .c(x16), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n89_), .O(z11));
  nand2  g049(.a(x18), .b(x03), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n57_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n69_), .O(new_n98_));
  inv1   g052(.a(x19), .O(new_n99_));
  aoi21  g053(.a(x27), .b(x18), .c(x22), .O(new_n100_));
  nand4  g054(.a(new_n100_), .b(new_n90_), .c(new_n81_), .d(new_n99_), .O(new_n101_));
  nand2  g055(.a(new_n92_), .b(x22), .O(new_n102_));
  oai21  g056(.a(new_n101_), .b(x17), .c(new_n102_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x16), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n98_), .c(new_n74_), .O(z12));
  inv1   g059(.a(x12), .O(new_n106_));
  nand2  g060(.a(x18), .b(x04), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n106_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n69_), .O(new_n109_));
  inv1   g063(.a(x23), .O(new_n110_));
  nor3   g064(.a(x22), .b(x21), .c(x20), .O(new_n111_));
  aoi21  g065(.a(new_n111_), .b(new_n82_), .c(new_n110_), .O(new_n112_));
  nor3   g066(.a(x23), .b(x22), .c(x21), .O(new_n113_));
  and2   g067(.a(new_n113_), .b(new_n84_), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n109_), .c(new_n74_), .O(z13));
  inv1   g070(.a(x05), .O(new_n117_));
  nand2  g071(.a(new_n50_), .b(x13), .O(new_n118_));
  oai21  g072(.a(new_n71_), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n69_), .O(new_n120_));
  inv1   g074(.a(x24), .O(new_n121_));
  nor2   g075(.a(x22), .b(x21), .O(new_n122_));
  nand4  g076(.a(new_n122_), .b(new_n84_), .c(new_n121_), .d(new_n110_), .O(new_n123_));
  oai21  g077(.a(new_n114_), .b(new_n121_), .c(new_n123_), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n74_), .c(x16), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n120_), .O(z14));
  inv1   g080(.a(x06), .O(new_n127_));
  nand2  g081(.a(new_n50_), .b(x14), .O(new_n128_));
  oai21  g082(.a(new_n71_), .b(new_n127_), .c(new_n128_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n69_), .O(new_n130_));
  nand2  g084(.a(new_n123_), .b(x25), .O(new_n131_));
  nor2   g085(.a(x23), .b(x22), .O(new_n132_));
  nor2   g086(.a(x25), .b(x24), .O(new_n133_));
  nand4  g087(.a(new_n133_), .b(new_n132_), .c(new_n91_), .d(new_n82_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n74_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n130_), .O(z15));
  nand2  g091(.a(x18), .b(x07), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n66_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n69_), .O(new_n140_));
  nand2  g094(.a(new_n134_), .b(x26), .O(new_n141_));
  nor3   g095(.a(x26), .b(x25), .c(x24), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n132_), .O(new_n143_));
  oai21  g097(.a(new_n143_), .b(new_n92_), .c(new_n141_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x16), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n140_), .c(new_n74_), .O(z16));
  inv1   g100(.a(x17), .O(new_n147_));
  inv1   g101(.a(x25), .O(new_n148_));
  aoi21  g102(.a(x27), .b(x18), .c(x26), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n148_), .c(new_n121_), .d(new_n110_), .O(new_n150_));
  nor3   g104(.a(new_n150_), .b(x22), .c(x21), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n81_), .c(x19), .d(new_n147_), .O(new_n152_));
  nand2  g106(.a(new_n61_), .b(x17), .O(new_n153_));
  aoi21  g107(.a(new_n153_), .b(new_n152_), .c(new_n69_), .O(z17));
  buf    g108(.a(x27), .O(z08));
endmodule


