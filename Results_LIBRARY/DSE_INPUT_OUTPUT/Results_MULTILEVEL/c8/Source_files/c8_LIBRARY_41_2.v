// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n178_, new_n179_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  oai21  g002(.a(new_n47_), .b(x18), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g004(.a(x20), .O(new_n51_));
  nand2  g005(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  inv1   g006(.a(x09), .O(new_n53_));
  nand2  g007(.a(x27), .b(new_n53_), .O(new_n54_));
  nand2  g008(.a(new_n48_), .b(x18), .O(new_n55_));
  nand3  g009(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(z01));
  inv1   g010(.a(x21), .O(new_n57_));
  nand2  g011(.a(new_n47_), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x10), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  nand3  g014(.a(new_n60_), .b(new_n58_), .c(new_n55_), .O(z02));
  inv1   g015(.a(new_n55_), .O(new_n62_));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n47_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(z03));
  inv1   g021(.a(x23), .O(new_n68_));
  nand2  g022(.a(new_n47_), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x12), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  nand3  g025(.a(new_n71_), .b(new_n69_), .c(new_n55_), .O(z04));
  inv1   g026(.a(x24), .O(new_n73_));
  nand2  g027(.a(new_n47_), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x13), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  nand3  g030(.a(new_n76_), .b(new_n74_), .c(new_n55_), .O(z05));
  inv1   g031(.a(x14), .O(new_n78_));
  nand2  g032(.a(x27), .b(new_n78_), .O(new_n79_));
  inv1   g033(.a(x25), .O(new_n80_));
  nand2  g034(.a(new_n47_), .b(new_n80_), .O(new_n81_));
  aoi21  g035(.a(new_n81_), .b(new_n79_), .c(new_n62_), .O(z06));
  inv1   g036(.a(x15), .O(new_n83_));
  nand2  g037(.a(x27), .b(new_n83_), .O(new_n84_));
  inv1   g038(.a(x26), .O(new_n85_));
  nand2  g039(.a(new_n47_), .b(new_n85_), .O(new_n86_));
  aoi21  g040(.a(new_n86_), .b(new_n84_), .c(new_n62_), .O(z07));
  nand2  g041(.a(new_n55_), .b(new_n47_), .O(z08));
  inv1   g042(.a(x17), .O(new_n89_));
  nor2   g043(.a(new_n48_), .b(new_n89_), .O(new_n90_));
  nor3   g044(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  inv1   g046(.a(x16), .O(new_n93_));
  inv1   g047(.a(x00), .O(new_n94_));
  inv1   g048(.a(x08), .O(new_n95_));
  nand2  g049(.a(x19), .b(x18), .O(new_n96_));
  oai22  g050(.a(new_n96_), .b(new_n94_), .c(x18), .d(new_n95_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n93_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n92_), .O(z09));
  nand2  g053(.a(x18), .b(x01), .O(new_n100_));
  oai21  g054(.a(x18), .b(new_n53_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n93_), .O(new_n102_));
  nor2   g056(.a(x20), .b(x17), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g058(.a(new_n104_), .O(new_n105_));
  oai21  g059(.a(new_n105_), .b(x18), .c(new_n48_), .O(new_n106_));
  nor2   g060(.a(x19), .b(x17), .O(new_n107_));
  inv1   g061(.a(new_n107_), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(x20), .c(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n106_), .c(new_n102_), .O(z10));
  nand2  g064(.a(x18), .b(x02), .O(new_n111_));
  oai21  g065(.a(x18), .b(new_n59_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n93_), .O(new_n113_));
  oai21  g067(.a(new_n57_), .b(new_n93_), .c(x19), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x18), .O(new_n115_));
  nor3   g069(.a(x20), .b(x19), .c(x17), .O(new_n116_));
  nor2   g070(.a(x21), .b(x20), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n107_), .O(new_n118_));
  oai21  g072(.a(new_n116_), .b(new_n57_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n115_), .c(new_n113_), .O(z11));
  nand2  g075(.a(x18), .b(x03), .O(new_n122_));
  oai21  g076(.a(x18), .b(new_n63_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n93_), .O(new_n124_));
  oai21  g078(.a(new_n65_), .b(new_n93_), .c(x19), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x18), .O(new_n126_));
  inv1   g080(.a(new_n118_), .O(new_n127_));
  nor2   g081(.a(new_n127_), .b(new_n65_), .O(new_n128_));
  nor2   g082(.a(x22), .b(x21), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n51_), .O(new_n130_));
  nor2   g084(.a(new_n130_), .b(new_n108_), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n126_), .c(new_n124_), .O(z12));
  inv1   g087(.a(x04), .O(new_n134_));
  oai22  g088(.a(new_n96_), .b(new_n134_), .c(x18), .d(new_n70_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n93_), .O(new_n136_));
  nand2  g090(.a(new_n129_), .b(new_n103_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(x23), .O(new_n138_));
  nor2   g092(.a(x23), .b(x22), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n116_), .c(new_n57_), .O(new_n140_));
  aoi21  g094(.a(new_n140_), .b(new_n138_), .c(x18), .O(new_n141_));
  nor2   g095(.a(new_n68_), .b(new_n48_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n141_), .c(x16), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n136_), .O(z13));
  inv1   g098(.a(x05), .O(new_n145_));
  oai22  g099(.a(new_n96_), .b(new_n145_), .c(x18), .d(new_n75_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n93_), .O(new_n147_));
  nand3  g101(.a(new_n139_), .b(new_n103_), .c(new_n57_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(x24), .O(new_n149_));
  nand4  g103(.a(new_n129_), .b(new_n116_), .c(new_n73_), .d(new_n68_), .O(new_n150_));
  aoi21  g104(.a(new_n150_), .b(new_n149_), .c(x18), .O(new_n151_));
  nor2   g105(.a(new_n73_), .b(new_n48_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n151_), .c(x16), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n147_), .O(z14));
  nand2  g108(.a(x18), .b(x06), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n78_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n93_), .O(new_n157_));
  oai21  g111(.a(new_n80_), .b(new_n93_), .c(x19), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x18), .O(new_n159_));
  nand2  g113(.a(new_n150_), .b(x25), .O(new_n160_));
  nor2   g114(.a(x25), .b(x24), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n139_), .c(new_n127_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n159_), .c(new_n157_), .O(z15));
  inv1   g119(.a(x07), .O(new_n166_));
  oai22  g120(.a(new_n96_), .b(new_n166_), .c(x18), .d(new_n83_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n93_), .O(new_n168_));
  nand4  g122(.a(new_n161_), .b(new_n139_), .c(new_n117_), .d(new_n89_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(x26), .O(new_n170_));
  nor3   g124(.a(x26), .b(x25), .c(x24), .O(new_n171_));
  nand4  g125(.a(new_n171_), .b(new_n139_), .c(new_n117_), .d(new_n107_), .O(new_n172_));
  aoi21  g126(.a(new_n172_), .b(new_n170_), .c(x18), .O(new_n173_));
  nor2   g127(.a(new_n85_), .b(new_n48_), .O(new_n174_));
  oai21  g128(.a(new_n174_), .b(new_n173_), .c(x16), .O(new_n175_));
  nand2  g129(.a(new_n175_), .b(new_n168_), .O(z16));
  nand3  g130(.a(new_n55_), .b(x27), .c(x17), .O(new_n177_));
  nor2   g131(.a(new_n48_), .b(x17), .O(new_n178_));
  nand4  g132(.a(new_n178_), .b(new_n171_), .c(new_n139_), .d(new_n117_), .O(new_n179_));
  aoi21  g133(.a(new_n179_), .b(new_n177_), .c(new_n93_), .O(z17));
endmodule


