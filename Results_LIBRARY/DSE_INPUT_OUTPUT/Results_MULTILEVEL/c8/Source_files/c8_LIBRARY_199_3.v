// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x17), .O(new_n48_));
  inv1   g002(.a(x18), .O(new_n49_));
  nor2   g003(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g004(.a(x27), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  oai21  g005(.a(x27), .b(x19), .c(new_n51_), .O(z00));
  inv1   g006(.a(x09), .O(new_n53_));
  nand2  g007(.a(x27), .b(new_n53_), .O(new_n54_));
  inv1   g008(.a(x20), .O(new_n55_));
  inv1   g009(.a(x27), .O(new_n56_));
  nand2  g010(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g011(.a(new_n57_), .b(new_n54_), .c(new_n50_), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x21), .O(new_n61_));
  nand2  g015(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n60_), .c(new_n50_), .O(z02));
  inv1   g017(.a(x11), .O(new_n64_));
  nand2  g018(.a(x27), .b(new_n64_), .O(new_n65_));
  inv1   g019(.a(x22), .O(new_n66_));
  nand2  g020(.a(new_n56_), .b(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n65_), .c(new_n50_), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n50_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x23), .c(new_n70_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n50_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x24), .c(new_n73_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n50_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x25), .c(new_n76_), .O(z06));
  inv1   g031(.a(x15), .O(new_n78_));
  nand2  g032(.a(x27), .b(new_n78_), .O(new_n79_));
  inv1   g033(.a(x26), .O(new_n80_));
  nand2  g034(.a(new_n56_), .b(new_n80_), .O(new_n81_));
  aoi21  g035(.a(new_n81_), .b(new_n79_), .c(new_n50_), .O(z07));
  inv1   g036(.a(new_n50_), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n56_), .O(z08));
  nand3  g038(.a(x19), .b(new_n49_), .c(x17), .O(new_n85_));
  oai21  g039(.a(x19), .b(x17), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  inv1   g042(.a(x00), .O(new_n89_));
  nand2  g043(.a(x18), .b(new_n48_), .O(new_n90_));
  oai22  g044(.a(new_n90_), .b(new_n89_), .c(x18), .d(new_n47_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n87_), .O(z09));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n53_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  oai21  g050(.a(new_n55_), .b(new_n88_), .c(new_n49_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(x17), .O(new_n98_));
  inv1   g052(.a(x19), .O(new_n99_));
  nor2   g053(.a(x20), .b(x19), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n48_), .O(new_n101_));
  oai21  g055(.a(new_n55_), .b(new_n99_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x16), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n98_), .c(new_n96_), .O(z10));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  oai21  g059(.a(x18), .b(new_n59_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  oai21  g061(.a(new_n61_), .b(new_n88_), .c(new_n49_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x17), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n99_), .c(new_n48_), .O(new_n111_));
  oai21  g065(.a(new_n100_), .b(new_n61_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(x16), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n109_), .c(new_n107_), .O(z11));
  inv1   g068(.a(x03), .O(new_n115_));
  oai22  g069(.a(new_n90_), .b(new_n115_), .c(x18), .d(new_n64_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n88_), .O(new_n117_));
  nor3   g071(.a(x21), .b(x20), .c(x19), .O(new_n118_));
  nor2   g072(.a(x22), .b(x21), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n100_), .O(new_n120_));
  oai21  g074(.a(new_n118_), .b(new_n66_), .c(new_n120_), .O(new_n121_));
  nand3  g075(.a(x22), .b(new_n49_), .c(x17), .O(new_n122_));
  inv1   g076(.a(new_n122_), .O(new_n123_));
  aoi21  g077(.a(new_n121_), .b(new_n48_), .c(new_n123_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n88_), .c(new_n117_), .O(z12));
  inv1   g079(.a(x04), .O(new_n126_));
  oai22  g080(.a(new_n90_), .b(new_n126_), .c(x18), .d(new_n69_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n88_), .O(new_n128_));
  nand2  g082(.a(new_n120_), .b(x23), .O(new_n129_));
  nor2   g083(.a(x23), .b(x22), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n100_), .c(new_n61_), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n129_), .c(x17), .O(new_n132_));
  nand3  g086(.a(x23), .b(new_n49_), .c(x17), .O(new_n133_));
  inv1   g087(.a(new_n133_), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n128_), .O(z13));
  inv1   g090(.a(x05), .O(new_n137_));
  oai22  g091(.a(new_n90_), .b(new_n137_), .c(x18), .d(new_n72_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n88_), .O(new_n139_));
  nand2  g093(.a(new_n131_), .b(x24), .O(new_n140_));
  nor3   g094(.a(x24), .b(x23), .c(x22), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n118_), .O(new_n142_));
  aoi21  g096(.a(new_n142_), .b(new_n140_), .c(x17), .O(new_n143_));
  nand3  g097(.a(x24), .b(new_n49_), .c(x17), .O(new_n144_));
  inv1   g098(.a(new_n144_), .O(new_n145_));
  oai21  g099(.a(new_n145_), .b(new_n143_), .c(x16), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n139_), .O(z14));
  nand2  g101(.a(x18), .b(x06), .O(new_n148_));
  oai21  g102(.a(x18), .b(new_n75_), .c(new_n148_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n88_), .O(new_n150_));
  inv1   g104(.a(x25), .O(new_n151_));
  oai21  g105(.a(new_n151_), .b(new_n88_), .c(new_n49_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(x17), .O(new_n153_));
  aoi21  g107(.a(new_n141_), .b(new_n118_), .c(new_n151_), .O(new_n154_));
  nor2   g108(.a(x25), .b(x24), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n130_), .O(new_n156_));
  nor2   g110(.a(new_n156_), .b(new_n111_), .O(new_n157_));
  oai21  g111(.a(new_n157_), .b(new_n154_), .c(x16), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n153_), .c(new_n150_), .O(z15));
  inv1   g113(.a(x07), .O(new_n160_));
  oai22  g114(.a(new_n90_), .b(new_n160_), .c(x18), .d(new_n78_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n88_), .O(new_n162_));
  nand4  g116(.a(new_n155_), .b(new_n130_), .c(new_n110_), .d(new_n99_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x26), .O(new_n164_));
  nor2   g118(.a(x24), .b(x23), .O(new_n165_));
  nor2   g119(.a(x26), .b(x25), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n165_), .c(new_n119_), .d(new_n100_), .O(new_n167_));
  aoi21  g121(.a(new_n167_), .b(new_n164_), .c(x17), .O(new_n168_));
  nand3  g122(.a(x26), .b(new_n49_), .c(x17), .O(new_n169_));
  inv1   g123(.a(new_n169_), .O(new_n170_));
  oai21  g124(.a(new_n170_), .b(new_n168_), .c(x16), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n162_), .O(z16));
  nand3  g126(.a(new_n110_), .b(x19), .c(new_n48_), .O(new_n173_));
  inv1   g127(.a(x24), .O(new_n174_));
  nand3  g128(.a(new_n166_), .b(new_n130_), .c(new_n174_), .O(new_n175_));
  oai22  g129(.a(new_n175_), .b(new_n173_), .c(new_n56_), .d(new_n48_), .O(new_n176_));
  nand2  g130(.a(new_n176_), .b(x16), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(new_n83_), .O(z17));
endmodule


