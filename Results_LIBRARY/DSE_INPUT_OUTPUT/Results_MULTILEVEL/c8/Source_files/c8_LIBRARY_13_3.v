// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x24), .b(x19), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x21), .O(new_n57_));
  inv1   g011(.a(x27), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n56_), .c(new_n49_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  aoi21  g015(.a(x27), .b(new_n61_), .c(new_n49_), .O(new_n62_));
  oai21  g016(.a(x27), .b(x22), .c(new_n62_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x19), .O(new_n67_));
  inv1   g021(.a(x24), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n67_), .c(new_n68_), .O(new_n69_));
  oai22  g023(.a(new_n69_), .b(x13), .c(x27), .d(x24), .O(z05));
  inv1   g024(.a(x14), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n71_), .c(new_n49_), .O(new_n72_));
  oai21  g026(.a(x27), .b(x25), .c(new_n72_), .O(z06));
  inv1   g027(.a(x15), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n49_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x26), .c(new_n75_), .O(z07));
  nor2   g030(.a(new_n49_), .b(new_n58_), .O(z08));
  inv1   g031(.a(x16), .O(new_n78_));
  nand2  g032(.a(x18), .b(x00), .O(new_n79_));
  oai21  g033(.a(x18), .b(new_n47_), .c(new_n79_), .O(new_n80_));
  nand3  g034(.a(new_n80_), .b(new_n48_), .c(new_n78_), .O(new_n81_));
  nor2   g035(.a(x19), .b(x17), .O(new_n82_));
  nand3  g036(.a(new_n68_), .b(x19), .c(x17), .O(new_n83_));
  inv1   g037(.a(new_n83_), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n81_), .O(z09));
  nand2  g040(.a(x18), .b(x01), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n52_), .c(new_n87_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n48_), .c(new_n78_), .O(new_n89_));
  inv1   g043(.a(x17), .O(new_n90_));
  inv1   g044(.a(x20), .O(new_n91_));
  nor2   g045(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g046(.a(x20), .b(x17), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n92_), .c(new_n67_), .O(new_n94_));
  nand3  g048(.a(new_n68_), .b(x20), .c(x19), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(x16), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n89_), .O(z10));
  nand2  g052(.a(x18), .b(x02), .O(new_n99_));
  oai21  g053(.a(x18), .b(new_n55_), .c(new_n99_), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n48_), .c(new_n78_), .O(new_n101_));
  nand2  g055(.a(new_n91_), .b(new_n90_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x21), .O(new_n103_));
  nor2   g057(.a(x21), .b(x20), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n90_), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n103_), .c(x19), .O(new_n106_));
  nand3  g060(.a(new_n68_), .b(x21), .c(x19), .O(new_n107_));
  inv1   g061(.a(new_n107_), .O(new_n108_));
  oai21  g062(.a(new_n108_), .b(new_n106_), .c(x16), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n101_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(x18), .b(new_n61_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n78_), .O(new_n113_));
  inv1   g067(.a(x22), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n78_), .c(new_n68_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(x19), .O(new_n116_));
  nor3   g070(.a(x21), .b(x20), .c(x17), .O(new_n117_));
  nor2   g071(.a(x22), .b(x21), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n82_), .c(new_n91_), .O(new_n119_));
  oai21  g073(.a(new_n117_), .b(new_n114_), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n116_), .c(new_n113_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n64_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n78_), .O(new_n125_));
  inv1   g079(.a(x23), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n78_), .c(new_n68_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(x19), .O(new_n128_));
  aoi21  g082(.a(new_n118_), .b(new_n93_), .c(new_n126_), .O(new_n129_));
  nand3  g083(.a(new_n91_), .b(new_n67_), .c(new_n90_), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n57_), .O(new_n132_));
  nor2   g086(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n129_), .c(x16), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n128_), .c(new_n125_), .O(z13));
  inv1   g089(.a(x13), .O(new_n136_));
  nand2  g090(.a(x18), .b(x05), .O(new_n137_));
  oai21  g091(.a(x18), .b(new_n136_), .c(new_n137_), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n48_), .c(new_n78_), .O(new_n139_));
  oai21  g093(.a(new_n132_), .b(new_n102_), .c(x24), .O(new_n140_));
  nor3   g094(.a(x24), .b(x23), .c(x22), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n117_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n67_), .c(x16), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n139_), .O(z14));
  nand2  g099(.a(x18), .b(x06), .O(new_n146_));
  oai21  g100(.a(x18), .b(new_n71_), .c(new_n146_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n78_), .O(new_n148_));
  inv1   g102(.a(x25), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n78_), .c(new_n68_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(x19), .O(new_n151_));
  aoi21  g105(.a(new_n141_), .b(new_n117_), .c(new_n149_), .O(new_n152_));
  nand2  g106(.a(new_n104_), .b(new_n82_), .O(new_n153_));
  nor2   g107(.a(x25), .b(x24), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n131_), .O(new_n155_));
  nor2   g109(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n152_), .c(x16), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n151_), .c(new_n148_), .O(z15));
  nand2  g112(.a(x18), .b(x07), .O(new_n159_));
  oai21  g113(.a(x18), .b(new_n74_), .c(new_n159_), .O(new_n160_));
  nand3  g114(.a(new_n160_), .b(new_n48_), .c(new_n78_), .O(new_n161_));
  nand2  g115(.a(x26), .b(x19), .O(new_n162_));
  nor2   g116(.a(x26), .b(x25), .O(new_n163_));
  nand4  g117(.a(new_n163_), .b(new_n131_), .c(new_n104_), .d(new_n82_), .O(new_n164_));
  aoi21  g118(.a(new_n164_), .b(new_n162_), .c(x24), .O(new_n165_));
  nand4  g119(.a(new_n154_), .b(new_n131_), .c(new_n104_), .d(new_n90_), .O(new_n166_));
  nand3  g120(.a(new_n166_), .b(x26), .c(new_n67_), .O(new_n167_));
  inv1   g121(.a(new_n167_), .O(new_n168_));
  oai21  g122(.a(new_n168_), .b(new_n165_), .c(x16), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(new_n161_), .O(z16));
  nand2  g124(.a(z08), .b(x17), .O(new_n171_));
  nor2   g125(.a(new_n67_), .b(x17), .O(new_n172_));
  nor3   g126(.a(x26), .b(x25), .c(x24), .O(new_n173_));
  nand4  g127(.a(new_n173_), .b(new_n172_), .c(new_n131_), .d(new_n104_), .O(new_n174_));
  aoi21  g128(.a(new_n174_), .b(new_n171_), .c(new_n78_), .O(z17));
endmodule


