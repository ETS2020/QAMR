// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x17), .O(new_n48_));
  inv1   g002(.a(x18), .O(new_n49_));
  nor2   g003(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g004(.a(x27), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  oai21  g005(.a(x27), .b(x19), .c(new_n51_), .O(z00));
  inv1   g006(.a(x09), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n53_), .c(new_n50_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x21), .O(new_n58_));
  inv1   g012(.a(x27), .O(new_n59_));
  nand2  g013(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n57_), .c(new_n50_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n50_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n50_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n50_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x24), .c(new_n69_), .O(z05));
  inv1   g024(.a(x14), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x25), .O(new_n73_));
  nand2  g027(.a(new_n59_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n50_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x26), .O(new_n78_));
  nand2  g032(.a(new_n59_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n50_), .O(z07));
  inv1   g034(.a(new_n50_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n59_), .O(z08));
  nand3  g036(.a(x19), .b(new_n49_), .c(x17), .O(new_n83_));
  oai21  g037(.a(x19), .b(x17), .c(new_n83_), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(x16), .O(new_n85_));
  inv1   g039(.a(x16), .O(new_n86_));
  inv1   g040(.a(x00), .O(new_n87_));
  nand2  g041(.a(x18), .b(new_n48_), .O(new_n88_));
  oai22  g042(.a(new_n88_), .b(new_n87_), .c(x18), .d(new_n47_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n85_), .O(z09));
  inv1   g045(.a(x01), .O(new_n92_));
  oai22  g046(.a(new_n88_), .b(new_n92_), .c(x18), .d(new_n53_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n86_), .O(new_n94_));
  and2   g048(.a(x20), .b(x19), .O(new_n95_));
  nor2   g049(.a(x20), .b(x19), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n95_), .c(new_n48_), .O(new_n97_));
  nand3  g051(.a(x20), .b(new_n49_), .c(x17), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(x16), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n94_), .O(z10));
  nand2  g055(.a(x18), .b(x02), .O(new_n102_));
  oai21  g056(.a(x18), .b(new_n56_), .c(new_n102_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n86_), .O(new_n104_));
  oai21  g058(.a(new_n58_), .b(new_n86_), .c(new_n49_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(x17), .O(new_n106_));
  inv1   g060(.a(x19), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n107_), .c(new_n48_), .O(new_n109_));
  oai21  g063(.a(new_n96_), .b(new_n58_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n106_), .c(new_n104_), .O(z11));
  inv1   g066(.a(x03), .O(new_n113_));
  oai22  g067(.a(new_n88_), .b(new_n113_), .c(x18), .d(new_n62_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n86_), .O(new_n115_));
  inv1   g069(.a(x22), .O(new_n116_));
  nor3   g070(.a(x21), .b(x20), .c(x19), .O(new_n117_));
  nor2   g071(.a(x22), .b(x21), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n96_), .O(new_n119_));
  oai21  g073(.a(new_n117_), .b(new_n116_), .c(new_n119_), .O(new_n120_));
  nand3  g074(.a(x22), .b(new_n49_), .c(x17), .O(new_n121_));
  inv1   g075(.a(new_n121_), .O(new_n122_));
  aoi21  g076(.a(new_n120_), .b(new_n48_), .c(new_n122_), .O(new_n123_));
  oai21  g077(.a(new_n123_), .b(new_n86_), .c(new_n115_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n65_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n86_), .O(new_n127_));
  inv1   g081(.a(x23), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n86_), .c(new_n49_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(x17), .O(new_n130_));
  aoi21  g084(.a(new_n118_), .b(new_n96_), .c(new_n128_), .O(new_n131_));
  nand2  g085(.a(new_n96_), .b(new_n48_), .O(new_n132_));
  nor2   g086(.a(x23), .b(x22), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n58_), .O(new_n134_));
  nor2   g088(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n131_), .c(x16), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n130_), .c(new_n127_), .O(z13));
  inv1   g091(.a(x05), .O(new_n138_));
  oai22  g092(.a(new_n88_), .b(new_n138_), .c(x18), .d(new_n68_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n86_), .O(new_n140_));
  nand3  g094(.a(new_n133_), .b(new_n96_), .c(new_n58_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x24), .O(new_n142_));
  nor3   g096(.a(x24), .b(x23), .c(x22), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n117_), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(new_n142_), .c(x17), .O(new_n145_));
  nand3  g099(.a(x24), .b(new_n49_), .c(x17), .O(new_n146_));
  inv1   g100(.a(new_n146_), .O(new_n147_));
  oai21  g101(.a(new_n147_), .b(new_n145_), .c(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n140_), .O(z14));
  nand2  g103(.a(x18), .b(x06), .O(new_n150_));
  oai21  g104(.a(x18), .b(new_n71_), .c(new_n150_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n86_), .O(new_n152_));
  oai21  g106(.a(new_n73_), .b(new_n86_), .c(new_n49_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(x17), .O(new_n154_));
  aoi21  g108(.a(new_n143_), .b(new_n117_), .c(new_n73_), .O(new_n155_));
  nor2   g109(.a(x25), .b(x24), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n133_), .O(new_n157_));
  nor2   g111(.a(new_n157_), .b(new_n109_), .O(new_n158_));
  oai21  g112(.a(new_n158_), .b(new_n155_), .c(x16), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(new_n154_), .c(new_n152_), .O(z15));
  inv1   g114(.a(x07), .O(new_n161_));
  oai22  g115(.a(new_n88_), .b(new_n161_), .c(x18), .d(new_n76_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n86_), .O(new_n163_));
  nand4  g117(.a(new_n156_), .b(new_n133_), .c(new_n108_), .d(new_n107_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(x26), .O(new_n165_));
  nor2   g119(.a(x24), .b(x23), .O(new_n166_));
  nor2   g120(.a(x26), .b(x25), .O(new_n167_));
  nand4  g121(.a(new_n167_), .b(new_n166_), .c(new_n118_), .d(new_n96_), .O(new_n168_));
  aoi21  g122(.a(new_n168_), .b(new_n165_), .c(x17), .O(new_n169_));
  nand3  g123(.a(x26), .b(new_n49_), .c(x17), .O(new_n170_));
  inv1   g124(.a(new_n170_), .O(new_n171_));
  oai21  g125(.a(new_n171_), .b(new_n169_), .c(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n163_), .O(z16));
  nand3  g127(.a(new_n108_), .b(x19), .c(new_n48_), .O(new_n174_));
  inv1   g128(.a(x24), .O(new_n175_));
  nand3  g129(.a(new_n167_), .b(new_n133_), .c(new_n175_), .O(new_n176_));
  oai22  g130(.a(new_n176_), .b(new_n174_), .c(new_n59_), .d(new_n48_), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(x16), .O(new_n178_));
  nand2  g132(.a(new_n178_), .b(new_n81_), .O(z17));
endmodule


