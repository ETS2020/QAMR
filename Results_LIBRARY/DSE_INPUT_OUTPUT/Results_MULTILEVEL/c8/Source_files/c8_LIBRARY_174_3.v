// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x08), .O(new_n47_));
  oai21  g001(.a(x18), .b(new_n47_), .c(x27), .O(new_n48_));
  oai21  g002(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g003(.a(x18), .O(new_n50_));
  nand2  g004(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(x27), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  nand3  g008(.a(x27), .b(new_n50_), .c(new_n54_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  nand2  g010(.a(new_n50_), .b(x11), .O(new_n57_));
  nand2  g011(.a(new_n57_), .b(x27), .O(new_n58_));
  oai21  g012(.a(x27), .b(x22), .c(new_n58_), .O(z03));
  nand2  g013(.a(new_n50_), .b(x12), .O(new_n60_));
  nand2  g014(.a(new_n60_), .b(x27), .O(new_n61_));
  oai21  g015(.a(x27), .b(x23), .c(new_n61_), .O(z04));
  nand2  g016(.a(new_n50_), .b(x13), .O(new_n63_));
  nand2  g017(.a(new_n63_), .b(x27), .O(new_n64_));
  oai21  g018(.a(x27), .b(x24), .c(new_n64_), .O(z05));
  nand2  g019(.a(new_n50_), .b(x14), .O(new_n66_));
  nand2  g020(.a(new_n66_), .b(x27), .O(new_n67_));
  oai21  g021(.a(x27), .b(x25), .c(new_n67_), .O(z06));
  inv1   g022(.a(x15), .O(new_n69_));
  oai21  g023(.a(x18), .b(new_n69_), .c(x27), .O(new_n70_));
  oai21  g024(.a(x27), .b(x26), .c(new_n70_), .O(z07));
  inv1   g025(.a(x16), .O(new_n72_));
  inv1   g026(.a(x00), .O(new_n73_));
  inv1   g027(.a(x27), .O(new_n74_));
  nand2  g028(.a(new_n74_), .b(x18), .O(new_n75_));
  oai22  g029(.a(new_n75_), .b(new_n73_), .c(x18), .d(new_n47_), .O(new_n76_));
  nand2  g030(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  nand2  g031(.a(x27), .b(x18), .O(new_n78_));
  xnor2a g032(.a(x19), .b(x17), .O(new_n79_));
  nand3  g033(.a(new_n79_), .b(new_n78_), .c(x16), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n77_), .O(z09));
  nand2  g035(.a(x18), .b(x01), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n51_), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n72_), .O(new_n84_));
  inv1   g038(.a(x20), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  nor2   g040(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nor3   g041(.a(x20), .b(x19), .c(x17), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n84_), .c(new_n78_), .O(z10));
  inv1   g044(.a(x02), .O(new_n91_));
  oai22  g045(.a(new_n75_), .b(new_n91_), .c(x18), .d(new_n54_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n72_), .O(new_n93_));
  inv1   g047(.a(x21), .O(new_n94_));
  inv1   g048(.a(x17), .O(new_n95_));
  inv1   g049(.a(x19), .O(new_n96_));
  nand4  g050(.a(new_n94_), .b(new_n85_), .c(new_n96_), .d(new_n95_), .O(new_n97_));
  oai21  g051(.a(new_n88_), .b(new_n94_), .c(new_n97_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n78_), .c(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n93_), .O(z11));
  nand2  g054(.a(x18), .b(x03), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n57_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n72_), .O(new_n103_));
  aoi21  g057(.a(x27), .b(x18), .c(x22), .O(new_n104_));
  nand4  g058(.a(new_n104_), .b(new_n94_), .c(new_n85_), .d(new_n96_), .O(new_n105_));
  nand2  g059(.a(new_n97_), .b(x22), .O(new_n106_));
  oai21  g060(.a(new_n105_), .b(x17), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(x16), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n103_), .c(new_n78_), .O(z12));
  nand2  g063(.a(x18), .b(x04), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n60_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n72_), .O(new_n112_));
  nor2   g066(.a(x22), .b(x21), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n86_), .c(new_n85_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x23), .O(new_n115_));
  nor3   g069(.a(x23), .b(x22), .c(x21), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n88_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x16), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n112_), .c(new_n78_), .O(z13));
  nand2  g074(.a(x18), .b(x05), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  inv1   g077(.a(x24), .O(new_n124_));
  aoi21  g078(.a(new_n116_), .b(new_n88_), .c(new_n124_), .O(new_n125_));
  nor2   g079(.a(x20), .b(x19), .O(new_n126_));
  nor2   g080(.a(x24), .b(x23), .O(new_n127_));
  nand4  g081(.a(new_n127_), .b(new_n113_), .c(new_n126_), .d(new_n95_), .O(new_n128_));
  inv1   g082(.a(new_n128_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n125_), .c(x16), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n123_), .c(new_n78_), .O(z14));
  nand2  g085(.a(x18), .b(x06), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n66_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n72_), .O(new_n134_));
  nand2  g088(.a(new_n128_), .b(x25), .O(new_n135_));
  nor2   g089(.a(x21), .b(x20), .O(new_n136_));
  nor2   g090(.a(x23), .b(x22), .O(new_n137_));
  nor2   g091(.a(x25), .b(x24), .O(new_n138_));
  nand4  g092(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n86_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x16), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n134_), .c(new_n78_), .O(z15));
  inv1   g096(.a(x07), .O(new_n143_));
  oai22  g097(.a(new_n75_), .b(new_n143_), .c(x18), .d(new_n69_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n72_), .O(new_n145_));
  nand2  g099(.a(new_n139_), .b(x26), .O(new_n146_));
  nor3   g100(.a(x26), .b(x25), .c(x24), .O(new_n147_));
  nand4  g101(.a(new_n147_), .b(new_n137_), .c(new_n136_), .d(new_n86_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand3  g103(.a(new_n149_), .b(new_n78_), .c(x16), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n145_), .O(z16));
  nor2   g105(.a(x26), .b(x25), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n137_), .c(new_n124_), .O(new_n153_));
  nand3  g107(.a(new_n136_), .b(x19), .c(new_n95_), .O(new_n154_));
  oai22  g108(.a(new_n154_), .b(new_n153_), .c(new_n74_), .d(new_n95_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n78_), .O(z17));
  buf    g111(.a(x27), .O(z08));
endmodule


