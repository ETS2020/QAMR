// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x23), .b(x18), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  aoi21  g005(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n48_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g010(.a(x11), .O(new_n57_));
  aoi21  g011(.a(x27), .b(new_n57_), .c(new_n48_), .O(new_n58_));
  oai21  g012(.a(x27), .b(x22), .c(new_n58_), .O(z03));
  inv1   g013(.a(new_n48_), .O(new_n60_));
  nand2  g014(.a(new_n60_), .b(x27), .O(new_n61_));
  inv1   g015(.a(x23), .O(new_n62_));
  inv1   g016(.a(x27), .O(new_n63_));
  nand3  g017(.a(new_n63_), .b(new_n62_), .c(x18), .O(new_n64_));
  oai21  g018(.a(new_n61_), .b(x12), .c(new_n64_), .O(z04));
  inv1   g019(.a(x24), .O(new_n66_));
  inv1   g020(.a(x18), .O(new_n67_));
  nand2  g021(.a(new_n62_), .b(new_n67_), .O(new_n68_));
  nand3  g022(.a(new_n68_), .b(new_n63_), .c(new_n66_), .O(new_n69_));
  oai21  g023(.a(new_n61_), .b(x13), .c(new_n69_), .O(z05));
  inv1   g024(.a(x14), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x25), .O(new_n73_));
  nand2  g027(.a(new_n63_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n48_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n48_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x26), .c(new_n77_), .O(z07));
  inv1   g032(.a(new_n61_), .O(z08));
  inv1   g033(.a(x17), .O(new_n80_));
  inv1   g034(.a(x19), .O(new_n81_));
  nor2   g035(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g036(.a(x19), .b(x17), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n82_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x16), .O(new_n85_));
  nand2  g039(.a(x18), .b(x00), .O(new_n86_));
  oai21  g040(.a(x18), .b(new_n47_), .c(new_n86_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n84_), .c(new_n60_), .O(z09));
  nand2  g043(.a(x18), .b(x01), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n51_), .c(new_n90_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n85_), .O(new_n92_));
  inv1   g046(.a(x20), .O(new_n93_));
  nor2   g047(.a(new_n83_), .b(new_n93_), .O(new_n94_));
  nor2   g048(.a(x20), .b(x19), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n80_), .O(new_n96_));
  inv1   g050(.a(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n94_), .c(x16), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n92_), .c(new_n60_), .O(z10));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  nand2  g054(.a(x23), .b(new_n67_), .O(new_n101_));
  oai21  g055(.a(new_n101_), .b(new_n54_), .c(new_n100_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n85_), .O(new_n103_));
  nand2  g057(.a(new_n96_), .b(x21), .O(new_n104_));
  nor2   g058(.a(x21), .b(x20), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n83_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n60_), .c(x16), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n103_), .O(z11));
  nand2  g063(.a(x18), .b(x03), .O(new_n110_));
  oai21  g064(.a(new_n101_), .b(new_n57_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n85_), .O(new_n112_));
  nand2  g066(.a(new_n106_), .b(x22), .O(new_n113_));
  nor2   g067(.a(x22), .b(x21), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n83_), .c(new_n93_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n60_), .c(x16), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n112_), .O(z12));
  inv1   g072(.a(x12), .O(new_n119_));
  nand2  g073(.a(x18), .b(x04), .O(new_n120_));
  oai21  g074(.a(new_n101_), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n85_), .O(new_n122_));
  and2   g076(.a(new_n115_), .b(x23), .O(new_n123_));
  nand3  g077(.a(new_n81_), .b(x18), .c(new_n80_), .O(new_n124_));
  nor2   g078(.a(x23), .b(x22), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n105_), .O(new_n126_));
  nor2   g080(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n123_), .c(x16), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n122_), .O(z13));
  inv1   g083(.a(x13), .O(new_n130_));
  nand2  g084(.a(x18), .b(x05), .O(new_n131_));
  oai21  g085(.a(x18), .b(new_n130_), .c(new_n131_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n85_), .O(new_n133_));
  oai21  g087(.a(new_n66_), .b(new_n85_), .c(x23), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n67_), .O(new_n135_));
  nor3   g089(.a(x23), .b(x22), .c(x21), .O(new_n136_));
  aoi21  g090(.a(new_n136_), .b(new_n97_), .c(new_n66_), .O(new_n137_));
  nand3  g091(.a(new_n114_), .b(new_n66_), .c(new_n62_), .O(new_n138_));
  nor2   g092(.a(new_n138_), .b(new_n96_), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n137_), .c(x16), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n135_), .c(new_n133_), .O(z14));
  nand2  g095(.a(x18), .b(x06), .O(new_n142_));
  oai21  g096(.a(x18), .b(new_n71_), .c(new_n142_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n85_), .O(new_n144_));
  oai21  g098(.a(new_n73_), .b(new_n85_), .c(x23), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n67_), .O(new_n146_));
  nor2   g100(.a(x25), .b(x24), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n125_), .O(new_n148_));
  oai22  g102(.a(new_n148_), .b(new_n106_), .c(new_n139_), .d(new_n73_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(x16), .O(new_n150_));
  nand3  g104(.a(new_n150_), .b(new_n146_), .c(new_n144_), .O(z15));
  nand2  g105(.a(x18), .b(x07), .O(new_n152_));
  oai21  g106(.a(new_n101_), .b(new_n76_), .c(new_n152_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n85_), .O(new_n154_));
  nand4  g108(.a(new_n147_), .b(new_n114_), .c(new_n95_), .d(new_n80_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x26), .O(new_n156_));
  nor3   g110(.a(x26), .b(x25), .c(x24), .O(new_n157_));
  nand4  g111(.a(new_n157_), .b(new_n125_), .c(new_n105_), .d(new_n83_), .O(new_n158_));
  aoi21  g112(.a(new_n158_), .b(new_n156_), .c(new_n67_), .O(new_n159_));
  and2   g113(.a(x26), .b(x23), .O(new_n160_));
  oai21  g114(.a(new_n160_), .b(new_n159_), .c(x16), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n154_), .O(z16));
  nand2  g116(.a(new_n157_), .b(new_n125_), .O(new_n163_));
  nand3  g117(.a(new_n105_), .b(x19), .c(new_n80_), .O(new_n164_));
  oai22  g118(.a(new_n164_), .b(new_n163_), .c(new_n63_), .d(new_n80_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x16), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n60_), .O(z17));
endmodule


