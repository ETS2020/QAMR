// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g00(.a(x18), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x16), .O(new_n48_));
  inv1   g02(.a(x08), .O(new_n49_));
  nand2  g03(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g04(.a(x19), .O(new_n51_));
  inv1   g05(.a(x27), .O(new_n52_));
  nand2  g06(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g07(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g08(.a(x09), .O(new_n55_));
  nand2  g09(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g10(.a(x20), .O(new_n57_));
  nand2  g11(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  aoi21  g12(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(z01));
  inv1   g13(.a(x10), .O(new_n60_));
  nand2  g14(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g15(.a(x21), .O(new_n62_));
  nand2  g16(.a(new_n52_), .b(new_n62_), .O(new_n63_));
  aoi21  g17(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(z02));
  inv1   g18(.a(x11), .O(new_n65_));
  nand2  g19(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g20(.a(x22), .O(new_n67_));
  nand2  g21(.a(new_n52_), .b(new_n67_), .O(new_n68_));
  aoi21  g22(.a(new_n68_), .b(new_n66_), .c(new_n48_), .O(z03));
  inv1   g23(.a(x12), .O(new_n70_));
  nand2  g24(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g25(.a(x23), .O(new_n72_));
  nand2  g26(.a(new_n52_), .b(new_n72_), .O(new_n73_));
  aoi21  g27(.a(new_n73_), .b(new_n71_), .c(new_n48_), .O(z04));
  inv1   g28(.a(x13), .O(new_n75_));
  aoi21  g29(.a(x27), .b(new_n75_), .c(new_n48_), .O(new_n76_));
  oai21  g30(.a(x27), .b(x24), .c(new_n76_), .O(z05));
  inv1   g31(.a(x14), .O(new_n78_));
  nand2  g32(.a(x27), .b(new_n78_), .O(new_n79_));
  inv1   g33(.a(x25), .O(new_n80_));
  nand2  g34(.a(new_n52_), .b(new_n80_), .O(new_n81_));
  aoi21  g35(.a(new_n81_), .b(new_n79_), .c(new_n48_), .O(z06));
  inv1   g36(.a(x15), .O(new_n83_));
  aoi21  g37(.a(x27), .b(new_n83_), .c(new_n48_), .O(new_n84_));
  oai21  g38(.a(x27), .b(x26), .c(new_n84_), .O(z07));
  nor2   g39(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g40(.a(x17), .O(new_n87_));
  nor2   g41(.a(new_n51_), .b(new_n87_), .O(new_n88_));
  nor2   g42(.a(x19), .b(x17), .O(new_n89_));
  oai21  g43(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  inv1   g44(.a(x16), .O(new_n91_));
  oai21  g45(.a(x18), .b(x08), .c(new_n91_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(new_n90_), .O(z09));
  nor2   g47(.a(new_n89_), .b(new_n57_), .O(new_n94_));
  nor3   g48(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  oai21  g49(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  nor2   g50(.a(x18), .b(x16), .O(new_n97_));
  inv1   g51(.a(new_n97_), .O(new_n98_));
  oai21  g52(.a(new_n98_), .b(new_n55_), .c(new_n96_), .O(z10));
  oai21  g53(.a(x18), .b(x10), .c(new_n91_), .O(new_n100_));
  nor2   g54(.a(x20), .b(x19), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(new_n87_), .O(new_n102_));
  nand4  g56(.a(new_n62_), .b(new_n57_), .c(new_n51_), .d(new_n87_), .O(new_n103_));
  inv1   g57(.a(new_n103_), .O(new_n104_));
  aoi21  g58(.a(new_n102_), .b(x21), .c(new_n104_), .O(new_n105_));
  oai21  g59(.a(new_n105_), .b(new_n91_), .c(new_n100_), .O(z11));
  nor3   g60(.a(x22), .b(x21), .c(x20), .O(new_n107_));
  aoi22  g61(.a(new_n107_), .b(new_n89_), .c(new_n103_), .d(x22), .O(new_n108_));
  oai22  g62(.a(new_n108_), .b(new_n91_), .c(new_n98_), .d(new_n65_), .O(z12));
  oai21  g63(.a(x18), .b(x12), .c(new_n91_), .O(new_n110_));
  nor2   g64(.a(x22), .b(x21), .O(new_n111_));
  nand3  g65(.a(new_n111_), .b(new_n89_), .c(new_n57_), .O(new_n112_));
  nor3   g66(.a(x23), .b(x22), .c(x21), .O(new_n113_));
  aoi22  g67(.a(new_n113_), .b(new_n95_), .c(new_n112_), .d(x23), .O(new_n114_));
  oai21  g68(.a(new_n114_), .b(new_n91_), .c(new_n110_), .O(z13));
  inv1   g69(.a(x24), .O(new_n116_));
  aoi21  g70(.a(new_n113_), .b(new_n95_), .c(new_n116_), .O(new_n117_));
  nor2   g71(.a(x24), .b(x23), .O(new_n118_));
  nand4  g72(.a(new_n118_), .b(new_n111_), .c(new_n101_), .d(new_n87_), .O(new_n119_));
  inv1   g73(.a(new_n119_), .O(new_n120_));
  oai21  g74(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  nand2  g75(.a(new_n97_), .b(x13), .O(new_n122_));
  nand2  g76(.a(new_n122_), .b(new_n121_), .O(z14));
  oai21  g77(.a(x18), .b(x14), .c(new_n91_), .O(new_n124_));
  nand4  g78(.a(new_n80_), .b(new_n116_), .c(new_n72_), .d(new_n67_), .O(new_n125_));
  inv1   g79(.a(new_n125_), .O(new_n126_));
  aoi22  g80(.a(new_n126_), .b(new_n104_), .c(new_n119_), .d(x25), .O(new_n127_));
  oai21  g81(.a(new_n127_), .b(new_n91_), .c(new_n124_), .O(z15));
  nor2   g82(.a(x21), .b(x20), .O(new_n129_));
  nor2   g83(.a(x23), .b(x22), .O(new_n130_));
  nor2   g84(.a(x25), .b(x24), .O(new_n131_));
  nand4  g85(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n89_), .O(new_n132_));
  nand2  g86(.a(new_n132_), .b(x26), .O(new_n133_));
  nor3   g87(.a(x26), .b(x25), .c(x24), .O(new_n134_));
  nand4  g88(.a(new_n134_), .b(new_n130_), .c(new_n129_), .d(new_n89_), .O(new_n135_));
  nand2  g89(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g90(.a(new_n136_), .b(x16), .O(new_n137_));
  nand2  g91(.a(new_n97_), .b(x15), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(new_n137_), .O(z16));
  inv1   g93(.a(new_n48_), .O(new_n140_));
  nor2   g94(.a(x26), .b(x25), .O(new_n141_));
  nand3  g95(.a(new_n141_), .b(new_n130_), .c(new_n116_), .O(new_n142_));
  nand3  g96(.a(new_n129_), .b(x19), .c(new_n87_), .O(new_n143_));
  oai22  g97(.a(new_n143_), .b(new_n142_), .c(new_n52_), .d(new_n87_), .O(new_n144_));
  nand2  g98(.a(new_n144_), .b(x16), .O(new_n145_));
  nand2  g99(.a(new_n145_), .b(new_n140_), .O(z17));
endmodule


