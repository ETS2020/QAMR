// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x18), .b(x16), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  aoi21  g015(.a(x27), .b(new_n61_), .c(new_n49_), .O(new_n62_));
  oai21  g016(.a(x27), .b(x23), .c(new_n62_), .O(z04));
  inv1   g017(.a(x13), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x24), .c(new_n65_), .O(z05));
  inv1   g020(.a(x14), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n49_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x25), .c(new_n68_), .O(z06));
  inv1   g023(.a(x15), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x26), .O(new_n72_));
  inv1   g026(.a(x27), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n71_), .c(new_n49_), .O(z07));
  nand2  g029(.a(new_n48_), .b(new_n73_), .O(z08));
  oai21  g030(.a(x16), .b(x00), .c(x18), .O(new_n77_));
  inv1   g031(.a(x17), .O(new_n78_));
  inv1   g032(.a(x19), .O(new_n79_));
  nor2   g033(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  nor2   g036(.a(x18), .b(x16), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(x08), .O(new_n84_));
  nand3  g038(.a(new_n84_), .b(new_n82_), .c(new_n77_), .O(z09));
  oai21  g039(.a(x16), .b(x01), .c(x18), .O(new_n86_));
  inv1   g040(.a(x20), .O(new_n87_));
  nor2   g041(.a(new_n81_), .b(new_n87_), .O(new_n88_));
  nor3   g042(.a(x20), .b(x19), .c(x17), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  nand2  g044(.a(new_n83_), .b(x09), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n90_), .c(new_n86_), .O(z10));
  oai21  g046(.a(x16), .b(x02), .c(x18), .O(new_n93_));
  inv1   g047(.a(x21), .O(new_n94_));
  nor2   g048(.a(x21), .b(x20), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n81_), .O(new_n96_));
  oai21  g050(.a(new_n89_), .b(new_n94_), .c(new_n96_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(x16), .O(new_n98_));
  nand2  g052(.a(new_n83_), .b(x10), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n98_), .c(new_n93_), .O(z11));
  inv1   g054(.a(x16), .O(new_n101_));
  nand2  g055(.a(x18), .b(x03), .O(new_n102_));
  oai21  g056(.a(x18), .b(new_n58_), .c(new_n102_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  inv1   g058(.a(x18), .O(new_n105_));
  nand2  g059(.a(new_n96_), .b(x22), .O(new_n106_));
  nor2   g060(.a(x22), .b(x21), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n81_), .c(new_n87_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n105_), .c(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n104_), .O(z12));
  oai21  g065(.a(x16), .b(x04), .c(x18), .O(new_n112_));
  nand2  g066(.a(new_n108_), .b(x23), .O(new_n113_));
  nor2   g067(.a(x23), .b(x22), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n89_), .c(new_n94_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x16), .O(new_n117_));
  nand2  g071(.a(new_n83_), .b(x12), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n117_), .c(new_n112_), .O(z13));
  nand2  g073(.a(x18), .b(x05), .O(new_n120_));
  oai21  g074(.a(x18), .b(new_n64_), .c(new_n120_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n101_), .O(new_n122_));
  nand3  g076(.a(new_n87_), .b(new_n79_), .c(new_n78_), .O(new_n123_));
  inv1   g077(.a(x22), .O(new_n124_));
  inv1   g078(.a(x23), .O(new_n125_));
  nand3  g079(.a(new_n125_), .b(new_n124_), .c(new_n94_), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n123_), .c(x24), .O(new_n127_));
  nor2   g081(.a(x20), .b(x19), .O(new_n128_));
  nor2   g082(.a(x24), .b(x23), .O(new_n129_));
  nand4  g083(.a(new_n129_), .b(new_n107_), .c(new_n128_), .d(new_n78_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n105_), .c(x16), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n122_), .O(z14));
  oai21  g087(.a(x16), .b(x06), .c(x18), .O(new_n134_));
  nand2  g088(.a(new_n130_), .b(x25), .O(new_n135_));
  nor2   g089(.a(x25), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n114_), .c(new_n95_), .d(new_n81_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand2  g093(.a(new_n83_), .b(x14), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n139_), .c(new_n134_), .O(z15));
  nand2  g095(.a(x18), .b(x07), .O(new_n142_));
  oai21  g096(.a(x18), .b(new_n70_), .c(new_n142_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n101_), .O(new_n144_));
  nand2  g098(.a(new_n137_), .b(x26), .O(new_n145_));
  nor3   g099(.a(x26), .b(x25), .c(x24), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n114_), .c(new_n95_), .d(new_n81_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n105_), .c(x16), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n144_), .O(z16));
  inv1   g104(.a(x24), .O(new_n151_));
  nor2   g105(.a(x26), .b(x25), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n114_), .c(new_n151_), .O(new_n153_));
  nand3  g107(.a(new_n95_), .b(x19), .c(new_n78_), .O(new_n154_));
  oai22  g108(.a(new_n154_), .b(new_n153_), .c(new_n73_), .d(new_n78_), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n105_), .c(x16), .O(new_n156_));
  inv1   g110(.a(new_n156_), .O(z17));
endmodule


