// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_;
  inv1   g00(.a(x08), .O(new_n47_));
  nor2   g01(.a(x18), .b(x16), .O(new_n48_));
  aoi21  g02(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g03(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g04(.a(x09), .O(new_n51_));
  aoi21  g05(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g06(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g07(.a(x10), .O(new_n54_));
  nand2  g08(.a(x27), .b(new_n54_), .O(new_n55_));
  inv1   g09(.a(x21), .O(new_n56_));
  inv1   g10(.a(x27), .O(new_n57_));
  nand2  g11(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g12(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z02));
  inv1   g13(.a(x11), .O(new_n60_));
  nand2  g14(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g15(.a(x22), .O(new_n62_));
  nand2  g16(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  aoi21  g17(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(z03));
  inv1   g18(.a(x12), .O(new_n65_));
  nand2  g19(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g20(.a(x23), .O(new_n67_));
  nand2  g21(.a(new_n57_), .b(new_n67_), .O(new_n68_));
  aoi21  g22(.a(new_n68_), .b(new_n66_), .c(new_n48_), .O(z04));
  inv1   g23(.a(x13), .O(new_n70_));
  nand2  g24(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g25(.a(x24), .O(new_n72_));
  nand2  g26(.a(new_n57_), .b(new_n72_), .O(new_n73_));
  aoi21  g27(.a(new_n73_), .b(new_n71_), .c(new_n48_), .O(z05));
  inv1   g28(.a(x14), .O(new_n75_));
  nand2  g29(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g30(.a(x25), .O(new_n77_));
  nand2  g31(.a(new_n57_), .b(new_n77_), .O(new_n78_));
  aoi21  g32(.a(new_n78_), .b(new_n76_), .c(new_n48_), .O(z06));
  inv1   g33(.a(x15), .O(new_n80_));
  nand2  g34(.a(x27), .b(new_n80_), .O(new_n81_));
  inv1   g35(.a(x26), .O(new_n82_));
  nand2  g36(.a(new_n57_), .b(new_n82_), .O(new_n83_));
  aoi21  g37(.a(new_n83_), .b(new_n81_), .c(new_n48_), .O(z07));
  nor2   g38(.a(new_n48_), .b(new_n57_), .O(z08));
  inv1   g39(.a(x00), .O(new_n86_));
  inv1   g40(.a(x17), .O(new_n87_));
  inv1   g41(.a(x19), .O(new_n88_));
  nor2   g42(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g43(.a(x19), .b(x17), .O(new_n90_));
  oai21  g44(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  inv1   g45(.a(x16), .O(new_n92_));
  nand2  g46(.a(x18), .b(new_n92_), .O(new_n93_));
  oai21  g47(.a(new_n93_), .b(new_n86_), .c(new_n91_), .O(z09));
  inv1   g48(.a(x01), .O(new_n95_));
  inv1   g49(.a(x20), .O(new_n96_));
  nor2   g50(.a(new_n90_), .b(new_n96_), .O(new_n97_));
  nor3   g51(.a(x20), .b(x19), .c(x17), .O(new_n98_));
  oai21  g52(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  oai21  g53(.a(new_n93_), .b(new_n95_), .c(new_n99_), .O(z10));
  inv1   g54(.a(x02), .O(new_n101_));
  nor2   g55(.a(x20), .b(x19), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n87_), .O(new_n103_));
  nand4  g57(.a(new_n56_), .b(new_n96_), .c(new_n88_), .d(new_n87_), .O(new_n104_));
  inv1   g58(.a(new_n104_), .O(new_n105_));
  aoi21  g59(.a(new_n103_), .b(x21), .c(new_n105_), .O(new_n106_));
  oai22  g60(.a(new_n106_), .b(new_n92_), .c(new_n93_), .d(new_n101_), .O(z11));
  inv1   g61(.a(x03), .O(new_n108_));
  nor3   g62(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  aoi22  g63(.a(new_n109_), .b(new_n90_), .c(new_n104_), .d(x22), .O(new_n110_));
  oai22  g64(.a(new_n110_), .b(new_n92_), .c(new_n93_), .d(new_n108_), .O(z12));
  inv1   g65(.a(x04), .O(new_n112_));
  nor2   g66(.a(x22), .b(x21), .O(new_n113_));
  nand3  g67(.a(new_n113_), .b(new_n90_), .c(new_n96_), .O(new_n114_));
  nor3   g68(.a(x23), .b(x22), .c(x21), .O(new_n115_));
  aoi22  g69(.a(new_n115_), .b(new_n98_), .c(new_n114_), .d(x23), .O(new_n116_));
  oai22  g70(.a(new_n116_), .b(new_n92_), .c(new_n93_), .d(new_n112_), .O(z13));
  inv1   g71(.a(x18), .O(new_n118_));
  oai21  g72(.a(new_n118_), .b(x05), .c(new_n92_), .O(new_n119_));
  aoi21  g73(.a(new_n115_), .b(new_n98_), .c(new_n72_), .O(new_n120_));
  nor2   g74(.a(x24), .b(x23), .O(new_n121_));
  nand4  g75(.a(new_n121_), .b(new_n113_), .c(new_n102_), .d(new_n87_), .O(new_n122_));
  inv1   g76(.a(new_n122_), .O(new_n123_));
  oai21  g77(.a(new_n123_), .b(new_n120_), .c(x16), .O(new_n124_));
  nand2  g78(.a(new_n124_), .b(new_n119_), .O(z14));
  oai21  g79(.a(new_n118_), .b(x06), .c(new_n92_), .O(new_n126_));
  nand4  g80(.a(new_n77_), .b(new_n72_), .c(new_n67_), .d(new_n62_), .O(new_n127_));
  inv1   g81(.a(new_n127_), .O(new_n128_));
  aoi22  g82(.a(new_n128_), .b(new_n105_), .c(new_n122_), .d(x25), .O(new_n129_));
  oai21  g83(.a(new_n129_), .b(new_n92_), .c(new_n126_), .O(z15));
  oai21  g84(.a(new_n118_), .b(x07), .c(new_n92_), .O(new_n131_));
  nor2   g85(.a(x21), .b(x20), .O(new_n132_));
  nor2   g86(.a(x23), .b(x22), .O(new_n133_));
  nor2   g87(.a(x25), .b(x24), .O(new_n134_));
  nand4  g88(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n90_), .O(new_n135_));
  nand2  g89(.a(new_n135_), .b(x26), .O(new_n136_));
  nor3   g90(.a(x26), .b(x25), .c(x24), .O(new_n137_));
  nand4  g91(.a(new_n137_), .b(new_n133_), .c(new_n132_), .d(new_n90_), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand2  g93(.a(new_n139_), .b(x16), .O(new_n140_));
  nand2  g94(.a(new_n140_), .b(new_n131_), .O(z16));
  nor2   g95(.a(new_n88_), .b(x17), .O(new_n142_));
  nand4  g96(.a(new_n142_), .b(new_n137_), .c(new_n133_), .d(new_n132_), .O(new_n143_));
  nand2  g97(.a(x27), .b(x17), .O(new_n144_));
  aoi21  g98(.a(new_n144_), .b(new_n143_), .c(new_n92_), .O(z17));
endmodule


