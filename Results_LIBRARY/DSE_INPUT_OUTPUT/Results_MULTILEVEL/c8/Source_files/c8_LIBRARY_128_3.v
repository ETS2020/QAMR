// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x18), .b(x16), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x23), .O(new_n63_));
  inv1   g017(.a(x27), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n62_), .c(new_n49_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n49_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n49_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n49_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  nor2   g031(.a(new_n49_), .b(new_n64_), .O(z08));
  xnor2a g032(.a(x19), .b(x17), .O(new_n79_));
  nor2   g033(.a(x16), .b(new_n47_), .O(new_n80_));
  aoi21  g034(.a(new_n79_), .b(x16), .c(new_n80_), .O(new_n81_));
  inv1   g035(.a(x16), .O(new_n82_));
  nand3  g036(.a(x18), .b(new_n82_), .c(x00), .O(new_n83_));
  oai21  g037(.a(new_n81_), .b(x18), .c(new_n83_), .O(z09));
  nand2  g038(.a(x18), .b(x01), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n52_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n82_), .O(new_n87_));
  inv1   g041(.a(x18), .O(new_n88_));
  inv1   g042(.a(x20), .O(new_n89_));
  nor2   g043(.a(x19), .b(x17), .O(new_n90_));
  inv1   g044(.a(x17), .O(new_n91_));
  nor2   g045(.a(x20), .b(x19), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g047(.a(new_n90_), .b(new_n89_), .c(new_n93_), .O(new_n94_));
  nand3  g048(.a(new_n94_), .b(new_n88_), .c(x16), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n87_), .O(z10));
  nand2  g050(.a(x18), .b(x02), .O(new_n97_));
  oai21  g051(.a(x18), .b(new_n55_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n82_), .O(new_n99_));
  inv1   g053(.a(x21), .O(new_n100_));
  nor3   g054(.a(x20), .b(x19), .c(x17), .O(new_n101_));
  nor2   g055(.a(x21), .b(x20), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n90_), .O(new_n103_));
  oai21  g057(.a(new_n101_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n88_), .c(x16), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n99_), .O(z11));
  nand2  g060(.a(x18), .b(x03), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n58_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n82_), .O(new_n109_));
  nand2  g063(.a(new_n103_), .b(x22), .O(new_n110_));
  nor2   g064(.a(x22), .b(x21), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n90_), .c(new_n89_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n88_), .c(x16), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n109_), .O(z12));
  oai21  g069(.a(x16), .b(x04), .c(x18), .O(new_n116_));
  nand2  g070(.a(new_n112_), .b(x23), .O(new_n117_));
  nor3   g071(.a(x23), .b(x22), .c(x21), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n101_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  nor2   g075(.a(x18), .b(x16), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x12), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n121_), .c(new_n116_), .O(z13));
  oai21  g078(.a(x16), .b(x05), .c(x18), .O(new_n125_));
  aoi21  g079(.a(new_n118_), .b(new_n101_), .c(new_n69_), .O(new_n126_));
  nor2   g080(.a(x24), .b(x23), .O(new_n127_));
  nand4  g081(.a(new_n127_), .b(new_n111_), .c(new_n92_), .d(new_n91_), .O(new_n128_));
  inv1   g082(.a(new_n128_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n126_), .c(x16), .O(new_n130_));
  nand2  g084(.a(new_n122_), .b(x13), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n130_), .c(new_n125_), .O(z14));
  oai21  g086(.a(x16), .b(x06), .c(x18), .O(new_n133_));
  nand2  g087(.a(new_n128_), .b(x25), .O(new_n134_));
  nor2   g088(.a(x23), .b(x22), .O(new_n135_));
  nor2   g089(.a(x25), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n135_), .c(new_n102_), .d(new_n90_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand2  g093(.a(new_n122_), .b(x14), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n139_), .c(new_n133_), .O(z15));
  nand2  g095(.a(x18), .b(x07), .O(new_n142_));
  oai21  g096(.a(x18), .b(new_n75_), .c(new_n142_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n82_), .O(new_n144_));
  nand2  g098(.a(new_n137_), .b(x26), .O(new_n145_));
  nor3   g099(.a(x26), .b(x25), .c(x24), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n135_), .c(new_n102_), .d(new_n90_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n88_), .c(x16), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n144_), .O(z16));
  inv1   g104(.a(x22), .O(new_n151_));
  nand2  g105(.a(new_n63_), .b(new_n151_), .O(new_n152_));
  inv1   g106(.a(x25), .O(new_n153_));
  inv1   g107(.a(x26), .O(new_n154_));
  nand3  g108(.a(new_n154_), .b(new_n153_), .c(new_n69_), .O(new_n155_));
  nor2   g109(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  nand3  g110(.a(new_n102_), .b(x19), .c(new_n91_), .O(new_n157_));
  inv1   g111(.a(new_n157_), .O(new_n158_));
  nor2   g112(.a(new_n64_), .b(new_n91_), .O(new_n159_));
  aoi21  g113(.a(new_n158_), .b(new_n156_), .c(new_n159_), .O(new_n160_));
  aoi21  g114(.a(new_n160_), .b(new_n88_), .c(new_n82_), .O(z17));
endmodule


