// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x23), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x18), .O(new_n61_));
  inv1   g015(.a(x23), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  oai22  g017(.a(new_n63_), .b(x12), .c(x27), .d(x23), .O(z04));
  inv1   g018(.a(x13), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x24), .O(new_n67_));
  inv1   g021(.a(x27), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n66_), .c(new_n49_), .O(z05));
  inv1   g024(.a(x14), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x25), .O(new_n73_));
  nand2  g027(.a(new_n68_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n49_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n49_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x26), .c(new_n77_), .O(z07));
  nor2   g032(.a(new_n49_), .b(new_n68_), .O(z08));
  and2   g033(.a(x19), .b(x17), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  inv1   g036(.a(x16), .O(new_n83_));
  nand2  g037(.a(x18), .b(x00), .O(new_n84_));
  oai21  g038(.a(x18), .b(new_n47_), .c(new_n84_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand3  g040(.a(new_n86_), .b(new_n82_), .c(new_n48_), .O(z09));
  nand2  g041(.a(x18), .b(x01), .O(new_n88_));
  oai21  g042(.a(x18), .b(new_n52_), .c(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n83_), .O(new_n90_));
  inv1   g044(.a(x20), .O(new_n91_));
  nor2   g045(.a(new_n81_), .b(new_n91_), .O(new_n92_));
  nor3   g046(.a(x20), .b(x19), .c(x17), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n92_), .c(x16), .O(new_n94_));
  nand3  g048(.a(new_n94_), .b(new_n90_), .c(new_n48_), .O(z10));
  inv1   g049(.a(x02), .O(new_n96_));
  nand2  g050(.a(new_n62_), .b(x18), .O(new_n97_));
  oai22  g051(.a(new_n97_), .b(new_n96_), .c(x18), .d(new_n55_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n83_), .O(new_n99_));
  inv1   g053(.a(x21), .O(new_n100_));
  nor2   g054(.a(x21), .b(x20), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n81_), .O(new_n102_));
  oai21  g056(.a(new_n93_), .b(new_n100_), .c(new_n102_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n48_), .c(x16), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n99_), .O(z11));
  inv1   g059(.a(x03), .O(new_n106_));
  oai22  g060(.a(new_n97_), .b(new_n106_), .c(x18), .d(new_n58_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n83_), .O(new_n108_));
  nand2  g062(.a(new_n102_), .b(x22), .O(new_n109_));
  nor2   g063(.a(x22), .b(x21), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n81_), .c(new_n91_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n48_), .c(x16), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n108_), .O(z12));
  inv1   g068(.a(x04), .O(new_n115_));
  nand2  g069(.a(new_n61_), .b(x12), .O(new_n116_));
  oai21  g070(.a(new_n97_), .b(new_n115_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n83_), .O(new_n118_));
  nand3  g072(.a(new_n111_), .b(x23), .c(new_n61_), .O(new_n119_));
  nor2   g073(.a(x23), .b(x22), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n93_), .c(new_n100_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n118_), .O(z13));
  nand2  g078(.a(x18), .b(x05), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n65_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n83_), .O(new_n127_));
  oai21  g081(.a(new_n67_), .b(new_n83_), .c(new_n61_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x23), .O(new_n129_));
  nand2  g083(.a(new_n111_), .b(x24), .O(new_n130_));
  nand4  g084(.a(new_n110_), .b(new_n93_), .c(new_n67_), .d(new_n62_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x16), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n129_), .c(new_n127_), .O(z14));
  nand2  g088(.a(x18), .b(x06), .O(new_n135_));
  oai21  g089(.a(x18), .b(new_n71_), .c(new_n135_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n83_), .O(new_n137_));
  oai21  g091(.a(new_n73_), .b(new_n83_), .c(new_n61_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x23), .O(new_n139_));
  nor3   g093(.a(x24), .b(x22), .c(x21), .O(new_n140_));
  aoi21  g094(.a(new_n140_), .b(new_n93_), .c(new_n73_), .O(new_n141_));
  nor2   g095(.a(x25), .b(x24), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n120_), .O(new_n143_));
  nor2   g097(.a(new_n143_), .b(new_n102_), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n141_), .c(x16), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n139_), .c(new_n137_), .O(z15));
  inv1   g100(.a(x07), .O(new_n147_));
  oai22  g101(.a(new_n97_), .b(new_n147_), .c(x18), .d(new_n76_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n83_), .O(new_n149_));
  inv1   g103(.a(x17), .O(new_n150_));
  nor2   g104(.a(x20), .b(x19), .O(new_n151_));
  nand4  g105(.a(new_n142_), .b(new_n110_), .c(new_n151_), .d(new_n150_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(x26), .O(new_n153_));
  nor2   g107(.a(x24), .b(x22), .O(new_n154_));
  nor2   g108(.a(x26), .b(x25), .O(new_n155_));
  nand4  g109(.a(new_n155_), .b(new_n154_), .c(new_n101_), .d(new_n81_), .O(new_n156_));
  aoi21  g110(.a(new_n156_), .b(new_n153_), .c(x23), .O(new_n157_));
  nand2  g111(.a(x26), .b(x23), .O(new_n158_));
  nor2   g112(.a(new_n158_), .b(x18), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n157_), .c(x16), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n149_), .O(z16));
  nand3  g115(.a(new_n101_), .b(x19), .c(new_n150_), .O(new_n162_));
  nand3  g116(.a(new_n155_), .b(new_n120_), .c(new_n67_), .O(new_n163_));
  oai22  g117(.a(new_n163_), .b(new_n162_), .c(new_n68_), .d(new_n150_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(x16), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(new_n48_), .O(z17));
endmodule


