// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x22), .O(new_n60_));
  inv1   g014(.a(x27), .O(new_n61_));
  nand2  g015(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n59_), .c(new_n49_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n61_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n49_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n49_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n49_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  nor2   g031(.a(new_n49_), .b(new_n61_), .O(z08));
  nand3  g032(.a(x19), .b(x18), .c(x17), .O(new_n79_));
  inv1   g033(.a(new_n79_), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  nor2   g036(.a(x19), .b(x18), .O(new_n83_));
  aoi22  g037(.a(new_n83_), .b(x08), .c(x18), .d(x00), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(x16), .c(new_n82_), .O(z09));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(x18), .b(x01), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n52_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  inv1   g043(.a(x20), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n86_), .c(x18), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(x19), .O(new_n92_));
  inv1   g046(.a(x17), .O(new_n93_));
  nor2   g047(.a(new_n90_), .b(new_n93_), .O(new_n94_));
  nor3   g048(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n92_), .c(new_n89_), .O(z10));
  aoi22  g051(.a(new_n83_), .b(x10), .c(x18), .d(x02), .O(new_n98_));
  nand2  g052(.a(new_n90_), .b(new_n93_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(x21), .O(new_n100_));
  inv1   g054(.a(x21), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n90_), .c(new_n93_), .O(new_n102_));
  aoi21  g056(.a(new_n102_), .b(new_n100_), .c(x19), .O(new_n103_));
  nand3  g057(.a(x21), .b(x19), .c(x18), .O(new_n104_));
  inv1   g058(.a(new_n104_), .O(new_n105_));
  oai21  g059(.a(new_n105_), .b(new_n103_), .c(x16), .O(new_n106_));
  oai21  g060(.a(new_n98_), .b(x16), .c(new_n106_), .O(z11));
  nand2  g061(.a(x18), .b(x03), .O(new_n108_));
  oai21  g062(.a(x18), .b(new_n58_), .c(new_n108_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n86_), .O(new_n110_));
  oai21  g064(.a(new_n60_), .b(new_n86_), .c(x18), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x19), .O(new_n112_));
  nand2  g066(.a(new_n102_), .b(x22), .O(new_n113_));
  nor2   g067(.a(x22), .b(x21), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n81_), .c(new_n90_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x16), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n112_), .c(new_n110_), .O(z12));
  nand2  g072(.a(x18), .b(x04), .O(new_n119_));
  oai21  g073(.a(x18), .b(new_n64_), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n86_), .O(new_n121_));
  inv1   g075(.a(x23), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n86_), .c(x18), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(x19), .O(new_n124_));
  inv1   g078(.a(new_n99_), .O(new_n125_));
  aoi21  g079(.a(new_n114_), .b(new_n125_), .c(new_n122_), .O(new_n126_));
  inv1   g080(.a(new_n95_), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n101_), .O(new_n129_));
  nor2   g083(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n126_), .c(x16), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n124_), .c(new_n121_), .O(z13));
  nand2  g086(.a(x18), .b(x05), .O(new_n133_));
  oai21  g087(.a(x18), .b(new_n67_), .c(new_n133_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n86_), .O(new_n135_));
  oai21  g089(.a(new_n69_), .b(new_n86_), .c(x18), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(x19), .O(new_n137_));
  oai21  g091(.a(new_n129_), .b(new_n99_), .c(x24), .O(new_n138_));
  nand4  g092(.a(new_n114_), .b(new_n95_), .c(new_n69_), .d(new_n122_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x16), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n137_), .c(new_n135_), .O(z14));
  nand2  g096(.a(x18), .b(x06), .O(new_n143_));
  nand2  g097(.a(new_n83_), .b(x14), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n86_), .O(new_n146_));
  nand3  g100(.a(new_n69_), .b(new_n122_), .c(new_n60_), .O(new_n147_));
  oai21  g101(.a(new_n147_), .b(new_n102_), .c(x25), .O(new_n148_));
  nor2   g102(.a(x21), .b(x20), .O(new_n149_));
  nor2   g103(.a(x25), .b(x24), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n128_), .c(new_n149_), .d(new_n93_), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n148_), .c(x19), .O(new_n152_));
  nand3  g106(.a(x25), .b(x19), .c(x18), .O(new_n153_));
  inv1   g107(.a(new_n153_), .O(new_n154_));
  oai21  g108(.a(new_n154_), .b(new_n152_), .c(x16), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n146_), .O(z15));
  nand2  g110(.a(x18), .b(x07), .O(new_n157_));
  oai21  g111(.a(x18), .b(new_n75_), .c(new_n157_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n86_), .O(new_n159_));
  nand2  g113(.a(x26), .b(x16), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(x18), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x19), .O(new_n162_));
  nand2  g116(.a(new_n151_), .b(x26), .O(new_n163_));
  nand2  g117(.a(new_n149_), .b(new_n81_), .O(new_n164_));
  nor2   g118(.a(x26), .b(x25), .O(new_n165_));
  nand3  g119(.a(new_n165_), .b(new_n128_), .c(new_n69_), .O(new_n166_));
  oai21  g120(.a(new_n166_), .b(new_n164_), .c(new_n163_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x16), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n162_), .c(new_n159_), .O(z16));
  inv1   g123(.a(new_n49_), .O(new_n170_));
  nand3  g124(.a(new_n149_), .b(x19), .c(new_n93_), .O(new_n171_));
  oai22  g125(.a(new_n171_), .b(new_n166_), .c(new_n61_), .d(new_n93_), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(x16), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(new_n170_), .O(z17));
endmodule


