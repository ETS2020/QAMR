// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_;
  inv1   g000(.a(x12), .O(new_n47_));
  nand2  g001(.a(x18), .b(new_n47_), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n53_), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x09), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  nand3  g013(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z01));
  inv1   g014(.a(x21), .O(new_n61_));
  nand2  g015(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x10), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  nand3  g018(.a(new_n64_), .b(new_n62_), .c(new_n48_), .O(z02));
  inv1   g019(.a(x11), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x22), .O(new_n68_));
  nand2  g022(.a(new_n53_), .b(new_n68_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n67_), .c(new_n49_), .O(z03));
  nor2   g024(.a(x27), .b(x23), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n47_), .c(new_n71_), .O(new_n72_));
  nand2  g026(.a(new_n71_), .b(x12), .O(new_n73_));
  oai21  g027(.a(new_n72_), .b(x18), .c(new_n73_), .O(z04));
  inv1   g028(.a(x13), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x24), .O(new_n77_));
  nand2  g031(.a(new_n53_), .b(new_n77_), .O(new_n78_));
  aoi22  g032(.a(new_n78_), .b(new_n76_), .c(x18), .d(new_n47_), .O(z05));
  inv1   g033(.a(x14), .O(new_n80_));
  nand2  g034(.a(x27), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x25), .O(new_n82_));
  nand2  g036(.a(new_n53_), .b(new_n82_), .O(new_n83_));
  aoi21  g037(.a(new_n83_), .b(new_n81_), .c(new_n49_), .O(z06));
  inv1   g038(.a(x26), .O(new_n85_));
  nand2  g039(.a(new_n53_), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x15), .O(new_n87_));
  nand2  g041(.a(x27), .b(new_n87_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n86_), .c(new_n48_), .O(z07));
  nor2   g043(.a(new_n49_), .b(new_n53_), .O(z08));
  inv1   g044(.a(x17), .O(new_n91_));
  nor2   g045(.a(new_n52_), .b(new_n91_), .O(new_n92_));
  nor2   g046(.a(x19), .b(x17), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n92_), .c(x16), .O(new_n94_));
  inv1   g048(.a(x16), .O(new_n95_));
  nand2  g049(.a(x18), .b(x00), .O(new_n96_));
  oai21  g050(.a(x18), .b(new_n50_), .c(new_n96_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n94_), .c(new_n48_), .O(z09));
  nand2  g053(.a(x18), .b(x01), .O(new_n100_));
  oai21  g054(.a(x18), .b(new_n58_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n95_), .O(new_n102_));
  nor2   g056(.a(new_n93_), .b(new_n56_), .O(new_n103_));
  nor3   g057(.a(x20), .b(x19), .c(x17), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n103_), .c(x16), .O(new_n105_));
  nand3  g059(.a(new_n105_), .b(new_n102_), .c(new_n48_), .O(z10));
  nand2  g060(.a(x18), .b(x02), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n63_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n95_), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n93_), .O(new_n111_));
  oai21  g065(.a(new_n104_), .b(new_n61_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(x16), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n109_), .c(new_n48_), .O(z11));
  inv1   g068(.a(x03), .O(new_n115_));
  nand2  g069(.a(x18), .b(x12), .O(new_n116_));
  oai22  g070(.a(new_n116_), .b(new_n115_), .c(x18), .d(new_n66_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n95_), .O(new_n118_));
  nand2  g072(.a(new_n111_), .b(x22), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n93_), .c(new_n56_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n48_), .c(x16), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n118_), .O(z12));
  inv1   g078(.a(x04), .O(new_n125_));
  aoi21  g079(.a(x18), .b(new_n125_), .c(x16), .O(new_n126_));
  nand2  g080(.a(new_n121_), .b(x23), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n104_), .c(new_n61_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  aoi21  g084(.a(new_n130_), .b(x16), .c(new_n126_), .O(new_n131_));
  inv1   g085(.a(x18), .O(new_n132_));
  nand3  g086(.a(new_n130_), .b(new_n132_), .c(x16), .O(new_n133_));
  oai21  g087(.a(new_n131_), .b(new_n47_), .c(new_n133_), .O(z13));
  inv1   g088(.a(x05), .O(new_n135_));
  oai22  g089(.a(new_n116_), .b(new_n135_), .c(x18), .d(new_n75_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n95_), .O(new_n137_));
  nand2  g091(.a(new_n129_), .b(x24), .O(new_n138_));
  nor2   g092(.a(x24), .b(x23), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n120_), .c(new_n104_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n48_), .c(x16), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n137_), .O(z14));
  inv1   g097(.a(x06), .O(new_n144_));
  oai22  g098(.a(new_n116_), .b(new_n144_), .c(x18), .d(new_n80_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n95_), .O(new_n146_));
  nand2  g100(.a(new_n140_), .b(x25), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand4  g102(.a(new_n148_), .b(new_n128_), .c(new_n110_), .d(new_n93_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand3  g104(.a(new_n150_), .b(new_n48_), .c(x16), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n146_), .O(z15));
  nand2  g106(.a(x18), .b(x07), .O(new_n153_));
  oai21  g107(.a(x18), .b(new_n87_), .c(new_n153_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n95_), .O(new_n155_));
  nand2  g109(.a(new_n149_), .b(x26), .O(new_n156_));
  nand4  g110(.a(new_n128_), .b(new_n85_), .c(new_n82_), .d(new_n77_), .O(new_n157_));
  oai21  g111(.a(new_n157_), .b(new_n111_), .c(new_n156_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x16), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(new_n155_), .c(new_n48_), .O(z16));
  nand3  g114(.a(new_n110_), .b(x19), .c(new_n91_), .O(new_n161_));
  oai22  g115(.a(new_n161_), .b(new_n157_), .c(new_n53_), .d(new_n91_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x16), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n48_), .O(z17));
endmodule


