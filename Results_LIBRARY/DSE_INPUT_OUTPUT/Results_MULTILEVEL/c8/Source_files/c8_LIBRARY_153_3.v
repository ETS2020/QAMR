// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_;
  inv1   g000(.a(x18), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  nor2   g005(.a(new_n48_), .b(new_n47_), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n51_), .c(new_n52_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x21), .O(new_n57_));
  inv1   g011(.a(x27), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n56_), .c(new_n52_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n58_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n52_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x23), .O(new_n68_));
  nand2  g022(.a(new_n58_), .b(new_n68_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n67_), .c(new_n52_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x24), .O(new_n73_));
  nand2  g027(.a(new_n58_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n52_), .O(z05));
  inv1   g029(.a(x14), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n52_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x25), .c(new_n77_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  aoi21  g033(.a(x27), .b(new_n79_), .c(new_n52_), .O(new_n80_));
  oai21  g034(.a(x27), .b(x26), .c(new_n80_), .O(z07));
  nor2   g035(.a(new_n52_), .b(new_n58_), .O(z08));
  inv1   g036(.a(new_n52_), .O(new_n83_));
  inv1   g037(.a(x17), .O(new_n84_));
  nor2   g038(.a(new_n48_), .b(new_n84_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  inv1   g042(.a(x08), .O(new_n89_));
  nand2  g043(.a(x18), .b(x00), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n89_), .c(new_n90_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand3  g046(.a(new_n92_), .b(new_n87_), .c(new_n83_), .O(z09));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n51_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  inv1   g050(.a(x20), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n88_), .c(new_n47_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x19), .O(new_n99_));
  nand3  g053(.a(new_n97_), .b(new_n48_), .c(new_n84_), .O(new_n100_));
  oai21  g054(.a(new_n97_), .b(new_n84_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x16), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n99_), .c(new_n96_), .O(z10));
  inv1   g057(.a(x02), .O(new_n104_));
  nand2  g058(.a(new_n48_), .b(x18), .O(new_n105_));
  oai22  g059(.a(new_n105_), .b(new_n104_), .c(x18), .d(new_n55_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  oai21  g061(.a(x20), .b(x17), .c(x21), .O(new_n108_));
  nor2   g062(.a(x21), .b(x20), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n84_), .O(new_n110_));
  aoi21  g064(.a(new_n110_), .b(new_n108_), .c(x19), .O(new_n111_));
  nor3   g065(.a(new_n57_), .b(new_n48_), .c(x18), .O(new_n112_));
  oai21  g066(.a(new_n112_), .b(new_n111_), .c(x16), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n107_), .O(z11));
  nand2  g068(.a(x18), .b(x03), .O(new_n115_));
  oai21  g069(.a(x18), .b(new_n61_), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n88_), .O(new_n117_));
  oai21  g071(.a(new_n63_), .b(new_n88_), .c(new_n47_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x19), .O(new_n119_));
  nand2  g073(.a(new_n110_), .b(x22), .O(new_n120_));
  nor2   g074(.a(x22), .b(x21), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n86_), .c(new_n97_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(x16), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n119_), .c(new_n117_), .O(z12));
  nand2  g079(.a(x18), .b(x04), .O(new_n126_));
  oai21  g080(.a(x18), .b(new_n66_), .c(new_n126_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n88_), .O(new_n128_));
  oai21  g082(.a(new_n68_), .b(new_n88_), .c(new_n47_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(x19), .O(new_n130_));
  nor2   g084(.a(x20), .b(x17), .O(new_n131_));
  aoi21  g085(.a(new_n121_), .b(new_n131_), .c(new_n68_), .O(new_n132_));
  nor2   g086(.a(x23), .b(x22), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n57_), .O(new_n134_));
  nor2   g088(.a(new_n134_), .b(new_n100_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n132_), .c(x16), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n130_), .c(new_n128_), .O(z13));
  inv1   g091(.a(x05), .O(new_n138_));
  oai22  g092(.a(new_n105_), .b(new_n138_), .c(x18), .d(new_n71_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n88_), .O(new_n140_));
  nand3  g094(.a(new_n133_), .b(new_n131_), .c(new_n57_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x24), .O(new_n142_));
  nor2   g096(.a(x24), .b(x23), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n109_), .c(new_n63_), .d(new_n84_), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(new_n142_), .c(x19), .O(new_n145_));
  nor3   g099(.a(new_n73_), .b(new_n48_), .c(x18), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n145_), .c(x16), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n140_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(x18), .b(new_n76_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n88_), .O(new_n151_));
  nand2  g105(.a(x25), .b(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n47_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(x19), .O(new_n154_));
  and2   g108(.a(new_n144_), .b(x25), .O(new_n155_));
  nand2  g109(.a(new_n109_), .b(new_n86_), .O(new_n156_));
  nor2   g110(.a(x25), .b(x24), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n133_), .O(new_n158_));
  nor2   g112(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n155_), .c(x16), .O(new_n160_));
  nand3  g114(.a(new_n160_), .b(new_n154_), .c(new_n151_), .O(z15));
  inv1   g115(.a(x07), .O(new_n162_));
  oai22  g116(.a(new_n105_), .b(new_n162_), .c(x18), .d(new_n79_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n88_), .O(new_n164_));
  nand4  g118(.a(new_n157_), .b(new_n133_), .c(new_n109_), .d(new_n84_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x26), .O(new_n166_));
  nor2   g120(.a(x26), .b(x25), .O(new_n167_));
  nand4  g121(.a(new_n167_), .b(new_n143_), .c(new_n121_), .d(new_n131_), .O(new_n168_));
  aoi21  g122(.a(new_n168_), .b(new_n166_), .c(x19), .O(new_n169_));
  nand2  g123(.a(x26), .b(x19), .O(new_n170_));
  nor2   g124(.a(new_n170_), .b(x18), .O(new_n171_));
  oai21  g125(.a(new_n171_), .b(new_n169_), .c(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n164_), .O(z16));
  nand3  g127(.a(new_n109_), .b(x19), .c(new_n84_), .O(new_n174_));
  nand3  g128(.a(new_n167_), .b(new_n133_), .c(new_n73_), .O(new_n175_));
  oai22  g129(.a(new_n175_), .b(new_n174_), .c(new_n58_), .d(new_n84_), .O(new_n176_));
  nand2  g130(.a(new_n176_), .b(x16), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(new_n83_), .O(z17));
endmodule


