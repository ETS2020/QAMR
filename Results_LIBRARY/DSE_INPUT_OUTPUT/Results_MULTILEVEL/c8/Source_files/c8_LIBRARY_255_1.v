// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x23), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x22), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nand2  g006(.a(x27), .b(new_n52_), .O(new_n53_));
  inv1   g007(.a(x20), .O(new_n54_));
  inv1   g008(.a(x27), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(new_n53_), .c(new_n49_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x21), .c(new_n59_), .O(z02));
  inv1   g014(.a(x22), .O(new_n61_));
  oai21  g015(.a(new_n55_), .b(x23), .c(new_n61_), .O(new_n62_));
  oai21  g016(.a(new_n55_), .b(x11), .c(new_n62_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n49_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x24), .c(new_n68_), .O(z05));
  inv1   g023(.a(x14), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x25), .O(new_n72_));
  nand2  g026(.a(new_n55_), .b(new_n72_), .O(new_n73_));
  aoi21  g027(.a(new_n73_), .b(new_n71_), .c(new_n49_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x26), .O(new_n77_));
  nand2  g031(.a(new_n55_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n49_), .O(z07));
  nor2   g033(.a(new_n49_), .b(new_n55_), .O(z08));
  inv1   g034(.a(new_n49_), .O(new_n81_));
  inv1   g035(.a(x17), .O(new_n82_));
  inv1   g036(.a(x19), .O(new_n83_));
  nor2   g037(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g038(.a(x19), .b(x17), .O(new_n85_));
  oai21  g039(.a(new_n85_), .b(new_n84_), .c(x16), .O(new_n86_));
  inv1   g040(.a(x16), .O(new_n87_));
  nand2  g041(.a(x18), .b(x00), .O(new_n88_));
  oai21  g042(.a(x18), .b(new_n47_), .c(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n86_), .c(new_n81_), .O(z09));
  nand2  g045(.a(x18), .b(x01), .O(new_n92_));
  oai21  g046(.a(x18), .b(new_n52_), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n87_), .O(new_n94_));
  nor2   g048(.a(new_n85_), .b(new_n54_), .O(new_n95_));
  nand3  g049(.a(new_n54_), .b(new_n83_), .c(new_n82_), .O(new_n96_));
  inv1   g050(.a(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n94_), .c(new_n81_), .O(z10));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  oai21  g054(.a(x18), .b(new_n58_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n87_), .O(new_n102_));
  inv1   g056(.a(x21), .O(new_n103_));
  nor2   g057(.a(x21), .b(x20), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n85_), .O(new_n105_));
  oai21  g059(.a(new_n97_), .b(new_n103_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x16), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n102_), .c(new_n49_), .O(z11));
  inv1   g062(.a(x11), .O(new_n109_));
  nand2  g063(.a(x18), .b(x03), .O(new_n110_));
  oai21  g064(.a(x18), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n87_), .O(new_n112_));
  nor2   g066(.a(x23), .b(x22), .O(new_n113_));
  nor2   g067(.a(new_n113_), .b(new_n103_), .O(new_n114_));
  nor2   g068(.a(x22), .b(x21), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n85_), .c(new_n54_), .O(new_n116_));
  oai21  g070(.a(new_n97_), .b(new_n61_), .c(new_n116_), .O(new_n117_));
  oai21  g071(.a(new_n117_), .b(new_n114_), .c(x16), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n112_), .c(new_n81_), .O(z12));
  nand2  g073(.a(new_n116_), .b(new_n48_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(x18), .b(new_n64_), .c(new_n122_), .O(new_n123_));
  aoi21  g077(.a(new_n123_), .b(new_n87_), .c(new_n49_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n121_), .O(z13));
  nand2  g079(.a(x18), .b(x05), .O(new_n126_));
  oai21  g080(.a(x18), .b(new_n67_), .c(new_n126_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n87_), .O(new_n128_));
  nand2  g082(.a(new_n85_), .b(x16), .O(new_n129_));
  inv1   g083(.a(x24), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n103_), .c(new_n54_), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n129_), .c(new_n48_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n61_), .O(new_n133_));
  nand3  g087(.a(new_n113_), .b(new_n97_), .c(new_n103_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(x24), .c(x16), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n133_), .c(new_n128_), .O(z14));
  nand2  g090(.a(x18), .b(x06), .O(new_n137_));
  oai21  g091(.a(x18), .b(new_n70_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n87_), .O(new_n139_));
  nand3  g093(.a(new_n104_), .b(new_n72_), .c(new_n130_), .O(new_n140_));
  oai21  g094(.a(new_n140_), .b(new_n129_), .c(new_n48_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n61_), .O(new_n142_));
  nand4  g096(.a(new_n115_), .b(new_n97_), .c(new_n130_), .d(new_n48_), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(x25), .c(x16), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n142_), .c(new_n139_), .O(z15));
  nand2  g099(.a(x18), .b(x07), .O(new_n146_));
  oai21  g100(.a(x18), .b(new_n75_), .c(new_n146_), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n81_), .c(new_n87_), .O(new_n148_));
  nand3  g102(.a(new_n72_), .b(new_n130_), .c(new_n103_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n96_), .c(x26), .O(new_n150_));
  nor2   g104(.a(x24), .b(x22), .O(new_n151_));
  nor2   g105(.a(x26), .b(x25), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n151_), .c(new_n104_), .d(new_n85_), .O(new_n153_));
  aoi21  g107(.a(new_n153_), .b(new_n150_), .c(x23), .O(new_n154_));
  nor2   g108(.a(new_n77_), .b(new_n61_), .O(new_n155_));
  oai21  g109(.a(new_n155_), .b(new_n154_), .c(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n148_), .O(z16));
  nand2  g111(.a(z08), .b(x17), .O(new_n158_));
  nor2   g112(.a(new_n83_), .b(x17), .O(new_n159_));
  nor3   g113(.a(x26), .b(x25), .c(x24), .O(new_n160_));
  nand4  g114(.a(new_n160_), .b(new_n159_), .c(new_n113_), .d(new_n104_), .O(new_n161_));
  aoi21  g115(.a(new_n161_), .b(new_n158_), .c(new_n87_), .O(z17));
endmodule


