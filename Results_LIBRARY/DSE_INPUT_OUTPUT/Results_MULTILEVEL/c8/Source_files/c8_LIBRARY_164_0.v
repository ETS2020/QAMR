// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x20), .b(x18), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x27), .O(new_n51_));
  inv1   g005(.a(x18), .O(new_n52_));
  inv1   g006(.a(x20), .O(new_n53_));
  oai21  g007(.a(new_n51_), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  oai21  g008(.a(new_n51_), .b(x09), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n56_), .c(new_n48_), .O(new_n57_));
  oai21  g011(.a(x27), .b(x21), .c(new_n57_), .O(z02));
  inv1   g012(.a(x11), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x22), .O(new_n61_));
  nand2  g015(.a(new_n51_), .b(new_n61_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n60_), .c(new_n48_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n48_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n51_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n48_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n48_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n48_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  inv1   g031(.a(new_n48_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n51_), .O(z08));
  inv1   g033(.a(x17), .O(new_n80_));
  inv1   g034(.a(x19), .O(new_n81_));
  nor2   g035(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g036(.a(x19), .b(x17), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n82_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x16), .O(new_n85_));
  nand2  g039(.a(x18), .b(x00), .O(new_n86_));
  oai21  g040(.a(x18), .b(new_n47_), .c(new_n86_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n84_), .c(new_n78_), .O(z09));
  inv1   g043(.a(x09), .O(new_n90_));
  nand2  g044(.a(x18), .b(x01), .O(new_n91_));
  nand2  g045(.a(x20), .b(new_n52_), .O(new_n92_));
  oai21  g046(.a(new_n92_), .b(new_n90_), .c(new_n91_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n85_), .O(new_n94_));
  nand4  g048(.a(new_n53_), .b(new_n81_), .c(x18), .d(new_n80_), .O(new_n95_));
  oai21  g049(.a(new_n83_), .b(new_n53_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(x16), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n94_), .O(z10));
  nand2  g052(.a(x18), .b(x02), .O(new_n99_));
  oai21  g053(.a(x18), .b(new_n56_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n85_), .O(new_n101_));
  inv1   g055(.a(x21), .O(new_n102_));
  oai21  g056(.a(new_n102_), .b(new_n85_), .c(x20), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n52_), .O(new_n104_));
  nor3   g058(.a(x20), .b(x19), .c(x17), .O(new_n105_));
  nor2   g059(.a(x21), .b(x20), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n83_), .O(new_n107_));
  oai21  g061(.a(new_n105_), .b(new_n102_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n104_), .c(new_n101_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(new_n92_), .b(new_n59_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n85_), .O(new_n113_));
  nor2   g067(.a(x21), .b(x19), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n80_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(x22), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n83_), .c(new_n53_), .O(new_n118_));
  aoi21  g072(.a(new_n118_), .b(new_n116_), .c(new_n52_), .O(new_n119_));
  nor2   g073(.a(new_n61_), .b(new_n53_), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n119_), .c(x16), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n113_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(new_n92_), .b(new_n64_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n85_), .O(new_n125_));
  nand2  g079(.a(new_n117_), .b(new_n83_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(x23), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n105_), .c(new_n102_), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n127_), .c(new_n52_), .O(new_n130_));
  inv1   g084(.a(x23), .O(new_n131_));
  nor2   g085(.a(new_n131_), .b(new_n53_), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(new_n130_), .c(x16), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n125_), .O(z13));
  nand2  g088(.a(x18), .b(x05), .O(new_n135_));
  oai21  g089(.a(new_n92_), .b(new_n67_), .c(new_n135_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n85_), .O(new_n137_));
  nand3  g091(.a(new_n128_), .b(new_n83_), .c(new_n102_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x24), .O(new_n139_));
  nand4  g093(.a(new_n117_), .b(new_n105_), .c(new_n69_), .d(new_n131_), .O(new_n140_));
  aoi21  g094(.a(new_n140_), .b(new_n139_), .c(new_n52_), .O(new_n141_));
  nor2   g095(.a(new_n69_), .b(new_n53_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n141_), .c(x16), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n137_), .O(z14));
  nand2  g098(.a(x18), .b(x06), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n72_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n85_), .O(new_n147_));
  nand2  g101(.a(x25), .b(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(x20), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n52_), .O(new_n150_));
  nand2  g104(.a(new_n140_), .b(x25), .O(new_n151_));
  nor2   g105(.a(x25), .b(x24), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n128_), .c(new_n106_), .d(new_n83_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(x16), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n150_), .c(new_n147_), .O(z15));
  nand2  g110(.a(x18), .b(x07), .O(new_n157_));
  oai21  g111(.a(new_n92_), .b(new_n75_), .c(new_n157_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n85_), .O(new_n159_));
  nand4  g113(.a(new_n152_), .b(new_n128_), .c(new_n114_), .d(new_n80_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(x26), .O(new_n161_));
  nor3   g115(.a(x26), .b(x25), .c(x24), .O(new_n162_));
  nand4  g116(.a(new_n162_), .b(new_n128_), .c(new_n106_), .d(new_n83_), .O(new_n163_));
  aoi21  g117(.a(new_n163_), .b(new_n161_), .c(new_n52_), .O(new_n164_));
  and2   g118(.a(x26), .b(x20), .O(new_n165_));
  oai21  g119(.a(new_n165_), .b(new_n164_), .c(x16), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n159_), .O(z16));
  nand2  g121(.a(new_n162_), .b(new_n128_), .O(new_n168_));
  nand3  g122(.a(new_n106_), .b(x19), .c(new_n80_), .O(new_n169_));
  oai22  g123(.a(new_n169_), .b(new_n168_), .c(new_n51_), .d(new_n80_), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(x16), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n78_), .O(z17));
endmodule


