// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x10), .O(new_n48_));
  nor2   g002(.a(x18), .b(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x21), .O(new_n55_));
  oai21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  inv1   g010(.a(x27), .O(new_n57_));
  nand3  g011(.a(new_n57_), .b(new_n55_), .c(x18), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n56_), .O(z02));
  inv1   g013(.a(x11), .O(new_n60_));
  aoi21  g014(.a(x27), .b(new_n60_), .c(new_n49_), .O(new_n61_));
  oai21  g015(.a(x27), .b(x22), .c(new_n61_), .O(z03));
  inv1   g016(.a(x12), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x23), .O(new_n65_));
  nand2  g019(.a(new_n57_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n49_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x24), .c(new_n69_), .O(z05));
  inv1   g024(.a(x14), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x25), .O(new_n73_));
  nand2  g027(.a(new_n57_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n49_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x26), .O(new_n78_));
  nand2  g032(.a(new_n57_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n49_), .O(z07));
  inv1   g034(.a(new_n49_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n57_), .O(z08));
  nor2   g036(.a(x18), .b(x10), .O(new_n83_));
  aoi22  g037(.a(new_n83_), .b(x08), .c(x18), .d(x00), .O(new_n84_));
  xnor2a g038(.a(x19), .b(x17), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n81_), .c(x16), .O(new_n86_));
  oai21  g040(.a(new_n84_), .b(x16), .c(new_n86_), .O(z09));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x01), .O(new_n89_));
  nand2  g043(.a(new_n83_), .b(x09), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  inv1   g046(.a(x20), .O(new_n93_));
  nor2   g047(.a(x19), .b(x17), .O(new_n94_));
  inv1   g048(.a(x17), .O(new_n95_));
  nor2   g049(.a(x20), .b(x19), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g051(.a(new_n94_), .b(new_n93_), .c(new_n97_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n81_), .c(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n92_), .O(z10));
  nor2   g054(.a(x21), .b(x20), .O(new_n101_));
  aoi22  g055(.a(new_n101_), .b(new_n94_), .c(new_n97_), .d(x21), .O(new_n102_));
  inv1   g056(.a(x18), .O(new_n103_));
  nor2   g057(.a(new_n103_), .b(x16), .O(new_n104_));
  aoi21  g058(.a(new_n104_), .b(x02), .c(new_n49_), .O(new_n105_));
  oai21  g059(.a(new_n102_), .b(new_n88_), .c(new_n105_), .O(z11));
  nand2  g060(.a(x18), .b(x03), .O(new_n107_));
  nand3  g061(.a(new_n103_), .b(x11), .c(new_n48_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n88_), .O(new_n110_));
  nand2  g064(.a(new_n101_), .b(new_n94_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x22), .O(new_n112_));
  nor2   g066(.a(x22), .b(x21), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n94_), .c(new_n93_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n81_), .c(x16), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n110_), .O(z12));
  nand2  g071(.a(x18), .b(x04), .O(new_n118_));
  nand3  g072(.a(new_n103_), .b(x12), .c(new_n48_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n88_), .O(new_n121_));
  nand2  g075(.a(new_n114_), .b(x23), .O(new_n122_));
  nor3   g076(.a(x20), .b(x19), .c(x17), .O(new_n123_));
  nor3   g077(.a(x23), .b(x22), .c(x21), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n81_), .c(x16), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n121_), .O(z13));
  nand2  g082(.a(x18), .b(x05), .O(new_n129_));
  oai21  g083(.a(x18), .b(new_n68_), .c(new_n129_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n88_), .O(new_n131_));
  inv1   g085(.a(x24), .O(new_n132_));
  aoi21  g086(.a(new_n124_), .b(new_n123_), .c(new_n132_), .O(new_n133_));
  nor2   g087(.a(x24), .b(x23), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n113_), .c(new_n96_), .d(new_n95_), .O(new_n135_));
  inv1   g089(.a(new_n135_), .O(new_n136_));
  oai21  g090(.a(new_n136_), .b(new_n133_), .c(x16), .O(new_n137_));
  nand3  g091(.a(new_n137_), .b(new_n131_), .c(new_n81_), .O(z14));
  nand2  g092(.a(x18), .b(x06), .O(new_n139_));
  nand3  g093(.a(new_n103_), .b(x14), .c(new_n48_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n88_), .O(new_n142_));
  nand2  g096(.a(new_n135_), .b(x25), .O(new_n143_));
  nor2   g097(.a(x23), .b(x22), .O(new_n144_));
  nor2   g098(.a(x25), .b(x24), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n144_), .c(new_n101_), .d(new_n94_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n143_), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n81_), .c(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n142_), .O(z15));
  nand2  g103(.a(x18), .b(x07), .O(new_n150_));
  nand3  g104(.a(new_n103_), .b(x15), .c(new_n48_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n88_), .O(new_n153_));
  nand2  g107(.a(new_n146_), .b(x26), .O(new_n154_));
  nor3   g108(.a(x26), .b(x25), .c(x24), .O(new_n155_));
  nand4  g109(.a(new_n155_), .b(new_n144_), .c(new_n101_), .d(new_n94_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n81_), .c(x16), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n153_), .O(z16));
  nand4  g113(.a(new_n144_), .b(new_n78_), .c(new_n73_), .d(new_n132_), .O(new_n160_));
  nand3  g114(.a(new_n101_), .b(x19), .c(new_n95_), .O(new_n161_));
  oai22  g115(.a(new_n161_), .b(new_n160_), .c(new_n57_), .d(new_n95_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x16), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n81_), .O(z17));
endmodule


