// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_;
  inv1   g000(.a(x19), .O(new_n47_));
  inv1   g001(.a(x27), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x18), .O(new_n52_));
  nand2  g006(.a(x23), .b(new_n52_), .O(new_n53_));
  nand3  g007(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g008(.a(x20), .O(new_n55_));
  nand2  g009(.a(new_n48_), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x09), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  nand3  g012(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(z01));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n48_), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x10), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  nand3  g017(.a(new_n63_), .b(new_n61_), .c(new_n53_), .O(z02));
  inv1   g018(.a(new_n53_), .O(new_n65_));
  inv1   g019(.a(x11), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x22), .O(new_n68_));
  nand2  g022(.a(new_n48_), .b(new_n68_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(z03));
  inv1   g024(.a(x12), .O(new_n71_));
  inv1   g025(.a(x23), .O(new_n72_));
  oai21  g026(.a(x27), .b(new_n72_), .c(new_n71_), .O(new_n73_));
  nand2  g027(.a(new_n48_), .b(new_n72_), .O(new_n74_));
  nand3  g028(.a(new_n74_), .b(new_n73_), .c(new_n53_), .O(z04));
  inv1   g029(.a(x13), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x24), .O(new_n78_));
  nand2  g032(.a(new_n48_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n65_), .O(z05));
  inv1   g034(.a(x14), .O(new_n81_));
  nand2  g035(.a(x27), .b(new_n81_), .O(new_n82_));
  inv1   g036(.a(x25), .O(new_n83_));
  nand2  g037(.a(new_n48_), .b(new_n83_), .O(new_n84_));
  aoi21  g038(.a(new_n84_), .b(new_n82_), .c(new_n65_), .O(z06));
  inv1   g039(.a(x26), .O(new_n86_));
  nand2  g040(.a(new_n48_), .b(new_n86_), .O(new_n87_));
  inv1   g041(.a(x15), .O(new_n88_));
  nand2  g042(.a(x27), .b(new_n88_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n87_), .c(new_n53_), .O(z07));
  nand2  g044(.a(new_n53_), .b(new_n48_), .O(z08));
  nor2   g045(.a(x23), .b(x18), .O(new_n92_));
  aoi22  g046(.a(new_n92_), .b(x08), .c(x18), .d(x00), .O(new_n93_));
  xnor2a g047(.a(x19), .b(x17), .O(new_n94_));
  nand3  g048(.a(new_n94_), .b(new_n53_), .c(x16), .O(new_n95_));
  oai21  g049(.a(new_n93_), .b(x16), .c(new_n95_), .O(z09));
  aoi22  g050(.a(new_n92_), .b(x09), .c(x18), .d(x01), .O(new_n97_));
  nor2   g051(.a(x19), .b(x17), .O(new_n98_));
  inv1   g052(.a(x17), .O(new_n99_));
  nand3  g053(.a(new_n55_), .b(new_n47_), .c(new_n99_), .O(new_n100_));
  oai21  g054(.a(new_n98_), .b(new_n55_), .c(new_n100_), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n53_), .c(x16), .O(new_n102_));
  oai21  g056(.a(new_n97_), .b(x16), .c(new_n102_), .O(z10));
  inv1   g057(.a(x16), .O(new_n104_));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  oai21  g059(.a(x18), .b(new_n62_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nor3   g061(.a(x20), .b(x19), .c(x17), .O(new_n108_));
  nand3  g062(.a(new_n98_), .b(new_n60_), .c(new_n55_), .O(new_n109_));
  oai21  g063(.a(new_n108_), .b(new_n60_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n107_), .c(new_n53_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n66_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n104_), .O(new_n115_));
  inv1   g069(.a(new_n109_), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n98_), .c(new_n55_), .O(new_n118_));
  oai21  g072(.a(new_n116_), .b(new_n68_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n115_), .c(new_n53_), .O(z12));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(x18), .b(new_n71_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n104_), .O(new_n124_));
  nand2  g078(.a(new_n118_), .b(x23), .O(new_n125_));
  nor2   g079(.a(x23), .b(x22), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n108_), .c(new_n60_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n124_), .c(new_n53_), .O(z13));
  inv1   g084(.a(new_n92_), .O(new_n131_));
  nand2  g085(.a(x18), .b(x05), .O(new_n132_));
  oai21  g086(.a(new_n131_), .b(new_n76_), .c(new_n132_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n104_), .O(new_n134_));
  nand2  g088(.a(new_n118_), .b(x24), .O(new_n135_));
  nor3   g089(.a(x24), .b(x22), .c(x21), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n108_), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(new_n135_), .c(x23), .O(new_n138_));
  nand3  g092(.a(x24), .b(x23), .c(x18), .O(new_n139_));
  inv1   g093(.a(new_n139_), .O(new_n140_));
  oai21  g094(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n134_), .O(z14));
  nand2  g096(.a(x18), .b(x06), .O(new_n143_));
  oai21  g097(.a(new_n131_), .b(new_n81_), .c(new_n143_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n104_), .O(new_n145_));
  nand3  g099(.a(new_n78_), .b(new_n68_), .c(new_n60_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n100_), .c(x25), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n117_), .c(new_n108_), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n147_), .c(x23), .O(new_n150_));
  nand3  g104(.a(x25), .b(x23), .c(x18), .O(new_n151_));
  inv1   g105(.a(new_n151_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n145_), .O(z15));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n88_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n104_), .O(new_n157_));
  oai21  g111(.a(new_n86_), .b(new_n104_), .c(x18), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x23), .O(new_n159_));
  nand2  g113(.a(new_n149_), .b(x26), .O(new_n160_));
  nor3   g114(.a(x26), .b(x25), .c(x24), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n126_), .c(new_n116_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n159_), .c(new_n157_), .O(z16));
  nand2  g119(.a(new_n161_), .b(new_n126_), .O(new_n166_));
  nand4  g120(.a(new_n60_), .b(new_n55_), .c(x19), .d(new_n99_), .O(new_n167_));
  oai22  g121(.a(new_n167_), .b(new_n166_), .c(new_n48_), .d(new_n99_), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(x16), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(new_n53_), .O(z17));
endmodule


