// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_;
  nor2   g000(.a(x21), .b(x18), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n47_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x20), .c(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  inv1   g011(.a(x18), .O(new_n58_));
  nand2  g012(.a(x27), .b(x21), .O(new_n59_));
  oai21  g013(.a(x21), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  nand2  g014(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  inv1   g015(.a(x21), .O(new_n62_));
  nand3  g016(.a(new_n51_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g017(.a(new_n63_), .b(new_n61_), .O(z02));
  inv1   g018(.a(x11), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n47_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x22), .c(new_n66_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x23), .O(new_n70_));
  nand2  g024(.a(new_n51_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n47_), .O(z04));
  inv1   g026(.a(x13), .O(new_n73_));
  aoi21  g027(.a(x27), .b(new_n73_), .c(new_n47_), .O(new_n74_));
  oai21  g028(.a(x27), .b(x24), .c(new_n74_), .O(z05));
  inv1   g029(.a(x14), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x25), .O(new_n78_));
  nand2  g032(.a(new_n51_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n47_), .O(z06));
  inv1   g034(.a(x15), .O(new_n81_));
  aoi21  g035(.a(x27), .b(new_n81_), .c(new_n47_), .O(new_n82_));
  oai21  g036(.a(x27), .b(x26), .c(new_n82_), .O(z07));
  nor2   g037(.a(new_n47_), .b(new_n51_), .O(z08));
  inv1   g038(.a(x16), .O(new_n85_));
  nand2  g039(.a(x18), .b(x00), .O(new_n86_));
  nand2  g040(.a(x21), .b(new_n58_), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n48_), .c(new_n86_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  inv1   g043(.a(new_n47_), .O(new_n90_));
  xnor2a g044(.a(x19), .b(x17), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n89_), .O(z09));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  oai21  g048(.a(new_n87_), .b(new_n54_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n85_), .O(new_n96_));
  inv1   g050(.a(x20), .O(new_n97_));
  nor2   g051(.a(x19), .b(x17), .O(new_n98_));
  inv1   g052(.a(x17), .O(new_n99_));
  nor2   g053(.a(x20), .b(x19), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  oai21  g055(.a(new_n98_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n90_), .c(x16), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n96_), .O(z10));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  oai21  g059(.a(new_n87_), .b(new_n57_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n85_), .O(new_n107_));
  nor2   g061(.a(new_n58_), .b(x17), .O(new_n108_));
  nor3   g062(.a(x21), .b(x20), .c(x19), .O(new_n109_));
  aoi22  g063(.a(new_n109_), .b(new_n108_), .c(new_n101_), .d(x21), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n85_), .c(new_n107_), .O(z11));
  nand2  g065(.a(x18), .b(x03), .O(new_n112_));
  oai21  g066(.a(new_n87_), .b(new_n65_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n85_), .O(new_n114_));
  nand2  g068(.a(new_n101_), .b(x22), .O(new_n115_));
  inv1   g069(.a(x22), .O(new_n116_));
  nand4  g070(.a(new_n98_), .b(new_n116_), .c(new_n62_), .d(new_n97_), .O(new_n117_));
  aoi21  g071(.a(new_n117_), .b(new_n115_), .c(new_n58_), .O(new_n118_));
  nor2   g072(.a(new_n116_), .b(new_n62_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n118_), .c(x16), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n114_), .O(z12));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(new_n87_), .b(new_n68_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n85_), .O(new_n124_));
  nand3  g078(.a(new_n98_), .b(new_n116_), .c(new_n97_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x23), .O(new_n126_));
  nor3   g080(.a(x20), .b(x19), .c(x17), .O(new_n127_));
  nor3   g081(.a(x23), .b(x22), .c(x21), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n126_), .c(new_n58_), .O(new_n130_));
  nor2   g084(.a(new_n70_), .b(new_n62_), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n130_), .c(x16), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n124_), .O(z13));
  nand2  g087(.a(x18), .b(x05), .O(new_n134_));
  oai21  g088(.a(x18), .b(new_n73_), .c(new_n134_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n85_), .O(new_n136_));
  inv1   g090(.a(x24), .O(new_n137_));
  oai21  g091(.a(new_n137_), .b(new_n85_), .c(x21), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n58_), .O(new_n139_));
  aoi21  g093(.a(new_n128_), .b(new_n127_), .c(new_n137_), .O(new_n140_));
  nand4  g094(.a(new_n137_), .b(new_n70_), .c(new_n116_), .d(new_n62_), .O(new_n141_));
  nor2   g095(.a(new_n141_), .b(new_n101_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n139_), .c(new_n136_), .O(z14));
  nand2  g098(.a(x18), .b(x06), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n76_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n85_), .O(new_n147_));
  oai21  g101(.a(new_n78_), .b(new_n85_), .c(x21), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n58_), .O(new_n149_));
  nor2   g103(.a(x21), .b(x20), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n98_), .O(new_n151_));
  nor2   g105(.a(x23), .b(x22), .O(new_n152_));
  nor2   g106(.a(x25), .b(x24), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  oai22  g108(.a(new_n154_), .b(new_n151_), .c(new_n142_), .d(new_n78_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x16), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(new_n149_), .c(new_n147_), .O(z15));
  nand2  g111(.a(x18), .b(x07), .O(new_n158_));
  oai21  g112(.a(new_n87_), .b(new_n81_), .c(new_n158_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n85_), .O(new_n160_));
  nand4  g114(.a(new_n153_), .b(new_n152_), .c(new_n100_), .d(new_n99_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x26), .O(new_n162_));
  nor3   g116(.a(x26), .b(x25), .c(x24), .O(new_n163_));
  nand4  g117(.a(new_n163_), .b(new_n152_), .c(new_n150_), .d(new_n98_), .O(new_n164_));
  aoi21  g118(.a(new_n164_), .b(new_n162_), .c(new_n58_), .O(new_n165_));
  and2   g119(.a(x26), .b(x21), .O(new_n166_));
  oai21  g120(.a(new_n166_), .b(new_n165_), .c(x16), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n160_), .O(z16));
  nand2  g122(.a(new_n163_), .b(new_n152_), .O(new_n169_));
  nand3  g123(.a(new_n150_), .b(x19), .c(new_n99_), .O(new_n170_));
  oai22  g124(.a(new_n170_), .b(new_n169_), .c(new_n51_), .d(new_n99_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n90_), .O(z17));
endmodule


