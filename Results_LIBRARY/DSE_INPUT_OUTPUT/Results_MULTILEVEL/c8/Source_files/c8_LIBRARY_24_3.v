// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_;
  nand2  g000(.a(x22), .b(x18), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x20), .O(new_n57_));
  nand2  g011(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  aoi21  g012(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(z01));
  inv1   g013(.a(x10), .O(new_n60_));
  nand2  g014(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x21), .O(new_n62_));
  nand2  g016(.a(new_n52_), .b(new_n62_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(z02));
  inv1   g018(.a(x18), .O(new_n65_));
  inv1   g019(.a(x22), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n65_), .c(new_n66_), .O(new_n67_));
  oai22  g021(.a(new_n67_), .b(x11), .c(x27), .d(x22), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n48_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x23), .c(new_n70_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x24), .O(new_n74_));
  nand2  g028(.a(new_n52_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n48_), .O(z05));
  inv1   g030(.a(x14), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n48_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x25), .c(new_n78_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  aoi21  g034(.a(x27), .b(new_n80_), .c(new_n48_), .O(new_n81_));
  oai21  g035(.a(x27), .b(x26), .c(new_n81_), .O(z07));
  nor2   g036(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g037(.a(x17), .O(new_n84_));
  nor2   g038(.a(new_n51_), .b(new_n84_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x00), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n49_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n87_), .c(new_n47_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n55_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nor2   g049(.a(new_n86_), .b(new_n57_), .O(new_n96_));
  nor3   g050(.a(x20), .b(x19), .c(x17), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n96_), .c(x16), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n95_), .c(new_n47_), .O(z10));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  oai21  g054(.a(x18), .b(new_n60_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n88_), .O(new_n102_));
  nand3  g056(.a(new_n57_), .b(new_n51_), .c(new_n84_), .O(new_n103_));
  nand2  g057(.a(x22), .b(x18), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n103_), .c(x21), .O(new_n105_));
  nor2   g059(.a(x21), .b(x20), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n86_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n102_), .c(new_n47_), .O(z11));
  inv1   g064(.a(x03), .O(new_n111_));
  nand2  g065(.a(new_n65_), .b(x11), .O(new_n112_));
  nand2  g066(.a(new_n66_), .b(x18), .O(new_n113_));
  oai21  g067(.a(new_n113_), .b(new_n111_), .c(new_n112_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n88_), .O(new_n115_));
  nand3  g069(.a(new_n107_), .b(x22), .c(new_n65_), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n86_), .c(new_n57_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n115_), .O(z12));
  inv1   g075(.a(x04), .O(new_n122_));
  oai22  g076(.a(new_n113_), .b(new_n122_), .c(x18), .d(new_n69_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n88_), .O(new_n124_));
  nand2  g078(.a(new_n107_), .b(x23), .O(new_n125_));
  nor2   g079(.a(x23), .b(x21), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n86_), .c(new_n57_), .O(new_n127_));
  aoi21  g081(.a(new_n127_), .b(new_n125_), .c(x22), .O(new_n128_));
  inv1   g082(.a(x23), .O(new_n129_));
  nor3   g083(.a(new_n129_), .b(new_n66_), .c(x18), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n128_), .c(x16), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n124_), .O(z13));
  nand2  g086(.a(x18), .b(x05), .O(new_n133_));
  oai21  g087(.a(x18), .b(new_n72_), .c(new_n133_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n88_), .O(new_n135_));
  oai21  g089(.a(new_n74_), .b(new_n88_), .c(new_n65_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(x22), .O(new_n137_));
  nand2  g091(.a(new_n127_), .b(x24), .O(new_n138_));
  nand4  g092(.a(new_n117_), .b(new_n97_), .c(new_n74_), .d(new_n129_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x16), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n137_), .c(new_n135_), .O(z14));
  inv1   g096(.a(x06), .O(new_n143_));
  oai22  g097(.a(new_n113_), .b(new_n143_), .c(x18), .d(new_n77_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n88_), .O(new_n145_));
  nand3  g099(.a(new_n74_), .b(new_n129_), .c(new_n62_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n103_), .c(x25), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n126_), .c(new_n97_), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n147_), .c(x22), .O(new_n150_));
  nand2  g104(.a(x25), .b(x22), .O(new_n151_));
  nor2   g105(.a(new_n151_), .b(x18), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n145_), .O(z15));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n80_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n88_), .O(new_n157_));
  nand2  g111(.a(x26), .b(x16), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n65_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(x22), .O(new_n160_));
  nand2  g114(.a(new_n149_), .b(x26), .O(new_n161_));
  nor2   g115(.a(x23), .b(x22), .O(new_n162_));
  nor3   g116(.a(x26), .b(x25), .c(x24), .O(new_n163_));
  nand4  g117(.a(new_n163_), .b(new_n162_), .c(new_n106_), .d(new_n86_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x16), .O(new_n166_));
  nand3  g120(.a(new_n166_), .b(new_n160_), .c(new_n157_), .O(z16));
  nand2  g121(.a(z08), .b(x17), .O(new_n168_));
  nor2   g122(.a(new_n51_), .b(x17), .O(new_n169_));
  nand4  g123(.a(new_n169_), .b(new_n163_), .c(new_n162_), .d(new_n106_), .O(new_n170_));
  aoi21  g124(.a(new_n170_), .b(new_n168_), .c(new_n88_), .O(z17));
endmodule


