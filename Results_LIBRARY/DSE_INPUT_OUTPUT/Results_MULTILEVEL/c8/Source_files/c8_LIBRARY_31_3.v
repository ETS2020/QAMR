// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_;
  nand2  g000(.a(x21), .b(x18), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  oai21  g013(.a(x27), .b(new_n59_), .c(new_n58_), .O(new_n60_));
  nand2  g014(.a(new_n52_), .b(new_n59_), .O(new_n61_));
  nand3  g015(.a(new_n61_), .b(new_n60_), .c(new_n47_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n52_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n48_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n48_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x23), .c(new_n69_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x24), .O(new_n73_));
  nand2  g027(.a(new_n52_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n48_), .O(z05));
  inv1   g029(.a(x14), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n48_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x25), .c(new_n77_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  aoi21  g033(.a(x27), .b(new_n79_), .c(new_n48_), .O(new_n80_));
  oai21  g034(.a(x27), .b(x26), .c(new_n80_), .O(z07));
  nor2   g035(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g036(.a(x16), .O(new_n83_));
  inv1   g037(.a(x00), .O(new_n84_));
  nand2  g038(.a(new_n59_), .b(x18), .O(new_n85_));
  oai22  g039(.a(new_n85_), .b(new_n84_), .c(x18), .d(new_n49_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  xnor2a g041(.a(x19), .b(x17), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n47_), .c(x16), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n87_), .O(z09));
  inv1   g044(.a(x01), .O(new_n91_));
  oai22  g045(.a(new_n85_), .b(new_n91_), .c(x18), .d(new_n55_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n83_), .O(new_n93_));
  inv1   g047(.a(x20), .O(new_n94_));
  nor2   g048(.a(x19), .b(x17), .O(new_n95_));
  inv1   g049(.a(x17), .O(new_n96_));
  nand3  g050(.a(new_n94_), .b(new_n51_), .c(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n47_), .c(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n93_), .O(z10));
  nor2   g054(.a(x18), .b(new_n58_), .O(new_n101_));
  aoi21  g055(.a(x18), .b(x02), .c(new_n101_), .O(new_n102_));
  nor3   g056(.a(x20), .b(x19), .c(x17), .O(new_n103_));
  nor2   g057(.a(x21), .b(x20), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n95_), .O(new_n105_));
  oai21  g059(.a(new_n103_), .b(new_n59_), .c(new_n105_), .O(new_n106_));
  aoi21  g060(.a(new_n106_), .b(x16), .c(new_n48_), .O(new_n107_));
  oai21  g061(.a(new_n102_), .b(x16), .c(new_n107_), .O(z11));
  inv1   g062(.a(x03), .O(new_n109_));
  oai22  g063(.a(new_n85_), .b(new_n109_), .c(x18), .d(new_n63_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n83_), .O(new_n111_));
  nand3  g065(.a(new_n95_), .b(new_n65_), .c(new_n94_), .O(new_n112_));
  oai21  g066(.a(new_n103_), .b(new_n65_), .c(new_n112_), .O(new_n113_));
  nor3   g067(.a(new_n65_), .b(new_n59_), .c(x18), .O(new_n114_));
  aoi21  g068(.a(new_n113_), .b(new_n59_), .c(new_n114_), .O(new_n115_));
  oai21  g069(.a(new_n115_), .b(new_n83_), .c(new_n111_), .O(z12));
  inv1   g070(.a(x04), .O(new_n117_));
  oai22  g071(.a(new_n85_), .b(new_n117_), .c(x18), .d(new_n68_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n83_), .O(new_n119_));
  nand2  g073(.a(new_n112_), .b(x23), .O(new_n120_));
  nor2   g074(.a(x23), .b(x22), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n95_), .c(new_n94_), .O(new_n122_));
  aoi21  g076(.a(new_n122_), .b(new_n120_), .c(x21), .O(new_n123_));
  inv1   g077(.a(x23), .O(new_n124_));
  nor3   g078(.a(new_n124_), .b(new_n59_), .c(x18), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n123_), .c(x16), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n119_), .O(z13));
  inv1   g081(.a(x05), .O(new_n128_));
  oai22  g082(.a(new_n85_), .b(new_n128_), .c(x18), .d(new_n71_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n83_), .O(new_n130_));
  nand2  g084(.a(new_n122_), .b(x24), .O(new_n131_));
  nor3   g085(.a(x24), .b(x23), .c(x22), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n103_), .O(new_n133_));
  aoi21  g087(.a(new_n133_), .b(new_n131_), .c(x21), .O(new_n134_));
  nor3   g088(.a(new_n73_), .b(new_n59_), .c(x18), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n134_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n130_), .O(z14));
  inv1   g091(.a(x06), .O(new_n138_));
  oai22  g092(.a(new_n85_), .b(new_n138_), .c(x18), .d(new_n76_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n83_), .O(new_n140_));
  nand3  g094(.a(new_n73_), .b(new_n124_), .c(new_n65_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n97_), .c(x25), .O(new_n142_));
  nor2   g096(.a(x25), .b(x24), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n121_), .c(new_n103_), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(new_n142_), .c(x21), .O(new_n145_));
  nand2  g099(.a(x25), .b(x21), .O(new_n146_));
  nor2   g100(.a(new_n146_), .b(x18), .O(new_n147_));
  oai21  g101(.a(new_n147_), .b(new_n145_), .c(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n140_), .O(z15));
  nand2  g103(.a(x18), .b(x07), .O(new_n150_));
  oai21  g104(.a(x18), .b(new_n79_), .c(new_n150_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n83_), .O(new_n152_));
  and2   g106(.a(x26), .b(x16), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(x18), .c(x21), .O(new_n154_));
  nand2  g108(.a(new_n144_), .b(x26), .O(new_n155_));
  nor3   g109(.a(x26), .b(x25), .c(x24), .O(new_n156_));
  nand4  g110(.a(new_n156_), .b(new_n121_), .c(new_n104_), .d(new_n95_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x16), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(new_n154_), .c(new_n152_), .O(z16));
  nand2  g114(.a(z08), .b(x17), .O(new_n161_));
  nor2   g115(.a(new_n51_), .b(x17), .O(new_n162_));
  nand4  g116(.a(new_n162_), .b(new_n156_), .c(new_n121_), .d(new_n104_), .O(new_n163_));
  aoi21  g117(.a(new_n163_), .b(new_n161_), .c(new_n83_), .O(z17));
endmodule


