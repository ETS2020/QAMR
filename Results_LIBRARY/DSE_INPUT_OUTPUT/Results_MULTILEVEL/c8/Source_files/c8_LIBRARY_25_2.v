// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x19), .b(x18), .O(new_n48_));
  inv1   g002(.a(x19), .O(new_n49_));
  inv1   g003(.a(x27), .O(new_n50_));
  nor2   g004(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g005(.a(new_n51_), .b(new_n48_), .c(new_n47_), .O(new_n52_));
  inv1   g006(.a(x18), .O(new_n53_));
  nand3  g007(.a(new_n50_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  nand2  g008(.a(new_n54_), .b(new_n52_), .O(z00));
  inv1   g009(.a(x09), .O(new_n56_));
  nor2   g010(.a(x19), .b(new_n53_), .O(new_n57_));
  aoi21  g011(.a(x27), .b(new_n56_), .c(new_n57_), .O(new_n58_));
  oai21  g012(.a(x27), .b(x20), .c(new_n58_), .O(z01));
  inv1   g013(.a(x10), .O(new_n60_));
  nand2  g014(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x21), .O(new_n62_));
  nand2  g016(.a(new_n50_), .b(new_n62_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(z02));
  inv1   g018(.a(x11), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n57_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x22), .c(new_n66_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x23), .O(new_n70_));
  nand2  g024(.a(new_n50_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n57_), .O(z04));
  inv1   g026(.a(x13), .O(new_n73_));
  aoi21  g027(.a(x27), .b(new_n73_), .c(new_n57_), .O(new_n74_));
  oai21  g028(.a(x27), .b(x24), .c(new_n74_), .O(z05));
  inv1   g029(.a(x14), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n57_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x25), .c(new_n77_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  aoi21  g033(.a(x27), .b(new_n79_), .c(new_n57_), .O(new_n80_));
  oai21  g034(.a(x27), .b(x26), .c(new_n80_), .O(z07));
  nor2   g035(.a(new_n57_), .b(new_n50_), .O(z08));
  inv1   g036(.a(x16), .O(new_n83_));
  inv1   g037(.a(x17), .O(new_n84_));
  nor2   g038(.a(new_n49_), .b(new_n84_), .O(new_n85_));
  aoi21  g039(.a(new_n48_), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  inv1   g040(.a(x00), .O(new_n87_));
  nand2  g041(.a(x19), .b(x18), .O(new_n88_));
  oai22  g042(.a(new_n88_), .b(new_n87_), .c(x18), .d(new_n47_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n83_), .O(new_n90_));
  oai21  g044(.a(new_n86_), .b(new_n83_), .c(new_n90_), .O(z09));
  inv1   g045(.a(x01), .O(new_n92_));
  oai22  g046(.a(new_n88_), .b(new_n92_), .c(x18), .d(new_n56_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n83_), .O(new_n94_));
  nand2  g048(.a(x20), .b(x17), .O(new_n95_));
  inv1   g049(.a(x20), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n49_), .c(new_n84_), .O(new_n97_));
  aoi21  g051(.a(new_n97_), .b(new_n95_), .c(x18), .O(new_n98_));
  nor2   g052(.a(new_n96_), .b(new_n49_), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n98_), .c(x16), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n94_), .O(z10));
  inv1   g055(.a(x02), .O(new_n102_));
  oai22  g056(.a(new_n88_), .b(new_n102_), .c(x18), .d(new_n60_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n83_), .O(new_n104_));
  oai21  g058(.a(x20), .b(x17), .c(x21), .O(new_n105_));
  nor2   g059(.a(x19), .b(x17), .O(new_n106_));
  nor2   g060(.a(x21), .b(x20), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g062(.a(new_n108_), .b(new_n105_), .c(x18), .O(new_n109_));
  nor2   g063(.a(new_n62_), .b(new_n49_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n109_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n104_), .O(z11));
  inv1   g066(.a(x03), .O(new_n113_));
  oai22  g067(.a(new_n88_), .b(new_n113_), .c(x18), .d(new_n65_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n83_), .O(new_n115_));
  nand2  g069(.a(new_n107_), .b(new_n84_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x22), .O(new_n117_));
  nor2   g071(.a(x22), .b(x21), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n106_), .c(new_n96_), .O(new_n119_));
  aoi21  g073(.a(new_n119_), .b(new_n117_), .c(x18), .O(new_n120_));
  and2   g074(.a(x22), .b(x19), .O(new_n121_));
  oai21  g075(.a(new_n121_), .b(new_n120_), .c(x16), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n115_), .O(z12));
  inv1   g077(.a(x04), .O(new_n124_));
  oai22  g078(.a(new_n88_), .b(new_n124_), .c(x18), .d(new_n68_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n83_), .O(new_n126_));
  nand3  g080(.a(new_n118_), .b(new_n96_), .c(new_n84_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(x23), .O(new_n128_));
  nor3   g082(.a(x20), .b(x19), .c(x17), .O(new_n129_));
  nor3   g083(.a(x23), .b(x22), .c(x21), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n128_), .c(x18), .O(new_n132_));
  nor2   g086(.a(new_n70_), .b(new_n49_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n132_), .c(x16), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n126_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  oai21  g090(.a(x18), .b(new_n73_), .c(new_n136_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n83_), .O(new_n138_));
  inv1   g092(.a(x24), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n83_), .c(x19), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x18), .O(new_n141_));
  aoi21  g095(.a(new_n130_), .b(new_n129_), .c(new_n139_), .O(new_n142_));
  nand3  g096(.a(new_n118_), .b(new_n139_), .c(new_n70_), .O(new_n143_));
  nor2   g097(.a(new_n143_), .b(new_n97_), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n142_), .c(x16), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n141_), .c(new_n138_), .O(z14));
  nand2  g100(.a(x18), .b(x06), .O(new_n147_));
  oai21  g101(.a(x18), .b(new_n76_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n83_), .O(new_n149_));
  inv1   g103(.a(x25), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n83_), .c(x19), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x18), .O(new_n152_));
  nor2   g106(.a(x23), .b(x22), .O(new_n153_));
  nor2   g107(.a(x25), .b(x24), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  oai22  g109(.a(new_n155_), .b(new_n108_), .c(new_n144_), .d(new_n150_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(x16), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n152_), .c(new_n149_), .O(z15));
  inv1   g112(.a(x07), .O(new_n159_));
  oai22  g113(.a(new_n88_), .b(new_n159_), .c(x18), .d(new_n79_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n83_), .O(new_n161_));
  nand4  g115(.a(new_n154_), .b(new_n153_), .c(new_n107_), .d(new_n84_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x26), .O(new_n163_));
  nor3   g117(.a(x26), .b(x25), .c(x24), .O(new_n164_));
  nand4  g118(.a(new_n164_), .b(new_n153_), .c(new_n107_), .d(new_n106_), .O(new_n165_));
  aoi21  g119(.a(new_n165_), .b(new_n163_), .c(x18), .O(new_n166_));
  and2   g120(.a(x26), .b(x19), .O(new_n167_));
  oai21  g121(.a(new_n167_), .b(new_n166_), .c(x16), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n161_), .O(z16));
  inv1   g123(.a(new_n57_), .O(new_n170_));
  nand2  g124(.a(new_n164_), .b(new_n153_), .O(new_n171_));
  nand3  g125(.a(new_n107_), .b(x19), .c(new_n84_), .O(new_n172_));
  oai22  g126(.a(new_n172_), .b(new_n171_), .c(new_n50_), .d(new_n84_), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(x16), .O(new_n174_));
  nand2  g128(.a(new_n174_), .b(new_n170_), .O(z17));
endmodule


