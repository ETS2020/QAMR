// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x25), .b(x20), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  inv1   g005(.a(x25), .O(new_n52_));
  nand2  g006(.a(x27), .b(x20), .O(new_n53_));
  oai21  g007(.a(new_n52_), .b(x20), .c(new_n53_), .O(new_n54_));
  nand2  g008(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  inv1   g009(.a(x20), .O(new_n56_));
  inv1   g010(.a(x27), .O(new_n57_));
  nand3  g011(.a(new_n57_), .b(x25), .c(new_n56_), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n55_), .O(z01));
  inv1   g013(.a(x10), .O(new_n60_));
  aoi21  g014(.a(x27), .b(new_n60_), .c(new_n48_), .O(new_n61_));
  oai21  g015(.a(x27), .b(x21), .c(new_n61_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n57_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n48_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x23), .O(new_n70_));
  nand2  g024(.a(new_n57_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n48_), .O(z04));
  inv1   g026(.a(x13), .O(new_n73_));
  aoi21  g027(.a(x27), .b(new_n73_), .c(new_n48_), .O(new_n74_));
  oai21  g028(.a(x27), .b(x24), .c(new_n74_), .O(z05));
  inv1   g029(.a(x14), .O(new_n76_));
  nand2  g030(.a(x27), .b(x25), .O(new_n77_));
  oai21  g031(.a(x25), .b(new_n56_), .c(new_n77_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand3  g033(.a(new_n57_), .b(new_n52_), .c(x20), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n79_), .O(z06));
  inv1   g035(.a(x15), .O(new_n82_));
  aoi21  g036(.a(x27), .b(new_n82_), .c(new_n48_), .O(new_n83_));
  oai21  g037(.a(x27), .b(x26), .c(new_n83_), .O(z07));
  nor2   g038(.a(new_n48_), .b(new_n57_), .O(z08));
  inv1   g039(.a(new_n48_), .O(new_n86_));
  inv1   g040(.a(x17), .O(new_n87_));
  inv1   g041(.a(x19), .O(new_n88_));
  nor2   g042(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g043(.a(x19), .b(x17), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  inv1   g045(.a(x16), .O(new_n92_));
  nand2  g046(.a(x18), .b(x00), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n47_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n91_), .c(new_n86_), .O(z09));
  nand2  g050(.a(x18), .b(x01), .O(new_n97_));
  oai21  g051(.a(x18), .b(new_n51_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n92_), .O(new_n99_));
  nor2   g053(.a(new_n90_), .b(new_n56_), .O(new_n100_));
  nor3   g054(.a(x20), .b(x19), .c(x17), .O(new_n101_));
  oai21  g055(.a(new_n101_), .b(new_n100_), .c(x16), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n99_), .c(new_n86_), .O(z10));
  nand2  g057(.a(x18), .b(x02), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n60_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n92_), .O(new_n106_));
  inv1   g060(.a(x21), .O(new_n107_));
  nand4  g061(.a(new_n107_), .b(new_n88_), .c(new_n87_), .d(x16), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x25), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n56_), .O(new_n110_));
  nand3  g064(.a(new_n90_), .b(x25), .c(new_n56_), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(x21), .c(x16), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n110_), .c(new_n106_), .O(z11));
  nand2  g067(.a(x18), .b(x03), .O(new_n114_));
  oai21  g068(.a(x18), .b(new_n63_), .c(new_n114_), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n86_), .c(new_n92_), .O(new_n116_));
  nand3  g070(.a(new_n107_), .b(new_n88_), .c(new_n87_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x25), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n56_), .O(new_n119_));
  nand3  g073(.a(x25), .b(new_n65_), .c(new_n107_), .O(new_n120_));
  inv1   g074(.a(new_n120_), .O(new_n121_));
  aoi22  g075(.a(new_n121_), .b(new_n101_), .c(new_n119_), .d(x22), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n92_), .c(new_n116_), .O(z12));
  nand2  g077(.a(x18), .b(x04), .O(new_n124_));
  oai21  g078(.a(x18), .b(new_n68_), .c(new_n124_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n92_), .O(new_n126_));
  nand2  g080(.a(new_n90_), .b(x16), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n107_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n127_), .c(x25), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n56_), .O(new_n131_));
  nand2  g085(.a(new_n121_), .b(new_n101_), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(x23), .c(x16), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n131_), .c(new_n126_), .O(z13));
  nand2  g088(.a(x18), .b(x05), .O(new_n135_));
  oai21  g089(.a(x18), .b(new_n73_), .c(new_n135_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n86_), .c(new_n92_), .O(new_n137_));
  inv1   g091(.a(x24), .O(new_n138_));
  nand3  g092(.a(new_n128_), .b(new_n90_), .c(new_n107_), .O(new_n139_));
  aoi21  g093(.a(new_n139_), .b(x25), .c(x20), .O(new_n140_));
  nor2   g094(.a(x21), .b(x20), .O(new_n141_));
  nor2   g095(.a(new_n52_), .b(x24), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n141_), .c(new_n128_), .d(new_n90_), .O(new_n143_));
  oai21  g097(.a(new_n140_), .b(new_n138_), .c(new_n143_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x16), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n137_), .O(z14));
  nand2  g100(.a(x18), .b(x06), .O(new_n147_));
  oai21  g101(.a(x18), .b(new_n76_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n92_), .O(new_n149_));
  nand3  g103(.a(new_n138_), .b(new_n70_), .c(new_n65_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n117_), .c(new_n56_), .O(new_n151_));
  oai21  g105(.a(new_n52_), .b(new_n56_), .c(new_n151_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(x16), .O(new_n153_));
  nand3  g107(.a(new_n153_), .b(new_n149_), .c(new_n86_), .O(z15));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n82_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n92_), .O(new_n157_));
  nand2  g111(.a(x26), .b(x16), .O(new_n158_));
  aoi21  g112(.a(new_n158_), .b(new_n157_), .c(new_n48_), .O(z16));
  nand2  g113(.a(z08), .b(x17), .O(new_n160_));
  nor2   g114(.a(new_n160_), .b(new_n92_), .O(z17));
endmodule


