// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x23), .b(x18), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  aoi21  g005(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n48_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g010(.a(x11), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x22), .O(new_n59_));
  inv1   g013(.a(x27), .O(new_n60_));
  nand2  g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n58_), .c(new_n48_), .O(z03));
  inv1   g016(.a(x18), .O(new_n63_));
  inv1   g017(.a(x23), .O(new_n64_));
  oai21  g018(.a(new_n60_), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  oai21  g019(.a(new_n60_), .b(x12), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n60_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n48_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x25), .O(new_n74_));
  nand2  g028(.a(new_n60_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n48_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n48_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x26), .c(new_n78_), .O(z07));
  inv1   g033(.a(new_n48_), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n60_), .O(z08));
  inv1   g035(.a(x16), .O(new_n82_));
  nand2  g036(.a(x18), .b(x00), .O(new_n83_));
  nand2  g037(.a(x23), .b(new_n63_), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n47_), .c(new_n83_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand2  g040(.a(x19), .b(x17), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  inv1   g042(.a(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n80_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n86_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(new_n84_), .b(new_n51_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n82_), .O(new_n95_));
  inv1   g049(.a(x20), .O(new_n96_));
  inv1   g050(.a(x17), .O(new_n97_));
  inv1   g051(.a(x19), .O(new_n98_));
  nand3  g052(.a(new_n96_), .b(new_n98_), .c(new_n97_), .O(new_n99_));
  oai21  g053(.a(new_n88_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n80_), .c(x16), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n95_), .O(z10));
  nand2  g056(.a(x18), .b(x02), .O(new_n103_));
  oai21  g057(.a(x18), .b(new_n54_), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n82_), .O(new_n105_));
  inv1   g059(.a(x21), .O(new_n106_));
  nor3   g060(.a(x20), .b(x19), .c(x17), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n88_), .O(new_n109_));
  oai21  g063(.a(new_n107_), .b(new_n106_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n105_), .c(new_n80_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n57_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n82_), .O(new_n115_));
  aoi21  g069(.a(new_n108_), .b(new_n88_), .c(new_n59_), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n88_), .c(new_n96_), .O(new_n118_));
  inv1   g072(.a(new_n118_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n115_), .c(new_n80_), .O(z12));
  inv1   g075(.a(x12), .O(new_n122_));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n82_), .O(new_n125_));
  nand2  g079(.a(new_n117_), .b(new_n96_), .O(new_n126_));
  nand2  g080(.a(new_n88_), .b(x16), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n126_), .c(x18), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n64_), .O(new_n129_));
  nand3  g083(.a(new_n118_), .b(x23), .c(x16), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n129_), .c(new_n125_), .O(z13));
  nand2  g085(.a(x18), .b(x05), .O(new_n132_));
  oai21  g086(.a(new_n84_), .b(new_n67_), .c(new_n132_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n82_), .O(new_n134_));
  nand2  g088(.a(new_n118_), .b(x24), .O(new_n135_));
  nor2   g089(.a(x24), .b(x23), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n117_), .c(new_n107_), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(new_n135_), .c(new_n63_), .O(new_n138_));
  nor2   g092(.a(new_n69_), .b(new_n64_), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n138_), .c(x16), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n134_), .O(z14));
  nand2  g095(.a(x18), .b(x06), .O(new_n142_));
  oai21  g096(.a(new_n84_), .b(new_n72_), .c(new_n142_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n82_), .O(new_n144_));
  nand3  g098(.a(new_n69_), .b(new_n59_), .c(new_n106_), .O(new_n145_));
  oai21  g099(.a(new_n145_), .b(new_n99_), .c(x25), .O(new_n146_));
  nor2   g100(.a(x23), .b(x22), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand4  g102(.a(new_n148_), .b(new_n147_), .c(new_n108_), .d(new_n88_), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n146_), .c(new_n63_), .O(new_n150_));
  nor2   g104(.a(new_n74_), .b(new_n64_), .O(new_n151_));
  oai21  g105(.a(new_n151_), .b(new_n150_), .c(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n144_), .O(z15));
  nand2  g107(.a(x18), .b(x07), .O(new_n154_));
  oai21  g108(.a(x18), .b(new_n77_), .c(new_n154_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n82_), .O(new_n156_));
  inv1   g110(.a(x26), .O(new_n157_));
  oai21  g111(.a(new_n157_), .b(new_n82_), .c(x23), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n63_), .O(new_n159_));
  nand2  g113(.a(new_n149_), .b(x26), .O(new_n160_));
  nand4  g114(.a(new_n147_), .b(new_n157_), .c(new_n74_), .d(new_n69_), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(new_n109_), .c(new_n160_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x16), .O(new_n163_));
  nand3  g117(.a(new_n163_), .b(new_n159_), .c(new_n156_), .O(z16));
  nand3  g118(.a(new_n108_), .b(x19), .c(new_n97_), .O(new_n165_));
  oai22  g119(.a(new_n165_), .b(new_n161_), .c(new_n60_), .d(new_n97_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(x16), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n80_), .O(z17));
endmodule


