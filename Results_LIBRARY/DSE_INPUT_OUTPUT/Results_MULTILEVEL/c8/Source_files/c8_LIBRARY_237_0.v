// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x08), .O(new_n47_));
  nor2   g01(.a(x18), .b(x16), .O(new_n48_));
  aoi21  g02(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g03(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g04(.a(x09), .O(new_n51_));
  aoi21  g05(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g06(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g07(.a(x10), .O(new_n54_));
  aoi21  g08(.a(x27), .b(new_n54_), .c(new_n48_), .O(new_n55_));
  oai21  g09(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g10(.a(x11), .O(new_n57_));
  aoi21  g11(.a(x27), .b(new_n57_), .c(new_n48_), .O(new_n58_));
  oai21  g12(.a(x27), .b(x22), .c(new_n58_), .O(z03));
  inv1   g13(.a(x12), .O(new_n60_));
  aoi21  g14(.a(x27), .b(new_n60_), .c(new_n48_), .O(new_n61_));
  oai21  g15(.a(x27), .b(x23), .c(new_n61_), .O(z04));
  inv1   g16(.a(x13), .O(new_n63_));
  nand2  g17(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g18(.a(x24), .O(new_n65_));
  inv1   g19(.a(x27), .O(new_n66_));
  nand2  g20(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  aoi21  g21(.a(new_n67_), .b(new_n64_), .c(new_n48_), .O(z05));
  inv1   g22(.a(x14), .O(new_n69_));
  aoi21  g23(.a(x27), .b(new_n69_), .c(new_n48_), .O(new_n70_));
  oai21  g24(.a(x27), .b(x25), .c(new_n70_), .O(z06));
  inv1   g25(.a(x15), .O(new_n72_));
  aoi21  g26(.a(x27), .b(new_n72_), .c(new_n48_), .O(new_n73_));
  oai21  g27(.a(x27), .b(x26), .c(new_n73_), .O(z07));
  inv1   g28(.a(new_n48_), .O(new_n75_));
  nand2  g29(.a(new_n75_), .b(new_n66_), .O(z08));
  inv1   g30(.a(x17), .O(new_n77_));
  inv1   g31(.a(x19), .O(new_n78_));
  nor2   g32(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g33(.a(x19), .b(x17), .O(new_n80_));
  oai21  g34(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  inv1   g35(.a(x16), .O(new_n82_));
  inv1   g36(.a(x18), .O(new_n83_));
  oai21  g37(.a(new_n83_), .b(x00), .c(new_n82_), .O(new_n84_));
  nand2  g38(.a(new_n84_), .b(new_n81_), .O(z09));
  inv1   g39(.a(x20), .O(new_n86_));
  nor2   g40(.a(new_n80_), .b(new_n86_), .O(new_n87_));
  nor3   g41(.a(x20), .b(x19), .c(x17), .O(new_n88_));
  oai21  g42(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  nor2   g43(.a(new_n83_), .b(x16), .O(new_n90_));
  nand2  g44(.a(new_n90_), .b(x01), .O(new_n91_));
  nand2  g45(.a(new_n91_), .b(new_n89_), .O(z10));
  oai21  g46(.a(new_n83_), .b(x02), .c(new_n82_), .O(new_n93_));
  nor2   g47(.a(x20), .b(x19), .O(new_n94_));
  nand2  g48(.a(new_n94_), .b(new_n77_), .O(new_n95_));
  inv1   g49(.a(x21), .O(new_n96_));
  nand4  g50(.a(new_n96_), .b(new_n86_), .c(new_n78_), .d(new_n77_), .O(new_n97_));
  inv1   g51(.a(new_n97_), .O(new_n98_));
  aoi21  g52(.a(new_n95_), .b(x21), .c(new_n98_), .O(new_n99_));
  oai21  g53(.a(new_n99_), .b(new_n82_), .c(new_n93_), .O(z11));
  nand2  g54(.a(new_n97_), .b(x22), .O(new_n101_));
  nor2   g55(.a(x22), .b(x21), .O(new_n102_));
  nand3  g56(.a(new_n102_), .b(new_n80_), .c(new_n86_), .O(new_n103_));
  and2   g57(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand2  g58(.a(new_n90_), .b(x03), .O(new_n105_));
  oai21  g59(.a(new_n104_), .b(new_n82_), .c(new_n105_), .O(z12));
  nor3   g60(.a(x23), .b(x22), .c(x21), .O(new_n107_));
  aoi22  g61(.a(new_n107_), .b(new_n88_), .c(new_n103_), .d(x23), .O(new_n108_));
  nand2  g62(.a(new_n90_), .b(x04), .O(new_n109_));
  oai21  g63(.a(new_n108_), .b(new_n82_), .c(new_n109_), .O(z13));
  aoi21  g64(.a(new_n107_), .b(new_n88_), .c(new_n65_), .O(new_n111_));
  nor2   g65(.a(x24), .b(x23), .O(new_n112_));
  nand4  g66(.a(new_n112_), .b(new_n102_), .c(new_n94_), .d(new_n77_), .O(new_n113_));
  inv1   g67(.a(new_n113_), .O(new_n114_));
  oai21  g68(.a(new_n114_), .b(new_n111_), .c(x16), .O(new_n115_));
  nand2  g69(.a(new_n90_), .b(x05), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n115_), .O(z14));
  oai21  g71(.a(new_n83_), .b(x06), .c(new_n82_), .O(new_n118_));
  inv1   g72(.a(x22), .O(new_n119_));
  inv1   g73(.a(x23), .O(new_n120_));
  inv1   g74(.a(x25), .O(new_n121_));
  nand4  g75(.a(new_n121_), .b(new_n65_), .c(new_n120_), .d(new_n119_), .O(new_n122_));
  inv1   g76(.a(new_n122_), .O(new_n123_));
  aoi22  g77(.a(new_n123_), .b(new_n98_), .c(new_n113_), .d(x25), .O(new_n124_));
  oai21  g78(.a(new_n124_), .b(new_n82_), .c(new_n118_), .O(z15));
  oai21  g79(.a(new_n83_), .b(x07), .c(new_n82_), .O(new_n126_));
  nor2   g80(.a(x21), .b(x20), .O(new_n127_));
  nor2   g81(.a(x23), .b(x22), .O(new_n128_));
  nor2   g82(.a(x25), .b(x24), .O(new_n129_));
  nand4  g83(.a(new_n129_), .b(new_n128_), .c(new_n127_), .d(new_n80_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(x26), .O(new_n131_));
  nor3   g85(.a(x26), .b(x25), .c(x24), .O(new_n132_));
  nand4  g86(.a(new_n132_), .b(new_n128_), .c(new_n127_), .d(new_n80_), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g88(.a(new_n134_), .b(x16), .O(new_n135_));
  nand2  g89(.a(new_n135_), .b(new_n126_), .O(z16));
  nor2   g90(.a(x26), .b(x25), .O(new_n137_));
  nand3  g91(.a(new_n137_), .b(new_n128_), .c(new_n65_), .O(new_n138_));
  nand3  g92(.a(new_n127_), .b(x19), .c(new_n77_), .O(new_n139_));
  oai22  g93(.a(new_n139_), .b(new_n138_), .c(new_n66_), .d(new_n77_), .O(new_n140_));
  nand2  g94(.a(new_n140_), .b(x16), .O(new_n141_));
  nand2  g95(.a(new_n141_), .b(new_n75_), .O(z17));
endmodule


