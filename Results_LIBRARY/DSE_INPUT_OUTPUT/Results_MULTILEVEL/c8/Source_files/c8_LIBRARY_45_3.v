// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x18), .O(new_n47_));
  inv1   g001(.a(x21), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x09), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x20), .O(new_n58_));
  nand2  g012(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n49_), .O(z01));
  aoi21  g014(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n61_));
  oai22  g015(.a(new_n61_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  aoi21  g017(.a(x27), .b(new_n63_), .c(new_n49_), .O(new_n64_));
  oai21  g018(.a(x27), .b(x22), .c(new_n64_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n49_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n49_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x24), .c(new_n70_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n49_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n49_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  inv1   g031(.a(new_n49_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n53_), .O(z08));
  inv1   g033(.a(x17), .O(new_n80_));
  nor2   g034(.a(new_n52_), .b(new_n80_), .O(new_n81_));
  nor2   g035(.a(x19), .b(x17), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n81_), .c(x16), .O(new_n83_));
  inv1   g037(.a(x16), .O(new_n84_));
  nand2  g038(.a(x18), .b(x00), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n50_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n83_), .c(new_n78_), .O(z09));
  nand2  g042(.a(x18), .b(x01), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n56_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n84_), .O(new_n91_));
  nor2   g045(.a(new_n82_), .b(new_n58_), .O(new_n92_));
  nor3   g046(.a(x20), .b(x19), .c(x17), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n92_), .c(x16), .O(new_n94_));
  nand3  g048(.a(new_n94_), .b(new_n91_), .c(new_n78_), .O(z10));
  inv1   g049(.a(x10), .O(new_n96_));
  nand2  g050(.a(x18), .b(x02), .O(new_n97_));
  oai21  g051(.a(x18), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n84_), .O(new_n99_));
  nand3  g053(.a(new_n82_), .b(new_n48_), .c(new_n58_), .O(new_n100_));
  oai21  g054(.a(new_n93_), .b(new_n48_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x16), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n99_), .c(new_n78_), .O(z11));
  nand2  g057(.a(x18), .b(x03), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n63_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n84_), .O(new_n106_));
  inv1   g060(.a(x22), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n84_), .c(new_n47_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x21), .O(new_n109_));
  nand4  g063(.a(new_n82_), .b(new_n107_), .c(new_n48_), .d(new_n58_), .O(new_n110_));
  oai21  g064(.a(new_n93_), .b(new_n107_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x16), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n109_), .c(new_n106_), .O(z12));
  inv1   g067(.a(x04), .O(new_n114_));
  nand2  g068(.a(new_n48_), .b(x18), .O(new_n115_));
  oai22  g069(.a(new_n115_), .b(new_n114_), .c(x18), .d(new_n66_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n84_), .O(new_n117_));
  nand3  g071(.a(new_n82_), .b(new_n107_), .c(new_n58_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x23), .O(new_n119_));
  nor2   g073(.a(x23), .b(x22), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n82_), .c(new_n58_), .O(new_n121_));
  aoi21  g075(.a(new_n121_), .b(new_n119_), .c(x21), .O(new_n122_));
  inv1   g076(.a(x23), .O(new_n123_));
  nor3   g077(.a(new_n123_), .b(new_n48_), .c(x18), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n122_), .c(x16), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n117_), .O(z13));
  inv1   g080(.a(x05), .O(new_n127_));
  oai22  g081(.a(new_n115_), .b(new_n127_), .c(x18), .d(new_n69_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n84_), .O(new_n129_));
  nand2  g083(.a(new_n121_), .b(x24), .O(new_n130_));
  nor3   g084(.a(x24), .b(x23), .c(x22), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n93_), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(new_n130_), .c(x21), .O(new_n133_));
  inv1   g087(.a(x24), .O(new_n134_));
  nor3   g088(.a(new_n134_), .b(new_n48_), .c(x18), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n129_), .O(z14));
  inv1   g091(.a(x06), .O(new_n138_));
  oai22  g092(.a(new_n115_), .b(new_n138_), .c(x18), .d(new_n72_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n84_), .O(new_n140_));
  nand3  g094(.a(new_n58_), .b(new_n52_), .c(new_n80_), .O(new_n141_));
  nand3  g095(.a(new_n134_), .b(new_n123_), .c(new_n107_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n141_), .c(x25), .O(new_n143_));
  nor2   g097(.a(x25), .b(x24), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n120_), .c(new_n93_), .O(new_n145_));
  aoi21  g099(.a(new_n145_), .b(new_n143_), .c(x21), .O(new_n146_));
  nand2  g100(.a(x25), .b(x21), .O(new_n147_));
  nor2   g101(.a(new_n147_), .b(x18), .O(new_n148_));
  oai21  g102(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n140_), .O(z15));
  nand2  g104(.a(x18), .b(x07), .O(new_n151_));
  oai21  g105(.a(x18), .b(new_n75_), .c(new_n151_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n84_), .O(new_n153_));
  nand2  g107(.a(x26), .b(x16), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n47_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x21), .O(new_n156_));
  nand2  g110(.a(new_n145_), .b(x26), .O(new_n157_));
  inv1   g111(.a(new_n100_), .O(new_n158_));
  nor3   g112(.a(x26), .b(x25), .c(x24), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(new_n120_), .c(new_n158_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n157_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x16), .O(new_n162_));
  nand3  g116(.a(new_n162_), .b(new_n156_), .c(new_n153_), .O(z16));
  nand2  g117(.a(new_n159_), .b(new_n120_), .O(new_n164_));
  nand4  g118(.a(new_n48_), .b(new_n58_), .c(x19), .d(new_n80_), .O(new_n165_));
  oai22  g119(.a(new_n165_), .b(new_n164_), .c(new_n53_), .d(new_n80_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(x16), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n78_), .O(z17));
endmodule


