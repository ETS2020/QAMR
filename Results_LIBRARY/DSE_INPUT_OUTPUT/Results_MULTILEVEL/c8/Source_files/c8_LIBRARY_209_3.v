// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n53_, new_n54_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_;
  inv1   g000(.a(x18), .O(new_n47_));
  nand2  g001(.a(x27), .b(new_n47_), .O(new_n48_));
  oai22  g002(.a(new_n48_), .b(x08), .c(x27), .d(x19), .O(z00));
  nand2  g003(.a(new_n47_), .b(x09), .O(new_n50_));
  nand2  g004(.a(new_n50_), .b(x27), .O(new_n51_));
  oai21  g005(.a(x27), .b(x20), .c(new_n51_), .O(z01));
  nand2  g006(.a(new_n47_), .b(x10), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(x27), .O(new_n54_));
  oai21  g008(.a(x27), .b(x21), .c(new_n54_), .O(z02));
  oai22  g009(.a(new_n48_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g010(.a(new_n48_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g011(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  inv1   g012(.a(x14), .O(new_n59_));
  oai21  g013(.a(x18), .b(new_n59_), .c(x27), .O(new_n60_));
  oai21  g014(.a(x27), .b(x25), .c(new_n60_), .O(z06));
  inv1   g015(.a(x15), .O(new_n62_));
  oai21  g016(.a(x18), .b(new_n62_), .c(x27), .O(new_n63_));
  oai21  g017(.a(x27), .b(x26), .c(new_n63_), .O(z07));
  inv1   g018(.a(new_n48_), .O(z08));
  inv1   g019(.a(x16), .O(new_n66_));
  inv1   g020(.a(x00), .O(new_n67_));
  nand2  g021(.a(new_n47_), .b(x08), .O(new_n68_));
  inv1   g022(.a(x27), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(x18), .O(new_n70_));
  oai21  g024(.a(new_n70_), .b(new_n67_), .c(new_n68_), .O(new_n71_));
  nand2  g025(.a(new_n71_), .b(new_n66_), .O(new_n72_));
  nand2  g026(.a(x27), .b(x18), .O(new_n73_));
  nand2  g027(.a(x19), .b(x17), .O(new_n74_));
  nor2   g028(.a(x19), .b(x17), .O(new_n75_));
  inv1   g029(.a(new_n75_), .O(new_n76_));
  nand2  g030(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand3  g031(.a(new_n77_), .b(new_n73_), .c(x16), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n72_), .O(z09));
  nand2  g033(.a(x18), .b(x01), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n50_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n66_), .O(new_n82_));
  inv1   g036(.a(x20), .O(new_n83_));
  nor2   g037(.a(new_n75_), .b(new_n83_), .O(new_n84_));
  nor3   g038(.a(x20), .b(x19), .c(x17), .O(new_n85_));
  oai21  g039(.a(new_n85_), .b(new_n84_), .c(x16), .O(new_n86_));
  nand3  g040(.a(new_n86_), .b(new_n82_), .c(new_n73_), .O(z10));
  nand2  g041(.a(x18), .b(x02), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n53_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n66_), .O(new_n90_));
  inv1   g044(.a(x21), .O(new_n91_));
  nor2   g045(.a(x21), .b(x20), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n75_), .O(new_n93_));
  oai21  g047(.a(new_n85_), .b(new_n91_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(x16), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n90_), .c(new_n73_), .O(z11));
  inv1   g050(.a(x11), .O(new_n97_));
  nand2  g051(.a(x18), .b(x03), .O(new_n98_));
  oai21  g052(.a(x18), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n66_), .O(new_n100_));
  inv1   g054(.a(x22), .O(new_n101_));
  aoi21  g055(.a(new_n92_), .b(new_n75_), .c(new_n101_), .O(new_n102_));
  nor2   g056(.a(x22), .b(x21), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n75_), .c(new_n83_), .O(new_n104_));
  inv1   g058(.a(new_n104_), .O(new_n105_));
  oai21  g059(.a(new_n105_), .b(new_n102_), .c(x16), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n100_), .c(new_n73_), .O(z12));
  inv1   g061(.a(x12), .O(new_n108_));
  nand2  g062(.a(x18), .b(x04), .O(new_n109_));
  oai21  g063(.a(x18), .b(new_n108_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n66_), .O(new_n111_));
  nand2  g065(.a(new_n104_), .b(x23), .O(new_n112_));
  nor3   g066(.a(x23), .b(x22), .c(x21), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n85_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(x16), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n111_), .c(new_n73_), .O(z13));
  inv1   g071(.a(x13), .O(new_n118_));
  nand2  g072(.a(x18), .b(x05), .O(new_n119_));
  oai21  g073(.a(x18), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n66_), .O(new_n121_));
  inv1   g075(.a(x24), .O(new_n122_));
  aoi21  g076(.a(new_n113_), .b(new_n85_), .c(new_n122_), .O(new_n123_));
  inv1   g077(.a(x17), .O(new_n124_));
  nor2   g078(.a(x20), .b(x19), .O(new_n125_));
  nor2   g079(.a(x24), .b(x23), .O(new_n126_));
  nand4  g080(.a(new_n126_), .b(new_n103_), .c(new_n125_), .d(new_n124_), .O(new_n127_));
  inv1   g081(.a(new_n127_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n123_), .c(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n121_), .c(new_n73_), .O(z14));
  inv1   g084(.a(x06), .O(new_n131_));
  oai22  g085(.a(new_n70_), .b(new_n131_), .c(x18), .d(new_n59_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n66_), .O(new_n133_));
  nand2  g087(.a(new_n127_), .b(x25), .O(new_n134_));
  nor2   g088(.a(x23), .b(x22), .O(new_n135_));
  nor2   g089(.a(x25), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n135_), .c(new_n92_), .d(new_n75_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n73_), .c(x16), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n133_), .O(z15));
  inv1   g094(.a(x07), .O(new_n141_));
  oai22  g095(.a(new_n70_), .b(new_n141_), .c(x18), .d(new_n62_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n66_), .O(new_n143_));
  nand2  g097(.a(new_n137_), .b(x26), .O(new_n144_));
  nor3   g098(.a(x26), .b(x25), .c(x24), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n135_), .c(new_n92_), .d(new_n75_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n73_), .c(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n143_), .O(z16));
  nor2   g103(.a(x26), .b(x25), .O(new_n150_));
  nand3  g104(.a(new_n150_), .b(new_n135_), .c(new_n122_), .O(new_n151_));
  nand3  g105(.a(new_n92_), .b(x19), .c(new_n124_), .O(new_n152_));
  oai22  g106(.a(new_n152_), .b(new_n151_), .c(new_n69_), .d(new_n124_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(x16), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n73_), .O(z17));
endmodule


