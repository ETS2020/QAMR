// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x17), .O(new_n48_));
  inv1   g002(.a(x18), .O(new_n49_));
  nor2   g003(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g004(.a(x27), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  oai21  g005(.a(x27), .b(x19), .c(new_n51_), .O(z00));
  inv1   g006(.a(x09), .O(new_n53_));
  nand2  g007(.a(x27), .b(new_n53_), .O(new_n54_));
  inv1   g008(.a(x20), .O(new_n55_));
  inv1   g009(.a(x27), .O(new_n56_));
  nand2  g010(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g011(.a(new_n57_), .b(new_n54_), .c(new_n50_), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n59_), .c(new_n50_), .O(new_n60_));
  oai21  g014(.a(x27), .b(x21), .c(new_n60_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n50_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x23), .O(new_n67_));
  nand2  g021(.a(new_n56_), .b(new_n67_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n66_), .c(new_n50_), .O(z04));
  inv1   g023(.a(x13), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x24), .O(new_n72_));
  nand2  g026(.a(new_n56_), .b(new_n72_), .O(new_n73_));
  aoi21  g027(.a(new_n73_), .b(new_n71_), .c(new_n50_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  nand2  g031(.a(new_n56_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n50_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  aoi21  g034(.a(x27), .b(new_n80_), .c(new_n50_), .O(new_n81_));
  oai21  g035(.a(x27), .b(x26), .c(new_n81_), .O(z07));
  nor2   g036(.a(new_n50_), .b(new_n56_), .O(z08));
  nand3  g037(.a(x19), .b(new_n49_), .c(x17), .O(new_n84_));
  oai21  g038(.a(x19), .b(x17), .c(new_n84_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(x16), .O(new_n86_));
  inv1   g040(.a(x16), .O(new_n87_));
  inv1   g041(.a(x00), .O(new_n88_));
  nand2  g042(.a(x18), .b(new_n48_), .O(new_n89_));
  oai22  g043(.a(new_n89_), .b(new_n88_), .c(x18), .d(new_n47_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n86_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n53_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n87_), .O(new_n95_));
  oai21  g049(.a(new_n55_), .b(new_n87_), .c(new_n49_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(x17), .O(new_n97_));
  inv1   g051(.a(x19), .O(new_n98_));
  nor2   g052(.a(x20), .b(x19), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n48_), .O(new_n100_));
  oai21  g054(.a(new_n55_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x16), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n97_), .c(new_n95_), .O(z10));
  nand2  g057(.a(x18), .b(x02), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n59_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n87_), .O(new_n106_));
  inv1   g060(.a(x21), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n87_), .c(new_n49_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x17), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n98_), .c(new_n48_), .O(new_n111_));
  oai21  g065(.a(new_n99_), .b(new_n107_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(x16), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n109_), .c(new_n106_), .O(z11));
  inv1   g068(.a(x03), .O(new_n115_));
  oai22  g069(.a(new_n89_), .b(new_n115_), .c(x18), .d(new_n62_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n87_), .O(new_n117_));
  inv1   g071(.a(x22), .O(new_n118_));
  nor3   g072(.a(x21), .b(x20), .c(x19), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n99_), .O(new_n121_));
  oai21  g075(.a(new_n119_), .b(new_n118_), .c(new_n121_), .O(new_n122_));
  nand3  g076(.a(x22), .b(new_n49_), .c(x17), .O(new_n123_));
  inv1   g077(.a(new_n123_), .O(new_n124_));
  aoi21  g078(.a(new_n122_), .b(new_n48_), .c(new_n124_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n87_), .c(new_n117_), .O(z12));
  inv1   g080(.a(x04), .O(new_n127_));
  oai22  g081(.a(new_n89_), .b(new_n127_), .c(x18), .d(new_n65_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n87_), .O(new_n129_));
  nand2  g083(.a(new_n121_), .b(x23), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n99_), .c(new_n107_), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(new_n130_), .c(x17), .O(new_n133_));
  nand3  g087(.a(x23), .b(new_n49_), .c(x17), .O(new_n134_));
  inv1   g088(.a(new_n134_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n129_), .O(z13));
  inv1   g091(.a(x05), .O(new_n138_));
  oai22  g092(.a(new_n89_), .b(new_n138_), .c(x18), .d(new_n70_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n87_), .O(new_n140_));
  nand2  g094(.a(new_n132_), .b(x24), .O(new_n141_));
  nor3   g095(.a(x24), .b(x23), .c(x22), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n119_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(x17), .O(new_n144_));
  nand3  g098(.a(x24), .b(new_n49_), .c(x17), .O(new_n145_));
  inv1   g099(.a(new_n145_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n140_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(x18), .b(new_n75_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n87_), .O(new_n151_));
  oai21  g105(.a(new_n77_), .b(new_n87_), .c(new_n49_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(x17), .O(new_n153_));
  aoi21  g107(.a(new_n142_), .b(new_n119_), .c(new_n77_), .O(new_n154_));
  nor2   g108(.a(x25), .b(x24), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n131_), .O(new_n156_));
  nor2   g110(.a(new_n156_), .b(new_n111_), .O(new_n157_));
  oai21  g111(.a(new_n157_), .b(new_n154_), .c(x16), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n153_), .c(new_n151_), .O(z15));
  inv1   g113(.a(x07), .O(new_n160_));
  oai22  g114(.a(new_n89_), .b(new_n160_), .c(x18), .d(new_n80_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n87_), .O(new_n162_));
  nand4  g116(.a(new_n155_), .b(new_n131_), .c(new_n110_), .d(new_n98_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x26), .O(new_n164_));
  nor2   g118(.a(x24), .b(x23), .O(new_n165_));
  nor2   g119(.a(x26), .b(x25), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n165_), .c(new_n120_), .d(new_n99_), .O(new_n167_));
  aoi21  g121(.a(new_n167_), .b(new_n164_), .c(x17), .O(new_n168_));
  nand3  g122(.a(x26), .b(new_n49_), .c(x17), .O(new_n169_));
  inv1   g123(.a(new_n169_), .O(new_n170_));
  oai21  g124(.a(new_n170_), .b(new_n168_), .c(x16), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n162_), .O(z16));
  inv1   g126(.a(new_n50_), .O(new_n173_));
  nand3  g127(.a(new_n110_), .b(x19), .c(new_n48_), .O(new_n174_));
  nand3  g128(.a(new_n166_), .b(new_n131_), .c(new_n72_), .O(new_n175_));
  oai22  g129(.a(new_n175_), .b(new_n174_), .c(new_n56_), .d(new_n48_), .O(new_n176_));
  nand2  g130(.a(new_n176_), .b(x16), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(new_n173_), .O(z17));
endmodule


