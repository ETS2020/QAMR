// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_;
  nor2   g000(.a(x21), .b(x18), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  nand2  g008(.a(x27), .b(new_n54_), .O(new_n55_));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n51_), .b(new_n56_), .O(new_n57_));
  aoi21  g011(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(z01));
  inv1   g012(.a(x18), .O(new_n59_));
  inv1   g013(.a(x21), .O(new_n60_));
  oai21  g014(.a(new_n51_), .b(new_n59_), .c(new_n60_), .O(new_n61_));
  oai21  g015(.a(new_n51_), .b(x10), .c(new_n61_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n51_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n47_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n47_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x23), .c(new_n69_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n71_), .c(new_n47_), .O(new_n72_));
  oai21  g026(.a(x27), .b(x24), .c(new_n72_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x25), .O(new_n76_));
  nand2  g030(.a(new_n51_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n47_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  nand2  g033(.a(x27), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x26), .O(new_n81_));
  nand2  g035(.a(new_n51_), .b(new_n81_), .O(new_n82_));
  aoi21  g036(.a(new_n82_), .b(new_n80_), .c(new_n47_), .O(z07));
  nor2   g037(.a(new_n47_), .b(new_n51_), .O(z08));
  inv1   g038(.a(new_n47_), .O(new_n85_));
  inv1   g039(.a(x17), .O(new_n86_));
  nor2   g040(.a(new_n50_), .b(new_n86_), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  inv1   g043(.a(x16), .O(new_n90_));
  nand2  g044(.a(x18), .b(x00), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n48_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand3  g047(.a(new_n93_), .b(new_n89_), .c(new_n85_), .O(z09));
  nand2  g048(.a(x18), .b(x01), .O(new_n95_));
  nand2  g049(.a(x21), .b(new_n59_), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n54_), .c(new_n95_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n90_), .O(new_n98_));
  nand3  g052(.a(new_n56_), .b(new_n50_), .c(new_n86_), .O(new_n99_));
  oai21  g053(.a(new_n88_), .b(new_n56_), .c(new_n99_), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n85_), .c(x16), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n98_), .O(z10));
  inv1   g056(.a(x10), .O(new_n103_));
  nand2  g057(.a(x18), .b(x02), .O(new_n104_));
  oai21  g058(.a(new_n96_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n90_), .O(new_n106_));
  nor2   g060(.a(new_n59_), .b(x17), .O(new_n107_));
  nor3   g061(.a(x21), .b(x20), .c(x19), .O(new_n108_));
  aoi22  g062(.a(new_n108_), .b(new_n107_), .c(new_n99_), .d(x21), .O(new_n109_));
  oai21  g063(.a(new_n109_), .b(new_n90_), .c(new_n106_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(x18), .b(new_n63_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n90_), .O(new_n113_));
  nand2  g067(.a(new_n86_), .b(x16), .O(new_n114_));
  nand3  g068(.a(new_n65_), .b(new_n56_), .c(new_n50_), .O(new_n115_));
  oai21  g069(.a(new_n115_), .b(new_n114_), .c(x18), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n60_), .O(new_n117_));
  nor2   g071(.a(x21), .b(x20), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n88_), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(x22), .c(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n117_), .c(new_n113_), .O(z12));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(new_n96_), .b(new_n68_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n90_), .O(new_n124_));
  nand3  g078(.a(new_n88_), .b(new_n65_), .c(new_n56_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x23), .O(new_n126_));
  nor3   g080(.a(x20), .b(x19), .c(x17), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n127_), .c(new_n60_), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n126_), .c(new_n59_), .O(new_n130_));
  inv1   g084(.a(x23), .O(new_n131_));
  nor2   g085(.a(new_n131_), .b(new_n60_), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(new_n130_), .c(x16), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n124_), .O(z13));
  nand2  g088(.a(x18), .b(x05), .O(new_n135_));
  oai21  g089(.a(new_n96_), .b(new_n71_), .c(new_n135_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n90_), .O(new_n137_));
  nand3  g091(.a(new_n128_), .b(new_n88_), .c(new_n56_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x24), .O(new_n139_));
  nor2   g093(.a(x24), .b(x23), .O(new_n140_));
  nand4  g094(.a(new_n140_), .b(new_n127_), .c(new_n65_), .d(new_n60_), .O(new_n141_));
  aoi21  g095(.a(new_n141_), .b(new_n139_), .c(new_n59_), .O(new_n142_));
  inv1   g096(.a(x24), .O(new_n143_));
  nor2   g097(.a(new_n143_), .b(new_n60_), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n142_), .c(x16), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n137_), .O(z14));
  nand2  g100(.a(x18), .b(x06), .O(new_n147_));
  oai21  g101(.a(new_n96_), .b(new_n74_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n90_), .O(new_n149_));
  nand3  g103(.a(new_n143_), .b(new_n131_), .c(new_n65_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n99_), .c(x25), .O(new_n151_));
  nor2   g105(.a(x25), .b(x24), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n128_), .c(new_n118_), .d(new_n88_), .O(new_n153_));
  aoi21  g107(.a(new_n153_), .b(new_n151_), .c(new_n59_), .O(new_n154_));
  nor2   g108(.a(new_n76_), .b(new_n60_), .O(new_n155_));
  oai21  g109(.a(new_n155_), .b(new_n154_), .c(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n149_), .O(z15));
  nand2  g111(.a(x18), .b(x07), .O(new_n158_));
  oai21  g112(.a(new_n96_), .b(new_n79_), .c(new_n158_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n90_), .O(new_n160_));
  nor2   g114(.a(x20), .b(x19), .O(new_n161_));
  nand4  g115(.a(new_n152_), .b(new_n128_), .c(new_n161_), .d(new_n86_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x26), .O(new_n163_));
  nor3   g117(.a(x26), .b(x25), .c(x24), .O(new_n164_));
  nand4  g118(.a(new_n164_), .b(new_n128_), .c(new_n118_), .d(new_n88_), .O(new_n165_));
  aoi21  g119(.a(new_n165_), .b(new_n163_), .c(new_n59_), .O(new_n166_));
  nor2   g120(.a(new_n81_), .b(new_n60_), .O(new_n167_));
  oai21  g121(.a(new_n167_), .b(new_n166_), .c(x16), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n160_), .O(z16));
  nand2  g123(.a(z08), .b(x17), .O(new_n170_));
  nand2  g124(.a(new_n118_), .b(x19), .O(new_n171_));
  inv1   g125(.a(new_n171_), .O(new_n172_));
  nand4  g126(.a(new_n172_), .b(new_n164_), .c(new_n128_), .d(new_n107_), .O(new_n173_));
  aoi21  g127(.a(new_n173_), .b(new_n170_), .c(new_n90_), .O(z17));
endmodule


