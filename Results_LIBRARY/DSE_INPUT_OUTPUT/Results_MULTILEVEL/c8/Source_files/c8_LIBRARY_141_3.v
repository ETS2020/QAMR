// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_;
  nand2  g000(.a(x21), .b(x18), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x18), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  oai22  g014(.a(new_n60_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n48_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x23), .O(new_n67_));
  nand2  g021(.a(new_n52_), .b(new_n67_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n66_), .c(new_n48_), .O(z04));
  inv1   g023(.a(x13), .O(new_n70_));
  aoi21  g024(.a(x27), .b(new_n70_), .c(new_n48_), .O(new_n71_));
  oai21  g025(.a(x27), .b(x24), .c(new_n71_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  nand2  g027(.a(x27), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x25), .O(new_n75_));
  nand2  g029(.a(new_n52_), .b(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n48_), .O(z06));
  inv1   g031(.a(x15), .O(new_n78_));
  aoi21  g032(.a(x27), .b(new_n78_), .c(new_n48_), .O(new_n79_));
  oai21  g033(.a(x27), .b(x26), .c(new_n79_), .O(z07));
  nor2   g034(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g035(.a(x16), .O(new_n82_));
  inv1   g036(.a(x00), .O(new_n83_));
  nand2  g037(.a(new_n59_), .b(x18), .O(new_n84_));
  oai22  g038(.a(new_n84_), .b(new_n83_), .c(x18), .d(new_n49_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  xnor2a g040(.a(x19), .b(x17), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n47_), .c(x16), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(z09));
  inv1   g043(.a(x01), .O(new_n90_));
  oai22  g044(.a(new_n84_), .b(new_n90_), .c(x18), .d(new_n55_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n82_), .O(new_n92_));
  inv1   g046(.a(x20), .O(new_n93_));
  nor2   g047(.a(x19), .b(x17), .O(new_n94_));
  nor3   g048(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  inv1   g049(.a(new_n95_), .O(new_n96_));
  oai21  g050(.a(new_n94_), .b(new_n93_), .c(new_n96_), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n47_), .c(x16), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n92_), .O(z10));
  inv1   g053(.a(x02), .O(new_n100_));
  nand2  g054(.a(new_n58_), .b(x10), .O(new_n101_));
  oai21  g055(.a(new_n84_), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n82_), .O(new_n103_));
  nand3  g057(.a(new_n96_), .b(x21), .c(new_n58_), .O(new_n104_));
  nand3  g058(.a(new_n94_), .b(new_n59_), .c(new_n93_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x16), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n103_), .O(z11));
  inv1   g062(.a(x03), .O(new_n109_));
  oai22  g063(.a(new_n84_), .b(new_n109_), .c(x18), .d(new_n62_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n82_), .O(new_n111_));
  inv1   g065(.a(x22), .O(new_n112_));
  nor2   g066(.a(x22), .b(x20), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n94_), .O(new_n114_));
  oai21  g068(.a(new_n95_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  nor3   g069(.a(new_n112_), .b(new_n59_), .c(x18), .O(new_n116_));
  aoi21  g070(.a(new_n115_), .b(new_n59_), .c(new_n116_), .O(new_n117_));
  oai21  g071(.a(new_n117_), .b(new_n82_), .c(new_n111_), .O(z12));
  inv1   g072(.a(x04), .O(new_n119_));
  oai22  g073(.a(new_n84_), .b(new_n119_), .c(x18), .d(new_n65_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n82_), .O(new_n121_));
  nand2  g075(.a(new_n114_), .b(x23), .O(new_n122_));
  nor2   g076(.a(x23), .b(x22), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n94_), .c(new_n93_), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n122_), .c(x21), .O(new_n125_));
  nor3   g079(.a(new_n67_), .b(new_n59_), .c(x18), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n125_), .c(x16), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n121_), .O(z13));
  nand2  g082(.a(x18), .b(x05), .O(new_n129_));
  oai21  g083(.a(x18), .b(new_n70_), .c(new_n129_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n82_), .O(new_n131_));
  inv1   g085(.a(x24), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(new_n82_), .c(new_n58_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x21), .O(new_n134_));
  nand2  g088(.a(new_n124_), .b(x24), .O(new_n135_));
  nor2   g089(.a(x24), .b(x23), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n95_), .c(new_n112_), .d(new_n59_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n134_), .c(new_n131_), .O(z14));
  nand2  g094(.a(x18), .b(x06), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n73_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n82_), .O(new_n143_));
  oai21  g097(.a(new_n75_), .b(new_n82_), .c(new_n58_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x21), .O(new_n145_));
  nor3   g099(.a(x24), .b(x23), .c(x22), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n95_), .c(new_n75_), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n123_), .O(new_n149_));
  nor2   g103(.a(new_n149_), .b(new_n105_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n147_), .c(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n145_), .c(new_n143_), .O(z15));
  inv1   g106(.a(x07), .O(new_n153_));
  oai22  g107(.a(new_n84_), .b(new_n153_), .c(x18), .d(new_n78_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n82_), .O(new_n155_));
  inv1   g109(.a(x17), .O(new_n156_));
  nor2   g110(.a(x20), .b(x19), .O(new_n157_));
  nand4  g111(.a(new_n148_), .b(new_n123_), .c(new_n157_), .d(new_n156_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x26), .O(new_n159_));
  nor2   g113(.a(x26), .b(x25), .O(new_n160_));
  nand4  g114(.a(new_n160_), .b(new_n136_), .c(new_n113_), .d(new_n94_), .O(new_n161_));
  aoi21  g115(.a(new_n161_), .b(new_n159_), .c(x21), .O(new_n162_));
  nand2  g116(.a(x26), .b(x21), .O(new_n163_));
  nor2   g117(.a(new_n163_), .b(x18), .O(new_n164_));
  oai21  g118(.a(new_n164_), .b(new_n162_), .c(x16), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(new_n155_), .O(z16));
  nand4  g120(.a(new_n59_), .b(new_n93_), .c(x19), .d(new_n156_), .O(new_n167_));
  nand3  g121(.a(new_n160_), .b(new_n123_), .c(new_n132_), .O(new_n168_));
  oai22  g122(.a(new_n168_), .b(new_n167_), .c(new_n52_), .d(new_n156_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(x16), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(new_n47_), .O(z17));
endmodule


