// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x05), .O(new_n48_));
  nor2   g002(.a(x18), .b(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x21), .O(new_n57_));
  inv1   g011(.a(x27), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n56_), .c(new_n49_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  aoi21  g015(.a(x27), .b(new_n61_), .c(new_n49_), .O(new_n62_));
  oai21  g016(.a(x27), .b(x22), .c(new_n62_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n58_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n49_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x25), .O(new_n74_));
  nand2  g028(.a(new_n58_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n49_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x26), .O(new_n79_));
  nand2  g033(.a(new_n58_), .b(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n78_), .c(new_n49_), .O(z07));
  inv1   g035(.a(new_n49_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n58_), .O(z08));
  inv1   g037(.a(x16), .O(new_n84_));
  nand2  g038(.a(x18), .b(x00), .O(new_n85_));
  inv1   g039(.a(x18), .O(new_n86_));
  nand3  g040(.a(new_n86_), .b(x08), .c(new_n48_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n84_), .O(new_n89_));
  xnor2a g043(.a(x19), .b(x17), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n82_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n89_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  nand3  g047(.a(new_n86_), .b(x09), .c(new_n48_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n84_), .O(new_n96_));
  inv1   g050(.a(x20), .O(new_n97_));
  nor2   g051(.a(x19), .b(x17), .O(new_n98_));
  inv1   g052(.a(x17), .O(new_n99_));
  nor2   g053(.a(x20), .b(x19), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  oai21  g055(.a(new_n98_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n82_), .c(x16), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n96_), .O(z10));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  nand3  g059(.a(new_n86_), .b(x10), .c(new_n48_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n84_), .O(new_n108_));
  nor3   g062(.a(x20), .b(x19), .c(x17), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n98_), .O(new_n111_));
  oai21  g065(.a(new_n109_), .b(new_n57_), .c(new_n111_), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n82_), .c(x16), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n108_), .O(z11));
  nand2  g068(.a(x18), .b(x03), .O(new_n115_));
  nand3  g069(.a(new_n86_), .b(x11), .c(new_n48_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n84_), .O(new_n118_));
  nand2  g072(.a(new_n111_), .b(x22), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n98_), .c(new_n97_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n82_), .c(x16), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n118_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  nand3  g079(.a(new_n86_), .b(x12), .c(new_n48_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n84_), .O(new_n128_));
  nand2  g082(.a(new_n121_), .b(x23), .O(new_n129_));
  nor3   g083(.a(x23), .b(x22), .c(x21), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n109_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n82_), .c(x16), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n128_), .O(z13));
  oai21  g088(.a(new_n86_), .b(new_n84_), .c(x05), .O(new_n135_));
  aoi21  g089(.a(new_n130_), .b(new_n109_), .c(new_n69_), .O(new_n136_));
  nor2   g090(.a(x24), .b(x23), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n120_), .c(new_n100_), .d(new_n99_), .O(new_n138_));
  inv1   g092(.a(new_n138_), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n136_), .c(x16), .O(new_n140_));
  nand3  g094(.a(new_n86_), .b(new_n84_), .c(x13), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n140_), .c(new_n135_), .O(z14));
  nand2  g096(.a(x18), .b(x06), .O(new_n143_));
  nand3  g097(.a(new_n86_), .b(x14), .c(new_n48_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n84_), .O(new_n146_));
  nand2  g100(.a(new_n138_), .b(x25), .O(new_n147_));
  nor2   g101(.a(x23), .b(x22), .O(new_n148_));
  nor2   g102(.a(x25), .b(x24), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n148_), .c(new_n110_), .d(new_n98_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n82_), .c(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n146_), .O(z15));
  nand2  g107(.a(x18), .b(x07), .O(new_n154_));
  nand3  g108(.a(new_n86_), .b(x15), .c(new_n48_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n84_), .O(new_n157_));
  nand2  g111(.a(new_n150_), .b(x26), .O(new_n158_));
  nor3   g112(.a(x26), .b(x25), .c(x24), .O(new_n159_));
  nand4  g113(.a(new_n159_), .b(new_n148_), .c(new_n110_), .d(new_n98_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n82_), .c(x16), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n157_), .O(z16));
  nand4  g117(.a(new_n148_), .b(new_n79_), .c(new_n74_), .d(new_n69_), .O(new_n164_));
  nand3  g118(.a(new_n110_), .b(x19), .c(new_n99_), .O(new_n165_));
  oai22  g119(.a(new_n165_), .b(new_n164_), .c(new_n58_), .d(new_n99_), .O(new_n166_));
  nand3  g120(.a(new_n166_), .b(new_n82_), .c(x16), .O(new_n167_));
  inv1   g121(.a(new_n167_), .O(z17));
endmodule


