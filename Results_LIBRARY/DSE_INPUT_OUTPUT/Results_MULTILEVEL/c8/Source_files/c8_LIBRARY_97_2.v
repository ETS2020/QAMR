// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  oai21  g002(.a(x25), .b(new_n48_), .c(new_n47_), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g004(.a(x25), .O(new_n51_));
  nand2  g005(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai22  g006(.a(new_n52_), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  oai22  g007(.a(new_n52_), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  oai22  g008(.a(new_n52_), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  inv1   g009(.a(x23), .O(new_n56_));
  oai21  g010(.a(x25), .b(new_n56_), .c(new_n47_), .O(new_n57_));
  oai21  g011(.a(new_n47_), .b(x12), .c(new_n57_), .O(z04));
  inv1   g012(.a(x24), .O(new_n59_));
  oai21  g013(.a(x25), .b(new_n59_), .c(new_n47_), .O(new_n60_));
  oai21  g014(.a(new_n47_), .b(x13), .c(new_n60_), .O(z05));
  oai21  g015(.a(new_n47_), .b(x14), .c(new_n52_), .O(z06));
  inv1   g016(.a(x26), .O(new_n63_));
  oai21  g017(.a(new_n63_), .b(x25), .c(new_n47_), .O(new_n64_));
  oai21  g018(.a(new_n47_), .b(x15), .c(new_n64_), .O(z07));
  inv1   g019(.a(x17), .O(new_n66_));
  nor2   g020(.a(new_n48_), .b(new_n66_), .O(new_n67_));
  nor2   g021(.a(x19), .b(x17), .O(new_n68_));
  oai21  g022(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  inv1   g023(.a(x16), .O(new_n70_));
  inv1   g024(.a(x08), .O(new_n71_));
  nand2  g025(.a(x18), .b(x00), .O(new_n72_));
  oai21  g026(.a(x18), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nand2  g028(.a(new_n47_), .b(x25), .O(new_n75_));
  nand3  g029(.a(new_n75_), .b(new_n74_), .c(new_n69_), .O(z09));
  inv1   g030(.a(new_n75_), .O(new_n77_));
  inv1   g031(.a(x09), .O(new_n78_));
  nand2  g032(.a(x18), .b(x01), .O(new_n79_));
  oai21  g033(.a(x18), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n70_), .O(new_n81_));
  inv1   g035(.a(x20), .O(new_n82_));
  nor2   g036(.a(new_n68_), .b(new_n82_), .O(new_n83_));
  nor3   g037(.a(x20), .b(x19), .c(x17), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  aoi21  g039(.a(new_n85_), .b(new_n81_), .c(new_n77_), .O(z10));
  inv1   g040(.a(x10), .O(new_n87_));
  nor2   g041(.a(x18), .b(new_n87_), .O(new_n88_));
  aoi21  g042(.a(x18), .b(x02), .c(new_n88_), .O(new_n89_));
  inv1   g043(.a(x21), .O(new_n90_));
  nor2   g044(.a(x21), .b(x20), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n68_), .O(new_n92_));
  oai21  g046(.a(new_n84_), .b(new_n90_), .c(new_n92_), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(x16), .c(new_n77_), .O(new_n94_));
  oai21  g048(.a(new_n89_), .b(x16), .c(new_n94_), .O(z11));
  inv1   g049(.a(x11), .O(new_n96_));
  nand2  g050(.a(x18), .b(x03), .O(new_n97_));
  oai21  g051(.a(x18), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n70_), .O(new_n99_));
  inv1   g053(.a(x22), .O(new_n100_));
  aoi21  g054(.a(new_n91_), .b(new_n68_), .c(new_n100_), .O(new_n101_));
  nor2   g055(.a(x22), .b(x21), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n68_), .c(new_n82_), .O(new_n103_));
  inv1   g057(.a(new_n103_), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n99_), .c(new_n77_), .O(z12));
  inv1   g060(.a(x12), .O(new_n107_));
  nand2  g061(.a(x18), .b(x04), .O(new_n108_));
  oai21  g062(.a(x18), .b(new_n107_), .c(new_n108_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n70_), .O(new_n110_));
  nand2  g064(.a(new_n103_), .b(x23), .O(new_n111_));
  nor3   g065(.a(x23), .b(x22), .c(x21), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n84_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x16), .O(new_n115_));
  aoi21  g069(.a(new_n115_), .b(new_n110_), .c(new_n77_), .O(z13));
  inv1   g070(.a(x13), .O(new_n117_));
  nand2  g071(.a(x18), .b(x05), .O(new_n118_));
  oai21  g072(.a(x18), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n70_), .O(new_n120_));
  aoi21  g074(.a(new_n112_), .b(new_n84_), .c(new_n59_), .O(new_n121_));
  nor2   g075(.a(x20), .b(x19), .O(new_n122_));
  nor2   g076(.a(x24), .b(x23), .O(new_n123_));
  nand4  g077(.a(new_n123_), .b(new_n102_), .c(new_n122_), .d(new_n66_), .O(new_n124_));
  inv1   g078(.a(new_n124_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n121_), .c(x16), .O(new_n126_));
  aoi21  g080(.a(new_n126_), .b(new_n120_), .c(new_n77_), .O(z14));
  inv1   g081(.a(x14), .O(new_n128_));
  nand2  g082(.a(x18), .b(x06), .O(new_n129_));
  oai21  g083(.a(x18), .b(new_n128_), .c(new_n129_), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n75_), .c(new_n70_), .O(new_n131_));
  nand3  g085(.a(new_n124_), .b(x27), .c(x25), .O(new_n132_));
  nor2   g086(.a(x23), .b(x22), .O(new_n133_));
  nor2   g087(.a(x25), .b(x24), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n133_), .c(new_n91_), .d(new_n68_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(x16), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n131_), .O(z15));
  inv1   g092(.a(x15), .O(new_n139_));
  nand2  g093(.a(x18), .b(x07), .O(new_n140_));
  oai21  g094(.a(x18), .b(new_n139_), .c(new_n140_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n70_), .O(new_n142_));
  oai21  g096(.a(new_n63_), .b(new_n70_), .c(x27), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x25), .O(new_n144_));
  nand2  g098(.a(new_n124_), .b(x26), .O(new_n145_));
  nor3   g099(.a(x26), .b(x25), .c(x24), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n133_), .c(new_n91_), .d(new_n68_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(x16), .O(new_n149_));
  nand3  g103(.a(new_n149_), .b(new_n144_), .c(new_n142_), .O(z16));
  nor2   g104(.a(new_n48_), .b(x17), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n146_), .c(new_n133_), .d(new_n91_), .O(new_n152_));
  nand2  g106(.a(x27), .b(x17), .O(new_n153_));
  aoi21  g107(.a(new_n153_), .b(new_n152_), .c(new_n70_), .O(z17));
  buf    g108(.a(x27), .O(z08));
endmodule


