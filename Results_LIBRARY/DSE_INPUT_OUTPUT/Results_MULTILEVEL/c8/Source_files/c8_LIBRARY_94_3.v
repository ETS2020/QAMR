// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x23), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x22), .O(new_n60_));
  inv1   g014(.a(x27), .O(new_n61_));
  nand2  g015(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n59_), .c(new_n49_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  inv1   g018(.a(x23), .O(new_n65_));
  oai21  g019(.a(x27), .b(new_n65_), .c(new_n64_), .O(new_n66_));
  nand2  g020(.a(new_n61_), .b(new_n65_), .O(new_n67_));
  nand3  g021(.a(new_n67_), .b(new_n66_), .c(new_n48_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x24), .O(new_n71_));
  nand2  g025(.a(new_n61_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n49_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n49_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x25), .c(new_n75_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x26), .O(new_n79_));
  nand2  g033(.a(new_n61_), .b(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n78_), .c(new_n49_), .O(z07));
  nand2  g035(.a(new_n48_), .b(new_n61_), .O(z08));
  and2   g036(.a(x19), .b(x17), .O(new_n83_));
  nor2   g037(.a(x19), .b(x17), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(x18), .b(x00), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n47_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n85_), .c(new_n48_), .O(z09));
  inv1   g044(.a(x01), .O(new_n91_));
  nand2  g045(.a(new_n65_), .b(x18), .O(new_n92_));
  oai22  g046(.a(new_n92_), .b(new_n91_), .c(x18), .d(new_n52_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n86_), .O(new_n94_));
  inv1   g048(.a(x20), .O(new_n95_));
  nor2   g049(.a(new_n84_), .b(new_n95_), .O(new_n96_));
  nor3   g050(.a(x20), .b(x19), .c(x17), .O(new_n97_));
  or2    g051(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n48_), .c(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n94_), .O(z10));
  nand2  g054(.a(x18), .b(x02), .O(new_n101_));
  oai21  g055(.a(x18), .b(new_n55_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n86_), .O(new_n103_));
  inv1   g057(.a(x21), .O(new_n104_));
  nor2   g058(.a(x21), .b(x20), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n84_), .O(new_n106_));
  oai21  g060(.a(new_n97_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(x16), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n103_), .c(new_n48_), .O(z11));
  nand2  g063(.a(x18), .b(x03), .O(new_n110_));
  oai21  g064(.a(x18), .b(new_n58_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n86_), .O(new_n112_));
  inv1   g066(.a(new_n106_), .O(new_n113_));
  nor2   g067(.a(x22), .b(x21), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n84_), .c(new_n95_), .O(new_n115_));
  oai21  g069(.a(new_n113_), .b(new_n60_), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x16), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n112_), .c(new_n48_), .O(z12));
  inv1   g072(.a(x04), .O(new_n119_));
  oai22  g073(.a(new_n92_), .b(new_n119_), .c(x18), .d(new_n64_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n86_), .O(new_n121_));
  inv1   g075(.a(x18), .O(new_n122_));
  nand3  g076(.a(new_n115_), .b(x23), .c(new_n122_), .O(new_n123_));
  nor2   g077(.a(x23), .b(x22), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n97_), .c(new_n104_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(x16), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n121_), .O(z13));
  inv1   g082(.a(x05), .O(new_n129_));
  oai22  g083(.a(new_n92_), .b(new_n129_), .c(x18), .d(new_n69_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n86_), .O(new_n131_));
  nand2  g085(.a(new_n115_), .b(x24), .O(new_n132_));
  nor3   g086(.a(x24), .b(x22), .c(x21), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n97_), .O(new_n134_));
  aoi21  g088(.a(new_n134_), .b(new_n132_), .c(x23), .O(new_n135_));
  nor3   g089(.a(new_n71_), .b(new_n65_), .c(x18), .O(new_n136_));
  oai21  g090(.a(new_n136_), .b(new_n135_), .c(x16), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n131_), .O(z14));
  nand2  g092(.a(x18), .b(x06), .O(new_n139_));
  oai21  g093(.a(x18), .b(new_n74_), .c(new_n139_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n86_), .O(new_n141_));
  inv1   g095(.a(x25), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n86_), .c(new_n122_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x23), .O(new_n144_));
  aoi21  g098(.a(new_n133_), .b(new_n97_), .c(new_n142_), .O(new_n145_));
  nor2   g099(.a(x25), .b(x24), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n124_), .O(new_n147_));
  nor2   g101(.a(new_n147_), .b(new_n106_), .O(new_n148_));
  oai21  g102(.a(new_n148_), .b(new_n145_), .c(x16), .O(new_n149_));
  nand3  g103(.a(new_n149_), .b(new_n144_), .c(new_n141_), .O(z15));
  nand2  g104(.a(x18), .b(x07), .O(new_n151_));
  oai21  g105(.a(x18), .b(new_n77_), .c(new_n151_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n86_), .O(new_n153_));
  oai21  g107(.a(new_n79_), .b(new_n86_), .c(new_n122_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(x23), .O(new_n155_));
  nand3  g109(.a(new_n146_), .b(new_n114_), .c(new_n97_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(x26), .O(new_n157_));
  nor3   g111(.a(x26), .b(x25), .c(x24), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n124_), .c(new_n113_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(x16), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n155_), .c(new_n153_), .O(z16));
  inv1   g116(.a(x17), .O(new_n163_));
  nand2  g117(.a(new_n158_), .b(new_n124_), .O(new_n164_));
  nand3  g118(.a(new_n105_), .b(x19), .c(new_n163_), .O(new_n165_));
  oai22  g119(.a(new_n165_), .b(new_n164_), .c(new_n61_), .d(new_n163_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(x16), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n48_), .O(z17));
endmodule


