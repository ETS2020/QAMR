// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x18), .O(new_n47_));
  nor2   g001(.a(x22), .b(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x20), .O(new_n57_));
  nand2  g011(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  aoi21  g012(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(z01));
  nor2   g013(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g014(.a(z08), .O(new_n61_));
  inv1   g015(.a(x21), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n63_), .b(x18), .O(new_n64_));
  nand3  g018(.a(new_n64_), .b(new_n52_), .c(new_n62_), .O(new_n65_));
  oai21  g019(.a(new_n61_), .b(x10), .c(new_n65_), .O(z02));
  nand3  g020(.a(new_n52_), .b(new_n63_), .c(new_n47_), .O(new_n67_));
  oai21  g021(.a(new_n61_), .b(x11), .c(new_n67_), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n48_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x23), .c(new_n70_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x24), .O(new_n74_));
  nand2  g028(.a(new_n52_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n48_), .O(z05));
  inv1   g030(.a(x14), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n48_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x25), .c(new_n78_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  aoi21  g034(.a(x27), .b(new_n80_), .c(new_n48_), .O(new_n81_));
  oai21  g035(.a(x27), .b(x26), .c(new_n81_), .O(z07));
  inv1   g036(.a(new_n48_), .O(new_n83_));
  inv1   g037(.a(x17), .O(new_n84_));
  nor2   g038(.a(new_n51_), .b(new_n84_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x00), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n49_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n87_), .c(new_n83_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n55_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nor2   g049(.a(new_n86_), .b(new_n57_), .O(new_n96_));
  nor3   g050(.a(x20), .b(x19), .c(x17), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n96_), .c(x16), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n95_), .c(new_n83_), .O(z10));
  inv1   g053(.a(x10), .O(new_n100_));
  nand2  g054(.a(x18), .b(x02), .O(new_n101_));
  oai21  g055(.a(x18), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n88_), .O(new_n103_));
  nor2   g057(.a(x21), .b(x20), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n86_), .O(new_n105_));
  oai21  g059(.a(new_n97_), .b(new_n62_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x16), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n103_), .c(new_n83_), .O(z11));
  inv1   g062(.a(x03), .O(new_n109_));
  inv1   g063(.a(x11), .O(new_n110_));
  nand2  g064(.a(x22), .b(x18), .O(new_n111_));
  oai22  g065(.a(new_n111_), .b(new_n109_), .c(x18), .d(new_n110_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n88_), .O(new_n113_));
  nor3   g067(.a(x19), .b(x18), .c(x17), .O(new_n114_));
  nand2  g068(.a(new_n63_), .b(new_n62_), .O(new_n115_));
  nor2   g069(.a(new_n115_), .b(x20), .O(new_n116_));
  aoi22  g070(.a(new_n116_), .b(new_n114_), .c(new_n105_), .d(x22), .O(new_n117_));
  oai21  g071(.a(new_n117_), .b(new_n88_), .c(new_n113_), .O(z12));
  inv1   g072(.a(x04), .O(new_n119_));
  oai22  g073(.a(new_n111_), .b(new_n119_), .c(x18), .d(new_n69_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n88_), .O(new_n121_));
  nand2  g075(.a(new_n105_), .b(x23), .O(new_n122_));
  nor3   g076(.a(x23), .b(x22), .c(x21), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n97_), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n122_), .c(x18), .O(new_n125_));
  inv1   g079(.a(x23), .O(new_n126_));
  nor2   g080(.a(new_n126_), .b(new_n63_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n125_), .c(x16), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n121_), .O(z13));
  nand2  g083(.a(x18), .b(x05), .O(new_n130_));
  oai21  g084(.a(x18), .b(new_n72_), .c(new_n130_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n88_), .O(new_n132_));
  oai21  g086(.a(new_n74_), .b(new_n88_), .c(x22), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x18), .O(new_n134_));
  aoi21  g088(.a(new_n123_), .b(new_n97_), .c(new_n74_), .O(new_n135_));
  nand3  g089(.a(new_n57_), .b(new_n51_), .c(new_n84_), .O(new_n136_));
  nand2  g090(.a(new_n74_), .b(new_n126_), .O(new_n137_));
  nor3   g091(.a(new_n137_), .b(new_n115_), .c(new_n136_), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n135_), .c(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n134_), .c(new_n132_), .O(z14));
  inv1   g094(.a(x06), .O(new_n141_));
  oai22  g095(.a(new_n111_), .b(new_n141_), .c(x18), .d(new_n77_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n88_), .O(new_n143_));
  nand3  g097(.a(new_n74_), .b(new_n126_), .c(new_n62_), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n136_), .c(x25), .O(new_n145_));
  nor2   g099(.a(x23), .b(x22), .O(new_n146_));
  nor2   g100(.a(x25), .b(x24), .O(new_n147_));
  nand4  g101(.a(new_n147_), .b(new_n146_), .c(new_n104_), .d(new_n86_), .O(new_n148_));
  aoi21  g102(.a(new_n148_), .b(new_n145_), .c(x18), .O(new_n149_));
  and2   g103(.a(x25), .b(x22), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n149_), .c(x16), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n143_), .O(z15));
  nand2  g106(.a(x18), .b(x07), .O(new_n153_));
  oai21  g107(.a(x18), .b(new_n80_), .c(new_n153_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n88_), .O(new_n155_));
  nand2  g109(.a(x26), .b(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(x22), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(x18), .O(new_n158_));
  nand2  g112(.a(new_n148_), .b(x26), .O(new_n159_));
  nor3   g113(.a(x26), .b(x25), .c(x24), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n146_), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(new_n105_), .c(new_n159_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x16), .O(new_n163_));
  nand3  g117(.a(new_n163_), .b(new_n158_), .c(new_n155_), .O(z16));
  nand2  g118(.a(z08), .b(x17), .O(new_n165_));
  nand4  g119(.a(new_n104_), .b(x19), .c(new_n47_), .d(new_n84_), .O(new_n166_));
  or2    g120(.a(new_n166_), .b(new_n161_), .O(new_n167_));
  aoi21  g121(.a(new_n167_), .b(new_n165_), .c(new_n88_), .O(z17));
endmodule


