// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nand2  g002(.a(x27), .b(x24), .O(new_n49_));
  nand2  g003(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  oai21  g004(.a(new_n47_), .b(x08), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nor2   g006(.a(x24), .b(x19), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x21), .O(new_n58_));
  nand2  g012(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n53_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  aoi21  g015(.a(x27), .b(new_n61_), .c(new_n53_), .O(new_n62_));
  oai21  g016(.a(x27), .b(x22), .c(new_n62_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n53_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  oai21  g021(.a(x24), .b(new_n48_), .c(new_n49_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g023(.a(x24), .O(new_n70_));
  nand3  g024(.a(new_n47_), .b(new_n70_), .c(x19), .O(new_n71_));
  nand2  g025(.a(new_n71_), .b(new_n69_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  aoi21  g027(.a(x27), .b(new_n73_), .c(new_n53_), .O(new_n74_));
  oai21  g028(.a(x27), .b(x25), .c(new_n74_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n53_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x26), .c(new_n77_), .O(z07));
  nor2   g032(.a(new_n53_), .b(new_n47_), .O(z08));
  inv1   g033(.a(x16), .O(new_n80_));
  inv1   g034(.a(new_n53_), .O(new_n81_));
  inv1   g035(.a(x08), .O(new_n82_));
  nand2  g036(.a(x18), .b(x00), .O(new_n83_));
  oai21  g037(.a(x18), .b(new_n82_), .c(new_n83_), .O(new_n84_));
  nand3  g038(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  inv1   g039(.a(x17), .O(new_n86_));
  nand3  g040(.a(x24), .b(new_n48_), .c(new_n86_), .O(new_n87_));
  oai21  g041(.a(new_n48_), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(x16), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n85_), .O(z09));
  nand2  g044(.a(x18), .b(x01), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n52_), .c(new_n91_), .O(new_n92_));
  nand3  g046(.a(new_n92_), .b(new_n81_), .c(new_n80_), .O(new_n93_));
  inv1   g047(.a(x20), .O(new_n94_));
  nand2  g048(.a(x24), .b(x17), .O(new_n95_));
  aoi21  g049(.a(new_n95_), .b(new_n48_), .c(new_n94_), .O(new_n96_));
  nor2   g050(.a(x19), .b(x17), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(x24), .c(new_n94_), .O(new_n98_));
  inv1   g052(.a(new_n98_), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n96_), .c(x16), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n93_), .O(z10));
  nand2  g055(.a(x18), .b(x02), .O(new_n102_));
  oai21  g056(.a(x18), .b(new_n56_), .c(new_n102_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n81_), .c(new_n80_), .O(new_n104_));
  oai21  g058(.a(x20), .b(x17), .c(x24), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n48_), .c(new_n58_), .O(new_n106_));
  nand2  g060(.a(new_n48_), .b(new_n86_), .O(new_n107_));
  nand3  g061(.a(x24), .b(new_n58_), .c(new_n94_), .O(new_n108_));
  nor2   g062(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g063(.a(new_n109_), .b(new_n106_), .c(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n104_), .O(z11));
  nand2  g065(.a(x18), .b(x03), .O(new_n112_));
  oai21  g066(.a(x18), .b(new_n61_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n80_), .O(new_n114_));
  nand2  g068(.a(new_n86_), .b(x16), .O(new_n115_));
  inv1   g069(.a(x22), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n58_), .c(new_n94_), .O(new_n117_));
  oai21  g071(.a(new_n117_), .b(new_n115_), .c(x24), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n48_), .O(new_n119_));
  nand4  g073(.a(new_n97_), .b(x24), .c(new_n58_), .d(new_n94_), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(x22), .c(x16), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n119_), .c(new_n114_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n64_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n80_), .O(new_n125_));
  nand3  g079(.a(new_n94_), .b(new_n86_), .c(x16), .O(new_n126_));
  inv1   g080(.a(x23), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n116_), .c(new_n58_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n126_), .c(x24), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n48_), .O(new_n130_));
  nor3   g084(.a(x20), .b(x19), .c(x17), .O(new_n131_));
  nand4  g085(.a(new_n131_), .b(x24), .c(new_n116_), .d(new_n58_), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(x23), .c(x16), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n130_), .c(new_n125_), .O(z13));
  nand2  g088(.a(x18), .b(x05), .O(new_n135_));
  oai21  g089(.a(x18), .b(new_n67_), .c(new_n135_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n81_), .c(new_n80_), .O(new_n137_));
  nor2   g091(.a(x23), .b(x22), .O(new_n138_));
  nand3  g092(.a(new_n131_), .b(new_n138_), .c(new_n58_), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(x24), .c(x16), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n137_), .O(z14));
  nor2   g095(.a(x18), .b(new_n73_), .O(new_n142_));
  aoi21  g096(.a(x18), .b(x06), .c(new_n142_), .O(new_n143_));
  aoi21  g097(.a(x25), .b(x16), .c(new_n53_), .O(new_n144_));
  oai21  g098(.a(new_n143_), .b(x16), .c(new_n144_), .O(z15));
  nand2  g099(.a(x18), .b(x07), .O(new_n146_));
  oai21  g100(.a(x18), .b(new_n76_), .c(new_n146_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n80_), .O(new_n148_));
  nand2  g102(.a(x26), .b(x16), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n148_), .c(new_n53_), .O(z16));
  nand2  g104(.a(z08), .b(x17), .O(new_n151_));
  nor2   g105(.a(new_n48_), .b(x17), .O(new_n152_));
  nor2   g106(.a(x21), .b(x20), .O(new_n153_));
  nor3   g107(.a(x26), .b(x25), .c(x24), .O(new_n154_));
  nand4  g108(.a(new_n154_), .b(new_n153_), .c(new_n152_), .d(new_n138_), .O(new_n155_));
  aoi21  g109(.a(new_n155_), .b(new_n151_), .c(new_n80_), .O(z17));
endmodule


