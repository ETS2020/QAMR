// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x18), .b(x17), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  aoi21  g005(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  nand2  g008(.a(x27), .b(new_n54_), .O(new_n55_));
  inv1   g009(.a(x21), .O(new_n56_));
  inv1   g010(.a(x27), .O(new_n57_));
  nand2  g011(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g012(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z02));
  inv1   g013(.a(x11), .O(new_n60_));
  aoi21  g014(.a(x27), .b(new_n60_), .c(new_n48_), .O(new_n61_));
  oai21  g015(.a(x27), .b(x22), .c(new_n61_), .O(z03));
  inv1   g016(.a(x12), .O(new_n63_));
  aoi21  g017(.a(x27), .b(new_n63_), .c(new_n48_), .O(new_n64_));
  oai21  g018(.a(x27), .b(x23), .c(new_n64_), .O(z04));
  inv1   g019(.a(x13), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n48_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x24), .c(new_n67_), .O(z05));
  inv1   g022(.a(x14), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x25), .O(new_n71_));
  nand2  g025(.a(new_n57_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n48_), .O(z06));
  inv1   g027(.a(x15), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x26), .O(new_n76_));
  nand2  g030(.a(new_n57_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n48_), .O(z07));
  inv1   g032(.a(new_n48_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n57_), .O(z08));
  inv1   g034(.a(x16), .O(new_n81_));
  inv1   g035(.a(x17), .O(new_n82_));
  inv1   g036(.a(x19), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(x18), .c(new_n82_), .O(new_n84_));
  inv1   g038(.a(new_n84_), .O(new_n85_));
  aoi21  g039(.a(x19), .b(x17), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(x18), .b(x00), .O(new_n87_));
  inv1   g041(.a(x18), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(x17), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n47_), .c(new_n87_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n81_), .O(new_n91_));
  oai21  g045(.a(new_n86_), .b(new_n81_), .c(new_n91_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(new_n89_), .b(new_n51_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n81_), .O(new_n95_));
  nand2  g049(.a(x20), .b(x19), .O(new_n96_));
  nor2   g050(.a(x20), .b(x19), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n82_), .O(new_n98_));
  aoi21  g052(.a(new_n98_), .b(new_n96_), .c(new_n88_), .O(new_n99_));
  and2   g053(.a(x20), .b(x17), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n99_), .c(x16), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n95_), .O(z10));
  nand2  g056(.a(x18), .b(x02), .O(new_n103_));
  oai21  g057(.a(x18), .b(new_n54_), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n81_), .O(new_n105_));
  nor2   g059(.a(x21), .b(x20), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n83_), .c(x16), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(x18), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n82_), .O(new_n109_));
  nand3  g063(.a(new_n97_), .b(x18), .c(new_n82_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(x21), .c(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n109_), .c(new_n105_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(new_n89_), .b(new_n60_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n81_), .O(new_n115_));
  nand2  g069(.a(new_n106_), .b(new_n83_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x22), .O(new_n117_));
  nor2   g071(.a(x19), .b(x17), .O(new_n118_));
  nor3   g072(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  aoi21  g074(.a(new_n120_), .b(new_n117_), .c(new_n88_), .O(new_n121_));
  and2   g075(.a(x22), .b(x17), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n121_), .c(x16), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n115_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n63_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n81_), .O(new_n127_));
  nand2  g081(.a(new_n97_), .b(x16), .O(new_n128_));
  nor2   g082(.a(x23), .b(x22), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n56_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n128_), .c(x18), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n82_), .O(new_n132_));
  nand2  g086(.a(new_n119_), .b(new_n85_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(x23), .c(x16), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n132_), .c(new_n127_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  oai21  g090(.a(new_n89_), .b(new_n66_), .c(new_n136_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n81_), .O(new_n138_));
  nand3  g092(.a(new_n129_), .b(new_n97_), .c(new_n56_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x24), .O(new_n140_));
  nor2   g094(.a(x22), .b(x21), .O(new_n141_));
  nor2   g095(.a(x24), .b(x23), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n141_), .c(new_n97_), .d(new_n82_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n140_), .c(new_n88_), .O(new_n144_));
  and2   g098(.a(x24), .b(x17), .O(new_n145_));
  oai21  g099(.a(new_n145_), .b(new_n144_), .c(x16), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n138_), .O(z14));
  nand2  g101(.a(x18), .b(x06), .O(new_n148_));
  oai21  g102(.a(x18), .b(new_n69_), .c(new_n148_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n81_), .O(new_n150_));
  nor2   g104(.a(x25), .b(x24), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n129_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n107_), .c(x18), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n82_), .O(new_n154_));
  inv1   g108(.a(new_n110_), .O(new_n155_));
  nand3  g109(.a(new_n142_), .b(new_n141_), .c(new_n155_), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(x25), .c(x16), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n154_), .c(new_n150_), .O(z15));
  nand2  g112(.a(x18), .b(x07), .O(new_n159_));
  oai21  g113(.a(new_n89_), .b(new_n74_), .c(new_n159_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n81_), .O(new_n161_));
  nand4  g115(.a(new_n151_), .b(new_n129_), .c(new_n106_), .d(new_n83_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x26), .O(new_n163_));
  nor3   g117(.a(x26), .b(x25), .c(x24), .O(new_n164_));
  nand4  g118(.a(new_n164_), .b(new_n129_), .c(new_n118_), .d(new_n106_), .O(new_n165_));
  aoi21  g119(.a(new_n165_), .b(new_n163_), .c(new_n88_), .O(new_n166_));
  nor2   g120(.a(new_n76_), .b(new_n82_), .O(new_n167_));
  oai21  g121(.a(new_n167_), .b(new_n166_), .c(x16), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n161_), .O(z16));
  nand2  g123(.a(new_n164_), .b(new_n129_), .O(new_n170_));
  nand3  g124(.a(new_n106_), .b(x19), .c(new_n82_), .O(new_n171_));
  oai22  g125(.a(new_n171_), .b(new_n170_), .c(new_n57_), .d(new_n82_), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(x16), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(new_n79_), .O(z17));
endmodule


