// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x11), .O(new_n48_));
  nor2   g002(.a(x18), .b(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nand2  g006(.a(x27), .b(new_n52_), .O(new_n53_));
  inv1   g007(.a(x20), .O(new_n54_));
  inv1   g008(.a(x27), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(new_n53_), .c(new_n49_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x21), .c(new_n59_), .O(z02));
  inv1   g014(.a(x22), .O(new_n61_));
  oai21  g015(.a(x27), .b(new_n61_), .c(new_n48_), .O(new_n62_));
  nand3  g016(.a(new_n55_), .b(new_n61_), .c(x18), .O(new_n63_));
  nand2  g017(.a(new_n63_), .b(new_n62_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x23), .O(new_n67_));
  nand2  g021(.a(new_n55_), .b(new_n67_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n66_), .c(new_n49_), .O(z04));
  inv1   g023(.a(x13), .O(new_n70_));
  aoi21  g024(.a(x27), .b(new_n70_), .c(new_n49_), .O(new_n71_));
  oai21  g025(.a(x27), .b(x24), .c(new_n71_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  nand2  g027(.a(x27), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x25), .O(new_n75_));
  nand2  g029(.a(new_n55_), .b(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n49_), .O(z06));
  inv1   g031(.a(x15), .O(new_n78_));
  nand2  g032(.a(x27), .b(new_n78_), .O(new_n79_));
  inv1   g033(.a(x26), .O(new_n80_));
  nand2  g034(.a(new_n55_), .b(new_n80_), .O(new_n81_));
  aoi21  g035(.a(new_n81_), .b(new_n79_), .c(new_n49_), .O(z07));
  inv1   g036(.a(new_n49_), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n55_), .O(z08));
  inv1   g038(.a(x17), .O(new_n85_));
  inv1   g039(.a(x19), .O(new_n86_));
  nor2   g040(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  inv1   g043(.a(x16), .O(new_n90_));
  nand2  g044(.a(x18), .b(x00), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n47_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand3  g047(.a(new_n93_), .b(new_n89_), .c(new_n83_), .O(z09));
  nand2  g048(.a(x18), .b(x01), .O(new_n95_));
  oai21  g049(.a(x18), .b(new_n52_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n90_), .O(new_n97_));
  nor2   g051(.a(new_n88_), .b(new_n54_), .O(new_n98_));
  nand3  g052(.a(new_n54_), .b(new_n86_), .c(new_n85_), .O(new_n99_));
  inv1   g053(.a(new_n99_), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n98_), .c(x16), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n97_), .c(new_n83_), .O(z10));
  nand2  g056(.a(x18), .b(x02), .O(new_n103_));
  inv1   g057(.a(x18), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n48_), .c(x10), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n90_), .O(new_n107_));
  inv1   g061(.a(x21), .O(new_n108_));
  nor2   g062(.a(x21), .b(x20), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n88_), .O(new_n110_));
  oai21  g064(.a(new_n100_), .b(new_n108_), .c(new_n110_), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n83_), .c(x16), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n107_), .O(z11));
  aoi21  g067(.a(new_n104_), .b(x11), .c(x22), .O(new_n114_));
  nand4  g068(.a(new_n114_), .b(new_n108_), .c(new_n54_), .d(new_n86_), .O(new_n115_));
  nand2  g069(.a(new_n110_), .b(x22), .O(new_n116_));
  oai21  g070(.a(new_n115_), .b(x17), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x16), .O(new_n118_));
  nor2   g072(.a(new_n104_), .b(x16), .O(new_n119_));
  aoi21  g073(.a(new_n119_), .b(x03), .c(new_n49_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n118_), .O(z12));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(x18), .b(new_n65_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n90_), .O(new_n124_));
  nand4  g078(.a(new_n88_), .b(new_n61_), .c(new_n108_), .d(new_n54_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x23), .O(new_n126_));
  nor3   g080(.a(x23), .b(x22), .c(x21), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n100_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(x16), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n124_), .c(new_n83_), .O(z13));
  nand2  g085(.a(x18), .b(x05), .O(new_n132_));
  oai21  g086(.a(x18), .b(new_n70_), .c(new_n132_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n90_), .O(new_n134_));
  inv1   g088(.a(x24), .O(new_n135_));
  aoi21  g089(.a(new_n127_), .b(new_n100_), .c(new_n135_), .O(new_n136_));
  nand4  g090(.a(new_n135_), .b(new_n67_), .c(new_n61_), .d(new_n108_), .O(new_n137_));
  nor2   g091(.a(new_n137_), .b(new_n99_), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n134_), .c(new_n83_), .O(z14));
  nand2  g094(.a(x18), .b(x06), .O(new_n141_));
  nand3  g095(.a(new_n104_), .b(x14), .c(new_n48_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n90_), .O(new_n144_));
  nor2   g098(.a(x23), .b(x22), .O(new_n145_));
  nor2   g099(.a(x25), .b(x24), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n145_), .c(new_n109_), .d(new_n88_), .O(new_n147_));
  oai21  g101(.a(new_n138_), .b(new_n75_), .c(new_n147_), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n83_), .c(x16), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n144_), .O(z15));
  nand2  g104(.a(x18), .b(x07), .O(new_n151_));
  nand3  g105(.a(new_n104_), .b(x15), .c(new_n48_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n90_), .O(new_n154_));
  nand2  g108(.a(new_n147_), .b(x26), .O(new_n155_));
  nand4  g109(.a(new_n145_), .b(new_n80_), .c(new_n75_), .d(new_n135_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n110_), .c(new_n155_), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n83_), .c(x16), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n154_), .O(z16));
  nand3  g113(.a(new_n109_), .b(x19), .c(new_n85_), .O(new_n160_));
  oai22  g114(.a(new_n160_), .b(new_n156_), .c(new_n55_), .d(new_n85_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x16), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n83_), .O(z17));
endmodule


