// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x21), .O(new_n57_));
  inv1   g011(.a(x27), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n56_), .c(new_n49_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n58_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n49_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x24), .O(new_n71_));
  nand2  g025(.a(new_n58_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n49_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x25), .O(new_n76_));
  nand2  g030(.a(new_n58_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n49_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  nand2  g033(.a(x27), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x26), .O(new_n81_));
  nand2  g035(.a(new_n58_), .b(new_n81_), .O(new_n82_));
  aoi21  g036(.a(new_n82_), .b(new_n80_), .c(new_n49_), .O(z07));
  nor2   g037(.a(new_n49_), .b(new_n58_), .O(z08));
  nand3  g038(.a(x19), .b(x18), .c(x17), .O(new_n85_));
  inv1   g039(.a(new_n85_), .O(new_n86_));
  nor2   g040(.a(x19), .b(x17), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n86_), .c(x16), .O(new_n88_));
  nor2   g042(.a(x19), .b(x18), .O(new_n89_));
  aoi22  g043(.a(new_n89_), .b(x08), .c(x18), .d(x00), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(x16), .c(new_n88_), .O(z09));
  aoi22  g045(.a(new_n89_), .b(x09), .c(x18), .d(x01), .O(new_n92_));
  inv1   g046(.a(x17), .O(new_n93_));
  inv1   g047(.a(x20), .O(new_n94_));
  nor2   g048(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nor2   g049(.a(x20), .b(x17), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n95_), .c(new_n48_), .O(new_n97_));
  nand3  g051(.a(x20), .b(x19), .c(x18), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(x16), .O(new_n100_));
  oai21  g054(.a(new_n92_), .b(x16), .c(new_n100_), .O(z10));
  inv1   g055(.a(x16), .O(new_n102_));
  nand2  g056(.a(x18), .b(x02), .O(new_n103_));
  oai21  g057(.a(x18), .b(new_n55_), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  oai21  g059(.a(new_n57_), .b(new_n102_), .c(x18), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x19), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n87_), .O(new_n109_));
  oai21  g063(.a(new_n96_), .b(new_n57_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n107_), .c(new_n105_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n61_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n102_), .O(new_n115_));
  oai21  g069(.a(new_n63_), .b(new_n102_), .c(x18), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x19), .O(new_n117_));
  nand3  g071(.a(new_n57_), .b(new_n94_), .c(new_n93_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x22), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n87_), .c(new_n94_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n117_), .c(new_n115_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n66_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n102_), .O(new_n127_));
  inv1   g081(.a(x23), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n102_), .c(x18), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(x19), .O(new_n130_));
  aoi21  g084(.a(new_n120_), .b(new_n96_), .c(new_n128_), .O(new_n131_));
  nand3  g085(.a(new_n94_), .b(new_n48_), .c(new_n93_), .O(new_n132_));
  nor2   g086(.a(x23), .b(x22), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n57_), .O(new_n134_));
  nor2   g088(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n131_), .c(x16), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n130_), .c(new_n127_), .O(z13));
  nand2  g091(.a(x18), .b(x05), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n69_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n102_), .O(new_n140_));
  oai21  g094(.a(new_n71_), .b(new_n102_), .c(x18), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x19), .O(new_n142_));
  nand3  g096(.a(new_n133_), .b(new_n96_), .c(new_n57_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x24), .O(new_n144_));
  nor2   g098(.a(x24), .b(x23), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n120_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n132_), .c(new_n144_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(x16), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n142_), .c(new_n140_), .O(z14));
  inv1   g103(.a(new_n89_), .O(new_n150_));
  nand2  g104(.a(x18), .b(x06), .O(new_n151_));
  oai21  g105(.a(new_n150_), .b(new_n74_), .c(new_n151_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n102_), .O(new_n153_));
  nand3  g107(.a(new_n71_), .b(new_n128_), .c(new_n63_), .O(new_n154_));
  oai21  g108(.a(new_n154_), .b(new_n118_), .c(x25), .O(new_n155_));
  nor2   g109(.a(x25), .b(x24), .O(new_n156_));
  nand4  g110(.a(new_n156_), .b(new_n133_), .c(new_n108_), .d(new_n93_), .O(new_n157_));
  aoi21  g111(.a(new_n157_), .b(new_n155_), .c(x19), .O(new_n158_));
  nand3  g112(.a(x25), .b(x19), .c(x18), .O(new_n159_));
  inv1   g113(.a(new_n159_), .O(new_n160_));
  oai21  g114(.a(new_n160_), .b(new_n158_), .c(x16), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n153_), .O(z15));
  nand2  g116(.a(x18), .b(x07), .O(new_n163_));
  oai21  g117(.a(new_n150_), .b(new_n79_), .c(new_n163_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n102_), .O(new_n165_));
  nand2  g119(.a(new_n157_), .b(x26), .O(new_n166_));
  nor2   g120(.a(x26), .b(x25), .O(new_n167_));
  nand4  g121(.a(new_n167_), .b(new_n145_), .c(new_n120_), .d(new_n96_), .O(new_n168_));
  aoi21  g122(.a(new_n168_), .b(new_n166_), .c(x19), .O(new_n169_));
  nand3  g123(.a(x26), .b(x19), .c(x18), .O(new_n170_));
  inv1   g124(.a(new_n170_), .O(new_n171_));
  oai21  g125(.a(new_n171_), .b(new_n169_), .c(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n165_), .O(z16));
  inv1   g127(.a(new_n49_), .O(new_n174_));
  nand3  g128(.a(new_n108_), .b(x19), .c(new_n93_), .O(new_n175_));
  nand3  g129(.a(new_n167_), .b(new_n133_), .c(new_n71_), .O(new_n176_));
  oai22  g130(.a(new_n176_), .b(new_n175_), .c(new_n58_), .d(new_n93_), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(x16), .O(new_n178_));
  nand2  g132(.a(new_n178_), .b(new_n174_), .O(z17));
endmodule


