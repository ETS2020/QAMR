// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_;
  nand2  g000(.a(x21), .b(x16), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x16), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  oai22  g014(.a(new_n60_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  inv1   g017(.a(x22), .O(new_n64_));
  nand2  g018(.a(new_n52_), .b(new_n64_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n63_), .c(new_n48_), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x23), .O(new_n69_));
  nand2  g023(.a(new_n52_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n48_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x24), .O(new_n74_));
  nand2  g028(.a(new_n52_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n48_), .O(z05));
  inv1   g030(.a(x14), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x25), .O(new_n79_));
  nand2  g033(.a(new_n52_), .b(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n78_), .c(new_n48_), .O(z06));
  inv1   g035(.a(x15), .O(new_n82_));
  aoi21  g036(.a(x27), .b(new_n82_), .c(new_n48_), .O(new_n83_));
  oai21  g037(.a(x27), .b(x26), .c(new_n83_), .O(z07));
  nand2  g038(.a(new_n47_), .b(new_n52_), .O(z08));
  nor2   g039(.a(x18), .b(new_n49_), .O(new_n86_));
  aoi21  g040(.a(x18), .b(x00), .c(new_n86_), .O(new_n87_));
  nand2  g041(.a(x19), .b(x17), .O(new_n88_));
  inv1   g042(.a(x17), .O(new_n89_));
  nand2  g043(.a(new_n51_), .b(new_n89_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n88_), .c(new_n59_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(x16), .O(new_n92_));
  oai21  g046(.a(new_n87_), .b(x16), .c(new_n92_), .O(z09));
  nor2   g047(.a(x18), .b(new_n55_), .O(new_n94_));
  aoi21  g048(.a(x18), .b(x01), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n90_), .b(x20), .O(new_n96_));
  nor2   g050(.a(x20), .b(x19), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n89_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n96_), .c(new_n59_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(x16), .O(new_n100_));
  oai21  g054(.a(new_n95_), .b(x16), .c(new_n100_), .O(z10));
  nor3   g055(.a(x20), .b(x19), .c(x17), .O(new_n102_));
  oai21  g056(.a(new_n102_), .b(x21), .c(x16), .O(new_n103_));
  inv1   g057(.a(x10), .O(new_n104_));
  nor2   g058(.a(x18), .b(new_n104_), .O(new_n105_));
  aoi21  g059(.a(x18), .b(x02), .c(new_n105_), .O(new_n106_));
  oai21  g060(.a(new_n106_), .b(x16), .c(new_n103_), .O(z11));
  nand2  g061(.a(x18), .b(x03), .O(new_n108_));
  oai21  g062(.a(x18), .b(new_n62_), .c(new_n108_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n58_), .O(new_n110_));
  nor2   g064(.a(new_n102_), .b(new_n64_), .O(new_n111_));
  nor2   g065(.a(x19), .b(x17), .O(new_n112_));
  nor2   g066(.a(x22), .b(x20), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n59_), .O(new_n115_));
  oai21  g069(.a(new_n115_), .b(new_n111_), .c(x16), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n110_), .O(z12));
  nor2   g071(.a(x18), .b(new_n67_), .O(new_n118_));
  aoi21  g072(.a(x18), .b(x04), .c(new_n118_), .O(new_n119_));
  aoi21  g073(.a(new_n113_), .b(new_n112_), .c(new_n69_), .O(new_n120_));
  inv1   g074(.a(x20), .O(new_n121_));
  nor2   g075(.a(x23), .b(x22), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n112_), .c(new_n121_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n59_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n120_), .c(x16), .O(new_n125_));
  oai21  g079(.a(new_n119_), .b(x16), .c(new_n125_), .O(z13));
  nand2  g080(.a(x18), .b(x05), .O(new_n127_));
  oai21  g081(.a(x18), .b(new_n72_), .c(new_n127_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n58_), .O(new_n129_));
  nand2  g083(.a(new_n123_), .b(x24), .O(new_n130_));
  nor3   g084(.a(x24), .b(x23), .c(x22), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n102_), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n130_), .c(new_n59_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x16), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n129_), .O(z14));
  nand2  g089(.a(x18), .b(x06), .O(new_n136_));
  oai21  g090(.a(x18), .b(new_n77_), .c(new_n136_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n58_), .O(new_n138_));
  aoi21  g092(.a(new_n131_), .b(new_n102_), .c(new_n79_), .O(new_n139_));
  nor2   g093(.a(x25), .b(x24), .O(new_n140_));
  nand4  g094(.a(new_n140_), .b(new_n122_), .c(new_n97_), .d(new_n89_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n59_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n139_), .c(x16), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n138_), .O(z15));
  nand2  g098(.a(x18), .b(x07), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n82_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n58_), .O(new_n147_));
  nand2  g101(.a(new_n141_), .b(x26), .O(new_n148_));
  nor2   g102(.a(x24), .b(x23), .O(new_n149_));
  nor2   g103(.a(x26), .b(x25), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n149_), .c(new_n113_), .d(new_n112_), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n148_), .c(new_n59_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(x16), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n147_), .O(z16));
  nand2  g108(.a(new_n150_), .b(new_n149_), .O(new_n155_));
  nand3  g109(.a(new_n113_), .b(x19), .c(new_n89_), .O(new_n156_));
  nor2   g110(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nor2   g111(.a(new_n52_), .b(new_n89_), .O(new_n158_));
  nor2   g112(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  aoi21  g113(.a(new_n159_), .b(new_n59_), .c(new_n58_), .O(z17));
endmodule


