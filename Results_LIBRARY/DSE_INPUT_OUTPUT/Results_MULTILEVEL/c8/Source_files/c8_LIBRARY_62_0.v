// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_;
  nor2   g000(.a(x24), .b(x18), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n47_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x20), .c(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  nand2  g013(.a(new_n51_), .b(new_n59_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n58_), .c(new_n47_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n47_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n47_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x18), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  oai21  g023(.a(new_n51_), .b(new_n68_), .c(new_n69_), .O(new_n70_));
  oai21  g024(.a(new_n51_), .b(x13), .c(new_n70_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n47_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n47_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  inv1   g031(.a(new_n47_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n51_), .O(z08));
  inv1   g033(.a(x17), .O(new_n80_));
  nor2   g034(.a(new_n50_), .b(new_n80_), .O(new_n81_));
  nor2   g035(.a(x19), .b(x17), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n81_), .c(x16), .O(new_n83_));
  inv1   g037(.a(x16), .O(new_n84_));
  nand2  g038(.a(x18), .b(x00), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n48_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n83_), .c(new_n78_), .O(z09));
  nand2  g042(.a(x18), .b(x01), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n54_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n84_), .O(new_n91_));
  inv1   g045(.a(x20), .O(new_n92_));
  nor2   g046(.a(new_n82_), .b(new_n92_), .O(new_n93_));
  nand3  g047(.a(new_n92_), .b(new_n50_), .c(new_n80_), .O(new_n94_));
  inv1   g048(.a(new_n94_), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n93_), .c(x16), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n91_), .c(new_n78_), .O(z10));
  nand2  g051(.a(x18), .b(x02), .O(new_n98_));
  nand2  g052(.a(x24), .b(new_n68_), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n57_), .c(new_n98_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n84_), .O(new_n101_));
  nor2   g055(.a(x21), .b(x20), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n82_), .O(new_n103_));
  oai21  g057(.a(new_n95_), .b(new_n59_), .c(new_n103_), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n78_), .c(x16), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n101_), .O(z11));
  nand2  g060(.a(x18), .b(x03), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n62_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n84_), .O(new_n109_));
  aoi21  g063(.a(new_n69_), .b(new_n68_), .c(x22), .O(new_n110_));
  nand4  g064(.a(new_n110_), .b(new_n59_), .c(new_n92_), .d(new_n50_), .O(new_n111_));
  nand2  g065(.a(new_n103_), .b(x22), .O(new_n112_));
  oai21  g066(.a(new_n111_), .b(x17), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(x16), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n109_), .c(new_n78_), .O(z12));
  nand2  g069(.a(x18), .b(x04), .O(new_n116_));
  oai21  g070(.a(x18), .b(new_n65_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n84_), .O(new_n118_));
  inv1   g072(.a(x22), .O(new_n119_));
  nand4  g073(.a(new_n82_), .b(new_n119_), .c(new_n59_), .d(new_n92_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x23), .O(new_n121_));
  nor2   g075(.a(x23), .b(x22), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n95_), .c(new_n59_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(x16), .O(new_n125_));
  nand3  g079(.a(new_n125_), .b(new_n118_), .c(new_n78_), .O(z13));
  inv1   g080(.a(x13), .O(new_n127_));
  nand2  g081(.a(x18), .b(x05), .O(new_n128_));
  oai21  g082(.a(new_n99_), .b(new_n127_), .c(new_n128_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n84_), .O(new_n130_));
  nand4  g084(.a(new_n92_), .b(new_n50_), .c(x18), .d(new_n80_), .O(new_n131_));
  inv1   g085(.a(x23), .O(new_n132_));
  nand4  g086(.a(new_n69_), .b(new_n132_), .c(new_n119_), .d(new_n59_), .O(new_n133_));
  nor2   g087(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  aoi21  g088(.a(new_n123_), .b(x24), .c(new_n134_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n84_), .c(new_n130_), .O(z14));
  nand2  g090(.a(x18), .b(x06), .O(new_n137_));
  oai21  g091(.a(new_n99_), .b(new_n72_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n84_), .O(new_n139_));
  nand3  g093(.a(new_n132_), .b(new_n119_), .c(new_n59_), .O(new_n140_));
  oai21  g094(.a(new_n140_), .b(new_n94_), .c(x25), .O(new_n141_));
  nor2   g095(.a(x25), .b(x24), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n122_), .c(new_n102_), .d(new_n82_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(new_n68_), .O(new_n144_));
  inv1   g098(.a(x25), .O(new_n145_));
  nor2   g099(.a(new_n145_), .b(new_n69_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n139_), .O(z15));
  nand2  g102(.a(x18), .b(x07), .O(new_n149_));
  oai21  g103(.a(x18), .b(new_n75_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n84_), .O(new_n151_));
  inv1   g105(.a(x26), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n84_), .c(x24), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n68_), .O(new_n154_));
  nand2  g108(.a(new_n143_), .b(x26), .O(new_n155_));
  nand4  g109(.a(new_n122_), .b(new_n152_), .c(new_n145_), .d(new_n69_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n103_), .c(new_n155_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(x16), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n154_), .c(new_n151_), .O(z16));
  nand3  g113(.a(new_n102_), .b(x19), .c(new_n80_), .O(new_n160_));
  oai22  g114(.a(new_n160_), .b(new_n156_), .c(new_n51_), .d(new_n80_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x16), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n78_), .O(z17));
endmodule


