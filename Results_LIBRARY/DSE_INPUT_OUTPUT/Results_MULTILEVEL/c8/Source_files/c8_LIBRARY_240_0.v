// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_;
  nor2   g000(.a(x24), .b(x19), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(x27), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand3  g005(.a(new_n51_), .b(x24), .c(new_n50_), .O(new_n52_));
  oai21  g006(.a(new_n49_), .b(x08), .c(new_n52_), .O(z00));
  inv1   g007(.a(x20), .O(new_n54_));
  inv1   g008(.a(x24), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nand3  g010(.a(new_n56_), .b(new_n51_), .c(new_n54_), .O(new_n57_));
  oai21  g011(.a(new_n49_), .b(x09), .c(new_n57_), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n59_), .c(new_n47_), .O(new_n60_));
  oai21  g014(.a(x27), .b(x21), .c(new_n60_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n47_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n47_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  oai21  g021(.a(new_n51_), .b(new_n50_), .c(new_n55_), .O(new_n68_));
  oai21  g022(.a(new_n51_), .b(x13), .c(new_n68_), .O(z05));
  inv1   g023(.a(x14), .O(new_n70_));
  aoi21  g024(.a(x27), .b(new_n70_), .c(new_n47_), .O(new_n71_));
  oai21  g025(.a(x27), .b(x25), .c(new_n71_), .O(z06));
  inv1   g026(.a(x15), .O(new_n73_));
  nand2  g027(.a(x27), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x26), .O(new_n75_));
  nand2  g029(.a(new_n51_), .b(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n47_), .O(z07));
  nand2  g031(.a(new_n48_), .b(new_n51_), .O(z08));
  inv1   g032(.a(x16), .O(new_n79_));
  inv1   g033(.a(x08), .O(new_n80_));
  nand2  g034(.a(x18), .b(x00), .O(new_n81_));
  oai21  g035(.a(x18), .b(new_n80_), .c(new_n81_), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n48_), .c(new_n79_), .O(new_n83_));
  inv1   g037(.a(x17), .O(new_n84_));
  nand3  g038(.a(x24), .b(new_n50_), .c(new_n84_), .O(new_n85_));
  oai21  g039(.a(new_n50_), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(x16), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n83_), .O(z09));
  inv1   g042(.a(x09), .O(new_n89_));
  nand2  g043(.a(x18), .b(x01), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n89_), .c(new_n90_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n48_), .c(new_n79_), .O(new_n92_));
  nand2  g046(.a(x24), .b(x17), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(new_n50_), .c(new_n54_), .O(new_n94_));
  nand4  g048(.a(x24), .b(new_n54_), .c(new_n50_), .d(new_n84_), .O(new_n95_));
  inv1   g049(.a(new_n95_), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n92_), .O(z10));
  nand2  g052(.a(x18), .b(x02), .O(new_n99_));
  oai21  g053(.a(x18), .b(new_n59_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n79_), .O(new_n101_));
  inv1   g055(.a(x21), .O(new_n102_));
  nand4  g056(.a(new_n102_), .b(new_n54_), .c(new_n84_), .d(x16), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x24), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n50_), .O(new_n105_));
  nand3  g059(.a(new_n95_), .b(x21), .c(x16), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n105_), .c(new_n101_), .O(z11));
  nand2  g061(.a(x18), .b(x03), .O(new_n108_));
  oai21  g062(.a(x18), .b(new_n62_), .c(new_n108_), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n48_), .c(new_n79_), .O(new_n110_));
  nor3   g064(.a(x21), .b(x20), .c(x17), .O(new_n111_));
  oai21  g065(.a(new_n111_), .b(new_n55_), .c(new_n50_), .O(new_n112_));
  nand3  g066(.a(new_n54_), .b(new_n50_), .c(new_n84_), .O(new_n113_));
  inv1   g067(.a(x22), .O(new_n114_));
  nand3  g068(.a(x24), .b(new_n114_), .c(new_n102_), .O(new_n115_));
  nor2   g069(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  aoi21  g070(.a(new_n112_), .b(x22), .c(new_n116_), .O(new_n117_));
  oai21  g071(.a(new_n117_), .b(new_n79_), .c(new_n110_), .O(z12));
  nand2  g072(.a(x18), .b(x04), .O(new_n119_));
  oai21  g073(.a(x18), .b(new_n65_), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n79_), .O(new_n121_));
  nor2   g075(.a(x20), .b(x17), .O(new_n122_));
  nor3   g076(.a(x23), .b(x22), .c(x21), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n122_), .c(x16), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(x24), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n50_), .O(new_n126_));
  inv1   g080(.a(x23), .O(new_n127_));
  nor2   g081(.a(new_n116_), .b(new_n127_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n126_), .c(new_n121_), .O(z13));
  inv1   g084(.a(x13), .O(new_n131_));
  nand2  g085(.a(x18), .b(x05), .O(new_n132_));
  oai21  g086(.a(x18), .b(new_n131_), .c(new_n132_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n48_), .c(new_n79_), .O(new_n134_));
  nand4  g088(.a(new_n123_), .b(new_n54_), .c(new_n50_), .d(new_n84_), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(x24), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n134_), .O(z14));
  nand2  g091(.a(x18), .b(x06), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n70_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n79_), .O(new_n140_));
  nand2  g094(.a(x25), .b(x16), .O(new_n141_));
  aoi21  g095(.a(new_n141_), .b(new_n140_), .c(new_n47_), .O(z15));
  nor2   g096(.a(x18), .b(new_n73_), .O(new_n143_));
  aoi21  g097(.a(x18), .b(x07), .c(new_n143_), .O(new_n144_));
  aoi21  g098(.a(x26), .b(x16), .c(new_n47_), .O(new_n145_));
  oai21  g099(.a(new_n144_), .b(x16), .c(new_n145_), .O(z16));
  nor2   g100(.a(x22), .b(x21), .O(new_n147_));
  nor2   g101(.a(x24), .b(x23), .O(new_n148_));
  nor2   g102(.a(x26), .b(x25), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n148_), .c(new_n147_), .d(new_n122_), .O(new_n150_));
  oai21  g104(.a(new_n51_), .b(new_n84_), .c(new_n150_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n48_), .O(z17));
endmodule


