// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  inv1   g002(.a(x20), .O(new_n49_));
  nor2   g003(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g004(.a(x27), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  oai21  g005(.a(x27), .b(x19), .c(new_n51_), .O(z00));
  inv1   g006(.a(x09), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n53_), .c(new_n50_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n56_), .c(new_n50_), .O(new_n57_));
  oai21  g011(.a(x27), .b(x21), .c(new_n57_), .O(z02));
  inv1   g012(.a(x11), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x22), .O(new_n61_));
  inv1   g015(.a(x27), .O(new_n62_));
  nand2  g016(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n60_), .c(new_n50_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n50_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x24), .O(new_n70_));
  nand2  g024(.a(new_n62_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n50_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  aoi21  g027(.a(x27), .b(new_n73_), .c(new_n50_), .O(new_n74_));
  oai21  g028(.a(x27), .b(x25), .c(new_n74_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n50_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x26), .c(new_n77_), .O(z07));
  inv1   g032(.a(new_n50_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n62_), .O(z08));
  inv1   g034(.a(x16), .O(new_n81_));
  nand2  g035(.a(x18), .b(x00), .O(new_n82_));
  oai21  g036(.a(x18), .b(new_n47_), .c(new_n82_), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g038(.a(x20), .b(x18), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(x19), .c(x17), .O(new_n86_));
  nor2   g040(.a(x19), .b(x17), .O(new_n87_));
  inv1   g041(.a(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(x16), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n84_), .c(new_n79_), .O(z09));
  inv1   g045(.a(x01), .O(new_n92_));
  nand2  g046(.a(new_n49_), .b(x18), .O(new_n93_));
  oai22  g047(.a(new_n93_), .b(new_n92_), .c(x18), .d(new_n53_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n81_), .O(new_n95_));
  nor3   g049(.a(new_n87_), .b(new_n49_), .c(x18), .O(new_n96_));
  nor3   g050(.a(x20), .b(x19), .c(x17), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n96_), .c(x16), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n95_), .O(z10));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  oai21  g054(.a(x18), .b(new_n56_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n81_), .O(new_n102_));
  inv1   g056(.a(x21), .O(new_n103_));
  oai21  g057(.a(new_n103_), .b(new_n81_), .c(new_n48_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(x20), .O(new_n105_));
  nand3  g059(.a(new_n87_), .b(new_n103_), .c(new_n49_), .O(new_n106_));
  oai21  g060(.a(new_n87_), .b(new_n103_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(x16), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n105_), .c(new_n102_), .O(z11));
  inv1   g063(.a(x03), .O(new_n110_));
  oai22  g064(.a(new_n93_), .b(new_n110_), .c(x18), .d(new_n59_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n81_), .O(new_n112_));
  nor3   g066(.a(x21), .b(x19), .c(x17), .O(new_n113_));
  nor2   g067(.a(x22), .b(x21), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n87_), .O(new_n115_));
  oai21  g069(.a(new_n113_), .b(new_n61_), .c(new_n115_), .O(new_n116_));
  nor3   g070(.a(new_n61_), .b(new_n49_), .c(x18), .O(new_n117_));
  aoi21  g071(.a(new_n116_), .b(new_n49_), .c(new_n117_), .O(new_n118_));
  oai21  g072(.a(new_n118_), .b(new_n81_), .c(new_n112_), .O(z12));
  inv1   g073(.a(x04), .O(new_n120_));
  oai22  g074(.a(new_n93_), .b(new_n120_), .c(x18), .d(new_n65_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n81_), .O(new_n122_));
  nand2  g076(.a(new_n115_), .b(x23), .O(new_n123_));
  nor2   g077(.a(x23), .b(x22), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n87_), .c(new_n103_), .O(new_n125_));
  aoi21  g079(.a(new_n125_), .b(new_n123_), .c(x20), .O(new_n126_));
  nand2  g080(.a(x23), .b(x20), .O(new_n127_));
  nor2   g081(.a(new_n127_), .b(x18), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n126_), .c(x16), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n122_), .O(z13));
  inv1   g084(.a(x05), .O(new_n131_));
  oai22  g085(.a(new_n93_), .b(new_n131_), .c(x18), .d(new_n68_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n81_), .O(new_n133_));
  nand2  g087(.a(new_n125_), .b(x24), .O(new_n134_));
  nor3   g088(.a(x24), .b(x23), .c(x22), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n113_), .O(new_n136_));
  aoi21  g090(.a(new_n136_), .b(new_n134_), .c(x20), .O(new_n137_));
  nor3   g091(.a(new_n70_), .b(new_n49_), .c(x18), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n137_), .c(x16), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n133_), .O(z14));
  nand2  g094(.a(x18), .b(x06), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n73_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n81_), .O(new_n143_));
  inv1   g097(.a(x25), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n81_), .c(new_n48_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x20), .O(new_n146_));
  aoi21  g100(.a(new_n135_), .b(new_n113_), .c(new_n144_), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n124_), .O(new_n149_));
  nor2   g103(.a(new_n149_), .b(new_n106_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n147_), .c(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n146_), .c(new_n143_), .O(z15));
  inv1   g106(.a(x07), .O(new_n153_));
  oai22  g107(.a(new_n93_), .b(new_n153_), .c(x18), .d(new_n76_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n81_), .O(new_n155_));
  inv1   g109(.a(x17), .O(new_n156_));
  nor2   g110(.a(x21), .b(x19), .O(new_n157_));
  nand4  g111(.a(new_n148_), .b(new_n124_), .c(new_n157_), .d(new_n156_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x26), .O(new_n159_));
  nor2   g113(.a(x24), .b(x23), .O(new_n160_));
  nor2   g114(.a(x26), .b(x25), .O(new_n161_));
  nand4  g115(.a(new_n161_), .b(new_n160_), .c(new_n114_), .d(new_n87_), .O(new_n162_));
  aoi21  g116(.a(new_n162_), .b(new_n159_), .c(x20), .O(new_n163_));
  nand2  g117(.a(x26), .b(x20), .O(new_n164_));
  nor2   g118(.a(new_n164_), .b(x18), .O(new_n165_));
  oai21  g119(.a(new_n165_), .b(new_n163_), .c(x16), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n155_), .O(z16));
  nand4  g121(.a(new_n103_), .b(new_n49_), .c(x19), .d(new_n156_), .O(new_n168_));
  nand3  g122(.a(new_n161_), .b(new_n124_), .c(new_n70_), .O(new_n169_));
  oai22  g123(.a(new_n169_), .b(new_n168_), .c(new_n62_), .d(new_n156_), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(x16), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n79_), .O(z17));
endmodule


