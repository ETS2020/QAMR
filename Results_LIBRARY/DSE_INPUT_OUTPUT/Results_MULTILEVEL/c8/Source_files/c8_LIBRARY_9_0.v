// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_;
  nor2   g000(.a(x20), .b(x18), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x18), .O(new_n54_));
  inv1   g008(.a(x20), .O(new_n55_));
  oai21  g009(.a(new_n51_), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  oai21  g010(.a(new_n51_), .b(x09), .c(new_n56_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n47_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x21), .c(new_n59_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n51_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n47_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n47_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x24), .O(new_n71_));
  nand2  g025(.a(new_n51_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n47_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n47_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x25), .c(new_n75_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x26), .O(new_n79_));
  nand2  g033(.a(new_n51_), .b(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n78_), .c(new_n47_), .O(z07));
  inv1   g035(.a(new_n47_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n51_), .O(z08));
  inv1   g037(.a(x17), .O(new_n84_));
  nor2   g038(.a(new_n50_), .b(new_n84_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x00), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n48_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n87_), .c(new_n82_), .O(z09));
  inv1   g046(.a(x09), .O(new_n93_));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  nand2  g048(.a(x20), .b(new_n54_), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n93_), .c(new_n94_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n88_), .O(new_n97_));
  nor2   g051(.a(new_n54_), .b(x17), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n55_), .c(new_n50_), .O(new_n99_));
  oai21  g053(.a(new_n86_), .b(new_n55_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(x16), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n97_), .O(z10));
  nand2  g056(.a(x18), .b(x02), .O(new_n103_));
  oai21  g057(.a(new_n95_), .b(new_n58_), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n88_), .O(new_n105_));
  nand2  g059(.a(new_n50_), .b(new_n84_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x21), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n86_), .O(new_n109_));
  aoi21  g063(.a(new_n109_), .b(new_n107_), .c(new_n54_), .O(new_n110_));
  inv1   g064(.a(x21), .O(new_n111_));
  nor2   g065(.a(new_n111_), .b(new_n55_), .O(new_n112_));
  oai21  g066(.a(new_n112_), .b(new_n110_), .c(x16), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n105_), .O(z11));
  nand2  g068(.a(x18), .b(x03), .O(new_n115_));
  oai21  g069(.a(x18), .b(new_n61_), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n88_), .O(new_n117_));
  oai21  g071(.a(new_n63_), .b(new_n88_), .c(x20), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n54_), .O(new_n119_));
  aoi21  g073(.a(new_n108_), .b(new_n86_), .c(new_n63_), .O(new_n120_));
  nand2  g074(.a(new_n63_), .b(new_n111_), .O(new_n121_));
  nor3   g075(.a(new_n121_), .b(new_n106_), .c(x20), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n120_), .c(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n119_), .c(new_n117_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(new_n95_), .b(new_n66_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n88_), .O(new_n127_));
  oai21  g081(.a(new_n121_), .b(new_n106_), .c(x23), .O(new_n128_));
  nor3   g082(.a(x20), .b(x19), .c(x17), .O(new_n129_));
  nor3   g083(.a(x23), .b(x22), .c(x21), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n128_), .c(new_n54_), .O(new_n132_));
  inv1   g086(.a(x23), .O(new_n133_));
  nor2   g087(.a(new_n133_), .b(new_n55_), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n127_), .O(z13));
  nand2  g090(.a(x18), .b(x05), .O(new_n137_));
  oai21  g091(.a(x18), .b(new_n69_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n88_), .O(new_n139_));
  oai21  g093(.a(new_n71_), .b(new_n88_), .c(x20), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n54_), .O(new_n141_));
  aoi21  g095(.a(new_n130_), .b(new_n129_), .c(new_n71_), .O(new_n142_));
  nand3  g096(.a(new_n55_), .b(new_n50_), .c(new_n84_), .O(new_n143_));
  nand4  g097(.a(new_n71_), .b(new_n133_), .c(new_n63_), .d(new_n111_), .O(new_n144_));
  nor2   g098(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  oai21  g099(.a(new_n145_), .b(new_n142_), .c(x16), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n141_), .c(new_n139_), .O(z14));
  nand2  g101(.a(x18), .b(x06), .O(new_n148_));
  oai21  g102(.a(x18), .b(new_n74_), .c(new_n148_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n88_), .O(new_n150_));
  inv1   g104(.a(x25), .O(new_n151_));
  oai21  g105(.a(new_n151_), .b(new_n88_), .c(x20), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n54_), .O(new_n153_));
  nor2   g107(.a(x23), .b(x22), .O(new_n154_));
  nor2   g108(.a(x25), .b(x24), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  oai22  g110(.a(new_n156_), .b(new_n109_), .c(new_n145_), .d(new_n151_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(x16), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n153_), .c(new_n150_), .O(z15));
  nand2  g113(.a(x18), .b(x07), .O(new_n160_));
  oai21  g114(.a(new_n95_), .b(new_n77_), .c(new_n160_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n88_), .O(new_n162_));
  nor2   g116(.a(x21), .b(x19), .O(new_n163_));
  nand4  g117(.a(new_n163_), .b(new_n155_), .c(new_n154_), .d(new_n84_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(x26), .O(new_n165_));
  nor3   g119(.a(x26), .b(x25), .c(x24), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n154_), .c(new_n108_), .d(new_n86_), .O(new_n167_));
  aoi21  g121(.a(new_n167_), .b(new_n165_), .c(new_n54_), .O(new_n168_));
  nor2   g122(.a(new_n79_), .b(new_n55_), .O(new_n169_));
  oai21  g123(.a(new_n169_), .b(new_n168_), .c(x16), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(new_n162_), .O(z16));
  nand3  g125(.a(new_n82_), .b(x27), .c(x17), .O(new_n172_));
  nand2  g126(.a(new_n108_), .b(x19), .O(new_n173_));
  inv1   g127(.a(new_n173_), .O(new_n174_));
  nand4  g128(.a(new_n174_), .b(new_n166_), .c(new_n154_), .d(new_n98_), .O(new_n175_));
  aoi21  g129(.a(new_n175_), .b(new_n172_), .c(new_n88_), .O(z17));
endmodule


