// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x21), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x16), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  aoi21  g011(.a(x27), .b(x16), .c(new_n47_), .O(new_n58_));
  oai22  g012(.a(new_n58_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g013(.a(x11), .O(new_n60_));
  nand2  g014(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x22), .O(new_n62_));
  nand2  g016(.a(new_n52_), .b(new_n62_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x23), .O(new_n67_));
  nand2  g021(.a(new_n52_), .b(new_n67_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n66_), .c(new_n48_), .O(z04));
  inv1   g023(.a(x13), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x24), .O(new_n72_));
  nand2  g026(.a(new_n52_), .b(new_n72_), .O(new_n73_));
  aoi21  g027(.a(new_n73_), .b(new_n71_), .c(new_n48_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  nand2  g031(.a(new_n52_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n48_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  aoi21  g034(.a(x27), .b(new_n80_), .c(new_n48_), .O(new_n81_));
  oai21  g035(.a(x27), .b(x26), .c(new_n81_), .O(z07));
  inv1   g036(.a(new_n48_), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n52_), .O(z08));
  inv1   g038(.a(x17), .O(new_n85_));
  nor2   g039(.a(new_n51_), .b(new_n85_), .O(new_n86_));
  nor2   g040(.a(x19), .b(x17), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n86_), .c(x16), .O(new_n88_));
  oai21  g042(.a(x18), .b(new_n49_), .c(new_n47_), .O(new_n89_));
  aoi21  g043(.a(x18), .b(x00), .c(new_n89_), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(x16), .c(new_n88_), .O(z09));
  inv1   g045(.a(x20), .O(new_n92_));
  nor2   g046(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  nor3   g047(.a(x20), .b(x19), .c(x17), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n93_), .c(x16), .O(new_n95_));
  oai21  g049(.a(x18), .b(new_n55_), .c(new_n47_), .O(new_n96_));
  aoi21  g050(.a(x18), .b(x01), .c(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(x16), .c(new_n95_), .O(z10));
  inv1   g052(.a(x16), .O(new_n99_));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  inv1   g054(.a(x18), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x10), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n100_), .c(new_n47_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  nor2   g058(.a(x20), .b(x19), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n85_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x21), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand4  g062(.a(new_n108_), .b(new_n51_), .c(new_n85_), .d(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n107_), .c(new_n104_), .O(z11));
  oai21  g064(.a(x22), .b(new_n99_), .c(x21), .O(new_n111_));
  nand2  g065(.a(x18), .b(x03), .O(new_n112_));
  oai21  g066(.a(x18), .b(new_n60_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n99_), .O(new_n114_));
  nor2   g068(.a(x22), .b(x21), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n87_), .c(new_n92_), .O(new_n116_));
  oai21  g070(.a(new_n94_), .b(new_n62_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x16), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n114_), .c(new_n111_), .O(z12));
  oai21  g073(.a(x23), .b(new_n99_), .c(x21), .O(new_n120_));
  nand2  g074(.a(x18), .b(x04), .O(new_n121_));
  oai21  g075(.a(x18), .b(new_n65_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n99_), .O(new_n123_));
  nor2   g077(.a(x22), .b(x20), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n87_), .c(new_n67_), .O(new_n125_));
  nor2   g079(.a(x23), .b(x22), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  nor2   g081(.a(new_n127_), .b(new_n106_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n125_), .c(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n123_), .c(new_n120_), .O(z13));
  oai21  g084(.a(x24), .b(new_n99_), .c(x21), .O(new_n131_));
  nand2  g085(.a(x18), .b(x05), .O(new_n132_));
  oai21  g086(.a(x18), .b(new_n70_), .c(new_n132_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n99_), .O(new_n134_));
  nand3  g088(.a(new_n126_), .b(new_n87_), .c(new_n92_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n115_), .b(new_n94_), .c(new_n72_), .d(new_n67_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n134_), .c(new_n131_), .O(z14));
  oai21  g094(.a(x25), .b(new_n99_), .c(x21), .O(new_n141_));
  nand2  g095(.a(x18), .b(x06), .O(new_n142_));
  oai21  g096(.a(x18), .b(new_n75_), .c(new_n142_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n99_), .O(new_n144_));
  nor3   g098(.a(x24), .b(x23), .c(x22), .O(new_n145_));
  aoi21  g099(.a(new_n145_), .b(new_n94_), .c(new_n77_), .O(new_n146_));
  nand2  g100(.a(new_n108_), .b(new_n87_), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n126_), .O(new_n149_));
  nor2   g103(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n146_), .c(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n144_), .c(new_n141_), .O(z15));
  oai21  g106(.a(x26), .b(new_n99_), .c(x21), .O(new_n153_));
  nand2  g107(.a(x18), .b(x07), .O(new_n154_));
  oai21  g108(.a(x18), .b(new_n80_), .c(new_n154_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n99_), .O(new_n156_));
  nand4  g110(.a(new_n148_), .b(new_n126_), .c(new_n105_), .d(new_n85_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(x26), .O(new_n158_));
  nor3   g112(.a(x26), .b(x25), .c(x24), .O(new_n159_));
  nand4  g113(.a(new_n159_), .b(new_n126_), .c(new_n108_), .d(new_n87_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x16), .O(new_n162_));
  nand3  g116(.a(new_n162_), .b(new_n156_), .c(new_n153_), .O(z16));
  nor2   g117(.a(x26), .b(x25), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n126_), .c(new_n72_), .O(new_n165_));
  nand3  g119(.a(new_n108_), .b(x19), .c(new_n85_), .O(new_n166_));
  oai22  g120(.a(new_n166_), .b(new_n165_), .c(new_n52_), .d(new_n85_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x16), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n83_), .O(z17));
endmodule


