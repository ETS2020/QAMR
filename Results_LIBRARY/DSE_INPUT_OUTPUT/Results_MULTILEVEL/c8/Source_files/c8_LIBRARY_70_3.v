// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_;
  nand2  g000(.a(x22), .b(x18), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n52_), .b(new_n60_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n59_), .c(new_n48_), .O(z02));
  inv1   g016(.a(x18), .O(new_n63_));
  inv1   g017(.a(x22), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  oai22  g019(.a(new_n65_), .b(x11), .c(x27), .d(x22), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n48_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x23), .c(new_n68_), .O(z04));
  inv1   g023(.a(x13), .O(new_n70_));
  aoi21  g024(.a(x27), .b(new_n70_), .c(new_n48_), .O(new_n71_));
  oai21  g025(.a(x27), .b(x24), .c(new_n71_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  aoi21  g027(.a(x27), .b(new_n73_), .c(new_n48_), .O(new_n74_));
  oai21  g028(.a(x27), .b(x25), .c(new_n74_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n48_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x26), .c(new_n77_), .O(z07));
  nor2   g032(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g033(.a(x17), .O(new_n80_));
  nor2   g034(.a(new_n51_), .b(new_n80_), .O(new_n81_));
  nor2   g035(.a(x19), .b(x17), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n81_), .c(x16), .O(new_n83_));
  inv1   g037(.a(x16), .O(new_n84_));
  nand2  g038(.a(x18), .b(x00), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n49_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n83_), .c(new_n47_), .O(z09));
  nand2  g042(.a(x18), .b(x01), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n55_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n84_), .O(new_n91_));
  inv1   g045(.a(x20), .O(new_n92_));
  nor2   g046(.a(new_n82_), .b(new_n92_), .O(new_n93_));
  nor3   g047(.a(x20), .b(x19), .c(x17), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n93_), .c(x16), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n91_), .c(new_n47_), .O(z10));
  nand2  g050(.a(x18), .b(x02), .O(new_n97_));
  oai21  g051(.a(x18), .b(new_n58_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n84_), .O(new_n99_));
  nor2   g053(.a(x21), .b(x20), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n82_), .O(new_n101_));
  oai21  g055(.a(new_n94_), .b(new_n60_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x16), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n99_), .c(new_n47_), .O(z11));
  inv1   g058(.a(x11), .O(new_n105_));
  nand2  g059(.a(x18), .b(x03), .O(new_n106_));
  oai21  g060(.a(x18), .b(new_n105_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n84_), .O(new_n108_));
  inv1   g062(.a(new_n101_), .O(new_n109_));
  nor2   g063(.a(new_n109_), .b(new_n64_), .O(new_n110_));
  nor3   g064(.a(x22), .b(x21), .c(x20), .O(new_n111_));
  and2   g065(.a(new_n111_), .b(new_n82_), .O(new_n112_));
  oai21  g066(.a(new_n112_), .b(new_n110_), .c(x16), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n108_), .c(new_n47_), .O(z12));
  nand2  g068(.a(x18), .b(x04), .O(new_n115_));
  oai21  g069(.a(x18), .b(new_n67_), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n84_), .O(new_n117_));
  inv1   g071(.a(x23), .O(new_n118_));
  oai21  g072(.a(new_n118_), .b(new_n84_), .c(new_n63_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x22), .O(new_n120_));
  nor2   g074(.a(new_n109_), .b(new_n118_), .O(new_n121_));
  nand3  g075(.a(new_n92_), .b(new_n51_), .c(new_n80_), .O(new_n122_));
  nor2   g076(.a(x23), .b(x22), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n60_), .O(new_n124_));
  nor2   g078(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n121_), .c(x16), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n120_), .c(new_n117_), .O(z13));
  nand3  g081(.a(new_n64_), .b(x18), .c(x05), .O(new_n128_));
  oai21  g082(.a(x18), .b(new_n70_), .c(new_n128_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n84_), .O(new_n130_));
  nor2   g084(.a(x23), .b(x21), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n82_), .c(new_n92_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x24), .O(new_n133_));
  nor3   g087(.a(x24), .b(x23), .c(x21), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n94_), .O(new_n135_));
  aoi21  g089(.a(new_n135_), .b(new_n133_), .c(x22), .O(new_n136_));
  nand2  g090(.a(x24), .b(x22), .O(new_n137_));
  nor2   g091(.a(new_n137_), .b(x18), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n130_), .O(z14));
  nand2  g094(.a(x18), .b(x06), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n73_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n84_), .O(new_n143_));
  inv1   g097(.a(x25), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n84_), .c(new_n63_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x22), .O(new_n146_));
  aoi21  g100(.a(new_n134_), .b(new_n94_), .c(new_n144_), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n123_), .O(new_n149_));
  nor2   g103(.a(new_n149_), .b(new_n101_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n147_), .c(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n146_), .c(new_n143_), .O(z15));
  nand2  g106(.a(x18), .b(x07), .O(new_n153_));
  oai21  g107(.a(x18), .b(new_n76_), .c(new_n153_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n84_), .O(new_n155_));
  nand2  g109(.a(x26), .b(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n63_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(x22), .O(new_n158_));
  nand2  g112(.a(new_n148_), .b(new_n131_), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n122_), .c(x26), .O(new_n160_));
  nor3   g114(.a(x26), .b(x25), .c(x24), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n123_), .c(new_n109_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n158_), .c(new_n155_), .O(z16));
  nand2  g119(.a(z08), .b(x17), .O(new_n166_));
  nor2   g120(.a(new_n51_), .b(x17), .O(new_n167_));
  nand4  g121(.a(new_n167_), .b(new_n161_), .c(new_n123_), .d(new_n100_), .O(new_n168_));
  aoi21  g122(.a(new_n168_), .b(new_n166_), .c(new_n84_), .O(z17));
endmodule


