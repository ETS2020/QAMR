// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_;
  inv1   g000(.a(x12), .O(new_n47_));
  nand2  g001(.a(x18), .b(new_n47_), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n53_), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x09), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  nand3  g013(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z01));
  inv1   g014(.a(x21), .O(new_n61_));
  nand2  g015(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x10), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  nand3  g018(.a(new_n64_), .b(new_n62_), .c(new_n48_), .O(z02));
  inv1   g019(.a(x11), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x22), .O(new_n68_));
  nand2  g022(.a(new_n53_), .b(new_n68_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n67_), .c(new_n49_), .O(z03));
  inv1   g024(.a(x18), .O(new_n71_));
  nand2  g025(.a(new_n53_), .b(x23), .O(new_n72_));
  nand3  g026(.a(new_n72_), .b(new_n71_), .c(new_n47_), .O(new_n73_));
  inv1   g027(.a(x23), .O(new_n74_));
  nand3  g028(.a(new_n53_), .b(new_n74_), .c(x12), .O(new_n75_));
  nand2  g029(.a(new_n75_), .b(new_n73_), .O(z04));
  inv1   g030(.a(x24), .O(new_n77_));
  nand2  g031(.a(new_n53_), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x13), .O(new_n79_));
  nand2  g033(.a(x27), .b(new_n79_), .O(new_n80_));
  nand3  g034(.a(new_n80_), .b(new_n78_), .c(new_n48_), .O(z05));
  inv1   g035(.a(x14), .O(new_n82_));
  nand2  g036(.a(x27), .b(new_n82_), .O(new_n83_));
  inv1   g037(.a(x25), .O(new_n84_));
  nand2  g038(.a(new_n53_), .b(new_n84_), .O(new_n85_));
  aoi21  g039(.a(new_n85_), .b(new_n83_), .c(new_n49_), .O(z06));
  inv1   g040(.a(x26), .O(new_n87_));
  nand2  g041(.a(new_n53_), .b(new_n87_), .O(new_n88_));
  inv1   g042(.a(x15), .O(new_n89_));
  nand2  g043(.a(x27), .b(new_n89_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n88_), .c(new_n48_), .O(z07));
  nand2  g045(.a(new_n48_), .b(new_n53_), .O(z08));
  inv1   g046(.a(x16), .O(new_n93_));
  inv1   g047(.a(x00), .O(new_n94_));
  nand2  g048(.a(x18), .b(x12), .O(new_n95_));
  oai22  g049(.a(new_n95_), .b(new_n94_), .c(x18), .d(new_n50_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  xnor2a g051(.a(x19), .b(x17), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n48_), .c(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n97_), .O(z09));
  nand2  g054(.a(x18), .b(x01), .O(new_n101_));
  oai21  g055(.a(x18), .b(new_n58_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n93_), .O(new_n103_));
  nor2   g057(.a(x19), .b(x17), .O(new_n104_));
  nor2   g058(.a(new_n104_), .b(new_n56_), .O(new_n105_));
  nor3   g059(.a(x20), .b(x19), .c(x17), .O(new_n106_));
  oai21  g060(.a(new_n106_), .b(new_n105_), .c(x16), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n103_), .c(new_n48_), .O(z10));
  inv1   g062(.a(x02), .O(new_n109_));
  oai22  g063(.a(new_n95_), .b(new_n109_), .c(x18), .d(new_n63_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n93_), .O(new_n111_));
  nor2   g065(.a(x21), .b(x20), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n104_), .O(new_n113_));
  oai21  g067(.a(new_n106_), .b(new_n61_), .c(new_n113_), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n48_), .c(x16), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n111_), .O(z11));
  inv1   g070(.a(x03), .O(new_n117_));
  oai22  g071(.a(new_n95_), .b(new_n117_), .c(x18), .d(new_n66_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n93_), .O(new_n119_));
  nand2  g073(.a(new_n113_), .b(x22), .O(new_n120_));
  nor2   g074(.a(x22), .b(x21), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n104_), .c(new_n56_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n48_), .c(x16), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n119_), .O(z12));
  inv1   g079(.a(x04), .O(new_n126_));
  aoi21  g080(.a(x18), .b(new_n126_), .c(x16), .O(new_n127_));
  nand2  g081(.a(new_n122_), .b(x23), .O(new_n128_));
  nor3   g082(.a(x23), .b(x22), .c(x21), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n106_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(x16), .c(new_n127_), .O(new_n132_));
  nand3  g086(.a(new_n131_), .b(new_n71_), .c(x16), .O(new_n133_));
  oai21  g087(.a(new_n132_), .b(new_n47_), .c(new_n133_), .O(z13));
  nand2  g088(.a(x18), .b(x05), .O(new_n135_));
  oai21  g089(.a(x18), .b(new_n79_), .c(new_n135_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n93_), .O(new_n137_));
  aoi21  g091(.a(new_n129_), .b(new_n106_), .c(new_n77_), .O(new_n138_));
  inv1   g092(.a(x17), .O(new_n139_));
  nand3  g093(.a(new_n56_), .b(new_n52_), .c(new_n139_), .O(new_n140_));
  nand3  g094(.a(new_n121_), .b(new_n77_), .c(new_n74_), .O(new_n141_));
  nor2   g095(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n138_), .c(x16), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n137_), .c(new_n48_), .O(z14));
  nand2  g098(.a(x18), .b(x06), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n82_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n93_), .O(new_n147_));
  nor2   g101(.a(x23), .b(x22), .O(new_n148_));
  nor2   g102(.a(x25), .b(x24), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n148_), .c(new_n112_), .d(new_n104_), .O(new_n150_));
  oai21  g104(.a(new_n142_), .b(new_n84_), .c(new_n150_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x16), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n147_), .c(new_n48_), .O(z15));
  nand2  g107(.a(x18), .b(x07), .O(new_n154_));
  oai21  g108(.a(x18), .b(new_n89_), .c(new_n154_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n93_), .O(new_n156_));
  nand2  g110(.a(new_n150_), .b(x26), .O(new_n157_));
  nand4  g111(.a(new_n148_), .b(new_n87_), .c(new_n84_), .d(new_n77_), .O(new_n158_));
  oai21  g112(.a(new_n158_), .b(new_n113_), .c(new_n157_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(x16), .O(new_n160_));
  nand3  g114(.a(new_n160_), .b(new_n156_), .c(new_n48_), .O(z16));
  nand3  g115(.a(new_n112_), .b(x19), .c(new_n139_), .O(new_n162_));
  oai22  g116(.a(new_n162_), .b(new_n158_), .c(new_n53_), .d(new_n139_), .O(new_n163_));
  nand3  g117(.a(new_n163_), .b(new_n48_), .c(x16), .O(new_n164_));
  inv1   g118(.a(new_n164_), .O(z17));
endmodule


