// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x19), .O(new_n47_));
  inv1   g001(.a(x24), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n48_), .c(new_n47_), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  nand2  g005(.a(x24), .b(x19), .O(new_n52_));
  inv1   g006(.a(new_n52_), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x21), .O(new_n58_));
  inv1   g012(.a(x27), .O(new_n59_));
  nand2  g013(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n57_), .c(new_n53_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  inv1   g017(.a(x22), .O(new_n64_));
  nand2  g018(.a(new_n59_), .b(new_n64_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n63_), .c(new_n53_), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n53_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x23), .c(new_n68_), .O(z04));
  aoi21  g023(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n70_));
  oai22  g024(.a(new_n70_), .b(x13), .c(x27), .d(x24), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x25), .O(new_n74_));
  nand2  g028(.a(new_n59_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n53_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n53_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x26), .c(new_n78_), .O(z07));
  nor2   g033(.a(new_n53_), .b(new_n59_), .O(z08));
  inv1   g034(.a(x16), .O(new_n81_));
  inv1   g035(.a(x08), .O(new_n82_));
  nand2  g036(.a(x18), .b(x00), .O(new_n83_));
  oai21  g037(.a(x18), .b(new_n82_), .c(new_n83_), .O(new_n84_));
  nand3  g038(.a(new_n84_), .b(new_n52_), .c(new_n81_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  nand3  g040(.a(new_n48_), .b(x19), .c(x17), .O(new_n87_));
  inv1   g041(.a(new_n87_), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n86_), .c(x16), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n85_), .O(z09));
  nand2  g044(.a(x18), .b(x01), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n51_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n81_), .O(new_n93_));
  inv1   g047(.a(x20), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n81_), .c(new_n48_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(x19), .O(new_n96_));
  inv1   g050(.a(x17), .O(new_n97_));
  nor2   g051(.a(new_n94_), .b(new_n97_), .O(new_n98_));
  nor3   g052(.a(x20), .b(x19), .c(x17), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n98_), .c(x16), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n96_), .c(new_n93_), .O(z10));
  nand2  g055(.a(x18), .b(x02), .O(new_n102_));
  oai21  g056(.a(x18), .b(new_n56_), .c(new_n102_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n52_), .c(new_n81_), .O(new_n104_));
  oai21  g058(.a(x20), .b(x17), .c(x21), .O(new_n105_));
  nor2   g059(.a(x21), .b(x20), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n97_), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n105_), .c(x19), .O(new_n108_));
  nand3  g062(.a(new_n48_), .b(x21), .c(x19), .O(new_n109_));
  inv1   g063(.a(new_n109_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n104_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n62_), .c(new_n113_), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n52_), .c(new_n81_), .O(new_n115_));
  nor3   g069(.a(x21), .b(x20), .c(x17), .O(new_n116_));
  nand4  g070(.a(new_n64_), .b(new_n58_), .c(new_n94_), .d(new_n97_), .O(new_n117_));
  oai21  g071(.a(new_n116_), .b(new_n64_), .c(new_n117_), .O(new_n118_));
  nand3  g072(.a(new_n48_), .b(x22), .c(x19), .O(new_n119_));
  inv1   g073(.a(new_n119_), .O(new_n120_));
  aoi21  g074(.a(new_n118_), .b(new_n47_), .c(new_n120_), .O(new_n121_));
  oai21  g075(.a(new_n121_), .b(new_n81_), .c(new_n115_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n67_), .c(new_n123_), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n52_), .c(new_n81_), .O(new_n125_));
  nand2  g079(.a(new_n117_), .b(x23), .O(new_n126_));
  nor2   g080(.a(x23), .b(x22), .O(new_n127_));
  nand4  g081(.a(new_n127_), .b(new_n58_), .c(new_n94_), .d(new_n97_), .O(new_n128_));
  aoi21  g082(.a(new_n128_), .b(new_n126_), .c(x19), .O(new_n129_));
  nand3  g083(.a(new_n48_), .b(x23), .c(x19), .O(new_n130_));
  inv1   g084(.a(new_n130_), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n129_), .c(x16), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n125_), .O(z13));
  inv1   g087(.a(x13), .O(new_n134_));
  nand2  g088(.a(x18), .b(x05), .O(new_n135_));
  oai21  g089(.a(x18), .b(new_n134_), .c(new_n135_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n81_), .O(new_n137_));
  nand2  g091(.a(new_n128_), .b(x24), .O(new_n138_));
  nor2   g092(.a(x22), .b(x21), .O(new_n139_));
  nor2   g093(.a(x24), .b(x23), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n139_), .c(new_n99_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x16), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n137_), .c(new_n52_), .O(z14));
  nand2  g098(.a(x18), .b(x06), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n72_), .c(new_n145_), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n52_), .c(new_n81_), .O(new_n147_));
  nand2  g101(.a(x25), .b(x19), .O(new_n148_));
  nor2   g102(.a(x20), .b(x19), .O(new_n149_));
  nor2   g103(.a(x25), .b(x23), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n139_), .c(new_n149_), .d(new_n97_), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n148_), .c(x24), .O(new_n152_));
  nand4  g106(.a(new_n140_), .b(new_n106_), .c(new_n64_), .d(new_n97_), .O(new_n153_));
  nand3  g107(.a(new_n153_), .b(x25), .c(new_n47_), .O(new_n154_));
  inv1   g108(.a(new_n154_), .O(new_n155_));
  oai21  g109(.a(new_n155_), .b(new_n152_), .c(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n147_), .O(z15));
  nand2  g111(.a(x18), .b(x07), .O(new_n158_));
  oai21  g112(.a(x18), .b(new_n77_), .c(new_n158_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n81_), .O(new_n160_));
  inv1   g114(.a(x26), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(new_n81_), .c(new_n48_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x19), .O(new_n163_));
  nand3  g117(.a(new_n127_), .b(new_n74_), .c(new_n48_), .O(new_n164_));
  oai21  g118(.a(new_n164_), .b(new_n107_), .c(x26), .O(new_n165_));
  nor3   g119(.a(x26), .b(x25), .c(x24), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n127_), .c(new_n106_), .d(new_n86_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n165_), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(x16), .O(new_n169_));
  nand3  g123(.a(new_n169_), .b(new_n163_), .c(new_n160_), .O(z16));
  nand3  g124(.a(new_n106_), .b(x19), .c(new_n97_), .O(new_n171_));
  nand3  g125(.a(new_n127_), .b(new_n161_), .c(new_n74_), .O(new_n172_));
  oai22  g126(.a(new_n172_), .b(new_n171_), .c(new_n59_), .d(new_n97_), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(x16), .O(new_n174_));
  nand2  g128(.a(new_n174_), .b(new_n52_), .O(z17));
endmodule


