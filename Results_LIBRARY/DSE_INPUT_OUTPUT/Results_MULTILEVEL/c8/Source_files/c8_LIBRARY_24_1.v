// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_;
  inv1   g000(.a(x18), .O(new_n47_));
  nand2  g001(.a(x22), .b(new_n47_), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x09), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x20), .O(new_n58_));
  nand2  g012(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n49_), .O(z01));
  inv1   g014(.a(x10), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x21), .O(new_n63_));
  nand2  g017(.a(new_n53_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(z02));
  inv1   g019(.a(x22), .O(new_n66_));
  aoi21  g020(.a(x27), .b(x18), .c(new_n66_), .O(new_n67_));
  oai22  g021(.a(new_n67_), .b(x11), .c(x27), .d(x22), .O(z03));
  inv1   g022(.a(x23), .O(new_n69_));
  nand2  g023(.a(new_n53_), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x12), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  nand3  g026(.a(new_n72_), .b(new_n70_), .c(new_n48_), .O(z04));
  inv1   g027(.a(x13), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x24), .O(new_n76_));
  nand2  g030(.a(new_n53_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n49_), .O(z05));
  inv1   g032(.a(x25), .O(new_n79_));
  nand2  g033(.a(new_n53_), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x14), .O(new_n81_));
  nand2  g035(.a(x27), .b(new_n81_), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n80_), .c(new_n48_), .O(z06));
  inv1   g037(.a(x26), .O(new_n84_));
  nand2  g038(.a(new_n53_), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(x15), .O(new_n86_));
  nand2  g040(.a(x27), .b(new_n86_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n85_), .c(new_n48_), .O(z07));
  nor2   g042(.a(new_n49_), .b(new_n53_), .O(z08));
  inv1   g043(.a(x17), .O(new_n90_));
  nor2   g044(.a(new_n52_), .b(new_n90_), .O(new_n91_));
  nor2   g045(.a(x19), .b(x17), .O(new_n92_));
  oai21  g046(.a(new_n92_), .b(new_n91_), .c(x16), .O(new_n93_));
  inv1   g047(.a(x16), .O(new_n94_));
  nand2  g048(.a(x18), .b(x00), .O(new_n95_));
  oai21  g049(.a(x18), .b(new_n50_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n93_), .c(new_n48_), .O(z09));
  nand2  g052(.a(x18), .b(x01), .O(new_n99_));
  oai21  g053(.a(x18), .b(new_n56_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n94_), .O(new_n101_));
  nor2   g055(.a(new_n92_), .b(new_n58_), .O(new_n102_));
  nor3   g056(.a(x20), .b(x19), .c(x17), .O(new_n103_));
  oai21  g057(.a(new_n103_), .b(new_n102_), .c(x16), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n101_), .c(new_n48_), .O(z10));
  nand2  g059(.a(x18), .b(x02), .O(new_n106_));
  oai21  g060(.a(x18), .b(new_n61_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n94_), .O(new_n108_));
  nand3  g062(.a(new_n58_), .b(new_n52_), .c(new_n90_), .O(new_n109_));
  nand2  g063(.a(x22), .b(new_n47_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n109_), .c(x21), .O(new_n111_));
  nor2   g065(.a(x21), .b(x20), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n92_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x16), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n108_), .c(new_n48_), .O(z11));
  nor2   g070(.a(x22), .b(x18), .O(new_n117_));
  aoi22  g071(.a(new_n117_), .b(x11), .c(x18), .d(x03), .O(new_n118_));
  nand3  g072(.a(new_n113_), .b(x22), .c(x18), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n92_), .c(new_n58_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  oai21  g077(.a(new_n118_), .b(x16), .c(new_n123_), .O(z12));
  inv1   g078(.a(new_n117_), .O(new_n125_));
  nand2  g079(.a(x18), .b(x04), .O(new_n126_));
  oai21  g080(.a(new_n125_), .b(new_n71_), .c(new_n126_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n94_), .O(new_n128_));
  nand2  g082(.a(new_n113_), .b(x23), .O(new_n129_));
  nor2   g083(.a(x23), .b(x21), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n92_), .c(new_n58_), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n129_), .c(x22), .O(new_n132_));
  nand3  g086(.a(x23), .b(x22), .c(x18), .O(new_n133_));
  inv1   g087(.a(new_n133_), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n128_), .O(z13));
  nand2  g090(.a(x18), .b(x05), .O(new_n137_));
  oai21  g091(.a(x18), .b(new_n74_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n94_), .O(new_n139_));
  oai21  g093(.a(new_n76_), .b(new_n94_), .c(x18), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x22), .O(new_n141_));
  nand2  g095(.a(new_n131_), .b(x24), .O(new_n142_));
  nand4  g096(.a(new_n120_), .b(new_n103_), .c(new_n76_), .d(new_n69_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x16), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n141_), .c(new_n139_), .O(z14));
  nand2  g100(.a(x18), .b(x06), .O(new_n147_));
  oai21  g101(.a(new_n125_), .b(new_n81_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n94_), .O(new_n149_));
  nand3  g103(.a(new_n76_), .b(new_n69_), .c(new_n63_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n109_), .c(x25), .O(new_n151_));
  nor2   g105(.a(x25), .b(x24), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n130_), .c(new_n103_), .O(new_n153_));
  aoi21  g107(.a(new_n153_), .b(new_n151_), .c(x22), .O(new_n154_));
  nand3  g108(.a(x25), .b(x22), .c(x18), .O(new_n155_));
  inv1   g109(.a(new_n155_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n154_), .c(x16), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n149_), .O(z15));
  nand2  g112(.a(x18), .b(x07), .O(new_n159_));
  oai21  g113(.a(x18), .b(new_n86_), .c(new_n159_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n94_), .O(new_n161_));
  oai21  g115(.a(new_n84_), .b(new_n94_), .c(x18), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x22), .O(new_n163_));
  nand2  g117(.a(new_n153_), .b(x26), .O(new_n164_));
  nor2   g118(.a(x23), .b(x22), .O(new_n165_));
  nor3   g119(.a(x26), .b(x25), .c(x24), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n165_), .c(new_n112_), .d(new_n92_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(x16), .O(new_n169_));
  nand3  g123(.a(new_n169_), .b(new_n163_), .c(new_n161_), .O(z16));
  nand2  g124(.a(z08), .b(x17), .O(new_n171_));
  nor2   g125(.a(new_n52_), .b(x17), .O(new_n172_));
  nand4  g126(.a(new_n172_), .b(new_n166_), .c(new_n165_), .d(new_n112_), .O(new_n173_));
  aoi21  g127(.a(new_n173_), .b(new_n171_), .c(new_n94_), .O(z17));
endmodule


