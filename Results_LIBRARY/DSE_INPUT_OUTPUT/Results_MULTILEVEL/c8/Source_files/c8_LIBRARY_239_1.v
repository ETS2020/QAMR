// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x19), .O(new_n47_));
  inv1   g001(.a(x27), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x18), .O(new_n52_));
  nand2  g006(.a(x21), .b(new_n52_), .O(new_n53_));
  nand3  g007(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g008(.a(x20), .O(new_n55_));
  nand2  g009(.a(new_n48_), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x09), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  nand3  g012(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(z01));
  inv1   g013(.a(x21), .O(new_n60_));
  aoi21  g014(.a(x27), .b(x18), .c(new_n60_), .O(new_n61_));
  oai22  g015(.a(new_n61_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g016(.a(new_n53_), .O(new_n63_));
  inv1   g017(.a(x11), .O(new_n64_));
  nand2  g018(.a(x27), .b(new_n64_), .O(new_n65_));
  inv1   g019(.a(x22), .O(new_n66_));
  nand2  g020(.a(new_n48_), .b(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n65_), .c(new_n63_), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x23), .O(new_n71_));
  nand2  g025(.a(new_n48_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n63_), .O(z04));
  inv1   g027(.a(x13), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x24), .O(new_n76_));
  nand2  g030(.a(new_n48_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n63_), .O(z05));
  inv1   g032(.a(x25), .O(new_n79_));
  nand2  g033(.a(new_n48_), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x14), .O(new_n81_));
  nand2  g035(.a(x27), .b(new_n81_), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n80_), .c(new_n53_), .O(z06));
  inv1   g037(.a(x15), .O(new_n84_));
  nand2  g038(.a(x27), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(x26), .O(new_n86_));
  nand2  g040(.a(new_n48_), .b(new_n86_), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n85_), .c(new_n63_), .O(z07));
  nand2  g042(.a(new_n53_), .b(new_n48_), .O(z08));
  inv1   g043(.a(x17), .O(new_n90_));
  nor2   g044(.a(new_n47_), .b(new_n90_), .O(new_n91_));
  nor2   g045(.a(x19), .b(x17), .O(new_n92_));
  oai21  g046(.a(new_n92_), .b(new_n91_), .c(x16), .O(new_n93_));
  inv1   g047(.a(x16), .O(new_n94_));
  nand2  g048(.a(x18), .b(x00), .O(new_n95_));
  oai21  g049(.a(x18), .b(new_n50_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n93_), .c(new_n53_), .O(z09));
  nand2  g052(.a(x18), .b(x01), .O(new_n99_));
  oai21  g053(.a(x18), .b(new_n57_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n94_), .O(new_n101_));
  nor2   g055(.a(new_n92_), .b(new_n55_), .O(new_n102_));
  nor3   g056(.a(x20), .b(x19), .c(x17), .O(new_n103_));
  oai21  g057(.a(new_n103_), .b(new_n102_), .c(x16), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n101_), .c(new_n53_), .O(z10));
  nor2   g059(.a(x21), .b(x18), .O(new_n106_));
  aoi22  g060(.a(new_n106_), .b(x10), .c(x18), .d(x02), .O(new_n107_));
  nand3  g061(.a(new_n55_), .b(new_n47_), .c(new_n90_), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(x21), .c(x18), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n92_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(x16), .O(new_n113_));
  oai21  g067(.a(new_n107_), .b(x16), .c(new_n113_), .O(z11));
  nand2  g068(.a(x18), .b(x03), .O(new_n115_));
  oai21  g069(.a(x18), .b(new_n64_), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n94_), .O(new_n117_));
  oai21  g071(.a(new_n66_), .b(new_n94_), .c(x18), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x21), .O(new_n119_));
  nand4  g073(.a(new_n92_), .b(new_n66_), .c(new_n60_), .d(new_n55_), .O(new_n120_));
  oai21  g074(.a(new_n103_), .b(new_n66_), .c(new_n120_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(x16), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n119_), .c(new_n117_), .O(z12));
  nand2  g077(.a(x18), .b(x04), .O(new_n124_));
  oai21  g078(.a(x18), .b(new_n69_), .c(new_n124_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n94_), .O(new_n126_));
  oai21  g080(.a(new_n71_), .b(new_n94_), .c(x18), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(x21), .O(new_n128_));
  nor2   g082(.a(x22), .b(x20), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n92_), .c(new_n71_), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n60_), .O(new_n132_));
  nor2   g086(.a(new_n132_), .b(new_n108_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n130_), .c(x16), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n128_), .c(new_n126_), .O(z13));
  inv1   g089(.a(new_n106_), .O(new_n136_));
  nand2  g090(.a(x18), .b(x05), .O(new_n137_));
  oai21  g091(.a(new_n136_), .b(new_n74_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n94_), .O(new_n139_));
  nand3  g093(.a(new_n131_), .b(new_n92_), .c(new_n55_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x24), .O(new_n141_));
  nor3   g095(.a(x24), .b(x23), .c(x22), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n103_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(x21), .O(new_n144_));
  nand3  g098(.a(x24), .b(x21), .c(x18), .O(new_n145_));
  inv1   g099(.a(new_n145_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n139_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(new_n136_), .b(new_n81_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n94_), .O(new_n151_));
  nand3  g105(.a(new_n76_), .b(new_n71_), .c(new_n66_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n108_), .c(x25), .O(new_n153_));
  nor2   g107(.a(x25), .b(x24), .O(new_n154_));
  nand3  g108(.a(new_n154_), .b(new_n131_), .c(new_n103_), .O(new_n155_));
  aoi21  g109(.a(new_n155_), .b(new_n153_), .c(x21), .O(new_n156_));
  nand3  g110(.a(x25), .b(x21), .c(x18), .O(new_n157_));
  inv1   g111(.a(new_n157_), .O(new_n158_));
  oai21  g112(.a(new_n158_), .b(new_n156_), .c(x16), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n151_), .O(z15));
  nand2  g114(.a(x18), .b(x07), .O(new_n161_));
  oai21  g115(.a(x18), .b(new_n84_), .c(new_n161_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n94_), .O(new_n163_));
  oai21  g117(.a(new_n86_), .b(new_n94_), .c(x18), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(x21), .O(new_n165_));
  nand2  g119(.a(new_n155_), .b(x26), .O(new_n166_));
  nor3   g120(.a(x26), .b(x25), .c(x24), .O(new_n167_));
  nand4  g121(.a(new_n167_), .b(new_n131_), .c(new_n110_), .d(new_n92_), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n166_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(x16), .O(new_n170_));
  nand3  g124(.a(new_n170_), .b(new_n165_), .c(new_n163_), .O(z16));
  nand3  g125(.a(new_n53_), .b(x27), .c(x17), .O(new_n172_));
  nor2   g126(.a(new_n47_), .b(x17), .O(new_n173_));
  nand4  g127(.a(new_n173_), .b(new_n167_), .c(new_n131_), .d(new_n110_), .O(new_n174_));
  aoi21  g128(.a(new_n174_), .b(new_n172_), .c(new_n94_), .O(z17));
endmodule


