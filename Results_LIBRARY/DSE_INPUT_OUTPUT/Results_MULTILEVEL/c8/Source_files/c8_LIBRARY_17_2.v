// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  oai21  g002(.a(new_n47_), .b(x18), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  inv1   g005(.a(x18), .O(new_n52_));
  nor2   g006(.a(x19), .b(new_n52_), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n56_), .c(new_n53_), .O(new_n57_));
  oai21  g011(.a(x27), .b(x21), .c(new_n57_), .O(z02));
  inv1   g012(.a(x11), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x22), .O(new_n61_));
  nand2  g015(.a(new_n47_), .b(new_n61_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n60_), .c(new_n53_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n53_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n47_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n53_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n53_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n53_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  nor2   g031(.a(new_n53_), .b(new_n47_), .O(z08));
  inv1   g032(.a(x17), .O(new_n79_));
  nor2   g033(.a(new_n48_), .b(new_n79_), .O(new_n80_));
  nor3   g034(.a(x19), .b(x18), .c(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  nor2   g036(.a(new_n48_), .b(new_n52_), .O(new_n83_));
  aoi22  g037(.a(new_n83_), .b(x00), .c(new_n52_), .d(x08), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(x16), .c(new_n82_), .O(z09));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(x18), .b(x01), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n51_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  inv1   g043(.a(x20), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n79_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n52_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  nor2   g047(.a(x19), .b(x17), .O(new_n94_));
  inv1   g048(.a(new_n94_), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(x20), .c(x16), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n93_), .c(new_n89_), .O(z10));
  aoi22  g051(.a(new_n83_), .b(x02), .c(new_n52_), .d(x10), .O(new_n98_));
  oai21  g052(.a(x20), .b(x17), .c(x21), .O(new_n99_));
  nor2   g053(.a(x21), .b(x20), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n94_), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(new_n99_), .c(x18), .O(new_n102_));
  inv1   g056(.a(x21), .O(new_n103_));
  nor2   g057(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n102_), .c(x16), .O(new_n105_));
  oai21  g059(.a(new_n98_), .b(x16), .c(new_n105_), .O(z11));
  nand2  g060(.a(x18), .b(x03), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n59_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n86_), .O(new_n109_));
  oai21  g063(.a(new_n61_), .b(new_n86_), .c(x19), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x18), .O(new_n111_));
  aoi21  g065(.a(new_n100_), .b(new_n94_), .c(new_n61_), .O(new_n112_));
  nand3  g066(.a(new_n61_), .b(new_n103_), .c(new_n90_), .O(new_n113_));
  nor2   g067(.a(new_n113_), .b(new_n95_), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n111_), .c(new_n109_), .O(z12));
  nand2  g070(.a(x18), .b(x04), .O(new_n117_));
  oai21  g071(.a(x18), .b(new_n64_), .c(new_n117_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n86_), .O(new_n119_));
  inv1   g073(.a(x23), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n86_), .c(x19), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(x18), .O(new_n122_));
  nand3  g076(.a(new_n90_), .b(new_n48_), .c(new_n79_), .O(new_n123_));
  inv1   g077(.a(new_n123_), .O(new_n124_));
  nor3   g078(.a(x23), .b(x22), .c(x21), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  oai21  g080(.a(new_n114_), .b(new_n120_), .c(new_n126_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(x16), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n122_), .c(new_n119_), .O(z13));
  nand2  g083(.a(x18), .b(x05), .O(new_n130_));
  oai21  g084(.a(x18), .b(new_n67_), .c(new_n130_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n86_), .O(new_n132_));
  oai21  g086(.a(new_n69_), .b(new_n86_), .c(x19), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x18), .O(new_n134_));
  aoi21  g088(.a(new_n125_), .b(new_n124_), .c(new_n69_), .O(new_n135_));
  nand2  g089(.a(new_n61_), .b(new_n103_), .O(new_n136_));
  nand2  g090(.a(new_n69_), .b(new_n120_), .O(new_n137_));
  nor3   g091(.a(new_n137_), .b(new_n123_), .c(new_n136_), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n135_), .c(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n134_), .c(new_n132_), .O(z14));
  nand2  g094(.a(new_n83_), .b(x06), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n72_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n86_), .O(new_n143_));
  nand3  g097(.a(new_n103_), .b(new_n90_), .c(new_n79_), .O(new_n144_));
  nand3  g098(.a(new_n69_), .b(new_n120_), .c(new_n61_), .O(new_n145_));
  oai21  g099(.a(new_n145_), .b(new_n144_), .c(x25), .O(new_n146_));
  nor2   g100(.a(x23), .b(x22), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand4  g102(.a(new_n148_), .b(new_n147_), .c(new_n100_), .d(new_n94_), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n146_), .c(x18), .O(new_n150_));
  inv1   g104(.a(x25), .O(new_n151_));
  nor2   g105(.a(new_n151_), .b(new_n48_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n143_), .O(z15));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n75_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n86_), .O(new_n157_));
  inv1   g111(.a(x26), .O(new_n158_));
  oai21  g112(.a(new_n158_), .b(new_n86_), .c(x19), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(x18), .O(new_n160_));
  nand2  g114(.a(new_n149_), .b(x26), .O(new_n161_));
  nand4  g115(.a(new_n147_), .b(new_n158_), .c(new_n151_), .d(new_n69_), .O(new_n162_));
  oai21  g116(.a(new_n162_), .b(new_n101_), .c(new_n161_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n160_), .c(new_n157_), .O(z16));
  inv1   g119(.a(new_n53_), .O(new_n166_));
  nand3  g120(.a(new_n100_), .b(x19), .c(new_n79_), .O(new_n167_));
  oai22  g121(.a(new_n167_), .b(new_n162_), .c(new_n47_), .d(new_n79_), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(x16), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(new_n166_), .O(z17));
endmodule


