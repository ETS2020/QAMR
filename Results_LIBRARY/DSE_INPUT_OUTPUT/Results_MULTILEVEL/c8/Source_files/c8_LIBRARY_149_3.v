// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x18), .b(x00), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nand2  g006(.a(x27), .b(new_n52_), .O(new_n53_));
  inv1   g007(.a(x20), .O(new_n54_));
  inv1   g008(.a(x27), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(new_n53_), .c(new_n49_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n59_), .c(new_n49_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  aoi21  g017(.a(x27), .b(new_n63_), .c(new_n49_), .O(new_n64_));
  oai21  g018(.a(x27), .b(x22), .c(new_n64_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n49_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n49_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x24), .c(new_n70_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n49_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n49_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  nor2   g031(.a(new_n49_), .b(new_n55_), .O(z08));
  xnor2a g032(.a(x19), .b(x17), .O(new_n79_));
  nand3  g033(.a(new_n79_), .b(new_n48_), .c(x16), .O(new_n80_));
  inv1   g034(.a(x16), .O(new_n81_));
  inv1   g035(.a(x18), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n81_), .c(x08), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n80_), .O(z09));
  nand2  g038(.a(x18), .b(x01), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n52_), .c(new_n85_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n81_), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  nor2   g042(.a(new_n88_), .b(new_n54_), .O(new_n89_));
  nor3   g043(.a(x20), .b(x19), .c(x17), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n87_), .c(new_n48_), .O(z10));
  nand2  g046(.a(x18), .b(x02), .O(new_n93_));
  oai22  g047(.a(new_n93_), .b(x00), .c(x18), .d(new_n58_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n81_), .O(new_n95_));
  nor2   g049(.a(x21), .b(x20), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n88_), .O(new_n97_));
  oai21  g051(.a(new_n90_), .b(new_n60_), .c(new_n97_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n48_), .c(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n95_), .O(z11));
  nand2  g054(.a(x18), .b(x03), .O(new_n101_));
  oai22  g055(.a(new_n101_), .b(x00), .c(x18), .d(new_n63_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n81_), .O(new_n103_));
  nand2  g057(.a(new_n97_), .b(x22), .O(new_n104_));
  nor2   g058(.a(x22), .b(x21), .O(new_n105_));
  nand3  g059(.a(new_n105_), .b(new_n88_), .c(new_n54_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n48_), .c(x16), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n103_), .O(z12));
  nand2  g063(.a(x18), .b(x04), .O(new_n110_));
  oai21  g064(.a(x18), .b(new_n66_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n81_), .O(new_n112_));
  nand2  g066(.a(new_n106_), .b(x23), .O(new_n113_));
  nor3   g067(.a(x23), .b(x22), .c(x21), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n90_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x16), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n112_), .c(new_n48_), .O(z13));
  nand2  g072(.a(x18), .b(x05), .O(new_n119_));
  oai21  g073(.a(x18), .b(new_n69_), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n81_), .O(new_n121_));
  inv1   g075(.a(x24), .O(new_n122_));
  aoi21  g076(.a(new_n114_), .b(new_n90_), .c(new_n122_), .O(new_n123_));
  inv1   g077(.a(x17), .O(new_n124_));
  nor2   g078(.a(x20), .b(x19), .O(new_n125_));
  nor2   g079(.a(x24), .b(x23), .O(new_n126_));
  nand4  g080(.a(new_n126_), .b(new_n105_), .c(new_n125_), .d(new_n124_), .O(new_n127_));
  inv1   g081(.a(new_n127_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n123_), .c(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n121_), .c(new_n48_), .O(z14));
  nand2  g084(.a(x18), .b(x06), .O(new_n131_));
  oai21  g085(.a(x18), .b(new_n72_), .c(new_n131_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n81_), .O(new_n133_));
  nand2  g087(.a(new_n127_), .b(x25), .O(new_n134_));
  nor2   g088(.a(x23), .b(x22), .O(new_n135_));
  nor2   g089(.a(x25), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n135_), .c(new_n96_), .d(new_n88_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n133_), .c(new_n48_), .O(z15));
  nand2  g094(.a(x18), .b(x07), .O(new_n141_));
  oai22  g095(.a(new_n141_), .b(x00), .c(x18), .d(new_n75_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n81_), .O(new_n143_));
  nand2  g097(.a(new_n137_), .b(x26), .O(new_n144_));
  nor3   g098(.a(x26), .b(x25), .c(x24), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n135_), .c(new_n96_), .d(new_n88_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n48_), .c(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n143_), .O(z16));
  nor2   g103(.a(x26), .b(x25), .O(new_n150_));
  nand3  g104(.a(new_n150_), .b(new_n135_), .c(new_n122_), .O(new_n151_));
  nand3  g105(.a(new_n96_), .b(x19), .c(new_n124_), .O(new_n152_));
  oai22  g106(.a(new_n152_), .b(new_n151_), .c(new_n55_), .d(new_n124_), .O(new_n153_));
  nand3  g107(.a(new_n153_), .b(new_n48_), .c(x16), .O(new_n154_));
  inv1   g108(.a(new_n154_), .O(z17));
endmodule


