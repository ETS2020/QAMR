// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  nor2   g000(.a(x18), .b(x16), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  nand2  g008(.a(x27), .b(new_n54_), .O(new_n55_));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n51_), .b(new_n56_), .O(new_n57_));
  aoi21  g011(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x21), .O(new_n61_));
  nand2  g015(.a(new_n51_), .b(new_n61_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n60_), .c(new_n47_), .O(z02));
  inv1   g017(.a(x11), .O(new_n64_));
  nand2  g018(.a(x27), .b(new_n64_), .O(new_n65_));
  inv1   g019(.a(x22), .O(new_n66_));
  nand2  g020(.a(new_n51_), .b(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n65_), .c(new_n47_), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x23), .O(new_n71_));
  nand2  g025(.a(new_n51_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n47_), .O(z04));
  inv1   g027(.a(x13), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x24), .O(new_n76_));
  nand2  g030(.a(new_n51_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n47_), .O(z05));
  inv1   g032(.a(x14), .O(new_n79_));
  aoi21  g033(.a(x27), .b(new_n79_), .c(new_n47_), .O(new_n80_));
  oai21  g034(.a(x27), .b(x25), .c(new_n80_), .O(z06));
  inv1   g035(.a(x15), .O(new_n82_));
  aoi21  g036(.a(x27), .b(new_n82_), .c(new_n47_), .O(new_n83_));
  oai21  g037(.a(x27), .b(x26), .c(new_n83_), .O(z07));
  nor2   g038(.a(new_n47_), .b(new_n51_), .O(z08));
  inv1   g039(.a(x17), .O(new_n86_));
  nor2   g040(.a(new_n50_), .b(new_n86_), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  inv1   g043(.a(x18), .O(new_n90_));
  nor2   g044(.a(new_n90_), .b(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(x00), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n89_), .O(z09));
  nor2   g047(.a(new_n88_), .b(new_n56_), .O(new_n94_));
  nor3   g048(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  nand2  g050(.a(new_n91_), .b(x01), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n96_), .O(z10));
  inv1   g052(.a(x16), .O(new_n99_));
  nor2   g053(.a(x20), .b(x19), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n86_), .O(new_n101_));
  nand4  g055(.a(new_n61_), .b(new_n56_), .c(new_n50_), .d(new_n86_), .O(new_n102_));
  inv1   g056(.a(new_n102_), .O(new_n103_));
  aoi21  g057(.a(new_n101_), .b(x21), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n91_), .b(x02), .O(new_n105_));
  oai21  g059(.a(new_n104_), .b(new_n99_), .c(new_n105_), .O(z11));
  nand2  g060(.a(new_n102_), .b(x22), .O(new_n107_));
  nor2   g061(.a(x22), .b(x21), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n88_), .c(new_n56_), .O(new_n109_));
  and2   g063(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand2  g064(.a(new_n91_), .b(x03), .O(new_n111_));
  oai21  g065(.a(new_n110_), .b(new_n99_), .c(new_n111_), .O(z12));
  oai21  g066(.a(new_n90_), .b(x04), .c(new_n99_), .O(new_n113_));
  nor3   g067(.a(x23), .b(x22), .c(x21), .O(new_n114_));
  aoi22  g068(.a(new_n114_), .b(new_n95_), .c(new_n109_), .d(x23), .O(new_n115_));
  oai21  g069(.a(new_n115_), .b(new_n99_), .c(new_n113_), .O(z13));
  aoi21  g070(.a(new_n114_), .b(new_n95_), .c(new_n76_), .O(new_n117_));
  nor2   g071(.a(x24), .b(x23), .O(new_n118_));
  nand4  g072(.a(new_n118_), .b(new_n108_), .c(new_n100_), .d(new_n86_), .O(new_n119_));
  inv1   g073(.a(new_n119_), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  nand2  g075(.a(new_n91_), .b(x05), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n121_), .O(z14));
  inv1   g077(.a(x25), .O(new_n124_));
  nand4  g078(.a(new_n124_), .b(new_n76_), .c(new_n71_), .d(new_n66_), .O(new_n125_));
  inv1   g079(.a(new_n125_), .O(new_n126_));
  aoi22  g080(.a(new_n126_), .b(new_n103_), .c(new_n119_), .d(x25), .O(new_n127_));
  nand2  g081(.a(new_n91_), .b(x06), .O(new_n128_));
  oai21  g082(.a(new_n127_), .b(new_n99_), .c(new_n128_), .O(z15));
  nor2   g083(.a(x21), .b(x20), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nor2   g085(.a(x25), .b(x24), .O(new_n132_));
  nand4  g086(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n88_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x26), .O(new_n134_));
  nor3   g088(.a(x26), .b(x25), .c(x24), .O(new_n135_));
  nand4  g089(.a(new_n135_), .b(new_n131_), .c(new_n130_), .d(new_n88_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(x16), .O(new_n138_));
  nand2  g092(.a(new_n91_), .b(x07), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n138_), .O(z16));
  inv1   g094(.a(new_n47_), .O(new_n141_));
  nor2   g095(.a(x26), .b(x25), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n131_), .c(new_n76_), .O(new_n143_));
  nand3  g097(.a(new_n130_), .b(x19), .c(new_n86_), .O(new_n144_));
  oai22  g098(.a(new_n144_), .b(new_n143_), .c(new_n51_), .d(new_n86_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x16), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n141_), .O(z17));
endmodule


