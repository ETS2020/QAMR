// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x18), .b(x05), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  aoi21  g005(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n48_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g010(.a(x11), .O(new_n57_));
  aoi21  g011(.a(x27), .b(new_n57_), .c(new_n48_), .O(new_n58_));
  oai21  g012(.a(x27), .b(x22), .c(new_n58_), .O(z03));
  inv1   g013(.a(x12), .O(new_n60_));
  aoi21  g014(.a(x27), .b(new_n60_), .c(new_n48_), .O(new_n61_));
  oai21  g015(.a(x27), .b(x23), .c(new_n61_), .O(z04));
  inv1   g016(.a(x13), .O(new_n63_));
  aoi21  g017(.a(x27), .b(new_n63_), .c(new_n48_), .O(new_n64_));
  oai21  g018(.a(x27), .b(x24), .c(new_n64_), .O(z05));
  inv1   g019(.a(x14), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x25), .O(new_n68_));
  inv1   g022(.a(x27), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n67_), .c(new_n48_), .O(z06));
  inv1   g025(.a(x15), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x26), .O(new_n74_));
  nand2  g028(.a(new_n69_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n48_), .O(z07));
  nor2   g030(.a(new_n48_), .b(new_n69_), .O(z08));
  inv1   g031(.a(x16), .O(new_n78_));
  nand2  g032(.a(x18), .b(x00), .O(new_n79_));
  inv1   g033(.a(x18), .O(new_n80_));
  nand3  g034(.a(new_n80_), .b(x08), .c(x05), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  inv1   g037(.a(new_n48_), .O(new_n84_));
  xnor2a g038(.a(x19), .b(x17), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n84_), .c(x16), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n83_), .O(z09));
  nand2  g041(.a(x18), .b(x01), .O(new_n88_));
  oai21  g042(.a(x18), .b(new_n51_), .c(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n78_), .O(new_n90_));
  inv1   g044(.a(x20), .O(new_n91_));
  nor2   g045(.a(x19), .b(x17), .O(new_n92_));
  nor2   g046(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  inv1   g047(.a(x17), .O(new_n94_));
  inv1   g048(.a(x19), .O(new_n95_));
  nand3  g049(.a(new_n91_), .b(new_n95_), .c(new_n94_), .O(new_n96_));
  inv1   g050(.a(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n93_), .c(x16), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n90_), .c(new_n84_), .O(z10));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  nand3  g054(.a(new_n80_), .b(x10), .c(x05), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n78_), .O(new_n103_));
  inv1   g057(.a(x21), .O(new_n104_));
  nor2   g058(.a(x21), .b(x20), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n92_), .O(new_n106_));
  oai21  g060(.a(new_n97_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n84_), .c(x16), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n103_), .O(z11));
  nand2  g063(.a(x18), .b(x03), .O(new_n110_));
  oai21  g064(.a(x18), .b(new_n57_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n78_), .O(new_n112_));
  inv1   g066(.a(x05), .O(new_n113_));
  aoi21  g067(.a(new_n80_), .b(new_n113_), .c(x22), .O(new_n114_));
  nand4  g068(.a(new_n114_), .b(new_n104_), .c(new_n91_), .d(new_n95_), .O(new_n115_));
  nand2  g069(.a(new_n106_), .b(x22), .O(new_n116_));
  oai21  g070(.a(new_n115_), .b(x17), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x16), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n112_), .c(new_n84_), .O(z12));
  nand2  g073(.a(x18), .b(x04), .O(new_n120_));
  oai21  g074(.a(x18), .b(new_n60_), .c(new_n120_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n78_), .O(new_n122_));
  inv1   g076(.a(x22), .O(new_n123_));
  nand4  g077(.a(new_n92_), .b(new_n123_), .c(new_n104_), .d(new_n91_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(x23), .O(new_n125_));
  nor3   g079(.a(x23), .b(x22), .c(x21), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n97_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n122_), .c(new_n84_), .O(z13));
  nand2  g084(.a(new_n80_), .b(new_n63_), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n78_), .c(x05), .O(new_n132_));
  inv1   g086(.a(x24), .O(new_n133_));
  aoi21  g087(.a(new_n126_), .b(new_n97_), .c(new_n133_), .O(new_n134_));
  inv1   g088(.a(x23), .O(new_n135_));
  nand4  g089(.a(new_n133_), .b(new_n135_), .c(new_n123_), .d(new_n104_), .O(new_n136_));
  nor2   g090(.a(new_n136_), .b(new_n96_), .O(new_n137_));
  oai21  g091(.a(new_n137_), .b(new_n134_), .c(x16), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n132_), .c(new_n84_), .O(z14));
  nand2  g093(.a(x18), .b(x06), .O(new_n140_));
  oai21  g094(.a(x18), .b(new_n66_), .c(new_n140_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n78_), .O(new_n142_));
  nor2   g096(.a(x23), .b(x22), .O(new_n143_));
  nor2   g097(.a(x25), .b(x24), .O(new_n144_));
  nand4  g098(.a(new_n144_), .b(new_n143_), .c(new_n105_), .d(new_n92_), .O(new_n145_));
  oai21  g099(.a(new_n137_), .b(new_n68_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(x16), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n142_), .c(new_n84_), .O(z15));
  nand2  g102(.a(x18), .b(x07), .O(new_n149_));
  oai21  g103(.a(x18), .b(new_n72_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n78_), .O(new_n151_));
  nand2  g105(.a(new_n145_), .b(x26), .O(new_n152_));
  nand4  g106(.a(new_n143_), .b(new_n74_), .c(new_n68_), .d(new_n133_), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n106_), .c(new_n152_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(x16), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n151_), .c(new_n84_), .O(z16));
  nand3  g110(.a(new_n105_), .b(x19), .c(new_n94_), .O(new_n157_));
  oai22  g111(.a(new_n157_), .b(new_n153_), .c(new_n69_), .d(new_n94_), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n84_), .c(x16), .O(new_n159_));
  inv1   g113(.a(new_n159_), .O(z17));
endmodule


