// Benchmark "FAU" written by ABC on Thu Aug 13 18:01:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x17), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nand2  g006(.a(x27), .b(new_n52_), .O(new_n53_));
  inv1   g007(.a(x20), .O(new_n54_));
  inv1   g008(.a(x27), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(new_n53_), .c(new_n49_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n59_), .c(new_n49_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n55_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n49_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x23), .c(new_n69_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n71_), .c(new_n49_), .O(new_n72_));
  oai21  g026(.a(x27), .b(x24), .c(new_n72_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n49_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x25), .c(new_n75_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x26), .O(new_n79_));
  nand2  g033(.a(new_n55_), .b(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n78_), .c(new_n49_), .O(z07));
  inv1   g035(.a(new_n49_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n55_), .O(z08));
  inv1   g037(.a(x17), .O(new_n84_));
  inv1   g038(.a(x19), .O(new_n85_));
  nor2   g039(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nor3   g040(.a(x19), .b(x18), .c(x17), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n86_), .c(x16), .O(new_n88_));
  inv1   g042(.a(x16), .O(new_n89_));
  inv1   g043(.a(x00), .O(new_n90_));
  nand2  g044(.a(x18), .b(x17), .O(new_n91_));
  oai22  g045(.a(new_n91_), .b(new_n90_), .c(x18), .d(new_n47_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n88_), .O(z09));
  nand2  g048(.a(x18), .b(x01), .O(new_n95_));
  oai21  g049(.a(x18), .b(new_n52_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n89_), .O(new_n97_));
  nor2   g051(.a(x20), .b(x19), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n48_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n84_), .O(new_n101_));
  nor2   g055(.a(x19), .b(x17), .O(new_n102_));
  inv1   g056(.a(new_n102_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(x20), .c(x16), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n101_), .c(new_n97_), .O(z10));
  nand2  g059(.a(x18), .b(x02), .O(new_n106_));
  oai21  g060(.a(x18), .b(new_n58_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n89_), .O(new_n108_));
  nor2   g062(.a(x21), .b(x20), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n85_), .c(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n48_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n84_), .O(new_n112_));
  nand3  g066(.a(new_n98_), .b(new_n48_), .c(new_n84_), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(x21), .c(x16), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n112_), .c(new_n108_), .O(z11));
  inv1   g069(.a(x03), .O(new_n116_));
  oai22  g070(.a(new_n91_), .b(new_n116_), .c(x18), .d(new_n63_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n89_), .O(new_n118_));
  nand2  g072(.a(new_n109_), .b(new_n85_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x22), .O(new_n120_));
  nor2   g074(.a(x22), .b(x21), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n102_), .c(new_n54_), .O(new_n122_));
  aoi21  g076(.a(new_n122_), .b(new_n120_), .c(x18), .O(new_n123_));
  nor2   g077(.a(new_n65_), .b(new_n84_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n123_), .c(x16), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n118_), .O(z12));
  inv1   g080(.a(x04), .O(new_n127_));
  oai22  g081(.a(new_n91_), .b(new_n127_), .c(x18), .d(new_n68_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n89_), .O(new_n129_));
  nand2  g083(.a(new_n121_), .b(new_n98_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(x23), .O(new_n131_));
  nor2   g085(.a(x23), .b(x22), .O(new_n132_));
  nand4  g086(.a(new_n132_), .b(new_n98_), .c(new_n60_), .d(new_n84_), .O(new_n133_));
  aoi21  g087(.a(new_n133_), .b(new_n131_), .c(x18), .O(new_n134_));
  and2   g088(.a(x23), .b(x17), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n134_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n129_), .O(z13));
  inv1   g091(.a(x05), .O(new_n138_));
  oai22  g092(.a(new_n91_), .b(new_n138_), .c(x18), .d(new_n71_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n89_), .O(new_n140_));
  nand3  g094(.a(new_n132_), .b(new_n98_), .c(new_n60_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x24), .O(new_n142_));
  nor2   g096(.a(x24), .b(x23), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n121_), .c(new_n98_), .d(new_n84_), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(new_n142_), .c(x18), .O(new_n145_));
  and2   g099(.a(x24), .b(x17), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n145_), .c(x16), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n140_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(x18), .b(new_n74_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n89_), .O(new_n151_));
  nor2   g105(.a(x25), .b(x24), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n132_), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n110_), .c(new_n48_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n84_), .O(new_n155_));
  inv1   g109(.a(new_n113_), .O(new_n156_));
  nand3  g110(.a(new_n143_), .b(new_n121_), .c(new_n156_), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(x25), .c(x16), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n155_), .c(new_n151_), .O(z15));
  inv1   g113(.a(x07), .O(new_n160_));
  oai22  g114(.a(new_n91_), .b(new_n160_), .c(x18), .d(new_n77_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n89_), .O(new_n162_));
  nand4  g116(.a(new_n152_), .b(new_n132_), .c(new_n109_), .d(new_n85_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x26), .O(new_n164_));
  nor3   g118(.a(x26), .b(x25), .c(x24), .O(new_n165_));
  nand4  g119(.a(new_n165_), .b(new_n132_), .c(new_n109_), .d(new_n102_), .O(new_n166_));
  aoi21  g120(.a(new_n166_), .b(new_n164_), .c(x18), .O(new_n167_));
  nor2   g121(.a(new_n79_), .b(new_n84_), .O(new_n168_));
  oai21  g122(.a(new_n168_), .b(new_n167_), .c(x16), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(new_n162_), .O(z16));
  nand2  g124(.a(new_n165_), .b(new_n132_), .O(new_n171_));
  nand3  g125(.a(new_n109_), .b(x19), .c(new_n84_), .O(new_n172_));
  oai22  g126(.a(new_n172_), .b(new_n171_), .c(new_n55_), .d(new_n84_), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(x16), .O(new_n174_));
  nand2  g128(.a(new_n174_), .b(new_n82_), .O(z17));
endmodule


