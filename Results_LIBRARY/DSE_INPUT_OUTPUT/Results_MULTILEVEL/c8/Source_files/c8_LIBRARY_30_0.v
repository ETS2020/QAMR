// Benchmark "FAU" written by ABC on Thu Aug 13 18:00:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x16), .O(new_n48_));
  inv1   g002(.a(x19), .O(new_n49_));
  oai21  g003(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  oai21  g004(.a(new_n47_), .b(x08), .c(new_n50_), .O(z00));
  inv1   g005(.a(x20), .O(new_n52_));
  nand3  g006(.a(new_n47_), .b(new_n52_), .c(x16), .O(new_n53_));
  oai21  g007(.a(new_n47_), .b(x09), .c(new_n53_), .O(z01));
  inv1   g008(.a(x21), .O(new_n55_));
  nand3  g009(.a(new_n47_), .b(new_n55_), .c(x16), .O(new_n56_));
  oai21  g010(.a(new_n47_), .b(x10), .c(new_n56_), .O(z02));
  inv1   g011(.a(x22), .O(new_n58_));
  nand3  g012(.a(new_n47_), .b(new_n58_), .c(x16), .O(new_n59_));
  oai21  g013(.a(new_n47_), .b(x11), .c(new_n59_), .O(z03));
  inv1   g014(.a(x23), .O(new_n61_));
  nand3  g015(.a(new_n47_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g016(.a(new_n47_), .b(x12), .c(new_n62_), .O(z04));
  inv1   g017(.a(x24), .O(new_n64_));
  nand3  g018(.a(new_n47_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g019(.a(new_n47_), .b(x13), .c(new_n65_), .O(z05));
  inv1   g020(.a(x25), .O(new_n67_));
  oai21  g021(.a(new_n67_), .b(new_n48_), .c(new_n47_), .O(new_n68_));
  oai21  g022(.a(new_n47_), .b(x14), .c(new_n68_), .O(z06));
  inv1   g023(.a(x26), .O(new_n70_));
  oai21  g024(.a(new_n70_), .b(new_n48_), .c(new_n47_), .O(new_n71_));
  oai21  g025(.a(new_n47_), .b(x15), .c(new_n71_), .O(z07));
  nand2  g026(.a(new_n47_), .b(x16), .O(z08));
  inv1   g027(.a(x17), .O(new_n74_));
  nor2   g028(.a(new_n49_), .b(new_n74_), .O(new_n75_));
  nor2   g029(.a(x19), .b(x17), .O(new_n76_));
  oai21  g030(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  inv1   g031(.a(x08), .O(new_n78_));
  nand2  g032(.a(x18), .b(x00), .O(new_n79_));
  oai21  g033(.a(x18), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nand3  g034(.a(new_n80_), .b(x27), .c(new_n48_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n77_), .O(z09));
  inv1   g036(.a(x09), .O(new_n83_));
  nand2  g037(.a(x18), .b(x01), .O(new_n84_));
  oai21  g038(.a(x18), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(x27), .c(new_n48_), .O(new_n86_));
  nor2   g040(.a(new_n76_), .b(new_n52_), .O(new_n87_));
  nor3   g041(.a(x20), .b(x19), .c(x17), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n86_), .O(z10));
  nand2  g044(.a(x18), .b(x02), .O(new_n91_));
  inv1   g045(.a(x18), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(x10), .O(new_n93_));
  nand3  g047(.a(new_n93_), .b(new_n91_), .c(x27), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n48_), .O(new_n95_));
  nor2   g049(.a(x20), .b(x19), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n74_), .O(new_n97_));
  nand4  g051(.a(new_n55_), .b(new_n52_), .c(new_n49_), .d(new_n74_), .O(new_n98_));
  inv1   g052(.a(new_n98_), .O(new_n99_));
  aoi21  g053(.a(new_n97_), .b(x21), .c(new_n99_), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n48_), .c(new_n95_), .O(z11));
  nand2  g055(.a(x18), .b(x03), .O(new_n102_));
  nand2  g056(.a(new_n92_), .b(x11), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n102_), .c(x27), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n48_), .O(new_n105_));
  nand2  g059(.a(new_n98_), .b(x22), .O(new_n106_));
  nor2   g060(.a(x22), .b(x21), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n76_), .c(new_n52_), .O(new_n108_));
  and2   g062(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  oai21  g063(.a(new_n109_), .b(new_n48_), .c(new_n105_), .O(z12));
  nand2  g064(.a(x18), .b(x04), .O(new_n111_));
  nand2  g065(.a(new_n92_), .b(x12), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n111_), .c(x27), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n48_), .O(new_n114_));
  nor3   g068(.a(x23), .b(x22), .c(x21), .O(new_n115_));
  aoi22  g069(.a(new_n115_), .b(new_n88_), .c(new_n108_), .d(x23), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(new_n48_), .c(new_n114_), .O(z13));
  inv1   g071(.a(x13), .O(new_n118_));
  nand2  g072(.a(x18), .b(x05), .O(new_n119_));
  oai21  g073(.a(x18), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(x27), .c(new_n48_), .O(new_n121_));
  aoi21  g075(.a(new_n115_), .b(new_n88_), .c(new_n64_), .O(new_n122_));
  nor2   g076(.a(x24), .b(x23), .O(new_n123_));
  nand4  g077(.a(new_n123_), .b(new_n107_), .c(new_n96_), .d(new_n74_), .O(new_n124_));
  inv1   g078(.a(new_n124_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n122_), .c(x16), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n121_), .O(z14));
  nand2  g081(.a(x18), .b(x06), .O(new_n128_));
  nand2  g082(.a(new_n92_), .b(x14), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n128_), .c(x27), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n48_), .O(new_n131_));
  nand4  g085(.a(new_n67_), .b(new_n64_), .c(new_n61_), .d(new_n58_), .O(new_n132_));
  inv1   g086(.a(new_n132_), .O(new_n133_));
  aoi22  g087(.a(new_n133_), .b(new_n99_), .c(new_n124_), .d(x25), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n48_), .c(new_n131_), .O(z15));
  inv1   g089(.a(x15), .O(new_n136_));
  nand2  g090(.a(x18), .b(x07), .O(new_n137_));
  oai21  g091(.a(x18), .b(new_n136_), .c(new_n137_), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(x27), .c(new_n48_), .O(new_n139_));
  nor2   g093(.a(x21), .b(x20), .O(new_n140_));
  nor2   g094(.a(x23), .b(x22), .O(new_n141_));
  nor2   g095(.a(x25), .b(x24), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(new_n76_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x26), .O(new_n144_));
  nor3   g098(.a(x26), .b(x25), .c(x24), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n141_), .c(new_n140_), .d(new_n76_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n139_), .O(z16));
  nand2  g103(.a(new_n47_), .b(new_n48_), .O(new_n150_));
  nand4  g104(.a(new_n141_), .b(new_n70_), .c(new_n67_), .d(new_n64_), .O(new_n151_));
  nand3  g105(.a(new_n140_), .b(x19), .c(new_n74_), .O(new_n152_));
  oai22  g106(.a(new_n152_), .b(new_n151_), .c(new_n47_), .d(new_n74_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(x16), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n150_), .O(z17));
endmodule


