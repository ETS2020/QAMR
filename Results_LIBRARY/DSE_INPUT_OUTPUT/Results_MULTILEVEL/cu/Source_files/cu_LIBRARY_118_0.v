// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n72_;
  inv1   g00(.a(x01), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand2  g03(.a(x05), .b(new_n28_), .O(new_n29_));
  aoi21  g04(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  inv1   g05(.a(x03), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(x02), .O(new_n34_));
  oai21  g09(.a(new_n30_), .b(new_n26_), .c(new_n34_), .O(z00));
  nand2  g10(.a(new_n27_), .b(x01), .O(new_n36_));
  or2    g11(.a(new_n36_), .b(new_n29_), .O(new_n37_));
  nand3  g12(.a(new_n32_), .b(x04), .c(x02), .O(new_n38_));
  aoi21  g13(.a(new_n38_), .b(new_n37_), .c(x03), .O(z01));
  nor2   g14(.a(x02), .b(x01), .O(z02));
  inv1   g15(.a(x00), .O(new_n41_));
  inv1   g16(.a(x13), .O(new_n42_));
  nand3  g17(.a(new_n42_), .b(x05), .c(new_n28_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n31_), .c(new_n41_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x01), .c(x02), .O(z04));
  nand3  g21(.a(new_n44_), .b(new_n31_), .c(x00), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x01), .c(x02), .O(z05));
  inv1   g23(.a(x07), .O(new_n49_));
  inv1   g24(.a(x12), .O(new_n50_));
  inv1   g25(.a(x10), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x00), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand4  g30(.a(new_n55_), .b(x13), .c(new_n50_), .d(new_n49_), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(new_n57_));
  nand4  g32(.a(new_n57_), .b(x05), .c(new_n27_), .d(x01), .O(new_n58_));
  nand3  g33(.a(new_n42_), .b(new_n32_), .c(x02), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(x04), .c(new_n31_), .O(new_n61_));
  inv1   g36(.a(new_n61_), .O(z06));
  nand4  g37(.a(new_n42_), .b(x05), .c(new_n28_), .d(new_n31_), .O(new_n63_));
  aoi21  g38(.a(new_n63_), .b(x01), .c(x02), .O(z07));
  nand3  g39(.a(x13), .b(new_n50_), .c(x05), .O(new_n65_));
  oai21  g40(.a(new_n65_), .b(new_n36_), .c(new_n59_), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(x04), .c(new_n31_), .O(new_n67_));
  inv1   g42(.a(new_n67_), .O(z08));
  nand2  g43(.a(x13), .b(x06), .O(new_n69_));
  oai21  g44(.a(x02), .b(x01), .c(new_n69_), .O(z09));
  aoi21  g45(.a(x06), .b(new_n31_), .c(new_n26_), .O(new_n71_));
  nand3  g46(.a(x06), .b(new_n32_), .c(new_n31_), .O(new_n72_));
  oai21  g47(.a(new_n71_), .b(x02), .c(new_n72_), .O(z10));
  nor2   g48(.a(x02), .b(x01), .O(z03));
endmodule


