// Benchmark "FAU" written by ABC on Thu Aug 13 15:09:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x02), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(x05), .b(new_n27_), .O(new_n28_));
  aoi21  g03(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g04(.a(x03), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(x02), .O(new_n33_));
  oai21  g08(.a(new_n29_), .b(x01), .c(new_n33_), .O(z00));
  inv1   g09(.a(x01), .O(new_n35_));
  oai21  g10(.a(new_n28_), .b(x03), .c(new_n35_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n26_), .O(new_n37_));
  nand4  g12(.a(new_n31_), .b(x04), .c(new_n30_), .d(x02), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(z01));
  nor3   g14(.a(x02), .b(x01), .c(x00), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(x05), .c(new_n27_), .d(new_n30_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x13), .O(z02));
  inv1   g17(.a(x13), .O(new_n43_));
  nand4  g18(.a(new_n30_), .b(new_n26_), .c(new_n35_), .d(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n43_), .c(x05), .d(new_n27_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z03));
  nor2   g22(.a(x02), .b(new_n35_), .O(z04));
  inv1   g23(.a(x07), .O(new_n49_));
  inv1   g24(.a(x12), .O(new_n50_));
  inv1   g25(.a(x00), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x00), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand4  g31(.a(new_n56_), .b(x13), .c(new_n50_), .d(new_n49_), .O(new_n57_));
  inv1   g32(.a(new_n57_), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(x05), .c(new_n26_), .d(new_n35_), .O(new_n59_));
  nand3  g34(.a(new_n43_), .b(new_n31_), .c(x02), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(x04), .c(new_n30_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(z06));
  nand4  g38(.a(new_n43_), .b(x05), .c(new_n27_), .d(new_n30_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n35_), .c(x02), .O(z07));
  nand2  g40(.a(new_n26_), .b(new_n35_), .O(new_n66_));
  nand3  g41(.a(x13), .b(new_n50_), .c(x05), .O(new_n67_));
  oai21  g42(.a(new_n67_), .b(new_n66_), .c(new_n60_), .O(new_n68_));
  nand3  g43(.a(new_n68_), .b(x04), .c(new_n30_), .O(new_n69_));
  inv1   g44(.a(new_n69_), .O(z08));
  inv1   g45(.a(x06), .O(new_n71_));
  nor3   g46(.a(z04), .b(new_n43_), .c(new_n71_), .O(z09));
  oai21  g47(.a(x05), .b(new_n26_), .c(new_n66_), .O(new_n73_));
  nand3  g48(.a(new_n73_), .b(x06), .c(new_n30_), .O(new_n74_));
  inv1   g49(.a(new_n74_), .O(z10));
  nor2   g50(.a(x02), .b(new_n35_), .O(z05));
endmodule


