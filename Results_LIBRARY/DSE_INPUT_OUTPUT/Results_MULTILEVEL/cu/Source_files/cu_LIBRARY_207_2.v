// Benchmark "FAU" written by ABC on Thu Aug 13 15:09:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n73_, new_n74_;
  inv1   g00(.a(x00), .O(new_n26_));
  nor2   g01(.a(x13), .b(new_n26_), .O(z03));
  inv1   g02(.a(x04), .O(new_n28_));
  oai21  g03(.a(x05), .b(new_n28_), .c(x02), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nor2   g06(.a(new_n31_), .b(x04), .O(new_n32_));
  inv1   g07(.a(new_n32_), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n30_), .c(x03), .O(new_n34_));
  aoi21  g09(.a(new_n34_), .b(new_n29_), .c(z03), .O(z00));
  inv1   g10(.a(x03), .O(new_n36_));
  inv1   g11(.a(z03), .O(new_n37_));
  nand3  g12(.a(new_n31_), .b(x04), .c(x02), .O(new_n38_));
  oai21  g13(.a(new_n33_), .b(x02), .c(new_n38_), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z01));
  inv1   g16(.a(x01), .O(new_n42_));
  nand4  g17(.a(new_n32_), .b(new_n36_), .c(new_n30_), .d(new_n42_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(new_n26_), .c(x13), .O(z02));
  inv1   g19(.a(x13), .O(new_n45_));
  nand4  g20(.a(new_n36_), .b(new_n30_), .c(x01), .d(new_n26_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(new_n47_));
  nand4  g22(.a(new_n47_), .b(new_n45_), .c(x05), .d(new_n28_), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(z04));
  inv1   g24(.a(x07), .O(new_n50_));
  inv1   g25(.a(x12), .O(new_n51_));
  nor2   g26(.a(x09), .b(x01), .O(new_n52_));
  nor2   g27(.a(x11), .b(new_n42_), .O(new_n53_));
  oai21  g28(.a(new_n53_), .b(new_n52_), .c(x00), .O(new_n54_));
  nor2   g29(.a(x08), .b(x01), .O(new_n55_));
  nor2   g30(.a(x10), .b(new_n42_), .O(new_n56_));
  oai21  g31(.a(new_n56_), .b(new_n55_), .c(new_n26_), .O(new_n57_));
  aoi21  g32(.a(new_n57_), .b(new_n54_), .c(new_n45_), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(new_n51_), .c(new_n50_), .d(x05), .O(new_n59_));
  nand4  g34(.a(new_n45_), .b(new_n31_), .c(x02), .d(new_n26_), .O(new_n60_));
  oai21  g35(.a(new_n59_), .b(x02), .c(new_n60_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(x04), .c(new_n36_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(z06));
  nor2   g38(.a(x02), .b(x00), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(x05), .c(new_n28_), .d(new_n36_), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(x13), .O(z07));
  nand4  g41(.a(x13), .b(new_n51_), .c(x05), .d(new_n30_), .O(new_n67_));
  nand3  g42(.a(new_n45_), .b(new_n31_), .c(x02), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g44(.a(new_n69_), .b(x04), .c(new_n36_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n37_), .O(z08));
  and2   g46(.a(x13), .b(x06), .O(z09));
  nand2  g47(.a(x05), .b(x02), .O(new_n73_));
  nand4  g48(.a(new_n73_), .b(new_n37_), .c(x06), .d(new_n36_), .O(new_n74_));
  inv1   g49(.a(new_n74_), .O(z10));
  nor2   g50(.a(x13), .b(new_n26_), .O(z05));
endmodule


