// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n70_, new_n71_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  oai21  g05(.a(new_n30_), .b(x04), .c(new_n29_), .O(new_n31_));
  nor2   g06(.a(x13), .b(x01), .O(z02));
  inv1   g07(.a(z02), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n31_), .c(new_n28_), .d(new_n26_), .O(z00));
  nor2   g09(.a(new_n30_), .b(x04), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n29_), .O(new_n36_));
  nand3  g11(.a(new_n30_), .b(x04), .c(x02), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n26_), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n33_), .O(z01));
  inv1   g15(.a(x00), .O(new_n41_));
  nand4  g16(.a(new_n35_), .b(new_n26_), .c(new_n29_), .d(new_n41_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x01), .c(x13), .O(z04));
  nand4  g18(.a(new_n35_), .b(new_n26_), .c(new_n29_), .d(x00), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(x01), .c(x13), .O(z05));
  inv1   g20(.a(x07), .O(new_n46_));
  inv1   g21(.a(x12), .O(new_n47_));
  inv1   g22(.a(x13), .O(new_n48_));
  nor2   g23(.a(x09), .b(x01), .O(new_n49_));
  inv1   g24(.a(x01), .O(new_n50_));
  nor2   g25(.a(x11), .b(new_n50_), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n49_), .c(x00), .O(new_n52_));
  nor2   g27(.a(x08), .b(x01), .O(new_n53_));
  nor2   g28(.a(x10), .b(new_n50_), .O(new_n54_));
  oai21  g29(.a(new_n54_), .b(new_n53_), .c(new_n41_), .O(new_n55_));
  aoi21  g30(.a(new_n55_), .b(new_n52_), .c(new_n48_), .O(new_n56_));
  nand4  g31(.a(new_n56_), .b(new_n47_), .c(new_n46_), .d(x05), .O(new_n57_));
  nand4  g32(.a(new_n48_), .b(new_n30_), .c(x02), .d(x01), .O(new_n58_));
  oai21  g33(.a(new_n57_), .b(x02), .c(new_n58_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(x04), .c(new_n26_), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(z06));
  nand4  g36(.a(new_n27_), .b(new_n26_), .c(new_n29_), .d(x01), .O(new_n62_));
  nor3   g37(.a(new_n62_), .b(x13), .c(new_n30_), .O(z07));
  nand4  g38(.a(x13), .b(new_n47_), .c(x05), .d(new_n29_), .O(new_n64_));
  nand3  g39(.a(new_n48_), .b(new_n30_), .c(x02), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(x04), .c(new_n26_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n33_), .O(z08));
  and2   g43(.a(x13), .b(x06), .O(z09));
  nand2  g44(.a(x05), .b(x02), .O(new_n70_));
  nand4  g45(.a(new_n70_), .b(new_n33_), .c(x06), .d(new_n26_), .O(new_n71_));
  inv1   g46(.a(new_n71_), .O(z10));
  nor2   g47(.a(x13), .b(x01), .O(z03));
endmodule


