// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  inv1   g04(.a(x07), .O(new_n30_));
  nand3  g05(.a(new_n30_), .b(x05), .c(new_n27_), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n28_), .c(new_n26_), .O(z00));
  inv1   g08(.a(x05), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand4  g10(.a(new_n30_), .b(x05), .c(new_n27_), .d(new_n29_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(new_n35_), .c(x03), .O(z01));
  inv1   g12(.a(x00), .O(new_n38_));
  nor2   g13(.a(x02), .b(x01), .O(new_n39_));
  nor3   g14(.a(x13), .b(x04), .c(x03), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(new_n30_), .c(new_n34_), .O(z02));
  nand3  g17(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(new_n30_), .c(new_n34_), .O(z03));
  nand4  g19(.a(new_n40_), .b(new_n29_), .c(x01), .d(new_n38_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n30_), .c(new_n34_), .O(z04));
  nand4  g21(.a(new_n26_), .b(new_n29_), .c(x01), .d(x00), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(new_n30_), .c(x05), .d(new_n27_), .O(new_n49_));
  nor2   g24(.a(new_n49_), .b(x13), .O(z05));
  inv1   g25(.a(x12), .O(new_n51_));
  inv1   g26(.a(x13), .O(new_n52_));
  nor2   g27(.a(x09), .b(x01), .O(new_n53_));
  inv1   g28(.a(x01), .O(new_n54_));
  nor2   g29(.a(x11), .b(new_n54_), .O(new_n55_));
  oai21  g30(.a(new_n55_), .b(new_n53_), .c(x00), .O(new_n56_));
  nor2   g31(.a(x08), .b(x01), .O(new_n57_));
  nor2   g32(.a(x10), .b(new_n54_), .O(new_n58_));
  oai21  g33(.a(new_n58_), .b(new_n57_), .c(new_n38_), .O(new_n59_));
  aoi21  g34(.a(new_n59_), .b(new_n56_), .c(new_n52_), .O(new_n60_));
  nand4  g35(.a(new_n60_), .b(new_n51_), .c(x05), .d(new_n29_), .O(new_n61_));
  nand3  g36(.a(new_n52_), .b(new_n34_), .c(x02), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g38(.a(new_n63_), .b(x04), .c(new_n26_), .O(new_n64_));
  nand2  g39(.a(x07), .b(x05), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n64_), .O(z06));
  nor2   g41(.a(x03), .b(x02), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(new_n30_), .c(x05), .d(new_n27_), .O(new_n68_));
  nor2   g43(.a(new_n68_), .b(x13), .O(z07));
  nand2  g44(.a(x05), .b(new_n29_), .O(new_n70_));
  nand3  g45(.a(x13), .b(new_n51_), .c(new_n30_), .O(new_n71_));
  oai21  g46(.a(new_n71_), .b(new_n70_), .c(new_n62_), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(x04), .c(new_n26_), .O(new_n73_));
  inv1   g48(.a(new_n73_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n65_), .O(z09));
  oai21  g51(.a(x07), .b(x02), .c(x05), .O(new_n77_));
  nand3  g52(.a(new_n77_), .b(x06), .c(new_n26_), .O(new_n78_));
  inv1   g53(.a(new_n78_), .O(z10));
endmodule


