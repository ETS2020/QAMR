// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n81_, new_n82_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x04), .O(new_n27_));
  aoi21  g02(.a(new_n27_), .b(x02), .c(x03), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  inv1   g04(.a(x03), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  nand3  g06(.a(x05), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  oai21  g08(.a(new_n28_), .b(x01), .c(new_n33_), .O(z00));
  nand3  g09(.a(x05), .b(new_n31_), .c(new_n29_), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  nand4  g11(.a(new_n26_), .b(x04), .c(x02), .d(new_n36_), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n35_), .c(x03), .O(z01));
  nor3   g13(.a(x02), .b(x01), .c(x00), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(x05), .c(new_n31_), .d(new_n30_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x13), .O(z02));
  nand2  g16(.a(x02), .b(x01), .O(new_n42_));
  nand3  g17(.a(new_n29_), .b(new_n36_), .c(x00), .O(new_n43_));
  inv1   g18(.a(x13), .O(new_n44_));
  nand4  g19(.a(new_n44_), .b(x05), .c(new_n31_), .d(new_n30_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z03));
  inv1   g21(.a(x00), .O(new_n47_));
  nand4  g22(.a(new_n30_), .b(new_n29_), .c(x01), .d(new_n47_), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n44_), .c(x05), .d(new_n31_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z04));
  nand3  g26(.a(new_n44_), .b(x05), .c(new_n31_), .O(new_n52_));
  inv1   g27(.a(new_n52_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n30_), .c(x00), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n29_), .c(new_n36_), .O(z05));
  inv1   g30(.a(x07), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nor3   g32(.a(x09), .b(x02), .c(x01), .O(new_n58_));
  nor2   g33(.a(x11), .b(new_n36_), .O(new_n59_));
  oai21  g34(.a(new_n59_), .b(new_n58_), .c(x00), .O(new_n60_));
  nor3   g35(.a(x08), .b(x02), .c(x01), .O(new_n61_));
  nor2   g36(.a(x10), .b(new_n36_), .O(new_n62_));
  oai21  g37(.a(new_n62_), .b(new_n61_), .c(new_n47_), .O(new_n63_));
  aoi21  g38(.a(new_n63_), .b(new_n60_), .c(new_n44_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n57_), .c(new_n56_), .d(x05), .O(new_n65_));
  nand3  g40(.a(new_n44_), .b(new_n26_), .c(x02), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(x04), .c(new_n30_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n42_), .O(z06));
  nand2  g44(.a(new_n30_), .b(new_n29_), .O(new_n70_));
  oai21  g45(.a(new_n70_), .b(new_n52_), .c(new_n42_), .O(z07));
  nor2   g46(.a(new_n31_), .b(x03), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(new_n44_), .c(new_n26_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n36_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(x02), .O(new_n75_));
  nor2   g50(.a(new_n44_), .b(x12), .O(new_n76_));
  nand4  g51(.a(new_n76_), .b(new_n72_), .c(x05), .d(new_n29_), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n75_), .O(z08));
  nand3  g53(.a(new_n42_), .b(x13), .c(x06), .O(new_n79_));
  inv1   g54(.a(new_n79_), .O(z09));
  nand2  g55(.a(x05), .b(x02), .O(new_n81_));
  nand3  g56(.a(new_n81_), .b(x06), .c(new_n30_), .O(new_n82_));
  nand2  g57(.a(new_n82_), .b(new_n42_), .O(z10));
endmodule


