// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  inv1   g04(.a(x01), .O(new_n30_));
  nand3  g05(.a(x05), .b(new_n27_), .c(new_n30_), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n28_), .c(new_n26_), .O(z00));
  nand2  g08(.a(new_n29_), .b(new_n30_), .O(new_n34_));
  inv1   g09(.a(new_n34_), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(x05), .c(new_n27_), .O(new_n36_));
  inv1   g11(.a(x05), .O(new_n37_));
  nand3  g12(.a(new_n37_), .b(x04), .c(x02), .O(new_n38_));
  aoi21  g13(.a(new_n38_), .b(new_n36_), .c(x03), .O(z01));
  inv1   g14(.a(x13), .O(new_n40_));
  nor2   g15(.a(x02), .b(x00), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(new_n40_), .c(new_n27_), .d(new_n26_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(new_n30_), .c(new_n37_), .O(z02));
  nand4  g18(.a(new_n26_), .b(new_n29_), .c(new_n30_), .d(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n40_), .c(x05), .d(new_n27_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z03));
  nor2   g22(.a(new_n37_), .b(new_n30_), .O(z04));
  inv1   g23(.a(z04), .O(new_n50_));
  inv1   g24(.a(x07), .O(new_n51_));
  inv1   g25(.a(x12), .O(new_n52_));
  inv1   g26(.a(x00), .O(new_n53_));
  inv1   g27(.a(x08), .O(new_n54_));
  nand2  g28(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g29(.a(x09), .O(new_n56_));
  nand2  g30(.a(new_n56_), .b(x00), .O(new_n57_));
  aoi21  g31(.a(new_n57_), .b(new_n55_), .c(new_n40_), .O(new_n58_));
  nand4  g32(.a(new_n58_), .b(new_n52_), .c(new_n51_), .d(x05), .O(new_n59_));
  nand3  g33(.a(new_n40_), .b(new_n37_), .c(x02), .O(new_n60_));
  oai21  g34(.a(new_n59_), .b(x02), .c(new_n60_), .O(new_n61_));
  nand3  g35(.a(new_n61_), .b(x04), .c(new_n26_), .O(new_n62_));
  nand2  g36(.a(new_n62_), .b(new_n50_), .O(z06));
  nand4  g37(.a(new_n35_), .b(x05), .c(new_n27_), .d(new_n26_), .O(new_n64_));
  nor2   g38(.a(new_n64_), .b(x13), .O(z07));
  nand3  g39(.a(x13), .b(new_n52_), .c(x05), .O(new_n66_));
  oai21  g40(.a(new_n66_), .b(new_n34_), .c(new_n60_), .O(new_n67_));
  nand3  g41(.a(new_n67_), .b(x04), .c(new_n26_), .O(new_n68_));
  inv1   g42(.a(new_n68_), .O(z08));
  nand3  g43(.a(new_n50_), .b(x13), .c(x06), .O(new_n70_));
  inv1   g44(.a(new_n70_), .O(z09));
  nand2  g45(.a(x05), .b(x02), .O(new_n72_));
  nand3  g46(.a(new_n72_), .b(x06), .c(new_n26_), .O(new_n73_));
  nand2  g47(.a(new_n73_), .b(new_n50_), .O(z10));
  zero   g48(.O(z05));
endmodule


