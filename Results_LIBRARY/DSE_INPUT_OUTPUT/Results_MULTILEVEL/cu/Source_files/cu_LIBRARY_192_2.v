// Benchmark "FAU" written by ABC on Thu Aug 13 15:09:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_;
  inv1   g00(.a(x02), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand2  g03(.a(x05), .b(new_n28_), .O(new_n29_));
  oai21  g04(.a(new_n29_), .b(x03), .c(new_n26_), .O(new_n30_));
  oai21  g05(.a(new_n27_), .b(new_n26_), .c(new_n30_), .O(z00));
  nand2  g06(.a(new_n27_), .b(x02), .O(new_n32_));
  nor2   g07(.a(x03), .b(x02), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  oai21  g09(.a(new_n34_), .b(new_n29_), .c(new_n32_), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x01), .O(new_n37_));
  nand3  g12(.a(new_n26_), .b(new_n37_), .c(new_n36_), .O(new_n38_));
  inv1   g13(.a(x03), .O(new_n39_));
  inv1   g14(.a(x13), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(x05), .c(new_n28_), .d(new_n39_), .O(new_n41_));
  oai21  g16(.a(new_n41_), .b(new_n38_), .c(new_n32_), .O(z02));
  nand4  g17(.a(new_n39_), .b(new_n26_), .c(new_n37_), .d(x00), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand4  g19(.a(new_n44_), .b(new_n40_), .c(x05), .d(new_n28_), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(z03));
  nand4  g21(.a(new_n39_), .b(new_n26_), .c(x01), .d(new_n36_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(new_n40_), .c(x05), .d(new_n28_), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(z04));
  nor3   g25(.a(x02), .b(new_n37_), .c(new_n36_), .O(new_n51_));
  nand4  g26(.a(new_n51_), .b(x05), .c(new_n28_), .d(new_n39_), .O(new_n52_));
  nor2   g27(.a(new_n52_), .b(x13), .O(z05));
  inv1   g28(.a(x07), .O(new_n54_));
  inv1   g29(.a(x12), .O(new_n55_));
  nor2   g30(.a(x09), .b(x01), .O(new_n56_));
  nor2   g31(.a(x11), .b(new_n37_), .O(new_n57_));
  oai21  g32(.a(new_n57_), .b(new_n56_), .c(x00), .O(new_n58_));
  nor2   g33(.a(x08), .b(x01), .O(new_n59_));
  nor2   g34(.a(x10), .b(new_n37_), .O(new_n60_));
  oai21  g35(.a(new_n60_), .b(new_n59_), .c(new_n36_), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(x13), .c(new_n55_), .d(new_n54_), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(new_n27_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(x04), .c(new_n39_), .d(new_n26_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n32_), .O(z06));
  nand3  g41(.a(new_n40_), .b(x05), .c(new_n28_), .O(new_n67_));
  oai21  g42(.a(new_n67_), .b(new_n34_), .c(new_n32_), .O(z07));
  nand3  g43(.a(x04), .b(new_n39_), .c(new_n26_), .O(new_n69_));
  nand3  g44(.a(x13), .b(new_n55_), .c(x05), .O(new_n70_));
  oai21  g45(.a(new_n70_), .b(new_n69_), .c(new_n32_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n32_), .O(z09));
  nand2  g48(.a(new_n33_), .b(x06), .O(new_n74_));
  inv1   g49(.a(new_n74_), .O(z10));
endmodule


