// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n78_, new_n79_;
  nor2   g00(.a(x13), .b(x01), .O(z03));
  inv1   g01(.a(x04), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nor2   g05(.a(new_n30_), .b(x04), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n29_), .c(x03), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n28_), .c(z03), .O(z00));
  inv1   g09(.a(z03), .O(new_n35_));
  nand3  g10(.a(new_n30_), .b(x04), .c(x02), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(new_n37_));
  aoi21  g12(.a(new_n31_), .b(new_n29_), .c(new_n37_), .O(new_n38_));
  oai21  g13(.a(new_n38_), .b(x03), .c(new_n35_), .O(z01));
  inv1   g14(.a(x13), .O(new_n41_));
  inv1   g15(.a(x00), .O(new_n42_));
  inv1   g16(.a(x03), .O(new_n43_));
  nand4  g17(.a(new_n43_), .b(new_n29_), .c(x01), .d(new_n42_), .O(new_n44_));
  inv1   g18(.a(new_n44_), .O(new_n45_));
  nand4  g19(.a(new_n45_), .b(new_n41_), .c(x05), .d(new_n27_), .O(new_n46_));
  inv1   g20(.a(new_n46_), .O(z04));
  nand4  g21(.a(new_n31_), .b(new_n43_), .c(new_n29_), .d(x00), .O(new_n48_));
  aoi21  g22(.a(new_n48_), .b(x01), .c(x13), .O(z05));
  inv1   g23(.a(x07), .O(new_n50_));
  inv1   g24(.a(x01), .O(new_n51_));
  inv1   g25(.a(x09), .O(new_n52_));
  nand2  g26(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g27(.a(x11), .O(new_n54_));
  nand3  g28(.a(x13), .b(new_n54_), .c(x01), .O(new_n55_));
  nand2  g29(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g30(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g31(.a(x08), .O(new_n58_));
  nand2  g32(.a(new_n58_), .b(new_n51_), .O(new_n59_));
  inv1   g33(.a(x10), .O(new_n60_));
  nand3  g34(.a(x13), .b(new_n60_), .c(x01), .O(new_n61_));
  nand2  g35(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand2  g36(.a(new_n62_), .b(new_n42_), .O(new_n63_));
  aoi21  g37(.a(new_n63_), .b(new_n57_), .c(x12), .O(new_n64_));
  nand4  g38(.a(new_n64_), .b(new_n50_), .c(x05), .d(new_n29_), .O(new_n65_));
  nand3  g39(.a(new_n41_), .b(new_n30_), .c(x02), .O(new_n66_));
  nand2  g40(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g41(.a(new_n67_), .b(x04), .c(new_n43_), .O(new_n68_));
  nand2  g42(.a(new_n68_), .b(new_n35_), .O(z06));
  nand4  g43(.a(new_n27_), .b(new_n43_), .c(new_n29_), .d(x01), .O(new_n70_));
  nor3   g44(.a(new_n70_), .b(x13), .c(new_n30_), .O(z07));
  inv1   g45(.a(x12), .O(new_n72_));
  nand4  g46(.a(x13), .b(new_n72_), .c(x05), .d(new_n29_), .O(new_n73_));
  nand2  g47(.a(new_n73_), .b(new_n66_), .O(new_n74_));
  nand3  g48(.a(new_n74_), .b(x04), .c(new_n43_), .O(new_n75_));
  nand2  g49(.a(new_n75_), .b(new_n35_), .O(z08));
  and2   g50(.a(x13), .b(x06), .O(z09));
  nand2  g51(.a(x05), .b(x02), .O(new_n78_));
  nand3  g52(.a(new_n78_), .b(x06), .c(new_n43_), .O(new_n79_));
  nand2  g53(.a(new_n79_), .b(new_n35_), .O(z10));
  zero   g54(.O(z02));
endmodule


