// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  nand2  g04(.a(x07), .b(x05), .O(new_n30_));
  oai21  g05(.a(new_n30_), .b(x04), .c(new_n29_), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(z00));
  inv1   g07(.a(x05), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  nand4  g09(.a(x07), .b(x05), .c(new_n27_), .d(new_n29_), .O(new_n35_));
  aoi21  g10(.a(new_n35_), .b(new_n34_), .c(x03), .O(z01));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x01), .O(new_n38_));
  nor3   g13(.a(x13), .b(x04), .c(x03), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(new_n29_), .c(new_n38_), .d(new_n37_), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(x07), .c(new_n33_), .O(z02));
  nand4  g16(.a(new_n26_), .b(new_n29_), .c(new_n38_), .d(x00), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  nand4  g18(.a(new_n43_), .b(x07), .c(x05), .d(new_n27_), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x13), .O(z03));
  nand4  g20(.a(new_n39_), .b(new_n29_), .c(x01), .d(new_n37_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(x07), .c(new_n33_), .O(z04));
  nand4  g22(.a(new_n26_), .b(new_n29_), .c(x01), .d(x00), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(x07), .c(x05), .d(new_n27_), .O(new_n50_));
  nor2   g25(.a(new_n50_), .b(x13), .O(z05));
  inv1   g26(.a(x07), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x05), .O(new_n53_));
  nand2  g28(.a(new_n26_), .b(x02), .O(new_n54_));
  inv1   g29(.a(x13), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n33_), .c(x04), .O(new_n56_));
  oai21  g31(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z06));
  nand4  g32(.a(new_n55_), .b(new_n27_), .c(new_n26_), .d(new_n29_), .O(new_n58_));
  aoi21  g33(.a(new_n58_), .b(x07), .c(new_n33_), .O(z07));
  inv1   g34(.a(x12), .O(new_n60_));
  nand4  g35(.a(x13), .b(new_n60_), .c(x05), .d(new_n29_), .O(new_n61_));
  nand3  g36(.a(new_n55_), .b(new_n33_), .c(x02), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g38(.a(new_n63_), .b(x04), .c(new_n26_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n53_), .O(z08));
  nand2  g40(.a(x13), .b(x06), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n53_), .O(z09));
  oai21  g42(.a(new_n52_), .b(x02), .c(x05), .O(new_n68_));
  nand3  g43(.a(new_n68_), .b(x06), .c(new_n26_), .O(new_n69_));
  inv1   g44(.a(new_n69_), .O(z10));
endmodule


