// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x01), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(x04), .O(new_n28_));
  aoi21  g03(.a(new_n28_), .b(x02), .c(x03), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  inv1   g05(.a(x03), .O(new_n31_));
  inv1   g06(.a(x04), .O(new_n32_));
  nand3  g07(.a(x05), .b(new_n32_), .c(new_n31_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  oai21  g09(.a(new_n29_), .b(new_n26_), .c(new_n34_), .O(z00));
  nand3  g10(.a(x05), .b(new_n32_), .c(new_n30_), .O(new_n36_));
  nand4  g11(.a(new_n27_), .b(x04), .c(x02), .d(x01), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n36_), .c(x03), .O(z01));
  nor3   g13(.a(x02), .b(x01), .c(x00), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(x05), .c(new_n32_), .d(new_n31_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x13), .O(z02));
  inv1   g16(.a(x13), .O(new_n42_));
  nand3  g17(.a(new_n42_), .b(x05), .c(new_n32_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n31_), .c(x00), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n30_), .c(x01), .O(z03));
  inv1   g21(.a(x00), .O(new_n47_));
  nand4  g22(.a(new_n31_), .b(new_n30_), .c(x01), .d(new_n47_), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n42_), .c(x05), .d(new_n32_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z04));
  nor2   g26(.a(new_n30_), .b(x01), .O(new_n52_));
  inv1   g27(.a(new_n52_), .O(new_n53_));
  nand3  g28(.a(new_n30_), .b(x01), .c(x00), .O(new_n54_));
  nand4  g29(.a(new_n42_), .b(x05), .c(new_n32_), .d(new_n31_), .O(new_n55_));
  oai21  g30(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z05));
  inv1   g31(.a(x07), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  inv1   g33(.a(x09), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n26_), .O(new_n60_));
  inv1   g35(.a(x11), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n30_), .c(x01), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(x00), .O(new_n64_));
  inv1   g39(.a(x08), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n26_), .O(new_n66_));
  inv1   g41(.a(x10), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(new_n30_), .c(x01), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n47_), .O(new_n70_));
  aoi21  g45(.a(new_n70_), .b(new_n64_), .c(new_n42_), .O(new_n71_));
  nand4  g46(.a(new_n71_), .b(new_n58_), .c(new_n57_), .d(x05), .O(new_n72_));
  nand3  g47(.a(new_n42_), .b(new_n27_), .c(x02), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g49(.a(new_n74_), .b(x04), .c(new_n31_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n53_), .O(z06));
  nand2  g51(.a(new_n31_), .b(new_n30_), .O(new_n77_));
  oai21  g52(.a(new_n77_), .b(new_n43_), .c(new_n53_), .O(z07));
  nor2   g53(.a(new_n32_), .b(x03), .O(new_n79_));
  nand3  g54(.a(new_n79_), .b(new_n42_), .c(new_n27_), .O(new_n80_));
  nand2  g55(.a(new_n80_), .b(x01), .O(new_n81_));
  nand2  g56(.a(new_n81_), .b(x02), .O(new_n82_));
  nor2   g57(.a(new_n42_), .b(x12), .O(new_n83_));
  nand4  g58(.a(new_n83_), .b(new_n79_), .c(x05), .d(new_n30_), .O(new_n84_));
  nand2  g59(.a(new_n84_), .b(new_n82_), .O(z08));
  inv1   g60(.a(x06), .O(new_n86_));
  nor3   g61(.a(new_n52_), .b(new_n42_), .c(new_n86_), .O(z09));
  nand2  g62(.a(x05), .b(x02), .O(new_n88_));
  nand3  g63(.a(new_n88_), .b(x06), .c(new_n31_), .O(new_n89_));
  nand2  g64(.a(new_n89_), .b(new_n53_), .O(z10));
endmodule


