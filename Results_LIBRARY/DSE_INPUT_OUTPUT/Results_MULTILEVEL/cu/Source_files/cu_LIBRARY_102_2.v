// Benchmark "FAU" written by ABC on Thu Aug 13 15:08:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  oai21  g03(.a(new_n28_), .b(x04), .c(new_n27_), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n26_), .O(z00));
  inv1   g05(.a(x04), .O(new_n31_));
  nor2   g06(.a(x03), .b(x02), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(x05), .c(new_n31_), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(z01));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nand3  g12(.a(new_n37_), .b(x05), .c(new_n31_), .O(new_n38_));
  inv1   g13(.a(new_n38_), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n36_), .c(new_n35_), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(new_n27_), .c(x03), .O(z02));
  nand4  g16(.a(new_n26_), .b(new_n27_), .c(new_n36_), .d(x00), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  nand4  g18(.a(new_n43_), .b(new_n37_), .c(x05), .d(new_n31_), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(z03));
  nand3  g20(.a(new_n39_), .b(x01), .c(new_n35_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n27_), .c(x03), .O(z04));
  nor3   g22(.a(x02), .b(new_n36_), .c(new_n35_), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(x05), .c(new_n31_), .d(new_n26_), .O(new_n49_));
  nor2   g24(.a(new_n49_), .b(x13), .O(z05));
  inv1   g25(.a(x07), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  aoi21  g30(.a(new_n55_), .b(new_n53_), .c(new_n35_), .O(new_n56_));
  inv1   g31(.a(x08), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n36_), .O(new_n58_));
  inv1   g33(.a(x10), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  aoi21  g35(.a(new_n60_), .b(new_n58_), .c(x00), .O(new_n61_));
  oai21  g36(.a(new_n61_), .b(new_n56_), .c(x13), .O(new_n62_));
  nor2   g37(.a(new_n62_), .b(x12), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(new_n51_), .c(x05), .d(x04), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n27_), .c(x03), .O(z06));
  nor2   g40(.a(new_n33_), .b(x13), .O(z07));
  nand3  g41(.a(new_n32_), .b(x05), .c(x04), .O(new_n67_));
  nor3   g42(.a(new_n67_), .b(new_n37_), .c(x12), .O(z08));
  inv1   g43(.a(x06), .O(new_n69_));
  oai21  g44(.a(x03), .b(new_n27_), .c(x13), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n69_), .O(z09));
  aoi21  g46(.a(new_n69_), .b(new_n27_), .c(x03), .O(z10));
endmodule


