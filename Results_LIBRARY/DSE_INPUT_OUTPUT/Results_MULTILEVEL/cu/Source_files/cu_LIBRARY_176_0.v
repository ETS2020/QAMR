// Benchmark "FAU" written by ABC on Thu Aug 13 15:09:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n73_, new_n74_;
  inv1   g00(.a(x05), .O(new_n26_));
  inv1   g01(.a(x13), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x03), .O(new_n29_));
  oai21  g04(.a(x04), .b(x02), .c(x05), .O(new_n30_));
  nand2  g05(.a(x04), .b(x02), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(x13), .c(new_n26_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n30_), .c(new_n29_), .O(z00));
  inv1   g08(.a(x02), .O(new_n34_));
  inv1   g09(.a(x04), .O(new_n35_));
  nand3  g10(.a(x05), .b(new_n35_), .c(new_n34_), .O(new_n36_));
  nand2  g11(.a(x13), .b(new_n26_), .O(new_n37_));
  or2    g12(.a(new_n37_), .b(new_n31_), .O(new_n38_));
  aoi21  g13(.a(new_n38_), .b(new_n36_), .c(x03), .O(z01));
  inv1   g14(.a(x00), .O(new_n40_));
  inv1   g15(.a(x01), .O(new_n41_));
  nor3   g16(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  nand3  g17(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(x05), .c(x13), .O(z02));
  nand3  g19(.a(new_n42_), .b(new_n41_), .c(x00), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x05), .c(x13), .O(z03));
  inv1   g21(.a(x03), .O(new_n47_));
  nand4  g22(.a(new_n47_), .b(new_n34_), .c(x01), .d(new_n40_), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n27_), .c(x05), .d(new_n35_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z04));
  nand3  g26(.a(new_n42_), .b(x01), .c(x00), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(x05), .c(x13), .O(z05));
  inv1   g28(.a(x07), .O(new_n54_));
  inv1   g29(.a(x12), .O(new_n55_));
  nor2   g30(.a(x09), .b(x01), .O(new_n56_));
  nor2   g31(.a(x11), .b(new_n41_), .O(new_n57_));
  oai21  g32(.a(new_n57_), .b(new_n56_), .c(x00), .O(new_n58_));
  nor2   g33(.a(x08), .b(x01), .O(new_n59_));
  nor2   g34(.a(x10), .b(new_n41_), .O(new_n60_));
  oai21  g35(.a(new_n60_), .b(new_n59_), .c(new_n40_), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(x13), .c(new_n55_), .d(new_n54_), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(new_n26_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(x04), .c(new_n47_), .d(new_n34_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n28_), .O(z06));
  inv1   g41(.a(new_n42_), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(x05), .c(x13), .O(z07));
  nor2   g43(.a(x03), .b(x02), .O(new_n69_));
  nand4  g44(.a(new_n69_), .b(new_n55_), .c(x05), .d(x04), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n27_), .O(z08));
  and2   g46(.a(x13), .b(x06), .O(z09));
  oai21  g47(.a(new_n26_), .b(x02), .c(new_n37_), .O(new_n73_));
  nand3  g48(.a(new_n73_), .b(x06), .c(new_n47_), .O(new_n74_));
  inv1   g49(.a(new_n74_), .O(z10));
endmodule


