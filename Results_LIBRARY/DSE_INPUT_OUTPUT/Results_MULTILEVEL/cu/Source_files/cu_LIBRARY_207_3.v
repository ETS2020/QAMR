// Benchmark "FAU" written by ABC on Thu Aug 13 15:09:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_;
  inv1   g00(.a(x00), .O(new_n26_));
  inv1   g01(.a(x13), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  oai21  g04(.a(x05), .b(new_n29_), .c(x02), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(x04), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  aoi21  g09(.a(new_n34_), .b(new_n31_), .c(x03), .O(new_n35_));
  aoi21  g10(.a(new_n35_), .b(new_n30_), .c(new_n28_), .O(z00));
  inv1   g11(.a(x03), .O(new_n37_));
  inv1   g12(.a(new_n28_), .O(new_n38_));
  nand3  g13(.a(new_n32_), .b(x04), .c(x02), .O(new_n39_));
  oai21  g14(.a(new_n34_), .b(x02), .c(new_n39_), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(z01));
  inv1   g17(.a(x01), .O(new_n43_));
  nand3  g18(.a(new_n31_), .b(new_n43_), .c(new_n26_), .O(new_n44_));
  nand4  g19(.a(new_n27_), .b(x05), .c(new_n29_), .d(new_n37_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(z02));
  nand4  g21(.a(new_n33_), .b(new_n37_), .c(new_n31_), .d(new_n43_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(new_n27_), .c(new_n26_), .O(z03));
  nand4  g23(.a(new_n37_), .b(new_n31_), .c(x01), .d(new_n26_), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n27_), .c(x05), .d(new_n29_), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(z04));
  nand4  g27(.a(new_n33_), .b(new_n37_), .c(new_n31_), .d(x01), .O(new_n53_));
  aoi21  g28(.a(new_n53_), .b(new_n27_), .c(new_n26_), .O(z05));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  inv1   g31(.a(x08), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n43_), .O(new_n58_));
  inv1   g33(.a(x10), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand4  g36(.a(new_n61_), .b(x13), .c(new_n56_), .d(new_n55_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(x05), .c(new_n31_), .d(new_n26_), .O(new_n64_));
  nand3  g39(.a(new_n27_), .b(new_n32_), .c(x02), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(x04), .c(new_n37_), .O(new_n67_));
  inv1   g42(.a(new_n67_), .O(z06));
  nand4  g43(.a(x05), .b(new_n29_), .c(new_n37_), .d(new_n31_), .O(new_n69_));
  nor2   g44(.a(new_n69_), .b(x13), .O(z07));
  nand4  g45(.a(x13), .b(new_n56_), .c(x05), .d(new_n31_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(x04), .c(new_n37_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n38_), .O(z08));
  nand3  g49(.a(x13), .b(x06), .c(new_n26_), .O(new_n75_));
  inv1   g50(.a(new_n75_), .O(z09));
  nand2  g51(.a(x05), .b(x02), .O(new_n77_));
  nand4  g52(.a(new_n77_), .b(new_n38_), .c(x06), .d(new_n37_), .O(new_n78_));
  inv1   g53(.a(new_n78_), .O(z10));
endmodule


