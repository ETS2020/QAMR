// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x19), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x15), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  nand3  g10(.a(new_n48_), .b(new_n55_), .c(new_n54_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(new_n49_), .b(new_n63_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n60_), .O(z01));
  nand2  g21(.a(new_n60_), .b(new_n49_), .O(z03));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand3  g27(.a(new_n72_), .b(new_n49_), .c(new_n46_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z06));
  nand3  g32(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g33(.a(x23), .O(new_n79_));
  inv1   g34(.a(x24), .O(new_n80_));
  nand3  g35(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n79_), .c(new_n80_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x25), .c(new_n49_), .O(new_n83_));
  nor3   g38(.a(x18), .b(x17), .c(x15), .O(new_n84_));
  inv1   g39(.a(x22), .O(new_n85_));
  nor2   g40(.a(new_n80_), .b(new_n85_), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n84_), .c(x21), .d(new_n48_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n83_), .O(z08));
  inv1   g43(.a(x15), .O(new_n89_));
  nand3  g44(.a(new_n62_), .b(new_n89_), .c(new_n63_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand4  g46(.a(new_n61_), .b(new_n54_), .c(new_n89_), .d(new_n63_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n61_), .c(new_n89_), .d(new_n63_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n49_), .O(z11));
  nor2   g51(.a(new_n55_), .b(new_n54_), .O(new_n97_));
  nand3  g52(.a(new_n48_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n48_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n61_), .c(new_n89_), .d(new_n63_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n61_), .b(new_n63_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nand3  g60(.a(new_n97_), .b(new_n53_), .c(x19), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .d(new_n89_), .O(z13));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n97_), .c(x15), .O(new_n109_));
  aoi21  g64(.a(new_n106_), .b(x21), .c(new_n104_), .O(new_n110_));
  oai22  g65(.a(new_n110_), .b(x15), .c(new_n109_), .d(new_n48_), .O(z14));
  nand3  g66(.a(new_n108_), .b(new_n97_), .c(x19), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x22), .O(new_n113_));
  inv1   g68(.a(new_n102_), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n53_), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n113_), .c(new_n105_), .d(new_n89_), .O(z15));
  nand2  g72(.a(new_n116_), .b(x23), .O(new_n118_));
  nand4  g73(.a(new_n108_), .b(new_n114_), .c(new_n79_), .d(new_n85_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n118_), .c(new_n105_), .d(new_n89_), .O(z16));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n121_));
  inv1   g76(.a(new_n106_), .O(new_n122_));
  nand3  g77(.a(new_n115_), .b(new_n80_), .c(new_n79_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n121_), .c(new_n105_), .d(new_n89_), .O(z17));
  oai21  g81(.a(new_n123_), .b(new_n106_), .c(x25), .O(new_n127_));
  nor3   g82(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n115_), .c(new_n122_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n127_), .c(new_n105_), .d(new_n89_), .O(z18));
endmodule


