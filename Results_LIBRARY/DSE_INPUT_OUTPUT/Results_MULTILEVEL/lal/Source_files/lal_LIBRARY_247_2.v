// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  inv1   g002(.a(x12), .O(new_n48_));
  nor2   g003(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g006(.a(z02), .b(new_n46_), .O(z00));
  inv1   g007(.a(x07), .O(new_n53_));
  inv1   g008(.a(x24), .O(new_n54_));
  inv1   g009(.a(x25), .O(new_n55_));
  inv1   g010(.a(x20), .O(new_n56_));
  inv1   g011(.a(x17), .O(new_n57_));
  inv1   g012(.a(x18), .O(new_n58_));
  inv1   g013(.a(x19), .O(new_n59_));
  nand3  g014(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g015(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  inv1   g016(.a(x21), .O(new_n62_));
  inv1   g017(.a(x22), .O(new_n63_));
  nor2   g018(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g019(.a(new_n64_), .b(new_n61_), .c(x23), .O(new_n65_));
  oai21  g020(.a(new_n65_), .b(new_n54_), .c(new_n55_), .O(new_n66_));
  nand2  g021(.a(x05), .b(x04), .O(new_n67_));
  nand4  g022(.a(new_n67_), .b(new_n66_), .c(new_n50_), .d(new_n53_), .O(z01));
  nand2  g023(.a(new_n66_), .b(new_n50_), .O(z03));
  inv1   g024(.a(x09), .O(new_n70_));
  inv1   g025(.a(x01), .O(new_n71_));
  aoi22  g026(.a(x10), .b(new_n71_), .c(new_n70_), .d(x00), .O(new_n72_));
  oai21  g027(.a(new_n70_), .b(x00), .c(new_n72_), .O(new_n73_));
  inv1   g028(.a(x10), .O(new_n74_));
  nand2  g029(.a(new_n74_), .b(x01), .O(new_n75_));
  inv1   g030(.a(x02), .O(new_n76_));
  nand2  g031(.a(x11), .b(new_n76_), .O(new_n77_));
  inv1   g032(.a(x11), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(x02), .O(new_n79_));
  nand3  g034(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n73_), .c(new_n50_), .O(new_n81_));
  inv1   g036(.a(x15), .O(new_n82_));
  nor3   g037(.a(new_n82_), .b(new_n48_), .c(x03), .O(new_n83_));
  aoi21  g038(.a(new_n48_), .b(x03), .c(new_n83_), .O(new_n84_));
  aoi21  g039(.a(new_n84_), .b(new_n81_), .c(x08), .O(z04));
  oai21  g040(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g041(.a(x14), .b(new_n46_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(new_n50_), .O(z06));
  aoi21  g043(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g044(.a(x23), .O(new_n90_));
  nand3  g045(.a(new_n61_), .b(x22), .c(x21), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(x24), .O(new_n93_));
  aoi21  g048(.a(new_n93_), .b(new_n55_), .c(new_n49_), .O(z08));
  nor4   g049(.a(new_n67_), .b(x15), .c(x12), .d(x07), .O(z09));
  nand3  g050(.a(new_n67_), .b(new_n57_), .c(new_n53_), .O(new_n96_));
  aoi21  g051(.a(new_n96_), .b(new_n48_), .c(x15), .O(z10));
  xor2a  g052(.a(x18), .b(x17), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n67_), .c(new_n82_), .d(new_n48_), .O(new_n99_));
  nor2   g054(.a(new_n99_), .b(x07), .O(z11));
  nand2  g055(.a(x18), .b(x17), .O(new_n101_));
  inv1   g056(.a(new_n101_), .O(new_n102_));
  nand3  g057(.a(new_n59_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g058(.a(new_n102_), .b(new_n59_), .c(new_n103_), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n67_), .c(new_n82_), .d(new_n48_), .O(new_n105_));
  nor2   g060(.a(new_n105_), .b(x07), .O(z12));
  nand3  g061(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g062(.a(new_n107_), .b(x20), .O(new_n108_));
  nor2   g063(.a(x20), .b(new_n59_), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(new_n102_), .O(new_n110_));
  nand4  g065(.a(new_n110_), .b(new_n108_), .c(new_n67_), .d(new_n53_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n48_), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(new_n82_), .O(z13));
  nand2  g068(.a(new_n110_), .b(x21), .O(new_n114_));
  aoi21  g069(.a(x05), .b(x04), .c(x07), .O(new_n115_));
  nor2   g070(.a(x21), .b(x20), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n102_), .c(x19), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(new_n82_), .c(new_n48_), .O(new_n118_));
  inv1   g073(.a(new_n118_), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n115_), .c(new_n114_), .O(z14));
  nand2  g075(.a(new_n117_), .b(x22), .O(new_n121_));
  inv1   g076(.a(new_n107_), .O(new_n122_));
  nand2  g077(.a(new_n63_), .b(new_n62_), .O(new_n123_));
  nor2   g078(.a(new_n123_), .b(x20), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand4  g080(.a(new_n125_), .b(new_n121_), .c(new_n67_), .d(new_n53_), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n48_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n82_), .O(z15));
  aoi21  g083(.a(new_n124_), .b(new_n122_), .c(new_n90_), .O(new_n129_));
  nand4  g084(.a(new_n116_), .b(new_n122_), .c(new_n90_), .d(new_n63_), .O(new_n130_));
  nand3  g085(.a(new_n130_), .b(new_n67_), .c(new_n53_), .O(new_n131_));
  oai21  g086(.a(new_n131_), .b(new_n129_), .c(new_n48_), .O(new_n132_));
  nand2  g087(.a(new_n132_), .b(new_n82_), .O(z16));
  nand3  g088(.a(new_n116_), .b(new_n90_), .c(new_n63_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(new_n107_), .c(x24), .O(new_n135_));
  inv1   g090(.a(new_n123_), .O(new_n136_));
  nor2   g091(.a(x24), .b(x23), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(new_n136_), .c(new_n109_), .d(new_n102_), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n135_), .c(new_n67_), .d(new_n53_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n48_), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(new_n82_), .O(z17));
  nand2  g096(.a(new_n138_), .b(x25), .O(new_n142_));
  inv1   g097(.a(new_n110_), .O(new_n143_));
  nand3  g098(.a(new_n55_), .b(new_n54_), .c(new_n90_), .O(new_n144_));
  nor2   g099(.a(new_n144_), .b(new_n123_), .O(new_n145_));
  aoi21  g100(.a(new_n145_), .b(new_n143_), .c(x15), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(new_n142_), .c(new_n115_), .d(new_n48_), .O(z18));
endmodule


