// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x18), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x21), .O(new_n49_));
  inv1   g04(.a(x22), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  oai21  g06(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(x24), .c(x25), .O(new_n53_));
  inv1   g08(.a(x07), .O(new_n54_));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x25), .O(new_n59_));
  nor2   g14(.a(x19), .b(x17), .O(new_n60_));
  inv1   g15(.a(x15), .O(new_n61_));
  nand2  g16(.a(x18), .b(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n60_), .b(x18), .c(new_n62_), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n59_), .c(new_n51_), .d(new_n58_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n57_), .O(z01));
  and2   g20(.a(new_n47_), .b(x16), .O(z02));
  nor3   g21(.a(x25), .b(x23), .c(x20), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x15), .c(x18), .O(new_n68_));
  inv1   g23(.a(x24), .O(new_n69_));
  nor2   g24(.a(new_n50_), .b(new_n49_), .O(new_n70_));
  oai21  g25(.a(new_n60_), .b(x20), .c(new_n70_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n51_), .c(new_n69_), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x25), .c(new_n68_), .O(z03));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n47_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g36(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z06));
  nand3  g38(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g39(.a(x17), .O(new_n85_));
  inv1   g40(.a(x18), .O(new_n86_));
  inv1   g41(.a(x19), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(new_n58_), .c(new_n50_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x21), .c(x23), .O(new_n90_));
  aoi21  g45(.a(x18), .b(x15), .c(x25), .O(new_n91_));
  oai21  g46(.a(new_n90_), .b(new_n69_), .c(new_n91_), .O(z08));
  nand2  g47(.a(new_n61_), .b(new_n54_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n55_), .c(new_n47_), .O(z09));
  nand4  g49(.a(new_n55_), .b(new_n85_), .c(new_n61_), .d(new_n54_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  xor2a  g51(.a(x18), .b(x17), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n55_), .c(new_n61_), .d(new_n54_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  nand2  g54(.a(x18), .b(x17), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(x19), .c(new_n61_), .O(new_n101_));
  nand3  g56(.a(new_n87_), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n55_), .c(new_n54_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(z12));
  inv1   g60(.a(new_n56_), .O(new_n106_));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  inv1   g63(.a(new_n100_), .O(new_n109_));
  nor2   g64(.a(x20), .b(new_n87_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n108_), .c(new_n106_), .d(new_n61_), .O(z13));
  oai21  g67(.a(x21), .b(x15), .c(new_n86_), .O(new_n113_));
  nand3  g68(.a(new_n58_), .b(x19), .c(x17), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x21), .O(new_n115_));
  nor2   g70(.a(x21), .b(x20), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n109_), .c(x19), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n115_), .c(new_n55_), .d(new_n54_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n61_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n113_), .O(z14));
  nand2  g75(.a(new_n117_), .b(x22), .O(new_n121_));
  inv1   g76(.a(new_n107_), .O(new_n122_));
  nor2   g77(.a(x22), .b(x21), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n58_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n121_), .c(new_n106_), .d(new_n61_), .O(z15));
  nand2  g80(.a(new_n124_), .b(x23), .O(new_n126_));
  nand4  g81(.a(new_n116_), .b(new_n122_), .c(new_n51_), .d(new_n50_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n126_), .c(new_n106_), .d(new_n61_), .O(z16));
  oai21  g83(.a(x24), .b(x15), .c(new_n86_), .O(new_n129_));
  nand3  g84(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n114_), .c(x24), .O(new_n131_));
  nor2   g86(.a(x24), .b(x23), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n123_), .c(new_n110_), .d(new_n109_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n131_), .c(new_n55_), .d(new_n54_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n61_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n129_), .O(z17));
  nand2  g91(.a(new_n133_), .b(x25), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n123_), .c(new_n110_), .d(new_n109_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n137_), .c(new_n106_), .d(new_n61_), .O(z18));
endmodule


