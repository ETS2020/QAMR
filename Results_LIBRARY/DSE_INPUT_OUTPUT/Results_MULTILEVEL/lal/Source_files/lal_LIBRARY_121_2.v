// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x12), .O(new_n48_));
  nor2   g03(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n62_), .c(new_n49_), .O(z01));
  nand3  g19(.a(new_n61_), .b(new_n50_), .c(new_n53_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  oai21  g21(.a(x08), .b(x03), .c(x15), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(x12), .O(new_n68_));
  inv1   g23(.a(x09), .O(new_n69_));
  inv1   g24(.a(x01), .O(new_n70_));
  aoi22  g25(.a(x10), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x10), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x01), .O(new_n74_));
  inv1   g29(.a(x02), .O(new_n75_));
  nand2  g30(.a(x11), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  nand2  g33(.a(new_n48_), .b(x03), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n78_), .c(new_n76_), .d(new_n74_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n72_), .c(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n68_), .O(z04));
  nor3   g37(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  inv1   g38(.a(x14), .O(new_n84_));
  nor3   g39(.a(new_n49_), .b(new_n84_), .c(x08), .O(z06));
  nand3  g40(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g41(.a(x24), .O(new_n87_));
  inv1   g42(.a(x22), .O(new_n88_));
  aoi21  g43(.a(new_n58_), .b(new_n54_), .c(new_n88_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x21), .c(x23), .O(new_n90_));
  nor2   g45(.a(new_n49_), .b(x25), .O(new_n91_));
  oai21  g46(.a(new_n90_), .b(new_n87_), .c(new_n91_), .O(z08));
  nand2  g47(.a(x05), .b(x04), .O(new_n93_));
  nor4   g48(.a(new_n93_), .b(x15), .c(x12), .d(x07), .O(z09));
  inv1   g49(.a(x15), .O(new_n95_));
  nand4  g50(.a(new_n93_), .b(new_n55_), .c(new_n95_), .d(new_n48_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(x07), .O(z10));
  xor2a  g52(.a(x18), .b(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n93_), .c(new_n95_), .d(new_n48_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(x07), .O(z11));
  inv1   g55(.a(x07), .O(new_n101_));
  nor2   g56(.a(new_n56_), .b(new_n55_), .O(new_n102_));
  nand3  g57(.a(new_n57_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n57_), .c(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n93_), .c(new_n101_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n48_), .c(x15), .O(z12));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nand4  g63(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n108_), .c(new_n93_), .d(new_n101_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n48_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n95_), .O(z13));
  and2   g67(.a(new_n109_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n102_), .c(x19), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n93_), .c(new_n101_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n113_), .c(new_n48_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n95_), .O(z14));
  nand2  g73(.a(new_n115_), .b(x22), .O(new_n119_));
  inv1   g74(.a(new_n107_), .O(new_n120_));
  nor2   g75(.a(x22), .b(x21), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n54_), .O(new_n122_));
  and2   g77(.a(new_n122_), .b(new_n95_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n119_), .c(new_n63_), .d(new_n48_), .O(z15));
  nand2  g79(.a(new_n122_), .b(x23), .O(new_n125_));
  inv1   g80(.a(x23), .O(new_n126_));
  nand4  g81(.a(new_n114_), .b(new_n120_), .c(new_n126_), .d(new_n88_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n95_), .c(new_n48_), .O(new_n128_));
  inv1   g83(.a(new_n128_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n125_), .c(new_n63_), .O(z16));
  nand2  g85(.a(new_n127_), .b(x24), .O(new_n131_));
  inv1   g86(.a(new_n109_), .O(new_n132_));
  nand3  g87(.a(new_n121_), .b(new_n87_), .c(new_n126_), .O(new_n133_));
  inv1   g88(.a(new_n133_), .O(new_n134_));
  aoi21  g89(.a(new_n134_), .b(new_n132_), .c(x15), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n131_), .c(new_n63_), .d(new_n48_), .O(z17));
  oai21  g91(.a(new_n133_), .b(new_n109_), .c(x25), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand3  g93(.a(new_n138_), .b(new_n121_), .c(new_n132_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n137_), .c(new_n93_), .d(new_n101_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n48_), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(new_n95_), .O(z18));
endmodule


