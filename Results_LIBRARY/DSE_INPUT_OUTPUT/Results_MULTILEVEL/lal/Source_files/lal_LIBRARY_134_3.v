// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n114_, new_n116_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  inv1   g02(.a(x19), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x22), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(x24), .c(x25), .O(new_n56_));
  inv1   g11(.a(x07), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n56_), .c(new_n50_), .O(new_n60_));
  inv1   g15(.a(x20), .O(new_n61_));
  inv1   g16(.a(x25), .O(new_n62_));
  nor2   g17(.a(x19), .b(x17), .O(new_n63_));
  nand2  g18(.a(new_n48_), .b(x18), .O(new_n64_));
  oai21  g19(.a(new_n63_), .b(x18), .c(new_n64_), .O(new_n65_));
  nand4  g20(.a(new_n65_), .b(new_n62_), .c(new_n54_), .d(new_n61_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n60_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n49_), .b(new_n68_), .O(z02));
  nor3   g24(.a(x25), .b(x23), .c(x20), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x19), .c(x18), .O(new_n71_));
  inv1   g26(.a(x24), .O(new_n72_));
  and2   g27(.a(x22), .b(x21), .O(new_n73_));
  oai21  g28(.a(new_n63_), .b(x20), .c(new_n73_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n54_), .c(new_n72_), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x25), .c(new_n71_), .O(z03));
  xnor2a g31(.a(x09), .b(x00), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  xnor2a g33(.a(x11), .b(x02), .O(new_n79_));
  xnor2a g34(.a(x12), .b(x03), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n77_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n50_), .O(z04));
  inv1   g38(.a(x13), .O(new_n84_));
  nand3  g39(.a(new_n50_), .b(new_n84_), .c(new_n46_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z05));
  nand2  g41(.a(x14), .b(new_n46_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n50_), .O(z06));
  aoi21  g43(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g44(.a(x17), .O(new_n90_));
  nand3  g45(.a(new_n48_), .b(new_n47_), .c(new_n90_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n61_), .c(new_n53_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(x21), .c(x23), .O(new_n93_));
  nor2   g48(.a(new_n49_), .b(x25), .O(new_n94_));
  oai21  g49(.a(new_n93_), .b(new_n72_), .c(new_n94_), .O(z08));
  inv1   g50(.a(x15), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n57_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n58_), .c(new_n50_), .O(z09));
  nand4  g53(.a(new_n58_), .b(new_n50_), .c(new_n90_), .d(new_n96_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(x07), .O(z10));
  xor2a  g55(.a(x18), .b(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n58_), .c(new_n96_), .d(new_n57_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n50_), .O(z11));
  oai21  g58(.a(new_n47_), .b(new_n90_), .c(new_n48_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n58_), .c(new_n96_), .d(new_n57_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n50_), .O(z12));
  nand3  g61(.a(new_n58_), .b(new_n50_), .c(new_n57_), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n61_), .c(new_n96_), .O(z13));
  nand3  g64(.a(new_n108_), .b(new_n52_), .c(new_n96_), .O(z14));
  inv1   g65(.a(new_n59_), .O(new_n111_));
  nor2   g66(.a(x22), .b(x15), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n111_), .c(new_n49_), .O(z15));
  nor2   g68(.a(x23), .b(x15), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n111_), .c(new_n49_), .O(z16));
  nor2   g70(.a(x24), .b(x15), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n111_), .c(new_n49_), .O(z17));
  nand3  g72(.a(new_n108_), .b(new_n62_), .c(new_n96_), .O(z18));
endmodule


