// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x18), .O(new_n47_));
  nor2   g002(.a(x19), .b(new_n47_), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x23), .O(new_n51_));
  nand2  g006(.a(x22), .b(x21), .O(new_n52_));
  nand2  g007(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g008(.a(new_n53_), .b(x24), .c(x25), .O(new_n54_));
  inv1   g009(.a(x07), .O(new_n55_));
  nand2  g010(.a(x05), .b(x04), .O(new_n56_));
  nand2  g011(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g012(.a(new_n57_), .b(new_n54_), .c(new_n49_), .O(new_n58_));
  inv1   g013(.a(x20), .O(new_n59_));
  inv1   g014(.a(x19), .O(new_n60_));
  nand2  g015(.a(new_n47_), .b(x17), .O(new_n61_));
  aoi21  g016(.a(new_n61_), .b(new_n60_), .c(x25), .O(new_n62_));
  nand3  g017(.a(new_n62_), .b(new_n51_), .c(new_n59_), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(new_n58_), .O(z01));
  inv1   g019(.a(x16), .O(new_n65_));
  nand2  g020(.a(new_n49_), .b(new_n65_), .O(z02));
  aoi21  g021(.a(new_n61_), .b(new_n60_), .c(x20), .O(new_n67_));
  aoi22  g022(.a(x22), .b(x21), .c(new_n60_), .d(x18), .O(new_n68_));
  oai21  g023(.a(new_n68_), .b(new_n67_), .c(new_n51_), .O(new_n69_));
  inv1   g024(.a(x24), .O(new_n70_));
  nand2  g025(.a(new_n49_), .b(new_n70_), .O(new_n71_));
  aoi21  g026(.a(new_n71_), .b(new_n69_), .c(x25), .O(z03));
  xnor2a g027(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g028(.a(x10), .b(x01), .O(new_n74_));
  xnor2a g029(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g030(.a(x12), .b(x03), .O(new_n76_));
  nand4  g031(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand3  g032(.a(new_n77_), .b(new_n49_), .c(new_n46_), .O(new_n78_));
  inv1   g033(.a(new_n78_), .O(z04));
  nor3   g034(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g035(.a(x14), .O(new_n81_));
  nor3   g036(.a(new_n48_), .b(new_n81_), .c(x08), .O(z06));
  aoi21  g037(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g038(.a(x17), .O(new_n84_));
  nand4  g039(.a(x24), .b(x22), .c(x21), .d(new_n84_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(new_n47_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(new_n60_), .O(new_n87_));
  oai21  g042(.a(new_n52_), .b(new_n59_), .c(new_n51_), .O(new_n88_));
  aoi21  g043(.a(new_n88_), .b(x24), .c(x25), .O(new_n89_));
  nand2  g044(.a(new_n89_), .b(new_n87_), .O(z08));
  inv1   g045(.a(x04), .O(new_n91_));
  inv1   g046(.a(x15), .O(new_n92_));
  nand4  g047(.a(new_n49_), .b(new_n92_), .c(new_n55_), .d(x05), .O(new_n93_));
  nor2   g048(.a(new_n93_), .b(new_n91_), .O(z09));
  nand4  g049(.a(new_n56_), .b(new_n84_), .c(new_n92_), .d(new_n55_), .O(new_n95_));
  nand2  g050(.a(new_n95_), .b(new_n49_), .O(z10));
  nand3  g051(.a(x19), .b(x18), .c(new_n84_), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(new_n61_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n56_), .c(new_n92_), .d(new_n55_), .O(new_n99_));
  inv1   g054(.a(new_n99_), .O(z11));
  nand2  g055(.a(x18), .b(x17), .O(new_n101_));
  nand4  g056(.a(new_n101_), .b(new_n56_), .c(x19), .d(new_n92_), .O(new_n102_));
  nor2   g057(.a(new_n102_), .b(x07), .O(z12));
  nand3  g058(.a(new_n56_), .b(new_n92_), .c(new_n55_), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(new_n49_), .O(new_n105_));
  nand3  g060(.a(new_n59_), .b(x18), .c(x17), .O(new_n106_));
  oai21  g061(.a(new_n59_), .b(x17), .c(new_n106_), .O(new_n107_));
  nand2  g062(.a(new_n107_), .b(x19), .O(new_n108_));
  nand2  g063(.a(x20), .b(new_n47_), .O(new_n109_));
  nand3  g064(.a(new_n109_), .b(new_n108_), .c(new_n105_), .O(z13));
  inv1   g065(.a(x21), .O(new_n111_));
  nor2   g066(.a(x20), .b(new_n84_), .O(new_n112_));
  nor2   g067(.a(x21), .b(x20), .O(new_n113_));
  inv1   g068(.a(new_n113_), .O(new_n114_));
  oai22  g069(.a(new_n114_), .b(new_n101_), .c(new_n112_), .d(new_n111_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(x19), .O(new_n116_));
  nand2  g071(.a(x21), .b(new_n47_), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(new_n116_), .c(new_n105_), .O(z14));
  inv1   g073(.a(x22), .O(new_n119_));
  aoi21  g074(.a(new_n113_), .b(x17), .c(new_n119_), .O(new_n120_));
  nor2   g075(.a(x22), .b(x21), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n59_), .O(new_n122_));
  nor2   g077(.a(new_n122_), .b(new_n101_), .O(new_n123_));
  oai21  g078(.a(new_n123_), .b(new_n120_), .c(x19), .O(new_n124_));
  nand2  g079(.a(x22), .b(new_n47_), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n124_), .c(new_n105_), .O(z15));
  aoi21  g081(.a(new_n121_), .b(new_n112_), .c(new_n51_), .O(new_n127_));
  nor2   g082(.a(x23), .b(x22), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n111_), .O(new_n129_));
  nor2   g084(.a(new_n129_), .b(new_n106_), .O(new_n130_));
  oai21  g085(.a(new_n130_), .b(new_n127_), .c(x19), .O(new_n131_));
  nand2  g086(.a(x23), .b(new_n47_), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n131_), .c(new_n105_), .O(z16));
  nor3   g088(.a(x23), .b(x22), .c(x21), .O(new_n134_));
  aoi21  g089(.a(new_n134_), .b(new_n112_), .c(new_n70_), .O(new_n135_));
  nand3  g090(.a(new_n121_), .b(new_n70_), .c(new_n51_), .O(new_n136_));
  nor2   g091(.a(new_n136_), .b(new_n106_), .O(new_n137_));
  oai21  g092(.a(new_n137_), .b(new_n135_), .c(x19), .O(new_n138_));
  nand2  g093(.a(x24), .b(new_n47_), .O(new_n139_));
  nand3  g094(.a(new_n139_), .b(new_n138_), .c(new_n105_), .O(z17));
  inv1   g095(.a(new_n104_), .O(new_n141_));
  nor2   g096(.a(x25), .b(x24), .O(new_n142_));
  nand4  g097(.a(new_n142_), .b(new_n128_), .c(new_n113_), .d(x17), .O(new_n143_));
  nand2  g098(.a(new_n143_), .b(x19), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(x18), .O(new_n145_));
  nand4  g100(.a(new_n59_), .b(x19), .c(x18), .d(x17), .O(new_n146_));
  oai21  g101(.a(new_n146_), .b(new_n136_), .c(x25), .O(new_n147_));
  nand3  g102(.a(new_n147_), .b(new_n145_), .c(new_n141_), .O(z18));
endmodule


