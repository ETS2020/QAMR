// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x22), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x22), .O(new_n49_));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(x24), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x24), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(x21), .c(x23), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n55_), .c(new_n51_), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(x07), .O(new_n66_));
  nand3  g21(.a(new_n66_), .b(new_n63_), .c(new_n54_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n47_), .b(new_n68_), .O(z02));
  nand2  g24(.a(new_n63_), .b(new_n54_), .O(z03));
  inv1   g25(.a(new_n47_), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n71_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n71_), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  nor3   g35(.a(new_n47_), .b(new_n80_), .c(x08), .O(z06));
  nand3  g36(.a(new_n71_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g37(.a(new_n60_), .b(new_n56_), .c(new_n49_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x21), .c(new_n52_), .O(new_n84_));
  oai22  g39(.a(new_n84_), .b(new_n55_), .c(new_n51_), .d(new_n49_), .O(z08));
  inv1   g40(.a(x04), .O(new_n86_));
  inv1   g41(.a(x07), .O(new_n87_));
  inv1   g42(.a(x15), .O(new_n88_));
  nand4  g43(.a(new_n71_), .b(new_n88_), .c(new_n87_), .d(x05), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n86_), .O(z09));
  nor2   g45(.a(new_n65_), .b(new_n47_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n57_), .c(new_n88_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(x07), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n64_), .c(new_n88_), .d(new_n87_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n71_), .O(z11));
  nor2   g51(.a(new_n58_), .b(new_n57_), .O(new_n97_));
  nand3  g52(.a(new_n59_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n59_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n64_), .c(new_n71_), .d(new_n88_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(x07), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n97_), .b(new_n56_), .c(x19), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n103_), .c(new_n66_), .d(new_n88_), .O(new_n105_));
  and2   g60(.a(new_n105_), .b(new_n71_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n97_), .c(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n66_), .c(new_n88_), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n107_), .c(new_n47_), .O(z14));
  nand3  g67(.a(new_n64_), .b(new_n88_), .c(new_n87_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n71_), .O(new_n114_));
  nand2  g69(.a(new_n109_), .b(x22), .O(new_n115_));
  inv1   g70(.a(new_n102_), .O(new_n116_));
  nand4  g71(.a(new_n108_), .b(new_n116_), .c(x24), .d(new_n49_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(z15));
  inv1   g73(.a(x21), .O(new_n119_));
  nand3  g74(.a(new_n50_), .b(new_n119_), .c(new_n56_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n102_), .c(x24), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n49_), .O(new_n122_));
  aoi21  g77(.a(new_n117_), .b(x23), .c(new_n113_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n122_), .O(z16));
  nand3  g79(.a(new_n108_), .b(x24), .c(new_n50_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n102_), .c(new_n49_), .O(new_n126_));
  aoi21  g81(.a(x24), .b(x22), .c(x15), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n66_), .O(z17));
  nand4  g83(.a(new_n91_), .b(new_n51_), .c(new_n88_), .d(new_n87_), .O(z18));
endmodule


