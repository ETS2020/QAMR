// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x14), .O(new_n47_));
  inv1   g002(.a(x18), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  oai21  g005(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g006(.a(x23), .O(new_n52_));
  inv1   g007(.a(x24), .O(new_n53_));
  nand2  g008(.a(x22), .b(x21), .O(new_n54_));
  aoi21  g009(.a(new_n54_), .b(new_n52_), .c(new_n53_), .O(new_n55_));
  aoi21  g010(.a(x05), .b(x04), .c(x07), .O(new_n56_));
  oai21  g011(.a(new_n55_), .b(x25), .c(new_n56_), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  inv1   g013(.a(x20), .O(new_n59_));
  inv1   g014(.a(x25), .O(new_n60_));
  oai21  g015(.a(x19), .b(x17), .c(new_n48_), .O(new_n61_));
  oai21  g016(.a(new_n48_), .b(x14), .c(new_n61_), .O(new_n62_));
  nand4  g017(.a(new_n62_), .b(new_n60_), .c(new_n52_), .d(new_n59_), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(new_n58_), .O(z01));
  inv1   g019(.a(x16), .O(new_n65_));
  nand2  g020(.a(new_n50_), .b(new_n65_), .O(z02));
  or2    g021(.a(new_n55_), .b(new_n49_), .O(new_n67_));
  nand3  g022(.a(new_n62_), .b(new_n52_), .c(new_n59_), .O(new_n68_));
  aoi21  g023(.a(new_n68_), .b(new_n67_), .c(x25), .O(z03));
  xnor2a g024(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g025(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g026(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g027(.a(x12), .b(x03), .O(new_n73_));
  nand4  g028(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g029(.a(new_n74_), .b(new_n50_), .c(new_n46_), .O(new_n75_));
  inv1   g030(.a(new_n75_), .O(z04));
  oai21  g031(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g032(.a(new_n48_), .b(x14), .c(new_n46_), .O(new_n78_));
  inv1   g033(.a(new_n78_), .O(z06));
  aoi21  g034(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g035(.a(x22), .O(new_n81_));
  inv1   g036(.a(x17), .O(new_n82_));
  inv1   g037(.a(x19), .O(new_n83_));
  nand3  g038(.a(new_n83_), .b(new_n48_), .c(new_n82_), .O(new_n84_));
  aoi21  g039(.a(new_n84_), .b(new_n59_), .c(new_n81_), .O(new_n85_));
  aoi21  g040(.a(new_n85_), .b(x21), .c(x23), .O(new_n86_));
  nor2   g041(.a(new_n49_), .b(x25), .O(new_n87_));
  oai21  g042(.a(new_n86_), .b(new_n53_), .c(new_n87_), .O(z08));
  inv1   g043(.a(x04), .O(new_n89_));
  inv1   g044(.a(x07), .O(new_n90_));
  inv1   g045(.a(x15), .O(new_n91_));
  nand4  g046(.a(new_n50_), .b(new_n91_), .c(new_n90_), .d(x05), .O(new_n92_));
  nor2   g047(.a(new_n92_), .b(new_n89_), .O(z09));
  nand2  g048(.a(x05), .b(x04), .O(new_n94_));
  nand4  g049(.a(new_n94_), .b(new_n50_), .c(new_n82_), .d(new_n91_), .O(new_n95_));
  nor2   g050(.a(new_n95_), .b(x07), .O(z10));
  xor2a  g051(.a(x18), .b(x17), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n94_), .c(new_n91_), .d(new_n90_), .O(new_n98_));
  nand2  g053(.a(new_n98_), .b(new_n50_), .O(z11));
  nor2   g054(.a(new_n48_), .b(new_n82_), .O(new_n100_));
  nand3  g055(.a(new_n83_), .b(x18), .c(x17), .O(new_n101_));
  oai21  g056(.a(new_n100_), .b(new_n83_), .c(new_n101_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n94_), .c(new_n91_), .d(new_n90_), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n50_), .O(z12));
  nand3  g059(.a(new_n94_), .b(new_n91_), .c(new_n90_), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(new_n50_), .O(new_n106_));
  aoi21  g061(.a(x19), .b(x17), .c(new_n59_), .O(new_n107_));
  nand4  g062(.a(new_n59_), .b(x19), .c(x18), .d(x17), .O(new_n108_));
  inv1   g063(.a(new_n108_), .O(new_n109_));
  oai21  g064(.a(new_n109_), .b(new_n107_), .c(new_n47_), .O(new_n110_));
  nand2  g065(.a(x20), .b(new_n48_), .O(new_n111_));
  nand3  g066(.a(new_n111_), .b(new_n110_), .c(new_n106_), .O(z13));
  oai21  g067(.a(x21), .b(x18), .c(x14), .O(new_n113_));
  nand2  g068(.a(new_n108_), .b(x21), .O(new_n114_));
  nor2   g069(.a(x21), .b(x20), .O(new_n115_));
  nand3  g070(.a(new_n115_), .b(new_n100_), .c(x19), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n56_), .c(new_n91_), .O(new_n117_));
  inv1   g072(.a(new_n117_), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n114_), .c(new_n113_), .O(z14));
  oai21  g074(.a(x22), .b(x18), .c(x14), .O(new_n120_));
  nand2  g075(.a(new_n116_), .b(x22), .O(new_n121_));
  nand3  g076(.a(x19), .b(x18), .c(x17), .O(new_n122_));
  inv1   g077(.a(new_n122_), .O(new_n123_));
  inv1   g078(.a(x21), .O(new_n124_));
  nand2  g079(.a(new_n81_), .b(new_n124_), .O(new_n125_));
  nor2   g080(.a(new_n125_), .b(x20), .O(new_n126_));
  aoi21  g081(.a(new_n126_), .b(new_n123_), .c(x15), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n121_), .c(new_n120_), .d(new_n56_), .O(z15));
  oai21  g083(.a(x23), .b(x18), .c(x14), .O(new_n129_));
  nand2  g084(.a(new_n126_), .b(new_n123_), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(x23), .O(new_n131_));
  nand4  g086(.a(new_n123_), .b(new_n115_), .c(new_n52_), .d(new_n81_), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n56_), .c(new_n91_), .O(new_n133_));
  inv1   g088(.a(new_n133_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n131_), .c(new_n129_), .O(z16));
  oai21  g090(.a(x24), .b(x18), .c(x14), .O(new_n136_));
  nand2  g091(.a(new_n132_), .b(x24), .O(new_n137_));
  nor2   g092(.a(x22), .b(x21), .O(new_n138_));
  nand3  g093(.a(new_n138_), .b(new_n53_), .c(new_n52_), .O(new_n139_));
  inv1   g094(.a(new_n139_), .O(new_n140_));
  aoi21  g095(.a(new_n140_), .b(new_n109_), .c(x15), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(new_n137_), .c(new_n136_), .d(new_n56_), .O(z17));
  oai21  g097(.a(x25), .b(x18), .c(x14), .O(new_n143_));
  oai21  g098(.a(new_n139_), .b(new_n108_), .c(x25), .O(new_n144_));
  nand3  g099(.a(new_n60_), .b(new_n53_), .c(new_n52_), .O(new_n145_));
  nor2   g100(.a(new_n145_), .b(new_n125_), .O(new_n146_));
  aoi21  g101(.a(new_n146_), .b(new_n109_), .c(x15), .O(new_n147_));
  nand4  g102(.a(new_n147_), .b(new_n144_), .c(new_n143_), .d(new_n56_), .O(z18));
endmodule


