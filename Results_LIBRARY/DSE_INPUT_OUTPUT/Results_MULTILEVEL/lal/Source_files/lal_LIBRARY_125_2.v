// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x04), .O(new_n48_));
  nor2   g03(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n53_), .c(x07), .O(new_n62_));
  nand3  g17(.a(x15), .b(x05), .c(x04), .O(new_n63_));
  oai21  g18(.a(new_n62_), .b(new_n49_), .c(new_n63_), .O(z01));
  nand3  g19(.a(new_n61_), .b(new_n50_), .c(new_n53_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n50_), .c(new_n46_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z06));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x24), .O(new_n78_));
  inv1   g33(.a(x22), .O(new_n79_));
  aoi21  g34(.a(new_n58_), .b(new_n54_), .c(new_n79_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(x21), .c(x23), .O(new_n81_));
  nor2   g36(.a(new_n49_), .b(x25), .O(new_n82_));
  oai21  g37(.a(new_n81_), .b(new_n78_), .c(new_n82_), .O(z08));
  inv1   g38(.a(x07), .O(new_n85_));
  nand2  g39(.a(new_n55_), .b(new_n85_), .O(new_n86_));
  aoi21  g40(.a(new_n86_), .b(new_n48_), .c(x15), .O(z10));
  xor2a  g41(.a(x18), .b(x17), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  aoi21  g43(.a(new_n89_), .b(new_n48_), .c(x15), .O(z11));
  nor2   g44(.a(new_n56_), .b(new_n55_), .O(new_n91_));
  nand3  g45(.a(new_n57_), .b(x18), .c(x17), .O(new_n92_));
  oai21  g46(.a(new_n91_), .b(new_n57_), .c(new_n92_), .O(new_n93_));
  nand2  g47(.a(new_n93_), .b(new_n85_), .O(new_n94_));
  aoi21  g48(.a(new_n94_), .b(new_n48_), .c(x15), .O(z12));
  inv1   g49(.a(x15), .O(new_n96_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g51(.a(new_n97_), .b(x20), .O(new_n98_));
  nor2   g52(.a(x07), .b(x04), .O(new_n99_));
  nor2   g53(.a(x20), .b(new_n57_), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(new_n91_), .O(new_n101_));
  nand4  g55(.a(new_n101_), .b(new_n99_), .c(new_n98_), .d(new_n96_), .O(z13));
  nor2   g56(.a(x21), .b(x20), .O(new_n103_));
  nand3  g57(.a(new_n103_), .b(new_n91_), .c(x19), .O(new_n104_));
  nand2  g58(.a(new_n104_), .b(new_n85_), .O(new_n105_));
  aoi21  g59(.a(new_n101_), .b(x21), .c(new_n105_), .O(new_n106_));
  oai21  g60(.a(new_n106_), .b(x04), .c(new_n96_), .O(z14));
  nand2  g61(.a(new_n104_), .b(x22), .O(new_n108_));
  inv1   g62(.a(new_n97_), .O(new_n109_));
  nor2   g63(.a(x22), .b(x21), .O(new_n110_));
  nand3  g64(.a(new_n110_), .b(new_n109_), .c(new_n54_), .O(new_n111_));
  nand4  g65(.a(new_n111_), .b(new_n108_), .c(new_n99_), .d(new_n96_), .O(z15));
  nand2  g66(.a(new_n111_), .b(x23), .O(new_n113_));
  inv1   g67(.a(x23), .O(new_n114_));
  nand4  g68(.a(new_n103_), .b(new_n109_), .c(new_n114_), .d(new_n79_), .O(new_n115_));
  nand4  g69(.a(new_n115_), .b(new_n113_), .c(new_n99_), .d(new_n96_), .O(z16));
  nand2  g70(.a(new_n115_), .b(x24), .O(new_n117_));
  nand3  g71(.a(new_n110_), .b(new_n78_), .c(new_n114_), .O(new_n118_));
  or2    g72(.a(new_n118_), .b(new_n101_), .O(new_n119_));
  nand4  g73(.a(new_n119_), .b(new_n117_), .c(new_n99_), .d(new_n96_), .O(z17));
  oai21  g74(.a(new_n118_), .b(new_n101_), .c(x25), .O(new_n121_));
  nor3   g75(.a(x25), .b(x24), .c(x23), .O(new_n122_));
  nand4  g76(.a(new_n122_), .b(new_n110_), .c(new_n100_), .d(new_n91_), .O(new_n123_));
  nand3  g77(.a(new_n123_), .b(new_n121_), .c(new_n85_), .O(new_n124_));
  nand2  g78(.a(new_n124_), .b(new_n48_), .O(new_n125_));
  nand2  g79(.a(new_n125_), .b(new_n96_), .O(z18));
  zero   g80(.O(z09));
endmodule


