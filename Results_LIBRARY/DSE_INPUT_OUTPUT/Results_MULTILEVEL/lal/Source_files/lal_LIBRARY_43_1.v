// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x19), .O(new_n47_));
  nor2   g002(.a(new_n47_), .b(x18), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x23), .O(new_n51_));
  nand2  g006(.a(x22), .b(x21), .O(new_n52_));
  nand2  g007(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g008(.a(new_n53_), .b(x24), .c(x25), .O(new_n54_));
  inv1   g009(.a(x07), .O(new_n55_));
  nand2  g010(.a(x05), .b(x04), .O(new_n56_));
  nand2  g011(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g012(.a(new_n57_), .b(new_n54_), .c(new_n49_), .O(new_n58_));
  inv1   g013(.a(x20), .O(new_n59_));
  inv1   g014(.a(x25), .O(new_n60_));
  inv1   g015(.a(x18), .O(new_n61_));
  nand2  g016(.a(new_n47_), .b(x17), .O(new_n62_));
  nand2  g017(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g018(.a(new_n63_), .b(new_n60_), .c(new_n51_), .d(new_n59_), .O(new_n64_));
  nand2  g019(.a(new_n64_), .b(new_n58_), .O(z01));
  inv1   g020(.a(x16), .O(new_n66_));
  nand2  g021(.a(new_n49_), .b(new_n66_), .O(z02));
  aoi21  g022(.a(new_n62_), .b(new_n61_), .c(x20), .O(new_n68_));
  aoi22  g023(.a(x22), .b(x21), .c(x19), .d(new_n61_), .O(new_n69_));
  oai21  g024(.a(new_n69_), .b(new_n68_), .c(new_n51_), .O(new_n70_));
  inv1   g025(.a(x24), .O(new_n71_));
  nand2  g026(.a(new_n49_), .b(new_n71_), .O(new_n72_));
  aoi21  g027(.a(new_n72_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g028(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g029(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g030(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g031(.a(x12), .b(x03), .O(new_n77_));
  nand4  g032(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand3  g033(.a(new_n78_), .b(new_n49_), .c(new_n46_), .O(new_n79_));
  inv1   g034(.a(new_n79_), .O(z04));
  nor3   g035(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g036(.a(x14), .O(new_n82_));
  nor3   g037(.a(new_n48_), .b(new_n82_), .c(x08), .O(z06));
  aoi21  g038(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g039(.a(x17), .O(new_n85_));
  nand4  g040(.a(x24), .b(x22), .c(x21), .d(new_n85_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(new_n47_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(new_n61_), .O(new_n88_));
  oai21  g043(.a(new_n52_), .b(new_n59_), .c(new_n51_), .O(new_n89_));
  aoi21  g044(.a(new_n89_), .b(x24), .c(x25), .O(new_n90_));
  nand2  g045(.a(new_n90_), .b(new_n88_), .O(z08));
  inv1   g046(.a(x04), .O(new_n92_));
  inv1   g047(.a(x15), .O(new_n93_));
  nand4  g048(.a(new_n49_), .b(new_n93_), .c(new_n55_), .d(x05), .O(new_n94_));
  nor2   g049(.a(new_n94_), .b(new_n92_), .O(z09));
  nand4  g050(.a(new_n56_), .b(new_n85_), .c(new_n93_), .d(new_n55_), .O(new_n96_));
  nand2  g051(.a(new_n96_), .b(new_n49_), .O(z10));
  nand3  g052(.a(new_n47_), .b(new_n61_), .c(x17), .O(new_n98_));
  oai21  g053(.a(new_n61_), .b(x17), .c(new_n98_), .O(new_n99_));
  nand4  g054(.a(new_n99_), .b(new_n56_), .c(new_n93_), .d(new_n55_), .O(new_n100_));
  inv1   g055(.a(new_n100_), .O(z11));
  nand2  g056(.a(x19), .b(new_n85_), .O(new_n102_));
  nand2  g057(.a(new_n102_), .b(new_n62_), .O(new_n103_));
  nand4  g058(.a(new_n103_), .b(new_n56_), .c(x18), .d(new_n93_), .O(new_n104_));
  nor2   g059(.a(new_n104_), .b(x07), .O(z12));
  nand3  g060(.a(new_n56_), .b(new_n93_), .c(new_n55_), .O(new_n106_));
  nand2  g061(.a(new_n106_), .b(new_n49_), .O(new_n107_));
  nand3  g062(.a(new_n59_), .b(x19), .c(x17), .O(new_n108_));
  oai21  g063(.a(new_n59_), .b(x17), .c(new_n108_), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(x18), .O(new_n110_));
  nand2  g065(.a(x20), .b(new_n47_), .O(new_n111_));
  nand3  g066(.a(new_n111_), .b(new_n110_), .c(new_n107_), .O(z13));
  inv1   g067(.a(x21), .O(new_n113_));
  nor2   g068(.a(x20), .b(new_n85_), .O(new_n114_));
  nor2   g069(.a(x21), .b(x20), .O(new_n115_));
  nand3  g070(.a(new_n115_), .b(x19), .c(x17), .O(new_n116_));
  oai21  g071(.a(new_n114_), .b(new_n113_), .c(new_n116_), .O(new_n117_));
  nand2  g072(.a(new_n117_), .b(x18), .O(new_n118_));
  nand2  g073(.a(x21), .b(new_n47_), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n118_), .c(new_n107_), .O(z14));
  inv1   g075(.a(x22), .O(new_n121_));
  aoi21  g076(.a(new_n115_), .b(x17), .c(new_n121_), .O(new_n122_));
  nand2  g077(.a(x19), .b(x17), .O(new_n123_));
  nor2   g078(.a(x22), .b(x21), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n59_), .O(new_n125_));
  nor2   g080(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  oai21  g081(.a(new_n126_), .b(new_n122_), .c(x18), .O(new_n127_));
  nand2  g082(.a(x22), .b(new_n47_), .O(new_n128_));
  nand3  g083(.a(new_n128_), .b(new_n127_), .c(new_n107_), .O(z15));
  aoi21  g084(.a(new_n124_), .b(new_n114_), .c(new_n51_), .O(new_n130_));
  nand3  g085(.a(new_n51_), .b(new_n121_), .c(new_n113_), .O(new_n131_));
  nor2   g086(.a(new_n131_), .b(new_n108_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(new_n130_), .c(x18), .O(new_n133_));
  nand2  g088(.a(x23), .b(new_n47_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n133_), .c(new_n107_), .O(z16));
  nor3   g090(.a(x23), .b(x22), .c(x21), .O(new_n136_));
  aoi21  g091(.a(new_n136_), .b(new_n114_), .c(new_n71_), .O(new_n137_));
  nand3  g092(.a(new_n124_), .b(new_n71_), .c(new_n51_), .O(new_n138_));
  nor2   g093(.a(new_n138_), .b(new_n108_), .O(new_n139_));
  oai21  g094(.a(new_n139_), .b(new_n137_), .c(x18), .O(new_n140_));
  nand2  g095(.a(x24), .b(new_n47_), .O(new_n141_));
  nand3  g096(.a(new_n141_), .b(new_n140_), .c(new_n107_), .O(z17));
  inv1   g097(.a(new_n57_), .O(new_n143_));
  oai21  g098(.a(x25), .b(x19), .c(new_n61_), .O(new_n144_));
  oai21  g099(.a(new_n138_), .b(new_n108_), .c(x25), .O(new_n145_));
  nand4  g100(.a(new_n60_), .b(new_n71_), .c(new_n51_), .d(new_n121_), .O(new_n146_));
  nor2   g101(.a(new_n146_), .b(new_n116_), .O(new_n147_));
  nor2   g102(.a(new_n147_), .b(x15), .O(new_n148_));
  nand4  g103(.a(new_n148_), .b(new_n145_), .c(new_n144_), .d(new_n143_), .O(z18));
endmodule


