// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x16), .O(new_n46_));
  inv1   g001(.a(x02), .O(new_n47_));
  inv1   g002(.a(x15), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g006(.a(new_n51_), .O(z00));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  inv1   g009(.a(x17), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand3  g012(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g013(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g014(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g016(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  nand2  g017(.a(x05), .b(x04), .O(new_n63_));
  nor2   g018(.a(new_n49_), .b(x07), .O(new_n64_));
  nand3  g019(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z01));
  nor2   g020(.a(new_n49_), .b(new_n46_), .O(z02));
  nand3  g021(.a(new_n61_), .b(new_n50_), .c(new_n53_), .O(new_n67_));
  inv1   g022(.a(new_n67_), .O(z03));
  nor2   g023(.a(x11), .b(x08), .O(new_n69_));
  oai21  g024(.a(new_n69_), .b(x15), .c(x02), .O(new_n70_));
  inv1   g025(.a(x08), .O(new_n71_));
  inv1   g026(.a(x09), .O(new_n72_));
  inv1   g027(.a(x01), .O(new_n73_));
  aoi22  g028(.a(x10), .b(new_n73_), .c(new_n72_), .d(x00), .O(new_n74_));
  oai21  g029(.a(new_n72_), .b(x00), .c(new_n74_), .O(new_n75_));
  inv1   g030(.a(x10), .O(new_n76_));
  nand2  g031(.a(new_n76_), .b(x01), .O(new_n77_));
  nand2  g032(.a(x11), .b(new_n47_), .O(new_n78_));
  xnor2a g033(.a(x12), .b(x03), .O(new_n79_));
  nand3  g034(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n75_), .c(new_n71_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n70_), .O(z04));
  oai21  g037(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g038(.a(x14), .b(new_n71_), .O(new_n84_));
  nand2  g039(.a(new_n84_), .b(new_n50_), .O(z06));
  nand3  g040(.a(new_n50_), .b(new_n71_), .c(x06), .O(z07));
  inv1   g041(.a(x24), .O(new_n87_));
  inv1   g042(.a(x22), .O(new_n88_));
  aoi21  g043(.a(new_n58_), .b(new_n54_), .c(new_n88_), .O(new_n89_));
  aoi21  g044(.a(new_n89_), .b(x21), .c(x23), .O(new_n90_));
  nor2   g045(.a(new_n49_), .b(x25), .O(new_n91_));
  oai21  g046(.a(new_n90_), .b(new_n87_), .c(new_n91_), .O(z08));
  inv1   g047(.a(x07), .O(new_n93_));
  nand2  g048(.a(new_n48_), .b(new_n93_), .O(new_n94_));
  oai21  g049(.a(new_n94_), .b(new_n63_), .c(new_n50_), .O(z09));
  nand4  g050(.a(new_n63_), .b(new_n55_), .c(new_n48_), .d(new_n93_), .O(new_n96_));
  nand2  g051(.a(new_n96_), .b(new_n50_), .O(z10));
  xor2a  g052(.a(x18), .b(x17), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n63_), .c(new_n48_), .d(new_n93_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(new_n50_), .O(z11));
  nand2  g055(.a(x18), .b(x17), .O(new_n101_));
  inv1   g056(.a(new_n101_), .O(new_n102_));
  nand3  g057(.a(new_n57_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g058(.a(new_n102_), .b(new_n57_), .c(new_n103_), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n63_), .c(new_n48_), .d(new_n93_), .O(new_n105_));
  inv1   g060(.a(new_n105_), .O(z12));
  nand3  g061(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g062(.a(new_n107_), .b(x20), .O(new_n108_));
  aoi21  g063(.a(x05), .b(x04), .c(x07), .O(new_n109_));
  nand4  g064(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n110_));
  nand4  g065(.a(new_n110_), .b(new_n109_), .c(new_n108_), .d(new_n48_), .O(z13));
  inv1   g066(.a(x21), .O(new_n112_));
  inv1   g067(.a(new_n110_), .O(new_n113_));
  nor2   g068(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nor2   g069(.a(x21), .b(x20), .O(new_n115_));
  nand3  g070(.a(new_n115_), .b(new_n102_), .c(x19), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n63_), .c(new_n93_), .O(new_n117_));
  oai21  g072(.a(new_n117_), .b(new_n114_), .c(new_n48_), .O(new_n118_));
  nand2  g073(.a(x15), .b(new_n47_), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n118_), .O(z14));
  nand3  g075(.a(new_n112_), .b(new_n54_), .c(x19), .O(new_n121_));
  oai21  g076(.a(new_n121_), .b(new_n101_), .c(x22), .O(new_n122_));
  inv1   g077(.a(new_n107_), .O(new_n123_));
  nor3   g078(.a(x22), .b(x21), .c(x20), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand4  g080(.a(new_n125_), .b(new_n122_), .c(new_n63_), .d(new_n93_), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n48_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n119_), .O(z15));
  inv1   g083(.a(x23), .O(new_n129_));
  aoi21  g084(.a(new_n124_), .b(new_n123_), .c(new_n129_), .O(new_n130_));
  nand4  g085(.a(new_n115_), .b(new_n123_), .c(new_n129_), .d(new_n88_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n63_), .c(new_n93_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(new_n130_), .c(new_n48_), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n119_), .O(z16));
  nand2  g089(.a(new_n131_), .b(x24), .O(new_n135_));
  nand4  g090(.a(new_n87_), .b(new_n129_), .c(new_n88_), .d(new_n112_), .O(new_n136_));
  inv1   g091(.a(new_n136_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n113_), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n135_), .c(new_n109_), .d(new_n48_), .O(z17));
  oai21  g094(.a(new_n136_), .b(new_n110_), .c(x25), .O(new_n140_));
  nor2   g095(.a(x20), .b(new_n57_), .O(new_n141_));
  nor2   g096(.a(x22), .b(x21), .O(new_n142_));
  nor3   g097(.a(x25), .b(x24), .c(x23), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n142_), .c(new_n141_), .d(new_n102_), .O(new_n144_));
  nand4  g099(.a(new_n144_), .b(new_n140_), .c(new_n63_), .d(new_n93_), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n48_), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(new_n119_), .O(z18));
endmodule


