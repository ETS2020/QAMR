// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x25), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n57_), .c(x23), .O(new_n60_));
  nand2  g15(.a(x24), .b(new_n47_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n60_), .c(new_n51_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n62_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n48_), .b(new_n68_), .O(z02));
  aoi21  g24(.a(new_n56_), .b(new_n52_), .c(new_n58_), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x23), .c(x24), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n51_), .c(new_n47_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z03));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n49_), .c(new_n46_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  nor3   g37(.a(new_n48_), .b(new_n82_), .c(x08), .O(z06));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g39(.a(x23), .O(new_n85_));
  nand3  g40(.a(new_n57_), .b(x22), .c(x21), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x24), .c(new_n47_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n51_), .O(z08));
  nand2  g44(.a(new_n47_), .b(new_n63_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n64_), .c(new_n49_), .O(z09));
  nand4  g46(.a(new_n64_), .b(new_n53_), .c(new_n47_), .d(new_n63_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n49_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n64_), .c(new_n47_), .d(new_n63_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n49_), .O(z11));
  nor2   g51(.a(new_n54_), .b(new_n53_), .O(new_n97_));
  nand3  g52(.a(new_n55_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n55_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n64_), .c(new_n47_), .d(new_n63_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nor2   g58(.a(x20), .b(new_n55_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n97_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n103_), .c(new_n66_), .d(new_n47_), .O(z13));
  inv1   g61(.a(x21), .O(new_n107_));
  aoi21  g62(.a(new_n104_), .b(new_n97_), .c(new_n107_), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n97_), .c(x19), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n64_), .c(new_n63_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n108_), .c(new_n47_), .O(new_n112_));
  nand2  g67(.a(x25), .b(x15), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n112_), .O(z14));
  nand2  g69(.a(new_n110_), .b(x22), .O(new_n115_));
  inv1   g70(.a(new_n102_), .O(new_n116_));
  nor3   g71(.a(x22), .b(x21), .c(x20), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n116_), .c(x15), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n115_), .c(new_n66_), .O(z15));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(new_n85_), .O(new_n120_));
  inv1   g75(.a(x22), .O(new_n121_));
  nand4  g76(.a(new_n109_), .b(new_n116_), .c(new_n85_), .d(new_n121_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n64_), .c(new_n63_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n120_), .c(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n113_), .O(z16));
  nand2  g80(.a(new_n122_), .b(x24), .O(new_n126_));
  nor2   g81(.a(x22), .b(x21), .O(new_n127_));
  nor2   g82(.a(x24), .b(x23), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n127_), .c(new_n104_), .d(new_n97_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n126_), .c(new_n66_), .d(new_n47_), .O(z17));
  nand2  g85(.a(new_n65_), .b(new_n49_), .O(new_n131_));
  nor2   g86(.a(new_n53_), .b(x15), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(x19), .c(x18), .O(new_n133_));
  nand3  g88(.a(new_n128_), .b(new_n109_), .c(new_n121_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n133_), .c(x25), .O(new_n135_));
  nand2  g90(.a(new_n104_), .b(x18), .O(new_n136_));
  inv1   g91(.a(new_n136_), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n137_), .c(new_n132_), .d(new_n127_), .O(new_n139_));
  nand3  g94(.a(new_n139_), .b(new_n135_), .c(new_n131_), .O(z18));
endmodule


