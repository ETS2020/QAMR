// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x19), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x23), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(x24), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g13(.a(new_n55_), .b(new_n51_), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nor2   g16(.a(x18), .b(x17), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x15), .c(new_n61_), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n51_), .c(new_n52_), .d(new_n60_), .O(new_n64_));
  oai21  g19(.a(new_n59_), .b(new_n48_), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n49_), .b(new_n66_), .O(z02));
  nand2  g22(.a(new_n55_), .b(new_n49_), .O(new_n68_));
  nand3  g23(.a(new_n63_), .b(new_n52_), .c(new_n60_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  nor3   g32(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z06));
  nand3  g35(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g36(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n62_), .c(x15), .O(new_n84_));
  oai21  g39(.a(new_n53_), .b(new_n60_), .c(new_n52_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x25), .O(new_n86_));
  oai21  g41(.a(new_n84_), .b(x19), .c(new_n86_), .O(z08));
  nand4  g42(.a(new_n47_), .b(new_n56_), .c(x05), .d(x04), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand4  g45(.a(new_n57_), .b(new_n90_), .c(new_n47_), .d(new_n56_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n49_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n57_), .c(new_n47_), .d(new_n56_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n49_), .O(z11));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(x19), .c(new_n47_), .O(new_n97_));
  nand3  g52(.a(new_n61_), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n57_), .c(new_n56_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n49_), .O(z12));
  inv1   g56(.a(new_n58_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  inv1   g59(.a(new_n96_), .O(new_n105_));
  nor2   g60(.a(x20), .b(new_n61_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n104_), .c(new_n102_), .d(new_n47_), .O(z13));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n105_), .c(x15), .O(new_n110_));
  aoi21  g65(.a(new_n107_), .b(x21), .c(new_n58_), .O(new_n111_));
  oai22  g66(.a(new_n111_), .b(x15), .c(new_n110_), .d(new_n61_), .O(z14));
  nor2   g67(.a(x22), .b(x21), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n105_), .c(new_n60_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x19), .O(new_n116_));
  nand3  g71(.a(new_n109_), .b(new_n105_), .c(x19), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(x22), .c(new_n58_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(x15), .c(new_n116_), .O(z15));
  nand3  g74(.a(new_n60_), .b(x18), .c(x17), .O(new_n120_));
  inv1   g75(.a(x21), .O(new_n121_));
  nor2   g76(.a(x23), .b(x22), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n120_), .c(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x19), .O(new_n125_));
  inv1   g80(.a(new_n103_), .O(new_n126_));
  nand3  g81(.a(new_n113_), .b(new_n126_), .c(new_n60_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(x23), .c(new_n58_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(x15), .c(new_n125_), .O(z16));
  nand2  g84(.a(new_n122_), .b(new_n109_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n103_), .c(x24), .O(new_n131_));
  nor2   g86(.a(x24), .b(x23), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n113_), .c(new_n106_), .d(new_n105_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n131_), .c(new_n102_), .d(new_n47_), .O(z17));
  nand3  g89(.a(new_n109_), .b(x18), .c(x17), .O(new_n135_));
  nor2   g90(.a(x25), .b(x24), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n122_), .O(new_n137_));
  oai21  g92(.a(new_n137_), .b(new_n135_), .c(new_n47_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(x19), .O(new_n139_));
  aoi21  g94(.a(new_n133_), .b(x25), .c(new_n58_), .O(new_n140_));
  oai21  g95(.a(new_n140_), .b(x15), .c(new_n139_), .O(z18));
endmodule


