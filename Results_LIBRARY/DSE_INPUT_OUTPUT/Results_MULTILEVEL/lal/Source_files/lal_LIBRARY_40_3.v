// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x19), .b(x15), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x21), .O(new_n50_));
  inv1   g05(.a(x22), .O(new_n51_));
  inv1   g06(.a(x23), .O(new_n52_));
  oai21  g07(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(x24), .c(x25), .O(new_n54_));
  inv1   g09(.a(x07), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n54_), .c(new_n47_), .O(new_n58_));
  inv1   g13(.a(x20), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  nor2   g15(.a(x18), .b(x17), .O(new_n61_));
  inv1   g16(.a(x15), .O(new_n62_));
  nand2  g17(.a(x19), .b(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n61_), .b(x19), .c(new_n63_), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n60_), .c(new_n52_), .d(new_n59_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n58_), .O(z01));
  nand2  g21(.a(new_n47_), .b(new_n46_), .O(z02));
  nor3   g22(.a(x25), .b(x23), .c(x20), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x15), .c(x19), .O(new_n69_));
  inv1   g24(.a(x24), .O(new_n70_));
  nor2   g25(.a(new_n51_), .b(new_n50_), .O(new_n71_));
  oai21  g26(.a(new_n61_), .b(x20), .c(new_n71_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n52_), .c(new_n70_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x25), .c(new_n69_), .O(z03));
  inv1   g29(.a(x08), .O(new_n75_));
  xnor2a g30(.a(x09), .b(x00), .O(new_n76_));
  xnor2a g31(.a(x10), .b(x01), .O(new_n77_));
  xnor2a g32(.a(x11), .b(x02), .O(new_n78_));
  xnor2a g33(.a(x12), .b(x03), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n47_), .c(new_n75_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z04));
  inv1   g37(.a(x13), .O(new_n83_));
  nand3  g38(.a(new_n47_), .b(new_n83_), .c(new_n75_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z05));
  inv1   g40(.a(x14), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g42(.a(new_n47_), .b(new_n75_), .c(x06), .O(z07));
  inv1   g43(.a(x17), .O(new_n89_));
  inv1   g44(.a(x18), .O(new_n90_));
  inv1   g45(.a(x19), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n59_), .c(new_n51_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x21), .c(x23), .O(new_n94_));
  aoi21  g49(.a(x19), .b(x15), .c(x25), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n70_), .c(new_n95_), .O(z08));
  nand4  g51(.a(new_n62_), .b(new_n55_), .c(x05), .d(x04), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z09));
  nand4  g53(.a(new_n56_), .b(new_n89_), .c(new_n62_), .d(new_n55_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z10));
  xor2a  g55(.a(x18), .b(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n56_), .c(new_n62_), .d(new_n55_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z11));
  nor2   g58(.a(new_n90_), .b(new_n89_), .O(new_n104_));
  nand4  g59(.a(new_n91_), .b(x18), .c(x17), .d(new_n62_), .O(new_n105_));
  oai21  g60(.a(new_n104_), .b(new_n91_), .c(new_n105_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n56_), .c(new_n55_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(z12));
  inv1   g63(.a(new_n57_), .O(new_n109_));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x20), .O(new_n111_));
  nor2   g66(.a(x20), .b(new_n91_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n104_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n109_), .d(new_n62_), .O(z13));
  nand2  g69(.a(new_n113_), .b(x21), .O(new_n115_));
  nor2   g70(.a(x21), .b(x20), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n104_), .c(x19), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n115_), .c(new_n109_), .d(new_n62_), .O(z14));
  nand2  g73(.a(new_n117_), .b(x22), .O(new_n119_));
  inv1   g74(.a(new_n110_), .O(new_n120_));
  nor2   g75(.a(x22), .b(x21), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n59_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n119_), .c(new_n109_), .d(new_n62_), .O(z15));
  nand2  g78(.a(new_n122_), .b(x23), .O(new_n124_));
  nand4  g79(.a(new_n116_), .b(new_n120_), .c(new_n52_), .d(new_n51_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n124_), .c(new_n109_), .d(new_n62_), .O(z16));
  nand2  g81(.a(new_n125_), .b(x24), .O(new_n127_));
  nand4  g82(.a(new_n70_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n128_));
  or2    g83(.a(new_n128_), .b(new_n113_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n127_), .c(new_n109_), .d(new_n62_), .O(z17));
  oai21  g85(.a(x25), .b(x15), .c(new_n91_), .O(new_n131_));
  nand3  g86(.a(new_n59_), .b(x18), .c(x17), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(new_n128_), .c(x25), .O(new_n133_));
  nor3   g88(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n121_), .c(new_n112_), .d(new_n104_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n133_), .c(new_n56_), .d(new_n55_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n62_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n131_), .O(z18));
endmodule


