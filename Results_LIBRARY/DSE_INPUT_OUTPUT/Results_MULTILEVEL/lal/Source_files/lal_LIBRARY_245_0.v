// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x24), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x15), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(new_n52_), .c(new_n53_), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(x22), .c(x21), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n54_), .c(new_n56_), .O(new_n64_));
  aoi21  g19(.a(x05), .b(x04), .c(x07), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n64_), .c(new_n55_), .O(z01));
  nand3  g21(.a(new_n63_), .b(x24), .c(new_n56_), .O(new_n67_));
  nand2  g22(.a(new_n53_), .b(x15), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n67_), .c(x25), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n49_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor3   g33(.a(new_n48_), .b(new_n78_), .c(x08), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g35(.a(x22), .O(new_n81_));
  aoi21  g36(.a(new_n61_), .b(new_n57_), .c(new_n81_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x21), .c(x23), .O(new_n83_));
  nor2   g38(.a(new_n48_), .b(x25), .O(new_n84_));
  oai21  g39(.a(new_n83_), .b(new_n53_), .c(new_n84_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(x05), .c(x04), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x24), .c(x15), .O(z09));
  nand2  g43(.a(x05), .b(x04), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(x24), .c(new_n58_), .d(new_n52_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x07), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n89_), .c(new_n86_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x24), .c(x15), .O(z11));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n60_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n60_), .c(new_n97_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n89_), .c(x24), .d(new_n52_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(x07), .O(z12));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x20), .O(new_n102_));
  nor2   g57(.a(x20), .b(new_n60_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n96_), .c(new_n53_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n102_), .c(new_n65_), .d(new_n52_), .O(z13));
  nand2  g60(.a(new_n103_), .b(new_n96_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x21), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n96_), .c(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(x24), .c(new_n52_), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n107_), .c(new_n65_), .O(z14));
  inv1   g67(.a(x21), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n57_), .c(x19), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n95_), .c(x22), .O(new_n115_));
  inv1   g70(.a(new_n101_), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n81_), .c(new_n113_), .d(new_n57_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n115_), .c(new_n89_), .d(new_n86_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x24), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n52_), .O(z15));
  nand2  g75(.a(new_n117_), .b(x23), .O(new_n121_));
  nor2   g76(.a(x23), .b(x22), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n108_), .c(new_n116_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(x24), .c(new_n52_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n121_), .c(new_n65_), .O(z16));
  nand2  g81(.a(new_n96_), .b(new_n65_), .O(new_n127_));
  nand3  g82(.a(new_n122_), .b(new_n103_), .c(new_n113_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n127_), .c(x24), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n52_), .O(z17));
  nand4  g85(.a(new_n65_), .b(new_n54_), .c(x24), .d(new_n52_), .O(z18));
endmodule


