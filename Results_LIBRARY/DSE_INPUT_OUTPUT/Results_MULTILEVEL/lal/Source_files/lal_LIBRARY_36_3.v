// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g000(.a(x08), .O(new_n46_));
  nand2  g001(.a(x18), .b(x15), .O(new_n47_));
  oai21  g002(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g003(.a(x21), .O(new_n49_));
  inv1   g004(.a(x22), .O(new_n50_));
  inv1   g005(.a(x23), .O(new_n51_));
  oai21  g006(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  aoi21  g007(.a(new_n52_), .b(x24), .c(x25), .O(new_n53_));
  inv1   g008(.a(x07), .O(new_n54_));
  nand2  g009(.a(x05), .b(x04), .O(new_n55_));
  nand2  g010(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g011(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(new_n57_));
  inv1   g012(.a(x20), .O(new_n58_));
  inv1   g013(.a(x25), .O(new_n59_));
  nor2   g014(.a(x19), .b(x17), .O(new_n60_));
  inv1   g015(.a(x15), .O(new_n61_));
  nand2  g016(.a(x18), .b(new_n61_), .O(new_n62_));
  oai21  g017(.a(new_n60_), .b(x18), .c(new_n62_), .O(new_n63_));
  nand4  g018(.a(new_n63_), .b(new_n59_), .c(new_n51_), .d(new_n58_), .O(new_n64_));
  nand2  g019(.a(new_n64_), .b(new_n57_), .O(z01));
  and2   g020(.a(new_n47_), .b(x16), .O(z02));
  nor3   g021(.a(x25), .b(x23), .c(x20), .O(new_n67_));
  oai21  g022(.a(new_n67_), .b(x15), .c(x18), .O(new_n68_));
  inv1   g023(.a(x24), .O(new_n69_));
  nor2   g024(.a(new_n50_), .b(new_n49_), .O(new_n70_));
  oai21  g025(.a(new_n60_), .b(x20), .c(new_n70_), .O(new_n71_));
  aoi21  g026(.a(new_n71_), .b(new_n51_), .c(new_n69_), .O(new_n72_));
  oai21  g027(.a(new_n72_), .b(x25), .c(new_n68_), .O(z03));
  xnor2a g028(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g029(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g030(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g031(.a(x12), .b(x03), .O(new_n77_));
  nand4  g032(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g034(.a(new_n79_), .b(new_n47_), .O(z04));
  inv1   g035(.a(x13), .O(new_n81_));
  nand3  g036(.a(new_n47_), .b(new_n81_), .c(new_n46_), .O(new_n82_));
  inv1   g037(.a(new_n82_), .O(z05));
  inv1   g038(.a(x14), .O(new_n84_));
  oai21  g039(.a(new_n84_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g040(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g041(.a(x22), .b(x21), .c(x20), .O(new_n87_));
  aoi21  g042(.a(new_n87_), .b(new_n51_), .c(new_n69_), .O(new_n88_));
  oai21  g043(.a(new_n88_), .b(x25), .c(new_n47_), .O(new_n89_));
  nor3   g044(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  nand4  g045(.a(new_n90_), .b(x24), .c(x22), .d(x21), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(new_n89_), .O(z08));
  nand2  g047(.a(new_n61_), .b(new_n54_), .O(new_n93_));
  oai21  g048(.a(new_n93_), .b(new_n55_), .c(new_n47_), .O(z09));
  inv1   g049(.a(x17), .O(new_n95_));
  nand4  g050(.a(new_n55_), .b(new_n95_), .c(new_n61_), .d(new_n54_), .O(new_n96_));
  nand2  g051(.a(new_n96_), .b(new_n47_), .O(z10));
  xor2a  g052(.a(x18), .b(x17), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n55_), .c(new_n61_), .d(new_n54_), .O(new_n99_));
  inv1   g054(.a(new_n99_), .O(z11));
  nand2  g055(.a(x18), .b(x17), .O(new_n101_));
  nand3  g056(.a(new_n101_), .b(x19), .c(new_n61_), .O(new_n102_));
  inv1   g057(.a(x19), .O(new_n103_));
  nand3  g058(.a(new_n103_), .b(x18), .c(x17), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand3  g060(.a(new_n105_), .b(new_n55_), .c(new_n54_), .O(new_n106_));
  nand2  g061(.a(new_n106_), .b(new_n47_), .O(z12));
  inv1   g062(.a(new_n56_), .O(new_n108_));
  nand3  g063(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(x20), .O(new_n110_));
  inv1   g065(.a(new_n101_), .O(new_n111_));
  nor2   g066(.a(x20), .b(new_n103_), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand4  g068(.a(new_n113_), .b(new_n110_), .c(new_n108_), .d(new_n61_), .O(z13));
  inv1   g069(.a(x18), .O(new_n115_));
  oai21  g070(.a(x21), .b(x15), .c(new_n115_), .O(new_n116_));
  nand3  g071(.a(new_n58_), .b(x19), .c(x17), .O(new_n117_));
  nor2   g072(.a(x21), .b(x20), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n111_), .c(x19), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n55_), .c(new_n54_), .O(new_n120_));
  aoi21  g075(.a(new_n117_), .b(x21), .c(new_n120_), .O(new_n121_));
  oai21  g076(.a(new_n121_), .b(x15), .c(new_n116_), .O(z14));
  nand2  g077(.a(new_n119_), .b(x22), .O(new_n123_));
  inv1   g078(.a(new_n109_), .O(new_n124_));
  nor2   g079(.a(x22), .b(x21), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n124_), .c(new_n58_), .O(new_n126_));
  nand4  g081(.a(new_n126_), .b(new_n123_), .c(new_n108_), .d(new_n61_), .O(z15));
  nand2  g082(.a(new_n126_), .b(x23), .O(new_n128_));
  nand4  g083(.a(new_n118_), .b(new_n124_), .c(new_n51_), .d(new_n50_), .O(new_n129_));
  nand4  g084(.a(new_n129_), .b(new_n128_), .c(new_n108_), .d(new_n61_), .O(z16));
  oai21  g085(.a(x24), .b(x15), .c(new_n115_), .O(new_n131_));
  nand3  g086(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(new_n117_), .c(x24), .O(new_n133_));
  nor2   g088(.a(x24), .b(x23), .O(new_n134_));
  nand4  g089(.a(new_n134_), .b(new_n125_), .c(new_n112_), .d(new_n111_), .O(new_n135_));
  nand4  g090(.a(new_n135_), .b(new_n133_), .c(new_n55_), .d(new_n54_), .O(new_n136_));
  nand2  g091(.a(new_n136_), .b(new_n61_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n131_), .O(z17));
  oai21  g093(.a(x25), .b(x15), .c(new_n115_), .O(new_n139_));
  nand4  g094(.a(new_n69_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n140_));
  oai21  g095(.a(new_n140_), .b(new_n117_), .c(x25), .O(new_n141_));
  nor3   g096(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nand4  g097(.a(new_n142_), .b(new_n125_), .c(new_n112_), .d(new_n111_), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n141_), .c(new_n55_), .d(new_n54_), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(new_n61_), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n139_), .O(z18));
endmodule


