// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x25), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n49_), .c(new_n50_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n47_), .c(new_n62_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n61_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n47_), .b(new_n67_), .O(z02));
  inv1   g23(.a(new_n61_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n47_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand3  g32(.a(new_n47_), .b(new_n77_), .c(new_n46_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g36(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g37(.a(x23), .O(new_n83_));
  nand3  g38(.a(new_n56_), .b(x22), .c(x21), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n50_), .c(x24), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(x25), .b(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n86_), .O(z08));
  nand2  g44(.a(new_n87_), .b(new_n62_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n63_), .c(new_n47_), .O(z09));
  nand4  g46(.a(new_n63_), .b(new_n52_), .c(new_n87_), .d(new_n62_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n63_), .c(new_n87_), .d(new_n62_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n47_), .O(z11));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n54_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n54_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n63_), .c(new_n87_), .d(new_n62_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z12));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nor2   g59(.a(x20), .b(new_n54_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n63_), .d(new_n62_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n87_), .O(new_n108_));
  nand2  g63(.a(new_n50_), .b(x15), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(z13));
  aoi21  g65(.a(new_n105_), .b(new_n98_), .c(new_n57_), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n98_), .c(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n63_), .c(new_n62_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n111_), .c(new_n87_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n109_), .O(z14));
  nand3  g71(.a(new_n57_), .b(new_n51_), .c(x19), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n97_), .c(x22), .O(new_n118_));
  inv1   g73(.a(new_n103_), .O(new_n119_));
  nor3   g74(.a(x22), .b(x21), .c(x20), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n118_), .c(new_n63_), .d(new_n62_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n87_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n109_), .O(z15));
  aoi21  g79(.a(new_n120_), .b(new_n119_), .c(new_n83_), .O(new_n125_));
  nand4  g80(.a(new_n112_), .b(new_n119_), .c(new_n83_), .d(new_n58_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n63_), .c(new_n62_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n125_), .c(new_n87_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n109_), .O(z16));
  nand2  g84(.a(new_n126_), .b(x24), .O(new_n130_));
  nand2  g85(.a(new_n63_), .b(new_n62_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  nand4  g87(.a(new_n49_), .b(new_n83_), .c(new_n58_), .d(new_n57_), .O(new_n133_));
  inv1   g88(.a(new_n133_), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n105_), .c(new_n98_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n132_), .c(new_n130_), .d(new_n87_), .O(z17));
  nand2  g91(.a(new_n131_), .b(new_n47_), .O(new_n137_));
  oai21  g92(.a(new_n133_), .b(new_n106_), .c(new_n87_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n50_), .O(new_n139_));
  nand3  g94(.a(new_n135_), .b(x25), .c(new_n87_), .O(new_n140_));
  nand3  g95(.a(new_n140_), .b(new_n139_), .c(new_n137_), .O(z18));
endmodule


