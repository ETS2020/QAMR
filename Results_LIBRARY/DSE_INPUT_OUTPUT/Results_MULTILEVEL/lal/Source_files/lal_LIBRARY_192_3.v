// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n104_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x18), .b(x17), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  nor3   g04(.a(x25), .b(x23), .c(x20), .O(new_n50_));
  oai21  g05(.a(new_n50_), .b(x18), .c(x17), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  oai21  g09(.a(x19), .b(x18), .c(new_n54_), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n55_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n52_), .c(new_n53_), .O(new_n58_));
  aoi21  g13(.a(x05), .b(x04), .c(x07), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n51_), .O(z01));
  inv1   g15(.a(new_n47_), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n46_), .O(z02));
  nand2  g17(.a(new_n58_), .b(new_n51_), .O(z03));
  inv1   g18(.a(x08), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n47_), .O(z04));
  inv1   g26(.a(x13), .O(new_n72_));
  nand3  g27(.a(new_n47_), .b(new_n72_), .c(new_n64_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z05));
  nand3  g29(.a(new_n47_), .b(x14), .c(new_n64_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z06));
  nand3  g31(.a(new_n47_), .b(new_n64_), .c(x06), .O(z07));
  inv1   g32(.a(x23), .O(new_n78_));
  nand3  g33(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n78_), .c(new_n52_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x25), .c(new_n47_), .O(new_n81_));
  nor3   g36(.a(x19), .b(x18), .c(x17), .O(new_n82_));
  nand4  g37(.a(new_n82_), .b(x24), .c(x22), .d(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n81_), .O(z08));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n85_), .c(new_n47_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand4  g45(.a(new_n85_), .b(new_n90_), .c(new_n87_), .d(new_n86_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n47_), .O(z10));
  inv1   g47(.a(x18), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n85_), .c(new_n87_), .d(new_n86_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n47_), .O(z11));
  nand4  g51(.a(new_n85_), .b(new_n47_), .c(x19), .d(new_n87_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(x07), .O(z12));
  nand3  g53(.a(new_n85_), .b(new_n47_), .c(new_n86_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n54_), .c(new_n87_), .O(z13));
  nor2   g56(.a(x21), .b(x15), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n100_), .O(z14));
  nor2   g58(.a(x22), .b(x15), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n59_), .c(new_n61_), .O(z15));
  nand3  g60(.a(new_n100_), .b(new_n78_), .c(new_n87_), .O(z16));
  nand3  g61(.a(new_n100_), .b(new_n52_), .c(new_n87_), .O(z17));
  nand3  g62(.a(new_n100_), .b(new_n53_), .c(new_n87_), .O(z18));
endmodule


