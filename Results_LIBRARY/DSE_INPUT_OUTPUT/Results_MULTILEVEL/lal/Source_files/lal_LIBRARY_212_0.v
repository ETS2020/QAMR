// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x25), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  aoi21  g04(.a(x05), .b(x04), .c(x07), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n53_), .c(x15), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n52_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n47_), .b(new_n64_), .O(z02));
  inv1   g20(.a(x23), .O(new_n66_));
  inv1   g21(.a(new_n60_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  and2   g23(.a(x24), .b(x15), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  nand2  g36(.a(new_n58_), .b(new_n54_), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(x22), .c(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n66_), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(x24), .c(x15), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n53_), .O(z08));
  nand2  g41(.a(x05), .b(x04), .O(new_n87_));
  nor4   g42(.a(new_n87_), .b(new_n53_), .c(x15), .d(x07), .O(z09));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n87_), .b(new_n55_), .c(new_n89_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x25), .c(x15), .O(z10));
  inv1   g46(.a(x15), .O(new_n92_));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n87_), .c(x25), .d(new_n92_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(x07), .O(z11));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n57_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n57_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n87_), .c(x25), .d(new_n92_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(x07), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nor2   g58(.a(x20), .b(new_n57_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n97_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nor2   g61(.a(new_n106_), .b(new_n53_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n103_), .c(new_n50_), .d(new_n92_), .O(z13));
  inv1   g63(.a(x21), .O(new_n109_));
  aoi21  g64(.a(new_n104_), .b(new_n97_), .c(new_n109_), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n97_), .c(x19), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n87_), .c(new_n89_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n110_), .c(x25), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n92_), .O(z14));
  nand3  g70(.a(new_n109_), .b(new_n54_), .c(x19), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n96_), .c(x22), .O(new_n117_));
  inv1   g72(.a(x22), .O(new_n118_));
  inv1   g73(.a(new_n102_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n118_), .c(new_n109_), .d(new_n54_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n117_), .c(new_n87_), .d(new_n89_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x25), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n92_), .O(z15));
  nand2  g78(.a(new_n120_), .b(x23), .O(new_n124_));
  nand4  g79(.a(new_n111_), .b(new_n119_), .c(new_n66_), .d(new_n118_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(x25), .c(new_n92_), .O(new_n126_));
  inv1   g81(.a(new_n126_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n124_), .c(new_n50_), .O(z16));
  nand2  g83(.a(new_n125_), .b(x24), .O(new_n129_));
  nor2   g84(.a(x24), .b(x23), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n118_), .c(new_n109_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  aoi21  g87(.a(new_n132_), .b(new_n106_), .c(new_n53_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n129_), .c(new_n50_), .d(new_n92_), .O(z17));
  nand2  g89(.a(new_n119_), .b(new_n50_), .O(new_n135_));
  nand3  g90(.a(new_n130_), .b(new_n111_), .c(new_n118_), .O(new_n136_));
  oai21  g91(.a(new_n136_), .b(new_n135_), .c(x25), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n92_), .O(z18));
endmodule


