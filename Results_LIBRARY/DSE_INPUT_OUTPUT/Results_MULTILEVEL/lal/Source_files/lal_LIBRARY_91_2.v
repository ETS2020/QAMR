// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x18), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x23), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(x24), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g13(.a(new_n55_), .b(new_n51_), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x18), .O(new_n61_));
  nor2   g16(.a(x19), .b(x17), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x15), .c(new_n61_), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n51_), .c(new_n52_), .d(new_n60_), .O(new_n64_));
  oai21  g19(.a(new_n59_), .b(new_n48_), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n49_), .b(new_n66_), .O(z02));
  nand2  g22(.a(new_n55_), .b(new_n49_), .O(new_n68_));
  nand3  g23(.a(new_n63_), .b(new_n52_), .c(new_n60_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  nor3   g32(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z06));
  nand3  g35(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g36(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n62_), .c(x15), .O(new_n84_));
  oai21  g39(.a(new_n53_), .b(new_n60_), .c(new_n52_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x25), .O(new_n86_));
  oai21  g41(.a(new_n84_), .b(x18), .c(new_n86_), .O(z08));
  nand2  g42(.a(new_n47_), .b(new_n56_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n57_), .c(new_n49_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand4  g45(.a(new_n57_), .b(new_n90_), .c(new_n47_), .d(new_n56_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n49_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n57_), .c(new_n47_), .d(new_n56_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z11));
  nand2  g50(.a(x19), .b(new_n90_), .O(new_n96_));
  inv1   g51(.a(x19), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(x18), .c(x17), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n96_), .c(x15), .O(new_n99_));
  nor2   g54(.a(new_n97_), .b(x18), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n99_), .c(new_n57_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(x07), .c(new_n49_), .O(z12));
  nand3  g57(.a(new_n60_), .b(x19), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x18), .O(new_n105_));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(x20), .c(new_n58_), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(x15), .c(new_n105_), .O(z13));
  inv1   g63(.a(new_n58_), .O(new_n109_));
  nand4  g64(.a(new_n60_), .b(x19), .c(x18), .d(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x21), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(x19), .c(x18), .d(x17), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n109_), .d(new_n47_), .O(z14));
  nand2  g69(.a(new_n113_), .b(x22), .O(new_n115_));
  inv1   g70(.a(x21), .O(new_n116_));
  inv1   g71(.a(x22), .O(new_n117_));
  inv1   g72(.a(new_n106_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(new_n60_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n115_), .c(new_n109_), .d(new_n47_), .O(z15));
  nand2  g75(.a(new_n119_), .b(x23), .O(new_n121_));
  nand4  g76(.a(new_n112_), .b(new_n118_), .c(new_n52_), .d(new_n117_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n121_), .c(new_n109_), .d(new_n47_), .O(z16));
  inv1   g78(.a(x24), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n52_), .c(new_n117_), .d(new_n116_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n103_), .c(new_n47_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(x18), .O(new_n127_));
  aoi21  g82(.a(new_n122_), .b(x24), .c(new_n58_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(x15), .c(new_n127_), .O(z17));
  nand3  g84(.a(new_n112_), .b(x19), .c(x17), .O(new_n130_));
  nand4  g85(.a(new_n51_), .b(new_n124_), .c(new_n52_), .d(new_n117_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n130_), .c(new_n47_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(x18), .O(new_n133_));
  oai21  g88(.a(new_n125_), .b(new_n110_), .c(x25), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n109_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n47_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n133_), .O(z18));
endmodule


