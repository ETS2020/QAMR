// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n92_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n102_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  nor2   g002(.a(x15), .b(x10), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  nand2  g004(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g005(.a(z02), .b(new_n46_), .O(z00));
  inv1   g006(.a(x24), .O(new_n52_));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  inv1   g009(.a(x17), .O(new_n55_));
  inv1   g010(.a(x18), .O(new_n56_));
  inv1   g011(.a(x19), .O(new_n57_));
  nand3  g012(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g013(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  inv1   g014(.a(x21), .O(new_n60_));
  inv1   g015(.a(x22), .O(new_n61_));
  nor2   g016(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g017(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  oai21  g018(.a(new_n63_), .b(new_n52_), .c(new_n53_), .O(new_n64_));
  nand2  g019(.a(x05), .b(x04), .O(new_n65_));
  inv1   g020(.a(new_n65_), .O(new_n66_));
  nor3   g021(.a(new_n66_), .b(new_n48_), .c(x07), .O(new_n67_));
  nand2  g022(.a(new_n67_), .b(new_n64_), .O(z01));
  nand2  g023(.a(new_n64_), .b(new_n49_), .O(z03));
  inv1   g024(.a(x00), .O(new_n70_));
  nand2  g025(.a(x09), .b(new_n70_), .O(new_n71_));
  inv1   g026(.a(x09), .O(new_n72_));
  nand2  g027(.a(new_n72_), .b(x00), .O(new_n73_));
  inv1   g028(.a(x02), .O(new_n74_));
  nand2  g029(.a(x11), .b(new_n74_), .O(new_n75_));
  nand3  g030(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  inv1   g031(.a(x11), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(x02), .O(new_n78_));
  inv1   g033(.a(x03), .O(new_n79_));
  nand2  g034(.a(x12), .b(new_n79_), .O(new_n80_));
  inv1   g035(.a(x12), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(x03), .O(new_n82_));
  nand3  g037(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(new_n83_));
  oai21  g038(.a(new_n83_), .b(new_n76_), .c(new_n49_), .O(new_n84_));
  inv1   g039(.a(x10), .O(new_n85_));
  nor2   g040(.a(new_n85_), .b(x01), .O(new_n86_));
  inv1   g041(.a(x15), .O(new_n87_));
  nor2   g042(.a(new_n87_), .b(x10), .O(new_n88_));
  aoi21  g043(.a(new_n88_), .b(x01), .c(new_n86_), .O(new_n89_));
  aoi21  g044(.a(new_n89_), .b(new_n84_), .c(x08), .O(z04));
  oai21  g045(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g046(.a(x14), .b(new_n46_), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n49_), .O(z06));
  nand3  g048(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g049(.a(new_n58_), .b(new_n54_), .c(new_n61_), .O(new_n95_));
  aoi21  g050(.a(new_n95_), .b(x21), .c(x23), .O(new_n96_));
  nor2   g051(.a(new_n48_), .b(x25), .O(new_n97_));
  oai21  g052(.a(new_n96_), .b(new_n52_), .c(new_n97_), .O(z08));
  inv1   g053(.a(x07), .O(new_n99_));
  nand3  g054(.a(new_n99_), .b(x05), .c(x04), .O(new_n100_));
  aoi21  g055(.a(new_n100_), .b(x10), .c(x15), .O(z09));
  nand3  g056(.a(new_n65_), .b(new_n55_), .c(new_n99_), .O(new_n102_));
  aoi21  g057(.a(new_n102_), .b(x10), .c(x15), .O(z10));
  xor2a  g058(.a(x18), .b(x17), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n65_), .c(new_n87_), .d(x10), .O(new_n105_));
  nor2   g060(.a(new_n105_), .b(x07), .O(z11));
  nor2   g061(.a(new_n56_), .b(new_n55_), .O(new_n107_));
  nand3  g062(.a(new_n57_), .b(x18), .c(x17), .O(new_n108_));
  oai21  g063(.a(new_n107_), .b(new_n57_), .c(new_n108_), .O(new_n109_));
  nand3  g064(.a(new_n109_), .b(new_n65_), .c(new_n99_), .O(new_n110_));
  aoi21  g065(.a(new_n110_), .b(x10), .c(x15), .O(z12));
  nand3  g066(.a(x19), .b(x18), .c(x17), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(x20), .O(new_n113_));
  nand4  g068(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n114_));
  nand4  g069(.a(new_n114_), .b(new_n113_), .c(new_n65_), .d(new_n99_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(x10), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(new_n87_), .O(z13));
  nand2  g072(.a(new_n114_), .b(x21), .O(new_n118_));
  nor2   g073(.a(new_n66_), .b(x07), .O(new_n119_));
  nand4  g074(.a(new_n107_), .b(new_n60_), .c(new_n54_), .d(x19), .O(new_n120_));
  nand3  g075(.a(new_n120_), .b(new_n87_), .c(x10), .O(new_n121_));
  inv1   g076(.a(new_n121_), .O(new_n122_));
  nand3  g077(.a(new_n122_), .b(new_n119_), .c(new_n118_), .O(z14));
  nand2  g078(.a(new_n120_), .b(x22), .O(new_n124_));
  inv1   g079(.a(new_n112_), .O(new_n125_));
  nor2   g080(.a(x22), .b(x21), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n125_), .c(new_n54_), .O(new_n127_));
  and2   g082(.a(new_n127_), .b(new_n87_), .O(new_n128_));
  nand4  g083(.a(new_n128_), .b(new_n124_), .c(new_n119_), .d(x10), .O(z15));
  nand2  g084(.a(new_n127_), .b(x23), .O(new_n130_));
  inv1   g085(.a(x23), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n61_), .c(new_n60_), .d(new_n54_), .O(new_n132_));
  inv1   g087(.a(new_n132_), .O(new_n133_));
  aoi21  g088(.a(new_n133_), .b(new_n125_), .c(x15), .O(new_n134_));
  nand4  g089(.a(new_n134_), .b(new_n130_), .c(new_n119_), .d(x10), .O(z16));
  oai21  g090(.a(new_n132_), .b(new_n112_), .c(x24), .O(new_n136_));
  nor2   g091(.a(x20), .b(new_n57_), .O(new_n137_));
  nor2   g092(.a(x24), .b(x23), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n126_), .c(new_n137_), .d(new_n107_), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n136_), .c(new_n65_), .d(new_n99_), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(x10), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n87_), .O(z17));
  nand4  g097(.a(new_n52_), .b(new_n131_), .c(new_n61_), .d(new_n60_), .O(new_n143_));
  oai21  g098(.a(new_n143_), .b(new_n114_), .c(x25), .O(new_n144_));
  nor3   g099(.a(x25), .b(x24), .c(x23), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(new_n126_), .c(new_n137_), .d(new_n107_), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(new_n144_), .c(new_n65_), .d(new_n99_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(x10), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n87_), .O(z18));
endmodule


