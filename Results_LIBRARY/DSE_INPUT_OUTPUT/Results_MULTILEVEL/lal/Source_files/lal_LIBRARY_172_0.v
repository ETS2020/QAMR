// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x15), .b(x04), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n49_), .c(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  oai21  g14(.a(new_n58_), .b(new_n47_), .c(new_n59_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nor2   g16(.a(new_n47_), .b(new_n61_), .O(z02));
  inv1   g17(.a(new_n47_), .O(new_n63_));
  nand3  g18(.a(new_n57_), .b(new_n63_), .c(new_n49_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(z03));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(new_n63_), .c(new_n46_), .O(new_n71_));
  inv1   g26(.a(new_n71_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n63_), .O(z05));
  nand2  g28(.a(x14), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n63_), .O(z06));
  nand3  g30(.a(new_n63_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g31(.a(x23), .O(new_n77_));
  nand2  g32(.a(new_n54_), .b(new_n50_), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(x22), .c(x21), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x24), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n49_), .c(new_n47_), .O(z08));
  inv1   g37(.a(x07), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n83_), .c(x05), .d(x04), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  inv1   g41(.a(x05), .O(new_n87_));
  nand3  g42(.a(new_n51_), .b(new_n83_), .c(new_n87_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x04), .c(x15), .O(z10));
  xor2a  g44(.a(x18), .b(x17), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n83_), .c(new_n87_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(x04), .c(x15), .O(z11));
  inv1   g47(.a(x04), .O(new_n93_));
  nor2   g48(.a(new_n52_), .b(new_n51_), .O(new_n94_));
  nand3  g49(.a(new_n53_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n53_), .c(new_n95_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n84_), .c(new_n83_), .d(new_n87_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(new_n93_), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n94_), .b(new_n50_), .c(x19), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n100_), .c(new_n83_), .d(new_n87_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x04), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n84_), .O(z13));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  nor2   g60(.a(x05), .b(new_n93_), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n94_), .c(x19), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n84_), .c(new_n83_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n106_), .c(new_n105_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x21), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nor3   g70(.a(new_n115_), .b(new_n99_), .c(x20), .O(new_n116_));
  nor2   g71(.a(new_n116_), .b(x15), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n112_), .c(new_n106_), .d(new_n83_), .O(z15));
  nor2   g73(.a(new_n116_), .b(new_n77_), .O(new_n119_));
  inv1   g74(.a(new_n99_), .O(new_n120_));
  nand4  g75(.a(new_n107_), .b(new_n120_), .c(new_n77_), .d(new_n114_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n83_), .c(new_n87_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n119_), .c(x04), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n84_), .O(z16));
  nand2  g79(.a(new_n121_), .b(x24), .O(new_n125_));
  inv1   g80(.a(x24), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n77_), .c(new_n114_), .d(new_n113_), .O(new_n127_));
  inv1   g82(.a(new_n127_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n94_), .c(new_n50_), .d(x19), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n125_), .c(new_n83_), .d(new_n87_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(x04), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n84_), .O(z17));
  nand2  g87(.a(new_n129_), .b(x25), .O(new_n133_));
  inv1   g88(.a(new_n101_), .O(new_n134_));
  nand3  g89(.a(new_n49_), .b(new_n126_), .c(new_n77_), .O(new_n135_));
  nor2   g90(.a(new_n135_), .b(new_n115_), .O(new_n136_));
  aoi21  g91(.a(new_n136_), .b(new_n134_), .c(x15), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n133_), .c(new_n106_), .d(new_n83_), .O(z18));
endmodule


