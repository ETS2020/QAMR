// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n90_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n99_, new_n101_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  inv1   g002(.a(x03), .O(new_n48_));
  nor2   g003(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g006(.a(z02), .b(new_n46_), .O(z00));
  inv1   g007(.a(x24), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x20), .O(new_n55_));
  inv1   g010(.a(x17), .O(new_n56_));
  inv1   g011(.a(x18), .O(new_n57_));
  inv1   g012(.a(x19), .O(new_n58_));
  nand3  g013(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g014(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  inv1   g015(.a(x21), .O(new_n61_));
  inv1   g016(.a(x22), .O(new_n62_));
  nor2   g017(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g018(.a(new_n63_), .b(new_n60_), .c(x23), .O(new_n64_));
  oai21  g019(.a(new_n64_), .b(new_n53_), .c(new_n54_), .O(new_n65_));
  nand2  g020(.a(x05), .b(x04), .O(new_n66_));
  inv1   g021(.a(new_n66_), .O(new_n67_));
  nor2   g022(.a(new_n67_), .b(x07), .O(new_n68_));
  aoi21  g023(.a(new_n68_), .b(new_n65_), .c(new_n49_), .O(z01));
  nand2  g024(.a(new_n65_), .b(new_n50_), .O(z03));
  inv1   g025(.a(x09), .O(new_n71_));
  inv1   g026(.a(x01), .O(new_n72_));
  aoi22  g027(.a(x10), .b(new_n72_), .c(new_n71_), .d(x00), .O(new_n73_));
  oai21  g028(.a(new_n71_), .b(x00), .c(new_n73_), .O(new_n74_));
  inv1   g029(.a(x10), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(x01), .O(new_n76_));
  inv1   g031(.a(x02), .O(new_n77_));
  nand2  g032(.a(x11), .b(new_n77_), .O(new_n78_));
  inv1   g033(.a(x11), .O(new_n79_));
  nand2  g034(.a(new_n79_), .b(x02), .O(new_n80_));
  nand3  g035(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  oai21  g036(.a(new_n81_), .b(new_n74_), .c(new_n50_), .O(new_n82_));
  inv1   g037(.a(x12), .O(new_n83_));
  nor2   g038(.a(new_n83_), .b(x03), .O(new_n84_));
  inv1   g039(.a(x15), .O(new_n85_));
  nor2   g040(.a(new_n85_), .b(x12), .O(new_n86_));
  aoi21  g041(.a(new_n86_), .b(x03), .c(new_n84_), .O(new_n87_));
  aoi21  g042(.a(new_n87_), .b(new_n82_), .c(x08), .O(z04));
  oai21  g043(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g044(.a(x14), .O(new_n90_));
  nor3   g045(.a(new_n49_), .b(new_n90_), .c(x08), .O(z06));
  aoi21  g046(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g047(.a(x23), .O(new_n93_));
  nand3  g048(.a(new_n60_), .b(x22), .c(x21), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g050(.a(new_n95_), .b(x24), .O(new_n96_));
  aoi21  g051(.a(new_n96_), .b(new_n54_), .c(new_n49_), .O(z08));
  inv1   g052(.a(x07), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(x05), .c(x04), .d(new_n48_), .O(new_n99_));
  nor2   g054(.a(new_n99_), .b(x15), .O(z09));
  nand4  g055(.a(new_n66_), .b(new_n56_), .c(new_n85_), .d(new_n98_), .O(new_n101_));
  nor2   g056(.a(new_n101_), .b(x03), .O(z10));
  xor2a  g057(.a(x18), .b(x17), .O(new_n103_));
  nand4  g058(.a(new_n103_), .b(new_n66_), .c(new_n85_), .d(new_n98_), .O(new_n104_));
  nor2   g059(.a(new_n104_), .b(x03), .O(z11));
  nor2   g060(.a(new_n57_), .b(new_n56_), .O(new_n106_));
  nand3  g061(.a(new_n58_), .b(x18), .c(x17), .O(new_n107_));
  oai21  g062(.a(new_n106_), .b(new_n58_), .c(new_n107_), .O(new_n108_));
  nand3  g063(.a(new_n108_), .b(new_n66_), .c(new_n98_), .O(new_n109_));
  aoi21  g064(.a(new_n109_), .b(new_n48_), .c(x15), .O(z12));
  nand3  g065(.a(x19), .b(x18), .c(x17), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(x20), .O(new_n112_));
  nor2   g067(.a(new_n67_), .b(x03), .O(new_n113_));
  nand4  g068(.a(new_n55_), .b(x19), .c(x18), .d(x17), .O(new_n114_));
  inv1   g069(.a(new_n114_), .O(new_n115_));
  nor2   g070(.a(new_n115_), .b(x15), .O(new_n116_));
  nand4  g071(.a(new_n116_), .b(new_n113_), .c(new_n112_), .d(new_n98_), .O(z13));
  nor2   g072(.a(new_n115_), .b(new_n61_), .O(new_n118_));
  nor2   g073(.a(x21), .b(x20), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n106_), .c(x19), .O(new_n120_));
  nand3  g075(.a(new_n120_), .b(new_n66_), .c(new_n98_), .O(new_n121_));
  oai21  g076(.a(new_n121_), .b(new_n118_), .c(new_n48_), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(new_n85_), .O(z14));
  inv1   g078(.a(new_n111_), .O(new_n124_));
  nor3   g079(.a(x22), .b(x21), .c(x20), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n66_), .c(new_n98_), .O(new_n127_));
  aoi21  g082(.a(new_n120_), .b(x22), .c(new_n127_), .O(new_n128_));
  oai21  g083(.a(new_n128_), .b(x03), .c(new_n85_), .O(z15));
  aoi21  g084(.a(new_n125_), .b(new_n124_), .c(new_n93_), .O(new_n130_));
  nand4  g085(.a(new_n119_), .b(new_n124_), .c(new_n93_), .d(new_n62_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n66_), .c(new_n98_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(new_n130_), .c(new_n48_), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n85_), .O(z16));
  nand2  g089(.a(new_n131_), .b(x24), .O(new_n135_));
  nor2   g090(.a(x22), .b(x21), .O(new_n136_));
  nor2   g091(.a(x24), .b(x23), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  inv1   g093(.a(new_n138_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n115_), .O(new_n140_));
  nand4  g095(.a(new_n140_), .b(new_n135_), .c(new_n66_), .d(new_n98_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n48_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(new_n85_), .O(z17));
  oai21  g098(.a(new_n138_), .b(new_n114_), .c(x25), .O(new_n144_));
  nor3   g099(.a(x25), .b(x24), .c(x23), .O(new_n145_));
  nand3  g100(.a(new_n145_), .b(new_n136_), .c(new_n115_), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(new_n144_), .c(new_n66_), .d(new_n98_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n48_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n85_), .O(z18));
endmodule


