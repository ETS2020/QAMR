// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(x19), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(x24), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n57_), .b(new_n53_), .c(new_n60_), .O(new_n61_));
  inv1   g16(.a(x20), .O(new_n62_));
  inv1   g17(.a(x19), .O(new_n63_));
  nor2   g18(.a(x18), .b(x17), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x15), .c(new_n63_), .O(new_n65_));
  nand4  g20(.a(new_n65_), .b(new_n53_), .c(new_n54_), .d(new_n62_), .O(new_n66_));
  oai21  g21(.a(new_n61_), .b(new_n49_), .c(new_n66_), .O(z01));
  nand2  g22(.a(new_n57_), .b(new_n50_), .O(new_n68_));
  nand3  g23(.a(new_n65_), .b(new_n54_), .c(new_n62_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor3   g34(.a(new_n49_), .b(new_n79_), .c(x08), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand3  g36(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n64_), .c(x15), .O(new_n84_));
  oai21  g39(.a(new_n55_), .b(new_n62_), .c(new_n54_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x25), .O(new_n86_));
  oai21  g41(.a(new_n84_), .b(x19), .c(new_n86_), .O(z08));
  nand2  g42(.a(new_n48_), .b(new_n58_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n59_), .c(new_n50_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand4  g45(.a(new_n59_), .b(new_n90_), .c(new_n48_), .d(new_n58_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n59_), .c(new_n48_), .d(new_n58_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n50_), .O(z11));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(x19), .c(new_n48_), .O(new_n97_));
  nand3  g52(.a(new_n63_), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n59_), .c(new_n58_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n50_), .O(z12));
  nand3  g56(.a(new_n62_), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x19), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(x20), .c(new_n60_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(x15), .c(new_n104_), .O(z13));
  inv1   g62(.a(new_n60_), .O(new_n108_));
  nand4  g63(.a(new_n62_), .b(x19), .c(x18), .d(x17), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x21), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(x19), .c(x18), .d(x17), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n110_), .c(new_n108_), .d(new_n48_), .O(z14));
  inv1   g68(.a(x21), .O(new_n114_));
  inv1   g69(.a(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n62_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n96_), .c(new_n48_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x19), .O(new_n118_));
  aoi21  g73(.a(new_n112_), .b(x22), .c(new_n60_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x15), .c(new_n118_), .O(z15));
  nand3  g75(.a(new_n54_), .b(new_n115_), .c(new_n114_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n102_), .c(new_n48_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x19), .O(new_n123_));
  inv1   g78(.a(new_n105_), .O(new_n124_));
  nor2   g79(.a(x22), .b(x21), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n62_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(x23), .c(new_n60_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(x15), .c(new_n123_), .O(z16));
  nand3  g83(.a(new_n111_), .b(new_n54_), .c(new_n115_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n105_), .c(x24), .O(new_n130_));
  inv1   g85(.a(new_n109_), .O(new_n131_));
  inv1   g86(.a(x24), .O(new_n132_));
  nand3  g87(.a(new_n125_), .b(new_n132_), .c(new_n54_), .O(new_n133_));
  inv1   g88(.a(new_n133_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n130_), .c(new_n108_), .d(new_n48_), .O(z17));
  oai21  g91(.a(new_n133_), .b(new_n109_), .c(x25), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand3  g93(.a(new_n138_), .b(new_n125_), .c(new_n131_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n137_), .c(new_n108_), .d(new_n48_), .O(z18));
endmodule


