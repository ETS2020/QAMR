// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  inv1   g14(.a(x21), .O(new_n60_));
  inv1   g15(.a(x22), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(new_n48_), .c(new_n53_), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(new_n66_));
  nor3   g21(.a(new_n66_), .b(new_n49_), .c(x07), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n64_), .O(z01));
  nand2  g23(.a(new_n63_), .b(x15), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(x24), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g36(.a(new_n58_), .b(new_n54_), .c(new_n61_), .O(new_n82_));
  inv1   g37(.a(x23), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x15), .O(new_n84_));
  aoi21  g39(.a(new_n82_), .b(x21), .c(new_n84_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n48_), .c(new_n53_), .O(z08));
  nor4   g41(.a(new_n65_), .b(x24), .c(x15), .d(x07), .O(z09));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n65_), .b(new_n55_), .c(new_n88_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n48_), .c(x15), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n65_), .c(new_n88_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n48_), .c(x15), .O(z11));
  inv1   g48(.a(x15), .O(new_n94_));
  nor2   g49(.a(new_n56_), .b(new_n55_), .O(new_n95_));
  nand3  g50(.a(new_n57_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n57_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n65_), .c(new_n48_), .d(new_n94_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nor2   g56(.a(new_n66_), .b(x07), .O(new_n102_));
  nand4  g57(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(x24), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n102_), .c(new_n101_), .d(new_n94_), .O(z13));
  nor2   g61(.a(new_n104_), .b(new_n60_), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n95_), .c(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n65_), .c(new_n88_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n107_), .c(new_n48_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n94_), .O(z14));
  nand2  g67(.a(new_n109_), .b(x22), .O(new_n113_));
  inv1   g68(.a(new_n100_), .O(new_n114_));
  nor3   g69(.a(x22), .b(x21), .c(x20), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n114_), .c(x24), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n113_), .c(new_n102_), .d(new_n94_), .O(z15));
  aoi21  g72(.a(new_n115_), .b(new_n114_), .c(new_n83_), .O(new_n118_));
  nand4  g73(.a(new_n108_), .b(new_n114_), .c(new_n83_), .d(new_n61_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n65_), .c(new_n88_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n118_), .c(new_n48_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n94_), .O(z16));
  nand2  g77(.a(new_n120_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n94_), .O(z17));
  nand4  g79(.a(new_n53_), .b(new_n83_), .c(new_n61_), .d(new_n60_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n103_), .c(new_n88_), .O(new_n126_));
  nor2   g81(.a(new_n126_), .b(new_n66_), .O(new_n127_));
  nand2  g82(.a(new_n119_), .b(x25), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n127_), .c(new_n48_), .d(new_n94_), .O(z18));
endmodule


