// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n90_, new_n93_, new_n94_, new_n95_,
    new_n97_, new_n98_, new_n100_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x12), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n48_), .c(x07), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  inv1   g23(.a(new_n48_), .O(new_n69_));
  nand2  g24(.a(new_n62_), .b(new_n69_), .O(z03));
  inv1   g25(.a(x12), .O(new_n71_));
  inv1   g26(.a(x03), .O(new_n72_));
  oai21  g27(.a(x08), .b(new_n72_), .c(new_n47_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  inv1   g29(.a(x09), .O(new_n75_));
  inv1   g30(.a(x01), .O(new_n76_));
  aoi22  g31(.a(x10), .b(new_n76_), .c(new_n75_), .d(x00), .O(new_n77_));
  oai21  g32(.a(new_n75_), .b(x00), .c(new_n77_), .O(new_n78_));
  inv1   g33(.a(x10), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x01), .O(new_n80_));
  inv1   g35(.a(x02), .O(new_n81_));
  nand2  g36(.a(x11), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(x11), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x02), .O(new_n84_));
  nand2  g39(.a(x12), .b(new_n72_), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n84_), .c(new_n82_), .d(new_n80_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n78_), .c(new_n46_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n74_), .O(z04));
  oai21  g43(.a(x13), .b(x08), .c(new_n69_), .O(z05));
  inv1   g44(.a(x14), .O(new_n90_));
  nor3   g45(.a(new_n48_), .b(new_n90_), .c(x08), .O(z06));
  aoi21  g46(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g47(.a(new_n56_), .b(new_n52_), .c(new_n59_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x21), .c(x23), .O(new_n94_));
  nor2   g49(.a(new_n48_), .b(x25), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n50_), .c(new_n95_), .O(z08));
  inv1   g51(.a(x07), .O(new_n97_));
  nand3  g52(.a(new_n64_), .b(new_n47_), .c(new_n97_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z09));
  nand4  g54(.a(new_n63_), .b(new_n53_), .c(new_n47_), .d(new_n97_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z10));
  xor2a  g56(.a(x18), .b(x17), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n63_), .c(new_n47_), .d(new_n97_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z11));
  nor2   g59(.a(new_n54_), .b(new_n53_), .O(new_n105_));
  nand3  g60(.a(new_n55_), .b(x18), .c(x17), .O(new_n106_));
  oai21  g61(.a(new_n105_), .b(new_n55_), .c(new_n106_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n63_), .c(new_n47_), .d(new_n97_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(z12));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x20), .O(new_n111_));
  nand4  g66(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n111_), .c(new_n63_), .d(new_n97_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n47_), .O(new_n114_));
  nand2  g69(.a(x15), .b(x12), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(z13));
  nand2  g71(.a(new_n112_), .b(x21), .O(new_n117_));
  nor2   g72(.a(new_n64_), .b(x07), .O(new_n118_));
  nand4  g73(.a(new_n105_), .b(new_n58_), .c(new_n52_), .d(x19), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n118_), .c(new_n117_), .d(new_n47_), .O(z14));
  nand2  g75(.a(new_n119_), .b(x22), .O(new_n121_));
  inv1   g76(.a(new_n110_), .O(new_n122_));
  nor2   g77(.a(x22), .b(x21), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n52_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n121_), .c(new_n118_), .d(new_n47_), .O(z15));
  nand2  g80(.a(new_n124_), .b(x23), .O(new_n126_));
  inv1   g81(.a(x23), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n59_), .c(new_n58_), .d(new_n52_), .O(new_n128_));
  inv1   g83(.a(new_n128_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n122_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n126_), .c(new_n118_), .d(new_n47_), .O(z16));
  oai21  g86(.a(new_n128_), .b(new_n110_), .c(x24), .O(new_n132_));
  nor2   g87(.a(x20), .b(new_n55_), .O(new_n133_));
  nor2   g88(.a(x24), .b(x23), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n123_), .c(new_n133_), .d(new_n105_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n132_), .c(new_n63_), .d(new_n97_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n47_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n115_), .O(z17));
  nand4  g93(.a(new_n50_), .b(new_n127_), .c(new_n59_), .d(new_n58_), .O(new_n139_));
  oai21  g94(.a(new_n139_), .b(new_n112_), .c(x25), .O(new_n140_));
  nor3   g95(.a(x25), .b(x24), .c(x23), .O(new_n141_));
  nand4  g96(.a(new_n141_), .b(new_n123_), .c(new_n133_), .d(new_n105_), .O(new_n142_));
  nand4  g97(.a(new_n142_), .b(new_n140_), .c(new_n63_), .d(new_n97_), .O(new_n143_));
  nand2  g98(.a(new_n143_), .b(new_n47_), .O(new_n144_));
  nand2  g99(.a(new_n144_), .b(new_n115_), .O(z18));
endmodule


