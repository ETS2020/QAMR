// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x15), .b(x10), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(new_n47_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n47_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n60_), .b(new_n48_), .O(z03));
  inv1   g20(.a(x09), .O(new_n66_));
  inv1   g21(.a(x02), .O(new_n67_));
  aoi22  g22(.a(x11), .b(new_n67_), .c(new_n66_), .d(x00), .O(new_n68_));
  oai21  g23(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  inv1   g24(.a(x11), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x02), .O(new_n71_));
  inv1   g26(.a(x03), .O(new_n72_));
  nand2  g27(.a(x12), .b(new_n72_), .O(new_n73_));
  inv1   g28(.a(x12), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(x03), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(new_n69_), .c(new_n48_), .O(new_n77_));
  inv1   g32(.a(x10), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(x01), .O(new_n79_));
  inv1   g34(.a(x15), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(x10), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x01), .c(new_n79_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n77_), .c(x08), .O(z04));
  nor3   g38(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g39(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n48_), .O(z06));
  aoi21  g41(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g42(.a(x23), .O(new_n88_));
  nand3  g43(.a(new_n57_), .b(x22), .c(x21), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(x24), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n51_), .c(new_n47_), .O(z08));
  inv1   g47(.a(x07), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(x05), .c(x04), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(x10), .c(x15), .O(z09));
  nand2  g50(.a(x05), .b(x04), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n53_), .c(new_n80_), .d(x10), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(x07), .O(z10));
  xor2a  g53(.a(x18), .b(x17), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n96_), .c(new_n80_), .d(x10), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(x07), .O(z11));
  nor2   g56(.a(new_n54_), .b(new_n53_), .O(new_n102_));
  nand3  g57(.a(new_n55_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n55_), .c(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n96_), .c(new_n93_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(x10), .c(x15), .O(z12));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nor2   g63(.a(x20), .b(new_n55_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n102_), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(x15), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n108_), .c(new_n61_), .d(x10), .O(z13));
  nand2  g68(.a(new_n110_), .b(x21), .O(new_n114_));
  nor2   g69(.a(x21), .b(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n102_), .c(x19), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n80_), .c(x10), .O(new_n117_));
  inv1   g72(.a(new_n117_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n114_), .c(new_n61_), .O(z14));
  inv1   g74(.a(new_n107_), .O(new_n120_));
  nor2   g75(.a(x22), .b(x21), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n52_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n96_), .c(new_n93_), .O(new_n123_));
  aoi21  g78(.a(new_n116_), .b(x22), .c(new_n123_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n78_), .c(new_n80_), .O(z15));
  nand2  g80(.a(new_n122_), .b(x23), .O(new_n126_));
  nor2   g81(.a(x23), .b(x22), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n115_), .c(new_n120_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n80_), .c(x10), .O(new_n129_));
  inv1   g84(.a(new_n129_), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n126_), .c(new_n61_), .O(z16));
  nand2  g86(.a(new_n128_), .b(x24), .O(new_n132_));
  nor2   g87(.a(x24), .b(x23), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n121_), .c(new_n109_), .d(new_n102_), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n132_), .c(new_n96_), .d(new_n93_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(x10), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n80_), .O(z17));
  nand2  g92(.a(new_n134_), .b(x25), .O(new_n138_));
  nor3   g93(.a(x25), .b(x24), .c(x23), .O(new_n139_));
  and2   g94(.a(new_n139_), .b(new_n121_), .O(new_n140_));
  aoi21  g95(.a(new_n140_), .b(new_n111_), .c(x15), .O(new_n141_));
  nand4  g96(.a(new_n141_), .b(new_n138_), .c(new_n61_), .d(x10), .O(z18));
endmodule


