// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(x20), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nor2   g18(.a(new_n49_), .b(x07), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z01));
  nand2  g20(.a(new_n62_), .b(new_n50_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z04));
  nor3   g28(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z06));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x24), .O(new_n78_));
  inv1   g33(.a(x23), .O(new_n79_));
  nand2  g34(.a(new_n56_), .b(new_n55_), .O(new_n80_));
  nand3  g35(.a(x22), .b(x21), .c(new_n57_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  aoi21  g37(.a(new_n59_), .b(new_n79_), .c(new_n54_), .O(new_n83_));
  aoi21  g38(.a(new_n82_), .b(new_n48_), .c(new_n83_), .O(new_n84_));
  oai22  g39(.a(new_n84_), .b(new_n78_), .c(new_n49_), .d(new_n53_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand2  g41(.a(new_n48_), .b(new_n86_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n63_), .c(new_n50_), .O(z09));
  nand4  g43(.a(new_n63_), .b(new_n55_), .c(new_n48_), .d(new_n86_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n63_), .c(new_n48_), .d(new_n86_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n50_), .O(z11));
  nor2   g48(.a(new_n56_), .b(new_n55_), .O(new_n94_));
  nand3  g49(.a(new_n57_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n57_), .c(new_n95_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n63_), .c(new_n48_), .d(new_n86_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z12));
  nand2  g53(.a(new_n63_), .b(new_n86_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n50_), .O(new_n100_));
  nand2  g55(.a(x17), .b(new_n48_), .O(new_n101_));
  nand2  g56(.a(x19), .b(x18), .O(new_n102_));
  oai21  g57(.a(new_n102_), .b(new_n101_), .c(x20), .O(new_n103_));
  nor2   g58(.a(x20), .b(new_n57_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(x18), .c(x17), .d(new_n48_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n100_), .O(z13));
  oai21  g61(.a(x21), .b(x15), .c(x20), .O(new_n107_));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n94_), .c(x19), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n63_), .d(new_n86_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n48_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n107_), .O(z14));
  oai21  g69(.a(x22), .b(x15), .c(x20), .O(new_n115_));
  inv1   g70(.a(x21), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(x19), .c(x18), .d(x17), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x22), .O(new_n118_));
  inv1   g73(.a(new_n108_), .O(new_n119_));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n54_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n118_), .c(new_n63_), .d(new_n86_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n115_), .O(z15));
  inv1   g79(.a(new_n99_), .O(new_n125_));
  nand2  g80(.a(new_n121_), .b(x23), .O(new_n126_));
  inv1   g81(.a(x22), .O(new_n127_));
  nand4  g82(.a(new_n110_), .b(new_n119_), .c(new_n79_), .d(new_n127_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n126_), .c(new_n125_), .d(new_n48_), .O(z16));
  nand2  g84(.a(new_n128_), .b(x24), .O(new_n130_));
  nand4  g85(.a(new_n78_), .b(new_n79_), .c(new_n127_), .d(new_n116_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n104_), .c(new_n94_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n130_), .c(new_n125_), .d(new_n48_), .O(z17));
  oai21  g89(.a(x25), .b(x15), .c(x20), .O(new_n135_));
  oai21  g90(.a(new_n131_), .b(new_n108_), .c(x25), .O(new_n136_));
  nor3   g91(.a(x25), .b(x24), .c(x23), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n120_), .c(new_n104_), .d(new_n94_), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n136_), .c(new_n63_), .d(new_n86_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n48_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n135_), .O(z18));
endmodule


