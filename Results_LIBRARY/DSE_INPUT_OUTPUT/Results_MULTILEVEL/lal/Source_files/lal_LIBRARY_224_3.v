// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x19), .b(x15), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  nor3   g04(.a(x25), .b(x23), .c(x20), .O(new_n50_));
  oai21  g05(.a(new_n50_), .b(x15), .c(x19), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  oai21  g09(.a(x18), .b(x17), .c(new_n54_), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n55_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n52_), .c(new_n53_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x07), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n58_), .c(new_n51_), .O(z01));
  nand2  g17(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g18(.a(new_n58_), .b(new_n51_), .O(z03));
  inv1   g19(.a(x08), .O(new_n65_));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(new_n47_), .c(new_n65_), .O(new_n71_));
  inv1   g26(.a(new_n71_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g30(.a(new_n47_), .b(new_n65_), .c(x06), .O(z07));
  inv1   g31(.a(x23), .O(new_n77_));
  nand3  g32(.a(x22), .b(x21), .c(x20), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n77_), .c(new_n52_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x25), .c(new_n47_), .O(new_n80_));
  nor3   g35(.a(x19), .b(x18), .c(x17), .O(new_n81_));
  nand4  g36(.a(new_n81_), .b(x24), .c(x22), .d(x21), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n80_), .O(z08));
  inv1   g38(.a(x07), .O(new_n84_));
  inv1   g39(.a(x15), .O(new_n85_));
  nand3  g40(.a(new_n60_), .b(new_n85_), .c(new_n84_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z09));
  inv1   g42(.a(x17), .O(new_n88_));
  nand4  g43(.a(new_n59_), .b(new_n88_), .c(new_n85_), .d(new_n84_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n59_), .c(new_n85_), .d(new_n84_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n47_), .O(z11));
  inv1   g48(.a(x19), .O(new_n94_));
  and2   g49(.a(x18), .b(x17), .O(new_n95_));
  nand3  g50(.a(new_n94_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n94_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n59_), .c(new_n85_), .d(new_n84_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n95_), .b(new_n54_), .c(x19), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n101_), .c(new_n61_), .d(new_n85_), .O(z13));
  oai21  g58(.a(x21), .b(x15), .c(new_n94_), .O(new_n104_));
  nand3  g59(.a(new_n54_), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x21), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n95_), .c(x19), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n59_), .d(new_n84_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n85_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n104_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  inv1   g67(.a(new_n100_), .O(new_n113_));
  nor2   g68(.a(x22), .b(x21), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n54_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n61_), .d(new_n85_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n107_), .c(new_n113_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n117_), .c(new_n61_), .d(new_n85_), .O(z16));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n121_));
  inv1   g76(.a(new_n102_), .O(new_n122_));
  nand3  g77(.a(new_n114_), .b(new_n52_), .c(new_n77_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n121_), .c(new_n61_), .d(new_n85_), .O(z17));
  oai21  g81(.a(new_n123_), .b(new_n102_), .c(x25), .O(new_n127_));
  nor3   g82(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n114_), .c(new_n122_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n127_), .c(new_n61_), .d(new_n85_), .O(z18));
endmodule


