// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x15), .b(x00), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(new_n47_), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  and2   g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n58_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n51_), .c(new_n52_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x07), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n61_), .c(new_n50_), .O(z01));
  nand2  g20(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g21(.a(new_n61_), .b(new_n47_), .O(z03));
  inv1   g22(.a(x10), .O(new_n68_));
  inv1   g23(.a(x02), .O(new_n69_));
  aoi22  g24(.a(x11), .b(new_n69_), .c(new_n68_), .d(x01), .O(new_n70_));
  oai21  g25(.a(new_n68_), .b(x01), .c(new_n70_), .O(new_n71_));
  inv1   g26(.a(x11), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x02), .O(new_n73_));
  inv1   g28(.a(x03), .O(new_n74_));
  nand2  g29(.a(x12), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x12), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x03), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n75_), .c(new_n73_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n71_), .c(new_n47_), .O(new_n79_));
  inv1   g34(.a(x09), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(x00), .O(new_n81_));
  nor2   g36(.a(x15), .b(x09), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x00), .c(new_n81_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n79_), .c(x08), .O(z04));
  oai21  g39(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g40(.a(x08), .O(new_n86_));
  nand3  g41(.a(new_n47_), .b(x14), .c(new_n86_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z06));
  nand3  g43(.a(new_n47_), .b(new_n86_), .c(x06), .O(z07));
  inv1   g44(.a(x23), .O(new_n90_));
  nand3  g45(.a(new_n58_), .b(x22), .c(x21), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(x24), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n52_), .c(new_n50_), .O(z08));
  inv1   g49(.a(x07), .O(new_n95_));
  inv1   g50(.a(x15), .O(new_n96_));
  nand3  g51(.a(new_n63_), .b(new_n96_), .c(new_n95_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z09));
  nand4  g53(.a(new_n62_), .b(new_n54_), .c(new_n96_), .d(new_n95_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z10));
  xor2a  g55(.a(x18), .b(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n62_), .c(new_n96_), .d(new_n95_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z11));
  nor2   g58(.a(new_n55_), .b(new_n54_), .O(new_n104_));
  nand3  g59(.a(new_n56_), .b(x18), .c(x17), .O(new_n105_));
  oai21  g60(.a(new_n104_), .b(new_n56_), .c(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n62_), .c(new_n96_), .d(new_n95_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(z12));
  nand3  g63(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n104_), .b(new_n53_), .c(x19), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n64_), .d(new_n96_), .O(z13));
  nand2  g67(.a(new_n111_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n104_), .c(x19), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n64_), .d(new_n96_), .O(z14));
  nand2  g71(.a(new_n115_), .b(x22), .O(new_n117_));
  inv1   g72(.a(new_n109_), .O(new_n118_));
  nor3   g73(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n118_), .c(x15), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n117_), .c(new_n64_), .O(z15));
  aoi21  g76(.a(new_n119_), .b(new_n118_), .c(new_n90_), .O(new_n122_));
  nor2   g77(.a(x23), .b(x22), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n114_), .c(new_n118_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n62_), .c(new_n95_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n122_), .c(new_n96_), .O(new_n126_));
  oai21  g81(.a(new_n96_), .b(x00), .c(new_n126_), .O(z16));
  nand2  g82(.a(new_n124_), .b(x24), .O(new_n128_));
  inv1   g83(.a(new_n111_), .O(new_n129_));
  nor2   g84(.a(x22), .b(x21), .O(new_n130_));
  nor2   g85(.a(x24), .b(x23), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n129_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n128_), .c(new_n64_), .d(new_n96_), .O(z17));
  nand2  g88(.a(new_n132_), .b(x25), .O(new_n134_));
  nor3   g89(.a(x25), .b(x24), .c(x23), .O(new_n135_));
  nand3  g90(.a(new_n135_), .b(new_n130_), .c(new_n129_), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n134_), .c(new_n64_), .d(new_n96_), .O(z18));
endmodule


