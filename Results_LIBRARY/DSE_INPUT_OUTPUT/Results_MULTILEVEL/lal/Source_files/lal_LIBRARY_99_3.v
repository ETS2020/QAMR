// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(x07), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g21(.a(new_n47_), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n46_), .O(z02));
  nand2  g23(.a(new_n62_), .b(new_n47_), .O(z03));
  inv1   g24(.a(x08), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n47_), .c(new_n70_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g35(.a(new_n70_), .b(x06), .c(new_n67_), .O(z07));
  inv1   g36(.a(x23), .O(new_n82_));
  nand3  g37(.a(new_n57_), .b(x22), .c(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x24), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n51_), .c(new_n67_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  inv1   g42(.a(x15), .O(new_n88_));
  nand3  g43(.a(new_n64_), .b(new_n88_), .c(new_n87_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z09));
  nand4  g45(.a(new_n63_), .b(new_n53_), .c(new_n88_), .d(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n47_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n63_), .c(new_n88_), .d(new_n87_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z11));
  nor2   g50(.a(new_n54_), .b(new_n53_), .O(new_n96_));
  nand3  g51(.a(new_n55_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n55_), .c(new_n97_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n63_), .c(new_n88_), .d(new_n87_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n47_), .O(z12));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x20), .O(new_n102_));
  nand4  g57(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n102_), .c(new_n63_), .d(new_n88_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n87_), .O(new_n105_));
  nand2  g60(.a(new_n88_), .b(x07), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n105_), .O(z13));
  inv1   g62(.a(new_n103_), .O(new_n108_));
  nor2   g63(.a(new_n108_), .b(new_n58_), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n96_), .c(x19), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n63_), .c(new_n88_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n109_), .c(new_n87_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n106_), .O(z14));
  inv1   g69(.a(new_n101_), .O(new_n115_));
  nor3   g70(.a(x22), .b(x21), .c(x20), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n63_), .c(new_n88_), .O(new_n118_));
  aoi21  g73(.a(new_n111_), .b(x22), .c(new_n118_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x07), .c(new_n106_), .O(z15));
  aoi21  g75(.a(new_n116_), .b(new_n115_), .c(new_n82_), .O(new_n121_));
  nand4  g76(.a(new_n110_), .b(new_n115_), .c(new_n82_), .d(new_n59_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n63_), .c(new_n88_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n121_), .c(new_n87_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n106_), .O(z16));
  nand2  g80(.a(new_n122_), .b(x24), .O(new_n126_));
  nor2   g81(.a(x22), .b(x21), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n50_), .c(new_n82_), .O(new_n128_));
  inv1   g83(.a(new_n128_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n108_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n126_), .c(new_n65_), .d(new_n88_), .O(z17));
  oai21  g86(.a(new_n128_), .b(new_n103_), .c(x25), .O(new_n132_));
  nor3   g87(.a(x25), .b(x24), .c(x23), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n127_), .c(new_n108_), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n132_), .c(new_n63_), .d(new_n88_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n87_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n106_), .O(z18));
endmodule


