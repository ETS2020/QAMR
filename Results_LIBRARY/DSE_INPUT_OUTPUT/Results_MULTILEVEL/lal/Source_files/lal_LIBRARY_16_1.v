// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x02), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n61_), .c(new_n49_), .d(new_n51_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nand3  g20(.a(new_n60_), .b(new_n49_), .c(new_n52_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  aoi21  g22(.a(x11), .b(new_n46_), .c(x15), .O(new_n68_));
  inv1   g23(.a(x09), .O(new_n69_));
  inv1   g24(.a(x01), .O(new_n70_));
  aoi22  g25(.a(x10), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x10), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x01), .O(new_n74_));
  inv1   g29(.a(x11), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n72_), .c(new_n46_), .O(new_n79_));
  oai21  g34(.a(new_n68_), .b(x02), .c(new_n79_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  nor3   g37(.a(new_n48_), .b(new_n82_), .c(x08), .O(z06));
  nand3  g38(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g39(.a(x23), .O(new_n85_));
  nand2  g40(.a(new_n57_), .b(new_n53_), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(x22), .c(x21), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(x24), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n52_), .c(new_n48_), .O(z08));
  inv1   g45(.a(new_n62_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n47_), .c(new_n51_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand4  g48(.a(new_n62_), .b(new_n54_), .c(new_n47_), .d(new_n51_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n62_), .c(new_n47_), .d(new_n51_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z11));
  nor2   g53(.a(new_n55_), .b(new_n54_), .O(new_n99_));
  nand3  g54(.a(new_n56_), .b(x18), .c(x17), .O(new_n100_));
  oai21  g55(.a(new_n99_), .b(new_n56_), .c(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n62_), .c(new_n47_), .d(new_n51_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n49_), .O(z12));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nor2   g60(.a(new_n91_), .b(x07), .O(new_n106_));
  nand4  g61(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n105_), .d(new_n47_), .O(z13));
  nand2  g63(.a(new_n107_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n99_), .c(x19), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n106_), .d(new_n47_), .O(z14));
  inv1   g67(.a(new_n104_), .O(new_n113_));
  nor3   g68(.a(x22), .b(x21), .c(x20), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n62_), .c(new_n51_), .O(new_n116_));
  aoi21  g71(.a(new_n111_), .b(x22), .c(new_n116_), .O(new_n117_));
  nand2  g72(.a(x15), .b(x02), .O(new_n118_));
  oai21  g73(.a(new_n117_), .b(x15), .c(new_n118_), .O(z15));
  aoi21  g74(.a(new_n114_), .b(new_n113_), .c(new_n85_), .O(new_n120_));
  inv1   g75(.a(x22), .O(new_n121_));
  nand4  g76(.a(new_n110_), .b(new_n113_), .c(new_n85_), .d(new_n121_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n62_), .c(new_n51_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n120_), .c(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n118_), .O(z16));
  nand3  g80(.a(new_n110_), .b(new_n85_), .c(new_n121_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n104_), .c(x24), .O(new_n127_));
  inv1   g82(.a(new_n107_), .O(new_n128_));
  nor2   g83(.a(x22), .b(x21), .O(new_n129_));
  nor2   g84(.a(x24), .b(x23), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n127_), .c(new_n62_), .d(new_n51_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n47_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n118_), .O(z17));
  oai21  g91(.a(new_n131_), .b(new_n107_), .c(x25), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand3  g93(.a(new_n138_), .b(new_n129_), .c(new_n128_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n137_), .c(new_n62_), .d(new_n51_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n47_), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(new_n118_), .O(z18));
endmodule


