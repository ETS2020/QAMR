// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x08), .O(new_n46_));
  nor2   g001(.a(x15), .b(x02), .O(new_n47_));
  nor3   g002(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g003(.a(x07), .O(new_n49_));
  inv1   g004(.a(new_n47_), .O(new_n50_));
  inv1   g005(.a(x25), .O(new_n51_));
  inv1   g006(.a(x20), .O(new_n52_));
  inv1   g007(.a(x17), .O(new_n53_));
  inv1   g008(.a(x18), .O(new_n54_));
  inv1   g009(.a(x19), .O(new_n55_));
  nand3  g010(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g011(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g014(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  nand2  g015(.a(x05), .b(x04), .O(new_n61_));
  nand4  g016(.a(new_n61_), .b(new_n60_), .c(new_n50_), .d(new_n49_), .O(z01));
  inv1   g017(.a(x16), .O(new_n63_));
  nor2   g018(.a(new_n47_), .b(new_n63_), .O(z02));
  nand3  g019(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(new_n65_));
  inv1   g020(.a(new_n65_), .O(z03));
  inv1   g021(.a(x09), .O(new_n67_));
  inv1   g022(.a(x01), .O(new_n68_));
  aoi22  g023(.a(x10), .b(new_n68_), .c(new_n67_), .d(x00), .O(new_n69_));
  oai21  g024(.a(new_n67_), .b(x00), .c(new_n69_), .O(new_n70_));
  inv1   g025(.a(x10), .O(new_n71_));
  nand2  g026(.a(new_n71_), .b(x01), .O(new_n72_));
  inv1   g027(.a(x03), .O(new_n73_));
  nand2  g028(.a(x12), .b(new_n73_), .O(new_n74_));
  inv1   g029(.a(x12), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(x03), .O(new_n76_));
  nand3  g031(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  oai21  g032(.a(new_n77_), .b(new_n70_), .c(new_n50_), .O(new_n78_));
  inv1   g033(.a(x11), .O(new_n79_));
  inv1   g034(.a(x15), .O(new_n80_));
  nor3   g035(.a(new_n80_), .b(new_n79_), .c(x02), .O(new_n81_));
  aoi21  g036(.a(new_n79_), .b(x02), .c(new_n81_), .O(new_n82_));
  aoi21  g037(.a(new_n82_), .b(new_n78_), .c(x08), .O(z04));
  oai21  g038(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g039(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(new_n50_), .O(z06));
  nand3  g041(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g042(.a(x23), .O(new_n88_));
  nand2  g043(.a(new_n56_), .b(new_n52_), .O(new_n89_));
  nand3  g044(.a(new_n89_), .b(x22), .c(x21), .O(new_n90_));
  nand2  g045(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(x24), .O(new_n92_));
  aoi21  g047(.a(new_n92_), .b(new_n51_), .c(new_n47_), .O(z08));
  nand3  g048(.a(x05), .b(x04), .c(x02), .O(new_n94_));
  nor3   g049(.a(new_n94_), .b(x15), .c(x07), .O(z09));
  inv1   g050(.a(x02), .O(new_n96_));
  nand4  g051(.a(new_n61_), .b(new_n53_), .c(new_n80_), .d(new_n49_), .O(new_n97_));
  nor2   g052(.a(new_n97_), .b(new_n96_), .O(z10));
  xor2a  g053(.a(x18), .b(x17), .O(new_n99_));
  nand3  g054(.a(new_n99_), .b(new_n61_), .c(new_n49_), .O(new_n100_));
  aoi21  g055(.a(new_n100_), .b(x02), .c(x15), .O(z11));
  nand2  g056(.a(x18), .b(x17), .O(new_n102_));
  inv1   g057(.a(new_n102_), .O(new_n103_));
  nand3  g058(.a(new_n55_), .b(x18), .c(x17), .O(new_n104_));
  oai21  g059(.a(new_n103_), .b(new_n55_), .c(new_n104_), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n61_), .c(new_n80_), .d(new_n49_), .O(new_n106_));
  nor2   g061(.a(new_n106_), .b(new_n96_), .O(z12));
  nand3  g062(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(x20), .O(new_n109_));
  nand3  g064(.a(new_n103_), .b(new_n52_), .c(x19), .O(new_n110_));
  nand4  g065(.a(new_n110_), .b(new_n109_), .c(new_n61_), .d(new_n49_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(x02), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(new_n80_), .O(z13));
  nand2  g068(.a(new_n110_), .b(x21), .O(new_n114_));
  aoi21  g069(.a(x05), .b(x04), .c(new_n96_), .O(new_n115_));
  nor2   g070(.a(x21), .b(x20), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n103_), .c(x19), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(new_n80_), .c(new_n49_), .O(new_n118_));
  inv1   g073(.a(new_n118_), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n115_), .c(new_n114_), .O(z14));
  nand2  g075(.a(new_n117_), .b(x22), .O(new_n121_));
  inv1   g076(.a(new_n108_), .O(new_n122_));
  inv1   g077(.a(x21), .O(new_n123_));
  inv1   g078(.a(x22), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nor2   g080(.a(new_n125_), .b(x20), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n121_), .c(new_n61_), .d(new_n49_), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(x02), .O(new_n129_));
  nand2  g084(.a(new_n129_), .b(new_n80_), .O(z15));
  aoi21  g085(.a(new_n126_), .b(new_n122_), .c(new_n88_), .O(new_n131_));
  nand4  g086(.a(new_n116_), .b(new_n122_), .c(new_n88_), .d(new_n124_), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n61_), .c(new_n49_), .O(new_n133_));
  oai21  g088(.a(new_n133_), .b(new_n131_), .c(x02), .O(new_n134_));
  nand2  g089(.a(new_n134_), .b(new_n80_), .O(z16));
  nand2  g090(.a(new_n132_), .b(x24), .O(new_n136_));
  inv1   g091(.a(new_n110_), .O(new_n137_));
  inv1   g092(.a(x24), .O(new_n138_));
  nor2   g093(.a(x22), .b(x21), .O(new_n139_));
  nand3  g094(.a(new_n139_), .b(new_n138_), .c(new_n88_), .O(new_n140_));
  inv1   g095(.a(new_n140_), .O(new_n141_));
  aoi21  g096(.a(new_n141_), .b(new_n137_), .c(x15), .O(new_n142_));
  nand4  g097(.a(new_n142_), .b(new_n136_), .c(new_n115_), .d(new_n49_), .O(z17));
  oai21  g098(.a(new_n140_), .b(new_n110_), .c(x25), .O(new_n144_));
  nand3  g099(.a(new_n51_), .b(new_n138_), .c(new_n88_), .O(new_n145_));
  nor2   g100(.a(new_n145_), .b(new_n125_), .O(new_n146_));
  aoi21  g101(.a(new_n146_), .b(new_n137_), .c(x15), .O(new_n147_));
  nand4  g102(.a(new_n147_), .b(new_n144_), .c(new_n115_), .d(new_n49_), .O(z18));
endmodule


