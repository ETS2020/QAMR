// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x24), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x15), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(new_n52_), .c(new_n53_), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(x22), .c(x21), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n54_), .c(new_n56_), .O(new_n64_));
  aoi21  g19(.a(x05), .b(x04), .c(x07), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n64_), .c(new_n55_), .O(z01));
  nand3  g21(.a(new_n63_), .b(x24), .c(new_n56_), .O(new_n67_));
  nand2  g22(.a(new_n53_), .b(x15), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n67_), .c(x25), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g35(.a(x22), .O(new_n81_));
  aoi21  g36(.a(new_n61_), .b(new_n57_), .c(new_n81_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x21), .c(x23), .O(new_n83_));
  nor2   g38(.a(new_n48_), .b(x25), .O(new_n84_));
  oai21  g39(.a(new_n83_), .b(new_n53_), .c(new_n84_), .O(z08));
  nand2  g40(.a(x05), .b(x04), .O(new_n86_));
  nor4   g41(.a(new_n86_), .b(new_n53_), .c(x15), .d(x07), .O(z09));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n86_), .b(new_n58_), .c(new_n88_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x24), .c(x15), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n86_), .c(new_n88_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(x24), .c(x15), .O(z11));
  nor2   g48(.a(new_n59_), .b(new_n58_), .O(new_n94_));
  nand3  g49(.a(new_n60_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n60_), .c(new_n95_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n86_), .c(x24), .d(new_n52_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(x07), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nor2   g55(.a(x20), .b(new_n60_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n94_), .c(new_n53_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n100_), .c(new_n65_), .d(new_n52_), .O(z13));
  inv1   g58(.a(x21), .O(new_n104_));
  aoi21  g59(.a(new_n101_), .b(new_n94_), .c(new_n104_), .O(new_n105_));
  nor2   g60(.a(x21), .b(x20), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(x19), .c(x18), .d(x17), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n86_), .c(new_n88_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n105_), .c(x24), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n52_), .O(z14));
  nand2  g65(.a(new_n107_), .b(x22), .O(new_n111_));
  inv1   g66(.a(new_n99_), .O(new_n112_));
  nor3   g67(.a(x22), .b(x21), .c(x20), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n112_), .c(new_n53_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n111_), .c(new_n65_), .d(new_n52_), .O(z15));
  aoi21  g70(.a(new_n113_), .b(new_n112_), .c(new_n56_), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n106_), .c(new_n112_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n86_), .c(new_n88_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n116_), .c(x24), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n52_), .O(z16));
  nand2  g76(.a(new_n94_), .b(new_n65_), .O(new_n122_));
  nand3  g77(.a(new_n117_), .b(new_n101_), .c(new_n104_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n122_), .c(x24), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n52_), .O(z17));
  nand4  g80(.a(new_n65_), .b(new_n54_), .c(x24), .d(new_n52_), .O(z18));
endmodule


