// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x15), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(x08), .O(new_n61_));
  nor2   g16(.a(new_n46_), .b(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z01));
  inv1   g19(.a(new_n62_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n47_), .O(z02));
  nand3  g21(.a(new_n65_), .b(new_n58_), .c(new_n50_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z03));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n61_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n65_), .O(z04));
  nor2   g30(.a(x13), .b(x08), .O(z05));
  nand2  g31(.a(x14), .b(new_n61_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n65_), .O(z06));
  nand2  g33(.a(new_n46_), .b(x08), .O(new_n79_));
  oai21  g34(.a(x08), .b(x06), .c(new_n79_), .O(z07));
  inv1   g35(.a(x24), .O(new_n81_));
  inv1   g36(.a(x22), .O(new_n82_));
  aoi21  g37(.a(new_n55_), .b(new_n51_), .c(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x21), .c(x23), .O(new_n84_));
  nor2   g39(.a(new_n62_), .b(x25), .O(new_n85_));
  oai21  g40(.a(new_n84_), .b(new_n81_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  inv1   g42(.a(new_n60_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n46_), .c(new_n87_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z09));
  nand4  g45(.a(new_n60_), .b(new_n52_), .c(new_n46_), .d(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n65_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n60_), .c(new_n46_), .d(new_n87_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z11));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n54_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n54_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n60_), .c(new_n46_), .d(new_n87_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n65_), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nor2   g58(.a(new_n88_), .b(x07), .O(new_n104_));
  nand4  g59(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n104_), .c(new_n103_), .d(new_n46_), .O(z13));
  inv1   g61(.a(x21), .O(new_n107_));
  inv1   g62(.a(new_n105_), .O(new_n108_));
  nor2   g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n97_), .c(x19), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n60_), .c(new_n87_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n109_), .c(new_n46_), .O(new_n113_));
  nand2  g68(.a(x15), .b(new_n61_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(z14));
  nand3  g70(.a(new_n107_), .b(new_n51_), .c(x19), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n96_), .c(x22), .O(new_n117_));
  inv1   g72(.a(new_n102_), .O(new_n118_));
  nor3   g73(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n117_), .c(new_n60_), .d(new_n87_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n46_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n114_), .O(z15));
  inv1   g78(.a(x23), .O(new_n124_));
  aoi21  g79(.a(new_n119_), .b(new_n118_), .c(new_n124_), .O(new_n125_));
  nand4  g80(.a(new_n110_), .b(new_n118_), .c(new_n124_), .d(new_n82_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n60_), .c(new_n87_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n125_), .c(new_n46_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n114_), .O(z16));
  nand2  g84(.a(new_n126_), .b(x24), .O(new_n130_));
  nand4  g85(.a(new_n81_), .b(new_n124_), .c(new_n82_), .d(new_n107_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n108_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n130_), .c(new_n104_), .d(new_n46_), .O(z17));
  oai21  g89(.a(new_n131_), .b(new_n105_), .c(x25), .O(new_n135_));
  nor2   g90(.a(x20), .b(new_n54_), .O(new_n136_));
  nor2   g91(.a(x22), .b(x21), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n97_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n135_), .c(new_n60_), .d(new_n87_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n46_), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(new_n114_), .O(z18));
endmodule


