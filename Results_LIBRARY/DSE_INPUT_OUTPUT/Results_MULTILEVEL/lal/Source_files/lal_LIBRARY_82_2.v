// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x17), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x21), .O(new_n51_));
  inv1   g06(.a(x22), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  oai21  g08(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(x24), .c(x25), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n55_), .c(new_n50_), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x25), .O(new_n61_));
  inv1   g16(.a(x17), .O(new_n62_));
  nor2   g17(.a(x19), .b(x18), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x15), .c(new_n62_), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n61_), .c(new_n53_), .d(new_n60_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n59_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  inv1   g23(.a(x24), .O(new_n69_));
  nor2   g24(.a(new_n52_), .b(new_n51_), .O(new_n70_));
  nor3   g25(.a(x19), .b(x18), .c(x17), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(x20), .c(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n53_), .c(new_n69_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x25), .c(new_n50_), .O(z03));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g32(.a(x12), .b(x03), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand3  g40(.a(x22), .b(x21), .c(x20), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n53_), .c(new_n69_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x25), .O(new_n88_));
  inv1   g43(.a(x19), .O(new_n89_));
  nor3   g44(.a(x18), .b(x17), .c(x15), .O(new_n90_));
  nor2   g45(.a(new_n69_), .b(new_n52_), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n90_), .c(x21), .d(new_n89_), .O(new_n92_));
  oai21  g47(.a(new_n88_), .b(new_n48_), .c(new_n92_), .O(z08));
  nand2  g48(.a(new_n47_), .b(new_n56_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n57_), .c(new_n50_), .O(z09));
  nand4  g50(.a(new_n57_), .b(new_n62_), .c(new_n47_), .d(new_n56_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z10));
  xor2a  g52(.a(x18), .b(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n57_), .c(new_n47_), .d(new_n56_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z11));
  inv1   g55(.a(x18), .O(new_n101_));
  nand2  g56(.a(x19), .b(new_n101_), .O(new_n102_));
  nand3  g57(.a(new_n89_), .b(x18), .c(x17), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n102_), .c(x15), .O(new_n104_));
  nor2   g59(.a(new_n89_), .b(x17), .O(new_n105_));
  oai21  g60(.a(new_n105_), .b(new_n104_), .c(new_n57_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(x07), .c(new_n50_), .O(z12));
  inv1   g62(.a(new_n58_), .O(new_n108_));
  nand3  g63(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x20), .O(new_n110_));
  nand4  g65(.a(new_n60_), .b(x19), .c(x18), .d(x17), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n108_), .d(new_n47_), .O(z13));
  nand2  g67(.a(new_n111_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(x19), .c(x18), .d(x17), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n108_), .d(new_n47_), .O(z14));
  nand2  g71(.a(x19), .b(x18), .O(new_n117_));
  nand3  g72(.a(new_n52_), .b(new_n51_), .c(new_n60_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n117_), .c(new_n47_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x17), .O(new_n120_));
  aoi21  g75(.a(new_n115_), .b(x22), .c(new_n58_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(x15), .c(new_n120_), .O(z15));
  oai21  g77(.a(new_n118_), .b(new_n109_), .c(x23), .O(new_n123_));
  inv1   g78(.a(new_n109_), .O(new_n124_));
  nand4  g79(.a(new_n114_), .b(new_n124_), .c(new_n53_), .d(new_n52_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n123_), .c(new_n108_), .d(new_n47_), .O(z16));
  nand3  g81(.a(new_n60_), .b(x19), .c(x18), .O(new_n127_));
  nand4  g82(.a(new_n69_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n127_), .c(new_n47_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x17), .O(new_n130_));
  aoi21  g85(.a(new_n125_), .b(x24), .c(new_n58_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(x15), .c(new_n130_), .O(z17));
  nand3  g87(.a(new_n114_), .b(x19), .c(x18), .O(new_n133_));
  nand4  g88(.a(new_n61_), .b(new_n69_), .c(new_n53_), .d(new_n52_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n133_), .c(new_n47_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(x17), .O(new_n136_));
  oai21  g91(.a(new_n128_), .b(new_n111_), .c(x25), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n108_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n136_), .O(z18));
endmodule


