// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x05), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  oai21  g08(.a(x15), .b(x04), .c(x05), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g16(.a(new_n60_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x23), .c(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n54_), .c(new_n53_), .O(z01));
  nor2   g20(.a(new_n49_), .b(new_n46_), .O(z02));
  nand3  g21(.a(new_n63_), .b(new_n50_), .c(new_n55_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z03));
  inv1   g23(.a(x08), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand3  g32(.a(new_n50_), .b(new_n77_), .c(new_n69_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand3  g34(.a(new_n50_), .b(x14), .c(new_n69_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z06));
  nand3  g36(.a(new_n50_), .b(new_n69_), .c(x06), .O(z07));
  inv1   g37(.a(x24), .O(new_n83_));
  inv1   g38(.a(x22), .O(new_n84_));
  aoi21  g39(.a(new_n60_), .b(new_n56_), .c(new_n84_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x21), .c(x23), .O(new_n86_));
  nor2   g41(.a(new_n49_), .b(x25), .O(new_n87_));
  oai21  g42(.a(new_n86_), .b(new_n83_), .c(new_n87_), .O(z08));
  nand2  g43(.a(new_n53_), .b(x04), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n48_), .c(new_n47_), .O(z09));
  nand2  g45(.a(x05), .b(x04), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n57_), .c(new_n48_), .d(new_n53_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n91_), .c(new_n48_), .d(new_n53_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n50_), .O(z11));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n59_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n59_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n91_), .c(new_n48_), .d(new_n53_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n50_), .O(z12));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nor2   g59(.a(x20), .b(new_n59_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n91_), .d(new_n53_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n48_), .O(new_n108_));
  nand2  g63(.a(x15), .b(new_n47_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(z13));
  nand2  g65(.a(new_n106_), .b(x21), .O(new_n111_));
  aoi21  g66(.a(x05), .b(x04), .c(x07), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n98_), .c(x19), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n112_), .c(new_n111_), .d(new_n48_), .O(z14));
  inv1   g70(.a(x21), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n56_), .c(x19), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n97_), .c(x22), .O(new_n118_));
  inv1   g73(.a(new_n103_), .O(new_n119_));
  nor3   g74(.a(x22), .b(x21), .c(x20), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n118_), .c(new_n91_), .d(new_n53_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n109_), .O(z15));
  inv1   g79(.a(x23), .O(new_n125_));
  aoi21  g80(.a(new_n120_), .b(new_n119_), .c(new_n125_), .O(new_n126_));
  nand4  g81(.a(new_n113_), .b(new_n119_), .c(new_n125_), .d(new_n84_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n91_), .c(new_n53_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n126_), .c(new_n48_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n109_), .O(z16));
  nand4  g85(.a(new_n125_), .b(new_n84_), .c(new_n116_), .d(new_n56_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n103_), .c(x24), .O(new_n132_));
  nor2   g87(.a(x22), .b(x21), .O(new_n133_));
  nor2   g88(.a(x24), .b(x23), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n133_), .c(new_n105_), .d(new_n98_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n132_), .c(new_n91_), .d(new_n53_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n48_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n109_), .O(z17));
  nand2  g93(.a(new_n135_), .b(x25), .O(new_n139_));
  nor3   g94(.a(x25), .b(x24), .c(x23), .O(new_n140_));
  nand4  g95(.a(new_n140_), .b(new_n133_), .c(new_n105_), .d(new_n98_), .O(new_n141_));
  nand4  g96(.a(new_n141_), .b(new_n139_), .c(new_n112_), .d(new_n48_), .O(z18));
endmodule


