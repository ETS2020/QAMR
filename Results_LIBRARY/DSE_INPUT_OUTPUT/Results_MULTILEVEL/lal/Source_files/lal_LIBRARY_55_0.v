// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(x22), .c(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n51_), .c(new_n47_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  oai21  g17(.a(x25), .b(x24), .c(new_n62_), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  oai21  g19(.a(new_n59_), .b(x23), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n49_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x24), .O(new_n68_));
  nand2  g23(.a(new_n49_), .b(new_n68_), .O(new_n69_));
  nand3  g24(.a(new_n58_), .b(new_n48_), .c(x15), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n49_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n49_), .O(z06));
  aoi22  g36(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x06), .O(z07));
  nand4  g37(.a(new_n57_), .b(x24), .c(x22), .d(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n51_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x15), .O(new_n85_));
  oai21  g40(.a(x25), .b(x24), .c(x23), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(z08));
  nor4   g42(.a(new_n60_), .b(new_n48_), .c(x15), .d(x07), .O(z09));
  nand3  g43(.a(new_n60_), .b(new_n53_), .c(new_n62_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x23), .c(x15), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n60_), .c(x23), .d(new_n47_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(x07), .O(z11));
  nor2   g48(.a(new_n54_), .b(new_n53_), .O(new_n94_));
  nand3  g49(.a(new_n55_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n55_), .c(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n60_), .c(new_n62_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(x23), .c(x15), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nor2   g55(.a(new_n61_), .b(x07), .O(new_n101_));
  nand3  g56(.a(new_n94_), .b(new_n52_), .c(x19), .O(new_n102_));
  and2   g57(.a(new_n102_), .b(x23), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .d(new_n47_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n52_), .c(x19), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n94_), .c(new_n48_), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n105_), .c(new_n101_), .d(new_n47_), .O(z14));
  inv1   g65(.a(x22), .O(new_n111_));
  aoi21  g66(.a(new_n108_), .b(new_n94_), .c(new_n111_), .O(new_n112_));
  inv1   g67(.a(new_n99_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n106_), .d(new_n52_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n60_), .c(new_n62_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n112_), .c(x23), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n47_), .O(z15));
  nand2  g72(.a(new_n101_), .b(new_n94_), .O(new_n118_));
  nand4  g73(.a(new_n111_), .b(new_n106_), .c(new_n52_), .d(x19), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n118_), .c(x23), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n47_), .O(z16));
  nand4  g76(.a(new_n101_), .b(new_n68_), .c(x23), .d(new_n47_), .O(z17));
  nand3  g77(.a(new_n60_), .b(new_n51_), .c(new_n62_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(x23), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n47_), .O(z18));
endmodule


