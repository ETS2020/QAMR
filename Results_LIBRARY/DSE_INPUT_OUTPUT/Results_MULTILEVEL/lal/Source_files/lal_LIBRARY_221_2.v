// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x18), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x19), .b(x17), .c(new_n53_), .O(new_n54_));
  and2   g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n51_), .c(new_n52_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(new_n48_), .b(new_n58_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(z01));
  and2   g16(.a(new_n48_), .b(x16), .O(z02));
  inv1   g17(.a(x23), .O(new_n63_));
  inv1   g18(.a(x18), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(x17), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n47_), .c(x20), .O(new_n66_));
  aoi21  g21(.a(new_n47_), .b(x18), .c(new_n55_), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(new_n66_), .c(new_n63_), .O(new_n68_));
  nand2  g23(.a(new_n48_), .b(new_n51_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n48_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n48_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand3  g36(.a(x22), .b(x21), .c(x20), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n63_), .c(new_n51_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x25), .c(new_n48_), .O(new_n84_));
  nor3   g39(.a(x19), .b(x18), .c(x17), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(x24), .c(x22), .d(x21), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n84_), .O(z08));
  nand2  g42(.a(x05), .b(x04), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n58_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n88_), .c(new_n48_), .O(z09));
  inv1   g46(.a(x17), .O(new_n92_));
  nand4  g47(.a(new_n88_), .b(new_n92_), .c(new_n89_), .d(new_n58_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n48_), .O(z10));
  nor2   g49(.a(new_n47_), .b(new_n64_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n65_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n88_), .c(new_n89_), .d(new_n58_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  nand2  g54(.a(x18), .b(x17), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n88_), .c(x19), .d(new_n89_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(x07), .O(z12));
  nand3  g57(.a(new_n88_), .b(new_n89_), .c(new_n58_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand3  g59(.a(new_n53_), .b(x18), .c(x17), .O(new_n105_));
  oai21  g60(.a(new_n53_), .b(x17), .c(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x19), .O(new_n107_));
  nand2  g62(.a(x20), .b(new_n64_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n104_), .O(z13));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x17), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x19), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x18), .O(new_n113_));
  nand4  g68(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(x21), .c(new_n103_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n113_), .O(z14));
  inv1   g71(.a(new_n103_), .O(new_n117_));
  nor2   g72(.a(x20), .b(new_n92_), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x19), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x18), .O(new_n122_));
  nand2  g77(.a(new_n110_), .b(x19), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n100_), .c(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n117_), .O(z15));
  nor3   g80(.a(x23), .b(x22), .c(x21), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n118_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x19), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(x18), .O(new_n129_));
  nand4  g84(.a(new_n119_), .b(new_n95_), .c(new_n53_), .d(x17), .O(new_n130_));
  aoi21  g85(.a(new_n130_), .b(x23), .c(new_n103_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n129_), .O(z16));
  aoi21  g87(.a(new_n126_), .b(new_n118_), .c(new_n51_), .O(new_n133_));
  nand3  g88(.a(new_n119_), .b(new_n51_), .c(new_n63_), .O(new_n134_));
  nor2   g89(.a(new_n134_), .b(new_n105_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(new_n133_), .c(x19), .O(new_n136_));
  nand2  g91(.a(x24), .b(new_n64_), .O(new_n137_));
  nand3  g92(.a(new_n137_), .b(new_n136_), .c(new_n104_), .O(z17));
  inv1   g93(.a(x22), .O(new_n139_));
  nand4  g94(.a(new_n52_), .b(new_n51_), .c(new_n63_), .d(new_n139_), .O(new_n140_));
  oai21  g95(.a(new_n140_), .b(new_n111_), .c(x19), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(x18), .O(new_n142_));
  oai21  g97(.a(new_n134_), .b(new_n114_), .c(x25), .O(new_n143_));
  nand3  g98(.a(new_n143_), .b(new_n142_), .c(new_n117_), .O(z18));
endmodule


