// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x16), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  inv1   g002(.a(x19), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g006(.a(new_n51_), .O(z00));
  inv1   g007(.a(x21), .O(new_n53_));
  inv1   g008(.a(x22), .O(new_n54_));
  inv1   g009(.a(x23), .O(new_n55_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  aoi21  g011(.a(new_n56_), .b(x24), .c(x25), .O(new_n57_));
  inv1   g012(.a(x07), .O(new_n58_));
  nand2  g013(.a(x05), .b(x04), .O(new_n59_));
  nand2  g014(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(new_n57_), .c(new_n50_), .O(new_n61_));
  inv1   g016(.a(x20), .O(new_n62_));
  inv1   g017(.a(x25), .O(new_n63_));
  nor2   g018(.a(x18), .b(x17), .O(new_n64_));
  nand2  g019(.a(x19), .b(new_n47_), .O(new_n65_));
  oai21  g020(.a(new_n64_), .b(x19), .c(new_n65_), .O(new_n66_));
  nand4  g021(.a(new_n66_), .b(new_n63_), .c(new_n55_), .d(new_n62_), .O(new_n67_));
  nand2  g022(.a(new_n67_), .b(new_n61_), .O(z01));
  nor2   g023(.a(new_n49_), .b(new_n46_), .O(z02));
  nor3   g024(.a(x25), .b(x23), .c(x20), .O(new_n70_));
  oai21  g025(.a(new_n70_), .b(x15), .c(x19), .O(new_n71_));
  inv1   g026(.a(x24), .O(new_n72_));
  nor2   g027(.a(new_n54_), .b(new_n53_), .O(new_n73_));
  oai21  g028(.a(new_n64_), .b(x20), .c(new_n73_), .O(new_n74_));
  aoi21  g029(.a(new_n74_), .b(new_n55_), .c(new_n72_), .O(new_n75_));
  oai21  g030(.a(new_n75_), .b(x25), .c(new_n71_), .O(z03));
  inv1   g031(.a(x08), .O(new_n77_));
  xnor2a g032(.a(x09), .b(x00), .O(new_n78_));
  xnor2a g033(.a(x10), .b(x01), .O(new_n79_));
  xnor2a g034(.a(x11), .b(x02), .O(new_n80_));
  xnor2a g035(.a(x12), .b(x03), .O(new_n81_));
  nand4  g036(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n78_), .O(new_n82_));
  nand2  g037(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  nand2  g038(.a(new_n83_), .b(new_n50_), .O(z04));
  oai21  g039(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g040(.a(new_n50_), .b(x14), .c(new_n77_), .O(new_n86_));
  inv1   g041(.a(new_n86_), .O(z06));
  aoi21  g042(.a(new_n77_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g043(.a(x17), .O(new_n89_));
  inv1   g044(.a(x18), .O(new_n90_));
  nand3  g045(.a(new_n48_), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  aoi21  g046(.a(new_n91_), .b(new_n62_), .c(new_n54_), .O(new_n92_));
  aoi21  g047(.a(new_n92_), .b(x21), .c(x23), .O(new_n93_));
  nor2   g048(.a(new_n49_), .b(x25), .O(new_n94_));
  oai21  g049(.a(new_n93_), .b(new_n72_), .c(new_n94_), .O(z08));
  nand2  g050(.a(new_n47_), .b(new_n58_), .O(new_n96_));
  oai21  g051(.a(new_n96_), .b(new_n59_), .c(new_n50_), .O(z09));
  nand4  g052(.a(new_n59_), .b(new_n89_), .c(new_n47_), .d(new_n58_), .O(new_n98_));
  inv1   g053(.a(new_n98_), .O(z10));
  xor2a  g054(.a(x18), .b(x17), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n59_), .c(new_n47_), .d(new_n58_), .O(new_n101_));
  inv1   g056(.a(new_n101_), .O(z11));
  nor2   g057(.a(new_n90_), .b(new_n89_), .O(new_n103_));
  nand3  g058(.a(new_n48_), .b(x18), .c(x17), .O(new_n104_));
  oai21  g059(.a(new_n103_), .b(new_n48_), .c(new_n104_), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n59_), .c(new_n47_), .d(new_n58_), .O(new_n106_));
  inv1   g061(.a(new_n106_), .O(z12));
  inv1   g062(.a(new_n60_), .O(new_n108_));
  nand3  g063(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(x20), .O(new_n110_));
  nor2   g065(.a(x20), .b(new_n48_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n103_), .O(new_n112_));
  nand4  g067(.a(new_n112_), .b(new_n110_), .c(new_n108_), .d(new_n47_), .O(z13));
  oai21  g068(.a(x21), .b(x15), .c(new_n48_), .O(new_n114_));
  nand3  g069(.a(new_n62_), .b(x18), .c(x17), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(x21), .O(new_n116_));
  nor2   g071(.a(x21), .b(x20), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(new_n103_), .c(x19), .O(new_n118_));
  nand4  g073(.a(new_n118_), .b(new_n116_), .c(new_n59_), .d(new_n58_), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n47_), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(new_n114_), .O(z14));
  nand2  g076(.a(new_n118_), .b(x22), .O(new_n122_));
  inv1   g077(.a(new_n109_), .O(new_n123_));
  nor3   g078(.a(x22), .b(x21), .c(x20), .O(new_n124_));
  aoi21  g079(.a(new_n124_), .b(new_n123_), .c(x15), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n122_), .c(new_n108_), .O(z15));
  oai21  g081(.a(x23), .b(x15), .c(new_n48_), .O(new_n127_));
  aoi21  g082(.a(new_n124_), .b(new_n103_), .c(new_n55_), .O(new_n128_));
  nand4  g083(.a(new_n117_), .b(new_n123_), .c(new_n55_), .d(new_n54_), .O(new_n129_));
  nand3  g084(.a(new_n129_), .b(new_n59_), .c(new_n58_), .O(new_n130_));
  oai21  g085(.a(new_n130_), .b(new_n128_), .c(new_n47_), .O(new_n131_));
  nand2  g086(.a(new_n131_), .b(new_n127_), .O(z16));
  oai21  g087(.a(x24), .b(x15), .c(new_n48_), .O(new_n133_));
  nand3  g088(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(new_n115_), .c(x24), .O(new_n135_));
  nor2   g090(.a(x22), .b(x21), .O(new_n136_));
  nor2   g091(.a(x24), .b(x23), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(new_n136_), .c(new_n111_), .d(new_n103_), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n135_), .c(new_n59_), .d(new_n58_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n47_), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(new_n133_), .O(z17));
  oai21  g096(.a(x25), .b(x15), .c(new_n48_), .O(new_n142_));
  nand4  g097(.a(new_n72_), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n143_));
  oai21  g098(.a(new_n143_), .b(new_n115_), .c(x25), .O(new_n144_));
  nor3   g099(.a(x25), .b(x24), .c(x23), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(new_n136_), .c(new_n111_), .d(new_n103_), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(new_n144_), .c(new_n59_), .d(new_n58_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n47_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n142_), .O(z18));
endmodule


