// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x25), .b(x15), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n47_), .c(new_n63_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n62_), .O(z01));
  nand2  g22(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g23(.a(new_n62_), .b(new_n47_), .O(z03));
  inv1   g24(.a(x08), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n47_), .c(new_n70_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g33(.a(new_n47_), .b(x14), .c(new_n70_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  nand3  g35(.a(new_n47_), .b(new_n70_), .c(x06), .O(z07));
  inv1   g36(.a(x23), .O(new_n82_));
  nand3  g37(.a(new_n57_), .b(x22), .c(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n51_), .c(x24), .O(new_n85_));
  inv1   g40(.a(x15), .O(new_n86_));
  nand2  g41(.a(x25), .b(new_n86_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n85_), .O(z08));
  inv1   g43(.a(new_n64_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n86_), .c(new_n63_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand4  g46(.a(new_n64_), .b(new_n53_), .c(new_n86_), .d(new_n63_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n47_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n64_), .c(new_n86_), .d(new_n63_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  nor2   g51(.a(new_n54_), .b(new_n53_), .O(new_n97_));
  nand3  g52(.a(new_n55_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n55_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n64_), .c(new_n86_), .d(new_n63_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n47_), .O(z12));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n64_), .b(new_n63_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nand4  g60(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .d(new_n86_), .O(z13));
  nand2  g62(.a(new_n106_), .b(x21), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n97_), .c(x19), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n108_), .c(new_n105_), .d(new_n86_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  inv1   g67(.a(new_n102_), .O(new_n113_));
  nor3   g68(.a(x22), .b(x21), .c(x20), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(x15), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n112_), .c(new_n105_), .O(z15));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n82_), .O(new_n117_));
  nand4  g72(.a(new_n109_), .b(new_n113_), .c(new_n82_), .d(new_n59_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n64_), .c(new_n63_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n117_), .c(new_n86_), .O(new_n120_));
  nand2  g75(.a(new_n51_), .b(x15), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(z16));
  aoi21  g77(.a(x25), .b(x15), .c(x24), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n82_), .c(new_n59_), .d(new_n58_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(x20), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(x19), .c(x18), .d(x17), .O(new_n126_));
  nand3  g81(.a(new_n109_), .b(new_n82_), .c(new_n59_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n102_), .c(x24), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n105_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n86_), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n126_), .c(new_n121_), .O(z17));
  nor2   g86(.a(x22), .b(x21), .O(new_n132_));
  nor2   g87(.a(x24), .b(x23), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n106_), .c(new_n86_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n51_), .O(new_n136_));
  nor2   g91(.a(x20), .b(new_n55_), .O(new_n137_));
  nand4  g92(.a(new_n133_), .b(new_n132_), .c(new_n137_), .d(new_n97_), .O(new_n138_));
  aoi21  g93(.a(new_n138_), .b(x25), .c(new_n104_), .O(new_n139_));
  oai21  g94(.a(new_n139_), .b(x15), .c(new_n136_), .O(z18));
endmodule


