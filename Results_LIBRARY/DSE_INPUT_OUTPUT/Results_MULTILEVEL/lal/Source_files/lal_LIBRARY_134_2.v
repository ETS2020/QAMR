// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  nor2   g02(.a(x19), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  nand2  g06(.a(x22), .b(x21), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(x24), .c(x25), .O(new_n54_));
  inv1   g09(.a(x07), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n54_), .c(new_n49_), .O(new_n58_));
  inv1   g13(.a(x20), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  inv1   g15(.a(x17), .O(new_n61_));
  inv1   g16(.a(x19), .O(new_n62_));
  oai21  g17(.a(x18), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n60_), .c(new_n51_), .d(new_n59_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n58_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x24), .O(new_n68_));
  inv1   g23(.a(new_n52_), .O(new_n69_));
  oai21  g24(.a(x19), .b(x17), .c(new_n59_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(x23), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(new_n68_), .c(new_n60_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z03));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z04));
  nor3   g35(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g36(.a(x14), .b(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n49_), .O(z06));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand4  g39(.a(x24), .b(x22), .c(x21), .d(new_n61_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n47_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n62_), .O(new_n87_));
  oai21  g42(.a(new_n52_), .b(new_n59_), .c(new_n51_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x24), .c(x25), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n87_), .O(z08));
  inv1   g45(.a(x15), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n55_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n56_), .c(new_n49_), .O(z09));
  nand4  g48(.a(new_n56_), .b(new_n49_), .c(new_n61_), .d(new_n91_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(x07), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n56_), .c(new_n91_), .d(new_n55_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z11));
  nor2   g53(.a(new_n47_), .b(new_n61_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n56_), .c(x19), .d(new_n91_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(x07), .c(new_n49_), .O(z12));
  nor2   g57(.a(x20), .b(new_n61_), .O(new_n103_));
  oai21  g58(.a(new_n103_), .b(new_n62_), .c(x18), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand3  g60(.a(new_n56_), .b(new_n91_), .c(new_n55_), .O(new_n106_));
  aoi21  g61(.a(new_n105_), .b(x20), .c(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n104_), .O(z13));
  inv1   g63(.a(new_n106_), .O(new_n109_));
  inv1   g64(.a(x21), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n59_), .c(x17), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x19), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x18), .O(new_n113_));
  nand3  g68(.a(new_n99_), .b(new_n59_), .c(x19), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x21), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n109_), .O(z14));
  nand2  g71(.a(new_n106_), .b(new_n49_), .O(new_n117_));
  nand2  g72(.a(new_n111_), .b(x22), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n99_), .c(new_n59_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x19), .O(new_n122_));
  nand2  g77(.a(x22), .b(new_n47_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n117_), .O(z15));
  aoi21  g79(.a(new_n119_), .b(new_n103_), .c(new_n51_), .O(new_n125_));
  nand3  g80(.a(new_n59_), .b(x18), .c(x17), .O(new_n126_));
  inv1   g81(.a(x22), .O(new_n127_));
  nand3  g82(.a(new_n51_), .b(new_n127_), .c(new_n110_), .O(new_n128_));
  nor2   g83(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n125_), .c(x19), .O(new_n130_));
  nand2  g85(.a(x23), .b(new_n47_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n117_), .O(z16));
  nor3   g87(.a(x23), .b(x22), .c(x21), .O(new_n133_));
  aoi21  g88(.a(new_n133_), .b(new_n103_), .c(new_n68_), .O(new_n134_));
  nand3  g89(.a(new_n119_), .b(new_n68_), .c(new_n51_), .O(new_n135_));
  nor2   g90(.a(new_n135_), .b(new_n126_), .O(new_n136_));
  oai21  g91(.a(new_n136_), .b(new_n134_), .c(x19), .O(new_n137_));
  nand2  g92(.a(x24), .b(new_n47_), .O(new_n138_));
  nand3  g93(.a(new_n138_), .b(new_n137_), .c(new_n117_), .O(z17));
  nand4  g94(.a(new_n60_), .b(new_n68_), .c(new_n51_), .d(new_n127_), .O(new_n140_));
  oai21  g95(.a(new_n140_), .b(new_n111_), .c(x19), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(x18), .O(new_n142_));
  oai21  g97(.a(new_n135_), .b(new_n114_), .c(x25), .O(new_n143_));
  nand3  g98(.a(new_n143_), .b(new_n142_), .c(new_n109_), .O(z18));
endmodule


