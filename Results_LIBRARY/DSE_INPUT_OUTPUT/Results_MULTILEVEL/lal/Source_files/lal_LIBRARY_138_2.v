// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n88_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x15), .O(new_n47_));
  nand2  g002(.a(new_n47_), .b(x02), .O(new_n48_));
  oai21  g003(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g004(.a(new_n48_), .O(new_n50_));
  inv1   g005(.a(x24), .O(new_n51_));
  inv1   g006(.a(x25), .O(new_n52_));
  inv1   g007(.a(x20), .O(new_n53_));
  inv1   g008(.a(x17), .O(new_n54_));
  inv1   g009(.a(x18), .O(new_n55_));
  inv1   g010(.a(x19), .O(new_n56_));
  nand3  g011(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  inv1   g013(.a(x21), .O(new_n59_));
  inv1   g014(.a(x22), .O(new_n60_));
  nor2   g015(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g016(.a(new_n61_), .b(new_n58_), .c(x23), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(new_n51_), .c(new_n52_), .O(new_n63_));
  nand2  g018(.a(x05), .b(x04), .O(new_n64_));
  inv1   g019(.a(new_n64_), .O(new_n65_));
  nor2   g020(.a(new_n65_), .b(x07), .O(new_n66_));
  aoi21  g021(.a(new_n66_), .b(new_n63_), .c(new_n50_), .O(z01));
  and2   g022(.a(new_n48_), .b(x16), .O(z02));
  nand2  g023(.a(new_n63_), .b(new_n48_), .O(z03));
  inv1   g024(.a(x09), .O(new_n70_));
  inv1   g025(.a(x01), .O(new_n71_));
  aoi22  g026(.a(x10), .b(new_n71_), .c(new_n70_), .d(x00), .O(new_n72_));
  oai21  g027(.a(new_n70_), .b(x00), .c(new_n72_), .O(new_n73_));
  inv1   g028(.a(x10), .O(new_n74_));
  nand2  g029(.a(new_n74_), .b(x01), .O(new_n75_));
  inv1   g030(.a(x03), .O(new_n76_));
  nand2  g031(.a(x12), .b(new_n76_), .O(new_n77_));
  inv1   g032(.a(x12), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(x03), .O(new_n79_));
  nand3  g034(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n73_), .c(new_n48_), .O(new_n81_));
  inv1   g036(.a(x11), .O(new_n82_));
  nor2   g037(.a(new_n82_), .b(x02), .O(new_n83_));
  nor2   g038(.a(new_n47_), .b(x11), .O(new_n84_));
  aoi21  g039(.a(new_n84_), .b(x02), .c(new_n83_), .O(new_n85_));
  aoi21  g040(.a(new_n85_), .b(new_n81_), .c(x08), .O(z04));
  oai21  g041(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g042(.a(x14), .b(new_n46_), .O(new_n88_));
  nand2  g043(.a(new_n88_), .b(new_n48_), .O(z06));
  nand3  g044(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g045(.a(x23), .O(new_n91_));
  nand3  g046(.a(new_n58_), .b(x22), .c(x21), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g048(.a(new_n93_), .b(x24), .O(new_n94_));
  aoi21  g049(.a(new_n94_), .b(new_n52_), .c(new_n50_), .O(z08));
  inv1   g050(.a(x02), .O(new_n96_));
  inv1   g051(.a(x07), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(x05), .c(x04), .d(new_n96_), .O(new_n98_));
  nor2   g053(.a(new_n98_), .b(x15), .O(z09));
  nand4  g054(.a(new_n64_), .b(new_n54_), .c(new_n47_), .d(new_n97_), .O(new_n100_));
  nor2   g055(.a(new_n100_), .b(x02), .O(z10));
  xor2a  g056(.a(x18), .b(x17), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n64_), .c(new_n47_), .d(new_n97_), .O(new_n103_));
  nor2   g058(.a(new_n103_), .b(x02), .O(z11));
  nor2   g059(.a(new_n55_), .b(new_n54_), .O(new_n105_));
  nand3  g060(.a(new_n56_), .b(x18), .c(x17), .O(new_n106_));
  oai21  g061(.a(new_n105_), .b(new_n56_), .c(new_n106_), .O(new_n107_));
  nand3  g062(.a(new_n107_), .b(new_n64_), .c(new_n97_), .O(new_n108_));
  aoi21  g063(.a(new_n108_), .b(new_n96_), .c(x15), .O(z12));
  nand3  g064(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(x20), .O(new_n111_));
  nor2   g066(.a(new_n65_), .b(x02), .O(new_n112_));
  nand4  g067(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n113_));
  inv1   g068(.a(new_n113_), .O(new_n114_));
  nor2   g069(.a(new_n114_), .b(x15), .O(new_n115_));
  nand4  g070(.a(new_n115_), .b(new_n112_), .c(new_n111_), .d(new_n97_), .O(z13));
  nor2   g071(.a(new_n114_), .b(new_n59_), .O(new_n117_));
  nor2   g072(.a(x21), .b(x20), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n105_), .c(x19), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n64_), .c(new_n97_), .O(new_n120_));
  oai21  g075(.a(new_n120_), .b(new_n117_), .c(new_n96_), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(new_n47_), .O(z14));
  inv1   g077(.a(new_n110_), .O(new_n123_));
  nor3   g078(.a(x22), .b(x21), .c(x20), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n64_), .c(new_n97_), .O(new_n126_));
  aoi21  g081(.a(new_n119_), .b(x22), .c(new_n126_), .O(new_n127_));
  oai21  g082(.a(new_n127_), .b(x02), .c(new_n47_), .O(z15));
  aoi21  g083(.a(new_n124_), .b(new_n123_), .c(new_n91_), .O(new_n129_));
  nand4  g084(.a(new_n118_), .b(new_n123_), .c(new_n91_), .d(new_n60_), .O(new_n130_));
  nand3  g085(.a(new_n130_), .b(new_n64_), .c(new_n97_), .O(new_n131_));
  oai21  g086(.a(new_n131_), .b(new_n129_), .c(new_n96_), .O(new_n132_));
  nand2  g087(.a(new_n132_), .b(new_n47_), .O(z16));
  nand2  g088(.a(new_n130_), .b(x24), .O(new_n134_));
  nor2   g089(.a(x22), .b(x21), .O(new_n135_));
  nor2   g090(.a(x24), .b(x23), .O(new_n136_));
  nand2  g091(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  inv1   g092(.a(new_n137_), .O(new_n138_));
  nand2  g093(.a(new_n138_), .b(new_n114_), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n134_), .c(new_n64_), .d(new_n97_), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(new_n96_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n47_), .O(z17));
  oai21  g097(.a(new_n137_), .b(new_n113_), .c(x25), .O(new_n143_));
  nor3   g098(.a(x25), .b(x24), .c(x23), .O(new_n144_));
  nand3  g099(.a(new_n144_), .b(new_n135_), .c(new_n114_), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(new_n143_), .c(new_n64_), .d(new_n97_), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(new_n96_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n47_), .O(z18));
endmodule


