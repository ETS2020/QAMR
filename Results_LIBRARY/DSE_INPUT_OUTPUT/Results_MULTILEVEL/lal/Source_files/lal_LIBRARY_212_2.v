// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x25), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g16(.a(new_n60_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x23), .c(x24), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n55_), .c(new_n47_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n54_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x23), .O(new_n68_));
  inv1   g23(.a(new_n62_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g25(.a(x24), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x15), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g36(.a(x14), .b(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n50_), .O(z06));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand2  g39(.a(new_n60_), .b(new_n56_), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(x22), .c(x21), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x24), .c(new_n47_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n55_), .O(z08));
  nand4  g44(.a(new_n47_), .b(new_n51_), .c(x05), .d(x04), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand4  g46(.a(new_n52_), .b(new_n57_), .c(new_n47_), .d(new_n51_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n50_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n52_), .c(new_n47_), .d(new_n51_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n59_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n59_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n52_), .c(new_n47_), .d(new_n51_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z12));
  inv1   g57(.a(new_n53_), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nor2   g60(.a(x20), .b(new_n59_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n98_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n105_), .c(new_n103_), .d(new_n47_), .O(z13));
  inv1   g63(.a(x21), .O(new_n109_));
  aoi21  g64(.a(new_n106_), .b(new_n98_), .c(new_n109_), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n98_), .c(x19), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n52_), .c(new_n51_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n110_), .c(new_n47_), .O(new_n114_));
  nand2  g69(.a(x25), .b(x15), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(z14));
  nand3  g71(.a(new_n109_), .b(new_n56_), .c(x19), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n97_), .c(x22), .O(new_n118_));
  inv1   g73(.a(new_n104_), .O(new_n119_));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n56_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n118_), .c(new_n52_), .d(new_n51_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n47_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n115_), .O(z15));
  nand2  g79(.a(new_n121_), .b(x23), .O(new_n125_));
  inv1   g80(.a(x22), .O(new_n126_));
  nand4  g81(.a(new_n111_), .b(new_n119_), .c(new_n68_), .d(new_n126_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n125_), .c(new_n103_), .d(new_n47_), .O(z16));
  nand2  g83(.a(new_n127_), .b(x24), .O(new_n129_));
  nor2   g84(.a(x24), .b(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n120_), .c(new_n106_), .d(new_n98_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n103_), .d(new_n47_), .O(z17));
  nor2   g87(.a(new_n57_), .b(x15), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(x19), .c(x18), .O(new_n134_));
  nand3  g89(.a(new_n130_), .b(new_n111_), .c(new_n126_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(new_n134_), .c(x25), .O(new_n136_));
  nand2  g91(.a(new_n106_), .b(x18), .O(new_n137_));
  inv1   g92(.a(new_n137_), .O(new_n138_));
  nor3   g93(.a(x25), .b(x24), .c(x23), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n138_), .c(new_n133_), .d(new_n120_), .O(new_n140_));
  nand3  g95(.a(new_n140_), .b(new_n136_), .c(new_n54_), .O(z18));
endmodule


