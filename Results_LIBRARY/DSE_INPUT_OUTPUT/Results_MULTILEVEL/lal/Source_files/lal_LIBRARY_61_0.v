// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x08), .O(new_n46_));
  nor2   g001(.a(x15), .b(x02), .O(new_n47_));
  nor3   g002(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g003(.a(x24), .O(new_n49_));
  inv1   g004(.a(x25), .O(new_n50_));
  inv1   g005(.a(x20), .O(new_n51_));
  inv1   g006(.a(x17), .O(new_n52_));
  inv1   g007(.a(x18), .O(new_n53_));
  inv1   g008(.a(x19), .O(new_n54_));
  nand3  g009(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g010(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g011(.a(x21), .O(new_n57_));
  inv1   g012(.a(x22), .O(new_n58_));
  nor2   g013(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g014(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(new_n49_), .c(new_n50_), .O(new_n61_));
  nand2  g016(.a(x05), .b(x04), .O(new_n62_));
  nor2   g017(.a(new_n47_), .b(x07), .O(new_n63_));
  nand3  g018(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z01));
  inv1   g019(.a(x16), .O(new_n65_));
  nor2   g020(.a(new_n47_), .b(new_n65_), .O(z02));
  inv1   g021(.a(new_n47_), .O(new_n67_));
  nand2  g022(.a(new_n61_), .b(new_n67_), .O(z03));
  inv1   g023(.a(x02), .O(new_n69_));
  inv1   g024(.a(x11), .O(new_n70_));
  oai21  g025(.a(new_n70_), .b(x08), .c(x15), .O(new_n71_));
  nand2  g026(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  inv1   g027(.a(x09), .O(new_n73_));
  inv1   g028(.a(x01), .O(new_n74_));
  aoi22  g029(.a(x10), .b(new_n74_), .c(new_n73_), .d(x00), .O(new_n75_));
  oai21  g030(.a(new_n73_), .b(x00), .c(new_n75_), .O(new_n76_));
  inv1   g031(.a(x10), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(x01), .O(new_n78_));
  nand2  g033(.a(new_n70_), .b(x02), .O(new_n79_));
  xnor2a g034(.a(x12), .b(x03), .O(new_n80_));
  nand3  g035(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  oai21  g036(.a(new_n81_), .b(new_n76_), .c(new_n46_), .O(new_n82_));
  nand2  g037(.a(new_n82_), .b(new_n72_), .O(z04));
  nor3   g038(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g039(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(new_n67_), .O(z06));
  nand3  g041(.a(new_n67_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g042(.a(new_n55_), .b(new_n51_), .c(new_n58_), .O(new_n88_));
  aoi21  g043(.a(new_n88_), .b(x21), .c(x23), .O(new_n89_));
  nor2   g044(.a(new_n47_), .b(x25), .O(new_n90_));
  oai21  g045(.a(new_n89_), .b(new_n49_), .c(new_n90_), .O(z08));
  inv1   g046(.a(x07), .O(new_n92_));
  nand3  g047(.a(new_n92_), .b(x05), .c(x04), .O(new_n93_));
  aoi21  g048(.a(new_n93_), .b(x02), .c(x15), .O(z09));
  nand3  g049(.a(new_n62_), .b(new_n52_), .c(new_n92_), .O(new_n95_));
  aoi21  g050(.a(new_n95_), .b(x02), .c(x15), .O(z10));
  inv1   g051(.a(x15), .O(new_n97_));
  xor2a  g052(.a(x18), .b(x17), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n62_), .c(new_n97_), .d(new_n92_), .O(new_n99_));
  nor2   g054(.a(new_n99_), .b(new_n69_), .O(z11));
  nand2  g055(.a(x18), .b(x17), .O(new_n101_));
  inv1   g056(.a(new_n101_), .O(new_n102_));
  nand3  g057(.a(new_n54_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g058(.a(new_n102_), .b(new_n54_), .c(new_n103_), .O(new_n104_));
  nand3  g059(.a(new_n104_), .b(new_n62_), .c(new_n92_), .O(new_n105_));
  aoi21  g060(.a(new_n105_), .b(x02), .c(x15), .O(z12));
  nand3  g061(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g062(.a(new_n107_), .b(x20), .O(new_n108_));
  nand4  g063(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n109_));
  nand4  g064(.a(new_n109_), .b(new_n108_), .c(new_n62_), .d(new_n92_), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(x02), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n97_), .O(z13));
  nor2   g067(.a(x20), .b(new_n54_), .O(new_n113_));
  aoi21  g068(.a(new_n113_), .b(new_n102_), .c(new_n57_), .O(new_n114_));
  nor2   g069(.a(x21), .b(x20), .O(new_n115_));
  nand3  g070(.a(new_n115_), .b(new_n102_), .c(x19), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n62_), .c(new_n92_), .O(new_n117_));
  oai21  g072(.a(new_n117_), .b(new_n114_), .c(x02), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n97_), .O(z14));
  nand3  g074(.a(new_n57_), .b(new_n51_), .c(x19), .O(new_n120_));
  oai21  g075(.a(new_n120_), .b(new_n101_), .c(x22), .O(new_n121_));
  inv1   g076(.a(new_n107_), .O(new_n122_));
  nor2   g077(.a(x22), .b(x21), .O(new_n123_));
  nand3  g078(.a(new_n123_), .b(new_n122_), .c(new_n51_), .O(new_n124_));
  nand4  g079(.a(new_n124_), .b(new_n121_), .c(new_n62_), .d(new_n92_), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(x02), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n97_), .O(z15));
  nand2  g082(.a(new_n124_), .b(x23), .O(new_n128_));
  inv1   g083(.a(x23), .O(new_n129_));
  nand4  g084(.a(new_n115_), .b(new_n122_), .c(new_n129_), .d(new_n58_), .O(new_n130_));
  nand3  g085(.a(new_n130_), .b(new_n97_), .c(new_n92_), .O(new_n131_));
  inv1   g086(.a(new_n131_), .O(new_n132_));
  nand4  g087(.a(new_n132_), .b(new_n128_), .c(new_n62_), .d(x02), .O(z16));
  nand4  g088(.a(new_n129_), .b(new_n58_), .c(new_n57_), .d(new_n51_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(new_n107_), .c(x24), .O(new_n135_));
  nor2   g090(.a(x24), .b(x23), .O(new_n136_));
  nand4  g091(.a(new_n136_), .b(new_n123_), .c(new_n113_), .d(new_n102_), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(new_n135_), .c(new_n62_), .d(new_n92_), .O(new_n138_));
  nand2  g093(.a(new_n138_), .b(x02), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n97_), .O(z17));
  nand4  g095(.a(new_n49_), .b(new_n129_), .c(new_n58_), .d(new_n57_), .O(new_n141_));
  oai21  g096(.a(new_n141_), .b(new_n109_), .c(x25), .O(new_n142_));
  nor3   g097(.a(x25), .b(x24), .c(x23), .O(new_n143_));
  nand4  g098(.a(new_n143_), .b(new_n123_), .c(new_n113_), .d(new_n102_), .O(new_n144_));
  nand4  g099(.a(new_n144_), .b(new_n142_), .c(new_n62_), .d(new_n92_), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(x02), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(new_n97_), .O(z18));
endmodule


