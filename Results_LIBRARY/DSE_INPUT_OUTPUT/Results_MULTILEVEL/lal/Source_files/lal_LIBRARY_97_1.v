// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  and2   g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n58_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n51_), .c(new_n52_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(x07), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n61_), .c(new_n48_), .O(z01));
  nand2  g18(.a(new_n61_), .b(new_n49_), .O(z03));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(new_n49_), .c(new_n46_), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(z04));
  nor3   g26(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor3   g28(.a(new_n48_), .b(new_n73_), .c(x08), .O(z06));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g30(.a(x23), .O(new_n76_));
  nand3  g31(.a(new_n58_), .b(x22), .c(x21), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x24), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n52_), .c(new_n48_), .O(z08));
  inv1   g35(.a(x07), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x05), .c(x04), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n47_), .c(x15), .O(z09));
  inv1   g38(.a(x15), .O(new_n84_));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n54_), .c(new_n47_), .d(new_n84_), .O(new_n86_));
  nor2   g41(.a(new_n86_), .b(x07), .O(z10));
  xor2a  g42(.a(x18), .b(x17), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n85_), .c(new_n47_), .d(new_n84_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(x07), .O(z11));
  nor2   g45(.a(new_n55_), .b(new_n54_), .O(new_n91_));
  nand3  g46(.a(new_n56_), .b(x18), .c(x17), .O(new_n92_));
  oai21  g47(.a(new_n91_), .b(new_n56_), .c(new_n92_), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n85_), .c(new_n47_), .d(new_n84_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(x07), .O(z12));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(x20), .O(new_n97_));
  nand4  g52(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n97_), .c(new_n85_), .d(new_n81_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n47_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n84_), .O(z13));
  nand2  g56(.a(new_n98_), .b(x21), .O(new_n102_));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n91_), .c(x19), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n47_), .c(new_n84_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n102_), .c(new_n62_), .O(z14));
  inv1   g62(.a(new_n96_), .O(new_n108_));
  nor2   g63(.a(x22), .b(x21), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n53_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n85_), .c(new_n81_), .O(new_n111_));
  aoi21  g66(.a(new_n104_), .b(x22), .c(new_n111_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(x16), .c(new_n84_), .O(z15));
  nand2  g68(.a(new_n110_), .b(x23), .O(new_n114_));
  nor2   g69(.a(x23), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n103_), .c(new_n108_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n47_), .c(new_n84_), .O(new_n117_));
  inv1   g72(.a(new_n117_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n114_), .c(new_n62_), .O(z16));
  nand2  g74(.a(new_n116_), .b(x24), .O(new_n120_));
  inv1   g75(.a(new_n98_), .O(new_n121_));
  nand3  g76(.a(new_n109_), .b(new_n51_), .c(new_n76_), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n121_), .c(x16), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n120_), .c(new_n62_), .d(new_n84_), .O(z17));
  oai21  g80(.a(new_n122_), .b(new_n98_), .c(x25), .O(new_n126_));
  nor3   g81(.a(x25), .b(x24), .c(x23), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n109_), .c(new_n121_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n126_), .c(new_n85_), .d(new_n81_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n47_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n84_), .O(z18));
  buf    g86(.a(x16), .O(z02));
endmodule


