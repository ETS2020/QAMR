// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x07), .O(new_n48_));
  nor2   g03(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  and2   g15(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(x23), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n53_), .c(new_n54_), .O(new_n63_));
  aoi21  g18(.a(x05), .b(x04), .c(x07), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(z01));
  nand2  g20(.a(new_n63_), .b(new_n50_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n50_), .c(new_n46_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z04));
  nor3   g28(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g32(.a(x22), .O(new_n78_));
  aoi21  g33(.a(new_n59_), .b(new_n55_), .c(new_n78_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(x21), .c(x23), .O(new_n80_));
  nor2   g35(.a(new_n49_), .b(x25), .O(new_n81_));
  oai21  g36(.a(new_n80_), .b(new_n53_), .c(new_n81_), .O(z08));
  nor2   g37(.a(new_n64_), .b(x15), .O(z09));
  nand2  g38(.a(x05), .b(x04), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n56_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n48_), .c(x15), .O(z10));
  xor2a  g41(.a(x18), .b(x17), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(new_n48_), .c(x15), .O(z11));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand3  g47(.a(new_n58_), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n58_), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n84_), .c(new_n90_), .d(new_n48_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z12));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x20), .O(new_n98_));
  nand3  g53(.a(new_n92_), .b(new_n55_), .c(x19), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n98_), .c(new_n64_), .d(new_n90_), .O(z13));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n92_), .c(x19), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n84_), .O(new_n103_));
  aoi21  g58(.a(new_n99_), .b(x21), .c(new_n103_), .O(new_n104_));
  oai21  g59(.a(new_n104_), .b(x07), .c(new_n90_), .O(z14));
  nand2  g60(.a(new_n102_), .b(x22), .O(new_n106_));
  inv1   g61(.a(new_n97_), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n55_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n106_), .c(new_n84_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n48_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n90_), .O(z15));
  nand2  g67(.a(new_n109_), .b(x23), .O(new_n113_));
  inv1   g68(.a(x23), .O(new_n114_));
  nand3  g69(.a(new_n101_), .b(new_n114_), .c(new_n78_), .O(new_n115_));
  inv1   g70(.a(new_n115_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n107_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n113_), .c(new_n64_), .d(new_n90_), .O(z16));
  oai21  g73(.a(new_n115_), .b(new_n97_), .c(x24), .O(new_n119_));
  nor2   g74(.a(x20), .b(new_n58_), .O(new_n120_));
  nor2   g75(.a(x24), .b(x23), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n108_), .c(new_n120_), .d(new_n92_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n119_), .c(new_n84_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n48_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n90_), .O(z17));
  nand2  g80(.a(new_n122_), .b(x25), .O(new_n126_));
  nor3   g81(.a(x25), .b(x24), .c(x23), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n108_), .c(new_n120_), .d(new_n92_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n126_), .c(new_n64_), .d(new_n90_), .O(z18));
endmodule


