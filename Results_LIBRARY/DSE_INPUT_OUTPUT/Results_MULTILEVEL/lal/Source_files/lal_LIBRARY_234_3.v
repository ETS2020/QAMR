// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x21), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  nor2   g08(.a(x25), .b(x24), .O(new_n54_));
  nor3   g09(.a(new_n54_), .b(new_n53_), .c(x07), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  inv1   g11(.a(x25), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  inv1   g14(.a(x17), .O(new_n60_));
  inv1   g15(.a(x18), .O(new_n61_));
  inv1   g16(.a(x19), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n58_), .c(new_n59_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x15), .c(x21), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n57_), .c(new_n56_), .O(new_n66_));
  oai21  g21(.a(new_n55_), .b(new_n49_), .c(new_n66_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n49_), .b(new_n68_), .O(z02));
  nand2  g24(.a(x24), .b(x23), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n48_), .O(new_n71_));
  oai21  g26(.a(new_n64_), .b(x23), .c(x24), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n47_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n71_), .c(x25), .O(z03));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g32(.a(x12), .b(x03), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n50_), .c(new_n46_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z04));
  inv1   g36(.a(x13), .O(new_n82_));
  nand3  g37(.a(new_n50_), .b(new_n82_), .c(new_n46_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z05));
  nand2  g39(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(z06));
  aoi21  g41(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g42(.a(x24), .O(new_n88_));
  nand2  g43(.a(new_n63_), .b(new_n58_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(x22), .c(x21), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n56_), .c(new_n88_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(x25), .c(new_n47_), .O(new_n92_));
  nand2  g47(.a(new_n70_), .b(new_n57_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n48_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n92_), .O(z08));
  inv1   g50(.a(x07), .O(new_n96_));
  nand3  g51(.a(new_n53_), .b(new_n47_), .c(new_n96_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z09));
  nand4  g53(.a(new_n52_), .b(new_n60_), .c(new_n47_), .d(new_n96_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z10));
  xor2a  g55(.a(x18), .b(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n52_), .c(new_n47_), .d(new_n96_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z11));
  nor2   g58(.a(new_n61_), .b(new_n60_), .O(new_n104_));
  nand3  g59(.a(new_n62_), .b(x18), .c(x17), .O(new_n105_));
  oai21  g60(.a(new_n104_), .b(new_n62_), .c(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n52_), .c(new_n47_), .d(new_n96_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n50_), .O(z12));
  nand3  g63(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n104_), .b(new_n58_), .c(x19), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n52_), .d(new_n96_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n47_), .O(new_n113_));
  oai21  g68(.a(x21), .b(new_n47_), .c(new_n113_), .O(z13));
  nand2  g69(.a(new_n111_), .b(x21), .O(new_n115_));
  nor2   g70(.a(new_n53_), .b(x07), .O(new_n116_));
  nor2   g71(.a(x21), .b(x20), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n104_), .c(x19), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n116_), .c(new_n115_), .d(new_n47_), .O(z14));
  nand2  g74(.a(new_n118_), .b(x22), .O(new_n120_));
  inv1   g75(.a(new_n109_), .O(new_n121_));
  nor2   g76(.a(x22), .b(x21), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n58_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n120_), .c(new_n116_), .d(new_n47_), .O(z15));
  nand2  g79(.a(new_n123_), .b(x23), .O(new_n125_));
  nand4  g80(.a(new_n117_), .b(new_n121_), .c(new_n56_), .d(new_n59_), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n125_), .c(new_n116_), .d(new_n47_), .O(z16));
  nand2  g82(.a(new_n126_), .b(x24), .O(new_n128_));
  inv1   g83(.a(new_n111_), .O(new_n129_));
  nand4  g84(.a(new_n122_), .b(new_n129_), .c(new_n88_), .d(new_n56_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n128_), .c(new_n116_), .d(new_n47_), .O(z17));
  nand2  g86(.a(new_n130_), .b(x25), .O(new_n132_));
  nand4  g87(.a(new_n122_), .b(new_n129_), .c(new_n54_), .d(new_n56_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n132_), .c(new_n116_), .d(new_n47_), .O(z18));
endmodule


