// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x03), .O(new_n47_));
  nand2  g02(.a(x06), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  and2   g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n58_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n51_), .c(new_n52_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x07), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n61_), .c(new_n50_), .O(z01));
  and2   g20(.a(new_n48_), .b(x16), .O(z02));
  nand2  g21(.a(new_n61_), .b(new_n48_), .O(z03));
  aoi21  g22(.a(x12), .b(new_n46_), .c(x06), .O(new_n68_));
  inv1   g23(.a(x09), .O(new_n69_));
  inv1   g24(.a(x01), .O(new_n70_));
  aoi22  g25(.a(x10), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x10), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x01), .O(new_n74_));
  inv1   g29(.a(x02), .O(new_n75_));
  nand2  g30(.a(x11), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  inv1   g33(.a(x12), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x03), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n78_), .c(new_n76_), .d(new_n74_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n72_), .c(new_n46_), .O(new_n82_));
  oai21  g37(.a(new_n68_), .b(x03), .c(new_n82_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g39(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n48_), .O(z06));
  oai21  g41(.a(new_n46_), .b(new_n47_), .c(x06), .O(z07));
  inv1   g42(.a(x22), .O(new_n88_));
  aoi21  g43(.a(new_n57_), .b(new_n53_), .c(new_n88_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x21), .c(x23), .O(new_n90_));
  nor2   g45(.a(new_n50_), .b(x25), .O(new_n91_));
  oai21  g46(.a(new_n90_), .b(new_n51_), .c(new_n91_), .O(z08));
  inv1   g47(.a(x07), .O(new_n93_));
  inv1   g48(.a(x15), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n62_), .c(new_n48_), .O(z09));
  nor2   g51(.a(new_n63_), .b(new_n50_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n54_), .c(new_n94_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z10));
  xor2a  g54(.a(x18), .b(x17), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n62_), .c(new_n94_), .d(new_n93_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n48_), .O(z11));
  nor2   g57(.a(new_n55_), .b(new_n54_), .O(new_n103_));
  nand3  g58(.a(new_n56_), .b(x18), .c(x17), .O(new_n104_));
  oai21  g59(.a(new_n103_), .b(new_n56_), .c(new_n104_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n62_), .c(new_n48_), .d(new_n94_), .O(new_n106_));
  nor2   g61(.a(new_n106_), .b(x07), .O(z12));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n103_), .b(new_n53_), .c(x19), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(x15), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n109_), .c(new_n97_), .d(new_n93_), .O(z13));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n103_), .c(x19), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n64_), .c(new_n94_), .O(new_n116_));
  aoi21  g71(.a(new_n110_), .b(x21), .c(new_n116_), .O(new_n117_));
  nor2   g72(.a(new_n117_), .b(new_n50_), .O(z14));
  nand2  g73(.a(new_n115_), .b(x22), .O(new_n119_));
  inv1   g74(.a(new_n108_), .O(new_n120_));
  nor2   g75(.a(x22), .b(x21), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n53_), .O(new_n122_));
  and2   g77(.a(new_n122_), .b(new_n94_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n119_), .c(new_n97_), .d(new_n93_), .O(z15));
  inv1   g79(.a(x23), .O(new_n125_));
  nand4  g80(.a(new_n114_), .b(new_n120_), .c(new_n125_), .d(new_n88_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n64_), .c(new_n94_), .O(new_n127_));
  aoi21  g82(.a(new_n122_), .b(x23), .c(new_n127_), .O(new_n128_));
  nor2   g83(.a(new_n128_), .b(new_n50_), .O(z16));
  nand2  g84(.a(new_n126_), .b(x24), .O(new_n130_));
  nand3  g85(.a(new_n121_), .b(new_n51_), .c(new_n125_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  aoi21  g87(.a(new_n132_), .b(new_n111_), .c(x15), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n130_), .c(new_n97_), .d(new_n93_), .O(z17));
  oai21  g89(.a(new_n131_), .b(new_n110_), .c(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  and2   g91(.a(new_n136_), .b(new_n121_), .O(new_n137_));
  aoi21  g92(.a(new_n137_), .b(new_n111_), .c(x15), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n135_), .c(new_n97_), .d(new_n93_), .O(z18));
endmodule


