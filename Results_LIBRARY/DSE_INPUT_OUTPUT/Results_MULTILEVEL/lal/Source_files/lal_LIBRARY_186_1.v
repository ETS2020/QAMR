// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  aoi21  g05(.a(x05), .b(x04), .c(x07), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(x15), .c(new_n47_), .O(new_n60_));
  oai22  g15(.a(new_n60_), .b(x25), .c(new_n51_), .d(new_n48_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n49_), .b(new_n62_), .O(z02));
  nor2   g18(.a(new_n60_), .b(x25), .O(z03));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor3   g28(.a(new_n48_), .b(new_n73_), .c(x08), .O(z06));
  nand3  g29(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g30(.a(x23), .O(new_n76_));
  nand3  g31(.a(new_n57_), .b(x22), .c(x21), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n76_), .c(new_n47_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x25), .c(x15), .O(new_n79_));
  nand2  g34(.a(x25), .b(new_n47_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n79_), .O(z08));
  inv1   g36(.a(x07), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(x05), .c(x04), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n47_), .c(x15), .O(z09));
  inv1   g39(.a(x15), .O(new_n85_));
  nand2  g40(.a(x05), .b(x04), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n47_), .c(new_n53_), .d(new_n85_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x07), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n86_), .c(new_n82_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n47_), .c(x15), .O(z11));
  nor2   g46(.a(new_n54_), .b(new_n53_), .O(new_n92_));
  nand3  g47(.a(new_n55_), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n55_), .c(new_n93_), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n86_), .c(new_n82_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n47_), .c(x15), .O(z12));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x20), .O(new_n98_));
  nor2   g53(.a(x20), .b(new_n55_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n92_), .c(x24), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n98_), .c(new_n51_), .d(new_n85_), .O(z13));
  nand2  g56(.a(new_n99_), .b(new_n92_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n92_), .c(x19), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n47_), .c(new_n85_), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n103_), .c(new_n51_), .O(z14));
  nand2  g63(.a(new_n105_), .b(x22), .O(new_n109_));
  inv1   g64(.a(new_n97_), .O(new_n110_));
  nor3   g65(.a(x22), .b(x21), .c(x20), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n110_), .c(x24), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n109_), .c(new_n51_), .d(new_n85_), .O(z15));
  aoi21  g68(.a(new_n111_), .b(new_n110_), .c(new_n76_), .O(new_n114_));
  nor2   g69(.a(x23), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n104_), .c(new_n110_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n86_), .c(new_n82_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n114_), .c(new_n47_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n85_), .O(z16));
  nand2  g74(.a(new_n117_), .b(new_n47_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n85_), .O(z17));
  nor2   g76(.a(x22), .b(x21), .O(new_n122_));
  nor2   g77(.a(x25), .b(x23), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n122_), .c(new_n99_), .d(new_n92_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n86_), .c(new_n82_), .O(new_n125_));
  aoi21  g80(.a(new_n116_), .b(x25), .c(new_n125_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(x24), .c(new_n85_), .O(z18));
endmodule


