// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n88_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x12), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  inv1   g13(.a(x21), .O(new_n59_));
  inv1   g14(.a(x22), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n58_), .c(x23), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n51_), .c(new_n52_), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(x07), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n63_), .c(new_n48_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nand2  g23(.a(new_n49_), .b(new_n68_), .O(z02));
  nand2  g24(.a(new_n63_), .b(new_n49_), .O(z03));
  aoi21  g25(.a(new_n46_), .b(x03), .c(x15), .O(new_n71_));
  inv1   g26(.a(x09), .O(new_n72_));
  inv1   g27(.a(x01), .O(new_n73_));
  aoi22  g28(.a(x10), .b(new_n73_), .c(new_n72_), .d(x00), .O(new_n74_));
  oai21  g29(.a(new_n72_), .b(x00), .c(new_n74_), .O(new_n75_));
  inv1   g30(.a(x10), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x01), .O(new_n77_));
  inv1   g32(.a(x02), .O(new_n78_));
  nand2  g33(.a(x11), .b(new_n78_), .O(new_n79_));
  inv1   g34(.a(x11), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x02), .O(new_n81_));
  inv1   g36(.a(x03), .O(new_n82_));
  nand2  g37(.a(x12), .b(new_n82_), .O(new_n83_));
  nand4  g38(.a(new_n83_), .b(new_n81_), .c(new_n79_), .d(new_n77_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n75_), .c(new_n46_), .O(new_n85_));
  oai21  g40(.a(new_n71_), .b(x12), .c(new_n85_), .O(z04));
  oai21  g41(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g42(.a(x14), .O(new_n88_));
  nor3   g43(.a(new_n48_), .b(new_n88_), .c(x08), .O(z06));
  nand3  g44(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g45(.a(x23), .O(new_n91_));
  nand3  g46(.a(new_n58_), .b(x22), .c(x21), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x24), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n52_), .c(new_n48_), .O(z08));
  inv1   g50(.a(x07), .O(new_n96_));
  nand3  g51(.a(new_n65_), .b(new_n47_), .c(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z09));
  nand4  g53(.a(new_n64_), .b(new_n54_), .c(new_n47_), .d(new_n96_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n49_), .O(z10));
  xor2a  g55(.a(x18), .b(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n64_), .c(new_n47_), .d(new_n96_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n49_), .O(z11));
  nor2   g58(.a(new_n55_), .b(new_n54_), .O(new_n104_));
  nand3  g59(.a(new_n56_), .b(x18), .c(x17), .O(new_n105_));
  oai21  g60(.a(new_n104_), .b(new_n56_), .c(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n64_), .c(new_n47_), .d(new_n96_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n49_), .O(z12));
  nand3  g63(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x20), .O(new_n110_));
  nand4  g65(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n66_), .d(new_n47_), .O(z13));
  inv1   g67(.a(new_n111_), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n59_), .O(new_n114_));
  nor2   g69(.a(x21), .b(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n104_), .c(x19), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n64_), .c(new_n96_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n114_), .c(new_n47_), .O(new_n118_));
  nand2  g73(.a(x15), .b(x12), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(z14));
  nand2  g75(.a(new_n116_), .b(x22), .O(new_n121_));
  inv1   g76(.a(new_n109_), .O(new_n122_));
  nor3   g77(.a(x22), .b(x21), .c(x20), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n122_), .c(x15), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n121_), .c(new_n66_), .O(z15));
  aoi21  g80(.a(new_n123_), .b(new_n122_), .c(new_n91_), .O(new_n126_));
  nand4  g81(.a(new_n115_), .b(new_n122_), .c(new_n91_), .d(new_n60_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n64_), .c(new_n96_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n126_), .c(new_n47_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n119_), .O(z16));
  nand2  g85(.a(new_n127_), .b(x24), .O(new_n131_));
  nor2   g86(.a(x22), .b(x21), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n51_), .c(new_n91_), .O(new_n133_));
  inv1   g88(.a(new_n133_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n113_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n131_), .c(new_n66_), .d(new_n47_), .O(z17));
  oai21  g91(.a(new_n133_), .b(new_n111_), .c(x25), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand3  g93(.a(new_n138_), .b(new_n132_), .c(new_n113_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n137_), .c(new_n64_), .d(new_n96_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n47_), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(new_n119_), .O(z18));
endmodule


