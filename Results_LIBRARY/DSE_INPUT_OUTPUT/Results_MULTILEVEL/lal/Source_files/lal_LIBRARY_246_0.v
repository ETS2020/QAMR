// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x15), .b(x13), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n62_), .c(new_n49_), .d(new_n52_), .O(z01));
  nand3  g19(.a(new_n61_), .b(new_n49_), .c(new_n53_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z04));
  aoi21  g28(.a(x15), .b(x08), .c(x13), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  nor3   g30(.a(new_n48_), .b(new_n75_), .c(x08), .O(z06));
  nand3  g31(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x23), .O(new_n78_));
  nand2  g33(.a(new_n58_), .b(new_n54_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(x22), .c(x21), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(x24), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n53_), .c(new_n48_), .O(z08));
  inv1   g38(.a(x13), .O(new_n84_));
  nor4   g39(.a(new_n63_), .b(x15), .c(new_n84_), .d(x07), .O(z09));
  nand3  g40(.a(new_n63_), .b(new_n55_), .c(new_n52_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x13), .c(x15), .O(z10));
  inv1   g42(.a(x15), .O(new_n88_));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n63_), .c(new_n88_), .d(x13), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x07), .O(z11));
  nor2   g46(.a(new_n56_), .b(new_n55_), .O(new_n92_));
  nand3  g47(.a(new_n57_), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n57_), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n63_), .c(new_n88_), .d(x13), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z12));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x20), .O(new_n98_));
  aoi21  g53(.a(x05), .b(x04), .c(x07), .O(new_n99_));
  nand4  g54(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(x15), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n99_), .c(new_n98_), .d(x13), .O(z13));
  nand2  g58(.a(new_n100_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n92_), .c(x19), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n88_), .c(x13), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n104_), .c(new_n99_), .O(z14));
  inv1   g64(.a(new_n97_), .O(new_n110_));
  nor3   g65(.a(x22), .b(x21), .c(x20), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n63_), .c(new_n52_), .O(new_n113_));
  aoi21  g68(.a(new_n106_), .b(x22), .c(new_n113_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n84_), .c(new_n88_), .O(z15));
  aoi21  g70(.a(new_n111_), .b(new_n110_), .c(new_n78_), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n105_), .c(new_n110_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n63_), .c(new_n52_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n116_), .c(x13), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n88_), .O(z16));
  nand2  g76(.a(new_n118_), .b(x24), .O(new_n122_));
  inv1   g77(.a(x24), .O(new_n123_));
  nor2   g78(.a(x22), .b(x21), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n78_), .O(new_n125_));
  inv1   g80(.a(new_n125_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n101_), .c(x15), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n122_), .c(new_n99_), .d(x13), .O(z17));
  oai21  g83(.a(new_n125_), .b(new_n100_), .c(x25), .O(new_n129_));
  nor3   g84(.a(x25), .b(x24), .c(x23), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n124_), .c(new_n101_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n63_), .d(new_n52_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(x13), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n88_), .O(z18));
endmodule


