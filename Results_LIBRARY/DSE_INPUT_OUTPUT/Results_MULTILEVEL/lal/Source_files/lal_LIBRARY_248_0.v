// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x18), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x21), .O(new_n50_));
  inv1   g05(.a(x22), .O(new_n51_));
  inv1   g06(.a(x23), .O(new_n52_));
  oai21  g07(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(x24), .O(new_n54_));
  inv1   g09(.a(x07), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g12(.a(new_n54_), .b(new_n49_), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x20), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  oai21  g15(.a(x19), .b(x17), .c(x15), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n49_), .c(new_n52_), .d(new_n59_), .O(new_n63_));
  oai21  g18(.a(new_n58_), .b(new_n47_), .c(new_n63_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n47_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n47_), .O(new_n67_));
  nand2  g22(.a(new_n54_), .b(new_n67_), .O(new_n68_));
  nand3  g23(.a(new_n62_), .b(new_n52_), .c(new_n59_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n67_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n67_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor3   g34(.a(new_n47_), .b(new_n79_), .c(x08), .O(z06));
  nand3  g35(.a(new_n67_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x24), .O(new_n82_));
  nand3  g37(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n52_), .c(new_n82_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x25), .c(new_n67_), .O(new_n85_));
  inv1   g40(.a(x17), .O(new_n86_));
  nand3  g41(.a(new_n60_), .b(new_n86_), .c(x15), .O(new_n87_));
  inv1   g42(.a(x19), .O(new_n88_));
  nand4  g43(.a(x24), .b(x22), .c(x21), .d(new_n88_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z08));
  nor4   g45(.a(new_n56_), .b(new_n60_), .c(x15), .d(x07), .O(z09));
  inv1   g46(.a(x15), .O(new_n92_));
  nand4  g47(.a(new_n56_), .b(x18), .c(new_n86_), .d(new_n92_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(x07), .O(z10));
  xor2a  g49(.a(x19), .b(x17), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n56_), .c(new_n55_), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(x18), .c(x15), .O(z12));
  nand2  g52(.a(x19), .b(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x20), .O(new_n99_));
  nand3  g54(.a(new_n59_), .b(x19), .c(x17), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n99_), .c(new_n56_), .d(new_n55_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x18), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n92_), .O(z13));
  inv1   g58(.a(new_n57_), .O(new_n104_));
  nand2  g59(.a(new_n100_), .b(x21), .O(new_n105_));
  nand4  g60(.a(new_n50_), .b(new_n59_), .c(x19), .d(x17), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nor2   g62(.a(new_n107_), .b(new_n60_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n105_), .c(new_n104_), .d(new_n92_), .O(z14));
  nand2  g64(.a(new_n106_), .b(x22), .O(new_n110_));
  nand3  g65(.a(new_n51_), .b(new_n50_), .c(new_n59_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n98_), .O(new_n112_));
  nor2   g67(.a(new_n112_), .b(new_n60_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n110_), .c(new_n104_), .d(new_n92_), .O(z15));
  oai21  g69(.a(new_n111_), .b(new_n98_), .c(x23), .O(new_n115_));
  nor2   g70(.a(x20), .b(new_n88_), .O(new_n116_));
  nor3   g71(.a(x23), .b(x22), .c(x21), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(x17), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n115_), .c(new_n56_), .d(new_n55_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x18), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n92_), .O(z16));
  nand2  g76(.a(new_n118_), .b(x24), .O(new_n122_));
  nand4  g77(.a(new_n82_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n123_));
  nor2   g78(.a(new_n123_), .b(new_n100_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n60_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n122_), .c(new_n104_), .d(new_n92_), .O(z17));
  oai21  g81(.a(new_n123_), .b(new_n100_), .c(x25), .O(new_n127_));
  nand4  g82(.a(new_n49_), .b(new_n82_), .c(new_n52_), .d(new_n51_), .O(new_n128_));
  inv1   g83(.a(new_n128_), .O(new_n129_));
  aoi21  g84(.a(new_n129_), .b(new_n107_), .c(new_n60_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n127_), .c(new_n104_), .d(new_n92_), .O(z18));
  nor2   g86(.a(new_n93_), .b(x07), .O(z11));
endmodule


