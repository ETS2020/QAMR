// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  nand2  g001(.a(x17), .b(x15), .O(new_n47_));
  oai21  g002(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  nor3   g003(.a(x25), .b(x23), .c(x20), .O(new_n49_));
  oai21  g004(.a(new_n49_), .b(x15), .c(x17), .O(new_n50_));
  inv1   g005(.a(x24), .O(new_n51_));
  inv1   g006(.a(x25), .O(new_n52_));
  inv1   g007(.a(x20), .O(new_n53_));
  oai21  g008(.a(x19), .b(x18), .c(new_n53_), .O(new_n54_));
  inv1   g009(.a(x21), .O(new_n55_));
  inv1   g010(.a(x22), .O(new_n56_));
  nor2   g011(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g012(.a(new_n57_), .b(new_n54_), .c(x23), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(new_n51_), .c(new_n52_), .O(new_n59_));
  nand2  g014(.a(x05), .b(x04), .O(new_n60_));
  inv1   g015(.a(new_n60_), .O(new_n61_));
  nor2   g016(.a(new_n61_), .b(x07), .O(new_n62_));
  nand3  g017(.a(new_n62_), .b(new_n59_), .c(new_n50_), .O(z01));
  inv1   g018(.a(x16), .O(new_n64_));
  nand2  g019(.a(new_n47_), .b(new_n64_), .O(z02));
  oai21  g020(.a(new_n57_), .b(x23), .c(x24), .O(new_n66_));
  nand2  g021(.a(new_n66_), .b(new_n47_), .O(new_n67_));
  inv1   g022(.a(x23), .O(new_n68_));
  nor2   g023(.a(x19), .b(x18), .O(new_n69_));
  inv1   g024(.a(x15), .O(new_n70_));
  nand2  g025(.a(x17), .b(new_n70_), .O(new_n71_));
  oai21  g026(.a(new_n69_), .b(x17), .c(new_n71_), .O(new_n72_));
  nand3  g027(.a(new_n72_), .b(new_n68_), .c(new_n53_), .O(new_n73_));
  aoi21  g028(.a(new_n73_), .b(new_n67_), .c(x25), .O(z03));
  xnor2a g029(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g030(.a(x10), .b(x01), .O(new_n76_));
  xnor2a g031(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g032(.a(x12), .b(x03), .O(new_n78_));
  nand4  g033(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand3  g034(.a(new_n79_), .b(new_n47_), .c(new_n46_), .O(new_n80_));
  inv1   g035(.a(new_n80_), .O(z04));
  inv1   g036(.a(x13), .O(new_n82_));
  nand3  g037(.a(new_n47_), .b(new_n82_), .c(new_n46_), .O(new_n83_));
  inv1   g038(.a(new_n83_), .O(z05));
  inv1   g039(.a(x14), .O(new_n85_));
  oai21  g040(.a(new_n85_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g041(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g042(.a(x17), .O(new_n88_));
  nand2  g043(.a(new_n69_), .b(new_n88_), .O(new_n89_));
  aoi21  g044(.a(new_n89_), .b(new_n53_), .c(new_n56_), .O(new_n90_));
  aoi21  g045(.a(new_n90_), .b(x21), .c(x23), .O(new_n91_));
  aoi21  g046(.a(x17), .b(x15), .c(x25), .O(new_n92_));
  oai21  g047(.a(new_n91_), .b(new_n51_), .c(new_n92_), .O(z08));
  inv1   g048(.a(x07), .O(new_n94_));
  nand3  g049(.a(new_n61_), .b(new_n70_), .c(new_n94_), .O(new_n95_));
  inv1   g050(.a(new_n95_), .O(z09));
  nand4  g051(.a(new_n60_), .b(new_n88_), .c(new_n70_), .d(new_n94_), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(new_n47_), .O(z10));
  nand3  g053(.a(x18), .b(new_n88_), .c(new_n70_), .O(new_n99_));
  oai21  g054(.a(x18), .b(new_n88_), .c(new_n99_), .O(new_n100_));
  nand3  g055(.a(new_n100_), .b(new_n60_), .c(new_n94_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n47_), .O(z11));
  inv1   g057(.a(x19), .O(new_n103_));
  and2   g058(.a(x18), .b(x17), .O(new_n104_));
  nand3  g059(.a(new_n103_), .b(x18), .c(x17), .O(new_n105_));
  oai21  g060(.a(new_n104_), .b(new_n103_), .c(new_n105_), .O(new_n106_));
  nand4  g061(.a(new_n106_), .b(new_n60_), .c(new_n70_), .d(new_n94_), .O(new_n107_));
  inv1   g062(.a(new_n107_), .O(z12));
  oai21  g063(.a(x20), .b(x15), .c(new_n88_), .O(new_n109_));
  aoi21  g064(.a(x19), .b(x18), .c(new_n53_), .O(new_n110_));
  nor2   g065(.a(x20), .b(new_n103_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n104_), .O(new_n112_));
  nand3  g067(.a(new_n112_), .b(new_n60_), .c(new_n94_), .O(new_n113_));
  oai21  g068(.a(new_n113_), .b(new_n110_), .c(new_n70_), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n109_), .O(z13));
  nand2  g070(.a(new_n112_), .b(x21), .O(new_n116_));
  nor2   g071(.a(x21), .b(x20), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(new_n104_), .c(x19), .O(new_n118_));
  nand4  g073(.a(new_n118_), .b(new_n116_), .c(new_n62_), .d(new_n70_), .O(z14));
  oai21  g074(.a(x22), .b(x15), .c(new_n88_), .O(new_n120_));
  nand3  g075(.a(new_n117_), .b(x19), .c(x18), .O(new_n121_));
  nand2  g076(.a(new_n121_), .b(x22), .O(new_n122_));
  nand3  g077(.a(x19), .b(x18), .c(x17), .O(new_n123_));
  inv1   g078(.a(new_n123_), .O(new_n124_));
  nor2   g079(.a(x22), .b(x21), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n124_), .c(new_n53_), .O(new_n126_));
  nand4  g081(.a(new_n126_), .b(new_n122_), .c(new_n60_), .d(new_n94_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n70_), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n120_), .O(z15));
  nand2  g084(.a(new_n126_), .b(x23), .O(new_n130_));
  nand4  g085(.a(new_n124_), .b(new_n117_), .c(new_n68_), .d(new_n56_), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n130_), .c(new_n62_), .d(new_n70_), .O(z16));
  oai21  g087(.a(x24), .b(x15), .c(new_n88_), .O(new_n133_));
  nand3  g088(.a(new_n53_), .b(x19), .c(x18), .O(new_n134_));
  nand3  g089(.a(new_n68_), .b(new_n56_), .c(new_n55_), .O(new_n135_));
  oai21  g090(.a(new_n135_), .b(new_n134_), .c(x24), .O(new_n136_));
  nor2   g091(.a(x24), .b(x23), .O(new_n137_));
  nand4  g092(.a(new_n137_), .b(new_n125_), .c(new_n111_), .d(new_n104_), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n136_), .c(new_n60_), .d(new_n94_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n70_), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(new_n133_), .O(z17));
  oai21  g096(.a(x25), .b(x15), .c(new_n88_), .O(new_n142_));
  nand4  g097(.a(new_n51_), .b(new_n68_), .c(new_n56_), .d(new_n55_), .O(new_n143_));
  oai21  g098(.a(new_n143_), .b(new_n134_), .c(x25), .O(new_n144_));
  nor3   g099(.a(x25), .b(x24), .c(x23), .O(new_n145_));
  nand4  g100(.a(new_n145_), .b(new_n125_), .c(new_n111_), .d(new_n104_), .O(new_n146_));
  nand4  g101(.a(new_n146_), .b(new_n144_), .c(new_n60_), .d(new_n94_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n70_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n142_), .O(z18));
endmodule


