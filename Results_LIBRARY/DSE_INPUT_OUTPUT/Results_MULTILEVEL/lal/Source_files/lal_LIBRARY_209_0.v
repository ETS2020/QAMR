// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x15), .b(x06), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(x07), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n62_), .c(new_n47_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n62_), .b(new_n48_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n48_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n48_), .O(z06));
  nand2  g34(.a(new_n46_), .b(x06), .O(z07));
  aoi21  g35(.a(new_n56_), .b(new_n52_), .c(new_n59_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x21), .c(x23), .O(new_n82_));
  nor2   g37(.a(new_n47_), .b(x25), .O(new_n83_));
  oai21  g38(.a(new_n82_), .b(new_n50_), .c(new_n83_), .O(z08));
  inv1   g39(.a(x06), .O(new_n85_));
  nor4   g40(.a(new_n63_), .b(x15), .c(x07), .d(new_n85_), .O(z09));
  inv1   g41(.a(x07), .O(new_n87_));
  nand3  g42(.a(new_n63_), .b(new_n53_), .c(new_n87_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x06), .c(x15), .O(z10));
  inv1   g44(.a(x15), .O(new_n90_));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n63_), .c(new_n90_), .d(new_n87_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(new_n85_), .O(z11));
  nor2   g48(.a(new_n54_), .b(new_n53_), .O(new_n94_));
  nand3  g49(.a(new_n55_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n55_), .c(new_n95_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n63_), .c(new_n90_), .d(new_n87_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(new_n85_), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nand4  g55(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n100_), .c(new_n63_), .d(new_n87_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x06), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n90_), .O(z13));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  nor2   g60(.a(new_n64_), .b(new_n85_), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n94_), .c(x19), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n90_), .c(new_n87_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n106_), .c(new_n105_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  inv1   g67(.a(new_n99_), .O(new_n113_));
  nor3   g68(.a(x22), .b(x21), .c(x20), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(x15), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n106_), .d(new_n87_), .O(z15));
  inv1   g71(.a(x23), .O(new_n117_));
  aoi21  g72(.a(new_n114_), .b(new_n113_), .c(new_n117_), .O(new_n118_));
  nand4  g73(.a(new_n107_), .b(new_n113_), .c(new_n117_), .d(new_n59_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n63_), .c(new_n87_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n118_), .c(x06), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n90_), .O(z16));
  nand2  g77(.a(new_n119_), .b(x24), .O(new_n123_));
  inv1   g78(.a(new_n101_), .O(new_n124_));
  nor2   g79(.a(x22), .b(x21), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n50_), .c(new_n117_), .O(new_n126_));
  inv1   g81(.a(new_n126_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n124_), .c(x15), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n123_), .c(new_n106_), .d(new_n87_), .O(z17));
  oai21  g84(.a(new_n126_), .b(new_n101_), .c(x25), .O(new_n130_));
  nor3   g85(.a(x25), .b(x24), .c(x23), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n125_), .c(new_n124_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n130_), .c(new_n63_), .d(new_n87_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(x06), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n90_), .O(z18));
endmodule


