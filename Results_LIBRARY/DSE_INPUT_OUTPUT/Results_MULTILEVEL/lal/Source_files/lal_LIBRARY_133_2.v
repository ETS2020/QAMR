// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x23), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  nor2   g06(.a(x25), .b(x24), .O(new_n52_));
  nor3   g07(.a(new_n52_), .b(new_n51_), .c(x07), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(x22), .c(x21), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n55_), .c(new_n54_), .d(new_n47_), .O(new_n63_));
  oai21  g18(.a(new_n53_), .b(new_n48_), .c(new_n63_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(x24), .O(new_n67_));
  inv1   g22(.a(new_n48_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g24(.a(new_n62_), .b(new_n54_), .c(new_n47_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n68_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  nor3   g33(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n68_), .O(z06));
  nand3  g36(.a(new_n68_), .b(new_n46_), .c(x06), .O(z07));
  nand4  g37(.a(new_n61_), .b(x24), .c(x22), .d(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n55_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n47_), .O(new_n85_));
  oai21  g40(.a(x25), .b(x24), .c(x23), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n51_), .b(new_n47_), .c(new_n88_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z09));
  nand4  g45(.a(new_n50_), .b(new_n57_), .c(new_n47_), .d(new_n88_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n50_), .c(new_n47_), .d(new_n88_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n68_), .O(z11));
  nor2   g50(.a(new_n58_), .b(new_n57_), .O(new_n96_));
  nand3  g51(.a(new_n59_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n59_), .c(new_n97_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n50_), .c(new_n47_), .d(new_n88_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z12));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x20), .O(new_n102_));
  nor2   g57(.a(x20), .b(new_n59_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n96_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n102_), .c(new_n50_), .d(new_n88_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n47_), .O(new_n106_));
  nand2  g61(.a(x23), .b(x15), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(z13));
  inv1   g63(.a(x21), .O(new_n109_));
  aoi21  g64(.a(new_n103_), .b(new_n96_), .c(new_n109_), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n96_), .c(x19), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n50_), .c(new_n88_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n110_), .c(new_n47_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n107_), .O(z14));
  inv1   g70(.a(new_n101_), .O(new_n116_));
  nor2   g71(.a(x22), .b(x21), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n56_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n50_), .c(new_n88_), .O(new_n119_));
  aoi21  g74(.a(new_n112_), .b(x22), .c(new_n119_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(x15), .c(new_n107_), .O(z15));
  nand2  g76(.a(new_n118_), .b(x23), .O(new_n122_));
  nor2   g77(.a(new_n51_), .b(x07), .O(new_n123_));
  nor2   g78(.a(x23), .b(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n111_), .c(new_n116_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n123_), .c(new_n122_), .d(new_n47_), .O(z16));
  oai21  g81(.a(x24), .b(x15), .c(x23), .O(new_n127_));
  nand2  g82(.a(new_n117_), .b(new_n56_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n101_), .c(x24), .O(new_n129_));
  nor2   g84(.a(x24), .b(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n117_), .c(new_n103_), .d(new_n96_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n50_), .d(new_n88_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n47_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n127_), .O(z17));
  nand2  g89(.a(new_n131_), .b(x25), .O(new_n135_));
  inv1   g90(.a(new_n104_), .O(new_n136_));
  nand4  g91(.a(new_n117_), .b(new_n136_), .c(new_n52_), .d(new_n54_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n123_), .d(new_n47_), .O(z18));
endmodule


