// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x17), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  nor3   g07(.a(x19), .b(x18), .c(x17), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  oai21  g10(.a(new_n53_), .b(x20), .c(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n51_), .c(new_n52_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor3   g14(.a(new_n59_), .b(new_n48_), .c(x07), .O(new_n60_));
  oai21  g15(.a(new_n57_), .b(x25), .c(new_n60_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n49_), .b(new_n62_), .O(z02));
  oai21  g18(.a(new_n55_), .b(x23), .c(x24), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n49_), .O(new_n65_));
  inv1   g20(.a(x20), .O(new_n66_));
  inv1   g21(.a(x17), .O(new_n67_));
  nor2   g22(.a(x19), .b(x18), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x15), .c(new_n67_), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(new_n51_), .c(new_n66_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n65_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n49_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  nor3   g33(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n49_), .O(z06));
  nand3  g36(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g37(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n68_), .c(x15), .O(new_n85_));
  oai21  g40(.a(new_n54_), .b(new_n66_), .c(new_n51_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x24), .c(x25), .O(new_n87_));
  oai21  g42(.a(new_n85_), .b(x17), .c(new_n87_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n59_), .b(new_n47_), .c(new_n89_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand2  g46(.a(new_n58_), .b(new_n89_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n47_), .c(x17), .O(z10));
  inv1   g48(.a(x18), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(x17), .c(new_n47_), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(x17), .c(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n58_), .c(new_n89_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z11));
  inv1   g53(.a(x19), .O(new_n99_));
  nor2   g54(.a(new_n94_), .b(new_n67_), .O(new_n100_));
  nand3  g55(.a(new_n99_), .b(x18), .c(x17), .O(new_n101_));
  oai21  g56(.a(new_n100_), .b(new_n99_), .c(new_n101_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n58_), .c(new_n47_), .d(new_n89_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z12));
  nand3  g59(.a(new_n66_), .b(x19), .c(x18), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n47_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x17), .O(new_n107_));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(x20), .c(new_n92_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(x15), .c(new_n107_), .O(z13));
  inv1   g65(.a(new_n92_), .O(new_n111_));
  nand4  g66(.a(new_n66_), .b(x19), .c(x18), .d(x17), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n100_), .c(x19), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n111_), .d(new_n47_), .O(z14));
  nand2  g71(.a(x19), .b(x18), .O(new_n117_));
  inv1   g72(.a(x21), .O(new_n118_));
  inv1   g73(.a(x22), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n66_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n117_), .c(new_n47_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x17), .O(new_n122_));
  aoi21  g77(.a(new_n115_), .b(x22), .c(new_n92_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(x15), .c(new_n122_), .O(z15));
  oai21  g79(.a(new_n120_), .b(new_n108_), .c(x23), .O(new_n125_));
  inv1   g80(.a(new_n108_), .O(new_n126_));
  nand4  g81(.a(new_n114_), .b(new_n126_), .c(new_n51_), .d(new_n119_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n125_), .c(new_n111_), .d(new_n47_), .O(z16));
  nand4  g83(.a(new_n52_), .b(new_n51_), .c(new_n119_), .d(new_n118_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n105_), .c(new_n47_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(x17), .O(new_n131_));
  aoi21  g86(.a(new_n127_), .b(x24), .c(new_n92_), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(x15), .c(new_n131_), .O(z17));
  nand3  g88(.a(new_n114_), .b(x19), .c(x18), .O(new_n134_));
  inv1   g89(.a(x25), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n52_), .c(new_n51_), .d(new_n119_), .O(new_n136_));
  oai21  g91(.a(new_n136_), .b(new_n134_), .c(new_n47_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(x17), .O(new_n138_));
  oai21  g93(.a(new_n129_), .b(new_n112_), .c(x25), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n111_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n47_), .O(new_n141_));
  nand2  g96(.a(new_n141_), .b(new_n138_), .O(z18));
endmodule


