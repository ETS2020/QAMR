// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  inv1   g002(.a(x18), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(x17), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g006(.a(z02), .b(new_n46_), .O(z00));
  inv1   g007(.a(x24), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x20), .O(new_n55_));
  oai21  g010(.a(x19), .b(x17), .c(new_n55_), .O(new_n56_));
  nand2  g011(.a(x22), .b(x21), .O(new_n57_));
  inv1   g012(.a(new_n57_), .O(new_n58_));
  aoi21  g013(.a(new_n58_), .b(new_n56_), .c(x23), .O(new_n59_));
  oai21  g014(.a(new_n59_), .b(new_n53_), .c(new_n54_), .O(new_n60_));
  nand2  g015(.a(x05), .b(x04), .O(new_n61_));
  inv1   g016(.a(new_n61_), .O(new_n62_));
  nor3   g017(.a(new_n62_), .b(new_n49_), .c(x07), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(new_n60_), .O(z01));
  inv1   g019(.a(x23), .O(new_n65_));
  inv1   g020(.a(x17), .O(new_n66_));
  nand2  g021(.a(x19), .b(new_n48_), .O(new_n67_));
  aoi21  g022(.a(new_n67_), .b(new_n66_), .c(x20), .O(new_n68_));
  aoi21  g023(.a(x18), .b(new_n66_), .c(new_n58_), .O(new_n69_));
  oai21  g024(.a(new_n69_), .b(new_n68_), .c(new_n65_), .O(new_n70_));
  nand2  g025(.a(new_n50_), .b(new_n53_), .O(new_n71_));
  aoi21  g026(.a(new_n71_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g027(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g028(.a(x10), .b(x01), .O(new_n74_));
  xnor2a g029(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g030(.a(x12), .b(x03), .O(new_n76_));
  nand4  g031(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand3  g032(.a(new_n77_), .b(new_n50_), .c(new_n46_), .O(new_n78_));
  inv1   g033(.a(new_n78_), .O(z04));
  oai21  g034(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g035(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(new_n50_), .O(z06));
  aoi21  g037(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g038(.a(x19), .O(new_n84_));
  nand4  g039(.a(x24), .b(x22), .c(x21), .d(new_n84_), .O(new_n85_));
  nand2  g040(.a(new_n85_), .b(new_n48_), .O(new_n86_));
  nand2  g041(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  oai21  g042(.a(new_n57_), .b(new_n55_), .c(new_n65_), .O(new_n88_));
  aoi21  g043(.a(new_n88_), .b(x24), .c(x25), .O(new_n89_));
  nand2  g044(.a(new_n89_), .b(new_n87_), .O(z08));
  inv1   g045(.a(x07), .O(new_n91_));
  inv1   g046(.a(x15), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g048(.a(new_n93_), .b(new_n61_), .c(new_n50_), .O(z09));
  nand3  g049(.a(new_n61_), .b(new_n92_), .c(new_n91_), .O(new_n95_));
  aoi21  g050(.a(new_n95_), .b(new_n48_), .c(x17), .O(z10));
  nand4  g051(.a(new_n61_), .b(new_n48_), .c(x17), .d(new_n92_), .O(new_n97_));
  oai21  g052(.a(new_n97_), .b(x07), .c(new_n50_), .O(z11));
  nand2  g053(.a(new_n84_), .b(x18), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(new_n67_), .O(new_n100_));
  nand4  g055(.a(new_n100_), .b(new_n61_), .c(new_n92_), .d(new_n91_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n50_), .O(z12));
  oai21  g057(.a(new_n93_), .b(new_n62_), .c(new_n50_), .O(new_n103_));
  nor2   g058(.a(new_n55_), .b(x19), .O(new_n104_));
  nand3  g059(.a(new_n55_), .b(x19), .c(x18), .O(new_n105_));
  inv1   g060(.a(new_n105_), .O(new_n106_));
  oai21  g061(.a(new_n106_), .b(new_n104_), .c(x17), .O(new_n107_));
  nand2  g062(.a(x20), .b(new_n48_), .O(new_n108_));
  nand3  g063(.a(new_n108_), .b(new_n107_), .c(new_n103_), .O(z13));
  oai21  g064(.a(x20), .b(new_n84_), .c(x21), .O(new_n110_));
  and2   g065(.a(x19), .b(x18), .O(new_n111_));
  nor2   g066(.a(x21), .b(x20), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g068(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(x17), .O(new_n115_));
  nand2  g070(.a(x21), .b(new_n48_), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n115_), .c(new_n103_), .O(z14));
  inv1   g072(.a(x21), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n55_), .c(x19), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(x22), .O(new_n120_));
  nor2   g075(.a(x22), .b(x21), .O(new_n121_));
  nand3  g076(.a(new_n121_), .b(new_n111_), .c(new_n55_), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(x17), .O(new_n124_));
  nand2  g079(.a(x22), .b(new_n48_), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n124_), .c(new_n103_), .O(z15));
  oai21  g081(.a(x23), .b(x18), .c(new_n66_), .O(new_n127_));
  nand2  g082(.a(new_n122_), .b(x23), .O(new_n128_));
  nor2   g083(.a(new_n62_), .b(x07), .O(new_n129_));
  nor3   g084(.a(x23), .b(x22), .c(x21), .O(new_n130_));
  aoi21  g085(.a(new_n130_), .b(new_n106_), .c(x15), .O(new_n131_));
  nand4  g086(.a(new_n131_), .b(new_n129_), .c(new_n128_), .d(new_n127_), .O(z16));
  oai21  g087(.a(x24), .b(x18), .c(new_n66_), .O(new_n133_));
  nand2  g088(.a(new_n130_), .b(new_n106_), .O(new_n134_));
  nand2  g089(.a(new_n134_), .b(x24), .O(new_n135_));
  nand3  g090(.a(new_n121_), .b(new_n53_), .c(new_n65_), .O(new_n136_));
  inv1   g091(.a(new_n136_), .O(new_n137_));
  aoi21  g092(.a(new_n137_), .b(new_n106_), .c(x15), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n135_), .c(new_n133_), .d(new_n129_), .O(z17));
  inv1   g094(.a(x22), .O(new_n140_));
  nand3  g095(.a(new_n53_), .b(new_n65_), .c(new_n140_), .O(new_n141_));
  oai21  g096(.a(new_n141_), .b(new_n119_), .c(x25), .O(new_n142_));
  nor2   g097(.a(x23), .b(x22), .O(new_n143_));
  nor2   g098(.a(x25), .b(x24), .O(new_n144_));
  nand4  g099(.a(new_n144_), .b(new_n143_), .c(new_n112_), .d(new_n111_), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(x17), .O(new_n147_));
  nand2  g102(.a(x25), .b(new_n48_), .O(new_n148_));
  nand3  g103(.a(new_n148_), .b(new_n147_), .c(new_n103_), .O(z18));
endmodule


