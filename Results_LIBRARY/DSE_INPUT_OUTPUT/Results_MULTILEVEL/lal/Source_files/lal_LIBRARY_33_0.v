// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x21), .b(x17), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x21), .O(new_n50_));
  oai21  g05(.a(x25), .b(x23), .c(x17), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(x22), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n53_), .c(new_n54_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n52_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n47_), .b(new_n65_), .O(z02));
  nand2  g21(.a(new_n62_), .b(new_n52_), .O(z03));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n48_), .O(z04));
  nor3   g29(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g33(.a(x24), .b(x23), .c(x25), .O(new_n79_));
  nand4  g34(.a(new_n60_), .b(x24), .c(x22), .d(x21), .O(new_n80_));
  oai21  g35(.a(new_n79_), .b(new_n47_), .c(new_n80_), .O(z08));
  nand2  g36(.a(x05), .b(x04), .O(new_n82_));
  inv1   g37(.a(x07), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n82_), .c(new_n48_), .O(z09));
  nand4  g41(.a(new_n82_), .b(x21), .c(new_n56_), .d(new_n84_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x07), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n82_), .c(new_n84_), .d(new_n83_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n48_), .O(z11));
  nor2   g46(.a(new_n57_), .b(new_n56_), .O(new_n92_));
  nand3  g47(.a(new_n58_), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n58_), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n82_), .c(new_n84_), .d(new_n83_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n48_), .O(z12));
  oai21  g51(.a(new_n50_), .b(x20), .c(new_n56_), .O(new_n97_));
  nand3  g52(.a(new_n82_), .b(new_n84_), .c(new_n83_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g54(.a(new_n58_), .b(new_n57_), .O(new_n100_));
  nand3  g55(.a(new_n55_), .b(x19), .c(x18), .O(new_n101_));
  oai21  g56(.a(new_n100_), .b(new_n55_), .c(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x17), .O(new_n103_));
  nand3  g58(.a(x21), .b(x20), .c(new_n56_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n99_), .O(z13));
  nand2  g60(.a(new_n98_), .b(new_n48_), .O(new_n106_));
  nand3  g61(.a(new_n92_), .b(new_n55_), .c(x19), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x21), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n92_), .c(x19), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(z14));
  oai21  g66(.a(x22), .b(new_n50_), .c(new_n56_), .O(new_n112_));
  nand2  g67(.a(new_n109_), .b(new_n100_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x22), .O(new_n114_));
  inv1   g69(.a(x22), .O(new_n115_));
  nand4  g70(.a(new_n100_), .b(new_n115_), .c(new_n50_), .d(new_n55_), .O(new_n116_));
  and2   g71(.a(new_n116_), .b(new_n84_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n114_), .c(new_n112_), .d(new_n63_), .O(z15));
  oai21  g73(.a(x23), .b(new_n50_), .c(new_n56_), .O(new_n119_));
  nand2  g74(.a(new_n116_), .b(x23), .O(new_n120_));
  inv1   g75(.a(new_n101_), .O(new_n121_));
  nor3   g76(.a(x23), .b(x22), .c(x21), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n121_), .c(x15), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n120_), .c(new_n119_), .d(new_n63_), .O(z16));
  nor3   g79(.a(x23), .b(x22), .c(x20), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n100_), .c(new_n53_), .O(new_n126_));
  nand2  g81(.a(new_n115_), .b(new_n50_), .O(new_n127_));
  inv1   g82(.a(x23), .O(new_n128_));
  nand2  g83(.a(new_n53_), .b(new_n128_), .O(new_n129_));
  nor3   g84(.a(new_n129_), .b(new_n127_), .c(new_n101_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n126_), .c(x17), .O(new_n131_));
  nand2  g86(.a(x24), .b(x21), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n131_), .c(new_n106_), .O(z17));
  nand3  g88(.a(new_n53_), .b(new_n128_), .c(new_n115_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n101_), .c(x25), .O(new_n135_));
  nand4  g90(.a(new_n54_), .b(new_n53_), .c(new_n128_), .d(new_n115_), .O(new_n136_));
  oai21  g91(.a(new_n136_), .b(new_n113_), .c(new_n135_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(x17), .O(new_n138_));
  nand2  g93(.a(x25), .b(x21), .O(new_n139_));
  nand3  g94(.a(new_n139_), .b(new_n138_), .c(new_n106_), .O(z18));
endmodule


