// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x03), .O(new_n47_));
  nor2   g02(.a(x06), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  and2   g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n58_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n51_), .c(new_n52_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x07), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n61_), .c(new_n48_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  nand2  g22(.a(new_n61_), .b(new_n49_), .O(z03));
  oai21  g23(.a(x12), .b(x08), .c(x06), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(x03), .O(new_n70_));
  inv1   g25(.a(x09), .O(new_n71_));
  inv1   g26(.a(x01), .O(new_n72_));
  aoi22  g27(.a(x10), .b(new_n72_), .c(new_n71_), .d(x00), .O(new_n73_));
  oai21  g28(.a(new_n71_), .b(x00), .c(new_n73_), .O(new_n74_));
  inv1   g29(.a(x10), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x01), .O(new_n76_));
  inv1   g31(.a(x02), .O(new_n77_));
  nand2  g32(.a(x11), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(x11), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x02), .O(new_n80_));
  nand2  g35(.a(x12), .b(new_n47_), .O(new_n81_));
  nand4  g36(.a(new_n81_), .b(new_n80_), .c(new_n78_), .d(new_n76_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n74_), .c(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n70_), .O(z04));
  oai21  g39(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g40(.a(x14), .b(new_n46_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n49_), .O(z06));
  nand2  g42(.a(x08), .b(x06), .O(new_n88_));
  oai21  g43(.a(x06), .b(x03), .c(new_n88_), .O(z07));
  inv1   g44(.a(x22), .O(new_n90_));
  aoi21  g45(.a(new_n57_), .b(new_n53_), .c(new_n90_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(x21), .c(x23), .O(new_n92_));
  nor2   g47(.a(new_n48_), .b(x25), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n51_), .c(new_n93_), .O(z08));
  inv1   g49(.a(x07), .O(new_n95_));
  inv1   g50(.a(x15), .O(new_n96_));
  nand3  g51(.a(new_n63_), .b(new_n96_), .c(new_n95_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z09));
  nor2   g53(.a(new_n63_), .b(new_n48_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n54_), .c(new_n96_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(x07), .O(z10));
  xor2a  g56(.a(x18), .b(x17), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n62_), .c(new_n96_), .d(new_n95_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n49_), .O(z11));
  nor2   g59(.a(new_n55_), .b(new_n54_), .O(new_n105_));
  nand3  g60(.a(new_n56_), .b(x18), .c(x17), .O(new_n106_));
  oai21  g61(.a(new_n105_), .b(new_n56_), .c(new_n106_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n62_), .c(new_n49_), .d(new_n96_), .O(new_n108_));
  nor2   g63(.a(new_n108_), .b(x07), .O(z12));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n105_), .b(new_n53_), .c(x19), .O(new_n112_));
  inv1   g67(.a(new_n112_), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(x15), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n111_), .c(new_n99_), .d(new_n95_), .O(z13));
  nor2   g70(.a(x21), .b(x20), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n105_), .c(x19), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n64_), .c(new_n96_), .O(new_n118_));
  aoi21  g73(.a(new_n112_), .b(x21), .c(new_n118_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(new_n48_), .O(z14));
  nand2  g75(.a(new_n117_), .b(x22), .O(new_n121_));
  inv1   g76(.a(new_n110_), .O(new_n122_));
  nor2   g77(.a(x22), .b(x21), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n53_), .O(new_n124_));
  and2   g79(.a(new_n124_), .b(new_n96_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n121_), .c(new_n99_), .d(new_n95_), .O(z15));
  inv1   g81(.a(x23), .O(new_n127_));
  nand4  g82(.a(new_n116_), .b(new_n122_), .c(new_n127_), .d(new_n90_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n64_), .c(new_n96_), .O(new_n129_));
  aoi21  g84(.a(new_n124_), .b(x23), .c(new_n129_), .O(new_n130_));
  nor2   g85(.a(new_n130_), .b(new_n48_), .O(z16));
  nand2  g86(.a(new_n128_), .b(x24), .O(new_n132_));
  nand3  g87(.a(new_n123_), .b(new_n51_), .c(new_n127_), .O(new_n133_));
  inv1   g88(.a(new_n133_), .O(new_n134_));
  aoi21  g89(.a(new_n134_), .b(new_n113_), .c(x15), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n132_), .c(new_n99_), .d(new_n95_), .O(z17));
  oai21  g91(.a(new_n133_), .b(new_n112_), .c(x25), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  and2   g93(.a(new_n138_), .b(new_n123_), .O(new_n139_));
  aoi21  g94(.a(new_n139_), .b(new_n113_), .c(x15), .O(new_n140_));
  nand4  g95(.a(new_n140_), .b(new_n137_), .c(new_n99_), .d(new_n95_), .O(z18));
endmodule


