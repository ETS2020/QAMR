// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x16), .O(new_n46_));
  nand2  g001(.a(x18), .b(x15), .O(new_n47_));
  nand3  g002(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(z00));
  nor3   g004(.a(x25), .b(x23), .c(x20), .O(new_n50_));
  oai21  g005(.a(new_n50_), .b(x15), .c(x18), .O(new_n51_));
  inv1   g006(.a(x24), .O(new_n52_));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  oai21  g009(.a(x19), .b(x17), .c(new_n54_), .O(new_n55_));
  inv1   g010(.a(x21), .O(new_n56_));
  inv1   g011(.a(x22), .O(new_n57_));
  nor2   g012(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g013(.a(new_n58_), .b(new_n55_), .c(x23), .O(new_n59_));
  oai21  g014(.a(new_n59_), .b(new_n52_), .c(new_n53_), .O(new_n60_));
  aoi21  g015(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  nand3  g016(.a(new_n61_), .b(new_n60_), .c(new_n51_), .O(z01));
  inv1   g017(.a(new_n47_), .O(new_n63_));
  nor2   g018(.a(new_n63_), .b(new_n46_), .O(z02));
  oai21  g019(.a(new_n58_), .b(x23), .c(x24), .O(new_n65_));
  nand2  g020(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  inv1   g021(.a(x23), .O(new_n67_));
  inv1   g022(.a(x18), .O(new_n68_));
  oai21  g023(.a(x19), .b(x17), .c(new_n68_), .O(new_n69_));
  oai21  g024(.a(new_n68_), .b(x15), .c(new_n69_), .O(new_n70_));
  nand3  g025(.a(new_n70_), .b(new_n67_), .c(new_n54_), .O(new_n71_));
  aoi21  g026(.a(new_n71_), .b(new_n66_), .c(x25), .O(z03));
  inv1   g027(.a(x08), .O(new_n73_));
  xnor2a g028(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g029(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g030(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g031(.a(x12), .b(x03), .O(new_n77_));
  nand4  g032(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand3  g033(.a(new_n78_), .b(new_n47_), .c(new_n73_), .O(new_n79_));
  inv1   g034(.a(new_n79_), .O(z04));
  inv1   g035(.a(x13), .O(new_n81_));
  nand3  g036(.a(new_n47_), .b(new_n81_), .c(new_n73_), .O(new_n82_));
  inv1   g037(.a(new_n82_), .O(z05));
  inv1   g038(.a(x14), .O(new_n84_));
  oai21  g039(.a(new_n84_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g040(.a(new_n73_), .b(x06), .c(new_n63_), .O(z07));
  nand3  g041(.a(x22), .b(x21), .c(x20), .O(new_n87_));
  aoi21  g042(.a(new_n87_), .b(new_n67_), .c(new_n52_), .O(new_n88_));
  oai21  g043(.a(new_n88_), .b(x25), .c(new_n47_), .O(new_n89_));
  nor3   g044(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  nand4  g045(.a(new_n90_), .b(x24), .c(x22), .d(x21), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(new_n89_), .O(z08));
  nand2  g047(.a(x05), .b(x04), .O(new_n93_));
  inv1   g048(.a(x07), .O(new_n94_));
  inv1   g049(.a(x15), .O(new_n95_));
  nand2  g050(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g051(.a(new_n96_), .b(new_n93_), .c(new_n47_), .O(z09));
  inv1   g052(.a(x17), .O(new_n98_));
  nand4  g053(.a(new_n93_), .b(new_n98_), .c(new_n95_), .d(new_n94_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(new_n47_), .O(z10));
  xor2a  g055(.a(x18), .b(x17), .O(new_n101_));
  nand4  g056(.a(new_n101_), .b(new_n93_), .c(new_n95_), .d(new_n94_), .O(new_n102_));
  inv1   g057(.a(new_n102_), .O(z11));
  nand2  g058(.a(x18), .b(x17), .O(new_n104_));
  nand3  g059(.a(new_n104_), .b(x19), .c(new_n95_), .O(new_n105_));
  inv1   g060(.a(x19), .O(new_n106_));
  nand3  g061(.a(new_n106_), .b(x18), .c(x17), .O(new_n107_));
  nand2  g062(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand3  g063(.a(new_n108_), .b(new_n93_), .c(new_n94_), .O(new_n109_));
  nand2  g064(.a(new_n109_), .b(new_n47_), .O(z12));
  nand3  g065(.a(x19), .b(x18), .c(x17), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(x20), .O(new_n112_));
  inv1   g067(.a(new_n104_), .O(new_n113_));
  nor2   g068(.a(x20), .b(new_n106_), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand4  g070(.a(new_n115_), .b(new_n112_), .c(new_n61_), .d(new_n95_), .O(z13));
  oai21  g071(.a(x21), .b(x15), .c(new_n68_), .O(new_n117_));
  nand3  g072(.a(new_n54_), .b(x19), .c(x17), .O(new_n118_));
  nor2   g073(.a(x21), .b(x20), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n113_), .c(x19), .O(new_n120_));
  nand3  g075(.a(new_n120_), .b(new_n93_), .c(new_n94_), .O(new_n121_));
  aoi21  g076(.a(new_n118_), .b(x21), .c(new_n121_), .O(new_n122_));
  oai21  g077(.a(new_n122_), .b(x15), .c(new_n117_), .O(z14));
  oai21  g078(.a(x22), .b(x15), .c(new_n68_), .O(new_n124_));
  nand3  g079(.a(new_n119_), .b(x19), .c(x17), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(x22), .O(new_n126_));
  inv1   g081(.a(new_n111_), .O(new_n127_));
  nor2   g082(.a(x22), .b(x21), .O(new_n128_));
  nand3  g083(.a(new_n128_), .b(new_n127_), .c(new_n54_), .O(new_n129_));
  nand4  g084(.a(new_n129_), .b(new_n126_), .c(new_n93_), .d(new_n94_), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(new_n95_), .O(new_n131_));
  nand2  g086(.a(new_n131_), .b(new_n124_), .O(z15));
  nand2  g087(.a(new_n129_), .b(x23), .O(new_n133_));
  nand4  g088(.a(new_n119_), .b(new_n127_), .c(new_n67_), .d(new_n57_), .O(new_n134_));
  nand4  g089(.a(new_n134_), .b(new_n133_), .c(new_n61_), .d(new_n95_), .O(z16));
  oai21  g090(.a(x24), .b(x15), .c(new_n68_), .O(new_n136_));
  nand3  g091(.a(new_n67_), .b(new_n57_), .c(new_n56_), .O(new_n137_));
  oai21  g092(.a(new_n137_), .b(new_n118_), .c(x24), .O(new_n138_));
  nor2   g093(.a(x24), .b(x23), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n128_), .c(new_n114_), .d(new_n113_), .O(new_n140_));
  nand4  g095(.a(new_n140_), .b(new_n138_), .c(new_n93_), .d(new_n94_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n95_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(new_n136_), .O(z17));
  oai21  g098(.a(x25), .b(x15), .c(new_n68_), .O(new_n144_));
  nand4  g099(.a(new_n52_), .b(new_n67_), .c(new_n57_), .d(new_n56_), .O(new_n145_));
  oai21  g100(.a(new_n145_), .b(new_n118_), .c(x25), .O(new_n146_));
  nor3   g101(.a(x25), .b(x24), .c(x23), .O(new_n147_));
  nand4  g102(.a(new_n147_), .b(new_n128_), .c(new_n114_), .d(new_n113_), .O(new_n148_));
  nand4  g103(.a(new_n148_), .b(new_n146_), .c(new_n93_), .d(new_n94_), .O(new_n149_));
  nand2  g104(.a(new_n149_), .b(new_n95_), .O(new_n150_));
  nand2  g105(.a(new_n150_), .b(new_n144_), .O(z18));
endmodule


