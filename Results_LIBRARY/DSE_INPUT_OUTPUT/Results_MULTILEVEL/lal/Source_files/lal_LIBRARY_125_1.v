// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n53_), .c(x07), .O(new_n62_));
  inv1   g17(.a(x04), .O(new_n63_));
  inv1   g18(.a(x05), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n62_), .b(new_n49_), .c(new_n66_), .O(z01));
  nand3  g22(.a(new_n61_), .b(new_n50_), .c(new_n53_), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n50_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n50_), .O(z06));
  nand3  g34(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g35(.a(x24), .O(new_n81_));
  inv1   g36(.a(x22), .O(new_n82_));
  aoi21  g37(.a(new_n58_), .b(new_n54_), .c(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x21), .c(x23), .O(new_n84_));
  nor2   g39(.a(new_n49_), .b(x25), .O(new_n85_));
  oai21  g40(.a(new_n84_), .b(new_n81_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  nand3  g42(.a(new_n65_), .b(new_n48_), .c(new_n87_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nor2   g44(.a(x17), .b(x07), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x15), .c(new_n63_), .O(new_n91_));
  nand4  g46(.a(new_n55_), .b(new_n48_), .c(new_n87_), .d(new_n64_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  oai21  g49(.a(x15), .b(x05), .c(x04), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n87_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n50_), .O(z11));
  nor2   g52(.a(new_n56_), .b(new_n55_), .O(new_n98_));
  nand3  g53(.a(new_n57_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n57_), .c(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n95_), .c(new_n87_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n50_), .O(z12));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nor2   g59(.a(new_n65_), .b(x07), .O(new_n105_));
  nor2   g60(.a(x20), .b(new_n57_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n98_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n105_), .c(new_n104_), .d(new_n48_), .O(z13));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n98_), .c(x19), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n87_), .O(new_n111_));
  aoi21  g66(.a(new_n107_), .b(x21), .c(new_n111_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(x15), .c(new_n95_), .O(z14));
  nand2  g68(.a(new_n110_), .b(x22), .O(new_n114_));
  inv1   g69(.a(new_n103_), .O(new_n115_));
  nor2   g70(.a(x22), .b(x21), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n54_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n114_), .c(new_n105_), .d(new_n48_), .O(z15));
  nand2  g73(.a(new_n117_), .b(x23), .O(new_n119_));
  inv1   g74(.a(x23), .O(new_n120_));
  nand4  g75(.a(new_n109_), .b(new_n115_), .c(new_n120_), .d(new_n82_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n119_), .c(new_n105_), .d(new_n48_), .O(z16));
  nand2  g77(.a(new_n121_), .b(x24), .O(new_n123_));
  nand3  g78(.a(new_n116_), .b(new_n81_), .c(new_n120_), .O(new_n124_));
  or2    g79(.a(new_n124_), .b(new_n107_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n123_), .c(new_n105_), .d(new_n48_), .O(z17));
  oai21  g81(.a(new_n124_), .b(new_n107_), .c(x25), .O(new_n127_));
  nor3   g82(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n116_), .c(new_n106_), .d(new_n98_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n127_), .c(new_n87_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n48_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n95_), .O(z18));
endmodule


