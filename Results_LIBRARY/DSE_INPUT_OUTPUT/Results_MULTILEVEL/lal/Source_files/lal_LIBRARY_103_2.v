// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x22), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x17), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  nor3   g06(.a(x25), .b(x23), .c(x20), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(new_n48_), .c(x17), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  oai21  g11(.a(x19), .b(x18), .c(new_n56_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  nor2   g13(.a(new_n48_), .b(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n57_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n54_), .c(new_n55_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(x07), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n61_), .c(new_n53_), .O(z01));
  nand2  g18(.a(new_n61_), .b(new_n53_), .O(z03));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g27(.a(new_n49_), .b(x14), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(x08), .O(z06));
  nand3  g29(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g30(.a(x23), .O(new_n76_));
  inv1   g31(.a(x17), .O(new_n77_));
  nor2   g32(.a(x19), .b(x18), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n77_), .c(x20), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n58_), .c(new_n76_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(x24), .c(x25), .O(new_n81_));
  oai21  g36(.a(new_n54_), .b(new_n76_), .c(new_n55_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  oai21  g38(.a(new_n81_), .b(new_n48_), .c(new_n83_), .O(z08));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n85_), .c(new_n49_), .O(z09));
  nand4  g44(.a(new_n85_), .b(new_n77_), .c(new_n87_), .d(new_n86_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  inv1   g46(.a(x18), .O(new_n92_));
  nand3  g47(.a(x22), .b(new_n92_), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(x17), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n85_), .c(new_n87_), .d(new_n86_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  inv1   g51(.a(x19), .O(new_n97_));
  aoi21  g52(.a(x22), .b(new_n92_), .c(new_n77_), .O(new_n98_));
  nand4  g53(.a(x22), .b(new_n97_), .c(x18), .d(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n85_), .c(new_n87_), .d(new_n86_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z12));
  nand3  g57(.a(new_n56_), .b(x19), .c(x18), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x22), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x17), .O(new_n105_));
  nand4  g60(.a(x22), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  nand3  g61(.a(new_n85_), .b(new_n87_), .c(new_n86_), .O(new_n107_));
  aoi21  g62(.a(new_n106_), .b(x20), .c(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n105_), .O(z13));
  nand2  g64(.a(new_n107_), .b(new_n49_), .O(new_n110_));
  nand2  g65(.a(new_n105_), .b(x21), .O(new_n111_));
  nand3  g66(.a(x22), .b(new_n58_), .c(new_n56_), .O(new_n112_));
  inv1   g67(.a(new_n112_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(x19), .c(x18), .d(x17), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n111_), .c(new_n110_), .O(z14));
  nand2  g70(.a(x19), .b(x18), .O(new_n116_));
  oai21  g71(.a(new_n112_), .b(new_n116_), .c(x17), .O(new_n117_));
  aoi21  g72(.a(x22), .b(new_n77_), .c(x15), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n62_), .O(z15));
  nand3  g74(.a(new_n85_), .b(new_n49_), .c(new_n86_), .O(new_n120_));
  inv1   g75(.a(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n76_), .c(new_n87_), .O(z16));
  nor2   g77(.a(x24), .b(x15), .O(new_n123_));
  aoi22  g78(.a(new_n123_), .b(new_n62_), .c(new_n48_), .d(x17), .O(z17));
  nand3  g79(.a(new_n121_), .b(new_n55_), .c(new_n87_), .O(z18));
endmodule


