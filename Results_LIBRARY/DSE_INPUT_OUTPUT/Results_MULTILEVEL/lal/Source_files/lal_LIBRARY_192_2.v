// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x17), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x19), .b(x18), .c(new_n53_), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n54_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n51_), .c(new_n52_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(new_n48_), .b(new_n60_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  and2   g18(.a(new_n48_), .b(x16), .O(z02));
  nand2  g19(.a(new_n59_), .b(new_n48_), .O(z03));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n48_), .O(z04));
  nor3   g27(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand2  g28(.a(new_n48_), .b(x14), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(x08), .O(z06));
  nand3  g30(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g31(.a(x23), .O(new_n77_));
  nand3  g32(.a(x22), .b(x21), .c(x20), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n77_), .c(new_n51_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x25), .c(new_n48_), .O(new_n80_));
  nor3   g35(.a(x19), .b(x18), .c(x17), .O(new_n81_));
  nand4  g36(.a(new_n81_), .b(x24), .c(x22), .d(x21), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n80_), .O(z08));
  nand2  g38(.a(x05), .b(x04), .O(new_n84_));
  inv1   g39(.a(x15), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n60_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n84_), .c(new_n48_), .O(z09));
  inv1   g42(.a(x17), .O(new_n88_));
  nand4  g43(.a(new_n84_), .b(new_n88_), .c(new_n85_), .d(new_n60_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n48_), .O(z10));
  nand4  g45(.a(new_n84_), .b(x18), .c(new_n88_), .d(new_n85_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(x07), .c(new_n48_), .O(z11));
  inv1   g47(.a(x19), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(x18), .c(x17), .O(new_n94_));
  oai21  g49(.a(new_n93_), .b(x17), .c(new_n94_), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n84_), .c(new_n85_), .d(new_n60_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z12));
  nor2   g52(.a(x20), .b(new_n93_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n47_), .c(x17), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand3  g55(.a(new_n84_), .b(new_n85_), .c(new_n60_), .O(new_n101_));
  aoi21  g56(.a(new_n100_), .b(x20), .c(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n99_), .O(z13));
  inv1   g58(.a(new_n101_), .O(new_n104_));
  nand3  g59(.a(new_n55_), .b(new_n53_), .c(x19), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n47_), .c(x17), .O(new_n107_));
  nand3  g62(.a(new_n98_), .b(x18), .c(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x21), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n104_), .O(z14));
  nand2  g65(.a(new_n101_), .b(new_n48_), .O(new_n111_));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n53_), .c(x19), .d(x17), .O(new_n113_));
  oai21  g68(.a(new_n106_), .b(new_n56_), .c(new_n113_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x18), .O(new_n115_));
  nand2  g70(.a(x22), .b(new_n88_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n111_), .O(z15));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n98_), .c(new_n55_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x18), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x17), .O(new_n121_));
  nand2  g76(.a(new_n112_), .b(new_n53_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n100_), .c(x23), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n104_), .O(z16));
  nand3  g79(.a(new_n51_), .b(new_n77_), .c(new_n56_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n105_), .c(x18), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(x17), .O(new_n127_));
  nand3  g82(.a(new_n118_), .b(new_n55_), .c(new_n53_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n100_), .c(x24), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n127_), .c(new_n104_), .O(z17));
  nand3  g85(.a(new_n118_), .b(new_n52_), .c(new_n51_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n105_), .c(x18), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(x17), .O(new_n133_));
  nand3  g88(.a(new_n112_), .b(new_n51_), .c(new_n77_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n108_), .c(x25), .O(new_n135_));
  nand3  g90(.a(new_n135_), .b(new_n133_), .c(new_n104_), .O(z18));
endmodule


