// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x22), .O(new_n47_));
  nor2   g002(.a(new_n47_), .b(x17), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x07), .O(new_n51_));
  nand2  g006(.a(x05), .b(x04), .O(new_n52_));
  inv1   g007(.a(x24), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  nand2  g009(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g010(.a(new_n55_), .b(new_n52_), .c(new_n51_), .O(new_n56_));
  nand2  g011(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  inv1   g012(.a(x23), .O(new_n58_));
  inv1   g013(.a(x20), .O(new_n59_));
  inv1   g014(.a(x21), .O(new_n60_));
  oai21  g015(.a(new_n60_), .b(new_n59_), .c(x17), .O(new_n61_));
  nand2  g016(.a(new_n61_), .b(x22), .O(new_n62_));
  nand3  g017(.a(new_n62_), .b(new_n54_), .c(new_n58_), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(new_n57_), .O(z01));
  inv1   g019(.a(x16), .O(new_n65_));
  nor2   g020(.a(new_n48_), .b(new_n65_), .O(z02));
  nand3  g021(.a(x22), .b(x21), .c(x20), .O(new_n67_));
  aoi21  g022(.a(new_n67_), .b(new_n58_), .c(new_n53_), .O(new_n68_));
  oai21  g023(.a(new_n68_), .b(x25), .c(new_n49_), .O(z03));
  xnor2a g024(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g025(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g026(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g027(.a(x12), .b(x03), .O(new_n73_));
  nand4  g028(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g029(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(new_n49_), .O(z04));
  nor3   g031(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g032(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n49_), .O(z06));
  aoi21  g034(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand3  g035(.a(x24), .b(x21), .c(x20), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(x17), .O(new_n82_));
  nand2  g037(.a(new_n82_), .b(x22), .O(new_n83_));
  aoi21  g038(.a(x24), .b(x23), .c(x25), .O(new_n84_));
  nand2  g039(.a(new_n84_), .b(new_n83_), .O(z08));
  inv1   g040(.a(x04), .O(new_n86_));
  inv1   g041(.a(x15), .O(new_n87_));
  nand4  g042(.a(new_n49_), .b(new_n87_), .c(new_n51_), .d(x05), .O(new_n88_));
  nor2   g043(.a(new_n88_), .b(new_n86_), .O(z09));
  nand3  g044(.a(new_n52_), .b(new_n87_), .c(new_n51_), .O(new_n90_));
  aoi21  g045(.a(new_n90_), .b(new_n47_), .c(x17), .O(z10));
  inv1   g046(.a(x17), .O(new_n92_));
  nand3  g047(.a(new_n47_), .b(x18), .c(new_n92_), .O(new_n93_));
  oai21  g048(.a(x18), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand4  g049(.a(new_n94_), .b(new_n52_), .c(new_n87_), .d(new_n51_), .O(new_n95_));
  inv1   g050(.a(new_n95_), .O(z11));
  nand2  g051(.a(x18), .b(x17), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(x19), .O(new_n98_));
  inv1   g053(.a(x19), .O(new_n99_));
  nand3  g054(.a(new_n99_), .b(x18), .c(x17), .O(new_n100_));
  nand2  g055(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand4  g056(.a(new_n101_), .b(new_n52_), .c(new_n87_), .d(new_n51_), .O(new_n102_));
  nand2  g057(.a(new_n102_), .b(new_n49_), .O(z12));
  oai21  g058(.a(x22), .b(x20), .c(new_n92_), .O(new_n104_));
  nand2  g059(.a(x19), .b(x18), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(x20), .O(new_n106_));
  aoi21  g061(.a(x05), .b(x04), .c(x07), .O(new_n107_));
  nand4  g062(.a(new_n59_), .b(x19), .c(x18), .d(x17), .O(new_n108_));
  inv1   g063(.a(new_n108_), .O(new_n109_));
  nor2   g064(.a(new_n109_), .b(x15), .O(new_n110_));
  nand4  g065(.a(new_n110_), .b(new_n107_), .c(new_n106_), .d(new_n104_), .O(z13));
  nor2   g066(.a(x22), .b(x21), .O(new_n112_));
  nand3  g067(.a(new_n52_), .b(new_n87_), .c(new_n51_), .O(new_n113_));
  oai21  g068(.a(new_n112_), .b(x17), .c(new_n113_), .O(new_n114_));
  nand3  g069(.a(new_n59_), .b(x19), .c(x18), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(x21), .O(new_n116_));
  nor2   g071(.a(x21), .b(x20), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(x19), .c(x18), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(x17), .O(new_n120_));
  nand3  g075(.a(new_n47_), .b(x21), .c(new_n92_), .O(new_n121_));
  nand3  g076(.a(new_n121_), .b(new_n120_), .c(new_n114_), .O(z14));
  nand2  g077(.a(new_n117_), .b(x19), .O(new_n123_));
  oai21  g078(.a(new_n123_), .b(new_n97_), .c(x22), .O(new_n124_));
  nand3  g079(.a(x19), .b(x18), .c(x17), .O(new_n125_));
  inv1   g080(.a(new_n125_), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n112_), .c(new_n59_), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n124_), .c(new_n107_), .d(new_n87_), .O(z15));
  oai21  g083(.a(x23), .b(x22), .c(new_n92_), .O(new_n129_));
  nand2  g084(.a(new_n112_), .b(new_n59_), .O(new_n130_));
  oai21  g085(.a(new_n130_), .b(new_n105_), .c(x23), .O(new_n131_));
  nor2   g086(.a(x23), .b(x22), .O(new_n132_));
  nand3  g087(.a(new_n132_), .b(new_n126_), .c(new_n117_), .O(new_n133_));
  nand3  g088(.a(new_n133_), .b(new_n107_), .c(new_n87_), .O(new_n134_));
  inv1   g089(.a(new_n134_), .O(new_n135_));
  nand3  g090(.a(new_n135_), .b(new_n131_), .c(new_n129_), .O(z16));
  oai21  g091(.a(x24), .b(x22), .c(new_n92_), .O(new_n137_));
  nand2  g092(.a(new_n132_), .b(new_n60_), .O(new_n138_));
  oai21  g093(.a(new_n138_), .b(new_n115_), .c(x24), .O(new_n139_));
  nand4  g094(.a(new_n53_), .b(new_n58_), .c(new_n47_), .d(new_n60_), .O(new_n140_));
  inv1   g095(.a(new_n140_), .O(new_n141_));
  aoi21  g096(.a(new_n141_), .b(new_n109_), .c(x15), .O(new_n142_));
  nand4  g097(.a(new_n142_), .b(new_n139_), .c(new_n137_), .d(new_n107_), .O(z17));
  oai21  g098(.a(x25), .b(x22), .c(new_n92_), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(new_n113_), .O(new_n145_));
  nand2  g100(.a(x25), .b(new_n92_), .O(new_n146_));
  nand4  g101(.a(new_n54_), .b(new_n53_), .c(new_n58_), .d(new_n60_), .O(new_n147_));
  oai21  g102(.a(new_n147_), .b(new_n108_), .c(new_n146_), .O(new_n148_));
  nand2  g103(.a(new_n148_), .b(new_n47_), .O(new_n149_));
  inv1   g104(.a(new_n115_), .O(new_n150_));
  nor2   g105(.a(x24), .b(x23), .O(new_n151_));
  nand3  g106(.a(new_n151_), .b(new_n150_), .c(new_n112_), .O(new_n152_));
  nand3  g107(.a(new_n152_), .b(x25), .c(x17), .O(new_n153_));
  nand3  g108(.a(new_n153_), .b(new_n149_), .c(new_n145_), .O(z18));
endmodule


