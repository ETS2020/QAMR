// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x18), .O(new_n47_));
  nor2   g002(.a(x19), .b(new_n47_), .O(new_n48_));
  nor3   g003(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g004(.a(new_n48_), .O(new_n50_));
  inv1   g005(.a(x21), .O(new_n51_));
  inv1   g006(.a(x22), .O(new_n52_));
  inv1   g007(.a(x23), .O(new_n53_));
  oai21  g008(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  aoi21  g009(.a(new_n54_), .b(x24), .c(x25), .O(new_n55_));
  inv1   g010(.a(x07), .O(new_n56_));
  nand2  g011(.a(x05), .b(x04), .O(new_n57_));
  nand2  g012(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(new_n55_), .c(new_n50_), .O(new_n59_));
  inv1   g014(.a(x20), .O(new_n60_));
  inv1   g015(.a(x19), .O(new_n61_));
  nand2  g016(.a(new_n47_), .b(x17), .O(new_n62_));
  aoi21  g017(.a(new_n62_), .b(new_n61_), .c(x25), .O(new_n63_));
  nand3  g018(.a(new_n63_), .b(new_n53_), .c(new_n60_), .O(new_n64_));
  nand2  g019(.a(new_n64_), .b(new_n59_), .O(z01));
  inv1   g020(.a(x16), .O(new_n66_));
  nor2   g021(.a(new_n48_), .b(new_n66_), .O(z02));
  aoi21  g022(.a(new_n62_), .b(new_n61_), .c(x20), .O(new_n68_));
  aoi22  g023(.a(x22), .b(x21), .c(new_n61_), .d(x18), .O(new_n69_));
  oai21  g024(.a(new_n69_), .b(new_n68_), .c(new_n53_), .O(new_n70_));
  inv1   g025(.a(x24), .O(new_n71_));
  nand2  g026(.a(new_n50_), .b(new_n71_), .O(new_n72_));
  aoi21  g027(.a(new_n72_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g028(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g029(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g030(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g031(.a(x12), .b(x03), .O(new_n77_));
  nand4  g032(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand3  g033(.a(new_n78_), .b(new_n50_), .c(new_n46_), .O(new_n79_));
  inv1   g034(.a(new_n79_), .O(z04));
  oai21  g035(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g036(.a(x14), .b(new_n46_), .O(new_n82_));
  nand2  g037(.a(new_n82_), .b(new_n50_), .O(z06));
  nand3  g038(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g039(.a(x22), .b(x21), .c(x20), .O(new_n85_));
  aoi21  g040(.a(new_n85_), .b(new_n53_), .c(new_n71_), .O(new_n86_));
  nor2   g041(.a(new_n86_), .b(x25), .O(new_n87_));
  nor3   g042(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  nand4  g043(.a(new_n88_), .b(x24), .c(x22), .d(x21), .O(new_n89_));
  oai21  g044(.a(new_n87_), .b(new_n48_), .c(new_n89_), .O(z08));
  inv1   g045(.a(x15), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(new_n56_), .O(new_n92_));
  oai21  g047(.a(new_n92_), .b(new_n57_), .c(new_n50_), .O(z09));
  inv1   g048(.a(x17), .O(new_n94_));
  nand4  g049(.a(new_n57_), .b(new_n50_), .c(new_n94_), .d(new_n91_), .O(new_n95_));
  nor2   g050(.a(new_n95_), .b(x07), .O(z10));
  nand2  g051(.a(x18), .b(new_n94_), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(new_n62_), .O(new_n98_));
  nand4  g053(.a(new_n98_), .b(new_n57_), .c(new_n91_), .d(new_n56_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(new_n50_), .O(z11));
  nor2   g055(.a(new_n47_), .b(new_n94_), .O(new_n101_));
  inv1   g056(.a(new_n101_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n57_), .c(x19), .d(new_n91_), .O(new_n103_));
  oai21  g058(.a(new_n103_), .b(x07), .c(new_n50_), .O(z12));
  nand3  g059(.a(new_n57_), .b(new_n91_), .c(new_n56_), .O(new_n105_));
  nand2  g060(.a(new_n105_), .b(new_n50_), .O(new_n106_));
  nand3  g061(.a(new_n60_), .b(x18), .c(x17), .O(new_n107_));
  oai21  g062(.a(new_n60_), .b(x17), .c(new_n107_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(x19), .O(new_n109_));
  nand2  g064(.a(x20), .b(new_n47_), .O(new_n110_));
  nand3  g065(.a(new_n110_), .b(new_n109_), .c(new_n106_), .O(z13));
  nand2  g066(.a(new_n60_), .b(x17), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(x21), .O(new_n113_));
  nor2   g068(.a(x21), .b(x20), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n101_), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(x19), .O(new_n117_));
  nand2  g072(.a(x21), .b(new_n47_), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n117_), .c(new_n106_), .O(z14));
  inv1   g074(.a(new_n105_), .O(new_n120_));
  nor2   g075(.a(x22), .b(x21), .O(new_n121_));
  nand3  g076(.a(new_n121_), .b(new_n60_), .c(x17), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(x19), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(x18), .O(new_n124_));
  nand3  g079(.a(new_n114_), .b(new_n101_), .c(x19), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(x22), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n124_), .c(new_n120_), .O(z15));
  nor2   g082(.a(x23), .b(x22), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n51_), .O(new_n129_));
  oai21  g084(.a(new_n129_), .b(new_n112_), .c(x19), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(x18), .O(new_n131_));
  nand3  g086(.a(x19), .b(x18), .c(x17), .O(new_n132_));
  inv1   g087(.a(new_n132_), .O(new_n133_));
  nand3  g088(.a(new_n133_), .b(new_n121_), .c(new_n60_), .O(new_n134_));
  aoi21  g089(.a(new_n134_), .b(x23), .c(new_n105_), .O(new_n135_));
  nand2  g090(.a(new_n135_), .b(new_n131_), .O(z16));
  nand2  g091(.a(new_n114_), .b(x17), .O(new_n137_));
  nand3  g092(.a(new_n71_), .b(new_n53_), .c(new_n52_), .O(new_n138_));
  oai21  g093(.a(new_n138_), .b(new_n137_), .c(x19), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(x18), .O(new_n140_));
  nand3  g095(.a(new_n133_), .b(new_n128_), .c(new_n114_), .O(new_n141_));
  aoi21  g096(.a(new_n141_), .b(x24), .c(new_n105_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(new_n140_), .O(z17));
  nor2   g098(.a(x25), .b(x24), .O(new_n144_));
  nand2  g099(.a(new_n144_), .b(new_n128_), .O(new_n145_));
  oai21  g100(.a(new_n145_), .b(new_n137_), .c(x19), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(x18), .O(new_n147_));
  nand3  g102(.a(new_n101_), .b(new_n60_), .c(x19), .O(new_n148_));
  nand3  g103(.a(new_n121_), .b(new_n71_), .c(new_n53_), .O(new_n149_));
  oai21  g104(.a(new_n149_), .b(new_n148_), .c(x25), .O(new_n150_));
  nand3  g105(.a(new_n150_), .b(new_n147_), .c(new_n120_), .O(z18));
endmodule


