// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x02), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n48_), .b(new_n62_), .O(z02));
  inv1   g18(.a(new_n48_), .O(new_n64_));
  nand3  g19(.a(new_n58_), .b(new_n64_), .c(new_n50_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  aoi21  g21(.a(x11), .b(new_n46_), .c(x15), .O(new_n67_));
  inv1   g22(.a(x09), .O(new_n68_));
  inv1   g23(.a(x01), .O(new_n69_));
  aoi22  g24(.a(x10), .b(new_n69_), .c(new_n68_), .d(x00), .O(new_n70_));
  oai21  g25(.a(new_n68_), .b(x00), .c(new_n70_), .O(new_n71_));
  inv1   g26(.a(x10), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x01), .O(new_n73_));
  inv1   g28(.a(x11), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(x02), .O(new_n75_));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n75_), .c(new_n73_), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(new_n71_), .c(new_n46_), .O(new_n78_));
  oai21  g33(.a(new_n67_), .b(x02), .c(new_n78_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n64_), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  nor3   g36(.a(new_n48_), .b(new_n81_), .c(x08), .O(z06));
  nand3  g37(.a(new_n64_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g38(.a(x23), .O(new_n84_));
  nand2  g39(.a(new_n55_), .b(new_n51_), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(x22), .c(x21), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x24), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(new_n50_), .c(new_n48_), .O(z08));
  nand2  g44(.a(x05), .b(x04), .O(new_n90_));
  inv1   g45(.a(x07), .O(new_n91_));
  nand2  g46(.a(new_n47_), .b(new_n91_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n90_), .c(new_n64_), .O(z09));
  nand4  g48(.a(new_n90_), .b(new_n52_), .c(new_n47_), .d(new_n91_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n64_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n90_), .c(new_n47_), .d(new_n91_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n64_), .O(z11));
  nor2   g53(.a(new_n53_), .b(new_n52_), .O(new_n99_));
  nand3  g54(.a(new_n54_), .b(x18), .c(x17), .O(new_n100_));
  oai21  g55(.a(new_n99_), .b(new_n54_), .c(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n90_), .c(new_n47_), .d(new_n91_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n64_), .O(z12));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n99_), .b(new_n51_), .c(x19), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n90_), .d(new_n91_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(new_n108_));
  nand2  g63(.a(x15), .b(x02), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(z13));
  nand2  g65(.a(new_n106_), .b(x21), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n99_), .c(x19), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n60_), .d(new_n47_), .O(z14));
  inv1   g69(.a(new_n104_), .O(new_n115_));
  nor3   g70(.a(x22), .b(x21), .c(x20), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n90_), .c(new_n91_), .O(new_n118_));
  aoi21  g73(.a(new_n113_), .b(x22), .c(new_n118_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x15), .c(new_n109_), .O(z15));
  aoi21  g75(.a(new_n116_), .b(new_n115_), .c(new_n84_), .O(new_n121_));
  nor2   g76(.a(x23), .b(x22), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n112_), .c(new_n115_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n90_), .c(new_n91_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n121_), .c(new_n47_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n109_), .O(z16));
  nand2  g81(.a(new_n123_), .b(x24), .O(new_n127_));
  inv1   g82(.a(new_n106_), .O(new_n128_));
  nor2   g83(.a(x22), .b(x21), .O(new_n129_));
  nor2   g84(.a(x24), .b(x23), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n129_), .c(new_n128_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n127_), .c(new_n60_), .d(new_n47_), .O(z17));
  nand2  g87(.a(new_n131_), .b(x25), .O(new_n133_));
  nor3   g88(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n129_), .c(new_n128_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n133_), .c(new_n60_), .d(new_n47_), .O(z18));
endmodule


