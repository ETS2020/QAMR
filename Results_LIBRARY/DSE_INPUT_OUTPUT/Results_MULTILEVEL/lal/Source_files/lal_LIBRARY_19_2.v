// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x17), .O(new_n47_));
  nor2   g02(.a(x22), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n52_), .c(new_n51_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x21), .O(new_n61_));
  inv1   g16(.a(x18), .O(new_n62_));
  inv1   g17(.a(x19), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n47_), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n60_), .c(new_n61_), .O(new_n65_));
  nand2  g20(.a(new_n59_), .b(new_n47_), .O(new_n66_));
  oai21  g21(.a(new_n65_), .b(new_n59_), .c(new_n66_), .O(new_n67_));
  nand3  g22(.a(new_n67_), .b(new_n54_), .c(new_n58_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n57_), .O(z01));
  inv1   g24(.a(x16), .O(new_n70_));
  nor2   g25(.a(new_n48_), .b(new_n70_), .O(z02));
  nor3   g26(.a(x25), .b(x23), .c(x20), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(new_n59_), .c(x17), .O(new_n73_));
  oai21  g28(.a(x19), .b(x18), .c(new_n60_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(x22), .c(x21), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n58_), .c(new_n53_), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(x25), .c(new_n73_), .O(z03));
  xnor2a g32(.a(x09), .b(x00), .O(new_n78_));
  xnor2a g33(.a(x10), .b(x01), .O(new_n79_));
  xnor2a g34(.a(x11), .b(x02), .O(new_n80_));
  xnor2a g35(.a(x12), .b(x03), .O(new_n81_));
  nand4  g36(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n78_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n49_), .O(z04));
  nor3   g39(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g40(.a(x14), .b(new_n46_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n49_), .O(z06));
  aoi21  g42(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g43(.a(new_n64_), .b(new_n60_), .c(new_n59_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x21), .c(x23), .O(new_n90_));
  nor2   g45(.a(new_n48_), .b(x25), .O(new_n91_));
  oai21  g46(.a(new_n90_), .b(new_n53_), .c(new_n91_), .O(z08));
  inv1   g47(.a(x04), .O(new_n93_));
  inv1   g48(.a(x15), .O(new_n94_));
  nand4  g49(.a(new_n49_), .b(new_n94_), .c(new_n51_), .d(x05), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n93_), .O(z09));
  nand4  g51(.a(new_n52_), .b(new_n47_), .c(new_n94_), .d(new_n51_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z10));
  nand3  g53(.a(x22), .b(new_n62_), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n62_), .b(x17), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n52_), .c(new_n94_), .d(new_n51_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z11));
  oai21  g57(.a(new_n62_), .b(new_n47_), .c(x19), .O(new_n103_));
  nand3  g58(.a(new_n63_), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n52_), .c(new_n94_), .d(new_n51_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n49_), .O(z12));
  nand3  g62(.a(new_n60_), .b(x19), .c(x18), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x22), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x17), .O(new_n110_));
  nand4  g65(.a(x22), .b(x19), .c(x18), .d(x17), .O(new_n111_));
  nand3  g66(.a(new_n52_), .b(new_n94_), .c(new_n51_), .O(new_n112_));
  aoi21  g67(.a(new_n111_), .b(x20), .c(new_n112_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n110_), .O(z13));
  nand2  g69(.a(new_n112_), .b(new_n49_), .O(new_n115_));
  nand2  g70(.a(new_n110_), .b(x21), .O(new_n116_));
  nand3  g71(.a(x22), .b(new_n61_), .c(new_n60_), .O(new_n117_));
  inv1   g72(.a(new_n117_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(x19), .c(x18), .d(x17), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n116_), .c(new_n115_), .O(z14));
  nand2  g75(.a(x19), .b(x18), .O(new_n121_));
  oai21  g76(.a(new_n117_), .b(new_n121_), .c(x17), .O(new_n122_));
  aoi21  g77(.a(x05), .b(x04), .c(x07), .O(new_n123_));
  aoi21  g78(.a(x22), .b(new_n47_), .c(x15), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(z15));
  nand3  g80(.a(new_n52_), .b(new_n49_), .c(new_n51_), .O(new_n126_));
  inv1   g81(.a(new_n126_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n58_), .c(new_n94_), .O(z16));
  nand3  g83(.a(new_n127_), .b(new_n53_), .c(new_n94_), .O(z17));
  nor2   g84(.a(x25), .b(x15), .O(new_n130_));
  aoi21  g85(.a(new_n130_), .b(new_n123_), .c(new_n48_), .O(z18));
endmodule


