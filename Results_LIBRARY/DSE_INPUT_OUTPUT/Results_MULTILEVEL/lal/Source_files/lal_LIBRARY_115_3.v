// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n49_), .c(new_n50_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n61_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n47_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n61_), .b(new_n47_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n47_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g29(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z06));
  nand3  g31(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g32(.a(new_n55_), .b(new_n51_), .c(new_n58_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(x21), .c(x23), .O(new_n79_));
  aoi21  g34(.a(x15), .b(x07), .c(x25), .O(new_n80_));
  oai21  g35(.a(new_n79_), .b(new_n49_), .c(new_n80_), .O(z08));
  nand2  g36(.a(x05), .b(x04), .O(new_n82_));
  inv1   g37(.a(x07), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n82_), .c(new_n47_), .O(z09));
  nand4  g41(.a(new_n82_), .b(new_n52_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n82_), .c(new_n84_), .d(new_n83_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n47_), .O(z11));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand3  g48(.a(new_n54_), .b(x18), .c(x17), .O(new_n94_));
  oai21  g49(.a(new_n93_), .b(new_n54_), .c(new_n94_), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n82_), .c(new_n84_), .d(new_n83_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z12));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x20), .O(new_n99_));
  nand4  g54(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n99_), .c(new_n62_), .d(new_n84_), .O(z13));
  nor2   g56(.a(x20), .b(new_n54_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n93_), .c(new_n57_), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n93_), .c(x19), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n82_), .c(new_n84_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n103_), .c(new_n83_), .O(new_n107_));
  nand2  g62(.a(new_n84_), .b(x07), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(z14));
  nand3  g64(.a(new_n57_), .b(new_n51_), .c(x19), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n92_), .c(x22), .O(new_n111_));
  inv1   g66(.a(new_n98_), .O(new_n112_));
  nor3   g67(.a(x22), .b(x21), .c(x20), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n111_), .c(new_n82_), .d(new_n84_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n83_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n108_), .O(z15));
  inv1   g72(.a(x23), .O(new_n118_));
  aoi21  g73(.a(new_n113_), .b(new_n112_), .c(new_n118_), .O(new_n119_));
  nand4  g74(.a(new_n104_), .b(new_n112_), .c(new_n118_), .d(new_n58_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n82_), .c(new_n84_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n119_), .c(new_n83_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n108_), .O(z16));
  nand4  g78(.a(new_n118_), .b(new_n58_), .c(new_n57_), .d(new_n51_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n98_), .c(x24), .O(new_n125_));
  nor2   g80(.a(x22), .b(x21), .O(new_n126_));
  nor2   g81(.a(x24), .b(x23), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n126_), .c(new_n102_), .d(new_n93_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n125_), .c(new_n82_), .d(new_n84_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n83_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n108_), .O(z17));
  nand4  g86(.a(new_n49_), .b(new_n118_), .c(new_n58_), .d(new_n57_), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(new_n100_), .c(x25), .O(new_n133_));
  nor3   g88(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n126_), .c(new_n102_), .d(new_n93_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n133_), .c(new_n82_), .d(new_n84_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n83_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n108_), .O(z18));
endmodule


