// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x20), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x21), .O(new_n51_));
  inv1   g06(.a(x22), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  oai21  g08(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(x24), .c(x25), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n55_), .c(new_n49_), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x25), .O(new_n61_));
  inv1   g16(.a(x17), .O(new_n62_));
  inv1   g17(.a(x18), .O(new_n63_));
  inv1   g18(.a(x19), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  and2   g20(.a(new_n65_), .b(new_n61_), .O(new_n66_));
  nand4  g21(.a(new_n66_), .b(new_n53_), .c(new_n60_), .d(new_n47_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n59_), .O(z01));
  inv1   g23(.a(x16), .O(new_n69_));
  nor2   g24(.a(new_n48_), .b(new_n69_), .O(z02));
  inv1   g25(.a(x24), .O(new_n71_));
  nor2   g26(.a(new_n52_), .b(new_n51_), .O(new_n72_));
  nand2  g27(.a(new_n65_), .b(new_n60_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n72_), .c(x23), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(new_n71_), .c(new_n61_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z03));
  xnor2a g31(.a(x09), .b(x00), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  xnor2a g33(.a(x11), .b(x02), .O(new_n79_));
  xnor2a g34(.a(x12), .b(x03), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n77_), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n49_), .c(new_n46_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z04));
  nor3   g38(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor3   g40(.a(new_n48_), .b(new_n85_), .c(x08), .O(z06));
  aoi21  g41(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g42(.a(new_n65_), .b(new_n60_), .c(new_n52_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x21), .c(x23), .O(new_n89_));
  nor2   g44(.a(new_n48_), .b(x25), .O(new_n90_));
  oai21  g45(.a(new_n89_), .b(new_n71_), .c(new_n90_), .O(z08));
  nand4  g46(.a(new_n47_), .b(new_n56_), .c(x05), .d(x04), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand4  g48(.a(new_n57_), .b(new_n62_), .c(new_n47_), .d(new_n56_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n49_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n57_), .c(new_n47_), .d(new_n56_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nor2   g53(.a(new_n63_), .b(new_n62_), .O(new_n99_));
  nand3  g54(.a(new_n64_), .b(x18), .c(x17), .O(new_n100_));
  oai21  g55(.a(new_n99_), .b(new_n64_), .c(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n57_), .c(new_n47_), .d(new_n56_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z12));
  nand2  g58(.a(new_n58_), .b(new_n49_), .O(new_n104_));
  nand2  g59(.a(x17), .b(new_n47_), .O(new_n105_));
  nand2  g60(.a(x19), .b(x18), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n105_), .c(x20), .O(new_n107_));
  nor2   g62(.a(x20), .b(new_n64_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(x18), .c(x17), .d(new_n47_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n104_), .O(z13));
  oai21  g65(.a(x21), .b(x15), .c(x20), .O(new_n111_));
  nand3  g66(.a(x19), .b(x18), .c(x17), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n99_), .c(x19), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n57_), .d(new_n56_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n47_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n111_), .O(z14));
  inv1   g73(.a(new_n58_), .O(new_n119_));
  nand2  g74(.a(new_n115_), .b(x22), .O(new_n120_));
  inv1   g75(.a(new_n112_), .O(new_n121_));
  nor2   g76(.a(x22), .b(x21), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n60_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n120_), .c(new_n119_), .d(new_n47_), .O(z15));
  oai21  g79(.a(x23), .b(x15), .c(x20), .O(new_n125_));
  nand3  g80(.a(new_n52_), .b(new_n51_), .c(x19), .O(new_n126_));
  inv1   g81(.a(new_n126_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n99_), .c(new_n53_), .O(new_n128_));
  nand4  g83(.a(new_n114_), .b(new_n121_), .c(new_n53_), .d(new_n52_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n57_), .c(new_n56_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n128_), .c(new_n47_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n125_), .O(z16));
  oai21  g87(.a(x24), .b(x15), .c(x20), .O(new_n133_));
  nand3  g88(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n112_), .c(x24), .O(new_n135_));
  nor2   g90(.a(x24), .b(x23), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n122_), .c(new_n108_), .d(new_n99_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n57_), .d(new_n56_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n133_), .O(z17));
  nand2  g95(.a(new_n137_), .b(x25), .O(new_n141_));
  nor3   g96(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  nand4  g97(.a(new_n142_), .b(new_n122_), .c(new_n108_), .d(new_n99_), .O(new_n143_));
  nand4  g98(.a(new_n143_), .b(new_n141_), .c(new_n119_), .d(new_n47_), .O(z18));
endmodule


