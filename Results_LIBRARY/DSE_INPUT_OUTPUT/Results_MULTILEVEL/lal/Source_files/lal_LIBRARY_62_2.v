// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x17), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor3   g16(.a(new_n61_), .b(new_n48_), .c(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n48_), .b(new_n64_), .O(z02));
  inv1   g20(.a(new_n48_), .O(new_n66_));
  nand2  g21(.a(new_n59_), .b(new_n66_), .O(z03));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand3  g27(.a(new_n72_), .b(new_n66_), .c(new_n46_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n66_), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n66_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g33(.a(x23), .O(new_n79_));
  inv1   g34(.a(x24), .O(new_n80_));
  nand3  g35(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n79_), .c(new_n80_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(x25), .O(new_n83_));
  nor3   g38(.a(x18), .b(x17), .c(x15), .O(new_n84_));
  inv1   g39(.a(x22), .O(new_n85_));
  nor2   g40(.a(new_n80_), .b(new_n85_), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n84_), .c(x21), .d(new_n54_), .O(new_n87_));
  oai21  g42(.a(new_n83_), .b(new_n48_), .c(new_n87_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n61_), .b(new_n47_), .c(new_n89_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand4  g46(.a(new_n60_), .b(new_n52_), .c(new_n47_), .d(new_n89_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n60_), .c(new_n47_), .d(new_n89_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x19), .O(new_n98_));
  nand3  g53(.a(new_n54_), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n60_), .c(new_n47_), .d(new_n89_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z12));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n60_), .b(new_n89_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n104_), .d(new_n47_), .O(z13));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(x19), .c(x18), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n47_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x17), .O(new_n112_));
  aoi21  g67(.a(new_n107_), .b(x21), .c(new_n105_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(x15), .c(new_n112_), .O(z14));
  nand2  g69(.a(new_n109_), .b(x19), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n97_), .c(x22), .O(new_n116_));
  inv1   g71(.a(x21), .O(new_n117_));
  inv1   g72(.a(new_n103_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n85_), .c(new_n117_), .d(new_n51_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n116_), .c(new_n106_), .d(new_n47_), .O(z15));
  nand3  g75(.a(new_n51_), .b(x19), .c(x18), .O(new_n121_));
  nor2   g76(.a(x23), .b(x22), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n117_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n121_), .c(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x17), .O(new_n125_));
  aoi21  g80(.a(new_n119_), .b(x23), .c(new_n105_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(x15), .c(new_n125_), .O(z16));
  nand4  g82(.a(new_n80_), .b(new_n79_), .c(new_n85_), .d(new_n117_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n121_), .c(new_n47_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x17), .O(new_n130_));
  nand3  g85(.a(new_n122_), .b(new_n109_), .c(new_n118_), .O(new_n131_));
  aoi21  g86(.a(new_n131_), .b(x24), .c(new_n105_), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(x15), .c(new_n130_), .O(z17));
  nand3  g88(.a(new_n122_), .b(new_n50_), .c(new_n80_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n110_), .c(new_n47_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(x17), .O(new_n136_));
  oai21  g91(.a(new_n128_), .b(new_n107_), .c(x25), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n106_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n136_), .O(z18));
endmodule


