// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x08), .O(new_n46_));
  nand2  g001(.a(x18), .b(x15), .O(new_n47_));
  oai21  g002(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  nor3   g003(.a(x25), .b(x23), .c(x20), .O(new_n49_));
  oai21  g004(.a(new_n49_), .b(x15), .c(x18), .O(new_n50_));
  inv1   g005(.a(x24), .O(new_n51_));
  inv1   g006(.a(x25), .O(new_n52_));
  inv1   g007(.a(x20), .O(new_n53_));
  oai21  g008(.a(x19), .b(x17), .c(new_n53_), .O(new_n54_));
  inv1   g009(.a(x21), .O(new_n55_));
  inv1   g010(.a(x22), .O(new_n56_));
  nor2   g011(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g012(.a(new_n57_), .b(new_n54_), .c(x23), .O(new_n58_));
  oai21  g013(.a(new_n58_), .b(new_n51_), .c(new_n52_), .O(new_n59_));
  aoi21  g014(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g015(.a(new_n60_), .b(new_n59_), .c(new_n50_), .O(z01));
  inv1   g016(.a(x16), .O(new_n62_));
  nand2  g017(.a(new_n47_), .b(new_n62_), .O(z02));
  oai21  g018(.a(new_n57_), .b(x23), .c(x24), .O(new_n64_));
  nand2  g019(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  inv1   g020(.a(x23), .O(new_n66_));
  inv1   g021(.a(x18), .O(new_n67_));
  oai21  g022(.a(x19), .b(x17), .c(new_n67_), .O(new_n68_));
  oai21  g023(.a(new_n67_), .b(x15), .c(new_n68_), .O(new_n69_));
  nand3  g024(.a(new_n69_), .b(new_n66_), .c(new_n53_), .O(new_n70_));
  aoi21  g025(.a(new_n70_), .b(new_n65_), .c(x25), .O(z03));
  xnor2a g026(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g027(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g028(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g029(.a(x12), .b(x03), .O(new_n75_));
  nand4  g030(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g031(.a(new_n76_), .b(new_n47_), .c(new_n46_), .O(new_n77_));
  inv1   g032(.a(new_n77_), .O(z04));
  oai21  g033(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g034(.a(x14), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g036(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g037(.a(x17), .O(new_n83_));
  inv1   g038(.a(x19), .O(new_n84_));
  nand3  g039(.a(new_n84_), .b(new_n67_), .c(new_n83_), .O(new_n85_));
  aoi21  g040(.a(new_n85_), .b(new_n53_), .c(new_n56_), .O(new_n86_));
  aoi21  g041(.a(new_n86_), .b(x21), .c(x23), .O(new_n87_));
  aoi21  g042(.a(x18), .b(x15), .c(x25), .O(new_n88_));
  oai21  g043(.a(new_n87_), .b(new_n51_), .c(new_n88_), .O(z08));
  nand2  g044(.a(x05), .b(x04), .O(new_n90_));
  inv1   g045(.a(x07), .O(new_n91_));
  inv1   g046(.a(x15), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g048(.a(new_n93_), .b(new_n90_), .c(new_n47_), .O(z09));
  nand4  g049(.a(new_n90_), .b(new_n83_), .c(new_n92_), .d(new_n91_), .O(new_n95_));
  inv1   g050(.a(new_n95_), .O(z10));
  nand3  g051(.a(new_n67_), .b(x17), .c(new_n92_), .O(new_n97_));
  oai21  g052(.a(new_n67_), .b(x17), .c(new_n97_), .O(new_n98_));
  nand3  g053(.a(new_n98_), .b(new_n90_), .c(new_n91_), .O(new_n99_));
  nand2  g054(.a(new_n99_), .b(new_n47_), .O(z11));
  nor2   g055(.a(new_n67_), .b(new_n83_), .O(new_n101_));
  nand3  g056(.a(new_n84_), .b(x18), .c(x17), .O(new_n102_));
  oai21  g057(.a(new_n101_), .b(new_n84_), .c(new_n102_), .O(new_n103_));
  nand4  g058(.a(new_n103_), .b(new_n90_), .c(new_n92_), .d(new_n91_), .O(new_n104_));
  inv1   g059(.a(new_n104_), .O(z12));
  oai21  g060(.a(x20), .b(x15), .c(new_n67_), .O(new_n106_));
  oai21  g061(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n107_));
  nor2   g062(.a(x20), .b(new_n84_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n101_), .O(new_n109_));
  nand4  g064(.a(new_n109_), .b(new_n107_), .c(new_n90_), .d(new_n91_), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(new_n92_), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n106_), .O(z13));
  oai21  g067(.a(x21), .b(x15), .c(new_n67_), .O(new_n113_));
  nand3  g068(.a(new_n53_), .b(x19), .c(x17), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(x21), .O(new_n115_));
  nor2   g070(.a(x21), .b(x20), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n101_), .c(x19), .O(new_n117_));
  nand4  g072(.a(new_n117_), .b(new_n115_), .c(new_n90_), .d(new_n91_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n92_), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n113_), .O(z14));
  oai21  g075(.a(x22), .b(x15), .c(new_n67_), .O(new_n121_));
  nand4  g076(.a(new_n55_), .b(new_n53_), .c(x19), .d(x17), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(x22), .O(new_n123_));
  nand3  g078(.a(x19), .b(x18), .c(x17), .O(new_n124_));
  inv1   g079(.a(new_n124_), .O(new_n125_));
  nor3   g080(.a(x22), .b(x21), .c(x20), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n123_), .c(new_n90_), .d(new_n91_), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n92_), .O(new_n129_));
  nand2  g084(.a(new_n129_), .b(new_n121_), .O(z15));
  oai21  g085(.a(x23), .b(x15), .c(new_n67_), .O(new_n131_));
  nor2   g086(.a(new_n84_), .b(new_n83_), .O(new_n132_));
  aoi21  g087(.a(new_n126_), .b(new_n132_), .c(new_n66_), .O(new_n133_));
  nor2   g088(.a(x23), .b(x22), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n125_), .c(new_n116_), .O(new_n135_));
  nand3  g090(.a(new_n135_), .b(new_n90_), .c(new_n91_), .O(new_n136_));
  oai21  g091(.a(new_n136_), .b(new_n133_), .c(new_n92_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(new_n131_), .O(z16));
  nand2  g093(.a(new_n135_), .b(x24), .O(new_n139_));
  inv1   g094(.a(new_n109_), .O(new_n140_));
  nor2   g095(.a(x22), .b(x21), .O(new_n141_));
  nand3  g096(.a(new_n141_), .b(new_n51_), .c(new_n66_), .O(new_n142_));
  inv1   g097(.a(new_n142_), .O(new_n143_));
  nand2  g098(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand4  g099(.a(new_n144_), .b(new_n139_), .c(new_n60_), .d(new_n92_), .O(z17));
  oai21  g100(.a(new_n142_), .b(new_n109_), .c(x25), .O(new_n146_));
  nor3   g101(.a(x25), .b(x24), .c(x23), .O(new_n147_));
  nand3  g102(.a(new_n147_), .b(new_n141_), .c(new_n140_), .O(new_n148_));
  nand4  g103(.a(new_n148_), .b(new_n146_), .c(new_n60_), .d(new_n92_), .O(z18));
endmodule


