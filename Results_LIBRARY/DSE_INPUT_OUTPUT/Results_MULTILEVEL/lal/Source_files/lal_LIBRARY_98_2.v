// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(x19), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  nor3   g09(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(x20), .c(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n53_), .c(new_n54_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor3   g15(.a(new_n60_), .b(new_n49_), .c(x07), .O(new_n61_));
  oai21  g16(.a(new_n58_), .b(x25), .c(new_n61_), .O(z01));
  oai21  g17(.a(new_n56_), .b(x23), .c(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  inv1   g19(.a(x20), .O(new_n65_));
  inv1   g20(.a(x19), .O(new_n66_));
  nor2   g21(.a(x18), .b(x17), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x15), .c(new_n66_), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n53_), .c(new_n65_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n64_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n50_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand3  g36(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n67_), .c(x15), .O(new_n84_));
  nand2  g39(.a(new_n56_), .b(x20), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n53_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x24), .c(x25), .O(new_n87_));
  oai21  g42(.a(new_n84_), .b(x19), .c(new_n87_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n60_), .b(new_n48_), .c(new_n89_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  inv1   g46(.a(x17), .O(new_n92_));
  nand4  g47(.a(new_n59_), .b(new_n92_), .c(new_n48_), .d(new_n89_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  xor2a  g49(.a(x18), .b(x17), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n59_), .c(new_n48_), .d(new_n89_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  and2   g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n66_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n66_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n59_), .c(new_n48_), .d(new_n89_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z12));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n59_), .b(new_n89_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nor2   g61(.a(x20), .b(new_n66_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n98_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n104_), .d(new_n48_), .O(z13));
  nand2  g64(.a(new_n108_), .b(x21), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n98_), .c(x19), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n110_), .c(new_n106_), .d(new_n48_), .O(z14));
  nand2  g68(.a(new_n112_), .b(x22), .O(new_n114_));
  inv1   g69(.a(new_n103_), .O(new_n115_));
  nor2   g70(.a(x22), .b(x21), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n65_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n114_), .c(new_n106_), .d(new_n48_), .O(z15));
  nand3  g73(.a(new_n65_), .b(x18), .c(x17), .O(new_n119_));
  inv1   g74(.a(x21), .O(new_n120_));
  nor2   g75(.a(x23), .b(x22), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n119_), .c(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(x19), .O(new_n124_));
  aoi21  g79(.a(new_n117_), .b(x23), .c(new_n105_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(x15), .c(new_n124_), .O(z16));
  nand2  g81(.a(new_n121_), .b(new_n111_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n103_), .c(x24), .O(new_n128_));
  nor2   g83(.a(x24), .b(x23), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n116_), .c(new_n107_), .d(new_n98_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n128_), .c(new_n106_), .d(new_n48_), .O(z17));
  nand2  g86(.a(new_n111_), .b(new_n98_), .O(new_n132_));
  nor2   g87(.a(x25), .b(x24), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n121_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n132_), .c(new_n48_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(x19), .O(new_n136_));
  aoi21  g91(.a(new_n130_), .b(x25), .c(new_n105_), .O(new_n137_));
  oai21  g92(.a(new_n137_), .b(x15), .c(new_n136_), .O(z18));
endmodule


