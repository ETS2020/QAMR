// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x17), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  nor3   g07(.a(x19), .b(x18), .c(x17), .O(new_n53_));
  and2   g08(.a(x22), .b(x21), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(x20), .c(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n51_), .c(new_n52_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nor3   g13(.a(new_n58_), .b(new_n48_), .c(x07), .O(new_n59_));
  oai21  g14(.a(new_n56_), .b(x25), .c(new_n59_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nand2  g16(.a(new_n49_), .b(new_n61_), .O(z02));
  oai21  g17(.a(new_n54_), .b(x23), .c(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n49_), .O(new_n64_));
  inv1   g19(.a(x20), .O(new_n65_));
  inv1   g20(.a(x17), .O(new_n66_));
  nor2   g21(.a(x19), .b(x18), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x15), .c(new_n66_), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n51_), .c(new_n65_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n64_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n49_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand2  g36(.a(new_n54_), .b(x20), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n51_), .c(new_n52_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x25), .c(new_n49_), .O(new_n84_));
  inv1   g39(.a(x19), .O(new_n85_));
  nor3   g40(.a(x18), .b(x17), .c(x15), .O(new_n86_));
  inv1   g41(.a(x22), .O(new_n87_));
  nor2   g42(.a(new_n52_), .b(new_n87_), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n86_), .c(x21), .d(new_n85_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n84_), .O(z08));
  inv1   g45(.a(x07), .O(new_n91_));
  nand3  g46(.a(new_n58_), .b(new_n47_), .c(new_n91_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand2  g48(.a(new_n57_), .b(new_n91_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n47_), .c(x17), .O(z10));
  inv1   g50(.a(x18), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(x17), .c(new_n47_), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(x17), .c(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n57_), .c(new_n91_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n49_), .O(z11));
  nor2   g55(.a(new_n96_), .b(new_n66_), .O(new_n101_));
  nand3  g56(.a(new_n85_), .b(x18), .c(x17), .O(new_n102_));
  oai21  g57(.a(new_n101_), .b(new_n85_), .c(new_n102_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n57_), .c(new_n47_), .d(new_n91_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z12));
  inv1   g60(.a(new_n94_), .O(new_n106_));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n101_), .b(new_n65_), .c(x19), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n108_), .c(new_n106_), .d(new_n47_), .O(z13));
  nand2  g65(.a(new_n109_), .b(x21), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n101_), .c(x19), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n106_), .d(new_n47_), .O(z14));
  nand2  g69(.a(new_n113_), .b(x22), .O(new_n115_));
  inv1   g70(.a(new_n107_), .O(new_n116_));
  nor2   g71(.a(x22), .b(x21), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n65_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n115_), .c(new_n106_), .d(new_n47_), .O(z15));
  nand2  g74(.a(new_n118_), .b(x23), .O(new_n120_));
  nand4  g75(.a(new_n112_), .b(new_n116_), .c(new_n51_), .d(new_n87_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n120_), .c(new_n106_), .d(new_n47_), .O(z16));
  nand3  g77(.a(new_n65_), .b(x19), .c(x18), .O(new_n123_));
  nand3  g78(.a(new_n117_), .b(new_n52_), .c(new_n51_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n123_), .c(new_n47_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(x17), .O(new_n126_));
  aoi21  g81(.a(new_n121_), .b(x24), .c(new_n94_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(x15), .c(new_n126_), .O(z17));
  oai21  g83(.a(new_n124_), .b(new_n109_), .c(x25), .O(new_n129_));
  inv1   g84(.a(new_n109_), .O(new_n130_));
  nor3   g85(.a(x25), .b(x24), .c(x23), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n117_), .c(new_n130_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n129_), .c(new_n106_), .d(new_n47_), .O(z18));
endmodule


