// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  inv1   g002(.a(x15), .O(new_n48_));
  nor2   g003(.a(x17), .b(new_n48_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand2  g005(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g006(.a(z02), .b(new_n46_), .O(z00));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x23), .O(new_n54_));
  nand2  g009(.a(x22), .b(x21), .O(new_n55_));
  nand2  g010(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g011(.a(new_n56_), .b(x24), .O(new_n57_));
  inv1   g012(.a(x07), .O(new_n58_));
  nand2  g013(.a(x05), .b(x04), .O(new_n59_));
  nand2  g014(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g015(.a(new_n57_), .b(new_n53_), .c(new_n60_), .O(new_n61_));
  inv1   g016(.a(x20), .O(new_n62_));
  inv1   g017(.a(x17), .O(new_n63_));
  nor2   g018(.a(x19), .b(x18), .O(new_n64_));
  oai21  g019(.a(new_n64_), .b(x15), .c(new_n63_), .O(new_n65_));
  nand4  g020(.a(new_n65_), .b(new_n53_), .c(new_n54_), .d(new_n62_), .O(new_n66_));
  oai21  g021(.a(new_n61_), .b(new_n49_), .c(new_n66_), .O(z01));
  nand2  g022(.a(new_n57_), .b(new_n50_), .O(new_n68_));
  nand3  g023(.a(new_n65_), .b(new_n54_), .c(new_n62_), .O(new_n69_));
  aoi21  g024(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g025(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g026(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g027(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g028(.a(x12), .b(x03), .O(new_n74_));
  nand4  g029(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g030(.a(new_n75_), .b(new_n50_), .c(new_n46_), .O(new_n76_));
  inv1   g031(.a(new_n76_), .O(z04));
  oai21  g032(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g033(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g034(.a(new_n79_), .b(new_n50_), .O(z06));
  nand3  g035(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g036(.a(x24), .b(x22), .c(x21), .O(new_n82_));
  inv1   g037(.a(new_n82_), .O(new_n83_));
  aoi21  g038(.a(new_n83_), .b(new_n64_), .c(x15), .O(new_n84_));
  oai21  g039(.a(new_n55_), .b(new_n62_), .c(new_n54_), .O(new_n85_));
  aoi21  g040(.a(new_n85_), .b(x24), .c(x25), .O(new_n86_));
  oai21  g041(.a(new_n84_), .b(x17), .c(new_n86_), .O(z08));
  nand4  g042(.a(new_n48_), .b(new_n58_), .c(x05), .d(x04), .O(new_n88_));
  inv1   g043(.a(new_n88_), .O(z09));
  aoi21  g044(.a(new_n60_), .b(new_n48_), .c(x17), .O(z10));
  inv1   g045(.a(x18), .O(new_n91_));
  nand3  g046(.a(new_n91_), .b(x17), .c(new_n48_), .O(new_n92_));
  oai21  g047(.a(new_n91_), .b(x17), .c(new_n92_), .O(new_n93_));
  nand3  g048(.a(new_n93_), .b(new_n59_), .c(new_n58_), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(new_n50_), .O(z11));
  nand2  g050(.a(x19), .b(new_n91_), .O(new_n96_));
  inv1   g051(.a(x19), .O(new_n97_));
  nand3  g052(.a(new_n97_), .b(x18), .c(x17), .O(new_n98_));
  aoi21  g053(.a(new_n98_), .b(new_n96_), .c(x15), .O(new_n99_));
  nor2   g054(.a(new_n97_), .b(x17), .O(new_n100_));
  oai21  g055(.a(new_n100_), .b(new_n99_), .c(new_n59_), .O(new_n101_));
  oai21  g056(.a(new_n101_), .b(x07), .c(new_n50_), .O(z12));
  nand3  g057(.a(new_n62_), .b(x19), .c(x18), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(x17), .O(new_n105_));
  nand3  g060(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  aoi21  g061(.a(new_n106_), .b(x20), .c(new_n60_), .O(new_n107_));
  oai21  g062(.a(new_n107_), .b(x15), .c(new_n105_), .O(z13));
  nand2  g063(.a(x19), .b(x18), .O(new_n109_));
  inv1   g064(.a(new_n109_), .O(new_n110_));
  nor2   g065(.a(x21), .b(x20), .O(new_n111_));
  aoi21  g066(.a(new_n111_), .b(new_n110_), .c(x15), .O(new_n112_));
  nand4  g067(.a(new_n62_), .b(x19), .c(x18), .d(x17), .O(new_n113_));
  aoi21  g068(.a(new_n113_), .b(x21), .c(new_n60_), .O(new_n114_));
  oai22  g069(.a(new_n114_), .b(x15), .c(new_n112_), .d(new_n63_), .O(z14));
  nor2   g070(.a(x22), .b(x21), .O(new_n116_));
  nand3  g071(.a(new_n116_), .b(new_n110_), .c(new_n62_), .O(new_n117_));
  nand2  g072(.a(new_n117_), .b(new_n48_), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(x17), .O(new_n119_));
  nand4  g074(.a(new_n111_), .b(x19), .c(x18), .d(x17), .O(new_n120_));
  aoi21  g075(.a(new_n120_), .b(x22), .c(new_n60_), .O(new_n121_));
  oai21  g076(.a(new_n121_), .b(x15), .c(new_n119_), .O(z15));
  inv1   g077(.a(x21), .O(new_n123_));
  nor2   g078(.a(x23), .b(x22), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  oai21  g080(.a(new_n125_), .b(new_n103_), .c(new_n48_), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(x17), .O(new_n127_));
  inv1   g082(.a(new_n106_), .O(new_n128_));
  nand3  g083(.a(new_n116_), .b(new_n128_), .c(new_n62_), .O(new_n129_));
  aoi21  g084(.a(new_n129_), .b(x23), .c(new_n60_), .O(new_n130_));
  oai21  g085(.a(new_n130_), .b(x15), .c(new_n127_), .O(z16));
  inv1   g086(.a(x22), .O(new_n132_));
  inv1   g087(.a(x24), .O(new_n133_));
  nand4  g088(.a(new_n133_), .b(new_n54_), .c(new_n132_), .d(new_n123_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(new_n103_), .c(new_n48_), .O(new_n135_));
  nand2  g090(.a(new_n135_), .b(x17), .O(new_n136_));
  nand3  g091(.a(new_n124_), .b(new_n111_), .c(new_n128_), .O(new_n137_));
  aoi21  g092(.a(new_n137_), .b(x24), .c(new_n60_), .O(new_n138_));
  oai21  g093(.a(new_n138_), .b(x15), .c(new_n136_), .O(z17));
  nand2  g094(.a(new_n111_), .b(new_n110_), .O(new_n140_));
  nand3  g095(.a(new_n124_), .b(new_n53_), .c(new_n133_), .O(new_n141_));
  oai21  g096(.a(new_n141_), .b(new_n140_), .c(new_n48_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(x17), .O(new_n143_));
  inv1   g098(.a(new_n60_), .O(new_n144_));
  oai21  g099(.a(new_n134_), .b(new_n113_), .c(x25), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(new_n48_), .O(new_n147_));
  nand2  g102(.a(new_n147_), .b(new_n143_), .O(z18));
endmodule


