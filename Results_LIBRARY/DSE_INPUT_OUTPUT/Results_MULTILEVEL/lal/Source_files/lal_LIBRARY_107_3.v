// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x15), .b(x02), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(new_n47_), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(new_n50_), .O(z01));
  nor2   g17(.a(new_n50_), .b(new_n46_), .O(z02));
  nand3  g18(.a(new_n59_), .b(new_n47_), .c(new_n51_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(z03));
  nor2   g20(.a(x11), .b(x08), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x15), .c(x02), .O(new_n67_));
  inv1   g22(.a(x08), .O(new_n68_));
  inv1   g23(.a(x09), .O(new_n69_));
  inv1   g24(.a(x01), .O(new_n70_));
  aoi22  g25(.a(x10), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x10), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x01), .O(new_n74_));
  inv1   g29(.a(x02), .O(new_n75_));
  nand2  g30(.a(x11), .b(new_n75_), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n72_), .c(new_n68_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n67_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g36(.a(new_n47_), .b(x14), .c(new_n68_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z06));
  nand3  g38(.a(new_n47_), .b(new_n68_), .c(x06), .O(z07));
  inv1   g39(.a(x23), .O(new_n85_));
  nand2  g40(.a(new_n56_), .b(new_n52_), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(x22), .c(x21), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(x24), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n51_), .c(new_n50_), .O(z08));
  nand2  g45(.a(x05), .b(x04), .O(new_n91_));
  inv1   g46(.a(x07), .O(new_n92_));
  inv1   g47(.a(x15), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n91_), .c(new_n47_), .O(z09));
  nand4  g50(.a(new_n91_), .b(new_n53_), .c(new_n93_), .d(new_n92_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n47_), .O(z10));
  xor2a  g52(.a(x18), .b(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n91_), .c(new_n93_), .d(new_n92_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n47_), .O(z11));
  nor2   g55(.a(new_n54_), .b(new_n53_), .O(new_n101_));
  nand3  g56(.a(new_n55_), .b(x18), .c(x17), .O(new_n102_));
  oai21  g57(.a(new_n101_), .b(new_n55_), .c(new_n102_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n91_), .c(new_n93_), .d(new_n92_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(z12));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n101_), .b(new_n52_), .c(x19), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n91_), .d(new_n92_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n93_), .O(new_n110_));
  nand2  g65(.a(x15), .b(new_n75_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n110_), .O(z13));
  nand2  g67(.a(new_n108_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n101_), .c(x19), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n61_), .d(new_n93_), .O(z14));
  inv1   g71(.a(new_n106_), .O(new_n117_));
  nor3   g72(.a(x22), .b(x21), .c(x20), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n91_), .c(new_n92_), .O(new_n120_));
  aoi21  g75(.a(new_n115_), .b(x22), .c(new_n120_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(x15), .c(new_n111_), .O(z15));
  aoi21  g77(.a(new_n118_), .b(new_n117_), .c(new_n85_), .O(new_n123_));
  nor2   g78(.a(x23), .b(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n114_), .c(new_n117_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n91_), .c(new_n92_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n123_), .c(new_n93_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n111_), .O(z16));
  nand2  g83(.a(new_n125_), .b(x24), .O(new_n129_));
  inv1   g84(.a(new_n108_), .O(new_n130_));
  nor2   g85(.a(x22), .b(x21), .O(new_n131_));
  nor2   g86(.a(x24), .b(x23), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n131_), .c(new_n130_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n129_), .c(new_n61_), .d(new_n93_), .O(z17));
  nand2  g89(.a(new_n133_), .b(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n131_), .c(new_n130_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n61_), .d(new_n93_), .O(z18));
endmodule


