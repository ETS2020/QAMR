// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  and2   g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n56_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n49_), .c(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  and2   g18(.a(new_n47_), .b(x16), .O(z02));
  nand2  g19(.a(new_n59_), .b(new_n47_), .O(z03));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n47_), .O(z04));
  inv1   g27(.a(x13), .O(new_n73_));
  nand3  g28(.a(new_n47_), .b(new_n73_), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g32(.a(new_n47_), .O(new_n78_));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n78_), .O(z07));
  inv1   g34(.a(x23), .O(new_n80_));
  nand3  g35(.a(new_n56_), .b(x22), .c(x21), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x24), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n50_), .c(new_n78_), .O(z08));
  inv1   g39(.a(x07), .O(new_n85_));
  inv1   g40(.a(x15), .O(new_n86_));
  nand3  g41(.a(new_n61_), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand4  g43(.a(new_n60_), .b(new_n52_), .c(new_n86_), .d(new_n85_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n60_), .c(new_n86_), .d(new_n85_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z11));
  nor2   g48(.a(new_n53_), .b(new_n52_), .O(new_n94_));
  nand3  g49(.a(new_n54_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n54_), .c(new_n95_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n60_), .c(new_n86_), .d(new_n85_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n47_), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nor2   g55(.a(x20), .b(new_n54_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n94_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n100_), .c(new_n62_), .d(new_n86_), .O(z13));
  nand2  g58(.a(new_n102_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n94_), .c(x19), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n62_), .d(new_n86_), .O(z14));
  inv1   g62(.a(new_n99_), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n60_), .c(new_n86_), .O(new_n111_));
  aoi21  g66(.a(new_n106_), .b(x22), .c(new_n111_), .O(new_n112_));
  nand2  g67(.a(new_n86_), .b(x07), .O(new_n113_));
  oai21  g68(.a(new_n112_), .b(x07), .c(new_n113_), .O(z15));
  aoi21  g69(.a(new_n109_), .b(new_n108_), .c(new_n80_), .O(new_n115_));
  inv1   g70(.a(x22), .O(new_n116_));
  nand4  g71(.a(new_n105_), .b(new_n108_), .c(new_n80_), .d(new_n116_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n60_), .c(new_n86_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n115_), .c(new_n85_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n113_), .O(z16));
  nand3  g75(.a(new_n105_), .b(new_n80_), .c(new_n116_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n99_), .c(x24), .O(new_n122_));
  nor2   g77(.a(x22), .b(x21), .O(new_n123_));
  nor2   g78(.a(x24), .b(x23), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n123_), .c(new_n101_), .d(new_n94_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n122_), .c(new_n60_), .d(new_n86_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n85_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n113_), .O(z17));
  nand2  g83(.a(new_n125_), .b(x25), .O(new_n129_));
  nor3   g84(.a(x25), .b(x24), .c(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n123_), .c(new_n101_), .d(new_n94_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n62_), .d(new_n86_), .O(z18));
endmodule


