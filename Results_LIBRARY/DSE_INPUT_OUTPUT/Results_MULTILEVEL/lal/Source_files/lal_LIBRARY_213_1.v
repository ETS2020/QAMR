// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_;
  inv1   g000(.a(x08), .O(new_n46_));
  nor2   g001(.a(x16), .b(new_n46_), .O(z00));
  inv1   g002(.a(x25), .O(new_n48_));
  inv1   g003(.a(x20), .O(new_n49_));
  inv1   g004(.a(x17), .O(new_n50_));
  inv1   g005(.a(x18), .O(new_n51_));
  inv1   g006(.a(x19), .O(new_n52_));
  nand3  g007(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g008(.a(x22), .b(x21), .O(new_n54_));
  aoi21  g009(.a(new_n53_), .b(new_n49_), .c(new_n54_), .O(new_n55_));
  oai21  g010(.a(new_n55_), .b(x23), .c(x24), .O(new_n56_));
  nand2  g011(.a(new_n56_), .b(new_n48_), .O(new_n57_));
  nand2  g012(.a(x05), .b(x04), .O(new_n58_));
  inv1   g013(.a(new_n58_), .O(new_n59_));
  inv1   g014(.a(x12), .O(new_n60_));
  nand2  g015(.a(x16), .b(new_n60_), .O(new_n61_));
  inv1   g016(.a(new_n61_), .O(new_n62_));
  nor3   g017(.a(new_n62_), .b(new_n59_), .c(x07), .O(new_n63_));
  nand2  g018(.a(new_n63_), .b(new_n57_), .O(z01));
  nand3  g019(.a(new_n61_), .b(new_n56_), .c(new_n48_), .O(new_n65_));
  inv1   g020(.a(new_n65_), .O(z03));
  inv1   g021(.a(x00), .O(new_n67_));
  nand2  g022(.a(x09), .b(new_n67_), .O(new_n68_));
  inv1   g023(.a(x09), .O(new_n69_));
  nand2  g024(.a(new_n69_), .b(x00), .O(new_n70_));
  inv1   g025(.a(x01), .O(new_n71_));
  nand2  g026(.a(x10), .b(new_n71_), .O(new_n72_));
  nand3  g027(.a(new_n72_), .b(new_n70_), .c(new_n68_), .O(new_n73_));
  inv1   g028(.a(x10), .O(new_n74_));
  nand2  g029(.a(new_n74_), .b(x01), .O(new_n75_));
  inv1   g030(.a(x02), .O(new_n76_));
  nand2  g031(.a(x11), .b(new_n76_), .O(new_n77_));
  inv1   g032(.a(x11), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(x02), .O(new_n79_));
  nand3  g034(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  oai21  g035(.a(new_n80_), .b(new_n73_), .c(new_n61_), .O(new_n81_));
  nor2   g036(.a(new_n60_), .b(x03), .O(new_n82_));
  nor2   g037(.a(x16), .b(x12), .O(new_n83_));
  aoi21  g038(.a(new_n83_), .b(x03), .c(new_n82_), .O(new_n84_));
  aoi21  g039(.a(new_n84_), .b(new_n81_), .c(x08), .O(z04));
  nor3   g040(.a(new_n62_), .b(x13), .c(x08), .O(z05));
  nand2  g041(.a(x14), .b(new_n46_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(new_n61_), .O(z06));
  aoi21  g043(.a(new_n46_), .b(x06), .c(new_n62_), .O(z07));
  inv1   g044(.a(x24), .O(new_n90_));
  inv1   g045(.a(x22), .O(new_n91_));
  aoi21  g046(.a(new_n53_), .b(new_n49_), .c(new_n91_), .O(new_n92_));
  aoi21  g047(.a(new_n92_), .b(x21), .c(x23), .O(new_n93_));
  nor2   g048(.a(new_n62_), .b(x25), .O(new_n94_));
  oai21  g049(.a(new_n93_), .b(new_n90_), .c(new_n94_), .O(z08));
  inv1   g050(.a(x07), .O(new_n96_));
  inv1   g051(.a(x15), .O(new_n97_));
  nand2  g052(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  oai21  g053(.a(new_n98_), .b(new_n58_), .c(new_n61_), .O(z09));
  nand4  g054(.a(new_n58_), .b(new_n50_), .c(new_n97_), .d(new_n96_), .O(new_n100_));
  nand2  g055(.a(new_n100_), .b(new_n61_), .O(z10));
  xor2a  g056(.a(x18), .b(x17), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n58_), .c(new_n97_), .d(new_n96_), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n61_), .O(z11));
  nor2   g059(.a(new_n51_), .b(new_n50_), .O(new_n105_));
  nand3  g060(.a(new_n52_), .b(x18), .c(x17), .O(new_n106_));
  oai21  g061(.a(new_n105_), .b(new_n52_), .c(new_n106_), .O(new_n107_));
  nand4  g062(.a(new_n107_), .b(new_n61_), .c(new_n58_), .d(new_n97_), .O(new_n108_));
  nor2   g063(.a(new_n108_), .b(x07), .O(z12));
  nand3  g064(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(x20), .O(new_n111_));
  nand2  g066(.a(new_n58_), .b(new_n96_), .O(new_n112_));
  inv1   g067(.a(new_n112_), .O(new_n113_));
  nand4  g068(.a(new_n49_), .b(x19), .c(x18), .d(x17), .O(new_n114_));
  nand4  g069(.a(new_n114_), .b(new_n113_), .c(new_n111_), .d(new_n97_), .O(new_n115_));
  and2   g070(.a(new_n115_), .b(new_n61_), .O(z13));
  nand2  g071(.a(new_n114_), .b(x21), .O(new_n117_));
  nor2   g072(.a(x21), .b(x20), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n105_), .c(x19), .O(new_n119_));
  nand3  g074(.a(new_n119_), .b(new_n61_), .c(new_n97_), .O(new_n120_));
  inv1   g075(.a(new_n120_), .O(new_n121_));
  nand3  g076(.a(new_n121_), .b(new_n117_), .c(new_n113_), .O(z14));
  nand2  g077(.a(new_n119_), .b(x22), .O(new_n123_));
  inv1   g078(.a(new_n110_), .O(new_n124_));
  nor2   g079(.a(x22), .b(x21), .O(new_n125_));
  nand3  g080(.a(new_n125_), .b(new_n124_), .c(new_n49_), .O(new_n126_));
  and2   g081(.a(new_n126_), .b(new_n61_), .O(new_n127_));
  nand4  g082(.a(new_n127_), .b(new_n123_), .c(new_n113_), .d(new_n97_), .O(z15));
  nand2  g083(.a(new_n126_), .b(x23), .O(new_n129_));
  inv1   g084(.a(x23), .O(new_n130_));
  nand4  g085(.a(new_n118_), .b(new_n124_), .c(new_n130_), .d(new_n91_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n113_), .c(new_n97_), .O(new_n132_));
  inv1   g087(.a(new_n132_), .O(new_n133_));
  aoi21  g088(.a(new_n133_), .b(new_n129_), .c(new_n62_), .O(z16));
  nand2  g089(.a(new_n131_), .b(x24), .O(new_n135_));
  nand3  g090(.a(new_n125_), .b(new_n90_), .c(new_n130_), .O(new_n136_));
  oai21  g091(.a(new_n136_), .b(new_n114_), .c(new_n97_), .O(new_n137_));
  nor2   g092(.a(new_n137_), .b(new_n112_), .O(new_n138_));
  aoi21  g093(.a(new_n138_), .b(new_n135_), .c(new_n62_), .O(z17));
  oai21  g094(.a(new_n136_), .b(new_n114_), .c(x25), .O(new_n140_));
  nor2   g095(.a(x25), .b(x24), .O(new_n141_));
  nand3  g096(.a(new_n141_), .b(new_n125_), .c(new_n130_), .O(new_n142_));
  oai21  g097(.a(new_n142_), .b(new_n114_), .c(new_n97_), .O(new_n143_));
  nor2   g098(.a(new_n143_), .b(new_n112_), .O(new_n144_));
  aoi21  g099(.a(new_n144_), .b(new_n140_), .c(new_n62_), .O(z18));
  buf    g100(.a(x16), .O(z02));
endmodule


