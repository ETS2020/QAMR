// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x11), .O(new_n47_));
  nor2   g02(.a(x15), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n61_), .c(new_n49_), .d(new_n51_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nand3  g20(.a(new_n60_), .b(new_n49_), .c(new_n52_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  inv1   g22(.a(x09), .O(new_n68_));
  inv1   g23(.a(x01), .O(new_n69_));
  aoi22  g24(.a(x10), .b(new_n69_), .c(new_n68_), .d(x00), .O(new_n70_));
  oai21  g25(.a(new_n68_), .b(x00), .c(new_n70_), .O(new_n71_));
  inv1   g26(.a(x10), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x01), .O(new_n73_));
  inv1   g28(.a(x03), .O(new_n74_));
  nand2  g29(.a(x12), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x12), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x03), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n75_), .c(new_n73_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n71_), .c(new_n49_), .O(new_n79_));
  inv1   g34(.a(x15), .O(new_n80_));
  nor3   g35(.a(new_n80_), .b(new_n47_), .c(x02), .O(new_n81_));
  aoi21  g36(.a(new_n47_), .b(x02), .c(new_n81_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n79_), .c(x08), .O(z04));
  nor3   g38(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor3   g40(.a(new_n48_), .b(new_n85_), .c(x08), .O(z06));
  nand3  g41(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g42(.a(x23), .O(new_n88_));
  nand2  g43(.a(new_n57_), .b(new_n53_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(x22), .c(x21), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(x24), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n52_), .c(new_n48_), .O(z08));
  nand3  g48(.a(new_n51_), .b(x05), .c(x04), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n47_), .c(x15), .O(z09));
  nand4  g50(.a(new_n62_), .b(new_n54_), .c(new_n80_), .d(new_n47_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(x07), .O(z10));
  xor2a  g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n62_), .c(new_n51_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n47_), .c(x15), .O(z11));
  nor2   g55(.a(new_n55_), .b(new_n54_), .O(new_n101_));
  nand3  g56(.a(new_n56_), .b(x18), .c(x17), .O(new_n102_));
  oai21  g57(.a(new_n101_), .b(new_n56_), .c(new_n102_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n62_), .c(new_n80_), .d(new_n47_), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(x07), .O(z12));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n101_), .b(new_n53_), .c(x19), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n62_), .d(new_n51_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n80_), .O(z13));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n101_), .c(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n62_), .c(new_n51_), .O(new_n114_));
  aoi21  g69(.a(new_n108_), .b(x21), .c(new_n114_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(x11), .c(new_n80_), .O(z14));
  nand2  g71(.a(new_n113_), .b(x22), .O(new_n117_));
  aoi21  g72(.a(x05), .b(x04), .c(x07), .O(new_n118_));
  inv1   g73(.a(new_n106_), .O(new_n119_));
  inv1   g74(.a(x21), .O(new_n120_));
  inv1   g75(.a(x22), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(x20), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n119_), .c(x15), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n118_), .c(new_n117_), .d(new_n47_), .O(z15));
  aoi21  g80(.a(new_n123_), .b(new_n119_), .c(new_n88_), .O(new_n126_));
  nand4  g81(.a(new_n112_), .b(new_n119_), .c(new_n88_), .d(new_n121_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n62_), .c(new_n51_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n126_), .c(new_n47_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n80_), .O(z16));
  nand2  g85(.a(new_n127_), .b(x24), .O(new_n131_));
  inv1   g86(.a(new_n108_), .O(new_n132_));
  inv1   g87(.a(x24), .O(new_n133_));
  nor2   g88(.a(x22), .b(x21), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n133_), .c(new_n88_), .O(new_n135_));
  inv1   g90(.a(new_n135_), .O(new_n136_));
  aoi21  g91(.a(new_n136_), .b(new_n132_), .c(x15), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n131_), .c(new_n118_), .d(new_n47_), .O(z17));
  oai21  g93(.a(new_n135_), .b(new_n108_), .c(x25), .O(new_n139_));
  nand3  g94(.a(new_n52_), .b(new_n133_), .c(new_n88_), .O(new_n140_));
  nor2   g95(.a(new_n140_), .b(new_n122_), .O(new_n141_));
  aoi21  g96(.a(new_n141_), .b(new_n132_), .c(x15), .O(new_n142_));
  nand4  g97(.a(new_n142_), .b(new_n139_), .c(new_n118_), .d(new_n47_), .O(z18));
endmodule


