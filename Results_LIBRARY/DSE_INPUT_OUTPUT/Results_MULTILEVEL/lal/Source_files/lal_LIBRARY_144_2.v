// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  aoi21  g01(.a(x16), .b(x15), .c(new_n46_), .O(z00));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nor2   g14(.a(x15), .b(new_n46_), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(new_n59_), .c(new_n58_), .d(new_n48_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n61_), .b(new_n63_), .O(z02));
  nand3  g19(.a(new_n61_), .b(new_n57_), .c(new_n49_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(x08), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n61_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(x08), .O(z06));
  nand2  g31(.a(new_n46_), .b(x06), .O(z07));
  inv1   g32(.a(x23), .O(new_n78_));
  nand2  g33(.a(new_n54_), .b(new_n50_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(x22), .c(x21), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(x24), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n49_), .c(new_n60_), .O(z08));
  nand3  g38(.a(new_n48_), .b(x05), .c(x04), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n46_), .c(x15), .O(z09));
  inv1   g40(.a(x15), .O(new_n86_));
  nand4  g41(.a(new_n59_), .b(new_n51_), .c(new_n86_), .d(new_n46_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x07), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n59_), .c(new_n48_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n46_), .c(x15), .O(z11));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand3  g48(.a(new_n53_), .b(x18), .c(x17), .O(new_n94_));
  oai21  g49(.a(new_n93_), .b(new_n53_), .c(new_n94_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n59_), .c(new_n48_), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(new_n46_), .c(x15), .O(z12));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x20), .O(new_n99_));
  aoi21  g54(.a(x05), .b(x04), .c(x07), .O(new_n100_));
  nor2   g55(.a(x20), .b(new_n53_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n93_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nor2   g58(.a(new_n103_), .b(x15), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n100_), .c(new_n99_), .d(new_n46_), .O(z13));
  nand2  g60(.a(new_n102_), .b(x21), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n93_), .c(x19), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n86_), .c(new_n46_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n106_), .c(new_n100_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  inv1   g67(.a(new_n98_), .O(new_n113_));
  inv1   g68(.a(x21), .O(new_n114_));
  inv1   g69(.a(x22), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nor2   g71(.a(new_n116_), .b(x20), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n112_), .c(new_n59_), .d(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n46_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n86_), .O(z15));
  aoi21  g76(.a(new_n117_), .b(new_n113_), .c(new_n78_), .O(new_n122_));
  nand4  g77(.a(new_n107_), .b(new_n113_), .c(new_n78_), .d(new_n115_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n59_), .c(new_n48_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n122_), .c(new_n46_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n86_), .O(z16));
  nand3  g81(.a(new_n107_), .b(new_n78_), .c(new_n115_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n98_), .c(x24), .O(new_n128_));
  inv1   g83(.a(new_n116_), .O(new_n129_));
  nor2   g84(.a(x24), .b(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n129_), .c(new_n101_), .d(new_n93_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n128_), .c(new_n59_), .d(new_n48_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n46_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n86_), .O(z17));
  nand2  g89(.a(new_n131_), .b(x25), .O(new_n135_));
  inv1   g90(.a(x24), .O(new_n136_));
  nand3  g91(.a(new_n49_), .b(new_n136_), .c(new_n78_), .O(new_n137_));
  nor2   g92(.a(new_n137_), .b(new_n116_), .O(new_n138_));
  aoi21  g93(.a(new_n138_), .b(new_n103_), .c(x15), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n135_), .c(new_n100_), .d(new_n46_), .O(z18));
endmodule


