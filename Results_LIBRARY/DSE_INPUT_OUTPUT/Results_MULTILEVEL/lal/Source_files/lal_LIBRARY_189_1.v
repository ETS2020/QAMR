// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x21), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n47_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n51_), .c(new_n52_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n48_), .c(x07), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(x25), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g23(.a(x24), .b(x23), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n47_), .O(new_n70_));
  aoi21  g25(.a(new_n57_), .b(new_n53_), .c(new_n59_), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(x23), .c(x24), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x15), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g32(.a(x12), .b(x03), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n49_), .c(new_n46_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n49_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand3  g40(.a(new_n58_), .b(x22), .c(x21), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n51_), .c(new_n52_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x25), .c(x15), .O(new_n88_));
  inv1   g43(.a(new_n69_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x25), .c(new_n47_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n88_), .O(z08));
  nor4   g46(.a(new_n63_), .b(x21), .c(x15), .d(x07), .O(z09));
  inv1   g47(.a(x07), .O(new_n93_));
  nand3  g48(.a(new_n63_), .b(new_n54_), .c(new_n93_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n47_), .c(x15), .O(z10));
  inv1   g50(.a(x15), .O(new_n96_));
  xor2a  g51(.a(x18), .b(x17), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n63_), .c(new_n47_), .d(new_n96_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z11));
  nor2   g54(.a(new_n55_), .b(new_n54_), .O(new_n100_));
  nand3  g55(.a(new_n56_), .b(x18), .c(x17), .O(new_n101_));
  oai21  g56(.a(new_n100_), .b(new_n56_), .c(new_n101_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n63_), .c(new_n47_), .d(new_n96_), .O(new_n103_));
  nor2   g58(.a(new_n103_), .b(x07), .O(z12));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nor2   g61(.a(x20), .b(new_n56_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n100_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n63_), .d(new_n93_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n96_), .O(z13));
  nor2   g66(.a(x21), .b(x15), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n108_), .c(new_n63_), .d(new_n93_), .O(z14));
  nand2  g68(.a(new_n108_), .b(x22), .O(new_n114_));
  nor2   g69(.a(new_n64_), .b(x07), .O(new_n115_));
  nor2   g70(.a(x22), .b(x20), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n100_), .c(x19), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n47_), .c(new_n96_), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n115_), .c(new_n114_), .O(z15));
  nand2  g75(.a(new_n117_), .b(x23), .O(new_n121_));
  inv1   g76(.a(new_n105_), .O(new_n122_));
  nor3   g77(.a(x23), .b(x22), .c(x20), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n122_), .c(x21), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n121_), .c(new_n115_), .d(new_n96_), .O(z16));
  aoi21  g80(.a(new_n123_), .b(new_n122_), .c(new_n52_), .O(new_n126_));
  nand4  g81(.a(new_n116_), .b(new_n122_), .c(new_n52_), .d(new_n51_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n63_), .c(new_n93_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n126_), .c(new_n47_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n96_), .O(z17));
  nor2   g85(.a(x23), .b(x22), .O(new_n131_));
  nor2   g86(.a(x25), .b(x24), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n131_), .c(new_n107_), .d(new_n100_), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n63_), .c(new_n93_), .O(new_n134_));
  aoi21  g89(.a(new_n127_), .b(x25), .c(new_n134_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(x21), .c(new_n96_), .O(z18));
endmodule


