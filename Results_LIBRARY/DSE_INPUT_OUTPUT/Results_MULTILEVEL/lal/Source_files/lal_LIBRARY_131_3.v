// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x06), .b(x03), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  nand2  g18(.a(new_n60_), .b(new_n47_), .O(z03));
  nor2   g19(.a(x12), .b(x08), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(x06), .c(x03), .O(new_n66_));
  inv1   g21(.a(x09), .O(new_n67_));
  inv1   g22(.a(x01), .O(new_n68_));
  aoi22  g23(.a(x10), .b(new_n68_), .c(new_n67_), .d(x00), .O(new_n69_));
  oai21  g24(.a(new_n67_), .b(x00), .c(new_n69_), .O(new_n70_));
  inv1   g25(.a(x10), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(x01), .O(new_n72_));
  inv1   g27(.a(x02), .O(new_n73_));
  nand2  g28(.a(x11), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x11), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x02), .O(new_n76_));
  inv1   g31(.a(x03), .O(new_n77_));
  nand2  g32(.a(x12), .b(new_n77_), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n76_), .c(new_n74_), .d(new_n72_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n70_), .c(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n66_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g37(.a(x14), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x08), .c(new_n47_), .O(z06));
  oai21  g39(.a(new_n46_), .b(x03), .c(x06), .O(z07));
  inv1   g40(.a(x22), .O(new_n86_));
  aoi21  g41(.a(new_n56_), .b(new_n52_), .c(new_n86_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x21), .c(x23), .O(new_n88_));
  nor2   g43(.a(new_n49_), .b(x25), .O(new_n89_));
  oai21  g44(.a(new_n88_), .b(new_n50_), .c(new_n89_), .O(z08));
  nand2  g45(.a(x05), .b(x04), .O(new_n91_));
  inv1   g46(.a(x07), .O(new_n92_));
  inv1   g47(.a(x15), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n91_), .c(new_n47_), .O(z09));
  nand2  g50(.a(new_n91_), .b(new_n47_), .O(new_n96_));
  nor4   g51(.a(new_n96_), .b(x17), .c(x15), .d(x07), .O(z10));
  xor2a  g52(.a(x18), .b(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n91_), .c(new_n93_), .d(new_n92_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n47_), .O(z11));
  nor2   g55(.a(new_n54_), .b(new_n53_), .O(new_n101_));
  nand3  g56(.a(new_n55_), .b(x18), .c(x17), .O(new_n102_));
  oai21  g57(.a(new_n101_), .b(new_n55_), .c(new_n102_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n91_), .c(new_n47_), .d(new_n93_), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(x07), .O(z12));
  inv1   g60(.a(new_n96_), .O(new_n106_));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n101_), .b(new_n52_), .c(x19), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(x15), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n108_), .c(new_n106_), .d(new_n92_), .O(z13));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n101_), .c(x19), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n61_), .c(new_n93_), .O(new_n115_));
  aoi21  g70(.a(new_n109_), .b(x21), .c(new_n115_), .O(new_n116_));
  nor2   g71(.a(new_n116_), .b(new_n49_), .O(z14));
  nand2  g72(.a(new_n114_), .b(x22), .O(new_n118_));
  inv1   g73(.a(new_n107_), .O(new_n119_));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n52_), .O(new_n121_));
  and2   g76(.a(new_n121_), .b(new_n93_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n118_), .c(new_n106_), .d(new_n92_), .O(z15));
  inv1   g78(.a(x23), .O(new_n124_));
  nand4  g79(.a(new_n113_), .b(new_n119_), .c(new_n124_), .d(new_n86_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n61_), .c(new_n93_), .O(new_n126_));
  aoi21  g81(.a(new_n121_), .b(x23), .c(new_n126_), .O(new_n127_));
  nor2   g82(.a(new_n127_), .b(new_n49_), .O(z16));
  nand2  g83(.a(new_n125_), .b(x24), .O(new_n129_));
  nand3  g84(.a(new_n120_), .b(new_n50_), .c(new_n124_), .O(new_n130_));
  inv1   g85(.a(new_n130_), .O(new_n131_));
  aoi21  g86(.a(new_n131_), .b(new_n110_), .c(x15), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n129_), .c(new_n106_), .d(new_n92_), .O(z17));
  oai21  g88(.a(new_n130_), .b(new_n109_), .c(x25), .O(new_n134_));
  nor3   g89(.a(x25), .b(x24), .c(x23), .O(new_n135_));
  and2   g90(.a(new_n135_), .b(new_n120_), .O(new_n136_));
  aoi21  g91(.a(new_n136_), .b(new_n110_), .c(x15), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n134_), .c(new_n106_), .d(new_n92_), .O(z18));
endmodule


