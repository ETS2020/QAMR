// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x18), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n52_), .c(new_n53_), .O(new_n55_));
  aoi21  g10(.a(x05), .b(x04), .c(x07), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(x25), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  inv1   g13(.a(x20), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  oai21  g15(.a(x19), .b(x17), .c(new_n48_), .O(new_n61_));
  oai21  g16(.a(new_n48_), .b(x15), .c(new_n61_), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n60_), .c(new_n52_), .d(new_n59_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n58_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n50_), .b(new_n65_), .O(z02));
  or2    g21(.a(new_n55_), .b(new_n49_), .O(new_n67_));
  nand3  g22(.a(new_n62_), .b(new_n52_), .c(new_n59_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n67_), .c(x25), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n50_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand3  g32(.a(new_n50_), .b(new_n77_), .c(new_n46_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z06));
  nand3  g36(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g37(.a(x22), .O(new_n83_));
  inv1   g38(.a(x17), .O(new_n84_));
  inv1   g39(.a(x19), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n48_), .c(new_n84_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n59_), .c(new_n83_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x21), .c(x23), .O(new_n88_));
  nor2   g43(.a(new_n49_), .b(x25), .O(new_n89_));
  oai21  g44(.a(new_n88_), .b(new_n53_), .c(new_n89_), .O(z08));
  nand2  g45(.a(x05), .b(x04), .O(new_n91_));
  inv1   g46(.a(x07), .O(new_n92_));
  nand2  g47(.a(new_n47_), .b(new_n92_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n91_), .c(new_n50_), .O(z09));
  nand4  g49(.a(new_n91_), .b(new_n84_), .c(new_n47_), .d(new_n92_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  xor2a  g51(.a(x18), .b(x17), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n91_), .c(new_n47_), .d(new_n92_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  nor2   g54(.a(new_n48_), .b(new_n84_), .O(new_n100_));
  nand3  g55(.a(new_n85_), .b(x18), .c(x17), .O(new_n101_));
  oai21  g56(.a(new_n100_), .b(new_n85_), .c(new_n101_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n91_), .c(new_n47_), .d(new_n92_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z12));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nor2   g61(.a(x20), .b(new_n85_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n100_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n56_), .d(new_n47_), .O(z13));
  oai21  g64(.a(x21), .b(x15), .c(new_n48_), .O(new_n110_));
  nand3  g65(.a(new_n59_), .b(x19), .c(x17), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x21), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n100_), .c(x19), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n112_), .c(new_n91_), .d(new_n92_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n47_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n110_), .O(z14));
  nand2  g72(.a(new_n114_), .b(x22), .O(new_n118_));
  inv1   g73(.a(new_n105_), .O(new_n119_));
  nor3   g74(.a(x22), .b(x21), .c(x20), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n119_), .c(x15), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n118_), .c(new_n56_), .O(z15));
  oai21  g77(.a(x23), .b(x15), .c(new_n48_), .O(new_n123_));
  nor2   g78(.a(new_n85_), .b(new_n84_), .O(new_n124_));
  aoi21  g79(.a(new_n124_), .b(new_n120_), .c(new_n52_), .O(new_n125_));
  nor2   g80(.a(x23), .b(x22), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n113_), .c(new_n119_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n91_), .c(new_n92_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n125_), .c(new_n47_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n123_), .O(z16));
  nand2  g85(.a(new_n127_), .b(x24), .O(new_n131_));
  inv1   g86(.a(new_n108_), .O(new_n132_));
  nor2   g87(.a(x22), .b(x21), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n53_), .c(new_n52_), .O(new_n134_));
  inv1   g89(.a(new_n134_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n131_), .c(new_n56_), .d(new_n47_), .O(z17));
  oai21  g92(.a(new_n134_), .b(new_n108_), .c(x25), .O(new_n138_));
  nor3   g93(.a(x25), .b(x24), .c(x23), .O(new_n139_));
  nand3  g94(.a(new_n139_), .b(new_n133_), .c(new_n132_), .O(new_n140_));
  nand4  g95(.a(new_n140_), .b(new_n138_), .c(new_n56_), .d(new_n47_), .O(z18));
endmodule


