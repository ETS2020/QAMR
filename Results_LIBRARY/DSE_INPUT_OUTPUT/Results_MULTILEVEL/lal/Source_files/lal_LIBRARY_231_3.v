// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x18), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  nor3   g03(.a(x25), .b(x23), .c(x20), .O(new_n49_));
  oai21  g04(.a(new_n49_), .b(x15), .c(x18), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x19), .b(x17), .c(new_n53_), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n54_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n51_), .c(new_n52_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n50_), .O(z01));
  and2   g16(.a(new_n47_), .b(x16), .O(z02));
  oai21  g17(.a(new_n57_), .b(x23), .c(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n47_), .O(new_n64_));
  inv1   g19(.a(x23), .O(new_n65_));
  inv1   g20(.a(x18), .O(new_n66_));
  oai21  g21(.a(x19), .b(x17), .c(new_n66_), .O(new_n67_));
  oai21  g22(.a(new_n66_), .b(x15), .c(new_n67_), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n65_), .c(new_n53_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n64_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n47_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g35(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x17), .O(new_n82_));
  inv1   g37(.a(x19), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n66_), .c(new_n82_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n53_), .c(new_n56_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x21), .c(x23), .O(new_n86_));
  aoi21  g41(.a(x18), .b(x15), .c(x25), .O(new_n87_));
  oai21  g42(.a(new_n86_), .b(new_n51_), .c(new_n87_), .O(z08));
  nand2  g43(.a(x05), .b(x04), .O(new_n89_));
  inv1   g44(.a(x07), .O(new_n90_));
  inv1   g45(.a(x15), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n89_), .c(new_n47_), .O(z09));
  nand4  g48(.a(new_n89_), .b(new_n82_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n47_), .O(z10));
  nand3  g50(.a(new_n66_), .b(x17), .c(new_n91_), .O(new_n96_));
  oai21  g51(.a(new_n66_), .b(x17), .c(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n89_), .c(new_n90_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n47_), .O(z11));
  and2   g54(.a(x18), .b(x17), .O(new_n100_));
  nand3  g55(.a(new_n83_), .b(x18), .c(x17), .O(new_n101_));
  oai21  g56(.a(new_n100_), .b(new_n83_), .c(new_n101_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n89_), .c(new_n91_), .d(new_n90_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z12));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nor2   g61(.a(x20), .b(new_n83_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n100_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n60_), .d(new_n91_), .O(z13));
  nand2  g64(.a(new_n108_), .b(x21), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n100_), .c(x19), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n110_), .c(new_n60_), .d(new_n91_), .O(z14));
  nand2  g68(.a(new_n112_), .b(x22), .O(new_n114_));
  inv1   g69(.a(new_n105_), .O(new_n115_));
  nor2   g70(.a(x22), .b(x21), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n53_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n114_), .c(new_n60_), .d(new_n91_), .O(z15));
  nand2  g73(.a(new_n117_), .b(x23), .O(new_n119_));
  nand4  g74(.a(new_n111_), .b(new_n115_), .c(new_n65_), .d(new_n56_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n119_), .c(new_n60_), .d(new_n91_), .O(z16));
  oai21  g76(.a(x24), .b(x15), .c(new_n66_), .O(new_n122_));
  nand3  g77(.a(new_n53_), .b(x19), .c(x17), .O(new_n123_));
  nand3  g78(.a(new_n65_), .b(new_n56_), .c(new_n55_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n123_), .c(x24), .O(new_n125_));
  nor2   g80(.a(x24), .b(x23), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n116_), .c(new_n107_), .d(new_n100_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n125_), .c(new_n89_), .d(new_n90_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n91_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n122_), .O(z17));
  nand2  g85(.a(new_n127_), .b(x25), .O(new_n131_));
  nor3   g86(.a(x25), .b(x24), .c(x23), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n116_), .c(new_n107_), .d(new_n100_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n131_), .c(new_n60_), .d(new_n91_), .O(z18));
endmodule


