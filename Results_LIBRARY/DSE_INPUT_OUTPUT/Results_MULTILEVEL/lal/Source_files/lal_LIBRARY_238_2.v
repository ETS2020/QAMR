// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x17), .O(new_n47_));
  nor2   g02(.a(x20), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x21), .O(new_n51_));
  inv1   g06(.a(x22), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  oai21  g08(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(x24), .c(x25), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n55_), .c(new_n49_), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x18), .O(new_n61_));
  inv1   g16(.a(x19), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n61_), .c(x25), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n53_), .c(new_n60_), .d(new_n47_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n59_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x24), .O(new_n68_));
  inv1   g23(.a(x25), .O(new_n69_));
  nor2   g24(.a(new_n52_), .b(new_n51_), .O(new_n70_));
  oai21  g25(.a(x19), .b(x18), .c(new_n60_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n70_), .c(x23), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(new_n68_), .c(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z03));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g32(.a(x12), .b(x03), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n49_), .c(new_n46_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z04));
  nor3   g36(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n49_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand3  g40(.a(new_n71_), .b(x22), .c(x21), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x24), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n49_), .c(new_n69_), .O(z08));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n56_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n57_), .c(new_n49_), .O(z09));
  nand4  g47(.a(new_n57_), .b(new_n47_), .c(new_n90_), .d(new_n56_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  xor2a  g49(.a(x18), .b(x17), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n57_), .c(new_n90_), .d(new_n56_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n49_), .O(z11));
  oai21  g52(.a(new_n61_), .b(new_n47_), .c(x19), .O(new_n98_));
  nand3  g53(.a(new_n62_), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n57_), .c(new_n90_), .d(new_n56_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n49_), .O(z12));
  nand3  g57(.a(new_n57_), .b(new_n90_), .c(new_n56_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  oai21  g61(.a(new_n104_), .b(new_n48_), .c(new_n106_), .O(z13));
  nand3  g62(.a(new_n57_), .b(new_n49_), .c(new_n56_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n51_), .c(new_n90_), .O(z14));
  nand3  g65(.a(new_n109_), .b(new_n52_), .c(new_n90_), .O(z15));
  nand3  g66(.a(new_n109_), .b(new_n53_), .c(new_n90_), .O(z16));
  nand3  g67(.a(new_n109_), .b(new_n68_), .c(new_n90_), .O(z17));
  nand3  g68(.a(new_n109_), .b(new_n69_), .c(new_n90_), .O(z18));
endmodule


