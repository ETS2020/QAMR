// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n111_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x21), .b(x15), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x21), .O(new_n50_));
  oai21  g05(.a(x25), .b(x23), .c(x15), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x22), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n54_), .c(new_n55_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n52_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n47_), .b(new_n65_), .O(z02));
  oai21  g21(.a(x23), .b(x21), .c(x24), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(x15), .O(new_n68_));
  nand2  g23(.a(new_n61_), .b(x21), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n48_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n48_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g36(.a(x24), .b(x23), .c(x25), .O(new_n82_));
  nand2  g37(.a(new_n59_), .b(new_n54_), .O(new_n83_));
  nand4  g38(.a(new_n83_), .b(x24), .c(x22), .d(x21), .O(new_n84_));
  oai21  g39(.a(new_n82_), .b(new_n47_), .c(new_n84_), .O(z08));
  nand2  g40(.a(x05), .b(x04), .O(new_n86_));
  nor4   g41(.a(new_n86_), .b(new_n50_), .c(x15), .d(x07), .O(z09));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n86_), .b(new_n56_), .c(new_n88_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x21), .c(x15), .O(z10));
  inv1   g45(.a(x15), .O(new_n91_));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n86_), .c(x21), .d(new_n91_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(x07), .O(z11));
  oai21  g49(.a(new_n57_), .b(new_n56_), .c(x19), .O(new_n95_));
  nand3  g50(.a(new_n58_), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n86_), .c(x21), .d(new_n91_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z12));
  nand2  g54(.a(x19), .b(x18), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n56_), .c(x20), .O(new_n101_));
  nand4  g56(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n101_), .c(new_n86_), .d(new_n88_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x21), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n91_), .O(z13));
  inv1   g60(.a(new_n100_), .O(new_n106_));
  nor2   g61(.a(new_n50_), .b(x20), .O(new_n107_));
  nor2   g62(.a(new_n56_), .b(x15), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n63_), .O(z14));
  nand4  g64(.a(new_n63_), .b(new_n55_), .c(x21), .d(new_n91_), .O(z15));
  inv1   g65(.a(x23), .O(new_n111_));
  nand4  g66(.a(new_n63_), .b(new_n111_), .c(x21), .d(new_n91_), .O(z16));
  inv1   g67(.a(x24), .O(new_n113_));
  nand3  g68(.a(new_n86_), .b(new_n113_), .c(new_n88_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x21), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n91_), .O(z17));
  nand3  g71(.a(new_n86_), .b(new_n53_), .c(new_n88_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x21), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n91_), .O(z18));
endmodule


