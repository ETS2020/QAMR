// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x16), .O(new_n46_));
  nand2  g001(.a(x20), .b(x19), .O(new_n47_));
  nand3  g002(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(z00));
  nor2   g004(.a(x25), .b(x23), .O(new_n50_));
  oai21  g005(.a(new_n50_), .b(x20), .c(x19), .O(new_n51_));
  inv1   g006(.a(x24), .O(new_n52_));
  inv1   g007(.a(x25), .O(new_n53_));
  inv1   g008(.a(x20), .O(new_n54_));
  oai21  g009(.a(x18), .b(x17), .c(new_n54_), .O(new_n55_));
  inv1   g010(.a(x21), .O(new_n56_));
  inv1   g011(.a(x22), .O(new_n57_));
  nor2   g012(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g013(.a(new_n58_), .b(new_n55_), .c(x23), .O(new_n59_));
  oai21  g014(.a(new_n59_), .b(new_n52_), .c(new_n53_), .O(new_n60_));
  aoi21  g015(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  nand3  g016(.a(new_n61_), .b(new_n60_), .c(new_n51_), .O(z01));
  inv1   g017(.a(new_n47_), .O(new_n63_));
  nor2   g018(.a(new_n63_), .b(new_n46_), .O(z02));
  nand2  g019(.a(new_n47_), .b(new_n52_), .O(new_n65_));
  inv1   g020(.a(x23), .O(new_n66_));
  aoi21  g021(.a(x22), .b(x21), .c(x19), .O(new_n67_));
  inv1   g022(.a(x17), .O(new_n68_));
  nor2   g023(.a(x19), .b(x18), .O(new_n69_));
  aoi21  g024(.a(new_n69_), .b(new_n68_), .c(x20), .O(new_n70_));
  oai21  g025(.a(new_n70_), .b(new_n67_), .c(new_n66_), .O(new_n71_));
  aoi21  g026(.a(new_n71_), .b(new_n65_), .c(x25), .O(z03));
  inv1   g027(.a(x08), .O(new_n73_));
  xnor2a g028(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g029(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g030(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g031(.a(x12), .b(x03), .O(new_n77_));
  nand4  g032(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n73_), .O(new_n79_));
  nand2  g034(.a(new_n79_), .b(new_n47_), .O(z04));
  inv1   g035(.a(x13), .O(new_n81_));
  nand3  g036(.a(new_n47_), .b(new_n81_), .c(new_n73_), .O(new_n82_));
  inv1   g037(.a(new_n82_), .O(z05));
  inv1   g038(.a(x14), .O(new_n84_));
  oai21  g039(.a(new_n84_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g040(.a(new_n73_), .b(x06), .c(new_n63_), .O(z07));
  aoi21  g041(.a(x24), .b(x23), .c(x25), .O(new_n87_));
  nand4  g042(.a(new_n55_), .b(x24), .c(x22), .d(x21), .O(new_n88_));
  oai22  g043(.a(new_n88_), .b(x19), .c(new_n87_), .d(new_n63_), .O(z08));
  nand2  g044(.a(x05), .b(x04), .O(new_n90_));
  inv1   g045(.a(x07), .O(new_n91_));
  inv1   g046(.a(x15), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g048(.a(new_n93_), .b(new_n90_), .c(new_n47_), .O(z09));
  nand4  g049(.a(new_n90_), .b(new_n47_), .c(new_n68_), .d(new_n92_), .O(new_n95_));
  nor2   g050(.a(new_n95_), .b(x07), .O(z10));
  xor2a  g051(.a(x18), .b(x17), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n90_), .c(new_n47_), .d(new_n92_), .O(new_n98_));
  nor2   g053(.a(new_n98_), .b(x07), .O(z11));
  nand2  g054(.a(x18), .b(x17), .O(new_n100_));
  nand3  g055(.a(new_n100_), .b(new_n54_), .c(x19), .O(new_n101_));
  inv1   g056(.a(x19), .O(new_n102_));
  nand3  g057(.a(new_n102_), .b(x18), .c(x17), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand4  g059(.a(new_n104_), .b(new_n90_), .c(new_n92_), .d(new_n91_), .O(new_n105_));
  inv1   g060(.a(new_n105_), .O(z12));
  nand3  g061(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand3  g062(.a(new_n107_), .b(new_n61_), .c(new_n92_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n54_), .O(new_n109_));
  oai21  g064(.a(new_n54_), .b(x19), .c(new_n109_), .O(z13));
  nand3  g065(.a(new_n56_), .b(x18), .c(x17), .O(new_n111_));
  nand2  g066(.a(new_n111_), .b(new_n54_), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(x19), .O(new_n113_));
  inv1   g068(.a(new_n100_), .O(new_n114_));
  nand3  g069(.a(new_n114_), .b(new_n54_), .c(x19), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(x21), .O(new_n116_));
  nand3  g071(.a(new_n90_), .b(new_n92_), .c(new_n91_), .O(new_n117_));
  inv1   g072(.a(new_n117_), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n116_), .c(new_n113_), .O(z14));
  nand2  g074(.a(new_n117_), .b(new_n47_), .O(new_n120_));
  nand2  g075(.a(new_n111_), .b(x22), .O(new_n121_));
  nor2   g076(.a(x22), .b(x21), .O(new_n122_));
  nand3  g077(.a(new_n122_), .b(new_n114_), .c(x19), .O(new_n123_));
  nand2  g078(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g079(.a(new_n124_), .b(new_n54_), .O(new_n125_));
  nand2  g080(.a(x22), .b(new_n102_), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n125_), .c(new_n120_), .O(z15));
  nor2   g082(.a(x23), .b(x22), .O(new_n128_));
  nand3  g083(.a(new_n128_), .b(new_n114_), .c(new_n56_), .O(new_n129_));
  nand2  g084(.a(new_n129_), .b(new_n54_), .O(new_n130_));
  nand2  g085(.a(new_n130_), .b(x19), .O(new_n131_));
  inv1   g086(.a(new_n107_), .O(new_n132_));
  nand3  g087(.a(new_n122_), .b(new_n132_), .c(new_n54_), .O(new_n133_));
  aoi21  g088(.a(new_n133_), .b(x23), .c(new_n117_), .O(new_n134_));
  nand2  g089(.a(new_n134_), .b(new_n131_), .O(z16));
  nand3  g090(.a(new_n52_), .b(new_n66_), .c(new_n57_), .O(new_n136_));
  oai21  g091(.a(new_n136_), .b(new_n111_), .c(new_n54_), .O(new_n137_));
  nand2  g092(.a(new_n137_), .b(x19), .O(new_n138_));
  nand3  g093(.a(new_n128_), .b(new_n56_), .c(new_n54_), .O(new_n139_));
  oai21  g094(.a(new_n139_), .b(new_n107_), .c(x24), .O(new_n140_));
  nand3  g095(.a(new_n140_), .b(new_n138_), .c(new_n118_), .O(z17));
  nand3  g096(.a(new_n128_), .b(new_n53_), .c(new_n52_), .O(new_n142_));
  oai21  g097(.a(new_n142_), .b(new_n111_), .c(new_n54_), .O(new_n143_));
  nand2  g098(.a(new_n143_), .b(x19), .O(new_n144_));
  nand3  g099(.a(new_n122_), .b(new_n52_), .c(new_n66_), .O(new_n145_));
  oai21  g100(.a(new_n145_), .b(new_n115_), .c(x25), .O(new_n146_));
  nand3  g101(.a(new_n146_), .b(new_n144_), .c(new_n118_), .O(z18));
endmodule


