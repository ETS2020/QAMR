// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n113_, new_n114_, new_n116_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  nor2   g02(.a(x20), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x19), .b(x17), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n54_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n51_), .c(new_n52_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor3   g15(.a(new_n60_), .b(new_n48_), .c(x07), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n58_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n48_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n58_), .b(new_n49_), .O(z03));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n49_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g28(.a(x14), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n49_), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g31(.a(x23), .O(new_n77_));
  inv1   g32(.a(x17), .O(new_n78_));
  inv1   g33(.a(x19), .O(new_n79_));
  nand4  g34(.a(x22), .b(x21), .c(new_n79_), .d(new_n78_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n77_), .c(x18), .O(new_n81_));
  aoi21  g36(.a(new_n55_), .b(new_n77_), .c(new_n53_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n81_), .c(x24), .O(new_n83_));
  nand2  g38(.a(new_n49_), .b(x25), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(z08));
  inv1   g40(.a(x04), .O(new_n86_));
  inv1   g41(.a(x07), .O(new_n87_));
  inv1   g42(.a(x15), .O(new_n88_));
  nand4  g43(.a(new_n49_), .b(new_n88_), .c(new_n87_), .d(x05), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n86_), .O(z09));
  nand4  g45(.a(new_n59_), .b(new_n49_), .c(new_n78_), .d(new_n88_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x07), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n59_), .c(new_n88_), .d(new_n87_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n49_), .O(z11));
  oai21  g50(.a(new_n47_), .b(new_n78_), .c(x19), .O(new_n96_));
  nand3  g51(.a(new_n79_), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n59_), .c(new_n88_), .d(new_n87_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n49_), .O(z12));
  nand3  g55(.a(new_n59_), .b(new_n88_), .c(new_n87_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  oai21  g59(.a(new_n102_), .b(new_n48_), .c(new_n104_), .O(z13));
  nor2   g60(.a(x21), .b(x15), .O(new_n106_));
  nand3  g61(.a(new_n59_), .b(new_n49_), .c(new_n87_), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n106_), .O(z14));
  nor2   g64(.a(x22), .b(x15), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n108_), .O(z15));
  nand3  g66(.a(new_n108_), .b(new_n77_), .c(new_n88_), .O(z16));
  nor2   g67(.a(new_n60_), .b(x07), .O(new_n113_));
  nor2   g68(.a(x24), .b(x15), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(new_n48_), .O(z17));
  nor2   g70(.a(x25), .b(x15), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n113_), .c(new_n48_), .O(z18));
endmodule


