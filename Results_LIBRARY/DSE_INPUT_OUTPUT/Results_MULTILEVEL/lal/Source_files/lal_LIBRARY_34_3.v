// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x10), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  nand2  g18(.a(new_n60_), .b(new_n47_), .O(z03));
  inv1   g19(.a(x09), .O(new_n65_));
  inv1   g20(.a(x02), .O(new_n66_));
  aoi22  g21(.a(x11), .b(new_n66_), .c(new_n65_), .d(x00), .O(new_n67_));
  oai21  g22(.a(new_n65_), .b(x00), .c(new_n67_), .O(new_n68_));
  inv1   g23(.a(x11), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(x02), .O(new_n70_));
  inv1   g25(.a(x03), .O(new_n71_));
  nand2  g26(.a(x12), .b(new_n71_), .O(new_n72_));
  inv1   g27(.a(x12), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x03), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n72_), .c(new_n70_), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(new_n68_), .c(new_n47_), .O(new_n76_));
  inv1   g31(.a(x01), .O(new_n77_));
  nor2   g32(.a(x10), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(x10), .O(new_n79_));
  nor2   g34(.a(x15), .b(new_n79_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n77_), .c(new_n78_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n76_), .c(x08), .O(z04));
  inv1   g37(.a(x13), .O(new_n83_));
  nand3  g38(.a(new_n47_), .b(new_n83_), .c(new_n46_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z05));
  inv1   g40(.a(x14), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g42(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g43(.a(x23), .O(new_n89_));
  nand3  g44(.a(new_n57_), .b(x22), .c(x21), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(x24), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n51_), .c(new_n49_), .O(z08));
  nand2  g48(.a(x05), .b(x04), .O(new_n94_));
  inv1   g49(.a(x07), .O(new_n95_));
  inv1   g50(.a(x15), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n94_), .c(new_n47_), .O(z09));
  nand4  g53(.a(new_n94_), .b(new_n53_), .c(new_n96_), .d(new_n95_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z10));
  xor2a  g55(.a(x18), .b(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n94_), .c(new_n96_), .d(new_n95_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z11));
  nor2   g58(.a(new_n54_), .b(new_n53_), .O(new_n104_));
  nand3  g59(.a(new_n55_), .b(x18), .c(x17), .O(new_n105_));
  oai21  g60(.a(new_n104_), .b(new_n55_), .c(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n94_), .c(new_n96_), .d(new_n95_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(z12));
  nand3  g63(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x20), .O(new_n110_));
  nor2   g65(.a(x20), .b(new_n55_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n104_), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n110_), .c(new_n61_), .d(new_n96_), .O(z13));
  nand2  g68(.a(new_n112_), .b(x21), .O(new_n114_));
  nor2   g69(.a(x21), .b(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n104_), .c(x19), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n114_), .c(new_n61_), .d(new_n96_), .O(z14));
  inv1   g72(.a(new_n109_), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n52_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n94_), .c(new_n95_), .O(new_n121_));
  aoi21  g76(.a(new_n116_), .b(x22), .c(new_n121_), .O(new_n122_));
  nand2  g77(.a(x15), .b(new_n79_), .O(new_n123_));
  oai21  g78(.a(new_n122_), .b(x15), .c(new_n123_), .O(z15));
  nand2  g79(.a(new_n120_), .b(x23), .O(new_n125_));
  nor2   g80(.a(x23), .b(x22), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n115_), .c(new_n118_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n125_), .c(new_n61_), .d(new_n96_), .O(z16));
  nand2  g83(.a(new_n127_), .b(x24), .O(new_n129_));
  nor2   g84(.a(x24), .b(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n119_), .c(new_n111_), .d(new_n104_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n94_), .d(new_n95_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n96_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n123_), .O(z17));
  nand2  g89(.a(new_n131_), .b(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n119_), .c(new_n111_), .d(new_n104_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n61_), .d(new_n96_), .O(z18));
endmodule


