// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x18), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  nor3   g03(.a(x25), .b(x23), .c(x20), .O(new_n49_));
  oai21  g04(.a(new_n49_), .b(x15), .c(x18), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x19), .b(x17), .c(new_n53_), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n54_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n51_), .c(new_n52_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n50_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n47_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n59_), .b(new_n50_), .O(z03));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(new_n47_), .c(new_n46_), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g27(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z06));
  nand3  g29(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g30(.a(x17), .O(new_n76_));
  inv1   g31(.a(x18), .O(new_n77_));
  inv1   g32(.a(x19), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n53_), .c(new_n56_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(x21), .c(x23), .O(new_n81_));
  aoi21  g36(.a(x18), .b(x15), .c(x25), .O(new_n82_));
  oai21  g37(.a(new_n81_), .b(new_n51_), .c(new_n82_), .O(z08));
  nand2  g38(.a(x05), .b(x04), .O(new_n84_));
  inv1   g39(.a(x07), .O(new_n85_));
  inv1   g40(.a(x15), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n84_), .c(new_n47_), .O(z09));
  nand4  g43(.a(new_n84_), .b(new_n76_), .c(new_n86_), .d(new_n85_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n84_), .c(new_n86_), .d(new_n85_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z11));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(x19), .c(new_n86_), .O(new_n95_));
  nand3  g50(.a(new_n78_), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n84_), .c(new_n85_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n47_), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n94_), .O(new_n102_));
  nor2   g57(.a(x20), .b(new_n78_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n101_), .c(new_n60_), .d(new_n86_), .O(z13));
  nand2  g60(.a(new_n104_), .b(x21), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n102_), .c(x19), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n60_), .d(new_n86_), .O(z14));
  oai21  g64(.a(x22), .b(x15), .c(new_n77_), .O(new_n110_));
  nand3  g65(.a(new_n107_), .b(x19), .c(x17), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x22), .O(new_n112_));
  inv1   g67(.a(new_n100_), .O(new_n113_));
  nor2   g68(.a(x22), .b(x21), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n53_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n84_), .d(new_n85_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n86_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n110_), .O(z15));
  nand2  g73(.a(new_n115_), .b(x23), .O(new_n119_));
  inv1   g74(.a(x23), .O(new_n120_));
  nand4  g75(.a(new_n107_), .b(new_n113_), .c(new_n120_), .d(new_n56_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n119_), .c(new_n60_), .d(new_n86_), .O(z16));
  nand2  g77(.a(new_n121_), .b(x24), .O(new_n123_));
  nand4  g78(.a(new_n51_), .b(new_n120_), .c(new_n56_), .d(new_n55_), .O(new_n124_));
  or2    g79(.a(new_n124_), .b(new_n104_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n123_), .c(new_n60_), .d(new_n86_), .O(z17));
  oai21  g81(.a(x25), .b(x15), .c(new_n77_), .O(new_n127_));
  nand3  g82(.a(new_n53_), .b(x19), .c(x17), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n124_), .c(x25), .O(new_n129_));
  nor3   g84(.a(x25), .b(x24), .c(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n114_), .c(new_n103_), .d(new_n102_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n84_), .d(new_n85_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n86_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n127_), .O(z18));
endmodule


