// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  inv1   g15(.a(x21), .O(new_n61_));
  inv1   g16(.a(x22), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(x23), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n53_), .c(new_n54_), .O(new_n65_));
  nand2  g20(.a(x05), .b(x04), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(x07), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n65_), .O(z01));
  nand2  g24(.a(new_n50_), .b(new_n46_), .O(z02));
  nand2  g25(.a(new_n65_), .b(new_n50_), .O(z03));
  inv1   g26(.a(x08), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  xnor2a g29(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n50_), .O(z04));
  inv1   g34(.a(x13), .O(new_n80_));
  nand3  g35(.a(new_n50_), .b(new_n80_), .c(new_n72_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  nand2  g37(.a(x14), .b(new_n72_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(z06));
  aoi21  g39(.a(new_n72_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g40(.a(new_n59_), .b(new_n55_), .c(new_n62_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x21), .c(x23), .O(new_n87_));
  nor2   g42(.a(new_n49_), .b(x25), .O(new_n88_));
  oai21  g43(.a(new_n87_), .b(new_n53_), .c(new_n88_), .O(z08));
  nand3  g44(.a(new_n67_), .b(new_n48_), .c(new_n47_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand4  g46(.a(new_n66_), .b(new_n56_), .c(new_n48_), .d(new_n47_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n66_), .c(new_n48_), .d(new_n47_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n50_), .O(z11));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n58_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n98_), .b(new_n58_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n66_), .c(new_n48_), .d(new_n47_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n50_), .O(z12));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nand4  g59(.a(new_n55_), .b(x19), .c(x18), .d(x17), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n104_), .c(new_n66_), .d(new_n48_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n47_), .O(new_n107_));
  nand2  g62(.a(new_n48_), .b(x07), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(z13));
  nand2  g64(.a(new_n105_), .b(x21), .O(new_n110_));
  nor2   g65(.a(x21), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n98_), .c(x19), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n110_), .c(new_n68_), .d(new_n48_), .O(z14));
  nand3  g68(.a(new_n61_), .b(new_n55_), .c(x19), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n97_), .c(x22), .O(new_n115_));
  inv1   g70(.a(new_n103_), .O(new_n116_));
  nor3   g71(.a(x22), .b(x21), .c(x20), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n115_), .c(new_n66_), .d(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n47_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n108_), .O(z15));
  inv1   g76(.a(x23), .O(new_n122_));
  aoi21  g77(.a(new_n117_), .b(new_n116_), .c(new_n122_), .O(new_n123_));
  nand4  g78(.a(new_n111_), .b(new_n116_), .c(new_n122_), .d(new_n62_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n66_), .c(new_n48_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n123_), .c(new_n47_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n108_), .O(z16));
  nand2  g82(.a(new_n124_), .b(x24), .O(new_n128_));
  nand4  g83(.a(new_n53_), .b(new_n122_), .c(new_n62_), .d(new_n61_), .O(new_n129_));
  or2    g84(.a(new_n129_), .b(new_n105_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n128_), .c(new_n68_), .d(new_n48_), .O(z17));
  oai21  g86(.a(new_n129_), .b(new_n105_), .c(x25), .O(new_n132_));
  nor2   g87(.a(x20), .b(new_n58_), .O(new_n133_));
  nor2   g88(.a(x22), .b(x21), .O(new_n134_));
  nor3   g89(.a(x25), .b(x24), .c(x23), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n134_), .c(new_n133_), .d(new_n98_), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n132_), .c(new_n66_), .d(new_n48_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n47_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n108_), .O(z18));
endmodule


