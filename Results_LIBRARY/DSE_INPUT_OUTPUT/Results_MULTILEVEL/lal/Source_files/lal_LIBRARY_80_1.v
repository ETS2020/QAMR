// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n90_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x17), .O(new_n47_));
  nor2   g002(.a(new_n47_), .b(x09), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x09), .O(new_n51_));
  nor3   g006(.a(x25), .b(x23), .c(x20), .O(new_n52_));
  oai21  g007(.a(new_n52_), .b(new_n51_), .c(x17), .O(new_n53_));
  inv1   g008(.a(x24), .O(new_n54_));
  inv1   g009(.a(x25), .O(new_n55_));
  inv1   g010(.a(x20), .O(new_n56_));
  oai21  g011(.a(x19), .b(x18), .c(new_n56_), .O(new_n57_));
  inv1   g012(.a(x21), .O(new_n58_));
  inv1   g013(.a(x22), .O(new_n59_));
  nor2   g014(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g015(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g016(.a(new_n61_), .b(new_n54_), .c(new_n55_), .O(new_n62_));
  nand2  g017(.a(x05), .b(x04), .O(new_n63_));
  inv1   g018(.a(new_n63_), .O(new_n64_));
  nor2   g019(.a(new_n64_), .b(x07), .O(new_n65_));
  nand3  g020(.a(new_n65_), .b(new_n62_), .c(new_n53_), .O(z01));
  inv1   g021(.a(x16), .O(new_n67_));
  nor2   g022(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g023(.a(new_n62_), .b(new_n53_), .O(z03));
  inv1   g024(.a(x01), .O(new_n70_));
  nand2  g025(.a(x10), .b(new_n70_), .O(new_n71_));
  inv1   g026(.a(x10), .O(new_n72_));
  nand2  g027(.a(new_n72_), .b(x01), .O(new_n73_));
  inv1   g028(.a(x02), .O(new_n74_));
  nand2  g029(.a(x11), .b(new_n74_), .O(new_n75_));
  nand3  g030(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  inv1   g031(.a(x11), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(x02), .O(new_n78_));
  inv1   g033(.a(x03), .O(new_n79_));
  nand2  g034(.a(x12), .b(new_n79_), .O(new_n80_));
  inv1   g035(.a(x12), .O(new_n81_));
  nand2  g036(.a(new_n81_), .b(x03), .O(new_n82_));
  nand3  g037(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(new_n83_));
  oai21  g038(.a(new_n83_), .b(new_n76_), .c(new_n49_), .O(new_n84_));
  nor2   g039(.a(new_n51_), .b(x00), .O(new_n85_));
  nor2   g040(.a(x17), .b(x09), .O(new_n86_));
  aoi21  g041(.a(new_n86_), .b(x00), .c(new_n85_), .O(new_n87_));
  aoi21  g042(.a(new_n87_), .b(new_n84_), .c(x08), .O(z04));
  nor3   g043(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g044(.a(x14), .O(new_n90_));
  nor3   g045(.a(new_n48_), .b(new_n90_), .c(x08), .O(z06));
  nand3  g046(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g047(.a(x18), .O(new_n93_));
  inv1   g048(.a(x19), .O(new_n94_));
  nand3  g049(.a(new_n94_), .b(new_n93_), .c(new_n47_), .O(new_n95_));
  aoi21  g050(.a(new_n95_), .b(new_n56_), .c(new_n59_), .O(new_n96_));
  aoi21  g051(.a(new_n96_), .b(x21), .c(x23), .O(new_n97_));
  nor2   g052(.a(new_n48_), .b(x25), .O(new_n98_));
  oai21  g053(.a(new_n97_), .b(new_n54_), .c(new_n98_), .O(z08));
  inv1   g054(.a(x07), .O(new_n100_));
  inv1   g055(.a(x15), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  oai21  g057(.a(new_n102_), .b(new_n63_), .c(new_n49_), .O(z09));
  nand4  g058(.a(new_n63_), .b(new_n47_), .c(new_n101_), .d(new_n100_), .O(new_n104_));
  nand2  g059(.a(new_n104_), .b(new_n49_), .O(z10));
  nand3  g060(.a(new_n93_), .b(x17), .c(x09), .O(new_n106_));
  oai21  g061(.a(new_n93_), .b(x17), .c(new_n106_), .O(new_n107_));
  nand4  g062(.a(new_n107_), .b(new_n63_), .c(new_n101_), .d(new_n100_), .O(new_n108_));
  inv1   g063(.a(new_n108_), .O(z11));
  nor2   g064(.a(new_n93_), .b(new_n47_), .O(new_n110_));
  nand3  g065(.a(new_n94_), .b(x18), .c(x17), .O(new_n111_));
  oai21  g066(.a(new_n110_), .b(new_n94_), .c(new_n111_), .O(new_n112_));
  nand4  g067(.a(new_n112_), .b(new_n63_), .c(new_n101_), .d(new_n100_), .O(new_n113_));
  nand2  g068(.a(new_n113_), .b(new_n49_), .O(z12));
  oai21  g069(.a(x20), .b(x17), .c(new_n51_), .O(new_n115_));
  nand3  g070(.a(x19), .b(x18), .c(x17), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(x20), .O(new_n117_));
  nor2   g072(.a(x20), .b(new_n94_), .O(new_n118_));
  aoi21  g073(.a(new_n118_), .b(new_n110_), .c(x15), .O(new_n119_));
  nand4  g074(.a(new_n119_), .b(new_n117_), .c(new_n115_), .d(new_n65_), .O(z13));
  oai21  g075(.a(new_n102_), .b(new_n64_), .c(new_n49_), .O(new_n121_));
  nand3  g076(.a(new_n56_), .b(x19), .c(x18), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(x21), .O(new_n123_));
  nor2   g078(.a(x21), .b(x20), .O(new_n124_));
  nand3  g079(.a(new_n124_), .b(new_n110_), .c(x19), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g081(.a(new_n126_), .b(x09), .O(new_n127_));
  nand2  g082(.a(x21), .b(new_n47_), .O(new_n128_));
  nand3  g083(.a(new_n128_), .b(new_n127_), .c(new_n121_), .O(z14));
  nand2  g084(.a(x19), .b(x18), .O(new_n130_));
  inv1   g085(.a(new_n130_), .O(new_n131_));
  aoi21  g086(.a(new_n124_), .b(new_n131_), .c(new_n59_), .O(new_n132_));
  nand3  g087(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(new_n133_));
  nor2   g088(.a(new_n133_), .b(new_n116_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(new_n132_), .c(x09), .O(new_n135_));
  nand2  g090(.a(x22), .b(new_n47_), .O(new_n136_));
  nand3  g091(.a(new_n136_), .b(new_n135_), .c(new_n121_), .O(z15));
  oai21  g092(.a(new_n133_), .b(new_n130_), .c(x23), .O(new_n138_));
  inv1   g093(.a(x23), .O(new_n139_));
  inv1   g094(.a(new_n116_), .O(new_n140_));
  nand4  g095(.a(new_n124_), .b(new_n140_), .c(new_n139_), .d(new_n59_), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(x09), .O(new_n143_));
  nand2  g098(.a(x23), .b(new_n47_), .O(new_n144_));
  nand3  g099(.a(new_n144_), .b(new_n143_), .c(new_n121_), .O(z16));
  nand3  g100(.a(new_n139_), .b(new_n59_), .c(new_n58_), .O(new_n146_));
  oai21  g101(.a(new_n146_), .b(new_n122_), .c(x24), .O(new_n147_));
  nor2   g102(.a(x22), .b(x21), .O(new_n148_));
  nor2   g103(.a(x24), .b(x23), .O(new_n149_));
  nand4  g104(.a(new_n149_), .b(new_n148_), .c(new_n118_), .d(new_n110_), .O(new_n150_));
  nand2  g105(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand2  g106(.a(new_n151_), .b(x09), .O(new_n152_));
  nand2  g107(.a(x24), .b(new_n47_), .O(new_n153_));
  nand3  g108(.a(new_n153_), .b(new_n152_), .c(new_n121_), .O(z17));
  nand4  g109(.a(new_n54_), .b(new_n139_), .c(new_n59_), .d(new_n58_), .O(new_n155_));
  oai21  g110(.a(new_n155_), .b(new_n122_), .c(x25), .O(new_n156_));
  nor3   g111(.a(x25), .b(x24), .c(x23), .O(new_n157_));
  nand4  g112(.a(new_n157_), .b(new_n148_), .c(new_n118_), .d(new_n110_), .O(new_n158_));
  nand2  g113(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  nand2  g114(.a(new_n159_), .b(x09), .O(new_n160_));
  nand2  g115(.a(x25), .b(new_n47_), .O(new_n161_));
  nand3  g116(.a(new_n161_), .b(new_n160_), .c(new_n121_), .O(z18));
endmodule


