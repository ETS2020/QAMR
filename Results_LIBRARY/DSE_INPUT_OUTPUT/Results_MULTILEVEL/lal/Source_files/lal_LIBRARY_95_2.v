// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x25), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x15), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  inv1   g16(.a(x15), .O(new_n62_));
  nand2  g17(.a(x24), .b(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(x07), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n64_), .O(z01));
  inv1   g23(.a(new_n64_), .O(z03));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n49_), .c(new_n46_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g32(.a(new_n49_), .b(x14), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(x08), .O(z06));
  nand3  g34(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g35(.a(x23), .O(new_n81_));
  nand3  g36(.a(new_n57_), .b(x22), .c(x21), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(x24), .c(new_n62_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n48_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand3  g41(.a(new_n66_), .b(new_n62_), .c(new_n86_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand4  g43(.a(new_n65_), .b(new_n53_), .c(new_n62_), .d(new_n86_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n49_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n65_), .c(new_n62_), .d(new_n86_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z11));
  nor2   g48(.a(new_n54_), .b(new_n53_), .O(new_n94_));
  nand3  g49(.a(new_n55_), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n94_), .b(new_n55_), .c(new_n95_), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n65_), .c(new_n62_), .d(new_n86_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z12));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x20), .O(new_n100_));
  nand4  g55(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n100_), .c(new_n67_), .d(new_n62_), .O(z13));
  nand2  g57(.a(new_n101_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n94_), .c(x19), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n103_), .c(new_n67_), .d(new_n62_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  inv1   g62(.a(new_n99_), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n108_), .c(x15), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n107_), .c(new_n67_), .O(z15));
  aoi21  g66(.a(new_n109_), .b(new_n108_), .c(new_n81_), .O(new_n112_));
  nand4  g67(.a(new_n104_), .b(new_n108_), .c(new_n81_), .d(new_n59_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n65_), .c(new_n86_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n112_), .c(new_n62_), .O(new_n115_));
  nand2  g70(.a(x25), .b(x15), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(z16));
  nand4  g72(.a(new_n81_), .b(new_n59_), .c(new_n58_), .d(new_n52_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n99_), .c(x24), .O(new_n119_));
  nor2   g74(.a(x20), .b(new_n55_), .O(new_n120_));
  nor2   g75(.a(x22), .b(x21), .O(new_n121_));
  nor2   g76(.a(x24), .b(x23), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n121_), .c(new_n120_), .d(new_n94_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n119_), .c(new_n65_), .d(new_n86_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n62_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n116_), .O(z17));
  nor2   g81(.a(x25), .b(x24), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n121_), .c(new_n81_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n101_), .c(new_n86_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n66_), .c(new_n62_), .O(new_n130_));
  nand4  g85(.a(x19), .b(x18), .c(x17), .d(new_n62_), .O(new_n131_));
  nand3  g86(.a(new_n122_), .b(new_n104_), .c(new_n59_), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(new_n131_), .c(x25), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n130_), .O(z18));
endmodule


