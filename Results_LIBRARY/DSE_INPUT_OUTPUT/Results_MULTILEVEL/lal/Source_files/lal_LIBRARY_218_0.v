// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x25), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x23), .c(x24), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n54_), .c(x15), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n53_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n47_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n63_), .O(z03));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z04));
  nor3   g29(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor3   g31(.a(new_n47_), .b(new_n76_), .c(x08), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g33(.a(x23), .O(new_n79_));
  nand2  g34(.a(new_n59_), .b(new_n55_), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(x22), .c(x21), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(x24), .c(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n54_), .O(z08));
  nor4   g39(.a(new_n51_), .b(new_n54_), .c(x15), .d(x07), .O(z09));
  inv1   g40(.a(x15), .O(new_n86_));
  nand4  g41(.a(new_n51_), .b(x25), .c(new_n56_), .d(new_n86_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(x07), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n51_), .c(new_n50_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x25), .c(x15), .O(z11));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand3  g48(.a(new_n58_), .b(x18), .c(x17), .O(new_n94_));
  oai21  g49(.a(new_n93_), .b(new_n58_), .c(new_n94_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n51_), .c(new_n50_), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(x25), .c(x15), .O(z12));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x20), .O(new_n99_));
  nand3  g54(.a(new_n93_), .b(new_n55_), .c(x19), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n99_), .c(new_n51_), .d(new_n50_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x25), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n86_), .O(z13));
  inv1   g58(.a(new_n52_), .O(new_n104_));
  nand2  g59(.a(new_n100_), .b(x21), .O(new_n105_));
  nor2   g60(.a(x21), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n93_), .c(x19), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(x25), .c(new_n86_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n105_), .c(new_n104_), .O(z14));
  inv1   g65(.a(x21), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n55_), .c(x19), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n92_), .c(x22), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  inv1   g69(.a(new_n98_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n114_), .c(new_n111_), .d(new_n55_), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n113_), .c(new_n51_), .d(new_n50_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x25), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n86_), .O(z15));
  nand2  g74(.a(new_n116_), .b(x23), .O(new_n120_));
  nand4  g75(.a(new_n106_), .b(new_n115_), .c(new_n79_), .d(new_n114_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(x25), .c(new_n86_), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n120_), .c(new_n104_), .O(z16));
  nand2  g79(.a(new_n121_), .b(x24), .O(new_n125_));
  inv1   g80(.a(x24), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n79_), .c(new_n114_), .d(new_n111_), .O(new_n127_));
  nor2   g82(.a(new_n127_), .b(new_n100_), .O(new_n128_));
  nor2   g83(.a(new_n128_), .b(new_n54_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n125_), .c(new_n104_), .d(new_n86_), .O(z17));
  nand3  g85(.a(x25), .b(new_n126_), .c(new_n79_), .O(new_n131_));
  inv1   g86(.a(new_n131_), .O(new_n132_));
  nor4   g87(.a(new_n92_), .b(new_n52_), .c(new_n58_), .d(x15), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n132_), .c(new_n106_), .d(new_n114_), .O(z18));
endmodule


