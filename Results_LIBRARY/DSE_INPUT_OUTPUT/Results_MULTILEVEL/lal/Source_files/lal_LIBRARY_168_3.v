// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x09), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n62_), .c(new_n49_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n47_), .b(new_n65_), .O(z02));
  nand2  g21(.a(new_n62_), .b(new_n47_), .O(z03));
  nor2   g22(.a(x08), .b(x00), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x15), .c(x09), .O(new_n69_));
  inv1   g24(.a(x09), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n69_), .O(z04));
  inv1   g32(.a(x13), .O(new_n78_));
  nand3  g33(.a(new_n47_), .b(new_n78_), .c(new_n46_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z05));
  nand3  g35(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z06));
  aoi21  g37(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g38(.a(x23), .O(new_n84_));
  nand3  g39(.a(new_n57_), .b(x22), .c(x21), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x24), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(new_n51_), .c(new_n49_), .O(z08));
  nand2  g43(.a(x05), .b(x04), .O(new_n89_));
  inv1   g44(.a(x07), .O(new_n90_));
  inv1   g45(.a(x15), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n89_), .c(new_n47_), .O(z09));
  nand4  g48(.a(new_n89_), .b(new_n53_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n89_), .c(new_n91_), .d(new_n90_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nor2   g53(.a(new_n54_), .b(new_n53_), .O(new_n99_));
  nand3  g54(.a(new_n55_), .b(x18), .c(x17), .O(new_n100_));
  oai21  g55(.a(new_n99_), .b(new_n55_), .c(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n89_), .c(new_n91_), .d(new_n90_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(z12));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nor2   g60(.a(x20), .b(new_n55_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n99_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n105_), .c(new_n89_), .d(new_n90_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n91_), .O(new_n109_));
  nand2  g64(.a(x15), .b(new_n70_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n109_), .O(z13));
  aoi21  g66(.a(new_n106_), .b(new_n99_), .c(new_n58_), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n99_), .c(x19), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n89_), .c(new_n90_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n112_), .c(new_n91_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n110_), .O(z14));
  inv1   g72(.a(new_n104_), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n52_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n89_), .c(new_n90_), .O(new_n121_));
  aoi21  g76(.a(new_n114_), .b(x22), .c(new_n121_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(x15), .c(new_n110_), .O(z15));
  nand2  g78(.a(new_n120_), .b(x23), .O(new_n124_));
  nand4  g79(.a(new_n113_), .b(new_n118_), .c(new_n84_), .d(new_n59_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n124_), .c(new_n63_), .d(new_n91_), .O(z16));
  nand2  g81(.a(new_n125_), .b(x24), .O(new_n127_));
  nor2   g82(.a(x24), .b(x23), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n119_), .c(new_n106_), .d(new_n99_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n127_), .c(new_n89_), .d(new_n90_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n91_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n110_), .O(z17));
  nand2  g87(.a(new_n129_), .b(x25), .O(new_n133_));
  nor3   g88(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n119_), .c(new_n106_), .d(new_n99_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n133_), .c(new_n63_), .d(new_n91_), .O(z18));
endmodule


