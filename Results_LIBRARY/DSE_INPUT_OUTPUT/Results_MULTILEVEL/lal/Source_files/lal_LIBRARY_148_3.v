// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x20), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x21), .O(new_n49_));
  inv1   g04(.a(x22), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  oai21  g06(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(x24), .c(x25), .O(new_n53_));
  inv1   g08(.a(x07), .O(new_n54_));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  nor3   g13(.a(x19), .b(x18), .c(x17), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(x25), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n51_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  inv1   g18(.a(x24), .O(new_n64_));
  nor2   g19(.a(new_n50_), .b(new_n49_), .O(new_n65_));
  oai21  g20(.a(new_n59_), .b(x20), .c(new_n65_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n51_), .c(new_n64_), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x25), .c(new_n47_), .O(z03));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand3  g28(.a(new_n73_), .b(new_n47_), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z04));
  inv1   g30(.a(x13), .O(new_n76_));
  nand3  g31(.a(new_n47_), .b(new_n76_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  nand3  g33(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  aoi22  g35(.a(x20), .b(x15), .c(new_n46_), .d(x06), .O(z07));
  inv1   g36(.a(x25), .O(new_n82_));
  inv1   g37(.a(x15), .O(new_n83_));
  nand3  g38(.a(x24), .b(x22), .c(x21), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g41(.a(x17), .O(new_n87_));
  inv1   g42(.a(x18), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g44(.a(x19), .O(new_n90_));
  nand2  g45(.a(new_n65_), .b(new_n90_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n89_), .c(new_n51_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(x24), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n86_), .c(new_n82_), .O(z08));
  nand4  g49(.a(new_n83_), .b(new_n54_), .c(x05), .d(x04), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z09));
  nand4  g51(.a(new_n55_), .b(new_n87_), .c(new_n83_), .d(new_n54_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n47_), .O(z10));
  xor2a  g53(.a(x18), .b(x17), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n55_), .c(new_n83_), .d(new_n54_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z11));
  nor2   g56(.a(new_n88_), .b(new_n87_), .O(new_n102_));
  nand3  g57(.a(new_n90_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n90_), .c(new_n103_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n55_), .c(new_n83_), .d(new_n54_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(z12));
  nand2  g61(.a(new_n56_), .b(new_n47_), .O(new_n107_));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(x15), .c(new_n58_), .O(new_n110_));
  nand3  g65(.a(new_n108_), .b(x20), .c(new_n83_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n107_), .O(z13));
  nor2   g67(.a(x21), .b(new_n90_), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n102_), .c(x15), .O(new_n114_));
  nand3  g69(.a(new_n102_), .b(new_n58_), .c(x19), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(x21), .c(new_n56_), .O(new_n116_));
  oai22  g71(.a(new_n116_), .b(x15), .c(new_n114_), .d(x20), .O(z14));
  inv1   g72(.a(new_n56_), .O(new_n118_));
  nor2   g73(.a(x21), .b(x20), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n102_), .c(x19), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x22), .O(new_n121_));
  nor2   g76(.a(x22), .b(x21), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n109_), .c(new_n58_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n121_), .c(new_n118_), .d(new_n83_), .O(z15));
  nor2   g79(.a(x23), .b(x22), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n49_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n108_), .c(new_n83_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n58_), .O(new_n128_));
  aoi21  g83(.a(new_n123_), .b(x23), .c(new_n56_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(x15), .c(new_n128_), .O(z16));
  nand3  g85(.a(new_n122_), .b(new_n64_), .c(new_n51_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n108_), .c(new_n83_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n58_), .O(new_n133_));
  nand3  g88(.a(new_n125_), .b(new_n119_), .c(new_n109_), .O(new_n134_));
  aoi21  g89(.a(new_n134_), .b(x24), .c(new_n56_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(x15), .c(new_n133_), .O(z17));
  oai21  g91(.a(new_n131_), .b(new_n115_), .c(x25), .O(new_n137_));
  inv1   g92(.a(new_n115_), .O(new_n138_));
  nor3   g93(.a(x25), .b(x24), .c(x23), .O(new_n139_));
  nand3  g94(.a(new_n139_), .b(new_n122_), .c(new_n138_), .O(new_n140_));
  nand4  g95(.a(new_n140_), .b(new_n137_), .c(new_n118_), .d(new_n83_), .O(z18));
endmodule


