// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x15), .b(x10), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(new_n47_), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(new_n50_), .O(z01));
  nor2   g19(.a(new_n50_), .b(new_n46_), .O(z02));
  nand3  g20(.a(new_n59_), .b(new_n47_), .c(new_n51_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  nor2   g22(.a(x08), .b(x01), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x15), .c(x10), .O(new_n69_));
  inv1   g24(.a(x08), .O(new_n70_));
  inv1   g25(.a(x09), .O(new_n71_));
  inv1   g26(.a(x10), .O(new_n72_));
  aoi22  g27(.a(new_n72_), .b(x01), .c(new_n71_), .d(x00), .O(new_n73_));
  oai21  g28(.a(new_n71_), .b(x00), .c(new_n73_), .O(new_n74_));
  xnor2a g29(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(new_n74_), .c(new_n70_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n69_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g35(.a(new_n47_), .b(x14), .c(new_n70_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z06));
  aoi21  g37(.a(new_n70_), .b(x06), .c(new_n50_), .O(z07));
  inv1   g38(.a(x24), .O(new_n84_));
  inv1   g39(.a(x22), .O(new_n85_));
  aoi21  g40(.a(new_n56_), .b(new_n52_), .c(new_n85_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x21), .c(x23), .O(new_n87_));
  nor2   g42(.a(new_n50_), .b(x25), .O(new_n88_));
  oai21  g43(.a(new_n87_), .b(new_n84_), .c(new_n88_), .O(z08));
  inv1   g44(.a(x07), .O(new_n90_));
  inv1   g45(.a(x15), .O(new_n91_));
  nand3  g46(.a(new_n62_), .b(new_n91_), .c(new_n90_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand4  g48(.a(new_n61_), .b(new_n53_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n61_), .c(new_n91_), .d(new_n90_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n47_), .O(z11));
  nor2   g53(.a(new_n54_), .b(new_n53_), .O(new_n99_));
  nand3  g54(.a(new_n55_), .b(x18), .c(x17), .O(new_n100_));
  oai21  g55(.a(new_n99_), .b(new_n55_), .c(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n61_), .c(new_n91_), .d(new_n90_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(z12));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nand4  g60(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n61_), .d(new_n90_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n91_), .O(new_n108_));
  nand2  g63(.a(x15), .b(new_n72_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(z13));
  and2   g65(.a(new_n106_), .b(x21), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n99_), .c(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n61_), .c(new_n90_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n111_), .c(new_n91_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n109_), .O(z14));
  nand2  g71(.a(new_n113_), .b(x22), .O(new_n117_));
  inv1   g72(.a(new_n104_), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n52_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n117_), .c(new_n63_), .d(new_n91_), .O(z15));
  nand2  g76(.a(new_n120_), .b(x23), .O(new_n122_));
  nor2   g77(.a(x23), .b(x22), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n112_), .c(new_n118_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n122_), .c(new_n63_), .d(new_n91_), .O(z16));
  nand2  g80(.a(new_n124_), .b(x24), .O(new_n126_));
  inv1   g81(.a(new_n106_), .O(new_n127_));
  nor2   g82(.a(x24), .b(x23), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n119_), .O(new_n129_));
  inv1   g84(.a(new_n129_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n126_), .c(new_n61_), .d(new_n90_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n91_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n109_), .O(z17));
  oai21  g89(.a(new_n129_), .b(new_n106_), .c(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n119_), .c(new_n127_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n61_), .d(new_n90_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n91_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n109_), .O(z18));
endmodule


