// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n103_, new_n105_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x19), .b(x18), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  aoi21  g06(.a(x22), .b(x21), .c(x23), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(new_n50_), .c(new_n51_), .O(new_n53_));
  aoi21  g08(.a(x05), .b(x04), .c(x07), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  oai21  g14(.a(x19), .b(x17), .c(new_n59_), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(x18), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n51_), .c(new_n58_), .d(new_n57_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n56_), .O(z01));
  inv1   g20(.a(new_n47_), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n46_), .O(z02));
  oai21  g22(.a(new_n52_), .b(new_n50_), .c(new_n47_), .O(new_n68_));
  nand3  g23(.a(new_n63_), .b(new_n58_), .c(new_n57_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  inv1   g25(.a(x08), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n47_), .c(new_n71_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g36(.a(new_n47_), .b(new_n71_), .c(x06), .O(z07));
  nand3  g37(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n58_), .c(new_n50_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x25), .c(new_n47_), .O(new_n85_));
  nor3   g40(.a(x19), .b(x18), .c(x17), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(x24), .c(x22), .d(x21), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n85_), .O(z08));
  nand2  g43(.a(x05), .b(x04), .O(new_n89_));
  inv1   g44(.a(x07), .O(new_n90_));
  inv1   g45(.a(x15), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n89_), .c(new_n47_), .O(z09));
  inv1   g48(.a(x17), .O(new_n94_));
  nand4  g49(.a(new_n89_), .b(new_n47_), .c(new_n94_), .d(new_n91_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z10));
  xor2a  g51(.a(x18), .b(x17), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n89_), .c(new_n91_), .d(new_n90_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n47_), .O(z11));
  oai21  g54(.a(new_n59_), .b(new_n94_), .c(new_n61_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n89_), .c(new_n91_), .d(new_n90_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n47_), .O(z12));
  nor2   g57(.a(x20), .b(x15), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n54_), .c(new_n66_), .O(z13));
  nor2   g59(.a(x21), .b(x15), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n54_), .c(new_n66_), .O(z14));
  nor2   g61(.a(x22), .b(x15), .O(new_n107_));
  nand3  g62(.a(new_n89_), .b(new_n47_), .c(new_n90_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n107_), .O(z15));
  nand3  g65(.a(new_n109_), .b(new_n58_), .c(new_n91_), .O(z16));
  nand3  g66(.a(new_n109_), .b(new_n50_), .c(new_n91_), .O(z17));
  nand3  g67(.a(new_n109_), .b(new_n51_), .c(new_n91_), .O(z18));
endmodule


