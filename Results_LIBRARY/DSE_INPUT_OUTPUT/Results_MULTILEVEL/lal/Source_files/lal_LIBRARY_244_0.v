// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x19), .b(x15), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor3   g16(.a(new_n61_), .b(new_n47_), .c(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n47_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n59_), .b(new_n48_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(new_n48_), .c(new_n46_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z04));
  nor3   g28(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n48_), .O(z06));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x23), .O(new_n78_));
  inv1   g33(.a(x24), .O(new_n79_));
  nand3  g34(.a(x22), .b(x21), .c(x20), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n78_), .c(new_n79_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x25), .c(new_n48_), .O(new_n82_));
  nand3  g37(.a(new_n53_), .b(new_n52_), .c(x15), .O(new_n83_));
  nand4  g38(.a(x24), .b(x22), .c(x21), .d(new_n54_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(z08));
  nor4   g40(.a(new_n60_), .b(new_n54_), .c(x15), .d(x07), .O(z09));
  inv1   g41(.a(x07), .O(new_n87_));
  nand3  g42(.a(new_n60_), .b(new_n52_), .c(new_n87_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x19), .c(x15), .O(z10));
  inv1   g44(.a(x15), .O(new_n90_));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n60_), .c(x19), .d(new_n90_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(x07), .O(z11));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n60_), .c(new_n87_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(x19), .c(x15), .O(z12));
  nand2  g51(.a(new_n94_), .b(x20), .O(new_n97_));
  nor2   g52(.a(new_n61_), .b(x07), .O(new_n98_));
  nand3  g53(.a(new_n51_), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(new_n54_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n98_), .c(new_n97_), .d(new_n90_), .O(z13));
  nand2  g57(.a(new_n99_), .b(x21), .O(new_n103_));
  inv1   g58(.a(new_n94_), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n104_), .c(new_n54_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n103_), .c(new_n98_), .d(new_n90_), .O(z14));
  nand2  g62(.a(new_n105_), .b(new_n104_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x22), .O(new_n109_));
  inv1   g64(.a(x21), .O(new_n110_));
  inv1   g65(.a(x22), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n51_), .O(new_n112_));
  nor2   g67(.a(new_n112_), .b(new_n94_), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n54_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n109_), .c(new_n98_), .d(new_n90_), .O(z15));
  oai21  g70(.a(new_n112_), .b(new_n94_), .c(x23), .O(new_n116_));
  nor3   g71(.a(x23), .b(x22), .c(x21), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n100_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n116_), .c(new_n60_), .d(new_n87_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x19), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n90_), .O(z16));
  nand2  g76(.a(new_n118_), .b(x24), .O(new_n122_));
  nand4  g77(.a(new_n79_), .b(new_n78_), .c(new_n111_), .d(new_n110_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  aoi21  g79(.a(new_n124_), .b(new_n100_), .c(new_n54_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n122_), .c(new_n98_), .d(new_n90_), .O(z17));
  oai21  g81(.a(new_n123_), .b(new_n99_), .c(x25), .O(new_n127_));
  nor2   g82(.a(x23), .b(x22), .O(new_n128_));
  nor2   g83(.a(x25), .b(x24), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n128_), .c(new_n105_), .d(new_n104_), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n127_), .c(new_n60_), .d(new_n87_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(x19), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n90_), .O(z18));
endmodule


