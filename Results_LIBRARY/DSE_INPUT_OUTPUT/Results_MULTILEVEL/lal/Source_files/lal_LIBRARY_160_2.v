// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x19), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x22), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(x24), .O(new_n56_));
  inv1   g11(.a(x07), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n56_), .b(new_n51_), .c(new_n59_), .O(new_n60_));
  inv1   g15(.a(x20), .O(new_n61_));
  inv1   g16(.a(x19), .O(new_n62_));
  nor2   g17(.a(x18), .b(x17), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x15), .c(new_n62_), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n51_), .c(new_n54_), .d(new_n61_), .O(new_n65_));
  oai21  g20(.a(new_n60_), .b(new_n48_), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n56_), .b(new_n49_), .O(new_n69_));
  nand3  g24(.a(new_n64_), .b(new_n54_), .c(new_n61_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n49_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  nor3   g33(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  nor3   g35(.a(new_n48_), .b(new_n80_), .c(x08), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g37(.a(x24), .O(new_n83_));
  nand3  g38(.a(x22), .b(x21), .c(x20), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n54_), .c(new_n83_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(x25), .O(new_n86_));
  nor2   g41(.a(new_n52_), .b(x19), .O(new_n87_));
  nor2   g42(.a(new_n83_), .b(new_n53_), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n87_), .c(new_n63_), .d(new_n47_), .O(new_n89_));
  oai21  g44(.a(new_n86_), .b(new_n48_), .c(new_n89_), .O(z08));
  nand4  g45(.a(new_n47_), .b(new_n57_), .c(x05), .d(x04), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  inv1   g47(.a(x17), .O(new_n93_));
  nand4  g48(.a(new_n58_), .b(new_n93_), .c(new_n47_), .d(new_n57_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n49_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n58_), .c(new_n47_), .d(new_n57_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(x19), .c(new_n47_), .O(new_n100_));
  nand3  g55(.a(new_n62_), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n58_), .c(new_n57_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n49_), .O(z12));
  inv1   g59(.a(new_n59_), .O(new_n105_));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x20), .O(new_n107_));
  nand4  g62(.a(new_n61_), .b(x19), .c(x18), .d(x17), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n105_), .d(new_n47_), .O(z13));
  nand2  g64(.a(new_n108_), .b(x21), .O(new_n110_));
  inv1   g65(.a(new_n99_), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(x19), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n110_), .c(new_n105_), .d(new_n47_), .O(z14));
  nand2  g69(.a(new_n113_), .b(x22), .O(new_n115_));
  inv1   g70(.a(new_n106_), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n53_), .c(new_n52_), .d(new_n61_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n115_), .c(new_n105_), .d(new_n47_), .O(z15));
  nand3  g73(.a(new_n61_), .b(x18), .c(x17), .O(new_n119_));
  nor2   g74(.a(x23), .b(x22), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n52_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n119_), .c(new_n47_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x19), .O(new_n123_));
  aoi21  g78(.a(new_n117_), .b(x23), .c(new_n59_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(x15), .c(new_n123_), .O(z16));
  nand4  g80(.a(new_n83_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n119_), .c(new_n47_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x19), .O(new_n128_));
  nand3  g83(.a(new_n120_), .b(new_n112_), .c(new_n116_), .O(new_n129_));
  aoi21  g84(.a(new_n129_), .b(x24), .c(new_n59_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(x15), .c(new_n128_), .O(z17));
  nand2  g86(.a(new_n112_), .b(new_n111_), .O(new_n132_));
  nand3  g87(.a(new_n120_), .b(new_n51_), .c(new_n83_), .O(new_n133_));
  oai21  g88(.a(new_n133_), .b(new_n132_), .c(new_n47_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(x19), .O(new_n135_));
  oai21  g90(.a(new_n126_), .b(new_n108_), .c(x25), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n105_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n47_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n135_), .O(z18));
endmodule


