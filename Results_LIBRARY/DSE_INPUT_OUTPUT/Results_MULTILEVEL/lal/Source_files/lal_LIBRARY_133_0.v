// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x23), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n51_), .c(new_n50_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  inv1   g11(.a(x23), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(x22), .c(x21), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n53_), .c(new_n57_), .d(x15), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n56_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n47_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n49_), .b(new_n52_), .O(new_n69_));
  nand3  g24(.a(new_n64_), .b(new_n57_), .c(x15), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n49_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  nor3   g33(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n49_), .O(z06));
  nand3  g36(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand4  g37(.a(new_n63_), .b(x24), .c(x22), .d(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n53_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x15), .O(new_n85_));
  nand2  g40(.a(new_n54_), .b(x23), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(z08));
  nor4   g42(.a(new_n51_), .b(new_n57_), .c(x15), .d(x07), .O(z09));
  inv1   g43(.a(x15), .O(new_n89_));
  nand4  g44(.a(new_n51_), .b(x23), .c(new_n59_), .d(new_n89_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x07), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n51_), .c(new_n50_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x23), .c(x15), .O(z11));
  nor2   g49(.a(new_n60_), .b(new_n59_), .O(new_n95_));
  nand3  g50(.a(new_n61_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n61_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n51_), .c(x23), .d(new_n89_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nor2   g56(.a(x20), .b(new_n61_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n95_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n51_), .d(new_n50_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x23), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n89_), .O(z13));
  inv1   g61(.a(x21), .O(new_n107_));
  aoi21  g62(.a(new_n102_), .b(new_n95_), .c(new_n107_), .O(new_n108_));
  nand4  g63(.a(new_n95_), .b(new_n107_), .c(new_n58_), .d(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n51_), .c(new_n50_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n108_), .c(x23), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n89_), .O(z14));
  inv1   g67(.a(new_n100_), .O(new_n113_));
  nor2   g68(.a(x22), .b(x21), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n58_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n51_), .c(new_n50_), .O(new_n116_));
  aoi21  g71(.a(new_n109_), .b(x22), .c(new_n116_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n57_), .c(new_n89_), .O(z15));
  nand2  g73(.a(new_n102_), .b(new_n107_), .O(new_n119_));
  nor3   g74(.a(new_n119_), .b(new_n57_), .c(x22), .O(new_n120_));
  nand3  g75(.a(new_n51_), .b(new_n89_), .c(new_n50_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n95_), .O(z16));
  nand3  g78(.a(new_n51_), .b(new_n52_), .c(new_n50_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x23), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n89_), .O(z17));
  nand3  g81(.a(new_n122_), .b(new_n53_), .c(x23), .O(z18));
endmodule


