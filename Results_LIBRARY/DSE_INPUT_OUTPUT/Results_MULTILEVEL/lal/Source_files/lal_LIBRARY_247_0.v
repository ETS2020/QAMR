// Benchmark "FAU" written by ABC on Fri Aug 14 00:10:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n88_, new_n91_, new_n92_, new_n93_, new_n94_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  nor2   g002(.a(x15), .b(x12), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  nand2  g004(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g005(.a(z02), .b(new_n46_), .O(z00));
  inv1   g006(.a(x07), .O(new_n52_));
  inv1   g007(.a(x24), .O(new_n53_));
  inv1   g008(.a(x25), .O(new_n54_));
  inv1   g009(.a(x20), .O(new_n55_));
  inv1   g010(.a(x17), .O(new_n56_));
  inv1   g011(.a(x18), .O(new_n57_));
  inv1   g012(.a(x19), .O(new_n58_));
  nand3  g013(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g014(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  inv1   g015(.a(x21), .O(new_n61_));
  inv1   g016(.a(x22), .O(new_n62_));
  nor2   g017(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g018(.a(new_n63_), .b(new_n60_), .c(x23), .O(new_n64_));
  oai21  g019(.a(new_n64_), .b(new_n53_), .c(new_n54_), .O(new_n65_));
  nand2  g020(.a(x05), .b(x04), .O(new_n66_));
  nand4  g021(.a(new_n66_), .b(new_n65_), .c(new_n49_), .d(new_n52_), .O(z01));
  nand2  g022(.a(new_n65_), .b(new_n49_), .O(z03));
  inv1   g023(.a(x09), .O(new_n69_));
  inv1   g024(.a(x01), .O(new_n70_));
  aoi22  g025(.a(x10), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g026(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g027(.a(x10), .O(new_n73_));
  nand2  g028(.a(new_n73_), .b(x01), .O(new_n74_));
  inv1   g029(.a(x02), .O(new_n75_));
  nand2  g030(.a(x11), .b(new_n75_), .O(new_n76_));
  inv1   g031(.a(x11), .O(new_n77_));
  nand2  g032(.a(new_n77_), .b(x02), .O(new_n78_));
  nand3  g033(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  oai21  g034(.a(new_n79_), .b(new_n72_), .c(new_n49_), .O(new_n80_));
  inv1   g035(.a(x12), .O(new_n81_));
  nor2   g036(.a(new_n81_), .b(x03), .O(new_n82_));
  inv1   g037(.a(x15), .O(new_n83_));
  nor2   g038(.a(new_n83_), .b(x12), .O(new_n84_));
  aoi21  g039(.a(new_n84_), .b(x03), .c(new_n82_), .O(new_n85_));
  aoi21  g040(.a(new_n85_), .b(new_n80_), .c(x08), .O(z04));
  oai21  g041(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g042(.a(x14), .b(new_n46_), .O(new_n88_));
  nand2  g043(.a(new_n88_), .b(new_n49_), .O(z06));
  aoi21  g044(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g045(.a(x23), .O(new_n91_));
  nand3  g046(.a(new_n60_), .b(x22), .c(x21), .O(new_n92_));
  nand2  g047(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g048(.a(new_n93_), .b(x24), .O(new_n94_));
  aoi21  g049(.a(new_n94_), .b(new_n54_), .c(new_n48_), .O(z08));
  nor4   g050(.a(new_n66_), .b(x15), .c(new_n81_), .d(x07), .O(z09));
  nand3  g051(.a(new_n66_), .b(new_n56_), .c(new_n52_), .O(new_n97_));
  aoi21  g052(.a(new_n97_), .b(x12), .c(x15), .O(z10));
  xor2a  g053(.a(x18), .b(x17), .O(new_n99_));
  nand4  g054(.a(new_n99_), .b(new_n66_), .c(new_n83_), .d(x12), .O(new_n100_));
  nor2   g055(.a(new_n100_), .b(x07), .O(z11));
  nand2  g056(.a(x18), .b(x17), .O(new_n102_));
  inv1   g057(.a(new_n102_), .O(new_n103_));
  nand3  g058(.a(new_n58_), .b(x18), .c(x17), .O(new_n104_));
  oai21  g059(.a(new_n103_), .b(new_n58_), .c(new_n104_), .O(new_n105_));
  nand4  g060(.a(new_n105_), .b(new_n66_), .c(new_n83_), .d(x12), .O(new_n106_));
  nor2   g061(.a(new_n106_), .b(x07), .O(z12));
  nand3  g062(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(x20), .O(new_n109_));
  nor2   g064(.a(x20), .b(new_n58_), .O(new_n110_));
  nand2  g065(.a(new_n110_), .b(new_n103_), .O(new_n111_));
  nand4  g066(.a(new_n111_), .b(new_n109_), .c(new_n66_), .d(new_n52_), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(x12), .O(new_n113_));
  nand2  g068(.a(new_n113_), .b(new_n83_), .O(z13));
  nand2  g069(.a(new_n111_), .b(x21), .O(new_n115_));
  aoi21  g070(.a(x05), .b(x04), .c(x07), .O(new_n116_));
  nor2   g071(.a(x21), .b(x20), .O(new_n117_));
  nand3  g072(.a(new_n117_), .b(new_n103_), .c(x19), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n83_), .c(x12), .O(new_n119_));
  inv1   g074(.a(new_n119_), .O(new_n120_));
  nand3  g075(.a(new_n120_), .b(new_n116_), .c(new_n115_), .O(z14));
  nand2  g076(.a(new_n118_), .b(x22), .O(new_n122_));
  inv1   g077(.a(new_n108_), .O(new_n123_));
  nand2  g078(.a(new_n62_), .b(new_n61_), .O(new_n124_));
  nor2   g079(.a(new_n124_), .b(x20), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand4  g081(.a(new_n126_), .b(new_n122_), .c(new_n66_), .d(new_n52_), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(x12), .O(new_n128_));
  nand2  g083(.a(new_n128_), .b(new_n83_), .O(z15));
  aoi21  g084(.a(new_n125_), .b(new_n123_), .c(new_n91_), .O(new_n130_));
  nand4  g085(.a(new_n117_), .b(new_n123_), .c(new_n91_), .d(new_n62_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n66_), .c(new_n52_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(new_n130_), .c(x12), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n83_), .O(z16));
  nand3  g089(.a(new_n117_), .b(new_n91_), .c(new_n62_), .O(new_n135_));
  oai21  g090(.a(new_n135_), .b(new_n108_), .c(x24), .O(new_n136_));
  inv1   g091(.a(new_n124_), .O(new_n137_));
  nor2   g092(.a(x24), .b(x23), .O(new_n138_));
  nand4  g093(.a(new_n138_), .b(new_n137_), .c(new_n110_), .d(new_n103_), .O(new_n139_));
  nand4  g094(.a(new_n139_), .b(new_n136_), .c(new_n66_), .d(new_n52_), .O(new_n140_));
  nand2  g095(.a(new_n140_), .b(x12), .O(new_n141_));
  nand2  g096(.a(new_n141_), .b(new_n83_), .O(z17));
  nand2  g097(.a(new_n139_), .b(x25), .O(new_n143_));
  inv1   g098(.a(new_n111_), .O(new_n144_));
  nand3  g099(.a(new_n54_), .b(new_n53_), .c(new_n91_), .O(new_n145_));
  nor2   g100(.a(new_n145_), .b(new_n124_), .O(new_n146_));
  aoi21  g101(.a(new_n146_), .b(new_n144_), .c(x15), .O(new_n147_));
  nand4  g102(.a(new_n147_), .b(new_n143_), .c(new_n116_), .d(x12), .O(z18));
endmodule


