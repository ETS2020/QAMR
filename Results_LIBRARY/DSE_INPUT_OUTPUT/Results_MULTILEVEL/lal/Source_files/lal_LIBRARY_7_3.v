// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x02), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n59_), .c(new_n49_), .O(z01));
  and2   g18(.a(new_n47_), .b(x16), .O(z02));
  nand3  g19(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  nor2   g21(.a(x11), .b(x08), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x15), .c(x02), .O(new_n68_));
  inv1   g23(.a(x09), .O(new_n69_));
  inv1   g24(.a(x01), .O(new_n70_));
  aoi22  g25(.a(x10), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x10), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x01), .O(new_n74_));
  inv1   g29(.a(x02), .O(new_n75_));
  nand2  g30(.a(x11), .b(new_n75_), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n72_), .c(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n68_), .O(z04));
  inv1   g35(.a(x13), .O(new_n81_));
  nand3  g36(.a(new_n47_), .b(new_n81_), .c(new_n46_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z05));
  nand3  g38(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z06));
  nand3  g40(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g41(.a(x23), .O(new_n87_));
  nand2  g42(.a(new_n55_), .b(new_n51_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(x22), .c(x21), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(x24), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n50_), .c(new_n49_), .O(z08));
  inv1   g47(.a(x07), .O(new_n93_));
  inv1   g48(.a(x15), .O(new_n94_));
  nand3  g49(.a(new_n61_), .b(new_n94_), .c(new_n93_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z09));
  nand4  g51(.a(new_n60_), .b(new_n52_), .c(new_n94_), .d(new_n93_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n47_), .O(z10));
  xor2a  g53(.a(x18), .b(x17), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n60_), .c(new_n94_), .d(new_n93_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z11));
  nor2   g56(.a(new_n53_), .b(new_n52_), .O(new_n102_));
  nand3  g57(.a(new_n54_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n54_), .c(new_n103_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n60_), .c(new_n94_), .d(new_n93_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(z12));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nand4  g63(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n108_), .c(new_n62_), .d(new_n94_), .O(z13));
  nand2  g65(.a(new_n109_), .b(x21), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n102_), .c(x19), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n62_), .d(new_n94_), .O(z14));
  inv1   g69(.a(new_n107_), .O(new_n115_));
  nor2   g70(.a(x22), .b(x21), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n51_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n60_), .c(new_n93_), .O(new_n118_));
  aoi21  g73(.a(new_n113_), .b(x22), .c(new_n118_), .O(new_n119_));
  nand2  g74(.a(x15), .b(new_n75_), .O(new_n120_));
  oai21  g75(.a(new_n119_), .b(x15), .c(new_n120_), .O(z15));
  nand2  g76(.a(new_n117_), .b(x23), .O(new_n122_));
  nor2   g77(.a(x23), .b(x22), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n112_), .c(new_n115_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n122_), .c(new_n62_), .d(new_n94_), .O(z16));
  nand2  g80(.a(new_n124_), .b(x24), .O(new_n126_));
  inv1   g81(.a(new_n109_), .O(new_n127_));
  nor2   g82(.a(x24), .b(x23), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n116_), .O(new_n129_));
  inv1   g84(.a(new_n129_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n126_), .c(new_n60_), .d(new_n93_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n94_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n120_), .O(z17));
  oai21  g89(.a(new_n129_), .b(new_n109_), .c(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n116_), .c(new_n127_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n60_), .d(new_n93_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n94_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n120_), .O(z18));
endmodule


