// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n51_), .c(new_n52_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n48_), .c(x07), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(x25), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n49_), .b(new_n67_), .O(z02));
  oai21  g23(.a(new_n60_), .b(x23), .c(x24), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n49_), .O(new_n70_));
  nand3  g25(.a(new_n56_), .b(new_n51_), .c(new_n47_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n70_), .c(x25), .O(z03));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  xnor2a g29(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z04));
  nor3   g34(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  nor3   g36(.a(new_n48_), .b(new_n81_), .c(x08), .O(z06));
  nand3  g37(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g38(.a(x25), .O(new_n84_));
  nand3  g39(.a(x24), .b(x22), .c(x21), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x15), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x20), .O(new_n87_));
  nand2  g42(.a(new_n54_), .b(new_n53_), .O(new_n88_));
  nand2  g43(.a(new_n60_), .b(new_n55_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n88_), .c(new_n51_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(x24), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n87_), .c(new_n84_), .O(z08));
  inv1   g47(.a(x07), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(x05), .c(x04), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n47_), .c(x15), .O(z09));
  inv1   g50(.a(x15), .O(new_n96_));
  nand4  g51(.a(new_n63_), .b(new_n47_), .c(new_n53_), .d(new_n96_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(x07), .O(z10));
  xor2a  g53(.a(x18), .b(x17), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n63_), .c(new_n93_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n47_), .c(x15), .O(z11));
  nor2   g56(.a(new_n54_), .b(new_n53_), .O(new_n102_));
  nand3  g57(.a(new_n55_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n55_), .c(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n63_), .c(new_n93_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n47_), .c(x15), .O(z12));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n63_), .c(new_n93_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n47_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n96_), .O(z13));
  nand2  g65(.a(new_n107_), .b(x21), .O(new_n111_));
  nor2   g66(.a(new_n64_), .b(x07), .O(new_n112_));
  nand4  g67(.a(new_n58_), .b(x19), .c(x18), .d(x17), .O(new_n113_));
  inv1   g68(.a(new_n113_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(x20), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n111_), .d(new_n96_), .O(z14));
  nand2  g71(.a(new_n113_), .b(x22), .O(new_n117_));
  nor2   g72(.a(x22), .b(x21), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n102_), .c(x19), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n47_), .c(new_n96_), .O(new_n120_));
  inv1   g75(.a(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n117_), .c(new_n112_), .O(z15));
  nand2  g77(.a(new_n119_), .b(x23), .O(new_n123_));
  inv1   g78(.a(new_n107_), .O(new_n124_));
  nor3   g79(.a(x23), .b(x22), .c(x21), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n124_), .c(x20), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n123_), .c(new_n112_), .d(new_n96_), .O(z16));
  aoi21  g82(.a(new_n125_), .b(new_n124_), .c(new_n52_), .O(new_n128_));
  nand4  g83(.a(new_n118_), .b(new_n124_), .c(new_n52_), .d(new_n51_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n63_), .c(new_n93_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n128_), .c(new_n47_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n96_), .O(z17));
  nand2  g87(.a(new_n129_), .b(x25), .O(new_n133_));
  nand4  g88(.a(new_n84_), .b(new_n52_), .c(new_n51_), .d(new_n59_), .O(new_n134_));
  inv1   g89(.a(new_n134_), .O(new_n135_));
  aoi21  g90(.a(new_n135_), .b(new_n114_), .c(x20), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n133_), .c(new_n112_), .d(new_n96_), .O(z18));
endmodule


