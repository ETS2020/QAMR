// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x16), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(z01));
  nand2  g17(.a(new_n60_), .b(new_n47_), .O(z03));
  xnor2a g18(.a(x09), .b(x00), .O(new_n64_));
  xnor2a g19(.a(x10), .b(x01), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  nand4  g22(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n47_), .c(new_n46_), .O(new_n69_));
  inv1   g24(.a(new_n69_), .O(z04));
  inv1   g25(.a(x13), .O(new_n71_));
  nand3  g26(.a(new_n47_), .b(new_n71_), .c(new_n46_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z05));
  nand3  g28(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g31(.a(x23), .O(new_n77_));
  nand3  g32(.a(new_n57_), .b(x22), .c(x21), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x24), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n51_), .c(new_n49_), .O(z08));
  nand2  g36(.a(x05), .b(x04), .O(new_n82_));
  inv1   g37(.a(x07), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n82_), .c(new_n47_), .O(z09));
  nand4  g41(.a(new_n82_), .b(new_n53_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n82_), .c(new_n84_), .d(new_n83_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z11));
  nor2   g46(.a(new_n54_), .b(new_n53_), .O(new_n92_));
  nand3  g47(.a(new_n55_), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n55_), .c(new_n93_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n82_), .c(new_n84_), .d(new_n83_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z12));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x20), .O(new_n98_));
  nand4  g53(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n98_), .c(new_n82_), .d(new_n83_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n84_), .O(new_n101_));
  inv1   g56(.a(x16), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x15), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n101_), .O(z13));
  nand2  g59(.a(new_n99_), .b(x21), .O(new_n105_));
  nor2   g60(.a(x21), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n92_), .c(x19), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n105_), .c(new_n61_), .d(new_n84_), .O(z14));
  inv1   g63(.a(new_n97_), .O(new_n109_));
  nor2   g64(.a(x22), .b(x21), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n52_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n82_), .c(new_n83_), .O(new_n112_));
  aoi21  g67(.a(new_n107_), .b(x22), .c(new_n112_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(x15), .c(new_n103_), .O(z15));
  nand2  g69(.a(new_n111_), .b(x23), .O(new_n115_));
  nor2   g70(.a(x23), .b(x22), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n106_), .c(new_n109_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n115_), .c(new_n61_), .d(new_n84_), .O(z16));
  nand2  g73(.a(new_n117_), .b(x24), .O(new_n119_));
  inv1   g74(.a(new_n99_), .O(new_n120_));
  nand3  g75(.a(new_n110_), .b(new_n50_), .c(new_n77_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n119_), .c(new_n61_), .d(new_n84_), .O(z17));
  oai21  g79(.a(new_n121_), .b(new_n99_), .c(x25), .O(new_n125_));
  nor3   g80(.a(x25), .b(x24), .c(x23), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n110_), .c(new_n120_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n125_), .c(new_n82_), .d(new_n83_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n84_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n103_), .O(z18));
  buf    g85(.a(x16), .O(z02));
endmodule


