// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n121_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x19), .O(new_n48_));
  nor2   g03(.a(x23), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  oai21  g09(.a(x18), .b(x17), .c(new_n54_), .O(new_n55_));
  inv1   g10(.a(x21), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n55_), .c(x25), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x19), .c(new_n53_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  inv1   g18(.a(x24), .O(new_n64_));
  inv1   g19(.a(x25), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n60_), .O(z01));
  nand2  g24(.a(new_n66_), .b(new_n60_), .O(z03));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n50_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n50_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand4  g36(.a(new_n55_), .b(x24), .c(x22), .d(x21), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n65_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n48_), .O(new_n84_));
  nand2  g39(.a(new_n66_), .b(x23), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n84_), .O(z08));
  inv1   g41(.a(x04), .O(new_n87_));
  inv1   g42(.a(x15), .O(new_n88_));
  nand4  g43(.a(new_n50_), .b(new_n88_), .c(new_n63_), .d(x05), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n87_), .O(z09));
  inv1   g45(.a(x17), .O(new_n91_));
  nand4  g46(.a(new_n61_), .b(new_n91_), .c(new_n88_), .d(new_n63_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n50_), .O(z10));
  xor2a  g48(.a(x18), .b(x17), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n61_), .c(new_n50_), .d(new_n88_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z11));
  and2   g51(.a(x18), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n48_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n48_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n61_), .c(new_n88_), .d(new_n63_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n50_), .O(z12));
  nand3  g56(.a(new_n61_), .b(new_n88_), .c(new_n63_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n50_), .O(new_n103_));
  oai21  g58(.a(new_n97_), .b(new_n53_), .c(x19), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nand4  g60(.a(new_n97_), .b(x23), .c(new_n54_), .d(x19), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(z13));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n97_), .c(new_n53_), .O(new_n109_));
  aoi21  g64(.a(new_n106_), .b(x21), .c(new_n102_), .O(new_n110_));
  oai21  g65(.a(new_n109_), .b(new_n48_), .c(new_n110_), .O(z14));
  oai21  g66(.a(new_n109_), .b(new_n48_), .c(x22), .O(new_n112_));
  nand3  g67(.a(x19), .b(x18), .c(x17), .O(new_n113_));
  inv1   g68(.a(new_n113_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n108_), .c(x23), .d(new_n57_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n112_), .c(new_n103_), .O(z15));
  nand3  g71(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n113_), .c(x23), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n103_), .O(z16));
  nor2   g74(.a(new_n62_), .b(x07), .O(new_n120_));
  nor2   g75(.a(x24), .b(x15), .O(new_n121_));
  aoi21  g76(.a(new_n121_), .b(new_n120_), .c(new_n49_), .O(z17));
  nor2   g77(.a(x25), .b(x15), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n120_), .c(new_n49_), .O(z18));
endmodule


