// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x16), .O(new_n47_));
  nor2   g002(.a(x19), .b(new_n47_), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  oai21  g004(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g005(.a(x25), .O(new_n51_));
  inv1   g006(.a(x21), .O(new_n52_));
  inv1   g007(.a(x22), .O(new_n53_));
  inv1   g008(.a(x23), .O(new_n54_));
  oai21  g009(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  nand2  g010(.a(new_n55_), .b(x24), .O(new_n56_));
  inv1   g011(.a(x07), .O(new_n57_));
  nand2  g012(.a(x05), .b(x04), .O(new_n58_));
  nand2  g013(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g014(.a(new_n56_), .b(new_n51_), .c(new_n59_), .O(new_n60_));
  inv1   g015(.a(x20), .O(new_n61_));
  inv1   g016(.a(x19), .O(new_n62_));
  nor2   g017(.a(x18), .b(x17), .O(new_n63_));
  oai21  g018(.a(new_n63_), .b(x16), .c(new_n62_), .O(new_n64_));
  nand4  g019(.a(new_n64_), .b(new_n51_), .c(new_n54_), .d(new_n61_), .O(new_n65_));
  oai21  g020(.a(new_n60_), .b(new_n48_), .c(new_n65_), .O(z01));
  nand2  g021(.a(new_n56_), .b(new_n49_), .O(new_n67_));
  nand3  g022(.a(new_n64_), .b(new_n54_), .c(new_n61_), .O(new_n68_));
  aoi21  g023(.a(new_n68_), .b(new_n67_), .c(x25), .O(z03));
  xnor2a g024(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g025(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g026(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g027(.a(x12), .b(x03), .O(new_n73_));
  nand4  g028(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand3  g029(.a(new_n74_), .b(new_n49_), .c(new_n46_), .O(new_n75_));
  inv1   g030(.a(new_n75_), .O(z04));
  oai21  g031(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g032(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g033(.a(new_n78_), .b(new_n49_), .O(z06));
  aoi21  g034(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g035(.a(x24), .O(new_n81_));
  nand3  g036(.a(x22), .b(x21), .c(x20), .O(new_n82_));
  aoi21  g037(.a(new_n82_), .b(new_n54_), .c(new_n81_), .O(new_n83_));
  nor2   g038(.a(new_n83_), .b(x25), .O(new_n84_));
  nor2   g039(.a(new_n52_), .b(x19), .O(new_n85_));
  nor2   g040(.a(new_n81_), .b(new_n53_), .O(new_n86_));
  nand4  g041(.a(new_n86_), .b(new_n85_), .c(new_n63_), .d(new_n47_), .O(new_n87_));
  oai21  g042(.a(new_n84_), .b(new_n48_), .c(new_n87_), .O(z08));
  inv1   g043(.a(x04), .O(new_n89_));
  inv1   g044(.a(x15), .O(new_n90_));
  nand4  g045(.a(new_n49_), .b(new_n90_), .c(new_n57_), .d(x05), .O(new_n91_));
  nor2   g046(.a(new_n91_), .b(new_n89_), .O(z09));
  inv1   g047(.a(x17), .O(new_n93_));
  nand4  g048(.a(new_n58_), .b(new_n49_), .c(new_n93_), .d(new_n90_), .O(new_n94_));
  nor2   g049(.a(new_n94_), .b(x07), .O(z10));
  xor2a  g050(.a(x18), .b(x17), .O(new_n96_));
  nand4  g051(.a(new_n96_), .b(new_n58_), .c(new_n49_), .d(new_n90_), .O(new_n97_));
  nor2   g052(.a(new_n97_), .b(x07), .O(z11));
  inv1   g053(.a(x18), .O(new_n99_));
  nor2   g054(.a(new_n99_), .b(new_n93_), .O(new_n100_));
  nand4  g055(.a(new_n62_), .b(x18), .c(x17), .d(new_n47_), .O(new_n101_));
  oai21  g056(.a(new_n100_), .b(new_n62_), .c(new_n101_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n58_), .c(new_n90_), .d(new_n57_), .O(new_n103_));
  inv1   g058(.a(new_n103_), .O(z12));
  inv1   g059(.a(new_n59_), .O(new_n105_));
  oai21  g060(.a(x20), .b(x16), .c(new_n62_), .O(new_n106_));
  inv1   g061(.a(new_n100_), .O(new_n107_));
  nand2  g062(.a(new_n107_), .b(x20), .O(new_n108_));
  nand4  g063(.a(new_n61_), .b(x19), .c(x18), .d(x17), .O(new_n109_));
  inv1   g064(.a(new_n109_), .O(new_n110_));
  nor2   g065(.a(new_n110_), .b(x15), .O(new_n111_));
  nand4  g066(.a(new_n111_), .b(new_n108_), .c(new_n106_), .d(new_n105_), .O(z13));
  oai21  g067(.a(x21), .b(x16), .c(new_n62_), .O(new_n113_));
  nand3  g068(.a(new_n58_), .b(new_n90_), .c(new_n57_), .O(new_n114_));
  nand2  g069(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g070(.a(new_n61_), .b(x18), .c(x17), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(x21), .O(new_n117_));
  nor2   g072(.a(x21), .b(x20), .O(new_n118_));
  nand2  g073(.a(new_n118_), .b(new_n100_), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g075(.a(new_n120_), .b(x19), .O(new_n121_));
  nand2  g076(.a(new_n85_), .b(new_n47_), .O(new_n122_));
  nand3  g077(.a(new_n122_), .b(new_n121_), .c(new_n115_), .O(z14));
  oai21  g078(.a(x22), .b(x16), .c(new_n62_), .O(new_n124_));
  nand2  g079(.a(new_n119_), .b(x22), .O(new_n125_));
  nand3  g080(.a(x19), .b(x18), .c(x17), .O(new_n126_));
  nor2   g081(.a(x22), .b(x21), .O(new_n127_));
  nand2  g082(.a(new_n127_), .b(new_n61_), .O(new_n128_));
  nor2   g083(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nor2   g084(.a(new_n129_), .b(x15), .O(new_n130_));
  nand4  g085(.a(new_n130_), .b(new_n125_), .c(new_n124_), .d(new_n105_), .O(z15));
  oai21  g086(.a(x23), .b(x16), .c(new_n62_), .O(new_n132_));
  oai21  g087(.a(new_n128_), .b(new_n107_), .c(x23), .O(new_n133_));
  inv1   g088(.a(new_n126_), .O(new_n134_));
  nand4  g089(.a(new_n134_), .b(new_n118_), .c(new_n54_), .d(new_n53_), .O(new_n135_));
  nand3  g090(.a(new_n135_), .b(new_n105_), .c(new_n90_), .O(new_n136_));
  inv1   g091(.a(new_n136_), .O(new_n137_));
  nand3  g092(.a(new_n137_), .b(new_n133_), .c(new_n132_), .O(z16));
  oai21  g093(.a(x24), .b(x16), .c(new_n62_), .O(new_n139_));
  nand2  g094(.a(new_n139_), .b(new_n114_), .O(new_n140_));
  nand3  g095(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n141_));
  oai21  g096(.a(new_n141_), .b(new_n116_), .c(x24), .O(new_n142_));
  nor2   g097(.a(x20), .b(new_n99_), .O(new_n143_));
  nor2   g098(.a(x24), .b(x23), .O(new_n144_));
  nand4  g099(.a(new_n144_), .b(new_n127_), .c(new_n143_), .d(x17), .O(new_n145_));
  nand2  g100(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g101(.a(new_n146_), .b(x19), .O(new_n147_));
  nand3  g102(.a(x24), .b(new_n62_), .c(new_n47_), .O(new_n148_));
  nand3  g103(.a(new_n148_), .b(new_n147_), .c(new_n140_), .O(z17));
  oai21  g104(.a(x25), .b(x16), .c(new_n62_), .O(new_n150_));
  nand2  g105(.a(new_n145_), .b(x25), .O(new_n151_));
  inv1   g106(.a(new_n127_), .O(new_n152_));
  nand3  g107(.a(new_n51_), .b(new_n81_), .c(new_n54_), .O(new_n153_));
  nor2   g108(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  aoi21  g109(.a(new_n154_), .b(new_n110_), .c(x15), .O(new_n155_));
  nand4  g110(.a(new_n155_), .b(new_n151_), .c(new_n150_), .d(new_n105_), .O(z18));
  buf    g111(.a(x16), .O(z02));
endmodule


