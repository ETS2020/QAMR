// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  nor2   g02(.a(x15), .b(x08), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  and2   g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n56_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n49_), .c(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  inv1   g17(.a(new_n48_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(z02));
  nand2  g19(.a(new_n59_), .b(new_n63_), .O(z03));
  xor2a  g20(.a(x09), .b(x00), .O(new_n66_));
  xor2a  g21(.a(x10), .b(x01), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x11), .b(x02), .O(new_n69_));
  inv1   g24(.a(x12), .O(new_n70_));
  inv1   g25(.a(x15), .O(new_n71_));
  aoi21  g26(.a(new_n70_), .b(x03), .c(new_n71_), .O(new_n72_));
  oai21  g27(.a(new_n70_), .b(x03), .c(new_n72_), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n68_), .c(x08), .O(z04));
  inv1   g30(.a(x13), .O(new_n76_));
  nand3  g31(.a(x15), .b(new_n76_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  nand3  g33(.a(x15), .b(x14), .c(new_n46_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  oai21  g35(.a(new_n71_), .b(x06), .c(new_n46_), .O(z07));
  inv1   g36(.a(x23), .O(new_n82_));
  nand3  g37(.a(new_n56_), .b(x22), .c(x21), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x24), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n50_), .c(new_n48_), .O(z08));
  nand2  g41(.a(x05), .b(x04), .O(new_n87_));
  nor4   g42(.a(new_n87_), .b(x15), .c(new_n46_), .d(x07), .O(z09));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n87_), .b(new_n52_), .c(new_n89_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x08), .c(x15), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n87_), .c(new_n89_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x08), .c(x15), .O(z11));
  nor2   g49(.a(new_n53_), .b(new_n52_), .O(new_n95_));
  nand3  g50(.a(new_n54_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n54_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n87_), .c(new_n71_), .d(x08), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nor2   g56(.a(x20), .b(new_n54_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n95_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(x15), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n101_), .c(new_n60_), .d(x08), .O(z13));
  nand2  g61(.a(new_n103_), .b(x21), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n95_), .c(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n71_), .c(x08), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n107_), .c(new_n60_), .O(z14));
  nand2  g67(.a(new_n109_), .b(x22), .O(new_n113_));
  inv1   g68(.a(new_n100_), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n51_), .O(new_n116_));
  and2   g71(.a(new_n116_), .b(new_n71_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n113_), .c(new_n60_), .d(x08), .O(z15));
  nand2  g73(.a(new_n116_), .b(x23), .O(new_n119_));
  nor2   g74(.a(x23), .b(x22), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n108_), .c(new_n114_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n71_), .c(x08), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n119_), .c(new_n60_), .O(z16));
  nand2  g79(.a(new_n121_), .b(x24), .O(new_n125_));
  nor2   g80(.a(x24), .b(x23), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n115_), .c(new_n102_), .d(new_n95_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n125_), .c(new_n87_), .d(new_n89_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(x08), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n71_), .O(z17));
  nand2  g85(.a(new_n127_), .b(x25), .O(new_n131_));
  nor3   g86(.a(x25), .b(x24), .c(x23), .O(new_n132_));
  and2   g87(.a(new_n132_), .b(new_n115_), .O(new_n133_));
  aoi21  g88(.a(new_n133_), .b(new_n104_), .c(x15), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n131_), .c(new_n60_), .d(x08), .O(z18));
endmodule


