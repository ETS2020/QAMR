// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x18), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x23), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  nor3   g05(.a(x19), .b(x18), .c(x17), .O(new_n51_));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x22), .O(new_n53_));
  nor2   g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g09(.a(new_n51_), .b(x20), .c(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n49_), .c(new_n50_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nor3   g13(.a(new_n58_), .b(new_n47_), .c(x07), .O(new_n59_));
  oai21  g14(.a(new_n56_), .b(x25), .c(new_n59_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nor2   g16(.a(new_n47_), .b(new_n61_), .O(z02));
  inv1   g17(.a(new_n47_), .O(new_n63_));
  oai21  g18(.a(new_n54_), .b(x23), .c(x24), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g20(.a(x20), .O(new_n66_));
  inv1   g21(.a(x18), .O(new_n67_));
  oai21  g22(.a(x19), .b(x17), .c(x15), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(new_n49_), .c(new_n66_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n65_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n63_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n63_), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n63_), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  nand3  g37(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n49_), .c(new_n50_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x25), .c(new_n63_), .O(new_n85_));
  inv1   g40(.a(x17), .O(new_n86_));
  nand3  g41(.a(new_n67_), .b(new_n86_), .c(x15), .O(new_n87_));
  inv1   g42(.a(x19), .O(new_n88_));
  nand4  g43(.a(x24), .b(x22), .c(x21), .d(new_n88_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z08));
  inv1   g45(.a(x07), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(x05), .c(x04), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(x18), .c(x15), .O(z09));
  inv1   g48(.a(x15), .O(new_n94_));
  nand4  g49(.a(new_n57_), .b(x18), .c(new_n86_), .d(new_n94_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z10));
  xor2a  g51(.a(x19), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n57_), .c(new_n91_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(x18), .c(x15), .O(z12));
  nand2  g54(.a(x19), .b(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nor2   g56(.a(new_n58_), .b(x07), .O(new_n102_));
  nand3  g57(.a(new_n66_), .b(x19), .c(x17), .O(new_n103_));
  and2   g58(.a(new_n103_), .b(x18), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n94_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  inv1   g61(.a(new_n100_), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n107_), .c(new_n67_), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n106_), .c(new_n102_), .d(new_n94_), .O(z14));
  nand2  g65(.a(new_n108_), .b(new_n107_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x22), .O(new_n112_));
  nand3  g67(.a(new_n53_), .b(new_n52_), .c(new_n66_), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n100_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n67_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n102_), .d(new_n94_), .O(z15));
  oai21  g71(.a(new_n113_), .b(new_n100_), .c(x23), .O(new_n117_));
  nor2   g72(.a(x20), .b(new_n88_), .O(new_n118_));
  nor2   g73(.a(x23), .b(x22), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n118_), .c(new_n52_), .d(x17), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n117_), .c(new_n57_), .d(new_n91_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x18), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n94_), .O(z16));
  nand3  g78(.a(new_n49_), .b(new_n53_), .c(new_n52_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n103_), .c(x24), .O(new_n125_));
  nor2   g80(.a(x22), .b(x21), .O(new_n126_));
  nor2   g81(.a(x24), .b(x23), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n126_), .c(new_n118_), .d(x17), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n125_), .c(new_n57_), .d(new_n91_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x18), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n94_), .O(z17));
  nand4  g86(.a(new_n50_), .b(new_n49_), .c(new_n53_), .d(new_n52_), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(new_n103_), .c(x25), .O(new_n133_));
  nor2   g88(.a(x25), .b(x24), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n119_), .c(new_n108_), .d(new_n107_), .O(new_n135_));
  nand4  g90(.a(new_n135_), .b(new_n133_), .c(new_n57_), .d(new_n91_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(x18), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n94_), .O(z18));
  nor2   g93(.a(new_n95_), .b(x07), .O(z11));
endmodule


