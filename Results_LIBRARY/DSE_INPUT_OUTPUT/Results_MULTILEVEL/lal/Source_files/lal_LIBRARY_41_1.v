// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x20), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n48_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nor3   g18(.a(new_n63_), .b(new_n49_), .c(x07), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n61_), .O(z01));
  nand2  g20(.a(new_n61_), .b(new_n50_), .O(z03));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z04));
  nor3   g28(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z06));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x24), .O(new_n78_));
  inv1   g33(.a(x15), .O(new_n79_));
  inv1   g34(.a(x23), .O(new_n80_));
  nand3  g35(.a(x22), .b(x21), .c(x20), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  nand2  g37(.a(new_n55_), .b(new_n54_), .O(new_n83_));
  nand3  g38(.a(x22), .b(x21), .c(new_n56_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n83_), .c(new_n80_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n48_), .c(new_n82_), .O(new_n86_));
  oai22  g41(.a(new_n86_), .b(new_n78_), .c(new_n49_), .d(new_n53_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(x05), .c(x04), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n48_), .c(x15), .O(z09));
  nand4  g45(.a(new_n62_), .b(new_n48_), .c(new_n54_), .d(new_n79_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x07), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n62_), .c(new_n88_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n48_), .c(x15), .O(z11));
  nor2   g50(.a(new_n55_), .b(new_n54_), .O(new_n96_));
  nand3  g51(.a(new_n56_), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n96_), .b(new_n56_), .c(new_n97_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n62_), .c(new_n48_), .d(new_n79_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(x07), .O(z12));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n62_), .c(new_n88_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n79_), .O(z13));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  nor2   g60(.a(x21), .b(new_n56_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n96_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n105_), .c(new_n62_), .d(new_n88_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n48_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n79_), .O(z14));
  inv1   g65(.a(x22), .O(new_n111_));
  aoi21  g66(.a(new_n106_), .b(new_n96_), .c(new_n111_), .O(new_n112_));
  nor2   g67(.a(x22), .b(x21), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n96_), .c(x19), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n62_), .c(new_n88_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n112_), .c(new_n48_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n79_), .O(z15));
  nand2  g72(.a(new_n114_), .b(x23), .O(new_n118_));
  nor2   g73(.a(new_n63_), .b(x07), .O(new_n119_));
  inv1   g74(.a(new_n101_), .O(new_n120_));
  nor3   g75(.a(x23), .b(x22), .c(x21), .O(new_n121_));
  aoi21  g76(.a(new_n121_), .b(new_n120_), .c(x20), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n119_), .c(new_n118_), .d(new_n79_), .O(z16));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x24), .O(new_n125_));
  inv1   g80(.a(x21), .O(new_n126_));
  nand4  g81(.a(new_n78_), .b(new_n80_), .c(new_n111_), .d(new_n126_), .O(new_n127_));
  inv1   g82(.a(new_n127_), .O(new_n128_));
  aoi21  g83(.a(new_n128_), .b(new_n120_), .c(x20), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n125_), .c(new_n119_), .d(new_n79_), .O(z17));
  oai21  g85(.a(new_n127_), .b(new_n101_), .c(x25), .O(new_n131_));
  nor2   g86(.a(x23), .b(x22), .O(new_n132_));
  nor2   g87(.a(x25), .b(x24), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n132_), .c(new_n106_), .d(new_n96_), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n131_), .c(new_n62_), .d(new_n88_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n48_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n79_), .O(z18));
endmodule


