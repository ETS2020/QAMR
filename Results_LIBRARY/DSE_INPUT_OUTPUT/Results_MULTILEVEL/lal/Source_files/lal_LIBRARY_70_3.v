// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x19), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x18), .b(x17), .c(new_n53_), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n54_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n51_), .c(new_n52_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n47_), .d(new_n50_), .O(z01));
  nand2  g16(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g17(.a(new_n59_), .b(new_n47_), .O(z03));
  inv1   g18(.a(x08), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n47_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g27(.a(new_n47_), .b(x14), .c(new_n64_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z06));
  aoi22  g29(.a(x24), .b(x19), .c(new_n64_), .d(x06), .O(z07));
  inv1   g30(.a(x19), .O(new_n76_));
  inv1   g31(.a(x23), .O(new_n77_));
  nand3  g32(.a(new_n54_), .b(x22), .c(x21), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n77_), .c(new_n51_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x25), .c(new_n76_), .O(new_n80_));
  nand2  g35(.a(x25), .b(new_n51_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(z08));
  inv1   g37(.a(x04), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  nand4  g39(.a(new_n47_), .b(new_n84_), .c(new_n50_), .d(x05), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n83_), .O(z09));
  inv1   g41(.a(x17), .O(new_n87_));
  nand4  g42(.a(new_n60_), .b(new_n87_), .c(new_n84_), .d(new_n50_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n47_), .O(z10));
  xor2a  g44(.a(x18), .b(x17), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n60_), .c(new_n47_), .d(new_n84_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x07), .O(z11));
  and2   g47(.a(x18), .b(x17), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(x24), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(x19), .O(new_n95_));
  nand3  g50(.a(new_n76_), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n60_), .c(new_n84_), .d(new_n50_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z12));
  nand3  g54(.a(new_n60_), .b(new_n84_), .c(new_n50_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n47_), .O(new_n101_));
  oai21  g56(.a(new_n94_), .b(new_n76_), .c(x20), .O(new_n102_));
  nand4  g57(.a(new_n93_), .b(new_n51_), .c(new_n53_), .d(x19), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(z13));
  nand3  g59(.a(new_n53_), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n51_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x19), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x21), .O(new_n108_));
  nand3  g63(.a(x19), .b(x18), .c(x17), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n51_), .c(new_n55_), .d(new_n53_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n108_), .c(new_n101_), .O(z14));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n93_), .c(x24), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n76_), .c(x22), .O(new_n115_));
  nand4  g70(.a(new_n113_), .b(new_n110_), .c(new_n51_), .d(new_n56_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n101_), .O(z15));
  nor3   g72(.a(x22), .b(x21), .c(x20), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n93_), .c(x24), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n76_), .c(x23), .O(new_n120_));
  nor2   g75(.a(x22), .b(x21), .O(new_n121_));
  nand3  g76(.a(new_n93_), .b(new_n53_), .c(x19), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n121_), .c(new_n51_), .d(new_n77_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n120_), .c(new_n101_), .O(z16));
  nor2   g80(.a(x24), .b(x15), .O(new_n126_));
  nand4  g81(.a(new_n113_), .b(new_n110_), .c(new_n77_), .d(new_n56_), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n126_), .c(new_n60_), .d(new_n50_), .O(z17));
  nand3  g83(.a(new_n77_), .b(new_n56_), .c(new_n55_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n105_), .c(x25), .O(new_n130_));
  nand3  g85(.a(new_n121_), .b(new_n52_), .c(new_n77_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n122_), .c(new_n130_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n51_), .O(new_n133_));
  nand2  g88(.a(x25), .b(new_n76_), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n133_), .c(new_n101_), .O(z18));
endmodule


