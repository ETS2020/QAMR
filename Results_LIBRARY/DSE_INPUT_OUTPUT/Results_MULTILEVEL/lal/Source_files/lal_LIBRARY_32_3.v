// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x16), .O(new_n46_));
  inv1   g001(.a(x14), .O(new_n47_));
  inv1   g002(.a(x17), .O(new_n48_));
  nor2   g003(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g004(.a(new_n49_), .O(new_n50_));
  nand3  g005(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g006(.a(new_n51_), .O(z00));
  nor3   g007(.a(x25), .b(x23), .c(x20), .O(new_n53_));
  oai21  g008(.a(new_n53_), .b(x14), .c(x17), .O(new_n54_));
  inv1   g009(.a(x24), .O(new_n55_));
  inv1   g010(.a(x25), .O(new_n56_));
  inv1   g011(.a(x20), .O(new_n57_));
  oai21  g012(.a(x19), .b(x18), .c(new_n57_), .O(new_n58_));
  inv1   g013(.a(x21), .O(new_n59_));
  inv1   g014(.a(x22), .O(new_n60_));
  nor2   g015(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g016(.a(new_n61_), .b(new_n58_), .c(x23), .O(new_n62_));
  oai21  g017(.a(new_n62_), .b(new_n55_), .c(new_n56_), .O(new_n63_));
  nand2  g018(.a(x05), .b(x04), .O(new_n64_));
  inv1   g019(.a(new_n64_), .O(new_n65_));
  nor2   g020(.a(new_n65_), .b(x07), .O(new_n66_));
  nand3  g021(.a(new_n66_), .b(new_n63_), .c(new_n54_), .O(z01));
  nor2   g022(.a(new_n49_), .b(new_n46_), .O(z02));
  nand2  g023(.a(new_n63_), .b(new_n54_), .O(z03));
  inv1   g024(.a(x08), .O(new_n70_));
  xnor2a g025(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g026(.a(x10), .b(x01), .O(new_n72_));
  xnor2a g027(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g028(.a(x12), .b(x03), .O(new_n74_));
  nand4  g029(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand3  g030(.a(new_n75_), .b(new_n50_), .c(new_n70_), .O(new_n76_));
  inv1   g031(.a(new_n76_), .O(z04));
  inv1   g032(.a(x13), .O(new_n78_));
  nand3  g033(.a(new_n50_), .b(new_n78_), .c(new_n70_), .O(new_n79_));
  inv1   g034(.a(new_n79_), .O(z05));
  aoi21  g035(.a(new_n48_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g036(.a(new_n50_), .b(new_n70_), .c(x06), .O(z07));
  inv1   g037(.a(x18), .O(new_n83_));
  inv1   g038(.a(x19), .O(new_n84_));
  nand3  g039(.a(new_n84_), .b(new_n83_), .c(new_n48_), .O(new_n85_));
  aoi21  g040(.a(new_n85_), .b(new_n57_), .c(new_n60_), .O(new_n86_));
  aoi21  g041(.a(new_n86_), .b(x21), .c(x23), .O(new_n87_));
  nor2   g042(.a(new_n49_), .b(x25), .O(new_n88_));
  oai21  g043(.a(new_n87_), .b(new_n55_), .c(new_n88_), .O(z08));
  inv1   g044(.a(x07), .O(new_n90_));
  inv1   g045(.a(x15), .O(new_n91_));
  nand2  g046(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g047(.a(new_n92_), .b(new_n64_), .c(new_n50_), .O(z09));
  nand4  g048(.a(new_n64_), .b(new_n48_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(new_n50_), .O(z10));
  nand3  g050(.a(new_n83_), .b(x17), .c(new_n47_), .O(new_n96_));
  oai21  g051(.a(new_n83_), .b(x17), .c(new_n96_), .O(new_n97_));
  nand4  g052(.a(new_n97_), .b(new_n64_), .c(new_n91_), .d(new_n90_), .O(new_n98_));
  inv1   g053(.a(new_n98_), .O(z11));
  nor2   g054(.a(new_n83_), .b(new_n48_), .O(new_n100_));
  nand3  g055(.a(new_n84_), .b(x18), .c(x17), .O(new_n101_));
  oai21  g056(.a(new_n100_), .b(new_n84_), .c(new_n101_), .O(new_n102_));
  nand4  g057(.a(new_n102_), .b(new_n64_), .c(new_n91_), .d(new_n90_), .O(new_n103_));
  nand2  g058(.a(new_n103_), .b(new_n50_), .O(z12));
  oai21  g059(.a(new_n92_), .b(new_n65_), .c(new_n50_), .O(new_n105_));
  nor2   g060(.a(new_n84_), .b(new_n83_), .O(new_n106_));
  nand4  g061(.a(new_n57_), .b(x19), .c(x18), .d(x17), .O(new_n107_));
  oai21  g062(.a(new_n106_), .b(new_n57_), .c(new_n107_), .O(new_n108_));
  nand2  g063(.a(new_n108_), .b(new_n47_), .O(new_n109_));
  nand2  g064(.a(x20), .b(new_n48_), .O(new_n110_));
  nand3  g065(.a(new_n110_), .b(new_n109_), .c(new_n105_), .O(z13));
  nand3  g066(.a(new_n57_), .b(x19), .c(x18), .O(new_n112_));
  nand2  g067(.a(new_n112_), .b(x21), .O(new_n113_));
  nor2   g068(.a(x21), .b(x20), .O(new_n114_));
  nand3  g069(.a(new_n114_), .b(new_n100_), .c(x19), .O(new_n115_));
  nand2  g070(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g071(.a(new_n116_), .b(new_n47_), .O(new_n117_));
  nand2  g072(.a(x21), .b(new_n48_), .O(new_n118_));
  nand3  g073(.a(new_n118_), .b(new_n117_), .c(new_n105_), .O(z14));
  aoi21  g074(.a(new_n114_), .b(new_n106_), .c(new_n60_), .O(new_n120_));
  nand3  g075(.a(x19), .b(x18), .c(x17), .O(new_n121_));
  nor2   g076(.a(x22), .b(x21), .O(new_n122_));
  nand2  g077(.a(new_n122_), .b(new_n57_), .O(new_n123_));
  nor2   g078(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  oai21  g079(.a(new_n124_), .b(new_n120_), .c(new_n47_), .O(new_n125_));
  nand2  g080(.a(x22), .b(new_n48_), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n125_), .c(new_n105_), .O(z15));
  inv1   g082(.a(x23), .O(new_n128_));
  nor3   g083(.a(x22), .b(x21), .c(x20), .O(new_n129_));
  aoi21  g084(.a(new_n129_), .b(new_n106_), .c(new_n128_), .O(new_n130_));
  nand3  g085(.a(new_n114_), .b(new_n128_), .c(new_n60_), .O(new_n131_));
  nor2   g086(.a(new_n131_), .b(new_n121_), .O(new_n132_));
  oai21  g087(.a(new_n132_), .b(new_n130_), .c(new_n47_), .O(new_n133_));
  nand2  g088(.a(x23), .b(new_n48_), .O(new_n134_));
  nand3  g089(.a(new_n134_), .b(new_n133_), .c(new_n105_), .O(z16));
  oai21  g090(.a(x24), .b(x17), .c(x14), .O(new_n136_));
  oai21  g091(.a(new_n131_), .b(new_n121_), .c(x24), .O(new_n137_));
  inv1   g092(.a(new_n107_), .O(new_n138_));
  nand3  g093(.a(new_n122_), .b(new_n55_), .c(new_n128_), .O(new_n139_));
  inv1   g094(.a(new_n139_), .O(new_n140_));
  aoi21  g095(.a(new_n140_), .b(new_n138_), .c(x15), .O(new_n141_));
  nand4  g096(.a(new_n141_), .b(new_n137_), .c(new_n136_), .d(new_n66_), .O(z17));
  oai21  g097(.a(x25), .b(x17), .c(x14), .O(new_n143_));
  oai21  g098(.a(new_n139_), .b(new_n107_), .c(x25), .O(new_n144_));
  inv1   g099(.a(new_n122_), .O(new_n145_));
  nand3  g100(.a(new_n56_), .b(new_n55_), .c(new_n128_), .O(new_n146_));
  nor2   g101(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  aoi21  g102(.a(new_n147_), .b(new_n138_), .c(x15), .O(new_n148_));
  nand4  g103(.a(new_n148_), .b(new_n144_), .c(new_n143_), .d(new_n66_), .O(z18));
endmodule


