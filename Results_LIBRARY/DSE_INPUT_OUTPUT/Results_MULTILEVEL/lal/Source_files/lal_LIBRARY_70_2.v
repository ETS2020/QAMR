// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x19), .O(new_n48_));
  nor2   g03(.a(x24), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x24), .O(new_n53_));
  nor3   g08(.a(x25), .b(x23), .c(x20), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(new_n53_), .c(x19), .O(new_n55_));
  inv1   g10(.a(x25), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  oai21  g12(.a(x18), .b(x17), .c(new_n57_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n58_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n53_), .c(new_n56_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n55_), .O(z01));
  nand2  g19(.a(new_n62_), .b(new_n55_), .O(z03));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n50_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor3   g29(.a(new_n49_), .b(new_n74_), .c(x08), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand3  g31(.a(x24), .b(x22), .c(x21), .O(new_n77_));
  nor3   g32(.a(new_n77_), .b(x18), .c(x17), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x25), .c(new_n48_), .O(new_n79_));
  nor2   g34(.a(x25), .b(x23), .O(new_n80_));
  oai21  g35(.a(new_n59_), .b(new_n57_), .c(new_n80_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(x24), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n79_), .O(z08));
  inv1   g38(.a(x04), .O(new_n84_));
  inv1   g39(.a(x07), .O(new_n85_));
  inv1   g40(.a(x15), .O(new_n86_));
  nand4  g41(.a(new_n50_), .b(new_n86_), .c(new_n85_), .d(x05), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n84_), .O(z09));
  inv1   g43(.a(x17), .O(new_n89_));
  nand2  g44(.a(x05), .b(x04), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n89_), .c(new_n86_), .d(new_n85_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n50_), .O(z10));
  xor2a  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n90_), .c(new_n50_), .d(new_n86_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(x07), .O(z11));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(x24), .O(new_n97_));
  nand3  g52(.a(new_n48_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n97_), .b(new_n48_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n90_), .c(new_n86_), .d(new_n85_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z12));
  nand3  g56(.a(new_n90_), .b(new_n86_), .c(new_n85_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n50_), .O(new_n103_));
  nand2  g58(.a(new_n97_), .b(x19), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  inv1   g60(.a(new_n96_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(x24), .c(new_n57_), .d(x19), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(z13));
  nand3  g63(.a(new_n57_), .b(x18), .c(x17), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x24), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x19), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x21), .O(new_n112_));
  inv1   g67(.a(x21), .O(new_n113_));
  nand3  g68(.a(x19), .b(x18), .c(x17), .O(new_n114_));
  inv1   g69(.a(new_n114_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(x24), .c(new_n113_), .d(new_n57_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n112_), .c(new_n103_), .O(z14));
  nor2   g72(.a(x21), .b(x20), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n106_), .c(new_n53_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n48_), .c(x22), .O(new_n120_));
  inv1   g75(.a(x22), .O(new_n121_));
  nand4  g76(.a(new_n118_), .b(new_n115_), .c(x24), .d(new_n121_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n103_), .O(z15));
  nor3   g78(.a(x22), .b(x21), .c(x20), .O(new_n124_));
  aoi21  g79(.a(new_n124_), .b(new_n106_), .c(new_n53_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n48_), .c(x23), .O(new_n126_));
  inv1   g81(.a(x23), .O(new_n127_));
  nand4  g82(.a(x24), .b(new_n127_), .c(new_n121_), .d(new_n113_), .O(new_n128_));
  inv1   g83(.a(new_n128_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n106_), .c(new_n57_), .d(x19), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n126_), .c(new_n103_), .O(z16));
  oai21  g86(.a(new_n128_), .b(new_n109_), .c(x19), .O(new_n132_));
  aoi21  g87(.a(x24), .b(new_n48_), .c(x15), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n132_), .c(new_n63_), .O(z17));
  nor2   g89(.a(x25), .b(x15), .O(new_n135_));
  aoi21  g90(.a(new_n135_), .b(new_n63_), .c(new_n49_), .O(z18));
endmodule


