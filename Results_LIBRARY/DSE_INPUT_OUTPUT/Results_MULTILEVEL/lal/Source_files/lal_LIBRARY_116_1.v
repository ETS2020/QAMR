// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x07), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n61_), .c(new_n47_), .d(new_n51_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n49_), .b(new_n64_), .O(z02));
  nand3  g20(.a(new_n60_), .b(new_n49_), .c(new_n52_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor3   g31(.a(new_n48_), .b(new_n76_), .c(x08), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g33(.a(x23), .O(new_n79_));
  nand2  g34(.a(new_n57_), .b(new_n53_), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(x22), .c(x21), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x24), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n52_), .c(new_n48_), .O(z08));
  aoi21  g39(.a(new_n62_), .b(new_n47_), .c(x07), .O(z09));
  nand4  g40(.a(new_n62_), .b(new_n54_), .c(new_n47_), .d(new_n51_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  xor2a  g42(.a(x18), .b(x17), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n62_), .c(new_n47_), .d(new_n51_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z11));
  nor2   g45(.a(new_n55_), .b(new_n54_), .O(new_n91_));
  nand3  g46(.a(new_n56_), .b(x18), .c(x17), .O(new_n92_));
  oai21  g47(.a(new_n91_), .b(new_n56_), .c(new_n92_), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n62_), .c(new_n47_), .d(new_n51_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z12));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(x20), .O(new_n97_));
  aoi21  g52(.a(x05), .b(x04), .c(x07), .O(new_n98_));
  nand4  g53(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n47_), .O(z13));
  nand2  g55(.a(new_n99_), .b(x21), .O(new_n101_));
  nor2   g56(.a(x21), .b(x20), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n91_), .c(x19), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n98_), .d(new_n47_), .O(z14));
  inv1   g59(.a(new_n96_), .O(new_n105_));
  nor2   g60(.a(x22), .b(x21), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n53_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n62_), .O(new_n108_));
  aoi21  g63(.a(new_n103_), .b(x22), .c(new_n108_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(x15), .c(new_n51_), .O(z15));
  nand2  g65(.a(new_n107_), .b(x23), .O(new_n111_));
  nor2   g66(.a(x23), .b(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n102_), .c(new_n105_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n98_), .d(new_n47_), .O(z16));
  nand2  g69(.a(new_n113_), .b(x24), .O(new_n115_));
  inv1   g70(.a(new_n99_), .O(new_n116_));
  inv1   g71(.a(x24), .O(new_n117_));
  nand3  g72(.a(new_n106_), .b(new_n117_), .c(new_n79_), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n115_), .c(new_n62_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n51_), .O(z17));
  oai21  g78(.a(new_n118_), .b(new_n99_), .c(x25), .O(new_n124_));
  nor3   g79(.a(x25), .b(x24), .c(x23), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n106_), .c(new_n116_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n62_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n47_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n51_), .O(z18));
endmodule


