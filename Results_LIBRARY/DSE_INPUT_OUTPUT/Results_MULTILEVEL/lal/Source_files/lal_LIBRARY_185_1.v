// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x09), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(new_n48_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n48_), .O(new_n67_));
  nand2  g22(.a(new_n60_), .b(new_n67_), .O(z03));
  inv1   g23(.a(x10), .O(new_n69_));
  inv1   g24(.a(x02), .O(new_n70_));
  aoi22  g25(.a(x11), .b(new_n70_), .c(new_n69_), .d(x01), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(x01), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x11), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x02), .O(new_n74_));
  inv1   g29(.a(x03), .O(new_n75_));
  nand2  g30(.a(x12), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x12), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x03), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n72_), .c(new_n67_), .O(new_n80_));
  inv1   g35(.a(x09), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(x00), .O(new_n82_));
  nor2   g37(.a(x15), .b(x09), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x00), .c(new_n82_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n80_), .c(x08), .O(z04));
  nor3   g40(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g41(.a(x14), .O(new_n87_));
  nor3   g42(.a(new_n48_), .b(new_n87_), .c(x08), .O(z06));
  aoi21  g43(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g44(.a(x22), .O(new_n90_));
  aoi21  g45(.a(new_n56_), .b(new_n52_), .c(new_n90_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(x21), .c(x23), .O(new_n92_));
  nor2   g47(.a(new_n48_), .b(x25), .O(new_n93_));
  oai21  g48(.a(new_n92_), .b(new_n50_), .c(new_n93_), .O(z08));
  inv1   g49(.a(x07), .O(new_n95_));
  nand3  g50(.a(new_n62_), .b(new_n47_), .c(new_n95_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z09));
  nand4  g52(.a(new_n61_), .b(new_n53_), .c(new_n47_), .d(new_n95_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n67_), .O(z10));
  xor2a  g54(.a(x18), .b(x17), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n61_), .c(new_n47_), .d(new_n95_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n67_), .O(z11));
  nor2   g57(.a(new_n54_), .b(new_n53_), .O(new_n103_));
  nand3  g58(.a(new_n55_), .b(x18), .c(x17), .O(new_n104_));
  oai21  g59(.a(new_n103_), .b(new_n55_), .c(new_n104_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n61_), .c(new_n47_), .d(new_n95_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n67_), .O(z12));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x20), .O(new_n109_));
  nor2   g64(.a(x20), .b(new_n55_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n103_), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n63_), .d(new_n47_), .O(z13));
  nand2  g67(.a(new_n111_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n103_), .c(x19), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n63_), .d(new_n47_), .O(z14));
  inv1   g71(.a(new_n108_), .O(new_n117_));
  nor3   g72(.a(x22), .b(x21), .c(x20), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n61_), .c(new_n95_), .O(new_n120_));
  aoi21  g75(.a(new_n115_), .b(x22), .c(new_n120_), .O(new_n121_));
  nand2  g76(.a(x15), .b(x09), .O(new_n122_));
  oai21  g77(.a(new_n121_), .b(x15), .c(new_n122_), .O(z15));
  inv1   g78(.a(x23), .O(new_n124_));
  aoi21  g79(.a(new_n118_), .b(new_n117_), .c(new_n124_), .O(new_n125_));
  nand4  g80(.a(new_n114_), .b(new_n117_), .c(new_n124_), .d(new_n90_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n61_), .c(new_n95_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n125_), .c(new_n47_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n122_), .O(z16));
  nand2  g84(.a(new_n126_), .b(x24), .O(new_n130_));
  nor2   g85(.a(x22), .b(x21), .O(new_n131_));
  nor2   g86(.a(x24), .b(x23), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n131_), .c(new_n110_), .d(new_n103_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n130_), .c(new_n61_), .d(new_n95_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n47_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n122_), .O(z17));
  nand2  g91(.a(new_n133_), .b(x25), .O(new_n137_));
  nor3   g92(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  nand4  g93(.a(new_n138_), .b(new_n131_), .c(new_n110_), .d(new_n103_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n137_), .c(new_n63_), .d(new_n47_), .O(z18));
endmodule


