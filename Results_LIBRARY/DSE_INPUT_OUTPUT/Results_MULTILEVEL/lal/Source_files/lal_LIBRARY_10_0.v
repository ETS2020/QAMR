// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x21), .b(x15), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n51_), .c(new_n50_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  inv1   g11(.a(x23), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x20), .O(new_n59_));
  inv1   g14(.a(x22), .O(new_n60_));
  inv1   g15(.a(x17), .O(new_n61_));
  inv1   g16(.a(x18), .O(new_n62_));
  inv1   g17(.a(x19), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n59_), .c(new_n60_), .O(new_n65_));
  nand2  g20(.a(new_n58_), .b(x15), .O(new_n66_));
  oai21  g21(.a(new_n65_), .b(new_n58_), .c(new_n66_), .O(new_n67_));
  nand3  g22(.a(new_n67_), .b(new_n53_), .c(new_n57_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n56_), .O(z01));
  inv1   g24(.a(x16), .O(new_n70_));
  nand2  g25(.a(new_n48_), .b(new_n70_), .O(z02));
  oai21  g26(.a(x25), .b(x23), .c(x15), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n58_), .O(new_n73_));
  nand2  g28(.a(new_n64_), .b(new_n59_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(x22), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n57_), .c(new_n52_), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(x25), .c(new_n73_), .O(z03));
  xnor2a g32(.a(x09), .b(x00), .O(new_n78_));
  xnor2a g33(.a(x10), .b(x01), .O(new_n79_));
  xnor2a g34(.a(x11), .b(x02), .O(new_n80_));
  xnor2a g35(.a(x12), .b(x03), .O(new_n81_));
  nand4  g36(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n78_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n48_), .O(z04));
  nor3   g39(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g40(.a(x14), .b(new_n46_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n48_), .O(z06));
  aoi21  g42(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g43(.a(x24), .b(x23), .c(x25), .O(new_n89_));
  nand4  g44(.a(new_n74_), .b(x24), .c(x22), .d(x21), .O(new_n90_));
  oai21  g45(.a(new_n89_), .b(new_n47_), .c(new_n90_), .O(z08));
  nand3  g46(.a(new_n50_), .b(x05), .c(x04), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(x21), .c(x15), .O(z09));
  inv1   g48(.a(x15), .O(new_n94_));
  nand4  g49(.a(new_n51_), .b(x21), .c(new_n61_), .d(new_n94_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z10));
  xor2a  g51(.a(x18), .b(x17), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n51_), .c(x21), .d(new_n94_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z11));
  oai21  g54(.a(new_n62_), .b(new_n61_), .c(x19), .O(new_n100_));
  nand3  g55(.a(new_n63_), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n51_), .c(x21), .d(new_n94_), .O(new_n103_));
  nor2   g58(.a(new_n103_), .b(x07), .O(z12));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nand4  g61(.a(new_n59_), .b(x19), .c(x18), .d(x17), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n51_), .d(new_n50_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x21), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n94_), .O(z13));
  nand3  g65(.a(new_n51_), .b(x17), .c(new_n50_), .O(new_n111_));
  nand3  g66(.a(new_n59_), .b(x19), .c(x18), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n111_), .c(x21), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n94_), .O(z14));
  nand3  g69(.a(new_n51_), .b(new_n60_), .c(new_n50_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x21), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n94_), .O(z15));
  nand3  g72(.a(new_n51_), .b(new_n94_), .c(new_n50_), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n57_), .c(x21), .O(z16));
  nand3  g75(.a(new_n51_), .b(new_n52_), .c(new_n50_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x21), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n94_), .O(z17));
  nand3  g78(.a(new_n119_), .b(new_n53_), .c(x21), .O(z18));
endmodule


