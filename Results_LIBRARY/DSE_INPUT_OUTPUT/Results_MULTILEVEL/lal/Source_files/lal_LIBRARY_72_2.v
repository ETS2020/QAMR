// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x19), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  oai21  g07(.a(x18), .b(x17), .c(new_n47_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n51_), .c(new_n52_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor3   g14(.a(new_n59_), .b(new_n49_), .c(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(z01));
  and2   g16(.a(new_n48_), .b(x16), .O(z02));
  inv1   g17(.a(x23), .O(new_n63_));
  nand2  g18(.a(new_n54_), .b(new_n48_), .O(new_n64_));
  inv1   g19(.a(x19), .O(new_n65_));
  inv1   g20(.a(x17), .O(new_n66_));
  inv1   g21(.a(x18), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n47_), .c(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n63_), .O(new_n71_));
  nand2  g26(.a(new_n48_), .b(new_n51_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n71_), .c(x25), .O(z03));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n48_), .O(z04));
  nor3   g35(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand2  g36(.a(x14), .b(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n48_), .O(z06));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand4  g39(.a(x22), .b(x21), .c(new_n67_), .d(new_n66_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n63_), .c(x19), .O(new_n86_));
  aoi21  g41(.a(new_n54_), .b(new_n63_), .c(new_n47_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n86_), .c(x24), .O(new_n88_));
  nand2  g43(.a(new_n48_), .b(x25), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(z08));
  inv1   g45(.a(x07), .O(new_n91_));
  inv1   g46(.a(x15), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n58_), .c(new_n48_), .O(z09));
  nand4  g49(.a(new_n58_), .b(new_n48_), .c(new_n66_), .d(new_n92_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x07), .O(z10));
  xor2a  g51(.a(x18), .b(x17), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n58_), .c(new_n48_), .d(new_n92_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x07), .O(z11));
  nand2  g54(.a(x18), .b(x17), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(x20), .c(x19), .O(new_n101_));
  nand3  g56(.a(new_n65_), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n58_), .c(new_n92_), .d(new_n91_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z12));
  oai21  g60(.a(new_n93_), .b(new_n59_), .c(new_n48_), .O(new_n106_));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n106_), .O(z13));
  nor2   g64(.a(x21), .b(x15), .O(new_n110_));
  nand3  g65(.a(new_n58_), .b(new_n48_), .c(new_n91_), .O(new_n111_));
  inv1   g66(.a(new_n111_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n110_), .O(z14));
  nor2   g68(.a(new_n59_), .b(x07), .O(new_n114_));
  nor2   g69(.a(x22), .b(x15), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n114_), .c(new_n49_), .O(z15));
  nand3  g71(.a(new_n112_), .b(new_n63_), .c(new_n92_), .O(z16));
  nand3  g72(.a(new_n112_), .b(new_n51_), .c(new_n92_), .O(z17));
  nand3  g73(.a(new_n112_), .b(new_n52_), .c(new_n92_), .O(z18));
endmodule


