// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x25), .b(x15), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  inv1   g16(.a(x21), .O(new_n62_));
  inv1   g17(.a(x22), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n61_), .c(x23), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(new_n54_), .c(new_n55_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n53_), .O(z01));
  inv1   g22(.a(new_n47_), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n46_), .O(z02));
  inv1   g24(.a(new_n66_), .O(z03));
  inv1   g25(.a(x08), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n47_), .O(z04));
  inv1   g33(.a(x13), .O(new_n79_));
  nand3  g34(.a(new_n47_), .b(new_n79_), .c(new_n71_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z05));
  nand3  g36(.a(new_n47_), .b(x14), .c(new_n71_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z06));
  aoi21  g38(.a(new_n71_), .b(x06), .c(new_n68_), .O(z07));
  inv1   g39(.a(x23), .O(new_n85_));
  nand3  g40(.a(new_n61_), .b(x22), .c(x21), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n55_), .c(x24), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(x25), .b(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n88_), .O(z08));
  nand4  g46(.a(new_n89_), .b(new_n50_), .c(x05), .d(x04), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand4  g48(.a(new_n51_), .b(new_n57_), .c(new_n89_), .d(new_n50_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n51_), .c(new_n89_), .d(new_n50_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n47_), .O(z11));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n59_), .b(x18), .c(x17), .O(new_n101_));
  oai21  g56(.a(new_n100_), .b(new_n59_), .c(new_n101_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n51_), .c(new_n89_), .d(new_n50_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n47_), .O(z12));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n100_), .b(new_n56_), .c(x19), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n51_), .d(new_n50_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n89_), .O(new_n109_));
  nand2  g64(.a(new_n55_), .b(x15), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n109_), .O(z13));
  inv1   g66(.a(new_n52_), .O(new_n112_));
  nand2  g67(.a(new_n107_), .b(x21), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n100_), .c(x19), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n113_), .c(new_n112_), .d(new_n89_), .O(z14));
  nand3  g71(.a(new_n62_), .b(new_n56_), .c(x19), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n99_), .c(x22), .O(new_n118_));
  inv1   g73(.a(new_n105_), .O(new_n119_));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n56_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n118_), .c(new_n51_), .d(new_n50_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n89_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n110_), .O(z15));
  nand2  g79(.a(new_n121_), .b(x23), .O(new_n125_));
  nand4  g80(.a(new_n114_), .b(new_n119_), .c(new_n85_), .d(new_n63_), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n125_), .c(new_n112_), .d(new_n89_), .O(z16));
  nand2  g82(.a(new_n126_), .b(x24), .O(new_n128_));
  inv1   g83(.a(new_n107_), .O(new_n129_));
  nand3  g84(.a(new_n120_), .b(new_n54_), .c(new_n85_), .O(new_n130_));
  inv1   g85(.a(new_n130_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n128_), .c(new_n112_), .d(new_n89_), .O(z17));
  oai21  g88(.a(new_n130_), .b(new_n107_), .c(x25), .O(new_n134_));
  nor3   g89(.a(x25), .b(x24), .c(x23), .O(new_n135_));
  nand3  g90(.a(new_n135_), .b(new_n120_), .c(new_n129_), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n134_), .c(new_n112_), .d(new_n89_), .O(z18));
endmodule


