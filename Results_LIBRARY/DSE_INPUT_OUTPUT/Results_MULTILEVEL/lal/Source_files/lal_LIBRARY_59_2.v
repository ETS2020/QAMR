// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(x19), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  nor3   g09(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n55_), .b(x20), .c(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n53_), .c(new_n54_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor3   g16(.a(new_n61_), .b(new_n49_), .c(x07), .O(new_n62_));
  oai21  g17(.a(new_n59_), .b(x25), .c(new_n62_), .O(z01));
  oai21  g18(.a(new_n57_), .b(x23), .c(x24), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n50_), .O(new_n65_));
  inv1   g20(.a(x20), .O(new_n66_));
  inv1   g21(.a(x19), .O(new_n67_));
  nor2   g22(.a(x18), .b(x17), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x15), .c(new_n67_), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(new_n53_), .c(new_n66_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n65_), .c(x25), .O(z03));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n50_), .c(new_n46_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z04));
  nor3   g33(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  nor3   g35(.a(new_n49_), .b(new_n80_), .c(x08), .O(z06));
  nand3  g36(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g37(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n68_), .c(x15), .O(new_n85_));
  oai21  g40(.a(new_n56_), .b(new_n66_), .c(new_n53_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x24), .c(x25), .O(new_n87_));
  oai21  g42(.a(new_n85_), .b(x19), .c(new_n87_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n61_), .b(new_n48_), .c(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n50_), .O(z09));
  inv1   g46(.a(x17), .O(new_n92_));
  nand4  g47(.a(new_n60_), .b(new_n92_), .c(new_n48_), .d(new_n89_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n50_), .O(z10));
  xor2a  g49(.a(x18), .b(x17), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n60_), .c(new_n48_), .d(new_n89_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n50_), .O(z11));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(x19), .c(new_n48_), .O(new_n99_));
  nand3  g54(.a(new_n67_), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n60_), .c(new_n89_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n50_), .O(z12));
  nand3  g58(.a(new_n66_), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n48_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x19), .O(new_n106_));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n60_), .b(new_n89_), .O(new_n108_));
  aoi21  g63(.a(new_n107_), .b(x20), .c(new_n108_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(x15), .c(new_n106_), .O(z13));
  inv1   g65(.a(new_n98_), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n111_), .c(x15), .O(new_n113_));
  nand4  g68(.a(new_n66_), .b(x19), .c(x18), .d(x17), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(x21), .c(new_n108_), .O(new_n115_));
  oai22  g70(.a(new_n115_), .b(x15), .c(new_n113_), .d(new_n67_), .O(z14));
  inv1   g71(.a(new_n108_), .O(new_n117_));
  nand3  g72(.a(new_n112_), .b(new_n111_), .c(x19), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x22), .O(new_n119_));
  inv1   g74(.a(x21), .O(new_n120_));
  inv1   g75(.a(x22), .O(new_n121_));
  inv1   g76(.a(new_n107_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n121_), .c(new_n120_), .d(new_n66_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n119_), .c(new_n117_), .d(new_n48_), .O(z15));
  nand2  g79(.a(new_n123_), .b(x23), .O(new_n125_));
  nand4  g80(.a(new_n112_), .b(new_n122_), .c(new_n53_), .d(new_n121_), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n125_), .c(new_n117_), .d(new_n48_), .O(z16));
  nand4  g82(.a(new_n54_), .b(new_n53_), .c(new_n121_), .d(new_n120_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n104_), .c(new_n48_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x19), .O(new_n130_));
  aoi21  g85(.a(new_n126_), .b(x24), .c(new_n108_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(x15), .c(new_n130_), .O(z17));
  nand2  g87(.a(new_n112_), .b(new_n111_), .O(new_n133_));
  inv1   g88(.a(x25), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n54_), .c(new_n53_), .d(new_n121_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(new_n133_), .c(new_n48_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(x19), .O(new_n137_));
  oai21  g92(.a(new_n128_), .b(new_n114_), .c(x25), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n117_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n48_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n137_), .O(z18));
endmodule


