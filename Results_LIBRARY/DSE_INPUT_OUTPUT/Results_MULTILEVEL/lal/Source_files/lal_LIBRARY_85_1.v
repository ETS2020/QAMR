// Benchmark "FAU" written by ABC on Fri Aug 14 00:08:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x03), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x23), .c(x24), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n54_), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n63_), .c(new_n50_), .d(new_n53_), .O(z01));
  nand3  g20(.a(new_n62_), .b(new_n50_), .c(new_n54_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  aoi21  g22(.a(x12), .b(new_n46_), .c(x15), .O(new_n68_));
  inv1   g23(.a(x09), .O(new_n69_));
  inv1   g24(.a(x01), .O(new_n70_));
  aoi22  g25(.a(x10), .b(new_n70_), .c(new_n69_), .d(x00), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x10), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x01), .O(new_n74_));
  inv1   g29(.a(x02), .O(new_n75_));
  nand2  g30(.a(x11), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  inv1   g33(.a(x12), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x03), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n78_), .c(new_n76_), .d(new_n74_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n72_), .c(new_n46_), .O(new_n82_));
  oai21  g37(.a(new_n68_), .b(x03), .c(new_n82_), .O(z04));
  nor3   g38(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor3   g40(.a(new_n49_), .b(new_n85_), .c(x08), .O(z06));
  nand3  g41(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g42(.a(x23), .O(new_n88_));
  nand2  g43(.a(new_n59_), .b(new_n55_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(x22), .c(x21), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(x24), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n54_), .c(new_n49_), .O(z08));
  inv1   g48(.a(new_n64_), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n48_), .c(new_n53_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z09));
  nand4  g51(.a(new_n64_), .b(new_n56_), .c(new_n48_), .d(new_n53_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z10));
  xor2a  g53(.a(x18), .b(x17), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n64_), .c(new_n48_), .d(new_n53_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z11));
  nor2   g56(.a(new_n57_), .b(new_n56_), .O(new_n102_));
  nand3  g57(.a(new_n58_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n58_), .c(new_n103_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n64_), .c(new_n48_), .d(new_n53_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n50_), .O(z12));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x20), .O(new_n108_));
  nor2   g63(.a(new_n94_), .b(x07), .O(new_n109_));
  nor2   g64(.a(x20), .b(new_n58_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n102_), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n108_), .d(new_n48_), .O(z13));
  inv1   g67(.a(x21), .O(new_n113_));
  aoi21  g68(.a(new_n110_), .b(new_n102_), .c(new_n113_), .O(new_n114_));
  nor2   g69(.a(x21), .b(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n102_), .c(x19), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n64_), .c(new_n53_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n114_), .c(new_n48_), .O(new_n118_));
  nand2  g73(.a(x15), .b(x03), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(z14));
  nand2  g75(.a(new_n116_), .b(x22), .O(new_n121_));
  inv1   g76(.a(new_n107_), .O(new_n122_));
  nor2   g77(.a(x22), .b(x21), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n55_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n121_), .c(new_n109_), .d(new_n48_), .O(z15));
  nand2  g80(.a(new_n124_), .b(x23), .O(new_n126_));
  nor2   g81(.a(x23), .b(x22), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n115_), .c(new_n122_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n126_), .c(new_n109_), .d(new_n48_), .O(z16));
  nand2  g84(.a(new_n128_), .b(x24), .O(new_n130_));
  inv1   g85(.a(new_n111_), .O(new_n131_));
  nor2   g86(.a(x24), .b(x23), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n123_), .c(new_n131_), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n130_), .c(new_n109_), .d(new_n48_), .O(z17));
  nand2  g89(.a(new_n133_), .b(x25), .O(new_n135_));
  nor3   g90(.a(x25), .b(x24), .c(x23), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n123_), .c(new_n131_), .O(new_n137_));
  nand4  g92(.a(new_n137_), .b(new_n135_), .c(new_n109_), .d(new_n48_), .O(z18));
endmodule


