// Benchmark "FAU" written by ABC on Fri Aug 14 00:09:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(new_n47_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n46_), .O(z02));
  nand3  g20(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  inv1   g22(.a(x08), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n68_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n47_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g33(.a(new_n47_), .b(new_n68_), .c(x06), .O(z07));
  inv1   g34(.a(x23), .O(new_n80_));
  nand2  g35(.a(new_n55_), .b(new_n51_), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x22), .c(x21), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x24), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n50_), .c(new_n64_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand3  g42(.a(new_n61_), .b(new_n87_), .c(new_n86_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand4  g44(.a(new_n60_), .b(new_n52_), .c(new_n87_), .d(new_n86_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n47_), .O(z10));
  xor2a  g46(.a(x18), .b(x17), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n60_), .c(new_n87_), .d(new_n86_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z11));
  nor2   g49(.a(new_n53_), .b(new_n52_), .O(new_n95_));
  nand3  g50(.a(new_n54_), .b(x18), .c(x17), .O(new_n96_));
  oai21  g51(.a(new_n95_), .b(new_n54_), .c(new_n96_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n60_), .c(new_n87_), .d(new_n86_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z12));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x20), .O(new_n101_));
  nor2   g56(.a(x20), .b(new_n54_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n95_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n62_), .d(new_n87_), .O(z13));
  inv1   g59(.a(x21), .O(new_n105_));
  aoi21  g60(.a(new_n102_), .b(new_n95_), .c(new_n105_), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n95_), .c(x19), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n60_), .c(new_n87_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n106_), .c(new_n86_), .O(new_n110_));
  oai21  g65(.a(x15), .b(new_n86_), .c(new_n110_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  inv1   g67(.a(new_n100_), .O(new_n113_));
  nor2   g68(.a(x22), .b(x21), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n51_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n62_), .d(new_n87_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n107_), .c(new_n113_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n117_), .c(new_n62_), .d(new_n87_), .O(z16));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n121_));
  inv1   g76(.a(new_n103_), .O(new_n122_));
  nor2   g77(.a(x24), .b(x23), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n114_), .c(new_n122_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n121_), .c(new_n62_), .d(new_n87_), .O(z17));
  nand2  g80(.a(new_n124_), .b(x25), .O(new_n126_));
  nor3   g81(.a(x25), .b(x24), .c(x23), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n114_), .c(new_n122_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n126_), .c(new_n62_), .d(new_n87_), .O(z18));
endmodule


