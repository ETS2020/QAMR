// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x6), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand4  g10(.a(new_n25_), .b(new_n21_), .c(x2), .d(new_n19_), .O(new_n26_));
  oai21  g11(.a(x2), .b(new_n19_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n17_), .O(new_n28_));
  nand3  g13(.a(new_n21_), .b(x3), .c(new_n16_), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n19_), .c(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(z1));
  aoi21  g17(.a(x4), .b(new_n16_), .c(x3), .O(new_n33_));
  nand2  g18(.a(x5), .b(x2), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n21_), .c(new_n20_), .d(new_n19_), .O(new_n35_));
  oai21  g20(.a(new_n33_), .b(new_n19_), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n17_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n31_), .O(z2));
  oai21  g23(.a(x4), .b(x3), .c(x7), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n40_));
  aoi21  g25(.a(x5), .b(new_n17_), .c(x3), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(x2), .c(x6), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(x1), .c(new_n40_), .O(z3));
  nor2   g28(.a(new_n20_), .b(x0), .O(new_n44_));
  nor2   g29(.a(x3), .b(x1), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(x2), .O(new_n46_));
  aoi21  g31(.a(x3), .b(new_n19_), .c(x0), .O(new_n47_));
  nand3  g32(.a(x3), .b(new_n19_), .c(x0), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n47_), .c(new_n16_), .O(new_n50_));
  nand2  g35(.a(x6), .b(new_n19_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n46_), .O(z4));
  xnor2a g37(.a(x3), .b(x2), .O(new_n53_));
  nand3  g38(.a(x5), .b(x2), .c(new_n17_), .O(new_n54_));
  oai21  g39(.a(new_n53_), .b(new_n17_), .c(new_n54_), .O(new_n55_));
  nor3   g40(.a(new_n20_), .b(new_n16_), .c(x0), .O(new_n56_));
  aoi21  g41(.a(new_n55_), .b(new_n19_), .c(new_n56_), .O(new_n57_));
  nand2  g42(.a(new_n20_), .b(x2), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(x1), .c(new_n17_), .O(new_n59_));
  oai21  g44(.a(new_n57_), .b(x6), .c(new_n59_), .O(z5));
  aoi21  g45(.a(new_n58_), .b(new_n29_), .c(new_n17_), .O(new_n61_));
  oai21  g46(.a(new_n25_), .b(new_n16_), .c(new_n21_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(new_n19_), .O(new_n63_));
  oai21  g48(.a(x4), .b(x2), .c(new_n20_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(x1), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n63_), .O(z6));
endmodule


