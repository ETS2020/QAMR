// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  nor2   g07(.a(x5), .b(x4), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n19_), .c(new_n26_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n18_), .O(new_n30_));
  aoi22  g15(.a(new_n30_), .b(new_n26_), .c(new_n28_), .d(new_n18_), .O(new_n31_));
  nor2   g16(.a(x1), .b(new_n16_), .O(new_n32_));
  nor2   g17(.a(new_n19_), .b(x2), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n32_), .c(new_n23_), .O(new_n34_));
  oai21  g19(.a(new_n31_), .b(x0), .c(new_n34_), .O(z1));
  nand3  g20(.a(new_n26_), .b(new_n18_), .c(x0), .O(new_n36_));
  oai21  g21(.a(new_n18_), .b(x0), .c(new_n36_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n24_), .c(x3), .O(new_n38_));
  nand3  g23(.a(new_n27_), .b(x4), .c(x2), .O(new_n39_));
  nand3  g24(.a(new_n29_), .b(x5), .c(new_n26_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n39_), .c(x1), .O(new_n41_));
  nand3  g26(.a(new_n29_), .b(x4), .c(new_n26_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n41_), .c(new_n19_), .O(new_n44_));
  nand3  g29(.a(x4), .b(new_n26_), .c(x1), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n38_), .O(z2));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n26_), .c(x1), .d(new_n16_), .O(new_n50_));
  nand2  g35(.a(x5), .b(new_n16_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n19_), .c(x2), .O(new_n52_));
  or2    g37(.a(new_n52_), .b(x1), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n50_), .c(new_n24_), .O(z3));
  nand2  g39(.a(new_n33_), .b(x0), .O(new_n55_));
  oai21  g40(.a(x3), .b(new_n26_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n18_), .O(new_n57_));
  aoi21  g42(.a(x3), .b(new_n18_), .c(x2), .O(new_n58_));
  nor2   g43(.a(new_n19_), .b(new_n26_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n58_), .c(new_n16_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n57_), .c(new_n23_), .O(z4));
  inv1   g46(.a(x4), .O(new_n62_));
  nand2  g47(.a(x5), .b(x1), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(new_n62_), .c(new_n19_), .O(new_n64_));
  nor2   g49(.a(new_n27_), .b(x1), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n64_), .c(x2), .O(new_n66_));
  nand3  g51(.a(new_n30_), .b(new_n24_), .c(new_n26_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n66_), .c(x0), .O(new_n68_));
  oai21  g53(.a(new_n62_), .b(new_n16_), .c(new_n27_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n19_), .c(x2), .O(new_n70_));
  nand2  g55(.a(new_n27_), .b(new_n62_), .O(new_n71_));
  nand4  g56(.a(new_n71_), .b(x3), .c(new_n26_), .d(x0), .O(new_n72_));
  aoi21  g57(.a(new_n72_), .b(new_n70_), .c(x1), .O(new_n73_));
  or2    g58(.a(new_n73_), .b(new_n68_), .O(z5));
  nand2  g59(.a(new_n72_), .b(new_n52_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n18_), .O(new_n76_));
  oai21  g61(.a(new_n24_), .b(new_n26_), .c(x3), .O(new_n77_));
  oai21  g62(.a(x4), .b(x2), .c(new_n77_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(x1), .c(new_n16_), .O(new_n79_));
  nand3  g64(.a(new_n79_), .b(new_n76_), .c(new_n24_), .O(z6));
endmodule


