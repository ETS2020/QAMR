// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x5), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(x0), .c(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  nor3   g10(.a(new_n19_), .b(new_n16_), .c(x0), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n25_), .c(new_n23_), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n23_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n16_), .c(new_n17_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n27_), .O(z1));
  aoi21  g16(.a(new_n19_), .b(x3), .c(x4), .O(new_n32_));
  inv1   g17(.a(x3), .O(new_n33_));
  nand3  g18(.a(new_n28_), .b(new_n33_), .c(new_n23_), .O(new_n34_));
  oai21  g19(.a(new_n32_), .b(new_n23_), .c(new_n34_), .O(new_n35_));
  nand3  g20(.a(x3), .b(new_n23_), .c(x0), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  aoi21  g22(.a(new_n35_), .b(new_n17_), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x3), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(x1), .c(new_n17_), .O(new_n41_));
  oai21  g26(.a(new_n38_), .b(x2), .c(new_n41_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n44_));
  oai21  g29(.a(new_n21_), .b(new_n16_), .c(new_n44_), .O(z3));
  nand3  g30(.a(x5), .b(new_n33_), .c(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n24_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n23_), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n23_), .c(x2), .O(new_n49_));
  nand2  g34(.a(new_n40_), .b(x2), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n49_), .c(new_n17_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n48_), .O(z4));
  oai21  g38(.a(new_n33_), .b(x2), .c(new_n46_), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(x0), .c(new_n26_), .O(new_n55_));
  aoi21  g40(.a(new_n39_), .b(x2), .c(new_n23_), .O(new_n56_));
  nand3  g41(.a(x6), .b(new_n33_), .c(new_n16_), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n17_), .O(new_n59_));
  oai21  g44(.a(new_n55_), .b(x1), .c(new_n59_), .O(z5));
  nand3  g45(.a(new_n54_), .b(new_n23_), .c(x0), .O(new_n61_));
  inv1   g46(.a(x4), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(x3), .c(new_n16_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n39_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(x1), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n61_), .O(z6));
endmodule


