// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  nor2   g04(.a(new_n17_), .b(x0), .O(new_n20_));
  nand2  g05(.a(x4), .b(x3), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  oai21  g10(.a(x5), .b(x3), .c(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n26_), .c(x1), .O(new_n28_));
  nand3  g13(.a(x4), .b(new_n16_), .c(x1), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n16_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n17_), .c(x0), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(z1));
  nand2  g20(.a(new_n18_), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(x4), .c(x1), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n18_), .c(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n34_), .O(z2));
  nand2  g31(.a(x5), .b(new_n25_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n48_));
  inv1   g33(.a(x4), .O(new_n49_));
  nor2   g34(.a(x7), .b(new_n49_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n20_), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n48_), .O(z3));
  nand3  g37(.a(x3), .b(new_n17_), .c(x0), .O(new_n53_));
  nand3  g38(.a(x4), .b(x1), .c(new_n25_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n33_), .b(x0), .c(new_n36_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n22_), .b(x2), .c(new_n25_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(z4));
  aoi21  g45(.a(new_n36_), .b(new_n32_), .c(new_n25_), .O(new_n61_));
  nor2   g46(.a(new_n26_), .b(x0), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(new_n17_), .O(new_n63_));
  inv1   g48(.a(new_n27_), .O(new_n64_));
  aoi21  g49(.a(new_n21_), .b(x2), .c(new_n17_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n64_), .c(new_n25_), .O(new_n66_));
  nand2  g51(.a(new_n49_), .b(x1), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n66_), .c(new_n63_), .O(z5));
  nor2   g53(.a(x5), .b(x3), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(x2), .c(new_n61_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(x1), .c(new_n23_), .O(z6));
endmodule


