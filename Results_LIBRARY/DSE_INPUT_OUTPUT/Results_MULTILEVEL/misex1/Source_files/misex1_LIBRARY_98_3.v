// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  aoi21  g02(.a(x2), .b(new_n17_), .c(x3), .O(new_n18_));
  nand2  g03(.a(x3), .b(x2), .O(new_n19_));
  oai22  g04(.a(new_n19_), .b(new_n17_), .c(new_n18_), .d(new_n16_), .O(z0));
  inv1   g05(.a(x3), .O(new_n21_));
  inv1   g06(.a(x5), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(x2), .c(new_n17_), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n17_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n24_), .c(x0), .O(z1));
  nand3  g14(.a(x4), .b(new_n25_), .c(new_n16_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n21_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  nand2  g17(.a(new_n22_), .b(x2), .O(new_n33_));
  nand2  g18(.a(new_n26_), .b(new_n25_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g20(.a(new_n35_), .b(new_n21_), .c(new_n17_), .d(new_n16_), .O(new_n36_));
  nand2  g21(.a(x3), .b(x0), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n36_), .c(new_n32_), .O(z2));
  oai21  g23(.a(x4), .b(x3), .c(x7), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(new_n25_), .c(x1), .d(new_n16_), .O(new_n40_));
  nand2  g25(.a(x5), .b(new_n16_), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n21_), .c(x2), .d(new_n17_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n40_), .O(z3));
  nand2  g28(.a(x3), .b(new_n17_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n25_), .c(new_n16_), .O(new_n45_));
  oai21  g30(.a(x3), .b(new_n17_), .c(x2), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n45_), .c(new_n37_), .O(z4));
  oai21  g32(.a(new_n25_), .b(x1), .c(new_n21_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x0), .O(new_n49_));
  oai21  g34(.a(x3), .b(new_n25_), .c(x1), .O(new_n50_));
  nand3  g35(.a(x5), .b(x2), .c(new_n17_), .O(new_n51_));
  nand3  g36(.a(x6), .b(new_n21_), .c(new_n25_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n16_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n49_), .c(new_n19_), .O(z5));
  oai21  g40(.a(x4), .b(x2), .c(new_n21_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(x1), .c(new_n16_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n42_), .O(z6));
endmodule


