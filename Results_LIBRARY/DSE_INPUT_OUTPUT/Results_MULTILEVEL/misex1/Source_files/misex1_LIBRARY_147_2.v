// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:31 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x6), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  nand3  g10(.a(x3), .b(new_n18_), .c(x0), .O(new_n26_));
  oai21  g11(.a(new_n18_), .b(x0), .c(new_n26_), .O(new_n27_));
  nor2   g12(.a(new_n23_), .b(x3), .O(new_n28_));
  aoi22  g13(.a(new_n28_), .b(new_n16_), .c(new_n27_), .d(new_n24_), .O(new_n29_));
  nand2  g14(.a(x6), .b(x5), .O(new_n30_));
  oai21  g15(.a(x5), .b(new_n19_), .c(new_n30_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(x2), .c(new_n18_), .d(new_n16_), .O(new_n32_));
  oai21  g17(.a(new_n29_), .b(x2), .c(new_n32_), .O(z1));
  inv1   g18(.a(x2), .O(new_n34_));
  aoi21  g19(.a(x4), .b(new_n34_), .c(x3), .O(new_n35_));
  oai21  g20(.a(x5), .b(new_n34_), .c(x6), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n19_), .c(new_n18_), .O(new_n37_));
  oai21  g22(.a(new_n35_), .b(new_n18_), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  inv1   g24(.a(new_n24_), .O(new_n40_));
  nor2   g25(.a(x1), .b(new_n16_), .O(new_n41_));
  nand2  g26(.a(x3), .b(new_n34_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n39_), .O(z2));
  inv1   g30(.a(x4), .O(new_n46_));
  nand4  g31(.a(new_n24_), .b(new_n46_), .c(new_n34_), .d(x1), .O(new_n47_));
  oai21  g32(.a(new_n23_), .b(new_n16_), .c(x5), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x2), .c(new_n18_), .O(new_n49_));
  oai21  g34(.a(new_n47_), .b(x0), .c(new_n49_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n19_), .O(new_n51_));
  nor2   g36(.a(new_n40_), .b(x7), .O(new_n52_));
  nand4  g37(.a(new_n52_), .b(new_n34_), .c(x1), .d(new_n16_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n51_), .O(z3));
  nand2  g39(.a(new_n19_), .b(x2), .O(new_n55_));
  oai21  g40(.a(new_n42_), .b(new_n16_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n18_), .O(new_n57_));
  aoi21  g42(.a(x3), .b(new_n18_), .c(x2), .O(new_n58_));
  nor2   g43(.a(new_n19_), .b(new_n34_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n58_), .c(new_n16_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n57_), .c(new_n24_), .O(z4));
  inv1   g46(.a(x5), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n19_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(x2), .c(new_n18_), .O(new_n64_));
  aoi21  g49(.a(x6), .b(new_n46_), .c(new_n62_), .O(new_n65_));
  oai21  g50(.a(x6), .b(new_n62_), .c(x3), .O(new_n66_));
  oai21  g51(.a(new_n65_), .b(x2), .c(new_n66_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(x1), .O(new_n68_));
  nand2  g53(.a(new_n23_), .b(new_n62_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n19_), .c(new_n34_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(new_n68_), .c(new_n64_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n16_), .O(new_n72_));
  nand2  g57(.a(new_n55_), .b(new_n42_), .O(new_n73_));
  nand4  g58(.a(new_n73_), .b(new_n24_), .c(new_n18_), .d(x0), .O(new_n74_));
  and2   g59(.a(new_n74_), .b(new_n24_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n72_), .O(z5));
  nand3  g61(.a(new_n48_), .b(new_n19_), .c(x2), .O(new_n77_));
  nand4  g62(.a(new_n24_), .b(x3), .c(new_n34_), .d(x0), .O(new_n78_));
  nand2  g63(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g64(.a(new_n79_), .b(new_n18_), .O(new_n80_));
  oai21  g65(.a(x4), .b(x2), .c(new_n19_), .O(new_n81_));
  nand4  g66(.a(new_n81_), .b(new_n24_), .c(x1), .d(new_n16_), .O(new_n82_));
  nand2  g67(.a(new_n82_), .b(new_n80_), .O(z6));
endmodule


