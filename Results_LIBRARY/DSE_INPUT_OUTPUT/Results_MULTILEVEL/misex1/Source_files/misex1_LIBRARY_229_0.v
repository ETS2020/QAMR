// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  nand3  g08(.a(x6), .b(x5), .c(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(x1), .O(new_n25_));
  nand3  g10(.a(x5), .b(new_n16_), .c(x1), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n17_), .O(new_n28_));
  nand2  g13(.a(x5), .b(x3), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  nand4  g15(.a(new_n30_), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(z1));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x6), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(x5), .c(new_n16_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n20_), .c(new_n19_), .O(new_n38_));
  oai21  g23(.a(x4), .b(x3), .c(x5), .O(new_n39_));
  nand2  g24(.a(x3), .b(x2), .O(new_n40_));
  oai21  g25(.a(new_n39_), .b(x2), .c(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x1), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n31_), .O(z2));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(x5), .c(new_n16_), .d(x1), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n17_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n49_));
  oai21  g34(.a(new_n47_), .b(x0), .c(new_n49_), .O(z3));
  nand2  g35(.a(new_n20_), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n16_), .b(x0), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n29_), .c(new_n51_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  inv1   g39(.a(new_n40_), .O(new_n55_));
  aoi21  g40(.a(x3), .b(new_n19_), .c(new_n33_), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n16_), .c(new_n55_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(x0), .c(new_n54_), .O(z4));
  nand2  g43(.a(new_n30_), .b(new_n16_), .O(new_n59_));
  aoi21  g44(.a(new_n51_), .b(new_n59_), .c(new_n17_), .O(new_n60_));
  nor3   g45(.a(new_n33_), .b(new_n16_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n19_), .O(new_n62_));
  aoi21  g47(.a(new_n29_), .b(x2), .c(new_n19_), .O(new_n63_));
  nand3  g48(.a(x6), .b(new_n20_), .c(new_n16_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n40_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n17_), .O(new_n66_));
  nor2   g51(.a(x5), .b(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n66_), .c(new_n62_), .O(z5));
  nand3  g54(.a(new_n33_), .b(new_n20_), .c(x2), .O(new_n70_));
  inv1   g55(.a(new_n70_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n60_), .c(new_n19_), .O(new_n72_));
  inv1   g57(.a(x4), .O(new_n73_));
  nand3  g58(.a(x5), .b(new_n73_), .c(new_n16_), .O(new_n74_));
  oai21  g59(.a(new_n67_), .b(new_n20_), .c(new_n74_), .O(new_n75_));
  nand3  g60(.a(new_n75_), .b(x1), .c(new_n17_), .O(new_n76_));
  nand2  g61(.a(new_n76_), .b(new_n72_), .O(z6));
endmodule


