// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x5), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  nor2   g03(.a(x3), .b(x1), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n16_), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x1), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(x0), .O(new_n26_));
  nand2  g11(.a(new_n16_), .b(x2), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x0), .c(new_n26_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(x3), .c(new_n24_), .O(new_n29_));
  inv1   g14(.a(x3), .O(new_n30_));
  nand2  g15(.a(x6), .b(new_n30_), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n24_), .c(x2), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n29_), .O(z1));
  inv1   g19(.a(x6), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n25_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n30_), .c(new_n24_), .O(new_n38_));
  inv1   g23(.a(x4), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n30_), .c(x2), .O(new_n40_));
  nor2   g25(.a(x5), .b(new_n30_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n40_), .c(x1), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n17_), .O(new_n44_));
  nand4  g29(.a(x3), .b(new_n25_), .c(new_n24_), .d(x0), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n25_), .c(x1), .d(new_n17_), .O(new_n48_));
  oai21  g33(.a(new_n19_), .b(x5), .c(x2), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(z3));
  nand2  g35(.a(x3), .b(new_n25_), .O(new_n51_));
  nand2  g36(.a(new_n30_), .b(x2), .O(new_n52_));
  oai21  g37(.a(new_n51_), .b(new_n17_), .c(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n24_), .O(new_n54_));
  oai21  g39(.a(new_n30_), .b(x0), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(x2), .O(new_n56_));
  nand2  g41(.a(x3), .b(new_n24_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n25_), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(z4));
  nand3  g44(.a(new_n16_), .b(new_n30_), .c(x2), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n24_), .c(x0), .O(new_n62_));
  nand2  g47(.a(new_n41_), .b(x2), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n32_), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  oai21  g51(.a(x4), .b(x2), .c(new_n30_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(x1), .c(new_n17_), .O(new_n68_));
  nand2  g53(.a(x5), .b(x2), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n68_), .c(new_n54_), .O(z6));
endmodule


