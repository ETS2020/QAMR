// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(x7), .b(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x7), .O(new_n23_));
  nand2  g08(.a(x3), .b(new_n16_), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x1), .c(new_n23_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(x0), .O(new_n26_));
  inv1   g11(.a(x3), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n27_), .c(new_n16_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n19_), .O(new_n31_));
  aoi22  g16(.a(new_n31_), .b(new_n16_), .c(new_n29_), .d(new_n19_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x0), .c(new_n26_), .O(z1));
  nand2  g18(.a(x4), .b(new_n16_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n27_), .c(new_n19_), .O(new_n35_));
  nand2  g20(.a(new_n28_), .b(x2), .O(new_n36_));
  nand2  g21(.a(new_n30_), .b(new_n16_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n36_), .c(x3), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n19_), .c(new_n35_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(x0), .c(new_n26_), .O(z2));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n16_), .c(x1), .O(new_n42_));
  nand4  g27(.a(new_n28_), .b(new_n27_), .c(x2), .d(new_n19_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n17_), .O(new_n45_));
  nand4  g30(.a(new_n20_), .b(x2), .c(new_n19_), .d(x0), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(z3));
  nand2  g32(.a(new_n23_), .b(x1), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(x3), .c(x0), .O(new_n49_));
  oai21  g34(.a(new_n27_), .b(x1), .c(x7), .O(new_n50_));
  nand3  g35(.a(x3), .b(new_n19_), .c(x0), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n49_), .c(new_n16_), .O(new_n53_));
  nand3  g38(.a(new_n20_), .b(x2), .c(new_n19_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(x0), .O(new_n56_));
  aoi21  g41(.a(new_n27_), .b(x1), .c(x0), .O(new_n57_));
  nor2   g42(.a(new_n23_), .b(new_n27_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n57_), .c(x2), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n56_), .c(new_n53_), .O(z4));
  nand2  g45(.a(new_n27_), .b(x2), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n24_), .c(x7), .O(new_n62_));
  nor3   g47(.a(new_n28_), .b(new_n16_), .c(x0), .O(new_n63_));
  aoi21  g48(.a(new_n62_), .b(x0), .c(new_n63_), .O(new_n64_));
  nand2  g49(.a(new_n61_), .b(x1), .O(new_n65_));
  nand2  g50(.a(x3), .b(x2), .O(new_n66_));
  nand3  g51(.a(x6), .b(new_n27_), .c(new_n16_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n17_), .O(new_n69_));
  oai21  g54(.a(new_n64_), .b(x1), .c(new_n69_), .O(z5));
  nor2   g55(.a(x4), .b(x2), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(x3), .c(x1), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n43_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n17_), .O(new_n74_));
  aoi21  g59(.a(new_n62_), .b(new_n19_), .c(x7), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(new_n17_), .c(new_n74_), .O(z6));
endmodule


