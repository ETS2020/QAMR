// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x7), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n18_), .c(new_n16_), .O(z0));
  oai21  g08(.a(new_n21_), .b(x3), .c(x1), .O(new_n24_));
  nand3  g09(.a(new_n21_), .b(x6), .c(new_n20_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand2  g11(.a(new_n21_), .b(x5), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n20_), .c(new_n16_), .O(new_n28_));
  aoi22  g13(.a(new_n28_), .b(new_n19_), .c(new_n26_), .d(new_n16_), .O(new_n29_));
  nand4  g14(.a(x3), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n30_));
  oai21  g15(.a(new_n29_), .b(x0), .c(new_n30_), .O(z1));
  nor2   g16(.a(new_n21_), .b(x3), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand3  g18(.a(new_n16_), .b(new_n19_), .c(x0), .O(new_n34_));
  oai21  g19(.a(new_n19_), .b(x0), .c(new_n34_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x3), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x2), .c(new_n21_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x1), .O(new_n39_));
  inv1   g24(.a(x5), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x2), .O(new_n41_));
  inv1   g26(.a(x6), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n20_), .c(new_n19_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n39_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n17_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n36_), .c(new_n33_), .O(z2));
  nand2  g33(.a(x7), .b(x3), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n50_));
  aoi21  g35(.a(x5), .b(new_n17_), .c(new_n16_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n19_), .c(x7), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x3), .c(new_n50_), .O(z3));
  nor2   g38(.a(x7), .b(x2), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x3), .c(x1), .O(new_n55_));
  nand2  g40(.a(x3), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n20_), .b(new_n16_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n17_), .O(new_n59_));
  nand2  g44(.a(x3), .b(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n20_), .b(x2), .O(new_n61_));
  oai21  g46(.a(new_n60_), .b(new_n17_), .c(new_n61_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n19_), .c(new_n32_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n59_), .O(z4));
  nand3  g49(.a(new_n21_), .b(new_n20_), .c(x2), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n60_), .c(new_n17_), .O(new_n66_));
  nand4  g51(.a(new_n21_), .b(x5), .c(new_n20_), .d(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n66_), .c(new_n19_), .O(new_n69_));
  nand4  g54(.a(new_n21_), .b(x6), .c(new_n20_), .d(new_n16_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(new_n56_), .c(new_n55_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n69_), .O(z5));
  aoi21  g58(.a(new_n61_), .b(new_n60_), .c(new_n17_), .O(new_n74_));
  nand3  g59(.a(new_n40_), .b(new_n20_), .c(x2), .O(new_n75_));
  inv1   g60(.a(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n74_), .c(new_n19_), .O(new_n77_));
  oai21  g62(.a(x4), .b(x2), .c(new_n20_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(x1), .c(new_n17_), .O(new_n79_));
  nand3  g64(.a(new_n79_), .b(new_n77_), .c(new_n33_), .O(z6));
endmodule


