// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x5), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n18_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n16_), .c(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(z0));
  xor2a  g09(.a(x2), .b(x0), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x3), .c(new_n19_), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n20_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n19_), .c(x2), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand2  g14(.a(x5), .b(x2), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n26_), .O(z1));
  nand2  g16(.a(new_n16_), .b(x2), .O(new_n32_));
  inv1   g17(.a(x2), .O(new_n33_));
  inv1   g18(.a(x6), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n20_), .c(new_n19_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n20_), .c(x2), .O(new_n39_));
  nor2   g24(.a(x5), .b(new_n20_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n39_), .c(x1), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(new_n43_));
  nor2   g28(.a(new_n20_), .b(x2), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n19_), .c(x0), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n33_), .c(x1), .d(new_n17_), .O(new_n48_));
  nand4  g33(.a(new_n16_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(z3));
  nand2  g35(.a(new_n44_), .b(x0), .O(new_n51_));
  nand3  g36(.a(new_n16_), .b(new_n20_), .c(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  oai21  g39(.a(new_n20_), .b(x0), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(x2), .O(new_n56_));
  nand2  g41(.a(x3), .b(new_n19_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n33_), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(z4));
  oai21  g44(.a(new_n20_), .b(x2), .c(new_n52_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n19_), .c(x0), .O(new_n61_));
  nand2  g46(.a(new_n40_), .b(x2), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n28_), .c(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n61_), .O(z5));
  oai21  g50(.a(x4), .b(x2), .c(new_n20_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(x1), .c(new_n17_), .O(new_n67_));
  oai21  g52(.a(x3), .b(new_n33_), .c(new_n51_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n19_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n67_), .c(new_n30_), .O(z6));
endmodule


