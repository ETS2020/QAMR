// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x6), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x2), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n16_), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  nor2   g08(.a(new_n19_), .b(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x1), .c(new_n17_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n22_), .O(z0));
  inv1   g11(.a(x5), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n19_), .O(new_n28_));
  nand4  g13(.a(new_n28_), .b(new_n18_), .c(x2), .d(new_n16_), .O(new_n29_));
  nand2  g14(.a(new_n23_), .b(x1), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n17_), .O(new_n32_));
  nor2   g17(.a(x1), .b(new_n17_), .O(new_n33_));
  nand4  g18(.a(new_n33_), .b(new_n18_), .c(x3), .d(new_n23_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n32_), .O(z1));
  aoi21  g20(.a(x4), .b(new_n23_), .c(x3), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n19_), .c(new_n16_), .O(new_n38_));
  oai21  g23(.a(new_n36_), .b(new_n16_), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand3  g25(.a(x3), .b(new_n23_), .c(x0), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n40_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n23_), .c(x1), .d(new_n17_), .O(new_n46_));
  aoi21  g31(.a(x5), .b(new_n17_), .c(x6), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(z3));
  aoi21  g34(.a(new_n19_), .b(x2), .c(new_n16_), .O(new_n50_));
  xnor2a g35(.a(x3), .b(x2), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n18_), .c(new_n50_), .O(new_n52_));
  nand2  g37(.a(new_n41_), .b(new_n20_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n18_), .c(new_n16_), .O(new_n54_));
  oai21  g39(.a(new_n52_), .b(x0), .c(new_n54_), .O(z4));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n37_), .b(x0), .c(new_n18_), .O(new_n57_));
  aoi21  g42(.a(new_n56_), .b(x0), .c(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n50_), .b(new_n24_), .c(new_n17_), .O(new_n59_));
  oai21  g44(.a(new_n58_), .b(x1), .c(new_n59_), .O(z5));
  nand2  g45(.a(new_n56_), .b(x0), .O(new_n61_));
  nor2   g46(.a(x5), .b(x3), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(x2), .c(x6), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n16_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n19_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(x1), .c(new_n17_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z6));
endmodule


