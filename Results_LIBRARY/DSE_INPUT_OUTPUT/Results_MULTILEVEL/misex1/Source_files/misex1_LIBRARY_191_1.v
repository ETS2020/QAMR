// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x7), .O(new_n18_));
  nand2  g03(.a(x3), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n17_), .c(x0), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n21_), .O(z0));
  inv1   g11(.a(x5), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n22_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(x2), .c(new_n17_), .O(new_n29_));
  inv1   g14(.a(x2), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x3), .c(new_n17_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n29_), .c(new_n18_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  nand4  g20(.a(x3), .b(new_n30_), .c(new_n17_), .d(x0), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n35_), .O(z1));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n22_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  nand2  g25(.a(new_n27_), .b(x2), .O(new_n41_));
  nand2  g26(.a(new_n31_), .b(new_n30_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n22_), .c(new_n17_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n40_), .c(new_n18_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n36_), .O(z2));
  aoi21  g32(.a(new_n30_), .b(x1), .c(x7), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n16_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n22_), .c(x2), .d(new_n17_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x0), .c(new_n50_), .O(z3));
  nand2  g36(.a(x3), .b(new_n30_), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n23_), .c(new_n16_), .O(new_n53_));
  nand3  g38(.a(new_n18_), .b(x2), .c(new_n16_), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n17_), .O(new_n56_));
  oai21  g41(.a(new_n22_), .b(x1), .c(new_n30_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n19_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n18_), .c(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(z4));
  nor3   g45(.a(new_n27_), .b(new_n30_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n53_), .c(new_n17_), .O(new_n62_));
  nor2   g47(.a(new_n24_), .b(new_n17_), .O(new_n63_));
  nand3  g48(.a(x6), .b(new_n22_), .c(new_n30_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n19_), .c(new_n18_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n16_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n62_), .O(z5));
  nand3  g52(.a(new_n27_), .b(new_n22_), .c(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n53_), .c(new_n17_), .O(new_n70_));
  nor2   g55(.a(x4), .b(x2), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(x3), .c(x1), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n18_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n16_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n70_), .O(z6));
endmodule


