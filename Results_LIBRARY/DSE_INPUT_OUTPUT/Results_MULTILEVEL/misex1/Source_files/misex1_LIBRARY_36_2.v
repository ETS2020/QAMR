// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x7), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x2), .c(new_n16_), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x3), .b(new_n27_), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x0), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n18_), .O(new_n32_));
  nand2  g17(.a(x6), .b(new_n19_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n18_), .c(x2), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  inv1   g20(.a(x7), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x2), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n35_), .c(new_n32_), .O(z1));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n27_), .O(new_n40_));
  nand3  g25(.a(x7), .b(new_n24_), .c(x2), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n19_), .c(new_n18_), .O(new_n43_));
  nor2   g28(.a(x4), .b(x3), .O(new_n44_));
  nand2  g29(.a(x7), .b(x3), .O(new_n45_));
  oai21  g30(.a(new_n44_), .b(x2), .c(new_n45_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(x1), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n43_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n16_), .O(new_n49_));
  nand3  g34(.a(new_n29_), .b(new_n18_), .c(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(z2));
  oai21  g36(.a(x4), .b(x3), .c(x7), .O(new_n52_));
  nand4  g37(.a(new_n52_), .b(new_n27_), .c(x1), .d(new_n16_), .O(new_n53_));
  aoi21  g38(.a(x5), .b(new_n16_), .c(new_n36_), .O(new_n54_));
  nand4  g39(.a(new_n54_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z3));
  nand3  g41(.a(x7), .b(new_n19_), .c(x2), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n30_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n18_), .O(new_n59_));
  aoi21  g44(.a(x3), .b(new_n18_), .c(x2), .O(new_n60_));
  nand3  g45(.a(x7), .b(x3), .c(x2), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n16_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n59_), .O(z4));
  aoi21  g49(.a(new_n57_), .b(new_n28_), .c(new_n16_), .O(new_n65_));
  nand4  g50(.a(x7), .b(x5), .c(x2), .d(new_n16_), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n65_), .c(new_n18_), .O(new_n68_));
  oai21  g53(.a(new_n62_), .b(new_n34_), .c(new_n16_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n68_), .O(z5));
  xnor2a g55(.a(x3), .b(x2), .O(new_n71_));
  oai22  g56(.a(new_n71_), .b(new_n16_), .c(new_n25_), .d(new_n27_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n18_), .O(new_n73_));
  oai21  g58(.a(x4), .b(x2), .c(new_n19_), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(x1), .c(new_n16_), .O(new_n75_));
  nand3  g60(.a(new_n75_), .b(new_n73_), .c(new_n37_), .O(z6));
endmodule


