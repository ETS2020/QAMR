// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  nor2   g03(.a(x3), .b(x1), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g06(.a(x1), .O(new_n22_));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n16_), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n22_), .O(new_n26_));
  aoi22  g11(.a(new_n26_), .b(new_n16_), .c(new_n24_), .d(new_n22_), .O(new_n27_));
  nand3  g12(.a(x3), .b(new_n22_), .c(x0), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x5), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n16_), .O(new_n30_));
  oai21  g15(.a(new_n27_), .b(x0), .c(new_n30_), .O(z1));
  inv1   g16(.a(x3), .O(new_n32_));
  nand2  g17(.a(x4), .b(new_n16_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n32_), .c(new_n22_), .O(new_n34_));
  oai21  g19(.a(x6), .b(x2), .c(x5), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n32_), .c(new_n22_), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n34_), .c(new_n17_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n30_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(x1), .c(new_n17_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x5), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  oai21  g28(.a(new_n16_), .b(new_n17_), .c(x5), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n32_), .c(new_n22_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  nor2   g31(.a(new_n32_), .b(x0), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n19_), .c(x2), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n22_), .c(x0), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n29_), .c(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n23_), .b(new_n22_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(z4));
  nand2  g37(.a(new_n32_), .b(x2), .O(new_n53_));
  nand3  g38(.a(x5), .b(x3), .c(new_n16_), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n53_), .c(new_n17_), .O(new_n55_));
  inv1   g40(.a(x5), .O(new_n56_));
  nor3   g41(.a(new_n56_), .b(new_n16_), .c(x0), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n22_), .O(new_n58_));
  oai21  g43(.a(x3), .b(new_n16_), .c(x1), .O(new_n59_));
  nand3  g44(.a(x6), .b(new_n32_), .c(new_n16_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(new_n61_));
  nor2   g46(.a(new_n32_), .b(new_n16_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(new_n17_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n58_), .O(z5));
  nand2  g49(.a(new_n23_), .b(x2), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n55_), .c(new_n22_), .O(new_n67_));
  oai21  g52(.a(x5), .b(x2), .c(x3), .O(new_n68_));
  inv1   g53(.a(x4), .O(new_n69_));
  nand3  g54(.a(x5), .b(new_n69_), .c(new_n16_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(x1), .c(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n67_), .O(z6));
endmodule


