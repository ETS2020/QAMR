// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x5), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n24_), .c(x0), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  nor3   g12(.a(new_n27_), .b(new_n24_), .c(x0), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n18_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n18_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n24_), .c(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(z1));
  oai21  g18(.a(x4), .b(x3), .c(x1), .O(new_n34_));
  nand3  g19(.a(new_n30_), .b(new_n19_), .c(new_n18_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(x0), .O(new_n36_));
  nand3  g21(.a(x3), .b(new_n18_), .c(x0), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n36_), .c(new_n24_), .O(new_n39_));
  nand4  g24(.a(x5), .b(x3), .c(x1), .d(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n24_), .c(x1), .d(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n18_), .b(x0), .O(new_n44_));
  nand3  g29(.a(x5), .b(new_n19_), .c(x2), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(z3));
  nand2  g31(.a(new_n45_), .b(new_n25_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n18_), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n18_), .c(x2), .O(new_n49_));
  nand3  g34(.a(x5), .b(x3), .c(x2), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n49_), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n48_), .O(z4));
  nand2  g38(.a(x3), .b(new_n24_), .O(new_n54_));
  aoi21  g39(.a(new_n45_), .b(new_n54_), .c(new_n16_), .O(new_n55_));
  nor2   g40(.a(new_n24_), .b(x0), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n55_), .c(new_n18_), .O(new_n57_));
  nand2  g42(.a(x5), .b(x3), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(x2), .c(new_n18_), .O(new_n59_));
  nand3  g44(.a(x6), .b(new_n19_), .c(new_n24_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n16_), .O(new_n62_));
  nand2  g47(.a(new_n27_), .b(x2), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n62_), .c(new_n57_), .O(z5));
  nand2  g49(.a(new_n45_), .b(new_n54_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n18_), .c(x0), .O(new_n66_));
  inv1   g51(.a(x4), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(x3), .c(new_n24_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(x1), .c(new_n16_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n66_), .O(z6));
endmodule


