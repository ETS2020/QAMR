// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x5), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(x0), .O(new_n25_));
  nand3  g10(.a(x3), .b(new_n24_), .c(x0), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n18_), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n19_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n24_), .c(new_n16_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x2), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n31_), .c(new_n28_), .O(z1));
  aoi21  g19(.a(x4), .b(new_n24_), .c(x3), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n19_), .c(new_n24_), .d(new_n18_), .O(new_n37_));
  oai21  g22(.a(new_n35_), .b(new_n18_), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  nor2   g24(.a(new_n19_), .b(x2), .O(new_n40_));
  nor2   g25(.a(x1), .b(new_n16_), .O(new_n41_));
  aoi22  g26(.a(new_n41_), .b(new_n40_), .c(new_n32_), .d(x2), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n39_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n24_), .c(x1), .d(new_n16_), .O(new_n45_));
  and2   g30(.a(new_n20_), .b(x5), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n24_), .c(new_n45_), .O(z3));
  nor2   g32(.a(x3), .b(new_n24_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n27_), .c(new_n18_), .O(new_n49_));
  oai21  g34(.a(new_n19_), .b(x0), .c(x5), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x2), .O(new_n51_));
  nand2  g36(.a(x3), .b(new_n18_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n24_), .c(new_n16_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z4));
  nand2  g39(.a(x5), .b(x3), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(x2), .c(new_n18_), .O(new_n56_));
  nand3  g41(.a(x5), .b(x3), .c(x2), .O(new_n57_));
  oai21  g42(.a(new_n29_), .b(x2), .c(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n16_), .O(new_n59_));
  nand3  g44(.a(x5), .b(new_n19_), .c(x2), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n26_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n18_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z5));
  oai21  g48(.a(new_n19_), .b(x2), .c(new_n60_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n18_), .c(x0), .O(new_n65_));
  inv1   g50(.a(x4), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(x3), .c(new_n24_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n55_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(x1), .c(new_n16_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n65_), .O(z6));
endmodule


