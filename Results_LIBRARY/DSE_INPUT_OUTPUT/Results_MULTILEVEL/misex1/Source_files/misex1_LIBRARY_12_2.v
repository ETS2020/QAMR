// Benchmark "FAU" written by ABC on Fri Aug 14 01:58:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x7), .b(x3), .c(x1), .d(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x5), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n18_), .c(new_n16_), .O(new_n24_));
  nand2  g09(.a(x6), .b(new_n18_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(new_n26_));
  aoi22  g11(.a(new_n26_), .b(new_n16_), .c(new_n24_), .d(new_n17_), .O(new_n27_));
  nand3  g12(.a(new_n16_), .b(new_n17_), .c(x0), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x7), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x3), .O(new_n30_));
  oai21  g15(.a(new_n27_), .b(x0), .c(new_n30_), .O(z1));
  nand2  g16(.a(x4), .b(new_n16_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n18_), .c(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n23_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x6), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n34_), .c(x3), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n17_), .c(new_n33_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x0), .c(new_n30_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n16_), .c(x1), .d(new_n20_), .O(new_n41_));
  nand2  g26(.a(x5), .b(new_n20_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n43_));
  inv1   g28(.a(x7), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(x3), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n43_), .c(new_n41_), .O(z3));
  nand2  g31(.a(x7), .b(x3), .O(new_n47_));
  nand2  g32(.a(new_n18_), .b(x2), .O(new_n48_));
  nand2  g33(.a(new_n16_), .b(x0), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n47_), .c(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n17_), .O(new_n51_));
  nand2  g36(.a(x7), .b(x1), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(x3), .c(x2), .O(new_n53_));
  nand3  g38(.a(x7), .b(x3), .c(x2), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n20_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n51_), .O(z4));
  nand3  g42(.a(x7), .b(x3), .c(new_n16_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n48_), .c(new_n20_), .O(new_n59_));
  nand3  g44(.a(x5), .b(new_n18_), .c(x2), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n18_), .b(new_n16_), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(new_n47_), .c(new_n17_), .O(new_n64_));
  oai21  g49(.a(new_n25_), .b(x2), .c(new_n54_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n64_), .c(new_n20_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n62_), .O(z5));
  nand3  g52(.a(new_n23_), .b(new_n18_), .c(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n59_), .c(new_n17_), .O(new_n70_));
  inv1   g55(.a(x4), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n18_), .c(new_n16_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n47_), .O(new_n73_));
  nand3  g58(.a(new_n73_), .b(x1), .c(new_n20_), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(new_n70_), .c(new_n45_), .O(z6));
endmodule


