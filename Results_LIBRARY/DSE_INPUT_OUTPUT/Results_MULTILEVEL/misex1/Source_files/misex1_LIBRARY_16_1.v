// Benchmark "FAU" written by ABC on Fri Aug 14 01:58:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  nand2  g07(.a(x5), .b(new_n19_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(z0));
  xor2a  g09(.a(x2), .b(x0), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x3), .c(new_n18_), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n18_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n16_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n26_), .c(new_n23_), .O(z1));
  nand2  g16(.a(x4), .b(new_n27_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n19_), .c(new_n18_), .O(new_n33_));
  oai21  g18(.a(new_n28_), .b(x2), .c(new_n19_), .O(new_n34_));
  nor2   g19(.a(new_n34_), .b(x1), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n33_), .c(new_n16_), .O(new_n36_));
  nor2   g21(.a(x1), .b(new_n16_), .O(new_n37_));
  nor2   g22(.a(new_n19_), .b(x2), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n37_), .c(x5), .d(new_n19_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n36_), .O(z2));
  inv1   g25(.a(x5), .O(new_n41_));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n27_), .c(x1), .d(new_n16_), .O(new_n43_));
  nand3  g28(.a(new_n19_), .b(x2), .c(new_n18_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n41_), .O(new_n46_));
  inv1   g31(.a(x7), .O(new_n47_));
  nor2   g32(.a(new_n18_), .b(x0), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n47_), .c(x3), .d(new_n27_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n46_), .O(z3));
  nand3  g35(.a(new_n47_), .b(new_n41_), .c(new_n27_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n19_), .c(new_n18_), .O(new_n52_));
  nand2  g37(.a(x3), .b(x2), .O(new_n53_));
  nand3  g38(.a(new_n41_), .b(new_n19_), .c(new_n27_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n52_), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n38_), .b(x0), .O(new_n57_));
  nand3  g42(.a(new_n41_), .b(new_n19_), .c(x2), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n18_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n56_), .O(z4));
  oai21  g46(.a(new_n19_), .b(x2), .c(new_n58_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n18_), .c(x0), .O(new_n63_));
  nor2   g48(.a(x5), .b(x2), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(x3), .c(x1), .O(new_n65_));
  nand4  g50(.a(x6), .b(new_n41_), .c(new_n19_), .d(new_n27_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n65_), .c(new_n53_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n16_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n63_), .O(z5));
  oai21  g54(.a(x4), .b(x2), .c(new_n19_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x1), .c(new_n16_), .O(new_n71_));
  nand2  g56(.a(new_n19_), .b(x2), .O(new_n72_));
  nand2  g57(.a(new_n57_), .b(new_n72_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n18_), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(new_n71_), .c(new_n23_), .O(z6));
endmodule


