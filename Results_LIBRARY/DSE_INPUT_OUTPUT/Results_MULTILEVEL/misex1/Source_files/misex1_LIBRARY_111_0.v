// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(new_n22_));
  nor2   g07(.a(x6), .b(x2), .O(new_n23_));
  or2    g08(.a(new_n23_), .b(new_n22_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  nand2  g10(.a(x3), .b(new_n19_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(x6), .c(new_n16_), .O(new_n27_));
  oai21  g12(.a(new_n25_), .b(x1), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand2  g14(.a(new_n19_), .b(x0), .O(new_n30_));
  nand3  g15(.a(x6), .b(x3), .c(new_n16_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(z1));
  nor2   g17(.a(x5), .b(x3), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n20_), .O(new_n35_));
  nor2   g20(.a(new_n16_), .b(x1), .O(new_n36_));
  aoi22  g21(.a(new_n36_), .b(new_n33_), .c(new_n35_), .d(x1), .O(new_n37_));
  oai21  g22(.a(new_n26_), .b(new_n17_), .c(x6), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  oai21  g24(.a(new_n37_), .b(x0), .c(new_n39_), .O(z2));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(x6), .c(new_n16_), .d(x1), .O(new_n42_));
  nand2  g27(.a(x5), .b(new_n17_), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n44_));
  oai21  g29(.a(new_n42_), .b(x0), .c(new_n44_), .O(z3));
  nand2  g30(.a(new_n20_), .b(x2), .O(new_n46_));
  nand4  g31(.a(x6), .b(x3), .c(new_n16_), .d(x0), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n19_), .O(new_n49_));
  nor2   g34(.a(new_n20_), .b(new_n16_), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  and2   g36(.a(new_n51_), .b(new_n27_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x0), .c(new_n49_), .O(z4));
  aoi21  g38(.a(new_n46_), .b(new_n31_), .c(new_n17_), .O(new_n54_));
  nand2  g39(.a(x5), .b(x2), .O(new_n55_));
  nor2   g40(.a(new_n55_), .b(x0), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n54_), .c(new_n19_), .O(new_n57_));
  inv1   g42(.a(x6), .O(new_n58_));
  nor2   g43(.a(x4), .b(x2), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(x3), .c(x1), .O(new_n60_));
  nand2  g45(.a(new_n20_), .b(new_n16_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n50_), .c(new_n17_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n57_), .O(z5));
  nand2  g49(.a(new_n33_), .b(x2), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n54_), .c(new_n19_), .O(new_n67_));
  nand3  g52(.a(x6), .b(new_n34_), .c(new_n16_), .O(new_n68_));
  oai21  g53(.a(new_n23_), .b(new_n20_), .c(new_n68_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(x1), .c(new_n17_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z6));
endmodule


