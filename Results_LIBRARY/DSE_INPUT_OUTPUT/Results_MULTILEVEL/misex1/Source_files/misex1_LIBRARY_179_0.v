// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x3), .b(x1), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  nor2   g06(.a(x3), .b(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n20_), .c(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n19_), .O(z0));
  inv1   g09(.a(x3), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(x2), .c(new_n16_), .O(new_n28_));
  nand3  g13(.a(x3), .b(new_n21_), .c(x0), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n28_), .c(x1), .O(z1));
  oai21  g15(.a(new_n27_), .b(x1), .c(new_n17_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(x2), .c(new_n16_), .O(new_n32_));
  nor2   g17(.a(new_n25_), .b(x2), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n20_), .c(x0), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n32_), .O(z2));
  aoi21  g20(.a(x5), .b(new_n16_), .c(x3), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(x2), .c(new_n20_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(z3));
  inv1   g23(.a(new_n29_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n22_), .c(new_n20_), .O(new_n40_));
  nand3  g25(.a(x3), .b(x2), .c(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(z4));
  oai21  g27(.a(new_n25_), .b(x1), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  oai21  g29(.a(new_n26_), .b(x1), .c(new_n25_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n16_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n44_), .c(new_n23_), .O(z5));
  aoi21  g32(.a(new_n36_), .b(x2), .c(new_n33_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(x1), .c(new_n19_), .O(z6));
endmodule


