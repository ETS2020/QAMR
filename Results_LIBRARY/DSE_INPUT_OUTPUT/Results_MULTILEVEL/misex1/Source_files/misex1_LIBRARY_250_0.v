// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_;
  nor2   g00(.a(x5), .b(x4), .O(new_n16_));
  inv1   g01(.a(new_n16_), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand3  g03(.a(x3), .b(x1), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n17_), .c(x2), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n21_), .c(new_n26_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n20_), .O(new_n30_));
  aoi22  g15(.a(new_n30_), .b(new_n26_), .c(new_n28_), .d(new_n20_), .O(new_n31_));
  nor2   g16(.a(x1), .b(new_n18_), .O(new_n32_));
  nor2   g17(.a(new_n21_), .b(x2), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n32_), .c(new_n16_), .O(new_n34_));
  oai21  g19(.a(new_n31_), .b(x0), .c(new_n34_), .O(z1));
  nand3  g20(.a(new_n26_), .b(new_n20_), .c(x0), .O(new_n36_));
  oai21  g21(.a(new_n20_), .b(x0), .c(new_n36_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n17_), .c(x3), .O(new_n38_));
  nand3  g23(.a(new_n27_), .b(x4), .c(x2), .O(new_n39_));
  nand3  g24(.a(new_n29_), .b(x5), .c(new_n26_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n39_), .c(x1), .O(new_n41_));
  nand3  g26(.a(new_n29_), .b(x4), .c(new_n26_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n41_), .c(new_n21_), .O(new_n44_));
  nand3  g29(.a(x4), .b(new_n26_), .c(x1), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n38_), .O(z2));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n26_), .c(x1), .d(new_n18_), .O(new_n50_));
  nand2  g35(.a(x5), .b(new_n18_), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(new_n21_), .c(x2), .d(new_n20_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n50_), .c(new_n17_), .O(z3));
  inv1   g38(.a(new_n33_), .O(new_n54_));
  nand2  g39(.a(new_n21_), .b(x2), .O(new_n55_));
  oai21  g40(.a(new_n54_), .b(new_n18_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n20_), .O(new_n57_));
  aoi21  g42(.a(x3), .b(new_n20_), .c(x2), .O(new_n58_));
  nand2  g43(.a(x3), .b(x2), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n18_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n57_), .c(new_n16_), .O(z4));
  aoi21  g47(.a(new_n55_), .b(new_n54_), .c(new_n18_), .O(new_n63_));
  nand3  g48(.a(x5), .b(new_n21_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n20_), .O(new_n66_));
  nand2  g51(.a(new_n55_), .b(x1), .O(new_n67_));
  nand3  g52(.a(x6), .b(new_n21_), .c(new_n26_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n67_), .c(new_n59_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n18_), .c(new_n16_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n66_), .O(z5));
  nand3  g56(.a(new_n27_), .b(new_n21_), .c(x2), .O(new_n72_));
  inv1   g57(.a(new_n72_), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n63_), .c(new_n20_), .O(new_n74_));
  oai21  g59(.a(x4), .b(x2), .c(new_n21_), .O(new_n75_));
  nand3  g60(.a(new_n75_), .b(x1), .c(new_n18_), .O(new_n76_));
  nand3  g61(.a(new_n76_), .b(new_n74_), .c(new_n17_), .O(z6));
endmodule


