// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(x1), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(new_n16_), .c(x0), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nand4  g04(.a(x3), .b(x2), .c(x1), .d(new_n19_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g06(.a(x1), .O(new_n22_));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n16_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n22_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n25_), .c(x0), .O(z1));
  inv1   g14(.a(x3), .O(new_n30_));
  inv1   g15(.a(x4), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x2), .c(new_n30_), .O(new_n32_));
  nor2   g17(.a(new_n16_), .b(x1), .O(new_n33_));
  aoi22  g18(.a(new_n33_), .b(new_n23_), .c(new_n32_), .d(x1), .O(new_n34_));
  nor3   g19(.a(x6), .b(x3), .c(x1), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x0), .c(new_n16_), .O(new_n36_));
  oai21  g21(.a(new_n34_), .b(x0), .c(new_n36_), .O(z2));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand4  g23(.a(new_n38_), .b(new_n16_), .c(x1), .d(new_n19_), .O(new_n39_));
  nand2  g24(.a(x5), .b(new_n19_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n30_), .c(x2), .d(new_n22_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z3));
  aoi21  g27(.a(x3), .b(new_n19_), .c(new_n17_), .O(new_n43_));
  nand2  g28(.a(x3), .b(new_n22_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n16_), .c(new_n19_), .O(new_n45_));
  oai21  g30(.a(new_n43_), .b(new_n16_), .c(new_n45_), .O(z4));
  oai21  g31(.a(x3), .b(new_n16_), .c(x1), .O(new_n47_));
  inv1   g32(.a(x5), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(x1), .c(new_n30_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(x2), .O(new_n50_));
  nand3  g35(.a(x6), .b(new_n30_), .c(new_n16_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n19_), .O(new_n53_));
  nand4  g38(.a(new_n30_), .b(x2), .c(new_n22_), .d(x0), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(z5));
  oai21  g40(.a(x4), .b(x2), .c(new_n30_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(x1), .c(new_n19_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n41_), .O(z6));
endmodule


