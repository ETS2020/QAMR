// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  inv1   g04(.a(x6), .O(new_n20_));
  nor2   g05(.a(x1), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x1), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n19_), .c(new_n16_), .O(new_n26_));
  nor2   g11(.a(x2), .b(new_n24_), .O(new_n27_));
  aoi21  g12(.a(new_n26_), .b(new_n24_), .c(new_n27_), .O(new_n28_));
  nor2   g13(.a(new_n19_), .b(x2), .O(new_n29_));
  aoi22  g14(.a(new_n29_), .b(new_n21_), .c(x6), .d(new_n19_), .O(new_n30_));
  oai21  g15(.a(new_n28_), .b(x0), .c(new_n30_), .O(z1));
  aoi21  g16(.a(x4), .b(new_n16_), .c(x3), .O(new_n32_));
  nand2  g17(.a(x5), .b(x2), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n19_), .c(new_n24_), .O(new_n34_));
  oai21  g19(.a(new_n32_), .b(new_n24_), .c(new_n34_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n17_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n30_), .O(z2));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand4  g23(.a(new_n38_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n39_));
  nand2  g24(.a(x5), .b(new_n17_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n19_), .c(x2), .d(new_n24_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n20_), .O(new_n43_));
  nor2   g28(.a(x7), .b(new_n19_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  nand2  g31(.a(x6), .b(new_n19_), .O(new_n47_));
  nand2  g32(.a(x3), .b(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n19_), .b(x2), .O(new_n49_));
  oai21  g34(.a(new_n48_), .b(new_n17_), .c(new_n49_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n24_), .O(new_n51_));
  aoi21  g36(.a(x3), .b(new_n24_), .c(x2), .O(new_n52_));
  nor2   g37(.a(new_n19_), .b(new_n16_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n52_), .c(new_n17_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n51_), .c(new_n47_), .O(z4));
  aoi21  g40(.a(new_n49_), .b(new_n48_), .c(new_n17_), .O(new_n56_));
  nand3  g41(.a(x5), .b(new_n19_), .c(x2), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n24_), .O(new_n59_));
  aoi21  g44(.a(new_n19_), .b(x2), .c(new_n24_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n53_), .c(new_n17_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n59_), .c(new_n47_), .O(z5));
  nand3  g47(.a(new_n25_), .b(new_n19_), .c(x2), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n56_), .c(new_n24_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n19_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(x1), .c(new_n17_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n65_), .c(new_n47_), .O(z6));
endmodule


