// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x3), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x2), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x0), .c(x6), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x1), .b(new_n20_), .O(new_n21_));
  nand2  g06(.a(x3), .b(x2), .O(new_n22_));
  oai22  g07(.a(new_n22_), .b(new_n21_), .c(new_n19_), .d(x1), .O(z0));
  inv1   g08(.a(x1), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x2), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x0), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  inv1   g14(.a(x2), .O(new_n30_));
  nand3  g15(.a(x3), .b(new_n30_), .c(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n28_), .c(new_n24_), .O(new_n33_));
  nand3  g18(.a(new_n30_), .b(x1), .c(new_n20_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(z1));
  aoi21  g20(.a(x4), .b(new_n30_), .c(x3), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n16_), .c(new_n24_), .O(new_n38_));
  oai21  g23(.a(new_n36_), .b(new_n24_), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n20_), .O(new_n40_));
  nand2  g25(.a(new_n32_), .b(new_n24_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n30_), .c(x1), .d(new_n20_), .O(new_n44_));
  aoi21  g29(.a(x5), .b(new_n20_), .c(x6), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n16_), .c(x2), .d(new_n24_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(z3));
  nand2  g32(.a(new_n16_), .b(new_n30_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n22_), .O(new_n49_));
  aoi22  g34(.a(new_n49_), .b(new_n29_), .c(new_n17_), .d(x1), .O(new_n50_));
  nand2  g35(.a(new_n31_), .b(new_n17_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n29_), .c(new_n24_), .O(new_n52_));
  oai21  g37(.a(new_n50_), .b(x0), .c(new_n52_), .O(z4));
  xnor2a g38(.a(x3), .b(x2), .O(new_n54_));
  nand3  g39(.a(x5), .b(x2), .c(new_n20_), .O(new_n55_));
  oai21  g40(.a(new_n54_), .b(new_n20_), .c(new_n55_), .O(new_n56_));
  nor2   g41(.a(new_n22_), .b(x0), .O(new_n57_));
  aoi21  g42(.a(new_n56_), .b(new_n24_), .c(new_n57_), .O(new_n58_));
  nand3  g43(.a(new_n17_), .b(x1), .c(new_n20_), .O(new_n59_));
  oai21  g44(.a(new_n58_), .b(x6), .c(new_n59_), .O(z5));
  oai22  g45(.a(new_n54_), .b(new_n20_), .c(new_n26_), .d(new_n30_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n29_), .c(new_n24_), .O(new_n62_));
  oai21  g47(.a(x4), .b(x2), .c(new_n16_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(x1), .c(new_n20_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n62_), .O(z6));
endmodule


