// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(x5), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand2  g06(.a(x1), .b(new_n17_), .O(new_n22_));
  nand2  g07(.a(x3), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  nand2  g10(.a(new_n16_), .b(x0), .O(new_n26_));
  nand2  g11(.a(x5), .b(x3), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n26_), .c(new_n22_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  inv1   g14(.a(x5), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x3), .c(new_n25_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n17_), .c(new_n30_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x1), .c(new_n29_), .O(z1));
  nand2  g19(.a(x4), .b(new_n25_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n18_), .c(new_n16_), .O(new_n36_));
  nand2  g21(.a(new_n25_), .b(new_n16_), .O(new_n37_));
  nand3  g22(.a(new_n31_), .b(x5), .c(new_n18_), .O(new_n38_));
  nor2   g23(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n36_), .c(new_n17_), .O(new_n40_));
  nor2   g25(.a(new_n27_), .b(x2), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n16_), .c(x0), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n40_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n25_), .c(x1), .d(new_n17_), .O(new_n45_));
  nand3  g30(.a(x5), .b(new_n18_), .c(x2), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n26_), .c(new_n45_), .O(z3));
  aoi21  g32(.a(new_n18_), .b(x2), .c(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n18_), .b(new_n25_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n23_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(x5), .c(new_n48_), .O(new_n51_));
  nand2  g36(.a(x3), .b(new_n25_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n17_), .c(new_n19_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(x5), .c(new_n16_), .O(new_n54_));
  oai21  g39(.a(new_n51_), .b(x0), .c(new_n54_), .O(z4));
  nand2  g40(.a(x2), .b(new_n16_), .O(new_n56_));
  nand3  g41(.a(x6), .b(new_n18_), .c(new_n25_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(new_n30_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n48_), .c(new_n17_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n54_), .O(z5));
  nand2  g45(.a(new_n52_), .b(new_n19_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(x0), .c(new_n30_), .O(new_n62_));
  oai21  g47(.a(x4), .b(x2), .c(new_n18_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(x1), .c(new_n17_), .O(new_n64_));
  oai21  g49(.a(new_n62_), .b(x1), .c(new_n64_), .O(z6));
endmodule


