// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x7), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x3), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(x1), .c(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g10(.a(x5), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n18_), .c(new_n16_), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n17_), .O(new_n29_));
  aoi22  g14(.a(new_n29_), .b(new_n16_), .c(new_n27_), .d(new_n17_), .O(new_n30_));
  nand3  g15(.a(new_n16_), .b(new_n17_), .c(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n21_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x3), .O(new_n33_));
  oai21  g18(.a(new_n30_), .b(x0), .c(new_n33_), .O(z1));
  nand3  g19(.a(x4), .b(new_n18_), .c(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n22_), .c(new_n17_), .O(new_n36_));
  nand2  g21(.a(new_n26_), .b(x2), .O(new_n37_));
  nand2  g22(.a(new_n28_), .b(new_n16_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n37_), .c(x3), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n17_), .c(new_n36_), .O(new_n40_));
  nand3  g25(.a(new_n21_), .b(x3), .c(new_n16_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n17_), .c(x0), .O(new_n43_));
  oai21  g28(.a(new_n40_), .b(x0), .c(new_n43_), .O(z2));
  nand2  g29(.a(x5), .b(new_n20_), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n46_));
  nand3  g31(.a(new_n16_), .b(x1), .c(new_n20_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n21_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x3), .O(new_n49_));
  nand2  g34(.a(x7), .b(x4), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n16_), .c(x1), .d(new_n20_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n46_), .O(z3));
  nand3  g37(.a(x3), .b(new_n16_), .c(x0), .O(new_n53_));
  nand2  g38(.a(new_n18_), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n17_), .O(new_n56_));
  oai21  g41(.a(x2), .b(x1), .c(x3), .O(new_n57_));
  aoi21  g42(.a(new_n50_), .b(x1), .c(new_n18_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(x2), .c(new_n57_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n20_), .O(new_n60_));
  nand2  g45(.a(x7), .b(x3), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n60_), .c(new_n56_), .O(z4));
  aoi21  g47(.a(new_n54_), .b(new_n41_), .c(new_n20_), .O(new_n63_));
  nand3  g48(.a(x5), .b(new_n18_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n18_), .b(new_n16_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n22_), .c(new_n17_), .O(new_n68_));
  nand3  g53(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n69_));
  oai21  g54(.a(new_n22_), .b(new_n16_), .c(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n68_), .c(new_n20_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n66_), .O(z5));
  nand3  g57(.a(new_n26_), .b(new_n18_), .c(x2), .O(new_n73_));
  inv1   g58(.a(new_n73_), .O(new_n74_));
  oai21  g59(.a(new_n74_), .b(new_n63_), .c(new_n17_), .O(new_n75_));
  inv1   g60(.a(x4), .O(new_n76_));
  nand3  g61(.a(new_n76_), .b(new_n18_), .c(new_n16_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n22_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(x1), .c(new_n20_), .O(new_n79_));
  nand2  g64(.a(new_n79_), .b(new_n75_), .O(z6));
endmodule


