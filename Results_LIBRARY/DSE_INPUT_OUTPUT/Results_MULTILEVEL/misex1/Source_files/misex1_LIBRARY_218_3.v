// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x5), .O(new_n16_));
  nand2  g01(.a(x3), .b(x2), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(new_n16_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x0), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n21_), .c(new_n19_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n22_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand3  g13(.a(new_n16_), .b(new_n26_), .c(x1), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  aoi21  g15(.a(new_n28_), .b(new_n20_), .c(new_n30_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n26_), .O(new_n32_));
  or2    g17(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(x0), .c(new_n33_), .O(z1));
  nand3  g19(.a(new_n26_), .b(new_n20_), .c(x0), .O(new_n35_));
  oai21  g20(.a(new_n20_), .b(x0), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x3), .O(new_n37_));
  inv1   g22(.a(x0), .O(new_n38_));
  nand3  g23(.a(x4), .b(new_n26_), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  nand2  g26(.a(new_n16_), .b(x2), .O(new_n42_));
  oai21  g27(.a(x6), .b(x2), .c(new_n42_), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n22_), .c(new_n20_), .d(new_n38_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n41_), .c(new_n37_), .O(z2));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n26_), .c(new_n38_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x1), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n38_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n22_), .c(x2), .d(new_n20_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(z3));
  xor2a  g37(.a(x3), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(x0), .O(new_n54_));
  nand2  g39(.a(new_n32_), .b(new_n38_), .O(new_n55_));
  nand3  g40(.a(new_n16_), .b(new_n22_), .c(x2), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n20_), .O(new_n58_));
  aoi21  g43(.a(new_n22_), .b(x2), .c(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(x5), .c(x1), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z4));
  nand3  g46(.a(new_n53_), .b(new_n20_), .c(x0), .O(new_n62_));
  aoi21  g47(.a(new_n22_), .b(x2), .c(new_n20_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n28_), .c(new_n38_), .O(new_n64_));
  nand2  g49(.a(x5), .b(x1), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n64_), .c(new_n62_), .O(z5));
  nand2  g51(.a(new_n56_), .b(new_n54_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n20_), .O(new_n68_));
  nor2   g53(.a(x4), .b(x2), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(x3), .c(new_n38_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n16_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(x1), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n68_), .O(z6));
endmodule


