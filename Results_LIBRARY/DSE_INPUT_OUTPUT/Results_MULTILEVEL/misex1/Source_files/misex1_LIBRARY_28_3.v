// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x6), .O(new_n20_));
  nor2   g05(.a(new_n17_), .b(x0), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x3), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  nor2   g08(.a(x2), .b(x1), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x6), .c(x0), .O(new_n25_));
  nand2  g10(.a(x6), .b(x1), .O(new_n26_));
  inv1   g11(.a(x0), .O(new_n27_));
  nand3  g12(.a(x2), .b(new_n17_), .c(new_n27_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n26_), .c(new_n25_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x3), .O(new_n30_));
  aoi21  g15(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nor3   g17(.a(new_n32_), .b(new_n16_), .c(x1), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n31_), .c(new_n27_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n30_), .O(z1));
  nand2  g20(.a(x4), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n18_), .c(new_n17_), .O(new_n37_));
  nand2  g22(.a(new_n32_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n20_), .b(new_n16_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n38_), .c(x3), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n17_), .c(new_n37_), .O(new_n41_));
  nand2  g26(.a(new_n24_), .b(x0), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n20_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x3), .O(new_n44_));
  oai21  g29(.a(new_n41_), .b(x0), .c(new_n44_), .O(z2));
  nand2  g30(.a(x7), .b(x4), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n16_), .c(x1), .d(new_n27_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n27_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x2), .c(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n18_), .O(new_n51_));
  nor3   g36(.a(x7), .b(x6), .c(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n51_), .O(z3));
  nand2  g39(.a(x3), .b(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n18_), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n55_), .b(new_n27_), .c(new_n56_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  oai21  g43(.a(new_n16_), .b(x0), .c(new_n20_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(x3), .O(new_n60_));
  nand3  g45(.a(new_n20_), .b(x3), .c(new_n17_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n16_), .c(new_n27_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(z4));
  aoi21  g48(.a(new_n56_), .b(new_n55_), .c(new_n27_), .O(new_n64_));
  nand3  g49(.a(x5), .b(new_n18_), .c(x2), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n64_), .c(new_n17_), .O(new_n67_));
  aoi21  g52(.a(new_n18_), .b(x2), .c(new_n17_), .O(new_n68_));
  nand2  g53(.a(x3), .b(x2), .O(new_n69_));
  oai21  g54(.a(new_n20_), .b(x2), .c(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n68_), .c(new_n27_), .O(new_n71_));
  nand2  g56(.a(x6), .b(x3), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n71_), .c(new_n67_), .O(z5));
  nand3  g58(.a(new_n32_), .b(new_n18_), .c(x2), .O(new_n74_));
  inv1   g59(.a(new_n74_), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(new_n64_), .c(new_n17_), .O(new_n76_));
  oai21  g61(.a(x4), .b(x2), .c(new_n18_), .O(new_n77_));
  nand3  g62(.a(new_n77_), .b(x1), .c(new_n27_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(new_n76_), .c(new_n72_), .O(z6));
endmodule


