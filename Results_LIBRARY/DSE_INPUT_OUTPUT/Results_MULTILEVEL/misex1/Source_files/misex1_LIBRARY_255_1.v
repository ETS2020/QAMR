// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_,
    new_n50_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nor2   g02(.a(x1), .b(new_n17_), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n16_), .c(x2), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(z0));
  inv1   g05(.a(x1), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(x3), .c(new_n21_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n17_), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n21_), .c(x0), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(x2), .O(z1));
  inv1   g11(.a(x2), .O(new_n27_));
  inv1   g12(.a(x4), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(x1), .O(new_n30_));
  nor2   g15(.a(x6), .b(x3), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n21_), .c(x2), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n17_), .O(new_n34_));
  nand3  g19(.a(new_n18_), .b(x3), .c(new_n27_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n34_), .O(z2));
  nand2  g21(.a(new_n29_), .b(x7), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(new_n27_), .c(x1), .d(new_n17_), .O(new_n38_));
  nand3  g23(.a(new_n18_), .b(new_n16_), .c(x2), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n38_), .O(z3));
  nand2  g25(.a(x3), .b(new_n27_), .O(new_n41_));
  nand2  g26(.a(new_n16_), .b(x2), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n21_), .c(x0), .O(new_n44_));
  oai21  g29(.a(new_n41_), .b(x1), .c(new_n17_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z4));
  nand3  g31(.a(new_n23_), .b(new_n27_), .c(new_n17_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n44_), .O(z5));
  nand2  g33(.a(x4), .b(new_n16_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(x1), .c(x2), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(x0), .c(new_n44_), .O(z6));
endmodule


