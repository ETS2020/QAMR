// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x4), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x2), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n16_), .O(new_n22_));
  nor2   g07(.a(new_n16_), .b(x0), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(x3), .c(x2), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  oai21  g10(.a(x5), .b(x3), .c(x2), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand3  g12(.a(x6), .b(new_n19_), .c(new_n27_), .O(new_n28_));
  oai21  g13(.a(new_n26_), .b(x1), .c(new_n28_), .O(new_n29_));
  nor2   g14(.a(x2), .b(new_n16_), .O(new_n30_));
  aoi21  g15(.a(new_n29_), .b(new_n18_), .c(new_n30_), .O(new_n31_));
  nor2   g16(.a(x4), .b(new_n19_), .O(new_n32_));
  nand4  g17(.a(new_n32_), .b(new_n27_), .c(new_n16_), .d(x0), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(x0), .c(new_n33_), .O(z1));
  nand3  g19(.a(new_n27_), .b(new_n16_), .c(x0), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n23_), .c(x3), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n27_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n19_), .c(new_n16_), .O(new_n43_));
  nand2  g28(.a(x4), .b(new_n27_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n17_), .O(new_n46_));
  nand2  g31(.a(x4), .b(new_n16_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n46_), .c(new_n37_), .O(z2));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n27_), .c(x1), .d(new_n17_), .O(new_n50_));
  aoi21  g35(.a(x5), .b(new_n17_), .c(x4), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z3));
  nor2   g38(.a(new_n19_), .b(x0), .O(new_n54_));
  nor2   g39(.a(x3), .b(x1), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(x2), .O(new_n56_));
  nand3  g41(.a(x3), .b(new_n27_), .c(x0), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n18_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n32_), .b(new_n16_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n27_), .c(new_n17_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n59_), .c(new_n56_), .O(z4));
  xnor2a g47(.a(x3), .b(x2), .O(new_n63_));
  nand3  g48(.a(x5), .b(x2), .c(new_n17_), .O(new_n64_));
  oai21  g49(.a(new_n63_), .b(new_n17_), .c(new_n64_), .O(new_n65_));
  nand2  g50(.a(x3), .b(x2), .O(new_n66_));
  aoi21  g51(.a(new_n28_), .b(new_n66_), .c(x0), .O(new_n67_));
  aoi21  g52(.a(new_n65_), .b(new_n16_), .c(new_n67_), .O(new_n68_));
  nand3  g53(.a(new_n20_), .b(x1), .c(new_n17_), .O(new_n69_));
  oai21  g54(.a(new_n68_), .b(x4), .c(new_n69_), .O(z5));
  nand3  g55(.a(new_n38_), .b(new_n19_), .c(x2), .O(new_n71_));
  oai21  g56(.a(new_n63_), .b(new_n17_), .c(new_n71_), .O(new_n72_));
  nand3  g57(.a(new_n27_), .b(x1), .c(new_n17_), .O(new_n73_));
  inv1   g58(.a(new_n73_), .O(new_n74_));
  aoi21  g59(.a(new_n72_), .b(new_n16_), .c(new_n74_), .O(new_n75_));
  nand3  g60(.a(x3), .b(x1), .c(new_n17_), .O(new_n76_));
  oai21  g61(.a(new_n75_), .b(x4), .c(new_n76_), .O(z6));
endmodule


