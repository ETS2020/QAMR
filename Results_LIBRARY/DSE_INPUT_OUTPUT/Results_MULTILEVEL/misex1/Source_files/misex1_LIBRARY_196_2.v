// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x5), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(x0), .c(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  nor3   g10(.a(new_n19_), .b(new_n16_), .c(x0), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n25_), .c(new_n23_), .O(new_n27_));
  inv1   g12(.a(x3), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n28_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(new_n17_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n27_), .O(z1));
  aoi21  g17(.a(new_n19_), .b(x3), .c(x4), .O(new_n33_));
  inv1   g18(.a(x6), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n28_), .c(new_n23_), .O(new_n35_));
  oai21  g20(.a(new_n33_), .b(new_n23_), .c(new_n35_), .O(new_n36_));
  nand3  g21(.a(x3), .b(new_n23_), .c(x0), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  aoi21  g23(.a(new_n36_), .b(new_n17_), .c(new_n38_), .O(new_n39_));
  nand4  g24(.a(x5), .b(x3), .c(x1), .d(new_n17_), .O(new_n40_));
  oai21  g25(.a(new_n39_), .b(x2), .c(new_n40_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n43_));
  oai21  g28(.a(new_n21_), .b(new_n16_), .c(new_n43_), .O(z3));
  nand3  g29(.a(x5), .b(new_n28_), .c(x2), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n24_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n23_), .O(new_n47_));
  aoi21  g32(.a(x3), .b(new_n23_), .c(x2), .O(new_n48_));
  nand3  g33(.a(x5), .b(x3), .c(x2), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n48_), .c(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n47_), .O(z4));
  xor2a  g37(.a(x3), .b(x2), .O(new_n53_));
  and2   g38(.a(new_n53_), .b(x0), .O(new_n54_));
  aoi21  g39(.a(new_n29_), .b(new_n16_), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n23_), .O(new_n56_));
  aoi21  g41(.a(new_n28_), .b(x2), .c(new_n23_), .O(new_n57_));
  nor2   g42(.a(x5), .b(new_n16_), .O(new_n58_));
  aoi21  g43(.a(new_n57_), .b(new_n17_), .c(new_n58_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(z5));
  inv1   g45(.a(new_n58_), .O(new_n61_));
  nand3  g46(.a(new_n53_), .b(new_n23_), .c(x0), .O(new_n62_));
  oai21  g47(.a(x4), .b(x2), .c(new_n28_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(x1), .c(new_n17_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(z6));
endmodule


