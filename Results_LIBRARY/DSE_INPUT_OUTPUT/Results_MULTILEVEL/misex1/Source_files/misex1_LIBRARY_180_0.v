// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nor3   g02(.a(new_n17_), .b(new_n16_), .c(x0), .O(new_n18_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(x0), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n21_));
  oai21  g06(.a(x4), .b(x2), .c(new_n21_), .O(z0));
  inv1   g07(.a(x0), .O(new_n23_));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x2), .c(new_n16_), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(x4), .c(new_n27_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n23_), .O(new_n32_));
  nand2  g17(.a(new_n16_), .b(x0), .O(new_n33_));
  nand3  g18(.a(x4), .b(x3), .c(new_n27_), .O(new_n34_));
  or2    g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(z1));
  nand2  g21(.a(x3), .b(x2), .O(new_n37_));
  nand2  g22(.a(x4), .b(new_n27_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n37_), .c(new_n16_), .O(new_n39_));
  inv1   g24(.a(x5), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(x2), .c(new_n16_), .O(new_n41_));
  nand3  g26(.a(new_n28_), .b(x4), .c(new_n27_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n41_), .c(x3), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n39_), .c(new_n23_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n35_), .O(z2));
  nand2  g30(.a(x5), .b(new_n23_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n17_), .c(x2), .d(new_n16_), .O(new_n47_));
  inv1   g32(.a(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x1), .c(new_n23_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(x4), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n27_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n47_), .O(z3));
  nand2  g37(.a(new_n17_), .b(x2), .O(new_n53_));
  nand4  g38(.a(x4), .b(x3), .c(new_n27_), .d(x0), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n16_), .O(new_n56_));
  inv1   g41(.a(new_n37_), .O(new_n57_));
  inv1   g42(.a(x4), .O(new_n58_));
  aoi21  g43(.a(x3), .b(new_n16_), .c(new_n58_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n27_), .c(new_n57_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(x0), .c(new_n56_), .O(z4));
  aoi21  g46(.a(new_n53_), .b(new_n34_), .c(new_n23_), .O(new_n62_));
  nor3   g47(.a(new_n40_), .b(new_n27_), .c(x0), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n62_), .c(new_n16_), .O(new_n64_));
  nand2  g49(.a(new_n37_), .b(new_n30_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n23_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z5));
  nand2  g52(.a(x3), .b(new_n27_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  aoi22  g54(.a(new_n69_), .b(x0), .c(new_n24_), .d(x2), .O(new_n70_));
  aoi21  g55(.a(new_n58_), .b(new_n27_), .c(new_n18_), .O(new_n71_));
  oai21  g56(.a(new_n70_), .b(x1), .c(new_n71_), .O(z6));
endmodule


