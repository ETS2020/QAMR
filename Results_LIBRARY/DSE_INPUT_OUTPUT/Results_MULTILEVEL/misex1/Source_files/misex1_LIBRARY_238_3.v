// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n64_;
  inv1   g00(.a(x4), .O(new_n16_));
  nand2  g01(.a(x3), .b(x2), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(new_n16_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x0), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n21_), .c(new_n19_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n22_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand3  g13(.a(new_n16_), .b(new_n26_), .c(x1), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  aoi21  g15(.a(new_n28_), .b(new_n20_), .c(new_n30_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n26_), .O(new_n32_));
  oai22  g17(.a(new_n32_), .b(new_n21_), .c(new_n31_), .d(x0), .O(z1));
  oai21  g18(.a(new_n22_), .b(x0), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x1), .O(new_n35_));
  inv1   g20(.a(x0), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n26_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(x3), .O(new_n41_));
  nand3  g26(.a(x3), .b(new_n26_), .c(x0), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  aoi21  g28(.a(new_n41_), .b(new_n36_), .c(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x1), .c(new_n35_), .O(z2));
  aoi21  g30(.a(x7), .b(x3), .c(x4), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n26_), .c(x1), .d(new_n36_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n36_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n22_), .c(x2), .d(new_n20_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  aoi21  g35(.a(new_n32_), .b(new_n23_), .c(new_n36_), .O(new_n51_));
  nor2   g36(.a(x5), .b(x3), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(x0), .c(new_n26_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n51_), .c(new_n20_), .O(new_n55_));
  oai21  g40(.a(x2), .b(x0), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n22_), .b(new_n26_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  aoi22  g43(.a(new_n58_), .b(new_n36_), .c(new_n56_), .d(x1), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n55_), .O(z4));
  aoi21  g45(.a(new_n28_), .b(new_n36_), .c(new_n51_), .O(new_n61_));
  nand4  g46(.a(new_n23_), .b(new_n16_), .c(x1), .d(new_n36_), .O(new_n62_));
  oai21  g47(.a(new_n61_), .b(x1), .c(new_n62_), .O(z5));
  aoi21  g48(.a(new_n52_), .b(x2), .c(new_n51_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(x1), .c(new_n62_), .O(z6));
endmodule


