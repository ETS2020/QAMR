// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(new_n22_));
  nor2   g07(.a(x6), .b(x2), .O(new_n23_));
  or2    g08(.a(new_n23_), .b(new_n22_), .O(z0));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  nor3   g10(.a(new_n25_), .b(new_n16_), .c(x1), .O(new_n26_));
  oai21  g11(.a(new_n20_), .b(x1), .c(x6), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x2), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n17_), .O(new_n29_));
  nand2  g14(.a(new_n19_), .b(x0), .O(new_n30_));
  nand3  g15(.a(x6), .b(x3), .c(new_n16_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(z1));
  nand3  g17(.a(new_n25_), .b(x2), .c(new_n17_), .O(new_n33_));
  nand4  g18(.a(x6), .b(x3), .c(new_n16_), .d(x0), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n19_), .O(new_n36_));
  oai21  g21(.a(x4), .b(x3), .c(x6), .O(new_n37_));
  nor2   g22(.a(new_n20_), .b(new_n16_), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(new_n39_));
  oai21  g24(.a(new_n37_), .b(x2), .c(new_n39_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(x1), .c(new_n17_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n36_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(x6), .c(new_n16_), .d(x1), .O(new_n44_));
  nand2  g29(.a(x5), .b(new_n17_), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n46_));
  oai21  g31(.a(new_n44_), .b(x0), .c(new_n46_), .O(z3));
  nand2  g32(.a(new_n20_), .b(x2), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n34_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n19_), .O(new_n50_));
  oai21  g35(.a(new_n38_), .b(new_n28_), .c(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(z4));
  aoi21  g37(.a(new_n48_), .b(new_n31_), .c(new_n17_), .O(new_n53_));
  nand2  g38(.a(x5), .b(x2), .O(new_n54_));
  nor2   g39(.a(new_n54_), .b(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n19_), .O(new_n56_));
  inv1   g41(.a(x6), .O(new_n57_));
  nor2   g42(.a(x4), .b(x2), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(x3), .c(x1), .O(new_n59_));
  nand2  g44(.a(new_n20_), .b(new_n16_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n38_), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n56_), .O(z5));
  nand2  g48(.a(new_n25_), .b(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n53_), .c(new_n19_), .O(new_n66_));
  inv1   g51(.a(x4), .O(new_n67_));
  nand3  g52(.a(x6), .b(new_n67_), .c(new_n16_), .O(new_n68_));
  oai21  g53(.a(new_n23_), .b(new_n20_), .c(new_n68_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(x1), .c(new_n17_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n66_), .O(z6));
endmodule


