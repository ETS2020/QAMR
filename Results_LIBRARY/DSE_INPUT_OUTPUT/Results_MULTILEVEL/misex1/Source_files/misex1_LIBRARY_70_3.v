// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:12 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  aoi21  g02(.a(new_n17_), .b(x2), .c(x1), .O(new_n18_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n19_));
  oai21  g04(.a(new_n18_), .b(new_n16_), .c(new_n19_), .O(z0));
  nor2   g05(.a(new_n17_), .b(x2), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(x1), .c(x0), .O(new_n22_));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n17_), .c(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n23_), .b(x1), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n24_), .b(x1), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n27_), .c(new_n22_), .O(z1));
  inv1   g14(.a(x1), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x2), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand4  g20(.a(new_n35_), .b(new_n17_), .c(new_n30_), .d(new_n16_), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x2), .c(new_n17_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x1), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n36_), .c(new_n22_), .O(z2));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n24_), .c(x1), .d(new_n16_), .O(new_n42_));
  nand2  g27(.a(x5), .b(new_n16_), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n17_), .c(x2), .d(new_n30_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z3));
  oai21  g30(.a(new_n24_), .b(x0), .c(x1), .O(new_n46_));
  nand2  g31(.a(x3), .b(new_n24_), .O(new_n47_));
  nand3  g32(.a(new_n17_), .b(x2), .c(new_n30_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(x0), .O(new_n50_));
  aoi21  g35(.a(new_n17_), .b(x1), .c(x0), .O(new_n51_));
  nor3   g36(.a(x5), .b(x3), .c(x1), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n51_), .c(x2), .O(new_n53_));
  nand3  g38(.a(new_n17_), .b(new_n24_), .c(new_n16_), .O(new_n54_));
  nand4  g39(.a(new_n54_), .b(new_n53_), .c(new_n50_), .d(new_n46_), .O(z4));
  nand3  g40(.a(x2), .b(new_n30_), .c(x0), .O(new_n56_));
  nand3  g41(.a(x6), .b(new_n24_), .c(new_n16_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n17_), .O(new_n59_));
  oai21  g44(.a(new_n21_), .b(x1), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n17_), .b(x0), .c(x2), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(x1), .O(new_n62_));
  oai21  g47(.a(new_n31_), .b(x1), .c(new_n17_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(x2), .c(new_n16_), .O(new_n64_));
  nand4  g49(.a(new_n64_), .b(new_n62_), .c(new_n60_), .d(new_n59_), .O(z5));
  nor2   g50(.a(x5), .b(x3), .O(new_n66_));
  xor2a  g51(.a(x3), .b(x2), .O(new_n67_));
  aoi22  g52(.a(new_n67_), .b(x0), .c(new_n66_), .d(x2), .O(new_n68_));
  oai21  g53(.a(x4), .b(x2), .c(new_n17_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(x1), .c(new_n16_), .O(new_n70_));
  oai21  g55(.a(new_n68_), .b(x1), .c(new_n70_), .O(z6));
endmodule


