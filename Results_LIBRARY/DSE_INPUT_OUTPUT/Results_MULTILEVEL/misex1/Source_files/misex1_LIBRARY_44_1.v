// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x6), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x2), .c(x0), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  nor2   g08(.a(new_n18_), .b(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x1), .c(new_n22_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n21_), .O(z0));
  oai21  g11(.a(x5), .b(x3), .c(x2), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x0), .O(new_n28_));
  nand3  g13(.a(x3), .b(new_n23_), .c(x0), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n17_), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n28_), .c(new_n16_), .O(new_n31_));
  nand3  g16(.a(new_n23_), .b(x1), .c(new_n22_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n23_), .c(x3), .O(new_n34_));
  nand2  g19(.a(x5), .b(x2), .O(new_n35_));
  nand4  g20(.a(new_n35_), .b(new_n17_), .c(new_n18_), .d(new_n16_), .O(new_n36_));
  oai21  g21(.a(new_n34_), .b(new_n16_), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n22_), .O(new_n38_));
  nor2   g23(.a(x1), .b(new_n22_), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(new_n17_), .c(x3), .d(new_n23_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n23_), .c(x1), .d(new_n22_), .O(new_n43_));
  aoi21  g28(.a(x5), .b(new_n22_), .c(x3), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(x2), .c(x6), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(x1), .c(new_n43_), .O(z3));
  nor2   g31(.a(new_n18_), .b(x0), .O(new_n47_));
  nor2   g32(.a(x3), .b(x1), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n47_), .c(x2), .O(new_n49_));
  aoi21  g34(.a(x3), .b(new_n16_), .c(x0), .O(new_n50_));
  nand3  g35(.a(x3), .b(new_n16_), .c(x0), .O(new_n51_));
  inv1   g36(.a(new_n51_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n50_), .c(new_n23_), .O(new_n53_));
  nand2  g38(.a(x6), .b(new_n16_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n53_), .c(new_n49_), .O(z4));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n35_), .b(x0), .c(new_n17_), .O(new_n57_));
  aoi21  g42(.a(new_n56_), .b(x0), .c(new_n57_), .O(new_n58_));
  aoi21  g43(.a(new_n18_), .b(x2), .c(new_n16_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n24_), .c(new_n22_), .O(new_n60_));
  oai21  g45(.a(new_n58_), .b(x1), .c(new_n60_), .O(z5));
  nand2  g46(.a(new_n56_), .b(x0), .O(new_n62_));
  nor2   g47(.a(x5), .b(x3), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(x2), .c(x6), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n16_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n18_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(x1), .c(new_n22_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z6));
endmodule


