// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_;
  inv1   g00(.a(x3), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x2), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x1), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nand2  g05(.a(new_n16_), .b(x2), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(new_n20_), .c(new_n18_), .O(z0));
  inv1   g07(.a(x0), .O(new_n23_));
  oai21  g08(.a(x5), .b(x3), .c(x2), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n16_), .c(new_n25_), .O(new_n26_));
  oai21  g11(.a(new_n24_), .b(x1), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  aoi21  g13(.a(x3), .b(x0), .c(x1), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x2), .c(new_n28_), .O(z1));
  inv1   g15(.a(x5), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x2), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n25_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n32_), .c(x3), .O(new_n35_));
  nand3  g20(.a(x3), .b(x2), .c(x1), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  aoi21  g22(.a(new_n35_), .b(new_n19_), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(x3), .b(new_n25_), .O(new_n39_));
  or2    g24(.a(new_n39_), .b(new_n20_), .O(new_n40_));
  oai21  g25(.a(new_n38_), .b(x0), .c(new_n40_), .O(z2));
  nand2  g26(.a(x5), .b(new_n23_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n16_), .c(x2), .d(new_n19_), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(z3));
  nor2   g29(.a(new_n16_), .b(new_n25_), .O(new_n45_));
  nor2   g30(.a(x3), .b(x1), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n45_), .c(new_n23_), .O(new_n47_));
  oai21  g32(.a(new_n39_), .b(new_n23_), .c(new_n21_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n19_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z4));
  oai21  g35(.a(new_n21_), .b(x1), .c(new_n39_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x0), .O(new_n52_));
  aoi21  g37(.a(x5), .b(new_n19_), .c(x3), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n25_), .c(new_n26_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n52_), .c(new_n18_), .O(z5));
  nand4  g41(.a(new_n31_), .b(new_n16_), .c(x2), .d(new_n19_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n52_), .c(new_n18_), .O(z6));
endmodule


