// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x3), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x2), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x0), .c(x5), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x1), .b(new_n20_), .O(new_n21_));
  nand2  g06(.a(x3), .b(x2), .O(new_n22_));
  oai22  g07(.a(new_n22_), .b(new_n21_), .c(new_n19_), .d(x1), .O(z0));
  xor2a  g08(.a(x2), .b(x0), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(x3), .c(x5), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  inv1   g11(.a(x1), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n26_), .c(new_n20_), .O(new_n31_));
  oai21  g16(.a(new_n25_), .b(x1), .c(new_n31_), .O(z1));
  inv1   g17(.a(x4), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x2), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x1), .O(new_n35_));
  nand2  g20(.a(x6), .b(new_n26_), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n28_), .c(new_n16_), .d(new_n27_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n20_), .O(new_n39_));
  nor2   g24(.a(x1), .b(new_n20_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n28_), .c(x3), .d(new_n26_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n26_), .c(x1), .d(new_n20_), .O(new_n44_));
  nand2  g29(.a(new_n17_), .b(new_n28_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n27_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(z3));
  nor2   g32(.a(new_n16_), .b(x0), .O(new_n48_));
  nor2   g33(.a(x3), .b(x1), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n48_), .c(x2), .O(new_n50_));
  aoi21  g35(.a(x3), .b(new_n27_), .c(x0), .O(new_n51_));
  nand3  g36(.a(x3), .b(new_n27_), .c(x0), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n26_), .O(new_n54_));
  nand2  g39(.a(x5), .b(new_n27_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n50_), .O(z4));
  nand2  g41(.a(x3), .b(new_n26_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n17_), .c(new_n20_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(x5), .c(new_n27_), .O(new_n59_));
  nand2  g44(.a(new_n17_), .b(x1), .O(new_n60_));
  nand3  g45(.a(x6), .b(new_n16_), .c(new_n26_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n60_), .c(new_n22_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n20_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n59_), .O(z5));
  oai21  g49(.a(x4), .b(x2), .c(new_n16_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(x1), .c(new_n20_), .O(new_n66_));
  nand3  g51(.a(x3), .b(new_n26_), .c(x0), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n45_), .c(new_n27_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z6));
endmodule


