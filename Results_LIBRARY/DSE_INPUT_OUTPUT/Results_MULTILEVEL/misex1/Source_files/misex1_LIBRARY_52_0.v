// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:08 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand4  g01(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(z0));
  inv1   g03(.a(x2), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x1), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  nor2   g06(.a(x2), .b(new_n21_), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n20_), .c(new_n16_), .O(new_n23_));
  oai21  g08(.a(x2), .b(new_n16_), .c(x3), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n23_), .O(z1));
  inv1   g11(.a(x3), .O(new_n27_));
  inv1   g12(.a(x4), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x2), .c(new_n27_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(x1), .c(new_n16_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n25_), .O(z2));
  oai22  g16(.a(x7), .b(new_n21_), .c(x4), .d(x3), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n19_), .c(new_n16_), .O(new_n33_));
  oai21  g18(.a(x3), .b(x1), .c(new_n33_), .O(z3));
  oai21  g19(.a(x4), .b(x3), .c(new_n21_), .O(new_n35_));
  nor2   g20(.a(new_n27_), .b(new_n19_), .O(new_n36_));
  aoi21  g21(.a(new_n35_), .b(new_n19_), .c(new_n36_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x0), .c(new_n25_), .O(z4));
  aoi21  g23(.a(new_n27_), .b(x2), .c(new_n21_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n20_), .c(new_n16_), .O(new_n40_));
  aoi21  g25(.a(new_n19_), .b(x0), .c(new_n27_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x1), .c(new_n40_), .O(z5));
  oai21  g27(.a(x4), .b(x2), .c(new_n27_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x1), .c(new_n16_), .O(new_n44_));
  nand4  g29(.a(x3), .b(new_n19_), .c(new_n21_), .d(x0), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z6));
endmodule


