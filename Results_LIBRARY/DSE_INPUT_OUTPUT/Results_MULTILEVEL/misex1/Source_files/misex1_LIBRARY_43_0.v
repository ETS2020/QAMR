// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(x7), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n16_), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n19_), .O(new_n26_));
  aoi22  g11(.a(new_n26_), .b(new_n16_), .c(new_n24_), .d(new_n19_), .O(new_n27_));
  nor2   g12(.a(new_n20_), .b(x2), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n19_), .c(x0), .O(new_n29_));
  nor2   g14(.a(x7), .b(x3), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  and2   g16(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  oai21  g17(.a(new_n27_), .b(x0), .c(new_n32_), .O(z1));
  nand2  g18(.a(x7), .b(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n20_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x1), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n25_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(x7), .c(new_n20_), .d(new_n19_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n29_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n46_));
  inv1   g31(.a(x7), .O(new_n47_));
  aoi21  g32(.a(x5), .b(new_n17_), .c(new_n16_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n19_), .c(new_n47_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(x3), .c(new_n46_), .O(z3));
  nor2   g35(.a(x3), .b(new_n16_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n28_), .c(x0), .O(new_n52_));
  oai21  g37(.a(new_n23_), .b(new_n17_), .c(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n19_), .O(new_n55_));
  aoi21  g40(.a(x3), .b(new_n19_), .c(x2), .O(new_n56_));
  nor2   g41(.a(new_n20_), .b(new_n16_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n56_), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n55_), .c(new_n31_), .O(z4));
  nand2  g44(.a(x3), .b(new_n16_), .O(new_n60_));
  nand3  g45(.a(x7), .b(new_n20_), .c(x2), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n60_), .c(new_n17_), .O(new_n62_));
  nand3  g47(.a(new_n51_), .b(x7), .c(x5), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n19_), .O(new_n65_));
  nand2  g50(.a(x7), .b(new_n16_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n20_), .c(new_n19_), .O(new_n67_));
  nand4  g52(.a(x7), .b(x6), .c(new_n20_), .d(new_n16_), .O(new_n68_));
  oai21  g53(.a(new_n20_), .b(new_n16_), .c(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n67_), .c(new_n17_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n65_), .O(z5));
  nand3  g56(.a(new_n51_), .b(x7), .c(new_n37_), .O(new_n72_));
  inv1   g57(.a(new_n72_), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n62_), .c(new_n19_), .O(new_n74_));
  inv1   g59(.a(x4), .O(new_n75_));
  nand3  g60(.a(x7), .b(new_n75_), .c(new_n16_), .O(new_n76_));
  nand2  g61(.a(new_n76_), .b(new_n20_), .O(new_n77_));
  nand3  g62(.a(new_n77_), .b(x1), .c(new_n17_), .O(new_n78_));
  nand2  g63(.a(new_n78_), .b(new_n74_), .O(z6));
endmodule


