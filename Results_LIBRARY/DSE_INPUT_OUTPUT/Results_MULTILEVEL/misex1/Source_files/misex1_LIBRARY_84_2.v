// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x6), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(x0), .c(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  oai21  g08(.a(x5), .b(x3), .c(x2), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n16_), .c(x0), .O(new_n25_));
  oai21  g10(.a(new_n24_), .b(x0), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n23_), .O(new_n27_));
  inv1   g12(.a(x3), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n28_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(new_n17_), .O(new_n31_));
  nand2  g16(.a(new_n19_), .b(x2), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n27_), .O(z1));
  inv1   g18(.a(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n28_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x1), .O(new_n36_));
  oai21  g21(.a(x5), .b(new_n16_), .c(x6), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n28_), .c(new_n23_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand2  g25(.a(x3), .b(new_n16_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  nor2   g27(.a(x1), .b(new_n17_), .O(new_n43_));
  aoi22  g28(.a(new_n43_), .b(new_n42_), .c(new_n19_), .d(x2), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n40_), .O(z2));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n47_));
  aoi21  g32(.a(x5), .b(new_n17_), .c(new_n19_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n28_), .c(x2), .d(new_n23_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  oai21  g35(.a(new_n29_), .b(new_n16_), .c(new_n25_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n23_), .O(new_n52_));
  aoi21  g37(.a(x3), .b(new_n23_), .c(x2), .O(new_n53_));
  nand3  g38(.a(x6), .b(x3), .c(x2), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n17_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n52_), .O(z4));
  nand2  g42(.a(new_n28_), .b(x2), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n41_), .c(new_n17_), .O(new_n59_));
  inv1   g44(.a(x5), .O(new_n60_));
  nor3   g45(.a(new_n60_), .b(new_n16_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n23_), .O(new_n62_));
  nand2  g47(.a(new_n58_), .b(x1), .O(new_n63_));
  nand2  g48(.a(x3), .b(x2), .O(new_n64_));
  nand3  g49(.a(x6), .b(new_n28_), .c(new_n16_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n17_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n62_), .c(new_n32_), .O(z5));
  nand3  g53(.a(new_n60_), .b(new_n28_), .c(x2), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n59_), .c(new_n23_), .O(new_n71_));
  oai21  g56(.a(x4), .b(x2), .c(new_n28_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(x1), .c(new_n17_), .O(new_n73_));
  nand3  g58(.a(new_n73_), .b(new_n71_), .c(new_n32_), .O(z6));
endmodule


