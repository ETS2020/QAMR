// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(x1), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(new_n16_), .c(x0), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nand4  g04(.a(x3), .b(x2), .c(x1), .d(new_n19_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g06(.a(x1), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nand4  g10(.a(new_n25_), .b(x2), .c(new_n22_), .d(new_n19_), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n23_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n22_), .c(new_n19_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n26_), .O(z1));
  nand2  g15(.a(x3), .b(x1), .O(new_n31_));
  nand4  g16(.a(new_n24_), .b(new_n23_), .c(x2), .d(new_n22_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  oai21  g19(.a(x4), .b(x3), .c(x1), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n23_), .c(new_n22_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n35_), .c(new_n19_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n34_), .O(z2));
  oai21  g25(.a(x3), .b(x1), .c(x2), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x0), .O(new_n42_));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n16_), .c(x1), .d(new_n19_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n42_), .c(new_n32_), .O(z3));
  nor2   g30(.a(new_n23_), .b(x0), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n17_), .c(x2), .O(new_n47_));
  nand2  g32(.a(x3), .b(new_n22_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n16_), .c(new_n19_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z4));
  oai21  g35(.a(new_n46_), .b(new_n16_), .c(x1), .O(new_n51_));
  oai21  g36(.a(new_n24_), .b(x1), .c(new_n23_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(x2), .c(new_n19_), .O(new_n53_));
  nand3  g38(.a(x6), .b(new_n23_), .c(new_n16_), .O(new_n54_));
  nand4  g39(.a(new_n54_), .b(new_n53_), .c(new_n51_), .d(new_n18_), .O(z5));
  aoi21  g40(.a(x4), .b(new_n23_), .c(x2), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n46_), .c(x1), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n32_), .c(new_n18_), .O(z6));
endmodule


