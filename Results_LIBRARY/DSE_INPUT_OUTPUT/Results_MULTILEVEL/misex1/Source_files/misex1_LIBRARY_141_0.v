// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:29 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  nor2   g04(.a(new_n17_), .b(x0), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x4), .c(x3), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x0), .O(new_n23_));
  oai21  g08(.a(x5), .b(x3), .c(x2), .O(new_n24_));
  aoi21  g09(.a(x6), .b(new_n18_), .c(x1), .O(new_n25_));
  oai22  g10(.a(new_n25_), .b(x2), .c(new_n24_), .d(x1), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(x4), .c(new_n23_), .O(new_n27_));
  nand4  g12(.a(x3), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(z1));
  aoi21  g14(.a(new_n18_), .b(x2), .c(new_n17_), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(x2), .c(new_n17_), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n32_), .c(x3), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n30_), .c(x4), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(x0), .c(new_n28_), .O(z2));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(x5), .b(new_n38_), .c(new_n23_), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n40_));
  nor2   g25(.a(x7), .b(new_n38_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n20_), .c(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n40_), .O(z3));
  nand2  g28(.a(x3), .b(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n18_), .b(x2), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(new_n23_), .O(new_n46_));
  nand3  g31(.a(new_n31_), .b(x4), .c(new_n18_), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(x0), .c(new_n16_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n46_), .c(new_n17_), .O(new_n49_));
  aoi21  g34(.a(x3), .b(new_n17_), .c(x2), .O(new_n50_));
  nand2  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x4), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n50_), .c(new_n23_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n49_), .O(z4));
  nand4  g39(.a(x5), .b(x4), .c(x2), .d(new_n23_), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n46_), .c(new_n17_), .O(new_n57_));
  inv1   g42(.a(new_n30_), .O(new_n58_));
  nand3  g43(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n51_), .c(new_n58_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(x4), .c(new_n23_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n57_), .O(z5));
  nand4  g47(.a(new_n31_), .b(x4), .c(new_n18_), .d(x2), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n46_), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n21_), .O(z6));
endmodule


