// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand4  g02(.a(new_n17_), .b(x2), .c(new_n16_), .d(x0), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(z0));
  inv1   g04(.a(x2), .O(new_n20_));
  inv1   g05(.a(x5), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n17_), .c(new_n20_), .O(new_n22_));
  inv1   g07(.a(x6), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x3), .c(new_n16_), .O(new_n24_));
  aoi22  g09(.a(new_n24_), .b(new_n20_), .c(new_n22_), .d(new_n16_), .O(new_n25_));
  nand4  g10(.a(x3), .b(new_n20_), .c(new_n16_), .d(x0), .O(new_n26_));
  oai21  g11(.a(new_n25_), .b(x0), .c(new_n26_), .O(z1));
  oai21  g12(.a(x4), .b(x3), .c(x1), .O(new_n28_));
  nand3  g13(.a(new_n23_), .b(new_n17_), .c(new_n16_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n28_), .c(x0), .O(new_n30_));
  nand3  g15(.a(x3), .b(new_n16_), .c(x0), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n30_), .c(new_n20_), .O(new_n33_));
  nor3   g18(.a(x5), .b(x3), .c(x0), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x1), .c(x2), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(z2));
  inv1   g21(.a(x0), .O(new_n37_));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand4  g23(.a(new_n38_), .b(new_n20_), .c(x1), .d(new_n37_), .O(new_n39_));
  nand2  g24(.a(x5), .b(new_n37_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n17_), .c(x2), .d(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z3));
  oai21  g27(.a(new_n17_), .b(new_n37_), .c(x2), .O(new_n43_));
  nand3  g28(.a(x3), .b(new_n20_), .c(x0), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n16_), .O(new_n46_));
  nand2  g31(.a(x3), .b(new_n16_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n20_), .c(new_n37_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(z4));
  xor2a  g34(.a(x3), .b(x2), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n16_), .c(x0), .O(new_n51_));
  oai21  g36(.a(x2), .b(new_n37_), .c(x1), .O(new_n52_));
  nand3  g37(.a(x6), .b(new_n17_), .c(new_n20_), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n22_), .c(new_n37_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n52_), .c(new_n51_), .O(z5));
  nor2   g41(.a(x5), .b(x3), .O(new_n57_));
  aoi22  g42(.a(new_n50_), .b(x0), .c(new_n57_), .d(x2), .O(new_n58_));
  nand2  g43(.a(x4), .b(new_n17_), .O(new_n59_));
  nand4  g44(.a(new_n59_), .b(new_n20_), .c(x1), .d(new_n37_), .O(new_n60_));
  oai21  g45(.a(new_n58_), .b(x1), .c(new_n60_), .O(z6));
endmodule


