// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand4  g01(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(z0));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x2), .O(new_n20_));
  nand3  g05(.a(x3), .b(new_n20_), .c(x0), .O(new_n21_));
  oai21  g06(.a(new_n20_), .b(x0), .c(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  oai22  g09(.a(new_n24_), .b(x3), .c(x2), .d(new_n19_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n16_), .O(new_n26_));
  inv1   g11(.a(x3), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x2), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n26_), .c(new_n23_), .O(z1));
  aoi21  g14(.a(x4), .b(new_n20_), .c(x3), .O(new_n30_));
  nand4  g15(.a(new_n24_), .b(new_n27_), .c(new_n20_), .d(new_n19_), .O(new_n31_));
  oai21  g16(.a(new_n30_), .b(new_n19_), .c(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n16_), .O(new_n33_));
  nand4  g18(.a(x3), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(z2));
  oai21  g20(.a(x4), .b(x3), .c(x7), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n20_), .c(x1), .d(new_n16_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(z3));
  oai21  g23(.a(new_n27_), .b(new_n16_), .c(x2), .O(new_n39_));
  oai21  g24(.a(new_n27_), .b(x1), .c(new_n16_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n39_), .c(new_n34_), .O(z4));
  nand2  g26(.a(new_n28_), .b(x1), .O(new_n42_));
  nand2  g27(.a(x3), .b(x2), .O(new_n43_));
  nand3  g28(.a(x6), .b(new_n27_), .c(new_n20_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n34_), .O(z5));
  oai21  g32(.a(x4), .b(x2), .c(new_n27_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x1), .c(new_n16_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n34_), .O(z6));
endmodule


