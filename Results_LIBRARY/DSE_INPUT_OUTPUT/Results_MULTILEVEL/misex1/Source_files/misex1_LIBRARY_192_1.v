// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(x5), .b(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  xor2a  g07(.a(x2), .b(x0), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(x3), .c(new_n19_), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n19_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n16_), .c(new_n17_), .O(new_n27_));
  inv1   g12(.a(x3), .O(new_n28_));
  nand2  g13(.a(x5), .b(new_n28_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n24_), .O(z1));
  inv1   g15(.a(x5), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(x4), .c(new_n16_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n28_), .c(new_n19_), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n16_), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n31_), .c(new_n28_), .d(new_n19_), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n33_), .c(new_n17_), .O(new_n37_));
  nand4  g22(.a(x3), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n37_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n41_));
  nand3  g26(.a(new_n28_), .b(x2), .c(new_n19_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n31_), .O(new_n44_));
  inv1   g29(.a(x7), .O(new_n45_));
  nor2   g30(.a(new_n19_), .b(x0), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n45_), .c(x3), .d(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n44_), .O(z3));
  nand3  g33(.a(new_n45_), .b(new_n31_), .c(new_n16_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n28_), .c(new_n19_), .O(new_n50_));
  nand2  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n20_), .b(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n50_), .c(new_n17_), .O(new_n54_));
  nand2  g39(.a(x3), .b(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n20_), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n55_), .b(new_n17_), .c(new_n56_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n19_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n54_), .O(z4));
  nand2  g44(.a(new_n56_), .b(new_n55_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n19_), .c(x0), .O(new_n61_));
  nor2   g46(.a(x5), .b(x2), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(x3), .c(x1), .O(new_n63_));
  nand4  g48(.a(x6), .b(new_n31_), .c(new_n28_), .d(new_n16_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n63_), .c(new_n51_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n61_), .O(z5));
  inv1   g52(.a(x4), .O(new_n68_));
  nand3  g53(.a(new_n31_), .b(new_n68_), .c(new_n16_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n28_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x1), .c(new_n17_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n58_), .O(z6));
endmodule


