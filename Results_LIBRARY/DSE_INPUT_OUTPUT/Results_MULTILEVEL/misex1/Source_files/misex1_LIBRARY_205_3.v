// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x4), .O(new_n16_));
  nand2  g01(.a(x3), .b(x2), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(new_n16_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x0), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n21_), .c(new_n19_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n22_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand3  g13(.a(new_n16_), .b(new_n26_), .c(x1), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  aoi21  g15(.a(new_n28_), .b(new_n20_), .c(new_n30_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n26_), .O(new_n32_));
  oai22  g17(.a(new_n32_), .b(new_n21_), .c(new_n31_), .d(x0), .O(z1));
  inv1   g18(.a(x0), .O(new_n34_));
  inv1   g19(.a(x5), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x2), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n26_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n36_), .c(x3), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n26_), .c(x0), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  aoi21  g26(.a(new_n39_), .b(new_n34_), .c(new_n41_), .O(new_n42_));
  nand4  g27(.a(new_n16_), .b(x3), .c(x1), .d(new_n34_), .O(new_n43_));
  oai21  g28(.a(new_n42_), .b(x1), .c(new_n43_), .O(z2));
  aoi21  g29(.a(x7), .b(x3), .c(x4), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n26_), .c(x1), .d(new_n34_), .O(new_n46_));
  nand2  g31(.a(x5), .b(new_n34_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n22_), .c(x2), .d(new_n20_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(z3));
  nand3  g34(.a(x3), .b(new_n20_), .c(x0), .O(new_n50_));
  nand3  g35(.a(new_n16_), .b(x1), .c(new_n34_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n26_), .O(new_n53_));
  inv1   g38(.a(new_n23_), .O(new_n54_));
  aoi21  g39(.a(x3), .b(new_n26_), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n20_), .O(new_n56_));
  nand4  g41(.a(new_n16_), .b(x3), .c(x2), .d(new_n34_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(z4));
  aoi21  g43(.a(new_n32_), .b(new_n23_), .c(new_n34_), .O(new_n59_));
  aoi21  g44(.a(new_n27_), .b(new_n25_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n59_), .c(new_n20_), .O(new_n61_));
  nand4  g46(.a(new_n23_), .b(new_n16_), .c(x1), .d(new_n34_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(z5));
  nand3  g48(.a(new_n35_), .b(new_n22_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n59_), .c(new_n20_), .O(new_n66_));
  aoi21  g51(.a(new_n23_), .b(new_n34_), .c(x4), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n20_), .c(new_n66_), .O(z6));
endmodule


