// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(x6), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand2  g06(.a(x1), .b(new_n17_), .O(new_n22_));
  nand2  g07(.a(x3), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand2  g11(.a(new_n18_), .b(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n25_), .b(x1), .c(new_n27_), .O(new_n28_));
  nor2   g13(.a(x2), .b(new_n16_), .O(new_n29_));
  aoi21  g14(.a(new_n28_), .b(x6), .c(new_n29_), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  nor2   g16(.a(new_n31_), .b(new_n18_), .O(new_n32_));
  nand4  g17(.a(new_n32_), .b(new_n26_), .c(new_n16_), .d(x0), .O(new_n33_));
  oai21  g18(.a(new_n30_), .b(x0), .c(new_n33_), .O(z1));
  nor2   g19(.a(x5), .b(x3), .O(new_n35_));
  inv1   g20(.a(x4), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(x2), .c(new_n18_), .O(new_n37_));
  nor2   g22(.a(new_n26_), .b(x1), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n35_), .c(new_n37_), .d(x1), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n26_), .c(x0), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x6), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n16_), .O(new_n42_));
  oai21  g27(.a(new_n39_), .b(x0), .c(new_n42_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n26_), .c(x1), .d(new_n17_), .O(new_n45_));
  aoi21  g30(.a(x5), .b(new_n17_), .c(new_n31_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n18_), .c(x2), .d(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z3));
  aoi21  g33(.a(new_n18_), .b(x2), .c(new_n16_), .O(new_n49_));
  nand2  g34(.a(new_n27_), .b(new_n23_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(x6), .c(new_n49_), .O(new_n51_));
  nand2  g36(.a(new_n40_), .b(new_n19_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(x6), .c(new_n16_), .O(new_n53_));
  oai21  g38(.a(new_n51_), .b(x0), .c(new_n53_), .O(z4));
  xor2a  g39(.a(x3), .b(x2), .O(new_n55_));
  nand2  g40(.a(x5), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(x0), .c(x6), .O(new_n57_));
  aoi21  g42(.a(new_n55_), .b(x0), .c(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n50_), .b(new_n49_), .c(new_n17_), .O(new_n59_));
  oai21  g44(.a(new_n58_), .b(x1), .c(new_n59_), .O(z5));
  nand2  g45(.a(new_n55_), .b(x0), .O(new_n61_));
  aoi21  g46(.a(new_n35_), .b(x2), .c(new_n31_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n16_), .O(new_n64_));
  oai21  g49(.a(x4), .b(x2), .c(new_n18_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(x1), .c(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z6));
endmodule


