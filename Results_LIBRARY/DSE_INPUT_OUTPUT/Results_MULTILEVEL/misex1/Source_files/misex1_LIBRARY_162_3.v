// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x5), .O(new_n16_));
  nand2  g01(.a(x3), .b(x2), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(new_n16_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n20_), .c(x0), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(z0));
  nor2   g10(.a(x2), .b(x0), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x5), .c(x1), .O(new_n27_));
  xor2a  g12(.a(x2), .b(x0), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(x3), .c(new_n20_), .O(new_n29_));
  inv1   g14(.a(x0), .O(new_n30_));
  inv1   g15(.a(x2), .O(new_n31_));
  nand3  g16(.a(x6), .b(new_n21_), .c(new_n31_), .O(new_n32_));
  oai21  g17(.a(new_n16_), .b(new_n31_), .c(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n29_), .c(new_n27_), .O(z1));
  aoi21  g20(.a(x4), .b(new_n31_), .c(x3), .O(new_n36_));
  nand3  g21(.a(new_n21_), .b(x2), .c(new_n20_), .O(new_n37_));
  oai21  g22(.a(new_n36_), .b(new_n20_), .c(new_n37_), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(new_n21_), .c(new_n31_), .d(new_n20_), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  aoi21  g26(.a(new_n38_), .b(new_n16_), .c(new_n41_), .O(new_n42_));
  nand4  g27(.a(x3), .b(new_n31_), .c(new_n20_), .d(x0), .O(new_n43_));
  oai21  g28(.a(new_n42_), .b(x0), .c(new_n43_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n31_), .c(new_n30_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x1), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n30_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n21_), .c(x2), .d(new_n20_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z3));
  nand2  g36(.a(x3), .b(new_n31_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n22_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(x0), .O(new_n54_));
  nand2  g39(.a(new_n52_), .b(new_n30_), .O(new_n55_));
  nand3  g40(.a(new_n16_), .b(new_n21_), .c(x2), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n20_), .O(new_n58_));
  oai21  g43(.a(new_n23_), .b(x0), .c(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(x1), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z4));
  nand3  g46(.a(new_n53_), .b(new_n20_), .c(x0), .O(new_n62_));
  nand2  g47(.a(new_n22_), .b(x1), .O(new_n63_));
  oai21  g48(.a(x5), .b(x3), .c(x2), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n63_), .c(new_n32_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  nand2  g51(.a(x5), .b(x1), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n66_), .c(new_n62_), .O(z5));
  nand2  g53(.a(new_n56_), .b(new_n54_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n20_), .O(new_n70_));
  nor2   g55(.a(x4), .b(x2), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(x3), .c(new_n30_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n16_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(x1), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n70_), .O(z6));
endmodule


