// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_;
  inv1   g00(.a(x0), .O(new_n16_));
  nor2   g01(.a(x2), .b(x1), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n17_), .c(new_n16_), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nor2   g07(.a(x3), .b(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n21_), .O(z0));
  nand2  g10(.a(x1), .b(new_n16_), .O(new_n26_));
  nor2   g11(.a(new_n19_), .b(x1), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x0), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n26_), .c(x2), .O(z1));
  nor2   g14(.a(new_n19_), .b(x2), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n16_), .c(new_n22_), .O(new_n31_));
  nand3  g16(.a(x4), .b(new_n18_), .c(x1), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n19_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(z2));
  oai21  g20(.a(new_n23_), .b(new_n16_), .c(new_n22_), .O(new_n36_));
  oai21  g21(.a(x4), .b(x3), .c(x7), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n18_), .c(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(z3));
  oai21  g24(.a(new_n27_), .b(new_n16_), .c(new_n18_), .O(new_n40_));
  oai21  g25(.a(x3), .b(new_n22_), .c(new_n16_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n24_), .O(z4));
  oai21  g27(.a(x3), .b(new_n18_), .c(new_n16_), .O(new_n43_));
  oai21  g28(.a(new_n30_), .b(new_n23_), .c(new_n22_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(z5));
  oai21  g30(.a(new_n17_), .b(new_n16_), .c(x3), .O(new_n46_));
  nand2  g31(.a(x4), .b(x1), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n18_), .c(new_n16_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n46_), .c(new_n24_), .O(z6));
endmodule


