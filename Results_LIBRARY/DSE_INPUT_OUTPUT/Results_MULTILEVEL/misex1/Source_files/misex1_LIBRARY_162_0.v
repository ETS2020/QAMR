// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(x5), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nor2   g06(.a(new_n16_), .b(x0), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x3), .c(x2), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  nand4  g10(.a(x5), .b(x3), .c(new_n16_), .d(x0), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n22_), .c(new_n25_), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n25_), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n17_), .c(new_n29_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x1), .c(new_n28_), .O(z1));
  nand2  g18(.a(x4), .b(new_n25_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n18_), .c(new_n16_), .O(new_n35_));
  nand2  g20(.a(new_n25_), .b(new_n16_), .O(new_n36_));
  nand3  g21(.a(new_n30_), .b(x5), .c(new_n18_), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n17_), .O(new_n39_));
  nor2   g24(.a(new_n29_), .b(new_n18_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n25_), .c(new_n16_), .d(x0), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n25_), .c(x1), .d(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n21_), .O(z3));
  nor2   g30(.a(new_n18_), .b(x0), .O(new_n46_));
  nor2   g31(.a(x3), .b(x1), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n46_), .c(x2), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n16_), .c(x0), .O(new_n49_));
  nand3  g34(.a(x3), .b(new_n16_), .c(x0), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n49_), .c(new_n25_), .O(new_n52_));
  nand2  g37(.a(new_n29_), .b(new_n16_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n48_), .O(z4));
  nand2  g39(.a(new_n31_), .b(new_n17_), .O(new_n55_));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(x0), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(x5), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n16_), .O(new_n59_));
  nand3  g44(.a(new_n19_), .b(x1), .c(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n59_), .O(z5));
  aoi21  g46(.a(new_n56_), .b(x0), .c(new_n29_), .O(new_n62_));
  oai21  g47(.a(x4), .b(x2), .c(new_n18_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(x1), .c(new_n17_), .O(new_n64_));
  oai21  g49(.a(new_n62_), .b(x1), .c(new_n64_), .O(z6));
endmodule


