// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n17_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x6), .c(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  aoi21  g08(.a(x3), .b(new_n23_), .c(x2), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  nand2  g10(.a(x3), .b(x2), .O(new_n26_));
  oai21  g11(.a(new_n25_), .b(x3), .c(new_n26_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n23_), .c(new_n24_), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n16_), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  nor2   g15(.a(x6), .b(x3), .O(new_n31_));
  aoi21  g16(.a(new_n30_), .b(new_n20_), .c(new_n31_), .O(new_n32_));
  oai21  g17(.a(new_n28_), .b(x0), .c(new_n32_), .O(z1));
  inv1   g18(.a(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n19_), .O(new_n35_));
  nor2   g20(.a(new_n16_), .b(x1), .O(new_n36_));
  nor2   g21(.a(x5), .b(x3), .O(new_n37_));
  aoi22  g22(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x1), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x0), .c(new_n32_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n41_));
  nand2  g26(.a(x5), .b(new_n17_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n19_), .c(x2), .d(new_n23_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(x6), .O(new_n45_));
  nor2   g30(.a(x7), .b(new_n19_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z3));
  inv1   g33(.a(new_n31_), .O(new_n49_));
  nand2  g34(.a(new_n19_), .b(x2), .O(new_n50_));
  oai21  g35(.a(new_n29_), .b(new_n17_), .c(new_n50_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n23_), .O(new_n52_));
  inv1   g37(.a(new_n26_), .O(new_n53_));
  aoi21  g38(.a(x3), .b(new_n23_), .c(x2), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n53_), .c(new_n17_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n52_), .c(new_n49_), .O(z4));
  aoi21  g41(.a(new_n50_), .b(new_n29_), .c(new_n17_), .O(new_n57_));
  nand3  g42(.a(x5), .b(new_n19_), .c(x2), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n57_), .c(new_n23_), .O(new_n60_));
  nor2   g45(.a(x4), .b(x2), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(x3), .c(x1), .O(new_n62_));
  nand2  g47(.a(new_n19_), .b(new_n16_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n62_), .c(new_n26_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n17_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n60_), .c(new_n49_), .O(z5));
  nand2  g51(.a(new_n37_), .b(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n57_), .c(new_n23_), .O(new_n69_));
  inv1   g54(.a(new_n62_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n17_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n69_), .c(new_n49_), .O(z6));
endmodule


