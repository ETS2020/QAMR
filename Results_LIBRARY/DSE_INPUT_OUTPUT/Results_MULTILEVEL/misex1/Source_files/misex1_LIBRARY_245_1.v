// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x4), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n18_), .c(new_n16_), .O(z0));
  xnor2a g08(.a(x2), .b(x0), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n21_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(x2), .c(new_n17_), .O(new_n27_));
  oai21  g12(.a(new_n24_), .b(new_n20_), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n19_), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n20_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n19_), .c(x2), .O(new_n31_));
  nand2  g16(.a(x4), .b(new_n20_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  aoi21  g18(.a(new_n31_), .b(new_n17_), .c(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n29_), .O(z1));
  nand2  g20(.a(new_n25_), .b(x2), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n16_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n36_), .c(x3), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n16_), .c(x0), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  aoi21  g26(.a(new_n39_), .b(new_n17_), .c(new_n41_), .O(new_n42_));
  and2   g27(.a(new_n32_), .b(new_n18_), .O(new_n43_));
  oai21  g28(.a(new_n42_), .b(x1), .c(new_n43_), .O(z2));
  nand2  g29(.a(new_n21_), .b(new_n20_), .O(new_n45_));
  oai21  g30(.a(x7), .b(new_n20_), .c(new_n45_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n47_));
  aoi21  g32(.a(x5), .b(new_n17_), .c(x4), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  nand2  g35(.a(new_n20_), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n40_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n19_), .O(new_n53_));
  aoi21  g38(.a(x3), .b(new_n19_), .c(x2), .O(new_n54_));
  nand2  g39(.a(x3), .b(x2), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n54_), .c(new_n17_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n53_), .c(new_n32_), .O(z4));
  nand2  g43(.a(x3), .b(new_n16_), .O(new_n59_));
  aoi21  g44(.a(new_n51_), .b(new_n59_), .c(new_n17_), .O(new_n60_));
  nand3  g45(.a(x5), .b(new_n20_), .c(x2), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n19_), .O(new_n63_));
  aoi21  g48(.a(new_n20_), .b(x2), .c(new_n19_), .O(new_n64_));
  oai21  g49(.a(new_n30_), .b(x2), .c(new_n55_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n64_), .c(new_n17_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n63_), .c(new_n32_), .O(z5));
  nor2   g52(.a(x5), .b(x3), .O(new_n68_));
  aoi21  g53(.a(new_n68_), .b(x2), .c(new_n60_), .O(new_n69_));
  aoi21  g54(.a(new_n64_), .b(new_n17_), .c(new_n33_), .O(new_n70_));
  oai21  g55(.a(new_n69_), .b(x1), .c(new_n70_), .O(z6));
endmodule


