// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x4), .O(new_n20_));
  nor2   g05(.a(new_n17_), .b(x0), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x3), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n18_), .c(new_n16_), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n17_), .O(new_n27_));
  aoi22  g12(.a(new_n27_), .b(new_n16_), .c(new_n25_), .d(new_n17_), .O(new_n28_));
  nand3  g13(.a(new_n16_), .b(new_n17_), .c(x0), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n20_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x3), .O(new_n31_));
  oai21  g16(.a(new_n28_), .b(x0), .c(new_n31_), .O(z1));
  nand2  g17(.a(x4), .b(new_n16_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n18_), .c(new_n17_), .O(new_n34_));
  nand2  g19(.a(new_n24_), .b(x2), .O(new_n35_));
  nand2  g20(.a(new_n26_), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n35_), .c(x3), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n17_), .c(new_n34_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x0), .c(new_n31_), .O(z2));
  inv1   g24(.a(x0), .O(new_n40_));
  nand2  g25(.a(x7), .b(x4), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n16_), .c(x1), .d(new_n40_), .O(new_n42_));
  nand2  g27(.a(x5), .b(new_n40_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x2), .c(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nor3   g31(.a(x7), .b(x4), .c(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n21_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(z3));
  nand2  g34(.a(x3), .b(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n18_), .b(x2), .O(new_n51_));
  oai21  g36(.a(new_n50_), .b(new_n40_), .c(new_n51_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n17_), .O(new_n53_));
  oai21  g38(.a(new_n16_), .b(x0), .c(new_n20_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(x3), .O(new_n55_));
  nand3  g40(.a(new_n20_), .b(x3), .c(new_n17_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n16_), .c(new_n40_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(new_n53_), .O(z4));
  aoi21  g43(.a(new_n51_), .b(new_n50_), .c(new_n40_), .O(new_n59_));
  nand3  g44(.a(x5), .b(new_n18_), .c(x2), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n51_), .b(x1), .O(new_n63_));
  nand2  g48(.a(x3), .b(x2), .O(new_n64_));
  nand3  g49(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n40_), .O(new_n67_));
  nand2  g52(.a(x4), .b(x3), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n67_), .c(new_n62_), .O(z5));
  nand3  g54(.a(new_n24_), .b(new_n18_), .c(x2), .O(new_n70_));
  inv1   g55(.a(new_n70_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n59_), .c(new_n17_), .O(new_n72_));
  oai21  g57(.a(x4), .b(x2), .c(new_n18_), .O(new_n73_));
  nand3  g58(.a(new_n73_), .b(x1), .c(new_n40_), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(new_n72_), .c(new_n68_), .O(z6));
endmodule


