// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x5), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x2), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n16_), .O(new_n22_));
  nand2  g07(.a(x1), .b(new_n17_), .O(new_n23_));
  nand2  g08(.a(x3), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(z0));
  nand3  g10(.a(x3), .b(x2), .c(new_n16_), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand3  g12(.a(x6), .b(new_n19_), .c(new_n27_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n26_), .c(x0), .O(new_n29_));
  nand4  g14(.a(x3), .b(new_n27_), .c(new_n16_), .d(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n29_), .c(new_n18_), .O(new_n32_));
  nand3  g17(.a(new_n27_), .b(x1), .c(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n32_), .O(z1));
  nand2  g19(.a(x4), .b(new_n27_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n19_), .c(new_n16_), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x2), .c(new_n19_), .O(new_n38_));
  nor2   g23(.a(new_n38_), .b(x1), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n36_), .c(new_n17_), .O(new_n40_));
  nand3  g25(.a(x3), .b(new_n27_), .c(x0), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n40_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n27_), .c(x1), .d(new_n17_), .O(new_n46_));
  nand4  g31(.a(new_n18_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(z3));
  nor2   g33(.a(new_n19_), .b(x0), .O(new_n49_));
  nor2   g34(.a(x3), .b(x1), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n49_), .c(x2), .O(new_n51_));
  aoi21  g36(.a(x3), .b(new_n16_), .c(x0), .O(new_n52_));
  nand3  g37(.a(x3), .b(new_n16_), .c(x0), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n52_), .c(new_n27_), .O(new_n55_));
  nand2  g40(.a(x5), .b(new_n16_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n51_), .O(z4));
  xor2a  g42(.a(x3), .b(x2), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n16_), .c(x0), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  aoi21  g45(.a(new_n28_), .b(new_n24_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n18_), .O(new_n62_));
  nand3  g47(.a(new_n20_), .b(x1), .c(new_n17_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(z5));
  oai21  g49(.a(x4), .b(x2), .c(new_n19_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(x1), .c(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n41_), .b(new_n20_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n18_), .c(new_n16_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z6));
endmodule


