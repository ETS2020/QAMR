// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  nor3   g08(.a(new_n23_), .b(new_n16_), .c(x1), .O(new_n24_));
  nor2   g09(.a(new_n20_), .b(x1), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x2), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n24_), .c(new_n17_), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  aoi21  g13(.a(new_n25_), .b(x0), .c(new_n28_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x2), .c(new_n27_), .O(z1));
  nand3  g15(.a(new_n23_), .b(x2), .c(new_n17_), .O(new_n31_));
  nand4  g16(.a(x6), .b(x3), .c(new_n16_), .d(x0), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  oai21  g19(.a(x6), .b(x2), .c(x3), .O(new_n35_));
  nand3  g20(.a(x6), .b(x4), .c(new_n16_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(x1), .c(new_n17_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n34_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(x6), .c(new_n16_), .d(x1), .O(new_n41_));
  nand2  g26(.a(x5), .b(new_n17_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n43_));
  oai21  g28(.a(new_n41_), .b(x0), .c(new_n43_), .O(z3));
  nand2  g29(.a(new_n20_), .b(x2), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n32_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n19_), .O(new_n47_));
  nor2   g32(.a(new_n25_), .b(new_n28_), .O(new_n48_));
  nor2   g33(.a(new_n20_), .b(new_n16_), .O(new_n49_));
  aoi21  g34(.a(new_n48_), .b(new_n16_), .c(new_n49_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(x0), .c(new_n47_), .O(z4));
  nand3  g36(.a(x6), .b(x3), .c(new_n16_), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n45_), .c(new_n17_), .O(new_n53_));
  nand2  g38(.a(x5), .b(x2), .O(new_n54_));
  nor2   g39(.a(new_n54_), .b(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n19_), .O(new_n56_));
  nor2   g41(.a(x4), .b(x2), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(x3), .c(x1), .O(new_n58_));
  nand2  g43(.a(new_n20_), .b(new_n16_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(new_n28_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n49_), .c(new_n17_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n56_), .O(z5));
  nand2  g47(.a(new_n23_), .b(x2), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n53_), .c(new_n19_), .O(new_n65_));
  inv1   g50(.a(x4), .O(new_n66_));
  nand3  g51(.a(x6), .b(new_n66_), .c(new_n16_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n35_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(x1), .c(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n65_), .O(z6));
endmodule


