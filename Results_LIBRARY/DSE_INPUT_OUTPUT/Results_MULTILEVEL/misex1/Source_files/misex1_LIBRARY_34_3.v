// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nand2  g02(.a(x3), .b(x1), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(new_n16_), .c(new_n17_), .O(z0));
  inv1   g04(.a(x1), .O(new_n20_));
  nor2   g05(.a(x5), .b(x3), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n17_), .O(new_n22_));
  inv1   g07(.a(x6), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x3), .c(new_n20_), .O(new_n24_));
  aoi22  g09(.a(new_n24_), .b(new_n17_), .c(new_n22_), .d(new_n20_), .O(new_n25_));
  nand4  g10(.a(x3), .b(new_n17_), .c(new_n20_), .d(x0), .O(new_n26_));
  oai21  g11(.a(new_n25_), .b(x0), .c(new_n26_), .O(z1));
  inv1   g12(.a(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(new_n17_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x0), .O(new_n30_));
  nand3  g15(.a(new_n23_), .b(new_n17_), .c(new_n16_), .O(new_n31_));
  oai21  g16(.a(x5), .b(new_n17_), .c(new_n31_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n28_), .c(new_n20_), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n28_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n17_), .c(new_n16_), .O(new_n36_));
  nor2   g21(.a(new_n28_), .b(new_n17_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n33_), .c(new_n30_), .O(z2));
  nand2  g26(.a(new_n35_), .b(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n17_), .c(x1), .d(new_n16_), .O(new_n43_));
  aoi21  g28(.a(new_n21_), .b(new_n20_), .c(x0), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n17_), .c(new_n43_), .O(z3));
  nand3  g30(.a(x3), .b(new_n17_), .c(x0), .O(new_n46_));
  oai21  g31(.a(new_n17_), .b(x0), .c(new_n46_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n20_), .c(x2), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n37_), .c(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z4));
  oai21  g36(.a(x3), .b(new_n17_), .c(x1), .O(new_n52_));
  inv1   g37(.a(x5), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(x1), .c(new_n28_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(x2), .O(new_n55_));
  nand3  g40(.a(x6), .b(new_n28_), .c(new_n17_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n52_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n26_), .O(z5));
  nor2   g44(.a(x4), .b(x2), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(x3), .c(x1), .O(new_n61_));
  nand3  g46(.a(new_n21_), .b(x2), .c(new_n20_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n16_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n26_), .O(z6));
endmodule


