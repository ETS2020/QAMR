// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x5), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n20_), .c(new_n16_), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n19_), .O(new_n26_));
  aoi22  g11(.a(new_n26_), .b(new_n16_), .c(new_n24_), .d(new_n19_), .O(new_n27_));
  nor2   g12(.a(new_n20_), .b(x2), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n19_), .c(x0), .O(new_n29_));
  oai21  g14(.a(new_n27_), .b(x0), .c(new_n29_), .O(z1));
  oai21  g15(.a(new_n28_), .b(x1), .c(x0), .O(new_n31_));
  nand2  g16(.a(new_n23_), .b(x2), .O(new_n32_));
  oai21  g17(.a(x6), .b(x2), .c(new_n32_), .O(new_n33_));
  nand4  g18(.a(new_n33_), .b(new_n20_), .c(new_n19_), .d(new_n17_), .O(new_n34_));
  inv1   g19(.a(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(new_n20_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x1), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n34_), .c(new_n31_), .O(z2));
  nand2  g23(.a(new_n20_), .b(x2), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x0), .O(new_n41_));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n43_));
  nand4  g28(.a(new_n23_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z3));
  xor2a  g30(.a(x3), .b(x2), .O(new_n46_));
  oai21  g31(.a(x5), .b(x3), .c(x0), .O(new_n47_));
  aoi22  g32(.a(new_n47_), .b(x2), .c(new_n46_), .d(x0), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n19_), .c(x2), .O(new_n49_));
  nor2   g34(.a(new_n20_), .b(new_n16_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n49_), .c(new_n17_), .O(new_n51_));
  oai21  g36(.a(new_n48_), .b(x1), .c(new_n51_), .O(z4));
  nand3  g37(.a(x6), .b(new_n16_), .c(new_n17_), .O(new_n53_));
  oai21  g38(.a(new_n16_), .b(new_n17_), .c(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  oai21  g40(.a(new_n28_), .b(x1), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n23_), .b(x1), .c(new_n20_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(x2), .c(new_n17_), .O(new_n58_));
  nand2  g43(.a(new_n39_), .b(x1), .O(new_n59_));
  nand4  g44(.a(new_n59_), .b(new_n58_), .c(new_n56_), .d(new_n55_), .O(z5));
  oai21  g45(.a(x5), .b(x1), .c(new_n17_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n20_), .c(x2), .O(new_n62_));
  nor2   g47(.a(x4), .b(x2), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(x3), .c(x1), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n62_), .c(new_n56_), .O(z6));
endmodule


