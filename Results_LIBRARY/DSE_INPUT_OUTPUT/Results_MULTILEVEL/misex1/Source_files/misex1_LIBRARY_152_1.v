// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  nand2  g08(.a(x6), .b(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  oai21  g10(.a(x5), .b(x3), .c(x2), .O(new_n26_));
  nand2  g11(.a(new_n23_), .b(x1), .O(new_n27_));
  oai21  g12(.a(new_n26_), .b(x1), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  nand3  g15(.a(x3), .b(new_n18_), .c(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n29_), .O(z1));
  aoi21  g19(.a(x4), .b(new_n23_), .c(x3), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n19_), .c(new_n18_), .O(new_n37_));
  oai21  g22(.a(new_n35_), .b(new_n18_), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n33_), .O(z2));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(x1), .c(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n30_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n23_), .O(new_n44_));
  nand2  g29(.a(x5), .b(new_n16_), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(z3));
  nand3  g32(.a(x3), .b(new_n23_), .c(x0), .O(new_n48_));
  oai21  g33(.a(x3), .b(new_n23_), .c(new_n48_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n18_), .O(new_n50_));
  aoi21  g35(.a(x3), .b(new_n18_), .c(x2), .O(new_n51_));
  nor2   g36(.a(new_n19_), .b(new_n23_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n51_), .c(new_n16_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n50_), .c(new_n24_), .O(z4));
  xnor2a g39(.a(x3), .b(x2), .O(new_n55_));
  nand3  g40(.a(x5), .b(x2), .c(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n55_), .b(new_n16_), .c(new_n56_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n18_), .O(new_n58_));
  aoi21  g43(.a(new_n19_), .b(x2), .c(new_n18_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n52_), .c(new_n16_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n58_), .c(new_n24_), .O(z5));
  inv1   g46(.a(x5), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n19_), .c(x2), .O(new_n63_));
  oai21  g48(.a(new_n55_), .b(new_n16_), .c(new_n63_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n18_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n19_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(x1), .c(new_n16_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n65_), .c(new_n24_), .O(z6));
endmodule


