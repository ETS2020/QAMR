// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n43_, new_n44_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  nand2  g02(.a(x3), .b(x2), .O(new_n18_));
  oai22  g03(.a(new_n18_), .b(new_n17_), .c(x3), .d(x1), .O(z0));
  inv1   g04(.a(x1), .O(new_n20_));
  nor2   g05(.a(x2), .b(new_n20_), .O(new_n21_));
  nor2   g06(.a(new_n18_), .b(x1), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n16_), .O(new_n23_));
  inv1   g08(.a(x2), .O(new_n24_));
  nand4  g09(.a(x3), .b(new_n24_), .c(new_n20_), .d(x0), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n23_), .O(z1));
  inv1   g11(.a(x3), .O(new_n27_));
  inv1   g12(.a(x4), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x2), .c(new_n27_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(x1), .c(new_n16_), .O(new_n30_));
  oai21  g15(.a(x2), .b(new_n16_), .c(x3), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n20_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n30_), .O(z2));
  oai21  g18(.a(x4), .b(x3), .c(x7), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n24_), .c(x1), .d(new_n16_), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(z3));
  inv1   g21(.a(new_n18_), .O(new_n37_));
  oai21  g22(.a(new_n21_), .b(new_n37_), .c(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n25_), .O(z4));
  aoi21  g24(.a(new_n27_), .b(x2), .c(new_n20_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n37_), .c(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n25_), .O(z5));
  oai21  g27(.a(x4), .b(x2), .c(new_n27_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x1), .c(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n25_), .O(z6));
endmodule


