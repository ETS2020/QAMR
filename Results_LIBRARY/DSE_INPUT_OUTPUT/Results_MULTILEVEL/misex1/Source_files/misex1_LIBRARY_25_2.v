// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand4  g01(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(z0));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x2), .O(new_n20_));
  nor2   g05(.a(x5), .b(x3), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x3), .c(new_n19_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n23_), .c(x0), .O(z1));
  inv1   g12(.a(x3), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x2), .c(new_n28_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(x1), .c(new_n16_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x2), .O(new_n33_));
  nand2  g18(.a(new_n24_), .b(new_n20_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n33_), .c(x3), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x0), .c(new_n19_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n31_), .O(z2));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n20_), .c(x1), .O(new_n39_));
  nand3  g24(.a(new_n21_), .b(x2), .c(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n19_), .b(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(z3));
  oai21  g29(.a(x3), .b(new_n19_), .c(x2), .O(new_n45_));
  oai21  g30(.a(new_n28_), .b(x1), .c(new_n20_), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(x0), .O(z4));
  oai21  g32(.a(x3), .b(new_n20_), .c(x1), .O(new_n48_));
  nand2  g33(.a(x3), .b(x2), .O(new_n49_));
  nand3  g34(.a(x6), .b(new_n28_), .c(new_n20_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n32_), .b(new_n20_), .c(new_n16_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(z5));
  nor2   g40(.a(x4), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(x3), .c(x1), .O(new_n57_));
  aoi21  g42(.a(new_n21_), .b(x2), .c(x0), .O(new_n58_));
  oai22  g43(.a(new_n58_), .b(x1), .c(new_n57_), .d(x0), .O(z6));
endmodule


