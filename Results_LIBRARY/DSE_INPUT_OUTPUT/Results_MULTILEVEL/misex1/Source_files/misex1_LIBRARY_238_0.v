// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(x4), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand2  g06(.a(x1), .b(new_n17_), .O(new_n22_));
  nand2  g07(.a(x3), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n18_), .c(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n25_), .b(x1), .c(new_n27_), .O(new_n28_));
  nor2   g13(.a(x2), .b(new_n16_), .O(new_n29_));
  aoi21  g14(.a(new_n28_), .b(x4), .c(new_n29_), .O(new_n30_));
  inv1   g15(.a(x4), .O(new_n31_));
  nor2   g16(.a(new_n31_), .b(new_n18_), .O(new_n32_));
  nand4  g17(.a(new_n32_), .b(new_n26_), .c(new_n16_), .d(x0), .O(new_n33_));
  oai21  g18(.a(new_n30_), .b(x0), .c(new_n33_), .O(z1));
  nand2  g19(.a(x4), .b(new_n26_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n18_), .c(new_n16_), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n26_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(x3), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n16_), .c(new_n36_), .O(new_n42_));
  nand3  g27(.a(x3), .b(new_n26_), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x4), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n16_), .O(new_n45_));
  oai21  g30(.a(new_n42_), .b(x0), .c(new_n45_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n26_), .c(x1), .d(new_n17_), .O(new_n48_));
  aoi21  g33(.a(x5), .b(new_n17_), .c(new_n31_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n18_), .c(x2), .d(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z3));
  nor2   g36(.a(new_n18_), .b(x0), .O(new_n52_));
  nor2   g37(.a(x3), .b(x1), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n52_), .c(x2), .O(new_n54_));
  aoi21  g39(.a(x3), .b(new_n16_), .c(x0), .O(new_n55_));
  nand3  g40(.a(x3), .b(new_n16_), .c(x0), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n26_), .O(new_n58_));
  nand2  g43(.a(new_n31_), .b(new_n16_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n58_), .c(new_n54_), .O(z4));
  xnor2a g45(.a(x3), .b(x2), .O(new_n61_));
  nand3  g46(.a(x5), .b(x2), .c(new_n17_), .O(new_n62_));
  oai21  g47(.a(new_n61_), .b(new_n17_), .c(new_n62_), .O(new_n63_));
  aoi21  g48(.a(new_n27_), .b(new_n23_), .c(x0), .O(new_n64_));
  aoi21  g49(.a(new_n63_), .b(new_n16_), .c(new_n64_), .O(new_n65_));
  nand3  g50(.a(new_n19_), .b(x1), .c(new_n17_), .O(new_n66_));
  oai21  g51(.a(new_n65_), .b(new_n31_), .c(new_n66_), .O(z5));
  nand3  g52(.a(new_n37_), .b(new_n18_), .c(x2), .O(new_n68_));
  oai21  g53(.a(new_n61_), .b(new_n17_), .c(new_n68_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(x4), .c(new_n16_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n18_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(x1), .c(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


