// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x7), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x6), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nand3  g04(.a(x3), .b(x1), .c(new_n19_), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n21_), .c(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n18_), .c(x2), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(z0));
  inv1   g11(.a(x2), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n21_), .O(new_n31_));
  aoi22  g16(.a(new_n31_), .b(new_n27_), .c(new_n29_), .d(new_n21_), .O(new_n32_));
  nand4  g17(.a(x3), .b(new_n27_), .c(new_n21_), .d(x0), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nor2   g19(.a(new_n34_), .b(new_n17_), .O(new_n35_));
  oai21  g20(.a(new_n32_), .b(x0), .c(new_n35_), .O(z1));
  aoi21  g21(.a(x4), .b(new_n27_), .c(x3), .O(new_n37_));
  nand3  g22(.a(new_n28_), .b(x2), .c(new_n21_), .O(new_n38_));
  oai21  g23(.a(new_n37_), .b(new_n21_), .c(new_n38_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n19_), .c(new_n34_), .O(new_n40_));
  nor3   g25(.a(x2), .b(x1), .c(x0), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n16_), .c(new_n30_), .d(new_n22_), .O(new_n42_));
  oai21  g27(.a(new_n40_), .b(new_n17_), .c(new_n42_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n27_), .c(x1), .d(new_n19_), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n19_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n22_), .c(x2), .d(new_n21_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n45_), .c(new_n18_), .O(z3));
  nand3  g33(.a(x3), .b(new_n27_), .c(x0), .O(new_n49_));
  oai21  g34(.a(x3), .b(new_n27_), .c(new_n49_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n21_), .O(new_n51_));
  aoi21  g36(.a(x3), .b(new_n21_), .c(x2), .O(new_n52_));
  nor2   g37(.a(new_n22_), .b(new_n27_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n52_), .c(new_n19_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n51_), .c(new_n18_), .O(z4));
  xnor2a g40(.a(x3), .b(x2), .O(new_n56_));
  nor2   g41(.a(new_n56_), .b(x1), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x0), .O(new_n58_));
  oai21  g43(.a(x4), .b(x2), .c(new_n22_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(x1), .c(new_n19_), .O(new_n60_));
  aoi21  g45(.a(x5), .b(new_n21_), .c(x3), .O(new_n61_));
  nand3  g46(.a(new_n16_), .b(new_n30_), .c(new_n21_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n22_), .c(new_n27_), .O(new_n63_));
  oai21  g48(.a(new_n61_), .b(new_n27_), .c(new_n63_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n19_), .O(new_n65_));
  nand4  g50(.a(new_n65_), .b(new_n60_), .c(new_n58_), .d(new_n18_), .O(z5));
  nand2  g51(.a(new_n28_), .b(x2), .O(new_n67_));
  oai21  g52(.a(new_n56_), .b(new_n19_), .c(new_n67_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n21_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n60_), .c(new_n17_), .O(z6));
endmodule


