// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(x6), .b(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x3), .O(new_n23_));
  nand2  g08(.a(x6), .b(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n16_), .c(x1), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  oai21  g11(.a(x6), .b(new_n26_), .c(new_n23_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(x2), .c(new_n19_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n17_), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(z1));
  nor2   g17(.a(new_n19_), .b(x0), .O(new_n33_));
  nand3  g18(.a(new_n16_), .b(new_n19_), .c(x0), .O(new_n34_));
  inv1   g19(.a(new_n34_), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n33_), .c(x3), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n19_), .O(new_n38_));
  nand2  g23(.a(x4), .b(new_n16_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n38_), .c(x0), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(x6), .c(new_n23_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n36_), .O(z2));
  inv1   g27(.a(x6), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n17_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n23_), .c(x2), .d(new_n19_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  inv1   g34(.a(x7), .O(new_n50_));
  nand4  g35(.a(new_n33_), .b(new_n50_), .c(x3), .d(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(z3));
  nand3  g37(.a(new_n50_), .b(new_n43_), .c(new_n16_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n23_), .c(new_n19_), .O(new_n54_));
  nand2  g39(.a(x3), .b(x2), .O(new_n55_));
  nand2  g40(.a(new_n20_), .b(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n54_), .c(new_n17_), .O(new_n58_));
  nand2  g43(.a(x3), .b(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n20_), .b(x2), .O(new_n60_));
  oai21  g45(.a(new_n59_), .b(new_n17_), .c(new_n60_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n19_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n58_), .O(z4));
  aoi21  g48(.a(new_n60_), .b(new_n59_), .c(new_n17_), .O(new_n64_));
  nand4  g49(.a(new_n43_), .b(x5), .c(new_n23_), .d(x2), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n64_), .c(new_n19_), .O(new_n67_));
  nor2   g52(.a(x6), .b(x2), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(x3), .c(x1), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n55_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n17_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n67_), .O(z5));
  nand2  g57(.a(new_n23_), .b(x2), .O(new_n73_));
  aoi21  g58(.a(new_n73_), .b(new_n59_), .c(new_n17_), .O(new_n74_));
  nand3  g59(.a(new_n26_), .b(new_n23_), .c(x2), .O(new_n75_));
  inv1   g60(.a(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n74_), .c(new_n19_), .O(new_n77_));
  inv1   g62(.a(x4), .O(new_n78_));
  nand3  g63(.a(new_n43_), .b(new_n78_), .c(new_n16_), .O(new_n79_));
  nand2  g64(.a(new_n79_), .b(new_n23_), .O(new_n80_));
  nand3  g65(.a(new_n80_), .b(x1), .c(new_n17_), .O(new_n81_));
  nand3  g66(.a(new_n81_), .b(new_n77_), .c(new_n24_), .O(z6));
endmodule


