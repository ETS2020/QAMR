// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  nand2  g00(.a(x3), .b(x2), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x6), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x1), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n19_), .c(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n18_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(x2), .c(new_n19_), .O(new_n28_));
  inv1   g13(.a(x2), .O(new_n29_));
  nand2  g14(.a(x3), .b(new_n19_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(x6), .c(new_n29_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n25_), .O(new_n33_));
  nand4  g18(.a(x3), .b(new_n29_), .c(new_n19_), .d(x0), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(z1));
  nand2  g20(.a(x3), .b(new_n29_), .O(new_n36_));
  nand2  g21(.a(x2), .b(new_n25_), .O(new_n37_));
  oai22  g22(.a(new_n37_), .b(new_n27_), .c(new_n36_), .d(new_n25_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n19_), .O(new_n39_));
  aoi21  g24(.a(x4), .b(new_n29_), .c(x3), .O(new_n40_));
  inv1   g25(.a(x6), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n20_), .c(new_n29_), .O(new_n42_));
  oai21  g27(.a(new_n40_), .b(new_n19_), .c(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  nand2  g29(.a(new_n41_), .b(x1), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(new_n39_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(x6), .c(new_n29_), .d(x1), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n25_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x0), .c(new_n50_), .O(z3));
  nand3  g36(.a(x6), .b(x1), .c(new_n25_), .O(new_n52_));
  oai21  g37(.a(new_n30_), .b(new_n25_), .c(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n29_), .O(new_n54_));
  aoi21  g39(.a(x3), .b(new_n29_), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n22_), .c(new_n19_), .O(new_n56_));
  nand4  g41(.a(x6), .b(x3), .c(x2), .d(new_n25_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(z4));
  aoi21  g43(.a(new_n36_), .b(new_n21_), .c(new_n25_), .O(new_n59_));
  nor3   g44(.a(new_n26_), .b(new_n29_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n59_), .c(new_n19_), .O(new_n61_));
  nand2  g46(.a(new_n21_), .b(x1), .O(new_n62_));
  nand3  g47(.a(x6), .b(new_n20_), .c(new_n29_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n62_), .c(new_n16_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n25_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n61_), .c(new_n45_), .O(z5));
  nand3  g51(.a(new_n26_), .b(new_n20_), .c(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n59_), .c(new_n19_), .O(new_n69_));
  nor2   g54(.a(x4), .b(x2), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(x3), .c(new_n25_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(x6), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(x1), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n69_), .O(z6));
endmodule


