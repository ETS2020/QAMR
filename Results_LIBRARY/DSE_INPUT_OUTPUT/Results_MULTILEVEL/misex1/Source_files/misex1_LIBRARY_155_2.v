// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x3), .b(x2), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x1), .c(new_n16_), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n16_), .c(new_n19_), .O(z0));
  inv1   g05(.a(x1), .O(new_n21_));
  oai21  g06(.a(x5), .b(x3), .c(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n24_), .c(new_n23_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(new_n22_), .c(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n21_), .O(new_n27_));
  nand3  g12(.a(new_n23_), .b(x1), .c(new_n16_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(z1));
  inv1   g14(.a(x4), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x2), .c(new_n24_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(x1), .c(new_n16_), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x6), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n23_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n34_), .c(x3), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x0), .c(new_n21_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n32_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n23_), .c(x1), .O(new_n41_));
  nand4  g26(.a(new_n33_), .b(new_n24_), .c(x2), .d(new_n21_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n41_), .c(x0), .O(z3));
  oai21  g28(.a(x2), .b(x0), .c(new_n21_), .O(new_n44_));
  aoi21  g29(.a(x3), .b(new_n21_), .c(x2), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n18_), .c(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(z4));
  oai21  g32(.a(x3), .b(new_n23_), .c(x1), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n25_), .c(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n16_), .O(new_n50_));
  oai21  g35(.a(new_n33_), .b(new_n23_), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z5));
  nor2   g38(.a(x4), .b(x2), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x3), .c(x1), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n42_), .c(x0), .O(z6));
endmodule


