// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nor2   g02(.a(x1), .b(new_n17_), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n16_), .c(x2), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(z0));
  inv1   g05(.a(x1), .O(new_n21_));
  oai21  g06(.a(x5), .b(x3), .c(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  nand3  g08(.a(x6), .b(new_n16_), .c(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n22_), .c(new_n21_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(new_n26_));
  nand3  g11(.a(new_n18_), .b(x3), .c(new_n23_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n26_), .O(z1));
  nand3  g13(.a(x3), .b(new_n21_), .c(x0), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  nor3   g15(.a(x6), .b(x3), .c(x0), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n30_), .c(new_n23_), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n16_), .c(x2), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n21_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n17_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n32_), .O(z2));
  nand3  g22(.a(new_n18_), .b(new_n16_), .c(x2), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(z3));
  nand2  g24(.a(x3), .b(new_n23_), .O(new_n40_));
  nand2  g25(.a(new_n16_), .b(x2), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n21_), .c(x0), .O(new_n43_));
  oai21  g28(.a(new_n40_), .b(x1), .c(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(z4));
  nand2  g30(.a(new_n42_), .b(x0), .O(new_n46_));
  nand2  g31(.a(new_n24_), .b(new_n22_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n17_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n46_), .c(x1), .O(z5));
  aoi21  g34(.a(new_n46_), .b(new_n34_), .c(x1), .O(z6));
endmodule


