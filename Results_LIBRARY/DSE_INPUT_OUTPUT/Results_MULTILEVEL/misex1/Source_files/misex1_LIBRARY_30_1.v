// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x5), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x2), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n16_), .O(new_n22_));
  nand2  g07(.a(x1), .b(new_n17_), .O(new_n23_));
  nand2  g08(.a(x3), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(z0));
  xor2a  g10(.a(x2), .b(x0), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(x3), .c(x5), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n16_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n28_), .c(new_n17_), .O(new_n31_));
  oai21  g16(.a(new_n27_), .b(x1), .c(new_n31_), .O(z1));
  inv1   g17(.a(x4), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x2), .c(new_n19_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x1), .O(new_n35_));
  nand2  g20(.a(x6), .b(new_n28_), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n18_), .c(new_n19_), .d(new_n16_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n17_), .O(new_n39_));
  nor2   g24(.a(x5), .b(new_n19_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n28_), .c(new_n16_), .d(x0), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n28_), .c(x1), .d(new_n17_), .O(new_n44_));
  nand4  g29(.a(new_n18_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z3));
  nand2  g31(.a(new_n19_), .b(new_n28_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n24_), .O(new_n48_));
  aoi22  g33(.a(new_n48_), .b(new_n18_), .c(new_n20_), .d(x1), .O(new_n49_));
  nand3  g34(.a(x3), .b(new_n28_), .c(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n18_), .c(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n49_), .b(x0), .c(new_n52_), .O(z4));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n16_), .c(x0), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  nand3  g41(.a(x6), .b(new_n19_), .c(new_n28_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n24_), .c(x0), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n18_), .O(new_n59_));
  nand3  g44(.a(new_n20_), .b(x1), .c(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n59_), .O(z5));
  oai21  g46(.a(x4), .b(x2), .c(new_n19_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(x1), .c(new_n17_), .O(new_n63_));
  nand3  g48(.a(new_n40_), .b(new_n28_), .c(x0), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n20_), .c(new_n18_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n16_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z6));
endmodule


