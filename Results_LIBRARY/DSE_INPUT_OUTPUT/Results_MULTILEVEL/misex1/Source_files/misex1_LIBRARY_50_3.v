// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  nor2   g04(.a(new_n17_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x6), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x3), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand2  g10(.a(x5), .b(new_n18_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n22_), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n18_), .O(new_n28_));
  nand2  g13(.a(new_n21_), .b(x1), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n28_), .c(x2), .O(new_n30_));
  aoi21  g15(.a(new_n27_), .b(new_n17_), .c(new_n30_), .O(new_n31_));
  nand3  g16(.a(new_n21_), .b(x3), .c(new_n16_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n17_), .c(x0), .O(new_n34_));
  oai21  g19(.a(new_n31_), .b(x0), .c(new_n34_), .O(z1));
  nand2  g20(.a(x4), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n18_), .c(new_n17_), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  nand2  g24(.a(new_n21_), .b(new_n16_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n39_), .c(x3), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n17_), .c(new_n37_), .O(new_n42_));
  nand3  g27(.a(new_n16_), .b(new_n17_), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(x3), .O(new_n45_));
  oai21  g30(.a(new_n42_), .b(x0), .c(new_n45_), .O(z2));
  inv1   g31(.a(x0), .O(new_n47_));
  nand2  g32(.a(x7), .b(x4), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n16_), .c(x1), .d(new_n47_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n47_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(x2), .c(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n18_), .O(new_n53_));
  nor3   g38(.a(x7), .b(x6), .c(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z3));
  nand2  g41(.a(new_n18_), .b(x2), .O(new_n57_));
  nand2  g42(.a(new_n16_), .b(x0), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n22_), .c(new_n57_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n17_), .O(new_n60_));
  aoi21  g45(.a(new_n29_), .b(x3), .c(x2), .O(new_n61_));
  nand3  g46(.a(new_n21_), .b(x3), .c(x2), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n61_), .c(new_n47_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n60_), .O(z4));
  aoi21  g50(.a(new_n57_), .b(new_n32_), .c(new_n47_), .O(new_n66_));
  nand3  g51(.a(x5), .b(new_n18_), .c(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n66_), .c(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n18_), .b(new_n16_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n22_), .c(new_n17_), .O(new_n71_));
  oai21  g56(.a(new_n28_), .b(x2), .c(new_n62_), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n71_), .c(new_n47_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n69_), .O(z5));
  nand3  g59(.a(new_n38_), .b(new_n18_), .c(x2), .O(new_n75_));
  inv1   g60(.a(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n66_), .c(new_n17_), .O(new_n77_));
  inv1   g62(.a(x4), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(new_n18_), .c(new_n16_), .O(new_n79_));
  aoi21  g64(.a(new_n79_), .b(new_n22_), .c(new_n17_), .O(new_n80_));
  aoi22  g65(.a(new_n80_), .b(new_n47_), .c(x6), .d(x3), .O(new_n81_));
  nand2  g66(.a(new_n81_), .b(new_n77_), .O(z6));
endmodule


