// Benchmark "FAU" written by ABC on Fri Aug 14 01:58:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n17_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x7), .c(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  xnor2a g08(.a(x2), .b(x0), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  nand2  g10(.a(x7), .b(new_n25_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(x2), .c(new_n17_), .O(new_n27_));
  oai21  g12(.a(new_n24_), .b(new_n19_), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n23_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n23_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n16_), .c(new_n17_), .O(new_n32_));
  nor2   g17(.a(x7), .b(x3), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n32_), .c(new_n29_), .O(z1));
  nand2  g20(.a(x4), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n19_), .c(new_n23_), .O(new_n37_));
  nand2  g22(.a(new_n25_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n30_), .b(new_n16_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n38_), .c(x3), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n23_), .c(new_n37_), .O(new_n41_));
  nand2  g26(.a(x3), .b(new_n16_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n20_), .c(new_n33_), .O(new_n44_));
  oai21  g29(.a(new_n41_), .b(x0), .c(new_n44_), .O(z2));
  inv1   g30(.a(x4), .O(new_n46_));
  nand3  g31(.a(x7), .b(new_n46_), .c(new_n19_), .O(new_n47_));
  oai21  g32(.a(x7), .b(new_n19_), .c(new_n47_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n17_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(x7), .c(x2), .d(new_n23_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x7), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n19_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n49_), .O(z3));
  nand3  g39(.a(x7), .b(new_n19_), .c(x2), .O(new_n55_));
  oai21  g40(.a(new_n42_), .b(new_n17_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n23_), .O(new_n57_));
  oai21  g42(.a(x2), .b(x1), .c(x3), .O(new_n58_));
  nand3  g43(.a(x7), .b(new_n19_), .c(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n17_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n57_), .O(z4));
  aoi21  g47(.a(new_n55_), .b(new_n42_), .c(new_n17_), .O(new_n63_));
  nand4  g48(.a(x7), .b(x5), .c(new_n19_), .d(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n23_), .O(new_n66_));
  nand2  g51(.a(x7), .b(new_n16_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n19_), .c(new_n23_), .O(new_n68_));
  nand4  g53(.a(x7), .b(x6), .c(new_n19_), .d(new_n16_), .O(new_n69_));
  oai21  g54(.a(new_n19_), .b(new_n16_), .c(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n68_), .c(new_n17_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n66_), .O(z5));
  nand2  g57(.a(new_n19_), .b(x2), .O(new_n73_));
  aoi21  g58(.a(new_n73_), .b(new_n42_), .c(new_n17_), .O(new_n74_));
  nand3  g59(.a(new_n25_), .b(new_n19_), .c(x2), .O(new_n75_));
  inv1   g60(.a(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n74_), .c(new_n23_), .O(new_n77_));
  oai21  g62(.a(x4), .b(x2), .c(new_n19_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(x1), .c(new_n17_), .O(new_n79_));
  nand3  g64(.a(new_n79_), .b(new_n77_), .c(new_n34_), .O(z6));
endmodule


