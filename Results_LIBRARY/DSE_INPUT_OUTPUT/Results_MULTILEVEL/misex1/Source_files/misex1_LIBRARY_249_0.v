// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(x7), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nor2   g07(.a(x7), .b(x3), .O(new_n23_));
  nand2  g08(.a(x7), .b(x6), .O(new_n24_));
  oai22  g09(.a(new_n24_), .b(x3), .c(new_n23_), .d(new_n19_), .O(new_n25_));
  nand2  g10(.a(x7), .b(x5), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n20_), .c(new_n16_), .O(new_n27_));
  aoi22  g12(.a(new_n27_), .b(new_n19_), .c(new_n25_), .d(new_n16_), .O(new_n28_));
  nand4  g13(.a(x3), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n29_));
  oai21  g14(.a(new_n28_), .b(x0), .c(new_n29_), .O(z1));
  inv1   g15(.a(new_n23_), .O(new_n31_));
  nand3  g16(.a(new_n16_), .b(new_n19_), .c(x0), .O(new_n32_));
  oai21  g17(.a(new_n19_), .b(x0), .c(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x3), .O(new_n34_));
  inv1   g19(.a(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(x7), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x1), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n20_), .c(new_n19_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n17_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n34_), .c(new_n31_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n48_));
  inv1   g33(.a(x7), .O(new_n49_));
  aoi21  g34(.a(x5), .b(new_n17_), .c(new_n16_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n19_), .c(new_n49_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(x3), .c(new_n48_), .O(z3));
  nand2  g37(.a(x3), .b(new_n16_), .O(new_n53_));
  nand2  g38(.a(new_n20_), .b(x2), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n53_), .c(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n38_), .b(new_n20_), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(x0), .c(new_n16_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n19_), .O(new_n58_));
  aoi21  g43(.a(x3), .b(new_n19_), .c(x2), .O(new_n59_));
  nand2  g44(.a(x3), .b(x2), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n17_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n58_), .c(new_n31_), .O(z4));
  nand3  g48(.a(x7), .b(new_n20_), .c(x2), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n53_), .c(new_n17_), .O(new_n65_));
  nand4  g50(.a(x7), .b(x5), .c(new_n20_), .d(x2), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n65_), .c(new_n19_), .O(new_n68_));
  nand2  g53(.a(x7), .b(new_n16_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n20_), .c(new_n19_), .O(new_n70_));
  nand2  g55(.a(new_n20_), .b(new_n16_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n24_), .c(new_n60_), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n70_), .c(new_n17_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n68_), .O(z5));
  nand3  g59(.a(new_n38_), .b(new_n20_), .c(x2), .O(new_n75_));
  inv1   g60(.a(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n55_), .c(new_n19_), .O(new_n77_));
  oai21  g62(.a(x4), .b(x2), .c(new_n20_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(x1), .c(new_n17_), .O(new_n79_));
  nand3  g64(.a(new_n79_), .b(new_n77_), .c(new_n31_), .O(z6));
endmodule


