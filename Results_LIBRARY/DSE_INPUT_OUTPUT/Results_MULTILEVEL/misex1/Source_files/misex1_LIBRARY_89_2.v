// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x3), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x2), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x1), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x2), .O(new_n20_));
  nor2   g05(.a(x3), .b(new_n20_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n19_), .c(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n18_), .O(z0));
  inv1   g08(.a(x0), .O(new_n24_));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n16_), .c(new_n20_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n24_), .O(new_n28_));
  nor2   g13(.a(new_n16_), .b(x2), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x0), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n28_), .c(x1), .O(z1));
  inv1   g16(.a(x5), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(x2), .c(new_n19_), .O(new_n33_));
  inv1   g18(.a(x6), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n20_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n16_), .c(new_n24_), .O(new_n37_));
  nand3  g22(.a(new_n29_), .b(new_n19_), .c(x0), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n37_), .c(new_n18_), .O(z2));
  nand2  g24(.a(x5), .b(new_n24_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n16_), .c(x2), .d(new_n19_), .O(new_n41_));
  oai21  g26(.a(x2), .b(new_n19_), .c(new_n41_), .O(z3));
  nand2  g27(.a(x3), .b(x2), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  nor2   g29(.a(x3), .b(x2), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(new_n24_), .O(new_n46_));
  oai21  g31(.a(new_n16_), .b(new_n24_), .c(new_n19_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  nand2  g33(.a(new_n21_), .b(new_n19_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(z4));
  xnor2a g35(.a(x3), .b(x2), .O(new_n51_));
  nand3  g36(.a(x5), .b(x2), .c(new_n24_), .O(new_n52_));
  oai21  g37(.a(new_n51_), .b(new_n24_), .c(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  nor2   g39(.a(x2), .b(new_n19_), .O(new_n55_));
  nand2  g40(.a(new_n43_), .b(new_n26_), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n24_), .c(new_n55_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n54_), .O(z5));
  nor2   g43(.a(x5), .b(x3), .O(new_n59_));
  xor2a  g44(.a(x3), .b(x2), .O(new_n60_));
  aoi22  g45(.a(new_n60_), .b(x0), .c(new_n59_), .d(x2), .O(new_n61_));
  nand3  g46(.a(new_n44_), .b(x1), .c(new_n24_), .O(new_n62_));
  oai21  g47(.a(new_n61_), .b(x1), .c(new_n62_), .O(z6));
endmodule


