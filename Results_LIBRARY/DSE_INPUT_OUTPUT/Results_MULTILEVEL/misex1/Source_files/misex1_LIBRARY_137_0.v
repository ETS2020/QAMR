// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(x7), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x7), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x1), .O(new_n25_));
  nand3  g10(.a(x7), .b(x6), .c(new_n20_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n25_), .c(x2), .O(new_n27_));
  aoi21  g12(.a(x7), .b(x5), .c(x3), .O(new_n28_));
  nor3   g13(.a(new_n28_), .b(new_n16_), .c(x1), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(new_n17_), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(z1));
  nand2  g17(.a(x7), .b(x4), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x2), .c(new_n20_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x1), .O(new_n35_));
  inv1   g20(.a(x5), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x2), .O(new_n37_));
  inv1   g22(.a(x6), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(x7), .c(new_n20_), .d(new_n19_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n35_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n31_), .O(z2));
  inv1   g29(.a(x4), .O(new_n45_));
  nand2  g30(.a(x7), .b(new_n45_), .O(new_n46_));
  nand2  g31(.a(new_n23_), .b(x3), .O(new_n47_));
  oai21  g32(.a(new_n46_), .b(x3), .c(new_n47_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n49_));
  aoi21  g34(.a(x5), .b(new_n17_), .c(new_n23_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(z3));
  nand2  g37(.a(x3), .b(new_n16_), .O(new_n53_));
  nand3  g38(.a(x7), .b(new_n20_), .c(x2), .O(new_n54_));
  oai21  g39(.a(new_n53_), .b(new_n17_), .c(new_n54_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n19_), .O(new_n56_));
  oai21  g41(.a(x2), .b(x1), .c(x3), .O(new_n57_));
  nand3  g42(.a(x7), .b(new_n20_), .c(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n56_), .O(z4));
  nand2  g46(.a(new_n20_), .b(x2), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n53_), .c(new_n17_), .O(new_n63_));
  nand3  g48(.a(x5), .b(new_n20_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n19_), .O(new_n66_));
  nand2  g51(.a(new_n62_), .b(x1), .O(new_n67_));
  nand2  g52(.a(x3), .b(x2), .O(new_n68_));
  nand3  g53(.a(x6), .b(new_n20_), .c(new_n16_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n17_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n66_), .c(new_n24_), .O(z5));
  aoi21  g57(.a(new_n54_), .b(new_n53_), .c(new_n17_), .O(new_n73_));
  nand4  g58(.a(x7), .b(new_n36_), .c(new_n20_), .d(x2), .O(new_n74_));
  inv1   g59(.a(new_n74_), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(new_n73_), .c(new_n19_), .O(new_n76_));
  oai21  g61(.a(new_n46_), .b(x2), .c(new_n20_), .O(new_n77_));
  nand3  g62(.a(new_n77_), .b(x1), .c(new_n17_), .O(new_n78_));
  nand2  g63(.a(new_n78_), .b(new_n76_), .O(z6));
endmodule


