// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x4), .O(new_n23_));
  nand2  g08(.a(x6), .b(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  inv1   g10(.a(new_n24_), .O(new_n26_));
  oai21  g11(.a(x5), .b(x3), .c(x2), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x1), .O(new_n29_));
  oai21  g14(.a(new_n27_), .b(x1), .c(new_n29_), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n28_), .c(new_n18_), .d(x0), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  aoi21  g17(.a(new_n30_), .b(new_n16_), .c(new_n32_), .O(new_n33_));
  nor2   g18(.a(x2), .b(x0), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(x6), .c(x4), .d(new_n19_), .O(new_n35_));
  oai21  g20(.a(new_n33_), .b(new_n26_), .c(new_n35_), .O(z1));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n28_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n19_), .c(new_n18_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(x6), .b(new_n19_), .c(new_n23_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n28_), .O(new_n45_));
  nand2  g30(.a(x3), .b(x2), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(new_n18_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n43_), .c(new_n16_), .O(new_n48_));
  nor2   g33(.a(new_n32_), .b(new_n26_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(z2));
  oai21  g35(.a(x4), .b(x3), .c(x7), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(new_n28_), .c(x1), .d(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n37_), .b(x0), .c(new_n19_), .O(new_n53_));
  nor2   g38(.a(new_n53_), .b(new_n28_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n18_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n52_), .c(new_n24_), .O(z3));
  nand3  g41(.a(x3), .b(new_n28_), .c(x0), .O(new_n57_));
  oai21  g42(.a(x3), .b(new_n28_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n18_), .O(new_n59_));
  inv1   g44(.a(new_n46_), .O(new_n60_));
  aoi21  g45(.a(x3), .b(new_n18_), .c(x2), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n59_), .c(new_n24_), .O(z4));
  inv1   g48(.a(new_n29_), .O(new_n64_));
  aoi21  g49(.a(x5), .b(new_n18_), .c(x3), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(x0), .c(new_n20_), .O(new_n66_));
  aoi22  g51(.a(new_n66_), .b(x2), .c(new_n64_), .d(new_n16_), .O(new_n67_));
  aoi21  g52(.a(x6), .b(new_n23_), .c(new_n19_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n18_), .c(x0), .O(new_n69_));
  nand4  g54(.a(x6), .b(x4), .c(new_n19_), .d(new_n16_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n28_), .O(new_n72_));
  oai21  g57(.a(new_n67_), .b(new_n26_), .c(new_n72_), .O(z5));
  nand3  g58(.a(new_n68_), .b(new_n28_), .c(x0), .O(new_n74_));
  inv1   g59(.a(new_n74_), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(new_n54_), .c(new_n18_), .O(new_n76_));
  oai21  g61(.a(new_n24_), .b(new_n28_), .c(x3), .O(new_n77_));
  oai21  g62(.a(x4), .b(x2), .c(new_n77_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(x1), .c(new_n16_), .O(new_n79_));
  nand3  g64(.a(new_n79_), .b(new_n76_), .c(new_n24_), .O(z6));
endmodule


