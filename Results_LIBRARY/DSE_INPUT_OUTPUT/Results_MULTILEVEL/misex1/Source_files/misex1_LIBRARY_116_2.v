// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x5), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n24_), .c(x0), .O(new_n25_));
  oai21  g10(.a(new_n24_), .b(x0), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n18_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n24_), .c(new_n16_), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x2), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(new_n27_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n24_), .c(x3), .O(new_n34_));
  nand4  g19(.a(new_n28_), .b(new_n19_), .c(new_n24_), .d(new_n18_), .O(new_n35_));
  oai21  g20(.a(new_n34_), .b(new_n18_), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n16_), .O(new_n37_));
  nor2   g22(.a(new_n19_), .b(x2), .O(new_n38_));
  nor2   g23(.a(x1), .b(new_n16_), .O(new_n39_));
  aoi22  g24(.a(new_n39_), .b(new_n38_), .c(new_n31_), .d(x2), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n37_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n24_), .c(x1), .d(new_n16_), .O(new_n43_));
  and2   g28(.a(new_n20_), .b(x5), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n24_), .c(new_n43_), .O(z3));
  nand3  g30(.a(x5), .b(new_n19_), .c(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n25_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n18_), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n18_), .c(x2), .O(new_n49_));
  nand3  g34(.a(x5), .b(x3), .c(x2), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n49_), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n48_), .O(z4));
  aoi21  g38(.a(new_n29_), .b(new_n24_), .c(new_n51_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x0), .c(new_n48_), .O(z5));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n18_), .c(x0), .O(new_n57_));
  oai21  g42(.a(x4), .b(x2), .c(new_n19_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(x1), .c(new_n16_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n57_), .c(new_n32_), .O(z6));
endmodule


