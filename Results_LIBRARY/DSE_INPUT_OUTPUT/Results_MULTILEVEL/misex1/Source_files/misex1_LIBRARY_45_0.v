// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(new_n17_), .c(x2), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n16_), .O(new_n20_));
  nand4  g05(.a(new_n18_), .b(x2), .c(new_n17_), .d(x0), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n20_), .O(z0));
  aoi21  g07(.a(x3), .b(new_n17_), .c(new_n16_), .O(new_n23_));
  oai21  g08(.a(x5), .b(x3), .c(new_n17_), .O(new_n24_));
  oai22  g09(.a(new_n24_), .b(x0), .c(new_n23_), .d(x2), .O(z1));
  inv1   g10(.a(x2), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  aoi22  g12(.a(new_n27_), .b(new_n16_), .c(x3), .d(new_n26_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(new_n20_), .O(z2));
  nand2  g14(.a(x5), .b(new_n16_), .O(new_n30_));
  nand4  g15(.a(new_n30_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(z3));
  oai21  g17(.a(x3), .b(new_n26_), .c(new_n16_), .O(new_n33_));
  xor2a  g18(.a(x3), .b(x2), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n17_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(z4));
  inv1   g21(.a(x5), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(x0), .O(new_n38_));
  aoi21  g23(.a(new_n34_), .b(x0), .c(new_n38_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(x1), .c(new_n33_), .O(z5));
  aoi22  g25(.a(new_n34_), .b(x0), .c(new_n27_), .d(x2), .O(new_n41_));
  nand4  g26(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n42_));
  oai21  g27(.a(new_n41_), .b(x1), .c(new_n42_), .O(z6));
endmodule


