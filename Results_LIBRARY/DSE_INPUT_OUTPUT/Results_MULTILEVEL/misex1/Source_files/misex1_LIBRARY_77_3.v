// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand2  g02(.a(new_n16_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n17_), .b(x2), .O(new_n19_));
  oai22  g04(.a(new_n19_), .b(new_n18_), .c(new_n17_), .d(new_n16_), .O(z0));
  inv1   g05(.a(x0), .O(new_n21_));
  oai21  g06(.a(x5), .b(x3), .c(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n16_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(new_n17_), .c(new_n23_), .O(new_n26_));
  oai21  g11(.a(new_n22_), .b(x1), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n21_), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n23_), .O(new_n29_));
  or2    g14(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n28_), .O(z1));
  nand3  g16(.a(x4), .b(new_n23_), .c(new_n21_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x1), .O(new_n34_));
  inv1   g19(.a(x5), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x2), .O(new_n36_));
  nand2  g21(.a(new_n24_), .b(new_n23_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n36_), .c(x3), .O(new_n38_));
  nand3  g23(.a(x3), .b(new_n23_), .c(x0), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  aoi21  g25(.a(new_n38_), .b(new_n21_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x1), .c(new_n34_), .O(z2));
  nand2  g27(.a(x7), .b(x4), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n23_), .c(x1), .d(new_n21_), .O(new_n44_));
  nand2  g29(.a(x5), .b(new_n21_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x2), .c(new_n16_), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n44_), .c(x3), .O(z3));
  nor2   g32(.a(new_n23_), .b(x1), .O(new_n48_));
  nor2   g33(.a(x3), .b(x2), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n48_), .c(new_n21_), .O(new_n50_));
  nand2  g35(.a(new_n39_), .b(new_n19_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z4));
  aoi21  g38(.a(new_n29_), .b(new_n19_), .c(new_n21_), .O(new_n54_));
  nor2   g39(.a(new_n22_), .b(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n26_), .b(x0), .c(new_n56_), .O(z5));
  nor2   g42(.a(x5), .b(x3), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(x2), .c(new_n54_), .O(new_n59_));
  nor2   g44(.a(x4), .b(x3), .O(new_n60_));
  nand4  g45(.a(new_n60_), .b(new_n23_), .c(x1), .d(new_n21_), .O(new_n61_));
  oai21  g46(.a(new_n59_), .b(x1), .c(new_n61_), .O(z6));
endmodule


