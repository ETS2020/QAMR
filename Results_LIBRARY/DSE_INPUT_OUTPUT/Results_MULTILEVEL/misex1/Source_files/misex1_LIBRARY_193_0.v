// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nor2   g03(.a(x1), .b(new_n18_), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n16_), .c(new_n17_), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x0), .O(new_n22_));
  nand2  g07(.a(x3), .b(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n20_), .O(z0));
  oai21  g11(.a(x5), .b(x3), .c(x2), .O(new_n27_));
  nand2  g12(.a(new_n16_), .b(x1), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(x1), .c(new_n28_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  oai21  g15(.a(new_n19_), .b(new_n17_), .c(new_n16_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(z1));
  aoi21  g17(.a(new_n17_), .b(x2), .c(new_n21_), .O(new_n33_));
  nor3   g18(.a(x5), .b(x3), .c(x1), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n33_), .c(new_n18_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n31_), .O(z2));
  nand2  g21(.a(x5), .b(new_n18_), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(new_n17_), .c(x2), .d(new_n21_), .O(new_n38_));
  nor2   g23(.a(x7), .b(new_n17_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n22_), .c(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(z3));
  nor2   g26(.a(x2), .b(x1), .O(new_n42_));
  xor2a  g27(.a(new_n42_), .b(x0), .O(new_n43_));
  nand3  g28(.a(new_n17_), .b(x2), .c(new_n21_), .O(new_n44_));
  oai21  g29(.a(new_n43_), .b(new_n17_), .c(new_n44_), .O(z4));
  nand2  g30(.a(new_n23_), .b(x0), .O(new_n46_));
  nand3  g31(.a(x5), .b(x2), .c(new_n18_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n21_), .O(new_n49_));
  oai21  g34(.a(new_n33_), .b(new_n24_), .c(new_n18_), .O(new_n50_));
  nand2  g35(.a(new_n17_), .b(new_n16_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(z5));
  inv1   g37(.a(x5), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n17_), .O(new_n54_));
  nand2  g39(.a(new_n46_), .b(new_n54_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n21_), .O(new_n56_));
  nand2  g41(.a(new_n33_), .b(new_n18_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n51_), .O(z6));
endmodule


