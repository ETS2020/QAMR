// Benchmark "FAU" written by ABC on Fri Aug 14 01:59:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_;
  nand2  g00(.a(x3), .b(x2), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x4), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x1), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n20_), .c(new_n18_), .O(z0));
  oai21  g08(.a(x5), .b(x3), .c(x2), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n21_), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  nand3  g12(.a(x4), .b(new_n25_), .c(x1), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  aoi21  g14(.a(new_n27_), .b(new_n19_), .c(new_n29_), .O(new_n30_));
  nand2  g15(.a(x3), .b(new_n25_), .O(new_n31_));
  or2    g16(.a(new_n31_), .b(new_n20_), .O(new_n32_));
  oai21  g17(.a(new_n30_), .b(x0), .c(new_n32_), .O(z1));
  nor2   g18(.a(new_n19_), .b(x0), .O(new_n34_));
  nand3  g19(.a(new_n25_), .b(new_n19_), .c(x0), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n34_), .c(x3), .O(new_n37_));
  oai21  g22(.a(x2), .b(x0), .c(x4), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x1), .O(new_n39_));
  inv1   g24(.a(x0), .O(new_n40_));
  inv1   g25(.a(x5), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x2), .O(new_n42_));
  oai21  g27(.a(x6), .b(x2), .c(new_n42_), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n21_), .c(new_n19_), .d(new_n40_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n39_), .c(new_n37_), .O(z2));
  nand2  g30(.a(x5), .b(new_n40_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n21_), .c(x2), .d(new_n19_), .O(new_n47_));
  inv1   g32(.a(x7), .O(new_n48_));
  nand4  g33(.a(new_n34_), .b(new_n48_), .c(x4), .d(new_n25_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  oai21  g36(.a(x5), .b(x3), .c(x0), .O(new_n52_));
  aoi22  g37(.a(new_n52_), .b(x2), .c(new_n51_), .d(x0), .O(new_n53_));
  oai21  g38(.a(x2), .b(x0), .c(x4), .O(new_n54_));
  xnor2a g39(.a(x3), .b(x2), .O(new_n55_));
  aoi22  g40(.a(new_n55_), .b(new_n40_), .c(new_n54_), .d(x1), .O(new_n56_));
  oai21  g41(.a(new_n53_), .b(x1), .c(new_n56_), .O(z4));
  aoi21  g42(.a(new_n31_), .b(new_n22_), .c(new_n40_), .O(new_n58_));
  aoi21  g43(.a(new_n26_), .b(new_n24_), .c(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n58_), .c(new_n19_), .O(new_n60_));
  nand4  g45(.a(new_n22_), .b(x4), .c(x1), .d(new_n40_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n60_), .O(z5));
  nor2   g47(.a(x5), .b(x3), .O(new_n63_));
  aoi22  g48(.a(new_n51_), .b(x0), .c(new_n63_), .d(x2), .O(new_n64_));
  nand3  g49(.a(new_n34_), .b(x4), .c(x3), .O(new_n65_));
  oai21  g50(.a(new_n64_), .b(x1), .c(new_n65_), .O(z6));
endmodule


