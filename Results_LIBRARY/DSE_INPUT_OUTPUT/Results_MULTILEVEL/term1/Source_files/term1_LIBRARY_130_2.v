// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_;
  inv1   g000(.a(x01), .O(new_n45_));
  nor2   g001(.a(x30), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  nor2   g004(.a(x32), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nor2   g006(.a(x33), .b(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nor2   g009(.a(x32), .b(new_n50_), .O(new_n54_));
  nor2   g010(.a(x33), .b(x03), .O(new_n55_));
  oai21  g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n46_), .O(z1));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  nor2   g014(.a(new_n50_), .b(new_n53_), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n50_), .b(new_n53_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x30), .c(x01), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n50_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n53_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n53_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n50_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n58_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xor2a  g027(.a(x03), .b(x02), .O(new_n72_));
  nand3  g028(.a(new_n72_), .b(x30), .c(x01), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(new_n74_));
  nand2  g030(.a(new_n67_), .b(new_n65_), .O(new_n75_));
  nand4  g031(.a(x06), .b(x05), .c(new_n50_), .d(new_n53_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(new_n78_));
  aoi21  g034(.a(new_n78_), .b(new_n70_), .c(x09), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(x08), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z2));
  inv1   g037(.a(x26), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  inv1   g039(.a(x15), .O(new_n84_));
  inv1   g040(.a(x16), .O(new_n85_));
  inv1   g041(.a(x20), .O(new_n86_));
  inv1   g042(.a(x21), .O(new_n87_));
  aoi22  g043(.a(new_n87_), .b(new_n85_), .c(new_n86_), .d(new_n84_), .O(new_n88_));
  inv1   g044(.a(x17), .O(new_n89_));
  inv1   g045(.a(x22), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g047(.a(x18), .O(new_n92_));
  inv1   g048(.a(x23), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  inv1   g050(.a(x19), .O(new_n95_));
  inv1   g051(.a(x24), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n94_), .c(new_n91_), .d(new_n88_), .O(new_n98_));
  and2   g054(.a(new_n98_), .b(new_n60_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n83_), .c(new_n82_), .d(x25), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(x30), .c(new_n45_), .O(z3));
  xor2a  g057(.a(x28), .b(x27), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n99_), .c(x30), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n82_), .c(x25), .d(x01), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(z4));
  inv1   g062(.a(x29), .O(new_n107_));
  inv1   g063(.a(x28), .O(new_n108_));
  nor2   g064(.a(new_n108_), .b(new_n83_), .O(new_n109_));
  nand3  g065(.a(new_n107_), .b(x28), .c(x27), .O(new_n110_));
  oai21  g066(.a(new_n109_), .b(new_n107_), .c(new_n110_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n98_), .c(new_n60_), .d(x30), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n82_), .c(x25), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(x30), .c(new_n45_), .O(z5));
  inv1   g071(.a(new_n109_), .O(new_n116_));
  nand2  g072(.a(new_n94_), .b(new_n91_), .O(new_n117_));
  nand2  g073(.a(x21), .b(new_n85_), .O(new_n118_));
  oai21  g074(.a(new_n117_), .b(new_n85_), .c(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n96_), .c(new_n95_), .O(new_n120_));
  nand3  g076(.a(new_n117_), .b(x21), .c(new_n85_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n120_), .c(new_n84_), .O(new_n122_));
  aoi21  g078(.a(x20), .b(new_n84_), .c(x29), .O(new_n123_));
  nand2  g079(.a(new_n97_), .b(new_n94_), .O(new_n124_));
  oai21  g080(.a(x21), .b(x16), .c(new_n91_), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g082(.a(x29), .b(new_n86_), .c(new_n84_), .O(new_n127_));
  oai21  g083(.a(new_n126_), .b(new_n123_), .c(new_n127_), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n122_), .c(new_n116_), .O(new_n129_));
  nand2  g085(.a(new_n98_), .b(new_n107_), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n129_), .c(new_n59_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(x30), .c(new_n82_), .d(x25), .O(new_n132_));
  nor2   g088(.a(new_n132_), .b(new_n45_), .O(z6));
  nand3  g089(.a(x29), .b(x28), .c(x27), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x31), .O(new_n135_));
  nand4  g091(.a(new_n97_), .b(new_n94_), .c(new_n91_), .d(new_n88_), .O(new_n136_));
  nand3  g092(.a(new_n60_), .b(x25), .c(x01), .O(new_n137_));
  inv1   g093(.a(x31), .O(new_n138_));
  nand3  g094(.a(new_n109_), .b(new_n138_), .c(x29), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x30), .c(new_n82_), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n136_), .c(new_n135_), .O(z7));
  nand2  g098(.a(new_n108_), .b(new_n83_), .O(new_n143_));
  inv1   g099(.a(x30), .O(new_n144_));
  inv1   g100(.a(x32), .O(new_n145_));
  nand3  g101(.a(x19), .b(new_n92_), .c(x13), .O(new_n146_));
  nand3  g102(.a(new_n95_), .b(x18), .c(x14), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n146_), .c(new_n89_), .O(new_n148_));
  nand4  g104(.a(x19), .b(x18), .c(new_n89_), .d(x12), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n148_), .c(x16), .O(new_n151_));
  nand3  g107(.a(x19), .b(x18), .c(x17), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n85_), .c(x11), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n151_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x15), .O(new_n156_));
  nand4  g112(.a(new_n153_), .b(x16), .c(new_n84_), .d(x10), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n145_), .c(x31), .d(new_n144_), .O(new_n159_));
  nand2  g115(.a(x32), .b(x30), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(new_n143_), .c(x29), .O(new_n162_));
  nand2  g118(.a(new_n143_), .b(x29), .O(new_n163_));
  nand4  g119(.a(new_n158_), .b(new_n145_), .c(x31), .d(x30), .O(new_n164_));
  nand2  g120(.a(x32), .b(new_n144_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n163_), .O(new_n167_));
  nand2  g123(.a(new_n95_), .b(x14), .O(new_n168_));
  nand4  g124(.a(new_n168_), .b(x18), .c(x17), .d(x16), .O(new_n169_));
  nor2   g125(.a(new_n169_), .b(new_n84_), .O(new_n170_));
  nand2  g126(.a(x16), .b(x15), .O(new_n171_));
  nand2  g127(.a(x18), .b(x17), .O(new_n172_));
  oai21  g128(.a(new_n172_), .b(new_n171_), .c(new_n95_), .O(new_n173_));
  nand2  g129(.a(x16), .b(x10), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n172_), .c(new_n84_), .O(new_n175_));
  inv1   g131(.a(x11), .O(new_n176_));
  oai21  g132(.a(new_n172_), .b(new_n176_), .c(new_n85_), .O(new_n177_));
  aoi21  g133(.a(x18), .b(x12), .c(x17), .O(new_n178_));
  nor2   g134(.a(x18), .b(x13), .O(new_n179_));
  nor3   g135(.a(new_n179_), .b(new_n178_), .c(new_n138_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n177_), .c(new_n175_), .d(new_n173_), .O(new_n181_));
  oai21  g137(.a(new_n181_), .b(new_n170_), .c(x32), .O(new_n182_));
  nand3  g138(.a(new_n182_), .b(new_n167_), .c(new_n162_), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(new_n82_), .c(x00), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n47_), .O(z8));
  inv1   g141(.a(x33), .O(new_n186_));
  nand4  g142(.a(new_n158_), .b(new_n186_), .c(x31), .d(x30), .O(new_n187_));
  nand2  g143(.a(x33), .b(new_n144_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n143_), .c(x29), .O(new_n190_));
  nand4  g146(.a(new_n158_), .b(new_n186_), .c(new_n138_), .d(new_n144_), .O(new_n191_));
  nand2  g147(.a(x33), .b(x30), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(new_n191_), .O(new_n193_));
  nand2  g149(.a(new_n193_), .b(new_n163_), .O(new_n194_));
  inv1   g150(.a(new_n178_), .O(new_n195_));
  inv1   g151(.a(new_n179_), .O(new_n196_));
  xnor2a g152(.a(x31), .b(x30), .O(new_n197_));
  nand3  g153(.a(new_n197_), .b(new_n196_), .c(new_n195_), .O(new_n198_));
  inv1   g154(.a(new_n198_), .O(new_n199_));
  nand4  g155(.a(new_n199_), .b(new_n177_), .c(new_n175_), .d(new_n173_), .O(new_n200_));
  oai21  g156(.a(new_n200_), .b(new_n170_), .c(x33), .O(new_n201_));
  nand3  g157(.a(new_n201_), .b(new_n194_), .c(new_n190_), .O(new_n202_));
  nand3  g158(.a(new_n202_), .b(new_n82_), .c(x00), .O(new_n203_));
  nand2  g159(.a(new_n203_), .b(new_n47_), .O(z9));
endmodule


