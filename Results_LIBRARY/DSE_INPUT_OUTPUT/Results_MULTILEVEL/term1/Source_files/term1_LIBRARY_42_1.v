// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x15), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(new_n46_), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n48_), .c(new_n49_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g012(.a(x03), .b(x02), .O(new_n57_));
  oai21  g013(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n52_), .c(new_n48_), .O(z1));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  inv1   g017(.a(new_n56_), .O(new_n62_));
  inv1   g018(.a(new_n57_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n55_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n55_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n60_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  nor2   g027(.a(new_n50_), .b(new_n61_), .O(new_n72_));
  nand2  g028(.a(new_n67_), .b(new_n65_), .O(new_n73_));
  nand3  g029(.a(new_n57_), .b(x06), .c(x05), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n71_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n70_), .c(x09), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(x08), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n48_), .O(z2));
  inv1   g035(.a(x26), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  inv1   g037(.a(x16), .O(new_n82_));
  inv1   g038(.a(x21), .O(new_n83_));
  nor2   g039(.a(x22), .b(x17), .O(new_n84_));
  aoi21  g040(.a(new_n83_), .b(new_n82_), .c(new_n84_), .O(new_n85_));
  nor2   g041(.a(x23), .b(x18), .O(new_n86_));
  nor2   g042(.a(x24), .b(x19), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nor3   g045(.a(x30), .b(x20), .c(x15), .O(new_n90_));
  aoi21  g046(.a(new_n89_), .b(new_n48_), .c(new_n90_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n56_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n81_), .c(new_n80_), .d(x25), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n61_), .O(z3));
  inv1   g050(.a(x15), .O(new_n95_));
  nand3  g051(.a(new_n45_), .b(x28), .c(new_n81_), .O(new_n96_));
  oai21  g052(.a(x28), .b(new_n81_), .c(new_n96_), .O(new_n97_));
  inv1   g053(.a(new_n84_), .O(new_n98_));
  inv1   g054(.a(x20), .O(new_n99_));
  aoi22  g055(.a(new_n83_), .b(new_n82_), .c(new_n99_), .d(new_n95_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n88_), .c(new_n98_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n97_), .c(new_n62_), .O(new_n102_));
  nand4  g058(.a(new_n89_), .b(new_n62_), .c(x28), .d(new_n81_), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n95_), .c(new_n102_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n80_), .c(x25), .d(x01), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n48_), .O(z4));
  inv1   g062(.a(x28), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n81_), .c(x29), .O(new_n108_));
  inv1   g064(.a(x29), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x28), .c(x27), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n108_), .c(new_n91_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n62_), .c(new_n80_), .d(x25), .O(new_n112_));
  nor2   g068(.a(new_n112_), .b(new_n61_), .O(z5));
  nor2   g069(.a(new_n107_), .b(new_n81_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x29), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x30), .c(x15), .O(new_n116_));
  nor2   g072(.a(x30), .b(new_n109_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n89_), .O(new_n120_));
  nor2   g076(.a(new_n81_), .b(x20), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n117_), .c(x28), .d(new_n95_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n62_), .c(new_n80_), .d(x25), .O(new_n124_));
  nor2   g080(.a(new_n124_), .b(new_n61_), .O(z6));
  nand4  g081(.a(new_n62_), .b(new_n80_), .c(x25), .d(x01), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n48_), .O(new_n127_));
  nand3  g083(.a(x29), .b(x28), .c(x27), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x31), .O(new_n129_));
  inv1   g085(.a(new_n86_), .O(new_n130_));
  inv1   g086(.a(new_n87_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n130_), .c(new_n85_), .O(new_n132_));
  inv1   g088(.a(x31), .O(new_n133_));
  nand4  g089(.a(new_n114_), .b(new_n133_), .c(x30), .d(x29), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n132_), .c(new_n129_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x15), .O(new_n136_));
  oai21  g092(.a(new_n132_), .b(new_n99_), .c(new_n133_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n45_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n136_), .c(new_n127_), .O(z7));
  nor2   g095(.a(x28), .b(x27), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  inv1   g097(.a(x17), .O(new_n142_));
  inv1   g098(.a(x18), .O(new_n143_));
  nand3  g099(.a(x19), .b(new_n143_), .c(x13), .O(new_n144_));
  inv1   g100(.a(x19), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(x18), .c(x14), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n144_), .c(new_n142_), .O(new_n147_));
  nand4  g103(.a(x19), .b(x18), .c(new_n142_), .d(x12), .O(new_n148_));
  inv1   g104(.a(new_n148_), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n147_), .c(x16), .O(new_n150_));
  nand3  g106(.a(x19), .b(x18), .c(x17), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n82_), .c(x11), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n49_), .c(x31), .d(new_n45_), .O(new_n155_));
  nand2  g111(.a(x32), .b(x30), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n155_), .c(new_n95_), .O(new_n157_));
  nand4  g113(.a(x17), .b(x16), .c(new_n95_), .d(x10), .O(new_n158_));
  nand3  g114(.a(new_n49_), .b(x31), .c(new_n45_), .O(new_n159_));
  nor4   g115(.a(new_n159_), .b(new_n158_), .c(new_n145_), .d(new_n143_), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n157_), .c(new_n141_), .O(new_n161_));
  nor2   g117(.a(new_n140_), .b(new_n109_), .O(new_n162_));
  inv1   g118(.a(new_n162_), .O(new_n163_));
  nand4  g119(.a(new_n154_), .b(new_n49_), .c(x31), .d(x30), .O(new_n164_));
  nand2  g120(.a(x32), .b(new_n45_), .O(new_n165_));
  oai21  g121(.a(new_n164_), .b(new_n95_), .c(new_n165_), .O(new_n166_));
  nand2  g122(.a(x18), .b(x17), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n82_), .c(new_n145_), .O(new_n168_));
  inv1   g124(.a(x11), .O(new_n169_));
  oai21  g125(.a(new_n167_), .b(new_n169_), .c(new_n82_), .O(new_n170_));
  nand2  g126(.a(x18), .b(x12), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n142_), .O(new_n172_));
  inv1   g128(.a(x13), .O(new_n173_));
  aoi21  g129(.a(new_n143_), .b(new_n173_), .c(new_n133_), .O(new_n174_));
  nand4  g130(.a(new_n174_), .b(new_n172_), .c(new_n170_), .d(new_n168_), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n48_), .O(new_n176_));
  nand2  g132(.a(new_n145_), .b(x14), .O(new_n177_));
  nand4  g133(.a(new_n177_), .b(x18), .c(x17), .d(x16), .O(new_n178_));
  inv1   g134(.a(new_n178_), .O(new_n179_));
  nand2  g135(.a(x16), .b(x10), .O(new_n180_));
  oai21  g136(.a(new_n180_), .b(new_n151_), .c(new_n45_), .O(new_n181_));
  nor2   g137(.a(new_n181_), .b(x15), .O(new_n182_));
  aoi21  g138(.a(new_n179_), .b(x15), .c(new_n182_), .O(new_n183_));
  aoi21  g139(.a(new_n183_), .b(new_n176_), .c(new_n49_), .O(new_n184_));
  aoi21  g140(.a(new_n166_), .b(new_n163_), .c(new_n184_), .O(new_n185_));
  oai21  g141(.a(new_n161_), .b(new_n109_), .c(new_n185_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n80_), .c(x00), .O(new_n187_));
  inv1   g143(.a(new_n187_), .O(z8));
  inv1   g144(.a(x00), .O(new_n189_));
  nand3  g145(.a(new_n154_), .b(new_n45_), .c(x15), .O(new_n190_));
  nand4  g146(.a(new_n152_), .b(x16), .c(new_n95_), .d(x10), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  nand3  g148(.a(new_n192_), .b(new_n53_), .c(new_n133_), .O(new_n193_));
  nand2  g149(.a(x33), .b(x30), .O(new_n194_));
  aoi21  g150(.a(new_n194_), .b(new_n193_), .c(new_n162_), .O(new_n195_));
  nand4  g151(.a(new_n154_), .b(new_n53_), .c(x31), .d(x30), .O(new_n196_));
  nand2  g152(.a(x33), .b(new_n45_), .O(new_n197_));
  nand2  g153(.a(new_n197_), .b(new_n196_), .O(new_n198_));
  nand3  g154(.a(new_n198_), .b(new_n141_), .c(x29), .O(new_n199_));
  aoi22  g155(.a(new_n45_), .b(new_n95_), .c(new_n145_), .d(x14), .O(new_n200_));
  nand4  g156(.a(new_n200_), .b(x18), .c(x17), .d(x16), .O(new_n201_));
  nand2  g157(.a(x16), .b(x15), .O(new_n202_));
  oai21  g158(.a(new_n202_), .b(new_n167_), .c(new_n145_), .O(new_n203_));
  oai21  g159(.a(new_n180_), .b(new_n167_), .c(new_n95_), .O(new_n204_));
  nand2  g160(.a(new_n143_), .b(new_n173_), .O(new_n205_));
  xnor2a g161(.a(x31), .b(x30), .O(new_n206_));
  nand4  g162(.a(new_n206_), .b(new_n205_), .c(new_n172_), .d(new_n170_), .O(new_n207_));
  inv1   g163(.a(new_n207_), .O(new_n208_));
  nand4  g164(.a(new_n208_), .b(new_n204_), .c(new_n203_), .d(new_n201_), .O(new_n209_));
  nand2  g165(.a(new_n209_), .b(x33), .O(new_n210_));
  nand2  g166(.a(new_n210_), .b(new_n199_), .O(new_n211_));
  oai21  g167(.a(new_n211_), .b(new_n195_), .c(new_n80_), .O(new_n212_));
  oai21  g168(.a(new_n212_), .b(new_n189_), .c(new_n48_), .O(z9));
endmodule


