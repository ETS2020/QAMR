// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_;
  inv1   g000(.a(x28), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  xnor2a g006(.a(x03), .b(x02), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n49_), .c(new_n50_), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g012(.a(x03), .b(x02), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  or2    g014(.a(new_n58_), .b(x33), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(new_n53_), .c(new_n49_), .O(z1));
  inv1   g016(.a(x09), .O(new_n61_));
  xnor2a g017(.a(x07), .b(x04), .O(new_n62_));
  inv1   g018(.a(x01), .O(new_n63_));
  nor2   g019(.a(new_n58_), .b(new_n63_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n55_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n55_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n62_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  nor2   g027(.a(new_n51_), .b(new_n63_), .O(new_n72_));
  nand2  g028(.a(new_n67_), .b(new_n65_), .O(new_n73_));
  nand3  g029(.a(new_n57_), .b(x06), .c(x05), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n71_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n70_), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n49_), .c(new_n61_), .d(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x26), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  inv1   g037(.a(new_n56_), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x17), .O(new_n86_));
  inv1   g042(.a(x22), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(x18), .O(new_n89_));
  inv1   g045(.a(x23), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g047(.a(x19), .O(new_n92_));
  inv1   g048(.a(x24), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n91_), .c(new_n88_), .d(new_n85_), .O(new_n95_));
  and2   g051(.a(new_n95_), .b(new_n82_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n49_), .c(new_n81_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n80_), .c(x25), .d(x01), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z3));
  xor2a  g056(.a(x28), .b(x27), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n96_), .c(new_n80_), .d(x25), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n63_), .c(new_n49_), .O(z4));
  nand3  g059(.a(new_n46_), .b(x28), .c(x27), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(x29), .O(new_n105_));
  inv1   g061(.a(x29), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x28), .c(x27), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n95_), .c(new_n82_), .d(new_n80_), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x25), .c(x01), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n49_), .O(z5));
  inv1   g068(.a(x25), .O(new_n113_));
  nand4  g069(.a(new_n46_), .b(x29), .c(x28), .d(x27), .O(new_n114_));
  oai21  g070(.a(new_n46_), .b(x28), .c(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n96_), .c(new_n80_), .O(new_n116_));
  nor3   g072(.a(new_n116_), .b(new_n113_), .c(new_n63_), .O(z6));
  inv1   g073(.a(new_n94_), .O(new_n118_));
  nand3  g074(.a(new_n91_), .b(new_n88_), .c(new_n85_), .O(new_n119_));
  nand3  g075(.a(new_n82_), .b(x25), .c(x01), .O(new_n120_));
  nor4   g076(.a(new_n120_), .b(new_n47_), .c(x31), .d(x26), .O(new_n121_));
  oai21  g077(.a(new_n119_), .b(new_n118_), .c(new_n121_), .O(z7));
  inv1   g078(.a(x15), .O(new_n123_));
  nand2  g079(.a(new_n92_), .b(x14), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(x18), .c(x17), .d(x16), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand4  g082(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n92_), .O(new_n128_));
  nand2  g084(.a(x18), .b(x17), .O(new_n129_));
  nand2  g085(.a(x16), .b(x10), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n129_), .c(new_n123_), .O(new_n131_));
  inv1   g087(.a(x11), .O(new_n132_));
  inv1   g088(.a(x16), .O(new_n133_));
  oai21  g089(.a(new_n129_), .b(new_n132_), .c(new_n133_), .O(new_n134_));
  nand2  g090(.a(x18), .b(x12), .O(new_n135_));
  inv1   g091(.a(x13), .O(new_n136_));
  nand2  g092(.a(new_n89_), .b(new_n136_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x31), .O(new_n138_));
  aoi21  g094(.a(new_n135_), .b(new_n86_), .c(new_n138_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n134_), .c(new_n131_), .d(new_n128_), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n126_), .c(new_n49_), .O(new_n141_));
  nand3  g097(.a(x30), .b(x29), .c(x27), .O(new_n142_));
  oai21  g098(.a(x30), .b(x27), .c(new_n142_), .O(new_n143_));
  aoi22  g099(.a(new_n143_), .b(new_n45_), .c(new_n46_), .d(new_n106_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x32), .O(new_n146_));
  nor2   g102(.a(new_n106_), .b(new_n81_), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x30), .c(new_n45_), .O(new_n149_));
  nand2  g105(.a(new_n45_), .b(new_n81_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n46_), .c(x29), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand3  g108(.a(x19), .b(new_n89_), .c(x13), .O(new_n153_));
  nand3  g109(.a(new_n92_), .b(x18), .c(x14), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n153_), .c(new_n86_), .O(new_n155_));
  nand4  g111(.a(x19), .b(x18), .c(new_n86_), .d(x12), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n155_), .c(x16), .O(new_n158_));
  nand3  g114(.a(x19), .b(x18), .c(x17), .O(new_n159_));
  inv1   g115(.a(new_n159_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n133_), .c(x11), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n158_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(x15), .O(new_n163_));
  nand3  g119(.a(x16), .b(new_n123_), .c(x10), .O(new_n164_));
  nor2   g120(.a(new_n164_), .b(new_n159_), .O(new_n165_));
  inv1   g121(.a(new_n165_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n163_), .O(new_n167_));
  nand4  g123(.a(new_n167_), .b(new_n152_), .c(new_n50_), .d(x31), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n146_), .O(new_n169_));
  nand3  g125(.a(new_n169_), .b(new_n80_), .c(x00), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(z8));
  nand4  g127(.a(x33), .b(x29), .c(new_n80_), .d(x00), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n46_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(x28), .O(new_n174_));
  aoi21  g130(.a(new_n161_), .b(new_n158_), .c(new_n123_), .O(new_n175_));
  aoi21  g131(.a(new_n150_), .b(x29), .c(x31), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n46_), .O(new_n177_));
  nand3  g133(.a(new_n147_), .b(x31), .c(x30), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  oai21  g135(.a(new_n165_), .b(new_n175_), .c(new_n179_), .O(new_n180_));
  oai21  g136(.a(new_n147_), .b(x31), .c(new_n46_), .O(new_n181_));
  nand3  g137(.a(x31), .b(x29), .c(x27), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(x30), .O(new_n183_));
  nand2  g139(.a(new_n135_), .b(new_n86_), .O(new_n184_));
  nand4  g140(.a(new_n137_), .b(new_n184_), .c(new_n134_), .d(new_n131_), .O(new_n185_));
  aoi21  g141(.a(new_n127_), .b(new_n92_), .c(new_n185_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n183_), .c(new_n181_), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n126_), .c(x33), .O(new_n188_));
  oai21  g144(.a(new_n180_), .b(x33), .c(new_n188_), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n80_), .c(x00), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(new_n174_), .O(z9));
endmodule


