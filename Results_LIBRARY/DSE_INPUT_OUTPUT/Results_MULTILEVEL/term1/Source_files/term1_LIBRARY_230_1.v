// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_;
  inv1   g000(.a(x29), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x26), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(new_n46_), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n48_), .c(new_n49_), .O(new_n52_));
  nand2  g008(.a(x03), .b(x02), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  nor2   g010(.a(x03), .b(x02), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  or2    g012(.a(new_n56_), .b(x33), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n52_), .c(new_n48_), .O(z1));
  inv1   g014(.a(x09), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  inv1   g018(.a(x02), .O(new_n63_));
  inv1   g019(.a(x03), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n64_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n63_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n63_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n64_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n62_), .c(new_n60_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  nor2   g027(.a(new_n50_), .b(new_n61_), .O(new_n72_));
  nand2  g028(.a(new_n67_), .b(new_n65_), .O(new_n73_));
  nand3  g029(.a(new_n55_), .b(x06), .c(x05), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n71_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n70_), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n48_), .c(new_n59_), .d(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x27), .O(new_n80_));
  nor2   g036(.a(x20), .b(x15), .O(new_n81_));
  nor2   g037(.a(x21), .b(x16), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g039(.a(x17), .O(new_n84_));
  inv1   g040(.a(x22), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x18), .O(new_n87_));
  inv1   g043(.a(x23), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g045(.a(x19), .O(new_n90_));
  inv1   g046(.a(x24), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n89_), .c(new_n86_), .d(new_n83_), .O(new_n93_));
  and2   g049(.a(new_n93_), .b(new_n53_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n80_), .c(x25), .d(x01), .O(new_n95_));
  aoi21  g051(.a(new_n95_), .b(new_n45_), .c(x26), .O(z3));
  inv1   g052(.a(x26), .O(new_n97_));
  xor2a  g053(.a(x28), .b(x27), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n94_), .c(new_n45_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n97_), .c(x25), .d(x01), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(z4));
  nand3  g058(.a(new_n94_), .b(new_n45_), .c(x28), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(x27), .c(new_n97_), .d(x25), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n61_), .O(z5));
  nand3  g062(.a(new_n94_), .b(x30), .c(new_n45_), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n97_), .c(x25), .d(x01), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(z6));
  nand3  g066(.a(new_n89_), .b(new_n86_), .c(new_n83_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  inv1   g068(.a(x31), .O(new_n113_));
  nand4  g069(.a(new_n53_), .b(new_n113_), .c(x25), .d(x01), .O(new_n114_));
  aoi21  g070(.a(new_n112_), .b(new_n92_), .c(new_n114_), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(x29), .c(new_n97_), .O(z7));
  inv1   g072(.a(x15), .O(new_n117_));
  nand2  g073(.a(x32), .b(x18), .O(new_n118_));
  nand2  g074(.a(new_n87_), .b(x13), .O(new_n119_));
  nand3  g075(.a(new_n49_), .b(x31), .c(x30), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x19), .O(new_n122_));
  inv1   g078(.a(x14), .O(new_n123_));
  nand2  g079(.a(x32), .b(new_n123_), .O(new_n124_));
  nand2  g080(.a(new_n90_), .b(x14), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n120_), .c(new_n124_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x18), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n122_), .c(new_n84_), .O(new_n128_));
  nand3  g084(.a(x18), .b(new_n84_), .c(x12), .O(new_n129_));
  nand4  g085(.a(new_n49_), .b(x31), .c(x30), .d(x19), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  inv1   g088(.a(x16), .O(new_n133_));
  nand2  g089(.a(x18), .b(x17), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n133_), .c(x11), .O(new_n136_));
  or2    g092(.a(new_n136_), .b(new_n130_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n132_), .c(new_n117_), .O(new_n138_));
  nand3  g094(.a(new_n135_), .b(x16), .c(x15), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n90_), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  nand2  g097(.a(x16), .b(x10), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n134_), .c(new_n117_), .O(new_n143_));
  inv1   g099(.a(x11), .O(new_n144_));
  oai21  g100(.a(new_n134_), .b(new_n144_), .c(new_n133_), .O(new_n145_));
  nand2  g101(.a(x18), .b(x12), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n84_), .O(new_n147_));
  inv1   g103(.a(x13), .O(new_n148_));
  nand2  g104(.a(x31), .b(x30), .O(new_n149_));
  aoi21  g105(.a(new_n87_), .b(new_n148_), .c(new_n149_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n147_), .c(new_n145_), .d(new_n143_), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n141_), .c(x32), .O(new_n152_));
  nand4  g108(.a(x17), .b(x16), .c(new_n117_), .d(x10), .O(new_n153_));
  inv1   g109(.a(new_n120_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x19), .c(x18), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n153_), .c(new_n152_), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n138_), .c(x00), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n45_), .c(x26), .O(z8));
  nand2  g114(.a(x33), .b(x18), .O(new_n159_));
  inv1   g115(.a(x30), .O(new_n160_));
  nor2   g116(.a(x33), .b(x31), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n160_), .c(new_n87_), .d(x13), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n159_), .c(new_n90_), .O(new_n163_));
  nand2  g119(.a(x33), .b(new_n123_), .O(new_n164_));
  nand4  g120(.a(new_n161_), .b(new_n160_), .c(new_n90_), .d(x14), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n164_), .c(new_n87_), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n163_), .c(x17), .O(new_n167_));
  nand3  g123(.a(new_n161_), .b(new_n160_), .c(x19), .O(new_n168_));
  or2    g124(.a(new_n168_), .b(new_n129_), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n167_), .c(new_n133_), .O(new_n170_));
  nor2   g126(.a(new_n168_), .b(new_n136_), .O(new_n171_));
  oai21  g127(.a(new_n171_), .b(new_n170_), .c(x15), .O(new_n172_));
  nand2  g128(.a(new_n87_), .b(new_n148_), .O(new_n173_));
  nand4  g129(.a(new_n173_), .b(new_n147_), .c(new_n113_), .d(new_n160_), .O(new_n174_));
  inv1   g130(.a(new_n174_), .O(new_n175_));
  nand4  g131(.a(new_n175_), .b(new_n145_), .c(new_n143_), .d(new_n140_), .O(new_n176_));
  nand4  g132(.a(new_n161_), .b(new_n160_), .c(x19), .d(x18), .O(new_n177_));
  nor2   g133(.a(new_n177_), .b(new_n153_), .O(new_n178_));
  aoi21  g134(.a(new_n176_), .b(x33), .c(new_n178_), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n172_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n45_), .c(new_n97_), .d(x00), .O(new_n181_));
  inv1   g137(.a(new_n181_), .O(z9));
endmodule


