// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x10), .O(new_n45_));
  nor2   g001(.a(x26), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(new_n46_), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n48_), .c(new_n49_), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g011(.a(x03), .b(x02), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  or2    g013(.a(new_n57_), .b(x33), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n52_), .c(new_n48_), .O(z1));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n57_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n54_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n53_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n53_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n54_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n60_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  nor2   g025(.a(new_n50_), .b(new_n61_), .O(new_n70_));
  nand2  g026(.a(new_n65_), .b(new_n63_), .O(new_n71_));
  nand3  g027(.a(new_n56_), .b(x06), .c(x05), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n69_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n68_), .c(x09), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n48_), .O(z2));
  inv1   g033(.a(x27), .O(new_n78_));
  nor2   g034(.a(x20), .b(x15), .O(new_n79_));
  nor2   g035(.a(x21), .b(x16), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nor2   g037(.a(x22), .b(x17), .O(new_n82_));
  nor2   g038(.a(x23), .b(x18), .O(new_n83_));
  nor2   g039(.a(x24), .b(x19), .O(new_n84_));
  nor3   g040(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  and2   g041(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n55_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n78_), .c(x25), .d(x01), .O(new_n88_));
  aoi21  g044(.a(new_n88_), .b(new_n45_), .c(x26), .O(z3));
  xor2a  g045(.a(x28), .b(x27), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n87_), .c(x25), .d(x01), .O(new_n91_));
  aoi21  g047(.a(new_n91_), .b(new_n45_), .c(x26), .O(z4));
  inv1   g048(.a(new_n55_), .O(new_n93_));
  inv1   g049(.a(x28), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n78_), .c(x29), .O(new_n95_));
  inv1   g051(.a(x29), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x28), .c(x27), .O(new_n97_));
  aoi21  g053(.a(new_n97_), .b(new_n95_), .c(new_n86_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n93_), .c(x25), .d(x01), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(new_n45_), .c(x26), .O(z5));
  nand3  g056(.a(x29), .b(x28), .c(x27), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x30), .O(new_n102_));
  inv1   g058(.a(x30), .O(new_n103_));
  nor2   g059(.a(new_n94_), .b(new_n78_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(x29), .O(new_n105_));
  aoi21  g061(.a(new_n105_), .b(new_n102_), .c(new_n86_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n93_), .c(x25), .d(x01), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n45_), .c(x26), .O(z6));
  inv1   g064(.a(x26), .O(new_n109_));
  nand3  g065(.a(new_n104_), .b(x30), .c(x29), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x31), .O(new_n111_));
  inv1   g067(.a(new_n82_), .O(new_n112_));
  inv1   g068(.a(new_n83_), .O(new_n113_));
  inv1   g069(.a(new_n84_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(new_n81_), .O(new_n115_));
  nor2   g071(.a(new_n55_), .b(new_n61_), .O(new_n116_));
  inv1   g072(.a(x25), .O(new_n117_));
  inv1   g073(.a(x31), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x30), .c(x29), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n104_), .c(new_n117_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n116_), .c(new_n115_), .d(new_n111_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n45_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n109_), .O(z7));
  nand2  g080(.a(new_n94_), .b(new_n78_), .O(new_n125_));
  inv1   g081(.a(x15), .O(new_n126_));
  inv1   g082(.a(x17), .O(new_n127_));
  inv1   g083(.a(x18), .O(new_n128_));
  nand3  g084(.a(x19), .b(new_n128_), .c(x13), .O(new_n129_));
  inv1   g085(.a(x19), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x18), .c(x14), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n129_), .c(new_n127_), .O(new_n132_));
  nand4  g088(.a(x19), .b(x18), .c(new_n127_), .d(x12), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  inv1   g091(.a(x16), .O(new_n136_));
  nor2   g092(.a(new_n130_), .b(new_n128_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(x17), .c(new_n136_), .d(x11), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n49_), .c(x31), .d(new_n103_), .O(new_n140_));
  nand2  g096(.a(x32), .b(x30), .O(new_n141_));
  oai21  g097(.a(new_n140_), .b(new_n126_), .c(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n125_), .c(x29), .O(new_n143_));
  nand2  g099(.a(new_n125_), .b(x29), .O(new_n144_));
  nand4  g100(.a(new_n139_), .b(new_n49_), .c(x31), .d(x30), .O(new_n145_));
  nand2  g101(.a(x32), .b(new_n103_), .O(new_n146_));
  oai21  g102(.a(new_n145_), .b(new_n126_), .c(new_n146_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g104(.a(new_n130_), .b(x14), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(x18), .c(x17), .d(x16), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  nand2  g107(.a(x18), .b(x17), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n136_), .c(new_n130_), .O(new_n153_));
  inv1   g109(.a(x11), .O(new_n154_));
  oai21  g110(.a(new_n152_), .b(new_n154_), .c(new_n136_), .O(new_n155_));
  nand2  g111(.a(x18), .b(x12), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n127_), .O(new_n157_));
  nor2   g113(.a(x18), .b(x13), .O(new_n158_));
  nor3   g114(.a(new_n158_), .b(new_n118_), .c(new_n126_), .O(new_n159_));
  nand4  g115(.a(new_n159_), .b(new_n157_), .c(new_n155_), .d(new_n153_), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n151_), .c(x32), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(new_n148_), .c(new_n143_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(x00), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n45_), .c(x26), .O(z8));
  aoi21  g120(.a(new_n138_), .b(new_n135_), .c(x33), .O(new_n165_));
  nand4  g121(.a(new_n165_), .b(x31), .c(x30), .d(x15), .O(new_n166_));
  nand2  g122(.a(x33), .b(new_n103_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(new_n125_), .c(x29), .O(new_n169_));
  nand4  g125(.a(new_n165_), .b(new_n118_), .c(new_n103_), .d(x15), .O(new_n170_));
  nand2  g126(.a(x33), .b(x30), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n144_), .O(new_n173_));
  xor2a  g129(.a(x31), .b(x30), .O(new_n174_));
  nor3   g130(.a(new_n174_), .b(new_n158_), .c(new_n126_), .O(new_n175_));
  nand4  g131(.a(new_n175_), .b(new_n157_), .c(new_n155_), .d(new_n153_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n151_), .c(x33), .O(new_n177_));
  nand3  g133(.a(new_n177_), .b(new_n173_), .c(new_n169_), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(x00), .O(new_n179_));
  aoi21  g135(.a(new_n179_), .b(new_n45_), .c(x26), .O(z9));
endmodule


