// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_;
  inv1   g000(.a(x22), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  xnor2a g006(.a(x03), .b(x02), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n49_), .c(new_n50_), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g012(.a(x03), .b(x02), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  or2    g014(.a(new_n58_), .b(x33), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(new_n53_), .c(new_n49_), .O(z1));
  xnor2a g016(.a(x07), .b(x04), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nor2   g018(.a(new_n58_), .b(new_n62_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n55_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n54_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n54_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n55_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n61_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  nor2   g026(.a(new_n51_), .b(new_n62_), .O(new_n71_));
  nand2  g027(.a(new_n66_), .b(new_n64_), .O(new_n72_));
  nand3  g028(.a(new_n57_), .b(x06), .c(x05), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n70_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n69_), .c(x09), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n49_), .O(z2));
  inv1   g034(.a(x26), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  inv1   g036(.a(x17), .O(new_n81_));
  nor2   g037(.a(x20), .b(x15), .O(new_n82_));
  nor2   g038(.a(x21), .b(x16), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g040(.a(x23), .b(x18), .O(new_n85_));
  nor2   g041(.a(x24), .b(x19), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  aoi22  g044(.a(new_n88_), .b(new_n49_), .c(new_n45_), .d(new_n81_), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n56_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n80_), .c(new_n79_), .d(x25), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n62_), .O(z3));
  inv1   g048(.a(new_n56_), .O(new_n93_));
  inv1   g049(.a(x28), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(x27), .O(new_n95_));
  nor2   g051(.a(x28), .b(new_n80_), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n95_), .c(new_n81_), .O(new_n97_));
  nand3  g053(.a(new_n88_), .b(x28), .c(new_n80_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n45_), .O(new_n100_));
  nand3  g056(.a(new_n46_), .b(x28), .c(new_n80_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n96_), .c(new_n88_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n93_), .c(new_n79_), .d(x25), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n62_), .c(new_n49_), .O(z4));
  nand2  g062(.a(x28), .b(x27), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x29), .O(new_n108_));
  inv1   g064(.a(x29), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x28), .c(x27), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n108_), .c(new_n89_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n93_), .c(new_n79_), .d(x25), .O(new_n112_));
  nor2   g068(.a(new_n112_), .b(new_n62_), .O(z5));
  nand2  g069(.a(new_n108_), .b(x29), .O(new_n114_));
  inv1   g070(.a(new_n83_), .O(new_n115_));
  nor2   g071(.a(new_n82_), .b(new_n81_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n87_), .c(new_n115_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n114_), .c(x30), .O(new_n118_));
  nor2   g074(.a(new_n80_), .b(x17), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n46_), .c(x29), .d(x28), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n45_), .O(new_n122_));
  aoi21  g078(.a(new_n87_), .b(new_n84_), .c(x30), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(x29), .c(x28), .d(x27), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n93_), .c(new_n79_), .d(x25), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n62_), .O(z6));
  inv1   g083(.a(new_n85_), .O(new_n128_));
  inv1   g084(.a(new_n86_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n128_), .c(new_n84_), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n81_), .O(new_n131_));
  nand4  g087(.a(new_n93_), .b(new_n79_), .c(x25), .d(x01), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n131_), .c(new_n49_), .O(new_n133_));
  nand3  g089(.a(x29), .b(x28), .c(x27), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x31), .O(new_n135_));
  inv1   g091(.a(x31), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x30), .c(x29), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n107_), .c(new_n135_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n45_), .O(new_n139_));
  oai21  g095(.a(new_n130_), .b(new_n45_), .c(new_n136_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n46_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n139_), .c(new_n133_), .O(z7));
  nand2  g098(.a(new_n94_), .b(new_n80_), .O(new_n143_));
  inv1   g099(.a(x18), .O(new_n144_));
  nand3  g100(.a(x19), .b(new_n144_), .c(x13), .O(new_n145_));
  inv1   g101(.a(x19), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(x18), .c(x14), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n145_), .c(new_n81_), .O(new_n148_));
  nand4  g104(.a(x19), .b(x18), .c(new_n81_), .d(x12), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n148_), .c(x16), .O(new_n151_));
  inv1   g107(.a(x16), .O(new_n152_));
  nand3  g108(.a(x19), .b(x18), .c(x17), .O(new_n153_));
  inv1   g109(.a(new_n153_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(new_n152_), .c(x11), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x15), .O(new_n157_));
  inv1   g113(.a(x15), .O(new_n158_));
  nand4  g114(.a(new_n154_), .b(x16), .c(new_n158_), .d(x10), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n50_), .c(x31), .d(new_n46_), .O(new_n161_));
  nand2  g117(.a(x32), .b(x30), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  nand3  g119(.a(new_n163_), .b(new_n143_), .c(x29), .O(new_n164_));
  nand2  g120(.a(new_n143_), .b(x29), .O(new_n165_));
  nand4  g121(.a(new_n160_), .b(new_n50_), .c(x31), .d(x30), .O(new_n166_));
  nand2  g122(.a(x32), .b(new_n46_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nand2  g125(.a(new_n146_), .b(x14), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(x18), .c(x17), .d(x16), .O(new_n171_));
  nor2   g127(.a(new_n171_), .b(new_n158_), .O(new_n172_));
  nand2  g128(.a(x16), .b(x15), .O(new_n173_));
  nand2  g129(.a(x18), .b(x17), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n173_), .c(new_n146_), .O(new_n175_));
  nand2  g131(.a(x16), .b(x10), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n174_), .c(new_n158_), .O(new_n177_));
  inv1   g133(.a(x11), .O(new_n178_));
  oai21  g134(.a(new_n174_), .b(new_n178_), .c(new_n152_), .O(new_n179_));
  aoi21  g135(.a(x18), .b(x12), .c(x17), .O(new_n180_));
  nor2   g136(.a(x18), .b(x13), .O(new_n181_));
  nor3   g137(.a(new_n181_), .b(new_n180_), .c(new_n136_), .O(new_n182_));
  nand4  g138(.a(new_n182_), .b(new_n179_), .c(new_n177_), .d(new_n175_), .O(new_n183_));
  oai21  g139(.a(new_n183_), .b(new_n172_), .c(x32), .O(new_n184_));
  nand3  g140(.a(new_n184_), .b(new_n169_), .c(new_n164_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n79_), .c(x00), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n49_), .O(z8));
  nor2   g143(.a(new_n181_), .b(new_n180_), .O(new_n188_));
  nand4  g144(.a(new_n188_), .b(new_n179_), .c(new_n177_), .d(new_n175_), .O(new_n189_));
  oai21  g145(.a(new_n189_), .b(new_n172_), .c(new_n49_), .O(new_n190_));
  nand2  g146(.a(new_n165_), .b(new_n136_), .O(new_n191_));
  nor2   g147(.a(new_n136_), .b(new_n109_), .O(new_n192_));
  aoi21  g148(.a(new_n192_), .b(new_n143_), .c(new_n46_), .O(new_n193_));
  aoi22  g149(.a(new_n193_), .b(new_n45_), .c(new_n191_), .d(new_n46_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n190_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(x33), .O(new_n196_));
  nand4  g152(.a(new_n143_), .b(x31), .c(x30), .d(x29), .O(new_n197_));
  oai22  g153(.a(new_n197_), .b(x22), .c(new_n191_), .d(x30), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(new_n160_), .O(new_n199_));
  oai21  g155(.a(new_n199_), .b(x33), .c(new_n196_), .O(new_n200_));
  nand3  g156(.a(new_n200_), .b(new_n79_), .c(x00), .O(new_n201_));
  inv1   g157(.a(new_n201_), .O(z9));
endmodule


