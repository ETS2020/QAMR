// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  nor2   g000(.a(x30), .b(x20), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nor2   g002(.a(x32), .b(x03), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  nor2   g004(.a(x33), .b(new_n48_), .O(new_n49_));
  oai21  g005(.a(new_n49_), .b(new_n47_), .c(x02), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  nor2   g007(.a(x32), .b(new_n48_), .O(new_n52_));
  nor2   g008(.a(x33), .b(x03), .O(new_n53_));
  oai21  g009(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z1));
  inv1   g011(.a(new_n45_), .O(new_n56_));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nor2   g014(.a(new_n48_), .b(new_n51_), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n48_), .b(new_n51_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n48_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n51_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n48_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n57_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  xnor2a g025(.a(x03), .b(x02), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand2  g027(.a(new_n65_), .b(new_n63_), .O(new_n72_));
  nand4  g028(.a(x06), .b(x05), .c(new_n48_), .d(new_n51_), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n68_), .c(x09), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n56_), .O(z2));
  inv1   g034(.a(x26), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  nor2   g036(.a(x21), .b(x16), .O(new_n81_));
  nor2   g037(.a(x22), .b(x17), .O(new_n82_));
  nor2   g038(.a(x23), .b(x18), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nor2   g040(.a(x24), .b(x19), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nor3   g043(.a(new_n87_), .b(new_n82_), .c(new_n81_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n56_), .O(new_n90_));
  inv1   g046(.a(x15), .O(new_n91_));
  inv1   g047(.a(x20), .O(new_n92_));
  nand3  g048(.a(x30), .b(new_n92_), .c(new_n91_), .O(new_n93_));
  aoi21  g049(.a(new_n93_), .b(new_n90_), .c(new_n59_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n80_), .c(new_n79_), .d(x25), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n58_), .O(z3));
  xor2a  g052(.a(x28), .b(x27), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n94_), .c(new_n79_), .d(x25), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n58_), .O(z4));
  inv1   g055(.a(x29), .O(new_n100_));
  inv1   g056(.a(x28), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n80_), .O(new_n102_));
  nand3  g058(.a(new_n100_), .b(x28), .c(x27), .O(new_n103_));
  oai21  g059(.a(new_n102_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  inv1   g060(.a(new_n82_), .O(new_n105_));
  aoi21  g061(.a(new_n92_), .b(new_n91_), .c(new_n81_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n86_), .c(new_n84_), .d(new_n105_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n104_), .c(new_n60_), .d(new_n79_), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x25), .c(x01), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n56_), .O(z5));
  inv1   g067(.a(new_n102_), .O(new_n112_));
  inv1   g068(.a(x30), .O(new_n113_));
  oai22  g069(.a(new_n88_), .b(new_n113_), .c(x20), .d(x15), .O(new_n114_));
  oai21  g070(.a(new_n112_), .b(new_n100_), .c(new_n114_), .O(new_n115_));
  nor2   g071(.a(new_n88_), .b(x30), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(x29), .c(x28), .d(x27), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n60_), .c(new_n79_), .d(x25), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n58_), .c(new_n56_), .O(z6));
  nand3  g076(.a(new_n102_), .b(x30), .c(x29), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x31), .O(new_n122_));
  inv1   g078(.a(new_n81_), .O(new_n123_));
  nand3  g079(.a(new_n93_), .b(new_n105_), .c(new_n123_), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n86_), .c(new_n84_), .O(new_n126_));
  inv1   g082(.a(x25), .O(new_n127_));
  nand3  g083(.a(new_n60_), .b(new_n56_), .c(x01), .O(new_n128_));
  inv1   g084(.a(x31), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x30), .c(x29), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n112_), .c(new_n79_), .O(new_n131_));
  nor3   g087(.a(new_n131_), .b(new_n128_), .c(new_n127_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n126_), .c(new_n122_), .O(z7));
  inv1   g089(.a(x19), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x14), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(x18), .c(x17), .d(x16), .O(new_n136_));
  nor2   g092(.a(new_n136_), .b(new_n91_), .O(new_n137_));
  nand2  g093(.a(x16), .b(x15), .O(new_n138_));
  nand2  g094(.a(x18), .b(x17), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n138_), .c(new_n134_), .O(new_n140_));
  nand2  g096(.a(x16), .b(x10), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n139_), .c(new_n91_), .O(new_n142_));
  inv1   g098(.a(x11), .O(new_n143_));
  inv1   g099(.a(x16), .O(new_n144_));
  oai21  g100(.a(new_n139_), .b(new_n143_), .c(new_n144_), .O(new_n145_));
  aoi21  g101(.a(x18), .b(x12), .c(x17), .O(new_n146_));
  nor2   g102(.a(x18), .b(x13), .O(new_n147_));
  nor3   g103(.a(new_n147_), .b(new_n146_), .c(new_n129_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n145_), .c(new_n142_), .d(new_n140_), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n137_), .c(new_n56_), .O(new_n150_));
  nor2   g106(.a(x28), .b(x27), .O(new_n151_));
  nor2   g107(.a(new_n151_), .b(new_n100_), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n113_), .c(x20), .O(new_n154_));
  inv1   g110(.a(new_n151_), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(x30), .c(x29), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n154_), .c(new_n150_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x32), .O(new_n158_));
  inv1   g114(.a(x32), .O(new_n159_));
  nand4  g115(.a(new_n155_), .b(new_n113_), .c(x29), .d(x20), .O(new_n160_));
  oai21  g116(.a(new_n152_), .b(new_n113_), .c(new_n160_), .O(new_n161_));
  inv1   g117(.a(x17), .O(new_n162_));
  inv1   g118(.a(x18), .O(new_n163_));
  nand3  g119(.a(x19), .b(new_n163_), .c(x13), .O(new_n164_));
  nand3  g120(.a(new_n134_), .b(x18), .c(x14), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n164_), .c(new_n162_), .O(new_n166_));
  nand4  g122(.a(x19), .b(x18), .c(new_n162_), .d(x12), .O(new_n167_));
  inv1   g123(.a(new_n167_), .O(new_n168_));
  oai21  g124(.a(new_n168_), .b(new_n166_), .c(x16), .O(new_n169_));
  nand3  g125(.a(x19), .b(x18), .c(x17), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n144_), .c(x11), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n169_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(x15), .O(new_n174_));
  nand4  g130(.a(new_n171_), .b(x16), .c(new_n91_), .d(x10), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  nand4  g132(.a(new_n176_), .b(new_n161_), .c(new_n159_), .d(x31), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n158_), .O(new_n178_));
  nand3  g134(.a(new_n178_), .b(new_n79_), .c(x00), .O(new_n179_));
  inv1   g135(.a(new_n179_), .O(z8));
  inv1   g136(.a(x33), .O(new_n181_));
  nand4  g137(.a(new_n176_), .b(new_n181_), .c(x31), .d(x30), .O(new_n182_));
  nand2  g138(.a(x33), .b(new_n113_), .O(new_n183_));
  nand2  g139(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  nand3  g140(.a(new_n184_), .b(new_n155_), .c(x29), .O(new_n185_));
  nand4  g141(.a(new_n176_), .b(new_n181_), .c(new_n129_), .d(new_n113_), .O(new_n186_));
  nand2  g142(.a(x33), .b(x30), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n186_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n153_), .O(new_n189_));
  inv1   g145(.a(new_n146_), .O(new_n190_));
  inv1   g146(.a(new_n147_), .O(new_n191_));
  xnor2a g147(.a(x31), .b(x30), .O(new_n192_));
  nand3  g148(.a(new_n192_), .b(new_n191_), .c(new_n190_), .O(new_n193_));
  inv1   g149(.a(new_n193_), .O(new_n194_));
  nand4  g150(.a(new_n194_), .b(new_n145_), .c(new_n142_), .d(new_n140_), .O(new_n195_));
  oai21  g151(.a(new_n195_), .b(new_n137_), .c(x33), .O(new_n196_));
  nand3  g152(.a(new_n196_), .b(new_n189_), .c(new_n185_), .O(new_n197_));
  nand3  g153(.a(new_n197_), .b(new_n79_), .c(x00), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(new_n56_), .O(z9));
endmodule


