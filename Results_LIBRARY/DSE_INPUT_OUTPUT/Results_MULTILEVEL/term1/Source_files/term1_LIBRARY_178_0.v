// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_;
  nor2   g000(.a(x30), .b(x19), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n49_), .b(new_n52_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n49_), .b(new_n52_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n49_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n58_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  xnor2a g026(.a(x03), .b(x02), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  nand2  g028(.a(new_n66_), .b(new_n64_), .O(new_n73_));
  nand4  g029(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n46_), .c(new_n57_), .d(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x26), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  oai22  g037(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n82_));
  nor2   g038(.a(x22), .b(x17), .O(new_n83_));
  nor2   g039(.a(x23), .b(x18), .O(new_n84_));
  nor3   g040(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n46_), .O(new_n87_));
  inv1   g043(.a(x19), .O(new_n88_));
  inv1   g044(.a(x24), .O(new_n89_));
  nand3  g045(.a(x30), .b(new_n89_), .c(new_n88_), .O(new_n90_));
  aoi21  g046(.a(new_n90_), .b(new_n87_), .c(new_n60_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n81_), .c(new_n80_), .d(x25), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n59_), .O(z3));
  xor2a  g049(.a(x28), .b(x27), .O(new_n94_));
  nor2   g050(.a(x24), .b(x19), .O(new_n95_));
  nor4   g051(.a(new_n95_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(new_n60_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n94_), .c(new_n80_), .d(x25), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n59_), .c(new_n46_), .O(z4));
  nand2  g055(.a(x28), .b(x27), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(x29), .O(new_n101_));
  inv1   g057(.a(x29), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(x28), .c(x27), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n101_), .c(new_n96_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n61_), .c(new_n80_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x25), .c(x01), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n46_), .O(z5));
  inv1   g064(.a(x30), .O(new_n109_));
  nor2   g065(.a(new_n85_), .b(new_n109_), .O(new_n110_));
  oai22  g066(.a(new_n110_), .b(new_n95_), .c(new_n100_), .d(new_n102_), .O(new_n111_));
  nor2   g067(.a(new_n85_), .b(x30), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(x29), .c(x28), .d(x27), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n61_), .c(new_n80_), .d(x25), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n59_), .c(new_n46_), .O(z6));
  oai21  g072(.a(x31), .b(new_n88_), .c(new_n109_), .O(new_n117_));
  nand4  g073(.a(new_n61_), .b(new_n80_), .c(x25), .d(x01), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g075(.a(x29), .b(x28), .c(x27), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x31), .O(new_n121_));
  inv1   g077(.a(new_n84_), .O(new_n122_));
  nor2   g078(.a(new_n83_), .b(new_n82_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x24), .O(new_n126_));
  inv1   g082(.a(x31), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(x29), .c(x28), .d(x27), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n126_), .c(new_n121_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x30), .O(new_n130_));
  nor2   g086(.a(new_n127_), .b(x30), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n125_), .c(x19), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n130_), .c(new_n119_), .O(z7));
  inv1   g089(.a(x18), .O(new_n134_));
  aoi22  g090(.a(x30), .b(new_n88_), .c(new_n134_), .d(x13), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(x17), .c(x16), .d(x15), .O(new_n136_));
  inv1   g092(.a(x28), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n81_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x30), .c(x29), .O(new_n139_));
  nand2  g095(.a(new_n138_), .b(x29), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n109_), .O(new_n141_));
  inv1   g097(.a(x17), .O(new_n142_));
  nor2   g098(.a(new_n88_), .b(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x16), .c(x15), .O(new_n144_));
  inv1   g100(.a(x15), .O(new_n145_));
  nand3  g101(.a(new_n143_), .b(x16), .c(x10), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  inv1   g103(.a(x16), .O(new_n148_));
  nand2  g104(.a(new_n143_), .b(x11), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  aoi21  g106(.a(x19), .b(x12), .c(x17), .O(new_n151_));
  nor2   g107(.a(x19), .b(x14), .O(new_n152_));
  nor3   g108(.a(new_n152_), .b(new_n151_), .c(new_n127_), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n150_), .c(new_n147_), .O(new_n154_));
  aoi21  g110(.a(new_n144_), .b(new_n134_), .c(new_n154_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n141_), .c(new_n139_), .d(new_n136_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x32), .O(new_n157_));
  inv1   g113(.a(x32), .O(new_n158_));
  nand3  g114(.a(new_n88_), .b(x18), .c(x14), .O(new_n159_));
  nand4  g115(.a(x30), .b(x19), .c(new_n134_), .d(x13), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n159_), .c(new_n142_), .O(new_n161_));
  nand2  g117(.a(new_n142_), .b(x12), .O(new_n162_));
  nand3  g118(.a(x30), .b(x19), .c(x18), .O(new_n163_));
  nor2   g119(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  oai21  g120(.a(new_n164_), .b(new_n161_), .c(x16), .O(new_n165_));
  inv1   g121(.a(new_n163_), .O(new_n166_));
  nand4  g122(.a(new_n166_), .b(x17), .c(new_n148_), .d(x11), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n165_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(x15), .O(new_n169_));
  nor2   g125(.a(new_n109_), .b(new_n88_), .O(new_n170_));
  nor2   g126(.a(new_n148_), .b(x15), .O(new_n171_));
  nor2   g127(.a(new_n134_), .b(new_n142_), .O(new_n172_));
  nand4  g128(.a(new_n172_), .b(new_n171_), .c(new_n170_), .d(x10), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n169_), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(new_n140_), .O(new_n175_));
  nand3  g131(.a(x18), .b(new_n142_), .c(x12), .O(new_n176_));
  nand3  g132(.a(new_n134_), .b(x17), .c(x13), .O(new_n177_));
  aoi21  g133(.a(new_n177_), .b(new_n176_), .c(new_n148_), .O(new_n178_));
  nand3  g134(.a(new_n172_), .b(new_n148_), .c(x11), .O(new_n179_));
  inv1   g135(.a(new_n179_), .O(new_n180_));
  oai21  g136(.a(new_n180_), .b(new_n178_), .c(x15), .O(new_n181_));
  nand4  g137(.a(new_n172_), .b(x16), .c(new_n145_), .d(x10), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n138_), .c(new_n109_), .d(x29), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n175_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n158_), .c(x31), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n157_), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n80_), .c(x00), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n46_), .O(z8));
  inv1   g145(.a(x00), .O(new_n190_));
  inv1   g146(.a(x33), .O(new_n191_));
  aoi21  g147(.a(new_n140_), .b(new_n127_), .c(x30), .O(new_n192_));
  nor2   g148(.a(new_n127_), .b(new_n102_), .O(new_n193_));
  aoi21  g149(.a(new_n193_), .b(new_n138_), .c(new_n109_), .O(new_n194_));
  nand2  g150(.a(new_n144_), .b(new_n134_), .O(new_n195_));
  nor2   g151(.a(new_n152_), .b(new_n151_), .O(new_n196_));
  nand4  g152(.a(new_n196_), .b(new_n150_), .c(new_n147_), .d(new_n195_), .O(new_n197_));
  nor3   g153(.a(new_n197_), .b(new_n194_), .c(new_n192_), .O(new_n198_));
  aoi21  g154(.a(new_n198_), .b(new_n136_), .c(new_n191_), .O(new_n199_));
  nand4  g155(.a(new_n174_), .b(new_n138_), .c(x31), .d(x29), .O(new_n200_));
  nand4  g156(.a(new_n183_), .b(new_n140_), .c(new_n127_), .d(new_n109_), .O(new_n201_));
  aoi21  g157(.a(new_n201_), .b(new_n200_), .c(x33), .O(new_n202_));
  oai21  g158(.a(new_n202_), .b(new_n199_), .c(new_n80_), .O(new_n203_));
  oai21  g159(.a(new_n203_), .b(new_n190_), .c(new_n46_), .O(z9));
endmodule


