// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  nor2   g000(.a(x30), .b(x14), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nor2   g002(.a(x32), .b(x03), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  nor2   g004(.a(x33), .b(new_n48_), .O(new_n49_));
  oai21  g005(.a(new_n49_), .b(new_n47_), .c(x02), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  nor2   g007(.a(x32), .b(new_n48_), .O(new_n52_));
  nor2   g008(.a(x33), .b(x03), .O(new_n53_));
  oai21  g009(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z1));
  inv1   g011(.a(new_n45_), .O(new_n56_));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nor2   g014(.a(new_n48_), .b(new_n51_), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n48_), .b(new_n51_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n48_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n51_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n48_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n57_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  xnor2a g025(.a(x03), .b(x02), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand2  g027(.a(new_n65_), .b(new_n63_), .O(new_n72_));
  nand4  g028(.a(x06), .b(x05), .c(new_n48_), .d(new_n51_), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n68_), .c(x09), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n56_), .O(z2));
  inv1   g034(.a(x26), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  oai22  g036(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n81_));
  nor2   g037(.a(x22), .b(x17), .O(new_n82_));
  nor2   g038(.a(x23), .b(x18), .O(new_n83_));
  nor2   g039(.a(x24), .b(x19), .O(new_n84_));
  nor4   g040(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n59_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n80_), .c(new_n79_), .d(x25), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n58_), .c(new_n56_), .O(z3));
  xor2a  g044(.a(x28), .b(x27), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n56_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n79_), .c(x25), .d(x01), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z4));
  inv1   g049(.a(new_n85_), .O(new_n94_));
  inv1   g050(.a(x29), .O(new_n95_));
  and2   g051(.a(x28), .b(x27), .O(new_n96_));
  nand3  g052(.a(new_n95_), .b(x28), .c(x27), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n94_), .c(new_n60_), .d(new_n79_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(x25), .c(x01), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n56_), .O(z5));
  inv1   g058(.a(x30), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(x28), .c(x27), .O(new_n104_));
  oai21  g060(.a(new_n96_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nor2   g061(.a(new_n103_), .b(x29), .O(new_n106_));
  aoi21  g062(.a(new_n105_), .b(x29), .c(new_n106_), .O(new_n107_));
  nor2   g063(.a(new_n107_), .b(new_n85_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n60_), .c(new_n79_), .d(x25), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n58_), .c(new_n56_), .O(z6));
  inv1   g066(.a(x14), .O(new_n111_));
  oai21  g067(.a(x31), .b(new_n111_), .c(new_n103_), .O(new_n112_));
  nor4   g068(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n113_));
  nand4  g069(.a(new_n60_), .b(new_n79_), .c(x25), .d(x01), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(new_n115_));
  nand3  g071(.a(x29), .b(x28), .c(x27), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x31), .O(new_n117_));
  inv1   g073(.a(x31), .O(new_n118_));
  nand3  g074(.a(new_n96_), .b(new_n118_), .c(x29), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nor2   g076(.a(new_n118_), .b(x30), .O(new_n121_));
  aoi22  g077(.a(new_n121_), .b(x14), .c(new_n120_), .d(x30), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n115_), .O(z7));
  nor2   g079(.a(x28), .b(x27), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x29), .O(new_n126_));
  inv1   g082(.a(x32), .O(new_n127_));
  inv1   g083(.a(x17), .O(new_n128_));
  inv1   g084(.a(x18), .O(new_n129_));
  nand3  g085(.a(x19), .b(new_n129_), .c(x13), .O(new_n130_));
  inv1   g086(.a(x19), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x18), .c(x14), .O(new_n132_));
  aoi21  g088(.a(new_n132_), .b(new_n130_), .c(new_n128_), .O(new_n133_));
  nand4  g089(.a(x19), .b(x18), .c(new_n128_), .d(x12), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  inv1   g092(.a(x16), .O(new_n137_));
  nand3  g093(.a(x19), .b(x18), .c(x17), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(x11), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x15), .O(new_n142_));
  inv1   g098(.a(x15), .O(new_n143_));
  nand4  g099(.a(new_n139_), .b(x16), .c(new_n143_), .d(x10), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n127_), .c(x31), .d(x30), .O(new_n146_));
  oai21  g102(.a(new_n127_), .b(x30), .c(new_n146_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n126_), .O(new_n148_));
  nand2  g104(.a(new_n131_), .b(x18), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n130_), .c(new_n128_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n135_), .c(x16), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n140_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x15), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n144_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n127_), .c(x31), .d(new_n103_), .O(new_n155_));
  oai21  g111(.a(new_n127_), .b(new_n103_), .c(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n125_), .c(x29), .O(new_n157_));
  nand2  g113(.a(new_n131_), .b(x14), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(x18), .c(x17), .d(x16), .O(new_n159_));
  nor2   g115(.a(new_n159_), .b(new_n143_), .O(new_n160_));
  nor2   g116(.a(new_n137_), .b(new_n143_), .O(new_n161_));
  nor2   g117(.a(new_n129_), .b(new_n128_), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n161_), .c(x19), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  nand3  g120(.a(new_n162_), .b(x16), .c(x10), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n143_), .O(new_n166_));
  nand2  g122(.a(new_n162_), .b(x11), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n137_), .O(new_n168_));
  aoi21  g124(.a(x18), .b(x12), .c(x17), .O(new_n169_));
  nor2   g125(.a(x18), .b(x13), .O(new_n170_));
  nor3   g126(.a(new_n170_), .b(new_n169_), .c(new_n118_), .O(new_n171_));
  nand4  g127(.a(new_n171_), .b(new_n168_), .c(new_n166_), .d(new_n164_), .O(new_n172_));
  oai21  g128(.a(new_n172_), .b(new_n160_), .c(x32), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(new_n157_), .c(new_n148_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(new_n79_), .c(x00), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n56_), .O(z8));
  inv1   g132(.a(new_n169_), .O(new_n177_));
  aoi21  g133(.a(new_n161_), .b(new_n139_), .c(new_n170_), .O(new_n178_));
  nand4  g134(.a(new_n178_), .b(new_n177_), .c(new_n168_), .d(new_n166_), .O(new_n179_));
  oai21  g135(.a(new_n179_), .b(new_n163_), .c(new_n56_), .O(new_n180_));
  aoi21  g136(.a(new_n126_), .b(new_n118_), .c(x30), .O(new_n181_));
  nand2  g137(.a(new_n162_), .b(x16), .O(new_n182_));
  inv1   g138(.a(new_n182_), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(x15), .c(new_n111_), .O(new_n184_));
  nand4  g140(.a(new_n184_), .b(new_n125_), .c(x31), .d(x29), .O(new_n185_));
  aoi22  g141(.a(new_n185_), .b(x30), .c(new_n181_), .d(x14), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n180_), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(x33), .O(new_n188_));
  inv1   g144(.a(x33), .O(new_n189_));
  nand4  g145(.a(new_n154_), .b(new_n126_), .c(new_n118_), .d(new_n103_), .O(new_n190_));
  nor2   g146(.a(new_n124_), .b(new_n118_), .O(new_n191_));
  nand4  g147(.a(new_n191_), .b(x30), .c(x29), .d(new_n131_), .O(new_n192_));
  nor2   g148(.a(new_n192_), .b(new_n129_), .O(new_n193_));
  nand4  g149(.a(new_n193_), .b(x17), .c(x16), .d(x15), .O(new_n194_));
  aoi21  g150(.a(new_n194_), .b(new_n190_), .c(new_n111_), .O(new_n195_));
  nand3  g151(.a(x18), .b(new_n128_), .c(x12), .O(new_n196_));
  nand3  g152(.a(new_n129_), .b(x17), .c(x13), .O(new_n197_));
  aoi21  g153(.a(new_n197_), .b(new_n196_), .c(new_n137_), .O(new_n198_));
  nand3  g154(.a(new_n162_), .b(new_n137_), .c(x11), .O(new_n199_));
  inv1   g155(.a(new_n199_), .O(new_n200_));
  oai21  g156(.a(new_n200_), .b(new_n198_), .c(x15), .O(new_n201_));
  nand3  g157(.a(new_n183_), .b(new_n143_), .c(x10), .O(new_n202_));
  aoi21  g158(.a(new_n202_), .b(new_n201_), .c(new_n124_), .O(new_n203_));
  nand4  g159(.a(new_n203_), .b(x31), .c(x30), .d(x29), .O(new_n204_));
  nor2   g160(.a(new_n204_), .b(new_n131_), .O(new_n205_));
  oai21  g161(.a(new_n205_), .b(new_n195_), .c(new_n189_), .O(new_n206_));
  nand2  g162(.a(new_n206_), .b(new_n188_), .O(new_n207_));
  nand3  g163(.a(new_n207_), .b(new_n79_), .c(x00), .O(new_n208_));
  inv1   g164(.a(new_n208_), .O(z9));
endmodule


