// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_;
  nor2   g000(.a(x30), .b(x27), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n47_), .c(new_n48_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g011(.a(x03), .b(x02), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n52_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n51_), .c(new_n47_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  inv1   g016(.a(new_n55_), .O(new_n61_));
  inv1   g017(.a(new_n56_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n54_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n53_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n53_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n54_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n59_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  nor2   g026(.a(new_n49_), .b(new_n60_), .O(new_n71_));
  nand2  g027(.a(new_n66_), .b(new_n64_), .O(new_n72_));
  nand3  g028(.a(new_n56_), .b(x06), .c(x05), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n70_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n69_), .c(x09), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n47_), .O(z2));
  inv1   g034(.a(x26), .O(new_n79_));
  oai22  g035(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n80_));
  inv1   g036(.a(x17), .O(new_n81_));
  inv1   g037(.a(x22), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g039(.a(x18), .O(new_n84_));
  inv1   g040(.a(x23), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x19), .O(new_n87_));
  inv1   g043(.a(x24), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n83_), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n80_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n55_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n79_), .c(x25), .d(x01), .O(new_n93_));
  aoi21  g049(.a(new_n93_), .b(x30), .c(x27), .O(z3));
  inv1   g050(.a(x25), .O(new_n95_));
  inv1   g051(.a(x27), .O(new_n96_));
  nand3  g052(.a(x30), .b(x28), .c(new_n96_), .O(new_n97_));
  oai21  g053(.a(x28), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n92_), .c(new_n79_), .O(new_n99_));
  nor3   g055(.a(new_n99_), .b(new_n95_), .c(new_n60_), .O(z4));
  inv1   g056(.a(new_n91_), .O(new_n101_));
  xnor2a g057(.a(x29), .b(x28), .O(new_n102_));
  inv1   g058(.a(x28), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n96_), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x30), .c(x29), .O(new_n106_));
  oai21  g062(.a(new_n102_), .b(new_n96_), .c(new_n106_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n101_), .c(new_n61_), .d(new_n79_), .O(new_n108_));
  nor3   g064(.a(new_n108_), .b(new_n95_), .c(new_n60_), .O(z5));
  inv1   g065(.a(x29), .O(new_n110_));
  xnor2a g066(.a(x30), .b(x28), .O(new_n111_));
  aoi21  g067(.a(x30), .b(new_n110_), .c(new_n96_), .O(new_n112_));
  oai21  g068(.a(new_n111_), .b(new_n110_), .c(new_n112_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n101_), .c(new_n61_), .d(new_n79_), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x25), .c(x01), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n47_), .O(z6));
  inv1   g073(.a(x30), .O(new_n118_));
  oai21  g074(.a(x31), .b(new_n118_), .c(new_n96_), .O(new_n119_));
  nand3  g075(.a(x30), .b(x29), .c(x28), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x31), .O(new_n121_));
  inv1   g077(.a(new_n80_), .O(new_n122_));
  nand4  g078(.a(new_n89_), .b(new_n86_), .c(new_n83_), .d(new_n122_), .O(new_n123_));
  nor2   g079(.a(new_n55_), .b(new_n60_), .O(new_n124_));
  inv1   g080(.a(x31), .O(new_n125_));
  nand4  g081(.a(new_n104_), .b(new_n125_), .c(x30), .d(x29), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n124_), .c(new_n79_), .d(x25), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n123_), .c(new_n121_), .d(new_n119_), .O(z7));
  inv1   g085(.a(x15), .O(new_n130_));
  nand2  g086(.a(new_n87_), .b(x14), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(x18), .c(x17), .d(x16), .O(new_n132_));
  nor2   g088(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand4  g089(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n87_), .O(new_n135_));
  nand2  g091(.a(x18), .b(x17), .O(new_n136_));
  nand2  g092(.a(x16), .b(x10), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n136_), .c(new_n130_), .O(new_n138_));
  inv1   g094(.a(x11), .O(new_n139_));
  inv1   g095(.a(x16), .O(new_n140_));
  oai21  g096(.a(new_n136_), .b(new_n139_), .c(new_n140_), .O(new_n141_));
  nand2  g097(.a(x18), .b(x12), .O(new_n142_));
  inv1   g098(.a(x13), .O(new_n143_));
  nand2  g099(.a(new_n84_), .b(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x31), .O(new_n145_));
  aoi21  g101(.a(new_n142_), .b(new_n81_), .c(new_n145_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n141_), .c(new_n138_), .d(new_n135_), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n133_), .c(new_n47_), .O(new_n148_));
  xnor2a g104(.a(x30), .b(x29), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x27), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n148_), .c(new_n120_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x32), .O(new_n152_));
  nor2   g108(.a(x28), .b(x27), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n110_), .c(x30), .O(new_n154_));
  nand3  g110(.a(new_n118_), .b(x29), .c(x27), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand3  g112(.a(x19), .b(new_n84_), .c(x13), .O(new_n157_));
  nand3  g113(.a(new_n87_), .b(x18), .c(x14), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n157_), .c(new_n81_), .O(new_n159_));
  nand4  g115(.a(x19), .b(x18), .c(new_n81_), .d(x12), .O(new_n160_));
  inv1   g116(.a(new_n160_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n159_), .c(x16), .O(new_n162_));
  nand3  g118(.a(x19), .b(x18), .c(x17), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n140_), .c(x11), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x15), .O(new_n167_));
  nand4  g123(.a(new_n164_), .b(x16), .c(new_n130_), .d(x10), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nand4  g125(.a(new_n169_), .b(new_n156_), .c(new_n48_), .d(x31), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n152_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n79_), .c(x00), .O(new_n172_));
  inv1   g128(.a(new_n172_), .O(z8));
  nand4  g129(.a(x33), .b(new_n103_), .c(new_n79_), .d(x00), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(x30), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n96_), .O(new_n176_));
  inv1   g132(.a(new_n153_), .O(new_n177_));
  nand4  g133(.a(new_n177_), .b(x31), .c(x30), .d(x29), .O(new_n178_));
  nand3  g134(.a(new_n125_), .b(new_n118_), .c(new_n110_), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n178_), .O(new_n180_));
  nand3  g136(.a(new_n180_), .b(new_n169_), .c(new_n52_), .O(new_n181_));
  oai21  g137(.a(new_n125_), .b(new_n110_), .c(x30), .O(new_n182_));
  oai21  g138(.a(x31), .b(x29), .c(new_n118_), .O(new_n183_));
  nand2  g139(.a(new_n142_), .b(new_n81_), .O(new_n184_));
  nand4  g140(.a(new_n144_), .b(new_n184_), .c(new_n141_), .d(new_n138_), .O(new_n185_));
  aoi21  g141(.a(new_n134_), .b(new_n87_), .c(new_n185_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n183_), .c(new_n182_), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n133_), .c(x33), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n181_), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n79_), .c(x00), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(new_n176_), .O(z9));
endmodule


