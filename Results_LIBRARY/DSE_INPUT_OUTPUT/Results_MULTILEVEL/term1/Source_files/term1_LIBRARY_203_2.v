// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_;
  inv1   g000(.a(x29), .O(new_n45_));
  nor2   g001(.a(x30), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(new_n46_), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n48_), .c(new_n49_), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g011(.a(x03), .b(x02), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  or2    g013(.a(new_n57_), .b(x33), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n52_), .c(new_n48_), .O(z1));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n57_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n54_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n53_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n53_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n54_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n60_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  nor2   g025(.a(new_n50_), .b(new_n61_), .O(new_n70_));
  nand2  g026(.a(new_n65_), .b(new_n63_), .O(new_n71_));
  nand3  g027(.a(new_n56_), .b(x06), .c(x05), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n69_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n68_), .c(x09), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n48_), .O(z2));
  inv1   g033(.a(x26), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  oai22  g035(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n80_));
  inv1   g036(.a(x17), .O(new_n81_));
  inv1   g037(.a(x22), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g039(.a(x18), .O(new_n84_));
  inv1   g040(.a(x23), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x19), .O(new_n87_));
  inv1   g043(.a(x24), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n83_), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n80_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n55_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n79_), .c(new_n78_), .d(x25), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n61_), .c(new_n48_), .O(z3));
  xor2a  g050(.a(x28), .b(x27), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n92_), .c(new_n78_), .d(x25), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n61_), .c(new_n48_), .O(z4));
  inv1   g053(.a(new_n55_), .O(new_n98_));
  inv1   g054(.a(new_n91_), .O(new_n99_));
  inv1   g055(.a(x28), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n79_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(x30), .c(x29), .O(new_n103_));
  nand3  g059(.a(new_n45_), .b(x28), .c(x27), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n99_), .c(new_n98_), .d(new_n78_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x25), .c(x01), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n48_), .O(z5));
  nand2  g065(.a(new_n101_), .b(x29), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n99_), .c(new_n98_), .d(x30), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n78_), .c(x25), .d(x01), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(z6));
  inv1   g070(.a(x31), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x28), .c(x27), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x30), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x29), .O(new_n118_));
  nand3  g074(.a(new_n101_), .b(x30), .c(x29), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x31), .O(new_n120_));
  inv1   g076(.a(new_n80_), .O(new_n121_));
  nand4  g077(.a(new_n89_), .b(new_n86_), .c(new_n83_), .d(new_n121_), .O(new_n122_));
  nand4  g078(.a(new_n98_), .b(new_n78_), .c(x25), .d(x01), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n122_), .c(new_n120_), .d(new_n118_), .O(z7));
  inv1   g081(.a(x30), .O(new_n126_));
  nand2  g082(.a(new_n100_), .b(new_n79_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x29), .O(new_n128_));
  nand3  g084(.a(x19), .b(new_n84_), .c(x13), .O(new_n129_));
  nand3  g085(.a(new_n87_), .b(x18), .c(x14), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n129_), .c(new_n81_), .O(new_n131_));
  nand4  g087(.a(x19), .b(x18), .c(new_n81_), .d(x12), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n131_), .c(x16), .O(new_n134_));
  inv1   g090(.a(x16), .O(new_n135_));
  nand3  g091(.a(x19), .b(x18), .c(x17), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(x11), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x15), .O(new_n140_));
  inv1   g096(.a(x15), .O(new_n141_));
  nand4  g097(.a(new_n137_), .b(x16), .c(new_n141_), .d(x10), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n128_), .c(new_n49_), .d(x31), .O(new_n144_));
  nand2  g100(.a(new_n87_), .b(x14), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(x18), .c(x17), .d(x16), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n141_), .O(new_n147_));
  nand2  g103(.a(x16), .b(x15), .O(new_n148_));
  nand2  g104(.a(x18), .b(x17), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n148_), .c(new_n87_), .O(new_n150_));
  nand4  g106(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n141_), .O(new_n152_));
  inv1   g108(.a(x11), .O(new_n153_));
  oai21  g109(.a(new_n149_), .b(new_n153_), .c(new_n135_), .O(new_n154_));
  nand2  g110(.a(x18), .b(x12), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n81_), .O(new_n156_));
  nor2   g112(.a(x18), .b(x13), .O(new_n157_));
  nor3   g113(.a(new_n157_), .b(new_n115_), .c(new_n126_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n156_), .c(new_n154_), .O(new_n159_));
  inv1   g115(.a(new_n159_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n152_), .c(new_n150_), .d(new_n128_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n147_), .c(x32), .O(new_n162_));
  oai21  g118(.a(new_n144_), .b(new_n126_), .c(new_n162_), .O(new_n163_));
  nand3  g119(.a(new_n163_), .b(new_n78_), .c(x00), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n48_), .O(z8));
  aoi21  g121(.a(new_n155_), .b(new_n81_), .c(new_n157_), .O(new_n166_));
  nand4  g122(.a(new_n166_), .b(new_n154_), .c(new_n152_), .d(new_n150_), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n147_), .c(new_n48_), .O(new_n168_));
  nand2  g124(.a(new_n115_), .b(new_n126_), .O(new_n169_));
  nand2  g125(.a(new_n127_), .b(x31), .O(new_n170_));
  aoi22  g126(.a(new_n170_), .b(x30), .c(new_n169_), .d(new_n45_), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(x33), .O(new_n173_));
  nand4  g129(.a(new_n127_), .b(x31), .c(x30), .d(x29), .O(new_n174_));
  oai21  g130(.a(new_n169_), .b(x29), .c(new_n174_), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n143_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(x33), .c(new_n173_), .O(new_n177_));
  nand3  g133(.a(new_n177_), .b(new_n78_), .c(x00), .O(new_n178_));
  inv1   g134(.a(new_n178_), .O(z9));
endmodule


