// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x27), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(new_n46_), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n48_), .c(new_n49_), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g011(.a(x03), .b(x02), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  or2    g013(.a(new_n57_), .b(x33), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n52_), .c(new_n48_), .O(z1));
  inv1   g015(.a(x09), .O(new_n60_));
  xnor2a g016(.a(x07), .b(x04), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nor2   g018(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n54_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n53_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n53_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n54_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n61_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  nor2   g026(.a(new_n50_), .b(new_n62_), .O(new_n71_));
  nand2  g027(.a(new_n66_), .b(new_n64_), .O(new_n72_));
  nand3  g028(.a(new_n56_), .b(x06), .c(x05), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n70_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n69_), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n48_), .c(new_n60_), .d(x08), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(z2));
  inv1   g034(.a(x26), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  oai22  g036(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n81_));
  nor2   g037(.a(x22), .b(x17), .O(new_n82_));
  nor2   g038(.a(x23), .b(x18), .O(new_n83_));
  nor2   g039(.a(x24), .b(x19), .O(new_n84_));
  nor4   g040(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n55_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n45_), .c(new_n80_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n79_), .c(x25), .d(x01), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(z3));
  xor2a  g046(.a(x28), .b(x27), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n86_), .c(new_n79_), .d(x25), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n62_), .c(new_n48_), .O(z4));
  inv1   g049(.a(new_n55_), .O(new_n94_));
  inv1   g050(.a(x28), .O(new_n95_));
  nor2   g051(.a(x29), .b(new_n95_), .O(new_n96_));
  nand2  g052(.a(x30), .b(x29), .O(new_n97_));
  nor2   g053(.a(new_n97_), .b(x28), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n96_), .c(x27), .O(new_n99_));
  nand2  g055(.a(x28), .b(x27), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n45_), .c(x29), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n99_), .c(new_n85_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n94_), .c(new_n79_), .d(x25), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n62_), .O(z5));
  inv1   g060(.a(new_n85_), .O(new_n105_));
  xnor2a g061(.a(x30), .b(x29), .O(new_n106_));
  nand2  g062(.a(x30), .b(new_n95_), .O(new_n107_));
  oai21  g063(.a(new_n106_), .b(new_n95_), .c(new_n107_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n105_), .c(new_n94_), .d(x27), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n79_), .c(x25), .d(x01), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(z6));
  oai21  g068(.a(x31), .b(x30), .c(new_n80_), .O(new_n113_));
  oai21  g069(.a(new_n97_), .b(new_n95_), .c(x31), .O(new_n114_));
  inv1   g070(.a(new_n83_), .O(new_n115_));
  inv1   g071(.a(new_n84_), .O(new_n116_));
  nor2   g072(.a(new_n82_), .b(new_n81_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(new_n118_));
  inv1   g074(.a(x31), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(x30), .c(x29), .d(x28), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n79_), .c(x25), .O(new_n121_));
  nor3   g077(.a(new_n121_), .b(new_n55_), .c(new_n62_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n118_), .c(new_n114_), .d(new_n113_), .O(z7));
  inv1   g079(.a(x15), .O(new_n124_));
  inv1   g080(.a(x19), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x14), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(x18), .c(x17), .d(x16), .O(new_n127_));
  nor2   g083(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand4  g084(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g086(.a(x18), .b(x17), .O(new_n131_));
  nand2  g087(.a(x16), .b(x10), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n131_), .c(new_n124_), .O(new_n133_));
  inv1   g089(.a(x11), .O(new_n134_));
  inv1   g090(.a(x16), .O(new_n135_));
  oai21  g091(.a(new_n131_), .b(new_n134_), .c(new_n135_), .O(new_n136_));
  inv1   g092(.a(x17), .O(new_n137_));
  nand2  g093(.a(x18), .b(x12), .O(new_n138_));
  inv1   g094(.a(x13), .O(new_n139_));
  inv1   g095(.a(x18), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x31), .O(new_n142_));
  aoi21  g098(.a(new_n138_), .b(new_n137_), .c(new_n142_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n136_), .c(new_n133_), .d(new_n130_), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n128_), .c(new_n48_), .O(new_n145_));
  inv1   g101(.a(new_n97_), .O(new_n146_));
  inv1   g102(.a(x29), .O(new_n147_));
  aoi21  g103(.a(new_n95_), .b(new_n80_), .c(new_n147_), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(x30), .O(new_n149_));
  aoi21  g105(.a(new_n146_), .b(x27), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n145_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x32), .O(new_n152_));
  nand3  g108(.a(new_n45_), .b(x29), .c(x28), .O(new_n153_));
  oai21  g109(.a(new_n106_), .b(new_n80_), .c(new_n153_), .O(new_n154_));
  nand3  g110(.a(x19), .b(new_n140_), .c(x13), .O(new_n155_));
  nand3  g111(.a(new_n125_), .b(x18), .c(x14), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n155_), .c(new_n137_), .O(new_n157_));
  nand4  g113(.a(x19), .b(x18), .c(new_n137_), .d(x12), .O(new_n158_));
  inv1   g114(.a(new_n158_), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n157_), .c(x16), .O(new_n160_));
  nand3  g116(.a(x19), .b(x18), .c(x17), .O(new_n161_));
  inv1   g117(.a(new_n161_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(new_n135_), .c(x11), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(x15), .O(new_n165_));
  nand4  g121(.a(new_n162_), .b(x16), .c(new_n124_), .d(x10), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand4  g123(.a(new_n167_), .b(new_n154_), .c(new_n49_), .d(x31), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n152_), .O(new_n169_));
  nand3  g125(.a(new_n169_), .b(new_n79_), .c(x00), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(z8));
  aoi21  g127(.a(new_n163_), .b(new_n160_), .c(new_n124_), .O(new_n172_));
  inv1   g128(.a(new_n166_), .O(new_n173_));
  aoi22  g129(.a(new_n45_), .b(new_n147_), .c(new_n95_), .d(new_n80_), .O(new_n174_));
  nand3  g130(.a(x31), .b(x30), .c(x29), .O(new_n175_));
  oai21  g131(.a(new_n174_), .b(x31), .c(new_n175_), .O(new_n176_));
  oai21  g132(.a(new_n173_), .b(new_n172_), .c(new_n176_), .O(new_n177_));
  oai21  g133(.a(new_n148_), .b(x31), .c(new_n45_), .O(new_n178_));
  oai21  g134(.a(new_n119_), .b(new_n147_), .c(x30), .O(new_n179_));
  nand2  g135(.a(new_n138_), .b(new_n137_), .O(new_n180_));
  nand4  g136(.a(new_n141_), .b(new_n180_), .c(new_n136_), .d(new_n133_), .O(new_n181_));
  aoi21  g137(.a(new_n129_), .b(new_n125_), .c(new_n181_), .O(new_n182_));
  nand3  g138(.a(new_n182_), .b(new_n179_), .c(new_n178_), .O(new_n183_));
  oai21  g139(.a(new_n183_), .b(new_n128_), .c(x33), .O(new_n184_));
  oai21  g140(.a(new_n177_), .b(x33), .c(new_n184_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n79_), .c(x00), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n48_), .O(z9));
endmodule


