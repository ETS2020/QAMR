// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  xnor2a g006(.a(x03), .b(x02), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n49_), .c(new_n50_), .O(new_n53_));
  inv1   g009(.a(x33), .O(new_n54_));
  inv1   g010(.a(x02), .O(new_n55_));
  inv1   g011(.a(x03), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g013(.a(x03), .b(x02), .O(new_n58_));
  oai21  g014(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(new_n53_), .c(new_n49_), .O(z1));
  xnor2a g016(.a(x07), .b(x04), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  inv1   g018(.a(new_n57_), .O(new_n63_));
  inv1   g019(.a(new_n58_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  nand2  g021(.a(x05), .b(new_n56_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x06), .c(new_n55_), .O(new_n67_));
  nand2  g023(.a(x06), .b(new_n55_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(x05), .c(new_n56_), .O(new_n69_));
  aoi21  g025(.a(new_n69_), .b(new_n67_), .c(x01), .O(new_n70_));
  oai21  g026(.a(new_n70_), .b(new_n65_), .c(new_n61_), .O(new_n71_));
  xor2a  g027(.a(x07), .b(x04), .O(new_n72_));
  nor2   g028(.a(new_n51_), .b(new_n62_), .O(new_n73_));
  nand2  g029(.a(new_n68_), .b(new_n66_), .O(new_n74_));
  nand3  g030(.a(new_n58_), .b(x06), .c(x05), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n72_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n71_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n49_), .O(z2));
  inv1   g036(.a(x25), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  inv1   g039(.a(x17), .O(new_n84_));
  inv1   g040(.a(x22), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x18), .O(new_n87_));
  inv1   g043(.a(x23), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g045(.a(x19), .O(new_n90_));
  inv1   g046(.a(x24), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n89_), .c(new_n86_), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n83_), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n57_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n82_), .c(new_n45_), .O(new_n96_));
  nor3   g052(.a(new_n96_), .b(new_n81_), .c(new_n62_), .O(z3));
  xor2a  g053(.a(x28), .b(x27), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n95_), .c(new_n45_), .d(x25), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n62_), .c(new_n49_), .O(z4));
  inv1   g056(.a(new_n94_), .O(new_n101_));
  inv1   g057(.a(x28), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n82_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  nand2  g060(.a(new_n46_), .b(x26), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n104_), .c(new_n101_), .d(x29), .O(new_n106_));
  inv1   g062(.a(x29), .O(new_n107_));
  nand4  g063(.a(new_n101_), .b(new_n107_), .c(x28), .d(x27), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(x26), .c(new_n106_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n63_), .c(x25), .d(x01), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n49_), .O(z5));
  nand4  g067(.a(new_n46_), .b(x28), .c(x27), .d(new_n45_), .O(new_n112_));
  oai21  g068(.a(new_n103_), .b(new_n46_), .c(new_n112_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x29), .O(new_n114_));
  oai21  g070(.a(new_n46_), .b(x29), .c(new_n114_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n101_), .c(new_n63_), .d(x25), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n62_), .c(new_n49_), .O(z6));
  oai21  g073(.a(x31), .b(x26), .c(new_n46_), .O(new_n118_));
  nand3  g074(.a(x29), .b(x28), .c(x27), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x31), .O(new_n120_));
  inv1   g076(.a(new_n83_), .O(new_n121_));
  nand4  g077(.a(new_n92_), .b(new_n89_), .c(new_n86_), .d(new_n121_), .O(new_n122_));
  nor2   g078(.a(new_n57_), .b(new_n62_), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x30), .c(x29), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n103_), .c(new_n81_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n123_), .c(new_n122_), .d(new_n120_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n45_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n118_), .O(z7));
  inv1   g086(.a(x15), .O(new_n131_));
  nand2  g087(.a(new_n90_), .b(x14), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(x18), .c(x17), .d(x16), .O(new_n133_));
  nor2   g089(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g090(.a(x16), .b(x15), .O(new_n135_));
  nand2  g091(.a(x18), .b(x17), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n135_), .c(new_n90_), .O(new_n137_));
  nand2  g093(.a(x16), .b(x10), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(new_n131_), .O(new_n139_));
  inv1   g095(.a(x11), .O(new_n140_));
  inv1   g096(.a(x16), .O(new_n141_));
  oai21  g097(.a(new_n136_), .b(new_n140_), .c(new_n141_), .O(new_n142_));
  aoi21  g098(.a(x18), .b(x12), .c(x17), .O(new_n143_));
  nor2   g099(.a(x18), .b(x13), .O(new_n144_));
  nor3   g100(.a(new_n144_), .b(new_n143_), .c(new_n124_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n142_), .c(new_n139_), .d(new_n137_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n134_), .c(new_n105_), .O(new_n147_));
  nor2   g103(.a(x28), .b(x27), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n107_), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n46_), .c(new_n45_), .O(new_n151_));
  inv1   g107(.a(new_n148_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(x30), .c(x29), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n151_), .c(new_n147_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(x32), .O(new_n155_));
  nand4  g111(.a(new_n152_), .b(new_n46_), .c(x29), .d(new_n45_), .O(new_n156_));
  oai21  g112(.a(new_n149_), .b(new_n46_), .c(new_n156_), .O(new_n157_));
  nand3  g113(.a(x19), .b(new_n87_), .c(x13), .O(new_n158_));
  nand3  g114(.a(new_n90_), .b(x18), .c(x14), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n158_), .c(new_n84_), .O(new_n160_));
  nand4  g116(.a(x19), .b(x18), .c(new_n84_), .d(x12), .O(new_n161_));
  inv1   g117(.a(new_n161_), .O(new_n162_));
  oai21  g118(.a(new_n162_), .b(new_n160_), .c(x16), .O(new_n163_));
  nand3  g119(.a(x19), .b(x18), .c(x17), .O(new_n164_));
  inv1   g120(.a(new_n164_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n141_), .c(x11), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n163_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(x15), .O(new_n168_));
  nand4  g124(.a(new_n165_), .b(x16), .c(new_n131_), .d(x10), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(new_n157_), .c(new_n50_), .d(x31), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n155_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(x00), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n49_), .O(z8));
  nand4  g130(.a(new_n170_), .b(new_n54_), .c(x31), .d(x30), .O(new_n175_));
  nand2  g131(.a(x33), .b(new_n46_), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  nand3  g133(.a(new_n177_), .b(new_n152_), .c(x29), .O(new_n178_));
  nand4  g134(.a(new_n170_), .b(new_n54_), .c(new_n124_), .d(new_n46_), .O(new_n179_));
  nand2  g135(.a(x33), .b(x30), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n179_), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n150_), .O(new_n182_));
  inv1   g138(.a(new_n143_), .O(new_n183_));
  inv1   g139(.a(new_n144_), .O(new_n184_));
  xnor2a g140(.a(x31), .b(x30), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n184_), .c(new_n183_), .O(new_n186_));
  inv1   g142(.a(new_n186_), .O(new_n187_));
  nand4  g143(.a(new_n187_), .b(new_n142_), .c(new_n139_), .d(new_n137_), .O(new_n188_));
  oai21  g144(.a(new_n188_), .b(new_n134_), .c(x33), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n182_), .c(new_n178_), .O(new_n190_));
  nand3  g146(.a(new_n190_), .b(new_n45_), .c(x00), .O(new_n191_));
  inv1   g147(.a(new_n191_), .O(z9));
endmodule


