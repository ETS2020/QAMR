// Benchmark "FAU" written by ABC on Fri Aug 14 02:51:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_;
  inv1   g000(.a(x30), .O(new_n45_));
  nand2  g001(.a(x33), .b(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  nand3  g005(.a(new_n49_), .b(x30), .c(new_n48_), .O(new_n50_));
  oai21  g006(.a(x33), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nand3  g009(.a(new_n49_), .b(x30), .c(x03), .O(new_n54_));
  oai21  g010(.a(x33), .b(x03), .c(new_n54_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  inv1   g012(.a(x33), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n56_), .c(new_n52_), .O(z1));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n48_), .b(new_n53_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(new_n63_));
  nand2  g019(.a(new_n48_), .b(new_n53_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(new_n65_));
  nand2  g021(.a(x05), .b(new_n48_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x06), .c(new_n53_), .O(new_n67_));
  nand2  g023(.a(x06), .b(new_n53_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(x05), .c(new_n48_), .O(new_n69_));
  aoi21  g025(.a(new_n69_), .b(new_n67_), .c(x01), .O(new_n70_));
  oai21  g026(.a(new_n70_), .b(new_n65_), .c(new_n60_), .O(new_n71_));
  xor2a  g027(.a(x07), .b(x04), .O(new_n72_));
  xnor2a g028(.a(x03), .b(x02), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n61_), .O(new_n74_));
  nand2  g030(.a(new_n68_), .b(new_n66_), .O(new_n75_));
  nand4  g031(.a(x06), .b(x05), .c(new_n48_), .d(new_n53_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  aoi21  g034(.a(new_n78_), .b(new_n71_), .c(x09), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(x08), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n46_), .O(z2));
  inv1   g037(.a(x26), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  oai22  g039(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nor2   g041(.a(x22), .b(x17), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nor2   g043(.a(x23), .b(x18), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nor2   g045(.a(x24), .b(x19), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n89_), .c(new_n87_), .d(new_n85_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n62_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n83_), .c(new_n82_), .d(x25), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n61_), .c(new_n46_), .O(z3));
  xor2a  g052(.a(x28), .b(x27), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n94_), .c(new_n82_), .d(x25), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n61_), .c(new_n46_), .O(z4));
  inv1   g055(.a(x29), .O(new_n100_));
  inv1   g056(.a(x28), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n83_), .O(new_n102_));
  nand3  g058(.a(new_n100_), .b(x28), .c(x27), .O(new_n103_));
  oai21  g059(.a(new_n102_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n92_), .c(new_n63_), .d(new_n82_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x25), .c(x01), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n46_), .O(z5));
  nand3  g064(.a(new_n45_), .b(x28), .c(x27), .O(new_n109_));
  oai21  g065(.a(new_n102_), .b(new_n45_), .c(new_n109_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x29), .O(new_n111_));
  nand2  g067(.a(x30), .b(new_n100_), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n111_), .c(new_n93_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n63_), .c(new_n82_), .d(x25), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n61_), .c(new_n46_), .O(z6));
  oai21  g071(.a(x33), .b(x31), .c(new_n45_), .O(new_n116_));
  nand3  g072(.a(x29), .b(x28), .c(x27), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x31), .O(new_n118_));
  nand4  g074(.a(new_n91_), .b(new_n89_), .c(new_n87_), .d(new_n85_), .O(new_n119_));
  nor2   g075(.a(new_n62_), .b(new_n61_), .O(new_n120_));
  inv1   g076(.a(x31), .O(new_n121_));
  nand4  g077(.a(new_n102_), .b(new_n121_), .c(x30), .d(x29), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n120_), .c(new_n82_), .d(x25), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n119_), .c(new_n118_), .d(new_n116_), .O(z7));
  inv1   g081(.a(x15), .O(new_n126_));
  inv1   g082(.a(x18), .O(new_n127_));
  inv1   g083(.a(x19), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(x14), .c(new_n127_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x17), .c(x16), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand2  g087(.a(x16), .b(x15), .O(new_n132_));
  nand2  g088(.a(x18), .b(x17), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n132_), .c(new_n128_), .O(new_n134_));
  nand2  g090(.a(x16), .b(x10), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n133_), .c(new_n126_), .O(new_n136_));
  inv1   g092(.a(x11), .O(new_n137_));
  inv1   g093(.a(x16), .O(new_n138_));
  oai21  g094(.a(new_n133_), .b(new_n137_), .c(new_n138_), .O(new_n139_));
  inv1   g095(.a(x17), .O(new_n140_));
  nand2  g096(.a(x18), .b(x12), .O(new_n141_));
  inv1   g097(.a(x13), .O(new_n142_));
  nand2  g098(.a(new_n127_), .b(new_n142_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x31), .O(new_n144_));
  aoi21  g100(.a(new_n141_), .b(new_n140_), .c(new_n144_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n139_), .c(new_n136_), .d(new_n134_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n131_), .c(new_n46_), .O(new_n147_));
  nand2  g103(.a(new_n101_), .b(new_n83_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x30), .c(x29), .O(new_n149_));
  nand2  g105(.a(new_n148_), .b(x29), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n57_), .c(new_n45_), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n149_), .c(new_n147_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x32), .O(new_n153_));
  nand4  g109(.a(new_n148_), .b(new_n57_), .c(new_n45_), .d(x29), .O(new_n154_));
  nand2  g110(.a(new_n150_), .b(x30), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand3  g112(.a(x19), .b(new_n127_), .c(x13), .O(new_n157_));
  nand3  g113(.a(new_n128_), .b(x18), .c(x14), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n157_), .c(new_n140_), .O(new_n159_));
  nand4  g115(.a(x19), .b(x18), .c(new_n140_), .d(x12), .O(new_n160_));
  inv1   g116(.a(new_n160_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n159_), .c(x16), .O(new_n162_));
  nand3  g118(.a(x19), .b(x18), .c(x17), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n138_), .c(x11), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x15), .O(new_n167_));
  nand4  g123(.a(new_n164_), .b(x16), .c(new_n126_), .d(x10), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nand4  g125(.a(new_n169_), .b(new_n156_), .c(new_n49_), .d(x31), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n153_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n82_), .c(x00), .O(new_n172_));
  inv1   g128(.a(new_n172_), .O(z8));
  nand4  g129(.a(new_n148_), .b(x31), .c(x30), .d(x29), .O(new_n174_));
  nand3  g130(.a(new_n150_), .b(new_n121_), .c(new_n45_), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  nand3  g132(.a(new_n176_), .b(new_n169_), .c(new_n57_), .O(new_n177_));
  inv1   g133(.a(new_n131_), .O(new_n178_));
  nand2  g134(.a(new_n141_), .b(new_n140_), .O(new_n179_));
  nor2   g135(.a(new_n121_), .b(new_n100_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n148_), .c(new_n143_), .d(new_n179_), .O(new_n181_));
  inv1   g137(.a(new_n181_), .O(new_n182_));
  and2   g138(.a(new_n182_), .b(new_n139_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n136_), .c(new_n134_), .d(new_n178_), .O(new_n184_));
  nand3  g140(.a(new_n184_), .b(x33), .c(x30), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n177_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n82_), .c(x00), .O(new_n187_));
  inv1   g143(.a(new_n187_), .O(z9));
endmodule


