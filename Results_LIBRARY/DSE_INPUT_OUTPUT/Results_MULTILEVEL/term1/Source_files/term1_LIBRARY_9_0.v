// Benchmark "FAU" written by ABC on Fri Aug 14 02:50:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_;
  nor2   g000(.a(x26), .b(x07), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n47_), .c(new_n48_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g011(.a(x03), .b(x02), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n52_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n51_), .c(new_n47_), .O(z1));
  nand2  g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x07), .O(new_n60_));
  nand2  g016(.a(x26), .b(new_n60_), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(x04), .c(new_n59_), .O(new_n62_));
  inv1   g018(.a(x01), .O(new_n63_));
  inv1   g019(.a(new_n55_), .O(new_n64_));
  inv1   g020(.a(new_n56_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nand2  g022(.a(x05), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x06), .c(new_n53_), .O(new_n68_));
  nand2  g024(.a(x06), .b(new_n53_), .O(new_n69_));
  nand3  g025(.a(new_n69_), .b(x05), .c(new_n54_), .O(new_n70_));
  aoi21  g026(.a(new_n70_), .b(new_n68_), .c(x01), .O(new_n71_));
  oai21  g027(.a(new_n71_), .b(new_n66_), .c(new_n62_), .O(new_n72_));
  inv1   g028(.a(x04), .O(new_n73_));
  nand2  g029(.a(x07), .b(new_n73_), .O(new_n74_));
  oai21  g030(.a(new_n61_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nor2   g031(.a(new_n49_), .b(new_n63_), .O(new_n76_));
  nand2  g032(.a(new_n69_), .b(new_n67_), .O(new_n77_));
  nand3  g033(.a(new_n56_), .b(x06), .c(x05), .O(new_n78_));
  aoi21  g034(.a(new_n78_), .b(new_n77_), .c(x01), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n76_), .c(new_n75_), .O(new_n80_));
  aoi21  g036(.a(new_n80_), .b(new_n72_), .c(x09), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(x08), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(z2));
  inv1   g039(.a(x27), .O(new_n84_));
  nor2   g040(.a(x20), .b(x15), .O(new_n85_));
  nor2   g041(.a(x21), .b(x16), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nor2   g043(.a(x22), .b(x17), .O(new_n88_));
  nor2   g044(.a(x23), .b(x18), .O(new_n89_));
  nor2   g045(.a(x24), .b(x19), .O(new_n90_));
  nor3   g046(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n55_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n84_), .c(x25), .d(x01), .O(new_n95_));
  aoi21  g051(.a(new_n95_), .b(x07), .c(x26), .O(z3));
  inv1   g052(.a(x26), .O(new_n97_));
  xor2a  g053(.a(x28), .b(x27), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n94_), .c(new_n97_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(x25), .c(x07), .d(x01), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(z4));
  inv1   g058(.a(x28), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n84_), .c(x29), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x28), .c(x27), .O(new_n106_));
  aoi21  g062(.a(new_n106_), .b(new_n104_), .c(new_n93_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n64_), .c(x25), .d(x01), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(x07), .c(x26), .O(z5));
  nand3  g065(.a(x29), .b(x28), .c(x27), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x30), .O(new_n111_));
  inv1   g067(.a(x30), .O(new_n112_));
  nor2   g068(.a(new_n103_), .b(new_n84_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n112_), .c(x29), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n111_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n92_), .c(new_n64_), .d(new_n97_), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(x25), .c(x07), .d(x01), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(z6));
  nand3  g075(.a(new_n113_), .b(x30), .c(x29), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x31), .O(new_n121_));
  inv1   g077(.a(new_n88_), .O(new_n122_));
  inv1   g078(.a(new_n89_), .O(new_n123_));
  inv1   g079(.a(new_n90_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n123_), .c(new_n122_), .d(new_n87_), .O(new_n125_));
  nor2   g081(.a(new_n55_), .b(new_n63_), .O(new_n126_));
  inv1   g082(.a(x25), .O(new_n127_));
  inv1   g083(.a(x31), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x30), .c(x29), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n113_), .c(new_n127_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n126_), .c(new_n125_), .d(new_n121_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x07), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n97_), .O(z7));
  nand2  g090(.a(new_n103_), .b(new_n84_), .O(new_n135_));
  inv1   g091(.a(x17), .O(new_n136_));
  inv1   g092(.a(x18), .O(new_n137_));
  nand3  g093(.a(x19), .b(new_n137_), .c(x13), .O(new_n138_));
  inv1   g094(.a(x19), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x18), .c(x14), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n138_), .c(new_n136_), .O(new_n141_));
  nand4  g097(.a(x19), .b(x18), .c(new_n136_), .d(x12), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n141_), .c(x16), .O(new_n144_));
  inv1   g100(.a(x16), .O(new_n145_));
  nand3  g101(.a(x19), .b(x18), .c(x17), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n145_), .c(x11), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x15), .O(new_n150_));
  inv1   g106(.a(x15), .O(new_n151_));
  nand4  g107(.a(new_n147_), .b(x16), .c(new_n151_), .d(x10), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n48_), .c(x31), .O(new_n154_));
  nand2  g110(.a(x32), .b(x30), .O(new_n155_));
  oai21  g111(.a(new_n154_), .b(x30), .c(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n135_), .c(x29), .O(new_n157_));
  nand2  g113(.a(new_n135_), .b(x29), .O(new_n158_));
  nand2  g114(.a(x32), .b(new_n112_), .O(new_n159_));
  oai21  g115(.a(new_n154_), .b(new_n112_), .c(new_n159_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  nand2  g117(.a(new_n139_), .b(x14), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(x18), .c(x17), .d(x16), .O(new_n163_));
  nor2   g119(.a(new_n163_), .b(new_n151_), .O(new_n164_));
  nand2  g120(.a(x16), .b(x15), .O(new_n165_));
  nand2  g121(.a(x18), .b(x17), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n165_), .c(new_n139_), .O(new_n167_));
  nand2  g123(.a(x16), .b(x10), .O(new_n168_));
  oai21  g124(.a(new_n168_), .b(new_n166_), .c(new_n151_), .O(new_n169_));
  inv1   g125(.a(x11), .O(new_n170_));
  oai21  g126(.a(new_n166_), .b(new_n170_), .c(new_n145_), .O(new_n171_));
  aoi21  g127(.a(x18), .b(x12), .c(x17), .O(new_n172_));
  nor2   g128(.a(x18), .b(x13), .O(new_n173_));
  nor3   g129(.a(new_n173_), .b(new_n172_), .c(new_n128_), .O(new_n174_));
  nand4  g130(.a(new_n174_), .b(new_n171_), .c(new_n169_), .d(new_n167_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n164_), .c(x32), .O(new_n176_));
  nand3  g132(.a(new_n176_), .b(new_n161_), .c(new_n157_), .O(new_n177_));
  nand4  g133(.a(new_n177_), .b(new_n97_), .c(x07), .d(x00), .O(new_n178_));
  inv1   g134(.a(new_n178_), .O(z8));
  nand4  g135(.a(new_n153_), .b(new_n52_), .c(x31), .d(x30), .O(new_n180_));
  nand2  g136(.a(x33), .b(new_n112_), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  nand3  g138(.a(new_n182_), .b(new_n135_), .c(x29), .O(new_n183_));
  nand4  g139(.a(new_n153_), .b(new_n52_), .c(new_n128_), .d(new_n112_), .O(new_n184_));
  nand2  g140(.a(x33), .b(x30), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n184_), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n158_), .O(new_n187_));
  inv1   g143(.a(new_n172_), .O(new_n188_));
  inv1   g144(.a(new_n173_), .O(new_n189_));
  xnor2a g145(.a(x31), .b(x30), .O(new_n190_));
  nand3  g146(.a(new_n190_), .b(new_n189_), .c(new_n188_), .O(new_n191_));
  inv1   g147(.a(new_n191_), .O(new_n192_));
  nand4  g148(.a(new_n192_), .b(new_n171_), .c(new_n169_), .d(new_n167_), .O(new_n193_));
  oai21  g149(.a(new_n193_), .b(new_n164_), .c(x33), .O(new_n194_));
  nand3  g150(.a(new_n194_), .b(new_n187_), .c(new_n183_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(x00), .O(new_n196_));
  aoi21  g152(.a(new_n196_), .b(x07), .c(x26), .O(z9));
endmodule


